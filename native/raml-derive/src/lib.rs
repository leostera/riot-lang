...
fn derive_struct_value(input: &DeriveInput, data_struct: &syn::DataStruct) -> TokenStream {
    ...
    let into_impl = quote! {
        impl Into<raml_core::Value> for #name {
            fn into(self) -> raml_core::Value {
                unsafe {
                    let layout = std::alloc::Layout::from_size_align_unchecked(
                        std::mem::size_of::<raml_core::BlockHeader>() + 
                        #field_count * std::mem::size_of::<raml_core::Value>(),
                        std::mem::align_of::<raml_core::BlockHeader>()
                    );
                    
                    let ptr = std::alloc::alloc(layout) as *mut raml_core::Block;
                    
                    let header = raml_core::BlockHeader::new(
                        #field_count,
                        0,
                        raml_core::GcColor::White
                    );
                    
                    std::ptr::write(
                        ptr as *mut raml_core::BlockHeader,
                        header
                    );
                    ...
                    raml_core::Value::from_block_ptr(ptr)
                }
            }
        }
    };
    ...
    let try_from_impl = quote! {
        impl TryFrom<raml_core::Value> for #name {
...
fn derive_enum_value(input: &DeriveInput, data_enum: &syn::DataEnum) -> TokenStream {
    ...
    let into_arms = variants.iter().enumerate().map(|(idx, variant)| {
        ...
        match &variant.fields {
            Fields::Unit => {
                quote! {
                    #name::#variant_name => raml_core::Value::int(#idx as isize),
                }
            }
