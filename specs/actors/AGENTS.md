...
This directory contains the readable formal model for `packages/actors`.

## Source Map

- `packages/actors/src/scheduler.ml`: receive semantics, timer handling, lifecycle transitions, worker queue behavior
- `packages/actors/src/process.ml`: mailbox/save-queue semantics, process state, links, and monitors
- `packages/actors/src/timer.ml` and `packages/actors/src/timer_wheel.ml`: timer identity, expiration, and interval rescheduling
- `packages/actors/tests/*.ml`: executable expectations that should stay aligned with the modeled design intent
