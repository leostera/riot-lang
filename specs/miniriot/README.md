...
- `MiniriotCommon.tla`: shared enums, message constructors, and pure helper operators used by the integration model and the slices.
- `MiniriotRuntime.tla`: the integration state machine for queue ownership, receive, timers, lifecycle, and syscall interactions.
- `MiniriotRuntime.cfg`: a tiny TLC smoke model for the integration spec.
...
- `MiniriotRuntime.cfg` uses `TLCGet("level") < 5`.
