...
6. Keep no-flag `tusk fmt` as the in-place rewrite path. With explicit positional paths, format only those files/directories; without them, keep the existing workspace-scan behavior.
...
`timeout 30 tusk test tusk-fmt:fmt_tests`
