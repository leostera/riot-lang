...
6. Keep `--json` machine-readable, line-delimited, timestamped, and incrementally emitted as `start`/`file`/`summary` events.
7. Do not reintroduce an `ocamlformat` dependency here; `krasny` is the formatter backend.
