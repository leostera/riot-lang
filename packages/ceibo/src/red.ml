        ...
        let child =
          ...
          | Green.Token token ->
              Token {
                green_token = token;
                parent = Some node;
                offset = !running_offset + List.fold_left
...
let rec to_json = fun ~kind_to_json ~text_to_json elem ->
  ...
  | Token tok ->
      Data.Json.Object [
        ...
        (
          "leading_trivia",
          Data.Json.Array
            (List.map (syntax_trivia_to_json ~kind_to_json ~text_to_json)
  ...
  | Node node ->
      Data.Json.Object [
        ...
        (
          "children",
          Data.Json.Array (Array.to_list (Array.map (to_json ~kind_to_json ~text_to_json) (SyntaxNode.children node)))
        )
      ]
