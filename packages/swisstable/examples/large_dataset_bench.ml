...
let bench_hashmap_remove_from_1m = fun () ->
  let map = HashMap.create () in
  for i = 0 to 999_999 do ignore (HashMap.insert map ~key:i ~value:(i * 2)) done;
  for i = 0 to 9_999 do ignore (HashMap.remove map ~key:(i * 100)) done
...
let bench_hashmap_insert_500k = fun () ->
  let map = HashMap.create () in
  for i = 0 to 499_999 do ignore (HashMap.insert map ~key:i ~value:(i * 2)) done

(* Swisstable: Insert 500k items *)
let bench_swisstable_insert_500k = fun () ->
  let map = Swisstable.with_capacity 500_000 in
  for i = 0 to 499_999 do ignore (Swisstable.insert map i (i * 2)) done

(* HashMap: Get from 500k items *)
let bench_hashmap_get_from_500k = fun () ->
  let map = HashMap.create () in
  for i = 0 to 499_999 do ignore (HashMap.insert map ~key:i ~value:(i * 2)) done;
  for i = 0 to 9_999 do ignore (HashMap.get map ~key:(i * 50)) done
...
let benchmarks = Bench.[
  ...
  compare_with_config ~config:xlarge_config "insert 1m items" [ make_case "HashMap" bench_hashmap_insert_1m; make_case "Swisstable" bench_swisstable_insert_1m ];
  compare_with_config ~config:xlarge_config "get from 1m items (10k lookups)" [ make_case "HashMap" bench_hashmap_get_from_1m; make_case "Swisstable" bench_swisstable_get_from_1m ];
  compare_with_config ~config:xlarge_config "get missing keys from 1m items" [ make_case "HashMap" bench_hashmap_get_missing_from_1m; make_case "Swisstable" bench_swisstable_get_missing_from_1m ];
  compare_with_config ~config:xlarge_config "iterate over 1m items" [ make_case "HashMap" bench_hashmap_iterate_1m; make_case "Swisstable" bench_swisstable_iterate_1m ];
  compare_with_config ~config:xlarge_config "remove from 1m items (10k removals)" [ make_case "HashMap" bench_hashmap_remove_from_1m; make_case "Swisstable" bench_swisstable_remove_from_1m ];
]

let main ~args =
  println "HashMap vs Swisstable - Large Dataset Performance\n";
  Bench.Cli.main ~name:"HashMap vs Swisstable - Large Datasets" ~benchmarks ~args
