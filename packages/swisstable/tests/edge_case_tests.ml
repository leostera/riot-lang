    ...
    Test.case "hash collisions"
      (
        fun _ctx ->
          ...
          for i = 0 to 19 do Test.assert_equal ~expected:(Some (i * 10)) ~actual:(Swisstable.get map i) done;
          Ok ()
      );
    ...
    Test.case "fill/empty/refill"
      (
        fun _ctx ->
          ...
          (* Verify new values *)
          for i = 0 to 49 do
            let key = Collections.Array.get_unchecked keys ~at:i in Test.assert_equal ~expected:(Some (i + 100)) ~actual:(Swisstable.get map key)
          done;
          Ok ()
      );
    Test.case "overwrite existing"
      (
        fun _ctx ->
          ...
          for i = 0 to 19 do
            let key = Collections.Array.get_unchecked keys ~at:i in
            let old = Swisstable.insert map key (i * 2) in Test.assert_equal ~expected:(Some i) ~actual:old
          done;
          Test.assert_equal ~expected:20 ~actual:(Swisstable.len map);
          (* Verify new values *)
          for i = 0 to 19 do
            let key = Collections.Array.get_unchecked keys ~at:i in Test.assert_equal ~expected:(Some (i * 2)) ~actual:(Swisstable.get map key)
          done;
          Ok ()
      );
    Test.case "sparse removal"
      (
        fun _ctx ->
          ...
          for i = 0 to 33 do
            let key = Collections.Array.get_unchecked keys ~at:(i * 3) in
            let _ = Swisstable.remove map key in ()
          done;
          ...
          for i = 0 to 99 do
            ...
            if i mod 3 = 0 && i / 3 <= 33 then
              Test.assert_equal ~expected:None ~actual:(Swisstable.get map key)
            else Test.assert_equal ~expected:(Some i) ~actual:(Swisstable.get map key)
          done;
          Ok ()
      );
