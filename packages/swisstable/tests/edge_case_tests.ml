...
let tests = [
  ...
  Test.case "remove/reinsert cycle" (fun () ->
    let map = Swisstable.create () in
  ...
  Test.case "fill/empty/refill" (fun () ->
    let map = Swisstable.create () in
  ...
  Test.case "overwrite existing" (fun () ->
    let map = Swisstable.create () in
  ...
  Test.case "grow then shrink" (fun () ->
    let map = Swisstable.create () in
  ...
  Test.case "interleaved operations" (fun () ->
    let map = Swisstable.create () in
  ...
  Test.case "entry API patterns" (fun () ->
    let map = Swisstable.create () in
...
let () =
  Miniriot.run ~main:(fun ~args:_ ->
      Test.Cli.main ~name:"swisstable:edge_case" ~tests ~args:Env.args)
    ~args:Env.args ()
