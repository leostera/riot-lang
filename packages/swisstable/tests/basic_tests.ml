...
let tests = [
  Test.case "create empty map" (fun () ->
  ...
  Test.case "overwrite existing key" (fun () ->
    let map = Swisstable.create () in
  ...
  Test.case "multiple inserts" (fun () ->
    let map = Swisstable.create () in
  ...
  Test.case "remove non-existent key" (fun () ->
    let map = Swisstable.create () in
  ...
  Test.case "of_list with duplicates" (fun () ->
  ...
  Test.case "resize with many elements" (fun () ->
    let map = Swisstable.create () in
    (* Pre-create keys to ensure consistent hashing *)
    let keys = Collections.Array.init 100 (fun i -> "key" ^ string_of_int i) in
    for i = 0 to 99 do
  ...
  Test.case "insert after remove" (fun () ->
    let map = Swisstable.create () in
    let _ = Swisstable.insert map "a" 1 in
    let _ = Swisstable.insert map "b" 2 in
...
let () =
  Miniriot.run ~main:(fun ~args:_ ->
      Test.Cli.main ~name:"swisstable:basic" ~tests ~args:Env.args)
    ~args:Env.args ()
