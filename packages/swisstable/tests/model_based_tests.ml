...
let insert_equivalence_prop =
  property "insert: swisstable matches hashmap"
    ...
    (fun pairs ->
      ...
      let all_match = List.for_all (fun (k, v) ->
        let r1 = Swisstable.insert swiss k v in
        let r2 = Collections.HashMap.insert hash k v in
        r1 = r2
      ) pairs in
      
      if not all_match then fail "Insert return values differ";
      
      (* Verify identical lengths *)
      if not (Swisstable.len swiss = Collections.HashMap.len hash) then
        fail "Lengths differ after inserts";
      
      (* Verify all keys accessible with same values *)
      List.for_all (fun (k, _) ->
        Swisstable.get swiss k = Collections.HashMap.get hash k
      ) pairs
    )
...
let get_equivalence_prop =
  property "get: swisstable matches hashmap"
    Arbitrary.(pair (list (pair int int)) (list int))
...
let remove_equivalence_prop =
  property "remove: swisstable matches hashmap"
    ...
    (fun (insert_pairs, remove_keys) ->
      ...
      if not all_match then fail "Remove return values differ";
...
let to_list_equivalence_prop =
  property "to_list: swisstable matches hashmap (unordered)"
    ...
    (fun pairs ->
      ...
      (* All entries in swiss_list are in hash_list *)
...
let keys_equivalence_prop =
  property "keys: swisstable matches hashmap (unordered)"
    ...
    (fun pairs ->
      ...
      (* All keys in swiss are in hashmap *)
      List.for_all (fun k -> List.mem k hash_keys) swiss_keys
    )
...
let values_equivalence_prop =
  property "values: swisstable matches hashmap (unordered)"
    ...
    (fun pairs ->
      ...
      (* Sort both and compare (values may have duplicates) *)
...
let fold_equivalence_prop =
  property "fold: swisstable matches hashmap"
    ...
    (fun pairs ->
      ...
      let hash_count = Collections.HashMap.fold (fun _ _ acc -> acc + 1) hash 0 in
      ...
      let hash_sum = Collections.HashMap.fold (fun _ v acc -> acc + v) hash 0 in
...
let or_insert_equivalence_prop =
  property "or_insert: swisstable matches hashmap"
    Arbitrary.(triple int int (list (pair int int)))
    (fun (key, default, pairs) ->
      ...
      if not (r1 = r2) then fail "or_insert return values differ";
...
let many_ops_equivalence_prop =
  property "many operations: swisstable matches hashmap"
    ...
    (fun pairs ->
      ...
      (* Remove half *)
      let keys_to_remove = List.filteri (fun i _ -> i mod 2 = 0) pairs in
      List.iter (fun (k, _) ->
      ...
      (* Verify all remaining entries match *)
      List.for_all (fun (k, _) ->
        Swisstable.get swiss k = Collections.HashMap.get hash k
      ) pairs
    )
...
let resize_equivalence_prop =
  property "resize: swisstable matches hashmap"
    Arbitrary.int
    (fun _seed ->
      let swiss = Swisstable.with_capacity 1 in  (* Force resizes *)
      ...
      let all_match = List.init 101 (fun i ->
        Swisstable.get swiss i = Collections.HashMap.get hash i
      ) |> List.for_all (fun x -> x) in
      
      if not all_match then fail "Entries differ after resize";
