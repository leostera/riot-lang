          ...
          List.for_all
            (
              fun ((k, v)) ->
                ...
                let r2 = Collections.HashMap.insert hash ~key:k ~value:v in r1 = r2
            )
            pairs
        in
...
let get_equivalence_prop =
  property "get: swisstable matches hashmap" (Arbitrary.pair (bounded_list_arb 50 Arbitrary.(pair int int)) (bounded_list_arb 50 Arbitrary.int))
    (
      fun ((insert_pairs, get_keys)) ->
        let swiss = Swisstable.create () in
        let hash = Collections.HashMap.create () in
        (* Populate both *)
        List.for_each insert_pairs ~fn:(
          fun ((k, v)) ->
        ...
        List.for_all
          (
            fun k -> Swisstable.get swiss k = Collections.HashMap.get hash ~key:k
          )
          get_keys
    )
...
let remove_equivalence_prop =
  property "remove: swisstable matches hashmap" (Arbitrary.pair (bounded_list_arb 50 Arbitrary.(pair int int)) (bounded_list_arb 50 Arbitrary.int))
    (
          ...
          List.for_all
            (
              fun k ->
                ...
                let r2 = Collections.HashMap.remove hash ~key:k in r1 = r2
            )
            remove_keys
        in
        if not all_match then
...
let contains_equivalence_prop =
  property "contains_key: swisstable matches hashmap" (Arbitrary.pair (bounded_list_arb 50 Arbitrary.(pair int int)) (bounded_list_arb 50 Arbitrary.int))
    (
  ...
  property "clear: swisstable matches hashmap" (bounded_list_arb 50 Arbitrary.(pair int int))
    (
      fun pairs ->
        ...
        (* Verify both empty *)
        Swisstable.len swiss = 0 && Collections.HashMap.length hash = 0 && Swisstable.is_empty swiss = Collections.HashMap.is_empty hash
    )

(* Property 6: to_list produces same entries (modulo order) *)
let to_list_equivalence_prop =
  property "to_list: swisstable matches hashmap (unordered)" (bounded_list_arb 50 Arbitrary.(pair int int))
    (
      fun pairs ->
        let swiss = Swisstable.create () in
        let hash = Collections.HashMap.create () in
        ...
        List.for_all
          (
            fun entry -> List.contains hash_list ~value:entry
          )
          swiss_list
    )
...
let keys_equivalence_prop =
  property "keys: swisstable matches hashmap (unordered)" (bounded_list_arb 50 Arbitrary.(pair int int))
    (
      fun pairs ->
        let swiss = Swisstable.create () in
        let hash = Collections.HashMap.create () in
        ...
        List.for_all
          (
            fun k -> List.contains hash_keys ~value:k
          )
          swiss_keys
    )
...
let values_equivalence_prop =
  property "values: swisstable matches hashmap (unordered)" (bounded_list_arb 50 Arbitrary.(pair int int))
    (
      fun pairs ->
        let swiss = Swisstable.create () in
        let hash = Collections.HashMap.create () in
        ...
        let sorted_hash = List.sort hash_values ~compare in sorted_swiss = sorted_hash
    )
      ...
      fun pairs ->
        ...
        let swiss_count =
          Swisstable.fold
            ...
            0
        in
        let hash_count = Collections.HashMap.fold_left hash ~init:0 ~fn:(
          fun acc _ _ -> acc + 1
        ) in
        ...
        let swiss_sum =
          Swisstable.fold
            ...
            0
        in
        let hash_sum = Collections.HashMap.fold_left hash ~init:0 ~fn:(
          fun acc _ value -> acc + value
        ) in swiss_sum = hash_sum
    )

(* Property 10: or_insert produces identical results *)
let or_insert_equivalence_prop =
  property "or_insert: swisstable matches hashmap" (Arbitrary.triple Arbitrary.int Arbitrary.int (bounded_list_arb 50 Arbitrary.(pair int int)))
    (
...
let many_ops_equivalence_prop =
  property "many operations: swisstable matches hashmap" (non_empty_bounded_list_arb 200 Arbitrary.(pair int int))
    (
      fun pairs ->
        let swiss = Swisstable.create () in
        let hash = Collections.HashMap.create () in
        ...
        (* Remove half *)
        let keys_to_remove = List.enumerate pairs |> List.filter_map ~fn:(
          fun (index, pair) ->
            if index mod 2 = 0 then
              Some pair
            else None
        ) in
        List.for_each keys_to_remove ~fn:(
          fun ((k, _)) ->
  ...
  property "resize: swisstable matches hashmap" Arbitrary.int
    (
      fun _seed ->
        ...
        let all_match =
          List.init ~count:101 ~fn:(
            fun i -> Swisstable.get swiss i = Collections.HashMap.get hash ~key:i
          ) |> List.for_all
            (
              fun x -> x
            )
