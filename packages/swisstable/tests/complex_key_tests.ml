...
type person = { age: int; email: string; active: bool }
...
let user_arb = Arbitrary.make ~print:(
  fun u -> "{id=" ^ Int.to_string u.id ^ "; name=" ^ u.name ^ "}"
) user_gen
...
let point_arb = Arbitrary.make ~print:(
  fun p -> "(" ^ Int.to_string p.x ^ "," ^ Int.to_string p.y ^ ")"
) point_gen
...
let person_arb = Arbitrary.make ~print:(
  fun p -> "{age=" ^ Int.to_string p.age ^ "; email=" ^ p.email ^ "; active=" ^ Bool.to_string p.active ^ "}"
) person_gen
...
let event_arb = Arbitrary.make ~print:(
  function
...
let status_arb = Arbitrary.make ~print:(
  function
  | Pending -> "Pending"
...
let customer_arb = Arbitrary.make ~print:(
  fun c -> "{user=" ^ Int.to_string c.user.id ^ "; location=(" ^ Int.to_string c.location.x ^ "," ^ Int.to_string c.location.y ^ ")}"
) customer_gen
...
let user_key_multiple_prop =
  property "record keys (user): multiple distinct users" (bounded_list_arb 50 (Arbitrary.pair user_arb Arbitrary.int))
    (
      fun pairs ->
        let map = Swisstable.create () in
        (* Insert all users *)
  ...
  property "record keys (person): all fields matter" Arbitrary.(triple person_arb int int)
    (
      fun ((person, val1, val2)) ->
        ...
        let r2 = Swisstable.get map different_person in r1 = Some val1 && r2 = Some val2
    )
...
let event_key_multiple_prop =
  property "variant keys (event): multiple distinct events" (bounded_list_arb 30 (Arbitrary.pair event_arb Arbitrary.int))
    (
      fun pairs ->
        let map = Swisstable.create () in
        let ref_map = Collections.HashMap.create () in
...
let tuple_triple_prop =
  property "tuple keys (int * string * bool): insert then get" Arbitrary.(pair (triple int string bool) int)
    (
      fun ((key, value)) ->
        let map = Swisstable.create () in
        let _ = Swisstable.insert map key value in
...
let customer_key_multiple_prop =
  property "nested keys (customer): multiple distinct customers" (bounded_list_arb 30 (Arbitrary.pair customer_arb Arbitrary.int))
    (
      fun pairs ->
        let map = Swisstable.create () in
        let ref_map = Collections.HashMap.create () in
...
let collision_point_prop =
  property "hash collisions (small point range): correctness maintained" (bounded_list_arb 50 Arbitrary.(pair int int))
    (
      fun pairs ->
        (* Use small point range (0-9) to force collisions *)
        let small_pairs = List.map pairs ~fn:(
          fun ((k, v)) ->
            let x =
              if k < 0 then
                (-k) mod 10
  ...
  property "variant keys: overwrite behavior" Arbitrary.(triple event_arb int int)
    (
      fun ((event, val1, val2)) ->
        ...
        let r2 = Swisstable.insert map event val2 in r1 = None && r2 = Some val1 && Swisstable.get map event = Some val2
    )
