...
let user_key_multiple_prop =
  property
    "record keys (user): multiple distinct users"
...
let event_key_multiple_prop =
  property
    "variant keys (event): multiple distinct events"
...
let customer_key_multiple_prop =
  property
    "nested keys (customer): multiple distinct customers"
...
let collision_point_prop =
  property
    "hash collisions (small point range): correctness maintained"
