...
let user_gen =
  Generator.map
    (fun (id, name) -> { id; name })
    (Generator.pair 
      (Generator.int_range 0 1000)
      Generator.string)

let user_arb = Arbitrary.make
  ~print:(fun u -> "{id=" ^ Int.to_string u.id ^ "; name=" ^ u.name ^ "}")
  user_gen
...
let point_arb = Arbitrary.make
  ~print:(fun p -> "(" ^ Int.to_string p.x ^ "," ^ Int.to_string p.y ^ ")")
  point_gen
...
let person_arb = Arbitrary.make
  ~print:(fun p -> "{age=" ^ Int.to_string p.age ^ "; email=" ^ p.email ^ "; active=" ^ Bool.to_string p.active ^ "}")
  person_gen

(* Event generator *)
let event_gen =
  Generator.one_of [
    Generator.map (fun (x, y) -> Click (x, y)) 
      (Generator.pair (Generator.int_range 0 1000) (Generator.int_range 0 1000));
    Generator.map (fun c -> KeyPress c) Generator.char;
    Generator.map (fun n -> Scroll n) (Generator.int_range (-100) 100);
    Generator.map (fun (w, h) -> Resize (w, h))
      (Generator.pair (Generator.int_range 0 2000) (Generator.int_range 0 2000));
  ]

let event_arb = Arbitrary.make
  ~print:(function
...
let customer_arb = Arbitrary.make
  ~print:(fun c -> 
    "{user=" ^ Int.to_string c.user.id ^ "; location=(" ^ 
    Int.to_string c.location.x ^ "," ^ Int.to_string c.location.y ^ ")}")
  customer_gen
...
let user_key_insert_get_prop =
  property "record keys (user): insert then get"
...
let user_key_multiple_prop =
  property "record keys (user): multiple distinct users"
    (Arbitrary.list (Arbitrary.pair user_arb Arbitrary.int))
    (fun pairs ->
      ...
      List.iter (fun (user, value) ->
        Collections.HashMap.insert ref_map user value |> ignore
      ) pairs;
      
      (* Verify all accessible *)
      Collections.HashMap.iter (fun user expected_value ->
        match Swisstable.get map user with
        | Some actual_value -> 
...
let point_key_prop =
  property "record keys (point): insert then get"
...
let person_key_prop =
  property "record keys (person): all fields matter"
...
let event_key_prop =
  property "variant keys (event): insert then get"
...
let event_key_multiple_prop =
  property "variant keys (event): multiple distinct events"
    (Arbitrary.list (Arbitrary.pair event_arb Arbitrary.int))
    (fun pairs ->
      ...
      (* Verify all accessible *)
      Collections.HashMap.iter (fun event expected ->
        match Swisstable.get map event with
...
let status_key_prop =
  property "variant keys (status): insert then get"
...
let tuple_int_int_prop =
  property "tuple keys (int * int): insert then get"
...
let tuple_triple_prop =
  property "tuple keys (int * string * bool): insert then get"
...
let tuple_user_int_prop =
  property "tuple keys (user * int): composite key"
...
let customer_key_prop =
  property "nested keys (customer): insert then get"
...
let customer_key_multiple_prop =
  property "nested keys (customer): multiple distinct customers"
    (Arbitrary.list (Arbitrary.pair customer_arb Arbitrary.int))
    (fun pairs ->
      assume (Collections.List.length pairs <= 30);
      ...
      Collections.HashMap.iter (fun customer expected ->
        match Swisstable.get map customer with
        | Some actual when actual = expected -> ()
...
let collision_point_prop =
  property "hash collisions (small point range): correctness maintained"
    ...
    (fun pairs ->
      ...
      let small_pairs = List.map (fun (k, v) ->
        let x = if k < 0 then (-k) mod 10 else k mod 10 in
        ({ x; y = x }, v)
      ) pairs in
      ...
      (* Verify all unique points in ref_map are accessible in swiss *)
      Collections.HashMap.iter (fun p v ->
...
let record_mixed_ops_prop =
  property "record keys: insert/remove/reinsert cycle"
    Arbitrary.(pair user_arb int)
    (fun (user, value) ->
      let map = Swisstable.create () in
      ...
      if not (r1 = None) then fail "Expected None on first insert";
      
      (* Remove *)
      let r2 = Swisstable.remove map user in
      if not (r2 = Some value) then fail "Expected Some(value) on remove";
      
      (* Reinsert *)
      let r3 = Swisstable.insert map user (value + 1) in
      if not (r3 = None) then fail "Expected None on reinsert";
...
let variant_overwrite_prop =
  property "variant keys: overwrite behavior"
    ...
    (fun (event, val1, val2) ->
      ...
      r1 = None && 
      r2 = Some val1 && 
      Swisstable.get map event = Some val2
    )
