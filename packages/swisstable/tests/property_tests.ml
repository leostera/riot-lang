...
let bounded_list_arb = fun ?(min = 0) max elem_arb ->
  let list_arb = Arbitrary.list elem_arb in { list_arb with gen = Generator.list_size (Generator.int_range min max) elem_arb.gen }
...
let swisstable = fun key_arb value_arb -> Arbitrary.make ~print:(
  fun map ->
    let entries = Swisstable.to_list map in
    let pairs_str = String.concat ", " (List.map entries ~fn:(
      fun ((k, v)) ->
        let k_str =
...
let insert_get_prop =
  property "insert then get returns the value" Arbitrary.(triple int int populated_map)
    (
      fun ((key, value, map)) ->
...
let remove_get_prop =
  property "get returns None after remove" Arbitrary.(pair int populated_map)
    (
      fun ((key, map)) ->
        (* First insert to ensure key exists *)
        let _ = Swisstable.insert map key 42 in
        (* Then remove *)
        let _ = Swisstable.remove map key in (* Verify it's gone *)
        Swisstable.get map key = None
    )

(* Property 3: Contains-Get Equivalence *)
let contains_get_equiv_prop =
  property "contains_key equivalent to is_some(get)" Arbitrary.(pair int populated_map)
    (
      fun ((key, map)) ->
        let has_key = Swisstable.contains_key map key in
        let get_result = Swisstable.get map key in has_key = Option.is_some get_result
    )

(* Property 4: Insert is idempotent for same key-value *)
let insert_idempotent_prop =
  property "inserting same (k,v) twice is idempotent" Arbitrary.(triple int int populated_map)
    (
      fun ((key, value, map)) ->
        let _ = Swisstable.insert map key value in
        let len1 = Swisstable.len map in
        let _ = Swisstable.insert map key value in
        let len2 = Swisstable.len map in len1 = len2
    )

(* Property 5: Remove absent key is no-op *)
let remove_absent_noop_prop =
  property "removing absent key is no-op" Arbitrary.(pair int populated_map)
    (
      fun ((key, map)) ->
        ...
        let len2 = Swisstable.len map in result = None && len1 = len2
    )

(* Property 6: Length after insert *)
let length_after_insert_prop =
  property "length increases by at most 1 after insert" Arbitrary.(triple int int populated_map)
    (
      fun ((key, value, map)) ->
        let len_before = Swisstable.len map in
        let _ = Swisstable.insert map key value in
        let len_after = Swisstable.len map in len_after = len_before || len_after = len_before + 1
    )

(* Property 7: Length after remove *)
let length_after_remove_prop =
  property "length decreases by at most 1 after remove" Arbitrary.(pair int populated_map)
    (
      fun ((key, map)) ->
        let len_before = Swisstable.len map in
        let _ = Swisstable.remove map key in
...
let to_list_length_prop =
  property "to_list length equals len" populated_map
    (
      fun map ->
        let lst = Swisstable.to_list map in Collections.List.length lst = Swisstable.len map
    )

(* Property 9: keys length equals len *)
let keys_length_prop =
  property "keys length equals len" populated_map
    (
      fun map ->
        let keys = Swisstable.keys map in Collections.List.length keys = Swisstable.len map
    )

(* Property 10: values length equals len *)
let values_length_prop =
  property "values length equals len" populated_map
    (
      fun map ->
        let values = Swisstable.values map in Collections.List.length values = Swisstable.len map
    )

(* Property 11: All entries in to_list are gettable *)
let to_list_entries_gettable_prop =
  property "all entries in to_list are gettable" populated_map
    (
      fun map ->
        let entries = Swisstable.to_list map in
        List.for_all
...
let fold_count_prop =
  property "fold counting equals len" populated_map
    (
      fun map ->
        let count =
          Swisstable.fold
...
let or_insert_vacant_prop =
  property "or_insert creates entry for vacant key" Arbitrary.(triple int int populated_map)
    (
      fun ((key, value, map)) ->
        (* Ensure key doesn't exist *)
        let _ = Swisstable.remove map key in
        ...
        let len_after = Swisstable.len map in result = value && len_after = len_before + 1
    )

(* Property 14: or_insert on occupied key returns existing *)
let or_insert_occupied_prop =
  property "or_insert returns existing value for occupied key" Arbitrary.(triple int int populated_map)
    (
      fun ((key, old_value, map)) ->
        ...
        let result = Swisstable.or_insert map key 9_999 in
        let len_after = Swisstable.len map in result = old_value && len_before = len_after
    )

(* Property 15: and_modify only affects existing keys *)
let and_modify_existing_prop =
  property "and_modify only modifies existing keys" Arbitrary.(pair int populated_map)
    (
      fun ((key, map)) ->
        (* Try to modify non-existent key *)
        let _ = Swisstable.remove map key in
        Swisstable.and_modify map key
          (
            fun x -> x + 1
          );
        Swisstable.get map key = None
...
let clear_makes_empty_prop =
  property "clear makes map empty" populated_map
    (
      fun map ->
...
let clear_all_none_prop =
  property "after clear, all gets return None" populated_map
    (
      fun map ->
        ...
        List.for_all
          (
            fun k -> Swisstable.get map k = None
          )
          keys
    )
  ...
  property "many insertions preserve all entries" (bounded_list_arb 100 Arbitrary.(pair int int))
    (
      fun pairs ->
        ...
        List.for_each pairs ~fn:(
          fun ((k, v)) -> Collections.HashMap.insert ref_map ~key:k ~value:v |> ignore
        );
        (* Verify all unique keys are accessible and match reference *)
        Collections.HashMap.for_each ref_map ~fn:(
          fun k expected_v ->
            match Swisstable.get map k with
            | Some actual_v ->
...
let insert_returns_previous_prop =
  property "insert returns previous value if key exists" Arbitrary.(triple int int int)
    (
      fun ((key, old_val, new_val)) ->
        let map = Swisstable.create () in
        let result1 = Swisstable.insert map key old_val in
        let result2 = Swisstable.insert map key new_val in result1 = None && result2 = Some old_val
    )

(* Property 21: Remove returns removed value *)
let remove_returns_value_prop =
  property "remove returns the removed value" Arbitrary.(pair int int)
    (
      fun ((key, value)) ->
        let map = Swisstable.create () in
        let _ = Swisstable.insert map key value in
        let result = Swisstable.remove map key in result = Some value
    )

(** {1 Empty Map Properties} *)
(* Property 22: Empty map has length 0 *)
let empty_map_length_prop =
  property "newly created map has length 0" Arbitrary.int
    (
      fun _ ->
        let map = Swisstable.create () in Swisstable.len map = 0 && Swisstable.is_empty map
    )

(* Property 23: Get on empty map returns None *)
let empty_map_get_prop =
  property "get on empty map returns None" Arbitrary.int
    (
      fun key ->
        let map = Swisstable.create () in Swisstable.get map key = None
    )
