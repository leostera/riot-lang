...
let tests = [
  ...
  Test.case "create capacity zero then insert" (fun () ->
  ...
  Test.case "insert and overwrite" (fun () ->
    let map = Swisstable.create () in
    Test.assert_equal ~expected:None ~actual:(Swisstable.insert map 1 2);
  ...
  Test.case "empty remove" (fun () ->
    let map = Swisstable.create () in
    Test.assert_equal ~expected:None ~actual:(Swisstable.remove map 1);
    Ok ());

  Test.case "lots of insertions (250)" (fun () ->
    let map = Swisstable.create () in
  ...
  Test.case "find and contains" (fun () ->
    let map = Swisstable.create () in
    let _ = Swisstable.insert map 1 2 in
    Test.assert_equal ~expected:(Some 2) ~actual:(Swisstable.get map 1);
  ...
  Test.case "iterate" (fun () ->
    ...
    Swisstable.iter (fun _k v ->
      Sync.Cell.set sum (Sync.Cell.get sum + v)
    ) map;
  ...
  Test.case "from_iter (of_list)" (fun () ->
  ...
  Test.case "expand (multiple resizes)" (fun () ->
    let map = Swisstable.create () in
  ...
  Test.case "insert after remove (tombstone reuse)" (fun () ->
    let map = Swisstable.create () in
  ...
  Test.case "or_insert with existing key" (fun () ->
    let map = Swisstable.create () in
    let _ = Swisstable.insert map "key" 10 in
  ...
  Test.case "int keys, string values" (fun () ->
    let map = Swisstable.create () in
  ...
  Test.case "resize under load" (fun () ->
    let map = Swisstable.create () in
    ...
    for i = 0 to 249 do
      ...
      let _ = Swisstable.insert map key (i + 1000) in
      ()
    done;
  ...
  Test.case "alternating insert/remove" (fun () ->
    let map = Swisstable.create () in
    let keys = make_keys 100 in
  ...
  Test.case "empty iteration" (fun () ->
    let map = Swisstable.create () in
    let count = Sync.Cell.create 0 in
    
    Swisstable.iter (fun _k _v ->
      Sync.Cell.set count (Sync.Cell.get count + 1)
    ) map;
...
let () =
  Miniriot.run ~main:(fun ~args:_ ->
      Test.Cli.main ~name:"swisstable:comprehensive" ~tests ~args:Env.args)
    ~args:Env.args ()
