...
let make_keys = fun n -> Collections.Array.init ~count:n ~fn:(
  fun i -> "key" ^ string_of_int i
)
...
let tests =
  [
    ...
    Test.case "create capacity zero then insert"
      (
        fun _ctx ->
    ...
    Test.case "insert and overwrite"
      (
        fun _ctx ->
          let map = Swisstable.create () in
          Test.assert_equal ~expected:None ~actual:(Swisstable.insert map 1 2);
    ...
    Test.case "lots of insertions (250)"
      (
        fun _ctx ->
          let map = Swisstable.create () in
    ...
    Test.case "iterate"
      (
        fun _ctx ->
          ...
          Swisstable.iter
            (
              fun _k v -> Sync.Cell.set sum (Sync.Cell.get sum + v)
            )
            map;
    ...
    Test.case "expand (multiple resizes)"
      (
        fun _ctx ->
          let map = Swisstable.create () in
          ...
          for i = 0 to 99 do
            let key = Collections.Array.get_unchecked keys ~at:i in Test.assert_equal ~expected:(Some i) ~actual:(Swisstable.get map key)
          done;
          Ok ()
      );
    Test.case "conflict remove"
      (
        fun _ctx ->
          let map = Swisstable.create () in
    ...
    Test.case "insert after remove (tombstone reuse)"
      (
        fun _ctx ->
          let map = Swisstable.create () in
    ...
    Test.case "and_modify"
      (
        fun _ctx ->
          ...
          Swisstable.and_modify map "counter"
            (
              fun x -> x + 1
            );
          ...
          Swisstable.and_modify map "missing"
            (
              fun x -> x + 1
            );
    ...
    Test.case "int keys, string values"
      (
        fun _ctx ->
          let map = Swisstable.create () in
    ...
    Test.case "alternating insert/remove"
      (
        fun _ctx ->
          let map = Swisstable.create () in
          let keys = make_keys 100 in
    ...
    Test.case "empty iteration"
      (
        fun _ctx ->
          ...
          Swisstable.iter
            (
              fun _k _v -> Sync.Cell.set count (Sync.Cell.get count + 1)
            )
            map;
