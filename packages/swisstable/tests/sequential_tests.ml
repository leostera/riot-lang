...
let operation_arb = Arbitrary.make ~print:(
  function
...
let random_sequence_prop =
  property "random operation sequence: maintains equivalence with hashmap" (non_empty_bounded_list_arb 100 operation_arb)
    (
      fun ops ->
        let swiss = Swisstable.create () in
        let hash = Collections.HashMap.create () in
        (* Apply all operations *)
        List.for_each ops ~fn:(
          fun op -> apply_operation op swiss hash
        );
        ...
        List.for_all
          (
            fun ((k, v)) -> Collections.HashMap.get hash ~key:k = Some v
          )
          swiss_list
    )
...
let insert_heavy_sequence_prop =
  property "insert-heavy sequence: correctness maintained" (non_empty_bounded_list_arb 100 Arbitrary.(pair int int))
    (
      fun pairs ->
        let swiss = Swisstable.create () in
        let hash = Collections.HashMap.create () in
        ...
        List.for_each pairs ~fn:(
          fun ((k, v)) -> apply_operation (Insert (k, v)) swiss hash
        );
        (* Get all *)
        List.for_each pairs ~fn:(
          fun ((k, _)) -> apply_operation (Get k) swiss hash
        );
        true
    )
...
let remove_heavy_sequence_prop =
  property "remove-heavy sequence: correctness maintained" (non_empty_bounded_list_arb 100 Arbitrary.(pair int int))
    (
      fun pairs ->
        let swiss = Swisstable.create () in
        let hash = Collections.HashMap.create () in
        ...
        (* Remove all *)
        List.for_each pairs ~fn:(
          fun ((k, _)) -> apply_operation (Remove k) swiss hash
        );
...
let interleaved_ops_prop =
  property "interleaved insert/remove: correctness maintained" (non_empty_bounded_list_arb 50 Arbitrary.int)
    (
      fun keys ->
        let swiss = Swisstable.create () in
        let hash = Collections.HashMap.create () in
        (* Insert, then remove, then insert again *)
        List.for_each keys ~fn:(
          fun k ->
        ...
        List.for_all
          (
            fun k -> Swisstable.get swiss k = Some (k * 3) && Collections.HashMap.get hash ~key:k = Some (k * 3)
          )
          keys
    )
...
let clear_interleaved_prop =
  property "clear in middle: correctness maintained" (Arbitrary.pair (bounded_list_arb 50 Arbitrary.(pair int int)) (bounded_list_arb 50 Arbitrary.(pair int int)))
    (
      fun ((before_clear, after_clear)) ->
        let swiss = Swisstable.create () in
        let hash = Collections.HashMap.create () in
        (* Insert before clear *)
        List.for_each before_clear ~fn:(
          fun ((k, v)) -> apply_operation (Insert (k, v)) swiss hash
        );
        (* Clear *)
        apply_operation Clear swiss hash;
        (* Insert after clear *)
        List.for_each after_clear ~fn:(
          fun ((k, v)) -> apply_operation (Insert (k, v)) swiss hash
        );
        ...
        Swisstable.len swiss = Collections.HashMap.length hash && List.for_all
          (
            fun ((k, _)) ->
              ...
              let in_hash = Collections.HashMap.contains_key hash k in in_swiss = in_hash
          )
          before_clear
    )
...
let overwrite_sequence_prop =
  property "overwrite sequence: latest value wins" (Arbitrary.pair Arbitrary.int (non_empty_bounded_list_arb 20 Arbitrary.int))
    (
      fun ((key, values)) ->
        let swiss = Swisstable.create () in
        let hash = Collections.HashMap.create () in
        ...
        List.for_each values ~fn:(
          fun v -> apply_operation (Insert (key, v)) swiss hash
        );
        (* Latest value should be present *)
        let latest = List.get_unchecked values ~at:(List.length values - 1) in Swisstable.get swiss key = Some latest && Collections.HashMap.get hash ~key:key = Some latest
    )
        ...
        List.for_all
          (
            fun op ->
              match op with
              | Insert (k, v) ->
                  Swisstable.insert swiss k v |> ignore;
                  true
              | Remove k ->
                  Swisstable.remove swiss k |> ignore;
                  true
              | Clear ->
                  Swisstable.clear swiss;
                  true
...
let long_sequence_prop =
  property "long sequence (200 ops): correctness maintained" Arbitrary.int
    (
      fun _seed ->
        let swiss = Swisstable.create () in
        let hash = Collections.HashMap.create () in
