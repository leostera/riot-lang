...
module Group = struct
  ...
  let repeat = fun byte ->
    ...
    let b = I64.logor b (I64.shift_left b 16) in I64.logor b (I64.shift_left b 32)

  let load = fun ctrl idx -> (* Use SIMD-optimized C function *)
  ...
  let bitmask_to_int = fun bits ->
    let extract_bit byte_pos = I64.to_int (I64.shift_right_logical (I64.logand bits (I64.shift_left 0x80L (byte_pos * 8))) (byte_pos * 8 + 7)) in extract_bit 0 lor (extract_bit 1 lsl 1) lor (extract_bit 2 lsl 2) lor (extract_bit 3 lsl 3) lor (extract_bit 4 lsl 4) lor (extract_bit 5 lsl 5) lor (extract_bit 6 lsl 6) lor (extract_bit 7 lsl 7)

  let match_tag_impl = fun ctrl idx tag -> (* Use SIMD-optimized C function - directly returns bitmask *)
  ...
  let match_tag = fun group tag ->
    ...
    let result = I64.logand (I64.logand (I64.sub cmp ones) (I64.lognot cmp)) deleted_marker in bitmask_to_int result

  let match_empty_impl = fun ctrl idx -> (* Use SIMD-optimized C function *)
  ...
  let match_empty = fun group ->
    ...
    let result = I64.logand (I64.logand group (I64.shift_left group 1)) deleted_marker in bitmask_to_int result

  let match_empty_or_deleted_impl = fun ctrl idx -> (* Use SIMD-optimized C function - this is the critical fast path! *)
  group_match_empty_or_deleted_simd ctrl idx

  (* Keep this version for backwards compatibility *)
  let match_empty_or_deleted = fun group ->
    let deleted_marker = repeat Tag.deleted in
    let result = I64.logand group deleted_marker in bitmask_to_int result

  (* Match FULL tags (high bit clear) *)
  let match_full = fun group ->
    let deleted_marker = repeat Tag.deleted in
    let result = I64.logand (I64.lognot group) deleted_marker in bitmask_to_int result
end

(** ProbeSeq module - triangular/quadratic probing sequence *)
module ProbeSeq = struct
  type t = { mutable pos: int; mutable stride: int }

  (* Start probing from hash position *)
  let start = fun hash bucket_mask ->
    let pos = hash land bucket_mask in { pos; stride = 0 }
...
module RawTable = struct
  ...
  let find = fun table key ->
    ...
    let h2 = hash_h2 hash in find_with_hash table key hash h2
  ...
  let find_insert_slot = fun table key ->
    let hash = hash_native key in find_insert_slot_with_hash table hash
  ...
  let get = fun table key ->
    ...
    match find_with_hash table key hash h2 with
    | None -> None
    | Some idx -> Option.map (Array.get_unchecked table.buckets ~at:idx) ~fn:(
      fun (_, value) -> value
    )
  ...
  let contains_key = fun table key ->
    let hash = hash_native key in
    let h2 = hash_h2 hash in Option.is_some (find_with_hash table key hash h2)

  (* Clear all entries *)
  let clear = fun table ->
    for idx = 0 to Array.length table.buckets - 1 do Array.set_unchecked table.buckets ~at:idx ~value:None done;
    Kernel.Bytes.fill table.ctrl ~offset:0 ~len:(Kernel.Bytes.length table.ctrl) ~char:(Kernel.Char.chr Tag.empty);
    table.len <- 0

  (* Iterate over all key-value pairs *)
  let iter = fun f table -> Array.for_each table.buckets ~fn:(
    function
    | Some (k, v) -> f k v
    | None -> ()
  )
  ...
  let fold = fun f table acc -> Array.fold_left table.buckets ~init:acc ~fn:(
    fun acc bucket ->
      match bucket with
      | Some (k, v) -> f k v acc
      | None -> acc
  ...
  (* Convert to list *)
  let to_list = fun table -> Array.fold_left table.buckets ~init:[] ~fn:(
    fun acc bucket ->
      match bucket with
      | Some (k, v) -> (k, v) :: acc
      | None -> acc
...
let keys = fun map ->
  ...
  RawTable.iter
    (
      fun k _ -> Cell.set result (k :: Cell.get result)
    )
    map;
  Cell.get result
...
let values = fun map ->
  ...
  RawTable.iter
    (
      fun _ v -> Cell.set result (v :: Cell.get result)
    )
    map;
  Cell.get result
...
let or_insert = fun map key default ->
  ...
  | Some v -> v
  | None ->
      let _ = insert map key default in default
  ...
  let module MapIter = struct
    ...
    let next = fun state ->
      if state.pos >= List.length state.items then
        ...
        let item = List.get_unchecked state.items ~at:state.pos in (Some item, { state with pos = state.pos + 1 })

    let size = fun state -> max 0 (List.length state.items - state.pos)
  end in
  let items = to_list map in Iter.Iterator.make (module MapIter) { MapIter.items; pos = 0 }
...
let to_mut_iter: type k v. (k, v) t -> (k * v) Iter.MutIterator.t = fun map ->
  let module MapIter = struct
    ...
    let next = fun state ->
      if state.pos >= List.length state.items then
        ...
        let item = List.get_unchecked state.items ~at:state.pos in state.pos <- state.pos + 1;
      Some item
  ...
  let items = to_list map in Iter.MutIterator.make (module MapIter) { MapIter.items; pos = 0 }
