...
let build_fixture = fun
  ...
  in
  let encoded = Serde_yaml.to_string manifest_encode value
  |> Result.expect ~msg:("expected " ^ label ^ " fixture to encode") in
  let decoded = Serde_yaml.from_string manifest_decode encoded
...
let bench_encode_in_memory = fun fixture () ->
  ignore (Serde_yaml.to_string manifest_encode fixture.value)
...
let bench_decode_in_memory = fun fixture () ->
  ignore (Serde_yaml.from_string manifest_decode fixture.encoded)
