...
let io_writer_of_buffer =
  let module Write = struct
    ...
    let write = fun buffer ~from ->
      ...
      IO.Buffer.append_slice buffer (IO.Buffer.readable from) |> Result.expect ~msg:"serde-yaml bench writer should append buffer contents";
      Ok written
    ...
    let write_vectored = fun buffer ~from ->
      ...
      IO.IoVec.for_each from ~fn:(
        fun chunk ->
          IO.Buffer.append_slice buffer chunk |> Result.expect ~msg:"serde-yaml bench writer should append slices";
...
let manifest_decode = De.record_mut ~fields:manifest_fields ~create:(
  ...
  fun builder ->
    match builder.ship, builder.emergency, builder.crew_count, builder.small, builder.bounty, builder.heading, builder.rank, builder.companion, builder.marker, builder.home, builder.tags, builder.scores, builder.stops, builder.mirrors with
      ...
      Ser.fields
        [
          ...
          Ser.field "companion" companion_encode
            (
              fun (value: manifest) -> value.companion
            );
...
let equal_stop_lists = fun left right ->
  ...
  | 0 -> List.zip left right |> List.all ~fn:(
    fun (left, right) -> equal_stop left right
  )
  | _ -> false

let equal_manifest = fun (left: manifest) (right: manifest) -> String.equal left.ship right.ship && Bool.equal left.emergency right.emergency && Int.equal left.crew_count right.crew_count && Int32.equal left.small right.small && Int64.equal left.bounty right.bounty && Float.equal left.heading right.heading && left.nickname = right.nickname && left.rank = right.rank && equal_companion left.companion right.companion && equal_berth left.home right.home && vec_to_list left.tags = vec_to_list right.tags && left.scores = right.scores && equal_stop_lists (vec_to_list left.stops) (vec_to_list right.stops) && equal_stop_lists (Array.to_list left.mirrors) (Array.to_list right.mirrors)
...
let build_fixture = fun ({ label; tag_count; score_count; stop_count; string_repeat }: fixture_spec) ->
  let value: manifest = {
    ...
    emergency = false;
    crew_count = 10;
    small = Int32.of_int 7;
    bounty = Int64.of_int 3_000_000_000;
    heading = 12.5;
  ...
  in
  let encoded = Serde_yaml.to_string manifest_encode value |> Result.expect ~msg:("expected " ^ label ^ " fixture to encode") in
  let decoded = Serde_yaml.from_string manifest_decode encoded |> Result.expect ~msg:("expected " ^ label ^ " fixture to decode") in
...
let bench_encode_in_memory = fun fixture () -> ignore (Serde_yaml.to_string manifest_encode fixture.value)

let bench_encode_writer = fun fixture () ->
  let buffer = IO.Buffer.create ~size:(String.length fixture.encoded) in ignore (Serde_yaml.to_writer manifest_encode (io_writer_of_buffer buffer) fixture.value)

let bench_decode_in_memory = fun fixture () -> ignore (Serde_yaml.from_string manifest_decode fixture.encoded)

let bench_decode_reader = fun fixture () -> ignore (Serde_yaml.from_reader manifest_decode (String.to_reader ~chunk_size:io_chunk_size fixture.encoded))
...
let benchmark_suite = fun fixture ->
  ...
  in
  Bench.[
    with_config ~config ("serde-yaml encode in-memory " ^ fixture.label ^ " dataset (" ^ size ^ ")") (bench_encode_in_memory fixture);
    with_config ~config ("serde-yaml encode writer " ^ fixture.label ^ " dataset (" ^ size ^ ")") (bench_encode_writer fixture);
    with_config ~config ("serde-yaml decode in-memory " ^ fixture.label ^ " dataset (" ^ size ^ ")") (bench_decode_in_memory fixture);
    with_config ~config ("serde-yaml decode reader " ^ fixture.label ^ " dataset (" ^ size ^ ")") (bench_decode_reader fixture);
  ]

let main ~args =
  let small_fixture = build_fixture small_fixture_spec in
  let large_fixture = build_fixture large_fixture_spec in
  let benchmarks = benchmark_suite small_fixture @ benchmark_suite large_fixture in Bench.Cli.main ~name:"serde-yaml benchmarks" ~benchmarks ~args
