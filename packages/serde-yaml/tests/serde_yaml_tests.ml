...
let io_writer_of_buffer =
  let module Write = struct
    ...
    let write = fun buffer ~from ->
      ...
      IO.Buffer.append_slice buffer (IO.Buffer.readable from) |> Result.expect ~msg:"serde-yaml test writer should append buffer contents";
      Ok written
    ...
    let write_vectored = fun buffer ~from ->
      ...
      IO.IoVec.for_each from ~fn:(
        fun chunk ->
          IO.Buffer.append_slice buffer chunk |> Result.expect ~msg:"serde-yaml test writer should append slices";
...
let fixture: manifest = {
  ...
  scores = [|
    ...
    9;
  |];
  stops = Vector.from_list [ ({ island = "Water 7"; supplies = 25 } : stop); ({ island = "Fish-Man Island"; supplies = 40 } : stop) ];
        ...
        | Ok encoded -> (
          ...
          | Ok decoded ->
              if equal_manifest fixture decoded then
                Ok ()
              else Error "decoded manifest did not match encoded manifest"
          | Error err -> Error (Serde.Error.to_string err)
        )
        | Error err -> Error (Serde.Error.to_string err)
    )
                    ...
                    | Ok seq_encoded -> (
                      ...
                      | Ok seq_decoded ->
                          if vec_to_list numbers = vec_to_list seq_decoded then
                            Ok ()
                          else Error "sequence roundtrip failed"
                    )
    )
  ...
  Test.case "serde-yaml decodes tagged variants and empty null fields"
    (
      fun _ctx ->
        ...
        | Ok decoded ->
            if String.equal decoded.ship "Going Merry" && Bool.equal decoded.emergency true && decoded.nickname = None && decoded.rank = Captain && equal_companion decoded.companion (Reindeer "Chopper") && vec_to_list decoded.tags = [] && Array.to_list decoded.scores = [] then
              Ok ()
            else Error "decoded tagged manifest did not match expected fields"
        | Error err -> Error (Serde.Error.to_string err)
    )
  ...
  Test.case "serde-yaml writes to writers and reads from readers"
    (
      fun _ctx ->
        ...
        | Ok () -> (
          match Serde_yaml.from_reader manifest_decode (String.to_reader ~chunk_size:3 (IO.Buffer.contents buffer)) with
          | Ok decoded ->
              if equal_manifest fixture decoded then
                Ok ()
              else Error "reader/writer roundtrip failed"
          | Error err -> Error (Serde.Error.to_string err)
        )
        | Error err -> Error (Serde.Error.to_string err)
    )
      ...
      fun _ctx ->
        ...
        | Ok decoded ->
            if String.equal decoded.ship "Sunny # not a comment" then
              Ok ()
            else Error "comment stripping damaged quoted string content"
        | Error err -> Error (Serde.Error.to_string err)
    )
