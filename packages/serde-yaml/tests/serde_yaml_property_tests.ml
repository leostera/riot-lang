...
let io_writer_of_buffer =
  let module Write = struct
    ...
    let write = fun buffer ~from ->
      ...
      IO.Buffer.append_slice buffer (IO.Buffer.readable from) |> Result.expect ~msg:"serde-yaml property writer should append buffer contents";
      Ok written
    ...
    let write_vectored = fun buffer ~from ->
      ...
      IO.IoVec.for_each from ~fn:(
        fun chunk ->
          IO.Buffer.append_slice buffer chunk |> Result.expect ~msg:"serde-yaml property writer should append slices";
...
let mode_decode =
  De.variant
    [
      ...
      De.Variant.newtype "Named" De.string
        (
          fun value -> Named value
        );
      De.Variant.newtype "Counted" De.int
        (
          fun value -> Counted value
        );
      De.Variant.newtype "Sampled" De.float
        (
          fun value -> Sampled value
        );
    ]
...
let sample_decode = De.record_mut ~fields:sample_fields ~create:(
  ...
  fun builder ->
    match builder.ready, builder.count, builder.small, builder.big, builder.ratio, builder.label, builder.alias, builder.mode, builder.companion, builder.marker, builder.home, builder.tags, builder.scores with
    | (Some ready, Some count, Some small, Some big, Some ratio, Some label, Some alias, Some mode, Some companion, Some marker, Some home, Some tags, Some scores) -> ({
      ready;
      count;
      small;
      big;
...
let sample_encode =
  Ser.record
    (
      Ser.fields
        [
          Ser.field "ready" Ser.bool
            (
              fun (value: sample) -> value.ready
            );
          ...
          Ser.field "label" Ser.string
            (
              fun (value: sample) -> value.label
            );
          Ser.field "alias" (Ser.option Ser.string)
            (
              fun (value: sample) -> value.alias
            );
          Ser.field "mode" mode_encode
            (
              fun (value: sample) -> value.mode
            );
          Ser.field "companion" companion_encode
            (
              fun (value: sample) -> value.companion
            );
...
let equal_berth = fun (left: berth) (right: berth) -> String.equal left.island right.island && Int.equal left.berth right.berth

let equal_string_vec = fun left right -> vec_to_list left = vec_to_list right

let equal_sample = fun (left: sample) (right: sample) -> Bool.equal left.ready right.ready && Int.equal left.count right.count && Int32.equal left.small right.small && Int64.equal left.big right.big && Float.equal left.ratio right.ratio && String.equal left.label right.label && left.alias = right.alias && equal_mode left.mode right.mode && equal_companion left.companion right.companion && equal_berth left.home right.home && equal_string_vec left.tags right.tags && left.scores = right.scores
...
let print_berth = fun (value: berth) -> "{ island = " ^ Printer.string value.island ^ "; berth = " ^ Printer.int value.berth ^ " }"
...
let sample_gen =
  Generator.map3
    (
      fun (((ready, count), small), (big, ratio)) (label, alias, tags) (scores, mode, companion, home) -> ({
        ready;
        count;
        small;
        big;
    ...
    (Generator.pair (Generator.pair (Generator.pair Generator.bool Generator.int) Generator.int32) (Generator.pair Generator.int64 finite_float_gen))
    ...
    (
      Generator.map
        ...
        (Generator.pair (Generator.pair (Generator.array Generator.int) mode_gen) (Generator.pair companion_gen berth_gen))
    )
...
let roundtrip_io = fun encode decode equal value ->
  ...
  | Ok () -> (
    match Serde_yaml.from_reader decode (String.to_reader ~chunk_size:io_chunk_size (IO.Buffer.contents buffer)) with
...
let bool_roundtrip_prop = run_property "serde-yaml property bool roundtrips" Arbitrary.bool (roundtrip_in_memory Ser.bool De.bool Bool.equal)

let int_roundtrip_prop = run_property "serde-yaml property int roundtrips" Arbitrary.int (roundtrip_in_memory Ser.int De.int Int.equal)

let int32_roundtrip_prop = run_property "serde-yaml property int32 roundtrips" Arbitrary.int32 (roundtrip_in_memory Ser.int32 De.int32 Int32.equal)

let int64_roundtrip_prop = run_property "serde-yaml property int64 roundtrips" Arbitrary.int64 (roundtrip_in_memory Ser.int64 De.int64 Int64.equal)

let float_roundtrip_prop = run_property "serde-yaml property float roundtrips" finite_float_arb (roundtrip_in_memory Ser.float De.float Float.equal)

let string_roundtrip_prop = run_property "serde-yaml property string roundtrips" Arbitrary.string (roundtrip_in_memory Ser.string De.string String.equal)

let option_string_roundtrip_prop = run_property "serde-yaml property option string roundtrips" Arbitrary.(option string) (roundtrip_in_memory (Ser.option Ser.string) (De.option De.string) (=))

let mode_roundtrip_prop = run_property ~examples:composite_examples "serde-yaml property mode roundtrips" mode_arb (roundtrip_in_memory mode_encode mode_decode equal_mode)

let companion_roundtrip_prop = run_property ~examples:composite_examples "serde-yaml property companion roundtrips" companion_arb (roundtrip_in_memory companion_encode companion_decode equal_companion)

let berth_roundtrip_prop = run_property ~examples:composite_examples "serde-yaml property berth roundtrips" berth_arb (roundtrip_in_memory berth_encode berth_decode equal_berth)

let string_vec_roundtrip_prop = run_property ~examples:composite_examples "serde-yaml property string vectors roundtrip" Arbitrary.(vector string) (roundtrip_in_memory (Ser.list Ser.string) (De.list De.string) equal_string_vec)

let int_array_roundtrip_prop = run_property ~examples:composite_examples "serde-yaml property int arrays roundtrip" Arbitrary.(array int) (roundtrip_in_memory (Ser.array Ser.int) (De.array De.int) (=))

let sample_roundtrip_prop = run_property ~examples:composite_examples "serde-yaml property sample roundtrips" sample_arb (roundtrip_in_memory sample_encode sample_decode equal_sample)

let sample_io_roundtrip_prop = run_property ~examples:composite_examples "serde-yaml property sample roundtrips over io" sample_arb (roundtrip_io sample_encode sample_decode equal_sample)
