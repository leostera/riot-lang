...
let preprocess = fun input ->
  ...
  let lines = ref [] in
  raw_lines |> List.enumerate |> List.for_each ~fn:(
    fun (index, raw_line) ->
      ...
      if not (String.equal (String.trim line) "") then
        ...
        let text = String.sub line ~offset:indent ~len:(String.length line - indent) in lines := { number; indent; text } :: !lines
  );
  ...
  if index >= String.length text then
    ...
    | 'u' ->
        if index + 4 >= String.length text then
          ...
          in
          let code = (hex_value (String.unsafe_get text (index + 1)) lsl 12) lor (hex_value (String.unsafe_get text (index + 2)) lsl 8) lor (hex_value (String.unsafe_get text (index + 3)) lsl 4) lor hex_value (String.unsafe_get text (index + 4)) in
          let rune =
            ...
            if Char.equal current '\'' then
              ...
              if Char.equal current ':' then
                ...
                if next >= length then
                  ...
                  if Char.equal (String.unsafe_get text next) ' ' || Char.equal (String.unsafe_get text next) '\t' then
                    Some index
                  else scan (index + 1) false false
              else scan (index + 1) false false
  in
...
let parse_key = fun text line_number ->
  ...
  if String.equal trimmed "" then
    ...
    if Char.equal (String.unsafe_get trimmed 0) '"' then
      ...
      if Int.equal (skip_spaces trimmed next) (String.length trimmed) then
        key
      else fail_line line_number "unexpected trailing characters after quoted key"
    else
      if Char.equal (String.unsafe_get trimmed 0) '\'' then
...
let split_mapping_head = fun line ->
  ...
  | Some index ->
      ...
      let rest = String.sub line.text ~offset:(index + 1) ~len:(String.length line.text - index - 1) |> String.trim in Some (key, rest)
  ...
  let normalized =
    if String.equal value ".inf" then
      ...
      if String.equal value "-.inf" then
        ...
        if String.equal value ".nan" then
          "nan"
        else value
  in
...
let parse_scalar_text = fun text line_number ->
  ...
  if String.equal value "null" || String.equal value "~" || String.equal value "" then
    ...
    if String.equal value "true" then
      Bool true
    else
      if String.equal value "false" then
        Bool false
      else
          ...
          if String.equal value "{}" then
            ...
            if Char.equal (String.unsafe_get value 0) '"' then
              ...
              if Int.equal (skip_spaces value next) (String.length value) then
                String string_value
              else fail_line line_number "unexpected trailing characters after string literal"
            else
              if Char.equal (String.unsafe_get value 0) '\'' then
                ...
                | _ ->
                    if String.contains value "." || String.contains value "e" || String.contains value "E" || String.equal value ".inf" || String.equal value "-.inf" || String.equal value ".nan" then
...
and parse_tagged = fun lines line index head parent_indent ->
  ...
  let rest =
    if Int.equal tag_end length then
      ""
    else String.sub head ~offset:tag_end ~len:(length - tag_end) |> String.trim
  in
  if String.equal rest "" then
    let payload, next = parse_nested_or_null lines index parent_indent in (Tagged (tag, payload), next)
  else (Tagged (tag, parse_scalar_text rest line.number), index)
and parse_value_head = fun lines line index head parent_indent ->
  ...
  if String.equal head "" then
    ...
    if Char.equal (String.unsafe_get head 0) '!' then
      parse_tagged lines line index head parent_indent
    else (parse_scalar_text head line.number, index)
and parse_sequence = fun lines start_index indent ->
  ...
  while !continue && !index < Array.length lines do
    ...
    if not (Int.equal line.indent indent) then
      ...
      if String.equal line.text "-" then
        let value, next = parse_nested_or_null lines (!index + 1) indent in items := value :: !items;
        index := next
      else
        if String.starts_with ~prefix:"- " line.text then
          let head = String.sub line.text ~offset:2 ~len:(String.length line.text - 2) in
          let value, next = parse_value_head lines line (!index + 1) head indent in items := value :: !items;
          index := next
        else continue := false
  done;
...
and parse_node = fun lines index indent ->
  if index >= Array.length lines then
    ...
    if not (Int.equal line.indent indent) then
      ...
      if String.equal line.text "-" || String.starts_with ~prefix:"- " line.text then
        parse_sequence lines index indent
      else
        match split_mapping_head line with
        | Some _ -> parse_mapping lines index indent
        | None ->
            let value, next = parse_value_head lines line (index + 1) line.text indent in (value, next)
  ...
  try
    ...
    if Int.equal (Array.length lines) 0 then
      ...
      if Int.equal next (Array.length lines) then
        ...
        let line = Array.get_unchecked lines ~at:next in Error (`Msg ("unexpected trailing YAML content at line " ^ Int.to_string line.number))
  with
