...
type state = { mutable current: Yaml_value.t }
    ...
    if Int.equal index (Array.length cases) then
      ...
      | De.Unit (case_tag, result) ->
          if String.equal tag case_tag then
            match payload with
            | None | Some Yaml_value.Null -> result
            | _ -> find_unit tag payload (index + 1)
          else find_unit tag payload (index + 1)
...
let from_string = fun decode input ->
  let* document = Parse.parse_document input in De.run decode backend { current = document }
