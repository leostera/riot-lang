...
let test_top_level_break_flattens_when_it_fits = fun _ctx ->
  Test.assert_equal ~expected:"hello world" ~actual:(format ~width:32 [ str "hello"; brk; str "world" ]);
  Ok ()

let test_top_level_break_wraps_when_it_does_not_fit = fun _ctx ->
  Test.assert_equal ~expected:"hello\nworld" ~actual:(format ~width:10 [ str "hello"; brk; str "world" ]);
  Ok ()
...
let test_softline_disappears_in_flat_mode = fun _ctx ->
  Test.assert_equal ~expected:"alphabeta" ~actual:(format ~width:32 [ group [ str "alpha"; softline; str "beta" ] ]);
  Ok ()

let test_join_interleaves_separator = fun _ctx ->
  Test.assert_equal ~expected:"alpha, beta, gamma" ~actual:(format ~width:32 [ join (concat [ str ","; brk ]) [ str "alpha"; str "beta"; str "gamma" ] ]);
  Ok ()
