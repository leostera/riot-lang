...
let hex_byte = fun value -> String.init ~len:2 ~fn:(
  fun index ->
    if Int.equal index 0 then
      hex_char ((value lsr 4) land 0x0f)
    else hex_char (value land 0x0f)
)
...
let int32_of_length = fun kind value ->
  if value > Int32.to_int Int32.max_int then
    length_error kind
  else Ok (Int32.of_int value)

let add_int32_le = fun buffer value -> let open Int32 in
let add_byte shift = IO.Buffer.add_char buffer (Char.chr (to_int (logand (shift_right_logical value shift) 0xffl))) in
add_byte 0;
add_byte 8;
add_byte 16;
add_byte 24

let add_int64_le = fun buffer value -> let open Int64 in
let add_byte shift = IO.Buffer.add_char buffer (Char.chr (to_int (logand (shift_right_logical value shift) 0xffL))) in
add_byte 0;
add_byte 8;
add_byte 16;
...
let rec encode_value = function
  ...
  | String value ->
      let* encoded_length = int32_of_length "string" (String.length value + 1)
      in
  ...
  | Document fields ->
      let* encoded = encode_document fields in Ok (type_document, encoded)
  | Array values ->
...
and encode_element = fun (key, value) ->
  ...
  let* () = add_cstring buffer key
  in
  IO.Buffer.add_string buffer payload;
  Ok (IO.Buffer.contents buffer)
and encode_document = fun fields ->
  let* encoded_fields = List.fold_left fields ~init:(Ok []) ~fn:(
    fun acc field ->
      let* acc = acc
      in
      let* encoded = encode_element field in Ok (encoded :: acc)
  )
  in
  let encoded_fields = List.rev encoded_fields in
  let payload_len = List.fold_left encoded_fields ~init:0 ~fn:(
    fun total field -> total + String.length field
  ) in
  let* encoded_length = int32_of_length "document" (payload_len + 5)
  in
...
let read_int32_le = fun input ->
  let* b0 = read_byte input
  in
  let* b1 = read_byte input
  in
  let* b2 = read_byte input
  in
  let* b3 = read_byte input in Ok Int32.(logor (of_int b0) (logor (shift_left (of_int b1) 8) (logor (shift_left (of_int b2) 16) (shift_left (of_int b3) 24))))
...
let* b2 = read_byte input
in
let* b3 = read_byte input
in
let* b4 = read_byte input
in
let* b5 = read_byte input
in
let* b6 = read_byte input
in
let* b7 = read_byte input in Ok (logor (of_int b0) (logor (shift_left (of_int b1) 8) (logor (shift_left (of_int b2) 16) (logor (shift_left (of_int b3) 24) (logor (shift_left (of_int b4) 32) (logor (shift_left (of_int b5) 40) (logor (shift_left (of_int b6) 48) (shift_left (of_int b7) 56))))))))

let read_double_le = fun input ->
  let* bits = read_int64_le input in Ok (Int64.float_of_bits bits)
...
let read_cstring = fun input ->
  ...
  let rec loop index =
    if index >= input.length then
      ...
      if Char.equal (String.unsafe_get input.source index) '\x00' then
        (
...
let read_length_prefixed_string = fun input ->
  let* length = read_int32_le input
  in
  let* length = int_of_positive_int32 "string" length 1l
  in
  let* () = ensure input length
  in
  ...
  if not (Char.equal (String.unsafe_get input.source terminator_index) '\x00') then
    ...
    let value = String.sub input.source ~offset:input.pos ~len:text_len in input.pos <- input.pos + length;
  Ok value
...
let array_of_document = fun fields ->
  let rec loop index fields acc =
    ...
    | (key, value) :: rest ->
        ...
        else error ("serde-bson array keys must be sequential numeric strings, got '" ^ key ^ "'")
  in
  loop 0 fields []

let rec read_value = fun input kind end_pos ->
  match kind with
  | 0x01 -> read_double_le input |> Result.map ~fn:(
    fun value -> Double value
  )
  | 0x02 -> read_length_prefixed_string input |> Result.map ~fn:(
    fun value -> String value
  )
  | 0x03 -> read_document_body input end_pos |> Result.map ~fn:(
    fun value -> Document value
  )
  | 0x04 ->
      let* fields = read_document_body input end_pos
      in
      let* values = array_of_document fields in Ok (Array values)
  | 0x08 ->
      ...
      if Int.equal value 0 then
        ...
        if Int.equal value 1 then
          Ok (Bool true)
        else error "serde-bson bool payload must be 0 or 1"
  | 0x0a -> Ok Null
  | 0x10 -> read_int32_le input |> Result.map ~fn:(
    fun value -> Int32 value
  )
  | 0x12 -> read_int64_le input |> Result.map ~fn:(
    fun value -> Int64 value
  )
...
and read_document_body = fun input _parent_end ->
  let start = input.pos in
  let* declared_length = read_int32_le input
  in
  let* declared_length = int_of_length_int32 declared_length
  in
  ...
  if end_pos > input.length then
    ...
    let rec loop acc =
      if Int.equal input.pos (end_pos - 1) then
        let* terminator = read_byte input
        in
        if Int.equal terminator 0 then
          Ok (List.rev acc)
        else error "serde-bson document terminator must be 0"
      else
        if input.pos > end_pos - 1 then
          error "serde-bson document overran its declared length"
        else
          let* kind = read_byte input
          in
          if Int.equal kind 0 then
            ...
            in
            let* value = read_value input kind end_pos in loop ((key, value) :: acc)
    in
    loop []
...
let from_string = fun input ->
  ...
  let* fields = read_document_body state state.length
  in
  if Int.equal state.pos state.length then
    Ok (Document fields)
  else error "serde-bson input has trailing bytes after the top-level document"
