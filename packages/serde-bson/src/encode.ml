...
type state = { mutable value: Bson_value.t option }
...
and backend: state Ser.backend = {
  bool = (
    fun state value -> set state (Bson_value.Bool value)
  );
  string = (
    fun state value -> set state (Bson_value.String value)
  );
  int = (
    fun state value -> set state (Bson_value.Int64 (Int64.of_int value))
  );
  int32 = (
    fun state value -> set state (Bson_value.Int32 value)
  );
  int64 = (
    fun state value -> set state (Bson_value.Int64 value)
  );
  float = (
    fun state value -> set state (Bson_value.Double value)
  );
  null = (
    fun state -> set state Bson_value.Null
  );
  option = (
