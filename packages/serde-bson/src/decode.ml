...
type state = { mutable current: Bson_value.t }
...
and record_backend: 'field 'acc 'value. state -> fields:'field De.Fields.t -> init:'acc -> step:('acc -> 'field option -> 'acc) -> finish:('acc -> 'value) -> 'value = fun state ~fields ~init ~step ~finish ->
  let acc = ref init in
  List.for_each (expect_document state.current) ~fn:(
    fun (key, field_value) ->
...
and backend: state De.backend = {
  ...
  int = (
    fun state -> int_of_int64 (int64_of_value state.current)
  );
  int32 = (
    fun state -> int32_of_int64 (int64_of_value state.current)
  );
  int64 = (
    fun state -> int64_of_value state.current
  );
  float = (
    fun state ->
      match state.current with
...
let from_string = fun decode input ->
  let* document = Wire.from_string input in De.run decode backend { current = document }

let from_reader = fun decode reader ->
  let* document = Wire.from_reader reader in De.run decode backend { current = document }
