...
let io_writer_of_buffer =
  let module Write = struct
    ...
    let write = fun buffer ~from ->
      ...
      IO.Buffer.append_slice buffer (IO.Buffer.readable from) |> Result.expect ~msg:"serde-bson bench writer should append buffer contents";
      Ok written
    ...
    let write_vectored = fun buffer ~from ->
      ...
      IO.IoVec.for_each ~fn:(
        fun chunk ->
          IO.Buffer.append_slice buffer chunk |> Result.expect ~msg:"serde-bson bench writer should append slices";
...
let build_fixture = fun ({ label; tag_count; score_count; stop_count; string_repeat }: fixture_spec) ->
  ...
  in
  let encoded = Serde_bson.to_string manifest_encode value |> Result.expect ~msg:("expected " ^ label ^ " fixture to encode") in
  let decoded = Serde_bson.from_string manifest_decode encoded |> Result.expect ~msg:("expected " ^ label ^ " fixture to decode") in
  if not (equal_manifest value decoded) then
    panic ("serde_bson_bench: fixture roundtrip failed for " ^ label);
  { label; value; encoded }
...
let bench_encode_in_memory = fun fixture () -> ignore (Serde_bson.to_string manifest_encode fixture.value)

let bench_encode_writer = fun fixture () ->
  let buffer = IO.Buffer.create ~size:(String.length fixture.encoded) in ignore (Serde_bson.to_writer manifest_encode (io_writer_of_buffer buffer) fixture.value)

let bench_decode_in_memory = fun fixture () -> ignore (Serde_bson.from_string manifest_decode fixture.encoded)

let bench_decode_reader = fun fixture () -> ignore (Serde_bson.from_reader manifest_decode (String.to_reader ~chunk_size:io_chunk_size fixture.encoded))
...
let benchmark_suite = fun fixture ->
  ...
  in
  Bench.[
    with_config ~config ("serde-bson encode in-memory " ^ fixture.label ^ " dataset (" ^ size ^ ")") (bench_encode_in_memory fixture);
    with_config ~config ("serde-bson encode writer " ^ fixture.label ^ " dataset (" ^ size ^ ")") (bench_encode_writer fixture);
    with_config ~config ("serde-bson decode in-memory " ^ fixture.label ^ " dataset (" ^ size ^ ")") (bench_decode_in_memory fixture);
    with_config ~config ("serde-bson decode reader " ^ fixture.label ^ " dataset (" ^ size ^ ")") (bench_decode_reader fixture);
  ]

let main ~args =
  let small_fixture = build_fixture small_fixture_spec in
  let large_fixture = build_fixture large_fixture_spec in
  let benchmarks = benchmark_suite small_fixture @ benchmark_suite large_fixture in Bench.Cli.main ~name:"serde-bson benchmarks" ~benchmarks ~args
