...
let build_fixture = fun
  ...
  in
  let encoded = Serde_bson.to_string manifest_encode value
  |> Result.expect ~msg:("expected " ^ label ^ " fixture to encode") in
  let decoded = Serde_bson.from_string manifest_decode encoded
...
let bench_encode_in_memory = fun fixture () ->
  ignore (Serde_bson.to_string manifest_encode fixture.value)

let bench_encode_writer = fun fixture () ->
  let buffer = IO.Buffer.create ~size:(String.length fixture.encoded) in
  ignore (Serde_bson.to_writer manifest_encode (io_writer_of_buffer buffer) fixture.value)

let bench_decode_in_memory = fun fixture () ->
  ignore (Serde_bson.from_string manifest_decode fixture.encoded)

let bench_decode_reader = fun fixture () ->
  ignore
    (Serde_bson.from_reader
      manifest_decode
...
let benchmark_suite = fun fixture ->
  ...
  Bench.[
    with_config
      ~config
      ("serde-bson encode in-memory " ^ fixture.label ^ " dataset (" ^ size ^ ")")
      (bench_encode_in_memory fixture);
    with_config
      ~config
      ("serde-bson encode writer " ^ fixture.label ^ " dataset (" ^ size ^ ")")
      (bench_encode_writer fixture);
    with_config
      ~config
      ("serde-bson decode in-memory " ^ fixture.label ^ " dataset (" ^ size ^ ")")
      (bench_decode_in_memory fixture);
    with_config
      ~config
      ("serde-bson decode reader " ^ fixture.label ^ " dataset (" ^ size ^ ")")
...
let () =
  Actors.run
    ~main:(fun ~args ->
      ...
      Bench.Cli.main ~name:"serde-bson benchmarks" ~benchmarks ~args)
    ~args:Env.args
    ()
