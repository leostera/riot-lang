...
let box_encode = fun encode ->
  Ser.record (Ser.fields [ Ser.field "value" encode (fun (box: 'value box) -> box.value) ])
...
let mode_roundtrip_prop = run_property
  ...
  "serde-bson property mode roundtrips"
  mode_arb
  (roundtrip_in_memory mode_encode mode_decode ( = ))

let berth_roundtrip_prop = run_property
  ~examples:composite_examples
  "serde-bson property berth roundtrips"
  berth_arb
  (roundtrip_record_in_memory berth_encode berth_decode ( = ))

let string_vec_roundtrip_prop = run_property
  ~examples:composite_examples
  "serde-bson property string vectors roundtrip"
...
let int_array_roundtrip_prop = run_property
  ~examples:composite_examples
  "serde-bson property int arrays roundtrip"
  Arbitrary.(array int)
  (roundtrip_in_memory (Ser.array Ser.int) (De.array De.int) ( = ))

let sample_roundtrip_prop = run_property
  ~examples:composite_examples
  "serde-bson property sample roundtrips"
  sample_arb
  (roundtrip_record_in_memory sample_encode sample_decode equal_sample)

let sample_io_roundtrip_prop = run_property
  ~examples:composite_examples
  "serde-bson property sample roundtrips over io"
  sample_arb
  (roundtrip_record_io sample_encode sample_decode equal_sample)
...
let () =
  Actors.run
    ~main:(fun ~args -> Test.Cli.main ~name:"serde_bson_property_tests" ~tests ~args ())
    ~args:Env.args
    ()
