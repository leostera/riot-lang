...
let io_writer_of_buffer =
  let module Write = struct
    ...
    let write = fun buffer ~from ->
      ...
      IO.Buffer.append_slice buffer (IO.Buffer.readable from) |> Result.expect ~msg:"serde-bson property writer should append buffer contents";
      Ok written
    ...
    let write_vectored = fun buffer ~from ->
      ...
      IO.IoVec.for_each ~fn:(
        fun chunk ->
          IO.Buffer.append_slice buffer chunk |> Result.expect ~msg:"serde-bson property writer should append slices";
...
type 'value box = { value: 'value }

type box_field =
  | Field_value

type 'value box_builder = { mutable value: 'value option }

let box_fields = De.fields [ De.field "value" Field_value ]

let box_decode = fun decode -> De.record_mut ~fields:box_fields ~create:(
  fun (): 'value box_builder -> { value = None }
) ~step:(
...
let box_encode = fun encode ->
  Ser.record
    (
      Ser.fields
        [
          Ser.field "value" encode
            (
              fun (box: 'value box) -> box.value
            );
        ]
    )
...
let mode_decode =
  De.variant
    [
      ...
      De.Variant.newtype "Navigator" De.string
        (
          fun value -> Navigator value
        );
    ]
...
let sample_decode = De.record_mut ~fields:sample_fields ~create:(
  ...
  fun builder ->
    match builder.ready, builder.count, builder.small, builder.big, builder.ratio, builder.label, builder.alias, builder.mode, builder.marker, builder.home, builder.tags, builder.scores with
...
let print_berth = fun (value: berth) -> "{ island = " ^ quote value.island ^ "; berth = " ^ Int.to_string value.berth ^ " }"
...
let roundtrip_in_memory = fun encode decode equal value ->
  ...
  | Ok encoded -> (
    ...
    | Ok decoded -> equal decoded.value value
    | Error err -> fail ("decode failed: " ^ Serde.Error.to_string err)
  )
  | Error err -> fail ("encode failed: " ^ Serde.Error.to_string err)

let roundtrip_record_in_memory = fun encode decode equal value ->
  match Serde_bson.to_string encode value with
  | Ok encoded -> (
    match Serde_bson.from_string decode encoded with
    | Ok decoded -> equal decoded value
    | Error err -> fail ("decode failed: " ^ Serde.Error.to_string err)
  )
  | Error err -> fail ("encode failed: " ^ Serde.Error.to_string err)

let roundtrip_record_io = fun encode decode equal value ->
  let buffer = IO.Buffer.create ~size:64 in
  match Serde_bson.to_writer encode (io_writer_of_buffer buffer) value with
  | Ok () -> (
    match Serde_bson.from_reader decode (String.to_reader ~chunk_size:io_chunk_size (IO.Buffer.contents buffer)) with
    | Ok decoded -> equal decoded value
...
let bool_roundtrip_prop = run_property "serde-bson property bool roundtrips" Arbitrary.bool (roundtrip_in_memory Ser.bool De.bool Bool.equal)

let int_roundtrip_prop = run_property "serde-bson property int roundtrips" Arbitrary.int (roundtrip_in_memory Ser.int De.int Int.equal)

let int32_roundtrip_prop = run_property "serde-bson property int32 roundtrips" Arbitrary.int32 (roundtrip_in_memory Ser.int32 De.int32 Int32.equal)

let int64_roundtrip_prop = run_property "serde-bson property int64 roundtrips" Arbitrary.int64 (roundtrip_in_memory Ser.int64 De.int64 Int64.equal)

let float_roundtrip_prop = run_property "serde-bson property float roundtrips" (Arbitrary.make ~print:Float.to_string finite_float_gen) (roundtrip_in_memory Ser.float De.float Float.equal)

let string_roundtrip_prop = run_property "serde-bson property string roundtrips" Arbitrary.string (roundtrip_in_memory Ser.string De.string String.equal)

let option_string_roundtrip_prop = run_property "serde-bson property option string roundtrips" Arbitrary.(option string) (roundtrip_in_memory (Ser.option Ser.string) (De.option De.string) (=))

let mode_roundtrip_prop = run_property ~examples:composite_examples "serde-bson property mode roundtrips" mode_arb (roundtrip_in_memory mode_encode mode_decode (=))

let berth_roundtrip_prop = run_property ~examples:composite_examples "serde-bson property berth roundtrips" berth_arb (roundtrip_record_in_memory berth_encode berth_decode (=))

let string_vec_roundtrip_prop = run_property ~examples:composite_examples "serde-bson property string vectors roundtrip" Arbitrary.(vector string) (roundtrip_in_memory (Ser.list Ser.string) (De.list De.string) equal_string_vec)

let int_array_roundtrip_prop = run_property ~examples:composite_examples "serde-bson property int arrays roundtrip" Arbitrary.(array int) (roundtrip_in_memory (Ser.array Ser.int) (De.array De.int) (=))

let sample_roundtrip_prop = run_property ~examples:composite_examples "serde-bson property sample roundtrips" sample_arb (roundtrip_record_in_memory sample_encode sample_decode equal_sample)

let sample_io_roundtrip_prop = run_property ~examples:composite_examples "serde-bson property sample roundtrips over io" sample_arb (roundtrip_record_io sample_encode sample_decode equal_sample)
...
let main ~args = Test.Cli.main ~name:"serde_bson_property_tests" ~tests ~args ()
