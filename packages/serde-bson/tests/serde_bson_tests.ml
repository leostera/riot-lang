...
let manifest_decode = De.record_mut ~fields:manifest_fields ~create:(
  ...
  fun builder ->
    match builder.ship, builder.emergency, builder.crew_count, builder.small, builder.bounty, builder.heading, builder.nickname, builder.mode, builder.marker, builder.home, builder.tags, builder.scores with
...
let equal_manifest = fun (left: manifest) (right: manifest) -> String.equal left.ship right.ship && Bool.equal left.emergency right.emergency && Int.equal left.crew_count right.crew_count && Int32.equal left.small right.small && Int64.equal left.bounty right.bounty && Float.equal left.heading right.heading && left.nickname = right.nickname && left.mode = right.mode && left.home = right.home && vec_to_list left.tags = vec_to_list right.tags && left.scores = right.scores
...
let test_variant_payload_uses_singleton_document = fun _ctx ->
  ...
  | Ok encoded ->
      if List.contains (byte_values encoded) ~value:0x03 then
        Ok ()
      else Error "expected BSON variant payload to include an embedded document type"
  | Error err -> Error ("encode failed: " ^ Serde.Error.to_string err)
