...
let io_writer_of_buffer =
  let module Write = struct
    ...
    let write = fun buffer ~from ->
      ...
      |> Result.expect ~msg:"serde-bson test writer should append buffer contents";
      Ok written
    ...
    let write_vectored = fun buffer ~from ->
      ...
      IO.IoVec.for_each
        ~fn:(fun chunk ->
          ...
          |> Result.expect ~msg:"serde-bson test writer should append slices";
...
let tests = [
  ...
  Test.case
    "serde-bson variants with payloads use singleton documents"
    test_variant_payload_uses_singleton_document;
]
