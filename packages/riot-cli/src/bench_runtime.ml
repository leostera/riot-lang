...
type suite_binary = Test_runtime.suite_binary = { package_name: Package_name.t; suite_name: string }
...
let is_benchmark_binary_name = fun name -> String.ends_with ~suffix:"_bench" name || String.ends_with ~suffix:"-bench" name
...
let realized_bench_packages = fun ?(package_filters = []) (workspace: Workspace.t) -> Workspace.realize_packages ~intent:Package.Bench workspace |> List.filter ~fn:Package.is_workspace_member |> List.filter ~fn:(
  fun (pkg: Package.t) -> matches_package_filters package_filters pkg.name
)

let collect_suite_binaries = fun (workspace: Workspace.t) ?(package_filters = []) ?suite_filter () -> realized_bench_packages ~package_filters workspace |> List.flat_map ~fn:(
...
let find_suite_source_path = fun ~(workspace:Workspace.t) (suite: suite_binary) ->
  match List.find (realized_bench_packages workspace) ~fn:(
...
let running_bench_case_of_json = fun json ->
  ...
  in
  let* event_type = get_string type_json
  in
  if not (String.equal event_type "BenchCaseStarted") then
    ...
    in
    let* iterations_json = field "iterations" fields
    in
    let* warmup_json = field "warmup" fields
    in
    let* index = get_int index_json
    in
    let* name = get_string name_json
    in
    let* iterations = get_int iterations_json
    in
    let* warmup = get_int warmup_json
    in
    Ok (
      Some {
        index;
...
let duration_of_nanos_json = fun json ->
  let* nanos = get_int json in Ok (Time.Duration.from_nanos nanos)

let gc_stats_of_json = fun json ->
  let* fields = get_object json
  in
  let* minor_collections_json = field "minor_collections" fields
  in
  let* major_collections_json = field "major_collections" fields
  in
  let* compactions_json = field "compactions" fields
  in
  let* minor_collections = get_int minor_collections_json
  in
  let* major_collections = get_int major_collections_json
  in
  let* compactions = get_int compactions_json in Ok { minor_collections; major_collections; compactions }

let statistics_of_json = fun json ->
  let* fields = get_object json
  in
  let* min_json = field "min_nanos" fields
  in
  let* max_json = field "max_nanos" fields
  in
  let* mean_json = field "mean_nanos" fields
  in
  let* median_json = field "median_nanos" fields
  in
  let* std_dev_json = field "std_dev_nanos" fields
  in
  let* iterations_json = field "iterations" fields
  in
  let* total_time_json = field "total_time_nanos" fields
  in
  let* gc_json = field "gc" fields
  in
  let* min = duration_of_nanos_json min_json
  in
  let* max = duration_of_nanos_json max_json
  in
  let* mean = duration_of_nanos_json mean_json
  in
  let* median = duration_of_nanos_json median_json
  in
  let* std_dev = duration_of_nanos_json std_dev_json
  in
  let* iterations = get_int iterations_json
  in
  let* total_time = duration_of_nanos_json total_time_json
  in
  let* gc = gc_stats_of_json gc_json
  in
  Ok {
    min;
    max;
...
let bench_status_of_json = fun json ->
  ...
  match status with
  | "completed" ->
      let* statistics_json = field "statistics" fields
      in
      let* statistics = statistics_of_json statistics_json in Ok (Completed statistics)
  | "failed" ->
...
let bench_result_of_json = fun json ->
  ...
  let* name = get_string name_json
  in
  let* result = bench_status_of_json json in Ok { index; name; result }
...
let speedup_ratio_of_json = fun json ->
  ...
  let* name_json = field "name" fields
  in
  let* ratio_json = field "ratio" fields
  in
  let* name = get_string name_json
  in
  let* ratio = get_float ratio_json in Ok (name, ratio)
...
let listed_bench_item_of_json = fun json ->
  ...
  let* name_json = field "name" fields
  in
  let* kind_json = field "kind" fields
  in
  let* iterations_json = field "iterations" fields
  in
  let* warmup_json = field "warmup" fields
  ...
  let* name = get_string name_json
  in
  let* kind_name = get_string kind_json
  in
  let* iterations = get_int iterations_json
  in
  let* warmup = get_int warmup_json
  ...
  let cases =
    match List.find fields ~fn:(
      fun (field_name, _) -> String.equal field_name "cases"
    ) with
    | None -> Ok []
    | Some (_, value) ->
        let* values = get_array value
        in
        let rec loop acc = function
          | [] -> Ok (List.reverse acc)
          | value :: rest ->
              let* name = get_string value in loop (name :: acc) rest
        in
        loop [] values
  in
  let* skip = skip
...
let comparison_case_result_of_json = fun json ->
  ...
  let* name = get_string name_json
  in
  let* statistics = statistics_of_json statistics_json in Ok { name; statistics }

let comparison_result_of_json = fun json ->
  let* fields = get_object json
  in
  let* description_json = field "description" fields
  in
  let* fastest_json = field "fastest" fields
  in
  let* case_results_json = field "case_results" fields
  in
  let* speedup_ratios_json = field "speedup_ratios" fields
  in
  let* description = get_string description_json
  in
  let* fastest = get_string fastest_json
  in
  let* case_results_values = get_array case_results_json
  in
  let* speedup_ratio_values = get_array speedup_ratios_json
  in
  ...
  let rec parse_ratios acc = function
    | [] -> Ok (List.reverse acc)
    | item :: rest ->
        let* parsed = speedup_ratio_of_json item in parse_ratios (parsed :: acc) rest
  in
  let* case_results = parse_cases [] case_results_values
  in
  let* speedup_ratios = parse_ratios [] speedup_ratio_values
  in
  Ok {
    description;
    case_results;
...
let bench_summary_of_json = fun json ->
  ...
  in
  let* completed_json = field "completed" fields
  in
  ...
  in
  let* completed = get_int completed_json
  in
...
let parse_bench_suite_output = fun stdout ->
  ...
  let* fields = get_object json
  in
  let* benchmarks_json = field "benchmarks" fields
  in
  let* comparisons_json = field "comparisons" fields
  in
  let* summary_json = field "summary" fields
  in
  let* benchmark_values = get_array benchmarks_json
  in
  let* comparison_values = get_array comparisons_json
  in
  let rec parse_results acc = function
    | [] -> Ok (List.reverse acc)
    | item :: rest ->
        let* parsed = bench_result_of_json item in parse_results (parsed :: acc) rest
  in
  ...
  in
  let* results = parse_results [] benchmark_values
  in
  let* comparisons = parse_comparisons [] comparison_values
  in
  let* summary = bench_summary_of_json summary_json
  in
  let* started_at_us = optional_int_field "started_at_us" fields
  in
  let* completed_at_us = optional_int_field "completed_at_us" fields
  in
  let* duration_us = optional_int_field "duration_us" fields in Ok (prefix_stdout, started_at_us, completed_at_us, duration_us, results, comparisons, summary)
...
let bench_event_to_json = function
  ...
  | SuiteCompleted { suite; status; stdout; stderr; started_at_us; completed_at_us; duration_us; results; comparisons; summary } ->
      ...
      let comparison_to_json (comparison: bench_comparison_result) =
        Data.Json.Object [
          ...
          "case_results", Data.Json.Array (List.map comparison.case_results ~fn:(
          ...
          "speedup_ratios", Data.Json.Array (List.map comparison.speedup_ratios ~fn:(
            fun (name, ratio) ->
              Data.Json.Object [
...
let materialized_suite_binary_path = fun ~(workspace:Workspace.t) ~profile ~(suite:suite_binary) ->
  let out_dir = Riot_model.Riot_dirs.out_dir_in_workspace ~workspace ~profile ~target:(Riot_model.Riot_dirs.host_target ()) in Path.(out_dir / Path.v (Package_name.to_string suite.package_name) / Path.v suite.suite_name)
...
let find_suite_binary_path_in_output = fun ~(workspace:Workspace.t) ~profile ~(store:Riot_store.Store.t) ~(suite:suite_binary) (output: Riot_build.Build_result.t) ->
  ...
  let ensure_materialized_fallback () =
    ...
    | Ok true -> ensure_executable_binary_path ~kind:"benchmark binary" fallback_path |> Result.map_err ~fn:(
      fun reason -> SuiteArtifactNotFound { suite; reason }
    )
    | Ok false | Error _ -> Error (SuiteArtifactNotFound { suite; reason = "suite '" ^ suite.suite_name ^ "' was not produced by build output" })
  in
  match Riot_build.Build_result.find_package output suite.package_name |> Option.and_then ~fn:(
    fun package_output -> Riot_build.Build_result.find_export package_output suite.suite_name
  ) with
  | None -> ensure_materialized_fallback ()
  | Some export_entry -> (
    match Riot_store.Store.export_source_path store export_entry with
    | Some path -> ensure_executable_binary_path ~kind:"benchmark binary" path |> Result.map_err ~fn:(
      fun reason -> SuiteArtifactNotFound { suite; reason }
    )
    | None -> ensure_materialized_fallback ()
  )
...
let run_suite = fun ~on_event ~suite ~extra_args binary_path ->
  ...
  let cmd = Command.make (Path.to_string binary_path) ~env:[
  ...
  match Command.output cmd ~on_idle:(
    ...
    fun line -> suite_progress_event_of_line line |> Option.for_each ~fn:(
      fun event ->
        running_bench_case_of_json event |> Result.iter ~fn:(
          fun current_case -> current_case |> Option.for_each ~fn:(
            fun current_case -> active_case := Some current_case
          )
        );
...
let list_suite_binary_capture = fun ~suite ~extra_args binary_path ->
  ...
  let cmd = Command.make (Path.to_string binary_path) ~env:[
    "RIOT_PACKAGE_NAME", Package_name.to_string suite.package_name;
  ] ~args:("list-benchmarks" :: extra_args) in Command.output cmd
...
let parse_listed_benchmarks_output = fun stdout ->
  ...
  let* benchmarks_json = field "benchmarks" fields
  in
  let* benchmarks = get_array benchmarks_json
  in
  let rec loop acc = function
    | [] -> Ok (List.reverse acc)
    | json :: rest ->
        let* listed = listed_bench_item_of_json json in loop (listed :: acc) rest
  in
  loop [] benchmarks
...
let list_suite = fun ~(workspace:Workspace.t) ~suite ~extra_args binary_path ->
  ...
  | Ok output -> (
    ...
    | Error reason -> Error (SuiteExecutionError { suite; reason = "failed to parse benchmark results from suite '" ^ suite.suite_name ^ "': " ^ reason })
    | Ok benchmarks -> Ok { suite; source_path = find_suite_source_path ~workspace suite; benchmarks }
  )
...
let store_for_request = fun (request: bench_request) -> Riot_store.Store.create_for_lane ~workspace:request.workspace ~profile:request.profile ~target:(Riot_dirs.host_target ())
