...
command "info" |> about "Show resolved workspace or package information" |> args [ positional "target" |> required false |> help "workspace or <pkg>[@<version>]"; flag "json" |> long "json" |> help "Emit machine-readable JSON output" ]

let target_of_matches: ArgParser.matches -> target = fun matches ->
  match ArgParser.get_one matches "target" with
  | None | Some "workspace" -> Workspace_target
...
let workspace_kind = fun ~(workspace_manager:Workspace_manager.t) (workspace: Workspace_manifest.t) ->
  ...
  | Ok toml -> (
    ...
    | Some fields when List.any fields ~fn:(
      fun (name, _) -> String.equal name "workspace"
    ) -> Workspace
...
let workspace_packages = fun (workspace: Workspace_manifest.t) -> workspace.packages |> List.filter ~fn:Package_manifest.is_workspace_member |> List.sort ~compare:(
  fun (left: Package_manifest.t) (right: Package_manifest.t) -> Package_name.compare left.name right.name
)
...
let package_json = fun ~(workspace_manager:Workspace_manager.t) ~(workspace:Workspace_manifest.t) (pkg: Package_manifest.t) ->
  ...
  let fields =
    ...
    ] @ manifest_json_fields ~workspace_manager package_root
  in
  Data.Json.Object fields

let workspace_json = fun ~(workspace_manager:Workspace_manager.t) ~(load_errors:Workspace_manager.load_error list) (workspace: Workspace_manifest.t) ->
  ...
  let fields =
    [
      ...
      "packages", Data.Json.Array (workspace_packages workspace |> List.map ~fn:(package_json ~workspace_manager ~workspace));
      "load_errors", Data.Json.Array (load_errors |> List.map ~fn:Workspace_manager.load_error_to_string |> List.map ~fn:Data.Json.string);
    ] @ manifest_json_fields ~workspace_manager workspace_root
  in
  Data.Json.Object fields
...
let print_workspace = fun ~(load_errors:Workspace_manager.load_error list) (workspace: Workspace_manifest.t) ->
  ...
  workspace_packages workspace |> List.for_each ~fn:(
    fun (pkg: Package_manifest.t) ->
      ...
      println ("  - " ^ Package_name.to_string pkg.name ^ " (" ^ relative_or_absolute_path ~root:workspace.root pkg.path ^ ")");
...
let print_package = fun (info: Info_package.t) ->
  ...
  if not (List.is_empty info.load_errors) then
    (
      println "";
      println "Load errors:";
      List.for_each info.load_errors ~fn:(
        fun err -> println ("  - " ^ err)
      )
    )
  ...
  | Package_target target ->
      ...
      (
        ...
        | Ok info ->
            ...
            else print_package info;
            Ok ()
        | Error err ->
            if json then
              print_json (Info_package.error_to_json ~error:err)
            else eprintln ("\027[1;31mError\027[0m: " ^ err.message);
