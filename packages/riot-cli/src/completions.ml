...
let install_zsh_completions = fun () ->
  let home_dir =
    Env.home_dir ()
    |> Option.expect ~msg:"Could not determine home directory"
  in
  ...
  Fs.write script target
  |> Result.expect ~msg:"Failed to install completions";
      ...
      | Some shell ->
          ...
          (
            ...
            | Zsh ->
                ...
                eprintln
                  "#   riot completions --shell zsh > /opt/homebrew/share/zsh/site-functions/_riot";
