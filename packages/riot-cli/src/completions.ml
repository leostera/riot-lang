...
command "completions" |> about "Generate shell completions or list completion data" |> args
  [
    ...
    flag "binaries" |> long "binaries" |> help "List available binaries in current workspace (format: \
               binary:package)";
    flag "tests" |> long "tests" |> help "List available test binaries in current workspace (format: \
               test:package)";
    flag "benchmarks" |> long "benchmarks" |> help "List available benchmark binaries in current workspace (format: \
               bench:package)";
...
let detect_shell = fun () ->
  ...
  | Some shell_path ->
      if String.ends_with ~suffix:"/zsh" shell_path || shell_path = "zsh" then
        ...
        if String.ends_with ~suffix:"/bash" shell_path || shell_path = "bash" then
          Some Shell_completions.Bash
        else
          if String.ends_with ~suffix:"/fish" shell_path || shell_path = "fish" then
...
let run_install = fun matches -> let open ArgParser in
...
let install_command = let open ArgParser in
let open ArgParser.Arg in
command "install" |> about "Install shell completions for the current user" |> args [ option "shell" |> long "shell" |> possible_values [ "bash"; "zsh"; "fish" ] |> help "Shell to install completions for" ]

let run_install_args = fun argv -> let open ArgParser in
...
let run = fun matches ->
  ...
  let completions_command = command in let open ArgParser in
  ...
  | Some shell_str -> (
    ...
    | Some shell ->
        ...
        (
          ...
          | Bash -> eprintln "# Bash completions not yet implemented"
          | Fish -> eprintln "# Fish completions not yet implemented"
        );
        Ok ()
  )
  | None ->
      ...
      if not (has_packages || has_binaries || has_tests || has_benchmarks || has_commands) then
        let () = print_help completions_command in Ok ()
      else
        ...
        | Error _ -> (* Not in a workspace or scan failed - silently succeed *)
        Ok ()
        | Ok (workspace, _load_errors) ->
            if has_packages then
              List.for_each (Shell_completions.list_packages workspace) ~fn:(
                fun pkg -> println pkg
              );
            if has_binaries then
              List.for_each (Shell_completions.list_binaries workspace) ~fn:(
                fun bin -> println bin
              );
            if has_tests then
              List.for_each (Shell_completions.list_tests workspace) ~fn:(
                fun test -> println test
              );
            if has_benchmarks then
              List.for_each (Shell_completions.list_benchmarks workspace) ~fn:(
                fun bench -> println bench
              );
            if has_commands then
              (
                let command_lines = Shell_completions.list_commands workspace in List.for_each command_lines ~fn:println
              );
            Ok ()
