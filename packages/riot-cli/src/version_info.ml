...
let metadata_path = fun () ->
  let* riot_home = riot_home_dir () in Ok Path.(riot_home / Path.v "release.json")
...
let optional_string_field = fun fields name ->
  ...
  | None | Some (_, Data.Json.Null) -> Ok None
  | Some (_, Data.Json.String value) -> Ok (Some value)
  | Some _ -> Error ("invalid '" ^ name ^ "' field")

let metadata_of_json = function
  | Data.Json.Object fields ->
      let* release_id = string_field fields "release_id"
      in
      let* build_sha = string_field fields "build_sha"
      in
      let* notes_url = optional_string_field fields "notes_url"
      in
      let* compare_url = optional_string_field fields "compare_url"
      in
      let* issues_url = optional_string_field fields "issues_url"
      in
      Ok {
        release_id;
        build_sha;
...
let of_json_string = fun content ->
  let* json = Data.Json.of_string content |> Result.map_err ~fn:Data.Json.error_to_string in metadata_of_json json
...
let of_version_string = fun version ->
  ...
  let find_marker haystack needle =
    ...
    let rec loop index =
      if index + needle_len > haystack_len then
        ...
        if String.equal (String.sub haystack ~offset:index ~len:needle_len) needle then
          Some index
        else loop (index + 1)
    in
    loop 0
  ...
  if String.length version <= prefix_len + marker_len + suffix_len then
    ...
    if not (String.starts_with ~prefix version) then
      None
    else
      ...
      | Some marker_pos ->
          ...
          if not (String.starts_with ~prefix:build_marker build_section) then
            ...
            if not (String.ends_with ~suffix build_section) then
              None
            else
...
let of_path = fun path ->
  let* content = Fs.read path |> Result.map_err ~fn:IO.error_message in of_json_string content

let write_path = fun ~path t ->
  let* () = Fs.create_dir_all (Path.dirname path) |> Result.map_err ~fn:IO.error_message
  in
  let content = Data.Json.to_string_pretty (json_of_metadata t) ^ "\n" in Fs.write content path |> Result.map_err ~fn:IO.error_message

let same_identity = fun left right -> String.equal left.release_id right.release_id && String.equal left.build_sha right.build_sha
...
let read_installed = fun () ->
  ...
  | Ok path -> (
    ...
    | Ok true -> of_path path |> Result.to_option
  )

let write_installed = fun t ->
  let* path = metadata_path () in write_path ~path t

let version_string_of = fun metadata -> "riot " ^ metadata.release_id ^ " (build " ^ metadata.build_sha ^ ")"
