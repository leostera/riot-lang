...
type implicit_local_target = { package_name: Riot_model.Package_name.t; binary_name: string }

let no_runnable_binaries_message = fun ?package_name () ->
  let hint = "create one with `riot new --bin ./packages/my-binary`" in
  match package_name with
  | Some package_name -> "package '" ^ Riot_model.Package_name.to_string package_name ^ "' has no runnable binaries; " ^ hint
...
let parse_target = fun ?package_filter name ->
  if Riot_deps.Git_dependency.looks_like_remote_spec name then
    ...
    | None -> (
      ...
      | Error _ as err -> err
      | Ok (source_spec, binary_name) -> Ok (Remote_source { source_spec; binary_name = Option.unwrap_or ~default:(default_remote_binary_name source_spec) binary_name })
    )
  else parse_local_target ?package_filter name
...
let implicit_local_targets = fun ?package_filter (workspace: Riot_model.Workspace.t) ->
  let package_matches_filter (pkg: Riot_model.Package.t) =
    ...
    | None -> true
  in
  Riot_model.Workspace.realize_packages ~intent:Riot_model.Package.Run workspace |> List.filter ~fn:Package.is_workspace_member |> List.filter ~fn:package_matches_filter |> List.flat_map ~fn:(
    fun (pkg: Riot_model.Package.t) -> Riot_model.Package.binaries_for_scope Riot_model.Package.Normal pkg |> List.map ~fn:(
...
let resolve_implicit_local_target = fun ?package_filter (workspace: Riot_model.Workspace.t) ->
  ...
  | targets ->
      let rendered = targets |> List.map ~fn:(
        fun { package_name; binary_name } -> Riot_model.Package_name.to_string package_name ^ ":" ^ binary_name
      ) |> String.concat ", " in Error ("multiple runnable binaries found; pass a binary name or --package (" ^ rendered ^ ")")

let json_requested_for_child = fun args -> List.any args ~fn:(
  fun arg -> String.equal arg "--json"
)
...
let run_error_to_json = fun (err: Run_runtime.run_error) ->
  ...
  in
  Data.Json.Object (("type", Data.Json.String "run.error") :: ("message", Data.Json.String (Run_runtime.run_error_message err)) :: details)
...
let write_run_event = fun ~mode (event: Run_runtime.run_event) ->
  ...
  | Build.Human -> (
    match event with
    | Run_runtime.Build _ -> ()
    | Run_runtime.RunningBinary { package; binary; _ } -> out ("    \027[1;32mBuilding\027[0m " ^ Riot_model.Package_name.to_string package ^ ":" ^ binary)
  )
...
let write_run_error = fun ~mode (err: Run_runtime.run_error) ->
  ...
  | Build.Human -> (
    ...
    | Run_runtime.BinaryNotFound { binary_name } -> out ("error: binary '" ^ binary_name ^ "' not found")
...
let binary_source_label = fun ~(workspace:Riot_model.Workspace.t) (binary: Run_runtime.runnable_binary) ->
...
let write_binary_list = fun ~(workspace:Riot_model.Workspace.t) binaries -> binaries |> List.for_each ~fn:(
  fun (binary: Run_runtime.runnable_binary) -> println (Riot_model.Package_name.to_string binary.package_name ^ ":" ^ binary.binary_name ^ " (" ^ binary_source_label ~workspace binary ^ ")")
)
...
let write_binary_list_json = fun ~(workspace:Riot_model.Workspace.t) binaries ->
  ...
  let binary_json (binary: Run_runtime.runnable_binary) =
    Data.Json.Object [
      ...
      "selector", Data.Json.String (Riot_model.Package_name.to_string binary.package_name ^ ":" ^ binary.binary_name);
    ]
  in
  write_json_event
    (
...
let run_with_workspace_info = fun ~workspace ~workspace_error matches ->
  ...
  if json_mode && not list_mode then
    let message = "riot run --json is only supported with --list; use `riot run -- --json` to forward JSON to the child binary" in write_workspace_error ~mode:Build.Json message;
    ...
    if list_mode then
      ...
      | Some workspace ->
          if Option.is_some (ArgParser.get_one matches "name") then
            let message = "riot run --list does not accept a binary name" in write_workspace_error ~mode:output_mode message;
            Error (Failure message)
          else
            if not (List.is_empty extra) then
              let message = "riot run --list does not accept forwarded arguments" in write_workspace_error ~mode:output_mode message;
              Error (Failure message)
            else
      ...
      let on_event (event: Run_runtime.run_event) =
        match event with
        | Run_runtime.Build build_event -> Build.write_build_event ~mode:output_mode ~seen_registry_updates build_event
      ...
      let resolved_target =
        ...
        | None -> (
          match workspace with
          | Some workspace -> resolve_implicit_local_target ?package_filter:pkg_filter workspace |> Result.map ~fn:(
            fun { package_name; binary_name } -> Local { package_name = Some package_name; binary_name }
          ) |> Result.map_err ~fn:(
            fun err -> Failure err
          )
