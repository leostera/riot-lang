...
command "doc" |> about "Generate package documentation" |> args
  [
...
let write_doc_event = fun ~workspace_root ~mode (event: Riot_doc.event) ->
  ...
  | Human -> (
    ...
    | Riot_doc.PackageGenerationFailed { package; version; error; _ } -> out ("   \027[1;31mFailed\027[0m " ^ Package_name.to_string package ^ "@" ^ version ^ ": " ^ error)
    | Riot_doc.PackageGenerationCompleted summary ->
        if not summary.cache_hit then
          out ("   \027[1;32mGenerated\027[0m " ^ Package_name.to_string summary.package ^ "@" ^ summary.version ^ " -> " ^ display_path ~workspace_root summary.output_dir)
  )

let run = fun ~workspace matches ->
  let mode = output_mode_of_matches matches in
  let request = build_request ~workspace matches in
  let* _summaries = Riot_doc.run ~on_event:(write_doc_event ~workspace_root:workspace.root ~mode) request in Ok ()
