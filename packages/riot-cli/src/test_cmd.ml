...
command "test" |> about "Run tests with optional case filtering" |> ArgParser.allow_trailing_args |> args
  [
...
let print_empty_hint = fun package_filter suite_filter ->
  ...
  | (Some package_name, Some suite_name) -> println ("No test suite '" ^ suite_name ^ "' found in package '" ^ Riot_model.Package_name.to_string package_name ^ "'")
  | (Some package_name, None) -> println ("No test suites found in package '" ^ Riot_model.Package_name.to_string package_name ^ "'")
...
let format_duration_us = fun duration_us ->
  if duration_us < 1_000 then
    ...
    if duration_us < 1_000_000 then
      Float.to_string ~precision:2 (Float.from_int duration_us /. 1000.0) ^ "ms"
    else Float.to_string ~precision:2 (Float.from_int duration_us /. 1000000.0) ^ "s"
...
let filter_summary_tests = fun ~small_only ~large_only tests ->
  if small_only then
    List.filter tests ~fn:(
      fun (test: slow_test) -> test.size = Test_runtime.Small
    )
  else
    if large_only then
      List.filter tests ~fn:(
        fun (test: slow_test) -> test.size = Test_runtime.Large
      )
    else tests
...
let record_suite_timing = fun ~small_only ~large_only (timing: timing_summary) ~suite_label (summary: Test_runtime.test_suite_summary) ->
  ...
  let slow_suite_tests: slow_test list = summary.results |> List.map ~fn:(
    fun (result: Test_runtime.test_case_result) -> ({
      suite_label;
  ...
  let slowest_tests: slow_test list = List.append timing.slowest_tests slow_suite_tests |> filter_summary_tests ~small_only ~large_only |> sort_summary_tests ~small_only ~large_only |> take 5 in
  timing.slowest_tests <- slowest_tests;
  summary.results |> List.for_each ~fn:(
    fun (result: Test_runtime.test_case_result) ->
      match result.result with
      | Test_runtime.Failed message -> Vector.push timing.failed_tests ~value:{
        suite_label;
        test_name = result.name;
        message;
        duration_us = result.duration_us
      }
      | Test_runtime.Timed_out { timeout_ms } -> Vector.push timing.failed_tests ~value:{
        suite_label;
        test_name = result.name;
        message = timeout_message timeout_ms;
        duration_us = result.duration_us
      }
      | Test_runtime.Passed | Test_runtime.Skipped -> ()
  )
...
let print_summary = fun ~small_only ~large_only ~label ~total ~passed ~failed ~skipped ~(timing:timing_summary) ->
  ...
  if timing.measured_test_count > 0 then
    (
      ...
      println ("  Average per test: " ^ format_duration_us (timing.measured_duration_us / timing.measured_test_count));
      if not (List.is_empty timing.slowest_tests) then
        (
          println (summary_section_title ~small_only ~large_only);
          timing.slowest_tests |> List.enumerate |> List.for_each ~fn:(
            fun (idx, (test: slow_test)) -> println ("    " ^ Int.to_string (idx + 1) ^ ". " ^ test.suite_label ^ " :: " ^ test.test_name ^ " (" ^ format_duration_us test.duration_us ^ ")")
          )
        )
    );
  if not (Vector.is_empty timing.failed_tests) then
    (
      ...
      timing.failed_tests |> Vector.to_array |> Array.to_list |> List.enumerate |> List.for_each ~fn:(
        fun (idx, (test: failed_test)) ->
          println ("    " ^ Int.to_string (idx + 1) ^ ". " ^ test.suite_label ^ " :: " ^ test.test_name ^ " (" ^ format_duration_us test.duration_us ^ ")");
...
let stamp_json_event = fun ~command_started_at ~duration_us (event: Test_runtime.test_event) (json: Data.Json.t) ->
  match json with
  | Data.Json.Object fields ->
      ...
      let fields =
        ...
        | Test_runtime.SuiteCompleted _ -> fields |> upsert_int_field "started_at_us" (Int.max 0 (elapsed_us - normalized_duration_us)) |> upsert_int_field "completed_at_us" elapsed_us
        | Test_runtime.Summary _ -> fields |> upsert_int_field "started_at_us" 0 |> upsert_int_field "completed_at_us" elapsed_us
        | Test_runtime.NoSuitesFound _ -> upsert_int_field "completed_at_us" elapsed_us fields
        | Test_runtime.Build _ -> fields
        | Test_runtime.TestSuitesCollected _ | Test_runtime.ResolvingSuiteBinary _ | Test_runtime.SuiteBinaryResolved _ | Test_runtime.ExecutingSuiteBinary _ | Test_runtime.SuiteHeartbeat _ | Test_runtime.SuiteBinaryFinished _ | Test_runtime.ParsingSuiteOutput _ -> upsert_int_field "emitted_at_us" elapsed_us fields
...
let write_test_event_json = fun ~command_started_at (event: Test_runtime.test_event) -> Test_runtime.test_event_to_json event |> Option.for_each ~fn:(
...
let suite_source_labels = fun ~(workspace:Riot_model.Workspace.t) -> Riot_model.Workspace.realize_packages ~intent:Riot_model.Package.Test workspace |> List.flat_map ~fn:(
  fun (pkg: Riot_model.Package.t) -> pkg.binaries |> List.filter_map ~fn:(
    fun (bin: Riot_model.Package.binary) ->
      if String.ends_with ~suffix:"_tests" bin.name || String.ends_with ~suffix:"-tests" bin.name then
        Some { package_name = pkg.name; suite_name = bin.name; label = source_path_label ~workspace Path.(pkg.path / bin.path) }
      else None
  )
)

let suite_source_label = fun ~(suite_labels:suite_source_label_entry list) (suite: Test_runtime.suite_binary) ->
  match suite_labels |> List.find ~fn:(
    fun (entry: suite_source_label_entry) -> Riot_model.Package_name.equal entry.package_name suite.package_name && String.equal entry.suite_name suite.suite_name
  ) with
  | Some entry -> entry.label
  | None -> Riot_model.Package_name.to_string suite.package_name ^ "/" ^ suite.suite_name

let listed_suite_source_label = fun ~(workspace:Riot_model.Workspace.t) ~(suite_labels:suite_source_label_entry list) (suite: Test_runtime.listed_test_suite) ->
  match suite.source_path with
  | Some path -> source_path_label ~workspace path
  | None -> suite_source_label ~suite_labels suite.suite

let listed_test_selector = fun (suite: Test_runtime.suite_binary) (test: Test_runtime.listed_test_case) -> Riot_model.Package_name.to_string suite.package_name ^ ":" ^ suite.suite_name ^ ":" ^ test.name
...
let listed_suite_path_json = fun ~(workspace:Riot_model.Workspace.t) (suite: Test_runtime.listed_test_suite) ->
  match suite.source_path with
  | Some path -> (
    match Path.strip_prefix path ~prefix:workspace.root with
...
let listed_suite_selector = fun (suite: Test_runtime.suite_binary) -> Riot_model.Package_name.to_string suite.package_name ^ ":" ^ suite.suite_name

let write_json_line = fun json -> println (Data.Json.to_string json)

let write_test_suite_listed_json = fun ~command_started_at ~(workspace:Riot_model.Workspace.t) (suite: Test_runtime.listed_test_suite) ->
  write_json_line
    (
      Data.Json.Object [
...
let write_test_case_listed_json = fun ~command_started_at (suite: Test_runtime.suite_binary) (test: Test_runtime.listed_test_case) ->
  write_json_line
    (
      Data.Json.Object [
...
let write_test_list = fun ~(workspace:Riot_model.Workspace.t) ~(suite_labels:suite_source_label_entry list) suites -> List.for_each suites ~fn:(
  fun (suite: Test_runtime.listed_test_suite) ->
    ...
    suite.tests |> List.for_each ~fn:(
      fun (test: Test_runtime.listed_test_case) ->
        ...
        in
        println ("  [" ^ Int.to_string test.index ^ "] " ^ type_prefix ^ " " ^ test.name ^ metadata ^ skip_suffix)
    )
)
...
let suite_stream_key = fun (suite: Test_runtime.suite_binary) -> Riot_model.Package_name.to_string suite.package_name ^ ":" ^ suite.suite_name

let qualified_test_name = fun (suite: Test_runtime.suite_binary) (result: Test_runtime.test_case_result) -> Riot_model.Package_name.to_string suite.package_name ^ "::" ^ suite.suite_name ^ "::" ^ result.name
...
let print_test_result = fun ~(suite:Test_runtime.suite_binary) (result: Test_runtime.test_case_result) ->
  ...
  | Test_runtime.Failed message ->
      ...
      if not (String.equal message "") then
        println ("       " ^ message)
  | Test_runtime.Timed_out { timeout_ms } -> println (prefix ^ " " ^ name ^ metadata ^ " ... TIMED OUT " ^ timeout_message timeout_ms ^ attempts_suffix result.attempts)
  | Test_runtime.Skipped -> println (prefix ^ " " ^ name ^ metadata ^ " ... skipped")

let write_test_event = fun ~(suite_labels:suite_source_label_entry list) ~(timing:timing_summary) ~small_only ~large_only ~(state:human_render_state) ~verbose (event: Test_runtime.test_event) ->
  match event with
  | Test_runtime.Build _ -> ()
  | Test_runtime.NoSuitesFound { package_name; suite_name } -> print_empty_hint package_name suite_name
  | Test_runtime.TestSuitesCollected _ | Test_runtime.ResolvingSuiteBinary _ | Test_runtime.SuiteBinaryResolved _ | Test_runtime.RunningSuite _ -> ()
  | Test_runtime.ExecutingSuiteBinary _ | Test_runtime.SuiteHeartbeat _ | Test_runtime.SuiteBinaryFinished _ | Test_runtime.ParsingSuiteOutput _ -> ()
  | Test_runtime.SuiteProgress { suite; event } -> Test_runtime.suite_progress_test_case_result event |> Result.to_option |> Option.flatten |> Option.for_each ~fn:(
    fun result ->
      let key = suite_stream_key suite in
      let _ = Collections.HashSet.insert state.streamed_suites ~value:key in print_test_result ~suite result
  )
  | Test_runtime.SuiteCompleted { suite; stdout; stderr; summary; _ } ->
      if summary.total > 0 then
        (
          record_suite_timing ~small_only ~large_only timing ~suite_label:(suite_source_label ~suite_labels suite) summary;
  ...
  | Test_runtime.Summary { total; passed; failed; skipped; failed_tests = _ } -> print_summary ~small_only ~large_only ~label:"Test Summary:" ~total ~passed ~failed ~skipped ~timing
    ...
    | Ok operational_config ->
        ...
        let size_filter =
          if small_only then
            ...
            else Test_selection.All
        in
        let* package_filters = package_filters
        in
        let* request = Test_selection.parse_request ~filter ~package_filters ~size_filter ~flaky_only |> Result.map_err ~fn:(
          fun error -> Failure error
        )
        in
        let extra_args = Test_selection.extra_args ~small_test_timeout:operational_config.test.small_test_timeout ~flaky_max_retries:operational_config.test.flaky_max_retries request trailing in
        if list_mode then
          ...
          in
          match Test_runtime.list_tests ?on_suite:(
            if output_mode = Build.Json then
              Some on_suite
            else None
          ) ?on_suite_error:(
          ...
          | Ok suites ->
              let suites = List.filter suites ~fn:(
                fun (suite: Test_runtime.listed_test_suite) -> not (List.is_empty suite.tests)
              ) in
              (
                match output_mode with
                | Build.Json -> write_test_list_completed_json ~command_started_at ~suite_count:!listed_suite_count ~test_count:!listed_test_count ~failed_suite_count:!failed_suite_count
                | Build.Human ->
                    if List.is_empty suites then
                      print_empty_list_hint request.package_filter request.suite_filter request.query
                    else write_test_list ~workspace ~suite_labels suites
              );
              Ok ()
          | Error err ->
              (
          ...
          let on_event (event: Test_runtime.test_event) =
            match event with
            | Test_runtime.Build build_event -> Build.write_build_event ~mode:output_mode ~seen_registry_updates build_event
            | _ -> (
              match output_mode with
              | Build.Json -> write_test_event_json ~command_started_at event
              | Build.Human -> write_test_event ~suite_labels ~timing ~small_only ~large_only ~state ~verbose event
            )
          in
          match Test_runtime.test ~on_event
            {
              workspace;
              package_filters = request.package_filters;
              suite_filter = request.suite_filter;
