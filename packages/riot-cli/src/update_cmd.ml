...
command "update" |> about "Re-resolve the workspace graph, update locked package versions, and rewrite riot.lock" |> args [ positional "package" |> required false |> multiple |> help "Optional package name to update; repeat by passing more names"; flag "json" |> long "json" |> help "Render events as JSON" ]
...
let package_names_of_matches = fun matches ->
  let rec parse_all acc = function
    | [] -> Ok (List.reverse acc)
    | package :: rest ->
        let* package = Riot_model.Package_name.from_string package |> Result.map_err ~fn:(
          fun error -> InvalidPackageName error
        ) in parse_all (package :: acc) rest
  in
...
let run = fun ~workspace matches ->
  ...
  | Error err -> fail err
  | Ok packages -> (
    match Riot_deps.update ~on_event:(write_event ~mode ~pm_session_id ~seen_registry_updates) ~workspace_manager ~workspace ~request:Riot_deps.{ packages } () with
    | Ok () -> Ok ()
