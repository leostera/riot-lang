...
let command =
  let open ArgParser in
    ...
    |> args
      [
        positional "package" |> required false |> multiple |> help "Optional package name to update; repeat by passing more names";
...
let package_names_of_matches = fun matches ->
  let rec parse_all acc = function
    | [] -> Ok (List.reverse acc)
    | package :: rest ->
        let* package = Riot_model.Package_name.from_string package
