...
let rec toml_json = function
  ...
  | Data.Toml.Table fields -> Data.Json.Object (List.map fields ~fn:(
...
let registry_of_optional = fun ?registry () ->
  ...
  | None -> Pkgs_ml.Registry.create_filesystem ~registry_name () |> Result.map_err ~fn:Pkgs_ml.Registry_cache.create_error_message

let local_workspace_package: (Riot_model.Workspace_manifest.t * Riot_model.Workspace_manager.load_error list) option -> Riot_model.Package_name.t -> (Riot_model.Workspace_manifest.t * Riot_model.Workspace_manager.load_error list * Riot_model.Package_manifest.t) option = fun local_workspace package_name ->
  match local_workspace with
  | None -> None
  | Some ((workspace: Riot_model.Workspace_manifest.t), load_errors) -> workspace.packages |> List.filter ~fn:Riot_model.Package_manifest.is_workspace_member |> List.find ~fn:(
    fun (pkg: Riot_model.Package_manifest.t) -> Riot_model.Package_name.equal pkg.name package_name
  ) |> Option.map ~fn:(
    fun pkg -> (workspace, load_errors, pkg)
  )
...
let matching_release = fun (document: Pkgs_ml.Sparse_index.package_document) requirement -> document.releases |> List.filter_map ~fn:(
...
let registry_package_info = fun ?registry ~target (parsed: Riot_deps.Registry_package_spec.t) () ->
  ...
  | Ok registry ->
      ...
      | Ok None -> error ~kind:"package_not_found" ~message:("package '" ^ package_name ^ "' was not found in " ^ Pkgs_ml.Registry.name registry)
      | Ok (Some document) -> (
        match matching_release document requirement with
        | None -> error ~kind:"version_not_found" ~message:("no release of '" ^ package_name ^ "' matches " ^ requirement_string)
        | Some (_, release) -> (
          match Pkgs_ml.Registry.materialize_release registry ~package_name ~version:release.version with
          | Error err -> error ~kind:"materialization_failed" ~message:err
          | Ok _ ->
              let root = Pkgs_ml.Registry_cache.package_src_dir cache ~package_name ~version:release.version in
              ...
              Ok {
                ...
                load_errors = [];
                links = package_links ~name:parsed.name ~version:(Some release.version) ?homepage_url:release.homepage ?repository_url:release.repository ?source_url:(Some release.canonical_locator) ()
              }
        )
      )

let workspace_package_info = fun ~workspace_root ~target ~load_errors (pkg: Riot_model.Package_manifest.t) ->
...
let resolve = fun ?registry ~local_workspace ~target () ->
  ...
  | Error err -> error ~kind:"invalid_target" ~message:(Riot_deps.Registry_package_spec.error_message err)
  | Ok parsed -> (
    match explicit_requirement, local_workspace_package local_workspace parsed.name with
    | false, Some (workspace, load_errors, pkg) -> workspace_package_info ~workspace_root:workspace.root ~target ~load_errors pkg
