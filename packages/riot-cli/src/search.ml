...
command "search" |> about "Search pkgs.ml for packages by name" |> args [ positional "query" |> help "Package query, for example mini or jsonrpc"; option "limit" |> long "limit" |> short 'n' |> help "Maximum number of results to return (default: 10)"; flag "json" |> long "json" |> help "Render results as JSON" ]
...
let json_of_result = fun (result: Riot_deps.suggested_package) ->
  Data.Json.Object [
    ...
    ("description", match result.description with
...
let write_human_results = fun ~query results ->
  ...
  | results -> List.for_each results ~fn:(
    fun (result: Riot_deps.suggested_package) ->
      match result.description with
      | Some description -> println (result.package ^ "@" ^ result.latest_version ^ " - " ^ description)
...
let run = fun matches ->
  ...
  | Ok request -> (
    ...
    | Ok results ->
        if json then
          results |> List.map ~fn:json_of_result |> fun items -> Data.Json.Array items |> Data.Json.to_string |> println
        else write_human_results ~query:request.query results;
        Ok ()
  )
