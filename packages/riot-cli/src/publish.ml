...
command "publish" |> about "Publish packages to the registry" |> args
  [
...
let write_block = fun text -> text |> String.split ~by:"\n" |> List.filter ~fn:(
...
let format_fix_diagnostics = fun result ->
  ...
  if List.length grouped > 0 then
    Some (Riot_fix.Diagnostic.grouped_list_to_formatted_output ~file:result.file ~source:result.final_source grouped)
  else None
...
let write_fix_result = fun ~workspace_root (result: Riot_fix.Runner.file_result) ->
  ...
  | None ->
      if result.changed then
        out ("\027[1;32m✓\027[0m " ^ rel ^ " (applied " ^ Int.to_string (List.length result.applied_fixes) ^ " safe fixes)");
      (
        match format_fix_parse_diagnostics result with
        | Some text ->
            out ("\027[1;31m✗\027[0m " ^ rel ^ " (" ^ Int.to_string (List.length result.parse_diagnostics) ^ " parse issues)");
            write_block text
        | None -> ()
      );
      (
        ...
        | Some text ->
            out ("\027[1;31m✗\027[0m " ^ rel ^ " (" ^ Int.to_string (List.length result.diagnostics) ^ " issues found)");
            write_block text
        | None -> ()
      )
...
let write_fix_summary = fun (summary: Riot_fix.Runner.summary) ->
  ...
  else out ("\027[1;31m✗\027[0m Found " ^ Int.to_string summary.remaining_diagnostics ^ " issues across " ^ Int.to_string summary.total_files ^ " files")
...
let render_packing = fun ~package ~version ~artifact_path -> "     \027[1;32mPacking\027[0m " ^ package ^ " " ^ version ^ " (" ^ artifact_path ^ ")"

let render_publishing = fun ~package ~version -> "  \027[1;32mPublishing\027[0m " ^ package ^ " " ^ version ^ " (" ^ publish_page_url ~package ~version ^ ")"

let render_skipping = fun ~package ~version -> "    \027[1;33mSkipping\027[0m " ^ package ^ " " ^ version ^ " (already published)"

let render_skipping_not_public = fun ~package ~version -> "    \027[1;33mSkipping\027[0m " ^ package ^ " " ^ version ^ " (package is not public)"
...
let write_publish_event = fun ~workspace_root ~seen_registry_updates ~displayed_packages ~progress event ->
  ...
  | Riot_publish.Build build_event -> Build.write_build_event ~mode:Build.Human ~seen_registry_updates build_event
  | Riot_publish.CheckStarted { package; version; stage = `fmt } -> out (render_formatting ~package:(Package_name.to_string package) ~version:(version_label version))
  | Riot_publish.CheckStarted { package; version; stage = `fix } -> out (render_checking ~package:(Package_name.to_string package) ~version:(version_label version))
  | Riot_publish.CheckStarted { package; version; stage = `build } -> out (render_compiling ~package:(Package_name.to_string package) ~version:(version_label version))
  | Riot_publish.CheckStarted _ -> ()
  | Riot_publish.CheckFinished _ -> ()
  | Riot_publish.Packing { package; version; artifact_path } -> out (render_packing ~package:(Package_name.to_string package) ~version:(Std.Version.to_string version) ~artifact_path:(relative_or_absolute ~root:workspace_root artifact_path))
  | Riot_publish.SkippedNotPublic { package; version } -> out (render_skipping_not_public ~package:(Package_name.to_string package) ~version:(version_label version))
  | Riot_publish.SkippedAlreadyPublished { package; version } -> out (render_skipping ~package:(Package_name.to_string package) ~version:(Std.Version.to_string version))
  | Riot_publish.DryRunPlanned prepared -> out (render_publishing ~package:(Riot_model.Package_name.to_string prepared.package.name) ~version:(Std.Version.to_string prepared.version))
  | Riot_publish.PackagePublished published -> out (render_publishing ~package:published.package_name ~version:published.package_version)
...
let published_release_json = fun (release: Pkgs_ml.Registry.published_release) ->
  Data.Json.Object [
    ...
    "materialization", Data.Json.Object [
...
let prepared_publish_json = fun ~workspace_root (prepared: Riot_deps.Publisher.prepared_publish) ->
  Data.Json.Object [
    ...
    "artifact_path", Data.Json.String (relative_or_absolute ~root:workspace_root prepared.artifact_path);
  ]
...
let publish_event_to_json = fun ~workspace_root event ->
  ...
  | Riot_publish.CheckStarted { package; version; stage } ->
      Some (
        json_event "publish.check.started"
          [
  ...
  | Riot_publish.CheckFinished { package; version; stage } ->
      Some (
        json_event "publish.check.finished"
          [
            "package", Data.Json.String (Package_name.to_string package);
            "version", json_version_or_null version;
            "stage", publish_stage_json stage;
...
let write_publish_event_json = fun ~workspace_root event -> publish_event_to_json ~workspace_root event |> Option.for_each ~fn:write_json_line

let write_publish_error_json = fun error ->
  write_json_line
    (
      json_event "publish.error"
        [
...
let run = fun (workspace: Workspace.t) matches ->
  let package_name =
    ...
    | None -> Ok None
    | Some package_name -> Package_name.from_string package_name |> Result.map ~fn:Option.some |> Result.map_err ~fn:(
      fun error -> Failure (Package_name.error_message error)
    )
  in
  ...
  | Ok request ->
      ...
      match Riot_publish.publish ~on_event:(
        if json then
          write_publish_event_json ~workspace_root:workspace.root
        else write_publish_event ~workspace_root:workspace.root ~seen_registry_updates ~displayed_packages ~progress
      ) ~workspace ~request:(publish_request ~skip_check:(ArgParser.get_flag matches "skip-check") request) ~mode () with
      | Error err ->
          if json then
