...
command "upgrade" |> about "Upgrade the globally installed riot binary" |> args [ option "version" |> long "version" |> help "Version to install (default: latest)" ]
...
let archive_url = fun ?version ~target ~base_url () ->
  let version = version_label version in base_url ^ "/riot/riot-" ^ version ^ "-" ^ target ^ ".tar.gz"
...
let installed_binary_path = fun () ->
  let* riot_home = riot_home_dir () in Ok Path.(riot_home / Path.v "bin" / Path.v "riot")

let install_temp_path = fun dst ->
  let dir = Path.dirname dst in
  let name = Path.basename dst in
  let pid = Process.id () in
  let nonce = Random.bits () |> Result.expect ~msg:"failed to generate upgrade nonce" in Path.(dir / Path.v ("." ^ name ^ ".upgrade-" ^ Int32.to_string pid ^ "-" ^ Int.to_string nonce))
...
let run_downloader = fun ~program ~args ~url ->
  ...
  | Ok { status; stderr; _ } ->
      let detail =
        ...
        else program ^ " failed with status " ^ Int.to_string status ^ ": " ^ String.trim stderr
      in
      Error detail
  | Error (Command.SystemError msg) -> Error ("failed to start " ^ program ^ " while downloading " ^ url ^ ": " ^ msg)
...
let download_archive = fun ~url ~dst ->
  ...
  match run_downloader ~program:"curl" ~args:[
    "-fsSL";
    "-o";
    dst_str;
    url;
  ...
  | Ok () -> Ok ()
  | Error curl_error when String.contains curl_error "failed to start curl" -> (
    match run_downloader ~program:"wget" ~args:[
      "-q";
      "-O";
      dst_str;
      url;
      ...
      protect ~finally:(
        ...
        (
          fun () ->
            ...
            | Ok () -> Ok ()
            | Error (Archive.Tar.Extract_source_error err) -> Error ("failed to read archive: " ^ IO.error_message err)
            | Error (Archive.Tar.Extract_fs_error err) -> Error ("failed to unpack archive: " ^ IO.error_message err)
            | Error (Archive.Tar.Extract_error err) -> Error ("failed to decode archive: " ^ tar_error_message err)
        )
...
let resolve_metadata_source = fun ?version () ->
  ...
  | None ->
      let base_url = Env.get Env.String ~var:"RIOT_UPGRADE_METADATA_BASE_URL" |> Option.unwrap_or ~default:default_metadata_base_url in Ok (Metadata_remote (metadata_url ?version ~base_url ()))
...
let resolved_metadata = fun ~latest_metadata ~extract_dir ~downloaded_binary ->
  ...
  | Some metadata -> Ok metadata
  | None ->
      let* extracted = read_extracted_metadata ~extract_dir
      in
...
let resolve_archive_source = fun ?version ~target () ->
  ...
  | None ->
      let base_url = Env.get Env.String ~var:"RIOT_UPGRADE_ARCHIVE_BASE_URL" |> Option.unwrap_or ~default:default_archive_base_url in Ok (Remote (archive_url ?version ~target ~base_url ()))

let ensure_install_dir = fun () ->
  let* riot_home = riot_home_dir ()
  in
  let* () = Fs.create_dir_all riot_home |> Result.map_err ~fn:IO.error_message in Fs.create_dir_all Path.(riot_home / Path.v "bin") |> Result.map_err ~fn:IO.error_message
...
let write_newer_release_message = fun ~current ~next -> out ("Riot " ^ display_label next ^ " is out! You're on " ^ display_label current)

let write_unchanged_message = fun metadata -> out ("Congrats! You're already on the latest version of Riot (which is " ^ display_label metadata ^ ")")
  ...
  | Ok () -> (
    let requested_metadata =
      if Env.get Env.String ~var:"RIOT_UPGRADE_ARCHIVE_PATH" |> Option.is_some then
        ...
        | Ok (Metadata_local path) -> Version_info.of_path path |> Result.map ~fn:Option.some
    ...
    in
    let* latest_metadata = requested_metadata |> Result.map ~fn:(Option.map ~fn:(
      fun (metadata: Version_info.t) -> { metadata with issues_url = Option.or_else metadata.issues_url ~fn:(
    ...
    in
    match resolve_archive_source ?version:(
      match latest_metadata with
    ...
    | Error message ->
        ...
        Error (Failure message)
    | Ok archive_source ->
        let* current_binary = installed_binary_path () |> Result.map_err ~fn:(
          fun message -> Failure message
        )
        in
              ...
              match with_tempdir_result "riot-upgrade"
                (
                  fun tempdir ->
                    ...
                    in
                    let* () = Fs.create_dir_all extract_dir |> Result.map_err ~fn:IO.error_message
                    in
                    let* () = extract_archive ~archive_path ~into:extract_dir
                    in
                    let* metadata = resolved_metadata ~latest_metadata ~extract_dir ~downloaded_binary
                    in
                    let unchanged =
                    ...
                    if unchanged then
                      ...
                      (
                        let () =
                          match current_metadata with
                          | Some current when not (Version_info.same_identity current metadata) -> write_newer_release_message ~current ~next:metadata
                          | Some _ | None -> ()
                        in
                        let install_dir = Path.dirname current_binary in
                        let* () = Fs.create_dir_all install_dir |> Result.map_err ~fn:IO.error_message
                        in
                        let* () = install_binary_atomically ~src:downloaded_binary ~dst:current_binary
                        in
                        let* () = Version_info.write_installed metadata
                        in
                        let duration = Time.Instant.duration_since ~earlier:started_at (Time.Instant.now ()) |> Time.Duration.to_millis in
