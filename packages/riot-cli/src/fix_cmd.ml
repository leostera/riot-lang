...
let build_package = fun ~mode ~(workspace:Riot_model.Workspace_manifest.t) ~package_name ~profile ?(transform_workspace = fun workspace -> workspace) () -> let* workspace =
  prepare_workspace workspace in
Build.build_command
