...
command "rm" |> about "Remove a dependency from a manifest section and refresh riot.lock" |> args
  [
...
let run = fun ~workspace matches ->
  ...
  let dependencies =
    ...
    | dependencies ->
        let rec parse_all acc = function
          | [] -> Ok (List.reverse acc)
          | dependency :: rest ->
              let* dependency = Package_name.from_string dependency |> Result.map_err ~fn:(
                fun error -> InvalidPackageName error
              ) in parse_all (dependency :: acc) rest
        in
  ...
  | Ok dependencies, Ok selection, Ok scope, Ok cwd ->
      ...
      (
        match Riot_deps.remove ~on_event:(write_event ~mode ~pm_session_id ~seen_registry_updates) ~workspace_manager ~workspace ~cwd ~request () with
        | Ok () -> Ok ()
        | Error error -> fail (RemoveFailed error)
      )
  | (Error err, _, _, _) | (_, Error err, _, _) | (_, _, Error err, _) -> fail err
