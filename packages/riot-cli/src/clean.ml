...
command "clean" |> about "Run tracked build cache GC, or remove the build root with --force" |> args [ flag "json" |> long "json" |> help "Emit machine-readable JSONL events"; flag "force" |> long "force" |> help "Remove the entire build root instead of keeping it and running tracked cache GC" ]
...
let run = fun ~(workspace:Riot_model.Workspace.t) matches ->
  ...
  let on_waiting lock_path = Build.write_build_phase_event ~mode (Riot_build.Event.BuildLockWaiting { lock_path }) in
