...
let split_once = fun value ch ->
  let rec find at =
    if at >= String.length value then
      ...
      if Char.equal (String.get_unchecked value ~at) ch then
        Some at
      else find (at + 1)
  in
  match find 0 with
  ...
  | Some idx ->
      ...
      let right = String.sub value ~offset:(idx + 1) ~len:(String.length value - idx - 1) in Some (left, right)
...
let parse_selector = fun pattern ->
  ...
  | Some (package_name, rest) when has_text package_name && has_text rest -> (
    let* package_name = Riot_model.Package_name.from_string package_name |> Result.map_err ~fn:Riot_model.Package_name.error_message
    in
    match split_once rest ':' with
    | Some (suite_name, query) when has_text suite_name && has_text query -> Ok (Some package_name, Some suite_name, Some query)
