...
let build_cli = fun () -> let open ArgParser in
...
in
command "riot" |> version (Version_info.version_string ()) |> about "OCaml build system and package manager" |> args [ flag "verbose" |> short 'v' |> long "verbose" |> help "Enable verbose output" |> count ] |> subcommands builtin_commands
...
let cli_elapsed_us = fun () -> Time.Instant.elapsed !cli_trace_origin |> Time.Duration.to_micros
...
let report_workspace_load_errors = fun load_errors -> List.for_each load_errors ~fn:(
  fun err -> eprintln ("\027[1;31mError\027[0m: " ^ Riot_model.Workspace_manager.load_error_to_string err)
)
...
let ensure_workspace = fun (workspace: Riot_model.Workspace_manifest.t) ->
  ...
  ) in Riot_deps.ensure_workspace ~workspace_manager ~mode:Riot_deps.Dep_solver.Refresh ~registry ~workspace () |> Result.map_err ~fn:(
    fun err -> Failure (Riot_model.Pm_error.message err)
  )

let workspace_load_error_message = fun load_errors -> String.concat "\n" (List.map load_errors ~fn:Riot_model.Workspace_manager.load_error_to_string)
  ...
  | Loaded (workspace, _load_errors) -> (
    ...
    | [ package_name; command_name ] -> (
      ...
      | Ok package_name -> (
        ...
        match List.find commands ~fn:(
          fun (cmd: Riot_model.Package_command.t) -> Riot_model.Package_name.equal cmd.package_name package_name && cmd.name = command_name
        ) with
        | None -> None
        | Some cmd ->
            Log.info ("Found command: " ^ Riot_model.Package_name.to_string cmd.package_name ^ ":" ^ cmd.name);
            Log.info ("Command binary path: " ^ Path.to_string cmd.command_binary);
            (* Build the package first to ensure command is up to date *)
            Log.info ("Building package: " ^ Riot_model.Package_name.to_string cmd.package_name);
            (
              match ensure_workspace workspace with
              | Error err ->
                  Log.error ("Failed to ensure workspace for build: " ^ Exception.to_string err);
                  Some (Error err)
              | Ok workspace -> (
...
let render_init_event = function
  ...
  | Riot_init.WorkspaceInitializationCompleted { next_steps; package_hints } ->
      ...
      println "";
      println "Next steps:";
      List.for_each next_steps ~fn:(
        fun step -> println ("  " ^ step)
      );
      println "";
      List.enumerate package_hints |> List.for_each ~fn:(
...
let run = fun ~args ->
  ...
  let get_workspace_scan () =
    ...
    | None ->
        ...
        let () =
          match workspace_scan with
          | Loaded (workspace, load_errors) -> trace_cli ("scan-workspace-loaded packages=" ^ Int.to_string (List.length workspace.packages) ^ " load_errors=" ^ Int.to_string (List.length load_errors))
          | NoWorkspace -> trace_cli "scan-workspace-no-workspace"
          | ScanFailed err -> trace_cli ("scan-workspace-failed reason=" ^ Info_cmd.workspace_scan_error_message err)
        in
        let _ = workspace_scan_cache := Some workspace_scan in
        let () = trace_cli "workspace-scan-cache-store" in workspace_scan
  in
  ...
  | _ ->
      ...
      | Ok matches -> (
            ...
            | Ok () ->
                ...
                (
                  ...
                  | Ok workspace ->
                      ...
                      let () = trace_cli "build-run-start" in Build.run ~workspace build_matches
                )
            | Error _ as e -> e
          )
          | Error _ as e -> e
        ...
        | Some ("run", run_matches) -> (
          ...
          let workspace, workspace_error =
            ...
            | Loaded (workspace, load_errors) -> (
              let _ = workspace in (None, Some (workspace_load_error_message load_errors))
            )
        ...
        | Some ("doc", doc_matches) ->
            let workspace =
              match get_workspace_scan () with
              | Loaded (workspace, load_errors) when List.is_empty load_errors -> ensure_workspace workspace |> Result.to_option
              | _ -> None
            in
            (
              match workspace with
              | Some workspace -> Doc.run ~workspace doc_matches |> Result.map_err ~fn:(
                fun err -> Failure err
              )
...
let main ~args =
  ...
  let () = trace_cli "main-return" in result
