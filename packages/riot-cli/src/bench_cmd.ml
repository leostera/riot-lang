...
command "bench" |> about "Run benchmarks with optional case filtering" |> ArgParser.allow_trailing_args |> args
  [
    ...
    flag "verbose" |> short 'v' |> long "verbose" |> help "Enable verbose output for benchmarks" |> count;
  ]

type invocation_args = { parsed: string list; trailing: string list }

let is_known_flag_without_value = function
  | "--list" | "--record" | "--release" | "--json" | "-v" | "--verbose" | "-h" | "--help" -> true
  | _ -> false

let is_known_flag_with_value = function
  | "-p" | "--package" | "-f" | "--filter" | "--compare" | "--iterations" | "--warmup" -> true
  | _ -> false
...
let bench_invocation_args = fun argv ->
  let rec loop parsed trailing = function
    ...
    | "--" :: rest -> { parsed = List.reverse parsed; trailing = List.append (List.reverse trailing) rest }
    | arg :: rest when is_known_flag_without_value arg -> loop (arg :: parsed) trailing rest
    | arg :: value :: rest when is_known_flag_with_value arg -> loop (value :: arg :: parsed) trailing rest
    | arg :: value :: rest when looks_like_flag arg && not (looks_like_flag value) -> loop parsed (value :: arg :: trailing) rest
    | arg :: rest -> loop parsed (arg :: trailing) rest
  in
  match argv with
  | [] -> { parsed = []; trailing = [] }
  | command_name :: rest ->
      let result = loop [ command_name ] [] rest in { parsed = result.parsed; trailing = result.trailing }
...
let reparsed_matches = fun matches ->
  ...
  | Some bench_argv ->
      let invocation = bench_invocation_args bench_argv in ArgParser.get_matches command invocation.parsed |> Result.map ~fn:(
        fun matches -> (matches, invocation.trailing)
      ) |> Result.map_err ~fn:(
        fun err -> Failure (ArgParser.error_message err)
      )
...
let print_empty_hint = fun package_filter ->
  ...
  | Some package_name -> println ("No benchmark suites found in package '" ^ Package_name.to_string package_name ^ "'")
...
let event_elapsed_us = fun ~command_started_at -> Time.Instant.elapsed command_started_at |> Time.Duration.to_micros

let listed_suite_source_label = fun ~(workspace:Riot_model.Workspace.t) (suite: Bench_runtime.listed_bench_suite) ->
...
let listed_bench_selector = fun (suite: Bench_runtime.suite_binary) (item: Bench_runtime.listed_bench_item) -> Package_name.to_string suite.package_name ^ ":" ^ suite.suite_name ^ ":" ^ item.name

let listed_bench_item_json = fun (suite: Bench_runtime.suite_binary) (item: Bench_runtime.listed_bench_item) ->
  let kind =
...
let listed_suite_path_json = fun ~(workspace:Riot_model.Workspace.t) (suite: Bench_runtime.listed_bench_suite) ->
  match suite.source_path with
  | Some path -> (
    match Path.strip_prefix path ~prefix:workspace.root with
...
let listed_suite_selector = fun (suite: Bench_runtime.suite_binary) -> Package_name.to_string suite.package_name ^ ":" ^ suite.suite_name

let write_json_line = fun json -> println (Data.Json.to_string json)

let write_bench_suite_listed_json = fun ~command_started_at ~(workspace:Riot_model.Workspace.t) (suite: Bench_runtime.listed_bench_suite) ->
  write_json_line
    (
      Data.Json.Object [
...
let write_bench_item_listed_json = fun ~command_started_at (suite: Bench_runtime.suite_binary) (item: Bench_runtime.listed_bench_item) ->
  write_json_line
    (
      Data.Json.Object [
...
let write_bench_list = fun ~(workspace:Riot_model.Workspace.t) suites -> List.for_each suites ~fn:(
...
let styled = fun color_hex text ->
  ...
  Tty.Style.default |> Tty.Style.fg color |> Tty.Style.bold |> fun style -> Tty.Style.styled style text
...
let render_relative_speed_line = fun ~fastest (name, ratio) -> "    " ^ styled_fastest_case_name fastest ^ " ran " ^ Float.to_string ~precision:2 ratio ^ "x faster than " ^ name
...
let print_comparison = fun (result: Bench_runtime.bench_comparison_result) ->
  ...
  result.case_results |> List.for_each ~fn:(
    fun (case_result: Bench_runtime.bench_comparison_case_result) ->
      ...
      println ("    iterations: " ^ Int.to_string stats.iterations);
      println ("    mean:       " ^ print_duration stats.mean ^ " ± " ^ print_duration stats.std_dev);
      println ("    min:        " ^ print_duration stats.min);
      println ("    max:        " ^ print_duration stats.max);
      print_gc ~indent:"    " stats.gc
  );
...
let classify_cv = fun value ->
  if Float.compare value 0.02 = Order.LT then
    ...
    if Float.compare value 0.05 != Order.GT then
      Good
    else
      if Float.compare value 0.10 != Order.GT then
        Meh
      else Bad
...
let delta_percent = fun current previous ->
  ...
  if Float.equal previous_nanos 0.0 then
    ...
    let current_nanos = Time.Duration.to_nanos current |> Int64.to_float in Some (((current_nanos -. previous_nanos) /. previous_nanos) *. 100.0)

let noise_margin_percent = fun ~current_cv ~baseline_cv ->
  let values = [ current_cv; baseline_cv ] |> List.filter_map ~fn:(
    fun value -> value
  ) |> List.map ~fn:(
  ...
  match values with
  | [] -> 0.0
  | values -> List.fold_left values ~init:0.0 ~fn:(
    fun acc value ->
...
let style_delta_cell = fun delta_value ~noise_margin_percent text ->
  ...
  | Some value ->
      if Float.compare (Float.abs value) noise_margin_percent != Order.GT then
        ...
        if Float.compare value 0.0 = Order.LT then
          styled "#98C379" text
        else styled "#E06C75" text
...
let gc_per_iteration = fun count iterations ->
  if Int.(iterations <= 0) then
    None
  else Some (Float.of_int count /. Float.of_int iterations)
...
let history_column_label = fun index (sample: History.history_sample) ->
  let base =
    if Int.equal index 0 then
      "prev"
    else "prev-" ^ Int.to_string index
  in
  if sample.partial then
    base ^ "*"
  else base

let history_table_header = fun ~label_width ~column_width ~current_partial (history: History.history_sample list) ->
  let current_label =
    if current_partial then
      "curr*"
    else "curr"
  in
  ([ String.pad_right ~width:label_width ' ' "" ] @ [ String.pad_left ~width:column_width ' ' "delta"; String.pad_left ~width:column_width ' ' current_label ]) @ (history |> List.enumerate |> List.map ~fn:(
    fun (index, sample) -> String.pad_left ~width:column_width ' ' (history_column_label index sample)
  )) |> String.concat " "

let print_history_table = fun ~current_partial ~baseline ~current_cv ~baseline_cv current (history: History.history_sample list) ->
  ...
  println header;
  metric_specs |> List.for_each ~fn:(
    fun (label, project) ->
      let current_value = project current in
      let delta_value = delta_percent current_value (project baseline) in
      let delta_cell = render_delta_text delta_value |> String.pad_left ~width:column_width ' ' |> style_delta_cell delta_value ~noise_margin_percent in
      let values = ([ String.pad_right ~width:label_width ' ' label ] @ [ delta_cell; String.pad_left ~width:column_width ' ' (print_duration current_value) ]) @ (history |> List.map ~fn:(
        fun (sample: History.history_sample) -> String.pad_left ~width:column_width ' ' (print_duration (project sample.statistics))
      )) |> String.concat " " in println values
  );
  ...
  let current_cv = render_cv current_cv_value |> String.pad_left ~width:column_width ' ' |> style_cv_text current_cv_value in
  let cv_delta =
    ...
    | _ -> "n/a"
  in
  let cv_history = history |> List.map ~fn:(
    fun (sample: History.history_sample) ->
      let value = Statistics.coefficient_of_variation sample.statistics in render_cv value |> String.pad_left ~width:column_width ' ' |> style_cv_text value
  ) |> String.concat " " in println (([ String.pad_right ~width:label_width ' ' "cv" ] @ [ String.pad_left ~width:column_width ' ' cv_delta; current_cv ]) @ [ cv_history ] |> String.concat " ")

let print_gc_history_table = fun ~current_partial ~(baseline:History.bench_statistics) (current: History.bench_statistics) (history: History.history_sample list) ->
  let label_width = 10 in
  let column_width = 12 in
  let header = history_table_header ~label_width ~column_width ~current_partial history in
  println header;
  gc_metric_specs |> List.for_each ~fn:(
    fun (label, project) ->
      ...
      let delta_cell = render_signed_int delta_value |> String.pad_left ~width:column_width ' ' |> style_signed_int_delta delta_value in
      let values = ([ String.pad_right ~width:label_width ' ' label ] @ [ delta_cell; String.pad_left ~width:column_width ' ' (Int.to_string current_value) ]) @ (history |> List.map ~fn:(
        fun (sample: History.history_sample) -> String.pad_left ~width:column_width ' ' (Int.to_string (project sample.statistics.gc))
      )) |> String.concat " " in println values
  )

let print_gc_rate_history_table = fun ~current_partial ~(baseline:History.bench_statistics) (current: History.bench_statistics) (history: History.history_sample list) ->
  let label_width = 10 in
  let column_width = 12 in
  let header = history_table_header ~label_width ~column_width ~current_partial history in
  println header;
  gc_metric_specs |> List.for_each ~fn:(
    fun (label, project) ->
      ...
      let delta_cell =
        match delta_value with
        | Some delta_value -> render_signed_float delta_value |> String.pad_left ~width:column_width ' ' |> style_signed_float_delta delta_value
        | None -> String.pad_left ~width:column_width ' ' "n/a"
      in
      let values = ([ String.pad_right ~width:label_width ' ' label ] @ [ delta_cell; String.pad_left ~width:column_width ' ' (render_gc_rate current_value) ]) @ (history |> List.map ~fn:(
        fun (sample: History.history_sample) ->
          let value = gc_per_iteration (project sample.statistics.gc) sample.statistics.iterations in String.pad_left ~width:column_width ' ' (render_gc_rate value)
      )) |> String.concat " " in println values
  )
...
let print_benchmark_history = fun ~current_partial (history: History.benchmark_history) ->
  ...
  println ("  noise margin: " ^ print_percent ((noise_margin_percent ~current_cv:history.current_cv ~baseline_cv:history.baseline_cv) /. 100.0));
  println ("  stability: " ^ styled_stability_label history.stability ^ " (cv " ^ style_cv_text history.current_cv (render_cv history.current_cv) ^ ", previous median " ^ style_cv_text history.baseline_cv (render_cv history.baseline_cv) ^ ")");
  print_history_table ~current_partial ~baseline:history.baseline ~current_cv:history.current_cv ~baseline_cv:history.baseline_cv history.current history.history;
  println "";
  println "  gc:";
...
let benchmark_history_json = fun (history: History.benchmark_history) ->
  Data.Json.Object [
    ...
    "current_cv", json_of_option history.current_cv ~some:(
      fun value -> Data.Json.Float value
    );
    "baseline_cv", json_of_option history.baseline_cv ~some:(
      fun value -> Data.Json.Float value
    );
...
let write_bench_history_json = fun ~command_started_at ~current_partial (suite: Bench_runtime.suite_binary) (history: History.suite_history) ->
  write_json_line
    (
      Data.Json.Object [
        ...
        "comparisons", Data.Json.Array (List.map history.comparisons ~fn:comparison_case_history_json);
        "emitted_at_us", Data.Json.Int (event_elapsed_us ~command_started_at);
      ]
    )
...
let upsert_int_field = fun name value fields ->
  let filtered = List.filter fields ~fn:(
    fun (field_name, _) -> not (String.equal field_name name)
  ) in
  filtered @ [
    name, Data.Json.Int value;
  ]

let stamp_json_event = fun ~command_started_at ~duration_us (event: Bench_runtime.bench_event) (json: Data.Json.t) ->
  match json with
  | Data.Json.Object fields ->
      ...
      let fields =
        ...
        | Bench_runtime.SuiteCompleted _ -> fields |> upsert_int_field "started_at_us" (Int.max 0 (elapsed_us - duration_us)) |> upsert_int_field "completed_at_us" elapsed_us
        | Bench_runtime.Summary _ -> fields |> upsert_int_field "started_at_us" 0 |> upsert_int_field "completed_at_us" elapsed_us
...
let write_json_event = fun ~command_started_at ~duration_us event (json: Data.Json.t) -> println (Data.Json.to_string (stamp_json_event ~command_started_at ~duration_us event json))
...
let write_bench_event = fun ?history_comparison ~current_partial state (event: Bench_runtime.bench_event) ->
  match event with
  ...
  | Bench_runtime.SuiteProgress { suite; event } -> Bench_runtime.suite_progress_active_case event |> Result.iter ~fn:(
    fun active_case -> active_case |> Option.for_each ~fn:(
      fun (active_case: Bench_runtime.running_bench_case) ->
        ensure_suite_header state suite;
        reset_active_case_line state;
  ...
  | Bench_runtime.SuiteCompleted { suite; stdout; stderr; results; comparisons; _ } ->
      let should_print_suite = not (results = []) || not (comparisons = []) || not (String.equal stdout "") || not (String.equal stderr "") in
      reset_active_case_line state;
        ...
        (
          ...
          List.for_each results ~fn:(
            fun result ->
              ...
              history_comparison |> Option.for_each ~fn:(
                fun (history: History.suite_history) -> history.benchmarks |> List.find ~fn:(
                  fun (benchmark_history: History.benchmark_history) -> String.equal benchmark_history.name result.name
                ) |> Option.for_each ~fn:(print_benchmark_history ~current_partial)
              )
          );
          List.for_each comparisons ~fn:(
            fun comparison ->
              print_comparison comparison;
              history_comparison |> Option.for_each ~fn:(
                fun (history: History.suite_history) -> history.comparisons |> List.filter ~fn:(
                  fun (comparison_history: History.comparison_case_history) -> String.equal comparison_history.description comparison.description
    ...
    (
      Data.Json.to_string
        (
          ...
          | Data.Json.Object fields -> Data.Json.Object (upsert_int_field "completed_at_us" (event_elapsed_us ~command_started_at) fields)
          | other -> other
        )
    );
  print "\n"

let bench_history_partial = fun (request: Test_selection.request) -> not (List.is_empty request.package_filters) || Option.is_some request.suite_filter || Option.is_some request.query

let bench_history_context = fun ~(workspace:Riot_model.Workspace.t) ~profile (request: Test_selection.request) ~argv -> History.create_run_context ~workspace_root:workspace.root ~profile ~filter:request.query ~partial:(bench_history_partial request) ~argv ()

let bench_history_of_gc = fun (gc: Bench_runtime.bench_gc_stats): History.gc_stats -> { minor_collections = gc.minor_collections; major_collections = gc.major_collections; compactions = gc.compactions }
...
let bench_history_of_comparison = fun (comparison: Bench_runtime.bench_comparison_result): History.bench_comparison_result ->
  {
    ...
    case_results = List.map comparison.case_results ~fn:(
      fun (case_result: Bench_runtime.bench_comparison_case_result) -> { History.name = case_result.name; statistics = bench_history_of_statistics case_result.statistics }
    );
    speedup_ratios = comparison.speedup_ratios
  }

let save_bench_history = fun context ~(suite:Bench_runtime.suite_binary) status started_at_us completed_at_us duration_us (results: Bench_runtime.bench_case_result list) (comparisons: Bench_runtime.bench_comparison_result list) (summary: Bench_runtime.bench_suite_summary) ->
...
let bench_history_compare = fun context ~(suite:Bench_runtime.suite_binary) ~limit status started_at_us completed_at_us duration_us (results: Bench_runtime.bench_case_result list) (comparisons: Bench_runtime.bench_comparison_result list) (summary: Bench_runtime.bench_suite_summary) ->
  if Int.(limit <= 0) then
    Ok None
  else
    ...
    in
    History.compare_suite_run context ~package_name:suite.package_name ~suite_name:suite.suite_name ~current ~limit |> Result.map ~fn:(
      fun (history: History.suite_history) ->
...
let run = fun ~(workspace:Riot_model.Workspace.t) matches ->
  ...
  let* (matches, trailing) = reparsed_matches matches
  in
  ...
  let* package_filters = package_filters
  in
  let* compare_limit = compare_limit
  in
  let* request = Test_selection.parse_request ~filter:pattern ~package_filters ~size_filter:Test_selection.All ~flaky_only:false |> Result.map_err ~fn:(
  ...
  if list_mode then
    ...
    in
    match Bench_runtime.list_benchmarks ?on_suite:(
      if output_mode = Build.Json then
        Some on_suite
      else None
    ) ?on_suite_error:(
    ...
    | Ok suites ->
        let suites = List.filter suites ~fn:(
          fun (suite: Bench_runtime.listed_bench_suite) -> not (List.is_empty suite.benchmarks)
        ) in
        (
          match output_mode with
          | Build.Json -> write_bench_list_completed_json ~command_started_at ~suite_count:!listed_suite_count ~benchmark_count:!listed_benchmark_count ~failed_suite_count:!failed_suite_count
          | Build.Human ->
              if List.is_empty suites then
                print_empty_list_hint request.package_filter request.query
              else write_bench_list ~workspace suites
        );
        Ok ()
    | Error err ->
        (
    ...
    let on_event (event: Bench_runtime.bench_event) =
      match event with
      | Bench_runtime.Build build_event -> Build.write_build_event ~mode:output_mode ~seen_registry_updates build_event
      | Bench_runtime.SuiteCompleted { suite; status; started_at_us; completed_at_us; duration_us; results; comparisons; summary; _ } ->
          let history_comparison =
            match compare_limit, history_context with
            | Some compare_limit, Some history_context -> Some (bench_history_compare history_context ~suite ~limit:compare_limit status started_at_us completed_at_us duration_us results comparisons summary)
            | _ -> None
          in
          let history_comparison =
            ...
            | Some (Error error) ->
                bench_history_warning ("failed to compare benchmark history for " ^ Package_name.to_string suite.package_name ^ "/" ^ suite.suite_name ^ ": " ^ error);
                None
          in
          if record_history then
            history_context |> Option.for_each ~fn:(
              fun history_context -> save_bench_history history_context ~suite status started_at_us completed_at_us duration_us results comparisons summary |> Result.iter_err ~fn:(
                fun error -> bench_history_warning ("failed to save benchmark history for " ^ Package_name.to_string suite.package_name ^ "/" ^ suite.suite_name ^ ": " ^ error)
              )
            );
          (
            match output_mode with
            | Build.Json ->
                Bench_runtime.bench_event_to_json event |> Option.for_each ~fn:(
                  fun json -> write_json_event ~command_started_at ~duration_us:(summary_duration_us ~command_started_at event) event json
                );
                history_comparison |> Option.for_each ~fn:(
                  fun history -> write_bench_history_json ~command_started_at ~current_partial:history_partial suite history
                )
            | Build.Human -> write_bench_event ?history_comparison ~current_partial:history_partial human_render_state event
          )
      | _ -> (
        match output_mode with
        | Build.Json -> Bench_runtime.bench_event_to_json event |> Option.for_each ~fn:(
          fun json -> write_json_event ~command_started_at ~duration_us:(summary_duration_us ~command_started_at event) event json
        )
        | Build.Human -> write_bench_event ~current_partial:history_partial human_render_state event
      )
    in
    match Bench_runtime.bench ~on_event
      {
        workspace;
        package_filters = request.package_filters;
        suite_filter = request.suite_filter;
