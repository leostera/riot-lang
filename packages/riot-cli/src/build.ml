...
type render_state = { mutable target_count: int option }
...
let build_request_label = fun (request: request) ->
  ...
  let targets =
    ...
    | Riot_model.Target.Exact targets -> Riot_model.Target.Set.to_list targets |> List.map ~fn:Riot_model.Target.to_string |> String.concat ","
  in
...
let stamp_json_event = fun (json: Data.Json.t) ->
  ...
  | Data.Json.Object fields ->
      let fields =
        if Option.is_some (List.find fields ~fn:(
          fun (name, _) -> String.equal name "emitted_at_us"
        )) then
          fields
        else
          fields @ [
...
let write_json_event = fun (json: Data.Json.t) -> println (Data.Json.to_string (stamp_json_event json))
...
let package_has_dev_artifact = fun ~(prefix:string) (package: Riot_model.Package.t) sources -> not (List.is_empty sources) || List.any package.binaries ~fn:(
  fun (binary: Riot_model.Package.binary) -> path_has_prefix ~prefix binary.path
)
...
let labeled_multiline_lines = fun ~label value ->
  ...
  | [] -> [ label ^ ":" ]
  | first :: rest -> (label ^ ": " ^ first) :: List.map rest ~fn:(
    fun line ->
...
let display_planner_file = fun path ->
  ...
  if Path.is_absolute path || String.starts_with ~prefix:"./" path_text || String.starts_with ~prefix:"../" path_text then
    path_text
  else "./" ^ path_text
...
let planning_error_lines = function
  ...
  | Riot_planner.Planning_error.ScanFailed { path; reason } -> [ error_line "failed to scan package sources"; "Riot could not read the source tree it needs to plan this package."; "path: " ^ Path.to_string path ] @ labeled_multiline_lines ~label:"reason" reason
  | Riot_planner.Planning_error.DependencyAnalysisFailed { reason } -> [ error_line "dependency analysis failed"; "Riot could not parse or analyze a source file while discovering module dependencies." ] @ labeled_multiline_lines ~label:"reason" reason
  | Riot_planner.Planning_error.GraphBuildFailed { reason } -> [ error_line "failed to build the module graph"; "Riot analyzed the package sources but could not assemble a valid build graph." ] @ labeled_multiline_lines ~label:"reason" reason
  | Riot_planner.Planning_error.SourceDependsOnUndeclaredPackageModule { package_name; source; requested_module; allowed_modules } ->
      let allowed_modules =
  ...
  | Riot_planner.Planning_error.TargetDependsOnInternalLibraryModule { target_name; source; requested_module; internal_module; public_module } ->
      [
  ...
  | Riot_planner.Planning_error.TargetDependsOnNamespacedInternalLibraryModule { target_name; source; requested_module; internal_module; public_module } ->
      let public_leaf = internal_module |> String.split ~by:"__" |> List.reverse |> List.head |> Option.unwrap_or ~default:requested_module in
      [
        error_line ("target " ^ target_name ^ " imports private module " ^ requested_module);
  ...
  | Riot_planner.Planning_error.TargetDependsOnOtherTargetRoot { target_name; source; requested_module; other_target_name; other_target_module; public_module } ->
      [
  ...
  | Riot_planner.Planning_error.InvalidExecutableMain { package_name; target_name; file; error; _ } ->
      let file = display_planner_file file in
      let headline, reason_lines =
        match error with
        | Riot_planner.Planning_error.MissingMain -> "`" ^ target_name ^ "` has no executable entry point", [ "But we could not find one." ]
        | Riot_planner.Planning_error.MultipleMainDefinitions { count } -> "`" ^ target_name ^ "` has more than one executable entry point", [ "But we found " ^ Int.to_string count ^ " top-level `main` definitions."; "Executable targets must define exactly one." ]
        | Riot_planner.Planning_error.InvalidMainParameters { parameters } ->
            ...
            in
            ("`" ^ target_name ^ "` has an invalid executable entry point", [ "But the `main` function we found does not have that shape."; "found parameters: " ^ parameters ])
      in
      [
        error_line headline;
        "";
  ...
  | Riot_planner.Planning_error.Exception { exn } -> [ error_line "unexpected planner exception"; "Riot hit an unexpected exception while planning this package." ] @ labeled_multiline_lines ~label:"reason" (Exception.to_string exn)

let workspace_load_error_line = function
  | Riot_model.Workspace_manager.PackageNotFound { package; path; dependant = None } -> "missing package: " ^ package ^ " (" ^ path ^ ")"
  | Riot_model.Workspace_manager.PackageNotFound { package; path; dependant = Some dependant } -> "missing package: " ^ package ^ " (required by " ^ dependant ^ ", " ^ path ^ ")"
  | Riot_model.Workspace_manager.PackageTomlReadFailed { package; path } -> "failed to read package toml: " ^ package ^ " (" ^ path ^ ")"
  | Riot_model.Workspace_manager.PackageTomlParseFailed { package; path } -> "failed to parse package toml: " ^ package ^ " (" ^ path ^ ")"
  | Riot_model.Workspace_manager.PackageFromTomlFailed { package; path; error } -> "failed to load package manifest: " ^ package ^ " (" ^ path ^ "): " ^ Riot_model.Package_manifest.error_message error
...
let workspace_planning_error_lines = function
  | Riot_planner.Workspace_planner.PackageNotFound { name; available } ->
      [
        ...
        "available packages: " ^ String.concat ", " (List.map available ~fn:Riot_model.Package_name.to_string);
      ]
  ...
  | Riot_planner.Workspace_planner.CycleDetected { cycle } -> [ error_line "package cycle detected"; "Riot found a cycle between packages, so it cannot plan them in dependency order."; "cycle: " ^ String.concat " -> " cycle ]
  | Riot_planner.Workspace_planner.MissingDependencies { missing } -> ([ error_line "missing package dependencies"; "Riot found package dependency edges that do not point at a loaded workspace or resolved package." ] @ List.map missing ~fn:(
    fun (dep: Riot_planner.Package_graph.missing_dependency) -> "missing: " ^ dep.package ^ " -> " ^ dep.dependency
  )) @ [ "examples:"; "  - add the missing package to the workspace"; "  - add a registry, path, or workspace dependency entry for the missing package" ]
  | Riot_planner.Workspace_planner.PackageLoadFailed { errors } -> [ error_line "failed to load workspace packages"; "Riot could not load every package manifest needed for planning." ] @ List.map errors ~fn:workspace_load_error_line
...
let telemetry_package_error_message = function
  ...
  | Build_telemetry.ExecutionFailed { message } | Build_telemetry.ActionExecutionFailed { message } -> message
  | Build_telemetry.ActionOutputsNotCreated { missing } -> "missing outputs: " ^ String.concat ", " (List.map missing ~fn:Path.to_string)
  | Build_telemetry.ActionDependenciesFailed { failed } -> "failed dependencies: " ^ String.concat ", " (List.map failed ~fn:Graph.SimpleGraph.Node_id.to_string)

let show_target_in_package_labels = function
  | Some { target_count = Some target_count } -> target_count > 1
  | Some { target_count = None } | None -> false

let display_build_package_name = fun ?render_state ~build_target package -> display_package_name ~build_target ~show_target:(show_target_in_package_labels render_state) package
...
let write_build_telemetry_event = fun ?render_state ~mode event ->
  ...
  | Human -> (
    match event with
    | Build_telemetry.CompilationStarted { package; build_target; _ } -> out ("    \027[1;32mBuilding\027[0m " ^ display_build_package_name ?render_state ~build_target package)
    | Build_telemetry.BuildCompleted { package; build_target; status = `Fresh; _ } -> out ("       \027[1;32mBuilt\027[0m " ^ display_build_package_name ?render_state ~build_target package)
    | Build_telemetry.BuildCompleted { package; build_target; status = `Cached; _ } -> out ("      \027[1;34mCached\027[0m " ^ display_build_package_name ?render_state ~build_target package)
    | Build_telemetry.BuildSkipped { package; build_target; reason; _ } -> out ("      \027[1;33mSkipped\027[0m " ^ display_build_package_name ?render_state ~build_target package ^ ": " ^ reason)
    | Build_telemetry.BuildFailed { package; build_target; error = PlanningFailed planning_error; _ } ->
        out ("      \027[1;31mFailed\027[0m " ^ display_build_package_name ?render_state ~build_target package);
        planning_error_lines planning_error |> List.for_each ~fn:(
          fun line -> out ("        " ^ line)
        )
    | Build_telemetry.BuildFailed { package; build_target; error; _ } -> out ("      \027[1;31mFailed\027[0m " ^ display_build_package_name ?render_state ~build_target package ^ ": " ^ telemetry_package_error_message error)
    | Build_telemetry.PackageOcamlcWarnings { package; build_target; messages; _ } -> messages |> List.for_each ~fn:(
      fun message -> out_prefixed_payload ~prefix:("     \027[1;33mWarning\027[0m " ^ display_build_package_name ?render_state ~build_target package ^ ": ") message
    )
    | Build_telemetry.BuildStarted _ | Build_telemetry.WorkspacePlanStarted _ | Build_telemetry.WorkspacePlanCompleted _ | Build_telemetry.WorkspaceManifestFilterCompleted _ | Build_telemetry.WorkspaceGraphCreated _ | Build_telemetry.WorkspaceTargetGraphFiltered _ | Build_telemetry.WorkspaceTopologicalSortCompleted _ | Build_telemetry.PlanningWorkspaceStarted _ | Build_telemetry.PlanningWorkspaceCompleted _ | Build_telemetry.PackagePlanningResult _ | Build_telemetry.PackagePlanningBreakdown _ | Build_telemetry.ActionStarted _ | Build_telemetry.ActionCommandStarted _ | Build_telemetry.ActionCompleted _ | Build_telemetry.ActionFailed _ | Build_telemetry.CacheHit _ | Build_telemetry.CacheMiss _ | Build_telemetry.WorkspaceStarted _ | Build_telemetry.WorkspaceCompleted _ -> ()
    | _ -> ()
  )
...
let write_build_phase_event = fun ?render_state ~mode phase ->
  ...
  | Human -> (
    match phase with
    | Riot_build.Event.TargetsResolved { target_count } -> out ("    Resolved " ^ Int.to_string target_count ^ " target(s)")
    | Riot_build.Event.ToolchainsEnsured { target_count } -> out ("    Ensured toolchains for " ^ Int.to_string target_count ^ " target(s)")
    | Riot_build.Event.ToolchainsValidated { target_count } -> out ("    Validated toolchains for " ^ Int.to_string target_count ^ " target(s)")
    ...
    | Riot_build.Event.PackagePlanningStarted { package_count; _ } -> out ("    Planning " ^ Int.to_string package_count ^ " package(s)")
    | Riot_build.Event.PackagePlanningFinished { execution_required_count; cached_count; skipped_count; failed_count; error_count; _ } ->
        let parts =
          [
    ...
    | Riot_build.Event.PackageExecutionStarted { package_count; _ } -> out ("    Executing " ^ Int.to_string package_count ^ " package(s)")
    | Riot_build.Event.PackageExecutionFinished { built_count; failed_count; error_count; _ } -> out ("    Package execution finished (" ^ Int.to_string built_count ^ " built, " ^ Int.to_string failed_count ^ " failed, " ^ Int.to_string error_count ^ " errored)")
    | Riot_build.Event.TargetBuildStarted _ | Riot_build.Event.TargetBuildFinished _ | Riot_build.Event.CacheGenerationRecordingStarted _ | Riot_build.Event.CacheGenerationRecorded _ | Riot_build.Event.ReturningResults _ -> ()
  )

let command_error_event_to_json = fun kind details -> Data.Json.Object (("type", Data.Json.String kind) :: details)
...
let format_pm_event = fun ~seen_registry_updates kind ->
  ...
  | Riot_model.Event.RegistryIndexUpdating { registry } ->
      if HashSet.contains seen_registry_updates ~value:registry then
        ...
        (
          let _ = HashSet.insert seen_registry_updates ~value:registry in Some ("    \027[1;32mUpdating\027[0m " ^ registry ^ " index")
        )
  | Riot_model.Event.PackageResolvedForBuild _ -> None
  | Riot_model.Event.PackageDownloadStarted { package; version; _ } -> Some ("    \027[1;32mFetching\027[0m " ^ Riot_model.Package_name.to_string package ^ " " ^ version)
  | Riot_model.Event.PackageDownloadQueued { package; version; _ } -> Some ("      \027[1;33mQueued\027[0m " ^ Riot_model.Package_name.to_string package ^ " (" ^ version ^ ")")
  | Riot_model.Event.DependencyResolutionStarted _ | Riot_model.Event.DependencyResolutionRefreshingLock _ | Riot_model.Event.DependencyResolutionFailed _ | Riot_model.Event.DependencyUniverseBuilding _ | Riot_model.Event.DependencyUniverseBuilt _ | Riot_model.Event.PackageMetadataFetchStarted _ | Riot_model.Event.PackageMetadataFetchFinished _ | Riot_model.Event.PackageMetadataFetchFailed _ | Riot_model.Event.SourceDependencyMaterializationFinished _ | Riot_model.Event.LockfileReadStarted _ | Riot_model.Event.LockfileReadFinished _ | Riot_model.Event.LockfileReadFailed _ | Riot_model.Event.LockfileWriteStarted _ | Riot_model.Event.LockfileWriteFinished _ | Riot_model.Event.LockfileWriteFailed _ | Riot_model.Event.DependencyResolutionFinished _ | Riot_model.Event.DependencyResolutionUsingExistingLock _ | Riot_model.Event.DependencyResolutionUnlocking _ | Riot_model.Event.PackageManifestFetchStarted _ | Riot_model.Event.PackageManifestFetchFinished _ | Riot_model.Event.PackageManifestFetchFailed _ | Riot_model.Event.PackageDownloadSkipped _ | Riot_model.Event.PackageMaterializationStarted _ | Riot_model.Event.PackageMaterializationFinished _ | Riot_model.Event.PackageMaterializationFailed _ -> None
  ...
  | Riot_model.Event.PackageVersionLocked { package; version } -> Some ("      \027[1;32mLocked\027[0m " ^ Riot_model.Package_name.to_string package ^ " (" ^ version ^ ")")
  | Riot_model.Event.PackageVersionsUnchanged _ -> Some "    Dependencies are already up to date"
  | Riot_model.Event.PackageVersionUpdated { package; from_version; to_version } -> Some ("    \027[1;32mUpdated\027[0m " ^ Riot_model.Package_name.to_string package ^ " (" ^ from_version ^ " -> " ^ to_version ^ ")")
...
let write_build_failed_error = fun ~mode errors ->
  ...
  | Json ->
      write_json_event
        (
          command_error_event_to_json "BuildFailed"
            [
...
let dev_artifacts_of_matches = fun matches ->
  ...
  else { tests = ArgParser.get_flag matches "tests"; examples = ArgParser.get_flag matches "examples"; benches = ArgParser.get_flag matches "benches" }
...
let parse_package_names = fun package_names ->
  let rec loop acc = function
    ...
    | package_name :: rest -> (
      ...
      | Error error -> Error (Failure ("invalid package name '" ^ package_name ^ "': " ^ Riot_model.Package_name.error_message error))
    )
  in
  loop [] package_names
...
let request_of_matches = fun ~workspace matches ->
  ...
  | Ok packages -> (
    ...
    | Ok requested_parallelism ->
        let dev_artifacts = dev_artifacts_of_matches matches in Ok (make_request ~workspace ~scope:(scope_of_dev_artifacts dev_artifacts) ~dev_artifacts ~profile:(profile_of_matches matches) ~mode:(output_mode_of_matches matches) ~requested_parallelism ~packages ~targets:(target_request_of_matches matches) ())
  )
...
let scaled_size_string = fun bytes divisor suffix ->
  ...
  let fraction = Int64.div (Int64.mul remainder 10L) divisor in Int64.to_string whole ^ "." ^ Int64.to_string fraction ^ " " ^ suffix
...
let size_to_string = fun size_bytes ->
  ...
  if Int64.compare size_bytes tib != Order.LT then
    ...
    if Int64.compare size_bytes gib != Order.LT then
      scaled_size_string size_bytes gib "GiB"
    else
      if Int64.compare size_bytes mib != Order.LT then
        scaled_size_string size_bytes mib "MiB"
      else
        if Int64.compare size_bytes kib != Order.LT then
          scaled_size_string size_bytes kib "KiB"
        else Int64.to_string size_bytes ^ " B"

let format_cache_gc_cleanup = fun (summary: Riot_store.Cache_gc.summary) -> Int.to_string summary.deleted_entries ^ " cache entries and " ^ Int.to_string summary.deleted_generations ^ " generations (" ^ size_to_string summary.size_before_bytes ^ " -> " ^ size_to_string summary.size_after_bytes ^ ")"
...
let write_cache_gc_event = fun ~mode event ->
  ...
  | Human -> (
    ...
    | Riot_store.Cache_gc.GcSkipped { summary; _ } -> out ("    Cache GC skipped: tracked cache is already within policy (" ^ size_to_string summary.size_after_bytes ^ "). Build root kept; use --force to remove it.")
    | Riot_store.Cache_gc.GcCompleted { summary; _ } -> out ("    \027[1;32mCleaned\027[0m tracked cache: " ^ format_cache_gc_cleanup summary ^ ". Build root kept.")
    | Riot_store.Cache_gc.GcFailed { error; _ } -> out ("\027[1;31mError\027[0m: cache GC failed: " ^ error)
    | Riot_store.Cache_gc.ForceCleanStarted { build_root } -> out ("    Removing build root " ^ Path.to_string build_root)
    | Riot_store.Cache_gc.ForceCleanCompleted { build_root } -> out ("    \027[1;32mRemoved\027[0m build root " ^ Path.to_string build_root)
    | Riot_store.Cache_gc.ForceCleanFailed { build_root; error } -> out ("\027[1;31mError\027[0m: failed to remove build root " ^ Path.to_string build_root ^ ": " ^ error)
  )
...
let write_package_not_found_error = fun ~mode ~package_name ~available_packages ->
  ...
  if mode = Json then
    write_json_event
      (
        command_error_event_to_json "PackageNotFound"
          [
            "package_name", Data.Json.String package_name;
            "available_packages", Data.Json.Array (List.map available_packages ~fn:(
              fun pkg -> Data.Json.String pkg
            ));
          ]
      )
  else
    (
      ...
      List.for_each available_packages ~fn:(
        fun pkg -> out ("  • " ^ pkg)
      )
    )
...
let write_packages_not_found_error = fun ~mode ~package_names ~available_packages ->
  ...
  if mode = Json then
    write_json_event
      (
        command_error_event_to_json "PackagesNotFound"
          [
            "package_names", Data.Json.Array (List.map package_names ~fn:(
              fun pkg -> Data.Json.String pkg
            ));
            "available_packages", Data.Json.Array (List.map available_packages ~fn:(
              fun pkg -> Data.Json.String pkg
...
let write_build_error = fun ~mode err ->
  ...
  | Riot_build.TargetSelectionFailed { pattern; available_targets } ->
      write_command_error ~mode "NoTargetsMatched"
        [
          "pattern", Data.Json.String pattern;
          "available_targets", Data.Json.Array (List.map available_targets ~fn:(
            fun target -> Data.Json.String (Riot_model.Target.to_string target)
          ));
        ]
        (Riot_build.error_message err)
  | Riot_build.PackageNotFound { package_name; available_packages } -> write_package_not_found_error ~mode ~package_name ~available_packages
  | Riot_build.PackagesNotFound { package_names; available_packages } -> write_packages_not_found_error ~mode ~package_names ~available_packages
  | Riot_build.ToolchainInstallFailed { target; error } ->
      write_command_error ~mode "ToolchainInstallFailed"
        [
  ...
  | Riot_build.ToolchainInitializationFailed { target; error } ->
      write_command_error ~mode "ToolchainInitializationFailed"
        [
          "target", Data.Json.String (Riot_model.Target.to_string target);
  ...
  | Riot_build.PlanningFailed planning_error ->
      if mode = Json then
                ...
                | PackageNotFound { name; available } ->
                    Data.Json.obj
                      [
                        ...
                        "available", Data.Json.array (List.map available ~fn:(
                          fun pkg -> Data.Json.string (Riot_model.Package_name.to_string pkg)
                        ));
                      ]
                | PackagesNotFound { names; available } ->
                    Data.Json.obj
                      [
                        ...
                        "names", Data.Json.array (List.map names ~fn:(
                          fun pkg -> Data.Json.string (Riot_model.Package_name.to_string pkg)
                        ));
                        "available", Data.Json.array (List.map available ~fn:(
                          fun pkg -> Data.Json.string (Riot_model.Package_name.to_string pkg)
                ...
                | MissingDependencies { missing } ->
                    Data.Json.obj
                      [
                        ...
                        "missing", Data.Json.array (List.map missing ~fn:(
                          fun dep ->
                            Data.Json.obj
                              [
                ...
                | PackageLoadFailed { errors } ->
                    Data.Json.obj
                      [
                        ...
                        "errors", Data.Json.array (List.map errors ~fn:(
                          fun error -> Data.Json.string (workspace_load_error_line error)
                        ));
                      ]);
              ]
          )
        ...
        (
          ...
          workspace_planning_error_lines planning_error |> List.for_each ~fn:(
            fun line -> out ("  " ^ line)
          )
        )
  | Riot_build.CycleDetected { cycle_nodes } ->
      write_command_error ~mode "CycleDetected"
        [
          "cycle_nodes", Data.Json.Array (List.map cycle_nodes ~fn:Data.Json.string);
        ]
        (Riot_build.error_message err)
  | Riot_build.BuildAlreadyRunning { lock_path } ->
      write_command_error ~mode "BuildAlreadyRunning"
        [
          "lock_path", Data.Json.String (Path.to_string lock_path);
        ]
        (Riot_build.error_message err)
  | Riot_build.InvalidRequestedParallelism value ->
      write_command_error ~mode "InvalidRequestedParallelism"
        [
          "value", Data.Json.Int value;
        ]
        (Riot_build.error_message err)
  | Riot_build.UnexpectedError { reason } ->
      write_command_error ~mode "UnexpectedError"
        [
          "reason", Data.Json.String reason;
        ]
        reason

let record_output_progress = fun progress output -> Riot_build.Build_result.packages output |> List.for_each ~fn:(
  fun package_output ->
...
let run_request = fun (request: request) ->
  ...
  let emit_pm_kind kind = write_pm_event ~mode:request.output_mode ~seen_registry_updates (Riot_model.Event.create ~session_id:pm_session_id ~level:Riot_model.Event.Info kind) in
  ...
  in
  let result = Riot_build.build ~on_event:on_build_event (Riot_build.Request.make ~workspace:request.workspace ~packages:request.packages ~targets:request.targets ~scope:request.scope ~dev_artifacts:request.dev_artifacts ~profile:request.profile ~requested_parallelism:request.requested_parallelism ()) |> Result.map ~fn:(
    fun output ->
    ...
    (
      ...
      | Human ->
          ...
          if progress.failed_count = 0 && progress.skipped_count = 0 then
            out ("    \027[1;32mFinished\027[0m in " ^ formatted_duration ^ "s (" ^ Int.to_string total_count ^ " built)")
          else
            if progress.failed_count > 0 then
              out ("    \027[1;31mFinished\027[0m in " ^ formatted_duration ^ "s (" ^ Int.to_string total_count ^ " built, " ^ Int.to_string progress.failed_count ^ " failed, " ^ Int.to_string progress.skipped_count ^ " skipped)")
            else out ("    \027[1;33mFinished\027[0m in " ^ formatted_duration ^ "s (" ^ Int.to_string total_count ^ " built, " ^ Int.to_string progress.skipped_count ^ " skipped)")
    );
  if request.show_finished_summary && !attempted_build then
  ...
  | Ok _ ->
      ...
      Ok ()
  | Error err ->
      trace_build_probe ~started_at:start_time ("run-request-return-error reason=" ^ Exception.to_string err);
      Error err

let print_workspace_load_errors = fun errors -> List.for_each errors ~fn:(
  fun err -> out ("\027[1;31mError\027[0m: " ^ Workspace_manager.load_error_to_string err)
)

type loaded_workspace = { workspace: Workspace.t }

let load_workspace_strict = fun cwd ->
  let workspace_manager = Workspace_manager.create () in
  let* registry = Pkgs_ml.Registry.create_filesystem ?riot_home:None ~registry_name:"pkgs.ml" () |> Result.map_err ~fn:(
    fun err -> Failure (Pkgs_ml.Registry_cache.create_error_message err)
  )
  in
  ...
  | Ok (workspace, _) ->
      let* workspace = Riot_deps.ensure_workspace ~workspace_manager ~mode:Riot_deps.Dep_solver.Refresh ~registry ~workspace () |> Result.map_err ~fn:(
        fun err -> Failure (Riot_model.Pm_error.message err)
      ) in Ok { workspace }
...
let build_command = fun ~workspace ?(scope = Runtime) ?(dev_artifacts = {tests = true; examples = true; benches = true}) ?(profile = "debug") ?(mode = Human) ?(show_finished_summary = true) ?(requested_parallelism = None) package_opt target_arch ->
  let packages = package_opt |> Option.to_list in run_request (make_request ~workspace ~scope ~dev_artifacts ~profile:(
    match profile with
    | "release" -> Riot_model.Profile.release
    | _ -> Riot_model.Profile.debug
  ) ~mode ~show_finished_summary ~requested_parallelism ~packages ~targets:(
...
let build_packages_command = fun ~workspace ?(scope = Runtime) ?(dev_artifacts = {tests = true; examples = true; benches = true}) ?(mode = Human) ?(show_finished_summary = true) ?(requested_parallelism = None) package_names target_arch ->
  ...
  | Error _ as err -> err
  | Ok packages -> run_request (make_request ~workspace ~scope ~dev_artifacts ~mode ~show_finished_summary ~requested_parallelism ~packages ~targets:(
    match target_arch with
    | Some target -> Riot_model.Target.parse target
    | None -> Riot_model.Target.Host
  ) ())
