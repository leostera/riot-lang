...
let command =
  let open ArgParser in
    ...
    |> about "Install a binary to ~/.riot/bin and project root"
    |> args
      [
        positional "package" |> help "Binary name to install";
        flag "local" |> long "local" |> help "Only install to project root, skip ~/.riot/bin";
      ]
...
let run = fun ~(workspace:Riot_model.Workspace.t) matches ->
  let open ArgParser in
    ...
    in
    match Riot_build.install ~on_event { workspace; binary_name; local_only } with
    | Ok () -> Ok ()
    | Error err ->
