...
command "yank" |> about "Yank a published package version from pkgs.ml" |> args [ positional "package" |> help "Package release in the form <name>@<version>" ]
...
let load_config = fun path ->
  ...
  | Error io_error -> Error (ConfigFailed ("failed to read config '" ^ Path.to_string path ^ "': " ^ IO.error_message io_error))
  | Ok false -> Ok User_config.default
  | Ok true -> User_config.load path |> Result.map_err ~fn:(
    fun err -> ConfigFailed (User_config.message err)
  )
...
let parse_package_spec = fun raw ->
  ...
  | [ package_name; version ] when not (String.equal (String.trim package_name) "") && not (String.equal (String.trim version) "") -> (
...
let prompt_confirmation = fun ~package_name ~version ->
  ...
  | Ok tty -> (
    ...
    | Error io_error ->
        let _ = Tty.restore tty in Error (PromptFailed (IO.error_message io_error))
    | Ok line ->
    ...
    | Ok config -> (
      ...
      | Some api_token -> (
        ...
        | Ok () -> (
          ...
          | Ok registry -> (
            match Pkgs_ml.Registry.yank_release registry ~api_token ~package_name:(Package_name.to_string package_name) ~version with
            | Error err -> fail (YankFailed err)
            | Ok _ ->
                eprintln ("Yanked " ^ Package_name.to_string package_name ^ "@" ^ version ^ " from pkgs.ml");
                Ok ()
          )
        )
      )
