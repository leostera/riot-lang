...
type suite_binary = { package_name: Package_name.t; suite_name: string }
...
let suite_progress_event_of_line = fun line ->
  ...
  if String.equal trimmed "" then
    ...
    | Ok (Data.Json.Object _ as json) -> (
      ...
      | Some "TestSummary" | None -> None
      | Some _ -> Some json
    )
    | Ok _ | Error _ -> None

let strip_progress_json_lines = fun lines -> lines |> List.filter ~fn:(
  fun line -> Option.is_none (suite_progress_event_of_line line)
)

let suite_progress_json = fun (suite: suite_binary) (json: Data.Json.t) ->
  match json with
  | Data.Json.Object fields -> Data.Json.Object (fields |> upsert_json_field "package" (Data.Json.String (Package_name.to_string suite.package_name)) |> upsert_json_field "suite" (Data.Json.String suite.suite_name))
  | other -> other

let is_test_binary_name = fun name -> String.ends_with ~suffix:"_tests" name || String.ends_with ~suffix:"-tests" name
...
let requested_packages = fun suites -> suites |> List.map ~fn:(
  fun (suite: suite_binary) -> suite.package_name
...
let realized_test_packages = fun ?(package_filters = []) (workspace: Workspace.t) -> Workspace.realize_packages ~intent:Package.Test workspace |> List.filter ~fn:Package.is_workspace_member |> List.filter ~fn:(
  fun (pkg: Package.t) -> matches_package_filters package_filters pkg.name
)

let collect_suite_binaries = fun (workspace: Workspace.t) ?(package_filters = []) ?suite_filter () -> realized_test_packages ~package_filters workspace |> List.flat_map ~fn:(
  fun (pkg: Package.t) -> List.filter_map pkg.binaries ~fn:(
...
let find_suite_source_path = fun ~(workspace:Workspace.t) (suite: suite_binary) ->
  match List.find (realized_test_packages workspace) ~fn:(
    fun (pkg: Package.t) -> Package_name.equal pkg.name suite.package_name
  ) with
  | None -> None
  | Some pkg -> List.find pkg.binaries ~fn:(
    fun (bin: Package.binary) -> String.equal bin.name suite.suite_name
  ) |> Option.map ~fn:(
    fun (bin: Package.binary) -> Path.(pkg.path / bin.path)
  )
...
let error_expected = fun expected actual -> Error ("expected " ^ expected ^ " but got " ^ json_type_name actual)
...
let split_json_stdout = fun stdout ->
  ...
  match indexed |> List.reverse |> List.find ~fn:(
    fun (_, line) -> not (String.equal (String.trim line) "")
  ) with
  | None -> Error "missing JSON output"
  | Some (json_idx, json_line) ->
      let prefix = indexed |> List.filter_map ~fn:(
        fun (idx, line) ->
          if idx < json_idx then
            Some line
          else None
      ) |> String.concat "\n" in Ok (prefix, json_line)
...
let test_type_of_json = fun json ->
  let* fields = get_object json
  in
  let* type_json = field "type" fields
  in
  let* kind = get_string type_json
  in
  match kind with
  | "test" -> Ok Test
  | "property" ->
      let* examples_json = field "examples" fields
      in
      let* examples = get_int examples_json in Ok (Property { examples })
  | other -> Error ("unknown test type " ^ other)

let test_size_of_json = fun fields ->
  match List.find fields ~fn:(
    fun (field_name, _) -> String.equal field_name "size"
  ) with
  | None -> Ok Small
  | Some (_, value) ->
      let* size = get_string value
      in
      match size with
      | "small" -> Ok Small
      | "large" -> Ok Large
...
let test_reliability_of_json = fun fields ->
  match List.find fields ~fn:(
    fun (field_name, _) -> String.equal field_name "reliability"
  ) with
  | None -> Ok Stable
  | Some (_, value) ->
      let* reliability = get_string value
      in
      ...
      | "flaky" ->
          let retry_attempts =
            match List.find fields ~fn:(
              fun (field_name, _) -> String.equal field_name "retry_attempts"
            ) with
            | Some (_, value) -> get_int value
            | None -> Ok 0
          in
          retry_attempts |> Result.map ~fn:(
            fun retry_attempts -> Flaky { retry_attempts }
          )
...
let test_status_of_json = fun json ->
  let* fields = get_object json
  in
  let* status_json = field "status" fields
  in
  let* status = get_string status_json
  in
  match status with
  | "passed" -> Ok Passed
  | "skipped" -> Ok Skipped
  | "failed" ->
      let* message_json = field "message" fields
      in
      let* message = get_string message_json in Ok (Failed message)
  | "timed_out" ->
      let* timeout_json = field "timeout_ms" fields
      in
      let* timeout_ms = get_int timeout_json in Ok (Timed_out { timeout_ms })
  | other -> Error ("unknown test status " ^ other)

let test_result_of_json = fun index json ->
  let* fields = get_object json
  in
  let* name_json = field "name" fields
  in
  let* name = get_string name_json
  in
  let* test_type = test_type_of_json json
  in
  let* size = test_size_of_json fields
  in
  let* reliability = test_reliability_of_json fields
  in
  let* attempts = optional_int_field "attempts" fields
  in
  let* result = test_status_of_json json
  in
  let* duration_us = optional_int_field "duration_us" fields
  in
  Ok {
    index;
    name;
...
let test_case_type_field_of_json = fun fields ->
  match List.find fields ~fn:(
    fun (field_name, _) -> String.equal field_name "test_type"
  ) with
  | None -> Ok Test
  ...
  | Some (_, Data.Json.String "property") ->
      let* examples =
        match List.find fields ~fn:(
          fun (field_name, _) -> String.equal field_name "examples"
        ) with
        | Some (_, value) -> get_int value
        | None -> Ok 0
      in
...
let suite_progress_test_case_result = fun json ->
  ...
  | Some "TestCaseCompleted" ->
      ...
      let* name = get_string name_json
      in
      let* test_type = test_case_type_field_of_json fields
      in
      let* size = test_size_of_json fields
      in
      let* reliability = test_reliability_of_json fields
...
let listed_test_case_of_json = fun json ->
  let* fields = get_object json
  in
  let* index_json = field "index" fields
  in
  let* name_json = field "name" fields
  in
  let* index = get_int index_json
  in
  let* name = get_string name_json
  in
  let* test_type = test_type_of_json json
  ...
  let skip =
    match List.find fields ~fn:(
      fun (field_name, _) -> String.equal field_name "skip"
    ) with
...
let test_summary_of_json = fun json ->
  let* fields = get_object json
  in
  let* total_json = field "total" fields
  in
  let* passed_json = field "passed" fields
  in
  let* failed_json = field "failed" fields
  in
  let* skipped_json = field "skipped" fields
  in
  let* total = get_int total_json
  in
  let* passed = get_int passed_json
  in
  let* failed = get_int failed_json
  in
  let* skipped = get_int skipped_json
  in
  let* duration_us = optional_int_field "duration_us" fields in Ok (total, passed, failed, skipped, Option.unwrap_or ~default:0 duration_us)

let parse_test_suite_output = fun stdout ->
  let* (prefix_stdout, json_line) = split_json_stdout stdout
  in
  let prefix_stdout = prefix_stdout |> String.split ~by:"\n" |> strip_progress_json_lines |> String.concat "\n" in
  let* json = Data.Json.of_string json_line |> Result.map_err ~fn:Data.Json.error_to_string
  in
  let* fields = get_object json
  in
  let* tests_json = field "tests" fields
  in
  let* summary_json = field "summary" fields
  in
  let* tests = get_array tests_json
  in
  let rec parse_results index acc = function
    | [] -> Ok (List.reverse acc)
    | test_json :: rest ->
        let* result = test_result_of_json index test_json in parse_results (index + 1) (result :: acc) rest
  in
  let* results = parse_results 1 [] tests
  in
  let* (total, passed, failed, skipped, summary_duration_us) = test_summary_of_json summary_json
  in
  let* started_at_us = optional_int_field "started_at_us" fields
  in
  let* completed_at_us = optional_int_field "completed_at_us" fields
  in
  let* duration_us = optional_int_field "duration_us" fields in Ok (prefix_stdout, started_at_us, completed_at_us, duration_us, {
    total;
    passed;
    failed;
    skipped;
...
let parse_listed_tests_output = fun stdout ->
  let* (_prefix_stdout, json_line) = split_json_stdout stdout
  in
  let* json = Data.Json.of_string json_line |> Result.map_err ~fn:Data.Json.error_to_string
  in
  let* fields = get_object json
  ...
  let rec loop acc = function
    | [] -> Ok (List.reverse acc)
    | json :: rest ->
        let* listed = listed_test_case_of_json json in loop (listed :: acc) rest
  in
  loop [] tests
...
let summarize_output = fun value ->
  ...
  if String.equal trimmed "" then
    ...
    if String.length trimmed <= limit then
      trimmed
    else String.sub trimmed ~offset:0 ~len:limit ^ "..."
...
let test_event_to_json = function
      ...
      Some (
        Data.Json.Object (
          [
            ...
            "binary_path", Data.Json.String (Path.to_string binary_path);
            "args", Data.Json.Array (List.map args ~fn:(
              fun arg -> Data.Json.String arg
            ));
          ] @ suite_event_fields suite
        )
      )
  ...
  | SuiteCompleted { suite; status; stdout; stderr; started_at_us; completed_at_us; duration_us; summary } ->
      let test_results = summary.results |> List.map ~fn:(
        fun (result: test_case_result) ->
          ...
          let reliability_fields =
            ...
            | Stable ->
                [
                  "reliability", Data.Json.String "stable";
                ]
          ...
          Data.Json.Object (
            ...
            ] @ status_fields) @ size_fields) @ reliability_fields) @ type_fields
          )
      ) in
      Some (
        Data.Json.Object [
  ...
  | Summary { total; passed; failed; skipped; failed_tests } ->
      let failed_tests = failed_tests |> List.map ~fn:(
...
let ensure_executable_binary_path = fun ~kind path ->
  ...
  | Ok metadata ->
      ...
      if mode land 0o111 != 0 then
        Ok path
      else Fs.set_permissions path (Fs.Permissions.of_mode (mode lor 0o111)) |> Result.map ~fn:(
        fun () -> path
      ) |> Result.map_err ~fn:(
        fun err -> "failed to mark " ^ kind ^ " executable: " ^ IO.error_message err
      )

let materialized_export_path = fun ~(workspace:Workspace.t) ~profile ~package_name ~export_name ->
  let out_dir = Riot_model.Riot_dirs.out_dir_in_workspace ~workspace ~profile ~target:(Riot_model.Riot_dirs.host_target ()) in Path.(out_dir / Path.v (Package_name.to_string package_name) / Path.v export_name)

let find_export_path_in_output = fun ~(workspace:Workspace.t) ~profile ~(store:Riot_store.Store.t) ~kind ~package_name ~export_name (output: Riot_build.Build_result.t) ->
  ...
  let ensure_materialized_fallback () =
    ...
    | Ok true -> ensure_executable_binary_path ~kind fallback_path |> Result.map_err ~fn:(
      fun reason -> reason
    )
    | Ok false | Error _ -> Error (kind ^ " '" ^ export_name ^ "' was not produced by build output")
  in
  match Riot_build.Build_result.find_package output package_name |> Option.and_then ~fn:(
    fun package_output -> Riot_build.Build_result.find_export package_output export_name
  ) with
  | None -> ensure_materialized_fallback ()
  | Some export_entry -> (
    match Riot_store.Store.export_source_path store export_entry with
    | Some path -> ensure_executable_binary_path ~kind path |> Result.map_err ~fn:(
      fun reason -> reason
    )
    | None -> ensure_materialized_fallback ()
  )

let find_suite_binary_path_in_output = fun ~(workspace:Workspace.t) ~profile ~(store:Riot_store.Store.t) ~(suite:suite_binary) (output: Riot_build.Build_result.t) -> find_export_path_in_output ~workspace ~profile ~store ~kind:"suite binary" ~package_name:suite.package_name ~export_name:suite.suite_name output |> Result.map_err ~fn:(
  fun reason -> SuiteArtifactNotFound { suite; reason }
)

let suite_ctx_json_value = fun ~workspace_root ~package_name ?source_file ~binary_path ~built_binaries () ->
  let built_binaries_json = built_binaries |> List.map ~fn:(
    fun (binary: Test.Context.built_binary) ->
      Data.Json.Object [
...
let runtime_output_packages = fun ~(workspace:Workspace.t) (output: Riot_build.Build_result.t) -> Workspace.realize_packages ~intent:Package.Runtime workspace |> List.filter ~fn:(
  fun (pkg: Package.t) -> Option.is_some (Riot_build.Build_result.find_package output pkg.name)
)

let reachable_runtime_packages = fun packages start_package_name ->
  let find_package package_name = List.find packages ~fn:(
  ...
  let rec visit acc package_name =
    if Collections.HashSet.contains seen ~value:package_name then
      ...
      | None -> acc
      | Some (pkg: Package.t) ->
          let acc = pkg :: acc in List.fold_left pkg.dependencies ~init:acc ~fn:(
            fun acc (dep: Package.dependency) -> visit acc dep.name
          )
  in
  visit [] start_package_name |> List.reverse

let runtime_output_built_binaries = fun ~(workspace:Workspace.t) ~package_name ~profile ~(store:Riot_store.Store.t) (output: Riot_build.Build_result.t) ->
  let packages = runtime_output_packages ~workspace output in reachable_runtime_packages packages package_name |> List.flat_map ~fn:(
    fun (pkg: Package.t) -> List.filter_map pkg.binaries ~fn:(
      fun (bin: Package.binary) ->
        match find_export_path_in_output ~workspace ~profile ~store ~kind:"built binary" ~package_name:pkg.name ~export_name:bin.name output with
...
let run_suite = fun ~on_event ~workspace_root ~suite ?source_file ~built_binaries ~extra_args binary_path ->
  let ctx_json = suite_ctx_json_value ~workspace_root ~package_name:suite.package_name ?source_file ~binary_path ~built_binaries () in
  ...
  match Command.output cmd ~on_idle:(
    fun elapsed -> on_event (SuiteHeartbeat { suite; binary_path; elapsed_us = Time.Duration.to_micros elapsed })
  ) ~on_stdout_line:(
    fun line -> suite_progress_event_of_line line |> Option.for_each ~fn:(
      fun event -> on_event (SuiteProgress { suite; event })
    )
  ) with
  ...
  | Ok output -> (
    ...
    let parsed_output =
      ...
      | Ok parsed -> Ok parsed
      | Error _reason when Int.equal output.status 0 && is_blank output.stdout && is_blank output.stderr -> Ok ("", None, None, None, empty_suite_summary)
      | Error reason -> Error reason
    in
    match parsed_output with
    | Error reason -> Error (SuiteExecutionError { suite; reason = parse_failure_reason ~suite ~output reason })
...
let list_suite_binary_capture = fun ~(suite:suite_binary) ~extra_args binary_path ->
  ...
  let cmd = Command.make (Path.to_string binary_path) ~args:("list-tests" :: extra_args) in Command.output cmd
...
let build_output = fun ~(workspace:Workspace.t) ~packages ~profile ?on_event () -> Riot_build.Request.make ~workspace ~packages ~targets:Target.Host ~scope:Riot_build.Request.Dev ~profile:(profile_of_name profile) () |> Riot_build.build ?on_event

let store_for_request = fun (request: test_request) -> Riot_store.Store.create_for_lane ~workspace:request.workspace ~profile:request.profile ~target:(Riot_dirs.host_target ())
...
let list_tests = fun ?(on_suite = no_listed_suite) ?(on_suite_error = no_list_error) (request: test_request) ->
  let suites = collect_suite_binaries request.workspace ~package_filters:request.package_filters ?suite_filter:request.suite_filter () in
  if suites = [] then
    Ok []
  else
    ...
    | Ok output ->
        ...
        let suite_binaries, missing_suites = resolve_suite_binaries ~workspace:request.workspace ~profile:request.profile ~store ~suites output in
        List.for_each missing_suites ~fn:(
        ...
        if suite_binaries = [] then
                  ...
                  let _worker =
                    spawn
                      (
                        fun () ->
                          let result =
                            try list_suite ~workspace:request.workspace ~suite ~extra_args:request.extra_args binary_path with
                            | exn -> Error (SuiteExecutionError { suite; reason = Exception.to_string exn })
                          in
          ...
          let rec collect active remaining acc =
            if active <= 0 then
              ...
              let suite, result = receive ~selector:(
                fun (msg: Message.t) ->
                  match msg with
                  ...
                  let _worker =
                    spawn
                      (
                        fun () ->
                          let result =
                            try list_suite ~workspace:request.workspace ~suite:next_suite ~extra_args:request.extra_args next_binary_path with
                            | exn -> Error (SuiteExecutionError { suite = next_suite; reason = Exception.to_string exn })
                          in
          ...
          in
          let initial_active, remaining = spawn_initial 0 suite_binaries in collect initial_active remaining [] |> Result.map ~fn:(
            fun collected -> collected |> List.sort ~compare:(
              fun (left, _) (right, _) -> compare_suite_binary left right
            ) |> List.map ~fn:(
              fun (_, value) -> value
            )
          )
...
let test = fun ?(on_event = no_event) (request: test_request) ->
  ...
  if suites = [] then
    (
      on_event (NoSuitesFound { package_name = selected_package_name request.package_filters; suite_name = request.suite_filter });
      Ok ()
    )
  else
    let () = on_event (TestSuitesCollected { package_name = selected_package_name request.package_filters; suite_name = request.suite_filter; suite_count = List.length suites }) in
    match build_output ~workspace:request.workspace ~packages:(requested_packages suites) ~profile:request.profile ~on_event:(
      fun event -> on_event (Build event)
    ) () with
    | Error err -> Error (BuildFailed err)
    | Ok output ->
        let store = store_for_request request in
        ...
        let rec loop = function
          ...
          | suite :: rest -> (
            ...
            match find_suite_binary_path_in_output ~workspace:request.workspace ~profile:request.profile ~store ~suite output with
            | Error _ as err -> err
            | Ok binary_path ->
                let source_file = find_suite_source_path ~workspace:request.workspace suite in
                let built_binaries = runtime_output_built_binaries ~workspace:request.workspace ~package_name:suite.package_name ~profile:request.profile ~store output in
                on_event (SuiteBinaryResolved { suite; binary_path });
                on_event (RunningSuite suite);
                match run_suite ~on_event ~workspace_root:request.workspace.root ~suite ?source_file ~built_binaries ~extra_args:request.extra_args binary_path with
                | Error _ as err -> err
                | Ok suite_output ->
                    ...
                    suite_output.summary.results |> List.for_each ~fn:(
                      fun (result: test_case_result) ->
                        match result.result with
                        | Failed message -> Vector.push failed_tests ~value:{
                          suite = suite_output.suite;
                          name = result.name;
                          message;
                          duration_us = result.duration_us
                        }
                        | Timed_out { timeout_ms } -> Vector.push failed_tests ~value:{
                          suite = suite_output.suite;
                          name = result.name;
