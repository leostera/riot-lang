...
command "login" |> about "Save your pkgs.ml API token" |> args [ option "token" |> long "token" |> help "API token to save without prompting" ]
...
let prompt_for_token = fun () ->
  ...
  match Tty.make () with
  | Error Tty.NoTtyConnected -> fail "failed to read API token: no tty connected"
  | Error (Tty.SystemError io_error) -> fail ("failed to read API token: " ^ IO.error_message io_error)
  | Ok tty -> (
