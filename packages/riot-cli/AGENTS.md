...
48. `riot info --json` is the canonical editor-facing workspace introspection surface. Keep it workspace-root scoped, include the resolved workspace manifest plus each package manifest, and encode workspace-load failures as structured stdout payloads in JSON mode.
