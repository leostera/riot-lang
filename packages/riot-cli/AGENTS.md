...
34. `Riot_cli.Cli.run` should stamp the default `X-Riot-Agent` value early through the `pkgs-ml` client API so downstream registry and CDN clients emit Riot version metadata consistently without relying on process environment state.
