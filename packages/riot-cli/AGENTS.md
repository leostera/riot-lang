...
34. `riot build --watch`, `riot test --watch`, and `riot run --watch` should watch selected workspace package roots plus their transitive workspace dependency roots, not the entire repository. Keep generated output roots, `.riot`, `.git`, `riot.lock`, and snapshot candidates such as `*.expected.new` from retriggering the loop. For `riot run --watch`, restart the local workspace binary when its dependency cone changes.
35. `riot build --deps` is a lockfile-driven Docker cache primitive. It should tolerate missing workspace member manifests, skip workspace/path packages, materialize registry/source packages from `riot.lock`, and build those third-party dependency libraries without refreshing the lockfile.
