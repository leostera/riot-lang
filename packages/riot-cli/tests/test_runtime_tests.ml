...
let sample_suite = { Riot_cli.Test_runtime.package_name = package_name "demo"; suite_name = "demo_tests" }
...
let expect_field = fun fields name expected ->
  ...
  | Some (_, actual) -> Error ("expected field " ^ name ^ " to equal " ^ Data.Json.to_string expected ^ ", got " ^ Data.Json.to_string actual)
  | None -> Error ("missing field " ^ name)

let test_suite_heartbeat_event_to_json = fun _ctx ->
  match Riot_cli.Test_runtime.test_event_to_json (SuiteHeartbeat { suite = sample_suite; binary_path = Path.v "/tmp/demo_tests"; elapsed_us = 1_234 }) with
  | Some (Data.Json.Object fields) ->
      let* () = expect_field fields "type" (Data.Json.String "SuiteHeartbeat")
      in
      let* () = expect_field fields "package" (Data.Json.String "demo")
      in
      let* () = expect_field fields "suite" (Data.Json.String "demo_tests")
      in
      let* () = expect_field fields "binary_path" (Data.Json.String "/tmp/demo_tests") in expect_field fields "elapsed_us" (Data.Json.Int 1_234)
...
let test_suite_progress_test_case_result_parses_completed_case = fun _ctx ->
  ...
  | Ok (Some Riot_cli.Test_runtime.{ index; name; test_type = Property { examples }; size = Large; reliability = Flaky { retry_attempts }; attempts; result = Passed; duration_us }) ->
      if Int.equal index 3 && String.equal name "alpha" && Int.equal examples 7 && Int.equal retry_attempts 2 && Int.equal attempts 3 && Int.equal duration_us 1_234 then
        Ok ()
      else Error "expected parsed completed case metadata to round-trip"
...
let tests = [ Test.case "suite heartbeat event renders json" test_suite_heartbeat_event_to_json; Test.case "suite progress completed case parses into a test result" test_suite_progress_test_case_result_parses_completed_case; Test.case "suite progress ignores non-completed events" test_suite_progress_test_case_result_ignores_non_completed_event ]
