...
let test_help_normalizes_to_global_help = fun _ctx ->
  Test.assert_equal ~expected:[ "riot"; "--help" ] ~actual:(Riot_cli.Cli.normalize_args [ "riot"; "help" ]);
  Ok ()
...
let tests = Test.[ case "lsp: parse stdio transport subcommand" test_lsp_stdio_subcommand_parses; case "cli: help normalizes to global help" test_help_normalizes_to_global_help; case "lsp: build package named lsp parses normally" test_build_package_named_lsp_parses ]
