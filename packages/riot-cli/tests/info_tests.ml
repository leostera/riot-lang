...
let make_registry_cache = fun riot_home -> Pkgs_ml.Registry_cache.create ~riot_home ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache"

let sample_release = fun ~version -> Pkgs_ml.Sparse_index.{
  version;
...
let sample_document = fun () -> Pkgs_ml.Sparse_index.{
  schema_version = 1;
  name = "serde-json";
  latest = "1.2.0";
...
let sample_registry = fun ~riot_home ->
  let cache = make_registry_cache riot_home in Pkgs_ml.Registry.in_memory ~cache ~packages:[ sample_document () ] ~releases:[
    {
...
let make_local_workspace = fun ?is_public root ->
  ...
  let manifest = Data.Toml.parse manifest_toml |> Result.expect ~msg:"expected toml parse" |> Riot_model.Package_manifest.from_toml ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:package_root ~relative_path:(Path.v "demo") |> Result.expect ~msg:"expected package manifest" in Riot_model.Workspace_manifest.make ~root ~packages:[ manifest ] ()
  ...
  with_tempdir_result "riot_cli_info_local"
    (
      fun tempdir ->
        ...
        match Riot_cli.Info_package.resolve ~registry ~local_workspace:(Some (workspace, [])) ~target:"demo" () with
  ...
  with_tempdir_result "riot_cli_info_registry"
    (
      fun tempdir ->
        let riot_home = Path.(tempdir / Path.v ".riot") in
        let registry = sample_registry ~riot_home in
        match Riot_cli.Info_package.resolve ~registry ~local_workspace:None ~target:"serde-json@1.2.0" () with
        | Error err -> Error err.message
        | Ok info ->
      ...
      fun tempdir ->
        ...
        | Ok info ->
            ...
            let docs_link =
              ...
              | Some (Data.Json.Object fields) -> Data.Json.get_field "docs_url" (Data.Json.Object fields)
              | _ -> None
            in
...
let test_info_workspace_scan_error_message_renders_typed_errors = fun _ctx ->
  let cwd_message = Riot_cli.Info_cmd.workspace_scan_error_message (Riot_cli.Info_cmd.CurrentDirReadFailed (Path.SystemError "cwd unavailable")) in
  let scan_message = Riot_cli.Info_cmd.workspace_scan_error_message (Riot_cli.Info_cmd.WorkspaceScanFailed Riot_model.Workspace_manager.NoWorkspaceRootFound) in
