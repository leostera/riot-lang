...
let pending_paths = fun workspace_root ->
  let fixture_pending = Path.(workspace_root / Path.v "packages/std/tests/fixtures/sample.expected.new") in
  let fixture_approved = Path.(workspace_root / Path.v "packages/std/tests/fixtures/sample.expected") in
  let custom_pending = Path.(workspace_root / Path.v "packages/syn/tests/fixtures/sample.expected_lossless.json.new") in
  let custom_approved = Path.(workspace_root / Path.v "packages/syn/tests/fixtures/sample.expected_lossless.json") in
  let workspace_pending = Path.(workspace_root / Path.v ".riot/snapshots/std/suite/case.expected.new") in
  let workspace_approved = Path.(workspace_root / Path.v ".riot/snapshots/std/suite/case.expected") in
  let build_pending = Path.(workspace_root / Path.v "_build/debug/std/ignored.expected.new") in (fixture_pending, fixture_approved, custom_pending, custom_approved, workspace_pending, workspace_approved, build_pending)
    ...
    (
      fun _ctx ->
        with_tempdir_result "snapshots_discover"
          (
            fun workspace_root ->
              let fixture_pending, _, custom_pending, _, workspace_pending, _, build_pending = pending_paths workspace_root in
                ...
                | Ok () -> (
                  ...
                  | Ok () -> (
                    ...
                    | Ok () -> (
                      ...
                      | Error err -> Error (IO.error_message err)
                      | Ok snapshots ->
                          let actual = List.map snapshots ~fn:(
                            fun snapshot -> Path.to_string snapshot.Riot_cli.Snapshots.pending
                          ) in
                          let expected = [ Path.to_string workspace_pending; Path.to_string fixture_pending; Path.to_string custom_pending ] |> List.sort ~compare:String.compare in
            ...
            fun workspace_root ->
              ...
              | Ok () -> (
                ...
                | Ok () -> (
                  ...
                  | Error msg -> Error msg
                  | Ok () -> (
                    match Riot_cli.Snapshots.discover_pending_snapshots ~workspace_root ~query:"lossless" () with
                    | Error err -> Error (IO.error_message err)
                    | Ok [ snapshot ] ->
                        Test.assert_equal ~expected:(Path.to_string custom_pending) ~actual:(Path.to_string snapshot.pending);
                        Ok ()
                    | Ok snapshots -> Error ("expected one filtered snapshot, got " ^ Int.to_string (List.length snapshots))
                  )
                )
              )
          )
      ...
      fun _ctx ->
        ...
        | Ok matches -> (
          ...
          | Some ("approve", approve_matches) ->
              Test.assert_equal ~expected:(Some "fixture") ~actual:(ArgParser.get_one approve_matches "query");
              Ok ()
    ...
    (
      fun _ctx ->
        with_tempdir_result "snapshots_review"
          (
            fun workspace_root ->
              let fixture_pending, fixture_approved, custom_pending, custom_approved, workspace_pending, workspace_approved, _ = pending_paths workspace_root in
              let setup_result =
                match write_file fixture_pending "fixture pending\n" with
                | Error _ as err -> err
                | Ok () -> (
              ...
              | Ok () -> (
                ...
                | Ok snapshots -> (
                  ...
                  in
                  match Riot_cli.Snapshots.review_pending_snapshots_with_decider ~workspace_root snapshots ~decide with
                  | Error err -> Error (IO.error_message err)
                  | Ok summary ->
                      ...
                      if not (String.equal fixture_approved_content "fixture pending\n") then
                        ...
                        if fixture_pending_exists then
                          Error "expected approved pending snapshot to be removed"
                        else
                          if custom_approved_exists then
                            Error "expected rejected snapshot not to create an approved file"
                          else
                            if custom_pending_exists then
                              Error "expected rejected pending snapshot to be removed"
                            else
                              if workspace_approved_exists then
                                Error "expected ignored snapshot not to create an approved file"
                              else
                                if not workspace_pending_exists then
                                  Error "expected ignored pending snapshot to remain"
                                else
                                  if not (Int.equal summary.Riot_cli.Snapshots.approved_count 1) || not (Int.equal summary.rejected_count 1) || not (Int.equal summary.ignored_count 1) || summary.quit then
                ...
                | Ok snapshots -> (
                  ...
                  | Ok summary ->
                      ...
                      if approved_exists then
                        ...
                        if not fixture_pending_exists || not custom_pending_exists then
                          Error "expected quit to leave pending snapshots untouched"
                        else
                          if not summary.Riot_cli.Snapshots.quit || not (Int.equal summary.approved_count 0) || not (Int.equal summary.rejected_count 0) || not (Int.equal summary.ignored_count 0) then
