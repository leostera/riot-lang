  ...
  with_tempdir_result
    ...
    (fun tempdir ->
      ...
      let* () =
        ...
        |> Result.map_err ~fn:Riot_cli.Add.message
      in
      ...
      let* lockfile =
        ...
        |> Result.map_err
          ~fn:(fun err ->
            "expected lockfile read to succeed: " ^ Riot_deps.Lockfile_store.error_message err)
      in
...
let test_add_message_renders_typed_command_errors = fun _ctx ->
  ...
  let expected_bootstrap =
    "failed to initialize riot workspace: manifest '/workspace/riot.toml' must decode to a TOML table"
  in
  let expected_load =
    "failed to load initialized riot workspace: package 'dep': failed to parse riot.toml at path deps/dep/riot.toml"
  in
...
let test_remove_outside_workspace_message = fun _ctx ->
  ...
  Test.assert_equal
    ~expected:"No riot.toml, so nothing to remove"
    ~actual:Riot_cli.Remove.no_workspace_message;
  Ok ()
...
let tests =
  Test.[
    case
      "package commands: add bootstraps an empty workspace outside a workspace"
      test_add_bootstraps_empty_workspace_outside_workspace;
    case
      "package commands: add renders typed command errors"
      test_add_message_renders_typed_command_errors;
    case
      "package commands: add accepts multiple dependencies"
      test_add_accepts_multiple_dependencies;
    case
      "package commands: remove outside a workspace reports no riot.toml"
      test_remove_outside_workspace_message;
    case
      "package commands: remove renders typed command errors"
      test_remove_message_renders_typed_command_errors;
    case
      "package commands: remove accepts multiple dependencies"
      test_remove_accepts_multiple_dependencies;
    case
      "package commands: update outside a workspace reports no riot.toml"
      test_update_outside_workspace_message;
    case "package commands: update accepts package names" test_update_accepts_package_names;
    case
      "package commands: new outside a workspace creates a standalone package"
      test_new_outside_workspace_creates_standalone_package;
  ]
