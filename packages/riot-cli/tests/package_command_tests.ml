...
let path_error_message = function
  ...
  | Path.SystemInvalidUtf8 { syscall; path } -> "system returned invalid utf-8 for " ^ syscall ^ ": " ^ path
  ...
  | Ok original ->
      ...
      | Ok () ->
          ...
          (
            try
              let result = fn () in
              let () = restore () in result
            with
            | exn ->
                let () = restore () in raise exn
          )
  ...
  with_tempdir_result "riot_cli_add_bootstrap"
    (
      fun tempdir ->
        ...
        in
        let* () = Riot_cli.Add.bootstrap_empty_workspace ~root:workspace_root |> Result.map_err ~fn:Riot_cli.Add.message
        in
        let* manifest_source = Result.map_err (Fs.read manifest_path) ~fn:IO.error_message
        in
        let* lockfile = Riot_deps.Lockfile_store.read ~workspace_root |> Result.map_err ~fn:(
          fun err -> "expected lockfile read to succeed: " ^ Riot_deps.Lockfile_store.error_message err
        )
        in
        let* matches = parse_add [ "add"; "hello" ]
        in
        let* selection = Riot_cli.Add.selection_of_matches ~default_selection:Riot_deps.Workspace matches |> Result.map_err ~fn:Riot_cli.Add.message
        in
        let workspace_manager = Riot_model.Workspace_manager.create () in
        let* (_workspace, load_errors) = Riot_model.Workspace_manager.scan workspace_manager workspace_root |> Result.map_err ~fn:(
        ...
        if not (String.contains manifest_source "[workspace]") then
          ...
          if not (String.contains manifest_source "members = []") then
            Error "expected bootstrap manifest to start with an empty workspace"
          else
            match lockfile with
            | None -> Error "expected add to create riot.lock"
            | Some lockfile ->
                let* manifest_exists = Result.map_err (Fs.exists manifest_path) ~fn:IO.error_message
                in
                let* lock_exists = Result.map_err (Fs.exists lockfile_path) ~fn:IO.error_message
                in
                if manifest_exists && lock_exists && List.is_empty lockfile.packages && load_errors = [] && selection = Riot_deps.Workspace then
                  Ok ()
                else Error "expected bootstrap to create a loadable empty workspace and default add to the workspace root"
    )

let test_add_message_renders_typed_command_errors = fun _ctx -> let open Riot_cli.Add in
let manifest_path = Path.v "/workspace/riot.toml" in
let bootstrap_message = message (WorkspaceBootstrapFailed (BootstrapDependencyHashFailed (Riot_deps.Lock_refresh.ManifestMustBeTable { manifest_path }))) in
let load_message = message (WorkspaceLoadFailed (WorkspaceLoadHadErrors [ Riot_model.Workspace_manager.PackageTomlParseFailed { package = "dep"; path = "deps/dep/riot.toml" } ])) in
...
let test_add_accepts_multiple_dependencies = fun _ctx ->
  ...
  in
  Test.assert_equal ~expected:[ "std"; "serde-json"; "../widgets" ] ~actual:(ArgParser.get_many matches "dependency");
  Ok ()

let test_remove_outside_workspace_message = fun _ctx ->
  let* matches = parse_remove [ "rm"; "hello" ]
  in
  let* () = Result.map_err (Riot_cli.Remove.run_without_workspace matches) ~fn:Exception.to_string
  in
  Test.assert_equal ~expected:"No riot.toml, so nothing to remove" ~actual:Riot_cli.Remove.no_workspace_message;
  Ok ()

let test_remove_message_renders_typed_command_errors = fun _ctx -> let open Riot_cli.Remove in
...
let test_remove_accepts_multiple_dependencies = fun _ctx ->
  let* matches = parse_remove [ "rm"; "std"; "serde-json" ]
  in
  Test.assert_equal ~expected:[ "std"; "serde-json" ] ~actual:(ArgParser.get_many matches "dependency");
  Ok ()

let test_update_outside_workspace_message = fun _ctx ->
  let* matches = parse_update [ "update" ]
  in
  let* () = Result.map_err (Riot_cli.Update_cmd.run_without_workspace matches) ~fn:Exception.to_string
  in
...
let test_update_accepts_package_names = fun _ctx ->
  let* matches = parse_update [ "update"; "std"; "serde-json" ]
  in
  ...
  with_tempdir_result "riot_cli_new_standalone"
    (
      fun tempdir ->
        ...
        let* matches = parse_new [ "new"; "hello-world" ]
        in
        let* () =
          with_current_dir_exn_result tempdir
            (
              fun () -> Riot_cli.New.run matches
            ) |> Result.map_err ~fn:Exception.to_string
        in
        let* package_exists = Result.map_err (Fs.exists Path.(package_root / Path.v "riot.toml")) ~fn:IO.error_message
        in
        let* main_exists = Result.map_err (Fs.exists Path.(package_root / Path.v "src" / Path.v "HelloWorld.ml")) ~fn:IO.error_message
        in
        let* manifest_source = Result.map_err (Fs.read Path.(package_root / Path.v "riot.toml")) ~fn:IO.error_message
        in
        if package_exists && main_exists && String.contains manifest_source "[package]" then
          Ok ()
        else Error "expected riot new outside a workspace to create a standalone package"
    )
