...
let test_parse_request_keeps_global_query = fun _ctx ->
  let request = Test_selection.parse_request ~filter:(Some "hello") ~package_filters:[] ~size_filter:Test_selection.All ~flaky_only:false |> Result.expect ~msg:"parse request failed" in
...
let test_parse_request_uses_package_flag_for_narrowing = fun _ctx ->
  let request = Test_selection.parse_request ~filter:(Some "hello") ~package_filters:[ package_name "std" ] ~size_filter:Test_selection.All ~flaky_only:false |> Result.expect ~msg:"parse request failed" in
...
let test_parse_request_keeps_multiple_package_filters = fun _ctx ->
  let request = Test_selection.parse_request ~filter:(Some "hello") ~package_filters:[ package_name "std"; package_name "syn" ] ~size_filter:Test_selection.All ~flaky_only:false |> Result.expect ~msg:"parse request failed" in
...
let test_parse_request_extracts_package_and_suite_selector = fun _ctx ->
  let request = Test_selection.parse_request ~filter:(Some "syn:diagnostic_tests") ~package_filters:[] ~size_filter:Test_selection.All ~flaky_only:false |> Result.expect ~msg:"parse request failed" in
...
let test_parse_request_extracts_package_suite_and_query = fun _ctx ->
  let request = Test_selection.parse_request ~filter:(Some "syn:diagnostic_tests:0001") ~package_filters:[] ~size_filter:Test_selection.All ~flaky_only:false |> Result.expect ~msg:"parse request failed" in
  Test.assert_equal ~expected:[ package_name "syn" ] ~actual:request.package_filters;
  Test.assert_equal ~expected:(Some (package_name "syn")) ~actual:request.package_filter;
  Test.assert_equal ~expected:(Some "diagnostic_tests") ~actual:request.suite_filter;
...
let test_parse_request_preserves_raw_query_with_package_flag = fun _ctx ->
  let request = Test_selection.parse_request ~filter:(Some "syn:diagnostic_tests") ~package_filters:[ package_name "std" ] ~size_filter:Test_selection.All ~flaky_only:false |> Result.expect ~msg:"parse request failed" in
  Test.assert_equal ~expected:[ package_name "std" ] ~actual:request.package_filters;
  Test.assert_equal ~expected:(Some (package_name "std")) ~actual:request.package_filter;
  Test.assert_equal ~expected:None ~actual:request.suite_filter;
...
let test_extra_args_omits_query_when_absent = fun _ctx ->
  let request = Test_selection.parse_request ~filter:None ~package_filters:[ package_name "std" ] ~size_filter:Test_selection.All ~flaky_only:false |> Result.expect ~msg:"parse request failed" in
...
let test_extra_args_prefixes_query_when_present = fun _ctx ->
  let request = Test_selection.parse_request ~filter:(Some "_long") ~package_filters:[] ~size_filter:Test_selection.All ~flaky_only:false |> Result.expect ~msg:"parse request failed" in
  let actual = Test_selection.extra_args request [ "--format"; "json" ] in
  Test.assert_equal ~expected:[ "_long"; "--format"; "json" ] ~actual;
  Ok ()

let test_extra_args_include_selection_flags = fun _ctx ->
  let request = Test_selection.parse_request ~filter:(Some "probe") ~package_filters:[] ~size_filter:Test_selection.Small ~flaky_only:true |> Result.expect ~msg:"parse request failed" in
...
let test_extra_args_include_policy_flags = fun _ctx ->
  let request = Test_selection.parse_request ~filter:None ~package_filters:[] ~size_filter:Test_selection.All ~flaky_only:false |> Result.expect ~msg:"parse request failed" in
  let actual = Test_selection.extra_args ~small_test_timeout:(Some (Time.Duration.from_millis 500)) ~flaky_max_retries:3 request [] in
...
let test_bench_command_parses_repeated_packages_and_filter = fun _ctx ->
  ...
  | Ok matches -> (
    ...
    | Some ("bench", bench_matches) ->
        ...
        Test.assert_equal ~expected:(Some 500) ~actual:(ArgParser.get_int bench_matches "iterations");
