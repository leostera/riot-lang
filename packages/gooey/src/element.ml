...
let row = fun ?(style = Style.empty) children ->
  let style = Style.(style |> row) in Container { style; children }

let column = fun ?(style = Style.empty) children ->
  let style = Style.(style |> column) in Container { style; children }

let spacer = fun ?(flex = 1.0) () -> Container { style = Style.(empty |> grow |> grow_weight flex); children = [] }
