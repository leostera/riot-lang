  ...
  List.iter (fun cmd ->
    ...
    | Rectangle { color; _ } ->
        ...
        for row = row_start to row_end - 1 do
          for col = col_start to col_end - 1 do
            if is_inside_scissor col row (Cell.get scissor_box) then begin
    ...
    | Text { content; color; _ } ->
        ...
        if is_inside_scissor col row (Cell.get scissor_box) then begin
          ...
          List.iteri (fun i line ->
            if i > 0 then Buffer.add_string buf (Tty.Escape_seq.cursor_position_seq (row + i + 1) (col + 1));
    ...
    | Border { width; color; _ } ->
        ...
        if width.top > 0 then begin
          Buffer.add_string buf (Tty.Escape_seq.cursor_position_seq (row_start + 1) (col_start + 1));
        ...
        for row = row_start + 1 to row_end - 2 do
          if width.left > 0 && is_inside_scissor col_start row (Cell.get scissor_box) then begin
            Buffer.add_string buf (Tty.Escape_seq.cursor_position_seq (row + 1) (col_start + 1));
            Buffer.add_string buf (format_with_fg tty_color "│")
          end;
          
          if width.right > 0 && is_inside_scissor (col_end - 1) row (Cell.get scissor_box) then begin
    ...
    | ScissorStart rect -> Cell.set scissor_box (Some rect)
    | ScissorEnd -> Cell.set scissor_box None
    | Custom _ -> ()
  ) commands;
