...
type custom_segment = { row: int; col: int; command_order: int; data: string }
...
let get_cell = fun grid ~row ~col ->
  let line = Array.get_unchecked grid ~at:row in Array.get_unchecked line ~at:col
...
let write_text_cell = fun cell color weight decoration grapheme grapheme_width ->
  ...
  cell.underline <- (
    ...
    | Style.NoDecoration | Style.Strikethrough -> false
  );
  cell.strike <- (
    match decoration with
    | Style.Strikethrough -> true
    | Style.NoDecoration | Style.Underline -> false
  )
...
let cell_to_string = fun cell ->
  if cell.char_width = 0 then
    ...
    if formats = [] then
      cell.char
    else Ansi_formatter.format_string formats cell.char
...
let render_to_string = fun commands ->
  if commands = [] then
    ...
    let max_col = ref 0 in List.for_each commands ~fn:(
      fun command ->
        match command.Render.command_type with
        | Render.ScissorStart _ | Render.ScissorEnd -> ()
        | Render.Custom { data } ->
            let _ = data in
  ...
  if !max_row = 0 || !max_col = 0 then
    ...
    let grid = Array.init ~count:!max_row ~fn:(
      fun _ -> Array.init ~count:!max_col ~fn:(
        fun _ -> make_cell ()
      )
    ) in
    let custom_segments = ref [] in
    let scissor_box = ref None in List.for_each (index_commands commands) ~fn:(
      fun (index, command) ->
        match command.Render.command_type with
        ...
        | Render.Rectangle { color; _ } ->
            ...
            let col_start, col_end = Utils.visible_col_range ~box:command.bounding_box ~scissor:!scissor_box ~limit:!max_col in
            for row = row_start to row_end - 1 do
              if Utils.is_inside_scissor ~col:col_start ~row !scissor_box || (col_end > col_start && Utils.is_inside_scissor ~col:(col_end - 1) ~row !scissor_box) then
                begin
            ...
            let lines = String.split_on_char '\n' content in lines |> List.enumerate |> List.for_each ~fn:(
              fun (line_index, line) ->
                ...
                if row < row_end then
                  let col_start = Utils.rect_col_start command.bounding_box in
                  let col_end = Int.min !max_col (Utils.rect_col_end command.bounding_box) in
                  let visible_col_start, visible_col_end = Utils.visible_col_range ~box:command.bounding_box ~scissor:!scissor_box ~limit:!max_col in
                  let cursor = ref col_start in
                  let graphemes = String.into_grapheme_iter line |> Std.Iter.Iterator.to_list in List.for_each graphemes ~fn:(
                    fun grapheme ->
                      ...
                      if next_col <= visible_col_start then
                        ...
                        if next_col > visible_col_end || next_col > col_end then
                          ()
                        else
                          if !cursor >= visible_col_start then
                            begin
                              if !cursor < !max_col then
                                write_text_cell (get_cell grid ~row ~col:!cursor) tty_color weight decoration grapheme_string grapheme_width;
        ...
        | Render.Custom { data } ->
            ...
            let row_start = Utils.rect_row_start command.bounding_box in
            let row_end = Int.min !max_row (Utils.rect_row_end command.bounding_box) in
            let col_start = Utils.rect_col_start command.bounding_box in lines |> List.enumerate |> List.for_each ~fn:(
              fun (line_index, line) ->
                let row = row_start + line_index in
                if row < row_end then
                  let visible_col_start, visible_col_end = Utils.visible_col_range ~box:command.bounding_box ~scissor:!scissor_box ~limit:!max_col in
  ...
  let rows = List.fold_left !custom_segments ~init:[] ~fn:(
    fun acc segment -> segment :: acc
  ) |> List.sort ~compare:(
    fun left right ->
      let by_row = Int.compare left.row right.row in
      match by_row with
      | Order.LT | Order.GT -> by_row
      | Order.EQ ->
          let by_col = Int.compare left.col right.col in
          (
            match by_col with
            | Order.LT | Order.GT -> by_col
  ...
  let rec render_row_segments = fun buffer row cursor ->
    ...
    | segment :: rest ->
        ...
        let next_cursor = Int.max cursor (segment_col + Tty.Escape_seq.width segment.data) in render_row_segments buffer row next_cursor rest
  in
  let buf = Buffer.create ~size:(!max_row * !max_col * 2) in
  for row = 0 to !max_row - 1 do
    if row > 0 then
      Buffer.add_char buf '\n';
    let row_segments = List.filter rows ~fn:(
      fun segment -> segment.row = row
    ) in
