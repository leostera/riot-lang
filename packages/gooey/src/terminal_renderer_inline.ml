...
let render_to_string = fun commands ->
            ...
            |> List.for_each
              ~fn:(fun (line_index, line) ->
                ...
                if row < row_end then
                  ...
                  let graphemes =
                    String.into_grapheme_iter line
  ...
  for row = 0 to !max_row - 1 do
    if row > 0 then
      Buffer.add_char buf '\n';
    let row_segments = List.filter rows ~fn:(fun segment -> segment.row = row) in
