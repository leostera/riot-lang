...
type clipped_command = { command: Render.command; clip_stack: Geometry.Rect.t list }
...
module Math = struct
  ...
  let list_last_index = fun values ->
    let len = List.length values in Int.max 0 (len - 1)
end
...
module Box_model = struct
  ...
  let horizontal_inset = fun (style: Style.t) ->
    let border = border_thickness style in Float.of_int (style.padding.left + style.padding.right + border + border)

  let vertical_inset = fun (style: Style.t) ->
    let border = border_thickness style in Float.of_int (style.padding.top + style.padding.bottom + border + border)

  let content_origin = fun (rect: Geometry.Rect.t) (style: Style.t) ->
    let border = Float.of_int (border_thickness style) in Geometry.Point.make ~x:(rect.x +. border +. Float.of_int style.padding.left) ~y:(rect.y +. border +. Float.of_int style.padding.top)
  ...
  let content_box = fun (rect: Geometry.Rect.t) (style: Style.t) ->
    ...
    let origin = content_origin rect style in Geometry.Rect.make ~x:origin.x ~y:origin.y ~width:(Math.clamp_non_negative (rect.width -. inset_x)) ~height:(Math.clamp_non_negative (rect.height -. inset_y))
...
let measurement_constraints = fun ?available_width ?available_height () -> Super.Config.constraints ?available_width ?available_height ()
...
let rec build_layout_tree: Element.t -> layout_node = fun element ->
  ...
  let children =
    ...
    | Element.Text _ | Element.Empty | Element.Custom _ -> []
  in
  {
    element;
    style;
...
let resolve_non_fit_size = fun sizing available ->
  ...
  | Style.Percent ratio ->
      Math.map_option available
        (
          fun value -> value *. ratio
        )
...
let child_available_width = fun parent_inner_width (style: Style.t) -> Math.option_subtract parent_inner_width (Box_model.margin_horizontal style.margin)

let child_available_height = fun parent_inner_height (style: Style.t) -> Math.option_subtract parent_inner_height (Box_model.margin_vertical style.margin)
  ...
  let width_hint =
    ...
    | None -> (
      ...
      | None ->
          ...
          | Style.NoWrap -> None
          | Style.Words | Style.Character -> constraints.max_width
    )
  in
...
let rec measure_node: layout_node -> node_constraints -> Super.Config.t -> unit = fun node constraints config ->
  ...
  | Element.Custom { measure; _ } -> measure_intrinsic_node node constraints (measure ~constraints:(measurement_constraints ?available_width:(content_width_constraint constraints node.style) ?available_height:(content_height_constraint constraints node.style) ()))
...
and measure_intrinsic_node: layout_node -> node_constraints -> Viewport.t -> unit = fun node constraints intrinsic ->
  ...
  let width = resolve_axis_size style.sizing.width (intrinsic.width +. Box_model.horizontal_inset style) constraints.max_width constraints.forced_width style.sizing.min_width style.sizing.max_width in
  let height = resolve_axis_size style.sizing.height (intrinsic.height +. Box_model.vertical_inset style) constraints.max_height constraints.forced_height style.sizing.min_height style.sizing.max_height in node.computed_size <- Viewport.make ~width ~height
and measure_text_node: layout_node -> node_constraints -> Super.Config.t -> string -> unit = fun node constraints config content ->
  let style = node.style in
  let measurement = config.Super.Config.text_measurer ~constraints:(measurement_constraints ?available_width:(text_width_constraint constraints style) ?available_height:(content_height_constraint constraints style) ()) content style in
...
and measure_container: layout_node -> node_constraints -> Super.Config.t -> unit = fun node constraints config ->
  ...
  in
  measure_children_for_intrinsic_size node ~parent_inner_width:(Math.option_subtract width_probe (Box_model.horizontal_inset style)) ~parent_inner_height:(Math.option_subtract height_probe (Box_model.vertical_inset style)) config;
  let intrinsic_width, intrinsic_height = container_intrinsic_size node in
  let width = resolve_axis_size style.sizing.width (intrinsic_width +. Box_model.horizontal_inset style) constraints.max_width constraints.forced_width style.sizing.min_width style.sizing.max_width in
  let height = resolve_axis_size style.sizing.height (intrinsic_height +. Box_model.vertical_inset style) constraints.max_height constraints.forced_height style.sizing.min_height style.sizing.max_height in
  node.computed_size <- Viewport.make ~width ~height;
  measure_children_for_final_size node ~parent_inner_width:(Some (Math.clamp_non_negative (width -. Box_model.horizontal_inset style))) ~parent_inner_height:(Some (Math.clamp_non_negative (height -. Box_model.vertical_inset style))) config
and measure_children_for_intrinsic_size: layout_node -> parent_inner_width:float option -> parent_inner_height:float option -> Super.Config.t -> unit = fun node ~parent_inner_width ~parent_inner_height config ->
  let direction = node.style.direction in List.for_each node.children ~fn:(
    fun child ->
      let forced_main =
...
and measure_children_for_final_size: layout_node -> parent_inner_width:float option -> parent_inner_height:float option -> Super.Config.t -> unit = fun node ~parent_inner_width ~parent_inner_height config ->
  ...
  List.for_each node.children ~fn:(
    fun child ->
  ...
  let fixed_outer_main = List.fold_left node.children ~init:0.0 ~fn:(
    fun acc child ->
      if is_main_axis_grow direction child.style then
        acc
      else
  ...
  let grow_margin_main = List.fold_left node.children ~init:0.0 ~fn:(
    fun acc child ->
      if is_main_axis_grow direction child.style then
        acc +. (
          match direction with
  ...
  let total_weight = List.fold_left node.children ~init:0.0 ~fn:(
    fun acc child ->
      if is_main_axis_grow direction child.style then
  ...
  in
  let remaining = Math.clamp_non_negative (total_available -. gap_space -. fixed_outer_main -. grow_margin_main) in List.for_each node.children ~fn:(
    fun child ->
      if is_main_axis_grow direction child.style then
        let share =
          if (
        ...
        in
        measure_child child direction ~parent_inner_width ~parent_inner_height ~forced_main:(Some share) config
  )
and measure_child: layout_node -> Style.direction -> parent_inner_width:float option -> parent_inner_height:float option -> forced_main:float option -> Super.Config.t -> unit = fun child direction ~parent_inner_width ~parent_inner_height ~forced_main config ->
...
and container_intrinsic_size: layout_node -> float * float = fun node ->
  let direction = node.style.direction in
  let total_main, max_cross = List.fold_left node.children ~init:(0.0, 0.0) ~fn:(
    fun (total_main, max_cross) child ->
...
let rec arrange_node: layout_node -> Geometry.Point.t -> unit = fun node origin ->
  node.final_box <- Geometry.Rect.make ~x:origin.x ~y:origin.y ~width:node.computed_size.width ~height:node.computed_size.height;
  match node.element with
...
and arrange_children: layout_node -> unit = fun node ->
  ...
  in
  let total_children_main = List.fold_left node.children ~init:0.0 ~fn:(
    fun acc child ->
      acc +. (
        match style.direction with
        | Style.LeftToRight -> Box_model.outer_width child
  ...
  ) +. Float.of_int (style.child_gap * Math.list_last_index node.children) in
  let main_offset = container_main_offset style.direction style (Math.clamp_non_negative (inner_main -. total_children_main)) in
  let cursor = ref main_offset in List.for_each node.children ~fn:(
    fun child ->
      ...
      in
      let cross_offset = container_cross_offset style.direction style (Math.clamp_non_negative (inner_cross -. child_outer_cross)) in
      let child_origin =
        match style.direction with
        | Style.LeftToRight -> Geometry.Point.make ~x:(content_rect.x +. !cursor +. Float.of_int child.style.margin.left) ~y:(content_rect.y +. cross_offset +. Float.of_int child.style.margin.top)
        | Style.TopToBottom -> Geometry.Point.make ~x:(content_rect.x +. cross_offset +. Float.of_int child.style.margin.left) ~y:(content_rect.y +. !cursor +. Float.of_int child.style.margin.top)
      in
...
let rect_equal = fun (left: Geometry.Rect.t) (right: Geometry.Rect.t) -> Float.compare left.x right.x = Order.EQ && Float.compare left.y right.y = Order.EQ && Float.compare left.width right.width = Order.EQ && Float.compare left.height right.height = Order.EQ

let push_annotated = fun commands ~clip_stack command -> Vector.push commands ~value:{ command; clip_stack }
...
let push_background = fun node commands ~clip_stack ->
  ...
  | Some color when (
    ...
    | Order.GT -> true
    | Order.LT | Order.EQ -> false
  ) -> push_annotated commands ~clip_stack { Render.bounding_box = node.final_box; command_type = Render.Rectangle { color; corner_radius = node.style.corner_radius }; z_index = node.style.z_index }
  | _ -> ()
...
let push_text = fun node commands ~clip_stack content ->
  ...
  if (
    ...
    let measurement =
      ...
      | None -> Super.Config.default_text_measurer ~constraints:(measurement_constraints ~available_width:text_rect.width ~available_height:text_rect.height ()) content style
    in
    ...
    let rec loop line_index = function
      ...
      | line :: rest ->
          ...
          let line_x = text_rect.x +. text_align_offset style (Math.clamp_non_negative (text_rect.width -. line_width)) in
          if line != "" then
            push_annotated commands ~clip_stack
              {
                Render.bounding_box = Geometry.Rect.make ~x:line_x ~y:(text_rect.y +. Float.of_int line_index) ~width:visible_width ~height:1.0;
...
let rec generate_commands: layout_node -> clipped_command Vector.t -> Geometry.Rect.t list -> unit = fun node commands clip_stack ->
  ...
  (
    ...
    | Element.Container _ -> List.for_each node.children ~fn:(
      fun child -> generate_commands child commands child_clip_stack
    )
    | Element.Custom { render; _ } ->
        let custom_commands = render node.final_box in List.for_each custom_commands ~fn:(
          fun command -> push_annotated commands ~clip_stack:child_clip_stack command
        )
    | Element.Empty -> ()
  )
...
let rec common_prefix_length = fun left right ->
  ...
  | left_head :: left_tail, right_head :: right_tail when rect_equal left_head right_head -> 1 + common_prefix_length left_tail right_tail
  | _ -> 0
...
let rec emit_scissor_ends = fun output z_index ->
  ...
  | _ :: rest ->
      ...
      Vector.push output ~value:{ Render.bounding_box = Geometry.Rect.zero; command_type = Render.ScissorEnd; z_index }

let emit_scissor_starts = fun output z_index rects -> List.for_each rects ~fn:(
  fun rect -> Vector.push output ~value:{ Render.bounding_box = rect; command_type = Render.ScissorStart rect; z_index }
)
...
let compute = fun ~config element ->
  ...
  measure_node layout_tree ({ empty_node_constraints with max_width = Some config.Super.Config.viewport.width; max_height = Some config.Super.Config.viewport.height }) config;
  arrange_node layout_tree Geometry.Point.zero;
  let commands = Vector.create () in
  generate_commands layout_tree commands [];
  let command_list = commands |> Vector.iter |> Iter.Iterator.to_list in index_commands command_list |> List.sort ~compare:(
    fun (left_index, left) (right_index, right) ->
      let by_z = Int.compare left.command.Render.z_index right.command.Render.z_index in
      match by_z with
      | Order.LT | Order.GT -> by_z
