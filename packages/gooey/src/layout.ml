...
let rec build_layout_tree (element : Element.t) : layout_node =
...
let clamp_option value min_opt max_opt =
  let v = match min_opt with Some min -> Float.max value min | None -> value in
...
let rec calculate_sizes (node : layout_node) (available : Viewport.t) (config : Super.Config.t) : unit =
  ...
  let intrinsic_size = match node.element with
    | Element.Text { content; _ } ->
        config.text_measurer content style
    | Element.Container _ ->
  ...
  (* Apply sizing rules *)
  let calculated_width = match style.sizing.width with
  ...
  let calculated_height = match style.sizing.height with
...
and calculate_container_fit_size (node : layout_node) (available : Viewport.t) (config : Super.Config.t) : Viewport.t =
  ...
  let total_width, total_height = List.fold_left (fun (w, h) child ->
    match style.direction with
    | Style.LeftToRight ->
        (w +. child.computed_size.width, Float.max h child.computed_size.height)
    | Style.TopToBottom ->
        (Float.max w child.computed_size.width, h +. child.computed_size.height)
  ) (0.0, 0.0) node.children in
  ...
  let height = match style.direction with
...
and layout_children (node : layout_node) (config : Super.Config.t) : unit =
  ...
  let fit_children, grow_children, fixed_children, percent_children =
    List.fold_left (fun (fit, grow, fixed, percent) child ->
  ...
  let fit_space = List.fold_left (fun acc child ->
    acc +. (match style.direction with
      | Style.LeftToRight -> child.computed_size.width
      | Style.TopToBottom -> child.computed_size.height)
  ) 0.0 fit_children in
  
  let fixed_space = List.fold_left (fun acc child ->
    let size = match style.direction with
      | Style.LeftToRight ->
    ...
    in
    acc +. size
  ) 0.0 fixed_children in
  
  let percent_space = List.fold_left (fun acc child ->
    let size = match style.direction with
      | Style.LeftToRight ->
          (match child.style.sizing.width with
  ...
  let remaining_space = Float.max 0.0 
  ...
  List.iter (fun child ->
    match style.direction with
    | Style.LeftToRight ->
        child.computed_size <- Viewport.make ~width:space_per_grow ~height:available_height
    | Style.TopToBottom ->
        child.computed_size <- Viewport.make ~width:available_width ~height:space_per_grow
  ) grow_children

(* Phase 3: Calculate positions *)
let rec calculate_positions (node : layout_node) (origin : Geometry.Point.t) : unit =
  let style = node.style in
