...
let wrap_characters = fun ~width text ->
  if width <= 0 then
    ...
    let rec flush current acc =
      ...
      | [] -> acc
      | _ ->
          let line = current |> List.rev |> List.map ~fn:Std.Unicode.Grapheme.to_string |> String.concat "" in line :: acc
    in
    let rec loop current current_width acc = function
      ...
      | grapheme :: rest ->
          ...
          if current = [] || next_width <= width then
            ...
            let acc = flush current acc in loop [ grapheme ] grapheme_width acc rest
    in
    loop [] 0 [] graphemes
...
let default_text_measurer = fun ~constraints text style ->
  ...
  let lines_rev = List.fold_left paragraphs ~init:[] ~fn:(
    fun acc paragraph ->
      let wrapped = wrap_paragraph style width_hint paragraph in List.fold_left wrapped ~init:acc ~fn:(
  ...
  let lines =
    ...
    | lines -> lines
  in
  let width = List.fold_left lines ~init:0 ~fn:(
    fun acc line -> Int.max acc (String.width line)
  ) |> Float.of_int in
  let height = Int.max 1 (List.length lines) |> Float.of_int in { size = Viewport.make ~width ~height; lines }
