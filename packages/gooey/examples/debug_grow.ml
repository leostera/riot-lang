...
let () =
  ...
  List.iter (fun cmd ->
    ...
    | Render.Rectangle _ -> 
        println ("Rectangle: x=" ^ Float.to_string cmd.bounding_box.x ^ 
          " y=" ^ Float.to_string cmd.bounding_box.y ^ 
          " w=" ^ Float.to_string cmd.bounding_box.width ^ 
          " h=" ^ Float.to_string cmd.bounding_box.height)
    | Render.Text txt -> 
        println ("Text '" ^ txt.content ^ "': x=" ^ Float.to_string cmd.bounding_box.x ^ 
          " y=" ^ Float.to_string cmd.bounding_box.y ^ 
          " w=" ^ Float.to_string cmd.bounding_box.width ^ 
          " h=" ^ Float.to_string cmd.bounding_box.height)
    | _ -> ()
