...
let badge = fun label ->
  Element.text
    ~style:Style.(empty |> padding (Padding.symmetric ~h:1 ~v:0) |> border ~width:1 ())
    label

let make_wide_row = fun count ->
  let children =
    List.init ~count
      ~fn:(fun index -> badge ("item-" ^ Int.to_string index))
  in
  Element.row
    ~style:Style.(
      empty
...
let make_card = fun title body ->
  Element.column
    ...
    [
      Element.text
        ~style:Style.(empty |> bold |> fg (Style.color "#F8FAFC"))
        title;
      Element.text
...
let make_mixed_dashboard = fun () ->
  ...
  let cards =
    List.init ~count:10
      ~fn:(fun index ->
        make_card ("card-" ^ Int.to_string index) body)
  in
  let content =
    Element.column
      ~style:Style.(
    ...
    List.init ~count
      ~fn:(fun index ->
        Element.text
          ...
          ("line "
          ^ Int.to_string index
          ^ ": Hello 世界 👍🏽 cafe\u{301} -- wrapped unicode content for Gooey benchmarks"))
  in
  Element.column
    ~style:Style.(
      empty
...
let bench_layout_wrapped_unicode = fun () ->
  let commands =
    layout ~config:(make_config ~width:80.0 ~height:120.0 ()) (make_wrapped_unicode_document 80)
  in
  ignore (List.length commands)

let bench_layout_custom_widgets = fun () ->
  let commands = layout ~config:(make_config ~width:48.0 ~height:80.0 ()) (make_custom_widget_board 40) in
  ignore (List.length commands)

let dashboard_commands =
...
let benchmarks =
  Bench.[
    ...
    (* Keep the pathological deep-fit tree as a tracked stress case without
       making the default benchmark suite exceed Gooey's validation budget. *)
