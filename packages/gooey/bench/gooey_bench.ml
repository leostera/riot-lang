...
let make_wide_row = fun count ->
  let children = List.init ~count ~fn:(fun index -> badge ("item-" ^ Int.to_string index)) in
  Element.row
    ~style:Style.(empty
    |> width (Fixed 320.0)
    |> child_gap 1
  ...
  Element.column
    ...
    [
      Element.text
        ~style:Style.(empty
        |> bold
        |> fg (Style.color "#F8FAFC"))
        title;
      Element.text
        ~style:Style.(empty
        |> width Grow
        |> text_wrap Words
        |> fg (Style.color "#CBD5E1"))
        body;
    ]
...
let make_mixed_dashboard = fun () ->
  ...
  let inspector =
    Element.column
      ...
      [ badge "filters"; badge "selection"; badge "events"; badge "search"; badge "help"; ]
  in
  Element.row
    ~style:Style.(empty
    |> width (Fixed 120.0)
...
let make_custom_widget_board = fun count ->
  let widget index =
    Element.custom
      ...
      ~render:(fun box ->
        render_custom_bar ("w" ^ Int.to_string index) 8 box)
      ()
  in
  Element.column
    ~style:Style.(empty
...
let bench_layout_custom_widgets = fun () ->
  let commands =
    layout ~config:(make_config ~width:48.0 ~height:80.0 ()) (make_custom_widget_board 40)
  in
  ignore (List.length commands)
