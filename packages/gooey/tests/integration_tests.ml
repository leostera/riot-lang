...
let test_complex_nested_layout () =
  ...
  let text_count = List.fold_left (fun acc cmd ->
...
let test_flexbox_style_layout () =
  ...
  let text_positions = List.filter_map (fun cmd ->
    match cmd.Render.command_type with
...
let test_responsive_percent_sizing () =
  let elem = Element.row [
    Element.container
      ~style:Style.(empty |> width (Percent 0.3) |> bg (`rgb (255, 0, 0)))
      [];
    Element.container
      ~style:Style.(empty |> width (Percent 0.7) |> bg (`rgb (0, 255, 0)))
      [];
  ] in
  let commands = layout ~config:(make_config ()) elem in
  
  let widths = List.filter_map (fun cmd ->
    match cmd.Render.command_type with
...
let test_card_ui_pattern () =
  let card = Element.column
    ...
    [
      Element.container
        ~style:Style.(empty |> height (Fixed 20.0) |> bg (`rgb (220, 220, 220)))
        [];
  ...
  let rect_count = List.fold_left (fun acc cmd ->
    match cmd.Render.command_type with
    | Rectangle _ -> acc + 1
    | _ -> acc
  ) 0 commands in
...
let test_grid_like_layout () =
  ...
  let colors = List.filter_map (fun cmd ->
    match cmd.Render.command_type with
  ...
  else Error "Grid layout should produce 4 rectangles with correct colors"

let test_alignment_with_fixed_sizes () =
  let elem = Element.column [
    Element.container 
  ...
  let boxes = List.filter_map (fun cmd ->
    match cmd.Render.command_type with
    | Rectangle _ -> Some (cmd.bounding_box.x, cmd.bounding_box.y, cmd.bounding_box.width, cmd.bounding_box.height)
    | _ -> None
  ) commands in
  ...
  else Error "Fixed-size elements should be positioned correctly in column"
...
let () =
  Miniriot.run
    ~main:(fun ~args -> Test.Cli.main ~name:"integration" ~tests ~args)
    ~args:Env.args ()
