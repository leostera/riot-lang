...
let test_grid_like_layout_uses_grow_cells = fun _ctx ->
  ...
  | [ (c1, r1); (c2, r2); (c3, r3); (c4, r4) ] when c1 = (`rgb (255, 0, 0)) && c2 = (`rgb (
    0,
    255,
    0
  )) && c3 = (`rgb (0, 0, 255)) && c4 = (`rgb (255, 255, 0)) && approx_eq r1.width 40.0 && approx_eq
    r2.width
    40.0 && approx_eq r3.height 12.0 && approx_eq r4.height 12.0 -> Ok ()
...
let test_column_alignment_can_center_page_content = fun _ctx ->
  let ui =
    Element.column
      ...
      [ Element.text "Clock"; Element.text "Press q" ]
  in
...
let tests =
  Test.[
    ...
    case
      "card pattern has consistent content offsets"
      test_card_pattern_has_consistent_content_offsets;
