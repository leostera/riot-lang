...
let test_text_element_preserves_style = fun _ctx ->
  ...
  && style.text_decoration = Style.Underline && style.foreground = Some (`rgb (255, 0, 0)) -> Ok ()
...
let test_row_sets_direction_only = fun _ctx ->
  ...
  && style.sizing.width = Style.Fixed 12.0
  && style.sizing.height = Style.Fit -> Ok ()
...
let test_column_sets_direction_only = fun _ctx ->
  ...
  && style.sizing.width = Style.Fit
  && style.sizing.height = Style.Fixed 6.0 -> Ok ()
...
let test_spacer_is_weighted_grow = fun _ctx ->
  ...
  | Element.Container { children; style } when children = []
  && style.sizing.width = Style.Grow
  && style.sizing.height = Style.Grow
  && approx_eq style.grow_weight 2.0 -> Ok ()
...
let test_custom_preserves_callbacks = fun _ctx ->
  ...
  | Element.Custom { measure; render; _ } ->
      ...
      if measure ~constraints:(Config.constraints ()) = measured && List.length commands = 1 then
        ...
        Error "Custom callbacks should be retained unchanged"
...
let test_custom_measure_receives_constraints = fun _ctx ->
  ...
  in
  match !seen with
  | Some { Config.available_width = Some width; available_height = Some height } when approx_eq
    width
    8.0
