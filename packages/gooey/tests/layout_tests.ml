...
let test_padding_offsets_first_child = fun _ctx ->
  let ui =
    Element.container
      ~style:Style.(empty
      |> padding (Style.Padding.make ~left:3 ~top:2 ()))
      [ Element.text "Hi" ]
  in
...
let test_spacer_consumes_remaining_width_in_rows = fun _ctx ->
  let ui =
    Element.row
      ~style:Style.(empty
      |> width Grow)
      [ Element.text "Left"; Element.spacer (); Element.text "Right" ]
  in
...
let test_spacer_consumes_remaining_height_in_columns = fun _ctx ->
  let ui =
    Element.column
      ~style:Style.(empty
      |> height Grow)
      [ Element.text "Top"; Element.spacer (); Element.text "Bottom" ]
  in
...
let tests =
  Test.[
    case
      "single text uses terminal cell measurement"
      test_single_text_uses_terminal_cell_measurement;
    case
      "multiline text uses longest line and line count"
      test_multiline_text_uses_longest_line_and_line_count;
    ...
    case
      "child gap applies once between row children"
      test_child_gap_applies_once_between_row_children;
    ...
    case
      "spacer consumes remaining height in columns"
      test_spacer_consumes_remaining_height_in_columns;
    case
      "non-fit parent still measures fit children"
      test_non_fit_parent_still_measures_fit_children;
    case
      "non-fit parent still measures percent children"
      test_non_fit_parent_still_measures_percent_children;
    ...
    case
      "alignment centers children inside extra space"
      test_alignment_centers_children_inside_extra_space;
    case
      "alignment can place children at bottom right"
      test_alignment_can_place_children_at_bottom_right;
    case "border and padding define the content box" test_border_and_padding_define_the_content_box;
    case
      "zero-sized children do not advance siblings"
      test_zero_sized_children_do_not_advance_siblings;
  ]
