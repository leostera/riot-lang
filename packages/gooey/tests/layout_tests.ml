...
let text_commands = fun commands -> List.filter_map commands ~fn:(
  fun command ->
...
let text_contents = fun commands -> List.map (text_commands commands) ~fn:(
  fun (content, _) -> content
)

let rectangle_boxes = fun commands -> List.filter_map commands ~fn:(
  fun command ->
    match command.Render.command_type with
...
let find_text_box = fun commands expected -> text_commands commands |> List.find ~fn:(
  fun (content, _) -> content = expected
) |> Option.map ~fn:(
  fun (_, box) -> box
)
...
let test_single_text_uses_terminal_cell_measurement = fun _ctx ->
  ...
  match commands with
  | [ { Render.command_type = Render.Text { content = "Hello"; _ }; bounding_box; _ } ] when approx_eq bounding_box.width 5.0 && approx_eq bounding_box.height 1.0 -> Ok ()
...
let test_multiline_text_uses_longest_line_and_line_count = fun _ctx ->
  ...
  match boxes with
  | [ ("Hi", first); ("there", second) ] when approx_eq first.width 2.0 && approx_eq second.width 5.0 && approx_eq first.y 0.0 && approx_eq second.y 1.0 -> Ok ()
  | _ -> Error "Multiline text should emit one line command per rendered line"

let test_padding_offsets_first_child = fun _ctx ->
  let ui = Element.container ~style:Style.(empty |> padding (Style.Padding.make ~left:3 ~top:2 ())) [ Element.text "Hi" ] in
...
let test_row_preserves_child_order = fun _ctx ->
  ...
  if texts = [ "A"; "B"; "C" ] then
    Ok ()
  else Error "Rows should preserve child order"
...
let test_column_preserves_child_order = fun _ctx ->
  ...
  if texts = [ "A"; "B" ] then
    Ok ()
  else Error "Columns should preserve child order"
...
let test_fixed_row_positions_children_exactly = fun _ctx ->
  let ui = Element.row [ Element.container ~style:Style.(empty |> width (Fixed 3.0) |> height (Fixed 1.0) |> bg (`rgb (255, 0, 0))) []; Element.container ~style:Style.(empty |> width (Fixed 4.0) |> height (Fixed 1.0) |> bg (`rgb (0, 255, 0))) [] ] in
...
let test_fixed_column_positions_children_exactly = fun _ctx ->
  let ui = Element.column [ Element.container ~style:Style.(empty |> width (Fixed 2.0) |> height (Fixed 2.0) |> bg (`rgb (255, 0, 0))) []; Element.container ~style:Style.(empty |> width (Fixed 2.0) |> height (Fixed 3.0) |> bg (`rgb (0, 255, 0))) [] ] in
  match rectangle_boxes (layout ~config:(make_config ()) ui) with
...
let test_percent_width_uses_parent_inner_width = fun _ctx ->
  let ui = Element.row ~style:Style.(empty |> width Grow) [ Element.container ~style:Style.(empty |> width (Percent 0.25) |> height (Fixed 1.0) |> bg (`rgb (255, 0, 0))) []; Element.container ~style:Style.(empty |> width (Percent 0.75) |> height (Fixed 1.0) |> bg (`rgb (0, 255, 0))) [] ] in
  match rectangle_boxes (layout ~config:(make_config ()) ui) with
...
let test_single_grow_child_gets_remaining_width = fun _ctx ->
  let ui = Element.row ~style:Style.(empty |> width Grow) [ Element.container ~style:Style.(empty |> width (Fixed 10.0) |> height (Fixed 1.0) |> bg (`rgb (255, 0, 0))) []; Element.container ~style:Style.(empty |> width Grow |> height (Fixed 1.0) |> bg (`rgb (0, 255, 0))) [] ] in
  match rectangle_boxes (layout ~config:(make_config ()) ui) with
...
let test_weighted_grow_splits_remaining_space = fun _ctx ->
  let ui = Element.row ~style:Style.(empty |> width Grow) [ Element.container ~style:Style.(empty |> width (Fixed 20.0) |> height (Fixed 1.0) |> bg (`rgb (255, 0, 0))) []; Element.container ~style:Style.(empty |> width Grow |> height (Fixed 1.0) |> grow_weight 1.0 |> bg (`rgb (0, 255, 0))) []; Element.container ~style:Style.(empty |> width Grow |> height (Fixed 1.0) |> grow_weight 2.0 |> bg (`rgb (0, 0, 255))) [] ] in
  match rectangle_boxes (layout ~config:(make_config ()) ui) with
...
let test_spacer_consumes_remaining_width_in_rows = fun _ctx ->
  let ui = Element.row ~style:Style.(empty |> width Grow) [ Element.text "Left"; Element.spacer (); Element.text "Right" ] in
...
let test_spacer_consumes_remaining_height_in_columns = fun _ctx ->
  let ui = Element.column ~style:Style.(empty |> height Grow) [ Element.text "Top"; Element.spacer (); Element.text "Bottom" ] in
...
let test_non_fit_parent_still_measures_fit_children = fun _ctx ->
  let ui = Element.container ~style:Style.(empty |> width (Fixed 20.0) |> height (Fixed 4.0)) [ Element.row ~style:Style.(empty |> bg (`rgb (255, 0, 0))) [ Element.text "Hi"; Element.text "!" ] ] in
  match rectangle_boxes (layout ~config:(make_config ()) ui) with
...
let test_non_fit_parent_still_measures_percent_children = fun _ctx ->
  let ui = Element.container ~style:Style.(empty |> width (Fixed 20.0) |> height (Fixed 4.0)) [ Element.container ~style:Style.(empty |> width (Percent 0.5) |> height (Fixed 2.0) |> bg (`rgb (255, 0, 0))) [ Element.text "Hi" ] ] in
  match rectangle_boxes (layout ~config:(make_config ()) ui) with
...
let test_margins_affect_sibling_spacing = fun _ctx ->
  let ui = Element.row ~style:Style.(empty |> width Grow) [ Element.container ~style:Style.(empty |> width (Fixed 10.0) |> height (Fixed 1.0) |> margin (Style.Margin.make ~right:2 ()) |> bg (`rgb (255, 0, 0))) []; Element.container ~style:Style.(empty |> width (Fixed 5.0) |> height (Fixed 1.0) |> bg (`rgb (0, 255, 0))) [] ] in
  match rectangle_boxes (layout ~config:(make_config ()) ui) with
...
let test_margins_affect_fit_parent_size = fun _ctx ->
  let ui = Element.row ~style:Style.(empty |> bg (`rgb (40, 40, 40))) [ Element.container ~style:Style.(empty |> width (Fixed 5.0) |> height (Fixed 1.0) |> margin (Style.Margin.symmetric ~h:1 ~v:0) |> bg (`rgb (255, 0, 0))) [] ] in
  match rectangle_boxes (layout ~config:(make_config ()) ui) with
...
let test_alignment_centers_children_inside_extra_space = fun _ctx ->
  let ui = Element.column ~style:Style.(empty |> width (Fixed 20.0) |> height (Fixed 10.0) |> align ~x:Center ~y:Middle) [ Element.container ~style:Style.(empty |> width (Fixed 4.0) |> height (Fixed 2.0) |> bg (`rgb (255, 0, 0))) [] ] in
  match rectangle_boxes (layout ~config:(make_config ()) ui) with
...
let test_alignment_can_place_children_at_bottom_right = fun _ctx ->
  let ui = Element.column ~style:Style.(empty |> width (Fixed 20.0) |> height (Fixed 10.0) |> align ~x:Right ~y:Bottom) [ Element.container ~style:Style.(empty |> width (Fixed 4.0) |> height (Fixed 2.0) |> bg (`rgb (255, 0, 0))) [] ] in
  match rectangle_boxes (layout ~config:(make_config ()) ui) with
...
let test_border_and_padding_define_the_content_box = fun _ctx ->
  let ui = Element.container ~style:Style.(empty |> width (Fixed 12.0) |> height (Fixed 5.0) |> border ~width:1 ~color:(`rgb (255, 255, 255)) () |> padding (Style.Padding.all 1)) [ Element.text "Hi" ] in
  match find_text_box (layout ~config:(make_config ()) ui) "Hi" with
...
let test_zero_sized_children_do_not_advance_siblings = fun _ctx ->
  let ui = Element.row [ Element.text "A"; Element.container ~style:Style.(empty |> width (Fixed 0.0) |> height (Fixed 0.0)) []; Element.text "B" ] in
  match text_commands (layout ~config:(make_config ()) ui) with
