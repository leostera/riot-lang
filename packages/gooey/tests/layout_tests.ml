...
let test_layout_with_padding () =
  let elem = Element.container
    ~style:Style.(empty |> padding (Padding.all 10))
    [Element.text "Hi"]
  in
  let commands = layout ~config:(make_config ()) elem in
...
let test_layout_with_child_gap () =
  let elem = Element.row 
    ~style:Style.(empty |> child_gap 5)
    [Element.text "A"; Element.text "B"]
  in
  let commands = layout ~config:(make_config ()) elem in
  
  let positions = List.filter_map (fun cmd ->
    match cmd.Render.command_type with
...
let test_layout_grow_sizing () =
  ...
  if List.length commands = 0 then Ok ()
  else Error "Empty grow container should produce no commands"

let test_layout_fixed_sizing () =
  let elem = Element.container
...
let test_nested_layout () =
  ...
  let texts = List.filter_map (fun cmd ->
    match cmd.Render.command_type with
...
let () =
  Miniriot.run
    ~main:(fun ~args -> Test.Cli.main ~name:"layout" ~tests ~args)
    ~args:Env.args ()
