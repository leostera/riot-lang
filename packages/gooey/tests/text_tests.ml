...
let test_default_text_measurer_multiline = fun _ctx ->
  let measurement = Config.default_text_measurer ~constraints:(Config.constraints ()) "Hi\nthere" Style.empty in
  if approx_eq measurement.size.width 5.0 && approx_eq measurement.size.height 2.0 then
    Ok ()
  else Error "default_text_measurer should use longest visible line and line count"

let test_default_text_measurer_cjk_width = fun _ctx ->
  let measurement = Config.default_text_measurer ~constraints:(Config.constraints ()) "中" Style.empty in
  if approx_eq measurement.size.width 2.0 then
    Ok ()
  else Error "CJK characters should measure to their terminal display width"

let test_default_text_measurer_emoji_width = fun _ctx ->
  let measurement = Config.default_text_measurer ~constraints:(Config.constraints ()) "👍" Style.empty in
  if approx_eq measurement.size.width 2.0 then
    Ok ()
  else Error "Emoji should measure to their terminal display width"

let test_default_text_measurer_combining_sequence = fun _ctx ->
  let measurement = Config.default_text_measurer ~constraints:(Config.constraints ()) "é" Style.empty in
  if approx_eq measurement.size.width 1.0 then
    Ok ()
  else Error "Combining mark sequences should occupy a single display cell"

let test_default_text_measurer_respects_available_width = fun _ctx ->
  let measurement = Config.default_text_measurer ~constraints:(Config.constraints ~available_width:3.0 ()) "aa bb" Style.empty in
  if measurement.lines = [ "aa"; "bb" ] && approx_eq measurement.size.width 2.0 && approx_eq measurement.size.height 2.0 then
    Ok ()
  else Error "default_text_measurer should honor the available width when wrapping"

let test_word_wrap_breaks_text_into_multiple_lines = fun _ctx ->
  let ui = Element.text ~style:Style.(empty |> width (Fixed 4.0) |> text_wrap Words) "aa bb cc" in
  let contents = List.map (text_commands (layout ~config:(make_config ()) ui)) ~fn:(
    fun ({ Render.content; _ }, _) -> content
  ) in
  if contents = [ "aa"; "bb"; "cc" ] then
    Ok ()
  else Error "Word wrapping should break text on word boundaries"

let test_character_wrap_breaks_long_words = fun _ctx ->
  let ui = Element.text ~style:Style.(empty |> width (Fixed 3.0) |> text_wrap Character) "abcdefg" in
  let contents = List.map (text_commands (layout ~config:(make_config ()) ui)) ~fn:(
    fun ({ Render.content; _ }, _) -> content
  ) in
  if contents = [ "abc"; "def"; "g" ] then
    Ok ()
  else Error "Character wrapping should break long text at grapheme boundaries"

let test_no_wrap_preserves_single_line_content = fun _ctx ->
  let ui = Element.text ~style:Style.(empty |> width (Fixed 3.0) |> text_wrap NoWrap) "abcdef" in
...
let test_text_align_center_offsets_text = fun _ctx ->
  let ui = Element.text ~style:Style.(empty |> width (Fixed 10.0) |> text_align TextCenter) "Hi" in
  match text_commands (layout ~config:(make_config ()) ui) with
...
let test_text_align_right_offsets_text = fun _ctx ->
  let ui = Element.text ~style:Style.(empty |> width (Fixed 10.0) |> text_align TextRight) "Hi" in
  match text_commands (layout ~config:(make_config ()) ui) with
...
let test_underline_survives_into_render_commands = fun _ctx ->
  let ui = Element.text ~style:Style.(empty |> underline) "underline" in
  match text_commands (layout ~config:(make_config ()) ui) with
...
let test_strikethrough_survives_into_render_commands = fun _ctx ->
  let ui = Element.text ~style:Style.(empty |> strikethrough) "strike" in
  match text_commands (layout ~config:(make_config ()) ui) with
...
let test_bold_survives_into_render_commands = fun _ctx ->
  let ui = Element.text ~style:Style.(empty |> bold) "bold" in
  match text_commands (layout ~config:(make_config ()) ui) with
...
let test_layout_uses_the_configured_text_measurer = fun _ctx ->
  let text_measurer: Config.text_measurer = fun ~constraints:_ _text _style -> { size = Viewport.make ~width:5.0 ~height:2.0; lines = [ "left"; "right" ] } in
  let config = Config.make ~viewport:(Viewport.make ~width:80.0 ~height:24.0) ~text_measurer () in
  let contents = List.map (text_commands (layout ~config (Element.text "ignored"))) ~fn:(
    fun ({ Render.content; _ }, _) -> content
  ) in
  if contents = [ "left"; "right" ] then
    Ok ()
  else Error "Layout should use the configured text measurer's wrapped lines"

let test_text_size_is_a_terminal_layout_no_op = fun _ctx ->
  let normal = layout ~config:(make_config ()) (Element.text ~style:Style.(empty |> text_size 12) "same") |> text_commands in
  let large = layout ~config:(make_config ()) (Element.text ~style:Style.(empty |> text_size 72) "same") |> text_commands in
  match normal, large with
  | [ (_, normal_box) ], [ (_, large_box) ] when approx_eq normal_box.width large_box.width && approx_eq normal_box.height large_box.height -> Ok ()
...
let tests = Test.[
  ...
  case "default text measurer respects available width" test_default_text_measurer_respects_available_width;
  ...
  case "strikethrough survives into render commands" test_strikethrough_survives_into_render_commands;
