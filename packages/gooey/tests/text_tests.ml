...
let test_default_text_measurer_respects_available_width = fun _ctx ->
  let measurement =
    Config.default_text_measurer
      ...
      Style.empty
  in
  if measurement.lines = [ "aa"; "bb" ]
     && approx_eq measurement.size.width 2.0
     && approx_eq measurement.size.height 2.0 then
    Ok ()
  else
...
let test_word_wrap_breaks_text_into_multiple_lines = fun _ctx ->
  ...
  let contents = List.map (text_commands (layout ~config:(make_config ()) ui)) ~fn:(fun ({ Render.content; _ }, _) -> content) in
...
let test_layout_uses_the_configured_text_measurer = fun _ctx ->
  ...
  in
  let contents =
    List.map (text_commands (layout ~config (Element.text "ignored"))) ~fn:(fun ({ Render.content; _ }, _) -> content)
  in
...
let test_text_size_is_a_terminal_layout_no_op = fun _ctx ->
  let normal =
    layout ~config:(make_config ()) (Element.text ~style:Style.(empty |> text_size 12) "same")
    |> text_commands
  in
  let large =
    layout ~config:(make_config ()) (Element.text ~style:Style.(empty |> text_size 72) "same")
    |> text_commands
  in
  match normal, large with
  | [ (_, normal_box) ], [ (_, large_box) ] when approx_eq normal_box.width large_box.width && approx_eq normal_box.height large_box.height ->
      Ok ()
