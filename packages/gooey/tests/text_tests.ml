...
let test_default_text_measurer_multiline = fun _ctx ->
  let measurement = Config.default_text_measurer
...
let test_layout_uses_the_configured_text_measurer = fun _ctx ->
  let text_measurer: Config.text_measurer = fun ~constraints:_ _text _style ->
    { size = Viewport.make ~width:5.0 ~height:2.0; lines = [ "left"; "right" ] } in
...
let test_text_size_is_a_terminal_layout_no_op = fun _ctx ->
  let normal = layout
    ~config:(make_config ())
    (Element.text ~style:Style.(empty |> text_size 12) "same")
  |> text_commands in
  let large = layout
    ~config:(make_config ())
    (Element.text ~style:Style.(empty |> text_size 72) "same")
  |> text_commands in
