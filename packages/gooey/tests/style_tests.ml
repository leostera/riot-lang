...
let test_empty_style () =
  let s = Style.empty in
  if s.direction = Style.LeftToRight && s.child_gap = 0 && s.z_index = 0 then Ok ()
  else Error "Style.empty should have correct defaults"

let test_padding_helpers () =
...
let test_margin_helpers () =
  (* Verify we can create margin structures *)
  let _m1 = Style.Margin.all 5 in
  let _m2 = Style.Margin.symmetric ~h:15 ~v:8 in
  Ok ()

let test_style_builder () =
  let s = Style.(empty |> bg (`rgb (255, 0, 0)) |> grow) in
  if s.background = Some (`rgb (255, 0, 0)) && s.sizing.width = Style.Grow then Ok ()
  else Error "Style builder should chain operations correctly"
...
let test_border () =
  ...
  if s.border_width = 2 && s.border_color = Some (`rgb (100, 100, 100)) then Ok ()
  else Error "Border styling failed"
...
let () =
  Miniriot.run
    ~main:(fun ~args -> Test.Cli.main ~name:"style" ~tests ~args)
    ~args:Env.args ()
