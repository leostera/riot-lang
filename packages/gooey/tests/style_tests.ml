...
let test_min_max_builders = fun _ctx ->
  let style =
    Style.(empty |> min_width 2.0 |> max_width 8.0 |> min_height 3.0 |> max_height 9.0)
  in
  if style.sizing.min_width = Some 2.0
     && style.sizing.max_width = Some 8.0
     && style.sizing.min_height = Some 3.0
     && style.sizing.max_height = Some 9.0 then
    Ok ()
  else
...
let test_padding_helpers = fun _ctx ->
  ...
  if p1.left = 4 && p1.bottom = 4
     && p2.left = 3 && p2.top = 2
     && p3.left = 1 && p3.right = 2 && p3.top = 3 && p3.bottom = 4 then
    Ok ()
  else
...
let test_grow_and_grow_weight = fun _ctx ->
  ...
  if style.sizing.width = Style.Grow
     && style.sizing.height = Style.Grow
     && approx_eq style.grow_weight 2.5 then
    Ok ()
  else
...
let test_text_style_builders = fun _ctx ->
  let style =
    Style.(empty |> text_wrap Character |> text_align TextRight |> strikethrough |> bold)
  in
  if style.text_wrap = Style.Character
     && style.text_align = Style.TextRight
     && style.text_decoration = Style.Strikethrough
     && style.font_weight = Style.Bold then
    Ok ()
  else
...
let test_border_builder = fun _ctx ->
  ...
  if style.border_width = 3
     && style.border_color = Some (`rgb (10, 20, 30))
     && style.corner_radius = radius then
    Ok ()
  else
    Error "Border builder should preserve width, color, and radius"

let test_color_parsing = fun _ctx ->
  if Style.color "#f00" = `rgb (255, 0, 0)
     && Style.color "#ff0000" = `rgb (255, 0, 0)
     && Style.color "00ff00" = `rgb (0, 255, 0) then
    Ok ()
  else
...
let test_color_parser_integrates_with_style_api = fun _ctx ->
  ...
  if style.foreground = Some red
     && style.background = Some red
     && style.border_color = Some red then
    Ok ()
  else
