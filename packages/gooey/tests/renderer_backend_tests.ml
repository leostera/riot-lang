...
let test_inline_renderer_handles_multiple_custom_segments_on_one_row = fun _ctx ->
  let output =
    Terminal_renderer_inline.render_to_string
      [
        custom_command ~x:0.0 ~y:0.0 ~width:3.0 "AAA";
        custom_command ~x:3.0 ~y:0.0 ~width:3.0 "BBB";
      ]
  in
...
let test_fullscreen_renderer_handles_custom_commands = fun _ctx ->
  let output =
    Terminal_renderer_fullscreen.render_to_string [ custom_command ~x:2.0 ~y:1.0 ~width:4.0 "demo" ]
  in
  if
...
let test_inline_renderer_clips_custom_commands_to_the_bounding_box = fun _ctx ->
  let visible =
    Terminal_renderer_inline.render_to_string [ custom_command ~x:0.0 ~y:0.0 ~width:3.0 "abcdef" ]
    |> Tty.Escape_seq.strip
  in
...
let test_fullscreen_renderer_clips_custom_commands_to_the_bounding_box = fun _ctx ->
  let visible =
    Terminal_renderer_fullscreen.render_to_string
...
let test_inline_renderer_emits_bold_and_underline_sequences = fun _ctx ->
  let output =
    Terminal_renderer_inline.render_to_string
      [
        text_command ~x:0.0 ~y:0.0 ~width:4.0 ~weight:Style.Bold ~decoration:Style.Underline "text";
      ]
  in
  if
    String.contains output Tty.Escape_seq.bold_seq
    && String.contains output Tty.Escape_seq.underline_seq
  then
    Ok ()
  else
...
let test_fullscreen_renderer_emits_strikethrough_sequences = fun _ctx ->
  let output =
    Terminal_renderer_fullscreen.render_to_string
      [ text_command ~x:0.0 ~y:0.0 ~width:6.0 ~decoration:Style.Strikethrough "strike" ]
  in
...
let test_inline_renderer_keeps_unicode_display_width = fun _ctx ->
  let output =
    Terminal_renderer_inline.render_to_string
      [ text_command ~x:0.0 ~y:0.0 ~width:2.0 "中"; text_command ~x:2.0 ~y:0.0 ~width:2.0 "👍" ]
  in
  let visible = Tty.Escape_seq.strip output in
...
let test_inline_renderer_clips_text_to_the_bounding_box = fun _ctx ->
  let visible =
    Terminal_renderer_inline.render_to_string [ text_command ~x:0.0 ~y:0.0 ~width:3.0 "abcdef" ]
    |> Tty.Escape_seq.strip
  in
  if String.contains visible "abc" && not (String.contains visible "abcd") then
    Ok ()
...
let test_fullscreen_renderer_clips_text_to_the_bounding_box = fun _ctx ->
  let visible =
    Terminal_renderer_fullscreen.render_to_string [ text_command ~x:0.0 ~y:0.0 ~width:3.0 "abcdef" ]
    |> Tty.Escape_seq.strip
  in
  if String.contains visible "abc" && not (String.contains visible "abcd") then
    Ok ()
...
let test_terminal_renderers_treat_text_size_as_a_render_no_op = fun _ctx ->
  ...
  let large_inline =
    ...
    |> Tty.Escape_seq.strip
  in
  let small_fullscreen =
    Terminal_renderer_fullscreen.render_to_string
      [ text_command ~x:0.0 ~y:0.0 ~width:4.0 ~size:12 "text" ]
    |> Tty.Escape_seq.strip
  in
  let large_fullscreen =
    Terminal_renderer_fullscreen.render_to_string
      [ text_command ~x:0.0 ~y:0.0 ~width:4.0 ~size:72 "text" ]
    |> Tty.Escape_seq.strip
  in
...
let tests =
  Test.[
    case
      "inline renderer handles multiple custom segments on one row"
      test_inline_renderer_handles_multiple_custom_segments_on_one_row;
    case
      "fullscreen renderer handles custom commands"
      test_fullscreen_renderer_handles_custom_commands;
    case
      "inline renderer clips custom commands to the bounding box"
      test_inline_renderer_clips_custom_commands_to_the_bounding_box;
    case
      "fullscreen renderer clips custom commands to the bounding box"
      test_fullscreen_renderer_clips_custom_commands_to_the_bounding_box;
    case
      "inline renderer emits bold and underline sequences"
      test_inline_renderer_emits_bold_and_underline_sequences;
    case
      "fullscreen renderer emits strikethrough sequences"
      test_fullscreen_renderer_emits_strikethrough_sequences;
    case
      "inline renderer keeps unicode display width"
      test_inline_renderer_keeps_unicode_display_width;
    case
      "inline renderer clips text to the bounding box"
      test_inline_renderer_clips_text_to_the_bounding_box;
    case
      "fullscreen renderer clips text to the bounding box"
      test_fullscreen_renderer_clips_text_to_the_bounding_box;
    case "inline renderer respects scissor for text" test_inline_renderer_respects_scissor_for_text;
    case
      "fullscreen renderer respects scissor for text"
      test_fullscreen_renderer_respects_scissor_for_text;
    case
      "inline renderer respects scissor for custom"
      test_inline_renderer_respects_scissor_for_custom;
    case
      "fullscreen renderer respects scissor for custom"
      test_fullscreen_renderer_respects_scissor_for_custom;
    case
      "terminal renderers treat text size as a render no op"
      test_terminal_renderers_treat_text_size_as_a_render_no_op;
  ]
