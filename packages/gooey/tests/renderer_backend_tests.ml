...
let text_command = fun ?(z = 0) ?(size = 12) ?(weight = Style.Normal) ?(decoration = Style.NoDecoration) ?(color = `rgb (255, 255, 255)) ~x ~y ~width content ->
  {
    ...
    command_type = Render.Text { content; color; size; weight; decoration };
    z_index = z;
  }
...
let test_fullscreen_renderer_handles_custom_commands = fun _ctx ->
  let output =
    Terminal_renderer_fullscreen.render_to_string [
      custom_command ~x:2.0 ~y:1.0 ~width:4.0 "demo";
    ]
  in
  if String.contains output "demo"
     && String.contains output (Tty.Escape_seq.cursor_position_seq 2 3) then
    Ok ()
  else
...
let test_inline_renderer_emits_bold_and_underline_sequences = fun _ctx ->
  ...
  in
  if String.contains output Tty.Escape_seq.bold_seq
     && String.contains output Tty.Escape_seq.underline_seq then
    Ok ()
  else
...
let test_inline_renderer_keeps_unicode_display_width = fun _ctx ->
  let output =
    Terminal_renderer_inline.render_to_string [
      text_command ~x:0.0 ~y:0.0 ~width:2.0 "中";
      text_command ~x:2.0 ~y:0.0 ~width:2.0 "👍";
    ]
  in
  let visible = Tty.Escape_seq.strip output in
...
let test_terminal_renderers_treat_text_size_as_a_render_no_op = fun _ctx ->
  let small_inline =
    Terminal_renderer_inline.render_to_string [ text_command ~x:0.0 ~y:0.0 ~width:4.0 ~size:12 "text" ]
    |> Tty.Escape_seq.strip
  in
  let large_inline =
    Terminal_renderer_inline.render_to_string [ text_command ~x:0.0 ~y:0.0 ~width:4.0 ~size:72 "text" ]
    |> Tty.Escape_seq.strip
  in
  let small_fullscreen =
    Terminal_renderer_fullscreen.render_to_string [ text_command ~x:0.0 ~y:0.0 ~width:4.0 ~size:12 "text" ]
    |> Tty.Escape_seq.strip
  in
  let large_fullscreen =
    Terminal_renderer_fullscreen.render_to_string [ text_command ~x:0.0 ~y:0.0 ~width:4.0 ~size:72 "text" ]
    |> Tty.Escape_seq.strip
  in
...
let () =
  Actors.run ~main:(fun ~args -> Test.Cli.main ~name:"renderer_backends" ~tests ~args) ~args:Env.args ()
