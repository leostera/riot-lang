...
let test_background_requires_a_real_box = fun _ctx ->
  let ui = Element.container
    ~style:Style.(empty |> width (Fixed 6.0) |> height (Fixed 2.0) |> bg (`rgb (100, 150, 200)))
    [] in
...
let test_empty_visual_container_emits = fun _ctx ->
  let ui = Element.container
    ~style:Style.(empty |> width (Fixed 3.0) |> height (Fixed 1.0) |> bg (`rgb (1, 2, 3)))
    [] in
