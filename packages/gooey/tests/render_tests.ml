...
let test_background_requires_a_real_box = fun _ctx ->
  ...
  | [ ({ Render.color = `rgb (100, 150, 200); _ }, box, _) ] when approx_eq box.width 6.0
  && approx_eq box.height 2.0 -> Ok ()
...
let test_border_width_is_clamped_to_terminal_thickness = fun _ctx ->
  let ui =
    Element.container
      ...
      |> width (Fixed 6.0)
      |> height (Fixed 3.0)
      |> border ~width:3 ~color:(`rgb (50, 50, 50)) ())
      []
  in
...
let test_background_and_border_both_emit = fun _ctx ->
  let ui =
    Element.container
      ...
      |> height (Fixed 2.0)
      |> bg (`rgb (255, 255, 255))
      |> border ~width:1 ~color:(`rgb (0, 0, 0)) ())
      []
  in
...
let test_equal_z_index_preserves_tree_order = fun _ctx ->
  ...
  let contents =
    List.map
      (find_text_commands (layout ~config:(make_config ()) ui))
      ~fn:(fun ({ Render.content; _ }, _, _) -> content)
  in
...
let test_z_index_sorting_still_applies = fun _ctx ->
  ...
  in
  let z_indices =
    List.map (layout ~config:(make_config ()) ui) ~fn:(fun command -> command.Render.z_index)
  in
...
let test_parent_background_and_child_text_both_appear = fun _ctx ->
  ...
  let commands = layout ~config:(make_config ()) ui in
  if
    List.length (find_rectangles commands) = 1 && List.length (find_text_commands commands) = 1
  then
    Ok ()
  else
...
let test_multiline_text_emits_one_text_command_per_line = fun _ctx ->
  ...
  let contents =
    List.map (find_text_commands commands) ~fn:(fun ({ Render.content; _ }, _, _) -> content)
  in
...
let test_custom_commands_pass_through = fun _ctx ->
  let ui =
    Element.custom
      ...
      ~render:(fun box -> [
        { Render.bounding_box = box; command_type = Render.Custom { data = "demo" }; z_index = 3 };
      ])
      ()
  in
...
let tests =
  Test.[
    ...
    case
      "border width is clamped to terminal thickness"
      test_border_width_is_clamped_to_terminal_thickness;
    ...
    case
      "parent background and child text both appear"
      test_parent_background_and_child_text_both_appear;
    case
      "multiline text emits one text command per line"
      test_multiline_text_emits_one_text_command_per_line;
