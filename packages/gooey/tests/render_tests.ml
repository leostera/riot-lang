...
let test_render_background () =
  let elem = Element.container
    ~style:Style.(empty |> bg (`rgb (100, 150, 200)))
    []
  in
  let commands = layout ~config:(make_config ()) elem in
...
let test_render_border () =
  ...
  let has_border = List.exists (fun cmd ->
    match cmd.Render.command_type with
    | Border { color = `rgb (50, 50, 50); _ } -> true
    | _ -> false
  ) commands in
  
  if has_border then Ok ()
  else Error "Expected Border command with rgb(50, 50, 50) color"

let test_render_background_and_border () =
  let elem = Element.container
    ~style:Style.(
      empty 
      |> bg (`rgb (255, 255, 255))
      |> border ~width:2 ~color:(`rgb (0, 0, 0)) ()
    )
    []
  in
  let commands = layout ~config:(make_config ()) elem in
  
  let has_rect = List.exists (fun cmd ->
    match cmd.Render.command_type with
  ...
  if has_rect && has_border then Ok ()
  else if not has_rect then Error "Missing background rectangle"
  else Error "Missing border"

let test_render_z_index_sorting () =
  let elem = Element.container [
...
let test_render_corner_radius () =
  ...
  let has_rect = List.exists (fun cmd ->
    match cmd.Render.command_type with
    | Rectangle { color = `rgb (255, 0, 0); corner_radius; _ } ->
        corner_radius.top_left = 8.0 
  ...
  if has_rect then Ok ()
  else Error "Expected Rectangle with corner radius"

let test_render_command_bounding_boxes () =
  ...
  let all_valid = List.for_all (fun (cmd : Render.command) ->
    cmd.bounding_box.width > 0.0 && cmd.bounding_box.height > 0.0
  ) commands in
  
  if all_valid then Ok ()
  else Error "Some commands have invalid bounding boxes"
...
let () =
  Miniriot.run
    ~main:(fun ~args -> Test.Cli.main ~name:"render" ~tests ~args)
    ~args:Env.args ()
