# Timer Wheel Implementation for Actors

This document describes the hierarchical timing wheel implementation added to actors.
...
let timer_id = Actors.Timer.send_after 
  target_pid 
  MyMessage 
  ~after:1.5  (* seconds *)
in
...
let timer_id = Actors.Timer.send_interval
  target_pid
  Tick
...
let sleep duration =
  try
    ...
    ()
  with
  | Actors.Receive_timeout -> ()  (* Expected *)
```
...
```ocaml
let () =
  let config = Actors.Config.make 
    ~timer_resolution:Actors.Config.Millisecond
    ()
  in
