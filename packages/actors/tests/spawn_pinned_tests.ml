...
let test_spawn_pinned_stays_on_requested_scheduler = fun () ->
  ...
  let pinned =
    spawn_pinned ~scheduler:expected_scheduler (fun () -> pinned_actor ~parent ~expected_scheduler)
  in
...
let () =
  let tests = [ Test.case "spawn_pinned stays on the requested scheduler" test_case; ] in
