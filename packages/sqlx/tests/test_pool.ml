...
module MockDriver : Driver.Intf = struct
  ...
  let name = "Mock"
  let connect () = Ok { id = Random.int 1000 }
  let close _ = ()
  let ping _ = true
  let prepare conn sql = Ok { sql }
  let execute _ _ = Ok { data = [] }
  let fetch_row rs = match rs.data with [] -> None | h :: _ -> Some h
  let rows_affected _ = 0
  let begin_transaction _ = Ok ()
  let commit _ = Ok ()
  let rollback _ = Ok ()
  let set_isolation_level _ _ = Ok ()
end
