...
module RowIterator = struct
  ...
  let size state =
    if state.exhausted then 0
    else
      let (Cursor cursor) = state.cursor in
      let module D = (val cursor.driver) in
      D.rows_affected cursor.result_set

  let clone state = { cursor = state.cursor; exhausted = state.exhausted }
end

let to_mut_iter cursor =
  MutIterator.make
    (module RowIterator)
...
let fetch_all cursor = cursor |> to_mut_iter |> MutIterator.to_list
