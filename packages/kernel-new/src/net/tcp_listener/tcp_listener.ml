...
let bind = fun ?(reuse_addr = true) ?(reuse_port = false) ?(backlog = 128) addr ->
  ...
  Result.map_error (fun code -> System (System_error.of_code code)) (FFI.bind ip port reuse_addr reuse_port backlog)
...
let local_addr = fun listener ->
  Result.map_error (fun code -> System (System_error.of_code code)) (Result.map socket_addr_of_pair (FFI.local_addr listener))
