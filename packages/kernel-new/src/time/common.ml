...
let nanos_per_second = Int64.of_int 1_000_000_000
...
let diff_ns = fun ~left_secs ~left_nanos ~right_secs ~right_nanos ->
  let secs = Int64.mul (Int64.of_int (left_secs - right_secs)) nanos_per_second in
  Int64.add secs (Int64.of_int (left_nanos - right_nanos))
