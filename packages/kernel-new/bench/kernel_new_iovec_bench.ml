...
let build_iovec = fun segments segment_size ->
  Kernel.Array.init segments (fun _ -> Kernel.String.make segment_size 'x')
  |> Kernel.IO.Iovec.of_string_array

let bench_into_string = fun segments segment_size () ->
  let _ = build_iovec segments segment_size |> Kernel.IO.Iovec.into_string in
  ()
...
let benchmarks =
  Bench.[
    with_config
      ...
      (bench_into_string 32 1024);
    with_config
      ~config:{ iterations = 20; warmup = 5 }
      "iovec into_string: 128 x 1KiB"
      (bench_into_string 128 1024);
    with_config
      ~config:{ iterations = 20; warmup = 5 }
      "iovec sub: 32 x 1KiB"
      (bench_sub 32 1024);
...
let () =
  Actors.run
    ~main:(fun ~args ->
      Bench.Cli.main ~name:"kernel_new_iovec_bench" ~benchmarks ~args)
    ~args:Env.args
    ()
