...
let find_token_by_text red_root text =
  Syn.Ceibo.Red.SyntaxNode.tokens red_root
  |> List.find_opt (fun token ->
         String.equal (Syn.Ceibo.Red.SyntaxToken.text token) text)

let warning_diagnostic ~rule_id ~message ~token ~fix =
  Tusk_fix.Diagnostic.make ~severity:Warning
    ~kind:(Tusk_fix.Diagnostic.Known { rule_id; message })
    ~span:(Syn.Ceibo.Red.SyntaxToken.span token)
    ~fix ()

let replace_token_rule ~rule_id ~needle ~replacement =
  ...
  Tusk_fix.Rule.make ~id:rule_id ~description:message ~explain:message
    ~run:(fun _ctx red_root ->
      ...
      | Some token ->
          let fix =
            Tusk_fix.Fix.make ~title:message
              ~operations:
                [
                  Tusk_fix.Fix.replace_token_with_text ~target:token
                    ~text:replacement;
                ]
          in
...
let overlapping_replace_rule ~rule_id ~needle ~replacement ~overlap_text =
  ...
  Tusk_fix.Rule.make ~id:rule_id ~description:message ~explain:message
    ~run:(fun _ctx red_root ->
      ...
      | Some token ->
          ...
          let overlap_span =
            Syn.Ceibo.Span.make ~start:(span.start + 1) ~end_:span.end_
          in
          let overlap_token =
            synthetic_token ~span:overlap_span ~text:overlap_text
          in
          let fix =
            Tusk_fix.Fix.make ~title:message
              ~operations:
                [
                  Tusk_fix.Fix.replace_token_with_text ~target:overlap_token
                    ~text:replacement;
                ]
          in
...
let tests =
  [
    ...
    Test.case "reject overlapping operations" (fun () ->
        ...
        Ok ());
    Test.case "source runner skips linting when parsing fails" (fun () ->
        let source = "let =" in
        let result =
          Tusk_fix.Source_runner.run_rule
            ~rule:(replace_token_rule ~rule_id:"test:broken-source"
                     ~needle:"let" ~replacement:"and")
            source
        in
    ...
    Test.case "rule-test applies snake-case type fixes and reruns cleanly"
      (fun () ->
        ...
        let result =
          Tusk_fix.Rule_test.run_rule
            ~rule:(Tusk_fix.Rules.Snake_case_type_names.make ())
            source
          |> Result.expect ~msg:"expected snake-case type rule to apply"
        in
        Test.assert_equal ~expected:1
          ~actual:(List.length result.applied_fixes);
        Test.assert_equal
        ...
        Ok ());
    Test.case "rule-test applies multiple non-overlapping fixes" (fun () ->
        let source = "type userProfile = int\nlet old_value = userProfile\n" in
        let result =
          Tusk_fix.Rule_test.run
            ~rules:
              [
                Tusk_fix.Rules.Snake_case_type_names.make ();
                replace_token_rule ~rule_id:"test:rename-binding"
                  ~needle:"old_value" ~replacement:"current_value";
              ]
            source
          |> Result.expect ~msg:"expected multiple safe fixes to apply"
        in
        Test.assert_equal ~expected:2
          ~actual:(List.length result.initial.diagnostics);
        Test.assert_equal ~expected:2
          ~actual:(List.length result.applied_fixes);
        Test.assert_equal
          ~expected:
            (Some "type user_profile = int\nlet current_value = userProfile\n")
          ~actual:result.fixed_source;
        let remaining =
          match result.after with
          | Some after -> after.diagnostics
      ...
      (fun () ->
        ...
        let actual =
          Tusk_fix.Rule_test.run
            ~rules:
              [
                replace_token_rule ~rule_id:"test:replace-whole-token"
                  ~needle:"old_value" ~replacement:"value";
                overlapping_replace_rule ~rule_id:"test:replace-tail"
                  ~needle:"old_value" ~replacement:"tail"
