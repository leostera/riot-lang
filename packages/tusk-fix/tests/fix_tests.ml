...
let synthetic_token = fun ~span ~text ->
  let green =
    Syn.Ceibo.Green.make_token ~leading_trivia:[]
      ~kind:Syn.SyntaxKind.WHITESPACE ~text ~width:(String.length text)
  in
...
let warning_diagnostic = fun ~rule_id ~message ~token ~fix -> Tusk_fix.Diagnostic.make ~severity:Warning ~kind:(Tusk_fix.Diagnostic.Known {
  rule_id;
  message
}) ~span:(Syn.Ceibo.Red.SyntaxToken.span token) ~fix ()
  ...
  Tusk_fix.Rule.make ~id:rule_id ~description:message ~explain:message
    ~run:(fun _ctx red_root ->
      ...
      | None ->
          []
      | Some token ->
          let fix = Tusk_fix.Fix.make ~title:message ~operations:[
            Tusk_fix.Fix.replace_token_with_text ~target:token ~text:replacement
          ] in
...
let tests = [ Test.case "apply single operation"
    (fun () ->
      ...
      let actual = Tusk_fix.Fix.apply_operation ~source (Tusk_fix.Fix.replace_token_with_text ~target:token ~text:"Std")
    ...
    (fun () ->
      ...
      let fixes = [
        Tusk_fix.Fix.make ~title:"replace Queue" ~operations:[
        ...
        Tusk_fix.Fix.make ~title:"replace Stdlib" ~operations:[
          Tusk_fix.Fix.replace_token_with_text ~target:stdlib_token ~text:"Std"
        ]
      ] in
    ...
    (fun () ->
      ...
      let fix = Tusk_fix.Fix.make ~title:"bad overlap" ~operations:[
    ...
    (fun () ->
      let source = "let =" in
      let result = Tusk_fix.Source_runner.run_rule ~rule:(replace_token_rule
      ~rule_id:"test:broken-source"
      ~needle:"let"
      ~replacement:"and") source in
    ...
    (fun () ->
      ...
      let result = Tusk_fix.Rule_test.run ~rules:[
    ...
    (fun () ->
      ...
      let actual = Tusk_fix.Rule_test.run ~rules:[
        replace_token_rule ~rule_id:"test:replace-whole-token" ~needle:"old_value" ~replacement:"value";
        overlapping_replace_rule ~rule_id:"test:replace-tail" ~needle:"old_value" ~replacement:"tail" ~overlap_text:"ld_value"
      ] source in
      Test.assert_error actual;
      Ok ()) ]

let () = Miniriot.run ~main:(fun ~args:_ -> Test.Cli.main ~name:"tusk-fix:fix" ~tests ~args:Env.args) ~args:Env.args ()
