...
let generated_provider = fun plan provider ->
  let module_name = generated_module_name provider in
  {
    ...
    module_name;
    copied_source_path =
      Path.(plan.providers_dir / Path.v (module_name ^ ".ml"));
...
let plan = fun ~workspace_root ~target_dir_root providers ->
  ...
  let generated_dir =
    Path.(target_dir_root / Path.v "tusk-fix" / Path.v "fixme-runner" / Path.v hash) in
  ...
  let plan = {
    ...
    workspace_root;
    workspace_toml_path =
      Path.(workspace_root / Path.v "tusk.toml");
    toolchain_toml_path =
      Path.(workspace_root / Path.v "ocaml-toolchain.toml");
    package_dir;
    package_toml_path =
      Path.(package_dir / Path.v "tusk.toml");
    src_dir;
    providers_dir;
    library_path =
...
let dependency_entries = fun workspace_root providers ->
  let workspace_packages =
    ...
    | Ok (workspace, _errors) -> Tusk_model.Workspace.(workspace.packages)
    | Error _ -> []
  in
    ...
    |> List.concat_map
      (fun ({ provider; _ }: generated_provider) ->
        workspace_packages |> List.find_opt
          ...
          (fun (pkg: Tusk_model.Package.t) -> (pkg.path, pkg.build_dependencies)) |> Option.unwrap_or
...
let main_source = String.concat
  "\n"
  [ "open Std"; ""; "let () ="; "  Miniriot.run ~main:Fixme_runner.main ~args:Env.args ()"; ""; ]
