...
let rule_description =
  "Source filenames and subdirectories should use snake_case"
...
let make_diagnostic name =
  Diagnostic.make ~severity:Warning
    ~kind:(Diagnostic.Known { rule_id; message = rule_description })
    ~span:(Syn.Ceibo.Span.make ~start:0 ~end_:0)
    ~suggestion:
      ("Rename `" ^ name ^ "` to use lowercase letters, digits, and underscores only.")
    ()
