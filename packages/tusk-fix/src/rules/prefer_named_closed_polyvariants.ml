...
let rule_explain =
  {|
Prefer named closed polymorphic variants over inline ones.

Examples:
  Avoid:   val decode : [ `json | `xml ] -> payload
  Better:  type format = [ `json | `xml ]
           val decode : format -> payload

  Avoid:   type formats = [ `json | `xml ] list
  Better:  type format = [ `json | `xml ]
