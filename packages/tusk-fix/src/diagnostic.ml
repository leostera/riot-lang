...
let colored_header_label severity rule_id =
  "[" ^ severity_to_colored_string severity ^ "] " ^ rule_id

let explain_hint severity rule_id =
  "  For more information about this "
  ^ severity_to_string severity
  ^ ", try `tusk fix --explain " ^ rule_id ^ "`"
...
let to_formatted_output ~file ~source diag =
  ...
  let basic_info =
    [ header_label diag.severity (rule_id diag); ""; message diag ]
  in
  ...
  let lines_with_explain =
    lines_with_suggestion @ [ ""; explain_hint diag.severity (rule_id diag) ]
  in
  header ^ "\n" ^ String.concat "\n" lines_with_explain ^ "\n\n"
...
let group_diagnostics (diags : t list) : grouped list =
  ...
  |> Iter.Iterator.map ~fn:(fun
       ((severity, message, rule_id, suggestion, _fix_title), spans) ->
