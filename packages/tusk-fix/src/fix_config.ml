          ...
          | Some (Data.Toml.Table fix_items) ->
              let ignore_patterns =
                ...
                | Some (Data.Toml.Array items) ->
                    List.filter_map Data.Toml.get_string items
                | _ -> []
              in
              let rules =
                match List.assoc_opt "rules" fix_items with
                | Some (Data.Toml.Array items) ->
                    List.filter_map parse_rule_override items
                | _ -> []
              in
...
let load_scope ~cwd =
  ...
  | Ok (workspace, _load_errors) ->
      let workspace_toml = Path.(workspace.root / Path.v "tusk.toml") in
      ...
      let packages =
        ...
        |> List.map (fun (pkg : Package.t) ->
               let package_toml = Path.(pkg.path / Path.v "tusk.toml") in
               {
                 ...
                 config = load_fix_config package_toml;
               })
      in
      Some {
...
let rec glob_match pattern text =
  if String.equal pattern "" then
    ...
    || (String.length text > 0
       && glob_match pattern (String.sub text 1 (String.length text - 1)))
  else
    String.length text > 0
...
let matches_pattern pattern candidate =
...
let matches_ignore_patterns file patterns =
...
let should_ignore_file scope file =
  ...
  | Some scope ->
      let workspace_ignored =
        matches_ignore_patterns file scope.workspace_config.ignore_patterns
      in
      if workspace_ignored then
        ...
        | Some package_scope ->
            matches_ignore_patterns file package_scope.config.ignore_patterns
        | None -> false

let set_rule_state states name enabled =
  (name, enabled)
...
let effective_rule_states scope file =
  match scope with
  ...
  | Some scope ->
      ...
      match find_package_scope scope file with
      | Some package_scope ->
          apply_rule_overrides base_states package_scope.config.rules
      | None -> base_states

let pipeline_for_file scope file =
