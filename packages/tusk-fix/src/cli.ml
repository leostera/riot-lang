        ...
        | rule :: rest ->
            ...
            let acc =
              ...
              | "riot", Some "riot", Some category_name, Some current when not
                (String.equal category_name current) -> rule_line
              :: ("  " ^ category_name ^ ":")
              :: acc
...
let list_diagnostics_text = fun entries ->
    ...
    |> List.map
      (fun entry ->
        bold (display_rule_id_text entry.Explanation.rule_id) ^ " - " ^ entry.Explanation.message)
    |> String.concat "\n"
...
let run_result_with = fun ~on_result ~mode ~scope ~limit ~files ->
    ...
    let _coordinator = Coordinator.start
      {input = Coordinator.Files files; concurrency; limit; mode; scope; owner} in
    let rec loop results_rev diagnostics_seen limit_reached =
      ...
      | `AllComplete _summary ->
          ...
          {result = Runner.{files; summary}; limit_reached; }
    in
    loop [] 0 false
...
let run_with_coordinator = fun ~format ~mode ~scope ~limit ~roots () ->
    ...
    if format = Reporter.Text then
      ...
      let outcome =
        let owner = self () in
        let _coordinator = Coordinator.start
          {input = Coordinator.Roots roots; concurrency; limit; mode; scope; owner; } in
        let rec loop results_rev diagnostics_seen limit_reached =
          let selector = function
            | Messages.FileResult result -> `select (`FileResult result)
            | Messages.AllComplete summary -> `select (`AllComplete summary)
