...
type parse_phase =
  | ReadingHeader of {
      buffer : IO.Buffer.t;  (** Accumulating 5-byte header *)
      bytes_read : int;
    }
  | ReadingPayload of {
      compressed : bool;
...
type parse_result =
  | Message of LocalMessage.t
  | Need_more
  | Error of parse_error

let create ?(max_message_size = 4 * 1024 * 1024) () =
  {
    max_message_size;
    phase =
      Sync.Cell.create (ReadingHeader { buffer = IO.Buffer.create 5; bytes_read = 0 });
  }

let reset state =
  Sync.Cell.set state.phase (ReadingHeader { buffer = IO.Buffer.create 5; bytes_read = 0 })

let buffered_bytes state =
  match Sync.Cell.get state.phase with
...
let read_n_bytes reader buffer n =
  let bytes = IO.Bytes.create n in
  match IO.Reader.read reader bytes with
  | Ok bytes_read when bytes_read > 0 ->
      IO.Buffer.add_subbytes buffer bytes 0 bytes_read;
      bytes_read
  | Ok _ -> 0
  | Error _ -> 0

let rec parse state reader =
  ...
  | ReadingHeader { buffer; bytes_read } ->
      ...
      if actual_read = 0 && bytes_read = 0 then Need_more
        ...
        if length > state.max_message_size then (
          ...
          (* Zero-length message *)
          reset state;
          Message ({ compressed; payload = IO.Bytes.empty } : LocalMessage.t))
        else (
            ...
            (ReadingPayload
               {
                 compressed;
                 length;
                 buffer = IO.Buffer.create length;
                 bytes_read = 0;
               });
  ...
  | ReadingPayload { compressed; length; buffer; bytes_read } ->
      ...
      if actual_read = 0 && bytes_read = 0 then Need_more
        ...
        let payload = IO.Buffer.to_bytes buffer in
        reset state;
        Message ({ compressed; payload } : LocalMessage.t)
