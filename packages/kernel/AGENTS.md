...
11. Keep `Kernel.IO.IoSlice`, `Iovec`, `Buffer`, and `StringView` checked-by-default. Range-sensitive operations should return `Result.t`; reserve `_unchecked` helpers for hot paths that have already established bounds once.
