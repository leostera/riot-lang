...
let test_basic_watching () =
  (* Create a watcher *)
  let watcher = Fs.Events.create () 
                |> Result.expect ~msg:"Failed to create watcher" in
  
  (* Watch /tmp directory *)
  let _ = Fs.Events.watch watcher ~path:"/tmp" ~latency:0.1
          |> Result.expect ~msg:"Failed to watch /tmp" in
