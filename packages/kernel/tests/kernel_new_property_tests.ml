...
let array_to_list = fun values ->
  let rec loop index acc =
    if index < 0 then
      acc
    else loop (index - 1) (Kernel.Array.get_unchecked values ~at:index :: acc)
  in
      ...
      protect ~finally:(
        ...
        (
          fun () ->
            ...
            | Kernel.Result.Error error -> fail (Kernel.Error.to_string (Kernel.Error.from_async error))
            | Kernel.Result.Ok events -> List.any events ~fn:(
...
let connect_stream = fun poll addr ->
  ...
  | Kernel.Result.Error error -> fail (Kernel.Error.to_string (Kernel.Error.from_net_tcp_stream error))
          ...
          if wait_writable poll ~token (Kernel.Net.TcpStream.to_source stream) then
            ...
            | Kernel.Result.Error error ->
                if is_tcp_stream_would_block error then
                  finish (attempts - 1)
                else fail (Kernel.Error.to_string (Kernel.Error.from_net_tcp_stream error))
          else fail "expected property tcp connect to become writable"
      in
      finish 8
    ...
    | Kernel.Result.Error error ->
        if is_tcp_listener_would_block error then
          if wait_readable poll ~token:(Kernel.Async.Token.make 802) (Kernel.Net.TcpListener.to_source listener) then
            loop ()
          else fail "expected property tcp listener to become readable"
        else fail (Kernel.Error.to_string (Kernel.Error.from_net_tcp_listener error))
  in
  loop ()
    ...
    if len = 0 then
      ...
      | Kernel.Result.Error error ->
          if is_tcp_stream_would_block error then
            if wait_writable poll ~token (Kernel.Net.TcpStream.to_source stream) then
              loop pos len
            else fail "expected property tcp write to become writable"
          else fail (Kernel.Error.to_string (Kernel.Error.from_net_tcp_stream error))
  in
    ...
    if len = 0 then
      ...
      | Kernel.Result.Error error ->
          if is_tcp_stream_would_block error then
            if wait_writable poll ~token (Kernel.Net.TcpStream.to_source stream) then
              loop pos len
            else fail "expected property tcp vectored write to become writable"
          else fail (Kernel.Error.to_string (Kernel.Error.from_net_tcp_stream error))
  in
      ...
      | Kernel.Result.Error error ->
          if is_tcp_stream_would_block error then
            if wait_readable poll ~token (Kernel.Net.TcpStream.to_source stream) then
              loop pos len
            else fail "expected property tcp read to become readable"
          else fail (Kernel.Error.to_string (Kernel.Error.from_net_tcp_stream error))
  in
  loop 0 (Kernel.Bytes.length buffer)
  ...
  let rec loop pos remaining =
    if remaining = 0 then
      ...
      | Kernel.Result.Ok read ->
          ...
          else loop (pos + read) (remaining - read)
      | Kernel.Result.Error error ->
          if is_tcp_stream_would_block error then
            if wait_readable poll ~token (Kernel.Net.TcpStream.to_source stream) then
              loop pos remaining
            else fail "expected property tcp vectored read to become readable"
          else fail (Kernel.Error.to_string (Kernel.Error.from_net_tcp_stream error))
  in
  loop 0 len
    ...
    | Kernel.Result.Error error ->
        if is_udp_would_block error then
          if wait_readable poll ~token (Kernel.Net.UdpSocket.to_source socket) then
            loop ()
          else fail "expected property udp socket to become readable"
        else fail (Kernel.Error.to_string (Kernel.Error.from_net_udp_socket error))
  in
  loop ()
  ...
  if value = 0 then
    ...
    let rec loop remaining acc =
      if remaining = 0 then
        ...
        let digit = String.make ~len:1 ~char:(String.get_unchecked digits ~at:(remaining mod 16)) in loop (remaining / 16) (digit :: acc)
    in
    String.concat "" (loop value [])

let ipv6_text = fun raw_groups ->
  let length = Kernel.Array.length raw_groups in
  let groups = Kernel.Array.init ~count:8 ~fn:(
    fun index -> hex_group (Kernel.Array.get_unchecked raw_groups ~at:(index mod length))
  ) in String.concat ":" (array_to_list groups)

let bounded_string_arb = fun ~min_len ~max_len -> Arbitrary.map_gen (Generator.string_size (Generator.int_range min_len max_len) Generator.char_printable) Arbitrary.string

let bounded_string_array_arb = fun ~min_count ~max_count ~min_len ~max_len ->
  let element_arb = bounded_string_arb ~min_len ~max_len in Arbitrary.map_gen (Generator.array_size (Generator.int_range min_count max_count) element_arb.gen) (Arbitrary.array element_arb)

let simple_segment_arb = Arbitrary.map_gen (Generator.string_size (Generator.int_range 1 16) Generator.char_lowercase) Arbitrary.string

let simple_segment_array_arb = fun ~min_count ~max_count -> Arbitrary.map_gen (Generator.array_size (Generator.int_range min_count max_count) simple_segment_arb.gen) (Arbitrary.array simple_segment_arb)

let non_empty_int_array_arb = fun ~max_count -> Arbitrary.map_gen (Generator.array_size (Generator.int_range 1 max_count) Generator.int) (Arbitrary.array Arbitrary.int)

let iovec_into_string_roundtrips =
  property "IO.IoVec of_string_array flattens with preserved order" Arbitrary.(array string)
    (
      fun values ->
        let iov = Kernel.IO.IoVec.from_string_array values |> Result.unwrap in Kernel.IO.IoVec.to_string iov = String.concat "" (array_to_list values)
    )

let iovec_sub_matches_flattened_substring =
  property "IO.IoVec.sub matches the flattened substring" Arbitrary.(pair (bounded_string_array_arb ~min_count:1 ~max_count:6 ~min_len:1 ~max_len:16) (pair int int))
    (
      fun (values, (raw_pos, raw_len)) ->
        ...
        let actual = Kernel.IO.IoVec.to_string (Kernel.IO.IoVec.sub ~pos ~len iov |> Result.unwrap) in actual = expected
    )

let array_of_list_roundtrips =
  property "Array.from_list roundtrips arbitrary integer arrays" Arbitrary.(array int)
    (
      fun values -> array_to_list (Kernel.Array.from_list (array_to_list values)) = array_to_list values
    )

let array_map_preserves_order =
  property "Array.map preserves element order" Arbitrary.(array int)
    (
      fun values -> array_to_list (Kernel.Array.map values ~fn:(
        fun value -> value + 1
      )) = List.map (array_to_list values) ~fn:(
        fun value -> value + 1
      )
    )
...
let option_unwrap_or_prefers_some =
  property "Option.unwrap_or prefers the Some branch" Arbitrary.(pair int int)
    (
      fun (value, default) -> Kernel.Option.unwrap_or (Kernel.Option.Some value) ~default = value && Kernel.Option.unwrap_or Kernel.Option.None ~default = default
    )
...
let result_map_error_preserves_ok =
  property "Result.map_err preserves the Ok branch" Arbitrary.int
    (
      fun value ->
        match Kernel.Result.map_err (Kernel.Result.Ok value) ~fn:(
          fun _ -> "mapped"
        ) with
...
let bytes_string_roundtrip =
  property "Bytes.of_string and Bytes.to_string roundtrip arbitrary strings" Arbitrary.string
    (
      fun value -> Kernel.String.equal (Kernel.Bytes.to_string (Kernel.Bytes.from_string value)) value
    )

let bytes_sub_string_matches_string_sub =
  property "Bytes.sub_string matches String.sub on valid slices" Arbitrary.(pair (bounded_string_arb ~min_len:1 ~max_len:256) (pair int int))
    (
      fun (value, (raw_pos, raw_len)) ->
        ...
        let remaining = total - pos in
        let len = Int.abs raw_len mod (remaining + 1) in
        let bytes = Kernel.Bytes.from_string value in Kernel.String.equal (Kernel.Bytes.sub_string bytes ~offset:pos ~len) (String.sub value ~offset:pos ~len)
    )

let string_bytes_roundtrip =
  property "String.to_bytes and String.of_bytes roundtrip arbitrary strings" Arbitrary.string
    (
      fun value -> Kernel.String.equal (Kernel.String.from_bytes (Kernel.String.to_bytes value)) value
    )
...
let path_join_preserves_segment_order =
  property "Path.join preserves simple segment order" Arbitrary.(pair simple_segment_arb simple_segment_arb)
    (
      fun (left, right) -> Kernel.Path.to_string (Kernel.Path.join left right) = format Std.Format.[ str left; str "/"; str right ]
    )

let path_fold_join_preserves_many_segment_order =
  property "Path.join preserves many simple segments in order" (simple_segment_array_arb ~min_count:1 ~max_count:8)
    (
      fun segments ->
        let values = array_to_list segments in
        let actual = List.fold_left values ~init:None ~fn:(
          fun acc part ->
            match acc with
            | None -> Some part
...
let path_join_is_associative_for_simple_segments =
  property "Path.join is associative for simple segments" Arbitrary.(triple simple_segment_arb simple_segment_arb simple_segment_arb)
    (
      fun (a, b, c) ->
        let left = Kernel.Path.to_string (Kernel.Path.join (Kernel.Path.join a b) c) in
        let right = Kernel.Path.to_string (Kernel.Path.join a (Kernel.Path.join b c)) in left = right
    )
...
let path_join_treats_empty_sides_as_identity =
  property "Path.join treats empty sides as identity" Arbitrary.string
    (
      fun value -> Kernel.String.equal (Kernel.Path.to_string (Kernel.Path.join "" value)) value && Kernel.String.equal (Kernel.Path.to_string (Kernel.Path.join value "")) value
    )
    ...
    (
      fun use_v6 ->
        let value =
          ...
          else Kernel.Net.IpAddr.v4_loopback
        in
    ...
    (
      fun (use_v6, raw_port) ->
        ...
        | Kernel.Result.Ok addr ->
            let addr_ip, addr_port = Kernel.Net.SocketAddr.to_parts addr in Kernel.Net.IpAddr.equal addr_ip ip && addr_port = port
    )

let socket_addr_ipv4_roundtrips =
  property "Net.SocketAddr.make roundtrips arbitrary ipv4 parts" Arbitrary.(pair (pair (pair int int) (pair int int)) int)
    (
...
let socket_addr_ipv6_roundtrips =
  property "Net.SocketAddr.make roundtrips arbitrary ipv6 parts" Arbitrary.(pair (non_empty_int_array_arb ~max_count:8) int)
    (
  ...
  property "Net.SocketAddr.loopback_v6 preserves loopback parts" Arbitrary.int
    (
      fun raw_port ->
        ...
        let addr_ip, addr_port = Kernel.Net.SocketAddr.to_parts addr in Kernel.Net.IpAddr.equal addr_ip Kernel.Net.IpAddr.v6_loopback && addr_port = port
    )

let file_slice_roundtrips =
  property "Fs.File partial write and read preserve the selected slice" Arbitrary.(pair (bounded_string_arb ~min_len:1 ~max_len:256) (pair int int))
    (
                    ...
                    | Kernel.Result.Ok written ->
                        ...
                        if written != len then
                          ...
                          | Kernel.Result.Ok input ->
                              ...
                              | Kernel.Result.Error error -> fail (Kernel.Fs.File.error_to_string error)
                              | Kernel.Result.Ok read ->
                                  let _ = Kernel.Fs.File.close input in read = len && Kernel.Bytes.sub_string bytes ~offset:pos ~len = Kernel.Bytes.sub_string buffer ~offset:0 ~len
                  with
                  | error ->
                      let _ = Kernel.Fs.File.close file in raise error
          )
    )

let file_vectored_roundtrips =
  property "Fs.File vectored writes and scalar reads preserve payload" (bounded_string_array_arb ~min_count:1 ~max_count:4 ~min_len:1 ~max_len:32)
    (
      fun values ->
        let pieces = array_to_list values in
        let total = List.fold_left pieces ~init:0 ~fn:(
          fun size part -> size + String.length part
        ) in
                    ...
                    | Kernel.Result.Ok written ->
                        ...
                        if written != Kernel.IO.IoVec.length iov then
                          ...
                          | Kernel.Result.Ok input ->
                              ...
                              | Kernel.Result.Error error -> fail (Kernel.Fs.File.error_to_string error)
                              | Kernel.Result.Ok read ->
                                  let _ = Kernel.Fs.File.close input in read = total && Kernel.Bytes.sub_string buffer ~offset:0 ~len:read = String.concat "" pieces
                  with
                  | error ->
                      let _ = Kernel.Fs.File.close file in raise error
          )
...
let file_scalar_write_vectored_read_roundtrips =
  property "Fs.File scalar writes and vectored reads preserve payload" (bounded_string_arb ~min_len:1 ~max_len:256)
    (
      fun payload ->
                    ...
                    | Kernel.Result.Ok written ->
                        ...
                        if written != Kernel.Bytes.length bytes then
                          ...
                          | Kernel.Result.Ok input ->
                              ...
                              | Kernel.Result.Error error -> fail (Kernel.Fs.File.error_to_string error)
                              | Kernel.Result.Ok read ->
                                  let _ = Kernel.Fs.File.close input in read = String.length payload && String.sub (Kernel.IO.IoVec.to_string iov) ~offset:0 ~len:read = payload
                  with
                  | error ->
                      let _ = Kernel.Fs.File.close file in raise error
          )
...
let file_scalar_and_vectored_partial_writes_agree =
  property "Fs.File scalar and vectored partial writes agree on the selected slice" Arbitrary.(pair (bounded_string_array_arb ~min_count:1 ~max_count:4 ~min_len:1 ~max_len:32) (pair int int))
    (
      fun (values, (raw_pos, raw_len)) ->
        let iov = Kernel.IO.IoVec.from_string_array values |> Result.unwrap in
                      ...
                      | Kernel.Result.Ok written ->
                          ...
                          if written != len then
                            ...
                            | Kernel.Result.Ok input ->
                                ...
                                | Kernel.Result.Error error -> fail (Kernel.Fs.File.error_to_string error)
                                | Kernel.Result.Ok read ->
                                    let _ = Kernel.Fs.File.close input in read = len && Kernel.Bytes.sub_string buffer ~offset:0 ~len:read = expected
                    with
                    | error ->
                        let _ = Kernel.Fs.File.close file in raise error
            )
...
let file_scalar_and_vectored_partial_reads_agree =
  property "Fs.File scalar and vectored partial reads agree on the selected prefix" Arbitrary.(pair (bounded_string_arb ~min_len:1 ~max_len:256) int)
    (
      fun (payload, raw_len) ->
        let total = String.length payload in
                        ...
                        if written != total then
                          ...
                          let scalar_result =
                            ...
                            | Kernel.Result.Ok input ->
                                ...
                                | Kernel.Result.Error error -> fail (Kernel.Fs.File.error_to_string error)
                                | Kernel.Result.Ok read ->
                                    let _ = Kernel.Fs.File.close input in read = len && Kernel.Bytes.sub_string buffer ~offset:1 ~len = expected
                          in
                          let vectored_result =
                            match Kernel.Fs.File.open_read path with
                            | Kernel.Result.Error error -> fail (Kernel.Fs.File.error_to_string error)
                            | Kernel.Result.Ok input ->
                                ...
                                | Kernel.Result.Error error -> fail (Kernel.Fs.File.error_to_string error)
                                | Kernel.Result.Ok read ->
                                    let _ = Kernel.Fs.File.close input in read = len && String.sub (Kernel.IO.IoVec.to_string iov) ~offset:1 ~len = expected
                          in
...
let tcp_loopback_roundtrips_small_payload =
  property "Net.TcpStream loopback roundtrips small payloads" (bounded_string_arb ~min_len:1 ~max_len:64)
    (
      fun payload ->
        with_poll
          (
            fun poll ->
              ...
              | Kernel.Result.Error error -> fail (Kernel.Error.to_string (Kernel.Error.from_net_tcp_listener error))
                                  ...
                                  protect ~finally:(
                                    ...
                                    (
                                      fun () ->
                                        ...
                                        Kernel.Bytes.sub_string buffer ~offset:0 ~len:(String.length payload) = payload
                                    )
                              )
                    )
          )
...
let tcp_vectored_loopback_roundtrips_small_payload =
  property "Net.TcpStream loopback roundtrips small vectored payloads" (bounded_string_array_arb ~min_count:1 ~max_count:4 ~min_len:1 ~max_len:16)
    (
      fun values ->
        let pieces = array_to_list values in
                                  ...
                                  protect ~finally:(
                                    ...
                                    (
                                      fun () ->
                                        ...
                                        read_exact_vectored poll ~token:(Kernel.Async.Token.make 807) server inbound ~len:total;
...
let tcp_vectored_loopback_roundtrips_offset_receive_slices =
  property "Net.TcpStream loopback roundtrips vectored payloads into offset receive slices" (bounded_string_array_arb ~min_count:1 ~max_count:6 ~min_len:1 ~max_len:16)
    (
      fun values ->
        let pieces = array_to_list values in
        let payload = String.concat "" pieces in
                                  ...
                                  protect ~finally:(
                                    ...
                                    (
                                      fun () ->
                                        ...
                                        read_exact_vectored poll ~token:(Kernel.Async.Token.make 809) server slice ~len:total;
...
let udp_loopback_roundtrips_small_payload =
  property "Net.UdpSocket loopback preserves small datagrams" (bounded_string_arb ~min_len:1 ~max_len:64)
    (
      fun payload ->
        with_poll
          (
            fun poll ->
              ...
              | Kernel.Result.Error error -> fail (Kernel.Error.to_string (Kernel.Error.from_net_udp_socket error))
                                  ...
                                  | Kernel.Result.Ok server_addr ->
                                      ...
                                      | Kernel.Result.Ok written ->
                                          if written != String.length payload then
                                            ...
                                            let read, _from = recv_from_udp poll ~token:(Kernel.Async.Token.make 805) server buffer in read = String.length payload && Kernel.Bytes.sub_string buffer ~offset:0 ~len:read = payload
                              )
                    )
          )
    )
...
let main ~args = Test.Cli.main ~execution_mode:Test.Cli.Linear ~name:"kernel_new_property_tests" ~tests ~args ()
