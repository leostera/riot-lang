...
let lift_system_time result =
  ...
  | Kernel.Result.Error error -> Error (Kernel.Error.to_string (Kernel.Error.from_time_system_time error))
    ...
    (
      fun () ->
        ...
        if List.any events ~fn:(
          fun event -> Kernel.Async.Token.equal token (Kernel.Async.Event.token event) && Kernel.Async.Event.is_readable event
        ) then
          Ok ()
        else Error "expected timer source to wake the poller"
    )
...
let test_of_parts_roundtrips = fun _ctx ->
  let* value = lift_system_time (Kernel.Time.SystemTime.from_parts ~secs:42 ~nanos:123_456_789)
  in
  let (secs, nanos) = Kernel.Time.SystemTime.to_parts value in
  if secs = 42 && nanos = 123_456_789 then
    Ok ()
  else Error "expected system time parts roundtrip"

let test_now_is_at_or_after_epoch = fun _ctx ->
  let* now = lift_system_time (Kernel.Time.SystemTime.now ())
  in
  if match Kernel.Time.SystemTime.compare now Kernel.Time.SystemTime.epoch with
  | Kernel.Order.LT -> false
  | Kernel.Order.EQ | Kernel.Order.GT -> true then
    Ok ()
  else Error "expected current system time to be at or after the epoch"
...
let test_now_has_valid_nanoseconds = fun _ctx ->
  ...
  if nanos >= 0 && nanos < 1_000_000_000 then
    Ok ()
  else Error "expected current system time nanoseconds to be normalized"

let test_system_time_diff_ns_matches_parts = fun _ctx ->
  let* later = lift_system_time (Kernel.Time.SystemTime.from_parts ~secs:12 ~nanos:250_000_000)
  in
  let* earlier = lift_system_time (Kernel.Time.SystemTime.from_parts ~secs:10 ~nanos:125_000_000)
  in
  if Kernel.Time.SystemTime.diff_ns later earlier = 2_125_000_000L then
    Ok ()
  else Error "expected system time diff_ns to preserve raw nanosecond deltas"

let test_monotonic_now_is_non_decreasing = fun _ctx ->
  let* earlier = lift_monotonic (Kernel.Time.Monotonic.now ())
  in
  let* later = lift_monotonic (Kernel.Time.Monotonic.now ())
  in
  if match Kernel.Time.Monotonic.compare earlier later with
  | Kernel.Order.LT | Kernel.Order.EQ -> true
  | Kernel.Order.GT -> false then
    Ok ()
  else Error "expected monotonic now to be non-decreasing"

let test_monotonic_now_has_valid_nanoseconds = fun _ctx ->
  let* now = lift_monotonic (Kernel.Time.Monotonic.now ())
  in
  let nanos = Kernel.Time.Monotonic.subsec_nanos now in
  if nanos >= 0 && nanos < 1_000_000_000 then
    Ok ()
  else Error "expected monotonic now nanoseconds to be normalized"
...
let test_monotonic_diff_ns_matches_parts = fun _ctx ->
  ...
  let expected = Int64.add (Int64.mul (Int64.from_int (later_secs - earlier_secs)) 1_000_000_000L) (Int64.from_int (later_nanos - earlier_nanos)) in
  if Kernel.Time.Monotonic.diff_ns later earlier = expected then
    Ok ()
  else Error "expected monotonic diff_ns to preserve raw nanosecond deltas"
...
let test_timer_after_ns_wakes_poll = fun _ctx ->
  with_poll
    (
      fun poll ->
        let* timer = lift_timer (Kernel.Time.Timer.after_ns 5_000_000L) in wait_for_timer poll ~token:(Kernel.Async.Token.make 700) timer
    )
    ...
    (
      fun poll ->
        ...
        protect ~finally:(
          ...
          (
            fun () ->
              let* () = wait_for_readable_token poll ~token
              in
  ...
  with_poll
    (
      fun poll ->
        ...
        let* () = lift_async (Kernel.Async.Poll.register poll first_token Kernel.Async.Interest.readable source)
        in
        protect ~finally:(
          fun () ->
            let _ = Kernel.Async.Poll.deregister poll source in ()
          ...
          (
            fun () ->
              let* () = wait_for_readable_token poll ~token:first_token
              in
              let* () = lift_async (Kernel.Async.Poll.reregister poll second_token Kernel.Async.Interest.readable source) in wait_for_readable_token poll ~token:second_token
          )
    )
      ...
      fun poll ->
        let rec create remaining acc =
          if remaining = 0 then
            ...
            let* timer = lift_timer (Kernel.Time.Timer.after_ns 5_000_000L) in create (remaining - 1) (timer :: acc)
        in
        let* timers = create 12 []
        in
...
let test_timer_every_ns_repeats = fun _ctx ->
  with_poll
    (
      fun poll ->
        let* timer = lift_timer (Kernel.Time.Timer.every_ns 5_000_000L)
        in
        let source = Kernel.Time.Timer.to_source timer in
            ...
            fun () ->
              let rec poll_twice remaining =
                if remaining = 0 then
                  ...
                  else Error "expected repeating timer to remain readable across multiple polls"
              in
              poll_twice 2
          )
    )
  ...
  with_poll
    (
      fun poll ->
        ...
        let* () = lift_async (Kernel.Async.Poll.register poll token_a Kernel.Async.Interest.readable source)
        in
        protect ~finally:(
          fun () ->
            let _ = Kernel.Async.Poll.deregister poll source in ()
          ...
          (
            fun () ->
              let* () = lift_async (Kernel.Async.Poll.reregister poll token_b Kernel.Async.Interest.readable source)
              in
              let* () = wait_for_readable_token poll ~token:token_b in Ok ()
          )
    )
      ...
      fun poll ->
        ...
        let rec loop remaining =
          if remaining = 0 then
            ...
            let* () = lift_async (Kernel.Async.Poll.reregister poll token_b Kernel.Async.Interest.readable source)
            in
            let* () = wait_for_readable_token poll ~token:token_b
            in
    ...
    (
      fun poll ->
        ...
        if has_readable_token first_token quiet then
          ...
          let* () = lift_async (Kernel.Async.Poll.register poll second_token Kernel.Async.Interest.readable source)
          in
          protect ~finally:(
            fun () ->
              let _ = Kernel.Async.Poll.deregister poll source in ()
    ...
    (
      fun poll ->
        ...
        protect ~finally:(
          ...
          (
            fun () ->
              let* first = lift_async (Kernel.Async.Poll.poll ~timeout:100_000_000L ~max_events:8 poll)
              in
              if not (has_readable_token token first) then
                ...
                let* () = lift_async (Kernel.Async.Poll.deregister poll source)
                in
                let* second = lift_async (Kernel.Async.Poll.poll ~timeout:20_000_000L ~max_events:8 poll)
                in
  ...
  with_poll
    (
      fun poll ->
        ...
        let* timer = lift_timer (Kernel.Time.Timer.every_ns interval_ns)
        in
        let source = Kernel.Time.Timer.to_source timer in
        let token = Kernel.Async.Token.make 703 in
        let* start = lift_monotonic (Kernel.Time.Monotonic.now ())
        in
        let* () = lift_async (Kernel.Async.Poll.register poll token Kernel.Async.Interest.readable source)
        in
        protect ~finally:(
          ...
          (
            fun () ->
              let rec wait_for_ticks seen =
                if seen = 3 then
                  let* finish = lift_monotonic (Kernel.Time.Monotonic.now ())
                  in
                  let elapsed = Kernel.Time.Monotonic.diff_ns finish start in
                  if elapsed >= 10_000_000L && elapsed < 1_000_000_000L then
                    Ok ()
                  else Error "expected repeating timer ticks to stay within a reasonable tolerance"
                else
                  let* events = lift_async (Kernel.Async.Poll.poll ~timeout:200_000_000L ~max_events:8 poll)
                  in
                  if has_readable_token token events then
                    wait_for_ticks (seen + 1)
                  else Error "expected repeating timer to keep producing ticks over time"
              in
              wait_for_ticks 0
          )
    )
...
let test_timer_after_ns_elapsed_time_is_reasonable = fun _ctx ->
  with_poll
    (
      fun poll ->
        let interval_ns = 20_000_000L in
        let* timer = lift_timer (Kernel.Time.Timer.after_ns interval_ns)
        in
        let source = Kernel.Time.Timer.to_source timer in
        ...
        protect ~finally:(
          ...
          (
            fun () ->
              ...
              if elapsed >= 10_000_000L && elapsed < 1_000_000_000L then
                Ok ()
              else Error "expected one-shot timer latency to stay within a reasonable tolerance"
          )
    )
...
let test_system_time_accessors_match_the_constructor = fun _ctx ->
  let* value = lift_system_time (Kernel.Time.SystemTime.from_parts ~secs:17 ~nanos:123_456_789)
  in
  let secs, nanos = Kernel.Time.SystemTime.to_parts value in
  if secs = 17 && nanos = 123_456_789 && Kernel.Time.SystemTime.secs value = 17 && Kernel.Time.SystemTime.subsec_nanos value = 123_456_789 then
    Ok ()
  else Error "expected SystemTime accessors to preserve constructor inputs exactly"

let test_system_time_compare_and_equal_are_antisymmetric = fun _ctx ->
  let* earlier = lift_system_time (Kernel.Time.SystemTime.from_parts ~secs:1 ~nanos:5)
  in
  let* same = lift_system_time (Kernel.Time.SystemTime.from_parts ~secs:1 ~nanos:5)
  in
  let* later = lift_system_time (Kernel.Time.SystemTime.from_parts ~secs:1 ~nanos:6)
  in
  if Kernel.Time.SystemTime.equal earlier same && Kernel.Time.SystemTime.compare earlier same = Kernel.Order.EQ && Kernel.Time.SystemTime.compare earlier later = Kernel.Order.LT && Kernel.Time.SystemTime.compare later earlier = Kernel.Order.GT then
    Ok ()
  else Error "expected SystemTime compare and equal to preserve antisymmetric ordering"

let test_system_time_diff_ns_is_antisymmetric = fun _ctx ->
  let* left = lift_system_time (Kernel.Time.SystemTime.from_parts ~secs:10 ~nanos:5)
  in
  let* right = lift_system_time (Kernel.Time.SystemTime.from_parts ~secs:7 ~nanos:10)
  in
  if Kernel.Time.SystemTime.diff_ns left right = Int64.neg (Kernel.Time.SystemTime.diff_ns right left) then
    Ok ()
  else Error "expected SystemTime.diff_ns to be antisymmetric"
...
let test_monotonic_accessors_are_consistent_on_now = fun _ctx ->
  ...
  if secs = Kernel.Time.Monotonic.secs now && nanos = Kernel.Time.Monotonic.subsec_nanos now then
    Ok ()
  else Error "expected Monotonic accessors to agree with to_parts"

let test_timer_accessors_report_timeout_and_repeat_flag = fun _ctx ->
  let* one_shot = lift_timer (Kernel.Time.Timer.after_ns 7L)
  in
  let* repeating = lift_timer (Kernel.Time.Timer.every_ns 11L)
  in
  if Kernel.Time.Timer.timeout_ns one_shot = 7L && not (Kernel.Time.Timer.repeats one_shot) && Kernel.Time.Timer.timeout_ns repeating = 11L && Kernel.Time.Timer.repeats repeating then
    Ok ()
  else Error "expected Timer accessors to preserve the configured timeout and repeat flag"

let test_one_shot_timer_stays_quiet_before_deadline = fun _ctx ->
  with_poll
    (
      fun poll ->
        let* timer = lift_timer (Kernel.Time.Timer.after_ns 20_000_000L)
        in
...
let test_reregistering_one_shot_before_fire_resets_the_deadline = fun _ctx ->
  with_poll
    (
      fun poll ->
        let* timer = lift_timer (Kernel.Time.Timer.after_ns 25_000_000L)
        in
        let source = Kernel.Time.Timer.to_source timer in
        ...
        protect ~finally:(
          ...
          (
            fun () ->
              let* quiet_before = lift_async (Kernel.Async.Poll.poll ~timeout:15_000_000L poll)
              in
              if has_readable_token token quiet_before then
                Error "expected the original timer deadline to still be in the future"
              else
                let* () = lift_async (Kernel.Async.Poll.reregister poll replacement Kernel.Async.Interest.readable source)
                in
                let* quiet_after = lift_async (Kernel.Async.Poll.poll ~timeout:15_000_000L poll)
                in
                if has_readable_token replacement quiet_after then
                  ...
                  let* ready = lift_async (Kernel.Async.Poll.poll ~timeout:40_000_000L poll)
                  in
                  if has_readable_token replacement ready then
                    Ok ()
                  else Error "expected the rearmed one-shot timer to fire after the reset deadline"
          )
    )
...
let main ~args = Test.Cli.main ~execution_mode:Test.Cli.Linear ~name:"kernel_new_system_time_tests" ~tests ~args ()
