...
let with_pipe = fun fn ->
  let* pipe = lift_file (Kernel.Fs.File.pipe ())
  in
  protect ~finally:(
    fun () ->
...
let with_two_pipes = fun fn ->
  let* first = lift_file (Kernel.Fs.File.pipe ())
  in
  let* second = lift_file (Kernel.Fs.File.pipe ())
  in
  protect ~finally:(
    fun () ->
...
let with_pipes = fun count fn ->
  let rec create remaining acc =
    if remaining = 0 then
      ...
      let* pipe = lift_file (Kernel.Fs.File.pipe ()) in create (remaining - 1) ((pipe.read_end, pipe.write_end) :: acc)
  in
  let* pipes = create count []
  in
  protect ~finally:(
...
let has_token = fun token events -> List.any events ~fn:(
  fun event -> Kernel.Async.Token.equal token (Kernel.Async.Event.token event)
)
...
let with_processes = fun count fn ->
  ...
  let rec spawn remaining acc =
    if remaining = 0 then
      ...
      | Kernel.Result.Error error -> Error (Kernel.Process.error_to_string error)
  in
  let* processes = spawn count []
  in
  protect ~finally:(
    fun () -> close_processes processes
  )
    (
      fun () -> fn (List.reverse processes)
    )

let wait_for_event = fun ?(timeout = 100_000_000L) poll -> lift_async (Kernel.Async.Poll.poll ~timeout poll)
        ...
        with_poll
          (
            fun poll ->
              ...
              let* () = lift_async (Kernel.Async.Poll.register poll token Kernel.Async.Interest.readable (Kernel.Fs.File.to_source read_end))
              in
              let payload = Kernel.Bytes.from_string "x" in
              let* written = lift_file (Kernel.Fs.File.write write_end payload)
              in
              if written != 1 then
                ...
                if found then
                  Ok ()
                else Error "expected poll to report readability for pipe source"
          )
    )
        ...
        with_poll
          (
            fun poll ->
              ...
              in
              let* () = lift_file (Kernel.Fs.File.close write_end)
              in
              let* events = lift_async (Kernel.Async.Poll.poll ~timeout:100_000_000L poll)
              in
              let found = List.any events ~fn:(
                fun event -> Kernel.Async.Event.is_read_closed event && Kernel.Async.Token.equal token (Kernel.Async.Event.token event)
              ) in
              if found then
                Ok ()
              else Error "expected poll to report read closure when the pipe writer closes"
          )
    )
          ...
          (
            fun poll ->
              ...
              if written != 1 then
                ...
                let* events = lift_async (Kernel.Async.Poll.poll ~timeout:0L poll)
                in
                let found = List.any events ~fn:(
                  fun event -> Kernel.Async.Event.is_readable event && Kernel.Async.Token.equal token (Kernel.Async.Event.token event)
                ) in
        ...
        with_poll
          (
            fun poll ->
              ...
              let* () = lift_async (Kernel.Async.Poll.register poll token_a Kernel.Async.Interest.writable source)
              in
              let* () = lift_async (Kernel.Async.Poll.reregister poll token_b Kernel.Async.Interest.writable source)
              in
              let* events = wait_for_event poll
              in
              let found = List.any events ~fn:(
                fun event -> Kernel.Async.Event.is_writable event && Kernel.String.equal (Kernel.Async.Token.unsafe_value (Kernel.Async.Event.token event)) "second"
              ) in
              if found then
                Ok ()
              else Error "expected reregister to replace the writable token"
          )
    )
        ...
        with_poll
          (
            fun poll ->
              ...
              let* () = lift_async (Kernel.Async.Poll.register poll token Kernel.Async.Interest.writable source)
              in
              let* () = lift_async (Kernel.Async.Poll.reregister poll token Kernel.Async.Interest.readable source)
              in
              let* events = lift_async (Kernel.Async.Poll.poll ~timeout:0L poll)
              in
              let found = List.any events ~fn:(
                fun event -> Kernel.Async.Event.is_writable event && Kernel.Async.Token.equal token (Kernel.Async.Event.token event)
              ) in
              if found then
        ...
        with_poll
          (
            fun poll ->
              ...
              let* () = lift_async (Kernel.Async.Poll.register poll first Kernel.Async.Interest.writable source)
              in
              let* () = lift_async (Kernel.Async.Poll.register poll second Kernel.Async.Interest.writable source)
              in
              let* events = wait_for_event poll
              in
              let saw_first = List.any events ~fn:(
                fun event -> Kernel.Async.Event.is_writable event && Kernel.Async.Token.equal first (Kernel.Async.Event.token event)
              ) in
              let saw_second = List.any events ~fn:(
                fun event -> Kernel.Async.Event.is_writable event && Kernel.Async.Token.equal second (Kernel.Async.Event.token event)
              ) in
              if saw_second && not saw_first then
                Ok ()
              else Error "expected duplicate register to replace the writable token in place"
          )
    )
        ...
        with_poll
          (
            fun poll ->
              ...
              let* () = lift_async (Kernel.Async.Poll.register poll token Kernel.Async.Interest.readable source)
              in
              let* written = lift_file (Kernel.Fs.File.write write_end (Kernel.Bytes.from_string "x"))
              in
              if written != 1 then
                ...
                if found then
                  Ok ()
                else Error "expected noop deregisters to preserve later source registration"
          )
    )
          ...
          (
            fun poll ->
              let rec register index = function
                | [] -> Ok ()
                | (read_end, _) :: rest ->
                    let* () = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make index) Kernel.Async.Interest.readable (Kernel.Fs.File.to_source read_end)) in register (index + 1) rest
              in
              ...
              in
              let* () = register 0 pipes
              in
              let* () = write_all pipes
              in
              let* events = lift_async (Kernel.Async.Poll.poll ~timeout:100_000_000L ~max_events:128 poll)
              in
              ...
              if all_seen 0 then
                Ok ()
              else Error "expected poll to surface readability for many registered sources"
          )
    )
...
let test_token_roundtrips_structured_values = fun _ctx ->
  ...
  if Kernel.String.equal tag "pipe" && value = 99 then
    Ok ()
  else Error "expected token to roundtrip its structured value"
  ...
  with_poll
    (
      fun poll ->
        ...
        | (Kernel.Result.Error (Kernel.Async.InvalidTimeoutNs { timeout_ns }), Kernel.Result.Error (Kernel.Async.InvalidMaxEvents { max_events })) when timeout_ns = (-1L) && max_events = 0 -> Ok ()
            ...
            fun processes ->
              ...
              let rec poll_until attempts =
                if attempts = 0 then
                  ...
                  let* events = lift_async (Kernel.Async.Poll.poll ~timeout:100_000_000L ~max_events:32 poll) in mark_events events;
                let* () = mark_exits 0 processes
                in
                if all_seen 0 then
                  Ok ()
  ...
  with_poll
    (
      fun poll ->
        ...
        in
        let* timers = create 16 []
        in
        let timers = List.reverse timers in
        let rec register index = function
          | [] -> Ok ()
          | timer :: rest ->
              let* () = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make index) Kernel.Async.Interest.readable (Kernel.Time.Timer.to_source timer)) in register (index + 1) rest
        in
        let seen = Kernel.Array.make ~count:16 ~value:false in
        let rec mark = function
          | [] -> ()
            ...
            fun poll ->
              ...
              let rec loop remaining =
                if remaining = 0 then
                  ...
                  let* () = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make remaining) Kernel.Async.Interest.readable source)
                  in
                  let* () = lift_async (Kernel.Async.Poll.deregister poll source) in loop (remaining - 1)
              in
              loop 256
          )
    )
            ...
            fun poll ->
              ...
              let rec loop remaining =
                if remaining = 0 then
                  ...
                  let* () = lift_async (Kernel.Async.Poll.register poll token Kernel.Async.Interest.writable source)
                  in
                  let* () = lift_async (Kernel.Async.Poll.reregister poll replacement Kernel.Async.Interest.writable source)
                  in
                  let* events = wait_for_event poll
                  in
                  let found = List.any events ~fn:(
                    fun event -> Kernel.Async.Event.is_writable event && Kernel.Async.Token.equal replacement (Kernel.Async.Event.token event)
                  ) in
                  if not found then
      ...
      fun read_end write_end ->
        with_poll
          (
            fun poll ->
              let stdio = Kernel.Process.{ stdin = Stdin.Null; stdout = Stdout.Null; stderr = Stderr.Null } in
              let* timer = lift_timer (Kernel.Time.Timer.after_ns 5_000_000L)
              in
                          ...
                          protect ~finally:(
                            ...
                            (
                              fun () ->
                                ...
                                in
                                let* () = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make "pipe") Kernel.Async.Interest.readable pipe_source)
                                in
                                let* () = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make "timer") Kernel.Async.Interest.readable timer_source)
                                in
                                let* () = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make "process") Kernel.Async.Interest.priority process_source)
                                in
                                let* () = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make "udp") Kernel.Async.Interest.readable udp_source)
                                in
                                protect ~finally:(
                                  fun () ->
                                  ...
                                  (
                                    fun () ->
                                      ...
                                      if written != 1 then
                                        ...
                                        let* sent = lift_udp (Kernel.Net.UdpSocket.send_to client server_addr (Kernel.Bytes.from_string "u"))
                                        in
                                        if sent != 1 then
...
let test_interest_add_and_remove_roundtrip = fun _ctx ->
  ...
  | Some interest ->
      if Kernel.Async.Interest.is_readable interest && not (Kernel.Async.Interest.is_writable interest) then
        Ok ()
      else Error "expected Interest.remove to leave only the readable bit"
...
let test_token_id_is_stable = fun _ctx ->
  ...
  if Kernel.Async.Token.id token = Kernel.Async.Token.id token then
    Ok ()
  else Error "expected Token.id to stay stable for the same token"
  ...
  with_poll
    (
      fun poll ->
        ...
        if events = [] then
          Ok ()
        else Error "expected a fresh poller with timeout=0 to be quiet"
    )
        ...
        with_poll
          (
            fun poll ->
              ...
              let payload = Kernel.Bytes.from_string "x" in
              let* () = lift_async (Kernel.Async.Poll.register poll first_token Kernel.Async.Interest.readable first_source)
              in
              let* () = lift_async (Kernel.Async.Poll.register poll second_token Kernel.Async.Interest.readable second_source)
              in
              let* _ = lift_file (Kernel.Fs.File.write first.write_end payload)
              in
              let* _ = lift_file (Kernel.Fs.File.write second.write_end payload)
              in
              let* first_batch = lift_async (Kernel.Async.Poll.poll ~timeout:100_000_000L ~max_events:1 poll)
              in
              let first_seen =
                if has_token first_token first_batch then
                  Some (first.read_end, first_token, second_token)
                else
                  if has_token second_token first_batch then
              ...
              | Some (ready_file, ready_token, other_token) ->
                  let* () = drain_one_byte ready_file
                  in
                  let* second_batch = lift_async (Kernel.Async.Poll.poll ~timeout:100_000_000L ~max_events:1 poll)
                  in
                  if has_token ready_token first_batch && not (has_token other_token first_batch) && has_token other_token second_batch then
                    Ok ()
                  else Error "expected max_events=1 polls to batch readiness across successive calls"
          )
    )
        ...
        with_poll
          (
            fun poll ->
              ...
              let* () = lift_async (Kernel.Async.Poll.register poll token Kernel.Async.Interest.readable source)
              in
              let* _ = lift_file (Kernel.Fs.File.write write_end payload)
              in
              let* events = lift_async (Kernel.Async.Poll.poll ~timeout:100_000_000L ~max_events:8 poll)
              in
              let matches = List.filter events ~fn:(
                fun event -> Kernel.Async.Event.is_readable event && Kernel.Async.Token.equal token (Kernel.Async.Event.token event)
              ) in
              if List.length matches <= 1 then
                Ok ()
              else Error "expected duplicate registration with the same token to avoid duplicate readiness events"
          )
    )
        ...
        with_poll
          (
            fun poll ->
              ...
              let* () = lift_async (Kernel.Async.Poll.register poll token Kernel.Async.Interest.writable source)
              in
              let* () = lift_file (Kernel.Fs.File.close read_end)
              in
              let* events = lift_async (Kernel.Async.Poll.poll ~timeout:100_000_000L poll)
              in
              if List.any events ~fn:(
                fun event -> Kernel.Async.Token.equal token (Kernel.Async.Event.token event) && Kernel.Async.Event.is_write_closed event
              ) then
                Ok ()
              else Error "expected a pipe writer to report write_closed after the reader closes"
          )
    )
        ...
        with_poll
          (
            fun poll ->
              ...
              in
              if List.any events ~fn:(
                fun event -> Kernel.Async.Token.equal token (Kernel.Async.Event.token event) && Kernel.Async.Event.is_readable event && not (Kernel.Async.Event.is_error event)
              ) then
                Ok ()
              else Error "expected ordinary readiness events to stay out of the error bucket"
          )
    )
  ...
  with_pipe
    (
      fun read_end _write_end ->
        ...
        let* poll = lift_async (Kernel.Async.Poll.make ())
        in
        let* () = lift_async (Kernel.Async.Poll.close poll)
        in
        if expect_bad_fd (Kernel.Async.Poll.poll ~timeout:0L poll) && expect_bad_fd (Kernel.Async.Poll.register poll token Kernel.Async.Interest.readable source) && expect_bad_fd (Kernel.Async.Poll.reregister poll token Kernel.Async.Interest.readable source) && expect_bad_fd (Kernel.Async.Poll.deregister poll source) then
          Ok ()
        else Error "expected a closed poller to reject later operations with a typed bad-fd error"
    )
...
let main ~args = Test.Cli.main ~execution_mode:Linear ~name:"kernel_new_async_tests" ~tests ~args ()
