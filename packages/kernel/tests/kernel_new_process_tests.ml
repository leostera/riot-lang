...
let rec close_processes = fun processes ->
  ...
  | [] -> ()
  | process :: rest ->
      let _ = Kernel.Process.close process in close_processes rest
  ...
  let rec loop () =
    ...
    | Kernel.Result.Error error ->
        if is_would_block error then
          let* () = wait_readable poll ~token (Kernel.Fs.File.to_source file) in loop ()
        else Error (Kernel.Fs.File.error_to_string error)
  in
  loop ()
  ...
  let rec loop pos len =
    if len = 0 then
      ...
      | Kernel.Result.Ok written ->
          ...
          else loop (pos + written) (len - written)
      | Kernel.Result.Error error ->
          if is_would_block error then
...
let wait_for_exit = fun poll ~token process ->
  ...
  let rec loop () =
    let* status = lift_process (Kernel.Process.try_wait process)
    in
    match status with
    | Some status -> Ok status
    | None ->
        let source = Kernel.Process.to_source process in
        let* () = lift_async (Kernel.Async.Poll.register poll token Kernel.Async.Interest.priority source)
        in
        protect ~finally:(
          fun () ->
            let _ = Kernel.Async.Poll.deregister poll source in ()
          ...
          (
            fun () ->
              let* _ = lift_async (Kernel.Async.Poll.poll ~timeout:exit_poll_timeout poll) in loop ()
          )
  in
  loop ()
...
let wait_for_priority_token = fun poll ~token ->
  let rec loop attempts =
    if attempts = 0 then
      ...
      in
      if List.any events ~fn:(
        fun event -> Kernel.Async.Token.equal token (Kernel.Async.Event.token event) && Kernel.Async.Event.is_priority event
      ) then
        Ok ()
...
let send_signal_command = fun ~signal process ->
  ...
  let* helper = lift_process (Kernel.Process.spawn ~program:"/bin/kill" ~args:[|
    signal;
    Kernel.Int.to_string (Kernel.Process.pid process);
  |] ~stdio ())
  in
  with_process helper
    (
      fun helper ->
        with_poll
          (
            fun poll ->
              let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make ("kill-helper", signal)) helper
              in
              if status = Kernel.Process.Exited 0 then
                Ok ()
              else Error "expected /bin/kill to deliver the requested signal cleanly"
          )
    )
...
let test_current_pid_is_positive = fun _ctx ->
  if Kernel.Process.current_pid () > 0 then
    Ok ()
  else Error "expected current_pid to be positive"
...
let test_stdout_pipe_roundtrips = fun _ctx ->
  ...
  let* process = lift_process (Kernel.Process.spawn ~program:"/bin/echo" ~args:[|
    "-n";
    "hello";
  |] ~stdio ())
  in
      ...
      fun process ->
        ...
        | Some stdout ->
            with_poll
              (
                fun poll ->
                  let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 520) process
                  in
                  let* payload = read_all poll ~token:(Kernel.Async.Token.make 501) stdout
                  in
                  if payload = "hello" && status = Kernel.Process.Exited 0 then
                    Ok ()
                  else Error "expected echo stdout payload and zero exit status"
              )
    )
...
let test_stdin_and_stdout_pipes_roundtrip = fun _ctx ->
  ...
  let* process = lift_process (Kernel.Process.spawn ~program:"/bin/cat" ~args:[||] ~stdio ())
  in
  with_process process
    (
      fun process ->
            ...
            with_poll
              (
                fun poll ->
                  ...
                  let* () = write_all poll ~token:(Kernel.Async.Token.make 502) stdin payload
                  in
                  let* () = lift_file (Kernel.Fs.File.close stdin)
                  in
                  let* echoed = read_once poll ~token:(Kernel.Async.Token.make 503) stdout
                  in
                  let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 521) process
                  in
                  if echoed = "ping" && status = Kernel.Process.Exited 0 then
                    Ok ()
                  else Error "expected cat to echo stdin and exit cleanly"
              )
        | _ -> Error "expected stdin and stdout pipes"
    )

let test_stderr_redirect_to_stdout_merges_streams = fun _ctx ->
  let stdio = Kernel.Process.{ stdin = Stdin.Null; stdout = Stdout.Pipe; stderr = Stderr.RedirectToStdout } in
  let* process = lift_process (Kernel.Process.spawn ~program:"/bin/sh" ~args:[|
    "-c";
      ...
      fun process ->
        ...
        | Some stdout ->
            with_poll
              (
                fun poll ->
                  let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 522) process
                  in
                  let* payload = read_all poll ~token:(Kernel.Async.Token.make 504) stdout
                  in
                  if payload = "outerr" && status = Kernel.Process.Exited 0 then
                    Ok ()
                  else Error "expected redirected stderr to be merged into stdout"
              )
    )
      ...
      fun process ->
        ...
        | (Some stdout, Some stderr) ->
            with_poll
              (
                fun poll ->
                  let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 536) process
                  in
                  let* stdout_payload = read_all poll ~token:(Kernel.Async.Token.make 509) stdout
                  in
                  let* stderr_payload = read_all poll ~token:(Kernel.Async.Token.make 510) stderr
                  in
                  if status = Kernel.Process.Exited 0 && stdout_payload = "out" && stderr_payload = "err" then
                    Ok ()
                  else Error "expected stdout and stderr pipes to stay drainable after process exit"
              )
      ...
      fun process ->
        ...
        | None ->
            with_poll
              (
                fun poll ->
                  let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 523) process
                  in
                  if status = Kernel.Process.Exited 0 then
                    Ok ()
                  else Error "expected process to exit cleanly after async poll"
              )
    )
    ...
    (
      fun process ->
        with_poll
          (
            fun poll ->
              let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 531) process
              in
              if status = Kernel.Process.Exited 0 then
                Ok ()
              else Error "expected async process source to report a clean exit"
          )
    )
        ...
        with_poll
          (
            fun poll ->
              ...
              let* () = lift_async (Kernel.Async.Poll.register poll token_a Kernel.Async.Interest.priority source)
              in
              protect ~finally:(
                fun () ->
                  let _ = Kernel.Async.Poll.deregister poll source in ()
                ...
                (
                  fun () ->
                    let* () = lift_async (Kernel.Async.Poll.reregister poll token_b Kernel.Async.Interest.priority source)
                    in
                    let* () = wait_for_priority_token poll ~token:token_b
                    in
          ...
          (
            fun poll ->
              ...
              protect ~finally:(
                ...
                (
                  fun () ->
                    let* () = lift_process (Kernel.Process.close process)
                    in
                    let* () = wait_for_priority_token poll ~token
                    in
                    match Kernel.Process.try_wait process with
                    | Kernel.Result.Ok (Some (Kernel.Process.Exited 0)) -> Ok ()
      ...
      fun process ->
        with_poll
          (
            fun poll ->
              let source = Kernel.Process.to_source process in
              let* () = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make "process-deregister-then-close") Kernel.Async.Interest.priority source)
              in
              let* () = lift_async (Kernel.Async.Poll.deregister poll source)
              in
              let* () = lift_process (Kernel.Process.close process)
              in
              let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 533) process
              in
              if status = Kernel.Process.Exited 0 then
                Ok ()
              else Error "expected deregister-before-close process ownership to preserve exit observation"
          )
    )
              ...
              protect ~finally:(
                ...
                (
                  fun () ->
                    let* () = wait_for_priority_token poll ~token
                    in
                    let* first = lift_process (Kernel.Process.try_wait process)
                    in
                    let* second = lift_process (Kernel.Process.try_wait process)
                    in
    ...
    (
      fun process ->
        with_poll
          (
            fun poll ->
              let* () = lift_process (Kernel.Process.kill process ~signal:9)
              in
              let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 524) process
              in
              if status = Kernel.Process.Signaled 9 then
                Ok ()
              else Error "expected killed process to report a signaled status"
          )
    )
    ...
    (
      fun process ->
        with_poll
          (
            fun poll ->
              let* () = lift_process (Kernel.Process.kill process ~signal:15)
              in
              let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 525) process
              in
              if status = Kernel.Process.Signaled 15 then
                Ok ()
              else Error "expected sigterm to report the delivered signal number"
          )
    )
        ...
        with_poll
          (
            fun poll ->
              ...
              let* () = lift_process (Kernel.Process.close process)
              in
              let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 537) process
              in
              if status = Kernel.Process.Signaled 9 then
                Ok ()
              else Error "expected kill then close to preserve signaled exit observation"
          )
    )
        ...
        with_poll
          (
            fun poll ->
              ...
              let* () = lift_process (Kernel.Process.kill process ~signal:15)
              in
              let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 538) process
              in
              if status = Kernel.Process.Signaled 15 then
                Ok ()
              else Error "expected close then kill to preserve signaled exit observation"
          )
    )
    ...
    (
      fun process ->
        with_poll
          (
            fun poll ->
              let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 526) process
              in
              if status = Kernel.Process.Exited 7 then
                Ok ()
              else Error "expected process exit status to preserve a non-zero code"
          )
    )
...
let test_spawn_applies_custom_environment = fun _ctx ->
  ...
  let* process = lift_process (Kernel.Process.spawn ~program:"/bin/sh" ~args:[|
    "-c";
    "printf %s \"$KERNEL_NEW_PROCESS_TEST\"";
  |] ~env:[|"KERNEL_NEW_PROCESS_TEST", "env-ok"|] ~stdio ())
  in
  with_process process
    (
      fun process ->
        ...
        | Some stdout ->
            with_poll
              (
                fun poll ->
                  let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 527) process
                  in
                  let* payload = read_all poll ~token:(Kernel.Async.Token.make 505) stdout
                  in
                  if payload = "env-ok" && status = Kernel.Process.Exited 0 then
                    Ok ()
                  else Error "expected spawned process to see custom environment"
              )
    )
...
let test_spawn_applies_current_dir = fun _ctx ->
  with_tempdir "kernel_new_process"
    (
      fun tempdir ->
        let stdio = Kernel.Process.{ stdin = Stdin.Null; stdout = Stdout.Pipe; stderr = Stderr.Null } in
        let* process = lift_process (Kernel.Process.spawn ~program:"/bin/pwd" ~args:[||] ~current_dir:tempdir ~stdio ())
        in
        with_process process
          (
            fun process ->
              ...
              | Some stdout ->
                  with_poll
                    (
                      fun poll ->
                        let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 528) process
                        in
                        let* payload = read_all poll ~token:(Kernel.Async.Token.make 506) stdout
                        in
                        let output =
                          if String.length payload != 0 && String.get_unchecked payload ~at:(String.length payload - 1) = '\n' then
                            String.sub payload ~offset:0 ~len:(String.length payload - 1)
                          else payload
                        in
                        let* expected = lift_file (Kernel.Fs.File.canonicalize tempdir)
                        in
                        let* actual = lift_file (Kernel.Fs.File.canonicalize (Kernel.Path.from_string output))
                        in
                        if Kernel.Path.to_string actual = Kernel.Path.to_string expected && status = Kernel.Process.Exited 0 then
                          Ok ()
                        else Error "expected spawned process to run in the configured current_dir"
                    )
          )
    )
  ...
  with_tempdir "kernel_new_process"
    (
      fun tempdir ->
        ...
        let* input_file = lift_file (Kernel.Fs.File.open_write input_path)
        in
        let* () =
          with_file input_file
            (
              fun () ->
                let* _ = lift_file (Kernel.Fs.File.write input_file (Kernel.Bytes.from_string "file-stdio")) in Ok ()
            )
        in
        let* stdin_file = lift_file (Kernel.Fs.File.open_read input_path)
        in
        protect ~finally:(
          fun () ->
          ...
          (
            fun () ->
              let* stdout_file = lift_file (Kernel.Fs.File.open_write ~create:true ~truncate:true output_path)
              in
              protect ~finally:(
                fun () ->
                ...
                (
                  fun () ->
                    let stdio = Kernel.Process.{ stdin = Stdin.File stdin_file; stdout = Stdout.File stdout_file; stderr = Stderr.Null } in
                    let* process = lift_process (Kernel.Process.spawn ~program:"/bin/cat" ~args:[||] ~stdio ())
                    in
                    let* status =
                      with_process process
                    ...
                    in
                    let* output = lift_file (Kernel.Fs.File.open_read output_path)
                    in
                    ...
                    let* payload =
                      with_file output
                        (
                          fun () ->
                            let* count = lift_file (Kernel.Fs.File.read output buffer) in Ok (Kernel.Bytes.sub_string buffer ~offset:0 ~len:count)
                        )
                    in
                    if status = Kernel.Process.Exited 0 && payload = "file-stdio" then
                      Ok ()
                    else Error "expected file-backed stdio to roundtrip through the child process"
                )
          )
    )
            ...
            fun () ->
              ...
              protect ~finally:(
                ...
                (
                  fun () ->
                    let stdio = Kernel.Process.{ stdin = Stdin.File stdin_file; stdout = Stdout.File stdout_file; stderr = Stderr.Null } in
                    let* process = lift_process (Kernel.Process.spawn ~program:"/usr/bin/true" ~args:[||] ~stdio ())
                    in
                    with_process process
                      (
                        fun process ->
                          ...
                          in
                          if status = Kernel.Process.Exited 0 && Kernel.Process.stdin process = None && Kernel.Process.stdout process = None && Kernel.Process.stderr process = None then
                            Ok ()
                          else Error "expected file-backed stdio to avoid creating kernel pipe handles"
                      )
                )
          )
    )
...
let test_inherited_stdio_uses_no_kernel_pipes = fun _ctx ->
  let stdio = Kernel.Process.{ stdin = Stdin.Inherit; stdout = Stdout.Inherit; stderr = Stderr.Inherit } in
  let* process = lift_process (Kernel.Process.spawn ~program:"/usr/bin/true" ~args:[||] ~stdio ())
  in
  with_process process
    (
      fun process ->
        with_poll
          (
            fun poll ->
              let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 530) process
              in
              if status = Kernel.Process.Exited 0 && Kernel.Process.stdin process = None && Kernel.Process.stdout process = None && Kernel.Process.stderr process = None then
                Ok ()
              else Error "expected inherited stdio to avoid creating kernel pipes"
          )
    )
    ...
    (
      fun process ->
        with_poll
          (
            fun poll ->
              match Kernel.Process.stdin process, Kernel.Process.stdout process, Kernel.Process.stderr process with
              | (Some stdin, None, Some stderr) ->
                  let payload = Kernel.Bytes.from_string "pipes" in
                  let* () = write_all poll ~token:(Kernel.Async.Token.make 507) stdin payload
                  in
                  let* () = lift_file (Kernel.Fs.File.close stdin)
                  in
                  let* echoed = read_once poll ~token:(Kernel.Async.Token.make 508) stderr
                  in
                  let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 533) process
                  in
                  if echoed = "pipes" && status = Kernel.Process.Exited 0 then
                    Ok ()
                  else Error "expected pipe-backed stdio ownership to match the requested channels"
...
let test_spawn_missing_program_reports_no_such_file = fun _ctx ->
  ...
  | Kernel.Result.Ok process ->
      let _ = Kernel.Process.close process in Error "expected missing program spawn to fail"
...
let test_process_close_tolerates_preclosed_pipe_handles = fun _ctx ->
  ...
  | (Some stdin, Some stdout) ->
      let* () = lift_file (Kernel.Fs.File.close stdin)
      in
      let* () = lift_file (Kernel.Fs.File.close stdout)
      in
      let* () = lift_process (Kernel.Process.close process) in Ok ()
  | _ ->
      let _ = Kernel.Process.close process in Error "expected process pipes to exist"
    ...
    (
      fun process ->
        with_poll
          (
            fun poll ->
              let* first = wait_for_exit poll ~token:(Kernel.Async.Token.make 534) process
              in
              let* second = lift_process (Kernel.Process.try_wait process)
              in
              match second with
    ...
    (
      fun process ->
        with_poll
          (
            fun poll ->
              let* _ = wait_for_exit poll ~token:(Kernel.Async.Token.make 535) process
              in
          ...
          if remaining = 0 then
            ...
            let outcome =
              with_process process
                (
                  fun process ->
                    let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make (600 + remaining)) process
                    in
                    if status = Kernel.Process.Exited 0 then
                      Ok ()
                    else Error "expected repeated spawned process to exit cleanly"
                )
            in
            let* () = outcome in loop (remaining - 1)
        in
        loop 32
    )
      ...
      fun poll ->
        let rec spawn_many remaining acc =
          if remaining = 0 then
            ...
            let* process = lift_process (Kernel.Process.spawn ~program:"/bin/cat" ~args:[||] ~stdio ()) in spawn_many (remaining - 1) (process :: acc)
        in
        let* processes = spawn_many 12 []
        in
        let sources = List.map processes ~fn:Kernel.Process.to_source in
        let rec deregister_all = function
          | [] -> ()
          | source :: rest ->
              let _ = Kernel.Async.Poll.deregister poll source in deregister_all rest
        in
        protect ~finally:(
          fun () ->
            ...
            fun () ->
              let rec trigger_exit_burst = function
                ...
                | process :: rest -> (
                  ...
                  | Some stdin ->
                      let* () = lift_file (Kernel.Fs.File.close stdin) in trigger_exit_burst rest
                )
              in
              let rec register index = function
                | [] -> Ok ()
                | process :: rest ->
                    let* () = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make index) Kernel.Async.Interest.priority (Kernel.Process.to_source process)) in register (index + 1) rest
              in
                ...
                | process :: rest ->
                    if Kernel.Array.get_unchecked seen ~at:index then
                      ...
                      | Some (Kernel.Process.Exited 0) ->
                          ...
                          mark_observed (index + 1) rest
                      | Some _ -> Error "expected burst-exit process sources to preserve a clean exit status"
                      | None -> mark_observed (index + 1) rest
              in
              let rec poll_until attempts =
                let* () = mark_observed 0 processes
                in
                if all_seen 0 then
                  Ok ()
                else
              ...
              in
              let* () = register 0 processes
              in
              let* () = trigger_exit_burst processes
              in
              let* () = poll_until 16 in verify processes
          )
    )
        ...
        let rec spawn_many remaining acc =
          if remaining = 0 then
            ...
            let* process = lift_process (Kernel.Process.spawn ~program:"/bin/sh" ~args:[|
              "-c";
              "sleep 5";
            |] ~stdio ()) in spawn_many (remaining - 1) (process :: acc)
        in
        let* processes = spawn_many 12 []
        in
        let sources = List.map processes ~fn:Kernel.Process.to_source in
          ...
          (
            fun () ->
              let rec signal_all = function
                | [] -> Ok ()
                | process :: rest ->
                    let* () = lift_process (Kernel.Process.kill process ~signal:15) in signal_all rest
              in
              let rec register index = function
                | [] -> Ok ()
                | process :: rest ->
                ...
                | process :: rest ->
                    if Kernel.Array.get_unchecked seen ~at:index then
                      ...
                      | Some (Kernel.Process.Signaled 15) ->
                          Kernel.Array.set seen ~at:index ~value:true;
                          mark_observed (index + 1) rest
                      | Some _ -> Error "expected burst-signaled process sources to preserve a signaled status"
                      | None -> mark_observed (index + 1) rest
              in
              let rec poll_until attempts =
                let* () = mark_observed 0 processes
    ...
    (
      fun process ->
        ...
        with_poll
          (
            fun poll ->
              let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 910) process
              in
              let after = Kernel.Process.pid process in
              if before > 0 && before = after && status = Kernel.Process.Exited 0 then
                Ok ()
              else Error "expected Process.pid to be positive and stable across exit observation"
          )
    )
    ...
    (
      fun process ->
        with_poll
          (
            fun poll ->
              let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 911) process
              in
              if status = Kernel.Process.Exited 0 && Kernel.Process.stdin process = None && Kernel.Process.stdout process = None && Kernel.Process.stderr process = None then
                Ok ()
              else Error "expected all-null stdio to create no owned kernel pipes"
          )
    )
...
let test_stdin_null_delivers_immediate_eof_to_the_child = fun _ctx ->
  ...
  let* process = lift_process (Kernel.Process.spawn ~program:"/bin/sh" ~args:[|
    "-c";
    "if IFS= read -r line; then exit 1; else exit 0; fi";
  |] ~stdio ())
  in
  with_process process
    (
      fun process ->
        with_poll
          (
            fun poll ->
              let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 912) process
              in
              if status = Kernel.Process.Exited 0 then
                Ok ()
              else Error "expected Stdin.Null to present immediate eof to the child"
          )
    )
      ...
      fun process ->
        ...
        | (None, Some stderr) ->
            with_poll
              (
                fun poll ->
                  let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 913) process
                  in
                  let* payload = read_all poll ~token:(Kernel.Async.Token.make 914) stderr
                  in
                  if status = Kernel.Process.Exited 0 && payload = "err" then
                    Ok ()
                  else Error "expected Stdout.Null to discard stdout while leaving stderr capture intact"
              )
      ...
      fun process ->
        ...
        | (Some stdout, None) ->
            with_poll
              (
                fun poll ->
                  let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 915) process
                  in
                  let* payload = read_all poll ~token:(Kernel.Async.Token.make 916) stdout
                  in
                  if status = Kernel.Process.Exited 0 && payload = "out" then
                    Ok ()
                  else Error "expected Stderr.Null to discard stderr while leaving stdout capture intact"
              )
    ...
    (
      fun () ->
        ...
        | Kernel.Result.Ok () ->
            let* process = lift_process (Kernel.Process.spawn ~program:"/bin/sh" ~args:[|
              "-c";
              "test \"$RIOT_KERNEL_NEW_PROCESS_PARENT_VAR\" = present";
            |] ~env:[||] ~stdio ())
            in
            with_process process
              (
                fun process ->
                  with_poll
                    (
                      fun poll ->
                        let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 917) process
                        in
                        if status = Kernel.Process.Exited 0 then
                          Ok ()
                        else Error "expected env:[||] to keep unrelated parent variables visible"
                    )
              )
    )
...
let test_spawn_env_override_leaves_unrelated_parent_vars_intact = fun _ctx ->
  ...
  protect ~finally:(
    ...
    (
      fun () ->
        match Kernel.Env.set ~var:preserved ~value:"keep", Kernel.Env.set ~var:replaced ~value:"before" with
        | (Kernel.Result.Ok (), Kernel.Result.Ok ()) ->
            let* process = lift_process (Kernel.Process.spawn ~program:"/bin/sh" ~args:[|
              "-c";
              "test \"$RIOT_KERNEL_NEW_PROCESS_REPLACE\" = after && test \"$RIOT_KERNEL_NEW_PROCESS_KEEP\" = keep";
            |] ~env:[|replaced, "after"|] ~stdio ())
            in
            with_process process
              (
                fun process ->
                  with_poll
                    (
                      fun poll ->
                        let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 918) process
                        in
                        if status = Kernel.Process.Exited 0 then
                          Ok ()
                        else Error "expected env overrides to leave unrelated parent vars intact"
                    )
              )
      ...
      fun tempdir ->
        ...
        let* () =
          with_file input_file
            (
              fun () ->
                let* written = lift_file (Kernel.Fs.File.write input_file (Kernel.Bytes.from_string "012345"))
                in
                if written = 6 then
                  Ok ()
                else Error "expected stdin offset fixture write to make progress"
            )
        in
        let* stdin_file = lift_file (Kernel.Fs.File.open_read input_path)
        in
        protect ~finally:(
          ...
          (
            fun () ->
              ...
              let* consumed = lift_file (Kernel.Fs.File.read stdin_file scratch)
              in
              if consumed != 2 then
                Error "expected to advance the file-backed stdin handle before spawning"
              else
                let stdio = Kernel.Process.{ stdin = Stdin.File stdin_file; stdout = Stdout.Pipe; stderr = Stderr.Null } in
                let* process = lift_process (Kernel.Process.spawn ~program:"/bin/cat" ~args:[||] ~stdio ())
                in
                with_process process
                  (
                    fun process ->
                      ...
                      | Some stdout ->
                          with_poll
                            (
                              fun poll ->
                                let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make 919) process
                                in
                                let* payload = read_all poll ~token:(Kernel.Async.Token.make 920) stdout
                                in
                                if status = Kernel.Process.Exited 0 && payload = "2345" then
                                  Ok ()
                                else Error "expected file-backed stdin to honor the current file offset"
                            )
                  )
          )
    )
          ...
          (
            fun poll ->
              ...
              protect ~finally:(
                ...
                (
                  fun () ->
                    let* () = send_signal_command ~signal:"-STOP" process
                    in
                    let rec wait_for_stop attempts =
                      if attempts = 0 then
                        ...
                        | None ->
                            let* _ = lift_async (Kernel.Async.Poll.poll ~timeout:100_000_000L poll) in wait_for_stop (attempts - 1)
                    in
                    wait_for_stop 8
                )
          )
              ...
              protect ~finally:(
                ...
                (
                  fun () ->
                    ...
                    let* () = wait_for_stop 8
                    in
                    let* () = send_signal_command ~signal:"-CONT" process
                    in
...
let test_spawn_missing_current_dir_reports_no_such_file = fun _ctx ->
  let stdio = Kernel.Process.{ stdin = Stdin.Null; stdout = Stdout.Null; stderr = Stderr.Null } in
  match Kernel.Process.spawn ~program:"/usr/bin/true" ~args:[||] ~current_dir:(Kernel.Path.from_string "/definitely/missing/kernel-new-process-dir") ~stdio () with
  | Kernel.Result.Error (Kernel.Process.System Kernel.SystemError.NoSuchFileOrDirectory) -> Ok ()
  | Kernel.Result.Error error -> Error (Kernel.Process.error_to_string error)
  | Kernel.Result.Ok process ->
      let _ = Kernel.Process.close process in Error "expected spawn with a missing current_dir to fail"
  ...
  with_tempdir "kernel_new_process"
    (
      fun tempdir ->
        ...
        let* file = lift_file (Kernel.Fs.File.open_write file_path)
        in
        let* _ =
          with_file file
            (
        ...
        | Kernel.Result.Error error -> Error (Kernel.Process.error_to_string error)
        | Kernel.Result.Ok process ->
            let _ = Kernel.Process.close process in Error "expected spawn with a regular-file current_dir to report not_directory"
    )
...
let main ~args = Test.Cli.main ~execution_mode:Test.Cli.Linear ~name:"kernel_new_process_tests" ~tests ~args ()
