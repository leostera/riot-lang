...
let test_append_roundtrip = fun _ctx ->
  let buffer =
    Kernel.IO.Buffer.create ()
    |> Result.unwrap
  in
  let suffix =
    Kernel.IO.IoVec.IoSlice.create ~size:4
    |> Result.unwrap
  in
...
let test_consume_compacts_existing_storage = fun _ctx ->
  let buffer =
    Kernel.IO.Buffer.create ~size:4 ()
    |> Result.unwrap
  in
  let _ =
...
let test_growth_preserves_contents = fun _ctx ->
  ...
  in
  if
    Kernel.IO.Buffer.capacity buffer >= 8
    && String.equal (Kernel.IO.Buffer.to_string buffer) "abcdefgh"
  then
    Ok ()
  else
...
let test_writable_slice_commit_exposes_written_bytes = fun _ctx ->
  let buffer =
    Kernel.IO.Buffer.create ~size:8 ()
    |> Result.unwrap
  in
  let _ =
    Kernel.IO.Buffer.ensure_free buffer 5
    |> Result.unwrap
  in
  let writable = Kernel.IO.Buffer.writable buffer in
  write_string writable "hello";
  let _ =
    Kernel.IO.Buffer.commit buffer 5
    |> Result.unwrap
  in
...
let test_to_iovec_views_readable_slice = fun _ctx ->
  ...
  in
  let actual =
    Kernel.IO.Buffer.to_iovec buffer
...
let tests = [
  ...
  Test.case
    "Buffer writable_slice becomes readable after commit"
    test_writable_slice_commit_exposes_written_bytes;
