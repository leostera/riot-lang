...
let test_of_string_roundtrips_raw_text = fun _ctx ->
  ...
  if Kernel.String.equal (Kernel.Path.to_string (Kernel.Path.from_string raw)) raw then
    Ok ()
  else Error "expected Path.of_string and Path.to_string to roundtrip raw path text"

let test_join_treats_empty_sides_as_identity = fun _ctx ->
  let raw = "domains/admin" in
  if Kernel.String.equal (Kernel.Path.to_string (Kernel.Path.join "" raw)) raw && Kernel.String.equal (Kernel.Path.to_string (Kernel.Path.join raw "")) raw then
    Ok ()
  else Error "expected Path.join to treat empty sides as identity"
...
let test_join_avoids_duplicate_separators = fun _ctx ->
  ...
  if Kernel.String.equal value "domains/admin" then
    Ok ()
  else Error "expected Path.join to avoid inserting an extra separator"
...
let test_infix_join_matches_function = fun _ctx ->
  ...
  if Kernel.String.equal (Kernel.Path.to_string Kernel.Path.(left / right)) (Kernel.Path.to_string (Kernel.Path.join left right)) then
    Ok ()
  else Error "expected Path./ to match Path.join"
