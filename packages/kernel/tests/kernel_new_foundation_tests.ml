...
let test_bool_to_string_uses_stable_lowercase_literals = fun _ctx ->
  if Kernel.String.equal (Kernel.Bool.to_string true) "true" && Kernel.String.equal (Kernel.Bool.to_string false) "false" then
    Ok ()
  else Error "expected Bool.to_string to use stable lowercase literals"
...
let test_array_init_builds_in_index_order = fun _ctx ->
  ...
  let next = ref 0 in
  let built = Kernel.Array.init ~count:4 ~fn:(
    fun index ->
      Kernel.Array.set seen ~at:!next ~value:index;
      next := !next + 1;
      index * 2
  ) in
  if !next = 4 && Kernel.Array.get_unchecked seen ~at:0 = 0 && Kernel.Array.get_unchecked seen ~at:1 = 1 && Kernel.Array.get_unchecked seen ~at:2 = 2 && Kernel.Array.get_unchecked seen ~at:3 = 3 && Kernel.Array.get_unchecked built ~at:0 = 0 && Kernel.Array.get_unchecked built ~at:1 = 2 && Kernel.Array.get_unchecked built ~at:2 = 4 && Kernel.Array.get_unchecked built ~at:3 = 6 then
    Ok ()
  else Error "expected Array.init to visit each index once from left to right"

let test_option_map_leaves_none_unforced = fun _ctx ->
  let called = ref false in
  let value = Kernel.Option.map None ~fn:(
    fun _ ->
      called := true;
      1
  ) in
  if not !called && Kernel.Option.is_none value && Kernel.Option.unwrap_or value ~default:3 = 3 then
    Ok ()
  else Error "expected Option.map to leave None untouched and avoid calling its mapper"

let test_result_and_then_short_circuits_errors = fun _ctx ->
  let called = ref false in
  let value = Kernel.Result.and_then (Kernel.Result.Error "boom") ~fn:(
    fun _ ->
      called := true;
      Kernel.Result.Ok 1
  ) in
...
let test_bool_not_flips_both_branches = fun _ctx ->
  if Kernel.Bool.not true = false && Kernel.Bool.not false = true then
    Ok ()
  else Error "expected Bool.not to invert both boolean branches"
...
let test_char_unsafe_of_int_roundtrips_byte_boundaries = fun _ctx ->
  ...
  if Kernel.Char.to_int zero = 0 && Kernel.Char.to_int max_byte = 255 then
    Ok ()
  else Error "expected Char.unsafe_of_int to preserve byte-sized boundaries"
...
let test_int_to_string_matches_runtime_for_negative_values = fun _ctx ->
  let samples =
    [
      (-1), "-1";
      (-42), "-42";
      Kernel.Int.min_int, "-4611686018427387904";
    ]
  in
  let all_match = List.all samples ~fn:(
    fun (sample, expected) -> Kernel.Int.to_string sample = expected
  ) in
  if all_match then
    Ok ()
  else Error "expected Int.to_string to match the runtime decimal rendering for negative values"

let test_float_equal_and_compare_match_runtime_semantics = fun _ctx ->
  let nan = 0.0 /. 0.0 in
  let negative_zero = (-0.0) in
  let positive_zero = 0.0 in
  if Kernel.Float.equal nan nan = ((=) nan nan) && Kernel.Float.equal negative_zero positive_zero = ((=) negative_zero positive_zero) && Kernel.Float.compare nan nan = compare nan nan && Kernel.Float.compare nan positive_zero = compare nan positive_zero && Kernel.Float.compare negative_zero positive_zero = compare negative_zero positive_zero then
    Ok ()
  else Error "expected Kernel.Float to preserve the raw runtime equality and ordering semantics"
...
let test_array_make_aliases_mutable_payloads = fun _ctx ->
  ...
  Kernel.Bytes.set_unchecked (Kernel.Array.get_unchecked values ~at:1) ~at:0 ~char:'b';
  if Kernel.Bytes.to_string (Kernel.Array.get_unchecked values ~at:0) = "ba" && Kernel.Bytes.to_string (Kernel.Array.get_unchecked values ~at:2) = "ba" then
    Ok ()
  else Error "expected Array.make to alias the repeated mutable payload"

let test_option_none_helpers_are_exact_complements = fun _ctx ->
  let value = None in
  if not (Kernel.Option.is_some value) && Kernel.Option.is_none value && Kernel.Option.unwrap_or value ~default:7 = 7 then
    Ok ()
  else Error "expected Option None helpers to stay complementary and use the fallback"

let test_string_make_fills_every_slot = fun _ctx ->
  let value = Kernel.String.make ~len:4 ~char:'x' in
  if Kernel.String.length value = 4 && Kernel.String.get_unchecked value ~at:0 = 'x' && Kernel.String.get_unchecked value ~at:1 = 'x' && Kernel.String.get_unchecked value ~at:2 = 'x' && Kernel.String.get_unchecked value ~at:3 = 'x' then
    Ok ()
  else Error "expected String.make to fill every character slot"

let test_string_append_preserves_embedded_nul_bytes = fun _ctx ->
  let value = Kernel.String.append "a\000" "b" in
  if Kernel.String.length value = 3 && Kernel.String.get_unchecked value ~at:0 = 'a' && Kernel.String.get_unchecked value ~at:1 = '\000' && Kernel.String.get_unchecked value ~at:2 = 'b' then
    Ok ()
  else Error "expected String.append to preserve embedded nul bytes"
...
let test_bytes_fill_only_touches_requested_window = fun _ctx ->
  ...
  if Kernel.Bytes.to_string value = "abXXef" then
    Ok ()
  else Error "expected Bytes.fill to leave bytes outside the requested window unchanged"
...
let test_bytes_blit_handles_overlapping_ranges = fun _ctx ->
  ...
  if Kernel.Bytes.to_string value = "ababcd" then
    Ok ()
  else Error "expected Bytes.blit to behave like memmove on overlapping ranges"
...
let test_bytes_sub_string_zero_length_is_empty = fun _ctx ->
  if Kernel.Bytes.sub_string (Kernel.Bytes.from_string "riot") ~offset:2 ~len:0 = "" then
    Ok ()
  else Error "expected Bytes.sub_string with len=0 to return an empty string"

let test_path_join_preserves_root_sanity = fun _ctx ->
  if Kernel.Path.to_string (Kernel.Path.join "/" "tmp") = "/tmp" then
    Ok ()
  else Error "expected Path.join to keep the root separator sane"

let test_path_join_does_not_duplicate_separators = fun _ctx ->
  if Kernel.Path.to_string (Kernel.Path.join "a" "/b") = "a/b" then
    Ok ()
  else Error "expected Path.join to avoid duplicate separators when the right side already starts with one"
...
let test_iovec_with_capacity_matches_create_count_one = fun _ctx ->
  ...
  let count_segments iov =
    let seen = ref 0 in
    Kernel.IO.IoVec.for_each iov ~fn:(
      fun _ -> seen := !seen + 1
    );
    !seen
  in
  if Kernel.IO.IoVec.length left = Kernel.IO.IoVec.length right && count_segments left = 1 && count_segments right = 1 then
    Ok ()
  else Error "expected IoVec.with_capacity to match a single-segment create"
...
let test_iovec_create_distributes_remainder_deterministically = fun _ctx ->
  ...
  let lengths = ref [] in
  Kernel.IO.IoVec.for_each iov ~fn:(
    fun segment -> lengths := Kernel.IO.IoVec.IoSlice.length segment :: !lengths
  );
  if List.reverse !lengths = [ 2; 2; 1 ] then
    Ok ()
  else Error "expected IoVec.create to distribute remainder bytes from left to right"
...
let test_iovec_of_bytes_array_copies_source_buffers = fun _ctx ->
  ...
  if Kernel.IO.IoVec.to_string iov = "riot" then
    Ok ()
  else Error "expected IoVec.of_bytes_array to copy source buffers into owned storage"
...
let test_iovec_into_bytes_returns_a_fresh_copy = fun _ctx ->
  ...
  if Kernel.Bytes.to_string flattened = "riot" then
    Ok ()
  else Error "expected IoVec.into_bytes to return a fresh stable copy"
...
let test_iovec_iter_reports_left_to_right_segment_metadata = fun _ctx ->
  ...
  Kernel.IO.IoVec.for_each iov ~fn:(
    fun segment ->
      let len = Kernel.IO.IoVec.IoSlice.length segment in seen := (len, Kernel.IO.IoVec.IoSlice.to_string segment) :: !seen
  );
  if List.reverse !seen = [
    2, "ab";
    0, "";
    3, "cde";
  ] then
    Ok ()
  else Error "expected IoVec.for_each to preserve segment order and metadata"
...
let test_iovec_sub_zero_length_is_empty = fun _ctx ->
  ...
  if Kernel.IO.IoVec.length sub = 0 then
    Ok ()
  else Error "expected IoVec.sub with len=0 to return an empty iovec"
