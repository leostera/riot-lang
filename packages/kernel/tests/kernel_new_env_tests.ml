...
let test_invalid_var_name_is_rejected = fun _ctx ->
  ...
  | (Kernel.Result.Error (Kernel.Env.InvalidVarName { name = "bad=name" }), Kernel.Result.Error (
    Kernel.Env.InvalidVarName { name = "" }
  )) -> Ok ()
...
let test_set_var_overwrites_existing_value = fun _ctx ->
  ...
  protect
    ...
    (fun () ->
      ...
      let snapshot = Kernel.Env.vars () in
      if
        Kernel.Env.get ~var:name = Some "after" && vars_contain snapshot ~name ~value:"after" ()
      then
        Ok ()
      else
...
let tests = [
  ...
  Test.case
    "Env missing vars and home_dir reflect the process environment"
    test_missing_var_and_home_dir_behave_as_expected;
  ...
  Test.case
    "Env.remove_var on a missing name is harmless"
    test_remove_var_on_missing_name_is_harmless;
  Test.case "Env.set_var overwrites an existing value" test_set_var_overwrites_existing_value;
  Test.case
    "Env.set_current_dir rejects missing paths cleanly"
    test_set_current_dir_missing_path_fails_cleanly;
  Test.case
    "Env.set_current_dir rejects regular files cleanly"
    test_set_current_dir_regular_file_fails_cleanly;
]
