...
let vars_contain = fun entries ~name ?value () -> Kernel.Array.fold_left entries ~acc:false ~fn:(
...
let test_args_include_program_name = fun _ctx ->
  if Kernel.Array.length Kernel.Env.args > 0 then
    Ok ()
  else Error "expected kernel env args to include at least the program name"
...
let test_set_get_and_remove_var_roundtrip = fun _ctx ->
  ...
  protect ~finally:(
    ...
    (
      fun () ->
        let* () = lift (Kernel.Env.set ~var:name ~value:"kernel-new")
        in
        let value = Kernel.Env.get ~var:name in
        let found = Kernel.Array.fold_left (Kernel.Env.vars ()) ~acc:false ~fn:(
          fun found (entry_name, entry_value) -> found || (Kernel.String.equal entry_name name && Kernel.String.equal entry_value "kernel-new")
        ) in
        let* () = lift (Kernel.Env.remove ~var:name)
        in
        if value = Some "kernel-new" && found && Kernel.Env.get ~var:name = None then
          Ok ()
        else Error "expected kernel env variable roundtrip to preserve value and cleanup"
    )
...
let test_missing_var_and_home_dir_behave_as_expected = fun _ctx ->
  ...
  if missing && home_matches then
    Ok ()
  else Error "expected missing vars and home_dir to reflect the process environment"
...
let test_vars_snapshots_are_independent = fun _ctx ->
  ...
  protect ~finally:(
    ...
    (
      fun () ->
        let* () = lift (Kernel.Env.set ~var:name ~value:"before")
        in
        let before = Kernel.Env.vars () in
        let* () = lift (Kernel.Env.set ~var:name ~value:"after")
        in
        let after = Kernel.Env.vars () in
        if vars_contain before ~name ~value:"before" () && not (vars_contain before ~name ~value:"after" ()) && vars_contain after ~name ~value:"after" () then
          Ok ()
        else Error "expected env snapshots to preserve the values visible at each call site"
    )

let test_current_dir_roundtrips = fun _ctx ->
  let* original = lift (Kernel.Env.current_dir ())
  in
  match Fs.with_tempdir ~prefix:"kernel_new_env"
    (
      fun tempdir ->
        ...
        protect ~finally:(
          ...
          (
            fun () ->
              let* () = lift (Kernel.Env.set_current_dir tempdir)
              in
              let* current = lift (Kernel.Env.current_dir ())
              in
...
let test_invalid_var_name_is_rejected = fun _ctx ->
  ...
  | (Kernel.Result.Error (Kernel.Env.InvalidVarName { name = "bad=name" }), Kernel.Result.Error (Kernel.Env.InvalidVarName { name = "" })) -> Ok ()
...
let test_set_var_accepts_equals_in_values = fun _ctx ->
  ...
  protect ~finally:(
    ...
    (
      fun () ->
        let* () = lift (Kernel.Env.set ~var:name ~value:"a=b=c")
        in
...
let test_remove_var_on_missing_name_is_harmless = fun _ctx ->
  ...
  if Kernel.Env.get ~var:name = None then
    Ok ()
  else Error "expected removing a missing environment variable to be harmless"
    ...
    (
      fun () ->
        ...
        if Kernel.Env.get ~var:name = Some "after" && vars_contain snapshot ~name ~value:"after" () then
          Ok ()
        else Error "expected Env.set_var to overwrite the existing value"
    )
  ...
  with_tempdir "kernel_new_env_edge"
    (
      fun root ->
        ...
        match Kernel.Fs.File.open_write path |> function
...
let main ~args = Test.Cli.main ~execution_mode:Test.Cli.Linear ~name:"kernel_new_env_tests" ~tests ~args ()
