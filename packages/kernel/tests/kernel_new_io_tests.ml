...
let with_poll = fun fn ->
  ...
  | Kernel.Result.Ok poll -> (
    try
      ...
      let _ = Kernel.Async.Poll.close poll in value
    with
    | error ->
        let _ = Kernel.Async.Poll.close poll in raise error
  )
...
let test_stdin_read_len_zero_noop = fun _ctx ->
  ...
  let* read = lift_stdin (Kernel.IO.Stdin.read ~len:0 buffer)
  in
  if read = 0 then
    Ok ()
  else Error "expected stdin.read with len=0 to return 0"

let test_stdout_write_len_zero_noop = fun _ctx ->
  let buffer = Kernel.Bytes.create ~size:16 in
  let* written = lift_stdout (Kernel.IO.Stdout.write ~len:0 buffer)
  in
  if written = 0 then
    Ok ()
  else Error "expected stdout.write with len=0 to return 0"

let test_stderr_write_len_zero_noop = fun _ctx ->
  let buffer = Kernel.Bytes.create ~size:16 in
  let* written = lift_stderr (Kernel.IO.Stderr.write ~len:0 buffer)
  in
  if written = 0 then
    Ok ()
  else Error "expected stderr.write with len=0 to return 0"

let test_stdin_read_rejects_invalid_slice = fun _ctx ->
  let buffer = Kernel.Bytes.create ~size:16 in assert_invalid_slice ~error_to_string:Kernel.IO.Stdin.error_to_string ~is_invalid_slice:(
    fun value ->
      match value with
...
let test_stdout_write_rejects_invalid_slice = fun _ctx ->
  let buffer = Kernel.Bytes.create ~size:16 in assert_invalid_slice ~error_to_string:Kernel.IO.Stdout.error_to_string ~is_invalid_slice:(
    fun value ->
      match value with
...
let test_stderr_write_rejects_invalid_slice = fun _ctx ->
  let buffer = Kernel.Bytes.create ~size:16 in assert_invalid_slice ~error_to_string:Kernel.IO.Stderr.error_to_string ~is_invalid_slice:(
    fun value ->
      match value with
...
let test_stdin_read_vectored_len_zero_noop = fun _ctx ->
  ...
  let* read = lift_stdin (Kernel.IO.Stdin.read_vectored iovec)
  in
  if read = 0 then
    Ok ()
  else Error "expected stdin.read_vectored with empty segment to return 0"

let test_stdout_write_vectored_len_zero_noop = fun _ctx ->
  let iovec = Kernel.IO.IoVec.from_bytes_array [|Kernel.Bytes.create ~size:0|] |> Result.unwrap in
  let* written = lift_stdout (Kernel.IO.Stdout.write_vectored iovec)
  in
  if written = 0 then
    Ok ()
  else Error "expected stdout.write_vectored with empty segment to return 0"

let test_stderr_write_vectored_len_zero_noop = fun _ctx ->
  let iovec = Kernel.IO.IoVec.from_bytes_array [|Kernel.Bytes.create ~size:0|] |> Result.unwrap in
  let* written = lift_stderr (Kernel.IO.Stderr.write_vectored iovec)
  in
  if written = 0 then
    Ok ()
  else Error "expected stderr.write_vectored with empty segment to return 0"
  ...
  with_poll
    (
      fun poll ->
        ...
        | Kernel.Result.Ok () ->
            let* _events = lift_async (Kernel.Async.Poll.poll ~timeout:0L poll)
            in
            let* () = lift_async (Kernel.Async.Poll.deregister poll source) in Ok ()
...
let tests =
  [
    ...
    Test.case "Kernel.IO.Stdout.write_vectored len=0 is a no-op" test_stdout_write_vectored_len_zero_noop;
    Test.case "Kernel.IO.Stderr.write_vectored len=0 is a no-op" test_stderr_write_vectored_len_zero_noop;
    Test.case "Kernel.IO.Stdin source register/deregister roundtrips" test_stdin_source_register_and_deregister;
    Test.case "Kernel.IO.Stdout source register/deregister roundtrips" test_stdout_source_register_and_deregister;
