...
let string_of_tcp_listener_error = fun error -> Kernel.Error.to_string (Kernel.Error.from_net_tcp_listener error)

let string_of_tcp_stream_error = fun error -> Kernel.Error.to_string (Kernel.Error.from_net_tcp_stream error)
...
let with_poll = fun fn ->
  let* poll = lift_async (Kernel.Async.Poll.make ())
  in
  protect ~finally:(
    fun () ->
...
let wait_for = fun poll ~token ~interest ~source ~pred ->
  let* () = lift_async (Kernel.Async.Poll.register poll token interest source)
  in
  protect ~finally:(
    fun () ->
    ...
    (
      fun () ->
        let* events = lift_async (Kernel.Async.Poll.poll ~timeout:100_000_000L poll)
        in
        let found = List.any events ~fn:(
          fun event -> Kernel.Async.Token.equal token (Kernel.Async.Event.token event) && pred event
        ) in
        if found then
          Ok ()
        else Error "expected readiness event"
    )

let wait_readable = fun poll ~token source -> wait_for poll ~token ~interest:Kernel.Async.Interest.readable ~source ~pred:Kernel.Async.Event.is_readable

let wait_writable = fun poll ~token source -> wait_for poll ~token ~interest:Kernel.Async.Interest.writable ~source ~pred:Kernel.Async.Event.is_writable

let has_readable_token = fun token events -> List.any events ~fn:(
  fun event -> Kernel.Async.Event.is_readable event && Kernel.Async.Token.equal token (Kernel.Async.Event.token event)
)
...
let rec write_all_stream = fun poll ~token stream buffer ~pos ~len ->
  if len = 0 then
    ...
    | Kernel.Result.Ok written ->
        ...
        else write_all_stream poll ~token stream buffer ~pos:(pos + written) ~len:(len - written)
    | Kernel.Result.Error error ->
        if is_tcp_stream_would_block error then
          let* () = wait_writable poll ~token (Kernel.Net.TcpStream.to_source stream) in write_all_stream poll ~token stream buffer ~pos ~len
        else Error (string_of_tcp_stream_error error)
...
let rec write_all_vectored = fun poll ~token stream iov ~pos ~len ->
  if len = 0 then
    ...
    | Kernel.Result.Ok written ->
        ...
        else write_all_vectored poll ~token stream iov ~pos:(pos + written) ~len:(len - written)
    | Kernel.Result.Error error ->
        if is_tcp_stream_would_block error then
          let* () = wait_writable poll ~token (Kernel.Net.TcpStream.to_source stream) in write_all_vectored poll ~token stream iov ~pos ~len
        else Error (string_of_tcp_stream_error error)
...
let rec read_exact_stream = fun poll ~token stream buffer ~pos ~len ->
  if len = 0 then
    ...
    | Kernel.Result.Ok read ->
        ...
        else read_exact_stream poll ~token stream buffer ~pos:(pos + read) ~len:(len - read)
    | Kernel.Result.Error error ->
        if is_tcp_stream_would_block error then
          let* () = wait_readable poll ~token (Kernel.Net.TcpStream.to_source stream) in read_exact_stream poll ~token stream buffer ~pos ~len
        else Error (string_of_tcp_stream_error error)
...
let rec read_exact_vectored = fun poll ~token stream iov ~pos ~len ->
  if len = 0 then
    ...
    | Kernel.Result.Ok read ->
        ...
        else read_exact_vectored poll ~token stream iov ~pos:(pos + read) ~len:(len - read)
    | Kernel.Result.Error error ->
        if is_tcp_stream_would_block error then
          let* () = wait_readable poll ~token (Kernel.Net.TcpStream.to_source stream) in read_exact_vectored poll ~token stream iov ~pos ~len
        else Error (string_of_tcp_stream_error error)
...
let rec accept_stream = fun poll listener ->
  ...
  | Kernel.Result.Error error ->
      if is_tcp_listener_would_block error then
        let* () = wait_readable poll ~token:(Kernel.Async.Token.make 301) (Kernel.Net.TcpListener.to_source listener) in accept_stream poll listener
      else Error (string_of_tcp_listener_error error)

let connect_stream = fun poll addr ->
  let* connect_result = lift_tcp_stream (Kernel.Net.TcpStream.connect addr)
  in
  ...
  | Kernel.Net.TcpStream.InProgress stream ->
      ...
      let rec finish attempts =
        if attempts = 0 then
          ...
          let* () = wait_writable poll ~token (Kernel.Net.TcpStream.to_source stream)
          in
...
let with_tcp_pair_at = fun listener_addr fn ->
  with_poll
    (
      fun poll ->
        let* listener = lift_tcp_listener (Kernel.Net.TcpListener.bind listener_addr)
        in
        protect ~finally:(
          fun () -> close_listener listener
        )
          (
            fun () ->
              let* bound_addr = lift_tcp_listener (Kernel.Net.TcpListener.local_addr listener)
              in
              let* client = connect_stream poll bound_addr
              in
              protect ~finally:(
                fun () -> close_stream client
              )
                (
                  fun () ->
                    let* (server, peer) = accept_stream poll listener
                    in
                    protect ~finally:(
                      fun () -> close_stream server
                    )
                      (
...
let wait_udp_readable = fun poll ~token socket -> wait_readable poll ~token (Kernel.Net.UdpSocket.to_source socket)
...
let recv_from_udp = fun poll ~token socket buffer ->
  let rec loop () =
    ...
    | Kernel.Result.Error error ->
        ...
        else Error (string_of_udp_error error)
  in
  loop ()
...
let with_udp_pair_at = fun bind_addr fn ->
  with_poll
    (
      fun poll ->
        let* server = lift_udp (Kernel.Net.UdpSocket.bind bind_addr)
        in
        protect ~finally:(
          fun () -> close_udp server
        )
          (
            fun () ->
              let* client = lift_udp (Kernel.Net.UdpSocket.bind bind_addr)
              in
              protect ~finally:(
                fun () -> close_udp client
              )
                (
                  fun () ->
                    let* server_addr = lift_udp (Kernel.Net.UdpSocket.local_addr server)
                    in
                    let* client_addr = lift_udp (Kernel.Net.UdpSocket.local_addr client) in fn ~poll ~server ~server_addr ~client ~client_addr
                )
          )
    )
  ...
  with_udp_pair
    (
      fun ~poll ~server ~server_addr ~client ~client_addr ->
        let* () = lift_udp (Kernel.Net.UdpSocket.connect server client_addr)
        in
        let* () = lift_udp (Kernel.Net.UdpSocket.connect client server_addr) in fn ~poll ~server ~server_addr ~client ~client_addr
    )

let test_ip_addr_validates_ipv4_and_ipv6 = fun _ctx ->
  match Kernel.Net.IpAddr.from_string "127.0.0.1", Kernel.Net.IpAddr.from_string "::1", Kernel.Net.IpAddr.from_string "nope" with
  | (Kernel.Result.Ok ipv4, Kernel.Result.Ok ipv6, Kernel.Result.Error (Kernel.Net.IpAddr.InvalidText _)) ->
      if Kernel.String.equal (Kernel.Net.IpAddr.to_string ipv4) "127.0.0.1" && Kernel.String.equal (Kernel.Net.IpAddr.to_string ipv6) "::1" then
        Ok ()
      else Error "expected ip parsing to preserve canonical loopback forms"
  ...
  with_tcp_pair
    (
      fun ~poll ~listener:_ ~listener_addr ~client ~server ~peer ->
        let* client_local = lift_tcp_stream (Kernel.Net.TcpStream.local_addr client)
        in
        let* client_peer = lift_tcp_stream (Kernel.Net.TcpStream.peer_addr client)
        in
        let* server_local = lift_tcp_stream (Kernel.Net.TcpStream.local_addr server)
        in
        let* server_peer = lift_tcp_stream (Kernel.Net.TcpStream.peer_addr server)
        in
        ...
        if client_peer_port != listener_port || server_local_port != listener_port || accepted_peer_port != client_local_port || server_peer_port != client_local_port || client_peer_ip != listener_ip || server_local_ip != listener_ip || server_peer_ip != client_local_ip then
          ...
          let pong =
            Kernel.IO.IoVec.from_string_array
              ...
              |] |> Result.unwrap
          in
          let* () = write_all_stream poll ~token:(Kernel.Async.Token.make 303) client ping ~pos:0 ~len:(Kernel.Bytes.length ping)
          in
          let server_buf = Kernel.Bytes.create ~size:4 in
          let* () = read_exact_stream poll ~token:(Kernel.Async.Token.make 304) server server_buf ~pos:0 ~len:4
          in
          let* () = write_all_vectored poll ~token:(Kernel.Async.Token.make 305) server pong ~pos:0 ~len:(Kernel.IO.IoVec.length pong)
          in
          let client_buf = Kernel.IO.IoVec.create ~count:2 ~size:4 () |> Result.unwrap in
          let* () = read_exact_vectored poll ~token:(Kernel.Async.Token.make 306) client client_buf ~pos:0 ~len:4
          in
          if Kernel.String.equal (Kernel.Bytes.sub_string server_buf ~offset:0 ~len:4) "ping" && Kernel.String.equal (Kernel.IO.IoVec.to_string client_buf) "pong" then
            Ok ()
          else Error "expected tcp loopback roundtrip to preserve scalar and vectored payloads"
    )
      ...
      fun ~poll ~listener:_ ~listener_addr:_ ~client ~server ~peer:_ ->
        ...
        let rec write_many remaining =
          if remaining = 0 then
            ...
            let* () = write_all_vectored poll ~token:(Kernel.Async.Token.make ("tcp-vburst-write", remaining)) client outbound ~pos:0 ~len:total in write_many (remaining - 1)
        in
        let rec read_many remaining acc =
          if remaining = 0 then
            Ok (Kernel.String.concat "" (List.reverse acc))
          else
            let* () = read_exact_stream poll ~token:(Kernel.Async.Token.make ("tcp-vburst-read", remaining)) server inbound ~pos:0 ~len:total in read_many (remaining - 1) (Kernel.Bytes.sub_string inbound ~offset:0 ~len:total :: acc)
        in
        let* () = write_many 8
        in
        let* actual = read_many 8 []
        in
        if actual = Kernel.String.concat "" (List.init ~count:8 ~fn:(
          fun _ -> payload
        )) then
          Ok ()
        else Error "expected repeated small vectored tcp writes to preserve segment order"
    )
...
let test_tcp_stream_reports_eof_after_peer_close = fun _ctx ->
  with_tcp_pair
    (
      fun ~poll ~listener:_ ~listener_addr:_ ~client ~server ~peer:_ ->
        let* () = lift_tcp_stream (Kernel.Net.TcpStream.close server)
        in
        let* () = wait_readable poll ~token:(Kernel.Async.Token.make 309) (Kernel.Net.TcpStream.to_source client)
        in
        let buffer = Kernel.Bytes.create ~size:8 in
        let* read = lift_tcp_stream (Kernel.Net.TcpStream.read client buffer)
        in
        if read = 0 then
          Ok ()
        else Error "expected tcp stream read to report eof after peer close"
    )
...
let test_tcp_stream_shutdown_write_reports_eof_to_peer = fun _ctx ->
  with_tcp_pair
    (
      fun ~poll ~listener:_ ~listener_addr:_ ~client ~server ~peer:_ ->
        let* () = lift_tcp_stream (Kernel.Net.TcpStream.shutdown client Kernel.Net.TcpStream.Write)
        in
        let* () = wait_readable poll ~token:(Kernel.Async.Token.make 312) (Kernel.Net.TcpStream.to_source server)
        in
        let buffer = Kernel.Bytes.create ~size:8 in
        let* read = lift_tcp_stream (Kernel.Net.TcpStream.read server buffer)
        in
        if read = 0 then
          Ok ()
        else Error "expected tcp write shutdown to surface eof to the peer"
    )
...
let test_tcp_stream_shutdown_read_preserves_write_half = fun _ctx ->
  with_tcp_pair
    (
      fun ~poll ~listener:_ ~listener_addr:_ ~client ~server ~peer:_ ->
        let* () = lift_tcp_stream (Kernel.Net.TcpStream.shutdown client Kernel.Net.TcpStream.Read)
        in
        let payload = Kernel.Bytes.from_string "ping" in
        let* () = write_all_stream poll ~token:(Kernel.Async.Token.make 314) client payload ~pos:0 ~len:(Kernel.Bytes.length payload)
        in
        let buffer = Kernel.Bytes.create ~size:4 in
        let* () = read_exact_stream poll ~token:(Kernel.Async.Token.make 315) server buffer ~pos:0 ~len:4
        in
        if Kernel.String.equal (Kernel.Bytes.sub_string buffer ~offset:0 ~len:4) "ping" then
          Ok ()
        else Error "expected read-shutdown tcp stream to preserve the write half"
    )
...
let test_tcp_stream_shutdown_read_write_disables_both_halves = fun _ctx ->
  with_tcp_pair
    (
      fun ~poll ~listener:_ ~listener_addr:_ ~client ~server ~peer:_ ->
        let* () = lift_tcp_stream (Kernel.Net.TcpStream.shutdown client Kernel.Net.TcpStream.ReadWrite)
        in
        let* () = wait_readable poll ~token:(Kernel.Async.Token.make 318) (Kernel.Net.TcpStream.to_source server)
        in
        let buffer = Kernel.Bytes.create ~size:8 in
        let* read = lift_tcp_stream (Kernel.Net.TcpStream.read server buffer)
        in
...
let test_tcp_stream_peer_write_shutdown_preserves_local_write_half = fun _ctx ->
  with_tcp_pair
    (
      fun ~poll ~listener:_ ~listener_addr:_ ~client ~server ~peer:_ ->
        let* () = lift_tcp_stream (Kernel.Net.TcpStream.shutdown server Kernel.Net.TcpStream.Write)
        in
        let* () = wait_readable poll ~token:(Kernel.Async.Token.make 319) (Kernel.Net.TcpStream.to_source client)
        in
        let eof_buffer = Kernel.Bytes.create ~size:8 in
        let* eof_read = lift_tcp_stream (Kernel.Net.TcpStream.read client eof_buffer)
        in
        if eof_read != 0 then
          Error "expected peer write shutdown to surface eof to the local reader"
        else
          let payload = Kernel.Bytes.from_string "pong" in
          let* () = write_all_stream poll ~token:(Kernel.Async.Token.make 320) client payload ~pos:0 ~len:(Kernel.Bytes.length payload)
          in
          let buffer = Kernel.Bytes.create ~size:4 in
          let* () = read_exact_stream poll ~token:(Kernel.Async.Token.make 321) server buffer ~pos:0 ~len:4
          in
          if Kernel.String.equal (Kernel.Bytes.sub_string buffer ~offset:0 ~len:4) "pong" then
            Ok ()
          else Error "expected peer write shutdown to preserve the local write half"
    )
  ...
  with_tcp_pair
    (
      fun ~poll:_ ~listener:_ ~listener_addr:_ ~client ~server ~peer:_ ->
        let* () = lift_tcp_stream (Kernel.Net.TcpStream.finish_connect client)
        in
        let* () = lift_tcp_stream (Kernel.Net.TcpStream.finish_connect client)
        in
        let* () = lift_tcp_stream (Kernel.Net.TcpStream.finish_connect server) in Ok ()
    )

let test_tcp_listener_ipv6_local_addr_roundtrips = fun _ctx ->
  let* listener = lift_tcp_listener (Kernel.Net.TcpListener.bind (Kernel.Net.SocketAddr.loopback_v6 ~port:0))
  in
  protect ~finally:(
    fun () -> close_listener listener
  )
    (
      fun () ->
        let* addr = lift_tcp_listener (Kernel.Net.TcpListener.local_addr listener)
        in
        if Kernel.Net.SocketAddr.port addr > 0 && Kernel.Net.IpAddr.to_string (Kernel.Net.SocketAddr.ip addr) = "::1" then
          Ok ()
        else Error "expected ipv6 tcp listener to preserve the loopback address"
    )
  ...
  with_tcp_pair_at (Kernel.Net.SocketAddr.loopback_v6 ~port:0)
    (
      fun ~poll ~listener:_ ~listener_addr ~client ~server ~peer ->
        ...
        in
        let client_peer_ip = Kernel.Net.IpAddr.to_string (Kernel.Net.SocketAddr.ip client_peer) in
        if not (Kernel.String.equal listener_ip "::1") || not (Kernel.String.equal accepted_peer_ip "::1") || not (Kernel.String.equal client_peer_ip "::1") then
          Error "expected ipv6 tcp loopback peers to preserve the loopback address"
        else
          let payload = Kernel.Bytes.from_string "ipv6" in
          let* () = write_all_stream poll ~token:(Kernel.Async.Token.make 316) client payload ~pos:0 ~len:(Kernel.Bytes.length payload)
          in
          let buffer = Kernel.Bytes.create ~size:4 in
          let* () = read_exact_stream poll ~token:(Kernel.Async.Token.make 317) server buffer ~pos:0 ~len:4
          in
          if Kernel.String.equal (Kernel.Bytes.sub_string buffer ~offset:0 ~len:4) "ipv6" then
            Ok ()
          else Error "expected ipv6 tcp loopback to preserve payload"
    )

let test_tcp_listener_bind_rejects_in_use_address = fun _ctx ->
  let* listener = lift_tcp_listener (Kernel.Net.TcpListener.bind (Kernel.Net.SocketAddr.loopback_v4 ~port:0))
  in
  protect ~finally:(
    fun () -> close_listener listener
  )
    (
      fun () ->
        ...
        | Kernel.Result.Ok extra ->
            let _ = Kernel.Net.TcpListener.close extra in Error "expected second tcp listener bind to fail on the same address"
...
let test_tcp_listener_accept_after_close_reports_bad_file_descriptor = fun _ctx ->
  let* listener = lift_tcp_listener (Kernel.Net.TcpListener.bind (Kernel.Net.SocketAddr.loopback_v4 ~port:0))
  in
  let* () = lift_tcp_listener (Kernel.Net.TcpListener.close listener)
  in
  match Kernel.Net.TcpListener.accept listener with
        ...
        protect ~finally:(
          ...
          (
            fun () ->
              ...
              let* () = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make "listener-close-then-deregister") Kernel.Async.Interest.readable source)
              in
              let* () = lift_tcp_listener (Kernel.Net.TcpListener.close listener)
              in
              let* () = lift_async (Kernel.Async.Poll.deregister poll source)
              in
              let* _ = lift_async (Kernel.Async.Poll.poll ~timeout:0L poll) in Ok ()
          )
    )
        ...
        protect ~finally:(
          ...
          (
            fun () ->
              ...
              in
              let* clients = connect_many 16 []
              in
              protect ~finally:(
                fun () -> close_streams clients
              )
                (
                  fun () ->
                    let rec accept_many remaining acc =
                      if remaining = 0 then
                        ...
                        let* (server, _) = accept_stream poll listener in accept_many (remaining - 1) (server :: acc)
                    in
                    let* servers = accept_many 16 []
                    in
                    protect ~finally:(
                      fun () -> close_streams servers
                    )
                      (
                        fun () ->
                          if List.length servers = 16 then
                            Ok ()
                          else Error "expected tcp listener to accept every queued client in the burst"
                      )
                )
          )
    )
  ...
  with_tcp_pair
    (
      fun ~poll ~listener:_ ~listener_addr:_ ~client:_ ~server ~peer:_ ->
        ...
        let* () = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make "tcp-close-then-deregister") Kernel.Async.Interest.readable source)
        in
        let* () = lift_tcp_stream (Kernel.Net.TcpStream.close server)
        in
        let* () = lift_async (Kernel.Async.Poll.deregister poll source)
...
let test_tcp_stream_source_deregister_before_close_is_harmless = fun _ctx ->
  with_tcp_pair
    (
      fun ~poll ~listener:_ ~listener_addr:_ ~client:_ ~server ~peer:_ ->
        let source = Kernel.Net.TcpStream.to_source server in
        let* () = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make "tcp-deregister-then-close") Kernel.Async.Interest.readable source)
        in
        let* () = lift_async (Kernel.Async.Poll.deregister poll source)
        in
        let* () = lift_tcp_stream (Kernel.Net.TcpStream.close server)
    ...
    (
      fun poll ->
        ...
        protect ~finally:(
          ...
          (
            fun () ->
              let* listener_addr = lift_tcp_listener (Kernel.Net.TcpListener.local_addr listener)
              in
              let rec connect_many remaining acc =
                if remaining = 0 then
                  Ok acc
                else
                  let* stream = connect_stream poll listener_addr in connect_many (remaining - 1) (stream :: acc)
              in
              let rec accept_many remaining acc =
                if remaining = 0 then
                  Ok acc
                else
                  let* (stream, _) = accept_stream poll listener in accept_many (remaining - 1) (stream :: acc)
              in
                      ...
                      (
                        fun () ->
                          ...
                          let rec register index = function
                            | [] -> Ok ()
                            | stream :: rest ->
                                let* () = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make index) Kernel.Async.Interest.readable (Kernel.Net.TcpStream.to_source stream)) in register (index + 1) rest
                          in
                          let rec write_all_clients = function
                            | [] -> Ok ()
                            | stream :: rest ->
                                let* written = lift_tcp_stream (Kernel.Net.TcpStream.write stream (Kernel.Bytes.from_string "x"))
                                in
                                if written != 1 then
                                  Error "expected tcp write to make progress during many-stream readiness"
                                else write_all_clients rest
                          in
                          ...
                          in
                          let* () = register 0 servers
                          in
                          let* () = write_all_clients clients
                          in
                          let rec poll_until attempts =
                            if attempts = 0 then
                              Error "expected many tcp streams to become readable after client writes"
                            else
                              let* events = lift_async (Kernel.Async.Poll.poll ~timeout:100_000_000L ~max_events:64 poll) in mark events;
                            if all_seen 0 then
                              Ok ()
                            else poll_until (attempts - 1)
                          in
                          poll_until 12
                      )
                )
...
let test_udp_socket_send_to_and_recv_from = fun _ctx ->
  with_poll
    (
      fun poll ->
        let* server = lift_udp (Kernel.Net.UdpSocket.bind (Kernel.Net.SocketAddr.loopback_v4 ~port:0))
        in
        protect ~finally:(
          fun () -> close_udp server
        )
          (
            fun () ->
              let* client = lift_udp (Kernel.Net.UdpSocket.bind (Kernel.Net.SocketAddr.loopback_v4 ~port:0))
              in
              protect ~finally:(
                fun () -> close_udp client
              )
                (
                  fun () ->
                    let* server_addr = lift_udp (Kernel.Net.UdpSocket.local_addr server)
                    in
                    let* client_addr = lift_udp (Kernel.Net.UdpSocket.local_addr client)
                    in
                    let* sent = lift_udp (Kernel.Net.UdpSocket.send_to client server_addr (Kernel.Bytes.from_string "ping"))
                    in
                    if sent != 4 then
                      Error "expected udp send_to to send one datagram"
                    else
                      let server_buf = Kernel.Bytes.create ~size:32 in
                      let* (read, from) = recv_from_udp poll ~token:(Kernel.Async.Token.make 307) server server_buf
                      in
                      if read != 4 || not (Kernel.String.equal (Kernel.Bytes.sub_string server_buf ~offset:0 ~len:read) "ping") || Kernel.Net.SocketAddr.port from != Kernel.Net.SocketAddr.port client_addr then
                        Error "expected udp recv_from to preserve sender and payload"
                      else
                        let* () = lift_udp (Kernel.Net.UdpSocket.connect server client_addr)
                        in
                        let* () = lift_udp (Kernel.Net.UdpSocket.connect client server_addr)
                        in
                        let* _ = lift_udp (Kernel.Net.UdpSocket.send server (Kernel.Bytes.from_string "pong"))
                        in
                        let client_buf = Kernel.Bytes.create ~size:32 in
                        let* reply = recv_udp poll ~token:(Kernel.Async.Token.make 308) client client_buf
                        in
                        if reply = 4 && Kernel.String.equal (Kernel.Bytes.sub_string client_buf ~offset:0 ~len:reply) "pong" then
                          Ok ()
                        else Error "expected connected udp send and recv to preserve payload"
                )
          )
    )
          ...
          (
            fun () ->
              ...
              protect ~finally:(
                ...
                (
                  fun () ->
                    let* other = lift_udp (Kernel.Net.UdpSocket.bind (Kernel.Net.SocketAddr.loopback_v4 ~port:0))
                    in
                    protect ~finally:(
                      fun () -> close_udp other
                    )
                      (
                        fun () ->
                          ...
                          let* () = lift_async (Kernel.Async.Poll.register poll token Kernel.Async.Interest.readable source)
                          in
                          protect ~finally:(
                            fun () ->
                              let _ = Kernel.Async.Poll.deregister poll source in ()
                            ...
                            (
                              fun () ->
                                let* sent = lift_udp (Kernel.Net.UdpSocket.send_to other server_addr (Kernel.Bytes.from_string "rogue"))
                                in
                                if sent != 5 then
                                  Error "expected udp send_to to send the rogue datagram"
                                else
                                  let* events = lift_async (Kernel.Async.Poll.poll ~timeout:1_000_000L poll)
                                  in
                                  let server_ready = List.any events ~fn:(
                                    fun event -> Kernel.Async.Event.is_readable event && Kernel.Async.Token.equal token (Kernel.Async.Event.token event)
                                  ) in
                      ...
                      (
                        fun () ->
                          ...
                          protect ~finally:(
                            ...
                            (
                              fun () ->
                                let* sent_a = lift_udp (Kernel.Net.UdpSocket.send_to other server_addr (Kernel.Bytes.from_string "rogue-a"))
                                in
                                let* sent_b = lift_udp (Kernel.Net.UdpSocket.send_to other server_addr (Kernel.Bytes.from_string "rogue-b"))
                                in
                                if sent_a != 7 || sent_b != 7 then
                                  Error "expected udp send_to to preserve both foreign datagrams"
                                else
                                  let* quiet = lift_async (Kernel.Async.Poll.poll ~timeout:20_000_000L ~max_events:8 poll)
                                  in
                                  if has_readable_token token quiet then
                                    ...
                                    | Kernel.Result.Error Kernel.Net.UdpSocket.WouldBlock ->
                                        let* sent = lift_udp (Kernel.Net.UdpSocket.send client (Kernel.Bytes.from_string "peer"))
                                        in
                                        if sent != 4 then
                                          Error "expected connected udp send to write one datagram"
                                        else
                                          let* events = lift_async (Kernel.Async.Poll.poll ~timeout:100_000_000L ~max_events:8 poll)
                                          in
                                          if not (has_readable_token token events) then
                                            Error "expected connected udp peer traffic to become readable"
                                          else
                                            let* (read, from) = lift_udp (Kernel.Net.UdpSocket.recv_from server buffer)
                                            in
                                            if read = 4 && Kernel.String.equal (Kernel.Bytes.sub_string buffer ~offset:0 ~len:read) "peer" && Kernel.Net.SocketAddr.port from = Kernel.Net.SocketAddr.port client_addr then
                                              Ok ()
                                            else Error "expected connected udp recv_from to preserve the accepted peer"
                                    | Kernel.Result.Error error -> Error (string_of_udp_error error)
                                    | Kernel.Result.Ok _ -> Error "expected connected udp socket to ignore foreign datagrams before peer traffic"
                            )
                      )
                )
          )
      ...
      fun poll ->
        let rec bind_many remaining acc =
          if remaining = 0 then
            ...
            let* server = lift_udp (Kernel.Net.UdpSocket.bind (Kernel.Net.SocketAddr.loopback_v4 ~port:0))
            in
            let* client = lift_udp (Kernel.Net.UdpSocket.bind (Kernel.Net.SocketAddr.loopback_v4 ~port:0)) in bind_many (remaining - 1) ((server, client) :: acc)
        in
        let rec close_many = function
          | [] -> ()
          | (server, client) :: rest ->
              let _ = Kernel.Net.UdpSocket.close server in
              let _ = Kernel.Net.UdpSocket.close client in close_many rest
        in
        let* pairs = bind_many 16 []
        in
        protect ~finally:(
          fun () -> close_many pairs
        )
          (
            fun () ->
              ...
              let rec register index = function
                | [] -> Ok ()
                | (server, _) :: rest ->
                    let* () = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make index) Kernel.Async.Interest.readable (Kernel.Net.UdpSocket.to_source server)) in register (index + 1) rest
              in
              let rec send_all = function
                ...
                | (server, client) :: rest ->
                    let* server_addr = lift_udp (Kernel.Net.UdpSocket.local_addr server)
                    in
                    let* sent = lift_udp (Kernel.Net.UdpSocket.send_to client server_addr (Kernel.Bytes.from_string "x"))
                    in
                    if sent != 1 then
                      Error "expected udp send_to to write one byte"
                    else send_all rest
              in
              let* () = register 0 pairs
              in
              let* () = send_all pairs
              in
        ...
        protect ~finally:(
          ...
          (
            fun () ->
              ...
              in
              close_even 0 pairs;
              let* () = send_live 0 pairs
              in
              let rec poll_until attempts =
                if attempts = 0 then
                  Error "expected closed registered udp sockets to not poison remaining readiness"
                else
                  let* events = lift_async (Kernel.Async.Poll.poll ~timeout:100_000_000L ~max_events:32 poll) in mark events;
                if live_seen 0 then
                  Ok ()
                else poll_until (attempts - 1)
              in
...
let test_udp_socket_source_deregister_after_close_is_harmless = fun _ctx ->
  with_poll
    (
      fun poll ->
        let* socket = lift_udp (Kernel.Net.UdpSocket.bind (Kernel.Net.SocketAddr.loopback_v4 ~port:0))
        in
        protect ~finally:(
          fun () -> close_udp socket
        )
          (
            fun () ->
              let source = Kernel.Net.UdpSocket.to_source socket in
              let* () = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make "udp-close-then-deregister") Kernel.Async.Interest.readable source)
              in
              let* () = lift_udp (Kernel.Net.UdpSocket.close socket)
              in
              let* () = lift_async (Kernel.Async.Poll.deregister poll source)
              in
              let* _ = lift_async (Kernel.Async.Poll.poll ~timeout:0L poll) in Ok ()
      ...
      fun poll ->
        ...
        protect ~finally:(
          ...
          (
            fun () ->
              let source = Kernel.Net.UdpSocket.to_source socket in
              let* () = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make "udp-deregister-then-close") Kernel.Async.Interest.readable source)
              in
              let* () = lift_async (Kernel.Async.Poll.deregister poll source)
              in
              let* () = lift_udp (Kernel.Net.UdpSocket.close socket)
                    ...
                    protect ~finally:(
                      ...
                      (
                        fun () ->
                          ...
                          let rec write_live index clients servers =
                            ...
                            | (client :: client_rest, _server :: server_rest) ->
                                if index land 1 = 0 then
                                  ...
                                  let* written = lift_tcp_stream (Kernel.Net.TcpStream.write client (Kernel.Bytes.from_string "x"))
                                  in
                                  if written != 1 then
                                    Error "expected tcp write to make progress for live registered streams"
                                  else write_live (index + 1) client_rest server_rest
                          ...
                          let* () = write_live 0 clients servers
                          in
                          let rec poll_until attempts =
                            if attempts = 0 then
...
let test_udp_socket_ipv6_send_to_and_recv_from = fun _ctx ->
  with_poll
    (
      fun poll ->
        let* server = lift_udp (Kernel.Net.UdpSocket.bind (Kernel.Net.SocketAddr.loopback_v6 ~port:0))
        in
        protect ~finally:(
          fun () -> close_udp server
        )
          (
            fun () ->
              let* client = lift_udp (Kernel.Net.UdpSocket.bind (Kernel.Net.SocketAddr.loopback_v6 ~port:0))
              in
              protect ~finally:(
                fun () -> close_udp client
              )
                (
                  fun () ->
                    let* server_addr = lift_udp (Kernel.Net.UdpSocket.local_addr server)
                    in
                    let* sent = lift_udp (Kernel.Net.UdpSocket.send_to client server_addr (Kernel.Bytes.from_string "ipv6"))
                    in
                    if sent != 4 then
                      Error "expected ipv6 udp send_to to send one datagram"
                    else
                      let buffer = Kernel.Bytes.create ~size:32 in
                      let* (read, from) = recv_from_udp poll ~token:(Kernel.Async.Token.make 314) server buffer
                      in
                      if read = 4 && Kernel.String.equal (Kernel.Bytes.sub_string buffer ~offset:0 ~len:read) "ipv6" && Kernel.String.equal (Kernel.Net.IpAddr.to_string (Kernel.Net.SocketAddr.ip from)) "::1" then
                        Ok ()
                      else Error "expected ipv6 udp loopback to preserve payload and peer address"
                )
          )
    )
          ...
          (
            fun () ->
              ...
              protect ~finally:(
                ...
                (
                  fun () ->
                    let* other = lift_udp (Kernel.Net.UdpSocket.bind (Kernel.Net.SocketAddr.loopback_v6 ~port:0))
                    in
                    protect ~finally:(
                      fun () -> close_udp other
                    )
                              ...
                              fun () ->
                                ...
                                if sent != 5 then
                                  ...
                                  if has_readable_token token quiet then
                                    ...
                                    | Kernel.Result.Error Kernel.Net.UdpSocket.WouldBlock ->
                                        let* sent = lift_udp (Kernel.Net.UdpSocket.send client (Kernel.Bytes.from_string "v6ok"))
                                        in
                                        if sent != 4 then
                                          ...
                                          if not (has_readable_token token events) then
                                            ...
                                            let* (read, from) = lift_udp (Kernel.Net.UdpSocket.recv_from server buffer)
                                            in
                                            if read = 4 && Kernel.String.equal (Kernel.Bytes.sub_string buffer ~offset:0 ~len:read) "v6ok" && Kernel.String.equal (Kernel.Net.IpAddr.to_string (Kernel.Net.SocketAddr.ip from)) "::1" then
                                              Ok ()
                                            else Error "expected connected ipv6 udp recv_from to preserve the loopback peer"
                                    | Kernel.Result.Error error -> Error (string_of_udp_error error)
                                    | Kernel.Result.Ok _ -> Error "expected connected ipv6 udp socket to ignore foreign datagrams before peer traffic"
                            )
                      )
                )
          )
  ...
  let rec loop remaining =
    if remaining = 0 then
      ...
      if Kernel.Net.SocketAddr.port addr > 0 then
        loop (remaining - 1)
      else Error "expected repeated tcp listener binds to yield an ephemeral port"
  in
  loop 64
...
let test_udp_socket_repeated_bind_and_close_stays_healthy = fun _ctx ->
  let rec loop remaining =
    if remaining = 0 then
      ...
      let* socket = lift_udp (Kernel.Net.UdpSocket.bind (Kernel.Net.SocketAddr.loopback_v4 ~port:0))
      in
      let* addr = lift_udp (Kernel.Net.UdpSocket.local_addr socket)
      in
      let* () = lift_udp (Kernel.Net.UdpSocket.close socket)
      in
      if Kernel.Net.SocketAddr.port addr > 0 then
        loop (remaining - 1)
      else Error "expected repeated udp binds to yield an ephemeral port"
  in
  loop 128
  ...
  protect ~finally:(
    ...
    (
      fun () ->
        ...
        | Kernel.Result.Error error -> Error (Kernel.String.append "expected destination-address error, got " (string_of_udp_error error))
...
let test_udp_send_and_recv_after_close_report_bad_file_descriptor = fun _ctx ->
  ...
  protect ~finally:(
    ...
    (
      fun () ->
        let* peer = lift_udp (Kernel.Net.UdpSocket.bind (Kernel.Net.SocketAddr.loopback_v4 ~port:0))
        in
        protect ~finally:(
          fun () -> close_udp peer
        )
          (
            fun () ->
              let* peer_addr = lift_udp (Kernel.Net.UdpSocket.local_addr peer)
              in
              let* () = lift_udp (Kernel.Net.UdpSocket.connect server peer_addr)
              in
              let* () = lift_udp (Kernel.Net.UdpSocket.close server)
              in
              let buffer = Kernel.Bytes.create ~size:8 in
              match Kernel.Net.UdpSocket.send server (Kernel.Bytes.from_string "x"), Kernel.Net.UdpSocket.recv server buffer with
              ...
              | (Kernel.Result.Error send_error, Kernel.Result.Error recv_error) -> Error (Kernel.String.concat " | " [ Kernel.String.concat "" [ "expected closed udp send to fail with bad file descriptor but got "; string_of_udp_error send_error ]; Kernel.String.concat "" [ "expected closed udp recv to fail with bad file descriptor but got "; string_of_udp_error recv_error ] ])
...
let test_udp_bind_rejects_in_use_address = fun _ctx ->
  ...
  let* first = lift_udp (Kernel.Net.UdpSocket.bind addr)
  in
  protect ~finally:(
    fun () -> close_udp first
  )
    (
      fun () ->
        let* bound_addr = lift_udp (Kernel.Net.UdpSocket.local_addr first)
        in
...
let test_socket_addr_ipv6_to_string_is_bracketed = fun _ctx ->
  ...
  if rendered = "[::1]:80" then
    Ok ()
  else Error "expected SocketAddr.to_string to render IPv6 addresses with brackets"
        ...
        protect ~finally:(
          ...
          (
            fun () ->
              ...
              | Kernel.Result.Ok 0 ->
                  let* events = lift_async (Kernel.Async.Poll.poll ~timeout:20_000_000L ~max_events:8 poll)
                  in
                  if has_readable_token token events then
                    Error "expected TcpStream.write ~len:0 to leave the peer unreadable"
                  else Ok ()
              | Kernel.Result.Ok _ -> Error "expected TcpStream.write ~len:0 to report zero bytes written"
              | Kernel.Result.Error error -> Error (string_of_tcp_stream_error error)
          )
    )
...
let test_tcp_stream_close_twice_reports_bad_file_descriptor = fun _ctx ->
  with_tcp_pair
    (
      fun ~poll:_ ~listener:_ ~listener_addr:_ ~client ~server:_ ~peer:_ ->
        let* () = lift_tcp_stream (Kernel.Net.TcpStream.close client)
        in
  ...
  with_tcp_pair
    (
      fun ~poll:_ ~listener:_ ~listener_addr:_ ~client ~server:_ ~peer:_ ->
        ...
        | (Kernel.Result.Error local_error, Kernel.Result.Error peer_error) -> Error (Kernel.String.concat " | " [ Kernel.String.append "unexpected local_addr error: " (string_of_tcp_stream_error local_error); Kernel.String.append "unexpected peer_addr error: " (string_of_tcp_stream_error peer_error) ])
  ...
  with_tcp_pair
    (
      fun ~poll:_ ~listener:_ ~listener_addr:_ ~client ~server:_ ~peer:_ ->
        let* () = lift_tcp_stream (Kernel.Net.TcpStream.shutdown client Kernel.Net.TcpStream.Write)
        in
        let* () = lift_tcp_stream (Kernel.Net.TcpStream.shutdown client Kernel.Net.TcpStream.Write) in Ok ()
    )
    ...
    (
      fun ~poll ~server ~server_addr ~client ~client_addr:_ ->
        ...
        if sent != 4 then
          ...
          let* read = recv_udp poll ~token:(Kernel.Async.Token.make 925) server buffer
          in
          if read = 4 && Kernel.Bytes.sub_string buffer ~offset:0 ~len:read = "ping" then
            Ok ()
          else Error "expected recv on an unconnected udp socket to accept peer traffic"
    )
  ...
  with_connected_udp_pair
    (
      fun ~poll:_ ~server ~server_addr:_ ~client ~client_addr:_ ->
        match Kernel.Net.UdpSocket.send client ~pos:2 ~len:3 (Kernel.Bytes.create ~size:4), Kernel.Net.UdpSocket.recv server ~pos:(-1) (Kernel.Bytes.create ~size:4) with
        | (Kernel.Result.Error (Kernel.Net.UdpSocket.InvalidSlice { pos = 2; len = 3; buffer_len = 4 }), Kernel.Result.Error (Kernel.Net.UdpSocket.InvalidSlice { pos = (-1); _ })) -> Ok ()
        | (Kernel.Result.Error send_error, Kernel.Result.Error recv_error) -> Error (Kernel.String.concat " | " [ Kernel.String.append "unexpected udp send invalid-slice error: " (string_of_udp_error send_error); Kernel.String.append "unexpected udp recv invalid-slice error: " (string_of_udp_error recv_error) ])
  ...
  with_connected_udp_pair
    (
      fun ~poll ~server ~server_addr:_ ~client ~client_addr:_ ->
        let* sent = lift_udp (Kernel.Net.UdpSocket.send client (Kernel.Bytes.from_string ""))
        in
        if sent != 0 then
          Error "expected sending a zero-length udp datagram to report zero bytes"
        else
          let buffer = Kernel.Bytes.create ~size:1 in
          let* read = recv_udp poll ~token:(Kernel.Async.Token.make 926) server buffer
          in
          if read = 0 then
            Ok ()
          else Error "expected the peer to observe an empty udp datagram"
    )
...
let main ~args = Test.Cli.main ~execution_mode:Test.Cli.Linear ~name:"kernel_new_net_tests" ~tests ~args ()
