...
let test_of_bytes_copies_input = fun _ctx ->
  ...
  if Kernel.String.equal value "riot" then
    Ok ()
  else Error "expected String.of_bytes to keep the returned string stable after byte mutation"
...
let test_to_bytes_copies_input = fun _ctx ->
  ...
  if Kernel.String.equal original "riot" then
    Ok ()
  else Error "expected String.to_bytes to keep the original string immutable"

let test_init_builds_expected_string = fun _ctx ->
  let built = Kernel.String.init ~len:4 ~fn:(
    fun index -> Kernel.Char.from_int_unchecked (65 + index)
  ) in
  if Kernel.String.equal built "ABCD" then
    Ok ()
  else Error "expected String.init to build characters in index order"
...
let test_concat_preserves_separator_order = fun _ctx ->
  ...
  if Kernel.String.equal value "domains/admin/users" then
    Ok ()
  else Error "expected String.concat to preserve value and separator order"
...
let test_capitalize_ascii_preserves_tail_casing = fun _ctx ->
  ...
  if Kernel.String.equal lower_mixed "MutIterator" && Kernel.String.equal already_capitalized "MutIterator" then
    Ok ()
  else Error "expected String.capitalize_ascii to uppercase only the first character and preserve the rest unchanged"

let test_equal_matches_only_identical_strings = fun _ctx ->
  if Kernel.String.equal "tests/fixtures" "tests/fixtures" && not (Kernel.String.equal "tests/fixtures" "tests/fixtures/") && not (Kernel.String.equal "tests/fixtures" "tests/generated") then
    Ok ()
  else Error "expected String.equal to match only byte-identical strings"

let test_starts_with_matches_exact_prefix_and_descendants = fun _ctx ->
  if Kernel.String.starts_with ~prefix:"tests/fixtures" "tests/fixtures" && Kernel.String.starts_with ~prefix:"tests/fixtures" "tests/fixtures/case.ml" && Kernel.String.starts_with ~prefix:"tests/fixtures/" "tests/fixtures/case.ml" && not (Kernel.String.starts_with ~prefix:"tests/fixtures/" "tests/fixtures") then
    Ok ()
  else Error "expected String.starts_with to match exact prefixes without inventing path-boundary rules"

let test_starts_with_is_raw_prefix_not_path_segment_match = fun _ctx ->
  if Kernel.String.starts_with ~prefix:"tests/fixtures" "tests/fixtures-generated" && Kernel.String.starts_with ~prefix:"tests/fixtures" "tests/fixtures_extra" && not (Kernel.String.starts_with ~prefix:"tests/fixtures/" "tests/fixtures-generated") then
    Ok ()
  else Error "expected String.starts_with to use raw byte prefixes rather than path-aware matching"
