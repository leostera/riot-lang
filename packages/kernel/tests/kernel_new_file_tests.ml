...
let with_tempdir = fun prefix fn ->
  match Fs.with_tempdir ~prefix
    (
      fun tempdir -> fn (Kernel.Path.from_string (Path.to_string tempdir))
    ) with
  | Ok result -> result
...
let with_temp_path = fun prefix filename fn ->
  match Fs.with_tempdir ~prefix
    (
      fun tempdir ->
        let path = Kernel.Path.(Path.to_string tempdir / filename) in fn path
    ) with
  | Ok result -> result
  | Error err -> Error (IO.error_message err)

let with_file = fun file fn -> protect ~finally:(
  fun () ->
...
let array_contains = fun values target ->
  let rec loop index =
    if index = Kernel.Array.length values then
      ...
      if Kernel.String.equal (Kernel.Array.get_unchecked values ~at:index) target then
        true
      else loop (index + 1)
  in
  loop 0
...
let array_has_exact_members = fun actual expected ->
  let rec members_present index =
    if index = Kernel.Array.length expected then
      ...
      if array_contains actual (Kernel.Array.get_unchecked expected ~at:index) then
...
let test_file_scalar_write_roundtrips = fun _ctx ->
  with_temp_path "kernel_new_file" "scalar.bin"
    (
      fun path ->
        let* file = lift (Kernel.Fs.File.open_write path)
        in
        ...
        let* () =
          with_file file
            (
              fun () ->
                let* written = lift (Kernel.Fs.File.write file payload)
                in
                if written = Kernel.Bytes.length payload then
                  Ok ()
                else Error "expected full scalar write"
            )
        in
        let* file = lift (Kernel.Fs.File.open_read path)
        in
        ...
        let* actual =
          with_file file
            (
              fun () ->
                let* read = lift (Kernel.Fs.File.read file buf) in Ok (Kernel.Bytes.sub_string buf ~offset:0 ~len:read)
            )
        in
        if Kernel.String.equal actual "hello kernel-new" then
          Ok ()
        else Error "expected scalar file roundtrip to preserve payload"
    )
    ...
    (
      fun path ->
          ...
          with_file file
            (
              fun () ->
                ...
                let* written = lift (Kernel.Fs.File.write_vectored file payload)
                in
                if written = expected then
                  Ok ()
                else Error "expected full vectored write"
            )
        in
        let* file = lift (Kernel.Fs.File.open_read path)
        in
        ...
        if Kernel.String.equal actual "hello vectored world" then
          Ok ()
        else Error "expected vectored file roundtrip to preserve payload"
    )
      ...
      fun path ->
        ...
        let* () =
          with_file file
            (
              fun () ->
                let* written = lift (Kernel.Fs.File.write file ~pos:2 ~len:7 payload)
                in
                if written = 7 then
                  Ok ()
                else Error "expected partial file write to write the requested slice"
            )
        in
        let* file = lift (Kernel.Fs.File.open_read path)
        in
        ...
        let* actual =
          with_file file
            (
              fun () ->
                let* read = lift (Kernel.Fs.File.read file ~pos:3 ~len:7 buf) in Ok (read, Kernel.Bytes.to_string buf)
            )
        in
        let read, contents = actual in
        if read = 7 && Kernel.String.equal contents "xxxpayloadyyy" then
          Ok ()
        else Error "expected partial file read to fill only the requested slice"
    )
  ...
  with_tempdir "kernel_new_file"
    (
      fun tempdir ->
        ...
        let* () = lift (Kernel.Fs.File.create_dir child_dir ~perm:0o755)
        in
        let* file = lift (Kernel.Fs.File.open_write child_file)
        in
        let* () =
          with_file file
            (
              fun () ->
                let* _ = lift (Kernel.Fs.File.write file (Kernel.Bytes.from_string "a")) in Ok ()
            )
        in
        let* names = lift (Kernel.Fs.File.read_dir_names tempdir)
        in
        let has_child = Kernel.Array.fold_left names ~acc:false ~fn:(
          fun found name -> found || Kernel.String.equal name "child"
        ) in
        let has_file = Kernel.Array.fold_left names ~acc:false ~fn:(
          fun found name -> found || Kernel.String.equal name "alpha.txt"
        ) in
        let* metadata = lift (Kernel.Fs.File.metadata child_dir)
        in
        if has_child && has_file && Kernel.Fs.File.Metadata.is_dir metadata then
          Ok ()
        else Error "expected directory creation and read_dir_names to expose entries"
    )
  ...
  with_tempdir "kernel_new_file"
    (
      fun tempdir ->
        ...
        let* file = lift (Kernel.Fs.File.open_write target)
        in
        let* () =
          with_file file
            (
              fun () ->
                let* _ = lift (Kernel.Fs.File.write file (Kernel.Bytes.from_string "kernel")) in Ok ()
            )
        in
        protect ~finally:(
          fun () ->
          ...
          (
            fun () ->
              let* () = lift (Kernel.Fs.File.symlink ~src:target ~dst:link)
              in
              let* link_target = lift (Kernel.Fs.File.read_link link)
              in
              let* followed = lift (Kernel.Fs.File.metadata link)
              in
              let* raw = lift (Kernel.Fs.File.symlink_metadata link)
              in
              let* canonical = lift (Kernel.Fs.File.canonicalize link)
              in
              let* canonical_target = lift (Kernel.Fs.File.canonicalize target)
              in
              let link_target_matches = Kernel.String.equal (Kernel.Path.to_string link_target) (Kernel.Path.to_string target) in
              let canonical_matches = Kernel.String.equal (Kernel.Path.to_string canonical) (Kernel.Path.to_string canonical_target) in
              ...
              if link_target_matches && followed_is_file && raw_is_symlink && canonical_matches then
                Ok ()
              else Error "expected symlink metadata and canonicalize to distinguish link from target"
          )
    )
    ...
    (
      fun tempdir ->
        ...
        protect ~finally:(
          ...
          (
            fun () ->
              let* () = lift (Kernel.Fs.File.remove_file target)
              in
              let* exists = lift (Kernel.Fs.File.exists link)
              in
              let* metadata = lift (Kernel.Fs.File.symlink_metadata link)
              in
              if not exists && Kernel.Fs.File.Metadata.is_symlink metadata then
                Ok ()
              else Error "expected dangling symlink_metadata to preserve symlink kind"
          )
    )
        ...
        protect ~finally:(
          ...
          (
            fun () ->
              ...
              | Kernel.Result.Error (Kernel.Fs.File.System Kernel.SystemError.NoSuchFileOrDirectory) when Kernel.Fs.File.Metadata.is_symlink raw -> Ok ()
              | Kernel.Result.Error error -> Error (Kernel.Fs.File.error_to_string error)
        ...
        protect ~finally:(
          ...
          (
            fun () ->
              let* () = lift (Kernel.Fs.File.symlink ~src:target ~dst:link)
              in
              let* by_lstat = lift (Kernel.Fs.File.lstat link)
              in
              let* by_symlink_metadata = lift (Kernel.Fs.File.symlink_metadata link)
              in
              if Kernel.Fs.File.Metadata.is_symlink by_lstat && Kernel.Fs.File.Metadata.is_symlink by_symlink_metadata && Kernel.Fs.File.Metadata.ino by_lstat = Kernel.Fs.File.Metadata.ino by_symlink_metadata then
                Ok ()
              else Error "expected lstat to match symlink_metadata for a symbolic link"
          )
    )
  ...
  with_tempdir "kernel_new_file"
    (
      fun tempdir ->
        ...
        let* raw = lift (Kernel.Fs.File.symlink_metadata link)
        in
        let* () = lift (Kernel.Fs.File.remove_file link)
        in
        let* target_exists = lift (Kernel.Fs.File.exists target)
        in
        let* link_exists = lift (Kernel.Fs.File.exists link)
        in
        if Kernel.Fs.File.Metadata.is_file followed && Kernel.Fs.File.Metadata.is_symlink raw && target_exists && not link_exists then
          Ok ()
        else Error "expected metadata to follow symlinks while removing the symlink leaves the target intact"
    )
  ...
  with_tempdir "kernel_new_file"
    (
      fun tempdir ->
        ...
        let* file = lift (Kernel.Fs.File.open_write source)
        in
        let* () =
          with_file file
            (
              fun () ->
                let* _ = lift (Kernel.Fs.File.write file payload) in Ok ()
            )
        in
        let* () = lift (Kernel.Fs.File.copy ~src:source ~dst:copied)
        in
        let* () = lift (Kernel.Fs.File.rename ~src:copied ~dst:renamed)
        in
        let* file = lift (Kernel.Fs.File.open_read renamed)
        in
        ...
        in
        let* exists = lift (Kernel.Fs.File.exists renamed)
        in
        if exists && Kernel.String.equal actual "copy me" then
          Ok ()
        else Error "expected copy and rename to preserve payload"
    )
      ...
      fun path ->
        ...
        let* (path_metadata, fd_metadata) =
          with_file file
            (
              fun () ->
                let* _ = lift (Kernel.Fs.File.write file payload)
                in
                let* fd_metadata = lift (Kernel.Fs.File.fstat file)
                in
                let* path_metadata = lift (Kernel.Fs.File.metadata path) in Ok (path_metadata, fd_metadata)
            )
        in
        if Kernel.Fs.File.Metadata.ino path_metadata = Kernel.Fs.File.Metadata.ino fd_metadata && Kernel.Fs.File.Metadata.len path_metadata = Kernel.Fs.File.Metadata.len fd_metadata && Kernel.Fs.File.Metadata.nlink path_metadata = Kernel.Fs.File.Metadata.nlink fd_metadata then
          Ok ()
        else Error "expected fstat metadata to match path metadata for the same file"
    )
      ...
      fun tempdir ->
        ...
        let* () =
          with_file file
            (
              fun () ->
                let* _ = lift (Kernel.Fs.File.write file (Kernel.Bytes.from_string "linked")) in Ok ()
            )
        in
        let* () = lift (Kernel.Fs.File.hard_link ~src:source ~dst:link)
        in
        let* source_metadata = lift (Kernel.Fs.File.metadata source)
        in
        let* link_metadata = lift (Kernel.Fs.File.metadata link)
        in
        let* () = lift (Kernel.Fs.File.create_dir directory ~perm:0o755)
        in
        let* () = lift (Kernel.Fs.File.remove_file link)
        in
        let* link_exists = lift (Kernel.Fs.File.exists link)
        in
        let* () = lift (Kernel.Fs.File.remove_dir directory)
        in
        let* directory_exists = lift (Kernel.Fs.File.exists directory)
        in
        if Kernel.Fs.File.Metadata.nlink source_metadata = 2 && Kernel.Fs.File.Metadata.ino source_metadata = Kernel.Fs.File.Metadata.ino link_metadata && not link_exists && not directory_exists then
          Ok ()
        else Error "expected hard_link to share metadata and remove ops to clean up paths"
    )
      ...
      fun tempdir ->
        ...
        let* () =
          with_file file
            (
              fun () ->
                let* _ = lift (Kernel.Fs.File.write file (Kernel.Bytes.from_string "x")) in Ok ()
            )
        in
  ...
  with_tempdir "kernel_new_file"
    (
      fun tempdir ->
        ...
        let* () = lift (Kernel.Fs.File.create_dir directory ~perm:0o755)
        in
        let* file = lift (Kernel.Fs.File.open_write file_path)
        in
        let* () =
          with_file file
            (
        ...
        in
        let* dir_exists = lift (Kernel.Fs.File.exists directory)
        in
        let* file_exists = lift (Kernel.Fs.File.exists file_path)
        in
        let* missing_exists = lift (Kernel.Fs.File.exists missing)
        in
        let* dir_is_directory = lift (Kernel.Fs.File.is_directory directory)
        in
        let* file_is_directory = lift (Kernel.Fs.File.is_directory file_path)
        in
        if dir_exists && file_exists && not missing_exists && dir_is_directory && not file_is_directory then
          Ok ()
        else Error "expected exists and is_directory to distinguish directories, files, and missing paths"
    )
      ...
      fun path ->
          ...
          with_file file
            (
              fun () ->
                ...
                if written = Kernel.Bytes.length payload then
                  Ok ()
                else Error "expected scalar write to seed vectored read fixture"
            )
        in
        let* file = lift (Kernel.Fs.File.open_read path)
        in
        ...
        let* actual =
          with_file file
            (
              fun () ->
                let* read = lift (Kernel.Fs.File.read_vectored file iov) in Ok (read, Kernel.IO.IoVec.to_string iov)
            )
        in
        let read, contents = actual in
        ...
        if read = Kernel.Bytes.length payload && Kernel.String.equal prefix "hello vectored read" then
          Ok ()
        else Error "expected read_vectored to preserve payload across iovec segments"
    )
    ...
    (
      fun path ->
        ...
        let file_is_tty =
          with_file file
            (
              fun () -> Ok (Kernel.Fs.File.is_tty file)
            )
        in
        let* file_is_tty = file_is_tty
        in
        let* pipe = lift (Kernel.Fs.File.pipe ())
        in
        protect ~finally:(
          fun () ->
          ...
          (
            fun () ->
              ...
              if not file_is_tty && not pipe_is_tty then
                Ok ()
              else Error "expected regular files and pipes to report non-tty"
          )
    )
  ...
  with_temp_path "kernel_new_file" "missing.bin"
    (
      fun path ->
        ...
        | Kernel.Result.Error error -> Error (Kernel.String.append "expected no-such-file error, got " (Kernel.Fs.File.error_to_string error))
    )
...
let test_repeated_pipe_open_and_close_stays_healthy = fun _ctx ->
  let rec loop remaining =
    if remaining = 0 then
      ...
      let* pipe = lift (Kernel.Fs.File.pipe ())
      in
      let* () = lift (Kernel.Fs.File.close pipe.read_end)
      in
      let* () = lift (Kernel.Fs.File.close pipe.write_end) in loop (remaining - 1)
  in
  loop 256
  ...
  with_tempdir "kernel_new_file"
    (
      fun tempdir ->
        ...
        let* () = lift (Kernel.Fs.File.create_dir nested ~perm:0o755)
        in
        let* file = lift (Kernel.Fs.File.open_write file_path)
        in
        let* () =
          with_file file
            (
              fun () ->
                let* written = lift (Kernel.Fs.File.write file (Kernel.Bytes.from_string "x"))
                in
                if written = 1 then
                  Ok ()
                else Error "expected directory fixture file write to make progress"
            )
        in
        let* names = lift (Kernel.Fs.File.read_dir_names tempdir)
        in
        let expected =
          ...
          |]
        in
        if not (array_contains names ".") && not (array_contains names "..") && array_has_exact_members names expected then
          Ok ()
        else Error "expected read_dir_names to skip dot entries and preserve visible contents"
    )
      ...
      fun tempdir ->
        ...
        let* () =
          with_file file
            (
              fun () ->
                let* written = lift (Kernel.Fs.File.write file (Kernel.Bytes.from_string "kernel"))
                in
                if written = 6 then
                  Ok ()
                else Error "expected target file write to make progress"
            )
        in
        protect ~finally:(
          fun () ->
          ...
          (
            fun () ->
              let* () = lift (Kernel.Fs.File.symlink ~src:target_ref ~dst:link)
              in
              let* () = lift (Kernel.Fs.File.symlink ~src:link_ref ~dst:nested_link)
              in
              let* canonical = lift (Kernel.Fs.File.canonicalize nested_link)
              in
              let* canonical_target = lift (Kernel.Fs.File.canonicalize target)
              in
              if not (Kernel.String.equal (Kernel.Path.to_string canonical) (Kernel.Path.to_string canonical_target)) then
                Error "expected nested symlink canonicalize to resolve to the final target"
              else
                let* target_path = lift (Kernel.Fs.File.read_link nested_link)
                in
  ...
  with_tempdir "kernel_new_file"
    (
      fun tempdir ->
        ...
        let* file = lift (Kernel.Fs.File.open_write original)
        in
        let* () =
          with_file file
            (
              fun () ->
                let* written = lift (Kernel.Fs.File.write file (Kernel.Bytes.from_string "linked"))
                in
                if written = 6 then
                  Ok ()
                else Error "expected hard-link fixture write to make progress"
            )
        in
        let* () = lift (Kernel.Fs.File.hard_link ~src:original ~dst:alias)
        in
        let* () = lift (Kernel.Fs.File.rename ~src:original ~dst:moved)
        in
        let* moved_stats = lift (Kernel.Fs.File.metadata moved)
        in
        let* alias_stats = lift (Kernel.Fs.File.metadata alias)
        in
        if Kernel.Fs.File.Metadata.nlink moved_stats != 2 || Kernel.Fs.File.Metadata.nlink alias_stats != 2 then
          Error "expected renamed hard links to keep a link count of two"
        else
          let* () = lift (Kernel.Fs.File.remove_file alias)
          in
          let* remaining = lift (Kernel.Fs.File.metadata moved)
          in
          let* original_exists = lift (Kernel.Fs.File.exists original)
          in
          if Kernel.Fs.File.Metadata.nlink remaining = 1 && not original_exists then
            Ok ()
          else Error "expected removing one hard-link path to leave a single remaining name"
    )
      ...
      fun path ->
        ...
        let* () =
          with_file file
            (
              fun () ->
                let* written = lift (Kernel.Fs.File.write_vectored file slice)
                in
                if written = 12 then
                  Ok ()
                else Error "expected vectored subslice write to write the selected slice only"
            )
        in
        let* file = lift (Kernel.Fs.File.open_read path)
        in
        ...
        let* actual =
          with_file file
            (
              fun () ->
                let* read = lift (Kernel.Fs.File.read file buffer) in Ok (Kernel.Bytes.sub_string buffer ~offset:0 ~len:read)
            )
        in
        if Kernel.String.equal actual "hello kernel" then
          Ok ()
        else Error "expected vectored subslice roundtrip to preserve the selected payload"
    )
  ...
  with_tempdir "kernel_new_file"
    (
      fun tempdir ->
        ...
        let* () = lift (Kernel.Fs.File.create_dir target_dir ~perm:0o755)
        in
        protect ~finally:(
          fun () ->
            let _ = Kernel.Fs.File.remove_file link in
          ...
          (
            fun () ->
              let* () = lift (Kernel.Fs.File.symlink ~src:target_dir ~dst:link)
              in
              let* followed = lift (Kernel.Fs.File.metadata link)
              in
              let* raw = lift (Kernel.Fs.File.lstat link)
              in
              let* by_alias = lift (Kernel.Fs.File.symlink_metadata link)
              in
              if Kernel.Fs.File.Metadata.is_dir followed && Kernel.Fs.File.Metadata.is_symlink raw && Kernel.Fs.File.Metadata.is_symlink by_alias then
                Ok ()
              else Error "expected metadata to follow symlink targets while lstat inspects the link itself"
          )
    )
  ...
  with_tempdir "kernel_new_file"
    (
      fun tempdir ->
        ...
        let* alpha_file = lift (Kernel.Fs.File.open_write alpha)
        in
        let* () =
          with_file alpha_file
            (
              fun () ->
                let* written = lift (Kernel.Fs.File.write alpha_file (Kernel.Bytes.from_string "a"))
                in
                if written = 1 then
                  Ok ()
                else Error "expected alpha fixture write to make progress"
            )
        in
        let* beta_file = lift (Kernel.Fs.File.open_write beta)
        in
        let* () =
          with_file beta_file
            (
              fun () ->
                let* written = lift (Kernel.Fs.File.write beta_file (Kernel.Bytes.from_string "b"))
                in
                if written = 1 then
                  Ok ()
                else Error "expected beta fixture write to make progress"
            )
        in
        let* first = lift (Kernel.Fs.File.read_dir_names tempdir)
        in
        let* () = lift (Kernel.Fs.File.rename ~src:alpha ~dst:gamma)
        in
        let* () = lift (Kernel.Fs.File.remove_file beta)
        in
        let* second = lift (Kernel.Fs.File.read_dir_names tempdir)
        in
        if array_has_exact_members first
          [|
            "alpha.txt";
            "beta.txt";
          |] && array_has_exact_members second [|"gamma.txt"|] then
          Ok ()
        else Error "expected read_dir_names to return fresh snapshots across repeated calls"
    )
          ...
          with_file file
            (
              fun () ->
                ...
                if written = 6 then
                  Ok ()
                else Error "expected symlink target fixture write to make progress"
            )
        in
        protect ~finally:(
          fun () ->
          ...
          (
            fun () ->
              let* () = lift (Kernel.Fs.File.symlink ~src:target ~dst:link)
              in
              let* () = lift (Kernel.Fs.File.rename ~src:target ~dst:moved)
              in
              let* raw = lift (Kernel.Fs.File.symlink_metadata link)
              in
              match Kernel.Fs.File.metadata link with
          ...
          with_file file
            (
              fun () ->
                ...
                if written = 6 then
                  Ok ()
                else Error "expected broken symlink fixture write to make progress"
            )
        in
        protect ~finally:(
          fun () ->
          ...
          (
            fun () ->
              ...
              let* () = lift (Kernel.Fs.File.remove_file target)
              in
              let* () = lift (Kernel.Fs.File.rename ~src:link ~dst:moved_link)
              in
              let* raw = lift (Kernel.Fs.File.symlink_metadata moved_link)
              in
              let* link_target = lift (Kernel.Fs.File.read_link moved_link)
              in
              if Kernel.Fs.File.Metadata.is_symlink raw && Kernel.String.equal (Kernel.Path.to_string link_target) (Kernel.Path.to_string target) then
                Ok ()
              else Error "expected renaming a broken symlink to preserve the link itself and its target text"
          )
    )
      ...
      fun tempdir ->
          ...
          with_file file
            (
              fun () ->
                ...
                if written = 6 then
                  Ok ()
                else Error "expected hard-link alias fixture write to make progress"
            )
        in
        let* () = lift (Kernel.Fs.File.hard_link ~src:original ~dst:alias)
        in
        let* () = lift (Kernel.Fs.File.remove_file original)
        in
        let* alias_metadata = lift (Kernel.Fs.File.metadata alias)
        in
        let* alias_file = lift (Kernel.Fs.File.open_read alias)
        in
        ...
        let* payload =
          with_file alias_file
            (
              fun () ->
                let* read = lift (Kernel.Fs.File.read alias_file buffer) in Ok (Kernel.Bytes.sub_string buffer ~offset:0 ~len:read)
            )
        in
        let* original_exists = lift (Kernel.Fs.File.exists original)
        in
        let* alias_exists = lift (Kernel.Fs.File.exists alias)
        in
        if not original_exists && alias_exists && Kernel.Fs.File.Metadata.nlink alias_metadata = 1 && Kernel.String.equal payload "linked" then
          Ok ()
        else Error "expected removing the original hard-link path to leave one remaining alias"
    )
    ...
    (
      fun tempdir ->
        ...
        let rec loop index cases =
          ...
          | [] -> Ok ()
          | (pos, len) :: rest ->
              let path = Kernel.Path.(tempdir / String.concat "" [ "scalar-slice-"; Int.to_string index; ".bin" ]) in
              let* file = lift (Kernel.Fs.File.open_write path)
              in
              let* () =
                with_file file
                  (
                    fun () ->
                      let* written = lift (Kernel.Fs.File.write file ~pos ~len payload)
                      in
                      if written = len then
                        Ok ()
                      else Error "expected scalar slice write matrix to write the requested length"
                  )
              in
              let* file = lift (Kernel.Fs.File.open_read path)
              in
              ...
              let* actual =
                with_file file
                  (
                    fun () ->
                      let* read = lift (Kernel.Fs.File.read file buffer) in Ok (read, Kernel.Bytes.sub_string buffer ~offset:0 ~len:read)
                  )
              in
              let read, contents = actual in
    ...
    (
      fun tempdir ->
        ...
        let rec loop index cases =
          ...
          | [] -> Ok ()
          | (pos, len) :: rest ->
              let path = Kernel.Path.(tempdir / String.concat "" [ "vectored-slice-"; Int.to_string index; ".bin" ]) in
                ...
                with_file file
                  (
                    fun () ->
                      ...
                      if written = len then
                        Ok ()
                      else Error "expected vectored slice write matrix to write the requested length"
                  )
              in
              let* file = lift (Kernel.Fs.File.open_read path)
              in
    ...
    (
      fun tempdir ->
        ...
        protect ~finally:(
          ...
          (
            fun () ->
              let* () = lift (Kernel.Fs.File.symlink ~src:right ~dst:left)
              in
              let* () = lift (Kernel.Fs.File.symlink ~src:left ~dst:right)
              in
      ...
      fun tempdir ->
              ...
              with_file file
                (
                  fun () ->
                    ...
                    if written = 1 then
                      Ok ()
                    else Error "expected larger snapshot fixture write to make progress"
                )
            in
            create_many (index + 1)
        in
        let rec rename_prefix index =
          if index = 8 then
            ...
            let* () = lift (Kernel.Fs.File.rename ~src:(path_for "entry" index) ~dst:(path_for "renamed" index)) in rename_prefix (index + 1)
        in
        let rec remove_suffix index =
          if index = 8 then
            Ok ()
          else
            let* () = lift (Kernel.Fs.File.remove_file (path_for "entry" (index + 8))) in remove_suffix (index + 1)
        in
        let* () = create_many 0
        in
        let* first = lift (Kernel.Fs.File.read_dir_names tempdir)
        ...
        let* second = lift (Kernel.Fs.File.read_dir_names tempdir)
        in
        if Kernel.Array.length first = 32 && Kernel.Array.length second = 24 && array_contains first "entry-0.txt" && array_contains first "entry-31.txt" && array_contains second "renamed-0.txt" && array_contains second "entry-31.txt" && not (array_contains second "entry-0.txt") && not (array_contains second "entry-8.txt") then
          Ok ()
        else Error "expected larger read_dir_names snapshots to reflect renames and removals across calls"
    )
  ...
  with_temp_path "kernel_new_file" "close-twice.txt"
    (
      fun path ->
        let* file = lift (Kernel.Fs.File.open_write path)
        in
        let* () = lift (Kernel.Fs.File.close file)
        in
    ...
    (
      fun tempdir ->
        ...
        | Kernel.Result.Ok file ->
            let _ = Kernel.Fs.File.close file in Error "expected open_write ~create:false to reject a missing path"
    )
...
let test_open_write_append_preserves_existing_bytes = fun _ctx ->
  with_temp_path "kernel_new_file" "append.txt"
    (
      fun path ->
        let* file = lift (Kernel.Fs.File.open_write ~create:true ~truncate:true path)
        in
        let* () =
          with_file file
            (
              fun () ->
                let* written = lift (Kernel.Fs.File.write file (Kernel.Bytes.from_string "old"))
                in
                if written = 3 then
                  Ok ()
                else Error "expected append fixture write to make progress"
            )
        in
        let* file = lift (Kernel.Fs.File.open_write ~create:false ~truncate:false ~append:true path)
        in
        let* () =
          with_file file
            (
              fun () ->
                let* written = lift (Kernel.Fs.File.write file (Kernel.Bytes.from_string "new"))
                in
                if written = 3 then
                  Ok ()
                else Error "expected append write to make progress"
            )
        in
        let* file = lift (Kernel.Fs.File.open_read path)
        in
        ...
        if payload = "oldnew" then
          Ok ()
        else Error "expected append mode to preserve the existing bytes and extend the file"
    )
      ...
      fun path ->
        ...
        let* () =
          with_file file
            (
              fun () ->
                let* written = lift (Kernel.Fs.File.write file (Kernel.Bytes.from_string "riot"))
                in
                if written = 4 then
                  Ok ()
                else Error "expected zero-read fixture write to make progress"
            )
        in
        let* file = lift (Kernel.Fs.File.open_read path)
        in
        ...
        let* actual =
          with_file file
            (
              fun () ->
                let* read = lift (Kernel.Fs.File.read file ~pos:2 ~len:0 buffer) in Ok (read, Kernel.Bytes.to_string buffer)
            )
        in
        match actual with
        | (0, "unchanged") -> Ok ()
    ...
    (
      fun path ->
        ...
        let* written =
          with_file file
            (
              fun () -> lift (Kernel.Fs.File.write file ~pos:1 ~len:0 (Kernel.Bytes.from_string "riot"))
            )
        in
        if written != 0 then
          ...
          let* read =
            with_file file
              (
                fun () -> lift (Kernel.Fs.File.read file buffer)
              )
          in
          if read = 0 then
            Ok ()
          else Error "expected File.write ~len:0 to leave file contents unchanged"
    )
          ...
          with_file file
            (
              fun () ->
                ...
                if written = 4 then
                  Ok ()
                else Error "expected vectored zero-segment fixture write to make progress"
            )
        in
        let* file = lift (Kernel.Fs.File.open_read path)
        in
    ...
    (
      fun path ->
        ...
        let* written =
          with_file file
            (
              fun () -> lift (Kernel.Fs.File.write_vectored file iov)
            )
        in
        if written != 0 then
          ...
          if read = 0 then
            Ok ()
          else Error "expected zero-length write_vectored to leave the file empty"
    )
  ...
  with_tempdir "kernel_new_file"
    (
      fun tempdir ->
        ...
        let* () = lift (Kernel.Fs.File.create_dir target ~perm:0o755)
        in
        let* () = lift (Kernel.Fs.File.symlink ~src:target ~dst:link)
        in
        protect ~finally:(
          ...
          (
            fun () ->
              let* () = lift (Kernel.Fs.File.remove_dir target)
              in
              let* is_directory = lift (Kernel.Fs.File.is_directory link)
              in
              if not is_directory then
                Ok ()
              else Error "expected is_directory on a dangling symlink to report false"
          )
    )
  ...
  with_tempdir "kernel_new_file"
    (
      fun tempdir ->
        ...
        let* source_file = lift (Kernel.Fs.File.open_write source)
        in
        let* () =
          with_file source_file
            (
              fun () ->
                let* written = lift (Kernel.Fs.File.write source_file (Kernel.Bytes.from_string "new"))
                in
                if written = 3 then
                  Ok ()
                else Error "expected source fixture write to make progress"
            )
        in
        let* destination_file = lift (Kernel.Fs.File.open_write destination)
        in
        let* () =
          with_file destination_file
            (
              fun () ->
                let* written = lift (Kernel.Fs.File.write destination_file (Kernel.Bytes.from_string "old-old"))
                in
                if written = 7 then
                  Ok ()
                else Error "expected destination fixture write to make progress"
            )
        in
        let* () = lift (Kernel.Fs.File.copy ~src:source ~dst:destination)
        in
        let* destination_file = lift (Kernel.Fs.File.open_read destination)
        in
        let buffer = Kernel.Bytes.create ~size:16 in
        let* payload =
          with_file destination_file
            (
              fun () ->
                let* read = lift (Kernel.Fs.File.read destination_file buffer) in Ok (Kernel.Bytes.sub_string buffer ~offset:0 ~len:read)
            )
        in
        if payload = "new" then
          Ok ()
        else Error "expected copy to overwrite the destination contents instead of appending"
    )
      ...
      fun tempdir ->
        ...
        let* () =
          with_file source_file
            (
              fun () ->
                let* written = lift (Kernel.Fs.File.write source_file payload)
                in
                if written = Kernel.Bytes.length payload then
                  Ok ()
                else Error "expected large copy fixture write to write the whole payload"
            )
        in
        let* () = lift (Kernel.Fs.File.copy ~src:source ~dst:destination)
        in
        ...
        let* actual =
          with_file destination_file
            (
              fun () ->
                let* read = lift (Kernel.Fs.File.read destination_file buffer) in Ok (read, Kernel.Bytes.to_string buffer)
            )
        in
        match actual with
        | (read, contents) when read = Kernel.Bytes.length payload && contents = Kernel.Bytes.to_string payload -> Ok ()
          ...
          with_file source_file
            (
              fun () ->
                ...
                if written = Kernel.Bytes.length payload then
                  Ok ()
                else Error "expected copy-permissions fixture write to write the whole payload"
            )
        in
        let* () = lift (Kernel.Fs.File.set_permissions source ~perm:0o755)
        in
        let* () = lift (Kernel.Fs.File.copy ~src:source ~dst:destination)
        in
        let* source_metadata = lift (Kernel.Fs.File.metadata source)
        in
        let* destination_metadata = lift (Kernel.Fs.File.metadata destination)
        in
        if Kernel.Fs.File.Metadata.permissions source_metadata = Kernel.Fs.File.Metadata.permissions destination_metadata then
          Ok ()
        else Error "expected copy to preserve source permissions"
    )
      ...
      fun tempdir ->
        ...
        let* actual =
          with_file file
            (
              fun () ->
                let* written = lift (Kernel.Fs.File.write file (Kernel.Bytes.from_string "rename"))
                in
                if written != 6 then
                  Error "expected rename fixture write to make progress"
                else
                  let* before = lift (Kernel.Fs.File.fstat file)
                  in
                  let* () = lift (Kernel.Fs.File.rename ~src:original ~dst:moved)
                  in
                  let* after = lift (Kernel.Fs.File.fstat file)
                  in
                  let* by_path = lift (Kernel.Fs.File.metadata moved) in Ok (before, after, by_path)
            )
        in
        match actual with
        | (before, after, by_path) ->
            if Kernel.Fs.File.Metadata.dev before = Kernel.Fs.File.Metadata.dev after && Kernel.Fs.File.Metadata.dev after = Kernel.Fs.File.Metadata.dev by_path && Kernel.Fs.File.Metadata.ino before = Kernel.Fs.File.Metadata.ino after && Kernel.Fs.File.Metadata.ino after = Kernel.Fs.File.Metadata.ino by_path then
              Ok ()
            else Error "expected fstat to keep describing the same open file across a path rename"
    )
...
let main ~args = Test.Cli.main ~execution_mode:Test.Cli.Linear ~name:"kernel_new_file_tests" ~tests ~args ()
