...
let collect_names = fun dir ->
  let rec loop acc =
    let* next = lift (Kernel.Fs.ReadDir.read_name dir)
    in
    match next with
  ...
  with_tempdir "kernel_new_read_dir"
    (
      fun root ->
        ...
        let* () = lift_file (Kernel.Fs.File.create_dir child_dir ~perm:0o755)
        in
        let* file = lift_file (Kernel.Fs.File.open_write child_file)
        in
        let* () =
          with_file file
            (
              fun () ->
                let* _ = lift_file (Kernel.Fs.File.write file (Kernel.Bytes.from_string "alpha")) in Ok ()
            )
        in
        let* dir = lift (Kernel.Fs.ReadDir.open_dir root)
        in
        let later_file = Kernel.Path.(root / "later.txt") in
        let* later = lift_file (Kernel.Fs.File.open_write later_file)
        in
        let* () =
          with_file later
            (
              fun () ->
                let* _ = lift_file (Kernel.Fs.File.write later (Kernel.Bytes.from_string "later")) in Ok ()
            )
        in
        protect ~finally:(
          fun () ->
          ...
          (
            fun () ->
              ...
              if array_contains names "nested" && array_contains names "alpha.txt" && not (array_contains names "later.txt") && not (array_contains names ".") && not (array_contains names "..") then
                Ok ()
              else Error "expected directory iteration to return only the original snapshotted entry names"
          )
    )
  ...
  with_tempdir "kernel_new_read_dir"
    (
      fun root ->
        ...
        in
        let* () = lift_file (Kernel.Fs.File.symlink ~src:child_file ~dst:child_link)
        in
        protect ~finally:(
          fun () ->
          ...
          (
            fun () ->
              ...
              let rec loop saw_dir saw_file saw_link =
                let* next = lift (Kernel.Fs.ReadDir.read_entry dir)
                in
                match next with
                | None ->
                    if saw_dir && saw_file && saw_link then
                      Ok ()
                    else Error "expected read_entry to classify file, directory, and symlink entries"
                | Some entry ->
                    let saw_dir = saw_dir || (Kernel.String.equal (Kernel.Path.to_string entry.path) "nested" && entry.kind = Kernel.Fs.ReadDir.Directory) in
                    let saw_file = saw_file || (Kernel.String.equal (Kernel.Path.to_string entry.path) "alpha.txt" && entry.kind = Kernel.Fs.ReadDir.RegularFile) in
                    let saw_link = saw_link || (Kernel.String.equal (Kernel.Path.to_string entry.path) "alpha.link" && entry.kind = Kernel.Fs.ReadDir.SymbolicLink) in loop saw_dir saw_file saw_link
              in
              protect ~finally:(
                fun () ->
                  let _ = Kernel.Fs.ReadDir.close dir in ()
  ...
  with_tempdir "kernel_new_read_dir"
    (
      fun root ->
        ...
        let* dir = lift (Kernel.Fs.ReadDir.open_dir root)
        in
        let* () = lift_file (Kernel.Fs.File.remove_file child_file)
        in
        protect ~finally:(
          fun () ->
            let _ = Kernel.Fs.ReadDir.close dir in ()
  ...
  with_tempdir "kernel_new_read_dir"
    (
      fun root ->
        let* dir = lift (Kernel.Fs.ReadDir.open_dir root)
        in
        let* () = lift (Kernel.Fs.ReadDir.close dir)
        in
...
let main ~args = Test.Cli.main ~execution_mode:Test.Cli.Linear ~name:"kernel_new_read_dir_tests" ~tests ~args ()
