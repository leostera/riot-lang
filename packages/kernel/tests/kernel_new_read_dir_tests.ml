...
let lift result =
  ...
  | Kernel.Result.Error error ->
      Error (Kernel.Error.to_string (Kernel.Error.from_fs_read_dir error))
...
let tests = [
  Test.case
    "Fs.ReadDir snapshots names and excludes dot entries"
    test_open_dir_reads_snapshotted_names;
  Test.case "Fs.ReadDir read_entry reports current entry kinds" test_read_entry_reports_entry_kinds;
  Test.case
    "Fs.ReadDir read_entry surfaces removed snapshotted entries"
    test_read_entry_surfaces_removed_entries;
