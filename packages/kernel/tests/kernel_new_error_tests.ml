...
let test_error_envelope_reports_stable_module_tags = fun _ctx ->
  let read_dir = Kernel.Error.of_fs_read_dir Kernel.Fs.ReadDir.Closed in
  let net_addr = Kernel.Error.of_net_addr (Kernel.Net.Addr.HostNotFound { host = "localhost" }) in
...
let test_error_envelope_extracts_nested_read_dir_file_system_errors = fun _ctx ->
  let error = Kernel.Error.of_fs_read_dir
