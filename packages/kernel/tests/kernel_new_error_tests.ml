...
let test_error_envelope_extracts_nested_process_file_system_errors = fun _ctx ->
  let error =
    Kernel.Error.from_process
      (Kernel.Process.File (Kernel.Fs.File.System Kernel.SystemError.BadFileDescriptor))
  in
  if
...
let test_error_envelope_extracts_nested_read_dir_file_system_errors = fun _ctx ->
  let error =
    Kernel.Error.from_fs_read_dir
      (Kernel.Fs.ReadDir.File (Kernel.Fs.File.System Kernel.SystemError.NoSuchFileOrDirectory))
  in
  if
...
let test_system_error_of_code_preserves_unknown_payloads = fun _ctx ->
  ...
  | Kernel.SystemError.Unknown 9_999 ->
      if
        Kernel.SystemError.to_string (Kernel.SystemError.Unknown 9_999) = "unknown kernel error"
      then
        Ok ()
      else
...
let test_error_to_string_prefixes_the_stable_module_name = fun _ctx ->
  let error =
    Kernel.Error.from_net_addr (Kernel.Net.Addr.HostNotFound { host = "riot.invalid" })
...
let tests = [
  Test.case
    "Error envelope reports module and system context"
    test_error_envelope_reports_module_and_system;
  Test.case
    "Error envelope extracts nested process file system errors"
    test_error_envelope_extracts_nested_process_file_system_errors;
  Test.case
    "Error envelope reports stable module tags"
    test_error_envelope_reports_stable_module_tags;
  Test.case
    "Error envelope keeps non-system errors distinct"
    test_error_envelope_system_is_none_for_non_system_errors;
  Test.case
    "Error envelope extracts nested read_dir file system errors"
    test_error_envelope_extracts_nested_read_dir_file_system_errors;
  Test.case
    "SystemError.from_code maps representative known values"
    test_system_error_of_code_maps_known_values;
  Test.case
    "SystemError.from_code preserves unknown payloads"
    test_system_error_of_code_preserves_unknown_payloads;
  Test.case
    "Error.to_string prefixes the stable module name"
    test_error_to_string_prefixes_the_stable_module_name;
]
