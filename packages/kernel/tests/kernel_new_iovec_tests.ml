...
let test_iovec_roundtrips_string_payload = fun _ctx ->
  ...
  if Kernel.String.equal (Kernel.IO.IoVec.to_string iovec) "hello riot" then
    Ok ()
  else Error "expected iovec string roundtrip to preserve segment order"
...
let test_iovec_sub_slices_segments = fun _ctx ->
  ...
  if Kernel.String.equal actual "lo ri" then
    Ok ()
  else Error "expected iovec slicing to preserve partial segment boundaries"

let tests = [ Test.case "IoVec string roundtrips preserve payload" test_iovec_roundtrips_string_payload; Test.case "IoVec sub slices across segment boundaries" test_iovec_sub_slices_segments ]
