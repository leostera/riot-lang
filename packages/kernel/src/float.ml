...
let to_string = fun ?(precision = 6) value ->
  let precision =
    ...
    | Order.LT -> 0
    | Order.EQ | Order.GT -> precision
  in
  let format = String.concat "" [ "%."; Int.to_string precision; "f" ] in Caml_runtime.format_float format value
...
let cbrt = fun value ->
  ...
  | Order.EQ | Order.GT -> Caml_runtime.pow_float value exponent
