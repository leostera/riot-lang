...
type t = { mutable storage: IoSlice.t; mutable start: int; mutable len: int }
...
let compact = fun buffer ->
  if buffer.start = 0 then
    ...
    if buffer.len = 0 then
      ...
      (
        IoSlice.blit_unchecked ~src:buffer.storage ~src_off:buffer.start ~dst:buffer.storage ~dst_off:0 ~len:buffer.len;
        buffer.start <- 0
      )
...
let ensure_free = fun buffer needed ->
  if needed < 0 then
    ...
    if writable_bytes buffer >= needed then
      Ok ()
    else
      if capacity buffer - buffer.len >= needed then
        (
          compact buffer;
          Ok ()
        )
        ...
        (
          ...
          let grown =
            if current_capacity = 0 then
              ...
              if current_capacity * 2 > min_capacity then
                current_capacity * 2
              else min_capacity
          in
...
let writable = fun buffer -> IoSlice.sub_unchecked buffer.storage ~off:(buffer.start + buffer.len) ~len:(writable_bytes buffer)

let commit = fun buffer requested ->
  if requested < 0 then
    Error (Error.Negative_length requested)
  else
    if requested > writable_bytes buffer then
...
let append_at_end = fun buffer len append ->
  ...
  | Error _ as error -> error
  | Ok () -> append buffer.storage (buffer.start + buffer.len) len |> Result.and_then ~fn:(
    fun () -> commit buffer len
  )

let append_string = fun buffer value ->
  let len = String.length value in
  append_at_end buffer len
    (
...
let get = fun buffer ~at ->
  ...
  else Ok (IoSlice.get_unchecked buffer.storage ~at:(buffer.start + at))
