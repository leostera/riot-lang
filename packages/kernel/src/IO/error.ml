...
let message = function
  ...
  | Index_out_of_bounds { buffer_length; at } -> "index out of bounds: index="
  ^ Int.to_string at
  ^ ", length="
  ^ Int.to_string buffer_length
  | Range_out_of_bounds { buffer_length; offset; len } -> "range out of bounds: offset="
  ^ Int.to_string offset
  ^ ", len="
  ^ Int.to_string len
  ^ ", length="
  ^ Int.to_string buffer_length
  | Shift_out_of_bounds { buffer_length; by } -> "shift out of bounds: by="
  ^ Int.to_string by
  ^ ", length="
  ^ Int.to_string buffer_length
  | Split_out_of_bounds { buffer_length; at } -> "split out of bounds: at="
  ^ Int.to_string at
  ^ ", length="
  ^ Int.to_string buffer_length
  | Commit_out_of_bounds { writable_bytes; requested } -> "commit out of bounds: requested="
  ^ Int.to_string requested
  ^ ", writable="
  ^ Int.to_string writable_bytes
  | Consume_out_of_bounds { readable_bytes; requested } -> "consume out of bounds: requested="
  ^ Int.to_string requested
  ^ ", readable="
