...
let of_write_src : type dst err. (dst, err) write -> dst -> (dst, err) t = fun write dst -> Writer (
  write,
  dst
)

let write : type dst err. (dst, err) t -> buf:string -> (int, err) result = fun (Writer ((module W), dst)) ~buf -> W.write
dst
~buf

let write_owned_vectored : type dst err. (dst, err) t -> bufs:Iovec.t -> (int, err) result = fun (Writer ((module W), dst)) ~bufs -> W.write_owned_vectored
dst
~bufs

let flush : type dst err. (dst, err) t -> (unit, err) result = fun (Writer ((module W), dst)) -> W.flush
dst
...
let write_all : type dst err. (dst, err) t -> buf:string -> (unit, err) result = fun (Writer ((module W), dst)) ~buf ->
  ...
  let rec write_loop = fun buf len ->
...
let write_all_vectored : type dst err. (dst, err) t -> bufs:Iovec.t -> (unit, err) result = fun (Writer ((module W), dst)) ~bufs ->
  ...
  let rec write_loop = fun bufs len ->
