...
external unsafe_blit: src:t -> src_offset:int -> dst:t -> dst_offset:int -> len:int -> unit = "kernel_new_iovec_slice_blit" [@@noalloc]

external unsafe_blit_from_bytes: bytes -> src_offset:int -> dst:t -> dst_offset:int -> len:int -> unit = "kernel_new_iovec_slice_blit_from_bytes" [@@noalloc]

external unsafe_blit_from_string: string -> src_offset:int -> dst:t -> dst_offset:int -> len:int -> unit = "kernel_new_iovec_slice_blit_from_string" [@@noalloc]

external unsafe_blit_to_bytes: src:t -> src_offset:int -> dst:bytes -> dst_offset:int -> len:int -> unit = "kernel_new_iovec_slice_blit_to_bytes" [@@noalloc]
...
let validate_range = fun buffer_len ~off ~len ->
  if off < 0 then
    ...
    if len < 0 then
      ...
      if off > buffer_len || len > buffer_len - off then
...
let shift = fun value by ->
  ...
  else Ok (unsafe_sub value ~offset:by ~len:(value_len - by))
...
let split_at = fun value at ->
  ...
  else Ok (unsafe_sub value ~offset:0 ~len:at, unsafe_sub value ~offset:at ~len:(value_len - at))
  ...
  if prefix_len > length value then
    ...
    let rec loop index =
      if index >= prefix_len then
        ...
        if get_unchecked value ~at:index != String.get_unchecked prefix ~at:index then
          false
        else loop (index + 1)
    in
    loop 0
...
let index_char = fun value needle ->
  let rec loop index =
    if index >= length value then
      ...
      if get_unchecked value ~at:index = needle then
        Some index
      else loop (index + 1)
  in
  loop 0
  ...
  if needle_len = 0 then
    ...
    let rec matches start needle_index =
      if needle_index >= needle_len then
        ...
        if get_unchecked value ~at:(start + needle_index) != String.get_unchecked needle ~at:needle_index then
          false
        else matches start (needle_index + 1)
    in
    let rec loop index =
