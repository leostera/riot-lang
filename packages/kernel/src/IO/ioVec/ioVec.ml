...
let create = fun ?(count = 1) ~size () ->
  if count <= 0 then
    ...
    if size < 0 then
      ...
      let remainder = size mod count in Ok (Array.init ~count ~fn:(
        fun index ->
          let chunk =
...
let length = fun segments -> Array.fold_left segments ~fn:(
  fun total segment -> total + IoSlice.length segment
) ~acc:0
...
let sub = fun ?(pos = 0) ~len segments ->
  ...
  if pos < 0 then
    ...
    if len < 0 then
      ...
      if pos > total || len > total - pos then
...
let to_bytes = fun segments ->
  ...
  let rec loop index cursor =
    if index >= Array.length segments then
      ...
      let segment = Array.get_unchecked segments ~at:index in IoSlice.blit_to_bytes_unchecked segment ~src_off:0 out ~dst_off:cursor ~len:(IoSlice.length segment);
    loop (index + 1) (cursor + IoSlice.length segment)
  in
  let _ = loop 0 0 in out
