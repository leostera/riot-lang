...
let sub = fun ?(pos = 0) ~len t ->
  let curr = Cell.create 0 in
  t |> Array.to_list |> List.filter_map
    (fun iov ->
...
let from_buffer = fun buf -> of_bytes (Buffer.to_bytes buf)

let into_string = fun t ->
  let buf = Buffer.create (length t) in
  iter t
    (fun iov ->
      Buffer.add_bytes buf (Bytes.sub iov.ba iov.off iov.len));
  Buffer.contents buf
