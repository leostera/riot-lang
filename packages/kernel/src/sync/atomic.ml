...
let exchange = fun atomic value ->
  Loc.exchange [%atomic.loc atomic.contents] value

let compare_and_set = fun atomic current next ->
  Loc.compare_and_set [%atomic.loc atomic.contents] current next

let fetch_and_add = fun atomic incr ->
  Loc.fetch_and_add [%atomic.loc atomic.contents] incr
