...
let error_to_string = fun value ->
  match value with
  | InvalidBacklog { backlog } -> String.concat "" [ "invalid listener backlog: "; Int.to_string backlog ]
...
let bind = fun ?(reuse_addr = true) ?(reuse_port = false) ?(backlog = 128) addr ->
  if backlog <= 0 then
    ...
    let port = Socket_addr.port addr in FFI.bind ip port reuse_addr reuse_port backlog |> Result.map_err ~fn:(
      fun code -> error_of_system (System_error.from_code code)
    )

let accept = fun listener ->
  let* (stream, addr) = FFI.accept listener |> Result.map_err ~fn:(
    fun code -> error_of_system (System_error.from_code code)
  )
  in
  let* addr = socket_addr_of_pair addr in Result.Ok (stream, addr)

let close = fun listener -> FFI.close listener |> Result.map_err ~fn:(
  fun code -> error_of_system (System_error.from_code code)
)

let local_addr = fun listener ->
  let* addr = FFI.local_addr listener |> Result.map_err ~fn:(
    fun code -> error_of_system (System_error.from_code code)
