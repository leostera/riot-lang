...
let connect addr =
  ...
  try
    retry_eintr (fun () -> Unix.connect (Fd.to_unix fd) sock_addr);
...
let read fd ?(pos = 0) ?len buf =
  ...
  try Ok (retry_eintr (fun () -> UnixLabels.read (Fd.to_unix fd) ~buf ~pos ~len))
  with Unix.Unix_error (err, _, _) -> Error (IO.error_of_unix err)

let write fd ?(pos = 0) ?len buf =
  let len = Option.unwrap_or len ~default:(Bytes.length buf - 1) in
  try Ok (retry_eintr (fun () -> UnixLabels.write (Fd.to_unix fd) ~buf ~pos ~len))
  with Unix.Unix_error (err, _, _) -> Error (IO.error_of_unix err)

external std_sys_readv : Unix.file_descr -> Iovec.t -> int = "kernel_unix_readv"

let read_vectored fd iov =
  try Ok (retry_eintr (fun () -> std_sys_readv (Fd.to_unix fd) iov))
  with Unix.Unix_error (err, _, _) -> Error (IO.error_of_unix err)
...
let write_vectored fd iov =
  try Ok (retry_eintr (fun () -> std_sys_writev (Fd.to_unix fd) iov))
  with Unix.Unix_error (err, _, _) -> Error (IO.error_of_unix err)
...
let sendfile fd ~file ~off ~len =
  try Ok (retry_eintr (fun () -> std_sys_sendfile (Fd.to_unix file) (Fd.to_unix fd) off len))
  with Unix.Unix_error (err, _, _) -> Error (IO.error_of_unix err)
