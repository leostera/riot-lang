...
let resolver_error_of_code = fun ~host code ->
  match code with
  | value when value = resolver_error_host_not_found -> HostNotFound { host }
  | value when value = resolver_error_temporary_failure -> TemporaryFailure { host }
  | value when value = resolver_error_resolution_failed -> ResolutionFailed { host }
  | other -> (
...
let resolve_all = fun ~kind ~host ~port ->
  ...
  in
  let* literal = resolve_literal ~host ~port
  in
  match literal with
  | Some addrs -> Result.Ok addrs
  | None ->
      let* raw_addrs = Result.map_err (FFI.resolve host port kind) ~fn:(resolver_error_of_code ~host)
      in
      if Array.length raw_addrs = 0 then
        Result.Error (NoAddressesFound { host; port })
      else
        let* first_addr = socket_addr_of_pair (Array.get_unchecked raw_addrs ~at:0)
        in
        ...
        let rec build index =
          if index >= Array.length raw_addrs then
            ...
            let* addr = socket_addr_of_pair (Array.get_unchecked raw_addrs ~at:index) in Array.set out ~at:index ~value:addr;
          build (index + 1)
        in
        build 0
...
let resolve_first_stream = fun ~host ~port ->
  let* addrs = resolve_stream ~host ~port in Result.Ok (Array.get_unchecked addrs ~at:0)
...
let resolve_first_datagram = fun ~host ~port ->
  let* addrs = resolve_datagram ~host ~port in Result.Ok (Array.get_unchecked addrs ~at:0)
