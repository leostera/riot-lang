...
external create_client_engine : hostname:string -> engine
  = "kernel_tls_create_client_engine"

external create_server_engine : cert_file:string -> key_file:string -> engine
  = "kernel_tls_create_server_engine"

external pump_encrypted_in : engine -> bytes -> pos:int -> len:int -> int
  = "kernel_tls_pump_encrypted_in"

external read_encrypted_out : engine -> bytes -> int
  = "kernel_tls_read_encrypted_out"

external _read_decrypted : engine -> bytes -> pos:int -> len:int -> int
  = "kernel_tls_read_decrypted"

external _write_plaintext : engine -> bytes -> pos:int -> len:int -> int
  = "kernel_tls_write_plaintext"
...
type write_result = Written of int | Need_network_read | Need_network_write

(* Wrapper for read_decrypted *)
let read_decrypted engine buf ~pos ~len =
  let n = _read_decrypted engine buf pos len in
  if n > 0 then Read n
  else if n = 0 then Eof
  else if n = -1 then Need_network_read
  else if n = -2 then Need_network_write
  else Eof  (* Shouldn't happen, treat as EOF *)

(* Wrapper for write_plaintext *)
let write_plaintext engine buf ~pos ~len =
  let n = _write_plaintext engine buf pos len in
  if n > 0 then Written n
  else if n = -1 then Need_network_read
  else if n = -2 then Need_network_write
  else Need_network_write  (* Shouldn't happen, retry write *)
