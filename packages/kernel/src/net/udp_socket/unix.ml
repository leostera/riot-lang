...
let bind = fun ?(reuse_addr = true) ?(reuse_port = false) addr ->
  ...
  let port = Socket_addr.port addr in Result.map_err (FFI.bind ip port reuse_addr reuse_port) ~fn:(
    fun code -> error_of_system (System_error.from_code code)
  )

let connect = fun socket addr ->
  let ip = Ip_addr.to_string (Socket_addr.ip addr) in
  let port = Socket_addr.port addr in Result.map_err (FFI.connect socket ip port) ~fn:(
    fun code -> error_of_system (System_error.from_code code)
  )

let close = fun socket -> Result.map_err (FFI.close socket) ~fn:(
  fun code -> error_of_system (System_error.from_code code)
)

let local_addr = fun socket ->
  let* addr = Result.map_err (FFI.local_addr socket) ~fn:(
    fun code -> error_of_system (System_error.from_code code)
  ) in socket_addr_of_pair addr

let recv = fun socket ?(pos = 0) ?len buf ->
  let len = Option.unwrap_or len ~default:(Bytes.length buf - pos) in
  let* () = validate_slice buf ~pos ~len in Result.map_err (FFI.recv socket buf pos len) ~fn:(
    fun code -> error_of_system (System_error.from_code code)
  )

let recv_from = fun socket ?(pos = 0) ?len buf ->
  let len = Option.unwrap_or len ~default:(Bytes.length buf - pos) in
  let* () = validate_slice buf ~pos ~len
  in
  let* (read_count, addr) = Result.map_err (FFI.recv_from socket buf pos len) ~fn:(
    fun code -> error_of_system (System_error.from_code code)
  )
  in
  let* addr = socket_addr_of_pair addr in Result.Ok (read_count, addr)

let send = fun socket ?(pos = 0) ?len buf ->
  let len = Option.unwrap_or len ~default:(Bytes.length buf - pos) in
  let* () = validate_slice buf ~pos ~len in Result.map_err (FFI.send socket buf pos len) ~fn:(
    fun code -> error_of_system (System_error.from_code code)
  )
...
let send_to = fun socket addr ?(pos = 0) ?len buf ->
  ...
  in
  let ip = Ip_addr.to_string (Socket_addr.ip addr) in
  let port = Socket_addr.port addr in Result.map_err (FFI.send_to socket ip port buf (pos, len)) ~fn:(
    fun code -> error_of_system (System_error.from_code code)
  )
