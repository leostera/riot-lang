...
module Raw = struct
  ...
  type 'value term_sync = { mutable state: 'value state; mut: Sync.Mutex.t; cond: Sync.Condition.t }
...
type 'value t = { domain: Raw.id; term_sync: 'value Raw.term_sync }
...
module DLS = struct
  ...
  external compare_and_set_dls_state: dls_state -> dls_state -> bool = "caml_domain_dls_compare_and_set" [@@noalloc]

  let create_dls = fun () ->
    let state = Array.make ~count:8 ~value:Obj_opt.none in set_dls_state state
  ...
  let rec maybe_grow = fun index ->
    ...
    if index < size then
      ...
      in
      let grown = Array.make ~count:(next_size size) ~value:Obj_opt.none in Array.blit state ~src_offset:0 ~dst:grown ~dst_offset:0 ~len:size;
    if compare_and_set_dls_state state grown then
      grown
    else maybe_grow index

  let set = fun ((index, _init): 'value key) value ->
    let state = maybe_grow index in Array.set_unchecked state ~at:index ~value:(Obj_opt.some (opaque_identity value))
  ...
  let get = fun ((index, init): 'value key) ->
    ...
    if Obj_opt.is_some value then
      ...
      if array_compare_and_set state index value packed then
        ...
        else System_error.panic "Thread.DLS.get observed an uninitialized slot after compare-and-set"

  type key_value =
    | KV : 'value key * 'value -> key_value

  let get_initial_keys = fun () -> List.map (Sync.Atomic.get parent_keys) ~fn:(
    fun (KI (key, split)) -> KV (key, split (get key))
  )

  let set_initial_keys = fun values -> List.for_each values ~fn:(
    fun (KV (key, value)) -> set key value
  )
end
...
let spawn = fun fn ->
  ...
  let term_sync = Raw.{ state = Running; mut = Sync.Mutex.create (); cond = Sync.Condition.create () } in
  let body () =
    DLS.create_dls ();
    DLS.set_initial_keys initial_keys;
    fn ()
  in
  let domain = Raw.spawn body term_sync in { domain; term_sync }
