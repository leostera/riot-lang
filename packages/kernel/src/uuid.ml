...
let of_string str =
  try Result.Ok (of_string_native str)
  with Invalid_argument msg -> Result.Error (`Invalid_uuid msg)
...
let equal a b = Stdlib.Bytes.equal a b

let to_bytes uuid = Stdlib.Bytes.copy uuid

let of_bytes bytes =
  if Stdlib.Bytes.length bytes = 16 then Result.Ok (Stdlib.Bytes.copy bytes)
  else Result.Error (`Invalid_uuid "UUID must be exactly 16 bytes")

let version uuid =
  if Stdlib.Bytes.length uuid < 7 then None
  else
    let byte6 = Stdlib.Bytes.get uuid 6 |> Stdlib.Char.code in
    let ver = (byte6 lsr 4) land 0x0F in
    if ver >= 1 && ver <= 8 then Some ver else None
