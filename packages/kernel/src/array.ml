...
let make = fun ~count ~value -> Caml_runtime.array_make count value
...
let get = fun values ~at ->
  if at < 0 || at >= length values then
    None
  else Some (Caml_runtime.array_get values at)

let get_unchecked = fun values ~at -> Caml_runtime.array_unsafe_get values at

let set = fun values ~at ~value ->
  if at < 0 || at >= length values then
    System_error.panic ("Array.set received an out-of-bounds index: " ^ Int.to_string at)
  else Caml_runtime.array_set values at value

let set_unchecked = fun values ~at ~value -> Caml_runtime.array_unsafe_set values at value
...
let clone = fun values ->
  ...
  if count = 0 then
    ...
    fun index -> get_unchecked values ~at:index
  )
...
let blit = fun source ~src_offset ~dst ~dst_offset ~len ->
  if len <= 0 then
    ...
    if Ptr.equal source dst && dst_offset > src_offset then
      let rec loop index =
        if index < 0 then
          ()
        else
          (
            set_unchecked dst ~at:(dst_offset + index) ~value:(get_unchecked source ~at:(src_offset + index));
            loop (index - 1)
          )
      in
      loop (len - 1)
...
let sub = fun values ~offset ~len ->
  if len = 0 then
    [||]
  else init ~count:len ~fn:(
    fun index -> get_unchecked values ~at:(offset + index)
  )
...
let fold_left = fun array ~fn ~acc ->
  let rec loop index acc =
    if index >= length array then
      acc
    else loop (index + 1) (fn acc (get_unchecked array ~at:index))
  in
  loop 0 acc
...
let fold_right = fun array ~fn ~acc ->
  let rec loop index acc =
    if index < 0 then
      acc
    else loop (index - 1) (fn (get_unchecked array ~at:index) acc)
  in
