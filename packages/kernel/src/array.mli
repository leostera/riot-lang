...
(** Use `unsafe_get values index` only when `index` is already proven in bounds. *)
val unsafe_get: 'value t -> int -> 'value

(** Use `unsafe_set values index value` only when `index` is already proven in bounds. *)
val unsafe_set: 'value t -> int -> 'value -> unit
