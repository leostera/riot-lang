...
let sub = fun value start slice_length ->
  let value_length = length value in
  if start < 0 || slice_length < 0 || start > value_length - slice_length then
    raise (Invalid_argument "String.sub");
  let out = Caml_runtime.bytes_create slice_length in
  Caml_runtime.string_blit value start out 0 slice_length;
  ...
  if needle_length = 0 then
    ...
    let rec matches offset index =
      if index >= needle_length then
        true
      else if get haystack (offset + index) != get needle index then
        false
      else
  ...
  if prefix_length > value_length then
    ...
    let rec loop index =
      if index >= prefix_length then
        true
      else if get prefix index != get value index then
        false
      else
        loop (index + 1)
    in
  ...
  if suffix_length > value_length then
    ...
    let rec loop index =
      if index >= suffix_length then
        true
      else if get suffix index != get value (offset + index) then
        false
      else
        loop (index + 1)
    in
...
let index_opt = fun value needle ->
  let rec loop index =
    if index >= length value then
      None
    else if get value index = needle then
      Some index
    else
      loop (index + 1)
  in
...
let trim = fun value ->
  ...
  let rec find_start index =
    if index >= value_length then
      value_length
    else if is_trim_char (get value index) then
      find_start (index + 1)
    else
      index
  in
  ...
  if finish < start then
    ...
    sub value start (finish - start + 1)

let split_on_char = fun separator value ->
  let value_length = length value in
  let rec loop start index acc =
    if index >= value_length then
      List.rev (sub value start (value_length - start) :: acc)
    else if get value index = separator then
      loop (index + 1) (index + 1) (sub value start (index - start) :: acc)
    else
...
let lowercase_ascii = fun value ->
  init (length value) (fun index -> lowercase_ascii_char (get value index))

let capitalize_ascii = fun value ->
  if is_empty value then
    empty
  else
    let first = get value 0 in
...
let map = fun fn value -> init (length value) (fun index -> fn (get value index))

let iter = fun fn value ->
  let rec loop index =
    if index >= length value then
      ()
    else (
...
let exists = fun predicate value ->
  let rec loop index =
    if index >= length value then
      false
    else if predicate (get value index) then
      true
    else
      loop (index + 1)
  in
...
let for_all = fun fn value ->
  let rec loop index =
    if index >= length value then
      true
    else if fn (get value index) then
      loop (index + 1)
    else
      false
  in
...
let fold_left = fun fn init value ->
  let rec loop index acc =
    if index >= length value then
      acc
    else
      loop (index + 1) (fn acc (get value index))
  in
  loop 0 init

let escaped =
  let decimal_digit value = Char.unsafe_of_int (Char.to_int '0' + value) in
...
let get_utf_8_uchar =
  ...
  let utf_8_uchar_2 b0 b1 = ((b0 land 0x1f) lsl 6) lor (b1 land 0x3f) in
  let utf_8_uchar_3 b0 b1 b2 = ((b0 land 0x0f) lsl 12) lor ((b1 land 0x3f) lsl 6) lor (b2 land 0x3f) in
  let utf_8_uchar_4 b0 b1 b2 b3 = ((b0 land 0x07) lsl 18)
  lor ((b1 land 0x3f) lsl 12)
  lor ((b2 land 0x3f) lsl 6)
  lor (b3 land 0x3f) in
  fun source index ->
    let b0 = get source index |> Char.to_int in
    let max_index = length source - 1 in
    let get_byte byte_index = get source byte_index |> Char.to_int in
    match Char.unsafe_of_int b0 with
    | '\x00' .. '\x7F' ->
        Unicode.Rune.utf_decode 1 (Unicode.Rune.unsafe_of_int b0)
    | '\xC2' .. '\xDF' ->
        ...
        if index > max_index then
          ...
          if not_in_x80_to_xBF b1 then
            ...
            Unicode.Rune.utf_decode 2 (Unicode.Rune.unsafe_of_int (utf_8_uchar_2 b0 b1))
    | '\xE0' ->
        let index = index + 1 in
        if index > max_index then
          Unicode.Rune.utf_decode_invalid 1
          ...
          if not_in_xA0_to_xBF b1 then
            ...
            if index > max_index then
              ...
              if not_in_x80_to_xBF b2 then
                ...
                Unicode.Rune.utf_decode 3 (Unicode.Rune.unsafe_of_int (utf_8_uchar_3 b0 b1 b2))
    | '\xE1' .. '\xEC'
    | '\xEE' .. '\xEF' ->
        let index = index + 1 in
        if index > max_index then
                ...
                if index > max_index then
                  ...
                  if not_in_x80_to_xBF b3 then
                    ...
                    Unicode.Rune.utf_decode
                      4
                      (Unicode.Rune.unsafe_of_int (utf_8_uchar_4 b0 b1 b2 b3))
    | '\xF1' .. '\xF3' ->
        let index = index + 1 in
        if index > max_index then
          Unicode.Rune.utf_decode_invalid 1
