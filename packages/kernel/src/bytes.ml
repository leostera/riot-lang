...
let get_unchecked = fun value ~at ->
  Caml_runtime.bytes_get value at
...
let blit = fun src ~src_offset ~dst ~dst_offset ~len ->
  ...
  if src_offset < 0 || len < 0 || src_offset + len > src_len then
    Error (OutOfBoundSet { bytes = src; lenght = src_len; at = src_offset; char = '\000' })
  else if dst_offset < 0 || len < 0 || dst_offset + len > dst_len then
    Error (OutOfBoundSet { bytes = dst; lenght = dst_len; at = dst_offset; char = '\000' })
  else
...
let fill = fun value ~offset ~len ~char ->
  Caml_runtime.bytes_fill value offset len char
