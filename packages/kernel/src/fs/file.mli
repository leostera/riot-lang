...
val write_vectored : t -> Iovec.t -> (int, error) result
...
val lseek : t -> int64 -> seek_command -> (int64, error) result
...
val lockf : t -> lock_command -> int -> (unit, error) result

val close_fd : t -> (unit, error) result

val get_temp_dir : unit -> (string, error) result

val temp_dir : ?temp_dir:string -> string -> string -> (string, error) result
