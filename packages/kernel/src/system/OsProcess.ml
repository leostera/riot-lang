...
let spawn ~program ~args ?(env = []) ?cwd ~stdio () =
  try
    ...
    let stdin_read, stdin_write, stdin_child, stdin_parent =
      ...
      | `File fd -> (Fd.to_unix fd, None, Fd.to_unix fd, None)
    in
...
let close t =
  ...
  (match t.stdout_fd with Some fd -> Fd.close fd | None -> ());
  (match t.stderr_fd with Some fd -> Fd.close fd | None -> ());
  (match t.stdin_fd with Some fd -> Fd.close fd | None -> ());
  ()
