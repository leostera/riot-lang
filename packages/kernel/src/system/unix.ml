...
module Host = struct
  ...
  let equal = fun left right -> left.architecture = right.architecture && left.vendor = right.vendor && left.os = right.os && left.abi = right.abi
  ...
  let substring = fun value ~offset ~len -> value |> Bytes.from_string |> Bytes.sub_unchecked ~offset ~len |> Bytes.to_string
    ...
    let rec loop start index acc =
      if index >= length then
        ...
        if String.get_unchecked value ~at:index = '-' then
          let part = substring value ~offset:start ~len:(index - start) in loop (index + 1) (index + 1) (part :: acc)
