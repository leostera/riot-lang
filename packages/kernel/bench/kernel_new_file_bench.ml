...
let with_file = fun file fn ->
  try
    ...
    let _ = Kernel.Fs.File.close file in value
  with
  | error ->
...
let vectored_payload = Kernel.IO.IoVec.from_string_array (Kernel.Array.init ~count:4 ~fn:(
  fun _ -> Kernel.String.make ~len:1_024 ~char:'x'
)) |> Result.unwrap
      ...
      fun tempdir ->
        let rec create_many index =
          if index = 128 then
            ...
            let name = format Std.Format.[ str "entry-"; int index; str ".txt" ] in
