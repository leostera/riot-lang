...
let bench_vectored_read = fun () ->
  with_temp_path
    ...
    (fun path ->
      ...
      | Kernel.Result.Ok file ->
          let iov =
            Kernel.IO.IoVec.create ~count:4 ~size:1_024 ()
            |> Result.unwrap
          in
          with_file
            file
...
let benchmarks =
  Bench.[
    ...
    with_config
      ~config:{ iterations = 20; warmup = 5 }
      "file partial read: 2KiB@512"
      bench_partial_read;
    with_config
      ~config:{ iterations = 20; warmup = 5 }
      "file vectored read: 4 x 1KiB"
      bench_vectored_read;
    ...
    with_config
      ...
      "file read_dir_names: 128 entries"
      bench_read_dir_names_large;
  ]
