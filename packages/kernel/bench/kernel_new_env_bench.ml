...
let benchmarks = Bench.[ with_config ~config:{ iterations = 100; warmup = 10 } "env get existing var" bench_get_var; with_config ~config:{ iterations = 100; warmup = 10 } "env current_dir" bench_current_dir; with_config ~config:{ iterations = 50; warmup = 10 } "env vars snapshot" bench_vars ]
