...
let bench_now = fun () ->
  ...
  | Kernel.Result.Ok _ -> ()
  | Kernel.Result.Error error -> Kernel.SystemError.panic (Kernel.Error.to_string (Kernel.Error.from_time_system_time error))
...
let bench_compare = fun () ->
  ...
  | (Kernel.Result.Error error, _) | (_, Kernel.Result.Error error) -> Kernel.SystemError.panic (Kernel.Error.to_string (Kernel.Error.from_time_system_time error))

let bench_monotonic_now = fun () ->
  match Kernel.Time.Monotonic.now () with
  | Kernel.Result.Ok _ -> ()
  | Kernel.Result.Error error -> Kernel.SystemError.panic (Kernel.Error.to_string (Kernel.Error.from_time_monotonic error))
...
let bench_monotonic_compare = fun () ->
  ...
  | (Kernel.Result.Error error, _) | (_, Kernel.Result.Error error) -> Kernel.SystemError.panic (Kernel.Error.to_string (Kernel.Error.from_time_monotonic error))
  ...
  with_poll
    (
      fun poll ->
        ...
        | Kernel.Result.Error error -> Kernel.SystemError.panic (Kernel.Error.to_string (Kernel.Error.from_time_timer error))
          ...
          (
            fun () ->
              let rec register index = function
                | [] -> ()
                | timer :: rest ->
                    let _ = Kernel.Async.Poll.register poll (Kernel.Async.Token.make index) Kernel.Async.Interest.readable (Kernel.Time.Timer.to_source timer) in register (index + 1) rest
              in
