...
let build_request = fun ~header_count ~body_len ->
  let header_lines = Kernel.Array.init ~count:header_count ~fn:(
    fun index -> "x-header-" ^ Int.to_string index ^ ": value-" ^ Int.to_string index ^ "\r\n"
  ) |> Kernel.Array.fold_left ~acc:"" ~fn:(
    fun acc line -> acc ^ line
  ) in "POST /benchmark HTTP/1.1\r\n" ^ header_lines ^ "\r\n" ^ String.make ~len:body_len ~char:'x'
...
let bench_sub_and_advance = fun slice () ->
  let _ =
    slice |> fun slice -> Kernel.IO.IoVec.IoSlice.shift slice 5 |> Result.unwrap |> Kernel.IO.IoVec.IoSlice.sub ~off:0 ~len:32 |> Result.unwrap
  in
  ()
...
let benchmarks = Bench.[
  with_config ~config:{ iterations = 20; warmup = 5 } "io_slice starts_with: small request" (bench_starts_with small_slice "POST ");
  with_config ~config:{ iterations = 20; warmup = 5 } "io_slice index_of_char ' ': medium request" (bench_index_of_char medium_slice ' ');
  with_config ~config:{ iterations = 20; warmup = 5 } "io_slice index_of_string CRLFCRLF: medium request" (bench_index_of_string medium_slice "\r\n\r\n");
  with_config ~config:{ iterations = 10; warmup = 3 } "io_slice sub+advance: large request" (bench_sub_and_advance large_slice);
  with_config ~config:{ iterations = 10; warmup = 3 } "io_slice to_string: medium request" (bench_to_string medium_slice);
]
