...
let build_request = fun ~header_count ~body_len ->
  let header_lines =
    Kernel.Array.init
      ~count:header_count
      ~fn:(fun index ->
        "x-header-" ^ Int.to_string index ^ ": value-" ^ Int.to_string index ^ "\r\n")
    |> Kernel.Array.fold_left ~acc:"" ~fn:(fun acc line -> acc ^ line)
  in
  "POST /benchmark HTTP/1.1\r\n" ^ header_lines ^ "\r\n" ^ String.make ~len:body_len ~char:'x'

let small_slice =
  Kernel.IO.IoVec.IoSlice.from_string (build_request ~header_count:4 ~body_len:128)
  |> Result.unwrap

let medium_slice =
  Kernel.IO.IoVec.IoSlice.from_string (build_request ~header_count:32 ~body_len:4_096)
  |> Result.unwrap

let large_slice =
  Kernel.IO.IoVec.IoSlice.from_string (build_request ~header_count:64 ~body_len:65_536)
  |> Result.unwrap
