...
let with_poll = fun fn ->
  ...
  | Kernel.Result.Ok poll -> (
    try
      ...
      let _ = Kernel.Async.Poll.close poll in result
    with
    | error ->
  ...
  with_poll
    (
      fun poll ->
        ...
        let _ = Kernel.Async.Poll.register poll (Kernel.Async.Token.make "stdin") Kernel.Async.Interest.readable source in
  ...
  with_poll
    (
      fun poll ->
        ...
        let _ = Kernel.Async.Poll.register poll (Kernel.Async.Token.make "stdout") Kernel.Async.Interest.writable source in
        let _ = Kernel.Async.Poll.poll ~timeout:0L poll in
        let _ = Kernel.Async.Poll.deregister poll source in ()
    )
  ...
  with_poll
    (
      fun poll ->
        ...
        let _ = Kernel.Async.Poll.register poll (Kernel.Async.Token.make "stderr") Kernel.Async.Interest.writable source in
        let _ = Kernel.Async.Poll.poll ~timeout:0L poll in
        let _ = Kernel.Async.Poll.deregister poll source in ()
    )

let benchmarks = Bench.[
  with_config ~config:{ iterations = 50; warmup = 10 } "io stdin read len 0" bench_stdin_read_len_zero;
  with_config ~config:{ iterations = 50; warmup = 10 } "io stdout write len 0" bench_stdout_write_len_zero;
  with_config ~config:{ iterations = 50; warmup = 10 } "io stderr write len 0" bench_stderr_write_len_zero;
  with_config ~config:{ iterations = 50; warmup = 10 } "io stdin readv len 0" bench_stdin_read_vectored_len_zero;
  with_config ~config:{ iterations = 50; warmup = 10 } "io stdout writev len 0" bench_stdout_write_vectored_len_zero;
  with_config ~config:{ iterations = 50; warmup = 10 } "io stderr writev len 0" bench_stderr_write_vectored_len_zero;
  with_config ~config:{ iterations = 50; warmup = 10 } "io stdin source poll/close" bench_stdin_source_poll;
  with_config ~config:{ iterations = 50; warmup = 10 } "io stdout source poll/close" bench_stdout_source_poll;
