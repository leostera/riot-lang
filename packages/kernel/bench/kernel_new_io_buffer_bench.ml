...
let medium_slice =
  ...
  Kernel.IO.IoVec.IoSlice.blit_from_string_unchecked medium_chunk ~src_off:0 slice ~dst_off:0 ~len:(String.length medium_chunk);
  slice
...
let benchmarks = Bench.[
  with_config ~config:{ iterations = 20; warmup = 5 } "io buffer append_string: 4096 x 16B" (bench_append_string ~count:4_096 ~chunk:small_chunk);
  with_config ~config:{ iterations = 10; warmup = 3 } "io buffer append_slice: 128 x 1KiB" (bench_append_slice ~count:128 medium_slice);
  with_config ~config:{ iterations = 10; warmup = 3 } "io buffer direct write+commit: 128 x 1KiB" (bench_direct_write_commit ~count:128 ~chunk:medium_chunk);
  with_config ~config:{ iterations = 20; warmup = 5 } "io buffer consume+refill: 4096 x 16B" (bench_consume_and_refill ~count:4_096 ~chunk:small_chunk);
]
