...
let panic_tcp_listener = fun error -> Kernel.SystemError.panic (Kernel.Error.to_string (Kernel.Error.from_net_tcp_listener error))

let panic_tcp_stream = fun error -> Kernel.SystemError.panic (Kernel.Error.to_string (Kernel.Error.from_net_tcp_stream error))
...
let rec accept_stream = fun poll listener ->
  ...
  | Kernel.Result.Error error ->
      if is_tcp_listener_would_block error then
        (
          wait_readable poll ~token:(Kernel.Async.Token.make 402) (Kernel.Net.TcpListener.to_source listener);
          accept_stream poll listener
        )
      else panic_tcp_listener error
...
let bench_tcp_loopback_roundtrip = fun () ->
  with_poll
    (
      fun poll ->
        let listener = lift_tcp_listener (Kernel.Net.TcpListener.bind (Kernel.Net.SocketAddr.loopback_v4 ~port:0)) in
        protect ~finally:(
                    ...
                    protect ~finally:(
                      ...
                      (
                        fun () ->
                          ...
                          write_all_stream poll ~token:(Kernel.Async.Token.make 403) client payload ~pos:0 ~len:4;
                          read_exact_stream poll ~token:(Kernel.Async.Token.make 404) server server_buf ~pos:0 ~len:4;
                          write_all_stream poll ~token:(Kernel.Async.Token.make 405) server reply ~pos:0 ~len:4;
                          read_exact_stream poll ~token:(Kernel.Async.Token.make 406) client client_buf ~pos:0 ~len:4
                      )
                )
          )
    )
                    ...
                    protect ~finally:(
                      ...
                      (
                        fun () ->
                          ...
                          write_all_vectored poll ~token:(Kernel.Async.Token.make 410) client payload ~pos:0 ~len:(Kernel.IO.IoVec.length payload);
                          read_exact_vectored poll ~token:(Kernel.Async.Token.make 411) server server_buf ~pos:0 ~len:(Kernel.IO.IoVec.length payload);
                          write_all_vectored poll ~token:(Kernel.Async.Token.make 412) server reply ~pos:0 ~len:(Kernel.IO.IoVec.length reply);
                          read_exact_vectored poll ~token:(Kernel.Async.Token.make 413) client client_buf ~pos:0 ~len:(Kernel.IO.IoVec.length reply)
                      )
                )
          )
    )
                  ...
                  fun () ->
                    let rec accept_many remaining acc =
                      if remaining = 0 then
                        ...
                        let server = accept_stream poll listener in accept_many (remaining - 1) (server :: acc)
                    in
    ...
    (
      fun poll ->
        ...
        protect ~finally:(
          ...
          (
            fun () ->
              let client = lift_udp (Kernel.Net.UdpSocket.bind (Kernel.Net.SocketAddr.loopback_v4 ~port:0)) in
              protect ~finally:(
                ...
                (
                  fun () ->
                    ...
                    let _ = lift_udp (Kernel.Net.UdpSocket.send_to client server_addr (Kernel.Bytes.from_string "ping")) in
                  ...
                  fun () ->
                    ...
                    let rec send_many remaining =
                      if remaining = 0 then
                        ...
                        let _ = lift_udp (Kernel.Net.UdpSocket.send_to client server_addr payload) in send_many (remaining - 1)
                    in
                    ...
                    let rec recv_many remaining =
                      if remaining = 0 then
                        ...
                        let _ = recv_from_udp poll ~token:(Kernel.Async.Token.make ("udp-burst", remaining)) server buffer in recv_many (remaining - 1)
                    in
                    send_many 32;
                    recv_many 32
                )
                    ...
                    protect ~finally:(
                      ...
                      (
                        fun () ->
                          ...
                          write_all_stream poll ~token:(Kernel.Async.Token.make 420) client bulk_payload ~pos:0 ~len:bulk_len;
                          read_exact_stream poll ~token:(Kernel.Async.Token.make 421) server server_buf ~pos:0 ~len:bulk_len;
                          write_all_stream poll ~token:(Kernel.Async.Token.make 422) server bulk_reply ~pos:0 ~len:bulk_len;
                          read_exact_stream poll ~token:(Kernel.Async.Token.make 423) client client_buf ~pos:0 ~len:bulk_len
                      )
                )
          )
    )
          ...
          (
            fun () ->
              ...
              protect ~finally:(
                ...
                (
                  fun () ->
                    let other = lift_udp (Kernel.Net.UdpSocket.bind (Kernel.Net.SocketAddr.loopback_v4 ~port:0)) in
                    protect ~finally:(
                      fun () -> close_udp other
                    )
                      (
                        fun () ->
                          ...
                          let _ = lift_udp (Kernel.Net.UdpSocket.connect server client_addr) in
                          let _ = lift_udp (Kernel.Net.UdpSocket.connect client server_addr) in
                          let _ = lift_udp (Kernel.Net.UdpSocket.send_to other server_addr (Kernel.Bytes.from_string "rogue")) in
          ...
          (
            fun () ->
              let rec register index = function
                | [] -> ()
                | (server, _) :: rest ->
                    let _ = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make index) Kernel.Async.Interest.readable (Kernel.Net.UdpSocket.to_source server)) in register (index + 1) rest
              in
              let rec send_all = function
                | [] -> ()
                | (server, client) :: rest ->
                    let server_addr = lift_udp (Kernel.Net.UdpSocket.local_addr server) in
                    let _ = lift_udp (Kernel.Net.UdpSocket.send_to client server_addr (Kernel.Bytes.from_string "x")) in send_all rest
              in
              register 0 pairs;
              send_all pairs;
            ...
            fun () ->
              ...
              let rec connect_many remaining clients servers =
                if remaining = 0 then
                  ...
                  let client = connect_stream poll addr in
                  let server = accept_stream poll listener in connect_many (remaining - 1) (client :: clients) (server :: servers)
              in
                ...
                (
                  fun () ->
                    let rec register index = function
                      | [] -> ()
                      | server :: rest ->
                          let _ = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make index) Kernel.Async.Interest.readable (Kernel.Net.TcpStream.to_source server)) in register (index + 1) rest
                    in
                    let rec send_all index = function
                      | [] -> ()
                      | client :: rest ->
                          let payload = Kernel.Bytes.from_string "x" in
                          write_all_stream poll ~token:(Kernel.Async.Token.make (800 + index)) client payload ~pos:0 ~len:1;
