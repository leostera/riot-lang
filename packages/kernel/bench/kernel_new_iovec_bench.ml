...
let build_strings = fun ~count ~segment_size -> Kernel.Array.init ~count ~fn:(
  fun _ -> Kernel.String.make ~len:segment_size ~char:'x'
)

let build_bytes = fun ~count ~segment_size -> build_strings ~count ~segment_size |> Kernel.Array.map ~fn:Kernel.Bytes.from_string

let build_iovec = fun ~count ~segment_size -> build_strings ~count ~segment_size |> Kernel.IO.IoVec.from_string_array |> Result.unwrap
...
let bench_for_each = fun iovec () -> Kernel.IO.IoVec.for_each iovec ~fn:(
  fun _ -> ()
)
...
let benchmarks = Bench.[
  with_config ~config:{ iterations = 20; warmup = 5 } "iovec from_string_array: 4096 x 16B" (bench_from_string_array small_string_segments);
  with_config ~config:{ iterations = 10; warmup = 3 } "iovec from_string_array: 128 x 1KiB" (bench_from_string_array medium_string_segments);
  with_config ~config:{ iterations = 10; warmup = 3 } "iovec from_string_array: 32 x 4KiB" (bench_from_string_array large_string_segments);
  with_config ~config:{ iterations = 20; warmup = 5 } "iovec from_bytes_array: 4096 x 16B" (bench_from_bytes_array small_bytes_segments);
  with_config ~config:{ iterations = 10; warmup = 3 } "iovec from_bytes_array: 128 x 1KiB" (bench_from_bytes_array medium_bytes_segments);
  with_config ~config:{ iterations = 10; warmup = 3 } "iovec from_bytes_array: 32 x 4KiB" (bench_from_bytes_array large_bytes_segments);
  ...
  with_config ~config:{ iterations = 20; warmup = 5 } "iovec sub middle: 4096 x 16B" (bench_sub_middle ~segment_size:16 small_iovec);
  with_config ~config:{ iterations = 10; warmup = 3 } "iovec sub middle: 128 x 1KiB" (bench_sub_middle ~segment_size:1_024 medium_iovec);
  with_config ~config:{ iterations = 5; warmup = 2 } "iovec sub middle: 32 x 4KiB" (bench_sub_middle ~segment_size:4_096 large_iovec);
