...
let lift_process result =
  ...
  | Kernel.Result.Error error -> Kernel.SystemError.panic (Kernel.Error.to_string (Kernel.Error.from_process error))

let lift_async result =
  match result with
  | Kernel.Result.Ok value -> value
  | Kernel.Result.Error error -> Kernel.SystemError.panic (Kernel.Error.to_string (Kernel.Error.from_async error))

let lift_file result =
  match result with
  | Kernel.Result.Ok value -> value
  | Kernel.Result.Error error -> Kernel.SystemError.panic (Kernel.Error.to_string (Kernel.Error.from_fs_file error))
...
let with_process = fun process fn ->
  try
    ...
    let _ = Kernel.Process.close process in value
  with
  | error ->
      let _ = Kernel.Process.close process in raise error
  ...
  let rec spawn remaining acc =
    if remaining = 0 then
      ...
      let process = lift_process (Kernel.Process.spawn ~program:"/bin/sh" ~args:[|
        "-c";
        "sleep 0.02";
      |] ~stdio ()) in spawn (remaining - 1) (process :: acc)
  in
...
let read_once = fun poll ~token file ->
  ...
  let rec loop () =
    ...
    | Kernel.Result.Error error ->
        ...
        else Kernel.SystemError.panic (Kernel.Error.to_string (Kernel.Error.from_fs_file error))
  in
  loop ()
...
let wait_for_exit = fun poll ~token process ->
  ...
  let rec loop () =
    ...
    | Kernel.Result.Ok (Some _status as status) -> status
    | Kernel.Result.Ok None ->
        let source = Kernel.Process.to_source process in
        let _ = lift_async (Kernel.Async.Poll.register poll token Kernel.Async.Interest.priority source) in
        protect ~finally:(
          fun () ->
            let _ = Kernel.Async.Poll.deregister poll source in ()
        )
...
let bench_spawn_true = fun () ->
  with_poll
    (
      fun poll ->
        let process = lift_process (Kernel.Process.spawn ~program:"/usr/bin/true" ~args:[||] ~stdio:Kernel.Process.default_stdio ()) in
          ...
          (
            fun processes ->
              let rec register index = function
                | [] -> ()
                | process :: rest ->
                    let _ = lift_async (Kernel.Async.Poll.register poll (Kernel.Async.Token.make index) Kernel.Async.Interest.priority (Kernel.Process.to_source process)) in register (index + 1) rest
              in
        ...
        with_process process
          (
            fun process ->
              ...
              | None -> Kernel.SystemError.panic "expected stdout pipe"
              | Some stdout ->
                  let _ = wait_for_exit poll ~token:(Kernel.Async.Token.make 612) process in drain_all poll ~token:(Kernel.Async.Token.make 602) stdout
          )
    )
        ...
        with_process process
          (
            fun process ->
              ...
              | None -> Kernel.SystemError.panic "expected stdout pipe"
              | Some stdout ->
                  let _ = wait_for_exit poll ~token:(Kernel.Async.Token.make 613) process in drain_all poll ~token:(Kernel.Async.Token.make 603) stdout
          )
    )

let benchmarks = Bench.[
  with_config ~config:{ iterations = 25; warmup = 5 } "process spawn true and poll exit" bench_spawn_true;
  with_config ~config:{ iterations = 25; warmup = 5 } "process spawn echo with stdout pipe and poll exit" bench_spawn_echo_with_pipe;
  with_config ~config:{ iterations = 20; warmup = 5 } "process stdout pipe drain after exit: 4KiB" bench_stdout_pipe_drain_small_output;
  with_config ~config:{ iterations = 15; warmup = 3 } "process stdout pipe drain after exit: 64KiB" bench_stdout_pipe_drain_medium_output;
