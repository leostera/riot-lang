...
CAMLprim value kernel_new_process_execv(value program_val, value argv_val) {
  CAMLparam2(program_val, argv_val);

  int argc = Wosize_val(argv_val);
  ...
  for (int index = 0; index < argc; index++) {
    argv[index] = (char *)String_val(Field(argv_val, index));
  }
  argv[argc] = NULL;

  execv(String_val(program_val), argv);
