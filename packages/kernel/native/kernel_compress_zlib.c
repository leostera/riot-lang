...
static struct custom_operations gzip_encoder_ops = {
    "riot.kernel.gzip_encoder",
    gzip_encoder_finalize,
...
CAMLprim value kernel_gzip_create_encoder(value v_level) {
    CAMLparam1(v_level);
    ...
    int ret = deflateInit2(&encoder->stream, level, Z_DEFLATED, 15 + 16, 8, Z_DEFAULT_STRATEGY);
    if (ret != Z_OK) {
...
CAMLprim value kernel_gzip_encode(
    value v_encoder,
    value v_src,
    value v_src_pos,
    value v_src_len,
    ...
    switch (flush) {
        ...
        case 2:
            ...
            break;
        default:
            CAMLreturn(alloc_step_result(3, 0, 0, 0));
    }

    encoder->stream.next_in = (Bytef *)Bytes_val(v_src) + src_pos;
    encoder->stream.avail_in = (uInt)src_len;
    encoder->stream.next_out = (Bytef *)Bytes_val(v_dst) + dst_pos;
    encoder->stream.avail_out = (uInt)dst_len;

    int ret = deflate(&encoder->stream, z_flush);
...
CAMLprim value kernel_gzip_close_encoder(value v_encoder) {
    CAMLparam1(v_encoder);
    gzip_encoder_finalize(v_encoder);
    CAMLreturn(Val_unit);
}

CAMLprim value kernel_gzip_encode_bytecode(value *argv, int argn) {
    return kernel_gzip_encode(argv[0], argv[1], argv[2], argv[3], argv[4], argv[5], argv[6], argv[7]);
}
