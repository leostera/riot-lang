...
let test_zero_movement_elimination () =
...
let test_nested_sequence_flattening () =
...
let test_complex_optimization () =
  let input = 
    Seq [
      ...
      MoveUp 0;  (* Should be eliminated *)
      Text " - ";
      Text "Done";
      HideCursor;
      HideCursor;  (* Duplicate, should keep only one *)
    ]
  in
  ...
  | Seq [
      MoveCursor (10, 20);
      Fg (Tty.Color.ANSI 2, [Text "Status: OK"]);
      Text " - Done";
      HideCursor
    ] -> print_endline "✓ Complex optimization works"
  | _ -> 
      (* Debug output *)
      let rec ast_to_string = function
        ...
        | Fg (_, children) -> format "Fg([%s])" (String.concat "; " (List.map ast_to_string children))
        | Bg (_, children) -> format "Bg([%s])" (String.concat "; " (List.map ast_to_string children))
...
let test_ansi_output () =
  ...
  assert (String.contains_s output "\x1b[1;1H");  (* MoveCursor(0,0) *)
  assert (String.contains_s output "\x1b[2J");     (* Clear *)
  assert (String.contains_s output "\x1b[31m");    (* Red foreground *)
  assert (String.contains_s output "\x1b[1m");     (* Bold *)
  assert (String.contains_s output "Error: ");
  assert (String.contains_s output "Failed");
  
  print_endline "✓ ANSI output generation works"

let test_color_reset () =
  (* Ensure colors are properly reset after scope *)
  let ast = 
    Seq [
      Fg (Tty.Color.ANSI 2, [Text "Green"]);
      Text " Normal"
    ]
  in
  ...
  assert (String.contains_s output "\x1b[32m");    (* Green *)
  assert (String.contains_s output "\x1b[39m");    (* Reset fg color *)
  assert (String.contains_s output "Green Normal");
  
  print_endline "✓ Color reset works correctly"

let test_nested_style_resets () =
  ...
  assert (String.contains_s output "\x1b[1m");     (* Bold on *)
  assert (String.contains_s output "\x1b[3m");     (* Italic on *)
  assert (String.contains_s output "\x1b[23m");    (* Italic off *)
  assert (String.contains_s output "\x1b[22m");    (* Bold off at the end *)
  
  print_endline "✓ Nested style resets work correctly"

let test_synchronized_updates () =
  let ast = 
    Seq [
      BeginSync;
      Text "Atomic update";
  ...
  assert (String.contains_s output "\x1b[?2026h"); (* Begin sync *)
  assert (String.contains_s output "\x1b[?2026l"); (* End sync *)
...
let test_rgb_colors () =
  ...
  assert (String.contains_s output "\x1b[38;2;255;0;0m");  (* RGB red fg *)
  assert (String.contains_s output "\x1b[48;2;0;0;255m");  (* RGB blue bg *)
  
  print_endline "✓ RGB colors work"

let test_ansi256_colors () =
  let ast = 
    Seq [
      Fg (Tty.Color.ANSI256 196, [Text "256-color red"]);
    ]
  in
  
  let output = render ast in
  
  assert (String.contains_s output "\x1b[38;5;196m");  (* 256-color *)
