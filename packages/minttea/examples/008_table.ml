...
let users = [
  ...
  { id = 4; name = "Diana Prince"; email = "diana@example.com"; status = "Active"; joined = "2024-03-05" };
  { id = 5; name = "Eve Anderson"; email = "eve@example.com"; status = "Pending"; joined = "2024-04-12" };
  { id = 6; name = "Frank Miller"; email = "frank@example.com"; status = "Active"; joined = "2023-11-28" };
  { id = 7; name = "Grace Hopper"; email = "grace@example.com"; status = "Active"; joined = "2024-01-08" };
  { id = 8; name = "Henry Ford"; email = "henry@example.com"; status = "Inactive"; joined = "2023-10-15" };
  { id = 9; name = "Iris West"; email = "iris@example.com"; status = "Active"; joined = "2024-02-14" };
  { id = 10; name = "Jack Ryan"; email = "jack@example.com"; status = "Active"; joined = "2024-03-22" };
  { id = 11; name = "Karen Page"; email = "karen@example.com"; status = "Pending"; joined = "2024-04-30" };
  { id = 12; name = "Leo Messi"; email = "leo@example.com"; status = "Active"; joined = "2024-05-01" };
  { id = 13; name = "Maria Garcia"; email = "maria@example.com"; status = "Active"; joined = "2023-09-20" };
  { id = 14; name = "Nathan Drake"; email = "nathan@example.com"; status = "Inactive"; joined = "2023-08-15" };
  { id = 15; name = "Olivia Pope"; email = "olivia@example.com"; status = "Active"; joined = "2024-06-10" };
]

(* Convert user to table row *)
let user_to_row user =
  [
...
let update event model =
  ...
  | Event.KeyDown (Event.Up, _) ->
      let table = match Table.selected_index model.table with
  ...
  | Event.KeyDown (Event.Enter, _) ->
      ...
      let selected_user = 
        ...
        | Some idx when idx < List.length model.users ->
            Some (List.nth model.users idx)
        | _ -> None
      in
      ({ model with selected_user }, Command.Noop)
      
  | Event.KeyDown (Event.Key "c", _) ->
      (* Clear selection *)
...
let view model =
  ...
  column ~style:Style.(empty |> padding (Padding.all 1))
    [
      (* Title *)
      text ~style:Style.(empty |> bold |> fg (`rgb (100, 200, 255)))
      ...
      (match model.selected_user with
       ...
       | None ->
           text ~style:Style.(empty |> fg (`rgb (100, 100, 100)))
             "Press Enter to view user details");
...
let () = 
  ...
  (* Create table *)
  let table = 
    Table.make columns rows
    |> Table.set_height ~height:10
    |> Table.focus
  in
