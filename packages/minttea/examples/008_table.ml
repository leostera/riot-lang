...
(* Sample data type *)
type user = { id: int; name: string; email: string; status: string; joined: string }

(* Model *)
type model = { table: Table.t; selected_user: user option; users: user list }
...
column ~style:Style.(empty |> padding (Style.Padding.all 1))
  [
    ...
    (
      ...
      | Some user ->
          column ~style:Style.(empty |> border ~width:1 ~color:(`rgb (0, 255, 0)) () |> padding (Style.Padding.all 1))
            [
