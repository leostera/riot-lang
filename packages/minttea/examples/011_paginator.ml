...
(* Model *)
type model = { current_page: int; pages: string list }
...
let update = fun event model ->
  ...
  | Event.KeyDown (Event.Left, _) ->
      let current_page = max 0 (model.current_page - 1) in ({ model with current_page }, Command.Noop)
  | Event.KeyDown (Event.Right, _) ->
      let max_page = List.length model.pages - 1 in
      let current_page = min max_page (model.current_page + 1) in ({ model with current_page }, Command.Noop)
  | Event.KeyDown (Event.Home, _) -> { model with current_page = 0 }, Command.Noop
  | Event.KeyDown (Event.End, _) ->
      let current_page = List.length model.pages - 1 in ({ model with current_page }, Command.Noop)
  ...
  (
    ...
    | '1' .. '5' as c ->
        ...
        let max_page = List.length model.pages - 1 in
        let current_page = min max_page (max 0 page) in ({ model with current_page }, Command.Noop)
...
let dots = List.init ~count:total_pages ~fn:(
  fun i ->
...
column ~style:Style.(empty |> padding (Style.Padding.all 1))
  [
    container ~style:Style.(empty |> border ~width:1 ~color:(`rgb (100, 150, 200)) () |> padding (Style.Padding.all 2) |> min_height 15.0) [ text page_content ];
    text "";
