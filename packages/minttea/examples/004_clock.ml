...
type model = { time: DateTime.t; timer_ref: Timer.id Ref.t }
...
let init = fun model ->
  let timer_ref, timer_cmd = Command.timer ~after:(Time.Duration.from_secs 1) in ({ model with timer_ref }, timer_cmd)

(* Update: Handle events *)
let update = fun event model ->
  match event with
  | Event.KeyDown (Event.Key "q", _) | Event.KeyDown (Event.Key "Q", _) | Event.KeyDown (Event.Escape, _) -> model, Command.Quit
  | Event.Timer ref when Ref.equal ref model.timer_ref ->
      ...
      let new_model = { time = DateTime.now (); timer_ref } in (new_model, timer_cmd)
...
let render_time_ascii = fun time_str ->
  let chars = String.fold_left ~fn:(
    fun acc ch -> ch :: acc
  ) ~init:[] time_str |> List.rev in
  ...
  let lines = ref [] in
  for row = 0 to 4 do
    let line_parts = List.map ~fn:(
      fun art -> Array.get_unchecked art ~at:row
    ) art_chars in
    let line = String.concat "" line_parts in lines := line :: !lines
  done;
  List.rev !lines
...
in
let time_str = pad_two model.time.hour ^ ":" ^ pad_two model.time.minute ^ ":" ^ pad_two model.time.second in
(* Render ASCII art *)
let ascii_lines = render_time_ascii time_str in
(* Create elements for each line of ASCII art *)
let ascii_elements = List.map ~fn:(
  fun line -> text ~style:Style.(empty |> fg (`rgb (0, 255, 127)) |> bold) line
) ascii_lines in (* Center everything *)
column ~style:Style.(empty |> align ~x:Center ~y:Middle |> padding (Style.Padding.all 2)) [ column ascii_elements; text ""; text ~style:Style.(empty |> fg (`rgb (100, 100, 100))) "Press 'q' to quit" ]
