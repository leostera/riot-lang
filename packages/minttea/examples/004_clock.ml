...
let render_time_ascii = fun time_str ->
  let chars =
    String.fold_left ~fn:(fun acc ch -> ch :: acc) ~init:[] time_str
    |> List.rev
  in
  ...
  let lines = ref [] in
  for row = 0 to 4 do
    let line_parts = List.map ~fn:(fun art -> Array.get_unchecked art ~at:row) art_chars in
...
let view = fun model ->
  ...
  in
  let time_str =
    pad_two model.time.hour ^ ":" ^ pad_two model.time.minute ^ ":" ^ pad_two model.time.second
  in
  (* Render ASCII art *)
