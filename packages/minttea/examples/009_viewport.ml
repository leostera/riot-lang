...
let generate_content = fun () ->
  ...
  lines := "" :: !lines;
  for i = 1 to 100 do
    let line = Int.to_string i ^ ". This is line number " ^ Int.to_string i ^ " - scroll to see more content!" in lines := line :: !lines
  done;
  lines := "" :: !lines;
  lines := "THE END" :: !lines;
  lines := "" :: !lines;
...
let update = fun event model ->
  ...
  | Event.KeyDown (Event.Up, _) ->
      let viewport = Viewport.scroll_up model.viewport ~lines:1 in ({ model with viewport }, Command.Noop)
  | Event.KeyDown (Event.Down, _) ->
      let viewport = Viewport.scroll_down model.viewport ~lines:1 in ({ model with viewport }, Command.Noop)
  | Event.KeyDown (Event.PageUp, _) ->
      let height = Viewport.height model.viewport in
      let viewport = Viewport.scroll_up model.viewport ~lines:height in ({ model with viewport }, Command.Noop)
  | Event.KeyDown (Event.PageDown, _) ->
      let height = Viewport.height model.viewport in
      let viewport = Viewport.scroll_down model.viewport ~lines:height in ({ model with viewport }, Command.Noop)
  | Event.KeyDown (Event.Home, _) ->
      (* Scroll to top by scrolling up by a large amount *)
      let viewport = Viewport.scroll_up model.viewport ~lines:10_000 in ({ model with viewport }, Command.Noop)
  | Event.KeyDown (Event.End, _) ->
      (* Scroll to bottom by scrolling down by a large amount *)
      let viewport = Viewport.scroll_down model.viewport ~lines:10_000 in ({ model with viewport }, Command.Noop)
...
column ~style:Style.(empty |> padding (Style.Padding.all 1))
  [
    ...
    container ~style:Style.(empty |> border ~width:1 ~color:(`rgb (100, 100, 200)) () |> padding (Style.Padding.all 1)) [ text (Viewport.view model.viewport) ];
    text "";
    column ~style:Style.(empty |> fg (`rgb (100, 100, 100)))
      [
        text "Navigation:";
