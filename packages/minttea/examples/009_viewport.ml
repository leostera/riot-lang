...
let update event model =
  ...
  | Event.KeyDown (Event.Home, _) ->
      ...
      let viewport = Viewport.scroll_up model.viewport ~lines:10000 in
      ({ model with viewport }, Command.Noop)
      
  | Event.KeyDown (Event.End, _) ->
      (* Scroll to bottom by scrolling down by a large amount *)
      let viewport = Viewport.scroll_down model.viewport ~lines:10000 in
      ({ model with viewport }, Command.Noop)
...
let () = 
  ...
  let viewport = 
    Viewport.make ~width:70 ~height:15
