...
(* Sample initial text *)
let initial_text = 
  "Welcome to the Minttea Text Editor!\n\
   \n\
...
let update event model =
  match event with
  | Event.KeyDown (Event.Key "q", _) | Event.KeyDown (Event.Escape, _) ->
      (model, Command.Quit)
  ...
  | Event.KeyDown (Event.Backspace, _) ->
      ...
      let text = if len > 0 then String.sub model.text 0 (len - 1) else "" in
      ({ model with text; saved = false }, Command.Noop)
  ...
  column ~style:Style.(empty |> padding (Padding.all 1))
    [
      ...
      row ~style:Style.(empty |> bg (`rgb (40, 40, 40)) |> padding (Padding.symmetric ~h:2 ~v:1))
        [
          ...
          text ~style:Style.(empty |> fg (
            if model.saved then `rgb (0, 255, 0) else `rgb (255, 200, 0)
          ))
            (if model.saved then "Saved" else "Modified");
        ];
      
      (* Status bar *)
      text ~style:Style.(empty |> fg (`rgb (150, 150, 150)))
        ("Line " ^ Int.to_string (model.cursor_row + 1) ^ ":" ^ 
          Int.to_string (model.cursor_col + 1) ^ " | Total lines: " ^ 
      ...
      text ~style:Style.(empty |> fg (`rgb (100, 100, 100)))
        "Type to edit • Ctrl+S: Save • Ctrl+C: Clear • q: Quit";
    ]
