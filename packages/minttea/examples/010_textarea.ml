...
(* Model *)
type model = { text: string; cursor_row: int; cursor_col: int; saved: bool }
...
let update = fun event model ->
  ...
  | Event.KeyDown (Event.Key s, _) when String.length s = 1 ->
      (* Insert character *)
      let text = model.text ^ s in ({ model with text; saved = false }, Command.Noop)
  | Event.KeyDown (Event.Enter, _) ->
      (* Add new line *)
      let text = model.text ^ "\n" in ({ model with text; saved = false }, Command.Noop)
  ...
  | Event.KeyDown (Event.Up, _) ->
      let cursor_row = max 0 (model.cursor_row - 1) in ({ model with cursor_row }, Command.Noop)
  | Event.KeyDown (Event.Down, _) ->
      let cursor_row = model.cursor_row + 1 in ({ model with cursor_row }, Command.Noop)
  | Event.KeyDown (Event.Left, _) ->
      let cursor_col = max 0 (model.cursor_col - 1) in ({ model with cursor_col }, Command.Noop)
  | Event.KeyDown (Event.Right, _) ->
      let cursor_col = model.cursor_col + 1 in ({ model with cursor_col }, Command.Noop)
...
column ~style:Style.(empty |> padding (Style.Padding.all 1))
  [
    row ~style:Style.(empty |> bg (`rgb (40, 40, 40)) |> padding (Style.Padding.symmetric ~h:2 ~v:1))
      [
        text ~style:Style.(empty |> bold |> fg (`rgb (100, 200, 255))) "📝 Minttea Text Editor";
        spacer ~flex:1.0 ();
        text ~style:Style.(empty |> fg
          (
            if model.saved then
              `rgb (0, 255, 0)
            else `rgb (255, 200, 0)
    ...
    text ~style:Style.(empty |> fg (`rgb (150, 150, 150))) ("Line " ^ Int.to_string (model.cursor_row + 1) ^ ":" ^ Int.to_string (model.cursor_col + 1) ^ " | Total lines: " ^ Int.to_string line_count);
    text "";
    container ~style:Style.(empty |> border ~width:1 ~color:(`rgb (0, 255, 127)) () |> padding (Style.Padding.all 1) |> min_height 15.0 |> min_width 70.0) [ text model.text ];
    text "";
