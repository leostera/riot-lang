...
let main ~args:_ =
  ...
  (* 5-8. Build everything in a buffer first, EXACTLY like Minttea with \027[H at start *)
  let buf = Buffer.create (cols * rows * 2) in
  
  Buffer.add_string buf "\x1b[H";  (* Home - like emit does *)
  Buffer.add_string buf "\x1b[0m";  (* Reset - like emit does *)
