...
let view = fun model ->
  ...
  let tab_elements =
    model.tabs
    |> List.enumerate
    |> List.map ~fn:(fun (index, tab) -> render_tab ~active:model.active_tab index tab)
  in
  let tab_bar =
    row
      ~style:Style.(empty
