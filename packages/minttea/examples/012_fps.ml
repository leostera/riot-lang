...
let update = fun event model ->
  ...
  | Event.Frame _instant ->
      ...
      let _frame_status = Fps.tick model.fps in (model, Command.Noop)
  | Event.Resize size ->
...
let box_style = Style.(empty |> bg (`rgb (255, 100, 100)) |> fg (`rgb (255, 255, 255)) |> width (Fixed 10.0) |> height (Fixed 3.0) |> padding (Style.Padding.all 1)) in
  ...
  [
    row ~style:Style.(empty |> bg (`rgb (40, 40, 40)) |> padding (Style.Padding.symmetric ~h:2 ~v:1))
      [
        ...
        spacer ~flex:1.0 ();
        text ~style:Style.(empty |> fg (`rgb (255, 200, 100))) ("Speed: " ^ Float.to_string model.speed ^ "x");
        text " | ";
    ...
    text "";
    container ~style:Style.(empty |> border ~width:1 ~color:(`rgb (100, 100, 200)) () |> min_height (float_of_int (model.height - 10)) |> min_width (float_of_int (model.width - 4))) [ container ~style:Style.(empty |> margin (Style.Margin.make ~left:(int_of_float model.box_x) ~top:(int_of_float model.box_y) ())) [ container ~style:box_style [ text "BOX" ] ] ];
    text "";
...
let main ~args:_ =
  ...
  in
  let config = Minttea.config ~fps:60 () in Minttea.run ~config initial_model app
