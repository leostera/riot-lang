...
type model = { list: string Listbox.t; selected: string option }
...
let update = fun event model ->
  ...
  | Event.KeyDown (Event.Enter, _) ->
      ...
      let selected = Listbox.selected_item model.list in ({ model with selected }, Command.Quit)
  | Event.KeyDown (Event.Up, _) ->
      let list = Listbox.select_prev model.list in ({ model with list }, Command.Noop)
  | Event.KeyDown (Event.Down, _) ->
      let list = Listbox.select_next model.list in ({ model with list }, Command.Noop)
...
column ~style:Style.(empty |> padding (Style.Padding.all 1))
  [
    ...
    (
      ...
      | Some item -> text ~style:Style.(empty |> fg (`rgb (0, 255, 0)) |> padding (Style.Padding.make ~top:1 ())) ("You selected: " ^ item)
      | None -> empty
    );
  ]
