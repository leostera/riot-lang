...
let switch_focus = fun model ->
  ...
  | PasswordField ->
      { model with focus = SubmitButton; password_input = Textinput.blur model.password_input }
  | SubmitButton ->
      { model with focus = NameField; name_input = Textinput.focus model.name_input }
...
let update = fun event model ->
  ...
  | Event.KeyDown (Event.Escape, _) when not model.submitted -> (model, Command.Quit)
