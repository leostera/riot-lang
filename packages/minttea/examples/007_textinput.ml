...
let switch_focus model =
  ...
  | SubmitButton ->
      { model with 
        focus = NameField;
        name_input = Textinput.focus model.name_input;
      }

(* Handle form submission *)
let submit_form model =
...
let update event model =
  ...
  | Event.KeyDown (Event.Key s, _) when String.length s = 1 ->
      ...
      let new_model = match model.focus with
        | NameField ->
  ...
  | Event.KeyDown (Event.Backspace, _) ->
      ...
      let new_model = match model.focus with
        | NameField ->
            ...
            let value = if len > 0 then String.sub current 0 (len - 1) else "" in
...
let view model =
  ...
  if model.submitted then
    ...
    column ~style:Style.(empty |> padding (Padding.all 2))
      [
        text ~style:Style.(empty |> fg (`rgb (0, 255, 0)) |> bold) 
    ...
    column ~style:Style.(empty |> padding (Padding.all 2))
      [
        ...
        (* Name field *)
        container ~style:(highlight_style (model.focus = NameField))
          [text (Textinput.view model.name_input)];
        
        (* Email field *)
        container ~style:(highlight_style (model.focus = EmailField))
          [text (Textinput.view model.email_input)];
        
        (* Password field *)
        container ~style:(highlight_style (model.focus = PasswordField))
        ...
        (match model.error with
         | Some msg ->
             column [
               text "";
                text ~style:Style.(empty |> fg (`rgb (255, 0, 0)))
                  ("⚠ " ^ msg);
             ]
         | None -> empty);
        ...
        text ~style:Style.(empty |> fg (`rgb (100, 100, 100)))
          "Tab: Next field • Enter: Submit • Escape: Quit";
      ]
