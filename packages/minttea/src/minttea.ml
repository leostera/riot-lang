...
let run = fun ?(config = config ()) initial_model app -> let open Std in
Program.run ~app ~config ~initial_model |> Result.map_err ~fn:(
  fun reason -> Failure reason
)

let start = fun ?(config = config ()) (app: 'model App.t) initial_model ->
  let main ~args:_ = let open Std in
