...
let do_render = fun t str ->
  ...
  let apply_padding str =
    ...
    let padded_lines = List.map ~fn:(fun line -> pad_left ^ line ^ pad_right) lines in
    let str_with_h_padding = String.concat "\n" padded_lines in
    (* Apply vertical padding (top and bottom) *)
    let pad_top = String.concat "\n" (List.init ~count:t.padding_top ~fn:(fun _ -> "")) in
    let pad_bottom = String.concat "\n" (List.init ~count:t.padding_bottom ~fn:(fun _ -> "")) in
    let result =
      (
  ...
  let str =
    ...
    | Some h ->
        ...
        if current_height >= h then
          ...
          (
            match align with
            | `Top -> lines @ List.init ~count:padding_needed ~fn:(fun _ -> empty_line)
            | `Bottom -> List.init ~count:padding_needed ~fn:(fun _ -> empty_line) @ lines
            | `Center ->
                let top_pad = padding_needed / 2 in
                let bottom_pad = padding_needed - top_pad in
                (List.init ~count:top_pad ~fn:(fun _ -> empty_line) @ lines)
  ...
  let str =
    ...
    List.map lines ~fn:(fun line -> Formatter.format_string format_seq line)
    |> String.concat "\n"
  in
  (* handle border *)
  let str =
