...
let height x t = { t with height = Fixed x }
let width x t = match x with
  | Some w -> { t with width = Fixed w }
  | None -> { t with width = Auto }
let max_height x t = { t with constraints = { t.constraints with max_height = Some x } }
let max_width x t = { t with constraints = { t.constraints with max_width = Some x } }

(* New size API *)
let width_auto t = { t with width = Auto }
let width_fixed x t = { t with width = Fixed x }
let width_flex x t = { t with width = Flex x }

let height_auto t = { t with height = Auto }
let height_fixed x t = { t with height = Fixed x }
let height_flex x t = { t with height = Flex x }

(* Constraint API *)
let min_width x t = { t with constraints = { t.constraints with min_width = Some x } }
let min_height x t = { t with constraints = { t.constraints with min_height = Some x } }

(* Overflow API *)
let overflow x t = { t with overflow = x }
...
let do_render t str =
  ...
  let target_width = match t.width with
    | Fixed w -> Some w
    | Auto | Flex _ -> None
  in
  let target_height = match t.height with
    | Fixed h -> Some h
    | Auto | Flex _ -> None
  in
  
  (* Apply horizontal alignment/padding if width is set *)
  let str = match target_width with
    | Some w ->
  ...
  let str = match target_height with
    | Some h ->
        ...
        if current_height >= h then
          ...
          let empty_line = match target_width with
          ...
          | `Center ->
              ...
              List.make ~len:top_pad ~fn:(fun _ -> empty_line) @ lines @ List.make ~len:bottom_pad ~fn:(fun _ -> empty_line))
          |> String.concat "\n"
    | None -> str
  in
...
let get_padding_left t = t.padding_left
let get_padding_right t = t.padding_right
let get_padding_top t = t.padding_top
let get_padding_bottom t = t.padding_bottom
...
let get_underline t = t.underline
