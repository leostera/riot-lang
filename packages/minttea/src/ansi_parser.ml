...
let finalize_params p =
  if p.current_param > 0 || List.length p.params > 0 then
    p.params <- p.params @ [p.current_param];
  p.current_param <- 0

(* Parse mouse event from SGR protocol *)
let parse_mouse_sgr params =
  match params with
  | [button; x; y] ->
      let button_type = match button land 0x3 with
        | 0 -> Event.Left
        | 1 -> Event.Middle
        | 2 -> Event.Right
...
let csi_to_event p =
  ...
  | Some '~' ->
      ...
      | _ -> None)
  | Some 'M' | Some 'm' when String.length p.intermediate = 1 && p.intermediate.[0] = '<' ->
      (* SGR mouse protocol *)
...
let parse_byte p byte =
  ...
  (* OSC String state - for window title, etc *)
