...
let filter_query t = t.filter_query
let is_filtering t = t.filtering_active

let selected_item t =
  if List.length t.filtered_items = 0 then None
  else
...
let clamp_selection t =
  ...
  if len = 0 then { t with selected = 0 }
  else
...
let string_contains haystack needle =
  ...
  if n_len = 0 then true
    ...
    let rec search pos =
      if pos + n_len > h_len then false
      else
        let rec match_at i =
          if i >= n_len then true
          else if haystack.[pos + i] = needle.[i] then match_at (i + 1)
          else false
        in
        if match_at 0 then true
        else search (pos + 1)
    in
    search 0

let apply_filter t query =
  if query = "" then
...
let start_filtering t = 
  if t.filter_enabled then { t with filtering_active = true }
  else t

let stop_filtering t = { t with filtering_active = false }

let select t idx =
  { t with selected = idx } |> clamp_selection
...
let select_prev t =
  if List.length t.filtered_items = 0 then t
  else { t with selected = max 0 (t.selected - 1) }

let select_first t = { t with selected = 0 }
...
let view t =
  ...
  if total = 0 then
    ...
    List.iteri (fun idx item ->
      if idx >= start_idx && idx <= end_idx then begin
        ...
        let prefix = if is_selected then t.cursor_char else String.make (String.length t.cursor_char) ' ' in
        ...
        in
        if idx > start_idx then add_char buf '\n';
  ...
  if t.filtering_active then begin
    add_string buf "\n\n/";
