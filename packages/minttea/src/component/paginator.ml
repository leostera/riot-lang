...
let set_total_pages = fun t ~total:items ->
  if items < 1 then
    ...
    in
    let new_t = { t with total_pages = n } in (new_t, n)

let get_slice_bounds = fun t length ->
  let start = t.page * t.per_page in
  let end_pos = min ((t.page * t.per_page) + t.per_page) length in (start, end_pos)

let items_on_page = fun t total_items ->
  if total_items < 1 then
    0
  else
    let start, end_pos = get_slice_bounds t total_items in end_pos - start
...
let numerals_view = fun t text_style ->
  let txt = t.numerals_format (t.page + 1) t.total_pages in Style.render text_style txt
