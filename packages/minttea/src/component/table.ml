...
let move_up = fun t n ->
  { t with cursor = t.cursor - n }
  |> clamp_cursor

let move_down = fun t n ->
  { t with cursor = t.cursor + n }
  |> clamp_cursor
...
let render_separator = fun (columns: column list) ->
  let parts = List.map ~fn:(fun (col: column) -> String.make ~len:col.width ~char:'-') columns in
  String.concat "  " parts

let render_header = fun (columns: column list) ->
  let headers = List.map ~fn:(fun (col: column) -> render_cell col.title col.width) columns in
