...
type t = { frames: string array; current_frame: int; fps: Fps.t; loop: bool }
...
let advance_frame = fun m ->
  ...
  if m.loop then
    ...
    let last_frame = Array.length m.frames - 1 in min last_frame next_frame

let update = fun ?now m ->
  if Fps.tick ?now m.fps = `frame then
    let current_frame = advance_frame m in { m with current_frame }
  else m

let view = fun s ->
  if s.current_frame < 0 || s.current_frame >= Array.length s.frames then
    panic "Sprite.view: current frame out of bounds"
  else Array.get_unchecked s.frames ~at:s.current_frame
