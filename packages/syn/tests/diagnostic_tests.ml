...
let test_diagnostic test_path diagnostic_path =
  let source =
    Fs.read (Path.v test_path) |> Result.expect ~msg:"Failed to read test file"
  in
  let diagnostic_json =
    Fs.read (Path.v diagnostic_path)
...
let discover_diagnostics () =
  ...
  let dir_exists =
    Fs.exists diagnostics_dir |> Result.unwrap_or ~default:false
  in
  if not dir_exists then []
  else
    let entries_iter =
      Fs.read_dir diagnostics_dir
      |> Result.expect ~msg:"Failed to read diagnostics directory"
    in
    ...
    List.filter_map
      (fun entry ->
        ...
        if String.ends_with ~suffix:".ml" path then
          ...
          let exists =
            Fs.exists (Path.v diagnostic_path)
            |> Result.unwrap_or ~default:false
          in
          if exists then Some (path, diagnostic_path) else None
        else None)
      entries
