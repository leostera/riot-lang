...
let canonicalize_json =
  let rec loop = function
    | Json.Object fields -> Json.Object (fields |> List.map ~fn:(
...
let test_diagnostic = fun ~(ctx:Test.FixtureRunner.ctx) ->
  ...
  let source =
    ...
    | Ok source -> source
    | Error error -> panic ("failed to create diagnostic source slice: " ^ Kernel.IO.Error.message error)
  in
  let parse_result = Syn.parse ~filename:ctx.fixture_path source in
  let actual_diagnostics = parse_result.Parser.diagnostics in
  let items = ref [] in
  Vector.iter actual_diagnostics |> Iterator.for_each ~fn:(
    fun diagnostic -> items := Diagnostic.to_json diagnostic :: !items
  );
  let actual_json = Json.Array (List.reverse !items) |> canonicalize_json in Test.Snapshot.assert_with ~ctx:ctx.test ~render:(
...
let main ~args =
  let tests = Test.FixtureRunner.cases () ~dir:(Path.v "packages/syn/tests/diagnostics") ~filter:filter_diagnostic_fixture ~run:(
    fun ctx -> test_diagnostic ~ctx
  ) in Test.Cli.main ~name:"syn-diagnostics" ~tests ~args ()
