...
let should_skip_parse_fixture = fun path ->
  let basename = Path.basename path in List.any parse_skips ~fn:(
    fun name -> String.equal basename name
  )
...
let is_locally_modified_fixture = fun modified_fixture_paths path -> HashSet.contains modified_fixture_paths ~value:path
...
let test_fixture = fun ~(ctx:Test.FixtureRunner.ctx) ->
  ...
  if Vector.length parse_result.Parser.diagnostics > 0 then
    ...
    if root.SyntaxTree.full_width = source_length then
      Ok ()
    else Error ("parse root width mismatch: expected " ^ Int.to_string source_length ^ " bytes, got " ^ Int.to_string root.SyntaxTree.full_width)
...
let test_tagged_quoted_string_token = fun _ctx ->
  ...
  if Vector.length parse_result.Parser.diagnostics > 0 then
    ...
    let string_token = ref None in Ast.Node.for_each_token root ~fn:(
      fun token ->
...
let main ~args =
  ...
  let fixture_tests = Test.FixtureRunner.cases () ~dir:fixture_root ~filter:(has_lossless_snapshot modified_fixture_paths) ~snapshot_path:(
    fun path -> Some (lossless_snapshot_path path)
  ) ~run:(
    fun ctx -> test_fixture ~ctx
  ) in
  let tests = Test.case "tagged_quoted_string_token" test_tagged_quoted_string_token :: fixture_tests in Test.Cli.main ~name:"syn-fixtures" ~tests ~args ()
