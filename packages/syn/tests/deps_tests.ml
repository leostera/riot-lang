...
let parse_modules = fun ~env ~filename source ->
  ...
  | Error (Syn.Deps.Parse_diagnostics diagnostics) -> Error ("parse diagnostics: " ^ String.concat "; " (List.map diagnostics ~fn:Syn.Diagnostic.to_string))

let alias_exports = fun names -> List.fold_left names ~init:Syn.Deps.Env.empty ~fn:(
  fun exports name -> Syn.Deps.Env.add_path exports ~path:[ name ] ~free_names:[ name ]
)

let open_paths = fun paths env -> List.fold_left paths ~init:env ~fn:(
  fun env path -> Syn.Deps.Env.open_path env ~path
)

let test_deps_collect_value_declaration_modules_from_implicit_alias_opens = fun _ctx ->
  let env = Syn.Deps.Env.empty |> Syn.Deps.Env.add_scoped_binding ~path:[ "Kernel"; "Aliases" ] ~free_names:[ "Kernel__Aliases" ] ~exports:(alias_exports [ "Result" ]) |> Syn.Deps.Env.add_scoped_binding ~path:[ "Kernel"; "Net"; "Aliases" ] ~free_names:[ "Kernel__Net__Aliases" ] ~exports:(alias_exports [ "Socket_addr" ]) |> open_paths [ [ "Kernel"; "Aliases" ]; [ "Kernel"; "Net"; "Aliases" ] ] in
  ...
  | Ok modules -> Error ("expected deps [Result, Socket_addr], got [" ^ String.concat ", " modules ^ "]")
  | Error err -> Error err

let test_deps_collect_opened_public_root_module_instead_of_child_module = fun _ctx ->
  let env = Syn.Deps.Env.empty |> Syn.Deps.Env.add_binding ~path:[ "Syn" ] ~free_names:[ "Syn" ] ~exports:(alias_exports [ "Token" ]) in
...
let test_deps_do_not_collect_exported_children_from_module_alias = fun _ctx ->
  let env = Syn.Deps.Env.empty |> Syn.Deps.Env.add_binding ~path:[ "Kernel" ] ~free_names:[ "Kernel" ] ~exports:(alias_exports [ "Array"; "Result" ]) in
...
let test_deps_do_not_collect_scoped_exported_children_from_module_alias = fun _ctx ->
  let env = Syn.Deps.Env.empty |> Syn.Deps.Env.add_scoped_binding ~path:[ "Kernel" ] ~free_names:[ "Kernel" ] ~exports:(alias_exports [ "Array"; "Result" ]) in
  match parse_modules ~env ~filename:"kernel_new_addr_tests.ml" "module Kernel = Kernel\nlet len = Kernel.Array.length values\n" with
  | Ok modules when modules = [ "Kernel" ] -> Ok ()
  | Ok modules -> Error ("expected deps [Kernel], got [" ^ String.concat ", " modules ^ "]")
  | Error err -> Error err
...
let test_deps_do_not_collect_exported_children_from_facade_alias = fun _ctx ->
  let env = Syn.Deps.Env.empty |> Syn.Deps.Env.add_scoped_binding ~path:[ "Syntax_tree" ] ~free_names:[ "Syntax_tree" ] ~exports:(alias_exports [ "Builder" ]) in
...
let test_deps_open_scoped_root_keeps_child_modules_on_opened_root = fun _ctx ->
  let env = Syn.Deps.Env.empty |> Syn.Deps.Env.add_scoped_binding ~path:[ "Std" ] ~free_names:[ "Std" ] ~exports:(alias_exports [ "Array" ]) in
...
|ocaml} in
  ...
  | Ok modules -> Error ("expected deps [Prelude, Regex_stubs], got [" ^ String.concat ", " modules ^ "]")
  | Error err -> Error err

let test_deps_collect_opened_module_root_for_exported_child = fun _ctx ->
  let env = Syn.Deps.Env.empty |> Syn.Deps.Env.add_binding ~path:[ "Iter" ] ~free_names:[ "Iter" ] ~exports:(alias_exports [ "Iterator"; "MutIterator" ]) in
...
let test_deps_collect_manifest_type_modules_from_implicit_alias_opens = fun _ctx ->
  let env = Syn.Deps.Env.empty |> Syn.Deps.Env.add_scoped_binding ~path:[ "Kernel"; "Fs"; "Aliases" ] ~free_names:[ "Kernel__Fs__Aliases" ] ~exports:(alias_exports [ "File" ]) |> open_paths [ [ "Kernel"; "Fs"; "Aliases" ] ] in
...
let test_deps_collect_qualified_public_root_from_implicit_root_alias_open = fun _ctx ->
  let fs_exports = Syn.Deps.Env.empty |> Syn.Deps.Env.add_binding ~path:[ "Fs" ] ~free_names:[ "Fs" ] ~exports:(alias_exports [ "File" ]) in
  let env = Syn.Deps.Env.empty |> Syn.Deps.Env.add_scoped_binding ~path:[ "Kernel"; "Aliases" ] ~free_names:[ "Kernel__Aliases" ] ~exports:fs_exports |> open_paths [ [ "Kernel"; "Aliases" ] ] in
...
let test_deps_collect_variant_payload_modules_from_implicit_alias_opens = fun _ctx ->
  let env = Syn.Deps.Env.empty |> Syn.Deps.Env.add_scoped_binding ~path:[ "Kernel"; "Aliases" ] ~free_names:[ "Kernel__Aliases" ] ~exports:(alias_exports [ "System_error" ]) |> open_paths [ [ "Kernel"; "Aliases" ] ] in
...
let test_deps_collect_field_access_modules_from_implicit_alias_opens = fun _ctx ->
  let env = Syn.Deps.Env.empty |> Syn.Deps.Env.add_scoped_binding ~path:[ "Kernel"; "Async"; "Aliases" ] ~free_names:[ "Kernel__Async__Aliases" ] ~exports:(alias_exports [ "Libc" ]) |> open_paths [ [ "Kernel"; "Async"; "Aliases" ] ] in
...
|ocaml} in
  ...
  | Ok modules -> Error ("expected deps [Dep_graph, Module], got [" ^ String.concat ", " modules ^ "]")
  | Error err -> Error err
...
let tests = Test.[
  ...
  case "deps keep unknown qualified roots after open" test_deps_keep_unknown_qualified_roots_after_open;
