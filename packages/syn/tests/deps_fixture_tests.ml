...
let source_slice = fun source -> IO.IoVec.IoSlice.from_string source |> Result.expect ~msg:"failed to create deps fixture source slice"
...
let test_fixture = fun ~(ctx:Test.FixtureRunner.ctx) -> Test.Snapshot.assert_with ~ctx:ctx.test ~render:(
  fun value -> value
) ~actual:(render_actual ~fixture_path:ctx.fixture_path)

let main ~args =
  let tests = Test.FixtureRunner.cases () ~dir:fixture_root ~filter:has_expected ~run:(
    fun ctx -> test_fixture ~ctx
  ) in Test.Cli.main ~name:"syn-deps-fixtures" ~tests ~args ()
