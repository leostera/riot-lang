...
let nth_structure_item = fun (root: Ast.source_file) target ->
  ...
  let seen = ref 0 in
  Ast.SourceFile.for_each_structure_item root
    ~fn:(fun item ->
      match !found with
      | Some _ -> ()
      | None ->
...
let nth_signature_item = fun (root: Ast.source_file) target ->
  let found = ref None in
  let seen = ref 0 in
  Ast.SourceFile.for_each_signature_item root
    ~fn:(fun item ->
      match !found with
      | Some _ -> ()
      | None ->
...
let assert_last_ident_text = fun path expected ->
  let token = Ast.Path.last_ident path |> require_some ~msg:"expected last path ident" in
...
let test_expression_views = fun _ctx ->
  ...
  | Ast.Expr.Match { scrutinee; first_case } ->
      ...
      ignore
        (case.Ast.MatchCase.body |> expect_some ~msg:"expected case body" |> Result.expect ~msg:"case body");
      Ok ()
...
let test_assignment_operator_views = fun _ctx ->
  ...
  let assert_assignment_operator body expected =
    match Ast.Expr.view body with
    | Ast.Expr.Assign { operator=Some operator; _ } -> Test.assert_equal
      ~expected
      ~actual:(Ast.Token.text operator)
...
let test_labeled_application_after_poly_variant_argument = fun _ctx ->
  ...
  let rec application_parts expr args =
    ...
    | Ast.Expr.Apply { callee=Some callee; argument=Some argument } -> application_parts
      callee
      (argument :: args)
    | _ -> (expr, args)
  in
...
let test_poly_variant_tuple_pattern_boundary = fun _ctx ->
  let source = {ocaml|let equal_constraint = fun left right ->
...
|ocaml}
  ...
  | Ast.Expr.Match { first_case=Some first_case; _ } ->
      let case = Ast.MatchCase.view first_case in
      let pattern = case.Ast.MatchCase.pattern |> require_some ~msg:"expected first match case pattern" in
      (
        match Ast.Pattern.view pattern with
        | Ast.Pattern.Tuple -> ()
      ...
      (
        match Ast.Pattern.view (Vector.get_unchecked children ~at:0), Ast.Pattern.view
          (Vector.get_unchecked children ~at:1) with
...
let test_signature_and_type_views = fun _ctx ->
  ...
  (
    ...
    | Ast.SignatureItem.Type decl ->
        ...
        let manifest = Ast.TypeDeclaration.manifest decl |> require_some ~msg:"expected type manifest" in
        ...
        (
          ...
          | Ast.TypeExpr.Apply { argument=Some argument; constructor=Some constructor } ->
              ...
              (
                ...
                | Ast.TypeExpr.Tuple {
                  separator=Ast.TypeExpr.Comma;
                  left=Some left;
                  right=Some right
                } -> (
                    match Ast.TypeExpr.view left, Ast.TypeExpr.view right with
                    | Ast.TypeExpr.Var { name=Some left_name }, Ast.TypeExpr.Var {
  ...
  (
    ...
    | Ast.TypeExpr.Poly { body=Some body } ->
        ...
        (
          ...
          | Ast.TypeExpr.Arrow { left=Some left; right=Some _ } -> (
              match Ast.TypeExpr.view left with
              | Ast.TypeExpr.Labeled { label=Some label; annotation=Some _; _ } -> Test.assert_equal
                ~expected:"reader"
                ~actual:(Ast.Token.text label)
...
let test_type_declaration_parameters = fun _ctx ->
  ...
  | Ast.SignatureItem.Type decl ->
      ...
      Ast.TypeDeclaration.for_each_parameter decl
        ~fn:(
          function
          | Ast.TypeDeclaration.Named { name; variance; _ } -> named := Some (
            Ast.Token.text name,
...
let test_type_declaration_body_group_views = fun _ctx ->
  ...
  (
    ...
    | Ast.SignatureItem.Type decl ->
        ...
        Ast.VariantType.for_each_constructor variant
          ~fn:(fun constructor ->
  ...
  | Ast.SignatureItem.Type decl ->
      ...
      Ast.RecordType.for_each_field record
        ~fn:(fun field ->
          ...
          (
            ...
            | Ast.TypeExpr.Path { path } ->
                let last = Ast.Path.last_ident path |> require_some ~msg:"expected field type path" in
...
let test_type_alias_record_representation_views = fun _ctx ->
  ...
  | Ast.SignatureItem.Type decl ->
      ...
      Ast.RecordType.for_each_field record
        ~fn:(fun field ->
          let name = Ast.RecordField.name field |> require_some ~msg:"expected field name" in
  ...
  (
    ...
    | Ast.SignatureItem.TypeExtension decl ->
        ...
        (
          ...
          | Some payload -> (
              match Ast.TypeExpr.view payload with
              | Ast.TypeExpr.Var { name=Some payload_name } -> Test.assert_equal
                ~expected:"a"
                ~actual:(Ast.Token.text payload_name)
...
let test_module_declaration_tokens = fun _ctx ->
  ...
  (
    ...
    | Ast.StructureItem.Module decl ->
        let name = Ast.ModuleDeclaration.name decl |> require_some ~msg:"expected module wildcard name" in
...
let test_module_type_declaration_tokens = fun _ctx ->
  ...
  (
    ...
    | Ast.SignatureItem.ModuleType decl -> Test.assert_equal
  ...
  | Ast.SignatureItem.ModuleType decl ->
      ...
      (
        ...
        | Some base ->
            ...
            let name = Ast.Path.last_ident path |> require_some ~msg:"expected constrained base name" in
            Test.assert_equal ~expected:"Driver" ~actual:(Ast.Token.text name)
        | None -> panic "expected constrained base module type"
      );
      let seen = ref 0 in
      Ast.ModuleTypeDeclaration.for_each_constraint decl
        ~fn:(fun constraint_ ->
          let index = !seen in
          seen := !seen + 1;
          ...
          | Ast.ModuleTypeConstraint.Unknown _ ->
...
let last_path_text = fun path ->
  let token = Ast.Path.last_ident path |> require_some ~msg:"expected path ident" in
...
let test_record_views = fun _ctx ->
  ...
  Ast.RecordExpr.for_each_field record_view
    ~fn:(fun field ->
      let name = field.Ast.RecordExpr.path |> require_some ~msg:"expected record field path" |> last_path_text in
  ...
  Ast.RecordExpr.for_each_field update_view
    ~fn:(fun field ->
      let name = field.Ast.RecordExpr.path |> require_some ~msg:"expected update field path" |> last_path_text in
  ...
  (
    ...
    | Ast.LocalOpenExpr.Delimited { module_path=Some module_path; body=Some body; _ } ->
        ...
        Ast.RecordExpr.for_each_field scoped_record
          ~fn:(fun field ->
            let name = field.Ast.RecordExpr.path
...
let test_binding_operator_views = fun _ctx ->
  ...
  let clauses = ref [] in
  Ast.BindingOperatorExpr.for_each_clause binding_operator
    ~fn:(fun clause ->
      let keyword = clause.Ast.BindingOperatorExpr.keyword
      |> require_some ~msg:"expected binding operator keyword"
      |> Ast.Token.text in
      let operator = clause.Ast.BindingOperatorExpr.operator
...
let test_local_open_views = fun _ctx ->
  ...
  (
    ...
    | Ast.LocalOpenExpr.LetOpen {
      ...
      _
        ...
        (
          match Ast.Expr.view body with
          | Ast.Expr.Path { path } -> Test.assert_equal
            ~expected:"result"
  ...
  let local_open_record_pattern = Ast.LocalOpenPattern.cast local_open_record_pattern
...
let test_unreachable_expression_views = fun _ctx ->
  ...
  Ast.Expr.for_each_match_case match_expr
    ~fn:(fun match_case ->
...
let test_extension_views = fun _ctx ->
  ...
  (
    ...
    | Ast.StructureItem.Extension item -> Test.assert_equal
  ...
  (
    ...
    | Ast.StructureItem.Attribute item -> Test.assert_equal
      ~expected:"[@@@warning \"-32\"]"
      ~actual:(attribute_shell_text
...
let test_special_pattern_views = fun _ctx ->
  ...
  | [locally_abstract;first_class_module] ->
      ...
      let locally_abstract = Ast.LocallyAbstractTypePattern.cast locally_abstract
      ...
      let first_class_module = Ast.FirstClassModulePattern.cast first_class_module
      ...
      | Ast.Pattern.OptionalParamDefault parameter -> (
          ...
          | Ast.Parameter.OptionalDefault {
            ...
            default=Some default
              ...
              (
                match Ast.Pattern.view pattern with
                | Ast.Pattern.Path { path } -> Test.assert_equal
                  ~expected:"cfg"
                  ~actual:(last_path_text path)
...
let tests = [
  ...
  Test.case "ast keeps comma outside polymorphic variant pattern payloads" test_poly_variant_tuple_pattern_boundary;
