...
type t3 = None | Some (* value *) of (* type *) int

(* Variant with multiple arguments *)
type t4 =
  | Pair (* constructor *) of
      (* first *) int (* separator *) * (* second *) string

(* Polymorphic variant with trivia *)
type t5 = [ `A (* first *) | `B (* second *) of (* payload *) int ]

(* GADT with trivia *)
type _ t6 =
  | Int (* int case *) : (* result *) int t6
  | String (* string case *) : (* result *) string t6
