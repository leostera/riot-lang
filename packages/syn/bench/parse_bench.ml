...
let load_fixture = fun name path ->
  let source = Fs.read path |> Result.expect ~msg:("failed to read parse benchmark fixture: " ^ Path.to_string path) in
  {
    name;
    path;
    source;
...
let touch_parse = fun (result: Parser.parse_result) ->
  let root = SyntaxTree.root result.Parser.tree in checksum := !checksum lxor root.SyntaxTree.full_width lxor Vector.length result.Parser.diagnostics

let bench_parse = fun fixture ->
  let result = parse ~filename:fixture.path fixture.slice in touch_parse result
...
let has_lossless_snapshot = fun path ->
  let snapshot_path = Path.to_string path ^ ".expected_lossless.json" |> Path.from_string |> Result.expect ~msg:"lossless snapshot path should stay valid UTF-8" in Fs.exists snapshot_path |> Result.unwrap_or ~default:false

let load_valid_fixture_corpus = fun () ->
  let fixtures = Vector.with_capacity ~size:1_100 in
  Fs.Walker.walk ~roots:[ Path.v "packages/syn/tests/fixtures" ] ~sort:true ~f:(
    fun item ->
      let path = Fs.Walker.FileItem.path item in
      (
        if is_source_file path && has_lossless_snapshot path then
          let name = Fs.Walker.FileItem.name item in Vector.push fixtures ~value:(load_fixture name path)
      );
      Fs.Walker.Continue
...
let fixture_benchmark = fun ~config fixture ->
  Bench.with_config ~config ("parse: " ^ fixture.name)
    (
...
let selected_benchmarks = fun () ->
  [
    fixture_benchmark ~config:tiny_config (load_fixture "tiny let binding" (Path.v "packages/syn/tests/fixtures/0001_basic.ml"));
    fixture_benchmark ~config:small_config (load_fixture "quoted extensions" (Path.v "packages/syn/tests/fixtures/ocaml_quotedextensions.ml"));
    fixture_benchmark ~config:medium_config (load_fixture "docstrings" (Path.v "packages/syn/tests/fixtures/ocaml_docstrings.ml"));
    fixture_benchmark ~config:large_config (load_fixture "vendored makedepend" (Path.v "packages/syn/tests/deps_fixtures/0027_vendored_makedepend.ml"));
  ]

let corpus_benchmark = fun () ->
  let fixtures = load_valid_fixture_corpus () in
  Bench.with_config ~config:corpus_config ("parse: valid fixture corpus (" ^ Int.to_string (Vector.length fixtures) ^ " files)")
    (
