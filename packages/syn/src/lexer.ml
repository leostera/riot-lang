...
let delimiter_of_quoted_string_header = fun header ~is_extension ->
  if not is_extension then
    ...
    | Some i when i + 1 < String.length trimmed -> String.sub trimmed ~offset:(i + 1) ~len:(String.length trimmed - i - 1)
    | _ -> ""

let rec find_quoted_string_pipe_offset = fun cursor offset ~is_extension ->
  match Cursor.peek_n cursor offset with
  | Some '|' -> Some offset
  | Some c when is_extension && (is_quoted_string_header_char c || is_whitespace c) -> find_quoted_string_pipe_offset cursor (offset + 1) ~is_extension
  | Some c when is_quoted_string_header_char c -> find_quoted_string_pipe_offset cursor (offset + 1) ~is_extension
  | Some _ -> None
  | None -> None
...
let rec lex_block_comment = fun cursor depth content_start token_start ->
  ...
  | None ->
      ...
      let end_ = Cursor.position cursor in make_token ~kind:(Token.Comment { value; terminated = false }) ~span:{ start = token_start; end_ }
  | Some '(' -> (
    Cursor.advance cursor;
    match Cursor.peek cursor with
    | Some '*' ->
  ...
  | Some '*' -> (
    ...
    | Some ')' ->
        ...
        if depth = 1 then
          ...
          let end_ = Cursor.position cursor in make_token ~kind:(Token.Comment { value; terminated = true }) ~span:{ start = token_start; end_ }
  ...
  let rec lex_content depth =
    ...
    | None ->
        ...
        if is_docstring then
          make_token ~kind:(Token.Docstring { value; terminated = false }) ~span:{ start = token_start; end_ }
        else make_token ~kind:(Token.Comment { value; terminated = false }) ~span:{ start = token_start; end_ }
    | Some '(' -> (
      Cursor.advance cursor;
      match Cursor.peek cursor with
      | Some '*' ->
    ...
    | Some '*' -> (
      ...
      | Some ')' ->
          ...
          if depth = 1 then
            ...
            let end_ = Cursor.position cursor in
            if is_docstring then
              make_token ~kind:(Token.Docstring { value; terminated = true }) ~span:{ start = token_start; end_ }
            else make_token ~kind:(Token.Comment { value; terminated = true }) ~span:{ start = token_start; end_ }
    ...
    if ident = "_" then
      ...
      | Some kw -> (
        if Keyword.is_opening kw then
          let delim = Token.delimiter_of_keyword kw |> Option.unwrap in Token.OpenDelim delim
        else
          if Keyword.is_closing kw then
          ...
          (* Default fallback *)
            ...
            match kw with
            | Lnot | Land | Lor | Lxor | Lsl | Lsr | Asr | Mod -> Token.Ident ident
            | _ -> Token.Keyword kw
      )
...
let lex_raw_ident = fun cursor token_start ->
  ...
  let len = Cursor.position cursor - start in
  let ident = Cursor.slice cursor start len in
  let end_ = Cursor.position cursor in make_token ~kind:(Token.Ident ("\\#" ^ ident)) ~span:{ start = token_start; end_ }
...
let lex_number = fun cursor token_start ->
  ...
  let consume_float_exponent () =
    if exponent_follows () then
      ...
      let _ =
        ...
        | _ -> ()
      in
      let _ = Cursor.take_while cursor is_digit_or_underscore in Some (Cursor.slice cursor start (Cursor.position cursor - start) |> remove_underscores)
    else None
  in
  ...
  | _ ->
      ...
      let kind =
        ...
        | Some c when is_alpha c ->
            let _ = consume_numeric_suffix () in Token.Literal (Int 0)
        | Some '.' -> (
          match Cursor.peek_n cursor 1 with
...
let lex_string = fun cursor token_start ->
  ...
  let end_ = Cursor.position cursor in make_token ~kind:(Token.Literal (String { value; terminated })) ~span:{ start = token_start; end_ }
...
let lex_quoted_string = fun cursor token_start ->
  ...
  | Some pipe_offset ->
      let delimiter =
        if pipe_offset = 1 then
          ""
        else Cursor.slice cursor (Cursor.position cursor + 1) (pipe_offset - 1)
      in
      for _ = 0 to pipe_offset do Cursor.advance cursor done;
      let start = Cursor.position cursor in
      let rec loop () =
  ...
  | None ->
      Cursor.advance cursor;
      let end_ = Cursor.position cursor in make_token ~kind:(Token.OpenDelim Brace) ~span:(Ceibo.Span.make ~start:token_start ~end_)
...
let lex_char = fun cursor token_start ->
  ...
  in
  let end_ = Cursor.position cursor in make_token ~kind ~span:{ start = token_start; end_ }
...
let lex_type_var = fun cursor token_start ->
  ...
  *)
  Cursor.advance cursor;
  let end_ = Cursor.position cursor in make_token ~kind:Token.Quote ~span:{ start = token_start; end_ }
...
let next = fun cursor delim_stack ->
  ...
  if Cursor.is_eof cursor then
    ...
    | Some '(' -> (
      ...
      | _ ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:(Token.OpenDelim Paren) ~span:(Ceibo.Span.make ~start ~end_)
    )
    | Some ')' ->
        Cursor.advance cursor;
        let end_ = Cursor.position cursor in make_token ~kind:(Token.CloseDelim Paren) ~span:(Ceibo.Span.make ~start ~end_)
    | Some '[' -> (
      Cursor.advance cursor;
      match Cursor.peek cursor with
      | Some '|' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:(Token.OpenDelim Array) ~span:(Ceibo.Span.make ~start ~end_)
      | _ ->
          let end_ = Cursor.position cursor in make_token ~kind:(Token.OpenDelim Bracket) ~span:(Ceibo.Span.make ~start ~end_)
    )
    | Some ']' ->
        Cursor.advance cursor;
        let end_ = Cursor.position cursor in make_token ~kind:(Token.CloseDelim Bracket) ~span:(Ceibo.Span.make ~start ~end_)
    | Some '{' -> (
      match Cursor.peek_n cursor 1 with
      ...
      | _ ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:(Token.OpenDelim Brace) ~span:(Ceibo.Span.make ~start ~end_)
    )
    | Some '}' ->
        Cursor.advance cursor;
        let end_ = Cursor.position cursor in make_token ~kind:(Token.CloseDelim Brace) ~span:(Ceibo.Span.make ~start ~end_)
    | Some '+' -> (
      Cursor.advance cursor;
      match Cursor.peek cursor with
      | Some '.' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.PlusDot ~span:(Ceibo.Span.make ~start ~end_)
      | _ ->
          let end_ = Cursor.position cursor in make_token ~kind:Token.Plus ~span:(Ceibo.Span.make ~start ~end_)
    )
    | Some '-' -> (
      Cursor.advance cursor;
      match Cursor.peek cursor with
      | Some '>' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.Arrow ~span:(Ceibo.Span.make ~start ~end_)
      | Some '.' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.MinusDot ~span:(Ceibo.Span.make ~start ~end_)
      | _ ->
          let end_ = Cursor.position cursor in make_token ~kind:Token.Minus ~span:(Ceibo.Span.make ~start ~end_)
    )
    | Some '*' -> (
      Cursor.advance cursor;
      match Cursor.peek cursor with
      | Some '*' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.StarStar ~span:(Ceibo.Span.make ~start ~end_)
      | Some '.' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.StarDot ~span:(Ceibo.Span.make ~start ~end_)
      | _ ->
          let end_ = Cursor.position cursor in make_token ~kind:Token.Star ~span:(Ceibo.Span.make ~start ~end_)
    )
    | Some '/' -> (
      Cursor.advance cursor;
      match Cursor.peek cursor with
      | Some '.' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.SlashDot ~span:(Ceibo.Span.make ~start ~end_)
      | _ ->
          let end_ = Cursor.position cursor in make_token ~kind:Token.Slash ~span:(Ceibo.Span.make ~start ~end_)
    )
    | Some '%' -> (
      Cursor.advance cursor;
      match Cursor.peek cursor with
      | Some '>' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.PercentGt ~span:(Ceibo.Span.make ~start ~end_)
      | _ ->
          let end_ = Cursor.position cursor in make_token ~kind:Token.Percent ~span:(Ceibo.Span.make ~start ~end_)
    )
    | Some '^' ->
        Cursor.advance cursor;
        let end_ = Cursor.position cursor in make_token ~kind:Token.Caret ~span:(Ceibo.Span.make ~start ~end_)
    | Some '=' -> (
      Cursor.advance cursor;
      match Cursor.peek cursor with
      | Some '>' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.FatArrow ~span:(Ceibo.Span.make ~start ~end_)
      | Some '=' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.EqEq ~span:(Ceibo.Span.make ~start ~end_)
      | _ ->
          let end_ = Cursor.position cursor in make_token ~kind:Token.Eq ~span:(Ceibo.Span.make ~start ~end_)
    )
    | Some '<' -> (
      Cursor.advance cursor;
      match Cursor.peek cursor with
      | Some '=' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.LtEq ~span:(Ceibo.Span.make ~start ~end_)
      | Some '-' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.LeftArrow ~span:(Ceibo.Span.make ~start ~end_)
      | Some '>' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.Ne ~span:(Ceibo.Span.make ~start ~end_)
      | Some '%' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.LtPercent ~span:(Ceibo.Span.make ~start ~end_)
      | _ ->
          let end_ = Cursor.position cursor in make_token ~kind:Token.Lt ~span:(Ceibo.Span.make ~start ~end_)
    )
    | Some '>' -> (
      Cursor.advance cursor;
      match Cursor.peek cursor with
      | Some '=' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.GtEq ~span:(Ceibo.Span.make ~start ~end_)
      | _ ->
          let end_ = Cursor.position cursor in make_token ~kind:Token.Gt ~span:(Ceibo.Span.make ~start ~end_)
    )
    | Some '!' -> (
      Cursor.advance cursor;
      match Cursor.peek cursor with
      | Some '=' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.BangEq ~span:(Ceibo.Span.make ~start ~end_)
      | _ ->
          let end_ = Cursor.position cursor in make_token ~kind:Token.Bang ~span:(Ceibo.Span.make ~start ~end_)
    )
    | Some '&' -> (
      Cursor.advance cursor;
      match Cursor.peek cursor with
      | Some '&' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.And ~span:(Ceibo.Span.make ~start ~end_)
      | _ ->
          let end_ = Cursor.position cursor in make_token ~kind:Token.Ampersand ~span:(Ceibo.Span.make ~start ~end_)
    )
    | Some '|' -> (
      Cursor.advance cursor;
      match Cursor.peek cursor with
      | Some '|' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.Or ~span:(Ceibo.Span.make ~start ~end_)
      | Some '>' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.PipeGt ~span:(Ceibo.Span.make ~start ~end_)
      | Some ']' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:(Token.CloseDelim Array) ~span:(Ceibo.Span.make ~start ~end_)
      | _ ->
          let end_ = Cursor.position cursor in make_token ~kind:Token.Pipe ~span:(Ceibo.Span.make ~start ~end_)
    )
    | Some ':' -> (
      Cursor.advance cursor;
      match Cursor.peek cursor with
      | Some ':' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.ColonColon ~span:(Ceibo.Span.make ~start ~end_)
      | Some '=' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.ColonEq ~span:(Ceibo.Span.make ~start ~end_)
      | _ ->
          let end_ = Cursor.position cursor in make_token ~kind:Token.Colon ~span:(Ceibo.Span.make ~start ~end_)
    )
    | Some ';' ->
        Cursor.advance cursor;
        let end_ = Cursor.position cursor in make_token ~kind:Token.Semi ~span:(Ceibo.Span.make ~start ~end_)
    | Some ',' ->
        Cursor.advance cursor;
        let end_ = Cursor.position cursor in make_token ~kind:Token.Comma ~span:(Ceibo.Span.make ~start ~end_)
    | Some '.' -> (
      Cursor.advance cursor;
      match Cursor.peek cursor with
      | Some '.' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.DotDot ~span:(Ceibo.Span.make ~start ~end_)
      | _ ->
          let end_ = Cursor.position cursor in make_token ~kind:Token.Dot ~span:(Ceibo.Span.make ~start ~end_)
    )
    | Some '?' ->
        Cursor.advance cursor;
        let end_ = Cursor.position cursor in make_token ~kind:Token.Question ~span:(Ceibo.Span.make ~start ~end_)
    | Some '@' -> (
      Cursor.advance cursor;
      match Cursor.peek cursor with
      | Some '@' ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:Token.AtAt ~span:(Ceibo.Span.make ~start ~end_)
      | _ ->
          let end_ = Cursor.position cursor in make_token ~kind:Token.At ~span:(Ceibo.Span.make ~start ~end_)
    )
    | Some '#' ->
        Cursor.advance cursor;
        let end_ = Cursor.position cursor in make_token ~kind:Token.Hash ~span:(Ceibo.Span.make ~start ~end_)
    | Some '~' ->
        Cursor.advance cursor;
        let end_ = Cursor.position cursor in make_token ~kind:Token.Tilde ~span:(Ceibo.Span.make ~start ~end_)
    | Some '`' ->
        Cursor.advance cursor;
        let end_ = Cursor.position cursor in make_token ~kind:Token.Backtick ~span:(Ceibo.Span.make ~start ~end_)
    | Some '$' ->
        Cursor.advance cursor;
        let end_ = Cursor.position cursor in make_token ~kind:Token.Dollar ~span:(Ceibo.Span.make ~start ~end_)
    ...
    | Some '\\' -> (
      ...
      | Some '#' -> (
        ...
        | _ ->
            Cursor.advance cursor;
            let end_ = Cursor.position cursor in make_token ~kind:(Token.Unknown '\\') ~span:(Ceibo.Span.make ~start ~end_)
      )
      | _ ->
          Cursor.advance cursor;
          let end_ = Cursor.position cursor in make_token ~kind:(Token.Unknown '\\') ~span:(Ceibo.Span.make ~start ~end_)
    ...
    | Some c ->
        Cursor.advance cursor;
        let end_ = Cursor.position cursor in make_token ~kind:(Token.Unknown c) ~span:(Ceibo.Span.make ~start ~end_)
  ...
  let attach_pending_trivia token pending_rev =
    Token.with_leading_trivia token
      (
        ...
        List.for_each leading ~fn:(
          fun item -> Vector.push trivia ~value:item
        );
