...
let expected_message = fun diag ->
  ...
  | MalformedTypeVariable _ ->
      "type variable identifier (e.g., 'a, 'b)"
  | MissingLetBindingPattern _ ->
      "pattern (e.g., variable, _, Some x, or literal)"
  | MissingLetBindingEquals _ ->
      "="
  | MissingLetBindingExpr _ ->
      "expression"
  | UnexpectedStructureItem _ ->
      "structure item (e.g., let, type, module)"
  | UnexpectedSignatureItem _ ->
      "signature item (e.g., val, type, module)"
  | InvalidPattern _ ->
      "identifier or pattern"
  | InvalidExpression _ ->
      "expression"
  | InvalidConstant _ ->
      "constant (integer, float, string, or char)"
  | InvalidTypeExpression _ ->
      "type definition"
  | MissingLetKeyword _ ->
      "let keyword"
  | MissingTypeKeyword _ ->
      "type keyword"
  | MissingTypeName _ ->
      "type name"
  ...
  | UnclosedTypeParams _ ->
      ")"
  | EmptyCharLiteral ->
      "non-empty character literal"
  | MultiCharLiteral _ ->
      "single character"
  | UnclosedCharLiteral _ ->
      "' (closing quote)"
  ...
  | TuplePatternExtraComma _ ->
      "pattern element (not comma)"
  | ConstructorPatternNeedsParens { constructor; _ } ->
      "parentheses around " ^ constructor ^ " arguments"
  | ConsPatternMissingHead _ ->
      "pattern before ::"
  | ConsPatternMissingTail _ ->
      "pattern after ::"
  | OrPatternMissing _ ->
      "pattern (not bare |)"
  | OrPatternDouble _ ->
      "pattern between | operators"
  | MutableFieldMissingName _ ->
      "field name after mutable"
  ...
  | PolyTypeMissingVarName _ ->
      "type variable name after '"
  | PolyTypeMissingDot _ ->
      ". (dot) after type variables in polymorphic type"
  ...
  | MissingWithKeyword _ ->
      "with keyword"
  | InvalidModuleName _ ->
      "module name starting with uppercase letter"
...
let fix_message = fun diag ->
  ...
  | MissingLetBindingEquals _ ->
      Some "add = between the pattern and expression"
  | MissingLetBindingExpr _ ->
      Some "add an expression after the ="
  | MissingTypeName _ ->
      Some "add a type name after the type keyword"
  | MissingTypeDeclEquals _ ->
  ...
  | MultiCharLiteral _ ->
      Some "use only one character in the literal"
  | UnclosedCharLiteral _ ->
      Some "add a closing ' (quote) after the character"
  ...
  | ConsPatternMissingHead _ ->
      Some "add a pattern before the :: operator"
  | ConsPatternMissingTail _ ->
      Some "add a pattern after the :: operator"
  | OrPatternMissing _ ->
      Some "add patterns on both sides of the | operator"
  | OrPatternDouble _ ->
...
let main_message = fun diag ->
  ...
  | UnclosedCharLiteral { text } ->
      "missing ' (quote) after " ^ text
  | UnexpectedClosingDelimiter { delimiter; _ } ->
      "unexpected extra closing " ^ delimiter
  | MissingModuleDeclEquals _ ->
      "expected = between module name and module expression"
  | MissingExternalColon _ ->
        ...
        | Some (Object kind_fields), Some (Object span_fields) -> (
            ...
            | Some id ->
                ...
                let fix =
                  ...
                  |> Option.unwrap_or ~default:""
                in
                let strip_prefix_str = fun str ~prefix ->
                  ...
                  if str_len >= prefix_len && String.starts_with ~prefix str then
                    ...
                    ""
                in
                let parse_missing_binary_operand = fun expected ->
                ...
                let parse_unclosed_delimiter = fun expected fix ->
                  ...
                  | _ -> ""
                in
                let parse_bracketed_type_name = fun fix ->
                  ...
                  if tail = "" then
                    ...
                    ""
                in
                let parse_quoted_field = fun expected ~prefix ->
