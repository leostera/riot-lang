...
let malformed_type_variable = fun ~found:token ~text ~span ->
  ...
  let found = { kind = kind_str; text } in make ~kind:(MalformedTypeVariable { found }) ~span

let missing_let_binding_pattern = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MissingLetBindingPattern { found }) ~span

let missing_let_binding_equals = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MissingLetBindingEquals { found }) ~span

let missing_let_binding_expr = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MissingLetBindingExpr { found }) ~span

let unexpected_structure_item = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(UnexpectedStructureItem { found }) ~span

let unexpected_signature_item = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(UnexpectedSignatureItem { found }) ~span

let invalid_pattern = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(InvalidPattern { found }) ~span

let invalid_expression = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(InvalidExpression { found }) ~span

let invalid_constant = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(InvalidConstant { found }) ~span

let invalid_type_expression = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(InvalidTypeExpression { found }) ~span

let missing_let_keyword = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MissingLetKeyword { found }) ~span

let missing_type_keyword = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MissingTypeKeyword { found }) ~span

let missing_type_name = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MissingTypeName { found }) ~span

let missing_type_decl_equals = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MissingTypeDeclEquals { found }) ~span

let unclosed_delimiter = fun ~opener ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(UnclosedDelimiter { opener; found }) ~span
...
let unclosed_type_params = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(UnclosedTypeParams { found }) ~span

let missing_binary_operand = fun ~operator ~side ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MissingBinaryOperand { operator; side; found }) ~span

let consecutive_binary_operators = fun ~operators ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(ConsecutiveBinaryOperators { operators; found }) ~span

let invalid_type_parameter = fun ~text ~found:token ~text_found ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text = text_found } in make ~kind:(InvalidTypeParameter { text; found }) ~span

let uppercase_type_variable = fun ~text ~found:token ~text_found ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text = text_found } in make ~kind:(UppercaseTypeVariable { text; found }) ~span

let uppercase_type_name = fun ~text ~found:token ~text_found ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text = text_found } in make ~kind:(UppercaseTypeName { text; found }) ~span

let bracketed_type_parameters = fun ~type_name ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(BracketedTypeParameters { type_name; found }) ~span

let list_double_semicolon = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(ListDoubleSemicolon { found }) ~span

let if_missing_then = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(IfMissingThen { found }) ~span

let match_missing_scrutinee = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MatchMissingScrutinee { found }) ~span

let match_missing_with = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MatchMissingWith { found }) ~span

let match_missing_pattern = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MatchMissingPattern { found }) ~span

let match_guard_missing_expr = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MatchGuardMissingExpr { found }) ~span

let tuple_pattern_extra_comma = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(TuplePatternExtraComma { found }) ~span

let constructor_pattern_needs_parens = fun ~constructor ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(ConstructorPatternNeedsParens { constructor; found }) ~span

let cons_pattern_missing_head = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(ConsPatternMissingHead { found }) ~span

let cons_pattern_missing_tail = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(ConsPatternMissingTail { found }) ~span

let or_pattern_missing = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(OrPatternMissing { found }) ~span

let or_pattern_double = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(OrPatternDouble { found }) ~span

let mutable_field_missing_name = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MutableFieldMissingName { found }) ~span

let record_field_missing_colon = fun ~field_name ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(RecordFieldMissingColon { field_name; found }) ~span

let record_field_missing_type = fun ~field_name ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(RecordFieldMissingType { field_name; found }) ~span

let poly_type_missing_var_name = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(PolyTypeMissingVarName { found }) ~span

let poly_type_missing_dot = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(PolyTypeMissingDot { found }) ~span

let unexpected_closing_delimiter = fun ~delimiter ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(UnexpectedClosingDelimiter { delimiter; found }) ~span

let missing_module_decl_equals = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MissingModuleDeclEquals { found }) ~span

let missing_external_colon = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MissingExternalColon { found }) ~span

let missing_exception_name = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MissingExceptionName { found }) ~span

let missing_module_path = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MissingModulePath { found }) ~span

let missing_module_type_name = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MissingModuleTypeName { found }) ~span

let missing_module_type_expr = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MissingModuleTypeExpr { found }) ~span

let missing_module_expr = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MissingModuleExpr { found }) ~span

let missing_with_keyword = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(MissingWithKeyword { found }) ~span

let invalid_module_name = fun ~found:token ~text ~span ->
  let kind_str = Token.to_string token in
  let found = { kind = kind_str; text } in make ~kind:(InvalidModuleName { found }) ~span
...
let expected_message = fun diag ->
  ...
  | ConstructorPatternNeedsParens { constructor; _ } -> "parentheses around " ^ constructor ^ " arguments"
...
let fix_message = fun diag ->
  match diag.kind with
  | MalformedTypeVariable _ -> Some "remove the space or comment between the quote and the variable name"
  ...
  | InvalidPattern { found } -> (
    ...
    | "keyword" -> Some ("use a different name like " ^ found.text ^ "_ or a raw identifier like \\#" ^ found.text)
  ...
  | ConsecutiveBinaryOperators _ -> Some "add an expression between the operators or remove one operator"
  | InvalidTypeParameter { text; _ } ->
      if text = "__" then
        Some "use _ instead of __"
      else Some "use a valid type parameter like 'a or _"
  | UppercaseTypeVariable { text; _ } -> Some ("change " ^ text ^ " to " ^ String.lowercase_ascii text)
  | UppercaseTypeName { text; _ } ->
      let lower = String.lowercase_ascii text in Some ("change " ^ text ^ " to " ^ lower)
  | BracketedTypeParameters { type_name; _ } -> Some ("put generics on the left of the type name with parenthesis, like \
          this: ('a, 'b) " ^ type_name)
  | ListDoubleSemicolon _ -> Some "remove one semicolon - list elements are separated by single \
         semicolons"
  ...
  | ConstructorPatternNeedsParens { constructor; _ } -> Some ("wrap arguments in parentheses: " ^ constructor ^ " (...)")
  ...
  | RecordFieldMissingColon { field_name; _ } -> Some ("add a colon after field name '" ^ field_name ^ "'")
  | RecordFieldMissingType { field_name; _ } -> Some ("add a type definition after the colon for field '" ^ field_name ^ "'")
  ...
  | InvalidModuleName { found } ->
      ...
      else Some ("capitalize " ^ found.text ^ " as " ^ String.capitalize_ascii found.text)
  | _ -> None
...
let hint_message = fun diag -> diag |> error_id |> Error.explain

let id = fun err -> err |> error_id |> Error.id_to_string
...
let main_message = fun diag ->
  ...
  | _ ->
      ...
      let found_kind = (found_token diag).kind in "expected " ^ expected ^ ", found " ^ found_kind

(** Convert diagnostic to JSON for machine consumption *)
let to_json = fun err -> let open Data.Json in
...
Object [
  ...
  "span", Object [
    "start", Int err.span.start;
    "end", Int err.span.end_;
  ];
]

(* parse_result removed - now in Parser module *)
let from_json = fun json -> let open Data.Json in
let field name fields = List.find fields ~fn:(
...
| Object fields -> (
  ...
  | Some (Object kind_fields), Some (Object span_fields) -> (
    let id_opt = Option.and_then (field "id" kind_fields) ~fn:(
      function
      | String s -> Some s
      | _ -> None
    ) in
    ...
    let found =
      ...
      | Some (Object found_fields) ->
          let kind = Option.and_then (field "kind" found_fields) ~fn:(
            function
            | String s -> Some s
            | _ -> None
          ) |> Option.unwrap_or ~default:"" in
          let text = Option.and_then (field "text" found_fields) ~fn:(
            function
            | String s -> Some s
            | _ -> None
          ) |> Option.unwrap_or ~default:"" in { kind; text }
      | _ -> { kind = ""; text = "" }
    in
    let start = Option.and_then (field "start" span_fields) ~fn:(
      function
      | Int n -> Some n
      | _ -> None
    ) |> Option.unwrap_or ~default:0 in
    let end_ = Option.and_then (field "end" span_fields) ~fn:(
      function
      | Int n -> Some n
      | _ -> None
    ) |> Option.unwrap_or ~default:0 in
    ...
    match id_opt with
    | Some id ->
        let expected = Option.and_then (field "expected" kind_fields) ~fn:(
          function
          | String s -> Some s
          | _ -> None
        ) |> Option.unwrap_or ~default:"" in
        let fix = Option.and_then (field "fix" kind_fields) ~fn:(
          function
          | String s -> Some s
          | _ -> None
        ) |> Option.unwrap_or ~default:"" in
        ...
        let parse_missing_binary_operand expected =
          ...
          if String.starts_with ~prefix:right_prefix expected then
            ...
            if String.starts_with ~prefix:left_prefix expected then
              ("left", strip_prefix_str expected ~prefix:left_prefix)
            else ("", "")
        in
        let parse_consecutive_binary_operators expected =
          let operators_prefix = "expression between operators in " in strip_prefix_str expected ~prefix:operators_prefix
        in
        let parse_unclosed_delimiter expected fix =
          ...
          | "end" ->
              if String.starts_with ~prefix:"add end to close the begin block" fix then
                ...
                if String.starts_with ~prefix:"add end to close the struct block" fix then
                  "struct"
                else
                  if String.starts_with ~prefix:"add end to close the signature" fix then
                    "sig"
                  else ""
          | _ -> ""
        in
        let parse_bracketed_type_name fix =
          let prefix = "put generics on the left of the type name with parenthesis, like this: " in
        ...
        let kind =
          ...
          | Some E0014_UnclosedDelimiter -> UnclosedDelimiter { opener = parse_unclosed_delimiter expected fix; found }
          ...
          | Some E0020_MissingBinaryOperand ->
              let side, operator = parse_missing_binary_operand expected in MissingBinaryOperand { operator; side; found }
          | Some E0021_ConsecutiveBinaryOperators -> ConsecutiveBinaryOperators { operators = parse_consecutive_binary_operators expected; found }
          | Some E0022_InvalidTypeParameter -> InvalidTypeParameter { text = found.text; found }
          | Some E0023_UppercaseTypeVariable -> UppercaseTypeVariable { text = found.text; found }
          | Some E0024_UppercaseTypeName -> UppercaseTypeName { text = found.text; found }
          | Some E0025_BracketedTypeParameters -> BracketedTypeParameters { type_name = parse_bracketed_type_name fix; found }
          ...
          | Some E0033_ConstructorPatternNeedsParens -> ConstructorPatternNeedsParens { constructor = ""; found }
          ...
          | Some E0039_RecordFieldMissingColon -> RecordFieldMissingColon { field_name = parse_quoted_field expected ~prefix:"colon after field name '"; found }
          | Some E0040_RecordFieldMissingType -> RecordFieldMissingType { field_name = parse_quoted_field expected ~prefix:"type definition for field '"; found }
          | Some E0041_PolyTypeMissingVarName -> PolyTypeMissingVarName { found }
          | Some E0042_PolyTypeMissingDot -> PolyTypeMissingDot { found }
          | Some E0043_UnexpectedClosingDelimiter -> UnexpectedClosingDelimiter { delimiter = found.text; found }
