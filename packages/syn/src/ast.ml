...
let is_expr_kind = function
  | Syntax_kind.LET_EXPR | Syntax_kind.LOCAL_OPEN_EXPR | Syntax_kind.LET_MODULE_EXPR | Syntax_kind.LET_EXCEPTION_EXPR | Syntax_kind.BINDING_OPERATOR_EXPR | Syntax_kind.FIRST_CLASS_MODULE_EXPR | Syntax_kind.EXTENSION_EXPR | Syntax_kind.UNREACHABLE_EXPR | Syntax_kind.OBJECT_EXPR | Syntax_kind.NEW_EXPR | Syntax_kind.IF_EXPR | Syntax_kind.MATCH_EXPR | Syntax_kind.FUN_EXPR | Syntax_kind.FUNCTION_EXPR | Syntax_kind.TRY_EXPR | Syntax_kind.WHILE_EXPR | Syntax_kind.FOR_EXPR | Syntax_kind.ASSERT_EXPR | Syntax_kind.LAZY_EXPR | Syntax_kind.ATTRIBUTE_EXPR | Syntax_kind.SEQUENCE_EXPR | Syntax_kind.APPLY_EXPR | Syntax_kind.INFIX_EXPR | Syntax_kind.PREFIX_EXPR | Syntax_kind.ASSIGN_EXPR | Syntax_kind.FIELD_ACCESS_EXPR | Syntax_kind.METHOD_CALL_EXPR | Syntax_kind.POLY_VARIANT_EXPR | Syntax_kind.LABELED_ARG | Syntax_kind.OPTIONAL_ARG | Syntax_kind.ARRAY_INDEX_EXPR | Syntax_kind.STRING_INDEX_EXPR | Syntax_kind.TYPED_EXPR | Syntax_kind.PATH_EXPR | Syntax_kind.LITERAL_EXPR | Syntax_kind.PAREN_EXPR | Syntax_kind.TUPLE_EXPR | Syntax_kind.LIST_EXPR | Syntax_kind.ARRAY_EXPR | Syntax_kind.RECORD_EXPR | Syntax_kind.RECORD_UPDATE_EXPR -> true
  | _ -> false

let is_pattern_kind = function
  | Syntax_kind.WILDCARD_PATTERN | Syntax_kind.PATH_PATTERN | Syntax_kind.APPLY_PATTERN | Syntax_kind.LITERAL_PATTERN | Syntax_kind.PAREN_PATTERN | Syntax_kind.TUPLE_PATTERN | Syntax_kind.LIST_PATTERN | Syntax_kind.ARRAY_PATTERN | Syntax_kind.RECORD_PATTERN | Syntax_kind.POLY_VARIANT_PATTERN | Syntax_kind.EXTENSION_PATTERN | Syntax_kind.ATTRIBUTE_PATTERN | Syntax_kind.LOCAL_OPEN_PATTERN | Syntax_kind.LOCALLY_ABSTRACT_TYPE_PATTERN | Syntax_kind.FIRST_CLASS_MODULE_PATTERN | Syntax_kind.INTERVAL_PATTERN | Syntax_kind.CONSTRAINT_PATTERN | Syntax_kind.ALIAS_PATTERN | Syntax_kind.OR_PATTERN | Syntax_kind.CONS_PATTERN | Syntax_kind.LAZY_PATTERN | Syntax_kind.EXCEPTION_PATTERN | Syntax_kind.LABELED_PARAM | Syntax_kind.OPTIONAL_PARAM | Syntax_kind.OPTIONAL_PARAM_DEFAULT -> true
  | _ -> false
...
let is_path_kind = function
  | Syntax_kind.PATH_EXPR | Syntax_kind.PATH_PATTERN | Syntax_kind.PATH_TYPE | Syntax_kind.PATH_MODULE_EXPR | Syntax_kind.PATH_MODULE_TYPE -> true
  | _ -> false

let is_type_expr_kind = function
  | Syntax_kind.TYPE_EXPR | Syntax_kind.PATH_TYPE | Syntax_kind.VAR_TYPE | Syntax_kind.WILDCARD_TYPE | Syntax_kind.ARROW_TYPE | Syntax_kind.POLY_TYPE | Syntax_kind.LABELED_TYPE | Syntax_kind.TUPLE_TYPE | Syntax_kind.APPLY_TYPE | Syntax_kind.PAREN_TYPE | Syntax_kind.OPAQUE_TYPE -> true
  | _ -> false
...
let is_module_expr_kind = function
  | Syntax_kind.MODULE_EXPR | Syntax_kind.PATH_MODULE_EXPR | Syntax_kind.STRUCT_MODULE_EXPR | Syntax_kind.FUNCTOR_MODULE_EXPR | Syntax_kind.APPLY_MODULE_EXPR | Syntax_kind.CONSTRAINT_MODULE_EXPR | Syntax_kind.PAREN_MODULE_EXPR | Syntax_kind.OPAQUE_MODULE_EXPR -> true
  | _ -> false

let is_module_type_kind = function
  | Syntax_kind.MODULE_TYPE_EXPR | Syntax_kind.PATH_MODULE_TYPE | Syntax_kind.SIGNATURE_MODULE_TYPE | Syntax_kind.TYPEOF_MODULE_TYPE | Syntax_kind.FUNCTOR_MODULE_TYPE | Syntax_kind.WITH_MODULE_TYPE | Syntax_kind.PAREN_MODULE_TYPE | Syntax_kind.OPAQUE_MODULE_TYPE -> true
  | _ -> false
...
let first_child_node_matching = fun (node: node) ~matches ->
  ...
  Syntax_tree.for_each_child node.tree (syntax_node node) ~fn:(
    ...
    | Syntax_tree.Token _ | Syntax_tree.Missing _ -> ()
  );
  !found
...
let for_each_child_node_matching = fun (node: node) ~matches ~fn -> Syntax_tree.for_each_child node.tree (syntax_node node) ~fn:(
  function
  | Syntax_tree.Node id ->
      let child = wrap_node node.tree id in
      if node_matches child matches then
...
let first_child_token_matching = fun (node: node) ~matches ->
  ...
  Syntax_tree.for_each_child node.tree (syntax_node node) ~fn:(
    ...
    | Syntax_tree.Node _ | Syntax_tree.Missing _ -> ()
  );
  !found
...
let child_token_at = fun (node: node) index ->
  ...
  | Some (Syntax_tree.Node _) | Some (Syntax_tree.Missing _) | None -> None
...
let first_ident_token = fun (node: node) -> first_child_token_matching node ~matches:(
  fun kind -> Syntax_kind.(kind = IDENT)
)

let first_ident_or_underscore_token = fun (node: node) -> first_child_token_matching node ~matches:(
  fun kind -> Syntax_kind.(kind = IDENT || kind = UNDERSCORE)
)
...
let rec for_each_token_in_node = fun (node: node) ~fn -> Syntax_tree.for_each_child node.tree (syntax_node node) ~fn:(
  function
...
let for_each_token_after_child_token = fun (node: node) ~matches ~fn ->
  let seen_boundary = ref false in Syntax_tree.for_each_child node.tree (syntax_node node) ~fn:(
    function
    | Syntax_tree.Token id ->
        let token = wrap_token node.tree id in
...
module Token = struct
  ...
  let contains_char = fun (token: token) needle -> Syntax_tree.token_contains_char token.tree (syntax_token token) needle

  let text_is = fun (token: token) expected -> Syntax_tree.token_text_is token.tree (syntax_token token) expected
  ...
  let text_equal = fun (left: token) (right: token) ->
    ...
    if not (Int.equal left_width (width right)) then
      ...
      if Int.equal left_width 0 then
        true
      else
        let left_raw = Vector.get_unchecked left.tree.Syntax_tree.raw_tokens ~at:(syntax_token left).Syntax_tree.body_raw in
        let right_raw = Vector.get_unchecked right.tree.Syntax_tree.raw_tokens ~at:(syntax_token right).Syntax_tree.body_raw in
        ...
        let rec loop index =
          if Int.(index >= left_width) then
            ...
            if Char.equal (IO.IoVec.IoSlice.get_unchecked left_slice ~at:index) (IO.IoVec.IoSlice.get_unchecked right_slice ~at:index) then
              loop Int.(index + 1)
            else false
        in
        loop 0
  ...
  let has_newline = fun (token: token) -> Syntax_tree.token_has_newline token.tree (syntax_token token)

  let text = fun (token: token) -> Syntax_tree.token_text token.tree (syntax_token token)

  let leading_text = fun (token: token) ->
    let syntax_token = syntax_token token in Syntax_tree.raw_range_text token.tree ~raw_lo:syntax_token.Syntax_tree.raw_lo ~raw_hi:syntax_token.Syntax_tree.body_raw
    ...
    let rec loop raw_index =
      if Int.(raw_index >= syntax_token.Syntax_tree.body_raw) then
        ...
        if matches raw.Raw_token.kind then
          true
        else loop Int.(raw_index + 1)
    in
    loop syntax_token.Syntax_tree.raw_lo

  let has_leading_whitespace = fun token -> has_leading_raw token ~matches:(
    fun kind -> Syntax_kind.(kind = WHITESPACE)
  )

  let has_leading_comment = fun token -> has_leading_raw token ~matches:(
    fun kind -> Syntax_kind.(kind = COMMENT || kind = DOCSTRING)
  )

  let has_leading_docstring = fun token -> has_leading_raw token ~matches:(
    fun kind -> Syntax_kind.(kind = DOCSTRING)
  )

  let full_text = fun (token: token) ->
    let raw_lo, raw_hi =
      let token = syntax_token token in (token.Syntax_tree.raw_lo, token.Syntax_tree.raw_hi)
    in
...
module Node = struct
  ...
  let raw_range = fun (node: node) ->
    let node = syntax_node node in (node.Syntax_tree.raw_lo, node.Syntax_tree.raw_hi)
  ...
  let for_each_child_node = fun (node: node) ~fn -> for_each_child node ~fn:(
    function
    | Syntax_tree.Node id -> fn (wrap_node node.tree id)
    | Syntax_tree.Token _ | Syntax_tree.Missing _ -> ()
  )

  let for_each_child_token = fun (node: node) ~fn -> for_each_child node ~fn:(
    function
    | Syntax_tree.Token id -> fn (wrap_token node.tree id)
    | Syntax_tree.Node _ | Syntax_tree.Missing _ -> ()
  )
  ...
  let first_child_node = fun (node: node) ~kind:expected_kind -> first_child_node_matching node ~matches:(
    fun kind -> Syntax_kind.(kind = expected_kind)
  )

  let first_child_token = fun (node: node) ~kind:expected_kind -> first_child_token_matching node ~matches:(
    fun kind -> Syntax_kind.(kind = expected_kind)
  )

  let first_token = fun (node: node) -> first_child_token_matching node ~matches:(
    fun _ -> true
  )

  let first_descendant_token = fun (node: node) -> first_descendant_token_matching node ~matches:(
    fun _ -> true
  )
end
...
module TypeExpr = struct
  ...
  let rec view = fun (type_expr: type_expr) ->
    ...
    | Syntax_kind.TYPE_EXPR -> (
      match first_child_node_matching type_expr ~matches:is_type_expr_kind, nth_child_node_matching type_expr 1 ~matches:is_type_expr_kind with
      | Some child, None -> (
    ...
    | Syntax_kind.ARROW_TYPE -> Arrow { left = nth_child_node_matching type_expr 0 ~matches:is_type_expr_kind; right = nth_child_node_matching type_expr 1 ~matches:is_type_expr_kind }
    | Syntax_kind.POLY_TYPE -> Poly { body = first_child_node_matching type_expr ~matches:is_type_expr_kind }
    | Syntax_kind.LABELED_TYPE -> Labeled { optional_token = first_child_token_matching type_expr ~matches:(
      fun kind -> Syntax_kind.(kind = QUESTION)
    ); label = first_ident_token type_expr; annotation = first_child_node_matching type_expr ~matches:is_type_expr_kind }
    | Syntax_kind.TUPLE_TYPE -> Tuple { left = nth_child_node_matching type_expr 0 ~matches:is_type_expr_kind; right = nth_child_node_matching type_expr 1 ~matches:is_type_expr_kind; separator = tuple_separator type_expr }
    | Syntax_kind.APPLY_TYPE -> Apply { argument = nth_child_node_matching type_expr 0 ~matches:is_type_expr_kind; constructor = nth_child_node_matching type_expr 1 ~matches:is_type_expr_kind }
    | Syntax_kind.PAREN_TYPE -> Parenthesized { inner = first_child_node_matching type_expr ~matches:is_type_expr_kind }
    | Syntax_kind.OPAQUE_TYPE -> Opaque type_expr
    | Syntax_kind.ERROR -> Error type_expr
    | _ -> Unknown type_expr

  let for_each_child_type = fun (type_expr: type_expr) ~fn -> for_each_child_node_matching type_expr ~matches:is_type_expr_kind ~fn
    ...
    if not (child_token_kind_is_in node close_index Syntax_kind.RBRACKET) then
      ...
      let rec loop index depth =
        if Int.(index < 0) then
          ...
          if child_token_kind_is_in node index Syntax_kind.RBRACKET then
            loop Int.(index - 1) Int.(depth + 1)
          else
            if child_token_kind_is_in node index Syntax_kind.LBRACKET then
              if Int.equal depth 1 then
                let next = Int.add index 1 in
                if child_token_kind_is_in node next Syntax_kind.AT || child_token_kind_is_in node next Syntax_kind.ATAT then
                  Some index
                else None
              else loop Int.(index - 1) Int.(depth - 1)
            else loop Int.(index - 1) depth
      in
    ...
    | Some first_suffix_index ->
        let rec loop index =
          if Int.(index >= first_suffix_index) then
            ...
            | Some (Syntax_tree.Token _) | Some (Syntax_tree.Missing _) | None -> loop (Int.add index 1)
        in
        loop 0
          ...
          if Int.(index < Node.child_count type_expr) then
            (
              (
                ...
                | Some (Syntax_tree.Node id) ->
                    let node = wrap_node type_expr.tree id in Node.for_each_token node ~fn
                | Some (Syntax_tree.Missing _) | None -> ()
              );
  ...
  let for_each_poly_type_name = fun (type_expr: type_expr) ~fn ->
    ...
    let before_dot = ref true in Syntax_tree.for_each_child type_expr.tree (syntax_node type_expr) ~fn:(
      function
      | Syntax_tree.Token id ->
          ...
          if token_kind_is token Syntax_kind.DOT then
            ...
            if !before_dot && token_kind_is token Syntax_kind.IDENT then
              fn token
      | Syntax_tree.Node _ | Syntax_tree.Missing _ -> ()
    )

  let poly_type_keyword_token = fun (type_expr: type_expr) ->
    let type_expr = unwrap_poly_node type_expr in first_child_token_matching type_expr ~matches:(
      fun kind -> Syntax_kind.(kind = TYPE_KW)
    )
end
...
module RecordType = struct
  ...
  let for_each_field = fun record_type ~fn -> for_each_child_node_matching record_type ~matches:is_record_field_kind ~fn
end
...
module VariantType = struct
  ...
  let for_each_constructor = fun variant_type ~fn -> for_each_child_node_matching variant_type ~matches:is_variant_constructor_kind ~fn
end
...
module Path = struct
  ...
  let for_each_ident = fun (path: path) ~fn -> Node.for_each_child_token path ~fn:(
    fun token ->
      if token_kind_is token Syntax_kind.IDENT then
        fn token
  )
...
module Expr: sig
  ...
  let first_operator_token = fun (node: node) -> first_child_token_matching node ~matches:(
    fun kind -> not (Syntax_kind.(kind = IDENT) || Syntax_kind.(kind = INT) || Syntax_kind.(kind = FLOAT) || Syntax_kind.(kind = STRING) || Syntax_kind.(kind = CHAR) || Syntax_kind.(kind = TRUE_KW) || Syntax_kind.(kind = FALSE_KW))
  )

  let first_direct_token = fun (node: node) -> first_child_token_matching node ~matches:(
    fun _kind -> true
  )
  ...
  let view = fun (expr: expr) ->
    ...
    | Syntax_kind.LET_EXPR -> Let { first_binding = first_let_binding_child expr; body = nth_expr_child expr 0 }
    ...
    | Syntax_kind.BINDING_OPERATOR_EXPR -> BindingOperator { first_binding = first_let_binding_child expr; body = nth_expr_child expr 0 }
    ...
    | Syntax_kind.IF_EXPR -> If { condition = nth_expr_child expr 0; then_branch = nth_expr_child expr 1; else_branch = nth_expr_child expr 2 }
    | Syntax_kind.MATCH_EXPR -> Match { scrutinee = nth_expr_child expr 0; first_case = first_match_case_child expr }
    ...
    | Syntax_kind.TRY_EXPR -> Try { body = nth_expr_child expr 0; first_case = first_match_case_child expr }
    | Syntax_kind.WHILE_EXPR -> While { condition = nth_expr_child expr 0; body = nth_expr_child expr 1 }
    ...
    | Syntax_kind.SEQUENCE_EXPR -> Sequence { left = nth_expr_child expr 0; right = nth_expr_child expr 1 }
    | Syntax_kind.APPLY_EXPR -> Apply { callee = nth_expr_child expr 0; argument = nth_expr_child expr 1 }
    | Syntax_kind.INFIX_EXPR -> Infix { left = nth_expr_child expr 0; operator = first_direct_token expr; right = nth_expr_child expr 1 }
    | Syntax_kind.PREFIX_EXPR -> Prefix { operator = first_operator_token expr; operand = first_expr_child expr }
    | Syntax_kind.ASSIGN_EXPR -> Assign { target = nth_expr_child expr 0; operator = first_direct_token expr; value = nth_expr_child expr 1 }
    | Syntax_kind.FIELD_ACCESS_EXPR -> FieldAccess { target = nth_expr_child expr 0; field = last_ident_token expr }
    | Syntax_kind.METHOD_CALL_EXPR -> MethodCall { target = nth_expr_child expr 0; method_ = last_ident_token expr }
    ...
    | Syntax_kind.ARRAY_INDEX_EXPR -> ArrayIndex { target = nth_expr_child expr 0; index = nth_expr_child expr 1 }
    | Syntax_kind.STRING_INDEX_EXPR -> StringIndex { target = nth_expr_child expr 0; index = nth_expr_child expr 1 }
    | Syntax_kind.TYPED_EXPR -> Typed { expr = first_expr_child expr; annotation = first_type_expr_child expr }
    | Syntax_kind.LABELED_ARG -> LabeledArg { label = first_ident_token expr; value = first_expr_child expr }
    | Syntax_kind.OPTIONAL_ARG -> OptionalArg { label = first_ident_token expr; value = first_expr_child expr }
    | Syntax_kind.ERROR -> Error expr
    | _ -> Unknown expr

  let for_each_child_expr = fun (expr: expr) ~fn -> for_each_child_node_matching expr ~matches:is_expr_kind ~fn

  let for_each_match_case = fun (expr: expr) ~fn -> for_each_child_node_matching expr ~matches:is_match_case_kind ~fn
end
...
module RecordExpr: sig
  ...
  let cast = fun (expr: expr) ->
    if node_kind_is expr Syntax_kind.RECORD_EXPR || node_kind_is expr Syntax_kind.RECORD_UPDATE_EXPR then
      Some expr
    else None
  ...
  let field_of_node = fun (field: record_expr_field) ->
    ...
    | Some expr, value when node_kind_is expr Syntax_kind.PATH_EXPR -> { path = Path.cast expr; value; node = field }
  ...
  let for_each_field = fun (record: t) ~fn -> Syntax_tree.for_each_child record.tree (syntax_node record) ~fn:(
    function
    | Syntax_tree.Node id ->
...
module LocalOpenExpr: sig
  ...
  let opening_token = fun expr ->
    match first_child_token_matching expr ~matches:(
      fun kind -> Syntax_kind.(kind = LPAREN || kind = LBRACKET || kind = LBRACKET_BAR || kind = LBRACE)
    ) with
    | Some token -> Some token
    | None -> (
      match nth_expr_child expr 1 with
      | Some body -> first_child_token_matching body ~matches:(
        fun kind -> Syntax_kind.(kind = LBRACKET || kind = LBRACKET_BAR || kind = LBRACE)
      )
      | None -> None
    )
  ...
  let closing_token = fun expr ->
    match first_child_token_matching expr ~matches:(
      fun kind -> Syntax_kind.(kind = RPAREN || kind = RBRACKET || kind = BAR_RBRACKET || kind = RBRACE)
    ) with
    | Some token -> Some token
    | None -> (
      match nth_expr_child expr 1 with
      | Some body -> first_child_token_matching body ~matches:(
        fun kind -> Syntax_kind.(kind = RBRACKET || kind = BAR_RBRACKET || kind = RBRACE)
      )
      | None -> None
    )
...
module LetModuleExpr: sig
  ...
  let module_index = fun expr -> token_index expr ~from:0 ~matches:(
    fun kind -> Syntax_kind.(kind = MODULE_KW)
  )

  let equals_index = fun expr -> token_index expr ~from:0 ~matches:(
    fun kind -> Syntax_kind.(kind = EQ)
  )

  let in_index = fun expr -> token_index expr ~from:0 ~matches:(
    fun kind -> Syntax_kind.(kind = IN_KW)
  )
  ...
  let module_body_bounds = fun expr ->
    ...
    | Some equals_index, Some in_index when equals_index < in_index -> Some (equals_index + 1, in_index)
    | _ -> None
      ...
      | Some (start, stop) ->
          let rec loop index =
            if index >= stop then
              ...
              | Some (Syntax_tree.Token _) | Some (Syntax_tree.Missing _) | None -> loop (index + 1)
          in
          loop start
      | None -> None
    in
  ...
  let module_body = fun expr ->
    let has_child_node_kind node expected =
      let found = ref false in
      Node.for_each_child_node node ~fn:(
        fun child ->
  ...
  let for_each_module_body_path_ident = fun expr ~fn ->
    match module_body_node expr with
    | Some node when node_kind_is node Syntax_kind.PATH_MODULE_EXPR -> for_each_token_in_node node ~fn:(
      fun token ->
        if token_kind_is token Syntax_kind.IDENT then
          fn token
    )
...
module LetExceptionExpr: sig
  ...
  let exception_index = fun expr -> token_index expr ~from:0 ~matches:(
    fun kind -> Syntax_kind.(kind = EXCEPTION_KW)
  )

  let of_index = fun expr -> token_index expr ~from:0 ~matches:(
    fun kind -> Syntax_kind.(kind = OF_KW)
  )
...
module FirstClassModuleExpr: sig
  ...
  let range_is_path = fun expr start stop ->
    let rec loop index saw_ident expect_ident =
      if index >= stop then
        ...
        match child_token_at expr index with
        | Some token when token_kind_is token Syntax_kind.IDENT && expect_ident -> loop (index + 1) true false
        | Some token when token_kind_is token Syntax_kind.DOT && saw_ident && not expect_ident -> loop (index + 1) saw_ident true
        | _ -> false
    in
    loop start false true

  let module_path_bounds = fun expr ->
    match token_index expr ~from:0 ~matches:(
      fun kind -> Syntax_kind.(kind = MODULE_KW)
    ) with
    | None -> None
    | Some module_index ->
        let start = module_index + 1 in token_index expr ~from:start ~matches:(
          fun kind -> Syntax_kind.(kind = COLON || kind = RPAREN)
        ) |> Option.map ~fn:(
          fun stop -> (start, stop)
        )
  ...
  let ascription_bounds = fun expr ->
    match token_index expr ~from:0 ~matches:(
      fun kind -> Syntax_kind.(kind = COLON)
    ) with
    | None -> None
    | Some colon_index ->
        let start = colon_index + 1 in token_index expr ~from:start ~matches:(
          fun kind -> Syntax_kind.(kind = RPAREN)
        ) |> Option.map ~fn:(
          fun stop -> (start, stop)
        )
  ...
  let for_each_module_path_ident = fun expr ~fn ->
    match module_path_bounds expr with
    | Some (start, stop) when range_is_path expr start stop -> for_each_ident_in_range expr start stop ~fn
    | _ -> ()
...
module BindingOperatorExpr: sig
  ...
  let binding_operator_keyword = fun token -> token_kind_is token Syntax_kind.LET_KW || token_kind_is token Syntax_kind.AND_KW

  let binding_operator_suffix = fun token -> token_kind_is token Syntax_kind.STAR || token_kind_is token Syntax_kind.PLUS
      ...
      if Int.(index >= child_count) then
        ...
        | Some (Syntax_tree.Token id) ->
            ...
            if binding_operator_keyword token then
              ...
              if binding_operator_suffix token then
                loop Int.(index + 1) keyword (Some token)
              else loop Int.(index + 1) keyword operator
        | Some (Syntax_tree.Node id) ->
        ...
        | Some (Syntax_tree.Missing _) | None -> loop Int.(index + 1) keyword operator
    in
    loop 0 None None
end
...
module Pattern: sig
  ...
  let literal_token = fun pattern -> first_child_token_matching pattern ~matches:(
    fun kind -> Syntax_kind.(kind = INT || kind = FLOAT || kind = STRING || kind = CHAR || kind = TRUE_KW || kind = FALSE_KW)
  )

  let literal_sign_token = fun pattern -> first_child_token_matching pattern ~matches:(
    fun kind -> Syntax_kind.(kind = PLUS || kind = MINUS || kind = PLUSDOT || kind = MINUSDOT)
  )
  ...
  let view = fun (pattern: pattern) ->
    ...
    | Syntax_kind.APPLY_PATTERN -> Apply { callee = nth_pattern_child pattern 0; argument = nth_pattern_child pattern 1 }
    ...
    | Syntax_kind.INTERVAL_PATTERN -> Interval { left = nth_pattern_child pattern 0; right = nth_pattern_child pattern 1 }
    | Syntax_kind.CONSTRAINT_PATTERN -> Constraint { pattern = first_pattern_child pattern; annotation = first_type_expr_child pattern }
    | Syntax_kind.ALIAS_PATTERN -> Alias { pattern = nth_pattern_child pattern 0; alias = nth_pattern_child pattern 1 }
    | Syntax_kind.OR_PATTERN -> Or { left = nth_pattern_child pattern 0; right = nth_pattern_child pattern 1 }
    | Syntax_kind.CONS_PATTERN -> Cons { head = nth_pattern_child pattern 0; tail = nth_pattern_child pattern 1 }
  ...
  let for_each_child_pattern = fun (pattern: pattern) ~fn -> for_each_child_node_matching pattern ~matches:is_pattern_kind ~fn
end
...
module LocallyAbstractTypePattern: sig
  ...
  let for_each_type_name = fun pattern ~fn -> Node.for_each_child_token pattern ~fn:(
    fun token ->
      if token_kind_is token Syntax_kind.IDENT then
        fn token
  )
...
module FirstClassModulePattern: sig
  ...
  let module_index = fun pattern -> token_index pattern ~from:0 ~matches:(
    fun kind -> Syntax_kind.(kind = MODULE_KW)
  )
  ...
  let binder = fun pattern ->
    ...
    | Some module_index -> (
      ...
      | Some token when token_kind_is token Syntax_kind.IDENT || token_kind_is token Syntax_kind.UNDERSCORE -> Some token
      | _ -> None
    )
    | None -> None
  ...
  let ascription_bounds = fun pattern ->
    match token_index pattern ~from:0 ~matches:(
      fun kind -> Syntax_kind.(kind = COLON)
    ) with
    | None -> None
    | Some colon_index ->
        let start = colon_index + 1 in token_index pattern ~from:start ~matches:(
          fun kind -> Syntax_kind.(kind = RPAREN)
        ) |> Option.map ~fn:(
          fun stop -> (start, stop)
        )
  ...
  let for_each_ascription_path_ident = fun pattern ~fn ->
    ...
    | Some (start, stop) when range_is_path pattern start stop -> for_each_ident_in_range pattern start stop ~fn
    | _ -> ()
end
...
module RecordPattern: sig
  ...
  let open_wildcard = fun (record: t) ->
    let rec loop index previous_token_kind =
      if index >= Node.child_count record then
        ...
        | Some (Syntax_tree.Token id) ->
            let token = wrap_token record.tree id in loop (index + 1) (Some (Token.kind token))
        | Some (Syntax_tree.Node id) ->
            let child = wrap_node record.tree id in
            ...
            else loop (index + 1) previous_token_kind
        | Some (Syntax_tree.Missing _) | None -> loop (index + 1) previous_token_kind
    in
    loop 0 None
  ...
  let child_node_at = fun (record: t) index ->
    ...
    | Some (Syntax_tree.Token _) | Some (Syntax_tree.Missing _) | None -> None
...
module LocalOpenPattern: sig
  ...
  let opening_token = fun pattern ->
    let matches kind = Syntax_kind.(kind = LPAREN || kind = LBRACE || kind = LBRACKET || kind = LBRACKET_BAR) in
  ...
  let closing_token = fun pattern ->
    let matches kind = Syntax_kind.(kind = RPAREN || kind = RBRACE || kind = RBRACKET || kind = BAR_RBRACKET) in
    match first_child_token_matching pattern ~matches with
    | Some token -> Some token
    | None -> (
      match delimiter_child pattern with
...
module Parameter: sig
  ...
  let parameter_label_token = fun parameter ->
    ...
    | Some token -> Some token
    | None -> first_descendant_token_matching parameter ~matches:(
      fun kind -> Syntax_kind.(kind = IDENT)
    )

  let view = fun (parameter: parameter) ->
    match Node.kind parameter with
    | Syntax_kind.LABELED_PARAM -> Labeled { label = parameter_label_token parameter; pattern = first_pattern_child parameter }
    | Syntax_kind.OPTIONAL_PARAM -> Optional { label = parameter_label_token parameter; pattern = first_pattern_child parameter }
    | Syntax_kind.OPTIONAL_PARAM_DEFAULT -> OptionalDefault { label = parameter_label_token parameter; pattern = first_pattern_child parameter; default = first_expr_child parameter }
    | _ -> Unknown parameter

  let has_explicit_pattern_parens = fun parameter -> Option.is_some (Node.first_child_token parameter ~kind:Syntax_kind.LPAREN)
end
...
module MatchCase: sig
  ...
  let view = fun (match_case: match_case) ->
    let guard, body =
      ...
      else (None, nth_expr_child match_case 0)
    in
...
module LetBinding: sig
  ...
  let for_each_parameter = fun (binding: let_binding) ~fn ->
    let seen_first = ref false in for_each_child_node_matching binding ~matches:is_pattern_kind ~fn:(
      fun pattern ->
        if !seen_first then
...
module LetDeclaration = struct
  ...
  let for_each_binding = fun (decl: let_declaration) ~fn -> for_each_child_node_matching decl ~matches:is_let_binding_kind ~fn
end
...
module TypeDeclaration = struct
  ...
  let first_member_node = fun decl -> first_child_node_matching decl ~matches:(
    fun kind -> Syntax_kind.(kind = TYPE_DECL_MEMBER)
  )
  ...
  let rec collect_type_parameter_modifiers_in = fun node index variance injective ->
    match child_token_at_node node index with
    | Some token when token_kind_is token Syntax_kind.PLUS || token_kind_is token Syntax_kind.MINUS -> collect_type_parameter_modifiers_in node (index + 1) (Some token) injective
    | Some token when token_kind_is token Syntax_kind.BANG -> collect_type_parameter_modifiers_in node (index + 1) variance (Some token)
  ...
  let rec skip_parenthesized_type_parameters_in = fun node index ->
    ...
    | Some Syntax_kind.EOF | None -> index
  ...
  let rec find_node_in = fun node index ~matches ->
    if index >= Node.child_count node then
      ...
      | Some (Syntax_tree.Node id) ->
          ...
          if node_matches child matches then
            Some child
          else find_node_in node (index + 1) ~matches
      | Some (Syntax_tree.Token _) | Some (Syntax_tree.Missing _) | None -> find_node_in node (index + 1) ~matches

  let name = fun decl ->
    let node = member_or_decl decl in
    let rec loop index =
      match child_token_at_node node index with
      | Some token when token_kind_is token Syntax_kind.TYPE_KW || token_kind_is token Syntax_kind.NONREC_KW -> loop (index + 1)
      | Some token when token_kind_is token Syntax_kind.LPAREN -> loop (skip_parenthesized_type_parameters_in node (index + 1))
      | Some token when token_kind_is token Syntax_kind.PLUS || token_kind_is token Syntax_kind.MINUS || token_kind_is token Syntax_kind.BANG || token_kind_is token Syntax_kind.QUOTE || token_kind_is token Syntax_kind.UNDERSCORE ->
  ...
  let for_each_parameter = fun decl ~fn ->
    ...
    let rec parse_head index =
      match child_token_at_node node index with
      | Some token when token_kind_is token Syntax_kind.TYPE_KW || token_kind_is token Syntax_kind.NONREC_KW -> parse_head (index + 1)
      | Some token when token_kind_is token Syntax_kind.LPAREN -> parse_head (parse_parenthesized (index + 1))
      | Some token when token_kind_is token Syntax_kind.PLUS || token_kind_is token Syntax_kind.MINUS || token_kind_is token Syntax_kind.BANG || token_kind_is token Syntax_kind.QUOTE || token_kind_is token Syntax_kind.UNDERSCORE ->
          let next = emit_type_parameter_in node index ~fn in
          if next > index then
            parse_head next
  ...
  module Member = struct
    ...
    let child_at = fun member index ->
      if index < 0 || index >= child_count member then
        None
      else Node.child_at member.node index
    ...
    let for_each_child_token = fun member ~fn -> for_each_child member ~fn:(
      function
      | Syntax_tree.Token id -> fn (wrap_token member.node.tree id)
      | Syntax_tree.Node _ | Syntax_tree.Missing _ -> ()
    )

    let for_each_child_node = fun member ~fn -> for_each_child member ~fn:(
      function
    ...
    let name = fun member ->
      let rec loop index =
        match child_token_at member index with
        | Some token when token_kind_is token Syntax_kind.TYPE_KW || token_kind_is token Syntax_kind.AND_KW || token_kind_is token Syntax_kind.NONREC_KW -> loop (index + 1)
        | Some token when token_kind_is token Syntax_kind.LPAREN -> loop (skip_parenthesized_type_parameters_in member.node (index + 1))
        | Some token when token_kind_is token Syntax_kind.PLUS || token_kind_is token Syntax_kind.MINUS || token_kind_is token Syntax_kind.BANG || token_kind_is token Syntax_kind.QUOTE || token_kind_is token Syntax_kind.UNDERSCORE ->
    ...
    let for_each_parameter = fun member ~fn ->
      ...
      let rec parse_head index =
        match child_token_at member index with
        | Some token when token_kind_is token Syntax_kind.TYPE_KW || token_kind_is token Syntax_kind.AND_KW || token_kind_is token Syntax_kind.NONREC_KW -> parse_head (index + 1)
        | Some token when token_kind_is token Syntax_kind.LPAREN -> parse_head (parse_parenthesized (index + 1))
        | Some token when token_kind_is token Syntax_kind.PLUS || token_kind_is token Syntax_kind.MINUS || token_kind_is token Syntax_kind.BANG || token_kind_is token Syntax_kind.QUOTE || token_kind_is token Syntax_kind.UNDERSCORE ->
            let next = emit_type_parameter_in member.node index ~fn in
            if next > index then
  ...
  let for_each_member = fun decl ~fn ->
    ...
    Node.for_each_child decl ~fn:(
      ...
      | Syntax_tree.Node id ->
          ...
          index := !index + 1
      | Syntax_tree.Token _ | Syntax_tree.Missing _ -> index := !index + 1
    );
  ...
  let fold_members = fun decl init fn ->
    let acc = ref init in
    for_each_member decl ~fn:(
      fun member -> acc := fn !acc member
    );
    !acc
end
...
module TypeExtensionDeclaration = struct
  ...
  let head_node = fun decl -> first_child_node_matching decl ~matches:(
    fun kind -> Syntax_kind.(kind = TYPE_EXTENSION_DECL_HEAD)
  )

  let body_node = fun decl -> first_child_node_matching decl ~matches:(
    fun kind -> Syntax_kind.(kind = TYPE_EXTENSION_DECL_BODY)
  )
  ...
  let rec collect_type_parameter_modifiers = fun decl index variance injective ->
    match child_token_at decl index with
    | Some token when token_kind_is token Syntax_kind.PLUS || token_kind_is token Syntax_kind.MINUS -> collect_type_parameter_modifiers decl (index + 1) (Some token) injective
    | Some token when token_kind_is token Syntax_kind.BANG -> collect_type_parameter_modifiers decl (index + 1) variance (Some token)
    | _ -> index, variance, injective
  ...
  let for_each_name_ident = fun decl ~fn ->
    ...
    | Some head ->
        ...
        let rec parse_head index =
          match child_token_at head index with
          | Some token when token_kind_is token Syntax_kind.TYPE_KW -> parse_head (index + 1)
          | Some token when token_kind_is token Syntax_kind.LPAREN -> parse_head (skip_parenthesized_type_parameters head (index + 1))
          | Some token when token_kind_is token Syntax_kind.PLUS || token_kind_is token Syntax_kind.MINUS || token_kind_is token Syntax_kind.BANG || token_kind_is token Syntax_kind.QUOTE || token_kind_is token Syntax_kind.UNDERSCORE ->
  ...
  let name = fun decl ->
    let found = ref None in
    for_each_name_ident decl ~fn:(
      fun token -> found := Some token
    );
    !found
  ...
  if not (child_token_kind_is node close_index Syntax_kind.RBRACKET) then
    ...
    let rec loop index depth =
      if Int.(index < 0) then
        ...
        if child_token_kind_is node index Syntax_kind.RBRACKET then
          loop Int.(index - 1) Int.(depth + 1)
        else
          if child_token_kind_is node index Syntax_kind.LBRACKET then
            if Int.equal depth 1 then
              let next = Int.(index + 1) in
              if child_token_kind_is node next Syntax_kind.AT || child_token_kind_is node next Syntax_kind.ATAT then
                Some index
              else None
            else loop Int.(index - 1) Int.(depth - 1)
          else loop Int.(index - 1) depth
...
module ModuleDeclaration = struct
  ...
  let first_member_node = fun decl -> first_child_node_matching decl ~matches:(
    fun kind -> Syntax_kind.(kind = MODULE_DECL_MEMBER)
  )
  ...
  let separator_token = fun decl -> first_child_token_matching (member_or_decl decl) ~matches:(
    fun kind -> Syntax_kind.(kind = EQ || kind = COLON)
  )
  ...
  module Member = struct
    ...
    let find_token = fun member kind ->
      ...
      let rec loop index =
        if index >= count then
          ...
          if child_token_kind_is member index kind then
            Some index
          else loop (index + 1)
      in
      loop 0
    ...
    let module_expr = fun member ->
      match find_node member ~matches:(
        fun kind -> is_module_expr_kind kind || Syntax_kind.(kind = MODULE_EXPR)
      ) with
    ...
    let module_type = fun member ->
      match find_node member ~matches:(
        fun kind -> is_module_type_kind kind || Syntax_kind.(kind = MODULE_TYPE_EXPR)
      ) with
    ...
    | Some separator_index ->
        ...
        let rec loop index =
          if index >= count then
            ...
            | Some node when node_matches node
              (
                fun kind -> is_module_expr_kind kind || is_module_type_kind kind
              ) -> Some node
            | _ -> loop (index + 1)
        in
  ...
  let body = fun decl ->
    ...
    | Some node when node_kind_is node Syntax_kind.PATH_MODULE_EXPR || node_kind_is node Syntax_kind.PATH_MODULE_TYPE -> Path
    | Some node when node_kind_is node Syntax_kind.STRUCT_MODULE_EXPR ->
        if has_child_node_kind node Syntax_kind.STRUCTURE_ITEM then
          Struct
        else EmptyStruct
  ...
  let for_each_body_path_ident = fun decl ~fn ->
    match body_specific_node decl with
    | Some node when node_kind_is node Syntax_kind.PATH_MODULE_EXPR || node_kind_is node Syntax_kind.PATH_MODULE_TYPE -> for_each_token_in_node node ~fn:(
      fun token ->
        if token_kind_is token Syntax_kind.IDENT then
          fn token
    )
  ...
  let for_each_typeof_body_path_ident = fun decl ~fn ->
    match typeof_body_node decl with
    | Some node -> for_each_token_in_node node ~fn:(
      fun token ->
        if token_kind_is token Syntax_kind.IDENT then
          fn token
    )
  ...
  let for_each_structure_item = fun decl ~fn ->
    match structure_body_node decl with
    | Some node -> for_each_child_node_matching node ~matches:(
      fun kind -> Syntax_kind.(kind = STRUCTURE_ITEM)
    ) ~fn
    | None -> ()
  ...
  let for_each_signature_item = fun decl ~fn ->
    match signature_body_node decl with
    | Some node -> for_each_child_node_matching node ~matches:(
      fun kind -> Syntax_kind.(kind = SIGNATURE_ITEM)
    ) ~fn
    | None -> ()
  ...
  let for_each_sig_body_token = fun decl ~fn ->
    ...
    | None -> ()
    | Some node ->
        let inside = ref false in Node.for_each_child node ~fn:(
          function
          | Syntax_tree.Token id ->
              let token = wrap_token node.tree id in
              if token_kind_is token Syntax_kind.SIG_KW then
                ...
                if token_kind_is token Syntax_kind.END_KW then
                  inside := false
                else
                  if !inside then
                    fn token
...
module ModuleTypeDeclaration = struct
  ...
  let head_node = fun decl -> first_child_node_matching decl ~matches:(
    fun kind -> Syntax_kind.(kind = MODULE_TYPE_DECL_HEAD)
  )

  let body_group = fun decl -> first_child_node_matching decl ~matches:(
    fun kind -> Syntax_kind.(kind = MODULE_TYPE_DECL_BODY)
  )
  ...
  let body_node = fun decl ->
    match body_group decl with
    | Some body -> first_child_node_matching body ~matches:(
      fun kind -> Syntax_kind.(kind = MODULE_TYPE_EXPR)
    )
    | None -> None
  ...
  let for_each_body_path_ident = fun decl ~fn ->
    match body_specific_node decl with
    | Some node when node_kind_is node Syntax_kind.PATH_MODULE_TYPE -> for_each_token_in_node node ~fn:(
      fun token ->
        if token_kind_is token Syntax_kind.IDENT then
          fn token
    )
  ...
  let for_each_constraint = fun decl ~fn ->
    match constrained_body_node decl with
    | None -> ()
    | Some node -> Node.for_each_child_node node ~fn:(
      fun child ->
        if node_kind_is child Syntax_kind.WITH_TYPE_CONSTRAINT || node_kind_is child Syntax_kind.WITH_MODULE_CONSTRAINT then
          fn child
    )
end
...
module ModuleTypeConstraint = struct
  ...
  let cast = fun (node: node) ->
    if node_kind_is node Syntax_kind.WITH_TYPE_CONSTRAINT || node_kind_is node Syntax_kind.WITH_MODULE_CONSTRAINT then
      Some node
    else None
  ...
  let view = fun (constraint_: t) ->
    if node_kind_is constraint_ Syntax_kind.WITH_TYPE_CONSTRAINT then
      Type { path = type_path constraint_; operator = first_child_token_matching constraint_ ~matches:(
        fun kind -> Syntax_kind.(kind = EQ || kind = COLONEQ || kind = PLUS)
      ); body = type_body constraint_ }
    else
      if node_kind_is constraint_ Syntax_kind.WITH_MODULE_CONSTRAINT then
        Module { path = module_path constraint_; body = module_body constraint_ }
      else Unknown constraint_
end
...
module OpenDeclaration = struct
  ...
  let for_each_path_ident = fun decl ~fn -> Node.for_each_child_token decl ~fn:(
    fun token ->
      if token_kind_is token Syntax_kind.IDENT then
        fn token
  )
    ...
    let rec find_body index =
      if index >= Node.child_count decl then
        ...
        | Some (Syntax_tree.Node id) ->
            ...
            else find_body (index + 1)
        | Some (Syntax_tree.Token _) | Some (Syntax_tree.Missing _) | None -> find_body (index + 1)
    in
    find_body 0
...
let for_each_declaration_name_token = fun decl ~keyword ~fn ->
  ...
  let done_ = ref false in Node.for_each_child_token decl ~fn:(
    fun token ->
      if not !done_ then
        if token_kind_is token Syntax_kind.COLON then
          ...
          if !seen_keyword then
            ...
            if token_kind_is token keyword then
              seen_keyword := true
  )
...
module ValueDeclaration = struct
  ...
  let for_each_name_token = fun decl ~fn -> for_each_declaration_name_token decl ~keyword:Syntax_kind.VAL_KW ~fn

  let for_each_annotation_token = fun decl ~fn -> for_each_token_after_child_token decl ~matches:(
    fun kind -> Syntax_kind.(kind = COLON)
  ) ~fn
end
...
module ExternalDeclaration = struct
  ...
  let for_each_name_token = fun decl ~fn -> for_each_declaration_name_token decl ~keyword:Syntax_kind.EXTERNAL_KW ~fn

  let for_each_primitive_string = fun decl ~fn -> Node.for_each_child_token decl ~fn:(
    fun token ->
  ...
  let for_each_attribute_token = fun decl ~fn ->
    ...
    let after_primitives = ref false in Node.for_each_child_token decl ~fn:(
      fun token ->
...
module ExceptionDeclaration = struct
  ...
  let head_node = fun decl -> first_child_node_matching decl ~matches:(
    fun kind -> Syntax_kind.(kind = EXCEPTION_DECL_HEAD)
  )
  ...
  let view = fun decl ->
    match first_child_node_matching decl ~matches:(
      fun kind -> Syntax_kind.(kind = EXCEPTION_ALIAS || kind = EXCEPTION_PAYLOAD)
    ) with
...
module StructureItem = struct
  ...
  let declaration = fun (item: structure_item) -> first_child_node_matching item ~matches:(
    fun kind -> not (Syntax_kind.(kind = ERROR))
  )
...
module SignatureItem = struct
  ...
  let declaration = fun (item: signature_item) -> first_child_node_matching item ~matches:(
    fun _ -> true
  )
...
module Implementation = struct
  ...
  let for_each_item = fun (impl: implementation) ~fn -> for_each_child_node_matching impl ~matches:(
    fun kind -> Syntax_kind.(kind = STRUCTURE_ITEM)
  ) ~fn
end
...
module Interface = struct
  ...
  let for_each_item = fun (interface: interface) ~fn -> for_each_child_node_matching interface ~matches:(
    fun kind -> Syntax_kind.(kind = SIGNATURE_ITEM)
  ) ~fn
end
...
module SourceFile = struct
  ...
  let implementation = fun (source_file: source_file) -> Node.first_child_node source_file ~kind:Syntax_kind.IMPLEMENTATION

  let interface = fun (source_file: source_file) -> Node.first_child_node source_file ~kind:Syntax_kind.INTERFACE
