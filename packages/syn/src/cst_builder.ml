...
let split_type_declaration_trailing_trivia =
  ...
  let current_docstrings, docstrings_for_next =
    ...
    else if not prefer_current_continuation then
      ([], contiguous_docstrings)
    else
      match contiguous_docstrings with
      | [] ->
          ([], [])
      | first :: rest ->
          ([ first ], rest)
    ...
    | Syntax_kind.IF_EXPR -> (
        ...
        | condition_node :: then_node :: else_nodes ->
            Cst.Expression.If {
              ...
              then_token = direct_required_token_with_text ~context:[ "if_expression" ] node "then";
...
and fun_expression_from_node = fun node ->
  ...
  | body_node :: rev_prefix_nodes ->
      ...
      Some {
        ...
        arrow_token = direct_required_token_with_text ~context:[ "fun_expression" ] node "->";
  ...
  if is_binding_operator_expression_node node then
    ...
    | Some (`Value (is_recursive_binding, binding_pattern_node, prefix_nodes, bound_value_node, and_binding_nodes, body_node)) ->
        ...
        Some {syntax_node = node; keyword_token = (
          ...
          |> List.filter_map (fun binding -> binding); body = expression_from_node body_node; is_recursive = is_recursive_binding; attributes = []}
    | _ -> None
      ...
      | body_exprs, false -> (
          ...
          | body_expr :: _ ->
              Some {
                ...
                when_token = None;
                arrow_token = direct_required_token_with_text ~context:[ "match_case" ] node "->";
                pattern = pattern_from_node pattern_node;
                guard = None;
                body = body_expr
              }
...
let declaration_name_token_from_node = fun node ->
  let operator_token_from_node =
    fun node ->
      direct_non_trivia_tokens node |> List.find_opt
        (fun syntax_token ->
          let text = Ceibo.Red.SyntaxToken.text syntax_token in
          not (String.equal text "(" || String.equal text ")")) |> Option.map token
  in
  direct_non_trivia_nodes node |> List.find_map
    (fun child ->
      match Ceibo.Red.SyntaxNode.kind child with
