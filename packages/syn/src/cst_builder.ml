            ...
            if kind = Syntax_kind.WHITESPACE then
              ...
              let item : record_field_item option =
                if saw_newline then
                  ...
                  standalone_trivia_item_from_token
                    ~comment_item_of_comment:(fun comment ->
                      (TrailingComment comment: record_field_item))
                    ~docstring_item_of_docstring:(fun docstring ->
      ...
      | Some index ->
          ...
          let sign, digits =
            if String.length exponent_body = 0 then
              ...
              | '+' -> (
                Some Cst.Positive,
                substring exponent_body 1 (String.length exponent_body - 1)
              )
              | '-' -> (
                Some Cst.Negative,
                substring exponent_body 1 (String.length exponent_body - 1)
...
let normalize_structure_items_owned_trivia = fun ~source items ->
    normalize_ordered_items_owned_trivia
      ~source
      {
        ...
        | syntax_tokens when not (List.is_empty syntax_tokens) ->
            ...
            if String.equal (Cst.Token.text lifted) "_" then
              ...
              let sigil_token =
                ...
                | first :: _ when String.equal (Ceibo.Red.SyntaxToken.text first) "'" -> Some (token
                  first)
                | _ -> None
              in
...
let rec pattern_from_node = fun node ->
    ...
    let tuple_pattern_from_node node =
      ...
      let rec loop children saw_tilde pending_label awaiting_payload acc =
        ...
        | Ceibo.Red.Token syntax_token :: rest when is_trivia
          (Ceibo.Red.SyntaxToken.kind syntax_token) ->
...
and expression_from_node = fun node ->
    ...
    let expression =
      ...
      | Syntax_kind.ASSERT_EXPR -> (
          match direct_non_trivia_nodes node
          |> List.find_opt can_lift_expression_node
          |> Option.map expression_from_node with
      ...
      | Syntax_kind.APPLY_EXPR -> (
          ...
          = Syntax_kind.ATTRIBUTE_EXPR ->
              ...
              (
                ...
                | _ -> (
                    match constructor_path_from_expression_node callee_node, apply_argument_from_node
                      attribute_node with
        ...
        body_node ->
          ...
          Some (
            ({syntax_node = node; keyword_token = (
                  ...
                  (fun binding -> binding) |> let_binding_chain_of_list; body = class_expression_from_node
                  body_node}: Cst.class_let_expression)
          )
      | _ -> None
...
let variant_constructor_from_node = fun node ->
    ...
    |> List.filter
      (fun child -> not (is_attribute_node child && not (can_lift_core_type_node child))) in
...
let raw_annotation_payload_from_shell = fun shell_node ->
    ...
    let payload_non_trivia_tokens =
      ...
      let rec skip_sigils = function
        | syntax_token :: rest when is_annotation_sigil_text
          (Ceibo.Red.SyntaxToken.text syntax_token) -> skip_sigils rest
        | rest -> rest
      in
    ...
    let trivia_entries =
      ...
      |> List.filter
        (fun syntax_token ->
          let token_span = Ceibo.Red.SyntaxToken.span syntax_token in
          not
            (List.exists (fun owned_span -> span_contains owned_span token_span) owned_trivia_spans))
      |> List.filter_map
        (fun syntax_token ->
          standalone_trivia_item_from_token ~comment_item_of_comment ~docstring_item_of_docstring syntax_token
          |> Option.map
        ...
        List.iteri
          (fun index ({ constrained_type; replacement_type; _ }: Cst.module_type_constraint) ->
            validate_core_type
              ...
              constrained_type;
            validate_core_type
              ~context:((("module_type.with.constraint[" ^ Int.to_string index ^ "].replacement")
              :: context))
              replacement_type)
          constraints
    ...
    | Cst.Expression.ObjectOverride { fields; _ } ->
        List.iteri
          (fun index (field: Cst.object_override_field) ->
            Option.iter
              (validate_expression
                ~context:((("expression.object_override.field[" ^ Int.to_string index ^ "].value")
                :: context)))
              field.value)
          fields
...
let validate_type_extension = fun ~context ({ constructors; _ }: Cst.TypeExtension.t) ->
    List.iteri
      (fun index constructor ->
        Option.iter
          (validate_constructor_arguments
            ~context:((("item.type_extension.constructor[" ^ Int.to_string index ^ "].arguments")
            :: context)))
          (Cst.VariantConstructor.arguments constructor);
        Option.iter
          (validate_core_type
...
let validate_structure_item = fun ~context ->
    ...
    | Cst.StructureItem.LetBinding { binding_pattern; value; and_binding; _ } ->
        ...
        let rec validate_and_bindings = fun index ->
            ...
            | Some binding ->
                validate_pattern
                  ~context:((("item.let_binding.and_bindings[" ^ Int.to_string index ^ "].pattern")
                  :: context))
                  (Cst.LetBinding.binding_pattern binding);
                validate_expression
                  ~context:((("item.let_binding.and_bindings[" ^ Int.to_string index ^ "].value")
                  :: context))
                  (Cst.LetBinding.value binding);
...
let structure_items_from_syntax_nodes = fun nodes ->
    ...
    | items -> (
        match nodes with
        | first :: _ -> Ok (normalize_structure_items
          ~source:(source_text_of_syntax_node_tree first)
          items)
        | [] -> Ok items
      )
    | exception Bail error -> Error error
...
let signature_items_from_syntax_nodes = fun nodes ->
    ...
    | items -> (
        match nodes with
        | first :: _ -> Ok (normalize_signature_items
          ~source:(source_text_of_syntax_node_tree first)
          items)
        | [] -> Ok items
      )
