...
let walker = fun visitor ->
  let rec walk = {apply_argument = (fun ctx node ->
      ...
      visitor.visit_variant_constructor ctx walk node); descend_apply_argument = (fun ctx node -> descend_apply_argument
    walk
    ctx
    node); descend_attribute = (fun ctx node -> descend_attribute walk ctx node); descend_binding_operator_binding = (fun ctx node -> descend_binding_operator_binding
    walk
    ctx
    node); descend_class_declaration = (fun ctx node -> descend_class_declaration walk ctx node); descend_class_expression = (fun ctx node -> descend_class_expression
    walk
    ctx
    node); descend_class_field = (fun ctx node -> descend_class_field walk ctx node); descend_class_definition = (fun ctx node -> descend_class_definition
    walk
    ctx
    node); descend_class_type = (fun ctx node -> descend_class_type walk ctx node); descend_class_type_declaration = (fun ctx node -> descend_class_type_declaration
    walk
    ctx
    node); descend_class_type_field = (fun ctx node -> descend_class_type_field walk ctx node); descend_core_type = (fun ctx node -> descend_core_type
    walk
    ctx
    node); descend_exception_declaration = (fun ctx node -> descend_exception_declaration walk ctx node); descend_expression = (fun ctx node -> descend_expression
    walk
    ctx
    node); descend_extension = (fun ctx node -> descend_extension walk ctx node); descend_external_declaration = (fun ctx node -> descend_external_declaration
    walk
    ctx
    node); descend_functor_parameter = (fun ctx node -> descend_functor_parameter walk ctx node); descend_implementation = (fun ctx node -> descend_implementation
    walk
    ctx
    node); descend_include_statement = (fun ctx node -> descend_include_statement walk ctx node); descend_interface = (fun ctx node -> descend_interface
    walk
    ctx
    node); descend_let_binding = (fun ctx node -> descend_let_binding walk ctx node); descend_match_case = (fun ctx node -> descend_match_case
    walk
    ctx
    node); descend_module_signature = (fun ctx node -> descend_module_signature walk ctx node); descend_module_structure = (fun ctx node -> descend_module_structure
    walk
    ctx
    ...
    walk
    ctx
    node); descend_module_type_constraint = (fun ctx node -> descend_module_type_constraint
    walk
    ctx
    node); descend_module_type_declaration = (fun ctx node -> descend_module_type_declaration
    walk
    ctx
    node); descend_object_member = (fun ctx node -> descend_object_member walk ctx node); descend_object_type_field = (fun ctx node -> descend_object_type_field
    walk
    ctx
    node); descend_open_statement = (fun ctx node -> descend_open_statement walk ctx node); descend_parameter = (fun ctx node -> descend_parameter
    walk
    ctx
    node); descend_pattern = (fun ctx node -> descend_pattern walk ctx node); descend_payload = (fun ctx node -> descend_payload
    walk
    ctx
    ...
    walk
    ctx
    node); descend_row_field = (fun ctx node -> descend_row_field walk ctx node); descend_signature_item = (fun ctx node -> descend_signature_item
    walk
    ctx
    node); descend_source_file = (fun ctx node -> descend_source_file walk ctx node); descend_structure_item = (fun ctx node -> descend_structure_item
    walk
    ctx
    node); descend_type_binder = (fun ctx node -> descend_type_binder walk ctx node); descend_type_constraint = (fun ctx node -> descend_type_constraint
    walk
    ctx
    node); descend_type_declaration = (fun ctx node -> descend_type_declaration walk ctx node); descend_type_definition = (fun ctx node -> descend_type_definition
    walk
    ctx
    node); descend_type_extension = (fun ctx node -> descend_type_extension walk ctx node); descend_type_parameter = (fun ctx node -> descend_type_parameter
    walk
    ctx
    node); descend_value_declaration = (fun ctx node -> descend_value_declaration walk ctx node); descend_variant_constructor = (fun ctx node -> descend_variant_constructor
    walk
    ctx
    node)}
  in
