...
let main ~args =
  ...
  command "syn" |> version "0.1.0" |> about "OCaml syntax analysis tool" |> subcommands [ command "tokenize" |> about "Print token stream for a file" |> args [ positional "FILE" |> help "OCaml source file to tokenize" |> required true; flag "json" |> long "json" |> help "Output in JSON format" ]; command "parse" |> about "Parse file with the streaming parser" |> args [ positional "FILE" |> help "OCaml source file to parse" |> required true; flag "json" |> long "json" |> help "Output syntax tree as JSON" ]; command "explain" |> about "Explain an error code" |> args [ positional "ERROR_CODE" |> help "Error code to explain (e.g. E0001)" |> required true ] ] in
