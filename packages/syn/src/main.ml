...
let slice_of_file_contents = fun contents -> IO.IoVec.IoSlice.from_string contents |> Result.expect ~msg:"failed to create syn source slice"
...
let span_text = fun source span ->
  ...
  if width <= 0 then
    ""
  else String.sub source ~offset:span.Ceibo.Span.start ~len:width
...
let trivia_kind_to_json = function
  ...
  | Syn.Token.WhitespaceTrivia ->
      Data.Json.Object [
        "kind", Data.Json.string "whitespace";
      ]
...
let token_to_json = fun ~source (token: Syn.Token.t) ->
  Data.Json.Object [
    ...
    "leading_trivia", Data.Json.Array (List.map token.Syn.Token.leading_trivia ~fn:(trivia_to_json ~source));
  ]

let vector_to_json = fun vector ~fn -> Data.Json.Array (Vector.iter vector |> Iterator.map ~fn |> Iterator.to_list)
...
let handle_token_stream = fun sub_matches ->
  ...
  | Ok content ->
      ...
      if json then
        let json_tokens = List.map tokens ~fn:(token_to_json ~source:content) in println (Data.Json.to_string (Data.Json.array json_tokens))
      else List.for_each tokens ~fn:(
        fun tok -> println (Syn.Token.show_kind tok.Syn.Token.kind)
      )
  ...
  | Ok source ->
      ...
      if json then
        ...
        if Vector.length result.Syn.Parser.diagnostics != 0 then
          Syn.DiagnosticReporter.print ~file ~source (diagnostics_to_list result.Syn.Parser.diagnostics)
        else
          let root = Syn.SyntaxTree.root result.Syn.Parser.tree in Log.info ("Parsed successfully: " ^ Int.to_string root.Syn.SyntaxTree.full_width ^ " bytes")
...
let handle_deps = fun sub_matches ->
  ...
  | Ok source -> (
    ...
    | Ok deps ->
        ...
        else List.for_each (Syn.Deps.modules deps) ~fn:println
    | Error error ->
        if json then
          ...
          (
            match error with
            | Syn.Deps.Parse_diagnostics diagnostics -> Syn.DiagnosticReporter.print ~file ~source diagnostics
          );
        System.exit 1
  )
...
let main ~args =
  ...
  command "syn" |> version "0.1.0" |> about "OCaml syntax analysis tool" |> subcommands
    [
      command "tokenize" |> about "Print token stream for a file" |> args [ positional "FILE" |> help "OCaml source file to tokenize" |> required true; flag "json" |> long "json" |> help "Output in JSON format" ];
      command "parse" |> about "Parse file with the streaming parser" |> args [ positional "FILE" |> help "OCaml source file to parse" |> required true; flag "json" |> long "json" |> help "Output syntax tree as JSON" ];
      command "deps" |> about "Print syntactic module dependencies for a file" |> args [ positional "FILE" |> help "OCaml source file to inspect" |> required true; flag "json" |> long "json" |> help "Output dependencies as JSON" ];
      command "explain" |> about "Explain an error code" |> args [ positional "ERROR_CODE" |> help "Error code to explain (e.g. E0001)" |> required true ];
    ] in
