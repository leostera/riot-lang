...
let slice_of_file_contents = fun contents ->
  IO.IoVec.IoSlice.from_string contents |> Result.expect ~msg:"failed to create syn source slice"
...
let trivia_kind_to_json = function
  | Syn.Token.CommentTrivia { terminated; _ } -> Data.Json.Object [
    ("kind", Data.Json.string "comment");
    ("terminated", Data.Json.bool terminated);
  ]
  | Syn.Token.DocstringTrivia { terminated; _ } -> Data.Json.Object [
...
let vector_to_json = fun vector ~fn ->
  Data.Json.Array (Vector.iter vector |> Iterator.map ~fn |> Iterator.to_list)
...
let parse_error_to_json = function
  | Syn.Deps.Parse_diagnostics diagnostics -> Data.Json.Object [
  ...
  | Ok source -> (
      ...
      | Error error ->
          if json then
            ...
            (
              match error with
              | Syn.Deps.Parse_diagnostics diagnostics -> Syn.DiagnosticReporter.print
                ~file
                ~source
                diagnostics
            );
    ...
    let open ArgParser in
      ...
      |> subcommands
        [
          ...
          |> args
            [
              positional "FILE" |> help "OCaml source file to inspect" |> required true;
              flag "json" |> long "json" |> help "Output dependencies as JSON"
            ];
          command "explain"
