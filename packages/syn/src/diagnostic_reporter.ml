...
let make_source_layout = fun source ->
  ...
  for index = 0 to Array.length lines - 1 do
    ...
    let line = Array.get_unchecked lines ~at:index in offset := !offset + String.length line + 1
  done;
  { lines; line_starts }
...
let line_for_pos = fun layout pos ->
  ...
  in
  let start_offset = Array.get layout.line_starts ~at:line_idx |> Option.unwrap_or ~default:0 in (line_idx, Int.max 0 (pos - start_offset))
...
let extract_code_snippet_from_layout = fun layout (span: Ceibo.Span.t) ->
  if Array.length layout.lines = 0 then
    ...
    if line_idx >= 0 && line_idx < Array.length layout.lines then
      ...
      let pointer_line = String.make ~len:start_col ~char:' ' ^ "^" in Some (code_line, pointer_line, line_idx + 1)
    else None
