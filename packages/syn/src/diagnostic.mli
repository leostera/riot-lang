...
(** ## Convenience Constructors
    ...
    These helpers create diagnostics for common error types without
    needing to construct the `kind` variant manually. *)
...
(** Create an "unexpected token" diagnostic.
    ...
    Example:
    ```ocaml
    Diagnostic.make_unexpected_token
...
(** Create an "unexpected end of file" diagnostic.
    ...
    Example:
    ```ocaml
    Diagnostic.make_unexpected_eof
      ~expected:"expression"
      ~span:eof_span
    ```
*)
...
(** Create an "invalid syntax" diagnostic.
    ...
    Example:
    ```ocaml
    Diagnostic.make_invalid_syntax
      ~context:"let binding"
      ~span:binding_span
    ```
*)
...
(** Create an "unclosed delimiter" diagnostic.
    ...
    Example:
    ```ocaml
    Diagnostic.make_unclosed_delimiter
      ~delimiter:"("
      ~opened_at:start_pos
      ~span:current_span
    ```
...
(** Create a "mismatched delimiter" diagnostic.
    ...
    Example:
    ```ocaml
    Diagnostic.make_mismatched_delimiter
      ~expected:"end"
      ~found:")"
      ~span:delimiter_span
    ```
...
(** `to_string diag` converts a diagnostic to a human-readable error message.
    ...
    Unexpected token '+' (expected identifier) at 5..6
    Invalid syntax in let binding at 12..18
    ```
    
    Example:
    ```ocaml
    let msg = Diagnostic.to_string diag in
