...
(** OCaml Parser - Token Stream to Syntax Tree
    ...
    This module implements a recursive descent parser that converts a flat
    token stream into a lossless Ceibo syntax tree.
    ...
    The parser is a single-pass recursive descent parser that:
    1. Consumes tokens from the lexer
    2. Builds green tree nodes bottom-up
    3. Collects diagnostics for any problems encountered
    4. Never backtracks or fails
    ...
    let result = Parser.parse ~source tokens in
    
    (* Always succeeds - check diagnostics for errors *)
    match result.diagnostics with
    | [] -> 
        Printf.printf "Parsed successfully!\n";
        (* Work with result.tree *)
    | errors ->
        List.iter (fun diag ->
    ...
    ```ocaml
    let source = "let x = " in  (* incomplete *)
    let tokens = Lexer.tokenize source in
    let result = Parser.parse ~source tokens in
    
    (* Still returns a tree! *)
    Printf.printf "Tree has %d diagnostics\n" 
      (List.length result.diagnostics);
    
    (* The tree contains ERROR/MISSING nodes *)
    (* Diagnostics describe what went wrong *)
    List.iter (fun diag ->
      let span_str = Ceibo.Span.to_string diag.span in
    ...
    (* The green tree is position-independent *)
    let green_tree = result.tree in
    Printf.printf "Total width: %d bytes\n" green_tree.width;
    
    (* Create a red view for positioned queries *)
    let root = Ceibo.Red.new_root green_tree in
    
    (* Traverse the tree *)
    Ceibo.Red.SyntaxNode.preorder root (function
      | Ceibo.Red.Node node ->
          let kind = Ceibo.Red.SyntaxNode.kind node in
          let span = Ceibo.Red.SyntaxNode.span node in
          Printf.printf "%s at %s\n"
            (Syntax_kind.to_string kind)
...
type parse_result = {
  ...
  (** List of parse errors and warnings.
      ...
      Empty list means no errors were found. Each diagnostic describes
      a specific problem with its location in the source. *)
}
(** Result of parsing.
    
    The parser **always** returns a parse result, even if the source code
    is malformed. The `diagnostics` field indicates whether there were
    any problems.
