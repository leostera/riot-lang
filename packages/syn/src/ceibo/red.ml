...
module SyntaxNode = struct
  ...
  let child_count (node : ('kind, 'text) syntax_node) = Green.child_count (green node)
  ...
  let child (node : ('kind, 'text) syntax_node) index =
    ...
    | Some elem ->
        ...
        in
        Some (match elem with
        | Green.Token t -> Token { green_token = t; parent = Some node; offset = child_offset }
        | Green.Node n -> Node { green_node = n; parent = Some node; offset = child_offset })
