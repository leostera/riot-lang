...
(** `tokenize source` lexes source code into a flat list of tokens.
    ...
    Example:
    ```ocaml
    let tokens = Syn.tokenize "let x = 42" in
    (* tokens = [Keyword Let; Whitespace; Ident "x"; Whitespace; Eq; ...] *)
    ```
*)
...
(** `parse source` parses source code into a Ceibo green tree with diagnostics.
    ...
    Example:
    ```ocaml
    let result = Syn.parse "let x = 1 + 2" in
    
    (* Use the green tree directly *)
    Printf.printf "Tree width: %d\n" result.tree.width;
    
    (* Or create a red view for positioned traversal *)
    let root = Ceibo.Red.new_root result.tree in
    let span = Ceibo.Red.SyntaxNode.span root in
    Printf.printf "Covers: %s\n" (Ceibo.Span.to_string span)
    ```
*)
