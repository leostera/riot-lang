...
module Env = struct
  module Names = struct
    ...
    let union = fun left right -> List.unique (List.sort (left @ right) ~compare:String.compare) ~compare:String.compare
  ...
  let merge = fun left right -> List.fold_left right ~init:left ~fn:(
    fun env (name, node) -> add name node env
  )

  let rec rebind = fun free_names ->
    function
    | Node (_, children) -> Node (free_names, List.map children ~fn:(
      fun (name, child) -> (name, rebind free_names child)
    ))

  let rebind_exports = fun free_names exports -> List.map exports ~fn:(
    fun (name, node) -> (name, rebind free_names node)
  )
  ...
  let rec add_binding = fun env ~path ~free_names ~exports ->
    ...
    | [ segment ] ->
        ...
        in
        let Node (free, children) = existing in
        let merged_children = merge children (rebind_exports free_names exports) in add segment (Node (Names.union free free_names, merged_children)) env
    | segment :: rest ->
        let existing =
          match List.find env ~fn:(
            fun (name, _) -> String.equal name segment
        ...
        in
        let Node (free, children) = existing in
        let updated_children = add_binding children ~path:rest ~free_names ~exports in add segment (Node (free, updated_children)) env
  ...
  let rec add_scoped_binding = fun env ~path ~free_names ~exports ->
    ...
    | [ segment ] ->
        ...
        in
        let Node (free, children) = existing in
        let merged_children = merge children exports in add segment (Node (Names.union free free_names, merged_children)) env
    | segment :: rest ->
        let existing =
          match List.find env ~fn:(
            fun (name, _) -> String.equal name segment
        ...
        in
        let Node (free, children) = existing in
        let updated_children = add_scoped_binding children ~path:rest ~free_names ~exports in add segment (Node (free, updated_children)) env
  ...
  let rec collect_free = function
    | Node (free, children) -> List.fold_left children ~init:free ~fn:(
      fun acc (_, child) -> Names.union acc (collect_free child)
    )

  let merge_children env node = merge env (children node)

  let find = fun name env -> List.find env ~fn:(
    fun (key, _) -> String.equal key name
  ) |> Option.map ~fn:(
    fun (_, value) -> value
  )
...
module DepSet = struct
  ...
  let add = fun deps name ->
    let _ = HashSet.insert deps ~value:name in deps

  let add_names = fun deps names ->
    List.for_each names ~fn:(
      fun name ->
...
type t = { modules: string list; env: Env.t; exports: Env.t }
...
let to_json = fun t ->
  Json.Object [
    "modules", Json.Array (List.map t.modules ~fn:(
...
let is_module_head = fun segment -> String.length segment > 0 && is_uppercase_ascii (String.get_unchecked segment ~at:0)
...
module Ast_deps = struct
  ...
  let ast_path_segments = fun path ->
    ...
    A.Path.for_each_ident path ~fn:(
      fun token -> Vector.push segments ~value:(A.Token.text token)
    );
  ...
  let direct_path_between = fun node start stop ->
    let rec loop index expect_ident acc =
      if index >= stop then
        ...
        match child_token_at node index with
        | Some token when expect_ident && Syntax_kind.(token_kind token = IDENT) -> loop (index + 1) false (A.Token.text token :: acc)
        | Some token when (not expect_ident) && Syntax_kind.(token_kind token = DOT) -> loop (index + 1) true acc
        | _ -> None
    in
    loop start true []
  ...
  let collect_direct_module_refs_between = fun env deps node start stop ->
    let rec collect_path index acc =
      if index >= stop then
        ...
        match child_token_at node index with
        | Some token when Syntax_kind.(token_kind token = IDENT) -> collect_path (index + 1) (A.Token.text token :: acc)
    ...
    let rec scan index deps =
      if index >= stop then
        ...
        | Some token when Syntax_kind.(token_kind token = IDENT) ->
            let next, segments = collect_path index [] in scan next (add_module_segments env deps segments)
        | _ -> scan (index + 1) deps
    in
    scan start deps
  ...
  let collect_direct_module_accesses_between = fun env deps node start stop ->
    let rec collect_path index acc saw_dot =
      if index >= stop then
        ...
        match child_token_at node index with
        | Some token when Syntax_kind.(token_kind token = IDENT) -> collect_path (index + 1) (A.Token.text token :: acc) saw_dot
  ...
  let rec direct_module_binding_between env deps node start stop =
    ...
    | None -> (
      ...
      | Some token when Syntax_kind.(token_kind token = STRUCT_KW) -> direct_struct_binding_between env deps node (start + 1) stop
      | _ ->
          let deps = collect_direct_module_refs_between env deps node start stop in Ok (deps, Env.bound)
    )
  and direct_struct_binding_between env deps node start stop =
    ...
    let rec find_token index limit kind =
      if index >= limit then
        ...
        if child_token_kind_is node index kind then
          Some index
        else find_token (index + 1) limit kind
    in
    let rec member_stop index =
      if index >= stop || child_token_kind_is node index Syntax_kind.MODULE_KW || child_token_kind_is node index Syntax_kind.END_KW then
        index
      else member_stop (index + 1)
    in
    let rec scan index deps bindings =
      if index >= stop then
        Ok (deps, Env.make_node bindings)
      else
        if child_token_kind_is node index Syntax_kind.MODULE_KW then
          let name =
  ...
  let first_direct_child_node = fun node kind -> first_child_node_matching node ~matches:(
    fun child_kind -> Syntax_kind.(child_kind = kind)
  )

  let fold_child_nodes = fun node init fn ->
    let acc = ref init in
    A.Node.for_each_child_node node ~fn:(
      fun child -> acc := fn !acc child
    );
    !acc
  ...
  let collect_child_nodes = fun collect env deps node ->
    fold_child_nodes node (Ok deps)
      (
        fun acc child ->
          let* deps = acc in collect env deps child
      )
  ...
  let collect_direct_type_payload_paths = fun env deps node ->
    ...
    let rec scan active index deps =
      if index >= count then
        ...
        match child_token_at node index with
        | Some token when Syntax_kind.(token_kind token = OF_KW || token_kind token = COLON) -> scan true (index + 1) deps
        | Some token when Syntax_kind.(token_kind token = PIPE || token_kind token = AND_KW) -> scan false (index + 1) deps
        | Some token when active && Syntax_kind.(token_kind token = IDENT) ->
            let next, segments = collect_path index [] in scan active next (add_parent_segments env deps segments)
  ...
  let collect_direct_type_extension_path = fun env deps node ->
    ...
    let rec find_plus index =
      if index >= count then
        ...
        if child_token_kind_is node index Syntax_kind.PLUS then
          Some index
        else find_plus (index + 1)
    in
    let rec collect_before_plus index plus_index acc =
      if index >= plus_index then
        ...
        match child_token_at node index with
        | Some token when Syntax_kind.(token_kind token = IDENT) -> collect_before_plus (index + 1) plus_index (A.Token.text token :: acc)
        | Some token when Syntax_kind.(token_kind token = DOT) -> collect_before_plus (index + 1) plus_index acc
  ...
  let rec bind_pattern_modules env node =
    ...
    | Syntax_kind.FIRST_CLASS_MODULE_PATTERN ->
        ...
        let rec find_binding index seen_module =
          if index >= count then
            ...
            match child_token_at node index with
            | Some token when Syntax_kind.(token_kind token = MODULE_KW) -> find_binding (index + 1) true
            | Some token when seen_module && Syntax_kind.(token_kind token = IDENT) -> Some (A.Token.text token)
    ...
    | _ ->
        fold_child_nodes node env
          (
            fun env child -> bind_pattern_modules env child
          )

  let rec collect_node env deps node =
    match node_kind node with
    | Syntax_kind.PATH_EXPR | Syntax_kind.PATH_PATTERN | Syntax_kind.PATH_TYPE -> Ok (add_parent_segments env deps (path_segments node))
    ...
    | Syntax_kind.FIELD_ACCESS_EXPR ->
        ...
        in
        Ok deps
    | Syntax_kind.LOCAL_OPEN_EXPR | Syntax_kind.LOCAL_OPEN_PATTERN -> collect_local_open env deps node
    ...
    | Syntax_kind.TYPE_DECL ->
        ...
        let deps = collect_direct_type_payload_paths env deps node in collect_child_nodes collect_node env deps node
    | Syntax_kind.OPAQUE_TYPE -> Ok (collect_direct_module_accesses_between env deps node 0 (A.Node.child_count node))
    ...
    | Syntax_kind.MODULE_TYPE_DECL ->
        let* deps = collect_module_type_decl env deps node in Ok deps
    ...
    | Some expr -> (
      ...
      | Some local_open ->
          let module_path, body =
            ...
            | A.LocalOpenExpr.LetOpen { module_path; body; _ } | A.LocalOpenExpr.Delimited { module_path; body; _ } -> (module_path, body)
          in
          let deps, env =
            match module_path with
            | Some path -> (
              match ast_path_segments path with
              | head :: _ as segments when is_module_head head -> open_alias ~fallback:true env deps segments
              | _ -> (deps, env)
            )
            | None -> (deps, env)
          in
  ...
  and collect_let_binding env deps node =
    ...
    | Some binding ->
        ...
        A.LetBinding.for_each_parameter binding ~fn:(
        ...
        let* deps =
          ...
          | None -> Ok deps
        in
        let* deps = Vector.iter parameters |> Iterator.fold ~init:(Ok deps) ~fn:(
          fun parameter acc ->
            let* deps = acc in collect_node env deps parameter
        )
        in
        let* deps =
        ...
        let env =
          ...
          | None -> env
        in
        let env = Vector.iter parameters |> Iterator.fold ~init:env ~fn:(
          fun parameter env -> bind_pattern_modules env parameter
        ) in
        (
  ...
  and collect_let_module_expr env deps node =
    ...
    let rec find_token index kind =
      if index >= count then
        ...
        if child_token_kind_is node index kind then
          Some index
        else find_token (index + 1) kind
    in
    let eq_index = find_token 0 Syntax_kind.EQ in
    let in_index = find_token 0 Syntax_kind.IN_KW in
    let body_expr =
      match in_index with
      | Some in_index -> find_node_after (in_index + 1) ~matches:(
        fun kind -> not (Syntax_kind.(kind = MODULE_EXPR))
      )
      | None -> None
    in
    let module_expr =
    ...
    let* (deps, binding) =
      ...
      | None -> (
        match eq_index, in_index with
        | Some eq_index, Some in_index -> direct_module_binding_between env deps node (eq_index + 1) in_index
        | _ -> Ok (deps, Env.bound)
      )
    in
    let env =
  ...
  and collect_fun_expr env deps node =
    ...
    let rec find_arrow index =
      if index >= count then
        ...
        if child_token_kind_is node index Syntax_kind.ARROW then
          Some index
        else find_arrow (index + 1)
    in
    ...
    | Some arrow_index ->
        let* deps = collect_patterns 0 arrow_index deps
        in
        let env = bind_patterns 0 arrow_index env in
        (
          match find_node_between node (arrow_index + 1) count ~matches:(
            fun _ -> true
          ) with
          | Some body -> collect_node env deps body
          | None -> Ok deps
  ...
  and module_binding env deps node =
    ...
    | Syntax_kind.STRUCT_MODULE_EXPR ->
        let* (deps, _, bindings) = collect_structure_items_in env deps node in Ok (deps, Env.make_node bindings)
    | Syntax_kind.CONSTRAINT_MODULE_EXPR | Syntax_kind.PAREN_MODULE_EXPR -> (
      match first_child_node_matching node ~matches:is_module_expr_kind with
      | Some inner -> module_binding env deps inner
      | None ->
          let* deps = collect_node env deps node in Ok (deps, Env.bound)
    )
    | Syntax_kind.FUNCTOR_MODULE_EXPR | Syntax_kind.APPLY_MODULE_EXPR | Syntax_kind.OPAQUE_MODULE_EXPR ->
        let* deps = collect_node env deps node in Ok (deps, Env.bound)
    | _ ->
        let* deps = collect_node env deps node in Ok (deps, Env.bound)
  and collect_module_expression env deps node =
    let* (deps, _) = module_binding env deps node in Ok deps
  and module_type_binding env deps node =
    let node = unwrap_module_type node in
    match node_kind node with
    | Syntax_kind.PATH_MODULE_TYPE -> Ok (add_parent_segments env deps (path_segments node), Env.bound)
    | Syntax_kind.SIGNATURE_MODULE_TYPE ->
        let* (deps, _, bindings) = collect_signature_items_in env deps node in Ok (deps, Env.make_node bindings)
    ...
    | Syntax_kind.TYPEOF_MODULE_TYPE -> (
      match first_child_node_matching node ~matches:(
        fun kind -> is_module_expr_kind kind || Syntax_kind.(kind = MODULE_EXPR)
      ) with
      | Some module_expr -> module_binding env deps module_expr
      | None -> Ok (deps, Env.bound)
    )
    | Syntax_kind.FUNCTOR_MODULE_TYPE | Syntax_kind.WITH_MODULE_TYPE | Syntax_kind.OPAQUE_MODULE_TYPE ->
        let* deps = collect_node env deps node in Ok (deps, Env.bound)
    | _ ->
        let* deps = collect_node env deps node in Ok (deps, Env.bound)
  and collect_module_type env deps node =
    let* (deps, _) = module_type_binding env deps node in Ok deps
  and collect_functor_head env deps member =
    ...
    let rec path_after_colon index acc =
      if index >= stop || Member.child_token_kind_is member index Syntax_kind.RPAREN then
        ...
        | Some token when Syntax_kind.(token_kind token = IDENT) -> path_after_colon (index + 1) (A.Token.text token :: acc)
    ...
    let rec scan index env deps =
      if index >= stop then
        ...
        if Member.child_token_kind_is member index Syntax_kind.LPAREN then
          let name =
          ...
          let deps =
            let rec find_colon i =
              if i >= stop || Member.child_token_kind_is member i Syntax_kind.RPAREN then
                ...
                if Member.child_token_kind_is member i Syntax_kind.COLON then
    ...
    if A.ModuleDeclaration.is_recursive node then
      ...
      let* deps =
        A.ModuleDeclaration.fold_members node (Ok deps)
          (
            fun acc member ->
              let* deps = acc in collect_module_member_rhs env deps member
          )
      in
      ...
      A.ModuleDeclaration.fold_members node (Ok (deps, env, bindings))
        (
          fun acc member ->
            let* (deps, env, bindings) = acc
            in
            let* (deps, binding) = module_member_binding env deps member
            in
            match module_member_name member with
            | Some name ->
                let env = Env.add name binding env in
                let bindings = Env.add name binding bindings in Ok (deps, env, bindings)
  ...
  and collect_module_type_decl env deps node =
    ...
    | Some body -> (
      match first_child_node_matching body ~matches:(
  ...
  and include_signature_binding env deps node =
    ...
    | None -> (
      ...
      | None -> (
        ...
        | None ->
            let deps = collect_direct_module_refs_between env deps node 1 count in Ok (deps, Env.bound)
      )
    )
  and collect_open_decl env deps node =
    let count = A.Node.child_count node in
    match direct_path_between node 1 count with
    | Some (head :: _ as segments) when is_module_head head -> Ok (open_alias ~fallback:true env deps segments)
    | Some _ -> Ok (deps, env)
    | None ->
        let deps = collect_direct_module_refs_between env deps node 1 count in Ok (deps, env)
  and collect_include_structure env deps node =
    let* (deps, binding) = include_structure_binding env deps node
    in
    let deps = add_names deps (Env.collect_free binding) in Ok (deps, Env.merge_children env binding)
  and collect_include_signature env deps node =
    let* (deps, binding) = include_signature_binding env deps node
    in
    let deps = add_names deps (Env.top_free binding) in Ok (deps, Env.merge_children env binding)
  and collect_structure_item env deps bindings item =
    match first_child_node_matching item ~matches:(
      fun _ -> true
    ) with
    | Some decl when node_kind_is decl Syntax_kind.MODULE_DECL -> collect_module_decl env deps bindings decl
    ...
    | Some decl ->
        let* deps = collect_node env deps decl in Ok (deps, env, bindings)
    | None -> Ok (deps, env, bindings)
  ...
  let finalize_impl = fun env impl ->
    let* (deps, env, exports) = collect_structure_binding env (DepSet.empty ()) impl
    in
    let deps = add_names deps (Env.collect_free (Env.make_node exports)) in Ok (deps, env, exports)

  let finalize_intf = fun env intf ->
    let* (deps, env, exports) = collect_signature_binding env (DepSet.empty ()) intf in Ok (deps, env, exports)
