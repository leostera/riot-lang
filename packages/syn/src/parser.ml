...
let is_parenthesized_functor_type_start = fun parser ->
  let starts_with_parameter_name =
    ...
    | _ -> false
  in
  let rec scan = fun offset depth saw_colon ->
    match (peek_n parser offset).Token.kind with
    | Token.EOF -> false
...
let starts_with_module_type_keyword = fun parser -> peek_kind parser = Token.Keyword Keyword.Module
&& (peek_n parser 1).Token.kind = Token.Keyword Keyword.Type

let starts_with_module_type_of_keywords = fun parser -> peek_kind parser = Token.Keyword Keyword.Module
&& (peek_n parser 1).Token.kind = Token.Keyword Keyword.Type
&& (peek_n parser 2).Token.kind = Token.Keyword Keyword.Of

let starts_with_class_type_keyword = fun parser -> peek_kind parser = Token.Keyword Keyword.Class
&& (peek_n parser 1).Token.kind = Token.Keyword Keyword.Type
...
let parse_parens = fun parser content_parser ->
  ...
  let open_paren =
    ...
    parser
    (Token.OpenDelim Token.Paren)
    (fun found -> Diagnostic.unclosed_delimiter
    ~opener:"("
    ~found
...
and parse_tuple_type = fun parser ->
  ...
  if peek_kind parser = Token.Star then
    let rec collect_tuple_types = fun acc ->
      if peek_kind parser = Token.Star then
...
and parse_parametric_type = fun parser ->
  let rec parse_with_arg = fun arg_type ->
    ...
    | Token.Ident _ ->
        ...
        let rec parse_type_path = fun acc ->
          match peek_kind parser with
          | Token.Ident _ ->
              let ident = consume parser in
              let trivia = consume_trivia parser in
      ...
      (
        ...
        | _ ->
            ...
            if peek_kind parser = Token.Comma then
              let rec parse_type_args = fun acc ->
                if peek_kind parser = Token.Comma then
                  let comma = consume parser in
                  let trivia_after_comma = consume_trivia parser in
...
and parse_module_type_constraints = fun parser ->
  let missing_constraint_rhs_boundary = fun parser ->
    match peek_kind parser with
    | Token.EOF
    ...
    | _ -> false
  in
  let rec parse_constraints = fun acc ->
    match peek_kind parser with
    | Token.Keyword Keyword.Type ->
        let type_kw = consume parser in
        let trivia_after_type = consume_trivia parser in
...
and parse_poly_variant_type = fun parser ->
  ...
  | Token.OpenDelim Token.Bracket ->
      ...
      let push_children = fun acc children ->
      ...
      let rec parse_fields = fun () ->
        ...
        | Token.Ident _ ->
            ...
            let colon =
              ...
              parser
              Token.Colon
              (fun found -> Diagnostic.record_field_missing_colon
...
and parse_labeled_tuple_pattern_element = fun parser ->
  let invalid_pattern = fun found ->
    Diagnostic.invalid_pattern ~found ~text:(token_text parser found) ~span:(current_span parser) in
  let tilde =
...
and parse_tuple_pattern_or_as = fun parser ->
  ...
  if peek_kind parser = Token.Comma then
    ...
    let rec parse_tuple_rest = fun acc ->
      match peek_kind parser with
      | Token.Comma ->
          let comma = consume parser in
          let trivia_after_comma = consume_trivia parser in
...
and parse_ident_or_constructor_pattern = fun parser ->
  ...
  if ident_starts_uppercase text then
    let make_constructor_pattern = fun path_segments trivia_after_path ->
    ...
    in
    let rec collect_path_segments = fun acc ->
      match peek_kind parser with
      | Token.Dot -> (
...
and parse_tuple_pattern = fun parser open_paren trivia_after_open first_pat trivia_after_first ->
  ...
  let rec parse_rest_elements = fun acc_patterns acc_commas_trivia ->
    if peek_kind parser = Token.Comma then
      let comma = consume parser in
      let trivia_after_comma = consume_trivia parser in
...
and parse_operator_pattern = fun parser open_paren trivia_after_open ->
  ...
  let rec collect_nested = fun close_kind ->
    match peek_kind parser with
  ...
  and collect_operator_tokens = fun () ->
    match peek_kind parser with
    | Token.OpenDelim Token.Bracket ->
        let open_bracket = consume parser in
...
and parse_paren_pattern = fun parser ->
  let open_paren =
    ...
    parser
    (Token.OpenDelim Token.Paren)
    (fun found -> Diagnostic.invalid_pattern
    ~found
    ~text:(token_text parser found)
    ~span:(current_span parser))
  in
...
and parse_qualified_field = fun parser ->
  ...
  let first_ident =
    parse_ident
    parser
    (fun parser found -> Diagnostic.invalid_pattern
    ~found
    ~text:(token_text parser found)
    ~span:(current_span parser))
  in
  ...
  | Token.Quote -> (
      ...
      | Token.Ident _ -> (
          ...
          | Token.Quote when String.length ident_text = 1 ->
              ...
              [ make_token parser quote; make_token parser ident; make_token parser closing_quote;  ]
          | Token.Quote ->
...
and parse_labeled_param = fun parser ->
  ...
  let tilde =
    ...
    parser
    Token.Tilde
    (fun found -> Diagnostic.invalid_expression
    ~found
    ~text:(token_text parser found)
    ~span:(current_span parser))
  in
  ...
  let parse_label_and_pattern = fun parser ->
    let label =
      parse_ident
      parser
      (fun parser found -> Diagnostic.invalid_expression
      ~found
      ~text:(token_text parser found)
      ~span:(current_span parser))
    in
...
and parse_optional_param = fun parser ->
  let parse_default_param_content = fun parser ->
...
and parse_object_type_field = fun parser ->
  ...
  let colon =
    ...
    parser
    Token.Colon
    (fun found -> Diagnostic.invalid_type_expression
    ~found
    ~text:(token_text parser found)
    ~span:(expected_span parser))
  in
...
and parse_postfix_expr = fun parser ->
  ...
  let rec parse_postfix = fun expr ->
    match peek_kind parser with
    | Token.Hash ->
        let hash = consume parser in
        let trivia_after_hash = consume_trivia parser in
    ...
    | Token.Dot -> (
        ...
        | kind when is_custom_index_operator_token kind ->
            if not (has_custom_index_payload_after_dot parser) then
              ...
              let trivia_after_dot = consume_trivia parser in
              let rec collect_op_tokens = fun acc ->
                match peek_kind parser with
                | kind when is_custom_index_operator_token kind ->
                    let tok = consume parser in
  ...
  let rec climb = fun left ->
    ...
    | Some (prec, is_right_assoc) when prec >= min_prec ->
        ...
        if next_is_operator && not next_can_be_prefix then
          ...
          let rec skip_to_recovery = fun consumed_tokens ->
            match peek_kind parser with
            | Token.Keyword Keyword.In
...
and parse_tuple_expr = fun parser ->
  ...
  | Token.Comma ->
      ...
      let rec parse_elements = fun acc ->
        match peek_kind parser with
        | Token.Comma ->
            let comma = consume parser in
            let trivia_after_comma = consume_trivia parser in
...
and parse_function_expr = fun parser ->
  ...
  | Token.Keyword Keyword.Function ->
      ...
      let rec interleave = fun cases trivias acc ->
...
and parse_let_in_expr = fun parser ->
  match peek_kind parser with
  | Token.Keyword Keyword.Let ->
      let parse_missing_in_body = fun parser ->
        match peek_kind parser with
        | Token.EOF
        | Token.Keyword Keyword.End
        | Token.Keyword Keyword.In
        ...
        | None -> (* Check for 'open' keyword - let open Module in expr *)
          (
            ...
            | Token.Keyword Keyword.Open ->
                ...
                parser in
                (* Parse module path *)
                let rec parse_module_path = fun acc ->
                  match peek_kind parser with
                  | Token.Ident _ ->
                      let ident = consume parser in
            ...
            | _ ->
                ...
                in
                let trivia_after_bound = consume_trivia parser in
                let rec parse_and_bindings = fun acc ->
                  match peek_kind parser with
                  | Token.Keyword Keyword.And ->
                      let and_kw = consume parser in
                      let trivia_after_and = consume_trivia parser in
      ...
      let bracket_error_early =
        ...
        | Token.Ident _ ->
            ...
            if next_tok.Token.kind = Token.Lt then
              ...
              let rec skip_bracketed = fun acc ->
                match peek_kind parser with
                | Token.Gt ->
      ...
      | _ ->
            ...
            | None ->
                ...
                let name =
                  ...
                  | Token.Ident _ ->
                      ...
                      let rec parse_path_segments = fun acc ->
                        let trivia = consume_trivia parser in
                        if has_dot_ident_continuation parser then
                          let dot = consume parser in
                          let trivia_after_dot = consume_trivia parser in
          ...
          if Ceibo.Green.kind (Ceibo.Green.Node type_name) = Syntax_kind.ERROR then
            ...
            if eq_children = [] then
              ...
              let trivia_after_first = consume_trivia parser in
              let rec parse_and_types = fun acc ->
                match peek_kind parser with
                | Token.Keyword Keyword.And ->
                    let and_kw = consume parser in
                    let trivia_after_and = consume_trivia parser in
              ...
              if and_decls = [] then
                ...
                ([ Ceibo.Green.Node first_type_decl ] @ tokens_to_green parser trivia_after_first @ and_decls)
    )
  else
    let found_tok = peek parser in
...
and parse_module_type_decl = fun parser ->
  ...
  let eq =
    ...
    parser
    Token.Eq
    (fun found -> Diagnostic.missing_let_binding_equals
    ~found
    ~text:(token_text parser found)
    ~span:(expected_span parser))
  in
...
and parse_module_decl = fun parser ->
  ...
  let rec parse_functor_params = fun acc ->
    if peek_kind parser = Token.OpenDelim Token.Paren then
      let lparen = consume parser in
      let trivia_after_lparen = consume_trivia parser in
  ...
  let trivia_after_first_start = position parser in
  let trivia_after_first = consume_trivia parser in
  let rec parse_and_modules = fun acc ->
    match peek_kind parser with
    | Token.Keyword Keyword.And ->
        let and_kw = consume parser in
        let trivia_after_and = consume_trivia parser in
...
and parse_module_type_expr = fun parser ->
  ...
  let base =
    ...
    | Token.Keyword Keyword.Functor ->
        ...
        let rec parse_functor_type_params = fun acc ->
          if peek_kind parser = Token.OpenDelim Token.Paren then
            let lparen = consume parser in
            let trivia_after_lparen = consume_trivia parser in
...
and parse_module_expr = fun parser ->
  ...
  let base =
    ...
    | Token.Ident name ->
        ...
        if not (ident_starts_uppercase name) then
          ...
          let rec parse_path_rest = fun acc ->
            let trivia = consume_trivia parser in
            if has_dot_ident_continuation parser then
              let dot = consume parser in
              let trivia_after_dot = consume_trivia parser in
  ...
  let rec parse_applications = fun expr ->
    if peek_kind parser = Token.OpenDelim Token.Paren then
      let trivia = consume_trivia parser in
      let lparen = consume parser in
      let trivia_after_lparen = consume_trivia parser in
...
and parse_external_decl = fun parser ->
  ...
  let colon =
    ...
    parser
    Token.Colon
    (fun found -> Diagnostic.missing_external_colon
    ~found
    ~text:(token_text parser found)
    ~span:(expected_span parser))
  in
  ...
  let rec collect_primitives = fun () ->
    match peek_kind parser with
    | Token.Literal (Token.String _) ->
...
and parse_structure_item = fun ?(in_block = false) parser ->
  ...
  | Token.Keyword Keyword.Let ->
      ...
      if starts_let_expr then
        ...
        let rec scan_for_in = fun depth struct_depth delim_depth ->
          match peek_kind parser with
          | Token.EOF ->
              ()
...
and parse = fun ~cst_kind ~parse_item ~source ~tokens ->
  ...
  let same_diagnostic = fun left right ->
...
let parse_interface = fun ~source tokens -> parse
~cst_kind:`Interface
~parse_item:parse_signature_item
~source
~tokens
