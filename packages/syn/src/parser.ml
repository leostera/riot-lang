...
and parse_sequence_expr = fun parser ->
    ...
    then
      (
        ...
        let parts = ref
          ([ Ceibo.Green.Node first ] @ tokens_to_green parser trivia_before_first_semi) in
        let rec parse_rest () =
        ...
        if is_extension then
          ...
          let rec parse_elements () =
            ...
            | Token.Semi ->
                ...
                let trivia_after_semi = consume_trivia parser in
                if
                  peek_kind parser = Token.CloseDelim Token.Bracket || peek_kind parser = Token.EOF
                then
...
and parse_record_field = fun parser ->
    ...
    if peek_kind parser = Token.Eq then
      ...
      make_node
        Syntax_kind.RECORD_FIELD
        (field_name_parts @ tokens_to_green parser trivia_after_name)
