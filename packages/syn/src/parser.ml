...
let green_expr_is_module_path_like = fun expr ->
  ...
  let rec loop =
    ...
    | ident :: "." :: rest ->
        ident_starts_uppercase ident && loop rest
    | _ ->
        false
  in
...
let is_custom_index_operator_token =
  function
  | Token.At
  | Token.Question
  | Token.Percent
...
let is_custom_index_open_delim =
  function
...
let has_custom_index_payload_after_dot = fun parser ->
  let rec loop offset =
    ...
    | kind when is_custom_index_operator_token kind ->
        loop (offset + 1)
    | kind ->
        offset > 0 && is_custom_index_open_delim kind
  in
  loop 0
  ...
  let rec scan offset depth saw_colon =
    ...
    | Token.CloseDelim Token.Paren ->
        if depth = 0 then
          ...
          scan (offset + 1) (depth - 1) saw_colon
    | Token.Colon when depth = 0 ->
        scan (offset + 1) depth true
    | _ ->
        scan (offset + 1) depth saw_colon
  in
...
let is_parenthesized_module_type_expr_start = fun parser ->
  let rec scan offset =
    ...
    | Token.OpenDelim Token.Paren ->
        scan (offset + 1)
    | Token.Keyword Keyword.Module ->
        (peek_n parser (offset + 1)).Token.kind = Token.Keyword Keyword.Type
...
let consume_trivia = fun parser ->
  Token_cursor.consume_leading_trivia parser.cursor

let peek_trivia = fun parser ->
  Token_cursor.peek_leading_trivia parser.cursor
  |> List.map Token.trivia_to_token
  ...
  |> List.exists
       (fun (trivia : Token.trivia) ->
         ...
         | Token.WhitespaceTrivia ->
             String.contains (Token_cursor.view parser.cursor trivia.Token.span) "\n"
         | _ ->
             false)
...
let make_token = fun parser token ->
  ...
  let width = String.length text in
  let leading_trivia =
    List.map (green_trivia_of_token_trivia parser) token.Token.leading_trivia
  in
  let green_token =
...
and parse_class_type_params parser =
  match peek_kind parser with
  | Token.OpenDelim Token.Bracket ->
      ...
      let rec parse_params acc =
        let param =
          match peek_kind parser with
          | Token.Quote | Token.Underscore | Token.Plus | Token.Minus | Token.Bang ->
              parse_type_param parser
          | _ ->
              let found = peek parser in
              let text = token_text parser found in
              let diagnostic =
                Diagnostic.invalid_type_parameter ~text ~found ~text_found:text
                  ~span:(current_span parser)
              in
              make_error_node parser ~diagnostic ~consumed_tokens:[]
        in
        ...
        let acc =
          acc @ [ Ceibo.Green.Node param ] @ tokens_to_green parser trivia_after_param
        in
        match peek_kind parser with
        | Token.Comma ->
            let comma = consume parser in
...
and parse_typexpr parser =
  ...
  | Token.Quote ->
      ...
      if !might_be_poly && !type_var_count > 0 then parse_poly_type parser
      else parse_alias_type parser
  | Token.Keyword Keyword.Type ->
...
and parse_alias_type parser =
  ...
  | Token.Keyword Keyword.As ->
      ...
      let alias =
        match peek_kind parser with
        | Token.Quote | Token.Underscore -> parse_primary_type parser
        | _ ->
            let found_tok = peek parser in
            let diagnostic =
      ...
      make_node Syntax_kind.TYPE_ALIAS
...
and parse_primary_type parser =
  ...
  | Token.OpenDelim Token.Bracket ->
      ...
      else parse_poly_variant_type parser
  | Token.OpenDelim Token.Paren ->
      ...
      | Token.Keyword Keyword.Module ->
          ...
          let constraint_children =
            if peek_kind parser = Token.Keyword Keyword.With then
              ...
              let found = peek parser in
              let diagnostic =
                Diagnostic.missing_with_keyword ~found
                  ~text:(token_text parser found)
                  ~span:(point_span found.Token.span.start)
              in
              report_diagnostic parser diagnostic;
              parse_module_type_constraints parser
            else []
...
and parse_module_type_constraints parser =
    ...
    | Token.Keyword Keyword.Type ->
        ...
        let eq_children =
          if peek_kind parser = Token.Eq || peek_kind parser = Token.ColonEq then
            ...
            let found = peek parser in
            let diagnostic =
              Diagnostic.missing_type_decl_equals ~found
                ~text:(token_text parser found)
                ~span:(expected_span parser)
            in
            report_diagnostic parser diagnostic;
      ...
      if peek_kind parser = Token.Pipe then
        ...
        let extra_children, pat =
          if can_start_pattern parser then ([], parse_tuple_pattern_or_as parser)
            ...
            let diagnostic =
              Diagnostic.or_pattern_double ~found:extra_pipe
                ~text:(token_text parser extra_pipe)
                ~span:extra_pipe.Token.span
            in
            let error_node =
              make_error_node parser ~diagnostic ~consumed_tokens:[ extra_pipe ]
            in
            let extra_children =
              [ Ceibo.Green.Node error_node ]
              @ tokens_to_green parser trivia_after_extra_pipe
            in
            let pat =
              if can_start_pattern parser then parse_tuple_pattern_or_as parser
              else make_node Syntax_kind.ERROR []
            in
            (extra_children, pat))
          else
        ...
        parse_pipe_patterns
          (acc @ pipe_trivia_green @ extra_children @ [ Ceibo.Green.Node pat ])
      else
  ...
  | Token.Minus | Token.Plus | Token.MinusDot | Token.PlusDot ->
      ...
      let signed_literal =
        ...
        | ( (Token.Minus | Token.Plus),
            ...
            make_node Syntax_kind.INT_LITERAL
        ...
        | ( (Token.Minus | Token.Plus | Token.MinusDot | Token.PlusDot),
            ...
            let literal = consume parser in
            make_node Syntax_kind.FLOAT_LITERAL
              ([ make_token parser sign ]
              @ tokens_to_green parser trivia_after_sign
              @ [ make_token parser literal ])
        | _ ->
...
and can_start_pattern parser =
  ...
  | Token.OpenDelim Token.Brace
  | Token.Plus | Token.Minus | Token.PlusDot | Token.MinusDot
  | Token.Keyword Keyword.True
  | Token.Keyword Keyword.False
  | Token.Keyword Keyword.Lazy
  | Token.Keyword Keyword.Exception
...
and can_start_pattern_arg parser =
  match peek_kind parser with
  | Token.Tilde ->
      false
  | _ ->
...
and parse_paren_pattern parser =
  ...
  if peek_kind parser = Token.CloseDelim Token.Paren then
    ...
    let next_kind = (Token_cursor.peek_n parser.cursor 1).Token.kind in
  ...
  (* Check for first-class module pattern:
    ...
    let trivia_after_comma = consume_trivia parser in
    let diagnostic =
      Diagnostic.tuple_pattern_extra_comma ~found:comma
        ~text:(token_text parser comma)
        ~span:comma.Token.span
    in
    let close_paren =
      expect parser (Token.CloseDelim Token.Paren) (fun found ->
          Diagnostic.unclosed_delimiter ~opener:")" ~found
...
and parse_attribute parser =
  ...
  | Token.OpenDelim Token.Bracket ->
      ...
      while (not (should_stop ())) && peek_kind parser != Token.EOF do
        ...
        | Token.CloseDelim Token.Bracket when !nested_brackets > 0 ->
...
and parse_extension_name parser =
  ...
  if peek_kind parser = Token.Percent &&
     match next_kind with Token.Ident _ -> true | _ -> false
  then
...
and parse_optional_param parser =
  ...
  | Token.OpenDelim Token.Paren ->
      ...
      let open_p, content, close_p =
        parse_parens parser parse_default_param_content
      in
...
and parse_fun_expr parser =
  ...
  | Token.Keyword Keyword.Fun ->
      ...
      let ext_nodes, trivia_after_ext, attr_nodes =
        parse_keyword_extension_and_attributes parser
      in
...
and parse_object_method parser =
  ...
  let rec parse_params acc =
    match peek_kind parser with
    | Token.Eq | Token.Colon | Token.EOF | Token.CloseDelim Token.ObjectEnd ->
        acc
    | _ ->
            ...
            if green_expr_is_module_path_like expr then
              ...
              if starts_operator_expr then
                ...
                | Token.CloseDelim Token.Paren ->
                    ...
                    let trivia_after_close = consume_trivia parser in
                    let operator_expr =
                      make_node Syntax_kind.IDENT_EXPR operator_children
                    in
                    let local_open =
  ...
  | Token.LeftArrow | Token.ColonEq ->
    ...
    let right =
      ...
      | Token.CloseDelim Token.ObjectEnd ->
          let diagnostic =
            Diagnostic.missing_binary_operand ~operator:(token_text parser arrow)
              ~side:"right" ~found:(peek parser)
      ...
      | Token.Keyword Keyword.Module ->
          ...
          let module_expr =
            ...
            | Token.Ident name when not (ident_starts_uppercase name) ->
                let ident = consume parser in
                let diagnostic =
                  Diagnostic.invalid_module_name ~found:ident
            ...
            | _ ->
                let diagnostic =
                  Diagnostic.missing_module_expr ~found:(peek parser)
                    ~text:(token_text parser (peek parser))
                    ~span:(expected_span parser)
                in
                make_error_node parser ~diagnostic ~consumed_tokens:[]
        ...
        let rec parse_elements () =
          ...
          | Token.Semi ->
              ...
              then
                ...
                in
                elements :=
                  !elements @ [ Ceibo.Green.Node error_node ]
                  @ tokens_to_green parser trivia_after_semi;
                parse_elements ()
          | _ ->
              ...
              let trivia_after_elem = consume_trivia parser in
              elements :=
                !elements @ [ Ceibo.Green.Node elem ]
                @ tokens_to_green parser trivia_after_elem;
...
and parse_record_expr parser =
  ...
  | Token.OpenDelim Token.Brace ->
      ...
      if peek_kind parser = Token.CloseDelim Token.Brace then
        ...
        let first_expr =
          match peek_kind parser with
          | Token.OpenDelim Token.Brace | Token.OpenDelim Token.Paren ->
              parse_expr parser
...
and parse_let_in_expr parser =
  ...
  | Token.Keyword Keyword.Let ->
      ...
      | Token.Keyword Keyword.Open ->
          ...
          (* Parse body expression *)
          let body_expr =
            if Option.is_some in_kw then parse_expr parser
            else parse_missing_in_body parser
          in
      ...
      let bound_expr =
        if missing_let_in_expr_boundary parser then
          let found_tok = peek parser in
          let diagnostic =
            Diagnostic.missing_let_binding_expr ~found:found_tok
              ~text:(token_text parser found_tok)
              ~span:(expected_span parser)
          in
          make_error_node parser ~diagnostic ~consumed_tokens:[]
      ...
      let rec parse_and_bindings acc =
        ...
        | Token.Keyword Keyword.And ->
            ...
            let trivia_after_ext2 =
              if ext_nodes2 != [] then consume_trivia parser else []
            in
  ...
  if peek_kind parser = Token.Keyword Keyword.Type then (
      ...
      let type_params, trivia_after_params =
        ...
        | None -> (
            ...
            | Token.Ident name
              when String.length name > 0 && String.get name 0 = '_' ->
                let tok = consume parser in
                let diagnostic =
                  Diagnostic.invalid_type_parameter ~text:name ~found:tok
                    ~text_found:name ~span:(current_span parser)
                in
                let error_node =
                  make_error_node parser ~diagnostic ~consumed_tokens:[ tok ]
                in
            ...
            | Token.Bang ->
                ...
                let valid_after_bang =
                  next_kind = Token.Quote || next_kind = Token.Underscore
                  || next_kind = Token.Plus || next_kind = Token.Minus
                in
                if valid_after_bang then parse_type_params parser
                else
                  let tok = consume parser in
                  let text = token_text parser tok in
                  let diagnostic =
            ...
            if eq_children = [] then
              ...
              let extensible_children =
                ...
                | Token.DotDot ->
                    let dotdot = consume parser in
                    [ Ceibo.Green.Node
                        (make_node Syntax_kind.TYPE_EXTENSIBLE
                           [ make_token parser dotdot ]) ]
                | _ ->
...
and parse_module_type_decl parser =
  ...
  let type_name =
    ...
    | _ ->
        let found = peek parser in
        let diagnostic =
          Diagnostic.missing_module_type_name ~found
            ~text:(token_text parser found)
            ~span:(expected_span parser)
        in
        report_diagnostic parser diagnostic;
  ...
  let type_expr =
    if can_start_module_type_expr parser then
      ...
      let diagnostic =
        Diagnostic.missing_module_type_expr ~found:(peek parser)
          ~text:(token_text parser (peek parser))
          ~span:(expected_span parser)
      in
      make_error_node parser ~diagnostic ~consumed_tokens:[]
...
and parse_module_decl parser =
  ...
  let module_expr_parts =
    if peek_kind parser = Token.Eq || peek_kind parser = Token.ColonEq then
      ...
      let found_tok = peek parser in
      let diagnostic =
        Diagnostic.missing_module_decl_equals ~found:found_tok
          ~text:(token_text parser found_tok)
          ~span:(expected_span parser)
      in
      report_diagnostic parser diagnostic;
      let module_expr = parse_module_expr parser in
      [ Ceibo.Green.Node (make_node Syntax_kind.ERROR []) ]
      @ [ Ceibo.Green.Node module_expr ]
    else
  ...
  let rec parse_and_modules acc =
    ...
    | Token.Keyword Keyword.And ->
        ...
        in
        let module_decl2 =
          make_node Syntax_kind.MODULE_DECL
            (attr_nodes2
  ...
  let with_wrapped =
    ...
    if peek_kind parser = Token.Keyword Keyword.With then
      ...
      make_node Syntax_kind.MODULE_TYPE_EXPR
        ([ Ceibo.Green.Node base_with_attrs ]
  ...
  let base =
    ...
    | Token.OpenDelim Token.Paren ->
        ...
        let inner =
          ...
          in
            ...
            if is_parenthesized_module_type_expr_start parser then
              parse_module_type_expr parser
            else parse_module_expr parser
        in
        let trivia_before_rparen = consume_trivia parser in
        let rparen =
          expect parser (Token.CloseDelim Token.Paren) (fun found ->
    ...
    | Token.Ident name ->
        ...
        if not (ident_starts_uppercase name) then
          let diagnostic =
            Diagnostic.invalid_module_name ~found:first_ident
              ~text:(token_text parser first_ident)
              ~span:first_ident.Token.span
          in
...
and parse_struct_expr parser =
  ...
  let end_kw =
    expect parser (Token.CloseDelim Token.StructEnd) (fun found ->
        Diagnostic.unclosed_delimiter ~opener:"struct" ~found
          ~text:(token_text parser found)
          ~span:(expected_span parser))
  in
...
and parse_sig_expr parser =
  ...
  let end_kw =
    expect parser (Token.CloseDelim Token.SigEnd) (fun found ->
        Diagnostic.unclosed_delimiter ~opener:"sig" ~found
          ~text:(token_text parser found)
          ~span:(expected_span parser))
  in
...
and parse_external_decl parser =
  ...
  let colon =
    expect parser Token.Colon (fun found ->
        Diagnostic.missing_external_colon ~found
          ~text:(token_text parser found)
          ~span:(expected_span parser))
  in
  let trivia_after_colon = consume_trivia parser in
...
and parse_exception_decl parser =
  ...
  let name =
    ...
    | _ ->
        let found = peek parser in
        let diagnostic =
          Diagnostic.missing_exception_name ~found
            ~text:(token_text parser found)
            ~span:(expected_span parser)
        in
        report_diagnostic parser diagnostic;
...
and parse_ident_like parser =
  ...
  | Token.Unknown '\\' ->
      ...
      in
      make_node Syntax_kind.IDENT_EXPR
        ([ make_token parser slash ] @ hash_children @ name_children)
  | _ ->
      let found = peek parser in
      let diagnostic =
        Diagnostic.invalid_expression ~found ~text:(token_text parser found)
...
and parse_class_decl parser =
  ...
  in
  make_node Syntax_kind.CLASS_DECL
...
and parse_class_type_decl parser =
  let class_kw = consume parser in
  let trivia_after_class = consume_trivia parser in
  let type_kw = consume parser in
  let trivia_after_type = consume_trivia parser in
  ...
  let body_children =
    ...
    | _ ->
        ...
        [ Ceibo.Green.Node (make_error_node parser ~diagnostic ~consumed_tokens:[]) ]
  in
  make_node Syntax_kind.CLASS_TYPE_DECL
    ([ make_token parser class_kw ]
    @ tokens_to_green parser trivia_after_class
    @ [ make_token parser type_kw ]
    @ tokens_to_green parser trivia_after_type
...
and parse_open_stmt ~signature parser =
  ...
  let rec parse_module_path () =
    ...
    | _ ->
        let found = peek parser in
        let diagnostic =
          Diagnostic.missing_module_path ~found
            ~text:(token_text parser found)
            ~span:(expected_span parser)
        in
        report_diagnostic parser diagnostic;
...
and parse_structure_item ?(in_block = false) parser =
  ...
  | Token.Keyword Keyword.Class ->
      if starts_with_class_type_keyword parser then parse_class_type_decl parser
      else parse_class_decl parser
...
and parse ~cst_kind ~parse_item ~source ~tokens =
  ...
  let dedupe_diagnostics diagnostics =
    let rec loop prev acc = function
      ...
      | diag :: rest -> (
          match prev with
          | Some previous when same_diagnostic previous diag ->
              loop prev acc rest
          | _ ->
              loop (Some diag) (diag :: acc) rest)
    in
  ...
  { source; tokens; kind = cst_kind; tree; diagnostics }
