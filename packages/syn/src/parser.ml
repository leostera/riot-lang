...
let create = fun source ->
  let token_stream = Lexer.tokenize source |> Raw_token.of_lexer_tokens ~source in
  ...
  let events = Syntax_tree.Builder.create ~source ~token_stream ~event_capacity:(Int.max 32 ((significant_len * 3) + (raw_len / 4))) ~diagnostic_capacity:8 () in
  {
    source;
    token_stream;
    raw_tokens;
...
let checkpoint = fun (p: parser) -> { saved_pos = p.pos; saved_last_eof_unclosed_delimiter_offset = p.last_eof_unclosed_delimiter_offset; builder_checkpoint = Syntax_tree.Builder.checkpoint p.events }
...
let significant_raw_at = fun p index ->
  ...
  if count = 0 then
    ...
    if index < 0 then
      ...
      else Vector.get_unchecked p.significant_tokens ~at:index
...
let text_between = fun p ~start ~end_ ->
  ...
  if len <= 0 then
    ""
  else Slice.sub_unchecked p.source ~off:start ~len |> Slice.to_string

let raw_char_is = fun p raw ~offset expected ->
  let index = raw.Raw_token.span.Ceibo.Span.start + offset in
  raw.Raw_token.span.Ceibo.Span.start >= 0 && index < raw.Raw_token.span.Ceibo.Span.end_ && index < Slice.length p.source && try Slice.get_unchecked p.source ~at:index = expected with
  ...
  width = len && start >= 0 && end_ <= Slice.length p.source && try
    let rec loop index =
      if index >= len then
        ...
        if Slice.get_unchecked p.source ~at:(start + index) = String.get_unchecked expected ~at:index then
          loop (index + 1)
        else false
    in
    loop 0
...
let legacy_token = fun raw -> { Token.kind = raw.Raw_token.legacy_kind; span = raw.Raw_token.span; leading_trivia = [] }

let found_token = fun p raw: Diagnostic.found_token ->
  let token = legacy_token raw in { kind = Token.to_string token; text = token_text p raw }

let diagnostic_at_current = fun p kind ->
  let raw = current p in Diagnostic.make ~kind ~span:raw.Raw_token.span

let diagnostic_with_current = fun p make ->
  let raw = current p in make ~found:(legacy_token raw) ~text:(token_text p raw) ~span:raw.Raw_token.span

let diagnostic_with_current_at = fun p make span ->
  let raw = current p in make ~found:(legacy_token raw) ~text:(token_text p raw) ~span

let diagnostic_with_raw_at = fun p raw make span -> make ~found:(legacy_token raw) ~text:(token_text p raw) ~span

let diagnostic_with_eof_at = fun p make span -> diagnostic_with_raw_at p (eof p) make span

let diagnostic_with_raw_found_at = fun p raw make span -> make ~found:(legacy_token raw) ~text:(token_text p raw) ~span
...
let missing_let_binding_pattern = fun p -> diagnostic_with_current_at p Diagnostic.missing_let_binding_pattern (zero_span (previous_end_offset p))

let invalid_pattern_at_previous_end = fun p -> diagnostic_with_current_at p Diagnostic.invalid_pattern (zero_span (previous_end_offset p))

let invalid_expression_at_previous_end = fun p -> diagnostic_with_current_at p Diagnostic.invalid_expression (zero_span (previous_end_offset p))

let missing_let_binding_equals = fun p -> diagnostic_with_current_at p Diagnostic.missing_let_binding_equals (zero_span (current_offset p))

let missing_let_binding_equals_at_eof = fun p -> diagnostic_with_eof_at p Diagnostic.missing_let_binding_equals (zero_span (previous_end_offset p))

let missing_let_binding_equals_eof_at_current_offset = fun p -> diagnostic_with_eof_at p Diagnostic.missing_let_binding_equals (zero_span (current_offset p))

let missing_let_binding_equals_at_previous_end = fun p -> diagnostic_with_current_at p Diagnostic.missing_let_binding_equals (zero_span (previous_end_offset p))

let missing_let_binding_expr = fun p -> diagnostic_with_current_at p Diagnostic.missing_let_binding_expr (zero_span (previous_end_offset p))

let missing_type_name = fun p -> diagnostic_with_current p Diagnostic.missing_type_name

let missing_type_name_at_current_offset = fun p -> diagnostic_with_current_at p Diagnostic.missing_type_name (zero_span (current_offset p))

let missing_type_decl_equals = fun p -> diagnostic_with_current_at p Diagnostic.missing_type_decl_equals (zero_span (previous_end_offset p))

let invalid_type_expression_at_previous_end = fun p -> diagnostic_with_current_at p Diagnostic.invalid_type_expression (zero_span (previous_end_offset p))

let bracketed_type_parameters = fun p ~type_name -> diagnostic_with_current p (Diagnostic.bracketed_type_parameters ~type_name)

let malformed_type_variable_at = fun p raw span -> diagnostic_with_raw_found_at p raw Diagnostic.malformed_type_variable span

let unclosed_type_params = fun p -> diagnostic_with_current_at p Diagnostic.unclosed_type_params (zero_span (current_offset p))

let unclosed_type_params_at_previous_end = fun p -> diagnostic_with_current_at p Diagnostic.unclosed_type_params (zero_span (previous_end_offset p))

let invalid_type_parameter_at = fun p raw span ->
  let text = token_text p raw in Diagnostic.invalid_type_parameter ~text ~found:(legacy_token raw) ~text_found:text ~span

let uppercase_type_variable_at = fun p ~quote ~ident ->
  let text = token_text p ident in Diagnostic.uppercase_type_variable ~text ~found:(legacy_token ident) ~text_found:text ~span:(Ceibo.Span.make ~start:quote.Raw_token.span.Ceibo.Span.start ~end_:ident.Raw_token.span.Ceibo.Span.end_)

let unclosed_delimiter = fun p ~opener ->
  diagnostic_with_current_at p (Diagnostic.unclosed_delimiter ~opener)
    (
      if is_eof p then
        zero_span (previous_end_offset p)
      else (current p).Raw_token.span
    )

let unexpected_closing_delimiter = fun p ~delimiter -> diagnostic_with_current p (Diagnostic.unexpected_closing_delimiter ~delimiter)

let missing_binary_operand = fun p ~operator ~side -> diagnostic_with_current_at p (Diagnostic.missing_binary_operand ~operator ~side) (zero_span (current_offset p))

let missing_binary_operand_after_operator = fun p ~operator ~side -> diagnostic_with_current_at p (Diagnostic.missing_binary_operand ~operator ~side) (zero_span (previous_end_offset p))

let consecutive_binary_operators = fun p ~operators -> diagnostic_with_current p (Diagnostic.consecutive_binary_operators ~operators)

let list_double_semicolon = fun p -> diagnostic_with_current p Diagnostic.list_double_semicolon

let if_missing_then = fun p -> diagnostic_with_current_at p Diagnostic.if_missing_then (zero_span (previous_end_offset p))

let match_missing_scrutinee = fun p -> diagnostic_with_current p Diagnostic.match_missing_scrutinee

let match_missing_scrutinee_at_previous_end = fun p -> diagnostic_with_current_at p Diagnostic.match_missing_scrutinee (zero_span (previous_end_offset p))

let match_missing_with = fun p -> diagnostic_with_current_at p Diagnostic.match_missing_with (zero_span (previous_end_offset p))

let match_missing_pattern = fun p -> diagnostic_with_current_at p Diagnostic.match_missing_pattern (zero_span (previous_end_offset p))

let match_guard_missing_expr = fun p -> diagnostic_with_current_at p Diagnostic.match_guard_missing_expr (zero_span (previous_end_offset p))
...
let cons_pattern_missing_tail = fun p -> diagnostic_with_current_at p Diagnostic.cons_pattern_missing_tail (zero_span (previous_end_offset p))

let or_pattern_missing = fun p -> diagnostic_with_current_at p Diagnostic.or_pattern_missing (zero_span (previous_end_offset p))

let or_pattern_double = fun p -> diagnostic_with_current p Diagnostic.or_pattern_double

let mutable_field_missing_name = fun p -> diagnostic_with_current_at p Diagnostic.mutable_field_missing_name (zero_span (previous_end_offset p))

let record_field_missing_colon = fun p ~field_name -> diagnostic_with_current_at p (Diagnostic.record_field_missing_colon ~field_name) (zero_span (previous_end_offset p))

let record_field_missing_type = fun p ~field_name -> diagnostic_with_current_at p (Diagnostic.record_field_missing_type ~field_name) (zero_span (previous_end_offset p))

let missing_module_decl_equals = fun p -> diagnostic_with_current_at p Diagnostic.missing_module_decl_equals (zero_span (previous_end_offset p))

let missing_external_colon = fun p -> diagnostic_with_current_at p Diagnostic.missing_external_colon (zero_span (previous_end_offset p))

let missing_exception_name = fun p -> diagnostic_with_current_at p Diagnostic.missing_exception_name (zero_span (previous_end_offset p))

let missing_module_path = fun p -> diagnostic_with_current_at p Diagnostic.missing_module_path (zero_span (previous_end_offset p))

let missing_module_type_name = fun p -> diagnostic_with_current_at p Diagnostic.missing_module_type_name (zero_span (previous_end_offset p))

let missing_module_type_expr = fun p -> diagnostic_with_current_at p Diagnostic.missing_module_type_expr (zero_span (previous_end_offset p))

let missing_module_expr = fun p -> diagnostic_with_current_at p Diagnostic.missing_module_expr (zero_span (previous_end_offset p))

let missing_with_keyword = fun p -> diagnostic_with_current_at p Diagnostic.missing_with_keyword (zero_span (current_offset p))
...
let unclosed_char_literal = fun p -> Diagnostic.unclosed_char_literal ~text:(token_text p (current p)) ~span:(current p).Raw_token.span
    ...
    (
      ...
      if is_eof p then
        ...
        | _ ->
            ...
            Event.Buffer.error p.events diagnostic
      else Event.Buffer.error p.events diagnostic
    )
...
let starts_structure_item = function
  | Syntax_kind.LET_KW | Syntax_kind.TYPE_KW | Syntax_kind.MODULE_KW | Syntax_kind.OPEN_KW | Syntax_kind.INCLUDE_KW | Syntax_kind.EXTERNAL_KW | Syntax_kind.EXCEPTION_KW | Syntax_kind.CLASS_KW -> true
  | _ -> false
...
let starts_with_typeof_module_expr_at = fun p position -> Syntax_kind.(kind_at_position p position = MODULE_KW && peek_kind_at_position p position 1 = TYPE_KW && peek_kind_at_position p position 2 = OF_KW)

let starts_with_module_type_decl_at = fun p position -> Syntax_kind.(kind_at_position p position = MODULE_KW && peek_kind_at_position p position 1 = TYPE_KW && peek_kind_at_position p position 2 != OF_KW)
...
let starts_signature_item = function
  | Syntax_kind.VAL_KW | Syntax_kind.TYPE_KW | Syntax_kind.MODULE_KW | Syntax_kind.OPEN_KW | Syntax_kind.INCLUDE_KW | Syntax_kind.EXTERNAL_KW | Syntax_kind.EXCEPTION_KW | Syntax_kind.CLASS_KW -> true
  | _ -> false

let starts_signature_item_at_position = fun p position ->
  match kind_at_position p position with
  | Syntax_kind.MODULE_KW -> not (starts_with_typeof_module_expr_at p position) && (starts_with_module_type_decl_at p position || Syntax_kind.(peek_kind_at_position p position 1 != TYPE_KW))
  | kind -> starts_signature_item kind

let starts_structure_item_at = fun p -> starts_structure_item_at_position p p.pos

let starts_signature_item_at = fun p -> starts_signature_item_at_position p p.pos || (at p Syntax_kind.LBRACKET && (Syntax_kind.(peek_kind p 1 = PERCENT) || Syntax_kind.(peek_kind p 1 = AT) || Syntax_kind.(peek_kind p 1 = ATAT)))
...
let leading_trivia_contains_newline = fun p ->
  ...
  let previous_raw =
    if p.pos <= 0 then
      (-1)
    else significant_raw_at p (p.pos - 1)
  in
  let rec loop index =
...
let leading_trivia_contains_newline_at = fun p position ->
  ...
  let previous_raw =
    if position <= 0 then
      (-1)
    else significant_raw_at p (position - 1)
  in
  let rec loop index =
    if index >= current_raw || index >= raw_count p then
      false
  ...
  let rec loop index after_newline =
    if index >= end_ then
      ...
      if char = '\n' then
        ...
        if after_newline && (char = ' ' || char = '\t') then
          true
        else loop (index + 1) after_newline
  in
  if start < 0 || end_ > Slice.length p.source then
    false
  else loop start false
...
let at_item_boundary_at = fun p position ~signature ->
  ...
  if Syntax_kind.(raw.Raw_token.kind = EOF) then
    ...
    if Syntax_kind.(raw.Raw_token.kind = END_KW) then
      true
    else
      if signature && starts_signature_item_at_position p position then
        true
      else
        if not (leading_trivia_contains_newline_at p position) then
          false
        else
          if signature then
            starts_signature_item_at_position p position
          else starts_structure_item_at_position p position
...
let at_item_boundary = fun p ~signature ->
  if is_eof p then
    ...
    if at_end_keyword p then
      true
    else
      if signature && starts_signature_item_at p then
        true
      else
        if not (leading_trivia_contains_newline p) then
          false
        else
          if signature then
            starts_signature_item_at p || (at p Syntax_kind.LBRACKET && (Syntax_kind.(peek_kind p 1 = PERCENT) || Syntax_kind.(peek_kind p 1 = AT) || Syntax_kind.(peek_kind p 1 = ATAT)))
          else starts_structure_item_at p || (at p Syntax_kind.LBRACKET && (Syntax_kind.(peek_kind p 1 = PERCENT) || Syntax_kind.(peek_kind p 1 = AT) || Syntax_kind.(peek_kind p 1 = ATAT)))

let expression_boundary = fun p ~stop_at_item ~stop_at_semi ~stop_at_comma ~signature ->
  match current_kind p with
  | Syntax_kind.EOF | Syntax_kind.AND_KW | Syntax_kind.IN_KW | Syntax_kind.THEN_KW | Syntax_kind.ELSE_KW | Syntax_kind.WITH_KW | Syntax_kind.WHEN_KW | Syntax_kind.ARROW | Syntax_kind.PIPE | Syntax_kind.RPAREN | Syntax_kind.RBRACKET | Syntax_kind.RBRACE | Syntax_kind.BAR_RBRACKET | Syntax_kind.END_KW | Syntax_kind.DONE_KW -> true
...
let trailing_sequence_boundary = fun p ->
  match current_kind p with
  | Syntax_kind.EOF | Syntax_kind.AND_KW | Syntax_kind.IN_KW | Syntax_kind.PIPE | Syntax_kind.RPAREN | Syntax_kind.RBRACKET | Syntax_kind.RBRACE | Syntax_kind.BAR_RBRACKET | Syntax_kind.END_KW | Syntax_kind.DONE_KW -> true
  | _ -> false

let can_start_atom = function
  | Syntax_kind.IDENT | Syntax_kind.INT | Syntax_kind.FLOAT | Syntax_kind.STRING | Syntax_kind.CHAR | Syntax_kind.TRUE_KW | Syntax_kind.FALSE_KW | Syntax_kind.LPAREN | Syntax_kind.BEGIN_KW | Syntax_kind.LBRACKET | Syntax_kind.LBRACKET_BAR | Syntax_kind.LBRACE | Syntax_kind.LET_KW | Syntax_kind.IF_KW | Syntax_kind.MATCH_KW | Syntax_kind.FUN_KW | Syntax_kind.FUNCTION_KW | Syntax_kind.TRY_KW | Syntax_kind.ASSERT_KW | Syntax_kind.LAZY_KW | Syntax_kind.WHILE_KW | Syntax_kind.FOR_KW | Syntax_kind.OBJECT_KW | Syntax_kind.NEW_KW | Syntax_kind.BACKTICK | Syntax_kind.TILDE | Syntax_kind.QUESTION | Syntax_kind.PLUS | Syntax_kind.MINUS | Syntax_kind.PLUSDOT | Syntax_kind.MINUSDOT | Syntax_kind.BANG -> true
  | _ -> false
...
let missing_binding_expr_boundary = fun p ~signature ~top_level ->
  let let_expr_has_in_before_item_boundary () =
    let next_depth depth kind =
      match kind with
      | Syntax_kind.LPAREN | Syntax_kind.LBRACE | Syntax_kind.LBRACKET | Syntax_kind.LBRACKET_BAR | Syntax_kind.BEGIN_KW | Syntax_kind.OBJECT_KW | Syntax_kind.STRUCT_KW | Syntax_kind.SIG_KW -> depth + 1
      | Syntax_kind.RPAREN | Syntax_kind.RBRACE | Syntax_kind.RBRACKET | Syntax_kind.BAR_RBRACKET | Syntax_kind.END_KW when depth > 0 -> depth - 1
      | _ -> depth
    in
    let rec loop position depth =
      ...
      if Syntax_kind.(kind = EOF) then
        ...
        if Int.equal depth 0 && position > p.pos && at_item_boundary_at p position ~signature then
          false
        else
          if Int.equal depth 0 && Syntax_kind.(kind = IN_KW) then
            true
          else loop (position + 1) (next_depth depth kind)
    in
    Syntax_kind.(current_kind p = LET_KW) && loop p.pos 0
  in
  is_eof p || (top_level && at_item_boundary p ~signature && not (can_start_atom (current_kind p) && (leading_trivia_has_post_newline_indent p || let_expr_has_in_before_item_boundary ())))

let can_start_pattern_atom = function
  | Syntax_kind.IDENT | Syntax_kind.UNDERSCORE | Syntax_kind.INT | Syntax_kind.FLOAT | Syntax_kind.STRING | Syntax_kind.CHAR | Syntax_kind.TRUE_KW | Syntax_kind.FALSE_KW | Syntax_kind.LPAREN | Syntax_kind.LBRACKET | Syntax_kind.LBRACKET_BAR | Syntax_kind.LBRACE | Syntax_kind.BACKTICK | Syntax_kind.HASH | Syntax_kind.TILDE | Syntax_kind.QUESTION | Syntax_kind.PLUS | Syntax_kind.MINUS | Syntax_kind.PLUSDOT | Syntax_kind.MINUSDOT | Syntax_kind.LAZY_KW | Syntax_kind.EXCEPTION_KW -> true
  | _ -> false

let sign_token = function
  | Syntax_kind.PLUS | Syntax_kind.MINUS | Syntax_kind.PLUSDOT | Syntax_kind.MINUSDOT -> true
  | _ -> false

let literal_after_sign = function
  | Syntax_kind.INT | Syntax_kind.FLOAT -> true
  | _ -> false

let prefix_operator = function
  | Syntax_kind.PLUS | Syntax_kind.MINUS | Syntax_kind.PLUSDOT | Syntax_kind.MINUSDOT | Syntax_kind.BANG -> true
  | _ -> false
...
let closing_punctuation = function
  | Syntax_kind.RPAREN | Syntax_kind.RBRACKET | Syntax_kind.RBRACE | Syntax_kind.BAR_RBRACKET -> true
  | _ -> false

let operator_pattern_token = function
  | Syntax_kind.PLUS | Syntax_kind.MINUS | Syntax_kind.STAR | Syntax_kind.SLASH | Syntax_kind.PERCENT | Syntax_kind.CARET | Syntax_kind.EQ | Syntax_kind.LT | Syntax_kind.GT | Syntax_kind.LTE | Syntax_kind.GTE | Syntax_kind.NE | Syntax_kind.BANG | Syntax_kind.AMPAMP | Syntax_kind.BARBAR | Syntax_kind.PIPE | Syntax_kind.AMPERSAND | Syntax_kind.AT | Syntax_kind.HASH | Syntax_kind.TILDE | Syntax_kind.QUESTION | Syntax_kind.DOLLAR | Syntax_kind.COLONCOLON | Syntax_kind.COLONEQ | Syntax_kind.ARROW | Syntax_kind.LEFT_ARROW | Syntax_kind.STARSTAR | Syntax_kind.EQEQ | Syntax_kind.BANGEQ | Syntax_kind.ATAT | Syntax_kind.PIPEGT | Syntax_kind.PERCENTGT | Syntax_kind.LTPERCENT | Syntax_kind.PLUSDOT | Syntax_kind.MINUSDOT | Syntax_kind.STARDOT | Syntax_kind.SLASHDOT -> true
  | _ -> false
...
let can_start_poly_variant_payload = fun p ->
  match current_kind p with
  | Syntax_kind.TILDE | Syntax_kind.QUESTION -> symbolic_operator_part (peek_kind p 1)
...
let previous_ident_starts_uppercase = fun p ->
  let raw = previous p in Syntax_kind.(raw.Raw_token.kind = IDENT) && identifier_text_starts_uppercase (token_text p raw)

let parenthesized_operator_start = fun p -> at p Syntax_kind.DOT || (operator_pattern_token (current_kind p) && (Syntax_kind.(peek_kind p 1 = RPAREN) || symbolic_operator_part (peek_kind p 1)))

let index_opener = function
  | Syntax_kind.LPAREN | Syntax_kind.LBRACKET | Syntax_kind.LBRACE -> true
  | _ -> false
...
let symbolic_sequence_followed_by_index_opener = fun p offset ->
  let rec loop offset consumed =
    ...
    if symbolic_operator_part kind then
      loop (offset + 1) true
    else consumed && index_opener kind
  in
  loop offset false
...
let symbolic_sequence_followed_by_closer = fun p offset closer ->
  let rec loop offset consumed =
    ...
    if symbolic_operator_part kind then
      loop (offset + 1) true
    else consumed && Syntax_kind.(kind = closer)
  in
  loop offset false

let binding_operator_suffix = function
  | Syntax_kind.STAR | Syntax_kind.PLUS -> true
  | _ -> false

let binding_operator_keyword = function
  | Syntax_kind.LET_KW | Syntax_kind.AND_KW -> true
  | _ -> false

let infix_binding_power = function
  | Syntax_kind.BARBAR -> Some 10
  | Syntax_kind.AMPAMP -> Some 15
  | Syntax_kind.EQ | Syntax_kind.EQEQ | Syntax_kind.BANGEQ | Syntax_kind.NE | Syntax_kind.LT | Syntax_kind.GT | Syntax_kind.LTE | Syntax_kind.GTE | Syntax_kind.LTPERCENT -> Some 20
  | Syntax_kind.COLONCOLON | Syntax_kind.AT | Syntax_kind.ATAT | Syntax_kind.PIPEGT -> Some 30
  | Syntax_kind.DOLLAR -> Some 30
  | Syntax_kind.AMPERSAND -> Some 35
  | Syntax_kind.PLUS | Syntax_kind.MINUS | Syntax_kind.CARET -> Some 40
  | Syntax_kind.STAR | Syntax_kind.SLASH | Syntax_kind.PERCENT | Syntax_kind.PLUSDOT | Syntax_kind.MINUSDOT | Syntax_kind.STARDOT | Syntax_kind.SLASHDOT | Syntax_kind.PERCENTGT | Syntax_kind.OPERATOR_KW -> Some 50
  | Syntax_kind.STARSTAR -> Some 60
  | _ -> None

let infix_operator_keyword_binding_power = fun p ->
  if current_text_is p "mod" || current_text_is p "land" || current_text_is p "lor" || current_text_is p "lxor" || current_text_is p "lsl" || current_text_is p "lsr" || current_text_is p "asr" then
    Some 50
  else None

let operator_keyword_at = fun p offset ->
  let raw = peek p offset in Syntax_kind.(raw.Raw_token.kind = IDENT) && (raw_text_is p raw "mod" || raw_text_is p raw "land" || raw_text_is p raw "lor" || raw_text_is p raw "lxor" || raw_text_is p raw "lsl" || raw_text_is p raw "lsr" || raw_text_is p raw "asr")
...
let operator_name_start_at = fun p offset ->
  let kind = peek_kind p offset in Syntax_kind.(kind = DOT || kind = OPERATOR_KW) || symbolic_operator_part kind || operator_keyword_at p offset || (binding_operator_keyword kind && binding_operator_suffix (peek_kind p (offset + 1)))
...
let is_attribute_shell = fun p -> at p Syntax_kind.LBRACKET && (Syntax_kind.(peek_kind p 1 = AT) || Syntax_kind.(peek_kind p 1 = ATAT))
...
let consume_extension_payload = fun p ->
  ...
  else consume_balanced_until p ~closer:Syntax_kind.RBRACKET 0
  ...
  if at p Syntax_kind.LPAREN && operator_name_start_at p 1 then
    (
      ...
      let next_depth depth =
        match current_kind p with
        | Syntax_kind.LPAREN | Syntax_kind.LBRACE | Syntax_kind.LBRACKET | Syntax_kind.LBRACKET_BAR -> depth + 1
        | Syntax_kind.RPAREN | Syntax_kind.RBRACE | Syntax_kind.RBRACKET | Syntax_kind.BAR_RBRACKET when depth > 0 -> depth - 1
        | _ -> depth
      in
      let rec loop depth =
        if not (is_eof p || (depth = 0 && (at p Syntax_kind.RPAREN || at p Syntax_kind.COLON || at_item_boundary p ~signature))) then
          (
...
let consume_value_name = fun p ~signature ->
  ...
  else consume_parenthesized_operator_name p ~signature
...
let consume_first_class_module_shell = fun p ->
  ...
  if at p Syntax_kind.RPAREN then
    ...
    if at p Syntax_kind.IDENT then
      ...
      if not (at p Syntax_kind.COLON || at p Syntax_kind.WITH_KW || at p Syntax_kind.TYPE_KW || is_eof p) then
        bump p;
  let parse_type_constraint () =
    ...
    (
      if at p Syntax_kind.IDENT then
        bump p
      else Event.Buffer.error p.events (missing_type_name_at_current_offset p)
    );
    ...
    else Event.Buffer.error p.events (missing_type_decl_equals p)
  in
  if at p Syntax_kind.COLON then
    ...
    if at p Syntax_kind.TYPE_KW then
      ...
      else consume_balanced_until p ~closer:Syntax_kind.RPAREN 0;
  expect_closer p Syntax_kind.RPAREN ~opener:"("
...
let rec parse_expression = fun p ~signature ~stop_at_item ?(stop_at_semi = false) ?(stop_at_comma = false) min_bp ->
  let rec loop lhs =
    if expression_boundary p ~stop_at_item ~stop_at_semi ~stop_at_comma ~signature then
      ...
      if at p Syntax_kind.SEMI && not stop_at_semi && min_bp <= 0 then
        (
        ...
        if at p Syntax_kind.COMMA && min_bp <= 15 then
          (
            let marker = precede p lhs in
            bump p;
          ...
          if at p Syntax_kind.DOT then
            (
              match peek_kind p 1 with
              | Syntax_kind.LPAREN ->
                  if previous_ident_starts_uppercase p then
                    loop (parse_delimited_local_open_expr p lhs ~signature ~opener:Syntax_kind.LPAREN ~closer:Syntax_kind.RPAREN)
                  else
                    (
                      let marker = precede p lhs in
                      bump p;
              ...
              | Syntax_kind.BANG -> loop (parse_dot_bang_expr p lhs ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma)
              | kind when symbolic_operator_part kind && symbolic_sequence_followed_by_index_opener p 1 -> loop (parse_extended_index_expr p lhs ~signature ~stop_at_item)
              | _ -> lhs
            )
          else
            if at p Syntax_kind.HASH then
              if Syntax_kind.(peek_kind p 1 = IDENT) then
                ...
                if min_bp <= 50 then
                  (
                    let marker = precede p lhs in
                    consume_symbolic_operator p;
                    let _rhs = parse_expression p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma 51 in loop (complete p marker Syntax_kind.INFIX_EXPR)
                  )
                else lhs
            else
              if (at p Syntax_kind.LEFT_ARROW || at p Syntax_kind.COLONEQ) && min_bp <= 5 then
                (
                  let marker = precede p lhs in
                  ...
                  else ignore (parse_expression p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma 6);
                  loop (complete p marker Syntax_kind.ASSIGN_EXPR)
                )
              else
                if at p Syntax_kind.COLON && min_bp <= 5 then
                  (
                    let marker = precede p lhs in
                    bump p;
                  ...
                  | Some bp when bp >= min_bp ->
                      ...
                      if expression_boundary p ~stop_at_item ~stop_at_semi ~stop_at_comma ~signature then
                        ...
                        if Option.is_some (current_infix_binding_power p) && not (prefix_operator (current_kind p)) then
                          Event.Buffer.error p.events (consecutive_binary_operators p ~operators:(operator ^ " " ^ token_text p (current p)))
                        else ignore (parse_expression p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma (bp + 1));
                  ...
                  | _ when not (is_attribute_suffix p) && can_start_atom (current_kind p) && min_bp <= application_binding_power ->
                      let marker = precede p lhs in
                      let _argument = parse_expression p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma (application_binding_power + 1) in loop (complete p marker Syntax_kind.APPLY_EXPR)
                  | _ -> lhs
  in
...
and parse_prefix_or_atom = fun p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma ->
  ...
  | kind when prefix_operator kind ->
      if symbolic_operator_part (peek_kind p 1) then
        ...
        (
          let marker = start_node p in
          bump p;
          let _operand = parse_expression p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma 70 in complete p marker Syntax_kind.PREFIX_EXPR
        )
  ...
  | Syntax_kind.BACKTICK -> parse_poly_variant_expr p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma
  | Syntax_kind.TILDE ->
      if symbolic_operator_part (peek_kind p 1) then
        parse_symbolic_prefix_expr p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma
      else parse_label_arg_expr p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma Syntax_kind.LABELED_ARG
  | Syntax_kind.QUESTION ->
      if symbolic_operator_part (peek_kind p 1) then
        parse_symbolic_prefix_expr p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma
      else parse_label_arg_expr p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma Syntax_kind.OPTIONAL_ARG
  ...
  | Syntax_kind.FUNCTION_KW -> parse_function_expr p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma
  | Syntax_kind.TRY_KW -> parse_try_expr p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma
  | Syntax_kind.ASSERT_KW -> parse_unary_keyword_expr p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma Syntax_kind.ASSERT_EXPR
  | Syntax_kind.LAZY_KW -> parse_unary_keyword_expr p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma Syntax_kind.LAZY_EXPR
  ...
  | Syntax_kind.INT | Syntax_kind.FLOAT | Syntax_kind.STRING | Syntax_kind.CHAR | Syntax_kind.TRUE_KW | Syntax_kind.FALSE_KW -> parse_literal_expr p
  | Syntax_kind.QUOTE when Syntax_kind.(peek_kind p 1 = IDENT) ->
      let marker = start_node p in
      let quote = current p in
      bump p;
      let ident = current p in
      let text = text_between p ~start:quote.Raw_token.span.Ceibo.Span.start ~end_:ident.Raw_token.span.Ceibo.Span.end_ in
      Event.Buffer.error p.events (Diagnostic.unclosed_char_literal ~text ~span:(zero_span ident.Raw_token.span.Ceibo.Span.end_));
      bump p;
      complete p marker Syntax_kind.ERROR
  | Syntax_kind.UNKNOWN when raw_starts_with p (current p) '\'' ->
      let marker = start_node p in
      let text = token_text p (current p) in
      Event.Buffer.error p.events
        (
          if text = "''" then
            empty_char_literal p
          else unclosed_char_literal p
        );
      bump p;
      complete p marker Syntax_kind.ERROR
  | Syntax_kind.LPAREN | Syntax_kind.BEGIN_KW -> parse_parenthesized_expr p ~signature ~stop_at_item
  | Syntax_kind.LBRACKET ->
      if is_extension_shell p then
        parse_extension_expr p
      else parse_list_expr p ~signature
  ...
  | kind -> (
    ...
    | None ->
        ...
        if not (is_eof p) then
          bump p
        else Event.Buffer.missing p.events ~kind:Syntax_kind.IDENT ~offset:(current_offset p);
        complete p marker Syntax_kind.ERROR
  )
...
and parse_delimited_local_open_expr = fun p lhs ~signature ~opener:_ ~closer ->
  ...
  if not (at p closer || is_eof p) then
    ...
    else ignore (parse_expression p ~signature ~stop_at_item:false 0);
...
and parse_poly_variant_expr = fun p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma ->
  ...
  bump p;
  expect p Syntax_kind.IDENT (invalid_expression p);
  if (not (expression_boundary p ~stop_at_item ~stop_at_semi ~stop_at_comma ~signature)) && can_start_poly_variant_payload p then
...
and parse_label_arg_expr = fun p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma kind ->
  ...
  if at p Syntax_kind.COLON then
    (
      bump p;
      ignore (parse_expression p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma (application_binding_power + 1))
    );
...
and parse_parenthesized_expr = fun p ~signature ~stop_at_item ->
  ...
  if Syntax_kind.(opener = LPAREN) && at p Syntax_kind.MODULE_KW then
    ...
    if Syntax_kind.(opener = LPAREN) && parenthesized_operator_start p then
      (
        consume_balanced_until p ~closer:Syntax_kind.RPAREN 0;
        expect_closer p Syntax_kind.RPAREN ~opener:"(";
        complete p marker Syntax_kind.PATH_EXPR
...
and parse_record_expr = fun p ~signature ->
  ...
  let rec looks_like_record_field_head offset saw_ident =
    ...
    | Syntax_kind.DOT when saw_ident && Syntax_kind.(peek_kind p (offset + 1) = IDENT) -> looks_like_record_field_head (offset + 2) true
    | Syntax_kind.EQ | Syntax_kind.SEMI | Syntax_kind.RBRACE when saw_ident -> true
    | _ -> false
  in
  let rec update_head_ahead depth offset =
    ...
    if Syntax_kind.(kind = EOF) then
      ...
      if Int.(depth = 0) && Syntax_kind.(kind = WITH_KW) then
        true
      else
        if Int.(depth = 0) && Syntax_kind.(kind = SEMI || kind = RBRACE) then
          false
        else
          match kind with
          | Syntax_kind.LPAREN | Syntax_kind.LBRACKET | Syntax_kind.LBRACE | Syntax_kind.LBRACKET_BAR | Syntax_kind.BEGIN_KW | Syntax_kind.STRUCT_KW | Syntax_kind.SIG_KW | Syntax_kind.OBJECT_KW -> update_head_ahead Int.(depth + 1) (offset + 1)
          | Syntax_kind.RPAREN | Syntax_kind.RBRACKET | Syntax_kind.RBRACE | Syntax_kind.BAR_RBRACKET | Syntax_kind.END_KW -> update_head_ahead (Int.max 0 (depth - 1)) (offset + 1)
  ...
  let parse_record_field () =
    ...
    if at p Syntax_kind.IDENT then
      ignore (parse_path_expr p)
    else ignore (parse_expression p ~signature ~stop_at_item:false ~stop_at_semi:true 0);
    if at p Syntax_kind.EQ then
      (
        bump p;
        ignore (parse_expression p ~signature ~stop_at_item:false ~stop_at_semi:true 0)
  ...
  let kind =
    if at p Syntax_kind.RBRACE || is_eof p then
      ...
      if looks_like_record_field_head 0 false then
        (
          parse_fields ();
          Syntax_kind.RECORD_EXPR
        )
      else
        if update_head_ahead 0 0 then
          (
            ignore (parse_expression p ~signature ~stop_at_item:false ~stop_at_semi:true 0);
...
and parse_let_expr = fun p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma ->
  ...
  if binding_operator_suffix (current_kind p) then
    ...
    if at p Syntax_kind.OPEN_KW then
      (
        bump p;
      ...
      if at p Syntax_kind.MODULE_KW then
        (
          ...
          if at p Syntax_kind.IDENT || at p Syntax_kind.PERCENT then
            bump p
          else expect p Syntax_kind.IDENT (invalid_expression p);
          ...
          if at p Syntax_kind.IN_KW || is_eof p then
            Event.Buffer.error p.events (missing_module_expr p)
          else ignore (parse_module_expr p ~signature);
          if at p Syntax_kind.IN_KW then
            (
              bump p;
              ignore (parse_expression p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma 0)
            )
          else recover_missing_in ();
          complete p marker Syntax_kind.LET_MODULE_EXPR
        )
      else
        if at p Syntax_kind.EXCEPTION_KW then
          (
            bump p;
...
and parse_dot_bang_expr = fun p lhs ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma ->
  ...
  if at p Syntax_kind.LPAREN then
    ...
    if can_start_atom (current_kind p) then
      ignore (parse_prefix_or_atom p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma)
    else Event.Buffer.error p.events (invalid_expression p);
  complete p marker Syntax_kind.LOCAL_OPEN_EXPR
...
and parse_if_expr = fun p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma ->
  ...
  else Event.Buffer.error p.events (if_missing_then p);
  if at p Syntax_kind.ELSE_KW then
    (
      bump p;
...
and parse_match_expr = fun p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma ->
  ...
  if at p Syntax_kind.WITH_KW then
    bump p
  else Event.Buffer.error p.events (match_missing_with p);
...
and parse_match_cases = fun p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma ->
  ...
  in
  let rec loop first =
    if at p Syntax_kind.PIPE || (first && (at p Syntax_kind.ARROW || can_start_pattern_atom (current_kind p))) then
      (
        parse_case ();
        loop false
      )
...
and parse_while_expr = fun p ~signature ~stop_at_item ~stop_at_semi ~stop_at_comma ->
  ...
  ignore (parse_expression p ~signature ~stop_at_item:false ~stop_at_semi:false ~stop_at_comma:false 0);
...
and parse_pattern = fun ?(stop_type_at_arrow = true) p -> ignore (parse_pattern_bp p ~stop_type_at_arrow 0)
    ...
    if is_attribute_suffix p then
      ...
      | Some _ -> (
        ...
        | Syntax_kind.AS_KW ->
            ...
            else Event.Buffer.error p.events (invalid_pattern_at_previous_end p);
        ...
        | Syntax_kind.COLONCOLON ->
            ...
            else Event.Buffer.error p.events (cons_pattern_missing_tail p);
        ...
        | Syntax_kind.PIPE ->
            ...
            if at p Syntax_kind.PIPE then
              ...
              if can_start_pattern_atom (current_kind p) then
                ignore (parse_pattern_no_apply p ~stop_type_at_arrow)
              else Event.Buffer.error p.events (or_pattern_missing p);
...
and parse_pattern_atom = fun p ~stop_type_at_arrow ->
  ...
  | Syntax_kind.INT | Syntax_kind.FLOAT | Syntax_kind.STRING | Syntax_kind.CHAR | Syntax_kind.TRUE_KW | Syntax_kind.FALSE_KW -> parse_single_token_pattern p Syntax_kind.LITERAL_PATTERN
  ...
  | Syntax_kind.EXCEPTION_KW -> parse_unary_pattern p ~stop_type_at_arrow Syntax_kind.EXCEPTION_PATTERN
...
and parse_path_pattern = fun p ->
  ...
  expect p Syntax_kind.IDENT (invalid_pattern p);
  consume_path_segments p;
  if at p Syntax_kind.DOT && (Syntax_kind.(peek_kind p 1 = LPAREN) || Syntax_kind.(peek_kind p 1 = LBRACE) || Syntax_kind.(peek_kind p 1 = LBRACKET) || Syntax_kind.(peek_kind p 1 = LBRACKET_BAR)) then
    (
      bump p;
      (
        match current_kind p with
        ...
        | Syntax_kind.LBRACKET ->
            ...
            else ignore (parse_list_pattern p)
  ...
  else complete p marker Syntax_kind.PATH_PATTERN
    ...
    if at p Syntax_kind.MODULE_KW then
      (
        ...
        if at p Syntax_kind.IDENT || at p Syntax_kind.UNDERSCORE then
          bump p
        else expect p Syntax_kind.IDENT (invalid_pattern p);
        consume_balanced_until p ~closer:Syntax_kind.RPAREN 0;
        expect_closer p Syntax_kind.RPAREN ~opener:"(";
        complete p marker Syntax_kind.FIRST_CLASS_MODULE_PATTERN
      )
    else
      if binding_operator_keyword (current_kind p) && binding_operator_suffix (peek_kind p 1) && Syntax_kind.(peek_kind p 2 = RPAREN) then
        (
          bump p;
          bump p;
          expect_closer p Syntax_kind.RPAREN ~opener:"(";
        ...
        (
          if not (at_closer ()) then
            if at p Syntax_kind.COMMA then
              ...
              if parenthesized_operator_start p then
                ...
                if operator_pattern_token (current_kind p) && Syntax_kind.(peek_kind p 1 = RPAREN) then
                  ignore (parse_single_token_pattern p Syntax_kind.PATH_PATTERN)
                else parse_pattern ~stop_type_at_arrow:false p;
          let rec parse_comma_tail saw_comma =
            if at p Syntax_kind.COMMA then
              (
                bump p;
          ...
          complete p marker
            (
              ...
              else Syntax_kind.PAREN_PATTERN
            )
        )
...
and type_expr_boundary = fun p ~stop_at_arrow ->
  match current_kind p with
  | Syntax_kind.EOF | Syntax_kind.AS_KW | Syntax_kind.AND_KW | Syntax_kind.CONSTRAINT_KW | Syntax_kind.PIPE | Syntax_kind.WHEN_KW | Syntax_kind.EQ | Syntax_kind.COMMA | Syntax_kind.RPAREN | Syntax_kind.RBRACKET | Syntax_kind.RBRACE | Syntax_kind.BAR_RBRACKET | Syntax_kind.END_KW | Syntax_kind.SEMI -> true
  | Syntax_kind.ARROW when stop_at_arrow -> true
  | kind when leading_trivia_contains_newline p && (starts_signature_item kind || starts_structure_item kind) -> true
  | _ -> false
  ...
  if at p Syntax_kind.AS_KW then
    (
      ...
      if type_expr_boundary p ~stop_at_arrow:false then
        Event.Buffer.missing p.events ~kind:Syntax_kind.IDENT ~offset:(current_offset p)
      else ignore (parse_type_atom p ~stop_at_arrow:false)
    );
...
and parse_poly_type = fun p ~stop_at_arrow ->
  ...
  if type_expr_boundary p ~stop_at_arrow then
    Event.Buffer.missing p.events ~kind:Syntax_kind.IDENT ~offset:(current_offset p)
  else ignore (parse_type_bp p ~stop_at_arrow 0);
...
and parse_labeled_type = fun p ->
  ...
  if type_expr_boundary p ~stop_at_arrow:true then
    Event.Buffer.missing p.events ~kind:Syntax_kind.IDENT ~offset:(current_offset p)
  else ignore (parse_type_bp p ~stop_at_arrow:true 0);
...
and parse_type_atom = fun p ~stop_at_arrow ->
  ...
  | Syntax_kind.QUESTION when Syntax_kind.(peek_kind p 1 = IDENT) && Syntax_kind.(peek_kind p 2 = COLON) -> parse_labeled_type p
  | Syntax_kind.IDENT ->
      let marker = start_node p in
      bump p;
      consume_path_segments p;
  ...
  | Syntax_kind.QUOTE ->
      ...
      if at p Syntax_kind.IDENT then
        bump p
      else Event.Buffer.error p.events (malformed_type_variable_at p (current p) (current p).Raw_token.span);
  ...
  let rec loop lhs =
    if is_attribute_suffix p then
      ...
      if type_expr_boundary p ~stop_at_arrow then
        ...
        if (not stop_at_arrow) && at p Syntax_kind.ARROW && min_bp <= 5 then
          (
            let marker = precede p lhs in
            bump p;
            if type_expr_boundary p ~stop_at_arrow then
            ...
            else ignore (parse_type_bp p ~stop_at_arrow 5);
            loop (complete p marker Syntax_kind.ARROW_TYPE)
          )
        else
          if at p Syntax_kind.STAR && min_bp <= 10 then
            (
              let marker = precede p lhs in
              bump p;
              if type_expr_boundary p ~stop_at_arrow then
              ...
              else ignore (parse_type_bp p ~stop_at_arrow 11);
              loop (complete p marker Syntax_kind.TUPLE_TYPE)
            )
          else
            if at p Syntax_kind.IDENT && min_bp <= 30 then
              (
                let marker = precede p lhs in
...
and parse_type_expr = fun p ~allow_leading_poly_type_after_newline ~stop_at_arrow ->
  let marker = start_node p in
  if type_expr_boundary p ~stop_at_arrow && not (allow_leading_poly_type_after_newline && at p Syntax_kind.TYPE_KW) then
    (
      Event.Buffer.missing p.events ~kind:Syntax_kind.IDENT ~offset:(current_offset p);
...
and parse_let_binding = fun p ~signature ~top_level ->
  ...
  else ignore (parse_pattern_no_apply p ~stop_type_at_arrow:false);
  let rec parse_params () =
    if not (at p Syntax_kind.EQ || is_eof p || (top_level && at_item_boundary p ~signature) || not (has_eq_before_item_boundary p ~signature)) then
      if at p Syntax_kind.COLON then
        (
          bump p;
          parse_type_expr p ~allow_leading_poly_type_after_newline:true ~stop_at_arrow:false
  ...
  if at p Syntax_kind.EQ then
    (
      ...
      if missing_binding_expr_boundary p ~signature ~top_level then
        ...
        if (not top_level) && closing_punctuation (current_kind p) then
          (
            let error_marker = start_node p in
            let raw = current p in
            bump p;
            Event.Buffer.error p.events (diagnostic_with_raw_found_at p raw Diagnostic.invalid_expression (zero_span (current_offset p)));
            ignore (complete p error_marker Syntax_kind.ERROR)
          )
        else ignore (parse_expression p ~signature ~stop_at_item:top_level 0)
    )
  else
    (
      ...
      Event.Buffer.error p.events
        (
          if (not (at_item_boundary p ~signature)) && not (has_eq_before_item_boundary p ~signature) then
            ...
            if at_item_boundary p ~signature then
              missing_let_binding_equals_at_previous_end p
            else missing_let_binding_equals p
        );
      if not (expression_boundary p ~stop_at_item:top_level ~stop_at_semi:false ~stop_at_comma:false ~signature) then
...
and raw_ident_is_capitalized = fun p raw ->
  ...
  | Some ('A' .. 'Z') -> true
  | _ -> false
and ident_at_is_capitalized = fun p offset -> raw_ident_is_capitalized p (raw_at p (significant_raw_at p (p.pos + offset)))
and starts_bare_variant_constructor = fun p -> ident_at_is_capitalized p 0 && not (Syntax_kind.(peek_kind p 1 = DOT))
and starts_bare_variant_constructor_at = fun p offset -> ident_at_is_capitalized p offset && not (Syntax_kind.(peek_kind p (offset + 1) = DOT))
and at_type_decl_member_boundary = fun p ~signature -> is_eof p || at_item_boundary p ~signature || at p Syntax_kind.AND_KW || at p Syntax_kind.CONSTRAINT_KW
and at_record_type_field_boundary = fun p -> is_eof p || at p Syntax_kind.SEMI || at p Syntax_kind.RBRACE
and parse_record_type_field = fun p ->
  ...
  let field_name =
    if at p Syntax_kind.IDENT then
      let text = token_text p (current p) in bump p;
      Some text
    else
      (
  ...
  if at p Syntax_kind.COLON then
    ...
    if Option.is_some field_name && not missing_colon then
      ();
  if at_record_type_field_boundary p then
    (
      match field_name with
      | Some field_name when at p Syntax_kind.SEMI || at p Syntax_kind.RBRACE -> Event.Buffer.error p.events (record_field_missing_type p ~field_name)
      | _ -> ()
    )
  else ignore (parse_type_expr p ~allow_leading_poly_type_after_newline:false ~stop_at_arrow:false);
  ...
  (
    ...
    | Syntax_kind.OF_KW ->
        ...
        if at_type_decl_member_boundary p ~signature || at p Syntax_kind.PIPE then
          ...
          if at p Syntax_kind.LBRACE then
...
and parse_variant_type = fun p ~signature ->
  ...
  let rec parse_constructors consumed =
    if at_type_decl_member_boundary p ~signature then
      ...
      if at p Syntax_kind.PIPE || starts_bare_variant_constructor p then
        (
...
and type_decl_body_contains_unsupported_type_syntax = fun p ~signature ->
  ...
  let rec loop position depth =
    ...
    if Syntax_kind.(kind = EOF) then
      ...
      if position > p.pos && depth = 0 && at_item_boundary_at p position ~signature then
        false
      else
        if depth = 0 && Syntax_kind.(kind = AND_KW || kind = CONSTRAINT_KW) then
          false
        else
          if Syntax_kind.(kind = AS_KW || kind = WITH_KW) then
            true
          else loop (position + 1) (next_depth depth kind)
  in
  loop p.pos 0
and type_decl_body_needs_opaque_parse = fun p ~signature ->
  match current_kind p with
  | Syntax_kind.PIPE | Syntax_kind.LBRACE | Syntax_kind.PRIVATE_KW -> true
...
and parse_type_decl_representation = fun p ~signature ->
  if at p Syntax_kind.LBRACE || (at p Syntax_kind.PRIVATE_KW && Syntax_kind.(peek_kind p 1 = LBRACE)) then
    ignore (parse_record_type p)
  else
    if at p Syntax_kind.PIPE || starts_bare_variant_constructor p || (at p Syntax_kind.PRIVATE_KW && (Syntax_kind.(peek_kind p 1 = PIPE) || starts_bare_variant_constructor_at p 1)) then
      ignore (parse_variant_type p ~signature)
    else
      if at p Syntax_kind.DOTDOT then
        bump p
      else consume_until_type_decl_member_boundary p ~signature 0
      ...
      if type_decl_body_needs_opaque_parse p ~signature then
        ...
        (
          ...
          if at p Syntax_kind.EQ then
            ...
            if not (is_eof p || at_item_boundary p ~signature || at p Syntax_kind.AND_KW || at p Syntax_kind.CONSTRAINT_KW) then
              consume_until_item_boundary p ~signature
        )
...
and skip_type_extension_param_modifiers = fun p index ->
  ...
  | Syntax_kind.PLUS | Syntax_kind.MINUS | Syntax_kind.BANG -> skip_type_extension_param_modifiers p (index + 1)
  | _ -> index
...
and skip_type_extension_params = fun p index ->
  ...
  | Syntax_kind.PLUS | Syntax_kind.MINUS | Syntax_kind.BANG | Syntax_kind.QUOTE | Syntax_kind.UNDERSCORE ->
      let next = skip_type_extension_param p index in
      if next > index then
...
and starts_with_type_extension_decl = fun p ->
  if not (at p Syntax_kind.TYPE_KW) then
    ...
    if Syntax_kind.(peek_kind p index = IDENT) then
      let index = skip_type_extension_name_segments p (index + 1) in Syntax_kind.(peek_kind p index = PLUS) && Syntax_kind.(peek_kind p (index + 1) = EQ)
    else false
...
and consume_type_variable = fun p ->
  ...
  if at p Syntax_kind.IDENT then
    (
      ...
      (
        match first with
        | Some first when first >= 'A' && first <= 'Z' -> Event.Buffer.error p.events (uppercase_type_variable_at p ~quote ~ident)
        | _ -> ()
      );
      bump p
    )
...
and consume_type_parameter = fun p ->
  let rec consume_modifiers first =
    match current_kind p with
    | Syntax_kind.PLUS | Syntax_kind.MINUS | Syntax_kind.BANG ->
        let raw = current p in
        bump p;
  ...
  | _ -> (
    ...
    | Some raw -> Event.Buffer.error p.events (invalid_type_parameter_at p raw (current p).Raw_token.span)
    | None -> ()
  )
...
and consume_paren_type_params = fun p ->
  ...
  let rec consume_items () =
    ...
    | Syntax_kind.COMMA ->
        bump p;
        consume_items ()
    | Syntax_kind.QUOTE | Syntax_kind.PLUS | Syntax_kind.MINUS | Syntax_kind.BANG | Syntax_kind.UNDERSCORE ->
...
and consume_type_params = fun p ->
  ...
  | Syntax_kind.IDENT when current_text_is p "__" && Syntax_kind.(peek_kind p 1 = IDENT) ->
      ...
      consume_type_params p
  | Syntax_kind.AT | Syntax_kind.CARET | Syntax_kind.LBRACKET when Syntax_kind.(peek_kind p 1 = IDENT) ->
      consume_invalid_standalone_type_param p;
      consume_type_params p
  | Syntax_kind.LPAREN ->
  ...
  if at p Syntax_kind.PLUS && Syntax_kind.(peek_kind p 1 = EQ) then
    (
      ...
      if is_eof p || at_item_boundary p ~signature then
        ...
        if starts_type_extension_body p then
          ignore (parse_variant_type p ~signature)
        else
          (
            Event.Buffer.error p.events (invalid_type_expression p);
...
and type_decl_tail_depth_after = fun depth kind ->
  match kind with
  | Syntax_kind.LPAREN | Syntax_kind.LBRACE | Syntax_kind.LBRACKET | Syntax_kind.LBRACKET_BAR | Syntax_kind.BEGIN_KW | Syntax_kind.OBJECT_KW | Syntax_kind.STRUCT_KW | Syntax_kind.SIG_KW -> depth + 1
  | Syntax_kind.RPAREN | Syntax_kind.RBRACE | Syntax_kind.RBRACKET | Syntax_kind.BAR_RBRACKET when Int.(depth > 0) -> depth - 1
...
and parse_type_decl_member = fun p ~signature ~first ->
  ...
  if first then
    expect p Syntax_kind.TYPE_KW (invalid_type_expression p)
  else expect p Syntax_kind.AND_KW (invalid_type_expression p);
  ...
  (
    ...
    | Syntax_kind.EQ ->
        ...
        else parse_type_decl_body p ~signature;
        consume_type_decl_constraints p ~signature
    | Syntax_kind.EOF -> ()
    | _ when at_item_boundary p ~signature -> ()
    | Syntax_kind.LBRACKET when is_attribute_shell p -> consume_until_type_decl_member_boundary p ~signature 0
    | _ ->
        Event.Buffer.missing p.events ~kind:Syntax_kind.EQ ~offset:(current_offset p);
...
and module_expr_boundary = fun p ~signature -> is_eof p || at_end_keyword p || at p Syntax_kind.IN_KW || at p Syntax_kind.AND_KW || at_item_boundary p ~signature
and module_type_boundary = fun p ~signature -> is_eof p || at_end_keyword p || at p Syntax_kind.IN_KW || at p Syntax_kind.AND_KW || at p Syntax_kind.EQ || at_item_boundary p ~signature
...
and module_decl_head_boundary = fun p ~signature depth -> Int.equal depth 0 && (is_eof p || at p Syntax_kind.COLON || at p Syntax_kind.EQ || at p Syntax_kind.AND_KW || at p Syntax_kind.STRUCT_KW || at p Syntax_kind.FUNCTOR_KW || at p Syntax_kind.IDENT || at_end_keyword p || at_item_boundary p ~signature)
  ...
  let rec loop () =
    ...
    if not (is_eof p || at_end_keyword p) then
      (
        ...
        else recover_current_as_error p (unexpected_signature_item p);
        loop ()
      )
  in
  loop ();
...
and parse_opaque_module_expr = fun p ~signature ->
  ...
  else consume_until_module_expr_boundary p ~signature;
...
and parse_opaque_module_type = fun p ~signature ->
  ...
  else consume_until_module_type_boundary p ~signature;
...
and parse_parenthesized_module_expr = fun p ~signature ->
  ...
  complete p marker
    (
      ...
      else Syntax_kind.PAREN_MODULE_EXPR
    )
and parse_functor_module_expr = fun p ~signature ->
  ...
  if not (module_expr_boundary p ~signature) then
    ignore (parse_module_expr p ~signature)
  else Event.Buffer.error p.events (missing_module_expr p);
...
and parse_module_expr_atom = fun p ~signature ->
  ...
  | Syntax_kind.IDENT -> parse_module_path_node p Syntax_kind.PATH_MODULE_EXPR (missing_module_expr p)
...
and parse_functor_module_type = fun p ~signature ->
  ...
  if not (module_type_boundary p ~signature) then
    ignore (parse_module_type_expr p ~signature)
  else Event.Buffer.error p.events (missing_module_type_expr p);
...
and parse_module_type_atom = fun p ~signature ->
  ...
  | Syntax_kind.MODULE_KW when starts_with_typeof_module_expr_keyword p -> parse_typeof_module_type p ~signature
  ...
  | Syntax_kind.IDENT -> parse_module_path_node p Syntax_kind.PATH_MODULE_TYPE (missing_module_type_expr p)
...
and consume_module_type_with_constraint = fun p ~signature ->
  let parse_type_path () =
    ...
    else Event.Buffer.error p.events (missing_type_name p);
    complete p marker Syntax_kind.PATH_TYPE
  in
  let parse_type_constraint () =
    let marker = start_node p in
  ...
  let parse_module_constraint () =
    ...
    else Event.Buffer.error p.events (missing_module_decl_equals p);
  ...
  let rec parse_constraint () =
    ...
    if at p Syntax_kind.AND_KW && (Syntax_kind.(peek_kind p 1 = TYPE_KW) || Syntax_kind.(peek_kind p 1 = MODULE_KW)) then
      (
        bump p;
        parse_constraint ()
      )
...
and parse_module_type_decl = fun p ~signature ->
  ...
  (
    if at p Syntax_kind.IDENT then
      bump p
    else Event.Buffer.error p.events (missing_module_type_name p)
  );
  ...
  if at p Syntax_kind.EQ then
    ...
    if is_eof p || at_item_boundary p ~signature then
      ()
    else consume_until_item_boundary p ~signature;
...
and parse_module_decl_member = fun p ~signature ~first ->
  ...
  (
    if at p Syntax_kind.IDENT || at p Syntax_kind.UNDERSCORE then
      bump p
    else Event.Buffer.error p.events (invalid_module_name p)
  );
  ...
  if at p Syntax_kind.COLON then
    (
      ...
      if is_eof p || at p Syntax_kind.EQ || at_item_boundary p ~signature then
        Event.Buffer.error p.events (missing_module_type_expr p)
      else ignore (parse_module_type_expr p ~signature)
    );
  if at p Syntax_kind.EQ then
    (
      bump p;
    ...
    if at p Syntax_kind.STRUCT_KW || at p Syntax_kind.IDENT then
      (
        Event.Buffer.missing p.events ~kind:Syntax_kind.EQ ~offset:(current_offset p);
      ...
      if not (is_eof p || at p Syntax_kind.AND_KW || at_item_boundary p ~signature) then
...
and parse_exception_decl = fun p ~signature ->
  ...
  if at p Syntax_kind.IDENT then
    bump p
  else Event.Buffer.error p.events (missing_exception_name p);
...
and parse_include_decl = fun p ~signature ->
  ...
  let missing_target = is_eof p || at_end_keyword p || (leading_trivia_contains_newline p && at_item_boundary p ~signature) in
...
and parse_opaque_decl = fun p ~signature kind diagnostic -> consume_opaque_until_item_boundary p ~signature kind diagnostic
...
and parse_structure_item = fun p ->
  ...
  (
    match current_kind p with
    | Syntax_kind.LBRACKET when is_extension_shell p -> parse_bracketed_item_shell p Syntax_kind.EXTENSION_ITEM
    | Syntax_kind.LBRACKET when is_attribute_shell p -> parse_bracketed_item_shell p Syntax_kind.ATTRIBUTE_ITEM
    | Syntax_kind.LET_KW when binding_operator_suffix (peek_kind p 1) -> parse_expr_item p ~signature:false
    | Syntax_kind.LET_KW when Syntax_kind.(peek_kind p 1 = OPEN_KW) || Syntax_kind.(peek_kind p 1 = MODULE_KW) -> parse_expr_item p ~signature:false
    | Syntax_kind.LET_KW -> parse_let_decl p ~signature:false
    | Syntax_kind.TYPE_KW when starts_with_type_extension_decl p -> parse_type_extension_decl p ~signature:false
    | Syntax_kind.TYPE_KW -> parse_type_decl p ~signature:false
    | Syntax_kind.MODULE_KW when starts_with_module_type_decl_keyword p -> parse_module_type_decl p ~signature:false
    ...
    | Syntax_kind.CLASS_KW when Syntax_kind.(peek_kind p 1 = TYPE_KW) -> parse_class_type_decl p ~signature:false
    | Syntax_kind.CLASS_KW -> parse_opaque_decl p ~signature:false Syntax_kind.CLASS_DECL (invalid_expression p)
...
and parse_signature_item = fun p ->
  ...
  (
    ...
    | Syntax_kind.TYPE_KW when starts_with_type_extension_decl p -> parse_type_extension_decl p ~signature:true
    | Syntax_kind.TYPE_KW -> parse_type_decl p ~signature:true
    | Syntax_kind.MODULE_KW when starts_with_module_type_decl_keyword p -> parse_module_type_decl p ~signature:true
    ...
    | Syntax_kind.CLASS_KW when Syntax_kind.(peek_kind p 1 = TYPE_KW) -> parse_class_type_decl p ~signature:true
    | Syntax_kind.CLASS_KW -> parse_opaque_decl p ~signature:true Syntax_kind.CLASS_DECL (invalid_expression p)
    | Syntax_kind.OBJECT_KW -> parse_opaque_decl p ~signature:true Syntax_kind.CLASS_DECL (invalid_expression p)
    | _ ->
        Event.Buffer.error p.events (invalid_expression p);
