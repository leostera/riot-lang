...
(** Tokens - Lexical Elements
    ...
    identifiers, literals, operators, delimiters, and trivia (whitespace/comments).
    ...
    The text of the token is not stored directly - use the span to extract
    it from the original source when needed.
*)
...
(** `delimiter_of_keyword kw` returns the delimiter type for keywords that
    act as opening delimiters.
    
    Returns `Some delimiter` for: `begin`, `struct`, `sig`, `object`
    Returns `None` for all other keywords.
    
    Example:
    ```ocaml
    delimiter_of_keyword Keyword.Begin = Some BeginEnd
    delimiter_of_keyword Keyword.Struct = Some StructEnd
    delimiter_of_keyword Keyword.Let = None
    ```
*)
...
(** `show_kind kind` returns a human-readable name for a token kind.
    ...
    Example:
    ```ocaml
    show_kind (Ident "foo") = "identifier"
    show_kind (Literal (Int 42)) = "integer"
    show_kind Plus = "'+'"
    show_kind EOF = "end of file"
    ```
*)
