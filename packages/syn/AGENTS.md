...
43. Keep raw payload anchors behind explicit `CstBuilder` relift helpers when the public CST still stores syntax-node anchors. Consumers that truly need a structured OCaml payload should relift through `CstBuilder`, not reparse source text or duplicate red-tree lifting logic downstream.
44. Keep attribute and extension payloads opaque by default. If a payload is not explicitly committed to a structured OCaml payload variant, preserve its shell-local token slice as `Cst.Payload.Opaque_tokens` instead of guessing that the payload grammar is OCaml.
