...
39. Keep optional parameter structure lossless in `Syn.Cst`: split plain `?x` parameters from defaulted `?(x = 0)` parameters explicitly, and preserve typed binding patterns plus `default_value` on the defaulted shape instead of forcing downstream tools to recover them from parameter source text.
