...
64. Keep `module` bindings split by file context. Implementation-side `module M = ...` forms belong on `ModuleStructure`; interface-side `module M : S` forms belong on `ModuleSignature`; grouped `module rec ... and ...` declarations should live on `and_declarations` instead of a separate recursive wrapper node.
