...
let ansi_inputs = [|0; 9; 42; 103; 196; 231; 255|]
...
let xyz_inputs =
  Array.init
    ~count:(Array.length rgb_inputs)
    ~fn:(fun index -> RGB.to_xyz (Array.get_unchecked rgb_inputs ~at:index))

let grayscale_inputs = [|0; 1; 12; 64; 128; 193; 255|]
...
let next_index = fun state values ->
  let index = state mod Array.length values in
  (index, state + 1)
...
let computed_ansi_to_rgb = fun (`ansi index) ->
  ...
  if index < 16 then
    ...
    `rgb (cube_level red, cube_level green, cube_level blue)
  else
    let shade = 8 + ((index - 232) * 10) in
    `rgb (shade, shade, shade)

let channel_lut =
  Array.init ~count:256
    ~fn:(fun channel ->
