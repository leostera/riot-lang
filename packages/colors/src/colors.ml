...
module Internal = struct
  ...
  module Byte_rgb = struct
    ...
    let clamp = fun (`rgb (red, green, blue)) -> (clamp_channel red, clamp_channel green, clamp_channel blue)

    let normalize = fun rgb ->
      let red, green, blue = clamp rgb in `rgb (red, green, blue)
    ...
    let distance_squared = fun (left_red, left_green, left_blue) (right_red, right_green, right_blue) ->
      ...
      let diff_blue = left_blue - right_blue in (diff_red * diff_red) + (diff_green * diff_green) + (diff_blue * diff_blue)
  end
  ...
  module XYZ_space = struct
    let chromaticity = fun x y z ->
      ...
      if Float.equal denom 0.0 then
        `uv (0.0, 0.0)
      else `uv (4.0 *. x /. denom, 9.0 *. y /. denom)

    let linear_rgb_to_xyz = fun (`lrgb (red, green, blue)) ->
      let x = (0.412_390_799_265_959_48 *. red) +. (0.357_584_339_383_877_96 *. green) +. (0.180_480_788_401_834_29 *. blue) in
      let y = (0.212_639_005_871_510_36 *. red) +. (0.715_168_678_767_755_93 *. green) +. (0.072_192_315_360_733_715 *. blue) in
      let z = (0.019_330_818_715_591_851 *. red) +. (0.119_194_779_794_625_99 *. green) +. (0.950_532_152_249_660_58 *. blue) in `xyz (x, y, z)

    let xyz_to_linear_rgb = fun (`xyz (x, y, z)) ->
      let red = (3.240_969_941_904_521_4 *. x) -. (1.537_383_177_570_093_5 *. y) -. (0.498_610_760_293_003_28 *. z) in
      let green = ((-0.969_243_636_280_879_83) *. x) +. (1.875_967_501_507_720_7 *. y) +. (0.041_555_057_407_175_613 *. z) in
      let blue = (0.055_630_079_696_993_609 *. x) -. (0.203_976_958_888_976_57 *. y) +. (1.056_971_514_242_878_6 *. z) in `lrgb (red, green, blue)
  end
    ...
    let rec find_known_uv = fun reference named ->
      ...
      | { xyz; uv } :: rest ->
          if xyz = reference then
            Some uv
          else find_known_uv reference rest
  ...
  module Transfer_curve = struct
    ...
    let decode_channel = fun channel ->
      ...
      else Float.pow ((normalized +. gamma_offset) /. gamma_scale) 2.4
    ...
    let encode_channel = fun channel ->
      ...
      let encoded =
        ...
        else (gamma_scale *. Float.pow clamped (1.0 /. 2.4)) -. gamma_offset
      in
  ...
  module LUV_space = struct
    ...
    let normalized_lightness_of_y_ratio = fun y_ratio ->
      if y_ratio <= epsilon then
        (kappa *. y_ratio) /. 100.0
      else (1.16 *. Float.cbrt y_ratio) -. 0.16

    let y_of_normalized_lightness = fun ~white_y lightness ->
      if lightness <= 0.08 then
        white_y *. lightness *. 100.0 /. kappa
      else
        let cube_root = (lightness +. 0.16) /. 1.16 in white_y *. cube_root *. cube_root *. cube_root
  end
  ...
  module Hex_rgb = struct
    let digit_char = fun value ->
      ...
      else Char.from_int_unchecked (Char.to_int 'a' + (value - 10))
    ...
    let parse_byte = fun value ~offset ->
      match digit_value (String.get_unchecked value ~at:offset), digit_value (String.get_unchecked value ~at:(offset + 1)) with
    ...
    let of_string = fun value ->
      ...
      if String.length normalized != 6 then
        ...
        match parse_byte normalized ~offset:0, parse_byte normalized ~offset:2, parse_byte normalized ~offset:4 with
    ...
    let to_string = fun rgb ->
      ...
      let byte_to_hex value =
        ...
        let low = value mod 16 in String.make ~len:1 ~char:(digit_char high) ^ String.make ~len:1 ~char:(digit_char low)
      in
  ...
  module Interpolation = struct
    ...
    let inclusive_gradient = fun ~steps ~start ~finish ~blend_unclamped ->
      if steps <= 0 then
        ...
        if steps = 1 then
          [|start|]
        else Array.init ~count:steps ~fn:(
          fun index ->
            if index = 0 then
              start
            else
              if index = steps - 1 then
                ...
                let mix = Float.from_int index /. Float.from_int (steps - 1) in blend_unclamped start finish ~mix
        )
  end
  ...
  module Format = struct
    let color = fun value ->
      ...
      | `rgb (red, green, blue) -> "RGB(" ^ Int.to_string red ^ "," ^ Int.to_string green ^ "," ^ Int.to_string blue ^ ")"
      | `lrgb (red, green, blue) -> "LinearRGB(" ^ Float.to_string red ^ "," ^ Float.to_string green ^ "," ^ Float.to_string blue ^ ")"
      | `xyz (x, y, z) -> "XYZ(" ^ Float.to_string x ^ "," ^ Float.to_string y ^ "," ^ Float.to_string z ^ ")"
      | `luv (lightness, u, v) -> "LUV(" ^ Float.to_string lightness ^ "," ^ Float.to_string u ^ "," ^ Float.to_string v ^ ")"
...
module ANSI = struct
  ...
  let to_rgb = fun (`ansi index) ->
    let clamped = Int.min max_index (Int.max 0 index) in Array.get_unchecked Ansi_table.to_rgb ~at:clamped
  ...
  let nearest = fun rgb ->
    ...
    let rec scan index best_index best_distance =
      if index >= Array.length Ansi_table.to_rgb then
        ...
        else scan (index + 1) best_index best_distance
    in
    let (`rgb (red, green, blue)) = Array.get_unchecked Ansi_table.to_rgb ~at:0 in scan 1 0 (Internal.Byte_rgb.distance_squared source (red, green, blue))
end
...
module Linear_RGB = struct
  let linearize = fun (`rgb (red, green, blue)) -> `lrgb (Internal.Transfer_curve.decode_channel red, Internal.Transfer_curve.decode_channel green, Internal.Transfer_curve.decode_channel blue)

  let delinearize = fun (`lrgb (red, green, blue)) -> `rgb (Internal.Transfer_curve.encode_channel red, Internal.Transfer_curve.encode_channel green, Internal.Transfer_curve.encode_channel blue)
...
module XYZ = struct
  ...
  let to_luv_with_ref = fun (`xyz (_, y, _) as xyz) ~wref ->
    ...
    let v = 13.0 *. lightness *. (v_prime -. white_v) in `luv (lightness, u, v)
...
module LUV = struct
  let distance = fun (`luv (left_lightness, left_u, left_v)) (`luv (right_lightness, right_u, right_v)) ->
    ...
    let diff_v = right_v -. left_v in Float.sqrt ((diff_lightness *. diff_lightness) +. (diff_u *. diff_u) +. (diff_v *. diff_v))
  ...
  let to_xyz_with_ref = fun (`luv (lightness, u, v)) ~wref ->
    ...
    if Float.equal lightness 0.0 then
      ...
      let z = y *. (12.0 -. (3.0 *. u_prime) -. (20.0 *. v_prime)) /. (4.0 *. v_prime) in `xyz (x, y, z)
  ...
  let blend_unclamped = fun (`luv (left_lightness, left_u, left_v) as left) (`luv (right_lightness, right_u, right_v) as right) ~mix ->
    if Float.equal mix 0.0 then
      ...
      if Float.equal mix 1.0 then
        right
      else `luv (Internal.Float_domain.lerp left_lightness right_lightness mix, Internal.Float_domain.lerp left_u right_u mix, Internal.Float_domain.lerp left_v right_v mix)

  let blend = fun left right ~mix -> blend_unclamped left right ~mix:(Internal.Interpolation.clamp_mix mix)

  let gradient = fun start finish ~steps -> Internal.Interpolation.inclusive_gradient ~steps ~start ~finish ~blend_unclamped
end
...
module RGB = struct
  ...
  let contrast_ratio = fun left right ->
    ...
    else (right_luminance +. 0.05) /. (left_luminance +. 0.05)
    ...
    if left = right then
      ...
      if Float.equal mix 0.0 then
        ...
        if Float.equal mix 1.0 then
          ...
          let right_luv = to_luv right in LUV.blend_unclamped left_luv right_luv ~mix |> LUV.to_rgb

  let blend = fun left right ~mix -> blend_unclamped left right ~mix:(Internal.Interpolation.clamp_mix mix)

  let gradient = fun start finish ~steps ->
    let start = Internal.Byte_rgb.normalize start in
    let finish = Internal.Byte_rgb.normalize finish in Internal.Interpolation.inclusive_gradient ~steps ~start ~finish ~blend_unclamped
end
