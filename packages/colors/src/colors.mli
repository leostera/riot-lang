(** Color space conversions and perceptually uniform color blending.
    ...
    perceptually uniform color blending in the LUV color space. It's based on
    the go-colorful library and implements CIE color space mathematics.
    ...
    ```ocaml open Std open Colors

    let () = (* Start with ANSI color *) let ansi_red = `ansi 9 in

    (* Convert to RGB *) let rgb = ANSI.to_rgb ansi_red in println (to_string
    rgb); (* Output: RGB(255,0,0) *)

    (* Convert through color spaces *) let lrgb = Linear_RGB.linearize rgb in
    let xyz = Linear_RGB.to_xyz lrgb in let luv = XYZ.to_luv xyz in

    println (to_string luv); (* Output: LUV(0.5323,1.7512,0.3742) *) ```

    ## Example: Perceptually Uniform Color Blending

    The killer feature: blend colors the way humans perceive them, not the way
    computers calculate them.

    ```ocaml let () = (* Naive RGB blending looks wrong *) let blue = `rgb (0,
    0, 255) in let yellow = `rgb (255, 255, 0) in

    (* This gives you gray - not what you expect! *) let (`rgb (r, g, b)) = `rgb
    ((0 + 255) / 2, (0 + 255) / 2, (255 + 0) / 2) in println (format "Naive
    blend: RGB(%d,%d,%d)" r g b); (* Output: RGB(127,127,127) - Gray! *)

    (* Perceptually uniform blending in LUV space *) let blend = RGB.blend blue
    yellow ~mix:0.5 in println (to_string blend); (* Output: RGB(0,242,54) -
    Green! Matches human perception *) ```

    ## Example: Smooth Color Gradients

    ```ocaml let create_gradient start_color end_color steps = List.init steps
    (fun i -> let mix = Float.of_int i /. Float.of_int (steps - 1) in RGB.blend
    start_color end_color ~mix )

    let () = let gradient = create_gradient (`rgb (255, 0, 0)) (* Red *) (`rgb
    (0, 0, 255)) (* Blue *) 10 in

    List.iter (fun color -> println (to_string color) ) gradient ```

    ## Example: Working with Different White Points

    ```ocaml (* Use custom white reference for specific lighting conditions *)
    let custom_white = `xyz (1.0, 1.0, 1.0) in

    let color = `rgb (200, 150, 100) in let lrgb = Linear_RGB.linearize color in
    let xyz = Linear_RGB.to_xyz lrgb in

    (* Convert to LUV with custom white point *) let luv = XYZ.to_luv_with_ref
    xyz ~wref:custom_white in

    (* Convert back with same reference *) let xyz' = LUV.to_xyz_with_ref luv
    ~wref:custom_white in let lrgb' = XYZ.to_linear_rgb xyz' in let color' =
    Linear_RGB.delinearize lrgb' in
    ...
    ``` ANSI (256 colors) ↓ (lookup table) RGB (0-255) ↓ (gamma correction: 2.4)
    Linear RGB (0.0-1.0) ↓ (matrix transformation) XYZ (device-independent) ↓
    (with white point reference) LUV (perceptually uniform) ```

    Each step is reversible, allowing round-trip conversions.

    ## Understanding Perceptual Uniformity

    In RGB space, equal numeric distances don't correspond to equal perceived
    color differences. For example:
    ...
    correspond to equal perceived color differences. This makes it ideal for
    color blending, interpolation, and calculating color similarity.
...
module ANSI : sig
  ...
  (** Convert an ANSI 256-color palette entry to RGB.
      ...
      Example: ```ocaml let red = ANSI.to_rgb (`ansi 9) in (* Returns: `rgb
      (255, 0, 0) *) ``` *)
end
...
module White_reference : sig
  ...
  (** Standard D65 white point (daylight illuminant at 6504K).
      ...
      to average daylight with correlated color temperature of 6504K.

      Value: (0.95047, 1.00000, 1.08883)

      Reference:
      [Standard illuminants](https://en.wikipedia.org/wiki/Standard_illuminant)
  *)
end
...
module Linear_RGB : sig
  ...
  (** Convert linear RGB to CIE XYZ color space.
      ...
      device-independent and serves as the bridge between RGB and perceptually
...
module LUV : sig
  ...
  (** Blend two colors in LUV space.
      ...
      Blending in LUV space produces perceptually uniform results, meaning the
      perceived color difference changes linearly with `mix`.

      Example: ```ocaml let blue = `luv (0.32, -0.09, -1.13) in let yellow =
      `luv (0.97, 0.07, 0.68) in let green = LUV.blend blue yellow ~mix:0.5 in
...
module RGB : sig
  ...
  (** Blend two RGB colors in perceptually uniform LUV space.
      ...
      automatically: 1. Converts RGB → Linear RGB → XYZ → LUV 2. Blends in LUV
      space 3. Converts back LUV → XYZ → Linear RGB → RGB
      ...
      Example: ```ocaml (* Naive RGB blend: (0+255)/2, (0+255)/2, (255+0)/2 =
      gray *) (* Perceptual LUV blend: produces green as humans perceive it *)
      let blend = RGB.blend (`rgb (0, 0, 255)) (* Blue *) (`rgb (255, 255, 0))
      (* Yellow *) ~mix:0.5 in (* Returns: `rgb (0, 242, 54) - Green! *) ``` *)
end
