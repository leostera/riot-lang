...
let expect_string_equal = fun ~label ~expected ~actual ->
  expect (String.equal expected actual) (label ^ ": expected " ^ expected ^ ", got " ^ actual)
...
let expect_finite = fun ~label value ->
  expect (Float.is_finite value) (label ^ ": expected finite float")
...
let test_to_string_formats_public_variants = fun _ctx ->
  let* () = expect_string_equal
    ...
    ~expected:"ANSI(9)"
    ~actual:(to_string ((`ansi 9:> color))) in
  let* () = expect_string_equal
    ~label:"rgb to_string"
    ~expected:"RGB(255,128,0)"
    ~actual:(to_string ((`rgb (255, 128, 0):> color))) in
  let* () = expect_prefix
    ~label:"linear rgb to_string"
    ~prefix:"LinearRGB("
    ~actual:(to_string ((`lrgb (1.0, 0.5, 0.25):> color))) in
  let* () = expect_prefix
    ~label:"xyz to_string"
    ~prefix:"XYZ("
    ~actual:(to_string ((`xyz (0.1, 0.2, 0.3):> color))) in
  let* () = expect_prefix
    ~label:"luv to_string"
    ~prefix:"LUV("
    ~actual:(to_string ((`luv (0.1, 0.2, 0.3):> color))) in
...
let test_ansi_known_values_and_clamping = fun _ctx ->
  ...
  for_each
    ...
    ~fn:(fun (index, expected) ->
      ANSI.to_rgb (`ansi index) |> expect_rgb_equal ~label:("ansi " ^ Int.to_string index) ~expected)
...
let test_ansi_formula_segments = fun _ctx ->
  let rec check_cube index =
    if index > 231 then
      ...
      let* () = ANSI.to_rgb (`ansi index)
      |> expect_rgb_equal ~label:("ansi cube " ^ Int.to_string index) ~expected in
      check_cube (index + 1)
  in
  let rec check_grayscale index =
    if index > 255 then
      ...
      let* () = ANSI.to_rgb (`ansi index)
      |> expect_rgb_equal ~label:("ansi grayscale " ^ Int.to_string index) ~expected in
    ...
    if index > 255 then
      ...
      | `rgb (r, g, b) ->
          ...
          let* () = expect
            ...
            ("ansi index out of range: " ^ Int.to_string index) in
          loop (index + 1)
  in
  loop 0
...
let test_linear_rgb_known_values = fun _ctx ->
  ...
  |> expect_lrgb_close ~label:"linearize black" ~epsilon:1.e-12 ~expected:(0.0, 0.0, 0.0) in
  let* () = Linear_RGB.linearize (`rgb (255, 255, 255))
  |> expect_lrgb_close ~label:"linearize white" ~epsilon:1.e-12 ~expected:(1.0, 1.0, 1.0) in
  let* () = Linear_RGB.linearize (`rgb (255, 0, 0))
  |> expect_lrgb_close ~label:"linearize red" ~epsilon:1.e-12 ~expected:(1.0, 0.0, 0.0) in
  let* () = Linear_RGB.linearize (`rgb (0, 255, 0))
  |> expect_lrgb_close ~label:"linearize green" ~epsilon:1.e-12 ~expected:(0.0, 1.0, 0.0) in
  let* () = Linear_RGB.linearize (`rgb (0, 0, 255))
  |> expect_lrgb_close ~label:"linearize blue" ~epsilon:1.e-12 ~expected:(0.0, 0.0, 1.0) in
  let* () = expect_float_close
    ~label:"linearize 128"
    ~epsilon:1.e-10
    ~expected:0.215_860_500_113_899_26
    ~actual:(linearize_channel 128) in
  let* () = expect_float_close
    ~label:"linearize 1"
    ~epsilon:1.e-12
    ~expected:0.000_303_526_983_548_837_5
    ~actual:(linearize_channel 1) in
  let* () = expect_float_close
    ~label:"linearize 12"
    ~epsilon:1.e-12
    ~expected:0.003_676_507_324_047_436
    ~actual:(linearize_channel 12) in
  ...
  let* () = expect_int_equal
    ~label:"delinearize mid gray"
    ~expected:128
    ~actual:(delinearize_channel 0.215_860_500_113_899_26) in
  let* () = expect_int_equal
    ~label:"delinearize channel 1"
    ~expected:1
    ~actual:(delinearize_channel 0.000_303_526_983_548_837_5) in
  let* () = expect_int_equal
  ...
  let rec loop channel =
    if channel > 255 then
      ...
      let* () = expect
        ...
        ("linearize out of range for channel " ^ Int.to_string channel) in
      let* () = expect_int_equal
        ~label:("channel roundtrip " ^ Int.to_string channel)
        ~expected:channel
        ~actual:(delinearize_channel linear) in
      loop (channel + 1)
  in
  loop 0

let test_xyz_known_conversions = fun _ctx ->
  let* () = Linear_RGB.to_xyz (`lrgb (0.0, 0.0, 0.0))
  |> expect_xyz_close ~label:"xyz black" ~epsilon:1.e-12 ~expected:(0.0, 0.0, 0.0) in
  let* () = Linear_RGB.to_xyz (`lrgb (1.0, 1.0, 1.0))
  |> expect_xyz_close ~label:"xyz white" ~epsilon:3.e-4 ~expected:(0.950_47, 1.0, 1.088_83) in
  let* () = Linear_RGB.to_xyz (`lrgb (1.0, 0.0, 0.0))
  |> expect_xyz_close
    ~label:"xyz red"
    ~epsilon:1.e-12
    ~expected:(0.412_390_799_265_959_48, 0.212_639_005_871_510_36, 0.019_330_818_715_591_851) in
  let* () = Linear_RGB.to_xyz (`lrgb (0.0, 1.0, 0.0))
  |> expect_xyz_close
    ~label:"xyz green"
    ~epsilon:1.e-12
    ~expected:(0.357_584_339_383_877_96, 0.715_168_678_767_755_93, 0.119_194_779_794_625_99) in
  let* () = Linear_RGB.to_xyz (`lrgb (0.0, 0.0, 1.0))
  |> expect_xyz_close
    ~label:"xyz blue"
    ~epsilon:1.e-12
    ~expected:(0.180_480_788_401_834_29, 0.072_192_315_360_733_715, 0.950_532_152_249_660_58) in
...
let test_uv_and_luv_known_values = fun _ctx ->
  let* () = XYZ.to_uv (`xyz (0.0, 0.0, 0.0))
  |> expect_uv_close ~label:"uv zero" ~epsilon:1.e-12 ~expected:(0.0, 0.0) in
  let* () = XYZ.to_uv White_reference.d65
  |> expect_uv_close
    ~label:"uv d65"
    ~epsilon:1.e-12
    ~expected:(0.197_839_824_821_407_77, 0.468_336_302_932_409_7) in
  let* () = XYZ.to_luv White_reference.d65
  |> expect_luv_close ~label:"luv d65" ~epsilon:1.e-12 ~expected:(1.0, 0.0, 0.0) in
  let* () = XYZ.to_luv_with_ref White_reference.d65 ~wref:White_reference.d65
  |> expect_luv_close ~label:"luv explicit d65" ~epsilon:1.e-12 ~expected:(1.0, 0.0, 0.0) in
...
let test_custom_white_reference_roundtrip_and_validation = fun _ctx ->
  ...
  let* () = LUV.to_xyz_with_ref luv ~wref:custom_white
  |> expect_xyz_close ~label:"custom white roundtrip" ~epsilon:1.e-9 ~expected:(0.2, 0.3, 0.4) in
  let invalid_xyz_ref =
    try
...
let test_xyz_luv_roundtrip_corpus_and_finite_values = fun _ctx ->
  for_each edge_case_colors
    ~fn:(fun rgb ->
      ...
      let* () = LUV.to_xyz luv |> expect_xyz_finite ~label:"luv to xyz finite" in
      let* () =
        LUV.to_xyz luv |> expect_xyz_close ~label:"xyz luv xyz roundtrip" ~epsilon:1.e-7
          ~expected:(
            match xyz with
            | `xyz (x, y, z) -> (x, y, z)
          )
...
let test_rgb_hex_known_values_and_clamping = fun _ctx ->
  ...
  | Error message -> Error ("expected RGB.of_hex whitespace-trimmed input to succeed, got error: "
  ^ message)

let test_rgb_hex_rejects_invalid_inputs = fun _ctx ->
  let invalid_values = [ ""; "#12345"; "#1234567"; "#gg0000"; "xyzxyz"; "#12_345" ] in
  for_each invalid_values
    ~fn:(fun value ->
      match RGB.of_hex value with
      | Ok rgb -> Error ("expected RGB.of_hex to reject "
      ^ value
      ^ ", got "
      ^ to_string ((rgb:> color)))
      | Error _ -> Ok ())
...
let test_blend_behavior_and_ranges = fun _ctx ->
  ...
  let* () = LUV.blend luv_a luv_b ~mix:0.0
  |> expect_luv_close ~label:"luv blend start" ~epsilon:1.e-12 ~expected:(0.2, (-0.4), 0.8) in
  let* () = LUV.blend luv_a luv_b ~mix:1.0
  |> expect_luv_close ~label:"luv blend end" ~epsilon:1.e-12 ~expected:(0.6, 0.2, (-0.4)) in
  let* () = LUV.blend luv_a luv_b ~mix:0.5
  |> expect_luv_close ~label:"luv blend midpoint" ~epsilon:1.e-12 ~expected:(0.4, (-0.1), 0.2) in
  let* () = LUV.blend luv_a luv_b ~mix:(-1.0)
  |> expect_luv_close ~label:"luv blend lower clamp" ~epsilon:1.e-12 ~expected:(0.2, (-0.4), 0.8) in
  let* () = LUV.blend luv_a luv_b ~mix:2.0
  |> expect_luv_close ~label:"luv blend upper clamp" ~epsilon:1.e-12 ~expected:(0.6, 0.2, (-0.4)) in
  let* () = RGB.blend identical identical ~mix:0.25
  |> expect_rgb_equal ~label:"rgb blend identical 0.25" ~expected:(12, 34, 56) in
  let* () = RGB.blend identical identical ~mix:0.5
  |> expect_rgb_equal ~label:"rgb blend identical 0.5" ~expected:(12, 34, 56) in
  let* () = RGB.blend identical identical ~mix:0.75
  |> expect_rgb_equal ~label:"rgb blend identical 0.75" ~expected:(12, 34, 56) in
  let* () = RGB.blend gray_a gray_b ~mix:0.0
  |> expect_rgb_equal ~label:"rgb blend clamp start" ~expected:(32, 32, 32) in
  let* () = RGB.blend gray_a gray_b ~mix:1.0
  |> expect_rgb_equal ~label:"rgb blend clamp end" ~expected:(224, 224, 224) in
  let* () =
            ...
            ~fn:(fun right ->
              for_each mixes
                ~fn:(fun mix ->
                  ...
                  | `rgb (r, g, b) ->
                      let in_range value = value >= 0 && value <= 255 in
                      expect (in_range r && in_range g && in_range b) "expected blended rgb channels to stay within byte range"))
        in
        check_pairs rest
  in
