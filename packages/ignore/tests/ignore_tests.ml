...
let write = fun path text -> Fs.write text path |> Result.map_err ~fn:IO.error_message
...
let path_is_within_skipped_packages_subtree = fun ~root path ->
  ...
  | Error _ -> false
  | Ok rel_path -> Path.components rel_path |> List.map ~fn:Path.to_string |> List.any ~fn:(
    fun component -> List.contains packages_root_skipped_directories ~value:component
  )
...
let find_packages_root = fun () ->
  let rec loop dir =
    ...
    let* has_agents = Fs.exists agents_md |> Result.map_err ~fn:IO.error_message
    in
    let* has_packages_dir = Fs.is_dir packages_dir |> Result.map_err ~fn:IO.error_message
    in
  ...
  in
  let* cwd = Env.current_dir () |> Result.map_err ~fn:(
    fun _ -> "failed to read current dir"
  ) in loop cwd
        ...
        | (label, paths) :: remaining ->
            if baseline = paths then
              ...
              let mismatch =
                ...
                | expected :: _, actual :: _ when not (String.equal expected actual) -> "first mismatch: expected " ^ expected ^ " but got " ^ actual
              ...
              in
              Error (label ^ " did not match " ^ baseline_label ^ " (" ^ mismatch ^ "; baseline_count=" ^ Int.to_string (List.length baseline) ^ "; actual_count=" ^ Int.to_string (List.length paths) ^ ")")
      in
      loop rest
...
let collect_paths = fun walker -> Ignore.Walker.to_list walker |> Result.map ~fn:(List.map ~fn:Fs.Walker.FileItem.path_string) |> Result.map_err ~fn:(
  function
  | Ignore.Walker.File_system { cause; _ } -> IO.error_message cause
  | Ignore.Walker.Invalid_glob { path; line; message; _ } -> Path.to_string path ^ ":" ^ Int.to_string line ^ ": " ^ message
)

let collect_file_paths = fun ~root walker -> Ignore.Walker.to_list walker |> Result.map ~fn:(List.filter_map ~fn:(
  fun (entry: Fs.Walker.FileItem.t) ->
    ...
    if path_is_within_skipped_packages_subtree ~root path then
      ...
      | Fs.Walker.Directory | Fs.Walker.Symlink | Fs.Walker.Other -> None
)) |> Result.map ~fn:sorted |> Result.map_err ~fn:(
  function
  | Ignore.Walker.File_system { cause; _ } -> IO.error_message cause
  | Ignore.Walker.Invalid_glob { path; line; message; _ } -> Path.to_string path ^ ":" ^ Int.to_string line ^ ": " ^ message
)
...
let collect_file_paths_parallel = fun walker ->
  ...
  let seen = ref [] in Ignore.Walker.walk walker ~f:(
    fun entry ->
      match Fs.Walker.FileItem.kind entry with
      | Fs.Walker.File ->
          with_lock lock
            (
              fun () -> seen := Fs.Walker.FileItem.path_string entry :: !seen
            );
          Fs.Walker.Continue
...
let contains_path = fun paths suffix -> List.any paths ~fn:(
  fun path -> String.ends_with ~suffix path
)

let tests =
  [
    Test.case "ignore walker skips hidden directories by default"
      (
        fun _ctx ->
          with_tempdir "ignore_hidden"
            (
              fun root ->
                ...
                let* () = Fs.create_dir_all hidden_dir |> Result.map_err ~fn:IO.error_message
                in
                let* () = write visible "visible"
                in
                let* () = write hidden_file "secret"
                in
                let* walker = Ignore.Walker.create ~roots:[ root ] () |> Result.map_err ~fn:(
                  fun _ -> "walker create failed"
                )
                in
    ...
    Test.case "ignore walker respects root gitignore files"
      (
        fun _ctx ->
          with_tempdir "ignore_gitignore"
            (
              fun root ->
                ...
                let* () = Fs.create_dir_all vendor_dir |> Result.map_err ~fn:IO.error_message
                in
                ...
                in
                let* walker = Ignore.Walker.create ~roots:[ root ] ~hidden:false () |> Result.map_err ~fn:(
                  fun _ -> "walker create failed"
                )
                in
                let* paths = collect_paths walker
    ...
    Test.case "custom ignore files override gitignore files"
      (
        fun _ctx ->
          with_tempdir "ignore_custom"
            (
              fun root ->
                ...
                let* () = write file "keep\n"
                in
                let* walker = Ignore.Walker.create ~roots:[ root ] ~hidden:false ~custom_ignore_filenames:[ ".dockerignore" ] () |> Result.map_err ~fn:(
                  fun _ -> "walker create failed"
                )
                in
                let* paths = collect_paths walker
    ...
    Test.case "override globs keep unmatched directories but filter files"
      (
        fun _ctx ->
          with_tempdir "ignore_overrides"
            (
              fun root ->
                ...
                let* () = Fs.create_dir_all dir |> Result.map_err ~fn:IO.error_message
                in
                let* () = write file_ml "let main = 1\n"
                in
                let* () = write file_txt "notes\n"
                in
                let* walker = Ignore.Walker.create ~roots:[ root ] ~hidden:false ~overrides:[ "*.ml" ] () |> Result.map_err ~fn:(
                  fun _ -> "walker create failed"
                )
                in
                let* paths = collect_paths walker
    ...
    Test.case "parallel ignore walker keeps complete multiple-root file set"
      (
        fun _ctx ->
          with_tempdir "ignore_parallel_multiple_roots"
            (
              fun root ->
                let src_dir = Path.(root / Path.v "src") in
                ...
                let* () = Fs.create_dir_all net_dir |> Result.map_err ~fn:IO.error_message
                in
                let* () = Fs.create_dir_all native_dir |> Result.map_err ~fn:IO.error_message
                in
                let* () = write Path.(net_dir / Path.v "udp_socket.mli") "type t\n"
                in
                let* () = write Path.(net_dir / Path.v "udp_socket.ml") "type t = unit\n"
                in
                let* () = write Path.(net_dir / Path.v "udp_server.mli") "type handler = socket:Udp_socket.t -> bytes -> unit\n"
                in
                let* () = write Path.(net_dir / Path.v "udp_server.ml") "type handler = socket:Udp_socket.t -> bytes -> unit\n"
                in
                ...
                let rec run iteration =
                  if iteration = 0 then
                    ...
                    let* walker = Ignore.Walker.create ~roots:[ src_dir; native_dir ] ~concurrency ~sort:false ~hidden:false () |> Result.map_err ~fn:(
                      fun _ -> "walker create failed"
                    )
                    in
                    let* actual = collect_file_paths_parallel walker
                    in
                    if actual = expected then
                      run (iteration - 1)
                    else Error ("expected parallel ignore walker file set [" ^ String.concat ", " expected ^ "] but got [" ^ String.concat ", " actual ^ "]")
                in
                run 25
            )
      );
    Test.case ~size:Large "ignore walker to_list packages file set is stable across concurrency and sort modes"
      (
        fun _ctx ->
          let* packages_root = find_packages_root ()
          in
          let configs =
            [
          ...
          let rec collect acc = function
            ...
            | (label, concurrency, sort) :: rest ->
                let* walker = Ignore.Walker.create ~roots:[ packages_root ] ~concurrency ~sort () |> Result.map_err ~fn:glob_error_message
                in
                let* paths = collect_file_paths ~root:packages_root walker in collect ((label, paths) :: acc) rest
          in
