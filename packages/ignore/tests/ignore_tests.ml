...
let path_is_within_skipped_packages_subtree = fun ~root path ->
  ...
  | Error _ -> false
  | Ok rel_path -> Path.components rel_path
  |> List.map ~fn:Path.to_string
  |> List.any ~fn:(fun component -> List.contains packages_root_skipped_directories ~value:component)

let find_packages_root = fun () ->
  let rec loop dir =
    let agents_md = Path.(dir / Path.v "AGENTS.md") in
    let packages_dir = Path.(dir / Path.v "packages") in
    let* has_agents = Fs.exists agents_md |> Result.map_err ~fn:IO.error_message in
    let* has_packages_dir = Fs.is_dir packages_dir |> Result.map_err ~fn:IO.error_message in
    if has_agents && has_packages_dir then
      ...
      | Some parent ->
          if Path.equal parent dir then
            ...
            loop parent
  in
  let* cwd = Env.current_dir () |> Result.map_err ~fn:(fun _ -> "failed to read current dir") in
  loop cwd
...
let collect_file_paths = fun ~root walker ->
  Ignore.Walker.to_list walker |> Result.map
    ~fn:(List.filter_map
      ~fn:(fun (entry: Fs.Walker.FileItem.t) ->
        ...
        if path_is_within_skipped_packages_subtree ~root path then
          ...
          | Fs.Walker.Directory
          | Fs.Walker.Symlink
          | Fs.Walker.Other -> None)) |> Result.map ~fn:sorted |> Result.map_err
    ~fn:(
      function
      | Ignore.Walker.File_system { cause; _ } -> IO.error_message cause
      | Ignore.Walker.Invalid_glob { path; line; message; _ } -> Path.to_string path
...
let collect_file_paths_parallel = fun walker ->
  ...
  Ignore.Walker.walk walker
    ~f:(fun entry ->
      ...
      | Fs.Walker.Directory
      | Fs.Walker.Symlink
      | Fs.Walker.Other -> Fs.Walker.Continue) |> Result.map ~fn:(fun () -> !seen |> sorted) |> Result.map_err
    ~fn:(
      function
      | Ignore.Walker.File_system { cause; _ } -> IO.error_message cause
      | Ignore.Walker.Invalid_glob { path; line; message; _ } -> Path.to_string path
...
let contains_path = fun paths suffix ->
  List.any paths ~fn:(fun path -> String.ends_with ~suffix path)
    ...
    (fun _ctx ->
      with_tempdir "ignore_hidden"
        (fun root ->
          ...
          let* () = Fs.create_dir_all hidden_dir |> Result.map_err ~fn:IO.error_message in
          let* () = write visible "visible" in
          let* () = write hidden_file "secret" in
          let* walker = Ignore.Walker.create ~roots:[ root ] ()
          |> Result.map_err ~fn:(fun _ -> "walker create failed") in
    ...
    (fun _ctx ->
      with_tempdir "ignore_gitignore"
        (fun root ->
          ...
          let* () = Fs.create_dir_all vendor_dir |> Result.map_err ~fn:IO.error_message in
    ...
    (fun _ctx ->
      with_tempdir "ignore_overrides"
        (fun root ->
          ...
          let* () = Fs.create_dir_all dir |> Result.map_err ~fn:IO.error_message in
          ...
          Ok ())); Test.case "parallel ignore walker keeps complete multiple-root file set"
    (fun _ctx ->
      with_tempdir "ignore_parallel_multiple_roots"
        (fun root ->
          ...
          let* () = Fs.create_dir_all net_dir |> Result.map_err ~fn:IO.error_message in
          let* () = Fs.create_dir_all native_dir |> Result.map_err ~fn:IO.error_message in
    ...
    (fun _ctx ->
      ...
      let rec collect acc = function
        ...
        | (label, concurrency, sort) :: rest ->
            let* walker = Ignore.Walker.create ~roots:[ packages_root ] ~concurrency ~sort ()
            |> Result.map_err ~fn:glob_error_message in
