...
let path_is_within_skipped_packages_subtree = fun ~root path ->
  ...
  | Ok rel_path ->
      ...
      |> List.any ~fn:(fun component ->
        List.contains packages_root_skipped_directories ~value:component)
...
let collect_file_paths = fun ~root walker ->
  Ignore.Walker.to_list walker
  |> Result.map ~fn:(List.filter_map ~fn:(fun (entry: Fs.Walker.FileItem.t) ->
    ...
    (fun _ctx ->
      with_tempdir "ignore_parallel_multiple_roots"
        (fun root ->
          ...
          let* () =
            write Path.(net_dir / Path.v "udp_server.mli")
              "type handler = socket:Udp_socket.t -> bytes -> unit\n"
          in
          let* () =
            write Path.(net_dir / Path.v "udp_server.ml")
              "type handler = socket:Udp_socket.t -> bytes -> unit\n"
          in
          ...
          in
          run 25));
  Test.case "ignore walker to_list packages file set is stable across concurrency and sort modes"
    (fun _ctx ->
