...
type parse_error = { line: int; input: string; message: string; offset: int option }

type file_error =
  | File_system of Fs.error
  | Invalid_glob of parse_error

type rule = { matcher: Glob.t; action: Match.t; only_dir: bool }

type t = { root: Path.t; rules: rule list; unmatched_is_ignore_on_files: bool }
  ...
  let rec loop line_number acc = function
    | [] ->
        ...
        let unmatched_is_ignore_on_files =
          match syntax with
          | Ignore_file -> false
          | Override -> List.any rules ~fn:(
            fun rule -> Match.is_whitelist rule.action
          )
        in
...
let from_file = fun ~syntax path ->
  ...
  | Ok true -> (
    ...
    | Ok text ->
        of_string ~root:(Path.dirname path) ~syntax text |> fun result ->
          Result.map result ~fn:Option.some |> fun result -> Result.map_err result ~fn:(
            fun err -> Invalid_glob err
          )
    | Error err -> Error (File_system err)
  )

let relative_path_string = fun matcher path ->
  let relative = Path.strip_prefix path ~prefix:matcher.root |> Result.unwrap_or ~default:path in Path.to_string relative |> strip_leading_dot_slash

let matches_rule = fun rule ~candidate ~is_dir ->
  if rule.only_dir && not is_dir then
    false
  else Glob.matches rule.matcher ~str:candidate |> Result.unwrap_or ~default:false
