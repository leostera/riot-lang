...
let relative_path_string = fun matcher path ->
  let relative = Path.strip_prefix path ~prefix:matcher.root |> Result.unwrap_or ~default:path in
  Path.to_string relative |> strip_leading_dot_slash

let matches_rule = fun rule ~candidate ~is_dir ->
  if rule.only_dir && not is_dir then
    false
  else
    Glob.matches rule.matcher ~str:candidate |> Result.unwrap_or ~default:false
