...
type config = { count_only: bool; concurrency: int; repeat: int; roots: Path.t list }
...
let run_once = fun config walker ->
  let count = Sync.Atomic.make 0 in Ignore.Walker.walk walker ~f:(
    fun entry ->
