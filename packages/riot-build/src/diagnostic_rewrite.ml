  ...
  | Ok rel ->
      ...
      | Ok true ->
          if Riot_model.Package.is_workspace_member package then
            ...
            Some (Path.to_string actual)
      | Ok false
      | Error _ -> None

let rewrite_diagnostics = fun ~package ~sandbox_dir diagnostics ->
  List.map
    ~fn:(Riot_toolchain.Ocamlc.Diagnostic.map_path (rewrite_path ~package ~sandbox_dir))
    diagnostics

let rewrite_ocamlc_result = fun ~package ~sandbox_dir result ->
  match result with
  | Riot_toolchain.Ocamlc.Success success ->
      Riot_toolchain.Ocamlc.Success {
        success with
        diagnostics = rewrite_diagnostics ~package ~sandbox_dir success.diagnostics;
      }
  | Riot_toolchain.Ocamlc.Failed failure ->
