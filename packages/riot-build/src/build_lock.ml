...
type t = { path: Path.t; file: Fs.File.t }
...
let path = fun ~target_dir_root ~profile ~target -> Path.(target_dir_root / Path.v profile / Path.v (Riot_model.Target.to_string target) / Path.v "riot.lock")
...
let existing_lanes = fun ~target_dir_root -> list_subdirectories target_dir_root |> List.filter ~fn:(
  fun dir -> not (String.equal (Path.basename dir) "cache")
) |> List.flat_map ~fn:(
  fun profile_dir ->
    let profile = Path.basename profile_dir in list_subdirectories profile_dir |> List.filter_map ~fn:(
      fun target_dir ->
        let lock_path = Path.(target_dir / Path.v "riot.lock") in
        if not (path_exists lock_path) then
          None
        else Riot_model.Target.from_string (Path.basename target_dir) |> Result.map ~fn:(
          fun target -> { profile; target }
        ) |> Result.to_option
    )
  ...
  with_in_process_lock_counts_lock
    (
      fun () ->
        ...
        | Some count ->
            let next = count + 1 in
            let _ = Collections.HashMap.insert in_process_lock_counts ~key ~value:next in next
        | None ->
            let _ = Collections.HashMap.insert in_process_lock_counts ~key ~value:1 in 1
    )
    ...
    (
      fun () ->
        ...
        | Some count when count > 1 ->
            ...
            let _ = Collections.HashMap.insert in_process_lock_counts ~key ~value:next in next
        | Some _ ->
            let _ = Collections.HashMap.remove in_process_lock_counts ~key in 0
        | None -> 0
    )
...
let lock_failure = fun action path -> Failure (format Std.Format.[
  str "Failed to ";
  str action;
...
let rec retry = fun ~on_waiting ?(announced = false) t ->
  ...
  | Ok true -> Ok t
  | Ok false -> retry ~on_waiting ~announced:true t
  | Error _ ->
      release t;
...
let acquire = fun ~on_waiting ~target_dir_root ~profile ~target fn ->
  ...
  if has_in_process_lock lock_path then
    (
      ...
      try
        let result = fn () in
        let _ = decrement_in_process_lock_count lock_path in result
      with
      | exn ->
          let _ = decrement_in_process_lock_count lock_path in raise exn
    )
  else
