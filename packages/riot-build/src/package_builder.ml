...
let convert_action_error = function
  ...
  | Action_scheduler.OutputsNotCreated { missing } -> Telemetry_events.ActionOutputsNotCreated { missing }
  | Action_scheduler.DependenciesFailed { failed } -> Telemetry_events.ActionDependenciesFailed { failed }
...
let package_error_to_string = function
  ...
  | ActionOutputsNotCreated { missing } -> "Outputs not created: " ^ String.concat ", " (List.map missing ~fn:Path.to_string)
  | ActionDependenciesFailed { failed } -> "Dependencies failed: " ^ Int.to_string (List.length failed) ^ " actions"
...
let package_error_to_json = function
  ...
  | ActionOutputsNotCreated { missing } ->
      Std.Data.Json.Object [
        ...
        "missing", Std.Data.Json.Array (List.map missing ~fn:(
...
type detailed_result = { result: build_result; graph_update: graph_update option }
...
let planned_graph_update = fun (execution_plan: execution_plan) -> Planned_package { hash = execution_plan.hash; module_graph = execution_plan.module_graph; action_graph = execution_plan.action_graph }
...
let apply_graph_update = fun package_graph package_key package graph_update ->
  ...
  | Some node ->
      ...
      | None -> ()
      | Some (Planned_package { hash; module_graph; action_graph }) -> node.value <- Riot_planner.Package_graph.Planned {
        package;
        scope;
        module_graph;
        action_graph;
      ...
      | Some (Cached_package { hash; artifact; depset; exports }) -> node.value <- Riot_planner.Package_graph.Cached {
        package;
        scope;
        hash;
        artifact;
      ...
      | Some (Built_package { hash; artifact; depset; module_graph; action_graph; status }) -> node.value <- Riot_planner.Package_graph.Built {
        package;
        scope;
        module_graph;
        action_graph;
      ...
      | Some (Skipped_package { reason }) -> node.value <- Riot_planner.Package_graph.Skipped { package; scope; reason }
...
let build_result_to_json = fun (result: build_result) ->
  Std.Data.Json.Object [
    ...
    "ocamlc_warnings", Std.Data.Json.Array (List.map result.ocamlc_warnings ~fn:(
      fun msg -> Std.Data.Json.String msg
    ));
    "duration_ms", Std.Data.Json.Int (Int.from_float (Duration.to_secs_float result.duration *. 1000.0));
  ]
...
let collect_source_files = fun package ->
  ...
  | Error _ -> []
  | Ok reader ->
      let all_files = Std.Iter.MutIterator.to_list reader in List.filter_map all_files ~fn:(
        fun file_path ->
          let path_str = Path.to_string file_path in
          if String.ends_with ~suffix:".ml" path_str || String.ends_with ~suffix:".mli" path_str || String.ends_with ~suffix:".c" path_str || String.ends_with ~suffix:".h" path_str then
            let abs_path =
...
let compute_export_entries: Action_graph.t -> Riot_store.Store.export_entry list = fun action_graph ->
  let entries = Action_graph.nodes action_graph |> List.flat_map ~fn:(
    fun (node: Action_node.t) ->
      let is_package_export = List.any node.value.actions ~fn:(
        function
        | Action.CreateLibrary _ | Action.CreateExecutable _ | Action.CreateSharedLibrary _ -> true
        | Action.CompileInterface _ | Action.CompileImplementation _ | Action.GenerateInterface _ | Action.CompileC _ | Action.CopyFile _ | Action.WriteFile _ | Action.BuildForeignDependency _ -> false
      ) in
      if not is_package_export then
        []
      else
        let action_hash_hex = Crypto.Digest.hex (Action_node.get_hash node) in List.map node.value.outs ~fn:(
          fun out_path -> Riot_store.Store.{ name = Path.basename out_path; path = out_path; action_hash = action_hash_hex }
        )
  ) in
  let seen = HashSet.create () in List.filter_map entries ~fn:(
    fun (entry: Riot_store.Store.export_entry) ->
      if HashSet.contains seen ~value:entry.name then
        None
      else
        (
          let _ = HashSet.insert seen ~value:entry.name in Some entry
        )
  )

let collect_package_artifact_outputs = fun ~sandbox_dir ~outputs ->
  let seen = HashSet.create () in outputs |> List.filter_map ~fn:(
    fun out_path ->
      ...
      | Ok _ ->
          ...
          if HashSet.contains seen ~value:abs_path_str then
            ...
            (
              let _ = HashSet.insert seen ~value:abs_path_str in Some abs_path
            )
      | Error _ -> None
  )
...
let emits_visible_progress = function
  ...
  | Riot_planner.Package_graph.Runtime | Riot_planner.Package_graph.Dev -> true
...
let plan_detailed = fun ~workspace ~toolchain ~store ~package_graph ~package_key ~(package:Package.t) ~build_ctx ->
  ...
  match Riot_planner.plan_package_with_graph ~workspace ~toolchain ~store ~package_graph ~package_key ~package ~build_ctx with
  | Error err ->
  ...
  | Ok (MissingDependencies { missing; _ }) ->
      let missing_names = List.map missing ~fn:(
        fun p -> Package_name.to_string p.Package.name
      ) in
      let duration = Instant.duration_since ~earlier:start (Instant.now ()) in
  ...
  | Ok (FailedDependencies { failed; _ }) ->
      let failed_names = List.map failed ~fn:(
        fun p -> Package_name.to_string p.Package.name
      ) in
      let duration = Instant.duration_since ~earlier:start (Instant.now ()) in
  ...
  | Ok (Riot_planner.Package_planner.Cached { package_key = planned_key; hash = package_hash; artifact; depset; exports }) ->
      let duration = Instant.duration_since ~earlier:start (Instant.now ()) in
...
let execution_inputs = fun (execution_plan: execution_plan) ->
  let package = execution_plan.package in List.concat [ package.sources.src; package.sources.native; package.sources.tests ]

let execution_outputs = fun (execution_plan: execution_plan) -> Action_graph.nodes execution_plan.action_graph |> List.flat_map ~fn:(
...
let prepare_execution = fun ~workspace ~toolchain ~store ~execution_plan ~build_ctx ->
  ...
  if execution_plan.emit_visible_progress then
    Telemetry.emit (BuildStarted { session_id; package; target = Workspace_planner.Package package_name });
  Log.info ("Package " ^ package_name_string ^ ": executing action graph");
  Log.info ("Package " ^ package_name_string ^ ": executing action graph with " ^ Int.to_string (List.length (Action_graph.nodes execution_plan.action_graph)) ^ " nodes");
  if execution_plan.emit_visible_progress && List.length (Action_graph.nodes execution_plan.action_graph) > 0 then
    Telemetry.emit
      (
        CompilationStarted {
          session_id;
  ...
  try
    let sandbox = Sandbox.create ~workspace ~profile:profile_name ~target:target_triplet () ~package_name:package.Package.name in
  ...
  | exn ->
      ...
      let error = ExecutionFailed { message = error_msg } in Error (failed_execution_result ~session_id ~build_target:target_triplet ~execution_plan ~error ~graph_error:error_msg)

let execute_action = fun ~store ~(prepared_execution:prepared_execution) ~build_ctx ~completed action -> Action_executor.execute_node ~completed ~store ~session_id:build_ctx.Build_ctx.session_id prepared_execution.toolchain (Sandbox.get_dir prepared_execution.sandbox) action
...
let finalize_execution = fun ~workspace ~store ~(prepared_execution:prepared_execution) ~completed ~build_ctx ->
  ...
  let package_name = package.Package.name in
  let package_name_string = Package_name.to_string package_name in
  let target_dir = Path.(Riot_model.Riot_dirs.out_dir_in_workspace ~workspace ~profile:profile_name ~target:target_triplet / Path.v package_name_string) in
  ...
  in
  try
    let action_result = Action_scheduler.summarize_completed ~action_graph:execution_plan.action_graph ~completed_results:completed in
    match action_result.Action_scheduler.first_failure with
    | Some first_error ->
        let error = convert_action_error first_error in cleanup_and_return (failed_execution_result ~session_id ~build_target:target_triplet ~execution_plan ~error ~graph_error:(package_error_to_string error))
    | None ->
        ...
        Riot_store.Store.materialize_package_exports store ~exports:export_entries ~target_dir |> Result.expect ~msg:("Failed to materialize package exports for " ^ package_name_string);
        let artifact = Riot_store.Store.save store ~package:package_name_string ~ocamlc_warnings ~exports:export_entries ~hash:execution_plan.hash ~sandbox_dir ~outs:package_outputs |> Result.expect ~msg:("Failed to save package hash artifact for " ^ package_name_string) in
  ...
  | exn ->
      let error_msg = "Exception: " ^ Exception.to_string exn in
      let error = ExecutionFailed { message = error_msg } in cleanup_and_return (failed_execution_result ~session_id ~build_target:target_triplet ~execution_plan ~error ~graph_error:error_msg)
...
let execute_detailed = fun ~workspace ~toolchain ~store ~execution_plan ~build_ctx ->
  ...
  | Ok prepared_execution ->
      let action_result = Action_scheduler.run ~action_graph:execution_plan.action_graph ~sandbox:prepared_execution.sandbox ~store ~session_id:build_ctx.Build_ctx.session_id prepared_execution.toolchain ~concurrency:build_ctx.Build_ctx.parallelism in
      let completed: (Graph.SimpleGraph.Node_id.t, Action_executor.execution_result) HashMap.t = HashMap.create () in
      List.for_each action_result.completed_actions ~fn:(
        fun completed_action ->
...
let build_detailed = fun ~workspace ~toolchain ~store ~package_graph ~package_key ~package ~build_ctx ->
  ...
  | Execution_required execution_plan -> execute_detailed ~workspace ~toolchain ~store ~execution_plan ~build_ctx

let build = fun ~workspace ~toolchain ~store ~package_graph ~package_key ~package ~build_ctx ->
  let detailed_result = build_detailed ~workspace ~toolchain ~store ~package_graph ~package_key ~package ~build_ctx in
  apply_graph_update package_graph detailed_result.result.package_key detailed_result.result.package detailed_result.graph_update;
