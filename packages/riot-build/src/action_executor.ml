...
let make_flags_absolute = fun sandbox_dir flags -> List.map flags ~fn:(
  fun flag ->
    match flag with
...
let resolve_include_paths = fun sandbox_dir includes -> List.map includes ~fn:(
  fun inc ->
    let inc_str = Path.to_string inc in
    if Path.is_absolute inc then
      inc
    else
      if String.starts_with ~prefix:"+" inc_str then
        inc
...
let run_action = fun ~session_id ~package ~node ?c_compiler ocamlc sandbox_dir action ->
  ...
  | Action.CompileInterface { source; outputs = output :: _; includes; flags } ->
      ...
      let invocation = Riot_toolchain.Ocamlc.compile_interface ocamlc ~cwd:sandbox_dir ~includes:abs_includes ~flags:abs_flags ~output:abs_output abs_source in run_ocamlc_invocation ~session_id ~package ~node ~sandbox_dir invocation
  | Action.CompileImplementation { source; outputs = output :: _; includes; flags } ->
      let abs_source = Path.join sandbox_dir source in
      let abs_output = Path.join sandbox_dir output in
      let abs_includes = resolve_include_paths sandbox_dir includes in
      let abs_flags = make_flags_absolute sandbox_dir flags in
      let invocation = Riot_toolchain.Ocamlc.compile_impl ocamlc ~cwd:sandbox_dir ~includes:abs_includes ~flags:abs_flags ~output:abs_output abs_source in run_ocamlc_invocation ~session_id ~package ~node ~sandbox_dir invocation
  | Action.GenerateInterface { source; outputs = output :: _; includes; flags } ->
      let abs_source = Path.join sandbox_dir source in
      let abs_output = Path.join sandbox_dir output in
      let abs_includes = resolve_include_paths sandbox_dir includes in
      let abs_flags = make_flags_absolute sandbox_dir flags in
      let invocation = Riot_toolchain.Ocamlc.generate_interface ocamlc ~cwd:sandbox_dir ~includes:abs_includes ~flags:abs_flags ~output:abs_output abs_source in run_ocamlc_invocation ~session_id ~package ~node ~sandbox_dir invocation
  | Action.CompileC { source; outputs = output :: _; ccflags } ->
      ...
      let invocation = Riot_toolchain.Ocamlc.compile_c ocamlc ~cwd:sandbox_dir ~includes:source_dir ?cc:c_compiler ~ccflags ~output:abs_output abs_source in run_ocamlc_invocation ~session_id ~package ~node ~sandbox_dir invocation
  | Action.CreateLibrary { outputs = output :: _; objects; includes } ->
      let abs_output = Path.join sandbox_dir output in
      let rel_objects = objects in
      let abs_includes = resolve_include_paths sandbox_dir includes in
      let invocation = Riot_toolchain.Ocamlc.create_library ocamlc ~cwd:sandbox_dir ~includes:abs_includes ~output:abs_output rel_objects in run_ocamlc_invocation ~session_id ~package ~node ~sandbox_dir invocation
  | Action.CreateExecutable { outputs = output :: _; objects; libraries; includes; cclibs; ccopt_flags; cclib_flags } -> (
    Log.debug ("[ACTION_EXECUTOR] CreateExecutable: output=" ^ Path.to_string output ^ ", " ^ Int.to_string (List.length objects) ^ " objects, " ^ Int.to_string (List.length libraries) ^ " libraries: [" ^ String.concat ", " (List.map libraries ~fn:Path.to_string) ^ "], cclibs: [" ^ String.concat ", " (List.map cclibs ~fn:Path.to_string) ^ "], ccopt_flags: [" ^ String.concat ", " ccopt_flags ^ "], cclib_flags: [" ^ String.concat ", " cclib_flags ^ "]");
    let abs_output = Path.join sandbox_dir output in
    let abs_objects = List.map objects ~fn:(Path.join sandbox_dir) in
    let abs_libraries = libraries in
    let abs_includes = resolve_include_paths sandbox_dir includes in
    let abs_cclibs = cclibs in
    let invocation = Riot_toolchain.Ocamlc.create_executable ocamlc ~cwd:sandbox_dir ~includes:abs_includes ~libs:abs_libraries ?cc:c_compiler ~cclibs:abs_cclibs ~ccopt_flags ~cclib_flags ~output:abs_output abs_objects in
    ...
    | Riot_toolchain.Ocamlc.Success _ -> (
      ...
      | Ok () -> result
      | Error err ->
          Log.warn ("Failed to set executable permissions on " ^ Path.to_string abs_output ^ ": " ^ IO.error_message err);
          result
    )
    | _ -> result
  )
  | Action.CreateSharedLibrary { outputs = output :: _; objects; libraries; includes; cclibs; ccopt_flags; cclib_flags } -> (
    Log.debug ("[ACTION_EXECUTOR] CreateSharedLibrary: output=" ^ Path.to_string output ^ ", " ^ Int.to_string (List.length objects) ^ " objects, " ^ Int.to_string (List.length libraries) ^ " libraries: [" ^ String.concat ", " (List.map libraries ~fn:Path.to_string) ^ "], cclibs: [" ^ String.concat ", " (List.map cclibs ~fn:Path.to_string) ^ "], ccopt_flags: [" ^ String.concat ", " ccopt_flags ^ "], cclib_flags: [" ^ String.concat ", " cclib_flags ^ "]");
    let abs_output = Path.join sandbox_dir output in
    let abs_objects = List.map objects ~fn:(Path.join sandbox_dir) in
    let abs_libraries = libraries in
    let abs_includes = resolve_include_paths sandbox_dir includes in
    let abs_cclibs = cclibs in
    let invocation = Riot_toolchain.Ocamlc.create_shared_library ocamlc ~cwd:sandbox_dir ~includes:abs_includes ~libs:abs_libraries ?cc:c_compiler ~cclibs:abs_cclibs ~ccopt_flags ~cclib_flags ~output:abs_output abs_objects in run_ocamlc_invocation ~session_id ~package ~node ~sandbox_dir invocation
  )
  | Action.CompileInterface { outputs = []; _ } | Action.CompileImplementation { outputs = []; _ } | Action.GenerateInterface { outputs = []; _ } | Action.CompileC { outputs = []; _ } | Action.CreateLibrary { outputs = []; _ } | Action.CreateExecutable { outputs = []; _ } | Action.CreateSharedLibrary { outputs = []; _ } -> ocamlc_failed "Action has no outputs"
  | Action.CopyFile { source; destination } -> (
    ...
    | Error _ -> ocamlc_failed ("Copy failed: " ^ Path.to_string source ^ " -> " ^ Path.to_string destination)
  )
  | Action.WriteFile { destination; content } -> (
    let dest_path = Path.join sandbox_dir destination in
    Log.debug ("WriteFile: writing to " ^ Path.to_string dest_path ^ " (" ^ Int.to_string (String.length content) ^ " bytes)");
  ...
  | Action.BuildForeignDependency { name; path; build_cmd; outputs; env } -> (
    ...
    | cmd_name :: cmd_args ->
        ...
        | Ok output when output.Command.status = 0 ->
            ...
            let abs_outputs = List.map outputs ~fn:(
              fun out -> Path.normalize (Path.join path out)
            ) in
            let missing = List.filter abs_outputs ~fn:(
              fun out ->
            ...
            if List.length missing > 0 then
              ocamlc_failed ("Foreign build succeeded but outputs not created: " ^ String.concat ", " (List.map missing ~fn:Path.to_string))
            else ocamlc_success ("Built foreign dependency: " ^ name)
        | Ok output ->
            Log.error ("Foreign build failed: " ^ name ^ " - exit code " ^ Int.to_string output.Command.status);
            if String.length output.Command.stderr > 0 then
              Log.error ("stderr: " ^ output.Command.stderr);
            ocamlc_failed ("Foreign build failed: " ^ name ^ " - exit code " ^ Int.to_string output.Command.status)
...
let execute_actions = fun ~session_id ~(node:Action_node.t) toolchain sandbox_dir actions ->
  ...
  let rec execute_next ocamlc_warnings = function
    ...
    | action :: rest -> (
      ...
      match result with
      | Riot_toolchain.Ocamlc.Success _ ->
          let action_warnings = Riot_toolchain.Ocamlc.get_ocamlc_warnings result in execute_next (ocamlc_warnings @ action_warnings) rest
...
let verify_outputs = fun outputs ->
  let missing = List.filter outputs ~fn:(
    fun out ->
      match Fs.exists out with
      | Ok true -> false
      | Ok false | Error _ -> true
...
let save_action_artifact = fun ~store ~package ~action_hash ~ocamlc_warnings ~sandbox_dir ~outputs -> Riot_store.Store.save store ~package ~ocamlc_warnings ~hash:action_hash ~sandbox_dir ~outs:outputs |> Result.map_err ~fn:Riot_store.Store.error_message

let has_failed_dependencies = fun completed (node: Action_node.t) -> List.any node.deps ~fn:(
  fun dep_id ->
...
let resolve_source_for_copy = fun ~(package:Riot_model.Package.t) ~src_path ->
  ...
  | Some p -> Ok p
  | None -> Error ("source not found for " ^ Path.to_string src_path ^ " (checked " ^ String.concat ", " (List.map candidates ~fn:Path.to_string) ^ ")")
  ...
  if has_failed_dependencies completed node then
    ...
    (
      ...
      | Some artifact ->
          ...
          let _ = Riot_store.Store.promote store artifact.Riot_store.Artifact.hash ~target_dir:sandbox_dir |> Result.expect ~msg:("Failed to materialize cached action artifact: " ^ G.Node_id.to_string node.id) in
      ...
      | None ->
          ...
          Telemetry.emit (Telemetry_events.ActionStarted { session_id; package = node.value.package; action = node });
          let copy_result: (unit, string) Result.t = List.fold_left sources ~init:(Ok ()) ~fn:(
            fun acc src_path ->
              match acc with
              | Error _ -> acc
              | Ok () ->
          ...
          | Ok () -> (
            ...
            | Ok ocamlc_warnings ->
                let needs_output_verification = List.any actions ~fn:(
                  function
                  | Action.BuildForeignDependency _ -> false
                  | _ -> true
                ) in
                if not needs_output_verification then
                  match save_action_artifact ~store ~package:(Riot_model.Package_name.to_string node.value.package.name) ~action_hash ~ocamlc_warnings ~sandbox_dir ~outputs:(List.map outputs ~fn:(Path.join sandbox_dir)) with
                  | Error message ->
                      {
                        node_id = node.id;
                  ...
                  | Ok () ->
                      match save_action_artifact ~store ~package:(Riot_model.Package_name.to_string node.value.package.name) ~action_hash ~ocamlc_warnings ~sandbox_dir ~outputs:abs_outputs with
                      | Error message ->
                          {
                            node_id = node.id;
                            status = Failed (ExecutionFailed { message });
