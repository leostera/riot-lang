...
type completed_action = { node: Action_node.t; result: execution_result }
...
let make_graph = fun completed_results action_graph ->
  ...
  Action_graph.nodes action_graph |> List.for_each ~fn:(
    fun (node: Action_node.t) ->
      let node_id = HashMap.get node_ids ~key:node.id |> Option.expect ~msg:("missing scheduler node for action " ^ Graph.SimpleGraph.Node_id.to_string node.id) in List.for_each node.deps ~fn:(
        fun dependency_id ->
          let dependency_node_id = HashMap.get node_ids ~key:dependency_id |> Option.expect ~msg:("missing scheduler dependency node for action " ^ Graph.SimpleGraph.Node_id.to_string dependency_id) in Graph_scheduler.Graph.add_dependency graph ~node:node_id ~depends_on:dependency_node_id
      )
  );
  graph
...
let first_failure_of_completed_actions = fun completed_actions -> find_first_map completed_actions ~fn:(
...
let ocamlc_warnings_of_completed_actions = fun completed_actions ->
  let seen = HashSet.create () in completed_actions |> List.fold_left ~init:[] ~fn:(
    fun acc completed_action -> List.fold_left completed_action.result.ocamlc_warnings ~init:acc ~fn:(
      fun acc warning ->
        if HashSet.contains seen ~value:warning then
          acc
        else
          let _ = HashSet.insert seen ~value:warning in acc @ [ warning ]
    )
  )

let find_result = fun (result: t) (node: Action_node.t) -> find_first_map result.completed_actions ~fn:(
  fun completed_action ->
...
let summarize_completed = fun ~action_graph ~completed_results ->
  let completed_actions = Action_graph.nodes action_graph |> List.filter_map ~fn:(
    fun (node: Action_node.t) ->
      match HashMap.get completed_results ~key:node.id with
      | Some result -> Some { node; result }
      | None -> None
  ) in { completed_actions; first_failure = first_failure_of_completed_actions completed_actions; ocamlc_warnings = ocamlc_warnings_of_completed_actions completed_actions }
...
let run = fun ~action_graph ~sandbox ~store ~session_id toolchain ~concurrency ->
  ...
  let _ = Graph_scheduler.run ~config:(Graph_scheduler.Run_config.make ~parallelism:concurrency ~mode:Graph_scheduler.Run_config.Continue_on_failure ()) ~on_event:(
    fun () -> ()
  ) ~graph ~execute:(
    fun ~graph ~node:_ ~payload ->
      let result = Action_executor.execute_node ~completed:completed_results ~store ~session_id toolchain sandbox_dir payload in
      Graph_scheduler.Handle.record graph (Remember_result { node = payload; result });
      Ok result
  ) in summarize_completed ~action_graph ~completed_results
