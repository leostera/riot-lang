...
type t = { packages: package_result list }
...
let artifact_of_status = function
  | Built artifact | Cached artifact -> Some artifact
...
let scope_of_package_key = fun key ->
  ...
  if String.ends_with ~suffix:":dev" value then
    ...
    if String.ends_with ~suffix:":runtime" value then
      Runtime
    else
      if String.ends_with ~suffix:":build" value then
        Build
      else Unknown
...
let should_prefer = fun current incoming ->
  ...
  if scope_comparison = Order.GT then
    ...
    if scope_comparison = Order.LT then
      false
    else Int.compare (status_priority incoming.status) (status_priority current.status) = Order.GT

let merge_artifacts = fun current incoming -> List.fold_left incoming ~init:current ~fn:(
  fun acc (artifact: Riot_store.Artifact.t) ->
    if List.any acc ~fn:(
      fun existing -> Crypto.Hash.equal existing.Riot_store.Artifact.hash artifact.hash
    ) then
      acc
    else acc @ [ artifact ]
)
...
let rec upsert_package_result = fun packages incoming ->
  ...
  | current :: rest ->
      ...
      else current :: upsert_package_result rest incoming
...
let of_build_results = fun results -> { packages = List.fold_left results ~init:[] ~fn:(
  fun acc result -> upsert_package_result acc (package_result_of_build_result result)
) }

let packages = fun t -> t.packages

let find_package = fun t name -> List.find t.packages ~fn:(
  fun pkg -> Riot_model.Package_name.equal pkg.package_name name
)
...
let rec find_export_in_artifacts = fun artifacts export_name ->
  ...
  | (artifact: Riot_store.Artifact.t) :: rest -> (
    match List.find artifact.exports ~fn:(
      fun (entry: Riot_store.Manifest.export_entry) -> String.equal entry.name export_name
    ) with
...
let failure_reason_message = function
  | PackagePlanningFailed planning_error -> "Planning failed: " ^ Riot_planner.Planning_error.to_string planning_error
  | PackageExecutionFailed { message } -> "Execution failed: " ^ message
  | PackageActionFailed { message } -> "Action failed: " ^ message
  | PackageActionOutputsNotCreated { missing } -> "Outputs not created: " ^ String.concat ", " (List.map missing ~fn:Path.to_string)
  | PackageActionDependenciesFailed { failed } -> "Dependencies failed: " ^ Int.to_string (List.length failed) ^ " actions"
  ...
  | PackageActionOutputsNotCreated { missing } ->
      Data.Json.Object [
        ...
        "missing", Data.Json.Array (List.map missing ~fn:(
          fun path -> Data.Json.String (Path.to_string path)
        ));
      ]
...
let failure_of_build_result = fun (result: Package_builder.build_result) ->
  let reason =
    ...
    | Package_builder.Built _ | Package_builder.Cached _ -> UnknownFailure
  in
...
let failure_message = fun (failure: failure) -> Riot_model.Package_name.to_string failure.package_name ^ ": " ^ failure.message
