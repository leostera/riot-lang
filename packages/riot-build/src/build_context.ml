...
let same_session = fun left right ->
  String.equal
    (Riot_model.Session_id.to_string left)
    (Riot_model.Session_id.to_string right)

let emit_public_event = fun context event ->
  Std.Telemetry.emit (Public_build_event {
    session_id = context.session_id;
    event;
  })

let emit_phase = fun context phase ->
  emit_public_event context (Phase phase)
...
let emit_cache_gc = fun context event ->
  emit_public_event context (CacheGc event)
...
let forward_telemetry_event = fun context (event: Std.Telemetry.event) ->
  ...
  | Public_build_event { session_id; event } when same_session session_id context.session_id -> (
      ...
      | CacheGc event ->
          context.on_event (Event.CacheGc event)
      | Phase phase ->
          context.on_event (Event.Phase phase)
    )
...
let flush_events = fun context ->
  ...
  Std.Telemetry.emit (Flush_build_events {
    session_id = context.session_id;
    reply_to = self ();
    request_id;
  });
  let selector message =
    match message with
    | Build_events_flushed { request_id=got } when Int.equal got request_id ->
        `select ()
    | _ ->
        `skip
  in
