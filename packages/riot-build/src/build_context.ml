...
let same_session = fun left right -> String.equal (Riot_model.Session_id.to_string left) (Riot_model.Session_id.to_string right)

let emit_public_event = fun context event -> Std.Telemetry.emit (Public_build_event { session_id = context.session_id; event })

let emit_phase = fun context phase -> emit_public_event context (Phase phase)

let emit_building_target = fun context ~target ~host -> emit_public_event context (BuildingTarget { target; host })
...
let forward_telemetry_event = fun context (event: Std.Telemetry.event) ->
  ...
  | Flush_build_events { session_id; reply_to; request_id } when same_session session_id context.session_id -> send reply_to (Build_events_flushed { request_id })
  | _ -> (
    match Telemetry_events.event_session_id event with
    | Some session_id when same_session session_id context.session_id -> context.on_event (Event.Telemetry event)
    | Some _ | None -> ()
  )

let flush_events = fun context ->
  let request_id = next_flush_request_id () in
  Std.Telemetry.emit (Flush_build_events { session_id = context.session_id; reply_to = self (); request_id });
...
let make = fun ?(on_event = no_event) request ->
  ...
  let* parallelism = requested_parallelism request
  in
  Ok {
