...
let target_to_json = fun target ->
  Data.Json.String (
    ...
    | Workspace_planner.Packages pkgs -> "packages:" ^ String.concat "," (List.map pkgs ~fn:Package_name.to_string)
  )
...
let target_of_json = function
  ...
  | Data.Json.String target_str when String.starts_with ~prefix:"packages:" target_str ->
      ...
      in
      Result.map packages ~fn:(
        fun packages -> Workspace_planner.Packages packages
      )
  | Data.Json.String pkg -> Package_name.from_string pkg |> Result.map ~fn:(
    fun pkg -> Workspace_planner.Package pkg
  ) |> Result.map_err ~fn:(
    fun error -> Data.Json.string (Package_name.error_message error)
  )
...
let build_target_of_json = function
  | Data.Json.String target -> Target.from_string target |> Result.map_err ~fn:(
    fun error -> Data.Json.String (Target.error_message error)
  )
...
let planning_breakdown_of_json = function
  | Data.Json.Object fields -> (
    match Data.Json.get_field "dependency_count" (Data.Json.Object fields), Data.Json.get_field "dependency_check_duration_ms" (Data.Json.Object fields), Data.Json.get_field "input_hash_duration_ms" (Data.Json.Object fields), Data.Json.get_field "artifact_lookup_duration_ms" (Data.Json.Object fields), Data.Json.get_field "artifact_cache_hit" (Data.Json.Object fields), Data.Json.get_field "plan_bundle_lookup_duration_ms" (Data.Json.Object fields), Data.Json.get_field "plan_bundle_decode_duration_ms" (Data.Json.Object fields), Data.Json.get_field "plan_bundle_cache_hit" (Data.Json.Object fields), Data.Json.get_field "module_plan_duration_ms" (Data.Json.Object fields) with
...
let workspace_graph_breakdown_of_json = function
  | Data.Json.Object fields -> (
    match Data.Json.get_field "build_node_realization_count" (Data.Json.Object fields), Data.Json.get_field "build_node_realization_duration_ms" (Data.Json.Object fields), Data.Json.get_field "runtime_node_realization_count" (Data.Json.Object fields), Data.Json.get_field "runtime_node_realization_duration_ms" (Data.Json.Object fields), Data.Json.get_field "dev_node_realization_count" (Data.Json.Object fields), Data.Json.get_field "dev_node_realization_duration_ms" (Data.Json.Object fields), Data.Json.get_field "edge_wiring_duration_ms" (Data.Json.Object fields) with
    | (Some (Data.Json.Int build_node_realization_count), Some (Data.Json.Int build_node_realization_duration_ms), Some (Data.Json.Int runtime_node_realization_count), Some (Data.Json.Int runtime_node_realization_duration_ms), Some (Data.Json.Int dev_node_realization_count), Some (Data.Json.Int dev_node_realization_duration_ms), Some (Data.Json.Int edge_wiring_duration_ms)) ->
        Ok {
          ...
          runtime_node_realization_duration = Time.Duration.from_millis runtime_node_realization_duration_ms;
          dev_node_realization_count;
          dev_node_realization_duration = Time.Duration.from_millis dev_node_realization_duration_ms;
...
let to_json: Telemetry.event -> Data.Json.t option = function
  ...
  | WorkspacePlanCompleted { session_id; target; workspace_package_count; planned_package_count; duration } ->
      Some (
        Data.Json.Object [
  ...
  | WorkspaceManifestFilterCompleted { session_id; target; filtered_workspace_package_count; duration } ->
      Some (
        Data.Json.Object [
  ...
  | WorkspaceTargetGraphFiltered { session_id; target; node_count; duration } ->
      Some (
        Data.Json.Object [
  ...
  | PlanningWorkspaceCompleted { session_id; target; duration; planned_count; missing_count; failed_count } ->
      Some (
        Data.Json.Object [
  ...
  | PackagePlanningResult { session_id; package; target; status; duration; reason } ->
      Some (
        Data.Json.Object (
          [
  ...
  | PackageOcamlcWarnings { session_id; package; target; build_target; source; messages } ->
      Some (
        Data.Json.Object [
          ...
          "messages", Data.Json.Array (List.map messages ~fn:(
            fun msg -> Data.Json.String msg
          ));
        ]
      )
  | BuildCompleted { session_id; package; target; build_target; status; duration } ->
      Some (
        Data.Json.Object [
  ...
  | BuildFailed { session_id; package; target; build_target; error } ->
      let error_json =
        match error with
  ...
  | BuildSkipped { session_id; package; target; build_target; reason } ->
      Some (
        Data.Json.Object [
  ...
  | ActionCompleted { session_id; package; action; artifact; status; duration } ->
      let artifact_files = Data.Json.Array (List.map artifact.files ~fn:(
        fun p -> Data.Json.String (Path.to_string p)
      )) in
      Some (
        Data.Json.Object [
  ...
  | WorkspaceCompleted { session_id; target; total_duration; cached_count; built_count; failed_count } ->
      Some (
        Data.Json.Object [
...
let event_session_id: Telemetry.event -> Session_id.t option = function
  | BuildStarted { session_id; _ } | WorkspacePlanStarted { session_id; _ } | WorkspacePlanCompleted { session_id; _ } | WorkspaceManifestFilterCompleted { session_id; _ } | WorkspaceGraphCreated { session_id; _ } | WorkspaceTargetGraphFiltered { session_id; _ } | WorkspaceTopologicalSortCompleted { session_id; _ } | PlanningWorkspaceStarted { session_id; _ } | PlanningWorkspaceCompleted { session_id; _ } | PackagePlanningResult { session_id; _ } | PackagePlanningBreakdown { session_id; _ } | CompilationStarted { session_id; _ } | PackageOcamlcWarnings { session_id; _ } | BuildCompleted { session_id; _ } | BuildFailed { session_id; _ } | BuildSkipped { session_id; _ } | ActionStarted { session_id; _ } | ActionCommandStarted { session_id; _ } | ActionCompleted { session_id; _ } | ActionFailed { session_id; _ } | CacheHit { session_id; _ } | CacheMiss { session_id; _ } | WorkspaceStarted { session_id; _ } | WorkspaceCompleted { session_id; _ } -> Some session_id
  | _ -> None

let from_json: Data.Json.t -> (Telemetry.event, Data.Json.t) result = fun json ->
  let get_field fields ~name = List.find fields ~fn:(
  ...
  | Data.Json.Object fields -> (
    ...
    | Some (Data.Json.String "BuildStarted") -> (
      match get_field fields ~name:"session_id", get_field fields ~name:"package", get_field fields ~name:"target" with
    ...
    | Some (Data.Json.String "WorkspacePlanStarted") -> (
      ...
      | Some (Data.Json.String target_str), Some (Data.Json.Int workspace_package_count) -> (
        ...
        | Ok target -> Ok (WorkspacePlanStarted { session_id = get_session_id fields; target; workspace_package_count })
        | Error e -> Error e
      )
      | _ -> Error (Data.Json.String "Invalid WorkspacePlanStarted event")
    )
    | Some (Data.Json.String "WorkspacePlanCompleted") -> (
      match get_field fields ~name:"target", get_field fields ~name:"workspace_package_count", get_field fields ~name:"planned_package_count", get_field fields ~name:"duration_ms" with
    ...
    | Some (Data.Json.String "WorkspaceManifestFilterCompleted") -> (
      match get_field fields ~name:"target", get_field fields ~name:"filtered_workspace_package_count", get_field fields ~name:"duration_ms" with
    ...
    | Some (Data.Json.String "WorkspaceGraphCreated") -> (
      match get_field fields ~name:"target", get_field fields ~name:"node_count", get_field fields ~name:"breakdown", get_field fields ~name:"duration_ms" with
    ...
    | Some (Data.Json.String "WorkspaceTargetGraphFiltered") -> (
      match get_field fields ~name:"target", get_field fields ~name:"node_count", get_field fields ~name:"duration_ms" with
    ...
    | Some (Data.Json.String "WorkspaceTopologicalSortCompleted") -> (
      match get_field fields ~name:"target", get_field fields ~name:"sorted_package_count", get_field fields ~name:"duration_ms" with
    ...
    | Some (Data.Json.String "PlanningWorkspaceStarted") -> (
      ...
      | Some (Data.Json.String target_str), Some (Data.Json.Int package_count) -> (
        match target_of_json (Data.Json.String target_str) with
        | Ok target -> Ok (PlanningWorkspaceStarted { session_id = get_session_id fields; target; package_count })
        | Error e -> Error e
      )
      | _ -> Error (Data.Json.String "Invalid PlanningWorkspaceStarted event")
    )
    | Some (Data.Json.String "PlanningWorkspaceCompleted") -> (
      match get_field fields ~name:"target", get_field fields ~name:"duration_ms", get_field fields ~name:"planned_count", get_field fields ~name:"missing_count", get_field fields ~name:"failed_count" with
    ...
    | Some (Data.Json.String "PackagePlanningResult") -> (
      match get_field fields ~name:"package", get_field fields ~name:"target", get_field fields ~name:"status", get_field fields ~name:"duration_ms" with
      | (Some package_json, Some (Data.Json.String target_str), Some status_json, Some (Data.Json.Int duration_ms)) -> (
        match Package.from_json package_json with
        | Ok package -> (
          match target_of_json (Data.Json.String target_str), planning_status_of_json status_json with
          | Ok target, Ok status ->
              Ok (
    ...
    | Some (Data.Json.String "PackagePlanningBreakdown") -> (
      match get_field fields ~name:"package", get_field fields ~name:"target", get_field fields ~name:"breakdown" with
      | (Some package_json, Some (Data.Json.String target_str), Some breakdown_json) -> (
        match Package.from_json package_json with
        | Ok package -> (
          match target_of_json (Data.Json.String target_str), planning_breakdown_of_json breakdown_json with
          | Ok target, Ok breakdown ->
              Ok (
    ...
    | Some (Data.Json.String "PackageOcamlcWarnings") -> (
      match get_field fields ~name:"session_id", get_field fields ~name:"package", get_field fields ~name:"target", get_field fields ~name:"source", get_field fields ~name:"messages" with
      | Some (Data.Json.String session_id_str), Some package_json, Some target_json, Some source_json, Some (Data.Json.Array messages_json) -> (
        match Package.from_json package_json with
        | Ok package -> (
          match target_of_json target_json, build_target_from_fields fields, warning_source_of_json source_json with
    ...
    | Some (Data.Json.String "BuildFailed") -> (
      match get_field fields ~name:"package", get_field fields ~name:"target", get_field fields ~name:"error" with
            ...
            let error_result =
              ...
              | Data.Json.Object error_fields -> (
                  ...
                  | Some (Data.Json.Object planning_fields) -> (
                    ...
                    | Some (Data.Json.String "cyclic_dependency") -> (
                      ...
                      | Some (Data.Json.Array arr) ->
                          let cycle = List.filter_map arr ~fn:(
                            function
                            | Data.Json.String s -> Some s
                            | _ -> None
                          ) in Ok (PlanningFailed (Planning_error.CyclicDependency { cycle }))
                      | _ -> Ok (ExecutionFailed { message = "Planning failed: cyclic dependency" })
                    )
                    | Some (Data.Json.String "scan_failed") -> (
                      match get_field planning_fields ~name:"path", get_field planning_fields ~name:"reason" with
                      | (Some (Data.Json.String path), Some (Data.Json.String reason)) -> Ok (PlanningFailed (Planning_error.ScanFailed { path = Path.v path; reason }))
                      | _ -> Ok (ExecutionFailed { message = "Planning failed: scan failed" })
                    )
                    | Some (Data.Json.String "dependency_analysis_failed") -> (
                      match get_field planning_fields ~name:"reason" with
                      | Some (Data.Json.String reason) -> Ok (PlanningFailed (Planning_error.DependencyAnalysisFailed { reason }))
                      | _ -> Ok (ExecutionFailed { message = "Planning failed: dependency analysis failed" })
                    )
                    | Some (Data.Json.String "graph_build_failed") -> (
                      match get_field planning_fields ~name:"reason" with
                      | Some (Data.Json.String reason) -> Ok (PlanningFailed (Planning_error.GraphBuildFailed { reason }))
                      | _ -> Ok (ExecutionFailed { message = "Planning failed: graph build failed" })
                    )
                    | Some (Data.Json.String "source_depends_on_undeclared_package_module") -> (
                      match get_field planning_fields ~name:"package_name", get_field planning_fields ~name:"source", get_field planning_fields ~name:"requested_module", get_field planning_fields ~name:"allowed_modules" with
                      | (Some (Data.Json.String package_name), Some (Data.Json.String source), Some (Data.Json.String requested_module), Some (Data.Json.Array allowed_modules)) ->
                          let allowed_modules = List.filter_map allowed_modules ~fn:(
                            function
                      ...
                      | _ -> Ok (ExecutionFailed { message = "Planning failed: source depends on undeclared package module" })
                    )
                    | Some (Data.Json.String "target_depends_on_internal_library_module") -> (
                      match get_field planning_fields ~name:"target_name", get_field planning_fields ~name:"source", get_field planning_fields ~name:"requested_module", get_field planning_fields ~name:"internal_module", get_field planning_fields ~name:"public_module" with
                      ...
                      | _ -> Ok (ExecutionFailed { message = "Planning failed: target depends on internal library module" })
                    )
                    | Some (Data.Json.String "target_depends_on_namespaced_internal_library_module") -> (
                      ...
                      | _ -> Ok (ExecutionFailed { message = "Planning failed: target depends on namespaced internal library module" })
                    )
                    | Some (Data.Json.String "target_depends_on_other_target_root") -> (
                      match get_field planning_fields ~name:"target_name", get_field planning_fields ~name:"source", get_field planning_fields ~name:"requested_module", get_field planning_fields ~name:"other_target_name", get_field planning_fields ~name:"other_target_module", get_field planning_fields ~name:"public_module" with
                    ...
                    | Some (Data.Json.String "invalid_executable_main") -> (
                      let executable_main_error_of_json = function
                        ...
                        | _ -> None
                      in
                      match get_field planning_fields ~name:"package_name", get_field planning_fields ~name:"target_name", get_field planning_fields ~name:"source", get_field planning_fields ~name:"file", get_field planning_fields ~name:"error" with
                      | (package_name_json, Some (Data.Json.String target_name), Some (Data.Json.String source), file_json, Some error_json) -> (
                        ...
                        | None -> Ok (ExecutionFailed { message = "Planning failed: invalid executable main" })
                      )
                      | _ -> Ok (ExecutionFailed { message = "Planning failed: invalid executable main" })
                    )
                    | Some (Data.Json.String "exception") -> (
                      match get_field planning_fields ~name:"message" with
                      | Some (Data.Json.String msg) -> Ok (PlanningFailed (Planning_error.Exception { exn = Failure msg }))
                      | _ -> Ok (ExecutionFailed { message = "Planning failed: exception" })
                    )
                    | _ -> Ok (ExecutionFailed { message = "Planning failed: unknown planning error" })
                  )
                  | _ -> Ok (ExecutionFailed { message = "Planning failed: missing error details" })
                )
                | Some (Data.Json.String "execution_failed") -> (
                  ...
                  | _ -> Ok (ExecutionFailed { message = "Execution failed: missing message" })
                )
                | Some (Data.Json.String "action_failed") -> (
                  match get_field error_fields ~name:"message" with
                  | Some (Data.Json.String msg) -> Ok (ActionExecutionFailed { message = msg })
                ...
                | Some (Data.Json.String "outputs_not_created") -> (
                  ...
                  | Some (Data.Json.Array arr) ->
                      let missing = List.filter_map arr ~fn:(
                        function
                        | Data.Json.String s -> Some (Path.v s)
                        | _ -> None
                      ) in Ok (ActionOutputsNotCreated { missing })
                  | _ -> Ok (ExecutionFailed { message = "Outputs not created: missing list" })
                )
                | Some (Data.Json.String "dependencies_failed") -> Ok (ActionDependenciesFailed { failed = [] })
    ...
    | Some (Data.Json.String "BuildSkipped") -> (
      match get_field fields ~name:"package", get_field fields ~name:"target", get_field fields ~name:"reason" with
    ...
    | Some (Data.Json.String "WorkspaceStarted") -> (
      ...
      | (Some (Data.Json.String target_str), Some (Data.Json.Int package_count)) -> (
        match target_of_json (Data.Json.String target_str) with
        | Ok target -> Ok (WorkspaceStarted { session_id = get_session_id fields; target; package_count })
        | Error e -> Error e
      )
      | _ -> Error (Data.Json.String "Invalid WorkspaceStarted event")
    )
    | Some (Data.Json.String "WorkspaceCompleted") -> (
      match get_field fields ~name:"target", get_field fields ~name:"total_duration_ms", get_field fields ~name:"cached_count", get_field fields ~name:"built_count", get_field fields ~name:"failed_count" with
    ...
    | Some (Data.Json.String "ActionStarted") | Some (Data.Json.String "ActionCommandStarted") | Some (Data.Json.String "ActionCompleted") | Some (Data.Json.String "ActionFailed") | Some (Data.Json.String "CacheHit") | Some (Data.Json.String "CacheMiss") -> Error (Data.Json.String "Action events require Action_node.t (not serializable)")
    | Some (Data.Json.String typ) -> Error (Data.Json.String ("Unknown telemetry event type: " ^ typ))
