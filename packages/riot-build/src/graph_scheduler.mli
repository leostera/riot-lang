...
module Graph : sig
  ...
  val create: apply_mutation:(('work, 'mutation) t -> 'mutation -> unit) -> unit -> ('work, 'mutation) t
...
val run: config:Run_config.t -> on_event:('event -> unit) -> graph:('work, 'mutation) Graph.t -> execute:(graph:('work, 'mutation, 'event) Handle.t -> node:Node_id.t -> payload:'work -> ('result, 'error) result) -> ('work, 'result, 'error) run_result
