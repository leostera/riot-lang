...
let telemetry_handler_id = fun context -> "riot-build:" ^ Riot_model.Session_id.to_string context.build.Build_context.session_id
...
let emit_targets_resolved = fun context targets -> emit_runtime_phase context (Event.TargetsResolved { target_count = List.length targets })

let emit_toolchains_ensured = fun context targets -> emit_runtime_phase context (Event.ToolchainsEnsured { target_count = List.length targets })

let emit_toolchains_validated = fun context targets -> emit_runtime_phase context (Event.ToolchainsValidated { target_count = List.length targets })
...
let emit_target_build_finished = fun context ~target ~result_count ~had_partial_failure -> emit_runtime_phase context (Event.TargetBuildFinished { target; result_count; had_partial_failure })

let emit_cache_generation_recording_started = fun context ~lane_count ~new_entry_count -> emit_runtime_phase context (Event.CacheGenerationRecordingStarted { lane_count; new_entry_count })

let emit_cache_generation_recorded = fun context ~lane_count ~new_entry_count -> emit_runtime_phase context (Event.CacheGenerationRecorded { lane_count; new_entry_count })

let emit_returning_results = fun context ~result_count ~had_partial_failure -> emit_runtime_phase context (Event.ReturningResults { result_count; had_partial_failure })

let error_message = function
  | ToolchainInstallFailed { target; error } -> "Failed to install toolchain for " ^ Riot_model.Target.to_string target ^ ": " ^ toolchain_install_error_message error
  | ToolchainInitializationFailed { target; error } -> "Failed to initialize toolchain for " ^ Riot_model.Target.to_string target ^ ": " ^ toolchain_initialization_error_message error
  | BuildFailed { errors } -> (
...
let ensure_toolchains_for_targets = fun context targets ->
  ...
  let missing = List.filter targets ~fn:(
    fun target ->
      match Riot_toolchain.check_toolchain_status ~version:context.build.toolchain_config.version ~target with
      | Riot_toolchain.NotInstalled _ | Riot_toolchain.Incomplete _ -> true
      | Riot_toolchain.Installed _ -> false
  ) in
  let rec loop = function
    | [] -> Ok ()
    | target :: rest -> (
      match Riot_toolchain.download_and_install_toolchain context.build.toolchain_config.version ~host:context.build.host ~target with
      | Ok () -> loop rest
      | Error message -> Error (ToolchainInstallFailed { target; error = toolchain_install_error message })
    )
  in
  let* () = loop missing
  in
...
let validate_target_toolchains = fun context targets ->
  ...
  let rec loop = function
    ...
    | target :: rest -> (
      ...
      | Ok _ -> loop rest
      | Error message -> Error (ToolchainInitializationFailed { target; error = toolchain_initialization_error message })
    )
  in
  let* () = loop targets
  in
...
let referenced_hashes_of_artifact = fun (artifact: Riot_store.Artifact.t) -> Std.Crypto.Digest.hex artifact.hash :: List.map artifact.exports ~fn:(
  fun (entry: Riot_store.Manifest.export_entry) -> entry.action_hash
) |> sort_uniq_strings

let generation_lane_of_results = fun ~profile ~target results ->
  let hashes = List.flat_map results ~fn:(
    fun (result: Package_builder.build_result) ->
      match result.status with
      | Package_builder.Built artifact | Package_builder.Cached artifact -> referenced_hashes_of_artifact artifact
      | Package_builder.Skipped _ | Package_builder.Failed _ -> []
  ) |> sort_uniq_strings in Riot_store.Cache_gc.{ profile; target; hashes }

let new_entries_of_results = fun ~profile ~target results -> List.filter_map results ~fn:(
  fun (result: Package_builder.build_result) ->
    match result.status with
    | Package_builder.Built artifact -> Some Riot_store.Cache_gc.{ profile; target; hash = Std.Crypto.Digest.hex artifact.Riot_store.Artifact.hash }
    | Package_builder.Cached _ | Package_builder.Skipped _ | Package_builder.Failed _ -> None
)

let generation_lane_of_result = fun context (lane_result: Lane_result.t) -> generation_lane_of_results ~profile:context.build.profile.name ~target:(Lane_result.target lane_result) (Lane_result.results lane_result)

let new_entries_of_lane_result = fun context (lane_result: Lane_result.t) -> new_entries_of_results ~profile:context.build.profile.name ~target:(Lane_result.target lane_result) (Lane_result.results lane_result)
...
let new_entry_count_of_lane_results = fun context lane_results -> List.map lane_results ~fn:(new_entries_of_lane_result context) |> List.concat |> List.length
...
let record_cache_generation_if_needed = fun context lane_results had_partial_failure ->
  ...
  if context.record_cache_generation && not had_partial_failure then
    (
      ...
      let* () = record_successful_build_cache_generation context lane_results
      in
      if emit_cache_generation_events then
...
let failed_results = fun results -> List.filter results ~fn:(
  fun (result: Package_builder.build_result) ->
    match result.status with
...
let run_lanes = fun context ~toolchain ->
  let* lanes = Build_work.prepare_lanes context.build context.resolved ~toolchain |> Result.map_err ~fn:map_prepare_error
  in
  List.for_each lanes ~fn:(
    fun lane -> emit_target_build_started context (Build_lane.target lane)
  );
  ...
  let () = List.for_each summary.completions ~fn:(
    fun (completion: Build_work.completion) -> emit_target_build_finished context ~target:completion.target ~result_count:completion.result_count ~had_partial_failure:completion.had_partial_failure
  ) in
  if List.length all_errors > 0 then
    ...
    if summary.had_failure && not context.allow_partial_failures then
      let failures = lane_results |> List.map ~fn:(
        fun lane_result -> failed_results (Lane_result.results lane_result)
      ) |> List.concat in Error (BuildFailed { errors = failures })
    else Ok (lane_results, summary.had_failure)
...
let do_build = fun context ->
  ...
  let* () = ensure_toolchains_for_targets context (Resolved_build.targets context.resolved)
  in
  let* () = validate_target_toolchains context (Resolved_build.targets context.resolved)
  in
  emit_runtime_starting context;
  let* toolchain = Riot_toolchain.init ~config:context.build.toolchain_config |> Result.map_err ~fn:(
    fun message -> ToolchainInitializationFailed { target = context.build.host; error = toolchain_initialization_error message }
  )
  in
  emit_runtime_started context;
  let* (lane_results, had_partial_failure) = run_lanes context ~toolchain
  in
  let all_results = List.map ~fn:Lane_result.results lane_results |> List.concat in
  let* () = record_cache_generation_if_needed context lane_results had_partial_failure
  in
...
let execute = fun ?(allow_partial_failures = false) ?(record_cache_generation = true) build spec ->
  let* context = make_context ~allow_partial_failures ~record_cache_generation build spec
  in
  with_telemetry_bridge context
    (
      fun () -> do_build context
    )
