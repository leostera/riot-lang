...
let sandbox_id = fun ~package_name ->
  ...
  let id = Package_name.to_string package_name ^ "-" ^ truncated_hash in Path.v id
...
let create = fun ~workspace ?(profile = "debug") ?(target = Riot_model.Riot_dirs.host_target ()) () ~package_name ->
  let sandbox_dir = Path.(Riot_model.Riot_dirs.sandbox_dir_in_workspace ~workspace ~profile ~target / sandbox_id ~package_name) |> absolute_path in
  Fs.create_dir_all sandbox_dir |> Result.expect ~msg:("Failed to create sandbox dir: " ^ (Path.to_string sandbox_dir));
...
let copy_object_files = fun ~store ~sandbox ~package ~depset ->
  let _ = store in
  let _ = package in
  let depset = Riot_planner.Dependency.transitive_closure depset in List.for_each depset ~fn:(
    fun dep ->
      match Fs.read_dir dep.Riot_planner.Dependency.artifact_dir with
      | Error _ -> ()
      | Ok reader ->
          let entries = Std.Iter.MutIterator.to_list reader in entries |> List.filter ~fn:(
            ...
            fun entry ->
              let src =
                if Path.is_absolute entry then
                  entry
                else Path.(dep.Riot_planner.Dependency.artifact_dir / entry)
              in
              let dest = Path.(sandbox.dir / v (Path.basename entry)) in
              match Fs.copy ~src ~dst:dest with
              | Ok () -> ()
              | Error _ -> Log.warn ("Skipping unavailable dependency object file " ^ Path.to_string src)
          )
  )

let copy_inputs = fun ~sandbox ~package ~inputs -> List.for_each inputs ~fn:(
  fun rel_path ->
    ...
    Fs.create_dir_all dest_parent |> Result.expect ~msg:("Failed to create parent dir: " ^ (Path.to_string dest_parent));
    Fs.copy ~src ~dst:dest |> Result.expect ~msg:("Failed to copy input " ^ Path.to_string src ^ " to " ^ (Path.to_string dest))
)
...
let with_sandbox = fun ~workspace ?(profile = "debug") ?(target = Riot_model.Riot_dirs.host_target ()) ~package ~inputs ~depset ~store ~expected_outputs f ->
  ...
  let result = f sandbox in result
