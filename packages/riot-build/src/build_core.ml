...
let error_message = function
  | TargetSelectionFailed { pattern; available_targets } ->
      ...
      ^ pattern
      ^ "'. Available targets: "
      ^ (available_targets |> List.map ~fn:Riot_model.Target.to_string |> String.concat ", ")
...
let map_runtime_error = function
  ...
  | Build_runtime.ToolchainInitializationFailed { target; error } -> ToolchainInitializationFailed {
    target;
    error
  }
