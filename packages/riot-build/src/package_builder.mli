...
*)
val build: workspace:Workspace.t -> toolchain:Riot_toolchain.t -> store:Riot_store.Store.t -> package_graph:Package_graph.t -> package_key:Package.key -> package:Package.t -> build_ctx:Build_ctx.t -> build_result

val plan_detailed: workspace:Workspace.t -> toolchain:Riot_toolchain.t -> store:Riot_store.Store.t -> package_graph:Package_graph.t -> package_key:Package.key -> package:Package.t -> build_ctx:Build_ctx.t -> plan_outcome

val prepare_execution: workspace:Workspace.t -> toolchain:Riot_toolchain.t -> store:Riot_store.Store.t -> execution_plan:execution_plan -> build_ctx:Build_ctx.t -> (prepared_execution, detailed_result) result

val execute_action: store:Riot_store.Store.t -> prepared_execution:prepared_execution -> build_ctx:Build_ctx.t -> completed:(Graph.SimpleGraph.Node_id.t, Action_executor.execution_result) HashMap.t -> Action_node.t -> Action_executor.execution_result

val finalize_execution: workspace:Workspace.t -> store:Riot_store.Store.t -> prepared_execution:prepared_execution -> completed:(Graph.SimpleGraph.Node_id.t, Action_executor.execution_result) HashMap.t -> build_ctx:Build_ctx.t -> detailed_result
