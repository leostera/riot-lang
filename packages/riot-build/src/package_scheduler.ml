...
let dependency_nodes = fun lane package_key ->
  ...
  | None -> []
  | Some node -> Riot_planner.Package_graph.get_dependencies_for_node
    (Build_lane.package_graph lane)
    node

let dependency_keys = fun lane package_key ->
  dependency_nodes lane package_key |> List.map ~fn:Riot_planner.Package_graph.get_key

let skipped_reason = fun failed_packages ->
  let failed_names = failed_packages
  |> List.map
    ~fn:(fun (package: Riot_model.Package.t) -> Riot_model.Package_name.to_string package.name) in
  "needs " ^ String.concat ", " failed_names

let dependency_failed_state = function
  | Finalized {
    detailed_result={
      result={ status=Package_builder.Failed _ | Package_builder.Skipped _; package; _ };
      _
    };
    _
  } -> Some package
  | Finalized {
    detailed_result={ result={ status=Package_builder.Built _ | Package_builder.Cached _; _ }; _ };
    _
  }
  | AwaitingPlan
...
let remember_action_result = fun package_states lane package_key action_id result ->
  ...
  | Some AwaitingPlan ->
      panic
        ("package scheduler: action result recorded before planning for " ^ package_key_id package_key)
  | Some (Finalized _) ->
      panic
        ("package scheduler: action result recorded after finalization for " ^ package_key_id package_key)
  | None ->
      panic
...
let record_package_state = fun handle lane package_key state ->
  Graph_scheduler.Handle.record handle (Set_package_state { lane; package_key; state })

let record_action_result = fun handle lane package_key (action: Action_node.t) result ->
  Graph_scheduler.Handle.record
    handle
    (Remember_action_result { lane; package_key; action_id = action.id; result })
...
let package_node = fun lane package_key ->
  Riot_planner.Package_graph.get_node_by_key (Build_lane.package_graph lane) package_key

let add_action_work = fun graph ~finalize_node_id lane package_key action_graph ->
  let action_node_ids: (Graph.SimpleGraph.Node_id.t, Graph_scheduler.Node_id.t) HashMap.t = HashMap.create
    () in
  Action_graph.nodes action_graph |> List.for_each
    ~fn:(fun (action: Action_node.t) ->
      let node_id = Graph_scheduler.Handle.add_node
        graph
        ~payload:(ExecuteAction { lane; package_key; action }) in
      let _ = HashMap.insert action_node_ids ~key:action.id ~value:node_id in
      ());
  Action_graph.nodes action_graph |> List.for_each
    ~fn:(fun (action: Action_node.t) ->
      let action_node_id = HashMap.get action_node_ids ~key:action.id
      |> Option.expect
        ~msg:("missing scheduler node for action " ^ Graph.SimpleGraph.Node_id.to_string action.id) in
      List.for_each action.deps
        ~fn:(fun dependency_id ->
          let dependency_node_id = HashMap.get action_node_ids ~key:dependency_id
          |> Option.expect
            ~msg:("missing scheduler dependency node for action "
            ^ Graph.SimpleGraph.Node_id.to_string dependency_id) in
          Graph_scheduler.Handle.add_dependency graph ~node:action_node_id ~depends_on:dependency_node_id);
          ...
          | Package_builder.Execution_required execution_plan ->
              ...
              (
                ...
                | Ok prepared_execution ->
                    let finalize_node_id = HashMap.get
                      node_ids
                      ~key:(finalize_node_key lane package_key)
                    |> Option.expect
                      ~msg:("missing finalize node for " ^ package_state_key lane package_key) in
                    record_package_state
                      graph
                      lane
                      package_key
...
let execute_action_work = fun ~package_states ~graph lane package_key action ->
  match get_package_state package_states lane package_key with
  | Some (AwaitingFinalization execution_state) ->
      let result = Package_builder.execute_action
        ~store:(Build_lane.store lane)
      ...
      Ok (Executed_action { lane; package_key; action; result })
  | Some AwaitingPlan ->
      Error {
        lane;
...
let finalize_package_work = fun ~package_states ~graph lane package_key ->
  ...
  | Some (AwaitingFinalization execution_state) ->
      let detailed_result = Package_builder.finalize_execution
        ~workspace:(Build_lane.workspace lane)
        ~store:(Build_lane.store lane)
        ~prepared_execution:execution_state.prepared_execution
        ~completed:execution_state.completed_actions
        ~build_ctx:(Build_lane.build_ctx lane) in
  ...
  | None ->
      Error { lane; reason = "package state missing for finalization " ^ package_key_id package_key }
...
let make_state = fun lanes ->
  ...
  List.for_each
    ...
    ~fn:(fun lane ->
      ...
      |> List.for_each
        ~fn:(fun package_key -> remember_package_state package_states lane package_key AwaitingPlan));
  { package_states }
...
let make_graph = fun state lanes ->
  let graph =
    Graph_scheduler.Graph.create
      ~apply_mutation:(fun _ mutation ->
        match mutation with
        | Apply_graph_update { lane; package_key; package; graph_update } -> Package_builder.apply_graph_update
          (Build_lane.package_graph lane)
          package_key
          package
          graph_update
        | Set_package_state { lane; package_key; state=next_state } -> remember_package_state
          state.package_states
          lane
          package_key
          next_state
        | Remember_action_result { lane; package_key; action_id; result } -> remember_action_result
          state.package_states
          lane
          package_key
          action_id
  ...
  List.for_each lanes
    ~fn:(fun lane ->
      Build_lane.package_keys lane |> List.for_each
        ~fn:(fun package_key ->
          let plan_node_id = Graph_scheduler.Graph.add_node
            graph
            ~payload:(PlanPackage { lane; package_key }) in
          let finalize_node_id = Graph_scheduler.Graph.add_node
            graph
            ~payload:(FinalizePackage { lane; package_key }) in
  ...
  List.for_each lanes
    ~fn:(fun lane ->
      Build_lane.package_keys lane |> List.for_each
        ~fn:(fun package_key ->
          let plan_node_id = HashMap.get node_ids ~key:(plan_node_key lane package_key)
          |> Option.expect ~msg:("missing plan node for " ^ package_state_key lane package_key) in
          let finalize_node_id = HashMap.get node_ids ~key:(finalize_node_key lane package_key)
          |> Option.expect ~msg:("missing finalize node for " ^ package_state_key lane package_key) in
          Graph_scheduler.Graph.add_dependency graph ~node:finalize_node_id ~depends_on:plan_node_id;
          dependency_keys lane package_key |> List.for_each
            ~fn:(fun dependency_key ->
              match HashMap.get node_ids ~key:(finalize_node_key lane dependency_key) with
              | Some dependency_finalize_node_id -> Graph_scheduler.Graph.add_dependency
                graph
                ~node:plan_node_id
      ...
      Build_lane.package_keys lane |> List.fold_left ~init:counts
        ~fn:(fun counts package_key ->
          ...
          | Some (AwaitingFinalization _) -> {
            counts
            with awaiting_finalization = counts.awaiting_finalization + 1
          }
...
let pending_descriptions = fun lanes package_states ->
  lanes |> List.flat_map
    ~fn:(fun lane ->
      Build_lane.package_keys lane |> List.filter_map
        ~fn:(fun package_key ->
          match get_package_state package_states lane package_key with
...
let stalled_errors = fun lanes package_states ->
  ...
  if pending = [] then
    ...
    let reason = "package scheduler made no progress with pending work: " ^ String.concat ", " pending in
    List.map lanes ~fn:(fun lane -> { lane; reason })

let cleanup_pending_execution = fun package_states ->
  HashMap.to_list package_states |> List.for_each
    ~fn:(fun (_, state) ->
      match state with
  ...
  List.fold_left results
    ...
    ~fn:(fun counts (result: (work_item, output, error) Graph_scheduler.node_result) ->
      ...
      | PlanPackage _ -> (
          ...
          | Error _ ->
              { counts with error_count = counts.error_count + 1 }
          ...
          | Ok (Planned (PlanningFinalized { detailed_result; _ })) ->
              ...
              (
                ...
                | Package_builder.Skipped _ -> {
                  counts
                  with skipped_count = counts.skipped_count + 1
                }
...
let lane_result_of_states = fun package_states lane ->
  ...
  if package_results = [] then
    ...
    in
    Some (
      { target = Build_lane.target lane; results = package_results; had_partial_failure }:
        Lane_result.t
    )
...
let summarize = fun lanes package_states errors ->
  ...
  { completions; lane_results; errors; had_failure }
      ...
      let results =
        Graph_scheduler.run ~config:(Graph_scheduler.Run_config.make
          ...
          ~execute:(fun ~graph ~node:_ ~payload ->
            match payload with
            | PlanPackage { lane; package_key } -> plan_package_work
              ~package_states:state.package_states
              ~node_ids
              ~graph
              lane
              package_key
            | ExecuteAction { lane; package_key; action } -> execute_action_work
              ~package_states:state.package_states
              ~graph
              lane
              package_key
              action
            | FinalizePackage { lane; package_key } -> finalize_package_work
              ~package_states:state.package_states
              ~graph
              lane
              package_key)
