...
let plan_node_key = fun lane package_key ->
  package_state_key lane package_key ^ "#plan"

let finalize_node_key = fun lane package_key ->
  package_state_key lane package_key ^ "#finalize"
...
let remember_action_result = fun package_states lane package_key action_id result ->
  ...
  | Some (AwaitingFinalization execution_state) ->
      ...
      ()
  | Some AwaitingPlan ->
      panic ("package scheduler: action result recorded before planning for " ^ package_key_id package_key)
  | Some (Finalized _) ->
      panic ("package scheduler: action result recorded after finalization for " ^ package_key_id package_key)
  | None ->
      panic ("package scheduler: missing package state for action result " ^ package_key_id package_key)
...
let finalized_state = fun ~source detailed_result ->
  Finalized { source; detailed_result }
...
let add_action_work = fun graph ~finalize_node_id lane package_key action_graph ->
  ...
  |> List.for_each ~fn:(fun (action: Action_node.t) ->
    let node_id =
      Graph_scheduler.Handle.add_node graph ~payload:(ExecuteAction {
        lane;
        package_key;
        action;
      })
  ...
  |> List.for_each ~fn:(fun (action: Action_node.t) ->
    ...
    in
    List.for_each action.deps ~fn:(fun dependency_id ->
...
let make_graph = fun state lanes ->
  ...
  List.for_each lanes
    ~fn:(fun lane ->
      ...
      |> List.for_each ~fn:(fun package_key ->
        let plan_node_id =
          Graph_scheduler.Graph.add_node
            graph
            ~payload:(PlanPackage { lane; package_key })
        in
        let finalize_node_id =
          Graph_scheduler.Graph.add_node
            graph
            ~payload:(FinalizePackage { lane; package_key })
        in
        let _ = HashMap.insert
          node_ids
  ...
  List.for_each lanes
    ~fn:(fun lane ->
      ...
      |> List.for_each ~fn:(fun package_key ->
        ...
        let finalize_node_id =
          HashMap.get node_ids ~key:(finalize_node_key lane package_key)
          |> Option.expect ~msg:("missing finalize node for " ^ package_state_key lane package_key)
        in
...
let total_package_count = fun lanes ->
  lanes
  |> List.fold_left ~acc:0 ~fn:(fun count lane ->
    count + List.length (Build_lane.package_keys lane))
...
let pending_counts = fun lanes package_states ->
  lanes
  |> List.fold_left
    ~acc:{ awaiting_plan = 0; awaiting_finalization = 0; finalized = 0; }
    ~fn:(fun counts lane ->
      Build_lane.package_keys lane
        ...
        Graph_scheduler.run
          ...
          ~execute:(fun ~graph ~node:_ ~payload ->
            ...
            | PlanPackage { lane; package_key } ->
                plan_package_work ~package_states:state.package_states ~node_ids ~graph lane package_key
            | ExecuteAction { lane; package_key; action } ->
                execute_action_work ~package_states:state.package_states ~graph lane package_key action
