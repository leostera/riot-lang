...
let run_package_scheduler = fun request ->
  with_prepared_lanes request
    (fun lanes ->
      let events = ref [] in
      let summary =
        Package_scheduler.run ~parallelism:1 ~on_event:(fun event -> events := event :: !events) lanes
      in
...
let test_package_scheduler_follows_finalized_dependency_frontiers = fun _ctx ->
  match
    Fs.with_tempdir ~prefix:"riot_package_scheduler_frontiers"
      (fun tmpdir ->
        ...
        let summary, events = run_package_scheduler
          (make_request ~workspace ~packages:[ package_name "app" ] ()) in
        if summary.errors != [] then
          ...
          Test.assert_equal ~expected:[ (3, 2, 3, 0, 0, 0, 0, 0) ] ~actual:planning_counts;
          Test.assert_equal ~expected:[ (3, 3, 3, 0, 0) ] ~actual:execution_counts;
          Ok ())
  with
  | Ok result -> result
...
let test_package_scheduler_succeeds_for_dependency_chain = fun _ctx ->
  match
    Fs.with_tempdir ~prefix:"riot_package_scheduler_success"
      (fun tmpdir ->
        let lib = make_package ~root:tmpdir ~name:"lib" ~source:"let value = 1\n" () in
        let app = make_package
          ~root:tmpdir
        ...
        let summary, _events = run_package_scheduler
          (make_request ~workspace ~packages:[ package_name "app" ] ()) in
        if summary.errors != [] then
          Error "expected package scheduler run to avoid internal errors"
        else
          let lane_result = lane_result summary in
          let result_keys = Lane_result.results lane_result
          |> List.map
            ~fn:(fun (result: Package_builder.build_result) -> package_key_string result.package_key) in
          let expected_keys = [
...
let test_package_scheduler_skips_dependents_after_failed_dependency = fun _ctx ->
  match
    Fs.with_tempdir ~prefix:"riot_package_scheduler_failure"
      (fun tmpdir ->
        ...
        if summary.errors != [] then
          ...
          if planning_counts != [ (2, 1, 1, 1, 0, 1, 0, 0) ] then
            ...
            | _ -> Error ("expected dependency failure to produce failed then skipped package results, got "
...
let test_package_scheduler_phase_events_have_consistent_counts = fun _ctx ->
  match
    Fs.with_tempdir ~prefix:"riot_package_scheduler_event_counts"
      (fun tmpdir ->
        let lib = make_package ~root:tmpdir ~name:"lib" ~source:"let value = 1\n" () in
        let app = make_package
          ~root:tmpdir
        ...
        if summary.errors != [] then
          ...
          let planning_counts = planning_phase_counts events in
          let execution_counts = execution_phase_counts events in
          let planning_ok = planning_counts
          |> List.all
            ~fn:(fun (package_count, deferred_count, execution_required_count, finalized_count, cached_count, skipped_count, failed_count, error_count) ->
              ...
              && failed_count <= finalized_count) in
          let execution_ok = execution_counts
          |> List.all
...
let test_package_scheduler_keeps_multi_lane_results_isolated = fun _ctx ->
  match
    Fs.with_tempdir ~prefix:"riot_package_scheduler_multi_lane"
      (fun tmpdir ->
        ...
        if summary.errors != [] then
          ...
          let all_result_counts_are_one = summary.Package_scheduler.completions
          |> List.all
            ~fn:(fun (completion: Package_scheduler.completion) -> completion.result_count = 1) in
          let all_lane_results_are_singletons = summary.Package_scheduler.lane_results
          |> List.all ~fn:(fun lane_result -> List.length (Lane_result.results lane_result) = 1) in
...
let test_package_scheduler_rerun_uses_cached_dependency_frontiers = fun _ctx ->
  match
    Fs.with_tempdir ~prefix:"riot_package_scheduler_cached_rerun"
      (fun tmpdir ->
        let lib = make_package ~root:tmpdir ~name:"lib" ~source:"let value = 1\n" () in
        let app = make_package
          ~root:tmpdir
        ...
        if first_summary.errors != [] then
          ...
          if not all_cached then
            ...
            Test.assert_equal ~expected:[ (2, 1, 0, 2, 2, 0, 0, 0) ] ~actual:planning;
            Ok ()
          ))
  with
  | Ok result -> result
...
let test_package_scheduler_cached_rerun_preserves_multi_lane_isolation = fun _ctx ->
  match
    Fs.with_tempdir ~prefix:"riot_package_scheduler_cached_multi_lane"
      (fun tmpdir ->
        let host_target = Riot_model.Target.current in
        let secondary_target =
          if String.equal (Riot_model.Target.to_string host_target) "x86_64-unknown-linux-gnu" then
        ...
        if first_summary.errors != [] then
          ...
          let all_cached =
            ...
            |> List.all
              ~fn:(fun lane_result ->
                result_statuses lane_result |> List.all
                  ~fn:(
                    function
                    | Package_builder.Cached _ -> true
                    | Package_builder.Built _
          ...
          in
          let all_result_counts_are_one = second_summary.Package_scheduler.completions
          |> List.all
            ~fn:(fun (completion: Package_scheduler.completion) -> completion.result_count = 1) in
          if actual_targets != expected_targets then
...
let test_package_scheduler_reports_stalled_pending_work = fun _ctx ->
  match
    Fs.with_tempdir ~prefix:"riot_package_scheduler_stall"
      (fun tmpdir ->
        let lib = make_package ~root:tmpdir ~name:"lib" ~source:"let value = 1\n" () in
        let app = make_package
          ~root:tmpdir
        ...
        with_prepared_lanes request
          (fun lanes ->
            ...
            | [ lane ] ->
                ...
                let lib_node = Package_graph.get_node_by_key package_graph lib_key
                |> Option.expect ~msg:"expected lib node in prepared package graph" in
                let app_node = Package_graph.get_node_by_key package_graph app_key
                |> Option.expect ~msg:"expected app node in prepared package graph" in
                ...
                let stalled_errors = summary.Package_scheduler.errors
                |> List.all
                  ~fn:(fun (error: Package_scheduler.error) ->
                    String.contains error.reason "made no progress" && String.contains error.reason "awaiting plan") in
                let completion_ok =
                ...
                if not summary.Package_scheduler.had_failure then
                  ...
                  Test.assert_equal ~expected:[ (2, 2, 0, 0, 0, 0, 0, 0) ] ~actual:planning_counts;
                  Ok ()
                )
...
let tests =
  let open Test in [
    case "package scheduler: follows finalized dependency frontiers" test_package_scheduler_follows_finalized_dependency_frontiers;
    case "package scheduler: succeeds for dependency chain" test_package_scheduler_succeeds_for_dependency_chain;
    ...
    case "package scheduler: phase events have consistent counts" test_package_scheduler_phase_events_have_consistent_counts;
    case "package scheduler: keeps multi-lane results isolated" test_package_scheduler_keeps_multi_lane_results_isolated;
    case "package scheduler: rerun uses cached dependency frontiers" test_package_scheduler_rerun_uses_cached_dependency_frontiers;
