...
let package_dependency = fun name -> Riot_model.Package.{
...
let make_package = fun ~root ~name ~source ?(dependencies = []) () ->
  ...
  Riot_model.Package.make ~name:pkg_name ~path:pkg_dir ~relative_path:(Path.v name) ~dependencies ~library:{ path = Path.v "src/lib.ml" } ~sources:{
...
let make_request = fun ~workspace ~packages ?(targets = Riot_model.Target.Host) ?(requested_parallelism = None) () -> Riot_build.Request.make ~workspace ~packages ~targets ~scope:Riot_build.Request.Runtime ~profile:Riot_model.Profile.debug ~requested_parallelism ()
...
let run_package_scheduler = fun request ->
  with_prepared_lanes request
    (
      fun lanes ->
        let events = ref [] in
        let summary = Package_scheduler.run ~parallelism:1 ~on_event:(
          fun event -> events := event :: !events
        ) lanes in (summary, List.reverse !events)
    )
...
let lane_result = fun summary ->
  ...
  | results -> panic ("expected exactly one lane result, got " ^ Int.to_string (List.length results))

let result_statuses = fun lane_result -> Lane_result.results lane_result |> List.map ~fn:(
  fun (result: Package_builder.build_result) -> result.status
)
...
let planning_phase_counts = fun events -> events |> List.filter_map ~fn:(
  function
  | Package_scheduler.PlanningFinished { package_count; deferred_count; execution_required_count; finalized_count; cached_count; skipped_count; failed_count; error_count; _ } -> Some (package_count, deferred_count, execution_required_count, finalized_count, cached_count, skipped_count, failed_count, error_count)
  | _ -> None
)

let execution_phase_counts = fun events -> events |> List.filter_map ~fn:(
  function
  | Package_scheduler.ExecutionFinished { package_count; finalized_count; built_count; failed_count; error_count; _ } -> Some (package_count, finalized_count, built_count, failed_count, error_count)
  | _ -> None
)

let lane_targets = fun summary -> summary.Package_scheduler.lane_results |> List.map ~fn:Lane_result.target |> List.sort ~compare:Riot_model.Target.compare
  ...
  match Fs.with_tempdir ~prefix:"riot_package_scheduler_frontiers"
    (
      fun tmpdir ->
        ...
        let mid = make_package ~root:tmpdir ~name:"mid" ~dependencies:[ package_dependency "lib" ] ~source:"let value = Lib.value\n" () in
        let app = make_package ~root:tmpdir ~name:"app" ~dependencies:[ package_dependency "mid" ] ~source:"let value = Mid.value\n" () in
        let workspace = make_workspace ~root:tmpdir ~packages:[ lib; mid; app ] () in
        let summary, events = run_package_scheduler (make_request ~workspace ~packages:[ package_name "app" ] ()) in
        if summary.errors != [] then
          Error "expected dependency frontier scheduling to avoid internal errors"
        else
          if summary.had_failure then
            Error "expected dependency frontier scheduling to succeed"
          else
            let planning_counts = planning_phase_counts events in
            let execution_counts = execution_phase_counts events in Test.assert_equal ~expected:[
              3, 2, 3, 0, 0, 0, 0, 0;
...
let test_package_scheduler_succeeds_for_dependency_chain = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_package_scheduler_success"
    (
      fun tmpdir ->
        let lib = make_package ~root:tmpdir ~name:"lib" ~source:"let value = 1\n" () in
        let app = make_package ~root:tmpdir ~name:"app" ~dependencies:[ package_dependency "lib" ] ~source:"let value = Lib.value\n" () in
        let workspace = make_workspace ~root:tmpdir ~packages:[ lib; app ] () in
        let summary, _events = run_package_scheduler (make_request ~workspace ~packages:[ package_name "app" ] ()) in
        if summary.errors != [] then
          Error "expected package scheduler run to avoid internal errors"
        else
          let lane_result = lane_result summary in
          let result_keys = Lane_result.results lane_result |> List.map ~fn:(
            fun (result: Package_builder.build_result) -> package_key_string result.package_key
          ) in
          let expected_keys = [ Package_graph.package_key ~package_name:"lib" Package_graph.Runtime |> package_key_string; Package_graph.package_key ~package_name:"app" Package_graph.Runtime |> package_key_string ] in
          let completed_all = result_statuses lane_result |> List.all ~fn:(
            function
            | Package_builder.Built _ | Package_builder.Cached _ -> true
            | Package_builder.Skipped _ | Package_builder.Failed _ -> false
          ) in
...
let test_package_scheduler_skips_dependents_after_failed_dependency = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_package_scheduler_failure"
    (
      fun tmpdir ->
        let lib = make_package ~root:tmpdir ~name:"lib" ~source:"let value : int = \"not an int\"\n" () in
        let app = make_package ~root:tmpdir ~name:"app" ~dependencies:[ package_dependency "lib" ] ~source:"let value = Lib.value\n" () in
        let workspace = make_workspace ~root:tmpdir ~packages:[ lib; app ] () in
        let summary, events = run_package_scheduler (make_request ~workspace ~packages:[ package_name "app" ] ()) in
        if summary.errors != [] then
            ...
            if execution_counts != [
              ...
              | [ Package_builder.Failed _; Package_builder.Skipped _ ] ->
                  if summary.had_failure && Lane_result.had_partial_failure lane_result then
                    Ok ()
                  else Error "expected failing dependency lane to report partial failure"
              | _ -> Error ("expected dependency failure to produce failed then skipped package results, got " ^ String.concat ", " (List.map statuses ~fn:status_label))
    ) with
  | Ok result -> result
  | Error err -> Error ("tempdir failed: " ^ IO.error_message err)
    ...
    (
      fun tmpdir ->
        ...
        if summary.errors != [] then
          ...
          let planning_counts = planning_phase_counts events in
          let execution_counts = execution_phase_counts events in
          let planning_ok = planning_counts |> List.all ~fn:(
            fun (package_count, deferred_count, execution_required_count, finalized_count, cached_count, skipped_count, failed_count, error_count) -> package_count = execution_required_count + finalized_count + error_count && deferred_count <= package_count && cached_count <= finalized_count && skipped_count <= finalized_count && failed_count <= finalized_count
          ) in
          let execution_ok = execution_counts |> List.all ~fn:(
            fun (package_count, finalized_count, built_count, failed_count, error_count) -> package_count = finalized_count + error_count && built_count <= finalized_count && failed_count <= finalized_count
          ) in
  ...
  match Fs.with_tempdir ~prefix:"riot_package_scheduler_multi_lane"
    (
      fun tmpdir ->
        ...
        let workspace = make_workspace ~root:tmpdir ~toolchain_targets:(Riot_model.Target.Set.to_list requested_targets |> List.map ~fn:Riot_model.Target.to_string) ~packages:[ make_package ~root:tmpdir ~name:"demo" ~source:"let value = 42\n" () ] () in
        let summary, events = run_package_scheduler (make_request ~workspace ~packages:[ package_name "demo" ] ~targets:(Riot_model.Target.Exact requested_targets) ~requested_parallelism:(Some 1) ()) in
        if summary.errors != [] then
          Error "expected multi-lane package scheduler run to avoid internal errors"
        else
          if List.length summary.Package_scheduler.lane_results != 2 then
            Error "expected one lane result per requested target"
          else
            if List.length summary.Package_scheduler.completions != 2 then
              Error "expected one completion per requested target"
            else
              let actual_targets = lane_targets summary in
              let expected_targets = Riot_model.Target.Set.to_list requested_targets |> List.sort ~compare:Riot_model.Target.compare in
              let all_result_counts_are_one = summary.Package_scheduler.completions |> List.all ~fn:(
                fun (completion: Package_scheduler.completion) -> completion.result_count = 1
              ) in
              let all_lane_results_are_singletons = summary.Package_scheduler.lane_results |> List.all ~fn:(
                fun lane_result -> List.length (Lane_result.results lane_result) = 1
              ) in
              let lane_count_events_ok = events |> List.filter_map ~fn:(
                function
                | Package_scheduler.PlanningStarted { lane_count; _ } | Package_scheduler.PlanningFinished { lane_count; _ } | Package_scheduler.ExecutionStarted { lane_count; _ } | Package_scheduler.ExecutionFinished { lane_count; _ } -> Some lane_count
              ) |> List.all ~fn:(
                fun lane_count -> lane_count = 2
              ) in
              if actual_targets != expected_targets then
                Error "expected multi-lane scheduler results to preserve requested targets"
              else
                if not all_result_counts_are_one then
                  Error "expected each multi-lane completion to report one package result"
                else
                  if not all_lane_results_are_singletons then
                    Error "expected each multi-lane lane result to stay isolated to one package"
                  else
                    if not lane_count_events_ok then
    ...
    (
      fun tmpdir ->
        ...
        if first_summary.errors != [] then
          ...
          if second_summary.errors != [] then
            Error "expected cached rerun to avoid internal scheduler errors"
          else
            if second_summary.had_failure then
              ...
              let all_cached = List.all statuses ~fn:(
                function
                | Package_builder.Cached _ -> true
                | Package_builder.Built _ | Package_builder.Skipped _ | Package_builder.Failed _ -> false
              ) in
              if not all_cached then
                Error "expected cached rerun to finalize every package from cache"
              else
                if execution_rounds != [] then
  ...
  match Fs.with_tempdir ~prefix:"riot_package_scheduler_cached_multi_lane"
    (
      fun tmpdir ->
        ...
        let workspace = make_workspace ~root:tmpdir ~toolchain_targets:(Riot_model.Target.Set.to_list requested_targets |> List.map ~fn:Riot_model.Target.to_string) ~packages:[ demo ] () in
        let request = make_request ~workspace ~packages:[ package_name "demo" ] ~targets:(Riot_model.Target.Exact requested_targets) ~requested_parallelism:(Some 1) () in
        let first_summary, _first_events = run_package_scheduler request in
        let second_summary, second_events = run_package_scheduler request in
        if first_summary.errors != [] then
          ...
          if second_summary.errors != [] then
            ...
            if second_summary.had_failure then
              ...
              if List.length second_summary.Package_scheduler.lane_results != 2 then
                Error "expected cached multi-lane rerun to keep one result per lane"
              else
                let actual_targets = lane_targets second_summary in
                let expected_targets = Riot_model.Target.Set.to_list requested_targets |> List.sort ~compare:Riot_model.Target.compare in
                let execution_rounds = execution_phase_counts second_events in
                let all_cached = second_summary.Package_scheduler.lane_results |> List.all ~fn:(
                  fun lane_result -> result_statuses lane_result |> List.all ~fn:(
                    function
                    | Package_builder.Cached _ -> true
                    | Package_builder.Built _ | Package_builder.Skipped _ | Package_builder.Failed _ -> false
                  )
                ) in
                let all_result_counts_are_one = second_summary.Package_scheduler.completions |> List.all ~fn:(
                  fun (completion: Package_scheduler.completion) -> completion.result_count = 1
                ) in
                if actual_targets != expected_targets then
          ...
          (
            fun lanes ->
              ...
              | [ lane ] ->
                  ...
                  let lib_node = Package_graph.get_node_by_key package_graph lib_key |> Option.expect ~msg:"expected lib node in prepared package graph" in
                  let app_node = Package_graph.get_node_by_key package_graph app_key |> Option.expect ~msg:"expected app node in prepared package graph" in
                  ...
                  let stalled_errors = summary.Package_scheduler.errors |> List.all ~fn:(
                    fun (error: Package_scheduler.error) -> String.contains error.reason "made no progress" && String.contains error.reason "awaiting plan"
                  ) in
                  let completion_ok =
              ...
              | _ -> Error "expected single-lane scheduler test setup"
          )
    ) with
  | Ok result -> result
  | Error err -> Error ("tempdir failed: " ^ IO.error_message err)
