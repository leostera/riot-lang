...
let test_toolchain = Riot_toolchain.init ~config:Riot_model.Toolchain_config.default |> Result.expect ~msg:"Failed to initialize test toolchain"
...
let test_collect_source_files = fun _ctx ->
  match Fs.with_tempdir ~prefix:"pkg_test"
    (
      fun tmpdir ->
        ...
        let package = Riot_model.Package.make ~name:(package_name "test") ~path:tmpdir ~relative_path:(Path.v ".") () in
        let files = Package_builder.collect_source_files package in
        let has_ml = List.any files ~fn:(
          fun p -> Path.to_string p = Path.to_string ml_file
        ) in
        let has_mli = List.any files ~fn:(
          fun p -> Path.to_string p = Path.to_string mli_file
        ) in
        let has_c = List.any files ~fn:(
          fun p -> Path.to_string p = Path.to_string c_file
        ) in
        let has_txt = List.any files ~fn:(
        ...
        if has_ml && has_mli && has_c && (not has_txt) && count = 3 then
          Ok ()
        else Error ("Expected exactly .ml, .mli, .c files. Got " ^ Int.to_string count ^ " files: has_ml=" ^ Bool.to_string has_ml ^ " has_mli=" ^ Bool.to_string has_mli ^ " has_c=" ^ Bool.to_string has_c ^ " has_txt=" ^ Bool.to_string has_txt ^ ". Files: " ^ String.concat ", " (List.map files ~fn:(
          fun p -> Path.basename p
        )))
    ) with
  | Ok r -> r
...
let test_build_result_status_variants = fun _ctx ->
  let package = Riot_model.Package.make ~name:(package_name "test") ~path:(Path.v ".") ~relative_path:(Path.v ".") () in
  let artifact = Riot_store.Artifact.{
    hash = Crypto.hash_string "test";
    files = [];
    ocamlc_warnings = [];
    exports = []
  } in
  let result_cached = Package_builder.{
    package_key = Riot_planner.Package_graph.package_key ~package_name:(Package_name.to_string package.name) Riot_planner.Package_graph.Runtime;
    package;
    status = Cached artifact;
    ocamlc_warnings = [];
    duration = Time.Duration.from_millis 5
  } in
  let result_built = Package_builder.{
    package_key = Riot_planner.Package_graph.package_key ~package_name:(Package_name.to_string package.name) Riot_planner.Package_graph.Runtime;
    package;
  ...
  let result_failed = Package_builder.{
    package_key = Riot_planner.Package_graph.package_key ~package_name:(Package_name.to_string package.name) Riot_planner.Package_graph.Runtime;
    package;
...
let test_build_writes_hash_manifest_with_exports = fun _ctx ->
  match Fs.with_tempdir ~prefix:"pkg_builder_export_manifest_test"
    (
      fun tmpdir ->
        ...
        let package = Riot_model.Package.make ~name:(package_name "pkg") ~path:package_dir ~relative_path:(Path.v "pkg") ~library:{ path = Path.v "src/lib.ml" } ~sources:{
        ...
        let build_ctx =
          let session_id = Riot_model.Session_id.make () in Riot_model.Build_ctx.make ~session_id ~profile:Riot_model.Profile.debug ()
        in
        let result = Package_builder.build ~workspace ~toolchain:test_toolchain ~store ~package_graph ~package_key:(Riot_planner.Package_graph.package_key ~package_name:(Package_name.to_string package.name) Riot_planner.Package_graph.Runtime) ~package ~build_ctx in
        match result.status with
        | Package_builder.Failed err -> Error ("build failed: " ^ Package_builder.package_error_to_string err)
        | Package_builder.Skipped { reason } -> Error ("build skipped: " ^ reason)
        | Package_builder.Built artifact | Package_builder.Cached artifact ->
            ...
            | Some manifest ->
                if List.length manifest.Riot_store.Manifest.exports > 0 then
                  Ok ()
                else Error "expected hash manifest to include exported outputs"
    ) with
  | Ok r -> r
  | Error _ -> Error "Tempdir creation failed"
...
let make_library_package = fun ~root ~name ?(dependencies = []) source ->
  ...
  let _ = Fs.create_dir_all src_dir |> Result.expect ~msg:"create src dir failed" in
  let _ = Fs.write source source_path |> Result.expect ~msg:"write source failed" in Riot_model.Package.make ~name:(package_name name) ~path:package_dir ~relative_path:(Path.v name) ~dependencies:(List.map dependencies ~fn:workspace_dependency) ~library:{ path = Path.v ("src/" ^ name ^ ".ml") } ~sources:{
...
let test_dependency_source_change_rebuilds_dependent_package = fun _ctx ->
  match Fs.with_tempdir ~prefix:"pkg_builder_dep_rebuild"
    (
      fun tmpdir ->
        ...
        let workspace = Riot_model.Workspace.make_realized ~root:tmpdir ~packages:[ dep; app ] ~target_dir:"target" () in
        let store = Riot_store.Store.create ~workspace in
        let build_ctx = make_test_build_ctx () in
        let build_package ~package_graph package = Package_builder.build ~workspace ~toolchain:test_toolchain ~store ~package_graph ~package_key:(Riot_planner.Package_graph.package_key ~package_name:(Package_name.to_string package.Riot_model.Package.name) Riot_planner.Package_graph.Runtime) ~package ~build_ctx in
        let first_graph = Riot_planner.Package_graph.create ~scope:Riot_planner.Package_graph.Runtime workspace |> Result.unwrap in
        ...
        let second_graph = Riot_planner.Package_graph.create ~scope:Riot_planner.Package_graph.Runtime workspace |> Result.unwrap in
        ...
        | _, _, _, Package_builder.Failed err -> Error ("dependent rebuild failed: " ^ Package_builder.package_error_to_string err)
        | _, Package_builder.Failed err, _, _ -> Error ("initial dependent build failed: " ^ Package_builder.package_error_to_string err)
        | Package_builder.Failed err, _, _, _ -> Error ("initial dependency build failed: " ^ Package_builder.package_error_to_string err)
        | _, _, Package_builder.Failed err, _ -> Error ("dependency rebuild failed: " ^ Package_builder.package_error_to_string err)
