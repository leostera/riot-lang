...
let make_valid_workspace = fun ?(package_names = [ "demo" ]) ?toolchain_targets tmpdir ->
  ...
  | Some targets ->
      write_toolchain_config ~root:tmpdir ~targets
  | None ->
      ());
    ...
    Fs.with_tempdir
      ...
      (fun tmpdir ->
        ...
        | Ok output -> (
            ...
            | None ->
                Error "expected output for package demo"
          ))
  with
  | Ok result -> result
  | Error err -> Error ("tempdir failed: " ^ IO.error_message err)
    ...
    Fs.with_tempdir
      ...
      (fun tmpdir ->
        ...
        | Error (Riot_build.TargetSelectionFailed { pattern; _ }) when String.equal pattern "linux" ->
            Ok ()
        | Error err ->
            Error ("expected target-selection error, got: "
            ^ Riot_build.error_message err)
        | Ok _ ->
            Error "expected target selection to fail before execution")
  with
  | Ok result -> result
  | Error err -> Error ("tempdir failed: " ^ IO.error_message err)
  ...
  match
    Fs.with_tempdir
      ...
      (fun tmpdir ->
        ...
        | Error err ->
            Error ("expected compiler failure, got: "
            ^ Riot_build.error_message err)
        | Ok _ ->
    ...
    Fs.with_tempdir
      ...
      (fun tmpdir ->
        ...
        match
          build_request
            ~on_event:(function
              | Riot_build.Event.Phase (Riot_build.Event.TargetsResolved { target_count = count }) ->
        ...
        with
        | Error err ->
            Error ("expected exact target build to succeed, got: "
            ^ Riot_build.error_message err)
        | Ok _ -> (
            ...
            | Some count ->
                ...
                Ok ()
            | None ->
                Error "expected targets_resolved event"
          ))
  with
  | Ok result -> result
  | Error err -> Error ("tempdir failed: " ^ IO.error_message err)
...
let tests =
  ...
  [
    case
      "build core: output maps build result statuses"
      test_output_maps_build_result_statuses;
    case
    ...
    case
      "build core: build reports missing single package"
      test_build_reports_missing_single_package;
    case
