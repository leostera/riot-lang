...
let target = fun value -> Riot_model.Target.from_string value |> Result.expect ~msg:("invalid target triple: " ^ value)
...
let write_workspace_manifest = fun ~root ~members ->
  let members = members |> List.map ~fn:(
    fun member -> "  \"" ^ Path.to_string member ^ "\""
  ) |> String.concat ",\n" in
  let content = "[workspace]\nmembers = [\n" ^ members ^ "\n]\n" in Fs.write content Path.(root / Path.v "riot.toml") |> Result.expect ~msg:"Write workspace riot.toml failed"

let write_toolchain_config = fun ~root ~targets ->
  let target_lines = targets |> List.map ~fn:(
    fun target -> "\"" ^ target ^ "\""
  ) |> String.concat ", " in Fs.write ("[toolchain]\nversion = \"" ^ Riot_model.Toolchain_config.default_ocaml_version ^ "\"\ntargets = [" ^ target_lines ^ "]\n") Path.(root / Path.v "ocaml-toolchain.toml") |> Result.expect ~msg:"Write ocaml-toolchain.toml failed"
...
let make_package = fun ~root ~name ~value ->
  ...
  Fs.write ("[package]\nname = \"" ^ name ^ "\"\nversion = \"0.0.1\"\n\n[lib]\npath = \"src/lib.ml\"\n") Path.(pkg_dir / Path.v "riot.toml") |> Result.expect ~msg:"Write riot.toml failed";
  Riot_model.Package.make ~name:package_name ~path:pkg_dir ~relative_path:(Path.v name) ~library:{ path = Path.v "src/lib.ml" } ~sources:{
...
let make_valid_workspace = fun ?(package_names = ["demo"]) ?toolchain_targets tmpdir ->
  let packages = map_with_index package_names ~fn:(
    fun index name -> make_package ~root:tmpdir ~name ~value:(Int.to_string (index + 1))
  ) in
...
let make_workspace_with_sources = fun ?toolchain_targets ~root ~packages () ->
  let packages = List.map packages ~fn:(
    fun (name, value) -> make_package ~root ~name ~value
  ) in
  write_workspace_manifest ~root ~members:(List.map packages ~fn:(
    fun (pkg: Riot_model.Package.t) -> pkg.relative_path
  ));
  (
    match toolchain_targets with
...
let make_broken_workspace = fun ?target_dir tmpdir ->
  ...
  let package = Riot_model.Package.make ~name:(package_name "demo") ~path:pkg_dir ~relative_path:(Path.v "demo") ~library:{ path = Path.v "src/lib.ml" } ~sources:{
    src = [ Path.v "src/lib.ml" ];
    native = [];
    tests = [];
    examples = [];
    bench = []
  } () in Riot_model.Workspace.make_realized ~root:tmpdir ?target_dir ~packages:[ package ] ()

let make_request = fun ~workspace ?(packages = [package_name "demo"]) ?(targets = Riot_model.Target.Host) ?(scope = Riot_build.Request.Runtime) ?(profile = Riot_model.Profile.debug) ?(requested_parallelism = None) () -> Riot_build.Request.make ~workspace ~packages ~targets ~scope ~profile ~requested_parallelism ()

let build_request = fun ?on_event request -> Riot_build.build ?on_event request

let package_names = fun output -> Riot_build.Build_result.packages output |> List.map ~fn:Riot_build.Build_result.package_name

let sort_package_names = fun package_names -> List.sort package_names ~compare:Riot_model.Package_name.compare

let expect_int = fun ~label ~expected ~actual ->
  if Int.equal expected actual then
    Ok ()
  else Error (label ^ ": expected " ^ Int.to_string expected ^ ", got " ^ Int.to_string actual)
...
let expect_string_list = fun ~label ~expected ~actual ->
  ...
  if equal expected actual then
    Ok ()
  else Error (label ^ ": expected [" ^ String.concat ", " expected ^ "], got [" ^ String.concat ", " actual ^ "]")
...
let expect_subsequence = fun ~haystack ~needle ->
  let rec loop haystack needle =
    ...
    | _, [] -> Ok ()
    | [], _ -> Error ("expected phase subsequence " ^ String.concat " -> " needle ^ " in " ^ String.concat " -> " haystack)
    | actual :: haystack_rest, expected :: needle_rest ->
        if String.equal actual expected then
          loop haystack_rest needle_rest
        else loop haystack_rest needle
  in
...
let make_build_result = fun ~scope ~(package:Riot_model.Package.t) ~status ->
  {
    Package_builder.package_key = Riot_model.Package.key_of_string (Riot_model.Package_name.to_string package.name ^ ":" ^ scope);
    package;
    status;
    ocamlc_warnings = [];
...
let make_runtime_build_result = fun ~package ~status -> make_build_result ~scope:"runtime" ~package ~status

let test_output_maps_build_result_statuses = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_output_statuses"
    (
      fun tmpdir ->
        ...
        let output =
          Riot_build.Build_result.of_build_results
            [
              make_build_result ~scope:"runtime" ~package:built_pkg ~status:(Package_builder.Built (make_artifact "built"));
              make_build_result ~scope:"runtime" ~package:cached_pkg ~status:(Package_builder.Cached (make_artifact "cached"));
              make_build_result ~scope:"runtime" ~package:skipped_pkg ~status:(Package_builder.Skipped { reason = "not requested" });
              make_build_result ~scope:"runtime" ~package:failed_pkg ~status:(Package_builder.Failed (Package_builder.ExecutionFailed { message = "boom" }));
            ]
        in
        ...
        in
        let* built = expect_package "built"
        in
        let* cached = expect_package "cached"
        in
        let* skipped = expect_package "skipped"
        in
        let* failed = expect_package "failed"
        in
        let* () =
...
let test_output_exposes_artifacts_and_exports = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_output_exports"
    (
      fun tmpdir ->
        ...
        let artifact = {
          ...
          ocamlc_warnings = [];
          exports = [ { Riot_store.Manifest.name = "demo"; path = "demo.exe"; action_hash = "abc123" } ]
        }
        in
        let output = Riot_build.Build_result.of_build_results [ make_runtime_build_result ~package ~status:(Package_builder.Built artifact) ] in
        ...
        | Some package_output -> (
          ...
          | Some found_artifact ->
              if not (String.equal (Crypto.Digest.hex found_artifact.hash) (Crypto.Digest.hex artifact.hash)) then
...
let test_output_prefers_dev_scope_and_merges_exports = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_output_scope_merge"
    (
      fun tmpdir ->
        let package = make_package ~root:tmpdir ~name:"demo" ~value:"1" in
        let build_artifact = make_artifact "demo_build" in
        let runtime_artifact = make_artifact "demo_runtime" in
        let dev_artifact = make_artifact ~exports:[ { Riot_store.Manifest.name = "build_core_tests"; path = "build_core_tests"; action_hash = "dev-export" } ] "demo_dev" in
        let output = Riot_build.Build_result.of_build_results [ make_build_result ~scope:"build" ~package ~status:(Package_builder.Built build_artifact); make_build_result ~scope:"runtime" ~package ~status:(Package_builder.Cached runtime_artifact); make_build_result ~scope:"dev" ~package ~status:(Package_builder.Built dev_artifact) ] in
        ...
        | Some package_output -> let open Std.Result.Syntax in
        let* () =
          match Riot_build.Build_result.package_status package_output with
          | Riot_build.Build_result.Built artifact when Crypto.Hash.equal artifact.hash dev_artifact.hash -> Ok ()
...
let test_build_returns_successful_output = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_success"
    (
      fun tmpdir ->
        ...
        | Ok output -> (
          ...
          | Some package_output -> (
            match Riot_build.Build_result.package_status package_output with
            | Riot_build.Build_result.Built _ | Riot_build.Build_result.Cached _ -> Ok ()
            | Riot_build.Build_result.Skipped reason -> Error ("expected successful package output, got skipped: " ^ reason)
            | Riot_build.Build_result.Failed message -> Error ("expected successful package output, got failure: " ^ message)
          )
...
let test_build_uses_all_packages_when_none_are_requested = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_all_packages"
    (
      fun tmpdir ->
        ...
        | Error err -> Error ("expected build to succeed, got: " ^ Riot_build.error_message err)
        | Ok output ->
            Test.assert_equal ~expected:(sort_package_names [ package_name "demo"; package_name "util" ]) ~actual:(package_names output |> sort_package_names);
            Ok ()
    ) with
  | Ok result -> result
  | Error err -> Error ("tempdir failed: " ^ IO.error_message err)
...
let test_build_returns_outputs_for_requested_packages = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_requested_packages"
    (
      fun tmpdir ->
        let prepared_workspace = make_valid_workspace ~package_names:[ "demo"; "util" ] tmpdir in
        match build_request (make_request ~workspace:prepared_workspace ~packages:[ package_name "util" ] ()) with
        | Error err -> Error ("expected build to succeed, got: " ^ Riot_build.error_message err)
        | Ok output ->
...
let test_build_reports_missing_single_package = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_missing_package"
    (
      fun tmpdir ->
        let prepared_workspace = make_valid_workspace ~package_names:[ "demo"; "util" ] tmpdir in
        match build_request (make_request ~workspace:prepared_workspace ~packages:[ package_name "missing" ] ()) with
        | Error (Riot_build.PackageNotFound { package_name = missing_package; available_packages }) ->
            ...
            Test.assert_equal ~expected:(sort_package_names [ package_name "demo"; package_name "util" ]) ~actual:(sort_package_names available_packages);
            Ok ()
        | Error err -> Error ("expected package-not-found error, got: " ^ Riot_build.error_message err)
        | Ok _ -> Error "expected missing package build to fail"
    ) with
  | Ok result -> result
  | Error err -> Error ("tempdir failed: " ^ IO.error_message err)

let test_build_reports_missing_multiple_packages = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_missing_packages"
    (
      fun tmpdir ->
        let prepared_workspace = make_valid_workspace ~package_names:[ "demo"; "util" ] tmpdir in
        match build_request (make_request ~workspace:prepared_workspace ~packages:[ package_name "demo"; package_name "missing"; package_name "other" ] ()) with
        | Error (Riot_build.PackagesNotFound { package_names = missing_packages; available_packages }) ->
            Test.assert_equal ~expected:(sort_package_names [ package_name "missing"; package_name "other" ]) ~actual:(sort_package_names missing_packages);
            Test.assert_equal ~expected:(sort_package_names [ package_name "demo"; package_name "util" ]) ~actual:(sort_package_names available_packages);
            Ok ()
        | Error err -> Error ("expected packages-not-found error, got: " ^ Riot_build.error_message err)
        | Ok _ -> Error "expected missing packages build to fail"
    ) with
  | Ok result -> result
  | Error err -> Error ("tempdir failed: " ^ IO.error_message err)

let test_build_reports_target_selection_failures_before_execution = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_target_miss"
    (
      fun tmpdir ->
        let prepared_workspace = make_valid_workspace ~toolchain_targets:[ "aarch64-apple-darwin" ] tmpdir in
        match build_request (make_request ~workspace:prepared_workspace ~targets:(Riot_model.Target.parse "linux") ()) with
...
let test_build_failure_surfaces_compiler_errors = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_failure"
    (
      fun tmpdir ->
        ...
        | Error (Riot_build.BuildFailed { errors }) ->
            if List.length errors > 0 then
              Ok ()
            else Error "expected at least one build error"
...
let test_build_can_return_cached_outputs_on_repeat_builds = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_cached_output"
    (
      fun tmpdir ->
        let prepared_workspace = make_valid_workspace tmpdir in
        ...
        | Ok output -> (
          ...
          | Some package_output -> (
            ...
            | Riot_build.Build_result.Skipped reason -> Error ("expected cached package output, got skipped: " ^ reason)
            | Riot_build.Build_result.Failed message -> Error ("expected cached package output, got failure: " ^ message)
          )
...
let test_cached_build_does_not_emit_generation_recording_events = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_cached_generation"
    (
      fun tmpdir ->
        let prepared_workspace = make_valid_workspace tmpdir in
        let request = make_request ~workspace:prepared_workspace () in
...
let test_build_emits_runtime_phases_in_order = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_phase_order"
    (
      fun tmpdir ->
        let prepared_workspace = make_valid_workspace tmpdir in
        let seen = ref [] in
        ...
        | Error err -> Error ("expected build to succeed, got: " ^ Riot_build.error_message err)
        | Ok _ -> expect_subsequence ~haystack:!seen ~needle:[
          "targets_resolved";
          "toolchains_ensured";
...
let test_build_emits_detailed_build_telemetry = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_telemetry"
    (
      fun tmpdir ->
        let prepared_workspace = make_valid_workspace tmpdir in
        let seen = ref [] in
        ...
        | Error err -> Error ("expected build to succeed, got: " ^ Riot_build.error_message err)
        | Ok _ -> expect_subsequence ~haystack:!seen ~needle:[ "build_started"; "compilation_started"; "build_completed" ]
    ) with
  | Ok result -> result
  | Error err -> Error ("tempdir failed: " ^ IO.error_message err)

let test_build_preserves_exact_target_subset = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_exact_targets"
    (
      fun tmpdir ->
        ...
        let prepared_workspace = make_valid_workspace ~toolchain_targets:(expected_targets |> List.map ~fn:(
          fun target -> Riot_model.Target.to_string target
        ) |> List.sort ~compare:String.compare) tmpdir in
        ...
        match build_request ~on_event:(
          ...
          | _ -> ()
        ) (make_request ~workspace:prepared_workspace ~targets:(Riot_model.Target.Exact requested_targets) ()) with
        | Error err -> Error ("expected exact target build to succeed, got: " ^ Riot_build.error_message err)
        | Ok _ ->
            match !target_count with
            | Some count -> let open Std.Result.Syntax in
            let* () = expect_int ~label:"resolved target count" ~expected:expected_target_count ~actual:count
            in
            let* () = expect_int ~label:"started target count" ~expected:expected_target_count ~actual:!started_count in expect_int ~label:"finished target count" ~expected:expected_target_count ~actual:!finished_count
...
let test_build_multi_target_outputs_and_events = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_multi_target_runtime"
    (
      fun tmpdir ->
        let host_target = Riot_model.Target.current in
        let secondary_target =
        ...
        let expected_targets = Riot_model.Target.Set.to_list requested_targets in
        let expected_target_count = List.length expected_targets in
        let expected_target_names = List.map expected_targets ~fn:(
          fun target -> Riot_model.Target.to_string target
        ...
        match build_request ~on_event:(
          function
          | Riot_build.Event.Phase (Riot_build.Event.TargetsResolved { target_count }) -> seen_target_count := Some target_count
          | Riot_build.Event.Phase (Riot_build.Event.TargetBuildStarted { target }) -> started_targets := Riot_model.Target.to_string target :: !started_targets
        ...
        | Error err -> Error ("expected multi-target build to succeed, got: " ^ Riot_build.error_message err)
        | Ok output ->
            ...
            let* () = expect_int ~label:"resolved target count" ~expected:expected_target_count ~actual:count
            in
            let* () = expect_int ~label:"started target count" ~expected:expected_target_count ~actual:(List.length !started_targets)
            in
            let* () = expect_int ~label:"finished target count" ~expected:expected_target_count ~actual:(List.length !finished_targets)
            in
            let started_sorted = sort_target_names !started_targets in
            let finished_sorted = sort_target_names !finished_targets in
            let* () = expect_string_list ~label:"started targets" ~expected:expected_target_names ~actual:started_sorted
            in
            let* () = expect_string_list ~label:"finished targets" ~expected:expected_target_names ~actual:finished_sorted
            in
            Test.assert_false !any_partial_failure;
            let build_result_status =
              if not (List.all !finished_counts ~fn:(
                fun result_count -> result_count = 1
              )) then
                Error ("expected each target lane to report one package result, got " ^ String.concat ", " (List.map !finished_counts ~fn:Int.to_string))
              else
                (
                  match Riot_build.Build_result.find_package output (package_name "demo") with
                  | None -> Error "expected output for package demo"
                  | Some package_output -> (
                    match Riot_build.Build_result.package_status package_output with
                    | Riot_build.Build_result.Built _ | Riot_build.Build_result.Cached _ -> Ok ()
                    | Riot_build.Build_result.Skipped reason -> Error ("expected demo package to be built, got skipped: " ^ reason)
                    | Riot_build.Build_result.Failed message -> Error ("expected demo package to be built, got failed: " ^ message)
                  )
                )
            in
            ...
            | Error err -> Error err
            | Ok () ->
                let validate_output_dirs = List.map expected_targets ~fn:(
                  fun target ->
                    Riot_model.Riot_dirs.out_dir_in_workspace ~workspace:prepared_workspace ~profile:expected_profile.name ~target |> fun out_dir -> Path.(out_dir / Path.v "demo")
                ) in
                let missing_dirs = List.filter validate_output_dirs ~fn:(
                  fun output_dir -> not (Fs.exists output_dir |> Result.unwrap_or ~default:false)
                ) in
                if not (List.is_empty missing_dirs) then
                  Error ("expected package output directories to exist for each target: " ^ String.concat ", " (List.map missing_dirs ~fn:Path.to_string))
                else Ok ()
    ) with
  | Ok result -> result
  | Error err -> Error ("tempdir failed: " ^ IO.error_message err)
...
let test_build_multi_target_partial_failures_fail_by_default = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_multi_target_partial_fail_default"
    (
      fun tmpdir ->
        let requested_targets = make_two_targets () in
        let expected_target_count = Riot_model.Target.Set.length requested_targets in
        let expected_targets = Riot_model.Target.Set.to_list requested_targets |> List.map ~fn:(
          fun target -> Riot_model.Target.to_string target
        ) |> List.sort ~compare:String.compare in
        let prepared_workspace = make_workspace_with_sources ~root:tmpdir ~toolchain_targets:expected_targets ~packages:[
          "good", "let value = 2\n";
          "bad", "let broken =";
        ] () in
        let seen_target_count = ref None in
        ...
        match build_request ~on_event:(
          ...
          | _ -> ()
        ) (make_request ~workspace:prepared_workspace ~targets:(Riot_model.Target.Exact requested_targets) ~packages:[ package_name "good"; package_name "bad" ] ()) with
        ...
        | Error (Riot_build.BuildFailed _) -> (
          ...
          let* () = expect_int ~label:"finished target count" ~expected:expected_target_count ~actual:(List.length !finished_targets)
          in
          let* () = expect_int ~label:"partial failure flag count" ~expected:expected_target_count ~actual:(List.length !partial_failure_flags)
          in
          if not (List.all !partial_failure_flags ~fn:(
            fun partial -> partial
          )) then
            Error "expected each target build to report partial failures"
          else
            if !saw_returning_results then
              Error "expected no returning_results event when default multi-target build fails"
            else
              let sort_target_names = List.sort ~compare:String.compare in
              let* () = expect_string_list ~label:"started targets" ~expected:expected_targets ~actual:(sort_target_names !started_targets) in expect_string_list ~label:"finished targets" ~expected:expected_targets ~actual:(sort_target_names !finished_targets)
        )
        | Error err -> Error ("expected build to fail with BuildFailed, got: " ^ Riot_build.error_message err)
    ) with
  | Ok result -> result
  | Error err -> Error ("tempdir failed: " ^ IO.error_message err)

let test_build_rejects_zero_jobs = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_zero_jobs"
    (
      fun tmpdir ->
        let prepared_workspace = make_valid_workspace tmpdir in
        match build_request (make_request ~workspace:prepared_workspace ~requested_parallelism:(Some 0) ()) with
        | Ok _ -> Error "expected zero jobs request to fail"
        | Error (Riot_build.InvalidRequestedParallelism _) -> Ok ()
        | Error err -> Error ("expected InvalidRequestedParallelism, got: " ^ Riot_build.error_message err)
    ) with
  | Ok result -> result
  | Error err -> Error ("tempdir failed: " ^ IO.error_message err)

let test_build_rejects_zero_jobs_with_message = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_zero_jobs_message"
    (
      fun tmpdir ->
        let prepared_workspace = make_valid_workspace tmpdir in
        match build_request (make_request ~workspace:prepared_workspace ~requested_parallelism:(Some 0) ()) with
        | Ok _ -> Error "expected zero jobs request to fail"
        | Error (Riot_build.InvalidRequestedParallelism _ as err) when String.equal (Riot_build.error_message err) "invalid requested parallelism (0): jobs must be >= 1" -> Ok ()
        | Error err -> Error ("expected specific message for invalid jobs, got: " ^ Riot_build.error_message err)
    ) with
  | Ok result -> result
  | Error err -> Error ("tempdir failed: " ^ IO.error_message err)

let test_build_rejects_negative_jobs = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_negative_jobs"
    (
      fun tmpdir ->
        let prepared_workspace = make_valid_workspace tmpdir in
        match build_request (make_request ~workspace:prepared_workspace ~requested_parallelism:(Some (-1)) ()) with
...
let test_build_rejects_negative_jobs_with_message = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_negative_jobs_message"
    (
      fun tmpdir ->
        let prepared_workspace = make_valid_workspace tmpdir in
        match build_request (make_request ~workspace:prepared_workspace ~requested_parallelism:(Some (-1)) ()) with
        | Ok _ -> Error "expected negative jobs request to fail"
        | Error (Riot_build.InvalidRequestedParallelism _ as err) when String.equal (Riot_build.error_message err) "invalid requested parallelism (-1): jobs must be >= 1" -> Ok ()
        | Error err -> Error ("expected specific message for invalid jobs, got: " ^ Riot_build.error_message err)
    ) with
  | Ok result -> result
  | Error err -> Error ("tempdir failed: " ^ IO.error_message err)
...
let test_build_accepts_positive_jobs = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_high_jobs"
    (
      fun tmpdir ->
        let prepared_workspace = make_valid_workspace tmpdir in
        let requested_parallelism = Thread.available_parallelism * 2 + 3 in
        match build_request (make_request ~workspace:prepared_workspace ~requested_parallelism:(Some requested_parallelism) ()) with
        | Ok _ -> Ok ()
...
[
  ...
  case ~size:Large "build core: default partial failure defaults to failing in multi-target builds" test_build_multi_target_partial_failures_fail_by_default;
