...
let make_node = fun ~package ?(dep_nodes = []) ~actions ~outs () ->
  ...
  let deps = List.map dep_nodes ~fn:(fun (node: Riot_planner.Action_node.t) -> node.id) in
  let spec =
  ...
  let _ =
    List.for_each dep_nodes ~fn:(fun dep_node ->
