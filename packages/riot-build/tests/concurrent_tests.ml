...
let test_concurrent_builds_different_packages = fun _ctx ->
  match Fs.with_tempdir ~prefix:"concurrent_test"
    (
      fun tmpdir ->
        let pkg1 = make_package tmpdir "pkg-1" "let x = 1" in
        let pkg2 = make_package tmpdir "pkg-2" "let x = 2" in
        let workspace = Riot_model.Workspace.make_realized ~root:tmpdir ~packages:[ pkg1; pkg2 ] ~target_dir:(Path.to_string Path.(Path.v "target")) () in
        ...
        let _worker1 =
          spawn
            (
              fun () ->
                let result = Package_builder.build ~workspace ~toolchain ~store ~build_ctx:(make_test_build_ctx ()) ~package_graph ~package_key:(Riot_planner.Package_graph.package_key ~package_name:(Riot_model.Package_name.to_string pkg1.name) Riot_planner.Package_graph.Runtime) ~package:pkg1 in
                let status =
        ...
        let _worker2 =
          spawn
            (
              fun () ->
                let result = Package_builder.build ~workspace ~toolchain ~store ~build_ctx:(make_test_build_ctx ()) ~package_graph ~package_key:(Riot_planner.Package_graph.package_key ~package_name:(Riot_model.Package_name.to_string pkg2.name) Riot_planner.Package_graph.Runtime) ~package:pkg2 in
                let status =
                  match result.status with
                  | Built _ | Cached _ -> Ok ()
                  | Skipped _ -> Error "skipped"
        ...
        | BuildComplete (name1, Ok ()), BuildComplete (name2, Ok ()) ->
            if (name1 = "pkg-1" && name2 = "pkg-2") || (name1 = "pkg-2" && name2 = "pkg-1") then
              Ok ()
            else Error ("Unexpected package names: " ^ name1 ^ ", " ^ name2)
  ...
  match Fs.with_tempdir ~prefix:"concurrent_test"
    (
      fun tmpdir ->
        ...
        let workspace = Riot_model.Workspace.make_realized ~root:tmpdir ~packages:[ package ] ~target_dir:(Path.to_string Path.(Path.v "target")) () in
        let toolchain = Riot_toolchain.init ~config:Riot_model.Toolchain_config.default |> Result.expect ~msg:"Failed to initialize toolchain" in
        let store = Riot_store.Store.create ~workspace in
        let package_graph = Riot_planner.Package_graph.create ~scope:Riot_planner.Package_graph.Runtime workspace |> Result.unwrap in
        let parent = self () in
        let _worker1 =
          spawn
            (
              fun () ->
                let result = Package_builder.build ~workspace ~toolchain ~store ~build_ctx:(make_test_build_ctx ()) ~package_graph ~package_key:(Riot_planner.Package_graph.package_key ~package_name:(Riot_model.Package_name.to_string package.name) Riot_planner.Package_graph.Runtime) ~package in
                let status =
                  match result.status with
                  | Built _ | Cached _ -> Ok ()
                  | Skipped _ -> Error "skipped"
      ...
      fun tmpdir ->
        ...
        | Built _ -> (
          ...
          | (BuildCompleteWithCache (_, cached1, Ok ()), BuildCompleteWithCache (_, cached2, Ok ())) ->
              let _ = cached1 in
              let _ = cached2 in Ok ()
          | BuildCompleteWithCache (name, _, Error err), _ -> Error (name ^ " build failed: " ^ err)
          | _, BuildCompleteWithCache (name, _, Error err) -> Error (name ^ " build failed: " ^ err)
          | _ -> Error "Unexpected message"
        )
...
[ case ~size:Large "concurrent: different packages don't interfere" test_concurrent_builds_different_packages; case ~size:Large "concurrent: same package builds safely" test_concurrent_builds_same_package; case "concurrent: shared cache works correctly" test_concurrent_builds_with_shared_cache ]
