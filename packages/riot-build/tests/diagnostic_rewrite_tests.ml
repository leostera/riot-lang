...
let test_rewrite_ocamlc_result_rewrites_workspace_paths = fun _ctx ->
  match Fs.with_tempdir
    ~prefix:"diagnostic_rewrite_test"
    (fun tmpdir ->
      let package = make_package ~root:tmpdir ~name:"riot-cli" in
      let package_src = Path.(package.path / Path.v "src") in
      let sandbox_dir =
        Path.(tmpdir / Path.v "_build" / Path.v "sandbox" / Path.v "riot-cli-a9441944")
      in
      let _ =
        Fs.create_dir_all package_src
        |> Result.expect ~msg:"failed to create package src"
      in
      let _ =
        Fs.write
          "let displayed_packages = HashMap.create ()"
          Path.(package_src / Path.v "install.ml")
        |> Result.expect ~msg:"failed to write package source"
      in
      let diagnostic =
        make_warning (Path.to_string Path.(sandbox_dir / Path.v "src" / Path.v "install.ml"))
        |> Riot_toolchain.Ocamlc.Diagnostic.parse
        |> List.head
        |> Option.expect ~msg:"expected one parsed diagnostic"
      in
      ...
      | _ -> Error "expected exactly one rewritten warning") with
  | Ok result -> result
  | Error err -> Error ("tempdir creation failed: " ^ IO.error_message err)

let test_rewrite_ocamlc_result_leaves_generated_paths_alone = fun _ctx ->
  match Fs.with_tempdir
    ~prefix:"diagnostic_rewrite_generated_test"
    (fun tmpdir ->
      let package = make_package ~root:tmpdir ~name:"riot-cli" in
      let sandbox_dir =
        Path.(tmpdir / Path.v "_build" / Path.v "sandbox" / Path.v "riot-cli-a9441944")
      in
      let diagnostic =
        make_warning (Path.to_string Path.(sandbox_dir / Path.v "Riot_cli__Aliases.ml.gen"))
        |> Riot_toolchain.Ocamlc.Diagnostic.parse
        |> List.head
        |> Option.expect ~msg:"expected one parsed diagnostic"
      in
      let rewritten =
        Diagnostic_rewrite.rewrite_ocamlc_result
          ~package
          ~sandbox_dir
      ...
      then
        ...
        Error ("expected generated sandbox path to stay unchanged, got: " ^ rendered)) with
  | Ok result -> result
  | Error err -> Error ("tempdir creation failed: " ^ IO.error_message err)
...
let tests =
  Test.[
    case
      "rewrite ocamlc result rewrites workspace paths"
      test_rewrite_ocamlc_result_rewrites_workspace_paths;
    case
      "rewrite ocamlc result leaves generated paths alone"
      test_rewrite_ocamlc_result_leaves_generated_paths_alone;
  ]
