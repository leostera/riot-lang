...
let check_cache_invalidation_results = fun first_build second_build ->
  ...
  in
  match first_build.Riot_executor.Package_builder.status with
  | Failed err ->
  ...
  | Built _
  | Cached _ -> (
      match second_build.Riot_executor.Package_builder.status with
      | Built _ -> Ok ()
...
let test_build_stats_action_cache_counters = fun _ctx ->
  let stats = Riot_build.Protocol.BuildStats.make () in
  Riot_build.Protocol.BuildStats.inc_action_cache_hits stats;
  Riot_build.Protocol.BuildStats.inc_action_cache_hits stats;
  Riot_build.Protocol.BuildStats.inc_action_cache_misses stats;
  if
    Riot_build.Protocol.BuildStats.get_action_cache_hits stats = 2
    && Riot_build.Protocol.BuildStats.get_action_cache_misses stats = 1
  then
    Ok ()
  else
...
|};
          ...
          ()
        in
        match Riot_build.start_local ~workspace ~registry ~config:Riot_build.Server_config.default () with
        | Error err -> Error ("expected local server to start: " ^ Riot_build.error_message err)
        | Ok server_pid ->
            send
              server_pid
              (Riot_build.Protocol.ServerRequest (Riot_build.Protocol.GetWorkspaceConfig {
                client_pid = self ()
              }));
            let selector msg =
              match msg with
              | Riot_build.Protocol.ServerResponse (Riot_build.Protocol.WorkspaceConfig {
                workspace;
                toolchain=_
              }) -> `select workspace
              | _ -> `skip
...
|};
        ...
        in
        let seen = ref [] in
        match Riot_build.start_local
          ~emit:(fun kind -> seen := kind :: !seen)
          ~workspace
          ~registry
          ~config:Riot_build.Server_config.default
          () with
        | Error err -> Error ("expected local server to start: " ^ Riot_build.error_message err)
        | Ok _ ->
            if List.exists
