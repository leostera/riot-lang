...
let test_cache_store_creation_is_lazy_until_first_save = fun _ctx ->
  match Fs.with_tempdir ~prefix:"cache_test_lazy"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        let cache_dir = Path.(tmpdir / Path.v "target" / Path.v "debug" / Path.v (Riot_model.Target.to_string (Riot_model.Riot_dirs.host_target ())) / Path.v "cache") in
        ...
        | Ok false ->
            ...
            let _ = Result.expect (Riot_store.Store.save store ~package:"test" ~hash ~sandbox_dir:sandbox ~outs:[ output ]) ~msg:"Save failed" in
            (
              match Fs.exists cache_dir with
              | Ok true -> Ok ()
...
let test_simple_file_caching = fun _ctx ->
  match Fs.with_tempdir ~prefix:"cache_test_simple"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
...
let test_cache_hit_retrieval = fun _ctx ->
  match Fs.with_tempdir ~prefix:"cache_test_hit"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        ...
        let _ = Result.expect (Riot_store.Store.save store ~package:"pkg" ~hash ~sandbox_dir:sandbox ~outs:[ output ]) ~msg:"Save failed" in
...
let test_cache_promotion_workflow = fun _ctx ->
  match Fs.with_tempdir ~prefix:"cache_test_promotion"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        ...
        let _ = Result.expect (Riot_store.Store.save store ~package:"mylib" ~hash ~sandbox_dir:sandbox ~outs:[ out1; out2 ]) ~msg:"Save failed" in
...
let test_different_hashes_isolated = fun _ctx ->
  match Fs.with_tempdir ~prefix:"cache_test_isolated"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        ...
        let _ = Result.expect (Riot_store.Store.save store ~package:"test" ~hash:hash1 ~sandbox_dir:sandbox ~outs:[ output ]) ~msg:"Save v1 failed" in
        Result.expect (Fs.write "version2" output) ~msg:"Write v2 failed";
        let hash2 = Crypto.hash_string "action_v2" in
        let _ = Result.expect (Riot_store.Store.save store ~package:"test" ~hash:hash2 ~sandbox_dir:sandbox ~outs:[ output ]) ~msg:"Save v2 failed" in
        if Riot_store.Store.exists store hash1 && Riot_store.Store.exists store hash2 then
          (
            ...
            Result.expect (Riot_store.Store.promote store hash1 ~target_dir:target1) ~msg:"Promote v1 failed";
            Result.expect (Riot_store.Store.promote store hash2 ~target_dir:target2) ~msg:"Promote v2 failed";
            let content1 = Result.expect (Fs.read_to_string Path.(target1 / Path.v "output.txt")) ~msg:"Read v1 failed" in
            let content2 = Result.expect (Fs.read_to_string Path.(target2 / Path.v "output.txt")) ~msg:"Read v2 failed" in
