...
let target = fun triple -> Riot_model.Target.from_string triple |> Result.expect ~msg:("expected valid target triple: " ^ triple)
  ...
  with_tempdir "riot_build_lock_reentrant"
    (
      fun tmpdir ->
        ...
        BuildLock.acquire ~on_waiting:(
          fun _ -> ()
        ) ~target_dir_root:(target_dir_root workspace) ~profile:"debug" ~target:host_target
          (
            fun () ->
              let parent = self () in
              let _worker =
                spawn
                  (
                    fun () ->
                      ...
                      match BuildLock.acquire ~on_waiting:(
                        fun _ -> ()
                      ) ~target_dir_root:(target_dir_root workspace) ~profile:"debug" ~target:host_target
                        (
                          fun () ->
              ...
              | Some (Ok waited) ->
                  if Time.Duration.to_millis waited < 200 then
                    Ok ()
                  else Error "Reentrant acquire in the same process should not block"
          ...
          (
            fun () ->
                ...
                spawn
                  (
                    fun () ->
                      ...
                      match BuildLock.acquire ~on_waiting:(
                        fun _ -> ()
                      ) ~target_dir_root:(target_dir_root workspace) ~profile:"debug" ~target:linux_target
                        (
                          fun () ->
                            let waited = Time.Instant.elapsed start in
                            send parent (BuildLockAcquired waited);
                      ...
                      | Ok () -> Ok ()
                      | Error _ ->
                          send parent (BuildLockAcquireFailed "Failed to acquire build lock for second target");
                          Ok ()
                  )
              in
              let selector msg =
              ...
              | Ok waited ->
                  if Time.Duration.to_millis waited < 200 then
                    Ok ()
                  else Error "Different target locks should not block each other"
  ...
  with_tempdir "riot_build_lock_existing_lanes"
    (
      fun tmpdir ->
        let workspace = make_workspace tmpdir in
        let host_target = target "aarch64-apple-darwin" in
        let linux_target = target "aarch64-unknown-linux-gnu" in let open ResultSyntax in
        let* () =
          BuildLock.acquire ~on_waiting:(
            fun _ -> ()
          ) ~target_dir_root:(target_dir_root workspace) ~profile:"release" ~target:linux_target
            (
              fun () -> Ok ()
            )
        in
        ...
        let actual = BuildLock.existing_lanes ~target_dir_root:(target_dir_root workspace) |> List.map ~fn:(
          fun (lane: BuildLock.lane) -> lane.profile ^ ":" ^ Riot_model.Target.to_string lane.target
        ) in
        let expected = [ "debug:aarch64-apple-darwin"; "release:aarch64-unknown-linux-gnu" ] in
        if actual = expected then
          Ok ()
        else Error ("Expected existing_lanes to return sorted profile/target lanes, got " ^ String.concat ", " actual)
    )
...
let test_acquire_existing_lanes_succeeds_when_no_lanes_exist = fun _ctx ->
  with_tempdir "riot_build_lock_empty_lanes"
    (
      fun tmpdir ->
        let workspace = make_workspace tmpdir in
        BuildLock.acquire_existing_lanes ~on_waiting:(
          fun _ -> ()
