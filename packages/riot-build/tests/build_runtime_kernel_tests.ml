...
let render_build_event = fun (event: Riot_build.Event.t) ->
  ...
  | Riot_build.Event.BuildingTarget { target; host } -> "BuildingTarget("
...
let public_phase_names = fun events ->
  List.reverse events |> List.filter_map
    ~fn:(
      function
...
let expect_public_phase_subsequence = fun events ->
  ...
  let rec loop haystack needle =
    ...
    | _, [] -> Ok ()
    | [], _ -> Error ("expected public phase subsequence "
...
let summarize_build_failure = fun (err: Riot_build.error) events ->
  let recent_events = List.reverse events
  |> List.take ~len:12
  |> List.reverse
        ...
        | Ok workspace ->
            ...
            | Ok output -> (
                ...
                | Some result -> (
                    ...
                    | Riot_build.Build_result.Cached _ -> Result.map_err
                      (expect_public_phase_subsequence !events)
                      ~fn:(fun err -> err ^ "\nrecent events: " ^ summarize_recent_events !events)
                    | Riot_build.Build_result.Skipped reason -> Error ("expected kernel build to run, got skipped: "
                    ^ reason)
