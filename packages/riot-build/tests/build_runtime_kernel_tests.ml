  ...
  | Ok (workspace, errors) ->
      if List.is_empty errors then
        ...
        let* registry = Pkgs_ml.Registry.create_filesystem ?riot_home:None ~registry_name:"pkgs.ml" () |> Result.map_err ~fn:(
          fun err -> "registry init failed: " ^ Pkgs_ml.Registry_cache.create_error_message err
        ) in Riot_deps.ensure_workspace ~workspace_manager:manager ~mode:Riot_deps.Dep_solver.Refresh ~registry ~workspace () |> Result.map_err ~fn:Riot_model.Pm_error.message
...
let render_build_event = fun (event: Riot_build.Event.t) ->
  ...
  | Riot_build.Event.BuildingTarget { target; host } -> "BuildingTarget(" ^ Riot_model.Target.to_string target ^ "," ^ Bool.to_string host ^ ")"
...
let public_phase_names = fun events -> List.reverse events |> List.filter_map ~fn:(
  function
  | Riot_build.Event.Phase phase -> Some (phase_name phase)
  | Riot_build.Event.Pm _ | Riot_build.Event.BuildingTarget _ | Riot_build.Event.Telemetry _ | Riot_build.Event.CacheGc _ -> None
)
...
let expect_public_phase_subsequence = fun events ->
  ...
  let rec loop haystack needle =
    ...
    | _, [] -> Ok ()
    | [], _ -> Error ("expected public phase subsequence " ^ String.concat " -> " needle ^ "\nactual phases: " ^ String.concat " -> " haystack)
...
let summarize_build_failure = fun (err: Riot_build.error) events ->
  let recent_events = List.reverse events |> List.take ~len:12 |> List.reverse |> List.map ~fn:render_build_event |> String.concat " -> " in Riot_build.error_message err ^ "\nrecent events: " ^ recent_events

let summarize_recent_events = fun events -> List.reverse events |> List.take ~len:20 |> List.reverse |> List.map ~fn:render_build_event |> String.concat " -> "

let test_build_runtime_builds_repo_kernel = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_kernel_runtime"
    (
      fun tempdir ->
        match load_repo_workspace () with
        | Error _ as err -> err
        | Ok workspace ->
            let workspace = clone_workspace_with_target workspace ~target_dir:Path.(tempdir / Path.v "target") in
            let events = ref [] in
            match Riot_build.build ~on_event:(
              fun event -> events := event :: !events
            ) (Riot_build.Request.make ~workspace ~packages:[ package_name "kernel" ] ~targets:Riot_model.Target.Host ~scope:Riot_build.Request.Runtime ~profile:Riot_model.Profile.debug ()) with
            ...
            | Ok output -> (
              ...
              | Some result -> (
                ...
                | Riot_build.Build_result.Built _ | Riot_build.Build_result.Cached _ -> Result.map_err (expect_public_phase_subsequence !events) ~fn:(
                  fun err -> err ^ "\nrecent events: " ^ summarize_recent_events !events
                )
                | Riot_build.Build_result.Skipped reason -> Error ("expected kernel build to run, got skipped: " ^ reason)
                | Riot_build.Build_result.Failed message -> Error ("kernel build failed: " ^ message)
              )
            )
    ) with
  | Ok result -> result
