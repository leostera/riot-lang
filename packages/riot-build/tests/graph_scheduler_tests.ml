...
let make_graph = fun ~apply_mutation tasks ->
  ...
  let node_ids = List.map tasks ~fn:(
    fun task -> Graph_scheduler.Graph.add_node graph ~payload:task
  ) in (graph, node_ids)

let result_labels = fun results -> List.map results.Graph_scheduler.results ~fn:(
  fun (result: (int, string, string) Graph_scheduler.node_result) ->
    match result.outcome with
    | Ok value -> Int.to_string result.payload ^ ":" ^ value
    | Error err -> Int.to_string result.payload ^ ":error:" ^ err
)

let run_graph = fun ?(parallelism = 1) ?(mode = Graph_scheduler.Run_config.Continue_on_failure) ?(on_event = fun () -> ()) ~graph ~execute () -> Graph_scheduler.run ~config:(Graph_scheduler.Run_config.make ~parallelism ~mode ()) ~on_event ~graph ~execute

let run_tasks = fun ?parallelism ?mode ?on_event ?(apply_mutation = fun _ (_:mutation) -> ()) ~tasks ~execute () ->
  let graph, _ = make_graph ~apply_mutation tasks in run_graph ?parallelism ?mode ?on_event ~graph ~execute ()

let test_graph_scheduler_runs_generated_work = fun _ctx ->
  let results = run_tasks ~tasks:[ 1 ] ~execute:(
...
let test_graph_scheduler_queues_generated_work_without_waiting_for_wave = fun _ctx ->
  ...
  let results = run_tasks ~parallelism:2 ~tasks:[ 1; 2 ] ~execute:(
    fun ~graph ~node:_ ~payload ->
      match payload with
      | 1 ->
          ignore (Graph_scheduler.Handle.add_node graph ~payload:3);
  ...
  if not !generated_started_before_slow_finished then
    ...
    (
      Test.assert_equal ~expected:[ "1:root"; "2:slow"; "3:generated" ] ~actual:(result_labels results);
      Ok ()
    )
...
let test_graph_scheduler_records_errors_and_continues_ready_work = fun _ctx ->
  ...
  Test.assert_equal ~expected:[ "1:error:boom"; "2:ok"; "3:generated" ] ~actual:(result_labels results);
  Ok ()

let test_graph_scheduler_generated_dependencies_gate_execution = fun _ctx ->
  let left_finished = ref false in
  let results = run_tasks ~parallelism:2 ~tasks:[ 1 ] ~execute:(
    fun ~graph ~node:_ ~payload ->
      match payload with
      | 1 ->
...
let test_graph_scheduler_static_dependencies_gate_execution = fun _ctx ->
  let order = ref [] in
  let graph, node_ids = make_graph ~apply_mutation:(
    fun _ (_: mutation) -> ()
  ) [ 1; 2; 3 ] in
  match node_ids with
  | [ one; two; three ] ->
      ...
      let results = run_graph ~parallelism:3 ~graph ~execute:(
        fun ~graph:_ ~node:_ ~payload ->
          order := !order @ [ payload ];
          Ok ("done-" ^ Int.to_string payload)
      ) () in
      Test.assert_equal ~expected:[ 1; 2; 3 ] ~actual:!order;
      Test.assert_equal ~expected:[ "1:done-1"; "2:done-2"; "3:done-3" ] ~actual:(result_labels results);
      Ok ()
...
let test_graph_scheduler_fail_fast_stops_new_generated_work = fun _ctx ->
  ...
  let results = run_tasks ~parallelism:2 ~mode:Graph_scheduler.Run_config.Fail_fast ~tasks:[ 1; 2 ] ~execute:(
    fun ~graph ~node:_ ~payload ->
      match payload with
      | 1 -> Error "boom"
      | 2 ->
...
let test_graph_scheduler_emits_events_in_completion_order = fun _ctx ->
  ...
  | [ one; two ] ->
      Graph_scheduler.Graph.add_dependency graph ~node:two ~depends_on:one;
      let _ = Graph_scheduler.run ~config:(Graph_scheduler.Run_config.make ~parallelism:1 ~mode:Graph_scheduler.Run_config.Continue_on_failure ()) ~graph ~on_event:(
...
let test_graph_scheduler_applies_recorded_mutations = fun _ctx ->
  let applied = ref [] in
  let results = run_tasks ~tasks:[ 1; 2 ] ~apply_mutation:(
    fun _ mutation ->
      match mutation with
...
let test_graph_scheduler_empty_graph_returns_empty_results = fun _ctx ->
  ...
  let results = run_graph ~graph ~execute:(
    fun ~graph:_ ~node:_ ~payload:_ -> Ok "unused"
  ) () in
...
let test_graph_scheduler_ignores_workers_from_previous_runs = fun _ctx ->
  let rec loop run_index =
    if run_index = 20 then
      ...
      let results = run_tasks ~parallelism:4 ~tasks:[ 1; 2 ] ~execute:(
        fun ~graph:_ ~node:_ ~payload:task -> Ok ("run-" ^ Int.to_string run_index ^ "-" ^ Int.to_string task)
      ) () in
      match result_labels results with
      | [ _; _ ] -> loop (run_index + 1)
      | labels -> Error ("expected each repeated scheduler run to complete two tasks, got [" ^ String.concat ", " labels ^ "]")
  in
  loop 0
...
[
  ...
  case "graph scheduler: generated work does not wait for sibling wave" test_graph_scheduler_queues_generated_work_without_waiting_for_wave;
  case "graph scheduler: records errors and continues ready work" test_graph_scheduler_records_errors_and_continues_ready_work;
  case "graph scheduler: generated dependencies gate execution" test_graph_scheduler_generated_dependencies_gate_execution;
  case "graph scheduler: static dependencies gate execution" test_graph_scheduler_static_dependencies_gate_execution;
  case "graph scheduler: fail-fast blocks new generated work" test_graph_scheduler_fail_fast_stops_new_generated_work;
  case "graph scheduler: emits events in completion order" test_graph_scheduler_emits_events_in_completion_order;
  case "graph scheduler: applies recorded mutations" test_graph_scheduler_applies_recorded_mutations;
  case "graph scheduler: empty graphs return no results" test_graph_scheduler_empty_graph_returns_empty_results;
  case "graph scheduler: ignores ready workers from previous runs" test_graph_scheduler_ignores_workers_from_previous_runs;
]
