...
let run_graph = fun ?(parallelism = 1) ?(mode = Graph_scheduler.Run_config.Continue_on_failure) ?(on_event = fun () ->
  ()) ~graph ~execute () ->
  Graph_scheduler.run
    ...
    run_tasks ~tasks:[ 1 ]
      ~execute:(fun ~graph ~node:_ ~payload ->
        ...
        | 3 ->
            Ok "right"
        | task ->
            Error ("unexpected task " ^ Int.to_string task))
      ()
  in
...
let test_graph_scheduler_queues_generated_work_without_waiting_for_wave = fun _ctx ->
  ...
  let results =
    run_tasks ~parallelism:2 ~tasks:[ 1; 2 ]
      ~execute:(fun ~graph ~node:_ ~payload ->
        match payload with
        | 1 ->
            ignore (Graph_scheduler.Handle.add_node graph ~payload:3);
...
let test_graph_scheduler_generated_dependencies_gate_execution = fun _ctx ->
  ...
  let results =
    run_tasks ~parallelism:2 ~tasks:[ 1 ]
      ~execute:(fun ~graph ~node:_ ~payload ->
        match payload with
        | 1 ->
...
let test_graph_scheduler_static_dependencies_gate_execution = fun _ctx ->
  let order = ref [] in
  let graph, node_ids =
    make_graph ~apply_mutation:(fun _ (_: mutation) -> ()) [ 1; 2; 3 ]
  in
  match node_ids with
  | [one;two;three] ->
...
let test_graph_scheduler_fail_fast_stops_new_generated_work = fun _ctx ->
  ...
  let results =
    run_tasks ~parallelism:2 ~mode:Graph_scheduler.Run_config.Fail_fast ~tasks:[ 1; 2 ]
      ~execute:(fun ~graph ~node:_ ~payload ->
        match payload with
        | 1 ->
            Error "boom"
...
let test_graph_scheduler_emits_events_in_completion_order = fun _ctx ->
  let events = ref [] in
  let graph, node_ids =
    make_graph ~apply_mutation:(fun _ (_: mutation) -> ()) [ 1; 2 ]
  in
  match node_ids with
  | [one;two] ->
      Graph_scheduler.Graph.add_dependency graph ~node:two ~depends_on:one;
      let _ =
        Graph_scheduler.run ~config:(Graph_scheduler.Run_config.make
          ~parallelism:1
          ~mode:Graph_scheduler.Run_config.Continue_on_failure ()) ~graph ~on_event:(fun event ->
          events := !events @ [ event ])
          ~execute:(fun ~graph ~node:_ ~payload ->
...
let test_graph_scheduler_empty_graph_returns_empty_results = fun _ctx ->
  let graph =
    Graph_scheduler.Graph.create ~apply_mutation:(fun _ (_: mutation) -> ()) ()
  in
  let results =
    run_graph ~graph ~execute:(fun ~graph:_ ~node:_ ~payload:_ -> Ok "unused") ()
  in
...
let test_graph_scheduler_ignores_workers_from_previous_runs = fun _ctx ->
  let rec loop run_index =
    if run_index = 20 then
      ...
      | labels -> Error ("expected each repeated scheduler run to complete two tasks, got ["
