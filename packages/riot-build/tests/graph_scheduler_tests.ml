...
let make_graph = fun ~apply_mutation tasks ->
  ...
  let node_ids =
    List.map tasks
      ~fn:(fun task -> Graph_scheduler.Graph.add_node graph ~payload:task)
  in
  graph, node_ids
...
let test_graph_scheduler_empty_graph_returns_empty_results = fun _ctx ->
  ...
  let results =
    run_graph
      ~graph
      ~execute:(fun ~graph:_ ~node:_ ~payload:_ -> Ok "unused")
      ()
  in
...
let tests =
  ...
  [
    ...
    case
      "graph scheduler: applies recorded mutations"
      test_graph_scheduler_applies_recorded_mutations;
    case
