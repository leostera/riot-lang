...
let make_package = fun ~root ~name ->
  let path = Path.(root / Path.v "packages" / Path.v name) in Riot_model.Package.make ~name:(package_name name) ~path ~relative_path:(Path.v ("packages/" ^ name)) ~library:{ path = Path.v "src/lib.ml" } ()

let make_node = fun ~package ~srcs ->
  let graph = Riot_planner.Action_graph.create () in
  let spec = Riot_planner.Action_node.make ~actions:[ Riot_planner.Action.WriteFile { destination = Path.v "out.txt"; content = "ok" } ] ~outs:[ Path.v "out.txt" ] ~srcs ~package ~toolchain:(test_toolchain ()) ~dependency_hashes:(
...
let test_execute_node_copies_package_relative_sources = fun _ctx ->
  match Fs.with_tempdir ~prefix:"action_exec_src_copy_rel"
    (
      fun tmpdir ->
        with_workspace_dirs tmpdir
          (
            fun sandbox ->
...
let test_execute_node_copies_workspace_relative_sources = fun _ctx ->
  match Fs.with_tempdir ~prefix:"action_exec_src_copy_ws"
    (
      fun tmpdir ->
        with_workspace_dirs tmpdir
          (
...
let test_execute_node_cache_hit_materializes_outputs = fun _ctx ->
  match Fs.with_tempdir ~prefix:"action_exec_cache_hit"
    (
      fun tmpdir ->
        let sandbox = Path.(tmpdir / Path.v "sandbox") in
        ...
        | Action_executor.Executed ->
            ...
            let second = Action_executor.execute_node ~completed ~store ~session_id toolchain sandbox node in
            (
...
let tests = Test.[ case "execute_node copies package-relative sources" test_execute_node_copies_package_relative_sources; case "execute_node copies workspace-relative sources" test_execute_node_copies_workspace_relative_sources; case "execute_node cache hit materializes outputs" test_execute_node_cache_hit_materializes_outputs ]
