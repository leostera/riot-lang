...
let make_request = fun ~workspace ~packages -> Riot_build.Request.make ~workspace ~packages ~targets:Riot_model.Target.Host ~scope:Riot_build.Request.Runtime ~profile:Riot_model.Profile.debug ()
...
let with_prepared_lanes = fun request fn ->
  ...
  let lanes = Build_work.prepare_lanes context resolved ~toolchain |> Result.expect ~msg:"expected lane preparation to succeed" in fn lanes

let package_key_string = fun key -> Riot_model.Package.key_to_string key

let initial_plan_package_keys = fun lane -> Build_work.initial_plan_packages lane |> List.map ~fn:(
  fun planned -> Build_work.plan_package_key planned |> package_key_string
)

let initial_plan_targets = fun lane -> Build_work.initial_plan_packages lane |> List.map ~fn:Build_work.plan_package_target
          ...
          (
            fun lanes ->
              ...
              | [ lane ] ->
                  ...
                  let all_targets_match = initial_plan_targets lane |> List.all ~fn:(
                    fun target -> Riot_model.Target.equal host_target target
                  ) in
              ...
              | items -> Error ("expected one initial host lane, got " ^ Int.to_string (List.length items))
          )
    ) with
  | Ok result -> result
          ...
          (
            fun lanes ->
              ...
              | [ lane ] ->
                  ...
                  let expected_keys = [ Package_graph.package_key ~package_name:"app" Package_graph.Runtime |> package_key_string; Package_graph.package_key ~package_name:"lib" Package_graph.Runtime |> package_key_string; Package_graph.package_key ~package_name:"util" Package_graph.Runtime |> package_key_string ] |> List.sort ~compare:String.compare in
...
[ case "build work prepare: initial plan packages preserve dependency-first order" test_initial_plan_packages_follow_topological_order; case "build work prepare: initial plan packages include all workspace packages when unfiltered" test_initial_plan_packages_include_all_workspace_packages_when_unfiltered ]
