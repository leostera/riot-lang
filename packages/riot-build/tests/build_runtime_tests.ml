...
let make_valid_workspace = fun ?target_dir ?toolchain_targets tmpdir ->
  let package = make_package ~root:tmpdir ~name:"demo" ~source:"let value = 42\n" in make_workspace ?target_dir ?toolchain_targets ~root:tmpdir ~packages:[ package ] ()

let make_workspace_with_sources = fun ?toolchain_targets ~root ~packages () ->
  let packages = List.map packages ~fn:(
    fun (name, source) -> make_package ~root ~name ~source
  ) in make_workspace ?toolchain_targets ~root ~packages ()
...
let load_prepared_workspace = fun ~root ->
  ...
  | Ok (workspace, load_errors) ->
      if not (List.is_empty load_errors) then
        ...
        let registry = Pkgs_ml.Registry.create_filesystem ?riot_home:None ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"registry init failed" in Riot_deps.ensure_workspace ~workspace_manager ~mode:Riot_deps.Dep_solver.Refresh ~registry ~workspace () |> Result.map_err ~fn:Riot_model.Pm_error.message
...
let write_path_dependency_package = fun ~root ~dir_name ~package_name ->
  ...
  Fs.write ("[package]\nname = \"" ^ package_name ^ "\"\nversion = \"0.0.1\"\n\n[lib]\npath = \"src/" ^ package_name ^ ".ml\"\n") Path.(pkg_dir / Path.v "riot.toml") |> Result.expect ~msg:"Write path dependency riot.toml failed"
...
let write_app_package_with_path_dependency = fun ~root ~dep_path ->
  ...
  Fs.write ("[package]\nname = \"app\"\nversion = \"0.0.1\"\n\n[lib]\npath = \"src/app.ml\"\n\n[dependencies]\n" ^ "dep = { path = \"" ^ dep_path ^ "\" }\n") Path.(pkg_dir / Path.v "riot.toml") |> Result.expect ~msg:"Write app riot.toml failed"
...
let make_request = fun ~workspace ?(profile = Riot_model.Profile.debug) () -> Riot_build.Request.make ~workspace ~packages:[ package_name "demo" ] ~targets:Riot_model.Target.Host ~scope:Riot_build.Request.Runtime ~profile ()

let make_runtime_request = fun ~workspace ~package_names ~targets ?(scope = Riot_build.Request.Runtime) ?(profile = Riot_model.Profile.debug) ?(requested_parallelism = None) () -> Riot_build.Request.make ~workspace ~packages:package_names ~targets:(Riot_model.Target.Exact targets) ~scope ~profile ~requested_parallelism ()

let make_runtime_inputs = fun ?on_event ~workspace ~package_names ~targets ?scope ?profile ?requested_parallelism () ->
  let request = make_runtime_request ~workspace ~package_names ~targets ?scope ?profile ?requested_parallelism () in
  let context = Build_context.make ?on_event request |> Result.expect ~msg:"expected build context creation to succeed" in
  let resolved = Resolved_build.resolve context request |> Result.expect ~msg:"expected build intent resolution to succeed" in (context, resolved)
...
let write_nested_udp_workspace = fun ~root ~creation_order ->
  ...
  Fs.write "[package]\nname = \"demo\"\nversion = \"0.0.1\"\n\n[lib]\npath = \"src/demo.ml\"\n" Path.(pkg_dir / Path.v "riot.toml") |> Result.expect ~msg:"Write nested riot.toml failed";
  let file_for_key = function
    | "demo_ml" -> Path.(src_dir / Path.v "demo.ml"), "module Net = Net\n"
    | "net_ml" -> Path.(net_dir / Path.v "net.ml"), "module Udp_socket = Udp_socket\nmodule Udp_server = Udp_server\n"
    | "udp_socket_mli" -> Path.(net_dir / Path.v "udp_socket.mli"), "type t\n"
    | "udp_socket_ml" -> Path.(net_dir / Path.v "udp_socket.ml"), "type t = unit\n"
    | "udp_server_mli" -> Path.(net_dir / Path.v "udp_server.mli"), "type handler = socket:Udp_socket.t -> bytes -> unit\nval run : handler -> unit\n"
    | "udp_server_ml" -> Path.(net_dir / Path.v "udp_server.ml"), "type handler = socket:Udp_socket.t -> bytes -> unit\nlet run _ = ()\n"
    | key -> panic ("unknown nested udp workspace file key: " ^ key)
  in
  List.for_each creation_order ~fn:(
    fun key ->
      let path, contents = file_for_key key in Fs.write contents path |> Result.expect ~msg:("Write nested source failed: " ^ key)
  )

let test_release_build_uses_release_lane = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_release_runtime"
    (
      fun tmpdir ->
        let workspace = make_valid_workspace tmpdir in
        let host_target = Riot_model.Riot_dirs.host_target () in
        let release_package_dir =
          Riot_model.Riot_dirs.out_dir_in_workspace ~workspace ~profile:"release" ~target:host_target |> fun out_dir -> Path.(out_dir / Path.v "demo")
        in
        ...
        | Ok _ ->
            if not (Fs.exists release_package_dir |> Result.unwrap_or ~default:false) then
              ...
              if Fs.exists debug_package_dir |> Result.unwrap_or ~default:false then
...
let test_build_respects_custom_target_dir = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_custom_target_runtime"
    (
      fun tmpdir ->
        ...
        let default_release_dir =
          Riot_model.Riot_dirs.out_dir_with_target ~workspace_root:workspace.root ~profile:"release" ~target:host_target |> fun out_dir -> Path.(out_dir / Path.v "demo")
        in
        match build_request (make_request ~workspace ~profile:Riot_model.Profile.release ()) with
        | Error err -> Error ("expected custom-target build to succeed, got: " ^ Riot_build.error_message err)
        | Ok _ ->
            if not (Fs.exists release_package_dir |> Result.unwrap_or ~default:false) then
              Error ("expected release output under custom target dir " ^ Path.to_string release_package_dir)
            else
              if Fs.exists default_release_dir |> Result.unwrap_or ~default:false then
...
let test_nested_udp_workspace_builds_across_file_creation_orders = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_nested_udp_runtime"
    (
      fun tmpdir ->
        let orders =
          [
        ...
        let rec run = function
          ...
          | (order_name, creation_order) :: rest ->
              ...
              begin
                match Riot_model.Workspace_manager.scan workspace_manager root with
                | Error err -> Error ("workspace scan failed for creation order " ^ order_name ^ ": " ^ Riot_model.Workspace_manager.scan_error_message err)
                | Ok (workspace, load_errors) ->
                    if not (List.is_empty load_errors) then
                      Error ("workspace scan had load errors for creation order " ^ order_name)
                    else
                      let registry = Pkgs_ml.Registry.create_filesystem ?riot_home:None ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"registry init failed" in
                      let prepared_workspace = Riot_deps.ensure_workspace ~workspace_manager ~mode:Riot_deps.Dep_solver.Refresh ~registry ~workspace () |> Result.expect ~msg:"workspace prepare failed" in
                      match build_request (make_request ~workspace:prepared_workspace ()) with
                      | Ok _ -> run rest
                      | Error err -> Error ("nested udp build failed for creation order " ^ order_name ^ ": " ^ Riot_build.error_message err)
              end
        in
        run orders
    ) with
...
let test_manifest_path_dependency_change_invalidates_package_cache = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_dep_path_runtime"
    (
      fun tmpdir ->
        ...
        let request = make_runtime_request ~workspace:prepared_workspace ~package_names:[ package_name "app" ] ~targets:(Riot_model.Target.make_set [ Riot_model.Target.current ]) () in
        ...
        | Ok output -> (
          ...
          | Some package_output -> (
            ...
            | Riot_build.Build_result.Skipped reason -> Error ("expected app package to rebuild, got skipped: " ^ reason)
            | Riot_build.Build_result.Failed message -> Error ("expected app package to rebuild, got failure: " ^ message)
          )
        )
    ) with
  | Ok result -> result
...
let test_manifest_build_dependency_path_change_invalidates_package_cache = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_build_dep_path_runtime"
    (
      fun tmpdir ->
        write_workspace_manifest ~root:tmpdir ~members:[ Path.v "packages/app" ];
        write_path_dependency_package ~root:tmpdir ~dir_name:"dep-one" ~package_name:"dep";
...
let test_execute_rejects_invalid_parallelism = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_invalid_parallelism_runtime"
    (
      fun tmpdir ->
        let workspace = make_valid_workspace tmpdir in
        let request = make_runtime_request ~workspace ~package_names:[ package_name "demo" ] ~targets:(Riot_model.Target.make_set [ Riot_model.Target.current ]) ~scope:Riot_build.Request.Runtime ~profile:Riot_model.Profile.debug ~requested_parallelism:(Some 0) () in
        match Build_context.make request with
        | Error (Build_context.InvalidRequestedParallelism 0) -> Ok ()
        | Error err -> (
          match err with
          | Build_context.InvalidRequestedParallelism requested -> Error ("expected invalid parallelism 0, got " ^ Int.to_string requested)
        )
        | Ok _ -> Error "expected invalid parallelism to reject context creation"
    ) with
  | Ok result -> result
  | Error err -> Error ("tempdir failed: " ^ IO.error_message err)

let test_execute_does_not_record_cache_generation_when_disabled = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_no_cache_recording"
    (
      fun tmpdir ->
        let workspace = make_valid_workspace tmpdir in
        let saw_cache_event = ref false in
        let context, spec = make_runtime_inputs ~workspace ~package_names:[ package_name "demo" ] ~targets:(Riot_model.Target.make_set [ Riot_model.Target.current ]) ~on_event:(
          fun event ->
            match event with
            | Riot_build.Event.Phase (Riot_build.Event.CacheGenerationRecordingStarted _) | Riot_build.Event.Phase (Riot_build.Event.CacheGenerationRecorded _) -> saw_cache_event := true
            | _ -> ()
        ) () in
...
let test_execute_partial_failures_by_default = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_default_partial_failure"
    (
      fun tmpdir ->
        let workspace = make_workspace_with_sources ~root:tmpdir ~packages:[
          "good", "let value = 1\n";
          "bad", "let broken =";
        ] () in
        ...
        let context, spec = make_runtime_inputs ~workspace ~package_names:[ package_name "good"; package_name "bad" ] ~targets:(Riot_model.Target.make_set [ Riot_model.Target.current ]) ~requested_parallelism:(Some 1) ~on_event:(
          fun event ->
            match event with
        ...
        | Error (Build_runtime.BuildFailed { errors }) ->
            ...
            if not has_bad_failure then
              ...
              if not !saw_partial_failure then
...
let test_execute_allows_partial_failures = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_allow_partial_failures"
    (
      fun tmpdir ->
        let workspace = make_workspace_with_sources ~root:tmpdir ~packages:[
          "good", "let value = 2\n";
        ...
        let bad = package_name "bad" in
        let context, spec = make_runtime_inputs ~workspace ~package_names:[ good; bad ] ~targets:(Riot_model.Target.make_set [ Riot_model.Target.current ]) ~requested_parallelism:(Some 1) ~on_event:(
          fun _ -> ()
        ) () in
        let result = Riot_build.Internal.Build_runtime.execute ~allow_partial_failures:true context spec in
        match result with
        | Error err -> Error ("expected partial failures to be returned, got: " ^ Build_runtime.error_message err)
        | Ok results ->
            ...
            | Some good_result ->
                ...
                | Riot_build.Build_result.Built _ | Riot_build.Build_result.Cached _ -> (
                ...
                | Riot_build.Build_result.Skipped _ | Riot_build.Build_result.Failed _ -> Error "expected good package result to be successful"
    ) with
  | Ok result -> result
  | Error err -> Error ("tempdir failed: " ^ IO.error_message err)

let test_execute_allows_multi_target_partial_failures = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_allow_partial_failures_multi_target"
    (
      fun tmpdir ->
        ...
        let expected_targets = requested_targets |> Riot_model.Target.Set.to_list |> List.map ~fn:Riot_model.Target.to_string |> List.sort ~compare:String.compare in
        let expected_target_count = List.length expected_targets in
        let workspace = make_workspace_with_sources ~root:tmpdir ~toolchain_targets:expected_targets ~packages:[
          "good", "let value = 2\n";
          "bad", "let broken =";
        ] () in
        let good = package_name "good" in
        ...
        let context, spec = make_runtime_inputs ~workspace ~package_names:[ good; bad ] ~targets:requested_targets ~requested_parallelism:(Some 1) ~on_event:(
          fun event ->
            match event with
        ...
        | Ok results ->
            if not (Int.equal (List.length !started_targets) expected_target_count) then
              ...
              if not (Int.equal (List.length !finished_targets) expected_target_count) then
                Error ("expected " ^ Int.to_string expected_target_count ^ " target builds finished")
              else
                if not (Int.equal (List.length !finished_counts) expected_target_count) then
                  Error ("expected " ^ Int.to_string expected_target_count ^ " target result_count events")
                else
                  if not (List.all !finished_counts ~fn:(
                    fun count -> count = 2
                  )) then
                    Error ("expected each lane to report two package results, got " ^ String.concat ", " (List.map !finished_counts ~fn:Int.to_string))
                  else
                    if not (List.all !partial_flags ~fn:(
                      fun partial -> partial
                    )) then
                      ...
                      let rec equal_target_lists left right =
                        ...
                        | [], [] -> true
                        | left :: left_rest, right :: right_rest -> String.equal left right && equal_target_lists left_rest right_rest
                        | _, _ -> false
                      in
                      if not (equal_target_lists expected_targets started_sorted) then
                        ...
                        if not (equal_target_lists expected_targets finished_sorted) then
                          ...
                          let good_ok =
                            ...
                            | Some package_output -> (
                              match Riot_build.Build_result.package_status package_output with
                              | Riot_build.Build_result.Built _ | Riot_build.Build_result.Cached _ -> true
                              | _ -> false
                            )
                            | None -> false
                          in
...
let test_execute_multi_target_reports_global_returning_results = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_multi_target_returning_results"
    (
      fun tmpdir ->
        let host_target = Riot_model.Target.current in
        let secondary_target =
        ...
        let workspace = make_workspace_with_sources ~root:tmpdir ~toolchain_targets:(Riot_model.Target.Set.to_list requested_targets |> List.map ~fn:Riot_model.Target.to_string) ~packages:[
          "good", "let value = 2\n";
          "bad", "let broken =";
        ] () in
        let context, spec = make_runtime_inputs ~workspace ~package_names:[ package_name "good"; package_name "bad" ] ~targets:requested_targets ~requested_parallelism:(Some 1) ~on_event:(
          fun event ->
            match event with
            | Riot_build.Event.Phase (Riot_build.Event.ReturningResults { result_count; had_partial_failure }) -> returning_event := Some (result_count, had_partial_failure)
            | _ -> ()
        ) () in
        match Riot_build.Internal.Build_runtime.execute ~allow_partial_failures:true context spec with
        | Error err -> Error ("expected partial failures to be returned, got: " ^ Build_runtime.error_message err)
        | Ok _ ->
            ...
            | Some (result_count, had_partial_failure) ->
                if not (Int.equal result_count expected_return_count) then
                  Error ("expected returning result count " ^ Int.to_string expected_return_count ^ ", got " ^ Int.to_string result_count)
                else
                  if not had_partial_failure then
...
let test_execute_multi_target_all_success_reports_aggregated_results = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_multi_target_success_returning"
    (
      fun tmpdir ->
        let host_target = Riot_model.Target.current in
        let secondary_target =
        ...
        let context, spec = make_runtime_inputs ~workspace ~package_names:[ package_name "good"; package_name "nice" ] ~targets:requested_targets ~requested_parallelism:(Some 1) ~on_event:(
          fun event ->
            match event with
            | Riot_build.Event.Phase (Riot_build.Event.ReturningResults { result_count; had_partial_failure }) -> returning_event := Some (result_count, had_partial_failure)
            | _ -> ()
...
let test_execute_multi_target_partial_failures_skip_cache_recording = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_multi_target_partial_skip_cache"
    (
      fun tmpdir ->
        let host_target = Riot_model.Target.current in
        let secondary_target =
        ...
        match Riot_build.Internal.Build_runtime.execute ~allow_partial_failures:true context spec with
        | Error err -> Error ("expected build to succeed with allow flag, got: " ^ Build_runtime.error_message err)
        | Ok _ ->
...
let test_execute_multi_target_success_records_cache_generation = fun _ctx ->
  match Fs.with_tempdir ~prefix:"riot_build_multi_target_cache_generation"
    (
      fun tmpdir ->
        let host_target = Riot_model.Target.current in
        let secondary_target =
        ...
        | Ok _ ->
            if not !recording_started then
              ...
              if not !recording_recorded then
...
[
  ...
  case ~size:Large "build runtime: manifest build dependency path change invalidates package cache" test_manifest_build_dependency_path_change_invalidates_package_cache;
