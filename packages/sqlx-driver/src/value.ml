...
let equal = fun a b ->
  ...
  | Time (h1, min1, s1, us1), Time (h2, min2, s2, us2) -> h1 = h2 && min1 = min2 && s1 = s2 && us1 = us2
  | Uuid x, Uuid y -> x = y
  | Json x, Json y -> x = y
...
let compare = fun a b ->
  ...
  | Timestamp x, Timestamp y -> Time.SystemTime.compare (DateTime.to_system_time x) (DateTime.to_system_time y)
  | TimestampWithTimezone x, TimestampWithTimezone y -> Time.SystemTime.compare (DateTime.to_system_time x) (DateTime.to_system_time y)
