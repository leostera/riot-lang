...
let to_string = function
  | Connection_error { message; cause=Some cause } -> "Connection failed: "
  ^ message
  ^ "\n"
  ...
  | Preparation_error { sql; cause } -> "Failed to prepare statement: "
  ^ sql
  ^ "\n"
  ^ format_db_error cause
  | Execution_error { cause } -> "Execution failed\n" ^ format_db_error cause
  | Transaction_error { message; cause=Some cause } -> "Transaction error: "
  ^ message
  ^ "\n"
  ^ format_db_error cause
