...
let of_string msg = Generic_error msg

(* Create connection error *)
let connection_error ~message ?cause () = Connection_error { message; cause }

(* Create query error *)
let query_error ~sql cause = Query_error { sql = Some sql; cause }

(* Create preparation error *)
let preparation_error ~sql cause = Preparation_error { sql; cause }

(* Create execution error *)
let execution_error cause = Execution_error { cause }

(* Create transaction error *)
let transaction_error ~message ?cause () = Transaction_error { message; cause }

(* Create pool error *)
let pool_error msg = Pool_error msg

(* Format db_error for display *)
let format_db_error err =
  let parts = ["Database error: " ^ err.message] in
  
  let parts = match err.code with
  ...
  let parts = match (err.table_name, err.column_name) with
  ...
  let parts = match err.context with
...
let to_string = function
  ...
  | Connection_error { message; cause = None } ->
      "Connection failed: " ^ message
  | Query_error { sql = Some sql; cause } ->
      "Query failed: " ^ sql ^ "\n" ^ format_db_error cause
  | Query_error { sql = None; cause } ->
      "Query failed\n" ^ format_db_error cause
  ...
  | Transaction_error { message; cause = None } ->
      "Transaction error: " ^ message
  | Pool_error msg ->
      "Pool error: " ^ msg
  | Generic_error msg ->
...
let is_constraint_violation ~name = function
  | Query_error { cause; _ } | Execution_error { cause } | Preparation_error { cause; _ } ->
...
let is_unique_violation err =
  ...
  | Some cause ->
      (match cause.code with
       | Some "23505" -> true  (* PostgreSQL unique violation *)
       | _ -> false)
  | None -> false
...
let is_foreign_key_violation err =
  ...
  | Some cause ->
      (match cause.code with
       | Some "23503" -> true  (* PostgreSQL foreign key violation *)
       | _ -> false)
  | None -> false

(* Check if error is a not null violation *)
let is_not_null_violation err =
  match get_db_error err with
  | Some cause ->
      (match cause.code with
       | Some "23502" -> true  (* PostgreSQL not null violation *)
       | _ -> false)
  | None -> false
