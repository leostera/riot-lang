...
let roundtrip_request:
  type params res. (params, res) Lsp.Method.request -> Json.t -> (Json.t, string) result = fun method_ ->
  fun json ->
    let* (id, params) = Lsp.request_of_json method_ json in
    Ok (Lsp.request_to_json ~id method_ params)

let roundtrip_notification:
  type params. params Lsp.Method.notification -> Json.t -> (Json.t, string) result = fun method_ ->
  fun json ->
...
let roundtrip_fixture = fun relpath ->
  fun json ->
    ...
    | "notifications/did_open.json" -> roundtrip_notification
      Lsp.Text_document_methods.Did_open.notification
      json
    | "notifications/did_change.json" -> roundtrip_notification
      Lsp.Text_document_methods.Did_change.notification
      json
    | "notifications/did_close.json" -> roundtrip_notification
      Lsp.Text_document_methods.Did_close.notification
      json
    | "notifications/publish_diagnostics.json" -> roundtrip_notification
      Lsp.Text_document_methods.Publish_diagnostics.notification
      json
    ...
    | "responses/formatting.json" -> roundtrip_response
      ...
      json
    | "responses/code_action.json" -> roundtrip_response
      Lsp.Text_document_methods.Code_action.request
      json
