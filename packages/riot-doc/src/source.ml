...
let interface_source_of_node = fun ~(package:Riot_model.Package.t) (node: Riot_planner.Module_node.t G.node) ->
  match node.value.kind, node.value.file with
  | Riot_planner.Module_node.MLI mod_, Riot_planner.Module_node.Concrete path when Path.to_string path != "" ->
      let source_path =
        if Path.is_absolute path then
          path
        else Path.(package.path / path)
      in
      let relative_path = relative_path_for ~package_path:package.path path in
      let module_name = Module.module_name mod_ |> Module_name.to_string in
      let module_path =
        Module.module_name mod_ |> Module_name.namespace |> Namespace.to_list |> fun prefix -> prefix @ [ module_name ]
      in
      let qualified_name = Module.qualified_name mod_ in
      let content = Fs.read source_path |> Result.unwrap_or ~default:"" in
      Some {
...
let collect_interfaces = fun ~workspace ~store ~release (package: Riot_model.Package.t) ->
  ...
  let* toolchain = Riot_toolchain.init ~config:toolchain_config |> Result.map_err ~fn:(
    fun err -> "failed to initialize toolchain for documentation planning: " ^ err
  )
  in
    ...
    source_groups = [
      Riot_planner.Module_graph.{
        ...
        root_mode = (
          ...
          | Some _ -> Riot_planner.Module_graph.Library_root { library_name = Riot_model.Package_name.to_string package.name }
  ...
  in
  let* plan = Riot_planner.Module_planner.plan_node plan_input |> Result.map_err ~fn:Riot_planner.Planning_error.to_string in Ok (G.map plan.module_graph ~fn:(
    fun (_id, node) -> interface_source_of_node ~package node
  ) |> List.filter_map ~fn:(
    function
    | Some source -> Some source
    | None -> None
  ) |> List.unique ~compare:(
    fun left right -> String.compare (module_path_key left.module_path) (module_path_key right.module_path)
  ))

let build_lookup = fun sources -> { sources; by_module_path = List.map sources ~fn:(
  fun source -> (module_path_key source.module_path, source)
); by_qualified_name = List.map sources ~fn:(
  fun source -> (source.qualified_name, source)
) }

let find_root_interface = fun ~package_name sources ->
  let expected_path = [ package_module_name package_name ] in List.find sources ~fn:(
    fun source -> source.module_path = expected_path
  )

let find_by_module_path = fun lookup module_path -> List.find lookup.by_module_path ~fn:(
  fun (key, _) -> String.equal key (module_path_key module_path)
) |> Option.map ~fn:(
  fun (_, source) -> source
)
...
let resolve_module_path = fun lookup ~current_path ~target_path ->
  ...
  let qualified_match =
    match target_path with
    | [ qualified_name ] when String.contains qualified_name "_" -> List.find lookup.by_qualified_name ~fn:(
      fun (key, _) -> String.equal key qualified_name
    ) |> Option.map ~fn:(
      fun (_, source) -> source
    )
    | _ -> None
  ...
  | None -> (
    ...
    | None -> (
      ...
      | [] -> None
      | target_name :: _ ->
          match lookup.sources |> List.find ~fn:(
            fun source -> source.module_name = target_name && source_prefix source = current_path
          ) with
          | Some source -> Some source
          | None -> List.find lookup.sources ~fn:(
            fun source -> source.module_name = target_name
          )
    )
  )
