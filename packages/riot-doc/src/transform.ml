...
let leading_docstrings = fun node ->
  ...
  (
    ...
    | None -> ()
    | Some token ->
        Syn.Ast.Token.for_each_leading_trivia_item token
          ~fn:(
            function
...
let variant_constructor_details = fun source variant ->
  ...
  Syn.Ast.VariantType.for_each_constructor variant
    ~fn:(fun constructor ->
...
let record_field_details = fun source record ->
  let details = Vector.with_capacity ~size:4 in
  Syn.Ast.RecordType.for_each_field record
    ~fn:(fun field ->
...
let detail_groups_of_type_member = fun source member ->
  match Syn.Ast.TypeDeclaration.Member.variant_type member, Syn.Ast.TypeDeclaration.Member.record_type
    member with
  | Some variant, _ ->
...
let items_of_type_declaration = fun ?docstring source decl ->
  ...
  Syn.Ast.TypeDeclaration.for_each_member decl
    ~fn:(fun member ->
      ...
      if not (String.equal name "") then
        Vector.push
          ...
          ~value:(make_item
            ?docstring
            ~detail_groups:(detail_groups_of_type_member source member)
  ...
  let rec loop = function
    ...
    | item :: rest -> (
        ...
        | Syn.Ast.SignatureItem.Module decl ->
            ...
            let nested_items = collect_signature_items
              (Syn.Ast.ModuleDeclaration.for_each_signature_item decl) in
            let* child =
        ...
        | Syn.Ast.SignatureItem.Value decl ->
            value_item_of_declaration ?docstring:attached_doc source decl
            |> List.for_each ~fn:(fun item -> Vector.push acc_items ~value:item);
            loop rest
...
and of_interface_source = fun ~lookup ?path ?docstring (source_file: Source.interface_source) ->
  let* source_slice = IO.IoVec.IoSlice.from_string source_file.content
  |> Result.map_err
    ~fn:(fun error ->
      "failed to read source for "
      ^ Path.to_string source_file.relative_path
      ^ ": "
      ^ IO.IoSlice.error_message error) in
