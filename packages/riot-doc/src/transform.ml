...
let clean_docstring = fun raw -> raw |> String.split ~by:"\n" |> List.map ~fn:(
  fun line ->
...
let slugify = fun text -> text |> String.map ~fn:(
  fun ch ->
    match ch with
    | '/' | '\\' | '.' | ' ' | '-' | ':' | '(' | ')' -> '_'
    | _ -> ch
...
let snippet_of_node = fun source node ->
  let start_, finish = Syn.Ast.Node.raw_range node in
  let start_ = Int.max 0 start_ in
  let finish = Int.min (String.length source) finish in
  if finish <= start_ then
    ""
  else String.sub source ~offset:start_ ~len:(finish - start_) |> String.trim

let strip_comments = fun text ->
  let depth, pending, acc = String.fold_left ~fn:(
...
let clean_member_signature = fun text -> text |> String.trim |> strip_prefix "|" |> String.trim |> strip_suffix ";" |> String.trim
...
let read_identifier = fun text idx ->
  if idx >= String.length text then
    ...
    if not (is_ident_char (String.get_unchecked text ~at:idx)) then
      None
    else
      let rec loop cursor =
      ...
      in
      let finish = loop idx in Some (String.sub text ~offset:idx ~len:(finish - idx), finish)
...
let extract_percent_macro_names = fun line ->
  let rec loop idx acc =
    if idx >= String.length line then
      ...
      if String.get_unchecked line ~at:idx = '%' then
...
let find_substring = fun text pattern ->
  ...
  let rec loop idx =
    if idx + pattern_length > text_length then
      ...
      if String.sub text ~offset:idx ~len:pattern_length = pattern then
        Some idx
      else loop (idx + 1)
  in
  loop 0
...
let macro_items_of_snippet = fun ?docstring snippet ->
  ...
  let names = extract_deriving_macro_names snippet @ extract_percent_macro_names snippet |> List.unique ~compare:String.compare in names |> List.map ~fn:(
    fun name ->
      {
...
let make_detail = fun ?docstring ~name signature -> ({ name = name; signature = clean_member_signature signature; docstring = docstring } : Doctree.item_detail)
...
let leading_docstrings = fun node ->
  ...
  (
    ...
    | None -> ()
    | Some token -> Syn.Ast.Token.for_each_leading_trivia_item token ~fn:(
      function
      ...
      | Syn.Ast.Token.Whitespace | Syn.Ast.Token.Comment _ -> ()
    )
  );
  vector_to_list docs

let leading_docstring = fun node -> leading_docstrings node |> docstring_of_docs
...
let split_initial_docstrings = fun ~is_source_root docs ->
  if is_source_root then
    ...
    | _ ->
        ...
        in
        (overview, item_doc)
  else (None, docstring_of_docs docs)
...
let variant_constructor_details = fun source variant ->
  ...
  Syn.Ast.VariantType.for_each_constructor variant ~fn:(
    fun constructor ->
      let name = token_text (Syn.Ast.VariantConstructor.name constructor) in
      if not (String.equal name "") then
        Vector.push details ~value:(make_detail ?docstring:(leading_docstring constructor) ~name (snippet_of_node source constructor))
  );
  vector_to_list details

let record_field_details = fun source record ->
  let details = Vector.with_capacity ~size:4 in
  Syn.Ast.RecordType.for_each_field record ~fn:(
    fun field ->
      let name = token_text (Syn.Ast.RecordField.name field) in
      if not (String.equal name "") then
        Vector.push details ~value:(make_detail ?docstring:(leading_docstring field) ~name (snippet_of_node source field))
  );
  vector_to_list details

let detail_groups_of_type_member = fun source member ->
  match Syn.Ast.TypeDeclaration.Member.variant_type member, Syn.Ast.TypeDeclaration.Member.record_type member with
  | Some variant, _ ->
      let details = variant_constructor_details source variant in
      if details = [] then
        []
      else [ ({ title = "Constructors"; details = details } : Doctree.item_detail_group) ]
  | None, Some record ->
      let details = record_field_details source record in
      if details = [] then
        []
      else [ ({ title = "Fields"; details = details } : Doctree.item_detail_group) ]
  | None, None -> []
...
let items_of_type_declaration = fun ?docstring source decl ->
  ...
  Syn.Ast.TypeDeclaration.for_each_member decl ~fn:(
    fun member ->
      let name = type_member_name member fallback_name in
      if not (String.equal name "") then
        Vector.push items ~value:(make_item ?docstring ~detail_groups:(detail_groups_of_type_member source member) ~kind:Doctree.Type_item ~name snippet)
  );
  if Vector.is_empty items && not (String.equal fallback_name "") then
    Vector.push items ~value:(make_item ?docstring ~kind:Doctree.Type_item ~name:fallback_name snippet);
...
let value_name = fun for_each_name_token ->
  ...
  for_each_name_token ~fn:(
    fun token -> Vector.push tokens ~value:(Syn.Ast.Token.text token)
  );
  vector_to_list tokens |> String.concat ""
...
let value_item_of_declaration = fun ?docstring source decl ->
  ...
  else make_item ?docstring ~kind:Doctree.Function_item ~name snippet :: macro_items_of_snippet ?docstring snippet
...
let module_path_segments = fun decl ->
  ...
  Syn.Ast.ModuleDeclaration.for_each_body_path_ident decl ~fn:(
    fun token -> Vector.push segments ~value:(Syn.Ast.Token.text token)
  );
  ...
  let rec loop = function
    ...
    | item :: rest -> (
      ...
      let initial_overview, attached_doc =
        ...
        else (None, docstring_of_docs leading_docs)
      in
      ...
      | Syn.Ast.SignatureItem.Module decl ->
          ...
          let nested_items = collect_signature_items (Syn.Ast.ModuleDeclaration.for_each_signature_item decl) in
          let* child =
            if nested_items != [] then
              module_of_signature_items ~lookup ~source ~source_path ~path:child_path ?docstring:attached_doc ~is_source_root:false ~snippet:child_snippet nested_items
            else
              match module_path_segments decl with
              | [] -> module_doc_of_empty ~source_path ~path:child_path ?docstring:attached_doc ~snippet:child_snippet ()
              | target_path -> (
                match Source.resolve_module_path lookup ~current_path:path ~target_path with
                | Some child_source -> of_interface_source ~lookup ~path:child_path ?docstring:attached_doc child_source
                | None -> module_doc_of_empty ~source_path ~path:child_path ?docstring:attached_doc ~snippet:child_snippet ()
              )
          in
      ...
      | Syn.Ast.SignatureItem.ModuleType decl ->
          ...
          if not (String.equal name "") then
            Vector.push acc_items ~value:(make_item ?docstring:attached_doc ~kind:Doctree.Module_item ~name (snippet_of_node source decl));
          loop rest
      | Syn.Ast.SignatureItem.Type decl ->
          items_of_type_declaration ?docstring:attached_doc source decl |> List.for_each ~fn:(
            fun item -> Vector.push acc_items ~value:item
          );
          loop rest
      | Syn.Ast.SignatureItem.Value decl ->
          value_item_of_declaration ?docstring:attached_doc source decl |> List.for_each ~fn:(
            fun item -> Vector.push acc_items ~value:item
          );
          loop rest
      | Syn.Ast.SignatureItem.External decl ->
          external_item_of_declaration ?docstring:attached_doc source decl |> List.for_each ~fn:(
            fun item -> Vector.push acc_items ~value:item
          );
          loop rest
      | Syn.Ast.SignatureItem.TypeExtension _ | Syn.Ast.SignatureItem.Open _ | Syn.Ast.SignatureItem.Include _ | Syn.Ast.SignatureItem.Exception _ | Syn.Ast.SignatureItem.Class _ | Syn.Ast.SignatureItem.Extension _ | Syn.Ast.SignatureItem.Attribute _ | Syn.Ast.SignatureItem.Error _ | Syn.Ast.SignatureItem.Unknown _ -> loop rest
    )
  in
  loop items
and of_interface_source = fun ~lookup ?path ?docstring (source_file: Source.interface_source) ->
  let* source_slice = IO.IoVec.IoSlice.from_string source_file.content |> Result.map_err ~fn:(
    fun error -> "failed to read source for " ^ Path.to_string source_file.relative_path ^ ": " ^ IO.IoSlice.error_message error
  )
  in
  ...
  in
  module_of_signature_items ~lookup ~source ~source_path:source_file.relative_path ~path ?docstring ~is_source_root:true ~snippet:(String.trim source) items
