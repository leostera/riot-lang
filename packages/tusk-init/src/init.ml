...
let command =
  ...
  let open Arg in
  command "init"
  |> about "Initialize a new Tusk workspace"
  |> args
       [
         positional "path" 
         |> help "Path for new workspace (default: current directory)";
         
         flag "name" 
         |> long "name" 
         |> short 'n'
         |> help "Workspace name (default: directory basename)";
         
         flag "lib" 
         |> long "lib" 
         |> help "Create library package (default)";
         
         flag "bin" 
         |> long "bin" 
         |> help "Create binary package";
       ]
...
let package_name_to_module_name name =
  Module_name.of_string name |> Module_name.to_string

(** Create workspace tusk.toml *)
let create_workspace_toml target_dir workspace_name =
...
kind = "native"
|} in
  let path = Path.(target_dir / Path.v "tusk.toml") in
  match Fs.write content path with
  | Ok () -> 
      println "✓ Created tusk.toml";
      Ok ()
  | Error _e -> 
      Error (Failure "Failed to create tusk.toml")
...
|} in
  ...
  | Ok () -> 
      ...
      Ok ()
  | Error _e -> 
      Error (Failure "Failed to create ocaml-toolchain.toml")

(** Create .gitignore *)
let create_gitignore target_dir =
...
|} in
  ...
  | Ok () -> 
      ...
      Ok ()
  | Error _e -> 
      Error (Failure "Failed to create .gitignore")

(** Create README.md *)
let create_readme target_dir workspace_name =
  let content = {|# |} ^ workspace_name ^ {|

A Tusk workspace for OCaml development.
...
```bash
tusk new packages/my-new-package
```

Then add it to `tusk.toml` workspace members.

## Structure

- `packages/` - Workspace packages
- `tusk.toml` - Workspace configuration
...
|} in
  ...
  | Ok () -> 
      ...
      Ok ()
  | Error _e -> 
      Error (Failure "Failed to create README.md")

(** Create default package structure *)
let create_default_package target_dir workspace_name is_library =
  let pkg_dir = Path.(target_dir / Path.v "packages" / Path.v workspace_name) in
  let src_dir = Path.(pkg_dir / Path.v "src") in
  
  (* Create directories *)
  let* () = match Fs.create_dir_all src_dir with
    | Ok () -> Ok ()
...
|} in
  ...
  let* () = match Fs.write pkg_toml_content pkg_toml_path with
    | Ok () -> Ok ()
    | Error _e -> Error (Failure "Failed to create package tusk.toml")
  in
...
let run matches =
  ...
  let target_dir = match path_arg with
    | Some p -> 
        let path = match Path.of_string p with
          | Ok p -> p
          | Error _ -> Path.v p
        in
        if Path.is_absolute path then path
          ...
          Path.(cwd / path)
    | None -> 
        Env.current_dir () |> Result.expect ~msg:"Cannot get current directory"
  in
  
  (* Determine workspace/package name *)
  let workspace_name = match name_flag with
    | Some name -> name
  ...
  let* () = match Fs.create_dir_all target_dir with
    | Ok () -> Ok ()
  ...
  println "  tusk build";
  println ("  tusk run " ^ validated_name);
  println "";
