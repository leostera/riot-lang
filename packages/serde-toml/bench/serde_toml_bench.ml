...
let io_writer_of_buffer =
  let module Write = struct
    ...
    let write = fun buffer ~from ->
      ...
      IO.Buffer.append_slice buffer (IO.Buffer.readable from) |> Result.expect ~msg:"serde-toml bench writer should append buffer contents";
      Ok written
    ...
    let write_vectored = fun buffer ~from ->
      ...
      IO.IoVec.for_each from ~fn:(
        fun chunk ->
          IO.Buffer.append_slice buffer chunk |> Result.expect ~msg:"serde-toml bench writer should append slices";
...
let manifest_decode = De.record_mut ~fields:manifest_fields ~create:(
  ...
  fun (builder: manifest_builder) ->
    match builder.ship, builder.emergency, builder.crew_count, builder.status, builder.home, builder.tags, builder.scores, builder.stops, builder.mirrors with
      ...
      Ser.fields
        [
          ...
          Ser.field "status" status_encode
            (
              fun (value: manifest) -> value.status
            );
...
let repeat = fun text count ->
  ...
  for _index = 1 to count do IO.Buffer.add_string buffer text done;
...
let tags_of_count = fun count ->
  ...
  for index = 0 to count - 1 do Vector.push tags ~value:("grand-line-marker-" ^ Int.to_string index) done;
  tags
...
let stop_to_toml = fun (value: stop) ->
  Toml.Table [
    "island", Toml.String value.island;
    "supplies", Toml.Int value.supplies;
  ]

let berth_to_toml = fun (value: berth) ->
  Toml.Table [
    "island", Toml.String value.island;
    "berth", Toml.Int value.berth;
  ]
...
let manifest_to_toml = fun (value: manifest) ->
  Toml.Table [
    ...
    "tags", Toml.Array (vec_to_list value.tags |> List.map ~fn:(
      fun item -> Toml.String item
    ));
    "scores", Toml.Array (Array.to_list value.scores |> List.map ~fn:(
      fun item -> Toml.Int item
    ));
...
let stop_of_toml = fun value ->
  let table = table_of_toml value in ({ island = string_of_toml (field table "island"); supplies = int_of_toml (field table "supplies") } : stop)

let berth_of_toml = fun value ->
  let table = table_of_toml value in ({ island = string_of_toml (field table "island"); berth = int_of_toml (field table "berth") } : berth)

let vec_of_list = fun values ->
  let vec = Vector.with_capacity ~size:(List.length values) in
  List.for_each values ~fn:(
    fun value -> Vector.push vec ~value
  );
  vec
...
let equal_manifest = fun (left: manifest) (right: manifest) ->
  let equal_lists equal left right =
    ...
    | _ -> false
  in
  String.equal left.ship right.ship && Bool.equal left.emergency right.emergency && Int.equal left.crew_count right.crew_count && left.status = right.status && equal_berth left.home right.home && vec_to_list left.tags = vec_to_list right.tags && Array.to_list left.scores = Array.to_list right.scores && equal_lists equal_stop (vec_to_list left.stops) (vec_to_list right.stops) && equal_lists equal_stop (Array.to_list left.mirrors) (Array.to_list right.mirrors)
...
let build_fixture = fun ({ label; tag_count; score_count; stop_count; string_repeat }: fixture_spec) ->
  ...
  in
  let encoded = Serde_toml.to_string manifest_encode value |> Result.expect ~msg:("expected " ^ label ^ " fixture to encode") in
  let parsed_with_std = Toml.parse encoded |> Result.expect ~msg:("expected " ^ label ^ " fixture to parse with Std.Data.Toml") in
...
let bench_serde_encode_in_memory = fun fixture () -> ignore (Serde_toml.to_string manifest_encode fixture.value)

let bench_serde_encode_writer = fun fixture () ->
  let buffer = IO.Buffer.create ~size:(String.length fixture.encoded) in ignore (Serde_toml.to_writer manifest_encode (io_writer_of_buffer buffer) fixture.value)

let bench_serde_decode_in_memory = fun fixture () -> ignore (Serde_toml.from_string manifest_decode fixture.encoded)

let bench_serde_decode_reader = fun fixture () -> ignore (Serde_toml.from_reader manifest_decode (String.to_reader ~chunk_size:io_chunk_size fixture.encoded))

let bench_std_parse = fun fixture () -> ignore (Toml.parse fixture.encoded)

let bench_std_decode_typed = fun fixture () -> ignore (fixture.encoded |> Toml.parse |> Result.expect ~msg:("expected " ^ fixture.label ^ " fixture to parse with Std.Data.Toml") |> manifest_of_toml)
...
let benchmark_suite = fun fixture ->
  ...
  in
  Bench.[
    with_config ~config ("serde-toml encode in-memory " ^ fixture.label ^ " payload (" ^ serde_size ^ ")") (bench_serde_encode_in_memory fixture);
    with_config ~config ("serde-toml encode writer " ^ fixture.label ^ " payload (" ^ serde_size ^ ")") (bench_serde_encode_writer fixture);
    with_config ~config ("serde-toml decode in-memory " ^ fixture.label ^ " payload (" ^ serde_size ^ ")") (bench_serde_decode_in_memory fixture);
    with_config ~config ("serde-toml decode reader " ^ fixture.label ^ " payload (" ^ serde_size ^ ")") (bench_serde_decode_reader fixture);
    with_config ~config ("std-data-toml parse-only " ^ fixture.label ^ " payload (" ^ serde_size ^ ")") (bench_std_parse fixture);
    with_config ~config ("std-data-toml decode typed " ^ fixture.label ^ " payload (" ^ serde_size ^ ")") (bench_std_decode_typed fixture);
    with_config ~config ("std-data-toml render " ^ fixture.label ^ " tree (" ^ std_size ^ ")") (bench_std_render fixture);
  ]

let main ~args =
  let small_fixture = build_fixture small_fixture_spec in
  let large_fixture = build_fixture large_fixture_spec in
  let benchmarks = benchmark_suite small_fixture @ benchmark_suite large_fixture in Bench.Cli.main ~name:"serde-toml benchmarks" ~benchmarks ~args
