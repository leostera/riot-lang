...
type pose_builder = { mutable island: string option; mutable bearing: float option }
...
let status_decode = De.variant [ De.Variant.unit "Active" Active; De.Variant.unit "Draft" Draft; De.Variant.unit "Archived" Archived ]
...
let pet_decode =
  De.variant
    [
      ...
      De.Variant.newtype "Reindeer" De.string
        (
          fun value -> Reindeer value
        );
    ]
...
let pose_decode = De.record_mut ~fields:pose_fields ~create:(
  fun (): pose_builder -> { island = None; bearing = None }
) ~step:(
      ...
      Ser.fields
        [
          Ser.field "island" Ser.string
            (
              fun (value: pose) -> value.island
            );
          Ser.field "bearing" Ser.float
            (
              fun (value: pose) -> value.bearing
            );
        ]
    )
...
let sample_decode = De.record_mut ~fields:sample_fields ~create:(
  fun (): sample_builder ->
    {
      title = None;
      active = None;
  ...
  fun (builder: sample_builder) ->
    match builder.title, builder.active, builder.count, builder.small, builder.big, builder.ratio, builder.status, builder.pet, builder.marker, builder.pose, builder.tags, builder.scores, builder.stops, builder.mirrors with
...
let sample_encode =
  Ser.record
    (
      Ser.fields
        [
          Ser.field "title" Ser.string
            (
              fun (value: sample) -> value.title
            );
          Ser.field "active" Ser.bool
            (
              fun (value: sample) -> value.active
            );
          Ser.field "count" Ser.int
            (
              fun (value: sample) -> value.count
            );
          Ser.field "small" Ser.int32
            (
              fun (value: sample) -> value.small
            );
          Ser.field "big" Ser.int64
            (
              fun (value: sample) -> value.big
            );
          Ser.field "ratio" Ser.float
            (
              fun (value: sample) -> value.ratio
            );
          Ser.field "nickname" (Ser.option Ser.string)
            (
              fun (value: sample) -> value.nickname
            );
          Ser.field "status" status_encode
            (
              fun (value: sample) -> value.status
            );
          Ser.field "pet" pet_encode
            (
              fun (value: sample) -> value.pet
            );
          Ser.field "marker" Ser.null
            (
              fun (value: sample) -> value.marker
            );
          Ser.field "pose" pose_encode
            (
              fun (value: sample) -> value.pose
            );
          Ser.field "tags" (Ser.list Ser.string)
            (
              fun (value: sample) -> value.tags
            );
          Ser.field "scores" (Ser.array Ser.int)
            (
              fun (value: sample) -> value.scores
            );
          Ser.field "stops" (Ser.list stop_encode)
            (
              fun (value: sample) -> value.stops
            );
          Ser.field "mirrors" (Ser.array stop_encode)
            (
              fun (value: sample) -> value.mirrors
            );
        ]
    )

let single_field_decode = fun field_name decode ->
  let fields = De.fields [ De.field field_name () ] in De.record_mut ~fields ~create:(
    fun () -> ref None
  ) ~step:(
    fun reader value field ->
      match field with
...
let empty_decode = De.record_mut ~fields:(De.fields []) ~create:(
  fun () -> ()
) ~step:(
  fun _reader _builder _field -> ()
) ~finish:(
  fun () -> ()
)
...
let equal_vec = fun equal left right ->
  ...
  | 0 -> List.zip left right |> List.all ~fn:(
    fun (left, right) -> equal left right
  )
  | _ -> false

let equal_pose = fun (left: pose) (right: pose) -> String.equal left.island right.island && Float.equal left.bearing right.bearing

let equal_stop = fun (left: stop) (right: stop) -> String.equal left.island right.island && Int.equal left.supplies right.supplies
...
let equal_sample = fun (left: sample) (right: sample) -> String.equal left.title right.title && Bool.equal left.active right.active && Int.equal left.count right.count && Int32.equal left.small right.small && Int64.equal left.big right.big && Float.equal left.ratio right.ratio && left.nickname = right.nickname && equal_status left.status right.status && equal_pet left.pet right.pet && equal_pose left.pose right.pose && equal_vec String.equal left.tags right.tags && left.scores = right.scores && equal_vec equal_stop left.stops right.stops && Array.to_list left.mirrors = Array.to_list right.mirrors
...
let status_gen =
  Generator.frequency
    [
      1, Generator.return Active;
      1, Generator.return Draft;
      1, Generator.return Archived;
    ]
...
let sample_gen =
  Generator.map3
    ...
    (Generator.pair (Generator.pair (Generator.pair Generator.string Generator.bool) Generator.int) (Generator.triple Generator.int32 Generator.int64 finite_float_gen))
    (Generator.pair (Generator.option Generator.string) (Generator.pair status_gen (Generator.pair pet_gen pose_gen)))
    (Generator.pair string_vec_gen (Generator.pair int_array_gen (Generator.pair stop_vec_gen stop_array_gen)))
  ...
  Test.property ~size:Test.Large name ~examples
    (
      fun _ctx ->
        ...
        | Property.Success -> Ok ()
        | Property.Failure { counter_example; shrink_steps } -> Error (String.concat "\n" [ "Property failed"; "Counter-example (after " ^ Int.to_string shrink_steps ^ " shrink steps):"; counter_example ])
...
let roundtrip_io = fun encode decode equal value ->
  ...
  | Ok () -> (
    match Serde_toml.from_reader decode (String.to_reader ~chunk_size:io_chunk_size (IO.Buffer.contents buffer)) with
    | Ok decoded -> equal decoded value
...
let unit_roundtrip_prop =
  run_property "serde-toml property empty record roundtrips" Arbitrary.bool
    (
      fun _ ->
        roundtrip_in_memory empty_encode empty_decode
          (
            fun () () -> true
          )
          ()
...
let bool_roundtrip_prop = run_property "serde-toml property bool field roundtrips" Arbitrary.bool (roundtrip_in_memory (single_field_encode "value" Ser.bool) (single_field_decode "value" De.bool) Bool.equal)

let int_roundtrip_prop = run_property "serde-toml property int field roundtrips" Arbitrary.int (roundtrip_in_memory (single_field_encode "value" Ser.int) (single_field_decode "value" De.int) Int.equal)

let int32_roundtrip_prop = run_property "serde-toml property int32 field roundtrips" Arbitrary.int32 (roundtrip_in_memory (single_field_encode "value" Ser.int32) (single_field_decode "value" De.int32) Int32.equal)

let int64_roundtrip_prop = run_property "serde-toml property int64 field roundtrips" Arbitrary.int64 (roundtrip_in_memory (single_field_encode "value" Ser.int64) (single_field_decode "value" De.int64) Int64.equal)

let float_roundtrip_prop = run_property "serde-toml property float field roundtrips" finite_float_arb (roundtrip_in_memory (single_field_encode "value" Ser.float) (single_field_decode "value" De.float) Float.equal)

let string_roundtrip_prop = run_property "serde-toml property string field roundtrips" Arbitrary.string (roundtrip_in_memory (single_field_encode "value" Ser.string) (single_field_decode "value" De.string) String.equal)

let option_string_roundtrip_prop = run_property "serde-toml property option string field roundtrips" Arbitrary.(option string) (roundtrip_in_memory (single_field_encode "value" (Ser.option Ser.string)) (optional_field_decode "value" (De.option De.string)) (=))

let string_list_roundtrip_prop = run_property ~examples:composite_examples "serde-toml property string list field roundtrips" string_vec_arb (roundtrip_in_memory (single_field_encode "value" (Ser.list Ser.string)) (single_field_decode "value" (De.list De.string)) (equal_vec String.equal))

let int_array_roundtrip_prop = run_property ~examples:composite_examples "serde-toml property int array field roundtrips" int_array_arb (roundtrip_in_memory (single_field_encode "value" (Ser.array Ser.int)) (single_field_decode "value" (De.array De.int)) (=))

let status_roundtrip_prop = run_property ~examples:composite_examples "serde-toml property unit enum field roundtrips" status_arb (roundtrip_in_memory (single_field_encode "value" status_encode) (single_field_decode "value" status_decode) equal_status)

let pet_roundtrip_prop = run_property ~examples:composite_examples "serde-toml property newtype enum field roundtrips" pet_arb (roundtrip_in_memory (single_field_encode "value" pet_encode) (single_field_decode "value" pet_decode) equal_pet)

let pose_roundtrip_prop = run_property ~examples:composite_examples "serde-toml property nested record field roundtrips" pose_arb (roundtrip_in_memory (single_field_encode "value" pose_encode) (single_field_decode "value" pose_decode) equal_pose)

let stop_array_roundtrip_prop = run_property ~examples:composite_examples "serde-toml property record array field roundtrips" (Arbitrary.make ~print:(Printer.array print_stop) stop_array_gen) (roundtrip_in_memory (single_field_encode "value" (Ser.array stop_encode)) (single_field_decode "value" (De.array stop_decode)) (=))

let sample_roundtrip_prop = run_property ~examples:composite_examples "serde-toml property sample roundtrips" sample_arb (roundtrip_in_memory sample_encode sample_decode equal_sample)

let sample_io_roundtrip_prop = run_property ~examples:composite_examples "serde-toml property sample roundtrips over io" sample_arb (roundtrip_io sample_encode sample_decode equal_sample)
