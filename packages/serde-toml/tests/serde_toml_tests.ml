...
let crew_member_fields = De.fields
  [
...
let voyage_decode =
  De.record_mut ~fields:voyage_fields ~create:(fun () : voyage_builder ->
    { ship = None; destination = None; stops = None })
    ~step:(fun reader builder field ->
      match field with
...
let crew_member_decode =
  De.record_mut ~fields:crew_member_fields
...
let crew_member_encode = Ser.record
  (
    Ser.fields
      [
        ...
        Ser.field "checkpoints" (Ser.array Ser.int) (fun (value: crew_member) -> value.checkpoints);
...
let manifest_decode =
  De.record_mut ~fields:manifest_fields
    ...
    ~step:(fun reader builder field ->
      ...
      | Some Manifest_reserves -> builder.reserves <- Some (De.read
        reader
        (De.array crew_member_decode))
...
let manifest_encode = Ser.record
  (Ser.fields
    [
      ...
      Ser.field "reserves" (Ser.array crew_member_encode) (fun (value: manifest) -> value.reserves);
      Ser.field "scout" pet_encode (fun (value: manifest) -> value.scout);
    ])

let roster_decode =
  De.record_mut ~fields:roster_fields ~create:(fun () : roster_builder -> { crew = None })
    ~step:(fun reader builder field ->
      match field with
...
let test_rejects_top_level_scalars = fun _ctx ->
  ...
  | Error err -> Error ("expected top-level scalar encode to fail clearly, got "
