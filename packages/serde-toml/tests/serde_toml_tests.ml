...
let io_writer_of_buffer =
  let module Write = struct
    ...
    let write = fun buffer ~from ->
      ...
      IO.Buffer.append_slice buffer (IO.Buffer.readable from) |> Result.expect ~msg:"serde-toml test writer should append buffer contents";
      Ok written
    ...
    let write_vectored = fun buffer ~from ->
      ...
      IO.IoVec.for_each from ~fn:(
        fun chunk ->
          IO.Buffer.append_slice buffer chunk |> Result.expect ~msg:"serde-toml test writer should append slices";
...
type voyage = { ship: string; destination: pose; stops: stop vec }
...
type roster = { crew: crew_member vec }
...
type roster_builder = { mutable crew: crew_member vec option }
...
let equal_crew_member = fun (left: crew_member) (right: crew_member) -> String.equal left.name right.name && Int64.equal left.bounty right.bounty && Bool.equal left.active right.active && Int.equal left.cabins right.cabins && Int32.equal left.small right.small && Float.equal left.ratio right.ratio && left.nickname = right.nickname && vec_to_list left.skills = vec_to_list right.skills && left.checkpoints = right.checkpoints && equal_role left.role right.role && equal_pet left.pet right.pet && equal_pose left.pose right.pose

let equal_manifest = fun (left: manifest) (right: manifest) -> String.equal left.ship right.ship && Bool.equal left.emergency right.emergency && equal_crew_member left.featured right.featured && vec_to_list left.crew = vec_to_list right.crew && Array.to_list left.reserves = Array.to_list right.reserves && equal_pet left.scout right.scout

let role_decode = De.variant [ De.Variant.unit "Captain" Captain; De.Variant.unit "Doctor" Doctor; De.Variant.unit "Navigator" Navigator ]
...
let voyage_fields = De.fields [ De.field "ship" Voyage_ship; De.field "destination" Voyage_destination; De.field "stops" Voyage_stops ]
...
let voyage_decode = De.record_mut ~fields:voyage_fields ~create:(
  fun (): voyage_builder -> { ship = None; destination = None; stops = None }
) ~step:(
  fun reader builder field ->
    match field with
      ...
      Ser.fields
        [
          Ser.field "ship" Ser.string
            (
              fun (value: voyage) -> value.ship
            );
          Ser.field "destination" pose_encode
            (
              fun (value: voyage) -> value.destination
            );
          Ser.field "stops" (Ser.list stop_encode)
            (
              fun (value: voyage) -> value.stops
            );
        ]
    )

let crew_member_decode = De.record_mut ~fields:crew_member_fields ~create:(
  fun (): crew_member_builder ->
    {
      name = None;
      bounty = None;
      active = None;
  ...
  fun builder ->
    match builder.name, builder.bounty, builder.active, builder.cabins, builder.small, builder.ratio, builder.skills, builder.checkpoints, builder.role, builder.pet, builder.flag, builder.pose with
    | (Some name, Some bounty, Some active, Some cabins, Some small, Some ratio, Some skills, Some checkpoints, Some role, Some pet, Some flag, Some pose) ->
        let nickname =
...
let crew_member_encode =
  Ser.record
    (
      Ser.fields
        [
          Ser.field "name" Ser.string
            (
              fun (value: crew_member) -> value.name
            );
          Ser.field "bounty" Ser.int64
            (
              fun (value: crew_member) -> value.bounty
            );
          Ser.field "active" Ser.bool
            (
              fun (value: crew_member) -> value.active
            );
          Ser.field "cabins" Ser.int
            (
              fun (value: crew_member) -> value.cabins
            );
          Ser.field "small" Ser.int32
            (
              fun (value: crew_member) -> value.small
            );
          Ser.field "ratio" Ser.float
            (
              fun (value: crew_member) -> value.ratio
            );
          Ser.field "nickname" (Ser.option Ser.string)
            (
              fun (value: crew_member) -> value.nickname
            );
          Ser.field "skills" (Ser.list Ser.string)
            (
              fun (value: crew_member) -> value.skills
            );
          Ser.field "checkpoints" (Ser.array Ser.int)
            (
              fun (value: crew_member) -> value.checkpoints
            );
          Ser.field "role" role_encode
            (
              fun (value: crew_member) -> value.role
            );
          Ser.field "pet" pet_encode
            (
              fun (value: crew_member) -> value.pet
            );
          Ser.field "flag" Ser.null
            (
              fun (value: crew_member) -> value.flag
            );
          Ser.field "pose" pose_encode
            (
              fun (value: crew_member) -> value.pose
            );
        ]
    )
...
let manifest_decode = De.record_mut ~fields:manifest_fields ~create:(
  ...
  fun reader builder field ->
    ...
    | Some Manifest_reserves -> builder.reserves <- Some (De.read reader (De.array crew_member_decode))
    | Some Manifest_scout -> builder.scout <- Some (De.read reader pet_decode)
    | None -> ignore (De.read reader De.skip_any)
) ~finish:(
  fun builder ->
    match builder.ship, builder.emergency, builder.featured, builder.crew, builder.reserves, builder.scout with
    ...
    (
      Ser.fields
        [
          ...
          Ser.field "featured" crew_member_encode
            (
              fun (value: manifest) -> value.featured
            );
          Ser.field "crew" (Ser.list crew_member_encode)
            (
              fun (value: manifest) -> value.crew
            );
          Ser.field "reserves" (Ser.array crew_member_encode)
            (
              fun (value: manifest) -> value.reserves
            );
          Ser.field "scout" pet_encode
            (
              fun (value: manifest) -> value.scout
            );
        ]
    )

let roster_decode = De.record_mut ~fields:roster_fields ~create:(
  fun (): roster_builder -> { crew = None }
) ~step:(
  fun reader builder field ->
    match field with
    ...
    (
      Ser.fields
        [
          Ser.field "crew" (Ser.list crew_member_encode)
            (
              fun (value: roster) -> value.crew
            );
        ]
    )

let voyage_value: voyage = { ship = "Going Merry"; destination = ({ island = "Alabasta"; bearing = 90.0 } : pose); stops = Vector.from_list [ ({ island = "Whisky Peak"; supplies = 3 } : stop); ({ island = "Little Garden"; supplies = 5 } : stop) ] }
...
let test_encodes_tables_and_arrays_of_tables = fun _ctx ->
  ...
  | Ok encoded -> expect_equal ~expected:expected_voyage_toml ~actual:encoded ~message:"expected serde-toml to render tables after scalar keys and lists of records as arrays-of-tables"
...
let test_roundtrips_voyages = fun _ctx ->
  ...
  | Ok actual ->
      if String.equal actual.ship voyage_value.ship && equal_pose actual.destination voyage_value.destination && vec_to_list actual.stops = vec_to_list voyage_value.stops then
        Ok ()
      else Error "expected serde-toml voyage roundtrip to preserve array-of-tables items"
  ...
  | Ok (Serde_toml.Toml_value.Table items) -> (
    ...
    | Some (Serde_toml.Toml_value.Array [ Serde_toml.Toml_value.Table first ]) ->
        if Option.is_some (Std.Collections.Proplist.get first ~key:"pose") then
          ...
          let root_keys = items |> List.map ~fn:(
            fun (key, _) -> key
          ) |> String.concat ", " in
          let first_keys = first |> List.map ~fn:(
            fun (key, _) -> key
          ) |> String.concat ", " in Error ("expected parser to attach [crew.pose] to the current crew array item" ^ "; root keys: " ^ root_keys ^ "; first crew keys: " ^ first_keys)
...
let test_roundtrips_rosters = fun _ctx ->
  ...
  in
  let* () =
    if String.contains encoded "[crew.pet]" && String.contains encoded "[crew.flag]" && String.contains encoded "[crew.pose]" then
      Ok ()
    else Error "expected roster encoding to emit nested table headers for crew items"
  in
  let* () =
    ...
    | Ok (Serde_toml.Toml_value.Table items) -> (
      ...
      | Some (Serde_toml.Toml_value.Array ((Serde_toml.Toml_value.Table first) :: _)) ->
          ...
          if has "name" && has "pet" && has "flag" && has "pose" then
            ...
            let first_keys = first |> List.map ~fn:(
              fun (key, _) -> key
            ) |> String.concat ", " in Error ("expected parser to preserve nested tables inside crew array items, missing: " ^ missing ^ "; root keys: " ^ root_keys ^ "; first crew keys: " ^ first_keys)
      | _ -> Error "expected parser to rebuild crew as an array-of-tables"
    )
    | Ok _ -> Error "expected parser to return a top-level table"
...
let test_decodes_nested_document = fun _ctx ->
  ...
  | Ok actual ->
      if equal_crew_member actual
        ...
        Ok ()
      else Error "expected serde-toml to decode nested tables and inline enum payloads"
...
let test_roundtrips_over_io = fun _ctx ->
  ...
  in
  match Serde_toml.from_reader manifest_decode (String.to_reader ~chunk_size:5 (IO.Buffer.contents buffer)) with
  | Ok actual when equal_manifest actual manifest_value -> Ok ()
...
let test_rejects_top_level_scalars = fun _ctx ->
  ...
  | Error err -> Error ("expected top-level scalar encode to fail clearly, got " ^ Serde.Error.to_string err)
