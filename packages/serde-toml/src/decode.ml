...
type state = { mutable current: Document.value }
...
let rec option_backend: 'value. state -> 'value De.t -> 'value option = fun state decode -> Some (decode.run backend state)
...
let* document = Parse.parse_document input in De.run decode backend { current = Document.Table document }
