...
type state = { mutable value: Toml_value.t option; allow_omit: bool }
...
and encode_array: 'value. state -> 'value Serde.Ser.t -> 'value array -> unit = fun state encode values ->
  let items = ref [] in
  ...
  set state (Toml_value.Array (List.rev !items))
and encode_record: 'value. state -> 'value Serde.Ser.fields -> 'value -> unit = fun state fields value ->
  let items = ref [] in
...
and encode_variant: 'value. state -> 'value Serde.Ser.variant_cases -> 'value -> unit = fun state cases value ->
  let rec loop index =
      ...
      | Ser.Newtype (tag, encode, unwrap) -> (
        ...
        | Some payload ->
            ...
            set state
              (
                Toml_value.Table [
                  tag, expect_value child "variant payload";
                ]
              )
        | None -> loop (index + 1)
      )
...
and backend: state Ser.backend = {
  bool = (
    fun state value -> set state (Toml_value.Bool value)
  );
  string = (
    fun state value -> set state (Toml_value.String value)
  );
  int = (
    fun state value -> set state (Toml_value.Int (Int64.of_int value))
  );
  int32 = (
    fun state value -> set state (Toml_value.Int (Int64.of_int32 value))
  );
  int64 = (
    fun state value -> set state (Toml_value.Int value)
  );
  float = (
    fun state value -> set state (Toml_value.Float value)
  );
  null = (
    fun state -> set state (Toml_value.Table [])
  );
  option = (
    fun state encode value ->
      ...
      | None ->
          if state.allow_omit then
            state.value <- None
          else encode_error "TOML has no null representation for option values in this position"
...
let to_string = fun encode value ->
  ...
  let* () = Ser.run encode backend state value
  in
  match state.value with
...
let to_writer = fun encode writer value ->
  let* encoded = to_string encode value
  in
