  ...
  match
    Fs.with_tempdir ~prefix:"store_nested_test"
      (fun tmpdir ->
        ...
        let _ = Tusk_store.Store.promote store hash ~target_dir:target |> Result.expect ~msg:"promote should succeed" in
  ...
  match
    Fs.with_tempdir ~prefix:"store_plan_bundle_test"
      (fun tmpdir ->
        ...
        let _ = Tusk_store.Store.save_plan_bundle store ~hash ~plan |> Result.expect ~msg:"save_plan_bundle should succeed" in
  ...
  match
    Fs.with_tempdir ~prefix:"store_exports_round_trip_test"
      (fun tmpdir ->
        ...
        match Tusk_store.Store.load_package_exports store ~package:"pkg" ~profile:"debug" ~target:"x86_64-test" with
  ...
  match
    Fs.with_tempdir ~prefix:"store_load_exports_malformed_test"
      (fun tmpdir ->
        ...
        let store = Tusk_store.Store.create ~workspace in
        let exports_path =
          Path.(Tusk_model.Tusk_dirs.cache_dir ~workspace_root:tmpdir
          / Path.v "exports"
          / Path.v "debug"
          / Path.v "x86_64-test"
          / Path.v "pkg.json") in
  ...
  match
    Fs.with_tempdir ~prefix:"store_materialize_exports_test"
      (fun tmpdir ->
        ...
        let _ = Tusk_store.Store.materialize_package_exports store ~exports ~target_dir
