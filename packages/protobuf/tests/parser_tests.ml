        ...
        List.filter_map
          (fun path ->
            ...
            if String.ends_with ~suffix name then
              ...
              in
              let test_file = base_path ^ "/" ^ name in
...
let generate_expected_files () =
  ...
  List.iter
    (fun proto_name ->
      ...
      | Error err ->
          println ("Error reading " ^ proto_str ^ ": " ^ IO.error_message err)
      | Ok content -> (
          match Protobuf.ProtofileFormat.parse content with
          | Error err ->
              println ("Skipping " ^ proto_str ^ " (parse error: " ^ err ^ ")")
          | Ok ast -> (
              ...
              | Ok () ->
                  ...
                  if !count mod 10 = 0 then
                    println ("Generated " ^ string_of_int !count ^ " files...")
              | Error _ -> println ("Error writing " ^ expected_file))))
    proto_files;

  println ("Done! Generated " ^ string_of_int !count ^ " expected files")
