...
let update_named_expectation = fun expected key content ->
  let rec loop acc remaining =
    ...
    | (existing_key, _) :: rest when String.equal existing_key key -> List.append (List.reverse acc) ((key, content) :: rest)
...
let save_tree_entries = fun tmpdir store contents ->
  let rec loop acc entries =
    ...
    | (index, content) :: rest ->
        ...
        let _ = Fs.write content payload |> Result.expect ~msg:"write tree payload should succeed" in
  ...
  Fs.with_tempdir ~prefix:"contentstore-prop-commit-order"
    (
      fun tmpdir ->
        ...
        in
        let left = Contentstore.create ~root:Path.(tmpdir / Path.v "left-cache") ~ns:(namespace [ "workload" ]) ~policy:Contentstore.Policy.default in
        let right = Contentstore.create ~root:Path.(tmpdir / Path.v "right-cache") ~ns:(namespace [ "workload" ]) ~policy:Contentstore.Policy.default in
    ...
    (
      fun (object_contents, named_contents, tree_contents) ->
        Fs.with_tempdir ~prefix:"contentstore-prop-no-temp"
          (
            fun tmpdir ->
              let store = Contentstore.create ~root:Path.(tmpdir / Path.v "cache") ~ns:(namespace [ "workload" ]) ~policy:Contentstore.Policy.default in
              match run_workload ~tmpdir ~store ~object_contents ~named_contents ~tree_contents with
              | Error () -> false
              | Ok _ -> scope_is_empty store "immutable" && scope_is_empty store "mutable" && scope_is_empty store "trees"
        ...
        Fs.with_tempdir ~prefix:"contentstore-prop-reopen-workload"
          (
            fun tmpdir ->
              ...
              let store = Contentstore.create ~root ~ns:(namespace [ "workload" ]) ~policy:Contentstore.Policy.default in
              match run_workload ~tmpdir ~store ~object_contents ~named_contents ~tree_contents with
              | Error () -> false
              | Ok (objects, named, trees) ->
                  let reopened = Contentstore.create ~root ~ns:(namespace [ "workload" ]) ~policy:Contentstore.Policy.default in verify_object_entries reopened objects && verify_named_entries reopened named && verify_tree_entries reopened trees
          ) |> Result.unwrap_or ~default:false
    )
...
let tests =
  [
    ...
    Test.property "mixed workload leaves no temp files" ~size:Large ~examples
      (
        fun _ctx -> assert_property "mixed workload leaves no temp files" mixed_workload_leaves_no_temp_files
      );
    Test.property "reopen preserves reachable workload data" ~size:Large ~examples
      (
