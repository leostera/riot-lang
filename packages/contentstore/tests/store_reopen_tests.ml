...
let create_store = fun root parts -> Contentstore.create ~root ~ns:(namespace parts) ~policy:Contentstore.Policy.default

let with_root = fun prefix fn ->
  Fs.with_tempdir ~prefix
    (
      fun tmpdir ->
        let root = Path.(tmpdir / Path.v "cache") in fn ~tmpdir ~root
  ...
  with_root "contentstore-reopen-named"
    (
      fun ~tmpdir:_ ~root ->
        ...
        let _ = Contentstore.save_named_object writer ~key:"current" ~content:"payload" |> Result.expect ~msg:"save_named_object should succeed" in
  ...
  with_root "contentstore-reopen-tree"
    (
      fun ~tmpdir ~root ->
        ...
        let _ = Fs.write "hello" Path.(source_dir / Path.v "nested" / Path.v "payload.txt") |> Result.expect ~msg:"write payload should succeed" in
        ...
        match Fs.read_to_string Path.(Contentstore.hash_dir_of reader hash / Path.v "nested" / Path.v "payload.txt") with
        | Ok "hello" -> Ok ()
...
let tests = [ Test.case "reopen sees previously saved object" test_reopen_sees_previously_saved_object; Test.case "reopen sees previously saved named object" test_reopen_sees_previously_saved_named_object; Test.case "reopen sees previously committed tree" test_reopen_sees_previously_committed_tree ]
