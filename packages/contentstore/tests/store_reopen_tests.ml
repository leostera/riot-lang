...
let test_reopen_sees_previously_saved_named_object = fun _ctx ->
  with_root
    "contentstore-reopen-named"
    (fun ~tmpdir:_ ~root ->
      let writer = create_store root [ "named" ] in
      let _ =
        Contentstore.save_named_object writer ~key:"current" ~content:"payload"
        |> Result.expect ~msg:"save_named_object should succeed"
      in
      let reader = create_store root [ "named" ] in
      match Contentstore.open_named_object reader ~key:"current"
      |> Result.map ~fn:read_opened_file with
      | Ok "payload" -> Ok ()
...
let test_reopen_sees_previously_committed_tree = fun _ctx ->
  with_root
    ...
    (fun ~tmpdir ~root ->
      ...
      let _ =
        Fs.create_dir_all Path.(source_dir / Path.v "nested")
        |> Result.expect ~msg:"create source dir should succeed"
      in
      let _ =
        Fs.write "hello" Path.(source_dir / Path.v "nested" / Path.v "payload.txt")
        |> Result.expect ~msg:"write payload should succeed"
      in
      let _ =
        Contentstore.commit_dir writer ~hash ~source_dir
...
let tests = [
  ...
  Test.case
    "reopen sees previously saved named object"
    test_reopen_sees_previously_saved_named_object;
