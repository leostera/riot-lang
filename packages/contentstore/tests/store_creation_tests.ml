...
let make_store = fun root parts -> Contentstore.create ~root ~ns:(namespace parts) ~policy:Contentstore.Policy.default
    ...
    (
      fun ~tmpdir:_ ~root ->
        ...
        if exists && List.is_empty entries then
          Ok ()
        else Error "expected create on an existing empty root to leave it unchanged"
    )
    ...
    (
      fun ~tmpdir:_ ~root ->
        ...
        if Path.equal (Contentstore.root store) root then
          Ok ()
        else Error "expected root to return the configured store root"
    )
    ...
    (
      fun ~tmpdir:_ ~root ->
        ...
        if Path.equal (Contentstore.hash_dir_of store hash) (Contentstore.hash_dir_of store hash) then
          Ok ()
        else Error "expected hash_dir_of to be stable for the same hash"
    )
  ...
  with_root "contentstore-hash-dir-empty-hash"
    (
      fun ~tmpdir:_ ~root ->
        let store = make_store root [ "modules" ] in
        try
          let _ = Contentstore.hash_dir_of store (Crypto.Sha256.hash_string "") in Error "expected hash_dir_of to reject the SHA-256 empty digest"
        with
        | _ -> Ok ()
    )
    ...
    (
      fun ~tmpdir ~root ->
        ...
        if Contentstore.exists store hash then
          Ok ()
        else Error "expected exists to report true after commit_dir"
    )
  ...
  with_root "contentstore-create-reserved-ns"
    (
      fun ~tmpdir:_ ~root ->
        ...
        let _ = Contentstore.save_object store ~hash ~content:"payload" |> Result.expect ~msg:"save_object should succeed" in
        let hex = Crypto.Digest.hex hash in
        let object_path = Path.(root / Path.v "objects" / Path.v "objects" / Path.v "__named" / Path.v "ab" / Path.v (String.sub hex ~offset:0 ~len:2) / Path.v hex) in
        ...
        if object_exists && not tree_exists then
          Ok ()
        else Error "expected reserved-like namespace parts to stay isolated under objects/"
    )
