...
let test_create_on_existing_empty_root = fun _ctx ->
  with_root
    "contentstore-create-empty-root"
    (fun ~tmpdir:_ ~root ->
      let _ =
        Fs.create_dir_all root
        |> Result.expect ~msg:"create root should succeed"
      in
...
let test_create_on_missing_root_is_lazy = fun _ctx ->
  with_root
    "contentstore-create-missing-root"
    (fun ~tmpdir:_ ~root ->
      let _store = make_store root [ "modules" ] in
      let exists =
        Fs.exists root
...
let test_create_on_existing_populated_root = fun _ctx ->
  with_root
    "contentstore-create-populated-root"
    (fun ~tmpdir:_ ~root ->
      ...
      in
      let _ =
        Fs.write "keep me" marker
        |> Result.expect ~msg:"write marker should succeed"
      in
      let _store = make_store root [ "modules" ] in
...
let test_hash_dir_differs_for_distinct_hashes = fun _ctx ->
  with_root
    "contentstore-hash-dir-distinct"
    (fun ~tmpdir:_ ~root ->
      let store = make_store root [ "modules" ] in
      let left = Crypto.Sha256.hash_string "left" in
      let right = Crypto.Sha256.hash_string "right" in
      if
        Path.equal (Contentstore.hash_dir_of store left) (Contentstore.hash_dir_of store right)
      then
...
let test_hash_dir_rejects_empty_hash = fun _ctx ->
  with_root
    "contentstore-hash-dir-empty-hash"
    (fun ~tmpdir:_ ~root ->
      let store = make_store root [ "modules" ] in
      try
...
let test_exists_is_true_after_commit_dir = fun _ctx ->
  with_root
    "contentstore-exists-commit-dir"
    (fun ~tmpdir ~root ->
      let store = make_store root [ "modules" ] in
...
let test_first_write_creates_missing_root = fun _ctx ->
  with_root
    "contentstore-create-first-write"
    (fun ~tmpdir:_ ~root ->
      let store = make_store root [ "modules" ] in
...
let test_reserved_like_namespace_parts_are_isolated = fun _ctx ->
  with_root
    "contentstore-create-reserved-ns"
    (fun ~tmpdir:_ ~root ->
...
let tests = [
  ...
  Test.case
    "create on existing populated root preserves contents"
    test_create_on_existing_populated_root;
  ...
  Test.case
    "reserved-like namespace parts are isolated safely"
    test_reserved_like_namespace_parts_are_isolated;
]
