...
let open_object_to_string = fun store ~hash -> Contentstore.open_object store ~hash |> Result.map ~fn:read_opened_file

let object_namespace_dir = fun store -> Path.(Contentstore.root store / Path.v "objects" / Path.v "typ" / Path.v "modules")
  ...
  with_store "contentstore-object-large" [ "typ"; "modules" ]
    (
      fun ~tmpdir:_ ~store ->
        ...
        let content = String.init ~len:1_000_000 ~fn:(
          fun index -> Char.from_int_unchecked (Char.to_int 'a' + (index mod 26))
        ) in
        let _ = Contentstore.save_object store ~hash ~content |> Result.expect ~msg:"save_object should succeed" in
  ...
  with_store "contentstore-object-creates-namespace" [ "typ"; "modules" ]
    (
      fun ~tmpdir:_ ~store ->
        ...
        let _ = Contentstore.save_object store ~hash:(Crypto.hash_string "creates-namespace") ~content:"payload" |> Result.expect ~msg:"save_object should succeed" in
        ...
        if (not exists_before) && exists_after then
          Ok ()
        else Error "expected save_object to create the object namespace directory lazily"
    )
  ...
  with_store "contentstore-object-distinct-hashes" [ "typ"; "modules" ]
    (
      fun ~tmpdir:_ ~store ->
        ...
        let _ = Contentstore.save_object store ~hash:right_hash ~content:"right" |> Result.expect ~msg:"save right object should succeed" in
...
let test_unicode_namespace_roundtrip = fun _ctx ->
  Fs.with_tempdir ~prefix:"contentstore-object-unicode-namespace"
    (
      fun tmpdir ->
        let store = Contentstore.create ~root:Path.(tmpdir / Path.v "cache") ~ns:(namespace [ "módulos"; "東京" ]) ~policy:Contentstore.Policy.default in
