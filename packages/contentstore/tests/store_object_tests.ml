...
let test_save_large_object_roundtrip = fun _ctx ->
  with_store
    ...
    (fun ~tmpdir:_ ~store ->
      ...
      | Ok loaded when String.length loaded = String.length content && String.equal loaded content ->
          Ok ()
...
let test_save_object_first_writer_wins = fun _ctx ->
  with_store
    "contentstore-object-first-wins"
    [ "typ" ]
    (fun ~tmpdir:_ ~store ->
      let hash = Crypto.hash_string "same-object-hash" in
      let _ =
        Contentstore.save_object store ~hash ~content:"first"
        |> Result.expect ~msg:"first save_object should succeed"
      in
      let _ =
        Contentstore.save_object store ~hash ~content:"second"
        |> Result.expect ~msg:"second save_object should succeed"
      in
      match open_object_to_string store ~hash with
      | Ok "first" -> Ok ()
...
let test_save_object_same_content_is_idempotent = fun _ctx ->
  with_store
    "contentstore-object-idempotent"
    [ "typ"; "modules" ]
    (fun ~tmpdir:_ ~store ->
...
let test_save_object_creates_namespace_dir = fun _ctx ->
  with_store
    "contentstore-object-creates-namespace"
    [ "typ"; "modules" ]
    (fun ~tmpdir:_ ~store ->
...
let test_different_hashes_in_same_namespace_are_isolated = fun _ctx ->
  with_store
    "contentstore-object-distinct-hashes"
    [ "typ"; "modules" ]
    (fun ~tmpdir:_ ~store ->
      let left_hash = Crypto.hash_string "left-object" in
      let right_hash = Crypto.hash_string "right-object" in
      let _ =
        Contentstore.save_object store ~hash:left_hash ~content:"left"
        |> Result.expect ~msg:"save left object should succeed"
      in
      let _ =
        Contentstore.save_object store ~hash:right_hash ~content:"right"
        |> Result.expect ~msg:"save right object should succeed"
      in
      match (
...
let test_unicode_namespace_roundtrip = fun _ctx ->
  Fs.with_tempdir
    ~prefix:"contentstore-object-unicode-namespace"
    (fun tmpdir ->
      let store =
        Contentstore.create
          ~root:Path.(tmpdir / Path.v "cache")
...
let test_open_object_missing_is_structured = fun _ctx ->
  with_store
    "contentstore-open-object-missing"
    [ "typ" ]
    (fun ~tmpdir:_ ~store ->
...
let test_save_file_rejects_directory_source = fun _ctx ->
  with_store
    "contentstore-save-file-invalid-source"
    [ "typ" ]
    (fun ~tmpdir ~store ->
      ...
      | Error (Contentstore.Store.Invalid_source_path { reason = Contentstore.Store.Source_not_file; _ }) ->
          Ok ()
      | Error err -> Error ("unexpected error: " ^ Contentstore.Store.error_message err)
...
let tests = [
  ...
  Test.case
    "save_object with the same content is idempotent"
    test_save_object_same_content_is_idempotent;
  Test.case "save_file/open_object roundtrip" test_save_file_roundtrip;
  Test.case
    "save_object creates the namespace directory lazily"
    test_save_object_creates_namespace_dir;
  Test.case
    "different hashes in one namespace are isolated"
    test_different_hashes_in_same_namespace_are_isolated;
  Test.case
    "many objects in one namespace remain readable"
    test_many_objects_in_one_namespace_remain_readable;
