...
let object_path = fun store hash ->
  let hex = Crypto.Digest.hex hash in Path.(Contentstore.root store / Path.v "objects" / Path.v "objects" / Path.v (String.sub hex ~offset:0 ~len:2) / Path.v hex)

let named_object_path = fun store key ->
  let key_hash = Crypto.hash_string key |> Crypto.Digest.hex in Path.(Contentstore.root store / Path.v "named" / Path.v "objects" / Path.v (String.sub key_hash ~offset:0 ~len:2) / Path.v key_hash)
    ...
    (
      fun content ->
        with_store "contentstore-prop-layout-stable" [ "objects" ]
          (
            fun ~tmpdir:_ ~store ->
              let hash = Crypto.hash_string content in Path.equal (Contentstore.hash_dir_of store hash) (Contentstore.hash_dir_of store hash)
          )
    )
        ...
        with_store "contentstore-prop-layout-distinct" [ "objects" ]
          (
            fun ~tmpdir:_ ~store ->
              ...
              let right_hash = Crypto.hash_string right in not (Path.equal (Contentstore.hash_dir_of store left_hash) (Contentstore.hash_dir_of store right_hash))
          )
    )
        ...
        with_store "contentstore-prop-layout-domains" [ "objects" ]
          (
            fun ~tmpdir:_ ~store ->
              ...
              let temp_root = Path.(Contentstore.root store / Path.v "tmp") in not (Path.equal tree_path object_path) && not (Path.equal tree_path named_path) && not (Path.equal object_path named_path) && not (Path.equal tree_path temp_root) && not (Path.equal object_path temp_root) && not (Path.equal named_path temp_root)
          )
    )

let tests =
  [
    Test.property "hash_dir_of is deterministic" ~examples
      (
        fun _ctx -> assert_property "hash_dir_of is deterministic" hash_dir_is_deterministic
      );
    Test.property "distinct hashes have distinct tree paths" ~examples
      (
        fun _ctx -> assert_property "distinct hashes have distinct tree paths" distinct_hashes_have_distinct_tree_paths
      );
    Test.property "layout domains are disjoint" ~examples
      (
        fun _ctx -> assert_property "layout domains are disjoint" layout_domains_are_disjoint
      );
  ]

let main ~args = Test.Cli.main ~name:"contentstore_store_layout_property_tests" ~tests ~args ()
