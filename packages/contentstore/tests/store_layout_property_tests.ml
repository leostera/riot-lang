...
let hash_dir_is_deterministic =
  Property.for_all
    ...
    (fun content ->
      with_store
        "contentstore-prop-layout-stable"
        [ "objects" ]
        (fun ~tmpdir:_ ~store ->
...
let distinct_hashes_have_distinct_tree_paths =
  Property.for_all
    distinct_string_pair_arb
    (fun (left, right) ->
      with_store
        "contentstore-prop-layout-distinct"
        [ "objects" ]
        (fun ~tmpdir:_ ~store ->
...
let layout_domains_are_disjoint =
  Property.for_all
    ...
    (fun (content, key) ->
      with_store
        "contentstore-prop-layout-domains"
        [ "objects" ]
        (fun ~tmpdir:_ ~store ->
          let hash = Crypto.hash_string content in
...
let tests = [
  ...
  Test.property
    ...
    (fun _ctx ->
      assert_property
        "distinct hashes have distinct tree paths"
        distinct_hashes_have_distinct_tree_paths);
  Test.property
