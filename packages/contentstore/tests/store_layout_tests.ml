...
let named_object_path = fun store key ->
  let hash = Crypto.hash_string key |> Crypto.Digest.hex in
  Path.(Contentstore.root store
  / Path.v "named"
  / Path.v "plans"
  / Path.v "releases"
...
let test_named_layout_is_sharded = fun _ctx ->
  with_store "contentstore-layout-named" [ "plans"; "releases" ]
    (fun ~tmpdir:_ ~store ->
      let _ = Contentstore.save_named_object store ~key:"latest" ~content:"hello"
...
let test_writes_leave_no_temp_files = fun _ctx ->
  with_store "contentstore-layout-temp-cleanup" [ "typ" ]
    (fun ~tmpdir:_ ~store ->
      let _ = Contentstore.save_object store ~hash:(Crypto.hash_string "object-temp") ~content:"one"
      |> Result.expect ~msg:"save_object should succeed" in
      let _ = Contentstore.save_named_object store ~key:"latest" ~content:"two"
      |> Result.expect ~msg:"save_named_object should succeed" in
      ...
      if
        List.is_empty immutable_entries && List.is_empty mutable_entries && List.is_empty tree_entries
      then
        Ok ()
      else
