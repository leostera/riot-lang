...
let test_save_named_object_overwrites = fun _ctx ->
  with_store "contentstore-save-named-object" [ "plans" ]
    (fun ~tmpdir:_ ~store ->
      let _ = Contentstore.save_named_object store ~key:"Colors" ~content:"first"
      |> Result.expect ~msg:"first save_named_object should succeed" in
      let _ = Contentstore.save_named_object store ~key:"Colors" ~content:"second"
...
let test_different_named_keys_are_isolated = fun _ctx ->
  with_store "contentstore-save-named-distinct-keys" [ "plans" ]
    (fun ~tmpdir:_ ~store ->
      let _ = Contentstore.save_named_object store ~key:"left" ~content:"left"
...
let test_save_named_object_creates_namespace_dir = fun _ctx ->
  with_store "contentstore-save-named-creates-namespace" [ "plans" ]
    (fun ~tmpdir:_ ~store ->
      ...
      let _ = Contentstore.save_named_object store ~key:"latest" ~content:"payload"
...
let test_save_named_same_content_is_idempotent = fun _ctx ->
  with_store "contentstore-save-named-idempotent" [ "plans" ]
    (fun ~tmpdir:_ ~store ->
      let _ = Contentstore.save_named_object store ~key:"current" ~content:"payload"
      |> Result.expect ~msg:"first save_named_object should succeed" in
      let _ = Contentstore.save_named_object store ~key:"current" ~content:"payload"
      |> Result.expect ~msg:"second save_named_object should succeed" in
