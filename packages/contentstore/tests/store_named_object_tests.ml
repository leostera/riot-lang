...
let open_named_object_to_string = fun store ~key -> Contentstore.open_named_object store ~key |> Result.map ~fn:read_opened_file

let named_namespace_dir = fun store -> Path.(Contentstore.root store / Path.v "named" / Path.v "plans")

let test_save_named_object_overwrites = fun _ctx ->
  with_store "contentstore-save-named-object" [ "plans" ]
    (
      fun ~tmpdir:_ ~store ->
        let _ = Contentstore.save_named_object store ~key:"Colors" ~content:"first" |> Result.expect ~msg:"first save_named_object should succeed" in
        let _ = Contentstore.save_named_object store ~key:"Colors" ~content:"second" |> Result.expect ~msg:"second save_named_object should succeed" in
...
let test_save_named_file_overwrites = fun _ctx ->
  with_store "contentstore-save-named-file" [ "plans" ]
    (
      fun ~tmpdir ~store ->
...
let test_different_named_keys_are_isolated = fun _ctx ->
  with_store "contentstore-save-named-distinct-keys" [ "plans" ]
    (
      fun ~tmpdir:_ ~store ->
        let _ = Contentstore.save_named_object store ~key:"left" ~content:"left" |> Result.expect ~msg:"save left named object should succeed" in
        let _ = Contentstore.save_named_object store ~key:"right" ~content:"right" |> Result.expect ~msg:"save right named object should succeed" in
...
let test_save_named_object_creates_namespace_dir = fun _ctx ->
  with_store "contentstore-save-named-creates-namespace" [ "plans" ]
    (
      fun ~tmpdir:_ ~store ->
        ...
        let _ = Contentstore.save_named_object store ~key:"latest" ~content:"payload" |> Result.expect ~msg:"save_named_object should succeed" in
        ...
        if (not exists_before) && exists_after then
          Ok ()
        else Error "expected save_named_object to create the named namespace directory lazily"
    )
...
let test_named_long_key_roundtrip = fun _ctx ->
  with_store "contentstore-save-named-long-key" [ "plans" ]
    (
      fun ~tmpdir:_ ~store ->
        let key = String.init ~len:2_048 ~fn:(
          fun _ -> 'k'
        ) in
        let _ = Contentstore.save_named_object store ~key ~content:"payload" |> Result.expect ~msg:"save_named_object should succeed" in
        match open_named_object_to_string store ~key with
...
let test_save_named_same_content_is_idempotent = fun _ctx ->
  with_store "contentstore-save-named-idempotent" [ "plans" ]
    (
      fun ~tmpdir:_ ~store ->
        let _ = Contentstore.save_named_object store ~key:"current" ~content:"payload" |> Result.expect ~msg:"first save_named_object should succeed" in
        let _ = Contentstore.save_named_object store ~key:"current" ~content:"payload" |> Result.expect ~msg:"second save_named_object should succeed" in
...
let test_named_overwrite_with_larger_content = fun _ctx ->
  with_store "contentstore-save-named-larger-content" [ "plans" ]
    (
      fun ~tmpdir:_ ~store ->
        let _ = Contentstore.save_named_object store ~key:"current" ~content:"small" |> Result.expect ~msg:"first save_named_object should succeed" in
        let larger = String.init ~len:8_192 ~fn:(
          fun _ -> 'x'
        ) in
        let _ = Contentstore.save_named_object store ~key:"current" ~content:larger |> Result.expect ~msg:"second save_named_object should succeed" in
        match open_named_object_to_string store ~key:"current" with
  ...
  with_store "contentstore-save-named-smaller-content" [ "plans" ]
    (
      fun ~tmpdir:_ ~store ->
        ...
        let _ = Contentstore.save_named_object store ~key:"current" ~content:larger |> Result.expect ~msg:"first save_named_object should succeed" in
        let _ = Contentstore.save_named_object store ~key:"current" ~content:"small" |> Result.expect ~msg:"second save_named_object should succeed" in
        match open_named_object_to_string store ~key:"current" with
        | Ok "small" -> Ok ()
  ...
  with_store "contentstore-save-named-failed-overwrite" [ "plans" ]
    (
      fun ~tmpdir:_ ~store ->
        ...
        let parent_dir = Path.(Contentstore.root store / Path.v "named" / Path.v "plans" / Path.v (String.sub key_hash ~offset:0 ~len:2)) in
        ...
        let result =
          ...
          | Error err ->
              let _ = Fs.set_permissions parent_dir Fs.Permissions.executable in Error ("unexpected error: " ^ Contentstore.Store.error_message err)
          | Ok () ->
              let _ = Fs.set_permissions parent_dir Fs.Permissions.executable in Error "expected save_named_object overwrite to fail in an unwritable shard dir"
        in
        result
    )

let test_named_missing_is_structured = fun _ctx ->
  with_store "contentstore-open-named-object-missing" [ "plans" ]
    (
      fun ~tmpdir:_ ~store ->
...
let tests =
  [
    ...
    Test.case "save empty named object/open_named_object roundtrip" test_save_named_empty_object_roundtrip;
    Test.case "save binary named object/open_named_object roundtrip" test_save_named_binary_object_roundtrip;
    ...
    Test.case "save_named_object creates the namespace directory lazily" test_save_named_object_creates_namespace_dir;
    ...
    Test.case "save_named_object with the same content is idempotent" test_save_named_same_content_is_idempotent;
    Test.case "named overwrite with larger content stays readable" test_named_overwrite_with_larger_content;
    Test.case "named overwrite with smaller content stays readable" test_named_overwrite_with_smaller_content;
    Test.case "failed named overwrite preserves the previous value" test_failed_named_overwrite_preserves_previous_value;
