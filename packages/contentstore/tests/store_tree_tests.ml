...
let write_tree = fun root entries ->
  let rec loop entries =
    ...
    | (relative_path, content) :: rest ->
        ...
        let _ = Fs.write content path |> Result.expect ~msg:"write tree file should succeed" in loop rest
  in
  loop entries
  ...
  with_store "contentstore-commit-dir" [ "modules" ]
    (
      fun ~tmpdir ~store ->
        ...
        let committed = Fs.read_to_string Path.(Contentstore.hash_dir_of store hash / target_file) |> Result.expect ~msg:"committed payload should exist" in
        if String.equal committed "first" then
          Ok ()
        else Error "expected first tree writer to win"
    )
  ...
  with_store "contentstore-commit-dir-nested" [ "modules" ]
    (
      fun ~tmpdir ~store ->
        ...
        match read_tree_file committed_root Path.(Path.v "a" / Path.v "payload.txt"), read_tree_file committed_root Path.(Path.v "a" / Path.v "b" / Path.v "binary.bin"), read_tree_file committed_root Path.(Path.v "c" / Path.v "d" / Path.v "leaf.txt") with
    ...
    (
      fun ~tmpdir ~store ->
        ...
        if exists && List.is_empty entries then
          Ok ()
        else Error "expected commit_dir of an empty directory to create an empty destination"
    )
      ...
      fun ~tmpdir ~store ->
        ...
        let rec write_many index =
          if index = 100 then
            ...
            let _ = Fs.create_dir_all (Path.dirname path) |> Result.expect ~msg:"create parent dirs should succeed" in
            let _ = Fs.write ("payload-" ^ Int.to_string index) path |> Result.expect ~msg:"write payload should succeed" in write_many (index + 1)
        in
        ...
        let _ = Contentstore.commit_dir store ~hash ~source_dir |> Result.expect ~msg:"commit_dir should succeed" in verify_many (Contentstore.hash_dir_of store hash) 0
    )
  ...
  with_store "contentstore-commit-dir-empty-subdirs" [ "modules" ]
    (
      fun ~tmpdir ~store ->
        ...
        let _ = Contentstore.commit_dir store ~hash ~source_dir |> Result.expect ~msg:"commit_dir should succeed" in
        let exists = Fs.exists Path.(Contentstore.hash_dir_of store hash / Path.v "a" / Path.v "b") |> Result.expect ~msg:"exists should succeed" in
        if exists then
          Ok ()
        else Error "expected commit_dir to preserve empty subdirectories"
    )
  ...
  with_store "contentstore-commit-dir-distinct-hashes" [ "modules" ]
    (
      fun ~tmpdir ~store ->
        ...
        let _ = Contentstore.commit_dir store ~hash:left_hash ~source_dir:left_dir |> Result.expect ~msg:"left commit_dir should succeed" in
        let _ = Contentstore.commit_dir store ~hash:right_hash ~source_dir:right_dir |> Result.expect ~msg:"right commit_dir should succeed" in
        match Fs.read_to_string Path.(Contentstore.hash_dir_of store left_hash / Path.v "payload.txt"), Fs.read_to_string Path.(Contentstore.hash_dir_of store right_hash / Path.v "payload.txt") with
        | (Ok "left", Ok "right") -> Ok ()
