  ...
  let rec loop entries =
    ...
    | (relative_path, content) :: rest ->
        ...
        let _ =
          ...
          |> Result.expect ~msg:"create parent dirs should succeed"
        in
        let _ =
  ...
  with_store
    ...
    (fun ~tmpdir ~store ->
      ...
      let _ =
        ...
        |> Result.expect ~msg:"create first dir should succeed"
      in
      let _ =
        Fs.create_dir_all second_dir
        |> Result.expect ~msg:"create second dir should succeed"
      in
      let _ =
      ...
      in
      let _ =
        Contentstore.commit_dir store ~hash ~source_dir:first_dir
        |> Result.expect ~msg:"first commit_dir should succeed"
      in
      let _ =
        Contentstore.commit_dir store ~hash ~source_dir:second_dir
        |> Result.expect ~msg:"second commit_dir should succeed"
      in
      let committed =
        Fs.read_to_string Path.(Contentstore.hash_dir_of store hash / target_file)
...
let test_commit_dir_with_one_file = fun _ctx ->
  with_store
    "contentstore-commit-dir-one-file"
    [ "modules" ]
    (fun ~tmpdir ~store ->
...
let test_commit_dir_preserves_nested_structure_and_bytes = fun _ctx ->
  with_store
    "contentstore-commit-dir-nested"
    [ "modules" ]
    (fun ~tmpdir ~store ->
      ...
      in
      let _ =
        write_tree source_dir entries
        |> Result.expect ~msg:"write tree should succeed"
      in
      let _ =
        Contentstore.commit_dir store ~hash ~source_dir
        |> Result.expect ~msg:"commit_dir should succeed"
    ...
    (fun ~tmpdir ~store ->
      ...
      let rec write_many index =
        if index = 100 then
          ...
          in
          let _ =
            Fs.write ("payload-" ^ Int.to_string index) path
            |> Result.expect ~msg:"write payload should succeed"
          in
          write_many (index + 1)
      in
      let rec verify_many root index =
        if index = 100 then
          ...
          | Ok loaded when String.equal loaded ("payload-" ^ Int.to_string index) ->
              verify_many root (index + 1)
          | _ -> Error "expected all files in a large committed tree to remain readable"
      in
      let _ =
        write_many 0
        |> Result.expect ~msg:"write large tree should succeed"
      in
      let _ =
        Contentstore.commit_dir store ~hash ~source_dir
        |> Result.expect ~msg:"commit_dir should succeed"
...
let test_commit_dir_rejects_file_source = fun _ctx ->
  with_store
    "contentstore-commit-dir-invalid-source"
    [ "modules" ]
    (fun ~tmpdir ~store ->
      let source = Path.(tmpdir / Path.v "source.txt") in
      let _ =
        Fs.write "not a directory" source
...
let test_commit_dir_rejects_missing_source = fun _ctx ->
  with_store
    "contentstore-commit-dir-missing-source"
    [ "modules" ]
    (fun ~tmpdir ~store ->
...
let test_commit_dir_preserves_empty_subdirectories = fun _ctx ->
  with_store
    "contentstore-commit-dir-empty-subdirs"
    [ "modules" ]
    (fun ~tmpdir ~store ->
      ...
      let _ =
        ...
        |> Result.expect ~msg:"create nested empty dir should succeed"
      in
      let _ =
        Contentstore.commit_dir store ~hash ~source_dir
        |> Result.expect ~msg:"commit_dir should succeed"
      in
      let exists =
        Fs.exists Path.(Contentstore.hash_dir_of store hash / Path.v "a" / Path.v "b")
        |> Result.expect ~msg:"exists should succeed"
      in
      if exists then
        Ok ()
...
let test_commit_dir_consumes_source_dir_on_success = fun _ctx ->
  with_store
    "contentstore-commit-dir-consumes-source"
    [ "modules" ]
    (fun ~tmpdir ~store ->
...
let test_duplicate_commit_consumes_second_source_dir = fun _ctx ->
  with_store
    "contentstore-commit-dir-disposable-source"
    [ "modules" ]
    (fun ~tmpdir ~store ->
      ...
      in
      let _ =
        Fs.write "first" Path.(first_dir / Path.v "payload.txt")
        |> Result.expect ~msg:"write first payload should succeed"
      in
      let _ =
        Fs.write "second" Path.(second_dir / Path.v "payload.txt")
        |> Result.expect ~msg:"write second payload should succeed"
      in
      let _ =
        Contentstore.commit_dir store ~hash ~source_dir:first_dir
...
let test_different_hashes_create_distinct_tree_destinations = fun _ctx ->
  with_store
    "contentstore-commit-dir-distinct-hashes"
    [ "modules" ]
    (fun ~tmpdir ~store ->
      ...
      let _ =
        ...
        |> Result.expect ~msg:"create left dir should succeed"
      in
      let _ =
        Fs.create_dir_all right_dir
        |> Result.expect ~msg:"create right dir should succeed"
      in
      let _ =
        Fs.write "left" Path.(left_dir / Path.v "payload.txt")
        |> Result.expect ~msg:"write left payload should succeed"
      in
      let _ =
        Fs.write "right" Path.(right_dir / Path.v "payload.txt")
        |> Result.expect ~msg:"write right payload should succeed"
      in
      let _ =
        Contentstore.commit_dir store ~hash:left_hash ~source_dir:left_dir
        |> Result.expect ~msg:"left commit_dir should succeed"
      in
      let _ =
        Contentstore.commit_dir store ~hash:right_hash ~source_dir:right_dir
        |> Result.expect ~msg:"right commit_dir should succeed"
      in
      match (
...
let test_commit_dir_source_inside_store_is_safe = fun _ctx ->
  with_store
    "contentstore-commit-dir-inside-store"
    [ "modules" ]
    (fun ~tmpdir:_ ~store ->
      ...
      | Ok "payload" -> Ok ()
      | Ok _ ->
          Error "expected commit_dir to stay consistent when the source dir lives under the store root"
      | Error _ ->
          Error "expected committed tree to stay readable when source_dir starts inside the store")
...
let tests = [
  ...
  Test.case
    "commit_dir preserves nested structure and bytes"
    test_commit_dir_preserves_nested_structure_and_bytes;
  Test.case
    "commit_dir on an empty directory creates an empty destination"
    test_commit_dir_of_empty_directory_creates_empty_destination;
  ...
  Test.case
    "commit_dir preserves empty subdirectories"
    test_commit_dir_preserves_empty_subdirectories;
  Test.case
    "commit_dir consumes the source directory on success"
    test_commit_dir_consumes_source_dir_on_success;
  Test.case
    "duplicate commit_dir consumes the second source directory"
    test_duplicate_commit_consumes_second_source_dir;
  Test.case
    "different hashes create distinct tree destinations"
    test_different_hashes_create_distinct_tree_destinations;
  Test.case
    "commit_dir is safe when the source dir starts inside the store"
    test_commit_dir_source_inside_store_is_safe;
]
