...
let object_parent_dir = fun store hash ->
  let hex = Crypto.Digest.hex hash in Path.(Contentstore.root store / Path.v "objects" / Path.v "errors" / Path.v (String.sub hex ~offset:0 ~len:2))

let named_parent_dir = fun store key ->
  let key_hash = Crypto.hash_string key |> Crypto.Digest.hex in Path.(Contentstore.root store / Path.v "named" / Path.v "errors" / Path.v (String.sub key_hash ~offset:0 ~len:2))

let test_save_object_into_unwritable_namespace_is_structured = fun _ctx ->
  with_store "contentstore-error-save-object-perms" [ "errors" ]
    (
      fun ~tmpdir:_ ~store ->
        ...
        in
        let _ = Fs.set_permissions blocked_dir Fs.Permissions.executable in result
    )

let test_save_named_object_into_unwritable_namespace_is_structured = fun _ctx ->
  with_store "contentstore-error-save-named-perms" [ "errors" ]
    (
      fun ~tmpdir:_ ~store ->
        let key = "current" in
...
let test_open_object_permission_error_is_not_missing = fun _ctx ->
  with_store "contentstore-error-open-object-perms" [ "errors" ]
    (
      fun ~tmpdir:_ ~store ->
        ...
        let _ = Fs.set_permissions path (Fs.Permissions.of_mode 0o000) |> Result.expect ~msg:"chmod object should succeed" in
        match Contentstore.open_object store ~hash with
        | Error (Contentstore.Store.Io _) -> Ok ()
        | Error (Contentstore.Store.Missing _) -> Error "expected permission-denied object read to stay distinct from missing"
        | Error err -> Error ("unexpected error: " ^ Contentstore.Store.error_message err)
...
let test_open_named_object_permission_error_is_not_missing = fun _ctx ->
  with_store "contentstore-error-open-named-perms" [ "errors" ]
    (
      fun ~tmpdir:_ ~store ->
        let key = "current" in
        ...
        let _ = Fs.set_permissions path (Fs.Permissions.of_mode 0o000) |> Result.expect ~msg:"chmod named object should succeed" in
        match Contentstore.open_named_object store ~key with
        | Error (Contentstore.Store.Io _) -> Ok ()
        | Error (Contentstore.Store.Missing _) -> Error "expected permission-denied named read to stay distinct from missing"
        | Error err -> Error ("unexpected error: " ^ Contentstore.Store.error_message err)
...
let test_commit_dir_failure_reports_context = fun _ctx ->
  with_store "contentstore-error-commit-dir-context" [ "errors" ]
    (
      fun ~tmpdir ~store ->
        ...
        let _ = Fs.create_dir_all trees_root |> Result.expect ~msg:"create trees root should succeed" in
        let _ = Fs.set_permissions trees_root (Fs.Permissions.of_mode 0o555) |> Result.expect ~msg:"chmod trees root should succeed" in
        let result =
          ...
          | Error (Contentstore.Store.Io { path; related_path = Some _; _ }) ->
              if Path.equal path (Contentstore.hash_dir_of store hash) then
                Ok ()
              else Error "expected commit_dir failure to report the destination path"
          | Error err -> Error ("unexpected error: " ^ Contentstore.Store.error_message err)
          | Ok () -> Error "expected commit_dir to fail when the trees root is unwritable"
        in
        let _ = Fs.set_permissions trees_root Fs.Permissions.executable in result
    )

let tests =
  [
    Test.case "save_object into unwritable namespace is structured" test_save_object_into_unwritable_namespace_is_structured;
    Test.case "save_named_object into unwritable namespace is structured" test_save_named_object_into_unwritable_namespace_is_structured;
    Test.case "open_object permission error is not missing" test_open_object_permission_error_is_not_missing;
    Test.case "open_named_object permission error is not missing" test_open_named_object_permission_error_is_not_missing;
