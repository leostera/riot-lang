...
let test_open_object_permission_error_is_not_missing = fun _ctx ->
  with_store "contentstore-error-open-object-perms" [ "errors" ]
    (fun ~tmpdir:_ ~store ->
      ...
      let _ = Fs.set_permissions path (Fs.Permissions.of_mode 0o000)
      |> Result.expect ~msg:"chmod object should succeed" in
...
let test_open_named_object_permission_error_is_not_missing = fun _ctx ->
  with_store "contentstore-error-open-named-perms" [ "errors" ]
    (fun ~tmpdir:_ ~store ->
      ...
      let _ = Fs.set_permissions path (Fs.Permissions.of_mode 0o000)
      |> Result.expect ~msg:"chmod named object should succeed" in
...
let test_commit_dir_failure_reports_context = fun _ctx ->
  with_store "contentstore-error-commit-dir-context" [ "errors" ]
    (fun ~tmpdir ~store ->
      ...
      let _ = Fs.set_permissions trees_root (Fs.Permissions.of_mode 0o555)
      |> Result.expect ~msg:"chmod trees root should succeed" in
      let result =
