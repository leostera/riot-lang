    ...
    Generator.map
      (
        fun (left_key, right_key, content) ->
          ...
          else (left_key, right_key, content)
      )
...
let string_non_empty_list_pair_arb =
  let gen = Generator.pair Generator.string (Generator.non_empty_list Generator.string) in Arbitrary.make ~print:(Printer.pair Printer.string (Printer.list Printer.string)) gen
...
let with_store = fun prefix parts fn ->
  Fs.with_tempdir ~prefix
    (
      fun tmpdir ->
        let store = Contentstore.create ~root:Path.(tmpdir / Path.v "cache") ~ns:(namespace parts) ~policy:Contentstore.Policy.default in fn ~tmpdir ~store
...
let named_objects_keep_last_writer =
  Property.for_all Arbitrary.(pair string string)
    (
      fun (first, second) ->
        with_store "contentstore-prop-named-last-wins" [ "named" ]
          (
            fun ~tmpdir:_ ~store ->
              let _ = Contentstore.save_named_object store ~key:"current" ~content:first |> Result.expect ~msg:"first save_named_object should succeed" in
              let _ = Contentstore.save_named_object store ~key:"current" ~content:second |> Result.expect ~msg:"second save_named_object should succeed" in
              match Contentstore.open_named_object store ~key:"current" |> Result.map ~fn:read_opened_file with
        ...
        with_store "contentstore-prop-named-keys" [ "named" ]
          (
            fun ~tmpdir:_ ~store ->
              ...
              let _ = Contentstore.save_named_object store ~key:left_key ~content:left_content |> Result.expect ~msg:"save left named object should succeed" in
              let _ = Contentstore.save_named_object store ~key:right_key ~content:right_content |> Result.expect ~msg:"save right named object should succeed" in
              match Contentstore.open_named_object store ~key:left_key |> Result.map ~fn:read_opened_file, Contentstore.open_named_object store ~key:right_key |> Result.map ~fn:read_opened_file with
              | (Ok loaded_left, Ok loaded_right) -> String.equal loaded_left left_content && String.equal loaded_right right_content
              | _ -> false
          )
    )
          ...
          (
            fun ~tmpdir:_ ~store ->
              ...
              let rec write_all latest remaining =
                ...
                | [] -> latest
                | content :: rest ->
                    let _ = Contentstore.save_named_object store ~key ~content |> Result.expect ~msg:"save_named_object should succeed" in write_all content rest
              in
              let latest = write_all "" contents in
              let reopened = Contentstore.create ~root ~ns:(namespace [ "named" ]) ~policy:Contentstore.Policy.default in
