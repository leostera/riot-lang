  ...
  Property.for_all Arbitrary.(pair string string)
    (fun (first, second) ->
      with_store "contentstore-prop-named-last-wins" [ "named" ]
        (fun ~tmpdir:_ ~store ->
          let _ = Contentstore.save_named_object store ~key:"current" ~content:first
          |> Result.expect ~msg:"first save_named_object should succeed" in
          let _ = Contentstore.save_named_object store ~key:"current" ~content:second
    ...
    (fun (left_key, right_key, content) ->
      with_store "contentstore-prop-named-keys" [ "named" ]
        (fun ~tmpdir:_ ~store ->
          ...
          let _ = Contentstore.save_named_object store ~key:left_key ~content:left_content
