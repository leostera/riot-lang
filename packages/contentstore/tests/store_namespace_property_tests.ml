    ...
    (fun (left_id, right_id, content) ->
      ...
      with_stores "contentstore-prop-namespace-named" left_id right_id
        (fun ~left ~right ->
          ...
          let left_content = "left:" ^ content in
          let right_content = "right:" ^ content in
          let _ = Contentstore.save_named_object left ~key ~content:left_content
...
let () =
  Actors.run
    ~main:(fun ~args -> Test.Cli.main ~name:"contentstore_store_namespace_property_tests" ~tests ~args)
    ~args:Env.args
    ()
