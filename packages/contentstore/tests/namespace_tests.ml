...
let test_namespace_roundtrips_parts = fun _ctx ->
  let ns =
    Contentstore.Namespace.from_parts [ "typ"; "modules" ]
    |> Result.expect ~msg:"namespace should be valid"
  in
...
let test_namespace_accepts_unicode_parts = fun _ctx ->
  let ns =
    Contentstore.Namespace.from_parts [ "módulos"; "東京" ]
    |> Result.expect ~msg:"unicode namespace should be valid"
  in
  match Contentstore.Namespace.parts ns with
  | [ "módulos"; "東京" ] -> Ok ()
