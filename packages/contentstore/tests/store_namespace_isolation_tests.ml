...
let with_stores = fun prefix fn ->
  Fs.with_tempdir
    ...
    (fun tmpdir ->
      ...
      let left =
        Contentstore.create ~root ~ns:(namespace [ "left" ]) ~policy:Contentstore.Policy.default
      in
      let right =
        Contentstore.create ~root ~ns:(namespace [ "right" ]) ~policy:Contentstore.Policy.default
      in
      fn ~left ~right)
...
let test_hash_addressed_objects_are_namespaced = fun _ctx ->
  with_stores
    "contentstore-namespace-objects"
    (fun ~left ~right ->
      let hash = Crypto.hash_string "shared-hash" in
      let _ =
        Contentstore.save_object left ~hash ~content:"left"
        |> Result.expect ~msg:"left save should succeed"
      in
      let _ =
        Contentstore.save_object right ~hash ~content:"right"
        |> Result.expect ~msg:"right save should succeed"
      in
...
let test_named_objects_are_namespaced = fun _ctx ->
  with_stores
    "contentstore-namespace-named"
    (fun ~left ~right ->
      let _ =
        Contentstore.save_named_object left ~key:"current" ~content:"left"
        |> Result.expect ~msg:"left save should succeed"
      in
      let _ =
        Contentstore.save_named_object right ~key:"current" ~content:"right"
        |> Result.expect ~msg:"right save should succeed"
      in
      match (
