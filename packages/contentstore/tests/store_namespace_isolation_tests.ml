  ...
  Fs.with_tempdir ~prefix
    (
      fun tmpdir ->
        ...
        let right = Contentstore.create ~root ~ns:(namespace [ "right" ]) ~policy:Contentstore.Policy.default in fn ~left ~right
...
let tests = [ Test.case "hash-addressed objects are namespaced" test_hash_addressed_objects_are_namespaced; Test.case "named objects are namespaced" test_named_objects_are_namespaced ]
