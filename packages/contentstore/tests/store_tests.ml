...
let test_commit_dir_first_writer_wins = fun _ctx ->
  Fs.with_tempdir ~prefix:"contentstore-commit-dir"
    (fun tmpdir ->
      ...
      let _ = Contentstore.Store.commit_dir store ~hash ~source_dir:first_dir |> Result.expect ~msg:"first commit_dir should succeed" in
      let _ = Contentstore.Store.commit_dir store ~hash ~source_dir:second_dir
      |> Result.expect ~msg:"second commit_dir should also succeed" in
      let committed = Fs.read_to_string
        Path.(Contentstore.Store.hash_dir_of store hash / target_file)
      |> Result.expect ~msg:"committed payload should exist" in
      if String.equal committed "first" then
        Ok ()
      else
        Error "expected first writer to win for the same content address") |> Result.unwrap_or
...
let test_blob_roundtrip = fun _ctx ->
  Fs.with_tempdir ~prefix:"contentstore-blob"
    (fun tmpdir ->
      ...
      let _ = Contentstore.Store.save_blob store ~namespace:"typ" ~hash ~content:"hello"
      |> Result.expect ~msg:"save_blob should succeed" in
      match Contentstore.Store.load_blob store ~namespace:"typ" ~hash with
      | Some "hello" -> Ok ()
      | Some other -> Error ("unexpected blob content: " ^ other)
      | None -> Error "expected saved blob") |> Result.unwrap_or
    ~default:(Error "tempdir creation failed")
...
let test_json_bundle_roundtrip = fun _ctx ->
  Fs.with_tempdir ~prefix:"contentstore-json"
    (fun tmpdir ->
      ...
      let _ = Contentstore.Store.save_json_bundle store ~namespace:"module-typings" ~hash ~json
      |> Result.expect ~msg:"save_json_bundle should succeed" in
      ...
      | None -> Error "expected saved JSON bundle") |> Result.unwrap_or
    ~default:(Error "tempdir creation failed")
...
let test_named_json_bundle_roundtrip = fun _ctx ->
  Fs.with_tempdir ~prefix:"contentstore-named-json"
    (fun tmpdir ->
      ...
      let _ = Contentstore.Store.save_named_json_bundle
        store
        ~namespace:"typ/module-typings"
        ~key:"Colors"
        ~json:first
      |> Result.expect ~msg:"first named json save should succeed" in
      let _ = Contentstore.Store.save_named_json_bundle
        store
        ~namespace:"typ/module-typings"
        ~key:"Colors"
        ~json:second
      |> Result.expect ~msg:"second named json save should succeed" in
      ...
      | None -> Error "expected saved named json bundle") |> Result.unwrap_or
    ~default:(Error "tempdir creation failed")
