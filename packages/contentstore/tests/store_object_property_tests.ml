...
let save_object_roundtrip =
  Property.for_all
    ...
    (fun content ->
      with_store
        "contentstore-prop-object-roundtrip"
        [ "objects" ]
        (fun ~tmpdir:_ ~store ->
...
let save_object_is_idempotent =
  Property.for_all
    ...
    (fun content ->
      with_store
        "contentstore-prop-object-idempotent"
        [ "objects" ]
        (fun ~tmpdir:_ ~store ->
...
let object_roundtrip_survives_reopen =
  Property.for_all
    ...
    (fun content ->
      with_store
        "contentstore-prop-object-reopen"
        [ "objects" ]
        (fun ~tmpdir ~store ->
          ...
          let reopened =
            Contentstore.create
              ...
              ~policy:Contentstore.Policy.default
          in
          match Contentstore.open_object reopened ~hash
          |> Result.map ~fn:read_opened_file with
          | Ok loaded -> String.equal loaded content
          | Error _ -> false))
...
let successful_object_writes_leave_no_temp_files =
  Property.for_all
    ...
    (fun contents ->
      with_store
        "contentstore-prop-object-no-temp"
        [ "objects" ]
        (fun ~tmpdir:_ ~store ->
...
let tests = [
  ...
  Test.property
    ...
    (fun _ctx ->
      assert_property
        "successful object writes leave no temp files"
        successful_object_writes_leave_no_temp_files);
]
