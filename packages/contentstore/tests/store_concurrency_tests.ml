...
let test_concurrent_same_hash_object_writers_converge = fun _ctx ->
  with_store "contentstore-concurrent-same-hash" [ "concurrent" ]
    (
      fun ~tmpdir:_ ~store ->
...
let test_concurrent_same_key_named_writers_converge = fun _ctx ->
  with_store "contentstore-concurrent-same-key" [ "concurrent" ]
    (
      fun ~tmpdir:_ ~store ->
        let key = "current" in
        let parent = self () in
        ...
        | Ok () -> (
          ...
          | Ok loaded when String.equal loaded left || String.equal loaded right -> Ok ()
          | Ok _ -> Error "expected concurrent same-key writers to converge to one complete named object"
          | Error err -> Error (Contentstore.Store.error_message err)
        )
    )

let test_concurrent_same_hash_commit_dir_writers_converge = fun _ctx ->
  with_store "contentstore-concurrent-commit-dir" [ "concurrent" ]
    (
      fun ~tmpdir ~store ->
        ...
        let make_source name payload =
          ...
          let _ = Fs.write payload Path.(source_dir / Path.v "payload.txt") |> Result.expect ~msg:"write payload should succeed" in source_dir
        in
        ...
        | Ok () -> (
          ...
          | Ok loaded when String.equal loaded "left-tree" || String.equal loaded "right-tree" -> Ok ()
          | Ok _ -> Error "expected concurrent same-hash commit_dir writers to converge to one complete tree"
...
let test_readers_see_old_or_new_named_values_during_overwrite = fun _ctx ->
  with_store "contentstore-concurrent-reader-writer" [ "concurrent" ]
    (
      fun ~tmpdir:_ ~store ->
        let parent = self () in
        let key = "current" in
              ...
              fun () ->
                ...
                let rec loop remaining =
                  if remaining = 0 then
                    ...
                    | Ok value -> Error ("reader observed a partial named value during overwrite: " ^ String.escaped value)
                    | Error (Contentstore.Store.Missing _) -> Error "reader observed a missing named value during overwrite"
                    | Error err -> Error (Contentstore.Store.error_message err)
                in
...
let test_mixed_workload_does_not_cross_corrupt = fun _ctx ->
  with_store "contentstore-concurrent-mixed-workload" [ "concurrent" ]
    (
      fun ~tmpdir ~store ->
        let parent = self () in
        ...
        | Error err -> Error err
        | Ok () -> (
          match Contentstore.open_object store ~hash:object_hash |> Result.map ~fn:read_opened_file, Contentstore.open_named_object store ~key:"current" |> Result.map ~fn:read_opened_file, Fs.read_to_string Path.(Contentstore.hash_dir_of store tree_hash / Path.v "payload.txt") with
...
let tests =
  [
    Test.case "concurrent same-hash object writers converge" test_concurrent_same_hash_object_writers_converge;
    Test.case "concurrent same-key named writers converge" test_concurrent_same_key_named_writers_converge;
    Test.case "concurrent same-hash commit_dir writers converge" test_concurrent_same_hash_commit_dir_writers_converge;
    Test.case "readers see old or new named values during overwrite" test_readers_see_old_or_new_named_values_during_overwrite;
