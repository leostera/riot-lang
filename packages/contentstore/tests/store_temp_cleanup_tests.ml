...
let object_parent_dir = fun store hash ->
  let hex = Crypto.Digest.hex hash in Path.(Contentstore.root store / Path.v "objects" / Path.v "cleanup" / Path.v (String.sub hex ~offset:0 ~len:2))

let named_parent_dir = fun store key ->
  let key_hash = Crypto.hash_string key |> Crypto.Digest.hex in Path.(Contentstore.root store / Path.v "named" / Path.v "cleanup" / Path.v (String.sub key_hash ~offset:0 ~len:2))
...
let test_failed_save_object_cleans_immutable_temp_files = fun _ctx ->
  with_store "contentstore-temp-cleanup-object" [ "cleanup" ]
    (
      fun ~tmpdir:_ ~store ->
        ...
        let result =
          ...
          | Error (Contentstore.Store.Io _) -> (
            ...
            if List.is_empty entries then
              Ok ()
            else Error "expected failed save_object to clean immutable temp files"
          )
...
let test_failed_save_named_object_cleans_mutable_temp_files = fun _ctx ->
  with_store "contentstore-temp-cleanup-named" [ "cleanup" ]
    (
      fun ~tmpdir:_ ~store ->
        let key = "current" in
        ...
        let result =
          ...
          | Error (Contentstore.Store.Io _) -> (
            ...
            if List.is_empty entries then
              Ok ()
            else Error "expected failed save_named_object to clean mutable temp files"
          )
          | Error err -> Error ("unexpected error: " ^ Contentstore.Store.error_message err)
...
let test_failed_save_file_cleans_immutable_temp_files = fun _ctx ->
  with_store "contentstore-temp-cleanup-save-file" [ "cleanup" ]
    (
      fun ~tmpdir ~store ->
        ...
        let result =
          ...
          | Error (Contentstore.Store.Io _) -> (
            ...
            if List.is_empty entries then
              Ok ()
            else Error "expected failed save_file to clean immutable temp files"
          )
          | Error err -> Error ("unexpected error: " ^ Contentstore.Store.error_message err)
...
let tests = [ Test.case "failed save_object cleans immutable temp files" test_failed_save_object_cleans_immutable_temp_files; Test.case "failed save_named_object cleans mutable temp files" test_failed_save_named_object_cleans_mutable_temp_files; Test.case "failed save_file cleans immutable temp files" test_failed_save_file_cleans_immutable_temp_files ]
