...
type t = { root: Path.t; ns: Namespace.t; policy: policy }
...
let hash_dir_of = fun store hash ->
  let _ = checked_hash_hex "hash_dir_of" hash in Layout.tree_dir store.root hash

let exists = fun store hash -> Fs.exists (hash_dir_of store hash) |> Result.unwrap_or ~default:false

let open_path = fun path ->
  let* path_exists = Fs.exists path |> Result.map_err ~fn:(
    fun detail ->
      Io {
        op = "exists";
        path;
  ...
  if not path_exists then
    Error (Missing { path })
  else Fs.File.open_read path |> Result.map_err ~fn:(
    fun detail ->
      Io {
        op = "open_read";
        path;
...
let save_object = fun store ~hash ~content ->
  ...
  let temp = temp_path store ~scope:Layout.Immutable ~seed:(seed_of_hash hash) in Atomic.write_object_if_absent ~temp ~dst:destination ~content

let save_file = fun store ~hash ~source ->
  let _ = checked_hash_hex "save_file" hash in
  let destination = Layout.object_path store.root ~ns:store.ns ~hash in
  let temp = temp_path store ~scope:Layout.Immutable ~seed:(seed_of_hash hash) in Atomic.copy_file_if_absent ~source ~temp ~dst:destination

let open_object = fun store ~hash ->
  let _ = checked_hash_hex "open_object" hash in open_path (Layout.object_path store.root ~ns:store.ns ~hash)

let save_named_object = fun store ~key ~content ->
  let destination = Layout.named_object_path store.root ~ns:store.ns ~key in
  let temp = temp_path store ~scope:Layout.Mutable ~seed:(seed_of_key key) in Atomic.replace_with_object ~temp ~dst:destination ~content

let save_named_file = fun store ~key ~source ->
  let destination = Layout.named_object_path store.root ~ns:store.ns ~key in
  let temp = temp_path store ~scope:Layout.Mutable ~seed:(seed_of_key key) in Atomic.replace_with_file ~source ~temp ~dst:destination

let open_named_object = fun store ~key -> open_path (Layout.named_object_path store.root ~ns:store.ns ~key)

let commit_dir = fun store ~hash ~source_dir ->
  let destination = hash_dir_of store hash in
  let temp = temp_path store ~scope:Layout.Trees ~seed:(seed_of_hash hash) in Atomic.commit_dir_if_absent ~source_dir ~staging:temp ~dst:destination
