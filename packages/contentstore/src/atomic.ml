...
let io_error = fun ~op ~path ?related_path detail ->
  Store_error.Io { op; path; related_path; detail = Store_error.Fs detail }
...
let validate_source_path = fun ~kind path ->
  ...
  |> Result.map_err ~fn:(fun detail -> io_error ~op:"exists" ~path detail) in
  if not exists then
    Error (Store_error.Invalid_source_path { path; reason = Store_error.Source_missing })
  else
    match kind with
    | Directory ->
        let* is_directory = Fs.is_dir path
        |> Result.map_err ~fn:(fun detail -> io_error ~op:"is_dir" ~path detail) in
...
let link_temp_if_absent = fun ~temp ~dst ->
  let result =
    ...
    |> Result.map_err ~fn:(fun detail -> io_error ~op:"exists" ~path:dst detail) in
...
let rec copy_tree = fun ~src ~dst ->
  ...
  |> Result.map_err ~fn:(fun detail -> io_error ~op:"read_dir" ~path:src detail) in
  let rec loop () =
    ...
    | Some entry ->
        ...
        let* is_directory = Fs.is_dir src_entry
        |> Result.map_err ~fn:(fun detail -> io_error ~op:"is_dir" ~path:src_entry detail) in
        let* () =
          if is_directory then
            ...
            | Ok () -> (
                ...
                | Error detail ->
                    let result =
                      ...
                      | Error exists_detail ->
                          Error (io_error ~op:"exists" ~path:dst ~related_path:source_dir exists_detail)
                    in
                    cleanup_dir staging;
                    result
              )
