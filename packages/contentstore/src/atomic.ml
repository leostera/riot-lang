...
let ensure_dir = fun path -> Fs.create_dir_all path |> Result.map_err ~fn:(
  fun detail -> io_error ~op:"create_dir_all" ~path detail
)
...
let validate_source_path = fun ~kind path ->
  let* exists = Fs.exists path |> Result.map_err ~fn:(
    fun detail -> io_error ~op:"exists" ~path detail
  )
  in
  if not exists then
    ...
    match kind with
    | Directory ->
        let* is_directory = Fs.is_dir path |> Result.map_err ~fn:(
          fun detail -> io_error ~op:"is_dir" ~path detail
        )
        in
        if is_directory then
          Ok ()
        else Error (Store_error.Invalid_source_path { path; reason = Store_error.Source_not_directory })
    | File ->
        let* is_directory = Fs.is_dir path |> Result.map_err ~fn:(
          fun detail -> io_error ~op:"is_dir" ~path detail
        )
...
let copy_to_temp = fun ~src ~temp ->
  let* () = validate_source_path ~kind:File src
  in
  let* () = ensure_parent temp
  in
...
let link_temp_if_absent = fun ~temp ~dst ->
  let result =
    ...
    in
    let* already_exists = Fs.exists dst |> Result.map_err ~fn:(
      fun detail -> io_error ~op:"exists" ~path:dst detail
    )
    in
...
let write_object_if_absent = fun ~temp ~dst ~content ->
  let* () = write_temp_file ~temp ~content in link_temp_if_absent ~temp ~dst

let copy_file_if_absent = fun ~source ~temp ~dst ->
  let* () = copy_to_temp ~src:source ~temp in link_temp_if_absent ~temp ~dst
...
let replace_with_object = fun ~temp ~dst ~content ->
  let* () = write_temp_file ~temp ~content in replace_with_temp ~temp ~dst

let replace_with_file = fun ~source ~temp ~dst ->
  let* () = copy_to_temp ~src:source ~temp in replace_with_temp ~temp ~dst

let rec copy_tree = fun ~src ~dst ->
  let* () = ensure_dir dst
  in
  let* entries = Fs.read_dir src |> Result.map_err ~fn:(
    fun detail -> io_error ~op:"read_dir" ~path:src detail
  )
  in
  let rec loop () =
    ...
    | Some entry ->
        ...
        let* is_directory = Fs.is_dir src_entry |> Result.map_err ~fn:(
          fun detail -> io_error ~op:"is_dir" ~path:src_entry detail
        )
        in
        let* () =
          if is_directory then
            copy_tree ~src:src_entry ~dst:dst_entry
          else Fs.copy ~src:src_entry ~dst:dst_entry |> Result.map_err ~fn:(
            fun detail -> io_error ~op:"copy" ~path:dst_entry ~related_path:src_entry detail
          )
        in
        loop ()
  in
...
let commit_dir_if_absent = fun ~source_dir ~staging ~dst ->
  let wrap_destination_error ~op error =
    ...
    | _ -> Error error
  in
  let* () = validate_source_path ~kind:Directory source_dir
  in
  let* () = ensure_parent dst |> Result.or_else ~fn:(wrap_destination_error ~op:"create_dir_all")
  in
  let* already_exists = Fs.exists dst |> Result.map_err ~fn:(
    fun detail -> io_error ~op:"exists" ~path:dst ~related_path:source_dir detail
  )
  in
  if already_exists then
    (
          ...
          | Error detail ->
              let result =
                ...
                | Ok true ->
                    cleanup_dir source_dir;
                    Ok ()
                | Ok false -> Error (io_error ~op:"rename" ~path:dst ~related_path:staging detail)
                | Error exists_detail -> Error (io_error ~op:"exists" ~path:dst ~related_path:source_dir exists_detail)
              in
              cleanup_dir staging;
              result
        )
