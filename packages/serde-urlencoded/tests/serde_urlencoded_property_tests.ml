...
let sample_decode = De.record_mut ~fields:sample_fields ~create:(
  ...
  fun (builder: sample_builder) ->
    match builder.name, builder.age, builder.active, builder.small, builder.big, builder.ratio, builder.tags, builder.scores, builder.status with
      ...
      Ser.fields
        [
          Ser.field "name" Ser.string
            (
              fun (value: sample) -> value.name
            );
          Ser.field "age" Ser.int
            (
              fun (value: sample) -> value.age
            );
...
let equal_sample = fun (left: sample) (right: sample) -> String.equal left.name right.name && Int.equal left.age right.age && Bool.equal left.active right.active && Int32.equal left.small right.small && Int64.equal left.big right.big && equal_float left.ratio right.ratio && equal_vec left.tags right.tags && left.scores = right.scores && left.nickname = right.nickname && equal_status left.status right.status
...
let sample_gen =
  Generator.map3
    ...
    (Generator.pair (Generator.pair (Generator.pair Generator.string Generator.int) Generator.bool) (Generator.triple Generator.int32 Generator.int64 finite_float_gen))
    (Generator.triple non_empty_string_vec_gen non_empty_int_array_gen (Generator.option Generator.string))
    status_gen
...
let roundtrip_in_memory = fun encode decode equal value ->
  ...
  | Ok encoded -> (
    match Serde_urlencoded.of_string decode encoded with
...
let roundtrip_io = fun encode decode equal value ->
  ...
  | Ok () -> (
    match Serde_urlencoded.of_reader decode (String.to_reader ~chunk_size:io_chunk_size (IO.Buffer.contents buffer)) with
    | Ok decoded -> equal decoded value
...
let unit_roundtrip_prop =
  run_property "serde-urlencoded property empty record roundtrips" Arbitrary.bool
    (
      fun _ ->
...
let bool_roundtrip_prop = run_property "serde-urlencoded property bool field roundtrips" Arbitrary.bool (roundtrip_in_memory (single_field_encode "value" Ser.bool) (single_field_decode "value" De.bool) Bool.equal)

let int_roundtrip_prop = run_property "serde-urlencoded property int field roundtrips" Arbitrary.int (roundtrip_in_memory (single_field_encode "value" Ser.int) (single_field_decode "value" De.int) Int.equal)

let int32_roundtrip_prop = run_property "serde-urlencoded property int32 field roundtrips" Arbitrary.int32 (roundtrip_in_memory (single_field_encode "value" Ser.int32) (single_field_decode "value" De.int32) Int32.equal)

let int64_roundtrip_prop = run_property "serde-urlencoded property int64 field roundtrips" Arbitrary.int64 (roundtrip_in_memory (single_field_encode "value" Ser.int64) (single_field_decode "value" De.int64) Int64.equal)

let float_roundtrip_prop = run_property "serde-urlencoded property float field roundtrips" finite_float_arb (roundtrip_in_memory (single_field_encode "value" Ser.float) (single_field_decode "value" De.float) equal_float)

let string_roundtrip_prop = run_property "serde-urlencoded property string field roundtrips" Arbitrary.string (roundtrip_in_memory (single_field_encode "value" Ser.string) (single_field_decode "value" De.string) String.equal)

let option_string_roundtrip_prop = run_property "serde-urlencoded property option string field roundtrips" Arbitrary.(option string) (roundtrip_in_memory (single_field_encode "value" (Ser.option Ser.string)) (optional_field_decode "value" (De.option De.string)) (=))

let string_list_roundtrip_prop = run_property ~examples:composite_examples "serde-urlencoded property string list field roundtrips" non_empty_string_vec_arb (roundtrip_in_memory (single_field_encode "value" (Ser.list Ser.string)) (single_field_decode "value" (De.list De.string)) equal_vec)

let int_array_roundtrip_prop = run_property ~examples:composite_examples "serde-urlencoded property int array field roundtrips" non_empty_int_array_arb (roundtrip_in_memory (single_field_encode "value" (Ser.array Ser.int)) (single_field_decode "value" (De.array De.int)) (=))

let status_roundtrip_prop = run_property ~examples:composite_examples "serde-urlencoded property unit enum field roundtrips" status_arb (roundtrip_in_memory (single_field_encode "value" status_encode) (single_field_decode "value" status_decode) equal_status)

let sample_roundtrip_prop = run_property ~examples:composite_examples "serde-urlencoded property sample roundtrips" sample_arb (roundtrip_in_memory sample_encode sample_decode equal_sample)

let sample_io_roundtrip_prop = run_property ~examples:composite_examples "serde-urlencoded property sample roundtrips over io" sample_arb (roundtrip_io sample_encode sample_decode equal_sample)
