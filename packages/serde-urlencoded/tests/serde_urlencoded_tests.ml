...
let address_encode = Ser.record
  (Ser.fields [ Ser.field "city" Ser.string (fun (value: address) -> value.city) ])

let nested_encode = Ser.record
  (Ser.fields
    [
...
let test_rejects_nested_record_values = fun _ctx ->
  ...
  | Error err -> Error ("expected nested record encode to fail clearly, got "
...
let test_rejects_payload_variant_values = fun _ctx ->
  match Serde_urlencoded.to_string
    (Ser.record (Ser.fields [ Ser.field "pet" pet_encode (fun value -> value) ]))
    (Dog "Chouchou") with
  | Error (`Msg message) when String.contains message "payload variant" -> Ok ()
  | Error err -> Error ("expected payload variant encode to fail clearly, got "
  ^ Serde.Error.to_string err)
