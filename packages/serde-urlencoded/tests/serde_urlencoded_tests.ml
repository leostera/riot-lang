...
let address_encode =
  Ser.record
    (
      Ser.fields
        [
          Ser.field "city" Ser.string
            (
              fun (value: address) -> value.city
            );
        ]
    )
      ...
      Ser.fields
        [
          Ser.field "title" Ser.string
            (
              fun (value: nested) -> value.title
            );
          Ser.field "address" address_encode
            (
              fun (value: nested) -> value.address
            );
        ]
    )
...
let encoded_sample_without_nickname = "name=Monkey+D.+Luffy&age=19&active=true&small=12&big=345&ratio=1.25&tags=riot&tags=serde+ml&scores=1&scores=2&status=Draft"

let encoded_sample_with_nickname = "name=Monkey+D.+Luffy&age=19&active=true&small=12&big=345&ratio=1.25&tags=riot&tags=serde+ml&scores=1&scores=2&nickname=strawhat&status=Draft"

let test_encodes_records = fun _ctx ->
  match Serde_urlencoded.to_string sample_encode sample_value with
  | Ok actual -> expect_equal ~expected:encoded_sample_without_nickname ~actual ~message:"expected serde-urlencoded encoder to serialize flat records with repeated keys for sequences"
...
let test_decodes_records = fun _ctx ->
  ...
  | Ok actual ->
      if equal_sample actual sample_value then
        Ok ()
      else Error "expected serde-urlencoded decoder to parse flat records and repeated keys"
...
let test_roundtrips_records = fun _ctx ->
  ...
  | Ok actual ->
      if equal_sample actual sample_value_with_nickname then
        Ok ()
      else Error "expected serde-urlencoded encode/decode to roundtrip supported record values"
  | Error err -> Error ("roundtrip decode failed: " ^ Serde.Error.to_string err)

let test_decodes_from_reader = fun _ctx ->
  match Serde_urlencoded.of_reader sample_decode (String.to_reader ~chunk_size:2 encoded_sample_with_nickname) with
  | Ok actual ->
      if equal_sample actual sample_value_with_nickname then
        Ok ()
      else Error "expected serde-urlencoded decoder to read from an IO.Reader"
...
let test_writes_to_writer = fun _ctx ->
  ...
  | Ok () -> expect_equal ~expected:encoded_sample_with_nickname ~actual:(IO.Buffer.contents buffer) ~message:"expected serde-urlencoded encoder to write to an IO.Writer"
...
let test_rejects_nested_record_values = fun _ctx ->
  ...
  | Error err -> Error ("expected nested record encode to fail clearly, got " ^ Serde.Error.to_string err)
...
let test_rejects_payload_variant_values = fun _ctx ->
  ...
  | Error err -> Error ("expected payload variant encode to fail clearly, got " ^ Serde.Error.to_string err)
...
let test_encodes_top_level_unit_as_empty = fun _ctx ->
  ...
  | Ok actual -> expect_equal ~expected:"" ~actual ~message:"expected top-level unit encoding to be empty"
