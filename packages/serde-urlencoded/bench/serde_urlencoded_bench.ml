...
let tags_of_count = fun count ->
  ...
  for index = 0 to count - 1 do Vector.push tags ~value:("grand-line-log-" ^ Int.to_string index) done;
  tags
...
let payload_decode = De.record_mut ~fields:payload_fields ~create:(
  fun (): payload_builder ->
    {
      name = None;
      role = None;
      ...
      Ser.fields
        [
          Ser.field "name" Ser.string
            (
              fun (value: payload) -> value.name
            );
          Ser.field "role" Ser.string
            (
              fun (value: payload) -> value.role
            );
          Ser.field "crew" Ser.string
            (
              fun (value: payload) -> value.crew
            );
          Ser.field "age" Ser.int
            (
              fun (value: payload) -> value.age
            );
          Ser.field "active" Ser.bool
            (
              fun (value: payload) -> value.active
            );
          Ser.field "small" Ser.int32
            (
              fun (value: payload) -> value.small
            );
          Ser.field "big" Ser.int64
            (
              fun (value: payload) -> value.big
            );
          Ser.field "ratio" Ser.float
            (
              fun (value: payload) -> value.ratio
            );
          Ser.field "tags" (Ser.list Ser.string)
            (
              fun (value: payload) -> value.tags
            );
          Ser.field "scores" (Ser.array Ser.int)
            (
              fun (value: payload) -> value.scores
            );
          Ser.field "nickname" (Ser.option Ser.string)
            (
              fun (value: payload) -> value.nickname
            );
          Ser.field "status" status_encode
            (
              fun (value: payload) -> value.status
            );
        ]
    )
...
let build_fixture = fun ({ label; tag_count; score_count; string_repeat }: fixture_spec) ->
  ...
  in
  let encoded = Serde_urlencoded.to_string payload_encode value |> Result.expect ~msg:("expected " ^ label ^ " fixture to encode") in { label; value; encoded }
...
let bench_encode_in_memory = fun fixture () -> ignore (Serde_urlencoded.to_string payload_encode fixture.value)

let bench_encode_writer = fun fixture () ->
  let buffer = IO.Buffer.create ~size:(String.length fixture.encoded) in ignore (Serde_urlencoded.to_writer payload_encode (io_writer_of_buffer buffer) fixture.value)

let bench_decode_in_memory = fun fixture () -> ignore (Serde_urlencoded.of_string payload_decode fixture.encoded)

let bench_decode_reader = fun fixture () -> ignore (Serde_urlencoded.of_reader payload_decode (String.to_reader ~chunk_size:io_chunk_size fixture.encoded))
...
let benchmark_suite = fun fixture ->
  ...
  in
  Bench.[
    with_config ~config ("serde-urlencoded encode in-memory " ^ fixture.label ^ " payload (" ^ size ^ ")") (bench_encode_in_memory fixture);
    with_config ~config ("serde-urlencoded encode writer " ^ fixture.label ^ " payload (" ^ size ^ ")") (bench_encode_writer fixture);
    with_config ~config ("serde-urlencoded decode in-memory " ^ fixture.label ^ " payload (" ^ size ^ ")") (bench_decode_in_memory fixture);
    with_config ~config ("serde-urlencoded decode reader " ^ fixture.label ^ " payload (" ^ size ^ ")") (bench_decode_reader fixture);
  ]

let main ~args =
  let small_fixture = build_fixture small_fixture_spec in
  let large_fixture = build_fixture large_fixture_spec in
  let benchmarks = benchmark_suite small_fixture @ benchmark_suite large_fixture in Bench.Cli.main ~name:"serde-urlencoded benchmarks" ~benchmarks ~args
