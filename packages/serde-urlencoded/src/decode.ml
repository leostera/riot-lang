...
type grouped_field = { key: string; values: string array }

type grouped_field_acc = { key: string; values: string Vector.t }

type context =
  | Top_level
  | Field_values of string array

type state = { fields: grouped_field array; mutable root_consumed: bool; mutable context: context }
...
let parse_fields = fun input ->
  let pairs = Net.Uri.Query.parse input |> List.filter ~fn:(
    fun ((key, value)) -> not (String.equal key "" && String.equal value "")
  ) in
  ...
  List.for_each pairs ~fn:(
    fun ((key, value)) ->
      ...
      | Some index ->
          let group = Vector.get_unchecked groups ~at:index in Vector.push group.values ~value
      | None ->
  ...
  Array.init ~count:(Vector.len groups) ~fn:(
    fun index ->
      let group = Vector.get_unchecked groups ~at:index in ({ key = group.key; values = Vector.to_array group.values } : grouped_field)
  )
...
let rec backend: state De.backend = {
  bool = (
    fun state -> parse_bool (expect_single_value state "bool")
  );
  string = (
    fun state -> expect_single_value state "string"
  );
  int = (
    fun state -> parse_int (expect_single_value state "int")
  );
  int32 = (
    fun state -> parse_int32 (expect_single_value state "int32")
  );
  int64 = (
    fun state -> parse_int64 (expect_single_value state "int64")
  );
  float = (
    fun state -> parse_float (expect_single_value state "float")
  );
  skip_any = (
    fun state ->
      match state.context with
    ...
    fun state decode ->
      ...
      | Top_level ->
          ...
          if Int.equal (Array.length state.fields) 0 then
            None
          else Some (decode.run backend state)
    ...
    fun state decode ->
      ...
      | Field_values values ->
          ...
          for index = 0 to Array.length values - 1 do
            let value = Array.get_unchecked values ~at:index in Vector.push result ~value:(decode.run backend (state_of_value value))
          done;
          result
  );
  array = (
    fun state decode ->
      ...
      | Field_values values -> Array.init ~count:(Array.length values) ~fn:(
        fun index ->
          let value = Array.get_unchecked values ~at:index in decode.run backend (state_of_value value)
      )
  );
  record = (
    fun state ~fields ~init ~step ~finish ->
      ...
      | Top_level ->
          ...
          for index = 0 to Array.length state.fields - 1 do
            ...
            let tag = De.Fields.match_slice fields field.key ~offset:0 ~length:(String.length field.key) in acc := with_values state field.values
              (
                fun () -> step !acc tag
              )
          done;
    ...
    fun state ~fields ~create ~step ~finish ->
      ...
      | Top_level ->
          ...
          for index = 0 to Array.length state.fields - 1 do
            ...
            with_values state field.values
              (
                fun () -> step builder tag
              )
          done;
          finish builder
  );
    ...
    fun state cases ->
      ...
      let rec loop index saw_payload_case =
        if Int.equal index (Array.length cases) then
          ...
          else raise (Serde.Decode_error `invalid_tag)
        else
          match Array.get_unchecked cases ~at:index with
          | De.Unit (tag, result) ->
              if String.equal value tag then
                result
              else loop (index + 1) saw_payload_case
...
let finish_decode = fun state result ->
  ...
  | Ok value ->
      if state.root_consumed || Int.equal (Array.length state.fields) 0 then
        Ok value
      else Error (`Msg "unsupported top-level value for application/x-www-form-urlencoded input")

let of_string = fun decode input ->
  let state = { fields = parse_fields input; root_consumed = false; context = Top_level } in De.run decode backend state |> finish_decode state
