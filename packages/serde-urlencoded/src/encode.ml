...
type state = { output: IO.Buffer.t; mutable first: bool; mutable context: context }
...
let write_pair = fun state key value ->
  if state.first then
    state.first <- false
  else IO.Buffer.add_char state.output '&';
...
let rec backend: state Ser.backend = {
  bool = (
    fun state value -> emit_scalar state "bool" Bool.to_string value
  );
  string = (
    fun state value ->
      emit_scalar state "string"
        (
          fun text -> text
        )
        value
  ...
  int = (
    fun state value -> emit_scalar state "int" Int.to_string value
  );
  int32 = (
    fun state value -> emit_scalar state "int32" Int32.to_string value
  );
  int64 = (
    fun state value -> emit_scalar state "int64" Int64.to_string value
  );
  float = (
    fun state value -> emit_scalar state "float" float_to_string value
  );
  null = (
    fun state ->
      match current_key state.context with
  ...
  list = (
    fun state encode values ->
      match current_key state.context with
      | Some _ -> Vector.for_each values ~fn:(
  ...
  array = (
    fun state encode values ->
      match current_key state.context with
      | Some _ ->
          for index = 0 to Array.length values - 1 do encode.run backend state (Array.get_unchecked values ~at:index) done
          ...
          for index = 0 to Array.length fields - 1 do
            ...
            | Ser.Field (name, encode, get) ->
                with_field state name
                  (
                    fun () -> encode.run backend state (get value)
                  )
          done
