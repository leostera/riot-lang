...
let error_message = function
  ...
  | DependencySectionMustBeTable { manifest_path; section } -> "manifest dependency section '[" ^ section ^ "]' in '" ^ Path.to_string manifest_path ^ "' must be a table"
...
let rec canonicalize_toml_value = function
  ...
  | Std.Data.Toml.Table fields -> Std.Data.Toml.Table (fields |> List.map ~fn:(
    fun (key, value) -> (key, canonicalize_toml_value value)
  ) |> List.sort ~compare:(
...
let dependency_section_value = fun ~manifest_path section_name toml ->
  ...
  | Std.Data.Toml.Table fields -> (
    match List.find fields ~fn:(
      fun (key, _) -> String.equal key section_name
    ) |> Option.map ~fn:(
      fun (_, value) -> value
    ) with
...
let load_manifest_toml = fun ~workspace_manager manifest_path -> Riot_model.Workspace_manager.load_riot_toml workspace_manager manifest_path |> Result.map_err ~fn:(
  fun error -> ManifestLoadFailed { manifest_path; error }
)

let manifest_dependency_fingerprint = fun ~workspace_manager ~workspace_root manifest_path ->
  let* toml = load_manifest_toml ~workspace_manager manifest_path
  in
  let* dependencies = dependency_section_value ~manifest_path "dependencies" toml
  in
  let* build_dependencies = dependency_section_value ~manifest_path "build-dependencies" toml
  in
  let* dev_dependencies = dependency_section_value ~manifest_path "dev-dependencies" toml
  in
  Ok (
    Std.Data.Toml.Table [
...
let dependency_hash = fun ~workspace_manager ~workspace_root ~manifest_paths ->
  ...
  let rec loop acc = function
    | [] ->
        let canonical = Std.Data.Toml.Array (List.reverse acc) |> canonicalize_toml_value |> Std.Data.Toml.to_string in Ok (Crypto.hash_string canonical |> Crypto.Digest.hex)
    | manifest_path :: rest ->
        let* fingerprint = manifest_dependency_fingerprint ~workspace_manager ~workspace_root manifest_path in loop (fingerprint :: acc) rest
  in
  loop [] manifest_paths

let needs_refresh = fun ~workspace_manager ~workspace_root ~manifest_paths ~lockfile ->
  let* current_dependency_hash = dependency_hash ~workspace_manager ~workspace_root ~manifest_paths
  in
  match lockfile with
  | None -> Ok true
  | Some (lockfile: Riot_model.Lockfile.t) -> Ok (not (String.equal lockfile.dependency_hash current_dependency_hash))
