...
let error_message = function
  ...
  | DependencySectionMustBeTable { manifest_path; section } ->
      "manifest dependency section '["
      ^ section
      ^ "]' in '"
      ^ Path.to_string manifest_path
      ^ "' must be a table"
  | ManifestMustBeTable { manifest_path } ->
      "manifest '" ^ Path.to_string manifest_path ^ "' must decode to a TOML table"
...
let dependency_section_value = fun ~manifest_path section_name toml ->
  match toml with
  | Std.Data.Toml.Table fields -> (
      match List.find fields ~fn:(fun (key, _) -> String.equal key section_name)
