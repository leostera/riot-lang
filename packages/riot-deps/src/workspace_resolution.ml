...
let ensure_workspace = fun ?(emit = no_emit) ~workspace_manager ~mode ~registry ~(workspace:Riot_model.Workspace_manifest.t) () -> let* (_lockfile, resolved_packages) =
  ensure_lock ~workspace_manager ~emit ~mode ~registry ~workspace () in
