...
let materialization_state = fun ~registry ~(pkg:Riot_model.Lockfile.package) ->
  ...
  | Some version ->
      ...
      let manifest_path =
        Pkgs_ml.Registry_cache.package_src_dir (Pkgs_ml.Registry.cache registry) ~package_name ~version |> fun root -> Path.(root / Path.v "riot.toml")
      in
      match Fs.exists manifest_path with
      | Error err -> Error (Error.MaterializationFailed { error = "failed to check package manifest '" ^ Path.to_string manifest_path ^ "': " ^ IO.error_message err })
      | Ok true -> Ok Already_materialized
      | Ok false ->
          let package_name = Riot_model.Package_name.to_string pkg.id.name in
          let archive_path = Pkgs_ml.Registry_cache.archive_path (Pkgs_ml.Registry.cache registry) ~package_name ~version in
          match Fs.exists archive_path with
          | Error err -> Error (Error.MaterializationFailed { error = "failed to check cached package archive '" ^ Path.to_string archive_path ^ "': " ^ IO.error_message err })
          | Ok has_archive ->
              Ok (
                if has_archive then
                  Needs_materialization_from_cache
...
let ensure_registry_package = fun ?(emit = no_emit) ~registry ~(pkg:Riot_model.Lockfile.package) () ->
  ...
  match pkg.id.version with
  | None -> Error (Error.MaterializationFailed { error = "registry lock package '" ^ package ^ "' is missing an exact version" })
  | Some version -> (
    let root = Pkgs_ml.Registry_cache.package_src_dir (Pkgs_ml.Registry.cache registry) ~package_name:package ~version in
    let path = Path.to_string root in
    let* state = materialization_state ~registry ~pkg
    in
    match state with
    | Already_materialized ->
        emit
        ...
        Ok root
    | Needs_materialization_from_cache | Needs_download ->
...
let ensure_packages = fun ?(emit = no_emit) ~registry ~(lockfile:Riot_model.Lockfile.t) () ->
  let rec loop = function
    ...
    | (pkg: Riot_model.Lockfile.package) :: rest -> (
      ...
      | Riot_model.Lockfile.Workspace | Riot_model.Lockfile.Path _ | Riot_model.Lockfile.Source _ -> loop rest
