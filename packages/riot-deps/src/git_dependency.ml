...
let message = function
  | InvalidSourceSpec { source; reason } -> "invalid source dependency '" ^ source ^ "': " ^ invalid_source_spec_message reason
  | UnsupportedSourceHost { source; host } -> "source dependency '" ^ source ^ "' uses unsupported host '" ^ host ^ "'"
  | CachedRepositoryInvalid { path } -> "cached git dependency repository at '" ^ Path.to_string path ^ "' is not a git checkout"
  | PackageRootMissing { path } -> "materialized source dependency is missing package root '" ^ Path.to_string path ^ "'"
  ...
  | GitCommandSpawnFailed { command; error } -> "failed to spawn git command '" ^ command ^ "': " ^ command_spawn_error_message error

let split_once = fun ~on text ->
  match String.index_of text ~char:on with
  | None -> None
  | Some idx -> Some (String.sub text ~offset:0 ~len:idx, String.sub text ~offset:(idx + 1) ~len:(String.length text - idx - 1))
...
let looks_like_github_shorthand = fun raw ->
  ...
  | owner :: repo :: _ when not (String.equal owner "") && not (String.equal repo "") && not (String.equal owner ".") && not (String.equal owner "..") && not (String.starts_with ~prefix:"." owner) && not (String.starts_with ~prefix:"/" owner) -> true
  | _ -> false

let looks_like_remote_spec = fun raw ->
  let raw = String.trim raw in String.starts_with ~prefix:"http://" raw || String.starts_with ~prefix:"https://" raw || String.starts_with ~prefix:"github.com/" raw || looks_like_github_shorthand raw
...
let parse_spec = fun raw ->
  ...
  | [ locator; ref_ ] ->
      Ok {
        ...
        ref_ = if String.equal (String.trim ref_) "" then
          None
        else Some (String.trim ref_)
      }
...
let parse_source_locator = fun source_locator ->
  ...
  let normalized =
    ...
    | owner :: repo :: _ when not (String.equal owner "") && not (String.equal repo "") && not (String.equal owner ".") && not (String.equal owner "..") && not (String.starts_with ~prefix:"." owner) && not (String.starts_with ~prefix:"/" owner) && not (String.contains owner ".") -> "github.com/" ^ normalized
    | _ -> normalized
  in
  match String.split ~by:"/" normalized with
...
let run_git = fun ?cwd args ->
  ...
  | Error error -> Error (GitCommandSpawnFailed { command = Command.to_string command; error = CommandError error })
...
let remote_url_of_locator = fun ({ host; owner; repo; _ }: locator) -> "https://" ^ host ^ "/" ^ owner ^ "/" ^ repo ^ ".git"

let checkout_target = fun ~repo_dir ~ref_ ->
  match run_git ~cwd:repo_dir
    [
      "rev-parse";
      "--verify";
      "--quiet";
...
let sync_checkout = fun ?(update = true) ~repo_dir ~remote_url ~ref_ () ->
  ...
  let parent =
    ...
    | None -> Path.v "."
  in
  let* has_repo_root = Fs.exists repo_dir |> Result.map_err ~fn:(
    fun err -> GitCommandSpawnFailed { command = "fs.exists"; error = IoError err }
  )
  in
  if has_repo_root then
    let* has_git_dir = Fs.exists repo_git_dir |> Result.map_err ~fn:(
      fun err -> GitCommandSpawnFailed { command = "fs.exists"; error = IoError err }
    )
    in
    ...
    let* () = Fs.create_dir_all parent |> Result.map_err ~fn:(
      fun err -> GitCommandSpawnFailed { command = "fs.create_dir_all"; error = IoError err }
    )
    in
    let* _ =
      run_git ~cwd:parent
...
let materialize = fun ?(update = true) ~source_locator ~ref_ () ->
  let* locator = parse_source_locator source_locator
  in
  let repository_root = Riot_model.Riot_dirs.git_registry_repo_dir ~host:locator.host ~owner:locator.owner ~repo:locator.repo in
  let ref_ = Option.unwrap_or ~default:"main" ref_ in
  let* checkout_status = sync_checkout ~update ~repo_dir:repository_root ~remote_url:(remote_url_of_locator locator) ~ref_ ()
  in
  let package_root =
  ...
  in
  let* exists = Fs.exists package_root |> Result.map_err ~fn:(
    fun err -> GitCommandSpawnFailed { command = "fs.exists"; error = IoError err }
  )
  in
  if not exists then
