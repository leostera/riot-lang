...
let parse_name = fun spec raw_name ->
  let name = String.trim raw_name in Riot_model.Package_name.from_string name |> Result.map_err ~fn:(
    fun error -> InvalidPackageName { spec; name; error }
  )

let from_string = fun spec ->
  match String.split ~by:"@" spec with
  | [ name ] ->
      let* name = parse_name spec name in Ok { name; requirement = Some Std.Version.any }
  | [ name; requirement ] ->
      let* name = parse_name spec name
      in
      let requirement = String.trim requirement in
      let* requirement = Std.Version.parse_requirement requirement |> Result.map_err ~fn:(
        fun error -> InvalidRequirement { spec; requirement; error }
      ) in Ok { name; requirement = Some requirement }
...
let error_message = function
  | InvalidShape { spec } -> "invalid registry package spec '" ^ spec ^ "': expected <name> or <name>@<version>"
  | InvalidPackageName { spec; error; _ } -> "invalid registry package spec '" ^ spec ^ "': " ^ Riot_model.Package_name.error_message error
  | InvalidRequirement { spec; error; _ } -> "invalid registry package spec '" ^ spec ^ "': " ^ version_parse_error_message error
