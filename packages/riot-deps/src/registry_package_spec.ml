...
let from_string = fun spec ->
  ...
  | [name;requirement] ->
      ...
      let* requirement = Std.Version.parse_requirement requirement
      |> Result.map_err ~fn:(fun error -> InvalidRequirement { spec; requirement; error }) in
      Ok { name; requirement = Some requirement }
  | _ ->
      Error (InvalidShape { spec })
...
let error_message = function
  ...
  | InvalidPackageName { spec; error; _ } -> "invalid registry package spec '"
  ^ spec
  ^ "': "
  ^ Riot_model.Package_name.error_message error
  | InvalidRequirement { spec; error; _ } -> "invalid registry package spec '"
  ^ spec
  ^ "': "
  ^ version_parse_error_message error
