...
let message = function
  | NotGitRepository { path } -> "package root '" ^ Path.to_string path ^ "' is not inside a git repository"
  | MissingOriginRemote { path } -> "git repository at '" ^ Path.to_string path ^ "' is missing remote 'origin'"
  | InvalidRepositoryRoot { path; error } -> "git returned invalid repository root '" ^ path ^ "': " ^ path_error_message error
  | PackageOutsideRepository { package_root; repository_root } -> "package root '" ^ Path.to_string package_root ^ "' is not inside repository root '" ^ Path.to_string repository_root ^ "'"
  ...
  | GitCommandSpawnFailed { command; error } -> "failed to spawn git command '" ^ command ^ "': " ^ command_error_message error
...
let run_git = fun ~cwd args ->
  ...
  | Error error -> Error (GitCommandSpawnFailed { command = Command.to_string command; error })
  | Ok output when not (Int.equal output.status 0) ->
      ...
      if String.contains stderr "not a git repository" then
        ...
        if String.contains stderr "No such remote 'origin'" then
...
let normalize_remote_url = fun raw_url ->
  ...
  if String.starts_with ~prefix:"https://" url then
    ...
    if String.starts_with ~prefix:"http://" url then
      ...
      if String.starts_with ~prefix:"ssh://git@" url then
        ...
        if String.starts_with ~prefix:"git@" url then
          ...
          | Some (host, path) -> mk_locator host path
          | None -> Error (UnsupportedRemoteUrl { url = raw_url })
        else Error (UnsupportedRemoteUrl { url = raw_url })
