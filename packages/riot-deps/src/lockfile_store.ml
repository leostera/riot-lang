...
let error_message = function
  | StatFailed { path; error } -> "failed to check lockfile '"
  ^ Path.to_string path
  ^ "': "
  ^ IO.error_message error
  | ReadFailed { path; error } -> "failed to read lockfile '"
  ^ Path.to_string path
  ^ "': "
  ^ IO.error_message error
  | TomlParseFailed { path; error } -> "failed to parse lockfile TOML '"
  ^ Path.to_string path
  ^ "': "
  ^ Std.Data.Toml.error_to_string error
  | DecodeFailed { path; error } -> "failed to decode lockfile '"
  ^ Path.to_string path
  ^ "': "
  ^ Riot_model.Lockfile.error_message error
  | WriteFailed { path; error } -> "failed to write lockfile '"
  ^ Path.to_string path
  ^ "': "
  ^ IO.error_message error
...
let read = fun ~workspace_root ->
  ...
  | Error err ->
      Error (StatFailed { path = lock_path; error = err })
  | Ok false ->
      Ok None
  | Ok true -> (
