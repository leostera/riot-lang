...
let package_id_of_local_entry = fun (entry:local_entry) ->
  ...
  | Riot_model.Lockfile.Registry _ ->
      panic "local catalog entries cannot use registry provenance"
...
let lock_root_of_local_entry = fun ~workspace_root (entry:local_entry) ->
  ...
  | Riot_model.Lockfile.Path _ ->
      Some (relative_path_from ~base:workspace_root entry.package.path)
  | Riot_model.Lockfile.Registry _ ->
      None
...
let dependency_target_name = fun (catalog:catalog) ~declared_from ~required_by (
  ...
  | { source_locator=Some source_locator; _ } ->
      let* pkg = load_source_dependency_package ~dependency_name:dep.name ~source_locator ~ref_:dep.source.ref_ in
      let* () = validate_source_requirement ~dependency_name:dep.name ~source_locator dep pkg in
      let* () =
        register_local_entry catalog {
          package = pkg;
          provenance = Riot_model.Lockfile.Source { locator = source_locator; ref_ = dep.source.ref_ };
        }
      in
      Ok (Some pkg.name)
  | _ ->
...
let parse_registry_version = fun ~package_name version_string ->
  Std.Version.parse version_string |> Result.map_error (fun _ ->
    Error.Unexpected {
...
let matching_registry_versions = fun (catalog:catalog) ~package_name ~ranges ->
  ...
  let versions =
    ...
    | Unlock ->
        []
    | Refresh -> (
        match find_existing_registry_package ~registry_name ~existing_lock:catalog.ctx.existing_lock ~package_name with
        | Some pkg -> (
...
let registry_provider_dependencies_of_locked_dependency = fun (dep:Riot_model.Lockfile.dependency) ->
  ...
  | _ ->
      Ok (dep.package.name, Pubgrub.full)
...
let provider_dependencies_of_registry_package = fun (catalog:catalog) ~package_name version ->
  ...
  | _ ->
      ...
      (
        match document_opt with
        | None ->
            Ok (Pubgrub.Provider.Unavailable ("package '" ^ package_name ^ "' was not found"))
        | Some document -> (
            match List.find_opt
              (fun (release: Pkgs_ml.Sparse_index.release) ->
                String.equal release.version version_string)
              document.releases with
            | None ->
                        ...
                        (
                          let () =
                            record_required_by
                              ...
                              (Some Riot_model.Pm_error.{ package = package_name; path = None })
                          in
                          let ranges =
...
let provider_dependencies_of_root = fun (catalog:catalog) (workspace_packages:Riot_model.Package.t list) ->
  let workspace_deps =
    List.map
      (fun (pkg:Riot_model.Package.t) -> (pkg.name, Pubgrub.full))
      workspace_packages
  in
  let* runtime_deps =
    provider_dependencies_of_manifest
...
let selected_package_id = fun (catalog:catalog) ~selected_versions ~package_name ->
  ...
  | Some entry ->
      Ok (package_id_of_local_entry entry)
  | None -> (
      match HashMap.get selected_versions package_name with
      | Some version ->
          registry_package_id_of_solution catalog ~package_name version
      | None ->
          Error (Error.Unexpected {
  ...
  let rec loop acc = function
    ...
    | (dep:Riot_model.Package.dependency) :: rest ->
        if dep.source.builtin then
          ...
          let* target_name_opt =
            dependency_target_name catalog ~declared_from ~required_by:None dep
          in
          match target_name_opt with
          | None ->
              loop acc rest
...
let lock_registry_package = fun (catalog:catalog) ~selected_versions ~package_name version ->
  ...
  | _ ->
      ...
      let* dependencies_result =
        provider_dependencies_of_registry_package catalog ~package_name version
      in
...
let lock_deps = fun ?(emit = no_emit) ~mode ~registry ~existing_lock ~workspace () ->
  let started = Time.Instant.now () in
  let ctx = { emit; mode; registry; existing_lock; workspace } in
  ...
  | Ok (Pubgrub.Solver.Success solution) ->
      ...
      let rec lock_external_local_packages acc = function
        ...
        | package_name :: rest -> (
            ...
            | None ->
                lock_external_local_packages acc rest
          )
      in
      ...
      let rec lock_registry_packages acc = function
        ...
        | package_name :: rest -> (
            ...
            | None ->
                lock_registry_packages acc rest
          )
      in
      ...
      let packages = merge_lock_packages
        (workspace_lock_packages @ external_local_packages @ registry_packages @ preserved)
      in
