...
let is_apple_junk_entry = fun name -> String.starts_with ~prefix:"._" name || String.equal name ".DS_Store" || String.equal name "__MACOSX"
...
let message = function
  ...
  | MissingPublishDescription { package } -> "package '" ^ package ^ "' is missing [package].description"
  | MissingPublishLicense { package } -> "package '" ^ package ^ "' is missing [package].license"
  | PackageNotPublic { package } -> "package '" ^ package ^ "' must set [package].public = true to be published"
  | MissingManifest { package_root } -> "package root '" ^ Path.to_string package_root ^ "' is missing riot.toml at archive root"
  | RuntimeDependencyNotPublishable { package; dependency; reason = `PathOnly path } -> "runtime dependency '" ^ dependency ^ "' in package '" ^ package ^ "' is path-only and cannot be published (path = " ^ Path.to_string path ^ ")"
  | RuntimeDependencyNotPublishable { package; dependency; reason = `WorkspaceOnly } -> "runtime dependency '" ^ dependency ^ "' in package '" ^ package ^ "' is workspace-only and cannot be published"
  | RuntimeDependencyNotPublishable { package; dependency; reason = `MissingVersionOrPath } -> "runtime dependency '" ^ dependency ^ "' in package '" ^ package ^ "' must declare a version or publishable source"
  | RuntimeDependencyRegistryLookupFailed { package; dependency; registry; error } -> "failed to verify runtime dependency '" ^ dependency ^ "' for package '" ^ package ^ "' in registry '" ^ registry ^ "': " ^ error
  | RuntimeDependencyNotFoundInRegistry { package; dependency; registry } -> "runtime dependency '" ^ dependency ^ "' for package '" ^ package ^ "' was not found in registry '" ^ registry ^ "'"
  | SymlinkNotAllowed { path } -> "publish artifacts do not support symlinks: " ^ Path.to_string path
  | UnsupportedEntry { path; kind } -> "publish artifacts only support regular files and directories; found " ^ kind ^ " at " ^ Path.to_string path
  | DirectoryReadFailed { path; error } -> "failed to read directory '" ^ Path.to_string path ^ "': " ^ IO.error_message error
  | MetadataReadFailed { path; error } -> "failed to read metadata for '" ^ Path.to_string path ^ "': " ^ metadata_error_message error
  | ArtifactReadFailed { path; error } -> "failed to read publish artifact '" ^ Path.to_string path ^ "': " ^ IO.error_message error
  | TarCommandFailed { command; status; stdout; stderr } ->
      ...
      in
      "failed to create publish artifact with '" ^ command ^ "' (exit " ^ Int.to_string status ^ "): " ^ detail
  | TarCommandSpawnFailed { command; error } -> "failed to spawn publish artifact command '" ^ command ^ "': " ^ command_error_message error
  ...
  | CyclicWorkspacePublishOrder { cycle } -> "workspace publish order contains a cycle: " ^ String.concat " -> " cycle

let should_skip_entry = fun path ->
  let name = Path.basename path in is_apple_junk_entry name || List.any excluded_entry_names ~fn:(String.equal name)
...
let publisher_error_of_walker_error = fun ~package_root (err: Fs.Walker.error) ->
  ...
  | Some path -> (
    ...
    else MetadataReadFailed { path; error = MetadataIoError err.cause }
  )
  ...
  | Some version -> (
    ...
    | Some _ -> (
      ...
      | Some _ -> (
        ...
        | Some false | None -> Error (PackageNotPublic { package = package_name })
      )
    )
  )

let validate_runtime_dependency = fun ~(package:Riot_model.Package.t) (dep: Riot_model.Package.dependency) ->
  let package_name = Riot_model.Package_name.to_string package.name in
  ...
  let rec loop = function
    ...
    | dep :: rest -> (
      if Riot_model.Package.is_builtin_dependency dep then
        ...
        if List.any publishing_workspace_packages ~fn:(
          fun package_name -> Riot_model.Package_name.equal dep.name package_name
        ) then
          loop rest
        else
          if Option.is_some dep.source.source_locator then
            loop rest
          else
            let dependency_name = Riot_model.Package_name.to_string dep.name in
            ...
            | Ok None -> Error (RuntimeDependencyNotFoundInRegistry { package = package_name; dependency = dependency_name; registry = Pkgs_ml.Registry.name registry })
            | Ok (Some _) -> loop rest
    )
  in
  loop package.dependencies
...
let published_version_exists = fun ~registry ~package_name ~version ->
  ...
  | Ok None -> Ok false
  | Ok (Some document) ->
      let version = Std.Version.to_string version in Ok (List.any document.releases ~fn:(
        fun (release: Pkgs_ml.Sparse_index.release) -> String.equal release.version version
      ))
...
let collect_relative_files = fun ~package_root ->
  ...
  in
  let iter = walker |> Fs.Walker.filter_entry ~f:(
    fun (entry: Fs.Walker.FileItem.t) -> not (should_skip_entry (Fs.Walker.FileItem.path entry))
  ...
  let rec loop acc iter =
    ...
    | Some (Ok (entry: Fs.Walker.FileItem.t)), iter' -> (
      ...
      | Other -> Error (UnsupportedEntry { path; kind = walker_kind_to_string (Fs.Walker.FileItem.kind entry) })
    )
  in
  loop [] iter

let publish_artifact_path = fun ~target_dir_root ~(package:Riot_model.Package.t) ~version ->
  let package_name = Riot_model.Package_name.to_string package.name in Path.(target_dir_root / Path.v "release" / Path.v "publish" / Path.v package_name / Path.v (Std.Version.to_string version) / Path.v "package.tar.gz")
...
let create_artifact = fun ~target_dir_root ~(package:Riot_model.Package.t) ~version ->
  ...
  | Error _ as err -> err
  | Ok relative_files ->
      let relative_files = List.sort relative_files ~compare:(
      ...
      if not (List.any relative_files ~fn:(Path.equal (Path.v "riot.toml"))) then
        ...
        let artifact_path = publish_artifact_path ~target_dir_root ~package ~version in create_archive ~package_root:package.path ~artifact_path ~relative_files
...
let assoc_package = fun packages name -> List.find packages ~fn:(
  fun (pkg_name, _pkg) -> Riot_model.Package_name.equal pkg_name name
) |> Option.map ~fn:(
  fun (_, package) -> package
)
...
let workspace_runtime_dependency_names = fun ~workspace_packages (pkg: Riot_model.Package.t) ->
  let is_workspace_dependency (dep: Riot_model.Package.dependency) =
    if dep.source.workspace then
      ...
      | None -> false
  in
  pkg.dependencies |> List.filter ~fn:is_workspace_dependency |> List.map ~fn:(
    fun (dep: Riot_model.Package.dependency) -> dep.name
  )

let workspace_publish_order = fun ~packages ->
  let workspace_packages = packages |> List.filter ~fn:Riot_model.Package.is_workspace_member |> List.map ~fn:(
    fun (pkg: Riot_model.Package.t) -> (pkg.name, pkg)
  ) in
  let rec visit ~visiting ~visited ordered name =
    if List.any visited ~fn:(Riot_model.Package_name.equal name) then
      Ok (visited, ordered)
    else
      if List.any visiting ~fn:(Riot_model.Package_name.equal name) then
        Error (CyclicWorkspacePublishOrder { cycle = List.reverse (name :: visiting) |> List.map ~fn:Riot_model.Package_name.to_string })
      else
        ...
        | Some pkg ->
            ...
            let rec visit_dependencies visited ordered = function
              | [] ->
                  let visited = name :: visited in Ok (visited, pkg :: ordered)
              | dep_name :: rest -> (
  ...
  in
  walk_names [] [] (List.map workspace_packages ~fn:(
    fun (package_name, _) -> package_name
  ))
