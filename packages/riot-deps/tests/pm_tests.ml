...
let package_name = fun name -> Package_name.from_string name |> Result.expect ~msg:("Expected valid package name: " ^ name)
...
let source = fun ?(workspace = false) ?(builtin = false) ?path ?source_locator ?ref_ ?version () -> Package.{
  workspace;
  builtin;
  path;
  source_locator;
...
let make_package = fun ?(dependencies = []) ?(build_dependencies = []) ?(dev_dependencies = []) ~name ~path () ->
  ...
  let publish = Package.{
    ...
    is_public = Some true
  } in Package.make ~name ~path ~relative_path:path ~dependencies ~dev_dependencies ~build_dependencies ~sources:(make_sources ()) ~publish ()

let make_registry_cache = fun () -> Pkgs_ml.Registry_cache.create ~riot_home:(Path.v "/Users/example/.riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to initialize"

let make_registry_cache_at = fun riot_home -> Pkgs_ml.Registry_cache.create ~riot_home ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to initialize"

let make_release = fun ?(dependencies = []) ?(yanked = false) ~version () -> Pkgs_ml.Sparse_index.{
  version;
  ...
  dependencies;
  yanked;
  yanked_at = if yanked then
    Some "2026-04-06T10:00:00.000Z"
  else None;
  yanked_by_github_login = if yanked then
    Some "leostera"
  else None
}

let make_registry_dependency = fun name -> Pkgs_ml.Sparse_index.{
  name;
...
let make_registry_document = fun ?(releases = []) ~name ~latest () -> Pkgs_ml.Sparse_index.{
  schema_version = 1;
  name;
  latest;
...
let make_registry = fun packages -> Pkgs_ml.Registry.in_memory ~cache:(make_registry_cache ()) ~packages ()

let make_registry_with_releases = fun ~packages ~releases -> Pkgs_ml.Registry.in_memory ~cache:(make_registry_cache ()) ~packages ~releases ()
...
let workspace_package = fun ~workspace_root (pkg: Package.t) ->
  ...
  | Ok relative_path -> Package.make ~name:pkg.name ~path:pkg.path ~relative_path ~dependencies:pkg.dependencies ~dev_dependencies:pkg.dev_dependencies ~build_dependencies:pkg.build_dependencies ~foreign_dependencies:pkg.foreign_dependencies ~binaries:pkg.binaries ?library:pkg.library ~sources:pkg.sources ~compiler:pkg.compiler ~commands:pkg.commands ~fix_providers:pkg.fix_providers ~publish:pkg.publish ()
  | Error _ -> pkg
...
let make_workspace_manifest = fun ?(workspace_root = Path.v "/workspace") ?(dependencies = []) ?(dev_dependencies = []) ?(build_dependencies = []) packages ->
  let packages = List.map packages ~fn:(workspace_package ~workspace_root) in Riot_model.Workspace_manifest.make_realized ~root:workspace_root ~packages ~dependencies ~dev_dependencies ~build_dependencies ()

let make_workspace = fun ?(workspace_root = Path.v "/workspace") ?(dependencies = []) ?(dev_dependencies = []) ?(build_dependencies = []) packages ->
  let packages = List.map packages ~fn:(workspace_package ~workspace_root) in Riot_model.Workspace.make_realized ~root:workspace_root ~packages ~dependencies ~dev_dependencies ~build_dependencies ()

let run_lock_deps = fun ?emit ?(registry = make_registry []) ?(workspace_root = Path.v "/workspace") ~mode ~existing_lock packages ->
  let workspace = make_workspace_manifest ~workspace_root packages in Riot_deps.Dep_solver.lock_deps ?emit ~mode ~registry ~existing_lock ~workspace ()

let ensure_lock = fun ?emit ?(registry = make_registry []) ?(workspace_root = Path.v "/workspace") packages ->
  let workspace = make_workspace_manifest ~workspace_root packages in Riot_deps.ensure_lock ?emit ~workspace_manager:(workspace_manager ()) ~mode:Riot_deps.Dep_solver.Refresh ~registry ~workspace ()

let collect_event_names = fun fn ->
  let names = ref [] in
  let emit event = names := Riot_model.Event.name event :: !names in
  match fn emit with
...
let list_tar_entries = fun artifact_path ->
  ...
  | Ok output when not (Int.equal output.status 0) -> Error ("failed to list artifact entries: " ^ output.stderr)
  | Ok output -> Ok (String.split output.stdout ~by:"\n" |> List.filter ~fn:(
    fun line -> not (String.equal line "")
  ))

let run_git = fun ~cwd args ->
  let command = Command.make "env" ~args:(
    [
      "-u";
      "GIT_DIR";
      "-u";
...
let prepare_local_git_repo = fun ~root ?subdir ~package_name ?(version = "0.0.1") () ->
  ...
  write_file manifest_path ("[package]\n" ^ "name = \"" ^ package_name ^ "\"\n" ^ "version = \"" ^ version ^ "\"\n" ^ "description = \"" ^ package_name ^ "\"\n" ^ "license = \"Apache-2.0\"\n" ^ "public = true\n");
  write_file source_path "let answer = 42\n";
  run_git_steps ~cwd:repo_root
    [
      [ "init"; "-b"; "main" ];
...
let make_fetch_recorder = fun ?(post_handler = fun _uri ~headers:_ ~body:_ -> Error "unexpected POST") get_handler ->
  let requests = ref [] in
  let record ~method_ uri ~headers ~body = requests := {
    method_;
    url = Net.Uri.to_string uri;
    headers;
    body
  } :: !requests in
  let fetch = Pkgs_ml.Registry.make_fetch ~get:(
    fun uri ->
...
let test_publisher_rejects_path_only_runtime_dependencies = fun _ctx ->
  let package = make_package ~name:"demo" ~path:(Path.v "/workspace/packages/demo") ~dependencies:[ dependency "std" (source ~path:(Path.v "../std") ()) ] () in
  ...
  | Error (Riot_deps.Publisher.RuntimeDependencyNotPublishable { dependency; reason = `PathOnly path; _ }) ->
      if String.equal dependency "std" && Path.equal path (Path.v "../std") then
        Ok ()
      else Error "unexpected path-only runtime dependency payload"
...
let test_publisher_allows_path_with_version_runtime_dependencies = fun _ctx ->
  let package = make_package ~name:"demo" ~path:(Path.v "/workspace/packages/demo") ~dependencies:[ dependency "std" (source ~path:(Path.v "../std") ~version:Std.Version.any ()) ] () in
  match Riot_deps.Publisher.validate_runtime_dependencies ~package with
  | Ok () -> Ok ()
  | Error err -> Error ("expected path+version runtime dependency to be publishable: " ^ Riot_deps.Publisher.message err)
...
|};
        ...
        match Riot_deps.Publisher.create_artifact ~target_dir_root:root ~package ~version:(Std.Version.make ~major:0 ~minor:1 ~patch:0 ()) with
        ...
        | Ok artifact -> (
          ...
          | Ok entries ->
              ...
              if entries = expected then
                Ok ()
              else Error ("unexpected publish artifact entries: " ^ String.concat "," entries)
        )
    )
...
|};
        ...
        | Error (Riot_deps.Publisher.SymlinkNotAllowed { path }) ->
            if Path.equal path link then
              Ok ()
            else Error "unexpected symlink rejection path"
...
|};
        ...
        let package = make_package ~name:"demo" ~path:package_root () in
        let fetch, requests =
          make_fetch_recorder ~post_handler:(
            fun _uri ~headers:_ ~body:_ -> Ok { Pkgs_ml.Registry.status_code = 200; body = {|{
  "artifact_sha256": "deadbeef",
  "package_name": "demo",
    ...
    "source": false
        ...
        | Ok prepared -> (
          ...
          | Ok published -> (
            ...
            | [ request ] ->
                let has_header name value = List.any request.headers ~fn:(
                  fun (header_name, header_value) -> String.equal header_name name && String.equal header_value value
                ) in
                if String.equal request.method_ "POST" && String.equal request.url "https://api.pkgs.ml/v1/publish" && has_header "authorization" "Bearer root-secret" && has_header "content-type" "application/gzip" && String.equal published.package_name "demo" && String.equal published.package_version "0.1.0" then
                  Ok ()
                else Error "unexpected publish request or response"
...
|};
        ...
        let fetch, _requests =
          make_fetch_recorder ~post_handler:(
            fun _uri ~headers:_ ~body:_ -> Ok { Pkgs_ml.Registry.status_code = 404; body = {|{
  ...
  "message": "package `demo` was not found in registry `pkgs.ml`"
        ...
        | Ok prepared -> (
          ...
          | Error (Riot_deps.Publisher.RegistryPublishFailed { locator; error }) ->
              if String.equal locator "github.com/example/demo" && String.equal error "package `demo` was not found in registry `pkgs.ml`" then
                Ok ()
              else Error "unexpected registry publish error payload"
  ...
  with_tempdir "riot_deps_publish_missing_artifact"
    (
      fun root ->
        ...
        let fetch, _requests =
          make_fetch_recorder ~post_handler:(
            fun _uri ~headers:_ ~body:_ -> Error "publish should not be called when the artifact is missing"
          )
            (
              fun uri -> Error ("unexpected GET " ^ Net.Uri.to_string uri)
            )
        ...
        | Error (Riot_deps.Publisher.ArtifactReadFailed { path; _ }) ->
            if Path.equal path artifact_path then
              Ok ()
            else Error "expected missing artifact error to preserve artifact path"
        | Error err -> Error ("unexpected publish error: " ^ Riot_deps.Publisher.message err)
...
let test_publisher_workspace_publish_order_uses_runtime_local_dependencies = fun _ctx ->
  ...
  let util = make_package ~name:"util" ~path:(Path.v "packages/util") ~dependencies:[ dependency "core" (source ~workspace:true ()) ] () in
  let app = make_package ~name:"app" ~path:(Path.v "packages/app") ~dependencies:[ dependency "util" (source ~path:(Path.v "../util") ~version:Std.Version.any ()) ] () in
  ...
  | Ok ordered ->
      if List.map ordered ~fn:(
        fun (pkg: Package.t) -> pkg.name
      ) = [ package_name "core"; package_name "util"; package_name "app" ] then
        Ok ()
      else Error "unexpected workspace publish order"

let test_publisher_workspace_publish_order_ignores_dev_and_build_dependencies = fun _ctx ->
  let core = make_package ~name:"core" ~path:(Path.v "packages/core") () in
  let app = make_package ~name:"app" ~path:(Path.v "packages/app") ~build_dependencies:[ dependency "core" (source ~workspace:true ()) ] ~dev_dependencies:[ dependency "core" (source ~workspace:true ()) ] () in
  match Riot_deps.Publisher.workspace_publish_order ~packages:[ app; core ] with
  | Error err -> Error ("expected publish order to succeed: " ^ Riot_deps.Publisher.message err)
  | Ok ordered ->
      if List.map ordered ~fn:(
        fun (pkg: Package.t) -> pkg.name
      ) = [ package_name "app"; package_name "core" ] then
        Ok ()
      else Error "expected workspace publish order to ignore dev/build edges"

let test_publisher_workspace_publish_order_reports_cycles = fun _ctx ->
  let a = make_package ~name:"a" ~path:(Path.v "packages/a") ~dependencies:[ dependency "b" (source ~workspace:true ()) ] () in
  let b = make_package ~name:"b" ~path:(Path.v "packages/b") ~dependencies:[ dependency "a" (source ~workspace:true ()) ] () in
...
let test_publisher_validate_registry_dependencies_skips_workspace_publish_set = fun _ctx ->
  let core = make_package ~name:"core" ~path:(Path.v "packages/core") () in
  let app = make_package ~name:"app" ~path:(Path.v "packages/app") ~dependencies:[ dependency "core" (source ~path:(Path.v "../core") ~version:Std.Version.any ()) ] () in
  let registry = make_registry [] in
  match Riot_deps.Publisher.validate_registry_dependencies ~registry ~publishing_workspace_packages:[ core.name; app.name ] ~package:app with
  | Ok () -> Ok ()
  | Error err -> Error ("expected workspace publish set to skip registry lookup: " ^ Riot_deps.Publisher.message err)
...
|};
        ...
        match run_git_steps ~cwd:root
          [
            ...
            [
              ...
              "add";
              "origin";
              "https://github.com/example/riot.git";
            ];
            [ "add"; "." ];
            [
              "-c";
        ...
        | Ok _ -> (
          ...
          | Error err -> Error ("expected git provenance discovery to succeed: " ^ Riot_deps.Git_provenance.message err)
          | Ok provenance ->
              if String.equal provenance.locator "github.com/example/riot/packages/demo" && Path.equal provenance.repository_root canonical_root && provenance.package_subdir = Some (Path.v "packages/demo") && String.equal provenance.origin_url "https://github.com/example/riot.git" && String.length provenance.selector = 40 then
                Ok ()
              else Error "unexpected nested git provenance"
        )
        | Error err -> Error err
    )
...
|};
        ...
        | Ok _ -> (
          ...
          | Error err -> Error ("expected git provenance discovery to succeed: " ^ Riot_deps.Git_provenance.message err)
          | Ok provenance ->
              if String.equal provenance.locator "github.com/example/demo" && Path.equal provenance.repository_root canonical_root && provenance.package_subdir = None && String.equal provenance.origin_url "git@github.com:example/demo.git" && String.length provenance.selector = 40 then
                Ok ()
              else Error "unexpected root git provenance"
        )
        | Error err -> Error err
    )
      ...
      fun root ->
        ...
        | Error (Riot_deps.Git_provenance.NotGitRepository { path }) ->
            ...
            if Path.equal path canonical_root then
              Ok ()
            else Error "expected not-git-repository error to preserve package root"
        | Error err -> Error ("expected not-git-repository error, got: " ^ Riot_deps.Git_provenance.message err)
...
|};
  ...
  "materialization": {
    ...
    "source": false
            ...
            in
            let registry = Pkgs_ml.Registry.filesystem ~fetch (make_registry_cache ()) in
            match Riot_deps.Publisher.publish ~registry ~target_dir_root:root ~publishing_workspace_packages:[] ~package ~api_token:"root-secret" with
            | Error err -> Error ("expected publish to succeed: " ^ Riot_deps.Publisher.message err)
            | Ok published -> (
              if String.equal published.package_name "demo" && List.any !requests ~fn:(
                fun request -> String.equal request.method_ "POST" && String.length request.url > 0
              ) then
                Ok ()
              else Error "unexpected publish request discovered from git provenance"
            )
          )
        )
        | Error err -> Error err
...
|};
        ...
        | Ok _ -> (
          ...
          | Ok selector -> (
            ...
            match Riot_deps.Publisher.prepare_publish ~registry ~target_dir_root:root ~publishing_workspace_packages:[] ~package with
            | Error err -> Error ("expected prepare_publish to succeed: " ^ Riot_deps.Publisher.message err)
            | Ok prepared ->
                if has_name "demo" prepared.package.name && String.equal prepared.locator "github.com/example/riot/packages/demo" && String.equal prepared.selector selector && String.length (Path.to_string prepared.artifact_path) > 0 then
                  Ok ()
                else Error "unexpected prepared publish payload"
          )
        )
        | Error err -> Error err
    )
...
let test_lock_deps_projects_workspace_packages = fun _ctx ->
  ...
  let app_pkg = make_package ~name:"app" ~path:(Path.v "/workspace/packages/app") ~dependencies:[ dependency "std" (source ~workspace:true ()) ] ~build_dependencies:[ dependency "std" (source ~workspace:true ()) ] () in
  ...
  | Ok lockfile ->
      ...
      let std_lock = List.get lockfile.packages ~at:1 |> Option.expect ~msg:"expected std lock package" in
      if lockfile.format_version = 1 && has_name "app" app_lock.id.name && has_name "std" std_lock.id.name && app_lock.provenance = Riot_model.Lockfile.Workspace && std_lock.provenance = Riot_model.Lockfile.Workspace && app_lock.root = Some (Path.v "packages/app") && std_lock.root = Some (Path.v "packages/std") && List.length app_lock.dependencies = 1 && List.length app_lock.build_dependencies = 1 && ((List.head app_lock.dependencies |> Option.expect ~msg:"expected app dependency").package.name |> has_name "std") then
        Ok ()
      else Error "expected workspace packages to be projected into the lockfile"
...
version = "1.2.3"
|};
        let app_pkg = make_package ~name:"app" ~path:Path.(workspace_root / Path.v "packages/app") ~dependencies:[ dependency "foo" (source ~path:(Path.v "../../vendor/foo") ()) ] () in
        ...
        | Ok lockfile -> (
          let app_lock = List.find lockfile.packages ~fn:(
            fun (pkg: Lockfile.package) -> has_name "app" pkg.id.name
          ) in
          let foo_lock = List.find lockfile.packages ~fn:(
            fun (pkg: Lockfile.package) -> has_name "foo" pkg.id.name
          ) in
          match app_lock, foo_lock with
          | Some app_lock, Some foo_lock ->
              if List.length lockfile.packages = 2 && ((List.head app_lock.dependencies |> Option.expect ~msg:"expected app dependency").package.name |> has_name "foo") && foo_lock.root = Some (Path.v "vendor/foo") && foo_lock.provenance = Riot_model.Lockfile.Path (Path.v "../../vendor/foo") then
                Ok ()
              else Error "expected path dependency to resolve to an exact local lock package"
...
|};
        ...
        | Ok lockfile -> (
          ...
          let bar_lock = List.find lockfile.packages ~fn:(
            fun (pkg: Lockfile.package) -> has_name "bar" pkg.id.name
          ) in
          match foo_lock, bar_lock with
          | Some foo_lock, Some bar_lock ->
              if List.length lockfile.packages = 3 && ((List.head foo_lock.dependencies |> Option.expect ~msg:"expected foo dependency").package.name |> has_name "bar") && bar_lock.root = Some (Path.v "vendor/bar") && bar_lock.provenance = Riot_model.Lockfile.Path (Path.v "../bar") then
                Ok ()
              else Error "expected nested path dependency roots to resolve from the declaring package"
...
let test_lock_deps_falls_back_to_registry_when_path_dependency_is_missing = fun _ctx ->
  with_tempdir "riot_deps_missing_path_registry_fallback"
    (
      fun workspace_root ->
        let app_pkg = make_package ~name:"app" ~path:Path.(workspace_root / Path.v "packages/app") ~dependencies:[ { name = package_name "std"; source = source ~path:(Path.v "../../vendor/std") ~version:Std.Version.any () } ] () in
        let registry = Pkgs_ml.Registry.in_memory ~cache:(make_registry_cache_at Path.(workspace_root / Path.v ".riot")) ~packages:[ make_registry_document ~name:"std" ~latest:"0.2.0" ~releases:[ make_release ~version:"0.2.0" () ] () ] ~releases:[ make_release_source ~package_name:"std" ~version:"0.2.0" {|
[package]
name = "std"
version = "0.2.0"
|} ] () in
        match run_lock_deps ~registry ~workspace_root ~mode:Refresh ~existing_lock:None [ app_pkg ] with
        | Error err -> Error ("expected missing path+version dependency to fall back to registry: " ^ pm_error_message err)
        | Ok lockfile -> (
          let app_lock = List.find lockfile.packages ~fn:(
            fun (pkg: Lockfile.package) -> has_name "app" pkg.id.name
          ) in
          let registry_std = List.find lockfile.packages ~fn:(
            fun (pkg: Riot_model.Lockfile.package) -> has_name "std" pkg.id.name && pkg.id.registry = Some "pkgs.ml"
          ) in
          let local_std = List.find lockfile.packages ~fn:(
            fun (pkg: Lockfile.package) -> has_name "std" pkg.id.name && pkg.id.registry = None
          ) in
          match app_lock, registry_std, local_std with
          | Some app_lock, Some registry_std, None ->
              if List.length lockfile.packages = 2 && (List.head app_lock.dependencies |> Option.expect ~msg:"expected app dependency").package = registry_std.id then
                Ok ()
              else Error "expected app to lock against the registry release when the local path is absent"
...
let test_lock_deps_collapses_workspace_path_dependencies = fun _ctx ->
  let std_pkg = make_package ~name:"std" ~path:(Path.v "/workspace/packages/std") () in
  let app_pkg = make_package ~name:"app" ~path:(Path.v "/workspace/packages/app") ~dependencies:[ dependency "std" (source ~path:(Path.v "../std") ()) ] () in
  match run_lock_deps ~mode:Refresh ~existing_lock:None [ app_pkg; std_pkg ] with
  ...
  | Ok lockfile -> (
    ...
    let std_lock = List.find lockfile.packages ~fn:(
      fun (pkg: Lockfile.package) -> has_name "std" pkg.id.name
    ) in
    ...
    | Some app_lock, Some std_lock ->
        if List.length lockfile.packages = 2 && app_lock.dependencies = [
          ...
          Ok ()
        else Error "expected workspace path dependency to reuse the workspace lock package"
...
let test_lock_deps_resolves_registry_dependencies_to_exact_versions = fun _ctx ->
  with_tempdir "riot_deps_registry_exact_versions"
    (
      fun workspace_root ->
        let app_pkg = make_package ~name:"app" ~path:Path.(workspace_root / Path.v "packages/app") ~dependencies:[ dependency "std" (source ~version:Std.Version.any ()) ] () in
        let registry = Pkgs_ml.Registry.in_memory ~cache:(make_registry_cache_at Path.(workspace_root / Path.v ".riot")) ~packages:[ make_registry_document ~name:"std" ~latest:"0.2.0" ~releases:[ make_release ~version:"0.2.0" ~dependencies:[ make_registry_dependency "kernel" ] () ] (); make_registry_document ~name:"kernel" ~latest:"1.0.0" ~releases:[ make_release ~version:"1.0.0" () ] () ] ~releases:[ make_release_source ~package_name:"std" ~version:"0.2.0" {|
[package]
name = "std"
version = "0.2.0"
...
|} ] () in
        ...
        | Ok lockfile -> (
          ...
          let std_lock = List.find lockfile.packages ~fn:(
            fun (pkg: Riot_model.Lockfile.package) -> has_name "std" pkg.id.name && pkg.id.version = Some "0.2.0"
          ) in
          let kernel_lock = List.find lockfile.packages ~fn:(
            fun (pkg: Riot_model.Lockfile.package) -> has_name "kernel" pkg.id.name && pkg.id.version = Some "1.0.0"
          ) in
          ...
          | Some app_lock, Some std_lock, Some kernel_lock ->
              ...
              if List.length lockfile.packages = 3 && (
                ...
                Ok ()
              else Error "expected registry dependency to resolve to exact external lock packages"
...
let test_lock_deps_reports_missing_registry_package_with_required_by = fun _ctx ->
  ...
  let app_pkg = make_package ~name:"app" ~path:app_root ~dependencies:[ dependency "std" (source ~version:Std.Version.any ()) ] () in
  ...
  | Error (Riot_deps.Error.PackageNotFound { package; registry; required_by = Some required_by }) ->
      if String.equal package "std" && String.equal registry "pkgs.ml" && String.equal required_by.package "app" && required_by.path = Some app_root then
        Ok ()
      else Error "expected missing registry package error to include the requiring workspace package"
...
let test_lock_deps_reports_missing_registry_version_with_available_versions = fun _ctx ->
  ...
  let app_pkg = make_package ~name:"app" ~path:app_root ~dependencies:[ dependency "minttea" (source ~version:requirement ()) ] () in
  let registry = make_registry [ make_registry_document ~name:"minttea" ~latest:"0.2.5" ~releases:[ make_release ~version:"0.1.0" (); make_release ~version:"0.2.5" () ] () ] in
  ...
  | Error (Riot_deps.Error.RegistryVersionNotFound { package; registry; requirement; available_versions; required_by = Some required_by }) ->
      if String.equal package "minttea" && String.equal registry "pkgs.ml" && String.equal requirement "0.3" && available_versions = [ "0.1.0"; "0.2.5" ] && String.equal required_by.package "app" && required_by.path = Some app_root then
        Ok ()
      else Error "expected registry version error to include requirement, available versions, and required-by"
...
let test_lock_deps_supports_major_minor_prefix_requirements = fun _ctx ->
  ...
  let app_pkg = make_package ~name:"app" ~path:(Path.v "/workspace/packages/app") ~dependencies:[ dependency "minttea" (source ~version:requirement ()) ] () in
  let registry = make_registry [ make_registry_document ~name:"minttea" ~latest:"1.0.0" ~releases:[
  ...
  | Ok lockfile -> (
    match List.find lockfile.packages ~fn:(
      fun (pkg: Riot_model.Lockfile.package) -> has_name "minttea" pkg.id.name
    ) with
    | Some pkg when pkg.id.version = Some "0.2.3" -> Ok ()
    | Some pkg -> Error ("expected 0.2 requirement to pick highest 0.2.x release, got " ^ Option.unwrap_or ~default:"<none>" pkg.id.version)
...
let test_lock_deps_supports_major_prefix_requirements = fun _ctx ->
  ...
  | Ok lockfile -> (
    ...
    | Some pkg -> Error ("expected 0 requirement to pick highest 0.x.y release, got " ^ Option.unwrap_or ~default:"<none>" pkg.id.version)
    | None -> Error "expected minttea to be locked"
  )

let test_lock_deps_prefers_workspace_packages_over_registry_for_matching_names = fun _ctx ->
  let std_pkg = make_package ~name:"std" ~path:(Path.v "/workspace/packages/std") () in
  let app_pkg = make_package ~name:"app" ~path:(Path.v "/workspace/packages/app") ~dependencies:[ dependency "std" (source ~version:Std.Version.any ()) ] () in
  match run_lock_deps ~registry:(make_registry []) ~mode:Refresh ~existing_lock:None [ app_pkg; std_pkg ] with
  | Error err -> Error ("expected workspace package to satisfy matching registry requirement locally: " ^ pm_error_message err)
  | Ok lockfile -> (
    let app_lock = List.find lockfile.packages ~fn:(
      fun (pkg: Lockfile.package) -> has_name "app" pkg.id.name
    ) in
    ...
    | Some app_lock, Some std_lock ->
        if List.length lockfile.packages = 2 && app_lock.dependencies = [
          ...
          Ok ()
        else Error "expected matching workspace packages to win over registry resolution"
    | _ -> Error "expected app and std workspace packages to appear in the lockfile"
  )
...
std = "*"
|};
        let app_pkg = make_package ~name:"app" ~path:Path.(workspace_root / Path.v "packages/app") ~dependencies:[ dependency "std" (source ~path:(Path.v "../../vendor/std") ()); dependency "model" (source ~path:(Path.v "../../vendor/model") ()) ] () in
        let registry = make_registry [ make_registry_document ~name:"std" ~latest:"9.9.9" ~releases:[ make_release ~version:"9.9.9" () ] () ] in
        match run_lock_deps ~registry ~workspace_root ~mode:Refresh ~existing_lock:None [ app_pkg ] with
        | Error err -> Error ("expected local path package to beat registry dependency: " ^ pm_error_message err)
        | Ok lockfile -> (
          let local_std = List.find lockfile.packages ~fn:(
            fun (pkg: Riot_model.Lockfile.package) -> has_name "std" pkg.id.name && pkg.id.registry = None
          ) in
          let registry_std = List.find lockfile.packages ~fn:(
            fun (pkg: Riot_model.Lockfile.package) -> has_name "std" pkg.id.name && pkg.id.registry = Some "pkgs.ml"
          ) in
          let model_lock = List.find lockfile.packages ~fn:(
            fun (pkg: Lockfile.package) -> has_name "model" pkg.id.name
          ) in
          match local_std, registry_std, model_lock with
          | Some local_std, None, Some model_lock ->
              if List.length model_lock.dependencies = 1 && (List.head model_lock.dependencies |> Option.expect ~msg:"expected model dependency").package = local_std.id then
                Ok ()
              else Error "expected version-only dependency to reuse the available local path package"
...
let test_lock_deps_ignores_builtin_dependencies = fun _ctx ->
  let app_pkg = make_package ~name:"app" ~path:(Path.v "/workspace/packages/app") ~dependencies:[ dependency "stdlib" (source ~builtin:true ~version:Std.Version.any ()) ] () in
  match run_lock_deps ~registry:(make_registry []) ~mode:Refresh ~existing_lock:None [ app_pkg ] with
...
let test_lock_deps_ignores_builtin_registry_release_dependencies = fun _ctx ->
  with_tempdir "riot_deps_builtin_registry_deps"
    (
      fun workspace_root ->
        let app_pkg = make_package ~name:"app" ~path:Path.(workspace_root / Path.v "packages/app") ~dependencies:[ dependency "std" (source ~version:Std.Version.any ()) ] () in
        let registry = Pkgs_ml.Registry.in_memory ~cache:(make_registry_cache_at Path.(workspace_root / Path.v ".riot")) ~packages:[ make_registry_document ~name:"std" ~latest:"0.1.0" ~releases:[ make_release ~version:"0.1.0" ~dependencies:[ make_registry_dependency "stdlib"; make_registry_dependency "unix" ] () ] () ] ~releases:[ make_release_source ~package_name:"std" ~version:"0.1.0" {|
[package]
name = "std"
version = "0.1.0"
...
|} ] () in
        ...
        | Ok lockfile -> (
          let std_lock = List.find lockfile.packages ~fn:(
            fun (pkg: Riot_model.Lockfile.package) -> has_name "std" pkg.id.name && pkg.id.version = Some "0.1.0"
          ) in
          match std_lock with
...
let test_lock_deps_handles_cyclic_registry_dependencies = fun _ctx ->
  with_tempdir "riot_deps_cyclic_registry"
    (
      fun workspace_root ->
        let app_pkg = make_package ~name:"app" ~path:Path.(workspace_root / Path.v "packages/app") ~dependencies:[ dependency "foo" (source ~version:Std.Version.any ()) ] () in
        let registry = Pkgs_ml.Registry.in_memory ~cache:(make_registry_cache_at Path.(workspace_root / Path.v ".riot")) ~packages:[ make_registry_document ~name:"foo" ~latest:"1.0.0" ~releases:[ make_release ~version:"1.0.0" ~dependencies:[ make_registry_dependency "bar" ] () ] (); make_registry_document ~name:"bar" ~latest:"2.0.0" ~releases:[ make_release ~version:"2.0.0" ~dependencies:[ make_registry_dependency "foo" ] () ] () ] ~releases:[ make_release_source ~package_name:"foo" ~version:"1.0.0" {|
[package]
name = "foo"
version = "1.0.0"
...
|} ] () in
        ...
        | Ok lockfile -> (
          let foo_lock = List.find lockfile.packages ~fn:(
            fun (pkg: Riot_model.Lockfile.package) -> has_name "foo" pkg.id.name && pkg.id.version = Some "1.0.0"
          ) in
          let bar_lock = List.find lockfile.packages ~fn:(
            fun (pkg: Riot_model.Lockfile.package) -> has_name "bar" pkg.id.name && pkg.id.version = Some "2.0.0"
          ) in
          match foo_lock, bar_lock with
          | Some foo_lock, Some bar_lock ->
              let foo_dependency =
              ...
              if List.length lockfile.packages = 3 && (
                ...
                Ok ()
              else Error "expected cyclic registry dependencies to terminate with exact cross-links"
...
std = { path = "../std" }
|};
        let app_pkg = make_package ~name:"app" ~path:Path.(workspace_root / Path.v "packages/app") ~dependencies:[ dependency "std" (source ~path:(Path.v "../../vendor/std") ()) ] () in
        match run_lock_deps ~workspace_root ~mode:Refresh ~existing_lock:None [ app_pkg ] with
        ...
        | Ok lockfile -> (
          ...
          let fixme_lock = List.find lockfile.packages ~fn:(
            fun (pkg: Lockfile.package) -> has_name "fixme" pkg.id.name
          ) in
          match std_lock, fixme_lock with
          | Some std_lock, Some fixme_lock ->
              if List.length lockfile.packages = 3 && List.length std_lock.build_dependencies = 1 && ((List.head std_lock.build_dependencies |> Option.expect ~msg:"expected std build dependency").package.name |> has_name "fixme") && List.length fixme_lock.dependencies = 1 && ((List.head fixme_lock.dependencies |> Option.expect ~msg:"expected fixme dependency").package.name |> has_name "std") then
                Ok ()
              else Error "expected local path dependency cycle to reuse in-flight lock nodes"
  ...
  with_tempdir "riot_deps_refresh_preserve_registry"
    (
      fun workspace_root ->
        ...
        let app_pkg = make_package ~name:"app" ~path:Path.(workspace_root / Path.v "packages/app") ~dependencies:[ dependency "std" (source ~version:requirement ()) ] () in
        let existing_lock = Riot_model.Lockfile.{
          format_version = 1;
        ...
        let registry = Pkgs_ml.Registry.in_memory ~cache:(make_registry_cache_at Path.(workspace_root / Path.v ".riot")) ~packages:[ make_registry_document ~name:"std" ~latest:"0.2.0" ~releases:[ make_release ~version:"0.1.0" (); make_release ~version:"0.2.0" () ] () ] ~releases:[ make_release_source ~package_name:"std" ~version:"0.1.0" {|
[package]
name = "std"
version = "0.1.0"
...
version = "0.2.0"
|} ] () in
        match run_lock_deps ~registry ~workspace_root ~mode:Refresh ~existing_lock:(Some existing_lock) [ app_pkg ] with
        | Error err -> Error ("expected refresh lock to preserve registry version: " ^ pm_error_message err)
        | Ok lockfile ->
            let app_lock = List.head lockfile.packages |> Option.expect ~msg:"expected app lock package" in
            if List.length lockfile.packages = 2 && (List.head app_lock.dependencies |> Option.expect ~msg:"expected app dependency").package.version = Some "0.1.0" && ((List.get lockfile.packages ~at:1 |> Option.expect ~msg:"expected std lock package").id.version = Some "0.1.0") then
              Ok ()
            else Error "expected refresh to preserve existing locked registry selections"
    )
...
let test_lock_refresh_discards_stale_external_nodes = fun _ctx ->
  ...
  | Error err -> Error ("expected refresh lock to discard stale existing nodes: " ^ pm_error_message err)
  | Ok lockfile ->
      if List.length lockfile.packages = 1 && ((List.head lockfile.packages |> Option.expect ~msg:"expected app lock package").id.name |> has_name "app") then
        Ok ()
      else Error "expected refresh to discard stale external lock nodes"
...
let test_lock_refresh_discards_removed_workspace_packages = fun _ctx ->
  ...
  match run_lock_deps ~mode:Refresh ~existing_lock:(Some existing_lock) [ app_pkg ] with
  | Error err -> Error ("expected refresh lock to discard removed workspace packages: " ^ pm_error_message err)
  | Ok lockfile ->
      if List.length lockfile.packages = 1 && ((List.head lockfile.packages |> Option.expect ~msg:"expected app lock package").id.name |> has_name "app") then
        Ok ()
      else Error "expected refresh to discard removed workspace packages"
...
let test_unlock_discards_existing_external_nodes = fun _ctx ->
  ...
  | Ok lockfile ->
      if List.length lockfile.packages = 1 && ((List.head lockfile.packages |> Option.expect ~msg:"expected app lock package").id.name |> has_name "app") then
        Ok ()
      else Error "expected unlock to discard preserved external lock nodes"
...
mime = "*"
|};
        let registry = Pkgs_ml.Registry.in_memory ~cache:(make_registry_cache_at Path.(workspace_root / Path.v ".riot")) ~packages:[ make_registry_document ~name:"std" ~latest:"0.2.0" ~releases:[ make_release ~version:"0.1.0" (); make_release ~version:"0.2.0" () ] (); make_registry_document ~name:"mime" ~latest:"0.2.0" ~releases:[ make_release ~version:"0.1.0" (); make_release ~version:"0.2.0" () ] () ] ~releases:[
          make_release_source ~package_name:"std" ~version:"0.1.0" {|
[package]
name = "std"
version = "0.1.0"
|};
...
|};
        ...
        let lock_id name version = Riot_model.Lockfile.{
          registry = Some "pkgs.ml";
          name = package_name name;
          version = Some version;
          sha256 = Some "deadbeef"
        } in
        let app_id = Riot_model.Lockfile.{
          registry = None;
          name = package_name "app";
          version = None;
          sha256 = None
          ...
          packages = [
            {
              ...
              root = Some (Path.v "packages/app");
              provenance = Workspace;
              dependencies = [ { name = package_name "std"; package = lock_id "std" "0.1.0" }; { name = package_name "mime"; package = lock_id "mime" "0.1.0" } ];
              build_dependencies = [];
              dev_dependencies = []
            };
            {
        ...
        } in Riot_deps.Lockfile_store.write ~workspace_root existing_lock |> Result.map_err ~fn:Riot_deps.Lockfile_store.error_message |> Result.and_then ~fn:(
          fun () ->
            let workspace_manager = Riot_model.Workspace_manager.create () in Riot_model.Workspace_manager.scan workspace_manager workspace_root |> Result.map_err ~fn:(
              fun err -> "expected workspace scan to succeed: " ^ Riot_model.Workspace_manager.scan_error_message err
            ) |> Result.and_then ~fn:(
              fun (workspace, load_errors) ->
                if not (List.is_empty load_errors) then
                  Error "expected workspace scan to have no load errors"
                else
                  let events = ref [] in Riot_deps.update ~on_event:(
                    fun event -> events := event :: !events
                  ) ~registry ~workspace_manager ~workspace ~request:Riot_deps.{ packages = [ package_name "std" ] } () |> Result.map_err ~fn:Riot_deps.package_error_message |> Result.and_then ~fn:(
                    fun () -> Riot_deps.Lockfile_store.read ~workspace_root |> Result.map_err ~fn:Riot_deps.Lockfile_store.error_message |> Result.and_then ~fn:(
                      function
                      ...
                      | Some (lockfile: Lockfile.t) ->
                          ...
                          let mime_lock = List.find lockfile.packages ~fn:(
                            fun (pkg: Lockfile.package) -> has_name "mime" pkg.id.name
                          ) in
                          let updated_std = List.any !events ~fn:(
                            function
                            | Riot_model.Event.PackageVersionUpdated { package; from_version; to_version } -> has_name "std" package && String.equal from_version "0.1.0" && String.equal to_version "0.2.0"
                            | _ -> false
                          ) in
                          match std_lock, mime_lock with
                          | Some std_lock, Some mime_lock when std_lock.id.version = Some "0.2.0" && mime_lock.id.version = Some "0.1.0" && updated_std -> Ok ()
  ...
  with_tempdir "riot_deps_missing_lock"
    (
      fun workspace_root ->
        ...
        match Riot_deps.Lock_refresh.needs_refresh ~workspace_manager ~workspace_root ~manifest_paths:[ manifest_path ] ~lockfile:None with
        | Ok true -> Ok ()
  ...
  with_tempdir "riot_deps_matching_dep_hash"
    (
      fun workspace_root ->
        ...
        Fs.write "[workspace]\nmembers = []\n" manifest_path |> Result.expect ~msg:"expected manifest write to succeed";
        let workspace_manager = workspace_manager () in
        let dependency_hash = Riot_deps.Lock_refresh.dependency_hash ~workspace_manager ~workspace_root ~manifest_paths:[ manifest_path ] |> Result.expect ~msg:"expected dependency hash to compute" in
        let lockfile = Riot_model.Lockfile.{ format_version = 1; dependency_hash; packages = [] } in
        match Riot_deps.Lock_refresh.needs_refresh ~workspace_manager ~workspace_root ~manifest_paths:[ manifest_path ] ~lockfile:(Some lockfile) with
        | Ok false -> Ok ()
  ...
  with_tempdir "riot_deps_bad_dep_section"
    (
      fun workspace_root ->
        ...
        let workspace_manager = workspace_manager () in
        match Riot_deps.Lock_refresh.dependency_hash ~workspace_manager ~workspace_root ~manifest_paths:[ manifest_path ] with
        | Error (Riot_deps.Lock_refresh.DependencySectionMustBeTable { manifest_path = path; section }) ->
            if Path.equal path manifest_path && String.equal section "dependencies" then
              Ok ()
            else Error "expected dependency-section error to preserve path and section"
        | Error err -> Error ("unexpected lock refresh error: " ^ Riot_deps.Lock_refresh.error_message err)
...
let test_lockfile_store_roundtrips = fun _ctx ->
  with_tempdir "riot_deps_lockfile_store"
    (
      fun workspace_root ->
        let lockfile = Riot_model.Lockfile.{
          format_version = 1;
        ...
        | Error err -> Error ("expected lockfile write to succeed: " ^ Riot_deps.Lockfile_store.error_message err)
        | Ok () -> (
          match Riot_deps.Lockfile_store.read ~workspace_root with
          | Error err -> Error ("expected lockfile read to succeed: " ^ Riot_deps.Lockfile_store.error_message err)
          | Ok None -> Error "expected written lockfile to exist"
          | Ok (Some reloaded) ->
              if reloaded.format_version = 1 && String.equal reloaded.dependency_hash "deadbeef" && List.length reloaded.packages = 1 && ((List.head reloaded.packages |> Option.expect ~msg:"expected reloaded app package").id.name |> has_name "app") then
                Ok ()
              else Error "expected lockfile store roundtrip to preserve package data"
        )
    )
      ...
      fun workspace_root ->
        ...
        | Error (Riot_deps.Lockfile_store.TomlParseFailed { path; _ }) ->
            if Path.equal path lock_path then
              Ok ()
            else Error "expected TOML parse error to preserve lockfile path"
        | Error (Riot_deps.Lockfile_store.DecodeFailed { path; _ }) ->
            if Path.equal path lock_path then
              Ok ()
            else Error "expected lockfile decode error to preserve lockfile path"
...
version = "0.0.1"
|};
        let workspace = make_workspace_manifest ~workspace_root ~dependencies:[ dependency "std" (source ~version:Std.Version.any ()) ] [ make_package ~name:"app" ~path:app_root () ] in
        let workspace_manager = Riot_model.Workspace_manager.create () in
        match Riot_deps.remove ~workspace_manager ~workspace ~cwd:app_root ~request:Riot_deps.{ selection = Current; scope = Runtime; dependencies = [ package_name "std" ] } () with
        ...
        | Error (Riot_deps.DependencyNotFoundInSection { path; section; dependency }) ->
            if Path.equal path Path.(app_root / Path.v "riot.toml") && String.equal section "dependencies" && String.equal dependency "std" then
              Ok ()
            else Error "unexpected dependency-not-found payload for inherited dependency removal"
...
version = "0.0.1"
|};
        let workspace = make_workspace_manifest ~workspace_root [ make_package ~name:"app" ~path:app_root ~dependencies:[ dependency "std" (source ~version:Std.Version.any ()) ] () ] in
        let workspace_manager = Riot_model.Workspace_manager.create () in
        match Riot_deps.remove ~workspace_manager ~workspace ~cwd:app_root ~request:Riot_deps.{ selection = Current; scope = Runtime; dependencies = [ package_name "std" ] } () with
        ...
        | Error (Riot_deps.ManifestUpdateFailed (Riot_deps.Manifest_edit.DependencySectionMustBeTable { path; section })) ->
            if Path.equal path app_manifest && String.equal section "dependencies" then
              Ok ()
            else Error "unexpected manifest update payload for non-table dependency section"
        | Error err -> Error ("unexpected remove error: " ^ Riot_deps.package_error_message err)
    )
...
|};
        let workspace_manager = Riot_model.Workspace_manager.create () in Riot_model.Workspace_manager.scan workspace_manager workspace_root |> Result.map_err ~fn:(
          ...
          fun (workspace, load_errors) ->
            if not (List.is_empty load_errors) then
              Error "expected workspace scan to have no load errors"
            else Riot_deps.remove ~workspace_manager ~workspace ~cwd:app_root ~request:Riot_deps.{ selection = Current; scope = Runtime; dependencies = [ package_name "widgets"; package_name "gadgets" ] } () |> Result.map_err ~fn:Riot_deps.package_error_message |> Result.and_then ~fn:(
              fun () -> Fs.read_to_string Path.(app_root / Path.v "riot.toml") |> Result.map_err ~fn:IO.error_message |> Result.and_then ~fn:(
                fun manifest_source -> Riot_deps.Lockfile_store.read ~workspace_root |> Result.map_err ~fn:(
                  fun err -> "expected lockfile read to succeed: " ^ Riot_deps.Lockfile_store.error_message err
                ) |> Result.and_then ~fn:(
                  fun maybe_lockfile ->
                    match maybe_lockfile with
                    | None -> Error "expected remove to rewrite riot.lock"
                    | Some (lockfile: Lockfile.t) ->
                        if (not (String.contains manifest_source "widgets")) && (not (String.contains manifest_source "gadgets")) && List.all lockfile.packages ~fn:(
                          fun (pkg: Lockfile.package) -> not (has_name "widgets" pkg.id.name) && not (has_name "gadgets" pkg.id.name)
                        ) then
                          Ok ()
                        else Error "expected multi-remove to remove both deps from manifest and lock"
                )
              )
            )
        )
...
|};
        let workspace_manager = Riot_model.Workspace_manager.create () in Riot_model.Workspace_manager.scan workspace_manager workspace_root |> Result.map_err ~fn:(
          ...
          fun (workspace, load_errors) ->
            if not (List.is_empty load_errors) then
              Error "expected workspace scan to have no load errors"
            else Riot_deps.add ~workspace_manager ~workspace ~cwd:app_root ~request:Riot_deps.{ selection = Current; scope = Runtime; dependencies = [ "../lib" ] } () |> Result.map_err ~fn:Riot_deps.package_error_message |> Result.and_then ~fn:(
              fun () -> Fs.read_to_string Path.(app_root / Path.v "riot.toml") |> Result.map_err ~fn:IO.error_message |> Result.and_then ~fn:(
                fun manifest_source -> Riot_deps.Lockfile_store.read ~workspace_root |> Result.map_err ~fn:(
                  fun err -> "expected lockfile read to succeed: " ^ Riot_deps.Lockfile_store.error_message err
                ) |> Result.and_then ~fn:(
                  fun maybe_lockfile ->
                    ...
                    | Some (lockfile: Riot_model.Lockfile.t) ->
                        ...
                        if String.contains manifest_source "widgets = { path = \"../lib\" }" && Option.is_some app_lock && List.any lockfile.packages ~fn:(
                          fun (pkg: Lockfile.package) -> has_name "widgets" pkg.id.name
                        ) then
                          Ok ()
                        else Error "expected path add to write discovered package name and refresh riot.lock"
                )
              )
            )
        )
...
|};
        let workspace_manager = Riot_model.Workspace_manager.create () in Riot_model.Workspace_manager.scan workspace_manager workspace_root |> Result.map_err ~fn:(
          ...
          fun (workspace, load_errors) ->
            if not (List.is_empty load_errors) then
              Error "expected workspace scan to have no load errors"
            else Riot_deps.add ~workspace_manager ~workspace ~cwd:app_root ~request:Riot_deps.{ selection = Current; scope = Runtime; dependencies = [ "../widgets"; "../gadgets" ] } () |> Result.map_err ~fn:Riot_deps.package_error_message |> Result.and_then ~fn:(
              fun () -> Fs.read_to_string Path.(app_root / Path.v "riot.toml") |> Result.map_err ~fn:IO.error_message |> Result.and_then ~fn:(
                fun manifest_source -> Riot_deps.Lockfile_store.read ~workspace_root |> Result.map_err ~fn:(
                  fun err -> "expected lockfile read to succeed: " ^ Riot_deps.Lockfile_store.error_message err
                ) |> Result.and_then ~fn:(
                  fun maybe_lockfile ->
                    ...
                    | None -> Error "expected add to rewrite riot.lock"
                    | Some (lockfile: Lockfile.t) ->
                        if String.contains manifest_source "widgets = { path = \"../widgets\" }" && String.contains manifest_source "gadgets = { path = \"../gadgets\" }" && List.any lockfile.packages ~fn:(
                          fun (pkg: Lockfile.package) -> has_name "widgets" pkg.id.name
                        ) && List.any lockfile.packages ~fn:(
                          fun (pkg: Lockfile.package) -> has_name "gadgets" pkg.id.name
                        ) then
                          Ok ()
                        else Error "expected multi-add to write both discovered package names and refresh riot.lock"
                )
              )
            )
        )
        ...
        let workspace_manager = Riot_model.Workspace_manager.create () in Riot_model.Workspace_manager.scan workspace_manager workspace_root |> Result.map_err ~fn:(
          ...
          fun (workspace, load_errors) ->
            if not (List.is_empty load_errors) then
              ...
              match Riot_deps.add ~workspace_manager ~workspace ~cwd:app_root ~request:Riot_deps.{ selection = Current; scope = Runtime; dependencies = [ "../missing" ] } () with
              | Error (Riot_deps.PathDependencyLoadFailed { dependency; path; error = Riot_deps.PathDependencyManifestReadFailed _ }) ->
                  if String.equal dependency "../missing" && Path.equal path Path.(workspace_root / Path.v "packages/missing") then
                    Ok ()
                  else Error "unexpected missing path dependency payload"
...
let test_git_dependency_parse_spec_normalizes_github_source = fun _ctx ->
  ...
  | Ok { source_locator; ref_ } ->
      if String.equal source_locator "github.com/riot-tests/widgets-add" && ref_ = Some "main" then
        Ok ()
      else Error "expected github source spec to normalize into locator + ref"
  | Error err -> Error ("expected git dependency spec to parse: " ^ Riot_deps.Git_dependency.message err)
...
let test_git_dependency_parse_spec_reports_multiple_ref_suffixes = fun _ctx ->
  ...
  | Error (Riot_deps.Git_dependency.InvalidSourceSpec { source; reason = Riot_deps.Git_dependency.TooManyRefSuffixes }) ->
      if String.equal source "github.com/riot/tests#main#extra" then
        Ok ()
      else Error "expected invalid source spec to preserve raw source"
  | Error err -> Error ("expected multiple ref suffix error, got: " ^ Riot_deps.Git_dependency.message err)
...
let test_git_dependency_parse_source_locator_reports_invalid_shape = fun _ctx ->
  ...
  | Error (Riot_deps.Git_dependency.InvalidSourceSpec { source; reason = Riot_deps.Git_dependency.InvalidLocatorShape }) ->
      if String.equal source "github.com/riot" then
        Ok ()
      else Error "expected invalid locator shape to preserve raw source"
  | Error err -> Error ("expected invalid locator shape error, got: " ^ Riot_deps.Git_dependency.message err)
  ...
  with_tempdir "riot_deps_git_checkout"
    (
      fun root ->
        ...
        let checkout = Path.(root / Path.v "checkout") in prepare_local_git_repo ~root:origin ~package_name:"widgets" () |> Result.and_then ~fn:(
          fun _repo_root -> Riot_deps.Git_dependency.sync_checkout ~repo_dir:checkout ~remote_url:(Path.to_string origin) ~ref_:"main" () |> Result.map_err ~fn:Riot_deps.Git_dependency.message |> Result.and_then ~fn:(
            fun _ -> Fs.read_to_string Path.(checkout / Path.v "riot.toml") |> Result.map_err ~fn:IO.error_message |> Result.and_then ~fn:(
              fun manifest_source ->
                if String.contains manifest_source "name = \"widgets\"" then
                  Ok ()
                else Error "expected git dependency checkout to clone the local repository"
            )
          )
        )
    )
...
public = true
|};
              run_git_steps ~cwd:origin [ [ "add"; "." ]; [ "commit"; "-m"; "update" ] ] |> Result.and_then ~fn:(
                fun _ -> Riot_deps.Git_dependency.sync_checkout ~update:false ~repo_dir:checkout ~remote_url:(Path.to_string origin) ~ref_:"main" () |> Result.map_err ~fn:Riot_deps.Git_dependency.message |> Result.and_then ~fn:(
                  fun _ -> Fs.read_to_string Path.(checkout / Path.v "riot.toml") |> Result.map_err ~fn:IO.error_message |> Result.and_then ~fn:(
                    fun manifest_source ->
                      if String.contains manifest_source "name = \"widgets\"" then
                        Ok ()
                      else
                        if String.contains manifest_source "name = \"widgets-next\"" then
                          Error "expected sync_checkout ~update:false to keep the cached checkout without fetching upstream changes"
                        else Error "expected cached checkout to preserve the original manifest contents"
                  )
                )
              )
          )
        ...
        let workspace_manager = Riot_model.Workspace_manager.create () in Riot_model.Workspace_manager.scan workspace_manager workspace_root |> Result.map_err ~fn:(
          ...
          fun (workspace, load_errors) ->
            if not (List.is_empty load_errors) then
              ...
              match Riot_deps.add ~workspace_manager ~workspace ~cwd:app_root ~request:Riot_deps.{ selection = Current; scope = Runtime; dependencies = [ "https://gitlab.com/leostera/widgets" ] } () with
              ...
              | Error (Riot_deps.DependencySpecInvalid { dependency; error = Riot_deps.SourceDependencySpecError (Riot_deps.Git_dependency.UnsupportedSourceHost { host; _ }) }) ->
                  if String.equal dependency "https://gitlab.com/leostera/widgets" && String.equal host "gitlab.com" then
                    Ok ()
                  else Error "unexpected unsupported source dependency payload"
              | Error err -> Error ("unexpected add error: " ^ Riot_deps.package_error_message err)
        )
    )
...
let test_package_error_message_renders_typed_source_dependency_errors = fun _ctx ->
  ...
  let cases =
    [
      Riot_deps.SourceDependencyMaterializationFailed (Riot_deps.Git_dependency.PackageRootMissing { path = Path.v "/cache/widgets" }), "materialized source dependency is missing package root";
      Riot_deps.SourceDependencyManifestReadFailed (IO.Unknown_error "read boom"), "read boom";
      Riot_deps.SourceDependencyTomlParseFailed (Data.Toml.Parse_error { position = 7; context = "package"; reason = "bad toml" }), "bad toml";
      Riot_deps.SourceDependencyManifestDecodeFailed Package.ManifestMustBeTable, "package manifest must be a table";
    ]
  in
  let rec loop = function
    ...
    | (error, expected) :: rest ->
        ...
        if String.contains message "failed to load source dependency 'github.com/riot-tests/widgets'" && String.contains message expected then
          loop rest
        else Error ("expected source dependency error message to include '" ^ expected ^ "', got: " ^ message)
  in
  loop cases

let test_package_error_message_renders_typed_workspace_reload_errors = fun _ctx ->
  let workspace_root = Path.v "/workspace" in
  let scan_message = Riot_deps.package_error_message (Riot_deps.WorkspaceReloadFailed { workspace_root; error = Riot_model.Workspace_manager.NoWorkspaceRootFound }) in
  let load_message = Riot_deps.package_error_message (Riot_deps.WorkspaceReloadHadErrors { workspace_root; errors = [ Riot_model.Workspace_manager.PackageTomlParseFailed { package = "app"; path = "/workspace/packages/app" } ] }) in
  if String.contains scan_message "no workspace root found" && String.contains load_message "package 'app': failed to parse riot.toml" then
    Ok ()
  else Error "expected typed workspace reload errors to render through package_error_message"

let test_package_error_message_renders_typed_registry_initialization_errors = fun _ctx ->
  let message = Riot_deps.package_error_message (Riot_deps.RegistryInitializationFailed { registry = "pkgs.ml"; error = Riot_deps.RegistryFilesystemInitializationFailed Pkgs_ml.Registry_cache.HomeDirectoryUnavailable }) in
  if String.contains message "failed to initialize registry 'pkgs.ml'" && String.contains message "failed to determine home directory for pkgs.ml cache" then
    Ok ()
  else Error "expected typed registry initialization errors to render through package_error_message"

let test_package_error_message_renders_typed_registry_operation_errors = fun _ctx ->
  let lookup_message = Riot_deps.package_error_message (Riot_deps.RegistryLookupFailed { package = "widgets"; registry = "pkgs.ml"; error = Riot_deps.RegistryPackageDocumentReadFailed "lookup failed" }) in
  let search_message = Riot_deps.package_error_message (Riot_deps.RegistrySearchFailed { query = "widg"; registry = "pkgs.ml"; error = Riot_deps.RegistrySearchRequestFailed "search failed" }) in
  ...
  in
  if String.contains lookup_message "lookup failed" && String.contains search_message "search failed" && String.contains materialization_message "package manifest must be a table" then
    Ok ()
  else Error "expected typed registry operation errors to render through package_error_message"

let test_package_error_message_lists_search_suggestions = fun _ctx ->
  let message = Riot_deps.package_error_message (Riot_deps.RegistryPackageNotFound { package = "kernl"; registry = "pkgs.ml"; suggestions = [ { Riot_deps.package = "kernel"; latest_version = "0.0.1"; description = Some "Core primitives" }; { Riot_deps.package = "kernel-tools"; latest_version = "0.1.0"; description = None } ] }) in
  if String.contains message "package 'kernl' was not found in registry 'pkgs.ml'" && String.contains message "Did you mean:" && String.contains message "kernel@0.0.1 - Core primitives" && String.contains message "kernel-tools@0.1.0" then
    Ok ()
  else Error ("unexpected suggestion message:\n" ^ message)

let test_search_returns_registry_results = fun _ctx ->
  let release = { (make_release ~version:"0.0.1" ()) with description = Some "Bootstrap build tool for the Riot toolchain" } in
  let registry = make_registry [ make_registry_document ~name:"miniriot" ~latest:"0.0.1" ~releases:[ release ] (); make_registry_document ~name:"jsonrpc" ~latest:"0.1.0" ~releases:[ make_release ~version:"0.1.0" () ] () ] in
  ...
  | Ok [ result ] ->
      if String.equal result.package "miniriot" && String.equal result.latest_version "0.0.1" && Option.equal result.description (Some "Bootstrap build tool for the Riot toolchain") ~fn:String.equal then
        Ok ()
      else Error "unexpected search result payload"
  ...
  with_tempdir "riot_deps_ensure_lock_missing"
    (
      fun workspace_root ->
        ...
        let app_pkg = make_package ~name:"app" ~path:Path.(workspace_root / Path.v "packages/app") ~dependencies:[ dependency "std" (source ~workspace:true ()) ] () in
        match collect_event_names
          (
            fun emit -> ensure_lock ~emit ~registry:(make_registry []) ~workspace_root [ app_pkg; std_pkg ]
          ) with
        | Error err -> Error ("expected ensure_lock to refresh missing lock: " ^ pm_error_message err)
        | Ok ((lockfile, resolved), event_names) ->
            let lock_path = Riot_model.Riot_dirs.package_lock_path ~workspace_root in
            if List.length lockfile.packages = 2 && List.length resolved = 2 && List.contains event_names ~value:"riot.pm.resolution.started" && List.contains event_names ~value:"riot.pm.resolution.refreshing_lock" && List.contains event_names ~value:"riot.pm.lockfile.write.started" && List.contains event_names ~value:"riot.pm.lockfile.write.finished" && List.contains event_names ~value:"riot.pm.resolution.finished" && Result.unwrap_or ~default:false (Fs.exists lock_path) then
              Ok ()
            else Error "expected ensure_lock to write a fresh lockfile and emit PM lifecycle events"
    )
  ...
  with_tempdir "riot_deps_ensure_lock_existing"
    (
      fun workspace_root ->
        ...
        let std_pkg = make_package ~name:"std" ~path:Path.(workspace_root / Path.v "packages/std") () in
        let app_pkg = make_package ~name:"app" ~path:Path.(workspace_root / Path.v "packages/app") ~dependencies:[ dependency "std" (source ~workspace:true ()) ] () in
        let existing_lock = run_lock_deps ~workspace_root ~mode:Refresh ~existing_lock:None [ app_pkg; std_pkg ] |> Result.expect ~msg:"expected workspace lock projection to succeed" in
        let workspace_manager = workspace_manager () in
        let dependency_hash = Riot_deps.Lock_refresh.dependency_hash ~workspace_manager ~workspace_root ~manifest_paths:[ manifest_path; Path.(workspace_root / Path.v "packages/std/riot.toml"); Path.(workspace_root / Path.v "packages/app/riot.toml") ] |> Result.expect ~msg:"expected dependency hash to compute" in
        ...
        | Ok ((lockfile, resolved), event_names) ->
            if List.length lockfile.packages = 2 && List.length resolved = 2 && List.contains event_names ~value:"riot.pm.resolution.using_existing_lock" && not (List.contains event_names ~value:"riot.pm.lockfile.write.started") && not (List.contains event_names ~value:"riot.pm.resolution.finished") then
              Ok ()
            else Error "expected ensure_lock to reuse a fresh existing lock without rewriting it"
    )
  ...
  with_tempdir "riot_deps_ensure_lock_materializes"
    (
      fun workspace_root ->
        ...
        let requirement = Std.Version.parse_requirement "*" |> Result.expect ~msg:"expected requirement to parse" in
        let app_pkg = make_package ~name:"app" ~path:Path.(workspace_root / Path.v "packages/app") ~dependencies:[ dependency "std" (source ~version:requirement ()) ] () in
        let registry_cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(workspace_root / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to initialize" in
        let registry = Pkgs_ml.Registry.in_memory ~cache:registry_cache ~packages:[ make_registry_document ~name:"std" ~latest:"0.2.0" ~releases:[ make_release ~version:"0.2.0" () ] () ] ~releases:[
          {
        ...
        match collect_event_names
          (
            fun emit -> ensure_lock ~emit ~registry ~workspace_root [ app_pkg ]
          ) with
        | Error err -> Error ("expected ensure_lock to materialize registry packages: " ^ pm_error_message err)
        | Ok ((_, resolved), event_names) ->
            let manifest_path =
              Pkgs_ml.Registry_cache.package_src_dir registry_cache ~package_name:"std" ~version:"0.2.0" |> fun root -> Path.(root / Path.v "riot.toml")
            in
            if List.length resolved = 2 && Result.unwrap_or ~default:false (Fs.exists manifest_path) && List.contains event_names ~value:"riot.pm.universe.building" && List.contains event_names ~value:"riot.pm.universe.built" && List.contains event_names ~value:"riot.pm.package_metadata.fetch.started" && List.contains event_names ~value:"riot.pm.package_metadata.fetch.finished" && List.contains event_names ~value:"riot.pm.package.locked" && List.contains event_names ~value:"riot.pm.package_materialization.started" && List.contains event_names ~value:"riot.pm.package_materialization.finished" && List.contains event_names ~value:"riot.pm.package_manifest.fetch.started" && List.contains event_names ~value:"riot.pm.package_manifest.fetch.finished" && List.contains event_names ~value:"riot.pm.package_resolved_for_build" then
              Ok ()
            else Error "expected ensure_lock to lazily materialize external package manifests during projection"
    )
  ...
  with_tempdir "riot_deps_ensure_lock_materializes_existing"
    (
      fun workspace_root ->
        ...
        let existing_lock = Riot_deps.Dep_solver.lock_deps ~mode:Riot_deps.Dep_solver.Refresh ~registry ~existing_lock:None ~workspace:(make_workspace_manifest ~workspace_root [ app_pkg ]) () |> Result.expect ~msg:"expected initial lock solve to succeed" in
        let existing_lock = { existing_lock with dependency_hash = Riot_deps.Lock_refresh.dependency_hash ~workspace_manager:(workspace_manager ()) ~workspace_root ~manifest_paths:[ manifest_path; Path.(workspace_root / Path.v "packages/app/riot.toml") ] |> Result.expect ~msg:"expected dependency hash to compute" } in
        Riot_deps.Lockfile_store.write ~workspace_root existing_lock |> Result.expect ~msg:"expected initial lockfile write to succeed";
        let materialized_std_root = Pkgs_ml.Registry_cache.package_src_dir registry_cache ~package_name:"std" ~version:"0.2.0" in
        Fs.remove_dir_all materialized_std_root |> Result.expect ~msg:"expected materialized registry package to be removed";
        match collect_event_names
          (
            fun emit -> ensure_lock ~emit ~registry ~workspace_root [ app_pkg ]
          ) with
        | Error err -> Error ("expected ensure_lock to reuse lock and materialize missing packages: " ^ pm_error_message err)
        | Ok ((_, resolved), event_names) ->
            if List.length resolved = 2 && List.contains event_names ~value:"riot.pm.resolution.using_existing_lock" && not (List.contains event_names ~value:"riot.pm.resolution.refreshing_lock") && Result.unwrap_or ~default:false (Fs.exists Path.(materialized_std_root / Path.v "riot.toml")) && List.contains event_names ~value:"riot.pm.package_materialization.finished" && not (List.contains event_names ~value:"riot.pm.lockfile.write.started") then
              Ok ()
            else Error "expected ensure_lock to reuse the lock while still materializing missing registry packages"
    )
...
let test_ensure_workspace_projects_materialized_registry_packages = fun _ctx ->
  with_tempdir "riot_deps_ensure_workspace"
    (
      fun workspace_root ->
        let workspace_manifest = Path.(workspace_root / Path.v "riot.toml") in
        Fs.write "[workspace]\nmembers = [\"packages/app\"]\n" workspace_manifest |> Result.expect ~msg:"expected workspace manifest to be written";
        write_file Path.(workspace_root / Path.v "packages/app/riot.toml") "[package]\nname = \"app\"\n";
        let registry_cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(workspace_root / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to initialize" in
        let app_pkg = make_package ~name:"app" ~path:Path.(workspace_root / Path.v "packages/app") ~dependencies:[ dependency "std" (source ~version:Std.Version.any ()) ] () in
        let app_pkg = Riot_model.Package.make ~name:app_pkg.name ~path:app_pkg.path ~relative_path:(Path.v "packages/app") ~dependencies:app_pkg.dependencies ~dev_dependencies:app_pkg.dev_dependencies ~build_dependencies:app_pkg.build_dependencies ~foreign_dependencies:app_pkg.foreign_dependencies ~binaries:app_pkg.binaries ?library:app_pkg.library ~sources:app_pkg.sources ~compiler:app_pkg.compiler ~commands:app_pkg.commands ~fix_providers:app_pkg.fix_providers ~publish:app_pkg.publish () in
...
|};
        ...
        | Ok resolved_workspace ->
            let std_pkg = List.find resolved_workspace.packages ~fn:(
              fun (pkg: Riot_model.Package_manifest.t) -> has_name "std" pkg.name
            ) in
            let expected_std_root = Pkgs_ml.Registry_cache.package_src_dir registry_cache ~package_name:"std" ~version:"0.2.0" in
            match std_pkg with
            | Some std_pkg ->
                if List.map resolved_workspace.packages ~fn:(
                  fun (pkg: Package_manifest.t) -> pkg.name
                ) = [ package_name "app"; package_name "std" ] && Path.equal std_pkg.path expected_std_root then
                  Ok ()
                else Error "expected ensure_workspace to return a build-ready workspace with registry packages"
    ...
    (
      fun workspace_root ->
        ...
        let registry = Pkgs_ml.Registry.in_memory ~cache:registry_cache ~packages:[
          make_registry_document ~name:"std" ~latest:"0.2.0" ~releases:[ make_release ~version:"0.2.0" () ] ();
          make_registry_document ~name:"kernel" ~latest:"1.0.0" ~releases:[ make_release ~version:"1.0.0" () ] ();
          make_registry_document ~name:"fixme" ~latest:"0.5.0" ~releases:[ make_release ~version:"0.5.0" () ] ();
          make_registry_document ~name:"propane" ~latest:"0.3.0" ~releases:[ make_release ~version:"0.3.0" () ] ();
        ] ~releases:[
          {
            Pkgs_ml.Registry.package_name = "std";
            version = "0.2.0";
...
|};
        ...
        let existing_lock = Riot_model.Lockfile.{
          format_version = 1;
          dependency_hash = Riot_deps.Lock_refresh.dependency_hash ~workspace_manager:(workspace_manager ()) ~workspace_root ~manifest_paths:[ workspace_manifest; Path.(workspace_root / Path.v "packages/app/riot.toml") ] |> Result.expect ~msg:"expected dependency hash to compute";
          packages = [
            {
              id = {
                registry = None;
        ...
        Riot_deps.Lockfile_store.write ~workspace_root existing_lock |> Result.expect ~msg:"expected initial lockfile write to succeed";
        match Riot_deps.ensure_lock ~workspace_manager:(workspace_manager ()) ~mode:Riot_deps.Dep_solver.Refresh ~registry ~workspace:(make_workspace_manifest ~workspace_root [ app_pkg ]) () with
        | Error err -> Error ("expected ensure_lock to repair broken registry dependency scopes: " ^ pm_error_message err)
        | Ok (lockfile, resolved) ->
            ...
            match std_lock with
            | Some std_lock ->
                if List.length resolved = 5 && List.length lockfile.packages = 5 && List.length std_lock.build_dependencies = 1 && List.length std_lock.dev_dependencies = 1 && ((List.head std_lock.build_dependencies |> Option.expect ~msg:"expected repaired build dependency").name |> has_name "fixme") && ((List.head std_lock.dev_dependencies |> Option.expect ~msg:"expected repaired dev dependency").name |> has_name "propane") then
                  Ok ()
                else Error "expected ensure_lock to rewrite stale registry dependency scopes"
...
|};
        ...
        | Error err -> Error ("expected ensure_workspace to succeed: " ^ pm_error_message err)
        | Ok resolved_workspace -> (
          match List.find resolved_workspace.packages ~fn:(
            fun (pkg: Riot_model.Package_manifest.t) -> has_name "std" pkg.name
          ) with
          | None -> Error "expected ensure_workspace to project std into the workspace"
          | Some std_pkg ->
              let binary_names = std_pkg.declared_binaries |> List.map ~fn:(
                fun (bin: Riot_model.Package.binary) -> bin.name
              ) |> List.sort ~compare:String.compare in
              if binary_names = [ "std-example"; "std-tests" ] then
                Ok ()
              else Error "expected ensure_workspace to preserve declared external binaries across projection"
        )
    )

let test_projection_resolves_workspace_packages = fun _ctx ->
  let std_pkg = make_package ~name:"std" ~path:(Path.v "/workspace/packages/std") () in
  let app_pkg = make_package ~name:"app" ~path:(Path.v "/workspace/packages/app") ~dependencies:[ dependency "std" (source ~workspace:true ()) ] () in
  let lockfile = run_lock_deps ~mode:Riot_deps.Dep_solver.Refresh ~existing_lock:None [ app_pkg; std_pkg ] |> Result.expect ~msg:"expected lock projection to succeed" in
  match Riot_deps.Projection.resolve_packages ~registry:(make_registry []) ~workspace_root:(Path.v "/workspace") ~packages:(manifests_of_packages [ app_pkg; std_pkg ]) ~lockfile () with
  | Error err -> Error ("expected projection to resolve workspace packages: " ^ pm_error_message err)
  | Ok resolved ->
      let app = List.head resolved |> Option.expect ~msg:"expected resolved app package" in
      if List.length resolved = 2 && has_name "app" app.id.name && List.length app.runtime_resolved = 1 && ((List.head app.runtime_resolved |> Option.expect ~msg:"expected resolved runtime dependency").resolved_id.name |> has_name "std") then
        Ok ()
      else Error "expected projection to preserve resolved runtime dependency ids"
...
|} fixme_manifest_path |> Result.expect ~msg:"expected fixme manifest to be written";
        ...
        match collect_event_names
          (
            fun emit -> Riot_deps.Projection.resolve_packages ~emit ~registry ~workspace_root ~packages:(manifests_of_packages [ app_pkg ]) ~lockfile ()
          ) with
        | Error err -> Error ("expected projection to load external manifests: " ^ pm_error_message err)
        | Ok (resolved, event_names) ->
            let std_resolved = List.find resolved ~fn:(
              fun (pkg: Riot_model.Package.resolved) -> has_name "std" pkg.id.name && pkg.id.version = Some "0.2.0"
            ) in
            let kernel_resolved = List.find resolved ~fn:(
              fun (pkg: Riot_model.Package.resolved) -> has_name "kernel" pkg.id.name && pkg.id.version = Some "1.0.0"
            ) in
            let fixme_resolved = List.find resolved ~fn:(
              fun (pkg: Riot_model.Package.resolved) -> has_name "fixme" pkg.id.name && pkg.id.version = Some "0.5.0"
            ) in
            match std_resolved, kernel_resolved, fixme_resolved with
            | Some std_resolved, Some kernel_resolved, Some fixme_resolved ->
                if List.length resolved = 4 && List.contains event_names ~value:"riot.pm.package_manifest.fetch.started" && List.contains event_names ~value:"riot.pm.package_manifest.fetch.finished" && List.contains event_names ~value:"riot.pm.package_resolved_for_build" && Path.to_string std_resolved.materialized_root = Path.to_string std_root && List.length std_resolved.runtime_resolved = 1 && List.length std_resolved.build_resolved = 1 && ((List.head std_resolved.runtime_resolved |> Option.expect ~msg:"expected resolved std runtime dependency").resolved_id.name |> has_name "kernel") && ((List.head std_resolved.build_resolved |> Option.expect ~msg:"expected resolved std build dependency").resolved_id.name |> has_name "fixme") && Path.to_string kernel_resolved.materialized_root = Path.to_string kernel_root && Path.to_string fixme_resolved.materialized_root = Path.to_string fixme_root then
                  Ok ()
                else Error "expected projection to include external lockfile packages"
...
|};
        ...
        match run_lock_deps ~registry ~workspace_root ~mode:Refresh ~existing_lock:None [ app_pkg ] with
        | Error err -> Error ("expected lock solve to preserve registry dependency scopes: " ^ pm_error_message err)
        | Ok lockfile ->
            let std_lock = List.find lockfile.packages ~fn:(
              fun (pkg: Riot_model.Lockfile.package) -> has_name "std" pkg.id.name && pkg.id.version = Some "0.2.0"
            ) in
            match std_lock with
            | Some std_lock ->
                if List.length lockfile.packages = 5 && List.length std_lock.dependencies = 1 && List.length std_lock.build_dependencies = 1 && List.length std_lock.dev_dependencies = 1 && ((List.head std_lock.dependencies |> Option.expect ~msg:"expected std runtime dependency").name |> has_name "kernel") && ((List.head std_lock.build_dependencies |> Option.expect ~msg:"expected std build dependency").name |> has_name "fixme") && ((List.head std_lock.dev_dependencies |> Option.expect ~msg:"expected std dev dependency").name |> has_name "propane") then
                  Ok ()
                else Error "expected registry lock package to preserve runtime, build, and dev dependencies"
...
|} std_manifest_path |> Result.expect ~msg:"expected invalid std manifest to be written";
        ...
        let registry_cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(workspace_root / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to initialize" in
        let registry = Pkgs_ml.Registry.in_memory ~cache:registry_cache ~packages:[] () in
        match Riot_deps.Projection.resolve_packages ~registry ~workspace_root ~packages:(manifests_of_packages [ app_pkg ]) ~lockfile () with
        | Ok _ -> Error "expected invalid external manifest to fail projection"
        | Error err ->
            let message = pm_error_message err in
            if String.contains message "must be a string or table" || String.contains message "failed to decode package manifest" then
              Ok ()
            else Error ("unexpected projection error: " ^ pm_error_message err)
    )
...
let test_projection_fails_when_lockfile_is_missing_package = fun _ctx ->
  ...
  match Riot_deps.Projection.resolve_packages ~registry:(make_registry []) ~workspace_root:(Path.v "/workspace") ~packages:(manifests_of_packages [ app_pkg ]) ~lockfile () with
  ...
  | Error err ->
      if String.contains (pm_error_message err) "lockfile is missing package 'app'" then
        Ok ()
      else Error ("unexpected error: " ^ pm_error_message err)
...
let test_git_dependency_parse_source_locator_accepts_github_shorthand = fun _ctx ->
  ...
  | Ok locator ->
      if String.equal locator.host "github.com" && String.equal locator.owner "leostera" && String.equal locator.repo "riot" && Option.map locator.subdir ~fn:Path.to_string = Some "packages/riot-cli" && Riot_deps.Git_dependency.looks_like_remote_spec "leostera/riot/packages/riot-cli" then
        Ok ()
      else Error "unexpected shorthand source locator decode"
...
let test_load_registry_workspace_materializes_release = fun _ctx ->
  with_tempdir "riot_deps_load_registry_workspace"
    (
      fun root ->
        let registry_cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(root / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to initialize" in
        let version = "0.1.0" in
        let registry = Pkgs_ml.Registry.in_memory ~cache:registry_cache ~packages:[ make_registry_document ~name:"demo" ~latest:version ~releases:[ make_release ~version () ] () ] ~releases:[
          {
...
|};
        ...
        | Ok loaded ->
            if has_name "demo" loaded.package_name && Int.equal (List.length loaded.workspace.packages) 1 && Path.equal loaded.workspace.root (Pkgs_ml.Registry_cache.package_src_dir registry_cache ~package_name:"demo" ~version) then
              Ok ()
            else Error "unexpected loaded registry workspace"
        | Error err -> Error ("expected registry workspace load to succeed: " ^ Riot_deps.package_error_message err)
    )
  ...
  with_tempdir "riot_deps_load_registry_workspace_yanked"
    (
      fun root ->
        let registry_cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(root / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to initialize" in
        let version = "0.1.0" in
        let registry = Pkgs_ml.Registry.in_memory ~cache:registry_cache ~packages:[ make_registry_document ~name:"demo" ~latest:version ~releases:[ make_release ~version ~yanked:true () ] () ] () in
        ...
        | Error (Riot_deps.RegistryReleaseYanked { package; version; registry }) ->
            if String.equal package "demo" && String.equal version "0.1.0" && String.equal registry "pkgs.ml" then
              Ok ()
            else Error "expected yanked registry release error to preserve package identity"
        | Error err -> Error ("expected yanked registry release error, got: " ^ Riot_deps.package_error_message err)
...
let test_registry_package_spec_roundtrips_bare_name = fun _ctx ->
  ...
  | Ok spec ->
      if String.equal (Riot_deps.Registry_package_spec.to_string spec) "demo" then
        Ok ()
      else Error "expected bare registry package spec to render without @*"
  | Error err -> Error ("expected bare registry package spec to parse: " ^ Riot_deps.Registry_package_spec.error_message err)
...
let test_registry_package_spec_preserves_explicit_requirement = fun _ctx ->
  ...
  | Ok spec ->
      if String.equal (Riot_deps.Registry_package_spec.to_string spec) "demo@>= 1.2.3" then
        Ok ()
      else Error "expected explicit registry requirement to roundtrip"
  | Error err -> Error ("expected explicit registry package spec to parse: " ^ Riot_deps.Registry_package_spec.error_message err)
...
let test_registry_package_spec_reports_invalid_shape = fun _ctx ->
  ...
  | Error (Riot_deps.Registry_package_spec.InvalidShape { spec }) ->
      if String.equal spec "demo@1.0.0@extra" then
        Ok ()
      else Error "expected invalid shape error to preserve original spec"
  | Error err -> Error ("expected invalid shape error, got: " ^ Riot_deps.Registry_package_spec.error_message err)
...
let test_registry_package_spec_reports_invalid_package_name = fun _ctx ->
  ...
  | Error (Riot_deps.Registry_package_spec.InvalidPackageName { spec; name; error = Riot_model.Package_name.InvalidLeadingCharacter _ }) ->
      if String.equal spec "Demo@1.0.0" && String.equal name "Demo" then
        Ok ()
      else Error "expected invalid package name error to preserve spec and parsed name"
  | Error err -> Error ("expected invalid package name error, got: " ^ Riot_deps.Registry_package_spec.error_message err)
...
let test_registry_package_spec_reports_invalid_requirement = fun _ctx ->
  ...
  | Error (Riot_deps.Registry_package_spec.InvalidRequirement { spec; requirement; _ }) ->
      if String.equal spec "demo@>= nope" && String.equal requirement ">= nope" then
        Ok ()
      else Error "expected invalid requirement error to preserve spec and requirement"
  | Error err -> Error ("expected invalid requirement error, got: " ^ Riot_deps.Registry_package_spec.error_message err)
