...
let test_pipe_readwrite () =
  ...
  | Ok poll -> (
      ...
      match
        Gluon.register poll ~fd:read_fd ~token:read_token
      ...
      | Ok () -> (
          ...
          | Ok events -> (
              ...
              else Format.printf "✗ Got unexpected events on empty pipe\n";
              ...
              | Ok events ->
                  if Array.length events = 1 then (
                    ...
                    if is_correct then (
                      Format.printf
                        "✓ Event is readable and has correct token\n";
                      ...
                      match
                        Gluon.register poll ~fd:write_fd ~token:write_token
...
let test_reregister () =
  ...
  | Ok poll -> (
      ...
      match
        Gluon.register poll ~fd:read_fd ~token:token1
          ~interests:Interest.readable
      with
      | Error _ ->
      ...
      | Ok () -> (
          ...
          match
            Gluon.reregister poll ~fd:read_fd ~token:token2
              ~interests:Interest.(readable + writable)
          with
          | Error _ ->
          ...
          | Ok () -> (
              ...
              | Ok events ->
                  if Array.length events > 0 then (
                    let event = events.(0) in
                    let has_new_token =
                      Token.equal ~eq:( = ) (Event.token event) token2
                    in
