  ...
  | Ok poll -> (
      ...
      try
        ...
        for i = 0 to num_pipes - 1 do
          let r, w =
            match Gluon.pipe () with Ok p -> p | Error _ -> raise Exit
          in

          let token = Token.make i in
          match
            Gluon.register poll ~fd:r ~token ~interests:Interest.readable
          with
          | Error _ ->
              Format.printf "✗ Failed to register fd %d\n" i;
              raise Exit
          | Ok () -> pipes := (i, r, w) :: !pipes
        done;
        ...
        | Ok events ->
            ...
            let received_indices =
              Array.map
                (fun event -> Token.unsafe_to_value (Event.token event))
                events
              |> Array.to_list
            in
            ...
            let all_found =
              List.for_all
                (fun expected -> List.mem expected received_indices)
                test_indices
            in

            if all_found then Format.printf "✓ All expected events received\n"
            else Format.printf "✗ Some expected events missing\n";
...
let test_reregister_stress () =
  ...
  | Error _ ->
      ...
      false
  | Ok poll -> (
      let r, w = match Gluon.pipe () with Ok p -> p | Error _ -> exit 1 in
...
let test_concurrent_events () =
  ...
  | Ok poll -> (
      ...
      | Ok events ->
          ...
          if all_valid then Format.printf "✓ All events are valid\n"
          else Format.printf "✗ Some events are invalid\n";
...
let test_error_conditions () =
  ...
  | Ok poll -> (
      ...
      let r2, w2 = match Gluon.pipe () with Ok p -> p | Error _ -> exit 1 in
      (match Gluon.deregister poll ~fd:r2 with
        | Error _ ->
      ...
      match
        Gluon.reregister poll ~fd:r2 ~token ~interests:Interest.readable
      with
      | Error _ ->
