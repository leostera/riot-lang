...
let test_tcp_echo_server () =
  ...
  let _ =
    Gluon.register poll ~fd:listener ~token:listener_token
  ...
  let client =
    ...
    | Error _ ->
        ...
        exit 1
    | Ok client_status -> (
        match client_status with `Connected c | `In_progress c -> c)
  in
  ...
  if Array.length events = 0 || not (Event.is_readable events.(0)) then (
  ...
  let _ =
    Gluon.register poll ~fd:server_client ~token:server_token
      ~interests:Interest.readable
  in
  ...
  if not found_server_event then (
  ...
  let _ =
    Gluon.register poll ~fd:client ~token:client_token
      ~interests:Interest.readable
  in
  ...
  if not found_client_event then (
