...
let point_arb = Arbitrary.make
  ~shrink:point_shrinker
  ~print:point_printer
  ~small:(fun p -> 
    let abs_x = if p.x < 0 then -p.x else p.x in
    let abs_y = if p.y < 0 then -p.y else p.y in
    abs_x + abs_y)
  point_gen
...
let color_reflexive_prop =
  property "color equality is reflexive"
    color_arb
    (fun c -> c = c)
...
let point_translation_prop =
  property "translating by zero preserves point"
    point_arb
    (fun p ->
...
let non_empty_list_prop =
  property "non-empty list generator never produces empty list"
    Arbitrary.(make 
      (Generator.non_empty_list Generator.int))
    (fun lst -> List.length lst > 0)

(* Generate lists with specific sizes *)
let fixed_size_list_prop =
  property "list of size 5 always has 5 elements"
    Arbitrary.(make
      (Generator.list_repeat 5 Generator.int))
