...
let point_shrinker = fun point ->
  ...
  let x_only = List.map x_shrunk ~fn:(
    fun x -> { x; y = point.y }
  ) in
  let y_only = List.map y_shrunk ~fn:(
    fun y -> { x = point.x; y }
  ) in x_only @ y_only

let point_printer = fun p -> "(" ^ Int.to_string p.x ^ ", " ^ Int.to_string p.y ^ ")"

let point_arb = Arbitrary.make ~shrink:point_shrinker ~print:point_printer ~small:(
  fun p ->
    let abs_x =
      if p.x < 0 then
        -p.x
...
let color_reflexive_prop =
  property "color equality is reflexive" color_arb
    (
      fun c -> c = c
    )
...
let point_distance_positive_prop =
  property "distance from origin is non-negative" point_arb
    (
      fun p ->
        let dist_sq = p.x * p.x + p.y * p.y in dist_sq >= 0
    )
...
let point_translation_prop =
  property "translating by zero preserves point" point_arb
    (
      fun p ->
        let translated = { x = p.x + 0; y = p.y + 0 } in translated.x = p.x && translated.y = p.y
    )

(* === Generator Combinators === *)
(* Generate non-empty lists *)
let non_empty_list_prop =
  property "non-empty list generator never produces empty list" Arbitrary.(make (Generator.non_empty_list Generator.int))
    (
...
let fixed_size_list_prop =
  property "list of size 5 always has 5 elements" Arbitrary.(make (Generator.list_repeat 5 Generator.int))
    (
...
let mostly_small_ints_prop =
  property "frequency weighted generator" Arbitrary.(make ~shrink:Shrinker.int ~print:Printer.int ~small:(
    fun x ->
      if x < 0 then
        -x
      else x
    ...
    (
      Generator.frequency
        [
          9, Generator.int_range 0 10;
          1, Generator.int_range 100 1_000;
        ]
    ))
    (
      fun n -> n >= 0
