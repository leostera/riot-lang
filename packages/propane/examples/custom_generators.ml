...
let point_shrinker = fun point ->
  ...
  let x_only = List.map x_shrunk ~fn:(fun x -> { x; y = point.y }) in
  let y_only = List.map y_shrunk ~fn:(fun y -> { x = point.x; y }) in
  x_only @ y_only
...
let color_reflexive_prop = property "color equality is reflexive" color_arb (fun c -> c = c)
...
let point_translation_prop =
  property
    "translating by zero preserves point"
    point_arb
    (fun p ->
