...
let vector_push_pop_prop =
  property "vector push then pop returns the element"
    Arbitrary.(pair int (vector int))
    (fun (x, vec) ->
      let vec_copy = Collections.Vector.of_list 
        (Collections.Vector.into_iter vec |> Iter.Iterator.to_list) in
      Collections.Vector.push vec_copy x;
...
let hashmap_insert_get_prop =
  property "hashmap insert then get returns the value"
...
let hashset_insert_contains_prop =
  property "hashset contains element after insert"
    Arbitrary.(pair int (hashset int))
    (fun (x, set) ->
      Collections.HashSet.insert set x |> ignore;
      Collections.HashSet.contains set x)
...
let deque_lifo_prop =
  property "deque push_back/pop_back acts as stack"
    Arbitrary.(list int)
    (fun items ->
...
let array_length_prop =
  property "array length equals list length"
    Arbitrary.(list int)
    (fun lst ->
      let arr = Collections.Array.of_list lst in
      Collections.Array.length arr = List.length lst)

(* Property: Vector sort is sorted *)
let vector_sort_prop =
  property "vector sort produces sorted result"
