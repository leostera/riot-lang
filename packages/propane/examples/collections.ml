...
let vector_push_pop_prop =
  property "vector push then pop returns the element" Arbitrary.(pair int (vector int))
    (
      fun ((x, vec)) ->
        let vec_copy = Std.Collections.Vector.from_list (Std.Collections.Vector.iter vec |> Iter.Iterator.to_list) in
...
let hashmap_insert_get_prop =
  property "hashmap insert then get returns the value" Arbitrary.(triple string int (hashmap string int))
    (
      fun ((key, value, map)) ->
...
let hashset_insert_contains_prop =
  property "hashset contains element after insert" Arbitrary.(pair int (hashset int))
    (
      fun ((x, set)) ->
        let _ = Std.Collections.HashSet.insert set ~value:x in Std.Collections.HashSet.contains set ~value:x
    )
    ...
    (
      fun items ->
        ...
        List.for_each items ~fn:(
          fun item -> Std.Collections.Queue.push q ~value:item
        );
        ...
        in
        let popped = pop_all [] in popped = items
    )
  ...
  property "deque push_back/pop_back acts as stack" Arbitrary.(list int)
    (
      fun items ->
        ...
        List.for_each items ~fn:(
          fun item -> Std.Collections.Deque.push_back d ~value:item
        );
        let rec pop_all acc =
...
let array_length_prop =
  property "array length equals list length" Arbitrary.(list int)
    (
      fun lst ->
        let arr = Std.Collections.Array.from_list lst in Std.Collections.Array.length arr = List.length lst
    )
