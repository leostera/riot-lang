...
let sqrt_squared_prop =
  property "sqrt(x)^2 ≈ x for positive numbers"
    Arbitrary.float
    (fun x ->
      assume (x >= 0.0);
      ...
      diff < 0.0001)  (* Allow small floating point error *)

(* Using implies for conditional properties *)
let implies_example_prop =
  property "list sort preserves length (when non-empty)"
    Arbitrary.(list int)
    (fun lst ->
...
let division_properties_prop =
  property "division properties with assumptions"
    Arbitrary.(pair int int)
    (fun (a, b) ->
      assume (b != 0);
      assume (a mod b = 0);  (* a is divisible by b *)
      (a / b) * b = a)

(* Assumption that filters most cases *)
let rare_case_prop =
  property "properties on rare valid inputs"
    Arbitrary.(pair int int)
    (fun (a, b) ->
      assume (a > 0 && b > 0);
      assume (a < 100 && b < 100);
      assume (a mod 7 = 0);  (* Only multiples of 7 *)
      a >= 7)

(* Using fail for explicit failure with message *)
let fail_example_prop =
  property "explicit failure with custom message"
    Arbitrary.int
    (fun n ->
...
let chained_assumptions_prop =
  property "multiple related assumptions"
    Arbitrary.(triple int int int)
    (fun (a, b, c) ->
      assume (a > 0);
      assume (b > a);  (* b must be larger than a *)
      assume (c > b);  (* c must be larger than b *)
      (* Now we know: c > b > a > 0 *)
      c > a + 1)

(* Assumption with generation - filter valid dates *)
let valid_date_prop =
  property "valid dates have 1-31 days"
    Arbitrary.(pair (int) (int))
    (fun (month, day) ->
      let month = 1 + (month mod 12) in  (* 1-12 *)
      let day = 1 + (day mod 31) in      (* 1-31 *)
      
      (* Assume valid day for month *)
      let max_days = match month with
        | 2 -> 28  (* Simplified, ignore leap years *)
        | 4 | 6 | 9 | 11 -> 30
        | _ -> 31
      in
      assume (day <= max_days);
...
let precondition_prop =
  property "function behavior with preconditions"
    Arbitrary.(list int)
    (fun lst ->
      assume (List.length lst >= 2);
      match lst with
      | [] | [_] -> false  (* Should never happen due to assume *)
