...
let failing_list_prop =
  property "DEMO: list sum is always positive (FALSE - will find counter-example)"
    Arbitrary.(list int)
    (fun lst ->
      let sum = List.fold_left (+) 0 lst in
      sum >= 0)  (* This is false when list contains negative numbers *)

(* This will fail and shrink to minimal counter-example *)
let failing_string_length_prop =
  property "DEMO: string length never exceeds 5 (FALSE - demonstrates shrinking)"
    Arbitrary.string
    (fun s -> String.length s <= 5)

(* This demonstrates shrinking finding the boundary *)
let failing_boundary_prop =
  property "DEMO: all ints are less than 50 (FALSE)"
    Arbitrary.int
    (fun n -> n < 50)

(* Custom shrinker example *)
let custom_shrink_demo_prop =
  let my_int_arb = Arbitrary.make
    ~shrink:(Shrinker.towards 100)  (* Shrink towards 100 instead of 0 *)
    ~print:Printer.int
    Generator.int
  in
  property "DEMO: all ints equal 100 (FALSE - shrinks towards 100)"
    my_int_arb
    (fun n -> n = 100)

(* Demonstrating shrinking with complex types *)
let failing_nested_prop =
  property "DEMO: nested lists never contain empty sublist (FALSE)"
    Arbitrary.(list (list int))
    (fun nested ->
      let has_empty = List.exists (fun sublist -> List.length sublist = 0) nested in
      not has_empty)

(* Example showing minimal shrinking *)
let minimal_failure_prop =
  property "DEMO: pairs where both are positive (FALSE)"
    Arbitrary.(pair int int)
    (fun (a, b) ->
      a > 0 && b > 0)  (* Will shrink to find minimal negative case *)
...
let passing_with_assume_prop =
  property "PASS: division by non-zero works"
    Arbitrary.(pair int int)
    (fun (a, b) ->
      assume (b != 0);
      let result = a / b in
...
let sqrt_with_assume_prop =
  property "PASS: sqrt with proper domain"
    Arbitrary.float
    (fun x ->
      assume (x >= 0.0);
      Float.sqrt x >= 0.0)
...
let tests = [
  (* Comment out failing tests for CI, uncomment to see shrinking *)
  (* failing_list_prop; *)
  (* failing_string_length_prop; *)
  (* failing_boundary_prop; *)
  (* custom_shrink_demo_prop; *)
  (* failing_nested_prop; *)
  (* minimal_failure_prop; *)
  
  (* These should pass *)
  passing_with_assume_prop;
  sqrt_with_assume_prop;
]
