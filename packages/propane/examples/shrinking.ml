...
let failing_list_prop =
  property "DEMO: list sum is always positive (FALSE - will find counter-example)" Arbitrary.(list int)
    (
      fun lst ->
        let sum = List.fold_left lst ~init:0 ~fn:(
...
let failing_string_length_prop =
  property "DEMO: string length never exceeds 5 (FALSE - demonstrates shrinking)" Arbitrary.string
    (
...
let failing_boundary_prop =
  property "DEMO: all ints are less than 50 (FALSE)" Arbitrary.int
    (
      fun n -> n < 50
    )
...
let custom_shrink_demo_prop =
  ...
  property "DEMO: all ints equal 100 (FALSE - shrinks towards 100)" my_int_arb
    (
      fun n -> n = 100
    )
...
let failing_nested_prop =
  property "DEMO: nested lists never contain empty sublist (FALSE)" Arbitrary.(list (list int))
    (
      fun nested ->
        let has_empty = List.any nested ~fn:(
          fun sublist -> List.length sublist = 0
        ) in not has_empty
    )

(* Example showing minimal shrinking *)
let minimal_failure_prop =
  property "DEMO: pairs where both are positive (FALSE)" Arbitrary.(pair int int)
    (
      fun ((a, b)) -> a > 0 && b > 0
    )
...
let passing_with_assume_prop =
  property "PASS: division by non-zero works" Arbitrary.(pair int int)
    (
      fun ((a, b)) ->
        assume (b != 0);
        let result = a / b in result * b + (a mod b) = a
    )
...
let main ~args = Test.Cli.main ~name:"propane-shrinking-examples" ~tests ~args ()
