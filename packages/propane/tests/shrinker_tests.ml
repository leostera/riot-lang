...
let test_towards_zero_shrinks () =
  ...
  let rec check = function
    | [] -> Ok ()
    | x :: xs ->
        let abs_x = if x < 0 then -x else x in
        if abs_x < 100 then check xs
        else Error ("towards 0 produced value not closer to 0: " ^ Int.to_string x)
  in
  check shrunk

let test_towards_target_shrinks () =
  ...
  let rec check = function
    | [] -> Ok ()
    | x :: xs ->
        if x >= 50 && x <= 100 then check xs
        else Error ("towards 50 produced out-of-range value: " ^ Int.to_string x)
  in
  check shrunk

let test_int_at_target_no_shrink () =
  let shrinker = Shrinker.towards 0 in
  let shrunk = Shrinker.shrink shrinker 0 in
  
  if List.length shrunk = 0 then Ok ()
  else Error "shrinking value at target should produce empty list"

let test_list_shrinker_removes_elements () =
  ...
  let rec check = function
    | [] -> Ok ()
    | lst :: rest ->
        if List.length lst < List.length original then check rest
        else Error "list shrinker should produce shorter lists"
  in
  check shrunk

let test_nil_shrinker_produces_nothing () =
  let shrinker = Shrinker.nil in
  let shrunk = Shrinker.shrink shrinker 42 in
  
  if List.length shrunk = 0 then Ok ()
  else Error "nil shrinker should produce empty list"

let test_shrinking_is_finite () =
  ...
  let rec shrink_until_done value count =
    if count > 1000 then
      ...
      | [] -> Ok ()  (* Reached a minimal value *)
      | x :: _ -> shrink_until_done x (count + 1)  (* Take first candidate and continue *)
  in
  shrink_until_done 100000 0

let test_string_shrinker_produces_shorter () =
  ...
  let rec check = function
    | [] -> Ok ()
    | s :: rest ->
        if String.length s <= String.length original then check rest
        else Error "string shrinker produced longer string"
  in
  check shrunk
