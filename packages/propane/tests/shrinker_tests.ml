...
let test_towards_target_only_moves_closer = fun _ctx ->
  ...
  if List.all shrunk ~fn:(
    fun candidate -> Int.abs (candidate - 50) < Int.abs (original - 50)
  ) then
    Ok ()
  else Error "towards produced a candidate that was not closer to the target"
...
let test_string_shrinker_never_returns_longer_values = fun _ctx ->
  ...
  if List.all shrunk ~fn:(
    fun candidate -> String.length candidate <= String.length original
  ) then
    Ok ()
  else Error "string shrinker returned a longer value"

let test_string_shrinker_can_simplify_characters = fun _ctx ->
  let shrunk = Shrinker.shrink Shrinker.string "z" in
  if List.any shrunk ~fn:(
    fun candidate -> String.length candidate = 1 && candidate != "z"
  ) then
    Ok ()
  else Error "string shrinker did not produce any single-character simplification"
...
let test_list_shrinker_removes_elements = fun _ctx ->
  ...
  if List.any shrunk ~fn:(
    fun candidate -> List.length candidate < List.length original
  ) then
    Ok ()
  else Error "list shrinker did not remove any elements"
...
let test_list_shrinker_uses_the_element_shrinker = fun _ctx ->
  ...
  if List.contains shrunk ~value:[ 0 ] then
    Ok ()
  else Error "list shrinker did not use the provided element shrinker"
...
let test_hashmap_shrinker_shrinks_keys_and_values = fun _ctx ->
  ...
  if List.any shrunk ~fn:(
    fun candidate -> Collections.HashMap.get candidate ~key:0 = Some 20 || Collections.HashMap.get candidate ~key:10 = Some 0 || Collections.HashMap.get candidate ~key:0 = Some 0
  ) then
    Ok ()
  else Error "hashmap shrinker did not shrink keys or values"
...
let test_option_shrinker_can_drop_the_payload = fun _ctx ->
  ...
  if List.contains shrunk ~value:None then
    Ok ()
  else Error "option shrinker did not produce None"
