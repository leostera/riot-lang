...
let point_arb =
  ...
  let shrink point =
    let xs =
      List.map (Shrinker.towards 0 point.x) ~fn:(fun x -> { x; y = point.y })
    in
    let ys =
      List.map (Shrinker.towards 0 point.y) ~fn:(fun y -> { x = point.x; y })
    in
    xs @ ys
  in
...
let test_custom_point_arbitrary_shrinks_toward_the_origin = fun _ctx ->
  let prop =
    Property.for_all point_arb (fun _point -> false)
  in
...
let test_option_shrinking_can_reach_none = fun _ctx ->
  let arb =
    Arbitrary.make ~shrink:(Shrinker.option Shrinker.int) ~print:(Printer.option Printer.int)
      ~small:(fun value ->
        match value with
        | None -> 0
...
let test_hashmap_failures_use_a_stable_printer = fun _ctx ->
  let arb = Arbitrary.make
    ...
    (Generator.return (Collections.HashMap.from_list [ (2, 1); (1, 2) ])) in
  let test_case =
    Property.property "stable hashmap report" arb (fun _ -> false)
  in
  match test_case.fn dummy_ctx with
  | Error report ->
