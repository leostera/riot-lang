...
let failing_int_arb = Arbitrary.make
  ~shrink:Shrinker.int
  ~print:Printer.int
  ~small:Int.abs
...
let test_for_all_uses_the_default_name = fun _ctx ->
  let prop =
    Property.for_all Arbitrary.int (fun _ -> true)
  in
...
let test_check_reports_failure_and_shrinks = fun _ctx ->
  let prop =
    Property.for_all failing_int_arb (fun value -> value < 0)
  in
  match Property.check prop with
...
let test_check_reports_custom_failure_messages = fun _ctx ->
  let prop =
    Property.for_all Arbitrary.int (fun _ -> fail "boom")
  in
  match Property.check prop with
...
let test_check_reports_exceptions = fun _ctx ->
  let prop =
    Property.for_all Arbitrary.int (fun _ -> raise (Failure "boom"))
  in
  match Property.check prop with
...
let test_check_respects_max_shrink_steps = fun _ctx ->
  ...
  let prop =
    Property.for_all arb (fun _ -> false)
  in
...
let test_check_is_reproducible_for_a_fixed_seed = fun _ctx ->
  ...
  | (Property.Failure { counter_example=left_counter_example; shrink_steps=left_shrink_steps }, Property.Failure {
    counter_example=right_counter_example;
    shrink_steps=right_shrink_steps
  }) ->
...
let test_implies_uses_assumption_semantics = fun _ctx ->
  let prop =
    Property.for_all Arbitrary.int (fun _ -> implies false false)
  in
  match Property.check ~config:{ Property.default_config with test_count = 5 } prop with
  | Property.Assumption_violated -> Ok ()
...
let test_property_reads_PROPANE_TESTS = fun _ctx ->
  with_env_bindings [ ("PROPANE_TESTS", Some "17") ]
    (fun () ->
      let test_case =
        Property.property "env examples" Arbitrary.int (fun _ -> true)
      in
      let expected =
        Test.property "expected examples" ~examples:17 (fun _ctx -> Ok ())
      in
...
let test_property_reads_PROPANE_SEED = fun _ctx ->
  with_env_bindings [ ("PROPANE_TESTS", Some "1"); ("PROPANE_SEED", Some "123"); ]
    (fun () ->
      let arb = Arbitrary.make ~print:Printer.int Generator.int in
      let test_case =
        Property.property "env seed" arb (fun _ -> false)
      in
