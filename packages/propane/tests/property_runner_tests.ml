...
let with_env_bindings = fun bindings fn ->
  ...
  let restore = fun () ->
    List.for_each saved ~fn:(fun (name, value) ->
      match value with
      | Some old -> let _ = Env.set ~var:name ~value:old in ()
      | None -> let _ = Env.set ~var:name ~value:"" in ())
  in
  List.for_each bindings ~fn:(fun (name, value) ->
    match value with
    | Some current -> let _ = Env.set ~var:name ~value:current in ()
    | None -> let _ = Env.set ~var:name ~value:"" in ());
  try
    let result = fn () in
    restore ();
...
let test_check_reports_exceptions = fun _ctx ->
  ...
  | Property.Error { exception_; backtrace } ->
      if
        String.contains (Kernel.Exception.to_string exception_) "boom"
...
let test_assume_false_discards_cases = fun _ctx ->
  let prop = Property.for_all Arbitrary.int (fun _ ->
    assume false;
    true)
  in
...
let test_size_schedule_uses_the_configured_max_size = fun _ctx ->
  ...
  let arb =
    Arbitrary.make ~print:Printer.int
      (Generator.sized (fun size -> Generator.return size))
  in
  let prop =
...
let test_property_reads_PROPANE_TESTS = fun _ctx ->
  with_env_bindings [ ("PROPANE_TESTS", Some "17") ] (fun () ->
...
let test_property_reads_PROPANE_MAX_SIZE = fun _ctx ->
  with_env_bindings
    ...
    (fun () ->
      ...
      in
      let test_case =
        Property.property "env max size" arb (fun size ->
          seen := !seen @ [ size ];
          true)
      in
