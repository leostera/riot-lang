...
let with_env_bindings = fun bindings fn ->
  let saved = List.map bindings ~fn:(
    fun (name, _) -> (name, Env.get Env.String ~var:name)
  ) in
  let restore () = List.for_each saved ~fn:(
    fun (name, value) ->
      match value with
      | Some old ->
...
let failing_int_arb = Arbitrary.make ~shrink:Shrinker.int ~print:Printer.int ~small:Int.abs (Generator.return 100)
...
let test_for_all_uses_the_default_name = fun _ctx ->
  ...
  if Property.get_name prop = "unnamed property" then
    Ok ()
  else Error "for_all should use the default display name"
...
let test_check_reports_failure_and_shrinks = fun _ctx ->
  let prop =
    Property.for_all failing_int_arb
      (
        fun value -> value < 0
      )
  in
  ...
  | Property.Failure { counter_example; shrink_steps } ->
      if counter_example = "0" && shrink_steps > 0 then
        Ok ()
      else Error "check should shrink failing integers toward 0"
...
let test_check_reports_custom_failure_messages = fun _ctx ->
  ...
  | Property.Failure { counter_example; _ } ->
      if String.contains counter_example "Message: boom" then
        Ok ()
      else Error "custom failure messages should be included in the failure report"
...
let test_check_reports_exceptions = fun _ctx ->
  let prop =
    Property.for_all Arbitrary.int
      (
        fun _ -> raise (Failure "boom")
      )
  in
  match Property.check prop with
  | Property.Error { exception_; backtrace } ->
      if String.contains (Exception.to_string exception_) "boom" && String.contains backtrace "Backtrace:" then
        Ok ()
      else Error "unexpected exception reports should keep the exception and backtrace"
...
let test_check_respects_max_shrink_steps = fun _ctx ->
  ...
  | Property.Failure { counter_example; shrink_steps } ->
      if counter_example = "2" && shrink_steps = 1 then
        Ok ()
      else Error "check should stop shrinking after the configured number of steps"
...
let test_check_is_reproducible_for_a_fixed_seed = fun _ctx ->
  ...
  | (Property.Failure { counter_example = left_counter_example; shrink_steps = left_shrink_steps }, Property.Failure { counter_example = right_counter_example; shrink_steps = right_shrink_steps }) ->
      if left_counter_example = right_counter_example && left_shrink_steps = right_shrink_steps then
        Ok ()
      else Error "fixed seeds should produce identical failures"
...
let test_missing_printer_uses_a_placeholder = fun _ctx ->
  ...
  | Property.Failure { counter_example; _ } ->
      if counter_example = "<no printer available>" then
        Ok ()
      else Error "missing printers should produce a readable placeholder"
  | _ -> Error "expected a failing property"
...
let test_missing_shrinker_reports_the_original_value = fun _ctx ->
  ...
  | Property.Failure { counter_example; shrink_steps } ->
      if counter_example = "42" && shrink_steps = 0 then
        Ok ()
      else Error "properties without shrinkers should report the original failing value"
  | _ -> Error "expected a failing property"
...
let test_small_influences_shrink_choice = fun _ctx ->
  ...
  | Property.Failure { counter_example; _ } ->
      if counter_example = "1" then
        Ok ()
      else Error "small metrics should rank shrink candidates before recursive shrinking"
  | _ -> Error "expected a failing property"

let test_size_schedule_uses_the_configured_max_size = fun _ctx ->
  let seen = ref [] in
  let arb =
    Arbitrary.make ~print:Printer.int
      (
        Generator.sized
          (
            fun size -> Generator.return size
          )
      )
  in
  let prop =
  ...
  | Property.Success ->
      if !seen = [
        ...
        Ok ()
      else Error "size schedule should ramp from 0 to max_size during the property run"
...
let test_property_reads_PROPANE_TESTS = fun _ctx ->
  with_env_bindings
    ...
    (
      fun () ->
        let test_case =
          Property.property "env examples" Arbitrary.int
            (
              fun _ -> true
            )
        in
        let expected =
          Test.property "expected examples" ~examples:17
            (
              fun _ctx -> Ok ()
            )
        in
        if test_case.test_type = expected.test_type then
          Ok ()
        else Error "PROPANE_TESTS should control the example count of wrapped properties"
    )

let test_property_reads_PROPANE_SEED = fun _ctx ->
  with_env_bindings
    [
      "PROPANE_TESTS", Some "1";
      "PROPANE_SEED", Some "123";
    ]
    (
      fun () ->
        let arb = Arbitrary.make ~print:Printer.int Generator.int in
        let test_case =
          Property.property "env seed" arb
            (
              fun _ -> false
            )
        in
        ...
        | (Error left, Error right) ->
            if left = right then
              Ok ()
            else Error "PROPANE_SEED should make wrapped property failures reproducible"
...
let test_property_reads_PROPANE_MAX_SIZE = fun _ctx ->
  with_env_bindings
    [
      "PROPANE_TESTS", Some "5";
      "PROPANE_MAX_SIZE", Some "4";
      "PROPANE_SEED", Some "7";
    ]
    (
      fun () ->
        ...
        in
        let test_case =
          Property.property "env max size" arb
            (
              fun size ->
                seen := !seen @ [ size ];
                true
        ...
        | Ok () ->
            if !seen = [
              ...
              Ok ()
            else Error "PROPANE_MAX_SIZE should drive the size growth of wrapped properties"
...
let tests = Test.[
  ...
  case "missing shrinker reports the original value" test_missing_shrinker_reports_the_original_value;
  case "small influences shrink choice" test_small_influences_shrink_choice;
  case "size schedule uses the configured max size" test_size_schedule_uses_the_configured_max_size;
...
let main ~args = Test.Cli.main ~execution_mode:Test.Cli.Linear ~name:"propane/property_runner_tests" ~tests ~args ()
