...
let expect_raises = fun thunk ->
  try
    let _ = thunk () in Error "expected generator constructor to raise"
  with
  | _ -> Ok ()
  ...
  let rec loop remaining seen_low seen_high =
    if remaining = 0 then
      if seen_low && seen_high then
        Ok ()
      else Error "int_range did not reach both inclusive bounds"
    else
      ...
      else loop (remaining - 1) (seen_low || value = 5) (seen_high || value = 10)
  in
...
let test_sized_receives_the_ambient_size = fun _ctx ->
  ...
  if value = 23 then
    Ok ()
  else Error ("expected sized generator to receive 23, got " ^ Int.to_string value)
...
let test_resize_overrides_the_ambient_size = fun _ctx ->
  ...
  if value = 9 then
    Ok ()
  else Error ("expected resize to force size 9, got " ^ Int.to_string value)
  ...
  let rec loop remaining =
    if remaining = 0 then
      ...
      if Unicode.Rune.is_print value then
        loop (remaining - 1)
      else Error ("rune_printable produced a non-printable rune: " ^ Int.to_string (Unicode.Rune.to_int value))
  in
  loop 200
...
let test_string_size_uses_the_requested_length = fun _ctx ->
  ...
  if String.length value = 8 then
    Ok ()
  else Error ("expected string_size to produce length 8, got " ^ Int.to_string (String.length value))
...
let test_frequency_bias_is_observable = fun _ctx ->
  ...
  let rec loop remaining zeroes ones =
    if remaining = 0 then
      if zeroes > ones then
        Ok ()
      else Error "frequency did not favor the heavier branch"
    else
  ...
  let rec loop remaining seen_positive seen_negative =
    if remaining = 0 then
      if seen_positive && seen_negative then
        Ok ()
      else Error "float generator did not cover both signs"
    else
      let value = Generator.generate rnd Generator.float in loop (remaining - 1) (seen_positive || value > 0.0) (seen_negative || value < 0.0)
  in
