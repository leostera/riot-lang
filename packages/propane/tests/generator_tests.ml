...
let test_int_range_single_value () =
  let gen = Generator.int_range 7 7 in
  let rnd = Random.State.make [| 123 |] in
  let value = Generator.generate rnd gen in
  
  if value = 7 then Ok ()
  else Error ("int_range with low=high should return that value, got: " ^ Int.to_string value)

let test_one_of_picks_from_list () =
...
let test_map_transforms_correctly () =
  let gen = Generator.int_range 1 10 in
  let doubled = Generator.map (fun x -> x * 2) gen in
  let rnd = Random.State.make [| 42 |] in
...
let test_deterministic_with_same_seed () =
  let gen = Generator.int_range 1 1000 in
  
  let rnd1 = Random.State.make [| 12345 |] in
  let value1 = Generator.generate rnd1 gen in
  
  let rnd2 = Random.State.make [| 12345 |] in
  let value2 = Generator.generate rnd2 gen in
  
  if value1 = value2 then Ok ()
  else Error "same seed should produce same values"

let test_string_generates_strings () =
