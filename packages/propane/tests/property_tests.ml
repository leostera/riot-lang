...
let int_range_property =
  property "int_range produces values in range"
    Arbitrary.(pair int int)
    (fun (low, high) ->
      (* Use reasonable bounds *)
      let low = low mod 100 in
      let high = high mod 100 in
      let (low, high) = if low <= high then (low, high) else (high, low) in
...
let list_length_property =
  property "list length preserved by reverse"
    Arbitrary.(list int)
...
let map_append_property =
  property "map distributes over append"
    Arbitrary.(triple (list int) (list int) int)
    (fun (xs, ys, n) ->
      let f x = x + n in
...
let pair_property =
  property "pair fst/snd round-trip"
    Arbitrary.(pair int string)
    (fun p ->
      let (a, b) = p in
      fst p = a && snd p = b
    )
...
let option_none_property =
  property "option map preserves None"
    ...
    (fun opt ->
      ...
      | Some _ -> true  (* Don't care about Some case here *)
    )
...
let int_commutative_property =
  property "int addition is commutative"
    ...
    (fun (a, b) ->
      ...
      let a = a mod 10000 in
      let b = b mod 10000 in
      a + b = b + a)
...
let int_associative_property =
  property "int addition is associative"
    ...
    (fun (a, b, c) ->
      ...
      let a = a mod 10000 in
      let b = b mod 10000 in
      let c = c mod 10000 in
      (a + b) + c = a + (b + c))

(* Property: assumes work correctly *)
let assume_property =
  property "assumes filter test cases"
    Arbitrary.int
    (fun n ->
      assume (n >= 0);
      n >= 0  (* Should always pass since we filtered *)
    )
