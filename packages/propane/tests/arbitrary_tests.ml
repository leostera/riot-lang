...
let test_arbitrary_int_generates () =
...
let test_arbitrary_int_has_shrinker () =
  let arb = Arbitrary.int in
  ...
  | Some shrinker ->
      ...
      else Error "int shrinker should produce candidates"

let test_arbitrary_int_has_printer () =
  let arb = Arbitrary.int in
  ...
  | Some printer ->
      let s = printer 42 in
      if s = "42" then Ok ()
      else Error ("printer produced wrong output: " ^ s)

let test_arbitrary_list_generates () =
...
let test_arbitrary_list_has_printer () =
  let arb = Arbitrary.list Arbitrary.int in
  match arb.print with
  ...
  | Some printer ->
      ...
      else Error "printer produced empty string"

let test_arbitrary_pair_generates () =
...
let test_arbitrary_option_generates_both () =
  let arb = Arbitrary.option Arbitrary.int in
  let rnd = Random.State.make [| 42 |] in
  
  (* Generate many values and check we get both None and Some *)
  let rec check n seen_none seen_some =
    if n = 0 then
      if seen_none && seen_some then Ok ()
      else Error "option should generate both None and Some"
    else
...
let test_arbitrary_string_generates () =
...
let test_arbitrary_bool_generates_both () =
  let arb = Arbitrary.bool in
  let rnd = Random.State.make [| 42 |] in
  
  (* Generate many values and check we get both true and false *)
  let rec check n seen_true seen_false =
    if n = 0 then
      if seen_true && seen_false then Ok ()
      else Error "bool should generate both true and false"
    else
...
let test_arbitrary_int_has_small () =
  let arb = Arbitrary.int in
  ...
  | Some small_fn ->
      ...
      if size1 >= 0 && size2 >= 0 then Ok ()
      else Error "small function should return non-negative values"
