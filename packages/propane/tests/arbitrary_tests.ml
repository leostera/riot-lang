...
let test_int_wires_generator_shrinker_printer_and_small = fun _ctx ->
  ...
  | (Some shrinker, Some printer, Some small_fn) ->
      ...
      then
        ...
        Error "Arbitrary.int is not wired to the expected components"
...
let test_list_keeps_structural_shrinker_without_element_shrinker = fun _ctx ->
  ...
  | Some shrinker ->
      if List.any (shrinker [ 1; 2; 3 ]) ~fn:(fun candidate -> List.length candidate < 3) then
        Ok ()
      else
        Error "list arbitrary did not keep structural shrinking"
...
let test_array_small_is_array_length = fun _ctx ->
  ...
  | Some small ->
      ...
      if small value = 3 then
        ...
        Error "array small metric should equal array length"
...
let test_vector_small_is_vector_length = fun _ctx ->
  ...
  | Some small ->
      let value = Collections.Vector.from_list [ 1; 2; 3; 4; ] in
      if small value = 4 then
        Ok ()
      else
        Error "vector small metric should equal vector length"
...
let test_hashmap_small_is_entry_count = fun _ctx ->
  ...
  | Some small ->
      let value = Collections.HashMap.from_list [ (1, "a"); (2, "b"); ] in
      if small value = 2 then
        Ok ()
      else
        Error "hashmap small metric should equal entry count"
...
let test_hashset_small_is_cardinality = fun _ctx ->
  ...
  | Some small ->
      ...
      if small value = 3 then
        ...
        Error "hashset small metric should equal set cardinality"
...
let test_queue_small_is_length = fun _ctx ->
  ...
  | Some small ->
      ...
      if small value = 2 then
        ...
        Error "queue small metric should equal queue length"
...
let test_deque_small_is_length = fun _ctx ->
  ...
  | Some small ->
      ...
      if small value = 3 then
        ...
        Error "deque small metric should equal deque length"
...
let test_heap_small_is_size = fun _ctx ->
  ...
  | Some small ->
      let value = Collections.Heap.from_list [ 1; 2; 3; 4; 5; ] in
      if small value = 5 then
        Ok ()
      else
        Error "heap small metric should equal heap size"
...
let test_option_small_counts_none_and_some = fun _ctx ->
  ...
  | Some small ->
      if small None = 0 && small (Some 5) = 6 then
        ...
        Error "option small metric should treat None as smaller than Some"
...
let test_result_small_counts_payload_size = fun _ctx ->
  ...
  | Some small ->
      if small (Ok 5) = 6 && small (Error (-5)) = 6 then
        ...
        Error "result small metric should include the active branch payload"
...
let tests =
  Test.[
    ...
    case
      "int wires generator shrinker printer and small"
      test_int_wires_generator_shrinker_printer_and_small;
    case
      "list omits printer when element printer is missing"
      test_list_omits_printer_when_element_printer_is_missing;
    case
      "list keeps structural shrinker without element shrinker"
      test_list_keeps_structural_shrinker_without_element_shrinker;
    ...
    case
      "map preserves inverse driven printer and small"
      test_map_preserves_inverse_driven_printer_and_small;
