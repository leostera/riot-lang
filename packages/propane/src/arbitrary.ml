...
let make ?shrink ?print ?small gen = { gen; shrink; print; small }
...
let bool = {
  ...
  small = Some (fun b -> if b then 1 else 0);
}
...
let list elem_arb = {
  gen = Generator.list elem_arb.gen;
  shrink = (match elem_arb.shrink with
    | Some elem_shrinker -> Some (Shrinker.list elem_shrinker)
    | None -> Some (Shrinker.list Shrinker.nil));
  print = (match elem_arb.print with
...
let array elem_arb = {
  gen = Generator.array elem_arb.gen;
  shrink = (match elem_arb.shrink with
...
let vector elem_arb = {
  gen = Generator.vector elem_arb.gen;
  shrink = (match elem_arb.shrink with
...
let hashmap key_arb value_arb = {
  gen = Generator.hashmap key_arb.gen value_arb.gen;
  shrink = (match key_arb.shrink, value_arb.shrink with
    | Some key_shrinker, Some value_shrinker -> 
        Some (Shrinker.hashmap key_shrinker value_shrinker)
    | _ -> Some (Shrinker.hashmap Shrinker.nil Shrinker.nil));
  print = (match key_arb.print, value_arb.print with
    | Some key_printer, Some value_printer ->
        Some (Printer.hashmap key_printer value_printer)
    | _ -> None);
  small = Some Collections.HashMap.len;
}

let hashset elem_arb = {
  gen = Generator.hashset elem_arb.gen;
  shrink = (match elem_arb.shrink with
...
let queue elem_arb = {
  gen = Generator.queue elem_arb.gen;
  shrink = (match elem_arb.shrink with
...
let deque elem_arb = {
  gen = Generator.deque elem_arb.gen;
  shrink = (match elem_arb.shrink with
...
let heap elem_arb = {
  gen = Generator.heap elem_arb.gen;
  shrink = (match elem_arb.shrink with
...
let pair arb_a arb_b = {
  gen = Generator.pair arb_a.gen arb_b.gen;
  shrink = (match arb_a.shrink, arb_b.shrink with
    | Some shrinker_a, Some shrinker_b ->
        Some (Shrinker.pair shrinker_a shrinker_b)
    | _ -> None);
  print = (match arb_a.print, arb_b.print with
    | Some printer_a, Some printer_b ->
        Some (Printer.pair printer_a printer_b)
    | _ -> None);
  small = None;
}

let triple arb_a arb_b arb_c = {
  gen = Generator.triple arb_a.gen arb_b.gen arb_c.gen;
  shrink = (match arb_a.shrink, arb_b.shrink, arb_c.shrink with
    | Some shrinker_a, Some shrinker_b, Some shrinker_c ->
        Some (Shrinker.triple shrinker_a shrinker_b shrinker_c)
    | _ -> None);
  print = (match arb_a.print, arb_b.print, arb_c.print with
    | Some printer_a, Some printer_b, Some printer_c ->
        Some (Printer.triple printer_a printer_b printer_c)
    | _ -> None);
  small = None;
}
...
let option elem_arb = {
  gen = Generator.option elem_arb.gen;
  shrink = (match elem_arb.shrink with
...
let result ok_arb err_arb = {
  gen = Generator.result ok_arb.gen err_arb.gen;
  shrink = (match ok_arb.shrink, err_arb.shrink with
    | Some ok_shrinker, Some err_shrinker ->
        Some (Shrinker.result ok_shrinker err_shrinker)
    | _ -> None);
  print = (match ok_arb.print, err_arb.print with
    | Some ok_printer, Some err_printer ->
        Some (Printer.result ok_printer err_printer)
    | _ -> None);
  small = None;
}
...
let map f f_inv arb = {
  gen = Generator.map f arb.gen;
  shrink = (match arb.shrink with
    | Some shrinker -> Some (Shrinker.map f f_inv shrinker)
    | None -> None);
  print = (match arb.print with
    | Some printer -> Some (fun b -> printer (f_inv b))
    | None -> None);
  small = (match arb.small with
    | Some small_fn -> Some (fun b -> small_fn (f_inv b))
    | None -> None);
}

let map_gen gen arb = { arb with gen }
