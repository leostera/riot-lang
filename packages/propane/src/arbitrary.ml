...
let make = fun ?shrink ?print ?small gen ->
  {
    gen;
    shrink;
    print;
...
let int32 = {
  ...
  small = Some (
    fun n -> Int32.to_int (Int32.abs n)
  )
}
...
let int64 = {
  ...
  small = Some (
    fun n -> Int64.to_int (Int64.abs n)
  )
}
...
let float = {
  ...
  small = Some (
    fun f -> Float.to_int (Float.abs f)
  )
}
...
let triple = fun arb_a arb_b arb_c ->
  {
    ...
    shrink = (
      ...
      | Some shrinker_a, Some shrinker_b, Some shrinker_c -> Some (Shrinker.triple shrinker_a shrinker_b shrinker_c)
      | _ -> None
    );
    print = (
      match arb_a.print, arb_b.print, arb_c.print with
      | Some printer_a, Some printer_b, Some printer_c -> Some (Printer.triple printer_a printer_b printer_c)
      | _ -> None
    );
    small = (
      ...
      | Some small_a, Some small_b, Some small_c ->
          Some (
            fun ((a, b, c)) -> small_a a + small_b b + small_c c
          )
      | _ -> None
    )
  }
