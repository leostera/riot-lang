...
let frequency = fun weighted_gens ->
  ...
  | _ ->
      ...
      {run = fun rnd size ->
          ...
          let rec find = fun acc remaining ->
...
let fix = fun f ->
  let rec self = fun n -> f self n in
...
let int32 = {run = fun rnd _size ->
    Random.State.int32 rnd Int32.max_int}

let int64 = {run = fun rnd _size ->
...
let positive_int = {run = fun rnd _size ->
...
let float_positive = {run = fun rnd _size ->
    Random.State.float rnd max_float}
...
let string =
  sized
    (fun size ->
      ...
      and_then len_gen
        (fun len ->
          let rec build_string = fun acc n ->
            if n <= 0 then
