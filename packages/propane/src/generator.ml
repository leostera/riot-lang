...
type 'value t = { run: Random.Rng.t -> int -> 'value }
...
let random_int32_range = fun rnd ~low ~high -> sample (Random.int32_range ~rng:rnd ~min:low ~max:high ())

let random_int64_range = fun rnd ~low ~high -> sample (Random.int64_range ~rng:rnd ~min:low ~max:high ())

let random_float_range = fun rnd ~low ~high -> sample (Random.float_range ~rng:rnd ~min:low ~max:high ())

let invalid_arg = fun message -> raise (Invalid_argument message)

(* Helper: convert char list to string *)
let string_of_char_list = fun chars -> String.concat "" (List.map chars ~fn:(
...
let build_list_values = fun rnd size gen len ->
  let rec loop remaining acc =
    if remaining <= 0 then
      List.reverse acc
    else loop (remaining - 1) (gen.run rnd size :: acc)
  in
  loop len []
...
let map2 = fun f gen1 gen2 ->
  {
    run = fun rnd size ->
      ...
      let v2 = gen2.run rnd size in f v1 v2
  }
...
let map3 = fun f gen1 gen2 gen3 ->
  {
    run = fun rnd size ->
      ...
      let v3 = gen3.run rnd size in f v1 v2 v3
  }
...
let and_then = fun gen f ->
  {
    run = fun rnd size ->
      ...
      let gen' = f v in gen'.run rnd size
  }
  ...
  | _ ->
      let choices = Array.from_list gens in
      {
        run = fun rnd size ->
          ...
          let gen = Array.get_unchecked choices ~at:idx in gen.run rnd size
      }
  ...
  | _ ->
      ...
      let total_weight = populate 0 0 in
      {
        run = fun rnd size ->
          ...
          let (_, gen) = Array.get_unchecked choices ~at:index in gen.run rnd size
      }
...
let int_range = fun low high ->
    ...
    invalid_arg "Generator.int_range: low > high";
  {
    run = fun rnd _size ->
      if low = high then
        low
      else random_int_range rnd ~low ~high
  }
...
let int32_range = fun low high ->
    ...
    invalid_arg "Generator.int32_range: low > high";
  {
    run = fun rnd _size ->
      if low = high then
        low
      else random_int32_range rnd ~low ~high
  }
...
let int64_range = fun low high ->
    ...
    invalid_arg "Generator.int64_range: low > high";
  {
    run = fun rnd _size ->
      if low = high then
        low
      else random_int64_range rnd ~low ~high
  }
...
let non_zero_int =
  ...
  let neg: int t = int_range Int.min_int (-1) in one_of [ pos; neg ]

(* Floats *)
let float = {
  run = fun rnd _size -> random_float_range rnd ~low:(-.1000000.0) ~high:1000000.0
}
...
let float_positive = {
  run = fun rnd _size -> random_float_range rnd ~low:0.0 ~high:1000000.0
}
...
let weighted_bool = fun weight_true weight_false ->
  ...
  frequency
    [
      weight_true, return true;
      weight_false, return false;
    ]
...
let char_range = fun low high ->
    ...
    invalid_arg "Generator.char_range: low > high";
  {
    run = fun rnd _size ->
      ...
      let high_code = Char.code high in Char.from_int_unchecked (random_int_range rnd ~low:low_code ~high:high_code)
  }
...
let string =
  sized
    (
      fun size ->
        {
          run = fun rnd _ambient_size ->
            let len = random_int_range rnd ~low:0 ~high:size in build_string rnd size char len
        }
    )
  ...
  sized
    (
      fun size ->
        {
          run = fun rnd _ambient_size ->
            let len = random_int_range rnd ~low:0 ~high:size in build_string rnd size char_gen len
        }
    )
...
let string_size = fun size_gen char_gen ->
  {
    run = fun rnd size ->
      ...
      if len <= 0 then
        ""
      else build_string rnd size char_gen len
  }
  ...
  sized
    (
      fun size ->
        {
          run = fun rnd _ambient_size ->
            let len = random_int_range rnd ~low:0 ~high:size in build_list_values rnd size gen len
        }
    )
...
let list_size = fun size_gen gen ->
  {
    run = fun rnd size ->
      ...
      if len <= 0 then
        []
      else build_list_values rnd size gen len
  }

let list_repeat = fun n gen -> list_size (return n) gen

let non_empty_list = fun gen ->
  and_then (int_range 1 10)
    (
      fun len -> list_size (return len) gen
    )
...
let quad = fun gen1 gen2 gen3 gen4 ->
  {
    run = fun rnd size ->
      ...
      let d = gen4.run rnd size in (a, b, c, d)
  }
...
let hashmap = fun key_gen value_gen ->
  let pair_gen = pair key_gen value_gen in map Collections.HashMap.from_list (list pair_gen)

let hashmap_size = fun size_gen key_gen value_gen ->
  let pair_gen = pair key_gen value_gen in map Collections.HashMap.from_list (list_size size_gen pair_gen)
    ...
    (
      fun lst ->
        ...
        List.for_each lst ~fn:(
          fun value -> Collections.Deque.push_back d ~value
        );
        d
    )
    (list gen)
  ...
  map
    (
      fun lst ->
        ...
        List.for_each lst ~fn:(
          fun value -> Collections.Heap.push h ~value
        );
        h
    )
    (list gen)
