...
let towards target =
  fun value ->
    if value = target then []
      ...
      let steps = List.rev steps in  (* Reverse to get decreasing order *)
      let steps = match steps with
        | _ :: rest -> rest  (* Remove first (largest) to avoid returning original *)
        | [] -> []
      in
      List.map (fun step ->
        if diff > 0 then target + step
        else target - step
      ) steps
...
let int_towards target =
  fun value ->
    if value = target then []
      ...
      List.map (fun step ->
        if value > target then value - step
        else value + step
      ) steps
...
let int32 =
  fun value ->
    if value = 0l then []
      ...
      List.map (fun step ->
        if value > 0l then Int32.sub value step
        else Int32.add value step
      ) steps
...
let int64 =
  fun value ->
    if value = 0L then []
      ...
      List.map (fun step ->
        if value > 0L then Int64.sub value step
        else Int64.add value step
      ) steps
...
let float =
  fun value ->
    if value = 0.0 then []
      ...
      let rec halve n acc =
        if n = 0.0 || Float.abs n < 0.0001 then acc
        else halve (n /. 2.0) (n :: acc)
      in
      let steps = halve (Float.abs value) [] in
      List.map (fun step ->
        if value > 0.0 then value -. step
        else value +. step
      ) steps
...
let array elem_shrinker =
  fun arr ->
    let arr_to_list a =
      let rec build i acc =
        if i < 0 then acc
        else build (i - 1) (a.(i) :: acc)
      in
...
let pair shrinker_a shrinker_b =
  fun (a, b) ->
    let shrunk_a = List.map (fun a' -> (a', b)) (shrinker_a a) in
    let shrunk_b = List.map (fun b' -> (a, b')) (shrinker_b b) in
    shrunk_a @ shrunk_b

let triple shrinker_a shrinker_b shrinker_c =
  fun (a, b, c) ->
    let shrunk_a = List.map (fun a' -> (a', b, c)) (shrinker_a a) in
    let shrunk_b = List.map (fun b' -> (a, b', c)) (shrinker_b b) in
    let shrunk_c = List.map (fun c' -> (a, b, c')) (shrinker_c c) in
    shrunk_a @ shrunk_b @ shrunk_c

(* === OPTION & RESULT SHRINKERS === *)

let option elem_shrinker =
  fun opt ->
    match opt with
    | None -> []
    | Some x ->
        let none_candidate = [None] in
        let shrunk_some = List.map (fun x' -> Some x') (elem_shrinker x) in
        none_candidate @ shrunk_some

let result ok_shrinker err_shrinker =
  fun res ->
    match res with
...
let map f f_inv shrinker =
  fun b ->
    let a = f_inv b in
    List.map f (shrinker a)

let filter pred shrinker =
  fun value ->
    List.filter pred (shrinker value)

(* === LOW-LEVEL INTERFACE === *)

let shrink shrinker value = shrinker value
