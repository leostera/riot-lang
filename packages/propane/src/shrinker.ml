...
let string = fun value ->
  ...
  if len = 0 then
    ...
    let rec remove_positions = fun n acc ->
      if n >= len then
        acc
      else
    ...
    let rec shrink_length = fun curr acc ->
      if curr <= 0 then
        acc
      else
        let half = curr / 2 in
...
let array = fun elem_shrinker ->
  fun arr ->
    let arr_to_list = fun a ->
      let rec build = fun i acc ->
        if i < 0 then
          acc
        else
          build (i - 1) (a.(i) :: acc)
