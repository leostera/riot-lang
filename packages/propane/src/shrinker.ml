        ...
        (
          ...
          let shrunk =
            List.map
              (char current_char)
              ~fn:(fun char' ->
                string_replace_at value ~at:index ~char:char')
          in
...
let list = fun elem_shrinker ->
  fun lst ->
      ...
      let rec shrink_elements index =
        if index < 0 then
          ...
          (
            let value =
              nth lst ~at:index
              |> Option.expect ~msg:"valid shrink element index"
            in
            let shrunk =
              List.map
                (elem_shrinker value)
                ~fn:(fun value' ->
                  replace_nth lst ~at:index ~value:value')
            in
    ...
    dedupe
      (
        (removed @ shortened) @ (
          Vector.to_array shrunk_elements
          |> Array.to_list
        )
      )
...
let option = fun elem_shrinker ->
  fun opt ->
    ...
    | Some x ->
        ...
        let shrunk_some = List.map (elem_shrinker x) ~fn:(fun x' -> Some x') in
