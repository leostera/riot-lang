...
let order_candidates = fun arb candidates ->
  ...
  | None -> candidates
  | Some small -> List.sort candidates ~compare:(
    fun left right -> Int.compare (small left) (small right)
  )
...
let size_for_test = fun config index ->
  ...
  if config.test_count <= 1 then
    max_size
  else (index * max_size) / (config.test_count - 1)
        ...
        if steps_taken >= max_steps then
          ...
          let rec try_candidates = function
            ...
            | candidate :: rest ->
                ...
                let fails =
                  try
                    let result = predicate candidate in not result
                  with
                ...
                in
                if fails then
                  let next_step = steps_taken + 1 in Option.for_each on_progress ~fn:(
                    fun on_progress ->
                      on_progress
                        (
                ...
                else (* This candidate passes, try next one *)
                try_candidates rest
          in
...
let check = fun ?(config = default_config) ?(on_progress = fun _ -> ()) (Prop prop) ->
  ...
  let rec test_loop n =
    if n >= config.test_count then
      ...
      if !assumptions_failed > config.test_count * 10 then
        Assumption_violated
      else
        ...
        let test_result =
          ...
          | exn ->
              let backtrace = Exception.raw_backtrace_to_string (Exception.get_raw_backtrace ()) in Some (`Exception (value, exn, backtrace))
        in
        match test_result with
        | None -> (* Assumption failed, try next test *)
        test_loop n
        | Some `Passed ->
            tests_run := !tests_run + 1;
            on_progress (Test.Context.PropertyIterationPassed { current = n + 1; total = config.test_count; size });
            if config.verbose then
              println ("Test " ^ Int.to_string (!tests_run) ^ "/" ^ Int.to_string config.test_count ^ " passed (size=" ^ Int.to_string size ^ ")");
            test_loop (n + 1)
        | Some (`Failed value) ->
            (* Property failed! Shrink to find minimal counter-example *)
            on_progress (Test.Context.PropertyCounterExampleFound { current = n + 1; total = config.test_count; size });
            let (minimal_value, shrink_steps) = shrink_counter_example ~on_progress ~iteration:(n + 1) ~total:config.test_count arbitrary value predicate config.max_shrink_steps in
            let counter_example =
        ...
        | Some (`Failed_with_msg (value, msg)) ->
            ...
            let value_str =
              ...
              | None -> "<no printer available>"
            in
            let counter_example = value_str ^ "\nMessage: " ^ msg in Failure { counter_example; shrink_steps }
...
let get_test_count_from_env = fun () -> Env.get Env.Int ~var:"PROPANE_TESTS" |> Option.unwrap_or ~default:default_config.test_count

let get_seed_from_env = fun () -> Env.get Env.Int ~var:"PROPANE_SEED"

let get_max_size_from_env = fun () -> Env.get Env.Int ~var:"PROPANE_MAX_SIZE" |> Option.unwrap_or ~default:default_config.max_size
  ...
  Test.property name ~examples:test_count
    (
      fun ctx ->
        ...
        | Success -> Ok ()
        | Failure { counter_example; shrink_steps } ->
            let msg = String.concat "\n" [ "Property failed"; "Counter-example (after " ^ Int.to_string shrink_steps ^ " shrink steps):"; counter_example ] in Error msg
        | Error { exception_; backtrace } ->
            let msg = String.concat "\n" [ "Exception raised:"; Exception.to_string exception_; backtrace ] in Error msg
