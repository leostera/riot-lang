...
let make_one_of = fun branch_count ->
  list_init branch_count (fun index -> Generator.return index) |> Generator.one_of
    ...
    (fun index ->
      let weight =
        if heavy_head && index = 0 then
          ...
          1
      in
      (weight, Generator.return index)) |> Generator.frequency
...
let bench_property_check = fun () ->
  let prop =
    Property.for_all Arbitrary.int (fun value -> value + 0 = value)
  in
...
let bench_shrinking_failure = fun () ->
  let arb = Arbitrary.make
    ...
    ~small:List.length
    (Generator.return [ 50; 25; 10 ]) in
  let prop =
    Property.for_all arb (fun values -> List.is_empty values)
  in
