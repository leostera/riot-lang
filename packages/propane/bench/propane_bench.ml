...
let make_rng = fun seed -> Random.Rng.standard ~seed:(Int.to_string seed) () |> Result.expect ~msg:"failed to create deterministic propane bench rng"
...
let make_one_of = fun branch_count ->
  list_init branch_count
    (
      fun index -> Generator.return index
    ) |> Generator.one_of
...
let bench_shrinking_failure = fun () ->
  let arb = Arbitrary.make ~shrink:(Shrinker.list Shrinker.int) ~print:(Printer.list Printer.int) ~small:List.length (Generator.return [ 50; 25; 10 ]) in
  let prop =
    Property.for_all arb
      (
        fun values -> List.is_empty values
      )
  in
  let config = { Property.default_config with test_count = 1; max_shrink_steps = 100; seed = Some 1 } in
  fun () ->
    let _ = Property.check ~config prop in ()
...
let benchmarks = Bench.[
  ...
  with_config ~config:medium_config "propane frequency 64 uniform" (bench_frequency 64 ~heavy_head:false);
  with_config ~config:medium_config "propane frequency 64 heavy head" (bench_frequency 64 ~heavy_head:true);
  ...
  with_config ~config:short_config "propane string generation size 32" (bench_string_generation 32);
  with_config ~config:heavy_config "propane string generation size 512" (bench_string_generation 512);
  ...
  with_config ~config:short_config "propane property check assumption heavy" (bench_assumption_heavy_property ());
  with_config ~config:heavy_config "propane property check shrinking list failure" (bench_shrinking_failure ());
]
