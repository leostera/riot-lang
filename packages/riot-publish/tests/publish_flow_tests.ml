...
let make_deps = fun
  ...
  Riot_publish.For_test.{
    ...
    run_build_check = (fun ~emit:_ ~workspace:_ ~package_name:_ ~profile:_ ->
      ...
      Ok ());
    plan_publish = (fun ~registry:_ ~publishing_workspace_packages:_ ~package ->
      call_log := "metadata" :: !call_log;
      Ok (make_plan package));
    prepare_publish_artifact = (fun ~target_dir_root:_ plan ->
      call_log := "prepare" :: !call_log;
      Ok (make_prepared plan.package));
    publish_prepared = (fun ~registry ~api_token prepared ->
...
let test_skip_check_skips_fix_stage = fun _ctx ->
  ...
  | Ok _ ->
      ...
      let actual_calls = List.reverse !call_log in
      let fix_events =
        List.filter actual_events ~fn:(fun name ->
