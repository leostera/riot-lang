...
let make_prepared = fun package ->
  Riot_deps.Publisher.{
    ...
    locator = "github.com/example/" ^ Riot_model.Package_name.to_string package.name;
    selector = "deadbeef";
    artifact_path = Path.(Path.v "/tmp"
    / Path.v (Riot_model.Package_name.to_string package.name ^ ".tar.gz"));
  }
...
let make_published_release = fun package_name ->
  Pkgs_ml.Registry.{
    ...
    manifest = {
      key = package_name ^ "/manifest.json";
...
let test_dry_run_emits_preflight_events_in_order = fun _ctx ->
  ...
  match Riot_publish.For_test.publish_with
    ...
    () with
  | Error err ->
      Error ("expected dry run to succeed, got error: " ^ Riot_publish.publish_error_message err)
  | Ok outcomes ->
...
let test_skip_check_skips_fix_stage = fun _ctx ->
  ...
  match Riot_publish.For_test.publish_with
    ...
    () with
  | Error err ->
      Error ("expected skip-check dry run to succeed, got error: "
      ^ Riot_publish.publish_error_message err)
  | Ok _ ->
      let actual_events = List.reverse !events in
      let actual_calls = List.reverse !call_log in
      let fix_events = List.filter actual_events ~fn:(fun name -> String.contains name "fix") in
...
let test_publish_mode_uses_token_and_emits_published = fun _ctx ->
  ...
  match Riot_publish.For_test.publish_with
    ...
    () with
  | Error err ->
      Error ("expected publish mode to succeed, got error: "
      ^ Riot_publish.publish_error_message err)
  | Ok outcomes ->
      let actual_calls = List.reverse !call_log in
      let actual_events = List.reverse !events in
      if
        not (actual_calls = [ "token"; "fmt"; "fix"; "build"; "metadata"; "prepare"; "publish"; ])
      then
        Error ("unexpected call order: " ^ String.concat ", " actual_calls)
...
let test_already_published_package_is_skipped_before_checks = fun _ctx ->
  ...
  match Riot_publish.For_test.publish_with
    ...
    () with
  | Error err ->
      Error ("expected already-published package to be skipped, got error: "
      ^ Riot_publish.publish_error_message err)
  | Ok outcomes ->
      let actual_calls = List.reverse !call_log in
      let actual_events = List.reverse !events in
...
let tests =
  Test.[
    ...
    case
      "publish flow: already-published package is skipped before checks"
      test_already_published_package_is_skipped_before_checks;
  ]
