...
let make_registry = fun () ->
  let cache =
    Pkgs_ml.Registry_cache.create
      ...
      ()
    |> Result.expect ~msg:"expected in-memory registry cache"
  in
...
let make_deps = fun ?(workspace_publish_order = fun ~packages -> Ok packages) ?(published_version_exists = fun ~registry:_ ~package_name:_ ~version:_ -> Ok false) () ->
  Riot_publish.For_test.{
    ...
    run_fix_check = (fun ~emit:_ ~registry:_ ~workspace:_ ~request:_ ~package:_ ->
      panic_unexpected "fix");
    run_build_check = (fun ~emit:_ ~workspace:_ ~package_name:_ ~profile:_ ->
      panic_unexpected "build");
    plan_publish = (fun ~registry:_ ~publishing_workspace_packages:_ ~package:_ ->
      panic_unexpected "metadata");
...
let test_private_package_is_skipped = fun _ctx ->
  ...
  match Riot_publish.For_test.publish_with
    ...
    () with
  | Error err ->
      Error ("expected private package to be skipped, got error: "
...
let test_workspace_selection_orders_public_packages_only = fun _ctx ->
  ...
  match Riot_publish.For_test.publish_with
    ...
    () with
  | Error err ->
      Error ("expected workspace selection to succeed, got error: "
      ^ Riot_publish.publish_error_message err)
  | Ok outcomes ->
...
let test_publish_error_message_renders_typed_registry_initialization_error = fun _ctx ->
  ...
  in
  let expected =
    "failed to initialize registry 'pkgs.ml': failed to determine home directory for pkgs.ml cache"
  in
...
let test_publish_error_message_renders_typed_workspace_errors = fun _ctx ->
  ...
  let expected_load =
    "failed to load workspace '/workspace': package 'dep': failed to read riot.toml at path deps/dep/riot.toml"
  in
...
let test_publish_error_message_renders_check_exceptions = fun _ctx ->
  ...
  let fmt_message =
    Riot_publish.publish_error_message
      (Riot_publish.FmtCheckFailed { package; error = Failure "formatting failed" })
  in
  let fix_message =
    Riot_publish.publish_error_message
      (Riot_publish.FixCheckFailed { package; error = Failure "lint failed" })
  in
...
let tests =
  Test.[
    case
      "publish selection: workspace without packages errors"
      test_workspace_without_packages_errors;
    ...
    case
      "publish selection: renders typed workspace errors"
      test_publish_error_message_renders_typed_workspace_errors;
    case
      "publish selection: renders typed build check errors"
      test_publish_error_message_renders_typed_build_check_error;
    case
      "publish selection: renders check exceptions"
      test_publish_error_message_renders_check_exceptions;
  ]
