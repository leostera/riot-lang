...
let message = function
  | ReadFailed { path; error } ->
      "failed to read config '" ^ Path.to_string path ^ "': " ^ error
  | ParseFailed { path; error } ->
      "failed to parse config '" ^ Path.to_string path ^ "': " ^ error
  | InvalidConfig { error } ->
      "invalid config: " ^ error
  | InvalidRegistryConfig { registry_name; error } ->
      "invalid registry config for '" ^ registry_name ^ "': " ^ error
...
let registry_of_toml = fun ~registry_name value ->
  ...
  | _ ->
      Error (InvalidRegistryConfig {
        registry_name;
        error = "registry entry must be a table";
      })
...
let of_toml = fun value ->
  ...
  | Toml.Table fields -> (
      ...
      | Some _ ->
          Error (InvalidConfig { error = "top-level 'registry' entry must be a table" })
    )
  | _ ->
      Ok empty
...
let load = fun path ->
  ...
  | Ok source -> (
      ...
      | Error parse_error ->
          Error (ParseFailed {
            path;
            error = Toml.error_to_string parse_error;
          })
      | Ok toml -> (
...
let api_token = fun config ~registry_name ->
  match List.find_opt (fun (name, _registry) -> String.equal name registry_name) config.registries with
  | None -> None
