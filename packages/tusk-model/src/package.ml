...
let validate_name = fun name ->
  let is_alpha = fun c -> (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') in
  let is_lowercase = fun c -> c >= 'a' && c <= 'z' in
  let is_digit = fun c -> c >= '0' && c <= '9' in
  let is_alphanum = fun c -> is_alpha c || is_digit c in
  let is_valid_char = fun c -> is_alphanum c || c = '-' || c = '_' in
...
let parse_dependency : string -> Toml.value -> workspace_deps:dependency list -> dependency = fun name value ~workspace_deps ->
  match value with
  | Toml.Table attrs -> (
...
(foreign_dependency, string) result = fun name value ~package_path ->
  ...
  | Toml.Table attrs -> (
      ...
      in
      let get_string_list_opt = fun key ->
        match List.assoc_opt key attrs with
        | Some (Toml.Array arr) ->
            let strings =
              List.filter_map
      ...
      | Ok path_str, Ok build_cmd, Ok outputs ->
          ...
          let scan_foreign_inputs = fun foreign_path ->
            let rec scan_recursive = fun ~from_dir ~rel_path ~exclude_dirs ->
...
let provider_excluded_relpaths = fun ~(package_path:Path.t) providers ->
  ...
  let collect_provider_tree = fun rel_path ->
    ...
    if String.equal basename "tusk_fix_rules.ml" && String.equal parent_basename "tusk_fix_rules" then
      let provider_dir = Path.(package_path / provider_parent) in
      let rec scan_dir_recursive = fun ~from_dir ~rel_path ->
        match Fs.read_dir from_dir with
        | Error _ -> []
        | Ok iter ->
            let entries = Std.Iter.MutIterator.to_list iter in
...
(t, string) result = fun toml ~workspace_deps ~workspace_dev_deps ~workspace_build_deps ~path ~relative_path ->
  ...
  | Toml.Table items ->
      ...
      let dependencies = parse_dependency_section "dependencies" items ~workspace_deps in
      let dev_dependencies = parse_dependency_section "dev-dependencies" items ~workspace_deps:workspace_dev_deps in
      let build_dependencies = parse_dependency_section "build-dependencies" items ~workspace_deps:workspace_build_deps in
      let binaries =
  ...
  | Json.Object fields -> (
      ...
      | (Some (Json.String name), Some (Json.String path_str), Some (Json.String rel_path_str)) -> (
          ...
          | Ok path, Ok relative_path ->
              let parse_dependencies_field = fun field_name ->
              ...
              let build_dependencies = parse_dependencies_field "build_dependencies" in
              let binaries =
...
|}
    ...
    in
    let workspace_dep = fun name -> {name; source = Workspace} in
    let pkg = from_toml
    toml
