...
let is_built (cmd : t) : bool =
  match Fs.exists cmd.command_binary with
  | Ok true -> true
  | _ -> false

let status_string (cmd : t) : string =
  if is_built cmd then "ready" else "not built"

let parse_from_toml 
    (toml_entries : Toml.value list)
    ~(package_name : string)
    ~(package_path : Path.t)
    : t list =
  
  List.filter_map (fun cmd_toml ->
    match cmd_toml with
    | Toml.Table cmd_items ->
        ...
        | (Some (Toml.String name), Some (Toml.String path)) ->
            ...
            let command_binary = 
              Path.(v "_build" / v "debug" / v "out" / v package_name / v name)
            in
