...
type t =
  ...
  | RegistryLatestReleaseMissing of {
      package: string;
      latest_version: string
    }
  | PackageMetadataReadFailed of {
      package: string;
      registry: string;
      error: string
    }
...
let path_of_json = function
  | Json.String path -> (
      ...
      | Error (Path.SystemInvalidUtf8 { syscall; path = invalid_path }) ->
          Error ("invalid path '" ^ path ^ "': " ^ syscall ^ " returned invalid utf-8 for '" ^ invalid_path ^ "'")
      | Error (Path.SystemError err) ->
  ...
  | Json.Object fields -> (
      ...
      | Some (Json.String "ManifestReadFailed") -> (
          ...
          | Some path_json, Some (Json.String error) ->
              path_of_json path_json |> Result.map (fun manifest_path -> ManifestReadFailed { manifest_path; error })
          | _ -> Error "invalid ManifestReadFailed"
        )
      | Some (Json.String "ManifestParseFailed") -> (
          match List.assoc_opt "manifest_path" fields, List.assoc_opt "error" fields with
          | Some path_json, Some (Json.String error) ->
              path_of_json path_json |> Result.map (fun manifest_path -> ManifestParseFailed { manifest_path; error })
          | _ -> Error "invalid ManifestParseFailed"
        )
      | Some (Json.String "PathDependencyLoadFailed") -> (
          match List.assoc_opt "dependency_name" fields, List.assoc_opt "dependency_path" fields, List.assoc_opt "error" fields with
      ...
      | Some (Json.String "PathDependencyDecodeFailed") -> (
          match List.assoc_opt "dependency_name" fields, List.assoc_opt "manifest_path" fields, List.assoc_opt "error" fields with
          | Some (Json.String dependency_name), Some path_json, Some (Json.String error) ->
              path_of_json path_json
              |> Result.map (fun manifest_path -> PathDependencyDecodeFailed { dependency_name; manifest_path; error })
      ...
      | Some (Json.String "PackageMetadataReadFailed") -> (
          match List.assoc_opt "package" fields, List.assoc_opt "registry" fields, List.assoc_opt "error" fields with
      ...
      | Some (Json.String "PackageNotFound") -> (
          ...
          | Some (Json.String package), Some (Json.String registry) ->
                ...
                | Some (Json.Object required_by_fields) -> (
                    ...
                    | Some (Json.String package) ->
                        let path_result =
                          ...
                          | None -> Ok None
                          | Some path_json ->
                              path_of_json path_json |> Result.map (fun path -> Some path)
                        in
              ...
              in
              required_by |> Result.map (fun required_by -> PackageNotFound { package; registry; required_by })
      ...
      | Some (Json.String "LockfileReadFailed") -> (
          ...
          | Some path_json, Some (Json.String error) ->
              path_of_json path_json |> Result.map (fun path -> LockfileReadFailed { path; error })
      ...
      | Some (Json.String "LockRefreshCheckFailed") -> (
          ...
          | Some path_json, Some (Json.String error) ->
              path_of_json path_json |> Result.map (fun workspace_root -> LockRefreshCheckFailed { workspace_root; error })
          | _ -> Error "invalid LockRefreshCheckFailed"
        )
      | Some (Json.String "LockfileWriteFailed") -> (
          match List.assoc_opt "path" fields, List.assoc_opt "error" fields with
          | Some path_json, Some (Json.String error) ->
              path_of_json path_json |> Result.map (fun path -> LockfileWriteFailed { path; error })
