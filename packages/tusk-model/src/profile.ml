...
let release = {
  ...
  no_assert = true;
  compact = true;
  unsafe = false;  (* Keep safe by default *)
  no_alias_deps = false;
  open_modules = [];
  warnings = [];
...
let apply_override base (override : profile_override) = {
  name = base.name;
  kind = (match override.kind with Inherit -> base.kind | Override k -> k);
  inline = (match override.inline with Inherit -> base.inline | Override opt -> opt);
  no_assert = (match override.no_assert with Inherit -> base.no_assert | Override b -> b);
  compact = (match override.compact with Inherit -> base.compact | Override b -> b);
  unsafe = (match override.unsafe with Inherit -> base.unsafe | Override b -> b);
  no_alias_deps = (match override.no_alias_deps with Inherit -> base.no_alias_deps | Override b -> b);
  open_modules = (match override.open_modules with Inherit -> base.open_modules | Override l -> l);
...
let apply_overrides base overrides =
...
let override_from_toml (table_items : (string * Std.Data.Toml.value) list) : profile_override =
  let open Std.Data.Toml in
  let get_string_list key =
    match List.assoc_opt key table_items with
    | Some (Array arr) ->
        Override (List.filter_map (function String s -> Some s | _ -> None) arr)
    | _ -> Inherit
  in
  let get_int_opt key =
    match List.assoc_opt key table_items with
...
let from_toml (table_items : (string * Std.Data.Toml.value) list) ~(base : t) : t =
  let open Std.Data.Toml in
  let get_string_list key =
    match List.assoc_opt key table_items with
    | Some (Array arr) ->
        List.filter_map (function String s -> Some s | _ -> None) arr
    | _ -> base.open_modules (* Keep base value if not present *)
  in
  let get_int_opt key =
    match List.assoc_opt key table_items with
    | Some (String s) -> (
  ...
  in
  {
    ...
    open_modules = get_string_list "open_modules";
    warnings = base.warnings; (* TODO: Parse warnings *)
    errors = base.errors; (* TODO: Parse errors *)
    cc_flags = (match List.assoc_opt "cc_flags" table_items with
      | Some (Array arr) ->
          List.filter_map (function String s -> Some s | _ -> None) arr
      | _ -> base.cc_flags);
...
let to_compiler_flags profile =
  let flags = [] in
  
  (* Optimization flags *)
  let flags = match profile.inline with
    | None -> flags
    | Some n -> ("-inline " ^ Int.to_string n) :: flags
  in
  let flags = if profile.no_assert then "-noassert" :: flags else flags in
  let flags = if profile.compact then "-compact" :: flags else flags in
  let flags = if profile.unsafe then "-unsafe" :: flags else flags in
  
  (* Module handling *)
  let flags = if profile.no_alias_deps then "-no-alias-deps" :: flags else flags in
  let flags = List.fold_left (fun acc m -> "-open" :: m :: acc) flags profile.open_modules in
...
let hash state profile =
  let module H = Crypto.Sha256 in
  H.write_string state profile.name;
  H.write_string state (match profile.kind with 
...
let to_json profile =
  ...
  obj [
    ...
    ("kind", string (match profile.kind with 
      | Ocaml_compiler.Bytecode -> "bytecode" 
      | Native -> "native"));
