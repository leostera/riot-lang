...
let test_large_json_response () =
  ...
  (* Build URI *)
  let uri = Net.Uri.of_string "http://127.0.0.1:1234/v1/chat/completions" 
      ...
      | Ok () ->
          ...
          | Ok (response, body) ->
              ...
              (* The key test: verify the response body is complete JSON, not truncated *)
                ...
                Error ("Response body truncated! Length: " ^ string_of_int body_len ^ 
                       ", Content-Length: " ^ (Option.unwrap_or ~default:"missing" content_length_hdr))
              
              (* Try to parse as JSON - this will fail if truncated *)
              else match Data.Json.of_string body with
              | Error err ->
                  Error ("JSON parse failed (body may be truncated): " ^ 
                         Data.Json.error_to_string err ^ 
                         ", body length: " ^ string_of_int body_len)
              | Ok json ->
                  ...
                  | Data.Json.Object fields -> (
                      ...
                      | Some (Data.Json.Array _choices) ->
                          ...
                          Ok ()
                      | Some _ ->
                          Error "'choices' field is not an array"
                      | None ->
...
let test_sse_parsing () =
  ...
  | Ok conn ->
      ...
      | Ok () ->
          ...
          Log.info ("First event: " ^ (match first_event with Some _ -> "got one" | None -> "none"));
          
          let events = match first_event with
