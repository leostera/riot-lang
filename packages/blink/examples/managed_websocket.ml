...
let () =
  Runtime.run
    ~main:(fun ~args:_ ->
      let url = Env.var Env.String ~name:"BLINK_WS_URL" |> Option.unwrap_or ~default:"wss://echo.websocket.events" in
      let uri = Net.Uri.of_string url |> Result.expect ~msg:"invalid WebSocket URL" in
      ...
      in
      Client.WebSocket.send_text client websocket "hello from Blink.Client" |> Result.expect ~msg:"managed WebSocket send failed";
      (
        match Client.WebSocket.receive client websocket with
        | Ok (Client.WebSocket.Text payload) -> println ("text: " ^ payload)
        | Ok (Client.WebSocket.Binary payload) -> println
          ("binary bytes=" ^ Int.to_string (String.length payload))
