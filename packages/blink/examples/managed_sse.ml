...
let () =
  Runtime.run
    ~main:(fun ~args:_ ->
      let url = Env.var Env.String ~name:"BLINK_SSE_URL" |> Option.unwrap_or ~default:"https://example.com/events" in
      let uri = Net.Uri.of_string url |> Result.expect ~msg:"invalid SSE URL" in
      ...
      in
      Client.request client conn request () |> Result.expect ~msg:"managed SSE request failed";
