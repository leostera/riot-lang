...
let make: reader:IO.Reader.t -> writer:IO.Writer.t -> of_io_error:(IO.error -> Error.t) -> uri:Net.Uri.t -> t = fun ~reader ~writer ~of_io_error ~uri ->
  Conn {
...
let request = fun (Conn conn) req ?body () ->
  ...
  let request_line = (Net.Http.Method.to_string method_) ^ " " ^ resource ^ " " ^ (Net.Http.Version.to_string version) ^ "\r\n" in
  let headers =
    (
      headers |> fun h -> Net.Http.Header.add h "host" (Net.Uri.host conn.uri |> Option.unwrap_or ~default:"localhost")
  ...
  let headers =
    ...
    | None -> headers
  in
  let headers_str = Net.Http.Header.to_list headers |> List.map ~fn:(
...
let read_more = fun (Conn conn) ->
  ...
  | Ok _ ->
      ...
      let _ = Buffer.append_slice conn.buffer readable |> Result.expect ~msg:"failed to append response chunk" in Ok ()
  | Error e -> Error (conn.of_io_error e)
  ...
  | ReadingChunkedBody ->
      let rec parse_chunks acc =
        ...
        | Http.Http1.Common.Done { value = { data = chunk_data; remaining }; _ } ->
            ...
            else (* Return the chunk immediately for streaming support *)
...
let messages = fun ?(on_message = fun _ -> ()) conn ->
  ...
  let rec loop () =
    ...
    | Ok msgs ->
        on_message msgs;
        msgs |> List.for_each ~fn:(
          fun msg -> Vector.push acc ~value:msg
        );
...
let await = fun ?(on_message = fun _ -> ()) (Conn conn as c) ->
  ...
  | Ok msgs ->
      ...
      let body_chunks = List.filter_map msgs ~fn:(
        function
        | Data chunk -> Some chunk
        | _ -> None
      ) in
      let body = String.concat "" body_chunks in Ok (response, body)

let close = fun _conn -> ()(* Reader/writer don't need explicit close *)
