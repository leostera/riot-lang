...
val request :
  t -> Net.Http.Request.t -> ?body:string -> unit -> (unit, Error.t) result

val stream : t -> (message list, Error.t) result

val messages :
  ?on_message:(message list -> unit) -> t -> (message list, Error.t) result

val await :
  ?on_message:(message list -> unit) ->
  t ->
  (Net.Http.Response.t * string, Error.t) result
