...
type action =
  ...
  | Check of { paths: Path.t list; package_filter: string option }
  ...
  let open ArgParser in
    ...
    |> args
      [
        ...
        positional "path" |> required false |> multiple |> help "OCaml file(s) or directory(ies) to typecheck (default: workspace packages)";
      ]
