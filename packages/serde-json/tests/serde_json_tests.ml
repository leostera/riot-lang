...
let io_writer_of_buffer =
  let module Write = struct
    ...
    let write = fun buffer ~from ->
      ...
      IO.Buffer.append_slice buffer (IO.Buffer.readable from) |> Result.expect ~msg:"serde-json test writer should append buffer contents";
      Ok written
    ...
    let write_vectored = fun buffer ~from ->
      ...
      IO.IoVec.for_each ~fn:(
        fun chunk ->
          IO.Buffer.append_slice buffer chunk |> Result.expect ~msg:"serde-json test writer should append slices";
...
let prefix_fields = De.fields [ De.field "help" Prefix_help; De.field "hello" Prefix_hello; De.field "hellsinborg" Prefix_hellsinborg ]
...
let person_decode = De.record_mut ~fields:person_fields ~create:(
  fun (): person_builder ->
    {
      name = None;
      age = None;
      ...
      Ser.fields
        [
          Ser.field "name" Ser.string
            (
              fun (value: person) -> value.name
            );
          Ser.field "age" Ser.int
            (
              fun (value: person) -> value.age
            );
          Ser.field "active" Ser.bool
            (
              fun (value: person) -> value.active
            );
          Ser.field "tags" (Ser.list Ser.string)
            (
              fun (value: person) -> value.tags
            );
          Ser.field "nickname" (Ser.option Ser.string)
            (
              fun (value: person) -> value.nickname
            );
          Ser.field "pet" pet_encode
            (
              fun (value: person) -> value.pet
            );
        ]
    )

let prefix_decode = De.record_mut ~fields:prefix_fields ~create:(
  fun (): prefix_builder -> { help = None; hello = None; hellsinborg = None }
) ~step:(
  fun reader builder field ->
    match field with
...
let equal_person = fun (left: person) (right: person) -> String.equal left.name right.name && Int.equal left.age right.age && Bool.equal left.active right.active && vec_to_list left.tags = vec_to_list right.tags && left.nickname = right.nickname && left.pet = right.pet
...
let test_decodes_record_and_skips_unknown_fields = fun _ctx ->
  ...
  | Ok actual ->
      if equal_person actual expected then
        Ok ()
      else Error "expected serde-json decoder to parse a record and skip unknown fields"
...
let test_decodes_unit_variant = fun _ctx ->
  ...
  | Ok actual ->
      if equal_person actual expected then
        Ok ()
      else Error "expected serde-json decoder to handle string-form unit variants"
...
let test_decodes_numeric_scalars = fun _ctx ->
  ...
  in
  let* () = expect_ok De.int "-12345" (-12_345) "expected serde-json decoder to parse top-level ints" in expect_ok De.float "1.25e3" 1250.0 "expected serde-json decoder to parse top-level floats with exponents"
...
let test_roundtrips_record = fun _ctx ->
  ...
  | Ok actual ->
      if equal_person actual person then
        Ok ()
      else Error "expected serde-json encode/decode to roundtrip person values"
...
let test_roundtrips_large_float = fun _ctx ->
  ...
  | Ok actual -> Error ("expected serde-json to preserve large floats, got " ^ Float.to_string actual ^ " from " ^ encoded)
