...
let error_at = fun pos message ->
  raise (Core.Decode_error (`Msg (message ^ " at position " ^ Int.to_string pos)))
      ...
      if state.pos >= state.len then
        ...
        if Char.equal current '"' then (
          ...
          let rec loop () =
            ...
            | Some '"' ->
                advance state;
                Core.Fields.match_buffer cases state.scratch
            | Some '\\' ->
                advance state;
                (match current_char state with
                | None ->
...
let parse_int64 = fun state ->
  ...
  if number.is_float then
    raise (Core.Decode_error `invalid_field_type)
  else
...
let rec option_backend: 'value. state -> 'value Core.t -> 'value option =
 fun state (decode : 'value Core.t) ->
  skip_whitespace state;
  match current_char state with
  | Some 'n' ->
      parse_null state;
...
and list_backend: 'value. state -> 'value Core.t -> 'value list =
 fun state (decode : 'value Core.t) ->
...
and variant_backend: 'value. state -> 'value Core.variant_cases -> 'value =
 fun state (cases : 'value Core.variant_cases) ->
  let rec find_unit tag = function
    | [] ->
        raise (Core.Decode_error `invalid_tag)
    | Core.Unit (expected, value) :: _ when String.equal expected tag ->
        value
    | _ :: rest ->
        find_unit tag rest
  in
  let rec find_object tag = function
    ...
    | Core.Unit (expected, value) :: _ when String.equal expected tag ->
        parse_null state;
        value
    | Core.Newtype (expected, decode, wrap) :: _ when String.equal expected tag ->
...
and backend: state Core.backend = {
  bool = parse_bool;
  string = parse_string;
  int = parse_int;
  int32 = parse_int32;
...
let of_string = fun decode input ->
  ...
  let* value = Core.run decode backend state in
  skip_whitespace state;
