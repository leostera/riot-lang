...
let make_bench_search_packages = fun () ->
  ...
  fun () ->
    let _ = Pkgs_ml.Registry.search_packages registry ~query:"riot" ~limit:32 ()
    |> Result.expect ~msg:"search_packages should succeed" in
    ()
...
let make_bench_materialize_release_hit = fun () ->
  ...
  fun () ->
    let _ = Pkgs_ml.Registry.materialize_release registry ~package_name:"std" ~version
    |> Result.expect ~msg:"materialize_release hit should succeed" in
    ()
