...
let make_scratch_dir = fun prefix ->
  ...
  let path = Path.(temp_root () / Path.v "pkgs-ml-bench" / Path.v (prefix ^ "-" ^ pid ^ "-" ^ nanos ^ "-" ^ counter)) in
...
let make_bench_search_packages = fun () ->
  let cache = Pkgs_ml.Registry_cache.create ~riot_home:(make_scratch_dir "search-cache") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"create registry cache should succeed" in
  let packages = List.init ~count:256 ~fn:(
    fun index ->
      let name =
        if Int.rem index 8 = 0 then
          "riot-" ^ Int.to_string index
        else "pkg-" ^ Int.to_string index
      in
  ...
  fun () ->
    let _ = Pkgs_ml.Registry.search_packages registry ~query:"riot" ~limit:32 () |> Result.expect ~msg:"search_packages should succeed" in ()
...
let make_release_source = fun version ->
  {
    ...
    files = [ { Pkgs_ml.Registry.path = Path.v "src/std.ml"; contents = "let answer = 42\n" }; { Pkgs_ml.Registry.path = Path.v "README.md"; contents = "# std\n" } ]
  }

let make_bench_materialize_release_miss = fun () ->
  let cache = Pkgs_ml.Registry_cache.create ~riot_home:(make_scratch_dir "materialize-miss-cache") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"create registry cache should succeed" in
  let counter = ref 0 in
  fun () ->
    let version = "0.1." ^ Int.to_string !counter in
    counter := !counter + 1;
    let registry = Pkgs_ml.Registry.in_memory ~cache ~packages:[] ~releases:[ make_release_source version ] () in
    let _ = Pkgs_ml.Registry.materialize_release registry ~package_name:"std" ~version |> Result.expect ~msg:"materialize_release miss should succeed" in ()

let make_bench_materialize_release_hit = fun () ->
  let cache = Pkgs_ml.Registry_cache.create ~riot_home:(make_scratch_dir "materialize-hit-cache") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"create registry cache should succeed" in
  let version = "1.0.0" in
  let registry = Pkgs_ml.Registry.in_memory ~cache ~packages:[] ~releases:[ make_release_source version ] () in
  let _ = Pkgs_ml.Registry.materialize_release registry ~package_name:"std" ~version |> Result.expect ~msg:"initial materialize_release should succeed" in
  fun () ->
    let _ = Pkgs_ml.Registry.materialize_release registry ~package_name:"std" ~version |> Result.expect ~msg:"materialize_release hit should succeed" in ()
