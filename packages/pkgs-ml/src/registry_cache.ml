...
let create = fun ?tusk_home ~registry_name ->
  ...
  | None -> (
      ...
      | Some home ->
          Ok {
            tusk_home = Path.(home / Path.v ".tusk");
            registry_name;
          }
      | None ->
          Error "failed to determine home directory for pkgs.ml cache"
    )
...
let index_dir = fun cache ->
  Path.(registry_dir cache / Path.v "index")

let archive_dir = fun cache ->
  Path.(registry_dir cache / Path.v "archive")

let archive_path = fun cache ~package_name ~version ->
  Path.(archive_dir cache / Path.v package_name / Path.v (version ^ ".tar"))

let src_dir = fun cache ->
  Path.(registry_dir cache / Path.v "src")
...
module Tests = struct
  let test_registry_split_layout () : (unit, string) result =
    let cache =
      create ~tusk_home:(Path.v "/tmp/.tusk") ~registry_name:"pkgs.ml"
      |> Result.unwrap
    in
