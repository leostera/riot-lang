...
type t = { riot_home: Path.t; registry_name: string }
...
let create = fun ?riot_home ~registry_name () ->
  ...
  | None -> (
    ...
    | Some home -> Ok { riot_home = Path.(home / Path.v ".riot"); registry_name }
...
let registry_dir = fun cache -> Path.(cache.riot_home / Path.v "registry" / Path.v cache.registry_name)
...
let archive_path = fun cache ~package_name ~version -> Path.(archive_dir cache / Path.v package_name / Path.v (version ^ ".tar"))

let src_dir = fun cache -> Path.(registry_dir cache / Path.v "src")

let package_src_dir = fun cache ~package_name ~version -> Path.(src_dir cache / Path.v package_name / Path.v version)
