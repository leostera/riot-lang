...
let field_value = fun fields ~field -> List.find fields ~fn:(
  fun (name, _) -> String.equal name field
) |> Option.map ~fn:(
  fun (_, value) -> value
)
...
let release_of_json = fun json ->
  ...
  | Data.Json.Object fields ->
      let* _schema_version =
        ...
        | Error _ -> Ok 1
      in
      let* version = string_field ~context:"release" ~field:"version" fields
      in
      let* published_at = string_field ~context:"release" ~field:"published_at" fields
      in
      let* canonical_locator = string_field ~context:"release" ~field:"canonical_locator" fields
      in
      let* repo_url = string_field ~context:"release" ~field:"repo_url" fields
      in
      let* subdir = string_field ~context:"release" ~field:"subdir" fields
      in
      let* artifact_sha256 = string_field_with_fallback ~context:"release" ~field:"artifact_sha256" ~fallback:"sha" fields
      in
      let* description = optional_string_field ~field:"description" fields
      in
      let* license = optional_string_field ~field:"license" fields
      in
      let* homepage = optional_string_field ~field:"homepage" fields
      in
      let* repository = optional_string_field ~field:"repository" fields
      in
      let* root_module = optional_string_field ~field:"root_module" fields
      in
      let* categories = optional_string_list_field ~field:"categories" fields
      in
      let* keywords = optional_string_list_field ~field:"keywords" fields
      in
      let* manifest_key = string_field ~context:"release" ~field:"manifest_key" fields
      in
      let* source_key = string_field ~context:"release" ~field:"source_key" fields
      in
      let* dependency_json = object_field ~context:"release" ~field:"dependencies" fields
      in
      let* dependencies = dependencies_of_json dependency_json
      in
      let* yanked =
      ...
      in
      let* yanked_at = optional_string_field ~field:"yanked_at" fields
      in
      let* yanked_by_github_login = optional_string_field ~field:"yanked_by_github_login" fields
      in
      Ok {
        version;
        published_at;
...
let config_of_json = fun json ->
  match json with
  | Data.Json.Object fields ->
      let* schema_version = int_field ~context:"config" ~field:"schema_version" fields
      in
      let* kind = string_field ~context:"config" ~field:"kind" fields
      in
      let* package_path_strategy = string_field ~context:"config" ~field:"package_path_strategy" fields
      in
      let* index_base_url = string_field ~context:"config" ~field:"index_base_url" fields
      in
      let* artifact_base_url = string_field ~context:"config" ~field:"artifact_base_url" fields
      in
      Ok {
        schema_version;
        kind;
...
let package_document_of_json = fun json ->
  match json with
  | Data.Json.Object fields ->
      let* schema_version = int_field ~context:"package document" ~field:"schema_version" fields
      in
      let* name = string_field ~context:"package document" ~field:"name" fields
      in
      let* latest = string_field ~context:"package document" ~field:"latest" fields
      in
      let* updated_at = string_field ~context:"package document" ~field:"updated_at" fields
      in
      let* releases_json = object_field ~context:"package document" ~field:"releases" fields
      in
      let* releases = releases_of_json releases_json
      in
      Ok {
        schema_version;
        name;
...
let package_relpath = fun package_name ->
  let name = normalized_name package_name in Path.(package_prefix name / Path.v (name ^ ".json"))
...
let package_document_url = fun config ~package_name ->
  ...
  | Ok base -> Net.Uri.join base (Path.to_string (package_relpath package_name)) |> Result.map_err ~fn:(
...
let release_source_url = fun config (release: release) ->
  ...
  | Ok base -> Net.Uri.join base release.source_key |> Result.map_err ~fn:(
    fun _ -> "failed to build sparse index archive url for '" ^ release.source_key ^ "'"
  )

let package_cache_path = fun cache ~package_name -> Path.(Registry_cache.index_dir cache / package_relpath package_name)
...
let read_cached_json = fun ~path ~decode ->
  ...
  | Ok true -> (
    ...
    | Error err -> Error ("failed to read sparse index file '" ^ Path.to_string path ^ "': " ^ IO.error_message err)
    | Ok source -> (
...
let read_cached_package_document = fun cache ~package_name -> read_cached_json ~path:(package_cache_path cache ~package_name) ~decode:package_document_of_string
...
let write_cached_json = fun ~path ~source ->
  ...
  | Error err -> Error ("failed to create sparse index parent directory for '" ^ Path.to_string path ^ "': " ^ IO.error_message err)
  | Ok () -> (
    match Fs.write source path with
    | Ok () -> Ok ()
    | Error err -> Error ("failed to write sparse index file '" ^ Path.to_string path ^ "': " ^ IO.error_message err)
  )

let write_cached_config = fun cache ~source -> write_cached_json ~path:(config_cache_path cache) ~source

let write_cached_package_document = fun cache ~package_name ~source -> write_cached_json ~path:(package_cache_path cache ~package_name) ~source
...
module Tests = struct
  let expect_relpath = fun ~package_name ~expected ->
    ...
    if String.equal actual expected then
      Ok ()
    else Error ("expected sparse index path '" ^ expected ^ "', got '" ^ actual ^ "'")
  ...
  let test_names_are_normalized_to_lowercase () = expect_relpath ~package_name:"AbCd" ~expected:"ab/cd/abcd.json" [@test]
  ...
  "artifact_base_url": "https://cdn.pkgs.ml"
    ...
    | Ok config ->
        if config.schema_version = 1 && String.equal config.kind "sparse" && String.equal config.package_path_strategy "cargo-lowercase-v1" && String.equal config.index_base_url "https://cdn.pkgs.ml/index/v1" && String.equal config.artifact_base_url "https://cdn.pkgs.ml" then
          Ok ()
        else Error "unexpected sparse index config contents"
  ...
  "releases": [
    ...
    | Ok document -> (
      ...
      | [ release ] ->
          if document.schema_version = 1 && String.equal document.name "kernel" && String.equal document.latest "0.0.1" && String.equal release.version "0.0.1" && String.equal release.manifest_key "packages/kernel/0.0.1/2aef0372bf5b6687db05bda80cde55f960cbfd9d.manifest.json" && List.length release.dependencies = 1 && (
            match List.get release.dependencies ~at:0 with
            | Some dependency -> String.equal dependency.name "std"
            | None -> false
          ) then
            Ok ()
          else Error "unexpected sparse index package document contents"
