...
let assoc_value = fun entries ~key -> List.find entries ~fn:(
  fun (entry_key, _) -> entry_key = key
) |> Option.map ~fn:(
  fun (_, value) -> value
)
...
type search_result = { package_name: string; latest_version: string; description: string option }
...
type release_file = { path: Path.t; contents: string }
...
type t = { cache: Registry_cache.t; fetch: fetch; source: source }
...
let blink_error_message = function
  ...
  | Blink.Error.TlsError (Net.TlsStream.Network_read_failed tcp_err) -> "tls read failed: " ^ io_error_message tcp_err
  | Blink.Error.TlsError (Net.TlsStream.Network_write_failed tcp_err) -> "tls write failed: " ^ io_error_message tcp_err
...
let default_http_headers = fun headers ->
  let has_riot_agent = List.any headers ~fn:(
...
let make_fetch = fun ~get ?post () ->
  let post =
    match post with
    | Some post ->
        fun uri ~headers ~body -> post uri ~headers:(default_http_headers headers) ~body
    | None ->
        fun _ ~headers:_ ~body:_ -> Error "POST fetch is not configured"
  in
  { get; post }
...
let default_fetch =
  let run method_ uri ~headers ?body () =
    ...
    try
      ...
      | Ok conn ->
          let request = List.fold_left headers ~init:(Net.Http.Request.create method_ uri) ~fn:(
          ...
          let response =
            try
              ...
              | Ok () -> (
                ...
                | Error err -> Error (blink_error_message err)
                | Ok (response, body) -> Ok { status_code = Net.Http.Status.to_int (Net.Http.Response.status response); body }
              )
            with
  ...
  in
  make_fetch ~get:(
    fun uri -> run Net.Http.Method.Get uri ~headers:[] ()
  ) ~post:(
    fun uri ~headers ~body -> run Net.Http.Method.Post uri ~headers ~body ()
  ) ()
...
let in_memory = fun ?config ~cache ?(releases = []) ~packages () ->
  let packages = List.map packages ~fn:(
    fun (document: Sparse_index.package_document) -> (Sparse_index.normalized_name document.name, document)
  ) in
  let releases = List.map releases ~fn:(
    fun (release: release_source) -> (release_key ~package_name:release.package_name ~version:release.version, release)
  ) in { cache; fetch = default_fetch; source = In_memory { config; packages; releases } }

let http_status_message = fun status_code ->
  let status = Net.Http.Status.of_int status_code in Int.to_string status_code ^ " " ^ Net.Http.Status.reason_phrase status

let protect_fetch = fun ~uri f ->
  try f () with
  | exn ->
      let _ = uri in Error (exn_message exn)

let fetch_required = fun registry uri ->
  match protect_fetch ~uri
    (
      fun () -> registry.fetch.get uri
    ) with
  | Ok { status_code = 200; body } -> Ok body
  | Ok { status_code; _ } -> Error ("request to '" ^ Net.Uri.to_string uri ^ "' failed with " ^ http_status_message status_code)
...
let post_required = fun registry uri ~headers ~body ->
  match protect_fetch ~uri
    (
      fun () -> registry.fetch.post uri ~headers ~body
    ) with
  | Ok { status_code = 200; body } -> Ok body
  | Ok { status_code; body } -> (
    ...
    | Ok (Data.Json.Object fields) -> (
      ...
      | _ -> Error ("request to '" ^ Net.Uri.to_string uri ^ "' failed with " ^ http_status_message status_code)
    )
    | Ok _ | Error _ -> Error ("request to '" ^ Net.Uri.to_string uri ^ "' failed with " ^ http_status_message status_code)
  )
  | Error err -> Error err
...
let cache_path_is_fresh = fun path ->
  ...
  | Error _ -> Ok false
  | Ok metadata ->
      let now = Time.SystemTime.now () |> Time.SystemTime.secs_float in Ok ((now -. Fs.Metadata.modified metadata) <= sparse_index_cache_ttl_secs)
...
let published_artifact_location_of_json = fun ~context json ->
  ...
  | Data.Json.Object fields ->
      let* key = string_field ~context ~field:"key" fields
      in
      let* url = string_field_with_fallback ~context ~field:"url" ~fallback:"cdn_url" fields in Ok { key; url }
...
let published_release_of_json = fun json ->
  match json with
  | Data.Json.Object fields ->
      let* artifact_sha256 = string_field ~context:"publish response" ~field:"artifact_sha256" fields
      in
      let* package_name = string_field ~context:"publish response" ~field:"package_name" fields
      in
      let* package_version = string_field ~context:"publish response" ~field:"package_version" fields
      in
      let* manifest_json = object_field ~context:"publish response" ~field:"manifest" fields
      in
      let* source_archive_json = object_field ~context:"publish response" ~field:"source_archive" fields
      in
      let* claim_json = object_field ~context:"publish response" ~field:"claim" fields
      in
      let* release_json = object_field ~context:"publish response" ~field:"release" fields
      in
      let* materialization_json = object_field ~context:"publish response" ~field:"materialization" fields
      in
      let* manifest = published_artifact_location_of_json ~context:"publish response.manifest" manifest_json
      in
      let* source_archive = published_artifact_location_of_json ~context:"publish response.source_archive" source_archive_json
      in
      let* claim = published_record_of_json ~context:"publish response.claim" claim_json
      in
      let* release = published_record_of_json ~context:"publish response.release" release_json
      in
      let* materialization = published_materialization_of_json ~context:"publish response.materialization" materialization_json
      in
      Ok {
        artifact_sha256;
        package_name;
...
let yanked_release_of_json = fun json ->
  match json with
  | Data.Json.Object fields ->
      let* package_name = string_field ~context:"yank response" ~field:"package_name" fields
      in
      let* package_version = string_field ~context:"yank response" ~field:"package_version" fields
      in
      let* yanked = bool_field ~context:"yank response" ~field:"yanked" fields
      in
      let* yanked_at = optional_string_field ~context:"yank response" ~field:"yanked_at" fields
      in
      let* yanked_by_github_login = optional_string_field ~context:"yank response" ~field:"yanked_by_github_login" fields
      in
      Ok {
        package_name;
        package_version;
...
let yank_release_url = fun ~registry_name ~package_name ~version ->
  let url = "https://api." ^ registry_name ^ "/v1/me/packages/" ^ package_name ^ "/versions/" ^ version ^ "/yank" in
  match Net.Uri.of_string url with
  | Ok uri -> Ok uri
  | Error _ -> Error ("failed to build yank url '" ^ url ^ "'")

let search_url = fun ~registry_name ~query ~limit ->
  let url = "https://api." ^ registry_name ^ "/v1/search?q=" ^ Net.Uri.form_encode query ^ "&limit=" ^ Int.to_string limit in
  match Net.Uri.of_string url with
  | Ok uri -> Ok uri
...
let search_result_of_json = fun json ->
  match json with
  | Data.Json.Object fields ->
      let* package_name = string_field ~context:"search result" ~field:"package_name" fields
      in
      let* latest_version = string_field ~context:"search result" ~field:"latest_version" fields
      in
      let* description = optional_string_field ~context:"search result" ~field:"description" fields in Ok { package_name; latest_version; description }
...
let read_package_document = fun registry ~package_name ->
  let fetch_package_document config =
    ...
    | Ok uri -> (
      ...
      | Ok (Some source) -> (
        ...
        | Error err -> Error ("failed to decode sparse index package document from '" ^ Net.Uri.to_string uri ^ "': " ^ err)
        | Ok document -> (
  ...
  | In_memory { packages; config = _; releases = _ } -> Ok (assoc_value packages ~key:(Sparse_index.normalized_name package_name))
...
let search_packages = fun registry ~query ?(limit = 5) () ->
  if String.equal (String.trim query) "" then
    ...
    | Filesystem -> (
      ...
      | Ok uri -> (
        ...
        | Ok source -> (
          ...
          | Error err -> Error ("failed to decode search response from '" ^ Net.Uri.to_string uri ^ "': " ^ (Data.Json.error_to_string err))
          | Ok json -> (
            match search_results_of_json json with
            | Ok results -> Ok results
            | Error err -> Error ("failed to decode search response from '" ^ Net.Uri.to_string uri ^ "': " ^ err)
          )
        )
      )
    )
    | In_memory { packages; config = _; releases = _ } ->
        ...
        packages |> List.map ~fn:(
          fun (_, document) -> document
        ) |> List.filter ~fn:(
          fun (document: Sparse_index.package_document) ->
            let normalized_name = Sparse_index.normalized_name document.name in String.equal normalized_name normalized_query || String.starts_with ~prefix:normalized_query normalized_name || String.contains normalized_name normalized_query
        ) |> List.sort ~compare:(
          fun (left: Sparse_index.package_document) (right: Sparse_index.package_document) -> String.compare left.name right.name
        ) |> take limit |> List.map ~fn:(
          fun (document: Sparse_index.package_document) ->
            {
              package_name = document.name;
              latest_version = document.latest;
              description = match document.releases with
...
let write_release_files = fun ~root (release: release_source) ->
  ...
  | Error err -> Error ("failed to create package source directory '" ^ Path.to_string root ^ "': " ^ IO.error_message err)
  | Ok () -> (
    match Fs.write release.manifest_toml manifest_path with
    | Error err -> Error ("failed to write package manifest '" ^ Path.to_string manifest_path ^ "': " ^ IO.error_message err)
    | Ok () ->
        let rec loop = function
          ...
          | (file: release_file) :: rest ->
              ...
              | Error err -> Error ("failed to create package source parent '" ^ Path.to_string file_path ^ "': " ^ IO.error_message err)
              | Ok () -> (
                match Fs.write file.contents file_path with
                | Error err -> Error ("failed to write package source file '" ^ Path.to_string file_path ^ "': " ^ IO.error_message err)
                | Ok () -> loop rest
              )
        in
        loop release.files
...
let cached_archive_is_gzip = fun archive_path ->
  ...
  | Error err -> Error ("failed to open cached package archive '" ^ Path.to_string archive_path ^ "': " ^ Fs.File.error_to_string err)
  | Ok file ->
      protect ~finally:(
        fun () ->
        ...
        (
          fun () ->
            ...
            | Error err -> Error ("failed to read cached package archive '" ^ Path.to_string archive_path ^ "': " ^ Fs.File.error_to_string err)
            | Ok bytes_read -> Ok (bytes_read = 2 && Char.equal (IO.Bytes.get_unchecked magic ~at:0) '\x1f' && Char.equal (IO.Bytes.get_unchecked magic ~at:1) '\x8b')
        )
...
let extract_cached_archive = fun ~archive_path ~root ->
  ...
  | Ok () -> (
    let fail detail =
      let _ = Fs.remove_dir_all root in Error ("failed to extract cached package archive '" ^ Path.to_string archive_path ^ "': " ^ detail)
    in
    match cached_archive_is_gzip archive_path with
    | Error _ as err ->
        let _ = Fs.remove_dir_all root in err
    | Ok true -> (
      match Fs.File.open_read archive_path with
...
let read_dir_entries = fun dir ->
  ...
  | Error err -> Error ("failed to read directory '" ^ Path.to_string dir ^ "': " ^ IO.error_message err)
  | Ok iter -> Ok (Iter.MutIterator.to_list iter)

let move_directory_contents = fun ~src ~dst ->
  let* entries = read_dir_entries src
  in
  let rec loop = function
    | [] -> Ok ()
    | entry :: rest ->
        ...
        let* () = Fs.rename ~src:from_path ~dst:to_path |> Result.map_err ~fn:(
          fun err -> "failed to move extracted entry '" ^ Path.to_string from_path ^ "' into '" ^ Path.to_string dst ^ "': " ^ IO.error_message err
        ) in loop rest
  in
  loop entries
...
let normalize_legacy_package_root = fun ~root ~(release:Sparse_index.release) ->
  ...
  | Error err -> Error ("failed to check package manifest '" ^ Path.to_string manifest_path ^ "': " ^ IO.error_message err)
  | Ok true -> Ok ()
  | Ok false ->
      let* entries = read_dir_entries root
      in
      let top_level_dirs = List.filter_map entries ~fn:(
        fun entry ->
      ...
      | None -> Error ("materialized archive did not contain riot.toml at package root '" ^ Path.to_string root ^ "'")
      | Some candidate_root ->
          let candidate_manifest = Path.(candidate_root / Path.v "riot.toml") in
          match Fs.exists candidate_manifest with
          | Error err -> Error ("failed to check candidate manifest '" ^ Path.to_string candidate_manifest ^ "': " ^ IO.error_message err)
          | Ok false -> Error ("materialized archive did not contain riot.toml at package root '" ^ Path.to_string root ^ "'")
          | Ok true ->
              let top_level_dir =
                match top_level_dirs with
                ...
                | [] -> root
              in
              let* () = move_directory_contents ~src:candidate_root ~dst:root
              in
              let* () = Fs.remove_dir_all top_level_dir |> Result.map_err ~fn:(
                fun err -> "failed to clean extracted archive root '" ^ Path.to_string top_level_dir ^ "': " ^ IO.error_message err
              )
              in
              match Fs.exists manifest_path with
              | Ok true -> Ok ()
              | Ok false -> Error ("normalized archive for '" ^ release.canonical_locator ^ "' is still missing riot.toml at '" ^ Path.to_string manifest_path ^ "'")
              | Error err -> Error ("failed to check normalized manifest '" ^ Path.to_string manifest_path ^ "': " ^ IO.error_message err)

let reset_materialized_root = fun root ->
  match Fs.exists root with
  | Error err -> Error ("failed to check package source directory '" ^ Path.to_string root ^ "': " ^ IO.error_message err)
  | Ok false -> Ok ()
  | Ok true -> Fs.remove_dir_all root |> Result.map_err ~fn:(
    fun err -> "failed to clean package source directory '" ^ Path.to_string root ^ "': " ^ IO.error_message err
  )

let find_release = fun registry ~package_name ~version ->
  match read_package_document registry ~package_name with
  | Error _ as err -> err
  | Ok None -> Error ("package '" ^ package_name ^ "' was not found in registry '" ^ name registry ^ "'")
  | Ok (Some document) -> (
    match List.find document.releases ~fn:(
      fun (release: Sparse_index.release) -> String.equal release.version version
    ) with
    | Some release when release.yanked -> Error ("package '" ^ package_name ^ "@" ^ version ^ "' was yanked from registry '" ^ name registry ^ "'")
    | Some release -> Ok release
    | None -> Error ("package '" ^ package_name ^ "' does not have release '" ^ version ^ "' in registry '" ^ name registry ^ "'")
  )
...
let write_cached_archive = fun ~archive_path ~contents ->
  ...
  | Error err -> Error ("failed to create package archive directory '" ^ Path.to_string archive_parent ^ "': " ^ IO.error_message err)
  | Ok () -> (
    match Fs.write contents temp_path with
    | Error err -> Error ("failed to write cached package archive '" ^ Path.to_string archive_path ^ "': " ^ IO.error_message err)
    | Ok () -> (
      match Fs.rename ~src:temp_path ~dst:archive_path with
      | Ok () -> Ok ()
      | Error err -> Error ("failed to finalize cached package archive '" ^ Path.to_string archive_path ^ "': " ^ IO.error_message err)
    )
  )

let remove_cached_archive = fun archive_path ->
  match Fs.exists archive_path with
  | Error err -> Error ("failed to check cached package archive '" ^ Path.to_string archive_path ^ "': " ^ IO.error_message err)
  | Ok false -> Ok ()
  | Ok true -> Fs.remove_file archive_path |> Result.map_err ~fn:(
    fun err -> "failed to remove cached package archive '" ^ Path.to_string archive_path ^ "': " ^ IO.error_message err
  )
...
let materialize_release = fun registry ~package_name ~version ->
  ...
  let finalize_extracted_root () =
    ...
    | Ok false ->
        let* release = find_release registry ~package_name ~version in normalize_legacy_package_root ~root ~release |> Result.map ~fn:(
          fun () -> `Materialized
        )
  in
  ...
  let extract_with_single_retry () =
    let rec attempt remaining_retries =
      ...
      | Error _ ->
          let* () = reset_materialized_root root
          in
          let* () = remove_cached_archive archive_path
          in
          let* () = fetch_release_archive registry ~package_name ~version ~archive_path in attempt (remaining_retries - 1)
    in
    attempt 1
  in
  match Fs.exists manifest_path with
  ...
  | Ok false -> (
    ...
    | In_memory { releases; config = _; packages = _ } -> (
      ...
      | None -> Error ("in-memory registry is missing source contents for '" ^ package_name ^ "@" ^ version ^ "'")
      | Some release ->
...
let publish_response = fun registry uri ~api_token ~artifact ->
  match post_required registry uri ~headers:[
...
let publish_from_locator = fun registry ~locator:_ ~selector:_ ~api_token ~artifact -> publish_artifact registry ~api_token ~artifact
