...
let test_registry_split_layout = fun _ctx ->
  let cache = Pkgs_ml.Registry_cache.create ~riot_home:(Path.v "/tmp/.riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to be created" in
  let index = Pkgs_ml.Registry_cache.index_dir cache |> Path.to_string in
  let archive = Pkgs_ml.Registry_cache.archive_path cache ~package_name:"std" ~version:"0.1.0" |> Path.to_string in
  let src = Pkgs_ml.Registry_cache.package_src_dir cache ~package_name:"std" ~version:"0.1.0" |> Path.to_string in
  if String.equal index "/tmp/.riot/registry/pkgs.ml/index" && String.equal archive "/tmp/.riot/registry/pkgs.ml/archive/std/0.1.0.tar" && String.equal src "/tmp/.riot/registry/pkgs.ml/src/std/0.1.0" then
    Ok ()
  else Error ("unexpected registry layout:\nindex=" ^ index ^ "\narchive=" ^ archive ^ "\nsrc=" ^ src)
...
let test_sparse_index_layout = fun _ctx ->
  ...
  if String.equal actual "/tmp/.riot/registry/pkgs.ml/index/ab/cd/abcd.json" then
    Ok ()
  else Error ("unexpected sparse index cache path: " ^ actual)
...
let test_sparse_index_document_parsing = fun _ctx ->
  ...
  | Ok document ->
      if String.equal document.name "kernel" && String.equal document.latest "0.0.1" && List.length document.releases = 1 && (
        match List.head document.releases with
        | Some release -> release.yanked
        | None -> false
      ) then
        Ok ()
      else Error "unexpected sparse index document contents"
  | Error err -> Error err

let test_registry_materialize_release_rejects_yanked_versions = fun _ctx ->
  let cache = Pkgs_ml.Registry_cache.create ~riot_home:(Path.v "/tmp/.riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to be created" in
  let document = Pkgs_ml.Sparse_index.package_document_of_string {|{
  "schema_version": 1,
  "name": "kernel",
  "latest": "0.0.1",
  "updated_at": "2026-03-27T15:27:35Z",
...
}|} |> Result.expect ~msg:"expected sparse index package to parse" in
...
let test_sparse_index_cached_reads = fun _ctx ->
  let config = Pkgs_ml.Sparse_index.config_of_string {|{
  "schema_version": 1,
  "kind": "sparse",
  "package_path_strategy": "cargo-lowercase-v1",
  "index_base_url": "https://cdn.pkgs.ml/index/v1",
  "artifact_base_url": "https://cdn.pkgs.ml"
}|} |> Result.expect ~msg:"expected sparse index config to parse" in
  let package = Pkgs_ml.Sparse_index.package_document_of_string {|{
  "schema_version": 1,
  "name": "kernel",
  "latest": "0.0.1",
  "updated_at": "2026-03-27T15:27:35Z",
  ...
  match Pkgs_ml.Registry.read_config registry, Pkgs_ml.Registry.read_package_document registry ~package_name:"Kernel" with
  | Ok (Some actual_config), Ok (Some actual_package) when String.equal actual_config.kind "sparse" && String.equal actual_package.name "kernel" -> Ok ()
...
let make_fetch_recorder = fun ?post_handler get_handler ->
  ...
  ) post_handler) () in (fetch, requests)

let with_riot_agent = fun value f ->
  Pkgs_ml.Registry.set_riot_agent value;
  try
    let result = f () in
    let _ = Pkgs_ml.Registry.set_riot_agent None in result
  with
  | exn ->
      let _ = Pkgs_ml.Registry.set_riot_agent None in raise exn
...
let test_filesystem_registry_fetches_config_on_cache_miss = fun _ctx ->
  match Fs.with_tempdir ~prefix:"pkgs_ml_fetch_config"
    (
      fun tempdir ->
        let cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(tempdir / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to be created" in
        let fetch, requests =
          make_fetch_recorder
            (
              fun uri ->
                ...
                else Error ("unexpected fetch url " ^ Net.Uri.to_string uri)
            )
        in
        ...
        | Ok (Some config) -> (
          ...
          | Ok (Some cached) ->
              let requested = List.reverse !requests |> List.map ~fn:(
                fun request -> request.url
              ) in
              if String.equal config.kind "sparse" && String.equal cached.index_base_url "https://cdn.pkgs.ml/index/v1" && requested = [ "https://cdn.pkgs.ml/index/v1/config.json" ] then
                Ok ()
              else Error "unexpected fetched sparse index config state"
        )
    ) with
...
let test_filesystem_registry_fetches_package_document_on_cache_miss = fun _ctx ->
  match Fs.with_tempdir ~prefix:"pkgs_ml_fetch_package"
    (
      fun tempdir ->
        let cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(tempdir / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to be created" in
        let fetch, requests =
          make_fetch_recorder
            (
              fun uri ->
                ...
                | "https://cdn.pkgs.ml/index/v1/config.json" -> Ok { Pkgs_ml.Registry.status_code = 200; body = sparse_index_config_json }
                | "https://cdn.pkgs.ml/index/v1/ke/rn/kernel.json" -> Ok { Pkgs_ml.Registry.status_code = 200; body = sparse_index_kernel_json }
        ...
        | Ok (Some document) -> (
          match Pkgs_ml.Sparse_index.read_cached_config cache, Pkgs_ml.Sparse_index.read_cached_package_document cache ~package_name:"Kernel" with
          | (Error err, _) | (_, Error err) -> Error err
          | (Ok None, _) | (_, Ok None) -> Error "expected fetched sparse index files to be cached"
          | Ok (Some _), Ok (Some cached) ->
              let requested = List.reverse !requests |> List.map ~fn:(
                fun request -> request.url
              ) in
              if String.equal document.name "kernel" && String.equal cached.name "kernel" && requested = [ "https://cdn.pkgs.ml/index/v1/config.json"; "https://cdn.pkgs.ml/index/v1/ke/rn/kernel.json" ] then
                Ok ()
              else Error "unexpected fetched sparse index package document state"
        )
    ) with
  | Error err -> Error (IO.error_message err)
  | Ok result -> result
...
let test_filesystem_registry_returns_none_for_missing_package_document = fun _ctx ->
  match Fs.with_tempdir ~prefix:"pkgs_ml_fetch_missing_package"
    (
      fun tempdir ->
        let cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(tempdir / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to be created" in
        let fetch, requests =
          make_fetch_recorder
            (
              fun uri ->
                match Net.Uri.to_string uri with
                | "https://cdn.pkgs.ml/index/v1/config.json" -> Ok { Pkgs_ml.Registry.status_code = 200; body = sparse_index_config_json }
                | "https://cdn.pkgs.ml/index/v1/mi/ss/missing.json" -> Ok { Pkgs_ml.Registry.status_code = 404; body = "" }
                | url -> Error ("unexpected fetch url " ^ url)
            )
        in
        let registry = Pkgs_ml.Registry.filesystem ~fetch cache in
        ...
        | Ok None -> (
          ...
          | Ok None ->
              ...
              if requested = [ "https://cdn.pkgs.ml/index/v1/config.json"; "https://cdn.pkgs.ml/index/v1/mi/ss/missing.json" ] then
                Ok ()
              else Error "unexpected sparse index fetch sequence for missing package document"
        )
    ) with
  | Error err -> Error (IO.error_message err)
  | Ok result -> result
...
let test_filesystem_registry_reuses_fresh_cached_config_without_fetch = fun _ctx ->
  match Fs.with_tempdir ~prefix:"pkgs_ml_fresh_config_cache"
    (
      fun tempdir ->
        let cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(tempdir / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to be created" in
        let* () = Pkgs_ml.Sparse_index.write_cached_config cache ~source:sparse_index_config_json
        in
        let fetch, requests =
          make_fetch_recorder
            (
              fun uri -> Error ("unexpected fetch url " ^ Net.Uri.to_string uri)
            )
        in
        let registry = Pkgs_ml.Registry.filesystem ~fetch cache in
        match Pkgs_ml.Registry.read_config registry with
        | Ok (Some config) when String.equal config.index_base_url "https://cdn.pkgs.ml/index/v1" && !requests = [] -> Ok ()
...
let test_filesystem_registry_refetches_stale_cached_config = fun _ctx ->
  match Fs.with_tempdir ~prefix:"pkgs_ml_stale_config_cache"
    (
      fun tempdir ->
        let cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(tempdir / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to be created" in
        let* () = Pkgs_ml.Sparse_index.write_cached_config cache ~source:sparse_index_config_json_stale
        in
        let* () = set_old_mtime (Pkgs_ml.Sparse_index.config_cache_path cache)
        in
        let fetch, requests =
          make_fetch_recorder
            (
        ...
        let registry = Pkgs_ml.Registry.filesystem ~fetch cache in
        match Pkgs_ml.Registry.read_config registry with
        | Ok (Some config) when String.equal config.index_base_url "https://cdn.pkgs.ml/index/v1" && List.map (List.reverse !requests) ~fn:(
          fun request -> request.url
...
let test_filesystem_registry_refetches_stale_cached_package_document = fun _ctx ->
  match Fs.with_tempdir ~prefix:"pkgs_ml_stale_package_cache"
    (
      fun tempdir ->
        let cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(tempdir / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to be created" in
        let* () = Pkgs_ml.Sparse_index.write_cached_config cache ~source:sparse_index_config_json
        in
        let* () = Pkgs_ml.Sparse_index.write_cached_package_document cache ~package_name:"kernel" ~source:sparse_index_kernel_json
        in
        let* () = set_old_mtime (Pkgs_ml.Sparse_index.package_cache_path cache ~package_name:"kernel")
        in
        ...
        let fetch, requests =
          make_fetch_recorder
            (
              fun uri ->
                match Net.Uri.to_string uri with
                | "https://cdn.pkgs.ml/index/v1/ke/rn/kernel.json" -> Ok { Pkgs_ml.Registry.status_code = 200; body = fresh_kernel_json }
                | url -> Error ("unexpected fetch url " ^ url)
            )
        in
        let registry = Pkgs_ml.Registry.filesystem ~fetch cache in
        match Pkgs_ml.Registry.read_package_document registry ~package_name:"kernel" with
        | Ok (Some document) when String.equal document.latest "0.0.3" && List.map (List.reverse !requests) ~fn:(
          fun request -> request.url
...
let test_registry_search_packages = fun _ctx ->
  match Fs.with_tempdir ~prefix:"pkgs_ml_search"
    (
      fun tempdir ->
        let cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(tempdir / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to be created" in
        let fetch, requests =
        ...
        | Ok [ { package_name = "kernel"; latest_version = "0.0.1"; description = Some "Core primitives" }; { package_name = "kernel-tools"; latest_version = "0.1.0"; description = None } ] when List.map (List.reverse !requests) ~fn:(
          fun request -> request.url
...
let test_registry_materializes_in_memory_release = fun _ctx ->
  match Fs.with_tempdir ~prefix:"pkgs_ml_materialize"
    (
      fun tempdir ->
        let cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(tempdir / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to be created" in
        let package = Pkgs_ml.Sparse_index.{
          schema_version = 1;
          name = "std";
          latest = "0.1.0";
        ...
        let registry = Pkgs_ml.Registry.in_memory ~cache ~packages:[ package ] ~releases:[
          {
            package_name = "std";
            version = "0.1.0";
        ...
        | Ok `Materialized ->
            let manifest_path =
              Pkgs_ml.Registry_cache.package_src_dir cache ~package_name:"std" ~version:"0.1.0" |> fun root -> Path.(root / Path.v "riot.toml")
            in
            let source_path =
              Pkgs_ml.Registry_cache.package_src_dir cache ~package_name:"std" ~version:"0.1.0" |> fun root -> Path.(root / Path.v "src/std.ml")
            in
            ...
            | (Error err, _) | (_, Error err) -> Error (IO.error_message err)
    ) with
  | Error err -> Error (IO.error_message err)
  | Ok result -> result

let test_registry_materialize_skips_existing_release = fun _ctx ->
  match Fs.with_tempdir ~prefix:"pkgs_ml_materialize_skip"
    (
      fun tempdir ->
        let cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(tempdir / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to be created" in
        let registry = Pkgs_ml.Registry.in_memory ~cache ~packages:[] ~releases:[
          {
            package_name = "std";
            version = "0.1.0";
            manifest_toml = "[package]\nname = \"std\"\n";
...
let tar_bytes_set_string = fun dst ~offset ~width value ->
  ...
  let copy_len = min width (IO.Bytes.length bytes) in IO.Bytes.blit_unchecked bytes ~src_offset:0 ~dst ~dst_offset:offset ~len:copy_len
...
let tar_octal_string = fun value ->
  let rec loop acc remaining =
    if Int64.equal remaining 0L then
      ...
      let ch = Char.from_int_unchecked (Char.code '0' + digit) in loop (String.make ~len:1 ~char:ch ^ acc) (Int64.div remaining 8L)
  in
...
let tar_zero_pad_left = fun width value ->
  ...
  else String.make ~len:(width - String.length value) ~char:'0' ^ value
...
let tar_pad_data = fun data ->
  ...
  if remainder = 0 then
    ""
  else String.make ~len:(tar_block_size - remainder) ~char:'\000'
...
let create_test_archive = fun ~source_root ~archive_path ->
  ...
  | Ok () -> (
    ...
    | (Error err, _) | (_, Error err) -> Error ("failed to read source fixture for test archive: " ^ IO.error_message err)
    | Ok manifest, Ok source ->
        ...
        Fs.write (IO.Buffer.contents buffer) archive_path |> Result.map_err ~fn:(
          fun err -> "failed to write test archive: " ^ IO.error_message err
        )
  )
...
let gzip_file = fun ~src ~dst ->
  ...
  | Ok () -> Compress.Gzip.compress_file ~src ~dst |> Result.map_err ~fn:(
    function
...
let test_filesystem_registry_materializes_cached_release = fun _ctx ->
  match Fs.with_tempdir ~prefix:"pkgs_ml_filesystem_materialize"
    (
      fun tempdir ->
        let cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(tempdir / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to be created" in
        ...
        Fs.write "[package]\nname = \"std\"\nversion = \"0.1.0\"\n" Path.(source_root / Path.v "riot.toml") |> Result.expect ~msg:"expected manifest to be written";
...
let test_filesystem_registry_materializes_gzip_cached_release = fun _ctx ->
  match Fs.with_tempdir ~prefix:"pkgs_ml_filesystem_materialize_gzip"
    (
      fun tempdir ->
        let cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(tempdir / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to be created" in
        let source_root = Path.(tempdir / Path.v "source/std-0.1.0") in
        ...
        | Ok () -> (
          ...
          | Ok () ->
              ...
              match Pkgs_ml.Registry.materialize_release registry ~package_name:"std" ~version:"0.1.0" with
              | Error err -> Error err
              | Ok `Already_present -> Error "expected gzipped cached archive to materialize on first attempt"
              | Ok `Materialized ->
                  let manifest_path =
                    Pkgs_ml.Registry_cache.package_src_dir cache ~package_name:"std" ~version:"0.1.0" |> fun root -> Path.(root / Path.v "riot.toml")
                  in
...
let test_filesystem_registry_downloads_release_archive_on_cache_miss = fun _ctx ->
  match Fs.with_tempdir ~prefix:"pkgs_ml_filesystem_registry_download"
    (
      fun tempdir ->
        let cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(tempdir / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to be created" in
        let source_root = Path.(tempdir / Path.v "source/std-0.1.0") in
        ...
        | Ok () -> (
          ...
          | Ok archive_body ->
                ...
                make_fetch_recorder
                  (
                    fun uri ->
                      match Net.Uri.to_string uri with
                      | "https://cdn.pkgs.ml/index/v1/config.json" -> Ok { Pkgs_ml.Registry.status_code = 200; body = sparse_index_config_json }
                      | "https://cdn.pkgs.ml/index/v1/3/s/std.json" -> Ok { Pkgs_ml.Registry.status_code = 200; body = sparse_index_std_release_json }
                      | "https://cdn.pkgs.ml/sources/std/0.1.0/deadbeef.tar.gz" -> Ok { Pkgs_ml.Registry.status_code = 200; body = archive_body }
                      | url -> Error ("unexpected fetch url " ^ url)
                  )
              in
              let registry = Pkgs_ml.Registry.filesystem ~fetch cache in
              match Pkgs_ml.Registry.materialize_release registry ~package_name:"std" ~version:"0.1.0" with
              | Error err -> Error err
              | Ok `Already_present -> Error "expected uncached release to download and materialize on first attempt"
              | Ok `Materialized ->
                  let archive_path = Pkgs_ml.Registry_cache.archive_path cache ~package_name:"std" ~version:"0.1.0" in
                  let manifest_path =
                    Pkgs_ml.Registry_cache.package_src_dir cache ~package_name:"std" ~version:"0.1.0" |> fun root -> Path.(root / Path.v "riot.toml")
                  ...
                  | (Error err, _, _) | (_, Error err, _) | (_, _, Error err) -> Error (IO.error_message err)
                  | Ok false, _, _ -> Error "expected downloaded archive to be cached"
                  | Ok true, Ok manifest, Ok source ->
                      let requested = List.reverse !requests |> List.map ~fn:(
                        fun request -> request.url
                      ) in
                      if String.equal manifest "[package]\nname = \"std\"\nversion = \"0.1.0\"\n" && String.equal source "let answer = 42\n" && requested = [ "https://cdn.pkgs.ml/index/v1/config.json"; "https://cdn.pkgs.ml/index/v1/3/s/std.json"; "https://cdn.pkgs.ml/sources/std/0.1.0/deadbeef.tar.gz" ] then
                        Ok ()
                      else Error "unexpected registry download/materialization state"
        )
    ) with
  | Error err -> Error (IO.error_message err)
  | Ok result -> result
...
let test_filesystem_registry_refetches_corrupt_cached_archive = fun _ctx ->
  match Fs.with_tempdir ~prefix:"pkgs_ml_filesystem_registry_retry_archive"
    (
      fun tempdir ->
        let cache = Pkgs_ml.Registry_cache.create ~riot_home:Path.(tempdir / Path.v ".riot") ~registry_name:"pkgs.ml" () |> Result.expect ~msg:"expected registry cache to be created" in
        let source_root = Path.(tempdir / Path.v "source/std-0.1.0") in
        ...
        | Ok () -> (
          ...
          | Ok archive_body ->
              ...
              in
              let archive_path = Pkgs_ml.Registry_cache.archive_path cache ~package_name:"std" ~version:"0.1.0" in
              Fs.create_dir_all Path.(tempdir / Path.v ".riot/registry/pkgs.ml/archive/std") |> Result.expect ~msg:"expected archive directory to be created";
              Fs.write "this is not a tarball" archive_path |> Result.expect ~msg:"expected corrupt archive to be written";
              let registry = Pkgs_ml.Registry.filesystem ~fetch cache in
              match Pkgs_ml.Registry.materialize_release registry ~package_name:"std" ~version:"0.1.0" with
              | Error err -> Error err
              | Ok `Already_present -> Error "expected corrupt cached archive to be replaced and materialized"
              | Ok `Materialized ->
                  let manifest_path =
                    Pkgs_ml.Registry_cache.package_src_dir cache ~package_name:"std" ~version:"0.1.0" |> fun root -> Path.(root / Path.v "riot.toml")
                  in
                  ...
                  | Ok manifest, Ok source when String.equal manifest "[package]\nname = \"std\"\nversion = \"0.1.0\"\n" && String.equal source "let answer = 42\n" ->
                      ...
                      if requested = [ "https://cdn.pkgs.ml/index/v1/config.json"; "https://cdn.pkgs.ml/index/v1/3/s/std.json"; "https://cdn.pkgs.ml/sources/std/0.1.0/deadbeef.tar.gz" ] then
                        Ok ()
                      else Error "expected corrupt cache retry to fetch replacement archive"
    ...
    "source": false
        ...
        | Ok published ->
            ...
            | [ request ] ->
                ...
                if String.equal request.method_ "POST" && String.equal request.url "https://api.pkgs.ml/v1/publish" && request.body = Some artifact && has_header "authorization" "Bearer root-secret" && has_header "content-type" "application/gzip" && has_header "X-Riot-Agent" "riot-cli@test" && String.equal published.artifact_sha256 "0123456789abcdef0123456789abcdef01234567" && String.equal published.package_name "minttea" && String.equal published.package_version "0.4.2" then
                  Ok ()
                else Error "unexpected artifact publish request or response"
...
let test_registry_publish_artifact_bubbles_transport_exceptions_as_errors = fun _ctx ->
  ...
  let fetch, _requests =
    make_fetch_recorder ~post_handler:(
      fun _uri ~headers:_ ~body:_ -> raise (Failure "SSL_write error")
    )
      (
        fun uri -> Error ("unexpected GET " ^ Net.Uri.to_string uri)
      )
  ...
  | Error err ->
      if String.equal err "SSL_write error" then
        Ok ()
      else Error ("unexpected publish artifact transport error: " ^ err)
  ...
  "yanked_by_github_login": "leostera"
        ...
        | Ok yanked_release -> (
          ...
          | [ request ] ->
              ...
              if String.equal request.method_ "POST" && String.equal request.url "https://api.pkgs.ml/v1/me/packages/std/versions/0.1.0/yank" && request.body = Some "" && has_header "authorization" "Bearer root-secret" && has_header "X-Riot-Agent" "riot-cli@test" && yanked_release.yanked && yanked_release.yanked_by_github_login = Some "leostera" then
                Ok ()
              else Error "unexpected yank request or response"
  ...
  "materialization": {
    ...
    "source": false
              ...
              | Ok _ -> (
                match List.reverse !requests with
                | [ request ] ->
                    let header = List.find request.headers ~fn:(
                      fun (name, _value) -> String.equal name "X-Riot-Agent"
                    ) in
                    if header = Some ("X-Riot-Agent", "riot-docs-pipeline@1.0") then
                      Ok ()
                    else Error "expected RIOT_AGENT_HEADER override to win over default agent"
                | _ -> Error "expected exactly one publish request"
              )
          )
    )
