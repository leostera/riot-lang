...
let package_error_to_telemetry_error =
  function
...
let mark_package_built_in_graph = fun package_graph ~runtime ~artifact ~status ->
  ...
  | None -> ()
  | Some node -> node.value <- Package_graph.Built {
...
let build_workspace_actions = fun ~(workspace:Workspace.t) ~toolchain ~store ~package_graph ~target ~build_ctx ~session_id ~nodes ->
  ...
  let materialize_initial_failure = fun package_key package status ->
    ...
    result
  in
  let update_planning_progress = fun package_key status ~duration ~package ~reason ->
    ...
    Telemetry.emit (PackagePlanningResult {session_id; package; target; status; duration; reason; })
  in
  let target_dir_for = fun package_name ->
    Path.(Tusk_dirs.out_dir_with_target
    ~workspace_root:workspace.root
    ~profile:profile_name
    ~target:target_triple_str
    / Path.v package_name) in
  let maybe_short_circuit_cached_package = fun ~package_key ~(package:Package.t) ~hash ~depset ~module_graph ~action_graph ->
  ...
  in
  let stage_runtime = fun ~package_key ~(package:Package.t) ~hash ~depset ~module_graph ~action_graph ->
    ...
    match Package_graph.get_node_by_key package_graph package_key with
    | Some node -> node.value <- Package_graph.Planned {
      package;
      scope = Package_graph.get_scope node.value;
      module_graph;
      action_graph;
    ...
    | None -> ()
  in
  let enqueue_all_ready_actions = fun ~package_key runtime enqueue_ready ->
    let rec loop = fun () ->
    ...
    loop ()
  in
  let rec plan_pass = fun pending_nodes ->
  ...
  in
  plan_pass nodes;
  let try_plan_pending_packages = fun () ->
    ...
    List.iter
      ...
      pending_entries
  in
  let activate_ready_packages = fun () ->
    ...
    HashMap.iter
      ...
      runtimes
  in
  let finalize_if_complete = fun package_key runtime ->
    if
  ...
  let rec workspace_worker_loop = fun () ->
  ...
  let planning_counts =
    fun () ->
      HashMap.into_iter planning_results |> Iter.Iterator.to_list |> List.fold_left
