  ...
  List.iter (fun action ->
    let result = match action with
      | Action.CompileInterface { source; output; includes; flags } ->
          Tusk_ocaml.Ocamlc.compile_interface ~toolchain ~cwd:execution_root 
            ~includes ~flags ~output source
      
      | Action.CompileImplementation { source; output; includes; flags } ->
          Tusk_ocaml.Ocamlc.compile_impl ~toolchain ~cwd:execution_root 
            ~includes ~flags ~output source
      
      | Action.GenerateInterface { source; output; includes; flags } ->
          Tusk_ocaml.Ocamlc.generate_interface ~toolchain ~cwd:execution_root 
            ~includes ~flags ~output source
      
      | Action.CompileC { source; output } ->
          Tusk_ocaml.Ocamlc.compile_c ~toolchain ~cwd:execution_root 
            ~includes:[] ~output source
      
      | Action.CreateLibrary { output; objects; includes } ->
          Tusk_ocaml.Ocamlc.create_library ~toolchain ~cwd:execution_root 
            ~includes ~output objects
      
      | Action.CreateExecutable { output; objects; libraries; includes } ->
          Tusk_ocaml.Ocamlc.create_executable ~toolchain ~cwd:execution_root 
            ~includes ~libs:libraries ~output objects
      
      | Action.CopyFile { source; destination } ->
          (match Fs.copy ~src:source ~dst:destination with
           | Ok () -> Tusk_ocaml.Ocamlc.Success "Copied"
           | Error _ -> Tusk_ocaml.Ocamlc.Failed (format "Copy failed: %s -> %s" 
      ...
      | Action.WriteFile { destination; content } ->
          (match Fs.write content destination with
           | Ok () -> Tusk_ocaml.Ocamlc.Success "Written"
           | Error _ -> Tusk_ocaml.Ocamlc.Failed (format "Write failed: %s" 
               (Path.to_string destination)))
    in
    match result with
    | Tusk_ocaml.Ocamlc.Success _ -> ()
    | Tusk_ocaml.Ocamlc.Failed err -> 
  ...
  let rec spawn_workers n acc =
    if n = 0 then acc
      ...
      let worker = spawn (fun () ->
        worker_loop ~owner:(self ()) ~completed ~toolchain ~execution_root ~work_queue ()
      ) in
    ...
    if remaining = 0 then
      ...
      | TaskCompleted result ->
          ...
          Log.info "Node %s completed: %s (%dms)"
            ...
            (match result.status with `Built -> "built" | `Cached -> "cached" | `Failed -> "failed")
            result.duration_ms;
