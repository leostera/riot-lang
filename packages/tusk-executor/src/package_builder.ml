...
let summarize_package_names = fun names ->
  let rec take_first = fun n acc remaining ->
  ...
  | Ok (Planned {
    ...
    action_graph
      ...
      (
        ...
        | Some node -> node.value <- Tusk_planner.Package_graph.Planned {
          package;
          scope = Tusk_planner.Package_graph.get_scope node.value;
          module_graph;
          action_graph;
      ...
      | Ok (artifact, export_entries) ->
          ...
          (
            match Tusk_planner.Package_graph.get_node_by_key package_graph planned_key with
            | Some node -> node.value <- Tusk_planner.Package_graph.Built {
              package;
              scope = Tusk_planner.Package_graph.get_scope node.value;
              module_graph;
              action_graph;
