...
let write_package = fun ~root ~name ~lib_body ~deps ->
  ...
  let deps_block =
    ...
    ^ "\n"
  in
  let tusk_toml = "[package]\nname = \""
  ^ name
  ^ "\"\nversion = \"0.0.1\"\n\n[lib]\npath = \"src/lib.ml\"\n"
  ^ deps_block in
  let _ = Fs.write tusk_toml Path.(pkg_dir / Path.v "tusk.toml") |> Result.expect ~msg:"write tusk.toml failed" in
  ()
        ...
        with_scanned_workspace tmpdir
          (fun workspace ->
            ...
            match Tusk_executor.Coordinator.build_workspace
              ...
              ~concurrency:2
              ~build_ctx
              ~session_id:build_ctx.Tusk_model.Build_ctx.session_id with
        ...
        with_scanned_workspace tmpdir
          (fun workspace ->
            ...
            | Ok result ->
                ...
                match Tusk_planner.Package_graph.get_node_by_key result.package_graph package_key with
                ...
                | Some node -> (
                    ...
                    | Tusk_planner.Package_graph.Built _ ->
                        Error "package graph left dependent package built after \
                           dependency failure"
                    | Tusk_planner.Package_graph.Skipped _ -> Ok ()
                  )))
  with
  | Ok x -> x
  | Error _ -> Error "tempdir creation failed"
