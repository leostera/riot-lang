  ...
  match
    Fs.with_tempdir ~prefix:"diagnostic_rewrite_test"
      (fun tmpdir ->
        ...
        let sandbox_dir = Path.(tmpdir / Path.v "_build" / Path.v "sandbox" / Path.v "tusk-cli-a9441944") in
        let _ = Fs.create_dir_all package_src |> Result.expect ~msg:"failed to create package src" in
        let _ =
          Fs.write "let displayed_packages = HashMap.create ()"
        ...
        in
        let rewritten =
          Tusk_executor.Diagnostic_rewrite.rewrite_ocamlc_result
            ~package
            ~sandbox_dir
            result
        in
  ...
  match
    Fs.with_tempdir ~prefix:"diagnostic_rewrite_generated_test"
      (fun tmpdir ->
        ...
        let sandbox_dir = Path.(tmpdir / Path.v "_build" / Path.v "sandbox" / Path.v "tusk-cli-a9441944") in
        let diagnostic =
          make_warning (Path.to_string Path.(sandbox_dir / Path.v "Tusk_cli__Aliases.ml.gen"))
          |> Tusk_toolchain.Ocamlc.Diagnostic.parse
          |> List.hd
        in
        let rewritten =
        ...
        if String.contains rendered (Path.to_string Path.(sandbox_dir / Path.v "Tusk_cli__Aliases.ml.gen")) then
          Ok ()
        else
...
let () =
  Miniriot.run ~main:(Test.Cli.main ~name ~tests) ~args:Env.args ()
