...
type wasm_func_type = { params : wasm_type list; results : wasm_type list }
...
let wasm_func_to_string (func : wasm_func) = format "(func $%s)" func.name
let wasm_module_to_string _module = "(module)"
...
let translate_from_lambda (expr : Lambda.Ir.lambda) : wasm_module =
  ...
  let rec collect_locals expr =
    ...
    | Lambda.Ir.Let { id; value; body; _ } ->
        if
          not
            (List.exists
               (fun (local_id, _) -> Identifier.equal local_id id)
               !locals_ref)
        then locals_ref := (id, I32) :: !locals_ref;
  ...
  and translate_prim op args =
    ...
    let op_instr =
      ...
      | Lambda.Ir.Pint_neg ->
          [ Const (ConstI32 0l) ] @ arg_instrs @ [ BinOp (I32, Sub) ]
      | _ -> [ Const (ConstI32 0l) ]
    in
    arg_instrs @ op_instr
  in
