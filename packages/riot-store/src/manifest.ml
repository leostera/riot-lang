...
let to_json manifest: Data.Json.t =
  ...
  Data.Json.Object [
    ...
    "ocamlc_warnings", Data.Json.Array (List.map manifest.ocamlc_warnings ~fn:(
...
let of_json = fun json -> let open Result in
let open Data.Json in
try
  match json with
  | Object fields -> (
    let get_field name = List.find fields ~fn:(
    ...
    let files =
      ...
      | Some (Array entries) ->
          let parse_entry = function
            | Object entry_fields -> (
              let get_entry_field name = List.find entry_fields ~fn:(
                fun (field_name, _) -> String.equal field_name name
              ) |> Option.map ~fn:(
                fun (_, value) -> value
              ) in
              match get_entry_field "path", get_entry_field "hash", get_entry_field "size" with
              | Some (String path), Some (String hash), Some (Int size) -> Ok { path = Path.v path; hash; size }
              | _ -> Error "Invalid file entry"
            )
            | _ -> Error "File entry must be an object"
          in
          List.fold_left entries ~init:(Ok []) ~fn:(
            fun acc entry ->
    ...
    let ocamlc_warnings =
      ...
      | None -> Ok []
      | Some (Array entries) -> List.fold_left entries ~init:(Ok []) ~fn:(
        fun acc entry ->
      ...
      | Some (Array entries) ->
          let parse_entry = function
            | Object entry_fields -> (
              ...
              match get_entry_field "name", get_entry_field "path", get_entry_field "action_hash" with
              | Some (String name), Some (String path), Some (String action_hash) -> Ok { name; path = Path.v path; action_hash }
    ...
    | (Error e, _, _, _, _, _, _) | (_, Error e, _, _, _, _, _) | (_, _, Error e, _, _, _, _) | (_, _, _, Error e, _, _, _) | (_, _, _, _, Error e, _, _) | (_, _, _, _, _, Error e, _) | (_, _, _, _, _, _, Error e) -> Error e
  )
...
let create = fun ?base_dir ?(ocamlc_warnings = []) ?(exports = []) () ~package ~build_hash ~files ->
  ...
  let file_entries = List.filter_map files ~fn:(
    fun (path, size) ->
      let readable_path =
      ...
      | Ok file ->
          let hash = Std.Crypto.Digest.hex (Std.Crypto.Sha512.hash_string file) in Some { path; hash; size }
      | Error _ -> None
  ) in
  {
    version = V1;
