...
type cache_entry = { hash: string; dir: Path.t; size_bytes: int64 }

type receipt = { hash: string; lanes: generation_lane list }

type receipt_file = { path: Path.t; receipt: receipt }
...
let generations_root = fun ~(workspace:Workspace.t) -> Path.(cache_root ~workspace / Path.v "generations")
...
let receipt_path = fun ~(workspace:Workspace.t) hash -> Path.(generations_root ~workspace / Path.v (receipt_filename hash))

let temp_receipt_path = fun ~(workspace:Workspace.t) hash -> Path.(generations_root ~workspace / Path.v (receipt_filename hash ^ ".tmp"))
...
let summary_message = fun summary ->
  ...
  else "removed " ^ Int.to_string summary.deleted_entries ^ " cache entries and " ^ Int.to_string summary.deleted_generations ^ " generations from tracked cache (" ^ size_to_string summary.size_before_bytes ^ " -> " ^ size_to_string summary.size_after_bytes ^ ")"
...
let event_message = function
  ...
  | ForceCleanFailed { build_root; error } -> "failed to remove build root " ^ Path.to_string build_root ^ ": " ^ error
  ...
  let rec dedupe acc = function
    ...
    | left :: ((right :: _) as rest) ->
        if String.equal left right then
          dedupe acc rest
        else dedupe (left :: acc) rest
  in
  values |> List.sort ~compare:String.compare |> dedupe []

let normalize_lane = fun (lane: generation_lane) -> { lane with hashes = sort_uniq_strings lane.hashes }

let normalize_lanes: generation_lane list -> generation_lane list = fun lanes -> lanes |> List.map ~fn:normalize_lane |> List.sort ~compare:(
  fun (left: generation_lane) (right: generation_lane) ->
    match String.compare left.profile right.profile with
    | Order.EQ -> String.compare (Riot_model.Target.to_string left.target) (Riot_model.Target.to_string right.target)
    | order -> order
)
...
let generation_hash_of_lanes = fun lanes -> Data.Json.Array (List.map lanes ~fn:lane_to_json) |> Data.Json.to_string |> Crypto.hash_string |> Crypto.Digest.hex
...
let cache_state_to_json = fun (state: cache_state) ->
  Data.Json.Object (
    ...
    | None ->
        ...
        | Some receipt_count ->
            [
              "receipt_count", Data.Json.Int receipt_count;
            ]
        | None -> []
  )
...
let generation_lane_of_json = fun json ->
  ...
  let* target =
    ...
    | Some value -> (
      match Data.Json.get_string value with
      | Some target -> Riot_model.Target.from_string target |> Result.map_err ~fn:Riot_model.Target.error_message
...
let path_is_directory = fun path -> Fs.metadata path |> Result.map ~fn:Fs.Metadata.is_dir |> Result.unwrap_or ~default:false
...
let is_json_file = fun path -> String.ends_with ~suffix:".json" (Path.basename path) && not (path_is_directory path)
...
let is_hash_dir_name = fun name ->
  ...
  let rec loop idx =
    if idx = len then
      ...
      if is_hex_char (String.get_unchecked name ~at:idx) then
        loop (idx + 1)
      else false
  in
  len = 64 && loop 0

let receipt_paths_desc = fun ~(workspace:Workspace.t) -> list_children (generations_root ~workspace) |> List.filter ~fn:is_json_file |> List.sort ~compare:(
  fun left right -> String.compare (Path.basename right) (Path.basename left)
)

let count_receipts = fun ~(workspace:Workspace.t) -> List.length (receipt_paths_desc ~workspace)

let ensure_cache_root = fun ~(workspace:Workspace.t) -> Fs.create_dir_all (cache_root ~workspace) |> Result.map_err ~fn:(
  fun err -> "failed to create workspace cache directory: " ^ IO.error_message err
)

let ensure_generations_root = fun ~(workspace:Workspace.t) -> Fs.create_dir_all (generations_root ~workspace) |> Result.map_err ~fn:(
...
let write_state = fun ~(workspace:Workspace.t) (state: cache_state) ->
  let* () = ensure_cache_root ~workspace in Fs.write (Data.Json.to_string_pretty (cache_state_to_json state)) (state_path ~workspace) |> Result.map_err ~fn:(
    fun err -> "failed to write cache state: " ^ IO.error_message err
  )
...
let read_state = fun ~(workspace:Workspace.t) ->
  ...
  if not (path_exists path) then
    ...
    let* content = Fs.read_to_string path |> Result.map_err ~fn:(
      fun err -> "failed to read cache state: " ^ IO.error_message err
    )
    in
    let* json = Data.Json.of_string content |> Result.map_err ~fn:(
      fun err -> "failed to parse cache state JSON: " ^ Data.Json.error_to_string err
    )
    in
    let* state = cache_state_of_json json in Ok (Some state)

let write_receipt = fun ~(workspace:Workspace.t) receipt ->
  let* () = ensure_generations_root ~workspace
  in
  ...
  if path_exists final_path then
    ...
    let* () = Fs.write (Data.Json.to_string_pretty (receipt_to_json receipt)) temp_path |> Result.map_err ~fn:(
    ...
    | Ok () -> Ok ()
    | Error err ->
        let _ = Fs.remove_file temp_path in Error ("failed to commit generation receipt: " ^ IO.error_message err)

let read_receipt_file = fun path ->
  let* content = Fs.read_to_string path |> Result.map_err ~fn:(
    fun err -> "failed to read generation receipt: " ^ IO.error_message err
  )
  in
  let* json = Data.Json.of_string content |> Result.map_err ~fn:(
...
let write_canonical_receipts = fun ~(workspace:Workspace.t) receipts -> List.fold_left receipts ~init:(Ok ()) ~fn:(
  fun acc_result (receipt_file: receipt_file) ->
    let* () = acc_result in write_receipt ~workspace receipt_file.receipt
)

let generation_hashes_of_receipts = fun receipts ->
  let seen = HashSet.create () in List.fold_left receipts ~init:[] ~fn:(
    fun acc (receipt_file: receipt_file) ->
      if HashSet.contains seen ~value:receipt_file.receipt.hash then
        acc
      else
        (
          let _ = HashSet.insert seen ~value:receipt_file.receipt.hash in receipt_file.receipt.hash :: acc
        )
  ) |> List.reverse

let rebuild_generation_hashes = fun ~(workspace:Workspace.t) ->
  let* receipts = load_receipts ~workspace
  in
  let* () = write_canonical_receipts ~workspace receipts in Ok (generation_hashes_of_receipts receipts)

let path_size_bytes = fun root ->
  if not (path_exists root) then
    Ok 0L
  else
    let total = ref 0L in Fs.Walker.walk ~sort:false ~roots:[ root ] ~f:(
      fun item ->
        match Fs.Walker.FileItem.kind item with
        | Fs.Walker.File ->
            let path = Fs.Walker.FileItem.path item in
            let len = Fs.metadata path |> Result.map ~fn:Fs.Metadata.len |> Result.unwrap_or ~default:0 in
            total := Int64.add !total (Int64.from_int len);
            Fs.Walker.Continue
        | Fs.Walker.Directory | Fs.Walker.Symlink | Fs.Walker.Other -> Fs.Walker.Continue
    ) () |> Result.map_err ~fn:IO.error_message |> Result.map ~fn:(
      fun () -> !total
    )

let collect_cache_entries = fun ~(workspace:Workspace.t) ->
  let profile_dirs = list_subdirectories workspace.target_dir_root |> List.filter ~fn:(
    fun dir -> not (String.equal (Path.basename dir) "cache")
  ) in
  ...
  and collect_targets acc = function
    ...
    | target_dir :: rest ->
        ...
        let hash_dirs = list_subdirectories cache_dir |> List.filter ~fn:(
          fun dir -> is_hash_dir_name (Path.basename dir)
        ) in
        let* acc = List.fold_left hash_dirs ~init:(Ok acc) ~fn:(
          fun acc_result dir ->
            let* acc = acc_result
            in
            let* size_bytes = path_size_bytes dir in Ok ({ hash = Path.basename dir; dir; size_bytes } :: acc)
        ) in collect_targets acc rest
  in
  collect_profiles [] profile_dirs

let total_size = fun entries -> List.fold_left entries ~init:0L ~fn:(
  fun acc (entry: cache_entry) -> Int64.add acc entry.size_bytes
)
...
let load_or_rebuild_tracked_size = fun ~(workspace:Workspace.t) ->
  ...
  | Ok (Some state) ->
      let* generation_hashes =
        ...
        | None ->
            let* generation_hashes = rebuild_generation_hashes ~workspace
            in
            let* () = write_state ~workspace { tracked_size_bytes = state.tracked_size_bytes; generation_hashes = Some generation_hashes; receipt_count = Some (List.length generation_hashes) } in Ok generation_hashes
      in
      Ok ({ tracked_size_bytes = state.tracked_size_bytes; generation_hashes; rebuilt = false } : tracked_size_snapshot)
  | Ok None | Error _ ->
      let* entries = collect_cache_entries ~workspace
      in
      let tracked_size_bytes = total_size entries in
      let* generation_hashes = rebuild_generation_hashes ~workspace
      in
      let* () = write_state ~workspace { tracked_size_bytes; generation_hashes = Some generation_hashes; receipt_count = Some (List.length generation_hashes) } in Ok ({ tracked_size_bytes; generation_hashes; rebuilt = true } : tracked_size_snapshot)
...
let live_hashes = fun receipts ->
  ...
  List.for_each receipts ~fn:(
    fun (receipt_file: receipt_file) -> List.for_each receipt_file.receipt.lanes ~fn:(
      fun (lane: generation_lane) -> List.for_each lane.hashes ~fn:(
        fun hash ->
...
let evaluate_retention = fun ~policy receipts entries ->
  ...
  let rec loop kept =
    ...
    let retained_entries = List.filter entries ~fn:(
      fun (entry: cache_entry) -> HashSet.contains live ~value:entry.hash
    ) in
...
let delete_path = fun path ~kind ->
  if not (path_exists path) then
    ...
    if String.equal kind "directory" then
      Fs.remove_dir_all path |> Result.map_err ~fn:(
        fun err -> "failed to remove " ^ kind ^ " " ^ Path.to_string path ^ ": " ^ IO.error_message err
      )
    else Fs.remove_file path |> Result.map_err ~fn:(
      fun err -> "failed to remove " ^ kind ^ " " ^ Path.to_string path ^ ": " ^ IO.error_message err
    )
...
let run_gc = fun ~(workspace:Workspace.t) ~policy ~generation_hashes ->
  ...
  let* receipts = load_receipts_for_hashes ~workspace candidate_hashes
  in
  let* entries = collect_cache_entries ~workspace
  in
  ...
  let kept_hashes = List.map kept_receipts ~fn:(
    fun (receipt: receipt_file) -> receipt.receipt.hash
  ) in
  let kept_paths = HashSet.create () in
  List.for_each kept_hashes ~fn:(
    fun hash ->
      let _ = HashSet.insert kept_paths ~value:(Path.to_string (receipt_path ~workspace hash)) in ()
  );
  let deleted_entries = List.filter entries ~fn:(
    fun (entry: cache_entry) -> not (HashSet.contains live ~value:entry.hash)
  ) in
  let deleted_receipts = List.filter (receipt_paths_desc ~workspace) ~fn:(
    fun path -> not (HashSet.contains kept_paths ~value:(Path.to_string path))
  ) in
  let* () = List.fold_left deleted_entries ~init:(Ok ()) ~fn:(
    fun acc (entry: cache_entry) ->
      let* () = acc in delete_path entry.dir ~kind:"directory"
  )
  in
  let* () = List.fold_left deleted_receipts ~init:(Ok ()) ~fn:(
    fun acc receipt_path ->
      let* () = acc in delete_path receipt_path ~kind:"file"
  )
  in
  let* () = write_state ~workspace { tracked_size_bytes = size_after_bytes; generation_hashes = Some kept_hashes; receipt_count = Some (List.length kept_hashes) }
  in
  Ok {
    ran_gc = true;
...
let load_policy = fun ~(workspace:Workspace.t) -> Riot_model.Workspace_operational_config.load ~workspace_root:workspace.root |> Result.map ~fn:(
  fun config -> config.Riot_model.Workspace_operational_config.cache
) |> Result.map_err ~fn:Riot_model.Workspace_operational_config.message

let should_run_gc = fun ~generation_count ~policy ~tracked_size_bytes -> generation_count > policy.Riot_model.Workspace_operational_config.keep_generations || Int64.compare tracked_size_bytes policy.max_size_bytes = Order.GT
...
let force_clean_with_events = fun ~(workspace:Workspace.t) ~on_event ->
  ...
  if not (path_exists build_root) then
    ...
    | Ok () ->
        ...
        Ok ()
    | Error err ->
        let error = "failed to remove build root " ^ Path.to_string build_root ^ ": " ^ IO.error_message err in
...
let new_entry_dir = fun ~(workspace:Workspace.t) (entry: new_cache_entry) -> Path.(workspace.target_dir_root / Path.v entry.profile / Path.v (Riot_model.Target.to_string entry.target) / Path.v "cache" / Path.v entry.hash)

let added_size_for_new_entries = fun ~(workspace:Workspace.t) new_entries ->
  let seen = HashSet.create () in List.fold_left new_entries ~init:(Ok 0L) ~fn:(
...
let record_successful_build_with_events = fun ~(workspace:Workspace.t) ~on_event ~lanes ~new_entries ->
  ...
  | latest_hash :: _ when String.equal latest_hash generation_hash ->
      let* () =
        if Int64.equal tracked_size_bytes tracked_size.tracked_size_bytes then
          Ok ()
        else write_state ~workspace { tracked_size_bytes; generation_hashes = Some current_hashes; receipt_count = Some (List.length current_hashes) }
      in
      Ok {
        ran_gc = false;
  ...
  | _ ->
      let next_hashes = generation_hash :: List.filter current_hashes ~fn:(
        fun hash -> not (String.equal hash generation_hash)
      ) in
      let* () =
        match write_state ~workspace { tracked_size_bytes; generation_hashes = Some next_hashes; receipt_count = Some (List.length next_hashes) } with
        | Ok () -> Ok ()
        | Error error -> report_error error
      in
      let* () =
...
let record_successful_build = fun ~(workspace:Workspace.t) ~lanes ~new_entries -> record_successful_build_with_events ~workspace ~on_event:no_event ~lanes ~new_entries
