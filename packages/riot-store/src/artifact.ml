...
let to_json = fun artifact ->
  ...
  Json.Object [
    ...
    "files", Json.Array (List.map artifact.files ~fn:(
      fun p -> Json.String (Path.to_string p)
    ));
    "ocamlc_warnings", Json.Array (List.map artifact.ocamlc_warnings ~fn:(
      fun msg -> Json.String msg
    ));
