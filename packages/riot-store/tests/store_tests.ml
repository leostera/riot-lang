...
let make_test_workspace = fun tmpdir -> Riot_model.Workspace.{
  name = None;
  root = tmpdir;
...
let write_output = fun sandbox relative_path content ->
  ...
  let _ = Fs.write content output |> Result.expect ~msg:"write output should succeed" in output

let write_workspace_cache_config = fun tmpdir ~keep_generations ~max_size ->
  let riot_dir = Path.(tmpdir / Path.v ".riot") in
  let _ = Fs.create_dir_all riot_dir |> Result.expect ~msg:"create .riot should succeed" in Fs.write ("[riot.cache]\nkeep_generations = " ^ Int.to_string keep_generations ^ "\nmax_size = \"" ^ max_size ^ "\"\n") Path.(riot_dir / Path.v "config.toml") |> Result.expect ~msg:"write .riot/config.toml should succeed"
...
let count_generation_receipts = fun ~(workspace:Riot_model.Workspace.t) ->
  ...
  | Error _ -> 0
  | Ok reader -> Std.Iter.MutIterator.to_list reader |> List.filter ~fn:(
    fun path ->
      let basename = Path.basename path in String.ends_with ~suffix:".json" basename
  ) |> List.length
...
let read_cache_state_generation_hashes = fun ~(workspace:Riot_model.Workspace.t) ->
  ...
  | Some (Data.Json.Array hashes) -> List.map hashes ~fn:(
    fun value -> Data.Json.get_string value |> Option.expect ~msg:"generation hash should be a string"
  )
  | _ -> []

let read_generation_lane_hashes = fun ~(workspace:Riot_model.Workspace.t) generation_hash ->
  let path = Path.(workspace.target_dir_root / Path.v "cache" / Path.v "generations" / Path.v (generation_hash ^ ".json")) in
  ...
  | Some (Data.Json.Array lanes) -> List.map lanes ~fn:(
    fun lane ->
      match Data.Json.get_field "hashes" lane with
      | Some (Data.Json.Array hashes) -> List.map hashes ~fn:(
        fun value -> Data.Json.get_string value |> Option.expect ~msg:"generation lane hash should be a string"
      )
...
let write_cache_entry = fun ~(workspace:Riot_model.Workspace.t) ~profile ~target ~hash ~size ->
  let entry_dir = Path.(workspace.target_dir_root / Path.v profile / Path.v (Riot_model.Target.to_string target) / Path.v "cache" / Path.v hash) in
  ...
  let _ = Fs.write (String.make ~len:size ~char:'x') payload |> Result.expect ~msg:"write cache payload should succeed" in entry_dir

let test_save_and_promote_nested_outputs = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_nested_test"
    (
      fun tmpdir ->
        ...
        let _ = Riot_store.Store.save store ~package:"pkg" ~hash ~sandbox_dir:sandbox ~outs:[ nested_obj; nested_bin ] |> Result.expect ~msg:"save should succeed" in
        ...
        if String.equal (read_file promoted_obj) "obj" && String.equal (read_file promoted_bin) "bin" then
          Ok ()
...
let test_get_preserves_relative_paths = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_manifest_path_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        ...
        let _ = Riot_store.Store.save store ~package:"pkg" ~hash ~sandbox_dir:sandbox ~outs:[ output ] |> Result.expect ~msg:"save should succeed" in
        match Riot_store.Store.get store hash with
        | None -> Error "expected cached artifact"
        | Some artifact ->
            if List.any artifact.files ~fn:(
              fun path -> Path.equal path (Path.v "deep/dir/x.cmxa")
            ) then
              Ok ()
...
let test_save_and_promote_self_host_style_outputs = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_self_host_output_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        ...
        in
        let out_paths = List.map outputs ~fn:(
          fun (name, content) ->
            let path = Path.(sandbox / Path.v name) in
            let _ = Fs.write content path |> Result.expect ~msg:("write " ^ name ^ " should succeed") in path
        ) in
        let hash = Crypto.hash_string "self-host-style-artifacts" in
        let _ = Riot_store.Store.save store ~package:"kernel" ~hash ~sandbox_dir:sandbox ~outs:out_paths |> Result.expect ~msg:"save should succeed for self-host-style outputs" in
        let target = Path.(tmpdir / Path.v "out") in
        ...
        let promoted_ok = List.all outputs ~fn:(
          fun (name, content) ->
            let path = Path.(target / Path.v name) in String.equal (read_file path) content
        ) in
        if promoted_ok then
          Ok ()
        else Error "self-host-style outputs were not preserved through store save/promote"
    ) with
  | Ok x -> x
  | Error _ -> Error "tempdir creation failed"

let test_exists_requires_manifest_file = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_exists_manifest_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
...
let test_put_if_absent_keeps_first_writer = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_put_if_absent_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        ...
        let _ = Riot_store.Store.save store ~package:"pkg" ~hash ~sandbox_dir:sandbox ~outs:[ output ] |> Result.expect ~msg:"first save should succeed" in
        let _ = Fs.write "second" output in
        let _ = Riot_store.Store.save store ~package:"pkg" ~hash ~sandbox_dir:sandbox ~outs:[ output ] |> Result.expect ~msg:"second save should succeed" in
        let target = Path.(tmpdir / Path.v "out") in
        let _ = Riot_store.Store.promote store hash ~target_dir:target |> Result.expect ~msg:"promote should succeed" in
...
let test_concurrent_same_hash_saves_share_cache_safely = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_concurrent_same_hash_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
          ...
          spawn
            (
              fun () ->
                ...
                let result = Riot_store.Store.save store ~package:"pkg" ~hash ~sandbox_dir:sandbox ~outs:[ output ] |> Result.map ~fn:(
                  fun _ -> ()
                ) in
        ...
        | ConcurrentSaveComplete (_, Ok ()), ConcurrentSaveComplete (_, Ok ()) ->
            ...
            if String.equal (read_file Path.(target / Path.v "pkg.cmx")) "shared-artifact" then
              Ok ()
            else Error "expected concurrent same-hash saves to leave a readable artifact"
        | ConcurrentSaveComplete (name, Error err), _ -> Error (name ^ " save failed: " ^ Riot_store.Store.error_message err)
        | _, ConcurrentSaveComplete (name, Error err) -> Error (name ^ " save failed: " ^ Riot_store.Store.error_message err)
        | _ -> Error "unexpected concurrent save result"
    ) with
  | Ok x -> x
  | Error _ -> Error "tempdir creation failed"

let test_plan_bundle_round_trip = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_plan_bundle_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        ...
        let plan =
          Std.Data.Json.Object [
            ...
            "action_graph", Std.Data.Json.Object [
...
let test_hash_manifest_round_trip_preserves_exports = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_manifest_exports_round_trip_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        ...
        let _ = Fs.write "cmxa" output in
        let exports = [ Riot_store.Store.{ name = "foo.cmxa"; path = Path.v "lib/foo.cmxa"; action_hash = Crypto.Digest.hex hash }; Riot_store.Store.{ name = "foo.cmxs"; path = Path.v "lib/foo.cmxs"; action_hash = Crypto.Digest.hex hash } ] in
        let _ = Riot_store.Store.save store ~package:"pkg" ~exports ~hash ~sandbox_dir:sandbox ~outs:[ output ] |> Result.expect ~msg:"save should succeed" in
...
let test_artifact_round_trip_preserves_ocamlc_warnings = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_ocamlc_warnings_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        ...
        let exports = [ Riot_store.Store.{ name = "lib.cmx"; path = Path.v "lib.cmx"; action_hash = Crypto.Digest.hex hash } ] in
        let warnings = [ "File \"lib.ml\", line 1, characters 0-1:\nWarning: example" ] in
        let _ = Riot_store.Store.save store ~package:"pkg" ~ocamlc_warnings:warnings ~exports ~hash ~sandbox_dir:sandbox ~outs:[ output ] |> Result.expect ~msg:"save should succeed" in
        match Riot_store.Store.get store hash with
        | None -> Error "expected saved artifact"
        | Some artifact ->
            if artifact.ocamlc_warnings = warnings && artifact.exports = exports then
              Ok ()
            else Error "expected hash manifest payload to round-trip through the store"
    ) with
  | Ok x -> x
  | Error _ -> Error "tempdir creation failed"

let test_export_source_path_round_trip = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_export_source_path_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        ...
        let _ = Riot_store.Store.save store ~package:"pkg" ~exports ~hash ~sandbox_dir:sandbox ~outs:[ Path.(sandbox / rel_path) ] |> Result.expect ~msg:"save should succeed" in
...
let test_export_source_path_rejects_absolute_export_paths = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_export_source_path_absolute_path_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        ...
        let exports = [ Riot_store.Store.{ name = "tool"; path = Path.v "/tmp/not-allowed"; action_hash = "deadbeef" } ] in
        let _ = Riot_store.Store.save store ~package:"pkg" ~exports ~hash ~sandbox_dir:sandbox ~outs:[ output ] |> Result.expect ~msg:"save should succeed" in
        match Riot_store.Store.export_source_path store (List.head exports |> Option.expect ~msg:"expected export entry") with
        | None -> Ok ()
...
let test_load_manifest_returns_none_for_malformed_payload = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_load_manifest_malformed_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        ...
        let _ = Fs.write "{\"version\":\"v1\",\"exports\":\"not-an-array\"}" manifest_path |> Result.expect ~msg:"write malformed manifest should succeed" in
        match Riot_store.Store.load_manifest store ~hash with
        | None -> Ok ()
...
let test_materialize_package_exports_from_action_artifact = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_materialize_exports_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        ...
        let _ = Riot_store.Store.save store ~package:"pkg" ~hash:action_hash ~sandbox_dir:sandbox ~outs:[ output ] |> Result.expect ~msg:"save action artifact should succeed" in
        let exports = [ Riot_store.Store.{ name = "tool"; path = Path.v "bin/tool"; action_hash = Crypto.Digest.hex action_hash } ] in
        let target_dir = Path.(tmpdir / Path.v "out" / Path.v "pkg") in
        let _ = Riot_store.Store.materialize_package_exports store ~exports ~target_dir |> Result.expect ~msg:"materialize_package_exports should succeed" in
...
let test_materialize_package_exports_fails_when_source_missing = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_materialize_exports_missing_source_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        let missing_action_hash = Crypto.hash_string "missing-action-hash" |> Crypto.Digest.hex in
        let exports = [ Riot_store.Store.{ name = "tool"; path = Path.v "bin/tool"; action_hash = missing_action_hash } ] in
        let target_dir = Path.(tmpdir / Path.v "out" / Path.v "pkg") in
...
let test_promote_overwrites_existing_target_files = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_promote_overwrite_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        ...
        let _ = Riot_store.Store.save store ~package:"kernel" ~hash ~sandbox_dir:sandbox ~outs:[ output ] |> Result.expect ~msg:"save should succeed" in
        ...
        if String.equal (read_file target_file) "fresh" then
          Ok ()
        else Error "promote should overwrite stale target files"
    ) with
  | Ok x -> x
  | Error _ -> Error "tempdir creation failed"

let test_save_and_promote_preserves_executable_permissions = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_promote_permissions_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        ...
        let source_mode = Fs.metadata output |> Result.expect ~msg:"read source metadata should succeed" |> Fs.Metadata.mode in
        let promoted_mode = Fs.metadata promoted |> Result.expect ~msg:"read promoted metadata should succeed" |> Fs.Metadata.mode in
        if source_mode = promoted_mode then
          Ok ()
        else Error "save/promote should preserve executable permissions"
    ) with
  | Ok x -> x
  | Error _ -> Error "tempdir creation failed"

let test_save_preserves_executable_permissions_in_cache = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_cache_permissions_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        ...
        let _ = Riot_store.Store.save store ~package:"std" ~hash ~sandbox_dir:sandbox ~outs:[ output ] |> Result.expect ~msg:"save should succeed" in
        let cached = Path.(Riot_store.Store.hash_dir_of store hash / Path.v "std_archive_tar_tests") in
        let source_mode = Fs.metadata output |> Result.expect ~msg:"read source metadata should succeed" |> Fs.Metadata.mode in
        let cached_mode = Fs.metadata cached |> Result.expect ~msg:"read cached metadata should succeed" |> Fs.Metadata.mode in
        if source_mode = cached_mode then
          Ok ()
        else Error "save should preserve executable permissions in cache"
    ) with
  | Ok x -> x
  | Error _ -> Error "tempdir creation failed"

let test_get_returns_none_when_export_source_missing = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_missing_export_cache_hit_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        let store = Riot_store.Store.create ~workspace in
        ...
        let _ = Riot_store.Store.save store ~package:"pkg" ~hash:action_hash ~sandbox_dir:sandbox ~outs:[ output ] |> Result.expect ~msg:"save action artifact should succeed" in
        let exports = [ Riot_store.Store.{ name = "tool"; path = Path.v "bin/tool"; action_hash = Crypto.Digest.hex action_hash } ] in
        let _ = Riot_store.Store.save store ~package:"pkg" ~exports ~hash:package_hash ~sandbox_dir:sandbox ~outs:[ output ] |> Result.expect ~msg:"save package artifact should succeed" in
...
let test_cache_gc_drops_unreferenced_entries_after_generation_overflow = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_cache_gc_generation_overflow_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        ...
        let entry_a = write_cache_entry ~workspace ~profile:"debug" ~target:host_target ~hash:hash_a ~size:16 in
        let entry_b = write_cache_entry ~workspace ~profile:"debug" ~target:host_target ~hash:hash_b ~size:16 in
        let _ = Riot_store.Cache_gc.record_successful_build ~workspace ~lanes:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hashes = [ hash_a ] } ] ~new_entries:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hash = hash_a } ] |> Result.expect ~msg:"first generation should record" in
        let summary =
          let _ = Riot_store.Cache_gc.record_successful_build ~workspace ~lanes:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hashes = [ hash_b ] } ] ~new_entries:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hash = hash_b } ] |> Result.expect ~msg:"second generation should record" in Riot_store.Cache_gc.clean ~workspace |> Result.expect ~msg:"clean should enforce generation retention"
        in
        ...
        if summary.ran_gc && summary.deleted_entries = 1 && not entry_a_exists && entry_b_exists then
          Ok ()
        else Error "expected generation overflow GC to keep only the newest live cache entry"
    ) with
  | Ok x -> x
  | Error _ -> Error "tempdir creation failed"

let test_record_successful_build_tracks_generation_count_in_state = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_cache_gc_generation_count_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        ...
        let hash_a = make_hash 'a' in
        let hash_b = make_hash 'b' in
        let _ = write_cache_entry ~workspace ~profile:"debug" ~target:host_target ~hash:hash_a ~size:16 in
        let _ = write_cache_entry ~workspace ~profile:"debug" ~target:host_target ~hash:hash_b ~size:16 in
        let first_summary = Riot_store.Cache_gc.record_successful_build ~workspace ~lanes:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hashes = [ hash_a ] } ] ~new_entries:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hash = hash_a } ] |> Result.expect ~msg:"first generation should record" in
        let second_summary = Riot_store.Cache_gc.record_successful_build ~workspace ~lanes:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hashes = [ hash_b ] } ] ~new_entries:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hash = hash_b } ] |> Result.expect ~msg:"second generation should record" in
        if first_summary.kept_generations = 1 && second_summary.kept_generations = 2 then
          Ok ()
        else Error ("expected generation counts 1 then 2, got " ^ Int.to_string first_summary.kept_generations ^ " then " ^ Int.to_string second_summary.kept_generations)
    ) with
  | Ok x -> x
  | Error _ -> Error "tempdir creation failed"

let test_record_successful_build_dedupes_identical_warm_generation = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_cache_gc_dedupe_generation_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        write_workspace_cache_config tmpdir ~keep_generations:4 ~max_size:"10 GiB";
        ...
        let _ = write_cache_entry ~workspace ~profile:"debug" ~target:host_target ~hash:hash_a ~size:16 in
        let first_summary = Riot_store.Cache_gc.record_successful_build ~workspace ~lanes:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hashes = [ hash_a ] } ] ~new_entries:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hash = hash_a } ] |> Result.expect ~msg:"first generation should record" in
        let second_summary = Riot_store.Cache_gc.record_successful_build ~workspace ~lanes:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hashes = [ hash_a ] } ] ~new_entries:[] |> Result.expect ~msg:"identical warm generation should be accepted" in
        ...
        if first_summary.kept_generations = 1 && second_summary.kept_generations = 1 && receipt_count = 1 && (generation_hashes |> List.length |> Int.equal 1) then
          Ok ()
        else Error ("expected identical warm generation to keep one receipt, got summaries " ^ Int.to_string first_summary.kept_generations ^ " and " ^ Int.to_string second_summary.kept_generations ^ " with " ^ Int.to_string receipt_count ^ " receipts")
    ) with
  | Ok x -> x
  | Error _ -> Error "tempdir creation failed"

let test_record_successful_build_keeps_new_warm_generation_when_closure_changes = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_cache_gc_distinct_generation_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        write_workspace_cache_config tmpdir ~keep_generations:4 ~max_size:"10 GiB";
        ...
        let _ = write_cache_entry ~workspace ~profile:"debug" ~target:host_target ~hash:hash_b ~size:16 in
        let _ = Riot_store.Cache_gc.record_successful_build ~workspace ~lanes:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hashes = [ hash_a ] } ] ~new_entries:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hash = hash_a } ] |> Result.expect ~msg:"first generation should record" in
        let second_summary = Riot_store.Cache_gc.record_successful_build ~workspace ~lanes:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hashes = [ hash_b ] } ] ~new_entries:[] |> Result.expect ~msg:"distinct warm generation should record" in
        let receipt_count = count_generation_receipts ~workspace in
        let generation_hashes = read_cache_state_generation_hashes ~workspace in
        match generation_hashes with
        | newest_hash :: older_hash :: [] ->
            if second_summary.kept_generations = 2 && receipt_count = 2 && read_generation_lane_hashes ~workspace newest_hash = [ [ hash_b ] ] && read_generation_lane_hashes ~workspace older_hash = [ [ hash_a ] ] then
              Ok ()
            else Error ("expected changed warm generation to keep two receipts, got summary " ^ Int.to_string second_summary.kept_generations ^ " with " ^ Int.to_string receipt_count ^ " receipts")
...
let test_record_successful_build_reorders_existing_cached_generation_to_front = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_cache_gc_generation_reorder_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        write_workspace_cache_config tmpdir ~keep_generations:4 ~max_size:"10 GiB";
        ...
        let _ = write_cache_entry ~workspace ~profile:"debug" ~target:host_target ~hash:hash_a ~size:16 in
        let _ = write_cache_entry ~workspace ~profile:"debug" ~target:host_target ~hash:hash_b ~size:16 in
        let _ = Riot_store.Cache_gc.record_successful_build ~workspace ~lanes:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hashes = [ hash_a ] } ] ~new_entries:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hash = hash_a } ] |> Result.expect ~msg:"generation A should record" in
        let _ = Riot_store.Cache_gc.record_successful_build ~workspace ~lanes:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hashes = [ hash_b ] } ] ~new_entries:[] |> Result.expect ~msg:"generation B should record without new entries" in
        let third_summary = Riot_store.Cache_gc.record_successful_build ~workspace ~lanes:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hashes = [ hash_a ] } ] ~new_entries:[] |> Result.expect ~msg:"generation A should move back to the front" in
        let receipt_count = count_generation_receipts ~workspace in
        let generation_hashes = read_cache_state_generation_hashes ~workspace in
        match generation_hashes with
        | newest_hash :: older_hash :: [] ->
            if third_summary.kept_generations = 2 && receipt_count = 2 && read_generation_lane_hashes ~workspace newest_hash = [ [ hash_a ] ] && read_generation_lane_hashes ~workspace older_hash = [ [ hash_b ] ] then
              Ok ()
            else Error "expected cached rollback generation to reorder state without writing a third payload"
...
let test_cache_gc_shrinks_retained_generations_to_meet_max_size = fun _ctx ->
  match Fs.with_tempdir ~prefix:"store_cache_gc_max_size_test"
    (
      fun tmpdir ->
        let workspace = make_test_workspace tmpdir in
        ...
        let entry_a = write_cache_entry ~workspace ~profile:"debug" ~target:host_target ~hash:hash_a ~size:64 in
        let _ = Riot_store.Cache_gc.record_successful_build ~workspace ~lanes:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hashes = [ hash_a ] } ] ~new_entries:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hash = hash_a } ] |> Result.expect ~msg:"generation A should record" in
        let entry_b = write_cache_entry ~workspace ~profile:"debug" ~target:host_target ~hash:hash_b ~size:64 in
        let _ = Riot_store.Cache_gc.record_successful_build ~workspace ~lanes:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hashes = [ hash_b ] } ] ~new_entries:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hash = hash_b } ] |> Result.expect ~msg:"generation B should record" in
        let entry_c = write_cache_entry ~workspace ~profile:"debug" ~target:host_target ~hash:hash_c ~size:64 in
        let summary =
          let _ = Riot_store.Cache_gc.record_successful_build ~workspace ~lanes:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hashes = [ hash_c ] } ] ~new_entries:[ Riot_store.Cache_gc.{ profile = "debug"; target = host_target; hash = hash_c } ] |> Result.expect ~msg:"generation C should record" in Riot_store.Cache_gc.clean ~workspace |> Result.expect ~msg:"clean should enforce max_size policy"
        in
        let entry_a_exists = Fs.exists entry_a |> Result.unwrap_or ~default:false in
        let entry_b_exists = Fs.exists entry_b |> Result.unwrap_or ~default:false in
        let entry_c_exists = Fs.exists entry_c |> Result.unwrap_or ~default:false in
        if summary.ran_gc && summary.kept_generations = 1 && not entry_a_exists && not entry_b_exists && entry_c_exists then
          Ok ()
        else Error "expected max_size GC to drop older retained generations until the cache fits"
    ) with
  | Ok x -> x
  | Error _ -> Error "tempdir creation failed"
