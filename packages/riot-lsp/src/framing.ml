...
let strip_line_ending = fun line ->
  ...
  if len >= 2 && String.sub line ~offset:(len - 2) ~len:2 = "\r\n" then
    ...
    if len >= 1 && String.get line ~at:(len - 1) = Some '\n' then
...
let header_separator = fun payload ->
  ...
  let rec loop index =
    if index + 3 < len && String.sub payload ~offset:index ~len:4 = "\r\n\r\n" then
      ...
      if index + 1 < len && String.sub payload ~offset:index ~len:2 = "\n\n" then
        Some (index, 2)
      else
        if index >= len then
          None
...
let encode = fun payload -> "Content-Length: " ^ Int.to_string (String.length payload) ^ "\r\n\r\n" ^ payload
...
let decode_one = fun framed ->
  ...
  | Some (header_end, separator_len) ->
      let headers = String.sub framed ~offset:0 ~len:header_end |> String.split_on_char '\n' |> List.map ~fn:strip_line_ending |> List.filter ~fn:(
      ...
      if String.length body < content_length then
        ...
        let rest = String.sub body ~offset:content_length ~len:(String.length body - content_length) in Ok (payload, rest)
...
let read = fun input ->
  let read_line () =
    ...
    let rec loop () =
      let* read = IO.Reader.read input ~into:chunk |> Result.map_err ~fn:IO.error_message
      in
      if read = 0 then
        Ok (IO.Buffer.contents buffer)
      else
        if not (Int.equal read 1) then
  ...
  let rec read_headers acc =
    let* raw_line = read_line ()
    in
    if String.equal raw_line "" then
      if List.is_empty acc then
        Ok None
      else Error "unexpected EOF while reading LSP headers"
    else
  ...
  | Some headers ->
      let* content_length = parse_content_length headers
      in
      let* payload = read_exact content_length in Ok (Some payload)

let write = fun output payload ->
  let buffer = IO.Buffer.from_string (encode payload) in IO.Writer.write_all output ~from:buffer |> Result.map_err ~fn:IO.error_message
