...
module File_log = struct
  type t = { path: Path.t; sink: Fs.File.t }
  ...
  let open_sink = fun ?path () ->
    ...
    in
    let* sink = Fs.File.open_append path |> Result.map_err ~fn:Fs.File.error_to_string in Ok { path; sink }

  let write = fun t ~level message ->
    let line = DateTime.to_iso8601 (DateTime.now_utc ()) ^ " | " ^ level ^ " | " ^ message ^ "\n" in ignore ((Fs.File.write_all t.sink line : (unit, _) result))
...
let write_outbound = fun output messages -> List.fold_left messages ~init:(Ok ()) ~fn:(
  fun acc json ->
    let* () = acc in Framing.write output (Std.Data.Json.to_string json) |> Result.map_err ~fn:(
...
let rec loop = fun logger ->
  fun input ->
    fun output ->
      fun state ->
        let* payload_opt = Framing.read input |> Result.map_err ~fn:(
          fun message -> Failure message
        )
        in
        ...
        | Some payload ->
            ...
            if not (List.is_empty outcome.outbound) then
              log logger ~level:"DEBUG" ("sending " ^ Int.to_string (List.length outcome.outbound) ^ " outbound message(s)");
            (
              match outcome.exit_code with
              | None -> ()
              | Some code -> log logger ~level:"INFO" ("lsp session exiting with code " ^ Int.to_string code)
            );
            let* () = write_outbound output outcome.outbound
            in
            match outcome.exit_code with
            | Some 0 -> Ok ()
