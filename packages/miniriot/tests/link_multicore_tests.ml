...
let linked_trap_exit_observer = fun ~parent ~child ->
    ...
    if Pid.equal from_pid child then
      ...
      | Error _ ->
          ...
          Result.Ok ()
      | Ok () -> Result.Error (Failure "expected abnormal EXIT reason")
    else
      Result.Error (Failure "received EXIT from unexpected pid")

let linked_non_trap_observer = fun ~parent ~child ->
    Process.link child;
    send parent Linked_ready;
    let _ = receive_any ~timeout:20.0 () in
    Result.Ok ()

let await_child_ready = fun expected_child ->
    let pid =
      receive
        ~selector:(
          function
...
let test_link_trap_exit_receives_exit_message = fun () ->
    let parent = self () in
    let child =
      spawn (fun () -> crashing_child parent)
    in
    let _ =
      spawn (fun () -> linked_trap_exit_observer ~parent ~child)
    in
...
let test_link_without_trap_exit_dies_on_abnormal_exit = fun () ->
    let parent = self () in
    let child =
      spawn (fun () -> crashing_child parent)
    in
    let observer =
      spawn (fun () -> linked_non_trap_observer ~parent ~child)
    in
        ...
        | Result.Ok () ->
            ...
            let pid, reason =
              receive
                ~selector:(
                  function
                  | Process.DOWN { pid; reason; _ } when Pid.equal pid observer -> `select (
                    pid,
                    reason
                  )
                  | _ -> `skip
...
let () =
  ...
  in
  let main ~args =
    match Test.Cli.main ~name:"link_multicore_tests" ~tests ~args:(normalize_args args) with
    | Result.Ok () -> Result.Ok ()
