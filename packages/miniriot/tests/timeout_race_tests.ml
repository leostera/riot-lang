...
let timed_receiver parent ~rounds =
  let rec loop round =
    if int_eq round rounds then (
      ...
      | Some idx when int_eq idx round -> loop (Int.succ round)
      | Some idx ->
          let msg =
            String.concat ""
              [ "round mismatch: expected "; Int.to_string round;
                ", got "; Int.to_string idx ]
          in
      ...
      | None ->
          let msg =
            String.concat ""
              [ "receive timed out at round "; Int.to_string round ]
          in
          send parent (Timeout_failed msg);
          Result.Error (Failure msg))
  in
...
let await_no_extra_round_ready () =
  let extra =
    try
      Some
        (receive
  ...
  | Some round ->
      Result.Error
        (String.concat ""
           [ "unexpected extra round readiness after completion: ";
             Int.to_string round ])

let test_receive_timeout_cancel_race () =
  let rounds = 96 in
  let parent = self () in
  let worker = spawn (fun () -> timed_receiver parent ~rounds) in
  let rec drive expected_round =
    ...
    | `ready round ->
        if int_eq round expected_round then (
          let _timer_id =
            Timer.send_after worker (Round_ping round) ~after:0.001
          in
          ...
          Result.Error
            (String.concat ""
               [ "unexpected round readiness: expected ";
                 Int.to_string expected_round; ", got ";
                 Int.to_string round ])
    | `finished ->
        if int_eq expected_round rounds then
          ...
          Result.Error
            (String.concat ""
               [ "worker finished early at round ";
...
let () =
  let tests =
    [ Test.case "receive timeout cancellation race" (fun () ->
          test_case "receive timeout cancellation race"
            test_receive_timeout_cancel_race);
    ]
  in
  ...
  let main ~args =
    match
      Test.Cli.main ~name:"timeout_race_tests" ~tests
