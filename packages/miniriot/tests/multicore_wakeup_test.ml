...
let sender ~target ~count ~parent =
...
let test () =
  ...
  let rec spawn_senders n =
    if Kernel.Int.equal n 0 then
      ...
      let _ =
        spawn (fun () ->
            sender ~target:receiver_pid ~count:messages_per_sender ~parent)
      in
        ...
        if Kernel.Int.equal sender_done senders then
          ...
          | Some Sender_started ->
              collect
                ...
                ~sender_done ~receiver_count
          | Some Sender_done ->
              collect
                ~sender_started
                ~sender_done:(Kernel.Int.succ sender_done)
                ~receiver_count
          | Some (Receiver_done next_count) ->
              collect
                ~sender_started ~sender_done ~receiver_count:(Some next_count)
          | Some _ ->
              collect ~sender_started ~sender_done ~receiver_count)
    | Some count ->
        Result.Error
          (Kernel.String.concat ""
             [ "receiver count mismatch: expected ";
               Kernel.Int.to_string expected_total;
               ", got "; Kernel.Int.to_string count ])
    | _ ->
        let next =
          try Some (receive_any ~timeout:20.0 ()) with
          | Receive_timeout -> None
        ...
        | Some Sender_done ->
            collect
              ...
              ~sender_done:(Kernel.Int.succ sender_done)
              ~receiver_count
        | Some (Receiver_done count) ->
            collect ~sender_started ~sender_done ~receiver_count:(Some count)
        | Some _ ->
            collect ~sender_started ~sender_done ~receiver_count)
  in
...
let () =
  ...
  let main ~args =
    match
      Test.Cli.main ~name:"multicore_wakeup_tests" ~tests
