...
let receiver = fun ~parent scenario ->
  ...
  in
  let rec collect = fun ~seen_data ~seen_done ->
    if int_eq seen_data expected_total then
      ...
      | `Data (sender_id, seq) ->
          if int_lt sender_id 0 || not (int_lt sender_id scenario.sender_count) then
            fail
            (String.concat "" [ "received message from invalid sender_id "; Int.to_string sender_id ])
          else
            let expected =
