...
module Tracking_source = struct
  ...
  let register state _selector _token _interest =
  ...
  let reregister state _selector _token _interest =
    register state _selector _token _interest

  let deregister state _selector =
...
let run_timeout_loop ~parent ~source ~rounds =
  let rec loop n =
    if int_eq n 0 then (
      ...
      let syscall_result =
        try
          let _ =
            syscall ~name:"syscall-timeout-test" ~interest:Interest.readable
              ~source ~timeout:0.002 (fun () ->
      ...
      | `unexpected_success ->
          send parent
            (Syscall_worker_error "syscall callback unexpectedly executed");
...
let test_syscall_timeout_deregisters_wait_registration () =
  let parent = self () in
  let state =
    {
      registered = false;
  ...
  in
  let source = Source.make (module Tracking_source) state in
  let rounds = 32 in
  let _worker = spawn (fun () -> run_timeout_loop ~parent ~source ~rounds) in
  let outcome =
    receive
      ~selector:(function
        ...
        | Syscall_unexpected_success ->
            `select
              (Result.Error "unexpected syscall callback success")
        | _ -> `skip)
      ~timeout:20.0 ()
  in
  match outcome with
...
let () =
  let tests =
    [
      Test.case "syscall timeout deregisters wait registration" (fun () ->
          test_case "syscall timeout deregisters wait registration"
            test_syscall_timeout_deregisters_wait_registration);
    ]
  in
  ...
  let main ~args =
    match
      Test.Cli.main ~name:"syscall_timeout_tests" ~tests
