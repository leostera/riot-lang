...
let spawn_burst = fun ~parent ~tasks ~yield_count ->
  let rec spawn_tasks = fun remaining ->
...
let collect_burst_completions = fun ~expected ->
  let rec loop = fun received ->
...
let () =
  ...
  in
  let main = fun ~args ->
    match Test.Cli.main ~name:"work_stealing_counters_tests" ~tests ~args:(normalize_args args) with
    | Result.Ok () -> Result.Ok ()
