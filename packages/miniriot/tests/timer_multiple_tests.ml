...
let rec int_list_equal = fun a b ->
  match (a, b) with
  | [], [] ->
      true
  | x :: xs, y :: ys -> (
...
let test = fun () ->
  let () = Kernel.println "Setting up 5 timers with different delays..." in
  let my_pid = self () in
  ...
  let rec collect = fun n acc ->
    if Kernel.Int.equal n 0 then
      Kernel.Collections.List.rev acc
    else
      match receive_any () with
      | Tick i ->
          Kernel.println (Kernel.String.concat "" [ "  Received Tick "; Kernel.Int.to_string i ]);
...
let () =
  ...
  in
  let main = fun ~args ->
    match Test.Cli.main ~name:"timer_multiple_tests" ~tests ~args:(normalize_args args) with
    | Result.Ok () -> Result.Ok ()
