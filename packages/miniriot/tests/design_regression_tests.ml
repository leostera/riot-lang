...
let selective_receive_timeout_probe parent =
...
let selective_receive_timeout_rearm_probe parent =
  send parent Probe_ready;
  let () =
    receive
      ~selector:(function
...
let test_selective_receive_timeout_ignores_unmatched_saved_messages () =
  let parent = self () in
  let worker =
  ...
  let _ =
    Timer.send_after worker Probe_expected ~after:0.10
  in
  match
    receive
      ~selector:(function
        ...
        | Probe_matched_expected ->
            `select
              (Result.Error
                 "receive matched a delayed message instead of timing out \
...
let test_selective_receive_timeout_does_not_rearm_after_unmatched_wakeup () =
  let parent = self () in
  let worker =
  ...
  in
  let _ =
    Timer.send_after worker Probe_noise ~after:0.01
  in
  let _ =
    Timer.send_after worker Probe_expected ~after:0.12
  in
  match
    receive
      ~selector:(function
        ...
        | Probe_matched_expected ->
            `select
              (Result.Error
                 "receive rearmed its timeout after an unmatched wakeup \
...
let test_interval_cancel_stops_future_ticks () =
  let me = self () in
  let timer_id =
    Timer.send_interval me Interval_tick ~interval:0.05
  in
  let () =
    receive
      ~selector:(function
  ...
  | Some () ->
      Result.Error
        "cancelled interval timer delivered a later tick"
...
let () =
    ...
    [
      Test.case
        ...
        (fun () ->
          test_case
            "selective receive timeout ignores unmatched saved messages"
            test_selective_receive_timeout_ignores_unmatched_saved_messages);
      Test.case
        "selective receive timeout does not rearm after unmatched wakeup"
        (fun () ->
          test_case
            "selective receive timeout does not rearm after unmatched wakeup"
            test_selective_receive_timeout_does_not_rearm_after_unmatched_wakeup);
      Test.case
        "interval cancel stops future ticks"
        (fun () ->
          test_case "interval cancel stops future ticks"
            test_interval_cancel_stops_future_ticks);
    ]
  in
  ...
  let main ~args =
    match
      Test.Cli.main ~name:"design_regression_tests" ~tests
