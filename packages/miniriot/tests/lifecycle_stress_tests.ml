...
let staged_crasher parent index =
  let pid = self () in
  ...
  if int_eq target index then
    Result.Error
      (Failure
         (String.concat ""
            [ "crash-"; Int.to_string index ]))
  else
    Result.Ok ()

let rec spawn_children ~parent ~count ~spawn_fn index acc =
  if int_eq index count then
    List.rev acc
  else
    let pid = spawn_fn index in
    spawn_children ~parent ~count ~spawn_fn (Int.succ index)
      ((index, pid) :: acc)

let await_ready_messages count =
  ...
  let rec loop received =
    if int_eq received count then
      ...
      let key =
        String.concat ":"
          [ Int.to_string index; Pid.to_string pid ]
      in
      if HashSet.insert seen key then
        ...
        Result.Error
          (String.concat ""
             [ "duplicate child readiness for "; key ])
  in
  loop 0

let crash_children children =
  List.iter
    (fun (index, pid) -> send pid (Crash_now index))
    children

let collect_down_messages expected =
  let seen = HashSet.create () in
  let rec loop received =
    if int_eq received expected then
      ...
      if HashSet.insert seen key then
        ...
        | Ok () ->
            Result.Error
              (String.concat ""
                 [ "expected abnormal DOWN for pid "; key ])
      else
        Result.Error
          (String.concat ""
             [ "duplicate DOWN for pid "; key ])
  in
  loop 0

let collect_exit_messages expected =
  let seen = HashSet.create () in
  let rec loop received =
    if int_eq received expected then
      Result.Ok ()
      ...
      if HashSet.insert seen key then
        ...
        | Ok () ->
            Result.Error
              (String.concat ""
                 [ "expected abnormal EXIT for pid "; key ])
      else
        Result.Error
          (String.concat ""
             [ "duplicate EXIT for pid "; key ])
  in
  loop 0

let test_monitor_exit_storm () =
  let parent = self () in
  let child_count = 48 in
  let spawn_fn index =
    let pid = spawn (fun () -> staged_crasher parent index) in
    let _monitor_ref = Process.monitor pid in
    pid
  in
  let children =
    spawn_children ~parent ~count:child_count ~spawn_fn 0 []
  in
...
let () =
  let tests =
    [ Test.case "monitor exit storm across workers" (fun () ->
          test_case "monitor exit storm" test_monitor_exit_storm);
      Test.case "trap_exit link storm across workers" (fun () ->
          test_case "trap_exit link storm" test_trap_exit_storm);
    ]
  in
  ...
  let main ~args =
    match
      Test.Cli.main ~name:"lifecycle_stress_tests" ~tests
