...
let () =
  ...
  let main () =
    ...
    yield (); yield (); (* Let worker process and exit *)
    states := "final" :: !states;
    
    (* Check we got expected transitions *)
    let has_state s = List.mem s !states in
    if has_state "main_start" && has_state "spawned" && 
       has_state "running" && has_state "final" then
      Process.Normal
    else
      Process.Exception (Failure "State transitions missing")
  in
