...
type Message.t += Hello of string | Count of int | Ping | Pong
...
let test_basic_send_receive () =
  ...
  if status <> 0 then
    Printf.printf
      "✗ test_basic_send_receive failed with status %d, received: %s\n" status
      (match !received with
      | None -> "None"
      | Some s -> Printf.sprintf "Some(%s)" s)
    (* Don't assert yet, let's see all test results *)
  else Printf.printf "✓ test_basic_send_receive\n"
...
let test_ping_pong () =
  ...
  let main () =
    ...
    for _ = 1 to 3 do
      ...
      match receive () with Pong -> () | _ -> failwith "Expected Pong"
    done;

    send ponger_pid Exit;
    yield ();

    if !pings = 3 && !pongs = 3 then Process.Normal
    else
      Process.Exception
        (Failure (Printf.sprintf "Pings: %d, Pongs: %d" !pings !pongs))
  in
