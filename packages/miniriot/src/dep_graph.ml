...
module Module: sig
  ...
  type t = { module_name: Module_name.t; file_path: string; namespaced_name: Namespace.t }

  let eq = fun a b -> String.equal a.file_path b.file_path

  let of_path = fun ~ns path ->
    let module_name = Module_name.of_path path in { file_path = path; module_name; namespaced_name = Namespace.add ns module_name }
...
module Build_results: sig
  ...
  *)
  ...
  let get_package_names = fun t -> (* Get list of all registered package names in registration order *)
...
module Module_registry: sig
  ...
  let create = fun () -> { modules = Hashtbl.create 16; intf_by_name = Hashtbl.create 16; impl_by_name = Hashtbl.create 16 }
  ...
  let print = fun t ->
    ...
    let sorted_entries =
      List.sort
        (
          fun ((_, mod1)) ((_, mod2)) -> String.compare (Module.module_name mod1 |> Module_name.to_string) (Module.module_name mod2 |> Module_name.to_string)
        )
        entries
    in
    List.iter
      (
        fun ((node_id, mod_)) -> printf "    Node %s -> %s | %s (%s)\n" (Graph.Node_id.to_string node_id) (Module.module_name mod_ |> Module_name.to_string) (Module.namespaced_name mod_) (Module.path mod_)
      )
      sorted_entries
end
...
let to_dot = fun dep_graph -> Graph.to_dot dep_graph.graph ~name:(Module_name.to_string dep_graph.package_name) ~node_to_label:(
  fun dep ->
    match dep.file with
...
module Alias_module = struct
  let template = fun (modules: Module.t list) ->
    ...
    let unique_modules =
      modules |> List.filter_map
        (
          fun mod_ ->
            match Module.kind mod_ with
            | `interface -> None
            | `implementation -> Some (Module.module_name mod_ |> Module_name.to_string, Module.namespaced_name mod_)
        ) |> List.sort_uniq
        (
  ...
  let make_node = fun (ns: Namespace.t) (modules: Module.t list) ->
    ...
    let kind = ML mod_ in { file; open_modules = []; kind }
end
...
module Library_interface = struct
  let template = fun (children: Module.t list) ->
    ...
    let unique_names =
      children |> List.map
        (
          fun mod_ -> Module.module_name mod_ |> Module_name.to_string
  ...
  let make_node = fun (lib: Module.t) (children: Module.t list) aliases ~exists ~actual_path ->
    ...
    in
    let open_modules = aliases in { file; open_modules; kind }
end
...
module Dependency_rules = struct
  let is_dependency_allowed = fun ~from_module ~to_module ->
    ...
    let parse_namespace name =
      String.split_on_char '_' name |> List.filter
        (
    ...
    (* ["Std"; "Net"; "Http"; "Request"] and ["Std"; "Net"; "Http"; "Response"] *)
      ...
      if (List.rev from_parts |> List.tl |> List.rev) = (List.rev to_parts |> List.tl |> List.rev) then
        true
      (* Parent depending on child: to_parts starts with from_parts *)
      (* ["Std"; "Net"] depending on ["Std"; "Net"; "Http"] *)
      else
        if List.length to_parts > List.length from_parts && List.take (List.length from_parts) to_parts = from_parts then
          true
        (* Ancestor's sibling: find common ancestor, check if to is its child *)
          ...
          let rec common_prefix l1 l2 =
            ...
            | _ -> []
          in
          let common = common_prefix from_parts to_parts in (* to_module must be exactly one level below common ancestor *)
          (* from ["Std"; "Net"; "Http"; "Client"; "Pool"] can depend on
             ["Std"; "Net"; "Server"] (Server is sibling of Http)
          *)
          List.length to_parts = List.length common + 1 && List.length from_parts > List.length common + 1
end
...
let rec do_scan = fun ~t ~ctx (file_tree: File_scanner.file_tree) ->
  ...
  | File_scanner.File ({ ext = ".ml" | ".mli"; _ } as file) -> handle_ocaml_module ~t ~ctx file
...
and handle_c_file = fun ~t ~ctx file ->
  ...
  let c_node = Graph.add_node t.graph node in (* printf "  Added C node %d for %s\n" (Graph.Node_id.to_int c_node.id) file.path; *)
...
and handle_ocaml_module = fun ~t ~ctx file ->
  ...
  if String.contains basename 'e' && String.contains basename 'v' then
    Printf.printf "[DEBUG] Processing module: %s -> %s (ns=%s)\n" file.path (Module.namespaced_name mod_) (Namespace.to_string ns);
  let is_binary =
    List.exists
      (
        fun (bin: Package.binary) ->
          let bin_basename = Filename.basename bin.path in
          let file_basename = Filename.basename file.path in bin_basename = file_basename
      )
        ...
        List.iter
          (
            fun intf_node_id ->
              ...
              | MLI intf_mod when Module.module_name intf_mod = mod_name -> (* Add edge from implementation to interface *)
...
and handle_library = fun ~t ~ctx { path; name; children } ->
  ...
  let base_path =
    if Namespace.is_empty ns then
      t.src_root ^ "/" ^ name
    else (* Nested library: packages/std/src/data/data *)
    path ^ "/" ^ name
  in
  ...
  *)
  ...
  let children =
    List.map
      (
        fun child ->
          match child with
          | File_scanner.File { ext = ".ml" | ".mli"; _ } -> (Some (Module.of_path ~ns (File_scanner.path child)), child)
          | _ -> (None, child)
      )
      children
  in
      ...
      (
        fun ((mod_opt, child)) ->
          ...
          | Some mod_, File_scanner.File { path; _ } ->
              ...
              if Module_name.to_string (Module.module_name mod_) = Module_name.to_string (Module.module_name impl_mod) && Filename.extension path = ".ml" then
                Some path
              else None
          | _ -> None
      )
    ...
    List.find_map
      (
        fun ((mod_opt, child)) ->
          ...
          | Some mod_, File_scanner.File { path; _ } ->
              (* Compare module names, not paths - this handles case normalization *)
              if Module_name.to_string (Module.module_name mod_) = Module_name.to_string (Module.module_name intf_mod) && Filename.extension path = ".mli" then
                Some path
              else None
          | _ -> None
      )
  ...
  let children_without_lib_files =
    ...
    let dir_names =
      List.filter_map
        ...
        children |> List.map String.lowercase_ascii |> List.sort_uniq String.compare
    in
    List.filter
      (
        fun ((mod_opt, child)) ->
          ...
          | Some mod_, File_scanner.File { path; _ } ->
              ...
              let is_subdir_lib = List.mem basename dir_names in (* Filter out both root and subdirectory library interface files *)
  ...
  let file_modules =
    List.filter_map
      (
        fun ((mod_opt, child)) ->
          match mod_opt, child with
          | Some mod_, File_scanner.File { ext = ".mli" | ".ml"; _ } -> Some mod_
          | _ -> None
      )
  ...
  let existing_module_names =
    List.map
      (
        fun m -> Module.module_name m |> Module_name.to_string
      )
      file_modules |> List.sort_uniq String.compare
  in
      ...
      (
        fun ((mod_opt, child)) ->
          ...
          | None, File_scanner.Dir { name; path; _ } ->
              ...
              else (* For directories, create a module from the directory name with .ml extension *)
  ...
  let aliases_node =
    let node = Alias_module.make_node ns child_modules in Graph.add_node t.graph node
  in
  ...
  *)
  let intf_node, impl_node =
    if has_library_interface_ml && not has_library_interface_mli then
      let impl = Library_interface.make_node impl_mod child_modules (aliases @ [ aliases_node ]) ~exists:true ~actual_path:library_interface_ml_path in
      let impl_node = Graph.add_node t.graph impl in Module_registry.register t.registry intf_mod impl_node.id;
      ...
      (
        ...
        let intf_node =
          let intf = Library_interface.make_node intf_mod child_modules (aliases @ [ aliases_node ]) ~exists:has_library_interface_mli ~actual_path:library_interface_mli_path in Graph.add_node t.graph intf
        in
        Module_registry.register t.registry intf_mod intf_node.id;
        let impl_node =
          let impl = Library_interface.make_node impl_mod child_modules (aliases @ [ aliases_node ]) ~exists:has_library_interface_ml ~actual_path:library_interface_ml_path in Graph.add_node t.graph impl
        in
        Module_registry.register t.registry impl_mod impl_node.id;
                ...
                (
                  fun node_id ->
                    try
                      ...
                      | (`interface, MLI mod_) | (`implementation, ML mod_) ->
  ...
  Printf.printf "[DEBUG] Adding %d subdir lib dependencies to %s\n" (List.length !subdir_lib_intfs) (Module.namespaced_name intf_mod);
  List.iter
    (
          ...
          fun dep ->
            ...
            if Module_name.to_string dep_name = "IO" then
              Printf.printf "[DEBUG] Resolving IO dep from %s\n"
                (
                  match node.value.file with
                  | Concrete p | Generated { path = p; _ } -> p
                );
