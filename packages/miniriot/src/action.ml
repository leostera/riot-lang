...
let print = fun action ->
  ...
  | CompileInterface { src_file; _ } -> Printf.printf "  Compiling %s\n" (Filename.basename src_file)
  | CompileImplementation { src_file; _ } -> Printf.printf "  Compiling %s\n" (Filename.basename src_file)
...
let print_action = fun action ->
  ...
  | CompileInterface { src_file; output; _ } -> Printf.printf "COMPILE_INTF: %s -> %s\n" src_file output
  | CompileImplementation { src_file; output; _ } -> Printf.printf "COMPILE_IMPL: %s -> %s\n" src_file output
  | CompileC { src_file; _ } -> Printf.printf "COMPILE_C: %s\n" src_file
  | CreateArchive { archive_name; object_files; _ } -> Printf.printf "BUILD_ARCHIVE: %s (%d objects)\n" archive_name (List.length object_files)
  | CreateExecutable { exe_name; main_module; archive; _ } -> Printf.printf "BUILD_EXECUTABLE: %s from %s + %s\n" exe_name main_module archive
  | SetPermissions { path; executable } -> Printf.printf "SET_PERMISSIONS: %s (executable=%b)\n" path executable

let print_build_plan = fun plan -> (* Quiet mode - only show action count *)
...
let execute_action = fun ~project_root ~package ~cc_flags ~ld_flags ~uses_stdlib ~uses_unix ~uses_dynlink action ->
  ...
  | CompileImplementation { sandbox_dir; src_file; output; includes; opens; is_aliases } -> (
    (* Now we're in sandbox_dir, so use relative paths *)
    (* Build flags for open modules *)
    let base_flags =
      List.map
  ...
  | CompileC { sandbox_dir; src_file } -> (
    ...
    let wrapped_cc_flags =
      cc_flags |> List.concat_map
        (
          fun flag -> [ "-ccopt"; flag ]
        )
    in
    match Ocaml_platform.Ocamlc.compile_c ~cc_flags:wrapped_cc_flags ~includes:[ "." ] ~output src_file with
    | Ok _ -> ()
    | Error err ->
        Printf.printf "%s\n%!" err;
        failwith "compilation error"
  ...
  | CreateArchive { sandbox_dir; archive_name; object_files; includes } -> (
    ...
    let has_c_stubs =
      List.exists
        (
          fun file -> Filename.check_suffix file ".o"
        )
        object_files
    in
    let wrapped_cc_flags =
    ...
    in
    match Ocaml_platform.Ocamlc.run ~includes:("." :: final_includes) ~output:(Some archive_name) ~mode:Ocaml_platform.Library ~flags ~extra_args object_files with
    | Ok _ -> ()
    | Error err ->
        Printf.printf "%s\n%!" err;
        failwith "compilation error"
  ...
  | CreateExecutable { sandbox_dir; exe_name; main_module; archive; dependencies } -> (
    ...
    let cmd_parts =
      (((((([ Ocaml_platform.Ocamlc.ocamlc_path ] @ (
        ...
        includes) @ [ "-o"; exe_name ]) @ libs) @ wrapped_cc_flags) @ wrapped_ld_flags) @ [ main_module ]
    in
...
let execute_build_plan = fun ~build_results plan ->
  ...
  List.iter (execute_action ~project_root:original_cwd ~package:plan.package ~cc_flags:plan.cc_flags ~ld_flags:plan.ld_flags ~uses_stdlib:plan.uses_stdlib ~uses_unix:plan.uses_unix ~uses_dynlink:plan.uses_dynlink) plan.actions;
...
let promote_outputs = fun (plan: build_plan) ->
  ...
  let package_name =
    ...
    | first :: _ ->
        let parts = String.split_on_char '/' first in List.nth parts 3
...
let from_dep_graph: Dep_graph.t -> build_plan = fun dep_graph ->
  let sandbox_dir = Printf.sprintf "_build/bootstrap/sandbox/%s" (Dep_graph.Module_name.to_string dep_graph.package_name) in
  let actions = ref [] in
  ...
  let transitive_cc_flags = Dep_graph.Build_results.get_transitive_cc_flags dep_graph.build_results dependencies in
  let transitive_ld_flags = Dep_graph.Build_results.get_transitive_ld_flags dep_graph.build_results dependencies in
      ...
      List.exists
        (
          fun node_id ->
            ...
            | Dep_graph.MLI intf_mod -> Dep_graph.Module.module_name intf_mod = Dep_graph.Module.module_name mod_
            | _ -> false
        )
        node_ids
    with
  ...
  Dep_graph.iter
    (
      fun node -> let open Dep_graph in
    ...
    (
      ...
      | { kind = ML mod_; file = Concrete path; open_modules; _ } ->
          ...
          if not (has_interface mod_) then
            let cmi_output = Module.cmi mod_ in outputs := Filename.concat sandbox_dir cmi_output :: !outputs
      ...
      | { file = Generated { path; contents }; _ } ->
          ...
          let action = WriteFile { path; content = contents } in actions := action :: !actions
      ...
      | { kind = H; _ } -> (* Header files already copied at the beginning *)
      ()
      | _ -> ()
    )
    dep_graph;
