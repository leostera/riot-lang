...
let default = {
  timer_resolution = Millisecond;
  scheduler_count = default_scheduler_count;
}

let make ?(timer_resolution = Millisecond) ?(scheduler_count = default_scheduler_count) ()
    =
  {
    timer_resolution;
...
let worker_count config = config.scheduler_count
