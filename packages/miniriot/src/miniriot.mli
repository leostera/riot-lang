...
module Net : sig
  ...
  module TcpClient : sig
    (** TCP client for line-based protocols.
        ...
        This module provides a simple TCP client that handles line-based
        protocols (where messages are delimited by newlines). It properly
        buffers data to handle cases where multiple messages arrive in a single
        read, or where a message spans multiple reads.
    ...
    type t
    (** The client connection type. Contains the TCP stream and internal
        buffers. *)
    ...
    (** [send client data] sends the string data to the server. The string
        should include any necessary delimiters (e.g., newlines). The entire
        string will be sent before returning. Returns [Error] if the send fails.
    *)

    val receive : t -> (string, string) result
    (** [receive client] reads from the server until a newline character is
        found, then returns the line (without the newline). If multiple lines
        were received in a single read, the additional data is buffered
        internally and will be returned by subsequent calls to [receive].
