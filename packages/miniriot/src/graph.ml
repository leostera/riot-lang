...
type 'value node = { id: Node_id.t; mutable deps: Node_id.t list; value: 'value }
...
let get_node = fun t node_id -> Hashtbl.find t.nodes node_id
  ...
  Hashtbl.iter
    (
      fun _id node ->
        ...
        in
        Buffer.add_string buffer (Printf.sprintf "  n%s [label=\"%s\"%s];\n" (Node_id.to_string node.id) label attrs_str)
    )
    graph.nodes;
  ...
  Hashtbl.iter
    (
      fun _id node ->
        List.iter
          (
            fun dep_id -> Buffer.add_string buffer (Printf.sprintf "  n%s -> n%s;\n" (Node_id.to_string node.id) (Node_id.to_string dep_id))
          )
          node.deps
    )
    graph.nodes;
    ...
    (
      fun my_id node ->
        ...
        List.iter
          (
            fun dep_id ->
              let current_rev_deps = Hashtbl.find reverse_deps dep_id in Hashtbl.replace reverse_deps dep_id (my_id :: current_rev_deps)
          )
          node.deps
    )
    graph.nodes;
