...
5. Keep cooperative yielding process-local: manual `Miniriot.yield` calls and `Proc_state.run` effect stepping should spend the same process-owned reduction budget instead of separate domain-local counters.
6. `miniriot` owns its package-provided `tusk-fix` rules under `fix/`; keep those diagnostics aligned with scheduler fairness and cooperative yielding semantics.
