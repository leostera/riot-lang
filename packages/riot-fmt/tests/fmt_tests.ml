...
let make_capture_writer = fun () ->
  ...
  ), fun () -> !chunks |> List.reverse |> String.concat "")
...
let parse_jsonl = fun output -> output |> String.split ~by:"\n" |> List.filter ~fn:(
  fun line -> not (String.equal line "")
) |> List.map ~fn:(
  fun line -> Data.Json.of_string line |> Result.expect ~msg:"parse json line"
)

let parse_package_manifest = fun ~path ~relative_path toml -> Data.Toml.parse toml |> Result.expect ~msg:"parse package manifest toml" |> Riot_model.Package_manifest.from_toml ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path ~relative_path |> Result.expect ~msg:"package manifest"
...
let test_fmt_accepts_multiple_paths = fun _ctx ->
  ...
  | Ok matches ->
      ...
      Test.assert_equal ~expected:[ "packages/blink/src/connection.ml"; "packages/syn/src/parser.ml" ] ~actual;
      Ok ()
...
let test_fmt_usage_shows_variadic_paths = fun _ctx ->
  ...
  if String.contains usage "path..." then
    Ok ()
  else Error ("expected variadic path usage, got: " ^ usage)
        ...
        (
          ...
          | Error _ ->
              if String.contains (stdout_contents ()) "needs.ml (needs formatting)" && String.equal (stderr_contents ()) "" then
                Ok ()
              else Error ("expected fmt --check to report the unformatted file, got stdout=" ^ stdout_contents () ^ " stderr=" ^ stderr_contents ())
        )
    )
    ...
    (
      fun tmpdir ->
        ...
        if Vector.length parsed.diagnostics = 0 then
          ...
          let expected = Syn.DiagnosticReporter.format ~file:(Path.to_string broken) ~source (diagnostics_to_list parsed.diagnostics) in
          (
            match Riot_fmt.run ~stdout ~stderr matches with
      ...
      fun tmpdir ->
        ...
        if Vector.length parsed.diagnostics = 0 then
          ...
          (
            ...
            | Error _ ->
                let file_event = stdout_contents () |> parse_jsonl |> List.find ~fn:(
                  fun json ->
                    match Data.Json.get_field "type" json with
                    | Some (Data.Json.String "file") -> true
                    | _ -> false
                ) |> Option.expect ~msg:"file event missing" in
                let expected = Some (Data.Json.Array (parsed.diagnostics |> diagnostics_to_list |> List.map ~fn:Syn.Diagnostic.to_json)) in
...
let test_fmt_explain_prints_syn_explanation = fun _ctx ->
  ...
  Test.assert_equal ~expected:(Syn.Error.explain Syn.Error.E0001_MalformedTypeVariable ^ "\n") ~actual:(stdout_contents ());
  Test.assert_equal ~expected:"" ~actual:(stderr_contents ());
  Ok ()
  ...
  with_tempdir "riot_fmt_workspace_scan"
    (
      fun tmpdir ->
        ...
        Fs.write "[workspace]\nmembers = [\"packages/demo\"]\n" Path.(workspace_root / Path.v "riot.toml") |> Result.expect ~msg:"write workspace manifest";
        Fs.write "[package]\nname = \"demo\"\nversion = \"0.1.0\"\n" Path.(package_root / Path.v "riot.toml") |> Result.expect ~msg:"write package manifest";
        Fs.write "let x = 1 + 2\n\nlet f x = x + 1\n" Path.(package_src / Path.v "demo.ml") |> Result.expect ~msg:"write local source";
        Fs.write "[package]\nname = \"dep\"\nversion = \"0.1.0\"\n" Path.(external_root / Path.v "riot.toml") |> Result.expect ~msg:"write external manifest";
        Fs.write "let x as = 5\n" Path.(external_tests / Path.v "broken.ml") |> Result.expect ~msg:"write external broken file";
        let workspace = Riot_model.Workspace.make ~root:workspace_root ~packages:[ parse_package_manifest ~path:package_root ~relative_path:(Path.v "packages/demo") "[package]\nname = \"demo\"\nversion = \"0.1.0\"\n"; parse_package_manifest ~path:external_root ~relative_path:external_root "[package]\nname = \"dep\"\nversion = \"0.1.0\"\n" ] () in
