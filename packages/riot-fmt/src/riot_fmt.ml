...
let command = let open ArgParser.Arg in
command "fmt" |> about "Format OCaml with krasny" |> args
  [
...
let workspace_roots = fun workspace -> workspace.Workspace.packages |> List.filter ~fn:Package_manifest.is_workspace_member |> List.map ~fn:(Workspace.package_root workspace)

type package_scope = { package_root: Path.t; config: Fmt_config.t }
...
let load_fmt_scope = function
  | Some workspace ->
      ...
      let packages = workspace.Workspace.packages |> List.filter ~fn:Package_manifest.is_workspace_member |> List.map ~fn:(
        fun (pkg: Package_manifest.t) ->
          let package_root = Workspace.package_root workspace pkg in
          let package_toml = Path.(package_root / Path.v "riot.toml") in { package_root; config = Fmt_config.load package_toml }
      ) in Some { workspace_root = workspace.Workspace.root; workspace_config = Fmt_config.load workspace_toml; packages }
  | None ->
...
let matches_ignore_pattern = fun ~root pattern path -> String.contains (relative_or_absolute ~root path) pattern

let find_package_scope = fun scope file ->
  scope.packages |> List.filter_map ~fn:(
    fun package_scope ->
    ...
    fun (left_len, _) (right_len, _) -> Int.compare right_len left_len
  ) |> List.map ~fn:(
    fun (_, package_scope) -> package_scope
  ) |> function
...
let should_ignore_file = fun scope file ->
  ...
  | None -> false
  | Some scope ->
      let matches patterns ~root = List.any patterns ~fn:(
        fun pattern -> matches_ignore_pattern ~root pattern file
      ) in
...
let write_text_file = fun ~writer ~root file_result -> Krasny.Report.write_text_file_result ~writer ~root file_result |> Result.expect ~msg:"failed to write fmt result"

let write_json_event = fun ~writer ~root event -> Krasny.Report.write_json_event ~writer ~root event |> Result.expect ~msg:"failed to write fmt JSON event"

let write_json_start = fun ~writer ~root ~mode ~concurrency -> write_json_event ~writer ~root (Krasny.Report.Start { mode; concurrency })

let write_json_file = fun ~writer ~root file_result -> write_json_event ~writer ~root (Krasny.Report.File file_result)

let write_json_summary = fun ~writer ~root (summary: Krasny.Runner.summary) -> write_json_event ~writer ~root (Krasny.Report.Summary summary)

let write_text_summary = fun ~writer ~mode (summary: Krasny.Runner.summary) -> Krasny.Report.write_text_summary ~writer ~mode summary |> Result.expect ~msg:"failed to write fmt summary"
...
let format_failed_file = fun (file_result: Krasny.Runner.file_result) ->
  ...
  | Ok source ->
      ...
      else Syn.DiagnosticReporter.format ~file:(Path.to_string file_result.file) ~source diagnostics

let write_failed_file = fun ~writer file_result ->
  let buffer = IO.Buffer.from_string (format_failed_file file_result) in IO.write_all writer ~from:buffer |> Result.expect ~msg:"failed to write fmt diagnostics"

let write_silent_failures = fun ~writer (result: Krasny.Runner.run_result) -> result.files |> List.filter ~fn:(
  fun (file_result: Krasny.Runner.file_result) -> file_result.status = Failed
...
let explicit_targets = fun matches -> ArgParser.get_many matches "path" |> List.map ~fn:Path.v |> List.sort ~compare:compare_paths |> List.unique ~compare:compare_paths
...
let run_mode = fun ?workspace ?(stdout = default_stdout) ?(stderr = default_stderr) ?(on_event = no_event) ~mode ~output_mode ~explicit_targets () ->
  ...
  let result: Krasny.Runner.run_result =
    if List.is_empty explicit_targets then
      match mode with
      | Krasny.Runner.Check -> Krasny.Runner.run_checks_streaming ~concurrency ~should_ignore:(should_ignore_file fmt_scope) ~roots:(resolve_search_roots workspace) ~on_result ()
      | Krasny.Runner.Verify -> Krasny.Runner.run_verify_streaming ~concurrency ~should_ignore:(should_ignore_file fmt_scope) ~roots:(resolve_search_roots workspace) ~on_result ()
      | Krasny.Runner.Format -> Krasny.Runner.run_format_streaming ~concurrency ~should_ignore:(should_ignore_file fmt_scope) ~roots:(resolve_search_roots workspace) ~on_result ()
    else
      match mode with
      | Krasny.Runner.Check -> Krasny.Runner.run_checks_streaming ~concurrency ~should_ignore:(should_ignore_file fmt_scope) ~roots:explicit_targets ~on_result ()
      | Krasny.Runner.Verify -> Krasny.Runner.run_verify_streaming ~concurrency ~should_ignore:(should_ignore_file fmt_scope) ~roots:explicit_targets ~on_result ()
      | Krasny.Runner.Format -> Krasny.Runner.run_format_streaming ~concurrency ~should_ignore:(should_ignore_file fmt_scope) ~roots:explicit_targets ~on_result ()
  in
  ...
  | Krasny.Runner.Check ->
      if result.summary.needs_formatting = 0 && result.summary.failed_files = 0 then
        Ok ()
      else Error (Failure "Formatting check failed")
  | Krasny.Runner.Verify ->
      if result.summary.unsafe_to_format = 0 && result.summary.failed_files = 0 then
        Ok ()
      else Error (Failure "Formatting verification failed")
  | Krasny.Runner.Format ->
      if result.summary.failed_files = 0 then
        Ok ()
      else Error (Failure "Formatting failed")

let run_check_paths = fun ?workspace ?(on_event = no_event) paths -> run_mode ?workspace ~on_event ~mode:Krasny.Runner.Check ~output_mode:Silent ~explicit_targets:(paths |> List.sort ~compare:compare_paths |> List.unique ~compare:compare_paths) ()
  ...
  | _ ->
      let mode =
        if check then
          ...
          if verify then
            Krasny.Runner.Verify
          else Krasny.Runner.Format
      in
      let output_mode =
        if get_flag fmt_matches "json" then
          Json
        else
          if mode = Krasny.Runner.Format then
            Silent
          else
            if mode = Krasny.Runner.Check then
              QuietCheck
            else Text
      in
      run_mode ?workspace ?stdout ?stderr ~mode ~output_mode ~explicit_targets:(explicit_targets fmt_matches) ()
