...
6. Keep no-flag `riot fmt` as the in-place rewrite path. With explicit positional paths, format only those files/directories; without them, keep the existing workspace-scan behavior.
