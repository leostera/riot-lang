...
let make_fact ~entity ~attribute ~value =
  ...
  let source_uri = Uri.of_string "bench:compare" in
...
let bench_inmemory_get_from_1k () =
  let graph = Poneglyph.create ~config:InMemory () in
...
let bench_inmemory_get_from_10k () =
  let graph = Poneglyph.create ~config:InMemory () in
...
let bench_inmemory_find_entities_1k () =
  let graph = Poneglyph.create ~config:InMemory () in
...
let bench_inmemory_get_current_facts_100 () =
  let graph = Poneglyph.create ~config:InMemory () in
...
let benchmarks =
  Bench.[
    (* State benchmarks - compare write performance *)
    compare "state 100 facts"
      [
        make_case "InMemory" bench_inmemory_state_100;
        make_case "LSM" bench_lsm_state_100;
      ];
    
    compare_with_config ~config:{ iterations = 20; warmup = 2 } "state 1k facts"
      [
        make_case "InMemory" bench_inmemory_state_1k;
        make_case "LSM" bench_lsm_state_1k;
      ];
    
    compare_with_config ~config:{ iterations = 5; warmup = 1 } "state 10k facts"
      [
        make_case "InMemory" bench_inmemory_state_10k;
        make_case "LSM" bench_lsm_state_10k;
      ];
    
    (* Get benchmarks - compare point lookup performance *)
    compare_with_config ~config:{ iterations = 20; warmup = 2 } "get from 1k facts"
      [
        make_case "InMemory" bench_inmemory_get_from_1k;
        make_case "LSM" bench_lsm_get_from_1k;
      ];
    
    compare_with_config ~config:{ iterations = 10; warmup = 2 } "get from 10k facts"
      [
        make_case "InMemory" bench_inmemory_get_from_10k;
        make_case "LSM" bench_lsm_get_from_10k;
      ];
    
    (* Query benchmarks - compare scan performance *)
    compare_with_config ~config:{ iterations = 15; warmup = 2 } "find_entities: 1k facts"
      [
        make_case "InMemory" bench_inmemory_find_entities_1k;
        make_case "LSM" bench_lsm_find_entities_1k;
      ];
    
    compare_with_config ~config:{ iterations = 20; warmup = 2 } "get_current_facts: 100 attrs"
      [
        make_case "InMemory" bench_inmemory_get_current_facts_100;
        make_case "LSM" bench_lsm_get_current_facts_100;
      ];
  ]
    ...
    ~main:(fun ~args:_ ->
      let config =
        Bench.Runner.{
          ...
          suite_info = { name = "Backend Comparison: InMemory vs LSM" };
        }
      in
