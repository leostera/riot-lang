...
Analysis of **31,500+ lines of LLM coding sessions** across **14 sessions** (3 projects + 11 OpenCode sessions totaling 3,339 messages, 13,848 conversation parts) revealed that LLMs spend massive amounts of time on **build archaeology**:

- **4,927 total bash commands** (3,189 + 1,738)
- **99.4% start with `cd`** to workspace root - constant working directory confusion!
- **2,224 grep/ripgrep searches** (1,273 + 951) - 45% of all bash commands
- **1,389 git operations** (411 + 978) - 28% of all bash commands  
- **~354 ls commands** checking for artifacts
- **~130 find commands** searching for files
- **341 "build | tail -N" patterns** checking incremental build output
- **87 complex multi-pipe commands** (grep | awk | sed | sort) extracting structured data from logs

**All to extract information the build system already knows!**
...
**Estimated time savings: 30-50% on debugging/archaeology tasks** (assuming effective adoption)
...
### Session Coverage (14 Total Sessions)

**Build system development (riot-ml/riot):**
- 22 sessions, 30,423 lines (original analysis)
- 11 OpenCode sessions, 3,339 messages, 13,848 parts
  - Largest: 1,732 messages (806 bash, 290 read, 449 edit)
  - Average: 303 messages per session

**Codebase study:**
- **ocaml-dune**: 1 session, 204 entries
- **ocaml-data-insights**: 2 sessions, 869 entries

### Tool Usage Aggregate (All 14 Sessions)

```
Total tool usage:
- 4,927 Bash commands (3,189 + 1,738)
- 2,670 Read operations (2,106 + 564)
- 3,270 Edit operations (2,536 + 734)
- 1,345 Grep tool searches (1,273 + 72)
- 78 Write operations
- 735 TodoWrite operations

Archaeology breakdown:
- 2,224 grep/ripgrep in bash (45% of bash commands!)
- 1,389 git operations (28% of bash commands)
- ~568 find/ls operations (11% of bash commands)

Total archaeology operations: ~6,500+ across all sessions
```

### A.0 NEW: The Working Directory Problem (99.4% of bash commands!)

#### Current Approach:
```bash
# Nearly EVERY command starts with cd (observed 1,695/1,738 times):
cd /Users/ostera/Developer/github.com/riot-ml/riot && ./minitusk build 2>&1 | tail -3
cd /Users/ostera/Developer/github.com/riot-ml/riot && grep "handle_edges" logs/tusk.0
cd /Users/ostera/Developer/github.com/riot-ml/riot && rg "Kernel\." packages/
```

**Why this happens:**
- LLMs cannot maintain working directory state across commands
- Defensive strategy: always `cd` to workspace root before every command
- Shows fundamental context management problem

#### Poneglyph Approach:
```datalog
# Workspace context available in queries, but cd pattern may persist:
?- edge(Workspace, "root", Root),
   edge(File, "absolute_path", Path).

# LLMs may still use cd out of habit, but at least queries 
# return absolute paths and workspace-relative context
```

**Potential benefit:** Could reduce defensive `cd` usage if tooling provides absolute paths consistently
...
### A.17 NEW: The "Incremental Build Check" Pattern (341 occurrences!)

#### Current Approach:
```bash
# The most common debugging pattern - check last N lines of build output:
./minitusk build 2>&1 | tail -3      # 60 times - "quick peek"
./minitusk build 2>&1 | tail -5      # 81 times - "standard check"
./minitusk build 2>&1 | tail -10     # 45 times - "detailed check"
./minitusk build 2>&1 | tail -20     # 37 times - "deep inspection"
./minitusk build 2>&1 | tail -100    # 14 times - "full error context"

# Combined with grep for errors:
./minitusk build 2>&1 | grep -B2 "^Error" | tail -5
./minitusk build 2>&1 | grep -E '(Finished|Error:|Fatal)' | tail -3
```

**Pattern breakdown:**
- `tail -3` or `tail -5`: Quick success/failure check (141 times)
- `tail -10` to `tail -20`: Understanding what failed (82 times)
- `tail -50` to `tail -100`: Full error debugging (27 times)

#### Poneglyph Approach:
```datalog
# Structured build events streamed to client:
?- edge(Build, "package", Pkg),
   edge(Build, "status", Status),
   edge(Build, "last_output", Lines),
   limit(Lines, 5).

# Or subscribe to build events:
?- subscribe_build_events(Pkg, ["error", "warning", "completion"]).
```

**Time saved:** Instant structured feedback vs parsing unstructured logs

---

### A.18 NEW: The "Log Archaeology Pipeline" Pattern (87 complex pipelines!)

#### Current Approach:
```bash
# Complex multi-stage pipelines to extract structured data:
grep "OCAMLDEP.*Running for" logs/tusk.0 | awk '{print $6}' | sed 's/:$//' | sort > /tmp/ocamldep_files.txt
tail -100 logs/tusk.0 | grep -A 12 "cmx_files BEFORE"
grep "handle_edges called" logs/tusk.0 | sort | uniq
cat logs/tusk.0 | grep "Compiling" | grep -E "(kernel|std)" | tail -20
```

**Common pipeline stages:**
1. `grep` - Extract relevant lines from logs
2. `awk` - Parse fields from log lines  
3. `sed` - Clean up formatting
4. `sort` - Order results
5. `head/tail` - Limit output

**All this data is already known by the build system!**

#### Poneglyph Approach:
```datalog
# Direct queries to structured build data:
?- edge(File, "ocamldep_run", true),
   edge(File, "handle_edges_called", Status),
   Status = false.

# Returns files where ocamldep ran but handle_edges didn't - no pipeline needed!
```

**Time saved:** ~10 minutes per pipeline → ~30 seconds query

---

### A.19 NEW: The "Git Archaeology" Pattern (978 git operations!)

#### Current Approach:
```bash
# Constant git status → diff → log cycle trying to understand what broke:
git status packages/
git diff packages/ --stat
git log --oneline -5
git diff --cached --stat
git status --short packages/
git log --oneline -15

# Sequences observed 139 times across sessions
```

**Common workflow:**
1. Check status (what changed?)
2. Check diff (how did it change?)
3. Check log (why did it change?)
4. Repeat to understand build breakage

#### Poneglyph Approach:
```datalog
# Integrated git + build state:
?- edge(Build, "failed"),
   edge(Build, "timestamp", T),
   edge(File, "modified_since", T),
   edge(File, "in_commit", Commit),
   edge(Commit, "message", Msg).

# Returns: Files changed since last successful build with commit context
```

**Time saved:** 978 git operations → handful of integrated queries
...
**Total tool usage (14 sessions):**
- **4,927 Bash commands**
  - Grep/ripgrep: 2,224 (45%)
  - Git operations: 1,389 (28%)
  - Find/ls: ~568 (11%)
  - Build commands: ~1,200 (24%)
  - **99.4% start with `cd`** to workspace root!
- **2,670 Read operations** (many to understand types/mechanisms)
- **1,345 Grep tool searches** (pattern hunting)
- **~6,500 total archaeology operations**
...
- Average 350 bash commands per session

**With Poneglyph (projected - optimistic estimate):**
- 2-5 Datalog queries could replace some bash archaeology
- Potential 50-70% reduction in archaeology operations
- 30-50% time savings on debugging/comprehension tasks
- **Estimated 30-50 hours saved across 14 sessions** (2-3.5 hours per session)
- Assumes LLMs learn to use graph queries effectively (big assumption!)

### Key Findings by Project Type

| Project Type | Main Pattern | Query Count | Top Need |
|--------------|--------------|-------------|----------|
| **Build System** (Riot - 33 sessions) | Build archaeology | 4,840 bash | "Why did build fail?" |
| **Codebase Study** (Dune) | Mechanism understanding | 23 greps | "How does X work?" |
| **Mixed** (Data Insights) | Structure + security | 64 find/ls/grep | "What exists + is it safe?" |
