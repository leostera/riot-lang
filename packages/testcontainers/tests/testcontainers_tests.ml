...
let marker = "riot-testcontainers-ready"
...
let assert_busybox_container = fun container ->
...
let test_busybox_container_lifecycle = fun _ctx ->
  Testcontainers.with_container busybox_image assert_busybox_container
  |> Result.map_err ~fn:Testcontainers.error_to_string

let test_suite_container_lifecycle = fun ctx ->
  match Test.Context.get ctx Testcontainers.current_container with
  | None -> Error "expected suite setup to expose the current container"
  | Some container ->
      assert_busybox_container container
      |> Result.map_err ~fn:Testcontainers.error_to_string
...
let tests =
  Test.[
    ...
    live_case "suite container lifecycle" test_suite_container_lifecycle;
  ]

let main ~args =
  let (setup, teardown) =
    if Testcontainers.docker_available () then
      (Some (Testcontainers.setup busybox_image), Some Testcontainers.teardown)
    else
      (None, None)
  in
  Test.Cli.main
