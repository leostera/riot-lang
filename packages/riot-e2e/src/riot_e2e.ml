...
let run_binary = fun ?cwd ?(env = []) binary_path args ->
  let cwd = Option.map cwd ~fn:Path.to_string in Command.make (Path.to_string binary_path) ?cwd ~env ~args |> Command.output |> Result.map_err ~fn:command_error_message

let run_riot = fun ctx ?cwd ?(env = []) args ->
  let* riot_binary_path = Test.Context.require_binary ctx "riot" in run_binary ?cwd ~env riot_binary_path args

let expect_success = fun ~cmd (output: command_output) ->
  if Int.equal output.status 0 then
    Ok output
  else Error (cmd ^ " failed: " ^ render_output output)
...
let expect_failure_contains = fun ~cmd ~needle (output: command_output) ->
  ...
  if Int.equal output.status 0 then
    ...
    if String.contains text needle then
      Ok output
    else Error (cmd ^ " failed without expected text `" ^ needle ^ "`: " ^ render_output output)
...
let assert_contains = fun path needle ->
  let* content = Fs.read_to_string path |> Result.map_err ~fn:IO.error_message
  in
...
let assert_executable = fun path ->
  ...
  if Fs.Permissions.user_execute permissions then
    Ok ()
  else Error ("expected path to be executable: " ^ Path.to_string path)

let assert_output_contains = fun ~cmd (output: command_output) needle ->
  let text = output.stdout ^ output.stderr in
  if String.contains text needle then
    Ok ()
  else Error (cmd ^ " output did not contain `" ^ needle ^ "`: " ^ render_output output)
  ...
  with_tempdir_result ~prefix:"riot_e2e_init_"
    (
      fun root ->
        ...
        let* init_output = run_riot ctx ~cwd:root ("init" :: workspace_name :: init_args)
        in
        let* _ = expect_success ~cmd:"riot init" init_output in fn workspace_root
    )
