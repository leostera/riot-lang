    ...
    (fun ctx ->
      ...
      with_initialized_workspace
        ...
        (fun workspace_root ->
          ...
          let* () = write_vendor_package workspace_root "widgets" in let* () =
            write_vendor_package workspace_root "gadgets" in let* add_output =
            run_riot
              ctx
              ~cwd:workspace_root
              [
                ...
                "../../vendor/widgets";
                "../../vendor/gadgets";
              ] in let* add_output = expect_success ~cmd:"riot add -p package-cmd-e2e" add_output in let* () =
            assert_output_contains
              ~cmd:"riot add -p package-cmd-e2e"
              add_output
              "riot.pm.manifest.updated" in let* () =
            assert_contains starter_manifest {|widgets = { path = "../../vendor/widgets" }|} in let* () =
            assert_contains starter_manifest {|gadgets = { path = "../../vendor/gadgets" }|} in let* () =
            assert_contains lockfile {|name = "widgets"|} in let* () =
            assert_contains lockfile {|name = "gadgets"|} in let* update_output =
            run_riot ctx ~cwd:workspace_root [ "update"; "--json"; "widgets"; "gadgets"; ] in let* _ =
            expect_success ~cmd:"riot update widgets gadgets" update_output in let* remove_output =
            run_riot
              ctx
              ~cwd:workspace_root
              [ "rm"; "-p"; workspace_name; "--json"; "widgets"; "gadgets"; ] in let* remove_output =
            expect_success ~cmd:"riot rm -p package-cmd-e2e" remove_output in let* () =
            assert_output_contains
              ~cmd:"riot rm -p package-cmd-e2e"
              remove_output
              "riot.pm.manifest.updated" in let* () = assert_not_contains starter_manifest "widgets" in let* () =
            assert_not_contains starter_manifest "gadgets" in let* () =
            assert_not_contains lockfile {|name = "widgets"|} in
