    ...
    (fun ctx ->
      ...
      with_initialized_workspace ctx workspace_name
        (fun workspace_root ->
          ...
          let* () = assert_output_contains ~cmd:"riot add -p package-cmd-e2e" add_output "riot.pm.manifest.updated" in
          ...
          let* update_output = run_riot
            ctx
            ~cwd:workspace_root
            [ "update"; "--json"; "widgets"; "gadgets" ] in
          let* _ = expect_success ~cmd:"riot update widgets gadgets" update_output in
          let* remove_output = run_riot
            ctx
            ~cwd:workspace_root
            [ "rm"; "-p"; workspace_name; "--json"; "widgets"; "gadgets" ] in
          let* remove_output = expect_success ~cmd:"riot rm -p package-cmd-e2e" remove_output in
          let* () = assert_output_contains ~cmd:"riot rm -p package-cmd-e2e" remove_output "riot.pm.manifest.updated" in
