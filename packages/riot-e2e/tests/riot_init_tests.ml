    ...
    (fun ctx ->
      ...
      with_initialized_workspace ~init_args:[ "--bin" ] ctx workspace_name
        (fun workspace_root ->
          ...
          let* () = assert_contains Path.(workspace_root / Path.v "config" / Path.v "dev.toml") {|name = "operational-defaults"|} in
          ...
          let* () = assert_contains Path.(skill_root / Path.v "references" / Path.v "commands.md") "riot test -p my-package -f parser" in
          let* () = assert_contains Path.(skill_root / Path.v "references" / Path.v "commands.md") "riot bench --iterations 100 --warmup 10" in
          let* () = assert_contains Path.(skill_root / Path.v "references" / Path.v "testing.md") "Writing e2e tests" in
          let* () = assert_contains
            Path.(skill_root / Path.v "references" / Path.v "benchmarking.md")
            "riot bench -p my-package -f lookup --compare 5" in
          let* () = assert_contains Path.(skill_root / Path.v "references" / Path.v "modules.md") "Riot does not expose transitive dependencies" in
          let* () = assert_contains Path.(skill_root / Path.v "references" / Path.v "modules.md") "Tests inherit normal package dependencies" in
          let* () = assert_contains Path.(package_root / Path.v "src" / Path.v "main.ml") "Std.Config.load ();" in
          let* () = assert_contains Path.(package_root / Path.v "src" / Path.v "main.ml") "Std.Log.start_link ()" in
    ...
    (fun ctx ->
      ...
      with_initialized_workspace ~init_args:[ "--bin" ] ctx workspace_name
        (fun workspace_root ->
          ...
          let* () = assert_output_contains ~cmd:"riot run --list --json" run_output {|"type":"RunList"|} in
          let* () = assert_output_contains ~cmd:"riot run --list --json" run_output {|"selector":"run-list-e2e:run-list-e2e"|} in
          assert_output_contains ~cmd:"riot run --list --json" run_output {|"path":"packages/run-list-e2e/src/main.ml"|}))
...
let test_riot_test_list_json_filters_generated_starter_test =
  Test.case ~size:Test.Large "riot test --list --json filters generated starter test"
    (fun ctx ->
      ...
      with_initialized_workspace ctx workspace_name
        (fun workspace_root ->
          let* test_output = run_riot
            ctx
            ~cwd:workspace_root
            [ "test"; "--list"; "--json"; "-p"; workspace_name; "-f"; "starter" ] in
          let* test_output = expect_success ~cmd:"riot test --list --json" test_output in
          let* () = assert_output_contains ~cmd:"riot test --list --json" test_output {|"type":"TestCaseListed"|} in
          let* () = assert_output_contains ~cmd:"riot test --list --json" test_output {|"name":"starter greeting"|} in
          let* () = assert_output_contains ~cmd:"riot test --list --json" test_output {|"type":"TestListCompleted"|} in
...
let test_riot_bench_list_json_succeeds_without_benchmarks =
  Test.case ~size:Test.Large "riot bench --list --json succeeds without benchmarks"
    (fun ctx ->
      ...
      with_initialized_workspace ctx workspace_name
        (fun workspace_root ->
          let* bench_output = run_riot
            ctx
            ~cwd:workspace_root
            [ "bench"; "--list"; "--json"; "-p"; workspace_name ] in
          let* bench_output = expect_success ~cmd:"riot bench --list --json" bench_output in
          let* () = assert_output_contains ~cmd:"riot bench --list --json" bench_output {|"type":"BenchListCompleted"|} in
          assert_output_contains ~cmd:"riot bench --list --json" bench_output {|"benchmark_count":0|}))
