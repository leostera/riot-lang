    ...
    (fun ctx ->
      ...
      with_initialized_workspace
        ...
        (fun workspace_root ->
          ...
          let* new_output =
            run_riot ctx ~cwd:workspace_root [ "new"; "--lib"; "./packages/extra-library" ] in let* _ =
            ...
            assert_contains
              ...
              {|  "packages/extra-library",|} in let* build_output =
            run_riot ctx ~cwd:workspace_root [ "build"; "-p"; package_name ] in let* _ =
            expect_success ~cmd:"riot build -p extra-library" build_output in Ok ()))
