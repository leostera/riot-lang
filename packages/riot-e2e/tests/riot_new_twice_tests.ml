    ...
    (fun ctx ->
      ...
      with_initialized_workspace ctx workspace_name
        (fun workspace_root ->
          ...
          let* new_library_output = run_riot
            ctx
            ~cwd:workspace_root
            [ "new"; "--lib"; "./packages/extra-library" ] in
          let* _ = expect_success ~cmd:"riot new --lib" new_library_output in
          let* new_binary_output = run_riot
            ctx
            ~cwd:workspace_root
          ...
          let* () = assert_contains Path.(workspace_root / Path.v "riot.toml") {|  "packages/extra-library",|} in
          let* () = assert_contains Path.(workspace_root / Path.v "riot.toml") {|  "packages/extra-binary",|} in
          let* build_library_output = run_riot
            ctx
            ~cwd:workspace_root
            [ "build"; "-p"; library_package_name ] in
          let* _ = expect_success ~cmd:"riot build -p extra-library" build_library_output in
          let* build_binary_output = run_riot
            ctx
            ~cwd:workspace_root
            [ "build"; "-p"; binary_package_name ] in
