...
let rec parse_line line =
  ...
  if line = "" || (String.length line > 0 && String.get line 0 = '#') then None
    ...
    let value_str = String.trim (String.sub line (idx + 1) (String.length line - idx - 1)) in
...
and parse_value value_str =
  ...
  if String.length value_str >= 2 && String.get value_str 0 = '"' && String.get value_str (String.length value_str - 1) = '"' then
    String (String.sub value_str 1 (String.length value_str - 2))
  else if String.length value_str >= 2 && String.get value_str 0 = '[' && String.get value_str (String.length value_str - 1) = ']' then
    let content = String.sub value_str 1 (String.length value_str - 2) in
    let items = String.split_on_char ',' content 
      |> List.map (fun s -> parse_value (String.trim s))
      |> List.filter (function String "" -> false | _ -> true) in
    Array items
...
let parse_section_header line =
  let line = String.trim line in
  if String.length line >= 2 && String.get line 0 = '[' && String.get line (String.length line - 1) = ']' then
    Some (String.sub line 1 (String.length line - 2))
  else None
  ...
  try
    ...
    List.iter (fun line ->
      ...
      | Some section_name ->
          ...
          | Some name -> sections := { name; items = List.rev !current_items } :: !sections
          | None -> ());
          (* Start new section *)
      ...
      | None ->
          ...
          | Some (key, value) -> current_items := (key, value) :: !current_items
          | None -> ()
    ) lines;
    ...
    let items = List.fold_left (fun acc section ->
      List.fold_left (fun acc (key, value) ->
        let full_key = if section.name = "" then key else section.name ^ "." ^ key in
        (full_key, value) :: acc
      ) acc section.items
    ) [] all_sections in
...
let rec find_value key = function
  | Table items -> 
      (try Some (List.assoc key items) with Not_found -> None)
  | _ -> None
