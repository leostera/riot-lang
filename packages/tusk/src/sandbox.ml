...
let get_dependency_includes sandbox =
  List.fold_left (fun acc dep ->
    let dep_target = Filename.concat (Filename.concat (Filename.concat (Filename.concat sandbox.root "target") "debug") "out") dep.Build_node.package.relative_path in
...
let rec get_transitive_dependencies node visited =
  if List.mem node.Build_node.package.name visited then
    ...
    let transitive_deps = List.concat (List.map (fun dep -> 
      get_transitive_dependencies dep visited
    ) direct_deps) in
...
let copy_dependency_artifacts sandbox =
  ...
  List.iter (fun dep ->
    ...
    let dep_target_dir = Filename.concat (Filename.concat (Filename.concat (Filename.concat sandbox.root "target") "debug") "out") dep.Build_node.package.relative_path in
    
    if System.file_exists dep_target_dir then begin
      Printf.printf "[Sandbox] Copying artifacts from %s\n" dep_name;
      flush stdout;
      
      (* List files in dependency target directory *)
      let files = System.list_dir dep_target_dir (fun file ->
      ...
      List.iter (fun file ->
        ...
        if not (System.file_exists dst) then begin
      ...
      Printf.printf "[Sandbox] Warning: Dependency %s has no artifacts in %s\n" dep_name dep_target_dir;
      flush stdout
    end
  ) all_deps

(** Run a list of actions in the sandbox *)
let run_actions ~sandbox ~blueprint =
  Printf.printf "[Sandbox] Running %d actions for %s in %s\n" 
    (List.length blueprint.Actions.actions) sandbox.node.Build_node.package.name sandbox.sandbox_dir;
  flush stdout;
  ...
  let result = 
    try
      ...
      List.iteri (fun i action ->
        Printf.printf "[Sandbox] Step %d: %s\n" (i + 1) (Actions.string_of_action action);
        flush stdout;
    ...
    with
    | exn ->
        let error_msg = Printf.sprintf "Sandbox execution failed: %s" (Printexc.to_string exn) in
        (false, error_msg)
  in
  ...
  if fst result then begin
    let copy_artifacts () =
      try
        (* Copy only declared outputs *)
        if !declared_outputs <> [] then begin
          ...
          List.iter (fun output_file ->
            ...
            if System.file_exists src then begin
              let dst = Filename.concat sandbox.target_dir output_file in
              System.copy_file src dst;
              (* Make executable files executable *)
              if not (String.contains output_file '.') then begin
                System.chmod dst 0o755;
                (* Also promote executable to target/<profile>/<name> *)
                let profile_dir = Filename.concat (Filename.concat sandbox.root "target") "debug" in
              ...
              Printf.printf "[Sandbox] Warning: Declared output %s not found in sandbox\n" output_file;
              flush stdout
            end
          ) !declared_outputs
        end else begin
          (* Fallback: if no outputs declared, use heuristics *)
          Printf.printf "[Sandbox] Warning: No outputs declared, using fallback heuristics\n";
          flush stdout;
          let pkg_name = sandbox.node.Build_node.package.name in
          let files = System.list_dir sandbox.sandbox_dir (fun file ->
            (* Main library archive *)
            (file = pkg_name ^ ".cma") ||
            (* Main module interface *)
            (file = pkg_name ^ ".cmi") ||
          ...
          List.iter (fun file ->
            ...
            System.copy_file src dst;
            (* Make executable files executable *)
            if not (String.contains file '.') then begin
              System.chmod dst 0o755;
              (* Also promote executable to target/<profile>/<name> *)
              let profile_dir = Filename.concat (Filename.concat sandbox.root "target") "debug" in
...
let cleanup sandbox =
  System.remove_dir sandbox.sandbox_dir