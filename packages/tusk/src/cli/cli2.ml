...
let cli =
  ...
  command "tusk" ~version:(Some "0.1.0")
  ...
  |> subcommand
       (command "build" ~about:(Some "Build packages") ()
       |> arg
            Arg.(
              option "package" |> short 'p' |> long "package"
              |> help "Build only the specified package"))
  |> subcommand
       (command "run" ~about:(Some "Run a binary") ()
       |> arg
            Arg.(
              option "binary" |> short 'b' |> long "binary"
              |> help "Specify which binary to run"))
  |> subcommand (command "clean" ~about:(Some "Clean build artifacts") ())
  |> subcommand
       (command "new" ~about:(Some "Create a new package") ()
       |> arg Arg.(positional "path" |> help "Path for new package"))
  |> subcommand
       (command "install" ~about:(Some "Install a binary to ~/.tusk/bin") ()
       |> arg
            Arg.(
              option "binary" |> short 'b' |> long "binary"
              |> help "Binary to install"))
  |> subcommand
       (command "server" ~about:(Some "Start or manage the tusk server") ()
       |> arg
            Arg.(
              option "action" |> help "Action: start, stop, kill, or status"
              |> possible_values [ "start"; "stop"; "kill"; "status" ]))
  |> subcommand (command "rpc" ~about:(Some "Send RPC command to server") ())
  |> subcommand
       (command "mcp" ~about:(Some "Start Model Context Protocol server") ())
  |> subcommand (command "version" ~about:(Some "Show tusk version") ())
  |> subcommand (command "help" ~about:(Some "Show help message") ())
