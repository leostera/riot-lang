...
let create ~node:(build_node : Build_node.t) ~workspace =
  ...
  Printf.printf "[MODULE_GRAPH] Creating module graph for %s (namespace: %s)\n%!"
    package.name (Namespace.to_string namespace);
...
and handle_library ~t ~ctx dir name children src_dir =
  ...
  let lib_aliases = if Namespace.is_empty ns then [ aliases_node ] else aliases @ [ aliases_node ] in
