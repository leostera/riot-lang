...
and can_build_package state pkg_name =
  ...
  | Some graph ->
      ...
      match List.find_opt (fun n -> n.Build_graph.package.name = pkg_name) nodes with
      | None -> false
      | Some node ->
          let deps = List.map (fun dep -> dep.Build_graph.package.name) node.dependencies in
...
let handle_task_complete state pkg_name success =
  if success then begin
    ...
    | Some graph ->
        let nodes = Build_graph.topological_sort graph in
        List.iter (fun node ->
          let pkg = node.Build_graph.package.name in
...
let rec server_loop state =
  ...
  | BuildAll cli_pid ->
      ...
      | Some graph ->
          ...
          let pkg_names = List.map (fun n -> n.Build_graph.package.name) sorted in
          Build_results.init_packages state.build_results pkg_names;
          
          (* Queue packages with no dependencies *)
          List.iter (fun node ->
            if node.Build_graph.dependencies = [] then begin
              Printf.printf "[Server] Queueing package with no deps: %s\n" node.package.name;
              Queue.add node.package.name state.current_queue
            end
          ) sorted;
          ...
          server_loop { state with workers; cli_pid = Some cli_pid })
