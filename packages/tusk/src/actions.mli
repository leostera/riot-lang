(** Build actions - Concrete build steps and blueprint generation
    ...
    This module defines the build actions that can be executed in a sandbox
    and provides blueprint generation for packages. *)
...
type action =
  ...
  | CompileC of string * string
  (** Compile C file: (source, output) *)
...
type dep_info = {
  ...
  (** Names of this dependency's dependencies *)
}
...
type blueprint = {
  ...
  actions : action list;
  (** Ordered list of build actions to execute *)
  
  toolchain : Toolchains.toolchain;
  (** OCaml toolchain to use for compilation *)
  
  hash : Hasher.hash option;
  (** Content-based hash of all inputs *)
}
...
val generate_blueprint : 
  Workspace.workspace -> Build_node.t ->
  dep_info list -> dep_info list -> 
  Toolchains.toolchain -> hash:Hasher.hash -> unit -> blueprint

(** {1 Action Execution} *)

(** Execute a single build action.
    Returns (result, output_message). *)
val execute_action : action -> Toolchains.toolchain -> action_result * string

(** Execute all actions in a blueprint.
    Returns (success, message) where success is true if all actions succeeded. *)
...
(** Compute content-based hash for a blueprint.
    Takes into account package metadata, dependencies, source files, and actions. *)
