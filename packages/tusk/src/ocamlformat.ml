  ...
  let rec search dir =
    ...
    | Ok ocamlformat_name ->
        ...
        if File.exists ~path:(Path.to_string ocamlformat_file) then
          ...
          | None -> None
          | Some parent ->
              if Path.equal parent dir then None
              else search parent
  in
  search (if File.is_directory ~path:(Path.to_string path) then path else 
...
let format_file ~toolchain ~file_path ~check_only =
  ...
  else if not (String.ends_with ~suffix:".ml" file_str || String.ends_with ~suffix:".mli" file_str) then
    Error (Printf.sprintf "Not an OCaml file: %s" file_str)
  else
    let check_flag = if check_only then "--check" else "--inplace" in
    let ocamlformat_bin = Toolchains.ocamlformat_path toolchain in
...
let format_code ~toolchain ~code ~file_path =
  ...
  let temp_file = Printf.sprintf "/tmp/tusk_format_%d%s" (Unix.getpid ()) extension in
  ...
  | Ok () ->
      let result = 
        let ocamlformat_bin = Toolchains.ocamlformat_path toolchain in
        let cmd = Printf.sprintf "%s --enable-outside-detected-project %s" ocamlformat_bin temp_file in
        match Command.run_command cmd with
        | Ok output -> 
            let changed = not (String.equal (String.trim output) (String.trim code)) in
