(** RPC protocol definitions for Tusk server communication
    ...
    between the Tusk server and various clients (CLI, LSP, MCP). *)
...
type response =
  ...
  (** Build has started with given ID *)
...
(** Serialize a request to a string.
    (Temporary implementation - will use proper JSON later) *)
