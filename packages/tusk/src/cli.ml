...
let find_binaries () =
  ...
  List.filter_map (fun package ->
    ...
    if System.file_exists main_ml_path then
...
let get_lsp_binary_path () =
  ...
  if System.file_exists lsp_path then
    lsp_path
  else
...
let lsp_command () =
  try
    ...
    if not merlin_exists then
      Printf.printf "⚠️  No .merlin file found. Run 'tusk build' to generate it.\n%!";
    ...
    if merlin_exists then
      ...
      Printf.printf "   Warning: No .merlin file, LSP may have limited functionality\n%!";
    ...
    let args = if Array.length (System.argv ()) > 2 then
