...
let tests = [
  ...
  Test.case "ordered-argument-kinds flags labeled arguments after positional ones"
    (fun _ctx ->
      ...
      let pipeline = Riot_fix.Pipeline.make
        ~rules:[ Riot_fix.Rules.Ordered_argument_kinds.make () ]
        () in
      let result = Riot_fix.Pipeline.run pipeline source in
      let codes = diagnostic_rule_ids result.diagnostics in
  ...
  Test.case "no-exn-suffix-functions flags exception-style function names"
    (fun _ctx ->
      ...
      let pipeline = Riot_fix.Pipeline.make
        ~rules:[ Riot_fix.Rules.No_exn_suffix_functions.make () ]
        () in
      let result = Riot_fix.Pipeline.run pipeline source in
      let codes = diagnostic_rule_ids result.diagnostics in
...
|});
  ...
  Test.case "alphabetized-named-arguments flags unsorted labeled arguments"
    (fun _ctx ->
      ...
      let pipeline = Riot_fix.Pipeline.make
        ~rules:[ Riot_fix.Rules.Alphabetized_named_arguments.make () ]
        () in
      let result = Riot_fix.Pipeline.run pipeline source in
      let codes = diagnostic_rule_ids result.diagnostics in
  ...
  Test.case "t-first-named-arguments flags t after other positional arguments"
    (fun _ctx ->
      ...
      let pipeline = Riot_fix.Pipeline.make
        ~rules:[ Riot_fix.Rules.T_first_named_arguments.make () ]
        () in
      let result = Riot_fix.Pipeline.run pipeline source in
      let codes = diagnostic_rule_ids result.diagnostics in
  ...
  Test.case "avoid-single-letter-type-names allows t"
    (fun _ctx ->
      ...
      let pipeline = Riot_fix.Pipeline.make
        ~rules:[ Riot_fix.Rules.Avoid_single_letter_type_names.make () ]
        () in
      let result = Riot_fix.Pipeline.run pipeline source in
      Test.assert_equal ~expected:0 ~actual:(List.length result.diagnostics);
      Ok ());
  ...
  Test.case "prefer-multiline-string-literals flags chained string literals"
    (fun _ctx ->
      ...
      let pipeline = Riot_fix.Pipeline.make
        ~rules:[ Riot_fix.Rules.Prefer_multiline_string_literals.make () ]
        () in
      let result = Riot_fix.Pipeline.run pipeline source in
      let codes = diagnostic_rule_ids result.diagnostics in
  ...
  Test.case "limit-function-parameters flags five positional parameters"
    (fun _ctx ->
      ...
      let pipeline = Riot_fix.Pipeline.make
        ~rules:[ Riot_fix.Rules.Limit_function_parameters.make () ]
        () in
      let result = Riot_fix.Pipeline.run pipeline source in
      let codes = diagnostic_rule_ids result.diagnostics in
  ...
  Test.case "limit-parenthesis-depth flags deeply parenthesized expressions"
    (fun _ctx ->
      ...
      let pipeline = Riot_fix.Pipeline.make
        ~rules:[ Riot_fix.Rules.Limit_parenthesis_depth.make () ]
        () in
      let result = Riot_fix.Pipeline.run pipeline source in
      let codes = diagnostic_rule_ids result.diagnostics in
...
|}
      in
      let pipeline = Riot_fix.Pipeline.make
        ~rules:[ Riot_fix.Rules.Limit_nested_match_depth.make () ]
        () in
      let result = Riot_fix.Pipeline.run pipeline source in
      let codes = diagnostic_rule_ids result.diagnostics in
...
|}
  ...
  Test.case "prefer-t-for-single-type-modules flags modules with one non-t type"
    (fun _ctx ->
      ...
      let pipeline = Riot_fix.Pipeline.make
        ~rules:[ Riot_fix.Rules.Prefer_t_for_single_type_modules.make () ]
        () in
      let result = Riot_fix.Pipeline.run pipeline source in
      let codes = diagnostic_rule_ids result.diagnostics in
  ...
  Test.case "no-public-mutable-fields flags mutable record fields in interfaces"
    (fun _ctx ->
      ...
      let pipeline = Riot_fix.Pipeline.make
        ~rules:[ Riot_fix.Rules.No_public_mutable_fields.make () ]
        () in
  ...
  Test.case "no-positional-bool-parameters flags inline bool parameters"
    (fun _ctx ->
      ...
      let pipeline = Riot_fix.Pipeline.make
        ~rules:[ Riot_fix.Rules.No_positional_bool_parameters.make () ]
        () in
      let result = Riot_fix.Pipeline.run pipeline source in
      let codes = diagnostic_rule_ids result.diagnostics in
  ...
  Test.case "prefer-named-closed-polyvariants flags inline closed polyvariants in values"
    (fun _ctx ->
      ...
      let pipeline = Riot_fix.Pipeline.make
        ~rules:[ Riot_fix.Rules.Prefer_named_closed_polyvariants.make () ]
        () in
      let result = Riot_fix.Pipeline.run ~filename:(Path.v "sample.mli") pipeline source in
      let codes = diagnostic_rule_ids result.diagnostics in
  ...
  Test.case "prefer-opaque-record-types flags public record types with matching accessors"
    (fun _ctx ->
      ...
      let pipeline = Riot_fix.Pipeline.make
        ~rules:[ Riot_fix.Rules.Prefer_opaque_record_types.make () ]
        () in
      let result = Riot_fix.Pipeline.run ~filename:(Path.v "sample.mli") pipeline source in
      let codes = diagnostic_rule_ids result.diagnostics in
  ...
  Test.case "prefer-records-over-large-tuples flags repeated tuple aliases"
    (fun _ctx ->
      ...
      let pipeline = Riot_fix.Pipeline.make
        ~rules:[ Riot_fix.Rules.Prefer_records_over_large_tuples.make () ]
        () in
      let result = Riot_fix.Pipeline.run pipeline source in
      Test.assert_equal
    ...
    (fun _ctx ->
      with_tempdir "riot_fix_limit"
        (fun tmpdir ->
          ...
          let outcome = Riot_fix.Cli.run_result
  ...
  Test.case "pipeline parses interface files with interface entrypoint"
    (fun _ctx ->
      ...
      let result = Riot_fix.Pipeline.run
        (Riot_fix.Pipeline.default ())
  ...
  Test.case "fixme runner binary path uses workspace build dir"
    (fun _ctx ->
      ...
      in
      let plan = Riot_fix.Fixme_runner.plan
