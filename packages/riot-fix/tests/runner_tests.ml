...
let package_name = fun value -> Riot_model.Package_name.from_string value |> Result.expect ~msg:("invalid package name: " ^ value)
...
let diagnostic_rule_ids = fun diagnostics -> diagnostics |> List.map ~fn:(
  fun diag -> Riot_fix.Rule_id.to_string (Riot_fix.Diagnostic.rule_id diag)
...
let tests =
  [
    ...
    Test.case "rule explanations explain type-name violations"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"snake-case-type-names" ~snippet:"snake_case"
      );
    Test.case "rule explanations explain short type variables"
      (
    ...
    Test.case "snake-case-function-names exposes an auto-fix"
      (
        fun _ctx ->
          let source = "let userProfile x = x\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Snake_case_function_names.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"let user_profile x = x\n"
      );
    ...
    Test.case "snake-case-function-names ignores camelCase value bindings"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Snake_case_function_names.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "rule explanations explain function-name violations"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"snake-case-function-names" ~snippet:"parse_user"
      );
    ...
    Test.case "class-case-module-names exposes an auto-fix"
      (
        fun _ctx ->
          let source = "module Foo_bar = struct end\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Class_case_module_names.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"module FooBar = struct end\n"
      );
    ...
    Test.case "rule explanations explain module-name violations"
      (
    ...
    Test.case "snake-case-variable-names exposes an auto-fix"
      (
        fun _ctx ->
          let source = "let currentUser = 42\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Snake_case_variable_names.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"let current_user = 42\n"
      );
    ...
    Test.case "rule explanations explain variable-name violations"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"snake-case-variable-names" ~snippet:"current_user"
      );
    ...
    Test.case "no-prime-variables exposes an auto-fix"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_prime_variables.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"let state3 = next_state\n"
      );
    ...
    Test.case "rule explanations explain prime-variable violations"
      (
    ...
    Test.case "snake-case-argument-names exposes an auto-fix"
      (
        fun _ctx ->
          let source = "let render userId = userId\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Snake_case_argument_names.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"let render user_id = userId\n"
      );
    Test.case "snake-case-argument-names keeps compliant arguments clean"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Snake_case_argument_names.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          Test.assert_equal ~expected:0 ~actual:(List.length result.diagnostics);
          Ok ()
      );
    Test.case "rule explanations explain argument-name violations"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"snake-case-argument-names" ~snippet:"display_name"
      );
    Test.case "ordered-argument-kinds flags labeled arguments after positional ones"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Ordered_argument_kinds.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "rule explanations explain argument-order violations"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"ordered-argument-kinds" ~snippet:"labeled arguments"
      );
    ...
    Test.case "no-open-bang exposes an auto-fix"
      (
        fun _ctx ->
          let source = "open! List\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_open_bang.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"open List\n"
      );
    ...
    Test.case "rule explanations explain open! violations"
      (
    ...
    Test.case "limit-open-statements flags a third file-level open"
      (
        fun _ctx ->
          let source = "open Std\nopen Http\nopen Json\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Limit_open_statements.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "rule explanations explain open-count violations"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"limit-open-statements" ~snippet:"two open statements"
      );
    Test.case "no-exn-suffix-functions flags exception-style function names"
      (
        fun _ctx ->
          let source = "let parse_exn text = text\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_exn_suffix_functions.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "rule explanations explain _exn function names"
      (
    ...
    Test.case "rule explanations explain unnecessary rec"
      (
    ...
    Test.case "no-useless-let-return exposes an auto-fix"
      (
        fun _ctx ->
          let source = "let render x = let value = parse x in value\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_useless_let_return.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"let render x = parse x\n"
      );
    Test.case "rule explanations explain useless let returns"
      (
    ...
    Test.case "no-redundant-else-unit flags else branches that only return unit"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_redundant_else_unit.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "no-redundant-else-unit exposes an auto-fix"
      (
        fun _ctx ->
          let source = "let render ok = if ok then log () else ()\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_redundant_else_unit.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"let render ok =if ok then log ()\n"
      );
    Test.case "rule explanations explain redundant else unit branches"
      (
    ...
    Test.case "no-boolean-comparisons-in-conditionals flags equality to true"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_boolean_comparisons_in_conditionals.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "no-boolean-comparisons-in-conditionals exposes an auto-fix"
      (
        fun _ctx ->
          let source = "let render is_ready = if is_ready = false then log ()\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_boolean_comparisons_in_conditionals.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"let render is_ready = if not (is_ready) then log ()\n"
      );
    ...
    Test.case "rule explanations explain boolean conditional comparisons"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"no-boolean-comparisons-in-conditionals" ~snippet:"if is_ready then render ()"
      );
    Test.case "prefer-sequences-over-let-unit flags let-unit sequencing"
      (
        fun _ctx ->
          let source = "let render () = let () = log () in flush ()\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Prefer_sequences_over_let_unit.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "prefer-sequences-over-let-unit exposes an auto-fix"
      (
        fun _ctx ->
          let source = "let render () = let () = log () in flush ()\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Prefer_sequences_over_let_unit.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"let render () = (log ()); flush ()\n"
      );
...
|} in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Prefer_sequences_over_let_unit.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:{|
let solve total feedback_ref =
...
|}
    ...
    Test.case "rule explanations explain let-unit sequencing"
      (
    ...
    Test.case "prefer-if-over-bool-match flags full boolean matches"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Prefer_if_over_bool_match.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "prefer-if-over-bool-match exposes an auto-fix"
      (
        fun _ctx ->
          let source = "let render ready = match ready with true -> render () | false -> fallback ()\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Prefer_if_over_bool_match.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"let render ready =if ready then render () else fallback ()\n"
      );
    Test.case "prefer-if-over-bool-match preserves multiline branch source in auto-fixes"
      (
        fun _ctx ->
          let source = "let f =\n  match f with\n  | true -> do_something\n  | _ ->\n      if f then\n        print\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Prefer_if_over_bool_match.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"let f =if f then do_something else if f then\n        print\n"
      );
    Test.case "rule explanations explain boolean match rewrites"
      (
    ...
    Test.case "alphabetized-named-arguments flags unsorted labeled arguments"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Alphabetized_named_arguments.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "rule explanations explain named-argument sorting violations"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"alphabetized-named-arguments" ~snippet:"Alphabetical order"
      );
    Test.case "t-first-named-arguments flags t after other positional arguments"
      (
        fun _ctx ->
          let source = "let render ~width ~height other t = t\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.T_first_named_arguments.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "rule explanations explain t-first named argument violations"
      (
    ...
    Test.case "snake-case-record-fields flags camelCase record fields"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Snake_case_record_fields.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "snake-case-record-fields exposes an auto-fix"
      (
        fun _ctx ->
          let source = "type user = { displayName : string; created_at : int }\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Snake_case_record_fields.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"type user = { display_name : string; created_at : int }\n"
      );
    Test.case "rule explanations explain record-field violations"
      (
    ...
    Test.case "class-case-constructors flags underscored constructors"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Class_case_constructors.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "class-case-constructors exposes an auto-fix"
      (
        fun _ctx ->
          let source = "type user = | Guest_user | RegisteredUser\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Class_case_constructors.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"type user = | GuestUser | RegisteredUser\n"
      );
    Test.case "rule explanations explain constructor-name violations"
      (
    ...
    Test.case "snake-case-polyvariant-tags flags non-snake-case tags"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Snake_case_polyvariant_tags.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "snake-case-polyvariant-tags exposes an auto-fix"
      (
        fun _ctx ->
          let source = "type user = [ `GuestUser | `registered_user ]\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Snake_case_polyvariant_tags.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"type user = [ `guest_user | `registered_user ]\n"
      );
    Test.case "rule explanations explain polyvariant-tag violations"
      (
    ...
    Test.case "avoid-single-letter-function-names ignores placeholder value bindings"
      (
        fun _ctx ->
          let source = "let f = 42\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Avoid_single_letter_function_names.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          Test.assert_equal ~expected:0 ~actual:(List.length result.diagnostics);
          Ok ()
      );
    Test.case "rule explanations explain single-letter function bindings"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"avoid-single-letter-function-names" ~snippet:"Placeholder names"
      );
    ...
    Test.case "avoid-single-letter-type-names allows t"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Avoid_single_letter_type_names.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          Test.assert_equal ~expected:0 ~actual:(List.length result.diagnostics);
          Ok ()
      );
    ...
    Test.case "rule explanations explain single-letter type names"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"avoid-single-letter-type-names" ~snippet:"conventional `t`"
      );
    Test.case "prefer-multiline-string-literals flags chained string literals"
      (
        fun _ctx ->
          let source = "let banner = \"hello \" ^ \"world\" ^ \"!\"\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Prefer_multiline_string_literals.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "rule explanations explain multiline string preference"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"prefer-multiline-string-literals" ~snippet:"multiline literal"
      );
    ...
    Test.case "rule explanations explain custom operators"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"no-custom-operators" ~snippet:"hard to search"
      );
    Test.case "prefer-pipelines-for-nested-calls flags very deep call chains"
      (
        fun _ctx ->
          let source = "let rendered = foo (bar (baz (hex 1)))\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Prefer_pipelines_for_nested_calls.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "prefer-pipelines-for-nested-calls exposes an auto-fix"
      (
        fun _ctx ->
          let source = "let rendered = foo (bar (baz (hex 1)))\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Prefer_pipelines_for_nested_calls.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"let rendered =1 |> hex |> baz |> bar |> foo\n"
      );
    Test.case "rule explanations explain nested pipeline preference"
      (
    ...
    Test.case "no-inline-parameter-type-annotations flags typed positional parameters"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_inline_parameter_type_annotations.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "rule explanations explain inline parameter annotations"
      (
    ...
    Test.case "no-function-shorthand exposes an auto-fix"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_function_shorthand.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let fix = result.diagnostics |> List.filter_map ~fn:Riot_fix.Diagnostic.fix |> List.head |> Option.expect ~msg:"expected no-function-shorthand fix" in
    ...
    Test.case "rule explanations explain function shorthand"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"no-function-shorthand" ~snippet:"Explicit parameters"
      );
    Test.case "limit-function-parameters flags five positional parameters"
      (
        fun _ctx ->
          let source = "let render a b c d e = a\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Limit_function_parameters.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "rule explanations explain parameter count limits"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"limit-function-parameters" ~snippet:"record-shaped concept"
      );
    Test.case "limit-parenthesis-depth flags deeply parenthesized expressions"
      (
        fun _ctx ->
          let source = "let wrapped = (((((value)))))\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Limit_parenthesis_depth.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "rule explanations explain parenthesis depth limits"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"limit-parenthesis-depth" ~snippet:"parenthesized expressions"
      );
...
let render x y z =
  ...
  | _ ->
      ...
      | _ ->
          ...
          | _ -> 1
|} in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Limit_nested_match_depth.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
...
|} in
    ...
    Test.case "rule explanations explain nested match depth limits"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"limit-nested-match-depth" ~snippet:"match towers"
      );
    Test.case "no-redundant-parentheses flags obvious grouping around identifiers"
      (
        fun _ctx ->
          let source = "let render value = (value)\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_redundant_parentheses.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "no-redundant-parentheses exposes an auto-fix"
      (
        fun _ctx ->
          let source = "let render value = (value)\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_redundant_parentheses.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"let render value =value\n"
      );
    Test.case "rule explanations explain redundant parentheses"
      (
    ...
    Test.case "no-eta-expansion exposes an auto-fix"
      (
        fun _ctx ->
          let source = "let wrap foo = fun value -> foo value\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_eta_expansion.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"let wrap foo = foo\n"
      );
    Test.case "rule explanations explain eta expansion"
      (
    ...
    Test.case "no-redundant-reraise exposes an auto-fix"
      (
        fun _ctx ->
          let source = "let render value = try render_inner value with exn -> raise exn\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_redundant_reraise.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"let render value = render_inner value\n"
      );
    Test.case "rule explanations explain redundant reraises"
      (
    ...
    Test.case "no-redundant-begin-end flags begin/end grouping"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_redundant_begin_end.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "no-redundant-begin-end exposes an auto-fix"
      (
        fun _ctx ->
          let source = "let render value = begin value end\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_redundant_begin_end.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"let render value = value\n"
      );
    ...
    Test.case "rule explanations explain redundant begin/end"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"no-redundant-begin-end" ~snippet:"begin ... end"
      );
    Test.case "prefer-scoped-field-access flags module-qualified record access"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Prefer_scoped_field_access.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "prefer-scoped-field-access exposes an auto-fix for field access"
      (
        fun _ctx ->
          let source = "let render record = record.Module.field\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Prefer_scoped_field_access.make () ] () in assert_single_fix_rewrite ~pipeline ~source ~expected:"let render record =Module.(record.field)\n"
      );
    ...
    Test.case "rule explanations explain scoped field access"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"prefer-scoped-field-access" ~snippet:"Module.{ field = value }"
      );
    Test.case "prefer-t-for-single-type-modules flags modules with one non-t type"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Prefer_t_for_single_type_modules.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "rule explanations explain single type modules"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"prefer-t-for-single-type-modules" ~snippet:"User.t"
      );
    Test.case "no-public-mutable-fields flags mutable record fields in interfaces"
      (
        fun _ctx ->
          let source = "type t = { mutable state : int }\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_public_mutable_fields.make () ] () in
    ...
    Test.case "rule explanations explain public mutable fields"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"no-public-mutable-fields" ~snippet:"mutable field"
      );
    Test.case "no-positional-bool-parameters flags inline bool parameters"
      (
        fun _ctx ->
          let source = "let render (enabled : bool) user = user\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.No_positional_bool_parameters.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "rule explanations explain positional bool parameters"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"no-positional-bool-parameters" ~snippet:"~enabled"
      );
    Test.case "prefer-named-closed-polyvariants flags inline closed polyvariants in values"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Prefer_named_closed_polyvariants.make () ] () in
          let result = Riot_fix.Pipeline.run ~filename:(Path.v "sample.mli") pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "rule explanations explain named closed polyvariants"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"prefer-named-closed-polyvariants" ~snippet:"type format"
      );
    Test.case "prefer-opaque-record-types flags public record types with matching accessors"
      (
        fun _ctx ->
          let source = "type t = { name : string }\nval name : t -> string\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Prefer_opaque_record_types.make () ] () in
          let result = Riot_fix.Pipeline.run ~filename:(Path.v "sample.mli") pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "rule explanations explain opaque record types"
      (
    ...
    Test.case "require-module-interfaces flags src modules without sibling mli files"
      (
        fun _ctx ->
          with_tempdir "riot_fix_interfaces"
            (
              fun tmpdir ->
                ...
                Test.assert_equal ~expected:[ "require-module-interfaces" ] ~actual:(diagnostic_rule_ids result.diagnostics);
                Ok ()
            )
      );
    ...
    Test.case "rule explanations explain missing module interfaces"
      (
    ...
    Test.case "snake-case-source-paths flags non-snake-case source filenames"
      (
        fun _ctx ->
          with_tempdir "riot_fix_source_paths"
            (
              fun tmpdir ->
                let src_dir = Path.(tmpdir / Path.v "packages" / Path.v "app" / Path.v "src") in
                ...
                let result = Riot_fix.Runner.run_file ~mode:Riot_fix.Runner.Check file in
                Test.assert_equal ~expected:[ "require-module-interfaces"; "snake-case-source-paths" ] ~actual:(diagnostic_rule_ids result.diagnostics);
                Ok ()
            )
      );
      ...
      (
        fun _ctx ->
          with_tempdir "riot_fix_source_paths"
            (
              fun tmpdir ->
                let src_dir = Path.(tmpdir / Path.v "packages" / Path.v "app" / Path.v "src" / Path.v "JsonHelpers") in
                let file = Path.(src_dir / Path.v "session_store.ml") in
                Fs.create_dir_all src_dir |> Result.expect ~msg:"mkdir src";
                write_file file "let session_store = ()\n";
                let result = Riot_fix.Runner.run_file ~mode:Riot_fix.Runner.Check file in
      ...
      (
        fun _ctx ->
          with_tempdir "riot_fix_source_paths"
            (
              fun tmpdir ->
                let src_dir = Path.(tmpdir / Path.v "packages" / Path.v "app" / Path.v "src" / Path.v "json_helpers") in
                let file = Path.(src_dir / Path.v "session_store.ml") in
                let interface = Path.(src_dir / Path.v "session_store.mli") in
                Fs.create_dir_all src_dir |> Result.expect ~msg:"mkdir src";
                write_file file "let session_store = ()\n";
    ...
    Test.case "rule explanations explain snake_case source paths"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"snake-case-source-paths" ~snippet:"snake_case"
      );
    Test.case "package-name-style flags package names that do not start with a letter"
      (
        fun _ctx ->
          with_tempdir "riot_fix_package_names"
            (
              fun tmpdir ->
                ...
                write_file file "let main = ()\n";
                let result = Riot_fix.Runner.run_file ~mode:Riot_fix.Runner.Check file in
                Test.assert_equal ~expected:[ "package-name-style" ] ~actual:(diagnostic_rule_ids result.diagnostics);
                Ok ()
            )
      );
    ...
    Test.case "rule explanations explain package name style"
      (
    ...
    Test.case "prefer-records-over-large-tuples flags repeated tuple aliases"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Prefer_records_over_large_tuples.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          Test.assert_equal ~expected:[ "prefer-records-over-large-tuples" ] ~actual:(diagnostic_rule_ids result.diagnostics);
          Ok ()
      );
    ...
    Test.case "rule explanations explain large tuple aliases"
      (
        fun _ctx -> assert_explanation_contains ~rule_id:"prefer-records-over-large-tuples" ~snippet:"record"
      );
    Test.case "cli list-rules text output prints one rule per line"
      (
        fun _ctx ->
          ...
          Test.assert_true (String.contains output "  \027[1mriot:snake-case-type-names\027[0m - Type names should use snake_case instead of camelCase");
          Test.assert_true (String.contains output "  \027[1mriot:snake-case-variable-names\027[0m - Variable names should use snake_case instead of camelCase");
          Test.assert_true (String.contains output "  \027[1mriot:prefer-record-destructuring-parameters\027[0m - Functions that immediately destructure a record argument should destructure it in the parameter");
    ...
    Test.case "cli list-diagnostics text output includes builtin and package diagnostics"
      (
        fun _ctx ->
          ...
          Test.assert_true (String.contains output "\027[1mriot:snake-case-type-names\027[0m - Type names should use snake_case instead of camelCase");
          Test.assert_true (String.contains output "\027[1mriot:descriptive-type-variables\027[0m - Type variables in type definitions should use descriptive names instead of short placeholders");
          Ok ()
      );
          ...
          with_tempdir "riot_fix_limit"
            (
              fun tmpdir ->
                ...
                let outcome = Riot_fix.Cli.run_result ~mode:Riot_fix.Runner.Check ~scope:None ~limit:(Some 1) ~files:[ file1; file2; file3 ] in
    ...
    Test.case "snake-case-type-names ignores non-type camelCase identifiers"
      (
        fun _ctx ->
          let source = "let userProfile = 42\n" in
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Snake_case_type_names.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
          Test.assert_equal ~expected:[] ~actual:codes;
          Ok ()
          ...
          with_tempdir "riot_fix_rules"
            (
              fun tmpdir ->
                ...
                write_file file "type userProfile = int\n";
                write_file interface "type user_profile = int\n";
                let result = Riot_fix.Runner.run_files ~pipeline_for_file:(Riot_fix.Config.pipeline_for_file (Riot_fix.Config.load_scope ~cwd:tmpdir)) ~mode:Riot_fix.Runner.Check [ file ] in
    ...
    Test.case "workspace rule overrides keep builtins enabled by default"
      (
        fun _ctx ->
          with_tempdir "riot_fix_default_rules"
            (
              fun tmpdir ->
                let workspace_toml = Path.(tmpdir / Path.v "riot.toml") in
                let package_dir = Path.(tmpdir / Path.v "packages" / Path.v "app") in
    ...
    Test.case "config table uses explicit rule state"
      (
        fun _ctx ->
          with_tempdir "riot_fix_rule_state"
            (
              fun tmpdir ->
                let workspace_toml = Path.(tmpdir / Path.v "riot.toml") in
                let package_dir = Path.(tmpdir / Path.v "packages" / Path.v "app") in
            ...
            (
              fun tmpdir ->
                ...
                let result =
                  with_cwd tmpdir
                    (
                      fun () -> run_cli [ Path.to_string file ]
                    )
                in
                Test.assert_error result;
            ...
            (
              fun tmpdir ->
                ...
                let result =
                  with_cwd tmpdir
                    (
                      fun () -> run_cli [ "--apply"; Path.to_string file ]
                    )
                in
            ...
            (
              fun tmpdir ->
                ...
                let result =
                  with_cwd tmpdir
                    (
                      fun () -> run_cli [ "--check"; Path.to_string file ]
                    )
                in
                Test.assert_error result;
                Test.assert_equal ~expected:"type userProfile = int\n" ~actual:(read_file file);
    ...
    Test.case "pipeline parses interface files with interface entrypoint"
      (
        fun _ctx ->
          ...
          let result = Riot_fix.Pipeline.run (Riot_fix.Pipeline.default ()) ~filename:(Path.v "sample.mli") source in
          ...
          with_tempdir "riot_fix_scan"
            (
              fun tmpdir ->
                ...
                let files = Riot_fix.File_scanner.(scan (create ~root:tmpdir ())) |> List.map ~fn:Path.to_string |> List.sort ~compare:String.compare in
          ...
          with_tempdir "riot_fix_scan"
            (
              fun tmpdir ->
                ...
                let files = Riot_fix.File_scanner.(scan (create ~root:tmpdir ~should_ignore:(
                  fun path -> String.contains (Path.to_string path) "/ignored"
                ) ())) |> List.map ~fn:Path.to_string |> List.sort ~compare:String.compare in
    ...
    Test.case "config scope discovers fix providers from workspace packages"
      (
        fun _ctx ->
          with_tempdir "riot_fix_provider_scope"
            (
              fun tmpdir ->
                let workspace_toml = Path.(tmpdir / Path.v "riot.toml") in
                ...
                | [ provider ] ->
                    Test.assert_equal ~expected:(Path.to_string Path.(package_dir / Path.v "fix" / Path.v "no_stdlib_provider.ml")) ~actual:(Path.to_string Riot_model.Fix_provider.(provider.source_path));
                    Ok ()
                | _ -> Error "expected one discovered provider"
            )
      );
    Test.case "config scope defaults provider path to fix/riot_fix_rules.ml"
      (
        fun _ctx ->
          with_tempdir "riot_fix_provider_default_path"
            (
              fun tmpdir ->
                let workspace_toml = Path.(tmpdir / Path.v "riot.toml") in
                ...
                match Riot_fix.Config.providers (Some scope) with
                | [ provider ] ->
                    Test.assert_equal ~expected:(Path.to_string Path.(fix_dir / Path.v "riot_fix_rules.ml")) ~actual:(Path.to_string Riot_model.Fix_provider.(provider.source_path));
                    Test.assert_equal ~expected:[ "demo:demo-rule" ] ~actual:provider.rules;
                    Ok ()
                | _ -> Error "expected one discovered provider"
            )
      );
    Test.case "config scope defaults provider path to fix/riot_fix_rules/riot_fix_rules.ml"
      (
        fun _ctx ->
          with_tempdir "riot_fix_provider_nested_default_path"
            (
              fun tmpdir ->
                let workspace_toml = Path.(tmpdir / Path.v "riot.toml") in
                let package_dir = Path.(tmpdir / Path.v "packages" / Path.v "demo") in
                ...
                write_file workspace_toml "[workspace]\nmembers = [\"packages/demo\"]\n";
                write_file Path.(package_dir / Path.v "riot.toml") "[package]\nname = \"demo\"\nversion = \"0.1.0\"\n\n[riot.fix.provider]\nrules = [\"demo-rule\"]\n";
                write_file Path.(provider_dir / Path.v "riot_fix_rules.ml") "let name = \"demo\"\nlet rules () = []\nlet explanations () = []\n";
                let scope = Riot_fix.Config.load_scope ~cwd:tmpdir |> Option.expect ~msg:"expected workspace scope" in
                match Riot_fix.Config.providers (Some scope) with
                | [ provider ] ->
                    Test.assert_equal ~expected:(Path.to_string Path.(provider_dir / Path.v "riot_fix_rules.ml")) ~actual:(Path.to_string Riot_model.Fix_provider.(provider.source_path));
                    Test.assert_equal ~expected:[ "demo:demo-rule" ] ~actual:provider.rules;
                    Ok ()
                | _ -> Error "expected one discovered provider"
            )
    ...
    Test.case "fixme runner includes provider build dependencies"
      (
        fun _ctx ->
          with_tempdir "riot_fix_provider_build_deps"
            (
              fun tmpdir ->
                let workspace_toml = Path.(tmpdir / Path.v "riot.toml") in
                ...
                write_file Path.(provider_dir / Path.v "riot.toml") "[package]\nname = \"demo\"\nversion = \"0.1.0\"\n\n[build-dependencies]\nhelper = { path = \"../helper\" }\n\n[riot.fix.provider]\nrules = [\"demo-rule\"]\n";
                write_file Path.(helper_dir / Path.v "riot.toml") "[package]\nname = \"helper\"\nversion = \"0.1.0\"\n\n[lib]\npath = \"src/helper.ml\"\n";
                ...
                in
                let plan = Riot_fix.Fixme_runner.materialize ~workspace_root:tmpdir ~target_dir_root:Path.(tmpdir / Path.v "_build") providers in
                let dependency_names = plan.package.Riot_model.Package.dependencies |> List.map ~fn:(
                  fun (dep: Riot_model.Package.dependency) -> dep.name
                ) in
          ...
          with_tempdir "riot_fix_runner_entrypoint"
            (
              fun tmpdir ->
                ...
                let provider = Riot_model.Fix_provider.{
                  name = "std";
                  package_name = package_name "std";
                  package_path;
    ...
    Test.case "fixme runner binary path uses workspace build dir"
      (
        fun _ctx ->
          ...
          let plan = Riot_fix.Fixme_runner.plan ~workspace_root:(Path.v "/workspace") ~target_dir_root:Path.(Path.v "/workspace" / Path.v "_build") [ provider ] in
    ...
    Test.case "fixme runner hash changes when provider support sources change"
      (
        fun _ctx ->
          with_tempdir "riot_fix_runner_hash"
            (
              fun tmpdir ->
                let workspace_root = tmpdir in
                let target_dir_root = Path.(workspace_root / Path.v "_build") in
    ...
    Test.case "rule query collects let bindings from the typed Ast"
      (
        fun _ctx ->
          ...
          Test.assert_equal ~expected:[ "render"; "other" ] ~actual:(bindings |> List.map ~fn:binding_name);
          Ok ()
      );
    Test.case "rule query collects type declarations from implementations and interfaces"
      (
        fun _ctx ->
          ...
          let implementation_types = Riot_fix.Rule_query.type_declarations (rule_context ~file_path:"sample.ml" implementation_source) |> List.map ~fn:type_declaration_name in
          let interface_types = Riot_fix.Rule_query.type_declarations (rule_context ~file_path:"sample.mli" interface_source) |> List.map ~fn:type_declaration_name in
    ...
    Test.case "prefer-record-destructuring-parameters flags immediate record unpacking"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Prefer_record_destructuring_parameters.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          let codes = diagnostic_rule_ids result.diagnostics in
    ...
    Test.case "prefer-record-destructuring-parameters flags repeated field access on one record parameter"
      (
        fun _ctx ->
          ...
          let pipeline = Riot_fix.Pipeline.make ~rules:[ Riot_fix.Rules.Prefer_record_destructuring_parameters.make () ] () in
          let result = Riot_fix.Pipeline.run pipeline source in
          Test.assert_equal ~expected:[ "prefer-record-destructuring-parameters" ] ~actual:(diagnostic_rule_ids result.diagnostics);
          Ok ()
      );
    ...
    Test.case "prefer-result-map-over-manual-match ignores rebuilt error payloads without crashing"
      (
        fun _ctx ->
          ...
          let rules = Riot_fix.Pipeline.default_rules () |> List.filter ~fn:(
            fun rule -> Riot_fix.Rule_id.equal (Riot_fix.Rule.id rule) (Riot_fix.Rule_id.of_string "std:prefer-result-map-over-manual-match")
          ) in
    ...
    Test.case "rule explanations explain record-destructuring parameters"
      (
        fun _ctx ->
          let _ = assert_explanation_contains ~rule_id:"prefer-record-destructuring-parameters" ~snippet:"let { ... } = value in ..." in Ok ()
      );
    Test.case "rule explanations explain ignored map traversal"
      (
        fun _ctx ->
          let _ = assert_explanation_contains ~rule_id:"std:prefer-iter-over-ignored-map" ~snippet:"List.iter" in Ok ()
      );
    Test.case "rule explanations explain List.is_empty preference"
      (
        fun _ctx ->
          let _ = assert_explanation_contains ~rule_id:"std:prefer-list-is-empty" ~snippet:"List.is_empty" in Ok ()
      );
  ]
