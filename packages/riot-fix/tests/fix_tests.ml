...
let replace_token_rule = fun ~rule_id ~needle ~replacement ->
  ...
  Riot_fix.Rule.make ~id:rule_id ~description:message ~explain:message
    ~run:(fun _ctx red_root ->
      ...
      | Some token ->
          let fix = Riot_fix.Fix.make
            ~title:message
            ~operations:[ Riot_fix.Fix.replace_token_with_text ~target:token ~text:replacement ] in
...
let tests = [ Test.case "apply single operation"
    ...
    (fun _ctx ->
      ...
      let fixes = [
        Riot_fix.Fix.make
          ~title:"replace Queue"
          ~operations:[
            Riot_fix.Fix.replace_token_with_text ~target:queue_type_token ~text:"Std.Collections.Queue";
            Riot_fix.Fix.replace_token_with_text ~target:queue_value_token ~text:"Std.Collections.Queue"
          ];
        Riot_fix.Fix.make
    ...
    (fun _ctx ->
      let source = "let =" in
      let result = Riot_fix.Source_runner.run_rule
    ...
    (fun _ctx ->
      ...
      let result = Riot_fix.Rule_test.run_rule ~rule:(Riot_fix.Rules.Snake_case_type_names.make ()) source
    ...
    (fun _ctx ->
      ...
      let result = Riot_fix.Rule_test.run
        ~rules:[
    ...
    (fun _ctx ->
      ...
      let actual = Riot_fix.Rule_test.run
        ~rules:[
