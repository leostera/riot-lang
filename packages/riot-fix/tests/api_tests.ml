...
let json_field = fun fields name -> fields |> List.find ~fn:(
...
let tests =
  [
                      ...
                      | Ok matches -> (
                        ...
                        | Ok request -> (
                          ...
                          | Riot_fix.Run { mode; target; output_mode; _ } ->
                              ...
                              Test.assert_equal ~expected:(Riot_fix.Report Riot_fix.Reporter.Json) ~actual:output_mode;
                              Ok ()
    ...
    Test.case "event to_json encodes progress events"
      (
        fun _ctx ->
          let event = Riot_fix.Event.FileProgress { file = Path.v "sample.ml"; progress = { phase = Fixme.Source_runner.RuleStarted { rule_id = Fixme.Rule_id.of_string "riot:snake-case-type-names" }; timestamp_ms = 42 } } in
          match Riot_fix.Event.to_json event with
          | Data.Json.Object fields ->
              Test.assert_equal ~expected:(Some (Data.Json.String "progress")) ~actual:(json_field fields "type");
              Test.assert_equal ~expected:(Some (Data.Json.String "rule_started")) ~actual:(json_field fields "stage");
              Ok ()
          ...
          with_tempdir "riot_fix_api"
            (
              fun tmpdir ->
                ...
                in
                match Riot_fix.fix ~on_event:(
                  fun event -> seen := event :: !seen
                ) request with
                | Ok _ -> Error "expected issues to remain"
                | Error err ->
                    Test.assert_true (String.contains (Exception.to_string err) "Issues remain after riot fix");
                    Test.assert_true (not (List.is_empty !seen));
                    Ok ()
            )
      );
    Test.case "fix_request_of_matches enables generated runner when providers are present"
      (
        fun _ctx ->
          with_tempdir "riot_fix_api"
            (
...
|} Path.(package_dir / Path.v "riot.toml") |> Result.expect ~msg:"failed to write package riot.toml";
                      Fs.write "let rules () = []\nlet explanations () = []\n" Path.(fix_dir / Path.v "riot_fix_rules.ml") |> Result.expect ~msg:"failed to write provider";
...
let main ~args:_ = Test.Cli.main ~execution_mode:Test.Cli.Linear ~name:"riot-fix:api" ~tests ~args:Env.args ()
