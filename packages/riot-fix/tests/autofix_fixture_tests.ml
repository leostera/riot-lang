...
let split_on_double_underscore = fun value ->
  let rec find idx =
    if idx + 1 >= String.length value then
      ...
      if String.get value ~at:idx = Some '_' && String.get value ~at:(idx + 1) = Some '_' then
        Some idx
      else find (idx + 1)
  in
  match find 0 with
  | Some idx -> Some (String.sub value ~offset:0 ~len:idx, String.sub value ~offset:(idx + 2) ~len:(String.length value - idx - 2))
  | None -> None
...
let find_rule = fun rule_id -> Riot_fix.Pipeline.default_rules () |> List.find ~fn:(
  fun rule -> Riot_fix.Rule_id.equal (Riot_fix.Rule.id rule) rule_id
) |> Option.ok_or ~error:("unknown rule fixture id: " ^ Riot_fix.Rule_id.to_string rule_id)

let result_to_json = fun result ->
  Json.obj
    [
      "diagnostics", Json.array (List.map result.Fixme.Rule_test.initial.diagnostics ~fn:Riot_fix.Diagnostic.to_json);
...
let test_fixture = fun ~(ctx:Test.FixtureRunner.ctx) ->
  let* rule_id = rule_id_of_fixture ctx.fixture_path
  in
  let* rule = find_rule rule_id
  in
  let* source = Fs.read ctx.fixture_path |> Result.map_err ~fn:IO.error_message
  in
  let* result = Fixme.Rule_test.run_rule ~rule ~filename:ctx.fixture_path source
  in
  let* () =
  ...
  let* () =
    ...
    | Some _ -> Error ("autofix fixture rewrote to invalid OCaml for rule " ^ Riot_fix.Rule_id.to_string rule_id)
    | None -> Error ("autofix fixture did not produce a post-fix analysis for rule " ^ Riot_fix.Rule_id.to_string rule_id)
  in
  Test.Snapshot.assert_text ~ctx:ctx.test ~actual:(Json.to_string_pretty (result_to_json result) ^ "\n")

let main ~args =
  let tests = Test.FixtureRunner.cases () ~dir:fixture_root ~filter:keep_ml ~snapshot_path:approved_snapshot_path ~run:(
    fun ctx -> test_fixture ~ctx
  ) in Test.Cli.main ~name:"riot-fix autofix fixtures" ~tests ~args ()
