...
|}
  ...
  let explanation = Api.Explanation.{ rule_id = package_rule_id; body = rule_explain; message = rule_description }
  ...
  let make_fix = fun token replacement -> Api.Fix.make ~title:("Replace " ^ Syn.Ast.Token.text token ^ " with " ^ replacement) ~operations:[ Api.Fix.replace_token_with_text ~target:token ~text:replacement ]
  ...
  let make_diagnostic = fun token ->
    ...
    let start, end_ = Syn.Ast.Token.raw_range token in Api.Diagnostic.make ~severity:Warning ~kind ~span:(Syn.Ceibo.Span.make ~start ~end_) ?suggestion ?fix ()

  let dedupe_diagnostics = fun diagnostics ->
    let seen = HashMap.create () in List.filter diagnostics ~fn:(
      fun diag ->
        let span = Api.Diagnostic.span diag in
        let key = Api.Rule_id.to_string (Api.Diagnostic.rule_id diag) ^ ":" ^ Int.to_string span.start ^ ":" ^ Int.to_string span.end_ in
        match HashMap.get seen ~key with
        | Some _ -> false
        | None ->
            let _ = HashMap.insert seen ~key ~value:true in true
    )
  ...
  let check_tree = fun (_ctx: Api.Rule.context) root ->
    Api.Traversal.find_tokens
      ...
      root |> List.map ~fn:make_diagnostic |> dedupe_diagnostics

  let rule = fun () -> Api.Rule.make ~id:package_rule_id ~description:rule_description ~explain:rule_explain ~run:check_tree ()
end
...
let fixture_filter = fun path ->
  ...
  | Some ".ml" ->
      ...
      if String.length name >= 4 && (String.get name ~at:0 |> Option.is_some_and ~fn:is_digit) && (String.get name ~at:1 |> Option.is_some_and ~fn:is_digit) && (String.get name ~at:2 |> Option.is_some_and ~fn:is_digit) && (String.get name ~at:3 |> Option.is_some_and ~fn:is_digit) then
        `keep
      else `skip
  | _ -> `skip

let append_snapshot_suffix = fun path suffix -> Path.to_string path ^ suffix |> Path.from_string |> Result.expect ~msg:"snapshot path should stay valid UTF-8"
...
let file_result_to_json = fun ~workspace_root result -> let open Json in
Object [
  ...
  "parse_diagnostics", Array (List.map Riot_fix.Runner.(result.parse_diagnostics) ~fn:Syn.Diagnostic.to_json);
  "diagnostics", Array (List.map Riot_fix.Runner.(result.diagnostics) ~fn:Riot_fix.Diagnostic.to_json);
]

let run_result_to_json = fun ~workspace_root result ->
  Json.Object [
    "summary", Riot_fix.Runner.summary_to_json Riot_fix.Runner.(result.summary);
    "files", Json.Array (List.map Riot_fix.Runner.(result.files) ~fn:(file_result_to_json ~workspace_root));
  ]
...
let test_fixture = fun ~(ctx:Test.FixtureRunner.ctx) ->
  ...
  let () = Riot_fix.Provider_registry.register_providers [ (module Fixture_std_provider : Riot_fix.Provider.S) ] in
  let result = Riot_fix.Runner.run_files ~mode:Check [ ctx.fixture_path ] in
  let actual_json = run_result_to_json ~workspace_root result in Test.Snapshot.assert_with ~ctx:ctx.test ~render:(
    fun json -> Json.to_string_pretty json ^ "\n"
  ) ~actual:actual_json

let main ~args =
  let tests = Test.FixtureRunner.cases () ~dir:tests_dir ~filter:fixture_filter ~snapshot_path:(
    fun path -> Some (approved_snapshot_path path)
  ) ~run:(
    fun ctx -> test_fixture ~ctx
  ) in Test.Cli.main ~name:"riot-fix:fixtures" ~tests ~args ()
