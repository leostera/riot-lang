...
let unavailable_build_package = fun ~workspace:_ ~package_name:_ ~profile:_ ?transform_workspace:_ () -> Error (Failure "No build_package callback was provided")
...
let output_mode_of_request = fun request ->
  ...
  | ListRules { format } | ListDiagnostics { format } -> Report format
...
let response_output response =
  match response with
  | Completed -> None
  | ListedRules { output; _ } | ListedDiagnostics { output; _ } | ExplainedRule { output; _ } -> Some output
...
let fix = fun ?(build_package = unavailable_build_package) ?(on_event = no_event) ?output_mode request ->
  ...
  in
  match request.action with
  | ListRules { format } -> Ok (ListedRules { format; output = Cli.Catalog.list_rules_output ~format })
  | ListDiagnostics { format } -> Ok (ListedDiagnostics { format; output = Cli.Catalog.list_diagnostics_output ~format })
  | ExplainRule { rule_id } -> explain_rule_output rule_id |> Result.map ~fn:(
    fun output -> ExplainedRule { rule_id; output }
  )
  | Run { mode; limit; target; use_generated_runner; output_mode = _ } ->
      (
        ...
        | Some scope, true ->
            let report_output =
              ...
              | Report _ -> true
            in
            Cli.Execution.run_generated_runner ~cwd:request.cwd ~build_package ~report_output ~mode ~limit ~target ~output_mode scope
        | _ -> Cli.Execution.run_with_coordinator ~on_event ~output_mode ~mode ~scope:request.scope ~limit ~roots:[ target ] ()
      ) |> Result.map ~fn:(
        fun () -> Completed
      )
