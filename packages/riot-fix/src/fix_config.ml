...
type package_scope = { package_root: Path.t; config: fix_config }
...
let parse_rule_override item =
  ...
  | Data.Toml.String name ->
      ...
      else Some { name; state = Enabled }
  | Data.Toml.Table attrs -> (
    let name_value = List.find attrs ~fn:(
      fun (attr_name, _) -> String.equal attr_name "name"
    ) |> Option.map ~fn:(
      fun (_, value) -> value
    ) in
    ...
    | Some (Data.Toml.String name) ->
        let state =
          let state_value = List.find attrs ~fn:(
            fun (attr_name, _) -> String.equal attr_name "state"
          ) |> Option.map ~fn:(
            fun (_, value) -> value
          ) in
          match state_value with
          | Some (Data.Toml.String state) -> Option.unwrap_or ~default:Enabled (parse_rule_state state)
          | _ -> Enabled
        in
        Some { name; state }
    | _ -> None
...
let parse_fix_config toml =
  ...
  | Data.Toml.Table items -> (
    let riot_value = List.find items ~fn:(
      fun (item_name, _) -> String.equal item_name "riot"
    ) |> Option.map ~fn:(
      fun (_, value) -> value
    ) in
    match riot_value with
    | Some (Data.Toml.Table riot_items) -> (
      let fix_value = List.find riot_items ~fn:(
        fun (item_name, _) -> String.equal item_name "fix"
      ) |> Option.map ~fn:(
        fun (_, value) -> value
      ) in
      ...
      | Some (Data.Toml.Table fix_items) ->
          let ignore_patterns =
            let ignore_value = List.find fix_items ~fn:(
              fun (item_name, _) -> String.equal item_name "ignore"
            ) |> Option.map ~fn:(
              fun (_, value) -> value
            ) in
          ...
          in
          let rules =
            let rules_value = List.find fix_items ~fn:(
              fun (item_name, _) -> String.equal item_name "rules"
            ) |> Option.map ~fn:(
              fun (_, value) -> value
            ) in
...
let load_scope = fun ~cwd ->
  ...
  | Ok (workspace, _load_errors) ->
      ...
      let packages = workspace.packages |> List.filter ~fn:Package_manifest.is_workspace_member |> List.map ~fn:(
        fun (pkg: Package_manifest.t) ->
          let package_toml = Path.(pkg.path / Path.v "riot.toml") in { package_root = pkg.path; config = load_fix_config package_toml }
      ) in
      Some {
        workspace;
...
let glob_match = fun pattern text ->
  ...
  for pattern_idx = 1 to pattern_len do
    ...
    if String.get_unchecked pattern ~at:(pattern_idx - 1) = '*' then
      (
        Array.set current ~at:0 ~value:(Array.get_unchecked !previous ~at:0);
        for text_idx = 1 to text_len do Array.set current ~at:text_idx ~value:(Array.get_unchecked !previous ~at:text_idx || Array.get_unchecked current ~at:(text_idx - 1)) done
      )
    else
      (
        for text_idx = 1 to text_len do Array.set current ~at:text_idx ~value:(Array.get_unchecked !previous ~at:(text_idx - 1) && String.get_unchecked pattern ~at:(pattern_idx - 1) = String.get_unchecked text ~at:(text_idx - 1)) done
      );
    previous := current
  done;
...
let matches_pattern = fun pattern candidate ->
  ...
  else String.equal pattern candidate || String.equal pattern basename || String.contains candidate pattern
...
let matches_ignore_patterns = fun file patterns ->
  let path = Path.to_string file in List.any patterns ~fn:(
    fun pattern -> matches_pattern pattern path
  )
...
let set_rule_state = fun states name enabled -> (name, enabled) :: List.filter states ~fn:(
  fun (existing, _) -> not (String.equal existing name)
)
...
let matching_rule_names = fun states name ->
  if String.contains name ":" then
    ...
    let names = List.map states ~fn:(
      fun (rule_name, _) -> rule_name
    ) in
    let exact_matches = List.filter names ~fn:(
      fun actual -> String.equal actual name
    ) in
...
let apply_rule_overrides = fun states overrides -> List.fold_left overrides ~init:states ~fn:(
  fun acc rule_override ->
    let enabled =
      ...
      | Disabled -> false
    in
    matching_rule_names acc rule_override.name |> List.fold_left ~init:acc ~fn:(
      fun acc rule_name -> set_rule_state acc rule_name enabled
    )
)

let default_rule_states = fun () -> Pipeline.default_rule_ids () |> List.map ~fn:(
  fun name -> (Rule_id.to_string name, true)
)
...
let effective_rule_states = fun scope file ->
  ...
  | Some scope ->
      let base_states =
        default_rule_states () |> fun states -> apply_rule_overrides states scope.workspace_config.rules
      in
      match find_package_scope scope file with
...
let pipeline_for_file = fun scope file ->
  let enabled_rule_ids = effective_rule_states scope file |> List.filter_map ~fn:(
    fun (name, enabled) ->
      if enabled then
        Some (Rule_id.of_string name)
      else None
  ) in Pipeline.make ~rules:(Pipeline.rules_by_id enabled_rule_ids) ()
