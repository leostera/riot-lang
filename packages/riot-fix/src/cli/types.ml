...
type build_package = workspace:Riot_model.Workspace_manifest.t -> package_name:Riot_model.Package_name.t -> profile:string -> ?transform_workspace:(Riot_model.Workspace.t -> Riot_model.Workspace.t) -> unit -> (unit, exn) result

type run_outcome = { result: Runner.run_result; limit_reached: bool }
