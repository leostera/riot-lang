...
val run_result: mode:Runner.mode -> scope:Fix_config.scope option -> limit:int option -> files:Path.t list -> run_outcome

val run: ?build_package:Types.build_package -> ArgParser.matches -> (unit, exn) result

val run_args: ?cwd:Path.t -> ?on_event:(event -> unit) -> ?report_output:bool -> build_package:Types.build_package -> string list -> (unit, exn) result

val run_check_paths: ?cwd:Path.t -> ?on_event:(event -> unit) -> ?report_output:bool -> build_package:Types.build_package -> Path.t list -> (unit, exn) result
