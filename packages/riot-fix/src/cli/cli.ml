...
type run_outcome = Types.run_outcome = { result: Runner.run_result; limit_reached: bool }
...
command "fix" |> about "Lint OCaml code and optionally apply safe fixes" |> args
  [
...
let resolved_output_mode = fun ?output_mode (request: Request.t) ->
  ...
  | None -> (
    ...
    | Request.ListRules { format } | Request.ListDiagnostics { format } -> Types.Report format
...
let run_request_direct = fun ~on_event ~output_mode (request: Request.t) ->
  ...
  | Request.Run { mode; limit; target; output_mode = _; use_generated_runner = _ } -> Execution.run_with_coordinator ~on_event ~output_mode ~mode ~scope:request.scope ~limit ~roots:[ target ] ()
...
let run_matches = fun ~build_package ?(on_event = Types.no_event) ?output_mode matches ->
  ...
  | Ok request ->
      ...
      | Request.ListDiagnostics { format } -> Catalog.list_diagnostics format
      | Request.ExplainRule { rule_id } -> Catalog.explain_rule rule_id
      | Request.Run { mode; limit; target; use_generated_runner; output_mode = _ } -> (
        ...
        | Some scope, true ->
            let report_output =
              ...
              | Types.Report _ -> true
            in
            Execution.run_generated_runner ~cwd:request.cwd ~build_package ~report_output ~mode ~limit ~target ~output_mode scope
...
let run_check_paths = fun ?cwd ?(on_event = Types.no_event) ?(report_output = false) ~build_package paths ->
  let args = "--check" :: List.map paths ~fn:Path.to_string in run_args ?cwd ~on_event ~report_output ~build_package args
