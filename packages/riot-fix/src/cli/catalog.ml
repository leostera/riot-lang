  ...
  |> List.sort
    ~compare:(fun left right ->
      ...
      if package_cmp != Order.EQ then
        ...
        let left_category =
          Pipeline.builtin_rule_category (Rule.id left)
          |> Option.unwrap_or ~default:"Other"
        in
        let right_category =
          Pipeline.builtin_rule_category (Rule.id right)
          |> Option.unwrap_or ~default:"Other"
        in
...
let list_rules_output = fun ~format ->
  ...
  | Reporter.Json ->
      Data.Json.Array (List.map rules ~fn:rule_to_json)
      |> Data.Json.to_string
...
let list_diagnostics_output = fun ~format ->
  ...
  | Reporter.Json ->
      Data.Json.Array (List.map entries ~fn:diagnostic_to_json)
      |> Data.Json.to_string
