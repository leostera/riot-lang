...
let display_rule_id_text = fun rule_id ->
  let package_name, local_id = split_rule_id rule_id in package_name ^ ":" ^ local_id

let display_rule_id = fun rule -> display_rule_id_text (Rule.id rule)

let sorted_rules = fun () -> Pipeline.default_rules () |> List.sort ~compare:(
  fun left right ->
    ...
    if package_cmp != Order.EQ then
      ...
      if String.equal left_package "riot" then
        let left_category = Pipeline.builtin_rule_category (Rule.id left) |> Option.unwrap_or ~default:"Other" in
        let right_category = Pipeline.builtin_rule_category (Rule.id right) |> Option.unwrap_or ~default:"Other" in
        let category_cmp = String.compare left_category right_category in
        if category_cmp != Order.EQ then
          category_cmp
        else String.compare left_local right_local
      else String.compare left_local right_local
)

let sorted_diagnostics = fun () -> Explanations.all () |> List.sort ~compare:(
  fun left right -> String.compare (display_rule_id_text Explanation.(left.rule_id)) (display_rule_id_text Explanation.(right.rule_id))
)

let rule_to_json = fun rule -> let open Data.Json in
  ...
  let rec build_lines = fun current_package current_category acc ->
    ...
    | rule :: rest ->
        ...
        let rule_line =
          ...
          else bold (display_rule_id rule) ^ " - " ^ Rule.description rule
        in
        let acc =
          match package_name, current_package, category, current_category with
          | "riot", Some "riot", Some category_name, Some current when not (String.equal category_name current) -> rule_line :: ("  " ^ category_name ^ ":") :: acc
          | "riot", Some "riot", _, _ -> rule_line :: acc
          | "riot", _, Some category_name, _ -> rule_line :: ("  " ^ category_name ^ ":") :: "riot:" :: "" :: acc
          | _, Some current, _, _ when not (String.equal current package_name) -> rule_line :: (package_name ^ ":") :: "" :: acc
...
let list_diagnostics_text = fun entries ->
  let bold text = "\027[1m" ^ text ^ "\027[0m" in entries |> List.map ~fn:(
    fun entry -> bold (display_rule_id_text Explanation.(entry.rule_id)) ^ " - " ^ Explanation.(entry.message)
  ) |> String.concat "\n"
