...
let package_entries = fun () -> Provider_registry.providers () |> List.map ~fn:Provider.explanations |> List.concat
...
let explain = fun rule_id ->
  let normalized = normalize_rule_id rule_id in all () |> List.find ~fn:(
    fun entry -> Rule_id.equal Explanation.(entry.rule_id) normalized
  )
