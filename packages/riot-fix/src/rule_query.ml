...
let structure_items = fun (ctx: Rule.context) ->
  ...
  Ast.SourceFile.for_each_structure_item ctx.source_file ~fn:(
...
let signature_items = fun (ctx: Rule.context) ->
  let items = Vector.with_capacity ~size:(Ast.Node.child_count ctx.source_file) in
  Ast.SourceFile.for_each_signature_item ctx.source_file ~fn:(
    fun item -> Vector.push items ~value:item
  );
  to_list items
...
let expressions = fun (ctx: Rule.context) ->
  Traversal.find_nodes
    ...
    ((ctx.source_file : Ast.Node.t)) |> List.filter_map ~fn:Ast.Expr.cast
...
let let_bindings = fun (ctx: Rule.context) ->
  Traversal.find_nodes
    ...
    ((ctx.source_file : Ast.Node.t)) |> List.filter_map ~fn:Ast.LetBinding.cast
...
let type_declarations = fun (ctx: Rule.context) ->
  Traversal.find_nodes
    ...
    ((ctx.source_file : Ast.Node.t)) |> List.filter_map ~fn:Ast.TypeDeclaration.cast
