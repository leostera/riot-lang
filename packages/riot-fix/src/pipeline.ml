...
let builtin_rule_category = fun rule_id ->
  ...
  |> List.find ~fn:(fun factory -> String.equal factory.id rule_id)
...
let matching_rule_ids = fun rules requested_id ->
  ...
  if List.contains available_ids ~value:requested_id then
    ...
    List.filter
      available_ids
      ~fn:(fun available_id -> String.equal (Rule_id.local_id available_id) local_id)
...
let rules_by_id = fun ids ->
  ...
  |> List.map
    ~fn:(fun id ->
      ...
      | matches -> matches)
  |> List.concat
  |> List.sort ~compare:Rule_id.compare
  |> List.unique ~compare:Rule_id.compare
  |> List.filter_map
    ~fn:(fun id -> List.find available_rules ~fn:(fun rule -> Rule_id.equal (Rule.id rule) id))
