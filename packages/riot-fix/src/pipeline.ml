...
type builtin_rule_factory = { category: string; id: string; make: unit -> Rule.t }
...
let builtin_rule_category = fun rule_id ->
  let rule_id = Rule_id.local_id rule_id in builtin_rule_factories () |> List.find ~fn:(
    fun factory -> String.equal factory.id rule_id
  ) |> Option.map ~fn:(
    fun factory -> factory.category
  )
...
let filtered_builtin_rules = fun package_rules ->
  let shadowed_ids = package_rules |> List.map ~fn:Rule.id |> List.map ~fn:unqualified_rule_id in builtin_rule_factories () |> List.map ~fn:(
    fun factory -> factory.make ()
  ) |> List.filter ~fn:(
    fun rule -> not (List.contains shadowed_ids ~value:(unqualified_rule_id (Rule.id rule)))
  )

let builtin_rules = fun () -> builtin_rule_factories () |> List.map ~fn:(
  fun factory -> factory.make ()
)

let run = fun pipeline ?filename ?on_progress source -> Fixme.Source_runner.run ~rules:pipeline.rules ?filename ?on_progress source

let default_rules = fun () ->
  let package_rules = package_rules () in filtered_builtin_rules package_rules @ package_rules
...
let matching_rule_ids = fun rules requested_id ->
  ...
  if List.contains available_ids ~value:requested_id then
    ...
    if Rule_id.has_package_name requested_id then
      []
    else
      let local_id = Rule_id.local_id requested_id in List.filter available_ids ~fn:(
        fun available_id -> String.equal (Rule_id.local_id available_id) local_id
      )

let rules_by_id = fun ids ->
  let available_rules = default_rules () in ids |> List.map ~fn:(
    fun id ->
      ...
      | [] ->
          ...
          []
      | matches -> matches
  ) |> List.concat |> List.sort ~compare:Rule_id.compare |> List.unique ~compare:Rule_id.compare |> List.filter_map ~fn:(
    fun id -> List.find available_rules ~fn:(
      fun rule -> Rule_id.equal (Rule.id rule) id
    )
  )
