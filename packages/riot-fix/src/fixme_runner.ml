...
let sanitize_component = fun text -> String.map text ~fn:(
  fun ch ->
...
let generated_module_name = fun (provider: Riot_model.Fix_provider.t) -> "Provider_" ^ sanitize_component (Riot_model.Package_name.to_string provider.package_name) ^ "_" ^ sanitize_component provider.name
...
let ocaml_module_name_of_path = fun path ->
  ...
  if String.length base = 0 then
    "Generated"
  else String.uppercase_ascii (String.sub base ~offset:0 ~len:1) ^ String.sub base ~offset:1 ~len:(String.length base - 1)
...
let support_module_sources = fun (provider: Riot_model.Fix_provider.t) ->
  ...
  | Error _ -> []
  | Ok iter -> Std.Iter.MutIterator.to_list iter |> List.filter_map ~fn:(
    fun entry ->
      let source_path = Path.(provider_dir / entry) in
      let entry_name = Path.basename source_path in
      if String.equal entry_name provider_basename || not (String.ends_with ~suffix:".ml" entry_name) then
        None
...
let provider_fingerprint = fun (provider: Riot_model.Fix_provider.t) ->
  let support_hashes = support_module_sources provider |> List.map ~fn:(
    fun (module_name, source_path) -> module_name ^ ":" ^ Path.to_string source_path ^ ":" ^ file_content_hash source_path
  ) |> String.concat "," in
  String.concat ":"
    [
...
let provider_hash = fun providers ->
  providers |> List.sort ~compare:(
    fun (left: Riot_model.Fix_provider.t) right -> String.compare (provider_fingerprint left) (provider_fingerprint right)
  ) |> List.map ~fn:provider_fingerprint |> fun fingerprints -> String.concat "\n" (generator_version :: fingerprints) |> Crypto.hash_string |> Crypto.Digest.hex
...
let provider_module_line = fun (provider: generated_provider) -> "    (module " ^ provider.module_name ^ " : Riot_fix.Provider.S);"

let registry_source = fun providers ->
  let provider_lines = providers |> List.map ~fn:(
    fun (provider: Riot_model.Fix_provider.t) -> "    (module " ^ generated_module_name provider ^ " : Riot_fix.Provider.S);"
  ) in
  String.concat "\n"
    [
      "open Std";
...
let embedded_provider_module_source = fun (provider: generated_provider) ->
  let source = Fs.read provider.provider.source_path |> Result.expect ~msg:("failed to read provider source " ^ Path.to_string provider.provider.source_path) in
  String.concat "\n"
    [
      "module " ^ provider.module_name ^ " = struct";
      String.concat "\n" (List.map provider.support_module_sources ~fn:(
        fun (module_name, source_path) ->
          let source = Fs.read source_path |> Result.expect ~msg:("failed to read provider support source " ^ Path.to_string source_path) in
          String.concat "\n"
            [
...
let dependency_entries = fun workspace_root providers ->
  ...
  let resolve_dependency_path ~package_path path =
    if Path.is_absolute path then
      Path.normalize path
    else Path.normalize Path.(package_path / path)
  in
  let workspace_package_path name = workspace_packages |> List.find ~fn:(
    fun (pkg: Riot_model.Package_manifest.t) -> Riot_model.Package_name.equal pkg.name name
  ) |> Option.map ~fn:(
    fun (pkg: Riot_model.Package_manifest.t) -> pkg.path
  ) in
  let provider_build_deps = providers |> List.map ~fn:(
    fun ({ provider; _ }: generated_provider) ->
      workspace_packages |> List.find ~fn:(
        fun (pkg: Riot_model.Package_manifest.t) -> Riot_model.Package_name.equal pkg.name provider.package_name
      ) |> Option.map ~fn:(
        fun (pkg: Riot_model.Package_manifest.t) -> (pkg.path, pkg.build_dependencies)
      ) |> Option.unwrap_or ~default:(provider.package_path, []) |> fun (package_path, build_dependencies) -> build_dependencies |> List.filter_map ~fn:(
        fun (dep: Riot_model.Package.dependency) ->
          match dep.source with
          | { workspace = true; _ } -> workspace_package_path dep.name |> Option.map ~fn:(
            fun path -> (dep.name, path)
          )
          | { builtin = true; _ } -> None
          | { path = Some path; _ } ->
              let path =
  ...
  let package_name value = Riot_model.Package_name.from_string value |> Result.expect ~msg:("expected valid package name: " ^ value) in
  let entries =
    ([
    ...
    ] @ provider_build_deps) @ List.map providers ~fn:(
  ...
  let rec dedupe_by_name seen acc = function
    ...
    | (name, path) :: rest ->
        ...
        else dedupe_by_name (name :: seen) ((name, path) :: acc) rest
  in
  dedupe_by_name [] [] entries

let dependency_of_entry = fun (name, path) -> Riot_model.Package.{
  name;
  source = {
    workspace = false;
    builtin = false;
...
let package_dependencies = fun ~workspace_root providers -> dependency_entries workspace_root providers |> List.map ~fn:dependency_of_entry
...
let plan = fun ~workspace_root ~target_dir_root providers ->
  ...
  let generated_dir = Path.(target_dir_root / Path.v "riot-fix" / Path.v "fixme-runner" / Path.v hash) in
  ...
  let binary_path = Path.(target_dir_root / Path.v "release" / Path.v (Riot_model.Target.to_string (Riot_model.Riot_dirs.host_target ())) / Path.v "out" / Path.v (Riot_model.Package_name.to_string package_name) / Path.v binary_name) in
  let placeholder_package = Riot_model.Package.synthetic ~name:package_name ~path:package_dir ~relative_path:(relative_path_for_package ~workspace_root package_dir) in
  let plan = {
    provider_hash = hash;
    generated_dir;
    package_dir;
  ...
  let package = Riot_model.Package.make ~name:package_name ~path:package_dir ~relative_path:(relative_path_for_package ~workspace_root package_dir) ~dependencies:(package_dependencies ~workspace_root providers) ~binaries:[ Riot_model.Package.{ name = binary_name; path = Path.v "src/main.ml" } ] ~library:Riot_model.Package.{ path = Path.v "src/fixme_runner.ml" } ~sources:Riot_model.Package.{
    src = [ Path.v "src/fixme_runner.ml"; Path.v "src/main.ml" ];
    native = [];
    tests = [];
    examples = [];
    bench = []
  } () in { plan with package; providers }
...
let main_source =
  String.concat "\n"
    [
      ...
      "";
      "let () =";
      "  Runtime.run ~main:Fixme_runner.main ~args:Env.args ()";
      "";
    ]
...
let ensure_directories = fun plan ->
  List.for_each
    ...
    ~fn:(
      fun path -> Fs.create_dir_all path |> Result.expect ~msg:("failed to create generated fixme runner dir " ^ Path.to_string path)
    )
...
let write_file = fun path content -> Fs.write content path |> Result.expect ~msg:("failed to write " ^ Path.to_string path)

let copy_provider_source = fun (provider: generated_provider) -> Fs.copy ~src:provider.provider.source_path ~dst:provider.copied_source_path |> Result.expect ~msg:("failed to copy provider source " ^ Path.to_string provider.provider.source_path)

let attach_to_workspace = fun workspace plan ->
  let other_packages = workspace.Riot_model.Workspace.packages |> List.filter ~fn:(
    fun (pkg: Riot_model.Package_manifest.t) -> not (Riot_model.Package_name.equal pkg.name plan.package_name)
  ) in { workspace with packages = other_packages @ [ Riot_model.Package_manifest.of_package plan.package ] }
