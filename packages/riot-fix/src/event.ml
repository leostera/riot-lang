...
let json_object_with_type = fun type_name json -> let open Data.Json in
match json with
...
let to_json event =
  match event with
  | Start { mode; concurrency } -> let open Data.Json in
  Object [
    "type", String "start";
    "mode", String (
      match mode with
  ...
  | FileStarted { file } -> let open Data.Json in
  Object [
    "type", String "file_started";
    "file", String (Path.to_string file);
    "timestamp_ms", Int (timestamp_ms ());
  ]
  | FileProgress { file; progress = event } -> let open Data.Json in
  let phase_fields =
  ...
  | Summary { summary; limit_reached } -> let open Data.Json in
  (
    match Runner.summary_to_json summary with
    | Object fields -> Object (("type", String "summary") :: ("limit_reached", Bool limit_reached) :: fields)
