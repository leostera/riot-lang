...
let rec expression_mentions_any_name = fun names expr ->
  match expr with
...
let rec parameter_arguments_match = fun parameter_names arguments ->
  match parameter_names, arguments with
  | [], [] ->
      true
  | parameter_name :: rest_names, argument :: rest_arguments -> (
      match path_name argument with
      | Some argument_name -> String.equal parameter_name argument_name
