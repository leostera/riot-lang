...
let diagnostic_for_type_declaration = fun value_declarations decl ->
  ...
  | Some "t" -> (
      ...
      | Syn.Cst.TypeDefinition.Record { fields; _ } ->
          let field_names = fields |> List.map Syn.Cst.RecordField.name in
          let accessor_names =
            value_declarations
            |> List.filter is_accessor_of_t
            |> List.map
              (fun (value_decl: Syn.Cst.value_declaration) ->
                value_decl.name_tokens |> List.map Syn.Cst.Token.text |> String.concat "")
            |> List.filter
              (fun name ->
