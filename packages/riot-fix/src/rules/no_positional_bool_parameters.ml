...
let rec is_bool_core_type = fun type_ ->
...
let parameter_inline_type = fun parameter ->
  match parameter with
  | Syn.Cst.Parameter.Positional { syntax_node; _ } -> first_core_type_node syntax_node
  | Syn.Cst.Parameter.Labeled _
  | Syn.Cst.Parameter.Optional _
  | Syn.Cst.Parameter.LocallyAbstract _ -> None

let rec arrow_parameters = fun type_ ->
  match strip_type_wrappers type_ with
  | Syn.Cst.CoreType.Poly { body; _ } -> arrow_parameters body
  | Syn.Cst.CoreType.Arrow { label; parameter_type; result_type; _ } -> (label, parameter_type)
  :: arrow_parameters result_type
  | _ -> []

let typed_value_function_type = fun binding ->
  let rec go = function
    | Syn.Cst.Expression.TypeAscription { expression; kind=Syn.Cst.Type { type_; _ }; _ }
    | Syn.Cst.Expression.Polymorphic { expression; type_; _ } ->
        if Syn.Cst.LetBinding.is_function binding then
          Some (expression, type_)
        else
          go expression
    | Syn.Cst.Expression.Parenthesized { inner; _ } -> go inner
    | _ -> None
  in
...
let diagnostic_for_parameter = fun parameter ->
  ...
  | Some type_node when is_bool_type_node type_node -> Some (make_diagnostic
    ~span:(Syn.Ceibo.Red.SyntaxNode.span (Syn.Cst.Parameter.syntax_node parameter)))
  | _ -> None

let diagnostics_for_binding = fun binding ->
  let inline_parameter_diagnostics = Syn.Cst.LetBinding.parameters binding |> List.filter_map ~fn:diagnostic_for_parameter in
...
let diagnostics_for_value_declaration = fun ({ name_tokens; type_; _ }: Syn.Cst.value_declaration) ->
  ...
  in
  arrow_parameters type_ |> List.filter_map
    ~fn:(
      function
