...
type binding_site = { syntax_node: Ast.Node.t; name_token: Ast.Token.t; is_function: bool }
...
let binding_site_of_let_binding = fun binding -> binding_name_token binding |> Option.map ~fn:(
  fun name_token -> { syntax_node = (binding : Ast.Node.t); name_token; is_function = false }
)
