...
let run_pipeline = fun ?on_progress pipeline file source -> Pipeline.run pipeline ~filename:file ?on_progress source
...
let run_file = fun ?pipeline ?pipeline_for_file ?on_progress ~mode file ->
  ...
  | Ok source -> (
    ...
    | Apply -> (
      ...
      | Error reason -> empty_result file (Some ("Failed to apply fixes for " ^ Path.to_string file ^ ": " ^ reason))
      | Ok None ->
          {
            file;
            final_source = source;
...
let summarize = fun files -> List.fold_left files ~init:{
  total_files = 0;
  changed_files = 0;
  ...
  fun acc result ->
    {
      ...
      remaining_diagnostics = acc.remaining_diagnostics + List.length result.diagnostics + List.length result.parse_diagnostics;
...
let run_files = fun ?pipeline ?pipeline_for_file ~mode files ->
  let files = List.sort files ~compare:(
    fun a b -> String.compare (Path.to_string a) (Path.to_string b)
  ) in
  let results = List.map files ~fn:(run_file ?pipeline ?pipeline_for_file ~mode) in { files = results; summary = summarize results }
...
let file_result_to_json = fun result -> let open Data.Json in
Object [
