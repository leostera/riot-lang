...
let test_library_cmxa_uses_store_location () =
  ...
  let expected =
    Path.(
      dep.artifact_dir
      / Tusk_model.Module_name.(of_string dep.package.name |> cmxa))
  in
...
let test_module_graph_prefers_implementation_when_interface_exists () =
  match
    Fs.with_tempdir ~prefix:"module_graph_prefers_impl" (fun tmpdir ->
        ...
        in
        let graph_builder =
          Tusk_planner.Module_graph.create
        ...
        let find_node_id expected_kind expected_name =
          let matches (_id, (node : Tusk_planner.Module_node.t G.node))
            ...
            | Tusk_planner.Module_node.ML mod_
              when expected_kind = `implementation ->
                String.equal
                  (Tusk_model.Module.module_name mod_
                  |> Tusk_model.Module_name.to_string)
                  expected_name
            | Tusk_planner.Module_node.MLI mod_
              when expected_kind = `interface ->
                String.equal
                  (Tusk_model.Module.module_name mod_
                  |> Tusk_model.Module_name.to_string)
                  expected_name
          ...
          | None ->
              Error
                ("expected node not found: " ^ expected_name)
        in
        match
        ...
        | Ok bar_mli_id, Ok foo_ml_id, Ok foo_mli_id -> (
            ...
            | Some node ->
                let depends_on_impl =
                  List.exists (G.Node_id.eq foo_ml_id) node.deps
                in
                let depends_on_intf =
                  List.exists (G.Node_id.eq foo_mli_id) node.deps
                in
                if depends_on_impl && not depends_on_intf then Ok ()
                else
                  Error
                    "expected bar.mli to depend on foo.ml implementation node only"
            )
...
let tests =
  Test.
    [
      ...
      case "module graph prefers implementation when interface exists"
