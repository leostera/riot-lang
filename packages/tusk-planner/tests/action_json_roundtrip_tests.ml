...
let assert_roundtrip = fun action ->
  ...
  | Ok decoded ->
      if Tusk_planner.Action.equal action decoded then
        Ok ()
      else
        Error "action changed after json roundtrip"
  | Error err -> Error ("failed to decode action json: " ^ err)

let compile_interface_roundtrip_preserves_fields = fun () -> assert_roundtrip
...
let compile_implementation_roundtrip_preserves_flags = fun () -> assert_roundtrip
...
let compile_c_roundtrip_preserves_ccflags = fun () -> assert_roundtrip
...
let create_executable_roundtrip_preserves_linker_fields = fun () -> assert_roundtrip
...
let create_shared_library_roundtrip_preserves_linker_fields = fun () -> assert_roundtrip
...
let build_foreign_dependency_roundtrip_preserves_env_and_outputs = fun () -> assert_roundtrip
...
let compile_implementation_roundtrip_preserves_combined_warning_flags = fun () -> assert_roundtrip
(Tusk_planner.Action.CompileImplementation {
