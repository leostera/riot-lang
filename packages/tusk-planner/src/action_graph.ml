...
Path.t list =
  ...
  in
  let get_dep_outputs = fun dep_id ->
...
let from_json = fun json ->
  let open Data.Json in
    ...
    | Some (Array node_jsons) -> (
        ...
        let parse_actions = fun actions_json ->
        ...
        in
        let parse_paths = fun paths_json ->
        ...
        in
        let parse_dependencies = fun deps_json ->
