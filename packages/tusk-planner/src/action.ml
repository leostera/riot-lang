...
let hash = fun action ->
  let open Crypto in
    ...
    let write_sorted_paths = fun hasher paths ->
      let sorted =
        List.sort
          (fun a b ->
    ...
    let write_sorted_flags = fun hasher flags ->
...
let to_json = fun action ->
  let open Data.Json in
    ...
    [
      ...
      ("flags", array (List.map string (Tusk_toolchain.Ocamlc.flags_to_string flags)));
...
let from_json = fun json ->
  let open Data.Json in
    let parse_path_list = fun field json ->
      ...
      | _ -> None
    in
    let parse_string_list = fun field json ->
      match get_field field json with
      | Some (Array arr) ->
          Some (
            List.filter_map
    ...
    | Some (String "CreateExecutable") -> (
        ...
        | Some outs, Some objects, Some libraries, Some includes, Some cclibs, Some ccopt_flags, Some cclib_flags -> Ok (CreateExecutable {
          outputs = outs;
          objects;
          libraries;
          includes;
    ...
    | Some (String "CreateSharedLibrary") -> (
        ...
        | Some outs, Some objects, Some libraries, Some includes, Some cclibs, Some ccopt_flags, Some cclib_flags -> Ok (CreateSharedLibrary {
          outputs = outs;
          objects;
          libraries;
          includes;
