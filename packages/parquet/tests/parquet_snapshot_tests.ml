...
let byte_to_hex = fun value ->
  let digit value =
    if value < 10 then
      ...
      Char.chr (Char.code 'a' + value - 10) in
...
let hex_of_string = fun value ->
  let parts = List.init ~count:(String.length value) ~fn:(fun index ->
      byte_to_hex (Char.code (String.get_unchecked value ~at:index))) in
  String.concat " " parts
...
let json_of_column_metadata = fun (value: Parquet.column_metadata) ->
  Json.Object
    [
      ...
      ("encodings", Json.Array (List.map value.encodings ~fn:(fun value -> Json.String (string_of_encoding value))));
      ("path_in_schema", Json.Array (List.map value.path_in_schema ~fn:(fun value -> Json.String value)));
      ...
      ("key_value_metadata", json_of_option (fun value -> Json.Array (List.map value ~fn:json_of_key_value)) value.key_value_metadata);
      ...
      ("encoding_stats", json_of_option (fun value -> Json.Array (List.map value ~fn:json_of_page_encoding_stats)) value.encoding_stats);
...
let json_of_column_chunk = fun (value: Parquet.column_chunk) ->
  Json.Object
    [
      ...
      ("encrypted_column_metadata", json_of_option (fun value -> Json.String value) value.encrypted_column_metadata);
    ]
...
let json_of_row_group = fun (value: Parquet.row_group) ->
  Json.Object
    [
      ...
      ("sorting_columns", json_of_option (fun value -> Json.Array (List.map value ~fn:json_of_sorting_column)) value.sorting_columns);
...
let json_of_file_metadata = fun (value: Parquet.file_metadata) ->
  Json.Object
    [
      ...
      ("column_orders", json_of_option (fun value -> Json.Array (List.map value ~fn:json_of_column_order)) value.column_orders);
    ]
