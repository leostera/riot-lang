...
let byte_to_hex = fun value ->
  ...
  in
  String.make ~len:1 ~char:(digit ((value lsr 4) land 0x0f)) ^ String.make ~len:1 ~char:(digit (value land 0x0f))

let hex_of_string = fun value ->
  let parts = List.init ~count:(String.length value) ~fn:(
    fun index -> byte_to_hex (Char.code (String.get_unchecked value ~at:index))
...
let json_of_schema_element = fun (value: Parquet.schema_element) ->
  Json.Object [
    "type", json_of_option
      (
        fun value -> Json.String (string_of_physical_type value)
      )
      value.type_;
    "type_length", json_of_option
      (
        fun value -> Json.Int value
      )
      value.type_length;
    "repetition_type", json_of_option
      (
        fun value -> Json.String (string_of_repetition_type value)
      )
      value.repetition_type;
    "name", Json.String value.name;
    "num_children", json_of_option
      (
        fun value -> Json.Int value
      )
      value.num_children;
    "converted_type", json_of_option
      (
        fun value -> Json.String (string_of_converted_type value)
      )
      value.converted_type;
    "scale", json_of_option
      (
        fun value -> Json.Int value
      )
      value.scale;
    "precision", json_of_option
      (
        fun value -> Json.Int value
      )
      value.precision;
    "field_id", json_of_option
      (
        fun value -> Json.Int value
      )
      value.field_id;
...
let json_of_column_metadata = fun (value: Parquet.column_metadata) ->
  Json.Object [
    ...
    "encodings", Json.Array (List.map value.encodings ~fn:(
      fun value -> Json.String (string_of_encoding value)
    ));
    "path_in_schema", Json.Array (List.map value.path_in_schema ~fn:(
      fun value -> Json.String value
    ));
    ...
    "key_value_metadata", json_of_option
      (
    ...
    "bloom_filter_length", json_of_option
      (
        fun value -> Json.Int value
      )
      value.bloom_filter_length;
  ]

let json_of_column_chunk = fun (value: Parquet.column_chunk) ->
  Json.Object [
    "file_path", json_of_option
      (
        fun value -> Json.String value
      )
      value.file_path;
    ...
    "offset_index_length", json_of_option
      (
        fun value -> Json.Int value
      )
      value.offset_index_length;
    "column_index_offset", json_of_option json_of_int64 value.column_index_offset;
    "column_index_length", json_of_option
      (
        fun value -> Json.Int value
      )
      value.column_index_length;
    "encrypted_column_metadata", json_of_option
      (
        fun value -> Json.String value
      )
      value.encrypted_column_metadata;
  ]
...
let json_of_row_group = fun (value: Parquet.row_group) ->
  Json.Object [
    ...
    "sorting_columns", json_of_option
      (
...
let json_of_file_metadata = fun (value: Parquet.file_metadata) ->
  Json.Object [
    ...
    "key_value_metadata", json_of_option
      ...
      value.key_value_metadata;
    "created_by", json_of_option
      (
        fun value -> Json.String value
      )
      value.created_by;
    "column_orders", json_of_option
      (
...
let snapshot_json_of_file = fun (value: Parquet.t) ->
  ...
  let footer = String.sub encoded ~offset:(String.length encoded - 8) ~len:8 |> Parquet.decode_footer_tail |> Result.expect ~msg:"snapshot file footer should decode" in
...
let test_snapshot_empty_file_layout = fun ctx -> Test.Snapshot.assert_json ~ctx ~actual:(snapshot_json_of_file empty_file)

let test_snapshot_nested_file_layout = fun ctx -> Test.Snapshot.assert_json ~ctx ~actual:(snapshot_json_of_file nested_file)

let tests = [ Test.case "parquet snapshot empty file layout" test_snapshot_empty_file_layout; Test.case "parquet snapshot nested file layout" test_snapshot_nested_file_layout ]
