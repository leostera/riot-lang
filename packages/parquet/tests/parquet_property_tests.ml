...
let io_writer_of_buffer =
  let module Write = struct
    ...
    let write = fun buffer ~from ->
      ...
      |> Result.expect ~msg:"parquet property writer should append buffer contents";
      Ok written
    ...
    let write_vectored = fun buffer ~from ->
      ...
      IO.IoVec.for_each
        ...
        ~fn:(fun chunk ->
          ...
          |> Result.expect ~msg:"parquet property writer should append slices";
...
let sorting_column_gen =
  Generator.map3
    (fun column_idx descending nulls_first -> ({ column_idx; descending; nulls_first }:
...
let page_encoding_stats_arb =
  Arbitrary.make ~print:print_page_encoding_stats page_encoding_stats_gen
  ...
  | [ row_group ] -> (
      ...
      | [ column_chunk ] -> (
          ...
          | Some column_metadata -> (
              ...
              | Some [ value ] -> value
              | _ ->
                  Property.fail "expected wrapped metadata to contain one page encoding stats entry"
            )
...
let schema_element_roundtrip_prop =
  run_property
...
let page_encoding_stats_roundtrip_prop =
  run_property
...
let column_metadata_roundtrip_prop =
  run_property
...
let column_chunk_roundtrip_prop =
  run_property
    ~examples:composite_examples
