...
let print_schema_element = fun (value: Parquet.schema_element) ->
  String.concat ""
    [
      ...
      Printer.option
        (
          fun value -> string_of_physical_type value
        )
        value.type_;
      "; type_length = ";
      ...
      Printer.option
        (
          fun value -> string_of_repetition_type value
        )
      ...
      Printer.option
        (
          fun value -> string_of_converted_type value
        )
...
let repetition_type_gen = Generator.one_of [ Generator.return Parquet.Required; Generator.return Parquet.Optional; Generator.return Parquet.Repeated ]
...
let schema_element_gen =
  Generator.map3
    ...
    (Generator.triple (Generator.option physical_type_gen) (Generator.option small_int32_gen) (Generator.option repetition_type_gen))
    (Generator.triple small_string_gen (Generator.option small_int32_gen) (Generator.option converted_type_gen))
    (Generator.triple (Generator.option small_int32_gen) (Generator.option small_int32_gen) (Generator.option small_int32_gen))
...
let sorting_column_gen =
  Generator.map3
    (
      fun column_idx descending nulls_first -> ({ column_idx; descending; nulls_first } : Parquet.sorting_column)
    )
    small_int32_gen
    Generator.bool
    Generator.bool
...
let column_metadata_gen =
  Generator.map3
    ...
    (
      Generator.map2
        (
          fun (num_values, total_uncompressed_size) (total_compressed_size, data_page_offset) -> (num_values, total_uncompressed_size, total_compressed_size, data_page_offset)
        )
    ...
    (
      Generator.map3
        (
          fun (key_value_metadata, index_page_offset) (dictionary_page_offset, encoding_stats) (bloom_filter_offset, bloom_filter_length) -> (key_value_metadata, index_page_offset, dictionary_page_offset, encoding_stats, bloom_filter_offset, bloom_filter_length)
        )
...
let column_chunk_gen =
  Generator.map3
    ...
    (Generator.pair (Generator.option small_int64_gen) (Generator.option small_int32_gen))
    (Generator.triple (Generator.option small_int64_gen) (Generator.option small_int32_gen) (Generator.option small_string_gen))
...
let file_gen =
  Generator.map2
    (
      fun body metadata -> ({ body; metadata } : Parquet.t)
    )
    body_string_gen
    file_metadata_gen
...
let file_metadata_with_key_value = fun value -> { base_metadata with key_value_metadata = Some [ value ] }

let file_metadata_with_schema_element = fun value -> { base_metadata with schema = [ base_schema_root; value ] }
...
let file_metadata_with_page_encoding_stats = fun value ->
  ...
  let row_group = { base_row_group with columns = [ column_chunk ] } in { base_metadata with row_groups = [ row_group ] }
...
let file_metadata_with_column_chunk = fun value ->
  let row_group = { base_row_group with columns = [ value ] } in { base_metadata with row_groups = [ row_group ] }
...
let key_value_roundtrip_prop = run_property "parquet property key_value roundtrips through metadata" key_value_arb (metadata_roundtrip file_metadata_with_key_value extract_only_key_value)

let schema_element_roundtrip_prop = run_property "parquet property schema_element roundtrips through metadata" schema_element_arb (metadata_roundtrip file_metadata_with_schema_element extract_only_schema_element)

let page_encoding_stats_roundtrip_prop = run_property "parquet property page_encoding_stats roundtrip through metadata" page_encoding_stats_arb (metadata_roundtrip file_metadata_with_page_encoding_stats extract_only_page_encoding_stats)

let column_metadata_roundtrip_prop = run_property ~examples:composite_examples "parquet property column_metadata roundtrips through metadata" column_metadata_arb (metadata_roundtrip file_metadata_with_column_metadata extract_only_column_metadata)

let column_chunk_roundtrip_prop = run_property ~examples:composite_examples "parquet property column_chunk roundtrips through metadata" column_chunk_arb (metadata_roundtrip file_metadata_with_column_chunk extract_only_column_chunk)

let row_group_roundtrip_prop = run_property ~examples:composite_examples "parquet property row_group roundtrips through metadata" row_group_arb (metadata_roundtrip file_metadata_with_row_group extract_only_row_group)
...
let file_roundtrip_prop = run_property ~examples:composite_examples "parquet property files roundtrip" file_arb file_roundtrip

let file_io_roundtrip_prop = run_property ~examples:composite_examples "parquet property files roundtrip over io" file_arb file_io_roundtrip
