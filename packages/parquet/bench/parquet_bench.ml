...
type fixture = { label: string; value: Parquet.t; metadata_bytes: string; encoded: string }
...
let schema_for_columns = fun column_count ->
  ...
  in
  let leaves = List.init ~count:column_count ~fn:(
    fun index -> ({
      type_ = Some (
        if Int.rem index 2 = 0 then
          Parquet.Int32
        else Parquet.Byte_array
      );
      type_length = None;
...
let column_metadata = fun index row_group_index ->
  ...
  let offset = Int64.of_int ((row_group_index * 10_000) + (index * 128)) in ({
    type_ = if Int.rem index 2 = 0 then
      Parquet.Int32
    else Parquet.Byte_array;
    encodings = [ Parquet.Plain; Parquet.Rle_dictionary ];
    path_in_schema = [ name ];
    codec = if Int.rem index 2 = 0 then
...
let row_group = fun column_count row_group_index ->
  let columns = List.init ~count:column_count ~fn:(
    fun index -> column_chunk index row_group_index
  ) in ({
    columns;
...
let build_fixture = fun (spec: fixture_spec) ->
  let metadata: Parquet.file_metadata = {
    ...
    key_value_metadata = Some [ { key = "label"; value = Some spec.label }; { key = "package"; value = Some "parquet" } ];
...
let bench_encode_writer = fun fixture () ->
  let buffer = IO.Buffer.create ~size:(String.length fixture.encoded) in ignore (Parquet.to_writer (io_writer_of_buffer buffer) fixture.value)

let bench_decode_file = fun fixture () -> ignore (Parquet.from_string fixture.encoded)

let bench_decode_reader = fun fixture () -> ignore (Parquet.from_reader (String.to_reader ~chunk_size:io_chunk_size fixture.encoded))
...
let benchmark_suite = fun fixture ->
  ...
  let metadata_size = human_size (String.length fixture.metadata_bytes) in Bench.[
    with_config ~config ("parquet encode metadata " ^ fixture.label ^ " fixture (" ^ metadata_size ^ ")") (bench_encode_metadata fixture);
    with_config ~config ("parquet decode metadata " ^ fixture.label ^ " fixture (" ^ metadata_size ^ ")") (bench_decode_metadata fixture);
    with_config ~config ("parquet encode file " ^ fixture.label ^ " fixture (" ^ file_size ^ ")") (bench_encode_file fixture);
    with_config ~config ("parquet encode writer " ^ fixture.label ^ " fixture (" ^ file_size ^ ")") (bench_encode_writer fixture);
    with_config ~config ("parquet decode file " ^ fixture.label ^ " fixture (" ^ file_size ^ ")") (bench_decode_file fixture);
    with_config ~config ("parquet decode reader " ^ fixture.label ^ " fixture (" ^ file_size ^ ")") (bench_decode_reader fixture);
  ]

let main ~args =
  let small_fixture = build_fixture small_fixture_spec in
  let large_fixture = build_fixture large_fixture_spec in
  let benchmarks = benchmark_suite small_fixture @ benchmark_suite large_fixture in Bench.Cli.main ~name:"parquet benchmarks" ~benchmarks ~args
