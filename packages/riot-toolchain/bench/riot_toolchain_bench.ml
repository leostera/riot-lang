...
let bench_compile_impl_to_string = fun () ->
  ...
  let invocation = Riot_toolchain.Ocamlc.compile_impl ocamlc ~cwd:(Path.v "/tmp/work") ~includes:[ Path.v "src"; Path.v "_build/generated"; Path.v "vendor/lib" ] ~flags:[
  ...
  ] ~output:(Path.v "foo.cmx") (Path.v "src/foo.ml") in
