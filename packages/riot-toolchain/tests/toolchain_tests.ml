...
let with_temp_dir = fun label f ->
  match Fs.with_tempdir ~prefix:("riot_toolchain_" ^ label)
    (
      fun dir -> f dir
    ) with
  | Ok result -> result
...
let target_strings = fun targets -> List.map targets ~fn:Riot_model.Target.to_string |> List.sort ~compare:String.compare

let first_distinct_target = fun host ->
  let candidates = [ "aarch64-apple-darwin"; "x86_64-unknown-linux-gnu"; "aarch64-unknown-linux-gnu" ] in candidates |> List.find ~fn:(
    fun candidate -> not (Riot_model.Target.equal host (target candidate))
  ) |> Option.map ~fn:target |> Option.expect ~msg:"expected at least one distinct target fixture"
...
let test_compile_impl_disables_no_cmi_file_by_default = fun _ctx ->
  ...
  let invocation = Riot_toolchain.Ocamlc.compile_impl ocamlc ~cwd:(Path.v "/tmp/work") ~includes:[ Path.v "src" ] ~flags:[] ~output:(Path.v "foo.cmx") (Path.v "src/foo.ml") in
  let command = Riot_toolchain.Ocamlc.to_string invocation in
  if String.contains command "-w -49" then
    Ok ()
  else Error ("expected default warning baseline to disable warning 49, got: " ^ command)

let test_compile_impl_disables_bad_module_name_for_dev_sources = fun _ctx ->
  let ocamlc = Riot_toolchain.Ocamlc.make (Path.v "/tmp/ocamlopt.opt") in
  let invocation = Riot_toolchain.Ocamlc.compile_impl ocamlc ~cwd:(Path.v "/tmp/work") ~includes:[ Path.v "examples" ] ~flags:[] ~output:(Path.v "001_hello_world.cmx") (Path.v "examples/001_hello_world.ml") in
  let command = Riot_toolchain.Ocamlc.to_string invocation in
  if String.contains command "-w -49-24" || String.contains command "-w -24-49" then
    Ok ()
  else Error ("expected dev-source warning baseline to disable warnings 49 and 24, got: " ^ command)
...
let test_compile_impl_renders_warn_error_and_raw_flags = fun _ctx ->
  let ocamlc = Riot_toolchain.Ocamlc.make (Path.v "/tmp/ocamlopt.opt") in
  let invocation = Riot_toolchain.Ocamlc.compile_impl ocamlc ~cwd:(Path.v "/tmp/work") ~includes:[ Path.v "src" ] ~flags:[ Riot_toolchain.Ocamlc.WarnError [ Riot_toolchain.Ocamlc.All ]; Riot_toolchain.Ocamlc.Raw "-O2"; Riot_toolchain.Ocamlc.Raw "-noassert" ] ~output:(Path.v "foo.cmx") (Path.v "src/foo.ml") in
  let command = Riot_toolchain.Ocamlc.to_string invocation in
  if not (String.contains command "-warn-error +a") then
    Error ("expected -warn-error +a in command, got: " ^ command)
  else
    if not (String.contains command " -O2 ") then
      Error ("expected raw -O2 flag in command, got: " ^ command)
    else
      if not (String.contains command " -noassert ") then
  ...
  | [ diagnostic ] -> (
    ...
    | Some location ->
        ...
        if not (String.equal rendered sample_ocaml_warning) then
          ...
          if not (String.equal location.path "/tmp/sandbox/pkg/src/install.ml") then
            Error ("unexpected parsed path: " ^ location.path)
          else
            if not (Riot_toolchain.Ocamlc.Diagnostic.is_warning diagnostic) then
              Error "expected parsed diagnostic to be classified as a warning"
            else Ok ()
    | None -> Error "expected parsed warning to include a location"
  )
  | diagnostics -> Error ("expected exactly one parsed warning block, got " ^ Int.to_string (List.length diagnostics))
...
let test_map_path_rewrites_rendered_diagnostic = fun _ctx ->
  ...
  | [ diagnostic ] ->
      ...
      if String.contains rendered "./packages/riot-cli/src/install.ml" then
        Ok ()
      else Error ("expected rewritten diagnostic path, got: " ^ rendered)
...
let test_unparseable_c_like_line_falls_back_to_raw = fun _ctx ->
  ...
  | [ diagnostic ] ->
      ...
      if String.equal rendered sample_unparseable_c_like_line then
        Ok ()
      else Error ("expected raw fallback to preserve the original text, got: " ^ rendered)
  | diagnostics -> Error ("expected exactly one raw diagnostic block, got " ^ Int.to_string (List.length diagnostics))
...
let test_parse_colored_ocaml_warning_diagnostic = fun _ctx ->
  ...
  | [ diagnostic ] -> (
    ...
    | Some location when String.equal location.path "/tmp/sandbox/pkg/src/install.ml" ->
        if Riot_toolchain.Ocamlc.Diagnostic.is_warning diagnostic then
          Ok ()
        else Error "expected colored diagnostic to still be classified as a warning"
  ...
  | diagnostics -> Error ("expected exactly one parsed colored warning block, got " ^ Int.to_string (List.length diagnostics))
                    ...
                    [
                      Data.Json.obj
                        [
                          ...
                          "artifact", Data.Json.string "ocaml-5.5.0-riot.4-aarch64-apple-darwin.tar.gz";
                          "artifact_url", Data.Json.string "https://cdn.pkgs.ml/ocaml/ocaml-5.5.0-riot.4-aarch64-apple-darwin.tar.gz";
                          "checksum_url", Data.Json.string "https://cdn.pkgs.ml/ocaml/ocaml-5.5.0-riot.4-aarch64-apple-darwin.tar.gz.sha256";
                      ...
                      Data.Json.obj
                        [
                          ...
                          "artifact_target", Data.Json.string "aarch64-apple-darwin-x-x86_64-unknown-linux-gnu";
                          "kind", Data.Json.string "cross";
                          "artifact", Data.Json.string "ocaml-5.5.0-riot.4-aarch64-apple-darwin-x-x86_64-unknown-linux-gnu.tar.gz";
                          "artifact_url", Data.Json.string "https://cdn.pkgs.ml/ocaml/ocaml-5.5.0-riot.4-aarch64-apple-darwin-x-x86_64-unknown-linux-gnu.tar.gz";
                          "checksum_url", Data.Json.string "https://cdn.pkgs.ml/ocaml/ocaml-5.5.0-riot.4-aarch64-apple-darwin-x-x86_64-unknown-linux-gnu.tar.gz.sha256";
            ...
            with_env_var "RIOT_OCAML_CDN_URL" (Some ("file://" ^ Path.to_string temp_dir))
              (
                fun () ->
                  ...
                  | Ok toolchains ->
                      let find_toolchain ~host ~target = List.find toolchains ~fn:(
                        fun (toolchain: Riot_toolchain.available_toolchain) -> Riot_model.Target.equal toolchain.host host && Riot_model.Target.equal toolchain.target target
                      ) in
                      ...
                      if not (Int.equal (List.length toolchains) 2) then
                        Error ("expected 2 available toolchains, got " ^ Int.to_string (List.length toolchains))
                      else
                        match find_toolchain ~host:native_host ~target:native_target, find_toolchain ~host:native_host ~target:cross_target with
                        | Some native, Some cross -> (
                          ...
                          | Riot_toolchain.Native, Riot_toolchain.Cross ->
                              if not (native.size_bytes = Some 123) then
                                ...
                                if not (String.equal cross.artifact_target "aarch64-apple-darwin-x-x86_64-unknown-linux-gnu") then
...
let test_get_host_triple_matches_std_host_triple = fun _ctx ->
  if Riot_model.Target.equal (Riot_toolchain.get_host_triple ()) Std.System.host_triple then
    Ok ()
  else Error "expected Riot_toolchain.get_host_triple to use Std.System.host_triple"
...
let test_list_toolchains_returns_typed_targets = fun _ctx ->
  ...
  let config = Riot_model.Toolchain_config.{ version = "5.5.0-riot.4"; source = Version "5.5.0-riot.4"; targets = [ host; cross ] } in
  let toolchains = Riot_toolchain.list_toolchains ~config in
  let targets = List.map toolchains ~fn:(
    fun info -> info.Riot_toolchain.target
  ) in
  let rendered = target_strings targets in
  let expected = target_strings [ host; cross ] in
  let host_info = List.find toolchains ~fn:(
    fun info -> Riot_model.Target.equal info.target host
  ) in
  let cross_info = List.find toolchains ~fn:(
    fun info -> Riot_model.Target.equal info.target cross
  ) in
