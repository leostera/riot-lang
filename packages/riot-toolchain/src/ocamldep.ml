...
let sort = fun t ~cwd ~files ->
  if files = [] then
    ...
    let cmd = "cd " ^ Path.to_string cwd ^ " && " ^ ocamldep ^ " -sort " ^ files_str ^ " 2>/dev/null" in Log.trace @@ "  $ " ^ cmd;
  let sorted_str =
  ...
  (* Return original list if ocamldep fails *)
    ...
    let sorted_basenames = String.split ~by:" " sorted_str in (* Filter out empty strings and return files in dependency order *)
    List.filter_map sorted_basenames ~fn:(
      fun s ->
        if s = "" then
          None
        else Some (Path.v s)
  ...
  if deps_str = "" then
    ...
    | [ _; deps_part ] ->
        ...
        if deps = "" then
          ...
          let result = String.split ~by:" " deps |> List.map ~fn:String.trim |> List.map ~fn:(
            fun modname -> Module_name.of_string ~namespace:package_namespace modname
          ) in Log.trace ("[OCAMLDEP] Parsed " ^ Int.to_string (List.length result) ^ " deps for " ^ file_str ^ ": " ^ deps);
        result
    | _ ->
...
let deps_with_flags = fun t ~cwd ~file ~flags ~package_namespace ->
  ...
  let cmd = "cd " ^ Path.to_string cwd ^ " && " ^ ocamldep ^ " -I . " ^ flags_str ^ " -modules " ^ file_str ^ " 2>/dev/null" in
  let deps_str =
    let command = Command.make ~args:[ "-c"; cmd ] "sh" in
    match Command.output command with
    | Ok output -> (
  ...
  if deps_str = "" then
    ...
    | [ _; deps_part ] ->
        ...
        if deps = "" then
          []
        else String.split ~by:" " deps |> List.map ~fn:String.trim |> List.map ~fn:(
          fun modname -> Module_name.of_string ~namespace:package_namespace modname
        )
    | _ -> []
...
let batch_deps = fun t ~cwd ~files ~package_namespace ->
  if files = [] then
    ...
    let ocamldep = Path.to_string t in
    let files_str = String.concat " " (List.map files ~fn:Path.to_string) in
    let cmd = "cd " ^ Path.to_string cwd ^ " && " ^ ocamldep ^ " -modules " ^ files_str in Log.trace ("[OCAMLDEP] Batch running for " ^ Int.to_string (List.length files) ^ " files");
  ...
  if output = "" then
    ...
    let lines = String.split ~by:"\n" output in List.filter_map lines ~fn:(
      fun line ->
        let trimmed = String.trim line in
        if trimmed = "" then
          None
          ...
          | [ file_part; deps_part ] ->
              ...
              let dep_list =
                if deps = "" then
                  []
                else String.split ~by:" " deps |> List.map ~fn:String.trim |> List.filter ~fn:(
...
let all_deps = fun t ~cwd ~files ~package_namespace -> List.map files ~fn:(
  fun file ->
    let deps = deps t ~cwd ~file ~package_namespace in (file, deps)
)
