...
let get_toolchain_path = fun version ->
  let host_triple = get_host_triple () in Path.(toolchain_base_dir / Path.v version / Path.v (target_to_string host_triple))

let get_toolchain_path_for_target = fun version target -> Path.(toolchain_base_dir / Path.v version / Path.v (target_to_string target))
...
let c_compiler = fun t ->
  if Riot_model.Target.equal t.target (get_host_triple ()) then
    ...
    CrossCompilingToolchain.detect ~toolchain_root:(path t) () ~target_triplet:t.target |> fun detection -> detection.c_compiler
...
let check_binaries_exist = fun toolchain ->
  ...
  | (Error err, _, _) | (_, Error err, _) | (_, _, Error err) -> Error ("Failed to check binaries: " ^ IO.error_message err)
...
let read_manifest_fingerprint = fun toolchain_path ->
  ...
  let parse_fingerprint json = let open Data.Json in
  parse_json_field_string json [ "toolchain_fingerprint" ] |> Option.or_ (parse_json_field_string json [ "fingerprint"; "value" ]) in
...
let manifest_error = fun target manifest_path ->
  let target = target_to_string target in "Missing or invalid manifest.json at " ^ manifest_path ^ " for " ^ target ^ ". Reinstall this toolchain from a republished archive that includes manifest.json " ^ "(the release artifact must include a stable toolchain_fingerprint)."
...
let ensure_manifest_present = fun toolchain_path target source ->
  ...
  | _ -> (
    ...
    | _ -> Error (manifest_error target (Path.to_string Path.(toolchain_path / Path.v "manifest.json")))
  )

let sysroot_candidates = fun ~toolchain_path ~target ->
  let target = target_to_string target in [ Path.(toolchain_path / Path.v "sysroot"); Path.(toolchain_path / Path.v ("sysroot-" ^ target)); Path.(toolchain_path / Path.v "gcc" / Path.v target / Path.v "sysroot") ]
...
let bundled_sysroot_marker_paths = fun sysroot target ->
  ...
  let glibc_marker =
    ...
    | ("aarch64", "linux") -> Some Path.(sysroot / Path.v "usr" / Path.v "include" / Path.v "aarch64-linux-gnu" / Path.v "bits" / Path.v "types" / Path.v "struct___jmp_buf_tag.h")
    | ("x86_64", "linux") -> Some Path.(sysroot / Path.v "usr" / Path.v "include" / Path.v "x86_64-linux-gnu" / Path.v "bits" / Path.v "types" / Path.v "struct___jmp_buf_tag.h")
    | _ -> None
  in
...
let missing_cross_components = fun ~toolchain_path ~target ->
  ...
  let compiler_candidates = [ Path.(toolchain_path / Path.v "bin" / Path.v (bin_prefix ^ "gcc")); Path.(toolchain_path / Path.v "gcc" / Path.v "bin" / Path.v (bin_prefix ^ "gcc")) ] in
...
let reset_toolchain_install = fun path ->
  ...
  | Ok true -> (
    ...
    | Ok () -> Ok ()
    | Error err -> Error ("Failed to remove existing toolchain at " ^ Path.to_string path ^ ": " ^ IO.error_message err)
  )
  | Ok false -> Ok ()
  | Error err -> Error ("Failed to inspect existing toolchain at " ^ Path.to_string path ^ ": " ^ IO.error_message err)
...
let download_and_install_toolchain = fun version ~host ~target ->
  ...
  let (binary_url, tar_filename, description) =
    if String.equal host_name target_name then
      ...
      let filename = "ocaml-" ^ version ^ "-" ^ host_name ^ ".tar.gz" in (url, filename, "native")
    else
      (* Cross-compilation toolchain *)
      let url = base_url ^ "/ocaml-" ^ version ^ "-" ^ host_name ^ "-x-" ^ target_name ^ ".tar.gz" in
      let filename = "ocaml-" ^ version ^ "-" ^ host_name ^ "-x-" ^ target_name ^ ".tar.gz" in (url, filename, "cross-compilation from " ^ host_name ^ " to " ^ target_name)
  in
  ...
  (* Download using curl *)
  let download_cmd = Command.make ~args:[
    "-L";
    "-o";
    Path.to_string tar_path;
    binary_url;
  ] "curl" in
  match Command.output download_cmd with
  | Error (Command.SystemError msg) -> Error ("Failed to download toolchain: " ^ msg)
  | Ok output when output.Command.status != 0 -> Error ("Failed to download toolchain from " ^ binary_url ^ "\nHTTP error or file not found")
  | Ok _ ->
      ...
      (
        ...
        | Ok () ->
            ...
            (* Extract using tar *)
            let extract_cmd = Command.make ~args:[
              "-xzf";
              Path.to_string tar_path;
              "-C";
            ...
            | Ok output when output.Command.status != 0 -> Error "Failed to extract toolchain tarball"
            | Ok _ -> (
              ...
              | Ok () ->
                  ...
                  println ("✓ OCaml " ^ version ^ " (" ^ target_name ^ ") installed successfully");
                  Ok ()
            )
      )
  ...
  | _ -> (
    ...
    | Ok true -> (
      ...
      | _ -> (
        ...
        | Error err -> Error ("Failed to create toolchain symlink from " ^ Path.to_string toolchain_path ^ " to " ^ Path.to_string abs_local ^ ": " ^ IO.error_message err)
      )
    )
    | _ ->
        ...
        (
          ...
          | Error msg ->
              let host_triple = get_host_triple () in Error ("Toolchain not found!\n\n\
...
let check_health = fun toolchain ->
  ...
  | Ok () -> (
    ...
    | Ok output when output.Command.status = 0 ->
        ...
        Ok ()
    | Ok output -> Error ("ocamlc exists but failed: exit code " ^ Int.to_string output.Command.status)
    ...
    | Path _ ->
        ...
        if not (Riot_model.Target.equal t.target (get_host_triple ())) then
          (
            ...
            | Some sysroot -> write_legacy_path_fingerprint (bundled_sysroot_marker_paths sysroot t.target)
            | None -> ()
          )
    | Version _ | Url _ -> (
      match read_manifest_fingerprint toolchain_path with
      | Some fingerprint when not (String.equal (String.trim fingerprint) "") -> Crypto.Sha256.write hasher fingerprint
      | _ -> panic ("Toolchain manifest fingerprint is required for non-local toolchains. " ^ "Install from a republished archive that includes manifest.json.")
    )
  in
...
let init_for_target = fun ~config ~target ->
  ...
  let refresh () =
    ...
    | Ok () -> (
      ...
      | Error missing -> Error ("Downloaded toolchain for " ^ target_to_string target ^ " but it is still incomplete: " ^ String.concat ", " missing)
    )
  ...
  | Ok true -> (
    ...
    | Error missing when not (Riot_model.Target.equal target host) ->
        Log.info ("Refreshing cross toolchain for " ^ target_to_string target ^ " because it is missing: " ^ String.concat ", " missing);
        refresh ()
  ...
  | _ ->
        ...
        (
          ...
          | Ok true ->
              ...
              (
                match Fs.exists toolchain_path with
                | Ok true ->
                    let toolchain = make_toolchain version source ~target in Ok toolchain
                | _ ->
                    let cwd = Env.current_dir () |> Result.expect ~msg:"Failed to get cwd" in
                    let abs_local =
                      if Path.is_absolute local_compiler then
      ...
      else (* Cross-compilation - download cross-toolchain *)
      refresh ()
...
let check_toolchain_status = fun ~version ~target ->
  ...
  | Ok false | Error _ -> NotInstalled { expected_path = toolchain_path }
  | Ok true -> (
...
let install_all_toolchains = fun ~config ->
  ...
  let host = get_host_triple () in
  let results = List.map toolchains ~fn:(
    fun info ->
  ...
  let successes = List.filter results ~fn:(
    function
    | Ok _ -> true
    | Error _ -> false
  ) in
  let failures = List.filter results ~fn:(
    function
    | Error _ -> true
    | Ok _ -> false
  ) in
  if List.length failures > 0 then
    let errors = List.filter_map results ~fn:(
      function
      | Error (target, msg) -> Some (target ^ ": " ^ msg)
      | _ -> None
    ) in Error ("Failed to install toolchains:\n  " ^ String.concat "\n  " errors)
  else
    let installed = List.filter successes ~fn:(
      function
      | Ok `Installed -> true
      | _ -> false
    ) |> List.length in
    let skipped = List.filter successes ~fn:(
      function
      | Ok `Skipped -> true
      | _ -> false
    ) |> List.length in Ok (installed, skipped)
...
let require_json_target_field = fun name json ->
  ...
  | Ok raw -> (
    ...
    | Ok target -> Ok target
    | Error msg -> Error ("Toolchain manifest field '" ^ name ^ "' must be a valid target triple: " ^ Riot_model.Target.error_message msg)
  )
    ...
    | Ok toolchains_json -> (
      ...
      | Some entries ->
          let rec loop acc = function
            | [] -> Ok (List.reverse acc |> List.sort ~compare:(
              fun left right ->
                ...
                if by_version != Order.EQ then
                  ...
                  if by_host != Order.EQ then
                    ...
                    let by_target = String.compare (Riot_model.Target.to_string left.target) (Riot_model.Target.to_string right.target) in
                    if by_target != Order.EQ then
                      by_target
                    else String.compare left.artifact_target right.artifact_target
            ))
            | entry :: rest -> (
