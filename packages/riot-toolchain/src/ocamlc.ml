...
module Diagnostic = struct
  ...
  let rec find_substring_from = fun text ~needle ~start ->
    ...
    if needle_len = 0 then
      ...
      if start + needle_len > text_len then
        None
      else
        if String.equal (String.sub text ~offset:start ~len:needle_len) needle then
          Some start
        else find_substring_from text ~needle ~start:(start + 1)
  ...
  let strip_ansi = fun line ->
    let rec loop acc idx =
      if idx >= String.length line then
        ...
        else loop (ch :: acc) (idx + 1)
    in
    loop [] 0

  let starts_with_ocaml_header = fun line ->
    let line = strip_ansi line in String.starts_with ~prefix:"File \"" line && String.contains line "\", line "
                ...
                let start_char, end_char =
                  ...
                  | Some chars_idx ->
                      ...
                      let chars_end =
                        ...
                        | None -> String.length suffix
                      in
                      let start_char = parse_int_opt (String.sub suffix ~offset:chars_start ~len:(dash_idx - chars_start)) in
                      let end_char =
                        if dash_idx < chars_end then
                          parse_int_opt (String.sub suffix ~offset:(dash_idx + 1) ~len:(chars_end - dash_idx - 1))
                        else None
                      in
                      (start_char, end_char)
                in
  ...
  let split_c_header = fun line ->
    ...
    | path :: line_no_str :: column_str :: rest when String.length path > 0 ->
        ...
        (
          ...
          | Some line_no, Some column when String.starts_with ~prefix:"warning:" trimmed_rest || String.starts_with ~prefix:"error:" trimmed_rest || String.starts_with ~prefix:"note:" trimmed_rest ->
              let suffix = String.sub line ~offset:(String.length path) ~len:(String.length line - String.length path) in Some ("", {
                path;
                line = Some line_no;
                start_char = None;
                end_char = None;
    ...
    let rec loop = function
      ...
      | line :: rest ->
          ...
          if String.starts_with ~prefix:"Warning " trimmed then
            ...
            if String.contains trimmed ": warning:" then
              (Warning, parse_bracket_code trimmed)
            else
              if String.starts_with ~prefix:"Error:" trimmed || String.contains trimmed ": error:" then
                (Error, parse_bracket_code trimmed)
              else
                if String.starts_with ~prefix:"Note:" trimmed || String.contains trimmed ": note:" then
    ...
    if String.equal trimmed "" then
      ...
      let rec loop acc current = function
        ...
        | line :: rest ->
            ...
            else loop acc (line :: current) rest
      in
      loop [] [] lines
...
let render_warning_baseline = fun warnings -> warnings |> List.map ~fn:(
  fun warning -> "-" ^ warning_code warning
) |> String.concat ""

let is_dev_source = fun source ->
  let path = Path.to_string source in String.contains path "/tests/" || String.contains path "/examples/" || String.contains path "/bench/" || String.starts_with ~prefix:"tests/" path || String.starts_with ~prefix:"examples/" path || String.starts_with ~prefix:"bench/" path
...
let warning_baseline_flags = fun source ->
  let disabled_warnings =
    ...
    else default_disabled_warnings
  in
...
let make_include_flags = fun dirs -> dirs |> List.map ~fn:(
  fun dir -> "-I " ^ dir
) |> String.concat " "
...
let build_invocation = fun t ~cwd ?cc ?(includes = []) ?(libs = []) ?(cclibs = []) ?(ccflags = []) ?(ccopt_flags = []) ?(cclib_flags = []) ?(output = None) ?(mode = Compile) ?(flags = []) sources ->
  ...
  let cclibs_str =
    if List.length cclibs > 0 then
      String.concat " " (List.map cclibs ~fn:(
        fun lib -> "-cclib " ^ Path.to_string lib
      ))
    else ""
  in
  let ccflags_str =
    if List.length ccflags > 0 then
      String.concat " " (List.map ccflags ~fn:(
        fun flag -> "-ccopt \"" ^ flag ^ "\""
      ))
    else ""
  in
  let ccopt_flags_str =
    if List.length ccopt_flags > 0 then
      String.concat " " (List.map ccopt_flags ~fn:(
        fun flag -> "-ccopt \"" ^ flag ^ "\""
      ))
    else ""
  in
  let cclib_flags_str =
    if List.length cclib_flags > 0 then
      String.concat " " (List.map cclib_flags ~fn:(
        fun flag -> "-cclib \"" ^ flag ^ "\""
      ))
    else ""
  in
  ...
  if mode = SharedLibrary then
    begin
      Log.info ("[OCAMLC] Building shared library with includes: " ^ String.concat ", " (List.map includes ~fn:Path.to_string));
...
let compile_interface = fun t ~cwd ~includes ~flags ~output source ->
  ...
  let has_impl_flag = List.any flags ~fn:(
    function
    | Impl _ -> true
    | _ -> false
  ) in
  let args =
    (((([ "-bin-annot"; "-c" ] @ warning_baseline_flags source) @ flags_to_string flags) @ List.fold_right includes_with_dot ~init:[] ~fn:(
      fun dir acc -> [ "-I"; Path.to_string dir ] @ acc
    )) @ [ "-o"; Path.to_string output ]) @ if has_impl_flag then
      []
    else [ Path.to_string source ]
  in
...
let generate_interface = fun t ~cwd ~includes ~flags ~output source ->
  let includes_with_dot = Path.v "." :: includes in
  let args = ((([ "-i" ] @ warning_baseline_flags source) @ flags_to_string flags) @ List.fold_right includes_with_dot ~init:[] ~fn:(
    fun dir acc -> [ "-I"; Path.to_string dir ] @ acc
  )) @ [ Path.to_string source ] in make_invocation ~output_mode:(WriteStdoutToFile output) ~cwd (String.concat " " ([ base_command t ] @ args))

let compile_c = fun t ~cwd ~includes ?cc ?(ccflags = []) ~output source -> build_invocation t ~cwd ?cc ~includes ~ccflags ~output:(Some output) ~mode:Compile [ Path.to_string source ]

let create_library = fun t ~cwd ~includes ~output objects -> build_invocation t ~cwd ~includes ~output:(Some output) ~mode:Library (List.map objects ~fn:Path.to_string)

let create_executable = fun t ~cwd ~includes ~output ~libs ?cc ?(cclibs = []) ?(ccopt_flags = []) ?(cclib_flags = []) objects ->
  let includes_with_dot = Path.v "." :: includes in build_invocation t ~cwd ?cc ~includes:includes_with_dot ~libs ~cclibs ~ccopt_flags ~cclib_flags ~output:(Some output) ~mode:Executable ~flags:[ LinkAll ] (List.map objects ~fn:Path.to_string)

let create_shared_library = fun t ~cwd ~includes ~output ~libs ?cc ?(cclibs = []) ?(ccopt_flags = []) ?(cclib_flags = []) objects ->
  let includes_with_dot = Path.v "." :: includes in build_invocation t ~cwd ?cc ~includes:includes_with_dot ~libs ~cclibs ~ccopt_flags ~cclib_flags ~output:(Some output) ~mode:SharedLibrary ~flags:[ LinkAll ] (List.map objects ~fn:Path.to_string)

let create_custom_executable = fun t ~cwd ~includes ~output ~libs ?cc objects ->
  let includes_with_dot = Path.v "." :: includes in build_invocation t ~cwd ?cc ~includes:includes_with_dot ~libs ~output:(Some output) ~mode:CustomExe (List.map objects ~fn:Path.to_string)

let to_string = fun invocation ->
  let env_prefix =
    match invocation.env with
    | [] -> ""
    | env -> String.concat " " (List.map env ~fn:(
...
let run = fun invocation ->
  ...
  | Ok output when output.Command.status = 0 -> (
    ...
    | WriteStdoutToFile file -> (
      ...
      | Error err -> Failed { message = "Failed to write " ^ Path.to_string file ^ ": " ^ IO.error_message err; diagnostics = [] }
    )
  )
  | Ok output -> (
    let diagnostics = Diagnostic.parse output.Command.stderr in
    match invocation.output_mode with
    | Normal -> Failed { message = "Command failed with status " ^ Int.to_string output.Command.status; diagnostics }
    | WriteStdoutToFile _ -> Failed { message = "ocamlc -i failed with exit code " ^ Int.to_string output.Command.status; diagnostics }
  )
...
let get_output = function
  ...
  | Failed { message; diagnostics } ->
      ...
      if String.equal rendered "" then
        message
      else message ^ ": " ^ rendered

let get_ocamlc_warnings = function
  | Success { diagnostics; _ } -> diagnostics |> List.filter ~fn:Diagnostic.is_warning |> List.map ~fn:Diagnostic.render
  | Failed _ -> []
