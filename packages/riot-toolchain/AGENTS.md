...
1. Keep external tool invocation centralized here instead of scattering compiler knowledge through riot.
...
9. Prepared compiler invocations should stay opaque to callers; keep execution semantics in `riot-toolchain` instead of exposing raw process execution to higher layers.
...
`timeout 30 riot build riot-toolchain`
