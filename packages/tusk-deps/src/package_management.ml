...
let target_manifest = fun ~(workspace:Tusk_model.Workspace.t) ~cwd selection scope ->
  ...
  | Workspace ->
      let path = Path.(workspace.root / Path.v "tusk.toml") in
      let* names = dependency_names_for_section ~manifest_path:path ~section:scope in
      Ok {
        path;
        dependencies = filter_dependencies_by_names names (dependency_lists_for_workspace scope workspace);
      }
  | Package package_name -> (
      ...
      with
      | Some pkg ->
          let path = Path.(pkg.path / Path.v "tusk.toml") in
          let* names = dependency_names_for_section ~manifest_path:path ~section:scope in
          Ok {
            path;
...
let lock_package_version_map = fun (lockfile: Tusk_model.Lockfile.t) ->
  List.fold_left
    (fun acc (pkg: Tusk_model.Lockfile.package) ->
...
let emit_updated_packages = fun ~(emit:event -> unit) ~(previous:Tusk_model.Lockfile.t) (
  current: Tusk_model.Lockfile.t
) ->
    ...
    (fun (pkg: Tusk_model.Lockfile.package) ->
      ...
      | Some registry, Some to_version -> (
          ...
          | Some from_version when not (String.equal from_version to_version) ->
              emit (PackageUpdated { package = pkg.id.name; from_version; to_version })
          | _ ->
              ()
        )
      | _ ->
