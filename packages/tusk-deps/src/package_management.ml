...
let error_message = function
  ...
  | RegistryPackageNotFound { package; registry } ->
      "package '" ^ package ^ "' was not found in registry '" ^ registry ^ "'"
  | RegistryVersionNotFound { package; requirement; registry } ->
      "package '" ^ package ^ "' has no release matching '" ^ requirement ^ "' in registry '" ^ registry ^ "'"
  ...
  | DependencyNotFoundInSection { path; section; dependency } ->
      "dependency '" ^ dependency ^ "' was not found in [" ^ section ^ "] of '" ^ Path.to_string path ^ "'"
...
let parse_dependency_spec = fun raw ->
  ...
  | [ name ] ->
      let* name =
        Tusk_model.Package.validate_name (String.trim name)
        |> Result.map_error (fun error -> DependencySpecInvalid { dependency = raw; error })
      in
...
let target_manifest = fun ~(workspace:Tusk_model.Workspace.t) ~cwd selection scope ->
  ...
  | Workspace ->
      Ok {
        path = Path.(workspace.root / Path.v "tusk.toml");
  ...
  | Package package_name -> (
      match workspace.packages
        ...
        |> List.find_opt (fun (pkg:Tusk_model.Package.t) -> String.equal pkg.name package_name) with
      | Some pkg ->
          Ok {
            path = Path.(pkg.path / Path.v "tusk.toml");
...
let lookup_named_package = fun ~(emit:event -> unit) ~registry parsed ->
  ...
  let* document =
    Pkgs_ml.Registry.read_package_document registry ~package_name:parsed.name
    |> Result.map_error (fun error ->
  ...
  | Some document ->
      emit (RegistryPackageLookupFinished { package = document.name; latest_version = document.latest });
...
let upsert_dependency = fun
  (dependencies:Tusk_model.Package.dependency list)
  ((dependency:Tusk_model.Package.dependency) as replacement)
  ->
  let rec loop
    (acc:Tusk_model.Package.dependency list)
    (remaining:Tusk_model.Package.dependency list)
    =
    match remaining with
    | [] -> List.rev (replacement :: acc)
    | current :: rest when String.equal current.name dependency.name ->
...
let remove_dependency = fun dependencies ~name ->
  let kept = List.filter (fun (dep:Tusk_model.Package.dependency) -> not (String.equal dep.name name)) dependencies in
...
let reload_workspace = fun ~(workspace_root:Path.t) ->
  let* workspace, load_errors =
    ...
    |> Result.map_error (fun error -> WorkspaceReloadFailed { workspace_root; error })
  in
