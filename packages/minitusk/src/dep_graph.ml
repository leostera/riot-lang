...
module Namespace: sig
  ...
  let to_string = fun t ->
    String.concat separator (List.map Module_name.to_string t)
...
module Module: sig
  ...
  let eq = fun a b ->
    String.equal a.file_path b.file_path
...
module Build_results: sig
  ...
  *)
  ...
  let has_module = fun t module_name ->
...
module Module_registry: sig
  ...
  let get = fun t node_id ->
    Hashtbl.find t.modules node_id
  ...
  let print = fun t ->
    ...
    let entries =
      Hashtbl.fold (fun node_id mod_ acc -> (node_id, mod_) :: acc) t.modules []
    in
    let sorted_entries =
      List.sort
        (fun ((_, mod1)) ((_, mod2)) ->
...
module Dependency_rules = struct
  let is_dependency_allowed = fun ~from_module ~to_module ->
    ...
    let parse_namespace = fun name ->
      String.split_on_char '_' name |> List.filter (fun s -> String.length s > 0) in
    ...
    else if List.rev from_parts |> List.tl |> List.rev = (List.rev to_parts |> List.tl |> List.rev) then
      true
    ...
    then
      ...
      let rec common_prefix = fun l1 l2 ->
        match (l1, l2) with
...
*)
  ...
  let existing_module_names = List.map (fun m -> Module.module_name m |> Module_name.to_string) file_modules
  |> List.sort_uniq String.compare in
      ...
      List.iter
        (fun dep ->
                ...
                | MLI alias_mod ->
                    ...
                    if String.ends_with ~suffix:"__Aliases" alias_name then
                      ...
                      let is_debug =
                        ...
                        || Module_name.to_string dep_name = "IO" in
                      if is_debug then
          ...
          let is_debug =
            ...
            || Module_name.to_string dep_name = "Global0" in
