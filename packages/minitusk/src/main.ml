  ...
  let get_string_value lines key =
    List.find_map (fun line ->
      if String.starts_with ~prefix:(key ^ " = ") line then
        let value = String.sub line (String.length key + 3) (String.length line - String.length key - 3) in
        let value = String.trim value in
        if String.length value >= 2 && value.[0] = '"' && value.[String.length value - 1] = '"' then
          Some (String.sub value 1 (String.length value - 2))
        else
          Some value
      else
        None
    ...
    and collect_array lines acc =
      ...
      | line :: rest ->
          ...
          if trimmed = "]" then
            ...
            collect_array rest (parse_array_item trimmed :: acc)
    and parse_array_item item =
      let item = String.trim item in
      let item = if String.ends_with ~suffix:"," item then
        String.sub item 0 (String.length item - 1)
      else item in
      let item = String.trim item in
      if String.length item >= 2 && item.[0] = '"' && item.[String.length item - 1] = '"' then
  ...
  let rec find_workspace_section = function
    ...
    | line :: rest ->
        if line = "[workspace]" then
          rest  (* Return lines after [workspace] *)
        else
          find_workspace_section rest
  in
  ...
  if Sys.file_exists toml_path then
    ...
    let rec find_package_section = function
      ...
      | line :: rest ->
          if line = "[package]" then
            ...
            find_package_section rest
    in
    let package_lines = find_package_section lines in
    let name = match Toml.get_string_value package_lines "name" with
      | Some n -> n
            ...
            let rec collect_deps acc = function
              ...
              | line :: rest ->
                  if String.length line > 0 && line.[0] = '[' then
                    ...
                    collect_deps (line :: acc) rest
            in
            collect_deps [] rest
          else
            find_dependencies_section rest
    in
    ...
    let dependencies = 
      List.filter_map (fun line ->
        ...
        if String.contains line '=' then
          let dep_name = String.trim (String.sub line 0 (String.index line '=')) in
          if dep_name <> "" then
            Some dep_name
          else
            None
...
let build_dependency_graph workspace =
  List.filter_map (fun pkg_path ->
    read_package_config pkg_path
  ) workspace.packages
...
let print_dependency_graph packages =
  ...
  List.iter (fun pkg ->
    ...
    if List.length pkg.dependencies > 0 then begin
...
let topological_sort packages =
  ...
  let pkg_map = List.fold_left (fun acc pkg ->
  ...
  List.rev !sorted |> List.filter_map (fun name ->
    List.assoc_opt name pkg_map
  )
...
let collect_sources pkg_path =
  ...
  let is_ml_file f = Filename.check_suffix f ".ml" || Filename.check_suffix f ".mli" in
  let is_c_file f = Filename.check_suffix f ".c" in
  
  let ml_sources = List.filter (fun f -> is_ml_file (Filename.basename f)) all_files in
  let c_sources = List.filter (fun f -> is_c_file (Filename.basename f)) all_files in
...
let build_package packages pkg sources c_sources outputs transitive_deps =
  ...
  Printf.printf "  Sources: %d ML, %d C\n%!" (List.length sources) (List.length c_sources);
  ...
  let has_lib_ml = List.exists (fun src -> Filename.basename src = "lib.ml") sources in
  ...
  List.iter (fun src ->
    ...
    let dest = 
      if basename = "lib.ml" then
        ...
        Filename.concat sandbox_dir basename
    in
  ...
  if not has_lib_ml && List.length sources = 1 then (
    ...
    if Filename.check_suffix basename ".ml" then (
      ...
      if List.exists (fun s -> s = mli_src) sources then (
        let cmd = Printf.sprintf "cp %s %s/%s.mli" mli_src sandbox_dir pkg.name in
        ignore (Unix.system cmd)
      )
    )
  ) else if not has_lib_ml && List.length sources = 2 then (
    (* Handle case with .ml and .mli *)
    let ml_file = List.find_opt (fun s -> Filename.check_suffix s ".ml") sources in
    let mli_file = List.find_opt (fun s -> Filename.check_suffix s ".mli") sources in
  ...
  let rec get_all_deps dep_name visited =
    if List.mem dep_name visited then visited
      ...
      | Some dep_pkg ->
          ...
          List.fold_left (fun acc d -> get_all_deps d acc) visited dep_pkg.dependencies
  in
  let all_dep_names = 
  ...
  List.iter (fun dep_name ->
    ...
    if Sys.file_exists dep_out && Sys.is_directory dep_out then (
      ...
      let cmd = Printf.sprintf "cp %s/*.cmi %s/ 2>/dev/null || true" 
        dep_out sandbox_dir in
      ignore (Unix.system cmd);
      let cmd = Printf.sprintf "cp %s/*.cma %s/ 2>/dev/null || true" 
        dep_out sandbox_dir in
      ignore (Unix.system cmd);
      (* For standalone modules without lib.ml, also copy .cmo files *)
      let cmd = Printf.sprintf "cp %s/*.cmo %s/ 2>/dev/null || true" 
        dep_out sandbox_dir in
      ignore (Unix.system cmd);
      (* Also copy C object files for packages with C stubs *)
      let cmd = Printf.sprintf "cp %s/*.o %s/ 2>/dev/null || true" 
        dep_out sandbox_dir in
      ignore (Unix.system cmd)
    )
  ) all_dep_names;
  ...
  let dep_includes = String.concat " " (List.map (fun dep -> "-I .") all_dep_names) in
  ...
  List.iter (fun c_src ->
    ...
    let cmd = Printf.sprintf "cd %s && %s -I +unix -c %s" sandbox_dir ocamlc c_file in
    Printf.printf "  $ %s\n%!" cmd;
  ...
  let sorted_ml_files = 
    if ml_files = [] then []
      ...
      (* Run ocamldep to get dependencies, using renamed files *)
      let files_str = String.concat " " (List.map (fun f -> 
        let basename = Filename.basename f in
        if basename = "lib.ml" then pkg.name ^ ".ml"
        else basename
      ) ml_files) in
      let ocamldep = Printf.sprintf "%s/.tusk/toolchains/5.3.0/bin/ocamldep" home in
      let cmd = Printf.sprintf "cd %s && %s -sort %s 2>/dev/null" sandbox_dir ocamldep files_str in
      ...
      if sorted_str = "" then ml_files
      else 
        let sorted_basenames = String.split_on_char ' ' sorted_str in
        List.filter_map (fun basename ->
          if basename = pkg.name ^ ".ml" then
            List.find_opt (fun f -> Filename.basename f = "lib.ml") ml_files
          else
            List.find_opt (fun f -> Filename.basename f = basename) ml_files
        ) sorted_basenames
    ) in
  ...
  List.iter (fun mli_src ->
    ...
    let cmd = Printf.sprintf "cd %s && %s -I +unix -I . %s -c %s" 
      sandbox_dir ocamlc dep_includes mli_file in
    Printf.printf "  $ %s\n%!" cmd;
    let ret = Unix.system cmd in
    if ret <> Unix.WEXITED 0 then (
      Printf.printf "Error: Command failed with %s\n%!" 
  ...
  List.iter (fun ml_src ->
    ...
    let c_objects = if c_sources <> [] && basename = "lib.ml" then
      String.concat " " (List.map (fun c_src ->
        let base = Filename.chop_suffix (Filename.basename c_src) ".c" in
        base ^ ".o"
      ) c_sources)
    else "" in
    let cmd = Printf.sprintf "cd %s && %s -I +unix -I . %s -c %s %s" 
      sandbox_dir ocamlc dep_includes ml_file c_objects in
    Printf.printf "  $ %s\n%!" cmd;
    let ret = Unix.system cmd in
    if ret <> Unix.WEXITED 0 then (
      Printf.printf "Error: Command failed with %s\n%!" 
  ...
  let cmo_files = List.map (fun src ->
    let basename = Filename.basename src in
    let base = 
  ...
  let c_obj_files = List.map (fun c_src ->
    let base = Filename.chop_suffix (Filename.basename c_src) ".c" in
    base ^ ".o"
  ) c_sources in
  let c_obj_list = String.concat " " c_obj_files in
  let all_objects = if c_obj_list = "" then cmo_list else cmo_list ^ " " ^ c_obj_list in
  let cmd = Printf.sprintf "cd %s && %s -a -o %s.cma %s" sandbox_dir ocamlc pkg.name all_objects in
  Printf.printf "  $ %s\n%!" cmd;
  let ret = Unix.system cmd in
  if ret <> Unix.WEXITED 0 then (
    Printf.printf "Error: Command failed with %s\n%!" 
  ...
  if c_sources <> [] then (
    List.iter (fun c_src ->
      let obj_name = (Filename.chop_suffix (Filename.basename c_src) ".c") ^ ".o" in
  ...
  let has_main_ml = List.exists (fun src -> Filename.basename src = "main.ml") sources in
  if has_main_ml then (
    (* Link all .cmo files and dependencies into executable *)
    let all_cmos = List.map (fun src ->
      let base = Filename.chop_suffix (Filename.basename src) ".ml" in
      base ^ ".cmo"
    ) sorted_ml_files in
    
    (* Get all transitive dependencies for linking *)
    let all_deps = all_dep_names in  (* Use the same list we computed for copying *)
    ...
    let sorted_deps = List.filter (fun pkg -> List.mem pkg.name all_deps) 
      (topological_sort packages) in
    ...
    let has_c_stubs = c_sources <> [] || 
      List.exists (fun dep_pkg -> 
        let (_, dep_c_sources) = collect_sources dep_pkg.path in
        dep_c_sources <> []
      ) sorted_deps in
    let custom_flag = if has_c_stubs then "-custom" else "" in
    let cmd = Printf.sprintf "cd %s && %s %s -I +unix -I . %s -o %s unix.cma %s %s" 
      sandbox_dir ocamlc custom_flag dep_includes pkg.name link_deps link_objs in
    Printf.printf "  $ %s\n%!" cmd;
    let ret = Unix.system cmd in
    if ret <> Unix.WEXITED 0 then (
      Printf.printf "Error: Command failed with %s\n%!" 
  ...
  List.iter (fun pkg ->
    ...
    let transitive_deps = 
      let rec collect pkg_name acc =
        ...
        | None -> acc
        | Some p ->
            let dep_outputs = try Hashtbl.find built_outputs p.name with Not_found -> [] in
            List.fold_left (fun acc dep ->
              collect dep (dep_outputs @ acc)
    ...
    (* Build *)
    let output_hashes = build_package packages pkg sources c_sources outputs transitive_deps in
