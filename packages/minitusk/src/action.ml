...
let print = fun action ->
  ...
  | CompileImplementation { src_file; _ } -> Printf.printf
    "  Compiling %s\n"
    (Filename.basename src_file)
...
let print_action = fun action ->
  ...
  | CompileImplementation { src_file; output; _ } -> Printf.printf
    "COMPILE_IMPL: %s -> %s\n"
    src_file
    output
  | CompileC { src_file; _ } -> Printf.printf "COMPILE_C: %s\n" src_file
  | CreateArchive { archive_name; object_files; _ } -> Printf.printf
    "BUILD_ARCHIVE: %s (%d objects)\n"
    archive_name
    (List.length object_files)
  | CreateExecutable { exe_name; main_module; archive; _ } -> Printf.printf
    "BUILD_EXECUTABLE: %s from %s + %s\n"
    exe_name
    main_module
    archive
  | SetPermissions { path; executable } -> Printf.printf
    "SET_PERMISSIONS: %s (executable=%b)\n"
    path
    executable
...
let execute_action = fun ~project_root ~package ~cc_flags ~ld_flags ~uses_stdlib ~uses_unix ~uses_dynlink action ->
  ...
  | CompileImplementation {
    ...
    is_aliases
      ...
      in
      let flags = base_flags @ stdlib_flags @ Ocaml_platform.[ Impl src_file ] @ if is_aliases then
  ...
  | CompileC { sandbox_dir; src_file } -> (
      ...
      let wrapped_cc_flags = cc_flags |> List.concat_map (fun flag -> [ "-ccopt"; flag ]) in
  ...
  | CreateArchive { sandbox_dir; archive_name; object_files; includes } -> (
      ...
      in
      let wrapped_cc_flags = cc_flags |> List.concat_map (fun flag -> [ "-ccopt"; flag ]) in
      let wrapped_ld_flags = ld_flags |> List.concat_map (fun flag -> [ "-cclib"; flag ]) in
      let extra_args = (
        if has_c_stubs then
          [ "-custom" ]
        else
  ...
  | CreateExecutable {
    ...
    dependencies
      ...
      let cmd_parts = [ Ocaml_platform.Ocamlc.ocamlc_path ] @ (
        if has_c_stubs then
          [ "-custom" ]
        else
          []
      ) @ List.concat_map (fun dir -> [ "-I"; dir ]) includes @ [ "-o"; exe_name ] @ libs @ wrapped_cc_flags @ wrapped_ld_flags @ [
        main_module
      ]
      in
...
let from_dep_graph : Dep_graph.t -> build_plan = fun dep_graph ->
  ...
  let uses_stdlib =
    ...
    || Dep_graph.Build_results.has_stdlib dep_graph.build_results dependencies in
  let uses_unix =
    Package.uses_unix dep_graph.package || Dep_graph.Build_results.has_unix dep_graph.build_results dependencies in
  let uses_dynlink =
    Package.uses_dynlink dep_graph.package
    || Dep_graph.Build_results.has_dynlink dep_graph.build_results dependencies in
