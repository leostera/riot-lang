...
let print = fun action ->
    ...
    | CompileInterface { src_file; _ } -> Printf.printf
      "  Compiling %s\n"
...
let from_dep_graph : Dep_graph.t -> build_plan = fun dep_graph ->
    ...
    let transitive_cc_flags = Dep_graph.Build_results.get_transitive_cc_flags
      dep_graph.build_results
      dependencies in
    let transitive_ld_flags = Dep_graph.Build_results.get_transitive_ld_flags
      dep_graph.build_results
      dependencies in
    ...
    let uses_unix =
      ...
      || Dep_graph.Build_results.has_unix dep_graph.build_results dependencies in
    let uses_dynlink =
