...
let scaffold_package = fun ~path ~name ~is_library ->
  ...
  let* () = Fs.create_dir_all src_dir |> Result.map_err ~fn:(
    fun _ -> "Failed to create src directory"
  )
  in
  ...
  in
  let* () = Fs.write ml_content main_ml |> Result.map_err ~fn:(
    fun _ -> "Failed to write package source file"
  )
  in
  let* () =
    match mli_content with
    | None -> Ok ()
    | Some content -> Fs.write content main_mli |> Result.map_err ~fn:(
      fun _ -> "Failed to write package interface file"
    )
  in
  let* () = Fs.write toml_content package_toml |> Result.map_err ~fn:(
    fun _ -> "Failed to write package manifest"
  ) in Ok (Path.to_string path, name)

let new_package = fun ~workspace ~path ~name ~is_library ->
  let* (created_path, created_name) = scaffold_package ~path ~name ~is_library
  in
  let* () = Manifest.add_workspace_member ~workspace ~path in Ok (created_path, created_name)
