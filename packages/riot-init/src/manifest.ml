...
let find_char_from = fun ~char ~start source ->
  let rec loop index =
    if index >= String.length source then
      ...
      if Char.equal (String.get_unchecked source ~at:index) char then
        Some index
      else loop (index + 1)
  in
  loop start

let workspace_manifest_path = fun (workspace: Workspace_manifest.t) -> Path.(workspace.root / Path.v "riot.toml")
...
let relative_member_path = fun ~(workspace:Workspace_manifest.t) path ->
  ...
  if Path.is_absolute normalized_path then
    Path.strip_prefix normalized_path ~prefix:(Path.normalize workspace.root) |> Result.map ~fn:Path.normalize |> Result.map_err ~fn:(
      fun _ -> "Package path must live under the workspace root"
    )
  else Ok (Path.normalize normalized_path)
...
let add_workspace_member_to_source = fun ~member source ->
  ...
  if String.contains source quoted_member then
    ...
    let after_members = String.sub source ~offset:close_index ~len:(String.length source - close_index) in
    let inserted_body =
      if String.is_empty (String.trim members_body) then
        "\n  " ^ quoted_member ^ ",\n"
      else
        if String.contains members_body "\n" then
...
let add_workspace_member = fun ~(workspace:Workspace_manifest.t) ~path ->
  let* relative_path = relative_member_path ~workspace path
  in
  let manifest_path = workspace_manifest_path workspace in
  let* manifest_source = Fs.read_to_string manifest_path |> Result.map_err ~fn:IO.error_message
  in
  let* updated_source = add_workspace_member_to_source ~member:(Path.to_string relative_path) manifest_source in Fs.write updated_source manifest_path |> Result.map_err ~fn:(
    fun err -> "Failed to update workspace manifest: " ^ IO.error_message err
  )
