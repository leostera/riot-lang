...
type event = Types.event =
  | WorkspaceInitializationStarted of {
      name: string;
      target_dir: Path.t;
    }
...
let command =
  ...
  |> Std.ArgParser.args
    [
      ...
      Std.ArgParser.Arg.flag "lib"
      |> Std.ArgParser.Arg.long "lib"
      |> Std.ArgParser.Arg.help "Create library package (default)";
      Std.ArgParser.Arg.flag "bin"
      |> Std.ArgParser.Arg.long "bin"
      |> Std.ArgParser.Arg.help "Create binary package";
    ]
...
let run = fun ~on_event matches ->
  ...
  let cwd =
    Env.current_dir ()
    |> Result.expect ~msg:"Cannot get current directory"
  in
