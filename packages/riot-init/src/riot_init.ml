...
(** Create README.md *)
let create_readme = fun target_dir workspace_name is_library ->
  let running_section =
    if is_library then
      ""
    else
...
(** Create Dockerfile *)
let create_dockerfile = fun target_dir workspace_name is_library ->
  let content =
    if is_library then
...
COPY --from=build /app/_build/release/*/|} ^ workspace_name ^ {| /usr/local/bin/|} ^ workspace_name ^ {|
...
ENTRYPOINT ["/usr/local/bin/|} ^ workspace_name ^ {|"]
CMD ["--help"]
|}
  in
...
let create_ci_workflow = fun target_dir ->
...
|}
  ...
  let* () =
    if is_library then
      ...
      let helper_ml_content =
        "open Std\n\n(** Return the starter greeting for " ^ workspace_name ^ ". *)\nlet hello = fun () -> \"Hello from " ^ workspace_name ^ "\"\n"
      in
      let helper_mli_content =
        "(** Return the starter greeting for " ^ workspace_name ^ ". *)\nval hello: unit -> string\n"
      in
...
let run = fun matches ->
  let open Result in
    let open ArgParser in
      ...
      let* () = create_readme target_dir validated_name is_library in
      let* () = create_dockerfile target_dir validated_name is_library in
      let* () = create_ci_workflow target_dir in
