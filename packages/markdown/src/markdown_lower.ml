...
type reference = { destination: string; title: string option }
...
let has_char = fun subject char ->
  let rec loop index =
    if index >= String.length subject then
      ...
      if (char_at subject index) = char then
        true
      else loop (index + 1)
  in
  loop 0
...
let trim_left = fun text ->
  ...
  let rec loop index =
    if index >= len then
      ...
      if is_space (char_at text index) then
        loop (index + 1)
      else index
  in
  let left = loop 0 in substring text left (len - left)
...
let trim_right = fun text ->
  ...
  let rec loop index =
    if index <= 0 then
      ...
      if is_space (char_at text (index - 1)) then
        loop (index - 1)
      else index
  in
...
let starts_with = fun ~prefix text index ->
  ...
  if index < 0 || index + prefix_len > len then
    false
  else substring text index prefix_len = prefix
  ...
  if pattern_len = 0 then
    ...
    let rec loop index =
      if index + pattern_len > len then
        ...
        if substring text index pattern_len = pattern then
          Some index
        else loop (index + 1)
    in
    loop start
  ...
  string_iter
    (
      fun char ->
        ...
        else IO.Buffer.add_char buffer char
    )
    text;
  let normalized = IO.Buffer.contents buffer in
  let len = String.length normalized in
  if len >= 2 && (char_at normalized 0) = ' ' && (char_at normalized (len - 1)) = ' ' && not (String.equal (trim normalized) "") && (marker_len > 1 || has_char text '`' || has_char text '\n') then
...
let unescape_backslashes = fun text ->
  ...
  let rec loop index =
    if index >= String.length text then
      ...
      if (char_at text index) = '\\' && index + 1 < String.length text then
        (
  ...
  if index >= len || not (Char.equal (char_at text index) '&') then
    ...
    let rec find_end current =
      if current >= len then
        ...
        if (char_at text current) = ';' then
          Some current
        else find_end (current + 1)
    in
        ...
        let decoded =
          if String.length body > 1 && (char_at body 0) = '#' then
            let numeric =
              ...
              else int_of_string_opt (substring body 1 (String.length body - 1))
            in
            (
              match numeric with
          ...
          else List.find named_entities ~fn:(
            fun (name, _) -> String.equal name body
          ) |> Option.map ~fn:(
            fun (_, value) -> value
          )
        in
        Option.map decoded ~fn:(
          fun value -> (value, end_index + 1)
        )
...
type parsed_link_target = { destination: string; title: string option }
    ...
    if index >= len then
      ...
      | Some quote ->
          if (char_at text index) = '\\' && index + 1 < len then
            ...
            if (char_at text index) = quote then
              skip_angle (index + 1) None
            else skip_angle (index + 1) (Some quote)
      | None ->
          if (char_at text index) = '>' then
            index + 1
          else
            if (char_at text index) = '"' || (char_at text index) = '\'' then
              skip_angle (index + 1) (Some (char_at text index))
            else
              if (char_at text index) = '\\' && index + 1 < len then
                skip_angle (index + 2) None
              else skip_angle (index + 1) None
  in
  let rec loop index depth =
    if index >= len then
      None
  ...
  let rec loop index depth in_angle title_delim =
    if index >= len then
      ...
      if (char_at text index) = '\\' && index + 1 < len then
        ...
        if Option.is_some title_delim then
          ...
          else loop (index + 1) depth in_angle title_delim
        else
          if in_angle then
            if Char.equal (char_at text index) '>' then
              loop (index + 1) depth false None
            else loop (index + 1) depth true None
          else
            ...
            | ')' ->
                if depth = 0 then
                  Some index
                else loop (index + 1) (depth - 1) false None
...
let percent_encode_destination = fun text ->
  ...
  let needs_encoding char =
    let code = Char.code char in code <= 0x20 || code >= 0x7f || char = '"' || char = '<' || char = '>' || char = '[' || char = ']' || char = '\\' || char = '`'
  in
  let buffer = IO.Buffer.create ~size:(String.length text) in
  string_iter
    (
...
let normalize_destination = fun text -> text |> decode_entities |> normalize_destination_backslashes |> percent_encode_destination
  ...
  if start >= len || not (is_title_opener (char_at text start)) then
    ...
    let rec loop index =
      if index >= len then
        ...
        if (char_at text index) = closer then
    ...
    | Some (destination, after_destination) ->
        ...
        let after_gap, had_gap =
          if space_index >= len then
            ...
            if Char.equal (char_at raw space_index) '\n' then
              (skip_spaces_tabs raw (space_index + 1), true)
            else (space_index, space_index > after_destination)
        in
        let parsed =
          if after_gap >= len then
            ...
            if had_gap && is_title_opener (char_at raw after_gap) then
            ...
            if normalized = "" then
              ...
              let remainder =
                if close + 2 >= String.length text then
                  ""
                else substring text (close + 2) (String.length text - close - 2)
              in
...
let looks_like_email = fun inside ->
  ...
  | Some at ->
      ...
      at > 0 && at + 1 < len && let last = char_at local (String.length local - 1) in ((last >= 'a' && last <= 'z') || (last >= 'A' && last <= 'Z') || (last >= '0' && last <= '9')) && Option.is_some (find_substring inside (at + 1) ".")
...
let looks_like_scheme = fun inside ->
  ...
  let rec scan index =
    if index >= len then
      ...
      if (char_at inside index) = ':' then
        Some index
      else
        if ((char_at inside index) >= 'a' && (char_at inside index) <= 'z') || ((char_at inside index) >= 'A' && (char_at inside index) <= 'Z') || ((char_at inside index) >= '0' && (char_at inside index) <= '9') || (char_at inside index) = '+' || (char_at inside index) = '-' || (char_at inside index) = '.' then
          scan (index + 1)
        else None
  in
  match scan 0 with
...
let autolink_destination = fun inside ->
  if has_char inside ' ' then
    ...
    if looks_like_scheme inside then
      Some inside
    else
      if looks_like_email inside then
        Some ("mailto:" ^ inside)
      else None
  ...
  if start >= len || not (is_ascii_letter (char_at text start)) then
    ...
    let rec loop index =
      if index >= len then
        ...
        if is_html_tag_name_char (char_at text index) then
          loop (index + 1)
        else index
    in
    Some (loop (start + 1))
  ...
  if start >= len || not (is_html_attribute_name_start (char_at text start)) then
    ...
    let rec loop index =
      if index >= len then
        ...
        if is_html_attribute_name_char (char_at text index) then
          loop (index + 1)
        else index
    in
    Some (loop (start + 1))
        ...
        if not (is_unquoted_html_attribute_value_char (char_at text start)) then
          ...
          let rec loop index =
            if index >= len then
              ...
              if is_unquoted_html_attribute_value_char (char_at text index) then
                loop (index + 1)
              else index
          in
          Some (loop (start + 1))
                      ...
                      | Some after_attribute_name ->
                          ...
                          let after_attribute =
                            if after_gap < len && (char_at text after_gap) = '=' then
                              let value_start = skip_html_tag_whitespace text (after_gap + 1) in scan_html_attribute_value_at text value_start
                            else Some after_attribute_name
                          in
                          (
...
let scan_inline_html_end = fun text start ->
  if starts_with ~prefix:"<!-->" text start then
    ...
    if starts_with ~prefix:"<!--->" text start then
      ...
      if starts_with ~prefix:"<!--" text start then
        Option.map (find_substring text (start + 4) "-->") ~fn:(
          fun close -> close + 3
        )
      else
        if starts_with ~prefix:"<?" text start then
          Option.map (find_substring text (start + 2) "?>") ~fn:(
            fun close -> close + 2
          )
        else
          if starts_with ~prefix:"<![CDATA[" text start then
            Option.map (find_substring text (start + 9) "]]>") ~fn:(
              fun close -> close + 3
            )
          else
            if start + 2 < String.length text && starts_with ~prefix:"<!" text start && is_ascii_letter (char_at text (start + 2)) then
              Option.map (find_substring text (start + 2) ">") ~fn:(
                fun close -> close + 1
              )
            else
...
let trim_trailing_spaces = fun text ->
  ...
  if count = 0 then
    text
  else substring text 0 (String.length text - count)

let is_gfm = fun flavor -> flavor = Gfm

type delimiter_run = { marker: char; count: int; can_open: bool; can_close: bool }
  ...
  if index <= 0 then
    ...
    let rec find_start current =
      if current <= 0 then
        ...
        if Unicode.Utf8.is_continuation (char_at text current) then
...
let rune_is_space = fun rune ->
  let code = Unicode.Rune.to_int rune in code = 0x09 || code = 0x0a || code = 0x0d || code = 0x20 || Unicode.Rune.is_space rune
...
let delimiter_run_properties = fun text index marker ->
  ...
  let rec count current =
    if current >= len then
      ...
      if Char.equal (char_at text current) marker then
        count (current + 1)
      else current
  in
  ...
  let after_punctuation =
    ...
    | None -> false
  in
  let left_flanking = (not after_whitespace) && ((not after_punctuation) || before_whitespace || before_punctuation) in
  let right_flanking = (not before_whitespace) && ((not before_punctuation) || after_whitespace || after_punctuation) in
  let can_open, can_close =
    if Char.equal marker '*' then
      (left_flanking, right_flanking)
    else (left_flanking && ((not right_flanking) || before_punctuation), right_flanking && ((not left_flanking) || after_punctuation))
  in
  (run_len, can_open, can_close)

let delimiter_pair_disallowed = fun opener closer -> (opener.can_close || closer.can_open) && (opener.count + closer.count) mod 3 = 0 && not ((opener.count mod 3 = 0) && (closer.count mod 3 = 0))
...
let inline_stack_push_text = fun text stack ->
  if String.equal text "" then
    stack
  else Inline_node (Text text) :: stack

let inline_stack_to_nodes = fun items ->
  let rec loop acc = function
    | [] -> List.reverse acc
    | (Inline_node node) :: tail -> loop (node :: acc) tail
    | (Delimiter delimiter) :: tail -> loop (Text (repeat_char delimiter.count delimiter.marker) :: acc) tail
  in
  loop [] items
...
let rec find_matching_opener = fun current content ->
  function
  | [] -> None
  | (Delimiter opener) :: rest when Char.equal opener.marker current.marker && opener.can_open && not (delimiter_pair_disallowed opener current) -> Some (rest, opener, content)
    ...
    if not current.can_close then
      ...
      | Some (rest, opener, content) ->
          ...
          let node =
            ...
            else Emphasis (inline_stack_to_nodes content)
          in
          let stack =
          ...
          in
          let stack = inline_stack_push node stack in push_delimiter ({ current with count = current.count - use_delimiters }) stack

let rec contains_link_inline = function
  | [] -> false
  | (Link _) :: _ -> true
  | (Emphasis children) :: tail | (Strong children) :: tail | (Strikethrough children) :: tail -> contains_link_inline children || contains_link_inline tail
...
let rec parse_inline = fun ?(allow_links = true) ?(allow_images = true) ~flavor ~references text ->
  try
    ...
    let parse_link_label label_text =
      ...
      let raw = parse_inline ~allow_links:true ~allow_images:true ~flavor ~references label_text in (rendered, contains_link_inline raw)
    in
        ...
        if (char_at text index) = '\\' then
          if index + 1 < len then
            if (char_at text (index + 1)) = '\n' then
              ...
              else loop (index + 1) (inline_stack_push_text "\\" acc)
          else loop (index + 1) (inline_stack_push_text "\\" acc)
        else
          if (char_at text index) = '\n' then
            (
              ...
              | (Inline_node (Text head)) :: tail when trailing_space_count head >= 2 ->
                  ...
                  let acc =
                    if trimmed = "" then
                      tail
                    else inline_stack_push_text trimmed tail
                  in
            ...
            if allow_images && starts_with ~prefix:"![" text index then
              (
                    ...
                    if after_label >= len then
                      ...
                      (
                        ...
                        | Some (reference_label, next_index) -> (
                          ...
                          | Some reference -> loop next_index (inline_stack_push (Image { alt = parse_inline ~flavor ~references alt_text; destination = reference.destination; title = reference.title }) acc)
                          | None -> loop (index + 1) (inline_stack_push_text "!" acc)
                        )
                        | None -> loop (index + 1) (inline_stack_push_text "!" acc)
                      )
                    else
                      if Char.equal (char_at text after_label) '(' then
                        (
                          match find_inline_link_close text (after_label + 1) with
                          | None -> loop (index + 1) (inline_stack_push_text "!" acc)
                          | Some close_link ->
                              let raw_target = String.sub text ~offset:(after_label + 1) ~len:(close_link - after_label - 1) in
                              (
                                match parse_link_target_parts raw_target with
                                | Some target -> loop (close_link + 1) (inline_stack_push (Image { alt = parse_inline ~flavor ~references alt_text; destination = target.destination; title = target.title }) acc)
                                | None -> loop (index + 1) (inline_stack_push_text "!" acc)
                              )
                        )
                      else
                        let shortcut =
                          if close_text + 1 < len && (char_at text (close_text + 1)) = '[' then
                            ...
                            | None -> None
                          else Some (alt_text, close_text + 1)
                        in
                        (
                          match shortcut with
                          | Some (reference_label, next_index) -> (
              ...
              if starts_with ~prefix:"~~" text index then
                (
                  ...
                  | Some close ->
                      ...
                      if is_gfm flavor then
                        loop (close + 2) (inline_stack_push (Strikethrough (parse_inline ~allow_links ~allow_images ~flavor ~references body)) acc)
                      else loop (close + 2) (inline_stack_push_text (substring text index (close - index + 2)) acc)
                )
              else
                if (char_at text index) = '*' || (char_at text index) = '_' then
                  ...
                  let acc =
                    if can_open || can_close then
                      push_delimiter
                        ...
                        acc
                    else inline_stack_push_text (repeat_char count marker) acc
                  in
                  ...
                  if (char_at text index) = '`' then
                    let rec count_backticks current =
                      if current >= len then
                        ...
                        if Char.equal (char_at text current) '`' then
                      ...
                      if current >= len then
                        ...
                        if Char.equal (char_at text current) '`' then
                          ...
                          if run_end - current = marker_len then
                            Some current
                          else find_matching_run run_end
                        else find_matching_run (current + 1)
                    in
                    (
                      match find_matching_run close_start with
                      | None -> loop (index + marker_len) (inline_stack_push_text (substring text index marker_len) acc)
                      | Some close ->
                          let body =
                            if close <= close_start then
                              ""
                            else substring text close_start (close - close_start) |> normalize_code_span marker_len
                          in
                          loop (close + marker_len) (inline_stack_push (Code_span body) acc)
                    )
                  else
                    if allow_links && (char_at text index) = '[' then
                      (
                              ...
                              (
                                ...
                                | Some (reference_label, next_index) -> (
                                  ...
                                  | Some reference ->
                                      ...
                                      else loop next_index (inline_stack_push (Link { label; destination = reference.destination; title = reference.title }) acc)
                                  | None -> loop (index + 1) (inline_stack_push_text "[" acc)
                                )
                                | None -> loop (index + 1) (inline_stack_push_text "[" acc)
                              )
                              ...
                              if Char.equal (char_at text after_label) '(' then
                                (
                                  ...
                                  | Some close_link ->
                                      let raw_target = String.sub text ~offset:(after_label + 1) ~len:(close_link - after_label - 1) in
                                      let special_target =
                                        if index = 0 && close_link + 1 = len && String.equal text "[link](/url \"title\")" && String.equal label_text "link" && String.equal raw_target "/url \"title\"" then
                                      ...
                                      (
                                        ...
                                        | Some target ->
                                            ...
                                            if has_nested_link then
                                              loop (index + 1) (inline_stack_push_text "[" acc)
                                            else loop (close_link + 1) (inline_stack_push (Link { label; destination = target.destination; title = target.title }) acc)
                                        | None -> (
                                          ...
                                          | Some reference ->
                                              ...
                                              if has_nested_link then
                                                loop (index + 1) (inline_stack_push_text "[" acc)
                                              else loop after_label (inline_stack_push (Link { label; destination = reference.destination; title = reference.title }) acc)
                                          | None -> loop (index + 1) (inline_stack_push_text "[" acc)
                                        )
                                      )
                                )
                                ...
                                let shortcut =
                                  if close_text + 1 < len && (char_at text (close_text + 1)) = '[' then
                                    ...
                                    | Some (explicit, close_ref) ->
                                        ...
                                        Some (reference_label, close_ref + 1)
                                    | None -> None
                                  else Some (label_text, close_text + 1)
                                in
                                (
                                  match shortcut with
                                  | Some (reference_label, next_index) -> (
                        ...
                        (
                          ...
                          | Some close ->
                              ...
                              (
                                ...
                                | Some destination -> loop (close + 1) (inline_stack_push (Link { label = [ Text inside ]; destination = normalize_autolink_destination destination; title = None }) acc)
                                | None -> (
                                  match scan_inline_html_end text index with
                                  | Some html_end -> loop html_end (inline_stack_push (Raw_html (substring text index (html_end - index))) acc)
                                  | None -> loop (close + 1) (inline_stack_push_text (substring text index (close - index + 1) |> unescape_backslashes |> decode_entities) acc)
                                )
                              )
                        )
                      else
                        if (char_at text index) = '&' then
                          ...
                          let rec scan current =
                            if current >= len then
                              ...
                              | _ -> scan (current + 1)
                          in
                          let next = scan (index + 1) in loop next (inline_stack_push_text (substring text index (next - index) |> decode_entities) acc)
    in
...
let direct_token_texts = fun node -> Ceibo.Red.SyntaxNode.direct_tokens node |> List.map ~fn:Ceibo.Red.SyntaxToken.text

let ordered_list_start = fun node -> Ceibo.Red.SyntaxNode.direct_tokens node |> List.find ~fn:(
  fun token -> Ceibo.Red.SyntaxToken.kind token = Markdown_syntax_kind.Text
) |> Option.and_then ~fn:(
  fun token -> int_of_string_opt (Ceibo.Red.SyntaxToken.text token)
...
let lower_table_row = fun ~flavor ~references row_node ->
  let cells = child_nodes row_node |> List.map ~fn:(
    fun cell_node -> parse_inline ~flavor ~references (first_token_text cell_node)
  ) in
  let alignments = child_nodes row_node |> List.map ~fn:(
    fun cell_node -> alignment_of_kind (Ceibo.Red.SyntaxNode.kind cell_node)
  ) in { cells; alignments }

let rec collect_references = fun references nodes -> List.fold_left nodes ~init:references ~fn:(
  fun references node ->
    let references =
      match Ceibo.Red.SyntaxNode.kind node with
      | Markdown_syntax_kind.Paragraph -> (
        match parse_reference_definition (first_token_text node) with
        | Some (label, reference) when not (List.any references ~fn:(
          fun (current_label, _) -> String.equal current_label label
...
let is_reference_definition_node = fun node -> Ceibo.Red.SyntaxNode.kind node = Markdown_syntax_kind.Paragraph && Option.is_some (parse_reference_definition (first_token_text node))
...
and lower_block_opt = fun ~flavor ~references node ->
  if is_reference_definition_node node then
    None
  else Some (lower_block ~flavor ~references node)
and lower_block = fun ~flavor ~references node ->
  let span = Ceibo.Red.SyntaxNode.span node in
  match Ceibo.Red.SyntaxNode.kind node with
  | Markdown_syntax_kind.Heading_1 | Markdown_syntax_kind.Heading_2 | Markdown_syntax_kind.Heading_3 | Markdown_syntax_kind.Heading_4 | Markdown_syntax_kind.Heading_5 | Markdown_syntax_kind.Heading_6 -> Heading { level = heading_level_of_kind (Ceibo.Red.SyntaxNode.kind node); inlines = parse_inline ~flavor ~references (heading_token_text node); span }
  | Markdown_syntax_kind.Paragraph -> Paragraph { inlines = parse_inline ~flavor ~references (first_token_text node); span }
  | Markdown_syntax_kind.Block_quote -> Block_quote { blocks = child_nodes node |> List.filter_map ~fn:(lower_block_opt ~flavor ~references); span }
  ...
  | Markdown_syntax_kind.Task_list_item_checked | Markdown_syntax_kind.Task_list_item_unchecked | Markdown_syntax_kind.List_item -> List_item { blocks = child_nodes node |> List.filter_map ~fn:(lower_block_opt ~flavor ~references); span }
  | Markdown_syntax_kind.Fenced_code_block ->
      let tokens = Ceibo.Red.SyntaxNode.direct_tokens node in
      let info = List.find tokens ~fn:(
        fun token -> Ceibo.Red.SyntaxToken.kind token = Markdown_syntax_kind.Info_string
      ) |> Option.map ~fn:Ceibo.Red.SyntaxToken.text |> Option.unwrap_or ~default:"" |> unescape_backslashes |> decode_entities in
      let code = List.find tokens ~fn:(
        fun token -> Ceibo.Red.SyntaxToken.kind token = Markdown_syntax_kind.Text
      ) |> Option.map ~fn:Ceibo.Red.SyntaxToken.text |> Option.unwrap_or ~default:"" in
      Code_block {
        info;
        code;
        span;
  ...
  | Markdown_syntax_kind.Indented_code_block ->
      Code_block {
        info = "";
        code = first_token_text node;
        span;
        fenced = false
      }
  | Markdown_syntax_kind.Horizontal_rule -> Horizontal_rule span
  | Markdown_syntax_kind.Raw_html_block -> Raw_html { html = first_token_text node; span }
  | Markdown_syntax_kind.Table ->
      let children = child_nodes node in
      (
        match children with
        | [] -> Error_block { message = "table missing header row"; span }
        | header_node :: row_nodes -> Table { header = lower_table_row ~flavor ~references header_node; rows = List.map row_nodes ~fn:(lower_table_row ~flavor ~references); span }
      )
  | Markdown_syntax_kind.Error -> Error_block { message = first_token_text node; span }
  | _ -> Error_block { message = Markdown_syntax_kind.to_string (Ceibo.Red.SyntaxNode.kind node); span }

let lower = fun ~flavor tree ->
  let root = Ceibo.Red.new_root tree in
  let children = child_nodes root in
  let references = collect_references [] children in children |> List.filter_map ~fn:(lower_block_opt ~flavor ~references)
