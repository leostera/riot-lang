...
let normalize_destination = fun text ->
  text
  |> decode_entities
  |> normalize_destination_backslashes
  |> percent_encode_destination
  ...
  if start >= len || not (is_title_opener (char_at text start)) then
    ...
    let rec loop index =
      if index >= len then
        ...
        Some (IO.Buffer.contents buffer
        |> decode_entities, index + 1)
      else if (char_at text index) = '\\' && index + 1 < len then
        if
...
let parse_reference_definition = fun text ->
  try
    if String.length text = 0 || not (Char.equal (char_at text 0) '[') then
      ...
      | Some (label, close) ->
          if
            close + 1 >= String.length text || not (Char.equal (char_at text (close + 1)) ':')
          then
            None
          else
...
let inline_stack_to_nodes = fun items ->
  let rec loop acc = function
    ...
    | (Delimiter delimiter) :: tail ->
        loop (Text (repeat_char delimiter.count delimiter.marker) :: acc) tail
  in
  loop [] items
...
let rec parse_inline = fun ?(allow_links = true) ?(allow_images = true) ~flavor ~references text ->
  try
    ...
    let parse_link_label label_text =
      let rendered =
        parse_inline ~allow_links:false ~allow_images:true ~flavor ~references label_text
      in
      ...
      if index >= len then
          ...
          | Some close ->
              let body =
                if close <= close_start then
                  ...
                  substring text close_start (close - close_start)
                  |> normalize_code_span marker_len
              in
        ...
        let rec scan current =
          if current >= len then
            ...
            | '!' when allow_images && current + 1 < len && (char_at text (current + 1)) = '[' ->
                current
            | _ -> scan (current + 1)
        in
        ...
        loop
          ...
          (
            inline_stack_push_text
              (
                substring text index (next - index)
                |> decode_entities
              )
              acc
          )
...
let lower_table_row = fun ~flavor ~references row_node ->
  let cells =
    ...
    |> List.map ~fn:(fun cell_node -> parse_inline ~flavor ~references (first_token_text cell_node))
  in
  let alignments =
    child_nodes row_node
...
and lower_block = fun ~flavor ~references node ->
  ...
  | Markdown_syntax_kind.Ordered_list_tight ->
      List {
        ...
        items =
          child_nodes node
          |> List.map ~fn:(lower_list_item ~flavor ~references);
        span;
      }
