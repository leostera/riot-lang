  ...
  let rec loop nodes =
    ...
    let rec append acc =
      ...
      | head :: tail ->
          (
            match head with
            | Text text -> IO.Buffer.add_string buffer (escape_html text)
            | Emphasis children ->
            ...
            | Code_span text ->
                ...
                IO.Buffer.add_string buffer "</code>"
            | Hard_break -> IO.Buffer.add_string buffer "<br />\n"
            | Raw_html html -> IO.Buffer.add_string buffer html
