...
let make_token = fun ~kind ~start ~end_ ~text -> { Markdown_token.kind; span = Ceibo.Span.make ~start ~end_; text }
    ...
    if index >= length then
      let acc =
        if line_start < length then
          let text = String.sub source ~offset:line_start ~len:(length - line_start) in make_token ~kind:Markdown_token.Line_text ~start:line_start ~end_:length ~text :: acc
        else acc
      in
      List.reverse (make_token ~kind:Markdown_token.EOF ~start:length ~end_:length ~text:"" :: acc)
    else
      if String.get_unchecked source ~at:index = '\n' then
        let text = String.sub source ~offset:line_start ~len:(index - line_start) in
        let acc = make_token ~kind:Markdown_token.Newline ~start:index ~end_:(index + 1) ~text:"\n" :: make_token ~kind:Markdown_token.Line_text ~start:line_start ~end_:index ~text :: acc in loop (index + 1) (index + 1) acc
      else loop (index + 1) line_start acc
  in
  loop 0 0 []
