...
let unsupported_feature = fun ~found ~feature ~span -> make ~kind:(Unsupported_feature { found; feature }) ~span

let unclosed_fenced_code_block = fun ~found ~opener ~span -> make ~kind:(Unclosed_fenced_code_block { found; opener }) ~span

let unexpected_control_character = fun ~found ~code ~span -> make ~kind:(Unexpected_control_character { found; code }) ~span
...
let found_token = fun diag ->
  ...
  | Invalid_markdown { found } | Unsupported_feature { found; _ } | Unclosed_fenced_code_block { found; _ } | Unexpected_control_character { found; _ } | Parser_internal { found; _ } -> found
...
let main_message = fun diag ->
  ...
  | Unsupported_feature { found; feature } -> "Unsupported markdown feature `" ^ feature ^ "` at " ^ found.text
  | Unclosed_fenced_code_block { found; opener } -> "Unclosed fenced code block started with " ^ opener ^ " at " ^ found.text
  | Unexpected_control_character { found; code } -> "Unexpected control character U+" ^ Int.to_string code ^ " (" ^ found.text ^ ")"
    ...
    | Invalid_markdown _ ->
        Data.Json.obj
          [
            ...
            "found", Data.Json.obj
              [
                "kind", Data.Json.string found.kind;
                "text", Data.Json.string found.text;
              ];
          ]
