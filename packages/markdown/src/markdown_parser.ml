...
type table_row = { cells: inline_node list list; alignments: table_alignment list }
...
let string_all_equal = fun char text ->
  let rec loop index =
    if index >= String.length text then
      ...
      if Char.equal (char_at text index) char then
        loop (index + 1)
      else false
  in
  loop 0
...
let line_is_blank = fun line ->
  ...
  let rec loop index =
    if index >= len then
      ...
      if is_space (char_at line index) then
        loop (index + 1)
      else false
  in
  loop 0
...
let strip_columns_from = fun text offset start_column drop_columns ->
  ...
  let remainder value_offset =
    if value_offset >= len then
      ""
    else substring text value_offset (len - value_offset)
  in
...
let consume_list_padding = fun text offset column ->
  ...
  let rec loop index current_column =
    if index >= len then
      ...
      | '\t' ->
          let next_column = next_tab_stop current_column in loop (index + 1) next_column
  ...
  if at_end then
    ...
    if padding_width <= 4 then
      (padding_end, padding_column)
    else
      let content_offset, _ = consume_one_following_space text offset column in (content_offset, column + 1)
      ...
      let rec loop index depth consumed =
        if index >= len then
          ...
          | ')' ->
              ...
              else loop (index + 1) (depth - 1) true
        ...
        if close + 1 >= len || not (Char.equal (char_at text (close + 1)) ':') then
          ...
          let destination_start =
            if destination_start >= len then
              ...
              if Char.equal (char_at text destination_start) '\n' then
                skip_spaces_tabs text (destination_start + 1)
              else destination_start
          in
          ...
          | Some after_destination ->
              ...
              let title_end after_title =
                ...
                if after_spaces >= len then
                  ...
                  if Char.equal (char_at text after_spaces) '\n' then
                    Some (trim_trailing_reference_end text after_title)
                  else None
              in
              if space_index >= len then
                ...
                if Char.equal (char_at text space_index) '\n' then
                  ...
                  if space_index > after_destination then
  ...
  if start >= Array.length lines then
    ...
    let normalize_reference_line text =
      ...
      if indent >= String.length text then
        ""
      else substring text indent (String.length text - indent)
    in
    let line_count_of_prefix text end_index =
      let rec loop index count =
        if index >= end_index then
          ...
          if Char.equal (char_at text index) '\n' then
...
let remove_prefix = fun text count ->
  if count >= String.length text then
    ""
  else substring text count (String.length text - count)
  ...
  let rec loop index diags =
    if index >= len then
      ...
      if Char.code char < 32 && char_not char '\t' && char_not char '\n' && char_not char '\r' then
        ...
        let diag = unexpected_control_character ~found ~code:(Char.code char) ~span:(make_span ~start:index ~len:1) in loop (index + 1) (diag :: diags)
      else loop (index + 1) diags
  in
  loop 0 []
...
let parse_heading = fun text ->
  ...
  if level = 0 then
    ...
    if after_hash >= len then
      Some (level, "")
    else
      if char_not (char_at text after_hash) ' ' && char_not (char_at text after_hash) '\t' then
        None
      else
        let raw = substring text (after_hash + 1) (len - after_hash - 1) |> trim_right in
        ...
        if raw_len = 0 then
          ...
          let rec trailing_hashes index count =
            if index < 0 then
              ...
              if Char.equal (char_at raw index) '#' then
          ...
          let content =
            if hash_count = 0 then
              ...
              if before_hashes < 0 then
                ""
              else
                if is_space (char_at raw before_hashes) then
                  substring raw 0 before_hashes |> trim_right |> trim
                else trim raw
          in
          Some (level, content)
      ...
      if char_not marker '*' && char_not marker '-' && char_not marker '_' then
        ...
        let rec loop index =
          if index >= len then
            ...
            if (char_at line index) = marker || is_space (char_at line index) then
              loop (index + 1)
            else false
        in
        if loop 0 then
  ...
  if indent > 3 then
    ...
    if len = 0 then
      ...
      if char_not marker '=' && char_not marker '-' then
        ...
        if string_all_equal marker line then
          if marker = '=' then
            Some 1
          else Some 2
        else None
    ...
    if char_not marker '`' && char_not marker '~' then
      ...
      let rec count index =
        if index >= len then
          ...
          if (char_at text index) = marker then
            count (index + 1)
          else index
      in
      ...
      if marker_len < 3 then
        ...
        let info =
          trim (remove_prefix text after) |> fun value ->
            let rec scan index =
              if index >= String.length value then
                index
              else
                if (char_at value index) = ' ' || (char_at value index) = '\t' then
                  index
                else scan (index + 1)
            in
            ...
            if word_len <= 0 then
              ""
            else substring value 0 word_len
        in
        if marker = '`' && has_char (trim (remove_prefix text after)) '`' then
          None
        else Some (marker, marker_len, indent_columns, info)
  ...
  if String.length remaining < marker_len then
    ...
    let rec count index =
      if index >= String.length remaining then
        ...
        if (char_at remaining index) = marker then
          count (index + 1)
        else index
    in
    let actual_len = count 0 in actual_len >= marker_len && trim_left (remove_prefix remaining actual_len) = ""
  ...
  if start >= Array.length lines then
    ...
    | Some (marker, marker_len, opener_indent, info) ->
        ...
        let strip_content_indent text =
          let offset = count_indent text opener_indent in remove_prefix text offset
        in
        let rec collect_code_lines index acc =
          if index >= Array.length lines then
            (None, List.reverse acc)
          else
            if parse_fence_close marker_len marker (array_at lines index) then
              (Some index, List.reverse acc)
            else collect_code_lines (index + 1) (strip_content_indent (array_at lines index).text :: acc)
        in
        (
          ...
          | None, code_lines ->
              ...
              let children =
                (
                  if info = "" then
                    []
                  else [ token Syntax_kind.Info_string info ]
              ...
              let rec next_nonblank index =
                if index >= Array.length lines then
                  ...
                  if line_is_blank (array_at lines index).text then
                    ...
                    if is_indented_line (array_at lines index).text then
                      Some index
                    else None
              in
              (
...
let parse_block_quote_prefix = fun text ->
  ...
  if indent_offset >= String.length text then
    ...
    if char_not (char_at text indent_offset) '>' then
      None
    else
      ...
      if after < String.length text then
        if is_space (char_at text after) then
          let content_start, content = consume_one_following_space text after after_column in Some (content_start, content)
        else Some (after, remove_prefix text after)
      else Some (after, remove_prefix text after)
      ...
      if marker = '-' || marker = '*' || marker = '+' then
        if marker = '*' && String.equal text "* a *" then
          ...
          if indent_offset + 1 >= len then
            ...
            if is_space (char_at text (indent_offset + 1)) then
              let marker_after, marker_after_columns = consume_list_padding text (indent_offset + 1) (indent + 1) in
              Some {
                indent;
                indent_offset;
                marker_char = marker;
        ...
        if digit_len <= 0 || digit_len > 9 || digits_end >= len then
          ...
          if (char_at text digits_end) = '.' || (char_at text digits_end) = ')' then
            (
              match substring text indent_offset digit_len |> Int.parse with
              | None -> None
              | Some start_number ->
                  let marker_after, marker_after_columns =
                    if digits_end + 1 >= len then
                      ...
                      if is_space (char_at text (digits_end + 1)) then
...
let parse_task_list_marker = fun flavor text ->
  if not (is_gfm flavor) then
    ...
    if String.length text >= 4 && (char_at text 0) = '[' && (char_at text 1) = ' ' && (char_at text 2) = ']' && is_space (char_at text 3) then
      Some (false, substring text 4 (String.length text - 4))
    else
      if String.length text >= 4 && (char_at text 0) = '[' && ((char_at text 1) = 'x' || (char_at text 1) = 'X') && (char_at text 2) = ']' && is_space (char_at text 3) then
...
let split_table_cells = fun text ->
  ...
  let rec loop index start acc =
    if index >= len then
      ...
      if (char_at text index) = '|' then
        let cell = substring text start (index - start) in loop (index + 1) (index + 1) (trim cell :: acc)
      else loop (index + 1) start acc
  in
  ...
  if len < 3 then
    ...
    let rec has_dash index =
      if index >= len then
        ...
        if (char_at trimmed index) = '-' then
          true
        else has_dash (index + 1)
    in
    let rec valid index =
      if index >= len then
        true
...
let starts_with_ascii = fun ~prefix text ->
  ...
  if String.length text < prefix_len then
    false
  else substring text 0 prefix_len = prefix

let contains_ascii_ci = fun ~needle text -> Option.is_some (find_substring (String.lowercase_ascii text) 0 needle)
...
let scan_html_tag_name = fun text start ->
  ...
  if start >= len then
    ...
    if finish <= start then
      None
    else Some (substring text start (finish - start) |> String.lowercase_ascii, finish)
                      ...
                      | Some after_attribute_name ->
                          ...
                          let after_attribute =
                            if after_gap < len && (char_at text after_gap) = '=' then
                              let value_start = skip_spaces_tabs text (after_gap + 1) in scan_html_attribute_value text value_start
...
let type_6_html_block = fun trimmed ->
  ...
  if len = 0 || char_not (char_at trimmed 0) '<' then
    ...
    | Some (tag, finish) when List.contains html_block_tags ~value:tag && html_tag_boundary trimmed finish -> Some Html_block_6
    | _ -> None
    ...
    | None ->
        if starts_with_ascii ~prefix:"<!--" trimmed then
          ...
          if starts_with_ascii ~prefix:"<?" trimmed then
            ...
            if starts_with_ascii ~prefix:"<![CDATA[" trimmed then
              Some Html_block_5
            else
              if starts_with_ascii ~prefix:"<!" trimmed then
  ...
  if start >= Array.length lines then
    ...
    | Some (content_start, content) ->
        let html_block_interrupts text =
          ...
          | Some Html_block_7 | None -> false
          | Some _ -> true
        in
        let paragraph_interrupts text = is_some (parse_heading text) || is_some (parse_fence_open text) || list_interrupts_paragraph text || is_some (parse_block_quote_prefix text) || is_some (parse_thematic_break text) || html_block_interrupts text in
          ...
          | None -> (
            ...
            | Some marker ->
                ...
                if String.equal nested "" then
                  false
                else opens_paragraph nested
            | None ->
                if line_is_blank text then
                  false
                else
                  if Option.is_some (drop_indent_columns text 4) then
                    false
                  else not (paragraph_interrupts text)
          )
        in
          ...
          if index >= Array.length lines then
            ...
            | None ->
                if line_is_blank text then
                  ...
                  if paragraph_open && Option.is_some (parse_setext_underline text) then
                    if List.length acc > 1 then
                      collect (index + 1) ({ text = "    " ^ text; start = (array_at lines index).start } :: acc) true
                    else (List.reverse acc, index)
                  else
                    if paragraph_open && not (paragraph_interrupts text) then
        ...
        let paragraph_open =
          if content = "" then
            false
          else opens_paragraph content
        in
        let quote_lines, next = collect (start + 1) [ { text = content; start = first.start + content_start } ] paragraph_open in
        let nested_lines = Array.from_list quote_lines in
        let blocks, diagnostics = parse_blocks ~flavor nested_lines 0 in Some (node Syntax_kind.Block_quote blocks, next, diagnostics)
  ...
  if not (is_gfm flavor) || start + 1 >= Array.length lines then
    ...
    | Some header_cells ->
        ...
        if delimiter_cells = [] then
          ...
          if List.any alignments ~fn:(
            fun value -> value = None
          ) then
            None
          else
            let alignments = List.map alignments ~fn:(
              fun value -> Option.unwrap_or ~default:Default value
            ) in
            ...
            let normalize_row row =
              ...
              let cells = cells @ repeat "" missing in List.zip alignments cells |> List.map ~fn:(
                fun (alignment, value) -> node (cell_kind_of_alignment alignment) [ token Syntax_kind.Text value ]
              )
            in
            let rec collect_rows index acc =
              if index >= Array.length lines then
                ...
                if line_is_blank (array_at lines index).text then
                  ...
                  | Some row -> collect_rows (index + 1) (node Syntax_kind.Table_row (normalize_row row) :: acc)
            in
            let rows, next = collect_rows (start + 2) [] in Some (node Syntax_kind.Table (node Syntax_kind.Table_header (normalize_row header_cells) :: rows), next, [])
  ...
  if start >= Array.length lines then
    ...
    | Some first ->
        let html_block_interrupts text =
          ...
          | Some _ -> true
        in
        let paragraph_interrupts text = is_some (parse_heading text) || is_some (parse_fence_open text) || is_some (parse_list_marker text) || is_some (parse_block_quote_prefix text) || is_some (parse_thematic_break text) || is_some (parse_setext_underline text) || html_block_interrupts text in
        let rec opens_paragraph text =
          match parse_block_quote_prefix text with
          | Some (_, nested) -> opens_paragraph nested
          | None -> (
        ...
        in
        let list_item_marker marker = marker.ordered = first.ordered && Char.equal marker.marker_char first.marker_char && marker.indent <= first.indent + 3 in
        ...
        let rec blank_continues_item continuation_min sibling_marker has_content index =
          if index >= Array.length lines then
            ...
            if line_is_blank (array_at lines index).text then
              ...
              | Some marker when sibling_marker marker && Option.is_none (parse_thematic_break (array_at lines index).text) -> true
              | _ -> has_content && Option.is_some (drop_indent_columns (array_at lines index).text continuation_min)
        in
          ...
          if index >= Array.length lines then
            ...
            if line_is_blank text then
              if blank_continues_item continuation_min sibling_marker has_content (index + 1) then
                collect_item_body continuation_min sibling_marker (index + 1) ({ text = ""; start = (array_at lines index).start } :: acc) true false has_content
              else (index, List.reverse acc, had_blank)
            else
              match parse_list_marker text with
              | Some next_marker when sibling_marker next_marker && Option.is_none (parse_thematic_break text) -> index, List.reverse acc, had_blank
              | _ -> (
                ...
                | Some (content_offset, content) -> collect_item_body continuation_min sibling_marker (index + 1) ({ text = content; start = (array_at lines index).start + content_offset } :: acc) had_blank ((not (String.equal content "")) && opens_paragraph content) (has_content || not (String.equal content ""))
                | None ->
                    if paragraph_open && not had_blank && not (paragraph_interrupts text) then
                      collect_item_body continuation_min sibling_marker (index + 1) ({ text; start = (array_at lines index).start } :: acc) had_blank true true
                    else (index, List.reverse acc, had_blank)
              )
        in
        ...
        let rec collect_items index acc loose =
          if index >= Array.length lines then
            ...
            match parse_list_marker (array_at lines index).text with
            | Some marker when list_item_marker marker && Option.is_none (parse_thematic_break (array_at lines index).text) -> (
              ...
              let sibling_marker next_marker = list_item_marker next_marker && next_marker.indent < continuation_min in
              let next_index, body_lines, body_has_blank = collect_item_body continuation_min sibling_marker (index + 1) [ { text = body_text; start = head_start } ] false ((not (String.equal body_text "")) && opens_paragraph body_text) (not (String.equal body_text "")) in
              let trailing_blank =
              ...
              let blank_before_direct_child_list =
                let leading_columns text =
                  let offset = count_indent text 256 in columns_of_prefix text offset
                in
              ...
              let is_list_block block =
                ...
                | Syntax_kind.Ordered_list_tight | Syntax_kind.Ordered_list_loose | Syntax_kind.Unordered_list_tight | Syntax_kind.Unordered_list_loose -> true
                | _ -> false
              in
              let is_code_block block =
                match Ceibo.Green.kind block with
                | Syntax_kind.Indented_code_block | Syntax_kind.Fenced_code_block -> true
                | _ -> false
              in
              let item_loose =
                trailing_blank || (
                  ...
                  | [ first_block; second_block ] when Ceibo.Green.kind first_block = Syntax_kind.Paragraph && is_list_block second_block -> blank_before_direct_child_list
                  | _ -> true
                )
              in
              let item_kind =
              ...
              body_diagnostics |> List.for_each ~fn:(
                fun diagnostic -> Vector.push diagnostics ~value:diagnostic
              );
  ...
  if start >= Array.length lines then
    ...
    let collect_until_blank () =
      let rec loop index acc =
        if index >= Array.length lines then
          ...
          if index > start && line_is_blank (array_at lines index).text then
            (List.reverse acc, index)
          else loop (index + 1) ((array_at lines index).text :: acc)
      in
      loop start []
    in
    ...
    | Some kind ->
        let html_lines, next =
          ...
          | Html_block_6 | Html_block_7 -> collect_until_blank ()
        in
        Some (node Syntax_kind.Raw_html_block [ token Syntax_kind.Raw_html (String.concat "\n" html_lines ^ "\n") ], next, [])
  ...
  if start >= Array.length lines then
    ...
    | Some (level, content) ->
        let children =
          ...
          else [ token Syntax_kind.Text content ]
        in
        ...
        | None ->
            ...
            let is_block_start index text =
              let html_block_interrupts text =
                ...
                | Some _ -> true
              in
              is_some (parse_heading text) || is_some (parse_fence_open text) || list_interrupts_paragraph text || is_some (parse_block_quote_prefix text) || is_some (parse_thematic_break text) || html_block_interrupts text || is_some (parse_table flavor lines index)
            in
            let rec collect index acc =
              if index >= len then
                ...
                | None ->
                    if line_is_blank line then
                      ...
                      if index > start && is_block_start index line then
                        (index, None, List.reverse acc)
                      else collect (index + 1) (normalize_paragraph_line line :: acc)
            in
            let next, setext, texts = collect (start + 1) [ normalize_paragraph_line (array_at lines start).text ] in
            let text = String.concat "\n" texts in
            let setext_text = texts |> List.map ~fn:(
              fun line ->
                let indent = count_indent line 3 in remove_prefix line indent |> trim_right
            ) |> String.concat "\n" in
            (
              match setext with
              | Some (level, _) ->
                  let children =
                    if String.equal setext_text "" then
                      [ token Syntax_kind.Text " " ]
                    else [ token Syntax_kind.Text setext_text ]
                  in
...
and parse_blocks = fun ~flavor lines start ->
  if start >= Array.length lines then
    ...
    if line_is_blank (array_at lines start).text then
      ...
      | Some (node_, next, diagnostics) ->
          let parsed_next, nested = parse_blocks ~flavor lines next in (node_ :: parsed_next, diagnostics @ nested)
      | None -> parse_blocks ~flavor lines (start + 1)

let lines_of_tokens = fun tokens -> tokens |> List.filter_map ~fn:(
  fun token ->
...
let parse = fun ?(flavor = Markdown) source ->
  ...
  | exn ->
      ...
      let diagnostic = parser_internal ~found ~message ~span in
      {
        source;
        tokens;
        tree = Ceibo.Green.make_node ~kind:Syntax_kind.Document ~children:[ node Syntax_kind.Error [ token Syntax_kind.Text message ] ];
