...
module String_view = struct
  ...
  let borrowed = fun ~source ~offset ~length ->
    Borrowed { source; offset; length }
  ...
  let length = function
    | Borrowed { length; _ } -> length
    | Owned value -> String.length value

  let get = fun t index ->
    match t with
    | Borrowed { source; offset; _ } -> source.[offset + index]
    | Owned value -> value.[index]

  let to_string = function
    | Borrowed { source; offset; length } -> String.sub source offset length
    | Owned value -> value

  let equal_string = fun t other ->
    let other_len = String.length other in
    match t with
    | Owned value ->
        String.equal value other
    | Borrowed { source; offset; length } ->
        if not (Int.equal length other_len) then
          false
        else
          let rec loop index =
            if Int.equal index other_len then
              true
