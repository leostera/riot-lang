...
module Fields = struct
  ...
  type 'tag edge = { first: char; label: string; next: 'tag node }
  and 'tag node = { tag: 'tag option; edges: 'tag edge array }
  and 'tag t = { root: 'tag node; tags: 'tag array }
  ...
  let drop_prefix = fun value prefix_length ->
    ...
    if Int.equal prefix_length length then
      ""
    else String.sub value ~offset:prefix_length ~len:(length - prefix_length)
  ...
  let common_prefix_length = fun left right ->
    ...
    let rec loop index =
      if Int.equal index limit then
        ...
        if Char.equal (String.get_unchecked left ~at:index) (String.get_unchecked right ~at:index) then
          loop (index + 1)
        else index
    in
    loop 0
  ...
  let longest_common_prefix_length = function
    | [] -> 0
    | first :: rest -> List.fold_left rest ~init:(String.length first.suffix) ~fn:(
  ...
  let group_by_first = fun entries ->
    ...
    in
    List.fold_left entries ~init:[] ~fn:(
      fun groups entry -> insert entry groups
    )

  let rec build_node: 'tag. 'tag pending list -> 'tag node = fun entries ->
    let (tag, non_empty_entries) = List.fold_left entries ~init:(None, []) ~fn:(
      fun (tag, non_empty_entries) entry ->
        ...
        else (tag, entry :: non_empty_entries)
    ) in
    let edges = group_by_first non_empty_entries |> List.map ~fn:(
      fun (_first, group) ->
        ...
        let next_entries = List.map group ~fn:(
          fun entry -> { suffix = drop_prefix entry.suffix prefix_length; tag = entry.tag }
        ) in { first = String.get_unchecked label ~at:0; label; next = build_node next_entries }
    ) in { tag; edges = Array.init ~count:(List.length edges) ~fn:(
      fun index -> list_nth edges index
    ) }

  let string_equals_slice = fun source ~offset ~length other -> let open Std.Int in
  if length != String.length other then
    ...
    let rec loop index =
      if index = length then
        ...
        if Char.equal (String.get_unchecked source ~at:(offset + index)) (String.get_unchecked other ~at:index) then
          loop (index + 1)
        else false
    in
    loop 0

  let bytes_equals_string = fun source ~offset ~length other -> let open Std.Int in
  if length != String.length other then
    false
  else
    let rec loop index =
      if index = length then
        ...
        if Char.equal (IO.Bytes.get_unchecked source ~at:(offset + index)) (String.get_unchecked other ~at:index) then
          loop (index + 1)
        else false
    in
    loop 0

  let ioslice_equals_string = fun source ~offset ~length other -> let open Std.Int in
  if length != String.length other then
    false
  else
    let rec loop index =
      if index = length then
        ...
        if Char.equal (IO.IoSlice.get_unchecked source ~at:(offset + index)) (String.get_unchecked other ~at:index) then
          loop (index + 1)
        else false
    in
    loop 0

  let buffer_equals_string = fun buffer ~offset ~length other -> let open Std.Int in
  if length != String.length other then
    false
  else
    let rec loop index =
      if index = length then
        ...
        if Char.equal (IO.Buffer.get_unchecked buffer ~at:(offset + index)) (String.get_unchecked other ~at:index) then
          loop (index + 1)
        else false
    in
    loop 0

  let find_edge: 'tag. 'tag edge array -> char -> 'tag edge option = fun edges first -> let open Std.Int in
  let rec loop index =
  ...
  let match_slice: 'tag. 'tag t -> string -> offset:int -> length:int -> 'tag option = fun fields source ~offset ~length -> let open Std.Int in
    ...
    if length = 0 then
      ...
      | Some edge ->
          ...
          if label_length > length then
            ...
            if string_equals_slice source ~offset ~length:label_length edge.label then
              loop edge.next (offset + label_length) (length - label_length)
            else None
  in
  loop fields.root offset length

  let match_bytes: 'tag. 'tag t -> bytes -> offset:int -> length:int -> 'tag option = fun fields source ~offset ~length -> let open Std.Int in
  let rec loop (node: 'tag node) offset length =
    if length = 0 then
      node.tag
    else
      ...
      | Some edge ->
          ...
          if label_length > length then
            ...
            if bytes_equals_string source ~offset ~length:label_length edge.label then
              loop edge.next (offset + label_length) (length - label_length)
            else None
  in
  loop fields.root offset length

  let match_ioslice: 'tag. 'tag t -> IO.IoSlice.t -> offset:int -> length:int -> 'tag option = fun fields source ~offset ~length -> let open Std.Int in
  let rec loop (node: 'tag node) offset length =
    if length = 0 then
      node.tag
    else
      ...
      | Some edge ->
          ...
          if label_length > length then
            ...
            if ioslice_equals_string source ~offset ~length:label_length edge.label then
              loop edge.next (offset + label_length) (length - label_length)
            else None
  in
  loop fields.root offset length

  let match_buffer: 'tag. 'tag t -> IO.Buffer.t -> 'tag option = fun fields buffer -> let open Std.Int in
  let rec loop (node: 'tag node) offset length =
    if length = 0 then
      node.tag
    else
      ...
      | Some edge ->
          ...
          if label_length > length then
            ...
            if buffer_equals_string buffer ~offset ~length:label_length edge.label then
              loop edge.next (offset + label_length) (length - label_length)
            else None
  in
  loop fields.root 0 (IO.Buffer.length buffer)

  let match_buffer_range: 'tag. 'tag t -> IO.Buffer.t -> offset:int -> length:int -> 'tag option = fun fields buffer ~offset ~length -> let open Std.Int in
  let rec loop (node: 'tag node) offset length =
    if length = 0 then
      node.tag
    else
  ...
  let make = fun cases ->
    let root = List.map cases ~fn:(
      fun case -> { suffix = case.key; tag = case.tag }
    ) |> build_node in
    let tags = Array.init ~count:(List.length cases) ~fn:(
      fun index -> list_nth cases index |> tag
    ) in { root; tags }
  ...
  let tag_at = fun fields index ->
    if index < 0 || index >= Array.length fields.tags then
      None
    else Some (Array.get_unchecked fields.tags ~at:index)
...
let and_then = fun decode next ->
  {
    run = fun backend state ->
      let value = decode.run backend state in (next value).run backend state
  }
...
let record = fun ~fields ~init ~step ~finish ->
  {
    run = fun backend state ->
      let reader = reader_of_backend backend state in backend.record state ~fields ~init ~step:(
        fun acc field -> step reader acc field
      ) ~finish
  }
...
let record_mut = fun ~fields ~create ~step ~finish ->
  {
    run = fun backend state ->
      let reader = reader_of_backend backend state in backend.record_mut state ~fields ~create ~step:(
        fun builder field -> step reader builder field
      ) ~finish
  }
  ...
  let record = fun ~fields ~init ~step ~finish ->
    {
      run = fun backend state ->
        ...
        in
        backend.record state ~fields ~init ~step:(
          fun acc field -> step reader acc field
        ) ~finish
    }
  ...
  let record_mut = fun ~fields ~create ~step ~finish ->
    {
      run = fun backend state ->
        ...
        in
        backend.record_mut state ~fields ~create ~step:(
          fun builder field -> step reader builder field
        ) ~finish
    }
