  ...
  Telemetry.attach handler_name
    (fun event ->
      (
        ...
        | Tusk_executor.Telemetry_events.CacheMiss _ -> Protocol.BuildStats.inc_action_cache_misses stats
        | _ -> ()
      );
    ...
    (
      ...
      | Error err -> (
          ...
          | Tusk_planner.Workspace_planner.MissingDependencies { missing } ->
              ...
              List.iter
              (fun ({ Tusk_planner.Package_graph.package; dependency }) -> Log.error
