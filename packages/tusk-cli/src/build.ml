...
let write_json_event (json : Data.Json.t) =
  print (Data.Json.to_string json);
  print "\n"

let telemetry_event_to_json event =
  match Tusk_executor.Telemetry_events.to_json event with
  | Some json -> json
  | None -> Data.Json.Null

let build_stats_to_json (stats : Local_session.build_stats) =
  Data.Json.Object
    [
...
let package_results_to_json (results : Tusk_executor.Package_builder.build_result list) =
  Data.Json.Array (List.map Tusk_executor.Package_builder.build_result_to_json results)

let build_failed_event_to_json session_id failed_at errors =
  Data.Json.Object
    [
...
let build_completed_event_to_json session_id completed_at stats results =
  Data.Json.Object
    [
...
let planning_failed_event_to_json session_id failed_at reason =
  Data.Json.Object
    [
...
let cycle_detected_event_to_json session_id detected_at cycle_nodes =
  Data.Json.Object
    [
...
let run_build_request ~workspace ~load_errors ?(scope = Runtime) ?(mode = Human)
    request target_arch =
  let client =
    ...
    Local_session.build_streaming client request
      ...
      ?target_arch (fun event ->
        ...
        | Json -> (
            ...
            | Local_session.BuildCompleted
                { session_id; completed_at; stats; results } ->
                build_completed_event_to_json session_id completed_at stats results
                |> write_json_event
            | Local_session.BuildFailed
                { session_id; failed_at; stats = _; built = _; errors } ->
            ...
            | Local_session.PlanningFailed { session_id; failed_at; reason } ->
                planning_failed_event_to_json session_id failed_at reason
                |> write_json_event
            | Local_session.CycleDetected
                { session_id; detected_at; cycle_nodes } ->
                cycle_detected_event_to_json session_id detected_at cycle_nodes
                |> write_json_event)
        | Human ->
            (match event with
            | Local_session.BuildStarted _ -> ()
                ...
                List.iter (fun (error : Tusk_executor.Package_builder.build_result) ->
                  ...
                  | Tusk_executor.Package_builder.Failed
                      ...
                      out
                        ...
                        ^ error.package.name ^ ": " ^ message )
                  | Tusk_executor.Package_builder.Failed
                      (Tusk_executor.Package_builder.ActionOutputsNotCreated
                       { missing }) ->
                      out "";
                      out
                        ...
                        ^ error.package.name)
                  | Tusk_executor.Package_builder.Failed
                      (Tusk_executor.Package_builder.ActionDependenciesFailed _) ->
                      out "";
                      out
            ...
            | Local_session.CycleDetected { cycle_nodes; _ } ->
                out
                  "      \027[1;31mError\027[0m: Cyclic dependency detected:";
                out ("         " ^ String.concat " ->\n         " cycle_nodes)))
  in
            ...
            write_json_event
              (command_error_event_to_json
                 ...
                 [
                   ...
                   ( "available_packages",
                     Data.Json.Array
                       (List.map (fun pkg -> Data.Json.String pkg) available_packages) );
                 ]);
            ...
            write_json_event
              (command_error_event_to_json
                 ...
                 [
                   ( "package_names",
                     Data.Json.Array
                       (List.map (fun pkg -> Data.Json.String pkg) package_names) );
                   ( "available_packages",
                     Data.Json.Array
                       (List.map (fun pkg -> Data.Json.String pkg) available_packages) );
                 ]);
...
let build_command
    ?workspace ?load_errors ?(scope = Runtime) ?(mode = Human) package_opt
    target_arch =
...
let build_packages_command
    ~workspace ~load_errors ?(scope = Runtime) ?(mode = Human) package_names
    target_arch =
  let request =
...
let run ~workspace ~load_errors matches =
  ...
  let mode = if get_flag matches "json" then Json else Human in
