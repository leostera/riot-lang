...
let pending_paths = fun workspace_root ->
  ...
  let workspace_pending =
    Path.(workspace_root / Path.v ".tusk/snapshots/std/suite/case.expected.new") in
  let workspace_approved = Path.(workspace_root / Path.v ".tusk/snapshots/std/suite/case.expected") in
              ...
              | Ok () -> (
                  ...
                  | Ok () -> (
                      ...
                      | Ok snapshots ->
                          let actual =
                            List.map
                              (fun snapshot -> Path.to_string snapshot.Tusk_cli.Snapshots.pending)
                              snapshots
                          in
                          let expected = [
      ...
      with_tempdir_result "snapshots_query"
        (fun workspace_root ->
          ...
          | Ok () -> (
              ...
              | Error msg -> Error msg
              | Ok () -> (
                  match Tusk_cli.Snapshots.discover_pending_snapshots
                    ~workspace_root
