...
let test_reentrant_acquire_in_same_process () =
  with_tempdir "tusk_build_lock" (fun tmpdir ->
      BuildLock.acquire ~workspace_root:tmpdir ~profile:"debug"
        ~target:"aarch64-apple-darwin" (fun () ->
          let parent = self () in
          let _worker =
            spawn (fun () ->
...
let test_releases_lock_on_exception () =
  with_tempdir "tusk_build_lock" (fun tmpdir ->
      ...
      try
        let _ =
          BuildLock.acquire ~workspace_root:tmpdir ~profile:"debug"
            ~target:"aarch64-apple-darwin" (fun () -> raise Synthetic_failure)
        in
      ...
      | Synthetic_failure -> (
          match
            BuildLock.acquire ~workspace_root:tmpdir ~profile:"debug"
              ~target:"aarch64-apple-darwin" (fun () -> Ok ())
          with
          | Ok () -> Ok ()
          | Error _ -> Error "Build lock was not released after exception"))

let test_different_targets_do_not_block_each_other () =
  with_tempdir "tusk_build_lock" (fun tmpdir ->
      BuildLock.acquire ~workspace_root:tmpdir ~profile:"debug"
        ~target:"aarch64-apple-darwin" (fun () ->
          let parent = self () in
          let _worker =
            spawn (fun () ->
                ...
                match
                  BuildLock.acquire ~workspace_root:tmpdir ~profile:"debug"
                    ~target:"aarch64-unknown-linux-gnu" (fun () ->
                      let waited = Time.Instant.elapsed start in
                      send parent (BuildLockAcquired waited);
                      Ok ())
                with
                ...
                | Error _ ->
                    send parent
                      (BuildLockAcquireFailed
                         "Failed to acquire build lock for second target");
                    Ok ())
          in
          let selector msg =
            match msg with
...
let tests =
  Test.
    [
      ...
      case "build lock: different targets do not block each other"
