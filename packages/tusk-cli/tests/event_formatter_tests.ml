...
let test_workspace_completed_is_silent () =
  let displayed_packages = HashSet.create () in
  let event =
    Tusk_executor.Telemetry_events.WorkspaceCompleted
      {
  ...
  if String.equal rendered "" then Ok ()
  else Error ("expected empty workspace summary, got: " ^ rendered)

let tests =
  Test.
    [
      case "event formatter: workspace completed is silent"
        test_workspace_completed_is_silent;
    ]
