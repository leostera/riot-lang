...
let prompt_to_json (p : prompt) =
  Json.Object [
    ...
    ("arguments", option_to_json (fun args -> 
      Json.Array (List.map prompt_argument_to_json args)) p.arguments);
  ]
...
let request_params_to_json = function
  ...
  | GetPromptParams { name; arguments } ->
      Json.Object [
        ("name", Json.String name);
        ("arguments", option_to_json (fun args ->
          Json.Object (List.map (fun (k, v) -> (k, Json.String v)) args)) arguments);
      ]
  | CompleteSamplingParams params ->
      Json.Object [
        ...
        ("stopSequences", option_to_json (fun ss -> 
          Json.Array (List.map (fun s -> Json.String s) ss)) params.stop_sequences);
...
let request_to_json (req : request) =
  let params_field = match req.params with
    | None -> []
...
let notification_to_json (notif : notification) =
  let params_field = match notif.params with
    | None -> []
