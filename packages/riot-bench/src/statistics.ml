...
let int_median = fun values ->
  values |> List.map ~fn:Float.of_int |> float_median |> Option.map ~fn:Float.to_int
...
let baseline_statistics = fun (history: history_sample list) ->
  let statistics =
    List.map history ~fn:(fun sample -> sample.statistics)
  in
  let duration_field project name = project statistics
  |> duration_median
