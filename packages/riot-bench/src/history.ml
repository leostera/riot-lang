...
let comparison_case_history = fun
  previous_runs description (current_case: bench_comparison_case_result) ->
  let history =
    List.filter_map
      previous_runs
      ~fn:(fun (stored: stored_suite_run) ->
...
let compare_suite_run = fun context ~package_name ~suite_name ~(current:suite_run) ~limit ->
  let* previous_runs = Storage.load_recent_suite_runs context ~package_name ~suite_name ~limit in
