...
val plan_goal_dependencies:
  t -> Goal.build_package -> (Work_request.t list, Error.t) result

val plan_package_artifact_dependencies:
  t -> Goal.build_package -> (Work_request.t list, Error.t) result

val execute_package_artifact:
  t -> Work_registry.t -> Goal.build_package -> (Work_result.t, Error.t) result

val plan_module_dependencies:
  t -> Goal.build_package -> (Work_request.t list, Error.t) result

val execute_module_dependencies:
  t -> Work_registry.t -> Goal.build_package -> (Work_result.t, Error.t) result

val plan_ocaml_archive:
  t -> Goal.build_package -> (Work_request.t list, Error.t) result

val execute_ocaml_archive:
  t -> Work_registry.t -> Goal.build_package -> (Work_result.t, Error.t) result

val plan_ocaml_source:
  t -> Rule.ocaml_source -> (Work_request.t list, Error.t) result

val plan_ocaml_generated:
...
val execute_ocaml_generated:
  t -> Work_registry.t -> Rule.ocaml_generated -> (Work_result.t, Error.t) result

val plan_c_object:
  t -> Rule.c_object -> (Work_request.t list, Error.t) result

val execute_c_object:
  t -> Work_registry.t -> Rule.c_object -> (Work_result.t, Error.t) result
