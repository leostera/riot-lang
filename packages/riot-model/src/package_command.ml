...
let parse_from_toml: Toml.value list -> package_name:Package_name.t -> package_path:Path.t -> t list = fun toml_entries ~package_name ~package_path -> List.filter_map toml_entries ~fn:(
  fun cmd_toml ->
    match cmd_toml with
    | Toml.Table cmd_items -> (
      ...
      | (Some (Toml.String name), Some (Toml.String path)) ->
          ...
          let command_binary = Path.(v "_build" / v "debug" / v "out" / v (Package_name.to_string package_name) / v name) in
          Some {
            name;
            description;
            package_name;
