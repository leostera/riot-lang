...
let strip_ansi_codes = fun str ->
  ...
  let rec strip read_index write_index =
    if read_index >= len then
      ...
      | None -> IO.Bytes.sub_unchecked out ~offset:0 ~len:write_index |> IO.Bytes.to_string
      | Some '\027' -> (
...
type t = { timestamp: DateTime.t; session_id: Session_id.t; level: level; kind: kind }
...
let display = function
  | BuildStarted { packages; _ } -> "Build started for " ^ Int.to_string (List.length packages) ^ " packages"
  | BuildComplete { duration_ms; succeeded; failed; _ } -> "Build completed in " ^ Int.to_string duration_ms ^ "ms (" ^ Int.to_string (List.length succeeded) ^ " succeeded, " ^ Int.to_string (List.length failed) ^ " failed)"
  ...
  | PackageComplete { package; success; duration_ms; _ } ->
      ...
      else "✗ Failed to build " ^ Package_name.to_string package
  | PackageSkipped { package; reason } ->
      let reason_str =
        match reason with
        | DependenciesFailed deps -> "dependencies failed: " ^ String.concat ", " (List.map deps ~fn:Package_name.to_string)
      in
  ...
  | CompileError { package; error } ->
      ...
      in
      "Error in " ^ Package_name.to_string package ^ " [" ^ error.file ^ ":" ^ Int.to_string error.line ^ ":" ^ Int.to_string col_start ^ "]: " ^ kind_msg
  | CycleDetected { packages } -> "Circular dependency detected: " ^ String.concat " -> " (List.map packages ~fn:Package_name.to_string)
  ...
  | CacheStored { package; artifacts; _ } -> "Cached " ^ Package_name.to_string package ^ " (" ^ Int.to_string (List.length artifacts) ^ " artifacts)"
  | WorkerPoolStarted { workers } -> "Started worker pool with " ^ Int.to_string workers ^ " workers"
  | WorkerStarted { worker_id } -> "Worker " ^ Worker_id.to_string worker_id ^ " started"
  | WorkerAssigned { worker_id; package } -> "Worker " ^ Worker_id.to_string worker_id ^ " assigned to " ^ Package_name.to_string package
  ...
  | ServerRestarted { packages; toolchain } -> "Server restarted with " ^ Int.to_string packages ^ " packages (toolchain: " ^ toolchain ^ ")"
  | WorkspaceEmpty -> "No packages found in workspace"
  | WorkspaceScanning -> "Scanning workspace..."
  | WorkspaceScanned { packages; duration_ms } -> "Scanned workspace: " ^ Int.to_string packages ^ " packages in " ^ Int.to_string duration_ms ^ "ms"
  | LockfileReadStarted { path } -> "Reading lockfile " ^ path
  | LockfileReadFinished { path; duration_ms } -> "Read lockfile " ^ path ^ " in " ^ Int.to_string duration_ms ^ "ms"
  | LockfileReadFailed { path; error } -> "Failed to read lockfile " ^ path ^ ": " ^ Pm_error.message error
  | LockfileWriteStarted { path } -> "Writing lockfile " ^ path
  | LockfileWriteFinished { path; duration_ms } -> "Wrote lockfile " ^ path ^ " in " ^ Int.to_string duration_ms ^ "ms"
  | LockfileWriteFailed { path; error } -> "Failed to write lockfile " ^ path ^ ": " ^ Pm_error.message error
  ...
  | DependencyResolutionFinished { duration_ms; resolved_packages; resolved_edges } -> "Resolved " ^ Int.to_string resolved_packages ^ " packages and " ^ Int.to_string resolved_edges ^ " edges in " ^ Int.to_string duration_ms ^ "ms"
  | DependencyResolutionFailed { error } -> "Dependency resolution failed: " ^ Pm_error.message error
  | RegistryIndexUpdating { registry } -> "Updating " ^ registry ^ " index"
  | DependencyUniverseBuilding { packages } -> "Building dependency universe for " ^ Int.to_string (List.length packages) ^ " packages"
  | DependencyUniverseBuilt { runtime_packages; build_packages; dev_packages; duration_ms } -> "Built dependency universe in " ^ Int.to_string duration_ms ^ "ms (runtime=" ^ Int.to_string runtime_packages ^ ", build=" ^ Int.to_string build_packages ^ ", dev=" ^ Int.to_string dev_packages ^ ")"
  | PackageMetadataFetchStarted { registry; package } -> "Fetching package metadata for " ^ Package_name.to_string package ^ " from " ^ registry
  | PackageMetadataFetchFinished { registry; package; version; duration_ms } -> (
    match version with
    | Some version -> "Fetched package metadata for " ^ Package_name.to_string package ^ "@" ^ version ^ " from " ^ registry ^ " in " ^ Int.to_string duration_ms ^ "ms"
    | None -> "Fetched package metadata for " ^ Package_name.to_string package ^ " from " ^ registry ^ " in " ^ Int.to_string duration_ms ^ "ms"
  )
  | PackageMetadataFetchFailed { registry; package; error } -> "Failed to fetch package metadata for " ^ Package_name.to_string package ^ " from " ^ registry ^ ": " ^ Pm_error.message error
  ...
  | PackageVersionLocked { package; version } -> "Locked " ^ Package_name.to_string package ^ " (" ^ version ^ ")"
  | PackageVersionsUnchanged { packages } -> "Dependencies are already up to date (" ^ Int.to_string packages ^ " locked packages unchanged)"
  | PackageVersionUpdated { package; from_version; to_version } -> "Updated " ^ Package_name.to_string package ^ " (" ^ from_version ^ " -> " ^ to_version ^ ")"
  | PackageManifestFetchStarted { package; version } -> "Fetching manifest for " ^ Package_name.to_string package ^ "@" ^ version
  | PackageManifestFetchFinished { package; version; duration_ms } -> "Fetched manifest for " ^ Package_name.to_string package ^ "@" ^ version ^ " in " ^ Int.to_string duration_ms ^ "ms"
  | PackageManifestFetchFailed { package; version; error } -> (
    match version with
    | Some version -> "Failed to fetch manifest for " ^ Package_name.to_string package ^ "@" ^ version ^ ": " ^ Pm_error.message error
    | None -> "Failed to fetch manifest for " ^ Package_name.to_string package ^ ": " ^ Pm_error.message error
  )
  | PackageDownloadStarted { package; version; _ } -> "Downloading " ^ Package_name.to_string package ^ "@" ^ version
  | PackageDownloadFinished { package; version; path; duration_ms } -> "Downloaded " ^ Package_name.to_string package ^ "@" ^ version ^ " to " ^ path ^ " in " ^ Int.to_string duration_ms ^ "ms"
  | PackageDownloadFailed { package; version; error; _ } -> "Failed to download " ^ Package_name.to_string package ^ "@" ^ version ^ ": " ^ Pm_error.message error
  | PackageDownloadSkipped { package; version; reason; _ } -> "Skipped download for " ^ Package_name.to_string package ^ "@" ^ version ^ " (" ^ reason ^ ")"
  | PackageCacheHit { package; version; path } -> "Package cache hit for " ^ Package_name.to_string package ^ "@" ^ version ^ " at " ^ path
  | PackageMaterializationStarted { package; version; _ } -> "Materializing " ^ Package_name.to_string package ^ "@" ^ version
  | PackageMaterializationFinished { package; version; path; duration_ms } -> "Materialized " ^ Package_name.to_string package ^ "@" ^ version ^ " at " ^ path ^ " in " ^ Int.to_string duration_ms ^ "ms"
  | PackageMaterializationFailed { package; version; error; _ } -> "Failed to materialize " ^ Package_name.to_string package ^ "@" ^ version ^ ": " ^ Pm_error.message error
  ...
  | PackageDownloadQueued { package; version; _ } -> "Queued download for " ^ Package_name.to_string package ^ "@" ^ version
  | BuildGraphCreating -> "Creating build graph..."
  | BuildGraphCreated { nodes; duration_ms } -> "Created build graph: " ^ Int.to_string nodes ^ " nodes in " ^ Int.to_string duration_ms ^ "ms"
  ...
  | QueueStats { ready; waiting; busy } -> "Queue: " ^ Int.to_string ready ^ " ready, " ^ Int.to_string waiting ^ " waiting, " ^ Int.to_string busy ^ " busy"
  | DependencyMissing { package; missing } -> Package_name.to_string package ^ " waiting for: " ^ String.concat ", " (List.map missing ~fn:Package_name.to_string)
  | DependencySatisfied { package } -> Package_name.to_string package ^ " dependencies satisfied"
  | CompilingInterface { package; file } -> "[" ^ Package_name.to_string package ^ "] Compiling interface " ^ file
  | CompilingImplementation { package; file } -> "[" ^ Package_name.to_string package ^ "] Compiling " ^ file
  | LinkingLibrary { package; output } -> "[" ^ Package_name.to_string package ^ "] Linking library " ^ output
  | LinkingExecutable { package; output } -> "[" ^ Package_name.to_string package ^ "] Linking executable " ^ output
  ...
  | WorkerPoolCreating { workers } -> "Creating worker pool with " ^ Int.to_string workers ^ " workers"
  | WorkerPoolCreated { workers; duration_ms } -> "Worker pool created with " ^ Int.to_string workers ^ " workers in " ^ Int.to_string duration_ms ^ "ms"
...
let package_names_of_json = function
  | Json.Array packages ->
      let rec loop acc = function
        | [] -> Ok (List.reverse acc)
        | json :: rest ->
            let* package = package_name_of_json json in loop (package :: acc) rest
      in
      loop [] packages
...
let kind_to_json = function
  ...
  | CacheStored { package; hash; artifacts } ->
      Json.Object [
        "package", package_name_json package;
        "hash", Json.String hash;
        "artifacts", Json.Array (List.map artifacts ~fn:(
          fun a -> Json.String a
        ));
      ]
  ...
  | DependencyResolutionUnlocking { path } ->
      Json.Object [
        "path", json_of_string_option path;
      ]
...
let to_json = fun event ->
  ...
  let clean_event =
    ...
    | CompileError { package; error } ->
        let clean_error = { error with raw = strip_ansi_codes error.raw; hint = strip_ansi_codes error.hint } in { event with kind = CompileError { package; error = clean_error } }
    | _ -> event
  in
  Json.Object [
    ...
    | Some (Json.String event_name) -> (
      ...
      | "riot.build.cache.stored" -> (
        ...
        | Json.Object data_fields ->
            ...
            let artifacts =
              ...
              | Some (Json.Array arr) -> List.filter_map ~fn:(
                function
        ...
        | Json.Object data_fields ->
            ...
            let error_kind =
              if message = "Syntax error" then
                ...
                if String.starts_with ~prefix:"Unbound value " message then
                  UnboundValue { name = String.sub message ~offset:14 ~len:(String.length message - 14) }
                else
                  if String.starts_with ~prefix:"Unbound module " message then
                    UnboundModule { name = String.sub message ~offset:15 ~len:(String.length message - 15) }
                  else
                    if String.starts_with ~prefix:"Cannot find file " message then
                      FileNotFound { filename = String.sub message ~offset:17 ~len:(String.length message - 17) }
      ...
      | "riot.pm.lockfile.read.finished" -> (
        ...
        | Json.Object data_fields -> (
          ...
          | Some (Json.String path), Some (Json.Int duration_ms) -> Ok (LockfileReadFinished { path; duration_ms })
      ...
      | "riot.pm.lockfile.write.finished" -> (
        ...
        | Json.Object data_fields -> (
          match Fields.get "path" data_fields, Fields.get "duration_ms" data_fields with
          | Some (Json.String path), Some (Json.Int duration_ms) -> Ok (LockfileWriteFinished { path; duration_ms })
      ...
      | "riot.pm.resolution.finished" -> (
        match data with
        | Json.Object data_fields -> (
          match Fields.get "duration_ms" data_fields, Fields.get "resolved_packages" data_fields, Fields.get "resolved_edges" data_fields with
          | Some (Json.Int duration_ms), Some (Json.Int resolved_packages), Some (Json.Int resolved_edges) -> Ok (DependencyResolutionFinished { duration_ms; resolved_packages; resolved_edges })
      ...
      | "riot.pm.universe.built" -> (
        match data with
        | Json.Object data_fields -> (
          match Fields.get "runtime_packages" data_fields, Fields.get "build_packages" data_fields, Fields.get "dev_packages" data_fields, Fields.get "duration_ms" data_fields with
          | Some (Json.Int runtime_packages), Some (Json.Int build_packages), Some (Json.Int dev_packages), Some (Json.Int duration_ms) ->
              Ok (
      ...
      | "riot.pm.package_metadata.fetch.started" -> (
        ...
        | Json.Object data_fields -> (
          ...
          | Some (Json.String registry), Some package_json -> (
            let* package = package_name_of_json package_json in Ok (PackageMetadataFetchStarted { registry; package })
          )
      ...
      | "riot.pm.package_metadata.fetch.finished" -> (
        match data with
        | Json.Object data_fields -> (
          match Fields.get "registry" data_fields, Fields.get "package" data_fields, Fields.get "duration_ms" data_fields with
          | Some (Json.String registry), Some package_json, Some (Json.Int duration_ms) ->
              let version =
                ...
                | None -> None
              in
              let* package = package_name_of_json package_json
              in
              Ok (
      ...
      | "riot.pm.package_metadata.fetch.failed" -> (
        match data with
        | Json.Object data_fields -> (
          match Fields.get "registry" data_fields, Fields.get "package" data_fields, Fields.get "error" data_fields with
      ...
      | "riot.pm.manifest.updated" -> (
        match data with
        | Json.Object data_fields -> (
          match Fields.get "path" data_fields, Fields.get "section" data_fields, Fields.get "operation" data_fields, Fields.get "dependency" data_fields with
      ...
      | "riot.pm.package.locked" -> (
        ...
        | Json.Object data_fields -> (
          ...
          | Some package_json, Some (Json.String version) ->
              let* package = package_name_of_json package_json in Ok (PackageVersionLocked { package; version })
      ...
      | "riot.pm.package.updated" -> (
        match data with
        | Json.Object data_fields -> (
          match Fields.get "package" data_fields, Fields.get "from_version" data_fields, Fields.get "to_version" data_fields with
          | Some package_json, Some (Json.String from_version), Some (Json.String to_version) ->
              let* package = package_name_of_json package_json in Ok (PackageVersionUpdated { package; from_version; to_version })
      ...
      | "riot.pm.package_manifest.fetch.started" -> (
        ...
        | Json.Object data_fields -> (
          match Fields.get "package" data_fields, Fields.get "version" data_fields with
          | Some package_json, Some (Json.String version) ->
              let* package = package_name_of_json package_json in Ok (PackageManifestFetchStarted { package; version })
      ...
      | "riot.pm.package_manifest.fetch.finished" -> (
        match data with
        | Json.Object data_fields -> (
          match Fields.get "package" data_fields, Fields.get "version" data_fields, Fields.get "duration_ms" data_fields with
          | Some package_json, Some (Json.String version), Some (Json.Int duration_ms) ->
              let* package = package_name_of_json package_json in Ok (PackageManifestFetchFinished { package; version; duration_ms })
      ...
      | "riot.pm.package_download.started" -> (
        match data with
        | Json.Object data_fields -> (
          match Fields.get "package" data_fields, Fields.get "version" data_fields, Fields.get "path" data_fields with
          | Some package_json, Some (Json.String version), Some (Json.String path) ->
              let* package = package_name_of_json package_json in Ok (PackageDownloadStarted { package; version; path })
      ...
      | "riot.pm.package_download.finished" -> (
        match data with
        | Json.Object data_fields -> (
          match Fields.get "package" data_fields, Fields.get "version" data_fields, Fields.get "path" data_fields, Fields.get "duration_ms" data_fields with
      ...
      | "riot.pm.package_download.failed" -> (
        match data with
        | Json.Object data_fields -> (
          match Fields.get "package" data_fields, Fields.get "version" data_fields, Fields.get "path" data_fields, Fields.get "error" data_fields with
      ...
      | "riot.pm.package_download.skipped" -> (
        match data with
        | Json.Object data_fields -> (
          match Fields.get "package" data_fields, Fields.get "version" data_fields, Fields.get "path" data_fields, Fields.get "reason" data_fields with
      ...
      | "riot.pm.package_cache.hit" -> (
        ...
        | Json.Object data_fields -> (
          match Fields.get "package" data_fields, Fields.get "version" data_fields, Fields.get "path" data_fields with
          | Some package_json, Some (Json.String version), Some (Json.String path) ->
              let* package = package_name_of_json package_json in Ok (PackageCacheHit { package; version; path })
      ...
      | "riot.pm.package_materialization.started" -> (
        ...
        | Json.Object data_fields -> (
          match Fields.get "package" data_fields, Fields.get "version" data_fields, Fields.get "path" data_fields with
          | Some package_json, Some (Json.String version), Some (Json.String path) ->
              let* package = package_name_of_json package_json in Ok (PackageMaterializationStarted { package; version; path })
      ...
      | "riot.pm.package_resolved_for_build" -> (
        match data with
        | Json.Object data_fields -> (
          match Fields.get "package" data_fields, Fields.get "path" data_fields, Fields.get "workspace" data_fields with
      ...
      | "riot.pm.package_download.queued" -> (
        ...
        | Json.Object data_fields -> (
          match Fields.get "package" data_fields, Fields.get "version" data_fields, Fields.get "path" data_fields with
          | Some package_json, Some (Json.String version), Some (Json.String path) ->
              let* package = package_name_of_json package_json in Ok (PackageDownloadQueued { package; version; path })
...
let from_json = fun json ->
  ...
  | Json.Object fields -> (
    let timestamp =
      ...
      | Some (Json.String _ts) -> (* For now, use current time - proper timestamp parsing can be added later *)
      DateTime.now ()
      | _ -> DateTime.now ()
    in
    let session_id =
...
module Tests = struct
  let package_name = fun name -> Result.expect (Package_name.from_string name) ~msg:("package name " ^ name)

  let test_lockfile_event_json_roundtrip (): (unit, string) result =
    let event = create ~session_id:(Session_id.of_string "test-session") ~level:Info (LockfileReadFinished { path = "/tmp/workspace/riot.lock"; duration_ms = 12 }) in
    ...
    | Ok { kind = LockfileReadFinished { path; duration_ms }; _ } ->
        if String.equal path "/tmp/workspace/riot.lock" && duration_ms = 12 then
          Ok ()
        else Error "expected lockfile read event to round-trip"
  ...
  let test_resolution_event_json_roundtrip (): (unit, string) result =
    let event =
      create ~session_id:(Session_id.of_string "test-session") ~level:Info
        (
    ...
    | Ok { kind = DependencyResolutionStarted { packages; mode = `Unlock }; _ } ->
        if packages = [ package_name "app"; package_name "std" ] then
          Ok ()
        else Error "expected dependency resolution packages to round-trip"
  ...
  let test_package_resolved_event_json_roundtrip (): (unit, string) result =
    ...
    | Ok { kind = PackageResolvedForBuild { package; version; path; workspace }; _ } ->
        if Package_name.equal package (package_name "std") && version = Some "0.1.0" && String.equal path "/Users/example/.riot/registry/pkgs.ml/src/std/0.1.0" && not workspace then
          Ok ()
        else Error "expected package resolved event to round-trip"
  ...
  let test_manifest_update_event_json_roundtrip (): (unit, string) result =
    ...
    | Ok { kind = DependencyManifestUpdated { path; section; operation = `Add; dependency }; _ } ->
        if String.equal path "/tmp/workspace/riot.toml" && String.equal section "dependencies" && String.equal dependency "std" then
          Ok ()
        else Error "expected dependency manifest update event to round-trip"
  ...
  let test_package_locked_event_json_roundtrip (): (unit, string) result =
    let event = create ~session_id:(Session_id.of_string "test-session") ~level:Info (PackageVersionLocked { package = package_name "std"; version = "0.2.0" }) in
    match from_json (to_json event) with
    | Ok { kind = PackageVersionLocked { package; version }; _ } ->
        if Package_name.equal package (package_name "std") && String.equal version "0.2.0" then
          Ok ()
        else Error "expected package locked event to round-trip"
  ...
  let test_package_versions_unchanged_event_json_roundtrip (): (unit, string) result =
    let event = create ~session_id:(Session_id.of_string "test-session") ~level:Info (PackageVersionsUnchanged { packages = 3 }) in
    match from_json (to_json event) with
    | Ok { kind = PackageVersionsUnchanged { packages }; _ } ->
        if Int.equal packages 3 then
          Ok ()
        else Error "expected package versions unchanged event to round-trip"
