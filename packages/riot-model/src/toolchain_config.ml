...
let sort_compare_target = fun left right -> String.compare (System.TargetTriple.to_string left) (System.TargetTriple.to_string right)
        ...
        | Some (Data.Toml.Table toolchain_items) -> (
          ...
          let targets =
            (
              ...
              | Some (Data.Toml.Array arr) -> List.filter_map arr ~fn:(
                function
                | Data.Toml.String s -> (
          ...
          | Some (Data.Toml.Table version_items) -> (
            ...
            | Some (Data.Toml.String path_str) -> (
              ...
              | Ok source_path ->
                  let version_name = Path.basename source_path ^ "-local" in { version = version_name; source = Path source_path; targets }
              | Error _ -> default
            )
            | _ -> (
