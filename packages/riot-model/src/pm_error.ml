...
type required_by = { package: string; path: Path.t option }
...
let rec headline = function
  | ManifestReadFailed { manifest_path; error } -> "failed to read manifest '" ^ Path.to_string manifest_path ^ "': " ^ error
  | ManifestParseFailed { manifest_path; error } -> "failed to parse manifest '" ^ Path.to_string manifest_path ^ "': " ^ error
  | PathDependencyLoadFailed { dependency_name; dependency_path; error } -> "failed to load path dependency '" ^ dependency_name ^ "' from " ^ Path.to_string dependency_path ^ ": " ^ message error
  | PathDependencyDecodeFailed { dependency_name; manifest_path; error } -> "failed to decode path dependency '" ^ dependency_name ^ "' from " ^ Path.to_string manifest_path ^ ": " ^ error
  | SourceDependencyLoadFailed { dependency_name; source_locator; ref_; error } ->
      let suffix =
        ...
        | None -> ""
      in
      "failed to load source dependency '" ^ dependency_name ^ "' from " ^ source_locator ^ suffix ^ ": " ^ error
  | SourceDependencyDecodeFailed { dependency_name; manifest_path; error } -> "failed to decode source dependency '" ^ dependency_name ^ "' from " ^ Path.to_string manifest_path ^ ": " ^ error
  | RegistryLatestReleaseMissing { package; latest_version } -> "registry package '" ^ package ^ "' declares latest version '" ^ latest_version ^ "' but that release is missing from the sparse index document"
  | RegistryReleaseYanked { package; registry; version; required_by = _ } -> "package `" ^ package ^ "@" ^ version ^ "` was yanked from registry `" ^ registry ^ "`"
  | PackageMetadataReadFailed { package; error; _ } -> "failed to read package document for '" ^ package ^ "': " ^ error
  | PackageNotFound { package; registry; required_by = _ } -> "package `" ^ package ^ "` was not found in registry `" ^ registry ^ "`"
  | RegistryVersionNotFound { package; registry; requirement; required_by = _; _ } -> "package `" ^ package ^ "` has no release matching `" ^ requirement ^ "` in registry `" ^ registry ^ "`"
  | LockfileReadFailed { path; error } -> "failed to read lockfile '" ^ Path.to_string path ^ "': " ^ error
  | LockRefreshCheckFailed { workspace_root; error } -> "failed to check lock freshness for workspace '" ^ Path.to_string workspace_root ^ "': " ^ error
  | LockfileWriteFailed { path; error } -> "failed to write lockfile '" ^ Path.to_string path ^ "': " ^ error
...
and detail_lines = function
  ...
  | RegistryReleaseYanked { required_by = Some required_by; _ } -> [ format_required_by required_by ]
  | _ -> []
and message error =
...
let path_of_json = function
  | Json.String path -> (
    ...
    | Ok path -> Ok path
    | Error (Path.InvalidUtf8 { path = invalid_path }) -> Error ("invalid path '" ^ path ^ "': invalid utf-8 in '" ^ invalid_path ^ "'")
    | Error (Path.SystemInvalidUtf8 { syscall; path = invalid_path }) -> Error ("invalid path '" ^ path ^ "': " ^ syscall ^ " returned invalid utf-8 for '" ^ invalid_path ^ "'")
...
let rec to_json = function
  ...
  | RegistryVersionNotFound { package; registry; requirement; available_versions; required_by } ->
      Json.Object [
        ...
        "available_versions", Json.Array (List.map available_versions ~fn:(
          fun version -> Json.String version
        ));
        "required_by", (
          match required_by with
          | None -> Json.Null
...
let rec of_json = function
  | Json.Object fields -> (
    ...
    | Some (Json.String "ManifestReadFailed") -> (
      ...
      | Some path_json, Some (Json.String error) -> path_of_json path_json |> Result.map ~fn:(
        fun manifest_path -> ManifestReadFailed { manifest_path; error }
      )
    ...
    | Some (Json.String "ManifestParseFailed") -> (
      match Fields.get "manifest_path" fields, Fields.get "error" fields with
      | Some path_json, Some (Json.String error) -> path_of_json path_json |> Result.map ~fn:(
        fun manifest_path -> ManifestParseFailed { manifest_path; error }
      )
      | _ -> Error "invalid ManifestParseFailed"
    )
    | Some (Json.String "PathDependencyLoadFailed") -> (
      match Fields.get "dependency_name" fields, Fields.get "dependency_path" fields, Fields.get "error" fields with
      | Some (Json.String dependency_name), Some path_json, Some error_json -> (
        match path_of_json path_json, of_json error_json with
        | Ok dependency_path, Ok error -> Ok (PathDependencyLoadFailed { dependency_name; dependency_path; error })
        | (Error err, _) | (_, Error err) -> Error err
      )
      | _ -> Error "invalid PathDependencyLoadFailed"
    )
    | Some (Json.String "PathDependencyDecodeFailed") -> (
      match Fields.get "dependency_name" fields, Fields.get "manifest_path" fields, Fields.get "error" fields with
      | Some (Json.String dependency_name), Some path_json, Some (Json.String error) -> path_of_json path_json |> Result.map ~fn:(
        fun manifest_path -> PathDependencyDecodeFailed { dependency_name; manifest_path; error }
      )
      | _ -> Error "invalid PathDependencyDecodeFailed"
    )
    | Some (Json.String "SourceDependencyLoadFailed") -> (
      match Fields.get "dependency_name" fields, Fields.get "source_locator" fields, Fields.get "ref" fields, Fields.get "error" fields with
    ...
    | Some (Json.String "SourceDependencyDecodeFailed") -> (
      match Fields.get "dependency_name" fields, Fields.get "manifest_path" fields, Fields.get "error" fields with
      | Some (Json.String dependency_name), Some path_json, Some (Json.String error) -> path_of_json path_json |> Result.map ~fn:(
        fun manifest_path -> SourceDependencyDecodeFailed { dependency_name; manifest_path; error }
      )
    ...
    | Some (Json.String "RegistryLatestReleaseMissing") -> (
      ...
      | Some (Json.String package), Some (Json.String latest_version) -> Ok (RegistryLatestReleaseMissing { package; latest_version })
      | _ -> Error "invalid RegistryLatestReleaseMissing"
    )
    | Some (Json.String "RegistryReleaseYanked") -> (
      match Fields.get "package" fields, Fields.get "registry" fields, Fields.get "version" fields, Fields.get "required_by" fields with
          ...
          let required_by =
            ...
            | Some (Json.Object required_by_fields) -> (
              ...
              | Some (Json.String package), Some path_json -> (
                ...
                | _ -> path_of_json path_json |> Result.map ~fn:(
                  fun path -> Some { package; path = Some path }
                )
              )
    ...
    | Some (Json.String "PackageMetadataReadFailed") -> (
      ...
      | Some (Json.String package), Some (Json.String registry), Some (Json.String error) -> Ok (PackageMetadataReadFailed { package; registry; error })
    ...
    | Some (Json.String "PackageNotFound") -> (
      ...
      | Some (Json.String package), Some (Json.String registry) ->
            ...
            | Some (Json.Object required_by_fields) -> (
              ...
              | Some (Json.String package) ->
                  let path_result =
                    ...
                    | Some Json.Null | None -> Ok None
                    | Some path_json -> path_of_json path_json |> Result.map ~fn:(
                      fun path -> Some path
                    )
                  in
                  path_result |> Result.map ~fn:(
                    fun path -> Some { package; path }
                  )
          ...
          in
          required_by |> Result.map ~fn:(
            fun required_by -> PackageNotFound { package; registry; required_by }
          )
      | _ -> Error "invalid PackageNotFound"
    )
    | Some (Json.String "RegistryVersionNotFound") -> (
      match Fields.get "package" fields, Fields.get "registry" fields, Fields.get "requirement" fields, Fields.get "available_versions" fields, Fields.get "required_by" fields with
    ...
    | Some (Json.String "LockfileReadFailed") -> (
      ...
      | Some path_json, Some (Json.String error) -> path_of_json path_json |> Result.map ~fn:(
        fun path -> LockfileReadFailed { path; error }
      )
    ...
    | Some (Json.String "LockRefreshCheckFailed") -> (
      ...
      | Some path_json, Some (Json.String error) -> path_of_json path_json |> Result.map ~fn:(
        fun workspace_root -> LockRefreshCheckFailed { workspace_root; error }
      )
    ...
    | Some (Json.String "LockfileWriteFailed") -> (
      match Fields.get "path" fields, Fields.get "error" fields with
      | Some path_json, Some (Json.String error) -> path_of_json path_json |> Result.map ~fn:(
        fun path -> LockfileWriteFailed { path; error }
      )
