...
let dependency_error_message = function
  | InvalidDependencyName { raw_name; error } -> "dependency name '" ^ raw_name ^ "' is invalid: " ^ Package_name.error_message error
  ...
  | DependencyCannotUseWorkspaceFlag { dependency_name } -> "workspace dependency '" ^ dependency_name ^ "' cannot use workspace = true"
  | DependencyFieldMustBeString { dependency_name; field } -> "dependency '" ^ dependency_name ^ "' has non-string " ^ dependency_field_name field
  ...
  | BuiltinDependencyDoesNotSupportOverrides { dependency_name } -> "builtin dependency '" ^ dependency_name ^ "' does not support path or source overrides"
  | BuiltinDependencyDoesNotSupportVersionRequirement { dependency_name; requirement } -> "builtin dependency '" ^ dependency_name ^ "' does not support version requirement '" ^ requirement ^ "'"
...
let validate_requirement = fun ~dependency_name requirement ->
  ...
  | Error err -> Error (DependencyError (InvalidDependencyRequirement { dependency_name; requirement; error = err }))
    ...
    if Option.is_some source.ref_ && Option.is_none source.source_locator then
      ...
      if source.builtin && (Option.is_some source.path || Option.is_some source.source_locator || Option.is_some source.ref_) then
        ...
        if source.builtin then
          ...
          | Some version -> Error (DependencyError (BuiltinDependencyDoesNotSupportVersionRequirement { dependency_name; requirement = Version.requirement_to_string version }))
        else
          if Option.is_some source.path || Option.is_some source.source_locator || Option.is_some source.version then
            Ok source
...
let parse_dependency: string -> Toml.value -> (Package.dependency, error) result = fun raw_name value ->
  let* name = Package_name.from_string raw_name |> Result.map_err ~fn:(
    fun error -> DependencyError (InvalidDependencyName { raw_name; error })
  )
  in
  ...
  | Toml.Table attrs -> (
    let path =
      ...
      | Some _ -> Error (DependencyError (DependencyFieldMustBeString { dependency_name; field = Path }))
      | None -> Ok None
    in
    let source_locator =
      match Fields.get "source" attrs, Fields.get "github" attrs with
      | Some _, Some _ -> Error (DependencyError (DependencyCannotSpecifySourceAndGithub { dependency_name }))
      | Some (Toml.String locator), None -> Ok (Some (normalize_source_locator locator))
      | Some _, None -> Error (DependencyError (DependencyFieldMustBeString { dependency_name; field = Source }))
      | None, Some (Toml.String github) -> Ok (Some (github_locator_of_value github))
      | None, Some _ -> Error (DependencyError (DependencyFieldMustBeString { dependency_name; field = Github }))
      | None, None -> Ok None
    in
    let ref_ =
      match Fields.get "ref" attrs with
      | Some (Toml.String ref_) -> Ok (Some (String.trim ref_))
      | Some _ -> Error (DependencyError (DependencyFieldMustBeString { dependency_name; field = Ref }))
      | None -> Ok None
    in
    let version =
      ...
      | Some _ -> Error (DependencyError (DependencyFieldMustBeString { dependency_name; field = Version }))
      | None -> Ok None
    in
...
let parse_members: Toml.value -> Path.t list = fun toml ->
  ...
  | Toml.Table items -> (
    ...
    | Some (Toml.Table workspace_items) -> (
      ...
      | Some (Toml.Array members) -> List.filter_map members ~fn:(
        fun m -> Option.map (Toml.get_string m) ~fn:Path.v
      )
      | _ -> []
    )
    | _ -> []
...
let parse_workspace_dependencies: Toml.value -> Package.dependency list = fun toml ->
  ...
  parse_dependency_section "dependencies" toml |> Result.map_err ~fn:error_message |> Result.expect ~msg:"workspace dependencies should be parsed through of_toml"

let parse_workspace_dev_dependencies: Toml.value -> Package.dependency list = fun toml -> parse_dependency_section "dev-dependencies" toml |> Result.map_err ~fn:error_message |> Result.expect ~msg:"workspace dev dependencies should be parsed through of_toml"

let parse_workspace_build_dependencies: Toml.value -> Package.dependency list = fun toml -> parse_dependency_section "build-dependencies" toml |> Result.map_err ~fn:error_message |> Result.expect ~msg:"workspace build dependencies should be parsed through of_toml"
...
let parse_profile_overrides: Toml.value -> (string * Profile.profile_override) list = fun toml ->
  ...
  | Toml.Table items -> (
    ...
    Log.debug ("[WORKSPACE] Top-level keys: " ^ String.concat ", " (List.map items ~fn:(
      fun (key, _) -> key
    )));
    ...
    | Some (Toml.Table profile_items) ->
        ...
        let result = List.filter_map profile_items ~fn:(
...
|} |> Result.expect ~msg:"expected test toml to parse" in
    ...
    if manifest.target_dir = Some "build-out" then
      Ok ()
    else Error "expected [riot].target_dir to be parsed" [@test]
...
|} |> Result.expect ~msg:"expected test toml to parse" in
    ...
    if manifest.name = Some "riot" then
      Ok ()
    else Error "expected [workspace].name to be parsed" [@test]
  ...
  let test_make_uses_custom_target_dir (): (unit, string) result =
    ...
    if Path.to_string workspace.target_dir_root = "/tmp/example/build-out" then
      Ok ()
    else Error "expected custom target_dir_root" [@test]
...
|} |> Result.expect ~msg:"expected workspace toml to parse" in
    ...
    | Ok manifest -> (
      ...
      | [ { Package.source = { workspace = false; builtin = false; path = None; source_locator = None; ref_ = None; version = Some requirement }; _ } ] ->
          if String.equal (Version.requirement_to_string requirement) ">= 1.2.3" then
            Ok ()
          else Error "expected workspace registry requirement to be parsed structurally"
...
|} |> Result.expect ~msg:"expected workspace toml to parse" in
  ...
  let test_discover_fix_providers (): (unit, string) result =
    let package_toml = Std.Data.Toml.parse {|
[package]
name = "std"
version = "0.1.0"
...
|} |> Result.expect ~msg:"expected package toml to parse" in
    let package = Package.from_toml package_toml ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:(Path.v "/tmp/example/packages/std") ~relative_path:(Path.v "packages/std") |> Result.expect ~msg:"expected package manifest" in
    let workspace = make_realized ~root:(Path.v "/tmp/example") ~packages:[ package ] () in
    match discover_fix_providers workspace with
    | [ provider ] ->
        if Package_name.equal provider.package_name (Package_name.from_string "std" |> Result.expect ~msg:"expected valid package name") && String.equal (Path.to_string provider.source_path) "/tmp/example/packages/std/fix/no_stdlib_provider.ml" && String.equal provider.name "std" && provider.rules = [ "std:no-stdlib" ] then
          Ok ()
...
|} |> Result.expect ~msg:"expected test toml to parse" in
    let manifest = of_toml toml |> Result.expect ~msg:"expected workspace manifest" in
    if List.map manifest.dependencies ~fn:(
      fun (dep: Package.dependency) -> dep.Package.name
    ) = [ package_name "std" ] && List.map manifest.dev_dependencies ~fn:(
      fun (dep: Package.dependency) -> dep.Package.name
    ) = [ package_name "propane" ] && List.map manifest.build_dependencies ~fn:(
      fun (dep: Package.dependency) -> dep.Package.name
    ) = [ package_name "fixme" ] then
      Ok ()
    else Error "expected workspace dependency classes to parse" [@test]
end [@test]
