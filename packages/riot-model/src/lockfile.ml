...
let error_message = function
  ...
  | MissingField { container; field } -> container_name container ^ " is missing required field '" ^ field ^ "'"
  | InvalidFieldType { container; field; expected } -> container_name container ^ " field '" ^ field ^ "' must be " ^ expected
  | InvalidPackageName { container; field; value; error } -> container_name container ^ " field '" ^ field ^ "' contains invalid package name '" ^ value ^ "': " ^ Package_name.error_message error
...
let parse_package_name = fun container ~field value -> Package_name.from_string value |> Result.map_err ~fn:(
  fun error ->
    InvalidPackageName {
      container;
      field;
...
let package_id_to_toml = fun (id: package_id) ->
  let fields =
    [
      "name", Toml.String (Package_name.to_string id.name);
    ]
  in
  let fields =
...
let package_id_of_toml = fun value ->
  let* fields = require_table PackageId value
  in
  let* raw_name = required_string_field PackageId ~field:"name" fields
  in
  let* name = parse_package_name PackageId ~field:"name" raw_name
  in
...
let package_id_of_fields = fun fields ->
  let* raw_name = required_string_field Package ~field:"name" fields
  in
  let* name = parse_package_name Package ~field:"name" raw_name
  in
  let registry = optional_string_field ~field:"registry" fields in
  let version = optional_string_field ~field:"version" fields in
  let sha256 = optional_string_field ~field:"sha256" fields in
...
let provenance_of_toml = fun value ->
  let* fields = require_table Provenance value
  in
  let* kind = required_string_field Provenance ~field:"kind" fields
  in
  match kind with
  ...
  | "source" ->
      let* locator = required_string_field Provenance ~field:"locator" fields
      in
      let ref_ = optional_string_field ~field:"ref" fields in Ok (Source { locator; ref_ })
  | "registry" ->
      let* registry = required_string_field Provenance ~field:"registry" fields in Ok (Registry { registry })
  ...
  if should_use_flat_registry_shape then
    ...
    let fields =
      if Package_name.equal dep.package.name dep.name then
        fields
      else ("package_name", Toml.String (Package_name.to_string dep.package.name)) :: fields
    in
    let fields =
...
let dependency_of_toml = fun value ->
  let* fields = require_table Dependency value
  in
  let* raw_name = required_string_field Dependency ~field:"name" fields
  in
  let* name = parse_package_name Dependency ~field:"name" raw_name
  in
  match Fields.get "package" fields with
  | Some package_value -> package_id_of_toml package_value |> Result.map ~fn:(
    fun package -> { name; package }
  )
  | None ->
      let package_name =
        ...
        | Some _ -> Error (InvalidFieldType { container = Dependency; field = "package_name"; expected = "a string" })
        | None -> Ok name
      in
      ...
      let registry =
        ...
        | _ ->
            if List.any fields ~fn:(
              fun (field_name, _value) -> String.equal field_name "version"
            ) || List.any fields ~fn:(
              fun (field_name, _value) -> String.equal field_name "sha256"
            ) then
              Some "pkgs.ml"
            else None
      in
...
let dependency_list_of_toml = fun value ->
  let* items = require_array DependencyList value
  in
...
let package_of_toml = fun value ->
  let* fields = require_table Package value
  in
  ...
  in
  let* dependencies_value = required_array_field Package ~field:"dependencies" fields
  in
  let* build_dependencies_value = required_array_field Package ~field:"build_dependencies" fields
  in
  let* dev_dependencies_value = required_array_field Package ~field:"dev_dependencies" fields
  in
  let* id =
  ...
  in
  let* provenance = provenance_of_toml provenance_value
  in
  let* dependencies = dependency_list_of_toml (Toml.Array dependencies_value)
  in
  let* build_dependencies = dependency_list_of_toml (Toml.Array build_dependencies_value)
  in
  let* dev_dependencies = dependency_list_of_toml (Toml.Array dev_dependencies_value)
  in
  Ok {
    id;
    root;
...
let to_toml = fun (lockfile: t) ->
  ...
  in
  let fields = ("packages", Toml.Array (List.map lockfile.packages ~fn:package_to_toml)) :: fields in Toml.Table (List.reverse fields)

let of_toml = fun value ->
  let* fields = require_table Lockfile value
  in
  let* format_version = required_int_field Lockfile ~field:"format_version" fields
  in
  let* dependency_hash = required_string_field Lockfile ~field:"dependency_hash" fields
  in
  let* packages = required_array_field Lockfile ~field:"packages" fields
  in
  let rec loop acc = function
...
let render_package_id = fun (id: package_id) ->
  let fields =
    [
      "name", render_string (Package_name.to_string id.name);
    ]
  in
  let fields =
    match id.registry with
  ...
  let fields =
    ...
    | None -> fields
  in
  "{ " ^ String.concat ", " (fields |> List.reverse |> List.map ~fn:(
...
let render_provenance = fun provenance ->
  ...
  | Path path -> "{ kind = " ^ render_string "path" ^ ", path = " ^ render_string (Path.to_string path) ^ " }"
  | Source { locator; ref_ } ->
      let fields =
        [
  ...
  | Registry { registry } -> "{ kind = " ^ render_string "registry" ^ ", registry = " ^ render_string registry ^ " }"
...
let render_dependency = fun (dep: dependency) ->
  ...
  if is_flat_registry_dependency then
    let fields =
      [
        "name", render_string (Package_name.to_string dep.name);
      ]
    in
    let fields =
      if Package_name.equal dep.package.name dep.name then
        fields
      else ("package_name", render_string (Package_name.to_string dep.package.name)) :: fields
    in
    let fields =
      match dep.package.version with
      | Some version -> ("version", render_string version) :: fields
  ...
  else "{ name = " ^ render_string (Package_name.to_string dep.name) ^ ", package = " ^ render_package_id dep.package ^ " }"

let render_dependency_list = fun deps -> "[" ^ String.concat ", " (List.map deps ~fn:render_dependency) ^ "]"
...
let to_string = fun lockfile ->
  let parts =
    [ "format_version = " ^ Int.to_string lockfile.format_version; "dependency_hash = " ^ render_string lockfile.dependency_hash ] @ (
      if List.is_empty lockfile.packages then
        [ "packages = []" ]
      else List.map lockfile.packages ~fn:render_package
    )
  in
  String.concat "\n\n" parts ^ "\n"
