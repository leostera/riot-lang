...
let resolve_source_path = fun provider_items ~package_path ->
  ...
  | _ -> default_source_paths
  |> List.find
    ~fn:(fun rel_path -> Fs.exists Path.(package_path / rel_path) |> Result.unwrap_or ~default:false)
...
let parse_provider = fun provider_toml ~package_name ~package_path ->
  ...
  | Toml.Table provider_items -> (
      ...
      let rules =
        ...
        | Some (Toml.Array items) -> List.filter_map items ~fn:Toml.get_string
