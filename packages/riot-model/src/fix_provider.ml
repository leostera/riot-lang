...
let resolve_source_path = fun provider_items ~package_path ->
  ...
  | _ -> default_source_paths |> List.find ~fn:(
    fun rel_path -> Fs.exists Path.(package_path / rel_path) |> Result.unwrap_or ~default:false
...
let parse_provider = fun provider_toml ~package_name ~package_path ->
  ...
  | Toml.Table provider_items -> (
    ...
    let rules =
      ...
      | Some (Toml.Array items) -> List.filter_map items ~fn:Toml.get_string |> List.map ~fn:(normalize_rule_id package_name)
      | _ -> []
    in
    Some {
...
let parse_from_toml = fun items ~package_name ~package_path ->
  ...
  | Some (Toml.Table riot_items) -> (
    ...
    | Some (Toml.Table fix_items) -> (
      ...
      | Some provider -> parse_provider provider ~package_name ~package_path |> Option.to_list
      | None -> []
    )
    | _ -> []
  )
...
let to_json = fun provider ->
  Json.Object [
    ...
    "rules", Json.Array (List.map provider.rules ~fn:(
      fun rule -> Json.String rule
    ));
  ]
