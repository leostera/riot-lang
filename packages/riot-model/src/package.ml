...
let canonicalize = fun (pkg: t) ->
  {
    pkg
    with dependencies = List.sort pkg.dependencies ~compare:compare_dependency;
...
|}
  ...
  let test_package_json_round_trips_source_dependency (): (unit, string) result =
    ...
    | Ok {
      dependencies=[
        { name; source={ source_locator=Some "github.com/riot-tests/widgets"; ref_=Some "main"; _ } }
      ];
      _
    } when Package_name.equal name (package_name "widgets") -> Ok ()
