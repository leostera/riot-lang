...
type dependency = { name: Package_name.t; source: dependency_source }

type resolved_dependency = { requirement: dependency; resolved_id: Lockfile.package_id }
      ...
      if by_path != Order.EQ then
        ...
        if by_source_locator != Order.EQ then
          ...
          if by_ref != Order.EQ then
            ...
            compare_option
              (
                fun left right -> String.compare (Version.requirement_to_string left) (Version.requirement_to_string right)
              )
              left.version
              right.version
...
let canonicalize_foreign_dependency = fun (foreign: foreign_dependency) -> { foreign with inputs = canonicalize_path_list foreign.inputs; outputs = canonicalize_path_list foreign.outputs }
...
let canonicalize_manifest_spec = fun (spec: manifest_spec) ->
  {
    ...
    foreign_dependencies = spec.foreign_dependencies |> List.map ~fn:canonicalize_foreign_dependency |> List.sort ~compare:compare_foreign_dependency;
    declared_binaries = List.sort spec.declared_binaries ~compare:compare_binary;
    compiler = { profile_overrides = List.sort spec.compiler.profile_overrides ~compare:compare_profile_override; target_overrides = List.sort spec.compiler.target_overrides ~compare:compare_target_override };
...
let canonicalize = fun (pkg: t) ->
  {
    ...
    foreign_dependencies = pkg.foreign_dependencies |> List.map ~fn:canonicalize_foreign_dependency |> List.sort ~compare:compare_foreign_dependency;
    binaries = List.sort pkg.binaries ~compare:compare_binary;
    sources = canonicalize_sources pkg.sources;
    compiler = { profile_overrides = List.sort pkg.compiler.profile_overrides ~compare:compare_profile_override; target_overrides = List.sort pkg.compiler.target_overrides ~compare:compare_target_override };
...
let is_builtin_dependency_name = fun name ->
  match name with
  | "unix" | "stdlib" | "threads" | "str" | "bigarray" | "dynlink" | "compiler-libs" | "graphics" -> true
  | _ -> false
...
let binary_scope: binary -> dependency_scope = fun (bin: binary) ->
  ...
  if String.starts_with ~prefix:"tests/" path_str || String.starts_with ~prefix:"examples/" path_str || String.starts_with ~prefix:"bench/" path_str then
    Dev
  else Normal
...
let dev_artifact_selected_for_binary = fun (dev_artifacts: dev_artifacts) (bin: binary) ->
  ...
  if String.starts_with ~prefix:"tests/" path_str then
    ...
    if String.starts_with ~prefix:"examples/" path_str then
      dev_artifacts.examples
    else
      if String.starts_with ~prefix:"bench/" path_str then
        dev_artifacts.benches
      else false

let scope_of_binary_name = fun (pkg: t) ~binary_name -> List.find pkg.binaries ~fn:(
  fun (bin: binary) -> String.equal bin.name binary_name
) |> Option.map ~fn:binary_scope

let binaries_for_scope = fun ?(dev_artifacts = all_dev_artifacts) scope (pkg: t) ->
  match scope with
  | Normal -> List.filter pkg.binaries ~fn:(
    fun bin -> binary_scope bin = Normal
  )
  | Dev -> List.filter pkg.binaries ~fn:(
    fun bin -> binary_scope bin = Dev && dev_artifact_selected_for_binary dev_artifacts bin
  )
  | Build -> []
...
let sources_for_scope = fun ?(dev_artifacts = all_dev_artifacts) scope (pkg: t) ->
  ...
  | Dev ->
      ...
      in
      {
        src;
        native = [];
        tests = if dev_artifacts.tests then
          pkg.sources.tests
        else [];
        examples = if dev_artifacts.examples then
          pkg.sources.examples
        else [];
        bench = if dev_artifacts.benches then
          pkg.sources.bench
        else []
      }
  | Build ->
  ...
  let rec loop (acc: resolved_dependency list) (requirements: dependency list) =
    ...
    | (requirement: dependency) :: rest ->
        if is_builtin_dependency requirement then
          ...
          (
            match List.find lock_dependencies ~fn:(
              fun (dep: Lockfile.dependency) -> Package_name.equal dep.name requirement.name
            ) with
            | Some resolved -> loop ({ requirement; resolved_id = resolved.package } :: acc) rest
            | None -> Error ("lockfile is missing resolved " ^ scope_name ^ " dependency '" ^ Package_name.to_string requirement.name ^ "'")
          )
  in
  loop [] manifest_dependencies

let resolve = fun ~(package:t) ~(lock_package:Lockfile.package) ~manifest_path ~materialized_root ->
  match resolve_scope ~scope_name:"runtime" ~manifest_dependencies:package.dependencies ~lock_dependencies:lock_package.dependencies with
  | Error _ as err -> err
  | Ok dependencies -> (
    match resolve_scope ~scope_name:"build" ~manifest_dependencies:package.build_dependencies ~lock_dependencies:lock_package.build_dependencies with
    | Error _ as err -> err
    | Ok build_dependencies -> (
      match resolve_scope ~scope_name:"dev" ~manifest_dependencies:package.dev_dependencies ~lock_dependencies:lock_package.dev_dependencies with
      | Error _ as err -> err
      | Ok dev_dependencies ->
          Ok {
            package;
...
let is_workspace_member: t -> bool = fun pkg ->
  let rel_str = Path.to_string pkg.relative_path in not (String.starts_with ~prefix:"../" rel_str || Path.is_absolute pkg.relative_path)
...
let publish_metadata_error_message = function
  ...
  | InvalidPackageVersion { package_name; version; error } -> "package '" ^ package_name ^ "' has invalid version '" ^ version ^ "': " ^ version_parse_error_to_string error
  | NonStringPublishField { package_name; field } -> "package '" ^ package_name ^ "' has non-string " ^ publish_field_name field
  | NonBooleanPublicFlag { package_name } -> "package '" ^ package_name ^ "' has non-boolean public flag"

let dependency_error_message = function
  | InvalidDependencyName { raw_name; error } -> "dependency '" ^ raw_name ^ "' has invalid package name: " ^ Package_name.error_message error
  | InvalidDependencyRequirement { dependency_name; requirement; error } -> "dependency '" ^ dependency_name ^ "' has invalid version requirement '" ^ requirement ^ "': " ^ version_parse_error_to_string error
  | NonBooleanWorkspaceFlag { dependency_name } -> "dependency '" ^ dependency_name ^ "' has non-boolean workspace flag"
  | NonStringDependencyField { dependency_name; field } -> "dependency '" ^ dependency_name ^ "' has non-string " ^ dependency_field_name field
  | DependencyCannotSpecifySourceAndGithub { dependency_name } -> "dependency '" ^ dependency_name ^ "' cannot specify both source and github"
  | WorkspaceDependencyCannotSpecifyOverrides { dependency_name } -> "dependency '" ^ dependency_name ^ "' cannot combine workspace = true with path, source, ref, or version"
  | DependencyRefRequiresSource { dependency_name } -> "dependency '" ^ dependency_name ^ "' cannot specify ref without source"
  | BuiltinDependencyCannotSpecifyOverrides { dependency_name } -> "builtin dependency '" ^ dependency_name ^ "' does not support path or source overrides"
  | BuiltinDependencyVersionRequirementNotSupported { dependency_name; requirement } -> "builtin dependency '" ^ dependency_name ^ "' does not support version requirement '" ^ requirement ^ "'"
...
let parse_name: (string * Toml.value) list -> string -> (Package_name.t, manifest_error) result = fun items fallback ->
  let raw_name =
    ...
    | _ -> fallback
  in
  Package_name.from_string raw_name |> Result.map_err ~fn:(
    fun error -> InvalidPackageName { raw_name; error }
  )

let parse_publish_metadata: (string * Toml.value) list -> (publish_metadata, publish_metadata_error) result = fun items ->
...
let resolve_workspace_dependency: Package_name.t -> dependency list -> dependency = fun name workspace_deps ->
  match List.find workspace_deps ~fn:(
    fun (d: dependency) -> Package_name.equal d.name name
  ) with
  | Some dep -> dep
  | None -> panic ("Dependency '" ^ Package_name.to_string name ^ "' with { workspace = true } not found in workspace dependencies")
...
let requirement_is_any = fun requirement -> String.equal (Version.requirement_to_string requirement) "*"
...
let normalize_source_locator = fun raw ->
  ...
  let raw =
    if String.starts_with ~prefix:"https://" raw then
      ...
      if String.starts_with ~prefix:"http://" raw then
...
let validate_dependency_source = fun ~dependency_name source ->
  if source.workspace && (source.builtin || Option.is_some source.path || Option.is_some source.source_locator || Option.is_some source.ref_ || Option.is_some source.version) then
    Error (WorkspaceDependencyCannotSpecifyOverrides { dependency_name })
  else
    if Option.is_some source.ref_ && Option.is_none source.source_locator then
      Error (DependencyRefRequiresSource { dependency_name })
    else
      if source.builtin && (Option.is_some source.path || Option.is_some source.source_locator || Option.is_some source.ref_) then
        ...
        if source.builtin then
          ...
          | Some version -> Error (BuiltinDependencyVersionRequirementNotSupported { dependency_name; requirement = Version.requirement_to_string version })
        else
          if source.workspace || Option.is_some source.path || Option.is_some source.source_locator || Option.is_some source.version then
            Ok source
          else Ok ({ source with version = Some Version.any })

let parse_dependency: string -> Toml.value -> workspace_deps:dependency list -> (dependency, dependency_error) result = fun raw_name value ~workspace_deps ->
  let* name = Package_name.from_string raw_name |> Result.map_err ~fn:(
    fun error -> InvalidDependencyName { raw_name; error }
  )
  in
  ...
  | Toml.Table attrs -> (
    ...
    | Some (Toml.Bool true) -> (
      let source = { (resolve_workspace_dependency name workspace_deps).source with workspace = true } in validate_dependency_source ~dependency_name source |> Result.map ~fn:(
        fun source -> { name; source }
      )
    )
    ...
    | _ -> (
      ...
      let source_locator =
        ...
        | Some _, None -> Error (NonStringDependencyField { dependency_name; field = DependencySource })
        | None, Some (Toml.String github) -> Ok (Some (github_locator_of_value github))
        | None, Some _ -> Error (NonStringDependencyField { dependency_name; field = DependencyGithub })
        | None, None -> Ok None
      in
      let ref_ =
      ...
      let version =
        ...
        | Some (Toml.String requirement) -> validate_requirement ~dependency_name requirement |> Result.map ~fn:(
          fun version -> Some version
        )
        | Some _ -> Error (NonStringDependencyField { dependency_name; field = DependencyVersion })
        | None -> Ok None
      in
      ...
      | Ok path, Ok source_locator, Ok ref_, Ok version -> validate_dependency_source ~dependency_name (make_source ~builtin:(is_builtin_dependency_name dependency_name) ?path ?source_locator ?ref_ ?version ()) |> Result.map ~fn:(
        fun source -> { name; source }
      )
    )
  )
  | Toml.String requirement -> (
    ...
    | Error _ as err -> err
    | Ok version -> validate_dependency_source ~dependency_name (make_source ~builtin:(is_builtin_dependency_name dependency_name) ~version ()) |> Result.map ~fn:(
      fun source -> { name; source }
    )
  )
  | _ -> Error (DependencyMustBeStringOrTable { dependency_name })

let parse_dependencies: (string * Toml.value) list -> workspace_deps:dependency list -> (dependency list, dependency_error) result = fun items ~workspace_deps ->
...
let parse_dependency_section = fun section_name items ~(workspace_deps:dependency list) ->
  ...
  | Some (Toml.Table dep_items) -> parse_dependencies dep_items ~workspace_deps |> Result.map_err ~fn:(
    fun error -> InvalidDependency error
  )
  ...
  | Json.Object fields -> (
    ...
    | Some (Json.String "path") -> (
      ...
      let version =
        ...
        | Some Json.Null | None -> Ok None
        | Some (Json.String requirement) -> validate_requirement ~dependency_name:"<json>" requirement |> Result.map_err ~fn:dependency_error_message |> Result.map ~fn:(
          fun version -> Some version
        )
    ...
    | _ ->
        ...
        | (Error err, _, _, _, _, _) | (_, Error err, _, _, _, _) | (_, _, Error err, _, _, _) | (_, _, _, Error err, _, _) | (_, _, _, _, Error err, _) | (_, _, _, _, _, Error err) -> Error err
  )
  | _ -> Error "dependency source must be a string or object"

let parse_foreign_dependency: string -> Toml.value -> package_path:Path.t -> (foreign_dependency, string) result = fun name value ~package_path ->
  match value with
  | Toml.Table attrs -> (
    let get_string key =
    ...
    let get_string_list key =
      match Fields.get key attrs with
      | Some (Toml.Array arr) ->
          let strings = List.filter_map ~fn:(
            function
    ...
    let get_env () =
      ...
      | Some (Toml.Table env_items) -> List.filter_map ~fn:(
        fun ((k, v)) ->
          match v with
    ...
    | Ok path_str, Ok build_cmd, Ok outputs ->
        ...
        let scan_foreign_inputs foreign_path =
          ...
          in
          let walker = Fs.Walker.filter_entry walker ~f:(
            fun (entry: Fs.Walker.FileItem.t) ->
              ...
              if Int.equal (Fs.Walker.FileItem.depth entry) 0 then
                ...
                | Ok rel_path ->
                    ...
                    in
                    let should_skip = String.starts_with ~prefix:"." entry_name || List.contains exclude_dirs ~value:entry_name in
                    if should_skip then
                      false
                    else
                      match Fs.Walker.FileItem.kind entry with
                      | Directory -> true
                      | File -> String.ends_with ~suffix:".rs" entry_name || String.ends_with ~suffix:".c" entry_name || String.ends_with ~suffix:".h" entry_name || String.ends_with ~suffix:".cpp" entry_name || String.ends_with ~suffix:".hpp" entry_name || entry_name = "Cargo.toml" || entry_name = "Cargo.lock" || entry_name = "build.rs" || entry_name = "CMakeLists.txt" || entry_name = "Makefile"
                      | Symlink | Other -> false
          ) in
          ...
          let rec loop acc iter =
            ...
            | Some (Ok (entry: Fs.Walker.FileItem.t)), iter' -> (
              ...
              | Directory | Symlink | Other -> loop acc iter'
            )
          in
          loop [] iter
        in
...
let parse_foreign_dependencies: (string * Toml.value) list -> package_path:Path.t -> (foreign_dependency list, string) result = fun items ~package_path ->
  Log.debug "[PACKAGE] parse_foreign_dependencies: checking for 'foreign-dependencies' key";
  Log.debug ("[PACKAGE] Available keys: " ^ String.concat ", " (List.map items ~fn:(
    fun (key, _) -> key
  )));
  (* Collect all keys that start with "foreign-dependencies." *)
  let foreign_dep_items = List.filter_map ~fn:(
    fun ((key, value)) ->
      if String.starts_with ~prefix:"foreign-dependencies." key then
        let prefix_len = String.length "foreign-dependencies." in
        let dep_name = String.sub key ~offset:prefix_len ~len:(String.length key - prefix_len) in Some (dep_name, value)
      else None
  ) items in
  ...
  if all_deps = [] then
    ...
    let results = List.map all_deps ~fn:(
      fun ((name, value)) -> parse_foreign_dependency name value ~package_path
    ) in
    let errors = List.filter_map ~fn:(
      fun r ->
        match r with
        | Error e -> Some e
        | Ok _ -> None
...
let parse_binary: Toml.value -> package_path:Path.t -> (binary, string) result = fun value ~package_path ->
  ...
  | Toml.Table items -> (
    ...
    | Some (Toml.String name), Some (Toml.String path_str) ->
        let bin_path = Path.v path_str in Ok { name; path = bin_path }
...
let parse_library: (string * Toml.value) list -> package_path:Path.t -> package_name:Package_name.t -> (library option, string) result = fun items ~package_path ~package_name ->
  ...
  | Some (Toml.Table lib_items) -> (
    ...
    | Some (Toml.String path_str) ->
        let lib_path = Path.(package_path / Path.v path_str) in Ok (Some { path = lib_path })
    | None ->
        let default_path = Path.(package_path / Path.v "src" / Path.v (package_name ^ ".ml")) in Ok (Some { path = default_path })
...
let parse_compiler_config: (string * Toml.value) list -> compiler_config = fun items ->
  let profile_overrides =
    ...
    | Some (Toml.Table profile_table) -> List.filter_map ~fn:(
      fun ((profile_name, value)) ->
        match value with
        | Toml.Table profile_items -> Some (profile_name, Profile.override_from_toml profile_items)
        | _ -> None
    ) profile_table
    | _ -> []
  in
  ...
  let target_overrides =
    ...
    | Some (Toml.Table target_table) -> List.filter_map ~fn:(
      fun ((platform, value)) ->
        match value with
        | Toml.Table platform_items ->
            let profile_override = Profile.override_from_toml platform_items in Some (platform, { profile_override = Some profile_override })
        | _ -> None
    ) target_table
    | _ -> []
  in
...
let provider_excluded_relpaths = fun ~(package_path:Path.t) providers ->
  let collect_relative_files ~root ~keep_file ~skip_dir =
    ...
    loop [] iter
  in
  let ocaml_source_suffix path_str = String.ends_with ~suffix:".ml" path_str || String.ends_with ~suffix:".mli" path_str in
  let collect_provider_tree rel_path =
    ...
    if String.equal basename "riot_fix_rules.ml" && String.equal parent_basename "riot_fix_rules" then
      let provider_dir = Path.(package_path / provider_parent) in collect_relative_files ~root:provider_dir ~skip_dir:(
        fun _ -> false
      ) ~keep_file:(
        fun rel_path -> ocaml_source_suffix (Path.to_string rel_path)
      )
    else [ rel_path ]
  in
  providers |> List.filter_map ~fn:(
    fun (provider: Fix_provider.t) ->
      ...
      | Error _ -> None
  ) |> List.concat |> List.unique ~compare:(
    fun left right -> String.compare (Path.to_string left) (Path.to_string right)
  )
  ...
  if List.is_empty enabled_buckets then
    ...
    if not (Path.exists package_path) || List.is_empty scan_roots then
      ...
      | Ok entries ->
              ...
              if not (Int.equal (Fs.Walker.FileItem.depth entry) 0) then
                ...
                | Ok rel_path -> (
                  ...
                  | File ->
                      ...
                      if not (should_skip_source_entry rel_path || should_skip_test_support_path rel_path || List.contains excluded_relpath_strings ~value:rel_path_string) then
                        (
                          match bucket_of_rel_path rel_path_components with
                          | Some bucket when not (source_bucket_enabled enabled_buckets bucket) -> ()
                          | Some Src | Some Tests | Some Examples | Some Bench when not (is_ocaml_module_file rel_path) -> ()
          ...
          in
          let () = trace_package ("scan-sources path=" ^ Path.to_string package_path ^ " intent=" ^ string_of_realization_intent intent ^ " total_us=" ^ Int.to_string (elapsed_us_since started_at) ^ " visited_entries=" ^ Int.to_string !visited_entries ^ " visited_directories=" ^ Int.to_string !visited_directories ^ " visited_files=" ^ Int.to_string !visited_files ^ " kept_src=" ^ Int.to_string (List.length sources.src) ^ " kept_tests=" ^ Int.to_string (List.length sources.tests) ^ " kept_native=" ^ Int.to_string (List.length sources.native) ^ " kept_examples=" ^ Int.to_string (List.length sources.examples) ^ " kept_bench=" ^ Int.to_string (List.length sources.bench)) in sources
      | Error _ ->
          let () = trace_package ("scan-sources-failed path=" ^ Path.to_string package_path ^ " intent=" ^ string_of_realization_intent intent ^ " total_us=" ^ Int.to_string (elapsed_us_since started_at)) in empty_sources

let scan_sources ~(package_path:Path.t) ?(excluded_relpaths = []) (): sources = scan_sources_for_intent ~intent:Dev ~package_path ~excluded_relpaths ()

(** Autodiscover test binaries from test files ending in _tests.ml or -tests.ml *)
let autodiscover_test_binaries: sources -> package_path:Path.t -> binary list = fun sources ~package_path -> List.filter_map ~fn:(
  fun test_file ->
    let filename = Path.basename test_file in
    if String.ends_with ~suffix:"_tests.ml" filename || String.ends_with ~suffix:"-tests.ml" filename then
      ...
      let binary_path = test_file in Some { name = binary_name; path = binary_path }
    else None
) sources.tests
...
let autodiscover_main_binary: sources -> package_name:Package_name.t -> binary list = fun sources ~package_name ->
  if List.any sources.src ~fn:(
    fun path -> Path.equal path (Path.v "src/main.ml")
  ) then
...
let autodiscover_example_binaries: sources -> package_path:Path.t -> binary list = fun sources ~package_path -> List.filter_map ~fn:(
  fun example_file ->
    let filename = Path.basename example_file in
    if String.ends_with ~suffix:".ml" filename then
      let binary_name = Path.remove_extension (Path.v filename) |> Path.to_string in (* example_file is already relative to package (e.g., examples/sqltool.ml) *)
      Some { name = binary_name; path = example_file }
    else None
) sources.examples

(** Autodiscover benchmark binaries from bench files ending in _bench.ml *)
let autodiscover_bench_binaries: sources -> package_path:Path.t -> binary list = fun sources ~package_path -> List.filter_map ~fn:(
  fun bench_file ->
    let filename = Path.basename bench_file in
    if String.ends_with ~suffix:"_bench.ml" filename then
      let binary_name = Path.remove_extension (Path.v filename) |> Path.to_string in (* bench_file is already relative to package (e.g., bench/foo_bench.ml) *)
...
let has_declared_binary_in_bucket = fun binaries bucket -> List.any binaries ~fn:(
  fun bin ->
...
let autodiscovered_binaries_for_intent = fun ~(intent:realization_intent) ~(sources:sources) ~package_name ~package_path ~declared_binaries ->
  let runtime_binaries =
    if has_declared_binary_in_bucket declared_binaries Src then
      []
    else autodiscover_main_binary sources ~package_name
  in
  let test_binaries =
    if has_declared_binary_in_bucket declared_binaries Tests then
      []
    else autodiscover_test_binaries sources ~package_path
  in
  let example_binaries =
    if has_declared_binary_in_bucket declared_binaries Examples then
      []
    else autodiscover_example_binaries sources ~package_path
  in
  let bench_binaries =
    if has_declared_binary_in_bucket declared_binaries Bench then
      []
    else autodiscover_bench_binaries sources ~package_path
  in
  match intent with
  | Build -> []
...
let merge_binaries: declared:binary list -> autodiscovered:binary list -> binary list = fun ~declared ~autodiscovered ->
  let seen_paths = declared |> List.map ~fn:(
    fun (bin: binary) -> Path.to_string bin.path
  ) in
  let _, discovered = List.fold_left autodiscovered ~init:(seen_paths, []) ~fn:(
  ...
  ) in declared @ List.reverse discovered

let parse_manifest_spec: Toml.value -> workspace_deps:dependency list -> workspace_dev_deps:dependency list -> workspace_build_deps:dependency list -> path:Path.t -> relative_path:Path.t -> (manifest_spec, manifest_error) result = fun toml ~workspace_deps ~workspace_dev_deps ~workspace_build_deps ~path ~relative_path ->
  match toml with
  | Toml.Table items -> (
    let fallback_name = Path.basename path in
    let* name = parse_name items fallback_name
    in
            ...
            | Ok dev_dependencies ->
                ...
                | Ok build_dependencies ->
                    let declared_binaries =
                      ...
                      | Ok bins -> bins
                      | Error msg ->
                          Log.warn ("[PACKAGE] Failed to parse binaries for " ^ Package_name.to_string name ^ ": " ^ msg);
                          []
                    in
                    let library =
                      ...
                      | Ok lib -> lib
                      | Error msg ->
                          Log.warn ("[PACKAGE] Failed to parse library for " ^ Package_name.to_string name ^ ": " ^ msg);
                          None
                    in
                    let foreign_dependencies =
                      ...
                      | Ok deps -> deps
                      | Error msg ->
                          Log.warn ("[PACKAGE] Failed to parse foreign dependencies for " ^ Package_name.to_string name ^ ": " ^ msg);
                          []
                    in
                    ...
                    let commands =
                      ...
                      | Some (Toml.Array cmd_entries) -> Package_command.parse_from_toml cmd_entries ~package_name:name ~package_path:path
                      | _ -> []
                    in
                    Ok (
...
let realize_manifest_spec = fun ~(intent:realization_intent) (manifest: manifest_spec) ->
  ...
  let autodiscovered = autodiscovered_binaries_for_intent ~intent ~sources ~package_name:manifest.name ~package_path:manifest.path ~declared_binaries in
  let binaries = merge_binaries ~declared:declared_binaries ~autodiscovered in make ~name:manifest.name ~path:manifest.path ~relative_path:manifest.relative_path ~dependencies:manifest.dependencies ~dev_dependencies:manifest.dev_dependencies ~build_dependencies:manifest.build_dependencies ~foreign_dependencies:manifest.foreign_dependencies ~binaries ?library:manifest.library ~sources ~compiler:manifest.compiler ~commands:manifest.commands ~fix_providers:manifest.fix_providers ~publish:manifest.publish ()

let of_manifest_spec = fun (manifest: manifest_spec) -> make ~name:manifest.name ~path:manifest.path ~relative_path:manifest.relative_path ~dependencies:manifest.dependencies ~dev_dependencies:manifest.dev_dependencies ~build_dependencies:manifest.build_dependencies ~foreign_dependencies:manifest.foreign_dependencies ~binaries:manifest.declared_binaries ?library:manifest.library ~compiler:manifest.compiler ~commands:manifest.commands ~fix_providers:manifest.fix_providers ~publish:manifest.publish ()

let from_toml: Toml.value -> workspace_deps:dependency list -> workspace_dev_deps:dependency list -> workspace_build_deps:dependency list -> path:Path.t -> relative_path:Path.t -> (t, manifest_error) result = fun toml ~workspace_deps ~workspace_dev_deps ~workspace_build_deps ~path ~relative_path -> parse_manifest_spec toml ~workspace_deps ~workspace_dev_deps ~workspace_build_deps ~path ~relative_path |> Result.map ~fn:(realize_manifest_spec ~intent:Dev)

let to_json: t -> Json.t = fun pkg ->
  let dependencies_json = Json.Array (List.map pkg.dependencies ~fn:(
    fun (dep: dependency) ->
      Json.Object [
  ...
  let dev_dependencies_json = Json.Array (List.map pkg.dev_dependencies ~fn:(
    fun (dep: dependency) ->
      Json.Object [
        "name", Json.String (Package_name.to_string dep.name);
        "source", dependency_source_to_json dep.source;
  ...
  let build_dependencies_json = Json.Array (List.map pkg.build_dependencies ~fn:(
    fun (dep: dependency) ->
      Json.Object [
        "name", Json.String (Package_name.to_string dep.name);
        "source", dependency_source_to_json dep.source;
  ...
  let binaries_json = Json.Array (List.map pkg.binaries ~fn:(
    fun (bin: binary) ->
      Json.Object [
  ...
  let library_json =
    ...
    | Some lib ->
        Json.Object [
          "path", Json.String (Path.to_string lib.path);
        ]
    | None -> Json.Null
  in
...
let from_json: Json.t -> (t, string) result = fun json ->
  ...
  | Json.Object fields -> (
    ...
    | (Some (Json.String name), Some (Json.String path_str), Some (Json.String rel_path_str)) -> (
      let* name = Package_name.from_string name |> Result.map_err ~fn:Package_name.error_message
      in
              ...
              | entry :: rest -> (
                ...
                | Json.Object dep_fields -> (
                  ...
                  | Some (Json.String dep_name), Some source_json -> (
                    let* dep_name = Package_name.from_string dep_name |> Result.map_err ~fn:Package_name.error_message
                    in
                    let* source = dependency_source_of_json source_json in loop ({ name = dep_name; source } :: acc) rest
                  )
            ...
            | Ok dev_dependencies -> (
              ...
              | Ok build_dependencies ->
                  let binaries =
                    ...
                    | Some (Json.Array bins) -> List.filter_map ~fn:(
                      function
                      | Json.Object bin_fields -> (
                        match Fields.get "name" bin_fields, Fields.get "path" bin_fields with
                        | (Some (Json.String bin_name), Some (Json.String bin_path)) -> Some { name = bin_name; path = Path.v bin_path }
                        | _ -> None
                      )
                      | _ -> None
                    ) bins
                    ...
                    | Some (Json.Object publish_fields) ->
                        let version =
                          ...
                          | Some (Json.String raw_version) -> (
                            ...
                            | Ok version -> Ok (Some version)
                            | Error err -> Error ("Invalid package publish version in JSON: " ^ version_parse_error_to_string err)
                          )
                          | Some Json.Null | None -> Ok None
...
let hash_with = fun (type s) ((module H: Hash_writer with type state = s)) state (pkg: t) ->
  ...
  (* Binaries metadata *)
  List.for_each pkg.binaries ~fn:(
    fun (bin: binary) ->
      H.write state bin.name;
      H.write state (Path.to_string bin.path)
  );
  List.for_each pkg.fix_providers ~fn:(
    fun (provider: Fix_provider.t) ->
  ...
  let hash_source_file file_path =
    ...
    if HashSet.insert seen_source_files ~value:path_str then
      ...
      | Error _ -> (* File read error - include path only *)
      H.write state path_str
  in
  ...
  List.for_each pkg.foreign_dependencies ~fn:(
...
[build-dependencies]
fixme = { path = "../fixme" }
|} |> Result.expect ~msg:"expected package toml to parse" in
    let workspace_dep name = { name = package_name name; source = source ~workspace:true () } in
    let pkg = from_toml toml ~workspace_deps:[ workspace_dep "std" ] ~workspace_dev_deps:[ workspace_dep "propane" ] ~workspace_build_deps:[] ~path:(Path.v "/tmp/example") ~relative_path:(Path.v "packages/example") |> Result.expect ~msg:"expected package manifest" in
    if List.map pkg.dependencies ~fn:(
      fun (dep: dependency) -> dep.name
    ) = [ package_name "std" ] && List.map pkg.dev_dependencies ~fn:(
      fun (dep: dependency) -> dep.name
    ) = [ package_name "propane" ] && List.map pkg.build_dependencies ~fn:(
      fun (dep: dependency) -> dep.name
    ) = [ package_name "fixme" ] then
      Ok ()
    else Error "expected dependency classes to round-trip" [@test]
...
std = ">= 1.2.3"
|} |> Result.expect ~msg:"expected package toml to parse" in
    let pkg = from_toml toml ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:(Path.v "/tmp/example") ~relative_path:(Path.v "packages/example") |> Result.expect ~msg:"expected package manifest" in
    match pkg.dependencies with
    | [ { source = { workspace = false; builtin = false; path = None; source_locator = None; ref_ = None; version = Some requirement }; _ } ] ->
        if String.equal (Version.requirement_to_string requirement) ">= 1.2.3" then
          Ok ()
        else Error "expected parsed dependency requirement to be preserved structurally"
...
|} |> Result.expect ~msg:"expected package toml to parse" in
    let pkg = from_toml toml ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:(Path.v "/tmp/example") ~relative_path:(Path.v "packages/example") |> Result.expect ~msg:"expected package manifest" in
    match pkg.dependencies with
    | [ { name; source = { builtin = true; version = Some requirement; _ } } ] when Package_name.equal name (package_name "stdlib") && requirement_is_any requirement -> Ok ()
...
|} |> Result.expect ~msg:"expected package toml to parse" in
    let pkg = from_toml toml ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:(Path.v "/tmp/example") ~relative_path:(Path.v "packages/example") |> Result.expect ~msg:"expected package manifest" in
    match pkg.dependencies with
    | [ { name; source = { source_locator = Some "github.com/riot-tests/widgets"; ref_ = None; _ } } ] when Package_name.equal name (package_name "widgets") -> Ok ()
...
|} |> Result.expect ~msg:"expected package toml to parse" in
    let pkg = from_toml toml ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:(Path.v "/tmp/example") ~relative_path:(Path.v "packages/example") |> Result.expect ~msg:"expected package manifest" in
    match pkg.dependencies with
    | [ { name; source = { source_locator = Some "github.com/riot-tests/monorepo/packages/widgets"; ref_ = Some "main"; _ } } ] when Package_name.equal name (package_name "widgets") -> Ok ()
...
stdlib = ">= 1.0.0"
|} |> Result.expect ~msg:"expected package toml to parse" in
    match from_toml toml ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:(Path.v "/tmp/example") ~relative_path:(Path.v "packages/example") with
...
|} |> Result.expect ~msg:"expected package toml to parse" in
  ...
  let test_package_json_round_trips_registry_requirement (): (unit, string) result =
    ...
    | Ok decoded -> (
      ...
      | [ { source = { workspace = false; builtin = false; path = None; source_locator = None; ref_ = None; version = Some decoded_requirement }; _ } ] ->
          if String.equal (Version.requirement_to_string decoded_requirement) ">= 1.2.3" then
            Ok ()
          else Error "expected registry requirement to survive package json roundtrip"
  ...
  let test_package_json_round_trips_source_dependency (): (unit, string) result =
    let package = {
      ...
      path = Path.v "/tmp/example";
      relative_path = Path.v "packages/example";
      dependencies = [ { name = package_name "widgets"; source = source ~source_locator:"github.com/riot-tests/widgets" ~ref_:"main" () } ];
      dev_dependencies = [];
      build_dependencies = [];
      foreign_dependencies = [];
      binaries = [];
    ...
    in
    match from_json (to_json package) with
    | Ok { dependencies = [ { name; source = { source_locator = Some "github.com/riot-tests/widgets"; ref_ = Some "main"; _ } } ]; _ } when Package_name.equal name (package_name "widgets") -> Ok ()
...
[build-dependencies]
ppx = {}
|} |> Result.expect ~msg:"expected test toml to parse" in
    let package = from_toml toml ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:(Path.v "/workspace/packages/app") ~relative_path:(Path.v "packages/app") |> Result.expect ~msg:"expected package manifest" in
    ...
    in
    match resolve ~package ~lock_package ~manifest_path:Path.(package.path / Path.v "riot.toml") ~materialized_root:package.path with
    | Ok resolved ->
        if List.length resolved.runtime_resolved = 1 && List.length resolved.build_resolved = 1 && (
          ...
          Ok ()
        else Error "expected resolved package projection to preserve exact ids"
    | Error err -> Error err [@test]
...
|} |> Result.expect ~msg:"expected test toml to parse" in
  ...
  let test_build_graph_dependencies_exclude_build_only_deps (): (unit, string) result =
    ...
    in
    let build_graph = build_graph_dependencies pkg |> List.map ~fn:(
      fun (dep: dependency) -> dep.name
    ) in
    let all = all_dependencies pkg |> List.map ~fn:(
      fun (dep: dependency) -> dep.name
    ) in
    if build_graph = [ package_name "std"; package_name "propane" ] && all = [ package_name "std"; package_name "propane"; package_name "fixme" ] then
      Ok ()
    else Error "expected build graph dependencies to exclude build-only deps" [@test]
  ...
  let test_dev_scope_dependencies_include_regular_dependencies (): (unit, string) result =
    ...
    in
    let dev_deps = dependencies_for_scope Dev pkg |> List.map ~fn:(
      fun (dep: dependency) -> dep.name
    ) in
    if dev_deps = [ package_name "std"; package_name "propane" ] then
      Ok ()
    else Error "expected dev scope dependencies to include regular dependencies" [@test]

  let test_root_module_name_sanitizes_hyphenated_package_names (): (unit, string) result =
    let pkg = synthetic ~name:(package_name "kernel-new") ~path:(Path.v "/tmp/kernel-new") ~relative_path:(Path.v "packages/kernel-new") in
    if String.equal (root_module_name pkg) "Kernel_new" then
      Ok ()
    else Error "expected hyphenated package names to sanitize to a valid root module name" [@test]
end
