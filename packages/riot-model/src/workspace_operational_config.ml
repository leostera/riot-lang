...
type test_policy = { small_test_timeout: Time.Duration.t option; flaky_max_retries: int }
...
let message = function
  | ReadFailed { path; error } -> "failed to read workspace config '" ^ Path.to_string path ^ "': " ^ IO.error_message error
  | ParseFailed { path; error } -> "failed to parse workspace config '" ^ Path.to_string path ^ "': " ^ Toml.error_to_string error
  | InvalidConfig { path; error } -> "invalid workspace config '" ^ Path.to_string path ^ "': " ^ invalid_config_error_message error

let normalize_size = fun raw ->
  let raw = String.trim raw in
  let compact_length = String.fold_left ~fn:(
    fun count c ->
  ...
  ) ~init:0 raw in String.lowercase_ascii (IO.Bytes.to_string compact)

let unit_multiplier = function
  | "" | "b" -> Some 1L
  | "k" | "kb" | "kib" -> Some 1_024L
  | "m" | "mb" | "mib" -> Some 1_048_576L
  | "g" | "gb" | "gib" -> Some 1_073_741_824L
  | "t" | "tb" | "tib" -> Some 1_099_511_627_776L
  | _ -> None
...
let parse_max_size = fun raw ->
  ...
  let rec split idx =
    if idx >= len then
      ...
      | '0' .. '9' | '.' -> split (idx + 1)
      | _ -> String.sub normalized ~offset:0 ~len:idx, String.sub normalized ~offset:idx ~len:(len - idx)
  in
    ...
    | Some multiplier -> (
      ...
      | Some number ->
          if number < 0.0 then
            ...
            let bytes = number *. Int64.to_float multiplier in Ok (Int64.from_float bytes)
    )
...
let parse_cache_policy = fun ~path fields ->
  ...
  let max_size_bytes =
    ...
    | Some (Toml.String raw) -> parse_max_size raw |> Result.map_err ~fn:(
      fun error -> InvalidMaxSize error
    )
  ...
  | (Error error, _) | (_, Error error) -> Error (InvalidConfig { path; error = CacheConfig error })
  ...
  if String.equal number_str "" then
    ...
    | Some multiplier -> (
      ...
      | Some number ->
          if number < 0.0 then
            Error NegativeValue
          else Ok (Time.Duration.from_secs_float (number *. multiplier))
    )
...
let parse_test_policy = fun ~path fields ->
  let small_test_timeout =
    ...
    | Some (Toml.String raw) -> parse_duration raw |> Result.map ~fn:Option.some |> Result.map_err ~fn:(
      fun error -> InvalidSmallTestTimeout error
    )
    | Some value -> (
      match Toml.get_int value with
  ...
  | (Error error, _) | (_, Error error) -> Error (InvalidConfig { path; error = TestConfig error })
