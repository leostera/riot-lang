...
type target_override = { profile_override: Profile.profile_override option }
...
val from_toml: Std.Data.Toml.value -> workspace_deps:dependency list -> workspace_dev_deps:dependency list -> workspace_build_deps:dependency list -> path:Path.t -> relative_path:Path.t -> (t, manifest_error) result

val parse_manifest_spec: Std.Data.Toml.value -> workspace_deps:dependency list -> workspace_dev_deps:dependency list -> workspace_build_deps:dependency list -> path:Path.t -> relative_path:Path.t -> (manifest_spec, manifest_error) result
...
val make: name:Package_name.t -> path:Path.t -> relative_path:Path.t -> ?dependencies:dependency list -> ?dev_dependencies:dependency list -> ?build_dependencies:dependency list -> ?foreign_dependencies:foreign_dependency list -> ?binaries:binary list -> ?library:library -> ?sources:sources -> ?compiler:compiler_config -> ?commands:Package_command.t list -> ?fix_providers:Fix_provider.t list -> ?publish:publish_metadata -> unit -> t
...
val resolve: package:t -> lock_package:Lockfile.package -> manifest_path:Path.t -> materialized_root:Path.t -> (resolved, string) result
