...
type registry = { api_url: Net.Uri.t; cdn_url: Net.Uri.t; api_token: string option }

type t = { registries: (string * registry) list }
...
let default = {
  registries = [
    "pkgs.ml", { api_url = Net.Uri.of_string "https://api.pkgs.ml" |> Result.unwrap; cdn_url = Net.Uri.of_string "https://cdn.pkgs.ml" |> Result.unwrap; api_token = None };
  ]
}
...
let registry_error_message = function
  | InvalidDefaultUri { field; error } -> "invalid default " ^ registry_field_name field ^ ": " ^ uri_error_message error
  | InvalidUri { field; error } -> "field '" ^ registry_field_name field ^ "' must be a valid URI: " ^ uri_error_message error
...
let message = function
  | ReadFailed { path; error } -> "failed to read config '" ^ Path.to_string path ^ "': " ^ IO.error_message error
  | ParseFailed { path; error } -> "failed to parse config '" ^ Path.to_string path ^ "': " ^ Toml.error_to_string error
  | WriteFailed { path; error } -> "failed to write config '" ^ Path.to_string path ^ "': " ^ IO.error_message error
  | InvalidConfig error -> "invalid config: " ^ config_error_message error
  | InvalidRegistryConfig { registry_name; error } -> "invalid registry config for '" ^ registry_name ^ "': " ^ registry_error_message error

let default_api_url = fun ~registry_name -> Net.Uri.of_string ("https://api." ^ registry_name) |> Result.map_err ~fn:(
  fun error -> InvalidRegistryConfig { registry_name; error = InvalidDefaultUri { field = Api_url; error } }
)

let default_cdn_url = fun ~registry_name -> Net.Uri.of_string ("https://cdn." ^ registry_name) |> Result.map_err ~fn:(
  fun error -> InvalidRegistryConfig { registry_name; error = InvalidDefaultUri { field = Cdn_url; error } }
)

let default_registry = fun ~registry_name ->
  let* api_url = default_api_url ~registry_name
  in
  let* cdn_url = default_cdn_url ~registry_name in Ok { api_url; cdn_url; api_token = None }

let registry_of_toml = fun ~registry_name value ->
  match value with
  | Toml.Table fields ->
      let* defaults = default_registry ~registry_name
      in
      let* api_url =
        match Fields.get "api_url" fields with
        | None -> Ok defaults.api_url
        | Some (Toml.String url) -> Net.Uri.of_string url |> Result.map_err ~fn:(
          fun error -> InvalidRegistryConfig { registry_name; error = InvalidUri { field = Api_url; error } }
        )
      ...
      let* cdn_url =
        ...
        | Some (Toml.String url) -> Net.Uri.of_string url |> Result.map_err ~fn:(
          fun error -> InvalidRegistryConfig { registry_name; error = InvalidUri { field = Cdn_url; error } }
        )
      ...
      let api_token =
        ...
        | Some _ -> Error (InvalidRegistryConfig { registry_name; error = FieldMustBeString Api_token })
      in
      Result.map api_token ~fn:(
        fun api_token -> { api_url; cdn_url; api_token }
      )
  | _ -> Error (InvalidRegistryConfig { registry_name; error = RegistryEntryMustBeTable })

let normalize_registry_name = fun name ->
  let len = String.length name in
  if len >= 2 && Char.equal (String.get_unchecked name ~at:0) '"' && Char.equal (String.get_unchecked name ~at:(len - 1)) '"' then
    String.sub name ~offset:1 ~len:(len - 2)
  else name

let rec collect_registries = fun ~path acc fields ->
  let has_registry_fields = List.any fields ~fn:(
    fun (name, _value) -> String.equal name "api_token"
  ) in
  let has_nested_tables = List.any fields ~fn:(
    fun (_name, value) ->
      match value with
      | Toml.Table _ -> true
      | _ -> false
...
let of_toml = fun value ->
  ...
  | Toml.Table fields -> (
    ...
    | None -> Ok empty
    | Some (Toml.Table registry_fields) -> collect_registries ~path:[] [] registry_fields |> Result.map ~fn:(
      fun registries -> { registries = List.reverse registries }
    )
...
let to_string = fun config ->
  let render_registry (registry_name, registry) =
    ...
    let fields = [ "api_url = " ^ render_string (Net.Uri.to_string registry.api_url); "cdn_url = " ^ render_string (Net.Uri.to_string registry.cdn_url) ] in
...
let save = fun config path -> Fs.write (to_string config) path |> Result.map_err ~fn:(
  fun io_error -> WriteFailed { path; error = io_error }
)
...
let upsert_registry = fun config ~registry_name ~update ->
  let rec loop acc = function
    ...
    | (name, registry) :: rest when String.equal name registry_name -> List.append (List.reverse acc) ((name, update registry) :: rest)
    | entry :: rest -> loop (entry :: acc) rest
  in
  { registries = loop [] config.registries }

let api_token = fun config ~registry_name ->
  match List.find config.registries ~fn:(
    fun (name, _registry) -> String.equal name registry_name
  ) with
  | None -> None
  | Some (_name, registry) -> registry.api_token

let set_api_token = fun config ~registry_name token -> upsert_registry config ~registry_name ~update:(
  fun registry -> { registry with api_token = Some token }
)

let clear_api_token = fun config ~registry_name -> upsert_registry config ~registry_name ~update:(
  fun registry -> { registry with api_token = None }
)
