...
let workspace_operational_config_path = fun ~workspace_root -> Path.(workspace_riot_dir ~workspace_root / Path.v "config.toml")
...
let git_registry_repo_dir = fun ~host ~owner ~repo -> Path.(git_registry_host_dir ~host / Path.v owner / Path.v repo)

let package_lock_path = fun ~workspace_root -> Path.(workspace_root / Path.v "riot.lock")

let toolchains_dir = fun toolchain_config ->
  let version = toolchain_config.Toolchain_config.version in Path.(dot_riot / Path.v "toolchains" / Path.v version)

let project_dir = fun workspace ->
  let project_id = Workspace.project_id workspace in Path.(dot_riot / Path.v "projects" / Path.v project_id)
...
let build_dir_root = fun ~workspace_root -> resolve_build_dir_root ~workspace_root (workspace_build_dir_name ~workspace_root)
...
let profile_dir_in_workspace = fun ~(workspace:Workspace.t) ~profile -> Path.(workspace.target_dir_root / Path.v profile)

let target_dir = fun ~workspace_root ~profile ~target -> Path.(profile_dir ~workspace_root ~profile / Path.v (target_dir_name target))

let target_dir_in_workspace = fun ~(workspace:Workspace.t) ~profile ~target -> Path.(profile_dir_in_workspace ~workspace ~profile / Path.v (target_dir_name target))

let out_dir_with_target = fun ~workspace_root ~profile ~target -> Path.(target_dir ~workspace_root ~profile ~target / Path.v "out")

let out_dir_in_workspace = fun ~(workspace:Workspace.t) ~profile ~target -> Path.(target_dir_in_workspace ~workspace ~profile ~target / Path.v "out")

let sandbox_dir_with_target = fun ~workspace_root ~profile ~target -> Path.(target_dir ~workspace_root ~profile ~target / Path.v "sandbox")

let sandbox_dir_in_workspace = fun ~(workspace:Workspace.t) ~profile ~target -> Path.(target_dir_in_workspace ~workspace ~profile ~target / Path.v "sandbox")

let cache_dir_with_target = fun ~workspace_root ~profile ~target -> Path.(target_dir ~workspace_root ~profile ~target / Path.v "cache")

let cache_dir_in_workspace = fun ~(workspace:Workspace.t) ~profile ~target -> Path.(target_dir_in_workspace ~workspace ~profile ~target / Path.v "cache")

let build_lock_path_with_target = fun ~workspace_root ~profile ~target -> Path.(target_dir ~workspace_root ~profile ~target / Path.v "riot.lock")

let build_lock_path_in_workspace = fun ~(workspace:Workspace.t) ~profile ~target -> Path.(target_dir_in_workspace ~workspace ~profile ~target / Path.v "riot.lock")
...
let cache_dir = fun ~workspace_root -> cache_dir_with_target ~workspace_root ~profile:"debug" ~target:(host_target ())

let out_dir = fun ~workspace_root -> out_dir_with_target ~workspace_root ~profile:"debug" ~target:(host_target ())

let sandbox_dir = fun ~workspace_root -> sandbox_dir_with_target ~workspace_root ~profile:"debug" ~target:(host_target ())
...
module Tests = struct
  let test_package_lock_path (): (unit, string) result =
    ...
    if String.equal actual "/tmp/workspace/riot.lock" then
      Ok ()
    else Error ("expected root riot.lock path, got " ^ actual) [@test]

  let test_workspace_target_dirs_use_custom_target_dir_root (): (unit, string) result =
    let workspace = Workspace.make ~root:(Path.v "/tmp/workspace") ~target_dir:"build-out" ~packages:[] () in
    ...
    if not (String.equal target_dir expected_target_dir) then
      ...
      if not (String.equal out_dir (expected_target_dir ^ "/out")) then
        Error ("expected out dir inside custom target dir, got " ^ out_dir)
      else
        if not (String.equal sandbox_dir (expected_target_dir ^ "/sandbox")) then
          Error ("expected sandbox dir inside custom target dir, got " ^ sandbox_dir)
        else
          if not (String.equal cache_dir (expected_target_dir ^ "/cache")) then
            Error ("expected cache dir inside custom target dir, got " ^ cache_dir)
          else
            if not (String.equal lock_path (expected_target_dir ^ "/riot.lock")) then
