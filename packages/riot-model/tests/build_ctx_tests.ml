...
let test_hash_changes_when_compilation_mode_changes = fun _ctx ->
  ...
  let host_ctx = Riot_model.Build_ctx.make
    ~session_id:(session_id "build-ctx-hash-host")
    ~profile:Riot_model.Profile.debug
    () in
  let cross_ctx = Riot_model.Build_ctx.make
...
let tests =
  Test.[
    ...
    case "Build_ctx Cross uses the configured target and sysroot" test_cross_mode_uses_the_cross_target_and_sysroot;
