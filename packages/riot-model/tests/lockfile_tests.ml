...
|}
  ...
  | Error err ->
      Error ("expected minimal lockfile to decode, got: "
...
|}
  ...
  let* toml = parse_toml source in
  match Riot_model.Lockfile.of_toml toml with
  | Error (Riot_model.Lockfile.MissingField {
    container = Riot_model.Lockfile.Lockfile;
    field = "dependency_hash";
  }) ->
      Ok ()
  | Error err ->
      Error ("expected missing dependency_hash, got: "
      ^ Riot_model.Lockfile.error_message err)
  | Ok _ ->
      Error "expected lockfile decode to fail when dependency_hash is missing"
...
|}
  ...
  let* toml = parse_toml source in
  match Riot_model.Lockfile.of_toml toml with
  | Error (Riot_model.Lockfile.InvalidPackageName {
  ...
  | Error err ->
      ...
      ^ Riot_model.Lockfile.error_message err)
  | Ok _ ->
      Error "expected lockfile decode to fail for invalid dependency package name"
...
|}
  ...
  let* toml = parse_toml source in
  match Riot_model.Lockfile.of_toml toml with
  | Error (Riot_model.Lockfile.UnknownProvenanceKind { value = "banana" }) ->
      Ok ()
  | Error err ->
      Error ("expected unknown provenance kind, got: "
      ^ Riot_model.Lockfile.error_message err)
  | Ok _ ->
      Error "expected lockfile decode to fail for unknown provenance kind"
...
let () =
  Runtime.run
    ~main:(fun ~args -> Test.Cli.main ~name:"lockfile" ~tests ~args ())
    ~args:Env.args
    ()
