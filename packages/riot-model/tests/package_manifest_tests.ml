...
let package_name = fun name -> Riot_model.Package_name.from_string name |> Result.expect ~msg:("Expected valid package name: " ^ name)
...
let parse_manifest = fun ~path ~relative_path toml -> Std.Data.Toml.parse toml |> Result.expect ~msg:"Expected package TOML to parse" |> Riot_model.Package_manifest.from_toml ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path ~relative_path |> Result.expect ~msg:"Expected package manifest to parse"

let binary_names = fun binaries -> binaries |> List.map ~fn:(
...
|} in
        if Riot_model.Package_name.equal manifest.name (package_name "demo") && manifest.declared_binaries = [ Riot_model.Package.{ name = "custom"; path = Path.v "examples/custom.ml" } ] then
          Ok ()
        else Error "expected package manifest to keep only declared binaries and metadata"
    )
...
|} |> Result.expect ~msg:"Expected package TOML to parse" in
  match Riot_model.Package_manifest.from_toml manifest ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:(Path.v "/tmp/demo") ~relative_path:(Path.v "packages/demo") with
  | Error (Riot_model.Package.InvalidDependency (Riot_model.Package.InvalidDependencyRequirement { dependency_name; requirement; _ })) when String.equal dependency_name "std" && String.equal requirement "not-a-semver-range" -> Ok ()
  | Error err -> Error ("expected typed dependency requirement error, got " ^ Riot_model.Package_manifest.error_message err)
...
|} in
        ...
        if pkg.sources.src = [ Path.v "src/main.ml" ] && pkg.sources.native = [] && pkg.sources.tests = [] && pkg.sources.examples = [] && pkg.sources.bench = [] && binary_names pkg.binaries = [ "demo" ] then
          Ok ()
        else Error "expected runtime realization to load only runtime sources and binaries"
    )

let test_manifest_realize_runtime_keeps_nested_runtime_modules = fun _ctx ->
  with_tempdir "riot_model_package_manifest_runtime_nested"
    (
      fun tmpdir ->
        let src_dir = Path.(tmpdir / Path.v "src") in
        let net_dir = Path.(src_dir / Path.v "net") in
        Result.expect (Fs.create_dir_all net_dir) ~msg:"Failed to create nested runtime source directory";
        ...
        Result.expect (Fs.write "type handler = socket:Udp_socket.t -> bytes -> unit\n" Path.(net_dir / Path.v "udp_server.mli")) ~msg:"Failed to write udp_server interface";
        Result.expect (Fs.write "type handler = socket:Udp_socket.t -> bytes -> unit\n" Path.(net_dir / Path.v "udp_server.ml")) ~msg:"Failed to write udp_server implementation";
        let manifest = parse_manifest ~path:tmpdir ~relative_path:(Path.v "packages/demo") {|
[package]
name = "demo"
version = "0.1.0"
...
|} in
        ...
        let rec run iteration =
          if iteration = 0 then
            ...
            let pkg = Riot_model.Package_manifest.realize ~intent:Riot_model.Package_manifest.Runtime manifest in
            let actual = pkg.sources.src |> List.sort ~compare:Path.compare in
            if actual = expected then
              run (iteration - 1)
            else Error ("expected nested runtime realization sources [" ^ String.concat ", " (List.map expected ~fn:Path.to_string) ^ "] but got [" ^ String.concat ", " (List.map actual ~fn:Path.to_string) ^ "]")
        in
        run 25
    )

let test_manifest_realize_runtime_stays_complete_across_repeated_parallel_scans = fun _ctx ->
  with_tempdir "riot_model_package_manifest_runtime_parallel_complete"
    (
      fun tmpdir ->
        let src_dir = Path.(tmpdir / Path.v "src") in
        let net_dir = Path.(src_dir / Path.v "net") in
        ...
        in
        List.for_each runtime_files ~fn:(
          fun (rel_path, contents) -> Result.expect (Fs.write contents Path.(tmpdir / rel_path)) ~msg:("Failed to write runtime file " ^ Path.to_string rel_path)
        );
        let manifest = parse_manifest ~path:tmpdir ~relative_path:(Path.v "packages/demo") {|
[package]
name = "demo"
version = "0.1.0"
|} in
        let expected_src = runtime_files |> List.map ~fn:(
          fun (path, _) -> path
        ) |> List.filter ~fn:(
          fun path -> String.starts_with ~prefix:"src/" (Path.to_string path)
        ) |> sort_paths in
        let expected_native = runtime_files |> List.map ~fn:(
          fun (path, _) -> path
        ) |> List.filter ~fn:(
          fun path -> String.starts_with ~prefix:"native/" (Path.to_string path)
        ) |> sort_paths in
        let rec run iteration =
          if iteration = 0 then
            Ok ()
            ...
            if actual_src = expected_src && actual_native = expected_native then
              run (iteration - 1)
            else Error ("expected repeated runtime realization sources src=[" ^ String.concat ", " (List.map expected_src ~fn:Path.to_string) ^ "] native=[" ^ String.concat ", " (List.map expected_native ~fn:Path.to_string) ^ "] but got src=[" ^ String.concat ", " (List.map actual_src ~fn:Path.to_string) ^ "] native=[" ^ String.concat ", " (List.map actual_native ~fn:Path.to_string) ^ "]")
        in
        run 25
    )
...
|} in
        ...
        if pkg.sources.src = [] && pkg.sources.native = [] && pkg.sources.tests = [] && pkg.sources.examples = [] && pkg.sources.bench = [] && pkg.binaries = [] then
          Ok ()
        else Error "expected build realization to skip loading sources and binaries"
    )
...
|} in
        ...
        if pkg.sources.src = [ Path.v "src/demo.ml" ] && pkg.sources.tests = [ Path.v "tests/demo_tests.ml" ] && pkg.sources.examples = [] && pkg.sources.bench = [] && binary_names pkg.binaries = [ "demo_tests" ] then
          Ok ()
        else Error "expected test realization to load test modules but ignore support fixtures"
    )

let test_manifest_to_package_preserves_declared_binaries_without_loading_sources = fun _ctx ->
  with_tempdir "riot_model_package_manifest_package"
    (
      fun tmpdir ->
        let src_dir = Path.(tmpdir / Path.v "src") in
        let tests_dir = Path.(tmpdir / Path.v "tests") in
...
|} in
        ...
        if binary_names pkg.binaries = [ "demo-example"; "demo-tests" ] && pkg.sources.src = [] && pkg.sources.native = [] && pkg.sources.tests = [] && pkg.sources.examples = [] && pkg.sources.bench = [] then
          Ok ()
        else Error "expected manifest conversion to preserve declared binaries without scanning sources"
    )
