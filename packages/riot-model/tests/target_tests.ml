...
let target_strings = fun set -> Riot_model.Target.Set.to_list set |> List.map ~fn:Riot_model.Target.to_string

let make_toolchain_config = fun targets -> Riot_model.Toolchain_config.{ version = Riot_model.Toolchain_config.default_ocaml_version; source = Version Riot_model.Toolchain_config.default_ocaml_version; targets = List.map targets ~fn:target }

let test_parse_normalizes_request_aliases = fun _ctx ->
  match Riot_model.Target.parse "HOST", Riot_model.Target.parse "NaTiVe", Riot_model.Target.parse "ALL", Riot_model.Target.parse "linux" with
  | Riot_model.Target.Host, Riot_model.Target.Host, Riot_model.Target.All, Riot_model.Target.Pattern "linux" -> Ok ()
...
let test_parse_uses_exact_for_valid_target_triples = fun _ctx ->
  ...
  | Riot_model.Target.Exact targets ->
      if target_strings targets = [ "x86_64-unknown-linux-gnu" ] then
        Ok ()
      else Error "expected exact target request to preserve the parsed triple"
  | Riot_model.Target.Host | Riot_model.Target.All | Riot_model.Target.Pattern _ -> Error "expected a valid target triple to parse as an Exact request"

let test_set_deduplicates_targets = fun _ctx ->
  let set = Riot_model.Target.Set.of_list [ target "x86_64-unknown-linux-gnu"; target "x86_64-unknown-linux-gnu"; target "aarch64-unknown-linux-gnu" ] in
  if target_strings set = [ "aarch64-unknown-linux-gnu"; "x86_64-unknown-linux-gnu" ] then
    Ok ()
  else Error "expected target set to deduplicate equal target triples"
...
let test_configured_targets_defaults_to_host = fun _ctx ->
  ...
  if target_strings configured_targets = [ Riot_model.Target.to_string host ] then
    Ok ()
  else Error "expected configured targets to default to the host target"
...
let test_configured_targets_preserves_typed_values = fun _ctx ->
  ...
  let configured_targets = Riot_model.Target.configured_targets ~host config in
  if target_strings configured_targets = [ "aarch64-unknown-linux-gnu"; "x86_64-unknown-linux-gnu" ] then
    Ok ()
  else Error "expected configured targets to preserve typed toolchain targets"
...
let test_configured_targets_deduplicates_repeated_values = fun _ctx ->
  ...
  if target_strings configured_targets = [ "x86_64-unknown-linux-gnu" ] then
    Ok ()
  else Error "expected configured targets to deduplicate repeated typed target triples"
...
let test_resolve_all_uses_all_configured_targets = fun _ctx ->
  ...
  | Ok targets when target_strings targets = [ "aarch64-apple-darwin"; "x86_64-unknown-linux-gnu" ] -> Ok ()
...
let test_resolve_pattern_matches_substrings = fun _ctx ->
  let host = target "aarch64-apple-darwin" in
  let configured_targets = Riot_model.Target.make_set [ host; target "x86_64-unknown-linux-gnu"; target "aarch64-unknown-linux-gnu" ] in
  match Riot_model.Target.resolve ~host ~configured_targets (Riot_model.Target.Pattern "linux") with
  | Ok targets when target_strings targets = [ "aarch64-unknown-linux-gnu"; "x86_64-unknown-linux-gnu" ] -> Ok ()
...
let test_resolve_reports_available_targets_on_miss = fun _ctx ->
  ...
  | Error { pattern; available_targets } ->
      if not (String.equal pattern "windows") then
        ...
        if not (String.equal (String.concat "," actual) "aarch64-apple-darwin,x86_64-unknown-linux-gnu") then
...
let tests = Test.[
  ...
  case "Target.parse uses Exact for valid target triples" test_parse_uses_exact_for_valid_target_triples;
  ...
  case "Target.configured_targets preserves typed configured values" test_configured_targets_preserves_typed_values;
  case "Target.configured_targets deduplicates repeated configured values" test_configured_targets_deduplicates_repeated_values;
  case "Target.resolve Host uses the host target" test_resolve_host_uses_the_host_target;
  case "Target.resolve All uses all configured targets" test_resolve_all_uses_all_configured_targets;
  case "Target.resolve pattern requests match configured target substrings" test_resolve_pattern_matches_substrings;
