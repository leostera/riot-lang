...
let test_parse_normalizes_request_aliases = fun _ctx ->
  ...
  | Riot_model.Target.Host,
    ...
    Riot_model.Target.Pattern "linux" ->
      Ok ()
  | _ ->
      Error "expected target request aliases and patterns to normalize case"
...
let test_resolve_host_uses_the_host_target = fun _ctx ->
  ...
  | Ok targets when target_strings targets = [ "aarch64-apple-darwin" ] ->
      Ok ()
  | Ok _ ->
      Error "expected Host request to resolve to the host target only"
  | Error _ ->
      Error "expected Host request to resolve successfully"
...
let test_resolve_all_uses_all_configured_targets = fun _ctx ->
  ...
  | Ok targets
    when target_strings targets = [ "aarch64-apple-darwin"; "x86_64-unknown-linux-gnu" ] ->
      Ok ()
  | Ok _ ->
      Error "expected All request to resolve to every configured target"
  | Error _ ->
      Error "expected All request to resolve successfully"
...
let test_resolve_pattern_matches_substrings = fun _ctx ->
  ...
  with
  | Ok targets
    when target_strings targets = [ "aarch64-unknown-linux-gnu"; "x86_64-unknown-linux-gnu" ] ->
      Ok ()
  | Ok _ ->
      Error "expected pattern request to resolve matching configured targets"
  | Error _ ->
      Error "expected pattern request to resolve successfully"
...
let test_resolve_reports_available_targets_on_miss = fun _ctx ->
  ...
  | Error { pattern; available_targets } ->
      if not (String.equal pattern "windows") then
        ...
        let actual =
          List.map available_targets ~fn:Riot_model.Target.to_string
        in
        if
          not
            (String.equal
        ...
        Ok ()
  | Ok _ ->
      Error "expected target miss to fail with available targets"
