...
let test_parse_normalizes_request_aliases = fun _ctx ->
  match Riot_model.Target.parse "HOST", Riot_model.Target.parse "NaTiVe", Riot_model.Target.parse "ALL", Riot_model.Target.parse
    "linux" with
...
let test_parse_uses_exact_for_valid_target_triples = fun _ctx ->
  ...
  | Riot_model.Target.Pattern _ -> Error "expected a valid target triple to parse as an Exact request"

let test_set_deduplicates_targets = fun _ctx ->
  let set = Riot_model.Target.Set.of_list
    [
...
let test_resolve_pattern_matches_substrings = fun _ctx ->
  let host = target "aarch64-apple-darwin" in
  let configured_targets = Riot_model.Target.make_set
    [ host; target "x86_64-unknown-linux-gnu"; target "aarch64-unknown-linux-gnu"; ] in
