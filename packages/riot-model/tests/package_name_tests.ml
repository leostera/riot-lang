...
let test_from_string_accepts_valid_package_name = fun _ctx ->
  ...
  | Error err ->
      Error ("expected valid package name, got error: " ^ Riot_model.Package_name.error_message err)
...
let test_from_string_rejects_invalid_leading_character = fun _ctx ->
  ...
  | Error err ->
      Error ("expected InvalidLeadingCharacter, got: " ^ Riot_model.Package_name.error_message err)
...
let test_from_string_rejects_invalid_suffix = fun _ctx ->
  ...
  | Error err ->
      Error ("expected TrailingDelimiter, got: " ^ Riot_model.Package_name.error_message err)

let tests =
  Test.[
    case "Package_name.from_string accepts valid names" test_from_string_accepts_valid_package_name;
    case
      "Package_name.from_string rejects invalid leading characters"
      test_from_string_rejects_invalid_leading_character;
