...
let source = fun ?(workspace = false) ?(builtin = false) ?path ?source_locator ?ref_ ?version () -> Riot_model.Package.{
  workspace;
  builtin;
  path;
  source_locator;
...
let make_command = fun () -> Package_command.{
  name = "demo";
...
let make_package = fun () ->
  ...
  let publish = Package.{
    ...
    is_public = Some true
  } in Package.make ~name:(package_name "minttea") ~path:(Path.v "packages/minttea") ~relative_path:(Path.v "packages/minttea") ~dependencies:[ { name = package_name "std"; source = source ~workspace:true () } ] ~dev_dependencies:[ { name = package_name "propane"; source = source ~workspace:true () } ] ~build_dependencies:[ { name = package_name "std"; source = source ~workspace:true () } ] ~binaries:[ { name = "demo-bin"; path = Path.v "src/demo_bin.ml" } ] ~library:{ path = Path.v "src/minttea.ml" } ~sources:{
...
let test_dev_scope_keeps_only_dev_outputs = fun _ctx ->
  ...
  let kept_dev_deps = (projected.dev_dependencies |> List.map ~fn:(
    fun (dep: Package.dependency) -> dep.name
  )) = [ package_name "propane" ] in
  let kept_runtime_deps = (projected.dependencies |> List.map ~fn:(
    fun (dep: Package.dependency) -> dep.name
  )) = [ package_name "std" ] in
  let kept_build_deps = (projected.build_dependencies |> List.map ~fn:(
    fun (dep: Package.dependency) -> dep.name
  )) = [ package_name "std" ] in
  let no_normal_binaries = projected.binaries |> List.all ~fn:(
    fun (bin: Riot_model.Package.binary) -> String.starts_with ~prefix:"tests/" (Path.to_string bin.path) || String.starts_with ~prefix:"examples/" (Path.to_string bin.path) || String.starts_with ~prefix:"bench/" (Path.to_string bin.path)
  ) in
  if no_library && no_commands && no_runtime_sources && kept_dev_deps && kept_runtime_deps && kept_build_deps && no_normal_binaries then
    Ok ()
...
let test_runtime_scope_keeps_build_dependencies_for_hashing = fun _ctx ->
  ...
  if kept_build_deps then
    Ok ()
  else Error "runtime scope should preserve build dependencies as cache inputs"
...
let test_package_hash_changes_when_build_dependency_path_changes = fun _ctx ->
  let with_build_dependency path =
    ...
    let publish = Package.{
      ...
      license = Some "Apache-2.0";
      is_public = Some true
    } in Package.make ~name:(package_name "minttea") ~path:(Path.v "packages/minttea") ~relative_path:(Path.v "packages/minttea") ~dependencies:[ { name = package_name "std"; source = source ~workspace:true () } ] ~dev_dependencies:[ { name = package_name "propane"; source = source ~workspace:true () } ] ~build_dependencies:[ { name = package_name "fixme"; source = source ~path:(Path.v path) () } ] ~binaries:[ { name = "demo-bin"; path = Path.v "src/demo_bin.ml" } ] ~library:{ path = Path.v "src/minttea.ml" } ~sources:{
      src = [ Path.v "src/minttea.ml"; Path.v "src/demo_cmd.ml" ];
      native = [];
      tests = [ Path.v "tests/model_tests.ml" ];
      examples = [];
  ...
  with_tempdir "riot_model_package"
    (
      fun tmpdir ->
        ...
        Result.expect (Fs.write "let () = ()\n" Path.(examples_dir / Path.v "test_https_httpbin.ml")) ~msg:"Failed to write explicit example";
        Result.expect (Fs.write "let () = ()\n" Path.(examples_dir / Path.v "simple_https.ml")) ~msg:"Failed to write autodiscovered example";
        let manifest = Std.Data.Toml.parse {|
[package]
name = "demo"
version = "0.1.0"
...
|} |> Result.expect ~msg:"Expected package TOML to parse" in
        let pkg = Riot_model.Package.from_toml manifest ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:tmpdir ~relative_path:(Path.v "packages/demo") |> Result.expect ~msg:"Expected package manifest to parse" in
        let binary_names = pkg.binaries |> List.map ~fn:(
        ...
        | _ -> Error ("expected declared example binaries to suppress same-bucket autodiscovery, got: [" ^ String.concat ", " binary_names ^ "]")
    )
...
path = "src/custom.ml"
|} |> Result.expect ~msg:"Expected package TOML to parse" in
        let pkg = Riot_model.Package.from_toml manifest ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:tmpdir ~relative_path:(Path.v ".") |> Result.expect ~msg:"Expected package manifest to parse" in
        let binary_names = pkg.binaries |> List.map ~fn:(
          fun (bin: Riot_model.Package.binary) -> bin.name
        ) |> List.sort ~compare:String.compare in
        match binary_names with
        | [ "custom" ] -> Ok ()
        | _ -> Error ("expected declared runtime binaries to suppress src/main.ml autodiscovery, got: [" ^ String.concat ", " binary_names ^ "]")
    )
...
|} |> Result.expect ~msg:"Expected package TOML to parse" in
        ...
        | [ Riot_model.Package.{ name; path } ] ->
            if String.equal name "hello-world" && Path.equal path (Path.v "src/main.ml") then
              Ok ()
            else Error ("expected src/main.ml to autodiscover hello-world runtime binary, got " ^ name ^ " at " ^ Path.to_string path)
        | binaries -> Error ("expected exactly one autodiscovered runtime binary, got " ^ Int.to_string (List.length binaries))
    )
...
|} |> Result.expect ~msg:"Expected package TOML to parse" in
        ...
        if pkg.sources.src = [ Path.v "src/demo.ml" ] && pkg.sources.native = [] then
          Ok ()
        else Error "expected hidden source entries to be ignored"
    )

let test_scan_sources_ignores_test_support_entries = fun _ctx ->
  with_tempdir "riot_model_test_support_sources"
    (
      fun tmpdir ->
        let src_dir = Path.(tmpdir / Path.v "src") in
        ...
        Result.expect (Fs.write "let () = ()\n" Path.(fixtures_dir / Path.v "0056_bad-module-name.ml")) ~msg:"Failed to write fixture source";
        ...
        Result.expect (Fs.write "let () = ()\n" Path.(autofix_fixtures_dir / Path.v "rewrite.ml")) ~msg:"Failed to write autofix fixture source";
        Result.expect (Fs.write "let () = ()\n" Path.(workspace_fixtures_dir / Path.v "nested.ml")) ~msg:"Failed to write workspace fixture source";
        let manifest = Std.Data.Toml.parse {|
[package]
name = "demo"
version = "0.1.0"
...
|} |> Result.expect ~msg:"Expected package TOML to parse" in
        let pkg = Riot_model.Package.from_toml manifest ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:tmpdir ~relative_path:(Path.v "packages/demo") |> Result.expect ~msg:"Expected package manifest to parse" in
        if pkg.sources.tests = [ Path.v "tests/demo_tests.ml" ] && pkg.binaries = [ Riot_model.Package.{ name = "demo_tests"; path = Path.v "tests/demo_tests.ml" } ] then
          Ok ()
        else Error "expected test support entries under tests/fixtures|generated|diagnostics|autofix_fixtures|workspace_fixtures to be ignored"
    )
  ...
  with_tempdir "riot_model_test_support_boundary_sources"
    (
      fun tmpdir ->
        ...
        Result.expect (Fs.write "let () = ()\n" Path.(fixtures_generated_dir / Path.v "keep.ml")) ~msg:"Failed to write similarly named test source";
        let manifest = Std.Data.Toml.parse {|
[package]
name = "demo"
version = "0.1.0"
...
|} |> Result.expect ~msg:"Expected package TOML to parse" in
        let pkg = Riot_model.Package.from_toml manifest ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:tmpdir ~relative_path:(Path.v "packages/demo") |> Result.expect ~msg:"Expected package manifest to parse" in
        let expected_tests = [ Path.v "tests/demo_tests.ml"; Path.v "tests/fixtures-generated/keep.ml" ] in
        if pkg.sources.tests = expected_tests then
          Ok ()
        else Error "expected only exact test support directories to be ignored"
    )
...
|} |> Result.expect ~msg:"Expected package TOML to parse" in
        ...
        if pkg.sources.src = [ Path.v "src/demo.ml" ] then
          Ok ()
        else Error "expected package-root gitignore entries to prune source scanning"
    )
...
|} |> Result.expect ~msg:"Expected package TOML to parse" in
        let pkg = Riot_model.Package.from_toml manifest ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:tmpdir ~relative_path:(Path.v "packages/demo") |> Result.expect ~msg:"Expected package manifest to parse" in
        if pkg.sources.src = [ Path.v "src/demo.ml" ] && pkg.sources.tests = [ Path.v "tests/demo_tests.ml" ] && pkg.sources.examples = [ Path.v "examples/demo.ml" ] && pkg.sources.bench = [ Path.v "bench/demo_bench.ml" ] && pkg.sources.native = [ Path.v "native/demo.c" ] then
          Ok ()
        else Error "expected non-OCaml files outside native/ to be ignored during source scanning"
    )
...
|} |> Result.expect ~msg:"Expected package TOML to parse" in
        ...
        if pkg.sources.tests = [ Path.v "tests/demo_tests.ml" ] && pkg.binaries = [ Riot_model.Package.{ name = "demo_tests"; path = Path.v "tests/demo_tests.ml" } ] then
          Ok ()
        else Error "expected tests/deps_fixtures to be treated as non-compilable support input"
    )
...
|} |> Result.expect ~msg:"expected workspace TOML to parse" in
...
let test_package_fmt_ignore_loads = fun _ctx ->
  with_tempdir "riot_model_fmt_config"
    (
      fun tmpdir ->
...
[fmt]
ignore = ["fixtures"]
|} |> Result.expect ~msg:"expected legacy fmt TOML to parse" in
  let config = Riot_model.Fmt_config.of_toml toml in
...
std = ">= 1.2.3"
|} |> Result.expect ~msg:"expected package TOML to parse" in
  let pkg = Riot_model.Package.from_toml manifest ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:(Path.v "/tmp/demo") ~relative_path:(Path.v "packages/demo") |> Result.expect ~msg:"expected package manifest to parse" in
  match pkg.dependencies with
  | [ { Riot_model.Package.source = { workspace = false; builtin = false; path = None; version = Some requirement }; _ } ] ->
...
|} |> Result.expect ~msg:"expected package TOML to parse" in
  match Package.from_toml manifest ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:(Path.v "/tmp/demo") ~relative_path:(Path.v "packages/demo") with
...
|} |> Result.expect ~msg:"expected package TOML to parse" in
  let pkg = Riot_model.Package.from_toml manifest ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:(Path.v "/tmp/demo") ~relative_path:(Path.v "packages/demo") |> Result.expect ~msg:"expected package manifest to parse" in
  match pkg.dependencies with
  | [ { Riot_model.Package.name; source = { builtin = true; version = Some requirement; _ } } ] when Package_name.equal name (package_name "stdlib") && String.equal (Std.Version.requirement_to_string requirement) "*" -> Ok ()
...
stdlib = ">= 1.0.0"
|} |> Result.expect ~msg:"expected package TOML to parse" in
  match Riot_model.Package.from_toml manifest ~workspace_deps:[] ~workspace_dev_deps:[] ~workspace_build_deps:[] ~path:(Path.v "/tmp/demo") ~relative_path:(Path.v "packages/demo") with
...
let test_package_json_roundtrips_registry_requirement = fun _ctx ->
  ...
  let package = Package.make ~name:(package_name "demo") ~path:(Path.v "/tmp/demo") ~relative_path:(Path.v "packages/demo") ~dependencies:[ { name = package_name "std"; source = source ~version:requirement () } ] () in
...
|} |> Result.expect ~msg:"expected workspace TOML to parse" in
  ...
  | Error (Riot_model.Workspace_manifest.DependencyError (Riot_model.Workspace_manifest.DependencyFieldMustBeString { dependency_name = "std"; field = Riot_model.Workspace_manifest.Version })) -> Ok ()
  | Error err -> Error ("expected typed dependency version error, got " ^ Riot_model.Workspace_manifest.error_message err)
...
let test_workspace_manager_resolves_member_path_dependencies_relative_to_package = fun _ctx ->
  with_tempdir "riot_model_workspace_paths"
    (
      fun root ->
        let write path content = Fs.write content path |> Result.expect ~msg:("expected write to succeed: " ^ Path.to_string path) in
        let mkdir path = Fs.create_dir_all path |> Result.expect ~msg:("expected mkdir to succeed: " ^ Path.to_string path) in
...
vendor = { path = "../vendor" }
|};
        write Path.(root / Path.v "packages/vendor/riot.toml") {|
[package]
name = "vendor"
version = "0.1.0"
...
kernel = { path = "../kernel" }
|};
        write Path.(root / Path.v "packages/kernel/riot.toml") {|
[package]
name = "kernel"
version = "0.1.0"
|};
        ...
        | Ok (workspace, errors) ->
            if errors != [] then
              Error ("expected no workspace loading errors, got: " ^ String.concat "; " (List.map errors ~fn:Riot_model.Workspace_manager.load_error_to_string))
            else
              let names = workspace.Riot_model.Workspace_manifest.packages |> List.map ~fn:(
                fun (p: Riot_model.Package_manifest.t) -> p.name
              ) |> List.sort ~compare:Riot_model.Package_name.compare |> List.map ~fn:Riot_model.Package_name.to_string in Test.assert_equal ~expected:[ "app"; "kernel"; "vendor" ] ~actual:names;
            Ok ()
    )
...
|};
        ...
        | Ok (_workspace, errors) -> (
          match errors with
          | [ Riot_model.Workspace_manager.PackageFromTomlFailed { package; error = Riot_model.Package.InvalidDependency (Riot_model.Package.InvalidDependencyRequirement { dependency_name; requirement; _ }); _ } ] when String.equal package "app" && String.equal dependency_name "minttea" && String.equal requirement "not-a-version" -> Ok ()
          | [ Riot_model.Workspace_manager.PackageFromTomlFailed { error; _ } ] -> Error ("unexpected member decode error: " ^ Riot_model.Package_manifest.error_message error)
...
|} manifest_path |> Result.map_err ~fn:IO.error_message
        ...
        | Error err -> Error ("expected typed manifest parse error, got " ^ Riot_model.Workspace_manager.manifest_load_error_message err)
  ...
  with_tempdir "riot_model_workspace_scan_missing_root"
    (
      fun root ->
        ...
        let result =
          let* () = Env.set_current_dir root |> Result.map_err ~fn:path_error_message
          in
          let workspace_manager = Riot_model.Workspace_manager.create () in
          ...
          | Error err -> Error ("expected NoWorkspaceRootFound, got " ^ Riot_model.Workspace_manager.scan_error_message err)
...
|} manifest_path |> Result.map_err ~fn:IO.error_message
        ...
        let workspace_manager = Riot_model.Workspace_manager.create () in
        match Riot_model.Workspace_manager.scan workspace_manager root with
        | Error (Riot_model.Workspace_manager.WorkspaceManifestDecodeFailed { path; _ }) when Path.equal path manifest_path -> Ok ()
        | Error err -> Error ("expected typed workspace manifest decode error, got " ^ Riot_model.Workspace_manager.scan_error_message err)
...
|};
        ...
        | Ok (workspace, errors) ->
            if not (List.is_empty errors) then
              Error ("expected missing path+version dependency to defer to later resolution, got: " ^ String.concat "; " (List.map errors ~fn:Riot_model.Workspace_manager.load_error_to_string))
            else
              let names = workspace.Riot_model.Workspace_manifest.packages |> List.map ~fn:(
                fun (p: Riot_model.Package_manifest.t) -> p.name
              ) |> List.sort ~compare:Riot_model.Package_name.compare |> List.map ~fn:Riot_model.Package_name.to_string in Test.assert_equal ~expected:[ "app" ] ~actual:names;
            Ok ()
    )
    ...
    (
      fun root ->
        ...
        let result =
          ...
          let* () = Fs.create_dir_all src_dir |> Result.map_err ~fn:IO.error_message
          in
          let* () = Fs.write {|
[package]
name = "demo"
...
|} Path.(root / Path.v "riot.toml") |> Result.map_err ~fn:IO.error_message
          in
          let* () = Fs.write "let () = print_endline \"demo\"\n" Path.(src_dir / Path.v "demo.ml") |> Result.map_err ~fn:IO.error_message
          in
          let* () = Env.set_current_dir root |> Result.map_err ~fn:path_error_message
          in
          let workspace_manager = Riot_model.Workspace_manager.create () in
          ...
          | Ok (workspace, errors) ->
              if not (List.is_empty errors) then
                Error ("expected no standalone package load errors, got: " ^ String.concat "; " (List.map errors ~fn:Riot_model.Workspace_manager.load_error_to_string))
              else
                match workspace.Riot_model.Workspace_manifest.packages with
                | [ package ] ->
                    if Riot_model.Package_name.equal package.name (package_name "demo") && Path.equal package.relative_path (Path.v ".") then
                      Ok ()
                    else Error ("expected detached package scan to synthesize a one-package workspace, got root=" ^ Path.to_string workspace.root ^ " package=" ^ Riot_model.Package_name.to_string package.name ^ " relative=" ^ Path.to_string package.relative_path)
                | packages -> Error ("expected one package, got " ^ Int.to_string (List.length packages) ^ " root=" ^ Path.to_string workspace.root ^ " names=" ^ String.concat ", " (List.map packages ~fn:(
                  fun (pkg: Riot_model.Package_manifest.t) -> Riot_model.Package_name.to_string pkg.name
                )))
        in
        let _ =
          match original_dir with
...
|} |> Result.expect ~msg:"expected user config TOML to parse" in
...
let test_user_config_load_reads_config_file = fun _ctx ->
  with_tempdir "riot_model_user_config"
    (
      fun tmpdir ->
...
|} |> Result.expect ~msg:"expected user config TOML to parse" in
  ...
  | Error err -> Error (Riot_model.User_config.message err)
  | Ok config -> (
    match List.find config.Riot_model.User_config.registries ~fn:(
      fun (name, _registry) -> String.equal name "pkgs.ml"
    ) with
    ...
    | Some (_name, registry) ->
        if not (String.equal (Net.Uri.to_string registry.api_url) "https://api.pkgs.ml/") then
          ...
          if not (String.equal (Net.Uri.to_string registry.cdn_url) "https://cdn.pkgs.ml/") then
            Error "expected cdn_url to parse"
          else
            if not (registry.api_token = Some "publish-token") then
              Error "expected api_token to parse"
            else Ok ()
  )

let test_user_config_save_roundtrips_default_registry_config = fun _ctx ->
  with_tempdir "riot_model_user_config_default"
    (
      fun tmpdir ->
        let config_path = Path.(tmpdir / Path.v "config.toml") in
        Riot_model.User_config.save Riot_model.User_config.default config_path |> Result.expect ~msg:"expected default config to write";
        match Riot_model.User_config.load config_path with
        | Error err -> Error (Riot_model.User_config.message err)
        | Ok config -> (
          match Riot_model.User_config.api_token config ~registry_name:"pkgs.ml" with
...
|} |> Result.expect ~msg:"expected user config TOML to parse" in
  match Riot_model.User_config.of_toml toml with
  | Error (Riot_model.User_config.InvalidRegistryConfig { registry_name; error = Riot_model.User_config.FieldMustBeString Riot_model.User_config.Api_url }) when String.equal registry_name "pkgs.ml" -> Ok ()
  ...
  with_tempdir "riot_model_workspace_operational_config_missing"
    (
      fun tmpdir ->
        ...
        | Ok config ->
            if config.cache.keep_generations = 10 && Int64.equal config.cache.max_size_bytes (Int64.mul 50L 1_073_741_824L) && Option.is_none config.test.small_test_timeout && config.test.flaky_max_retries = 0 then
              Ok ()
            else Error "expected missing .riot/config.toml to use built-in operational defaults"
    )
  ...
  with_tempdir "riot_model_workspace_operational_config_parse"
    (
      fun tmpdir ->
        ...
        Result.expect (Fs.write {|
[riot.cache]
keep_generations = 5
max_size = "2 GiB"
|} Path.(riot_dir / Path.v "config.toml")) ~msg:"Failed to write .riot/config.toml";
        match Riot_model.Workspace_operational_config.load ~workspace_root:tmpdir with
        | Error err -> Error (Riot_model.Workspace_operational_config.message err)
        | Ok config ->
            if config.cache.keep_generations = 5 && Int64.equal config.cache.max_size_bytes (Int64.mul 2L 1_073_741_824L) then
              Ok ()
            else Error "expected .riot/config.toml to override cache policy"
    )
...
|} Path.(riot_dir / Path.v "config.toml")) ~msg:"Failed to write .riot/config.toml";
        ...
        | Error err -> Error (Riot_model.Workspace_operational_config.message err)
        | Ok config ->
            if config.test.small_test_timeout = Some (Time.Duration.from_millis 500) && config.test.flaky_max_retries = 3 then
              Ok ()
            else Error "expected .riot/config.toml to parse riot.test policy"
    )
...
|} Path.(riot_dir / Path.v "config.toml")) ~msg:"Failed to write .riot/config.toml";
        match Riot_model.Workspace_operational_config.load ~workspace_root:tmpdir with
        | Error (Riot_model.Workspace_operational_config.InvalidConfig { error = CacheConfig (InvalidMaxSize (UnsupportedUnit unit_name)); _ }) when String.equal unit_name "frogs" -> Ok ()
        | Error err -> Error ("expected typed max_size unit error, got " ^ Riot_model.Workspace_operational_config.message err)
...
let test_debug_profile_defaults_to_native_with_debug_symbols = fun _ctx ->
  ...
  if profile.kind = Riot_model.Ocaml_compiler.Native && List.contains flags ~value:"-inline" && List.contains flags ~value:"0" && List.contains flags ~value:"-g" then
    Ok ()
  else Error ("expected debug profile to default to native with -inline 0 -g, got kind=" ^ Riot_model.Ocaml_compiler.compilation_kind_to_string profile.kind ^ " flags=[" ^ String.concat ", " flags ^ "]")
...
let test_release_profile_defaults_to_strict_native_optimization = fun _ctx ->
  ...
  if not (profile.kind = Riot_model.Ocaml_compiler.Native) then
    ...
    if not (List.contains flags ~value:"-noassert") then
      Error "expected release profile to include -noassert"
    else
      if not (List.contains flags ~value:"-compact") then
        Error "expected release profile to include -compact"
      else
        if not (List.contains flags ~value:"-inline" && List.contains flags ~value:"100") then
          Error "expected release profile to include -inline 100"
        else
          if not (List.contains flags ~value:"-warn-error" && List.contains flags ~value:"+a") then
