...
let write_toolchain_toml = fun ~root content ->
  Fs.write content Path.(root / Path.v "ocaml-toolchain.toml")
  |> Result.expect ~msg:"failed to write ocaml-toolchain.toml"

let make_workspace = fun root ->
  Riot_model.Workspace.make_realized ~root ~packages:[] ()

let target_strings = fun targets ->
  List.map targets ~fn:System.TargetTriple.to_string

let test_from_workspace_defaults_without_toolchain_file = fun _ctx ->
  with_tempdir "riot_model_toolchain_default"
    (fun root ->
      let config = Riot_model.Toolchain_config.from_workspace (make_workspace root) in
...
let test_from_workspace_parses_local_path_sources = fun _ctx ->
  with_tempdir "riot_model_toolchain_path"
    (fun root ->
      ...
      | Riot_model.Toolchain_config.Url _ ->
          Error "expected path-based toolchain source to parse as Path")

let tests =
  Test.[
    case
      "Toolchain_config.from_workspace defaults without ocaml-toolchain.toml"
      test_from_workspace_defaults_without_toolchain_file;
    case
      "Toolchain_config.from_workspace parses and normalizes typed targets"
      test_from_workspace_parses_and_normalizes_targets;
    case
      "Toolchain_config.from_workspace parses local path sources"
      test_from_workspace_parses_local_path_sources;
  ]

let () =
  Runtime.run ~main:(fun ~args -> Test.Cli.main ~name:"toolchain_config" ~tests ~args) ~args:Env.args ()
