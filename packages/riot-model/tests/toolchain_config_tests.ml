...
let write_toolchain_toml = fun ~root content -> Fs.write content Path.(root / Path.v "ocaml-toolchain.toml") |> Result.expect ~msg:"failed to write ocaml-toolchain.toml"
    ...
    (
      fun root ->
        ...
        if not (String.equal config.version Riot_model.Toolchain_config.default_ocaml_version) then
          ...
          if not (List.is_empty config.targets) then
    ...
    (
      fun root ->
        ...
        if not (String.equal config.version "5.5.0-riot.4") then
          ...
          if not (String.equal (String.concat "," (target_strings config.targets)) "aarch64-unknown-linux-gnu,x86_64-unknown-linux-gnu") then
      ...
      fun root ->
        ...
        | Riot_model.Toolchain_config.Path path ->
            if Path.equal path (Path.v "./vendor/ocaml") then
              Ok ()
            else Error "expected local path toolchain source to roundtrip"
        | Riot_model.Toolchain_config.Version _ | Riot_model.Toolchain_config.Url _ -> Error "expected path-based toolchain source to parse as Path"
    )

let tests = Test.[ case "Toolchain_config.from_root defaults without ocaml-toolchain.toml" test_from_workspace_defaults_without_toolchain_file; case "Toolchain_config.from_root parses and normalizes typed targets" test_from_workspace_parses_and_normalizes_targets; case "Toolchain_config.from_root parses local path sources" test_from_workspace_parses_local_path_sources ]
