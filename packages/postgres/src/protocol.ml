...
module Error = struct
  ...
  let severity err = err.severity
  let sqlstate err = err.sqlstate
  let message err = err.message
  let detail err = err.detail
  let hint err = err.hint
  let position err = err.position
  let internal_position err = err.internal_position
  let internal_query err = err.internal_query
  let where_context err = err.where_context
  let schema_name err = err.schema_name
  let table_name err = err.table_name
  let column_name err = err.column_name
  let datatype_name err = err.datatype_name
  let constraint_name err = err.constraint_name
  let source_file err = err.source_file
  let source_line err = err.source_line
  let source_routine err = err.source_routine
  ...
  let from_json json =
    ...
    in
    {
      ...
      sqlstate = (match get_string "sqlstate" with
                  | Some s -> Some (Sqlstate.of_string s)
                  | None -> None);
      message = (match get_string "message" with
                 | Some m -> m
                 | None -> "Unknown error");
  ...
  let to_json err =
    ...
    let fields = match err.severity with
      | Some sev -> fields @ [("severity", string sev)]
      | None -> fields
    in
    
    let fields = match err.sqlstate with
      | Some code -> fields @ [("sqlstate", string (Sqlstate.to_string code))]
      | None -> fields
    in
    
    let fields = match err.detail with
      | Some d -> fields @ [("detail", string d)]
      | None -> fields
    in
    
    let fields = match err.hint with
      | Some h -> fields @ [("hint", string h)]
      | None -> fields
    in
    
    let fields = match err.position with
      | Some p -> fields @ [("position", int p)]
      | None -> fields
    in
    
    let fields = match err.constraint_name with
      | Some n -> fields @ [("constraint", string n)]
      | None -> fields
    in
    
    let fields = match err.schema_name with
      | Some s -> fields @ [("schema", string s)]
      | None -> fields
    in
    
    let fields = match err.table_name with
      | Some t -> fields @ [("table", string t)]
      | None -> fields
    in
    
    let fields = match err.column_name with
      | Some c -> fields @ [("column", string c)]
      | None -> fields
    in
    
    let fields = match err.where_context with
  ...
  let to_string err =
    ...
    let parts = match err.sqlstate with
      | Some code -> 
          parts @ ["SQLSTATE: " ^ Sqlstate.to_string code]
      | None -> parts
    in
    
    (* Add severity *)
    let parts = match err.severity with
      | Some sev -> parts @ ["Severity: " ^ sev]
      | None -> parts
    in
    
    (* Add detail *)
    let parts = match err.detail with
      | Some d -> parts @ ["Detail: " ^ d]
      | None -> parts
    in
    
    (* Add hint *)
    let parts = match err.hint with
      | Some h -> parts @ ["Hint: " ^ h]
      | None -> parts
    in
    
    (* Add position *)
    let parts = match err.position with
      | Some p -> parts @ ["Position: " ^ string_of_int p]
      | None -> parts
    in
    
    (* Add constraint name if present *)
    let parts = match err.constraint_name with
      | Some n -> parts @ ["Constraint: " ^ n]
      | None -> parts
    in
    
    (* Add schema/table/column if present *)
    let parts = match (err.schema_name, err.table_name, err.column_name) with
      | Some s, Some t, Some c -> 
          parts @ ["Location: schema \"" ^ s ^ "\", table \"" ^ t ^ "\", column \"" ^ c ^ "\""]
      | Some s, Some t, None -> 
          parts @ ["Location: schema \"" ^ s ^ "\", table \"" ^ t ^ "\""]
      | None, Some t, Some c -> 
          parts @ ["Location: table \"" ^ t ^ "\", column \"" ^ c ^ "\""]
      | Some s, None, Some c ->
          parts @ ["Location: schema \"" ^ s ^ "\", column \"" ^ c ^ "\""]
    ...
    let parts = match err.where_context with
...
module Oid = struct
  ...
  let of_int n = n
  let to_int t = t
  let zero = 0  (* Special value: not from a table *)
...
module ColumnAttr = struct
  ...
  type t =
    | NotFromTable      (* 0 - computed column or not from a specific table *)
    | Position of int   (* 1..n - column position in table *)

  let of_int = function
    | 0 -> NotFromTable
    | n when n > 0 -> Position n
    | n -> Position n  (* Defensive: treat negative as position *)
...
module TypeSize = struct
  (* Type size in bytes *)
  type t =
    | VariableLength    (* -1: varchar, text, bytea, etc. *)
    | NullTerminated    (* -2: cstring *)
    | Fixed of int      (* >0: fixed number of bytes *)

  let of_int = function
    | -1 -> VariableLength
    | -2 -> NullTerminated
    | n when n > 0 -> Fixed n
    | n -> Fixed n  (* Defensive: treat other negatives as fixed *)
...
module TypeModifier = struct
  ...
  type t =
    | NoModifier        (* -1: no type modifier *)
    | Modifier of int   (* Type-specific encoded value *)
...
module Format = struct
  ...
  let of_int = function
    | 0 -> Text
    | 1 -> Binary
    | _ -> Text  (* Default to text for unknown values *)
...
module Reader = struct
  let parse_backend_message msg_type _length bytes =
        ...
        let rec read_fields n acc =
          if n = 0 then List.rev acc
            ...
            let format =
              ...
              |> Option.expect
                   ~msg:
                     ("Protocol error: expected format (field " ^ name ^ ")")
              |> Format.of_int
            in
            let field : Row.field =
              {
    ...
    | 'D' ->
        ...
        let rec read_columns n acc =
          if n = 0 then List.rev acc
            ...
            in
             if col_len = -1 then read_columns (n - 1) (Row.Null :: acc)
            else
              let value =
    ...
    | 'E' | 'N' ->
        ...
        if msg_char = 'E' then ErrorResponse error else NoticeResponse error
    | '1' -> ParseComplete
    | '2' -> BindComplete
    | '3' -> CloseComplete
    | 'n' -> NoData
