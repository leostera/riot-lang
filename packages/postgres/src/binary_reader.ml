...
let read_byte = fun reader ->
  if is_eof reader then
    ...
    let b = Char.code (Option.unwrap (Bytes.get reader.bytes ~at:reader.offset)) in reader.offset <- reader.offset + 1;
  Some b
...
let read_int32 = fun reader ->
  if remaining reader < 4 then
    ...
    let b4 = Char.code (Option.unwrap (Bytes.get reader.bytes ~at:(reader.offset + 3))) in reader.offset <- reader.offset + 4;
...
let read_int16 = fun reader ->
  if remaining reader < 2 then
    ...
    let b1 = Char.code (Option.unwrap (Bytes.get reader.bytes ~at:reader.offset)) in
    let b2 = Char.code (Option.unwrap (Bytes.get reader.bytes ~at:(reader.offset + 1))) in reader.offset <- reader.offset + 2;
...
let read_int64 = fun reader ->
  if remaining reader < 8 then
    ...
    let b8 = Int64.of_int (Char.code (Option.unwrap (Bytes.get reader.bytes ~at:(reader.offset + 7)))) in reader.offset <- reader.offset + 8;
  let result = Int64.(logor (logor (logor (shift_left b1 56) (shift_left b2 48)) (logor (shift_left b3 40) (shift_left b4 32))) (logor (logor (shift_left b5 24) (shift_left b6 16)) (logor (shift_left b7 8) b8))) in Some result
...
let read_string = fun reader ->
  ...
  let rec loop () =
    if is_eof reader then
      ...
      let c = Option.unwrap (Bytes.get reader.bytes ~at:reader.offset) in reader.offset <- reader.offset + 1;
    if c = '\x00' then
