...
module Config = struct
  ...
  let from_string = fun str ->
    ...
    | Ok _ -> (
      ...
      | _ -> Error "Invalid connection string format (use \
...
module Driver = struct
  ...
  type result_set = { rows: Sqlx_driver.Row.t Collections.Queue.t; mutable rows_affected: int }
  ...
  let read_message = fun stream ->
    ...
    | Ok () ->
        ...
        else Ok (msg_type, length, Bytes.create ~size:0)

  let perform_handshake = fun stream (cfg: Config.t) ->
    let startup_msg = Protocol.Writer.startup_message ~user:cfg.user ~database:cfg.database ~application_name:cfg.application_name in
    ...
    | Ok () ->
        let rec read_until_ready () =
          ...
          | Ok (msg_type, length, body) -> (
            ...
            | Protocol.AuthenticationCleartextPassword -> Error (AuthenticationNotSupported "Cleartext password")
            | Protocol.AuthenticationMD5Password _ -> Error (AuthenticationNotSupported "MD5 password")
            ...
            | Protocol.BackendKeyData { process_id; secret_key } ->
                Log.debug ("Backend key data: pid=" ^ string_of_int process_id ^ " secret=" ^ string_of_int secret_key);
                read_until_ready ()
            ...
            | Protocol.NoticeResponse err ->
                ...
                read_until_ready ()
            | _ -> Error (UnexpectedMessage ("During handshake: " ^ String.make ~len:1 ~char:(Char.chr msg_type)))
          )
        in
        read_until_ready ()
  ...
  let connect = fun (cfg: Config.t) ->
    ...
    | Error (Net.Addr.System_error _err) -> (* Host resolution failure - treat as connection refused *)
    Error (TransportError Net.TcpStream.Connection_refused)
    | Error (Net.Addr.Invalid_port_number _ | Net.Addr.Invalid_format _) -> (* Invalid address format - treat as connection refused *)
    Error (TransportError Net.TcpStream.Connection_refused)
    | Ok addr -> (
  ...
  let strip_timezone_name = fun str ->
    ...
    | Some idx when idx > 10 -> (
      ...
      let is_tz_name = String.length after_space <= 5 && String.for_all after_space ~fn:(
        fun c -> Char.uppercase_ascii c = c || c = '/' || c = '_'
      ) in
      if is_tz_name then
  ...
  let parse_pg_timestamp = fun str ->
    ...
    let iso_str =
      ...
      | Some idx ->
          ...
          let after = String.sub str ~offset:(idx + 1) ~len:(String.length str - idx - 1) in before ^ "T" ^ after ^ "Z"
      | None -> str ^ "Z"
    in
  ...
  let parse_pg_timestamptz = fun str ->
    ...
    let iso_str =
      ...
      | Some idx ->
          let before = String.sub str ~offset:0 ~len:idx in
          let after = String.sub str ~offset:(idx + 1) ~len:(String.length str - idx - 1) in before ^ "T" ^ after
      | None -> str
    in
    DateTime.parse iso_str
  ...
  let decode_value = fun (field: Protocol.Row.field) (value: string) ->
    ...
    | Protocol.TypeOid.Json | Protocol.TypeOid.Jsonb -> Sqlx_driver.Value.json value
    ...
    | Protocol.TypeOid.Text | Protocol.TypeOid.Varchar | Protocol.TypeOid.Char -> Sqlx_driver.Value.string value
    | Protocol.TypeOid.Bytea | Protocol.TypeOid.Oid | Protocol.TypeOid.Date | Protocol.TypeOid.Time | Protocol.TypeOid.Interval | Protocol.TypeOid.Unknown _ -> Sqlx_driver.Value.string value

  (* Format DateTime.t to PostgreSQL timestamp format *)
  let datetime_to_pg_format = fun dt ->
    let pad n width =
      let s = string_of_int n in String.make ~len:(max 0 (width - String.length s)) ~char:'0' ^ s
    in
    let micros, _precision = dt.DateTime.microseconds in (* Format: YYYY-MM-DD HH:MM:SS.microseconds *)
    pad dt.year 4 ^ "-" ^ pad dt.month 2 ^ "-" ^ pad dt.day 2 ^ " " ^ pad dt.hour 2 ^ ":" ^ pad dt.minute 2 ^ ":" ^ pad dt.second 2 ^ "." ^ pad micros 6
  ...
  let prepare = fun conn sql ->
    if conn.closed then
      ...
      let name = "stmt_" ^ string_of_int (Random.int 1_000_000 |> Result.expect ~msg:"failed to generate statement id") in
    ...
    if stmt.conn.closed then
      ...
      | Some stream -> (
        ...
        if use_extended_protocol then
          let parse_msg = Protocol.Writer.parse_message ~statement_name:stmt.name ~query:stmt.sql ~param_types:[] in
          ...
          let bind_msg = Protocol.Writer.bind_message ~portal_name:"" ~statement_name:stmt.name ~params:encoded_params in
          let execute_msg = Protocol.Writer.execute_message ~portal_name:"" ~max_rows:0 in
          let sync_msg = Protocol.Writer.sync_message () in
          match write_message stream parse_msg, write_message stream describe_msg, write_message stream bind_msg, write_message stream execute_msg, write_message stream sync_msg with
                  ...
                  | Protocol.DataRow cols ->
                      let row =
                        if List.length !column_info = List.length cols then
                          List.zip !column_info cols |> List.map ~fn:(
                        ...
                        else List.enumerate cols |> List.map ~fn:(
                          fun (index, row_val) ->
                            let value =
                              match row_val with
                              | Protocol.Row.Null -> Sqlx_driver.Value.null
              ...
              let rec read_query_results () =
                ...
                | Ok (msg_type, length, body) -> (
                  ...
                  | Protocol.NoticeResponse err ->
                      Log.info ("PostgreSQL notice: " ^ Protocol.Error.message err);
                      read_query_results ()
                  | _ -> Error (UnexpectedMessage ("During query: " ^ String.make ~len:1 ~char:(Char.chr msg_type)))
                )
              in
              read_query_results ()
      )
