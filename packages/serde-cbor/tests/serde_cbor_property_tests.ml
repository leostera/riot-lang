...
let io_writer_of_buffer =
  let module Write = struct
    ...
    let write = fun buffer ~from ->
      ...
      IO.Buffer.append_slice buffer (IO.Buffer.readable from) |> Result.expect ~msg:"serde-cbor property writer should append buffer contents";
      Ok written
    ...
    let write_vectored = fun buffer ~from ->
      ...
      IO.IoVec.for_each ~fn:(
        fun chunk ->
          IO.Buffer.append_slice buffer chunk |> Result.expect ~msg:"serde-cbor property writer should append slices";
...
let roundtrip_io = fun encode decode equal value ->
  ...
  | Ok () -> (
    match Serde_cbor.from_reader decode (String.to_reader ~chunk_size:io_chunk_size (IO.Buffer.contents buffer)) with
    | Ok decoded -> equal decoded value
...
let bool_roundtrip_prop = run_property "serde-cbor property bool roundtrips" Arbitrary.bool (roundtrip_in_memory Ser.bool De.bool Bool.equal)

let int_roundtrip_prop = run_property "serde-cbor property int roundtrips" Arbitrary.int (roundtrip_in_memory Ser.int De.int Int.equal)

let int32_roundtrip_prop = run_property "serde-cbor property int32 roundtrips" Arbitrary.int32 (roundtrip_in_memory Ser.int32 De.int32 Int32.equal)

let int64_roundtrip_prop = run_property "serde-cbor property int64 roundtrips" Arbitrary.int64 (roundtrip_in_memory Ser.int64 De.int64 Int64.equal)

let float_roundtrip_prop = run_property "serde-cbor property float roundtrips" (Arbitrary.make ~print:Float.to_string finite_float_gen) (roundtrip_in_memory Ser.float De.float Float.equal)

let string_roundtrip_prop = run_property "serde-cbor property string roundtrips" Arbitrary.string (roundtrip_in_memory Ser.string De.string String.equal)

let option_string_roundtrip_prop = run_property "serde-cbor property option string roundtrips" Arbitrary.(option string) (roundtrip_in_memory (Ser.option Ser.string) (De.option De.string) (=))

let string_list_roundtrip_prop = run_property ~examples:composite_examples "serde-cbor property string list roundtrips" Arbitrary.(vector string) (roundtrip_in_memory (Ser.list Ser.string) (De.list De.string) equal_string_vec)

let int_array_roundtrip_prop = run_property ~examples:composite_examples "serde-cbor property int array roundtrips" Arbitrary.(array int) (roundtrip_in_memory (Ser.array Ser.int) (De.array De.int) (=))

let mode_roundtrip_prop = run_property ~examples:composite_examples "serde-cbor property mode roundtrips" mode_arb (roundtrip_in_memory mode_encode mode_decode (=))

let berth_roundtrip_prop = run_property ~examples:composite_examples "serde-cbor property berth roundtrips" berth_arb (roundtrip_in_memory berth_encode berth_decode (=))

let sample_roundtrip_prop = run_property ~examples:composite_examples "serde-cbor property sample roundtrips" sample_arb (roundtrip_in_memory sample_encode sample_decode equal_sample)

let sample_io_roundtrip_prop = run_property ~examples:composite_examples "serde-cbor property sample roundtrips over io" sample_arb (roundtrip_io sample_encode sample_decode equal_sample)
