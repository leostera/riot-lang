...
let io_writer_of_buffer =
  let module Write = struct
    ...
    let write = fun buffer ~from ->
      ...
      IO.Buffer.append_slice buffer (IO.Buffer.readable from) |> Result.expect ~msg:"serde-cbor test writer should append buffer contents";
      Ok written
    ...
    let write_vectored = fun buffer ~from ->
      ...
      IO.IoVec.for_each ~fn:(
        fun chunk ->
          IO.Buffer.append_slice buffer chunk |> Result.expect ~msg:"serde-cbor test writer should append slices";
...
let byte_values = fun value -> List.init ~count:(String.length value) ~fn:(
  fun index -> Char.code (String.get_unchecked value ~at:index)
)
...
let test_small_positive_int_uses_single_byte = fun _ctx ->
  ...
  | Ok encoded -> Error ("expected CBOR encoding for 10 to be [0x0a], got " ^ String.concat "," (List.map (byte_values encoded) ~fn:Int.to_string))
...
let tests =
  [
    Test.case "serde-cbor small positive ints use a single byte" test_small_positive_int_uses_single_byte;
