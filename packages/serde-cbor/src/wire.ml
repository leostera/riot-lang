...
let add_uint64_be = fun buffer value -> let open Int64 in
...
let rec encode_value = fun buffer value ->
  ...
  | Int value ->
      ...
      else add_header buffer major_negative (Int64.lognot value)
...
let read_uint16_be = fun input ->
  ...
  in
  let* b1 = read_byte input in Ok ((b0 lsl 8) lor b1)
...
let read_uint32_be = fun input ->
  ...
  in
  let* b3 = read_byte input in Ok Int64.(logor (shift_left (of_int b0) 24) (logor (shift_left (of_int b1) 16) (logor (shift_left (of_int b2) 8) (of_int b3))))
...
in
let* b7 = read_byte input in Ok (logor (shift_left (of_int b0) 56) (logor (shift_left (of_int b1) 48) (logor (shift_left (of_int b2) 40) (logor (shift_left (of_int b3) 32) (logor (shift_left (of_int b4) 24) (logor (shift_left (of_int b5) 16) (logor (shift_left (of_int b6) 8) (of_int b7))))))))
...
let decode_half = fun bits ->
  ...
  if Int.equal exponent 0 then
    if Int.equal fraction 0 then
      sign *. 0.0
    else sign *. (2. ** (-14.0)) *. (Float.of_int fraction /. 1024.0)
  else
    if Int.equal exponent 0x1f then
      if Int.equal fraction 0 then
        sign *. Float.infinity
      else Float.nan
    else sign *. (2. ** Float.of_int (exponent - 15)) *. (1.0 +. (Float.of_int fraction /. 1024.0))
...
let read_count = fun input minor kind ->
  let* value = read_argument input minor
  in
  if (
...
let read_text = fun input minor ->
  let* length = read_count input minor "text"
  in
...
let int_of_negative = fun raw ->
  ...
  else Ok (Int64.lognot raw)

let rec read_value = fun input ->
  let* lead = read_byte input
  in
  ...
  | 0 ->
      let* value = read_argument input minor
      in
      let* value = int_of_positive value in Ok (Int value)
  | 1 ->
      let* raw = read_argument input minor
      in
      let* value = int_of_negative raw in Ok (Int value)
  | 3 -> read_text input minor |> Result.map ~fn:(
    fun value -> Text value
  )
  | 4 ->
      let* length = read_count input minor "array"
      in
      ...
      loop length []
  | 5 ->
      let* length = read_count input minor "map"
      in
      let rec loop remaining acc =
        if Int.equal remaining 0 then
          Ok (Map (List.rev acc))
        else
          let* key = read_value input
          in
          let* key =
            match key with
            | Text value -> Ok value
      ...
      loop length []
  | 6 ->
      let* _tag = read_argument input minor in read_value input
  | 7 -> (
    match minor with
    | 20 -> Ok (Bool false)
    | 21 -> Ok (Bool true)
    | 22 -> Ok Null
    | 25 -> read_uint16_be input |> Result.map ~fn:(
      fun bits -> Float (decode_half bits)
    )
    | 26 ->
        let* bits = read_uint32_be input in Ok (Float (Int32.float_of_bits (Int64.to_int32 bits)))
    | 27 ->
        let* bits = read_uint64_be input in Ok (Float (Int64.float_of_bits bits))
...
let from_string = fun input ->
  ...
  if Int.equal state.pos state.length then
    Ok value
  else error "serde-cbor input has trailing bytes after the top-level value"
