...
let bench_error_message = function
  ...
  | SuiteArtifactNotFound { reason; _ } ->
      reason
  | SuiteExecutionError { reason; _ } ->
      reason
  | SuitesFailed count ->
      Int.to_string count ^ " benchmark suite(s) failed"
...
let bench_event_to_json = function
  ...
  | SuiteCompleted { suite; status; stdout; stderr } ->
      Some
        (Data.Json.Object [
            ...
            let result =
              ...
              let rec loop = function
                ...
                | suite :: rest -> (
                    ...
                    with
                    | Error reason ->
                        Error (SuiteArtifactNotFound { suite; reason })
                    | Ok binary_path ->
