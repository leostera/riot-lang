...
let test_building_target_event_to_json = fun () ->
  let actual = Tusk_build.Event.to_json (Tusk_build.BuildingTarget { target = "aarch64-linux"; host = false }) in
  Test.assert_equal
    ~expected:(Some
      (Data.Json.Object [
...
let test_pm_event_to_json_reuses_tusk_model_event_shape = fun () ->
  ...
  let event =
    Tusk_model.Event.create
      ~session_id
      ~level:Tusk_model.Event.Info
      (Tusk_model.Event.PackageDownloadStarted {
        package = "std";
        version = "0.1.0";
        path = "/tmp/std";
      })
  ...
  | Some (Data.Json.Object fields) -> (
      ...
      | Some json ->
          Error ("expected PM event name in JSON, got " ^ Data.Json.to_string json)
      | None ->
          Error "expected PM event name in JSON"
    )
  | Some json ->
