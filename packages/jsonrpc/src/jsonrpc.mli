(** JSON-RPC 2.0 Protocol Implementation
    ...
    This module provides a complete, type-safe implementation of the JSON-RPC 2.0
...
(** Request/response ID type - can be string, number, or null.
...
type error_code =
  ...
  | ServerError of int (** -32000 to -32099: Reserved for implementation-defined server errors *)
...
(** JSON-RPC 2.0 Response - parameterized by result type.
    Uses OCaml's result type to ensure exactly one of success or error. *)
...
(** ApplicationProtocol interface - bridges typed application values and JSON-RPC.
    
    This module type defines how to convert between your application's typed
    request/response types and the JSON representation used by JSON-RPC.
    Both client and server use the same protocol to ensure type safety
    ...
    {[
      module MyProtocol : ApplicationProtocol = struct
        ...
        let request_to_params = function
          | GetUser id -> 
              { method_ = "getUser"; 
                params = Positional [Json.int id] }
          | CreateUser (name, email) ->
              { method_ = "createUser";
                params = Named [("name", Json.string name); 
                               ("email", Json.string email)] }
        
        let response_to_json = function
          | User u -> Json.obj ["type", Json.string "user"; 
                                "data", user_to_json u]
          | Success -> Json.obj ["type", Json.string "success"]
          | Error msg -> Json.obj ["type", Json.string "error";
...
module Client : sig
  (** Type-safe JSON-RPC 2.0 Client Implementation 
      ...
      The client is parameterized by request and response types through
      an ApplicationProtocol, ensuring type safety for all RPC calls.
  *)
  ...
  val create :
    ...
    'transport ->
    ('req, 'res) t
  (** Create a new client with the given transport and protocol.
      The protocol defines how to convert between typed values and JSON. *)
  ...
  (** Send a raw JSON-RPC request and wait for response.
      ...
      For typed calls, use send_request/receive_response instead. *)
  ...
  (** Send a notification (no response expected).
      Notifications are fire-and-forget - the server will not send a response. *)

  val call_batch : ('req, 'res) t -> 'req list -> (('res response) list, string) result
  (** Send a batch of typed requests and receive batch response.
      All requests are sent together and responses are returned together.
      Useful for reducing round-trip latency. *)

  val send_request : ('req, 'res) t -> 'req -> (unit, string) result
  (** Send a typed request without waiting for response.
      Use with receive_response for streaming or async patterns. *)

  val receive_response : ('req, 'res) t -> (('res response), string) result
  (** Receive and parse a typed response.
      Use after send_request to complete the request/response cycle. *)

  val close : ('req, 'res) t -> unit
  (** Close the client and underlying transport connection *)
end

(** Helper functions for creating responses - REMOVED: use ok and error above instead *)
...
module Server : sig
  ...
  val create : 
    ...
    ('req, 'res) t
  (** Create a new server with the given protocol and method handlers.
      Each handler will be called when its method name is invoked.
      The protocol defines how to convert between typed values and JSON. *)

  val handle_message : ('req, 'res) t -> (string -> unit) -> string -> unit
  (** Process a JSON-RPC message string and call the appropriate handler.
      The reply function will be called with a JSON-RPC response string ready to send.
      For notifications, handlers are called but no reply is sent.
      
      The server automatically:
      1. Converts the typed response to JSON using the protocol
      2. Wraps it in a JSON-RPC response envelope
      3. Passes the complete JSON string to the reply function
  *)
end
