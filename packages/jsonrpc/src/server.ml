...
let handle_message = fun (type req res) (server: (req, res) t) (reply: string -> unit) (message: string) ->
  ...
  | Ok json -> (
    ...
    | Ok request -> (
      ...
      Log.trace ("[JSONRPC SERVER] Available handlers: " ^ Int.to_string (List.length server.handlers));
      List.for_each server.handlers ~fn:(
        fun h -> Log.trace ("[JSONRPC SERVER]   - " ^ h.method_)
      );
      (* Find handler for method *)
      match List.find server.handlers ~fn:(
        fun h -> h.method_ = request.method_
      ) with
      | None ->
            ...
            if Common.is_notification request then
              (
                ...
                if cwd != cwd_after then
                  Log.trace ("[JSONRPC SERVER] CWD changed during handler! " ^ cwd ^ " -> " ^ cwd_after)
              )
            else
