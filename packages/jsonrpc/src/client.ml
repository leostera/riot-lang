...
let receive_response: type req res. (req, res) t -> (res Common.response, Common.error) result = fun (Client c as client) ->
  ...
  | Ok str -> (
    ...
    | Error e -> Error (Common.ParseError { raw_input = str; parse_error = Json.error_to_string e })
    | Ok json -> (
      match json with
      | Json.Object fields -> (
        match Std.Collections.Proplist.get fields ~key:"jsonrpc", Std.Collections.Proplist.get fields ~key:"id" with
          ...
          | Ok id -> (
            ...
            | Some (Json.Object err_fields) ->
                ...
                let message =
                  ...
                  | _ -> "Unknown error"
                in
                let data = Std.Collections.Proplist.get err_fields ~key:"data" in Error (Common.UnknownServerError { code; message; data })
            | Some err_json -> Error (Common.UnknownServerError { code = (-1); message = "Invalid error format"; data = Some err_json })
            | None -> (
              match Std.Collections.Proplist.get fields ~key:"result" with
              | Some result_json -> (
                match P.response_of_json result_json with
                | Ok parsed_result -> Ok { Common.jsonrpc = "2.0"; result = parsed_result; id }
                | Error err_json -> Error (Common.InternalError { context = "parse_response_result"; details = "Failed to parse result: " ^ (Json.to_string err_json) })
              )
              | None -> Error (Common.InvalidRequest { request_json = json; reason = "Response missing both result and error" })
            )
          )
          | Error e -> Error (Common.InvalidRequest { request_json = json; reason = "Invalid ID: " ^ e })
        )
        | _ -> Error (Common.InvalidRequest { request_json = json; reason = "Missing jsonrpc or id field" })
      )
      | _ -> Error (Common.InvalidRequest { request_json = json; reason = "Response must be an object" })
    )
  )
...
let call (type req res) (client: (req, res) t) ~method_ ?params (): (res, Common.error) result =
  ...
  let id = Common.Number c.next_id in
  c.next_id <- c.next_id + 1;
  let jsonrpc_req = Common.request ~method_ ~params:(Option.unwrap_or params ~default:Common.NoParams) ~id () in
  let json = Common.request_to_json jsonrpc_req in
  let str = Json.to_string json in
  match send_raw_request client str with
        ...
        | Json.Object fields -> (
          ...
          | None -> (
            ...
            | Some result_json -> (
              ...
              | Error err_json -> Error (Common.InternalError { context = "call_parse_result"; details = "Failed to parse result: " ^ (Json.to_string err_json) })
            )
            | None -> Error (Common.InvalidRequest { request_json = json; reason = "Response missing result" })
          )
        )
        | _ -> Error (Common.InvalidRequest { request_json = json; reason = "Response must be an object" })
      )
...
let call_batch: type req res. (req, res) t -> req list -> (res Common.response list, Common.error) result = fun client requests ->
  let (Client c) = client in
  let module P = (val c.protocol_mod : Common.ApplicationProtocol with type request = req and type response = res) in
  let jsonrpc_requests = List.map requests ~fn:(
    fun req ->
  ...
  | Ok () -> (
    ...
    | Ok str ->
        let* json = Json.of_string str |> Result.map_err ~fn:(
          fun e -> Common.ParseError { raw_input = str; parse_error = Json.error_to_string e }
        )
        in
        match json with
        | Json.Array responses ->
            let parsed_responses = List.fold_left responses ~init:(Ok []) ~fn:(
              fun acc json_resp ->
                let* responses = acc
                in
                ...
                | Json.Object fields -> (
                  ...
                  | Some (Json.String "2.0"), Some id_json -> (
                    ...
                    | Ok id -> (
                      ...
                      | Some result_json -> (
                        match P.response_of_json result_json with
                        | Ok parsed_result -> Ok ({ Common.jsonrpc = "2.0"; result = parsed_result; id } :: responses)
                        | Error err_json -> Error (Common.InternalError { context = "call_batch_parse_result"; details = "Failed to parse result: " ^ Json.to_string err_json })
                      )
                      | None -> Error (Common.InvalidRequest { request_json = json_resp; reason = "Response missing result" })
                    )
                    | Error e -> Error (Common.InvalidRequest { request_json = json_resp; reason = "Invalid ID: " ^ e })
                  )
                  | _ -> Error (Common.InvalidRequest { request_json = json_resp; reason = "Invalid response in batch" })
                )
                | _ -> Error (Common.InvalidRequest { request_json = json_resp; reason = "Batch response item must be an object" })
            ) in Result.map parsed_responses ~fn:List.rev
        | json -> Error (Common.InvalidRequest { request_json = json; reason = "Batch response must be an array" })
  )
