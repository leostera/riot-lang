...
let error_to_json err =
  ...
  let fields = match err.data with
...
let request_to_json (req : request) =
  ...
  let fields = match req.params with
    | NoParams -> fields
    | params -> ("params", params_to_json params) :: fields
  in
  let fields = match req.id with
    | None -> fields
...
let request_of_json json =
  ...
  | Json.Object fields -> (
      ...
      | Some (Json.String "2.0"), Some (Json.String method_) ->
          ...
          in
          let id_result = match List.assoc_opt "id" fields with
            | None -> Ok None
            | Some j -> (match id_of_json j with
              | Ok parsed_id -> Ok (Some parsed_id)
              | Error e -> Error e)
          in
          (match params, id_result with
          | Ok params, Ok id ->
              Ok { jsonrpc = "2.0"; method_; params; id }
          | Error e, _ -> Error e
          | _, Error e -> Error e)
...
let response_to_json (resp : response) =
  let fields = [
    ("jsonrpc", Json.String resp.jsonrpc);
    ("id", id_to_json resp.id);
  ] in
  let fields = match resp.result, resp.error with
      ...
      | Some (Json.String "2.0"), Some id_json -> (
          ...
          | Ok id ->
              ...
              let error = match List.assoc_opt "error" fields with
                | None -> Ok None
                | Some e -> match error_of_json e with
                  | Ok err -> Ok (Some err)
                  | Error msg -> Error msg
              in
              (match error with
...
let make_request ~method_ ?params ?id () =
  { jsonrpc = version; method_; params = Option.value params ~default:NoParams; id }
...
let make_notification ~method_ ?params () =
  { jsonrpc = version; method_; params = Option.value params ~default:NoParams; id = None }
