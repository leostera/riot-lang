...
let deps t ~cwd ~file ~package_namespace =
  let ocamldep = Path.to_string t in
...
(** Get dependencies for a single file with optional flags - returns
    Module_name.t list *)
let deps_with_flags t ~cwd ~file ~flags ~package_namespace =
  let ocamldep = Path.to_string t in
...
(** Get dependencies for multiple files in one ocamldep call - returns (file,
    deps) list *)
let batch_deps t ~cwd ~files ~package_namespace =
  if files = [] then []
  else
    let ocamldep = Path.to_string t in
    let files_str = String.concat " " (List.map Path.to_string files) in
...
let all_deps t ~cwd ~files ~package_namespace =
  List.map
    (fun file ->
