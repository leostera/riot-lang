...
let detect = fun ?toolchain_root ~target_triplet ->
  ...
  | None ->
      ...
      {sysroot = (
          ...
          | Some path -> Some path
          | None -> bundled_sr
        ); bin_dir = None; bin_prefix; c_compiler = None; }
  | Some cc_path ->
      let sysroot =
        match explicit_sysroot with
        | Some path -> Some path
      ...
      (
        match bundled_cc with
        | Some path when Path.equal path cc_path -> Log.info
        ("✓ Using bundled cross-compiler: " ^ Path.to_string path)
