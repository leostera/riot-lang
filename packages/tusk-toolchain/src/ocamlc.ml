...
let make_invocation ?(output_mode = Normal) ~cwd command_string =
  {
    cwd;
...
let build_invocation t ~cwd ?(includes = []) ?(libs = []) ?(cclibs = [])
    ?(ccflags = []) ?(ccopt_flags = []) ?(cclib_flags = []) ?(output = None)
    ?(mode = Compile) ?(flags = []) sources =
  ...
  let cclibs_str =
    if List.length cclibs > 0 then
      String.concat " "
        (List.map (fun lib -> "-cclib " ^ Path.to_string lib) cclibs)
    else ""
  in
  let ccflags_str =
    if List.length ccflags > 0 then
      String.concat " "
        (List.map (fun flag -> "-ccopt \"" ^ flag ^ "\"") ccflags)
    else ""
  in
  let ccopt_flags_str =
    if List.length ccopt_flags > 0 then
      String.concat " "
        (List.map (fun flag -> "-ccopt \"" ^ flag ^ "\"") ccopt_flags)
    else ""
  in
  let cclib_flags_str =
    if List.length cclib_flags > 0 then
      String.concat " "
        (List.map (fun flag -> "-cclib \"" ^ flag ^ "\"") cclib_flags)
    else ""
  in
...
let generate_interface t ~cwd ~includes ~flags ~output source =
  ...
  in
  make_invocation ~output_mode:(WriteStdoutToFile output) ~cwd
    (String.concat " " ([ base_command t ] @ args))

let compile_c t ~cwd ~includes ?(ccflags = []) ~output source =
  build_invocation t ~cwd ~includes ~ccflags ~output:(Some output) ~mode:Compile
    [ Path.to_string source ]

let create_library t ~cwd ~includes ~output objects =
  build_invocation t ~cwd ~includes ~output:(Some output) ~mode:Library
    (List.map Path.to_string objects)

let create_executable t ~cwd ~includes ~output ~libs ?(cclibs = [])
    ?(ccopt_flags = []) ?(cclib_flags = []) objects =
  let includes_with_dot = Path.v "." :: includes in
  build_invocation t ~cwd ~includes:includes_with_dot ~libs ~cclibs
    ~ccopt_flags ~cclib_flags ~output:(Some output) ~mode:Executable
    ~flags:[ LinkAll ]
    (List.map Path.to_string objects)

let create_shared_library t ~cwd ~includes ~output ~libs ?(cclibs = [])
    ?(ccopt_flags = []) ?(cclib_flags = []) objects =
  let includes_with_dot = Path.v "." :: includes in
  build_invocation t ~cwd ~includes:includes_with_dot ~libs ~cclibs
    ~ccopt_flags ~cclib_flags ~output:(Some output) ~mode:SharedLibrary
    ~flags:[ LinkAll ]
    (List.map Path.to_string objects)

let create_custom_executable t ~cwd ~includes ~output ~libs objects =
  let includes_with_dot = Path.v "." :: includes in
  build_invocation t ~cwd ~includes:includes_with_dot ~libs
    ~output:(Some output) ~mode:CustomExe
    (List.map Path.to_string objects)

let to_string invocation =
  let env_prefix =
    ...
    | env ->
        ...
        ^ " "
  in
  "cd " ^ Path.to_string invocation.cwd ^ " && " ^ env_prefix
  ^ invocation.command_string

let run invocation =
  Log.debug ("[OCAMLC] Running command: " ^ to_string invocation);
  let cmd =
    run_in_dir ~cwd:invocation.cwd ~env:invocation.env invocation.command_string
  in
  ...
  | Ok output when output.Command.status = 0 -> (
      ...
      | WriteStdoutToFile file -> (
          ...
          | Error err ->
              Failed
                ("Failed to write " ^ Path.to_string file ^ ": "
  ...
  | Ok output -> (
      ...
      | WriteStdoutToFile _ ->
          Failed
            ("ocamlc -i failed with exit code "
            ^ Int.to_string output.Command.status ^ ": "
            ^ output.Command.stderr))
