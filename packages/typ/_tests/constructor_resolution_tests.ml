...
let test_constructor_resolution_uses_expected_type = fun _ctx ->
  ...
  if not (List.is_empty diagnostics) then
    ...
    let key_pattern_type =
      inferred_type_at
        snapshot
        source_id
        (expect_substring_offset source "| Left -> \"left\"")
    in
    let button_pattern_type =
      inferred_type_at
        snapshot
        source_id
        (expect_substring_offset source "| Middle -> \"middle\"")
    in
    ...
    | Ok () -> (
        ...
        | Ok () ->
            match expect_optional_string
              ~label:"make_key"
              ~expected:(Some "key -> event")
              ~actual:make_key_scheme with
            | Error _ as err -> err
            | Ok () ->
...
let main ~args =
  let tests = [
    Test.case
      "constructor resolution uses expected types across patterns and expressions"
      test_constructor_resolution_uses_expected_type;
  ]
  in
