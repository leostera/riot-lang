...
let expect_cst = fun ~filename parse_result ->
  ...
  | Ok cst -> cst
  | Error (Syn.Parse_diagnostics diagnostics) -> panic ("expected successful CST for " ^ filename ^ " but parser reported diagnostics: " ^ String.concat "; " (List.map Syn.Diagnostic.to_string diagnostics))
  | Error (Syn.Cst_builder_error error) -> panic ("expected successful CST for " ^ filename ^ " but CST build failed: " ^ error.message)
...
let expect_optional_string = fun ~label ~expected ~actual ->
  if actual = expected then
    Ok ()
  else Error (label ^ ": expected " ^ Option.unwrap_or ~default:"<none>" expected ^ " but got " ^ Option.unwrap_or ~default:"<none>" actual)
...
let test_constructor_resolution_uses_expected_type = fun _ctx ->
  ...
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "constructor_resolution.ml") ~text:source in
  ...
  if not (List.is_empty diagnostics) then
    ...
    let key_pattern_type = inferred_type_at snapshot source_id (expect_substring_offset source "| Left -> \"left\"") in
    let button_pattern_type = inferred_type_at snapshot source_id (expect_substring_offset source "| Middle -> \"middle\"") in
    ...
    | Error _ as err -> err
    | Ok () -> (
      match expect_optional_string ~label:"button pattern" ~expected:(Some "mouse_button") ~actual:button_pattern_type with
      | Error _ as err -> err
      | Ok () ->
          match expect_optional_string ~label:"make_key" ~expected:(Some "key -> event") ~actual:make_key_scheme with
          | Error _ as err -> err
          | Ok () -> expect_optional_string ~label:"make_button" ~expected:(Some "mouse_button -> event") ~actual:make_button_scheme
    )

let main ~args =
  let tests = [ Test.case "constructor resolution uses expected types across patterns and expressions" test_constructor_resolution_uses_expected_type ] in Test.Cli.main ~name:"typ:constructor_resolution" ~tests ~args ()
