...
let lookup_binding_name = fun lookup env path -> lookup env (EntityId.of_string path) |> Option.map Env.Binding.name
...
let make_env = fun () ->
  Env.bind
    ...
    (Env.of_type_decls [ nested_shade_type_decl ]) |> fun env -> Env.with_local_open env (SurfacePath.of_name "Colors")

let alpha_scheme = TypeScheme.of_type (TypeRepr.arrow ~label:TypeRepr.Nolabel ~lhs:TypeRepr.int ~rhs:TypeRepr.int)

let beta_scheme = TypeScheme.of_type (TypeRepr.arrow ~label:TypeRepr.Nolabel ~lhs:TypeRepr.int ~rhs:TypeRepr.bool)

let join_names = fun names -> String.concat ", " names

let show_optional_name = fun value -> Option.unwrap_or ~default:"<none>" value

let expected_names_error = fun label expected actual -> Error (format Format.[
  str label;
  str " [";
...
let expected_optional_error = fun label expected actual -> Error (format Format.[
  str label;
  str " ";
...
let infer_exports = fun source_text ->
  ...
  let cst =
    ...
    | Ok cst -> cst
    | Error (Syn.Parse_diagnostics diagnostics) -> panic (format Format.[ str "expected successful CST for env2_fixture.ml but parser reported diagnostics: "; str (String.concat "; " (List.map Syn.Diagnostic.to_string diagnostics)) ])
    | Error (Syn.Cst_builder_error error) -> panic (format Format.[ str "expected successful CST for env2_fixture.ml but CST build failed: "; str error.message ])
  in
  let implicit_opens = [] in
  let source = Source.make_prepared ~source_id ~kind:Source.File ~module_name:(Source.infer_module_name origin) ~implicit_opens ~origin ~revision:0 ~source_hash:(Source.hash ~implicit_opens ~cst) ~parse_result ~cst in
  let semantic_tree = Lower.lower_source_file ~source cst in
  let inferred = Infer.infer_file ~imported_world:(ImportedWorld.empty ()) ~config:Config.default ~source semantic_tree in
  inferred.exports |> List.map
    (
...
let test_summary2_roundtrip = fun _ctx ->
  ...
  if not (expected_bindings = actual_bindings) then
    ...
    if not (expected_types = actual_types) then
...
let test_env_replay_matches_lookup = fun _ctx ->
  ...
  let replayed_shade =
    Env.lookup_type replayed (SurfacePath.of_string "Colors.shade") |> Option.map
      (
        fun (type_decl: FileSummary.type_decl) -> (SurfacePath.to_string type_decl.scope_path, type_decl.declaration.type_name)
      )
  in
...
let test_builtin_type_constructors_only_expose_syntax_backed_names = fun _ctx ->
  ...
  match List.find_opt
    ...
    forbidden with
  | None -> Ok ()
  | Some name -> Error (format Format.[ str "expected bare "; str name; str " to require an explicit dependency" ])

let test_bind_in_scope_keeps_local_module_names = fun _ctx ->
  let env =
    Env.empty |> fun env ->
      Env.bind_in_scope env ~scope_path:(SurfacePath.of_name "Helpers") (Env.singleton ~make_id ~name:"id" ~scheme:alpha_scheme ~provenance:Env.Binding.Ambient) |> fun env -> Env.bind_in_scope env ~scope_path:(SurfacePath.of_name "Helpers") (Env.singleton ~make_id ~name:"wrap" ~scheme:beta_scheme ~provenance:Env.Binding.Ambient)
  in
  let actual = Env.names env in
  let expected = [ "Helpers.id"; "Helpers.wrap" ] in
  if actual = expected then
    Ok ()
  else expected_names_error "expected scoped bindings" expected actual
...
let test_include_entries_strip_module_prefix_once = fun _ctx ->
  ...
  if actual = expected then
    Ok ()
  else expected_names_error "expected include entries" expected actual
...
let test_module_alias_entries_prefix_once = fun _ctx ->
  let env =
    Env.empty |> fun env ->
      Env.bind_in_scope env ~scope_path:(SurfacePath.of_name "Helpers") (Env.singleton ~make_id ~name:"id" ~scheme:alpha_scheme ~provenance:Env.Binding.Ambient) |> fun env -> Env.bind_in_scope env ~scope_path:(SurfacePath.of_name "Helpers") (Env.singleton ~make_id ~name:"wrap" ~scheme:beta_scheme ~provenance:Env.Binding.Ambient)
  in
  let actual = Env.entries_for_module_alias env ~alias_name:"Util" ~module_path:(SurfacePath.of_name "Helpers") |> Env.names in
  let expected = [ "Util.id"; "Util.wrap" ] in
  if actual = expected then
    Ok ()
  else expected_names_error "expected alias entries" expected actual

let test_item_scope_replay_keeps_module_paths_stable = fun _ctx ->
  let scope_path = SurfacePath.of_name "Helpers" in
  let introduced_id = Env.singleton ~make_id ~name:"id" ~scheme:alpha_scheme ~provenance:Env.Binding.Ambient in
  ...
  let env_after_item = Env.extend item_env [ Env.Binding.make ~id:(make_id (SurfacePath.of_name "wrap")) ~surface_path:(SurfacePath.of_name "wrap") ~scheme:beta_scheme ~provenance:Env.Binding.Ambient ] in
  ...
  if actual = expected then
    Ok ()
  else expected_names_error "expected replayed scoped env" expected actual
...
let test_export_render_keeps_nested_module_paths_stable = fun _ctx ->
  ...
  if actual = expected then
    Ok ()
  else expected_names_error "expected exported bindings" expected actual
...
let test_direct_infer_keeps_include_module_paths_stable = fun _ctx ->
  ...
  if actual = expected then
    Ok ()
  else expected_names_error "expected direct infer exports" expected actual
...
let test_direct_infer_rebinding_replaces_visible_export = fun _ctx ->
  ...
  if actual = expected then
    Ok ()
  else expected_optional_error "expected shadowed export scheme" expected actual

let test_direct_infer_rebinding_replaces_visible_nested_export = fun _ctx ->
  let actual = infer_export_scheme "module Helpers = struct\n  let value = 1\n  let value = true\nend\n" "Helpers.value" in
  let expected = Some "bool" in
  if actual = expected then
    Ok ()
  else expected_optional_error "expected shadowed nested export scheme" expected actual

let test_direct_infer_poly_variant_expression_uses_named_alias = fun _ctx ->
  let actual = infer_export_scheme "type rgb = [ `rgb of int * int * int ]\nlet blue = `rgb (0, 0, 255)\n" "blue" in
  let expected = Some "rgb" in
  if actual = expected then
    Ok ()
  else expected_optional_error "expected polyvariant expression scheme" expected actual
...
let test_direct_infer_anonymous_poly_variant_expression_keeps_structural_type = fun _ctx ->
  ...
  if actual = expected then
    Ok ()
  else expected_optional_error "expected anonymous polyvariant expression scheme" expected actual

let test_direct_infer_poly_variant_parameter_uses_named_alias = fun _ctx ->
  let actual = infer_export_scheme "type ansi = [ `ansi of int ]\nlet ansi_value = fun (`ansi i) -> i\n" "ansi_value" in
  let expected = Some "ansi -> int" in
  if actual = expected then
    Ok ()
  else expected_optional_error "expected polyvariant parameter scheme" expected actual

let test_direct_infer_poly_variant_match_uses_common_named_alias = fun _ctx ->
  let actual = infer_export_scheme {ocaml|
...
let first_channel = fun value ->
  ...
  | `ansi i -> i
  | `rgb (r, _, _) -> r
|ocaml} "first_channel" in
  let expected = Some "color -> int" in
  if actual = expected then
    Ok ()
  else expected_optional_error "expected polyvariant match scheme" expected actual
...
|ocaml} "classify" in
  ...
  if actual = expected then
    Ok ()
  else expected_optional_error "expected polyvariant widest-alias scheme" expected actual
...
|ocaml} "as_color" in
  ...
  if actual = expected then
    Ok ()
  else expected_optional_error "expected explicit polyvariant coercion scheme" expected actual
