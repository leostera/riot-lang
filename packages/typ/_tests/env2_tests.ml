...
let binding_path = fun binding ->
  Env.Binding.surface_path binding
  |> SurfacePath.to_string

let binding_paths = fun bindings ->
  bindings
  |> List.map binding_path
...
let lookup_binding_path = fun lookup env path ->
  lookup env (EntityId.of_string path)
  |> Option.map binding_path

let lookup_binding_name = fun lookup env path ->
  lookup env (EntityId.of_string path)
  |> Option.map Env.Binding.name
...
let make_env = fun () ->
  ...
  |> fun env -> Env.with_local_open env (SurfacePath.of_name "Colors")

let alpha_scheme =
  TypeScheme.of_type (TypeRepr.arrow ~label:TypeRepr.Nolabel ~lhs:TypeRepr.int ~rhs:TypeRepr.int)

let beta_scheme =
  TypeScheme.of_type (TypeRepr.arrow ~label:TypeRepr.Nolabel ~lhs:TypeRepr.int ~rhs:TypeRepr.bool)
...
let test_summary2_roundtrip = fun _ctx ->
  ...
  if not (expected_bindings = actual_bindings) then
    expected_names_error
      "summary2 roundtrip changed bindings: expected"
      expected_bindings
      actual_bindings
  else if not (expected_types = actual_types) then
    expected_names_error
      "summary2 roundtrip changed type decls: expected"
      expected_types
      actual_types
  else
    Ok ()
...
let test_env_replay_matches_lookup = fun _ctx ->
  ...
  let replayed_shade =
    Env.lookup_type replayed (SurfacePath.of_string "Colors.shade")
    |> Option.map
      (fun (type_decl: FileSummary.type_decl) -> (
        SurfacePath.to_string type_decl.scope_path,
        type_decl.declaration.type_name
      ))
  in
  if not (expected_to_string = actual_to_string) then
    expected_optional_error
      "env2 lookup mismatch for to_string: expected"
      expected_to_string
      actual_to_string
  else if not (expected_blend = actual_blend) then
    expected_optional_error
      "env2 lookup mismatch for RGB.blend name: expected"
      expected_blend
      actual_blend
    ...
    |> fun env ->
      ...
      |> fun env ->
        Env.bind_in_scope
          env
          ~scope_path:(SurfacePath.of_name "Helpers")
          (Env.singleton ~make_id ~name:"wrap" ~scheme:beta_scheme ~provenance:Env.Binding.Ambient)
  in
...
let test_include_entries_strip_module_prefix_once = fun _ctx ->
  ...
  in
  let actual =
    Env.entries_for_include env (SurfacePath.of_name "Helpers")
    |> Env.names
  in
...
let test_module_alias_entries_prefix_once = fun _ctx ->
  ...
  in
  let actual =
    Env.entries_for_module_alias env ~alias_name:"Util" ~module_path:(SurfacePath.of_name "Helpers")
    |> Env.names
  in
...
let main ~args =
  let tests = [
    ...
    Test.case
      "include entries strip module prefix once"
      test_include_entries_strip_module_prefix_once;
    Test.case "module alias entries prefix once" test_module_alias_entries_prefix_once;
    Test.case
      "item scope replay keeps module paths stable"
      test_item_scope_replay_keeps_module_paths_stable;
    Test.case
      "export render keeps nested module paths stable"
      test_export_render_keeps_nested_module_paths_stable;
    Test.case
      "direct infer keeps include module paths stable"
      test_direct_infer_keeps_include_module_paths_stable;
    Test.case
      "direct infer rebinding replaces visible export"
      test_direct_infer_rebinding_replaces_visible_export;
    Test.case
      "direct infer rebinding replaces visible nested export"
      test_direct_infer_rebinding_replaces_visible_nested_export;
    Test.case
      "direct infer polyvariant expression uses named alias"
      test_direct_infer_poly_variant_expression_uses_named_alias;
    Test.case
    ...
    Test.case
      "direct infer polyvariant parameter uses named alias"
      test_direct_infer_poly_variant_parameter_uses_named_alias;
    Test.case
      "direct infer polyvariant match uses common named alias"
      test_direct_infer_poly_variant_match_uses_common_named_alias;
    Test.case
      "direct infer polyvariant match prefers widest visible alias"
      test_direct_infer_poly_variant_match_prefers_widest_visible_alias;
    Test.case
