...
let toolchain_ocamlc_candidates = fun () ->
  let home_dir =
    Env.home_dir ()
    |> Option.expect ~msg:"HOME should exist to locate riot toolchains"
  in
  ...
  if not (path_exists toolchains_root) then
    ...
    let output =
      Command.make "find" ~args:[ Path.to_string toolchains_root; "-name"; "ocamlc"; "-print"; ]
      |> Command.output
...
let preferred_toolchain_ocamlc = fun ocamlc_paths ->
  ...
  in
  match ocamlc_paths
  |> List.filter (fun path -> Int.equal (score_path path) best_score) with
...
let oracle_ocamlc_path = fun () ->
  ...
  | Some path ->
      Path.of_string path
      |> Result.expect ~msg:"TYP_OCAMLC_ORACLE must be a valid UTF-8 path"
  | None ->
...
let oracle_stdlib_path =
  ...
  fun () ->
    ...
    | None ->
        ...
        let output =
          Command.make (Path.to_string ocamlc_path) ~args:[ "-where" ]
          |> Command.output
          |> Result.expect ~msg:"expected ocamlc -where oracle invocation to spawn"
        in
        if output.status != 0 then
          panic
            (format
...
let run_oracle_command = fun ~fixture_filename:_ ~args ->
  ...
  Fs.with_tempdir
    ...
    (fun tmpdir ->
      ...
      Fs.write source_text source_path
      |> Result.expect ~msg:"oracle fixture temp source should be writable";
      let output =
        Command.make (Path.to_string ocamlc_path) ~args:(args @ [ Path.to_string source_path ])
        |> Command.output
...
let prefix_export = fun prefix ({ name; scheme }: oracle_value_export) -> {
  name = qualify_name (Some prefix) name;
  scheme;
}
...
let prefix_type_alias = fun prefix (type_name, manifest) -> (
  prefix_type_name prefix type_name,
  manifest
)
...
let rec type_declaration_parts = fun ~prefix ~source_text declaration ->
  ...
  | Some next ->
      merge_oracle_signature_parts current (type_declaration_parts ~prefix ~source_text next)
  | None -> current
...
let rec module_type_declaration_parts = fun ~fixture_filename ~source_text declaration ->
  ...
  | Some module_type -> (
      ...
      | Some items ->
          let template_parts =
            signature_items_parts ~fixture_filename ~source_text ~prefix:None items
...
and module_declaration_parts = fun ~fixture_filename ~source_text ~prefix declaration ->
  ...
  | Cst.ModuleSignature.Signature module_type -> (
      match signature_items_of_module_type_oracle module_type with
      | Some items ->
          signature_items_parts ~fixture_filename ~source_text ~prefix:(Some module_name) items
      | None -> empty_oracle_signature_parts
    )
...
let run_interface_oracle = fun ~fixture_filename ~source_text ->
  let result =
    run_oracle_command ~fixture_filename ~source_text ~args:[ "-nopervasives"; "-nostdlib"; "-i" ]
  in
  ...
  let interface_parse_result =
    Syn.parse ~filename:(oracle_interface_filename fixture_filename) interface_text
  in
  let parts =
    match Syn.build_cst interface_parse_result with
    | Ok (Cst.Interface interface) ->
        signature_items_parts
          ~fixture_filename
          ~source_text:interface_text
          ~prefix:None
          interface.items
        |> expand_module_aliases
    | Ok (Cst.Implementation _) ->
        panic
          (format
            Format.[
  ...
  let value_exports =
    ...
    |> List.sort_uniq compare_value_export
  in
  ...
  let type_aliases =
    parts.type_aliases
    |> List.sort compare_type_alias
    |> List.sort_uniq compare_type_alias
  in
  let module_type_templates =
    parts.module_type_templates
    |> List.sort_uniq (fun left right -> String.compare left.name right.name)
  in
  {
    text = interface_text;
    value_exports;
...
let run_typedtree_oracle = fun ~fixture_filename ~source_text ->
  ...
  let result =
    run_oracle_command
      ~fixture_filename
      ~source_text
      ~args:[ "-nopervasives"; "-nostdlib"; "-I"; Path.to_string stdlib_path; "-dtypedtree"; "-c"; ]
  in
  let output = result.output in
  if output.status != 0 then
    panic
...
let oracle_module_type_template = fun (interface: oracle_interface) module_type_name ->
  oracle_module_type_templates interface
  |> List.find_opt
...
let package_constraint_entries = fun text ->
  ...
  if String.equal trimmed "" then
    ...
    let without_with =
      ...
      | None -> trimmed
    in
    split_on_needle ~needle:" and type " without_with
    |> List.filter_map
      (fun entry ->
        let entry =
...
let rec normalize_core_type_with_constraints = fun
  ...
  | Cst.CoreType.Arrow { label = None; parameter_type; result_type; _ } ->
      let (parameter_text, introduced_constraints) =
        ...
        | Cst.CoreType.FirstClassModule { module_name; package_type; _ } ->
            normalize_package_type_with_constraints
              interface
              constraints
      ...
      in
      let result_text =
        normalize_core_type_with_constraints
          interface
  ...
  | Cst.CoreType.FirstClassModule { module_name; package_type; _ } ->
      let (text, _introduced_constraints) =
        normalize_package_type_with_constraints
          interface
          constraints
          (Option.map Cst.Token.text module_name)
    ...
    (fun (constraint_: Cst.module_type_constraint) ->
      let type_name =
        ...
        | Cst.CoreType.Constr { constructor_path; arguments = []; _ } ->
            ...
            |> Option.unwrap_or
              ~default:(
                core_type_text interface.text constraint_.constrained_type
                |> String.trim
              )
        | _ ->
            core_type_text interface.text constraint_.constrained_type
      ...
      in
      let replacement =
        normalize_core_type_with_constraints interface constraints constraint_.replacement_type
      in
...
and normalize_package_type_with_constraints = fun
  ...
  let module_type_name =
    Syn.Cst.Ident.name package_type.module_type_path
    |> Option.unwrap_or ~default:(ident_text package_type.module_type_path)
  in
...
let package_rewrite = fun (interface: oracle_interface) segment ->
  ...
  let (wrapped, core) =
    if String.length trimmed >= 2 && trimmed.[0] = '(' && trimmed.[String.length trimmed - 1] = ')' then
      (true, String.sub trimmed 1 (String.length trimmed - 2)
      |> String.trim)
    else
      (false, trimmed)
  in
  ...
  | Some rest when not (String.starts_with ~prefix:"sig " (String.trim rest)) ->
      ...
      oracle_module_type_template interface module_type_name
      |> Option.map
  ...
  let rec loop propagated_constraints acc = function
    ...
    | segment :: rest ->
        ...
        (
          ...
          | Some (rewritten_segment, introduced_constraints) ->
              loop (propagated_constraints @ introduced_constraints) (rewritten_segment :: acc) rest
  ...
  if length < 4 || scheme.[0] != '(' then
    ...
    | Some close_index when close_index + 3 <= length ->
        ...
        if not (String.starts_with ~prefix:" ->" suffix) then
          ...
          let prefix =
            String.sub scheme 1 (close_index - 1)
            |> String.trim
          in
          if contains_substring ~needle:"*" prefix then
            Some (prefix, String.sub suffix 4 (String.length suffix - 4)
            |> String.trim)
          else
            None
    | _ -> None
...
let next_significant_slice = fun text start_index ->
  ...
  | Some index when index + 1 < String.length text && text.[index] = '-' && text.[index + 1] = '>' ->
      Some "->"
...
let normalize_typ_scheme = fun scheme ->
  ...
  | Some (left, right) when String.starts_with ~prefix:"'" (String.trim left) ->
      normalize_scheme_notation right
    ...
    (fun (name, scheme) ->
      {
        ...
        scheme =
          TypePrinter.scheme_to_string scheme
          |> normalize_typ_scheme;
      })
  |> List.sort compare_value_export
  ...
  |> List.map
    (fun ({ declaration; scope_path }: FileSummary.type_decl) ->
      if SurfacePath.is_empty scope_path then
        ...
        SurfacePath.append_name scope_path declaration.type_name
...
let normalized_value_exports = fun exports ->
  ...
  |> List.map
    (fun ({ name; scheme }: oracle_value_export) -> {
      name;
      scheme = normalize_scheme_notation scheme;
    })
  |> List.sort compare_value_export
...
let normalize_export_scheme_with_interface = fun
  ...
  (
    ...
    | None ->
        scheme
        |> normalize_named_package_types interface
  )
...
let normalized_oracle_exports_for_comparison = fun (interface: oracle_interface) exports ->
  ...
  |> List.map
    (fun ({ name; scheme = _ } as export) -> {
      name;
      scheme = normalize_export_scheme_with_interface interface export;
    })
  |> List.sort compare_value_export
  ...
  Data.Json.Object [
    ...
    ("typ_summary", Data.Json.Object [
      (
        "completeness",
        Data.Json.String (
          FileSummary.completeness report.file_summary
          |> completeness_to_string
        )
      );
...
let test_fixture = fun ~(ctx:Test.FixtureRunner.ctx) ->
  ...
  let source_text =
    with_oracle_stage
      ...
      (fun () ->
        ...
        |> Result.expect ~msg:"oracle fixture should exist")
  in
  let report =
    with_oracle_stage
      ~fixture_filename
  ...
  let actual_json =
    with_oracle_stage
      ...
      ~stage:"report_json"
      (fun () ->
        report_json report interface typedtree_text)
  in
  with_oracle_stage
    ~fixture_filename
