  ...
  let rec loop index =
    if needle_length = 0 then
      ...
      if index + needle_length > haystack_length then
        ...
        if String.sub haystack index needle_length = needle then
          true
        else loop (index + 1)
  in
  loop 0
...
let skipped_fixture_basename = fun basename -> contains_substring ~needle:"array_" basename || contains_substring ~needle:"recursive_modules" basename

let fixture_filter = fun path ->
  let basename = oracle_fixture_basename path in
  match Path.extension path with
  | Some ".ml" when oracle_range_filter basename && oracle_name_filter basename && not (skipped_fixture_basename basename) -> `keep
  | _ -> `skip
...
let with_check_source_stage = fun ~filename ~stage thunk ->
  try thunk () with
  | Stack_overflow -> panic (format Format.[
...
let check_source_text = fun ~filename text ->
  ...
  | Ok cst ->
      ...
      let session =
        with_check_source_stage ~filename ~stage:"session_empty"
          (
            fun () -> Session.empty ~config
          )
      in
      let origin = Source.Path filename in
      let module_name =
        with_check_source_stage ~filename ~stage:"infer_module_name"
          (
      ...
      let source_hash =
        with_check_source_stage ~filename ~stage:"source_hash"
          (
            fun () -> Source.hash ~implicit_opens ~cst
          )
      in
      let (session, source_id) =
        with_check_source_stage ~filename ~stage:"create_source"
          (
            fun () -> Session.create_source session ~kind:Source.File ~module_name ~implicit_opens ~origin ~source_hash ~parse_result ~cst
          )
      in
      let source =
        with_check_source_stage ~filename ~stage:"make_prepared_source"
          (
            fun () -> Source.make_prepared ~source_id ~kind:Source.File ~module_name ~implicit_opens ~origin ~revision:0 ~source_hash ~parse_result ~cst
          )
      in
      let fallback_analysis =
        with_check_source_stage ~filename ~stage:"fallback_source_analysis"
          (
            fun () -> SourceAnalysis.analyze ~config source
          )
      in
      let _ =
        with_check_source_stage ~filename ~stage:"direct_module_pairing_from_fallback_analysis"
          (
            fun () -> ModulePairing.of_sources ~internal_name:(LocalModules.InternalName.of_string module_name) [ { ModulePairing.source; analysis = fallback_analysis; visible_type_decls = Typ.Config.ambient_type_decls config } ]
          )
      in
      let prepared_snapshot =
        with_check_source_stage ~filename ~stage:"prepare_snapshot"
          (
        ...
        with_check_source_stage ~filename ~stage:"extract_semantic_tree"
          (
            fun () ->
              ...
              | Some semantic_tree -> (Some semantic_tree.item_tree, Some semantic_tree.body_arena, Some semantic_tree.origin_map)
              | None -> (None, None, None)
          )
      in
          ...
          fun () ->
            {
              ...
              exports = SourceAnalysis.exports analysis |> List.map
                (
                  fun (name, scheme) -> (SurfacePath.to_string name, scheme)
                );
...
let replace_all = fun ~needle ~replacement haystack ->
  ...
  let rec loop start acc =
    ...
    | Some index ->
        let prefix = String.sub haystack start (index - start) in loop (index + needle_length) (replacement :: prefix :: acc)
    | None ->
        let suffix = String.sub haystack start (haystack_length - start) in List.rev (suffix :: acc) |> String.concat ""
  in
  if needle_length = 0 then
    haystack
  else loop 0 []
...
let toolchain_ocamlc_candidates = fun () ->
  ...
  if not (path_exists toolchains_root) then
    ...
    let output = Command.make "find" ~args:[
      Path.to_string toolchains_root;
      "-name";
      "ocamlc";
      "-print";
    ] |> Command.output |> Result.expect ~msg:"expected toolchain oracle find command to spawn" in
    if output.status != 0 then
      []
    else
      output.stdout |> split_nonempty_lines |> List.filter_map
        (
          fun path -> Path.of_string path |> Result.to_option
        )

let preferred_toolchain_ocamlc = fun ocamlc_paths ->
  let host_tokens =
    oracle_host_tokens () |> List.filter
      (
        fun token -> not (String.equal token "") && not (String.equal token "unknown")
      )
  in
  let score_path path =
    let rendered = Path.to_string path in
    host_tokens |> List.filter
      (
        fun token -> contains_substring ~needle:token rendered
      ) |> List.length
  in
  let best_score = ocamlc_paths |> List.map score_path |> List.fold_left max 0 in
  match ocamlc_paths |> List.filter
    (
      fun path -> Int.equal (score_path path) best_score
    ) with
...
let oracle_ocamlc_path = fun () ->
  ...
  | None ->
      ...
      | None -> panic (format Format.[ str "expected exactly one riot-managed ocamlc oracle, found "; int (List.length ocamlc_paths) ])
...
let oracle_stdlib_path =
  ...
  fun () ->
    ...
    | None ->
        ...
        let output = Command.make (Path.to_string ocamlc_path) ~args:[ "-where" ] |> Command.output |> Result.expect ~msg:"expected ocamlc -where oracle invocation to spawn" in
        if output.status != 0 then
          panic (format Format.[
        ...
        let () = cached := Some path in path

type oracle_command_result = { output: Command.output; source_path: Path.t }
  ...
  Fs.with_tempdir ~prefix:"typ_oracle"
    (
      fun tmpdir ->
        ...
        let output = Command.make (Path.to_string ocamlc_path) ~args:(args @ [ Path.to_string source_path ]) |> Command.output |> Result.expect ~msg:"expected ocamlc oracle invocation to spawn" in { output; source_path }
    ) |> Result.expect ~msg:"oracle tempdir should be creatable"

let strip_identifier_stamps = fun text -> text

type oracle_value_export = { name: string; scheme: string }

type oracle_module_alias = { alias_name: string; target_name: string }
...
and oracle_module_type_template = { name: string; parts: oracle_signature_parts }
...
let split_once = fun line ch ->
  ...
  | Some index ->
      ...
      let right = String.sub line (index + 1) (String.length line - index - 1) in Some (left, right)
  | None -> None

let looks_like_type_abbreviation = fun manifest -> not (contains_substring ~needle:"|" manifest) && not (contains_substring ~needle:"{" manifest) && not (contains_substring ~needle:"}" manifest) && not (contains_substring ~needle:" of " manifest) && not (contains_substring ~needle:"private " manifest)
...
let compare_type_alias = fun (left_name, _left_manifest) (right_name, _right_manifest) -> String.compare left_name right_name
...
let prefix_export = fun prefix ({ name; scheme }: oracle_value_export) -> { name = qualify_name (Some prefix) name; scheme }

let prefix_type_name = fun prefix type_name -> qualify_name (Some prefix) type_name

let prefix_type_alias = fun prefix (type_name, manifest) -> (prefix_type_name prefix type_name, manifest)

let token_list_text = fun tokens ->
  tokens |> List.map Cst.Token.text |> String.concat " " |> String.split_on_char ' ' |> List.filter
    (
...
let syntax_text = fun source_text syntax_node ->
  let span = Ceibo.Red.SyntaxNode.span syntax_node in String.sub source_text span.start (span.end_ - span.start) |> String.trim

let core_type_text = fun source_text type_ -> syntax_text source_text (Cst.CoreType.syntax_node type_)
...
let rec type_declaration_parts = fun ~prefix ~source_text declaration ->
  ...
  | Some next -> merge_oracle_signature_parts current (type_declaration_parts ~prefix ~source_text next)
  | None -> current
...
let signature_items_of_module_type_oracle = fun module_type -> Syn.CstBuilder.signature_items_of_module_type module_type |> Result.to_option
  ...
  parts.module_aliases |> List.fold_left
    (
      fun (current: oracle_signature_parts) ({ alias_name; target_name }: oracle_module_alias) ->
        let aliased_exports =
          current.value_exports |> List.filter_map
            (
              fun (export: oracle_value_export) ->
                if String.starts_with ~prefix:(target_name ^ ".") export.name then
                  Some ({ export with name = alias_name ^ "." ^ String.sub export.name (String.length target_name + 1) (String.length export.name - String.length target_name - 1) })
                else None
            )
        in
        let aliased_export_types =
          current.value_export_types |> List.filter_map
            (
              fun (export_name, core_type) ->
                if String.starts_with ~prefix:(target_name ^ ".") export_name then
                  Some (alias_name ^ "." ^ String.sub export_name (String.length target_name + 1) (String.length export_name - String.length target_name - 1), core_type)
                else None
            )
        in
        let aliased_type_names =
          current.type_names |> List.filter_map
            (
              fun type_name ->
                if String.starts_with ~prefix:(target_name ^ ".") type_name then
                  Some (alias_name ^ "." ^ String.sub type_name (String.length target_name + 1) (String.length type_name - String.length target_name - 1))
                else None
            )
        in
        let aliased_type_aliases =
          current.type_aliases |> List.filter_map
            (
              fun (type_name, manifest) ->
                if String.starts_with ~prefix:(target_name ^ ".") type_name then
                  Some (alias_name ^ "." ^ String.sub type_name (String.length target_name + 1) (String.length type_name - String.length target_name - 1), manifest)
                else None
            )
        in
        {
          current with
          value_exports = current.value_exports @ aliased_exports;
...
let rec module_type_declaration_parts = fun ~fixture_filename ~source_text declaration ->
  ...
  | Some module_type -> (
    ...
    | Some items ->
        let template_parts = signature_items_parts ~fixture_filename ~source_text ~prefix:None items |> expand_module_aliases in { empty_oracle_signature_parts with module_type_templates = [ { name = Cst.ModuleTypeDeclaration.name declaration; parts = template_parts } ] }
...
and signature_item_parts = fun ~fixture_filename ~source_text ~prefix item ->
  ...
  | Cst.SignatureItem.ValueDeclaration declaration ->
      ...
      {
        empty_oracle_signature_parts with
        value_exports = [ { name; scheme = core_type_text source_text declaration.type_ } ];
        value_export_types = [
  ...
  | Cst.SignatureItem.TypeDeclaration declaration -> type_declaration_parts ~prefix ~source_text declaration
  | Cst.SignatureItem.ModuleDeclaration declaration -> module_declaration_parts ~fixture_filename ~source_text ~prefix declaration
  | Cst.SignatureItem.ModuleTypeDeclaration declaration -> module_type_declaration_parts ~fixture_filename ~source_text declaration
...
and module_declaration_parts = fun ~fixture_filename ~source_text ~prefix declaration ->
  ...
  | Cst.ModuleSignature.Signature module_type -> begin
    match signature_items_of_module_type_oracle module_type with
    | Some items -> signature_items_parts ~fixture_filename ~source_text ~prefix:(Some module_name) items
    | None -> empty_oracle_signature_parts
  end
  | Cst.ModuleSignature.Alias module_expression -> (
    match module_expression with
    | Cst.ModuleExpression.Path ident -> { empty_oracle_signature_parts with module_aliases = [ { alias_name = module_name; target_name = local_alias_target_name ~prefix (ident_text ident) } ] }
    | _ -> empty_oracle_signature_parts
  )
and signature_items_parts = fun ~fixture_filename ~source_text ~prefix items -> items |> List.map (signature_item_parts ~fixture_filename ~source_text ~prefix) |> List.fold_left merge_oracle_signature_parts empty_oracle_signature_parts

let run_interface_oracle = fun ~fixture_filename ~source_text ->
  let result = run_oracle_command ~fixture_filename ~source_text ~args:[ "-nopervasives"; "-nostdlib"; "-i" ] in
  ...
  let parts =
    ...
    | Ok (Cst.Interface interface) -> signature_items_parts ~fixture_filename ~source_text:interface_text ~prefix:None interface.items |> expand_module_aliases
    | Ok (Cst.Implementation _) -> panic (format Format.[
  ...
  let module_type_templates =
    parts.module_type_templates |> List.sort_uniq
      (
...
let run_typedtree_oracle = fun ~fixture_filename ~source_text ->
  ...
  let result = run_oracle_command ~fixture_filename ~source_text ~args:[
    "-nopervasives";
    "-nostdlib";
    "-I";
  ...
  output.stderr |> replace_all ~needle:(Path.to_string result.source_path) ~replacement:(Path.to_string fixture_filename) |> strip_identifier_stamps |> split_nonempty_lines |> String.concat "\n"
  ...
  let rec find_from index =
    if needle_length = 0 then
      ...
      if index + needle_length > text_length then
        ...
        if String.sub text index needle_length = needle then
          Some index
        else find_from (index + 1)
  in
  let rec loop start acc =
    match find_from start with
    | Some index ->
        let part = String.sub text start (index - start) in loop (index + needle_length) (part :: acc)
    | None ->
        let suffix = String.sub text start (text_length - start) in List.rev (suffix :: acc)
  in
  loop 0 []
...
let oracle_module_type_template = fun (interface: oracle_interface) module_type_name ->
  oracle_module_type_templates interface |> List.find_opt
    (
      fun ({ name; _ }: oracle_module_type_template) -> String.equal name module_type_name
    )
    ...
    split_on_needle ~needle:" and type " without_with |> List.filter_map
      (
        fun entry ->
          ...
          in
          split_once entry '=' |> Option.map
            (
              fun (name, replacement) -> (String.trim name, String.trim replacement)
            )
      )
      ...
      if index + needle_length > haystack_length then
        ...
        if String.sub haystack index needle_length = needle then
          let before_ok =
            if index = 0 then
              true
            else not (is_type_path_char haystack.[index - 1])
          in
          let after_index = index + needle_length in
          let after_ok =
            if after_index >= haystack_length then
              true
            else not (is_type_path_char haystack.[after_index])
          in
...
let apply_package_constraints = fun constraints scheme ->
  constraints |> List.fold_left
    (
      fun current (type_name, replacement) -> replace_standalone_package_occurrences ~needle:type_name ~replacement current
    )
    scheme
...
let apply_core_type_constraints = fun constraints scheme ->
  constraints |> List.fold_left
    (
      fun current (needle, replacement) -> replace_standalone_package_occurrences ~needle ~replacement current
    )
    scheme

let rec normalize_core_type_with_constraints = fun (interface: oracle_interface) constraints core_type ->
  match core_type with
  | Cst.CoreType.Parenthesized { inner; _ } -> normalize_core_type_with_constraints interface constraints inner
  | Cst.CoreType.Tuple { elements; _ } ->
      elements |> List.map
        (
          fun element ->
            match element with
            | Cst.CoreType.Arrow _ -> "(" ^ normalize_core_type_with_constraints interface constraints element ^ ")"
  ...
  | Cst.CoreType.Arrow { label = None; parameter_type; result_type; _ } ->
      let (parameter_text, introduced_constraints) =
        ...
        | Cst.CoreType.FirstClassModule { module_name; package_type; _ } ->
            normalize_package_type_with_constraints interface constraints (Option.map Cst.Token.text module_name) package_type |> (
              fun (text, introduced_constraints) -> ("(" ^ text ^ ")", introduced_constraints)
            )
        | Cst.CoreType.Arrow _ -> "(" ^ normalize_core_type_with_constraints interface constraints parameter_type ^ ")", []
        | _ -> normalize_core_type_with_constraints interface constraints parameter_type, []
      in
      let result_text = normalize_core_type_with_constraints interface (constraints @ introduced_constraints) result_type in parameter_text ^ " -> " ^ result_text
  | Cst.CoreType.FirstClassModule { module_name; package_type; _ } ->
      let (text, _introduced_constraints) = normalize_package_type_with_constraints interface constraints (Option.map Cst.Token.text module_name) package_type in "(" ^ text ^ ")"
  | _ -> core_type_text interface.text core_type |> apply_core_type_constraints constraints |> String.trim
and normalize_package_type_constraints = fun (interface: oracle_interface) constraints (package_type: Cst.package_type) ->
  package_type.constraints |> List.map
    (
      fun (constraint_: Cst.module_type_constraint) ->
        let type_name =
          match constraint_.constrained_type with
          | Cst.CoreType.Constr { constructor_path; arguments = []; _ } -> Syn.Cst.Ident.name constructor_path |> Option.unwrap_or ~default:(core_type_text interface.text constraint_.constrained_type |> String.trim)
          | _ -> core_type_text interface.text constraint_.constrained_type |> String.trim
        in
        let replacement = normalize_core_type_with_constraints interface constraints constraint_.replacement_type in (type_name, replacement)
    )
and normalize_package_type_with_constraints = fun (interface: oracle_interface) constraints module_name (package_type: Cst.package_type) ->
  ...
  let propagated_constraints =
    ...
    | Some module_name ->
        package_constraints |> List.map
          (
            fun (type_name, replacement) -> (module_name ^ "." ^ type_name, replacement)
          )
    | None -> []
  in
  let module_type_name = Syn.Cst.Ident.name package_type.module_type_path |> Option.unwrap_or ~default:(ident_text package_type.module_type_path) in
  match oracle_module_type_template interface module_type_name with
  | Some ({ parts; _ }: oracle_module_type_template) ->
      let values_text =
        parts.value_exports |> List.map
          (
            fun ({ name; scheme }: oracle_value_export) ->
              let scheme = scheme |> apply_package_constraints package_constraints |> apply_core_type_constraints constraints |> String.trim in "val " ^ name ^ " : " ^ scheme
          ) |> String.concat "; "
      in
      ("module sig " ^ values_text ^ " end", propagated_constraints)
  | None ->
      let fallback = "module " ^ module_type_name in (fallback, propagated_constraints)
...
let package_rewrite = fun (interface: oracle_interface) segment ->
  ...
  | Some rest when not (String.starts_with ~prefix:"sig " (String.trim rest)) ->
      let (binder_name, rest) =
        ...
        | Some (left, right) when not (contains_substring ~needle:"with type" left) && not (contains_substring ~needle:" " left) -> Some (String.trim left), String.trim right
      ...
      in
      let constraints_text = String.sub rest (String.length module_type_name) (String.length rest - String.length module_type_name) |> String.trim in
        ...
        (
          fun ({ parts; _ }: oracle_module_type_template) ->
            let rewritten_exports =
              parts.value_exports |> List.map
                (
                  fun ({ name; scheme }: oracle_value_export) -> { name; scheme = apply_package_constraints constraints scheme }
                )
            in
            let rewritten_segment =
              let values_text =
                rewritten_exports |> List.map
                  (
...
let normalize_named_package_types = fun (interface: oracle_interface) scheme ->
  ...
  let rec loop propagated_constraints acc = function
    ...
    | segment :: rest ->
        let segment =
          propagated_constraints |> List.fold_left
            (
              fun current (needle, replacement) -> replace_standalone_package_occurrences ~needle ~replacement current
            )
            segment
        ...
        (
          ...
          | Some (rewritten_segment, introduced_constraints) -> loop (propagated_constraints @ introduced_constraints) (rewritten_segment :: acc) rest
...
let strip_redundant_parentheses = fun scheme ->
  ...
  let rec loop index acc =
    if index >= length then
      ...
      if not (Char.equal scheme.[index] '(') then
        ...
        | Some close_index ->
            ...
            else loop (close_index + 1) (String.sub scheme index (close_index - index + 1) :: acc)
  in
  loop 0 []
...
let normalize_scheme_notation = fun scheme ->
        ...
        if text.[index] = '~' then
          let rec consume_label end_index =
            if end_index < length then
              ...
              | 'a' .. 'z' | 'A' .. 'Z' | '0' .. '9' | '_' -> consume_label (end_index + 1)
              | ':' -> Some end_index
              | _ -> None
            else None
          in
          (
            ...
            | Some label_end ->
                let label = String.sub text (index + 1) (label_end - index - 1) in loop (label_end + 1) (":" :: label :: acc)
            | None -> loop (index + 1) ("~" :: acc)
          )
        else loop (index + 1) (String.make 1 text.[index] :: acc)
    in
    loop 0 []
  in
  let rec loop current =
    ...
    | None ->
        let normalized =
          trimmed |> normalize_labeled_arrows |> strip_redundant_parentheses |> String.split_on_char ' ' |> List.filter
            (
        ...
        if String.equal normalized trimmed then
          normalized
        else loop normalized
  in
  loop scheme

let normalize_typ_scheme = fun scheme ->
  match split_once scheme '.' with
  | Some (left, right) when String.starts_with ~prefix:"'" (String.trim left) -> normalize_scheme_notation right
...
let is_value_export_name = fun name ->
  ...
  if String.length leaf = 0 then
    ...
    let first = String.get leaf 0 in not (first >= 'A' && first <= 'Z')

let typ_value_exports = fun (report: Check_result.t) ->
  FileSummary.exports report.file_summary |> List.filter
    (
      fun (name, _scheme) -> is_value_export_name (SurfacePath.to_string name)
    ) |> List.map
    (
      fun (name, scheme) -> { name = SurfacePath.to_string name; scheme = TypePrinter.scheme_to_string scheme |> normalize_typ_scheme }
  ...
  FileSummary.type_decls report.file_summary |> List.map
    (
      fun ({ declaration; scope_path }: FileSummary.type_decl) ->
        ...
        else SurfacePath.append_name scope_path declaration.type_name |> SurfacePath.to_string
...
let normalized_value_exports = fun exports ->
  exports |> List.map
    (
      fun ({ name; scheme }: oracle_value_export) -> { name; scheme = normalize_scheme_notation scheme }
    ) |> List.sort compare_value_export
          ...
          (
            fun type_name ->
              if String.starts_with ~prefix:(prefix ^ ".") type_name then
                Some (String.sub type_name (String.length prefix + 1) (String.length type_name - String.length prefix - 1), type_name)
              else None
          )
    | None ->
        type_names |> List.filter_map
    ...
    (
      fun current_scheme (bare_name, qualified_name) ->
        if String.equal bare_name qualified_name then
          current_scheme
        else replace_standalone_occurrences ~needle:bare_name ~replacement:qualified_name current_scheme
    )
    scheme
...
let expand_type_aliases = fun type_aliases scheme ->
  let rec loop remaining current =
    if remaining <= 0 then
      ...
      let next =
        type_aliases |> List.fold_left
          (
            fun current_scheme (type_name, manifest) -> replace_standalone_occurrences ~needle:type_name ~replacement:manifest current_scheme
          )
          current
      in
      if String.equal next current then
        normalize_scheme_notation current
      else loop (remaining - 1) next
  in
  loop 16 scheme

let normalize_export_scheme_with_interface = fun (interface: oracle_interface) ({ name; scheme }: oracle_value_export) ->
  (
  ...
  ) |> normalize_scheme_notation |> qualify_local_type_names ~scope:(export_scope name) interface.type_names |> expand_type_aliases interface.type_aliases

let normalized_oracle_exports_for_comparison = fun (interface: oracle_interface) exports ->
  exports |> List.map
    (
      fun ({ name; scheme = _ } as export) -> { name; scheme = normalize_export_scheme_with_interface interface export }
    ) |> List.sort compare_value_export

let json_string = fun json -> Data.Json.to_string_pretty json

let fail_json = fun ~label json -> panic (format Format.[ str label; str "\n"; str (json_string json) ])
...
let report_json = fun (report: Check_result.t) (interface: oracle_interface) typedtree_text ->
  Data.Json.Object [
    "comparison", Data.Json.Object [
      "exports", Data.Json.Object [
      ...
      "normalized_exports", Data.Json.Object [
        "ocamlc", Data.Json.Array (List.map export_to_json (normalized_oracle_exports_for_comparison interface interface.value_exports));
        "typ", Data.Json.Array (List.map export_to_json (normalized_oracle_exports_for_comparison interface (typ_value_exports report)));
      ];
      "type_names", Data.Json.Object [
        "ocamlc", Data.Json.Array (
          List.map
            (
...
let assert_no_error_diagnostics = fun (report: Check_result.t) ->
  if not (report.parse_diagnostics = []) || has_error_diagnostics report.lowering_diagnostics || has_error_diagnostics report.typing_diagnostics then
    fail_json ~label:"oracle fixture produced error diagnostics" (diagnostics_json ~parse:report.parse_diagnostics ~lowering:report.lowering_diagnostics ~typing:report.typing_diagnostics);
  if not (FileSummary.completeness report.file_summary = FileSummary.Complete) then
    fail_json ~label:"oracle fixture produced a partial file summary"
      (
        Data.Json.Object [
          "completeness", Data.Json.String (FileSummary.completeness report.file_summary |> completeness_to_string);
        ]
...
let test_fixture = fun ~(ctx:Test.FixtureRunner.ctx) ->
  ...
  let source_text =
    with_oracle_stage ~fixture_filename ~stage:"read_fixture"
      (
        fun () -> Fs.read ctx.fixture_path |> Result.expect ~msg:"oracle fixture should exist"
      )
  in
  let report =
    with_oracle_stage ~fixture_filename ~stage:"typ_check_source"
      (
        fun () -> check_source_text ~filename:fixture_filename source_text
      )
  in
  let interface =
    with_oracle_stage ~fixture_filename ~stage:"ocamlc_interface_oracle"
      (
        fun () -> run_interface_oracle ~fixture_filename ~source_text
      )
  in
  let typedtree_text =
    with_oracle_stage ~fixture_filename ~stage:"ocamlc_typedtree_oracle"
      (
        fun () -> run_typedtree_oracle ~fixture_filename ~source_text
      )
  in
  let actual_json =
    with_oracle_stage ~fixture_filename ~stage:"report_json"
      (
        fun () -> report_json report interface typedtree_text
      )
  in
  with_oracle_stage ~fixture_filename ~stage:"assert_no_error_diagnostics"
    (
      fun () -> assert_no_error_diagnostics report
    );
  with_oracle_stage ~fixture_filename ~stage:"assert_export_equivalence"
    (
      fun () -> assert_json_equal ~label:"oracle exports mismatch" ~expected:(Data.Json.Array (List.map export_to_json (normalized_oracle_exports_for_comparison interface interface.value_exports))) ~actual:(Data.Json.Array (List.map export_to_json (normalized_oracle_exports_for_comparison interface (typ_value_exports report))))
    );
  with_oracle_stage ~fixture_filename ~stage:"assert_type_name_equivalence"
    (
      fun () -> assert_json_equal ~label:"oracle type names mismatch" ~expected:(
        Data.Json.Array (
          List.map
            (
              fun name -> Data.Json.String name
  ...
  if skip_snapshot_assertion () then
    ...
    with_oracle_stage ~fixture_filename ~stage:"snapshot_assertion"
      (
...
let main ~args =
  ...
  ) in Test.Cli.main ~name:"typ:oracle_fixtures" ~tests ~args ()
