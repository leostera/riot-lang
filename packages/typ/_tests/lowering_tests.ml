...
let type_item_summary_to_json = fun value ->
  ...
  | ItemTree.Value _ -> Data.Json.Object [ ("tag", Data.Json.String "value"); ]
  ...
  | ItemTree.Exception _ -> Data.Json.Object [ ("tag", Data.Json.String "exception"); ]
  ...
  | ItemTree.Open _ -> Data.Json.Object [ ("tag", Data.Json.String "open"); ]
  | ItemTree.Include _ -> Data.Json.Object [ ("tag", Data.Json.String "include"); ]
  | ItemTree.ModuleAlias _ -> Data.Json.Object [ ("tag", Data.Json.String "module_alias"); ]

let actual_lowering_json = fun (report: Check_result.t) ->
  let body_arena =
    report.body_arena
    |> Option.expect ~msg:"expected lowered body arena in lowering test"
  in
  let choose_binding =
    BodyArena.bindings body_arena
...
let actual_type_item_lowering_json = fun (report: Check_result.t) ->
  let item_tree =
    report.item_tree
    |> Option.expect ~msg:"expected lowered item tree in lowering test"
  in
  Data.Json.Object [
    ("items", Data.Json.Array (
      ItemTree.items item_tree
      |> List.map type_item_summary_to_json
    ));
    (
...
let test_fun_cases_preserve_preceding_parameters = fun ctx ->
  ...
  in
  let report =
    check_source_text ~filename:(Path.v "packages/typ/tests/fun_cases_with_params.ml") source
  in
  Test.Snapshot.assert_inline_json
    ~ctx
    ~actual:(actual_lowering_json report)
    ~expected:expected_lowering_json
...
let test_manifest_type_aliases_lower_to_type_items = fun ctx ->
  ...
  let report =
    check_source_text ~filename:(Path.v "packages/typ/tests/type_alias_recovery.ml") source
  in
  Test.Snapshot.assert_inline_json
    ~ctx
    ~actual:(actual_type_item_lowering_json report)
    ~expected:expected_manifest_alias_lowering_json
...
let test_poly_variant_type_declarations_lower_to_type_items = fun ctx ->
  ...
  in
  let report =
    check_source_text ~filename:(Path.v "packages/typ/tests/poly_variant_types.ml") source
  in
  Test.Snapshot.assert_inline_json
    ~ctx
    ~actual:(actual_type_item_lowering_json report)
    ~expected:expected_poly_variant_type_lowering_json
...
let test_include_module_type_of_lowers_to_include_item = fun ctx ->
  ...
  in
  let report =
    check_source_text ~filename:(Path.v "packages/typ/tests/include_module_type_of.mli") source
  in
  Test.Snapshot.assert_inline_json
    ~ctx
    ~actual:(actual_type_item_lowering_json report)
    ~expected:expected_include_module_type_of_lowering_json

let test_include_dotted_module_path_lowers_to_include_item = fun ctx ->
  let source = String.concat "\n" [ "include Kernel.Int64"; "" ] in
  let report =
    check_source_text ~filename:(Path.v "packages/typ/tests/include_dotted_module_path.ml") source
  in
  Test.Snapshot.assert_inline_json
    ~ctx
    ~actual:(actual_type_item_lowering_json report)
...
let main ~args =
  let tests = [
    Test.case
      "fun cases preserve preceding parameters during lowering"
      test_fun_cases_preserve_preceding_parameters;
    Test.case
      "abstract type declarations lower to type items"
      test_abstract_type_declarations_lower_to_type_items;
    Test.case
      "manifest type aliases lower to type items"
      test_manifest_type_aliases_lower_to_type_items;
    Test.case
      "arrow type aliases preserve labels during lowering"
      test_arrow_type_aliases_preserve_labels_during_lowering;
    Test.case
      "polymorphic-variant type declarations lower to type items"
      test_poly_variant_type_declarations_lower_to_type_items;
    Test.case
      "include module type of lowers to include items"
      test_include_module_type_of_lowers_to_include_item;
    Test.case
      "include dotted module path lowers to include item"
      test_include_dotted_module_path_lowers_to_include_item;
    Test.case
      "include module type with constraint lowers to include item"
      test_include_module_type_with_constraint_lowers_to_include_item;
  ]
  in
