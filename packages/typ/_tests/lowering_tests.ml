...
let actual_lowering_json = fun (report: Check_result.t) ->
  ...
  let choose_binding =
    BodyArena.bindings body_arena |> List.find_opt
      (
        fun (binding: BodyArena.binding) -> binding.name = Some "choose"
      ) |> Option.expect ~msg:"expected choose binding in lowering test"
  in
...
let expected_lowering_json =
  Data.Json.Object [
    ...
    "exports", Data.Json.Array [
      ...
      Data.Json.Object [
        "name", Data.Json.String "picked";
        "scheme", Data.Json.String "int";
      ];
    ];
    "parameter_spine", Data.Json.Array [
      Data.Json.Object [
...
let actual_type_item_lowering_json = fun (report: Check_result.t) ->
  ...
  Data.Json.Object [
    ...
    "lowering_diagnostics", Data.Json.Array (List.map Diagnostic.to_json report.lowering_diagnostics);
  ]

let expected_abstract_type_lowering_json =
  Data.Json.Object [
...
let expected_manifest_alias_lowering_json =
  Data.Json.Object [
    "items", Data.Json.Array [
      Data.Json.Object [
        "tag", Data.Json.String "type";
...
let expected_arrow_manifest_alias_lowering_json =
  Data.Json.Object [
    "items", Data.Json.Array [
      Data.Json.Object [
        "tag", Data.Json.String "type";
...
let expected_poly_variant_type_lowering_json =
  Data.Json.Object [
    "items", Data.Json.Array [
      Data.Json.Object [
        "tag", Data.Json.String "type";
...
let expected_include_module_type_of_lowering_json =
  Data.Json.Object [
    "items", Data.Json.Array [
      Data.Json.Object [
        "tag", Data.Json.String "include";
...
let expected_include_module_type_with_constraint_lowering_json =
  Data.Json.Object [
    "items", Data.Json.Array [
      Data.Json.Object [
        "tag", Data.Json.String "include";
...
let test_fun_cases_preserve_preceding_parameters = fun ctx ->
  ...
  in
  let report = check_source_text ~filename:(Path.v "packages/typ/tests/fun_cases_with_params.ml") source in Test.Snapshot.assert_inline_json ~ctx ~actual:(actual_lowering_json report) ~expected:expected_lowering_json
...
let test_abstract_type_declarations_lower_to_type_items = fun ctx ->
  ...
  in
  let report = check_source_text ~filename:(Path.v "packages/typ/tests/abstract_types.ml") source in Test.Snapshot.assert_inline_json ~ctx ~actual:(actual_type_item_lowering_json report) ~expected:expected_abstract_type_lowering_json

let test_manifest_type_aliases_lower_to_type_items = fun ctx ->
  let source = String.concat "\n" [ "type name = string"; "let value = \"riot\""; "" ] in
  let report = check_source_text ~filename:(Path.v "packages/typ/tests/type_alias_recovery.ml") source in Test.Snapshot.assert_inline_json ~ctx ~actual:(actual_type_item_lowering_json report) ~expected:expected_manifest_alias_lowering_json

let test_arrow_type_aliases_preserve_labels_during_lowering = fun ctx ->
  let source = String.concat "\n" [ "type ('input, 'output) transform ="; "  'input -> step:('input -> 'output) -> ?fallback:'output -> 'output"; "" ] in
  let report = check_source_text ~filename:(Path.v "packages/typ/tests/arrow_type_alias.ml") source in Test.Snapshot.assert_inline_json ~ctx ~actual:(actual_type_item_lowering_json report) ~expected:expected_arrow_manifest_alias_lowering_json
...
let test_poly_variant_type_declarations_lower_to_type_items = fun ctx ->
  ...
  in
  let report = check_source_text ~filename:(Path.v "packages/typ/tests/poly_variant_types.ml") source in Test.Snapshot.assert_inline_json ~ctx ~actual:(actual_type_item_lowering_json report) ~expected:expected_poly_variant_type_lowering_json

let test_include_module_type_of_lowers_to_include_item = fun ctx ->
  let source = String.concat "\n" [ "include module type of Runtime.Process"; "val spawn : (unit -> (unit, exit_reason) result) -> Pid.t"; "" ] in
  let report = check_source_text ~filename:(Path.v "packages/typ/tests/include_module_type_of.mli") source in Test.Snapshot.assert_inline_json ~ctx ~actual:(actual_type_item_lowering_json report) ~expected:expected_include_module_type_of_lowering_json

let test_include_dotted_module_path_lowers_to_include_item = fun ctx ->
  let source = String.concat "\n" [ "include Kernel.Int64"; "" ] in
  let report = check_source_text ~filename:(Path.v "packages/typ/tests/include_dotted_module_path.ml") source in Test.Snapshot.assert_inline_json ~ctx ~actual:(actual_type_item_lowering_json report) ~expected:(
    Data.Json.Object [
      "items", Data.Json.Array [
        Data.Json.Object [
          "tag", Data.Json.String "include";
...
let test_include_module_type_with_constraint_lowers_to_include_item = fun ctx ->
  ...
  let report = check_source_text ~filename:(Path.v "packages/typ/tests/include_module_type_with_constraint.mli") source in Test.Snapshot.assert_inline_json ~ctx ~actual:(actual_type_item_lowering_json report) ~expected:expected_include_module_type_with_constraint_lowering_json
