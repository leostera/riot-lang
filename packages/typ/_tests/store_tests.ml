        ...
        | Ok () -> (
          ...
          | Ok () -> (
            ...
            | Some loaded_module, Some loaded_package ->
                let loaded_exports =
                  ModuleTypings.exports loaded_module |> List.map
                    (
                ...
                if not (List.equal String.equal loaded_exports [ "answer" ]) then
                  Error (format Format.[ str "unexpected module exports: "; str (String.concat ", " loaded_exports) ])
                else
                  if not (List.equal String.equal package_modules [ "Std" ]) then
                    Error (format Format.[ str "unexpected package modules: "; str (String.concat ", " package_modules) ])
                  else Ok ()
  ...
  with_store
    (
      fun contentstore store ->
        ...
        in
        match Contentstore.Store.save_named_json_bundle contentstore ~namespace:legacy_module_typings_name_namespace ~key:"Std" ~json:module_json with
        | Error _ as err -> err
        | Ok () -> (
          match Contentstore.Store.save_named_json_bundle contentstore ~namespace:legacy_package_typings_namespace ~key:"std" ~json:package_json with
          | Error _ as err -> err
          | Ok () -> (
            match Store.load_module_typings store ~module_name:"Std", Store.load_package_bundle store ~package_name:"std" with
            | None, None -> Ok ()
  ...
  with_store
    (
      fun contentstore store ->
        ...
        in
        match Contentstore.Store.save_named_json_bundle contentstore ~namespace:legacy_v3_module_typings_name_namespace ~key:"Std" ~json:module_json with
        | Error _ as err -> err
        | Ok () -> (
          match Contentstore.Store.save_named_json_bundle contentstore ~namespace:legacy_v3_package_typings_namespace ~key:"std" ~json:package_json with
          | Error _ as err -> err
          | Ok () -> (
            match Store.load_module_typings store ~module_name:"Std", Store.load_package_bundle store ~package_name:"std" with
            | None, None -> Ok ()
...
let main ~args =
  let tests = [ Test.case "store roundtrips current namespace" test_store_roundtrips_current_namespace; Test.case "store ignores legacy v2 namespace" test_store_ignores_legacy_v2_namespace; Test.case "store ignores legacy v3 namespace" test_store_ignores_legacy_v3_namespace ] in Test.Cli.main ~name:"typ:store" ~tests ~args ()
