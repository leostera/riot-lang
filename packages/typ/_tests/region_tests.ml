...
let test_local_reachable_vars_include_nested_escaped_vars = fun _ctx ->
  ...
  Region.with_region
    ...
    (fun outer ->
      let outer_ty =
        TypeRepr.make_var ~level:1 0
        |> Region.track_node regions
      in
      let escaped_ty =
        Region.with_region
          regions
          (fun _inner ->
            let escaped_ty =
              TypeRepr.make_var ~level:2 1
              |> Region.track_node regions
            in
            let _dead_ty =
              TypeRepr.make_var ~level:2 2
              |> Region.track_node regions
            in
            let lowered =
              TypeRepr.occurs_or_lower
                ...
                ~level:1
                ~on_lower:(fun ty ->
                  Region.add_to_pool regions ~level:(TypeRepr.level ty) ty
                  |> ignore)
                escaped_ty
            in
            if lowered then
      ...
      in
      let locals =
        Region.local_reachable_vars regions outer (TypeRepr.tuple [ outer_ty; escaped_ty ])
      in
      if locals = [ 0; 1 ] then
        Ok ()
      else
...
let main ~args =
  let tests = [
    Test.case
      "local reachable vars include nested escaped vars"
      test_local_reachable_vars_include_nested_escaped_vars;
  ]
  in
