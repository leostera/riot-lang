...
let summary_pair_for = fun source ->
  ...
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "persisted_summary.ml") ~text:source in
      ...
      begin
        ...
        | Ok decoded ->
            ...
            if not (actual_json = roundtripped_persisted_json) then
              ...
              else Test.Snapshot.assert_json ~ctx ~actual:(
                Data.Json.Object [
...
let test_recursive_type_decl_summary_roundtrip = fun ctx -> assert_roundtrip ~ctx "type inline_node =\n\
    \  | Text of string\n\
