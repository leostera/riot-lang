...
let export_names = fun export ->
  match export with
  | Some (FileSummary.TrustedExport { exports }) | Some (FileSummary.ErroredExport { exports }) ->
      List.map
        (
          fun (name, _scheme) -> SurfacePath.to_string name
        )
        exports
  | Some FileSummary.NoExport | None -> []
...
let export_scheme = fun snapshot source_id name ->
  ...
  | Some FileSummary.NoExport | None -> None
...
let definition_at = fun snapshot source_id offset -> Query.definition_at snapshot source_id (Position.make ~offset)
...
let definition_covers_offset = fun (definition: ModuleTypings.definition_site) offset -> Position.is_within_span (Position.make ~offset) definition.span
...
let offset_of_substring = fun text needle ->
  ...
  let rec loop start =
    if start > max_start then
      ...
      if String.sub text start needle_length = needle then
        Some start
      else loop (start + 1)
  in
  if needle_length = 0 then
    ...
    if needle_length > text_length then
      None
    else loop 0
...
let diagnostic_strings = fun snapshot source_id ->
  Query.diagnostics snapshot source_id |> List.map
    (
      ...
      | Query.Lowering diagnostic | Query.Typing diagnostic -> Diagnostic.to_string diagnostic
    )
...
let typing_diagnostic_summaries = fun snapshot source_id ->
  Query.diagnostics snapshot source_id |> List.filter_map
    (
      function
      | Query.Typing diagnostic -> Some (Diagnostic.code diagnostic, Diagnostic.severity diagnostic |> Diagnostic.severity_to_string, Diagnostic.message diagnostic)
      | Query.Parse _ | Query.Lowering _ -> None
    )
...
let test_local_module_aliases_include_public_wrapper_spellings = fun _ctx ->
  let aliases = LocalModules.local_module_aliases_of_internal_name (LocalModules.InternalName.of_string "Kernel_new__Net__Tcp_listener") |> List.map LocalModules.AmbientName.to_string in
...
let test_contextual_local_module_depth_keeps_single_segment_suffixes = fun _ctx ->
  ...
  let direct_depth = LocalModules.contextual_match_depth ~current_module_name ~required_module_name ~candidate_module_name:direct_candidate in
  let cousin_depth = LocalModules.contextual_match_depth ~current_module_name ~required_module_name ~candidate_module_name:cousin_candidate in
...
let test_local_module_implicit_opens_skip_enclosing_alias_wrappers = fun _ctx ->
  ...
  let include_root_alias = LocalModules.should_include_implicit_open ~current_module_name ~module_name:"Kernel_new__Aliases" in
  let include_fs_alias = LocalModules.should_include_implicit_open ~current_module_name ~module_name:"Kernel_new__Fs__Aliases" in
  let include_colors_alias = LocalModules.should_include_implicit_open ~current_module_name ~module_name:"Colors__Aliases" in
...
let trace_debug = fun snapshot source_id ->
  ...
  | Some analysis ->
      let item_lines =
        analysis.item_traces |> List.map
          (
            fun (trace: Check_result.item_trace) -> format Format.[
              str "item ";
      ...
      in
      let expr_lines =
        analysis.expr_traces |> List.map
          (
            fun (trace: Check_result.expr_trace) -> format Format.[
              str "expr ";
...
let module_typings_jsons = fun snapshot -> Snapshot.module_typings snapshot |> List.map ModuleTypings.Json.to_json
      ...
      ModuleTypings.type_decls typings |> List.map
        (
          fun (type_decl: FileSummary.type_decl) ->
            ...
            else SurfacePath.append_name type_decl.scope_path type_decl.declaration.type_name |> SurfacePath.to_string
        )

let module_typings_export_names = fun typings ->
  ModuleTypings.exports typings |> List.map
    (
      fun (name, _scheme) -> SurfacePath.to_string name
    )
...
let file_summary_export_names = fun snapshot source_id ->
  ...
  | None -> []
  | Some summary ->
      FileSummary.exports summary |> List.map
        (
          fun (name, _scheme) -> SurfacePath.to_string name
        )
...
let prepare_snapshot_or_error = fun session ~roots ->
  ...
  | Error missing -> Error (format Format.[ str "unexpected missing requirements: "; str (Data.Json.to_string (Session.MissingRequirements.to_json missing)) ])

let with_typ_store = fun f ->
  Fs.with_tempdir ~prefix:"typ-store"
    (
      fun tmpdir ->
        let contentstore = Contentstore.create ~root:Path.(tmpdir / Path.v "cache") ~policy:Contentstore.Policy.default () in
        let store = Store.create contentstore () in f store
...
let qualify_type_decls = fun module_name type_decls ->
  List.map
    (
      fun (type_decl: FileSummary.type_decl) -> { FileSummary.scope_path = SurfacePath.prepend_name module_name type_decl.scope_path; declaration = type_decl.declaration }
    )
    type_decls
...
let expect_cst = fun ~filename parse_result ->
  ...
  | Ok cst -> cst
  | Error (Syn.Parse_diagnostics diagnostics) -> panic (format Format.[
  ...
  | Error (Syn.Cst_builder_error error) -> panic (format Format.[
    str "expected successful CST for ";
    str filename;
...
let create_source = fun session ~kind ~origin ~text ->
  ...
  let implicit_opens = [] in Session.create_source session ~kind ~module_name:(Source.infer_module_name origin) ~implicit_opens ~origin ~source_hash:(Source.hash ~implicit_opens ~cst) ~parse_result ~cst
...
let update_source_text = fun session source_id ~kind ~origin ~text ->
  ...
  let parse_result = Syn.parse ~filename text in
  let cst = expect_cst ~filename:(Path.to_string filename) parse_result in
  let implicit_opens = [] in Session.update_source session source_id ~source_hash:(Source.hash ~implicit_opens ~cst) ~parse_result ~cst
...
let make_source_with_implicit_opens = fun ~implicit_opens ~source_id ~kind ~origin ~revision ~text ->
  ...
  in
  let parse_result = Syn.parse ~filename text in
  let cst = expect_cst ~filename:(Path.to_string filename) parse_result in Source.make_prepared ~source_id ~kind ~module_name:(Source.infer_module_name origin) ~implicit_opens ~origin ~revision ~source_hash:(Source.hash ~implicit_opens ~cst) ~parse_result ~cst

let make_source = fun ~source_id ~kind ~origin ~revision ~text -> make_source_with_implicit_opens ~implicit_opens:[] ~source_id ~kind ~origin ~revision ~text
...
let prepared_source = fun ~filename ~text ->
  ...
  let implicit_opens = [] in Source.make_prepared ~source_id ~kind ~module_name:(Source.infer_module_name origin) ~implicit_opens ~origin ~revision:0 ~source_hash:(Source.hash ~implicit_opens ~cst) ~parse_result ~cst
...
let imported_world_of_loaded_modules = fun loaded_modules (source: Source.t) ->
  ...
  in
  let visible_modules =
    required_names |> List.map LocalModules.RequiredName.of_string |> List.filter_map
      (
        fun required_name ->
          if LoadedModules.contains loaded_modules_index ~required_name then
            Some (SurfacePath.of_string (LocalModules.RequiredName.to_string required_name), PackageEnv.ModuleId.Loaded required_name)
          else None
      ) |> List.sort_uniq compare
  in
  let implicit_open_modules =
    source.implicit_opens |> List.map SurfacePath.to_string |> List.map LocalModules.RequiredName.of_string |> List.filter_map
      (
        fun required_name ->
          if LoadedModules.contains loaded_modules_index ~required_name then
            Some (SurfacePath.of_string (LocalModules.RequiredName.to_string required_name), PackageEnv.ModuleId.Loaded required_name)
  ...
  in
  ImportedWorld.create ~package_env:(PackageEnv.of_loaded_modules loaded_modules_index) ~scope_view:(ScopeView.create ~visible_modules ~implicit_open_modules)
...
let prepared_check_source = fun ~source_id ~filename ~internal_module_name ~local_module_name ~public_module_name ~text ->
  ...
  let source = Source.make_prepared ~source_id ~kind ~module_name:internal_module_name ~implicit_opens ~origin ~revision:0 ~source_hash:(Source.hash ~implicit_opens ~cst) ~parse_result ~cst in
  {
...
let scheme_has_named_path =
  let rec type_has_named_path ty =
    ...
    | TypeRepr.Option item | TypeRepr.Array item | TypeRepr.List item | TypeRepr.Seq item -> type_has_named_path item
    | TypeRepr.Result (left, right) -> type_has_named_path left || type_has_named_path right
    | TypeRepr.Named { arguments; _ } | TypeRepr.Tuple arguments -> List.exists type_has_named_path arguments
    ...
    | TypeRepr.Package signature ->
        List.exists
          ...
          signature.values
    | TypeRepr.Int | TypeRepr.Float | TypeRepr.Bool | TypeRepr.String | TypeRepr.Char | TypeRepr.Unit | TypeRepr.Var _ | TypeRepr.Hole _ -> false
  in
  fun scheme ->
    let (_quantified, body) = TypeScheme.to_explicit scheme in type_has_named_path body

let test_source_id_stays_stable_across_updates = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "stable.ml") ~text:"let x = 1" in
  let snapshot_before = Session.snapshot session in
  let session = update_source_text session source_id ~kind:Source.File ~origin:(Source.Label "stable.ml") ~text:"let y = 2" in
...
let test_snapshots_remain_immutable_after_updates = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "immutable.ml") ~text:"let x = 1" in
  let snapshot_before = Session.snapshot session in
  let session = update_source_text session source_id ~kind:Source.File ~origin:(Source.Label "immutable.ml") ~text:"let x = true" in
  let snapshot_after = Session.snapshot session in
...
let test_type_at_uses_smallest_indexed_expression = fun _ctx ->
  ...
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "type_at.ml") ~text:source in
...
let test_definition_at_uses_local_pattern_origin = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let source = "let id x = x\nlet answer = id 42\n" in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "definition_local.ml") ~text:source in
  let snapshot = Session.snapshot session in
  ...
  | Some definition ->
      Test.assert_equal ~expected:"definition_local.ml" ~actual:(source_origin_label definition.origin);
...
let test_definition_at_uses_exported_module_typings = fun _ctx ->
  ...
  let (session, _colors_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:colors_source in
  let client_source = "open Colors\nlet label = to_string \"ok\"\n" in
  let (session, client_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "client.ml") ~text:client_source in
  let snapshot = Session.snapshot session in
  ...
  | Some definition ->
      ...
      let expected_offset = offset_of_substring colors_source "to_string value" |> Option.expect ~msg:"expected export binder" in
      Test.assert_equal ~expected:true ~actual:(definition_covers_offset definition expected_offset);
      Ok ()
...
let test_definition_at_prefers_interface_export_origin = fun _ctx ->
  ...
  let (session, _intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.mli") ~text:intf_source in
  let (session, _impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:"let answer = 42\n" in
...
let test_definition_at_follows_include_reexports = fun _ctx ->
  ...
  let (session, _base_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "base.ml") ~text:base_source in
  let (session, _wrapper_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "wrapper.ml") ~text:"include Base\n" in
...
let test_snapshot_without_traces_still_reports_diagnostics_and_module_typings = fun _ctx ->
  ...
  let source = "let id x = x\nlet broken = missing\n" in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "no_traces.ml") ~text:source in
  let snapshot = Session.snapshot session in
  ...
  else Error (format Format.[ str "expected unbound-name diagnostics, got:\n"; str (String.concat "\n" diagnostics) ])

let test_snapshot_exposes_implicit_file_modules = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, colors_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:"module RGB = struct let blend x y = x end\nlet to_string value = value\n" in
  let (session, demo_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "blend_demo.ml") ~text:"open Colors\nlet midpoint = RGB.blend 1 2\nlet label = to_string \"ok\"\n" in
  let snapshot = Session.snapshot session in
  ...
  if demo_has_unbound_name then
    ...
    if not (color_exports = [ "to_string"; "RGB.blend" ]) then
      Error (format Format.[ str "unexpected colors exports: "; str (String.concat ", " color_exports) ])
    else
      if not (midpoint_type = Some "int -> int -> int") then
        Error (format Format.[ str "unexpected midpoint type: "; str (show_option midpoint_type) ])
      else
        if not (label_type = Some "string -> string") then
...
let test_prepare_snapshot_uses_implicit_opened_alias_modules_with_internal_names = fun _ctx ->
  ...
  let (session, _helper_source_id) = Session.create_source session ~kind:Source.File ~module_name:"Colors__Helper" ~implicit_opens:[] ~origin:(Source.Label "helper.ml") ~source_hash:(Source.hash ~implicit_opens:[] ~cst:helper_cst) ~parse_result:helper_parse_result ~cst:helper_cst in
  ...
  let (session, _aliases_source_id) = Session.create_source session ~kind:Source.Generated ~module_name:"Colors__Aliases" ~implicit_opens:[] ~origin:(Source.Label "Colors__Aliases.ml-gen") ~source_hash:(Source.hash ~implicit_opens:[] ~cst:aliases_cst) ~parse_result:aliases_parse_result ~cst:aliases_cst in
  ...
  let (session, colors_source_id) = Session.create_source session ~kind:Source.File ~module_name:"Colors" ~implicit_opens ~origin:(Source.Label "colors.ml") ~source_hash:(Source.hash ~implicit_opens ~cst:colors_cst) ~parse_result:colors_parse_result ~cst:colors_cst in
...
let test_prepare_snapshot_resolves_internal_module_dependencies_by_local_alias = fun _ctx ->
  ...
  let (session, int_source_id) = Session.create_source session ~kind:Source.File ~module_name:"Pkg__Int" ~implicit_opens:[] ~origin:(Source.Label "int.ml") ~source_hash:(Source.hash ~implicit_opens:[] ~cst:int_cst) ~parse_result:int_parse_result ~cst:int_cst in
  ...
  let (session, token_source_id) = Session.create_source session ~kind:Source.File ~module_name:"Pkg__Token" ~implicit_opens:[] ~origin:(Source.Label "token.ml") ~source_hash:(Source.hash ~implicit_opens:[] ~cst:token_cst) ~parse_result:token_parse_result ~cst:token_cst in
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        if value_type = Some "int" then
          Ok ()
        else Error (format Format.[ str "unexpected value type: "; str (show_option value_type) ])
...
let test_prepare_snapshot_prefers_internal_local_alias_dependencies_over_loaded_modules = fun _ctx ->
  ...
  let (session, unix_source_id) = Session.create_source session ~kind:Source.File ~module_name:"Ip_addr__Unix" ~implicit_opens:[] ~origin:(Source.Label "unix.ml") ~source_hash:(Source.hash ~implicit_opens:[] ~cst:unix_cst) ~parse_result:unix_parse_result ~cst:unix_cst in
  let (session, impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "ip_addr.ml") ~text:{ocaml|
      include Unix
    |ocaml} in
  let (session, intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "ip_addr.mli") ~text:{ocaml|
      type t
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty impl_diagnostics) then
        ...
        if not (List.is_empty intf_diagnostics) then
          Error (String.concat "\n" intf_diagnostics)
        else
          ...
          if has_signature_error snapshot impl_source_id then
            ...
            if has_signature_error snapshot intf_source_id then
              Error "expected interface signature inclusion to succeed"
            else
              if Option.is_none local_unix_typings then
                Error "expected internal Unix sibling to be included in the rooted snapshot"
              else
                if not (of_string_type = Some "string -> t") then
                  Error (format Format.[ str "unexpected of_string type: "; str (show_option of_string_type) ])
                else
                  if not (to_string_type = Some "t -> string") then
                    Error (format Format.[ str "unexpected to_string type: "; str (show_option to_string_type) ])
                  else Ok ()
...
let test_prepare_snapshot_uses_internal_local_alias_dependencies_transitively = fun _ctx ->
  ...
  let unix_parse_result = Syn.parse ~filename:(Path.v "unix.ml") unix_text in
  let unix_cst = expect_cst ~filename:"unix.ml" unix_parse_result in
  let (session, _unix_source_id) = Session.create_source session ~kind:Source.File ~module_name:"Ip_addr__Unix" ~implicit_opens:[] ~origin:(Source.Label "unix.ml") ~source_hash:(Source.hash ~implicit_opens:[] ~cst:unix_cst) ~parse_result:unix_parse_result ~cst:unix_cst in
  let (session, _ip_addr_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "ip_addr.ml") ~text:{ocaml|
      include Unix
    |ocaml} in
  let (session, _ip_addr_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "ip_addr.mli") ~text:{ocaml|
      type t
      ...
      val to_string : t -> string
    |ocaml} in
  let (session, _socket_addr_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "socket_addr.ml") ~text:{ocaml|
      let render value = Ip_addr.to_string value
    |ocaml} in
  let (session, _socket_addr_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "socket_addr.mli") ~text:{ocaml|
      val render : Ip_addr.t -> string
    |ocaml} in
  let (session, app_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "app.ml") ~text:{ocaml|
...
let test_prepare_snapshot_nested_internal_local_alias_dependencies_typecheck = fun _ctx ->
  let create_named_source session ~module_name ~filename ~text =
    let parse_result = Syn.parse ~filename:(Path.v filename) text in
    let cst = expect_cst ~filename parse_result in Session.create_source session ~kind:Source.File ~module_name ~implicit_opens:[] ~origin:(Source.Label filename) ~source_hash:(Source.hash ~implicit_opens:[] ~cst) ~parse_result ~cst
  in
  let session = Session.empty ~config:Config.default in
  let (session, _ip_addr_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Ip_addr" ~filename:"ip_addr.ml" ~text:{ocaml|
      include Unix
    |ocaml} in
  let (session, _ip_addr_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Ip_addr" ~filename:"ip_addr.mli" ~text:{ocaml|
      type t
      ...
      val to_string : t -> string
    |ocaml} in
  let (session, _socket_addr_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Socket_addr" ~filename:"socket_addr.ml" ~text:{ocaml|
      let render value = Ip_addr.to_string value
    |ocaml} in
  let (session, _socket_addr_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Socket_addr" ~filename:"socket_addr.mli" ~text:{ocaml|
      val render : Ip_addr.t -> string
    |ocaml} in
  let (session, app_source_id) = create_named_source session ~module_name:"Kernel_new" ~filename:"kernel_new.ml" ~text:{ocaml|
      let render value = Socket_addr.render value
    |ocaml} in
  let (session, _unix_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Ip_addr__Unix" ~filename:"unix.ml" ~text:{ocaml|
      type t = string
...
let test_prepare_snapshot_nested_unix_submodule_sees_sibling_ip_addr_exports = fun _ctx ->
  let module_name_suffix_aliases module_name =
    let segments =
      module_name |> String.split_on_char '.' |> List.filter
        (
  ...
  let create_named_source session ~module_name ~filename ~text =
    ...
    let cst = expect_cst ~filename parse_result in Session.create_source session ~kind:Source.File ~module_name ~implicit_opens:[] ~origin:(Source.Label filename) ~source_hash:(Source.hash ~implicit_opens:[] ~cst) ~parse_result ~cst
  in
  let register_local_aliases session source_id local_module_name =
    module_name_suffix_aliases local_module_name |> List.fold_left
      (
  ...
  in
  let session = Session.empty ~config:Config.default in
  let (session, net_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Net" ~filename:"net.mli" ~text:{ocaml|
      ...
      |ocaml} in
  let (session, net_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Net" ~filename:"net.ml" ~text:{ocaml|
        module IpAddr = Ip_addr
        ...
        module TcpStream = Tcp_stream
      |ocaml} in
  let (session, ip_addr_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Ip_addr" ~filename:"ip_addr.mli" ~text:{ocaml|
        type t
  ...
  let (session, unix_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Ip_addr__Unix" ~filename:"unix.ml" ~text:{ocaml|
        type t = string
  ...
  let (session, tcp_listener_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Tcp_listener__Unix" ~filename:"tcp_listener_unix.ml" ~text:{ocaml|
  ...
  let (session, tcp_listener_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Tcp_listener__Unix" ~filename:"tcp_listener_unix.mli" ~text:{ocaml|
...
let test_prepare_snapshot_wrapper_module_reexports_unix_exports_to_sibling_modules = fun _ctx ->
  ...
  in
  let session = Session.empty ~config:Config.default in
  let (session, ip_addr_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Ip_addr" ~filename:"ip_addr.ml" ~text:{ocaml|
        include Unix
      |ocaml} in
  ...
  let (session, ip_addr_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Ip_addr" ~filename:"ip_addr.ml" ~text:{ocaml|
      ...
      |ocaml} in
  let session = register_local_aliases session ip_addr_impl_source_id "Net.Ip_addr" in
  let (session, ip_addr_unix_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Ip_addr__Unix" ~filename:"ip_addr_unix.ml" ~text:{ocaml|
        type t = string
        type error =
          | InvalidText of { value: string }
  ...
  let (session, socket_addr_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Socket_addr" ~filename:"socket_addr.mli" ~text:{ocaml|
        type t
  ...
  let (session, socket_addr_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Socket_addr" ~filename:"socket_addr.ml" ~text:{ocaml|
        type t = {
          ip : Ip_addr.t;
          port : int;
        }
...
let test_prepare_snapshot_wrapper_module_preserves_same_path_nominal_value_types = fun _ctx ->
  ...
  in
  let session = Session.empty ~config:Config.default in
  let (session, foo_intf_source_id) = create_named_source session ~module_name:"Foo" ~filename:"foo.mli" ~text:{ocaml|
        type t
        type error = InvalidText of string

        val use : t -> error -> unit
      |ocaml} in
  let (session, foo_impl_source_id) = create_named_source session ~module_name:"Foo" ~filename:"foo.ml" ~text:{ocaml|
        include Unix
      |ocaml} in
  let (session, foo_unix_intf_source_id) = create_named_source session ~module_name:"Foo__Unix" ~filename:"foo_unix.mli" ~text:{ocaml|
        type t = string
        type error = InvalidText of string
  ...
  let (session, foo_unix_impl_source_id) = create_named_source session ~module_name:"Foo__Unix" ~filename:"foo_unix.ml" ~text:{ocaml|
        type t = string
        type error = InvalidText of string
  ...
  match prepare_snapshot_or_error session ~roots:[
    foo_intf_source_id;
    foo_impl_source_id;
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty foo_intf_diagnostics) then
        ...
        if not (List.is_empty foo_impl_diagnostics) then
          Error (format Format.[
...
let test_prepare_snapshot_wrapper_module_preserves_local_result_error_surface = fun _ctx ->
  ...
  in
  let session = Session.empty ~config:Config.default in
  let (session, result_intf_source_id) = create_named_source session ~module_name:"Result" ~filename:"result.mli" ~text:{ocaml|
        type ('ok, 'error) t =
          | Ok of 'ok
          | Error of 'error
      |ocaml} in
  let (session, result_impl_source_id) = create_named_source session ~module_name:"Result" ~filename:"result.ml" ~text:{ocaml|
        type ('ok, 'error) t =
          | Ok of 'ok
          | Error of 'error
      |ocaml} in
  let (session, ip_addr_intf_source_id) = create_named_source session ~module_name:"Ip_addr" ~filename:"ip_addr.mli" ~text:{ocaml|
        type t
        type error =
          | InvalidText of { value: string }
        ...
        val equal : t -> t -> bool
      |ocaml} in
  let (session, ip_addr_impl_source_id) = create_named_source session ~module_name:"Ip_addr" ~filename:"ip_addr.ml" ~text:{ocaml|
        include Unix
      |ocaml} in
  let (session, ip_addr_unix_intf_source_id) = create_named_source session ~module_name:"Ip_addr__Unix" ~filename:"ip_addr_unix.mli" ~text:{ocaml|
        type t
        type error =
          | InvalidText of { value: string }
  ...
  let (session, ip_addr_unix_impl_source_id) = create_named_source session ~module_name:"Ip_addr__Unix" ~filename:"ip_addr_unix.ml" ~text:{ocaml|
        type t = string
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty ip_addr_intf_diagnostics) then
        ...
        if not (List.is_empty ip_addr_impl_diagnostics) then
          Error (format Format.[
...
let test_prepare_snapshot_nested_module_alias_canonicalizes_sibling_error_types = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _tcp_listener_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "tcp_listener.mli") ~text:{ocaml|
        type error =
          | System of int

        val error_to_string : error -> string
      |ocaml} in
  let (session, _tcp_listener_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "tcp_listener.ml") ~text:{ocaml|
        type error =
          | System of int

        let error_to_string _ = ""
      |ocaml} in
  let (session, _net_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "net.mli") ~text:{ocaml|
        module TcpListener = Tcp_listener
      |ocaml} in
  let (session, _net_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "net.ml") ~text:{ocaml|
        module TcpListener = Tcp_listener
      |ocaml} in
  let (session, error_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "error.mli") ~text:{ocaml|
        type t =
      ...
      |ocaml} in
  let (session, error_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "error.ml") ~text:{ocaml|
        type t =
          | NetTcpListener of Net.TcpListener.error
  ...
  | Ok snapshot ->
      ...
      let net_error_to_string_type =
        ...
        | Some typings -> ModuleTypings.exports typings |> lookup_export "TcpListener.error_to_string" |> Option.map TypePrinter.scheme_to_string
        | None -> None
      in
      let net_type_names =
        match Session.Snapshot.find_module_typings_by_name snapshot "Net" with
        | Some typings ->
            ModuleTypings.type_decls typings |> List.map
              (
                fun (type_decl: FileSummary.type_decl) -> SurfacePath.append_name type_decl.scope_path type_decl.declaration.type_name |> SurfacePath.to_string
              )
        | None -> []
      in
      if not (List.is_empty error_intf_diagnostics) then
        ...
        if not (List.is_empty error_impl_diagnostics) then
          Error (format Format.[
...
let test_prepare_snapshot_kernel_named_wrapper_alias_preserves_nested_error_types = fun _ctx ->
  ...
  in
  let session = Session.empty ~config:Config.default in
  let (session, read_dir_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Fs__Read_dir" ~filename:"read_dir.mli" ~text:{ocaml|
        type error =
          | File of int
  ...
  let (session, read_dir_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Fs__Read_dir" ~filename:"read_dir.ml" ~text:{ocaml|
        type error =
          | File of int
  ...
  let (session, fs_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Fs" ~filename:"fs.mli" ~text:{ocaml|
        module ReadDir = Read_dir
      |ocaml} in
  let session = register_local_aliases session fs_intf_source_id "Fs" in
  let (session, fs_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Fs" ~filename:"fs.ml" ~text:{ocaml|
        module ReadDir = Read_dir
      |ocaml} in
  let session = register_local_aliases session fs_impl_source_id "Fs" in
  let (session, error_source_id) = create_named_source session ~module_name:"Kernel_new__Error" ~filename:"error.ml" ~text:{ocaml|
        type t =
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        if of_fs_read_dir_type = Some "Fs.ReadDir.error -> t" then
          Ok ()
        else Error ("unexpected of_fs_read_dir type: " ^ show_option of_fs_read_dir_type)
...
let test_prepare_snapshot_multiple_wrapper_aliases_preserve_nested_error_types = fun _ctx ->
  ...
  let (session, fs_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Fs" ~filename:"fs.ml" ~text:{ocaml|
      ...
      |ocaml} in
  let session = register_local_aliases session fs_impl_source_id "Fs" in
  let (session, tcp_listener_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Tcp_listener" ~filename:"tcp_listener.mli" ~text:{ocaml|
        type error =
          | System of int
      |ocaml} in
  let session = register_local_aliases session tcp_listener_intf_source_id "Net.TcpListener" in
  let (session, tcp_listener_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Tcp_listener" ~filename:"tcp_listener.ml" ~text:{ocaml|
        type error =
          | System of int
      |ocaml} in
  ...
  let (session, system_time_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Time__System_time" ~filename:"system_time.mli" ~text:{ocaml|
        type error =
          | System of int
      |ocaml} in
  let session = register_local_aliases session system_time_intf_source_id "Time.SystemTime" in
  let (session, system_time_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Time__System_time" ~filename:"system_time.ml" ~text:{ocaml|
        type error =
          | System of int
      |ocaml} in
  let session = register_local_aliases session system_time_impl_source_id "Time.SystemTime" in
  let (session, time_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Time" ~filename:"time.mli" ~text:{ocaml|
        module SystemTime = System_time
      |ocaml} in
  let session = register_local_aliases session time_intf_source_id "Time" in
  let (session, time_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Time" ~filename:"time.ml" ~text:{ocaml|
        module SystemTime = System_time
      |ocaml} in
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        if of_fs_read_dir_type = Some "Fs.ReadDir.error -> t" && of_net_tcp_listener_type = Some "Net.TcpListener.error -> t" && of_time_system_time_type = Some "Time.SystemTime.error -> t" then
          Ok ()
...
let test_prepare_snapshot_nested_include_wrapper_alias_preserves_error_types = fun _ctx ->
  ...
  in
  let session = Session.empty ~config:Config.default in
  let (session, unix_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Tcp_listener__Unix" ~filename:"tcp_listener_unix.mli" ~text:{ocaml|
        type error =
          | System of int
  ...
  let (session, unix_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Tcp_listener__Unix" ~filename:"tcp_listener_unix.ml" ~text:{ocaml|
        type error =
          | System of int
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        if of_net_tcp_listener_type = Some "Net.TcpListener.error -> t" then
          Ok ()
        else Error ("unexpected of_net_tcp_listener type: " ^ show_option of_net_tcp_listener_type)
...
let test_prepare_snapshot_planner_aliases_preserve_nested_constructor_owners = fun _ctx ->
  let create_named_source session ~kind ~module_name ~filename ~implicit_opens ~text =
    let parse_result = Syn.parse ~filename:(Path.v filename) text in
    let cst = expect_cst ~filename parse_result in Session.create_source session ~kind ~module_name ~implicit_opens ~origin:(Source.Label filename) ~source_hash:(Source.hash ~implicit_opens ~cst) ~parse_result ~cst
  in
  let session = Session.empty ~config:Config.default in
  let (session, _read_dir_intf_source_id) = create_named_source session ~kind:Source.File ~module_name:"Kernel_new__Fs__Read_dir" ~filename:"read_dir.mli" ~implicit_opens:[] ~text:{ocaml|
        type error =
          | File of int
      |ocaml} in
  let (session, _read_dir_impl_source_id) = create_named_source session ~kind:Source.File ~module_name:"Kernel_new__Fs__Read_dir" ~filename:"read_dir.ml" ~implicit_opens:[] ~text:{ocaml|
        type error =
          | File of int
      |ocaml} in
  let (session, _fs_aliases_source_id) = create_named_source session ~kind:Source.Generated ~module_name:"Kernel_new__Fs__Aliases" ~filename:"Kernel_new__Fs__Aliases.ml-gen" ~implicit_opens:[] ~text:{ocaml|
  ...
  let (session, _fs_intf_source_id) = create_named_source session ~kind:Source.File ~module_name:"Kernel_new__Fs" ~filename:"fs.mli" ~implicit_opens:fs_implicit_opens ~text:{ocaml|
        module ReadDir = Read_dir
      |ocaml} in
  let (session, _fs_impl_source_id) = create_named_source session ~kind:Source.File ~module_name:"Kernel_new__Fs" ~filename:"fs.ml" ~implicit_opens:fs_implicit_opens ~text:{ocaml|
        module ReadDir = Read_dir
      |ocaml} in
  let (session, _root_aliases_source_id) = create_named_source session ~kind:Source.Generated ~module_name:"Kernel_new__Aliases" ~filename:"Kernel_new__Aliases.ml-gen" ~implicit_opens:[] ~text:{ocaml|
  ...
  let (session, error_source_id) = create_named_source session ~kind:Source.File ~module_name:"Kernel_new__Error" ~filename:"error.ml" ~implicit_opens:error_implicit_opens ~text:{ocaml|
        type t =
          | FsReadDir of Fs.ReadDir.error
...
let test_prepare_snapshot_partial_wrapper_preserves_nested_module_exports = fun _ctx ->
  ...
  in
  let session = Session.empty ~config:Config.default in
  let (session, _non_zero_int_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Async__Non_zero_int" ~filename:"non_zero_int.mli" ~text:{ocaml|
        type t = int
      |ocaml} in
  let (session, _non_zero_int_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Async__Non_zero_int" ~filename:"non_zero_int.ml" ~text:{ocaml|
        type t = int
      |ocaml} in
  let (session, _interest_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Async__Interest" ~filename:"interest.mli" ~text:{ocaml|
      ...
      |ocaml} in
  let (session, _interest_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Async__Interest" ~filename:"interest.ml" ~text:{ocaml|
        type t = Non_zero_int.t
        ...
        let is_writable value =
          ...
          true
      |ocaml} in
  let (session, _adapter_unix_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Async__Adapter__Unix" ~filename:"adapter_unix.mli" ~text:{ocaml|
        type error =
          | Oops
      ...
      |ocaml} in
  let (session, _adapter_unix_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Async__Adapter__Unix" ~filename:"adapter_unix.ml" ~text:{ocaml|
        type selector = int
      ...
      |ocaml} in
  let (session, _adapter_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Async__Adapter" ~filename:"adapter.mli" ~text:{ocaml|
        type error =
          | Oops
      ...
      |ocaml} in
  let (session, _adapter_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Async__Adapter" ~filename:"adapter.ml" ~text:{ocaml|
        include Unix
      |ocaml} in
  let (session, poll_source_id) = create_named_source session ~module_name:"Kernel_new__Async__Poll" ~filename:"poll.ml" ~text:{ocaml|
        type t = {
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        if not (make_type = Some "unit -> t") then
          ...
          if not (close_type = Some "t -> unit") then
            Error ("unexpected close type: " ^ show_option close_type)
          else
            if not (poll_type = Some "t -> unit") then
...
let test_prepare_snapshot_paired_module_alias_preserves_interface_shaped_sibling_types = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _non_zero_int_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "non_zero_int.mli") ~text:{ocaml|
        type t = int
      |ocaml} in
  let (session, _non_zero_int_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "non_zero_int.ml") ~text:{ocaml|
        type t = int
      |ocaml} in
  let (session, _interest_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "interest.mli") ~text:{ocaml|
        type t
        ...
        val is_readable : t -> bool
      |ocaml} in
  let (session, _interest_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "interest.ml") ~text:{ocaml|
        type t = Non_zero_int.t
        ...
        let is_readable value =
          ...
          true
      |ocaml} in
  let (session, async_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "async.mli") ~text:{ocaml|
        module Interest : sig
          type t
      ...
      |ocaml} in
  let (session, async_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "async.ml") ~text:{ocaml|
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty async_intf_diagnostics) then
        ...
        if not (List.is_empty async_impl_diagnostics) then
...
let test_prepare_snapshot_query_order_preserves_in_progress_wrapper_exports = fun _ctx ->
  ...
  let (session, _ip_addr_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Ip_addr" ~filename:"ip_addr.ml" ~text:{ocaml|
        include Unix
      |ocaml} in
  let (session, _ip_addr_unix_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Ip_addr__Unix" ~filename:"ip_addr_unix.mli" ~text:{ocaml|
        type t
        ...
        val to_string : t -> string
      |ocaml} in
  let (session, _ip_addr_unix_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Ip_addr__Unix" ~filename:"ip_addr_unix.ml" ~text:{ocaml|
        type t = string
...
let test_prepare_snapshot_net_wrapper_graph_preserves_ip_addr_exports = fun _ctx ->
  ...
  let (session, socket_addr_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Socket_addr" ~filename:"socket_addr.ml" ~text:{ocaml|
      ...
      |ocaml} in
  let session = register_local_aliases session socket_addr_impl_source_id "Net.Socket_addr" in
  let (session, tcp_stream_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Tcp_stream" ~filename:"tcp_stream.mli" ~text:{ocaml|
        type t

        val local_addr_text : t -> string
      |ocaml} in
  let session = register_local_aliases session tcp_stream_intf_source_id "Net.Tcp_stream" in
  let (session, tcp_stream_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Tcp_stream" ~filename:"tcp_stream.ml" ~text:{ocaml|
        include Unix
      |ocaml} in
  let session = register_local_aliases session tcp_stream_impl_source_id "Net.Tcp_stream" in
  let (session, tcp_stream_unix_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Tcp_stream__Unix" ~filename:"tcp_stream_unix.ml" ~text:{ocaml|
        type t = int
  ...
  | Ok snapshot ->
      ...
      let ip_addr_exports =
        ...
        | None -> "<missing>"
      in
      let ip_addr_impl_exports = export_names (Query.export_of snapshot ip_addr_impl_source_id) |> String.concat ", " in
      let ip_addr_intf_exports = export_names (Query.export_of snapshot ip_addr_intf_source_id) |> String.concat ", " in
      let ip_addr_impl_diagnostics = diagnostic_strings snapshot ip_addr_impl_source_id in
      let ip_addr_intf_diagnostics = diagnostic_strings snapshot ip_addr_intf_source_id in
      if not (List.is_empty socket_addr_diagnostics) then
        ...
        if not (List.is_empty tcp_stream_diagnostics) then
          Error (format Format.[
...
let test_prepare_snapshot_net_wrapper_graph_preserves_ip_addr_exports_with_paired_unix = fun _ctx ->
  ...
  let (session, ip_addr_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Ip_addr" ~filename:"ip_addr.ml" ~text:{ocaml|
      ...
      |ocaml} in
  let session = register_local_aliases session ip_addr_impl_source_id "Net.Ip_addr" in
  let (session, ip_addr_unix_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Ip_addr__Unix" ~filename:"unix.mli" ~text:{ocaml|
        type t
  ...
  let (session, ip_addr_unix_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Ip_addr__Unix" ~filename:"unix.ml" ~text:{ocaml|
        type t = string
  ...
  let (session, tcp_stream_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Tcp_stream" ~filename:"tcp_stream.ml" ~text:{ocaml|
      ...
      |ocaml} in
  let session = register_local_aliases session tcp_stream_impl_source_id "Net.Tcp_stream" in
  let (session, tcp_stream_unix_source_id) = create_named_source session ~module_name:"Kernel_new__Net__Tcp_stream__Unix" ~filename:"unix.ml" ~text:{ocaml|
        type t = int
...
let test_implicit_opens_do_not_leak_into_module_exports = fun _ctx ->
  ...
  let (session, _aliases_source_id) = Session.create_source session ~kind:Source.Generated ~module_name:"Colors__Aliases" ~implicit_opens:[] ~origin:(Source.Label "Colors__Aliases.ml-gen") ~source_hash:(Source.hash ~implicit_opens:[] ~cst:aliases_cst) ~parse_result:aliases_parse_result ~cst:aliases_cst in
  let implicit_opens = [ SurfacePath.of_string "Colors__Aliases" ] in
  let (session, helper_source_id) = Session.create_source session ~kind:Source.File ~module_name:"Colors__Helper" ~implicit_opens ~origin:(Source.Label "helper.ml") ~source_hash:(Source.hash ~implicit_opens ~cst:helper_cst) ~parse_result:helper_parse_result ~cst:helper_cst in
  let snapshot = Session.snapshot session in
...
let test_snapshot_exports_interface_declarations = fun _ctx ->
  ...
  |ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "iface.mli") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    let type_names = exported_type_names snapshot source_id in Test.assert_equal ~expected:(Some "'a. 'a -> 'a") ~actual:id_type;
...
let test_snapshot_exports_interface_externals = fun _ctx ->
  ...
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "externals.mli") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
  else
    let strlen_type = export_scheme snapshot source_id "strlen" in Test.assert_equal ~expected:(Some "string -> int") ~actual:strlen_type;
  Ok ()

let test_snapshot_collects_module_typings = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _) = create_source session ~kind:Source.File ~origin:(Source.Label "alpha.ml") ~text:"let id x = x\n" in
...
let test_snapshot_module_typings_are_canonical_per_module = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:"let answer = 42\n" in
  let (session, _intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.mli") ~text:"val answer : int\n" in
  let snapshot = Session.snapshot session in
  let module_names =
    module_typings_jsons snapshot |> List.filter_map
      (
        function
        | Data.Json.Object fields -> (
...
let test_query_module_typings_of_uses_canonical_root_typings = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:"let answer = 42\n" in
  let (session, intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.mli") ~text:"val answer : int\n" in
  let snapshot = Session.snapshot session in
  let canonical_json =
    match Snapshot.module_typings snapshot with
    | [ typings ] -> ModuleTypings.Json.to_json typings |> Data.Json.to_string
    | typings -> panic (format Format.[ str "expected one canonical module typings value but got "; int (List.length typings) ])
  in
  let typings_json source_id =
    match Query.module_typings_of snapshot source_id with
    | Some typings -> ModuleTypings.Json.to_json typings |> Data.Json.to_string
    | None -> panic (format Format.[ str "expected module typings for "; str (SourceId.to_string source_id) ])
  in
...
let test_paired_modules_export_interface_shaped_file_summaries = fun _ctx ->
  ...
  let (session, intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.mli") ~text:"val answer : int\n" in
  let snapshot = Session.snapshot session in
  Test.assert_equal ~expected:[ "answer" ] ~actual:(export_names (Query.export_of snapshot impl_source_id));
  Test.assert_equal ~expected:[ "answer" ] ~actual:(export_names (Query.export_of snapshot intf_source_id));
  Test.assert_equal ~expected:[ "answer" ] ~actual:(file_summary_export_names snapshot impl_source_id);
  Test.assert_equal ~expected:[ "answer" ] ~actual:(file_summary_export_names snapshot intf_source_id);
  Ok ()

let test_paired_modules_report_signature_inclusion_mismatches = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:"let answer = true\n" in
  let (session, intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.mli") ~text:"val answer : int\n" in
  let snapshot = Session.snapshot session in
...
let test_paired_modules_skip_signature_inclusion_for_errored_implementation = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:"let answer = missing\n" in
  let (session, intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.mli") ~text:"val answer : int\n" in
  let snapshot = Session.snapshot session in
  let has_signature_error source_id =
    Query.diagnostics snapshot source_id |> List.exists
...
let test_paired_modules_skip_signature_inclusion_for_unsupported_interface_types = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "color.ml") ~text:"type t = Color\nlet to_escape_seq ~mode:_ _ = \"\"\n" in
  let (session, intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "color.mli") ~text:"type t\nval to_escape_seq : mode:[> `bg | `fg ] -> t -> string\n" in
  let snapshot = Session.snapshot session in
  let has_signature_error source_id =
    Query.diagnostics snapshot source_id |> List.exists
      (
  ...
  Test.assert_equal ~expected:[ "to_escape_seq" ] ~actual:(file_summary_export_names snapshot impl_source_id);
  Test.assert_equal ~expected:[ "to_escape_seq" ] ~actual:(file_summary_export_names snapshot intf_source_id);
  Ok ()

let test_paired_modules_accept_manifest_alias_specialization = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "hash.ml") ~text:{ocaml|
      type t = bytes
    ...
    |ocaml} in
  let (session, intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "hash.mli") ~text:{ocaml|
      type t
  ...
  Test.assert_equal ~expected:[ "of_bytes"; "to_bytes" ] ~actual:(module_typings_export_names impl_typings);
  Test.assert_equal ~expected:[ "of_bytes"; "to_bytes" ] ~actual:(module_typings_export_names intf_typings);
  Test.assert_equal ~expected:[ "of_bytes"; "to_bytes" ] ~actual:(file_summary_export_names snapshot impl_source_id);
  Test.assert_equal ~expected:[ "of_bytes"; "to_bytes" ] ~actual:(file_summary_export_names snapshot intf_source_id);
  Ok ()
...
let test_paired_modules_canonicalize_builtin_aliases_in_signature_inclusion = fun _ctx ->
  ...
  let (seed_session, string_alias_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "string_alias.mli") ~text:{ocaml|
      type t = string
    |ocaml} in
  ...
  let session = Session.empty ~config in
  let (session, impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "rng.ml") ~text:{ocaml|
      let normalize (value: string) = value

      let default () = ""
    |ocaml} in
  let (session, intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "rng.mli") ~text:{ocaml|
  ...
  Test.assert_equal ~expected:false ~actual:(has_signature_error snapshot impl_source_id);
  Test.assert_equal ~expected:false ~actual:(has_signature_error snapshot intf_source_id);
  Test.assert_equal ~expected:[ "default"; "normalize" ] ~actual:(module_typings_export_names impl_typings);
  Test.assert_equal ~expected:[ "default"; "normalize" ] ~actual:(module_typings_export_names intf_typings);
  Test.assert_equal ~expected:[ "default"; "normalize" ] ~actual:(file_summary_export_names snapshot impl_source_id);
  Test.assert_equal ~expected:[ "default"; "normalize" ] ~actual:(file_summary_export_names snapshot intf_source_id);
  Test.assert_equal ~expected:(Some "string -> string") ~actual:(export_scheme snapshot impl_source_id "normalize");
  Test.assert_equal ~expected:(Some "unit -> string") ~actual:(export_scheme snapshot intf_source_id "default");
  Ok ()
...
let test_paired_modules_accept_manifest_alias_value_usage_in_signature_inclusion = fun _ctx ->
  ...
  let (session, _non_zero_int_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Async__Non_zero_int" ~filename:"non_zero_int.ml" ~text:{ocaml|
        type t = int
      |ocaml} in
  let (session, interest_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Async__Interest" ~filename:"interest.ml" ~text:{ocaml|
        type t = Non_zero_int.t
      ...
      |ocaml} in
  let (session, interest_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Async__Interest" ~filename:"interest.mli" ~text:{ocaml|
        type t = Non_zero_int.t
  ...
  if actual_impl_readable = Some "Non_zero_int.t" && actual_impl_add = Some "Non_zero_int.t -> Non_zero_int.t -> Non_zero_int.t" && actual_impl_is_readable = Some "Non_zero_int.t -> bool" && actual_intf_readable = Some "Non_zero_int.t" && actual_intf_add = Some "Non_zero_int.t -> Non_zero_int.t -> Non_zero_int.t" && actual_intf_is_readable = Some "Non_zero_int.t -> bool" then
    Ok ()
  else Error (format Format.[
...
let test_manifest_option_aliases_canonicalize_during_inference = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, option_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "option.ml") ~text:{ocaml|
      ...
      let map = fun fn ->
        ...
        | None -> None
    |ocaml} in
  let (seed_session, path_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "path.ml") ~text:{ocaml|
      type t = string
  ...
  in
  let config = Config.default |> Config.with_loaded_modules ~loaded_modules:[ loaded_option; loaded_path ] in
  let session = Session.empty ~config in
  let (session, unix_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "unix.ml") ~text:{ocaml|
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        let () = Test.assert_equal ~expected:false ~actual:(has_signature_error snapshot unix_source_id) in
        let () = Test.assert_equal ~expected:true ~actual:(Option.is_some (export_scheme snapshot unix_source_id "home_dir")) in Ok ()

let test_manifest_aliases_canonicalize_across_modules = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, list_alias_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "list_alias.ml") ~text:{ocaml|
  ...
  let session = Session.empty ~config in
  let (session, use_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "use.ml") ~text:{ocaml|
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        let () = Test.assert_equal ~expected:true ~actual:(Option.is_some (export_scheme snapshot use_source_id "wrap")) in
        let () = Test.assert_equal ~expected:true ~actual:(Option.is_some (export_scheme snapshot use_source_id "unwrap")) in Ok ()
...
let test_paired_modules_accept_option_manifest_aliases_in_signature_inclusion = fun _ctx ->
  ...
  let session = Session.empty ~config in
  let (session, impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "env.ml") ~text:{ocaml|
      let normalize (value: string option) : string option = value
    |ocaml} in
  let (session, intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "env.mli") ~text:{ocaml|
        ...
        if has_signature_error snapshot impl_source_id then
          ...
          if has_signature_error snapshot intf_source_id then
            ...
            if impl_scheme != Some "string option -> string option" then
              ...
              if intf_scheme != Some "string option -> string option" then
...
let test_paired_modules_allow_private_top_level_exception_helpers = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "global0.ml") ~text:{ocaml|
      ...
      let panic _ =
        ...
        loop ()
    |ocaml} in
  let (session, intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "global0.mli") ~text:{ocaml|
      val panic : string -> 'a
    |ocaml} in
  let snapshot = Session.snapshot session in
  let impl_diagnostics = diagnostic_strings snapshot impl_source_id in
  ...
  if not (List.is_empty impl_diagnostics) then
    ...
    if not (List.is_empty intf_diagnostics) then
      ...
      let intf_exports = export_names (Query.export_of snapshot intf_source_id) in Test.assert_equal ~expected:[ "panic" ] ~actual:impl_exports;
...
let test_paired_modules_include_sibling_exports_during_signature_inclusion = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _ops_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "ops.ml") ~text:{ocaml|
      let abs x = x
    |ocaml} in
  let (session, impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "global0.ml") ~text:{ocaml|
      include Ops
  ...
  if not (List.is_empty impl_diagnostics) then
    ...
    if not (List.is_empty intf_diagnostics) then
      ...
      let impl_typings = Query.module_typings_of snapshot impl_source_id |> Option.expect ~msg:"missing impl typings" in
      let intf_typings = Query.module_typings_of snapshot intf_source_id |> Option.expect ~msg:"missing interface typings" in Test.assert_equal ~expected:false ~actual:(has_signature_error snapshot impl_source_id);
  Test.assert_equal ~expected:false ~actual:(has_signature_error snapshot intf_source_id);
  Test.assert_equal ~expected:[ "abs"; "answer" ] ~actual:(module_typings_export_names impl_typings);
  Test.assert_equal ~expected:[ "abs"; "answer" ] ~actual:(module_typings_export_names intf_typings);
  Test.assert_equal ~expected:[ "abs"; "answer" ] ~actual:(file_summary_export_names snapshot impl_source_id);
  Test.assert_equal ~expected:[ "abs"; "answer" ] ~actual:(file_summary_export_names snapshot intf_source_id);
  Ok ()

let test_paired_modules_include_paired_sibling_exports_during_signature_inclusion = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _ops_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "ops.ml") ~text:{ocaml|
      let abs x = x

      let answer = abs 1
    |ocaml} in
  let (session, _ops_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "ops.mli") ~text:{ocaml|
      val abs : int -> int
  ...
  if not (List.is_empty impl_diagnostics) then
    ...
    if not (List.is_empty intf_diagnostics) then
      ...
      let intf_typings = Query.module_typings_of snapshot intf_source_id |> Option.expect ~msg:"missing interface typings" in Test.assert_equal ~expected:false ~actual:(has_signature_error snapshot impl_source_id);
  Test.assert_equal ~expected:false ~actual:(has_signature_error snapshot intf_source_id);
  Test.assert_equal ~expected:[ "abs"; "answer"; "total" ] ~actual:(module_typings_export_names impl_typings);
  Test.assert_equal ~expected:[ "abs"; "answer"; "total" ] ~actual:(module_typings_export_names intf_typings);
  Test.assert_equal ~expected:[ "abs"; "answer"; "total" ] ~actual:(file_summary_export_names snapshot impl_source_id);
  Test.assert_equal ~expected:[ "abs"; "answer"; "total" ] ~actual:(file_summary_export_names snapshot intf_source_id);
  Ok ()

let test_source_input_hash_ignores_source_id_and_revision = fun _ctx ->
  let source_a = make_source ~source_id:(SourceId.of_int 0) ~kind:Source.File ~origin:(Source.Label "colors.ml") ~revision:1 ~text:"let id x = x\n" in
  let source_b = make_source ~source_id:(SourceId.of_int 99) ~kind:Source.File ~origin:(Source.Label "colors.ml") ~revision:42 ~text:"let id x = x\n" in
  let source_c = make_source ~source_id:(SourceId.of_int 99) ~kind:Source.File ~origin:(Source.Label "colors.ml") ~revision:43 ~text:"let id x = (x, x)\n" in
...
let test_source_input_hash_ignores_comments_and_docstrings = fun _ctx ->
  let source_a = make_source ~source_id:(SourceId.of_int 0) ~kind:Source.File ~origin:(Source.Label "colors.ml") ~revision:1 ~text:"let id x = x\n" in
  let source_b = make_source ~source_id:(SourceId.of_int 1) ~kind:Source.File ~origin:(Source.Label "colors.ml") ~revision:2 ~text:"(** docs *)\n(* banner *)\nlet id (* note *) x = x\n" in
  let hash_a = Source.input_hash source_a |> Crypto.Digest.hex in
  let hash_b = Source.input_hash source_b |> Crypto.Digest.hex in
...
let test_source_input_hash_changes_with_implicit_opens = fun _ctx ->
  let source_without_opens = make_source ~source_id:(SourceId.of_int 0) ~kind:Source.File ~origin:(Source.Label "colors.ml") ~revision:1 ~text:"let id x = x\n" in
  let source_with_opens = make_source_with_implicit_opens ~implicit_opens:[ SurfacePath.of_string "Colors__Aliases" ] ~source_id:(SourceId.of_int 1) ~kind:Source.File ~origin:(Source.Label "colors.ml") ~revision:2 ~text:"let id x = x\n" in
...
let test_snapshot_uses_loaded_module_typings = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, colors_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:"module RGB = struct let blend x y = x end\nlet to_string value = value\n" in
  let seed_snapshot = Session.snapshot seed_session in
  let loaded_colors =
  ...
  with_typ_store
    (
      fun store ->
        ...
        | Error missing -> Error (format Format.[ str "expected store-backed snapshot preparation to succeed, got "; str (Data.Json.to_string (Session.MissingRequirements.to_json missing)) ])
        | Ok snapshot ->
            ...
            if not (List.is_empty diagnostics) then
              ...
              let midpoint_type = inferred_type_at snapshot demo_source_id 34 in
              let label_type = inferred_type_at snapshot demo_source_id 58 in Test.assert_equal ~expected:(Some "int -> int -> int") ~actual:midpoint_type;
            Test.assert_equal ~expected:(Some "string -> string") ~actual:label_type;
            Ok ()
    )
  ...
  with_typ_store
    (
      fun store ->
        let config = Config.default |> Config.with_store ~store:(Some store) in
        let session = Session.empty ~config in
        let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "single.ml") ~text:"let value = 42\n" in
            ...
            if Option.is_some before then
              ...
              if not (List.is_empty diagnostics) then
                ...
                if value_type <> Some "int" then
                  ...
                  if List.map ModuleTypings.module_name rooted_typings <> [ "Single" ] then
                    Error ("unexpected rooted module typings: " ^ String.concat ", " (List.map ModuleTypings.module_name rooted_typings))
                  else
...
let test_fold_package_sources_persists_module_typings_from_authoritative_engine = fun _ctx ->
  with_typ_store
    (
      fun store ->
        let config = Config.default |> Config.with_store ~store:(Some store) in
        let source_a = prepared_check_source ~source_id:(SourceId.of_int 0) ~filename:"a.ml" ~internal_module_name:"A" ~local_module_name:"A" ~public_module_name:None ~text:"let answer = 42\n" in
        let source_b = prepared_check_source ~source_id:(SourceId.of_int 1) ~filename:"b.ml" ~internal_module_name:"B" ~local_module_name:"B" ~public_module_name:None ~text:"let use = A.answer\n" in
        ...
        match Check.fold_package_sources ~config ~ordered_sources:[ source_b; source_a ] ~init:[] ~f:(
          fun groups (group: Check.finished_group) -> group :: groups
        ) () with
        | Error Check.MissingRequirements { module_name; requirements } -> Error (format Format.[
        ...
        | Error Check.MissingModuleTypings { module_name } -> Error (format Format.[ str "missing authoritative module typings for "; str (LocalModules.InternalName.to_string module_name) ])
        | Error Check.MissingAnalysis { module_name; path } -> Error (format Format.[
          str "missing checked analysis for ";
          str (LocalModules.InternalName.to_string module_name);
          str " at ";
          str (Path.to_string path);
        ])
        | Error Check.StoreFailure { module_name; reason } -> Error (format Format.[
          str "while persisting module typings for ";
          str (LocalModules.InternalName.to_string module_name);
          str ": ";
          str reason;
        ])
        | Error Check.PackageStoreFailure { package_name; reason } -> Error (format Format.[
        ...
        | Ok result ->
            ...
            if Option.is_some before_a || Option.is_some before_b then
              ...
              | (Some a_typings, Some b_typings) ->
                  Test.assert_equal ~expected:[ "A"; "B" ] ~actual:(
                    groups |> List.map
                      (
                        fun (group: Check.finished_group) -> LocalModules.InternalName.to_string group.module_name
                      )
                  );
                  Test.assert_equal ~expected:[ "answer" ] ~actual:(module_typings_export_names a_typings);
                  Test.assert_equal ~expected:[ "use" ] ~actual:(module_typings_export_names b_typings);
                  Ok ()
              | (None, Some _) -> Error "expected authoritative build checking to persist sibling dependency module typings"
              | (Some _, None) -> Error "expected authoritative build checking to persist rooted module typings"
              | (None, None) -> Error "expected authoritative build checking to persist module typings as modules finish"
    )

let test_fold_package_sources_resolves_contextual_local_modules = fun _ctx ->
  let adapter = prepared_check_source ~source_id:(SourceId.of_int 0) ~filename:"async/adapter.ml" ~internal_module_name:"Kernel_new__Async__Adapter" ~local_module_name:"Async.Adapter" ~public_module_name:None ~text:"let id value = value\n" in
  let source = prepared_check_source ~source_id:(SourceId.of_int 1) ~filename:"async/source.ml" ~internal_module_name:"Kernel_new__Async__Source" ~local_module_name:"Async.Source" ~public_module_name:None ~text:"let run value = Adapter.id value\n" in
  let config = Config.default |> Config.with_capture_traces ~capture_traces:false in
  match Check.fold_package_sources ~config ~ordered_sources:[ source; adapter ] ~init:[] ~f:(
    fun groups (group: Check.finished_group) -> group :: groups
  ) () with
  | Error Check.MissingRequirements { module_name; requirements } -> Error (format Format.[
    str "unexpected missing requirements while checking ";
  ...
  | Error Check.MissingModuleTypings { module_name } -> Error (format Format.[ str "missing module typings for "; str (LocalModules.InternalName.to_string module_name) ])
  | Error Check.MissingAnalysis { module_name; path } -> Error (format Format.[
  ...
  | Ok result ->
      ...
      let source_group =
        groups |> List.find_opt
          (
            fun (group: Check.finished_group) -> String.equal (LocalModules.InternalName.to_string group.module_name) "Kernel_new__Async__Source"
          ) |> Option.expect ~msg:"expected Async.Source group"
      in
      let source_analysis =
        source_group.checked_sources |> List.find_opt
          (
            fun (checked_source: Check.checked_source) -> Path.to_string checked_source.path = "async/source.ml"
          ) |> Option.map
          (
            fun (checked_source: Check.checked_source) -> checked_source.analysis
          ) |> Option.expect ~msg:"expected Async.Source analysis"
      in
      let export_names =
        FileSummary.exports source_analysis.file_summary |> List.map
          (
            fun (name, _scheme) -> SurfacePath.to_string name
          )
      in
      Test.assert_equal ~expected:expected_order ~actual:module_order;
      Test.assert_equal ~expected:[] ~actual:(List.map Diagnostic.to_string source_analysis.typing_diagnostics);
      Test.assert_equal ~expected:[ "run" ] ~actual:export_names;
      Test.assert_equal ~expected:true ~actual:(LoadedModules.contains loaded_modules ~required_name:(LocalModules.RequiredName.of_string "Kernel_new__Async__Adapter"));
      Test.assert_equal ~expected:true ~actual:(LoadedModules.contains loaded_modules ~required_name:(LocalModules.RequiredName.of_string "Kernel_new__Async__Source"));
      Ok ()

let test_fold_package_sources_resolves_root_local_module_wrappers = fun _ctx ->
  let async_root = prepared_check_source ~source_id:(SourceId.of_int 0) ~filename:"async.ml" ~internal_module_name:"Kernel_new__Async" ~local_module_name:"Async" ~public_module_name:None ~text:"let answer = 42\n" in
  let consumer = prepared_check_source ~source_id:(SourceId.of_int 1) ~filename:"consumer.ml" ~internal_module_name:"Kernel_new__Consumer" ~local_module_name:"Consumer" ~public_module_name:None ~text:"let run = Async.answer\n" in
  let config = Config.default |> Config.with_capture_traces ~capture_traces:false in
  match Check.fold_package_sources ~config ~ordered_sources:[ consumer; async_root ] ~init:[] ~f:(
    fun groups (group: Check.finished_group) -> group :: groups
  ) () with
  | Error Check.MissingRequirements { module_name; requirements } -> Error (format Format.[
    str "unexpected missing requirements while checking ";
  ...
  | Ok result ->
      ...
      let consumer_group =
        groups |> List.find_opt
          (
            fun (group: Check.finished_group) -> String.equal (LocalModules.InternalName.to_string group.module_name) "Kernel_new__Consumer"
          ) |> Option.expect ~msg:"expected Kernel_new__Consumer group"
      in
      let consumer_analysis =
        consumer_group.checked_sources |> List.find_opt
          (
      ...
      in
      let export_names =
        FileSummary.exports consumer_analysis.file_summary |> List.map
          (
            fun (name, _scheme) -> SurfacePath.to_string name
          )
      in
      Test.assert_equal ~expected:[ "Kernel_new__Async"; "Kernel_new__Consumer" ] ~actual:module_order;
      Test.assert_equal ~expected:[] ~actual:(List.map Diagnostic.to_string consumer_analysis.typing_diagnostics);
      Test.assert_equal ~expected:[ "run" ] ~actual:export_names;
      Test.assert_equal ~expected:true ~actual:(LoadedModules.contains loaded_modules ~required_name:(LocalModules.RequiredName.of_string "Kernel_new__Async"));
      Test.assert_equal ~expected:true ~actual:(LoadedModules.contains loaded_modules ~required_name:(LocalModules.RequiredName.of_string "Kernel_new__Consumer"));
      Ok ()

let test_fold_package_sources_reports_local_module_cycles = fun _ctx ->
  let source_a = prepared_check_source ~source_id:(SourceId.of_int 0) ~filename:"a.ml" ~internal_module_name:"A" ~local_module_name:"A" ~public_module_name:None ~text:{ocaml|
open B

let answer = value
|ocaml} in
  let source_b = prepared_check_source ~source_id:(SourceId.of_int 1) ~filename:"b.ml" ~internal_module_name:"B" ~local_module_name:"B" ~public_module_name:None ~text:{ocaml|
open A

let value = answer
|ocaml} in
  match Check.fold_package_sources ~config:Config.default ~ordered_sources:[ source_a; source_b ] ~init:[] ~f:(
    fun groups (group: Check.finished_group) -> group :: groups
  ) () with
  ...
  | Error Check.MissingModuleTypings { module_name } -> Error ("unexpected missing module typings for " ^ LocalModules.InternalName.to_string module_name)
  | Error Check.MissingAnalysis { module_name; path } -> Error (format Format.[
...
let test_fold_package_sources_shares_imported_world_semantics_for_open_alias_include = fun _ctx ->
  let helpers = prepared_check_source ~source_id:(SourceId.of_int 0) ~filename:"helpers.ml" ~internal_module_name:"Helpers" ~local_module_name:"Helpers" ~public_module_name:None ~text:{ocaml|
type t = Wrap of int
...
|ocaml} in
  let consumer = prepared_check_source ~source_id:(SourceId.of_int 1) ~filename:"consumer.ml" ~internal_module_name:"Consumer" ~local_module_name:"Consumer" ~public_module_name:None ~text:{ocaml|
open Helpers
...
|ocaml} in
  let config = Config.default |> Config.with_capture_traces ~capture_traces:false in
  match Check.fold_package_sources ~config ~ordered_sources:[ consumer; helpers ] ~init:[] ~f:(
    fun groups (group: Check.finished_group) -> group :: groups
  ) () with
  | Error Check.MissingRequirements { module_name; requirements } -> Error (format Format.[
    str "unexpected missing requirements while checking ";
  ...
  | Ok result ->
      ...
      let consumer_analysis =
        groups |> List.find_opt
          (
            fun (group: Check.finished_group) -> String.equal (LocalModules.InternalName.to_string group.module_name) "Consumer"
          ) |> Option.expect ~msg:"expected Consumer group" |> fun (group: Check.finished_group) ->
          group.checked_sources |> List.find_opt
            (
              fun (checked_source: Check.checked_source) -> Path.to_string checked_source.path = "consumer.ml"
            ) |> Option.map
            (
      ...
      if not (List.is_empty diagnostics) then
        ...
        if Option.is_none (lookup_export "from_alias_type" exports) then
          ...
          let () = Test.assert_equal ~expected:(Some "int") ~actual:(export_scheme "via_local_open") in Ok ()

let test_prepare_snapshot_shares_imported_world_semantics_for_open_alias_include = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _helpers_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "helpers.ml") ~text:{ocaml|
type t = Wrap of int
type record = { field : int }
...
|ocaml} in
  let (session, consumer_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "consumer.ml") ~text:{ocaml|
open Helpers
module Alias = Helpers
include Helpers.Nested
...
|ocaml} in
  ...
  if not (List.is_empty diagnostics) then
    ...
    if not (List.mem "from_alias_type" exported_names) then
      ...
      let () = Test.assert_equal ~expected:(Some "int") ~actual:(export_scheme snapshot consumer_source_id "via_local_open") in Ok ()

let test_fold_package_sources_persists_package_bundle = fun _ctx ->
  with_typ_store
    (
      fun store ->
        let source = prepared_check_source ~source_id:(SourceId.of_int 0) ~filename:"async.ml" ~internal_module_name:"Kernel_new__Async" ~local_module_name:"Async" ~public_module_name:(Some "Async") ~text:"let answer = 42\n" in
        let package_fingerprint = Crypto.hash_string "kernel-new:async" in
        let config = Config.default |> Config.with_store ~store:(Some store) |> Config.with_capture_traces ~capture_traces:false in
        match Check.fold_package_sources ~package_name:"kernel-new" ~package_fingerprint ~config ~ordered_sources:[ source ] ~init:[] ~f:(
          fun groups (group: Check.finished_group) -> group :: groups
        ) () with
        | Error Check.MissingRequirements { module_name; requirements } -> Error (format Format.[
          str "unexpected missing requirements while checking ";
        ...
        | Ok result -> (
          ...
          | Some bundle ->
              ...
              let returned_modules = result.public_module_typings |> LoadedModules.values |> List.map ModuleTypings.module_name in
              Test.assert_equal ~expected:(Crypto.Digest.hex package_fingerprint) ~actual:(Crypto.Digest.hex bundle.fingerprint);
...
let test_fold_package_sources_keeps_base_loaded_modules_immutable = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, dep_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "dep.mli") ~text:"val id: 'a -> 'a\n" in
  let seed_snapshot = Session.snapshot seed_session in
  let dep_typings =
  ...
  in
  let config = Config.default |> Config.with_loaded_modules ~loaded_modules:[ dep_typings ] |> Config.with_capture_traces ~capture_traces:false in
  let source = prepared_check_source ~source_id:(SourceId.of_int 0) ~filename:"async.ml" ~internal_module_name:"Kernel_new__Async" ~local_module_name:"Async" ~public_module_name:(Some "Async") ~text:"let answer = 42\n" in
  match Check.fold_package_sources ~config ~ordered_sources:[ source ] ~init:[] ~f:(
    fun groups (group: Check.finished_group) -> group :: groups
  ) () with
  | Error Check.MissingRequirements { module_name; requirements } -> Error (format Format.[
    str "unexpected missing requirements while checking ";
  ...
  | Ok result ->
      let base_module_names = config.loaded_modules |> LoadedModules.values |> List.map ModuleTypings.module_name |> List.sort String.compare in
      let result_module_names = result.loaded_modules |> LoadedModules.values |> List.map ModuleTypings.module_name |> List.sort String.compare in
...
let test_prepare_snapshot_emits_structured_events = fun _ctx ->
  let events = ref [] in
  let config = Config.default |> Config.with_on_event ~on_event:(
    fun event -> events := !events @ [ event ]
  ) in
  let session = Session.empty ~config in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "demo.ml") ~text:{ocaml|
      let answer = 1
    |ocaml} in
  match Session.prepare_snapshot session ~roots:[ source_id ] with
  | Error missing -> Error (format Format.[ str "expected rooted snapshot, got "; str (Data.Json.to_string (Session.MissingRequirements.to_json missing)) ])
  | Ok snapshot ->
      ...
      if not (typ_event_instants_are_monotonic !events) then
        Error (format Format.[ str "expected monotonic typ event instants: "; str (typ_events_json !events) ])
      else
        match !events with
        | [ { Event.kind = Event.PrepareSnapshotStarted _; _ }; { Event.kind = Event.PrepareSnapshotFinished _; _ }; { Event.kind = Event.SnapshotMaterializationStarted _; _ }; { Event.kind = Event.SnapshotMaterializationFinished _; _ }; { Event.kind = Event.ModulePairingStarted _; _ }; { Event.kind = Event.SourceAnalysisStarted { source_id = analysis_source_id; module_name; mode = Event.SnapshotAnalysis; _ }; _ }; { Event.kind = Event.SourceAnalysisFinished { source_id = finished_source_id; module_name = finished_module_name; mode = Event.SnapshotAnalysis; export_status = Event.TrustedExport; _ }; _ }; { Event.kind = Event.ModulePairingFinished { module_name = paired_module_name; export_status = Event.TrustedExport; export_count = 1; type_decl_count = 0; _ }; _ } ] ->
            ...
            if List.for_all
              ...
              [ analysis_source_id; finished_source_id ] && List.for_all
              (
                fun actual_module_name -> String.equal actual_module_name "Demo"
              )
              [ module_name; finished_module_name; paired_module_name ] then
              Ok ()
            else Error (format Format.[ str "unexpected structured event payloads: "; str (typ_events_json !events) ])
        | _ -> Error (format Format.[ str "unexpected typ event payloads: "; str (typ_events_json !events) ])
...
let test_prepare_snapshot_emits_structured_diagnostics_in_events = fun _ctx ->
  ...
  | Ok snapshot ->
      let _ = Query.analysis_of_source snapshot source_id |> Option.expect ~msg:"expected analysis" in
      match !events with
      | [ { Event.kind = Event.PrepareSnapshotStarted _; _ }; { Event.kind = Event.PrepareSnapshotFinished _; _ }; { Event.kind = Event.ModulePairingStarted _; _ }; { Event.kind = Event.SourceAnalysisStarted _; _ }; { Event.kind = Event.SourceAnalysisFinished { source_id = finished_source_id; module_name; mode = Event.SnapshotAnalysis; parse_diagnostics = []; lowering_diagnostics = []; typing_diagnostics = [ _ ]; parse_diagnostic_count = 0; lowering_diagnostic_count = 0; typing_diagnostic_count = 1; export_status = Event.ErroredExport; _ }; _ }; { Event.kind = Event.ModulePairingFinished _; _ } ] when SourceId.equal finished_source_id source_id && String.equal module_name "Demo" -> Ok ()
      | _ -> Error (format Format.[ str "expected structured typing diagnostics in event payloads, got "; str (typ_events_json !events) ])
...
let test_prepare_snapshot_keeps_imported_value_payloads_out_of_ambient_bindings = fun _ctx ->
  ...
  let helpers_text = {ocaml|
    ...
    let value9 = 9
  |ocaml} in
  let (session, _helpers_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "helpers.ml") ~text:helpers_text in
  let consumer_text = {ocaml|
    open Helpers

    let answer = value0 + value9
  |ocaml} in
  let (session, consumer_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "consumer.ml") ~text:consumer_text in
  ...
  | Ok snapshot ->
      ...
      let consumer_starts =
        !events |> List.filter_map
          (
            fun ({ Event.kind; _ }: Event.t) ->
              match kind with
              | Event.SourceAnalysisStarted { source_id; module_name; mode = Event.SnapshotAnalysis; ambient_binding_count; ambient_type_decl_count; _ } when SourceId.equal source_id consumer_source_id -> Some (module_name, ambient_binding_count, ambient_type_decl_count)
              | _ -> None
          )
      in
      ...
      | [ ("Consumer", 0, 0) ] ->
          ...
          Ok ()
      | _ -> Error (format Format.[ str "expected consumer snapshot analysis to avoid imported ambient value payloads, got "; str (typ_events_json !events) ])
...
let test_prepare_snapshot_only_pairs_required_local_modules = fun _ctx ->
  ...
  let session = Session.empty ~config in
  let (session, _dep_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "dep.ml") ~text:{ocaml|
      let value = 1
    |ocaml} in
  let (session, _unused_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "unused.ml") ~text:{ocaml|
      let hidden = 2
    |ocaml} in
  let (session, app_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "app.ml") ~text:{ocaml|
      open Dep
  ...
  | Ok snapshot ->
      ...
      if List.mem "Unused" paired_modules then
        Error (format Format.[ str "unexpected unrelated module pairing: "; str (typ_events_json !events) ])
      else
        (
...
let test_prepare_snapshot_reuses_shared_transitive_local_modules = fun _ctx ->
  ...
  let (session, _base_source_id) = Session.create_source session ~kind:Source.File ~module_name:"Pkg__Base" ~implicit_opens:[] ~origin:(Source.Label "base.ml") ~source_hash:(Source.hash ~implicit_opens:[] ~cst:base_cst) ~parse_result:base_parse_result ~cst:base_cst in
  let (session, _left_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "left.ml") ~text:{ocaml|
      open Base

      let left = value
    |ocaml} in
  let (session, _right_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "right.ml") ~text:{ocaml|
      open Base
  ...
  | Ok snapshot ->
          ...
          (
            fun counts (event: Event.t) ->
              ...
              | Event.SourceAnalysisStarted { module_name; mode = Event.SnapshotAnalysis; _ } ->
                  let count = List.assoc_opt module_name counts |> Option.unwrap_or ~default:0 in (module_name, count + 1) :: List.remove_assoc module_name counts
              | _ -> counts
          )
          []
      in
      let module_pairing_counts =
        !events |> List.fold_left
          ...
          []
      in
      let count_for counts module_name = List.assoc_opt module_name counts |> Option.unwrap_or ~default:0 in
      let repeated_modules =
        [
          ...
          (
            fun module_name -> count_for snapshot_analysis_counts module_name > 1 || count_for module_pairing_counts module_name > 1
          )
      in
...
let test_prepare_snapshot_reuses_paired_local_modules_across_rooted_snapshots = fun _ctx ->
  ...
  let session = Session.empty ~config in
  let (session, file_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "file.mli") ~text:{ocaml|
      val read : unit -> int
    |ocaml} in
  let (session, file_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "file.ml") ~text:{ocaml|
      let read () = 1
    |ocaml} in
  let (session, fs_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "fs.mli") ~text:{ocaml|
      val read : unit -> int
    |ocaml} in
  let (session, fs_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "fs.ml") ~text:{ocaml|
      let read = File.read
    |ocaml} in
  let (session, left_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "left.ml") ~text:{ocaml|
      let left = Fs.read ()
    |ocaml} in
  let (session, right_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "right.ml") ~text:{ocaml|
      let right = Fs.read ()
    |ocaml} in
  let run_root source_id =
    match Session.prepare_snapshot session ~roots:[ source_id ] with
    | Error missing -> Error ("expected rooted snapshot, got " ^ (Session.MissingRequirements.to_json missing |> Data.Json.to_string))
    | Ok snapshot ->
        let _ = Query.analysis_of_source snapshot source_id |> Option.expect ~msg:"expected analysis" in
        let diagnostics = diagnostic_strings snapshot source_id in
        if List.is_empty diagnostics then
    ...
    | Ok () ->
            ...
            (
              fun counts (event: Event.t) ->
                ...
                | Event.SourceAnalysisStarted { source_id; mode = Event.SnapshotAnalysis; _ } ->
                    ...
                    let count = List.assoc_opt key counts |> Option.unwrap_or ~default:0 in (key, count + 1) :: List.remove_assoc key counts
                | _ -> counts
            )
            []
        in
        let module_pairing_counts =
          !events |> List.fold_left
            ...
            []
        in
        let count_for_source counts source_id = List.assoc_opt (SourceId.to_int source_id) counts |> Option.unwrap_or ~default:0 in
        let count_for_module counts module_name = List.assoc_opt module_name counts |> Option.unwrap_or ~default:0 in
        ...
        if not (List.is_empty repeated_sources) || not (List.is_empty repeated_modules) then
          ...
          (
            Test.assert_equal ~expected:1 ~actual:(count_for_source snapshot_analysis_counts file_intf_source_id);
            Test.assert_equal ~expected:1 ~actual:(count_for_source snapshot_analysis_counts file_impl_source_id);
            Test.assert_equal ~expected:1 ~actual:(count_for_source snapshot_analysis_counts fs_intf_source_id);
            Test.assert_equal ~expected:1 ~actual:(count_for_source snapshot_analysis_counts fs_impl_source_id);
...
let test_prepare_snapshot_reuses_shared_implicit_open_alias_modules = fun _ctx ->
  ...
  let (session, _aliases_source_id) = Session.create_source session ~kind:Source.Generated ~module_name:"Aliases" ~implicit_opens:[] ~origin:(Source.Label "Aliases.ml-gen") ~source_hash:(Source.hash ~implicit_opens:[] ~cst:aliases_cst) ~parse_result:aliases_parse_result ~cst:aliases_cst in
  let (session, _base_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "base.ml") ~text:{ocaml|
      let value = 1
    |ocaml} in
  ...
  let (session, _left_source_id) = Session.create_source session ~kind:Source.File ~module_name:"Left" ~implicit_opens ~origin:(Source.Label "left.ml") ~source_hash:(Source.hash ~implicit_opens ~cst:left_cst) ~parse_result:left_parse_result ~cst:left_cst in
  ...
  let (session, _right_source_id) = Session.create_source session ~kind:Source.File ~module_name:"Right" ~implicit_opens ~origin:(Source.Label "right.ml") ~source_hash:(Source.hash ~implicit_opens ~cst:right_cst) ~parse_result:right_parse_result ~cst:right_cst in
  let app_text = {ocaml|
    let total = Left.left + Right.right
  |ocaml} in
  ...
  let (session, app_source_id) = Session.create_source session ~kind:Source.File ~module_name:"App" ~implicit_opens ~origin:(Source.Label "app.ml") ~source_hash:(Source.hash ~implicit_opens ~cst:app_cst) ~parse_result:app_parse_result ~cst:app_cst in
  match Session.prepare_snapshot session ~roots:[ app_source_id ] with
  | Error missing -> Error (format Format.[ str "expected rooted snapshot, got "; str (Data.Json.to_string (Session.MissingRequirements.to_json missing)) ])
  | Ok snapshot ->
      let _ = Query.analysis_of_source snapshot app_source_id |> Option.expect ~msg:"expected analysis" in
  ...
  with_typ_store
    (
      fun store ->
        ...
        let seed_session = Session.empty ~config:Config.default in
        let (seed_session, colors_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:{ocaml|
          module RGB = struct
            let blend x y = x
          end
        ...
        let _ = Store.save_module_typings store loaded_colors |> Result.expect ~msg:"save_module_typings should succeed" in
        let events = ref [] in
        let config = Config.default |> Config.with_store ~store:(Some store) |> Config.with_on_event ~on_event:(
          fun event -> events := !events @ [ event ]
        ) in
        let session = Session.empty ~config in
        let (session, demo_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "blend_demo.ml") ~text:{ocaml|
          open Colors
        ...
        |ocaml} in
        match Session.prepare_snapshot session ~roots:[ demo_source_id ] with
        | Error missing -> Error (format Format.[ str "expected store-backed snapshot, got "; str (Data.Json.to_string (Session.MissingRequirements.to_json missing)) ])
        | Ok _snapshot ->
            ...
            if not (actual = expected) then
              Error ("unexpected hydration event order: " ^ (Data.Json.Array (!events |> List.map Event.to_json) |> Data.Json.to_string))
            else
              match !events with
              | [ { Event.kind = Event.PrepareSnapshotStarted _; _ }; { Event.kind = Event.HydrateModuleTypingsStarted { missing_modules; _ }; _ }; { Event.kind = Event.HydrateModuleTypingsFinished { hydrated_modules; loaded_module_count; _ }; _ }; { Event.kind = Event.PrepareSnapshotFinished { loaded_module_count = final_loaded_module_count; _ }; _ } ] ->
                  if missing_modules = [ "Colors"; "RGB" ] && hydrated_modules = [ "Colors" ] && loaded_module_count = baseline_loaded_module_count + 1 && final_loaded_module_count = baseline_loaded_module_count + 1 then
                    Ok ()
                  else Error ("unexpected hydration payloads: " ^ (Data.Json.Array (!events |> List.map Event.to_json) |> Data.Json.to_string))
              | _ -> Error ("unexpected hydration events: " ^ (Data.Json.Array (!events |> List.map Event.to_json) |> Data.Json.to_string))
    )
...
let test_prepare_snapshot_missing_requirements_emit_structured_events = fun _ctx ->
  ...
  | Error _missing ->
      ...
      match !events with
      | [ { Event.kind = Event.PrepareSnapshotStarted _; _ }; { Event.kind = Event.PrepareSnapshotFailed { missing_root_source_ids; missing_modules; _ }; _ } ] ->
          let () = Test.assert_equal ~expected:[] ~actual:(missing_root_source_ids |> List.map SourceId.to_int) in
          let () = Test.assert_equal ~expected:[ "Missing" ] ~actual:missing_modules in Ok ()
      | _ -> Error ("unexpected missing-requirements events: " ^ (Data.Json.Array (!events |> List.map Event.to_json) |> Data.Json.to_string))
...
let test_prepare_snapshot_missing_requirements_through_include_do_not_trigger_nested_typing = fun _ctx ->
  ...
  let session = Session.empty ~config in
  let (session, _wrapper_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "wrapper.ml") ~text:{ocaml|
  ...
  | Error _missing ->
      ...
      if List.equal String.equal expected actual then
        Ok ()
      else Error ("expected include-driven missing discovery to avoid nested typing work: " ^ (Data.Json.Array (!events |> List.map Event.to_json) |> Data.Json.to_string))

let test_prepare_snapshot_discovers_local_nested_module_dependencies = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _provider_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "provider.ml") ~text:{ocaml|
      module Nested = struct
        let value = 1
      end
    |ocaml} in
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        if answer_type = Some "int" then
          Ok ()
        else Error ("unexpected answer type for nested dependency: " ^ show_option answer_type)
...
let test_prepare_snapshot_discovers_local_include_dependencies = fun _ctx ->
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        if answer_type = Some "int" then
          Ok ()
        else Error ("unexpected answer type for include dependency: " ^ show_option answer_type)
...
let test_prepare_snapshot_discovers_local_implicit_open_dependencies = fun _ctx ->
  ...
  let (session, _aliases_source_id) =
    ...
    let cst = expect_cst ~filename:"aliases.ml" parse_result in Session.create_source session ~kind:Source.File ~module_name:"Aliases" ~implicit_opens:[] ~origin:(Source.Label "aliases.ml") ~source_hash:(Source.hash ~implicit_opens:[] ~cst) ~parse_result ~cst
  in
  let (session, consumer_source_id) =
    ...
    let cst = expect_cst ~filename:"consumer.ml" parse_result in Session.create_source session ~kind:Source.File ~module_name:"Consumer" ~implicit_opens ~origin:(Source.Label "consumer.ml") ~source_hash:(Source.hash ~implicit_opens ~cst) ~parse_result ~cst
  in
  match prepare_snapshot_or_error session ~roots:[ consumer_source_id ] with
  | Error _ as err -> err
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        if answer_type = Some "int" then
          Ok ()
        else Error ("unexpected answer type for implicit-open dependency: " ^ show_option answer_type)
...
let test_prepare_snapshot_reports_local_module_cycles_before_typing = fun _ctx ->
  ...
  let session = Session.empty ~config in
  let (session, a_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "a.ml") ~text:{ocaml|
      open B

      let answer = value
    |ocaml} in
  let (session, b_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "b.ml") ~text:{ocaml|
      open A
  ...
  | Error missing ->
      ...
      let expected =
        Data.Json.Array [
          Data.Json.Object [
            "tag", Data.Json.String "local_module_cycle";
            "module_names", Data.Json.Array [ Data.Json.String "A"; Data.Json.String "B" ];
            "source_ids", Data.Json.Array [ Data.Json.Int (SourceId.to_int a_source_id); Data.Json.Int (SourceId.to_int b_source_id) ];
          ];
        ] |> Data.Json.to_string
      in
      if not (String.equal expected actual) then
        ...
        if List.equal String.equal expected_event_names event_names then
          Ok ()
        else Error ("expected cycle discovery to fail before typing work: " ^ typ_events_json !events)
...
let test_prepare_snapshot_reports_match_coverage_diagnostics = fun _ctx ->
  ...
  let source = {ocaml|
    ...
    let redundant x =
      ...
      | Some value -> value
  |ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "match_coverage.ml") ~text:source in
  ...
  | Ok snapshot ->
      ...
      let () = Test.assert_equal ~expected:[
        "TYP1012", "warning", "non-exhaustive match: missing case None";
        "TYP1013", "warning", "match case is redundant";
      ] ~actual:diagnostics in Ok ()

let test_prepare_snapshot_includes_interface_sibling_dependencies = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:"let answer value = value\n" in
  let (session, intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.mli") ~text:"open Missing_module\nval answer : int -> int\n" in
  ...
  with_typ_store
    (
      fun store ->
        ...
        let (good_seed, good_source_id) = create_source good_seed ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:"module RGB = struct let blend x y = x end\nlet to_string value = value\n" in
        ...
        let (bad_seed, bad_source_id) = create_source bad_seed ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:"let to_string value = value\n" in
        ...
        let config = Config.default |> Config.with_store ~store:(Some store) |> Config.with_loaded_modules ~loaded_modules:[ good_colors ] in
        let session = Session.empty ~config in
        let (session, demo_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "blend_demo.ml") ~text:"open Colors\nlet midpoint = RGB.blend 1 2\nlet label = to_string \"ok\"\n" in
        let snapshot = Session.snapshot session in
        let diagnostics = diagnostic_strings snapshot demo_source_id in
        if not (List.is_empty diagnostics) then
          ...
          let () = Test.assert_equal ~expected:(Some "string -> string") ~actual:label_type in Ok ()
    )

let test_snapshot_uses_sibling_source_record_types = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, colors_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:"type point = { x: int; y: int }\n" in
  let source = {ocaml|
    open Colors
  ...
  |ocaml} in
  let (session, demo_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "blend_demo.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot demo_source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    in
    let type_decl_names =
      ModuleTypings.type_decls seed_summary |> List.map
        (
          fun (type_decl: FileSummary.type_decl) -> type_decl.declaration.type_name
        )
    in
    let field_access_offset =
      let access_start = offset_of_substring source "point.x +" |> Option.expect ~msg:"expected record field access in test source" in access_start + String.length "point."
    in
    ...
    if not (type_decl_names = [ "point" ]) then
      ...
      if not (origin_type = Some "Colors.point") then
        ...
        if not (total_type = Some "Colors.point -> int") then
          Error ("unexpected total type: " ^ show_option total_type)
        else
          if not (field_access_type = Some "int") then
...
let test_snapshot_uses_loaded_module_record_types = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, colors_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:"type point = { x: int; y: int }\n" in
  let seed_snapshot = Session.snapshot seed_session in
  let loaded_colors =
    match Query.module_typings_of seed_snapshot colors_source_id with
    | Some typings -> typings
  ...
  let source = {ocaml|
    ...
    let total point = point.x + point.y
  |ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "consumer.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
...
let test_include_reexports_loaded_module_record_types = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, helpers_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "helpers.ml") ~text:"type point = { x: int; y: int }\n" in
  let seed_snapshot = Session.snapshot seed_session in
  let loaded_helpers =
  ...
  let (consumer_session, consumer_source_id) = create_source consumer_session ~kind:Source.File ~origin:(Source.Label "consumer.ml") ~text:"include Helpers\n" in
  ...
  if not (List.is_empty consumer_diagnostics) then
    ...
    in
    let exported_type_decls =
      ModuleTypings.type_decls consumer_summary |> List.map
        (
          fun (type_decl: FileSummary.type_decl) -> (SurfacePath.to_segments type_decl.scope_path, type_decl.declaration.type_name)
        )
    in
    ...
    let (client_session, client_source_id) = create_source client_session ~kind:Source.File ~origin:(Source.Label "client.ml") ~text:client_source in
    ...
    if not (List.is_empty client_diagnostics) then
      ...
      if not
        ...
        Error (
          "unexpected exported type decls: " ^ String.concat ", "
            (
              List.map
                (
                  fun (scope, name) -> "[" ^ String.concat "." scope ^ "]." ^ name
                )
                exported_type_decls
            )
        )
      else
        if not (origin_type = Some "Consumer.point") then
          Error ("unexpected origin type: " ^ show_option origin_type)
        else
          if not (total_type = Some "Consumer.point -> int") then
            Error ("unexpected total type: " ^ show_option total_type)
          else Ok ()
...
let test_module_alias_reexports_loaded_module_record_types = fun _ctx ->
  ...
  let consumer_config = Config.default |> Config.with_loaded_modules ~loaded_modules:[ loaded_helpers ] in
  let consumer_session = Session.empty ~config:consumer_config in
  let (consumer_session, consumer_source_id) = create_source consumer_session ~kind:Source.File ~origin:(Source.Label "consumer.ml") ~text:"module Util = Helpers\n" in
  let consumer_snapshot = Session.snapshot consumer_session in
  let consumer_diagnostics = diagnostic_strings consumer_snapshot consumer_source_id in
  if not (List.is_empty consumer_diagnostics) then
    Error (String.concat "\n" consumer_diagnostics)
    ...
    if not (List.is_empty client_diagnostics) then
      ...
      if not
        ...
        if not (origin_type = Some "Consumer.Util.point") then
          Error ("unexpected origin type: " ^ show_option origin_type)
        else
          if not (total_type = Some "Consumer.Util.point -> int") then
            Error ("unexpected total type: " ^ show_option total_type)
          else Ok ()

let test_include_reexports_loaded_module_typings = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, helpers_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "helpers.ml") ~text:{ocaml|
type 'a option =
  | None
  | Some of 'a
...
|ocaml} in
  ...
  if not (List.is_empty diagnostics) then
    ...
    if not (id_type = Some "'a. 'a -> 'a") then
      ...
      if not (exported_names = [ "answer"; "id"; "wrap" ]) then
        Error ("unexpected exported names: " ^ String.concat ", " exported_names)
      else
        if not
          (
            wrap_type |> Option.map
              (
                fun text -> Option.is_some (offset_of_substring text "option")
...
let test_include_reexports_package_shaped_loaded_module_typings = fun _ctx ->
  ...
  let (kernel_seed_session, _result_impl_source_id) = create_source kernel_seed_session ~kind:Source.File ~origin:(Source.Label "result.ml") ~text:{ocaml|
let map f x = f x
|ocaml} in
  let (kernel_seed_session, _result_intf_source_id) = create_source kernel_seed_session ~kind:Source.File ~origin:(Source.Label "result.mli") ~text:{ocaml|
val map : ('a -> 'b) -> 'a -> 'b
|ocaml} in
  let (kernel_seed_session, _kernel_impl_source_id) = create_source kernel_seed_session ~kind:Source.File ~origin:(Source.Label "kernel.ml") ~text:{ocaml|
module Result = Result
|ocaml} in
  let (kernel_seed_session, _kernel_intf_source_id) = create_source kernel_seed_session ~kind:Source.File ~origin:(Source.Label "kernel.mli") ~text:{ocaml|
module Result = Result
|ocaml} in
  ...
  let session = Session.empty ~config in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "consumer.ml") ~text:{ocaml|
...
|ocaml} in
  ...
  if not (List.is_empty diagnostics) then
    ...
    let () = Test.assert_equal ~expected:[ "answer"; "map" ] ~actual:exported_names in Ok ()

let test_include_module_type_of_canonicalizes_loaded_nominal_types = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, actors_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "actors.mli") ~text:"module Process: sig\n  type exit_reason = exn\nend\n" in
  let seed_snapshot = Session.snapshot seed_session in
  let loaded_actors =
  ...
  in
  let source = prepared_source ~filename:"process.mli" ~text:{ocaml|
  ...
  let diagnostics = analysis.lowering_diagnostics @ analysis.typing_diagnostics |> List.map Diagnostic.to_string in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
  else
    ...
    | Some spawn_scheme ->
        if scheme_has_named_path spawn_scheme then
          Error ("spawn scheme still contains symbolic named paths: " ^ TypePrinter.scheme_to_string spawn_scheme)
        else
          (
            try
              let _ = ModuleTypings.of_file_summary ~module_name:"Process" ~source_hash:(Source.input_hash source) analysis.file_summary |> ModuleTypings.Json.to_json in Ok ()
            with
...
let test_include_module_type_of_loaded_modules_canonicalizes_nominal_types = fun _ctx ->
  ...
  let source = {ocaml|
    ...
    val spawn: (unit -> (unit, exit_reason) result) -> int
  |ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "process.mli") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    | Some typings ->
        ...
        (
          ...
          | None -> Error "expected spawn export"
          | Some spawn_scheme when scheme_has_named_path spawn_scheme -> Error ("spawn scheme still contains symbolic named paths: " ^ TypePrinter.scheme_to_string spawn_scheme)
          | Some _ -> (
            try
              let _ = ModuleTypings.Json.to_json typings in Ok ()
            with
            | Failure message -> Error message
          )
        )
...
let test_loaded_module_reexports_canonicalize_dependency_result_aliases = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, actors_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "actors.mli") ~text:"module Process: sig\n  type exit_reason = exn\nend\n" in
  let (seed_session, kernel_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "kernel.mli") ~text:{ocaml|
type ('ok, 'error) result =
  | Ok of 'ok
  | Error of 'error
|ocaml} in
  let (seed_session, _global_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "global.mli") ~text:"val spawn : (unit -> (unit, Runtime.Process.exit_reason) Kernel.result) -> int\n" in
  let (seed_session, std_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "std.mli") ~text:"include module type of Global\n" in
  let seed_snapshot = Session.snapshot seed_session in
  let loaded_actors =
    match Query.module_typings_of seed_snapshot actors_source_id with
    | Some typings -> typings
  ...
  in
  let config = Config.default |> Config.with_loaded_modules ~loaded_modules:[ loaded_std; loaded_kernel; loaded_actors ] in
  let session = Session.empty ~config in
  let source = {ocaml|
...
|ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
  else
    let pid_type = export_scheme snapshot source_id "pid" in Test.assert_equal ~expected:(Some "int") ~actual:pid_type;
  Ok ()

let test_operator_pattern_bindings_lower_and_typecheck = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "ops_aliases.ml") ~text:{ocaml|
  ...
  if not (List.is_empty diagnostics) then
    ...
    in
    match required_exports |> List.find_opt
      (
        fun name -> not (List.mem name exported_names)
      ) with
    | Some missing_export -> Error ("missing expected export " ^ missing_export ^ " from " ^ String.concat ", " exported_names)
    | None ->
        let () = Test.assert_equal ~expected:(Some "bool") ~actual:(export_scheme snapshot source_id "same") in
        let () = Test.assert_equal ~expected:(Some "int") ~actual:(export_scheme snapshot source_id "sum") in
        let () = Test.assert_equal ~expected:(Some "int") ~actual:(export_scheme snapshot source_id "piped") in Ok ()

let test_cons_patterns_lower_and_typecheck = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "cons_patterns.ml") ~text:{ocaml|
...
let test_recursive_operator_bindings_typecheck = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "recursive_operator_bindings.ml") ~text:{ocaml|
      let rec ( @ ) left right =
        match left with
        | [] -> right
...
let test_language_prelude_supports_angle_not_equal = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "prelude_ops.ml") ~text:{ocaml|
      let different = 1 <> 2
    |ocaml} in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    ...
    | Some missing_export -> Error ("missing expected export " ^ missing_export ^ " from " ^ String.concat ", " exported_names)
    | None ->
        let () = Test.assert_equal ~expected:(Some "bool") ~actual:(export_scheme snapshot source_id "different") in Ok ()

let test_kernel_ops_surface_typechecks = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "kernel_ops.ml") ~text:{ocaml|
      external equal : 'a -> 'a -> bool = "%equal"
      let ( = ) = equal
...
let test_opened_sibling_double_underscore_exports_typecheck = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _global0_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "global0.ml") ~text:{ocaml|
      ...
      let unix__chdir _ = ()
    |ocaml} in
  let (session, _global0_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "global0.mli") ~text:{ocaml|
      exception Sys__Not_found
    ...
    |ocaml} in
  let (session, env_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "env.ml") ~text:{ocaml|
      type 'a option =
        | None
        | Some of 'a
    ...
    |ocaml} in
  let (session, env_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "env.mli") ~text:{ocaml|
      type 'a option =
        | None
        | Some of 'a
  ...
  if not (List.is_empty diagnostics) then
    ...
    let () = Test.assert_equal ~expected:false ~actual:(has_signature_error snapshot env_impl_source_id) in
    let () = Test.assert_equal ~expected:false ~actual:(has_signature_error snapshot env_intf_source_id) in
    let () = Test.assert_equal ~expected:(Some "string -> string option") ~actual:(export_scheme snapshot env_impl_source_id "getenv") in
    let () = Test.assert_equal ~expected:(Some "string -> string") ~actual:(export_scheme snapshot env_impl_source_id "getenv_exn") in
    let () = Test.assert_equal ~expected:(Some "string -> string -> unit") ~actual:(export_scheme snapshot env_impl_source_id "putenv") in Ok ()

let test_nonrec_same_name_option_alias_prefers_outer_type = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "types.ml") ~text:{ocaml|
...
|ocaml} in
  ...
  if not (List.is_empty diagnostics) then
    ...
    if value_type = Some "int option" then
      Ok ()
    else Error ("unexpected exported value type: " ^ show_option value_type)
...
let test_source_analysis_with_opened_loaded_module_canonicalizes_nominal_types = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, common_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "common.mli") ~text:"type error = int\n" in
  let seed_snapshot = Session.snapshot seed_session in
  let loaded_common =
  ...
  let config = Config.default |> Config.with_loaded_modules ~loaded_modules in
  let source = prepared_source ~filename:"reader.mli" ~text:{ocaml|
      open Common
  ...
  if not (List.is_empty diagnostics) then
    ...
    | Some close_scheme ->
        if scheme_has_named_path close_scheme then
          Error ("close scheme still contains symbolic named paths: " ^ TypePrinter.scheme_to_string close_scheme)
        else
          (
            try
              let _ = ModuleTypings.of_file_summary ~module_name:"Reader" ~source_hash:(Source.input_hash source) analysis.file_summary |> ModuleTypings.Json.to_json in Ok ()
            with
            | Failure message -> Error message
          )
...
let test_snapshot_exports_opened_loaded_nominal_types_from_implementation = fun _ctx ->
  ...
  let session = Session.empty ~config in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "reader.ml") ~text:{ocaml|
open Common
...
|ocaml} in
  ...
  if not (List.is_empty diagnostics) then
    ...
    | Some typings -> (
      ...
      | None -> Error "expected close export"
      | Some close_scheme when scheme_has_named_path close_scheme -> Error ("close scheme still contains symbolic named paths: " ^ TypePrinter.scheme_to_string close_scheme)
      | Some _ -> (
        try
          let _ = ModuleTypings.Json.to_json typings in Ok ()
        with
...
let test_snapshot_type_decls_use_opened_sibling_nominal_types = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, common_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "common.mli") ~text:"type error = int\n" in
  let (session, reader_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "reader.ml") ~text:{ocaml|
      open Common
...
let test_prepare_snapshot_type_decls_use_opened_sibling_nominal_types = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _common_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "common.mli") ~text:"type error = int\n" in
  let (session, reader_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "reader.ml") ~text:{ocaml|
      open Common
      type wrapped = Wrap of error
    |ocaml} in
...
let test_prepare_snapshot_type_decls_use_opened_loaded_nominal_types_with_underscored_module_name = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, parser_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "markdown_parser.mli") ~text:"type inline_node = Text of string\n" in
  let seed_snapshot = Session.snapshot seed_session in
  let loaded_parser =
  ...
  let session = Session.empty ~config in
  let (session, reader_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "markdown_lower.ml") ~text:{ocaml|
...
let test_prepare_snapshot_polyvariant_exports_canonicalize_sibling_structural_types = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _ansi_table_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "ansi_table.ml") ~text:"let to_rgb = [| `rgb (0, 0, 0) |]\n" in
  let (session, colors_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:{ocaml|
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        if actual = expected then
          Ok ()
        else Error ("expected sibling structural polyvariant export scheme " ^ Option.unwrap_or ~default:"<none>" expected ^ " but got " ^ Option.unwrap_or ~default:"<none>" actual)
...
let test_prepare_snapshot_polyvariant_exports_canonicalize_sibling_structural_types_inside_arrows = fun _ctx ->
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        if actual = expected then
          Ok ()
        else Error ("expected sibling structural polyvariant arrow export scheme " ^ Option.unwrap_or ~default:"<none>" expected ^ " but got " ^ Option.unwrap_or ~default:"<none>" actual)
...
|ocaml} in
  ...
  if not (List.is_empty diagnostics) then
    ...
    if not (exported_names = [ "answer"; "Util.id"; "Util.wrap" ]) then
      ...
      if not (util_id_type = Some "'a. 'a -> 'a") then
        Error ("unexpected Util.id type: " ^ show_option util_id_type)
      else
        if not
          (
            util_wrap_type |> Option.map
              (
                fun text -> Option.is_some (offset_of_substring text "option")
              ) |> Option.unwrap_or ~default:false
          ) then
...
let test_module_alias_reexports_same_named_local_modules = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _cell_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "cell.ml") ~text:"let create value = value\n" in
  let (session, sync_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "sync.ml") ~text:"module Cell = Cell\nlet answer = Cell.create 1\n" in
  let snapshot = Session.snapshot session in
  ...
  if not (List.is_empty diagnostics) then
    ...
    if not (exported_names = [ "answer"; "Cell.create" ]) then
      ...
      if not (create_type = Some "'a. 'a -> 'a") then
...
let test_loaded_module_typings_preserve_nested_same_named_alias_exports = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, _cell_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "cell.ml") ~text:"let create value = value\n" in
  let (seed_session, _sync_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "sync.ml") ~text:"module Cell = Cell\n" in
  let (seed_session, std_source_id) = create_source seed_session ~kind:Source.File ~origin:(Source.Label "std.ml") ~text:"module Sync = Sync\n" in
  let seed_snapshot = Session.snapshot seed_session in
  let std_summary =
    match Query.module_typings_of seed_snapshot std_source_id with
    | Some typings -> typings
  ...
  let client_session = Session.empty ~config:client_config in
  let (client_session, client_source_id) = create_source client_session ~kind:Source.File ~origin:(Source.Label "client.ml") ~text:"open Std.Sync\nlet answer = Cell.create 1\n" in
  if not (List.equal String.equal std_exported_names [ "Sync.Cell.create" ]) then
    ...
    | Error missing -> Error ("missing requirements: " ^ (Session.MissingRequirements.to_json missing |> Data.Json.to_string))
    | Ok client_snapshot ->
        let client_diagnostics = diagnostic_strings client_snapshot client_source_id in
        if not (List.is_empty client_diagnostics) then
          Error (String.concat "\n" client_diagnostics)
          ...
          let () = Test.assert_equal ~expected:(Some "int") ~actual:answer_type in Ok ()

let test_paired_loaded_module_typings_preserve_nested_alias_exports_across_include_chain = fun _ctx ->
  let kernel_seed_session = Session.empty ~config:Config.default in
  let (kernel_seed_session, _cell_impl_source_id) = create_source kernel_seed_session ~kind:Source.File ~origin:(Source.Label "cell.ml") ~text:"let create value = value\n" in
  let (kernel_seed_session, _cell_intf_source_id) = create_source kernel_seed_session ~kind:Source.File ~origin:(Source.Label "cell.mli") ~text:"val create : 'a -> 'a\n" in
  let (kernel_seed_session, _sync_impl_source_id) = create_source kernel_seed_session ~kind:Source.File ~origin:(Source.Label "sync.ml") ~text:"module Cell = Cell\n" in
  let (kernel_seed_session, _sync_intf_source_id) = create_source kernel_seed_session ~kind:Source.File ~origin:(Source.Label "sync.mli") ~text:"module Cell = Cell\n" in
  let (kernel_seed_session, kernel_source_id) = create_source kernel_seed_session ~kind:Source.File ~origin:(Source.Label "kernel.ml") ~text:"module Sync = Sync\n" in
  let (kernel_seed_session, _kernel_intf_source_id) = create_source kernel_seed_session ~kind:Source.File ~origin:(Source.Label "kernel.mli") ~text:"module Sync = Sync\n" in
  let kernel_seed_snapshot = Session.snapshot kernel_seed_session in
  let kernel_summary =
  ...
  let kernel_create_type = ModuleTypings.exports kernel_summary |> lookup_export "Sync.Cell.create" |> Option.map TypePrinter.scheme_to_string in
  if not (List.equal String.equal kernel_exported_names [ "Sync.Cell.create" ]) then
    ...
    if not (kernel_create_type = Some "'a. 'a -> 'a") then
      ...
      let (std_seed_session, _sync_impl_source_id) = create_source std_seed_session ~kind:Source.File ~origin:(Source.Label "sync.ml") ~text:"include Kernel.Sync\n" in
      let (std_seed_session, _sync_intf_source_id) = create_source std_seed_session ~kind:Source.File ~origin:(Source.Label "sync.mli") ~text:"include module type of Kernel.Sync\n" in
      let (std_seed_session, std_source_id) = create_source std_seed_session ~kind:Source.File ~origin:(Source.Label "std.ml") ~text:"module Sync = Sync\n" in
      let (std_seed_session, _std_intf_source_id) = create_source std_seed_session ~kind:Source.File ~origin:(Source.Label "std.mli") ~text:"module Sync = Sync\n" in
      ...
      in
      let std_exported_names = module_typings_export_names std_summary in
      let std_create_type = ModuleTypings.exports std_summary |> lookup_export "Sync.Cell.create" |> Option.map TypePrinter.scheme_to_string in
      if not (List.equal String.equal std_exported_names [ "Sync.Cell.create" ]) then
        Error ("unexpected std exports: " ^ String.concat ", " std_exported_names)
      else
        if not (std_create_type = Some "'a. 'a -> 'a") then
          ...
          | Ok client_snapshot ->
              ...
              if not (List.is_empty client_diagnostics) then
                ...
                let callee_offset = offset_of_substring "open Std.Sync\nlet answer = Cell.create 1\n" "Cell.create" |> Option.expect ~msg:"expected Cell.create in client source" in
                let answer_type = export_scheme client_snapshot client_source_id "answer" in
                let callee_type = inferred_type_at client_snapshot client_source_id callee_offset in
                if answer_type = Some "int" then
                  Ok ()
                else Error ("unexpected answer type: " ^ show_option answer_type ^ " callee=" ^ show_option callee_type)

let test_prepare_snapshot_uses_nested_exports_from_local_include_of_loaded_module = fun _ctx ->
  let kernel_seed_session = Session.empty ~config:Config.default in
  let (kernel_seed_session, _cell_impl_source_id) = create_source kernel_seed_session ~kind:Source.File ~origin:(Source.Label "cell.ml") ~text:{ocaml|
let create value = value
|ocaml} in
  let (kernel_seed_session, _cell_intf_source_id) = create_source kernel_seed_session ~kind:Source.File ~origin:(Source.Label "cell.mli") ~text:{ocaml|
val create : 'a -> 'a
|ocaml} in
  let (kernel_seed_session, _sync_impl_source_id) = create_source kernel_seed_session ~kind:Source.File ~origin:(Source.Label "sync.ml") ~text:{ocaml|
module Cell = Cell
|ocaml} in
  let (kernel_seed_session, _sync_intf_source_id) = create_source kernel_seed_session ~kind:Source.File ~origin:(Source.Label "sync.mli") ~text:{ocaml|
module Cell = Cell
|ocaml} in
  let (kernel_seed_session, kernel_impl_source_id) = create_source kernel_seed_session ~kind:Source.File ~origin:(Source.Label "kernel.ml") ~text:{ocaml|
module Sync = Sync
|ocaml} in
  let (kernel_seed_session, _kernel_intf_source_id) = create_source kernel_seed_session ~kind:Source.File ~origin:(Source.Label "kernel.mli") ~text:{ocaml|
module Sync = Sync
|ocaml} in
  ...
  let (std_session, _sync_impl_source_id) = create_source std_session ~kind:Source.File ~origin:(Source.Label "sync.ml") ~text:{ocaml|
include Kernel.Sync
|ocaml} in
  let (std_session, _sync_intf_source_id) = create_source std_session ~kind:Source.File ~origin:(Source.Label "sync.mli") ~text:{ocaml|
...
let answer = Cell.create 1
|ocaml} in
  let (std_session, consumer_source_id) = create_source std_session ~kind:Source.File ~origin:(Source.Label "consumer.ml") ~text:consumer_source in
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        let () = Test.assert_equal ~expected:(Some "int") ~actual:answer_type in
        let () = Test.assert_equal ~expected:(Some "int -> int") ~actual:callee_type in Ok ()
...
let test_sibling_source_uses_loaded_module_record_reexport = fun _ctx ->
  ...
  let consumer_source = {ocaml|
    ...
    let origin = { x = 0; y = 0 }
  |ocaml} in
  let (session, _consumer_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "consumer.ml") ~text:consumer_source in
  ...
  if not (List.is_empty diagnostics) then
    ...
    let field_access_offset =
      let access_start = offset_of_substring client_source "Consumer.origin.x +" |> Option.expect ~msg:"expected record field access in test source" in access_start + String.length "Consumer.origin."
    in
    ...
    let () = Test.assert_equal ~expected:(Some "int") ~actual:field_access_type in Ok ()
...
let test_prepare_snapshot_reports_missing_roots = fun _ctx ->
  ...
  | Error missing ->
      ...
      in
      let () = Test.assert_equal ~expected ~actual in Ok ()

let test_prepare_snapshot_reports_missing_module_summary = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "uses_missing_module.ml") ~text:"open Missing_module\nlet answer = 1\n" in
  match Session.prepare_snapshot session ~roots:[ source_id ] with
...
let test_prepare_snapshot_collects_transitive_missing_modules = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "dependent.ml") ~text:"open Inner\nlet answer = 1\n" in
  let (session, _inner_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "inner.ml") ~text:"open Missing_module\nlet value = 42\n" in
  match Session.prepare_snapshot session ~roots:[ source_id ] with
...
let test_prepare_snapshot_collects_missing_module_for_qualified_reference = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "qualified.ml") ~text:"let answer = Missing_module.value 1\n" in
  match Session.prepare_snapshot session ~roots:[ source_id ] with
...
let test_prepare_snapshot_keeps_nested_sibling_modules_out_of_top_level_requirements = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _provider_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "provider.ml") ~text:"module Missing_module = struct let value = 42 end\n" in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "consumer.ml") ~text:"open Missing_module\nlet answer = value\n" in
  match Session.prepare_snapshot session ~roots:[ source_id ] with
...
let test_prepare_snapshot_keeps_loaded_nested_module_exports_out_of_top_level_requirements = fun _ctx ->
  ...
  let config = Config.default |> Config.with_loaded_modules ~loaded_modules:[ loaded_colors ] in
  let session = Session.empty ~config in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "consumer.ml") ~text:"let midpoint = RGB.blend 1 2\n" in
  match Session.prepare_snapshot session ~roots:[ source_id ] with
...
let test_prepare_snapshot_canonicalizes_missing_requirements = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, first_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "first.ml") ~text:"open Missing_module\nlet first = 1\n" in
  let (session, second_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "second.ml") ~text:"open Missing_module\nlet second = 2\n" in
  let missing_root_a = SourceId.of_int 99 in
  let missing_root_b = SourceId.of_int 42 in
  match Session.prepare_snapshot session ~roots:[
    second_source_id;
    missing_root_a;
    first_source_id;
    second_source_id;
      ...
      let expected =
        Data.Json.Array [
          ...
          Data.Json.Object [
            "tag", Data.Json.String "missing_module_summary";
            "module_name", Data.Json.String "Missing_module";
            "requested_by", Data.Json.Array [ Data.Json.Int (SourceId.to_int first_source_id); Data.Json.Int (SourceId.to_int second_source_id) ];
          ];
        ] |> Data.Json.to_string
      in
      if String.equal expected actual then
...
let test_prepare_snapshot_sorts_missing_modules_by_name = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, zed_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "zed_consumer.ml") ~text:"open Zed\nlet z = 1\n" in
  let (session, alpha_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "alpha_consumer.ml") ~text:"open Alpha\nlet a = 2\n" in
...
let test_prepare_snapshot_uses_local_prelude_variant_constructors = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _prelude_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "prelude.ml") ~text:{ocaml|
      type 'a option =
        | None
        | Some of 'a
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        if has_unbound_name snapshot source_id then
          ...
          let () = Test.assert_equal ~expected:true ~actual:(Option.is_some classify_type) in Ok ()
...
let test_prepare_snapshot_uses_sibling_int_module_with_local_prelude = fun _ctx ->
  ...
  let (session, _prelude_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "prelude.ml") ~text:{ocaml|
      type 'a option =
        ...
        | Some of 'a
    |ocaml} in
  let (session, _int_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "int.ml") ~text:{ocaml|
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        if render_type |> Option.map
          (
            fun text -> Option.is_some (offset_of_substring text "string")
          ) |> Option.unwrap_or ~default:false then
          Ok ()
        else Error ("unexpected render type: " ^ show_option render_type)

let test_prepare_snapshot_types_mutable_record_field_assignment = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "counter.ml") ~text:{ocaml|
      type counter = {
        mutable value: int;
      }
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        if bump_type = Some "counter -> int" then
          Ok ()
        else Error ("unexpected bump type: " ^ show_option bump_type)

let test_prepare_snapshot_types_local_source_module_pack = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "source.ml") ~text:{ocaml|
      type ('ok, 'error) result =
        | Ok of 'ok
        | Error of 'error
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        if to_source_type = Some "t -> Async.Source.t" || to_source_type = Some "int -> Async.Source.t" then
          Ok ()
        else Error ("unexpected to_source type: " ^ show_option to_source_type)
...
let test_check_source_recovers_when_snapshot_preparation_reports_missing_module_summaries = fun _ctx ->
  ...
  let diagnostics = List.length report.parse_diagnostics + List.length report.lowering_diagnostics + List.length report.typing_diagnostics in
  if diagnostics > 0 then
    Ok ()
  else Error "expected single-source session checking to surface diagnostics instead of panicking"
...
let test_match_guards_typecheck_in_pattern_scope = fun _ctx ->
  ...
  let source = {ocaml|
    ...
    let classify value =
      ...
      | None -> 0
  |ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "match_guard.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    if not (classify_type = Some "int option -> int") then
      ...
      if not (guard_binding_type = Some "int") then
...
let test_optional_arguments_can_be_omitted_and_reordered = fun _ctx ->
  ...
  |ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "optional_apply.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    let explicit_offset = offset_of_substring source "make_key ~kind:5 ~mods:6 7" |> Option.expect ~msg:"expected explicit call in test source" in
    ...
    let () = Test.assert_equal ~expected:(Some "?kind:int -> ?mods:int -> int -> int") ~actual:explicit_callee_type in Ok ()
...
|ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "optional_forwarding.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    if poll_type = Some "?max_events:int -> ?timeout:int64 -> unit -> (unit, string) result" then
      Ok ()
    else Error ("unexpected poll type: " ^ show_option poll_type)
...
|ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "inline_record_constructor_payload.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    if make_error_type = Some "int -> error" then
      Ok ()
    else Error ("unexpected make_error type: " ^ show_option make_error_type)
...
let test_records_flow_through_snapshot_queries = fun _ctx ->
  ...
  |ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "records.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
...
let test_fun_cases_keep_preceding_parameters = fun _ctx ->
  ...
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "fun_cases_with_params.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    let delta_offset =
      offset_of_substring source "base + delta" |> Option.expect ~msg:"expected labeled parameter reference in test source" |> fun start -> start + String.length "base + "
    in
    ...
    let () = Test.assert_equal ~expected:(Some "int") ~actual:delta_type in Ok ()
...
|ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "locally_abstract_let_annotation.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    if id_type = Some "'a. 'a -> 'a" then
      Ok ()
    else Error ("unexpected id type: " ^ show_option id_type)
...
|ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "for_loops.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    if loop_type = Some "int -> unit" && countdown_type = Some "int -> unit" then
      Ok ()
    else Error ("unexpected loop types: loop=" ^ show_option loop_type ^ ", countdown=" ^ show_option countdown_type)
...
let explicit flag =
  ...
  "ok"
|ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "if_sequence.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    if implicit_type = Some "bool -> string" && explicit_type = Some "bool -> string" then
      Ok ()
    else Error ("unexpected if/sequence types: implicit=" ^ show_option implicit_type ^ ", explicit=" ^ show_option explicit_type)
...
let increment_ok input =
  ...
  Ok (value + 1)
|ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "let_operator.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    if increment_ok_type = Some "'a. int -> (int, 'a) result" then
      Ok ()
    else Error ("unexpected increment_ok type: " ^ show_option increment_ok_type)
...
|ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "external_identity_cast_token.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    if unsafe_to_value_type = Some "'a. t -> 'a" && unsafe_to_int_type = Some "t -> int" && equal_type = Some "'a. ?eq:('a -> 'a -> bool) -> t -> t -> bool" && make_type = Some "'a. 'a -> t" then
      Ok ()
    else Error ("unexpected token cast types: unsafe_to_value=" ^ show_option unsafe_to_value_type ^ ", unsafe_to_int=" ^ show_option unsafe_to_int_type ^ ", equal=" ^ show_option equal_type ^ ", make=" ^ show_option make_type)
...
let run_packed =
  ...
  M.run 1
|ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "first_class_module_pack_and_unpack.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    if run_packed_type = Some "int" then
      Ok ()
    else Error ("unexpected run_packed type: " ^ show_option run_packed_type)
...
|ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "first_class_module_existential_event.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    if value_type = Some "t" && token_type = Some "t -> int" && is_error_type = Some "t -> bool" then
      Ok ()
    else Error ("unexpected first-class module existential types: value=" ^ show_option value_type ^ ", token=" ^ show_option token_type ^ ", is_error=" ^ show_option is_error_type)

let test_prepare_snapshot_paired_module_preserves_first_class_module_value_signatures = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, event_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "event.mli") ~text:{ocaml|
      module type Intf = sig
        type t
        val is_error : t -> bool
        val token : t -> int
      ...
      val is_error : t -> bool
    |ocaml} in
  let (session, event_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "event.ml") ~text:{ocaml|
      module type Intf = sig
        type t
        val is_error : t -> bool
        val token : t -> int
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty event_intf_diagnostics) then
        ...
        if not (List.is_empty event_impl_diagnostics) then
          ...
          if make_type = Some "'a. (module sig val is_error : 'a -> bool; val token : 'a -> int end) -> 'a -> t" && token_type = Some "t -> int" && is_error_type = Some "t -> bool" then
            Ok ()
          else Error (format Format.[
...
let test_prepare_snapshot_paired_module_allows_hidden_manifest_alias_with_same_variant_shape = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, adapter_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "adapter.mli") ~text:{ocaml|
      type error =
        | Boom
    |ocaml} in
  let (session, adapter_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "adapter.ml") ~text:{ocaml|
      type error =
        | Boom
    |ocaml} in
  let (session, async_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "async.mli") ~text:{ocaml|
      ...
      if not (List.is_empty async_intf_diagnostics) then
        ...
        if not (List.is_empty async_impl_diagnostics) then
          ...
          if id_type = Some "error -> error" then
            Ok ()
          else Error ("unexpected async id type: " ^ show_option id_type)

let test_prepare_snapshot_include_prefers_local_unix_wrapper_over_loaded_unix = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, path_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "path.mli") ~text:{ocaml|
      type t = string
    |ocaml} in
  let (session, path_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "path.ml") ~text:{ocaml|
      type t = string
    |ocaml} in
  let (session, unix_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "unix.mli") ~text:{ocaml|
      type t
      val close : t -> unit
      val open_file : Path.t -> t
    |ocaml} in
  let (session, unix_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "unix.ml") ~text:{ocaml|
      type t = int
      let close _ = ()
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty file_intf_diagnostics) then
        ...
        if not (List.is_empty file_impl_diagnostics) then
        ...
        if not (List.is_empty file_intf_diagnostics) then
          ...
          if not (List.is_empty file_impl_diagnostics) then
            ...
            if open_file_type = Some "Path.t -> t" then
              Ok ()
            else Error ("unexpected file open_file type after errored include: " ^ show_option open_file_type)

let test_prepare_snapshot_errored_wrapper_preserves_persisted_named_type_ids = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, source_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "source.mli") ~text:{ocaml|
      type t = int
    |ocaml} in
  let (session, source_impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "source.ml") ~text:{ocaml|
      type t = int
    |ocaml} in
  let (session, async_intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "async.mli") ~text:{ocaml|
      module Source : sig
      ...
      if not (List.is_empty file_intf_diagnostics) then
        ...
        if not (List.is_empty file_impl_diagnostics) then
          ...
          if to_source_type = Some "t -> Async.Source.t" then
            Ok ()
          else Error ("unexpected file to_source type after errored include: " ^ show_option to_source_type)
...
let test_prepare_snapshot_nested_internal_include_prefers_scoped_unix_wrapper = fun _ctx ->
  ...
  in
  let session = Session.empty ~config:Config.default in
  let (session, path_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Path" ~filename:"path.mli" ~text:{ocaml|
        type t = string
      |ocaml} in
  let session = register_local_aliases session path_intf_source_id "Path" in
  let (session, path_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Path" ~filename:"path.ml" ~text:{ocaml|
        type t = string
      |ocaml} in
  let session = register_local_aliases session path_impl_source_id "Path" in
  let (session, unix_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Fs__File__Unix" ~filename:"file_unix.mli" ~text:{ocaml|
        type t
        val close : t -> unit
        val open_file : Path.t -> t
      |ocaml} in
  let session = register_local_aliases session unix_intf_source_id "Fs.File.Unix" in
  let (session, unix_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Fs__File__Unix" ~filename:"file_unix.ml" ~text:{ocaml|
        type t = int
        let close _ = ()
        let open_file (_ : Path.t) = 0
      |ocaml} in
  let session = register_local_aliases session unix_impl_source_id "Fs.File.Unix" in
  let (session, file_intf_source_id) = create_named_source session ~module_name:"Kernel_new__Fs__File" ~filename:"file.mli" ~text:{ocaml|
        type t
        val close : t -> unit
        val open_file : Path.t -> t
      |ocaml} in
  let session = register_local_aliases session file_intf_source_id "Fs.File" in
  let (session, file_impl_source_id) = create_named_source session ~module_name:"Kernel_new__Fs__File" ~filename:"file.ml" ~text:{ocaml|
        include Unix
      |ocaml} in
...
|ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "gadt_constructor_existentials.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if List.is_empty diagnostics then
    ...
    if run_type = Some "'a. 'a tag -> 'a -> request -> unit" then
      Ok ()
    else Error ("unexpected run type: " ^ show_option run_type)
  else Error (String.concat "\n" diagnostics)
...
|ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "gadt_tuple_constructor_existentials.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if List.is_empty diagnostics then
    ...
    if handle_type = Some "'a. 'a tag -> response -> 'a option" then
      Ok ()
    else Error ("unexpected handle type: " ^ show_option handle_type)
  else Error (String.concat "\n" diagnostics)
...
let get: type state reply. state agent -> (state -> reply) -> reply = fun agent fn ->
  ...
  | _ ->
      ...
      loop ()
|ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "selector_receive_existentials.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if List.is_empty diagnostics then
    ...
    if get_type = Some "'a 'b. 'a agent -> ('a -> 'b) -> 'b" then
      Ok ()
    else Error ("unexpected get type: " ^ show_option get_type)
  else Error (String.concat "\n" diagnostics)
...
let stop: type state. state t -> unit = fun agent ->
  ...
  | _ -> panic "unexpected agent response"
|ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "extensible_selector_agent_shape.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if List.is_empty diagnostics then
    let get_type = export_scheme snapshot source_id "get" in
    ...
    if get_type = Some "'a 'b. 'a t -> ('a -> 'b) -> 'b" && get_and_update_type = Some "'a 'b. 'a t -> ('a -> 'b * 'a) -> 'b" && update_type = Some "'a. 'a t -> ('a -> 'a) -> unit" && cast_type = Some "'a. 'a t -> ('a -> 'a) -> unit" && stop_type = Some "'a. 'a t -> unit" then
      Ok ()
    else Error ("unexpected exported types: get=" ^ show_option get_type ^ ", get_and_update=" ^ show_option get_and_update_type ^ ", update=" ^ show_option update_type ^ ", cast=" ^ show_option cast_type ^ ", stop=" ^ show_option stop_type)
  else Error (String.concat "\n" diagnostics)

let test_loaded_module_selector_agent_shape_typechecks = fun _ctx ->
  let config = Config.default |> Config.with_capture_traces ~capture_traces:true in
  let session = Session.empty ~config in
  let (session, _type_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "type.ml") ~text:{ocaml|
type ('a, 'b) eq =
  | Equal : ('a, 'a) eq
|ocaml} in
  let (session, _ref_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "ref.ml") ~text:{ocaml|
type 'a t = int
...
let type_equal : type a b. a t -> b t -> (a, b) Type.eq option = fun _ _ -> None
|ocaml} in
  let (session, _pid_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "pid.ml") ~text:{ocaml|
type t = unit
|ocaml} in
  let (session, _message_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "message.ml") ~text:{ocaml|
type t = ..
|ocaml} in
  let (session, _global_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "global.ml") ~text:{ocaml|
type ('a, 'e) result =
  | Ok of 'a
  | Error of 'e
...
let panic : string -> 'a = fun _ ->
  ...
  loop ()
|ocaml} in
  let (session, agent_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "agent.ml") ~text:{ocaml|
open Global
...
type ('a, 'b) eq =
  | Equal : ('a, 'a) eq
|ocaml} in
  let (session, ref_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "ref.ml") ~text:{ocaml|
type 'a t = int
...
type t = ..
|ocaml} in
  let (session, global_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "global.ml") ~text:{ocaml|
type ('a, 'e) result =
  | Ok of 'a
  | Error of 'e
...
|ocaml} in
  ...
  if ref_exports = [ "equal"; "make"; "type_equal" ] && global_exports = [
    "panic";
    "receive";
    "self";
    "send";
    "spawn";
    "spawn_link";
  ] && ref_type_equal = Some "'a 'b. int -> int -> ('a, 'b) Type.eq option" && global_spawn = Some "(unit -> (unit, string) result) -> Pid.t" then
    Ok ()
  else Error ("unexpected module exports: ref=" ^ String.concat ", " ref_exports ^ " global=" ^ String.concat ", " global_exports ^ " ref.type_equal=" ^ show_option ref_type_equal ^ " global.spawn=" ^ show_option global_spawn)
...
|ocaml} in
  ...
  | Error (Syn.Deps.Parse_diagnostics diagnostics) -> Error (format Format.[ str "unexpected deps diagnostics: "; str (String.concat "; " (List.map Syn.Diagnostic.to_string diagnostics)) ])
  | Error (Syn.Deps.Cst_builder_error error) -> Error (format Format.[ str "unexpected deps CST error: "; str error.message ])
  | Ok deps ->
      ...
      if modules = [
        ...
        Ok ()
      else Error (format Format.[ str "unexpected deps: "; str (String.concat ", " modules) ])

let test_selector_alias_chain_across_loaded_modules_typechecks = fun _ctx ->
  let config = Config.default |> Config.with_capture_traces ~capture_traces:true in
  let session = Session.empty ~config in
  let (session, _actors_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "actors.ml") ~text:{ocaml|
type ('a, 'e) result =
  | Ok of 'a
  | Error of 'e
...
|ocaml} in
  ...
  if List.is_empty diagnostics then
    ...
    if get_type = Some "'a 'b. 'a t -> ('a -> 'b) -> 'b" then
      Ok ()
    else Error (format Format.[ str "unexpected get type: "; str (show_option get_type) ])
  else Error (String.concat "\n" (diagnostics @ trace_debug snapshot agent_source_id))
...
|ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "worker.ml") ~text:{ocaml|
type Message.t += Ping
...
type ('a, 'b) eq =
  | Equal : ('a, 'a) eq
|ocaml} in
  let (session, _kernel_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "kernel.ml") ~text:{ocaml|
module Type = Type
...
let dangerous_unsafe_cast : 'a -> 'b = fun _ ->
  ...
  loop ()
|ocaml} in
  let (session, _sync_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "sync.ml") ~text:{ocaml|
...
let id x = x
|ocaml} in
  let (session, _int64_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "int64.ml") ~text:{ocaml|
type t = int64
...
|ocaml} in
  ...
  | Ok snapshot ->
      ...
      if List.is_empty diagnostics then
        ...
        if make_type = Some "'a. unit -> 'a t" && type_equal_type = Some "'a 'b. 'a t -> 'b t -> ('a, 'b) Type.eq option" then
          Ok ()
        else Error (format Format.[
...
let test_extensible_variant_constructors_lower_and_typecheck = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, impl_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "message.ml") ~text:{ocaml|
      type message = ..
      ...
      let classify = fun message ->
        ...
        | Data value -> value
    |ocaml} in
  let (session, intf_source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "message.mli") ~text:{ocaml|
      type message = ..
      type message += Ack | Data of int
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty impl_diagnostics) then
        ...
        if not (List.is_empty intf_diagnostics) then
          ...
          let () = Test.assert_equal ~expected:(Some "message") ~actual:(export_scheme snapshot impl_source_id "ack") in
          let () = Test.assert_equal ~expected:(Some "message -> int") ~actual:(export_scheme snapshot impl_source_id "classify") in Ok ()
...
let test_expansive_bindings_stay_monomorphic = fun _ctx ->
  ...
  let source = {ocaml|
    ...
    let alias = id id
  |ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "value_restriction.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    let () = Test.assert_equal ~expected:(Some "'a -> 'a") ~actual:alias_type in Ok ()

let test_nonexpansive_list_bindings_still_generalize = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let source = "let empty = []\n" in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "list_value_restriction.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    let () = Test.assert_equal ~expected:(Some "'a. 'a list") ~actual:empty_type in Ok ()
...
let test_expansive_covariant_lists_still_generalize = fun _ctx ->
  ...
  let source = {ocaml|
    ...
    let xs = make ()
  |ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "relaxed_value_restriction.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    let () = Test.assert_equal ~expected:(Some "'a. 'a list") ~actual:xs_type in Ok ()
...
let test_expansive_covariant_nominal_types_still_generalize = fun _ctx ->
  ...
  let source = {ocaml|
    ...
    let boxed = make ()
  |ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "nominal_value_restriction.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
    ...
    let () = Test.assert_equal ~expected:(Some "'a. 'a box") ~actual:boxed_type in Ok ()
...
let test_expansive_covariant_record_types_still_generalize = fun _ctx ->
  ...
  let source = {ocaml|
    ...
    let boxed = make ()
  |ocaml} in
  let (session, source_id) = create_source session ~kind:Source.File ~origin:(Source.Label "record_value_restriction.ml") ~text:source in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
...
let main ~args =
  let tests =
    [
      ...
      Test.case "prepare_snapshot type declarations use opened loaded nominal types with underscored module name" test_prepare_snapshot_type_decls_use_opened_loaded_nominal_types_with_underscored_module_name;
