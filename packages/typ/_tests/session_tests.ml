...
let inferred_type_at = fun snapshot source_id offset ->
  Query.type_at snapshot source_id (Position.make ~offset)
  |> function
...
let test_local_module_aliases_include_public_wrapper_spellings = fun _ctx ->
  let aliases =
    LocalModules.local_module_aliases_of_internal_name
...
let test_contextual_local_module_depth_prefers_deeper_shared_prefix = fun _ctx ->
  let current_module_name =
    LocalModules.InternalName.of_string "Kernel_new__Net__Tcp_listener__Unix"
  in
  let required_module_name = LocalModules.RequiredName.of_string "Addr" in
  let candidate_module_name =
    LocalModules.InternalName.of_string "Kernel_new__Net__Tcp_listener__Addr"
  in
  let depth =
...
let test_contextual_local_module_depth_keeps_single_segment_suffixes = fun _ctx ->
  ...
  let direct_depth =
    LocalModules.contextual_match_depth
      ~current_module_name
      ~required_module_name
      ~candidate_module_name:direct_candidate
  in
  let cousin_depth =
    LocalModules.contextual_match_depth
      ~current_module_name
      ~required_module_name
      ~candidate_module_name:cousin_candidate
  in
...
let test_local_module_implicit_opens_skip_enclosing_alias_wrappers = fun _ctx ->
  ...
  let include_root_alias =
    LocalModules.should_include_implicit_open
      ~current_module_name
      ~module_name:"Kernel_new__Aliases"
  in
  let include_fs_alias =
    LocalModules.should_include_implicit_open
      ~current_module_name
      ~module_name:"Kernel_new__Fs__Aliases"
  in
  let include_colors_alias =
    LocalModules.should_include_implicit_open ~current_module_name ~module_name:"Colors__Aliases"
  in
...
let module_typings_jsons = fun snapshot ->
  Snapshot.module_typings snapshot
  |> List.map ModuleTypings.Json.to_json
...
let prepared_check_source = fun ~source_id ~filename ~internal_module_name ~local_module_name ~public_module_name ~text ->
  ...
  in
  {
    ...
    public_module_name =
      public_module_name
      |> Option.map LocalModules.AmbientName.of_string;
    source;
  }
...
let test_source_id_stays_stable_across_updates = fun _ctx ->
  ...
  let (session, source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "stable.ml") ~text:"let x = 1"
  in
...
let test_snapshots_remain_immutable_after_updates = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "immutable.ml") ~text:"let x = 1"
  in
  let snapshot_before = Session.snapshot session in
  let session =
    update_source_text
...
let test_type_at_uses_smallest_indexed_expression = fun _ctx ->
  ...
  let (session, source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "type_at.ml") ~text:source
  in
...
let test_definition_at_uses_local_pattern_origin = fun _ctx ->
  ...
  let snapshot = Session.snapshot session in
  let use_offset =
    offset_of_substring source "id 42"
    |> Option.expect ~msg:"expected local use"
  in
  ...
  | Some definition ->
      ...
      let expected_offset =
        offset_of_substring source "id x"
        |> Option.expect ~msg:"expected local binder"
      in
...
let test_definition_at_uses_exported_module_typings = fun _ctx ->
  ...
  let (session, _colors_source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:colors_source
  in
  let client_source = "open Colors\nlet label = to_string \"ok\"\n" in
  let (session, client_source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "client.ml") ~text:client_source
  in
  let snapshot = Session.snapshot session in
  let use_offset =
    offset_of_substring client_source "to_string \"ok\""
    |> Option.expect ~msg:"expected opened use"
  in
  ...
  | Some definition ->
      ...
      let expected_offset =
        offset_of_substring colors_source "to_string value"
        |> Option.expect ~msg:"expected export binder"
      in
      Test.assert_equal ~expected:true ~actual:(definition_covers_offset definition expected_offset);
      Ok ()
...
let test_definition_at_prefers_interface_export_origin = fun _ctx ->
  ...
  let (session, _intf_source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "colors.mli") ~text:intf_source
  in
  let (session, _impl_source_id) =
    create_source
      session
      ~kind:Source.File
  ...
  let snapshot = Session.snapshot session in
  let use_offset =
    offset_of_substring client_source "answer"
    |> Option.expect ~msg:"expected qualified use"
  in
  match definition_at snapshot client_source_id use_offset with
  ...
  | Some definition ->
      ...
      let expected_offset =
        offset_of_substring intf_source "answer"
        |> Option.expect ~msg:"expected interface declaration"
      in
      Test.assert_equal ~expected:true ~actual:(definition_covers_offset definition expected_offset);
      Ok ()
...
let test_definition_at_follows_include_reexports = fun _ctx ->
  ...
  let (session, _base_source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "base.ml") ~text:base_source
  in
  let (session, _wrapper_source_id) =
    create_source
      session
      ~kind:Source.File
  ...
  let snapshot = Session.snapshot session in
  let use_offset =
    offset_of_substring client_source "value"
    |> Option.expect ~msg:"expected reexported use"
  in
  match definition_at snapshot client_source_id use_offset with
  ...
  | Some definition ->
      ...
      let expected_offset =
        offset_of_substring base_source "value = 1"
        |> Option.expect ~msg:"expected base binder"
      in
      Test.assert_equal ~expected:true ~actual:(definition_covers_offset definition expected_offset);
      Ok ()
...
let test_snapshot_without_traces_still_reports_diagnostics_and_module_typings = fun _ctx ->
  let config =
    Config.default
    |> Config.with_capture_traces ~capture_traces:false
  in
  let session = Session.empty ~config in
  let source = "let id x = x\nlet broken = missing\n" in
  let (session, source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "no_traces.ml") ~text:source
  in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  let analysis =
    Query.analysis_of_source snapshot source_id
    |> Option.expect ~msg:"missing analysis"
  in
  let module_typings = Query.module_typings_of snapshot source_id in
  let missing_offset =
    offset_of_substring source "missing"
    |> Option.expect ~msg:"missing offset"
  in
...
let test_prepare_snapshot_prefers_internal_local_alias_dependencies_over_loaded_modules = fun _ctx ->
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty impl_diagnostics) then
        ...
        let local_unix_typings =
          Session.Snapshot.find_module_typings_by_name snapshot "Ip_addr__Unix"
        in
...
let test_prepare_snapshot_uses_internal_local_alias_dependencies_transitively = fun _ctx ->
  ...
  let (session, _unix_source_id) =
    Session.create_source
      ...
      ~cst:unix_cst
  in
  let (session, _ip_addr_impl_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "ip_addr.ml")
    ...
    |ocaml}
  in
  let (session, _ip_addr_intf_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "ip_addr.mli")
    ...
    |ocaml}
  in
  let (session, _socket_addr_impl_source_id) =
    create_source
      session
      ~kind:Source.File
    ...
    |ocaml}
  in
  let (session, _socket_addr_intf_source_id) =
    create_source
      session
      ~kind:Source.File
  ...
  let register_local_aliases session source_id local_module_name =
    ...
    |> List.fold_left
      (fun session module_name ->
        Session.register_source_alias session source_id ~module_name)
      session
  in
  let session = Session.empty ~config:Config.default in
  let (session, net_intf_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Net"
      ~filename:"net.mli"
      ~text:{ocaml|
      ...
      |ocaml}
  in
  let (session, net_impl_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Net"
      ~filename:"net.ml"
...
let test_prepare_snapshot_wrapper_module_preserves_same_path_nominal_value_types = fun _ctx ->
  ...
  | Ok snapshot ->
      ...
      let foo_exports =
        ...
        | Some typings ->
            module_typings_export_names typings
            |> String.concat ", "
        | None -> "<missing>"
      in
...
let test_prepare_snapshot_nested_module_alias_canonicalizes_sibling_error_types = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _tcp_listener_intf_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "tcp_listener.mli")
      ~text:{ocaml|
        type error =
          | System of int
      ...
      |ocaml}
  in
  let (session, _tcp_listener_impl_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "tcp_listener.ml")
      ~text:{ocaml|
        type error =
          | System of int
      ...
      |ocaml}
  in
  let (session, _net_intf_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "net.mli")
      ~text:{ocaml|
        module TcpListener = Tcp_listener
      |ocaml}
  in
  let (session, _net_impl_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "net.ml")
      ~text:{ocaml|
        module TcpListener = Tcp_listener
      |ocaml}
  in
  let (session, error_intf_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "error.mli")
      ~text:{ocaml|
        type t =
      ...
      |ocaml}
  in
  let (session, error_impl_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "error.ml")
      ~text:{ocaml|
        type t =
          | NetTcpListener of Net.TcpListener.error
...
let test_prepare_snapshot_kernel_named_wrapper_alias_preserves_nested_error_types = fun _ctx ->
  ...
  in
  let session = Session.empty ~config:Config.default in
  let (session, read_dir_intf_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Fs__Read_dir"
  ...
  let (session, read_dir_impl_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Fs__Read_dir"
      ~filename:"read_dir.ml"
      ~text:{ocaml|
        type error =
          | File of int
  ...
  let (session, fs_intf_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Fs"
      ~filename:"fs.mli"
      ~text:{ocaml|
  ...
  let (session, error_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Error"
      ~filename:"error.ml"
      ~text:{ocaml|
        type t =
...
let test_prepare_snapshot_multiple_wrapper_aliases_preserve_nested_error_types = fun _ctx ->
  ...
  let (session, tcp_listener_intf_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Net__Tcp_listener"
      ~filename:"tcp_listener.mli"
      ~text:{ocaml|
        type error =
          | System of int
      |ocaml}
  ...
  let (session, system_time_intf_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Time__System_time"
      ~filename:"system_time.mli"
      ~text:{ocaml|
        type error =
          | System of int
      |ocaml}
  ...
  let (session, system_time_impl_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Time__System_time"
      ~filename:"system_time.ml"
      ~text:{ocaml|
        type error =
          | System of int
      |ocaml}
  ...
  let (session, time_intf_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Time"
      ~filename:"time.mli"
      ~text:{ocaml|
  ...
  let (session, time_impl_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Time"
      ~filename:"time.ml"
...
let test_prepare_snapshot_planner_aliases_preserve_nested_constructor_owners = fun _ctx ->
  ...
  in
  let session = Session.empty ~config:Config.default in
  let (session, _read_dir_intf_source_id) =
    create_named_source
      session
      ~kind:Source.File
      ~module_name:"Kernel_new__Fs__Read_dir"
      ...
      |ocaml}
  in
  let (session, _read_dir_impl_source_id) =
    create_named_source
      session
      ~kind:Source.File
      ~module_name:"Kernel_new__Fs__Read_dir"
      ...
      |ocaml}
  in
  let (session, _fs_aliases_source_id) =
    create_named_source
      session
      ~kind:Source.Generated
      ~module_name:"Kernel_new__Fs__Aliases"
      ~filename:"Kernel_new__Fs__Aliases.ml-gen"
      ~implicit_opens:[]
      ~text:{ocaml|
  ...
  let (session, _fs_intf_source_id) =
    create_named_source
      session
      ~kind:Source.File
      ~module_name:"Kernel_new__Fs"
      ~filename:"fs.mli"
      ~implicit_opens:fs_implicit_opens
      ~text:{ocaml|
        module ReadDir = Read_dir
      |ocaml}
  in
  let (session, _fs_impl_source_id) =
    create_named_source
      session
      ~kind:Source.File
      ~module_name:"Kernel_new__Fs"
      ...
      |ocaml}
  in
  let (session, _root_aliases_source_id) =
    create_named_source
      session
      ~kind:Source.Generated
      ~module_name:"Kernel_new__Aliases"
      ~filename:"Kernel_new__Aliases.ml-gen"
      ~implicit_opens:[]
      ~text:{ocaml|
  ...
  let (session, error_source_id) =
    create_named_source
      ...
      ~module_name:"Kernel_new__Error"
      ~filename:"error.ml"
      ~implicit_opens:error_implicit_opens
      ~text:{ocaml|
        type t =
          | FsReadDir of Fs.ReadDir.error
  ...
  | Ok snapshot ->
      ...
      if List.is_empty diagnostics then
        ...
        let error_semantic_tree =
          ...
          | Some { SourceAnalysis.semantic_tree = Some semantic_tree; _ } ->
...
let test_prepare_snapshot_partial_wrapper_preserves_nested_module_exports = fun _ctx ->
  ...
  in
  let session = Session.empty ~config:Config.default in
  let (session, _non_zero_int_intf_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Async__Non_zero_int"
      ~filename:"non_zero_int.mli"
      ~text:{ocaml|
        type t = int
      |ocaml}
  in
  let (session, _non_zero_int_impl_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Async__Non_zero_int"
      ~filename:"non_zero_int.ml"
      ~text:{ocaml|
        type t = int
      |ocaml}
  in
  let (session, _interest_intf_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Async__Interest"
      ~filename:"interest.mli"
      ~text:{ocaml|
      ...
      |ocaml}
  in
  let (session, _interest_impl_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Async__Interest"
      ~filename:"interest.ml"
      ~text:{ocaml|
        type t = Non_zero_int.t
      ...
      |ocaml}
  in
  let (session, _adapter_unix_intf_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Async__Adapter__Unix"
      ~filename:"adapter_unix.mli"
      ~text:{ocaml|
        type error =
          | Oops
      ...
      |ocaml}
  in
  let (session, _adapter_unix_impl_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Async__Adapter__Unix"
      ~filename:"adapter_unix.ml"
      ~text:{ocaml|
        type selector = int
      ...
      |ocaml}
  in
  let (session, _adapter_intf_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Async__Adapter"
      ~filename:"adapter.mli"
      ~text:{ocaml|
        type error =
          | Oops
      ...
      |ocaml}
  in
  let (session, _adapter_impl_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Async__Adapter"
      ~filename:"adapter.ml"
      ...
      |ocaml}
  in
  let (session, poll_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Async__Poll"
      ~filename:"poll.ml"
      ~text:{ocaml|
        type t = {
...
let test_prepare_snapshot_paired_module_alias_preserves_interface_shaped_sibling_types = fun _ctx ->
  ...
  let (session, _non_zero_int_intf_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "non_zero_int.mli")
      ~text:{ocaml|
        type t = int
      |ocaml}
  in
  let (session, _non_zero_int_impl_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "non_zero_int.ml")
      ~text:{ocaml|
        type t = int
      |ocaml}
  in
  let (session, _interest_intf_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "interest.mli")
      ~text:{ocaml|
        type t
  ...
  let (session, _interest_impl_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "interest.ml")
      ~text:{ocaml|
        type t = Non_zero_int.t
      ...
      |ocaml}
  in
  let (session, async_intf_source_id) =
    create_source
      session
      ~kind:Source.File
      ...
      |ocaml}
  in
  let (session, async_impl_source_id) =
    create_source
      session
      ~kind:Source.File
...
let test_prepare_snapshot_query_order_preserves_in_progress_wrapper_exports = fun _ctx ->
  ...
  let (session, _ip_addr_impl_source_id) =
    create_named_source
      ...
      |ocaml}
  in
  let (session, _ip_addr_unix_intf_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Net__Ip_addr__Unix"
      ~filename:"ip_addr_unix.mli"
      ...
      |ocaml}
  in
  let (session, _ip_addr_unix_impl_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Net__Ip_addr__Unix"
      ~filename:"ip_addr_unix.ml"
...
let test_prepare_snapshot_net_wrapper_graph_preserves_ip_addr_exports = fun _ctx ->
  ...
  | Ok snapshot ->
      ...
      let ip_addr_impl_exports =
        export_names (Query.export_of snapshot ip_addr_impl_source_id)
        |> String.concat ", "
      in
      let ip_addr_intf_exports =
        export_names (Query.export_of snapshot ip_addr_intf_source_id)
        |> String.concat ", "
      in
      let ip_addr_impl_diagnostics = diagnostic_strings snapshot ip_addr_impl_source_id in
      let ip_addr_intf_diagnostics = diagnostic_strings snapshot ip_addr_intf_source_id in
...
let test_implicit_opens_do_not_leak_into_module_exports = fun _ctx ->
  let config =
    Config.default
    |> Config.with_capture_traces ~capture_traces:true
  in
  let session = Session.empty ~config in
  let helper_text = "let twice x = x + x\n" in
  let helper_parse_result = Syn.parse ~filename:(Path.v "helper.ml") helper_text in
...
let test_snapshot_exports_interface_declarations = fun _ctx ->
  ...
  in
  let (session, source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "iface.mli") ~text:source
  in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
...
let test_snapshot_exports_interface_externals = fun _ctx ->
  ...
  let (session, source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "externals.mli") ~text:source
  in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
...
let test_snapshot_collects_module_typings = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _) =
    create_source session ~kind:Source.File ~origin:(Source.Label "alpha.ml") ~text:"let id x = x\n"
  in
  let (session, _) =
    create_source
      session
...
let test_query_module_typings_of_uses_canonical_root_typings = fun _ctx ->
  ...
  let canonical_json =
    ...
    | [ typings ] ->
        ModuleTypings.Json.to_json typings
        |> Data.Json.to_string
    | typings ->
        panic
          (format
...
let test_paired_modules_report_signature_inclusion_mismatches = fun _ctx ->
  ...
  in
  let impl_typings =
    Query.module_typings_of snapshot impl_source_id
    |> Option.expect ~msg:"missing impl typings"
  in
  let intf_typings =
    Query.module_typings_of snapshot intf_source_id
    |> Option.expect ~msg:"missing interface typings"
  in
...
let test_paired_modules_canonicalize_builtin_aliases_in_signature_inclusion = fun _ctx ->
  ...
  let (seed_session, string_alias_source_id) =
    create_source
      seed_session
      ~kind:Source.File
      ~origin:(Source.Label "string_alias.mli")
      ~text:{ocaml|
      type t = string
    |ocaml}
  in
  ...
  let config =
    Config.default
    |> Config.with_loaded_modules ~loaded_modules:[ loaded_string_alias ]
  in
  let session = Session.empty ~config in
  let (session, impl_source_id) =
    create_source
...
let test_paired_modules_accept_manifest_alias_value_usage_in_signature_inclusion = fun _ctx ->
  ...
  let (session, _non_zero_int_impl_source_id) =
    create_named_source
      ...
      |ocaml}
  in
  let (session, interest_impl_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Async__Interest"
      ~filename:"interest.ml"
      ...
      |ocaml}
  in
  let (session, interest_intf_source_id) =
    create_named_source
      session
      ~module_name:"Kernel_new__Async__Interest"
      ~filename:"interest.mli"
...
let test_manifest_option_aliases_canonicalize_during_inference = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, option_source_id) =
    create_source
      seed_session
      ~kind:Source.File
      ~origin:(Source.Label "option.ml")
      ~text:{ocaml|
    ...
    |ocaml}
  in
  let (seed_session, path_source_id) =
    create_source
      seed_session
      ~kind:Source.File
  ...
  let config =
    Config.default
    |> Config.with_loaded_modules ~loaded_modules:[ loaded_option; loaded_path ]
  in
  let session = Session.empty ~config in
  let (session, unix_source_id) =
    create_source
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        let () =
          Test.assert_equal ~expected:false ~actual:(has_signature_error snapshot unix_source_id)
        in
        let () =
          Test.assert_equal
            ~expected:true
            ~actual:(Option.is_some (export_scheme snapshot unix_source_id "home_dir"))
        in
        Ok ()

let test_manifest_aliases_canonicalize_across_modules = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, list_alias_source_id) =
    create_source
      seed_session
      ~kind:Source.File
      ~origin:(Source.Label "list_alias.ml")
      ~text:{ocaml|
  ...
  let config =
    Config.default
    |> Config.with_loaded_modules ~loaded_modules:[ loaded_list_alias ]
  in
  let session = Session.empty ~config in
  let (session, use_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "use.ml")
      ~text:{ocaml|
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        let () =
          Test.assert_equal
            ~expected:true
            ~actual:(Option.is_some (export_scheme snapshot use_source_id "wrap"))
        in
        let () =
          Test.assert_equal
            ~expected:true
            ~actual:(Option.is_some (export_scheme snapshot use_source_id "unwrap"))
        in
        Ok ()
...
let test_paired_modules_accept_option_manifest_aliases_in_signature_inclusion = fun _ctx ->
  ...
  let config =
    Config.default
    |> Config.with_loaded_modules ~loaded_modules:[ loaded_option ]
  in
  let session = Session.empty ~config in
  let (session, impl_source_id) =
    create_source
...
let test_paired_modules_allow_private_top_level_exception_helpers = fun _ctx ->
  ...
  let (session, impl_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "global0.ml")
      ~text:{ocaml|
  ...
  let (session, intf_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "global0.mli")
      ~text:{ocaml|
      val panic : string -> 'a
    |ocaml}
  in
...
let test_paired_modules_include_paired_sibling_exports_during_signature_inclusion = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _ops_impl_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "ops.ml")
    ...
    |ocaml}
  in
  let (session, _ops_intf_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "ops.mli")
      ~text:{ocaml|
      val abs : int -> int
...
let test_source_input_hash_ignores_source_id_and_revision = fun _ctx ->
  ...
  in
  let hash_a =
    Source.input_hash source_a
    |> Crypto.Digest.hex
  in
  let hash_b =
    Source.input_hash source_b
    |> Crypto.Digest.hex
  in
  let hash_c =
    Source.input_hash source_c
    |> Crypto.Digest.hex
  in
...
let test_source_input_hash_changes_with_implicit_opens = fun _ctx ->
  ...
  let source_with_opens =
    make_source_with_implicit_opens
      ...
      ~text:"let id x = x\n"
  in
  let hash_without_opens =
    Source.input_hash source_without_opens
    |> Crypto.Digest.hex
  in
  let hash_with_opens =
    Source.input_hash source_with_opens
    |> Crypto.Digest.hex
  in
...
let test_snapshot_uses_loaded_module_typings = fun _ctx ->
  ...
  let config =
    Config.default
    |> Config.with_loaded_modules ~loaded_modules:[ loaded_colors ]
  in
  let session = Session.empty ~config in
  let (session, demo_source_id) =
    create_source
...
let test_fold_package_sources_resolves_contextual_local_modules = fun _ctx ->
  ...
  | Ok result ->
        ...
        |> List.find_opt
          (fun (group: Check.finished_group) ->
            String.equal
              ...
              "Kernel_new__Async__Source")
        |> Option.expect ~msg:"expected Async.Source group"
      in
      let source_analysis =
        source_group.checked_sources
        |> List.find_opt
          (fun (checked_source: Check.checked_source) ->
            Path.to_string checked_source.path = "async/source.ml")
      ...
      in
      let export_names =
        FileSummary.exports source_analysis.file_summary
        |> List.map (fun (name, _scheme) -> SurfacePath.to_string name)
      in
      ...
      let consumer_group =
        ...
        |> List.find_opt
          (fun (group: Check.finished_group) ->
            String.equal
              (LocalModules.InternalName.to_string group.module_name)
              "Kernel_new__Consumer")
        |> Option.expect ~msg:"expected Kernel_new__Consumer group"
      in
      let consumer_analysis =
        consumer_group.checked_sources
        |> List.find_opt
          (fun (checked_source: Check.checked_source) ->
            Path.to_string checked_source.path = "consumer.ml")
        |> Option.map (fun (checked_source: Check.checked_source) -> checked_source.analysis)
        |> Option.expect ~msg:"expected consumer analysis"
      in
      let export_names =
        FileSummary.exports consumer_analysis.file_summary
        |> List.map (fun (name, _scheme) -> SurfacePath.to_string name)
      in
      Test.assert_equal
        ~expected:[ "Kernel_new__Async"; "Kernel_new__Consumer" ]
        ~actual:module_order;
      Test.assert_equal
        ~expected:[]
...
|ocaml}
  ...
  | Error Check.MissingRequirements { requirements; _ } ->
      let actual =
        Session.MissingRequirements.to_json requirements
        |> Data.Json.to_string
      in
      let expected =
        Data.Json.Array [
  ...
  | Error Check.MissingModuleTypings { module_name } ->
      Error ("unexpected missing module typings for "
      ^ LocalModules.InternalName.to_string module_name)
  | Error Check.MissingAnalysis { module_name; path } ->
      Error (format
        Format.[
...
|ocaml}
  ...
  | Ok result ->
      ...
      let export_scheme name =
        lookup_export name exports
        |> Option.map TypePrinter.scheme_to_string
      in
...
let test_prepare_snapshot_shares_imported_world_semantics_for_open_alias_include = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _helpers_source_id) =
    create_source
      session
      ~kind:Source.File
...
|ocaml}
  ...
  if not (List.is_empty diagnostics) then
    ...
    let () =
      Test.assert_equal
        ~expected:(Some "int")
        ~actual:(export_scheme snapshot consumer_source_id "nested_value")
    in
    let () =
      Test.assert_equal
        ~expected:(Some "int")
        ~actual:(export_scheme snapshot consumer_source_id "from_alias")
    in
    let () =
      Test.assert_equal
        ~expected:(Some "int")
        ~actual:(export_scheme snapshot consumer_source_id "from_ctor")
    in
    let () =
      Test.assert_equal
        ~expected:(Some "int")
        ~actual:(export_scheme snapshot consumer_source_id "from_record")
    in
    let () =
      Test.assert_equal
        ~expected:(Some "int")
        ~actual:(export_scheme snapshot consumer_source_id "from_include")
    in
    let () =
      Test.assert_equal
        ~expected:(Some "int")
        ~actual:(export_scheme snapshot consumer_source_id "via_local_open")
    in
    Ok ()
    ...
    (fun store ->
      ...
      | Ok result -> (
          ...
          | Some bundle ->
              ...
              in
              let returned_modules =
                result.public_module_typings
...
let test_fold_package_sources_keeps_base_loaded_modules_immutable = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, dep_source_id) =
    create_source
      seed_session
      ~kind:Source.File
...
let test_prepare_snapshot_emits_structured_events = fun _ctx ->
  ...
  | Ok snapshot ->
      let _ =
        Query.analysis_of_source snapshot source_id
        |> Option.expect ~msg:"expected analysis"
      in
      if not (typ_event_instants_are_monotonic !events) then
        ...
        match !events with
        | [ { Event.kind = Event.PrepareSnapshotStarted _; _ }; { Event.kind = Event.PrepareSnapshotFinished _; _ }; { Event.kind = Event.SnapshotMaterializationStarted _; _ }; { Event.kind = Event.SnapshotMaterializationFinished _; _ }; { Event.kind = Event.ModulePairingStarted _; _ }; { Event.kind = Event.SourceAnalysisStarted { source_id = analysis_source_id; module_name; mode = Event.SnapshotAnalysis; _ }; _ }; { Event.kind = Event.SourceAnalysisFinished {
          source_id = finished_source_id;
          module_name = finished_module_name;
          mode = Event.SnapshotAnalysis;
          export_status = Event.TrustedExport;
          _
        }; _ }; { Event.kind = Event.ModulePairingFinished {
...
let test_prepare_snapshot_emits_structured_diagnostics_in_events = fun _ctx ->
  ...
  | Ok snapshot ->
      ...
      in
      match !events with
      | [ { Event.kind = Event.PrepareSnapshotStarted _; _ }; { Event.kind = Event.PrepareSnapshotFinished _; _ }; { Event.kind = Event.ModulePairingStarted _; _ }; { Event.kind = Event.SourceAnalysisStarted _; _ }; { Event.kind = Event.SourceAnalysisFinished {
        source_id = finished_source_id;
        module_name;
        mode = Event.SnapshotAnalysis;
        parse_diagnostics = [];
        ...
        _
      }; _ }; { Event.kind = Event.ModulePairingFinished _; _ } ] when SourceId.equal
        finished_source_id
        source_id
      && String.equal module_name "Demo" -> Ok ()
      | _ ->
          Error (format
            Format.[
...
let test_prepare_snapshot_keeps_imported_value_payloads_out_of_ambient_bindings = fun _ctx ->
  ...
  in
  let (session, _helpers_source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "helpers.ml") ~text:helpers_text
  in
  ...
  in
  let (session, consumer_source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "consumer.ml") ~text:consumer_text
  in
  ...
  | Ok snapshot ->
      ...
      | _ ->
          Error (format
            Format.[
              str
                "expected consumer snapshot analysis to avoid imported ambient value payloads, got ";
              str (typ_events_json !events);
            ])
...
let test_prepare_snapshot_only_pairs_required_local_modules = fun _ctx ->
  ...
  in
  let session = Session.empty ~config in
  let (session, _dep_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "dep.ml")
      ~text:{ocaml|
      let value = 1
    |ocaml}
  in
  let (session, _unused_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "unused.ml")
      ~text:{ocaml|
      let hidden = 2
    |ocaml}
  in
  ...
  | Ok snapshot ->
      let _ =
        Query.analysis_of_source snapshot app_source_id
        |> Option.expect ~msg:"expected analysis"
      in
      let paired_modules =
        !events
  ...
  | Ok snapshot ->
        ...
        |> List.fold_left
          (fun counts (event: Event.t) ->
            ...
            | Event.SourceAnalysisStarted { module_name; mode = Event.SnapshotAnalysis; _ } ->
                let count =
                  List.assoc_opt module_name counts
      ...
      if not (repeated_modules = []) then
        Error (format
          Format.[
            str
              "expected shared local modules to be analyzed and paired once, got repeated work for ";
...
let test_prepare_snapshot_reuses_paired_local_modules_across_rooted_snapshots = fun _ctx ->
  ...
  let (session, fs_impl_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "fs.ml")
      ~text:{ocaml|
      let read = File.read
    |ocaml}
  in
  let (session, left_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "left.ml")
  ...
  let run_root source_id =
    ...
    | Error missing ->
        Error (
          "expected rooted snapshot, got "
          ^ (
            Session.MissingRequirements.to_json missing
            |> Data.Json.to_string
          )
        )
    | Ok snapshot ->
  ...
  | Ok () -> (
      ...
      | Ok () ->
          ...
          let count_for_source counts source_id =
            List.assoc_opt (SourceId.to_int source_id) counts
            |> Option.unwrap_or ~default:0
          in
          ...
          if not (List.is_empty repeated_sources) || not (List.is_empty repeated_modules) then
            Error (format
              Format.[
                str
                  "expected paired local dependencies to be reused across rooted snapshots, got repeated source analyses [";
    ...
    (fun store ->
      ...
      | Ok _snapshot ->
          ...
          if not (actual = expected) then
            Error (
              "unexpected hydration event order: "
              ^ (
                Data.Json.Array (
                  !events
                  |> List.map Event.to_json
...
let test_prepare_snapshot_missing_requirements_emit_structured_events = fun _ctx ->
  ...
  | Error _missing ->
      ...
      | _ ->
          Error (
            "unexpected missing-requirements events: "
            ^ (
              Data.Json.Array (
                !events
                |> List.map Event.to_json
...
let test_prepare_snapshot_missing_requirements_through_include_do_not_trigger_nested_typing = fun _ctx ->
  ...
  | Error _missing ->
      ...
      if List.equal String.equal expected actual then
        ...
        Error (
          "expected include-driven missing discovery to avoid nested typing work: "
          ^ (
            Data.Json.Array (
              !events
              |> List.map Event.to_json
...
let test_prepare_snapshot_discovers_local_nested_module_dependencies = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _provider_source_id) =
    create_source
      session
      ~kind:Source.File
...
let test_prepare_snapshot_reports_match_coverage_diagnostics = fun _ctx ->
  ...
  in
  let (session, source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "match_coverage.ml") ~text:source
  in
  ...
  with_typ_store
    (fun store ->
      ...
      let config =
        Config.default
        |> Config.with_store ~store:(Some store)
        |> Config.with_loaded_modules ~loaded_modules:[ good_colors ]
      in
      let session = Session.empty ~config in
      let (session, demo_source_id) =
        create_source
...
let test_snapshot_uses_sibling_source_record_types = fun _ctx ->
  ...
  in
  let (session, demo_source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "blend_demo.ml") ~text:source
  in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot demo_source_id in
  if not (List.is_empty diagnostics) then
    ...
    in
    let type_decl_names =
      ModuleTypings.type_decls seed_summary
    ...
    let field_access_offset =
      let access_start =
        ...
        |> Option.expect ~msg:"expected record field access in test source"
      in
...
let test_snapshot_uses_loaded_module_record_types = fun _ctx ->
  ...
  in
  let (session, source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "consumer.ml") ~text:source
  in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
...
let test_include_reexports_loaded_module_record_types = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, helpers_source_id) =
    create_source
      seed_session
      ~kind:Source.File
      ~origin:(Source.Label "helpers.ml")
  ...
  let consumer_config =
    Config.default
    |> Config.with_loaded_modules ~loaded_modules:[ loaded_helpers ]
  in
  let consumer_session = Session.empty ~config:consumer_config in
  let (consumer_session, consumer_source_id) =
    create_source
      consumer_session
      ~kind:Source.File
      ~origin:(Source.Label "consumer.ml")
  ...
  if not (List.is_empty consumer_diagnostics) then
    ...
    let exported_type_decls =
      ModuleTypings.type_decls consumer_summary
      |> List.map
        (fun (type_decl: FileSummary.type_decl) -> (
          SurfacePath.to_segments type_decl.scope_path,
          type_decl.declaration.type_name
        ))
    in
    let client_config =
      Config.default
      |> Config.with_loaded_modules ~loaded_modules:[ consumer_summary ]
    in
    let client_session = Session.empty ~config:client_config in
    let client_source = "let origin = { x = 0; y = 0 }\nlet total point = point.x + point.y\n" in
    let (client_session, client_source_id) =
      create_source
        client_session
        ~kind:Source.File
...
let test_include_reexports_package_shaped_loaded_module_typings = fun _ctx ->
  ...
  let (kernel_seed_session, _result_impl_source_id) =
    create_source
      kernel_seed_session
      ~kind:Source.File
      ~origin:(Source.Label "result.ml")
      ~text:{ocaml|
let map f x = f x
|ocaml}
  in
  let (kernel_seed_session, _result_intf_source_id) =
    create_source
      kernel_seed_session
      ~kind:Source.File
      ~origin:(Source.Label "result.mli")
      ~text:{ocaml|
val map : ('a -> 'b) -> 'a -> 'b
|ocaml}
  in
  let (kernel_seed_session, _kernel_impl_source_id) =
    create_source
      kernel_seed_session
      ~kind:Source.File
...
|ocaml}
  in
  let (kernel_seed_session, _kernel_intf_source_id) =
    create_source
      kernel_seed_session
      ~kind:Source.File
...
let test_include_module_type_of_canonicalizes_loaded_nominal_types = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, actors_source_id) =
    create_source
      seed_session
      ~kind:Source.File
...
let test_include_module_type_of_loaded_modules_canonicalizes_nominal_types = fun _ctx ->
  ...
  let config =
    Config.default
    |> Config.with_loaded_modules ~loaded_modules:[ loaded_actors ]
  in
  let session = Session.empty ~config in
  let source =
    {ocaml|
  ...
  in
  let (session, source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "process.mli") ~text:source
  in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
...
let test_loaded_module_reexports_canonicalize_dependency_result_aliases = fun _ctx ->
  ...
  let (seed_session, actors_source_id) =
    create_source
      ...
      ~text:"module Process: sig\n  type exit_reason = exn\nend\n"
  in
  let (seed_session, kernel_source_id) =
    create_source
      seed_session
      ~kind:Source.File
      ~origin:(Source.Label "kernel.mli")
...
|ocaml}
  in
  let (seed_session, _global_source_id) =
    create_source
      seed_session
      ~kind:Source.File
  ...
  in
  let (seed_session, std_source_id) =
    create_source
      seed_session
      ~kind:Source.File
  ...
  let config =
    Config.default
    |> Config.with_loaded_modules ~loaded_modules:[ loaded_std; loaded_kernel; loaded_actors ]
  in
  let session = Session.empty ~config in
  let source =
    {ocaml|
...
|ocaml}
  in
  let (session, source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "colors.ml") ~text:source
  in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
...
let test_operator_pattern_bindings_lower_and_typecheck = fun _ctx ->
  ...
  let (session, source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "ops_aliases.ml")
      ~text:{ocaml|
  ...
  if not (List.is_empty diagnostics) then
    ...
    let exported_names =
      export_names (Query.export_of snapshot source_id)
      |> List.sort String.compare
    in
    let required_exports = [ "="; "+"; "|>"; "same"; "sum"; "piped"; ] in
    match required_exports
    |> List.find_opt (fun name -> not (List.mem name exported_names)) with
    | Some missing_export ->
        Error ("missing expected export "
        ^ missing_export
        ^ " from "
    ...
    | None ->
        let () =
          Test.assert_equal
            ~expected:(Some "bool")
            ~actual:(export_scheme snapshot source_id "same")
        in
        let () =
          Test.assert_equal ~expected:(Some "int") ~actual:(export_scheme snapshot source_id "sum")
        in
        let () =
          Test.assert_equal
            ~expected:(Some "int")
            ~actual:(export_scheme snapshot source_id "piped")
        in
        Ok ()
...
let test_cons_patterns_lower_and_typecheck = fun _ctx ->
  ...
  let (session, source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "cons_patterns.ml")
      ~text:{ocaml|
...
let test_recursive_operator_bindings_typecheck = fun _ctx ->
  ...
  let (session, source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "recursive_operator_bindings.ml")
      ~text:{ocaml|
      let rec ( @ ) left right =
        match left with
        | [] -> right
...
let test_language_prelude_supports_angle_not_equal = fun _ctx ->
  ...
  let (session, source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "prelude_ops.ml")
      ~text:{ocaml|
      let different = 1 <> 2
    |ocaml}
  in
  ...
  if not (List.is_empty diagnostics) then
    ...
    | None ->
        let () =
          Test.assert_equal
            ~expected:(Some "bool")
            ~actual:(export_scheme snapshot source_id "different")
        in
        Ok ()
...
let test_kernel_ops_surface_typechecks = fun _ctx ->
  ...
  let (session, source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "kernel_ops.ml")
      ~text:{ocaml|
      external equal : 'a -> 'a -> bool = "%equal"
      let ( = ) = equal
...
let test_opened_sibling_double_underscore_exports_typecheck = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _global0_impl_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "global0.ml")
    ...
    |ocaml}
  in
  let (session, _global0_intf_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "global0.mli")
    ...
    |ocaml}
  in
  let (session, env_impl_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "env.ml")
    ...
    |ocaml}
  in
  let (session, env_intf_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "env.mli")
  ...
  let snapshot = Session.snapshot session in
  let diagnostics =
    diagnostic_strings snapshot env_impl_source_id @ diagnostic_strings snapshot env_intf_source_id
  in
  if not (List.is_empty diagnostics) then
    Error (String.concat "\n" diagnostics)
  else
    let () =
      Test.assert_equal ~expected:false ~actual:(has_signature_error snapshot env_impl_source_id)
    in
    let () =
      Test.assert_equal ~expected:false ~actual:(has_signature_error snapshot env_intf_source_id)
    in
    let () =
      Test.assert_equal
        ~expected:(Some "string -> string option")
        ~actual:(export_scheme snapshot env_impl_source_id "getenv")
    in
    let () =
      Test.assert_equal
        ~expected:(Some "string -> string")
        ~actual:(export_scheme snapshot env_impl_source_id "getenv_exn")
    in
    let () =
      Test.assert_equal
        ~expected:(Some "string -> string -> unit")
        ~actual:(export_scheme snapshot env_impl_source_id "putenv")
    in
    Ok ()
...
let test_source_analysis_with_opened_loaded_module_canonicalizes_nominal_types = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, common_source_id) =
    create_source
      seed_session
      ~kind:Source.File
...
let test_snapshot_exports_opened_loaded_nominal_types_from_implementation = fun _ctx ->
  ...
  let config =
    Config.default
    |> Config.with_loaded_modules ~loaded_modules:[ loaded_common ]
  in
  let session = Session.empty ~config in
  let (session, source_id) =
    create_source
...
let test_prepare_snapshot_type_decls_use_opened_loaded_nominal_types_with_underscored_module_name = fun _ctx ->
  ...
  let config =
    Config.default
    |> Config.with_loaded_modules ~loaded_modules:[ loaded_parser ]
  in
  let session = Session.empty ~config in
  let (session, reader_source_id) =
    create_source
...
let test_loaded_module_typings_preserve_nested_same_named_alias_exports = fun _ctx ->
  let seed_session = Session.empty ~config:Config.default in
  let (seed_session, _cell_source_id) =
    create_source
      seed_session
      ~kind:Source.File
      ~origin:(Source.Label "cell.ml")
      ~text:"let create value = value\n"
  in
  let (seed_session, _sync_source_id) =
    create_source
      seed_session
      ~kind:Source.File
      ~origin:(Source.Label "sync.ml")
  ...
  let client_config =
    Config.default
    |> Config.with_loaded_modules ~loaded_modules:[ std_summary ]
  in
  let client_session = Session.empty ~config:client_config in
  let (client_session, client_source_id) =
    create_source
...
let test_paired_loaded_module_typings_preserve_nested_alias_exports_across_include_chain = fun _ctx ->
  ...
  let (kernel_seed_session, _cell_impl_source_id) =
    create_source
      ...
      ~text:"let create value = value\n"
  in
  let (kernel_seed_session, _cell_intf_source_id) =
    create_source
      kernel_seed_session
      ~kind:Source.File
      ...
      ~text:"val create : 'a -> 'a\n"
  in
  let (kernel_seed_session, _sync_impl_source_id) =
    create_source
      kernel_seed_session
      ~kind:Source.File
      ~origin:(Source.Label "sync.ml")
      ~text:"module Cell = Cell\n"
  in
  let (kernel_seed_session, _sync_intf_source_id) =
    create_source
      kernel_seed_session
      ~kind:Source.File
      ...
      ~text:"module Cell = Cell\n"
  in
  let (kernel_seed_session, kernel_source_id) =
    create_source
      kernel_seed_session
      ~kind:Source.File
      ~origin:(Source.Label "kernel.ml")
  ...
  if not (List.equal String.equal kernel_exported_names [ "Sync.Cell.create" ]) then
    ...
    let std_seed_config =
      Config.default
      |> Config.with_loaded_modules ~loaded_modules:[ kernel_summary ]
    in
    let std_seed_session = Session.empty ~config:std_seed_config in
    let (std_seed_session, _sync_impl_source_id) =
      create_source
        std_seed_session
        ~kind:Source.File
        ~origin:(Source.Label "sync.ml")
    ...
    in
    let (std_seed_session, _sync_intf_source_id) =
      create_source
        std_seed_session
        ~kind:Source.File
        ~origin:(Source.Label "sync.mli")
    ...
    in
    let (std_seed_session, std_source_id) =
      create_source
        std_seed_session
        ~kind:Source.File
        ~origin:(Source.Label "std.ml")
        ~text:"module Sync = Sync\n"
    in
    let (std_seed_session, _std_intf_source_id) =
      create_source
        std_seed_session
        ~kind:Source.File
        ~origin:(Source.Label "std.mli")
    ...
    if not (List.equal String.equal std_exported_names [ "Sync.Cell.create" ]) then
      ...
      | Ok client_snapshot ->
          ...
          if not (List.is_empty client_diagnostics) then
            ...
            let callee_offset =
              offset_of_substring "open Std.Sync\nlet answer = Cell.create 1\n" "Cell.create"
              |> Option.expect ~msg:"expected Cell.create in client source"
            in
            let answer_type = export_scheme client_snapshot client_source_id "answer" in
...
|ocaml}
  in
  let (kernel_seed_session, kernel_impl_source_id) =
    create_source
      kernel_seed_session
      ~kind:Source.File
      ~origin:(Source.Label "kernel.ml")
...
|ocaml}
  ...
  let (std_session, _sync_impl_source_id) =
    create_source
      std_session
      ~kind:Source.File
      ~origin:(Source.Label "sync.ml")
...
|ocaml}
  in
  let (std_session, _sync_intf_source_id) =
    create_source
      std_session
      ~kind:Source.File
      ~origin:(Source.Label "sync.mli")
...
|ocaml}
  in
  let (std_session, consumer_source_id) =
    create_source
      std_session
      ~kind:Source.File
      ~origin:(Source.Label "consumer.ml")
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        let answer_type = export_scheme snapshot consumer_source_id "answer" in
        let callee_offset =
          offset_of_substring consumer_source "Cell.create"
          |> Option.expect ~msg:"expected Cell.create in consumer source"
        in
...
let test_sibling_source_uses_loaded_module_record_reexport = fun _ctx ->
  ...
  |ocaml}
  in
  let (session, _consumer_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "consumer.ml")
  ...
  if not (List.is_empty diagnostics) then
    ...
    let field_access_offset =
      let access_start =
        offset_of_substring client_source "Consumer.origin.x +"
        |> Option.expect ~msg:"expected record field access in test source"
      in
...
let test_prepare_snapshot_collects_transitive_missing_modules = fun _ctx ->
  ...
  match Session.prepare_snapshot session ~roots:[ source_id ] with
  | Ok _ ->
      Error "expected rooted snapshot preparation to report transitive missing module summaries"
  | Error missing ->
      let actual =
        Session.MissingRequirements.to_json missing
        |> Data.Json.to_string
...
let test_prepare_snapshot_collects_missing_module_for_qualified_reference = fun _ctx ->
  ...
  match Session.prepare_snapshot session ~roots:[ source_id ] with
  | Ok _ ->
      Error "expected rooted snapshot preparation to report missing module summaries for qualified access"
  | Error missing ->
      let actual =
        Session.MissingRequirements.to_json missing
        |> Data.Json.to_string
...
let test_prepare_snapshot_keeps_nested_sibling_modules_out_of_top_level_requirements = fun _ctx ->
  ...
  match Session.prepare_snapshot session ~roots:[ source_id ] with
  | Ok _ ->
      Error "expected rooted snapshot preparation to keep sibling nested modules out of top-level module requirements"
  | Error missing ->
      let actual =
        Session.MissingRequirements.to_json missing
        |> Data.Json.to_string
...
let test_prepare_snapshot_keeps_loaded_nested_module_exports_out_of_top_level_requirements = fun _ctx ->
  ...
  match Session.prepare_snapshot session ~roots:[ source_id ] with
  | Ok _ ->
      Error "expected rooted snapshot preparation to keep loaded nested module exports out of top-level module requirements"
  | Error missing ->
      let actual =
        Session.MissingRequirements.to_json missing
        |> Data.Json.to_string
...
let test_prepare_snapshot_uses_local_prelude_variant_constructors = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, _prelude_source_id) =
    create_source
      session
      ~kind:Source.File
...
let test_prepare_snapshot_types_mutable_record_field_assignment = fun _ctx ->
  ...
  let (session, source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "counter.ml")
      ~text:{ocaml|
      type counter = {
        mutable value: int;
      }
...
let test_prepare_snapshot_types_local_source_module_pack = fun _ctx ->
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty diagnostics) then
        ...
        if
          to_source_type = Some "t -> Async.Source.t"
          || to_source_type = Some "int -> Async.Source.t"
        then
          Ok ()
        else
...
let test_check_source_recovers_when_snapshot_preparation_reports_missing_module_summaries = fun _ctx ->
  ...
  in
  let diagnostics =
    List.length report.parse_diagnostics
    + List.length report.lowering_diagnostics
    + List.length report.typing_diagnostics
  in
...
let test_match_guards_typecheck_in_pattern_scope = fun _ctx ->
  ...
  in
  let (session, source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "match_guard.ml") ~text:source
  in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    ...
    let guard_binding_offset =
      offset_of_substring source "n > 0"
      |> Option.expect ~msg:"expected match guard in test source"
    in
    let classify_type = export_scheme snapshot source_id "classify" in
...
let test_optional_arguments_can_be_omitted_and_reordered = fun _ctx ->
  ...
  in
  let (session, source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "optional_apply.ml") ~text:source
  in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    ...
    let reordered_offset =
      ...
      |> Option.expect ~msg:"expected reordered call in test source"
    in
    let explicit_offset =
      offset_of_substring source "make_key ~kind:5 ~mods:6 7"
      |> Option.expect ~msg:"expected explicit call in test source"
    in
...
let test_records_flow_through_snapshot_queries = fun _ctx ->
  ...
  in
  let (session, source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "records.ml") ~text:source
  in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
...
let test_fun_cases_keep_preceding_parameters = fun _ctx ->
  ...
  if not (List.is_empty diagnostics) then
    ...
    let delta_offset =
      offset_of_substring source "base + delta"
      |> Option.expect ~msg:"expected labeled parameter reference in test source"
      |> fun start -> start + String.length "base + "
    in
    ...
    let () =
      Test.assert_equal ~expected:(Some "int -> ~delta:int -> bool -> int") ~actual:choose_type
    in
...
|ocaml}
  in
  let (session, source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "for_loops.ml") ~text:source
  in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
    ...
    if loop_type = Some "int -> unit" && countdown_type = Some "int -> unit" then
      ...
      Error ("unexpected loop types: loop="
      ^ show_option loop_type
      ^ ", countdown="
...
|ocaml}
  in
  let (session, source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "if_sequence.ml") ~text:source
  in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
...
|ocaml}
  in
  let (session, source_id) =
    create_source session ~kind:Source.File ~origin:(Source.Label "let_operator.ml") ~text:source
  in
  let snapshot = Session.snapshot session in
  let diagnostics = diagnostic_strings snapshot source_id in
  if not (List.is_empty diagnostics) then
...
|ocaml}
  ...
  if not (List.is_empty diagnostics) then
    ...
    if
      value_type = Some "t" && token_type = Some "t -> int" && is_error_type = Some "t -> bool"
    then
      Ok ()
    else
...
let test_prepare_snapshot_paired_module_preserves_first_class_module_value_signatures = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, event_intf_source_id) =
    create_source
      session
      ~kind:Source.File
    ...
    |ocaml}
  in
  let (session, event_impl_source_id) =
    create_source
      session
      ~kind:Source.File
...
let test_prepare_snapshot_paired_module_allows_hidden_manifest_alias_with_same_variant_shape = fun _ctx ->
  ...
  let (session, adapter_intf_source_id) =
    ...
    |ocaml}
  in
  let (session, adapter_impl_source_id) =
    create_source
      session
      ~kind:Source.File
  ...
  | Ok snapshot ->
      ...
      if List.is_empty unix_impl_diagnostics then
        ...
        if open_file_type = Some "Path.t -> t" then
          ...
          Error ("unexpected file open_file type after errored include: "
          ^ show_option open_file_type)

let test_prepare_snapshot_errored_wrapper_preserves_persisted_named_type_ids = fun _ctx ->
  let session = Session.empty ~config:Config.default in
  let (session, source_intf_source_id) =
    create_source
      session
      ~kind:Source.File
    ...
    |ocaml}
  in
  let (session, source_impl_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "source.ml")
  ...
  | Ok snapshot ->
      ...
      if not (List.is_empty file_intf_diagnostics) then
        ...
        if to_source_type = Some "t -> Async.Source.t" then
          ...
          Error ("unexpected file to_source type after errored include: "
          ^ show_option to_source_type)
...
let test_loaded_module_selector_agent_shape_typechecks = fun _ctx ->
  ...
  in
  let session = Session.empty ~config in
  let (session, _type_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "type.ml")
      ~text:{ocaml|
type ('a, 'b) eq =
  | Equal : ('a, 'a) eq
|ocaml}
  in
  let (session, _ref_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "ref.ml")
      ~text:{ocaml|
type 'a t = int
...
|ocaml}
  in
  let (session, _pid_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "pid.ml")
      ~text:{ocaml|
type t = unit
|ocaml}
  in
  let (session, _message_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "message.ml")
      ~text:{ocaml|
type t = ..
|ocaml}
  in
  let (session, _global_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "global.ml")
      ~text:{ocaml|
type ('a, 'e) result =
  | Ok of 'a
  | Error of 'e
...
|ocaml}
  in
  let (session, agent_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "agent.ml")
      ~text:{ocaml|
open Global
...
let test_selector_alias_chain_across_loaded_modules_typechecks = fun _ctx ->
  ...
  in
  let session = Session.empty ~config in
  let (session, _actors_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "actors.ml")
      ~text:{ocaml|
type ('a, 'e) result =
  | Ok of 'a
  | Error of 'e
...
|ocaml}
  in
  let (session, _int64_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "int64.ml")
      ~text:{ocaml|
type t = int64
...
|ocaml}
  ...
  | Ok snapshot ->
      ...
      if List.is_empty diagnostics then
        ...
        if
          make_type = Some "'a. unit -> 'a t"
          && type_equal_type = Some "'a 'b. 'a t -> 'b t -> ('a, 'b) Type.eq option"
        then
          Ok ()
        else
          Error (format
...
let test_extensible_variant_constructors_lower_and_typecheck = fun _ctx ->
  ...
  let (session, intf_source_id) =
    create_source
      session
      ~kind:Source.File
      ~origin:(Source.Label "message.mli")
      ~text:{ocaml|
      type message = ..
      type message += Ack | Data of int
      ...
      if not (List.is_empty impl_diagnostics) then
        ...
        let () =
          Test.assert_equal
            ...
            ~actual:(export_scheme snapshot impl_source_id "ack")
        in
        let () =
          Test.assert_equal
            ~expected:(Some "message -> int")
            ~actual:(export_scheme snapshot impl_source_id "classify")
        in
        Ok ()
...
let main ~args =
  let tests = [
    Test.case
      "local module aliases include public wrapper spellings"
      test_local_module_aliases_include_public_wrapper_spellings;
    Test.case
      "contextual local module depth prefers deeper shared prefix"
      test_contextual_local_module_depth_prefers_deeper_shared_prefix;
    Test.case
      "contextual local module depth keeps single segment suffixes"
      test_contextual_local_module_depth_keeps_single_segment_suffixes;
    Test.case
      "local module implicit opens skip enclosing alias wrappers"
      test_local_module_implicit_opens_skip_enclosing_alias_wrappers;
    Test.case "source id stays stable across updates" test_source_id_stays_stable_across_updates;
    Test.case
      "snapshots remain immutable after updates"
      test_snapshots_remain_immutable_after_updates;
    Test.case
      "type_at uses smallest indexed expression"
      test_type_at_uses_smallest_indexed_expression;
    Test.case "definition_at uses local pattern origin" test_definition_at_uses_local_pattern_origin;
    Test.case
      "definition_at uses exported module typings"
      test_definition_at_uses_exported_module_typings;
    Test.case
      "definition_at prefers interface export origin"
      test_definition_at_prefers_interface_export_origin;
    ...
    Test.case
      "prepare_snapshot resolves internal module dependencies by local alias"
      test_prepare_snapshot_resolves_internal_module_dependencies_by_local_alias;
    Test.case
    ...
    Test.case
      "fold_package_sources resolves contextual local modules"
      test_fold_package_sources_resolves_contextual_local_modules;
    Test.case
      "fold_package_sources reports local module cycles"
      test_fold_package_sources_reports_local_module_cycles;
    Test.case
      "fold_package_sources resolves root local module wrappers"
      test_fold_package_sources_resolves_root_local_module_wrappers;
    Test.case
      "fold_package_sources shares imported world semantics for open alias include"
      test_fold_package_sources_shares_imported_world_semantics_for_open_alias_include;
    Test.case
      "fold_package_sources persists package bundle"
      test_fold_package_sources_persists_package_bundle;
    Test.case
      "fold_package_sources keeps base loaded modules immutable"
      test_fold_package_sources_keeps_base_loaded_modules_immutable;
    Test.case
      "prepare_snapshot shares imported world semantics for open alias include"
      test_prepare_snapshot_shares_imported_world_semantics_for_open_alias_include;
    Test.case
      "prepare_snapshot nested unix submodule sees sibling ip_addr exports"
      test_prepare_snapshot_nested_unix_submodule_sees_sibling_ip_addr_exports;
    Test.case
      "prepare_snapshot wrapper module reexports unix exports to sibling modules"
      test_prepare_snapshot_wrapper_module_reexports_unix_exports_to_sibling_modules;
    Test.case
      "prepare_snapshot wrapper module preserves same-path nominal value types"
      test_prepare_snapshot_wrapper_module_preserves_same_path_nominal_value_types;
    Test.case
      "prepare_snapshot wrapper module preserves local result error surface"
      test_prepare_snapshot_wrapper_module_preserves_local_result_error_surface;
    Test.case
      "prepare_snapshot partial wrapper preserves nested module exports"
      test_prepare_snapshot_partial_wrapper_preserves_nested_module_exports;
    Test.case
      "prepare_snapshot paired module alias preserves interface-shaped sibling types"
      test_prepare_snapshot_paired_module_alias_preserves_interface_shaped_sibling_types;
    Test.case
      "prepare_snapshot nested module alias canonicalizes sibling error types"
      test_prepare_snapshot_nested_module_alias_canonicalizes_sibling_error_types;
    Test.case
      "prepare_snapshot kernel named wrapper alias preserves nested error types"
      test_prepare_snapshot_kernel_named_wrapper_alias_preserves_nested_error_types;
    Test.case
      "prepare_snapshot multiple wrapper aliases preserve nested error types"
      test_prepare_snapshot_multiple_wrapper_aliases_preserve_nested_error_types;
    Test.case
      "prepare_snapshot nested include wrapper alias preserves error types"
      test_prepare_snapshot_nested_include_wrapper_alias_preserves_error_types;
    Test.case
      "prepare_snapshot planner aliases preserve nested constructor owners"
      test_prepare_snapshot_planner_aliases_preserve_nested_constructor_owners;
    Test.case
      "prepare_snapshot query order preserves in-progress wrapper exports"
      test_prepare_snapshot_query_order_preserves_in_progress_wrapper_exports;
    Test.case
      "prepare_snapshot net wrapper graph preserves ip_addr exports"
      test_prepare_snapshot_net_wrapper_graph_preserves_ip_addr_exports;
    Test.case
      "prepare_snapshot net wrapper graph preserves ip_addr exports with paired unix"
      test_prepare_snapshot_net_wrapper_graph_preserves_ip_addr_exports_with_paired_unix;
    Test.case
    ...
    Test.case
      "paired modules export interface-shaped file summaries"
      test_paired_modules_export_interface_shaped_file_summaries;
    Test.case
      "paired modules report signature inclusion mismatches"
      test_paired_modules_report_signature_inclusion_mismatches;
    Test.case
      "paired modules skip signature inclusion for errored implementation"
      test_paired_modules_skip_signature_inclusion_for_errored_implementation;
    Test.case
      "paired modules skip signature inclusion for unsupported interface types"
      test_paired_modules_skip_signature_inclusion_for_unsupported_interface_types;
    Test.case
      "paired modules accept manifest alias specialization"
      test_paired_modules_accept_manifest_alias_specialization;
    Test.case
      "paired modules canonicalize builtin aliases in signature inclusion"
      test_paired_modules_canonicalize_builtin_aliases_in_signature_inclusion;
    Test.case
      "paired modules accept manifest alias value usage in signature inclusion"
      test_paired_modules_accept_manifest_alias_value_usage_in_signature_inclusion;
    Test.case
      "manifest option aliases canonicalize during inference"
      test_manifest_option_aliases_canonicalize_during_inference;
    Test.case
      "manifest aliases canonicalize across modules"
      test_manifest_aliases_canonicalize_across_modules;
    Test.case
      "paired modules accept option manifest aliases in signature inclusion"
      test_paired_modules_accept_option_manifest_aliases_in_signature_inclusion;
    Test.case
      "source input hash ignores source id and revision"
      test_source_input_hash_ignores_source_id_and_revision;
    Test.case
      "source input hash ignores comments and docstrings"
      test_source_input_hash_ignores_comments_and_docstrings;
    Test.case
      "source input hash changes with implicit opens"
      test_source_input_hash_changes_with_implicit_opens;
    ...
    Test.case
      "prepare_snapshot emits structured events"
      test_prepare_snapshot_emits_structured_events;
    Test.case
      "prepare_snapshot emits structured diagnostics in events"
      test_prepare_snapshot_emits_structured_diagnostics_in_events;
    Test.case
    ...
    Test.case
      "prepare_snapshot only pairs required local modules"
      test_prepare_snapshot_only_pairs_required_local_modules;
    Test.case
    ...
    Test.case
      "prepare_snapshot reports match coverage diagnostics"
      test_prepare_snapshot_reports_match_coverage_diagnostics;
    Test.case
    ...
    Test.case
      "snapshot uses sibling source record types"
      test_snapshot_uses_sibling_source_record_types;
    Test.case
      "snapshot uses loaded module record types"
      test_snapshot_uses_loaded_module_record_types;
    Test.case
      "include reexports loaded module record types"
      test_include_reexports_loaded_module_record_types;
    Test.case
      "module alias reexports loaded module record types"
      test_module_alias_reexports_loaded_module_record_types;
    Test.case "include reexports loaded module typings" test_include_reexports_loaded_module_typings;
    Test.case
      "include reexports package-shaped loaded module typings"
      test_include_reexports_package_shaped_loaded_module_typings;
    Test.case
    ...
    Test.case
      "operator pattern bindings lower and typecheck"
      test_operator_pattern_bindings_lower_and_typecheck;
    ...
    Test.case
      "opened sibling double underscore exports typecheck"
      test_opened_sibling_double_underscore_exports_typecheck;
    Test.case
      "paired modules allow private top-level exception helpers"
      test_paired_modules_allow_private_top_level_exception_helpers;
    Test.case
      "paired modules include sibling exports during signature inclusion"
      test_paired_modules_include_sibling_exports_during_signature_inclusion;
    Test.case
      "paired modules include paired sibling exports during signature inclusion"
      test_paired_modules_include_paired_sibling_exports_during_signature_inclusion;
    Test.case
      "nonrec same-name option alias prefers outer type"
      test_nonrec_same_name_option_alias_prefers_outer_type;
    Test.case
      "nonrec same-name result alias prefers outer type"
      test_nonrec_same_name_result_alias_prefers_outer_type;
    Test.case
    ...
    Test.case
      "module aliases reexport loaded module typings"
      test_module_alias_reexports_loaded_module_typings;
    Test.case
      "module aliases reexport same-named local modules"
      test_module_alias_reexports_same_named_local_modules;
    Test.case
      "loaded module typings preserve nested same-named alias exports"
      test_loaded_module_typings_preserve_nested_same_named_alias_exports;
    Test.case
    ...
    Test.case
      "prepare_snapshot uses nested exports from local include of loaded module"
      test_prepare_snapshot_uses_nested_exports_from_local_include_of_loaded_module;
    Test.case
      "sibling sources use loaded module record reexports"
      test_sibling_source_uses_loaded_module_record_reexport;
    ...
    Test.case
      "prepare_snapshot canonicalizes missing requirements"
      test_prepare_snapshot_canonicalizes_missing_requirements;
    Test.case
      "prepare_snapshot sorts missing modules by name"
      test_prepare_snapshot_sorts_missing_modules_by_name;
    Test.case
    ...
    Test.case
      "prepare_snapshot types mutable record field assignment"
      test_prepare_snapshot_types_mutable_record_field_assignment;
    Test.case
      "prepare_snapshot types local source module pack"
      test_prepare_snapshot_types_local_source_module_pack;
    Test.case
      "check_source recovers when rooted preparation reports missing module summaries"
      test_check_source_recovers_when_snapshot_preparation_reports_missing_module_summaries;
    ...
    Test.case
      "optional argument forwarding preserves option wrapper"
      test_optional_argument_forwarding_preserves_option_wrapper;
    Test.case
      "inline record constructor payloads use constructor owner"
      test_inline_record_constructor_payloads_use_constructor_owner;
    Test.case
      "explicit locally abstract let annotations are checked"
      test_explicit_locally_abstract_let_annotations_are_checked;
    Test.case "for loops lower and typecheck" test_for_loops_lower_and_typecheck;
    Test.case
      "if branches do not capture trailing sequences"
      test_if_branches_do_not_capture_trailing_sequences;
    Test.case "let operators lower and typecheck" test_let_operator_lower_and_typecheck;
    Test.case
      "external identity cast token shape typechecks"
      test_external_identity_cast_token_shape_typechecks;
    Test.case
      "first-class module pack and unpack typecheck"
      test_first_class_module_pack_and_unpack_typecheck;
    Test.case
      "first-class module existential event shape typechecks"
      test_first_class_module_existential_event_shape_typechecks;
    Test.case
    ...
    Test.case
      "GADT tuple constructor existentials typecheck"
      test_gadt_tuple_constructor_existentials_typecheck;
    Test.case
      "selector receive with existential constructors typechecks"
      test_selector_receive_with_existential_constructors_typecheck;
    Test.case
      "extensible selector agent shape typechecks"
      test_extensible_selector_agent_shape_typechecks;
    Test.case
      "loaded module selector agent shape typechecks"
      test_loaded_module_selector_agent_shape_typechecks;
    Test.case
      "snapshot exposes loaded module agent support exports"
      test_snapshot_exposes_loaded_module_agent_support_exports;
    Test.case
      "deps collect loaded module agent dependencies"
      test_deps_collect_loaded_module_agent_dependencies;
    Test.case
      "selector alias chain across loaded modules typechecks"
      test_selector_alias_chain_across_loaded_modules_typechecks;
    Test.case
    ...
    Test.case
      "spawn alias chain across loaded modules typechecks"
      test_spawn_alias_chain_across_loaded_modules_typechecks;
    Test.case
      "function body annotation alias chain exports canonical result"
      test_function_body_annotation_alias_chain_exports_canonical_result;
    Test.case
      "extensible variant constructors lower and typecheck"
      test_extensible_variant_constructors_lower_and_typecheck;
    Test.case "expansive bindings stay monomorphic" test_expansive_bindings_stay_monomorphic;
    Test.case
      "nonexpansive list bindings still generalize"
      test_nonexpansive_list_bindings_still_generalize;
    Test.case
      "expansive covariant lists still generalize"
      test_expansive_covariant_lists_still_generalize;
    Test.case
    ...
    Test.case
      "fun cases keep preceding parameters in scope"
      test_fun_cases_keep_preceding_parameters;
