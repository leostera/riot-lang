...
let make_state = fun kind ->
  { kind; next_binding_stamp = 0; items = []; exports = []; diagnostics = [] }
...
let path_of_ident = fun ident ->
  ident |> Cst.Ident.segments |> List.map Cst.Token.text

let name_of_name_tokens = fun tokens ->
  tokens |> List.map Cst.Token.text |> String.concat ""

let lower_arrow_label = fun (label: Cst.arrow_label) ->
  {
    Semantic_tree.name = Cst.ArrowLabel.name label;
    optional_ = Cst.ArrowLabel.is_optional label;
  }

let push_diagnostic = fun state diagnostic ->
  state.diagnostics <- diagnostic :: state.diagnostics

let push_unsupported_type = fun state syntax_node summary ->
  push_diagnostic
    state
    (Diagnostics.Diagnostic.UnsupportedType
       {
...
let rec lower_type_expr = fun state (type_: Cst.core_type) ->
  match type_ with
  | Cst.CoreType.Wildcard _ -> Semantic_tree.AnyType
...
let rec annotation_of_expression = fun state (expression: Cst.expression) ->
  ...
  | Cst.Expression.TypeAscription { kind = Cst.Type { type_; _ }; _ } ->
      Some (lower_type_expr state type_)
  | Cst.Expression.TypeAscription { kind = Cst.Coerce { type_; _ }; _ } ->
      Some (lower_type_expr state type_)
  | Cst.Expression.TypeAscription { kind = Cst.ConstraintCoerce { to_type; _ }; _ } ->
      Some (lower_type_expr state to_type)
  | Cst.Expression.Polymorphic { type_; _ } ->
      Some (lower_type_expr state type_)
  | Cst.Expression.Parenthesized { inner; _ } ->
      annotation_of_expression state inner
  | _ -> None
      ...
      (Semantic_tree.TypeDeclaration
         {
           ...
           name;
           params =
             Cst.TypeDeclaration.type_params declaration
             |> List.filter_map
                  (fun type_parameter ->
           ...
           manifest = (
             ...
             | None ->
                 ...
                 | Cst.TypeDefinition.Alias { manifest; _ } ->
                     Some (lower_type_expr state manifest)
                 | _ -> None
           );
    ...
    (Semantic_tree.ExceptionDeclaration
       {
         ...
         rhs =
           ...
           | Some (Cst.Alias { alias; _ }) ->
               Some (Semantic_tree.ExceptionAlias (path_of_ident alias))
           | Some (Cst.Payload { payload_type; _ }) ->
               Some (Semantic_tree.ExceptionPayload (lower_type_expr state payload_type))
           | None -> None;
       })
...
let lower_structure_item = fun state ->
  ...
  | Cst.StructureItem.Expression expression ->
      push_item
        state
        (Semantic_tree.Expression
           { span = span_of_syntax_node (Cst.Expression.syntax_node expression) })
  | Cst.StructureItem.Attribute attribute ->
      push_unsupported state attribute.syntax_node "attribute"
  | Cst.StructureItem.Extension extension ->
      push_unsupported state extension.syntax_node "extension"
  | Cst.StructureItem.ClassDeclaration declaration ->
      push_unsupported state (Cst.ClassDefinition.syntax_node declaration) "class declaration"
  | Cst.StructureItem.ClassTypeDeclaration declaration ->
      push_unsupported state declaration.syntax_node "class type declaration"
  | Cst.StructureItem.TypeExtension declaration ->
      push_unsupported state (Cst.TypeExtension.syntax_node declaration) "type extension"
...
let lower_signature_item = fun state ->
  ...
  | Cst.SignatureItem.ClassDeclaration declaration ->
      push_unsupported state (Cst.ClassDeclaration.syntax_node declaration) "class declaration"
...
let lower_source_file = fun ~source:_ source_file ->
  ...
  let () =
    ...
    | Cst.Implementation implementation ->
        implementation.items |> List.iter (lower_structure_item state)
    | Cst.Interface interface ->
        interface.items |> List.iter (lower_signature_item state)
  in
  {
