...
let normalize_cycle = fun ~module_names ~source_ids ->
  (module_names |> List.sort_uniq String.compare, source_ids |> List.sort_uniq compare_source_ids)
...
let of_list = fun requirements ->
  let rec loop roots modules cycles = function
            ...
            (fun (left_modules, left_source_ids) (right_modules, right_source_ids) ->
              ...
              if Int.equal modules_compare 0 then
                compare_string_lists
                  (left_source_ids |> List.map SourceId.to_string)
                  (right_source_ids |> List.map SourceId.to_string)
              else
                modules_compare)
          |> List.map
    ...
    | MissingRootSource { source_id } :: tail ->
        loop (source_id :: roots) modules cycles tail
    | MissingModuleSummary { module_name; requested_by } :: tail ->
        loop roots (add_module_requirement modules module_name requested_by) cycles tail
...
let requirement_to_json = fun value ->
  ...
  | LocalModuleCycle { module_names; source_ids } -> Data.Json.Object [
    ("tag", Data.Json.String "local_module_cycle");
    (
      "module_names",
      Data.Json.Array (module_names |> List.map (fun module_name -> Data.Json.String module_name))
    );
    (
      "source_ids",
