...
let rec add_export = fun scope (path, scheme) ->
  ...
  | Some (name, tail) ->
      if SurfacePath.is_empty tail then
        ...
        {
          scope
          with modules = upsert_module
            scope.modules
            ~name
            ~f:(fun child -> add_export child (tail, scheme))
        }
...
let rec add_type_decl = fun scope (type_decl: FileSummary.type_decl) ->
  ...
  | Some (name, tail) -> {
    ...
    with modules = upsert_module
      scope.modules
      ~name
      ~f:(fun child -> add_type_decl child { type_decl with scope_path = tail })
  }
...
let rec exports = fun scope ->
  let local_exports = scope.values
  |> List.map (fun (name, scheme) -> (SurfacePath.of_name name, scheme)) in
  let nested_exports = scope.modules
  |> List.concat_map
    (fun { name; scope } ->
...
let rec type_decls = fun scope ->
  let nested_type_decls = scope.modules
  |> List.concat_map
    (fun { name; scope } ->
      type_decls scope
      |> List.map
        (fun (type_decl: FileSummary.type_decl) ->
          { type_decl with scope_path = SurfacePath.prepend_name name type_decl.scope_path })) in
...
let qualify_constructor_entry = fun ~root (entry: constructor_entry) ->
  if SurfacePath.is_empty root then
    ...
    { entry with owner_path = SurfacePath.append_path root entry.owner_path }
      ...
      lookup_module scope module_path |> Option.map
        (fun module_scope ->
          local_constructor_entries module_scope |> List.filter
            (fun entry ->
              String.equal entry.constructor.name name) |> List.map
            (qualify_constructor_entry ~root:module_path)) |> Option.unwrap_or ~default:[]
