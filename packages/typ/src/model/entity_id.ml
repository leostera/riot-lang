...
let resolved = fun ~binding_id ~surface_path ->
  { binding_id; surface_path }

let of_binding_id = fun binding_id ->
  {
    binding_id;
    surface_path = Binding_id.name binding_id;
  }

let binding_id = fun value ->
  value.binding_id

let surface_path = fun value ->
  value.surface_path
  ...
  Serde.Ser.record
    (Serde.Ser.fields
       [
         ...
         Serde.Ser.field "surface_path" Surface_path.serializer (fun (value : t) -> value.surface_path);
       ])
