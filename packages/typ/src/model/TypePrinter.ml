...
let rec visit_children = fun state ty ->
  ...
  | TypeRepr.Tuple arguments ->
      List.iter (discover_recursive_aliases state) arguments
  ...
  | TypeRepr.PolyVariant { tags; inherited; _ } ->
      List.iter
        (fun (tag: TypeRepr.poly_variant_tag) ->
          tag.payload_type |> Option.iter (discover_recursive_aliases state))
        tags;
  ...
  | TypeRepr.Var { link=Some linked; _ } ->
      discover_recursive_aliases state linked
...
let rec render_type = fun state nested active ty ->
  ...
  let key = order_for_ty state ty in
  match Collections.HashMap.get state.aliases key with
  | Some alias when List.mem key active ->
      alias
  | Some alias when Collections.HashSet.contains state.rendered_aliases key ->
      alias
  | alias ->
...
and render_type_body = fun state nested active ty ->
  ...
  | TypeRepr.PolyVariant { bound; tags; inherited } ->
      ...
      @ (inherited |> List.map (fun inherited_ty -> render_type state false active inherited_ty)) in
  ...
  | TypeRepr.Arrow { label; lhs; rhs } ->
      ...
      ^ " -> "
      ^ render_type state false active rhs in
      if nested then
        "(" ^ text ^ ")"
      else
        text
