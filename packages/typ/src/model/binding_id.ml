...
type t = { stamp: int; name: Surface_path.t }
...
let equal = fun left right -> Int.equal left.stamp right.stamp && Surface_path.equal left.name right.name
...
let serializer =
  Serde.Ser.record
    (
      Serde.Ser.fields
        [
          Serde.Ser.field "stamp" Serde.Ser.int
            (
              fun (value: t) -> value.stamp
            );
          Serde.Ser.field "name" Surface_path.serializer
            (
              fun (value: t) -> value.name
            );
        ]
    )
