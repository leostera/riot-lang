...
let rec equal = fun left right ->
  ...
  | Qualified (left_name, left_tail), Qualified (right_name, right_tail) -> String.equal left_name right_name && equal left_tail right_tail
  | _ -> false
...
let serializer = Serde.Ser.contramap to_segments (Serde.Ser.contramap Array.from_list (Serde.Ser.array Serde.Ser.string))
