...
let solve_var var type_ =
  match resolve type_ with
  | Type.Var other when Type.same_var var other ->
      Ok ()
  | resolved when occurs_in var resolved ->
      Error.solver_found_infinite_type ~var ~type_
  | resolved ->
...
let rec unify ~expected ~actual =
  ...
  | expected, actual -> Error.type_mismatch ~expected ~actual
...
and unify_many expected actual =
  ...
  if Int.(expected_len != actual_len) then
    Error.type_mismatch ~expected:(Type.Tuple expected) ~actual:(Type.Tuple actual)
  else
    List.zip expected actual |> List.fold_left ~init:(Ok ())
...
and unify_arrow expected actual =
  if not (Type.Label.equal expected.label actual.label) then
    Error.type_mismatch ~expected:(Type.Arrow expected) ~actual:(Type.Arrow actual)
  else
...
and unify_constructors expected actual =
  ...
  if same_constructor && same_args_length then
    ...
    Error.type_mismatch ~expected:(Type.Constructor expected) ~actual:(Type.Constructor actual)
