...
let summary_open = fun summary module_path -> Summary2.open_ summary module_path
...
let summary_ident_of_ident = fun ident ->
  {
    Summary2.local_id = Binding.ident_local_id ident;
    name = Binding.ident_name ident;
  }
...
let summary_provenance_of_provenance = function
  ...
  | Binding.Type_constructor { type_name; scope_path } ->
      Summary2.Type_constructor { type_name; scope_path }
  | Binding.Exception { name; scope_path } ->
      Summary2.Exception { name; scope_path }
  | Binding.Declared_value { name; scope_path } ->
      Summary2.Declared_value { name; scope_path }
  | Binding.Included { module_path } ->
      Summary2.Included { module_path }
  | Binding.Module_alias { alias_name; module_path } ->
      Summary2.Module_alias { alias_name; module_path }
...
let rec insert_scope_at_path : module_table -> module_path:IdentPath.t -> module_scope -> module_table =
...
let of_bindings = fun bindings ->
  ...
  qualified_bindings |> List.fold_left
    ...
    {
      empty with
      summary = Summary2.snapshot ~bindings:(List.map summary_binding_of_binding bindings) ~type_decls:[];
...
let bindings = fun env ->
  Value_env.bindings env.values @ bindings_with_prefix IdentPath.empty env.modules

let rec scope_type_decls_with_prefix : IdentPath.t -> module_scope -> FileSummary.type_decl list =
 fun prefix scope ->
  let local =
...
and module_type_decls_with_prefix : IdentPath.t -> module_table -> FileSummary.type_decl list =
 fun prefix modules ->
  Name_map.bindings modules |> List.concat_map
    (fun (_, binding) ->
      let module_prefix =
  ...
  | Some scope ->
      {
        ...
        values =
          scope.values |> Value_env.bindings |> List.map
            (fun binding ->
              Binding.with_provenance (Binding.Included { module_path }) binding)
...
let export_names_for_module_alias = fun env ~alias_name ~module_path ->
  match lookup_module_scope env module_path with
  | Some scope ->
      scope_bindings_with_prefix (IdentPath.of_name alias_name) scope
      |> of_bindings
      |> canonical_bindings
