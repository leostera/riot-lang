...
let rec qualify_type = fun local_types module_name ty ->
  ...
  | TypeRepr.PolyVariant { bound; tags; inherited } ->
      ...
      if
        List.for_all2 Std.Ptr.equal tags qualified_tags && List.for_all2 Std.Ptr.equal inherited qualified_inherited
      then
        ty
      else
    ...
    (fun (type_decl: FileSummary.type_decl) ->
      ...
      |> List.map
        (fun (constructor: TypeDecl.constructor) ->
          {
            constructor
            with scheme = qualify_scheme_with_local_types local_types ~module_name constructor.scheme;
