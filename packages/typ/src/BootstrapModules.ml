...
let module_summary = fun ~source_id ~module_name exports ->
  let summary =
    FileSummary.trusted ~source_id exports |> PersistedSummary.of_file_summary
  in
  let source_hash =
    PersistedSummary.Json.to_json summary
    |> Data.Json.to_string
    |> fun json -> Crypto.hash_string ("typ-bootstrap-module\x1f" ^ module_name ^ "\x1f" ^ json)
  in
...
let runtime_args =
  TypeScheme.Forall ([], TypeRepr.Hole (-2))
  ...
  TypeScheme.Forall (
    ...
    arrow
      ...
      (arrow
        ...
        args
        (TypeRepr.Result (TypeRepr.Unit, exit_reason)))
      (arrow
        ~label:(TypeRepr.Labelled "args")
        args
...
let summaries =
  [
    ...
    ("String", [
      ...
      ("sub", monomorphic (arrow TypeRepr.String (arrow TypeRepr.Int (arrow TypeRepr.Int TypeRepr.String))));
      ...
      ("concat", monomorphic (arrow TypeRepr.String (arrow (TypeRepr.List TypeRepr.String) TypeRepr.String)));
      ("to_seq", monomorphic (arrow TypeRepr.String (TypeRepr.Seq TypeRepr.Char)));
      ("starts_with", monomorphic (arrow ~label:(TypeRepr.Labelled "prefix") TypeRepr.String (arrow TypeRepr.String TypeRepr.Bool)));
    ]);
    ("List", [
    ...
    ("Array", [
      ("length", monomorphic (arrow (TypeRepr.Array (TypeRepr.Hole (-1))) TypeRepr.Int));
    ]);
    ("Buffer", [
    ...
    ("Std", [
      ("println", monomorphic (arrow TypeRepr.String TypeRepr.Unit));
      ("Env.args", runtime_args);
    ]);
    ("Actors", [
      ("run", runtime_run);
    ]);
  ...
  |> List.mapi
    (fun index (module_name, exports) ->
      module_summary
        ~source_id:(SourceId.of_int (-1000 - index))
        ~module_name
        exports)
