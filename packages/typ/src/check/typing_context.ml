    ...
    Serde.Ser.run =
      (fun backend state value ->
        let serializer =
          Serde.Ser.variant
            [
              Serde.Ser.Variant.unit "Int" (fun value -> match value with Int -> true | _ -> false);
              Serde.Ser.Variant.unit "Bool" (fun value -> match value with Bool -> true | _ -> false);
              Serde.Ser.Variant.unit "Char" (fun value -> match value with Char -> true | _ -> false);
              Serde.Ser.Variant.unit "String" (fun value -> match value with String -> true | _ -> false);
              Serde.Ser.Variant.unit "Float" (fun value -> match value with Float -> true | _ -> false);
              Serde.Ser.Variant.unit "Unit" (fun value -> match value with Unit -> true | _ -> false);
              Serde.Ser.Variant.newtype "List" type_expr_serializer (fun value ->
                match value with
                | List value -> Some value
                | _ -> None);
              Serde.Ser.Variant.newtype "Option" type_expr_serializer (fun value ->
                match value with
                | Option value -> Some value
                | _ -> None);
              Serde.Ser.Variant.newtype "Tuple"
                (Serde.Ser.contramap Array.of_list (Serde.Ser.array type_expr_serializer))
                (fun value ->
                  match value with
                  | Tuple values -> Some values
                  | _ -> None);
              Serde.Ser.Variant.newtype "Arrow" function_type_serializer (fun value ->
                match value with
                | Arrow value -> Some value
                | _ -> None);
              Serde.Ser.Variant.newtype "Var" Serde.Ser.int (fun value ->
                match value with
