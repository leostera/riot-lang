...
let span_of_token_body = fun token ->
  ...
  ((if end_ >= width then end_ - width else 0), end_)
...
let span_of_node = fun node ->
  ...
  | Some first ->
      ...
      SynAst.Node.for_each_token node ~fn:(
        fun token ->
...
let origin_of_node = fun node ->
  { span = span_of_node node; kind = SynAst.Node.kind node }
...
let path_of_tokens = fun tokens -> tokens |> List.map ~fn:token_text |> String.concat "" |> SurfacePath.of_name
...
let rec build_core_type = fun type_expr ->
  ...
  let view =
    ...
    | SynAst.TypeExpr.Wildcard -> TypeWildcard
    | SynAst.TypeExpr.Var { name } -> TypeVar (Option.map name ~fn:token_text)
    | SynAst.TypeExpr.Path { path } -> TypePath (path_of_syn_path path)
    ...
    | SynAst.TypeExpr.Tuple _ -> TypeTuple (child_type_exprs type_expr)
    | SynAst.TypeExpr.Labeled { annotation; _ } -> TypeLabeled { annotation = Option.map annotation ~fn:build_core_type }
    | SynAst.TypeExpr.Poly { body } -> TypePoly { body = Option.map body ~fn:build_core_type }
    | SynAst.TypeExpr.Parenthesized { inner = Some inner } -> (build_core_type inner).view
    | SynAst.TypeExpr.Parenthesized { inner = None } -> TypeError "missing parenthesized type"
    ...
    | SynAst.TypeExpr.Error node -> TypeError (node_summary node)
    | SynAst.TypeExpr.Unknown node -> TypeUnsupported (node_summary node)
  in
  ({ origin; view }: core_type)

and child_type_exprs = fun type_expr ->
  let children = ref [] in
  SynAst.TypeExpr.for_each_child_type type_expr ~fn:(fun child -> children := build_core_type child :: !children);
  List.reverse !children
...
let rec build_parameter = fun parameter ->
  ...
  let view =
    ...
    | SynAst.Parameter.Labeled { label; pattern } ->
        Labeled { label = Option.map label ~fn:token_text; pattern = Option.map pattern ~fn:build_pattern }
    | SynAst.Parameter.Optional { label; pattern } ->
        Optional { label = Option.map label ~fn:token_text; pattern = Option.map pattern ~fn:build_pattern }
...
and build_pattern = fun pattern ->
  ...
  let view =
    ...
    | SynAst.Pattern.Literal { token } -> PatternLiteral (Option.map token ~fn:literal_of_token |> Option.unwrap_or ~default:Unknown)
    ...
    | SynAst.Pattern.Cons { head; tail } ->
        PatternCons { head = Option.map head ~fn:build_pattern; tail = Option.map tail ~fn:build_pattern }
    ...
    | SynAst.Pattern.Attribute { inner } -> PatternAttribute { inner = Option.map inner ~fn:build_pattern }
    ...
    | SynAst.Pattern.Array | SynAst.Pattern.Record | SynAst.Pattern.PolyVariant | SynAst.Pattern.Extension
    | SynAst.Pattern.LocalOpen | SynAst.Pattern.LocallyAbstractType | SynAst.Pattern.FirstClassModule
    | SynAst.Pattern.Interval _ | SynAst.Pattern.Or _ | SynAst.Pattern.Lazy _ | SynAst.Pattern.Exception _ ->
        PatternUnsupported (Syn.SyntaxKind.to_string origin.kind)
  in
  ({ origin; view }: pattern)

and build_let_binding = fun binding ->
  let parameters = ref [] in
  SynAst.LetBinding.for_each_parameter binding ~fn:(fun parameter -> parameters := build_pattern parameter :: !parameters);
  ({
...
and build_expr = fun expression ->
  ...
  let view =
    ...
    | SynAst.Expr.Literal { token } -> ExprLiteral (Option.map token ~fn:literal_of_token |> Option.unwrap_or ~default:Unknown)
    | SynAst.Expr.Path { path } -> ExprPath (path_of_syn_path path)
    | SynAst.Expr.Parenthesized { inner } -> ExprParenthesized { inner = Option.map inner ~fn:build_expr }
    | SynAst.Expr.Attribute { inner } -> ExprAttribute { inner = Option.map inner ~fn:build_expr }
    | SynAst.Expr.Typed { expr; annotation } ->
        ExprTyped { expr = Option.map expr ~fn:build_expr; annotation = Option.map annotation ~fn:build_core_type }
    ...
    | SynAst.Expr.Sequence { left; right } ->
        ExprSequence { left = Option.map left ~fn:build_expr; right = Option.map right ~fn:build_expr }
    ...
    | SynAst.Expr.Apply { callee; argument } ->
        ExprApply { callee = Option.map callee ~fn:build_expr; argument = Option.map argument ~fn:build_expr }
    ...
    | SynAst.Expr.Let { first_binding; body } ->
        ExprLet { first_binding = Option.map first_binding ~fn:build_let_binding; body = Option.map body ~fn:build_expr }
    | SynAst.Expr.Assert { argument } -> ExprAssert { argument = Option.map argument ~fn:build_expr }
    | SynAst.Expr.LabeledArg { label; value } ->
        ExprLabeledArg { label = Option.map label ~fn:token_text; value = Option.map value ~fn:build_expr }
    | SynAst.Expr.OptionalArg { label; value } ->
        ExprOptionalArg { label = Option.map label ~fn:token_text; value = Option.map value ~fn:build_expr }
    ...
    | SynAst.Expr.Array | SynAst.Expr.Record | SynAst.Expr.RecordUpdate | SynAst.Expr.Extension
    | SynAst.Expr.FirstClassModule | SynAst.Expr.LocalOpen _ | SynAst.Expr.LetModule _
    | SynAst.Expr.LetException _ | SynAst.Expr.BindingOperator _ | SynAst.Expr.Unreachable
    | SynAst.Expr.Object | SynAst.Expr.New | SynAst.Expr.Try _ | SynAst.Expr.While _ | SynAst.Expr.For _
    | SynAst.Expr.Lazy _ | SynAst.Expr.Assign _ | SynAst.Expr.MethodCall _ | SynAst.Expr.PolyVariant _
    | SynAst.Expr.ArrayIndex _ | SynAst.Expr.StringIndex _ ->
        ExprUnsupported (Syn.SyntaxKind.to_string origin.kind)
  in
  ({ origin; view }: expr)

let build_let_declaration = fun declaration ->
  let bindings = ref [] in
  SynAst.LetDeclaration.for_each_binding declaration ~fn:(fun binding -> bindings := build_let_binding binding :: !bindings);
  ({
...
let build_value_declaration = fun declaration ->
  ({
    origin = origin_of_node declaration;
    name = name_of_declaration_tokens (SynAst.ValueDeclaration.for_each_name_token declaration) (SynAst.ValueDeclaration.name declaration);
...
let build_structure_item = fun item ->
  ...
  let view =
    ...
    | Type declaration -> StructureUnsupported (Syn.SyntaxKind.to_string (SynAst.Node.kind declaration))
    | TypeExtension declaration -> StructureUnsupported (Syn.SyntaxKind.to_string (SynAst.Node.kind declaration))
    | Exception declaration -> StructureUnsupported (Syn.SyntaxKind.to_string (SynAst.Node.kind declaration))
    | Class declaration -> StructureUnsupported (Syn.SyntaxKind.to_string (SynAst.Node.kind declaration))
    | Attribute attribute -> StructureUnsupported (Syn.SyntaxKind.to_string (SynAst.Node.kind attribute))
    | Extension extension -> StructureUnsupported (Syn.SyntaxKind.to_string (SynAst.Node.kind extension))
    | Module _ | ModuleType _ | Open _ | Include _ -> StructureUnsupported (Syn.SyntaxKind.to_string origin.kind)
...
let build_signature_item = fun item ->
  ...
  let view =
    ...
    | Type declaration -> SignatureUnsupported (Syn.SyntaxKind.to_string (SynAst.Node.kind declaration))
    | TypeExtension declaration -> SignatureUnsupported (Syn.SyntaxKind.to_string (SynAst.Node.kind declaration))
    | Exception declaration -> SignatureUnsupported (Syn.SyntaxKind.to_string (SynAst.Node.kind declaration))
    | Class declaration -> SignatureUnsupported (Syn.SyntaxKind.to_string (SynAst.Node.kind declaration))
    | Attribute attribute -> SignatureUnsupported (Syn.SyntaxKind.to_string (SynAst.Node.kind attribute))
    | Extension extension -> SignatureUnsupported (Syn.SyntaxKind.to_string (SynAst.Node.kind extension))
    | Module _ | ModuleType _ | Open _ | Include _ -> SignatureUnsupported (Syn.SyntaxKind.to_string origin.kind)
...
let of_parse_result = fun ~source:_ (parse_result: Syn.Parser.parse_result) ->
  ...
  let view =
    ...
    | Implementation implementation ->
        let items = ref [] in
        SynAst.Implementation.for_each_item implementation ~fn:(fun item -> items := build_structure_item item :: !items);
        Implementation (List.reverse !items)
    | Interface interface ->
        let items = ref [] in
        SynAst.Interface.for_each_item interface ~fn:(fun item -> items := build_signature_item item :: !items);
