...
let has_infer_snapshot = fun path ->
  Fs.exists (infer_snapshot_path path) |> Result.unwrap_or ~default:false
...
let source_slice = fun source -> IO.IoVec.IoSlice.from_string source |> Result.expect ~msg:"failed to create infer test source slice"
...
let render_infer_diagnostics = fun (diagnostics: Typ.Diagnostics.t) ->
  diagnostics.items |> Vector.iter |> Iter.Iterator.to_list |> render_diagnostics

let render_interface = fun (intf: Typ.Infer.ModuleInterface.t) ->
  intf |> Typ.Infer.ModuleInterface.values |> Typ.SignatureGenerator.from_values
...
let infer_test = fun (ctx: Test.FixtureRunner.ctx) ->
  ...
  let* ast = Typ.Ast.from_parse_result ~source parse_result |> Result.map_err ~fn:render_diagnostics in
  let actual = Typ.Infer.check ast |> render_result in
