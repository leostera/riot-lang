...
let checker_test = fun (ctx: Test.FixtureRunner.ctx) ->
  ...
  let* json_text = Serde_json.to_string Typ.Check.Typings.serializer typings |> Result.map_err ~fn:Serde.Error.to_string
  in
...
let tests = Test.FixtureRunner.cases () ~dir:fixtures_dir ~filter:fixture_filter ~snapshot_path:(
  fun path -> Some (Path.add_extension path ~ext:"checker.expected")
) ~run:checker_test
