...
let rec find_workspace_root = fun dir ->
  ...
  | Ok false | Error _ -> (
    ...
    | _ -> panic (format Format.[ str "expected benchmark to run inside the Riot workspace, starting from "; str (Path.to_string dir) ])
  )

let workspace_root = fun () -> Env.current_dir () |> Result.expect ~msg:"expected current working directory for typ lowering bench" |> find_workspace_root
...
let prepared_source_of_path = fun ~source_id path ->
  let text = Fs.read path |> Result.expect ~msg:("expected benchmark fixture at " ^ Path.to_string path) in
  let parse_result = Syn.parse ~filename:path text in
  let cst = expect_cst ~filename:(Path.to_string path) parse_result in
  let implicit_opens = [] in Typ.Model.Source.make_prepared ~source_id:(Typ.Model.SourceId.of_int source_id) ~kind:Typ.Model.Source.File ~module_name:(Typ.Model.Source.infer_module_name (Typ.Model.Source.Path path)) ~implicit_opens ~origin:(Typ.Model.Source.Path path) ~revision:0 ~source_hash:(Typ.Model.Source.hash ~implicit_opens ~cst) ~parse_result ~cst
...
let prepared_hot_sources = fun workspace_root ->
  hot_paths workspace_root |> List.mapi
    (
      fun index path -> prepared_source_of_path ~source_id:index path
    )
...
let bench_lower_source = fun label source ->
  Bench.with_config ~config:bench_config label
    (
      fun () ->
        repeat repeat_lowerings
          (
            fun () -> lower_once source
          )
    )
...
let bench_lower_bundle = fun label sources ->
  Bench.with_config ~config:bench_config label
    (
      fun () ->
        repeat repeat_hot_bundle
          (
            fun () -> sources |> List.iter lower_once
          )
    )
...
let benchmark_suite = fun () ->
  ...
  let source_by_path path =
    prepared_hot_sources |> List.find
      (
  ...
  in
  Bench.[
    bench_lower_source "lower kernel-new/error.ml x1000" (source_by_path (Path.join workspace_root (Path.v "packages/kernel-new/src/error.ml")));
    bench_lower_source "lower kernel-new/process/unix.ml x1000" (source_by_path (Path.join workspace_root (Path.v "packages/kernel-new/src/process/unix.ml")));
    bench_lower_source "lower kernel-new/fs/file/file.ml x1000" (source_by_path (Path.join workspace_root (Path.v "packages/kernel-new/src/fs/file/file.ml")));
    bench_lower_source "lower kernel-new/net/tcp_stream/unix.ml x1000" (source_by_path (Path.join workspace_root (Path.v "packages/kernel-new/src/net/tcp_stream/unix.ml")));
    bench_lower_source "lower kernel-new/net/tcp_listener/unix.ml x1000" (source_by_path (Path.join workspace_root (Path.v "packages/kernel-new/src/net/tcp_listener/unix.ml")));
