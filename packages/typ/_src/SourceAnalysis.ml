...
let definition_site_of_origin_id = fun ~(source:Source.t) ~semantic_tree origin_id ->
  ...
  | Some (semantic_tree: SemanticTree.file) ->
      OriginMap.find semantic_tree.origin_map origin_id
      |> Option.map
        (fun (origin: OriginMap.origin) -> ({ origin = source.origin; span = origin.span }:
          ModuleTypings.definition_site))
...
let alias_target_path = fun ~alias_name ~module_path path ->
  ...
  let suffix =
    SurfacePath.strip_prefix ~prefix:alias_prefix path
    |> Option.unwrap_or ~default:path
  in
  SurfacePath.append_path module_path suffix

let definition_target_of_binding_ref_in_tree = fun
  ~(source:Source.t) ~semantic_tree (binding_ref: Check_result.binding_ref) ->
  ...
  | Check_result.ModuleAlias { alias_name; module_path } ->
      Some (ModuleTypings.Export (alias_target_path
        ~alias_name
        ~module_path
        binding_ref.surface_path))
  | Check_result.Prelude
  | Check_result.TypeConstructor _ -> None
...
let analyze = fun ?(imported_world = ImportedWorld.empty ()) ~config (source: Source.t) ->
  ...
  let value_definitions =
    export_definitions_of_bindings
      ~source
      ~semantic_tree:(Some semantic_tree)
      inferred.export_bindings
  in
  let type_index =
    if config.capture_traces then
      let traced_exprs =
        inferred.expr_traces
        |> List.map
  ...
  let has_errors =
    ...
    || has_error_diagnostics inferred.diagnostics
  in
