...
let compare_source_ids = fun left right -> Int.compare (SourceId.to_int left) (SourceId.to_int right)
...
let add_module_requirement = fun modules module_name requested_by ->
  ...
  | Some existing ->
      let merged_requested_by = normalize_requesters (existing @ requested_by) in (module_name, merged_requested_by) :: List.remove_assoc module_name modules

let normalize_cycle = fun ~module_names ~source_ids -> (module_names |> List.sort_uniq String.compare, source_ids |> List.sort_uniq compare_source_ids)

let of_list = fun requirements ->
  let rec loop roots modules cycles = function
    | [] ->
        let root_requirements =
          roots |> List.sort_uniq compare_source_ids |> List.map
            (
              fun source_id -> MissingRootSource { source_id }
            )
        in
        let module_requirements =
          modules |> List.sort
            ...
            (
              fun (module_name, requested_by) -> MissingModuleSummary { module_name; requested_by = normalize_requesters requested_by }
            )
        in
            ...
            (
              fun (left_modules, left_source_ids) (right_modules, right_source_ids) ->
                ...
                if Int.equal modules_compare 0 then
                  compare_string_lists (left_source_ids |> List.map SourceId.to_string) (right_source_ids |> List.map SourceId.to_string)
                else modules_compare
            ) |> List.map
            (
    ...
    | (MissingModuleSummary { module_name; requested_by }) :: tail -> loop roots (add_module_requirement modules module_name requested_by) cycles tail
    | (LocalModuleCycle { module_names; source_ids }) :: tail ->
        let cycle = normalize_cycle ~module_names ~source_ids in
        let cycles =
          if List.exists
            (
              fun existing -> existing = cycle
            )
            cycles then
            cycles
          else cycle :: cycles
...
let requirement_to_json = fun value ->
  ...
  | LocalModuleCycle { module_names; source_ids } ->
      Data.Json.Object [
        ...
        "module_names", Data.Json.Array (
          module_names |> List.map
            (
              fun module_name -> Data.Json.String module_name
            )
        );
        "source_ids", Data.Json.Array (
          source_ids |> List.map
            (
              fun source_id -> Data.Json.Int (SourceId.to_int source_id)
            )
