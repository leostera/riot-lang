...
let pretty_name = fun index ->
  let suffix =
    if index < 26 then
      ""
    else string_of_int (index / 26)
  in
  let letter = Char.chr (Char.code 'a' + (index mod 26)) in "'" ^ String.make 1 letter ^ suffix
...
let order_for_ty = fun state ty ->
  ...
  if Int.equal (TypeRepr.mark ty) state.generation then
    ...
    let order = state.next_order in state.next_order <- order + 1;
...
let alias_for_ty = fun state ty ->
  ...
  | None ->
      ...
      let _ = Collections.HashMap.insert state.aliases key alias in alias

let rec visit_children = fun state ty ->
  match TypeRepr.view (TypeRepr.prune ty) with
  | TypeRepr.Int | TypeRepr.Float | TypeRepr.Bool | TypeRepr.String | TypeRepr.Char | TypeRepr.Unit | TypeRepr.Hole _ | TypeRepr.Var { link = None; _ } -> ()
  | TypeRepr.Option element | TypeRepr.Array element | TypeRepr.List element | TypeRepr.Seq element -> discover_recursive_aliases state element
  | TypeRepr.Result (ok_ty, error_ty) ->
      discover_recursive_aliases state ok_ty;
      discover_recursive_aliases state error_ty
  | TypeRepr.Named { arguments; _ } | TypeRepr.Tuple arguments -> List.iter (discover_recursive_aliases state) arguments
  | TypeRepr.Package signature ->
      List.iter
        (
          fun (value: TypeRepr.package_value) -> discover_recursive_aliases state (TypeScheme.body value.scheme)
        )
        signature.values
  | TypeRepr.PolyVariant { tags; inherited; _ } ->
      List.iter
        (
          fun (tag: TypeRepr.poly_variant_tag) -> tag.payload_type |> Option.iter (discover_recursive_aliases state)
        )
        tags;
...
and discover_recursive_aliases = fun state ty ->
  ...
  | _ ->
      ...
      Collections.HashMap.insert state.states key 2 |> ignore
...
let rec render_type = fun state nested active ty ->
  ...
  | alias ->
      ...
      match alias with
      | Some alias ->
          Collections.HashSet.insert state.rendered_aliases key |> ignore;
...
and render_type_body = fun state nested active ty ->
  ...
  | TypeRepr.Result (ok_ty, error_ty) ->
      let text = "(" ^ render_type state false active ok_ty ^ ", " ^ render_type state false active error_ty ^ ") result" in
      if nested then
        "(" ^ text ^ ")"
      else text
  ...
  | TypeRepr.Package signature ->
      let values =
        signature.values |> List.map
          (
            fun (value: TypeRepr.package_value) -> "val " ^ value.name ^ " : " ^ render_scheme state active value.scheme
          ) |> String.concat "; "
      in
  ...
  | TypeRepr.Named { head = { name; _ }; arguments } -> (
    ...
    | arguments ->
        "(" ^ (
          arguments |> List.map
            (
              fun argument -> render_type state false active argument
            ) |> String.concat ", "
        ) ^ ") " ^ SurfacePath.to_string name
  )
  | TypeRepr.PolyVariant { bound; tags; inherited } ->
      let members =
        (
          tags |> List.map
            (
              fun tag -> render_poly_variant_tag state active tag
            )
        ) @ (
          inherited |> List.map
            (
              fun inherited_ty -> render_type state false active inherited_ty
            )
        )
      in
  ...
  | TypeRepr.Tuple members ->
      members |> List.map
        (
          fun member -> render_type state true active member
        ) |> String.concat " * " |> fun text ->
        if nested then
          "(" ^ text ^ ")"
        else text
  | TypeRepr.Arrow { label; lhs; rhs } ->
      let text = render_arrow_label label ^ render_arrow_argument_type state active lhs ^ " -> " ^ render_type state false active rhs in
      if nested then
        "(" ^ text ^ ")"
      else text
and render_arrow_argument_type = fun state active ty ->
  match TypeRepr.view (TypeRepr.prune ty) with
  | TypeRepr.Arrow _ | TypeRepr.Tuple _ | TypeRepr.PolyVariant _ -> "(" ^ render_type state false active ty ^ ")"
...
let scheme_to_string = fun scheme ->
  let state = make_render_state () in render_scheme state [] scheme
