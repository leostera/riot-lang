...
let name = function
  | Local { name; _ } | Predef { name; _ } -> name
  | Persistent path -> (
...
let stamp = function
  | Local { stamp; _ } | Predef { stamp; _ } -> Some stamp
  | Persistent _ -> None
...
let compare = fun left right ->
  ...
  | (Local _, _) -> (-1)
  | (_, Local _) -> 1
  | (Persistent left, Persistent right) -> SurfacePath.compare left right
  | (Persistent _, _) -> (-1)
  | (_, Persistent _) -> 1
...
let to_string = function
  ...
  | Persistent path -> SurfacePath.to_string path
  | Predef { stamp; name } -> format Format.[
    str "predef(";
    str name;
    char '#';
    int stamp;
