...
type match_witness =
  ...
  | ConstructorWitness of {
      name: string;
      arguments: match_witness list;
    }
...
type t =
  ...
  | ApplicationArgumentLoweredAsPositional of {
  ...
  | RedundantMatchCase of {
      case_span: Syn.Ceibo.Span.t;
    }
...
let rec match_witness_to_string = fun value ->
  ...
  | TupleWitness elements ->
      format
        Format.[
          str "(";
          str (String.concat ", " (List.map match_witness_to_string elements));
          str ")";
        ]
  ...
  | ConstructorWitness { name; arguments = [ argument ] } ->
      format Format.[ str name; str " "; str (match_witness_atom_to_string argument) ]
...
let message = fun value ->
  ...
  | TypeMismatch { mismatch = TupleArityMismatch {
    left;
    right;
    left_arity;
    right_arity
...
let unsupported_reason_to_json = fun value ->
  ...
  | LiteralOutsideSupportedSubset { supported_literals } ->
      Data.Json.Object [
        ...
        (
          "supported_literals",
          Data.Json.Array (List.map supported_literal_to_json supported_literals)
        );
      ]
...
let fields_to_json = fun value ->
  ...
  | RedundantMatchCase { case_span } -> [ ("case_span", span_to_json case_span); ]
