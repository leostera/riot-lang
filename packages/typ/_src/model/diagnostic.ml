...
let severity = fun value ->
  match value with
  | CstBuilderError _ | UnsupportedSyntax _ | IgnoredPatternTypeConstraint _ | IgnoredPolymorphicAnnotation _ | UnboundName _ | TypeMismatch _ | ApplicationLabelMismatch _ | RecordResolutionError _ | OrPatternBindingsMismatch _ | SignatureInclusionError _ | UnsupportedSemanticExpression _ | RecursiveGroupRequiresSimpleVariableBinders _ -> Error
  | ParameterLoweredAsPositional _ | ApplicationArgumentLoweredAsPositional _ | IgnoredTypeAscription _ | UnsupportedInterfaceFile _ | NonexhaustiveMatch _ | RedundantMatchCase _ -> Warning
...
let unsupported_reason_to_string = fun value ->
  match value with
  | LiteralOutsideSupportedSubset { supported_literals } -> format Format.[ str "literal kind is outside the currently supported subset (supported: "; str (String.concat ", " (List.map supported_literal_to_string supported_literals)); str ")" ]
...
let signature_mismatch_message = fun value ->
  match value with
  | MissingValue { name } -> format Format.[ str "signature inclusion failed: implementation does not export value "; str name ]
  | ValueTypeMismatch { name; expected; actual } -> format Format.[
  ...
  | MissingTypeDeclaration { name } -> format Format.[ str "signature inclusion failed: implementation does not export type "; str name ]
  | TypeDeclarationMismatch { name; expected; actual } -> format Format.[
...
let rec match_witness_to_string = fun value ->
  ...
  | UnitWitness -> "()"
  | TupleWitness elements -> format Format.[ str "("; str (String.concat ", " (List.map match_witness_to_string elements)); str ")" ]
  | ConstructorWitness { name = "::"; arguments = [ head; tail ] } -> format Format.[ str (match_witness_atom_to_string head); str " :: "; str (match_witness_atom_to_string tail) ]
  | ConstructorWitness { name; arguments = [] } -> name
  | ConstructorWitness { name; arguments = [ argument ] } -> format Format.[ str name; str " "; str (match_witness_atom_to_string argument) ]
  | ConstructorWitness { name; arguments } -> format Format.[
    str name;
    str " (";
...
let message = fun value ->
  match value with
  | CstBuilderError { builder_error } -> format Format.[ str "Syn.build_cst failed before lowering: "; str (builder_error.message) ]
  | UnsupportedSyntax { syntax_kind; context; recovery; reason = None; _ } -> format Format.[
    str "unsupported ";
  ...
  | UnsupportedSyntax { syntax_kind; context; recovery; reason = Some reason; _ } -> format Format.[
    str "unsupported ";
    str (unsupported_context_to_string context);
    str " lowered using ";
    str (unsupported_recovery_to_string recovery);
  ...
  | UnboundName { name; _ } -> format Format.[ str "unbound name: "; str name ]
  | TypeMismatch { mismatch = ExpectedActual { expected; actual }; _ } -> format Format.[
    str "type mismatch: expected ";
    str expected;
    str " but got ";
    str actual;
  ]
  | TypeMismatch { mismatch = TupleArityMismatch { left; right; left_arity; right_arity }; _ } -> format Format.[
  ...
  | TypeMismatch { mismatch = OccursCheckFailed { variable_id; in_type }; _ } -> format Format.[
    str "type mismatch: occurs check failed for type variable ";
    int variable_id;
    str " in ";
    str in_type;
  ]
  | ApplicationLabelMismatch { expected_label; actual_labels = []; _ } -> format Format.[ str "application is missing an argument for "; str (application_label_to_string expected_label) ]
  | ApplicationLabelMismatch { expected_label; actual_labels; _ } -> format Format.[
  ...
  | RecordResolutionError { context; reason; _ } -> (
    match reason with
    | UnknownRecordLabels labels -> format Format.[ str (record_context_to_string context); str " uses unknown record labels: "; str (render_record_labels labels) ]
    | AmbiguousRecordLabels labels -> format Format.[ str (record_context_to_string context); str " is ambiguous for record labels: "; str (render_record_labels labels) ]
    | MissingRecordFields labels -> format Format.[ str (record_context_to_string context); str " is missing record fields: "; str (render_record_labels labels) ]
    | IncompatibleRecordLabels labels -> format Format.[ str (record_context_to_string context); str " labels do not belong to a single record type: "; str (render_record_labels labels) ]
  )
  | OrPatternBindingsMismatch { expected_names; actual_names; _ } -> format Format.[
  ...
  | NonexhaustiveMatch { witness; _ } -> format Format.[ str "non-exhaustive match: missing case "; str (match_witness_to_string witness) ]
  | RedundantMatchCase _ -> "match case is redundant"
  | SignatureInclusionError { mismatch; _ } -> signature_mismatch_message mismatch
  | UnsupportedSemanticExpression { summary; _ } -> format Format.[ str "unsupported semantic expression reached inference: "; str summary ]
...
let record_resolution_reason_to_json = fun value ->
  ...
  | UnknownRecordLabels labels ->
      Data.Json.Object [
        ...
        "labels", Data.Json.Array (
          List.map
            (
              fun label -> Data.Json.String label
            )
            labels
        );
      ]
...
let to_string = fun diagnostic -> format Format.[
