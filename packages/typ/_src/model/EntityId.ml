...
let of_binding_id = fun binding_id -> resolved ~binding_id ~surface_path:(SurfacePath.of_name (BindingId.name binding_id))

let binding_id = function
  | Resolved { binding_id; _ } -> Some binding_id
  | Unresolved _ | Apply _ -> None

let rec surface_path = function
  | Unresolved surface_path | Resolved { surface_path; _ } -> surface_path
  | Apply (callee, argument) -> SurfacePath.of_string (to_string callee ^ "(" ^ to_string argument ^ ")")
...
let compare = fun left right ->
  ...
  | (Unresolved left, Unresolved right) -> SurfacePath.compare left right
  | (Unresolved _, _) -> (-1)
  | (_, Unresolved _) -> 1
...
let append_name = fun entity name -> of_surface_path (SurfacePath.append_name (surface_path entity) name)

let prepend_name = fun name entity -> with_surface_path (SurfacePath.prepend_name name (surface_path entity)) entity

let append_path = fun left right ->
  let right_segments = right |> surface_path |> SurfacePath.to_segments in right_segments |> List.fold_left append_name left

let qualify = fun ~prefix entity -> with_surface_path (SurfacePath.append_path prefix (surface_path entity)) entity

let last_name = fun entity -> surface_path entity |> SurfacePath.last_name

let uncons = fun entity ->
  entity |> surface_path |> SurfacePath.uncons |> Option.map
    (
      fun (name, tail) -> (name, of_surface_path tail)
    )

let split_last = fun entity ->
  entity |> surface_path |> SurfacePath.split_last |> Option.map
    (
      fun (prefix, name) -> (with_surface_path prefix entity, name)
    )

let strip_prefix = fun ~prefix entity ->
  entity |> surface_path |> SurfacePath.strip_prefix ~prefix |> Option.map
    (
      fun suffix -> with_surface_path suffix entity
    )

let prefixes = fun entity ->
  entity |> surface_path |> SurfacePath.prefixes |> List.map
    (
      fun prefix -> with_surface_path prefix entity
    )
