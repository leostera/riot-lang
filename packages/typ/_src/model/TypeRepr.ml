...
and var_kind =
  | Flexible
  | Rigid
and var = { id: int; kind: var_kind; mutable link: t option }
...
let max_level = fun xs ->
  List.fold_left
    (
      fun acc ty -> Int.max acc ty.level
    )
    0
    xs
...
let package_signature_max_level = fun (signature: package_signature) ->
  signature.values |> List.fold_left
    (
      fun acc (value: package_value) -> Int.max acc value.scheme.body.level
    )
    0

let level_of_desc = function
  | Int | Float | Bool | String | Char | Unit | Hole _ -> 0
  | Option element | Array element | List element | Seq element -> element.level
  | Result (ok_ty, error_ty) -> Int.max ok_ty.level error_ty.level
  | Named { arguments; _ } | Tuple arguments -> max_level arguments
...
let named_path = fun ~name ~arguments ->
  let head = named_head ~type_constructor_id:(TypeConstructorId.of_path name) ~name in named ~head ~arguments
...
let diff = fun left right ->
  if List.is_empty left || List.is_empty right then
    ...
    List.filter
      (
        fun value -> not (Collections.HashSet.contains right_values value)
      )
      left
  ...
  let rec collect generation seen acc ty =
    ...
    if Int.equal ty.walk_mark generation then
      ...
      (
        ...
        match ty.desc with
        | Int | Float | Bool | String | Char | Unit | Hole _ -> acc
        | Option element -> collect generation seen acc element
        | Result (ok_ty, error_ty) ->
            let acc = collect generation seen acc ok_ty in collect generation seen acc error_ty
        ...
        | Package signature ->
            List.fold_left
              (
                fun acc (value: package_value) -> collect generation seen acc value.scheme.body
              )
              acc
              signature.values
        | Named { arguments; _ } ->
            List.fold_left
              (
                fun acc argument -> collect generation seen acc argument
              )
              acc
              arguments
        | PolyVariant { tags; inherited; _ } ->
            ...
            List.fold_left
              (
                fun acc inherited_type -> collect generation seen acc inherited_type
              )
              acc
              inherited
        | Tuple members ->
            List.fold_left
              (
                fun acc member -> collect generation seen acc member
              )
              acc
              members
        | Arrow { lhs; rhs; _ } ->
            let acc = collect generation seen acc lhs in collect generation seen acc rhs
        | Var { id; link = None; _ } ->
    ...
    if Int.equal ty.walk_mark generation then
      ...
      (
        ...
        let child_level =
          match ty.desc with
          | Int | Float | Bool | String | Char | Unit | Hole _ -> 0
          | Option element | Array element | List element | Seq element -> loop generation element
          | Result (ok_ty, error_ty) -> Int.max (loop generation ok_ty) (loop generation error_ty)
          | Named { arguments; _ } | Tuple arguments ->
              List.fold_left
                (
  ...
  let rec loop generation generalized_ids ty =
    ...
    if not (Int.equal ty.walk_mark generation) then
      (
        ty.walk_mark <- generation;
        match ty.desc with
        | Int | Float | Bool | String | Char | Unit | Hole _ -> ()
...
let generic_var_ids =
  ...
  in
  fun ty -> collect (next_walk_generation ()) (Collections.HashSet.create ()) [] ty |> List.rev
    ...
    | ty :: rest ->
        ...
        if Int.equal ty.mark generation then
          ...
          (
            ...
            let rest =
              match ty.desc with
              | Int | Float | Bool | String | Char | Unit | Hole _ | Var _ -> rest
              | Option element | Array element | List element | Seq element -> element :: rest
              | Package signature ->
                  List.fold_right
                    (
              ...
              | PolyVariant { tags; inherited; _ } ->
                  let rest =
                    List.fold_right
                      (
                        fun inherited_type acc -> inherited_type :: acc
                      )
                      inherited
                      rest
                  in
...
let merge_variances = fun left right ->
  List.fold_left
    (
      fun acc (var_id, variance) -> add_variance acc var_id variance
    )
    left
    right
    ...
    if Int.equal ty.walk_mark generation then
      ...
      (
        ...
        | Tuple members ->
            List.fold_left
              (
                fun acc member -> merge_variances acc (loop variance member)
              )
              []
              members
        | Arrow { lhs; rhs; _ } -> merge_variances (loop (flip_variance variance) lhs) (loop variance rhs)
        | Var { id; link = None; _ } ->
            [
              id, variance;
            ]
  ...
  collect_variances Covariant ty |> List.filter_map
    (
      fun (var_id, variance) ->
        ...
        | Contravariant | Invariant -> None
    )

let rec occurs = fun needle ty ->
  let ty = prune ty in
  match ty.desc with
  | Int | Float | Bool | String | Char | Unit | Hole _ -> false
  ...
  let rec loop = function
    ...
    | ty :: rest ->
        ...
        if Int.equal (mark ty) generation then
          ...
          if ty.level < minimum_level then
            (
            ...
            (
              ...
              | Int | Float | Bool | String | Char | Unit | Hole _ | Var _ -> loop rest
              | Option element | Array element | List element | Seq element -> loop (element :: rest)
              | Package signature ->
                  loop
                    (
                      List.fold_right
