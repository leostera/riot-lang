...
type t = { items: item list; items_by_id: (int, item) Collections.HashMap.t }
...
let of_list = fun items ->
  ...
  (
    items |> List.iter
      (
        fun item ->
          let _ = Collections.HashMap.insert items_by_id (item_id_of_item item |> ItemArenaId.to_int) item in ()
      )
  );
  { items; items_by_id }
...
let find_item = fun items item_id -> Collections.HashMap.get items.items_by_id (ItemArenaId.to_int item_id)
...
let item_to_json = fun value ->
  ...
  | Type (item: type_item) ->
      Data.Json.Object [
        ...
        "scope_path", Data.Json.Array (
          SurfacePath.to_segments item.scope_path |> List.map
            (
  ...
  | ExtensionConstructor (item: extension_constructor_item) ->
      Data.Json.Object [
        ...
        "scheme", Data.Json.String (TypePrinter.scheme_to_string item.scheme);
        ("inline_record_labels", match item.inline_record_labels with
        | Some labels ->
            Data.Json.Array (
              List.map
                (
...
let to_string = fun items ->
  let scope_prefix_of scope_path =
    if SurfacePath.is_empty scope_path then
      ""
    else format Format.[ str (SurfacePath.to_string scope_path); str " " ]
  in
  ...
  | _ ->
      items.items |> List.map
        (
          function
          | Type (item: type_item) -> format Format.[
            str "  ";
          ...
          | Exception (item: exception_item) -> format Format.[
            str "  ";
            str (ItemArenaId.to_string item.item_id);
            str " exception ";
            str (scope_prefix_of item.scope_path);
          ...
          | ExtensionConstructor (item: extension_constructor_item) -> format Format.[
            str "  ";
            str (ItemArenaId.to_string item.item_id);
          ...
          | Value (item: value_item) -> format Format.[
            str "  ";
            str (ItemArenaId.to_string item.item_id);
            str " value ";
            str (scope_prefix_of item.scope_path);
            ...
            bool item.recursive;
            str " bindings=[";
            str (item.binding_ids |> List.map BindingArenaId.to_string |> String.concat ", ");
            str "]";
          ]
          | DeclaredValue (item: declared_value_item) -> format Format.[
            str "  ";
            str (ItemArenaId.to_string item.item_id);
            str " declared_value ";
            str (scope_prefix_of item.scope_path);
          ...
          | Open (item: open_item) -> format Format.[
            str "  ";
            str (ItemArenaId.to_string item.item_id);
            str " open ";
            str (scope_prefix_of item.scope_path);
          ...
          | Include (item: include_item) -> format Format.[
            str "  ";
            str (ItemArenaId.to_string item.item_id);
            str " include ";
            str (scope_prefix_of item.scope_path);
          ...
          | ModuleAlias (item: module_alias_item) -> format Format.[
            str "  ";
            str (ItemArenaId.to_string item.item_id);
            str " module_alias ";
            str (scope_prefix_of item.scope_path);
          ...
          | Unsupported (item: unsupported_item) -> format Format.[
            str "  ";
            str (ItemArenaId.to_string item.item_id);
            str " unsupported ";
            str (scope_prefix_of item.scope_path);
