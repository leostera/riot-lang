...
let resolve_named_type_head_for_persistence = fun by_path name ->
  let qualified_external_head =
    ...
    | _ :: _ :: _ ->
        Some (TypeRepr.named_head ~type_constructor_id:(TypeConstructorId.of_path name) ~name)
    | _ -> None
  in
...
let canonicalize_type_for_persistence = fun by_path ->
  ...
  let rec loop ty =
    ...
    | None ->
        ...
        | TypeRepr.Char
        | TypeRepr.Unit
        | TypeRepr.Hole _ -> remember_identity ty
  ...
  let constructors =
    ...
    |> List.map
      (fun (constructor: TypeDecl.constructor) -> {
        constructor with
        scheme = canonicalize_scheme_for_persistence canonicalize_type constructor.scheme;
      })
  in
  let labels =
    declaration.labels
    |> List.map
      (fun (label: TypeDecl.label) -> {
        label with
        field_type = canonicalize_scheme_for_persistence canonicalize_type label.field_type;
      })
  in
  ...
  | FileSummary.TrustedExport { exports } ->
      FileSummary.TrustedExport {
        exports = List.map
          (fun (name, scheme) -> (
            name,
            canonicalize_scheme_for_persistence canonicalize_type scheme
          ))
          exports;
      }
...
let label_to_json = fun value ->
  ...
  | TypeRepr.Labelled label ->
      Data.Json.Object [ ("tag", Data.Json.String "labeled"); ("label", Data.Json.String label); ]
  | TypeRepr.Optional label ->
      Data.Json.Object [ ("tag", Data.Json.String "optional"); ("label", Data.Json.String label); ]
...
let constructor_to_json = fun (constructor: TypeDecl.constructor) ->
  ...
  let fields =
    ...
    | Some labels ->
        fields @ [ ("inline_record_labels", Data.Json.Array (List.map label_decl_to_json labels)); ]
    | None -> fields
  in
  Data.Json.Object fields
...
let manifest_to_json = fun value ->
  ...
  | TypeDecl.Alias manifest_type ->
      Data.Json.Object [ ("tag", Data.Json.String "alias"); ("type", type_to_json manifest_type); ]
...
let export_result_to_json = fun value ->
  ...
  | FileSummary.NoExport ->
      Data.Json.Object [ ("tag", Data.Json.String "no_export"); ("exports", Data.Json.Array []); ]
...
let source_origin_to_json = fun value ->
  ...
  | Source.Label label ->
      Data.Json.Object [ ("tag", Data.Json.String "label"); ("value", Data.Json.String label); ]
      ...
      let rec parse_tags acc = function
        ...
        | value :: rest ->
            ...
            let payload_type =
              ...
              | Some payload_type_json ->
                  type_of_json payload_type_json
...
and scheme_of_json = function
  ...
  | other ->
      Error ("expected module typings type scheme object but got " ^ json_type_name other)
...
let type_decl_of_json = fun json ->
        ...
        let rec loop acc = function
          ...
          | value :: rest ->
              ...
              let* variance =
                ...
                | other ->
                    Error (format Format.[ str "unknown module typings variance "; str other ])
              in
  ...
  let manifest =
    ...
    | Some manifest_json ->
        manifest_of_json manifest_json
        |> Result.map Option.some
    | None -> Ok None
  in
  let nonrec_ =
...
let source_origin_of_json = fun json ->
  ...
  | "path" -> (
      ...
      | Error (Path.InvalidUtf8 { path }) ->
          Error (format Format.[ str "invalid utf-8 path "; str path ])
