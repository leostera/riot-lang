...
let map_preserving = fun loop xs ->
  let rec walk changed acc = function
    ...
    | x :: rest ->
        let x2 = loop x in walk (changed || not (Std.Ptr.equal x x2)) (x2 :: acc) rest
  in
  walk false [] xs
...
let resolve_named_type_head_for_persistence = fun by_path name ->
  let qualified_external_head =
    ...
    | _ :: _ :: _ -> Some (TypeRepr.named_head ~type_constructor_id:(TypeConstructorId.of_path name) ~name)
    | _ -> None
  in
  Collections.HashMap.get by_path name |> Option.map
    ...
    Option.or_else resolved
      (
        fun () ->
          Option.or_else (BuiltinTypeConstructors.head_of_path name)
            (
...
let canonicalize_type_for_persistence = fun by_path ->
  ...
  let rec loop ty =
    ...
    | None ->
        ...
        | TypeRepr.Var { link = None; _ } -> remember_identity ty
        | TypeRepr.Var { link = Some linked; _ } ->
            let replacement = loop linked in remember_replacement ty replacement
        ...
        | TypeRepr.Package signature ->
            let shell = prepare_shell ty in
            let values =
              signature.values |> map_preserving
                (
        ...
        | TypeRepr.Named { head; arguments } ->
            ...
            let head2 = resolve_named_type_head_for_persistence by_path head.name |> Option.unwrap_or ~default:head in
            (
...
let canonicalize_type_decl_for_persistence = fun canonicalize_type (type_decl: FileSummary.type_decl) ->
            ...
            tags = List.map
              (
                fun (tag: TypeDecl.poly_variant_tag) ->
                  match tag.payload_type with
                  | Some payload_type -> { tag with payload_type = Some (canonicalize_type payload_type) }
                  | None -> tag
              )
              tags;
  ...
  let constructors =
    declaration.constructors |> List.map
      (
        fun (constructor: TypeDecl.constructor) -> { constructor with scheme = canonicalize_scheme_for_persistence canonicalize_type constructor.scheme }
      )
  in
  let labels =
    declaration.labels |> List.map
      (
        fun (label: TypeDecl.label) -> { label with field_type = canonicalize_scheme_for_persistence canonicalize_type label.field_type }
      )
  in
...
let canonicalize_payload_for_persistence = fun ~export_result ~type_decls ->
  ...
  let canonicalize_type = canonicalize_type_for_persistence by_path in (canonicalize_export_result_for_persistence canonicalize_type export_result, List.map (canonicalize_type_decl_for_persistence canonicalize_type) type_decls)

let compiled_scope_of_payload = fun ~export_result ~type_decls ->
  let exports =
    match export_result with
    | FileSummary.TrustedExport { exports } | FileSummary.ErroredExport { exports } -> exports
...
let complete = fun ~module_name ~source_hash ?(type_decls = []) ?(value_definitions = []) exports ->
  let (export_result, type_decls) = canonicalize_payload_for_persistence ~export_result:(FileSummary.TrustedExport { exports }) ~type_decls in
  {
    module_name;
    source_hash;
...
let trusted = fun ~module_name ~source_hash ?(type_decls = []) ?(value_definitions = []) exports -> complete ~module_name ~source_hash ~type_decls ~value_definitions exports

let errored = fun ~module_name ~source_hash ?(type_decls = []) ?(value_definitions = []) exports -> partial ~module_name ~source_hash ~type_decls ~value_definitions ~exports ()

let missing = fun ~module_name ~source_hash ?(type_decls = []) ?(value_definitions = []) () -> partial ~module_name ~source_hash ~type_decls ~value_definitions ()

let of_file_summary = fun ~module_name ~source_hash ?(value_definitions = []) (summary: FileSummary.t) ->
  let (export_result, type_decls) = canonicalize_payload_for_persistence ~export_result:summary.export_result ~type_decls:summary.type_decls in
  {
    module_name;
    source_hash;
...
let exports = fun value ->
  match value with
  | { export_result = FileSummary.TrustedExport { exports }; _ } | { export_result = FileSummary.ErroredExport { exports }; _ } -> exports
...
let error_expected = fun expected actual -> Error (format Format.[
  str "expected ";
  str expected;
  str " but got ";
  str (json_type_name actual);
])
...
let label_to_json = fun value ->
  ...
  | TypeRepr.Nolabel ->
      Data.Json.Object [
        "tag", Data.Json.String "nolabel";
      ]
  | TypeRepr.Labelled label ->
      Data.Json.Object [
        "tag", Data.Json.String "labeled";
        "label", Data.Json.String label;
      ]
  | TypeRepr.Optional label ->
      Data.Json.Object [
...
let rec type_to_json = fun ty ->
              ...
              fun (value: TypeRepr.package_value) ->
                ...
                Data.Json.Object [
                  ...
                  "scheme", Data.Json.Object [
                    "quantified", Data.Json.Array (
                      List.map
                        (
                          fun id -> Data.Json.Int id
                        )
                        quantified
                    );
  ...
  | TypeRepr.PolyVariant { bound; tags; inherited } ->
      ...
      let tag_to_json (tag: TypeRepr.poly_variant_tag) =
        let fields =
          [
            "name", Data.Json.String tag.name;
          ]
        in
        let fields =
          match tag.payload_type with
          | Some payload_type ->
              fields @ [
                "payload_type", type_to_json payload_type;
              ]
          | None -> fields
        in
      ...
      (
        fun (name, scheme) ->
          let scheme_json =
            ...
            | Failure message -> raise (Failure (format Format.[
...
let type_decl_to_json = fun (type_decl: FileSummary.type_decl) ->
  let fields =
    [
      "scope_path", Data.Json.Array (
        SurfacePath.to_segments type_decl.scope_path |> List.map
          (
            fun segment -> Data.Json.String segment
          )
      );
      ...
      "param_variances", Data.Json.Array (
        List.map
          (
...
let value_definitions_to_json = fun value_definitions -> Data.Json.Array (List.map value_definition_to_json value_definitions)
...
let synthetic_source_hash = fun ~module_name ~export_result ~type_decls ?(value_definitions = []) () ->
  ...
  in
  payload_to_json ~completeness ~export_result ~type_decls ~value_definitions |> Data.Json.to_string |> fun json -> Crypto.hash_string (format Format.[
    str "typ-module-typings\x1f";
    str module_name;
    str "\x1f";
    str json;
  ])
...
let label_of_json = fun json ->
  ...
  in
  let* tag_json = field "tag" fields
  in
  let* tag = get_string tag_json
  in
  match tag with
  | "nolabel" -> Ok TypeRepr.Nolabel
  | "labeled" ->
      let* label_json = field "label" fields
      in
      let* label = get_string label_json in Ok (TypeRepr.Labelled label)
  | "optional" ->
      let* label_json = field "label" fields
      in
      let* label = get_string label_json in Ok (TypeRepr.Optional label)
...
let rec type_of_json = fun json ->
  ...
  | "option" ->
      let* element_json = field "element" fields
      in
      let* element = type_of_json element_json in Ok (TypeRepr.option element)
  | "result" ->
      let* ok_json = field "ok" fields
      in
      let* error_json = field "error" fields
      in
      let* ok_ty = type_of_json ok_json
      in
      let* error_ty = type_of_json error_json in Ok (TypeRepr.result ok_ty error_ty)
  | "array" ->
      let* element_json = field "element" fields
      in
      let* element = type_of_json element_json in Ok (TypeRepr.array element)
  | "list" ->
      let* element_json = field "element" fields
      in
      let* element = type_of_json element_json in Ok (TypeRepr.list element)
  | "seq" ->
      let* element_json = field "element" fields
      in
      let* element = type_of_json element_json in Ok (TypeRepr.seq element)
  | "package" ->
      let* values_json = field "values" fields
      in
      let* values_json = get_array values_json
      in
      let rec loop acc = function
        | [] -> Ok (List.rev acc)
        | value_json :: rest ->
            let* value_fields = get_object value_json
            in
            let* name_json = field "name" value_fields
            in
            let* scheme_json = field "scheme" value_fields
            in
            let* name = get_string name_json
            in
            let* scheme = scheme_of_json scheme_json in loop (TypeRepr.package_value ~name ~scheme :: acc) rest
      in
      let* values = loop [] values_json in Ok (TypeRepr.package ~values)
  | "named" ->
      let* type_constructor_json = field "type_constructor_id" fields
      in
      let* type_constructor_id = TypeConstructorId.of_json type_constructor_json
      in
      ...
      let* arguments_json = field "arguments" fields
      in
      let* arguments_json = get_array arguments_json
      in
      let rec loop acc = function
        | [] -> Ok (List.rev acc)
        | head :: tail ->
            let* argument = type_of_json head in loop (argument :: acc) tail
      in
      let* arguments = loop [] arguments_json in Ok (TypeRepr.named ~head:(TypeRepr.named_head ~type_constructor_id ~name) ~arguments)
  | "poly_variant" ->
      let* bound_json = field "bound" fields
      in
      let* tags_json = field "tags" fields
      in
      let* inherited_json = field "inherited" fields
      in
      let* bound = get_string bound_json
      in
      let bound =
        match bound with
        ...
        | other -> Error (format Format.[ str "unknown module typings structural poly-variant bound "; str other ])
      in
      let* bound = bound
      in
      let* tags_json = get_array tags_json
      in
      let rec parse_tags acc = function
        | [] -> Ok (List.rev acc)
        | value :: rest ->
            let* fields = get_object value
            in
            let* name_json = field "name" fields
            in
            let* name = get_string name_json
            ...
            let payload_type =
              ...
              | None -> Ok None
            in
            let* payload_type = payload_type in parse_tags (TypeRepr.poly_variant_tag ?payload_type name :: acc) rest
      in
      let* tags = parse_tags [] tags_json
      in
      let* inherited_json = get_array inherited_json
      in
      ...
      in
      let* inherited = parse_inherited [] inherited_json in Ok (TypeRepr.poly_variant ~bound ~tags ~inherited)
  | "tuple" ->
      let* members_json = field "members" fields
      in
      let* members_json = get_array members_json
      in
      let rec loop acc = function
        | [] -> Ok (List.rev acc)
        | head :: tail ->
            let* member = type_of_json head in loop (member :: acc) tail
      in
      let* members = loop [] members_json in Ok (TypeRepr.tuple members)
  | "arrow" ->
      let* label_json = field "label" fields
      in
      let* lhs_json = field "lhs" fields
      in
      let* rhs_json = field "rhs" fields
      in
      let* label = label_of_json label_json
      in
      let* lhs = type_of_json lhs_json
      in
      let* rhs = type_of_json rhs_json in Ok (TypeRepr.arrow ~label ~lhs ~rhs)
  | "var" ->
      let* id_json = field "id" fields
      in
      let* id = get_int id_json in Ok (TypeRepr.make_var id)
  | "hole" ->
      let* id_json = field "id" fields
      in
      let* id = get_int id_json in Ok (TypeRepr.hole id)
  ...
  | Data.Json.Object fields -> (
    ...
    | (Some (Data.Json.Array quantified_json), Some body_json) ->
        let rec parse_quantified acc = function
          ...
          | other :: _ -> Error ("expected quantified type variable id int but got " ^ json_type_name other)
        in
        begin
...
let exports_of_json = fun json ->
  ...
  let rec loop acc = function
    ...
    | value :: rest ->
        ...
        let* name_json = field "name" fields
        in
        let* scheme_json = field "scheme" fields
        in
        let* name = get_string name_json
        in
        let* scheme = scheme_of_json scheme_json in loop ((SurfacePath.of_string name, scheme) :: acc) rest
  in
  loop [] values
...
let int_list_of_json = fun json ->
  ...
  let rec loop acc = function
    | [] -> Ok (List.rev acc)
    | value :: rest ->
        let* value = get_int value in loop (value :: acc) rest
  in
  loop [] values
...
let string_list_of_json = fun json ->
  ...
  let rec loop acc = function
    | [] -> Ok (List.rev acc)
    | value :: rest ->
        let* value = get_string value in loop (value :: acc) rest
  in
  loop [] values
...
let constructor_of_json = fun json ->
  let* fields = get_object json
  in
  let* constructor_id_json = field "constructor_id" fields
  in
  let* name_json = field "name" fields
  in
  let* scheme_json = field "scheme" fields
  ...
  let* constructor_id = get_int constructor_id_json
  in
  let* name = get_string name_json
  in
  let* scheme = scheme_of_json scheme_json
  in
  let* generalized =
  ...
  let* inline_record_labels =
    ...
    | Some labels_json ->
        let* labels_json = get_array labels_json
        in
        let parse_label_json label_json =
          let* fields = get_object label_json
          in
          let* label_id_json = field "label_id" fields
          in
          let* name_json = field "name" fields
          in
          let* field_type_json = field "field_type" fields
          in
          let* mutable_json = field "mutable" fields
          in
          let* label_id = get_int label_id_json
          in
          let* name = get_string name_json
          in
          let* field_type = scheme_of_json field_type_json
          in
          let mutable_ =
...
let poly_variant_tag_of_json = fun json ->
  ...
  let payload_type =
    ...
    | None -> Ok None
  in
  let* payload_type = payload_type in Ok ({ TypeDecl.name = name; payload_type } : TypeDecl.poly_variant_tag)
...
let manifest_of_json = fun json ->
  ...
  | "alias" ->
      ...
      in
      let* manifest_type = type_of_json type_json in Ok (TypeDecl.Alias manifest_type)
  | "poly_variant" ->
      let* bound_json = field "bound" fields
      in
      let* tags_json = field "tags" fields
      ...
      let bound =
        ...
        | other -> Error (format Format.[ str "unknown module typings poly-variant bound "; str other ])
      in
      let* bound = bound
      in
      let* tags_json = get_array tags_json
      ...
      in
      let* inherited = parse_inherited [] inherited_json in Ok (TypeDecl.PolyVariant { bound; tags; inherited })
  | other -> Error (format Format.[ str "unknown module typings manifest tag "; str other ])

let type_decl_of_json = fun json ->
  let* fields = get_object json
  in
  let* scope_path_json = field "scope_path" fields
  in
  let* type_constructor_id_json = field "type_constructor_id" fields
  in
  let* type_name_json = field "type_name" fields
  in
  let* param_ids_json = field "param_ids" fields
  in
  let param_variances_json = List.assoc_opt "param_variances" fields in
  let* constructors_json = field "constructors" fields
  in
  let* labels_json = field "labels" fields
  in
  let* scope_path = string_list_of_json scope_path_json
  in
  let* type_constructor_id = TypeConstructorId.of_json type_constructor_id_json
  in
  let* type_name = get_string type_name_json
  in
  let* param_ids = int_list_of_json param_ids_json
  in
  let param_variances =
    match param_variances_json with
    | Some param_variances_json ->
        let* values = get_array param_variances_json
        in
        let rec loop acc = function
          | [] -> Ok (List.rev acc)
          | value :: rest ->
              let* variance_name = get_string value
              in
              let* variance =
  ...
  in
  let* param_variances = param_variances
  in
  let* constructors_json = get_array constructors_json
  in
  ...
  in
  let* constructors = parse_constructors [] constructors_json
  in
  let* labels_json = get_array labels_json
  in
  let rec parse_labels acc = function
    | [] -> Ok (List.rev acc)
    | value :: rest ->
        let* label = label_decl_of_json value in parse_labels (label :: acc) rest
  in
...
let type_decls_of_json = fun json ->
  ...
  let rec loop acc = function
    | [] -> Ok (List.rev acc)
    | value :: rest ->
        let* type_decl = type_decl_of_json value in loop (type_decl :: acc) rest
  in
  loop [] values

let export_result_of_json = fun json ->
  let* export_result_fields = get_object json
  in
  let* tag_json = field "tag" export_result_fields
  in
  let* exports_json = field "exports" export_result_fields
  in
  let* tag = get_string tag_json
  in
  let* exports = exports_of_json exports_json
  in
  match tag with
...
let source_origin_of_json = fun json ->
  ...
  let* tag_json = field "tag" fields
  in
  let* value_json = field "value" fields
  in
  let* tag = get_string tag_json
  in
  let* value = get_string value_json
  in
  match tag with
  | "path" -> (
    ...
    | Error (Path.SystemInvalidUtf8 { syscall; path }) -> Error (format Format.[
      str "invalid utf-8 path from ";
      str syscall;
      str ": ";
      str path;
    ])
  ...
  | other -> Error (format Format.[ str "unknown module typings source origin tag "; str other ])
...
let value_definition_target_of_json = fun json ->
  ...
  match tag with
  | "site" ->
      let* origin_json = field "origin" fields
      in
      let* span_json = field "span" fields
      in
      let* origin = source_origin_of_json origin_json
      in
      let* span = span_of_json span_json in Ok (Site { origin; span })
  | "export" ->
      let* path_json = field "path" fields
      in
      let* path = get_string path_json in Ok (Export (SurfacePath.of_string path))
  | other -> Error (format Format.[ str "unknown module typings value definition target tag "; str other ])

let value_definition_of_json = fun json ->
  let* fields = get_object json
  in
  let* export_name_json = field "export_name" fields
  in
  let* target_json = field "target" fields
  in
  let* export_name = get_string export_name_json
  in
  let* target = value_definition_target_of_json target_json in Ok { export_name = SurfacePath.of_string export_name; target }
...
module Json = struct
  ...
  let of_json = fun json ->
    let* fields = get_object json
    in
    let* module_name_json = field "module_name" fields
    in
    let* source_hash_json = field "source_hash" fields
    in
    let* export_result_json = field "export_result" fields
    in
    let* module_name = get_string module_name_json
    in
    let* source_hash_hex = get_string source_hash_json
    in
    let* source_hash = hash_of_hex source_hash_hex
    in
    let* export_result = export_result_of_json export_result_json
    in
    let completeness =
    ...
    in
    let* type_decls = type_decls
    in
    let* value_definitions = value_definitions
    in
    Ok {
      module_name;
      source_hash;
