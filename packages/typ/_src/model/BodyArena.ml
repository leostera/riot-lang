...
let render_pattern_desc = function
  ...
  | PRecord { fields; open_ } ->
      "record { " ^ (
        fields
        |> List.map render_record_pattern_field
        |> String.concat ", "
      ) ^ " }" ^ if open_ then
        " open"
      else
...
let render_expr_desc = function
  ...
  | EFun (parameters, body_id) ->
      ...
      ^ (
        parameters
        |> List.map render_function_parameter
        |> String.concat ", "
      )
      ^ "] -> "
  ...
  | EApply (callee_id, arguments) ->
      ...
      ^ (
        arguments
        |> List.map render_apply_argument
        |> String.concat ", "
      )
      ^ "]"
  | ERecord { base_id; fields } ->
      ...
      ^ (
        fields
        |> List.map render_record_expr_field
        |> String.concat ", "
      )
      ^ " }"
  ...
  | ELet (binding_ids, body_id) ->
      "let ["
      ^ render_ids BindingArenaId.to_string binding_ids
      ^ "] in "
      ^ ExprArenaId.to_string body_id
...
let render_binding = fun (binding: binding) ->
  ...
  ^ PatternArenaId.to_string binding.pattern_id ^ (
    ...
    | None -> ""
  ) ^ " " ^ ExprArenaId.to_string binding.value_id ^ " recursive=" ^ Bool.to_string
    binding.recursive
...
let pattern_desc_to_json = function
  ...
  | PFloat digits ->
      Data.Json.Object [ ("tag", Data.Json.String "float"); ("digits", Data.Json.String digits); ]
  | PConstructor { constructor; arguments } ->
      Data.Json.Object [
  ...
  | PChar value ->
      Data.Json.Object [ ("tag", Data.Json.String "char"); ("value", Data.Json.String value); ]
  ...
  Data.Json.Object (
    [
      ...
      (
        "scope_path",
        Data.Json.Array (
          SurfacePath.to_segments binding.scope_path
...
let to_string = fun arena ->
  ...
  String.concat
    "\n"
    (((((([ "patterns:" ] @ pattern_lines) @ [ ""; "bindings:" ]) @ binding_lines)
    @ [ ""; "expressions:" ])
    @ expr_lines)
    @ [ "" ])
