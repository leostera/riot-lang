...
and record_pattern_field = { label: string; pattern_id: PatternArenaId.t }
...
type apply_argument = { label: label; implicit: bool; value_id: ExprArenaId.t }
...
type expr_desc =
  ...
  | EHole of string
and record_expr_field = { label: string; value_id: ExprArenaId.t }
and expr_node = { expr_id: ExprArenaId.t; origin_id: OriginId.t; desc: expr_desc }
and binding = {
...
let find_pattern = fun arena pat_id -> Collections.HashMap.get arena.patterns_by_id (PatternArenaId.to_int pat_id)

let find_expr = fun arena expr_id -> Collections.HashMap.get arena.expressions_by_id (ExprArenaId.to_int expr_id)

let find_binding = fun arena binding_id -> Collections.HashMap.get arena.bindings_by_id (BindingArenaId.to_int binding_id)
...
let render_record_pattern_field = fun (field: record_pattern_field) -> field.label ^ "=" ^ PatternArenaId.to_string field.pattern_id

let render_record_expr_field = fun (field: record_expr_field) -> field.label ^ "=" ^ ExprArenaId.to_string field.value_id
...
let render_pattern_desc = function
  ...
  | PUnit -> "unit"
  | PTuple elements -> "tuple [" ^ render_ids PatternArenaId.to_string elements ^ "]"
  | POr alternatives -> "or [" ^ render_ids PatternArenaId.to_string alternatives ^ "]"
  | PConstructor { constructor; arguments } -> "constructor " ^ SurfacePath.to_string constructor ^ " [" ^ render_ids PatternArenaId.to_string arguments ^ "]"
  | PRecord { fields; open_ } ->
      "record { " ^ (fields |> List.map render_record_pattern_field |> String.concat ", ") ^ " }" ^ if open_ then
        " open"
      else ""
  | PList elements -> "list [" ^ render_ids PatternArenaId.to_string elements ^ "]"
  | PAlias { pattern_id; alias } -> "alias " ^ alias ^ " = " ^ PatternArenaId.to_string pattern_id
...
let render_expr_desc = function
  | EVar name -> "var " ^ SurfacePath.to_string name
  | EInt digits -> "int " ^ digits
  | EFloat digits -> "float " ^ digits
  | EBool value -> "bool " ^ Bool.to_string value
  | EString value -> "string \"" ^ String.escaped value ^ "\""
  | EChar value -> "char '" ^ String.escaped value ^ "'"
  | EUnit -> "unit"
  | ETuple elements -> "tuple [" ^ render_ids ExprArenaId.to_string elements ^ "]"
  | EArray elements -> "array [" ^ render_ids ExprArenaId.to_string elements ^ "]"
  | ESequence elements -> "sequence [" ^ render_ids ExprArenaId.to_string elements ^ "]"
  | EWhile { condition_id; body_id } -> "while " ^ ExprArenaId.to_string condition_id ^ " do " ^ ExprArenaId.to_string body_id
  ...
  | EFun (parameters, body_id) -> "fun [" ^ (parameters |> List.map render_function_parameter |> String.concat ", ") ^ "] -> " ^ ExprArenaId.to_string body_id
  | EApply (callee_id, arguments) -> "apply " ^ ExprArenaId.to_string callee_id ^ " [" ^ (arguments |> List.map render_apply_argument |> String.concat ", ") ^ "]"
  ...
  | EFieldAccess { receiver_id; label } -> "field " ^ ExprArenaId.to_string receiver_id ^ "." ^ label
  | EFieldAssign { receiver_id; label; value_id } -> "field_assign " ^ ExprArenaId.to_string receiver_id ^ "." ^ label ^ " <- " ^ ExprArenaId.to_string value_id
  | EIndex (collection_id, index_id) -> "index " ^ ExprArenaId.to_string collection_id ^ " [" ^ ExprArenaId.to_string index_id ^ "]"
  | ELet (binding_ids, body_id) -> "let [" ^ render_ids BindingArenaId.to_string binding_ids ^ "] in " ^ ExprArenaId.to_string body_id
  | EIf (condition_id, then_id, else_id) -> "if " ^ ExprArenaId.to_string condition_id ^ " then " ^ ExprArenaId.to_string then_id ^ " else " ^ ExprArenaId.to_string else_id
          ...
          (
            fun (case: match_case) ->
              let guard_text =
                ...
                | None -> ""
              in
              "(" ^ PatternArenaId.to_string case.pattern_id ^ guard_text ^ " -> " ^ ExprArenaId.to_string case.body_id ^ ")"
          ) |> String.concat ", "
      in
  ...
  | ECoerce { value_id; target_type } -> "coerce " ^ ExprArenaId.to_string value_id ^ " :> " ^ TypePrinter.type_to_string target_type
  ...
  | ELocalModulePack { local_scope; package_type } ->
      ...
      in
      "local_pack " ^ (
        local_scope.binding_groups |> List.map
          (
            fun (group: local_module_binding_group) -> "[" ^ render_ids BindingArenaId.to_string group.binding_ids ^ "]"
          ) |> String.concat " "
      ) ^ (
  ...
  | ELocalModule { module_name; local_scope; body_id } ->
      ...
      ) ^ " in " ^ ExprArenaId.to_string body_id
  | ELocalOpen { module_path; body_id } -> "local_open " ^ SurfacePath.to_string module_path ^ " (" ^ ExprArenaId.to_string body_id ^ ")"
...
let render_binding = fun (binding: binding) ->
  ...
  "  " ^ BindingArenaId.to_string binding.binding_id ^ " " ^ qualified_name ^ " " ^ PatternArenaId.to_string binding.pattern_id ^ (
    match binding.annotation with
    | Some annotation -> " : " ^ TypePrinter.scheme_to_string annotation
    | None -> ""
  ) ^ " " ^ ExprArenaId.to_string binding.value_id ^ " recursive=" ^ Bool.to_string binding.recursive
...
let local_module_scope_to_json = fun (local_scope: local_module_scope) ->
  Data.Json.Object [
    "binding_groups", Data.Json.Array (List.map local_module_binding_group_to_json local_scope.binding_groups);
...
let pattern_desc_to_json = function
  ...
  | PConstructor { constructor; arguments } ->
      Data.Json.Object [
        ...
        "arguments", Data.Json.Array (
          List.map
            (
              fun pat_id -> Data.Json.Int (PatternArenaId.to_int pat_id)
            )
            arguments
        );
      ]
  ...
  | PFirstClassModule { module_name; package_type } ->
      Data.Json.Object [
        ...
        ("module_name", match module_name with
        | Some module_name -> Data.Json.String module_name
        | None -> Data.Json.Null);
        ("package_type", match package_type with
...
let match_case_to_json = fun (case: match_case) ->
  let guard_fields =
    ...
    | Some guard_id ->
        [
          "guard_id", Data.Json.Int (ExprArenaId.to_int guard_id);
        ]
    | None -> []
  in
  Data.Json.Object (
...
let label_to_json = function
  | Positional ->
      Data.Json.Object [
        "tag", Data.Json.String "positional";
      ]
  | Labeled label ->
      Data.Json.Object [
...
let function_parameter_to_json = fun (parameter: function_parameter) ->
  Data.Json.Object [
    ...
    ("default_value_id", match parameter.default_value_id with
  ...
  | ETuple elements ->
      Data.Json.Object [
        ...
        "elements", Data.Json.Array (
          List.map
            (
              fun expr_id -> Data.Json.Int (ExprArenaId.to_int expr_id)
            )
            elements
        );
      ]
  ...
  Data.Json.Object (
    [
      ...
      "scope_path", Data.Json.Array (
        SurfacePath.to_segments binding.scope_path |> List.map
          (
...
let to_string = fun arena ->
  ...
  let expr_lines =
    arena.expressions |> List.map
      (
        fun (node: expr_node) -> "  " ^ ExprArenaId.to_string node.expr_id ^ " " ^ OriginId.to_string node.origin_id ^ " " ^ render_expr_desc node.desc
      )
  in
  String.concat "\n" (((((([ "patterns:" ] @ pattern_lines) @ [ ""; "bindings:" ]) @ binding_lines) @ [ ""; "expressions:" ]) @ expr_lines) @ [ "" ])
