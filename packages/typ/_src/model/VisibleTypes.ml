    ...
    (fun acc (type_decl: FileSummary.type_decl) ->
      ...
      let acc =
        List.filter (fun candidate ->
          not (SurfacePath.equal (type_decl_key candidate) key)) acc
      in
      acc @ [ type_decl ])
    type_decls
    introduced
    ...
    let rec rebase_type ty =
      ...
      | TypeRepr.Package signature ->
          ...
          if Std.Ptr.equal signature.values values2 then
            ...
            TypeRepr.package ~values:values2
    in
                  ...
                  tags =
                    ...
                    |> List.map
                      (fun (tag: TypeDecl.poly_variant_tag) ->
                        match tag.payload_type with
                        | Some payload_type ->
                            { tag with payload_type = Some (rebase_type payload_type) }
                        | None -> tag);
        ...
        let constructors =
          ...
          |> List.map
            (fun (constructor: TypeDecl.constructor) ->
              {
                constructor with
                scheme = rebase_scheme constructor.scheme;
                inline_record_labels =
                  constructor.inline_record_labels
                  |> Option.map rebase_labels;
              })
        in
        let labels = rebase_labels declaration.labels in
...
let nonrec_resolvers = fun by_id by_path (current: FileSummary.type_decl) ->
  ...
  let not_current (type_decl: FileSummary.type_decl) =
...
let resolve_type_with = fun ~make ~resolve_named_type_decl ~resolve_named_type_head ->
  let same_head left right =
    ...
    && SurfacePath.equal left.TypeRepr.name right.TypeRepr.name
  in
  ...
  let rec loop ty =
    ...
    | TypeRepr.Option element ->
        ...
        if Std.Ptr.equal element resolved_element then
          ...
          make (TypeRepr.Option resolved_element)
    | TypeRepr.Result (ok_ty, error_ty) ->
        ...
        if Std.Ptr.equal ok_ty resolved_ok_ty && Std.Ptr.equal error_ty resolved_error_ty then
          ...
          make (TypeRepr.Result (resolved_ok_ty, resolved_error_ty))
    | TypeRepr.Array element ->
        let resolved_element = loop element in
        if Std.Ptr.equal element resolved_element then
          ty
        else
          make (TypeRepr.Array resolved_element)
    | TypeRepr.List element ->
        let resolved_element = loop element in
        if Std.Ptr.equal element resolved_element then
          ty
        else
          make (TypeRepr.List resolved_element)
    | TypeRepr.Seq element ->
        let resolved_element = loop element in
        if Std.Ptr.equal element resolved_element then
          ty
        else
          make (TypeRepr.Seq resolved_element)
    | TypeRepr.Named { head; arguments } ->
        ...
        (
          ...
          | Some type_decl -> (
              ...
              | None -> (
                  ...
                  | None ->
                      if
                        Std.Ptr.equal arguments resolved_arguments && same_head head resolved_head
                      then
                        ty
                      else
                        make
                          (TypeRepr.Named { head = resolved_head; arguments = resolved_arguments })
                )
            )
          | None -> (
              ...
              | None ->
                  ...
                  then
                    ...
                    make (TypeRepr.Named { head = resolved_head; arguments = resolved_arguments })
            )
        )
    | TypeRepr.PolyVariant { bound; tags; inherited } ->
        let resolved_tags =
            ...
            (fun (tag: TypeRepr.poly_variant_tag) ->
              ...
              | Some payload_type ->
                  ...
                  if Std.Ptr.equal payload_type resolved_payload_type then
                    ...
                    { tag with payload_type = Some resolved_payload_type }
              | None -> tag)
            tags
        in
        ...
        if Std.Ptr.equal tags resolved_tags && Std.Ptr.equal inherited resolved_inherited then
          ...
          make
            (TypeRepr.PolyVariant { bound; tags = resolved_tags; inherited = resolved_inherited })
    | TypeRepr.Tuple members ->
        let resolved_members = map_preserving loop members in
        if Std.Ptr.equal members resolved_members then
          ty
        else
          make (TypeRepr.Tuple resolved_members)
    | TypeRepr.Arrow { label; lhs; rhs } ->
        ...
        if Std.Ptr.equal lhs resolved_lhs && Std.Ptr.equal rhs resolved_rhs then
          ...
          make (TypeRepr.Arrow { label; lhs = resolved_lhs; rhs = resolved_rhs })
    | TypeRepr.Package signature ->
        let resolved_values =
          map_preserving
            (fun (value: TypeRepr.package_value) ->
              ...
              if Std.Ptr.equal value.scheme resolved_scheme then
                ...
                { value with scheme = resolved_scheme })
            signature.values
        in
        if Std.Ptr.equal signature.values resolved_values then
          ty
        else
          make (TypeRepr.Package { values = resolved_values })
  in
  loop
...
let resolve_type = fun visible_types ->
  resolve_type_with
    ...
    ~resolve_named_type_head:(resolve_named_type_head_in_index visible_types.by_path)
  ...
  let canonicalize_type_decl_heads (type_decl: FileSummary.type_decl) =
    let (resolve_named_type_head, resolve_named_type_decl) =
      if type_decl.declaration.nonrec_ then
        ...
        (default_resolve_named_type_head, fun _ -> None)
    in
    let resolve_type =
...
let of_type_decls = fun ?cached_by_id type_decls ->
  type_decls
  |> annotate_type_decl_variances ?cached_by_id
  |> index_type_decls
...
let bind_annotated_type_decls = fun base introduced ->
  if List.is_empty introduced then
    ...
    bind_type_decls base.type_decls introduced
    |> index_type_decls
...
let bind = fun base introduced ->
  introduced
  |> annotate_type_decl_variances ~cached_by_id:base.by_id
  |> bind_annotated_type_decls base
...
let normalized_poly_variant_tags = fun tags ->
  ...
  |> List.map
    (fun (tag: TypeRepr.poly_variant_tag) -> (
      tag.name,
      tag.payload_type
      |> Option.map TypePrinter.type_to_string
    ))
  |> List.sort compare
...
let canonicalize_inline_record_labels = fun visible_types labels ->
  ...
  |> List.map
    (fun (label: TypeDecl.label) -> {
      label with
      field_type = canonicalize_scheme visible_types label.field_type;
    })
...
let canonicalize_type_decl = fun visible_types (type_decl: FileSummary.type_decl) ->
      ...
      let () =
        ...
        |> List.iter
          (fun (candidate: FileSummary.type_decl) ->
            ...
            let _ =
              Collections.HashMap.insert by_id candidate.declaration.type_constructor_id candidate
            in
            ())
      in
      { type_decls; by_path; by_id }
  ...
  let manifest =
    ...
    | None -> None
    | Some (TypeDecl.Alias manifest_type) ->
        Some (TypeDecl.Alias (canonicalize_type visible_types manifest_type))
    | Some (TypeDecl.PolyVariant { bound; tags; inherited }) ->
        Some (
          TypeDecl.PolyVariant {
            bound;
              ...
              List.map
                (fun (tag: TypeDecl.poly_variant_tag) ->
                  ...
                  | Some payload_type ->
                      {
                        tag with
                        payload_type = Some (canonicalize_type visible_types payload_type);
                      }
                  | None -> tag)
                tags;
