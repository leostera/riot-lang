...
let constructor_to_json = fun (constructor: constructor) ->
  ...
  let fields =
    ...
    | Some labels ->
        fields @ [ ("inline_record_labels", Data.Json.Array (List.map label_to_json labels)); ]
    | None -> fields
  in
...
let poly_variant_tag_to_json = fun (tag: poly_variant_tag) ->
  ...
  let fields =
    ...
    | Some payload_type ->
        fields @ [ ("payload_type", Data.Json.String (TypePrinter.type_to_string payload_type)); ]
    | None -> fields
  in
  Data.Json.Object fields
...
let manifest_to_string = fun value ->
  match value with
  | Alias manifest_type ->
      format Format.[ str "= "; str (TypePrinter.type_to_string manifest_type) ]
  | PolyVariant { bound; tags; inherited } ->
      let prefix =
        match bound with
        | Exact -> ""
      ...
      in
      let members =
        List.map poly_variant_tag_to_string tags @ List.map TypePrinter.type_to_string inherited
      in
