...
let rec exports = fun scope ->
  let local_exports =
    scope.values
    |> List.map (fun (name, scheme) -> (SurfacePath.of_name name, scheme))
  in
  let nested_exports =
    scope.modules
    |> List.concat_map
...
let lookup_owned_constructor = fun scope path owner_type_constructor_id ->
  lookup_constructors scope path
  |> List.find_map
...
let local_record_decls = fun scope ->
  ...
  |> List.filter_map
    (fun (type_decl: FileSummary.type_decl) ->
      ...
      | labels ->
          Some {
            owner_path = SurfacePath.append_name
...
let lookup_record_decl_by_owner = fun scope owner_type_constructor_id ->
  ...
  |> List.find_map
    (fun (record_decl: record_decl) ->
      if
        TypeConstructorId.equal owner_type_constructor_id record_decl.owner_type_constructor_id
      then
        Some record_decl
      else
        None)
