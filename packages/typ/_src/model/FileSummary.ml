...
type type_decl = { scope_path: SurfacePath.t; declaration: TypeDecl.t }
...
let complete = fun ~source_id ?(type_decls = []) exports ->
  {
    ...
    export_result = TrustedExport { exports };
    type_decls
  }
...
let exports = fun value ->
  match value with
  | { export_result = TrustedExport { exports }; _ } | { export_result = ErroredExport { exports }; _ } -> exports
...
let export_status = fun value ->
  ...
  | { completeness = Partial; export_result = TrustedExport _; _ } | { export_result = ErroredExport _; _ } -> Errored
...
let render_exports = fun exports ->
  ...
  | _ ->
      exports |> List.map
        (
          fun (name, scheme) -> format Format.[ str (SurfacePath.to_string name); str " : "; str (TypePrinter.scheme_to_string scheme) ]
        ) |> String.concat ", "
...
let to_string = fun summary ->
  ...
  in
  match summary.export_result with
  | TrustedExport { exports } -> format Format.[
    str "  ";
  ...
  | ErroredExport { exports } -> format Format.[
    str "  ";
    str (SourceId.to_string summary.source_id);
    str " ";
    str completeness;
