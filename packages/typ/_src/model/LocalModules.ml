...
let create_interner = fun capacity -> { by_raw = Collections.HashMap.with_capacity capacity; entries = Array.make (Int.max 8 capacity) None; next_id = 0 }
...
let intern_entry = fun table raw make ->
  ...
  | None ->
      ...
      let _ = Collections.HashMap.insert table.by_raw raw id in id
...
let intern_ambient_name = fun value ->
  intern_entry ambient_names value
    (
      fun () -> { raw = value }
    )
...
let intern_required_name = fun value ->
  intern_entry required_names value
    (
      fun () -> { raw = value }
    )
...
let split_internal_module_name = fun module_name ->
  let rec find_separator index =
    if index + 1 >= String.length module_name then
      ...
      if module_name.[index] = '_' && module_name.[index + 1] = '_' then
        Some index
  ...
  let rec loop start acc =
    if start >= String.length module_name then
      ...
      | Some index ->
          let segment = String.sub module_name start (index - start) in loop (index + 2) (segment :: acc)
      | None ->
          let segment = String.sub module_name start (String.length module_name - start) in List.rev (segment :: acc)
  in
  loop 0 []
...
let class_case_segment = fun segment ->
  segment |> String.split_on_char '_' |> List.filter
    (
      fun piece -> not (String.equal piece "")
    ) |> List.map String.capitalize_ascii |> String.concat ""

let class_case_module_name = fun module_name ->
  module_name |> String.split_on_char '.' |> List.filter
    (
      fun segment -> not (String.equal segment "")
    ) |> List.map class_case_segment |> String.concat "."

let local_module_alias_strings_of_local_segments = fun local_segments ->
  match local_segments with
  | [] -> []
  | _ ->
      let dotted_name = String.concat "." local_segments in dedupe_preserving_order (module_name_suffix_aliases (class_case_module_name dotted_name) @ module_name_suffix_aliases dotted_name)
...
let local_module_alias_strings_of_internal_module_name = fun module_name -> split_internal_module_name module_name |> Array.of_list |> local_module_alias_strings_of_segments
    ...
    intern_entry internal_names value
      (
        fun () ->
          ...
          let direct_alias_set = direct_alias_strings |> List.map intern_required_name |> Collections.HashSet.of_list in
              ...
              (
                fun prefix_length ->
                  if prefix_length = 0 || prefix_length >= Array.length segments then
                    ...
                    let suffix_segments = Array.sub segments prefix_length (Array.length segments - prefix_length) in Some (suffix_segments |> relative_module_alias_strings_of_segments |> List.map intern_required_name |> Collections.HashSet.of_list)
              )
          in
          {
            raw = value;
...
let should_include_implicit_open = fun ~current_module_name ~module_name ->
  ...
  if Array.length current_segments <= 1 || not (String.ends_with ~suffix:"__Aliases" module_name) then
    ...
    | "Aliases" :: reversed_prefix ->
        let prefix = List.rev reversed_prefix in not (not (List.is_empty prefix) && List.length prefix <= Array.length current_segments && List.for_all2 String.equal prefix (Array.to_list (Array.sub current_segments 0 (List.length prefix))))
    | _ -> true

let matches_required_name = fun ~required_name candidate_module_name ->
  let candidate = internal_entry candidate_module_name in Int.equal candidate.required_name required_name || Collections.HashSet.contains candidate.direct_alias_set required_name
...
let common_prefix_length = fun left right ->
  ...
  let rec loop index =
    if index >= max_length then
      ...
      if String.equal left.(index) right.(index) then
        loop (index + 1)
      else index
  in
  loop 0
...
let contextual_match_depth = fun ~current_module_name ~required_module_name ~candidate_module_name ->
  ...
  in
  let max_prefix_length = common_prefix_length current_segments candidate.segments |> Int.min (Array.length candidate.segments - 1) in
  let rec loop best prefix_length =
    if prefix_length > max_prefix_length then
      ...
      | Some relative_aliases when Collections.HashSet.contains relative_aliases required_module_name ->
          let best = Some (Option.unwrap_or ~default:prefix_length best |> Int.max prefix_length) in loop best (prefix_length + 1)
      | Some _ | None -> loop best (prefix_length + 1)
  in
  loop best 1
