    ...
    | ({ source; analysis; _ } as candidate) :: tail ->
        if not (source_kind source = desired_kind) then
          ...
          (
            ...
            | Some { source = existing; _ } when prefer_source existing source ->
                loop (Some candidate) tail
...
let source_span = fun (source: Source.t) ->
  Syn.Cst.syntax_node_of_source_file source.cst
...
let canonical_scheme_string = fun visible_types scheme ->
  ...
  |> TypePrinter.scheme_to_string

let canonical_type_string = fun visible_types ty ->
  VisibleTypes.canonicalize_type visible_types ty
  |> TypePrinter.type_to_string
  ...
  let constructors =
    ...
    |> List.map
      (fun (constructor: TypeDecl.constructor) ->
        ...
        ^ inline_record
        ^ ":"
        ^ canonical_scheme_string visible_types constructor.scheme)
    |> String.concat ";"
  in
  let labels =
    decl.labels
...
let module_result_of_summary = fun ~internal_name ~source_hash ~value_definitions summary ->
  ModuleTypings.of_file_summary
    ~module_name:(LocalModules.InternalName.to_string internal_name)
    ~source_hash
    ~value_definitions
    summary
...
let with_module_view = fun module_result (analysis: SourceAnalysis.t) ->
  let file_summary =
    ModuleTypings.to_file_summary ~source_id:analysis.source.source_id module_result
  in
  {
    analysis with
    completeness = SourceAnalysis.completeness_of_file_summary file_summary;
      ...
      |> List.find_map
        (
          ...
          | ItemTree.DeclaredValue item when String.equal
            export_name
            (
              qualified_name item.scope_path item.value_name
              |> SurfacePath.to_string
            ) ->
              OriginMap.find semantic_tree.origin_map item.origin_id
              |> Option.map (fun (origin: OriginMap.origin) -> origin.span)
          | ItemTree.ExtensionConstructor item when String.equal
            export_name
            (
              qualified_name item.scope_path item.constructor_name
              |> SurfacePath.to_string
            ) ->
              OriginMap.find semantic_tree.origin_map item.origin_id
              |> Option.map (fun (origin: OriginMap.origin) -> origin.span)
      ...
      |> List.find_map
        (
          ...
          | ItemTree.Type item when String.equal
            type_name
            (
              qualified_name item.scope_path item.declaration.type_name
              |> SurfacePath.to_string
            ) ->
              OriginMap.find semantic_tree.origin_map item.origin_id
              |> Option.map (fun (origin: OriginMap.origin) -> origin.span)
...
let scheme_includes = fun ~visible_types actual_scheme expected_scheme ->
  ...
  let rec includes_type actual expected =
    ...
    | _ ->
        ...
        | (TypeRepr.Package actual_signature, TypeRepr.Package expected_signature) ->
...
let poly_variant_tag_equal = fun
  ...
  String.equal left.name right.name && (
    ...
    | (Some left_payload, Some right_payload) ->
        canonical_type_equal ~visible_types left_payload right_payload
    | _ -> false
  )
...
let manifest_equal = fun ~visible_types left_decl right_decl ->
  match (
    normalized_manifest ~visible_types left_decl,
    normalized_manifest ~visible_types right_decl
  ) with
  | (None, None) -> true
...
let type_decl_matches = fun ~visible_types interface_decl implementation_decl ->
  ...
  let implementation_arity =
    List.length implementation_decl.FileSummary.declaration.TypeDecl.param_ids
  in
  if not (interface_arity = implementation_arity) then
    ...
    let implementation_decl =
      if Option.is_none interface_decl.FileSummary.declaration.manifest then
        {
          implementation_decl with
          declaration = { implementation_decl.declaration with manifest = None };
        }
      else
        implementation_decl
    in
...
let interface_shaped_export_result = fun interface_summary implementation_summary ->
  ...
  | (FileSummary.TrustedExport _, FileSummary.TrustedExport _) ->
      FileSummary.TrustedExport { exports }
...
let paired_module_view = fun ~internal_name interface_pair implementation_pair ->
  ...
  let export_result =
    interface_shaped_export_result
      interface_analysis.file_summary
      implementation_analysis.file_summary
  in
  let type_decls =
  ...
  let source_hash =
    ModuleTypings.synthetic_source_hash
      ~module_name
      ~export_result
      ~type_decls
      ~value_definitions
...
let pair_interface_and_implementation = fun ~internal_name interface_pair implementation_pair ->
  let module_name = LocalModules.InternalName.to_string internal_name in
  let { analysis = interface_analysis; visible_type_decls = interface_visible_type_decls; _ }:
    source_input = interface_pair
  in
  let { analysis = implementation_analysis; visible_type_decls = implementation_visible_type_decls; _ }:
    source_input = implementation_pair
  in
  if
    not
      (should_check_signature_inclusion
        interface_analysis.file_summary
        implementation_analysis.file_summary)
  then
    ...
    in
    let visible_types =
      signature_visible_types
        ~visible_type_decls
        ~interface_decls:interface_type_decls
        ~implementation_decls:implementation_type_decls
    in
    let mismatches =
      value_mismatches ~visible_types interface_exports implementation_exports
      @ type_decl_mismatches ~visible_types interface_type_decls implementation_type_decls
    in
    if List.is_empty mismatches then
      paired_module_view ~internal_name interface_pair implementation_pair
    else
      let interface_diagnostics =
        mismatches
        |> List.map (interface_diagnostic interface_pair implementation_pair)
      in
      let implementation_diagnostics =
        mismatches
        |> List.map (implementation_diagnostic interface_pair implementation_pair)
      in
      let export_result =
        interface_shaped_export_result
          interface_analysis.file_summary
      ...
      let interface_analysis =
        mark_analysis_partial interface_analysis interface_diagnostics
        |> with_module_view module_result
      in
      let implementation_analysis =
        mark_analysis_partial implementation_analysis implementation_diagnostics
        |> with_module_view module_result
      in
      {
        module_result;
...
let singleton_module_result = fun ~internal_name ({ source; analysis; _ }: source_input) ->
  module_result_of_summary
    ~internal_name
...
let of_sources = fun ~internal_name sources ->
  ...
  | (Some interface_pair, Some implementation_pair) ->
      pair_interface_and_implementation ~internal_name interface_pair implementation_pair
  | (Some interface_pair, None) ->
      {
        module_result = singleton_module_result ~internal_name interface_pair;
        analyses_by_source = analyses_by_source sources;
        signature_mismatches = [];
      }
  | (None, Some implementation_pair) ->
      {
        module_result = singleton_module_result ~internal_name implementation_pair;
        analyses_by_source = analyses_by_source sources;
        signature_mismatches = [];
      }
  | (None, None) ->
