...
let of_loaded_modules = fun loaded_modules ->
  let env = empty () in
  LoadedModules.iter
    (fun required_name artifact ->
      add_loaded env ~required_name artifact)
    loaded_modules;
  env
...
let find_artifact = fun env module_id ->
  ...
  |> Option.map (fun entry -> entry.artifact)

let find_compiled_scope = fun env module_id ->
  find_entry env module_id
  |> Option.map (fun entry -> entry.compiled_scope)
      ...
      |> Option.map
        (fun entry ->
          ...
          ModuleSurface.qualify_type_decls
            ~module_name
            (VisibleTypes.type_decls entry.visible_types)))
  |> List.concat
