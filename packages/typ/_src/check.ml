...
let module_pairing_finished_event = fun module_name source_ids (pairing: ModulePairing.t) ->
  Event.ModulePairingFinished {
    module_name;
    source_ids;
    export_status = export_status_of_file_summary
      (ModuleTypings.to_file_summary ~source_id:(List.hd pairing.analyses_by_source |> fst) pairing.module_result);
...
let hidden_export_names_of_exports = fun exports ->
  exports |> List.map fst |> dedupe_by_key_preserving_order ~key:(fun path -> path)
...
let scope_view_for_source = fun ~(graph:package_graph) ~state ~(group:graph_group) (source: graph_source) ->
  ...
  source.required_names |> Array.iter
    (fun required_name ->
      let local_ids = LocalModuleGraph.best_matching_local_module_ids
        graph
        group
        ~required_module_name:required_name in
            ...
            |> List.map
              (fun module_id ->
                (
                  ...
                  PackageEnv.ModuleId.Local graph.groups.(module_id).internal_name
                )))
            !implicit_open_modules_rev
      );
...
let build_package_graph = fun ~ordered_sources ->
  LocalModuleGraph.create ~ordered_sources:(ordered_sources |> List.map graph_input_of_source)
  ...
  let analyzed_sources =
    ...
    |> List.fold_left
      (fun result (source: graph_source) ->
        ...
        | Ok analyzed_sources ->
            let prepared = source.input.payload in
            let unavailable_local_ids = LocalModuleGraph.dependency_local_ids graph source.dependency_set_id
            |> Array.to_list
...
let public_module_typings_of_compiled_modules = fun (graph:package_graph) compiled_modules_by_id ->
  ...
  Array.iteri
    (fun module_id (group: graph_group) ->
      ...
      | Some finished_group -> rebind_public_module_views group finished_group.module_result
