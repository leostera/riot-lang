...
let module_pairing_finished_event = fun module_name source_ids (pairing: ModulePairing.t) ->
  Event.ModulePairingFinished {
    module_name;
    source_ids;
    export_status = export_status_of_file_summary (ModuleTypings.to_file_summary ~source_id:(List.hd pairing.analyses_by_source |> fst) pairing.module_result);
...
let ambient_names_of_source = fun (source: prepared_source) ->
  dedupe_by_key_preserving_order ~key:(
    fun ambient_name -> ambient_name
  )
    (
      (LocalModules.local_module_aliases_of_internal_name source.internal_module_name @ [ source.local_module_name ]) @ (
...
let hidden_export_names_of_exports = fun exports -> exports |> List.map fst |> dedupe_by_key_preserving_order ~key:(
  fun path -> path
)

let dedupe_type_decls_preserving_order = fun type_decls -> dedupe_by_key_preserving_order ~key:ModuleSurface.type_decl_key type_decls

let visible_module_path = fun visible_name -> SurfacePath.of_string (LocalModuleGraph.visible_module_name_to_string visible_name)

let implicit_open_required_names_of_source = fun (source: prepared_source) ->
  source.source.implicit_opens |> List.map SurfacePath.to_string |> List.filter
    (
      fun module_name -> LocalModules.should_include_implicit_open ~current_module_name:source.internal_module_name ~module_name
    ) |> List.map LocalModules.RequiredName.of_string |> dedupe_by_key_preserving_order ~key:(
    fun required_name -> required_name
  )
...
let graph_input_of_source = fun (source: prepared_source) ->
  {
    ...
    visible_names = LocalModuleGraph.InternalName source.internal_module_name :: (
      ambient_names_of_source source |> List.map
        (
          fun ambient_name -> LocalModuleGraph.AmbientName ambient_name
        )
    );
    required_names = LocalModuleGraph.required_names_of_parse_result ~current_module_name:source.internal_module_name ~parse_result:source.source.parse_result ~implicit_opens:source.source.implicit_opens
  }
...
let scope_view_for_source = fun ~(graph:package_graph) ~state ~(group:graph_group) (source: graph_source) ->
  ...
  in
  let add_visible_module module_id module_name = visible_modules_rev := (SurfacePath.of_string module_name, module_id) :: !visible_modules_rev in
  let add_implicit_open_module required_name module_id = implicit_open_modules_rev := (SurfacePath.of_string (LocalModules.RequiredName.to_string required_name), module_id) :: !implicit_open_modules_rev in
  let add_local_group ~required_name module_id =
    ...
    | None -> ()
    | Some _ -> add_visible_module package_module_id (LocalModules.RequiredName.to_string required_name)
  in
  let add_loaded_required_name required_name =
    match PackageEnv.find_loaded state.package_env ~required_name with
    | None -> ()
    | Some _ ->
        let package_module_id = PackageEnv.ModuleId.Loaded required_name in add_visible_module package_module_id (LocalModules.RequiredName.to_string required_name)
  in
  source.required_names |> Array.iter
    (
      fun required_name ->
        let local_ids = LocalModuleGraph.best_matching_local_module_ids graph group ~required_module_name:required_name in
          ...
          (
            ...
            if required_name_is_implicit_open required_name then
              implicit_open_modules_rev := List.rev_append
                (
                  local_ids |> Array.to_list |> List.map
                    (
                      fun module_id -> (SurfacePath.of_string (LocalModules.RequiredName.to_string required_name), PackageEnv.ModuleId.Local graph.groups.(module_id).internal_name)
                    )
                )
...
let build_package_graph = fun ~ordered_sources -> LocalModuleGraph.create ~ordered_sources:(ordered_sources |> List.map graph_input_of_source)

let public_module_names_of_group = fun (group: graph_group) ->
  group.sources |> List.filter_map
    (
...
let cycle_error = fun (graph: package_graph) (cycle: LocalModuleGraph.cycle) ->
  ...
  in
  let requirements = MissingRequirements.of_list [ MissingRequirements.LocalModuleCycle { module_names = cycle.module_names; source_ids = cycle.source_ids } ] in MissingRequirements { module_name; requirements }
...
let rebind_public_module_views = fun (group: graph_group) module_typings ->
  public_module_names_of_group group |> Array.of_list |> Array.to_list |> List.map
    (
      fun public_name -> ModuleSurface.rebind_module_typings ~module_name:(LocalModules.AmbientName.to_string public_name) module_typings
    )

let analyze_group = fun ~(graph:package_graph) ~state ~config (group: graph_group) ->
  let local_module_names =
    LocalModuleGraph.dependency_local_ids graph
      (
        ...
        fun result (source: graph_source) ->
          ...
          | Ok analyzed_sources ->
              ...
              let unavailable_local_ids =
                LocalModuleGraph.dependency_local_ids graph source.dependency_set_id |> Array.to_list |> List.filter
                  (
              ...
              let missing_requirements =
                (
                  source.unresolved_local_names |> Array.to_list |> List.filter
                    (
                      fun required_name -> not (LoadedModules.contains state.loaded_modules ~required_name)
                    ) |> List.map
                    (
                      fun missing_module_name -> MissingRequirements.MissingModuleSummary { module_name = LocalModules.RequiredName.to_string missing_module_name; requested_by = [ source.input.source_id ] }
                    )
                ) @ (
                  unavailable_local_ids |> List.map
                    (
                      fun module_id -> MissingRequirements.MissingModuleSummary { module_name = LocalModules.InternalName.to_string graph.groups.(module_id).internal_name; requested_by = [ source.input.source_id ] }
                    )
                )
              in
              (
                match missing_requirements with
                | _ :: _ -> Error (MissingRequirements { module_name = group.internal_name; requirements = MissingRequirements.of_list missing_requirements })
                | [] ->
                    let scope_view = scope_view_for_source ~graph ~state ~group source in
                    let imported_world = ImportedWorld.create ~package_env:state.package_env ~scope_view in
                    ...
                    in
                    let analysis = SourceAnalysis.analyze ~imported_world ~config:source_config prepared.source in
                    let () =
                      TypConfig.emit_event source_config
                        (
  ...
  Array.iteri
    (
      fun module_id (group: graph_group) ->
        ...
        | Some finished_group -> rebind_public_module_views group finished_group.module_result |> List.iter (LoadedModules.add public_module_typings)
        | None -> ()
    )
    graph.groups;
...
let fold_package_sources = fun ?package_name ?package_fingerprint ~config ~ordered_sources ~init ~f () ->
  ...
  | Ok ordered_group_ids ->
      let initial_state = { package_env = PackageEnv.of_loaded_modules TypConfig.(config.loaded_modules); loaded_modules = LoadedModules.copy TypConfig.(config.loaded_modules); compiled_modules_by_id = Array.make (Array.length graph.groups) None } in
      let result =
        ordered_group_ids |> List.fold_left
          (
            fun result module_id ->
              ...
              | Ok (acc, state) ->
                  ...
                  | Ok analyzed_sources ->
                      ...
                      let source_ids =
                        analyzed_sources |> List.map
                          (
                            fun ((source: prepared_source), _analysis, _visible_type_decls) -> source.source.source_id
                          )
                      in
                      let () =
                        TypConfig.emit_event config
                          (
                      ...
                      let pairing =
                        analyzed_sources |> List.map
                          (
                            fun ((source: prepared_source), analysis, visible_type_decls) -> { ModulePairing.source = source.source; analysis; visible_type_decls }
                      ...
                      let checked_sources =
                        analyzed_sources |> List.map
                          (
                            fun ((source: prepared_source), analysis, _visible_type_decls) -> { path = source.display_path; analysis }
                          )
                      in
                      ...
                      | Ok () ->
                          ...
                          let finished_group = { module_name = group.internal_name; checked_sources; module_result } in
      ...
      | Error _ as err -> err
      | Ok (acc, state) ->
          let public_module_typings = public_module_typings_of_compiled_modules graph state.compiled_modules_by_id in
