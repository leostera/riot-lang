...
type traced_expr = { expr_id: ExprArenaId.t; origin_id: OriginId.t; inferred_type: TypeRepr.t }

let empty = []

let span_size = fun (span: Syn.Ceibo.Span.t) -> span.end_ - span.start

let sort_entries = fun entries ->
  entries |> List.sort
    (
      fun (left: entry) (right: entry) -> Int.compare (span_size left.span) (span_size right.span)
    )
  ...
  traced_exprs |> List.filter_map
    (
      fun (trace: traced_expr) ->
        OriginMap.find origin_map trace.origin_id |> Option.map
          (
...
let to_json = fun index -> Data.Json.Array (List.map entry_to_json index)
