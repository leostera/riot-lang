...
type frame = { level: int; boundary_level: int; mutable nodes: TypeRepr.t list }

type t = { mutable current_level: int; mutable current_mark: int; mutable frames: frame list }

let create = fun () ->
  let root = { level = 0; boundary_level = 0; nodes = [] } in { current_level = 0; current_mark = 0; frames = [ root ] }
...
let rec frame_for_level = fun level frames ->
  ...
  | frame :: rest ->
      if level >= frame.level then
        Some frame
      else frame_for_level level rest
...
let with_region_finalize = fun (regions: t) ~finalize f ->
  let child = { level = regions.current_level + 1; boundary_level = regions.current_level; nodes = [] } in
...
let with_region = fun (regions: t) f -> with_region_finalize regions ~finalize:(
  fun _ result -> result
) f
    ...
    (
      fun node ->
        ...
        if TypeRepr.pool_level node = Some frame.level && TypeRepr.level node > frame.boundary_level then
          match TypeRepr.view node with
          | TypeRepr.Var { id; _ } when Int.equal node.mark generation && not (TypeRepr.is_generic_var node) -> Some (node.mark_order, id)
          | _ -> None
        else None
    ) |> List.sort
    (
