    ...
    (
      fun acc (type_decl: FileSummary.type_decl) ->
        ...
        let acc =
          List.filter
            (
              fun (candidate: FileSummary.type_decl) -> not (SurfacePath.equal (qualify_name candidate.scope_path candidate.declaration.type_name) key)
            )
            acc
        in
        acc @ [ type_decl ]
...
let same_named_head left right = TypeConstructorId.equal TypeRepr.(left.type_constructor_id) TypeRepr.(right.type_constructor_id)
...
let same_named_type_head left right = TypeConstructorId.equal TypeRepr.(left.type_constructor_id) TypeRepr.(right.type_constructor_id) && SurfacePath.equal TypeRepr.(left.name) TypeRepr.(right.name)
...
let nonrec_resolvers = fun by_path (type_decl: FileSummary.type_decl) ->
  ...
  let not_current (candidate: FileSummary.type_decl) = not (TypeConstructorId.equal candidate.declaration.type_constructor_id current_id) in
  let resolve_named_type_head name =
    Collections.HashMap.get by_path name |> Option.filter not_current |> Option.map
      (
        fun (candidate: FileSummary.type_decl) -> TypeRepr.named_head ~type_constructor_id:candidate.declaration.type_constructor_id ~name
      ) |> fun resolved ->
      Option.or_else resolved
        (
          fun () -> BuiltinTypeConstructors.head_of_path name
  ...
  in
  let resolve_named_type_decl name = Collections.HashMap.get by_path name |> Option.filter not_current in (resolve_named_type_head, resolve_named_type_decl)
      ...
      (
        ...
        | TypeRepr.Option element ->
            ...
            if Std.Ptr.equal element canonical_element then
              ty
            else make (TypeRepr.Option canonical_element)
        | TypeRepr.Result (ok_ty, error_ty) ->
            ...
            if Std.Ptr.equal ok_ty canonical_ok_ty && Std.Ptr.equal error_ty canonical_error_ty then
              ty
            else make (TypeRepr.Result (canonical_ok_ty, canonical_error_ty))
        | TypeRepr.Array element ->
            let canonical_element = loop element in
            if Std.Ptr.equal element canonical_element then
              ty
            else make (TypeRepr.Array canonical_element)
        | TypeRepr.List element ->
            let canonical_element = loop element in
            if Std.Ptr.equal element canonical_element then
              ty
            else make (TypeRepr.List canonical_element)
        | TypeRepr.Seq element ->
            let canonical_element = loop element in
            if Std.Ptr.equal element canonical_element then
              ty
            else make (TypeRepr.Seq canonical_element)
        | TypeRepr.Named { head; arguments } ->
            ...
            (
              ...
              | Some type_decl -> (
                ...
                | None -> (
                  ...
                  | None ->
                      if Std.Ptr.equal arguments canonical_arguments && same_named_type_head head resolved_head then
                        ty
                      else make (TypeRepr.Named { head = resolved_head; arguments = canonical_arguments })
                )
              )
              | None -> (
        ...
        | TypeRepr.PolyVariant { bound; tags; inherited } ->
            ...
            if Std.Ptr.equal tags canonical_tags && Std.Ptr.equal inherited canonical_inherited then
              ty
            else make (TypeRepr.PolyVariant { bound; tags = canonical_tags; inherited = canonical_inherited })
        | TypeRepr.Tuple members ->
            let canonical_members = map_preserving loop members in
            if Std.Ptr.equal members canonical_members then
              ty
            else make (TypeRepr.Tuple canonical_members)
        | TypeRepr.Arrow { label; lhs; rhs } ->
            ...
            if Std.Ptr.equal lhs canonical_lhs && Std.Ptr.equal rhs canonical_rhs then
              ty
            else make (TypeRepr.Arrow { label; lhs = canonical_lhs; rhs = canonical_rhs })
        | TypeRepr.Package signature ->
            ...
            if Std.Ptr.equal signature.values canonical_values then
              ty
            else make (TypeRepr.Package { values = canonical_values })
      )
  in
  loop ty
...
let annotate_type_decl_variances = fun ?cached_by_id type_decls ->
  ...
  let canonicalize_type_decl (type_decl: FileSummary.type_decl) =
    let (resolve_named_type_head, resolve_named_type_decl) =
      ...
      else (default_resolve_named_type_head, resolve_named_type_decl_in_index by_path)
    in
    ...
    | TypeRepr.Named { head = { type_constructor_id; name }; arguments } ->
        ...
        let rec loop arguments parameter_variances =
          ...
          | (argument :: rest_arguments, parameter_variance :: rest_variances) ->
              collect_type_variances_into visiting (TypeDecl.compose_variance variance parameter_variance) acc argument;
...
let type_decls_for_module_alias = fun visible_types ~alias_name ~module_path -> VisibleTypes.type_decls_for_module_alias visible_types ~alias_name ~module_path

let make ~imported_world ~config file =
  let base_visible_types = VisibleTypes.of_type_decls (LanguagePrelude.type_decls @ ImportedWorld.visible_type_decls imported_world) in
  {
    file;
    config;
    imported_world;
...
let add_rigid_equation = fun (state: t) rigid_id replacement -> state.rigid_equations <- (rigid_id, replacement) :: List.remove_assoc rigid_id state.rigid_equations
...
let resolve_named_type_head = fun (state: t) name -> VisibleTypes.resolve_named_type_head state.visible_types name

let canonicalize_type = fun (state: t) -> VisibleTypes.resolve_type state.visible_types

let canonicalize_scheme = fun (state: t) scheme -> canonicalize_scheme_with (canonicalize_type state) scheme

let canonicalize_scheme_with_name_resolution = fun ~resolve_named_type_decl ~resolve_named_type_head scheme -> canonicalize_scheme_with (resolve_type_with ~make:TypeRepr.of_desc ~resolve_named_type_decl ~resolve_named_type_head) scheme

let canonicalize_type_with_name_resolution = fun ~resolve_named_type_decl ~resolve_named_type_head ty -> resolve_type_with ~make:TypeRepr.of_desc ~resolve_named_type_decl ~resolve_named_type_head ty

let canonicalize_type_decl_with_name_resolution = fun ~resolve_named_type_decl ~resolve_named_type_head (type_decl: FileSummary.type_decl) ->
  ...
  let canonicalize_inline_record_labels labels =
    labels |> List.map
      (
        fun (label: TypeDecl.label) -> { label with field_type = canonicalize_scheme_with canonicalize_type label.field_type }
      )
  in
  let declaration = type_decl.declaration in
  let manifest =
  ...
  let constructors =
    declaration.constructors |> List.map
      (
        fun (constructor: TypeDecl.constructor) -> { constructor with scheme = canonicalize_scheme_with canonicalize_type constructor.scheme; inline_record_labels = constructor.inline_record_labels |> Option.map canonicalize_inline_record_labels }
      )
  in
  let labels =
    declaration.labels |> List.map
...
let canonicalize_scheme_with_named_type_head = fun resolve_named_type_head scheme -> canonicalize_scheme_with_name_resolution ~resolve_named_type_decl:(
  fun _ -> None
) ~resolve_named_type_head scheme

let canonicalize_type_decl_with_named_type_head = fun resolve_named_type_head type_decl -> canonicalize_type_decl_with_name_resolution ~resolve_named_type_decl:(
  fun _ -> None
) ~resolve_named_type_head type_decl
...
let visible_type_decl_by_id = fun (state: t) type_constructor_id -> VisibleTypes.lookup_by_id state.visible_types type_constructor_id
...
let set_visible_type_decls = fun (state: t) type_decls ->
  let local_visible_types = VisibleTypes.of_type_decls ~cached_by_id:(VisibleTypes.by_id state.visible_types) type_decls in
