...
type layer =
  ...
  | Map of {
      map_record_decl: record_decl -> record_decl;
      next: t;
    }
...
let owner_type_constructor_id = fun (record_decl: record_decl) ->
  record_decl.owner_type_constructor_id
...
let prepend_record_decl = fun index record_decl ->
  ...
  |> List.fold_left
    (fun acc (label: TypeDecl.label) ->
      let existing =
        Name_map.find_opt label.name acc
...
let current_of_record_decls = fun record_decls ->
  record_decls
  |> List.rev
  |> List.fold_left prepend_record_decl Name_map.empty
...
let qualify_record_decl = fun root record_decl -> {
  record_decl with
  owner_path = SurfacePath.append_path root record_decl.owner_path;
}

let of_type_decls = fun type_decls ->
  let record_decls =
    type_decls
    |> List.filter_map record_decl_of_type_decl
  in
  {
  ...
  | Map { map_record_decl; next } ->
      ...
      |> merge_visible_components
        (
          ...
          |> map_components map_record_decl
        )
...
let rec lookup_all_label = fun env label_name ->
  let current =
    Name_map.find_opt label_name env.current
    |> Option.unwrap_or ~default:[]
  in
  match env.layer with
  | Nothing -> current
  | Open { root; components; next } ->
      let opened =
        Name_map.find_opt label_name components.by_name
        |> Option.unwrap_or ~default:[]
  ...
  | Map { map_record_decl; next } ->
      current @ (
        lookup_all_label next label_name
        |> List.map map_record_decl
      )
...
let rec lookup_owned = fun env owner_type_constructor_id ->
  ...
  | None -> (
      ...
      | Map { map_record_decl; next } ->
          lookup_owned next owner_type_constructor_id
          |> Option.map map_record_decl
    )
...
let visible_record_decls = fun env ->
  ...
  let rec gather acc env =
    let acc =
      current_record_decls env.current
      |> List.fold_left add_record_decl acc
    in
    match env.layer with
    | Nothing -> acc
    | Open { root; components; next } ->
