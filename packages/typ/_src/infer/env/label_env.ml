...
type components = { by_name: record_decl list Name_map.t; by_owner: owner_index }
...
let field = fun (record_decl: record_decl) label_name -> Name_map.find_opt (lookup_name label_name) record_decl.label_index

let field_names = fun (record_decl: record_decl) -> Name_map.bindings record_decl.label_index |> List.map fst
  ...
  | labels ->
      let label_index =
        labels |> List.fold_left
          (
            fun acc (label: TypeDecl.label) -> Name_map.add label.name label acc
          )
          Name_map.empty
      in
      Some {
  ...
  record_decl.labels |> List.fold_left
    (
      fun acc (label: TypeDecl.label) ->
        let existing = Name_map.find_opt label.name acc |> Option.unwrap_or ~default:[] in Name_map.add label.name (record_decl :: existing) acc
    )
    index

let current_of_record_decls = fun record_decls -> record_decls |> List.rev |> List.fold_left prepend_record_decl Name_map.empty

let owner_index_of_record_decls = fun record_decls ->
  record_decls |> List.fold_left
    (
      fun acc record_decl -> Owner_map.add record_decl.owner_type_constructor_id record_decl acc
    )
    Owner_map.empty
  ...
  Name_map.fold
    (
      fun label record_decls acc ->
        let current = Name_map.find_opt label acc |> Option.unwrap_or ~default:[] in Name_map.add label (current @ record_decls) acc
    )
    rest
    dominant
    ...
    (
      fun owner_id record_decl acc ->
        if Owner_map.mem owner_id acc then
          acc
        else Owner_map.add owner_id record_decl acc
    )
    rest
    dominant
...
let map_components = fun map_record_decl components -> { by_name = Name_map.map (List.map map_record_decl) components.by_name; by_owner = Owner_map.map map_record_decl components.by_owner }

let qualify_record_decl = fun root record_decl -> { record_decl with owner_path = SurfacePath.append_path root record_decl.owner_path }

let of_type_decls = fun type_decls ->
  let record_decls = type_decls |> List.filter_map record_decl_of_type_decl in { current = current_of_record_decls record_decls; by_owner = owner_index_of_record_decls record_decls; layer = Nothing }
...
let rec visible_components = fun env ->
  ...
  | Map { map_record_decl; next } -> current |> merge_visible_components (visible_components next |> map_components map_record_decl)
...
let local_only = fun env ->
  let record_decls = record_decls env in { current = current_of_record_decls record_decls; by_owner = owner_index_of_record_decls record_decls; layer = Nothing }

let map = fun map_record_decl env ->
  if is_empty env then
    env
  else { current = Name_map.empty; by_owner = Owner_map.empty; layer = Map { map_record_decl; next = env } }

let add_open = fun ~root opened env -> { current = Name_map.empty; by_owner = Owner_map.empty; layer = Open { root; components = visible_components opened; next = env } }

let merge_current = fun introduced existing ->
  Name_map.fold
    (
      fun label introduced_record_decls acc ->
        let current = Name_map.find_opt label acc |> Option.unwrap_or ~default:[] in Name_map.add label (introduced_record_decls @ current) acc
    )
    introduced
    existing

let merge_owner_index = fun introduced existing -> Owner_map.fold Owner_map.add introduced existing
...
let rec lookup_all_label = fun env label_name ->
  ...
  | Open { root; components; next } ->
      let opened = Name_map.find_opt label_name components.by_name |> Option.unwrap_or ~default:[] |> List.map (qualify_record_decl root) in (current @ opened) @ lookup_all_label next label_name
  | Map { map_record_decl; next } -> current @ (lookup_all_label next label_name |> List.map map_record_decl)
...
let rec lookup_owned = fun env owner_type_constructor_id ->
  ...
  | None -> (
    ...
    | Map { map_record_decl; next } -> lookup_owned next owner_type_constructor_id |> Option.map map_record_decl
  )
...
let visible_record_decls = fun env ->
  ...
  let rec gather acc env =
    ...
    | Open { root; components; next } ->
        ...
        in
        gather acc next
    | Map { map_record_decl; next } -> gather acc next |> List.map map_record_decl
  in
  gather [] env |> List.rev
