    ...
    let rec qualify_type ty =
      ...
      | TypeRepr.Named { head; arguments } ->
          ...
          let qualified_head =
            ...
            | Some type_decl ->
                { head with name = SurfacePath.append_path root (type_decl_key type_decl) }
            | None -> head
          in
          if
            Std.Ptr.equal head qualified_head
            && List.for_all2 Std.Ptr.equal arguments qualified_arguments
          then
            ty
          else
...
let summary_provenance_of_provenance = fun value ->
  ...
  | Binding.TypeConstructor { type_name; scope_path } ->
      Summary2.TypeConstructor { type_name; scope_path }
  ...
  | Binding.ModuleAlias { alias_name; module_path } ->
      Summary2.ModuleAlias { alias_name; module_path }
...
let rec insert_scope_at_path:
  module_table ->
  module_path:SurfacePath.t ->
  module_scope ->
  module_table = fun modules ~module_path introduced ->
...
let split_relative_binding = fun binding ->
  ...
  | Some (scope_path, name) when not (EntityId.is_empty scope_path) ->
      Some (EntityId.surface_path scope_path, Binding.with_path (EntityId.of_name name) binding)
  | _ -> None
...
let bind = fun (env: t) (introduced: t) ->
  {
    summary =
      (
        ...
        | (summary, Summary2.Empty) -> summary
...
let bind_in_scope = fun (env: t) ~scope_path (introduced: t) ->
  if SurfacePath.is_empty scope_path then
    ...
    let qualified_type_decls =
      Type_env.type_decls introduced.types
      |> List.map (qualify_type_decl scope_path)
    in
  ...
  |> List.fold_left
    (fun env (type_decl: FileSummary.type_decl) ->
      ...
      if SurfacePath.is_empty type_decl.scope_path then
        bind_scopes (module_scope_of_env env) (module_scope_of_env introduced)
        |> fun merged ->
          {
            env with
        ...
        let env =
          {
            env with
            constructors = Constructor_env.bind env.constructors qualified.constructors;
...
let lookup_all = fun env path ->
  if EntityId.is_bare path then
    ...
    | Some (module_path, name) -> (
        match lookup_module_scope env module_path with
        | Some scope ->
            Value_env.lookup_all scope.values name
  ...
  if SurfacePath.is_bare path then
    ...
    | Some (module_path, name) ->
        Option.and_then
          (lookup_module_scope env module_path)
          (fun scope ->
            Type_env.lookup scope.types name
            |> Option.map (qualify_type_decl module_path))
    | None -> None
...
let lookup_constructors = fun env path ->
  if SurfacePath.is_bare path then
    ...
    | Some (module_path, name) -> (
        match lookup_module_scope env module_path with
        | Some scope ->
            Constructor_env.lookup_all scope.constructors name
            |> List.map
...
let lookup_owned_constructor = fun env path owner_type_constructor_id ->
  ...
  if SurfacePath.is_bare path then
    ...
    | Some (module_path, name) -> (
        match lookup_module_scope env module_path with
        | Some scope ->
            lookup_local scope.constructors name
            |> Option.map
              (Constructor_env.qualify_entry
                ~root:module_path
                ~type_decls:(Type_env.type_decls scope.types))
...
let lookup_record_decls = fun env label_name ->
  Label_env.lookup_all env.labels (Label_env.lookup_name label_name)
  |> dedupe_record_decls
...
let rec scope_bindings_with_prefix: SurfacePath.t -> module_scope -> bindings = fun prefix scope ->
  let values =
    ...
    |> List.map (qualify_binding_with_scope ~root:prefix scope)
  in
...
let visible_type_decls = fun env ->
  Type_env.visible_type_decls env.types
  @ module_type_decls_with_prefix SurfacePath.empty env.modules

let types = fun env -> env.types

let record_decls = fun env ->
  Label_env.of_type_decls (type_decls env)
  |> Label_env.record_decls
  |> dedupe_record_decls
...
let singleton_module_scope = fun ~name scope -> {
  empty with
  modules = module_table_singleton { name; components = scope };
}
  ...
  | None ->
      cache_and_return
        ...
        (
          ...
          | Summary2.Bind (summary, introduced) ->
              bind (env_of_summary summary) (env_of_summary introduced)
          | Summary2.BindInScope (summary, scope_path, introduced) ->
              bind_in_scope (env_of_summary summary) ~scope_path (env_of_summary introduced)
          | Summary2.Open (summary, module_path) ->
              with_local_open (env_of_summary summary) module_path
  ...
  | None ->
      cache_and_return
        ...
        (
          ...
          | Summary2.Bind (summary, introduced) ->
              bind (env_of_summary_relative env summary) (env_of_summary introduced)
          | Summary2.BindInScope (summary, scope_path, introduced) ->
              bind_in_scope
          ...
          | Summary2.Open (summary, module_path) ->
              with_local_open (env_of_summary_relative env summary) module_path
          | Summary2.Qualify (summary, scope_path) ->
              qualify ~scope_path (env_of_summary_relative env summary)
        )
...
let register_entries = fun scope ~scope_path (env: t) ->
  ...
  {
    scope with
    locals = Path_map.add scope_path updated scope.locals;
    locals_cache = Path_map.empty;
  }
...
let for_item_scope = fun (env: t) scope ~scope_path ->
  ...
  scope_opens_for scope scope_path
  |> List.fold_left with_local_open base_env
