...
let hidden_name_set = fun (config: TypConfig.t) ->
  Collections.HashSet.of_list (List.map SurfacePath.to_string (TypConfig.hidden_export_names config))
...
let singleton_module_scope = fun ~name scope ->
  { empty with modules = module_table_singleton { name; components = scope } }
...
let register_entries = fun scope ~scope_path (env: t) ->
  ...
  in
  let updated = { summary = Summary2.bind existing.summary env.summary; env = bind existing.env env } in
