...
let prepend_entry = fun index entry ->
  let existing =
    Name_map.find_opt (name entry) index
...
let current_of_entries = fun entries ->
  entries
  |> List.rev
  |> List.fold_left prepend_entry Name_map.empty

let add_owner_entry = fun index entry ->
  let owner_entries =
    Owner_map.find_opt entry.owner_type_constructor_id index
    |> Option.unwrap_or ~default:Name_map.empty
  in
  let updated = Name_map.add (name entry) entry owner_entries in
  Owner_map.add entry.owner_type_constructor_id updated index

let owner_index_of_entries = fun entries ->
  entries
  |> List.fold_left add_owner_entry Owner_map.empty
...
let merge_visible_by_name = fun dominant rest ->
  Name_map.fold
    (fun entry_name rest_entries acc ->
      let current =
        Name_map.find_opt entry_name acc
        |> Option.unwrap_or ~default:[]
      in
...
let merge_visible_by_owner = fun dominant rest ->
  Owner_map.fold
    (fun owner_id rest_entries acc ->
      let current =
        Owner_map.find_opt owner_id acc
        |> Option.unwrap_or ~default:Name_map.empty
      in
      let merged =
        Name_map.fold
...
let qualify_entry = fun ~root ~type_decls entry ->
  ...
  in
  {
    ...
    constructor = {
      entry.constructor with
      scheme = qualified_scheme;
...
let current_entries = fun current ->
...
let rec visible_components = fun env ->
  ...
  | Map { map_entry; next } ->
      ...
      |> merge_visible_components
        (
          visible_components next
          |> map_components map_entry
        )
...
let rec lookup_all_name = fun env entry_name ->
  let current =
    Name_map.find_opt entry_name env.current
    |> Option.unwrap_or ~default:[]
  in
  match env.layer with
  | Nothing -> current
  | Open {
    ...
    next
      ...
      let opened =
        Name_map.find_opt entry_name components.by_name
        |> Option.unwrap_or ~default:[]
        |> List.map (qualify_entry ~root ~type_decls)
      in
      (current @ opened) @ lookup_all_name next entry_name
  | Map { map_entry; next } ->
      current @ (
        lookup_all_name next entry_name
        |> List.map map_entry
      )
...
let rec lookup_owned = fun env entry_name owner_type_constructor_id ->
  let lookup_local owner_index =
    Option.and_then
      (Owner_map.find_opt owner_type_constructor_id owner_index)
