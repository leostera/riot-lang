...
type components = { by_name: entry list Name_map.t; by_owner: owner_index }
...
and t = { current: current; by_owner: owner_index; layer: layer }
...
let prepend_entry = fun index entry ->
  let existing = Name_map.find_opt (name entry) index |> Option.unwrap_or ~default:[] in Name_map.add (name entry) (entry :: existing) index

let current_of_entries = fun entries -> entries |> List.rev |> List.fold_left prepend_entry Name_map.empty

let add_owner_entry = fun index entry ->
  let owner_entries = Owner_map.find_opt entry.owner_type_constructor_id index |> Option.unwrap_or ~default:Name_map.empty in
  let updated = Name_map.add (name entry) entry owner_entries in Owner_map.add entry.owner_type_constructor_id updated index
  ...
  Name_map.fold
    (
      fun entry_name rest_entries acc ->
        let current = Name_map.find_opt entry_name acc |> Option.unwrap_or ~default:[] in Name_map.add entry_name (current @ rest_entries) acc
    )
    rest
    dominant
            ...
            (
              fun entry_name entry acc ->
                if Name_map.mem entry_name acc then
                  acc
                else Name_map.add entry_name entry acc
            )
            rest_entries
            current
        in
...
let merge_visible_components = fun dominant rest -> { by_name = merge_visible_by_name dominant.by_name rest.by_name; by_owner = merge_visible_by_owner dominant.by_owner rest.by_owner }

let map_components = fun map_entry components -> { by_name = Name_map.map (List.map map_entry) components.by_name; by_owner = Owner_map.map (Name_map.map map_entry) components.by_owner }
...
let qualify_entry = fun ~root ~type_decls entry ->
  ...
  let qualified_inline_record_labels = entry.constructor.inline_record_labels |> Option.map (qualify_inline_record_labels ~root type_decls) in { owner_path = SurfacePath.append_path root entry.owner_path; owner_type_constructor_id = entry.owner_type_constructor_id; constructor = { entry.constructor with scheme = qualified_scheme; inline_record_labels = qualified_inline_record_labels } }
      ...
      (
        fun (type_decl: FileSummary.type_decl) ->
          ...
          type_decl.declaration.constructors |> List.map
            (
              fun constructor -> { owner_path; owner_type_constructor_id = type_decl.declaration.type_constructor_id; constructor }
            )
      )
  in
  { current = current_of_entries entries; by_owner = owner_index_of_entries entries; layer = Nothing }

let singleton = fun ~owner_path ~owner_type_constructor_id ~constructor ->
  let entry = { owner_path; owner_type_constructor_id; constructor } in { current = current_of_entries [ entry ]; by_owner = owner_index_of_entries [ entry ]; layer = Nothing }
...
let rec visible_components = fun env ->
  ...
  | Map { map_entry; next } -> current |> merge_visible_components (visible_components next |> map_components map_entry)
...
let local_only = fun env ->
  let entries = entries env in { current = current_of_entries entries; by_owner = owner_index_of_entries entries; layer = Nothing }

let map = fun map_entry env ->
  if is_empty env then
    env
  else { current = Name_map.empty; by_owner = Owner_map.empty; layer = Map { map_entry; next = env } }
  ...
  Name_map.fold
    (
      fun entry_name introduced_entries acc ->
        let current = Name_map.find_opt entry_name acc |> Option.unwrap_or ~default:[] in Name_map.add entry_name (introduced_entries @ current) acc
    )
    introduced
    existing
  ...
  Owner_map.fold
    (
      fun owner_id introduced_entries acc ->
        let current = Owner_map.find_opt owner_id acc |> Option.unwrap_or ~default:Name_map.empty in
        let merged = Name_map.fold Name_map.add introduced_entries current in Owner_map.add owner_id merged acc
    )
    introduced
    existing
...
let bind = fun env introduced ->
  if is_empty introduced then
    ...
    if is_empty env then
      introduced
    else { current = merge_current introduced.current env.current; by_owner = merge_owner_index introduced.by_owner env.by_owner; layer = env.layer }
...
let rec lookup_all_name = fun env entry_name ->
  ...
  | Open { root; type_decls; components; next } ->
      let opened = Name_map.find_opt entry_name components.by_name |> Option.unwrap_or ~default:[] |> List.map (qualify_entry ~root ~type_decls) in (current @ opened) @ lookup_all_name next entry_name
  | Map { map_entry; next } -> current @ (lookup_all_name next entry_name |> List.map map_entry)
...
let rec lookup_owned = fun env entry_name owner_type_constructor_id ->
  let lookup_local owner_index =
    Option.and_then (Owner_map.find_opt owner_type_constructor_id owner_index)
      (
        fun entries -> Name_map.find_opt entry_name entries
      )
  in
  ...
  | None -> (
    ...
    | Map { map_entry; next } -> lookup_owned next entry_name owner_type_constructor_id |> Option.map map_entry
  )
