...
type layer =
  ...
  | Map of {
      map_binding: Binding.t -> Binding.t;
      next: t;
    }
...
let current_of_bindings = fun bindings ->
  bindings
  |> List.rev
  |> List.fold_left prepend_binding Name_map.empty

let same_of_bindings = fun bindings ->
  bindings
  |> List.fold_left add_same Id_map.empty
...
let current_bindings = fun current ->
...
let rec find_same = fun env ident ->
  ...
  | None -> (
      ...
      | Map { map_binding; next } ->
          find_same next ident
          |> Option.map map_binding
    )
...
let rec lookup_all_name = fun env name ->
  let current =
    Name_map.find_opt name env.current
    |> Option.unwrap_or ~default:[]
  in
  match env.layer with
  | Nothing -> current
...
let export = fun config env ->
  let hidden_names =
    TypConfig.hidden_export_names config
