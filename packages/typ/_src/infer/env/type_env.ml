...
and t = { current: current; by_id: FileSummary.type_decl Id_map.t; layer: layer }
...
let prepend_decl = fun index type_decl ->
  ...
  let existing = Name_map.find_opt name index |> Option.unwrap_or ~default:[] in Name_map.add name (type_decl :: existing) index

let current_of_type_decls = fun type_decls -> type_decls |> List.rev |> List.fold_left prepend_decl Name_map.empty

let id_index_of_type_decls = fun type_decls ->
  type_decls |> List.fold_left
    (
      fun acc (type_decl: FileSummary.type_decl) -> Id_map.add type_decl.declaration.type_constructor_id type_decl acc
    )
    Id_map.empty
...
let components_of_type_decls = fun type_decls ->
  let by_name =
    type_decls |> List.rev |> List.fold_left
      (
        fun acc (type_decl: FileSummary.type_decl) -> Name_map.add (decl_name type_decl) type_decl acc
      )
      Name_map.empty
  in
  let by_id = id_index_of_type_decls type_decls in { by_name; by_id }

let current_visible_components = fun env ->
  {
    by_name = Name_map.fold
      (
    ...
    (
      fun name type_decl acc ->
        if Name_map.mem name acc then
          acc
        else Name_map.add name type_decl acc
    )
    rest
    dominant
    ...
    (
      fun id type_decl acc ->
        if Id_map.mem id acc then
          acc
        else Id_map.add id type_decl acc
    )
    rest
    dominant

let merge_visible_components = fun dominant rest -> { by_name = merge_visible_by_name dominant.by_name rest.by_name; by_id = merge_visible_by_id dominant.by_id rest.by_id }

let of_type_decls = fun type_decls -> { current = current_of_type_decls type_decls; by_id = id_index_of_type_decls type_decls; layer = Nothing }
...
let rec visible_components = fun env ->
  ...
  | Map { map_decl; next } ->
      let next_visible = visible_components next in current |> merge_visible_components { by_name = Name_map.map map_decl next_visible.by_name; by_id = Id_map.map map_decl next_visible.by_id }
...
let visible_type_decls =
  let rec collect env =
    let current =
      current_visible_components env |> fun components -> components.by_name
    in
    match env.layer with
    | Nothing -> current
    | Open { root; components; next } ->
        let opened = Name_map.map (qualify_type_decl root) components.by_name in current |> merge_visible_by_name opened |> merge_visible_by_name (collect next)
    | Map { map_decl; next } ->
        let next_visible = Name_map.map map_decl (collect next) in merge_visible_by_name current next_visible
  in
...
let map = fun map_decl env ->
  if is_empty env then
    env
  else { current = Name_map.empty; by_id = Id_map.empty; layer = Map { map_decl; next = env } }

let add_open = fun ~root opened env -> { current = Name_map.empty; by_id = Id_map.empty; layer = Open { root; components = visible_components opened; next = env } }

let merge_current = fun introduced existing ->
  Name_map.fold
    (
      fun name introduced_decls acc ->
        let current = Name_map.find_opt name acc |> Option.unwrap_or ~default:[] in Name_map.add name (introduced_decls @ current) acc
    )
    introduced
    existing
...
let bind = fun env introduced ->
  if is_empty introduced then
    ...
    if is_empty env then
      introduced
    else { current = merge_current introduced.current env.current; by_id = Id_map.fold Id_map.add introduced.by_id env.by_id; layer = env.layer }
  ...
  type_decls env |> List.filter_map
    (
      fun (type_decl: FileSummary.type_decl) ->
        SurfacePath.strip_prefix ~prefix:module_path type_decl.scope_path |> Option.map
          (
            fun scope_path -> { type_decl with scope_path }
          )
    ) |> of_type_decls

let entries_for_module_alias = fun env ~alias_name ~module_path -> entries_for_include env module_path |> qualify_entries (SurfacePath.of_name alias_name)
