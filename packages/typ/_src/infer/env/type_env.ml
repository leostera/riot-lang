...
type layer =
  ...
  | Map of {
      map_decl: FileSummary.type_decl -> FileSummary.type_decl;
      next: t;
    }
...
let current_of_type_decls = fun type_decls ->
  type_decls
  |> List.rev
  |> List.fold_left prepend_decl Name_map.empty
...
let current_type_decls = fun current ->
...
let visible_type_decls =
  let rec collect env =
    let current =
      current_visible_components env
      |> fun components -> components.by_name
    in
    match env.layer with
    | Nothing -> current
...
let entries_for_module_alias = fun env ~alias_name ~module_path ->
  ...
  |> qualify_entries (SurfacePath.of_name alias_name)
