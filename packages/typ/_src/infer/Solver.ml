...
let make_type = fun (solver: t) desc ->
  TypeRepr.of_desc desc
  |> track_node solver

let fresh_var = fun (solver: t) ->
  let id = solver.next_type_var_id in
  solver.next_type_var_id <- id + 1;
  TypeRepr.make_var ~level:(Region.current_level solver.regions) id
  |> track_node solver

let fresh_rigid_var = fun (solver: t) ->
  let id = solver.next_type_var_id in
  solver.next_type_var_id <- id + 1;
  TypeRepr.make_rigid_var ~level:(Region.current_level solver.regions) id
  |> track_node solver
              ...
              | TypeDecl.Invariant ->
                  if TypeRepr.level ty > boundary_level then
                    (
                      ...
                      Region.add_to_pool solver.regions ~level:boundary_level ty
                      |> ignore
                    )
            );
        let rest =
          ...
          | TypeRepr.Seq element -> (variance, element) :: rest
...
let exact_tags_contain_upper_tags = fun exact_tags upper_tags ->
  ...
  let rec loop payload_pairs exact_tags upper_tags =
    ...
    | ([], _ :: _) -> None
    | ((exact_tag: TypeRepr.poly_variant_tag) :: rest_exact, (upper_tag: TypeRepr.poly_variant_tag) :: rest_upper) -> (
...
let unify = fun (solver: t) ~left ~right ->
  ...
  let mark_pair_seen left right =
    ...
    if Collections.HashSet.contains seen_pairs key then
      ...
      (
        Collections.HashSet.insert seen_pairs key
        |> ignore;
        false
      )
  in
    ...
    | (left, right) :: rest ->
        ...
        if Std.Ptr.equal left right || mark_pair_seen left right then
              ...
              let rec add_value_pairs acc left_values right_values =
                ...
                | ((left_value: TypeRepr.package_value) :: left_rest, (
                  right_value: TypeRepr.package_value
                    ...
                    add_value_pairs
                      ((TypeScheme.body left_value.scheme, TypeScheme.body right_value.scheme)
                      :: acc)
                      left_rest
                      right_rest
                | _ -> None
          ...
          | (TypeRepr.PolyVariant { bound = left_bound; tags = left_tags; inherited = left_inherited }, TypeRepr.PolyVariant { bound = right_bound; tags = right_tags; inherited = right_inherited }) -> (
              ...
              | (TypeRepr.Exact, TypeRepr.UpperBound) when left_inherited = []
              && right_inherited = [] -> (
              ...
              | (TypeRepr.UpperBound, TypeRepr.Exact) when left_inherited = []
              && right_inherited = [] -> (
              ...
              | _ ->
                  ...
                  then
                    ...
                    (
                      match merge_poly_variant_tags left_tags right_tags with
                      | None when same_poly_variant_bound left_bound right_bound ->
                          Error (mismatch left right)
                      | Some (merged_tags, tag_pairs) ->
                          if
                            not
                              (List.length merged_tags = List.length left_tags
                              && List.length merged_tags = List.length right_tags)
                          then
                            Error (mismatch left right)
                          else
                            loop
          ...
          | (_, TypeRepr.Var { kind = Rigid; _ }) -> Error (mismatch left right)
          | (TypeRepr.Var var, _)
          | (_, TypeRepr.Var var) ->
              let (var_ty, other_ty) =
                match TypeRepr.view left with
              ...
              let lowered =
                TypeRepr.occurs_or_lower
                  ...
                  ~level
                  ~on_lower:(fun ty ->
                    Region.add_to_pool solver.regions ~level:(TypeRepr.level ty) ty
                    |> ignore)
                  other_ty
              in
              if lowered then
