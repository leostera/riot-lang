...
type generalization_group = { roots: TypeRepr.t list; expansive_roots: TypeRepr.t list }

type t = { regions: Region.t; mutable next_type_var_id: int }

let create = fun () -> { regions = Region.create (); next_type_var_id = 0 }

let next_mark = fun (solver: t) -> Region.next_mark solver.regions

let track_node = fun (solver: t) node -> Region.track_node solver.regions node
  ...
  let rec lower = function
    ...
    | (variance, ty) :: rest ->
        ...
        if not (Int.equal ty.mark generation) then
          ...
          let (should_process, variance) =
            ...
            | Some seen_variance ->
                ...
                if joined = seen_variance then
                  ...
                  (
                    let _ = Collections.HashMap.insert seen order joined in (true, joined)
                  )
            | None ->
                let _ = Collections.HashMap.insert seen order variance in (true, variance)
          in
          if not should_process || TypeRepr.level ty <= boundary_level then
            ...
            (
              match variance with
              | TypeDecl.Covariant -> ()
              | TypeDecl.Contravariant | TypeDecl.Invariant ->
        ...
        let rest =
          ...
          | TypeRepr.Int | TypeRepr.Float | TypeRepr.Bool | TypeRepr.String | TypeRepr.Char | TypeRepr.Unit | TypeRepr.Hole _ | TypeRepr.Var _ -> rest
          | TypeRepr.Option element | TypeRepr.List element | TypeRepr.Seq element -> (variance, element) :: rest
          | TypeRepr.Result (ok_ty, error_ty) -> (variance, ok_ty) :: (variance, error_ty) :: rest
          | TypeRepr.Array element -> (TypeDecl.Invariant, element) :: rest
          | TypeRepr.Named { head; arguments } ->
              ...
              let rec add_arguments acc arguments parameter_variances =
                ...
                | (argument :: rest_arguments, parameter_variance :: rest_variances) -> add_arguments ((TypeDecl.compose_variance variance parameter_variance, argument) :: acc) rest_arguments rest_variances
                | _ -> acc
              in
          ...
          | TypeRepr.Tuple members ->
              List.fold_left
                (
                  fun acc member -> (variance, member) :: acc
                )
                rest
                members
          | TypeRepr.Arrow { lhs; rhs; _ } -> (TypeDecl.flip_variance variance, lhs) :: (variance, rhs) :: rest
          | TypeRepr.Package signature ->
              List.fold_left
                (
                  fun acc (value: TypeRepr.package_value) -> (variance, TypeScheme.body value.scheme) :: acc
                )
                rest
                signature.values
        in
...
let with_local_level_gen = fun (solver: t) ~variance_of_named f ->
  Region.with_region_finalize solver.regions ~finalize:(
    fun frame (result, groups) ->
      let schemes = finalize_groups solver frame ~variance_of_named groups in (result, schemes)
  )
    (
      fun _frame -> f ()
    )

let instantiate = fun (solver: t) scheme -> TypeScheme.instantiate ~fresh_var:(
  fun () -> fresh_var solver
) ~make:(make_type solver) ~next_mark:(
  fun () -> next_mark solver
) scheme
...
let same_named_head = fun left right -> TypeRepr.(TypeConstructorId.equal left.type_constructor_id right.type_constructor_id)

let same_poly_variant_bound = fun left right ->
  match left, right with
  | (TypeRepr.Exact, TypeRepr.Exact) | (TypeRepr.UpperBound, TypeRepr.UpperBound) | (TypeRepr.LowerBound, TypeRepr.LowerBound) -> true
  | _ -> false
...
let merge_poly_variant_tags = fun left_tags right_tags ->
  let rec loop merged payload_pairs left_tags right_tags =
    ...
    | ((left_tag: TypeRepr.poly_variant_tag) :: rest_left, []) -> loop (left_tag :: merged) payload_pairs rest_left []
    | ([], (right_tag: TypeRepr.poly_variant_tag) :: rest_right) -> loop (right_tag :: merged) payload_pairs [] rest_right
    | ((left_tag: TypeRepr.poly_variant_tag) :: rest_left, (right_tag: TypeRepr.poly_variant_tag) :: rest_right) -> begin
      match String.compare left_tag.name right_tag.name with
      | 0 -> (
        match left_tag.payload_type, right_tag.payload_type with
        | (None, None) -> loop (left_tag :: merged) payload_pairs rest_left rest_right
        | (Some left_payload, Some right_payload) -> loop (({ left_tag with payload_type = Some left_payload }) :: merged) ((left_payload, right_payload) :: payload_pairs) rest_left rest_right
        | _ -> None
      )
      | order when order < 0 -> loop (left_tag :: merged) payload_pairs rest_left right_tags
      | _ -> loop (right_tag :: merged) payload_pairs left_tags rest_right
    end
  in
...
let exact_tags_contain_upper_tags = fun exact_tags upper_tags ->
  ...
  let rec loop payload_pairs exact_tags upper_tags =
    ...
    | ([], _ :: _) -> None
    | ((exact_tag: TypeRepr.poly_variant_tag) :: rest_exact, (upper_tag: TypeRepr.poly_variant_tag) :: rest_upper) -> begin
      ...
      | 0 -> (
        ...
        | (Some exact_payload, Some upper_payload) -> loop ((exact_payload, upper_payload) :: payload_pairs) rest_exact rest_upper
        | _ -> None
      )
      | order when order < 0 -> loop payload_pairs rest_exact upper_tags
      | _ -> None
    end
  in
  loop [] exact_tags upper_tags

let mismatch = fun left right -> Diagnostic.ExpectedActual { expected = TypePrinter.type_to_string left; actual = TypePrinter.type_to_string right }
...
let unify = fun (solver: t) ~left ~right ->
  ...
  let node_order ty =
    ...
    if Int.equal (TypeRepr.aux_mark ty) pair_generation then
      ...
      let order = next_node_order () in TypeRepr.set_aux_mark ty pair_generation;
        ...
        if Std.Ptr.equal left right || mark_pair_seen left right then
          ...
          | TypeRepr.Package left_signature, TypeRepr.Package right_signature ->
              let sort_values values =
                values |> List.sort
                  (
                    fun (left: TypeRepr.package_value) (right: TypeRepr.package_value) -> String.compare left.name right.name
                  )
              in
              ...
              let rec add_value_pairs acc left_values right_values =
                ...
                | ([], []) -> Some acc
                | ((left_value: TypeRepr.package_value) :: left_rest, (right_value: TypeRepr.package_value) :: right_rest) when String.equal left_value.name right_value.name -> add_value_pairs ((TypeScheme.body left_value.scheme, TypeScheme.body right_value.scheme) :: acc) left_rest right_rest
                | _ -> None
              in
              (
          ...
          | (TypeRepr.Hole _, _) | (_, TypeRepr.Hole _) -> loop rest
          | TypeRepr.Named { head = left_head; arguments = left_arguments; _ }, TypeRepr.Named { head = right_head; arguments = right_arguments; _ } ->
              if not (same_named_head left_head right_head) then
                Error (mismatch left right)
              else
                if List.length left_arguments != List.length right_arguments then
                  Error (mismatch left right)
                else loop (List.rev_append (List.combine left_arguments right_arguments) rest)
          | TypeRepr.PolyVariant { bound = left_bound; tags = left_tags; inherited = left_inherited }, TypeRepr.PolyVariant { bound = right_bound; tags = right_tags; inherited = right_inherited } -> begin
            ...
            | (TypeRepr.UpperBound, TypeRepr.UpperBound) when left_inherited = [] && right_inherited = [] -> (
              ...
              | Some (merged_tags, tag_pairs) ->
                  let merged = TypeRepr.poly_variant ~bound:TypeRepr.UpperBound ~tags:merged_tags ~inherited:[] in
                  let () = overwrite_type left merged in
                  let () = overwrite_type right merged in loop (List.rev_append tag_pairs rest)
            )
            | (TypeRepr.Exact, TypeRepr.UpperBound) when left_inherited = [] && right_inherited = [] -> (
              ...
              | None -> Error (mismatch left right)
              | Some tag_pairs ->
                  let merged = TypeRepr.poly_variant ~bound:TypeRepr.Exact ~tags:(sort_poly_variant_tags left_tags) ~inherited:[] in
                  let () = overwrite_type right merged in loop (List.rev_append tag_pairs rest)
            )
            | (TypeRepr.UpperBound, TypeRepr.Exact) when left_inherited = [] && right_inherited = [] -> (
              ...
              | None -> Error (mismatch left right)
              | Some tag_pairs ->
                  let merged = TypeRepr.poly_variant ~bound:TypeRepr.Exact ~tags:(sort_poly_variant_tags right_tags) ~inherited:[] in
                  let () = overwrite_type left merged in loop (List.rev_append tag_pairs rest)
            )
            | _ ->
                if not (same_poly_variant_bound left_bound right_bound) || List.length left_inherited != List.length right_inherited then
                  Error (mismatch left right)
                else
                  (
                    match merge_poly_variant_tags left_tags right_tags with
                    ...
                    | Some (merged_tags, tag_pairs) ->
                        if not (List.length merged_tags = List.length left_tags && List.length merged_tags = List.length right_tags) then
                          Error (mismatch left right)
                        else loop (List.rev_append tag_pairs (List.rev_append (List.combine left_inherited right_inherited) rest))
                    | None -> Error (mismatch left right)
                  )
          end
          ...
          | TypeRepr.Arrow { label = left_label; lhs = left_arg; rhs = left_res }, TypeRepr.Arrow { label = right_label; lhs = right_arg; rhs = right_res } ->
              if not (labels_match left_label right_label) then
                Error (mismatch left right)
              else loop ((left_arg, right_arg) :: (left_res, right_res) :: rest)
          | TypeRepr.Var left_var, TypeRepr.Var right_var when left_var.id = right_var.id -> loop rest
          ...
          | (TypeRepr.Var { kind = Rigid; _ }, _) | (_, TypeRepr.Var { kind = Rigid; _ }) -> Error (mismatch left right)
          | (TypeRepr.Var var, _) | (_, TypeRepr.Var var) ->
              let (var_ty, other_ty) =
                match TypeRepr.view left with
              ...
              let lowered = TypeRepr.occurs_or_lower ~generation:(next_mark solver) ~needle:var.id ~level ~on_lower:(
                fun ty -> Region.add_to_pool solver.regions ~level:(TypeRepr.level ty) ty |> ignore
              ) other_ty in
              if lowered then
                Error (Diagnostic.OccursCheckFailed { variable_id = var.id; in_type = TypePrinter.type_to_string other_ty })
              else
                (
