...
type named_owner = { type_constructor_id: TypeConstructorId.t }

type variance = TypeDecl.variance

type state = State.t

type pattern_bindings = { entries: Binding.t list; module_entries: (string * Env.t) list }
...
let env_of_compiled_scope = fun scope -> Env.bind (Env.of_entries ~make_id:BindingId.persistent ~provenance:Binding.Ambient (CompiledScope.exports scope)) (Env.of_type_decls (CompiledScope.type_decls scope)) |> Env.without_summary

let imported_visible_type_decls_for_module = fun (state: state) ~visible_path ~module_id -> ImportedWorld.visible_type_decls_for_module state.imported_world ~visible_path ~module_id

let imported_type_env_for_module = fun (state: state) ~visible_path ~module_id -> imported_visible_type_decls_for_module state ~visible_path ~module_id |> Env.of_type_decls
...
let lookup_imported_module_scope = fun (state: state) module_path -> ImportedWorld.lookup_module_scope state.imported_world module_path |> Option.map env_of_compiled_scope |> Option.map Env.module_scope_of_env

let lookup_module_scope = fun (state: state) env module_path ->
  Option.or_else (Env.lookup_module_scope env module_path)
    (
...
let with_local_open = fun (state: state) env module_path ->
  ...
  | Some scope -> Env.bind env (Env.entries_of_module_scope_for_include ~module_path scope |> Env.without_summary)
  | None -> env

let lookup_imported_binding = fun (state: state) path ->
  ImportedWorld.lookup_value state.imported_world path |> Option.map
    (
      fun scheme -> Binding.make ~id:(BindingId.persistent (EntityId.surface_path path)) ~surface_path:(EntityId.surface_path path) ~scheme ~provenance:Binding.Ambient
    )

let lookup_binding = fun (state: state) env path ->
  Option.or_else (Env.lookup env path)
    (
      fun () -> lookup_imported_binding state path
    )

let lookup_imported_type = fun (state: state) name -> ImportedWorld.lookup_type_decl state.imported_world name
...
let lookup_imported_constructors = fun (state: state) path ->
  if SurfacePath.is_bare path then
    ImportedWorld.implicit_open_modules state.imported_world |> List.concat_map
      (
        fun ({ ImportedWorld.visible_path; module_id }: ImportedWorld.opened_module) -> Env.lookup_constructors (imported_type_env_for_module state ~visible_path ~module_id) path
      )
  else
      ...
      (
        fun ({ ImportedWorld.visible_path; module_id; suffix }: ImportedWorld.resolved_module) ->
          if SurfacePath.is_empty suffix then
            []
          else Env.lookup_constructors (imported_type_env_for_module state ~visible_path ~module_id) path
    ...
    (
      fun () ->
        lookup_constructors state env path |> List.find_map
          (
            fun candidate ->
              if TypeConstructorId.equal owner_type_constructor_id (Env.Constructor_env.owner_type_constructor_id candidate) then
                Some candidate
              else None
          )
    )
      ...
      ImportedWorld.resolve_visible_module_prefix state.imported_world module_path |> Option.map
        (
          fun ({ ImportedWorld.visible_path; module_id; suffix }: ImportedWorld.resolved_module) ->
            if SurfacePath.is_empty suffix then
              []
            else Env.lookup_record_decls (imported_type_env_for_module state ~visible_path ~module_id) label_name
        ) |> Option.unwrap_or ~default:[]
  | None ->
      ImportedWorld.implicit_open_modules state.imported_world |> List.concat_map
        (
          fun ({ ImportedWorld.visible_path; module_id }: ImportedWorld.opened_module) -> Env.lookup_record_decls (imported_type_env_for_module state ~visible_path ~module_id) label_name
        )
    ...
    (
      fun () ->
        ImportedWorld.visible_modules state.imported_world |> List.find_map
          (
            fun (visible_path, module_id) -> Env.lookup_record_decl_by_owner (imported_type_env_for_module state ~visible_path ~module_id) owner_type_constructor_id
          )
    )
...
let has_surface_entries = fun state env visible_types module_path -> not (List.is_empty (Env.names (entries_for_include state env module_path))) || not (List.is_empty (type_decls_for_include visible_types module_path))
...
let resolve_module_path_in_scope = fun state env visible_types scope_path module_path ->
  if SurfacePath.is_empty scope_path || not (SurfacePath.is_bare module_path) then
    ...
    if has_surface_entries state env visible_types module_path then
      ...
      if has_surface_entries state env visible_types scoped_module_path then
        scoped_module_path
      else module_path
...
let fresh_binding_ident = fun state name -> BindingId.local ~stamp:(State.fresh_binding_local_id state) ~name

let binding_name_of_path = fun path -> SurfacePath.last_name path |> Option.unwrap_or ~default:(SurfacePath.to_string path)
...
let merge_pattern_bindings = fun left right -> { entries = left.entries @ right.entries; module_entries = left.module_entries @ right.module_entries }
...
let env_with_pattern_bindings = fun env bindings ->
  ...
  bindings.module_entries |> List.fold_left
    (
      fun env (name, module_env) -> Env.bind env (Env.singleton_module ~name module_env)
    )
    env

let scheme_body = TypeScheme.body

let prelude_env = fun (state: state) (config: TypConfig.t) -> Env.bind (Env.of_entries ~make_id:(
  fun path -> fresh_binding_ident state (binding_name_of_path path)
) ~provenance:Binding.Prelude config.prelude) (Env.of_type_decls LanguagePrelude.type_decls)
...
let pattern_binding = fun (state: state) pat_id ~name ~scheme -> Binding.make ~id:(fresh_binding_ident state name) ~surface_path:(SurfacePath.of_name name) ~scheme ~provenance:(Binding.LoweredPattern pat_id)

let generalized_pattern_binding = fun (state: state) pat_id ~name ty -> pattern_binding state pat_id ~name ~scheme:(TypeScheme.of_type ty)

let package_env = fun (state: state) pat_id (signature: TypeRepr.package_signature) ->
  let entries =
    signature.values |> List.map
      (
        fun (value: TypeRepr.package_value) -> pattern_binding state pat_id ~name:value.name ~scheme:value.scheme
      )
  in
  Env.of_bindings entries

let env_of_module_scope = fun scope -> Env.bind (Env.of_bindings (Env.Value_env.bindings (Env.scope_values scope))) (Env.of_type_decls (Env.Type_env.type_decls (Env.scope_types scope)))

let type_item_env = fun (_state: state) (type_item: ItemTree.type_item) -> Env.of_type_decls [ { FileSummary.scope_path = SurfacePath.empty; declaration = type_item.declaration } ]

let resolve_named_type_head_in_env = fun (state: state) env name ->
  lookup_type state env name |> Option.map
    (
    ...
    Option.or_else resolved
      (
        fun () -> State.resolve_named_type_head state name
      )
...
let canonicalize_type_decl_in_env = fun (state: state) env (type_decl: FileSummary.type_decl) ->
  ...
  let resolve_named_type_head name =
    ...
    else resolve_named_type_head_in_env state env name
  in
...
let constructor_bindings = fun (state: state) env ~name ~scheme ~provenance ~constructor_id ~inline_record_labels ->
  ...
  let inline_record_labels = inline_record_labels |> Option.map (VisibleTypes.canonicalize_inline_record_labels visible_types) in
  ...
  | TypeRepr.Named { head; _ } -> Env.singleton_constructor ~make_id:(
    fun path -> fresh_binding_ident state (binding_name_of_path path)
  ) ~name ~scheme ~provenance ~owner_path:head.name ~owner_type_constructor_id:head.type_constructor_id ~constructor_id ~inline_record_labels
  | _ -> Env.singleton ~make_id:(
    fun path -> fresh_binding_ident state (binding_name_of_path path)
  ) ~name ~scheme ~provenance

let exception_bindings = fun (state: state) env (exception_item: ItemTree.exception_item) -> constructor_bindings state env ~name:exception_item.exception_name ~scheme:exception_item.scheme ~provenance:(Binding.Exception { name = exception_item.exception_name; scope_path = exception_item.scope_path }) ~constructor_id:(ConstructorId.of_path (SurfacePath.append_name exception_item.scope_path exception_item.exception_name)) ~inline_record_labels:None

let extension_constructor_bindings = fun (state: state) env (extension_item: ItemTree.extension_constructor_item) -> constructor_bindings state env ~name:extension_item.constructor_name ~scheme:extension_item.scheme ~provenance:(Binding.DeclaredValue { name = extension_item.constructor_name; scope_path = extension_item.scope_path }) ~constructor_id:extension_item.constructor_id ~inline_record_labels:extension_item.inline_record_labels

let declared_value_bindings = fun (state: state) env (declared_value_item: ItemTree.declared_value_item) -> Env.singleton ~make_id:(
  fun path -> fresh_binding_ident state (binding_name_of_path path)
) ~name:declared_value_item.value_name ~scheme:(canonicalize_scheme_heads_in_env state env declared_value_item.scheme) ~provenance:(Binding.DeclaredValue { name = declared_value_item.value_name; scope_path = declared_value_item.scope_path })

let canonicalize_generalized_scheme_in_env = fun (state: state) env scheme -> canonicalize_scheme_heads_in_env state env scheme
  ...
  let rec loop ty =
    ...
    | TypeRepr.Option element ->
        ...
        if Std.Ptr.equal element copied_element then
          ty
        else make_type state (TypeRepr.Option copied_element)
    | TypeRepr.Result (ok_ty, error_ty) ->
        ...
        if Std.Ptr.equal ok_ty copied_ok_ty && Std.Ptr.equal error_ty copied_error_ty then
          ty
        else make_type state (TypeRepr.Result (copied_ok_ty, copied_error_ty))
    | TypeRepr.Array element ->
        let copied_element = loop element in
        if Std.Ptr.equal element copied_element then
          ty
        else make_type state (TypeRepr.Array copied_element)
    | TypeRepr.List element ->
        let copied_element = loop element in
        if Std.Ptr.equal element copied_element then
          ty
        else make_type state (TypeRepr.List copied_element)
    | TypeRepr.Seq element ->
        let copied_element = loop element in
        if Std.Ptr.equal element copied_element then
          ty
        else make_type state (TypeRepr.Seq copied_element)
    | TypeRepr.Named { head; arguments } ->
        ...
        if List.for_all2 Std.Ptr.equal arguments copied_arguments then
          ty
        else make_type state (TypeRepr.Named { head; arguments = copied_arguments })
              ...
              fun (tag: TypeRepr.poly_variant_tag) ->
                ...
                | Some payload_type ->
                    ...
                    if Std.Ptr.equal payload_type copied_payload_type then
                      tag
                    else { tag with payload_type = Some copied_payload_type }
                | None -> tag
            )
        in
        let copied_inherited = List.map loop inherited in
        if List.for_all2 Std.Ptr.equal tags copied_tags && List.for_all2 Std.Ptr.equal inherited copied_inherited then
          ty
        else make_type state (TypeRepr.PolyVariant { bound; tags = copied_tags; inherited = copied_inherited })
    | TypeRepr.Hole hole_id -> make_type state (TypeRepr.Hole hole_id)
    | TypeRepr.Tuple members ->
        ...
        if List.for_all2 Std.Ptr.equal members copied_members then
          ty
        else make_type state (TypeRepr.Tuple copied_members)
    | TypeRepr.Arrow { label; lhs; rhs } ->
        ...
        if Std.Ptr.equal lhs copied_lhs && Std.Ptr.equal rhs copied_rhs then
          ty
        else make_type state (TypeRepr.Arrow { label; lhs = copied_lhs; rhs = copied_rhs })
    | TypeRepr.Package signature ->
        ...
        if List.for_all2 Std.Ptr.equal signature.values copied_values then
          ty
        else TypeRepr.package ~values:copied_values
    | TypeRepr.Var var -> (
      match var.link with
  ...
  let rec loop ty =
    ...
    | TypeRepr.Option element ->
        ...
        if Std.Ptr.equal element normalized_element then
          ty
        else make_type state (TypeRepr.Option normalized_element)
    | TypeRepr.Result (ok_ty, error_ty) ->
        ...
        if Std.Ptr.equal ok_ty normalized_ok_ty && Std.Ptr.equal error_ty normalized_error_ty then
          ty
        else make_type state (TypeRepr.Result (normalized_ok_ty, normalized_error_ty))
    | TypeRepr.Array element ->
        let normalized_element = loop element in
        if Std.Ptr.equal element normalized_element then
          ty
        else make_type state (TypeRepr.Array normalized_element)
    | TypeRepr.List element ->
        let normalized_element = loop element in
        if Std.Ptr.equal element normalized_element then
          ty
        else make_type state (TypeRepr.List normalized_element)
    | TypeRepr.Seq element ->
        let normalized_element = loop element in
        if Std.Ptr.equal element normalized_element then
          ty
        else make_type state (TypeRepr.Seq normalized_element)
    | TypeRepr.Named { head; arguments } ->
        let normalized_arguments = List.map loop arguments in
        if List.for_all2 Std.Ptr.equal arguments normalized_arguments then
          ty
        else make_type state (TypeRepr.Named { head; arguments = normalized_arguments })
    | TypeRepr.PolyVariant { bound; tags; inherited } ->
        let normalized_tags =
          tags |> List.map
            (
              fun (tag: TypeRepr.poly_variant_tag) ->
                ...
                | Some payload_type ->
                    ...
                    if Std.Ptr.equal payload_type normalized_payload_type then
                      tag
                    else { tag with payload_type = Some normalized_payload_type }
                | None -> tag
            )
        in
        let normalized_inherited = List.map loop inherited in
        if List.for_all2 Std.Ptr.equal tags normalized_tags && List.for_all2 Std.Ptr.equal inherited normalized_inherited then
          ty
        else make_type state (TypeRepr.PolyVariant { bound; tags = normalized_tags; inherited = normalized_inherited })
    | TypeRepr.Tuple members ->
        let normalized_members = List.map loop members in
        if List.for_all2 Std.Ptr.equal members normalized_members then
          ty
        else make_type state (TypeRepr.Tuple normalized_members)
    | TypeRepr.Arrow { label; lhs; rhs } ->
        ...
        if Std.Ptr.equal lhs normalized_lhs && Std.Ptr.equal rhs normalized_rhs then
          ty
        else make_type state (TypeRepr.Arrow { label; lhs = normalized_lhs; rhs = normalized_rhs })
    | TypeRepr.Package signature ->
            ...
            (
              fun (value: TypeRepr.package_value) ->
                ...
                if Std.Ptr.equal value.scheme normalized_scheme then
                  value
                else { value with scheme = normalized_scheme }
            )
        in
        if List.for_all2 Std.Ptr.equal signature.values normalized_values then
          ty
        else TypeRepr.package ~values:normalized_values
...
let instantiate_rigid_scheme = fun (state: state) scheme -> TypeScheme.instantiate ~fresh_var:(
  fun () -> fresh_rigid_var state
) ~make:(make_type state) ~next_mark:(
  fun () -> Solver.next_mark state.solver
) scheme

let canonicalize_type_in_env = fun (state: state) env ty ->
  let canonicalized = State.canonicalize_type_with_name_resolution ~resolve_named_type_decl:(resolve_named_type_decl_in_env state env) ~resolve_named_type_head:(
    fun name -> resolve_named_type_head_in_env state env name
  ) ty in
...
let argument_matches_parameter_label = fun parameter_label argument_label ->
  ...
  | (TypeRepr.Optional expected, BodyArena.Optional actual) | (TypeRepr.Optional expected, BodyArena.Labeled actual) -> String.equal expected actual
  | _ -> false
...
let take_matching_argument = fun parameter_label arguments ->
  let rec loop prefix = function
    ...
    | ((argument: BodyArena.apply_argument) as candidate) :: rest ->
        ...
        else loop (candidate :: prefix) rest
  in
  loop [] arguments

let visible_type_decl = fun (state: state) name -> State.visible_type_decl state name

let visible_type_decl_by_id = fun (state: state) type_constructor_id -> State.visible_type_decl_by_id state type_constructor_id
...
let resolve_constructor_entry = fun (state: state) env constructor ~expected_ty ->
  ...
  | Some expected_owner -> (
    ...
    | None -> (
      match List.filter
        (
          fun candidate -> TypeConstructorId.equal expected_owner.type_constructor_id (Env.Constructor_env.owner_type_constructor_id candidate)
        )
        candidates with
...
let record_decl_matches_owner = fun (record_decl: record_type_decl) owner -> TypeConstructorId.equal (Env.Label_env.owner_type_constructor_id record_decl) owner.type_constructor_id
...
let instantiate_record_decl = fun (state: state) (record_decl: record_type_decl) ->
  ...
  let () =
    Env.Label_env.param_ids record_decl |> List.iter
      (
        fun quantified_id ->
  ...
  let owner_ty =
    make_type state
      (
        TypeRepr.Named {
          head = TypeRepr.named_head ~type_constructor_id:(Env.Label_env.owner_type_constructor_id record_decl) ~name:owner_path;
          arguments = Env.Label_env.param_ids record_decl |> List.map
            (
              fun quantified_id ->
  ...
  in
  let field_types =
    Env.Label_env.labels record_decl |> List.fold_left
      (
...
let record_field_type = fun field_types label_name -> Label_name_map.find_opt (Env.Label_env.lookup_name label_name) field_types

let type_decl_path = fun (type_decl: FileSummary.type_decl) -> qualify_name type_decl.scope_path type_decl.declaration.type_name

let instantiate_named_type_decl = fun (state: state) (type_decl: FileSummary.type_decl) ->
  let mapping = Collections.HashMap.with_capacity 8 in
  let arguments =
    type_decl.declaration.param_ids |> List.map
      (
        fun param_id ->
          let argument = fresh_var state in
          let _ = Collections.HashMap.insert mapping param_id argument in argument
      )
  in
  (make_type state (TypeRepr.Named { head = TypeRepr.named_head ~type_constructor_id:type_decl.declaration.type_constructor_id ~name:(type_decl_path type_decl); arguments }), mapping)

let instantiate_poly_variant_payload = fun (state: state) mapping (payload_type: TypeRepr.t option) ->
  payload_type |> Option.map
    (
      fun payload_type -> substitute_type_vars state payload_type mapping
    )
...
let poly_variant_candidate = fun (state: state) (type_decl: FileSummary.type_decl) ->
    ...
    if Collections.HashSet.contains visited type_constructor_id then
      ...
      | Some (TypeDecl.PolyVariant { tags; inherited; _ }) ->
          let acc =
            tags |> List.fold_left
              (
                fun acc (tag: TypeDecl.poly_variant_tag) -> Label_name_map.add tag.name tag.payload_type acc
              )
              acc
          in
  ...
  in
  match type_decl.declaration.manifest with
  | Some (TypeDecl.PolyVariant { bound; _ }) -> Some { type_decl; bound; payloads = collect (Collections.HashSet.create ()) Label_name_map.empty type_decl }
  | _ -> None
  ...
  visible_type_decls state |> List.filter_map (poly_variant_candidate state) |> List.filter
    (
      fun candidate ->
        tags |> List.for_all
          (
            fun tag -> Label_name_map.mem tag candidate.payloads
          )
    ) |> List.sort compare_poly_variant_candidates |> function
    | candidate :: _ -> Some candidate
    | [] -> None
    ...
    (
      fun tag ->
        if Collections.HashSet.contains seen tag then
          ...
          let () = Collections.HashSet.insert seen tag |> ignore in true
    )
          ...
          | Some (best_candidate, best_score) ->
              if candidate_score > best_score then
                ...
                if candidate_score < best_score then
                  ...
                  if compare_poly_variant_candidates candidate best_candidate < 0 then
                    Some (candidate, candidate_score)
                  else best
    )
    None |> Option.map fst
...
let poly_variant_candidate_for_type = fun (state: state) ty ->
  ...
  | TypeRepr.Named { head = { type_constructor_id; _ }; _ } -> Option.and_then (visible_type_decl_by_id state type_constructor_id) (poly_variant_candidate state)
  | _ -> None

let poly_variant_payloads_compatible = fun (state: state) left right ->
  match left, right with
  | (None, None) -> true
  | (Some left, Some right) -> String.equal (TypePrinter.type_to_string (canonicalize_type state left)) (TypePrinter.type_to_string (canonicalize_type state right))
  | _ -> false

let can_explicitly_coerce_poly_variant = fun (state: state) ~source_ty ~target_ty ->
  match poly_variant_candidate_for_type state source_ty, poly_variant_candidate_for_type state target_ty with
...
let anonymous_poly_variant_type = fun (state: state) tag payload_type -> make_type state (TypeRepr.PolyVariant { bound = TypeRepr.UpperBound; tags = [ TypeRepr.poly_variant_tag ?payload_type tag ]; inherited = [] })
...
let is_unresolved_type = fun ty ->
  match view ty with
  | TypeRepr.Var _ | TypeRepr.Hole _ -> true
  | _ -> false
...
let constrain_match_scrutinee_to_poly_variant = fun (state: state) scrutinee_ty cases ->
  let tags =
    cases |> List.filter_map
      (
        fun (case: BodyArena.match_case) -> top_level_poly_variant_tag state case.pattern_id
      )
  in
...
let add_record_resolution_error = fun (state: state) ~span ~context reason -> add_diagnostic state (Typ_diagnostic.RecordResolutionError { operation_span = span; context; reason })

let add_or_pattern_bindings_mismatch = fun (state: state) ~span ~expected_names ~actual_names -> add_diagnostic state (Typ_diagnostic.OrPatternBindingsMismatch { pattern_span = span; expected_names; actual_names })
...
let resolve_record_decl = fun env (state: state) ~field_names ~owner_hint ~span ~context ->
  ...
  let candidates =
    ...
    | Some owner -> (
      ...
      | Some record_decl when Env.Label_env.matches_fields record_decl field_names -> [ record_decl ]
      | _ ->
  ...
  let candidates =
    candidates |> List.filter
      (
        fun (record_decl: record_type_decl) -> record_decl_matches_explicit_field_owners record_decl field_names
      )
  in
  match candidates with
  ...
  | [] ->
      let unknown_labels =
        field_names |> List.filter
          (
            fun field_name -> List.is_empty (lookup_record_decls state env field_name)
          )
      in
      let reason =
        if not (List.is_empty unknown_labels) then
          Typ_diagnostic.UnknownRecordLabels unknown_labels
        else Typ_diagnostic.IncompatibleRecordLabels field_names
      in
      let () = add_record_resolution_error state ~span ~context reason in None
  | _ ->
      let () = add_record_resolution_error state ~span ~context (Typ_diagnostic.AmbiguousRecordLabels field_names) in None
...
let rec decompose_arrow_type = fun ty ->
  ...
  | TypeRepr.Arrow { lhs; rhs; _ } ->
      let (arguments, result_ty) = decompose_arrow_type rhs in (lhs :: arguments, result_ty)
  | _ -> [], ty
...
let instantiate_constructor_entry = fun ?(pattern_mode = false) (state: state) env constructor_entry ->
  ...
  let inline_record_labels =
    Env.Constructor_env.inline_record_labels constructor_entry |> Option.map
      (
        List.map
          (
...
let field_types_of_labels = fun (state: state) labels ->
  labels |> List.fold_left
    (
      fun acc (label: TypeDecl.label) -> Label_name_map.add (Env.Label_env.lookup_name label.name) label.field_type acc
    )
    Label_name_map.empty
    ...
    (
      fun (label: TypeDecl.label) ->
        if List.exists
          ...
          field_names then
          None
        else Some label.name
    )

let pattern_name = fun path -> SurfacePath.last_name path |> Option.unwrap_or ~default:(SurfacePath.to_string path)
...
let coverage_witness_of_constructor = fun (constructor: coverage_constructor) arguments ->
  ...
  | CoverageOptionNone | CoverageOptionSome | CoverageResultOk | CoverageResultError | CoverageListNil | CoverageListCons | CoverageVariant _ -> Typ_diagnostic.ConstructorWitness { name = constructor.name; arguments }

let rec coverage_product = function
  | [] -> [ [] ]
  | patterns :: rest ->
      let rest = coverage_product rest in
      patterns |> List.concat_map
        (
          fun pattern ->
            rest |> List.map
              (
                fun suffix -> pattern :: suffix
              )
        )
...
let supported_coverage_constructors_for_named_type = fun (state: state) head arguments ->
  ...
  | Some type_decl when List.is_empty type_decl.declaration.labels && not (List.is_empty type_decl.declaration.constructors) ->
      let mapping = Collections.HashMap.with_capacity 8 in
      let () =
        List.iter2
          (
        ...
        type_decl.declaration.constructors |> List.map
          (
            fun (constructor: TypeDecl.constructor) ->
              ...
              let (argument_types, _result_ty) = decompose_arrow_type specialized in make_coverage_constructor ~key:(CoverageVariant constructor.constructor_id) ~name:constructor.name ~argument_types
          )
      )
  | _ -> None
...
let supported_coverage_constructors_for_type = fun (state: state) ty ->
  match view ty with
  | TypeRepr.Bool -> Some [ make_coverage_constructor ~key:(CoverageBool false) ~name:"false" ~argument_types:[]; make_coverage_constructor ~key:(CoverageBool true) ~name:"true" ~argument_types:[] ]
  | TypeRepr.Unit -> Some [ make_coverage_constructor ~key:CoverageUnit ~name:"()" ~argument_types:[] ]
  | TypeRepr.Tuple element_types -> Some [ make_coverage_constructor ~key:(CoverageTuple (List.length element_types)) ~name:"tuple" ~argument_types:element_types ]
  | TypeRepr.Option element_ty -> Some [ make_coverage_constructor ~key:CoverageOptionNone ~name:"None" ~argument_types:[]; make_coverage_constructor ~key:CoverageOptionSome ~name:"Some" ~argument_types:[ element_ty ] ]
  | TypeRepr.Result (ok_ty, error_ty) -> Some [ make_coverage_constructor ~key:CoverageResultOk ~name:"Ok" ~argument_types:[ ok_ty ]; make_coverage_constructor ~key:CoverageResultError ~name:"Error" ~argument_types:[ error_ty ] ]
  | TypeRepr.List element_ty -> Some [ make_coverage_constructor ~key:CoverageListNil ~name:"[]" ~argument_types:[]; make_coverage_constructor ~key:CoverageListCons ~name:"::" ~argument_types:[ element_ty; ty ] ]
  | TypeRepr.Named { head; arguments } -> supported_coverage_constructors_for_named_type state head arguments
  | TypeRepr.Int | TypeRepr.Float | TypeRepr.String | TypeRepr.Char | TypeRepr.Array _ | TypeRepr.Seq _ | TypeRepr.Package _ | TypeRepr.PolyVariant _ | TypeRepr.Arrow _ | TypeRepr.Var _ | TypeRepr.Hole _ -> None

let supported_constructor_for_pattern = fun (state: state) expected_ty constructor_name argument_count ->
  Option.and_then (supported_coverage_constructors_for_type state expected_ty)
    (
      fun constructors ->
        constructors |> List.find_opt
          (
            fun constructor -> String.equal constructor.name constructor_name && Int.equal (coverage_constructor_arity constructor) argument_count
          )
    )

let rec coverage_patterns_of_pattern = fun (state: state) pat_id expected_ty ->
  let combine_with_constructor constructor pattern_groups =
    coverage_product pattern_groups |> List.map
      (
        fun arguments -> CoverageConstructor { constructor; arguments }
      )
  in
  ...
  | Some pattern -> (
    ...
    | BodyArena.PVar _ | BodyArena.PWildcard -> Some [ CoverageAny ]
    ...
    | BodyArena.PTuple elements -> (
      ...
      | Some [ constructor ] when (
        ...
        | _ -> false
      ) -> translate_many elements constructor.argument_types |> Option.map (combine_with_constructor constructor)
      | _ -> None
    )
    ...
    | BodyArena.PConstructor { constructor; arguments } -> (
      match supported_constructor_for_pattern state expected_ty (pattern_name constructor) (List.length arguments) with
      | Some constructor -> translate_many arguments constructor.argument_types |> Option.map (combine_with_constructor constructor)
      | None -> None
    )
    | BodyArena.PList elements -> (
      match view expected_ty with
      | TypeRepr.List element_ty ->
          let nil_constructor = make_coverage_constructor ~key:CoverageListNil ~name:"[]" ~argument_types:[] in
          let cons_constructor = make_coverage_constructor ~key:CoverageListCons ~name:"::" ~argument_types:[ element_ty; expected_ty ] in
          let rec loop = function
            | [] -> Some [ CoverageConstructor { constructor = nil_constructor; arguments = [] } ]
            | element_id :: rest -> (
              match coverage_patterns_of_pattern state element_id element_ty, loop rest with
              | (Some element_patterns, Some rest_patterns) ->
                  Some (
                    element_patterns |> List.concat_map
                      (
                        fun head ->
                          rest_patterns |> List.map
                            (
                              fun tail -> CoverageConstructor { constructor = cons_constructor; arguments = [ head; tail ] }
                            )
                      )
                  )
              | _ -> None
    ...
    | BodyArena.PAlias { pattern_id; _ } -> coverage_patterns_of_pattern state pattern_id expected_ty
    | BodyArena.PInt _ | BodyArena.PFloat _ | BodyArena.PString _ | BodyArena.PChar _ | BodyArena.PRecord _ | BodyArena.PFirstClassModule _ | BodyArena.PPolyVariant _ | BodyArena.PUnsupported _ -> None
  )
...
let specialize_coverage_matrix = fun constructor rows -> rows |> List.filter_map (specialize_coverage_row constructor)
...
let rec useful_coverage_vector = fun (state: state) rows patterns expected_types ->
  ...
  | (_ :: _, _) when (
    rows |> List.exists
      (
        fun row -> List.length row = List.length patterns && coverage_row_is_all_any row
      )
  ) -> None
  | (CoverageAny :: pattern_rest, _ :: type_rest) when (
    rows |> List.for_all
      ...
      useful_coverage_vector state (default_coverage_matrix rows) pattern_rest type_rest |> Option.map
        (
          fun witness -> Typ_diagnostic.WildcardWitness :: witness
        )
        ...
        constructors |> List.find_map
          (
            fun constructor ->
              ...
              useful_coverage_vector state specialized_rows specialized_patterns specialized_types |> Option.map
                (
                  fun witness ->
                    let (arguments, tail) = split_prefix arity witness in coverage_witness_of_constructor constructor arguments :: tail
                )
          )
    | None ->
        useful_coverage_vector state (default_coverage_matrix rows) pattern_rest type_rest |> Option.map
  ...
  | ((CoverageConstructor { constructor; arguments }) :: pattern_rest, expected_ty :: type_rest) ->
      ...
      useful_coverage_vector state specialized_rows specialized_patterns specialized_types |> Option.map
        (
          fun witness ->
            let (arguments, tail) = split_prefix (coverage_constructor_arity constructor) witness in coverage_witness_of_constructor constructor arguments :: tail
        )
  | _ -> None
...
let rec is_nonexpansive_expr = fun (state: state) expr_id ->
  ...
  | Some expr ->
      ...
      | BodyArena.EVar _ | BodyArena.EInt _ | BodyArena.EFloat _ | BodyArena.EBool _ | BodyArena.EString _ | BodyArena.EChar _ | BodyArena.EUnit -> true
      | BodyArena.ETuple element_ids -> List.for_all (is_nonexpansive_expr state) element_ids
      | BodyArena.EFun _ -> true
      | BodyArena.EModulePack _ | BodyArena.ELocalModulePack _ -> true
      | BodyArena.EApply (callee_id, arguments) ->
          let arguments_nonexpansive =
            arguments |> List.for_all
              (
                fun (argument: BodyArena.apply_argument) -> is_nonexpansive_expr state argument.value_id
              )
          in
          if not arguments_nonexpansive then
            ...
            (
              ...
              | Some "constructor_apply_expression" | Some "list_literal_apply" -> true
              | Some "infix_expression" -> (
                match SemanticTree.find_expr state.file callee_id with
                | Some { desc = BodyArena.EVar name; _ } when SurfacePath.equal name (SurfacePath.of_name "::") -> true
                | _ -> false
              )
              | _ -> false
            )
      ...
      | BodyArena.ERecord { base_id = Some _; _ } -> false
      | BodyArena.EFieldAccess { receiver_id; _ } -> is_nonexpansive_expr state receiver_id
      | BodyArena.EFieldAssign _ -> false
      | BodyArena.EIndex _ | BodyArena.EArray _ | BodyArena.ESequence _ | BodyArena.EWhile _ | BodyArena.EFor _ | BodyArena.EUnsupported _ | BodyArena.EHole _ -> false
      | BodyArena.ELet (binding_ids, body_id) -> List.for_all (is_nonexpansive_binding state) binding_ids && is_nonexpansive_expr state body_id
      | BodyArena.ELocalModule { local_scope; body_id; _ } ->
          List.for_all
            (
              fun (group: BodyArena.local_module_binding_group) -> List.for_all (is_nonexpansive_binding state) group.binding_ids
            )
            local_scope.binding_groups && is_nonexpansive_expr state body_id
      | BodyArena.EIf (condition_id, then_id, else_id) -> is_nonexpansive_expr state condition_id && is_nonexpansive_expr state then_id && is_nonexpansive_expr state else_id
      | BodyArena.EMatch (scrutinee_id, cases) ->
          is_nonexpansive_expr state scrutinee_id && List.for_all
            ...
            cases
      | BodyArena.ETry _ -> false
      | BodyArena.EPolyVariant { payload; _ } -> (
        match payload with
        | Some payload_id -> is_nonexpansive_expr state payload_id
        | None -> true
      )
      | BodyArena.ECoerce { value_id; _ } -> is_nonexpansive_expr state value_id
      | BodyArena.ELocalOpen { body_id; _ } -> is_nonexpansive_expr state body_id
...
let solver_group_for_entries = fun (state: state) expr_id root_ty entries ->
  ...
  if is_nonexpansive_expr state expr_id then
    Solver.group roots
  else Solver.group ~expansive_roots:[ root_ty ] roots
...
let origin_of_binding = fun (state: state) (binding: BodyArena.binding) -> SemanticTree.find_origin state.file binding.origin_id
...
let merge_exact_poly_variant_tags = fun (state: state) left_tags right_tags ->
  ...
  let insert_tag (tag: TypeRepr.poly_variant_tag) =
    ...
    | None ->
        let _ = Collections.HashMap.insert merged tag.name tag in Ok ()
    | Some existing_tag -> (
    ...
    | Ok () ->
        Some (
          Collections.HashMap.to_list merged |> List.map snd |> List.sort
            (
              fun (left: TypeRepr.poly_variant_tag) (right: TypeRepr.poly_variant_tag) -> String.compare left.name right.name
            )
        )
  )
...
let merge_poly_variant_accumulator = fun (state: state) left right ->
  let merge_into_var var linked other =
    ...
    | (Some linked_tags, Some other_tags) -> (
      ...
      | Some merged_tags ->
          let merged_ty = make_type state (TypeRepr.PolyVariant { bound = TypeRepr.Exact; tags = merged_tags; inherited = [] }) in
          let () = var.TypeRepr.link <- Some merged_ty in true
      | None -> false
    )
    | _ -> false
  in
...
let try_unify = fun (state: state) ~origin left right ->
  try
    ...
    ()
  with
  | Unify_error mismatch -> add_diagnostic state (Typ_diagnostic.TypeMismatch { mismatch_span = diagnostic_span origin; mismatch })
...
let unify_gadt = fun (state: state) ~origin left right ->
  let rec loop = function
    ...
    | (left, right) :: rest ->
        ...
        if Std.Ptr.equal left right then
          ...
          match TypeRepr.view left, TypeRepr.view right with
          | (TypeRepr.Int, TypeRepr.Int) | (TypeRepr.Float, TypeRepr.Float) | (TypeRepr.Bool, TypeRepr.Bool) | (TypeRepr.String, TypeRepr.String) | (TypeRepr.Char, TypeRepr.Char) | (TypeRepr.Unit, TypeRepr.Unit) -> loop rest
          | (TypeRepr.Option left_element, TypeRepr.Option right_element) | (TypeRepr.Array left_element, TypeRepr.Array right_element) | (TypeRepr.List left_element, TypeRepr.List right_element) | (TypeRepr.Seq left_element, TypeRepr.Seq right_element) -> loop ((left_element, right_element) :: rest)
          | (TypeRepr.Result (left_ok, left_error), TypeRepr.Result (right_ok, right_error)) -> loop ((left_ok, right_ok) :: (left_error, right_error) :: rest)
          | (TypeRepr.Named { head = left_head; arguments = left_arguments }, TypeRepr.Named { head = right_head; arguments = right_arguments }) ->
              if not (TypeConstructorId.equal left_head.type_constructor_id right_head.type_constructor_id) || List.length left_arguments != List.length right_arguments then
                Error (Diagnostic.ExpectedActual { expected = TypePrinter.type_to_string right; actual = TypePrinter.type_to_string left })
              else loop (List.rev_append (List.combine left_arguments right_arguments) rest)
          | (TypeRepr.Tuple left_members, TypeRepr.Tuple right_members) ->
              ...
              else loop (List.rev_append (List.combine left_members right_members) rest)
          | (TypeRepr.Arrow { label = left_label; lhs = left_lhs; rhs = left_rhs }, TypeRepr.Arrow { label = right_label; lhs = right_lhs; rhs = right_rhs }) ->
              if not (labels_match left_label right_label) then
                Error (Diagnostic.ExpectedActual { expected = TypePrinter.type_to_string right; actual = TypePrinter.type_to_string left })
              else loop ((left_lhs, right_lhs) :: (left_rhs, right_rhs) :: rest)
  ...
  | Ok () -> ()
  | Error mismatch ->
      add_diagnostic state (Typ_diagnostic.TypeMismatch { mismatch_span = diagnostic_span origin; mismatch });
      ()
...
let check_module_pack_against_signature = fun (state: state) env ~origin module_path ((signature: TypeRepr.package_signature)) ->
      ...
      signature.values |> List.iter
        (
          fun (value: TypeRepr.package_value) ->
            ...
            | Some binding ->
                let actual_ty = instantiate state (canonicalize_scheme_in_env state scope_env (Binding.scheme binding)) in
                let expected_ty = TypeScheme.body value.scheme in try_unify state ~origin actual_ty expected_ty
            | None -> add_diagnostic state (Typ_diagnostic.UnboundName { reference_span = diagnostic_span origin; name = SurfacePath.append_name module_path value.name |> SurfacePath.to_string })
        )
  | None -> add_diagnostic state (Typ_diagnostic.UnboundName { reference_span = diagnostic_span origin; name = SurfacePath.to_string module_path })
...
let rec constructor_pattern_argument_types = fun (state: state) constructor_ty arguments origin ->
  ...
  | _ :: rest ->
      ...
      let () = try_unify state ~origin constructor_ty (make_type state (TypeRepr.Arrow { label = TypeRepr.Nolabel; lhs = argument_ty; rhs = result_ty })) in
      let (rest_argument_types, final_result_ty) = constructor_pattern_argument_types state result_ty rest origin in (argument_ty :: rest_argument_types, final_result_ty)
and bind_inline_record_pattern = fun (state: state) env pat_id labels fields open_ ->
  ...
  let field_names =
    List.map
      (
        fun (field: BodyArena.record_pattern_field) -> field.label
      )
      fields
  in
  let missing_fields =
    if open_ then
      []
    else missing_inline_record_fields labels field_names
  in
  let () =
    if not (List.is_empty missing_fields) then
      add_record_resolution_error state ~span:(diagnostic_span origin) ~context:Typ_diagnostic.RecordPattern (Typ_diagnostic.MissingRecordFields missing_fields)
  in
...
and bind_argument_patterns = fun (state: state) env arguments argument_types ->
  let rec loop acc arguments argument_types =
    ...
    | (argument_id :: rest_arguments, argument_ty :: rest_types) -> loop (bind_pattern state env argument_id argument_ty :: acc) rest_arguments rest_types
    | (argument_id :: rest_arguments, []) -> loop (bind_pattern state env argument_id (fresh_hole state) :: acc) rest_arguments []
...
and bind_pattern = fun (state: state) env pat_id expected_ty ->
  ...
  let unify_or_pattern_bindings origin bindings alternatives =
    ...
    let rec loop current_bindings remaining =
      ...
      | alternative_bindings :: rest ->
          ...
          if (not (List.equal String.equal expected_names actual_names)) || (not (List.equal String.equal expected_module_names actual_module_names)) then
            let () = add_or_pattern_bindings_mismatch state ~span:(diagnostic_span origin) ~expected_names ~actual_names in None
          else
            let () =
              List.iter2
                (
                  fun (_, expected_scheme) (_, actual_scheme) -> try_unify state ~origin (scheme_body expected_scheme) (scheme_body actual_scheme)
                )
                current_bindings
                alternative_entries
            in
  ...
  | Some pattern -> (
    let () =
      ...
      | Some annotation -> try_unify state ~origin:(origin_of_pattern state pat_id) expected_ty (canonicalize_type_in_env state env annotation)
      | None -> ()
    in
    match pattern.desc with
    | BodyArena.PVar name -> pattern_bindings_of_entries [ generalized_pattern_binding state pat_id ~name expected_ty ]
    | BodyArena.PWildcard -> empty_pattern_bindings
    | BodyArena.PInt text ->
        let () = try_unify state ~origin:(origin_of_pattern state pat_id) expected_ty (integer_literal_type state text) in empty_pattern_bindings
    | BodyArena.PFloat _ ->
        let () = try_unify state ~origin:(origin_of_pattern state pat_id) expected_ty TypeRepr.float in empty_pattern_bindings
    | BodyArena.PBool _ ->
        let () = try_unify state ~origin:(origin_of_pattern state pat_id) expected_ty TypeRepr.bool in empty_pattern_bindings
    | BodyArena.PString _ ->
        let () = try_unify state ~origin:(origin_of_pattern state pat_id) expected_ty TypeRepr.string in empty_pattern_bindings
    | BodyArena.PChar _ ->
        let () = try_unify state ~origin:(origin_of_pattern state pat_id) expected_ty TypeRepr.char in empty_pattern_bindings
    | BodyArena.PUnit ->
        let () = try_unify state ~origin:(origin_of_pattern state pat_id) expected_ty TypeRepr.unit_ in empty_pattern_bindings
    | BodyArena.PTuple elements ->
        let element_types =
          List.map
            (
              fun _ -> fresh_var state
            )
            elements
        in
        let () = try_unify state ~origin:(origin_of_pattern state pat_id) expected_ty (make_type state (TypeRepr.Tuple element_types)) in
        List.fold_left2
          (
            fun acc element_id element_ty -> merge_pattern_bindings acc (bind_pattern state env element_id element_ty)
          )
          empty_pattern_bindings
          elements
          element_types
    | BodyArena.POr alternatives -> (
      ...
      | alternative :: rest ->
          ...
          let alternative_bindings =
            rest |> List.map
              (
                fun alternative_id -> bind_pattern state env alternative_id expected_ty
              )
          in
    ...
    | BodyArena.PConstructor { constructor; arguments } -> (
      ...
      | Some constructor_entry ->
          ...
          let () =
            ...
            else try_unify state ~origin expected_ty instantiated.result_ty
          in
          (
            ...
            | (Some labels, [ argument_id ]) -> (
              ...
              | Some { desc = BodyArena.PRecord { fields; open_ }; _ } -> bind_inline_record_pattern state env argument_id labels fields open_
    ...
    | BodyArena.PRecord { fields; open_ } -> (
      ...
      let field_names =
        List.map
          ...
          fields
      in
      match resolve_record_decl env state ~field_names ~owner_hint:(owner_of_type state expected_ty) ~span:(diagnostic_span origin) ~context:Typ_diagnostic.RecordPattern with
      | Some record_decl ->
                  ...
                  fun label_name ->
                    not
                      (
                        List.exists
                          (
                            fun requested_name -> String.equal (Env.Label_env.lookup_name requested_name) label_name
                          )
                          field_names
                      )
                )
      ...
      | None ->
          fields |> List.fold_left
            (
              fun acc (field: BodyArena.record_pattern_field) -> merge_pattern_bindings acc (bind_pattern state env field.pattern_id (fresh_hole state))
            )
            empty_pattern_bindings
    )
    | BodyArena.PList elements ->
        let element_ty = fresh_var state in
        let () = try_unify state ~origin:(origin_of_pattern state pat_id) expected_ty (make_type state (TypeRepr.List element_ty)) in
        elements |> List.fold_left
          (
            fun acc element_id -> merge_pattern_bindings acc (bind_pattern state env element_id element_ty)
          )
          empty_pattern_bindings
    | BodyArena.PAlias { pattern_id; alias } ->
        let bindings = bind_pattern state env pattern_id expected_ty in merge_pattern_bindings (pattern_bindings_of_entries [ generalized_pattern_binding state pat_id ~name:alias expected_ty ]) bindings
    | BodyArena.PFirstClassModule { module_name; package_type } ->
        let annotated_signature =
          match package_type with
          | Some package_type ->
              let package_type = canonicalize_type_in_env state env package_type in package_signature_of_type package_type
          | None -> None
        in
        ...
        let signature =
          ...
          | (Some annotated_signature, Some _expected_signature) ->
              let () = try_unify state ~origin:(origin_of_pattern state pat_id) expected_ty (TypeRepr.package ~values:annotated_signature.values) in Some annotated_signature
          | (Some annotated_signature, None) ->
              let () = try_unify state ~origin:(origin_of_pattern state pat_id) expected_ty (TypeRepr.package ~values:annotated_signature.values) in Some annotated_signature
          | (None, Some expected_signature) -> Some expected_signature
          | (None, None) ->
              let () = add_diagnostic state (Typ_diagnostic.TypeMismatch { mismatch_span = diagnostic_span (origin_of_pattern state pat_id); mismatch = Typ_diagnostic.ExpectedActual { expected = "(module ...)"; actual = TypePrinter.type_to_string expected_ty } }) in None
        in
        begin
          match module_name, signature with
          | (Some module_name, Some signature) ->
              {
                empty_pattern_bindings with
                module_entries = [
                  module_name, package_env state pat_id signature;
                ]
              }
        ...
        let payload_ty =
          ...
          | None when is_unresolved_type expected_ty -> (
            ...
            | Some candidate ->
                ...
                let () = try_unify state ~origin:(origin_of_pattern state pat_id) expected_ty candidate_ty in instantiate_poly_variant_payload state mapping (poly_variant_candidate_payload candidate tag)
            | None ->
                let payload_ty =
                  match payload with
                  ...
                  | None -> None
                in
                let () = try_unify state ~origin:(origin_of_pattern state pat_id) expected_ty (anonymous_poly_variant_type state tag payload_ty) in payload_ty
          )
          | None -> None
        in
        (
    ...
    | BodyArena.PUnsupported _ -> empty_pattern_bindings
  )
...
let record_expr_trace = fun (state: state) expr_id origin_id env_before inferred_type ->
  let binding_provenance = function
    ...
    | Binding.TypeConstructor { type_name; scope_path } -> Check_result.TypeConstructor { type_name; scope_path }
    ...
    | Binding.ModuleAlias { alias_name; module_path } -> Check_result.ModuleAlias { alias_name; module_path }
  in
  let resolved_binding =
    match SemanticTree.find_expr state.file expr_id with
    | Some { desc = BodyArena.EVar name; _ } ->
        lookup_binding state env_before (EntityId.of_surface_path name) |> Option.map
          (
            fun binding -> Check_result.{ entity_id = Binding.path binding; surface_path = Binding.surface_path binding; provenance = binding_provenance (Binding.provenance binding) }
          )
    | _ -> None
  in
...
let binding_ref_of_binding = fun binding ->
  let provenance =
    ...
    | Binding.ModuleAlias { alias_name; module_path } -> Check_result.ModuleAlias { alias_name; module_path }
  in
  Check_result.{ entity_id = Binding.path binding; surface_path = Binding.surface_path binding; provenance }
...
let add_application_non_function = fun (state: state) ~expr_id current_ty -> add_diagnostic state (Typ_diagnostic.TypeMismatch { mismatch_span = diagnostic_span (origin_of_expr state expr_id); mismatch = Typ_diagnostic.ExpectedActual { expected = "function"; actual = TypePrinter.type_to_string current_ty } })

let add_nonexhaustive_match = fun (state: state) ~expr_id witness -> add_diagnostic state (Typ_diagnostic.NonexhaustiveMatch { match_span = diagnostic_span (origin_of_expr state expr_id); witness })

let add_redundant_match_case = fun (state: state) (case: BodyArena.match_case) -> add_diagnostic state (Typ_diagnostic.RedundantMatchCase { case_span = diagnostic_span (origin_of_pattern state case.pattern_id) })

let analyze_match_coverage = fun (state: state) ~expr_id scrutinee_ty cases ->
  let rec loop rows cases =
    match cases with
    | [] ->
        useful_coverage_pattern state rows CoverageAny scrutinee_ty |> Option.iter
          (
            fun witness -> add_nonexhaustive_match state ~expr_id witness
          )
      ...
      | Some patterns ->
          let useful_case =
            patterns |> List.exists
              (
                fun pattern -> Option.is_some (useful_coverage_pattern state rows pattern scrutinee_ty)
              )
          in
          let () =
...
let rec infer_match_case = fun (state: state) env scrutinee_ty result_ty (case: BodyArena.match_case) ->
  ...
  let () =
    match case.guard_id with
    | Some guard_id ->
        let guard_ty = infer_expr state case_env guard_id in try_unify state ~origin:(origin_of_expr state guard_id) guard_ty TypeRepr.bool
    | None -> ()
  in
  let case_ty = infer_expr state case_env case.body_id in try_unify state ~origin:(origin_of_expr state case.body_id) result_ty case_ty
...
and infer_inline_record_expr = fun (state: state) env expr_id fields labels ->
  ...
  let field_names =
    List.map
      (
        fun (field: BodyArena.record_expr_field) -> field.label
      )
      fields
  in
  ...
  let () =
    if not (List.is_empty missing_fields) then
      add_record_resolution_error state ~span:(diagnostic_span (origin_of_expr state expr_id)) ~context:Typ_diagnostic.RecordConstruction (Typ_diagnostic.MissingRecordFields missing_fields)
  in
  List.iter
    (
      fun (field: BodyArena.record_expr_field) ->
        let field_ty =
          ...
          | None -> fresh_hole state
        in
        let inferred_field_ty = infer_expr state env field.value_id in try_unify state ~origin:(origin_of_expr state field.value_id) field_ty inferred_field_ty
    )
    fields
and infer_record_expr = fun (state: state) env expr_id base_id fields ->
  ...
  in
  match resolve_record_decl env state ~field_names ~owner_hint:(
    match base_ty with
    | Some base_ty -> owner_of_type state base_ty
    | None -> None
  ) ~span:operation_span ~context with
  | Some record_decl ->
      let (owner_ty, field_types) = instantiate_record_decl state record_decl in
      let () =
        match base_id, base_ty with
        | Some base_id, Some base_ty -> try_unify state ~origin:(origin_of_expr state base_id) base_ty owner_ty
        | _ -> ()
      in
      let missing_fields =
        match base_id with
      ...
      let () =
        if not (List.is_empty missing_fields) then
          add_record_resolution_error state ~span:operation_span ~context (Typ_diagnostic.MissingRecordFields missing_fields)
      in
      let () =
        List.iter
          (
  ...
  | Some expr ->
      let inferred_type =
        ...
        | BodyArena.EVar name -> (
          ...
          | Some binding ->
              let scheme = canonicalize_scheme_in_env state env (Binding.scheme binding) in instantiate state scheme
          | None -> (
            match origin_of_expr state expr_id with
            | Some origin when String.equal origin.label "constructor_expression" || String.equal origin.label "constructor_path_expression" -> (
              match resolve_constructor_without_expected state env name with
              | Some constructor_entry ->
                  let scheme = canonicalize_scheme_in_env state env (Env.Constructor_env.scheme constructor_entry) in instantiate state scheme
              | None ->
                  let hole = fresh_hole state in
                  let () = add_diagnostic state (Typ_diagnostic.UnboundName { reference_span = diagnostic_span (origin_of_expr state expr_id); name = SurfacePath.to_string name }) in hole
            )
            | _ ->
                let hole = fresh_hole state in
                let () = add_diagnostic state (Typ_diagnostic.UnboundName { reference_span = diagnostic_span (origin_of_expr state expr_id); name = SurfacePath.to_string name }) in hole
        ...
        | BodyArena.EInt text -> integer_literal_type state text
        | BodyArena.EFloat _ -> TypeRepr.float
        | BodyArena.EBool _ -> TypeRepr.bool
        | BodyArena.EString _ -> TypeRepr.string
        | BodyArena.EChar _ -> TypeRepr.char
        | BodyArena.EUnit -> TypeRepr.unit_
        | BodyArena.ETuple elements -> make_type state (TypeRepr.Tuple (List.map (infer_expr state env) elements))
        | BodyArena.EArray elements ->
            ...
            let () =
              List.iter
                (
                  fun element_id ->
                    let inferred_element = infer_expr state env element_id in try_unify state ~origin:(origin_of_expr state element_id) element_ty inferred_element
                )
                elements
            in
          ...
          | last_id :: reversed_prefix ->
              ...
              let () =
                List.iter
                  (
                    fun element_id ->
                      let element_ty = infer_expr state env element_id in try_unify state ~origin:(origin_of_expr state element_id) element_ty TypeRepr.unit_
                  )
                  prefix
              in
        ...
        | BodyArena.EWhile { condition_id; body_id } ->
            ...
            let () = try_unify state ~origin:(origin_of_expr state body_id) body_ty TypeRepr.unit_ in TypeRepr.unit_
        ...
        | BodyArena.EFun (parameters, body_id) ->
            let rec lower_parameters env = function
              ...
              | (parameter: BodyArena.function_parameter) :: rest ->
                  ...
                  let body_ty = lower_parameters (env_with_pattern_bindings env bindings) rest in make_type state (TypeRepr.Arrow { label = type_label_of_body_label parameter.label; lhs = arg_ty; rhs = body_ty })
            in
        ...
        | BodyArena.EApply (callee_id, arguments) ->
            ...
            let infer_apply_argument_against (argument: BodyArena.apply_argument) lhs =
              ...
              | (BodyArena.Optional _, true) ->
                  ...
                  let () = try_unify state ~origin:(origin_of_expr state expr_id) forwarded_option_ty (TypeRepr.option forwarded_value_ty) in
                  let () = try_unify state ~origin:(origin_of_expr state expr_id) forwarded_value_ty lhs in infer_expr_against state env argument.value_id (TypeRepr.option lhs)
              ...
              | _ -> (
                ...
                | TypeRepr.Arrow { label; lhs; rhs } -> (
                  ...
                  | Some ((argument: BodyArena.apply_argument), rest_arguments) ->
                      let _ = infer_apply_argument_against argument lhs in apply_with_known_type rhs rest_arguments
                  | None -> (
                    match label with
                    | TypeRepr.Optional _ -> apply_with_known_type rhs arguments
                    | _ ->
                        let () = add_application_label_mismatch state ~expr_id ~expected_label:label arguments in fresh_hole state
                  )
                )
                | TypeRepr.Var _ | TypeRepr.Hole _ -> apply_from_unknown_type current_ty arguments
                | _ ->
                    let () = add_application_non_function state ~expr_id current_ty in fresh_hole state
              )
              ...
              | (argument: BodyArena.apply_argument) :: rest ->
                  let argument_ty =
                    ...
                    | (BodyArena.Optional _, true) ->
                        let forwarded_option_ty = infer_expr state env argument.value_id in
                        let forwarded_value_ty = fresh_var state in
                        let () = try_unify state ~origin:(origin_of_expr state expr_id) forwarded_option_ty (TypeRepr.option forwarded_value_ty) in forwarded_value_ty
                    | _ -> infer_expr state env argument.value_id
                  in
                  let result_ty = fresh_var state in
                  let () = try_unify state ~origin:(origin_of_expr state expr_id) current_ty (make_type state (TypeRepr.Arrow { label = type_label_of_body_label argument.label; lhs = argument_ty; rhs = result_ty })) in apply_with_known_type result_ty rest
            in
            (
              match origin_of_expr state expr_id with
                ...
                | Some { desc = BodyArena.EVar constructor; _ } -> (
                  ...
                  | Some constructor_entry ->
                      ...
                      let constructor_ty =
                        List.fold_right
                          (
                            fun argument_ty result_ty -> make_type state (TypeRepr.Arrow { label = TypeRepr.Nolabel; lhs = argument_ty; rhs = result_ty })
                          )
                      ...
                      (
                        match instantiated.inline_record_labels, arguments with
                        | (Some labels, [ argument ]) when argument_matches_parameter_label TypeRepr.Nolabel argument.label -> (
                          match SemanticTree.find_expr state.file argument.value_id with
                          | Some { desc = BodyArena.ERecord { base_id = None; fields }; _ } ->
                              let () = infer_inline_record_expr state env argument.value_id fields labels in instantiated.result_ty
        ...
        | BodyArena.ERecord { base_id; fields } -> infer_record_expr state env expr_id base_id fields
        | BodyArena.EFieldAccess { receiver_id; label } ->
            let receiver_ty = infer_expr state env receiver_id in
            let field_names = [ label ] in
            begin
              match resolve_record_decl env state ~field_names ~owner_hint:(owner_of_type state receiver_ty) ~span:(diagnostic_span (origin_of_expr state expr_id)) ~context:Typ_diagnostic.RecordFieldAccess with
              | Some record_decl ->
                  let (owner_ty, field_types) = instantiate_record_decl state record_decl in
        ...
        | BodyArena.EFieldAssign { receiver_id; label; value_id } ->
            ...
            let field_names = [ label ] in
            begin
              match resolve_record_decl env state ~field_names ~owner_hint:(owner_of_type state receiver_ty) ~span:(diagnostic_span (origin_of_expr state expr_id)) ~context:Typ_diagnostic.RecordUpdate with
              | Some record_decl ->
                  let (owner_ty, field_types) = instantiate_record_decl state record_decl in
                  let () = try_unify state ~origin:(origin_of_expr state receiver_id) receiver_ty owner_ty in
                  begin
                    ...
                    | Some field_ty ->
                        ...
                        let () = try_unify state ~origin:(origin_of_expr state value_id) value_ty field_ty in TypeRepr.unit_
                    | None -> fresh_hole state
                  end
              | None ->
                  let _ = infer_expr state env value_id in fresh_hole state
            end
        | BodyArena.EIndex (collection_id, index_id) ->
            ...
            begin
              ...
              | _ ->
                  let () = try_unify state ~origin:(origin_of_expr state collection_id) collection_ty (make_type state (TypeRepr.Array element_ty)) in element_ty
            end
        | BodyArena.ELet (binding_ids, body_id) ->
            let env = infer_binding_group state env binding_ids in infer_expr state env body_id
        | BodyArena.EIf (condition_id, then_id, else_id) ->
            ...
            let () = try_unify state ~origin:(origin_of_expr state expr_id) then_ty else_ty in then_ty
        | BodyArena.EMatch (scrutinee_id, cases) ->
            ...
            let () = analyze_match_coverage state ~expr_id scrutinee_ty cases in result_ty
        | BodyArena.ETry (body_id, cases) ->
            let body_ty = infer_expr state env body_id in
            let exn_ty = make_type state (TypeRepr.Named { head = TypeRepr.named_head ~type_constructor_id:BuiltinTypeConstructors.exn_type_constructor_id ~name:(SurfacePath.of_name "exn"); arguments = [] }) in
            let result_ty = fresh_var state in
            ...
            let () = analyze_match_coverage state ~expr_id exn_ty cases in result_ty
        | BodyArena.EPolyVariant { tag; payload } -> (
          match best_poly_variant_candidate_for_tags state [ tag ] with
          | Some candidate ->
              let (candidate_ty, mapping) = instantiate_named_type_decl state candidate.type_decl in
              let payload_ty = instantiate_poly_variant_payload state mapping (poly_variant_candidate_payload candidate tag) in
              let () =
                match payload, payload_ty with
        ...
        | BodyArena.ECoerce { value_id; target_type } ->
            ...
            let () =
              if can_explicitly_coerce_poly_variant state ~source_ty ~target_ty:target_type then
                ()
              else try_unify state ~origin:(origin_of_expr state expr_id) source_ty target_type
            in
            target_type
        | BodyArena.EModulePack { module_path; package_type } -> (
          ...
          | Some package_type -> (
            ...
            | Some signature ->
                let () = check_module_pack_against_signature state env ~origin:(origin_of_expr state expr_id) module_path signature in package_type
            | None ->
                let () = add_diagnostic state (Typ_diagnostic.TypeMismatch { mismatch_span = diagnostic_span (origin_of_expr state expr_id); mismatch = Typ_diagnostic.ExpectedActual { expected = "(module ...)"; actual = TypePrinter.type_to_string package_type } }) in fresh_hole state
          )
          | None ->
              let () = add_diagnostic state (Typ_diagnostic.TypeMismatch { mismatch_span = diagnostic_span (origin_of_expr state expr_id); mismatch = Typ_diagnostic.ExpectedActual { expected = "(module S)"; actual = "(module ...)" } }) in fresh_hole state
        )
        | BodyArena.ELocalModulePack { local_scope; package_type } -> (
          ...
          | Some package_type -> (
            ...
            match package_signature_of_type package_type with
            | Some signature ->
                let () = check_local_module_pack_against_signature state env ~origin:(origin_of_expr state expr_id) local_scope signature in package_type
            | None ->
                let () = add_diagnostic state (Typ_diagnostic.TypeMismatch { mismatch_span = diagnostic_span (origin_of_expr state expr_id); mismatch = Typ_diagnostic.ExpectedActual { expected = "(module ...)"; actual = TypePrinter.type_to_string package_type } }) in fresh_hole state
          )
          | None ->
        ...
        | BodyArena.ELocalModule { module_name; local_scope; body_id } ->
            ...
            let body_env = Env.bind env (Env.singleton_module ~name:module_name local_module_env) in infer_expr state body_env body_id
        | BodyArena.ELocalOpen { module_path; body_id } -> infer_expr state (with_local_open state env module_path) body_id
        | BodyArena.EUnsupported summary ->
            let hole = fresh_hole state in
            let () = add_diagnostic state (Typ_diagnostic.UnsupportedSemanticExpression { expression_span = diagnostic_span (origin_of_expr state expr_id); summary }) in hole
        | BodyArena.EHole _ -> fresh_hole state
      in
      let () = record_expr_trace state expr_id expr.origin_id env inferred_type in inferred_type
and infer_expr_against = fun (state: state) env expr_id expected_ty ->
  ...
  | Some expr -> (
          ...
          (
            fun () ->
              let rec check_parameters env current_expected = function
                | [] ->
                    let _ = infer_expr_against state env body_id current_expected in expected_ty
                | (parameter: BodyArena.function_parameter) :: rest -> (
                  match TypeRepr.view (normalize_rigid_type state current_expected) with
                  | TypeRepr.Arrow { label; lhs; rhs } when labels_match label (type_label_of_body_label parameter.label) ->
                      let () =
                        match parameter.default_value_id with
                        | Some default_value_id ->
                      ...
                      let bound_ty =
                        ...
                        | _ -> lhs
                      in
                      let bindings = bind_pattern state env parameter.pattern_id bound_ty in check_parameters (env_with_pattern_bindings env bindings) rhs rest
                  | _ ->
                      let inferred_type = infer_expr state env expr_id in
                      let () = try_unify state ~origin:(origin_of_expr state expr_id) expected_ty inferred_type in inferred_type
                )
              in
    ...
    | BodyArena.EMatch (scrutinee_id, cases) ->
        ...
        let () = analyze_match_coverage state ~expr_id scrutinee_ty cases in expected_ty
    | BodyArena.EVar name -> (
      match origin_of_expr state expr_id with
      | Some origin when String.equal origin.label "constructor_expression" || String.equal origin.label "constructor_path_expression" -> (
        ...
        | Some { desc = BodyArena.EVar constructor; _ } -> (
          ...
          | Some constructor_entry ->
              ...
              let rec apply_with_known_type current_ty arguments =
                match arguments with
                | [] ->
                    let () = try_unify state ~origin:(origin_of_expr state expr_id) expected_ty current_ty in current_ty
                | (argument: BodyArena.apply_argument) :: rest -> (
                  match view current_ty with
                  | TypeRepr.Arrow { label; lhs; rhs } ->
                      if argument_matches_parameter_label label argument.label then
                        let _ = infer_expr_against state env argument.value_id lhs in apply_with_known_type rhs rest
                      else
                        let inferred_type = infer_expr state env expr_id in
                        let () = try_unify state ~origin:(origin_of_expr state expr_id) expected_ty inferred_type in inferred_type
                  | _ ->
              ...
              (
                ...
                | (Some labels, [ argument ]) when argument_matches_parameter_label TypeRepr.Nolabel argument.label -> (
                  match SemanticTree.find_expr state.file argument.value_id with
                  | Some { desc = BodyArena.ERecord { base_id = None; fields }; _ } ->
                      let () = infer_inline_record_expr state env argument.value_id fields labels in
                      let () = try_unify state ~origin:(origin_of_expr state expr_id) expected_ty instantiated.result_ty in instantiated.result_ty
                  | _ -> apply_with_known_type callee_ty arguments
                )
                | _ -> apply_with_known_type callee_ty arguments
              )
    ...
    | BodyArena.EModulePack { module_path; package_type } -> (
      let inferred_type =
        ...
        | (_, Some signature) ->
            let () = check_module_pack_against_signature state env ~origin:(origin_of_expr state expr_id) module_path signature in expected_ty
        | (Some package_type, None) ->
            let package_type = canonicalize_type_in_env state env package_type in
            let () = try_unify state ~origin:(origin_of_expr state expr_id) expected_ty package_type in package_type
        | (None, None) -> infer_expr state env expr_id
      in
      let () = try_unify state ~origin:(origin_of_expr state expr_id) expected_ty inferred_type in inferred_type
    )
    | BodyArena.ELocalModulePack { local_scope; package_type } -> (
      let inferred_type =
        match package_type, package_signature_of_type expected_ty with
        | (_, Some signature) ->
            let () = check_local_module_pack_against_signature state env ~origin:(origin_of_expr state expr_id) local_scope signature in expected_ty
        | (Some package_type, None) ->
            let package_type = canonicalize_type_in_env state env package_type in
            let () = try_unify state ~origin:(origin_of_expr state expr_id) expected_ty package_type in package_type
        | (None, None) -> infer_expr state env expr_id
...
and infer_local_module_env = fun (state: state) env (local_scope: BodyArena.local_module_scope) ->
  ...
  let env_after_bindings =
    local_scope.binding_groups |> List.fold_left
      (
        fun current_env (group: BodyArena.local_module_binding_group) -> infer_binding_group state current_env group.binding_ids
      )
      env_with_local_types
  in
  let introduced_entries = Env.introduced_entries env_with_local_types env_after_bindings in Env.bind local_type_env introduced_entries
and check_local_module_pack_against_signature = fun (state: state) env ~origin (local_scope: BodyArena.local_module_scope) ((signature: TypeRepr.package_signature)) ->
  let signature = instantiate_package_signature state signature in
    ...
    (
      fun (value: TypeRepr.package_value) ->
        if not (List.mem value.name local_binding_names) then
          add_diagnostic state (Typ_diagnostic.UnboundName { reference_span = diagnostic_span origin; name = value.name })
        else
          match Env.lookup local_env (EntityId.of_name value.name) with
          | Some binding ->
              let actual_ty = instantiate state (canonicalize_scheme_in_env state local_env (Binding.scheme binding)) in
              let expected_ty = TypeScheme.body value.scheme in try_unify state ~origin actual_ty expected_ty
          | None -> add_diagnostic state (Typ_diagnostic.UnboundName { reference_span = diagnostic_span origin; name = value.name })
    )
and infer_binding_group = fun (state: state) env binding_ids ->
  ...
  else infer_nonrecursive_group state env bindings
...
and infer_nonrecursive_group = fun (state: state) env bindings ->
            ...
            List.map
              (
                fun (binding: BodyArena.binding) ->
                  let export_annotation_scheme = binding.annotation |> Option.map (canonicalize_generalized_scheme_in_env state env) in
                  let checking_annotation_scheme = binding.annotation |> Option.map (canonicalize_scheme_in_env state env) in
                  let value_ty =
                    match checking_annotation_scheme with
                    | Some annotation ->
                        let expected_ty = instantiate_rigid_scheme state annotation in
                        let _ = infer_expr_against state env binding.value_id expected_ty in expected_ty
                    | None -> infer_expr state env binding.value_id
                  in
                  let bound_entries = bind_pattern state env binding.pattern_id value_ty in ((binding, export_annotation_scheme, bound_entries), solver_group_for_entries state binding.value_id value_ty bound_entries.entries)
              )
              bindings
          in
      ...
      (
        fun (binding, annotation_scheme, entries) schemes ->
          ...
          let generalized_entries =
            List.map2
              (
                fun entry scheme -> Binding.with_scheme (canonicalize_scheme_heads_in_env state env scheme) entry
              )
              entries.entries
              schemes
          in
  ...
  List.fold_left
    (
      fun env (_, entries) -> env_with_pattern_bindings env entries
    )
    env
...
and infer_recursive_group = fun (state: state) env bindings ->
  ...
  if List.is_empty unsupported_bindings then
                ...
                (
                  fun (binding: BodyArena.binding) ->
                    ...
                    | Some name ->
                        ...
                        in
                        Some (binding, export_annotation_scheme, Option.is_some checking_annotation_scheme, placeholder_ty, entry)
                    | None -> None
                )
            in
            let placeholders =
              placeholder_info |> List.map
                (
                  fun (_, _, _, _, entry) -> entry
                )
            in
                ...
                (
                  fun ((binding: BodyArena.binding), _annotation_scheme, has_annotation, placeholder_ty, _entry) ->
                    if has_annotation then
                      ...
                      let value_ty = infer_expr state provisional_env binding.value_id in try_unify state ~origin:(origin_of_binding state binding) placeholder_ty value_ty
                )
                placeholder_info
            in
            let groups =
              placeholder_info |> List.map
                (
                  fun ((binding: BodyArena.binding), _annotation_scheme, _has_annotation, placeholder_ty, entry) -> solver_group_for_entries state binding.value_id placeholder_ty [ entry ]
                )
            in
    ...
    let () =
      List.iter
        (
          fun (binding: BodyArena.binding) -> add_diagnostic state (Typ_diagnostic.RecursiveGroupRequiresSimpleVariableBinders { binding_span = diagnostic_span (origin_of_binding state binding) })
        )
    ...
    List.fold_left
      (
        fun env (binding: BodyArena.binding) ->
          ...
          let bound_entries = bind_pattern state env binding.pattern_id placeholder in env_with_pattern_bindings env bound_entries
      )
      env
      bindings
...
let infer_file = fun ~imported_world ~config ~(source:Source.t) file ->
  ...
  let rec loop export_state type_decls scope = function
    ...
    | item :: rest -> (
      ...
      | ItemTree.Type type_item ->
          let item_env = Env.bind (Env.for_item_scope export_state scope ~scope_path:type_item.scope_path) (type_item_env state type_item) in
          let introduced_type_decls = [ canonicalize_type_decl_in_env state item_env { FileSummary.scope_path = type_item.scope_path; declaration = type_item.declaration } ] in
            ...
            type_decls |> List.find_map
              (
                fun (candidate: FileSummary.type_decl) ->
                  if TypeConstructorId.equal candidate.declaration.type_constructor_id type_item.declaration.type_constructor_id then
                    Some candidate
                  else None
              ) |> Option.unwrap_or ~default:(List.hd introduced_type_decls)
          in
          let introduced = Env.of_type_decls [ { introduced_type_decl with scope_path = SurfacePath.empty } ] in
          ...
          let (export_state, scope) =
            ...
            else (Env.bind_in_scope export_state ~scope_path:type_item.scope_path introduced, Env.register_entries scope ~scope_path:type_item.scope_path introduced)
          in
          let () =
            if state.config.capture_traces then
              ...
              let binding_names =
                ...
                | None -> []
              in
              let exports_after = Env.render exports_after_env in state.item_traces <- ({ Check_result.item_id = type_item.item_id; binding_names; exports_after } : Check_result.item_trace) :: state.item_traces
          in
      ...
      | ItemTree.Exception exception_item ->
          ...
          let (export_state, scope) =
            if SurfacePath.is_empty exception_item.scope_path then
              (Env.bind export_state introduced, scope)
            else (Env.bind_in_scope export_state ~scope_path:exception_item.scope_path introduced, Env.register_entries scope ~scope_path:exception_item.scope_path introduced)
          in
          let () =
            if state.config.capture_traces then
              let exports_after_env = Env.export config export_state in
              let binding_names =
                ...
                | None -> []
              in
              let exports_after = Env.render exports_after_env in state.item_traces <- ({ Check_result.item_id = exception_item.item_id; binding_names; exports_after } : Check_result.item_trace) :: state.item_traces
          in
          loop export_state type_decls scope rest
      | ItemTree.ExtensionConstructor extension_item ->
          ...
          let (export_state, scope) =
            if SurfacePath.is_empty extension_item.scope_path then
              (Env.bind export_state introduced, scope)
            else (Env.bind_in_scope export_state ~scope_path:extension_item.scope_path introduced, Env.register_entries scope ~scope_path:extension_item.scope_path introduced)
          in
          let () =
            if state.config.capture_traces then
              let exports_after_env = Env.export config export_state in
              let binding_names =
                ...
                | None -> []
              in
              let exports_after = Env.render exports_after_env in state.item_traces <- ({ Check_result.item_id = extension_item.item_id; binding_names; exports_after } : Check_result.item_trace) :: state.item_traces
          in
          loop export_state type_decls scope rest
      | ItemTree.Value value_item ->
          ...
          let (export_state, scope) =
            if SurfacePath.is_empty value_item.scope_path then
              (Env.bind export_state introduced, scope)
            else (Env.bind_in_scope export_state ~scope_path:value_item.scope_path introduced, Env.register_entries scope ~scope_path:value_item.scope_path introduced)
          in
          let () =
            if state.config.capture_traces then
              let exports_after_env = Env.export config export_state in
              let binding_names =
                ...
                | None -> []
              in
              let exports_after = Env.render exports_after_env in state.item_traces <- ({ Check_result.item_id = value_item.item_id; binding_names; exports_after } : Check_result.item_trace) :: state.item_traces
          in
          loop export_state type_decls scope rest
      | ItemTree.DeclaredValue declared_value_item ->
          ...
          let (export_state, scope) =
            if SurfacePath.is_empty declared_value_item.scope_path then
              (Env.bind export_state introduced, scope)
            else (Env.bind_in_scope export_state ~scope_path:declared_value_item.scope_path introduced, Env.register_entries scope ~scope_path:declared_value_item.scope_path introduced)
          in
          let () =
            if state.config.capture_traces then
              let exports_after_env = Env.export config export_state in
              let binding_names =
                ...
                | None -> []
              in
              let exports_after = Env.render exports_after_env in state.item_traces <- ({ Check_result.item_id = declared_value_item.item_id; binding_names; exports_after } : Check_result.item_trace) :: state.item_traces
          in
          loop export_state type_decls scope rest
      | ItemTree.Open open_item ->
          let item_env = Env.for_item_scope export_state scope ~scope_path:open_item.scope_path in
          let module_path = resolve_module_path_in_scope state item_env state.visible_types open_item.scope_path open_item.module_path in
          ...
          let () =
            if state.config.capture_traces then
              let exports_after = Env.export config export_state |> Env.render in state.item_traces <- ({ Check_result.item_id = open_item.item_id; binding_names = []; exports_after } : Check_result.item_trace) :: state.item_traces
          in
          loop export_state type_decls scope rest
      | ItemTree.Include include_item ->
          let item_env = Env.for_item_scope export_state scope ~scope_path:include_item.scope_path in
          let module_path = resolve_module_path_in_scope state item_env state.visible_types include_item.scope_path include_item.module_path in
          let visible_exports_before =
            if state.config.capture_traces then
              Some (Env.export config export_state)
            else None
          in
          let introduced = entries_for_include state item_env module_path in
          let introduced_type_decls = type_decls_for_include state.visible_types module_path |> prefix_type_decls include_item.scope_path in
          let (export_state, scope) =
            if SurfacePath.is_empty include_item.scope_path then
              (Env.bind export_state introduced, scope)
            else (Env.bind_in_scope export_state ~scope_path:include_item.scope_path introduced, Env.register_entries scope ~scope_path:include_item.scope_path introduced)
          in
          let () =
            if state.config.capture_traces then
              let exports_after_env = Env.export config export_state in
              let binding_names =
                ...
                | None -> []
              in
              let exports_after = Env.render exports_after_env in state.item_traces <- ({ Check_result.item_id = include_item.item_id; binding_names; exports_after } : Check_result.item_trace) :: state.item_traces
          in
          let type_decls = bind_type_decls type_decls introduced_type_decls in
          let type_decls = set_visible_type_decls state type_decls in loop export_state type_decls scope rest
      | ItemTree.ModuleAlias module_alias_item ->
          ...
          let module_path = resolve_module_path_in_scope state item_env state.visible_types module_alias_item.scope_path module_alias_item.module_path in
          let visible_exports_before =
            if state.config.capture_traces then
              Some (Env.export_with_forced_names ~config:state.config ~forced_export_names:state.forced_export_names export_state)
            else None
          in
          let alias_export_names = export_names_for_module_alias state item_env ~alias_name:module_alias_item.alias_name ~module_path in
          let introduced = entries_for_module_alias state item_env ~alias_name:module_alias_item.alias_name ~module_path in
          let introduced_type_decls = type_decls_for_module_alias state.visible_types ~alias_name:module_alias_item.alias_name ~module_path |> prefix_type_decls module_alias_item.scope_path in
          let (export_state, scope) =
            if SurfacePath.is_empty module_alias_item.scope_path then
              (Env.bind export_state introduced, scope)
            else (Env.bind_in_scope export_state ~scope_path:module_alias_item.scope_path introduced, Env.register_entries scope ~scope_path:module_alias_item.scope_path introduced)
          in
          let forced_export_names =
            if SurfacePath.is_empty module_alias_item.scope_path then
              ...
              List.map
                ...
                alias_export_names
          in
          let () = state.forced_export_names <- forced_export_names @ state.forced_export_names in
          let () =
            if state.config.capture_traces then
              let exports_after_env = Env.export_with_forced_names ~config:state.config ~forced_export_names:state.forced_export_names export_state in
              let binding_names =
                match visible_exports_before with
                | Some visible_exports_before -> Env.introduced_names visible_exports_before exports_after_env
                | None -> []
              in
              let exports_after = Env.render exports_after_env in state.item_traces <- ({ Check_result.item_id = module_alias_item.item_id; binding_names; exports_after } : Check_result.item_trace) :: state.item_traces
          in
          let type_decls = bind_type_decls type_decls introduced_type_decls in
          let type_decls = set_visible_type_decls state type_decls in loop export_state type_decls scope rest
      | ItemTree.Unsupported unsupported_item ->
          let () =
            if state.config.capture_traces then
              let exports_after = Env.export config export_state |> Env.render in state.item_traces <- ({ Check_result.item_id = unsupported_item.item_id; binding_names = []; exports_after } : Check_result.item_trace) :: state.item_traces
          in
          loop export_state type_decls scope rest
    )
  in
  let (exports, type_decls, scope) = loop initial_env [] initial_scope (ItemTree.items file.item_tree) in
  let final_env =
    Env.for_item_scope exports scope ~scope_path:SurfacePath.empty |> fun env -> Env.bind env (Env.of_type_decls type_decls)
  in
  let export_env = Env.export_with_forced_names ~config:state.config ~forced_export_names:state.forced_export_names exports in
  let annotated_export_overrides =
    ItemTree.items file.item_tree |> List.fold_left
      (
        fun acc item ->
          match item with
          | ItemTree.Value { scope_path; binding_ids; _ } when SurfacePath.is_empty scope_path ->
              binding_ids |> List.fold_left
                (
                  fun acc binding_id ->
                    match SemanticTree.find_binding state.file binding_id with
                    | Some ({ name = Some name; annotation = Some annotation; _ }: BodyArena.binding) -> (name, canonicalize_generalized_scheme_in_env state final_env annotation) :: acc
                    | _ -> acc
                )
                acc
          | _ -> acc
  ...
  let exports =
    export_env |> Env.render |> List.map
      (
        fun (name, scheme) ->
          let scheme =
