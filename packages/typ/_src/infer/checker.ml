...
let with_local_open = fun (state: state) env module_path ->
  ...
  | Some scope ->
      Env.bind
        env
        (
          Env.entries_of_module_scope_for_include ~module_path scope
          |> Env.without_summary
        )
  | None -> env
...
let lookup_record_decl_by_owner = fun (state: state) env owner_type_constructor_id ->
  Option.or_else
    ...
    (fun () ->
      ImportedWorld.visible_modules state.imported_world
      |> List.find_map
...
let binding_name_of_path = fun path ->
  ...
  |> Option.unwrap_or ~default:(SurfacePath.to_string path)
...
let constructor_bindings = fun (state: state) env ~name ~scheme ~provenance ~constructor_id ~inline_record_labels ->
  ...
  let inline_record_labels =
    ...
    |> Option.map (VisibleTypes.canonicalize_inline_record_labels visible_types)
  in
  ...
  | TypeRepr.Named { head; _ } ->
      Env.singleton_constructor
        ~make_id:(fun path -> fresh_binding_ident state (binding_name_of_path path))
        ~name
        ~scheme
        ~provenance
...
let substitute_type_vars = fun (state: state) ty mapping ->
  let rec loop ty =
    ...
    | TypeRepr.PolyVariant { bound; tags; inherited } ->
        ...
        if
          List.for_all2 Std.Ptr.equal tags copied_tags
          && List.for_all2 Std.Ptr.equal inherited copied_inherited
        then
          ty
        else
          make_type
...
let canonicalize_type_in_env = fun (state: state) env ty ->
  let canonicalized =
    State.canonicalize_type_with_name_resolution
      ...
      ~resolve_named_type_head:(fun name ->
        resolve_named_type_head_in_env state env name)
      ty
  in
...
let record_decl_matches_owner = fun (record_decl: record_type_decl) owner ->
  TypeConstructorId.equal
    (Env.Label_env.owner_type_constructor_id record_decl)
    owner.type_constructor_id
      ...
      (fun acc (field: TypeDecl.label) ->
        let field_scheme =
          TypeScheme.map_type_preserving
            (fun ty ->
              substitute_type_vars state ty mapping)
            field.field_type
        in
...
let poly_variant_candidate = fun (state: state) (type_decl: FileSummary.type_decl) ->
  let rec collect visited acc (type_decl: FileSummary.type_decl) =
    ...
    if Collections.HashSet.contains visited type_constructor_id then
      ...
      let () =
        Collections.HashSet.insert visited type_constructor_id
        |> ignore
      in
  ...
  in
  match type_decl.declaration.manifest with
  | Some (TypeDecl.PolyVariant { bound; _ }) ->
      Some {
        type_decl;
        bound;
...
let poly_variant_tag_count = fun candidate ->
  candidate.payloads
  |> Label_name_map.bindings
  |> List.length
...
let best_poly_variant_candidate_for_tags = fun (state: state) tags ->
  visible_type_decls state
  |> List.filter_map (poly_variant_candidate state)
  |> List.filter
    (fun candidate ->
      tags
  ...
  |> List.filter
    (fun tag ->
      if Collections.HashSet.contains seen tag then
        ...
        let () =
          Collections.HashSet.insert seen tag
          |> ignore
        in
        true)
  ...
  | (Some source, Some target) when source.bound = TypeDecl.Exact ->
      ...
      |> List.for_all
        (fun (tag, source_payload) ->
          ...
          | Some target_payload ->
              poly_variant_payloads_compatible state source_payload target_payload
          | None -> false)
  | _ -> false
...
let rec top_level_poly_variant_tag = fun (state: state) pat_id ->
  ...
  | Some { desc = BodyArena.PAlias { pattern_id; _ }; _ } ->
      top_level_poly_variant_tag state pattern_id
  | _ -> None
...
let add_diagnostic = fun (state: state) diagnostic -> state.diagnostics <- diagnostic
:: state.diagnostics
...
let resolve_record_decl = fun env (state: state) ~field_names ~owner_hint ~span ~context ->
  ...
  | [] ->
      let unknown_labels =
        field_names
        |> List.filter (fun field_name -> List.is_empty (lookup_record_decls state env field_name))
      in
      let reason =
...
type coverage_pattern =
  | CoverageAny
  | CoverageConstructor of {
      constructor: coverage_constructor;
      arguments: coverage_pattern list;
    }
      ...
      Some (
        ...
        |> List.map
          (fun (constructor: TypeDecl.constructor) ->
            let specialized =
              substitute_type_vars state (TypeScheme.body constructor.scheme) mapping
            in
...
let supported_coverage_constructors_for_type = fun (state: state) ty ->
  ...
  | TypeRepr.Unit ->
      Some [ make_coverage_constructor ~key:CoverageUnit ~name:"()" ~argument_types:[] ]
  | TypeRepr.Tuple element_types ->
      Some [
        make_coverage_constructor
  ...
  | TypeRepr.List element_ty ->
      Some [
        ...
        make_coverage_constructor
          ...
          ~name:"::"
          ~argument_types:[ element_ty; ty ];
      ]
  | TypeRepr.Named { head; arguments } ->
      supported_coverage_constructors_for_named_type state head arguments
  | TypeRepr.Int
  | TypeRepr.Float
  | TypeRepr.String
  | TypeRepr.Char
...
let rec coverage_patterns_of_pattern = fun (state: state) pat_id expected_ty ->
  let combine_with_constructor constructor pattern_groups =
    coverage_product pattern_groups
    |> List.map (fun arguments -> CoverageConstructor { constructor; arguments })
  in
  ...
  | Some pattern -> (
      ...
      | BodyArena.PVar _
      | BodyArena.PWildcard -> Some [ CoverageAny ]
      ...
      | BodyArena.PConstructor { constructor; arguments } -> (
          ...
          | Some constructor ->
              translate_many arguments constructor.argument_types
              |> Option.map (combine_with_constructor constructor)
          | None -> None
        )
          ...
          | TypeRepr.List element_ty ->
              ...
              let cons_constructor =
                make_coverage_constructor
                  ~key:CoverageListCons
                  ~name:"::"
                  ~argument_types:[ element_ty; expected_ty ]
              in
              let rec loop = function
                | [] ->
      ...
      | BodyArena.PUnsupported _ -> None
    )

let coverage_any_patterns = fun count -> List.init count (fun _ -> CoverageAny)

let specialize_coverage_row = fun constructor (row: coverage_row) ->
  match row with
  | [] -> None
  | CoverageAny :: rest ->
      Some (coverage_any_patterns (coverage_constructor_arity constructor) @ rest)
...
let specialize_coverage_matrix = fun constructor rows ->
  rows
  |> List.filter_map (specialize_coverage_row constructor)
...
let useful_coverage_pattern = fun (state: state) rows pattern expected_ty ->
  Option.and_then
    (useful_coverage_vector state rows [ pattern ] [ expected_ty ])
    (
      function
...
let rec is_nonexpansive_expr = fun (state: state) expr_id ->
  ...
  | Some expr ->
      ...
      | BodyArena.ELocalModulePack _ -> true
      ...
      | BodyArena.EHole _ -> false
      | BodyArena.ELet (binding_ids, body_id) ->
          List.for_all (is_nonexpansive_binding state) binding_ids
...
let solver_group_for_entries = fun (state: state) expr_id root_ty entries ->
  let roots =
    entries
    |> List.map (fun entry -> TypeScheme.body (Binding.scheme entry))
  in
...
let merge_exact_poly_variant_tags = fun (state: state) left_tags right_tags ->
  ...
  | Ok () -> (
      ...
      | Ok () ->
          Some (
            Collections.HashMap.to_list merged
            |> List.map snd
            |> List.sort
    ...
    | (Some linked_tags, Some other_tags) -> (
        ...
        | Some merged_tags ->
            let merged_ty =
              make_type
                state
                (TypeRepr.PolyVariant { bound = TypeRepr.Exact; tags = merged_tags; inherited = [] })
            in
            let () =
...
let unify_gadt = fun (state: state) ~origin left right ->
  let rec loop = function
    ...
    | (left, right) :: rest ->
        let left =
          normalize_rigid_type state left
          |> TypeRepr.prune
        in
        let right =
          normalize_rigid_type state right
          |> TypeRepr.prune
        in
        if Std.Ptr.equal left right then
          ...
          | (TypeRepr.Unit, TypeRepr.Unit) -> loop rest
          ...
          | (TypeRepr.Named { head = left_head; arguments = left_arguments }, TypeRepr.Named { head = right_head; arguments = right_arguments }) ->
              if
                not
                  (TypeConstructorId.equal
                    left_head.type_constructor_id
                    right_head.type_constructor_id)
        ...
        (fun (value: TypeRepr.package_value) ->
          ...
          | Some binding ->
              let actual_ty =
                instantiate
                  state
                  (canonicalize_scheme_in_env state scope_env (Binding.scheme binding))
              in
              ...
              add_diagnostic
                ...
                (
                  Typ_diagnostic.UnboundName {
                    ...
                    name =
                      SurfacePath.append_name module_path value.name
...
and bind_argument_patterns = fun (state: state) env arguments argument_types ->
  let rec loop acc arguments argument_types =
    ...
    | (argument_id :: rest_arguments, argument_ty :: rest_types) ->
        loop (bind_pattern state env argument_id argument_ty :: acc) rest_arguments rest_types
    | (argument_id :: rest_arguments, []) ->
        loop (bind_pattern state env argument_id (fresh_hole state) :: acc) rest_arguments []
  ...
  | Some pattern -> (
      ...
      | BodyArena.PTuple elements ->
          ...
          let () =
            try_unify
              ...
              ~origin:(origin_of_pattern state pat_id)
              expected_ty
              (make_type state (TypeRepr.Tuple element_types))
          in
          List.fold_left2
      ...
      | BodyArena.POr alternatives -> (
          ...
          | alternative :: rest ->
              ...
              let alternative_bindings =
                ...
                |> List.map
                  (fun alternative_id ->
                    bind_pattern state env alternative_id expected_ty)
              in
      ...
      | BodyArena.PConstructor { constructor; arguments } -> (
          ...
          | Some constructor_entry ->
              let origin = origin_of_pattern state pat_id in
              let instantiated =
                instantiate_constructor_entry ~pattern_mode:true state env constructor_entry
              in
              let () =
              ...
              (
                ...
                | (Some labels, [ argument_id ]) -> (
                    ...
                    | Some { desc = BodyArena.PRecord { fields; open_ }; _ } ->
                        bind_inline_record_pattern state env argument_id labels fields open_
      ...
      | BodyArena.PList elements ->
          ...
          let () =
            try_unify
              ...
              ~origin:(origin_of_pattern state pat_id)
              expected_ty
              (make_type state (TypeRepr.List element_ty))
          in
          elements
          |> List.fold_left
            (fun acc element_id ->
            ...
            | (Some module_name, Some signature) ->
                {
                  ...
                  module_entries = [ (module_name, package_env state pat_id signature); ];
                }
...
let record_expr_trace = fun (state: state) expr_id origin_id env_before inferred_type ->
  let binding_provenance = function
    ...
    | Binding.TypeConstructor { type_name; scope_path } ->
        Check_result.TypeConstructor { type_name; scope_path }
    ...
    | Binding.ModuleAlias { alias_name; module_path } ->
...
let export_binding_refs = fun env ->
  Env.unique env
  |> Env.bindings
  |> List.sort (fun left right -> EntityId.compare (Binding.path left) (Binding.path right))
...
let rec infer_match_case = fun (state: state) env scrutinee_ty result_ty (
  ...
  let () =
    ...
    | Some guard_id ->
        ...
        try_unify
          state
          ~origin:(origin_of_expr state guard_id)
          guard_ty
          TypeRepr.bool
    | None -> ()
  in
  ...
  try_unify
    state
    ~origin:(origin_of_expr state case.body_id)
    result_ty
    case_ty
  ...
  let () =
    if not (List.is_empty missing_fields) then
      add_record_resolution_error
        ...
        ~context:Typ_diagnostic.RecordConstruction
        (Typ_diagnostic.MissingRecordFields missing_fields)
  in
  List.iter
    (fun (field: BodyArena.record_expr_field) ->
      ...
      try_unify
        state
        ~origin:(origin_of_expr state field.value_id)
        field_ty
        inferred_field_ty)
    fields
      ...
      let inferred_type =
            ...
            let () =
              List.iter
                (fun element_id ->
                  ...
                  try_unify
                    state
                    ~origin:(origin_of_expr state element_id)
                    element_ty
                    inferred_element)
                elements
            in
        ...
        | BodyArena.EFor {
          ...
          _
            ...
            let () =
              try_unify
                state
                ~origin:(origin_of_expr state start_id)
                start_ty
                TypeRepr.int
            in
            let () =
        ...
        | BodyArena.EApply (callee_id, arguments) ->
              ...
              | (BodyArena.Optional _, true) ->
                  ...
                  let () =
                    try_unify
                      ...
                      forwarded_option_ty
                      (TypeRepr.option forwarded_value_ty)
                  in
                  let () =
                    try_unify
                      state
            ...
            let rec apply_with_known_type current_ty arguments =
              ...
              | _ -> (
                  ...
                  | TypeRepr.Var _
                  | TypeRepr.Hole _ -> apply_from_unknown_type current_ty arguments
                  | _ ->
            ...
            begin
              ...
              | Some record_decl ->
                  ...
                  let () =
                    try_unify
                      state
                      ~origin:(origin_of_expr state receiver_id)
                      receiver_ty
                      owner_ty
                  in
                  begin
                  ...
                  begin
                    ...
                    | Some field_ty ->
                        ...
                        let () =
                          try_unify
                            state
                            ~origin:(origin_of_expr state value_id)
                            value_ty
                            field_ty
                        in
                        TypeRepr.unit_
        ...
        | BodyArena.EModulePack { module_path; package_type } -> (
            ...
            | Some package_type -> (
                ...
                | Some signature ->
                    let () =
                      check_module_pack_against_signature
                        state
                        env
                        ~origin:(origin_of_expr state expr_id)
                        module_path
        ...
        | BodyArena.ELocalModulePack { local_scope; package_type } -> (
            ...
            | Some package_type -> (
                ...
                | Some signature ->
                    let () =
                      check_local_module_pack_against_signature
                        state
                        env
                        ~origin:(origin_of_expr state expr_id)
                        local_scope
        ...
        | BodyArena.ELocalOpen { module_path; body_id } ->
            infer_expr
              state
              (with_local_open state env module_path)
              body_id
                ...
                | (parameter: BodyArena.function_parameter) :: rest -> (
                    ...
                    | TypeRepr.Arrow { label; lhs; rhs } when labels_match
                      ...
                      (type_label_of_body_label parameter.label) ->
                        ...
                        let bindings = bind_pattern state env parameter.pattern_id bound_ty in
                        check_parameters
                          (env_with_pattern_bindings env bindings)
                          rhs
                          rest
                    | _ ->
                      ...
                      (
                        ...
                        | (Some labels, [ argument ]) when argument_matches_parameter_label
                          ...
                          argument.label -> (
                            ...
                            | Some { desc = BodyArena.ERecord { base_id = None; fields }; _ } ->
                                let () =
                                  infer_inline_record_expr state env argument.value_id fields labels
                                in
                                let () =
                                  try_unify
                                    state
      ...
      if not (List.mem value.name local_binding_names) then
        ...
        | Some binding ->
            let actual_ty =
              instantiate
                state
                (canonicalize_scheme_in_env state local_env (Binding.scheme binding))
            in
            let expected_ty = TypeScheme.body value.scheme in
            try_unify state ~origin actual_ty expected_ty
        | None ->
  ...
  let (inferred_bindings, generalized_groups) =
    Solver.with_local_level_gen
      ...
      (fun () ->
        let inferred_bindings =
          List.map
            (fun (binding: BodyArena.binding) ->
              let export_annotation_scheme =
                binding.annotation
                |> Option.map (canonicalize_generalized_scheme_in_env state env)
              in
              let checking_annotation_scheme =
                binding.annotation
                |> Option.map (canonicalize_scheme_in_env state env)
              in
              let value_ty =
        ...
        in
        (inferred_bindings
        |> List.map fst, inferred_bindings
        |> List.map snd))
  in
            ...
            List.iter
              (fun ((binding: BodyArena.binding), _annotation_scheme, has_annotation, placeholder_ty, _entry) ->
                if has_annotation then
                  ...
                  try_unify
                    state
                    ~origin:(origin_of_binding state binding)
                    placeholder_ty
                    value_ty)
              placeholder_info
          in
  ...
  let initial_scope =
    ...
    |> List.fold_left
      (fun scope module_path ->
        Env.register_open scope ~scope_path:SurfacePath.empty ~module_path)
  ...
  let rec loop export_state type_decls scope = function
    ...
    | item :: rest -> (
        ...
        | ItemTree.Type type_item ->
            ...
            in
            let introduced =
              Env.of_type_decls [ { introduced_type_decl with scope_path = SurfacePath.empty } ]
            in
        ...
        | ItemTree.Exception exception_item ->
            let item_env =
              Env.for_item_scope export_state scope ~scope_path:exception_item.scope_path
            in
        ...
        | ItemTree.ExtensionConstructor extension_item ->
            let item_env =
              Env.for_item_scope export_state scope ~scope_path:extension_item.scope_path
            in
        ...
        | ItemTree.DeclaredValue declared_value_item ->
            let item_env =
              Env.for_item_scope export_state scope ~scope_path:declared_value_item.scope_path
            in
        ...
        | ItemTree.Include include_item ->
            ...
            let introduced = entries_for_include state item_env module_path in
            let introduced_type_decls =
              type_decls_for_include state.visible_types module_path
              |> prefix_type_decls include_item.scope_path
            in
            let (export_state, scope) =
        ...
        | ItemTree.ModuleAlias module_alias_item ->
            let item_env =
              Env.for_item_scope export_state scope ~scope_path:module_alias_item.scope_path
            in
            let module_path =
              resolve_module_path_in_scope
                state
            ...
            let visible_exports_before =
              if state.config.capture_traces then
                ...
                None
            in
            let alias_export_names =
              export_names_for_module_alias
                state
                item_env
            ...
            in
            let introduced_type_decls =
              type_decls_for_module_alias
                state.visible_types
                ~alias_name:module_alias_item.alias_name
                ~module_path
              |> prefix_type_decls module_alias_item.scope_path
            in
            let (export_state, scope) =
            ...
            let forced_export_names =
              if SurfacePath.is_empty module_alias_item.scope_path then
                ...
                List.map
                  (fun name ->
                    qualify_name module_alias_item.scope_path name
                    |> SurfacePath.to_string)
                  alias_export_names
            in
            let () =
            ...
            |> List.fold_left
              (fun acc binding_id ->
                ...
                | Some ({ name = Some name; annotation = Some annotation; _ }: BodyArena.binding) ->
                    (name, canonicalize_generalized_scheme_in_env state final_env annotation) :: acc
                | _ -> acc)
              acc
        | _ -> acc)
      []
