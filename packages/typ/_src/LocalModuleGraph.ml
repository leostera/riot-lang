...
let required_names_of_parse_result = fun ~current_module_name ~parse_result ~implicit_opens ->
  ...
  dedupe_by_key_preserving_order
    ...
    (
      (explicit_dependencies @ implicit_opens)
...
let visible_name_arrays_for_group = fun internal_name sources ->
  let visible_names =
    ...
    |> dedupe_by_key_preserving_order ~key:(fun visible_name -> visible_name)
  in
  let visible_names =
    if List.exists
      (
...
let candidate_ids_by_required_name = fun groups ->
  ...
  Collections.HashMap.iter
    (fun required_name module_ids_rev ->
      let _ =
        Collections.HashMap.insert by_name required_name (Array.of_list (List.rev module_ids_rev))
      in
      ())
    by_name_rev;
  by_name
...
let create = fun ~ordered_sources ->
  ...
  in
  let dependency_set_id_by_local_ids =
    Collections.HashMap.with_capacity (List.length ordered_sources + 1)
  in
  ...
  let intern_dependency_set local_ids =
    ...
    | None ->
        ...
        let _ =
          Collections.HashMap.insert dependency_set_id_by_local_ids local_ids_key dependency_set_id
        in
        dependency_set_id
  in
  let groups =
    Array.mapi
      (fun module_id (group: 'a group) ->
        ...
        let resolution_ids_by_required_name =
          source_requirements
          |> List.concat_map snd
          |> resolution_ids_by_required_name graph group
        in
        ...
        |> List.iter
          (fun ((source: 'a input_source), required_names) ->
            ...
            |> List.iter
              (fun required_module_name ->
                ...
                if Array.length local_ids = 0 then
                  ...
                  required_local_ids_rev := List.rev_append
                    (Array.to_list local_ids)
            ...
            let required_local_ids =
              ...
              |> Array.of_list
            in
            dependency_ids_rev := List.rev_append
              (Array.to_list required_local_ids)
              !dependency_ids_rev;
  ...
  let rec discover ordered_rev = function
    ...
    | group_id :: rest ->
        if Collections.HashSet.contains seen group_id then
          ...
          let dependencies =
            graph.groups.(group_id).dependency_ids
            |> Array.to_list
          in
...
let ordered_group_ids_from = fun graph group_ids ->
  ...
  let rec visit path ordered module_id =
    if not (Collections.HashSet.contains relevant module_id) then
      ...
      | 1 ->
          Error (
            cycle_module_ids path module_id
            |> cycle_of_module_ids graph
          )
      | _ ->
          state.(module_id) <- 1;
          let result =
