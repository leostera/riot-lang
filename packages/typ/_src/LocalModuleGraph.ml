...
let dedupe_module_ids_preserving_order = fun module_ids -> dedupe_by_key_preserving_order ~key:(
  fun module_id -> module_id
) module_ids
...
let required_names_of_parse_result = fun ~current_module_name ~parse_result ~implicit_opens ->
  ...
  in
  let implicit_opens =
    implicit_opens |> List.map SurfacePath.to_string |> List.filter
      (
      ...
      fun (source: 'a input_source) ->
        ...
        let existing_sources_rev =
          ...
          | None ->
              ...
              []
        in
        let _ = Collections.HashMap.insert sources_by_module_name module_name (source :: existing_sources_rev) in ()
    );
  !module_order_rev |> List.rev |> List.filter_map
    (
      fun module_name ->
        Collections.HashMap.get sources_by_module_name module_name |> Option.map
          (
            fun sources_rev -> (module_name, List.rev sources_rev)
          )
    )

let visible_name_arrays_for_group = fun internal_name sources ->
  let visible_names =
    sources |> List.concat_map
      (
        fun (source: 'a input_source) -> source.visible_names
      ) |> dedupe_by_key_preserving_order ~key:(
      fun visible_name -> visible_name
    )
  in
  let visible_names =
    if List.exists
      ...
      visible_names then
      visible_names
    else InternalName internal_name :: visible_names
  in
  ...
  Collections.HashMap.iter
    (
      fun required_name module_ids_rev ->
        let _ = Collections.HashMap.insert by_name required_name (Array.of_list (List.rev module_ids_rev)) in ()
    )
    by_name_rev;
  by_name

let dependency_local_ids = fun graph dependency_set_id -> graph.dependency_local_ids_by_set_id.(dependency_set_id)
    ...
    (
      fun candidate_id ->
        ...
        if not (Int.equal candidate_id group.id) then
          match LocalModules.contextual_match_depth ~current_module_name:group.internal_name ~required_module_name ~candidate_module_name:candidate_group.internal_name with
          | None -> ()
          | Some depth ->
              ...
              if Option.is_none !best_depth || depth > current_best then
                ...
                if Int.equal depth current_best then
...
let resolution_ids_by_required_name = fun graph (group: 'a group) required_names ->
  ...
  required_names |> dedupe_by_key_preserving_order ~key:(
    fun required_name -> required_name
  ) |> List.iter
    (
      fun required_name ->
...
let create = fun ~ordered_sources ->
  ...
  let groups =
    grouped_sources_array |> Array.mapi
      (
  ...
  in
  let dependency_set_id_by_local_ids = Collections.HashMap.with_capacity (List.length ordered_sources + 1) in
  ...
  let intern_dependency_set local_ids =
    ...
    | None ->
        ...
        let _ = Collections.HashMap.insert dependency_set_id_by_local_ids local_ids_key dependency_set_id in dependency_set_id
  in
  let groups =
    Array.mapi
      (
        fun module_id (group: 'a group) ->
          ...
          let source_requirements =
            sources |> List.map
              (
                fun (source: 'a input_source) -> (source, source.required_names)
              )
          in
          let resolution_ids_by_required_name = source_requirements |> List.concat_map snd |> resolution_ids_by_required_name graph group in
          ...
          source_requirements |> List.iter
            (
              fun ((source: 'a input_source), required_names) ->
                ...
                required_names |> List.iter
                  (
                    fun required_module_name ->
                      ...
                      else required_local_ids_rev := List.rev_append (Array.to_list local_ids) !required_local_ids_rev
                  );
                let required_local_ids = !required_local_ids_rev |> List.rev |> dedupe_module_ids_preserving_order |> Array.of_list in
                ...
                let _ = Collections.HashMap.insert graph.group_id_by_source_id (SourceId.to_int source.source_id) module_id in ()
            );
          let dependency_ids =
            !dependency_ids_rev |> List.rev |> List.filter
              (
                fun dependency_id -> not (Int.equal dependency_id module_id)
              ) |> dedupe_module_ids_preserving_order |> Array.of_list
          in
          { group with sources = List.rev !graph_sources_rev; dependency_ids }
      )
      groups
  in
  { graph with groups; dependency_local_ids_by_set_id = !dependency_local_ids_rev |> List.rev |> Array.of_list }
...
let cycle_of_module_ids = fun graph module_ids ->
  ...
  in
  let source_ids =
    module_ids |> List.concat_map
      (
        fun module_id ->
          graph.groups.(module_id).sources |> List.map
            (
              fun (source: 'a graph_source) -> source.input.source_id
            )
...
let reachable_group_ids = fun graph ~roots ->
  let root_group_ids =
    roots |> List.filter_map
      (
        fun source_id -> Collections.HashMap.get graph.group_id_by_source_id (SourceId.to_int source_id)
  ...
  let rec discover ordered_rev = function
    ...
    | group_id :: rest ->
        if Collections.HashSet.contains seen group_id then
          ...
          let dependencies = graph.groups.(group_id).dependency_ids |> Array.to_list in discover (group_id :: ordered_rev) (dependencies @ rest)
  in
...
let ordered_group_ids_from = fun graph group_ids ->
  ...
  let rec visit path ordered module_id =
    if not (Collections.HashSet.contains relevant module_id) then
      ...
      | 2 -> Ok ordered
      | 1 -> Error (cycle_module_ids path module_id |> cycle_of_module_ids graph)
      | _ ->
          state.(module_id) <- 1;
          let result =
            graph.groups.(module_id).dependency_ids |> Array.fold_left
              (
...
let ordered_group_ids = fun graph ->
  graph.groups |> Array.to_list |> List.map
    (
      fun (group: 'a group) -> group.id
...
let ordered_closure_group_ids = fun graph ~roots -> closure_group_ids graph ~roots |> ordered_group_ids_from graph

let closure_source_ids = fun graph ~roots ->
  closure_group_ids graph ~roots |> List.concat_map
    (
      fun group_id ->
        graph.groups.(group_id).sources |> List.map
          (
            fun (source: 'a graph_source) -> source.input.source_id
          )
    ) |> List.sort_uniq SourceId.compare
