  ...
  let module_names =
    ...
    |> List.fold_left
      (fun names (slot: analysis_slot) ->
        ...
        let existing =
          Collections.HashMap.get module_slots_by_name module_name
...
let slot_of_source_id = fun snapshot source_id ->
  snapshot.analyses
  |> List.find_opt (fun (slot: analysis_slot) -> SourceId.equal slot.source_id source_id)

let module_slots = fun snapshot module_name ->
  Collections.HashMap.get snapshot.module_slots_by_name module_name
  |> Option.unwrap_or ~default:[]
...
let rooted_slots = fun snapshot ->
  ...
  |> List.filter
    (fun (slot: analysis_slot) ->
      snapshot.roots
      |> List.exists (SourceId.equal slot.source_id))

let rooted_module_names = fun snapshot ->
  rooted_slots snapshot
...
let best_matching_local_module_names = fun (snapshot: t) (slot: analysis_slot) ~required_name ->
  ...
  let matching_candidates =
    candidate_local_module_names snapshot slot
    |> List.filter_map
      (fun candidate_module_name ->
  ...
  |> List.iter
    (fun required_name_string ->
      ...
      | _ ->
          ...
          |> List.iter
            (fun module_name ->
              let module_id = PackageEnv.ModuleId.Local (LocalModules.InternalName.of_string
                module_name)
              in
              add_visible_module module_id required_name;
              if required_name_is_implicit_open required_name then
...
let with_partial_module_view = fun module_typings (analysis: SourceAnalysis.t) ->
  let file_summary =
    ModuleTypings.to_file_summary ~source_id:analysis.source.source_id module_typings
  in
  {
    analysis with
...
and module_result_shared_cache_key = fun (snapshot: t) module_name ->
  ...
  | None ->
      ...
      in
      let _ =
        Collections.HashMap.insert snapshot.module_result_keys_cache module_name (Some cache_key)
      in
      cache_key
...
and visible_module_dependency_cache_key = fun (snapshot: t) ~role (visible_path, module_id) ->
  let module_key =
    ...
    | PackageEnv.ModuleId.Loaded required_name ->
        format Format.[ str "loaded:"; str (LocalModules.RequiredName.to_string required_name) ]
      ...
      if module_is_in_progress then
        ...
        | None ->
            ...
            | None ->
                ...
                let source_ids =
                  slots
                  |> List.map (fun (slot: analysis_slot) -> slot.source_id)
                in
                (
                  match slots with
                  | [] -> ()
...
let module_result_of_source = fun snapshot source_id ->
  slot_of_source_id snapshot source_id
  |> Option.map
    (fun (slot: analysis_slot) ->
      let module_name = Source.module_name slot.source in
...
let is_root = fun snapshot source_id ->
  snapshot.roots
  |> List.exists (SourceId.equal source_id)
...
let analyses = fun snapshot ->
  ...
  |> List.filter_map
    (fun (slot: analysis_slot) ->
      ...
      | Some (_module_name, Some result) ->
          List.assoc_opt slot.source_id ModulePairing.(result.analyses_by_source)
      | Some (_, None)
      | None -> None)

let file_summaries = fun snapshot ->
  analyses snapshot
  |> List.map (fun (analysis: SourceAnalysis.t) -> analysis.file_summary)
...
let find_analysis = fun snapshot source_id ->
  if not (is_root snapshot source_id) then
    ...
    match module_result_of_source snapshot source_id with
    | Some (_module_name, Some result) ->
        List.assoc_opt source_id ModulePairing.(result.analyses_by_source)
    | Some (_, None)
    | None -> None
