...
let collect_missing_module_summaries = fun session ~closure_sources ->
  ...
  let closure_source_ids = closure_sources
...
let local_source_closure = fun session ~graph ~roots ->
  ...
  session.sources |> List.filter
    (fun (source: Source.t) ->
      Collections.HashSet.contains closure_source_ids source.source_id)
...
let local_module_cycles = fun ~(graph:Source.t LocalModuleGraph.t) ~roots ->
  ...
  in
  let relevant_group_ids = LocalModuleGraph.closure_group_ids graph ~roots
    ...
    TypConfig.emit_event session.config
      (fun () ->
        Event.PrepareSnapshotFailed {
          ...
          missing_modules =
            MissingRequirements.requirements missing_requirements |> List.concat_map
              (
                function
