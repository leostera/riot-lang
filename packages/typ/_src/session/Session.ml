...
let source_of_id = fun session source_id -> Collections.HashMap.get session.sources_by_id (SourceId.to_int source_id)
...
let module_name_index_keys = fun (source: Source.t) ->
  let internal_name = LocalModules.InternalName.of_string (Source.module_name source) in (Source.module_name source) :: (LocalModules.local_module_aliases_of_internal_name internal_name |> List.map LocalModules.AmbientName.to_string) |> List.sort_uniq String.compare
...
let local_source_ids_for_module_in_scope = fun session ~current_module_name module_name ->
  ...
  let matching_sources =
    source_ids_of_module session module_name |> List.filter_map (source_of_id session) |> List.filter_map
      (
        fun (source: Source.t) ->
          LocalModules.contextual_match_depth ~current_module_name ~required_module_name ~candidate_module_name:(LocalModules.InternalName.of_string (Source.module_name source)) |> Option.map
            (
              fun depth -> (source.source_id, depth)
            )
      )
  in
  let best_depth =
    matching_sources |> List.fold_left
      (
...
let root_module_names = fun session roots -> roots |> List.filter_map (source_of_id session) |> List.map Source.module_name |> List.sort_uniq String.compare
...
let register_module_name = fun session ~module_name ~source_id ->
  ...
  let module_names =
    ...
    | None -> [ module_name ]
  in
  let _ = Collections.HashMap.insert session.module_names_by_source_id (SourceId.to_int source_id) module_names in session
...
let add_source_indexes = fun session (source: Source.t) ->
  ...
  module_name_index_keys source |> List.fold_left
    (
      fun session module_name -> register_module_name session ~module_name ~source_id:source.source_id
    )
    session

let update_source_indexes = fun session (source: Source.t) ->
  let _ = Collections.HashMap.insert session.sources_by_id (SourceId.to_int source.source_id) source in session
    ...
    (
      fun module_name ->
        let remaining_source_ids =
          source_ids_of_module session module_name |> List.filter
            (
...
let visible_module_names_of_source = fun session (source: Source.t) ->
  ...
  LocalModuleGraph.InternalName internal_name :: (
    module_names_of_source_id session source.source_id |> List.map
      (
...
let create_source = fun session ~kind ~module_name ~implicit_opens ~origin ~source_hash ~parse_result ~cst ->
  ...
  let source = Source.make_prepared ~source_id ~kind ~module_name ~implicit_opens ~origin ~revision:session.next_revision ~source_hash ~parse_result ~cst in
  let session =
    ({ session with next_source_id = session.next_source_id + 1; next_revision = session.next_revision + 1; sources = session.sources @ [ source ] }) |> fun session -> add_source_indexes session source
  in
  (session, source_id)
...
let merge_loaded_modules = fun preferred fallback -> LoadedModules.merge ~preferred ~fallback:(LoadedModules.of_list fallback) ~combine:(
  fun existing _incoming -> existing
)
...
let update_source = fun session source_id ~source_hash ~parse_result ~cst ->
  ...
  | Some source ->
      let updated = Source.make_prepared ~source_id:source.source_id ~kind:source.kind ~module_name:source.module_name ~implicit_opens:source.implicit_opens ~origin:source.origin ~revision ~source_hash ~parse_result ~cst in
      ({ session with next_revision = revision + 1; sources = replace_source session.sources updated }) |> fun session -> update_source_indexes session updated
...
let module_segments_of_export_name = fun export_name ->
  let rec loop acc = function
    | segment :: rest when String.length segment > 0 && is_uppercase_ascii segment.[0] -> loop (segment :: acc) rest
    | _ -> List.rev acc
  in
...
let nested_module_prefixes_of_surface = fun ~exports ~type_decls ->
  let export_prefixes =
    exports |> List.map fst |> List.filter_map
      (
        fun export_name ->
...
let nested_module_prefixes_of_typings = fun module_typings -> nested_module_prefixes_of_surface ~exports:(ModuleTypings.exports module_typings) ~type_decls:(ModuleTypings.type_decls module_typings)

let nested_module_prefixes_of_compiled_scope = fun compiled_scope -> nested_module_prefixes_of_surface ~exports:(CompiledScope.exports compiled_scope) ~type_decls:(CompiledScope.type_decls compiled_scope)
...
let deps_env_for_loaded_modules = fun session loaded_modules ->
  ...
  | None ->
      let add_summary_paths env summary =
        ...
        let env =
          ModuleTypings.exports summary |> List.fold_left
            (
              fun env (export_name, _) ->
                match module_segments_of_export_name export_name with
                | [] -> env
                | segments -> Syn.Deps.Env.add_path env ~path:(module_name :: segments) ~free_names:[ module_name ]
            )
            env
        in
          ...
          (
            fun env (type_decl: FileSummary.type_decl) ->
              if SurfacePath.is_empty type_decl.scope_path then
                env
              else Syn.Deps.Env.add_path env ~path:(module_name :: SurfacePath.to_segments type_decl.scope_path) ~free_names:[ module_name ]
          )
          env
      in
      let env =
        LoadedModules.fold
          ...
          Syn.Deps.Env.empty
      in
      let _ = Collections.HashMap.insert session.deps_envs_by_loaded_modules key env in (key, env)

let parse_result_for_source = fun session (source: Source.t) ->
  let _ = session in source.parse_result
...
let declared_modules = fun session (source: Source.t) ->
  ...
  | None ->
      ...
      in
      let _ = Collections.HashMap.insert session.declared_modules_by_source_hash key modules in modules

let module_path_of_ident = fun ident -> Syn.Cst.Ident.segments ident |> List.map Syn.Cst.Token.text |> String.concat "."
...
let rec module_path_of_module_expression = function
  ...
  | Syn.Cst.ModuleExpression.Constraint { module_expression; _ } -> module_path_of_module_expression module_expression
  | Syn.Cst.ModuleExpression.Attribute { module_expression; _ } -> module_path_of_module_expression module_expression
  | Syn.Cst.ModuleExpression.Structure _ | Syn.Cst.ModuleExpression.Functor _ | Syn.Cst.ModuleExpression.Apply _ | Syn.Cst.ModuleExpression.ApplyUnit _ | Syn.Cst.ModuleExpression.ModuleUnpack _ | Syn.Cst.ModuleExpression.Extension _ -> None
...
let rec module_path_of_module_type = function
  ...
  | Syn.Cst.ModuleType.Path _ | Syn.Cst.ModuleType.Signature _ | Syn.Cst.ModuleType.Functor _ | Syn.Cst.ModuleType.Extension _ -> None
      ...
      let module_paths =
        ...
        | Syn.Cst.Implementation implementation ->
            Syn.Cst.(implementation.items) |> List.filter_map
              (
                function
                | Syn.Cst.StructureItem.IncludeStatement include_statement -> include_path_of_target include_statement.target
                | _ -> None
              )
        | Syn.Cst.Interface interface ->
            Syn.Cst.(interface.items) |> List.filter_map
              (
                function
                | Syn.Cst.SignatureItem.IncludeStatement include_statement -> include_path_of_target include_statement.target
                | _ -> None
              )
      in
      let module_paths = List.sort_uniq String.compare module_paths in
      let _ = Collections.HashMap.insert session.top_level_includes_by_source_hash key module_paths in module_paths
...
let module_dependencies = fun session ~deps_env_key ~deps_env (source: Source.t) ->
  ...
  | None ->
      let modules =
        ...
        | Error _ -> []
      in
      let _ = Collections.HashMap.insert session.module_dependencies_by_env_and_source_hash key modules in modules
    ...
    (
      fun module_name ->
        if Collections.HashSet.contains seen module_name then
          ...
          let _ = Collections.HashSet.insert seen module_name in true
    )
...
let local_required_names_of_source = fun session ~deps_env_key ~deps_env (source: Source.t) ->
  ...
  let implicit_open_modules =
    source.implicit_opens |> List.map SurfacePath.to_string |> List.filter
      (
        fun module_name -> LocalModules.should_include_implicit_open ~current_module_name ~module_name
      )
  in
  module_dependencies session ~deps_env_key ~deps_env source @ implicit_open_modules |> dedupe_strings_preserving_order |> List.map LocalModules.RequiredName.of_string

let local_module_graph = fun session ->
  let (deps_env_key, deps_env) = deps_env_for_loaded_modules session session.config.loaded_modules in LocalModuleGraph.create ~ordered_sources:(
...
let collect_missing_module_summaries = fun session ~closure_sources ->
  ...
  let loaded_module_names = session.config.loaded_modules |> LoadedModules.names |> Collections.HashSet.of_list in
  let closure_source_ids =
    closure_sources |> List.map
      (
        fun (source: Source.t) -> source.source_id
  ...
  let local_dependency_source_ids source module_name =
    local_source_ids_for_module_in_scope session ~current_module_name:(Source.module_name source) module_name |> List.filter
      (
        fun source_id -> Collections.HashSet.contains closure_source_ids source_id
      )
  in
  let dependency_is_loaded_only module_name = Collections.HashSet.contains loaded_module_names (LocalModules.RequiredName.of_string module_name) && not (has_local_source_for_module session module_name) in
  ...
  let loaded_nested_module_prefixes module_name =
    ...
    | None -> []
    | Some (head, suffix) -> (
      match LoadedModules.get session.config.loaded_modules ~required_name:(LocalModules.RequiredName.of_string head) with
      | None -> []
      | Some summary ->
          ...
          let target_scope =
            if SurfacePath.is_empty suffix then
              Some root_scope
            else CompiledScope.lookup_module root_scope suffix
          in
          target_scope |> Option.map nested_module_prefixes_of_compiled_scope |> Option.unwrap_or ~default:[]
    )
  in
  let resolve_local_module_names ~current_module_name module_name =
    local_source_ids_for_module_in_scope session ~current_module_name module_name |> List.filter
      (
        fun source_id -> Collections.HashSet.contains closure_source_ids source_id
      ) |> List.filter_map (source_of_id session) |> List.map Source.module_name |> List.sort_uniq String.compare
  in
  let rec nested_module_prefixes_of_module_path ~current_module_name module_name =
    ...
    | local_module_names -> local_module_names |> List.concat_map local_nested_module_prefixes_of_internal_name |> List.sort_uniq String.compare
  and local_nested_module_prefixes_of_internal_name module_name =
    ...
    | None ->
        ...
        let module_sources = source_ids_in_closure_for_module module_name |> List.filter_map (source_of_id session) in
        let declared_nested_modules = module_sources |> List.concat_map (declared_modules session) in
        let included_nested_modules =
          module_sources |> List.concat_map
            (
              fun (source: Source.t) -> top_level_include_module_paths session source |> List.concat_map (nested_module_prefixes_of_module_path ~current_module_name:source.module_name)
            )
        in
        let nested_modules = declared_nested_modules @ included_nested_modules |> List.sort_uniq String.compare in
        let _ = Collections.HashMap.insert local_nested_module_prefixes_cache module_name nested_modules in nested_modules
  in
  let implicit_open_nested_modules source = implicit_open_module_names source |> List.concat_map (nested_module_prefixes_of_module_path ~current_module_name:(Source.module_name source)) |> List.sort_uniq String.compare in
  ...
  in
  let missing =
    closure_sources |> List.fold_left
      (
        fun missing (source: Source.t) ->
          ...
          let dependency_nested_modules =
            if List.is_empty unresolved_modules then
              ...
              source_modules |> List.concat_map (nested_module_prefixes_of_module_path ~current_module_name:(Source.module_name source)) |> fun nested_modules -> nested_modules @ implicit_open_nested_modules source |> List.sort_uniq String.compare
          in
            ...
            (
              fun missing module_name ->
                if List.mem module_name dependency_nested_modules then
                  missing
                else add_missing missing module_name source.source_id
            )
            missing
      )
      []
  ...
  MissingRequirements.of_list
    (
      missing |> List.map
        (
          fun (module_name, requested_by) -> MissingRequirements.MissingModuleSummary { module_name; requested_by }
        )
    )

let local_source_closure = fun session ~graph ~roots ->
  let closure_source_ids = LocalModuleGraph.closure_source_ids graph ~roots |> Collections.HashSet.of_list in
...
let local_module_cycles = fun ~(graph:Source.t LocalModuleGraph.t) ~roots ->
  let cycle_relevant_group (group: Source.t LocalModuleGraph.group) =
    group.sources |> List.exists
      (
        fun (source: Source.t LocalModuleGraph.graph_source) ->
          match source.input.payload.kind with
          | Source.Generated -> false
          | Source.File | Source.Fragment -> true
      )
  in
  let relevant_group_ids =
    LocalModuleGraph.closure_group_ids graph ~roots |> List.filter
      (
        fun group_id -> cycle_relevant_group graph.groups.(group_id)
      )
  in
  match LocalModuleGraph.ordered_subset_group_ids graph ~group_ids:relevant_group_ids with
  | Ok _ -> []
  | Error cycle -> [ MissingRequirements.LocalModuleCycle { module_names = cycle.module_names; source_ids = cycle.source_ids } ]

let prepare_snapshot = fun session ~roots ->
  let missing_root_source_ids =
    roots |> List.filter
      (
        fun root_id -> Option.is_none (source_of_id session root_id)
      )
  in
  let missing_roots =
    missing_root_source_ids |> List.map
      (
        fun source_id -> MissingRequirements.MissingRootSource { source_id }
      )
  in
      ...
      let rec hydrate_session session =
        ...
        | Some store ->
            let missing_module_names =
              MissingRequirements.requirements missing_modules |> List.filter_map
                (
                  function
            ...
            if List.is_empty missing_module_names then
              ...
              TypConfig.emit_event session.config
                (
                  fun () -> Event.HydrateModuleTypingsStarted { roots; missing_modules = missing_module_names }
                );
            let hydrated =
              missing_module_names |> List.filter_map
                (
                  fun module_name -> Store.load_module_typings store ~module_name
                )
            in
            let loaded_modules =
              if List.is_empty hydrated then
                session.config.loaded_modules
              else merge_loaded_modules session.config.loaded_modules hydrated
            in
            if List.is_empty missing_module_names then
              ()
            else
              TypConfig.emit_event session.config
                (
                  fun () -> Event.HydrateModuleTypingsFinished { roots; hydrated_modules = hydrated |> List.map ModuleTypings.module_name |> List.sort_uniq String.compare; loaded_module_count = LoadedModules.len loaded_modules }
                );
            if List.is_empty hydrated then
              (session, missing_modules)
            else
              let session = { session with config = TypConfig.with_loaded_module_index session.config ~loaded_modules } in hydrate_session session
      in
      let (session, missing_modules) = hydrate_session session in
      let missing_requirements = MissingRequirements.of_list (missing_roots @ MissingRequirements.requirements missing_modules) in (session, missing_requirements)
    else (session, MissingRequirements.of_list (missing_roots @ cycle_requirements))
  in
  ...
  if MissingRequirements.(missing_requirements |> is_empty) then
    ...
    TypConfig.emit_event session.config
      (
        fun () -> Event.SnapshotMaterializationStarted { roots; local_source_count = List.length sources; revision = session.next_revision }
      );
    let snapshot = Snapshot.make_with_shared_caches ~revision:session.next_revision ~roots ~config:snapshot_config ~sources ~shared_caches:session.shared_snapshot_caches in
    let module_count = sources |> List.map Source.module_name |> List.sort_uniq String.compare |> List.length in
    TypConfig.emit_event session.config
      (
        fun () ->
        ...
        (
          fun () ->
            Event.PrepareSnapshotFailed {
              ...
              missing_modules = MissingRequirements.requirements missing_requirements |> List.concat_map
                (
                  function
