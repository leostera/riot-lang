...
let diagnostics = fun snapshot source_id ->
  ...
  | Some analysis ->
      (
        (
          analysis.parse_diagnostics |> List.map
            (
              fun diagnostic -> Parse diagnostic
            )
        ) @ (
          analysis.lowering_diagnostics |> List.map
            (
              fun diagnostic -> Lowering diagnostic
            )
        )
      ) @ (
        analysis.typing_diagnostics |> List.map
          (
            fun diagnostic -> Typing diagnostic
          )
      )
