...
let qualify_local_head = fun local_types module_name (head: TypeRepr.named_type_head) ->
  ...
  | Some type_decl ->
      { head with name = SurfacePath.prepend_name module_name (type_decl_key type_decl) }
  | None -> (
  ...
  |> List.map
    (fun (label: TypeDecl.label) ->
      let qualified_field_type =
        TypeScheme.map_type_preserving (qualify_type local_types module_name) label.field_type
      in
                  ...
                  |> List.map
                    (fun (tag: TypeDecl.poly_variant_tag) ->
                      ...
                      | Some payload_type ->
                          {
                            tag with
                            payload_type = Some (qualify_type local_types module_name payload_type);
                          }
                      | None -> tag);
      ...
      let constructors =
        ...
        |> List.map
          (fun (constructor: TypeDecl.constructor) ->
            {
              constructor with
              scheme = qualify_scheme_with_local_types local_types ~module_name constructor.scheme;
...
let qualify_signature_type_decls = fun ~module_name type_decls ->
  ...
  List.map
    (fun (type_decl: FileSummary.type_decl) ->
      let qualified_decls =
        qualify_type_decls_with_local_types local_types ~module_name [ type_decl ]
      in
      match qualified_decls with
      | [ qualified_type_decl ] ->
          { qualified_type_decl with FileSummary.scope_path = type_decl.scope_path }
...
let rebind_value_definition = fun
  ~from_module_name ~to_module_name (definition: ModuleTypings.value_definition) -> {
  definition with
...
let rebind_module_typings = fun ~module_name typings ->
  ...
  let value_definitions =
    ModuleTypings.value_definitions typings
    |> List.map
      (rebind_value_definition
        ~from_module_name:(ModuleTypings.module_name typings)
