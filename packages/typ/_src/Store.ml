...
let load_module_typings_by_hash = fun store ~source_hash ->
  ...
  | None ->
      let typings =
        ...
        | None -> None
        | Some json ->
            ModuleTypings.Json.of_json json
            |> Result.to_option
      in
...
let rec load_package_module_typings = fun store ~package_name ->
  load_package_bundle store ~package_name
  |> Option.map (fun (bundle: package_bundle) -> bundle.typings)
      ...
      (
        ...
        |> List.iter
          (fun typings ->
            let module_name = ModuleTypings.module_name typings in
            let source_hash =
              ModuleTypings.source_hash typings
              |> source_hash_key
            in
            let _ =
              Collections.HashMap.insert store.module_typings_by_name module_name (Some typings)
            in
            let _ =
              Collections.HashMap.insert store.module_typings_by_hash source_hash (Some typings)
            in
            ())
      );
      Ok ()
