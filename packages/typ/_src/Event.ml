...
let source_ids_to_json = fun source_ids ->
  Data.Json.Array (
    source_ids |> List.map
      (
        fun source_id -> Data.Json.Int (SourceId.to_int source_id)
      )
  )
...
let parse_diagnostics_to_json = fun diagnostics -> Data.Json.Array (List.map Syn.Diagnostic.to_json diagnostics)

let diagnostics_to_json = fun diagnostics -> Data.Json.Array (List.map Diagnostic.to_json diagnostics)
...
let object_with_instant = fun instant_us fields ->
  Data.Json.Object (
    fields @ [
      "instant_us", Data.Json.Int instant_us;
    ]
  )
...
let to_json = fun event ->
  ...
  | PrepareSnapshotStarted { roots; root_modules; session_source_count; loaded_module_count } ->
  ...
  | HydrateModuleTypingsStarted { roots; missing_modules } ->
      object_with_instant instant_us
        [
  ...
  | HydrateModuleTypingsFinished { roots; hydrated_modules; loaded_module_count } ->
      object_with_instant instant_us
        [
  ...
  | PrepareSnapshotFailed { roots; missing_root_source_ids; missing_modules } ->
      object_with_instant instant_us
        [
  ...
  | PrepareSnapshotFinished { roots; local_source_count; loaded_module_count; revision } ->
      object_with_instant instant_us
        [
  ...
  | SnapshotMaterializationStarted { roots; local_source_count; revision } ->
      object_with_instant instant_us
        [
  ...
  | SnapshotMaterializationFinished { roots; local_source_count; module_count; revision } ->
      object_with_instant instant_us
        [
  ...
  | ModuleTypingsCollectionStarted { roots; rooted_module_count } ->
      object_with_instant instant_us
        [
  ...
  | ModuleTypingsCollectionFinished { roots; rooted_module_count; produced_module_count } ->
      object_with_instant instant_us
        [
  ...
  | SourceAnalysisStarted { source_id; module_name; mode; local_module_names; loaded_module_count; ambient_binding_count; ambient_type_decl_count } ->
      object_with_instant instant_us
        [
  ...
  | SourceAnalysisFinished { source_id; module_name; mode; parse_diagnostic_count; lowering_diagnostic_count; typing_diagnostic_count; parse_diagnostics; lowering_diagnostics; typing_diagnostics; export_status; export_count; type_decl_count } ->
      object_with_instant instant_us
        [
  ...
  | ModulePairingStarted { module_name; source_ids } ->
      object_with_instant instant_us
        [
  ...
  | ModulePairingFinished { module_name; source_ids; export_status; export_count; type_decl_count; mismatch_count; mismatch_subjects; mismatch_messages } ->
      object_with_instant instant_us
        [
