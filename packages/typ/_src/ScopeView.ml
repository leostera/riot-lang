  ...
  let visible_modules =
    ...
    |> List.map
      (fun (path, module_id) -> {
        path;
        module_id;
        depth = List.length (SurfacePath.to_segments path);
      })
  in
  ...
  let () =
    ...
    |> List.iter
      (fun ({ path; module_id; _ } as visible_module) ->
        ...
        | Some (head, _tail) ->
            let existing =
              Collections.HashMap.get visible_modules_by_head head
              |> Option.unwrap_or ~default:[]
            in
            let _ =
              Collections.HashMap.insert visible_modules_by_head head (visible_module :: existing)
            in
            ()
        | None -> ())
  in
...
let resolve_visible_module_prefix = fun scope_view path ->
  ...
  match SurfacePath.uncons path with
  | Some (head, _tail) ->
      Option.and_then (Collections.HashMap.get scope_view.visible_modules_by_head head) choose
  | None -> None
