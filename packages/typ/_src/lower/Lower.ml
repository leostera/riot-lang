...
let qualify_scoped_name = fun scope_path name -> SurfacePath.append_name scope_path name

let register_local_module_functor = fun (state: state) name template ->
  let path = qualify_scoped_name state.scope_path name in state.local_module_functors <- (path, template) :: state.local_module_functors

let resolve_local_module_functor = fun (state: state) path ->
  let lookup candidate_path =
    state.local_module_functors |> List.find_map
      (
  ...
  if SurfacePath.is_bare path then
    ...
    | Some name ->
        SurfacePath.prefixes state.scope_path |> List.rev |> List.find_map
          (
            fun scope_path -> lookup (qualify_scoped_name scope_path name)
          )
    | None -> None
  else lookup path
...
let resolve_named_type_name = fun (state: state) name ->
  let rec loop = function
    ...
    | scope_path :: rest ->
        if List.exists
          (
            fun (candidate_name, candidate_scope_path, _) -> String.equal candidate_name name && SurfacePath.equal candidate_scope_path scope_path
          )
          state.declared_type_names then
          let type_constructor_id =
            state.declared_type_names |> List.find_map
              (
                fun (candidate_name, candidate_scope_path, candidate_id) ->
                  if String.equal candidate_name name && SurfacePath.equal candidate_scope_path scope_path then
                    Some candidate_id
                  else None
              )
          in
          type_constructor_id |> Option.map
            (
              fun type_constructor_id -> TypeRepr.named_head ~type_constructor_id ~name:(qualify_scoped_name scope_path name)
            )
        else loop rest
  in
...
let resolve_named_type_path = fun (state: state) path ->
  let external_head () = TypeRepr.named_head ~type_constructor_id:(TypeConstructorId.of_path path) ~name:path in
  match SurfacePath.split_last path with
  | Some (scope_path, type_name) ->
      state.declared_type_names |> List.find_map
        (
          fun (candidate_name, candidate_scope_path, candidate_id) ->
            if String.equal candidate_name type_name && SurfacePath.equal candidate_scope_path scope_path then
        ...
        Option.or_else resolved
          (
            fun () ->
              Option.or_else (BuiltinTypeConstructors.head_of_path path)
                (
...
let register_module_type_template = fun (state: state) name template ->
  let path = qualify_scoped_name state.scope_path name in state.module_type_templates <- (path, template) :: state.module_type_templates

let resolve_module_type_template = fun (state: state) path ->
  let lookup candidate_path =
    state.module_type_templates |> List.find_map
      (
        fun (template_path, template) ->
          if SurfacePath.equal template_path candidate_path then
            Some template
...
let register_declared_type_name = fun (state: state) name ->
  match state.declared_type_names |> List.find_map
    (
      fun (candidate_name, candidate_scope_path, candidate_id) ->
        if String.equal candidate_name name && SurfacePath.equal candidate_scope_path state.scope_path then
          Some candidate_id
        else None
    ) with
  ...
  | None ->
      ...
      let () = state.next_type_constructor_id <- state.next_type_constructor_id + 1 in
      let binding = name, state.scope_path, type_constructor_id in
      let () = state.declared_type_names <- binding :: state.declared_type_names in type_constructor_id
...
let with_nonrec_current_type_name_hidden = fun (state: state) (declaration: Cst.TypeDeclaration.t) f ->
  ...
  | Some _ ->
      ...
      let () = state.declared_type_names <- List.filter
        (
          fun (candidate_name, candidate_scope_path, _) -> not (String.equal candidate_name type_name && SurfacePath.equal candidate_scope_path state.scope_path)
        )
        state.declared_type_names in
      try
        let lowered = f () in
        let () = state.declared_type_names <- saved_declared_type_names in lowered
      with
      | error ->
          let () = state.declared_type_names <- saved_declared_type_names in raise error

let is_module_name = fun name -> String.length name > 0 && Char.uppercase_ascii name.[0] = name.[0] && Char.lowercase_ascii name.[0] != name.[0]
...
let rec module_path_segments_of_expr = fun (state: state) ->
  ...
  | Cst.Expression.Path { path; _ } ->
      ...
      if List.is_empty segments || not (List.for_all is_module_name segments) then
        None
      else Some (resolve_local_module_alias_path state (SurfacePath.of_segments segments))
...
let rec module_path_segments_of_module_expression = fun (state: state) ->
  ...
  | Cst.ModuleExpression.Parenthesized { inner; _ } | Cst.ModuleExpression.Attribute { module_expression = inner; _ } | Cst.ModuleExpression.Constraint { module_expression = inner; _ } -> module_path_segments_of_module_expression state inner
  | _ -> None
...
let rec unpacked_expression_of_module_expression = function
  ...
  | Cst.ModuleExpression.Parenthesized { inner; _ } | Cst.ModuleExpression.Attribute { module_expression = inner; _ } | Cst.ModuleExpression.Constraint { module_expression = inner; _ } -> unpacked_expression_of_module_expression inner
  | _ -> None

let rec structure_items_of_module_expression = function
  | Cst.ModuleExpression.Parenthesized { inner; _ } | Cst.ModuleExpression.Attribute { module_expression = inner; _ } | Cst.ModuleExpression.Constraint { module_expression = inner; _ } -> structure_items_of_module_expression inner
...
let rec include_target_path_of_module_type = function
  ...
  | Cst.ModuleType.Parenthesized { inner; _ } | Cst.ModuleType.Attribute { module_type = inner; _ } | Cst.ModuleType.With { base = inner; _ } -> include_target_path_of_module_type inner
  | _ -> None
...
let builtin_type_of_name = fun name arguments ->
  ...
  | ("exn", []) -> Some (TypeRepr.named ~head:(TypeRepr.named_head ~type_constructor_id:BuiltinTypeConstructors.exn_type_constructor_id ~name:(SurfacePath.of_name "exn")) ~arguments:[])
  ...
  let rec loop replacements ty =
    ...
    | TypeRepr.Option element ->
        ...
        if Std.Ptr.equal element substituted_element then
          ty
        else TypeRepr.option substituted_element
    | TypeRepr.Result (ok_ty, error_ty) ->
        ...
        if Std.Ptr.equal ok_ty substituted_ok_ty && Std.Ptr.equal error_ty substituted_error_ty then
          ty
        else TypeRepr.result substituted_ok_ty substituted_error_ty
    | TypeRepr.Array element ->
        let substituted_element = loop replacements element in
        if Std.Ptr.equal element substituted_element then
          ty
        else TypeRepr.array substituted_element
    | TypeRepr.List element ->
        let substituted_element = loop replacements element in
        if Std.Ptr.equal element substituted_element then
          ty
        else TypeRepr.list substituted_element
    | TypeRepr.Seq element ->
        let substituted_element = loop replacements element in
        if Std.Ptr.equal element substituted_element then
          ty
        else TypeRepr.seq substituted_element
        ...
        begin
          ...
          | _ ->
              if List.for_all2 Std.Ptr.equal arguments substituted_arguments then
                ty
              else TypeRepr.named ~head ~arguments:substituted_arguments
        end
    ...
    | TypeRepr.PolyVariant { bound; tags; inherited } ->
        ...
        if List.for_all2 Std.Ptr.equal tags substituted_tags && List.for_all2 Std.Ptr.equal inherited substituted_inherited then
          ty
        else TypeRepr.poly_variant ~bound ~tags:substituted_tags ~inherited:substituted_inherited
    | TypeRepr.Tuple members ->
        ...
        if List.for_all2 Std.Ptr.equal members substituted_members then
          ty
        else TypeRepr.tuple substituted_members
    | TypeRepr.Arrow { label; lhs; rhs } ->
        ...
        if Std.Ptr.equal lhs substituted_lhs && Std.Ptr.equal rhs substituted_rhs then
          ty
        else TypeRepr.arrow ~label ~lhs:substituted_lhs ~rhs:substituted_rhs
  in
  loop
...
let next_type_param_id = fun type_params ->
  type_params |> List.fold_left
    (
      fun current_max (_, id) -> Int.max current_max id
    )
    (-1) |> Int.succ
...
let fresh_type_params_from_binders = fun type_params binders ->
  ...
  binders |> List.mapi
    (
      fun offset binder -> (Cst.TypeBinder.name binder, start + offset)
    )

let fresh_local_abstract_type_params_from_binders = fun (state: state) binders ->
  binders |> List.map
    (
      fun binder -> (Cst.TypeBinder.name binder, fresh_lowered_type_var_id state)
    )
...
let rec lower_core_type = fun (state: state) type_params core_type ->
  ...
  | Cst.CoreType.Parenthesized { inner; _ } | Cst.CoreType.Attribute { type_ = inner; _ } | Cst.CoreType.Alias { type_ = inner; _ } -> lower_core_type state type_params inner
  | Cst.CoreType.Poly { binders; body; _ } ->
      let params = fresh_type_params_from_binders type_params binders in lower_core_type state (params @ type_params) body
  ...
  | Cst.CoreType.Wildcard _ -> TypeRepr.make_var (fresh_wildcard_type_var_id state)
  | Cst.CoreType.Constr { constructor_path; arguments; _ } ->
      ...
      begin
        match match arguments, Cst.Ident.segments constructor_path with
  ...
  | Cst.CoreType.Arrow { label; parameter_type; result_type; _ } -> TypeRepr.arrow ~label:(lower_arrow_label label) ~lhs:(lower_core_type state type_params parameter_type) ~rhs:(lower_core_type state type_params result_type)
  | Cst.CoreType.Tuple { elements; _ } -> TypeRepr.tuple (List.map (lower_core_type state type_params) elements)
  | Cst.CoreType.FirstClassModule { package_type; _ } -> lower_package_type state type_params package_type
  | _ -> TypeRepr.hole unsupported_core_type_hole_id
and lower_package_type = fun (state: state) type_params (package_type: Cst.package_type) ->
  ...
  | Some template ->
      ...
      let () =
        package_type.constraints |> List.iter
          (
      ...
      in
      let values =
        template.values |> List.map
          (
            fun (value: TypeRepr.package_value) -> { value with scheme = TypeScheme.map_type_preserving (substitute_package_type replacements) value.scheme }
          )
      in
...
let variant_constructor_payload = fun (state: state) type_params (constructor: Cst.VariantConstructor.t) ->
  match Cst.VariantConstructor.result_type constructor with
  | Some _ -> (
    match Cst.VariantConstructor.arguments constructor with
    | Some (Cst.ConstructorArguments.Tuple [ Cst.CoreType.Record _ ]) -> Some (TypeRepr.hole unsupported_record_constructor_payload_hole_id)
    ...
    | Some (Cst.ConstructorArguments.Record _) -> Some (TypeRepr.hole unsupported_record_constructor_payload_hole_id)
    | None -> None
  )
  | None -> (
    match Cst.VariantConstructor.arguments constructor with
...
let add_origin = fun (state: state) ~semantic_id ~label syntax_node ->
  ...
  in
  let () = state.origins <- origin :: state.origins in origin_id
...
let add_pattern = fun (state: state) ~syntax_node ~label desc ->
  ...
  in
  let () = state.patterns <- node :: state.patterns in pat_id
...
let annotate_pattern = fun (state: state) pat_id annotation ->
  let rec loop acc = function
    ...
    | ((node: BodyArena.pattern_node) as current_node) :: rest ->
        ...
        else loop (current_node :: acc) rest
  in
...
let add_expr = fun (state: state) ~syntax_node ~label desc ->
  ...
  let () = state.expressions <- node :: state.expressions in expr_id
...
let add_binding = fun (state: state) ~syntax_node ~name ~pattern_id ~annotation ~value_id ~recursive ->
  ...
  in
  let () = state.bindings <- binding :: state.bindings in binding_id
...
let add_item = fun (state: state) ~syntax_node item ->
  ...
  in
  let () = state.items <- item :: state.items in item

let rec lower_record_field_scheme = fun (state: state) type_params core_type ->
  match core_type with
  | Cst.CoreType.Parenthesized { inner; _ } | Cst.CoreType.Attribute { type_ = inner; _ } -> lower_record_field_scheme state type_params inner
  | Cst.CoreType.Poly { binders; body; _ } ->
      ...
      let quantified = TypeRepr.union (List.map snd params) (TypeRepr.free_vars lowered) in TypeScheme.of_explicit ~quantified lowered
  | _ -> TypeScheme.of_type (lower_core_type state type_params core_type)
...
let lower_record_label = fun (state: state) type_params (field: Cst.RecordField.t) ->
  {
    ...
    let () = state.next_label_id <- state.next_label_id + 1 in label
...
let rec collect_core_type_var_names = fun core_type ->
  ...
  in
  match core_type with
  | Cst.CoreType.Parenthesized { inner; _ } | Cst.CoreType.Attribute { type_ = inner; _ } | Cst.CoreType.Alias { type_ = inner; _ } -> collect_core_type_var_names inner
  | Cst.CoreType.Var { name_token; _ } -> [ Cst.Token.text name_token ]
  | Cst.CoreType.Constr { arguments; _ } -> collect_many arguments
  | Cst.CoreType.Arrow { parameter_type; result_type; _ } -> collect_many [ parameter_type; result_type ]
  | Cst.CoreType.Tuple { elements; _ } -> collect_many elements
  | Cst.CoreType.Record { fields; _ } ->
      fields |> List.fold_left
        (
          fun acc (field: Cst.record_type_field) -> collect_core_type_var_names field.field_type |> List.fold_left append_unique acc
        )
        []
  | _ -> []
...
let append_constructor_type_var_name = fun acc name ->
  if List.exists
    ...
    acc @ [
      name, List.length acc;
    ]

let extend_constructor_type_params = fun params names -> List.fold_left append_constructor_type_var_name params names
...
let inline_record_labels_for_constructor = fun (state: state) constructor_params (constructor: Cst.VariantConstructor.t) ->
  match Cst.VariantConstructor.arguments constructor with
  | Some (Cst.ConstructorArguments.Record { fields; _ }) -> Some (List.map (lower_record_label state constructor_params) fields)
  | Some (Cst.ConstructorArguments.Tuple [ Cst.CoreType.Record { fields; _ } ]) -> Some (List.map (lower_record_type_label state constructor_params) fields)
  | _ -> (
    match Cst.VariantConstructor.payload_type constructor with
    | Some (Cst.CoreType.Record { fields; _ }) -> Some (List.map (lower_record_type_label state constructor_params) fields)
    | _ -> None
  )
...
let constructor_type_param_bindings = fun params (constructor: Cst.VariantConstructor.t) ->
  let params =
    ...
    | Some (Cst.ConstructorArguments.Record { fields; _ }) ->
        fields |> List.fold_left
          (
            fun acc field -> extend_constructor_type_params acc (collect_core_type_var_names (Cst.RecordField.field_type field))
          )
          params
    | None -> (
      match Cst.VariantConstructor.payload_type constructor with
      | Some (Cst.CoreType.Record { fields; _ }) -> collect_record_type_field_var_names fields |> List.fold_left append_constructor_type_var_name params
      | Some payload_type -> extend_constructor_type_params params (collect_core_type_var_names payload_type)
      | None -> params
    )
  in
  match Cst.VariantConstructor.result_type constructor with
  | Some result_type -> extend_constructor_type_params params (collect_core_type_var_names result_type)
  | None -> params
...
let lower_poly_variant_tag = fun (state: state) type_params (tag: Cst.PolyVariantTag.t) -> { TypeDecl.name = Cst.PolyVariantTag.name tag; payload_type = Cst.PolyVariantTag.payload_type tag |> Option.map (lower_core_type state type_params) }

let lower_poly_variant_manifest = fun (state: state) type_params (poly_variant: Cst.PolyVariant.t) ->
  let (tags, inherited) =
    Cst.PolyVariant.fields poly_variant |> List.fold_left
      (
        fun (tags, inherited) field ->
          match field with
          | Cst.RowField.Tag tag -> (lower_poly_variant_tag state type_params tag :: tags, inherited)
          | Cst.RowField.Inherit { type_; _ } -> (tags, lower_core_type state type_params type_ :: inherited)
      )
      ([], [])
  in
  TypeDecl.PolyVariant { bound = lower_poly_variant_bound (Cst.PolyVariant.kind poly_variant); tags = List.rev tags; inherited = List.rev inherited }
...
let add_unsupported_structure_item = fun (state: state) syntax_node -> add_unsupported_item state ~context:Typ_diagnostic.StructureItem syntax_node

let add_unsupported_signature_item = fun (state: state) syntax_node -> add_unsupported_item state ~context:Typ_diagnostic.SignatureItem syntax_node

let rec register_type_declaration_names = fun (state: state) (declaration: Cst.TypeDeclaration.t) ->
  let _ = register_declared_type_name state (Cst.Token.text (Cst.TypeDeclaration.name_token declaration)) in
...
let lowered_type_declaration = fun (state: state) (declaration: Cst.TypeDeclaration.t) ->
  ...
  let result_type = TypeRepr.named ~head:(TypeRepr.named_head ~type_constructor_id ~name:(qualify_scoped_name state.scope_path type_name)) ~arguments:(
    params |> List.map
      (
        fun (_, id) -> TypeRepr.make_var id
      )
  ) in
      ...
      (
        fun () ->
          let lowered_manifest_alias =
            Cst.TypeDeclaration.manifest_alias declaration |> Option.map
              (
                fun manifest -> TypeDecl.Alias (lower_core_type state params manifest)
              )
          in
          ...
          | Cst.TypeDefinition.Variant { constructors; _ } ->
              Some {
                ...
                param_ids = List.map snd params;
                param_variances = invariant_param_variances params;
                constructors = constructors |> List.map
                  (
                    fun (constructor: Cst.VariantConstructor.t) ->
                      ...
                      let inline_record_labels = inline_record_labels_for_constructor state constructor_params constructor in
                      {
                        ...
                        let () = state.next_constructor_id <- state.next_constructor_id + 1 in constructor
                  );
                labels = [];
                manifest = lowered_manifest_alias
              }
          ...
          | Cst.TypeDefinition.PolyVariant poly_variant ->
              Some {
                ...
                constructors = [];
                labels = [];
                manifest = Option.or_else lowered_manifest_alias
                  (
  ...
  | None ->
      let () = add_unsupported_structure_item state syntax_node in None
...
let lower_exception_declaration = fun (state: state) (declaration: Cst.exception_declaration) ->
  ...
  let exn_type = TypeRepr.named ~head:(TypeRepr.named_head ~type_constructor_id:BuiltinTypeConstructors.exn_type_constructor_id ~name:(SurfacePath.of_name "exn")) ~arguments:[] in
  let payload_type =
    match declaration.rhs with
    | Some (Cst.Payload { payload_type; _ }) -> Some (lower_core_type state [] payload_type)
    | Some (Cst.Alias _) | None -> None
  in
  let scheme =
    match payload_type with
    | Some payload_type -> TypeScheme.of_type (TypeRepr.arrow ~label:TypeRepr.Nolabel ~lhs:payload_type ~rhs:exn_type)
...
let lower_type_extension = fun (state: state) (extension: Cst.TypeExtension.t) ->
  ...
  let () =
    Cst.TypeExtension.constructors extension |> List.iter
      (
        fun (constructor: Cst.VariantConstructor.t) ->
          let constructor_params = constructor_type_param_bindings params constructor in
          let payload_type = variant_constructor_payload state constructor_params constructor in
          ...
          let inline_record_labels = inline_record_labels_for_constructor state constructor_params constructor in
          let () = state.next_constructor_id <- state.next_constructor_id + 1 in
          let _ = add_item state ~syntax_node (`ExtensionConstructor (constructor_id, Cst.VariantConstructor.name constructor, scheme, inline_record_labels)) in ()
      )
  in
  ()
...
let rec first_unsupported_core_type = fun core_type ->
  ...
  match core_type with
  | Cst.CoreType.Parenthesized { inner; _ } | Cst.CoreType.Attribute { type_ = inner; _ } | Cst.CoreType.Alias { type_ = inner; _ } -> first_unsupported_core_type inner
  ...
  | Cst.CoreType.Arrow { parameter_type; result_type; _ } ->
      Option.or_else (first_unsupported_core_type parameter_type)
        (
          fun () -> first_unsupported_core_type result_type
        )
  | Cst.CoreType.Tuple { elements; _ } -> find_many elements
  | Cst.CoreType.Wildcard _ -> None
  | Cst.CoreType.Class _ | Cst.CoreType.Extension _ | Cst.CoreType.Poly _ | Cst.CoreType.PolyVariant _ | Cst.CoreType.Record _ | Cst.CoreType.Object _ -> Some core_type
  | Cst.CoreType.FirstClassModule { package_type; _ } ->
      package_type.constraints |> List.find_map
        (
          fun (constraint_: Cst.module_type_constraint) ->
            Option.or_else (first_unsupported_core_type constraint_.constrained_type)
              (
...
let rec scheme_of_declared_core_type = fun (state: state) core_type ->
  match core_type with
  | Cst.CoreType.Parenthesized { inner; _ } | Cst.CoreType.Attribute { type_ = inner; _ } -> scheme_of_declared_core_type state inner
  | Cst.CoreType.Poly { binders; body; _ } ->
      let params = fresh_type_params_from_binders [] binders in
      let ty = lower_core_type state params body in
      let quantified = TypeRepr.union (List.map snd params) (TypeRepr.free_vars ty) in TypeScheme.of_explicit ~quantified ty
  | _ ->
      let params =
        collect_core_type_var_names core_type |> List.mapi
          (
            fun index name -> (name, index)
          )
      in
...
let lower_module_type_template =
  let register_abstract_type_name state type_name =
    let type_constructor_id = TypeConstructorId.make ~owner:state.source_owner ~local_id:state.next_type_constructor_id in
    let () = state.next_type_constructor_id <- state.next_type_constructor_id + 1 in
    let head = TypeRepr.named_head ~type_constructor_id ~name:(qualify_scoped_name state.scope_path type_name) in
    let binding = type_name, state.scope_path, type_constructor_id in
    let () = state.declared_type_names <- binding :: state.declared_type_names in (type_name, head)
  in
            ...
            items |> List.filter_map
              (
                ...
                | Cst.SignatureItem.ValueDeclaration declaration ->
                    let value_name = declared_value_name (Cst.ValueDeclaration.name_tokens declaration) in Some (TypeRepr.package_value ~name:value_name ~scheme:(scheme_of_declared_core_type state (Cst.ValueDeclaration.type_ declaration)))
                | Cst.SignatureItem.ExternalDeclaration declaration ->
                    let value_name = declared_value_name declaration.name_tokens in Some (TypeRepr.package_value ~name:value_name ~scheme:(scheme_of_declared_core_type state declaration.type_))
                | _ -> None
              )
          in
          let () = state.declared_type_names <- saved_declared_type_names in Some { abstract_types; values }
      | Error builder_error ->
          let () = add_diagnostic state (Typ_diagnostic.CstBuilderError { builder_error }) in None
    )
    | Cst.ModuleType.Path path -> resolve_module_type_template state (ident_path path)
    | Cst.ModuleType.With { base; constraints; _ } ->
        loop state base |> Option.map
          (
            fun template -> template_with_constraints state template constraints
          )
    | Cst.ModuleType.Parenthesized { inner; _ } | Cst.ModuleType.Attribute { module_type = inner; _ } -> loop state inner
    | _ -> None
  in
  loop
...
let fresh_synthetic_name = fun (state: state) prefix ->
  ...
  let () = state.next_synthetic_name <- state.next_synthetic_name + 1 in name

let with_scope = fun (state: state) scope_path f ->
  let previous_scope_path = state.scope_path in
  let () = state.scope_path <- scope_path in
  let result = f () in
  let () = state.scope_path <- previous_scope_path in result

let with_local_module_alias = fun (state: state) ~module_name ~module_path f ->
  let previous_aliases = state.local_module_aliases in
  let () = state.local_module_aliases <- (module_name, module_path) :: state.local_module_aliases in
  try
    let result = f () in
    let () = state.local_module_aliases <- previous_aliases in result
  with
  | error ->
      let () = state.local_module_aliases <- previous_aliases in raise error
...
let with_local_abstract_type_params = fun (state: state) type_params f ->
  ...
  let () = state.local_abstract_type_params <- type_params @ state.local_abstract_type_params in
  try
    let result = f () in
    let () = state.local_abstract_type_params <- previous_type_params in result
  with
  | error ->
      let () = state.local_abstract_type_params <- previous_type_params in raise error
...
let with_local_module_binding_groups = fun (state: state) ~module_name ~local_scope f ->
  ...
  let () = state.local_module_binding_groups <- (module_name, local_scope) :: state.local_module_binding_groups in
  try
    let result = f () in
    let () = state.local_module_binding_groups <- previous_local_scopes in result
  with
  | error ->
      let () = state.local_module_binding_groups <- previous_local_scopes in raise error
...
let operator_name = fun operator_tokens -> operator_tokens |> List.map Cst.Token.text |> String.concat ""
...
let lower_unsupported_pattern = fun (state: state) ?reason pattern syntax_kind ->
  ...
  in
  add_pattern state ~syntax_node ~label:"unsupported_pattern" (BodyArena.PUnsupported (SyntaxKind.to_string syntax_kind))
...
let lower_unsupported_expr = fun (state: state) ?reason expr syntax_kind ->
  ...
  in
  add_expr state ~syntax_node ~label:"unsupported_expression" (BodyArena.EHole (SyntaxKind.to_string syntax_kind))

let rec lower_pattern = fun (state: state) pattern ->
  match pattern with
  | Cst.Pattern.Identifier { syntax_node; name_token; _ } -> add_pattern state ~syntax_node ~label:"identifier_pattern" (BodyArena.PVar (Cst.Token.text name_token))
  | Cst.Pattern.Operator { syntax_node; operator_tokens; _ } -> add_pattern state ~syntax_node ~label:"operator_pattern" (BodyArena.PVar (operator_name operator_tokens))
  | Cst.Pattern.Wildcard { syntax_node; _ } -> add_pattern state ~syntax_node ~label:"wildcard_pattern" BodyArena.PWildcard
  | Cst.Pattern.Literal { syntax_node; literal; _ } -> (
    match literal with
    | Cst.PatternLiteral.Int integer -> add_pattern state ~syntax_node ~label:"int_pattern" (BodyArena.PInt (int_text integer))
    | Cst.PatternLiteral.Float float_ -> add_pattern state ~syntax_node ~label:"float_pattern" (BodyArena.PFloat (float_text float_))
    | Cst.PatternLiteral.Bool { value; _ } -> add_pattern state ~syntax_node ~label:"bool_pattern" (BodyArena.PBool value)
    | Cst.PatternLiteral.String string_ -> add_pattern state ~syntax_node ~label:"string_pattern" (BodyArena.PString string_.contents)
    | Cst.PatternLiteral.Char char_ -> add_pattern state ~syntax_node ~label:"char_pattern" (BodyArena.PChar char_.contents)
  ...
  | Cst.Pattern.Tuple { syntax_node; elements; _ } ->
      let element_ids =
        elements |> List.map
          (
            fun (element: Cst.tuple_pattern_element) -> lower_pattern state element.pattern
          )
      in
      add_pattern state ~syntax_node ~label:"tuple_pattern" (BodyArena.PTuple element_ids)
  | Cst.Pattern.Or { syntax_node; alternatives; _ } ->
      let alternative_ids = List.map (lower_pattern state) alternatives in add_pattern state ~syntax_node ~label:"or_pattern" (BodyArena.POr alternative_ids)
  | Cst.Pattern.Constructor { syntax_node; constructor_path; arguments; existentials; _ } ->
      let lowered_existentials =
        existentials |> Option.map
          (
            fun ({ Cst.binders; _ }: Cst.constructor_pattern_existentials) -> fresh_local_abstract_type_params_from_binders state binders
          ) |> Option.unwrap_or ~default:[]
      in
      with_local_abstract_type_params state lowered_existentials
        (
          fun () ->
            let argument_ids = List.map (lower_pattern state) arguments in add_pattern state ~syntax_node ~label:"constructor_pattern" (BodyArena.PConstructor { constructor = ident_path constructor_path; arguments = argument_ids })
        )
          ...
          (
            fun (field: Cst.record_pattern_field) ->
              let pattern_id =
                ...
                | None ->
                    let field_name = last_path_segment_text field.field_path in add_pattern state ~syntax_node:field.syntax_node ~label:"record_punned_field_pattern" (BodyArena.PVar field_name)
              in
  ...
  | Cst.Pattern.List { syntax_node; elements; _ } ->
      let element_ids = List.map (lower_pattern state) elements in add_pattern state ~syntax_node ~label:"list_pattern" (BodyArena.PList element_ids)
  | Cst.Pattern.Cons { syntax_node; head; tail; _ } ->
      let head_id = lower_pattern state head in
      let tail_id = lower_pattern state tail in add_pattern state ~syntax_node ~label:"cons_pattern" (BodyArena.PConstructor { constructor = SurfacePath.of_name "::"; arguments = [ head_id; tail_id ] })
  | Cst.Pattern.Alias { syntax_node; pattern; name_token; _ } ->
      let pattern_id = lower_pattern state pattern in add_pattern state ~syntax_node ~label:"alias_pattern" (BodyArena.PAlias { pattern_id; alias = Cst.Token.text name_token })
  | Cst.Pattern.PolyVariant { syntax_node; tag_token; payload; _ } ->
      let payload = payload |> Option.map (lower_pattern state) in add_pattern state ~syntax_node ~label:"poly_variant_pattern" (BodyArena.PPolyVariant { tag = Cst.Token.text tag_token; payload })
  | Cst.Pattern.FirstClassModule { syntax_node; binding; package_type; _ } ->
      ...
      in
      let package_type = package_type |> Option.map (lower_package_type state []) in add_pattern state ~syntax_node ~label:"first_class_module_pattern" (BodyArena.PFirstClassModule { module_name; package_type })
  | Cst.Pattern.Parenthesized { inner; _ } -> lower_pattern state inner
  | Cst.Pattern.Typed { pattern; type_; _ } ->
      ...
      let () = annotate_pattern state pattern_id annotation in pattern_id
  | _ -> lower_unsupported_pattern state pattern (unsupported_syntax_kind (Cst.Pattern.syntax_node pattern))
...
let positional_function_parameter = fun pattern_id -> ({ BodyArena.label = BodyArena.Positional; pattern_id; default_value_id = None } : BodyArena.function_parameter)

let labeled_function_parameter = fun label pattern_id -> ({ BodyArena.label = BodyArena.Labeled label; pattern_id; default_value_id = None } : BodyArena.function_parameter)

let optional_function_parameter = fun label ~default_value_id pattern_id -> ({ BodyArena.label = BodyArena.Optional label; pattern_id; default_value_id } : BodyArena.function_parameter)

let rec lower_parameter = fun (state: state) parameter ->
  match parameter with
  | Cst.Parameter.Positional { pattern; _ } -> positional_function_parameter (lower_pattern state pattern)
  | Cst.Parameter.Labeled labeled ->
      let pattern_id = recovered_parameter_pattern state labeled.syntax_node ~label:"labeled_parameter_pattern" parameter in labeled_function_parameter (Cst.Token.text labeled.label_token) pattern_id
  | Cst.Parameter.Optional optional ->
      let pattern_id = recovered_parameter_pattern state optional.syntax_node ~label:"optional_parameter_pattern" parameter in
      let default_value_id = optional.default_value |> Option.map (lower_expr state) in optional_function_parameter (Cst.Token.text optional.label_token) ~default_value_id pattern_id
  | parameter ->
      let syntax_node = Cst.Parameter.syntax_node parameter in
      let () = add_diagnostic state (Typ_diagnostic.ParameterLoweredAsPositional { parameter_span = Ceibo.Red.SyntaxNode.span syntax_node }) in positional_function_parameter (recovered_parameter_pattern state syntax_node ~label:"recovered_parameter" parameter)
and synthetic_var_pattern = fun (state: state) syntax_node ~label ->
  let name = fresh_synthetic_name state label in
  let pat_id = add_pattern state ~syntax_node ~label:("synthetic_" ^ label ^ "_pattern") (BodyArena.PVar name) in (name, pat_id)
  ...
  List.map
    (
      fun (case: Cst.match_case) ->
        ...
        let body_id = lower_expr state case.body in { BodyArena.pattern_id; guard_id; body_id }
    )
    cases
and lower_function_like = fun (state: state) ~syntax_node ~parameters ~body_annotation_type ~body ->
  let rec split_parameters local_type_params runtime_parameters = function
    | (Cst.Parameter.LocallyAbstract parameter) :: rest ->
        let new_params = fresh_local_abstract_type_params_from_binders state parameter.binders in split_parameters (new_params @ local_type_params) runtime_parameters rest
    | parameter :: rest -> split_parameters local_type_params (parameter :: runtime_parameters) rest
    ...
    (
      fun () ->
        ...
        let wrap_body_annotation body_id =
          ...
          | Some target_type -> add_expr state ~syntax_node ~label:"annotated_function_body" (BodyArena.ECoerce { value_id = body_id; target_type })
          | None -> body_id
        in
        let body_id =
          match body with
          ...
          | `Cases cases ->
              ...
              let argument_expr_id = add_expr state ~syntax_node ~label:"synthetic_function_argument" (BodyArena.EVar (SurfacePath.of_name synthetic_name)) in
              let match_id = add_expr state ~syntax_node ~label:"function_match_body" (BodyArena.EMatch (argument_expr_id, lower_match_cases state cases)) in
              let match_id = wrap_body_annotation match_id in
              let parameter_ids = parameter_ids @ [ positional_function_parameter synthetic_pattern_id ] in add_expr state ~syntax_node ~label:"wrapped_fun" (BodyArena.EFun (parameter_ids, match_id))
        in
        match body with
        | `Expr _ -> add_expr state ~syntax_node ~label:"fun_expression" (BodyArena.EFun (parameter_ids, body_id))
        | `Cases _ -> body_id
    )
and lower_binding_source = fun (state: state) ~syntax_node ~binding_pattern ~parameters ~value ~recursive ->
  let rec explicit_scheme_of_core_type core_type =
    match core_type with
    | Cst.CoreType.Parenthesized { inner; _ } | Cst.CoreType.Attribute { type_ = inner; _ } -> explicit_scheme_of_core_type inner
    | Cst.CoreType.Poly { binders; body; _ } ->
        let params = fresh_type_params_from_binders [] binders in
  ...
  let value_id =
    ...
    | _ -> lower_function_like state ~syntax_node ~parameters ~body_annotation_type:body_annotation ~body:(`Expr value)
  in
  let name = binding_name_of_pattern binding_pattern in add_binding state ~syntax_node ~name ~pattern_id ~annotation ~value_id ~recursive
and binding_ids_of_let_binding_group = fun (state: state) let_binding ->
  let recursive = Cst.LetBinding.is_recursive let_binding in
  let binding_ids =
    let_binding :: Cst.LetBinding.and_bindings let_binding |> List.map
      (
        fun (binding: Cst.let_binding) -> lower_binding_source state ~syntax_node:(Cst.LetBinding.syntax_node binding) ~binding_pattern:(Cst.LetBinding.binding_pattern binding) ~parameters:(Cst.LetBinding.parameters binding) ~value:(Cst.LetBinding.value binding) ~recursive
      )
  in
  binding_ids
and lower_let_binding_group = fun (state: state) let_binding ->
  let recursive = Cst.LetBinding.is_recursive let_binding in
  let binding_ids = binding_ids_of_let_binding_group state let_binding in add_item state ~syntax_node:(Cst.LetBinding.syntax_node let_binding) (`Value (binding_ids, recursive))
and lower_let_expression_bindings = fun (state: state) (let_expression: Cst.let_expression) ->
  ...
  let head = lower_binding_source state ~syntax_node:let_expression.syntax_node ~binding_pattern:let_expression.binding_pattern ~parameters:let_expression.parameters ~value:let_expression.bound_value ~recursive in
  let tail =
    match let_expression.and_binding with
    | None -> []
    | Some binding ->
        Cst.LetBinding.and_bindings binding |> fun rest ->
          binding :: rest |> List.map
            (
              fun (binding: Cst.let_binding) -> lower_binding_source state ~syntax_node:(Cst.LetBinding.syntax_node binding) ~binding_pattern:(Cst.LetBinding.binding_pattern binding) ~parameters:(Cst.LetBinding.parameters binding) ~value:(Cst.LetBinding.value binding) ~recursive
            )
  in
...
and lower_local_module_scope = fun (state: state) ~module_name module_expression ->
  ...
  in
  let rec loop = function
    | Cst.ModuleExpression.Parenthesized { inner; _ } | Cst.ModuleExpression.Attribute { module_expression = inner; _ } | Cst.ModuleExpression.Constraint { module_expression = inner; _ } -> loop inner
                    ...
                    items |> List.fold_left
                      (
                        fun (binding_groups, type_decls) item ->
                          match item with
                          | Cst.StructureItem.Comment _ | Cst.StructureItem.Docstring _ -> (binding_groups, type_decls)
                          | Cst.StructureItem.LetBinding binding -> (binding_groups @ [ { BodyArena.binding_ids = binding_ids_of_let_binding_group state binding } ], type_decls)
                          | Cst.StructureItem.TypeDeclaration declaration ->
                              ...
                              let rec collect declaration acc =
                                let acc =
                                  match lowered_type_declaration state declaration with
                                  | Some lowered_declaration -> acc @ [ { FileSummary.scope_path = SurfacePath.empty; declaration = lowered_declaration } ]
                                  | None -> acc
                                in
                                match Cst.TypeDeclaration.next_and_declaration declaration with
                          ...
                          | unsupported_item ->
                              let () = add_unsupported_structure_item state (Cst.StructureItem.syntax_node unsupported_item) in (binding_groups, type_decls)
                      )
                      ([], [])
                  in
...
and lower_apply = fun (state: state) expression ->
  let lower_argument = function
    | Cst.Positional argument -> ({ BodyArena.label = BodyArena.Positional; implicit = false; value_id = lower_expr state argument } : BodyArena.apply_argument)
    | Cst.Labeled { syntax_node; label_token; value; _ } ->
        let value_id =
          match value with
          | Some value -> lower_expr state value
          | None -> add_expr state ~syntax_node ~label:"implicit_labeled_argument" (BodyArena.EVar (SurfacePath.of_name (Cst.Token.text label_token)))
        in
        { BodyArena.label = BodyArena.Labeled (Cst.Token.text label_token); implicit = Option.is_none value; value_id }
    | Cst.Optional { syntax_node; label_token; value; _ } ->
        let value_id =
          match value with
          | Some value -> lower_expr state value
          | None -> add_expr state ~syntax_node ~label:"implicit_optional_argument" (BodyArena.EVar (SurfacePath.of_name (Cst.Token.text label_token)))
        in
        { BodyArena.label = BodyArena.Optional (Cst.Token.text label_token); implicit = Option.is_none value; value_id }
  in
  let rec collect = function
    | Cst.Expression.Apply { callee; argument; _ } ->
        let (callee_id, arguments) = collect callee in (callee_id, arguments @ [ lower_argument argument ])
    | callee ->
        let callee_id = lower_expr state callee in (callee_id, [])
  in
  let syntax_node = Cst.Expression.syntax_node expression in
  let (callee_id, arguments) = collect expression in add_expr state ~syntax_node ~label:"apply_expression" (BodyArena.EApply (callee_id, arguments))
and lower_infix = fun (state: state) (infix: Cst.infix_expression) ->
  ...
  let operator_id = add_expr state ~syntax_node ~label:"infix_operator" (BodyArena.EVar (SurfacePath.of_name operator_name)) in
  let left_id = lower_expr state infix.left in
  let right_id = lower_expr state infix.right in add_expr state ~syntax_node ~label:"infix_expression" (BodyArena.EApply (operator_id, [ { BodyArena.label = BodyArena.Positional; implicit = false; value_id = left_id }; { BodyArena.label = BodyArena.Positional; implicit = false; value_id = right_id } ]))
and lower_list_expression = fun (state: state) (list_expression: Cst.list_expression) ->
  let nil_id = add_expr state ~syntax_node:list_expression.syntax_node ~label:"list_nil_expression" (BodyArena.EVar (SurfacePath.of_name "[]")) in
  list_expression.elements |> List.rev |> List.fold_left
    (
      fun tail_id element ->
        let cons_id = add_expr state ~syntax_node:list_expression.syntax_node ~label:"list_cons_expression" (BodyArena.EVar (SurfacePath.of_name "::")) in
        let head_id = lower_expr state element in add_expr state ~syntax_node:list_expression.syntax_node ~label:"list_literal_apply" (BodyArena.EApply (cons_id, [ { BodyArena.label = BodyArena.Positional; implicit = false; value_id = head_id }; { BodyArena.label = BodyArena.Positional; implicit = false; value_id = tail_id } ]))
    )
    nil_id
and lower_binding_operator_name = fun (binding: Cst.binding_operator_binding) -> Cst.Token.text binding.keyword_token ^ Cst.Token.text binding.operator_token
and lower_let_operator_expression = fun (state: state) (let_operator: Cst.let_operator_expression) ->
  ...
  | Some _ ->
      ...
      in
      add_expr state ~syntax_node ~label:"unsupported_let_operator_expression" (BodyArena.EHole (SyntaxKind.to_string (unsupported_syntax_kind syntax_node)))
  | None ->
      let binding = let_operator.binding in
      let syntax_node = let_operator.syntax_node in
      let operator_id = add_expr state ~syntax_node ~label:"binding_operator_expression" (BodyArena.EVar (SurfacePath.of_name (lower_binding_operator_name binding))) in
      ...
      let body_fun_id = add_expr state ~syntax_node ~label:"binding_operator_body" (BodyArena.EFun ([ positional_function_parameter parameter_id ], body_id)) in add_expr state ~syntax_node ~label:"let_operator_expression" (BodyArena.EApply (operator_id, [ { BodyArena.label = BodyArena.Positional; implicit = false; value_id = bound_value_id }; { BodyArena.label = BodyArena.Positional; implicit = false; value_id = body_fun_id } ]))
and lower_expr = fun (state: state) expression ->
  match expression with
  | Cst.Expression.Path { syntax_node; path; _ } -> add_expr state ~syntax_node ~label:"path_expression" (BodyArena.EVar (ident_path path))
  | Cst.Expression.Constructor { syntax_node; constructor_path; payload; _ } -> (
    let constructor_name = ident_path constructor_path in
    match payload with
    | None -> add_expr state ~syntax_node ~label:"constructor_expression" (BodyArena.EVar constructor_name)
    | Some payload ->
        let callee_id = add_expr state ~syntax_node:(Cst.Ident.syntax_node constructor_path) ~label:"constructor_path_expression" (BodyArena.EVar constructor_name) in
        let payload_id = lower_expr state payload in add_expr state ~syntax_node ~label:"constructor_apply_expression" (BodyArena.EApply (callee_id, [ { BodyArena.label = BodyArena.Positional; implicit = false; value_id = payload_id } ]))
  )
  | Cst.Expression.FieldAccess { syntax_node; receiver; field_name; _ } -> (
    match module_path_segments_of_expr state receiver with
    | Some module_path ->
        let qualified_name = SurfacePath.append_name module_path (Cst.Token.text field_name) in add_expr state ~syntax_node ~label:"qualified_path_expression" (BodyArena.EVar qualified_name)
    | None ->
        let receiver_id = lower_expr state receiver in add_expr state ~syntax_node ~label:"field_access_expression" (BodyArena.EFieldAccess { receiver_id; label = Cst.Token.text field_name })
  )
  | Cst.Expression.Record (Cst.RecordExpression.Literal { syntax_node; fields; _ }) ->
      let fields =
        fields |> List.map
          (
            fun (field: Cst.record_expression_field) -> ({ BodyArena.label = path_text field.field_path; value_id = lower_expr state field.value } : BodyArena.record_expr_field)
          )
      in
      add_expr state ~syntax_node ~label:"record_expression" (BodyArena.ERecord { base_id = None; fields })
  | Cst.Expression.Record (Cst.RecordExpression.Update { syntax_node; base; fields; _ }) ->
      ...
      in
      add_expr state ~syntax_node ~label:"record_update_expression" (BodyArena.ERecord { base_id = Some base_id; fields })
  | Cst.Expression.FieldAssign { syntax_node; target; value; _ } ->
      let receiver_id = lower_expr state target.receiver in
      let value_id = lower_expr state value in add_expr state ~syntax_node ~label:"field_assign_expression" (BodyArena.EFieldAssign { receiver_id; label = Cst.Token.text target.field_name; value_id })
  | Cst.Expression.Operator { syntax_node; operator_tokens; _ } ->
      let operator = operator_tokens |> List.map Cst.Token.text |> String.concat "" in add_expr state ~syntax_node ~label:"operator_expression" (BodyArena.EVar (SurfacePath.of_name operator))
  | Cst.Expression.Literal literal -> (
    match literal with
    | Cst.Literal.Int integer -> add_expr state ~syntax_node:integer.syntax_node ~label:"int_literal" (BodyArena.EInt (int_text integer))
    | Cst.Literal.Float float_ -> add_expr state ~syntax_node:float_.syntax_node ~label:"float_literal" (BodyArena.EFloat (float_text float_))
    | Cst.Literal.Bool { syntax_node; value; _ } -> add_expr state ~syntax_node ~label:"bool_literal" (BodyArena.EBool value)
    | Cst.Literal.String string_ -> add_expr state ~syntax_node:string_.syntax_node ~label:"string_literal" (BodyArena.EString string_.contents)
    | Cst.Literal.Char char_ -> add_expr state ~syntax_node:char_.syntax_node ~label:"char_literal" (BodyArena.EChar char_.contents)
    | Cst.Literal.Unit { syntax_node; _ } -> add_expr state ~syntax_node ~label:"unit_literal" BodyArena.EUnit
  )
  | Cst.Expression.Tuple { syntax_node; elements; _ } ->
      let element_ids = List.map (lower_expr state) elements in add_expr state ~syntax_node ~label:"tuple_expression" (BodyArena.ETuple element_ids)
  | Cst.Expression.List list_expression -> lower_list_expression state list_expression
  | Cst.Expression.Array { syntax_node; elements; _ } ->
      let element_ids = List.map (lower_expr state) elements in add_expr state ~syntax_node ~label:"array_expression" (BodyArena.EArray element_ids)
  | Cst.Expression.Sequence { syntax_node; expressions; _ } ->
      let element_ids = List.map (lower_expr state) expressions in add_expr state ~syntax_node ~label:"sequence_expression" (BodyArena.ESequence element_ids)
  | Cst.Expression.ModulePack { syntax_node; module_expression; package_type; _ } -> (
    ...
    | Some module_path -> (
      ...
      | Some local_scope -> add_expr state ~syntax_node ~label:"local_module_pack_expression" (BodyArena.ELocalModulePack { local_scope; package_type = package_type |> Option.map (lower_package_type state []) })
      | None -> add_expr state ~syntax_node ~label:"module_pack_expression" (BodyArena.EModulePack { module_path; package_type = package_type |> Option.map (lower_package_type state []) })
    )
    | None ->
        let () =
          add_diagnostic state
        ...
        in
        add_expr state ~syntax_node ~label:"unsupported_module_pack_expression" (BodyArena.EHole (SyntaxKind.to_string (unsupported_syntax_kind syntax_node)))
  )
  | Cst.Expression.While while_expression ->
      let condition_id = lower_expr state while_expression.condition in
      let body_id = lower_expr state while_expression.body in add_expr state ~syntax_node:while_expression.syntax_node ~label:"while_expression" (BodyArena.EWhile { condition_id; body_id })
  | Cst.Expression.For for_expression ->
      let iterator_pattern_id = add_pattern state ~syntax_node:for_expression.syntax_node ~label:"for_iterator_pattern" (BodyArena.PVar (Cst.Token.text for_expression.iterator_token)) in
  ...
  | Cst.Expression.Parenthesized { inner; _ } -> lower_expr state inner
  | Cst.Expression.TypeAscription { syntax_node; expression; kind; _ } -> (
    match kind with
    | Cst.Coerce { type_; _ } ->
        let value_id = lower_expr state expression in add_expr state ~syntax_node ~label:"coercion_expression" (BodyArena.ECoerce { value_id; target_type = lower_core_type state [] type_ })
    | Cst.Type _ | Cst.ConstraintCoerce _ -> lower_expr state expression
  )
  | Cst.Expression.Polymorphic { syntax_node; expression; _ } ->
      let () = add_diagnostic state (Typ_diagnostic.IgnoredPolymorphicAnnotation { annotation_span = Ceibo.Red.SyntaxNode.span syntax_node }) in lower_expr state expression
  | Cst.Expression.Fun { syntax_node; parameters; body; _ } -> (
    match body with
    | Cst.Expression body -> lower_function_like state ~syntax_node ~parameters ~body_annotation_type:None ~body:(`Expr body)
    | Cst.Cases body -> lower_function_like state ~syntax_node ~parameters ~body_annotation_type:None ~body:(`Cases body.cases)
  )
  | Cst.Expression.Function { syntax_node; cases; _ } -> lower_function_like state ~syntax_node ~parameters:[] ~body_annotation_type:None ~body:(`Cases cases)
  | Cst.Expression.Apply _ -> lower_apply state expression
  | Cst.Expression.Index { syntax_node; collection; index; _ } ->
      let collection_id = lower_expr state collection in
      let index_id = lower_expr state index in add_expr state ~syntax_node ~label:"index_expression" (BodyArena.EIndex (collection_id, index_id))
  | Cst.Expression.Infix infix -> lower_infix state infix
  ...
  | Cst.Expression.Let let_expression ->
      ...
      let body_id = lower_expr state let_expression.body in add_expr state ~syntax_node:let_expression.syntax_node ~label:"let_expression" (BodyArena.ELet (binding_ids, body_id))
  | Cst.Expression.LetModule { syntax_node; module_name_token; module_expression; body; _ } ->
      ...
      begin
        ...
        | Some (expression, package_type) ->
            let pattern_id = add_pattern state ~syntax_node ~label:"let_module_unpack_pattern" (BodyArena.PFirstClassModule { module_name = Some source_module_name; package_type = package_type |> Option.map (lower_package_type state []) }) in
            let binding_id = add_binding state ~syntax_node ~name:None ~pattern_id ~annotation:None ~value_id:(lower_expr state expression) ~recursive:false in
            let body_id = lower_expr state body in add_expr state ~syntax_node ~label:"let_module_unpack_expression" (BodyArena.ELet ([ binding_id ], body_id))
        | None -> (
          match module_path_segments_of_module_expression state module_expression with
          | Some module_path ->
              with_local_module_alias state ~module_name:source_module_name ~module_path
                (
          ...
          | None -> (
            ...
            | Some local_scope ->
                let body_id =
                  with_local_module_binding_groups state ~module_name:source_module_name ~local_scope
                    (
                      fun () -> lower_expr state body
                    )
                in
                add_expr state ~syntax_node ~label:"local_module_expression" (BodyArena.ELocalModule { module_name = source_module_name; local_scope; body_id })
            | None ->
                let () =
                  add_diagnostic state
                    (
                ...
                in
                add_expr state ~syntax_node ~label:"unsupported_let_module_expression" (BodyArena.EHole (SyntaxKind.to_string (unsupported_syntax_kind syntax_node)))
          )
        )
      end
  | Cst.Expression.LetOperator let_operator -> lower_let_operator_expression state let_operator
  | Cst.Expression.Match { syntax_node; scrutinee; cases; _ } ->
      let scrutinee_id = lower_expr state scrutinee in
      let cases = lower_match_cases state cases in add_expr state ~syntax_node ~label:"match_expression" (BodyArena.EMatch (scrutinee_id, cases))
  | Cst.Expression.Try { syntax_node; body; cases; _ } ->
      let body_id = lower_expr state body in
      let cases = lower_match_cases state cases in add_expr state ~syntax_node ~label:"try_expression" (BodyArena.ETry (body_id, cases))
  | Cst.Expression.PolyVariant { syntax_node; tag_token; payload; _ } ->
      let payload = payload |> Option.map (lower_expr state) in add_expr state ~syntax_node ~label:"poly_variant_expression" (BodyArena.EPolyVariant { tag = Cst.Token.text tag_token; payload })
  | Cst.Expression.LocalOpen (LetOpen { syntax_node; module_path; body; _ }) | Cst.Expression.LocalOpen (Delimited { syntax_node; module_path; body; _ }) ->
      let body_id = lower_expr state body in add_expr state ~syntax_node ~label:"local_open_expression" (BodyArena.ELocalOpen { module_path = ident_path module_path; body_id })
  | Cst.Expression.Prefix { syntax_node; operator_token; operand; _ } -> (
    match Cst.Token.text operator_token, operand with
    | ("-", Cst.Expression.Literal (Cst.Literal.Int integer)) -> add_expr state ~syntax_node ~label:"negative_int_literal" (BodyArena.EInt ("-" ^ int_text integer))
    | ("-", Cst.Expression.Literal (Cst.Literal.Float float_)) -> add_expr state ~syntax_node ~label:"negative_float_literal" (BodyArena.EFloat ("-" ^ float_text float_))
    | _ ->
        let operator_id = add_expr state ~syntax_node ~label:"prefix_operator" (BodyArena.EVar (SurfacePath.of_name (prefix_operator_name (Cst.Token.text operator_token)))) in
        let operand_id = lower_expr state operand in add_expr state ~syntax_node ~label:"prefix_expression" (BodyArena.EApply (operator_id, [ { BodyArena.label = BodyArena.Positional; implicit = false; value_id = operand_id } ]))
  )
  | _ -> lower_unsupported_expr state expression (unsupported_syntax_kind (Cst.Expression.syntax_node expression))
...
let lower_top_level_expression = fun (state: state) expression ->
  ...
  let binding_id = add_binding state ~syntax_node ~name:None ~pattern_id ~annotation:None ~value_id ~recursive:false in add_item state ~syntax_node (`Value ([ binding_id ], false))

let rec lower_structure_item = fun (state: state) item ->
  match item with
  | Cst.StructureItem.Comment _ | Cst.StructureItem.Docstring _ -> ()
  ...
  | Cst.StructureItem.OpenStatement open_statement -> (
    ...
    | Some module_path ->
        let _ = add_item state ~syntax_node:(Cst.OpenStatement.syntax_node open_statement) (`Open (ident_path module_path)) in ()
    | None -> ()
  )
  ...
  | Cst.StructureItem.TypeDeclaration declaration ->
      ...
      in
      loop declaration
  | Cst.StructureItem.TypeExtension extension -> lower_type_extension state extension
  | Cst.StructureItem.ExternalDeclaration declaration -> lower_value_declaration state declaration.syntax_node declaration.name_tokens declaration.type_
  | Cst.StructureItem.ExceptionDeclaration declaration -> lower_exception_declaration state declaration
  | Cst.StructureItem.ModuleDeclaration declaration -> lower_module_declaration state declaration
  ...
  | item -> add_unsupported_structure_item state (Cst.StructureItem.syntax_node item)
and lower_signature_items_of_module_type = fun (state: state) module_type ~on_unsupported ->
  ...
  | Cst.ModuleType.Signature _ -> (
    ...
    | Error builder_error -> add_diagnostic state (Typ_diagnostic.CstBuilderError { builder_error })
  )
  | Cst.ModuleType.Parenthesized { inner; _ } | Cst.ModuleType.Attribute { module_type = inner; _ } -> lower_signature_items_of_module_type state inner ~on_unsupported
  | Cst.ModuleType.Path _ | Cst.ModuleType.TypeOf _ | Cst.ModuleType.Functor _ | Cst.ModuleType.With _ | Cst.ModuleType.Extension _ -> on_unsupported ()
and lower_signature_item = fun (state: state) item ->
  match item with
  | Cst.SignatureItem.Comment _ | Cst.SignatureItem.Docstring _ -> ()
  | Cst.SignatureItem.ValueDeclaration declaration -> lower_value_declaration state (Cst.ValueDeclaration.syntax_node declaration) (Cst.ValueDeclaration.name_tokens declaration) (Cst.ValueDeclaration.type_ declaration)
  | Cst.SignatureItem.ExternalDeclaration declaration -> lower_value_declaration state declaration.syntax_node declaration.name_tokens declaration.type_
  | Cst.SignatureItem.TypeDeclaration declaration ->
      let () = register_type_declaration_names state declaration in
      let rec loop declaration =
        let () = lower_type_declaration state declaration in
      ...
      in
      loop declaration
  | Cst.SignatureItem.TypeExtension extension -> lower_type_extension state extension
  ...
  | Cst.SignatureItem.IncludeStatement include_statement -> lower_signature_include_statement state include_statement
  | Cst.SignatureItem.ModuleDeclaration declaration -> lower_module_signature_declaration state declaration
  ...
  | item -> add_unsupported_signature_item state (Cst.SignatureItem.syntax_node item)
...
and lower_signature_include_statement = fun (state: state) (include_statement: Cst.include_statement) ->
  ...
  | Cst.ModuleType module_type -> (
    ...
    | Some module_path ->
        let _ = add_item state ~syntax_node (`Include module_path) in ()
    | None -> lower_signature_items_of_module_type state module_type ~on_unsupported:(
      fun () -> add_unsupported_signature_item state syntax_node
    )
  )
...
and lower_module_binding = fun (state: state) ~syntax_node ~module_name ~module_type ~module_expression ->
  ...
  let lower_functor_application ~nested_scope_path template argument_path =
    with_local_module_alias state ~module_name:template.parameter_name ~module_path:argument_path
      (
        fun () -> lower_module_items_under_scope ~nested_scope_path ~result_module_type:template.result_module_type template.body
      )
  in
  match module_expression with
  | Cst.ModuleExpression.Apply { callee; argument; _ } -> (
    match module_path_segments_of_module_expression state callee, module_path_segments_of_module_expression state argument with
    | Some callee_path, Some argument_path -> (
      match resolve_local_module_functor state callee_path with
      | Some template ->
          let nested_scope_path = SurfacePath.append_name state.scope_path module_name in lower_functor_application ~nested_scope_path template argument_path
      | None -> add_unsupported_structure_item state syntax_node
    )
  ...
  | _ -> (
    ...
    | None ->
        let nested_scope_path = SurfacePath.append_name state.scope_path module_name in lower_module_items_under_scope ~nested_scope_path ~result_module_type:module_type module_expression
  )
and lower_module_declaration = fun (state: state) (declaration: Cst.ModuleStructure.t) ->
  ...
  if Cst.ModuleStructure.is_recursive declaration || Option.is_some (Cst.ModuleStructure.next_and_declaration declaration) then
    ...
    if not (List.is_empty (Cst.ModuleStructure.functor_parameters declaration)) then
      match Cst.ModuleStructure.functor_parameters declaration with
      | [ parameter ] -> register_local_module_functor state (Cst.ModuleStructure.name declaration) { parameter_name = Cst.Token.text parameter.name_token; result_module_type = Cst.ModuleStructure.module_type declaration; body = Cst.ModuleStructure.module_expression declaration }
      | _ -> add_unsupported_structure_item state syntax_node
    else lower_module_binding state ~syntax_node ~module_name:(Cst.ModuleStructure.name declaration) ~module_type:(Cst.ModuleStructure.module_type declaration) ~module_expression:(Cst.ModuleStructure.module_expression declaration)
and lower_module_signature_declaration = fun (state: state) (declaration: Cst.ModuleSignature.t) ->
  ...
  if Cst.ModuleSignature.is_recursive declaration || Option.is_some (Cst.ModuleSignature.next_and_declaration declaration) || not (List.is_empty (Cst.ModuleSignature.functor_parameters declaration)) then
    ...
    | Cst.ModuleSignature.Alias module_expression -> (
      ...
      | Some module_path ->
          let _ = add_item state ~syntax_node (`ModuleAlias (module_name, module_path)) in ()
      | None -> add_unsupported_signature_item state syntax_node
    )
    | Cst.ModuleSignature.Signature module_type ->
        let nested_scope_path = SurfacePath.append_name state.scope_path module_name in
        with_scope state nested_scope_path
          (
            fun () -> lower_signature_items_of_module_type state module_type ~on_unsupported:(
              fun () -> add_unsupported_signature_item state syntax_node
            )
          )
...
let lower_source_file = fun ~source source_file ->
  ...
  in
  {
    ...
    body_arena = BodyArena.of_lists ~patterns:(List.rev state.patterns) ~expressions:(List.rev state.expressions) ~bindings:(List.rev state.bindings);
