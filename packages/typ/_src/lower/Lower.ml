...
let resolve_local_module_alias_path = fun (state: state) path ->
  ...
  | head :: tail -> (
      ...
      | Some resolved_path ->
          tail
          |> List.fold_left SurfacePath.append_name resolved_path
      | None -> path
    )
  | [] -> path
...
let resolve_local_module_functor = fun (state: state) path ->
  ...
  if SurfacePath.is_bare path then
    ...
    | Some name ->
        SurfacePath.prefixes state.scope_path
        |> List.rev
            ...
            |> List.find_map
              (fun (candidate_name, candidate_scope_path, candidate_id) ->
                if
                  ...
                  && SurfacePath.equal candidate_scope_path scope_path
                then
                  Some candidate_id
                else
                  None)
...
let resolve_named_type_path = fun (state: state) path ->
  let external_head () =
    TypeRepr.named_head ~type_constructor_id:(TypeConstructorId.of_path path) ~name:path
  in
...
let register_declared_type_name = fun (state: state) name ->
  match state.declared_type_names
  |> List.find_map
    (fun (candidate_name, candidate_scope_path, candidate_id) ->
      if
  ...
  | None ->
      let type_constructor_id =
        TypeConstructorId.make ~owner:state.source_owner ~local_id:state.next_type_constructor_id
      in
      let () =
...
let with_nonrec_current_type_name_hidden = fun (state: state) (declaration: Cst.TypeDeclaration.t) f ->
  ...
  | Some _ ->
      let type_name =
        Cst.TypeDeclaration.name_token declaration
        |> Cst.Token.text
      in
...
let is_module_name = fun name -> String.length name > 0 && Char.uppercase_ascii name.[0] = name.[0] && Char.lowercase_ascii
  name.[0] != name.[0]
...
let rec module_path_segments_of_module_expression = fun (state: state) ->
  ...
  | Cst.ModuleExpression.Constraint { module_expression = inner; _ } ->
      module_path_segments_of_module_expression state inner
  | _ -> None
...
let rec unpacked_expression_of_module_expression = function
  ...
  | Cst.ModuleExpression.Attribute { module_expression = inner; _ }
  | Cst.ModuleExpression.Constraint { module_expression = inner; _ } ->
      unpacked_expression_of_module_expression inner
  | _ -> None
...
let rec structure_items_of_module_expression = function
  ...
  | Cst.ModuleExpression.Attribute { module_expression = inner; _ }
  | Cst.ModuleExpression.Constraint { module_expression = inner; _ } ->
      structure_items_of_module_expression inner
...
let next_type_param_id = fun type_params ->
  type_params
  |> List.fold_left (fun current_max (_, id) -> Int.max current_max id) (-1)
  |> Int.succ

let fresh_type_params_from_binders = fun type_params binders ->
  let start = next_type_param_id type_params in
  binders
  |> List.mapi (fun offset binder -> (Cst.TypeBinder.name binder, start + offset))

let fresh_local_abstract_type_params_from_binders = fun (state: state) binders ->
  binders
  |> List.map (fun binder -> (Cst.TypeBinder.name binder, fresh_lowered_type_var_id state))
...
let rec lower_core_type = fun (state: state) type_params core_type ->
  ...
  | Cst.CoreType.Alias { type_ = inner; _ } -> lower_core_type state type_params inner
  ...
  | Cst.CoreType.Constr { constructor_path; arguments; _ } ->
      let name =
        constructor_path
        |> ident_path
        |> resolve_local_module_alias_path state
      in
  ...
  | Some template ->
          ...
          (fun (constraint_: Cst.module_type_constraint) ->
            ...
            | Some head ->
                let replacement_type =
                  lower_core_type state type_params constraint_.replacement_type
                in
                let _ =
                  Collections.HashMap.insert replacements head.type_constructor_id replacement_type
                in
                ()
            | _ -> ())
      in
      let values =
        ...
        |> List.map
          (fun (value: TypeRepr.package_value) -> {
            value with
            scheme = TypeScheme.map_type_preserving
...
let add_binding = fun (state: state) ~syntax_node ~name ~pattern_id ~annotation ~value_id ~recursive ->
  ...
  in
  let origin_id =
    add_origin state ~semantic_id:(OriginMap.Binding binding_id) ~label:"binding" syntax_node
  in
  let binding = {
...
let rec lower_record_field_scheme = fun (state: state) type_params core_type ->
  match core_type with
  | Cst.CoreType.Parenthesized { inner; _ }
  | Cst.CoreType.Attribute { type_ = inner; _ } -> lower_record_field_scheme state type_params inner
  | Cst.CoreType.Poly { binders; body; _ } ->
      let params = fresh_type_params_from_binders type_params binders in
...
let rec collect_core_type_var_names = fun core_type ->
  ...
  let collect_many items =
    List.fold_left
      (fun acc item ->
        collect_core_type_var_names item
  ...
  | Cst.CoreType.Arrow { parameter_type; result_type; _ } ->
      collect_many [ parameter_type; result_type ]
...
let rec register_type_declaration_names = fun (state: state) (declaration: Cst.TypeDeclaration.t) ->
  let _ =
    register_declared_type_name state (Cst.Token.text (Cst.TypeDeclaration.name_token declaration))
  in
...
let lowered_type_declaration = fun (state: state) (declaration: Cst.TypeDeclaration.t) ->
  ...
  let result_type =
    TypeRepr.named
      ...
      ~arguments:(
        params
        |> List.map (fun (_, id) -> TypeRepr.make_var id)
      )
  in
              ...
              constructors =
                ...
                |> List.map
                  (fun (constructor: Cst.VariantConstructor.t) ->
                    ...
                    let payload_type =
                      variant_constructor_payload state constructor_params constructor
                    in
                    let generalized =
...
let rec first_unsupported_core_type = fun core_type ->
  let find_many items =
    items
    |> List.find_map first_unsupported_core_type
  in
  match core_type with
  | Cst.CoreType.Parenthesized { inner; _ }
  | Cst.CoreType.Attribute { type_ = inner; _ }
...
let rec scheme_of_declared_core_type = fun (state: state) core_type ->
  match core_type with
  | Cst.CoreType.Parenthesized { inner; _ }
  | Cst.CoreType.Attribute { type_ = inner; _ } -> scheme_of_declared_core_type state inner
  | Cst.CoreType.Poly { binders; body; _ } ->
...
let lower_module_type_template =
  ...
  let rec loop state module_type =
              ...
              |> List.filter_map
                (
                  ...
                  | Cst.SignatureItem.ValueDeclaration declaration ->
                      let value_name =
                        declared_value_name (Cst.ValueDeclaration.name_tokens declaration)
                      in
    ...
    | Cst.ModuleType.Parenthesized { inner; _ }
    | Cst.ModuleType.Attribute { module_type = inner; _ } -> loop state inner
    | _ -> None
  in
  loop
...
let with_local_module_binding_groups = fun (state: state) ~module_name ~local_scope f ->
  ...
  let () =
    state.local_module_binding_groups <- (module_name, local_scope)
    :: state.local_module_binding_groups
  in
  try
    let result = f () in
    let () =
...
let rec lower_pattern = fun (state: state) pattern ->
  ...
  | Cst.Pattern.Literal { syntax_node; literal; _ } -> (
      ...
      | Cst.PatternLiteral.Float float_ ->
          add_pattern
            state
            ~syntax_node
            ~label:"float_pattern"
      ...
      | Cst.PatternLiteral.Char char_ ->
          add_pattern state ~syntax_node ~label:"char_pattern" (BodyArena.PChar char_.contents)
  ...
  | Cst.Pattern.PolyVariant { syntax_node; tag_token; payload; _ } ->
      let payload =
        payload
        |> Option.map (lower_pattern state)
      in
      add_pattern
        state
        ~syntax_node
  ...
  | Cst.Pattern.FirstClassModule { syntax_node; binding; package_type; _ } ->
      ...
      let package_type =
        package_type
        |> Option.map (lower_package_type state [])
      in
      add_pattern
        state
        ~syntax_node
...
let labeled_function_parameter = fun label pattern_id -> ({
  BodyArena.label = BodyArena.Labeled label;
  pattern_id;
  default_value_id = None;
}: BodyArena.function_parameter)
...
and synthetic_var_pattern = fun (state: state) syntax_node ~label ->
  ...
  let pat_id =
    add_pattern state ~syntax_node ~label:("synthetic_" ^ label ^ "_pattern") (BodyArena.PVar name)
  in
  (name, pat_id)
...
and lower_function_like = fun (state: state) ~syntax_node ~parameters ~body_annotation_type ~body ->
  ...
  with_local_abstract_type_params
    ...
    (fun () ->
      ...
      let body_annotation =
        body_annotation_type
        |> Option.map (lower_core_type state [])
      in
        ...
        | `Cases cases ->
            ...
            let argument_expr_id =
              add_expr
                ...
                (BodyArena.EVar (SurfacePath.of_name synthetic_name))
            in
            let match_id =
              add_expr
                state
            ...
            let parameter_ids =
              parameter_ids @ [ positional_function_parameter synthetic_pattern_id ]
            in
            add_expr
              state
              ~syntax_node
...
and binding_ids_of_let_binding_group = fun (state: state) let_binding ->
  ...
  let binding_ids =
    let_binding :: Cst.LetBinding.and_bindings let_binding
    |> List.map
                ...
                let (binding_groups, type_decls) =
                  ...
                  |> List.fold_left
                    (fun (binding_groups, type_decls) item ->
                      ...
                      | Cst.StructureItem.Docstring _ -> (binding_groups, type_decls)
                      ...
                      | unsupported_item ->
                          let () =
                            add_unsupported_structure_item
                              state
                              (Cst.StructureItem.syntax_node unsupported_item)
                          in
...
and lower_infix = fun (state: state) (infix: Cst.infix_expression) ->
  ...
  let operator_id =
    add_expr
      ...
      (BodyArena.EVar (SurfacePath.of_name operator_name))
  in
  ...
  | None ->
      ...
      let operator_id =
        add_expr
          ...
          (BodyArena.EVar (SurfacePath.of_name (lower_binding_operator_name binding)))
      in
      ...
      let body_fun_id =
        add_expr
          ...
          (BodyArena.EFun ([ positional_function_parameter parameter_id ], body_id))
      in
      add_expr
        state
        ~syntax_node
...
and lower_expr = fun (state: state) expression ->
  ...
  | Cst.Expression.FieldAssign { syntax_node; target; value; _ } ->
      ...
      add_expr
        ...
        (BodyArena.EFieldAssign {
          receiver_id;
          label = Cst.Token.text target.field_name;
          value_id;
        })
  ...
  | Cst.Expression.For for_expression ->
      let iterator_pattern_id =
        add_pattern
          state
          ~syntax_node:for_expression.syntax_node
          ~label:"for_iterator_pattern"
          (BodyArena.PVar (Cst.Token.text for_expression.iterator_token))
      in
    ...
    _
      ...
      begin
        ...
        | None -> (
            ...
            | None -> (
                match lower_local_module_scope
                  state
                  ~module_name:source_module_name
                  module_expression with
                | Some local_scope ->
                    let body_id =
      ...
      | _ ->
          let operator_id =
            add_expr
              ...
              (BodyArena.EVar (SurfacePath.of_name
                (prefix_operator_name (Cst.Token.text operator_token))))
          in
...
let lower_top_level_expression = fun (state: state) expression ->
  let syntax_node = Cst.Expression.syntax_node expression in
  let pattern_id =
    add_pattern state ~syntax_node ~label:"top_level_expression_pattern" BodyArena.PWildcard
  in
  let value_id = lower_expr state expression in
  let binding_id =
    add_binding
...
and lower_signature_items_of_module_type = fun (state: state) module_type ~on_unsupported ->
  ...
  | Cst.ModuleType.Extension _ -> on_unsupported ()
  ...
  | Cst.SignatureItem.ModuleTypeDeclaration declaration -> (
      ...
      | Some module_type ->
          ...
          begin
            ...
            | Some template -> register_module_type_template state module_type_name template
            | None ->
                add_unsupported_signature_item
                  state
                  (Cst.ModuleTypeDeclaration.syntax_node declaration)
          end
      | None ->
...
and lower_signature_include_statement = fun (state: state) (
...
and lower_module_binding = fun (state: state) ~syntax_node ~module_name ~module_type ~module_expression ->
  ...
  | Cst.ModuleExpression.Apply { callee; argument; _ } -> (
      match (
        module_path_segments_of_module_expression state callee,
        module_path_segments_of_module_expression state argument
      ) with
  ...
  | _ -> (
      ...
      | None ->
          ...
          lower_module_items_under_scope
            ~nested_scope_path
            ~result_module_type:module_type
            module_expression
    )
...
let lower_source_file = fun ~source source_file ->
  ...
  let () =
    ...
    | Cst.Interface interface ->
        let _items =
          interface.items
          |> List.map (lower_signature_item state)
        in
        ()
  in
  {
