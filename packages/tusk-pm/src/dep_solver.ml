...
let find_workspace_package_by_name = fun ~(workspace_packages: Tusk_model.Package.t list) ~package_name ->
  List.find_opt
    (fun (pkg: Tusk_model.Package.t) -> String.equal pkg.name package_name)
    workspace_packages
...
let find_workspace_package_by_root = fun ~(workspace_packages: Tusk_model.Package.t list) ~package_root ->
  ...
  List.find_opt
    (fun (pkg: Tusk_model.Package.t) -> Path.equal (Path.normalize pkg.path) package_root)
    workspace_packages
...
let rec lock_package_of_local_package = fun ~(ctx: context) ~state ~provenance (pkg: Tusk_model.Package.t) ->
  match
  ...
  | _ ->
      ...
      | None -> (
          ...
          | None -> (
              ...
              | Error err ->
                  ctx.emit (Tusk_model.Event.PackageMetadataFetchFailed { package = package_name; error = err });
...
and resolve_path_dependency = fun ~(ctx: context) ~state ~declared_from dependency_name dep_path ->
  ...
  | None -> (
      ...
      | Error _ as err -> err
      | Ok pkg -> (
          match find_workspace_package_by_name ~workspace_packages:ctx.workspace_packages ~package_name:pkg.name with
          | Some workspace_pkg ->
              Ok ({
                dependency =
                  Tusk_model.Lockfile.{
...
and resolve_manifest_dependencies = fun ~(ctx: context) ~state ~declared_from acc_packages acc_dependencies deps ->
  ...
  | dep :: rest -> (
      ...
      | Tusk_model.Package.Registry _ -> (
          match find_workspace_package_by_name ~workspace_packages:ctx.workspace_packages ~package_name:dep.name with
          | Some _ ->
              resolve_manifest_dependencies
                ~ctx
                ~state
...
let lock_deps = fun ?(emit = no_emit) ~mode ~registry ~existing_lock ~workspace_root packages ->
  ...
  let ctx = { emit; mode; registry; existing_lock; workspace_root; workspace_packages = packages } in
  emit
    (Tusk_model.Event.DependencyUniverseBuilding {
      packages = List.map (fun (pkg: Tusk_model.Package.t) -> pkg.name) packages
    });
  match
    lock_packages ~ctx ~state:empty_resolution_state [] [] packages
  with
