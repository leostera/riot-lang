...
let load_manifest_toml = fun ~manifest_path ->
  ...
  | Ok source -> (
      ...
      | Ok toml -> Ok toml
      | Error err -> Error ("failed to parse manifest TOML '"
      ^ Path.to_string manifest_path
      ^ "': "
      ^ Data.Toml.error_to_string err)
    )

let load_external_package = fun ~emit ~(lock_package:Tusk_model.Lockfile.package) ->
  ...
  let emit_started =
    ...
    | Some version ->
        emit (Tusk_model.Event.PackageManifestFetchStarted { package = package_name; version })
    | None -> ()
  in
  let emit_finished = fun () ->
    match version_opt with
    | Some version ->
        emit
...
let load_package_for_lock_package = fun ~emit ~(packages:Tusk_model.Package.t list) ~(lock_package:Tusk_model.Lockfile.package) ->
  ...
  | Tusk_model.Lockfile.Workspace -> (
      ...
      | None -> Error ("workspace package '" ^ lock_package.id.name ^ "' was not provided to projection")
    )
  | Tusk_model.Lockfile.Path _
  | Tusk_model.Lockfile.Registry _ -> load_external_package ~emit ~lock_package
...
let rec resolve_package_graph = fun ~emit ~(packages:Tusk_model.Package.t list) ~(lockfile:Tusk_model.Lockfile.t) seen acc pending ->
  match pending with
  | [] -> Ok (List.rev acc)
  | package_id :: rest ->
      let key = package_id_key package_id in
      if List.mem key seen then
        resolve_package_graph ~emit ~packages ~lockfile seen acc rest
      else
        match find_lock_package_by_id ~package_id ~lockfile with
        | None -> Error ("lockfile is missing package '" ^ package_id.name ^ "'")
        | Some lock_package -> (
            match load_package_for_lock_package ~emit ~packages ~lock_package with
            | Error _ as err -> err
            | Ok package -> (
...
let resolve_packages = fun ?(emit = no_emit) ~packages ~lockfile () ->
  let root_ids = List.map workspace_package_id_of_package packages in
  resolve_package_graph ~emit ~packages ~lockfile [] [] root_ids
