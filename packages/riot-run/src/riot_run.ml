...
let build_scope_for_binary = fun (workspace: Riot_model.Workspace.t) ~package_name ~binary_name ->
  match
    List.find (Riot_model.Workspace.realize_packages ~intent:Riot_model.Package.Dev workspace)
      ~fn:(fun (pkg: Riot_model.Package.t) ->
...
let list_binaries = fun (workspace: Riot_model.Workspace.t) ?package_filter () ->
  realized_runnable_packages ?package_filter workspace |> List.flat_map
    ~fn:(fun (pkg: Riot_model.Package.t) ->
      pkg.binaries |> List.filter ~fn:is_listed_runnable |> List.map
...
let find_built_binary_path = fun ~(store:Riot_store.Store.t) ~(output:Riot_build.Build_result.t) ~package_name ~binary_name ->
  ...
  | Some export_entry -> (
      ...
      | Some path -> ensure_executable_binary_path path
...
let run = fun ?(on_event = no_event) (request: run_request) ->
  let* package_name = resolve_binary
  ...
  let build_request = Riot_build.Request.make
    ~workspace:request.workspace
    ...
    () in
  let* output = Riot_build.build ~on_event:(fun event -> on_event (Build event)) build_request
