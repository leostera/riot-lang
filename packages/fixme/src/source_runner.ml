...
let lint_diagnostics ~rules ?filename
  ...
  if List.length parse_result.diagnostics > 0 then
    ...
    | Ok cst ->
        ...
        rules |> List.concat_map (fun rule -> Rule.run rule ctx red_tree)
...
let has_parse_errors result =
  List.length result.parse_diagnostics > 0
