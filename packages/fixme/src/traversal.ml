...
and let_bindings_of_expression = fun expr ->
    ...
    @ (cases |> List.concat_map let_bindings_of_match_case)
    | Syn.Cst.Expression.If { condition; then_branch; else_branch; _ } -> let_bindings_of_expression
      condition
...
let rec expressions_of_expression = fun expr ->
    let nested =
      ...
      @ (cases |> List.concat_map expressions_of_match_case)
      | Syn.Cst.Expression.If { condition; then_branch; else_branch; _ } -> expressions_of_expression
        condition
