...
let source_of_node node = Source_of_node node
let source_of_token token = Source_of_token token
let text value = Text value
let delete ~target = Delete { target }
let delete_node target = delete ~target:(Node target)
let replace ~target ~replacement = Replace { target; replacement }

let replace_node ~target ~replacement =
  replace ~target:(Node target) ~replacement:(source_of_node replacement)

let replace_node_with_text ~target ~text:value =
  replace ~target:(Node target) ~replacement:(text value)

let replace_token_with_text ~target ~text:value =
  replace ~target:(Token target) ~replacement:(text value)

let insert_before ~anchor ~content = Insert_before { anchor; content }
let insert_after ~anchor ~content = Insert_after { anchor; content }
let swap ~left ~right = Swap { left; right }
let make ~title ~operations = { title; operations }
let title fix = fix.title
let operations fix = fix.operations

let target_span = function
  | Node node ->
      Syn.Ceibo.Red.SyntaxNode.span node
  | Token token ->
      Syn.Ceibo.Red.SyntaxToken.span token

let source_slice ~source span =
  let len = span.Syn.Ceibo.Span.end_ - span.start in
  String.sub source span.start len

let replacement_text ~source = function
  | Source_of_node node ->
      source_slice ~source (Syn.Ceibo.Red.SyntaxNode.span node)
  | Source_of_token token ->
      source_slice ~source (Syn.Ceibo.Red.SyntaxToken.span token)
  | Text text ->
      text

let make_insert_at pos ~new_text =
  let span = Syn.Ceibo.Span.make ~start:pos ~end_:pos in
  { span; new_text }

let lower_operation ~source = function
  | Delete { target } ->
      Ok [ { span = target_span target; new_text = "" } ]
  | Replace { target; replacement } ->
      Ok
        [ { span = target_span target; new_text = replacement_text ~source replacement } ]
  | Insert_before { anchor; content } ->
      let anchor_span = target_span anchor in
      Ok
        [
          make_insert_at anchor_span.start
            ~new_text:(replacement_text ~source content);
        ]
  | Insert_after { anchor; content } ->
      let anchor_span = target_span anchor in
      Ok
        [
          make_insert_at anchor_span.end_
            ~new_text:(replacement_text ~source content);
        ]
  | Swap { left; right } ->
...
let apply_operation ~source operation =
...
let validate_fix ~source fix =
  ...
  | Error _ as err ->
      err
  | Ok edits ->
      validate_edits ~source (List.concat edits) |> Result.map (fun _ -> ())
...
let apply_fixes ~source fixes =
  let lowered =
    fixes
    |> List.map (fun fix -> List.map (lower_operation ~source) fix.operations)
    |> List.concat
  in
