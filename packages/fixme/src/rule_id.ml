...
let split = fun ~default_package value ->
  ...
  | Some idx ->
      ...
      let local_id = String.sub value ~offset:(idx + 1) ~len:(String.length value - idx - 1) in (package_name, local_id)
  | None -> default_package, value

let package_name = fun ~default_package value ->
  let package_name, _ = split ~default_package value in package_name

let local_id = fun value ->
  let _, local_id = split ~default_package:"riot" value in local_id

let qualify = fun ~package_name value ->
  if has_package_name value then
    value
  else package_name ^ ":" ^ value
