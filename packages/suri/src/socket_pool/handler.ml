...
let default = {
  to_string_error = (
    fun _err -> "unknown error"
  );
  handle_close = (
    fun _sock _state -> ()
  );
  handle_connection = (
    fun _sock state -> Continue state
  );
  handle_data = (
    fun _data _sock state -> Continue state
  );
  handle_error = (
    fun err _sock state -> Error (state, err)
  );
  handle_shutdown = (
    fun _sock _state -> Ok
  );
  handle_message = (
    fun _msg _conn state -> Continue state
  )
}
