...
let start_link = fun ~host ~port ?(acceptors = 100) ?(buffer_size = 4_096) ?(transport = Transport.tcp ()) (type s e) (handler: (s, e) Handler.handler) (initial_ctx: s) ->
  ...
  | Ok addr -> (
    ...
    | Ok listener ->
        ...
        let acceptor_supervisor = Supervisor.Dynamic.start_link ~intensity:{ max_restarts = 10; window = Time.Duration.from_secs 60 } ~max_children:(acceptors * 2) () in
        ...
        for i = 1 to acceptors do
          let start () =
            let state = Acceptor.{
              ...
              initial_ctx;
              transport
            } in Acceptor.spawn state
          in
          match Supervisor.Dynamic.start_child acceptor_supervisor ~start ~restart:Permanent ~shutdown:(Timeout (Time.Duration.from_secs 5)) () with
          | Ok _pid -> ()
