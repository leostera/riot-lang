...
let is_http1 = fun data ->
  ...
  List.exists
    (
      fun method_ -> String.starts_with ~prefix:method_ data
    )
    methods
  ...
  if Cell.get state.detected then
    ...
    begin
      ...
      | Some true ->
          ...
          let http2_state = Http2_handler.make_handler ~config:state.config ~handler:state.handler ~sniffed_data:buffer_data () in Socket_pool.Handler.Switch (Socket_pool.Handler.H { handler = http2_handler; state = http2_state })
      | Some false ->
          (* Looks like HTTP/1.1 *)
          if is_http1 buffer_data then
            begin
              Cell.set state.detected true;
              let http1_state = Http1_handler.make_handler ~config:state.config ~handler:state.handler ~sniffed_data:buffer_data () in Socket_pool.Handler.Switch (Socket_pool.Handler.H { handler = http1_handler; state = http1_state })
            end
          else Socket_pool.Handler.Error (state, `Detection_error "Unknown protocol")
      | None ->
          ...
          else Socket_pool.Handler.Continue state
    end
