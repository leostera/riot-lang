...
let parse_error_to_string = function
  | Http.Http2.Parser_reader.Incomplete_frame_header ->
  ...
  | Http.Http2.Parser_reader.Incomplete_settings_payload ->
      "incomplete settings payload"

let to_string_error = function
  | `Parse_error err ->
      "HTTP/2 parse error: " ^ parse_error_to_string err
  ...
  let settings_frame =
    Http.Http2.Frame.settings
      [
        ...
        Http.Http2.Frame.MaxFrameSize 16384;
        Http.Http2.Frame.InitialWindowSize 65535;
      ]
  in
...
let send_headers conn hpack_encoder stream_id headers end_stream =
  ...
  in
  let header_block =
    Http.Http2.Hpack.encode hpack_encoder ~headers ~sensitive_headers:[]
    |> Bytes.to_string
  in
  let frame =
...
let headers_to_request conn headers body =
  ...
  let uri =
    Net.Uri.of_string uri |> Result.expect ~msg:"HTTP/2 request path must be a valid URI"
  in
  let http_request =
...
let handle_stream conn state stream_id stream =
  if not (Cell.get stream.end_stream) then
    ...
    let headers =
      ...
      |> List.map (fun header -> (header.Http.Http2.Hpack.name, header.value))
    in
    ...
    | Http_handler.Response response ->
        ...
        ] @ Net.Http.Header.to_list response.headers in
        ...
        (match send_headers conn state.hpack_encoder stream_id response_headers false with
         ...
         | Ok () ->
             ...
             | Ok () ->
                 ...
                 Ok ())
    | Http_handler.Upgrade _ ->
        Error (`Protocol_error "HTTP/2 upgrades are not supported")
...
let process_frame conn state frame =
  ...
  | Http.Http2.Frame.Headers, Http.Http2.Frame.HeadersPayload { header_block_fragment; _ } ->
      ...
      (match Http.Http2.Hpack.decode state.hpack_decoder (Bytes.of_string header_block_fragment) with
       | Ok headers ->
           ...
           let stream = match HashMap.get state.streams stream_id with
             | Some s -> s
             | None ->
                 let s = {
                   stream_id;
  ...
  | Http.Http2.Frame.Data, Http.Http2.Frame.DataPayload { data; _ } ->
      ...
      (match HashMap.get state.streams stream_id with
       | None ->
           Error (`Protocol_error ("DATA for unknown stream " ^ Int.to_string stream_id))
       | Some stream ->
           (* Add data *)
