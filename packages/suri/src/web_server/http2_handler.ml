...
let parse_error_to_string = function
  ...
  | Http.Http2.Parser_reader.Frame_size_exceeds_maximum { size; max_size } -> "frame size " ^ Int.to_string size ^ " exceeds maximum " ^ Int.to_string max_size
  | Http.Http2.Parser_reader.Unknown_frame_type frame_type -> "unknown frame type " ^ Int.to_string frame_type
  | Http.Http2.Parser_reader.Invalid_payload_length { frame_type; expected; actual } -> "invalid " ^ frame_type ^ " payload length: expected " ^ Int.to_string expected ^ ", got " ^ Int.to_string actual
...
let send_settings = fun conn ->
  let settings_frame = Http.Http2.Frame.settings [ Http.Http2.Frame.MaxConcurrentStreams 100; Http.Http2.Frame.MaxFrameSize 16_384; Http.Http2.Frame.InitialWindowSize 65_535 ] in
...
let send_headers = fun conn hpack_encoder stream_id headers end_stream ->
  let headers = List.map ~fn:(
    fun ((name, value)) -> { Http.Http2.Hpack.name = name; value }
  ) headers in
...
let headers_to_request = fun conn headers body ->
  let method_ = Std.Collections.Proplist.get headers ~key:":method" |> Option.map ~fn:Net.Http.Method.of_string |> Option.unwrap_or ~default:Net.Http.Method.Get in
  let uri = Std.Collections.Proplist.get headers ~key:":path" |> Option.unwrap_or ~default:"/" in
  let headers = List.filter ~fn:(
  ...
  let http_request =
    let request = Net.Http.Request.create method_ uri in Net.Http.Request.with_headers request (Net.Http.Header.of_list headers)
  in
  let _ = conn in Request.of_http ~body http_request
...
let handle_stream = fun conn state stream_id stream ->
  ...
  (* Stream not complete yet *)
    ...
    let headers = List.rev (Cell.get stream.headers) |> List.map ~fn:(
      fun header -> (header.Http.Http2.Hpack.name, header.value)
    ) in
    ...
    | Http_handler.Response response ->
        ...
        (
          ...
          | Ok () ->
              ...
              | Error e -> Error e
              | Ok () ->
                  let _ = HashMap.remove state.streams ~key:stream_id in Ok ()
        )
        ...
        | Ok headers ->
            ...
            let stream =
              ...
              | None ->
                  ...
                  in
                  let _ = HashMap.insert state.streams ~key:stream_id ~value:s in s
            in
            (* Add headers *)
        ...
        begin
          if verify_preface buffer_data then
            begin
              ...
              Cell.set state.buffer (String.sub buffer_data ~offset:24 ~len:(String.length buffer_data - 24));
              (* Send SETTINGS *)
          ...
          else Socket_pool.Handler.Error (state, `Protocol_error "Invalid HTTP/2 preface")
        end
      else (* Need more data for preface *)
      Socket_pool.Handler.Continue state
    end
  else
    begin
