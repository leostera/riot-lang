...
let parse_error_to_string = function
  ...
  | Http.Http2.Parser_reader.Frame_size_exceeds_maximum { size; max_size } -> "frame size "
  ^ Int.to_string size
  ^ " exceeds maximum "
  ^ Int.to_string max_size
  | Http.Http2.Parser_reader.Unknown_frame_type frame_type -> "unknown frame type " ^ Int.to_string frame_type
  | Http.Http2.Parser_reader.Invalid_payload_length { frame_type; expected; actual } -> "invalid "
  ^ frame_type
...
let send_headers = fun conn hpack_encoder stream_id headers end_stream ->
    ...
    in
    let header_block = Http.Http2.Hpack.encode hpack_encoder ~headers ~sensitive_headers:[]
    |> Bytes.to_string in
...
let handle_stream = fun conn state stream_id stream ->
    if not (Cell.get stream.end_stream) then
      ...
      | Http_handler.Response response ->
          ...
          let response_headers = [ (":status", Int.to_string (Net.Http.Status.to_int status));  ]
