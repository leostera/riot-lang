...
let send_response = fun conn (res: Response.t) ->
  ...
  let headers =
    ...
    | _ when not (Net.Http.Header.has headers "content-length") -> Net.Http.Header.set headers "content-length" body_len
    | _ -> headers
  in
  let status_line = (Net.Http.Version.to_string res.version) ^ " " ^ (Int.to_string (Net.Http.Status.to_int res.status)) ^ " " ^ (Net.Http.Status.to_string res.status) ^ "\r\n" in
  let header_lines = Net.Http.Header.to_list headers |> List.map ~fn:(
    fun ((k, v)) -> k ^ ": " ^ v ^ "\r\n"
  ) |> String.concat "" in
...
let compute_websocket_accept = fun key ->
  ...
  let hash_bytes = Crypto.Hash.to_bytes hash in Encoding.Base64.encode_bytes hash_bytes
...
let websocket_to_socket_pool_handler: Channel.Handler.t -> Socket_pool.Handler.t = fun ws_handler ->
  let handler = {
    Socket_pool.Handler.to_string_error = (
      fun (`Unknown_opcode code) -> "Unknown WebSocket opcode: " ^ string_of_int code
    );
    handle_close = (
      fun _conn _state -> ()
    );
      ...
      fun data conn ws_handler ->
        ...
        (
          ...
          | `push (out_frames, new_handler) ->
              ...
              let frame_data = out_frames |> List.map ~fn:Http.Ws.Serializer.serialize |> String.concat "" in
              (
        ...
        | Need_more -> (* Need more data - keep waiting *)
    ...
    handle_error = (
      fun err _conn state -> Socket_pool.Handler.Error (state, err)
    );
    handle_shutdown = (
      fun _conn state -> Socket_pool.Handler.Close state
    );
    handle_message = (
...
let handle_websocket_upgrade = fun state socket_conn req ws_handler ->
  ...
  | None ->
      ...
      let _ = send_response socket_conn res in Socket_pool.Handler.Close state
  | Some key ->
      (* Compute accept key *)
      ...
      let header_lines = Net.Http.Header.to_list response_headers |> List.map ~fn:(
        fun ((k, v)) -> k ^ ": " ^ v ^ "\r\n"
      ) |> String.concat "" in
      let response_bytes = status_line ^ header_lines ^ "\r\n" in
      Log.info ("Sending WebSocket upgrade response (" ^ string_of_int (String.length response_bytes) ^ " bytes)");
      ...
      | Ok () ->
          ...
          let socket_pool_handler = websocket_to_socket_pool_handler ws_handler in Socket_pool.Handler.Switch socket_pool_handler
      | Error `Closed ->
...
let handle_request = fun state socket_conn (req: Request.t) ->
  ...
  (
    ...
    | Ok () ->
        ...
        let new_state = { state with is_keep_alive; requests_processed; parse_state = WaitingForHeaders } in
...
let handle_data_waiting_headers = fun full_data conn state ->
  match Http.Http1.Request.parse ~max_request_line:state.config.max_request_line_length ~max_headers:state.config.max_header_count ~max_header_length:state.config.max_header_length full_data with
  | Done { value = http_req; remaining } ->
      ...
      if body_received >= expected_length then
        let req = Request.of_http ~body:remaining http_req in handle_request ({ state with parse_state = WaitingForHeaders; sniffed_data = "" }) conn req
      else (* Need to read more body data - transition to WaitingForBody state *)
      Socket_pool.Handler.Continue ({ state with sniffed_data = ""; parse_state = WaitingForBody { http_req; expected_length; accumulated_body = remaining } })
  | Need_more -> Socket_pool.Handler.Continue ({ state with sniffed_data = full_data })
  | Error msg ->
      let res = Response.bad_request ~body:msg () in
      let _ = send_response conn res in Socket_pool.Handler.Close state
...
let handle_data_waiting_body = fun data conn state http_req expected_length accumulated_body ->
  ...
  if body_length >= expected_length then
    ...
    let result = handle_request ({ state with parse_state = WaitingForHeaders; sniffed_data = remaining_data }) conn req in (* If there's remaining data and we're keeping the connection alive, it might be the start of the next request *)
    result
  else (* Still need more data *)
  Socket_pool.Handler.Continue ({ state with parse_state = WaitingForBody { http_req; expected_length; accumulated_body = new_body } })

let handle_data = fun data conn state ->
  match state.parse_state with
  | WaitingForHeaders ->
      let full_data = state.sniffed_data ^ data in handle_data_waiting_headers full_data conn state
  | WaitingForBody { http_req; expected_length; accumulated_body } -> handle_data_waiting_body data conn state http_req expected_length accumulated_body
