...
let websocket_to_socket_pool_handler : Channel.Handler.t -> Socket_pool.Handler.t = fun ws_handler ->
  let handler = {
    Socket_pool.Handler.to_string_error = (fun (`Unknown_opcode code) -> "Unknown WebSocket opcode: "
    ^ string_of_int code);
      ...
      | Done { value=frame; remaining=_ } -> (* Process frame through the Channel handler *)
        (
          ...
          | `push (out_frames, new_handler) ->
              ...
              let frame_data = out_frames |> List.map Http.Ws.Serializer.serialize |> String.concat "" in
              (
