...
let compute_websocket_accept key =
...
let websocket_to_socket_pool_handler (ws_handler : Channel.Handler.t) : Socket_pool.Handler.t =
  let handler = {
    ...
    handle_connection = (fun conn ws_handler ->
    ...
    handle_data = (fun data conn ws_handler ->
      ...
      | Done { value = frame; remaining = _ } ->
          (* Process frame through the Channel handler *)
    ...
    handle_error = (fun err _conn state ->
      Socket_pool.Handler.Error (state, err));
    
    handle_shutdown = (fun _conn state ->
      Socket_pool.Handler.Close state);
    
    handle_message = (fun msg conn ws_handler ->
      let stream = Socket_pool.Connection.stream conn in
...
let handle_websocket_upgrade state socket_conn req ws_handler =
  ...
  | Some key ->
      ...
      let response_headers = Net.Http.Header.empty
...
let handle_request state socket_conn (req : Request.t) =
