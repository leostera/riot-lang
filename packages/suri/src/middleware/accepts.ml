...
let parse_accept_entry = fun entry ->
  ...
  | media_type :: params ->
      let quality = List.filter_map ~fn:parse_quality params |> List.head |> Option.unwrap_or ~default:1.0 in { media_type = String.trim media_type; quality }
...
) |> List.map ~fn:parse_accept_entry |> List.sort ~compare:(
  fun a b -> Float.compare b.quality a.quality
)
