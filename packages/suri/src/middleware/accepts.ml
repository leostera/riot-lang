...
let matches_pattern ~pattern ~content_type =
  ...
  | pat, ct -> (
      ...
      match
        ( String.split_on_char '/' pat,
          String.split_on_char '/' ct )
      with
      | [ type1; "*" ], [ type2; _ ] ->
          String.equal type1 type2
      | [ type1; sub1 ], [ type2; sub2 ] ->
          String.equal type1 type2 && String.equal sub1 sub2
      | _ -> false)
...
let parse_accept_entry entry =
  match String.split_on_char ';' entry with
  | [] -> { media_type = "*/*"; quality = 1.0 }
  | media_type :: params ->
      let quality =
        List.find_map parse_quality params |> Option.unwrap_or ~default:1.0
      in
...
let parse_accept header =
  ...
  |> List.sort (fun a b -> Float.compare b.quality a.quality)
...
let reject_not_acceptable conn config received =
  match config.on_reject with
  | Some handler -> handler conn received
  | None ->
      Conn.respond conn ~status:NotAcceptable
        ~body:"Not Acceptable"
      |> Conn.halt

(** Send 415 Unsupported Media Type response *)
let reject_unsupported_media_type conn config received =
  match config.on_reject with
  | Some handler -> handler conn received
  | None ->
      Conn.respond conn ~status:UnsupportedMediaType
...
let check_accept_header conn config =
  let headers = Conn.headers conn in
  match Net.Http.Header.get headers "accept" with
  | None -> (true, None) (* No Accept header = accept anything *)
  | Some accept ->
      ...
      let matches =
        List.exists
          (fun entry ->
            List.exists
              (fun pattern ->
                matches_pattern ~pattern ~content_type:entry.media_type)
              config.types)
          entries
      in
      (matches, Some accept)
...
let check_content_type_header conn config =
  let headers = Conn.headers conn in
  match Net.Http.Header.get headers "content-type" with
  | None -> (false, None) (* Body without Content-Type *)
  | Some ct -> (
      match get_base_content_type ct with
      | Some base ->
          let matches =
            List.exists
              (fun pattern -> matches_pattern ~pattern ~content_type:base)
              config.types
          in
          (matches, Some ct)
      | None -> (false, Some ct))
...
let has_request_body method_ =
  match method_ with
  | Net.Http.Method.Post | Put | Patch -> true
  | _ -> false
...
let make config =
  fun ~conn ~next ->
    ...
    let accept_ok, accept_value =
      if config.check_accept then check_accept_header conn config
      else (true, None)
    in
