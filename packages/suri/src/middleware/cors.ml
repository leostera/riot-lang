    ...
    | Some req_origin ->
        ...
        if not (is_origin_allowed origins req_origin) then
          begin
            Log.debug (String.concat "" [
              "[CORS] Rejected origin: "; req_origin;
              " for "; Net.Http.Method.to_string (Conn.method_ conn);
              " "; Conn.path conn
            ]);
            conn
        ...
        then
          begin
            ...
            Log.debug (String.concat "" [
              "[CORS] Preflight from origin: "; req_origin;
              " -> "; origin_val
            ]);
            ...
            let conn = 
              match max_age with
              | Some age -> 
                  Conn.with_header "access-control-max-age" 
                    (string_of_int age) conn
              | None -> conn
            in
        ...
        (* Simple CORS request - add headers to response *)
          ...
          begin
            ...
            Log.debug (String.concat "" [
              "[CORS] Simple request from origin: "; req_origin;
              " -> "; origin_val
            ]);
