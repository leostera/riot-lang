...
let middleware = fun ~origins ?(methods = [Net.Http.Method.Put; Patch; Delete]) ?(headers = []) ?(credentials = false) ?(expose = []) ?max_age () ->
  ...
  fun ~conn ~next ->
    ...
    | None -> (* No CORS request - pass through *)
    next conn
    | Some req_origin ->
        ...
        if not (is_origin_allowed origins req_origin) then
          begin
            ...
            conn |> Conn.respond ~status:Net.Http.Status.Forbidden ~body:"Origin not allowed" |> Conn.halt
          end
        (* Check if this is a preflight request *)
        else
          if Conn.method_ conn = Net.Http.Method.Options && (Net.Http.Header.get req_headers "access-control-request-method" |> Option.is_some) then
            begin
              ...
              let all_methods = [ Net.Http.Method.Get; Head; Post ] @ methods |> List.unique ~compare |> List.map ~fn:Net.Http.Method.to_string |> String.concat ", " in
              Log.debug
                (
                  String.concat ""
                    [
              ...
              let conn = conn |> Conn.respond ~status:Net.Http.Status.Ok |> Conn.with_header "access-control-allow-origin" origin_val |> Conn.with_header "access-control-allow-methods" all_methods in
            ...
            begin
              ...
              Log.debug
                (
                  String.concat ""
                    [
                      "[CORS] Simple request from origin: ";
                      req_origin;
                      " -> ";
                      origin_val;
                    ]
