  ...
  | Net.Http.Method.Post -> (
    ...
    | Some method_str -> (
      ...
      | Some new_method ->
          ...
          let conn' = Conn.with_method new_method conn in next conn'
      | None -> (* Invalid method - ignore and continue as POST *)
      next conn
    )
    | None -> (* No _method parameter - continue as POST *)
    next conn
  )
  | _ -> (* Not a POST - pass through unchanged *)
  next conn
