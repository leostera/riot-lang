...
let middleware ?(header = "x-forwarded-for") ~proxies ~conn ~next =
  (* If no proxies configured, pass through unchanged (safe default) *)
  if List.length proxies = 0 then next conn
  else
