...
let parse_forwarded_for = fun header_value -> String.split_on_char ',' header_value |> List.map ~fn:String.trim |> List.filter ~fn:(
  ...
  let rec walk_chain = function
    ...
    | ip_str :: rest ->
        ...
        if is_trusted_proxy proxies ip_str then
          walk_chain rest
        else (* First untrusted IP = real client *)
        Some ip_str
  in
...
let middleware = fun ?(header = "x-forwarded-for") () ~proxies ~conn ~next ->
  ...
  if List.length proxies = 0 then
    ...
    | None -> (* No header present, use existing peer *)
    next conn
    | Some header_value -> (
      ...
      | Some real_ip_str ->
          ...
          let conn' = Conn.with_peer new_peer conn in next conn'
      | None -> (* Couldn't determine real IP, use existing peer *)
      next conn
    )
