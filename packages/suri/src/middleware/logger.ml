...
let logger ~conn ~next =
  ...
  (* Build log message *)
  let msg = method_str ^ " " ^ path ^ " -> " ^ 
            Int.to_string status ^ " in " ^ 
            duration_str in
  
  (* Log at appropriate level based on status and duration *)
  if status >= 500 then Log.error msg
  else if status >= 400 then Log.warn msg
  else if duration_ms >= 1000 then Log.warn msg
  else Log.info msg;
