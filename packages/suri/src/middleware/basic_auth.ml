...
let sanitize_realm = fun realm ->
  ...
  chars |> List.enumerate |> List.for_each ~fn:(
    fun (index, char) ->
  ...
  | [ "Basic"; encoded ] -> (
    ...
    | Result.Ok decoded -> (
      ...
      | Option.Some idx ->
          ...
          let password = String.sub decoded ~offset:(idx + 1) ~len:(String.length decoded - idx - 1) in Option.some (username, password)
...
let unauthorized = fun conn realm ->
  ...
  let auth_header = "Basic realm=\"" ^ sanitized_realm ^ "\"" in Conn.respond conn ~status:Unauthorized ~body:"Unauthorized" |> Conn.with_header "www-authenticate" auth_header |> Conn.halt
...
let middleware = fun ?(realm = "Restricted Area") ?skip ~username ~password () ->
  fun ~conn ~next ->
    ...
    if should_skip then
      ...
      | Option.None -> (* No credentials provided - return 401 *)
      unauthorized conn realm
    ...
    if should_skip then
      ...
      | Option.Some (username, password) -> (
        ...
        | Option.Some user_data ->
            ...
            let conn' = assign "basic_auth_user" user_data conn in next conn'
