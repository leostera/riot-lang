...
type upgrade_info = { opts: Channel.Handler.upgrade_opts; handler: Channel.Handler.t }
...
let query_params = fun t ->
  ...
  | Some idx ->
      ...
      let pairs = String.split_on_char '&' query_string in List.filter_map ~fn:(
        fun pair ->
          match String.index_of pair ~char:'=' with
          | None -> None
          | Some eq_idx ->
              let key = String.sub pair ~offset:0 ~len:eq_idx in
              let value = String.sub pair ~offset:(eq_idx + 1) ~len:(String.length pair - eq_idx - 1) in (* URL decode the key and value *)
...
let render_component = fun ?(headers = []) status component t ->
  let t = List.fold_left headers ~init:t ~fn:(
    fun acc ((name, value)) -> with_header name value acc
  ) in t |> with_status status |> with_header "Content-Type" "text/html; charset=utf-8" |> with_body (Component.to_html component) |> send

let render_json = fun ?(headers = []) status json t ->
  let t = List.fold_left headers ~init:t ~fn:(
    fun acc ((name, value)) -> with_header name value acc
  ) in t |> with_status status |> with_header "Content-Type" "application/json" |> with_body (Data.Json.to_string json) |> send

let render_text = fun ?(headers = []) status text t ->
  let t = List.fold_left headers ~init:t ~fn:(
    fun acc ((name, value)) -> with_header name value acc
  ) in t |> with_status status |> with_header "Content-Type" "text/plain; charset=utf-8" |> with_body text |> send

let redirect = fun ?(headers = []) path t ->
  let t = List.fold_left headers ~init:t ~fn:(
    fun acc ((name, value)) -> with_header name value acc
  ) in t |> with_status Found |> with_header "Location" path |> with_body "" |> send
...
let to_response = fun t -> Web_server.Response.make t.resp_status ~headers:t.resp_headers ~body:t.resp_body ()
