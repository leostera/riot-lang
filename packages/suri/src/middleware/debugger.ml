      ...
      if after_sandbox >= String.length path then
        ...
        (
          ...
          | Some slash_pos ->
              ...
              let relative_path =
                String.sub rest ~offset:after_slash ~len:(String.length rest - after_slash)
              in
...
let get_package_sources = fun package_name ->
  ...
  | Ok (workspace, _load_errors) -> (
      ...
      | Result.Ok package_name_t -> (
          ...
          | Some manifest ->
              let package =
                Riot_model.Workspace_manifest.realize_package
                  ~intent:Riot_model.Package.Build
                  manifest
              in
              let sources =
                (((package.sources.src @ package.sources.tests) @ package.sources.examples)
...
let resolve_source_path = fun path ->
  ...
  | Some sandbox_info -> (
      ...
      | Some actual_path ->
          { resolved_path = make_workspace_relative actual_path; found_via_riot = true }
      | None ->
          (* Fallback: construct expected path *)
          let fallback =
            String.concat
              ""
              [ "./packages/"; sandbox_info.package_name; "/"; sandbox_info.relative_path; ]
          in
...
let extract_quoted = fun line pattern ->
  ...
  | Some start_quote ->
      ...
      (
        ...
        | None -> None
        | Some end_quote ->
            Some (String.sub line ~offset:after_quote ~len:(end_quote - after_quote))
      )
  ...
  let function_name =
    if String.starts_with ~prefix:"Raised at " line then
      ...
      (
        ...
        | Some space_pos ->
            Some (
              String.sub rest ~offset:0 ~len:space_pos
              |> String.trim
            )
...
let extract_module_from_function = fun func_name ->
  ...
  | Some dot_pos ->
      ...
      if String.contains module_part "_" then
        ...
        let non_empty = List.filter ~fn:(fun s -> s != "") components in
        (* If all components start with uppercase, it's canonical form *)
        let all_capitalized =
          List.for_all
            (fun s ->
              String.length s > 0
              && String.get_unchecked s ~at:0 >= 'A'
              && String.get_unchecked s ~at:0 <= 'Z')
            non_empty
        in
...
let render_stack_frame = fun frame ->
  ...
  in
  div
    ~attrs:[ class_ "stack-frame" ]
    [
      div
        ~attrs:[ class_ "frame-header" ]
        [
...
let render_request = fun conn ->
  ...
  div
    ~attrs:[ class_ "request-inspector" ]
    [
      h2 [ text "📨 Request" ];
      div
...
let render_response = fun conn ->
  ...
  div
    ~attrs:[ class_ "response-inspector" ]
    [
        ...
        [
          ...
          script
            ~attrs:[
              attr
                "src"
                "https://cdnjs.cloudflare.com/ajax/libs/highlight.js/11.11.1/languages/ocaml.min.js";
            ]
            "";
            ...
            [
              ...
              div
                ...
                [
                  ...
                  div ~attrs:[ class_ "right-column" ] [ render_request conn; render_response conn ];
                ];
            ];
        ];
    ]
