...
let make_workspace_relative path =
  ...
  | None -> path  (* Not in a workspace, return as-is *)
  | Some workspace_root ->
...
let resolve_source_path path =
  ...
  | Some sandbox_info ->
      ...
      (match find_source_via_tusk sandbox_info with
       | Some actual_path ->
           { resolved_path = make_workspace_relative actual_path; found_via_tusk = true }
       | None ->
...
let render_snippet snippet =
  ...
  (* Render each line separately so we can highlight the error line *)
  let line_number_divs = List.map (fun (line_num, _) ->
    let is_error = line_num = snippet.error_line in
    let classes = if is_error then "line-number highlighted-line" else "line-number" in
    div ~attrs:[class_ classes; attr "data-line-number" (Int.to_string line_num)] [
      text (Int.to_string line_num)
    ]
  ) snippet.lines in
  
  (* Each line gets its own code block for syntax highlighting *)
  let code_line_divs = List.map (fun (line_num, content) ->
    let is_error = line_num = snippet.error_line in
    let classes = if is_error then "code-line highlighted-line" else "code-line" in
    div ~attrs:[class_ classes; attr "id" (String.concat "" ["LC"; Int.to_string line_num])] [
      pre [
        code ~attrs:[class_ "language-ocaml"] [text content]
      ]
    ]
  ) snippet.lines in
  ...
  div ~attrs:[class_ "source-snippet"] [
    ...
    (* Source code - each line separately for highlighting *)
