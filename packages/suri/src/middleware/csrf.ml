...
let random_bytes = fun length ->
  ...
  for i = 0 to length - 1 do
    let byte = Random.int 256 |> Result.expect ~msg:"Failed to generate random byte" in IO.Bytes.set_unchecked bytes ~at:i ~char:(Char.chr byte)
  done;
...
let mask_token = fun raw_token_hex ->
  ...
  | Option.Some raw_bytes ->
      ...
      for i = 0 to 31 do
        ...
        let raw_byte = Char.code (String.get_unchecked raw_bytes ~at:i) in IO.Bytes.set_unchecked masked ~at:i ~char:(Char.chr (pad_byte lxor raw_byte))
      done;
      (* Combine pad + masked (64 bytes total) and base64 encode *)
      let combined = pad ^ IO.Bytes.to_string masked in Encoding.Base64.encode combined
...
let unmask_token = fun masked_b64 ->
  ...
  | Result.Ok decoded ->
      ...
      if not (len = 64) then
        ...
        for i = 0 to 31 do
          let pad_byte = Char.code (String.get_unchecked pad ~at:i) in
          let masked_byte = Char.code (String.get_unchecked masked ~at:i) in IO.Bytes.set_unchecked raw_bytes ~at:i ~char:(Char.chr (pad_byte lxor masked_byte))
        done;
      (* Convert back to hex *)
      let unmasked_hex = bytes_to_hex (IO.Bytes.to_string raw_bytes) in Option.some unmasked_hex
...
let middleware = fun ?(param_name = "_csrf_token") ?(header_name = "x-csrf-token") ?(skip_safe_methods = true) ?skip () ->
  fun ~conn ~next ->
    ...
    (* Skip safe methods if configured *)
      ...
      if skip_safe_methods && is_safe_method (Conn.method_ conn) then
        next conn
      else
        (* Get session *)
        ...
        let request_token =
          ...
          (
            ...
            | Option.Some token -> Option.some token
            | Option.None -> (* Try header as last resort *)
        ...
        | Option.Some token ->
            ...
            if verify_token session token then
              ...
              (* Invalid token *)
              conn |> Conn.respond ~status:Net.Http.Status.Forbidden ~body:"CSRF token invalid" |> Conn.halt
            end

(** {1 View Helpers} *)
(** Get current CSRF token for use in views *)
let get_token = fun conn ->
  let session = Session.get conn in get_or_create_token session

(** Generate HTML hidden field for forms *)
let hidden_field = fun conn ->
  let token = get_token conn in
  let masked = mask_token token in Component.input ~attrs:[ Component.type_ "hidden"; Component.name "_csrf_token"; Component.value masked ] ()

(** Generate HTML meta tag for AJAX *)
let meta_tag = fun conn ->
  let token = get_token conn in
  let masked = mask_token token in Component.meta ~attrs:[ Component.name "csrf-token"; Component.attr "content" masked ] ()
