...
let parse_http_date date_str =
  try
    (* HTTP dates are in format: "Day, DD Mon YYYY HH:MM:SS GMT" *)
    ...
    | [ _day; day; month; year; time; _tz ] ->
        ...
        let days_since_epoch =
          ...
          let years_since_1970 = year - 1970 in
          ...
          let days_in_month = [ 31; if is_leap then 29 else 28; 31; 30; 31; 30; 31; 31; 30; 31; 30; 31 ] in
          
          let days_from_months =
            List.take month days_in_month
            |> List.fold_left (fun acc days -> acc + days) 0
          in
...
let check_etag_match conn resp_headers =
  ...
  | Some client_etag, Some server_etag ->
      ...
      let client_etags = String.split_on_char ',' client_etag
        |> List.map String.trim in
      
      (* Check for wildcard match *)
      if List.mem "*" client_etags then true
      else
...
let check_modified_since conn resp_headers =
  let req_headers = Conn.headers conn in
...
let not_modified_response conn resp_headers =
