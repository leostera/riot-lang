  ...
  try
    ...
    | [ _day; day; month; year; time; _tz ] ->
        ...
        let days_since_epoch =
          ...
          let leap_years = (years_since_1970 / 4) - (years_since_1970 / 100) + (years_since_1970 / 400) in
          ...
          in
          let days_from_months = List.take days_in_month ~len:month |> List.fold_left ~fn:(
            fun acc days -> acc + days
          ) ~init:0 in days_from_years + days_from_months + day - 1
        in
        let total_seconds = (days_since_epoch * 86_400) + (hours * 3_600) + (minutes * 60) + seconds in Some (Float.of_int total_seconds)
    | _ -> None
  with
  | _ -> None
...
let check_etag_match = fun conn resp_headers ->
  ...
  | Some client_etag, Some server_etag ->
      ...
      if List.contains client_etags ~value:"*" then
        true
      else (* Check if server ETag matches any client ETag *)
...
let check_modified_since = fun conn resp_headers ->
  ...
  | Some client_date_str, Some server_date_str -> (
    ...
    | Some client_time, Some server_time -> (* Not modified if server time <= client time *)
...
let not_modified_response = fun conn resp_headers ->
  ...
  let conn' = Conn.with_body "" conn' in (* Preserve cacheable headers from response *)
  List.fold_left cacheable_headers ~init:conn' ~fn:(
...
let middleware = fun ~conn ~next ->
  ...
  match method_ with
  | Net.Http.Method.Get | Net.Http.Method.Head ->
      (* Process request *)
      let conn' = next conn in
      (* Get response headers *)
      let resp_headers = List.fold_left (Conn.resp_headers conn') ~init:Net.Http.Header.empty ~fn:(
        fun headers ((name, value)) -> Net.Http.Header.add headers name value
      ) in
      ...
      conn'
  | _ -> (* Other methods pass through unchanged *)
  next conn
