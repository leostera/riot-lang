...
let secure_compare s1 s2 =
  if not (String.length s1 = String.length s2) then false
  else
  ...
  | [payload; signature] ->
      ...
      if not (verify ~secret ~data:payload ~signature) then
        ...
        | Ok json_str ->
            ...
            | Error err -> 
                Error ("Failed to parse JSON: " ^ Data.Json.error_to_string err)
            | Ok json -> Ok json
