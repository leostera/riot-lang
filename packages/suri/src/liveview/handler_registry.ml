...
type 'msg t = { handlers: (string, string -> 'msg) Std.Collections.HashMap.t; mutable next_id: int }
...
let register = fun t handler ->
  ...
  let _ = Std.Collections.HashMap.insert t.handlers ~key:id ~value:handler in id
