...
let id = fun base_name ->
  ...
  let uuid_hex = Std.UUID.to_string_nodash uuid in base_name ^ "-" ^ uuid_hex
...
let rec attach_handler_ids registry (component: 'msg Component.t): 'msg Component.t =
  ...
  | Component.Fragment children -> Component.Fragment (List.map ~fn:(attach_handler_ids registry) children)
  | Component.El { tag; attrs; children } ->
      (* Process attributes and extract event handlers *)
      let regular_attrs = List.filter_map ~fn:(
        fun attr ->
          match attr with
          | Component.Attr _ -> Some attr
          | Component.Event _ -> None
      ) attrs in
      let event_handlers = List.filter_map ~fn:(
        fun attr ->
          match attr with
      ...
      ) event_handlers |> List.concat in
      (* Process children recursively *)
      let new_children = List.map ~fn:(attach_handler_ids registry) children in Component.El { tag; attrs = regular_attrs @ handler_attrs; children = new_children }

(** Render component with handler IDs attached *)
let render_with_handlers = fun registry component ->
  let component_with_ids = attach_handler_ids registry component in Component.to_html component_with_ids
...
module MountHandler (C: Component) = struct
  ...
  let extract_session_token = fun conn ->
    let params = Middleware.Conn.query_params conn in Std.Collections.Proplist.get params ~key:"session"
  ...
  let init = fun conn ->
    ...
    in
    let component = ComponentProcess.start_link this (module C) conn component_args in `ok { component }
  ...
  let handle_message = fun msg state ->
    ...
    | RenderPatch patch ->
        ...
        let frame = Http.Ws.Frame.text patch in `push ([ frame ], state)
    | _ -> `ok state
end
...
let html_template = fun ~element_id ~ws_path ?title ?styles initial_content ->
  let title_text = Option.unwrap_or title ~default:"LiveView App" in let open Component in
  let head_elements =
    [
  ...
  let head_elements =
    ...
    | None -> head_elements
  in
  let page = html [ head head_elements; body [ div ~attrs:[ id element_id ] [ initial_content ]; script ("const lv = new LiveView('" ^ element_id ^ "', '" ^ ws_path ^ "');\n" ^ "lv.connect();") ] ] in Component.to_html page
...
let serve_runtime ?(prefix = "/assets/liveview.js") (): Middleware.Pipeline.middleware = fun ~conn ~next ->
  ...
  if req_path = prefix then
    conn |> Middleware.Conn.with_status Net.Http.Status.Ok |> Middleware.Conn.with_header "Content-Type" "application/javascript; charset=utf-8" |> Middleware.Conn.with_body javascript_runtime |> Middleware.Conn.send
  else next conn
...
let mount = fun (type s m) ((module C: Component with type state = s and type msg = m)) (conn: Middleware.Conn.t) ->
  ...
  let opts = Channel.Handler.{ do_upgrade = true } in (opts, Channel.Handler.make (module M) conn)
...
let session_token = Session.encode ~secret ~json in Fragment [ div ~attrs:[
  id element_id;
  Attr ("data-lv-session", session_token);
] []; script ("const " ^ js_var_name ^ " = new LiveView('" ^ element_id ^ "', '" ^ ws_path ^ "');\n" ^ js_var_name ^ ".connect();") ]
...
let live = fun (type s m) ((module C: Component with type state = s and type msg = m)) ->
  ...
  let handler conn req =
    ...
    let is_websocket_upgrade =
      ...
      | Some upgrade, Some conn_header when String.lowercase_ascii upgrade = "websocket" && let conn_header = String.lowercase_ascii conn_header in conn_header = "upgrade" || String.contains conn_header "upgrade" -> true
      | _ -> false
    in
    if is_websocket_upgrade then
      begin
        Log.info ("LiveView: Mounting component at " ^ ws_path);
        let (opts, handler) = mount (module C) conn in Middleware.Conn.upgrade_websocket opts handler conn
      end
    else begin
      (* Not a WebSocket upgrade - return error *)
      conn |> Middleware.Conn.with_status Net.Http.Status.BadRequest |> Middleware.Conn.with_body "This endpoint only accepts WebSocket connections" |> Middleware.Conn.send
    end
  in
