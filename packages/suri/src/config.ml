...
let spec = Config.Spec.for_app ~app:"suri" [
  Config.Spec.string "host" 
    ~default:"0.0.0.0"
    ~help:"Server bind address";
  
  Config.Spec.int "port" 
    ~default:4000
    ~help:"Server port number";
  
  Config.Spec.int "acceptors" 
    ~default:System.available_parallelism
    ~help:"Number of acceptor processes";
  
  Config.Spec.int "max_request_line_length" 
    ~default:8192
    ~help:"Maximum HTTP request line length in bytes";
  
  Config.Spec.int "max_header_count" 
    ~default:100
    ~help:"Maximum number of HTTP headers";
  
  Config.Spec.int "max_header_length" 
    ~default:8192
    ~help:"Maximum HTTP header length in bytes";
  
  Config.Spec.int "buffer_size" 
    ~default:4096
    ~help:"Network buffer size in bytes";
  
  Config.Spec.string "liveview_secret" 
