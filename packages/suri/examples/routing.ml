...
let not_found_handler = fun conn req -> conn |> Conn.respond ~status:NotFound ~body:"404 - Page not found" |> Conn.send

(* Define routes *)
let routes = Middleware.Router.[ get "/" home_handler; get "/about" about_handler; get "/api/health" health_handler ]
