...
let routes =
  Middleware.Router.
    [ get "/" root; get "/api/status" api_status; get "/api/info" api_info ]
...
let app =
  Middleware.
    [
      request_id;
      logger;
      (* Serve static files from ./packages/suri/examples/public at /public URL *)
      static ~at:"/public" (Path.v "./packages/suri/examples/public") ();
      (* Enable directory browsing for demo *)
      static
        ~at:"/browse"
...
let () =
  Miniriot.run
    ~main:(fun ~args:_ ->
      ...
      Log.info
        (String.concat ""
           [ "Server: http://localhost:"; string_of_int port; "/" ]);
      Log.info "";
      Log.info "📂 Endpoints:";
      ...
      Log.info
        "  http://localhost:8080/public/assets/style.css → CSS with MIME type";
      ...
      Log.info
        "  http://localhost:8080/public/../../../etc/passwd → 403 Forbidden";
      Log.info
        "  http://localhost:8080/public/.env.example → 403 Forbidden (dotfile)";
      Log.info "";
      Log.info "💾 Caching:";
