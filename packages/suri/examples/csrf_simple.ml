...
let form_page conn =
  ...
  let page = html [
    ...
    body [
      ...
      (* Traditional form with hidden CSRF token *)
      h2 [text "HTML Form"];
      form ~attrs:[method_ "POST"; action "/submit"] [
        Middleware.Csrf.hidden_field conn;
        p [
          label [text "Name: "];
          input ~attrs:[type_ "text"; name "name"] ();
        ];
        button ~attrs:[type_ "submit"] [text "Submit Form"];
      ];
      
      (* AJAX form using meta tag *)
      h2 [text "AJAX Form"];
      p [
...
let submit_handler ~conn ~next:_ =
  ...
  let page = html [
    body [
      ...
      p [
        a ~attrs:[href "/"] [text "Back to form"];
      ];
    ];
  ] in
