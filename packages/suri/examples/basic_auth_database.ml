...
module DB = struct
  ...
  let users = [
    { id = 1; username = "alice"; password_hash = "hash_of_pass1"; role = "admin"; email = "alice@example.com" };
    { id = 2; username = "bob"; password_hash = "hash_of_pass2"; role = "user"; email = "bob@example.com" };
    { id = 3; username = "charlie"; password_hash = "hash_of_pass3"; role = "user"; email = "charlie@example.com" };
  ]
  
  let find_user username =
    List.find_opt (fun u -> String.equal u.username username) users
...
let dashboard_handler ~conn ~next:_ =
  ...
  | Some user ->
      let html = String.concat "" [
        ...
        "<h3>User Info</h3>";
        "<p><strong>ID:</strong> "; string_of_int user.DB.id; "</p>";
        "<p><strong>Username:</strong> "; user.DB.username; "</p>";
        "<p><strong>Email:</strong> "; user.DB.email; "</p>";
        "<p><strong>Role:</strong> "; user.DB.role; "</p>";
        "</div>";
  ...
  | Some user ->
      ...
      if String.equal user.DB.role "admin" then
        let html = String.concat "" [
          ...
          "<p>Welcome, <strong>"; user.DB.username; "</strong></p>";
        ...
        let html = String.concat "" [
          ...
          "<p>Your role: <strong>"; user.DB.role; "</strong></p>";
...
let () =
  Miniriot.run ~args:Env.args () ~main:(fun ~args:_ ->
    let app = Middleware.[
      request_id;
      logger;
      (* Custom validation for all protected routes *)
    ...
    let config = Suri.config ~port:3003 () in
