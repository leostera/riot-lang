...
(* In-memory database *)
let users = [ { id = 1; name = "Alice"; email = "alice@example.com" }; { id = 2; name = "Bob"; email = "bob@example.com" }; { id = 3; name = "Charlie"; email = "charlie@example.com" } ]
...
let users_to_json = fun users ->
  let user_jsons = List.map ~fn:user_to_json users in Data.Json.array user_jsons

(* CORS middleware *)
let cors_middleware = fun ~conn ~next ->
  let conn' = next conn in Conn.with_header "Access-Control-Allow-Origin" "*" conn'
...
let api_info_handler = fun conn req ->
  ...
  in
  conn |> Conn.with_status Ok |> Conn.with_header "Content-Type" "application/json" |> Conn.with_body (Data.Json.to_string info) |> Conn.send

let users_list_handler = fun conn req ->
  let json = users_to_json users in conn |> Conn.with_status Ok |> Conn.with_header "Content-Type" "application/json" |> Conn.with_body (Data.Json.to_string json) |> Conn.send
...
let user_handler = fun conn req ->
  ...
  | Some id_str -> (
    try
      ...
      match List.find users ~fn:(
        fun u -> u.id = id
      ) with
      | Some user ->
          let json = user_to_json user in conn |> Conn.with_status Ok |> Conn.with_header "Content-Type" "application/json" |> Conn.with_body (Data.Json.to_string json) |> Conn.send
      | None ->
          let error =
            Data.Json.obj
              [
                "error", Data.Json.string "User not found";
              ]
          in
    ...
    | Failure _ ->
        let error =
          Data.Json.obj
            [
              "error", Data.Json.string "Invalid user ID";
            ]
        in
  ...
  | None ->
      let error =
        Data.Json.obj
          [
            "error", Data.Json.string "Missing user ID";
          ]
      in
      conn |> Conn.with_status BadRequest |> Conn.with_header "Content-Type" "application/json" |> Conn.with_body (Data.Json.to_string error) |> Conn.send
...
let not_found_handler = fun conn req ->
  let error =
    Data.Json.obj
      [
        "error", Data.Json.string "Endpoint not found";
      ]
  in
  conn |> Conn.with_status NotFound |> Conn.with_header "Content-Type" "application/json" |> Conn.with_body (Data.Json.to_string error) |> Conn.send

(* Define routes *)
let routes = Middleware.Router.[ get "/" api_info_handler; get "/api/users" users_list_handler; get "/api/users/:id" user_handler ]
