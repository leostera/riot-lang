...
let payload_for_kind = function
  | Chunk_payload ->
      ...
      for _ = 1 to repeat_count do Buffer.add_string buffer chunk done;
      let data = Buffer.contents buffer in { data; expected_bytes = String.length data; expected_lines = 0 }
  | Char_payload ->
  ...
  | Line_payload ->
      ...
      for index = 1 to line_count do Buffer.add_string buffer (format Std.Format.[
        str "line-";
        str (Int.to_string index);
        str ": repeated payload for stdin benchmark";
        str "\n";
      ]) done;
      let data = Buffer.contents buffer in { data; expected_bytes = String.length data; expected_lines = line_count }

let payload_for_mode = function
  | Kernel_chunks | Std_chunks | Std_reader_chunks | Std_read_to_string -> payload_for_kind Chunk_payload
  | Kernel_chars | Std_chars | Std_buffered_chars -> payload_for_kind Char_payload
  ...
  let rec loop () =
    ...
    | None ->
        ...
        protect ~finally:(
          ...
          (
            fun () ->
              let* _ = lift_async (Kernel.Async.Poll.poll ~timeout:1_000_000L poll) in loop ()
          )
  in
  loop ()
...
let run_worker_process = fun mode payload ->
  ...
  let args =
    ...
    |]
  in
  let* process = lift_process (Kernel.Process.spawn ~program:(Path.to_string executable_path) ~args ~stdio ())
  in
  protect ~finally:(
    fun () ->
      ...
      fun () ->
        ...
        | (Some stdin_file, Some stderr_file) ->
            with_poll
              (
                fun poll ->
                  let* () = write_all poll ~token:(Kernel.Async.Token.make ("stdin-bench-write", mode_to_string mode)) stdin_file (Kernel.Bytes.from_string payload.data)
                  in
                  let* () = lift_file (Kernel.Fs.File.close stdin_file)
                  in
                  let* status = wait_for_exit poll ~token:(Kernel.Async.Token.make ("stdin-bench-exit", mode_to_string mode)) process
                  in
                  let* stderr = read_all poll ~token:(Kernel.Async.Token.make ("stdin-bench-stderr", mode_to_string mode)) stderr_file
                  in
                  match status with
...
let parse_worker_args = function
  | mode :: "--expected-bytes" :: expected_bytes :: "--expected-lines" :: expected_lines :: [] -> mode_of_string mode, Int.of_string expected_bytes, Int.of_string expected_lines
  | args -> panic ("stdin bench worker expected: <mode> --expected-bytes N --expected-lines N, got " ^ String.concat " " args)

let run_case = fun mode () ->
  let payload = payload_for_mode mode in expect_ok (run_worker_process mode payload)

let benchmarks = Bench.[
  with_config ~config:{ iterations = 8; warmup = 2 } "stdin kernel read chunks: 1 MiB" (run_case Kernel_chunks);
  with_config ~config:{ iterations = 8; warmup = 2 } "stdin std read chunks: 1 MiB" (run_case Std_chunks);
  with_config ~config:{ iterations = 8; warmup = 2 } "stdin std reader chunks: 1 MiB" (run_case Std_reader_chunks);
  with_config ~config:{ iterations = 8; warmup = 2 } "stdin std read_to_string: 1 MiB" (run_case Std_read_to_string);
  with_config ~config:{ iterations = 3; warmup = 1 } "stdin kernel read chars: 48 KiB" (run_case Kernel_chars);
  with_config ~config:{ iterations = 3; warmup = 1 } "stdin std read chars: 48 KiB" (run_case Std_chars);
  with_config ~config:{ iterations = 3; warmup = 1 } "stdin std buffered read chars: 48 KiB" (run_case Std_buffered_chars);
  with_config ~config:{ iterations = 3; warmup = 1 } "stdin std read_line: 5k piped lines" (run_case Std_lines);
]
