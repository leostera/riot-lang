...
let bench_iter = fun count () ->
  let vector = build_vector count in Vector.for_each vector ~fn:(
    fun _ -> ()
  )

let bench_append_growing_dst = fun count () ->
  let left = build_vector count in
  let right = build_vector count in Vector.append left right

let bench_append_preallocated_dst = fun count () ->
  let left = build_vector_with_capacity (count * 2) count in
  let right = build_vector_with_capacity count count in Vector.append left right

let benchmarks = Bench.[
  with_config ~config:{ iterations = 10; warmup = 2 } "push growing: 10k items" (bench_push_growing 10_000);
  with_config ~config:{ iterations = 10; warmup = 2 } "push preallocated: 10k items" (bench_push_preallocated 10_000);
  with_config ~config:{ iterations = 5; warmup = 1 } "push growing: 100k items" (bench_push_growing 100_000);
  with_config ~config:{ iterations = 5; warmup = 1 } "push preallocated: 100k items" (bench_push_preallocated 100_000);
  with_config ~config:{ iterations = 3; warmup = 1 } "push growing: 1M items" (bench_push_growing 1_000_000);
  with_config ~config:{ iterations = 3; warmup = 1 } "push preallocated: 1M items" (bench_push_preallocated 1_000_000);
  ...
  with_config ~config:{ iterations = 10; warmup = 2 } "append growing dst: 10k + 10k" (bench_append_growing_dst 10_000);
  with_config ~config:{ iterations = 10; warmup = 2 } "append preallocated dst: 10k + 10k" (bench_append_preallocated_dst 10_000);
  with_config ~config:{ iterations = 5; warmup = 1 } "append growing dst: 100k + 100k" (bench_append_growing_dst 100_000);
  with_config ~config:{ iterations = 5; warmup = 1 } "append preallocated dst: 100k + 100k" (bench_append_preallocated_dst 100_000);
]
