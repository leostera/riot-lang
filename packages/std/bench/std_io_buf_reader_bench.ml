...
let make_lines = fun ~count ~line_len ~char -> List.init ~count ~fn:(
  fun _ -> String.make ~len:line_len ~char ^ "\n"
) |> String.concat ""

let make_runes = fun ~count ->
  let unit = "A" ^ "\xC3\xA9" ^ "\xF0\x9F\x98\x80" in List.init ~count ~fn:(
    fun _ -> unit
  ) |> String.concat ""

type fixture = { name: string; payload: string; chunk_size: int }

let fixtures = [ { name = "1024 x 16B lines"; payload = make_lines ~count:1_024 ~line_len:16 ~char:'a'; chunk_size = 64 }; { name = "256 x 256B lines"; payload = make_lines ~count:256 ~line_len:256 ~char:'b'; chunk_size = 512 }; { name = "64 x 1KiB lines"; payload = make_lines ~count:64 ~line_len:1_024 ~char:'c'; chunk_size = 2_048 } ]
...
let consume_string_lines = fun payload ~chunk_size ->
  ...
  let rec loop () =
    ...
    | Error _ -> panic "std io buffered reader bench: string line read failed"
  in
  loop ()
...
let consume_slice_lines = fun payload ~chunk_size ~materialize ->
  ...
  let rec loop () =
    ...
    | Ok line ->
        ...
        else total := !total + IO.IoSlice.length line;
        loop ()
    | Error IO.End_of_file -> !total
    | Error _ -> panic "std io buffered reader bench: slice line read failed"
  in
  loop ()
...
let consume_read_slices = fun payload ~chunk_size ->
  ...
  let rec loop () =
    ...
    | Ok slice ->
        ...
        loop ()
    | Error IO.End_of_file -> !total
    | Error _ -> panic "std io buf reader bench: read_slice failed"
  in
  loop ()
...
let consume_read_runes = fun payload ~chunk_size ->
  ...
  let rec loop () =
    ...
    | Ok rune ->
        ...
        loop ()
    | Error IO.End_of_file -> !total
    | Error _ -> panic "std io buf reader bench: read_rune failed"
  in
  loop ()
...
let consume_read_bytes = fun payload ~chunk_size ->
  ...
  let rec loop () =
    ...
    | Ok byte ->
        ...
        loop ()
    | Error IO.End_of_file -> !total
    | Error _ -> panic "std io buf reader bench: read_byte failed"
  in
  loop ()
...
let consume_peek_consume_bytes = fun payload ~chunk_size ->
  ...
  let rec loop () =
    ...
    | Error IO.End_of_file -> !total
    | Error _ -> panic "std io buf reader bench: peek failed"
  in
  loop ()
...
let consume_buffered_consume_bytes = fun payload ~chunk_size ->
  ...
  let rec loop () =
    ...
    | Error IO.End_of_file -> !total
    | Error _ -> panic "std io buf reader bench: buffered failed"
  in
  loop ()
...
let benchmarks =
  let line_benchmarks = List.map fixtures ~fn:(
    fun fixture ->
      Bench.compare_with_config ~config:(config_for fixture) ("std io buf reader lines: " ^ fixture.name)
        [
