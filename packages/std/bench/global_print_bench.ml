...
let bench_copy_write = fun message () ->
  let bytes = Kernel.Bytes.from_string message in write_all bytes ~len:(String.length message)

let bench_zero_copy_write = fun message () ->
  let bytes = bytes_unsafe_of_string message in write_all bytes ~len:(String.length message)

let bench_zero_copy_write_raw_int = fun message () ->
  let bytes = bytes_unsafe_of_string message in write_all_raw_int bytes ~len:(String.length message)

let bench_zero_copy_write_raw_native = fun message () ->
  let bytes = bytes_unsafe_of_string message in write_all_raw_native bytes ~len:(String.length message)
...
let benchmarks = Bench.[
  ...
  with_config ~config "zero-copy write raw-native: medium" (bench_zero_copy_write_raw_native medium_message);
