...
let bench_from_string_array = fun payload () ->
  let _ = IO.Iovec.from_string_array payload in
  ()

let bench_from_bytes_array = fun payload () ->
  let _ = IO.Iovec.from_bytes_array payload in
  ()
...
let bench_sub_middle = fun ~segment_size iovec () ->
  ...
  let _ = IO.Iovec.sub ~pos:(segment_size / 2) ~len:(total / 2) iovec in
  ()

let bench_for_each = fun iovec () ->
  IO.Iovec.for_each iovec ~fn:(fun _ -> ())
...
let small_iovec = IO.Iovec.from_string_array small_string_segments
...
let medium_iovec = IO.Iovec.from_string_array medium_string_segments
...
let large_iovec = IO.Iovec.from_string_array large_string_segments
...
let () =
  Runtime.run
    ~main:(fun ~args -> Bench.Cli.main ~name:"std_io_iovec_bench" ~benchmarks ~args)
    ~args:Env.args
    ()
