...
type fixture = { name: string; payload: string; slice: IO.IoVec.IoSlice.t }

let make_numeric_array = fun count -> "[" ^ (List.init ~count ~fn:Int.to_string |> String.concat ",") ^ "]"

let make_string_payload = fun size -> {|{"message":"|} ^ String.make ~len:size ~char:'a' ^ "\"}"

let make_fixture = fun name payload ->
  let slice = IO.IoVec.IoSlice.from_string payload |> Result.expect ~msg:"failed to create io slice" in { name; payload; slice }
...
let bench_json_stream_string = fun payload ->
  ...
  | Ok _ -> ()
  | Error error -> panic ("JsonStream.from_string failed during benchmark: " ^ JsonStream.error_to_string error)

let bench_json_stream_slice = fun slice ->
  match JsonStream.from_slice slice with
  | Ok _ -> ()
  | Error error -> panic ("JsonStream.from_slice failed during benchmark: " ^ JsonStream.error_to_string error)
...
let benchmarks = List.map fixtures ~fn:(
  fun fixture ->
    Bench.compare_with_config ~config:(config_for fixture) ("json parse: " ^ fixture.name)
      [
        Bench.make_case "Json.of_string"
          (
            fun () -> bench_json fixture.payload
          );
        Bench.make_case "JsonStream.from_string"
          (
            fun () -> bench_json_stream_string fixture.payload
          );
        Bench.make_case "JsonStream.from_slice"
          (
            fun () -> bench_json_stream_slice fixture.slice
          );
      ]
)
