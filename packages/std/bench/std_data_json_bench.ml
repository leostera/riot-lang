...
let make_numeric_array = fun count ->
  "[" ^ (List.init ~count ~fn:Int.to_string |> String.concat ",") ^ "]"
...
let bench_json_stream_string = fun payload ->
  ...
  | Ok _ -> ()
  | Error error -> panic
    ("JsonStream.from_string failed during benchmark: " ^ JsonStream.error_to_string error)

let bench_json_stream_slice = fun slice ->
  match JsonStream.from_slice slice with
  | Ok _ -> ()
  | Error error -> panic
    ("JsonStream.from_slice failed during benchmark: " ^ JsonStream.error_to_string error)
