...
type origin_form_uri = { path: string; query: string option; fragment: string option }
  ...
  if Slice.length value - at != suffix_len then
    ...
    let rec loop index =
      if index >= suffix_len then
        ...
        if Slice.get_unchecked value ~at:(at + index) = String.get_unchecked suffix ~at:index then
          loop (index + 1)
        else false
    in
    loop 0
...
let optimized_method_from_slice = fun value ->
  ...
  | 6 ->
      if Slice.get_unchecked value ~at:0 = 'D' && equal_tail value ~at:1 "ELETE" then
        Method.Delete
      else Method.Extension (Slice.to_string value)
  | 7 -> (
    match Slice.get_unchecked value ~at:0 with
...
let optimized_version_from_slice = fun value ->
  ...
  | 6 ->
      if equal_tail value ~at:0 "HTTP/2" then
        ...
        if equal_tail value ~at:0 "HTTP/3" then
          Ok Version.Http3
  ...
  | 8 ->
      if equal_tail value ~at:0 "HTTP/0.9" then
        ...
        if equal_tail value ~at:0 "HTTP/1.0" then
          Ok Version.Http10
        else
          if equal_tail value ~at:0 "HTTP/1.1" then
            Ok Version.Http11
          else
            if equal_tail value ~at:0 "HTTP/2.0" then
              Ok Version.Http2
            else
              if equal_tail value ~at:0 "HTTP/3.0" then
                Ok Version.Http3
              else Error `InvalidVersion
  | _ -> Error `InvalidVersion
...
let optimized_origin_form_uri_from_slice = fun value ->
  ...
  if len > 65_535 then
    ...
    let fragment =
      ...
      | None -> None
      | Some f ->
          let fragment_start = f + 1 in Some (Slice.to_string (Slice.sub_unchecked value ~off:fragment_start ~len:(len - fragment_start)))
    in
...
let build_slice = fun value ->
  ...
  | Ok slice -> slice
  | Error error -> panic ("std io ioslice bench: failed to build slice: " ^ Kernel.IO.Error.message error)
...
let mixed_slices = Array.init ~count:100_000 ~fn:(
  fun index -> build_slice (Array.get_unchecked standard_methods ~at:(index mod Array.length standard_methods))
)

let mixed_version_slices = Array.init ~count:100_000 ~fn:(
  fun index -> build_slice (Array.get_unchecked standard_versions ~at:(index mod Array.length standard_versions))
)

let mixed_request_line_slices = Array.init ~count:100_000 ~fn:(
  fun index -> build_slice (Array.get_unchecked request_lines ~at:(index mod Array.length request_lines))
)

let mixed_header_line_slices = Array.init ~count:100_000 ~fn:(
  fun index -> build_slice (Array.get_unchecked header_lines ~at:(index mod Array.length header_lines))
)

let mixed_origin_form_slices = Array.init ~count:100_000 ~fn:(
  fun index -> build_slice (Array.get_unchecked origin_form_targets ~at:(index mod Array.length origin_form_targets))
)
...
let benchmarks = Bench.[
  compare_with_config ~config:{ iterations = 100; warmup = 10 } "std io ioslice pattern match on slices: mixed http methods x 100000" [ make_case "current from_slice shape" bench_current; make_case "optimized first-byte dispatch" bench_optimized ];
  compare_with_config ~config:{ iterations = 100; warmup = 10 } "std io ioslice pattern match on slices: mixed http versions x 100000" [ make_case "current from_slice shape" bench_version_current; make_case "optimized first-byte dispatch" bench_version_optimized ];
  compare_with_config ~config:{ iterations = 100; warmup = 10 } "std io ioslice delimiter scan: request line to first space x 100000" [ make_case "cursor take_until_char" bench_request_line_scan_current; make_case "slice index_char+sub" bench_request_line_scan_optimized ];
  compare_with_config ~config:{ iterations = 100; warmup = 10 } "std io ioslice delimiter scan: header line to colon x 100000" [ make_case "cursor take_until_char" bench_header_line_scan_current; make_case "slice index_char+sub" bench_header_line_scan_optimized ];
  compare_with_config ~config:{ iterations = 100; warmup = 10 } "std io ioslice uri parse: origin-form targets x 100000" [ make_case "Uri.from_slice" bench_uri_current; make_case "optimized origin-form fast path" bench_uri_optimized ];
]
