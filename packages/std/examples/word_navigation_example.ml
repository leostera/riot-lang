...
let () =
  ...
  println
    ("   Words: ["
    ^ String.concat "; " (List.map words ~fn:(fun w -> "\"" ^ w ^ "\""))
    ^ "]\n");
  ...
  println
    ("   Words: ["
    ^ String.concat "; " (List.map words2 ~fn:(fun w -> "\"" ^ w ^ "\""))
    ^ "]\n");
  ...
  println
    ("   Words: ["
    ^ String.concat "; " (List.map words3 ~fn:(fun w -> "\"" ^ w ^ "\""))
    ^ "]\n");
  ...
  println
    ("   Words: ["
    ^ String.concat "; " (List.map words4 ~fn:(fun w -> "\"" ^ w ^ "\""))
    ^ "]\n");
  ...
  println
    ("   Words: ["
    ^ String.concat "; " (List.map words5 ~fn:(fun w -> "\"" ^ w ^ "\""))
    ^ "]");
  ...
  println
    ("   Words: ["
    ^ String.concat "; " (List.map words6 ~fn:(fun w -> "\"" ^ w ^ "\""))
    ^ "]\n");
  ...
  let rec show_navigation pos =
    if pos >= String.length text then
      ...
      let word_part =
        String.sub text ~offset:pos ~len:(min (next - pos) (String.length text - pos))
      in
      let pos_str =
        if pos < 10 then
          " " ^ Int.to_string pos
