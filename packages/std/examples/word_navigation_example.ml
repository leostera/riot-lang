...
let main ~args:_ =
  ...
  println ("   Word boundaries at byte positions: [" ^ String.concat "; " (List.map boundaries ~fn:Int.to_string) ^ "]");
  let words = String.split_words s1 in
  println ("   Words: [" ^ String.concat "; " (List.map words ~fn:(
    fun w -> "\"" ^ w ^ "\""
  )) ^ "]\n");
  ...
  println ("   Words: [" ^ String.concat "; " (List.map words2 ~fn:(
    fun w -> "\"" ^ w ^ "\""
  )) ^ "]\n");
  ...
  println ("   Words: [" ^ String.concat "; " (List.map words3 ~fn:(
    fun w -> "\"" ^ w ^ "\""
  )) ^ "]\n");
  ...
  println ("   Words: [" ^ String.concat "; " (List.map words4 ~fn:(
    fun w -> "\"" ^ w ^ "\""
  )) ^ "]\n");
  ...
  println ("   Words: [" ^ String.concat "; " (List.map words5 ~fn:(
    fun w -> "\"" ^ w ^ "\""
  )) ^ "]");
  ...
  println ("   Words: [" ^ String.concat "; " (List.map words6 ~fn:(
    fun w -> "\"" ^ w ^ "\""
  )) ^ "]\n");
  ...
  let rec show_navigation pos =
    if pos >= String.length text then
      ...
      let pos_str =
        if pos < 10 then
          " " ^ Int.to_string pos
        else Int.to_string pos
      in
      let next_str =
        if next < 10 then
          " " ^ Int.to_string next
        else Int.to_string next
      in
