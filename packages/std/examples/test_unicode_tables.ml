...
let () =
  ...
  let alpha_hex = 
    ...
    let rec to_hex n acc =
      if n = 0 then if acc = "" then "0000" else String.make (4 - String.length acc) '0' ^ acc
  ...
  let arabic_hex = 
    ...
    in to_hex arabic_int ""
  in
  println ("Arabic digit ٥ (U+" ^ arabic_hex ^ "):");
  println ("  is_digit: " ^ Bool.to_string (Unicode.Rune.is_digit arabic_5) ^ " (should be true)");
  println "";
