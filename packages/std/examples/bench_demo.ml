...
let bench_vector_push = fun () ->
  let v = Vector.create () in Vector.push v ~value:42

let bench_vector_100_pushes = fun () ->
  let v = Vector.create () in
  for i = 1 to 100 do Vector.push v ~value:i done
...
let bench_array_set_100 = fun () ->
  ...
  for i = 0 to 99 do Array.set arr ~at:i ~value:i done

let bench_vector_push_100 = fun () ->
  let v = Vector.create () in
  for i = 0 to 99 do Vector.push v ~value:i done
...
let benchmarks = Bench.[
  ...
  compare "insert 100 sequential elements" [ make_case "Array.set" bench_array_set_100; make_case "Vector.push" bench_vector_push_100 ];
]
