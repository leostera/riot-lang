...
let () =
  ...
  (match Unicode.Utf8.decode_rune s 0 with
   | Some (r, next_pos) ->
       ...
       let hex_str =
         ...
         Bytes.to_string s
       in
       println ("   First rune of '" ^ s ^ "': U+" ^ hex_str ^ " at byte position " ^ string_of_int next_pos)
  ...
  println ("   Truncated to width 10: '" ^ truncated ^ "' (width: " ^ string_of_int (String.width truncated) ^ ")");
  println "";
