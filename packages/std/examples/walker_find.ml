...
type config = { count_only: bool; sort: bool; roots: Path.t list }
...
let should_ignore = fun entry -> Glob.matches ignored_globs ~str:(Fs.Walker.FileItem.path_string entry) |> Result.unwrap_or ~default:false
...
let main ~args =
  ...
  | Help ->
      ...
      Ok ()
  | Config config ->
      let walker = Fs.Walker.create ~roots:config.roots ~sort:config.sort () |> Result.expect ~msg:"walker_find configuration should be valid" |> Fs.Walker.filter_entry ~f:(
        fun entry -> not (should_ignore entry)
      ) in
      ...
      let rec loop iter =
        ...
        | Some (Error (err: Fs.Walker.error)), iter' ->
            eprintln ("walker error at depth " ^ Int.to_string err.depth ^ ": " ^ IO.error_message err.cause);
            loop iter'
