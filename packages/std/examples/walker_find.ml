...
let ignored_globs =
  Glob.create ignored_paths
  |> Result.expect ~msg:"walker_find ignore globs should compile"
  ...
  | Config config ->
      ...
      let rec loop iter =
        ...
        | (Some (Error (err: Fs.Walker.error)), iter') ->
            eprintln
              ("walker error at depth "
              ^ Int.to_string err.depth
              ^ ": "
              ^ IO.error_message err.cause);
            loop iter'
