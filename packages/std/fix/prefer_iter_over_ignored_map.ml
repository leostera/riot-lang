...
let diagnostic_for_expression = fun expr ->
    ...
    if not (path_matches ~expected:"ignore" head) then
      ...
      | [ mapped ] -> (
          ...
          | [_fn;_collection] when path_matches ~expected:"List.map" mapped_head -> Some (make_diagnostic
            ~iter_name:"List.iter"
            expr)
          | [_fn;_collection] when path_matches ~expected:"Iter.map" mapped_head -> Some (make_diagnostic
            ~iter_name:"Iter.iter"
            expr)
          | _ -> None
        )
