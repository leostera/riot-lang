...
let explanation = Api.Explanation.{ rule_id = package_rule_id; message = "Ignoring the result of List.map or Iter.map should usually use the corresponding iter form instead."; body = {|
...
let make_diagnostic = fun ~iter_name expr -> Api.Diagnostic.make ~severity:Warning ~kind:(Api.Diagnostic.Known { rule_id = package_rule_id; message = explanation.Api.Explanation.message }) ~span:(Syn.Ceibo.Red.SyntaxNode.span (Syn.Cst.Expression.syntax_node expr)) ~suggestion:("Use " ^ iter_name ^ " when the mapped result is ignored and the traversal exists only for side effects.") ()
...
let diagnostic_for_expression = fun expr ->
  ...
  if not (path_matches ~expected:"ignore" head) then
    ...
    | [ mapped ] -> (
      ...
      | [ _fn; _collection ] when path_matches ~expected:"List.map" mapped_head -> Some (make_diagnostic ~iter_name:"List.iter" expr)
      | [ _fn; _collection ] when path_matches ~expected:"Iter.map" mapped_head -> Some (make_diagnostic ~iter_name:"Iter.iter" expr)
      | _ -> None
    )
    | _ -> None
