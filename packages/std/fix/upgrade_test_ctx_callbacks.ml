...
let callback_argument = fun expr ->
  ...
  | None -> (
      match unwrap_expression expr with
      | Syn.Cst.Expression.Infix infix when String.equal
        (Syn.Cst.InfixExpression.operator infix)
        "@@" ->
    ...
    |> List.filter_map
      (fun binding ->
        ...
        | Some name_token ->
            ...
            let callback_uses =
              HashMap.get named_callback_counts name
              |> Option.unwrap_or ~default:0
            in
            let total_uses =
              HashMap.get path_use_counts name
              |> Option.unwrap_or ~default:0
            in
