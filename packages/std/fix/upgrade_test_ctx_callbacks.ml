...
let explanation = Api.Explanation.{ rule_id = package_rule_id; message = "Std.Test callbacks should accept a ctx argument instead of unit."; body = {|
...
let is_test_source_path = fun path ->
  let basename =
    ...
    | [] -> path
  in
  (String.contains path "/tests/" || String.starts_with ~prefix:"tests/" path) && (String.ends_with ~suffix:"_tests.ml" basename || String.ends_with ~suffix:"_test.ml" basename || String.starts_with ~prefix:"test_" basename)
...
let rec expression_name = fun expr ->
  ...
  | Syn.Cst.Expression.Path { path; _ } ->
      Syn.Cst.Ident.segments path |> List.map Syn.Cst.Token.text |> String.concat "." |> fun name -> Some name
...
let simple_path_name = fun expr ->
  match unwrap_expression expr with
  | Syn.Cst.Expression.Path { path; _ } when List.length (Syn.Cst.Ident.segments path) = 1 -> Syn.Cst.Ident.name path
  | _ -> None

let rec flatten_apply = fun expr ->
  match unwrap_expression expr with
  | Syn.Cst.Expression.Apply { callee; argument; _ } ->
      let head, arguments = flatten_apply callee in (head, arguments @ [ argument ])
...
let is_test_callback_callee = function
  | Some "case" | Some "property" | Some "skip" | Some "Test.case" | Some "Test.property" | Some "Test.skip" | Some "Std.Test.case" | Some "Std.Test.property" | Some "Std.Test.skip" -> true
  | _ -> false
...
let is_partial_test_invocation = fun expr ->
  ...
  if not (is_test_callback_callee (expression_name head)) then
    false
  else Int.equal (List.length (positional_arguments arguments)) 1
...
let is_unit_parameter = fun parameter ->
  ...
  | Syn.Cst.Parameter.Labeled _ | Syn.Cst.Parameter.Optional _ | Syn.Cst.Parameter.LocallyAbstract _ -> false

let parameter_fix = fun parameter ->
  let target = Syn.Cst.Parameter.syntax_node parameter in Api.Fix.make ~title:"Replace unit test callback parameter with _ctx" ~operations:[ Api.Fix.replace_node_with_text ~target ~text:" _ctx" ]

let make_diagnostic = fun parameter -> Api.Diagnostic.make ~severity:Warning ~kind:(Api.Diagnostic.Known { rule_id = package_rule_id; message = explanation.Api.Explanation.message }) ~span:(Syn.Ceibo.Red.SyntaxNode.span (Syn.Cst.Parameter.syntax_node parameter)) ~suggestion:"Change the callback parameter from () to _ctx." ~fix:(parameter_fix parameter) ()
...
let callback_reference = fun expr ->
  ...
  | Some callback ->
      simple_path_name callback |> Option.map
        (
          fun name -> Named name
        )
  | None -> None
...
let check_tree = fun (ctx: Api.Rule.context) _red_root ->
  ...
  let named_diagnostics =
    Riot_fix.Rule_query.let_bindings ctx |> List.filter_map
      (
        fun binding ->
          ...
          | Some name_token ->
              ...
              let callback_uses = HashMap.get named_callback_counts name |> Option.unwrap_or ~default:0 in
              ...
              if callback_uses = 0 || total_uses != callback_uses then
                None
              else binding_parameter binding |> Option.map make_diagnostic
      )
  in
  inline_diagnostics @ named_diagnostics

let rule = fun () -> Api.Rule.make ~id:package_rule_id ~description:explanation.message ~explain:explanation.body ~run:check_tree ()
