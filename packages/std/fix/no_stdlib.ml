...
let make_fix = fun token replacement -> Api.Fix.make ~title:("Replace " ^ Syn.Ceibo.Red.SyntaxToken.text token ^ " with " ^ replacement) ~operations:[ Api.Fix.replace_token_with_text ~target:token ~text:replacement ]
...
let make_diagnostic = fun token ->
  ...
  let kind = Api.Diagnostic.Known { rule_id = package_rule_id; message = make_message text } in Api.Diagnostic.make ~severity:Warning ~kind ~span:(Syn.Ceibo.Red.SyntaxToken.span token) ?suggestion ?fix ()
...
let check_tree = fun (_ctx: Api.Rule.context) red_root -> let open Syn.Ceibo.Red in
...
let diagnostic_for_open_stmt node =
  let non_trivia_children =
    SyntaxNode.children node |> List.filter
      (
        function
