...
let explanation = Api.Explanation.{ rule_id = package_rule_id; message = "Manual result matches that only rebuild Ok/Error should use Result.map."; body = {|
...
let rec unwrap_expression = fun expr ->
  ...
  | Syn.Cst.Expression.TypeAscription { expression; _ } | Syn.Cst.Expression.Polymorphic { expression; _ } -> unwrap_expression expression
  | _ -> expr
...
let matches_result_map_shape = fun (expr: Syn.Cst.match_expression) ->
  ...
  | [ first_case; second_case ] -> (
    ...
    | `OkCase _ok_name, `ErrorCase error_name -> is_ok_expression first_case.body && is_constructor_with_path_name "Error" error_name second_case.body
    | `ErrorCase error_name, `OkCase _ok_name -> is_constructor_with_path_name "Error" error_name first_case.body && is_ok_expression second_case.body
    | _ -> false
  )
  | _ -> false

let make_diagnostic = fun (expr: Syn.Cst.match_expression) -> Api.Diagnostic.make ~severity:Warning ~kind:(Api.Diagnostic.Known { rule_id = package_rule_id; message = explanation.Api.Explanation.message }) ~span:(Syn.Ceibo.Red.SyntaxNode.span expr.syntax_node) ~suggestion:"Prefer Result.map when the Ok branch changes and Error is forwarded unchanged." ()
