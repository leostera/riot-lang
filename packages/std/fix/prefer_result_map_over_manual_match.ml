...
let is_ok_expression = fun expr ->
  match H.constructor_payload ~name:"Ok" expr with
  | Some _ -> true
  | None -> false
...
let diagnostic_for_expression = fun expr ->
  ...
  | Match _ when matches_result_map_shape expr -> Some (make_diagnostic expr)
  | _ -> None
