...
let explanation = Api.Explanation.{ rule_id = package_rule_id; message = "List.rev (List.rev xs) cancels itself out and should be simplified."; body = {|
...
let make_fix = fun ~outer ~replacement -> Api.Fix.make ~title:"Replace List.rev (List.rev xs) with xs" ~operations:[ Api.Fix.replace_node ~target:(Syn.Cst.Expression.syntax_node outer) ~replacement:(Syn.Cst.Expression.syntax_node replacement) ]

let make_diagnostic = fun ~outer ~replacement -> Api.Diagnostic.make ~severity:Warning ~kind:(Api.Diagnostic.Known { rule_id = package_rule_id; message = explanation.Api.Explanation.message }) ~span:(Syn.Ceibo.Red.SyntaxNode.span (Syn.Cst.Expression.syntax_node outer)) ~suggestion:"Replace List.rev (List.rev xs) with xs or keep only one rev if order matters." ~fix:(make_fix ~outer ~replacement) ()
...
let rule = fun () -> Api.Rule.make ~id:package_rule_id ~description:"Double List.rev calls should be simplified" ~explain:explanation.body ~run:check_tree ()
