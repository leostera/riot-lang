...
let explanation = Api.Explanation.{ rule_id = package_rule_id; message = "List emptiness checks should use List.is_empty instead of comparing List.length to zero."; body = {|
...
let is_zero_literal = fun expr ->
  match unwrap_expression expr with
  | Syn.Cst.Expression.Literal (Syn.Cst.Literal.Int { digits; base; _ }) -> base = Syn.Cst.Decimal && String.equal digits "0"
  | _ -> false
...
let make_diagnostic = fun ~suggestion expr -> Api.Diagnostic.make ~severity:Warning ~kind:(Api.Diagnostic.Known { rule_id = package_rule_id; message = explanation.Api.Explanation.message }) ~span:(Syn.Ceibo.Red.SyntaxNode.span (Syn.Cst.Expression.syntax_node expr)) ~suggestion ()
...
let diagnostic_for_expression = fun expr ->
  ...
  | Syn.Cst.Expression.Infix infix -> (
    ...
    | "==", Some _list_expr, _ when is_zero_literal right -> Some (make_diagnostic ~suggestion:"Use List.is_empty xs for this emptiness check." expr)
    | ">", Some _list_expr, _ when is_zero_literal right -> Some (make_diagnostic ~suggestion:"Use not (List.is_empty xs) when checking that a list has elements." expr)
    | _ -> None
  )
  | _ -> None

let check_tree = fun (ctx: Api.Rule.context) _red_root ->
  let source_file = ctx.cst in Syn.Cst.SourceFile.structure_items source_file |> Option.unwrap_or ~default:[] |> List.concat_map Api.Traversal.expressions_of_structure_item |> List.filter_map diagnostic_for_expression
