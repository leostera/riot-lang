...
let is_zero_literal = fun expr ->
  match unwrap_expression expr with
  | Syn.Cst.Expression.Literal (Syn.Cst.Literal.Int { digits; base; _ }) -> base = Syn.Cst.Decimal
  && String.equal digits "0"
  | _ -> false
...
let diagnostic_for_expression = fun expr ->
  ...
  | Syn.Cst.Expression.Infix infix -> (
      ...
      | "==", Some _list_expr, _ when is_zero_literal right -> Some (make_diagnostic
