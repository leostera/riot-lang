...
let explanation = Api.Explanation.{ rule_id = package_rule_id; message = "Manual option matches that only rebuild Some/None should use Option.map."; body = {|
...
let matches_option_map_shape = fun (expr: Syn.Cst.match_expression) ->
  ...
  | [ first_case; second_case ] -> (
    ...
    | `SomeCase _bound_name, `NoneCase -> is_some_expression first_case.body && is_none_expression second_case.body
    | `NoneCase, `SomeCase _bound_name -> is_none_expression first_case.body && is_some_expression second_case.body
    | _ -> false
  )
  | _ -> false
