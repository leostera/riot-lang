type t = { mutable bytes: Kernel.Bytes.t; mutable length: int }
...
let create = fun ~size ->
  let initial_capacity = Kernel.Int.max 0 size in { bytes = Kernel.Bytes.create ~size:initial_capacity; length = 0 }
...
let ensure_capacity = fun buffer additional ->
  ...
  | Kernel.Order.LT | Kernel.Order.EQ -> ()
  | Kernel.Order.GT ->
      let rec next_capacity capacity =
        match Kernel.Int.compare capacity required with
        | Kernel.Order.GT | Kernel.Order.EQ -> capacity
        | Kernel.Order.LT ->
            if Kernel.Int.equal capacity 0 then
              next_capacity 1
            else next_capacity (Kernel.Int.mul capacity 2)
      in
...
let add_subbytes = fun buffer source offset slice_length ->
  ...
  | Kernel.Order.EQ | Kernel.Order.GT -> (
    ...
    | Kernel.Order.EQ | Kernel.Order.GT -> (
      ...
      | Kernel.Order.GT -> panic_invalid_range "add_subbytes" ~offset ~length:slice_length ~total:source_length
      | Kernel.Order.LT | Kernel.Order.EQ ->
          if Kernel.Int.equal slice_length 0 then
            ...
            (
              ...
              Kernel.Bytes.blit_unchecked source ~src_offset:offset ~dst:buffer.bytes ~dst_offset:buffer.length ~len:slice_length;
...
let add_substring = fun buffer source offset slice_length ->
  ...
  | Kernel.Order.EQ | Kernel.Order.GT -> (
    ...
    | Kernel.Order.EQ | Kernel.Order.GT -> (
      match Kernel.Int.compare offset (Kernel.Int.sub source_length slice_length) with
      | Kernel.Order.GT -> panic_invalid_range "add_substring" ~offset ~length:slice_length ~total:source_length
      | Kernel.Order.LT | Kernel.Order.EQ ->
          if Kernel.Int.equal slice_length 0 then
            ()
          else
            (
              ...
              ensure_capacity buffer slice_length;
              Kernel.Bytes.blit_unchecked source_bytes ~src_offset:offset ~dst:buffer.bytes ~dst_offset:buffer.length ~len:slice_length;
              buffer.length <- Kernel.Int.add buffer.length slice_length
            )
    )
  )
...
let contents = fun buffer ->
  let bytes = Kernel.Bytes.sub_unchecked buffer.bytes ~offset:0 ~len:buffer.length in Kernel.Bytes.to_string bytes
