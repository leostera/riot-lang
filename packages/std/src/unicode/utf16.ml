...
let position_of_offset = fun text ~offset ->
  ...
  let rec loop byte_offset line character =
    if byte_offset >= target then
      ...
      | Some (Rune (rune, width)) -> loop (byte_offset + width) line (character + code_units_of_rune rune)
  in
  loop 0 0 0
  ...
  if line < 0 || character < 0 then
    ...
    and find_character byte_offset current_character =
      if current_character = character then
        ...
        if byte_offset >= String.length text then
          Error ("character " ^ Int.to_string character ^ " is beyond the end of line " ^ Int.to_string position.line)
        else
          match next_step text byte_offset with
          | None -> Error ("character " ^ Int.to_string character ^ " is beyond the end of line " ^ Int.to_string position.line)
          | Some (Newline _) -> Error ("character " ^ Int.to_string character ^ " is beyond the end of line " ^ Int.to_string position.line)
          | Some (Rune (rune, width)) ->
              ...
              if next_character = character then
                ...
                else find_character (byte_offset + width) next_character
    in
    find_line 0 0
