...
let is_letter_extended = fun c -> (* ASCII letters *)
(c >= 0x41 && c <= 0x5a) || (c >= 0x61 && c <= 0x7a) || (c >= 0xc0 && c <= 0xff && c != 0xd7 && c != 0xf7) || (c >= 0x0100 && c <= 0x017f) || (c >= 0x0180 && c <= 0x024f) || (c >= 0x0370 && c <= 0x03ff) || (c >= 0x0400 && c <= 0x04ff) || (c >= 0x0530 && c <= 0x058f) || (c >= 0x05d0 && c <= 0x05ff) || (c >= 0x0600 && c <= 0x06ff) || (c >= 0x0900 && c <= 0x097f) || (c >= 0x4e00 && c <= 0x9fff) || (c >= 0xac00 && c <= 0xd7af) || (c >= 0x3040 && c <= 0x309f) || (c >= 0x30a0 && c <= 0x30ff)

(** Determine if a rune is a digit *)
let is_digit = fun c -> (* ASCII digits *)
(c >= 0x30 && c <= 0x39) || (c >= 0x0660 && c <= 0x0669) || (c >= 0x06f0 && c <= 0x06f9) || (c >= 0x0966 && c <= 0x096f)

(** Determine if a rune is whitespace *)
let is_whitespace = fun c ->
  match c with
  | 0x20 | 0x09 | 0x0a | 0x0b | 0x0c | 0x0d | 0xa0 | 0x1680 | 0x2000 | 0x2001 | 0x2002 | 0x2003 | 0x2004 | 0x2005 | 0x2006 | 0x2007 | 0x2008 | 0x2009 | 0x200a | 0x2028 | 0x2029 | 0x202f | 0x205f | 0x3000 -> true
  | _ -> false
  ...
  if is_whitespace c then
    ...
    (* ' and ' *)
      ...
      (* _ *)
        ...
        if is_letter_extended c then
          Letter
        else
          if is_digit c then
            Number
          else
            if (c >= 0x21 && c <= 0x2f) || (c >= 0x3a && c <= 0x40) || (c >= 0x5b && c <= 0x5e) || (c >= 0x60 && c <= 0x60) || (c >= 0x7b && c <= 0x7e) then
              Punctuation
            else Other
  ...
  if len = 0 then
              ...
              let next_prop =
                ...
                if next_pos >= len then
                  ...
                  | Some next_decode when Scalar.utf_decode_is_valid next_decode ->
                      ...
                      let next_code = Rune.to_int next_rune in Some (get_word_property next_code)
                  | _ -> None
              in
    ...
    | Some first_decode ->
        if not (Scalar.utf_decode_is_valid first_decode) then
          ...
          let first_prop = get_word_property (Rune.to_int first_rune) in scan first_len first_prop []
...
let find_prev_word_start = fun s pos ->
  let boundaries = find_word_boundaries s in
  let before = List.filter boundaries ~fn:(
    fun b -> b < pos
  ) in
