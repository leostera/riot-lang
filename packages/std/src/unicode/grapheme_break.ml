...
let is_control = fun c -> (c >= 0x0000 && c <= 0x001f) || (c >= 0x007f && c <= 0x009f) || (c = 0x00ad) || (c = 0x061c) || (c >= 0x180e && c <= 0x180e) || (c >= 0x200b && c <= 0x200f) || (c >= 0x2028 && c <= 0x202e) || (c >= 0x2060 && c <= 0x206f) || (c = 0xfeff) || (c >= 0xfff9 && c <= 0xfffb)

(* Interlinear annotation *)
(** Check if a code point is a prepend character *)
let is_prepend = fun c -> (* Simplified - includes Arabic/Hebrew formatting marks *)
(c >= 0x0600 && c <= 0x0605) || (c = 0x06dd) || (c = 0x070f) || (c = 0x0890 || c = 0x0891) || (c = 0x08e2)

(* Arabic disputed end *)
(** Check if a code point is a spacing mark *)
let is_spacing_mark = fun c -> (* Simplified - common spacing combining marks *)
(c >= 0x0903 && c <= 0x0903) || (c >= 0x093b && c <= 0x093b) || (c >= 0x093e && c <= 0x0940) || (c >= 0x0949 && c <= 0x094c) || (c >= 0x094e && c <= 0x094f) || (c >= 0x0982 && c <= 0x0983) || (c >= 0x09bf && c <= 0x09c0) || (c >= 0x09c7 && c <= 0x09c8) || (c >= 0x09cb && c <= 0x09cc) || (c >= 0x0a03 && c <= 0x0a03) || (c >= 0x0a3e && c <= 0x0a40) || (c >= 0x0a83 && c <= 0x0a83)
...
let is_hangul_lv = fun c ->
  if c < 0xac00 || c > 0xd7a3 then
    ...
    let t_count = 28 in ((c - s_base) mod t_count) = 0
...
let is_extended_pictographic = fun c -> let open Width_tables in
in_table emoji c
      ...
      if c = 0x200d then
        ...
        if is_control c then
          ...
          if is_combining c then
            ...
            if is_regional_indicator c then
              Regional_Indicator
            else
              if is_prepend c then
                Prepend
              else
                if is_spacing_mark c then
                  Spacing_Mark
                else
                  if is_hangul_l c then
                    L
                  else
                    if is_hangul_v c then
                      V
                    else
                      if is_hangul_t c then
                        T
                      else
                        if is_hangul_lv c then
                          LV
                        else
                          if is_hangul_lvt c then
                            LVT
                          else
                            if is_extended_pictographic c then
                              Extended_Pictographic
                            else Other
