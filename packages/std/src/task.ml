...
let await: type res. res t -> (res, exn) result = fun t ->
  let selector: Message.t -> [`select of (res, exn) result | `skip] = fun msg ->
    ...
    | _ -> `skip
  in
  let result = receive ~selector () in result
...
let rec await_all: type res. res t list -> (res, exn) result list = fun tasks ->
  ...
  | _ ->
      ...
      let find_task_by_ref pending ref = List.find pending ~fn:(
        fun (_index, task) -> Ref.equal task.ref ref
      ) in
      let is_one_of_our_refs pending ref = List.any pending ~fn:(
        fun (_index, task) -> Ref.equal task.ref ref
      ) in
      let remove_task pending task = List.filter pending ~fn:(
        fun (_index, pending_task) -> not (Ref.equal pending_task.ref task.ref)
      ) in
