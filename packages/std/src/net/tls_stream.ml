...
let network_read_failed = fun x -> Network_read_failed x

let network_write_failed = fun x -> Network_write_failed x
...
let write_plaintext t src : (int, error) result =
  let rec write_loop = fun pos remaining ->
    if remaining = 0 then
