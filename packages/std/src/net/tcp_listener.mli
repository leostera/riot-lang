...
(** Create and bind a TCP listener. The socket is automatically set to
    non-blocking mode. *)
val bind:
  ?reuse_addr:bool -> ?reuse_port:bool -> ?backlog:int -> Addr.stream_addr -> (t, error) Kernel.result
...
val accept:
  ?timeout:Time.Duration.t -> t -> (Kernel.Net.TcpStream.t * Addr.stream_addr, error) Kernel.result
