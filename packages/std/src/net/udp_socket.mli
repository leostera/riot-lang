...
val connect: t -> Addr.datagram_addr -> (unit, error) result

(** Read one datagram from a connected UDP socket. This suspends the process
    until a packet is available. *)
val recv: t -> bytes -> ?pos:int -> ?len:int -> ?timeout:Time.Duration.t -> unit -> (int, error) result
...
val send: t -> bytes -> ?pos:int -> ?len:int -> unit -> (int, error) result
