...
let get = fun headers name ->
  ...
  let rec find_header = function
    ...
    | (n, v) :: rest -> (
      ...
      | Order.EQ -> Some v
      | Order.LT | Order.GT -> find_header rest
    )
  in
  find_header headers

let get_all = fun headers name ->
  let normalized = normalize_name name in List.fold_left headers ~init:[] ~fn:(
    fun acc (n, v) ->
      match String.compare (normalize_name n) normalized with
      | Order.EQ -> v :: acc
      | Order.LT | Order.GT -> acc
  ) |> List.reverse

let remove = fun headers name ->
  let normalized = normalize_name name in List.filter headers ~fn:(
    fun (n, _) ->
      match String.compare (normalize_name n) normalized with
      | Order.EQ -> false
      | Order.LT | Order.GT -> true
...
let has = fun headers name ->
  let normalized = normalize_name name in List.any headers ~fn:(
    fun (n, _) ->
      match String.compare (normalize_name n) normalized with
      | Order.EQ -> true
...
let iter = fun f headers -> List.for_each headers ~fn:(
  fun (n, v) -> f n v
)

let fold = fun f headers acc -> List.fold_left headers ~init:acc ~fn:(
  fun acc (n, v) -> f n v acc
)
  ...
  let parse_content_type = fun value ->
    try
      ...
      | media_type :: param_parts ->
          ...
          let params = List.fold_left param_parts ~init:[] ~fn:(
            fun acc part ->
              ...
              | Some idx ->
                  ...
                  let value = String.trim (String.sub trimmed ~offset:(idx + 1) ~len:(String.length trimmed - idx - 1)) in (key, value) :: acc
          ) in Ok (media_type, List.reverse params)
    with
  ...
  let parse_authorization = fun value ->
    try
      ...
      | Some idx ->
          ...
          let credentials = String.trim (String.sub value ~offset:(idx + 1) ~len:(String.length value - idx - 1)) in Ok (scheme, credentials)
    with
    | _ -> Error `InvalidAuthorization

  let parse_cache_control = fun value ->
    let directives = String.split ~by:"," value in List.fold_left directives ~init:[] ~fn:(
      fun acc directive ->
        ...
        | Some idx ->
            ...
            let value = String.trim (String.sub trimmed ~offset:(idx + 1) ~len:(String.length trimmed - idx - 1)) in (name, Some value) :: acc
    ) |> List.reverse

  let parse_accept = fun value ->
    let entries = String.split ~by:"," value in List.fold_left entries ~init:[] ~fn:(
      fun acc entry ->
        ...
        | media_type :: param_parts ->
            ...
            let params = List.fold_left param_parts ~init:[] ~fn:(
              fun acc part ->
                ...
                | Some idx ->
                    let key = String.trim (String.sub trimmed ~offset:0 ~len:idx) in
                    let value = String.trim (String.sub trimmed ~offset:(idx + 1) ~len:(String.length trimmed - idx - 1)) in
            ...
            ) in (media_type, !quality, List.reverse params) :: acc
    ) |> List.reverse
end
