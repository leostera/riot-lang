...
let normalize_name name = String.lowercase_ascii name

let get headers name =
  let normalized = normalize_name name in
  let rec find_header = function
    | [] -> None
    | (n, v) :: _ when String.compare (normalize_name n) normalized = 0 ->
        Some v
...
let get_all headers name =
  ...
  List.fold_left
    (fun acc (n, v) ->
      if String.compare (normalize_name n) normalized = 0 then v :: acc else acc)
    [] headers
  |> List.rev
...
let iter f headers = List.iter (fun (n, v) -> f n v) headers

let fold f headers acc =
  List.fold_left (fun acc (n, v) -> f n v acc) acc headers

let length headers = List.length headers
let is_empty headers = headers = []
...
module Value = struct
  let parse_content_type value =
    ...
    with _ -> Error `InvalidContentType

  let parse_authorization value =
    try
    ...
    with _ -> Error `InvalidAuthorization

  let parse_cache_control value =
