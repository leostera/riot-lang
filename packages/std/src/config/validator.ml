...
let rec value_to_string = fun (v: Spec.value) ->
  ...
  | Spec.List items ->
      let items_str = String.concat ", " (Collections.List.map items ~fn:value_to_string) in "[" ^ items_str ^ "]"
  | Spec.DiscriminatedUnion { discriminant; variant; fields = _ } -> "<" ^ discriminant ^ "=" ^ variant ^ ">"
  | Spec.Map _ -> "<map>"
...
let rec value_equal = fun (v1: Spec.value) (v2: Spec.value) ->
  ...
  | Spec.DiscriminatedUnion { discriminant = d1; variant = v1; fields = f1 }, Spec.DiscriminatedUnion { discriminant = d2; variant = v2; fields = f2 } ->
      String.equal d1 d2 && String.equal v1 v2 && let rec map_equal l1 l2 =
        match l1, l2 with
        | [], [] -> true
        | (k1, v1) :: rest1, (k2, v2) :: rest2 -> String.equal k1 k2 && value_equal v1 v2 && map_equal rest1 rest2
        | _ -> false
      in
      map_equal f1 f2
...
let check_allowed_values = fun field_name (value: Spec.value) allowed_values ->
  ...
  | Some choices ->
      if mem_value value choices then
        ...
        let choices_str = String.concat ", " (Collections.List.map choices ~fn:value_to_string) in Error (field_name ^ ": invalid value " ^ value_str ^ ", must be one of: " ^ choices_str)

let find_assoc = fun key fields -> Collections.List.find fields ~fn:(
  fun (field_name, _value) -> String.equal field_name key
) |> Option.map ~fn:(
  fun (_field_name, value) -> value
)
  ...
  let type_result: (Spec.value, string) result =
    ...
    | Char { default } -> (
      ...
      | Some (Data.Toml.String s) -> (
        ...
        else Error (field_name ^ ": expected single character, got: " ^ s)
      )
    ...
    | List { item_spec; default } -> (
      ...
      | Some (Data.Toml.Array items) ->
          ...
          let rec validate_items = fun acc index ->
            ...
            | item :: rest ->
                let item_field = { item_spec with Spec.name = field_name ^ "[" ^ Int.to_string index ^ "]" } in
                (
      ...
      (
        ...
        (
          ...
          (
            ...
            | Ok (Spec.Map validated_fields) -> Ok (Spec.DiscriminatedUnion { discriminant; variant = variant_str; fields = validated_fields })
            | Ok _ -> Error (field_name ^ ": internal error - expected Map from validate_fields")
            | Error err -> Error err
          )
          | None ->
              let valid_variants = String.concat ", " (Collections.List.map cases ~fn:(
                fun (name, _) -> name
              )) in Error (field_name ^ ": unknown " ^ discriminant ^ " value '" ^ variant_str ^ "'. Valid values: " ^ valid_variants)
        )
...
let validate spec toml: (Spec.value, string) result = validate_fields (Spec.get_fields spec) (Some toml)
