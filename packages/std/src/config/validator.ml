...
let rec validate_field (field: Spec.field) toml_opt : (Spec.value, string) result =
  ...
  let type_result : (Spec.value, string) result =
    ...
    | Int { default } -> (
        ...
        | Some _ ->
            Error (field_name ^ ": expected integer")
        | None ->
            if required then
              Error (field_name ^ ": required field missing")
            else
    ...
    | Int32 { default } -> (
        ...
        | Some _ ->
            Error (field_name ^ ": expected int32")
        | None ->
            if required then
              Error (field_name ^ ": required field missing")
            else
    ...
    | Int64 { default } -> (
        ...
        | Some _ ->
            Error (field_name ^ ": expected int64")
        | None ->
            if required then
              Error (field_name ^ ": required field missing")
            else
    ...
    | Float { default } -> (
        ...
        | Some _ ->
            Error (field_name ^ ": expected float")
        | None ->
            if required then
              Error (field_name ^ ": required field missing")
            else
    ...
    | Uri { default } -> (
        ...
        | Some _ ->
            Error (field_name ^ ": expected URI string")
        | None ->
            if required then
              Error (field_name ^ ": required field missing")
            else
    ...
    | Datetime { default } -> (
        ...
        | Some _ ->
            Error (field_name ^ ": expected datetime string")
        | None ->
            if required then
              Error (field_name ^ ": required field missing")
            else
    ...
    | Path { default } -> (
        ...
        | Some _ ->
            Error (field_name ^ ": expected path string")
        | None ->
            if required then
              Error (field_name ^ ": required field missing")
            else
    ...
    | Uuid { default } -> (
        ...
        | Some _ ->
            Error (field_name ^ ": expected UUID string")
        | None ->
            if required then
              Error (field_name ^ ": required field missing")
            else
    ...
    | List { item_spec; default } -> (
        ...
        | Some _ ->
            Error (field_name ^ ": expected array")
        | None ->
            if required then
              Error (field_name ^ ": required field missing")
            else
    ...
    | DiscriminatedUnion { discriminant; cases } -> (
        ...
        | Some (Data.Toml.Table table) -> (* Extract discriminant value *)
          (
            ...
            | None ->
                Error (field_name ^ ": missing discriminant field '" ^ discriminant ^ "'")
          )
        | Some _ ->
            Error (field_name ^ ": expected table for discriminated union")
        | None ->
            if required then
              Error (field_name ^ ": required field missing")
            else
