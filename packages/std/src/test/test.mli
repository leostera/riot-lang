...
module Context : sig
  ...
  type built_binary = Test_context.built_binary = { name: string; path: Path.t }
...
*)
val property: ?size:size -> ?reliability:reliability -> string -> examples:int -> (ctx -> (unit, string) result) -> test_case
...
module Cli : sig
  ...
  val main: ?execution_mode:execution_mode -> name:string -> tests:test_case list -> args:string list -> unit -> (unit, Runtime.Actor.exit_reason) result
end
