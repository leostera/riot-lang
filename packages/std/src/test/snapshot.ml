...
type snapshot_paths = { approved: Path.t; pending: Path.t }
...
let append_path_suffix = fun path suffix -> Path.to_string path ^ suffix |> Path.from_string |> Result.expect ~msg:"snapshot path should stay valid UTF-8"
...
let sanitize_path_component = fun value ->
  ...
  let rec trim_left idx =
    if idx >= String.length rendered then
      ...
      if String.get_unchecked rendered ~at:idx = '-' then
        trim_left (idx + 1)
      else idx
  in
  ...
  let trimmed =
    if start_idx > end_idx then
      ""
    else String.sub rendered ~offset:start_idx ~len:(end_idx - start_idx + 1)
  in
...
let canonicalize_json =
  let rec loop = function
    | Data.Json.Object fields -> Data.Json.Object (fields |> List.map ~fn:(
      fun (key, value) -> (key, loop value)
    ) |> List.sort ~compare:(
...
let rec common_prefix_len = fun left right ->
  ...
  | (left_head :: left_tail, right_head :: right_tail) when String.equal left_head right_head -> 1 + common_prefix_len left_tail right_tail
  | _ -> 0
...
let make_diff_hunk = fun ~expected ~actual ->
  ...
  let raw_suffix_len = common_suffix_len (drop prefix_len expected_lines) (drop prefix_len actual_lines) in
...
let format_diff = fun ~expected_label ~actual_label ~expected ~actual ->
  ...
  let lines = ((([ "--- " ^ expected_label; "+++ " ^ actual_label; "@@ -" ^ Int.to_string hunk.start_line ^ "," ^ Int.to_string hunk.expected_count ^ " +" ^ Int.to_string hunk.start_line ^ "," ^ Int.to_string hunk.actual_count ^ " @@" ] @ List.map hunk.context_before ~fn:(format_line " ")) @ List.map hunk.expected_lines ~fn:(format_line "-")) @ List.map hunk.actual_lines ~fn:(format_line "+")) @ List.map hunk.context_after ~fn:(format_line " ") in String.concat "\n" lines
...
let resolve_paths = fun ~(ctx:Test_context.t) ->
  ...
  | None -> (
    ...
    | (Some workspace_root, Some package_name) ->
        ...
        let base = Path.(workspace_root / Path.v ".riot" / Path.v "snapshots" / Path.v package_name / Path.v suite_dir / Path.v test_name) in
        let approved = Path.add_extension base ~ext:"expected" in Ok { approved; pending = append_path_suffix approved ".new" }
    | (None, _) -> Error "Snapshot assertions require ctx.workspace_root to resolve external snapshot storage."
    | (_, None) -> Error "Snapshot assertions require ctx.package_name to resolve external snapshot storage."
  )
...
let mismatch_message = fun ~kind ~approved ~pending ~expected ~actual ->
  String.concat "\n"
    [
      ...
      "";
      "Diff:";
      format_diff ~expected_label:(Path.to_string approved) ~actual_label:(Path.to_string pending) ~expected ~actual;
    ]
        ...
        | Ok expected ->
            if String.equal expected actual then
              if pending_exists then
                ...
                (
                  emit_snapshot_progress ctx (Test_context.SnapshotAssertionMatched { mode = Test_context.External; format; approved_path = Some paths.approved });
                  Ok ()
                )
            else
              ...
              | Ok () ->
                  ...
                  Error (mismatch_message ~kind:"Snapshot mismatch." ~approved:paths.approved ~pending:paths.pending ~expected ~actual)
      else
        match write_pending_snapshot paths.pending actual with
        | Error err -> Error (IO.error_message err)
        | Ok () ->
...
let assert_json = fun ~ctx ~actual ->
  let rendered = actual |> canonicalize_json |> Data.Json.to_string_pretty in assert_text_with_format ~ctx ~format:Test_context.Json ~actual:rendered
...
let assert_inline_text_with_format = fun ~ctx ~format ~actual ~expected ->
  ...
  if String.equal actual expected then
    (
      emit_snapshot_progress ctx (Test_context.SnapshotAssertionMatched { mode = Test_context.Inline; format; approved_path = None });
      Ok ()
    )
  else
    (
...
let assert_inline_text = fun ~ctx ~actual ~expected -> assert_inline_text_with_format ~ctx ~format:Test_context.Text ~actual ~expected

let assert_inline_json = fun ~ctx ~actual ~expected ->
  let actual = actual |> canonicalize_json |> Data.Json.to_string_pretty in
  let expected = expected |> canonicalize_json |> Data.Json.to_string_pretty in assert_inline_text_with_format ~ctx ~format:Test_context.Json ~actual ~expected
