...
let is_snapshot_artifact = fun path ->
  let basename = Path.basename path in String.ends_with ~suffix:".expected" basename || String.ends_with ~suffix:".expected.new" basename

let discover_fixture_paths = fun root ->
  match Fs.Walker.to_list ~roots:[ root ] ~sort:true ~include_directories:false () with
  | Error err -> Error err
  | Ok found -> Ok (found |> List.filter_map ~fn:(
...
let cases = fun ?(filter = fun _ -> `keep) ?(snapshot_path = fun _ -> None) () ~dir ~run ->
  ...
  let fixtures =
    ...
    | Error err -> panic ("failed to discover fixtures under " ^ Path.to_string root ^ ": " ^ IO.error_message err)
  in
  fixtures |> List.filter ~fn:(keep_path filter) |> List.map ~fn:(
    fun fixture_path ->
