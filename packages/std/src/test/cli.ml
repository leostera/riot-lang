...
let event_test_type_fields = function
  | Test_case.UnitTest ->
      [
        "test_type", Data.Json.String "test";
      ]
  | Test_case.Property { examples } ->
      [
...
let descriptor_fields = fun (test: Runner.test_descriptor) ->
  ...
  ] @ event_test_type_fields test.test_type) @ reliability_fields test.reliability
...
let optional_path_fields = fun name value ->
  ...
  | Some path ->
      [
        name, Data.Json.String (Path.to_string path);
      ]
  | None -> []
...
let progress_fields = function
  ...
  | Test_context.SnapshotAssertionStarted { mode; format; approved_path; pending_path } ->
      ...
      ] @ optional_path_fields "approved_path" approved_path) @ optional_path_fields "pending_path" pending_path
...
let event_to_json = function
  ...
  | Runner.TestProgress { test; attempt; progress } ->
      Data.Json.Object (
        ([
          ...
          "emitted_at_us", Data.Json.Int (event_elapsed_us ());
        ] @ descriptor_fields test) @ progress_fields progress
      )
  | Runner.TestAttemptStarted { test; attempt; timeout } ->
      ...
      Data.Json.Object (
        ([
          ...
          "attempt", Data.Json.Int attempt;
          "emitted_at_us", Data.Json.Int (event_elapsed_us ());
        ] @ descriptor_fields test) @ timeout_fields
      )
  ...
  | Runner.TestAttemptFinished { test; attempt; result; duration } ->
      Data.Json.Object (
        ([
          ...
          "emitted_at_us", Data.Json.Int (event_elapsed_us ());
        ] @ descriptor_fields test) @ single_result_fields result
      )
  | Runner.TestFinished result ->
      Data.Json.Object (
        ...
        ]) @ reliability_fields result.reliability) @ single_result_fields result.result
      )
...
let matches_size = fun ~small_only ~large_only (test: Test_case.t) ->
  ...
  | (false, false, _) | (true, false, Test_case.Small) | (false, true, Test_case.Large) -> true
  | _ -> false
...
let filtered_tests = fun ~query ~small_only ~large_only ~flaky_only tests -> List.filter tests ~fn:(
  fun (test: Test_case.t) -> matches_query query test && matches_size ~small_only ~large_only test && matches_flaky ~flaky_only test
)
...
let write_tests_json = fun tests ->
  let rec to_json_items index = function
    ...
    | (test: Test_case.t) :: rest ->
        ...
        in
        Data.Json.Object ((base_fields @ test_type_fields test.test_type) @ reliability_fields test.reliability) :: to_json_items (index + 1) rest
  in
  ...
  println
    (
      Data.Json.to_string
        (
          Data.Json.Object [
            "tests", Data.Json.Array tests_json;
          ]
        )
    )
...
let suite_ctx_of_json = fun value ->
  let built_binary_of_json = function
    | Data.Json.Object fields -> (
      match List.find fields ~fn:(
        ...
        fun (name, _) -> String.equal name "path"
      ) with
      | Some (_, Data.Json.String name), Some (_, Data.Json.String path) -> Some Test_context.{ name; path = Path.v path }
      | _ -> None
    )
    | _ -> None
  in
  ...
  | Ok (Data.Json.Object fields) ->
      ...
      let built_binaries =
        match List.find fields ~fn:(
          fun (name, _) -> String.equal name "built_binaries"
        ) with
...
let suite_ctx_from_args = fun args ->
  let rec loop = function
    ...
    | arg :: _ when String.starts_with ~prefix:(ctx_json_arg ^ "=") arg ->
        ...
        let len = String.length arg - prefix_len in suite_ctx_of_json (String.sub arg ~offset:prefix_len ~len)
    | _ :: rest -> loop rest
  in
  loop args
...
let list_tests = fun ~json tests ->
  ...
  else List.for_each tests ~fn:(
    fun (test: Test_case.t) -> println test.name
  );
  Ok ()
...
let run_tests_cmd = let open Arg_parser.Arg in
command "run-tests" |> about "Run tests that match query" |> args
  [
    ...
    option "format" |> long "format" |> help "Output format: tap, json, junit, pretty, minimal" |> default "pretty" |> possible_values
      [
        "tap";
        "json";
        "junit";
...
let list_tests_cmd = let open Arg_parser.Arg in
...
let main = fun ?(execution_mode = Concurrent) ~name ~tests ~args () ->
  ...
  let cmd = command name |> about ("Test runner for " ^ name) |> subcommands [ list_tests_cmd; run_tests_cmd ] in
  ...
  | Ok matches -> (
    ...
    | Some ("list-tests", sub_matches) ->
        ...
        if small_only && large_only then
          ...
          in
          filtered_tests ~query ~small_only ~large_only ~flaky_only tests |> list_tests ~json:(get_flag sub_matches "json")
    | Some ("run-tests", sub_matches) -> (
      let format_str =
        if get_flag sub_matches "json" then
          "json"
        else get_one sub_matches "format" |> Option.unwrap_or ~default:"pretty"
      in
      ...
      | Ok reporter ->
          ...
          let requested_concurrency = get_int sub_matches "concurrency" |> Option.unwrap_or ~default:default_concurrency in
          let small_only = get_flag sub_matches "small" in
          let large_only = get_flag sub_matches "large" in
          let flaky_only = get_flag sub_matches "flaky" in
          let query =
          ...
          if small_only && large_only then
            ...
            let size_filter =
              if small_only then
                ...
                if large_only then
                  Runner.Only_large
                else Runner.All_sizes
            in
            let flaky_max_retries = get_int sub_matches "flaky-max-retries" |> Option.unwrap_or ~default:0 in
            ...
            let config = Runner.{
              ...
              suite_info;
              event_handler = if String.equal format_str "json" then
                json_event_handler
              else Runner.no_event_handler
            } in
