...
let from_millis = fun millis ->
  ...
  let nanos = millis mod 1_000 * 1_000_000 in { secs; nanos }

let from_micros = fun micros ->
  let secs = micros / 1_000_000 in
  let nanos = micros mod 1_000_000 * 1_000 in { secs; nanos }

let from_nanos = fun nanos ->
  let secs = nanos / 1_000_000_000 in
  let remaining_nanos = nanos mod 1_000_000_000 in { secs; nanos = remaining_nanos }

let from_secs_float = fun f ->
  let secs = Int.from_float f in
  let nanos = Int.from_float ((f -. Float.from_int secs) *. 1_000_000_000.0) in { secs; nanos }
...
let to_nanos = fun t -> Int64.add (Int64.mul (Int64.from_int t.secs) 1_000_000_000L) (Int64.from_int t.nanos)
...
let normalize = fun t ->
  if t.nanos >= 1_000_000_000 then
    ...
    let remaining_nanos = t.nanos mod 1_000_000_000 in { secs = t.secs + extra_secs; nanos = remaining_nanos }
  else
    if t.nanos < 0 then
      let borrow_secs = (Int.abs t.nanos + 999_999_999) / 1_000_000_000 in { secs = t.secs - borrow_secs; nanos = t.nanos + (borrow_secs * 1_000_000_000) }
    else t
...
let mul = fun t factor ->
  if factor <= 0 then
    ...
    let final_nanos = Int64.rem new_nanos 1_000_000_000L in { secs = Int64.to_int final_secs; nanos = Int64.to_int final_nanos }

let div = fun t divisor ->
  if divisor <= 0 then
    panic "Division by zero or negative number"
  else
    let total_nanos = Int64.add (Int64.mul (Int64.from_int t.secs) 1_000_000_000L) (Int64.from_int t.nanos) in
    let result_nanos = Int64.div total_nanos (Int64.from_int divisor) in
    let secs = Int64.div result_nanos 1_000_000_000L in
    let nanos = Int64.rem result_nanos 1_000_000_000L in { secs = Int64.to_int secs; nanos = Int64.to_int nanos }
...
let mul_f64 = fun t factor ->
  if factor <= 0.0 then
    ...
    let nanos = Int.from_float (Float.rem result_nanos_f 1_000_000_000.0) in { secs; nanos }
