(** # Time.Duration - Spans of time
    ...
    A type representing a span of time with nanosecond precision, typically
    used for timeouts, delays, and measuring elapsed time.
    ...
    let interval = Duration.from_millis 100 in
    let delay = Duration.make ~secs:1 ~nanos:500_000_000 in
    
    (* Convert to different units *)
    Duration.to_millis timeout  (* 30000 *)
    ...
    ```ocaml
    let d1 = Duration.from_secs 5 in
    let d2 = Duration.from_secs 3 in
    
    let total = Duration.add d1 d2 in  (* 8 seconds *)
    let diff = Duration.sub d1 d2 in  (* 2 seconds *)
    let doubled = Duration.mul d1 2 in  (* 10 seconds *)
    ```
    
    Safe operations with overflow protection:
    
    ```ocaml
    let huge = Duration.from_secs 1_000_000_000 in
    
    (* Checked operations return None on overflow *)
    match Duration.checked_add huge huge with
    | Some d -> Printf.printf "OK: %d\n" (Duration.to_secs d)
    | None -> Printf.printf "Overflow\n"
    
    (* Saturating operations clamp to max/zero *)
    let maxed = Duration.saturating_add huge huge in
    Duration.equal maxed Duration.max  (* true *)
    ```
...
(** A duration of zero time.
    ...
    ```ocaml
    Duration.is_zero Duration.zero  (* true *)
    Duration.to_secs Duration.zero  (* 0 *)
    ```
*)
...
(** The maximum representable duration (~584 billion years).
    ...
    ```ocaml
    let huge = Duration.from_secs 999_999_999_999 in
...
(** Creates a duration from seconds and nanoseconds.
    ...
    ```ocaml
    (* 1.5 seconds *)
    Duration.make ~secs:1 ~nanos:500_000_000
    
    (* 0.001 seconds = 1 millisecond *)
    Duration.make ~secs:0 ~nanos:1_000_000
    ```
    
    ## Note
    
    Nanoseconds should be in range 0-999,999,999. Values outside this range
    will be normalized into the seconds component.
*)
...
(** Creates a duration from a number of days (24-hour periods).
    ...
    ```ocaml
    let week = Duration.from_days 7 in
    Duration.to_secs week  (* 604800 *)
    ```
*)
...
(** Creates a duration from hours.
    ...
    ```ocaml
    Duration.from_hours 2 |> Duration.to_secs  (* 7200 *)
    ```
*)
...
(** Creates a duration from minutes.
    ...
    ```ocaml
    Duration.from_mins 30 |> Duration.to_secs  (* 1800 *)
    ```
*)
...
(** Creates a duration from seconds.
    ...
    ```ocaml
    let timeout = Duration.from_secs 60 in
    ```
*)
...
(** Creates a duration from milliseconds.
    ...
    ```ocaml
    Duration.from_millis 1500 |> Duration.to_secs_float  (* 1.5 *)
    ```
*)
...
(** Creates a duration from microseconds.
    ...
    ```ocaml
    Duration.from_micros 1_000_000 |> Duration.to_secs  (* 1 *)
    ```
*)
...
(** Creates a duration from nanoseconds.
    ...
    ```ocaml
    Duration.from_nanos 1_000_000_000 |> Duration.to_secs  (* 1 *)
    ```
*)
...
(** Creates a duration from weeks (7-day periods).
    ...
    ```ocaml
    Duration.from_weeks 2 |> Duration.to_days  (* 14 *)
    ```
*)
...
(** Creates a duration from floating-point seconds.
    ...
    ```ocaml
    let d = Duration.from_secs_float 1.5 in
    Duration.to_millis d  (* 1500 *)
    ```
*)
...
(** Extracts the whole seconds component, discarding fractional part.
    ...
    ```ocaml
    let d = Duration.from_millis 1500 in
    Duration.to_secs d  (* 1 - fractional part discarded *)
    ```
*)
...
(** Converts to floating-point seconds with fractional precision.
    ...
    ```ocaml
    let d = Duration.make ~secs:1 ~nanos:500_000_000 in
...
(** Converts to total milliseconds.
    ...
    ```ocaml
    Duration.from_secs 2 |> Duration.to_millis  (* 2000 *)
    ```
*)
...
(** Converts to total microseconds.
    ...
    ```ocaml
    Duration.from_millis 1 |> Duration.to_micros  (* 1000 *)
    ```
*)
...
(** Converts to total nanoseconds.
    ...
    ```ocaml
    Duration.from_secs 1 |> Duration.to_nanos  (* 1_000_000_000 *)
    ```
*)
...
(** Returns only the fractional milliseconds (0-999).
    ...
    ```ocaml
    let d = Duration.make ~secs:5 ~nanos:123_456_789 in
...
(** Returns only the fractional nanoseconds (0-999,999,999).
    ...
    ```ocaml
    let d = Duration.make ~secs:5 ~nanos:123_456_789 in
    Duration.subsec_nanos d  (* 123_456_789 *)
    Duration.to_secs d  (* 5 *)
    ```
*)
...
(** Returns [true] if duration is zero.
    ...
    ```ocaml
    Duration.is_zero Duration.zero  (* true *)
    Duration.is_zero (Duration.from_secs 0)  (* true *)
    Duration.is_zero (Duration.from_nanos 1)  (* false *)
    ```
*)
...
(** Adds two durations. Panics on overflow.
    ...
    ```ocaml
    let d1 = Duration.from_secs 5 in
    let d2 = Duration.from_millis 500 in
    Duration.add d1 d2 |> Duration.to_secs_float  (* 5.5 *)
    ```
...
(** Subtracts durations. Panics if result would be negative.
    ...
    ```ocaml
    let d1 = Duration.from_secs 10 in
    let d2 = Duration.from_secs 3 in
...
(** Multiplies duration by an integer. Panics on overflow.
    ...
    ```ocaml
    Duration.from_secs 5 |> Duration.mul 3 |> Duration.to_secs  (* 15 *)
    ```
*)
...
(** Divides duration by an integer. Panics if divisor is zero.
    ...
    ```ocaml
    Duration.from_secs 10 |> Duration.div 2 |> Duration.to_secs  (* 5 *)
    ```
*)
...
(** Returns [Some result] if addition doesn't overflow, [None] otherwise.
    ...
    ```ocaml
    Duration.checked_add Duration.max (Duration.from_secs 1)  (* None *)
    Duration.checked_add (Duration.from_secs 5) (Duration.from_secs 3)  (* Some 8s *)
    ```
*)
...
(** Returns [Some result] if subtraction is positive, [None] if negative.
    ...
    ```ocaml
    let d1 = Duration.from_secs 5 in
    let d2 = Duration.from_secs 10 in
...
(** Returns [Some result] if multiplication doesn't overflow, [None] otherwise.
    ...
    ```ocaml
    Duration.checked_mul Duration.max 2  (* None - overflow *)
    ```
*)
...
(** Returns [Some result] if divisor is non-zero, [None] for division by zero.
    ...
    ```ocaml
    Duration.checked_div (Duration.from_secs 10) 0  (* None *)
...
(** Adds durations, clamping to [max] on overflow.
    ...
    ```ocaml
    let result = Duration.saturating_add Duration.max (Duration.from_secs 1) in
    Duration.equal result Duration.max  (* true *)
    ```
*)
...
(** Subtracts durations, clamping to [zero] if result would be negative.
    ...
    let d1 = Duration.from_secs 5 in
    let d2 = Duration.from_secs 10 in
    let result = Duration.saturating_sub d1 d2 in
    Duration.is_zero result  (* true *)
    ```
*)
...
(** Multiplies duration, clamping to [max] on overflow.
    ...
    ```ocaml
    Duration.saturating_mul Duration.max 1000 |> Duration.equal Duration.max  (* true *)
    ```
*)
...
(** Multiplies duration by a floating-point factor.
    ...
    ```ocaml
    Duration.from_secs 10 |> Duration.mul_f64 1.5 |> Duration.to_secs  (* 15 *)
    ```
*)
...
(** Divides duration by a floating-point divisor.
    ...
    ```ocaml
    Duration.from_secs 10 |> Duration.div_f64 2.5 |> Duration.to_secs  (* 4 *)
    ```
*)
...
(** Returns the absolute difference between two durations.
    ...
    let d1 = Duration.from_secs 5 in
    let d2 = Duration.from_secs 10 in
    Duration.abs_diff d1 d2 |> Duration.to_secs  (* 5 *)
    Duration.abs_diff d2 d1 |> Duration.to_secs  (* 5 - same *)
    ```
*)
...
(** Returns the smaller of two durations.
    ...
    ```ocaml
    Duration.min (Duration.from_secs 5) (Duration.from_secs 10)
    |> Duration.to_secs  (* 5 *)
    ```
*)
...
(** Returns the larger of two durations. Note: conflicts with [max] constant.
    ...
    ```ocaml
    Duration.max (Duration.from_secs 5) (Duration.from_secs 10)
    |> Duration.to_secs  (* 10 *)
    ```
*)
...
(** Compares two durations. Returns negative if first < second, 0 if equal,
    ...
    ```ocaml
    Duration.compare (Duration.from_secs 5) (Duration.from_secs 10)  (* < 0 *)
    ```
*)
...
(** Tests equality of two durations.
    ...
    ```ocaml
    Duration.equal (Duration.from_secs 5) (Duration.from_millis 5000)  (* true *)
    ```
*)
