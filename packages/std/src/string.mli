(** # String - UTF-8 string manipulation
    ...
    iteration support. Strings in OCaml are sequences of bytes, and this
    module provides safe UTF-8 character iteration on top of that.
    ...
    let trimmed = String.trim "  spaces  " in
    
    (* UTF-8 iteration *)
    String.into_iter text
    |> Iterator.for_each (fun char ->
...
(** Creates a mutable iterator over UTF-8 characters.
    ...
    Iterates over Unicode characters (not bytes) in the string.
    ...
    ```ocaml
    let text = "Hello, 世界!" in
    let iter = String.into_mut_iter text in
    
    (* Count characters (not bytes) *)
    let char_count = MutIterator.count iter in
    println "Characters: %d" char_count;
    
    (* Process each character *)
    String.into_mut_iter text
    |> MutIterator.for_each (fun uchar ->
    ...
    UTF-8 decoding has some overhead. For byte-level operations,
    use standard String functions instead.
*)

val into_iter : string -> Uchar.t Iterator.t
(** Creates an immutable iterator over UTF-8 characters.
    
    Similar to [`into_mut_iter`] but returns an immutable iterator
    ...
    For strict UTF-8 validation, check bytes before iteration.
*)
