...
let rec encode_positive_int64 = fun value ->
  ...
  else encode_positive_int64 (Int64.div value 8L) ^ String.make ~len:1 ~char:(digit_char (Int64.to_int (Int64.rem value 8L)))
...
let rec encode_negative_int64 = fun value ->
  ...
  else encode_negative_int64 (Int64.div value 8L) ^ String.make ~len:1 ~char:(digit_char (Int64.to_int (Int64.neg (Int64.rem value 8L))))
...
let encode_signed_int64 = fun value ->
  ...
  else encode_positive_int64 value
  ...
  if String.equal s "" then
    ...
    if start >= len then
      ...
      let has_octal_prefix =
        if start + 1 < len then
          let marker = String.get_unchecked s ~at:(start + 1) in String.get_unchecked s ~at:start = '0' && (marker = 'o' || marker = 'O')
        else false
      in
      let digits =
        if has_octal_prefix then
          String.sub s ~offset:(start + 2) ~len:(len - start - 2)
        else String.sub s ~offset:start ~len:(len - start)
      in
      if String.equal digits "" then
        Error `Invalid_octal
      else
        if String.for_all digits ~fn:(
          function
          | '0' .. '7' -> true
          | _ -> false
        ) then
          Ok (sign ^ "0o" ^ digits)
        else Error `Invalid_octal
