...
(** Attach a handler *)
let attach id fn = HashMap.insert handlers id { id; fn } |> ignore

(** Detach a handler by ID *)
let detach id = HashMap.remove handlers id |> ignore

(** Detach all handlers *)
let detach_all () = HashMap.clear handlers

(** List all handler IDs *)
let list () = HashMap.keys handlers
