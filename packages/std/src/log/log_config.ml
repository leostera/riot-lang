...
let spec = Spec.(for_app ~app:"log" [ list (discriminated_union ~discriminant:"type" ~cases:[
...
let find_field = fun key fields -> List.find fields ~fn:(
...
let get = fun conf ->
  ...
  let handlers = List.map handlers_list ~fn:(
    fun handler_value ->
      ...
      | "file" ->
          ...
          in
          File { path; format }
      | _ -> panic ("Unknown handler type: " ^ variant ^ " (this should not happen - spec should enforce it)")
  ) in Ok { handlers }
