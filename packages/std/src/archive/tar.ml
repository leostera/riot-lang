...
type source = { reader: Reader.t; buffer: Buffer.t }
...
let entry_of_header = fun (header: Engine.header) ->
  let* path = path_of_string header.path
  in
  let* link_target =
...
let feed_from_source_entries = fun source tar_reader ->
  ...
  | Ok bytes_read ->
      ...
      let* consumed = Engine.feed_reader tar_reader ~src:bytes ~src_pos:0 ~src_len:bytes_read |> Result.map_err ~fn:(
        fun err -> Entries_error (Engine_error err)
      )
      in
      if consumed = bytes_read then
        Ok bytes_read
      else Error (Entries_error (Engine_error (Engine.Unknown_error "partial tar feed")))
...
let feed_from_source_extract = fun source tar_reader ->
  ...
  | Ok bytes_read ->
      ...
      let bytes = Buffer.to_bytes source.buffer in
      let* consumed = Engine.feed_reader tar_reader ~src:bytes ~src_pos:0 ~src_len:bytes_read |> Result.map_err ~fn:(
        fun err -> Extract_error (Engine_error err)
      )
      in
      if consumed = bytes_read then
        Ok bytes_read
      else Error (Extract_error (Engine_error (Engine.Unknown_error "partial tar feed")))
...
let entries = fun reader ->
  ...
  | Ok tar_reader ->
      protect ~finally:(
        fun () -> Engine.close_reader tar_reader
      )
        (
          fun () ->
            ...
            let rec loop acc =
              ...
              | Ok (Engine.Entry header) ->
                  let* entry = entry_of_header header |> Result.map_err ~fn:(
                    fun err -> Entries_error err
                  )
                  in
  ...
  if Path.is_absolute path then
    ...
    if path_str = "." then
      ...
      if path_str = "" then
        ...
        if List.any (Path.components normalized) ~fn:(
          fun component -> Path.to_string component = ".."
        ) then
          Error (Unsafe_path path_str)
        else Ok (Some normalized)

let register_target = fun seen path ->
  if HashSet.contains seen ~value:path then
    Error (Duplicate_entry path)
  else
    let _ = HashSet.insert seen ~value:path in Ok ()

let should_skip_metadata_entry = function
  | Other "x" | Other "g" -> true
  | _ -> false

let should_skip_entry_path = fun path ->
  let name = Path.basename path in String.starts_with ~prefix:"._" name || String.equal name ".DS_Store" || String.equal name "__MACOSX"
...
let write_entry_file = fun source tar_reader file ->
  ...
  let rec loop chunk_count =
    ...
    | Ok (Engine.Chunk bytes_read) ->
        ...
        let* () = Fs.File.write_all file data |> Result.map_err ~fn:(
          fun err -> Extract_fs_error (fs_error_of_file_error err)
        ) in loop (chunk_count + 1)
    | Ok Engine.Need_input -> (
      match feed_from_source_extract source tar_reader with
      | Ok 0 -> Error (Extract_error (Engine_error Engine.Unexpected_eof))
        ...
        (
          fun () ->
            ...
            let rec loop () =
              ...
              | Ok (Engine.Entry header) ->
                  let* entry = entry_of_header header |> Result.map_err ~fn:(
                    fun err -> Extract_error err
                  )
                  in
                  let* relative_path = safe_relative_path ~kind:entry.kind entry.path |> Result.map_err ~fn:(
                    fun err -> Extract_error err
                  )
                  in
                  let* () =
                    match entry.kind, relative_path with
                    | kind, _ when should_skip_metadata_entry kind -> drain_entry_extract source tar_reader
                    | _, Some path when should_skip_entry_path path -> drain_entry_extract source tar_reader
                    | Directory, None ->
                        let* () = Fs.create_dir_all into |> Result.map_err ~fn:(
                          fun err -> Extract_fs_error err
                        )
                        in
                        let* () = drain_entry_extract source tar_reader in set_permissions into entry.mode |> Result.map_err ~fn:(
                          fun err -> Extract_fs_error err
                        )
                    | Directory, Some relative_path ->
                        let target = Path.join into relative_path in
                        let* () = register_target seen target |> Result.map_err ~fn:(
                          fun err -> Extract_error err
                        )
                        in
                        let* () = Fs.create_dir_all target |> Result.map_err ~fn:(
                          fun err -> Extract_fs_error err
                        )
                        in
                        let* () = drain_entry_extract source tar_reader in set_permissions target entry.mode |> Result.map_err ~fn:(
                          fun err -> Extract_fs_error err
                        )
                    | File, Some relative_path ->
                        ...
                        let* () =
                          ...
                          | None -> Ok ()
                          | Some parent -> Fs.create_dir_all parent |> Result.map_err ~fn:(
                            fun err -> Extract_fs_error err
                          )
                        in
                        begin
                          ...
                          | Ok file ->
                              protect ~finally:(
                                ...
                                (
                                  fun () ->
                                    let* () = write_entry_file source tar_reader file in set_permissions target entry.mode |> Result.map_err ~fn:(
                                      fun err -> Extract_fs_error err
                                    )
                                )
                        end
                    | (File, None) | (Symlink, _) | (Hardlink, _) | (Other _, _) -> Error (Extract_error (Unsupported_entry_kind entry.kind))
                  in
                  loop ()
