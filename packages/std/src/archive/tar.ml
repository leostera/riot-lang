...
let feed_from_source_entries = fun source tar_reader ->
  ...
  | Ok bytes_read ->
      let () = yield () in
      let* consumed = Engine.feed_reader
        tar_reader
        ~src:source.buffer
        ~src_pos:0
        ~src_len:bytes_read
...
let next_entry_entries = fun source tar_reader ->
  let rec loop () =
    ...
    | Error err ->
        Error (Entries_error (Engine_error err))
...
let next_entry_extract = fun source tar_reader ->
  let rec loop () =
    match Engine.next_entry tar_reader with
    | Error err ->
        Error (Extract_error (Engine_error err))
    | Ok Engine.Need_input -> (
...
let entries = fun reader ->
  ...
  | Ok tar_reader ->
      protect ~finally:(fun () -> Engine.close_reader tar_reader)
        (fun () ->
