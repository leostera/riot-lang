...
let monotonic_time_nanos = fun () ->
  ...
  | Ok time ->
      let secs, nanos = Kernel.Time.Monotonic.to_parts time in Int64.add (Int64.mul (Int64.from_int secs) 1_000_000_000L) (Int64.from_int nanos)
...
let ensure_can_run_once = fun () ->
  if not (Atomic.compare_and_set has_run false true) then
    panic "Runtime.run can only be called once per process. Each test should be \
...
let create_process_registry = fun worker_count ->
  ...
  let shards = Array.init ~count:shard_count ~fn:(
    fun _ -> { lock = Runtime_mutex.create (); processes = HashMap.with_capacity ~size:64 }
  ) in { shards; size = Atomic.make 0 }
...
let shard_for_pid = fun registry pid ->
  let idx = Runtime_pid.to_int pid mod Array.length registry.shards in Array.get_unchecked registry.shards ~at:idx
...
let create = fun ~config ->
  ...
  | Ok io_poll ->
      ...
      let workers = Array.init ~count:worker_count ~fn:(
        fun index -> create_worker (Runtime_scheduler_id.of_int index)
      ) in
      {
...
let worker_by_id = fun t worker_id -> Array.get_unchecked t.workers ~at:(Runtime_scheduler_id.to_int worker_id)

let is_valid_worker_id = fun t worker_id ->
  let idx = Runtime_scheduler_id.to_int worker_id in idx >= 0 && idx < worker_count t

let pick_spawn_worker = fun t ->
  let total = worker_count t in
  if total = 1 then
    Runtime_scheduler_id.zero
  else Runtime_scheduler_id.of_int (Atomic.get t.processes.size mod total)
...
let request_shutdown = fun t ~status ->
  if Atomic.compare_and_set t.stop false true then
    ...
    if not (Int.equal status 0) then
      Atomic.set t.status status;
  ...
  List.for_each t.blocking_lanes ~fn:(
...
let enqueue_on_worker = fun t worker_id slot ->
  ...
  if is_valid_worker_id t worker_id then
    if try_mark_slot_queued slot then
      ...
      if Runtime_process.is_runnable (slot_process slot) then
        (
          increment t.counters.duplicate_enqueue_races;
          trace (Kernel.String.concat "" [ "duplicate enqueue prevented pid="; Runtime_pid.to_string (slot_pid slot) ])
        )
  ...
  | Normal | Pinned ->
      ...
      let worker_id =
        if is_valid_worker_id t owner then
          owner
        else Runtime_scheduler_id.zero
      in
...
let wake_process = fun t slot ->
  ...
  if Runtime_process.try_set_runnable_if_waiting proc then
    ...
    if Runtime_process.is_runnable proc then
      enqueue_owned_process t slot
...
let spawn_on_worker_with_placement = fun t ~worker_id ~placement fn ->
  ...
  (
    ...
    | Blocking -> enqueue_on_blocking_lane t slot
    | Normal | Pinned -> enqueue_on_worker t worker_id slot
  );
  pid

let spawn_on_worker = fun t ~worker_id fn -> spawn_on_worker_with_placement t ~worker_id ~placement:Normal fn

let spawn = fun t fn ->
  let worker_id = pick_spawn_worker t in spawn_on_worker t ~worker_id fn
...
let spawn_pinned = fun ?worker_id t fn ->
  let worker_id =
    ...
    | Some worker_id ->
        if is_valid_worker_id t worker_id then
          worker_id
        else panic "spawn_pinned got an invalid scheduler id"
    | None -> (
      match current_worker_id_opt () with
...
let wait_for_blocking_work = fun t slot ->
  ...
  | Some lane ->
      let proc = slot_process slot in
      Runtime_mutex.lock lane.lock;
      while (not (Atomic.get t.stop)) && Runtime_process.is_alive proc && not (Atomic.get slot.queued) do Runtime_condition.wait lane.cond lane.lock done;
      Runtime_mutex.unlock lane.lock;
      if Atomic.get t.stop || not (Runtime_process.is_alive proc) then
        None
      else
        if Atomic.compare_and_set slot.queued true false then
          Some slot
        else None
...
let install_receive_timeout = fun t proc secs ->
  ...
  | None ->
      ...
      let timer_id = add_timer t ~now ~duration_nanos ~mode:Runtime_timer.One_shot ~action:(Runtime_timer.Wake_process proc) in Runtime_process.set_receive_timeout proc timer_id
...
let install_syscall_timeout = fun t proc secs ->
  ...
  | None ->
      let now = monotonic_time_nanos () in
      let duration_nanos = Int64.from_float (secs *. 1_000_000_000.0) in
      let timer_id = add_timer t ~now ~duration_nanos ~mode:Runtime_timer.One_shot ~action:(Runtime_timer.Wake_process proc) in Runtime_process.set_syscall_timeout proc timer_id

let handle_receive = fun k t proc ~selector ~timeout -> let open Proc_state in
let timeout_expired =
  match Runtime_process.receive_timeout proc with
  | None -> false
...
let park_for_receive () =
  ...
  if Runtime_process.try_mark_awaiting_message proc then
    if Runtime_process.mailbox_count proc = 0 then
      ...
      (
        let _ = Runtime_process.try_mark_runnable_from_waiting_message proc in k Delay
      )
  else k Delay
in
...
if timeout_expired then
  ...
  let selected =
    if Runtime_process.has_empty_mailbox proc then
      None
    else scan_saved (Runtime_process.save_queue_count proc)
  in
  match selected with
  | Some selected -> continue_with selected
  | None -> park_for_receive ()

let handle_syscall = fun k t proc name interest source timeout -> let open Proc_state in
let timeout_state =
  match Runtime_process.syscall_timeout proc with
  | None -> `none
  | Some timer_id ->
...
| None -> (
  ...
  | `armed -> (* Spurious wakeup while still waiting on a registered syscall. *)
  k Suspend
  | `none ->
      ...
      (
        ...
        | Ok () ->
            ...
            k Suspend
        | Error err ->
            eprintln ("[Scheduler] ERROR: Failed to register I/O for process " ^ Runtime_pid.to_string (Runtime_process.pid proc) ^ ": " ^ io_registration_error_message err);
            Runtime_process.mark_as_runnable proc;
            k (Discontinue (Failure "Failed to register I/O"))
      )
)

let perform = fun t proc -> let open Proc_state in
...
let perform: type a b. (a, b) step_callback = fun k eff ->
  ...
  | Yield -> k Yield
  | Syscall { name; interest; source; timeout } -> handle_syscall k t proc name interest source timeout
  | _ -> k Suspend
in
{ perform }
...
let handle_exit_proc = fun t proc reason ->
  ...
  with_relations_lock t
    (
      fun () ->
        List.for_each (Runtime_process.get_monitored_by proc) ~fn:(
          fun (monitor_pid, monitor_ref) ->
            ...
            | Some monitor_slot ->
                ...
                Runtime_process.send_message monitor_proc (Runtime_process.Messages.DOWN { ref = monitor_ref; pid; reason });
                wake_process t monitor_slot
        );
        List.for_each (Runtime_process.get_monitors proc) ~fn:(
          fun (monitor_ref, monitored_pid) ->
            match get_process_slot t monitored_pid with
            | None -> ()
            | Some monitored_slot ->
                let monitored_proc = slot_process monitored_slot in Runtime_process.remove_monitored_by monitored_proc pid monitor_ref
        );
        List.for_each (Runtime_process.get_links proc) ~fn:(
          fun linked_pid ->
  ...
  else maybe_shutdown_if_empty t
...
let handle_run_proc = fun t ctx slot ->
  ...
  try
    let next =
      ...
      with
      | exn ->
          eprintln ("[Scheduler] ERROR: Proc_state.run raised for process " ^ pid_str ^ ": " ^ Kernel.Exception.to_string exn);
          raise exn
    in
    ...
    | Proc_state.Finished (Error { exn; backtrace }) ->
        eprintln ("[Scheduler] Process " ^ pid_str ^ " finished with exception: " ^ Kernel.Exception.to_string exn);
...
let wait_for_local_work = fun t (worker: worker) ->
  Runtime_mutex.lock worker.lock;
  while Queue.is_empty worker.queue && not (Atomic.get t.stop) do Runtime_condition.wait worker.cond worker.lock done;
  let slot =
    if Atomic.get t.stop then
      None
    else Queue.pop worker.queue
  in
  Runtime_mutex.unlock worker.lock;
  match slot with
  | None -> None
...
let steal_batch = fun (victim: worker) ->
  ...
  List.for_each kept ~fn:(
    fun slot -> Queue.push victim.queue ~value:slot
  );
...
let push_batch = fun (worker: worker) batch ->
  if not (List.is_empty batch) then
    (
      Runtime_mutex.lock worker.lock;
      List.for_each batch ~fn:(
        fun slot -> Queue.push worker.queue ~value:slot
      );
      Runtime_condition.signal worker.cond;
      Runtime_mutex.unlock worker.lock
    )
...
let attempt_steal = fun t (worker: worker) ->
      ...
      if Int.equal victim_idx self_idx then
        ...
        if List.is_empty batch then
          ...
          (
            ...
            List.for_each batch ~fn:(
              fun slot -> set_slot_owner_worker slot worker.id
            );
  ...
  if total <= 1 then
    ...
    else increment t.counters.failed_steals;
  did_steal
...
let deregister_io_in_reactor = fun t source ~context ->
  ...
  | Ok () -> ()
  | Error err -> eprintln ("[Scheduler] WARN: Failed to deregister I/O " ^ context ^ ": " ^ Async.error_to_string err)
...
let process_timers = fun t ->
  if Timer_wheel.size t.timer_wheel = 0 then
    ...
    let now = monotonic_time_nanos () in
    let expired = Timer_wheel.tick t.timer_wheel ~now in List.for_each expired ~fn:(
      fun timer ->
          ...
          | Runtime_timer.Wake_process proc ->
              ...
              if Runtime_process.has_syscall_timeout_id proc timer_id then
                (
                  ...
                  match Runtime_process.state proc with
                  | Waiting_io { source; _ } -> (* Syscall timeout resumes the waiting process with
                                       [Syscall_timeout]. The wait registration must be removed
                                       first so a subsequent syscall can reregister cleanly. *)
                  deregister_io_in_reactor t source ~context:("for timed out process " ^ Runtime_pid.to_string (Runtime_process.pid proc))
                  | _ -> ()
                );
              if Runtime_process.is_alive proc then
                (
...
let handle_reactor_command = fun t cmd ->
  ...
  | Add_timer { now; duration_nanos; mode; action; reply } ->
      let timer_id = Timer_wheel.add_timer t.timer_wheel ~now ~duration_nanos ~mode ~action in resolve_response reply timer_id
  ...
  | Deregister_io source -> (
    match Async.Poll.deregister t.io_poll source with
    | Ok () -> ()
    | Error err -> eprintln ("[Scheduler] WARN: Failed to deregister I/O source: " ^ Async.error_to_string err)
  )
  ...
  List.for_each events ~fn:(
    fun event ->
      ...
      | Some slot -> (
        match Runtime_process.state proc with
        | Waiting_io { source; _ } ->
            deregister_io_in_reactor t source ~context:("for process " ^ Runtime_pid.to_string (Runtime_process.pid proc));
            if Runtime_process.is_alive proc then
              (
...
module Reactor = struct
  let loop = fun scheduler -> Scheduler_reactor.loop ~has_pending_commands:has_pending_reactor_commands ~drain_commands:drain_reactor_commands ~handle_command:handle_reactor_command ~process_timers ~poll_io scheduler
end

module Worker = struct
  let loop = fun scheduler worker -> Scheduler_worker.loop ~pop_local ~step_process ~attempt_steal ~wait_for_local_work scheduler worker
end
