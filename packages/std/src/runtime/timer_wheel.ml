...
let calculate_slot = fun t expires_at ->
  ...
  if (
    ...
    let slot = Int64.to_int (Int64.rem (Int64.add (Int64.from_int t.current_slot) ticks) (Int64.from_int t.num_slots)) in Some slot
  else (* Timer is too far in the future - put in overflow *)
  None
...
let next_expiration = fun t ~now ->
  ...
  HashMap.for_each t.timers_by_id ~fn:(
    fun _id timer ->
