...
module Actor = struct
  ...
  let with_current_relations = fun f ->
    ...
    Scheduler.with_relations_lock t
      (
        fun () -> f t current
      )

  let set_flags = fun flags ->
    let current = Scheduler.get_current_process () in Proc.set_flags current flags
...
let run = fun ~main ~args ?config () ->
  ...
  let status = Scheduler.run ~config ~main:(
    fun () -> main ~args
  ) in sys_exit status
...
let spawn_pinned = fun ?scheduler fn ->
  let scheduler = Option.map scheduler ~fn:Scheduler_id.of_int in Scheduler.spawn_pinned ?worker_id:scheduler (Scheduler.get_scheduler ()) fn
...
module Timer = struct
  ...
  let send_after = fun target_pid (msg: Message.t) ~after ->
    ...
    let duration_nanos = Int64.from_float (after *. 1_000_000_000.0) in Scheduler.add_timer sch ~now ~duration_nanos ~mode:Timer.One_shot ~action:(Timer.Send_message (target_pid, msg))

  let send_interval = fun target_pid (msg: Message.t) ~interval ->
    let sch = Scheduler.get_scheduler () in
    let now = monotonic_time_nanos () in
    let duration_nanos = Int64.from_float (interval *. 1_000_000_000.0) in Scheduler.add_timer sch ~now ~duration_nanos ~mode:(Timer.Interval duration_nanos) ~action:(Timer.Send_message (target_pid, msg))

  let cancel = fun timer_id ->
    let sch = Scheduler.get_scheduler () in Scheduler.cancel_timer sch timer_id
end
...
let reset_trace_counters = fun () ->
  let scheduler = Scheduler.get_scheduler () in Scheduler.reset_trace_counters scheduler
