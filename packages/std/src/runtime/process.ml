...
let try_set_runnable_if_waiting = fun t ->
  ...
  match current with
  | Waiting_message | Waiting_io _ -> Runtime_atomic.compare_and_set t.state current Runnable
  | _ -> false

let try_mark_awaiting_message = fun t -> Runtime_atomic.compare_and_set t.state Running Waiting_message

let try_mark_runnable_from_waiting_message = fun t -> Runtime_atomic.compare_and_set t.state Waiting_message Runnable
...
let message_count = fun t ->
  with_lock t
    (
      fun () -> Mailbox.size t.mailbox + t.save_queue_size
    )

let mailbox_count = fun t ->
  with_lock t
    (
      fun () -> Mailbox.size t.mailbox
    )

let save_queue_count = fun t ->
  with_lock t
    (
      fun () -> t.save_queue_size
    )
...
let next_saved_message = fun t ->
  with_lock t
    (
      fun () -> pop_save_queue t
    )

let next_mailbox_message = fun t ->
  with_lock t
    (
      fun () -> Mailbox.next t.mailbox
    )
...
let send_message = fun t msg ->
  if is_alive t then
    let envelope = Message.envelope msg in Mailbox.queue t.mailbox envelope
...
let has_no_ready_tokens = fun t ->
  with_lock t
    (
      fun () -> List.is_empty t.ready_tokens
    )
...
let set_receive_timeout = fun t timer_id ->
  ...
  with_lock t
    (
      fun () -> t.receive_timeout <- Some timer_id
    )
...
let clear_receive_timeout = fun t ->
  ...
  with_lock t
    (
      fun () -> t.receive_timeout <- None
    )

let receive_timeout = fun t ->
  with_lock t
    (
      fun () -> t.receive_timeout
    )
...
let set_syscall_timeout = fun t timer_id ->
  ...
  with_lock t
    (
      fun () -> t.syscall_timeout <- Some timer_id
    )
...
let clear_syscall_timeout = fun t ->
  ...
  with_lock t
    (
      fun () -> t.syscall_timeout <- None
    )

let syscall_timeout = fun t ->
  with_lock t
    (
      fun () -> t.syscall_timeout
    )
...
let unlink = fun proc target_pid ->
  with_lock proc
    (
      fun () -> proc.links <- List.filter proc.links ~fn:(
        fun pid -> not (Pid.equal pid target_pid)
      )
    )
...
let demonitor = fun proc ref ->
  with_lock proc
    (
      fun () -> proc.monitors <- List.filter proc.monitors ~fn:(
        fun (r, _) ->
          match ref, r with
...
let get_links = fun proc ->
  with_lock proc
    (
      fun () -> proc.links
    )

let get_monitors = fun proc ->
  with_lock proc
    (
      fun () -> proc.monitors
    )

let get_monitored_by = fun proc ->
  with_lock proc
    (
      fun () -> proc.monitored_by
    )
...
let remove_monitored_by = fun proc monitor_pid ref ->
  with_lock proc
    (
      fun () -> proc.monitored_by <- List.filter proc.monitored_by ~fn:(
        fun (pid, r) ->
          not
            (
...
let is_linked = fun proc pid ->
  with_lock proc
    (
      fun () -> List.contains proc.links ~value:pid
    )

let is_monitoring = fun proc pid ->
  with_lock proc
    (
      fun () -> List.any proc.monitors ~fn:(
        fun (_, monitored_pid) -> Pid.equal pid monitored_pid
      )
    )
