...
let loop = fun ~(pop_local:worker -> process_slot option) ~(step_process:t -> domain_context -> process_slot -> unit) ~(attempt_steal:t -> worker -> bool) ~(wait_for_local_work:t -> worker -> process_slot option) (
  runtime: t
) (worker: worker) ->
