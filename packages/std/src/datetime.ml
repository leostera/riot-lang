...
let from_naive : naive -> tz:Tz.t -> t = fun naive ~tz ->
    ...
    | Tz.Local ->
        ...
        let local_seconds = normalized_tm.tm_hour * 3_600
        + normalized_tm.tm_min * 60
        + normalized_tm.tm_sec in
...
module Parser = struct
  ...
  let parse_2digits = fun s pos ->
      if pos + 2 > String.length s then
        ...
        try Some (int_of_string (String.sub s pos 2)) with
        | _ -> None
  ...
  let parse_4digits = fun s pos ->
      if pos + 4 > String.length s then
        ...
        try Some (int_of_string (String.sub s pos 4)) with
        | _ -> None
