...
let base64 hash =
  ...
  let rec encode i =
    if i < len then (
      ...
      let b2 =
        if i + 1 < len then Bytes.get bytes (i + 1) |> Char.code else 0
      in
      let b3 =
        if i + 2 < len then Bytes.get bytes (i + 2) |> Char.code else 0
      in
      ...
      if i + 2 < len then Buffer.add_char buf b64_chars.[b3 land 0x3f]
      else Buffer.add_char buf '=';
...
let to_int hash = Int64.to_int (to_int64 hash)
