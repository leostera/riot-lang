...
let days_from_0_to_1970 = 719528
let days_from_0_to_10000 = 2932897
...
let last_day_of_month ~year ~month =
  match month with
  | 1 | 3 | 5 | 7 | 8 | 10 | 12 -> 31
  | 4 | 6 | 9 | 11 -> 30
...
let is_valid_date { year; month; day } =
  ...
  else day <= last_day_of_month ~year ~month
...
let date_to_gregorian_days { year; month; day } =
  if not (is_valid_date { year; month; day }) then
    panic ("date_to_gregorian_days: invalid date " ^ string_of_int year ^ "-" ^ string_of_int month ^ "-" ^ string_of_int day)
  else dy year + dm month + df year month + day - 1

(** Convert day-of-year to date within a year *)
let year_day_to_date year day_of_year =
  let extra_day = if is_leap_year year then 1 else 0 in
  let rec find_month m =
    ...
    let days_in_month =
      ...
      | _ -> panic "year_day_to_date: invalid month"
    in
    if day_of_year < days_before + days_in_month then (m, day_of_year - days_before + 1)
    else find_month (m + 1)
  in
  find_month 1

(** Convert gregorian days to year and day-of-year *)
let day_to_year days =
  if days < 0 then panic "day_to_year: negative days";
  (* Estimate year using average days per year *)
  let y_max = days / days_per_year in
  let y_min = days / days_per_leap_year in
  (* Binary search refinement *)
  let rec refine y_min y_max =
    if y_min >= y_max then (y_min, days - dy y_min)
    else
      let y_mid = (y_min + y_max) / 2 in
      let d_mid = dy y_mid in
      let mid_length =
        if is_leap_year y_mid then days_per_leap_year else days_per_year
      in
      if days < d_mid then refine y_min (y_mid - 1)
      else if days - d_mid >= mid_length then refine (y_mid + 1) y_max
      else (y_mid, days - d_mid)
  in
  refine y_min y_max

let gregorian_days_to_date days =
...
let naive_to_gregorian_seconds date time =
...
let gregorian_seconds_to_naive secs =
...
let day_of_week ({ year; month; day } as date) =
  if not (is_valid_date date) then
    panic ("day_of_week: invalid date " ^ string_of_int year ^ "-" ^ string_of_int month ^ "-" ^ string_of_int day)
  else
    let days = date_to_gregorian_days date in
    ((days + 5) mod 7) + 1
...
let gregorian_days_of_iso_w01_1 year =
  ...
  if dow <= 4 then jan1 - dow + 1 else jan1 + 7 - dow + 1

let iso_week_number ({ year; month; day } as date) =
  if not (is_valid_date date) then
    panic ("iso_week_number: invalid date " ^ string_of_int year ^ "-" ^ string_of_int month ^ "-" ^ string_of_int day);
  ...
  else (* d >= w01_1_next_year *)
    (* Next year, week 01 *)
    { year = year + 1; week = 1 }

(** {1 Time Conversions} *)

let time_to_seconds { hour; minute; second } =
  (hour * seconds_per_hour) + (minute * seconds_per_minute) + second

let seconds_to_time secs =
