...
let dy = fun year ->
  if year <= 0 then
    ...
    let x = year - 1 in (x / 4) - (x / 100) + (x / 400) + (x * days_per_year) + days_per_leap_year
...
let gregorian_days_to_date = fun days ->
  ...
  let month, day = year_day_to_date year day_of_year in { year; month; day }

(** {1 Gregorian Seconds Conversions} *)
let naive_to_gregorian_seconds = fun date time ->
  let days = date_to_gregorian_days date in
  let secs = (time.hour * seconds_per_hour) + (time.minute * seconds_per_minute) + time.second in (days * seconds_per_day) + secs
...
let gregorian_seconds_to_naive = fun secs ->
  ...
  let second = remaining mod seconds_per_minute in (date, { hour; minute; second })
...
let day_of_week = fun ({ year; month; day } as date) ->
  if not (is_valid_date date) then
    ...
    let days = date_to_gregorian_days date in int_to_weekday (((days + 5) mod 7) + 1)
...
let gregorian_days_of_iso_w01_1 = fun year ->
  ...
  *)
  ...
  else jan1 + 7 - (weekday_to_int dow) + 1
...
(** {1 Time Conversions} *)
let time_to_seconds = fun { hour; minute; second } -> (hour * seconds_per_hour) + (minute * seconds_per_minute) + second
...
let seconds_to_time = fun secs ->
  ...
  let minute = remaining / seconds_per_minute in
  let second = remaining mod seconds_per_minute in { hour; minute; second }
...
let time_difference = fun ~src_date ~src_time ~dst_date ~dst_time ->
  ...
  let secs2 = naive_to_gregorian_seconds dst_date dst_time in seconds_to_daystime (secs2 - secs1)
