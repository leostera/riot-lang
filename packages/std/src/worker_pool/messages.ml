...
type worker_to_coordinator =
  | TaskCompleted of Pid.t
  (** Worker finished a task and is ready for more *)
...
type owner_to_coordinator =
  ...
  | SendTaskToWorker : 'task worker * 'task * 'task Ref.t -> owner_to_coordinator
...
type Message.t +=
  ...
  | ToWorker of coordinator_to_worker
      (** Message to worker from coordinator *)
