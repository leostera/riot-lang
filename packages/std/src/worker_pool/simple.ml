...
type Message.t +=
  | TaskResult : { idx: int; result: 'result; result_ref: 'result Ref.t } -> Message.t
  | TaskError : { idx: int; exn: exn; result_ref: 'result Ref.t } -> Message.t
  | Completed : { results: (int * 'result) list; result_ref: 'result Ref.t } -> Message.t
  | Failed : { idx: int; exn: exn; result_ref: 'result Ref.t } -> Message.t

let rec loop: type task res. (task, res) state -> (unit, Actor.exit_reason) result = fun state ->
  let selector: ([`WorkerReady of task worker | `TaskResult of int * res | `TaskError of int * exn]) selector = fun msg ->
    match msg with
  ...
  | `WorkerReady worker -> (
    ...
    | None ->
        ...
        if state.tasks_in_flight = 0 then
          (
            let results = List.sort state.results ~compare:(
              fun (left_idx, _) (right_idx, _) -> Int.compare left_idx right_idx
            ) in
...
let init = fun ~owner ~concurrency ~tasks ~result_ref ~fn () ->
  ...
  let indexed_tasks, _ = List.fold_left tasks ~init:([], 0) ~fn:(
    fun (acc, idx) task -> ((idx, task) :: acc, idx + 1)
  ) in
  let indexed_tasks = List.reverse indexed_tasks in
  let task_queue = Queue.create () in
  List.for_each indexed_tasks ~fn:(
    fun t -> Queue.push task_queue ~value:t
  );
  let results = [] in
...
*)
let run: type task result. ?concurrency:int -> tasks:task list -> fn:(task -> result) -> unit -> (int * result) list = fun ?(concurrency = Thread.available_parallelism) ~tasks ~fn () ->
