...
let for_each = fun set ~fn -> Hashmap.for_each set ~fn:(
  fun value () -> fn value
)

let fold_left = fun set ~init ~fn -> Hashmap.fold_left set ~init ~fn:(
  fun acc value () -> fn acc value
)
...
let is_subset = fun left right -> fold_left left ~init:true ~fn:(
  fun acc value -> acc && contains right ~value
)

let is_superset = fun left right -> is_subset right left

let is_disjoint = fun left right -> fold_left left ~init:true ~fn:(
  fun acc value -> acc && not (contains right ~value)
)
...
let mut_iter: type value. value t -> value Iter.MutIterator.t = fun set ->
  let module SetIter = struct
    type state = { mutable items: value list }
