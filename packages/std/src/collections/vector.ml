...
type 'value t = { mutable data: 'value array; mutable length: int }
...
let with_capacity = fun ~size -> { data = Array.make ~count:(Int.max 0 size) ~value:(dangerously_cast_value 0); length = 0 }

let length = fun vector -> vector.length

let len = length

let is_empty = fun vector -> Int.equal vector.length 0
  ...
  if current_capacity < required_capacity then
    let grown_capacity =
      if Int.equal current_capacity 0 then
        4
      else Int.mul current_capacity 2
    in
...
let reserve = fun vector ~size ->
  ...
  else ensure_capacity vector (Int.add vector.length size)
...
let get = fun vector ~at ->
  if at < 0 || at >= vector.length then
    None
  else Some (Array.get_unchecked vector.data ~at)
...
let insert = fun vector ~at ~value ->
  if at < 0 || at > vector.length then
    ...
    (
      ensure_capacity vector (Int.add vector.length 1);
      Array.blit vector.data ~src_offset:at ~dst:vector.data ~dst_offset:(Int.add at 1) ~len:(Int.sub vector.length at);
      Array.set_unchecked vector.data ~at ~value;
      vector.length <- Int.add vector.length 1
    )
...
let remove = fun vector ~at ->
  if at < 0 || at >= vector.length then
    ...
    let value = Array.get_unchecked vector.data ~at in Array.blit vector.data ~src_offset:(Int.add at 1) ~dst:vector.data ~dst_offset:at ~len:(Int.sub (Int.sub vector.length at) 1);
  vector.length <- Int.sub vector.length 1;
  Some value
...
let split_off = fun vector ~at ->
  if at < 0 || at > vector.length then
    ...
    let next = with_capacity ~size:moved in Array.blit vector.data ~src_offset:at ~dst:next.data ~dst_offset:0 ~len:moved;
  next.length <- moved;
  vector.length <- at;
  next
...
let sort_with = fun vector ~compare ->
  let rec shift_left item index =
    if index < 0 then
      ...
      | Order.LT | Order.EQ -> Int.add index 1
      | Order.GT ->
  ...
  for index = 1 to Int.sub vector.length 1 do
    ...
    let destination = shift_left item (Int.sub index 1) in Array.set_unchecked vector.data ~at:destination ~value:item
  done
...
let first = fun vector ->
  if Int.equal vector.length 0 then
    None
  else Some (Array.get_unchecked vector.data ~at:0)

let last = fun vector ->
  if Int.equal vector.length 0 then
    None
  else Some (Array.get_unchecked vector.data ~at:(Int.sub vector.length 1))

let from_list = fun values ->
  let data = Array.from_list values in { data; length = Array.length data }

let iter: type item. item t -> item Iter.Iterator.t = fun vector ->
  let module VecIter = struct
    type state = { vector: item t; position: int }
    ...
    let next = fun state ->
      if state.position >= state.vector.length then
        ...
        let item = Array.get_unchecked state.vector.data ~at:state.position in (Some item, { state with position = Int.add state.position 1 })

    let size = fun state -> Int.max 0 (Int.sub state.vector.length state.position)
  end in
  Iter.Iterator.make (module VecIter) { vector; position = 0 }

let mut_iter: type item. item t -> item Iter.MutIterator.t = fun vector ->
  let module VecIter = struct
    type state = { vector: item t; mutable position: int }
    ...
    let next = fun state ->
      if state.position >= state.vector.length then
        ...
        let item = Array.get_unchecked state.vector.data ~at:state.position in state.position <- Int.add state.position 1;
      Some item
