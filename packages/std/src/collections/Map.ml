...
module type S = sig
  ...
  val compare: left:'value t -> right:'value t -> fn:('value -> 'value -> Kernel.Order.t) -> Kernel.Order.t
...
module Make (Order: Std_order.Ordered) = struct
  ...
  type +'value t =
    | Empty
    | Node of { left: 'value t; key: key; value: 'value; right: 'value t; height: int }
  ...
  let make_node = fun left ~key ~value right ->
    ...
    Node {
      ...
      value;
      right;
      height = if left_height >= right_height then
        left_height + 1
      else right_height + 1
    }
  ...
  let balance = fun left ~key ~value right ->
    ...
    if left_height > right_height + 2 then
      ...
      | Node { left = left_left; key = left_key; value = left_value; right = left_right; _ } ->
          if height left_left >= height left_right then
            make_node left_left ~key:left_key ~value:left_value (make_node left_right ~key ~value right)
          else
            match left_right with
            | Empty -> panic "balance expected a left-right branch"
            | Node { left = left_right_left; key = pivot_key; value = pivot_value; right = left_right_right; _ } -> make_node (make_node left_left ~key:left_key ~value:left_value left_right_left) ~key:pivot_key ~value:pivot_value (make_node left_right_right ~key ~value right)
    else
      if right_height > left_height + 2 then
        match right with
        | Empty -> panic "balance expected a right branch"
        | Node { left = right_left; key = right_key; value = right_value; right = right_right; _ } ->
            if height right_right >= height right_left then
              make_node (make_node left ~key ~value right_left) ~key:right_key ~value:right_value right_right
            else
              match right_left with
              | Empty -> panic "balance expected a right-left branch"
              | Node { left = right_left_left; key = pivot_key; value = pivot_value; right = right_left_right; _ } -> make_node (make_node left ~key ~value right_left_left) ~key:pivot_key ~value:pivot_value (make_node right_left_right ~key:right_key ~value:right_value right_right)
      else make_node left ~key ~value right
  ...
  let rec insert = fun map ~key ~value ->
    ...
    | Node ({ left; key = current_key; value = current_value; right; height } as node) -> (
      match Order.compare key current_key with
      | Kernel.Order.EQ ->
          if Ptr.equal current_value value then
            map
          else Node ({ node with key; value })
      | Kernel.Order.LT ->
          let next_left = insert left ~key ~value in
          if Ptr.equal next_left left then
            map
          else balance next_left ~key:current_key ~value:current_value right
      | Kernel.Order.GT ->
          let next_right = insert right ~key ~value in
          if Ptr.equal next_right right then
            map
          else balance left ~key:current_key ~value:current_value next_right
    )
  ...
  and remove_minimum = function
    ...
    | Node { left; key; value; right; _ } -> balance (remove_minimum left) ~key ~value right
  and join = fun left ~key ~value right ->
    match left, right with
    | Empty, _ -> add_minimum ~key ~value right
    | _, Empty -> add_maximum left ~key ~value
    | Node { left = left_left; key = left_key; value = left_value; right = left_right; height = left_height }, Node { left = right_left; key = right_key; value = right_value; right = right_right; height = right_height } ->
        if left_height > right_height + 2 then
          balance left_left ~key:left_key ~value:left_value (join left_right ~key ~value right)
        else
          if right_height > left_height + 2 then
            balance (join left ~key ~value right_left) ~key:right_key ~value:right_value right_right
          else make_node left ~key ~value right
  and add_minimum = fun ~key ~value map ->
    match map with
    | Empty -> singleton ~key ~value
    | Node { left; key = current_key; value = current_value; right; _ } -> balance (add_minimum ~key ~value left) ~key:current_key ~value:current_value right
  and add_maximum = fun map ~key ~value ->
    match map with
    | Empty -> singleton ~key ~value
    | Node { left; key = current_key; value = current_value; right; _ } -> balance left ~key:current_key ~value:current_value (add_maximum right ~key ~value)
  and concat = fun left right ->
    match left, right with
    | (Empty, map) | (map, Empty) -> map
    | _ ->
        let key, value = minimum_unchecked right in join left ~key ~value (remove_minimum right)
  and merge_branches = fun left right ->
    match left, right with
    | (Empty, map) | (map, Empty) -> map
    | _ ->
        let key, value = minimum_unchecked right in balance left ~key ~value (remove_minimum right)
  ...
  let rec remove = fun map ~key ->
    match map with
    | Empty -> Empty
    | Node { left; key = current_key; value; right; _ } -> (
      match Order.compare key current_key with
      ...
      | Kernel.Order.LT ->
          ...
          if Ptr.equal next_left left then
            map
          else balance next_left ~key:current_key ~value right
      | Kernel.Order.GT ->
          let next_right = remove right ~key in
          if Ptr.equal next_right right then
            map
          else balance left ~key:current_key ~value next_right
    )
  ...
  let rec split = fun map ~key ->
    ...
    | Node { left; key = current_key; value; right; _ } -> (
      ...
      | Kernel.Order.LT ->
          let left_left, found, left_right = split left ~key in (left_left, found, join left_right ~key:current_key ~value right)
      | Kernel.Order.GT ->
          let right_left, found, right_right = split right ~key in (join left ~key:current_key ~value right_left, found, right_right)
    )
  ...
  let rec merge = fun ~left ~right ~fn ->
    match left, right with
    | Empty, Empty -> Empty
    | Node { left = left_left; key; value; right = left_right; height = left_height }, _ when left_height >= height right ->
        let right_left, right_value, right_right = split right ~key in concat_or_join (merge ~left:left_left ~right:right_left ~fn) ~key ~value:(fn ~key ~left:(Some value) ~right:right_value) (merge ~left:left_right ~right:right_right ~fn)
    | _, Node { left = right_left; key; value; right = right_right; _ } ->
        let left_left, left_value, left_right = split left ~key in concat_or_join (merge ~left:left_left ~right:right_left ~fn) ~key ~value:(fn ~key ~left:left_value ~right:(Some value)) (merge ~left:left_right ~right:right_right ~fn)
  ...
  let rec union = fun ~left ~right ~fn ->
    ...
    | Node { left = left_left; key = left_key; value = left_value; right = left_right; height = left_height }, Node { left = right_left; key = right_key; value = right_value; right = right_right; height = right_height } ->
        if left_height >= right_height then
          ...
          | Some right_value -> concat_or_join merged_left ~key:left_key ~value:(fn ~key:left_key ~left:left_value ~right:right_value) merged_right
        else
          ...
          | Some left_value -> concat_or_join merged_left ~key:right_key ~value:(fn ~key:right_key ~left:left_value ~right:right_value) merged_right
  ...
  let rec fold_left = fun map ~init ~fn ->
    ...
    | Node { left; key; value; right; _ } ->
        ...
        let acc = fn acc key value in fold_left right ~init:acc ~fn
  ...
  let rec filter = fun values ~fn ->
    ...
    | Node { left; key; value; right; _ } ->
        ...
        else concat next_left next_right
  ...
  let rec partition = fun values ~fn ->
    ...
    | Node { left; key; value; right; _ } ->
        ...
        else (concat left_true right_true, join left_false ~key ~value right_false)
  ...
  let rec push_left = fun map enumeration ->
    match map with
    | Empty -> enumeration
    | Node { left; key; value; right; _ } -> push_left left (More (key, value, right, enumeration))
    ...
    let rec loop left right =
      ...
      | More (left_key, left_value, left_right, left_rest), More (right_key, right_value, right_right, right_rest) -> (
        ...
        | Kernel.Order.EQ -> (
          ...
          | Kernel.Order.EQ -> loop (push_left left_right left_rest) (push_left right_right right_rest)
          | Kernel.Order.LT | Kernel.Order.GT as order -> order
        )
        | Kernel.Order.LT | Kernel.Order.GT as order -> order
      )
    in
  ...
  let equal = fun ~left ~right ~fn ->
    let rec loop left right =
      ...
      | More (left_key, left_value, left_right, left_rest), More (right_key, right_value, right_right, right_rest) -> (
        match Order.compare left_key right_key with
        | Kernel.Order.EQ -> fn left_value right_value && loop (push_left left_right left_rest) (push_left right_right right_rest)
        | Kernel.Order.LT | Kernel.Order.GT -> false
      )
    in
    loop (push_left left End) (push_left right End)
  ...
  let to_list =
    let rec loop acc = function
      | Empty -> acc
      | Node { left; key; value; right; _ } -> loop ((key, value) :: loop acc right) left
    in
    fun map -> loop [] map

  let from_list = fun entries -> List.fold_left entries ~acc:empty ~fn:(
    fun map (key, value) -> insert map ~key ~value
  )

  let all = fun map ~fn -> fold_left map ~init:true ~fn:(
    fun acc key value -> acc && fn key value
  )

  let any = fun map ~fn -> fold_left map ~init:false ~fn:(
    fun acc key value -> acc || fn key value
  )
end
