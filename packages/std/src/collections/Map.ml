...
module Make (Order : Order.Ordered) = struct
  ...
  and add_minimum = fun ~key ~value map ->
    ...
    | Node {
      ...
      right;
      _
    } -> balance (add_minimum ~key ~value left) ~key:current_key ~value:current_value right
  ...
  and add_maximum = fun map ~key ~value ->
    ...
    | Node {
      ...
      right;
      _
    } -> balance left ~key:current_key ~value:current_value (add_maximum right ~key ~value)
    ...
    | _, Node {
      ...
      _
        ...
        concat_or_join
          ...
          (merge ~left:left_right ~right:right_right ~fn)
    | _ ->
        panic "merge reached an impossible state"
    ...
    | Node {
      ...
      height=right_height
        ...
        if left_height >= right_height then
          ...
          | Some right_value -> concat_or_join
            merged_left
            ~key:left_key
          ...
          | Some left_value -> concat_or_join
            merged_left
            ~key:right_key
