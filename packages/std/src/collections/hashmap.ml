...
type ('key, 'value) t = { mutable buckets: ('key * 'value) list array; mutable size: int }
...
let with_capacity = fun ~size ->
  let capacity = normalize_capacity size in { buckets = Array.make ~count:capacity ~value:[]; size = 0 }
...
let hash_index = fun map key ->
  let hash = caml_hash 10 100 0 key in (hash lsr 1) mod Array.length map.buckets
...
let rec replace_in_bucket = fun key value bucket ->
  ...
  | (existing_key, existing_value) :: rest ->
      if existing_key = key then
        ...
        let previous, rest, inserted_new = replace_in_bucket key value rest in (previous, (existing_key, existing_value) :: rest, inserted_new)
...
let rec remove_from_bucket = fun key bucket ->
  ...
  | (existing_key, existing_value) :: rest ->
      if existing_key = key then
        ...
        let removed, rest, did_remove = remove_from_bucket key rest in (removed, (existing_key, existing_value) :: rest, did_remove)
...
let insert_without_resize = fun map ~key ~value ->
  ...
  let previous, bucket, inserted_new = replace_in_bucket key value (Array.get_unchecked map.buckets ~at:index) in
...
let resize = fun map new_capacity ->
  ...
  map.size <- 0;
  Array.for_each old_buckets ~fn:(
    fun bucket -> List.for_each bucket ~fn:(
      fun (key, value) ->
...
let remove = fun map ~key ->
  let index = hash_index map key in
  let removed, bucket, did_remove = remove_from_bucket key (Array.get_unchecked map.buckets ~at:index) in
  Array.set_unchecked map.buckets ~at:index ~value:bucket;
  if did_remove then
    map.size <- map.size - 1;
  removed
...
let fold_left = fun map ~init ~fn -> Array.fold_left map.buckets ~acc:init ~fn:(
  fun acc bucket -> List.fold_left bucket ~acc:acc ~fn:(
    fun acc (key, value) -> fn acc key value
  )
)

let keys = fun map -> fold_left map ~init:[] ~fn:(
  fun acc key _value -> key :: acc
)

let values = fun map -> fold_left map ~init:[] ~fn:(
  fun acc _key value -> value :: acc
)

let for_each = fun map ~fn -> fold_left map ~init:() ~fn:(
  fun () key value ->
    fn key value;
    ()
)

let to_list = fun map -> fold_left map ~init:[] ~fn:(
  fun acc key value -> (key, value) :: acc
)
...
let or_insert = fun map ~key ~default ->
  ...
  | Some value -> value
  | None ->
      let _ = insert map ~key ~value:default in default
...
let mut_iter: type key value. (key, value) t -> (key * value) Iter.MutIterator.t = fun map ->
  let module MapIter = struct
    type state = { mutable items: (key * value) list }
