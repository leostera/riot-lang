(** # Result - Error handling with the Result type
    ...
    In addition to working with pattern matching, `Result` provides a wide variety
    of methods for functional composition:
    ...
    OCaml doesn't have Rust's `?` operator, but you can achieve similar
    early returns with [`and_then`] or monadic let operators if available.
    ...
    (* Transform errors *)
    let config =
      Fs.read (Path.v "config.json")
      |> Result.map_err (fun _ -> "Config file not found")
      |> Result.and_then parse_json
      |> Result.expect ~msg:"Failed to load configuration"
    ```
*)

type ('a, 'e) t = ('a, 'e) Stdlib.result = Ok of 'a | Error of 'e
(** The Result type - either [`Ok`] with a success value or [`Error`] with an error value *)
...
(** Returns `true` if the result is [`Ok`].
    ...
    let y = Error "Error message" in
    assert (not (Result.is_ok y))
    ```
*)
...
(** Returns `true` if the result is [`Error`].
    ...
    ```ocaml
    let x = Ok 2 in
    assert (not (Result.is_err x));
    
    let y = Error "Error message" in
    assert (Result.is_err y)
    ```
*)
...
(** Returns `true` if the result is [`Ok`] and the value matches the predicate.
    ...
    ```ocaml
    let x = Ok 2 in
    assert (Result.is_ok_and (fun v -> v > 1) x);
    assert (not (Result.is_ok_and (fun v -> v > 5) x));
    
    let y = Error "Error" in
    assert (not (Result.is_ok_and (fun v -> v > 0) y))
    ```
*)
...
(** Returns `true` if the result is [`Error`] and the error matches the predicate.
    ...
    ```ocaml
    let is_not_found = Result.is_err_and (fun e ->
      String.contains e "not found"
    ) in
    
    let x = Error "File not found" in
    assert (is_not_found x);
    
    let y = Ok 123 in
    assert (not (is_not_found y))
    ```
*)
...
(** Maps a `Result<'a, 'e>` to `Result<'b, 'e>` by applying a function to the [`Ok`] value.
    ...
    assert (double (parse_int "21") = Ok 42);
    assert (double (parse_int "abc") = Error "Not a number")
    ```
*)

val map_error : ('e -> 'f) -> ('a, 'e) t -> ('a, 'f) t
(** Maps a `Result<'a, 'e>` to `Result<'a, 'f>` by applying a function to the [`Error`] value.
    ...
    ```ocaml
    let add_context = Result.map_error (fun e ->
      Printf.sprintf "Database error: %s" e
    ) in
    
    let result = Error "Connection timeout" in
    assert (add_context result = Error "Database error: Connection timeout")
    ```
*)
...
(** Maps a `Result<'a, 'e>` to `Result<'a, 'f>` by applying a function to the [`Error`] value.
    ...
    ```ocaml
    let stringify_error = Result.map_err string_of_int in
    
    let x = Ok 2 in
    assert (stringify_error x = Ok 2);
    
    let y = Error 13 in
    assert (stringify_error y = Error "13")
    ```
*)
...
(** Returns the result of applying function to [`Ok`] value, or default if [`Error`].
    ...
    ```ocaml
    let x = Ok "foo" in
    assert (Result.map_or ~default:42 String.length x = 3);
    
    let y = Error "bar" in
    assert (Result.map_or ~default:42 String.length y = 42)
    ```
*)
...
(** Maps a `Result<'a, 'e>` to `'b` by applying fallback function to [`Error`],
    ...
    let x = Ok "foo" in
    assert (Result.map_or_else 
      ~default:(fun _ -> k * 2) 
      String.length x = 3);
    
    let y = Error "bar" in
    assert (Result.map_or_else 
      ~default:(fun e -> String.length e) 
      String.length y = 3)
    ```
*)
...
(** Returns the first result if [`Ok`], otherwise returns the second result.
    ...
    ```ocaml
    let x = Ok 2 in
    let y = Error "late error" in
    assert (Result.or_ x y = Ok 2);
    
    let x = Error "early error" in
    let y = Ok 2 in
    assert (Result.or_ x y = Ok 2);
    
    let x = Error "early" in
    let y = Error "late" in
    assert (Result.or_ x y = Error "late")
    ```
*)
...
(** Calls function on [`Error`] value if present, passes through [`Ok`].
    ...
    ```ocaml
    let sq x = Ok (x * x) in
    let err x = Error x in
    
    assert (Result.or_else (Ok 2) sq = Ok 2);
    assert (Result.or_else (Ok 2) err = Ok 2);
    assert (Result.or_else (Error 3) sq = Ok 9);
    assert (Result.or_else (Error 3) err = Error 3)
    ```
*)
...
(** Returns the contained [`Ok`] value, consuming the result.
    ...
    (* This will panic: *)
    let y = Error "emergency failure" in
    Result.unwrap y  (* panic: "Called Result.unwrap on Error: emergency failure" *)
    ```
    
    ## Note
    
    Generally, prefer [`expect`] which provides a more helpful panic message,
    or [`unwrap_or`] / [`unwrap_or_else`] for non-panicking alternatives.
*)
...
(** Returns the contained [`Ok`] value or a provided default.
    ...
    let default = 2 in
    let x = Ok 9 in
    assert (Result.unwrap_or ~default x = 9);
    
    let y = Error "error" in
    assert (Result.unwrap_or ~default y = 2)
    ```
*)
...
(** Returns the contained [`Ok`] value or computes it from a closure.
    ...
    ```ocaml
    let count s = String.length s in
    assert (Result.unwrap_or_else ~fn:(fun () -> 2) (Ok 9) = 9);
    assert (Result.unwrap_or_else ~fn:(fun () -> count "foo") (Error "bar") = 3)
    ```
*)
...
(** Returns the contained [`Error`] value, consuming the result.
    ...
    ```ocaml
    let x = Error "emergency failure" in
    assert (Result.unwrap_err x = "emergency failure")
    
    (* This will panic: *)
    let y = Ok 2 in
    Result.unwrap_err y  (* panic: "Called Result.unwrap_err on Ok: 2" *)
    ```
*)
...
(** Returns the contained [`Ok`] value, consuming the result.
    ...
    let config = 
      Fs.read (Path.v "config.json")
      |> Result.expect ~msg:"Config file is required for the application"
    ```
    ...
    let process_config path =
      Fs.read path
      |> Result.expect ~msg:"config file should exist at startup"
    ```
    
    **Hint**: If you're having trouble choosing a message, consider using [`unwrap`] instead.
*)
...
(** Returns the contained [`Error`] value, consuming the result.
    ...
    Panics if the value is an [`Ok`], with the provided message and the
    success value.
    ...
    ```ocaml
    let x = Error "emergency failure" in
    assert (Result.expect_err ~msg:"Testing errors" x = "emergency failure")
    ```
*)
...
(** Converts from `Result<'a, 'e>` to [`Option<'a>`].
    ...
    ```ocaml
    let x = Ok 2 in
    assert (Result.ok_value x = Some 2);
    
    let y = Error "Nothing here" in
    assert (Result.ok_value y = None)
    ```
*)
...
(** Converts from `Result<'a, 'e>` to [`Option<'e>`].
    ...
    ```ocaml
    let x = Ok 2 in
    assert (Result.err_value x = None);
    
    let y = Error "Nothing here" in
    assert (Result.err_value y = Some "Nothing here")
    ```
*)
...
(** Calls the provided closure on the contained [`Ok`] value (if any).
    ...
    let result =
      parse_int "4"
      |> Result.inspect (fun x -> Printf.printf "Original: %d\n" x)
      |> Result.map (fun x -> x * x)
      |> Result.inspect (fun x -> Printf.printf "Squared: %d\n" x)
    (* Prints:
       Original: 4
       Squared: 16
       Returns: Ok 16 *)
...
(** Calls the provided closure on the contained [`Error`] value (if any).
    ...
    ```ocaml
    Fs.read (Path.v "config.json")
    |> Result.inspect_err (fun e -> 
         Log.warning "Failed to load config: %s" e)
    |> Result.unwrap_or ~default:default_config
    ```
*)
...
(** Calls function on [`Ok`] value if present, otherwise does nothing.
    ...
    ```ocaml
    let x = Ok 7 in
    Result.iter (fun v -> Printf.printf "Got: %d\n" v) x;
    (* Prints: Got: 7 *)
    
    let y = Error "nothing" in
    Result.iter (fun v -> Printf.printf "Got: %d\n" v) y
    (* Prints nothing *)
    ```
*)
...
(** Calls function on [`Error`] value if present, otherwise does nothing.
    ...
    ```ocaml
    let log_error = Result.iter_error (fun e ->
      Log.error "Operation failed: %s" e
    ) in
    
    log_error (Error "disk full");  (* Logs error *)
    log_error (Ok 123)              (* Does nothing *)
    ```
*)
...
(** Converts from `Result<'a, 'e>` to [`Option<'a>`].
    ...
    ```ocaml
    let x = Ok 2 in
    assert (Result.to_option x = Some 2);
    
    let y = Error "Nothing here" in
    assert (Result.to_option y = None)
    ```
*)
...
(** Converts from [`Option<'a>`] to `Result<'a, 'e>`.
    ...
    ```ocaml
    let x = Some "value" in
    assert (Result.of_option ~error:"no value" x = Ok "value");
    
    let y = None in
    assert (Result.of_option ~error:"no value" y = Error "no value")
    
    (* Common pattern with environment variables *)
    let port =
      Sys.getenv_opt "PORT"
      |> Result.of_option ~error:"PORT not set"
      |> Result.and_then (fun s ->
           try Ok (int_of_string s)
           with _ -> Error "PORT must be a number")
    ```
*)
...
(** Transposes a `Result` of an `Option` into an `Option` of a `Result`.
    ...
    ```ocaml
    let x = Ok (Some 5) in
    assert (Result.transpose x = Some (Ok 5));
    
    let y = Ok None in
    assert (Result.transpose y = None);
    
    let z = Error "error" in
    assert (Result.transpose z = Some (Error "error"))
    ```
*)
...
(** Converts from `Result<Result<'a, 'e>, 'e>` to `Result<'a, 'e>`.
    ...
    ```ocaml
    let x = Ok (Ok 2) in
    assert (Result.flatten x = Ok 2);
    
    let y = Ok (Error "late error") in
    assert (Result.flatten y = Error "late error");
    
    let z = Error "early error" in
    assert (Result.flatten z = Error "early error")
    
    (* Flattening is the reverse of nesting *)
    let nested = Ok (Ok 5) in
    assert (Result.flatten nested = Ok 5)
    ```
*)
...
(** Converts a list of `Result`s into a single `Result` containing a list.
    ...
    assert (Result.all results = Ok [1; 2; 3]);
    
    let with_error = [Ok 1; Error "oops"; Ok 3; Error "another"] in
    assert (Result.all with_error = Error "oops")
    
    (* Common pattern: validate multiple inputs *)
    let parse_numbers strings =
      strings
      |> List.map (fun s ->
           try Ok (int_of_string s)
           with _ -> Error (Printf.sprintf "'%s' is not a number" s))
      |> Result.all
    ```
*)
...
(** Applies one of two functions depending on the result variant.
    ...
    ```ocaml
    let to_string = Result.fold
      ~ok:string_of_int
      ~error:(fun e -> Printf.sprintf "Error: %s" e) in
    
    assert (to_string (Ok 42) = "42");
    assert (to_string (Error "failed") = "Error: failed")
    
    (* Convert any result to an exit code *)
    let to_exit_code = Result.fold ~ok:(fun _ -> 0) ~error:(fun _ -> 1)
    ```
*)
