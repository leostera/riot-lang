(** # Command - Process spawning and management
    ...
    let result = 
      Command.make "ls" ~args:["-la"]
      |> Command.output in
    
    match result with
    | Ok output ->
        println "Files:\n%s" output.stdout;
        println "Exit code: %d" output.status
    | Error (SystemError msg) ->
        println "Failed: %s" msg
    
    (* Check exit status only *)
    match Command.make "test" ~args:["-f"; "file.txt"] |> Command.status with
    | Ok 0 -> println "File exists"
    | Ok _ -> println "File not found"
    | Error _ -> println "Command failed"
    ```
...
(** System-level errors when spawning or running commands.
    ...
    Note: A command exiting with non-zero status is NOT an error.
    Only failures to start the process return [`Error`].
*)
...
(** Creates a new command configuration.
    ...
    ```ocaml
    (* Simple command *)
    let ls = Command.make "ls" in
    
    (* With arguments *)
    let grep = Command.make "grep" ~args:["-r"; "TODO"; "."] in
    
    (* With working directory *)
    let npm = Command.make "npm" 
      ~args:["install"]
      ~cwd:(Path.v "/project") in
    
    (* With environment *)
    let build = Command.make "cargo"
      ~args:["build"; "--release"]
      ~env:[("RUST_BACKTRACE", "1")] in
    
    (* Complex example *)
    let deploy = Command.make "kubectl"
    ...
    The command is resolved using the system's PATH environment variable
...
(** Executes command and captures its output.
    ...
    Runs the command as a child process, waits for completion, and
    ...
    match Command.make "git" ~args:["status"] |> Command.output with
    | Ok out when out.status = 0 ->
        println "Git status:\n%s" out.stdout
    | Ok out ->
        Printf.eprintf "Git failed with code %d:\n%s" 
          out.status out.stderr
    | Error (SystemError e) ->
        Printf.eprintf "Failed to run git: %s\n" e
    
    (* Parse JSON output *)
    let get_docker_images () =
      Command.make "docker" ~args:["images"; "--format"; "json"]
      |> Command.output
      |> Result.and_then (fun out ->
           if out.status = 0 then
             Data.Json.parse out.stdout
           else
             Error (SystemError out.stderr))
    ```
    
    ## Output Limits
    
    Both stdout and stderr are fully captured in memory. For commands
    that produce large output, consider using pipes or temporary files.
    
    ## Character Encoding
    
    Output is expected to be UTF-8. Invalid UTF-8 bytes may be replaced
...
(** Executes command and returns only its exit status.
    ...
    Runs the command as a child process without capturing output.
    ...
    match Command.make "make" ~args:["test"] |> Command.status with
    | Ok 0 -> println "Tests passed!"
    | Ok n -> Printf.eprintf "Tests failed with code %d\n" n
    | Error (SystemError e) -> Printf.eprintf "Cannot run tests: %s\n" e
    
    (* Use as a condition *)
    let has_git () =
      Command.make "git" ~args:["--version"]
      |> Command.status
      |> Result.map (fun s -> s = 0)
      |> Result.unwrap_or ~default:false
    
    (* Run interactive commands *)
    let edit_file path =
      Command.make (Sys.getenv_opt "EDITOR" |> Option.unwrap_or ~default:"vi")
        ~args:[Path.to_string path]
      |> Command.status
    ```
