(** # Data.Sexp - S-expression parsing and printing
    ...
    A library for working with S-expressions (symbolic expressions),
    a simple and readable data format commonly used in Lisp-like languages
    ...
    (* Parse from string *)
    match Sexp.of_string "(name Alice (age 30))" with
    | Ok sexp ->
        Sexp.to_string sexp  (* "(name Alice (age 30))" *)
    | Error msg ->
        Log.error "Parse error: %s" msg
    
    (* Build programmatically *)
    let person = Sexp.list [
      Sexp.atom "person";
      Sexp.list [Sexp.atom "name"; Sexp.atom "Bob"];
      Sexp.list [Sexp.atom "age"; Sexp.atom "25"]
    ] in
    Sexp.to_string person
    ...
    ```ocaml
    let config = Sexp.of_string "(config (debug true) (port 8080))"
      |> Result.unwrap in
    
    match Sexp.to_list config with
    | Some (Atom "config" :: fields) ->
        (* Look up specific field *)
        (match Sexp.assoc "port" fields with
    ...
    ```ocaml
    let complex = Sexp.list [
      Sexp.atom "database";
      Sexp.list [Sexp.atom "host"; Sexp.atom "localhost"];
      Sexp.list [Sexp.atom "credentials";
        Sexp.list [Sexp.atom "user"; Sexp.atom "admin"];
        Sexp.list [Sexp.atom "password"; Sexp.atom "secret"]
      ]
    ] in
    
    print_endline (Sexp.pretty_print complex)
...
(** Reads and parses S-expressions from a file.
    ...
    ```ocaml
    match Sexp.parse_file "config.sexp" with
    | Ok sexps ->
        List.iter (fun s ->
          Log.info "Config: %s" (Sexp.to_string s)
        ) sexps
    | Error msg ->
...
(** Pretty-prints an S-expression with indentation for readability.
    ...
    ```ocaml
    let nested = Sexp.list [
      Sexp.atom "outer";
      Sexp.list [Sexp.atom "inner"; Sexp.atom "value"]
    ] in
    
    Sexp.pretty_print nested
    (* "(outer
          (inner value))" *)
    ```
*)
...
(** Writes an S-expression to a file.
    ...
    match Sexp.to_file "config.sexp" config with
    | Ok () -> Log.info "Config saved"
    | Error msg -> Log.error "Write failed: %s" msg
    ```
*)
...
(** Association list lookup - finds the value for a key in a list of
    key-value pairs.
