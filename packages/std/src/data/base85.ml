...
let encode_bytes = fun bytes ->
  ...
  let rec encode_block = fun i ->
    if i >= len then
      ...
      let b1 =
        if remaining > 1 then
          Char.code (Bytes.get bytes (i + 1))
        else
          0
      in
      let b2 =
        if remaining > 2 then
          Char.code (Bytes.get bytes (i + 2))
        else
          0
      in
      let b3 =
        if remaining > 3 then
          Char.code (Bytes.get bytes (i + 3))
        else
          0
      in
...
let strip_delimiters = fun str ->
  ...
  let len = String.length str in
  if len >= 2 then
    if String.sub str (len - 2) 2 = "~>" then
...
let decode_bytes = fun str ->
  ...
  let rec decode_group = fun () ->
    if !cursor >= len then
      Ok (Buffer.contents result |> Bytes.unsafe_of_string)
    else
      let c = str.[!cursor] in
      if c = 'z' then
        (
