...
let error_to_string = function
  ...
  | Parse_error { position; context; reason } -> 
      format "Parse error at position %d (context: %s): %s" position context reason
  ...
  | Unexpected_char { position; found; expected } ->
      format "Unexpected character '%c' at position %d (expected %s)" found position expected
      ...
      | Ok content ->
          ...
          let rec parse_array () =
            ...
            let rec parse_items () =
              ...
              match current_char () with
              | ']' -> advance (); Array (List.rev !items)
              | _ ->
                  let value = parse_value () in
                  items := value :: !items;
                  skip_noise ();
                  (match peek () with
                  | Some ',' -> advance (); parse_items ()
                  | Some ']' -> advance (); Array (List.rev !items)
                  | None -> raise (Parse_exception (Unterminated_array { position = start_pos }))
                  | Some c -> parse_items ()) (* Allow newlines between items *)
            in
            parse_items ()
          ...
          try
            while not (at_end ()) do
              ...
              | '[' ->
                  ...
                  | Some name ->
                      Log.trace "[TOML] Saving section '%s' with %d items" name (List.length !current_items);
          ...
          | Exit ->
              ...
              (match !current_section with
              | Some name ->
                  Log.trace "[TOML] Saving final section '%s' with %d items" name (List.length !current_items);
                  sections := { name; items = List.rev !current_items } :: !sections
              | None -> ());
              
              let all_sections = List.rev !sections in
              Log.trace "[TOML] Successfully parsed %d sections" (List.length all_sections);
