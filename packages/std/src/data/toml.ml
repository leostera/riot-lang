...
let error_to_string = function
  ...
  | Parse_error { position; context; reason } -> "Parse error at position " ^ Int.to_string position ^ " (context: " ^ context ^ "): " ^ reason
  ...
  | Unexpected_char { position; found; expected } -> "Unexpected character '" ^ String.make ~len:1 ~char:found ^ "' at position " ^ Int.to_string position ^ " (expected " ^ expected ^ ")"
...
let parse = fun content ->
  ...
  let peek () =
    if at_end () then
      None
    else Some (String.get_unchecked content ~at:!pos)
  in
  let advance () =
    if not (at_end ()) then
      Cell.incr pos
  ...
  let current_char () =
    if at_end () then
      '\000'
    else String.get_unchecked content ~at:!pos
  in
  ...
  let parse_quoted_string () =
    ...
    if current_char () != '"' then
      raise (Parse_exception (Unexpected_char { position = !pos; found = current_char (); expected = "double-quote" }));
    advance ();
  ...
  let rec parse_array () =
    let start_pos = !pos in
    if current_char () != '[' then
      raise (Parse_exception (Unexpected_char { position = !pos; found = current_char (); expected = "[" }));
    advance ();
    let items = ref [] in
    let rec parse_items () =
      skip_noise ();
  ...
  and parse_inline_table () =
    ...
    let rec parse_items () =
      ...
      | _ -> (
        ...
        while (not (at_end ())) && current_char () != '=' && current_char () != '}' do advance () done;
        ...
        | '}' ->
            advance ();
            Table (List.reverse !items)
        | _ -> raise (Parse_exception (Parse_error { position = !pos; context = "inline table"; reason = "expected , or }" }))
      )
    in
    parse_items ()
  (* Parse a value *)
  and parse_value () =
    skip_noise ();
    if at_end () then
      raise (Parse_exception (Parse_error { position = !pos; context = "value"; reason = "unexpected end" }));
    match current_char () with
    | '"' -> parse_quoted_string ()
    | '[' -> parse_array ()
    | '{' -> parse_inline_table ()
    ...
    | '0' .. '9' | '-' | '+' ->
        ...
        if current_char () = '-' || current_char () = '+' then
          advance ();
        while not (at_end ()) && current_char () >= '0' && current_char () <= '9' do advance () done;
    ...
    | _ ->
        ...
        in
        advance_bare_string ();
        let str = String.trim (String.sub content ~offset:start ~len:(!pos - start)) in String str
  in
  (* Parse a key (identifier before =) *)
  let parse_key () =
    skip_ws ();
    let start = !pos in
    while (not (at_end ())) && current_char () != '=' do advance () done;
  ...
  let parse_section_header () =
    ...
    while (not (at_end ())) && current_char () != ']' do advance () done;
    if at_end () then
      raise (Parse_exception (Parse_error { position = start; context = "section"; reason = "unterminated" }));
    ...
    if is_array then
      (
        skip_ws ();
        if current_char () != ']' then
          raise (Parse_exception (Parse_error { position = !pos; context = "array section"; reason = "expected ]]" }));
        advance ()
      );
    skip_to_eol ();
    (name, is_array)
  ...
  let save_current_section () =
    ...
    | Some (name, true) ->
        let existing =
          ...
          | None -> []
        in
        array_sections := (name, Table (List.reverse !current_items) :: existing) :: assoc_remove name !array_sections
    | None ->
  ...
  try
    ...
    let rec insert_nested_table path value acc =
      ...
      | key :: rest ->
          ...
          let acc_without_key = List.filter acc ~fn:(
            fun ((k, _)) -> not (String.equal k key)
          ) in (key, Table updated_table) :: acc_without_key
    in
    (* Convert sections to nested tables *)
    let items = List.fold_left all_sections ~init:[] ~fn:(
      fun acc section ->
        if section.name = "" then
          section.items @ acc
        else
          (* Split dotted section names (e.g., "profile.debug" -> ["profile"; "debug"]) *)
          let path = String.split ~by:"." section.name in insert_nested_table path (Table section.items) acc
    ) in
    (* Add array sections as arrays *)
    let items_with_arrays = List.fold_left !array_sections ~init:items ~fn:(
      fun acc ((name, tables)) ->
        (* Split dotted array section names (e.g., "log.handler" -> ["log"; "handler"]) *)
        let path = String.split ~by:"." name in insert_nested_table path (Array (List.reverse tables)) acc
    ) in Ok (Table (List.reverse items_with_arrays))
  with
  | Parse_exception err -> Error err
  | exn -> Error (Parse_error { position = !pos; context = "unknown"; reason = Kernel.Exception.to_string exn })
...
let rec to_string = fun ?(indent = 0) value ->
  ...
  | Array items ->
      let items_str = String.concat ", " (List.map items ~fn:(to_string ~indent:(indent + 1))) in "[" ^ items_str ^ "]"
  | Table items ->
      let items_str = String.concat ",\n" (List.map items ~fn:(
        fun ((k, v)) -> ind ^ "  " ^ k ^ " = " ^ to_string ~indent:(indent + 1) v
      )) in
      if indent = 0 then
        "{\n" ^ items_str ^ "\n}"
      else "{ " ^ items_str ^ " }"
