  ...
  let parse_quoted_string () =
    ...
    let rec loop () =
      ...
      | '\\' ->
          ...
          if at_end () then
            raise
              (Parse_exception (Unterminated_string { position = start_pos }));
          (match current_char () with
  ...
  let rec parse_array () =
    ...
    if current_char () != '[' then
      raise
        (Parse_exception
           (Unexpected_char
              { position = !pos; found = current_char (); expected = "[" }));
    advance ();
  ...
  and parse_value () =
    ...
    if at_end () then
      raise
        (Parse_exception
           (Parse_error
              { position = !pos; context = "value"; reason = "unexpected end" }));
    match current_char () with
    | '"' -> parse_quoted_string ()
    | '[' -> parse_array ()
    | '{' -> parse_inline_table ()
  ...
  let parse_section_header () =
    ...
    if at_end () then
      raise
        (Parse_exception
           (Parse_error
              { position = start; context = "section"; reason = "unterminated" }));
