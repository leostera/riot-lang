(** # Data.Toml - TOML configuration file parser
    ...
    *)
    ...
    | Ok root ->
        ...
        (* Navigate to nested tables *)
        let server = Toml.get_table root
          |> Option.and_then (List.assoc_opt "server")
          |> Option.and_then Toml.get_table in
        
        (match server with
        | Some fields ->
            let host = List.assoc_opt "host" fields
              |> Option.and_then Toml.get_string in
    ...
    *)
    ...
    This is a minimal TOML parser focused on common configuration needs.
    Not all TOML 1.0 features are supported:
...
type value =
  ...
  | Bool of bool
(** TOML value representation supporting strings, booleans, arrays, and tables. *)
...
(** Converts a TOML parse error to a human-readable error message.
    ...
    ```ocaml
    match Toml.parse_file "bad.toml" with
    | Ok _ -> ()
    | Error err ->
...
(** Extracts a string value. Returns [None] if not a string.
    ...
    ```ocaml
    match value with
    | Toml.String s -> Printf.printf "Got: %s\n" s
    | _ -> ()
    
    (* Or using extractor: *)
    Toml.get_string value |> Option.iter (Printf.printf "Got: %s\n")
    ```
*)
...
(** Extracts an array value. Returns [None] if not an array.
    ...
    ```ocaml
    match Toml.get_array value with
    | Some items ->
        List.iter (fun item ->
          match Toml.get_string item with
          | Some s -> Printf.printf "Item: %s\n" s
          | None -> ()
        ) items
    | None -> ()
    ```
...
(** Extracts a table (section) as a list of key-value pairs.
    Returns [None] if not a table.
    
    ## Examples
    
    ```ocaml
    match Toml.get_table root with
    | Some fields ->
        (* Look up a specific field *)
        (match List.assoc_opt "server" fields with
        | Some server_table -> (* process server config *)
        | None -> Log.warn "No server config")
    | None -> ()
    ```
*)
