(** # Data.CSV - CSV parsing and serialization
    ...
    A CSV (Comma-Separated Values) parser and serializer for reading and 
    writing tabular data. Supports standard CSV format with customizable 
    ...
    let rec process_rows () =
      ...
      | Some (Ok row) ->
          println "Row: %s" (String.concat "," row);
          process_rows ()
      | Some (Error err) ->
          Log.error "Parse error: %s" (Data.Csv.error_to_string err)
      | None ->
          println "Done processing CSV"
    in
    process_rows ()
    ```
    ...
    ```ocaml
    let iter = Data.Csv.read (Path.v "users.csv") in
    
    (* Get headers first *)
    let headers = match Iter.MutIterator.next iter with
      | Some (Ok row) -> row
      | _ -> []
    in
    ...
    let rec process_data () =
      ...
      | Some (Ok row) ->
          ...
          let age = List.nth row 1 in
          println "%s is %s years old" name age;
          process_data ()
      | Some (Error err) ->
          Log.error "Parse error: %s" (Data.Csv.error_to_string err)
      | None -> ()
    ...
    ```ocaml
    let iter = Data.Csv.read (Path.v "large.csv") in
    let count = cell 0 in
    ...
    Csv.write ~headers ~data (Path.v "output.csv") |> Result.unwrap
    ```
    ...
    let iter = Csv.read ~config (Path.v "data.tsv") in
    (* process TSV file *)
    ```
    ...
    let iter = Csv.of_string csv_str in
    ...
    ```ocaml
    let iter = Csv.read (Path.v "data.csv") in
    let rows = Iter.MutIterator.to_list iter in
    (* rows is a list of (row, error) result *)
    
    (* Or filter out errors: *)
    let valid_rows = rows 
      |> List.filter_map (function Ok row -> Some row | Error _ -> None) in
    ```
*)
...
(** Creates a custom CSV configuration.
    ...
    ```ocaml
    let tsv_config = Csv.config ~delimiter:'\t' ()
    let excel_config = Csv.config ~delimiter:',' ~trim_fields:true ()
    ```
*)
...
(** Reads a CSV file incrementally, returning a mutable iterator over rows.
    This is the recommended way to read CSV files as it processes rows one at
    a time without loading the entire file into memory.
    
    Each iterator item is a [Result] containing either a successfully parsed
    row or a parse error. The iterator stops at the first parse error.
    ...
    let rec process () =
      ...
      | Some (Ok row) ->
          ...
          process ()
      | Some (Error err) ->
          Log.error "Parse error: %s" (Csv.error_to_string err)
      | None -> ()
    in
    process ()
    ```
...
(** Parses a CSV string incrementally, returning a mutable iterator over rows.
    ...
    ```ocaml
    let config = Csv.config ~delimiter:';' () in
    let iter = Csv.of_string ~config "a;b;c\n1;2;3" in
    ```
*)
...
(** Converts a parse error to a human-readable message.
    ...
    ```ocaml
    match Csv.of_string bad_input with
    | Ok _ -> ()
    | Error err ->
...
(** Writes CSV rows to a file.
    ...
    ```ocaml
    let config = Csv.config ~delimiter:'\t' () in
    Csv.write ~config ~headers ~data (Path.v "output.tsv") |> Result.unwrap
    ```
*)

val to_string :
  ?config:config -> ?headers:string list -> data:string list list -> string
(** Serializes CSV rows to a string.
    Fields containing delimiters, quotes, or newlines are automatically quoted.
    
    ## Examples
    
    ```ocaml
    Csv.to_string ~data:[["a"; "b"]; ["c"; "d"]]
    (* "a,b\nc,d\n" *)
    ...
    ```ocaml
    let headers = ["col1"; "col2"] in
    let data = [["a"; "b"]; ["c"; "d"]] in
    Csv.to_string ~headers ~data
    (* "col1,col2\na,b\nc,d\n" *)
    ```
    ...
    ```ocaml
    let config = Csv.config ~delimiter:';' () in
    Csv.to_string ~config ~headers ~data
    (* "col1;col2\na;b\nc;d\n" *)
    ```
*)
