(** # Data.Base16 - Hexadecimal encoding and decoding
    ...
    (* Encode to hex *)
    let hex = Base16.encode "Hello" in
    (* "48656C6C6F" *)
    
    (* Decode from hex *)
    match Base16.decode "48656C6C6F" with
    | Ok data -> (* "Hello" *)
    ...
    ```ocaml
    let hex = Base16.encode_lower "Hello" in
    (* "48656c6c6f" *)
    ```
    ...
    let hex = Base16.encode_bytes bytes in
    (* "64617461" *)
    ```
...
(** Encodes a string to uppercase hexadecimal.
    ...
    ```ocaml
    Base16.encode "Hi"  (* "4869" *)
    Base16.encode "\x00\xFF"  (* "00FF" *)
    ```
*)
...
(** Encodes a string to lowercase hexadecimal.
    ...
    ```ocaml
    Base16.encode_lower "Hi"  (* "4869" - wait this is still uppercase for digits *)
    Base16.encode_lower "\xAB\xCD"  (* "abcd" *)
    ```
*)
...
(** Encodes bytes to uppercase hexadecimal.
    ...
    ```ocaml
    let b = Bytes.of_string "test" in
    Base16.encode_bytes b  (* "74657374" *)
    ```
*)
...
(** Decodes a hexadecimal string. Accepts both uppercase and lowercase.
    ...
    ```ocaml
    Base16.decode "4869"  (* Ok "Hi" *)
    Base16.decode "48 69"  (* Error `Invalid_base16 - spaces not allowed *)
    Base16.decode "4G"  (* Error `Invalid_base16 - G not valid hex *)
    ```
