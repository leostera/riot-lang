...
let pad_year = fun year ->
  ...
  let padded =
    if String.length year_str >= 4 then
      year_str
    else String.make ~len:(4 - String.length year_str) ~char:'0' ^ year_str
  in
  if year < 0 then
    "-" ^ padded
  else padded

let to_iso8601_unchecked = fun { year; month; day } -> pad_year year ^ "-" ^ pad2 month ^ "-" ^ pad2 day

let make = fun ~year ~month ~day ->
  let date = { year; month; day } in
  if is_valid date then
    Ok date
  else Error (Invalid_date ("invalid civil date: " ^ to_iso8601_unchecked date))

let compare = fun left right -> Int.compare (Calendar.date_to_gregorian_days left) (Calendar.date_to_gregorian_days right)
...
let add_days = fun date days -> Calendar.gregorian_days_to_date (Calendar.date_to_gregorian_days date + days)

let diff_days = fun left right -> Calendar.date_to_gregorian_days left - Calendar.date_to_gregorian_days right

let day_of_week = Calendar.day_of_week

let day_of_year = fun ({ year; _ } as date) ->
  let first_day = { year; month = 1; day = 1 } in Calendar.date_to_gregorian_days date - Calendar.date_to_gregorian_days first_day + 1
...
let end_of_month = fun ({ year; month; _ } as date) -> { date with day = Calendar.last_day_of_month ~year ~month }
