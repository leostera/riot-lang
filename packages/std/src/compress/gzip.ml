...
let ensure_input_capacity = fun (t: reader) ->
  ...
  if available > 0 then
    ...
    let next = Bytes.create ~size:next_capacity in Bytes.blit_unchecked t.input ~src_offset:t.input_pos ~dst:next ~dst_offset:0 ~len:buffered;
  t.input <- next;
  t.input_pos <- 0;
...
let refill_input = fun (t: reader) ->
  ...
  | Ok bytes_read ->
      ...
      Bytes.blit_unchecked chunk_bytes ~src_offset:0 ~dst:t.input ~dst_offset:t.input_len ~len:bytes_read;
      ...
      if t.finished then
        ...
        if dst_len = 0 then
          ...
          if available_input = 0 then
            ...
            match Engine.decode t.decoder ~src:t.input ~src_pos:t.input_pos ~src_len:available_input ~dst:t.output ~dst_pos:0 ~dst_len:(Bytes.length t.output) with
          ...
          IoVec.for_each ~fn:(
            fun segment ->
              ...
              if remaining > 0 then
                ...
                let chunk_len = min length remaining in IoVec.IoSlice.blit_from_bytes_unchecked scratch ~src_off:!copied segment ~dst_off:0 ~len:chunk_len;
...
let compress = fun src dst ->
  ...
  in
  protect ~finally:(
    fun () -> Engine.close_encoder encoder
  )
    (
      fun () ->
        ...
        let rec encode_loop ~source_eof ~src_pos ~src_len ~chunk_count =
          ...
          if not source_eof && available_src = 0 then
            ...
            let flush =
              if source_eof then
                Engine.Finish
              else Engine.No_flush
            in
            match Engine.encode encoder ~src:src_buf ~src_pos ~src_len:available_src ~dst:dst_buf ~dst_pos:0 ~dst_len:(Bytes.length dst_buf) ~flush with
            ...
            | Ok step ->
                ...
                let* () =
                  if step.produced = 0 then
                    ...
                    let buf = Bytes.sub_unchecked dst_buf ~offset:0 ~len:step.produced |> Buffer.from_bytes in IO.write_all dst ~from:buf |> Result.map_err ~fn:(
                      fun err -> Stream_destination_error err
                    )
                in
                ...
                in
                match step.status with
                | Engine.Finished -> IO.flush dst |> Result.map_err ~fn:(
                  fun err -> Stream_destination_error err
                )
                | Engine.Need_output -> encode_loop ~source_eof ~src_pos:next_src_pos ~src_len:next_src_len ~chunk_count:next_chunk_count
                | Engine.Need_input ->
                    if source_eof && step.consumed = 0 && step.produced = 0 then
                      Error (Stream_gzip_error (Engine_error Engine.Buffer_error))
                    else
                      if source_eof then
                        encode_loop ~source_eof ~src_pos:next_src_pos ~src_len:next_src_len ~chunk_count:next_chunk_count
                      else encode_loop ~source_eof:false ~src_pos:0 ~src_len:0 ~chunk_count:next_chunk_count
        in
...
let decompress = fun src dst ->
  ...
  let rec loop chunk_count =
    ...
    | Ok 0 -> IO.flush dst |> Result.map_err ~fn:(
      fun err -> Stream_destination_error err
    )
    | Ok read_len ->
        ...
        begin
          ...
          | Ok () -> ()
          | Error error -> panic ("Compress.Gzip.decompress.append: " ^ Kernel.IO.Error.message error)
        end;
        let () =
          if chunk_count > 0 && Int.rem chunk_count 32 = 0 then
            yield ()
        in
        let* () = IO.write_all dst ~from:chunk |> Result.map_err ~fn:(
          fun err -> Stream_destination_error err
    ...
    (
      fun src_file ->
        with_open_output dst
          (
            fun dst_file -> decompress (Fs.File.to_reader src_file) (Fs.File.to_writer dst_file) |> Result.map_err ~fn:(
              function
  ...
  with_open_input src
    (
      fun src_file ->
        with_open_output dst
          (
            fun dst_file -> compress (Fs.File.to_reader src_file) (Fs.File.to_writer dst_file) |> Result.map_err ~fn:(
              function
              | Stream_source_error err -> File_io_error err
              | Stream_destination_error err -> File_io_error err
              | Stream_gzip_error err -> File_gzip_error err
  ...
  let rec loop () =
    ...
    | Ok read_len ->
        ...
        begin
          ...
          | Ok () -> Ok ()
          | Error error -> panic ("Compress.Gzip.decompress_string.append: " ^ Kernel.IO.Error.message error)
        end |> Result.and_then ~fn:(
          fun () -> loop ()
        )
    | Error (Source_error err) -> Error (Engine_error (Engine.Unknown_error ("unexpected in-memory source error: " ^ IO.error_message err)))
