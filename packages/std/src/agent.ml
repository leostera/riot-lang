...
(**
   ...
   ```
*)
type 'state t = { pid: Pid.t; state_ref: 'state Ref.t }

(* Message types *)
type agent_request =
  | Get : { reply_to: Pid.t; fn: 'state -> 'reply; state_ref: 'state Ref.t; reply_ref: 'reply Ref.t } -> agent_request
  | Update : { reply_to: Pid.t; fn: 'state -> 'state; state_ref: 'state Ref.t } -> agent_request
  | GetAndUpdate : {
    reply_to: Pid.t;
  ...
  | Cast : { fn: 'state -> 'state; state_ref: 'state Ref.t } -> agent_request
  | Stop : { reply_to: Pid.t } -> agent_request
...
let rec loop: type state. state Ref.t -> state -> (unit, exn) result = fun state_ref state ->
  ...
  | Get { reply_to; fn; state_ref = sr; reply_ref } -> (
    ...
    | None -> (* Message for different agent type, ignore *)
    loop state_ref state
  )
  ...
  | Cast { fn; state_ref = sr } -> (
    match Ref.type_equal state_ref sr with
    | Some Type.Equal ->
        let new_state = fn state in loop state_ref new_state
    | None -> loop state_ref state
  )
  | Stop { reply_to } ->
...
let cast: type state. state t -> fn:(state -> state) -> unit = fun agent ~fn -> send agent.pid (AgentRequest (Cast { fn; state_ref = agent.state_ref }))
