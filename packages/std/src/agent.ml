...
let rec loop : type state. state Ref.t -> state -> (unit, exn) result =
  fun state_ref state ->
    let selector msg =
      match msg with
...
let start : type state. (unit -> state) -> state t =
  fun init ->
    let state_ref : state Ref.t = Ref.make () in
    let pid = spawn (fun () -> loop state_ref (init ())) in
    { pid; state_ref }

let start_link : type state. (unit -> state) -> state t =
  fun init ->
    let state_ref : state Ref.t = Ref.make () in
    let pid = spawn_link (fun () -> loop state_ref (init ())) in
    { pid; state_ref }

let get : type state reply. state t -> (state -> reply) -> reply =
  fun agent fn ->
    let reply_ref : reply Ref.t = Ref.make () in
    send agent.pid (AgentRequest (Get { 
      reply_to = self (); 
      fn; 
...
let update : type state. state t -> (state -> state) -> unit =
  fun agent fn ->
    send agent.pid (AgentRequest (Update { 
      reply_to = self (); 
      fn; 
      state_ref = agent.state_ref
    }));
...
let get_and_update : type state reply. state t -> (state -> reply * state) -> reply =
  fun agent fn ->
    let reply_ref : reply Ref.t = Ref.make () in
    send agent.pid (AgentRequest (GetAndUpdate { 
      reply_to = self (); 
      fn; 
      state_ref = agent.state_ref;
      reply_ref 
...
let cast : type state. state t -> (state -> state) -> unit =
  fun agent fn ->
    send agent.pid (AgentRequest (Cast { 
      fn; 
      state_ref = agent.state_ref
    }))

let stop : type state. state t -> unit =
  fun agent ->
