...
let add_node = fun t ~id ?label ?(attrs = []) () ->
  let node = { id; label; attrs } in { t with nodes = node :: t.nodes }
...
let escape_string = fun s -> (* Escape quotes and backslashes for DOT format *)
String.fold_left ~fn:(
  fun acc c ->
    match c with
    | '"' -> acc ^ "\\\""
...
let format_attrs = fun attrs ->
  if attrs = [] then
    ...
    let attr_strs = List.map attrs ~fn:(
      fun (k, v) -> k ^ "=\"" ^ escape_string v ^ "\""
    ) in " [" ^ String.concat ", " attr_strs ^ "]"
...
let format_node = fun (node: node) ->
  let label_attr =
    ...
    | None -> []
  in
  let all_attrs = label_attr @ node.attrs in "  \"" ^ escape_string node.id ^ "\"" ^ format_attrs all_attrs ^ ";"
...
let format_edge = fun style edge ->
  ...
  let label_attr =
    ...
    | None -> []
  in
  let all_attrs = label_attr @ edge.attrs in "  \"" ^ escape_string edge.from_node ^ "\" " ^ arrow ^ " \"" ^ escape_string edge.to_node ^ "\"" ^ format_attrs all_attrs ^ ";"
...
let to_string = fun t ->
  ...
  let graph_attr_str =
    if t.graph_attrs = [] then
      ""
    else List.map t.graph_attrs ~fn:(
      fun (k, v) -> "  " ^ k ^ "=\"" ^ escape_string v ^ "\";\n"
    ) |> String.concat ""
  in
