...
let topo_sort = fun graph ->
  ...
  HashMap.for_each graph.nodes ~fn:(
    fun id _ ->
  ...
  *)
  HashMap.for_each graph.nodes ~fn:(
    fun my_id node ->
      ...
      let _ = HashMap.insert in_degree ~key:my_id ~value:my_in_degree in (* Also track reverse dependencies *)
      List.for_each node.deps ~fn:(
        fun dep_id ->
          let current_rev_deps = HashMap.get reverse_deps ~key:dep_id |> Option.unwrap_or ~default:[] in
          let _ = HashMap.insert reverse_deps ~key:dep_id ~value:(my_id :: current_rev_deps) in ()
      )
  );
  (* Find nodes with no incoming edges *)
  let queue = Queue.create () in
  let initial_nodes = HashMap.fold_left in_degree ~init:[] ~fn:(
    fun acc id count ->
      if count = 0 then
        id :: acc
      else acc
  ) |> List.sort ~compare:(
    fun a b -> Int.compare (Node_id.to_int a) (Node_id.to_int b)
  ) in
  List.for_each initial_nodes ~fn:(
    fun id -> Queue.push queue ~value:id
  );
  (* Process queue *)
  let sorted = cell [] in
  ...
  while not (Queue.is_empty queue) do
    ...
    let node = HashMap.get graph.nodes ~key:id |> Option.unwrap in
    sorted := node :: !sorted;
    Cell.incr processed;
    (* Decrease in-degree of nodes that depend on this one *)
    let rev_deps = HashMap.get reverse_deps ~key:id |> Option.unwrap_or ~default:[] |> List.sort ~compare:(
      fun a b -> Int.compare (Node_id.to_int a) (Node_id.to_int b)
    ) in List.for_each rev_deps ~fn:(
      fun dependent_id ->
        let count = HashMap.get in_degree ~key:dependent_id |> Option.unwrap in
  ...
  if !processed != HashMap.length graph.nodes then
    (
      ...
      let find_cycle () =
        let start_node = HashMap.fold_left in_degree ~init:None ~fn:(
          fun acc id count ->
            match acc with
            | Some _ -> acc
            | None ->
        ...
        | Some start_id ->
            ...
            let rec dfs node_id path =
              if HashMap.has_key rec_stack ~key:node_id then
                ...
                if HashMap.has_key visited ~key:node_id then
                  None
                else
                  (
                    ...
                    let result = List.fold_left node.deps ~init:None ~fn:(
                      fun acc dep_id ->
                        match acc with
                        | Some _ -> acc
                        | None -> dfs dep_id (node_id :: path)
                    ) in
                    let _ = HashMap.remove rec_stack ~key:node_id in result
                  )
            in
  ...
  else Ok (List.reverse !sorted)

let iter = fun graph ~fn -> HashMap.for_each graph.nodes ~fn:(
  fun node_id node_value -> fn node_id node_value
)
...
let reachable_from = fun graph start_nodes ->
  ...
  let rec visit node_id =
    if not (HashMap.has_key visited ~key:node_id) then
      (
        let _ = HashMap.insert visited ~key:node_id ~value:() in
        let node = HashMap.get graph.nodes ~key:node_id |> Option.unwrap in List.for_each node.deps ~fn:visit
      )
  in
  List.for_each start_nodes ~fn:(
    fun node -> visit node.id
  );
  HashMap.fold_left visited ~init:[] ~fn:(
    fun acc node_id () -> node_id :: acc
  ) |> List.reverse
