(** # DateTime - Calendar date and time operations
    ...
    Date and time utilities for working with calendar dates, timestamps,
    and time zones. Provides conversions between different representations.
    ...
    Log.info "Current time: %02d:%02d:%02d" now.hour now.minute now.second
    
    (* UTC time *)
    let utc = DateTime.now_utc () in
    Log.info "UTC: %s" (DateTime.to_iso8601 utc)
    (* "2025-08-27T21:07:31.426Z" *)
    ```
    ...
    let now = DateTime.now () in
    let timestamp = DateTime.to_timestamp now in
    (* 1724789251.426 *)
    
    (* Create from Unix timestamp *)
    let dt = DateTime.from_unix_time 1724789251.426 in
    Printf.printf "%04d-%02d-%02d" dt.year dt.month dt.day
    ```
    ...
    ```ocaml
    let utc = DateTime.now_utc () in
    let iso = DateTime.to_iso8601 utc in
    (* "2025-08-27T21:07:31.426822Z" *)
    
    (* Can be logged or stored *)
    Log.info "Event timestamp: %s" iso
    ```
    ...
    ```ocaml
    let local = DateTime.now () in
    let utc = DateTime.now_utc () in
    
    match local.time_zone with
    | Tz.Local -> Printf.printf "UTC offset: %d seconds" local.utc_offset
    | Tz.Etc_UTC -> Printf.printf "Already UTC"
    ```
    ...
    | DateTime | SystemTime |
    |----------|------------|
    | Calendar dates | Opaque time point |
    | Human-readable fields | No calendar conversion |
    | ISO 8601 formatting | No formatting |
    | Time zone aware | Time zone agnostic |
...
module Tz : sig
  ...
  (** Converts timezone to string representation.
      ...
      ```ocaml
      Tz.to_string Tz.Etc_UTC  (* "UTC" *)
      Tz.to_string Tz.Local    (* "Local" *)
      ```
  *)
end
...
(** Returns the current date and time in UTC.
    ...
    ```ocaml
    let utc = DateTime.now_utc () in
    assert (utc.time_zone = Tz.Etc_UTC);
    assert (utc.utc_offset = 0)
    ```
*)
...
(** Creates a datetime from a Unix timestamp (seconds since epoch).
    ...
    (* Specific timestamp *)
    let dt = DateTime.from_unix_time 1724789251.426822 in
    dt.year  (* 2025 *)
    ```
...
(** Converts datetime to Unix timestamp (seconds since epoch).
    ...
    ```ocaml
    let now = DateTime.now () in
    let ts = DateTime.to_timestamp now in
    (* 1724789251.426822 *)
    
    (* Round-trip conversion *)
    let dt' = DateTime.from_unix_time ts in
    dt'.year = now.year  (* true *)
    ```
*)
