(** # Option - Optional values
    ...
    contains a value, or [`None`], and does not. `Option` types are very common in
    OCaml code, as they have a number of uses:
    ...
    let result =
      divide 10 2
      |> Option.and_then (fun x -> divide x 2)
    ...
    similar early returns with [`and_then`] for chaining operations that might fail.
...
(** The [`None`] value.
    ...
    ```ocaml
    let x : int option = Option.none in
    assert (x = None)
    ```
*)
...
(** Returns `true` if the option is [`Some`] and the value matches the predicate.
    ...
    ```ocaml
    let x = Some 2 in
    assert (Option.is_some_and (fun x -> x > 1) x);
    assert (not (Option.is_some_and (fun x -> x > 5) x));
    
    let y = None in
    assert (not (Option.is_some_and (fun x -> x > 1) y))
    ```
*)
...
(** Returns `true` if the option is [`None`] or the value matches the predicate.
    ...
    ```ocaml
    let is_small = Option.is_none_or (fun x -> x < 10) in
    
    assert (is_small None);
    assert (is_small (Some 5));
    assert (not (is_small (Some 20)))
    ```
*)
...
val map : ('a -> 'b) -> 'a t -> 'b t
(** Maps an `Option<'a>` to `Option<'b>` by applying a function to the contained value.
    
    ## Examples
    
    ```ocaml
    let maybe_string = Some "Hello, World!" in
    let maybe_len = Option.map String.length maybe_string in
    assert (maybe_len = Some 13);
    
    let none_string : string option = None in
    let none_len = Option.map String.length none_string in
    assert (none_len = None)
    ```
*)
...
(** Returns the result of applying function to [`Some`] value, or default if [`None`].
    ...
    ```ocaml
    let x = Some "foo" in
    assert (Option.map_or ~default:42 String.length x = 3);
    
    let y = None in
    assert (Option.map_or ~default:42 String.length y = 42)
    ```
*)

val map_or_default : default:(unit -> 'b) -> ('a -> 'b) -> 'a t -> 'b
(** Returns result of applying function to [`Some`] value, or computes default if [`None`].
...
(** Maps an `Option<'a>` to `'b` by applying function to [`Some`], or computing default.
...
(** Returns [`None`] if the first option is [`None`], otherwise returns the second option.
    ...
    let x = Some 2 in
    let y = Some "foo" in
    assert (Option.and_ x y = Some "foo");
    
    let x = None in
    let y : string option = None in
    assert (Option.and_ x y = None)
    ```
*)

val and_then : 'a t -> ('a -> 'b t) -> 'b t
(** Returns [`None`] if the option is [`None`], otherwise calls function with the
    ...
    ```ocaml
    let sq_if_positive x =
      if x > 0 then Some (x * x)
      else None in
    
    assert (Option.and_then (Some 2) sq_if_positive = Some 4);
    assert (Option.and_then (Some 0) sq_if_positive = None);
    assert (Option.and_then None sq_if_positive = None)
    
    (* Chaining fallible operations *)
    let result =
      Sys.getenv_opt "CONFIG_PATH"
      |> Option.and_then (fun path -> 
           try Some (Fs.read (Path.v path) |> Result.unwrap)
           with _ -> None)
      |> Option.and_then parse_config
    ```
*)
...
(** Returns the first option if [`Some`], otherwise returns the second option.
    ...
    let x = Some 2 in
    let y = None in
    assert (Option.or_ x y = Some 2);
    
    let x = None in
    let y = Some 100 in
    assert (Option.or_ x y = Some 100);
...
(** Returns the option if [`Some`], otherwise calls function and returns its result.
    
    ## Examples
    
    ```ocaml
    let nobody = fun () -> None in
    let vikings = fun () -> Some "vikings" in
    
    assert (Option.or_else (Some "barbarians") vikings = Some "barbarians");
    assert (Option.or_else None vikings = Some "vikings");
    assert (Option.or_else None nobody = None)
    ```
*)
...
(** Returns [`Some`] if exactly one of the options is [`Some`], otherwise [`None`].
    ...
    let x = Some 2 in
    let y = None in
    assert (Option.xor x y = Some 2);
    
    let x = None in
    let y = Some 3 in
    assert (Option.xor x y = Some 3);
    
    let x = Some 4 in
    let y = Some 5 in
    assert (Option.xor x y = None);
...
(** Returns the contained [`Some`] value, consuming the option.
    ...
    ```ocaml
    let x = Some "air" in
    assert (Option.unwrap x = "air")
    
    (* This will panic: *)
    let y = None in
    Option.unwrap y  (* panic: "Called Option.unwrap on None" *)
    ```
...
(** Returns the contained [`Some`] value or a provided default.
    ...
    ```ocaml
    assert (Option.unwrap_or ~default:"bike" (Some "car") = "car");
...
(** Returns the contained [`Some`] value or computes it from a closure.
    ...
    ```ocaml
    let k = 10 in
    assert (Option.unwrap_or_else ~fn:(fun () -> 2 * k) (Some 4) = 4);
    assert (Option.unwrap_or_else ~fn:(fun () -> 2 * k) None = 20)
    ```
*)
...
(** Returns the contained [`Some`] value, consuming the option.
    ...
    ```ocaml
    let x = Some "value" in
    assert (Option.expect ~msg:"fruits are healthy" x = "value")
    
    (* This will panic with custom message: *)
    let y = None in
    Option.expect ~msg:"fruits are healthy" y
    (* panic: "fruits are healthy" *)
    ```
    ...
    ```ocaml
    let config = 
      Sys.getenv_opt "CONFIG_FILE"
      |> Option.expect ~msg:"env variable CONFIG_FILE should be set by wrapper script"
    ```
*)
...
(** Consumes the option, panicking if it is [`Some`].
    ...
    ```ocaml
    Option.unwrap_none None;  (* Does nothing *)
    
    (* This will panic: *)
    Option.unwrap_none (Some "value")
    (* panic: "Called Option.unwrap_none on Some: value" *)
    ```
*)
