...
let has_changes changes = List.length changes > 0

let additions changes =
  List.filter
    (fun { kind; _ } -> match kind with Added _ -> true | _ -> false)
    changes

let removals changes =
  List.filter
    (fun { kind; _ } -> match kind with Removed _ -> true | _ -> false)
    changes

let changes changes =
  List.filter
    (fun { kind; _ } -> match kind with Changed _ -> true | _ -> false)
    changes

let at_path target_path changes =
