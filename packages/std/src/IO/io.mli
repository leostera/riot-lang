(** {1 IO - Generic I/O abstractions}
    ...
    Unified I/O interfaces for reading from and writing to various sources
    and destinations (files, sockets, buffers, etc.).
    ...
    - {b Flexible errors}: Each source/destination can define its own error types
    ...
    - TCP streams might have [`Closed | `Connection_refused | `System_error of string]
...
(** {2 Convenience Functions}
    ...
    These are shortcuts to the corresponding functions in {!Reader} and {!Writer}. *)

val read : ('src, 'err) Reader.t -> ?timeout:int64 -> bytes -> (int, 'err) result
(** [read reader ?timeout buf] reads data into [buf].
    Equivalent to [Reader.read reader ?timeout buf]. *)

val read_vectored : ('src, 'err) Reader.t -> Iovec.t -> (int, 'err) result
(** [read_vectored reader iov] reads into multiple buffers.
    Equivalent to [Reader.read_vectored reader iov]. *)

val read_to_end : ('src, 'err) Reader.t -> buf:Buffer.t -> (int, 'err) result
(** [read_to_end reader ~buf] reads until EOF.
    Equivalent to [Reader.read_to_end reader ~buf]. *)

val write : ('dst, 'err) Writer.t -> buf:string -> (int, 'err) result
(** [write writer ~buf] writes data (may be partial).
    Equivalent to [Writer.write writer ~buf]. *)

val write_all : ('dst, 'err) Writer.t -> buf:string -> (unit, 'err) result
(** [write_all writer ~buf] writes all data, retrying as needed.
    Equivalent to [Writer.write_all writer ~buf]. *)

val write_owned_vectored : ('dst, 'err) Writer.t -> bufs:Iovec.t -> (int, 'err) result
(** [write_owned_vectored writer ~bufs] writes from multiple buffers.
    Equivalent to [Writer.write_owned_vectored writer ~bufs]. *)

val write_all_vectored : ('dst, 'err) Writer.t -> bufs:Iovec.t -> (unit, 'err) result
(** [write_all_vectored writer ~bufs] writes all vectored data.
    Equivalent to [Writer.write_all_vectored writer ~bufs]. *)

val flush : ('dst, 'err) Writer.t -> (unit, 'err) result
(** [flush writer] flushes buffered data.
    Equivalent to [Writer.flush writer]. *)
