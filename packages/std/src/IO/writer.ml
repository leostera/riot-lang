  ...
  fun (Writer ((module W), dst)) ~buf ->
    let rec loop remaining =
      if String.length remaining = 0 then
        ...
        | Ok written ->
            loop (String.sub remaining written (String.length remaining - written))
        | Error err -> Error err
    in
    loop buf

let write_all_vectored: type dst err. (dst, err) t -> bufs:Iovec.t -> (unit, err) result =
  fun (Writer ((module W), dst)) ~bufs ->
    let rec loop remaining =
      if Iovec.length remaining = 0 then
        ...
        | Ok written ->
            loop (Iovec.sub remaining ~pos:written ~len:(Iovec.length remaining - written))
        | Error err -> Error err
    in
    loop bufs
