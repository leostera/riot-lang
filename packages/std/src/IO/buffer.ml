...
let create = fun ~size ->
  ...
  { bytes = Bytes.create ~size:initial_capacity; length = 0 }
...
let ensure_capacity = fun buffer additional ->
  ...
  let current = Bytes.length buffer.bytes in
  ...
  | _ ->
      ...
      in
      let grown = Bytes.create ~size:(next_capacity current) in
      Bytes.blit_unchecked buffer.bytes ~src_offset:0 ~dst:grown ~dst_offset:0 ~len:buffer.length;
      buffer.bytes <- grown
...
let get = fun buffer ~at ->
  ...
  | _ -> (
      ...
      | -1 -> Some (Bytes.get_unchecked buffer.bytes ~at)
      | _ -> None
    )

let get_unchecked = fun buffer ~at -> Bytes.get_unchecked buffer.bytes ~at

let add_char = fun buffer value ->
  ensure_capacity buffer 1;
  Bytes.set_unchecked buffer.bytes ~at:buffer.length ~char:value;
...
let add_bytes = fun buffer source -> add_subbytes buffer source 0 (Bytes.length source)
...
let contents = fun buffer ->
  let bytes = Bytes.sub_unchecked buffer.bytes ~offset:0 ~len:buffer.length in
