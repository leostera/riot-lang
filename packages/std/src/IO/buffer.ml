...
let create = fun ~size ->
  let size = Kernel.Int.max 0 size in panic_result "create" (KernelBuffer.create ~size ())

let from_string = fun source ->
  let buffer = create ~size:(Kernel.String.length source) in
  let _ = KernelBuffer.append_string buffer source |> panic_result "from_string" in buffer

let from_bytes = fun source ->
  let buffer = create ~size:(Kernel.Bytes.length source) in
  let _ = KernelBuffer.append_bytes buffer source |> panic_result "from_bytes" in buffer

let from_slice = fun source ->
  let buffer = create ~size:(IoSlice.length source) in
  let _ = KernelBuffer.append_slice buffer source |> panic_result "from_slice" in buffer
