(** {1 Reader} 
    ...
    This allows code to be polymorphic over the source of data and error types.

    {2 Design}

    The Reader abstraction uses first-class modules to provide type erasure
    while maintaining type safety. Sources implement the [Read] signature,
    then get wrapped in a [Reader.t] for uniform handling.
    
    The Reader is parametrized by both the source type ['src] and error type ['err],
    allowing each source to define its own error types without forcing a common
    error hierarchy.
...
module type Read = sig
  ...
  (** [read src ?timeout buf] reads data from [src] into [buf].
      ...
      @param timeout Optional timeout in nanoseconds. If the read cannot 
...
(** Reader wrapping a readable source.
    ...
    preserving its capabilities. Code working with [Reader.t] doesn't need
    to know the specific source implementation.
