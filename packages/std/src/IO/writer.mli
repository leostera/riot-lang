(** {1 Writer}
    ...
    sockets, in-memory buffers, etc.) through a first-class module wrapper.
    This allows code to be polymorphic over the destination of data and error types.
    ...
    The Writer is parametrized by both the destination type ['dst] and error type ['err],
    allowing each destination to define its own error types without forcing a common
    error hierarchy.
    ...
    {[
      ...
      let rec write_loop offset =
        if offset >= String.length data then Ok ()
          ...
          let remaining = String.sub data offset (String.length data - offset) in
...
module type Write = sig
  ...
  (** [write dst ~buf] writes data from [buf] to [dst].
      ...
      @return Number of bytes actually written. May be less than [String.length buf]
      ...
      - Suspend the calling process if the destination is not ready (for async destinations)
...
(** Writer wrapping a writable destination.
    ...
    preserving its capabilities. Code working with [Writer.t] doesn't need
    to know the specific destination implementation.
    
    Type parameters:
    - ['dst] is the underlying destination type (e.g., [Kernel.Net.Tcp_stream.t])
...
(** [of_write_src (module Write) dst] creates a writer from a destination.
    ...
    This is typically called by destination modules (like [Net.TcpStream.to_writer])
...
(** [write writer ~buf] writes data to the writer.
    ...
    This performs a single write operation and may not write all the data.
    For most use cases, {!write_all} is preferred to ensure all data is written.
...
(** [write_all writer ~buf] writes all data, retrying as needed.
    ...
    {[
      let rec write_loop buf =
        ...
        else match IO.write writer ~buf with
...
(** [write_owned_vectored writer ~bufs] writes data from multiple buffers.
    ...
    More efficient than multiple [write] calls when gathering data from
    multiple buffers into a single write operation.
    ...
    {[
      ...
      let iov = Iovec.create ~count:2 ~size:(Bytes.length header + Bytes.length body) () in
...
(** [write_all_vectored writer ~bufs] writes all vectored data, retrying as needed.
