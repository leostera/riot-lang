(** # Cursor - Immutable string cursor for parsing
    ...
    An immutable cursor that provides safe, functional string traversal.
    Each operation returns a new cursor, leaving the original unchanged.
    ...
    Use [Cursor] for backtracking parsers. For single-pass parsing, use [MutCursor].
*)
...
(** Takes characters until predicate returns true.
    Returns (taken_string, cursor_at_matching_char) or None if predicate never matches.
    ...
    | Some (token, cursor) -> (* got "GET", cursor at space *)
    | None -> (* no space found, need more data *)
    
    (* Take until CRLF *)
    match Cursor.take_until cursor (fun c -> c = '\r') with
    | Some (line, cursor) -> 
        (* Skip the \r\n *)
        let cursor = Cursor.advance_by cursor 2 |> Option.unwrap in
        process line cursor
    | None -> Need_more
    ```
*)
