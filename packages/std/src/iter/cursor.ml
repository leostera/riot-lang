...
type t = { source: IoSlice.t; pos: int; length: int }

let unwrap_slice = fun context ->
  function
  | Kernel.Result.Ok value -> value
  | Kernel.Result.Error error -> panic (Kernel.String.concat "" [ context; ": "; Kernel.IO.Error.message error ])

let from_slice = fun source -> { source; pos = 0; length = IoSlice.length source }

let from_string = fun source -> from_slice (unwrap_slice "Iter.Cursor.from_string" (IoSlice.from_string source))
...
let peek = fun cursor ->
  if is_eof cursor then
    None
  else Some (IoSlice.get_unchecked cursor.source ~at:cursor.pos)

let peek_n = fun cursor count ->
  let target = cursor.pos + count in
  if target >= cursor.length then
    None
  else Some (IoSlice.get_unchecked cursor.source ~at:target)
...
let take_while = fun cursor predicate ->
  ...
  let rec loop pos =
    if pos >= cursor.length then
      ...
      if predicate (IoSlice.get_unchecked cursor.source ~at:pos) then
        loop (pos + 1)
      else pos
  in
  let stop = loop start in (IoSlice.sub_unchecked cursor.source ~off:start ~len:(stop - start), { cursor with pos = stop })

let take_while_string = fun cursor predicate ->
  let taken, cursor = take_while cursor predicate in (IoSlice.to_string taken, cursor)
...
let take_until = fun cursor predicate ->
  ...
  | None -> None
  | Some stop -> Some (IoSlice.sub_unchecked cursor.source ~off:start ~len:(stop - start), { cursor with pos = stop })
...
let take_until_char = fun cursor needle ->
  ...
  let rec loop pos =
    if pos >= cursor.length then
      ...
      if IoSlice.get_unchecked cursor.source ~at:pos = needle then
        Some pos
      else loop (pos + 1)
  in
  match loop start with
...
let take_n = fun cursor count ->
  if cursor.pos + count > cursor.length then
    None
  else Some (IoSlice.sub_unchecked cursor.source ~off:cursor.pos ~len:count, { cursor with pos = cursor.pos + count })
...
let remaining = fun cursor ->
  if is_eof cursor then
    IoSlice.empty
  else IoSlice.sub_unchecked cursor.source ~off:cursor.pos ~len:(cursor.length - cursor.pos)
