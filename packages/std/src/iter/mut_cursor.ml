...
type t = { source: IoSlice.t; mutable pos: int; length: int }
...
let from_string = fun source -> from_slice (unwrap_slice "Iter.MutCursor.from_string" (IoSlice.from_string source))
...
let take_until = fun cursor predicate ->
  ...
  let rec loop () =
    if cursor.pos >= cursor.length then
      ...
      if predicate (IoSlice.get_unchecked cursor.source ~at:cursor.pos) then
        Some cursor.pos
      else
        (
          cursor.pos <- cursor.pos + 1;
...
let take_until_char = fun cursor needle ->
  ...
  let rec loop () =
    if cursor.pos >= cursor.length then
      ...
      if IoSlice.get_unchecked cursor.source ~at:cursor.pos = needle then
        Some cursor.pos
      else
        (
          cursor.pos <- cursor.pos + 1;
