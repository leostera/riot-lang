...
let split_on_char delimiter str =
  ...
  let rec loop acc current i =
    if i >= len then List.rev (current :: acc)
    else
      let c = String.get str i in
      if c = delimiter then loop (current :: acc) "" (i + 1)
      else loop acc (current ^ String.make 1 c) (i + 1)
  in
  loop [] "" 0

let parse_pre_release_segment s =
  match parse_int s with
  | Some n -> Ok (Numeric n)
  | None ->
      if String.length s > 0 && String.for_all is_alphanumeric s then
        Ok (Alphanumeric s)
      else Error (Invalid_pre_release_segment s)

let parse_pre_release_identifiers s =
...
let parse version_string =
  ...
  let build_pos =
    try Some (String.index version_string '+') with Not_found -> None
  in
  ...
  let pre_pos =
    try Some (String.index core_and_pre '-') with Not_found -> None
  in
  ...
  let core, pre_string =
    ...
    | Some pos ->
        ...
        let p =
          String.sub core_and_pre (pos + 1)
            (String.length core_and_pre - pos - 1)
        in
        (c, Some p)
  in
...
let to_string v =
  ...
  let with_pre =
    ...
    | pre ->
        let pre_str =
          String.concat "." (List.map pre_release_segment_to_string pre)
        in
        base ^ "-" ^ pre_str
  in
  match v.build with None -> with_pre | Some build -> with_pre ^ "+" ^ build

(* Comparison *)

let compare_pre_release_segment s1 s2 =
  match (s1, s2) with
  | Numeric n1, Numeric n2 -> if n1 < n2 then Lt else if n1 > n2 then Gt else Eq
  | Numeric _, Alphanumeric _ -> Lt
  | Alphanumeric _, Numeric _ -> Gt
  | Alphanumeric a1, Alphanumeric a2 ->
      if a1 < a2 then Lt else if a1 > a2 then Gt else Eq

let rec compare_pre_release_lists l1 l2 =
  match (l1, l2) with
  | [], [] -> Eq
  | [], _ :: _ -> Lt
  | _ :: _, [] -> Gt
...
let compare v1 v2 =
  if v1.major < v2.major then Lt
  else if v1.major > v2.major then Gt
  else if v1.minor < v2.minor then Lt
  else if v1.minor > v2.minor then Gt
  else if v1.patch < v2.patch then Lt
  else if v1.patch > v2.patch then Gt
  else
    match (v1.pre, v2.pre) with
    | [], [] -> Eq
    | [], _ :: _ ->
        Gt (* version without pre-release > version with pre-release *)
    | _ :: _, [] -> Lt
    | pre1, pre2 -> compare_pre_release_lists pre1 pre2

let equal v1 v2 = compare v1 v2 = Eq
let lt v1 v2 = compare v1 v2 = Lt

let lte v1 v2 =
  let c = compare v1 v2 in
  c = Lt || c = Eq

let gt v1 v2 = compare v1 v2 = Gt
...
let parse_requirement req_string =
  let s = String.trim req_string in
  let len = String.length s in

  if len < 2 then Error (Invalid_format "Requirement too short")
  else
    let op, version_start =
      if String.length s >= 2 && String.sub s 0 2 = "~>" then (ReqTilde, 2)
      else if String.length s >= 2 && String.sub s 0 2 = "==" then (ReqEq, 2)
      else if String.length s >= 2 && String.sub s 0 2 = "!=" then (ReqNeq, 2)
      else if String.length s >= 2 && String.sub s 0 2 = ">=" then (ReqGte, 2)
      else if String.length s >= 2 && String.sub s 0 2 = "<=" then (ReqLte, 2)
      else if String.get s 0 = '>' then (ReqGt, 1)
      else if String.get s 0 = '<' then (ReqLt, 1)
      else
        (* Default to equality if no operator *)
        (ReqEq, 0)
    in
    let version_str =
      String.trim (String.sub s version_start (len - version_start))
    in
    match parse version_str with
    | Ok version -> Ok (op, version)
    | Error e -> Error e

let matches (op, req_version) test_version =
...
let make ~major ~minor ~patch ?(pre = []) ?build () =
