...
let split_on_char = fun delimiter str ->
  ...
  let rec loop acc current i =
    if i >= len then
      ...
      else loop acc (current ^ String.make ~len:1 ~char:c) (i + 1)
  in
  loop [] "" 0
...
let parse = fun version_string ->
  ...
  let core_and_pre, build =
    ...
    | Some pos ->
        ...
        let build_meta = String.sub version_string ~offset:(pos + 1) ~len:(len - pos - 1) in (core, Some build_meta)
  in
  ...
  let core, pre_string =
    ...
    | Some pos ->
        ...
        let p = String.sub core_and_pre ~offset:(pos + 1) ~len:(String.length core_and_pre - pos - 1) in (c, Some p)
  in
...
let to_string = fun v ->
  ...
  let with_pre =
    ...
    | [] -> base
    | pre ->
        let pre_str = String.concat "." (List.map ~fn:pre_release_segment_to_string pre) in base ^ "-" ^ pre_str
  in
  match v.build with
  | None -> with_pre
...
let lte = fun v1 v2 ->
  let c = compare v1 v2 in c = Order.LT || c = Order.EQ

let gt = fun v1 v2 -> compare v1 v2 = Order.GT

let gte = fun v1 v2 ->
  let c = compare v1 v2 in c = Order.GT || c = Order.EQ
          ...
          if starts_with "!=" then
            ...
            if starts_with ">=" then
              ...
              if starts_with "<=" then
                ...
                if len > 0 && String.get_unchecked s ~at:0 = '>' then
                  (Some ReqGt, 1)
                else
                  if len > 0 && String.get_unchecked s ~at:0 = '<' then
                    (Some ReqLt, 1)
...
let matches = fun requirement test_version ->
  ...
  | PrefixMinor (major, minor) -> Int.equal test_version.major major && Int.equal test_version.minor minor
  | Requirement (op, req_version) ->
      ...
      | ReqTilde ->
          ...
          let below_next =
            let next_minor = { req_version with minor = req_version.minor + 1; patch = 0; pre = [] } in lt test_version next_minor
          in
          at_least && below_next
...
module Tests = struct
  let test_requirement_to_string (): (unit, string) result =
    ...
    | Ok requirement ->
        if String.equal (requirement_to_string requirement) ">= 1.2.3" then
          Ok ()
        else Error "expected requirement_to_string to preserve operator and version"
  ...
  let test_any_requirement_roundtrip (): (unit, string) result =
    ...
    | Ok requirement ->
        if requirement_to_string requirement = "*" && matches requirement (make ~major:999 ~minor:0 ~patch:0 ()) then
          Ok ()
        else Error "expected '*' to roundtrip as unconstrained requirement"
    | Error _ -> Error "expected '*' requirement to parse" [@test]

  let test_prefix_minor_requirement_matches_patch_range (): (unit, string) result =
    match parse_requirement "0.2" with
    | Ok requirement ->
        if String.equal (requirement_to_string requirement) "0.2" && matches requirement (make ~major:0 ~minor:2 ~patch:0 ()) && matches requirement (make ~major:0 ~minor:2 ~patch:99 ()) && not (matches requirement (make ~major:0 ~minor:3 ~patch:0 ())) then
          Ok ()
        else Error "expected bare major.minor requirement to match only that patch range"
  ...
  let test_prefix_major_requirement_matches_minor_and_patch_range (): (unit, string) result =
    ...
    | Ok requirement ->
        if String.equal (requirement_to_string requirement) "0" && matches requirement (make ~major:0 ~minor:0 ~patch:0 ()) && matches requirement (make ~major:0 ~minor:99 ~patch:99 ()) && not (matches requirement (make ~major:1 ~minor:0 ~patch:0 ())) then
          Ok ()
        else Error "expected bare major requirement to match only that major range"
