...
let unwatch t watch_id = 
  Ev.unwatch t.kernel watch_id

(* Try to parse ONE event from the buffer, return (event option, bytes_consumed) *)
let try_parse_event buffer buffer_len =
  if buffer_len < 16 then (None, 0)  (* Need at least 16 bytes for len+flags+event_id *)
  else
    let path_len = Int32.to_int (IO.Bytes.get_int32_ne buffer 0) in
    let total_needed = 16 + path_len in
    if buffer_len < total_needed then (None, 0)  (* Not enough for full event *)
    else
...
let poll t = 
  let rec read_and_parse events =
    (* First, try to parse any complete events already in buffer *)
    ...
    | (None, _) ->
        ...
        let read_buf = IO.Bytes.create 1024 in
        match File.read file read_buf ~offset:0 ~len:1024 with
        | Ok 0 -> 
        ...
        | Ok n ->
            ...
            if new_len > IO.Bytes.length t.buffer then begin
              (* Need bigger buffer *)
            ...
            read_and_parse events  (* Try parsing again *)
        | Error (IO.Operation_would_block | IO.Resource_unavailable_try_again) ->
            (* If we have events already, return them *)
            if List.length events > 0 then Ok (List.rev events)
            (* Otherwise wait for readable *)
            else Miniriot.syscall ~name:"Fs.Events.read"
              ~interest:Kernel.Async.Interest.readable ~source:t.source
              (fun () -> read_and_parse events)
        | Error e -> Error e
  in
  read_and_parse []

let stop t = Ev.stop t.kernel
