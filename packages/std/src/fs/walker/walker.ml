...
let metadata_for_path_string = fun ~follow_symlinks path_string ->
  if follow_symlinks then
    Kernel.Fs.File.metadata path_string
    |> Common.convert_kernel_result
  else
    Kernel.Fs.File.lstat path_string
    |> Common.convert_kernel_result
...
let hinted_entry = fun opts ~depth ~dir_path (entry: ReadDir.entry) ->
  ...
  | ReadDir.BlockDevice
  | ReadDir.NamedPipe
  | ReadDir.Socket -> Ok (joined_entry ~depth ~kind:Other ~dir_path ~name)
  | ReadDir.Unknown ->
      let path_string = join_path_string dir_path name in
      entry_for_path_string opts ~depth path_string
...
let compare_item_path = fun left right ->
  ...
  | (Error (left: error), Error (right: error)) ->
      let left =
        Option.map left.path ~fn:Path.to_string
        |> Option.unwrap_or ~default:""
      in
      let right =
        Option.map right.path ~fn:Path.to_string
        |> Option.unwrap_or ~default:""
      in
