...
type state = { watcher: Events.t; subscriber: Pid.t; ignore_prefixes: Path.t list }

let should_ignore = fun ~ignore_prefixes path -> List.any ignore_prefixes ~fn:(
  fun prefix ->
...
let rec loop = fun state ->
  ...
  let filtered_events = List.filter events ~fn:(
    fun (event: Event.t) -> not (should_ignore ~ignore_prefixes:state.ignore_prefixes event.Event.path)
  ) in
...
let init = fun ~latency ~root:path ~ignore_prefixes ~subscriber ->
  ...
  let _watch_id = Events.watch watcher ~path ~latency |> Result.expect ~msg:("Failed to watch: " ^ (Path.to_string path)) in loop { subscriber; watcher; ignore_prefixes }

let start_link = fun ?(latency = (Time.Duration.from_millis 1)) ?(ignore_prefixes = []) ~root () ->
  let subscriber = self () in
  spawn_link
    (
      fun () -> init ~latency ~root ~ignore_prefixes ~subscriber
    )
