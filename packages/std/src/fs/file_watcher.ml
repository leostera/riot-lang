...
let should_ignore ~ignore_prefixes path =
...
let init ~latency ~root:path ~ignore_prefixes ~subscriber = 
...
let start_link ?(latency=(Time.Duration.from_millis 1)) ?(ignore_prefixes=[]) ~root () =
