...
type system_flags = {
  own_event: bool;
  (* This process caused it *)
  mount: bool;
  (* Volume mount *)
  unmount: bool;
  (* Volume unmount *)
  root_changed: bool;
  (* Watched root changed *)
  must_scan_subdirs: bool;
  (* Too many events, rescan needed *)
  user_dropped: bool;
  (* Events lost in userspace *)
  kernel_dropped: bool;
  (* Events lost in kernel *)
}
...
type t = {
  ...
  file_type: file_type;
  (* What kind of filesystem object *)
...
let from_kernel_event: Ev.event -> t = fun ev ->
  let has_flag flag = Int32.logand ev.flags flag != Int32.zero in
  {
    ...
    file_type = if has_flag Ev.Flag.is_file then
      File
    else
      if has_flag Ev.Flag.is_dir then
        Directory
      else
        if has_flag Ev.Flag.is_symlink then
          Symlink
        else Unknown;
    metadata = {
