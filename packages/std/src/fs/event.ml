...
let from_kernel_event (ev: Ev.event) : t =
  let has_flag flag = Int32.logand ev.flags flag != Int32.zero in
  {
    ...
    file_type = 
      if has_flag Ev.flag_is_file then File
      else if has_flag Ev.flag_is_dir then Directory
      else if has_flag Ev.flag_is_symlink then Symlink
      else Unknown;
    metadata = {
