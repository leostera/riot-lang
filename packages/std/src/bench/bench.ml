...
let case name fn = Single (Bench_case.case name fn)
let skip name fn = Single (Bench_case.skip name fn)
let with_config ~config name fn = Single (Bench_case.with_config ~config name fn)

(* Comparison constructors *)
let compare description cases = Compare (Bench_comparison.compare description cases)
