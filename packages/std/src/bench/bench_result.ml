...
let make_statistics timings =
  let durations = List.map (fun t -> t.duration) timings in
...
let find_fastest results =
  let compare_by_mean a b =
    Time.Duration.compare a.statistics.mean b.statistics.mean
  in
  let sorted = List.sort compare_by_mean results in
  List.hd sorted

let calculate_speedup fastest_mean other_mean =
...
let make_comparison_result description case_results =
  let fastest = find_fastest case_results in
  let speedup_ratios =
    List.map
      (fun res ->
        if res.name = fastest.name then (res.name, 1.0)
        else
          let ratio =
            calculate_speedup fastest.statistics.mean res.statistics.mean
          in
          (res.name, ratio))
      case_results
  in
