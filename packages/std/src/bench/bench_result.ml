...
let make_statistics = fun ?(gc = zero_gc_stats) timings ->
  let durations = List.map timings ~fn:(fun t -> t.duration) in
  let sorted = List.sort durations ~compare:Time.Duration.compare in
  (* Calculate total time *)
  let total_nanos =
    List.fold_left durations ~init:0L ~fn:(fun acc d -> Int64.add acc (Time.Duration.to_nanos d))
  in
  let iterations = List.length timings in
  let mean_nanos = Int64.div total_nanos (Int64.from_int iterations) in
  (* Min and max *)
  let min =
    List.get sorted ~at:0
    |> Option.unwrap
  in
  let max =
    List.get sorted ~at:(List.length sorted - 1)
    |> Option.unwrap
  in
  (* Median *)
  let median =
    List.get sorted ~at:(iterations / 2)
    |> Option.unwrap
  in
  (* Standard deviation *)
  let variance =
