...
let make_statistics = fun ?(gc = zero_gc_stats) timings ->
  let durations = List.map timings ~fn:(
    fun t -> t.duration
  ) in
  let sorted = List.sort durations ~compare:Time.Duration.compare in
  (* Calculate total time *)
  let total_nanos = List.fold_left durations ~init:0L ~fn:(
    fun acc d -> Int64.add acc (Time.Duration.to_nanos d)
  ) in
  ...
  (* Standard deviation *)
  let variance = List.fold_left durations ~init:0.0 ~fn:(
    fun acc d ->
      let diff = Int64.sub (Time.Duration.to_nanos d) mean_nanos in
      let diff_f = Int64.to_float diff in acc +. (diff_f *. diff_f)
  ) in
...
let make_summary = fun results -> List.fold_left results ~init:{
...
let find_fastest = fun results ->
  ...
  let sorted = List.sort results ~compare:compare_by_mean in List.get sorted ~at:0 |> Option.unwrap

let calculate_speedup = fun fastest_mean other_mean ->
  let fastest_ns = Time.Duration.to_nanos fastest_mean in
  let other_ns = Time.Duration.to_nanos other_mean in Int64.to_float other_ns /. Int64.to_float fastest_ns

let make_comparison_result = fun description case_results ->
  let fastest = find_fastest case_results in
  let speedup_ratios = List.map case_results ~fn:(
    fun res ->
      if res.name = fastest.name then
        (res.name, 1.0)
      else
        let ratio = calculate_speedup fastest.statistics.mean res.statistics.mean in (res.name, ratio)
  ) in
  {
    description;
    case_results;
