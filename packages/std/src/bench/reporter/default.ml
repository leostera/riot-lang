...
let init (info : Intf.suite_info) count =
  println "";
  println
    ("Running " ^ string_of_int count ^ " benchmarks from " ^ info.name ^ "...");
  println ""

let on_result index (result : Bench_result.t) =
  ...
  | Bench_result.Completed stats ->
      ...
      println
        ("  median:     "
        ^ Time.Duration.to_secs_string ~precision:6 stats.median);
      println
        ("  min:        " ^ Time.Duration.to_secs_string ~precision:6 stats.min);
      println
        ("  max:        " ^ Time.Duration.to_secs_string ~precision:6 stats.max);
      println
        ("  std_dev:    "
...
let finalize (summary : Bench_result.summary) =
  println
    ("Summary: " ^ string_of_int summary.total ^ " total, "
   ^ string_of_int summary.completed ^ " completed, "
   ^ string_of_int summary.skipped ^ " skipped, "
   ^ string_of_int summary.failed ^ " failed")

(* Comparison reporting *)

let on_comparison_start index description count =
  println "";
  println ("[" ^ string_of_int index ^ "] Comparison: " ^ description);
  println ("  Running " ^ string_of_int count ^ " implementations...");
  println ""

let on_comparison_case_result index name (stats : Bench_result.statistics) =
...
let on_comparison_summary (result : Bench_result.comparison_result) =
  println "  Summary:";
