...
let finalize = fun (summary: Bench_result.summary) -> println ("Summary: " ^ Int.to_string summary.total ^ " total, " ^ Int.to_string summary.completed ^ " completed, " ^ Int.to_string summary.skipped ^ " skipped, " ^ Int.to_string summary.failed ^ " failed")
...
let on_comparison_case_result = fun index name (stats: Bench_result.statistics) ->
  ...
  println ("    iterations: " ^ Int.to_string stats.iterations);
  println ("    mean:       " ^ Time.Duration.to_secs_string ~precision:6 stats.mean ^ " ± " ^ Time.Duration.to_secs_string ~precision:6 stats.std_dev);
  println ("    min:        " ^ Time.Duration.to_secs_string ~precision:6 stats.min);
  println ("    max:        " ^ Time.Duration.to_secs_string ~precision:6 stats.max);
  print_gc ~indent:"    " stats.gc;
  println ""
...
let on_comparison_summary = fun (result: Bench_result.comparison_result) ->
  ...
  List.for_each result.speedup_ratios ~fn:(
    fun ((name, ratio)) ->
