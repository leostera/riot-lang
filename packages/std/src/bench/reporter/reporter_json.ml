...
let benchmark_result_to_json = fun (result: Bench_result.t) -> let open Data.Json in
let base_fields =
  [
...
obj
  [
    ...
    "case_results", array (List.map result.case_results ~fn:(
    ...
    "speedup_ratios", array (List.map result.speedup_ratios ~fn:(
      fun (name, ratio) ->
        obj
          [
            "name", string name;
...
let finalize = fun (summary: Bench_result.summary) -> let open Data.Json in
