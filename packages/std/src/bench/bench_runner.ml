...
let run_single_benchmark index (bench : Bench_case.t) =
  if bench.skip then Bench_result.{ index; name = bench.name; result = Skipped }
  else
    try
...
let run_comparison index (module R : Reporter.Intf.Intf) (comp : Bench_comparison.t) =
  ...
  let valid_results = List.filter_map (fun x -> x) case_results in
  
  (* Create and report comparison summary *)
  if List.length valid_results >= 2 then begin
    let comp_result = 
      Bench_result.make_comparison_result comp.description valid_results
    in
...
let run_benchmarks ~config benchmarks =
  ...
  List.iter (fun item ->
    ...
    | Compare comp ->
        ...
        (* Comparisons don't contribute to main summary *)
  ) benchmarks;
