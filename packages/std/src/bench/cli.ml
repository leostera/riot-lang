...
let list_benchmarks benchmarks =
  List.iter
    (fun item ->
      match item with
      | Bench_runner.Single case -> println case.Bench_case.name
      | Bench_runner.Compare comp -> 
          println (comp.Bench_comparison.description ^ " (comparison)"))
    benchmarks;
  Ok ()
...
let apply_overrides ~iterations_override ~warmup_override benchmarks =
  ...
  | (None, None) -> benchmarks (* No overrides *)
  | _ ->
      List.map
        (fun item ->
          match item with
              ...
              let new_config =
                {
                  Bench_case.iterations =
                    Option.unwrap_or ~default:config.iterations
                      iterations_override;
                  warmup =
                    Option.unwrap_or ~default:config.warmup warmup_override;
                }
              in
                    ...
                    let new_config =
                      {
                        Bench_case.iterations =
                          Option.unwrap_or ~default:config.iterations
                            iterations_override;
                        warmup =
                          Option.unwrap_or ~default:config.warmup
                            warmup_override;
                      }
                    in
