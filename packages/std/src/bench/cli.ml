...
let run_benchmarks_cmd = let open Arg_parser.Arg in
command "run-benchmarks" |> about "Run benchmarks" |> args
  [
    ...
    option "format" |> long "format" |> help "Output format: default, json" |> default "default" |> possible_values [ "default"; "json" ];
...
let list_benchmarks_cmd = let open Arg_parser.Arg in
command "list-benchmarks" |> about "List all benchmarks" |> args
  [
...
let bench_item_to_json = fun index item ->
  ...
  | Bench_runner.Compare comp ->
      Data.Json.Object [
        ...
        "cases", Data.Json.Array (List.map comp.cases ~fn:(
          fun (case: Bench_case.t) -> Data.Json.String case.name
        ));
      ]
...
let list_benchmarks = fun ~json benchmarks ->
  if json then
    ...
    in
    let payload =
      benchmarks |> to_json_items 1 |> fun benchmarks ->
        Data.Json.Object [
          "benchmarks", Data.Json.Array benchmarks;
        ]
    in
    print (Data.Json.to_string payload);
    print "\n"
  else List.for_each benchmarks ~fn:(
    fun item ->
      match item with
...
let apply_overrides = fun ~iterations_override ~warmup_override benchmarks ->
  ...
  | _ -> List.map benchmarks ~fn:(
    fun item ->
      match item with
      | Bench_runner.Single case ->
          let config = case.Bench_case.config in
          let new_config = { Bench_case.iterations = Option.unwrap_or iterations_override ~default:config.iterations; warmup = Option.unwrap_or warmup_override ~default:config.warmup } in Bench_runner.Single ({ case with config = new_config })
      | Bench_runner.Compare comp ->
          (* Apply to all cases in comparison *)
          let new_cases = List.map comp.Bench_comparison.cases ~fn:(
            fun (case: Bench_case.t) ->
              let config = case.config in
              let new_config = { Bench_case.iterations = Option.unwrap_or iterations_override ~default:config.iterations; warmup = Option.unwrap_or warmup_override ~default:config.warmup } in { case with config = new_config }
          ) in Bench_runner.Compare ({ comp with cases = new_cases })
  )

let main = fun ~name ~benchmarks ~args ->
  let suite_info = get_suite_info name in
  let cmd = command name |> about ("Benchmark runner for " ^ name) |> subcommands [ list_benchmarks_cmd; run_benchmarks_cmd ] in
    ...
    | Some ("run-benchmarks", sub_matches) ->
        ...
        let format_str =
          if get_flag sub_matches "json" then
            "json"
          else get_one sub_matches "format" |> Option.unwrap_or ~default:"default"
        in
        let iterations_override = get_int sub_matches "iterations" in
        let warmup_override = get_int sub_matches "warmup" in
