...
let rec get_matches_internal = fun cmd args ->
  ...
  let validate_required () =
    let missing = List.find cmd.args ~fn:(
      fun arg ->
  ...
  let rec parse_args args_list =
    ...
    | subcmd :: rest -> (
      match List.find cmd.subcommands ~fn:(
        fun sub -> sub.name = subcmd
      ) with
      | Some sub -> (
      ...
      | None -> (* Prefer consuming as positional when not a subcommand *)
      parse_positional args_list
    )
  and parse_long_arg arg name rest =
    match arg.action with
    | SetTrue ->
        let _ = HashMap.insert matches.flags ~key:name ~value:1 in parse_args rest
    | Count ->
        let count = HashMap.get matches.flags ~key:name |> Option.unwrap_or ~default:0 in
        let _ = HashMap.insert matches.flags ~key:name ~value:(count + 1) in parse_args rest
    | Set | Append -> (
      match rest with
      | [] -> Error (InvalidValue (name, "missing value"))
      | value :: rest' ->
          let current = HashMap.get matches.values ~key:name |> Option.unwrap_or ~default:[] in
          let _ = HashMap.insert matches.values ~key:name ~value:(current @ [ value ]) in parse_args rest'
    )
    | SetFalse ->
        let _ = HashMap.insert matches.flags ~key:name ~value:0 in parse_args rest
  and parse_short_arg arg c rest = parse_long_arg arg arg.name rest
  and parse_positional pos_args =
    let positional_args: unit arg list = List.filter cmd.args ~fn:(
      fun positional_arg -> positional_arg.short = None && positional_arg.long = None
    ) in
    let unfilled_positionals: unit arg list = List.filter positional_args ~fn:(
    ...
    | arg :: _, value :: rest ->
        ...
        let _ = HashMap.insert matches.values ~key:arg.name ~value:(current @ [ value ]) in parse_args rest
    | arg :: _, [] when arg.required -> Error (MissingRequired arg.name)
    | _, [] -> validate_required ()
  in
  parse_args args
and find_arg_by_long = fun cmd long_name -> List.find cmd.args ~fn:(
  fun arg -> arg.long = Some long_name
)
and find_arg_by_short = fun cmd short_char -> List.find cmd.args ~fn:(
  fun arg -> arg.short = Some short_char
)
and print_help = fun cmd ->
  ...
  let positionals = List.filter cmd.args ~fn:(
    fun arg -> arg.short = None && arg.long = None
  ) in
  let options = List.filter cmd.args ~fn:(
    fun arg -> arg.short != None || arg.long != None
  ) in
  (* Usage section *)
  ...
  if List.length positionals > 0 then
    (
      println "\nArguments:";
      let max_arg_width: int = List.fold_left positionals ~init:0 ~fn:(
      ...
      ) in List.for_each positionals ~fn:(
        fun (arg: unit arg) ->
          let name =
            if arg.multiple then
              arg.name ^ "..."
  ...
  if List.length options > 0 then
    (
      ...
      let max_opt_width = List.fold_left options ~init:0 ~fn:(
        fun acc arg ->
          let short_len =
  ...
  if List.length cmd.subcommands > 0 then
    (
      ...
      let sorted_subs = List.sort cmd.subcommands ~compare:(
        fun a b -> String.compare a.name b.name
      ) in
      let max_name_len = List.fold_left sorted_subs ~init:0 ~fn:(
        fun acc sub -> max acc (String.length sub.name)
      ) in
      List.for_each sorted_subs ~fn:(
        fun sub ->
...
let get_flag = fun matches name -> (HashMap.get matches.flags ~key:name |> Option.unwrap_or ~default:0) > 0

let get_count = fun matches name -> HashMap.get matches.flags ~key:name |> Option.unwrap_or ~default:0

let get_many = fun matches name -> HashMap.get matches.values ~key:name |> Option.unwrap_or ~default:[]
...
let usage_string = fun cmd ->
  ...
  (* Add options if any *)
  let has_options = List.any cmd.args ~fn:(
    fun arg -> arg.short != None || arg.long != None
  ) in
  if has_options then
