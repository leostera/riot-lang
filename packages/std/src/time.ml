...
module Instant = struct
  ...
  (** TODO: The following system calls should be used by now() and implemented
      ...
      clock_gettime (Monotonic Clock) VXWorks clock_gettime (Monotonic Clock)
      SOLID get_tim WASI __wasi_clock_time_get (Monotonic Clock) Windows
      QueryPerformanceCounter Disclaimer: These system calls might change over
      time.
  ...
  let duration_since ~earlier later =
    ...
    else { secs = secs_diff; nanos = nanos_diff }
