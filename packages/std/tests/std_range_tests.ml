...
let test_intersection_respects_endpoint_inclusion = fun _ctx ->
  ...
  | (None, Some intersection) when Range.contains intersection 5
  && not (Range.contains intersection 4) -> Ok ()
...
let tests =
  Test.[
    ...
    case
      "is_empty tracks open and closed semantics"
      test_empty_detection_tracks_open_closed_semantics;
