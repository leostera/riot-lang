...
let test_create_sets_source_position_and_length_remaining = fun _ctx ->
  ...
  if String.equal (IoSlice.to_string (Cursor.source cursor)) "hello" && Int.equal (Cursor.position cursor) 0 && Int.equal (Cursor.length_remaining cursor) 5 then
    Ok ()
  else Error "Cursor.create should start at position 0 with the full length remaining"
...
let test_take_while_collects_prefix_and_returns_rest = fun _ctx ->
  ...
  if String.equal (IoSlice.to_string taken) "123" && String.equal (IoSlice.to_string (Cursor.remaining cursor)) "abc" then
    Ok ()
  else Error "Cursor.take_while should collect the matching prefix"
...
let test_skip_while_advances_without_returning_text = fun _ctx ->
  ...
  if String.equal (IoSlice.to_string (Cursor.remaining cursor)) "abc" then
    Ok ()
  else Error "Cursor.skip_while should advance while the predicate holds"

let test_take_until_stops_before_the_matching_character = fun _ctx ->
  match Cursor.take_until (Cursor.create "alpha:beta")
    (
      fun ch -> Char.equal ch ':'
    ) with
  | Some (taken, cursor) when String.equal (IoSlice.to_string taken) "alpha" && String.equal (IoSlice.to_string (Cursor.remaining cursor)) ":beta" -> Ok ()
...
let test_take_n_returns_requested_length = fun _ctx ->
  ...
  | Some (taken, cursor) when String.equal (IoSlice.to_string taken) "abc" && String.equal (IoSlice.to_string (Cursor.remaining cursor)) "def" -> Ok ()
...
let test_remaining_is_empty_at_eof = fun _ctx ->
  ...
  if String.equal (IoSlice.to_string (Cursor.remaining cursor)) "" && Cursor.is_eof cursor then
    Ok ()
  else Error "Cursor.remaining should be empty at EOF"
