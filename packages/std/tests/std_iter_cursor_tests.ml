...
let test_remaining_is_empty_at_eof = fun _ctx ->
  let cursor =
    Cursor.advance_by (Cursor.create "abc") 3
    |> Option.unwrap
  in
...
let tests =
  Test.[
    ...
    case
      "peek_n looks ahead without advancing"
      test_peek_n_returns_future_characters_without_advancing;
    ...
    case
      "skip_while advances without returning text"
      test_skip_while_advances_without_returning_text;
    case
