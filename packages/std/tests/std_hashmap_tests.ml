...
let sort_pairs = fun values -> List.sort values ~compare:(
  fun (left_key, _) (right_key, _) -> Int.compare left_key right_key
)
...
let test_with_capacity = fun _ctx ->
  ...
  if HashMap.is_empty map then
    Ok ()
  else Error "expected HashMap.with_capacity to start empty"
...
let test_insert_new = fun _ctx ->
  ...
  if Option.is_none previous && Int.equal (HashMap.length map) 1 && HashMap.get map ~key:1 = Some "a" then
    Ok ()
...
let test_get_existing = fun _ctx ->
  ...
  if HashMap.get map ~key:1 = Some "a" then
    Ok ()
  else Error "expected get existing key = Some value"
...
let test_get_missing = fun _ctx ->
  ...
  if HashMap.get map ~key:2 = None then
    Ok ()
  else Error "expected get missing key = None"
...
let test_remove_missing = fun _ctx ->
  ...
  if HashMap.remove map ~key:2 = None then
    Ok ()
  else Error "expected remove missing key = None"
...
let test_for_each = fun _ctx ->
  ...
  HashMap.for_each map ~fn:(
    fun key value -> seen.value <- (key, value) :: seen.value
  );
  if sort_pairs seen.value = [
    1, "a";
    2, "b";
  ] then
    Ok ()
...
let test_fold_left = fun _ctx ->
  ...
  in
  if Int.equal (HashMap.fold_left map ~init:0 ~fn:(
    fun acc _key value -> acc + value
  )) 5 then
    Ok ()
...
let test_to_list_roundtrip = fun _ctx ->
  ...
  if sort_pairs (HashMap.to_list rebuilt) = [
    1, "a";
    2, "b";
  ] then
    Ok ()
...
let test_and_modify_existing = fun _ctx ->
  ...
  in
  HashMap.and_modify map ~key:1 ~fn:(
    fun value -> value + 3
  );
