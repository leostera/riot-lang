...
let test_once_cell_starts_empty =
  Test.case "sync once_cell starts empty" (fun _ctx ->
...
let test_once_cell_set_initializes_once =
  Test.case "sync once_cell set initializes only once" (fun _ctx ->
    let cell = Sync.OnceCell.create () in
    match Sync.OnceCell.set cell "value" with
    | Error `AlreadyInitialized ->
        Error "expected the first set to succeed"
    | Ok () -> (
        match Sync.OnceCell.set cell "other" with
        | Ok () ->
            Error "expected the second set to fail"
        | Error `AlreadyInitialized ->
            if
              Sync.OnceCell.is_initialized cell
...
let test_once_cell_take_clears_storage =
  Test.case "sync once_cell take returns the value and clears storage" (fun _ctx ->
    let cell = Sync.OnceCell.create () in
    match Sync.OnceCell.set cell 9 with
    | Error `AlreadyInitialized ->
        Error "expected the initial set to succeed"
    | Ok () ->
...
let test_once_cell_get_or_init_runs_once =
  Test.case "sync once_cell get_or_init runs the initializer once" (fun _ctx ->
    let cell = Sync.OnceCell.create () in
...
let test_once_cell_get_or_try_init_retries_after_error =
  Test.case "sync once_cell get_or_try_init retries after an initialization error" (fun _ctx ->
    let cell = Sync.OnceCell.create () in
    let calls = Sync.Atomic.make 0 in
    let failing = fun () ->
      let _ = Sync.Atomic.fetch_and_add calls 1 in
    ...
    | Ok _ ->
        Error "expected the failing initializer to propagate its error"
    ...
    | Error _ -> (
        ...
        | Error reason ->
            Error ("expected the retry to succeed, got: " ^ reason)
        | Ok value ->
            if
