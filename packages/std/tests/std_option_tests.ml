...
let test_is_some_and = fun _ctx ->
  if Option.is_some_and (Some 4) ~fn:(
    fun value -> value > 0
  ) then
    Ok ()
...
let test_map_or = fun _ctx ->
  if Int.equal (Option.map_or None ~default:42 ~fn:(
    fun value -> value * 2
  )) 42 then
    Ok ()
...
let test_map_or_else = fun _ctx ->
  if Int.equal (Option.map_or_else (Some 5) ~default:(
    fun () -> 0
  ) ~fn:(
    fun value -> value + 1
  )) 6 then
...
let test_and_then = fun _ctx ->
  match Option.and_then (Some 3) ~fn:(
    fun value -> Some (value * value)
  ) with
  | Some 9 -> Ok ()
  | _ -> Error "expected and_then to chain Some values"

let test_or_else = fun _ctx ->
  match Option.or_else None ~fn:(
    fun () -> Some "fallback"
  ) with
  | Some "fallback" -> Ok ()
