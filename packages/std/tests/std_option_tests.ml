...
let test_equal = fun _ctx ->
  if
    Option.equal (Some 7) (Some 7) ~fn:Int.equal && not (Option.equal (Some 7) None ~fn:Int.equal)
  then
    Ok ()
  else
...
let test_map_or_else = fun _ctx ->
  if
    Int.equal (Option.map_or_else (Some 5) ~default:(fun () -> 0) ~fn:(fun value -> value + 1)) 6
  then
    Ok ()
  else
