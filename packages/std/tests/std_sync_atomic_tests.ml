...
let test_atomic_make_and_get =
  Test.case "sync atomic make stores and returns the initial value" (fun _ctx ->
...
let test_atomic_set_updates_value =
  Test.case "sync atomic set updates the stored value" (fun _ctx ->
...
let test_atomic_exchange_returns_previous_value =
  Test.case "sync atomic exchange returns the previous value and stores the new one" (fun _ctx ->
...
let test_atomic_compare_and_set_success_and_failure =
  Test.case "sync atomic compare_and_set only updates on a match" (fun _ctx ->
...
let test_atomic_fetch_and_add_returns_previous_value =
  Test.case "sync atomic fetch_and_add returns the previous value and increments" (fun _ctx ->
...
let test_atomic_fetch_and_add_serializes_concurrent_updates =
  Test.case "sync atomic fetch_and_add preserves concurrent increments" (fun _ctx ->
