...
let test_atomic_make_and_get =
  Test.case "sync atomic make stores and returns the initial value"
    (
      fun _ctx ->
        let atomic = Sync.Atomic.make "value" in
        if String.equal (Sync.Atomic.get atomic) "value" then
          Ok ()
        else Error "expected Atomic.make to preserve the initial value"
    )

let test_atomic_set_updates_value =
  Test.case "sync atomic set updates the stored value"
    (
      fun _ctx ->
        ...
        if Sync.Atomic.get atomic = 7 then
          Ok ()
        else Error "expected set to replace the stored value"
    )

let test_atomic_exchange_returns_previous_value =
  Test.case "sync atomic exchange returns the previous value and stores the new one"
    (
      fun _ctx ->
        ...
        if String.equal previous "left" && String.equal (Sync.Atomic.get atomic) "right" then
          Ok ()
        else Error "expected exchange to return the previous value and store the new one"
    )

let test_atomic_compare_and_set_success_and_failure =
  Test.case "sync atomic compare_and_set only updates on a match"
    (
      fun _ctx ->
        ...
        if first && not second && Sync.Atomic.get atomic = 9 then
          Ok ()
        else Error "expected compare_and_set to update only when the expected value matches"
    )

let test_atomic_fetch_and_add_returns_previous_value =
  Test.case "sync atomic fetch_and_add returns the previous value and increments"
    (
      fun _ctx ->
        ...
        if previous = 5 && Sync.Atomic.get atomic = 9 then
          Ok ()
        else Error "expected fetch_and_add to return the old value and apply the increment"
    )

let test_atomic_fetch_and_add_serializes_concurrent_updates =
  Test.case "sync atomic fetch_and_add preserves concurrent increments"
    (
      fun _ctx ->
        ...
        let rec await_workers remaining =
          if remaining = 0 then
            ...
            match await ~what:"atomic worker completion"
              (
                function
        ...
        | Ok () ->
            ...
            if Sync.Atomic.get counter = expected then
              Ok ()
            else Error "expected concurrent fetch_and_add calls to preserve every increment"
    )
