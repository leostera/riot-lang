...
let contains_id = fun ids needle ->
  List.any ids ~fn:(fun id -> SimpleGraph.Node_id.eq id needle)
...
let test_topo_sort_diamond_places_dependencies_first = fun _ctx ->
  ...
  | Ok sorted ->
      ...
      | Some root_pos, Some left_pos, Some right_pos, Some top_pos
        when root_pos < left_pos && root_pos < right_pos && left_pos < top_pos && right_pos < top_pos ->
          Ok ()
      | _ ->
          Error "topo_sort should place dependencies before dependents in a diamond graph")
...
let test_iter_visits_every_node_once = fun _ctx ->
  ...
  SimpleGraph.iter graph ~fn:(fun id _ ->
    Sync.Atomic.set seen (id :: Sync.Atomic.get seen));
...
let test_reachable_from_merges_multiple_start_nodes_without_duplicates = fun _ctx ->
  ...
  let shared_count =
    List.fold_left reachable ~acc:0 ~fn:(fun count id ->
      if SimpleGraph.Node_id.eq id shared.id then count + 1 else count)
  in
...
let test_node_id_accessors_are_consistent = fun _ctx ->
  ...
  else if not (String.equal (SimpleGraph.Node_id.to_string id) (Int.to_string (SimpleGraph.Node_id.to_int id))) then
...
let () =
  Runtime.run ~main:(fun ~args -> Test.Cli.main ~name:"graph_simple_graph" ~tests ~args) ~args:Env.args ()
