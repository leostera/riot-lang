...
let contains_id = fun ids needle ->
  List.any ids
    ~fn:(fun id ->
      SimpleGraph.Node_id.eq id needle)
...
let test_add_edge_records_dependency = fun _ctx ->
  ...
  if List.any a.deps
      ~fn:(fun id ->
        SimpleGraph.Node_id.eq id b.id) then
    Ok ()
  else
...
let test_topo_sort_diamond_places_dependencies_first = fun _ctx ->
  ...
  | Ok sorted ->
      ...
      let find_pos target =
        List.find positions
          ~fn:(fun (_, id) ->
            SimpleGraph.Node_id.eq id target)
...
let test_map_returns_one_item_per_node = fun _ctx ->
  ...
  let values =
    SimpleGraph.map graph ~fn:(fun (_id, node) -> node.value)
  in
  if
    List.length values = 2 && List.contains values ~value:"alpha" && List.contains values ~value:"beta"
  then
    Ok ()
  else
