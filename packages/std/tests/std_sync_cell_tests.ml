...
let test_cell_create_and_get =
  Test.case
    "sync cell create stores and returns the initial value"
    (fun _ctx ->
...
let test_cell_set_and_assign =
  Test.case
    "sync cell set and := update the stored value"
    (fun _ctx ->
...
let test_cell_incr_and_decr =
  Test.case
    "sync cell incr and decr adjust integer cells"
    (fun _ctx ->
...
let test_cell_replace_returns_old_value =
  Test.case
    "sync cell replace returns the old value and stores the new one"
    (fun _ctx ->
...
let test_cell_take_returns_old_value_and_sets_default =
  Test.case
    "sync cell take returns the old value and stores the default"
    (fun _ctx ->
...
let test_cell_swap_exchanges_values =
  Test.case
    "sync cell swap exchanges both cell values"
    (fun _ctx ->
      let left = Sync.Cell.create "left" in
      let right = Sync.Cell.create "right" in
      Sync.Cell.swap left right;
      if
        String.equal (Sync.Cell.get left) "right" && String.equal (Sync.Cell.get right) "left"
      then
        Ok ()
      else
...
let test_cell_compare_and_swap_updates_only_on_match =
  Test.case
    "sync cell compare_and_swap updates only when the expected value matches"
    (fun _ctx ->
...
let test_cell_equal_compares_stored_values =
  Test.case
    "sync cell equal compares the stored values"
    (fun _ctx ->
