    ...
    (
      fun _ctx ->
        ...
        if Sync.Cell.get cell = 42 && !cell = 42 then
          Ok ()
        else Error "expected Cell.create to preserve the initial value"
    )
    ...
    (
      fun _ctx ->
        ...
        if Sync.Cell.get cell = "right" then
          Ok ()
        else Error "expected set and := to update the stored value"
    )
  ...
  Test.case "sync cell update rewrites from the previous value"
    (
      fun _ctx ->
        ...
        Sync.Cell.update cell
          (
            fun value -> value * 3
          );
        if Sync.Cell.get cell = 15 then
          Ok ()
        else Error "expected update to replace the value with the callback result"
    )
    ...
    (
      fun _ctx ->
        ...
        if Sync.Cell.get cell = 9 then
          Ok ()
        else Error "expected incr/decr to adjust integer cells"
    )
    ...
    (
      fun _ctx ->
        ...
        if String.equal old_value "before" && String.equal (Sync.Cell.get cell) "after" then
          Ok ()
        else Error "expected replace to return the old value and store the new one"
    )
    ...
    (
      fun _ctx ->
        ...
        if taken = 8 && Sync.Cell.get cell = 0 then
          Ok ()
        else Error "expected take to return the previous value and install the default"
    )
    ...
    (
      fun _ctx ->
        ...
        if String.equal (Sync.Cell.get left) "right" && String.equal (Sync.Cell.get right) "left" then
          Ok ()
        else Error "expected swap to exchange both cell values"
    )
    ...
    (
      fun _ctx ->
        ...
        if first && not second && Sync.Cell.get cell = 7 then
          Ok ()
        else Error "expected compare_and_swap to update only on a matching value"
    )
    ...
    (
      fun _ctx ->
        ...
        if Sync.Cell.equal left right && not (Sync.Cell.equal left different) then
          Ok ()
        else Error "expected equal to compare stored values"
    )
