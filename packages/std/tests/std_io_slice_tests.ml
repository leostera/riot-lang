...
let test_io_slice_search_helpers = fun _ctx ->
  let slice =
    IO.IoVec.IoSlice.from_string "GET /path HTTP/1.1\r\n\r\n"
    |> Result.unwrap
  in
  if
...
let test_iobuffer_readable_uses_readable_bytes = fun _ctx ->
  ...
  in
  let actual =
    IO.IoBuffer.readable buffer
    |> IO.IoVec.IoSlice.to_string
  in
...
let tests = [
  ...
  Test.case
    "Std.IO.IoBuffer.readable exposes readable bytes"
    test_iobuffer_readable_uses_readable_bytes;
