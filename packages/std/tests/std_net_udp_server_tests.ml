...
let test_udp_server_serves_one_datagram = fun _ctx ->
  ...
  let handler ~socket ~from payload ~len =
    let received = Bytes.sub payload ~offset:0 ~len |> Result.expect ~msg:"server handler payload slice should be valid" |> Bytes.to_string in
  ...
  | Ok server -> (
    ...
    | Ok client ->
        ...
        match Net.UdpSocket.send_to client (Net.UdpServer.local_addr server) (Bytes.from_string "ping") () with
        | Error err ->
        ...
        | Ok _ -> (
          ...
          | _ -> (
            ...
            | Ok { bytes_read; _ } ->
                Net.UdpSocket.close client;
                if String.equal (Bytes.sub buffer ~offset:0 ~len:bytes_read |> Result.expect ~msg:"server reply slice should be valid" |> Bytes.to_string) "pong" then
                  Ok ()
                else Error "UdpServer reply payload was incorrect"
          )
        )
  )

let tests = Test.[ case "UdpServer bind and serve handle a datagram" test_udp_server_serves_one_datagram ]
