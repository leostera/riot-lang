...
let test_from_bytes_array_concatenates_segments = fun _ctx ->
  let iov =
    IoVec.from_bytes_array [|Bytes.from_string "ab"; Bytes.from_string "cd"|]
    |> Result.unwrap
  in
...
let test_for_each_visits_segments_in_insertion_order = fun _ctx ->
  let iov =
    IoVec.from_string_array [|"ab"; "c"; "def"|]
    |> Result.unwrap
  in
...
let test_sub_full_length_returns_all_bytes = fun _ctx ->
  ...
  in
  let full =
    IoVec.sub iov ~pos:0 ~len:(IoVec.length iov)
    |> Result.unwrap
  in
...
let tests =
  Test.[
    ...
    case
      "for_each visits segments in insertion order"
      test_for_each_visits_segments_in_insertion_order;
