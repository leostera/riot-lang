...
let test_with_capacity_creates_single_segment = fun _ctx ->
  let iov = Iovec.with_capacity 4 in
...
let test_from_bytes_roundtrips_through_to_bytes = fun _ctx ->
  ...
  let iov = Iovec.from_bytes bytes in
...
let test_from_string_roundtrips_through_to_string = fun _ctx ->
  let iov = Iovec.from_string "hello" in
...
let test_from_bytes_array_concatenates_segments = fun _ctx ->
  let iov = Iovec.from_bytes_array [| Bytes.from_string "ab"; Bytes.from_string "cd" |] in
...
let test_sub_returns_prefix = fun _ctx ->
  let iov = Iovec.from_string "abcdef" in
  if String.equal (Iovec.to_string (Iovec.sub iov ~len:3)) "abc" then
    Ok ()
  else
...
let test_sub_can_slice_across_segment_boundaries = fun _ctx ->
  let iov = Iovec.from_string_array [| "ab"; "cd"; "ef" |] in
  if String.equal (Iovec.to_string (Iovec.sub iov ~pos:1 ~len:4)) "bcde" then
    Ok ()
  else
...
let test_sub_full_length_returns_all_bytes = fun _ctx ->
  let iov = Iovec.from_string_array [| "ab"; "cd"; "ef" |] in
  let full = Iovec.sub iov ~pos:0 ~len:(Iovec.length iov) in
