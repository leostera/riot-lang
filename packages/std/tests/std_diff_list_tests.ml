...
let test_diff_identical_lists () =
  let diffs = diff_lists [ 1; 2 ] [ 1; 2 ] in
  if diffs = [] then Ok () else Error "Identical lists should produce no changes"

let test_diff_empty_lists () =
  let diffs = diff_lists [] [] in
  if not (Diff.has_changes diffs) then Ok ()
  else Error "Empty lists should produce no changes"
...
let test_diff_reordered_elements () =
  let diffs = diff_lists [ 1; 2 ] [ 2; 1 ] in
  if List.length (Diff.changes diffs) = 2 then Ok ()
  else Error "Expected both positions to change when reordered"

let test_diff_nested_lists () =
  let diffs =
    [ { Diff.path = [ Diff.Index 0 ]; kind = Diff.Changed ([ 1 ], [ 2 ]) } ]
  in
  match Diff.changes diffs with
...
let test_diff_mixed_changes () =
  ...
  then Ok ()
  else Error "Expected mixed list diff"
