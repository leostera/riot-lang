...
let rune_from_int = fun n -> Unicode.Rune.from_int n |> Option.unwrap

let test_read_line_preserves_leftover_content = fun _ctx ->
  let reader = IO.Reader.from_string "alpha\nbeta\ngamma" |> IO.BufReader.from_reader ~size:8 in
  ...
  | Ok line when String.equal (slice_to_string line) "alpha\n" -> (
      ...
      | Ok _ ->
          Error "BufReader.read_byte should read from the leftover buffer"
      | Error _ ->
          Error "BufReader.read_byte should not fail for in-memory readers"
    )
  | Ok _ ->
      Error "BufReader.read_line should preserve newline-terminated lines"
  | Error _ ->
      Error "BufReader.read_line should not fail for in-memory readers"

let test_peek_and_consume_use_buffered_window = fun _ctx ->
  let reader = IO.Reader.from_string "abcdef" |> IO.BufReader.from_reader ~size:4 in
  ...
  | Ok slice when String.equal (slice_to_string slice) "abcd" -> (
      ...
      | Ok _ ->
          Error "BufReader.consume should discard the requested prefix"
      | Error _ ->
          Error "BufReader.consume should not fail for in-memory readers"
    )
  | Ok _ ->
      Error "BufReader.peek should expose the first buffered chunk"
  | Error _ ->
      Error "BufReader.peek should not fail for in-memory readers"
...
let test_read_slice_returns_borrowed_delimited_chunks = fun _ctx ->
  ...
  | Ok first when String.equal (slice_to_string first) "alpha\n" -> (
      ...
      | Ok _ ->
          Error "BufReader.read_line should return the next borrowed line"
      | Error _ ->
          Error "BufReader.read_line should not fail for in-memory readers"
    )
  | Ok _ ->
      Error "BufReader.read_slice should return a delimiter-terminated borrowed chunk"
  | Error _ ->
      Error "BufReader.read_slice should not fail for in-memory readers"

let test_read_string_materializes_delimited_chunks = fun _ctx ->
  let reader = IO.Reader.from_string "alpha\nbeta" |> IO.BufReader.from_reader ~size:8 in
  ...
  | Ok _ ->
      Error "BufReader.read_string should materialize delimiter-terminated chunks"
  | Error _ ->
      Error "BufReader.read_string should not fail for in-memory readers"

let test_to_reader_round_trips_through_generic_io = fun _ctx ->
  let reader = IO.Reader.from_string "hello\nworld"
  ...
  | Ok () ->
      Error "BufReader.to_reader should preserve the first buffered chunk"
  | Error _ ->
      Error "BufReader.to_reader should not fail for in-memory readers"
...
let test_read_copies_into_owned_buffers = fun _ctx ->
  ...
  | Ok 2 when String.equal (IO.Buffer.contents first) "ab" -> (
      ...
      | Ok _ ->
          Error "BufReader.read should drain the buffered bytes before the next refill"
      | Error _ ->
          Error "BufReader.read should not fail for in-memory readers"
    )
  | Ok _ ->
      Error "BufReader.read should respect the destination buffer capacity"
  | Error _ ->
      Error "BufReader.read should not fail for in-memory readers"
...
let test_fill_reports_buffered_bytes = fun _ctx ->
  ...
  if not (Int.equal (IO.BufReader.size reader) 4) then
    ...
    | Ok _ ->
        Error "BufReader.fill should load the first buffered chunk"
    | Error _ ->
        Error "BufReader.fill should not fail for in-memory readers"
...
let test_buffered_exposes_current_window = fun _ctx ->
  ...
  | Ok slice when String.equal (slice_to_string slice) "abcd" -> (
      ...
      | Ok _ ->
          Error "BufReader.consume should discard the requested prefix from buffered slices"
      | Error _ ->
          Error "BufReader.consume should not fail for in-memory readers"
    )
  | Ok _ ->
      Error "BufReader.buffered should expose the full current buffered window"
  | Error _ ->
      Error "BufReader.buffered should not fail for in-memory readers"

let test_reset_switches_to_a_new_reader = fun _ctx ->
  let reader = IO.Reader.from_string "alpha\n" |> IO.BufReader.from_reader ~size:8 in
  match IO.BufReader.read_line reader with
  | Ok line when String.equal (slice_to_string line) "alpha\n" -> (
      IO.BufReader.reset reader ~reader:(IO.Reader.from_string "beta\n");
      match IO.BufReader.read_line reader with
      | Ok line when String.equal (slice_to_string line) "beta\n" -> Ok ()
      | Ok _ -> Error "BufReader.reset should replace the underlying reader and clear buffered bytes"
      | Error _ -> Error "BufReader.reset should not fail for in-memory readers"
    )
  | Ok _ ->
      Error "BufReader.read_line should consume the old reader before reset"
  | Error _ ->
      Error "BufReader.read_line should not fail for in-memory readers"

let test_read_rune_decodes_utf8_sequences = fun _ctx ->
  let payload = "A" ^ "\xC3\xA9" ^ "\xF0\x9F\x98\x80" in
  let reader = IO.Reader.from_string payload |> IO.BufReader.from_reader ~size:8 in
  ...
  | Ok rune when rune_to_int rune = rune_to_int (Unicode.Rune.from_char 'A') -> (
      ...
      | Ok _ ->
          Error "BufReader.read_rune should decode two-byte UTF-8 sequences"
      | Error _ ->
          Error "BufReader.read_rune should not fail for valid UTF-8"
    )
  | Ok _ ->
      Error "BufReader.read_rune should decode ASCII bytes as single-byte runes"
  | Error _ ->
      Error "BufReader.read_rune should not fail for valid UTF-8"

let test_read_rune_reports_invalid_data = fun _ctx ->
  let reader = IO.Reader.from_string "\xFF" |> IO.BufReader.from_reader ~size:4 in
  match IO.BufReader.read_rune reader with
...
let test_peek_and_consume_validate_counts = fun _ctx ->
  ...
  | Ok _ ->
      Error "BufReader.peek should require enough room for the requested exact length"
  | Error _ ->
      Error "BufReader.peek should report Buffer_full when len exceeds capacity"

let tests =
  Test.[
    case "BufReader.read_line preserves leftover content" test_read_line_preserves_leftover_content;
    case "BufReader.peek and consume use the buffered window" test_peek_and_consume_use_buffered_window;
    case "BufReader.read_slice returns borrowed delimited chunks" test_read_slice_returns_borrowed_delimited_chunks;
    case "BufReader.read_string materializes delimited chunks" test_read_string_materializes_delimited_chunks;
    ...
    case "BufReader.read_slice reports Buffer_full when delimiter does not fit" test_read_slice_reports_buffer_full_when_delimiter_does_not_fit;
