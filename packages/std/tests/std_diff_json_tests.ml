...
let test_diff_different_bools =
  Test.case "diff different booleans" @@ fun () ->
...
let test_diff_identical_floats =
  Test.case "diff identical floats" @@ fun () ->
...
let test_diff_different_floats =
  Test.case "diff different floats" @@ fun () ->
...
let test_diff_identical_strings =
  Test.case "diff identical strings" @@ fun () ->
...
let test_diff_array_element_changed =
  Test.case "diff array with one element changed" @@ fun () ->
...
let test_diff_array_shorter =
  ...
  if List.length removed >= 1 then Ok () else Error "Expected removed elements"

let test_diff_array_longer =
  Test.case "diff array with added elements" @@ fun () ->
  let a1 = Json.array [ Json.int 1 ] in
  let a2 = Json.array [ Json.int 1; Json.int 2; Json.int 3 ] in
  let diff = Json.diff a1 a2 in
  let added = additions diff in
  if List.length added >= 1 then Ok () else Error "Expected added elements"

let test_diff_array_completely_different =
  Test.case "diff completely different arrays" @@ fun () ->
  let a1 = Json.array [ Json.int 1; Json.int 2 ] in
  let a2 = Json.array [ Json.string "a"; Json.string "b" ] in
  let diff = Json.diff a1 a2 in
  if List.length diff > 0 then Ok () else Error "Expected differences"

let test_diff_nested_arrays =
  Test.case "diff nested arrays" @@ fun () ->
  let a1 =
    Json.array
      [
        Json.array [ Json.int 1; Json.int 2 ];
        Json.array [ Json.int 3; Json.int 4 ];
      ]
  in
  let a2 =
    Json.array
      [
        Json.array [ Json.int 1; Json.int 2 ];
        Json.array [ Json.int 3; Json.int 99 ];
      ]
  in
  let diff = Json.diff a1 a2 in
  match diff with
...
let test_diff_identical_objects =
  Test.case "diff identical objects" @@ fun () ->
...
let test_diff_object_field_added =
  Test.case "diff object with added field" @@ fun () ->
...
let test_diff_object_multiple_changes =
  ...
  in
  let o2 =
    Json.obj [ ("a", Json.int 1); ("b", Json.int 99); ("d", Json.int 4) ]
  in
  let diff = Json.diff o1 o2 in
        ...
        ( "user",
          Json.obj
            [
              ...
              ( "address",
                Json.obj
                  [ ("city", Json.string "NYC"); ("zip", Json.string "10001") ]
              );
            ] );
      ]
  in
        ...
        ( "user",
          Json.obj
            [
              ...
              ( "address",
                Json.obj
                  [ ("city", Json.string "SF"); ("zip", Json.string "10001") ]
              );
            ] );
      ]
  in
...
let test_diff_object_with_array =
  ...
  let o1 =
    Json.obj [ ("tags", Json.array [ Json.string "foo"; Json.string "bar" ]) ]
  in
  let o2 =
    Json.obj [ ("tags", Json.array [ Json.string "foo"; Json.string "baz" ]) ]
  in
  let diff = Json.diff o1 o2 in
  match diff with
  | [
            ...
            [
              Json.obj
                [
                  ("id", Json.int 1);
                  ( "roles",
                    Json.array [ Json.string "admin"; Json.string "moderator" ]
                  );
                ];
            ] );
      ]
...
let test_diff_value_to_null =
  Test.case "diff value to null" @@ fun () ->
...
let test_diff_empty_string_values =
  Test.case "diff empty strings" @@ fun () ->
  let diff = Json.diff (Json.string "") (Json.string "") in
  if List.length diff = 0 then Ok () else Error "Empty strings should match"

let test_diff_object_key_ordering =
  Test.case "diff objects with different key order" @@ fun () ->
  let o1 = Json.obj [ ("a", Json.int 1); ("b", Json.int 2) ] in
  let o2 = Json.obj [ ("b", Json.int 2); ("a", Json.int 1) ] in
  let diff = Json.diff o1 o2 in
  if List.length diff = 0 then Ok () else Error "Key order shouldn't matter"

let test_json_diff_with_helpers =
  Test.case "use helpers on JSON diff results" @@ fun () ->
  let o1 = Json.obj [ ("a", Json.int 1); ("b", Json.int 2) ] in
  ...
  if List.length added = 1 && List.length removed = 1 && List.length changed = 1
  then Ok ()
  else
    Error
...
let test_json_at_path =
  ...
  let o1 =
    Json.obj
      [
        ( "user",
          Json.obj [ ("name", Json.string "Alice"); ("age", Json.int 30) ] );
        ("config", Json.obj [ ("port", Json.int 8080) ]);
      ]
  in
  let o2 =
    Json.obj
