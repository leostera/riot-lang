...
let test_simple_string =
  Test.case "parse simple string value" @@ fun () ->
...
let test_quoted_string_with_escapes =
  Test.case "parse string with escapes" @@ fun () ->
...
let test_boolean_true =
  Test.case "parse boolean true" @@ fun () ->
...
let test_boolean_false =
  Test.case "parse boolean false" @@ fun () ->
...
let test_simple_array =
  Test.case "parse simple array" @@ fun () ->
...
let test_string_array =
  Test.case "parse string array" @@ fun () ->
  let input = {|tags = ["foo", "bar", "baz"]|} in
  match Toml.parse input with
  | Ok (Toml.Table items) -> (
      match get_array (List.assoc "tags" items) with
      | Some [ Toml.String "foo"; Toml.String "bar"; Toml.String "baz" ] ->
          Ok ()
...
let test_nested_array =
  Test.case "parse nested array" @@ fun () ->
...
let test_simple_inline_table =
  Test.case "parse simple inline table" @@ fun () ->
  let input = {|
[dependencies]
std = { path = "../std" }
|} in
...
let test_multiple_inline_tables =
  Test.case "parse multiple inline tables" @@ fun () ->
  let input =
    {|
[dependencies]
std = { path = "../std" }
...
let test_inline_table_multiple_keys =
  Test.case "parse inline table with multiple keys" @@ fun () ->
...
let test_inline_table_with_bool =
  Test.case "parse inline table with boolean" @@ fun () ->
  ...
  | Ok (Toml.Table items) -> (
      ...
      | Some attrs -> (
          match
            ( get_bool (List.assoc "enabled" attrs),
              get_bool (List.assoc "debug" attrs) )
          with
...
let test_nested_inline_tables =
  Test.case "parse nested inline tables" @@ fun () ->
...
let test_simple_section =
  Test.case "parse simple section" @@ fun () ->
  let input = {|
[package]
name = "test"
version = "1.0.0"
...
let test_multiple_sections =
  Test.case "parse multiple sections" @@ fun () ->
  let input =
    {|
[package]
name = "test"
...
let test_nested_section_names =
  Test.case "parse nested section names" @@ fun () ->
  let input = {|
[server.config]
port = "8080"
|} in
...
let test_array_of_tables_simple =
  Test.case "parse simple array of tables" @@ fun () ->
  let input =
    {|
[[bin]]
name = "tusk"
      ...
      | Some value -> (
          ...
          | Some bins when List.length bins = 2 -> (
              ...
              | [ Toml.Table bin1; Toml.Table bin2 ] -> (
                  match
                    ( get_string (List.assoc "name" bin1),
                      get_string (List.assoc "path" bin1) )
                  with
...
let test_array_of_tables_multiple_keys =
  Test.case "parse array of tables with multiple keys" @@ fun () ->
  let input =
    {|
[[fruits]]
name = "apple"
...
|}
  ...
  | Ok (Toml.Table sections) -> (
      ...
      | Some [ Toml.Table f1; Toml.Table f2 ]
...
let test_line_comment =
  Test.case "parse with line comment" @@ fun () ->
  let input = {|
# This is a comment
name = "test"
|} in
...
let test_inline_comment =
  Test.case "parse with inline comment" @@ fun () ->
...
let test_comment_in_section =
  Test.case "parse section with comment" @@ fun () ->
  let input = {|
...
let test_leading_whitespace =
  Test.case "parse with leading whitespace" @@ fun () ->
...
let test_trailing_whitespace =
  Test.case "parse with trailing whitespace" @@ fun () ->
  let input = {|name = "test"   |} in
  match Toml.parse input with
  | Ok (Toml.Table items) -> (
      match get_string (List.assoc "name" items) with
...
let test_whitespace_around_equals =
  Test.case "parse with whitespace around =" @@ fun () ->
  let input = {|name   =   "test"|} in
  match Toml.parse input with
  | Ok (Toml.Table items) -> (
      match get_string (List.assoc "name" items) with
...
let test_empty_lines =
  Test.case "parse with empty lines" @@ fun () ->
  let input = {|
...
let test_empty_string =
  Test.case "parse empty string value" @@ fun () ->
  let input = {|text = ""|} in
  match Toml.parse input with
  | Ok (Toml.Table items) -> (
      match get_string (List.assoc "text" items) with
...
let test_empty_inline_table =
  Test.case "parse empty inline table" @@ fun () ->
  let input = {|empty = {}|} in
  match Toml.parse input with
  | Ok (Toml.Table items) -> (
...
let test_empty_document =
  Test.case "parse empty document" @@ fun () ->
  let input = "" in
  match Toml.parse input with
  | Ok (Toml.Table []) -> Ok ()
  | Ok (Toml.Table items) ->
...
let test_only_comments =
  Test.case "parse document with only comments" @@ fun () ->
  let input = {|
# Comment 1
# Comment 2
# Comment 3
...
let test_real_tusk_toml =
  Test.case "parse actual tusk.toml structure" @@ fun () ->
  let content =
    {|[package]
name = "tusk"
version = "0.0.1"
...
let test_workspace_toml =
  Test.case "parse workspace.toml" @@ fun () ->
  let input =
    {|
[workspace]
...
let test_mixed_inline_and_section_tables =
  Test.case "parse mixed inline and section tables" @@ fun () ->
  let input = {|
inline = { a = "1", b = "2" }
...
let test_unterminated_string =
  Test.case "detect unterminated string" @@ fun () ->
...
let test_unterminated_array =
  Test.case "detect unterminated array" @@ fun () ->
...
let test_unterminated_inline_table =
  Test.case "detect unterminated inline table" @@ fun () ->
