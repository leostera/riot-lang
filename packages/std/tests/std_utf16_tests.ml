...
let test_code_units_ascii = fun _ctx ->
  ...
  | Some (rune, _) ->
      if Unicode.Utf16.code_units_of_rune rune = 1 then
        Ok ()
      else Error "ASCII rune should occupy one UTF-16 code unit"
...
let test_code_units_emoji = fun _ctx ->
  ...
  | Some (rune, _) ->
      if Unicode.Utf16.code_units_of_rune rune = 2 then
        Ok ()
      else Error "supplementary-plane rune should occupy two UTF-16 code units"
  | None -> Error "failed to decode emoji rune"

let test_position_of_offset_counts_surrogate_pairs = fun _ctx -> expect_position ~text:"a😀b" ~offset:5 ~line:0 ~character:3

let test_offset_of_position_counts_surrogate_pairs = fun _ctx -> expect_offset ~text:"a😀b" ~position:(position 0 3) ~offset:5

let test_position_of_offset_handles_crlf = fun _ctx -> expect_position ~text:"a\r\n😀\nZ" ~offset:7 ~line:1 ~character:2

let test_offset_of_position_handles_crlf = fun _ctx -> expect_offset ~text:"a\r\n😀\nZ" ~position:(position 1 2) ~offset:7
...
let tests = Test.[
  ...
  case "utf16 position_of_offset counts surrogate pairs" test_position_of_offset_counts_surrogate_pairs;
