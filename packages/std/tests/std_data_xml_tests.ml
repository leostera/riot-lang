...
let test_create_element_with_attrs = fun _ctx ->
  let elem = Xml.element "div" ~attrs:[
    "class", "test";
  ] [] in
  match elem with
...
let test_complex_document = fun _ctx ->
  let doc = Xml.element "html" [ Xml.element "head" [ Xml.element "title" [ Xml.text "Test" ] ]; Xml.element "body" [ Xml.element "h1" [ Xml.text "Hello" ]; Xml.element "p" [ Xml.text "World" ] ] ] in
