...
let producer_sizes_arb = Arbitrary.map_gen
...
let wait_for_producers = fun ~producer_count ->
  let rec loop remaining =
    if remaining = 0 then
      ...
      match
        await ~what:"queue property producer completion"
          (
            function
...
let collect_consumer_values = fun ~consumer_count ->
  let rec loop remaining acc =
    if remaining = 0 then
      ...
      match
        await ~what:"queue property consumer completion"
          (
            function
...
let preserves_per_producer_fifo = fun ~batches ~actual ->
  let rec loop producer remaining_batches =
    ...
    | batch :: rest ->
        let observed =
          List.filter actual ~fn:(fun value -> value / 1_000_000 = producer)
        in
...
let concurrent_producers_and_consumers_preserve_every_value =
  Property.for_all mixed_scenario_arb
    (fun (sizes, consumer_count) ->
      ...
      | Error error -> Property.fail
        ("mixed coordination failed while waiting for producers: " ^ error)
      | Ok () ->
          match collect_consumer_values ~consumer_count with
          | Error error -> Property.fail
            ("mixed coordination failed while waiting for consumers: " ^ error)
          | Ok consumed_lists ->
              let consumed = flatten consumed_lists in
...
let tests = [
  ...
  Test.property
    ~size:Test.Large "Queue concurrent producers preserve every value and per-producer FIFO order"
  ...
  Test.property
    ~size:Test.Large "Queue concurrent consumers drain each prefetched value exactly once"
    ~examples:concurrent_examples
    (fun ctx ->
      assert_property
        ctx
  ...
  Test.property
    ~size:Test.Large "Queue concurrent producers and consumers preserve every value"
    ~examples:concurrent_examples
    (fun ctx ->
      assert_property
        ctx
