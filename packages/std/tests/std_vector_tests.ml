...
let test_create = fun _ctx ->
  ...
  if Vector.is_empty vector && Int.equal (Vector.length vector) 0 then Ok ()
  else Error "expected Vector.create to start empty"

let test_with_capacity = fun _ctx ->
  let vector = Vector.with_capacity ~size:8 in
  if Vector.is_empty vector && Vector.capacity vector >= 8 then Ok ()
  else Error "expected Vector.with_capacity to reserve space"

let test_push_on_empty = fun _ctx ->
  let vector = Vector.create () in
  Vector.push vector ~value:1;
  if Vector.length vector = 1 && Vector.first vector = Some 1 && Vector.last vector = Some 1 then Ok ()
  else Error "expected first push to set length first and last"
...
let test_push_growth = fun _ctx ->
  ...
  if Vector.capacity vector > initial_capacity && Array.to_list (Vector.to_array vector) = [ 1; 2; 3; 4 ] then Ok ()
  else Error "expected growth to preserve contents"

let test_pop_empty = fun _ctx ->
  if Vector.pop (Vector.create ()) = None then Ok () else Error "expected Vector.pop empty = None"
...
let test_pop_non_empty = fun _ctx ->
  ...
  if popped = Some 3 && items = [ 1; 2 ] then Ok ()
  else Error "expected Vector.pop to remove last pushed value"

let test_insert_at_zero = fun _ctx ->
  let vector = Vector.from_list [ 2; 3 ] in
  Vector.insert vector ~at:0 ~value:1;
  if Array.to_list (Vector.to_array vector) = [ 1; 2; 3 ] then Ok ()
  else Error "expected insert at 0 to prepend"

let test_insert_at_length = fun _ctx ->
  let vector = Vector.from_list [ 1; 2 ] in
  Vector.insert vector ~at:2 ~value:3;
  if Array.to_list (Vector.to_array vector) = [ 1; 2; 3 ] then Ok ()
  else Error "expected insert at length to append"

let test_insert_middle = fun _ctx ->
  let vector = Vector.from_list [ 1; 3; 4 ] in
  Vector.insert vector ~at:1 ~value:2;
  if Array.to_list (Vector.to_array vector) = [ 1; 2; 3; 4 ] then Ok ()
  else Error "expected insert in middle to shift tail right"

let test_remove_out_of_bounds = fun _ctx ->
  if Vector.remove (Vector.from_list [ 1; 2 ]) ~at:3 = None then Ok ()
...
let test_remove_middle = fun _ctx ->
  ...
  let items = Array.to_list (Vector.to_array vector) in
  if removed = Some 2 && items = [ 1; 3 ] then Ok ()
  else Error "expected remove middle to compact tail"

let test_get = fun _ctx ->
  let vector = Vector.from_list [ 1; 2; 3 ] in
  if Vector.get vector ~at:1 = Some 2 && Vector.get vector ~at:3 = None then Ok ()
  else Error "expected get to handle valid and invalid indices"
...
let test_clear = fun _ctx ->
  let vector = Vector.from_list [ 1; 2; 3 ] in
  Vector.clear vector;
  if Vector.is_empty vector then Ok () else Error "expected clear to empty vector"

let test_reserve = fun _ctx ->
  let vector = Vector.create () in
  Vector.reserve vector ~size:10;
  if Vector.capacity vector >= 10 then Ok () else Error "expected reserve to increase capacity"

let test_append = fun _ctx ->
  let left = Vector.from_list [ 1; 2 ] in
  let right = Vector.from_list [ 3; 4 ] in
  Vector.append left right;
  if Array.to_list (Vector.to_array left) = [ 1; 2; 3; 4 ] && Vector.is_empty right then Ok ()
  else Error "expected append to move right into left and clear right"

let test_split_off = fun _ctx ->
  let vector = Vector.from_list [ 1; 2; 3; 4 ] in
  let tail = Vector.split_off vector ~at:2 in
  if Array.to_list (Vector.to_array vector) = [ 1; 2 ] && Array.to_list (Vector.to_array tail) = [ 3; 4 ] then Ok ()
...
let test_sort = fun _ctx ->
  ...
  Vector.sort vector;
  if Array.to_list (Vector.to_array vector) = [ 1; 2; 3 ] then Ok ()
  else Error "expected sort to order values ascending"

let test_reverse = fun _ctx ->
  let vector = Vector.from_list [ 1; 2; 3 ] in
  Vector.reverse vector;
  if Array.to_list (Vector.to_array vector) = [ 3; 2; 1 ] then Ok ()
  else Error "expected reverse to flip vector order"

let test_iter = fun _ctx ->
  if Iterator.to_list (Vector.iter (Vector.from_list [ 1; 2; 3 ])) = [ 1; 2; 3 ] then Ok ()
  else Error "expected iter to yield vector items in order"

let test_mut_iter = fun _ctx ->
  if MutIterator.to_list (Vector.mut_iter (Vector.from_list [ 1; 2; 3 ])) = [ 1; 2; 3 ] then Ok ()
  else Error "expected mut_iter to yield vector items in order"
