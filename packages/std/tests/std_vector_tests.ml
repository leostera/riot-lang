...
let test_push_growth = fun _ctx ->
  ...
  List.for_each
    ...
    ~fn:(
      fun value -> Vector.push vector ~value
    );
  if Vector.capacity vector > initial_capacity && Array.to_list (Vector.to_array vector) = [
    1;
    2;
    3;
    4;
...
let test_pop_empty = fun _ctx ->
  if Vector.pop (Vector.create ()) = None then
    Ok ()
  else Error "expected Vector.pop empty = None"
...
let test_insert_middle = fun _ctx ->
  ...
  if Array.to_list (Vector.to_array vector) = [
    1;
    2;
    3;
    4;
...
let test_clear = fun _ctx ->
  ...
  if Vector.is_empty vector then
    Ok ()
  else Error "expected clear to empty vector"
...
let test_concat = fun _ctx ->
  ...
  if Array.to_list (Vector.to_array combined) = [
    1;
    2;
    3;
    4;
  ] && Array.to_list (Vector.to_array left) = [ 1; 2 ] && Array.to_list (Vector.to_array right) = [ 3; 4 ] then
    Ok ()
  else Error "expected concat to copy both vectors without mutating them"
...
let test_extend = fun _ctx ->
  ...
  if Array.to_list (Vector.to_array left) = [
    ...
    3;
    4;
  ] && Array.to_list (Vector.to_array right) = [ 3; 4 ] then
    Ok ()
  else Error "expected extend to copy right into left without clearing right"
...
let test_split_off = fun _ctx ->
  ...
  if Array.to_list (Vector.to_array vector) = [ 1; 2 ] && Array.to_list (Vector.to_array tail) = [ 3; 4 ] then
    Ok ()
