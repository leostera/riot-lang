...
let test_body_from_string = fun _ctx ->
  ...
  if Body.length body != 5 then
    ...
    if Option.is_some (Body.to_slice_opt body) then
      Error "expected string-backed bodies to stay owned strings"
    else
      if not (String.equal (Body.to_string body) "hello") then
...
let test_body_from_slice = fun _ctx ->
  ...
  | Some borrowed ->
      if not (String.equal (IoSlice.to_string borrowed) "riot body") then
        ...
        if not (String.equal (Body.to_string body) "riot body") then
...
let test_request_with_body_slice = fun _ctx ->
  ...
  let request =
    Request.create Std.Net.Http.Method.Post uri |> fun request -> Request.with_body_slice request slice
  in
...
let test_response_builder_body_data = fun _ctx ->
  ...
  let response =
    Response.Builder.create Status.Ok |> fun builder -> Response.Builder.body_data builder body |> Response.Builder.build
  in
