...
let test_now_is_nondecreasing = fun _ctx ->
  ...
  if Instant.compare first second != Order.GT then
    Ok ()
  else Error "expected Instant.now to be nondecreasing"
...
let test_duration_since_same_instant_is_zero = fun _ctx ->
  ...
  if Duration.is_zero (Instant.duration_since ~earlier:instant instant) then
    Ok ()
  else Error "expected Instant.duration_since on the same instant to return zero"
...
let test_saturating_duration_since_swapped_returns_zero = fun _ctx ->
  ...
  if Duration.is_zero (Instant.saturating_duration_since ~earlier:later earlier) then
    Ok ()
  else Error "expected Instant.saturating_duration_since to clamp swapped instants to zero"
...
let test_elapsed_is_non_negative = fun _ctx ->
  ...
  if Duration.compare elapsed Duration.zero != Order.LT then
    Ok ()
  else Error "expected Instant.elapsed to be non-negative"
...
let test_add_then_duration_since_recovers_duration = fun _ctx ->
  ...
  if Duration.equal (Instant.duration_since ~earlier:start finish) delta then
    Ok ()
  else Error "expected Instant.add followed by duration_since to recover the original duration"
...
let test_compare_equal_min_and_max_obey_ordering_laws = fun _ctx ->
  ...
  if Instant.equal first first && Instant.compare first second = Order.LT && Instant.equal (Instant.min first second) first && Instant.equal (Instant.max first second) second then
    Ok ()
  else Error "expected Instant.compare/equal/min/max to obey standard ordering laws"
...
let test_duration_since_panics_when_earlier_is_later = fun _ctx ->
  ...
  try
    let _ = Instant.duration_since ~earlier:later earlier in Error "expected Instant.duration_since to panic when earlier > later"
  with
  | _ -> Ok ()
...
let main ~args = Test.Cli.main ~name:"Time.Instant" ~tests ~args ()
