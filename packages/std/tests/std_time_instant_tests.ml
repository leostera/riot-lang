...
let tests =
  Test.[
    ...
    case
      "Instant.duration_since on the same instant returns zero"
      test_duration_since_same_instant_is_zero;
    case
      "Instant.saturating_duration_since clamps swapped instants to zero"
      test_saturating_duration_since_swapped_returns_zero;
    case "Instant.elapsed is non-negative" test_elapsed_is_non_negative;
    case
      "Instant.add then duration_since recovers the duration"
      test_add_then_duration_since_recovers_duration;
    case
      "Instant.checked_add succeeds for representable values"
    ...
    case
      "Instant.compare/equal/min/max obey ordering laws"
      test_compare_equal_min_and_max_obey_ordering_laws;
    case
      "Instant.duration_since panics when earlier > later"
      test_duration_since_panics_when_earlier_is_later;
  ]
