      ...
      fun _ctx ->
        ...
        let rec wait_for_peer () =
          if Sync.Atomic.get counter >= 2 then
            ...
            if Time.Duration.to_millis (Time.Instant.elapsed started) >= 250 then
    ...
    (
      fun _ctx ->
        ...
        let result =
          ...
          else Error "expected linear suite to run one test at a time"
        in
        let _ = Sync.Atomic.fetch_and_add counter (-1) in result
    )
...
let sample_tests =
  [
    Test.case ~size:Test.Large "alpha_large"
      (
        fun _ctx -> Ok ()
      );
    ...
    Propane.property "gamma_property" Arbitrary.int
      (
        fun _ -> true
      );
    ...
    Test.case ~size:Test.Large "middle_large_case"
      (
        fun _ctx -> Ok ()
      );
    ...
    Test.case ~size:Test.Large "ordered_fast_second"
      (
        fun _ctx -> Ok ()
      );
    ...
    Test.case "ctx_probe"
      (
        fun ctx ->
          ...
          | Some "demo", Some workspace_root, Some binary_path when String.equal (Path.to_string workspace_root) "/tmp/demo-workspace" && String.equal (Path.to_string binary_path) "/tmp/demo-bin" -> Ok ()
          | _ -> Error "expected ctx_probe to receive structured context from --ctx"
      );
    Test.case "after_timeout"
      (
        fun _ctx -> Ok ()
      );
  ]

let linear_tests = [ make_linear_probe linear_probe_counter "linear_probe_alpha"; make_linear_probe linear_probe_counter "linear_probe_beta" ]
...
let split_lines = fun output -> output |> String.split ~by:"\n" |> List.filter ~fn:(
...
let parse_json_lines = fun stdout -> split_lines stdout |> List.map ~fn:(
  fun line -> Data.Json.of_string line |> Result.expect ~msg:"failed to parse jsonl line"
)
...
let last_summary_json = fun stdout -> parse_json_lines stdout |> List.reverse |> List.find ~fn:(
  fun json -> json_type json = Some "TestSummary"
) |> Option.expect ~msg:"expected a final TestSummary json line"

let assoc_value = fun key entries ->
  match List.find entries ~fn:(
    fun (entry_key, _) -> String.equal entry_key key
  ) with
...
let test_names_from_json = fun stdout ->
  ...
  | Some (Data.Json.Array tests) -> tests |> List.filter_map ~fn:(
    fun test_json ->
...
let run_sample_capture = fun args ->
  let cmd = Command.make (self_executable ()) ~env:[
    "PROPANE_TESTS", "7";
  ] ~args:("sample" :: args) in Command.output cmd |> Result.expect ~msg:"failed to run sample test cli"

let run_linear_sample_capture = fun args ->
  let cmd = Command.make (self_executable ()) ~env:[
    "PROPANE_TESTS", "7";
  ] ~args:("sample-linear" :: args) in Command.output cmd |> Result.expect ~msg:"failed to run linear sample test cli"
...
let test_list_tests_lists_all_cases = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    if names = expected then
      Ok ()
    else Error ("unexpected listed test names: " ^ String.concat ", " names)
...
let test_list_tests_json_includes_metadata = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    | first :: _ ->
        ...
        if has "index" (Data.Json.Int 1) && has "name" (Data.Json.String "alpha_large") && has "type" (Data.Json.String "test") && has "size" (Data.Json.String "large") && has "reliability" (Data.Json.String "stable") && has "skip" (Data.Json.Bool false) then
          Ok ()
        else Error "expected list-tests --json to include metadata fields"
...
let test_list_tests_respects_filters = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    let names = listed_test_fields_from_json output.stdout |> List.filter_map ~fn:(assoc_value "name") |> List.filter_map ~fn:(
      function
    ...
    if names = [ "flaky_then_ok" ] then
      Ok ()
    else Error ("unexpected filtered list test names: " ^ String.concat ", " names)
...
let test_list_tests_accepts_ctx_flag = fun _ctx ->
  let ctx_json = "{\"workspace_root\":\"/tmp/demo-workspace\",\
     ...
     \"built_binaries\":[{\"name\":\"demo\",\"path\":\"/tmp/demo-bin\"}]}" in
  let output =
    run_sample_capture
      [
        "list-tests";
  ...
  in
  if not (Int.equal output.status 0) then
    Error ("expected list-tests --ctx to succeed, got " ^ Int.to_string output.status ^ ": " ^ output.stdout)
  else
    let names = listed_test_fields_from_json output.stdout |> List.filter_map ~fn:(assoc_value "name") |> List.filter_map ~fn:(
      function
      | Data.Json.String name -> Some name
    ...
    if names = [ "ctx_probe" ] then
      Ok ()
    else Error ("unexpected list-tests --ctx names: " ^ String.concat ", " names)
...
let test_run_tests_pattern_matches_suffix_substring = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    if names = expected then
      Ok ()
    else Error ("unexpected filtered names for _large: " ^ String.concat ", " names)
...
let test_run_tests_pattern_matches_middle_substring = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    if names = [ "middle_large_case" ] then
      Ok ()
    else Error ("unexpected filtered names for large_case: " ^ String.concat ", " names)
...
let test_run_tests_returns_success_with_zero_matches = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    if test_names_from_json output.stdout = [] then
      Ok ()
    else Error "expected filtered run with no matches to report an empty test list"
...
let test_run_tests_json_flag_alias_emits_json = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    if names = expected then
      Ok ()
    else Error ("unexpected filtered names for --json: " ^ String.concat ", " names)
...
let test_run_tests_small_flag_filters_small_tests = fun _ctx ->
  ...
  if Int.equal output.status 0 then
    ...
    if names = expected then
      Ok ()
    else Error ("unexpected filtered names for --small: " ^ String.concat ", " names)
...
let test_run_tests_large_flag_filters_large_tests = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    if names = expected then
      Ok ()
    else Error ("unexpected filtered names for --large: " ^ String.concat ", " names)
...
let test_run_tests_flaky_flag_filters_flaky_tests = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    if test_names_from_json output.stdout = [ "flaky_then_ok" ] then
      Ok ()
    else Error ("unexpected filtered names for --flaky: " ^ String.concat ", " (test_names_from_json output.stdout))
...
let test_run_tests_json_includes_timing_fields = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    let tests_have_duration =
      match Data.Json.get_field "tests" json with
      | Some (Data.Json.Array tests) -> List.all tests ~fn:(
    ...
    let summary_has_duration =
      ...
      | None -> false
    in
    if has_int_field "started_at_us" json && has_int_field "completed_at_us" json && has_int_field "duration_us" json && summary_has_duration && tests_have_duration then
      Ok ()
    else Error "expected test json output to include timing fields"
...
let test_run_tests_json_includes_reliability_metadata = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    | Some (Data.Json.Array [ Data.Json.Object fields ]) ->
        ...
        if has "size" (Data.Json.String "small") && has "reliability" (Data.Json.String "flaky") && has "retry_attempts" (Data.Json.Int 2) && has "attempts" (Data.Json.Int 2) && has "status" (Data.Json.String "passed") then
          Ok ()
        else Error "expected flaky json output to include reliability metadata"
  ...
  if Int.equal output.status 0 then
    ...
    | Some (Data.Json.Array [ Data.Json.Object fields ]) ->
        ...
        if has "status" (Data.Json.String "timed_out") && has "timeout_ms" (Data.Json.Int 500) then
          Ok ()
        else Error "expected timeout probe json output to report a timeout"
...
let test_run_tests_json_emits_lifecycle_events = fun _ctx ->
  ...
  if has_event "TestSuiteStarted" && has_event "TestCaseStarted" && has_event "TestCaseAttemptStarted" && has_event "TestCaseAttemptFinished" && has_event "TestCaseCompleted" && has_event "TestSummary" then
    Ok ()
  else Error ("expected lifecycle json events, got: " ^ String.concat ", " event_types)

let test_run_tests_json_emits_property_metadata = fun _ctx ->
  let events = parse_json_lines (run_sample_capture [ "run-tests"; "gamma_property"; "--json" ]).stdout in
  match List.find events ~fn:(
    fun json -> json_type json = Some "TestCaseStarted"
  ) with
  | Some (Data.Json.Object fields) ->
      let has name value = assoc_value name fields = Some value in
      if has "name" (Data.Json.String "gamma_property") && has "test_type" (Data.Json.String "property") && has "examples" (Data.Json.Int 7) then
        Ok ()
      else Error "expected property test metadata in TestCaseStarted event"
  | _ -> Error "expected a TestCaseStarted event for gamma_property"

let test_run_tests_json_emits_property_progress = fun _ctx ->
  let events = parse_json_lines (run_sample_capture [ "run-tests"; "gamma_property"; "--json" ]).stdout in
  let progress_events = events |> List.filter_map ~fn:(
    fun json ->
  ...
  let property_passes = List.filter progress_events ~fn:(
    fun progress_type -> String.equal progress_type "property_iteration_passed"
  ) in
  if Int.equal (List.length property_passes) 7 then
    Ok ()
  else Error ("expected 7 property progress events, got: " ^ String.concat ", " progress_events)
...
let test_run_tests_json_emits_snapshot_progress = fun _ctx ->
  ...
  if List.contains progress_events ~value:"snapshot_assertion_started" && List.contains progress_events ~value:"snapshot_assertion_matched" then
    Ok ()
  else Error ("expected snapshot progress events, got: " ^ String.concat ", " progress_events)

let test_run_tests_json_emits_heartbeat_for_long_tests = fun _ctx ->
  let events = parse_json_lines (run_sample_capture [ "run-tests"; "large_timeout_probe"; "--json" ]).stdout in
  if List.exists
    (
      fun json -> json_type json = Some "TestCaseHeartbeat"
    )
    events then
    Ok ()
  else Error "expected a TestCaseHeartbeat event for a long-running test"
...
let test_run_tests_ctx_flag_populates_structured_context = fun _ctx ->
  ...
  if Int.equal output.status 0 then
    Ok ()
  else Error ("expected --ctx run to succeed, got " ^ Int.to_string output.status ^ ": " ^ output.stdout)
  ...
  if Int.equal output.status 0 then
    ...
    if not (names = expected) then
      ...
      | Some (Data.Json.Array tests) ->
          let find_status name = tests |> List.filter_map ~fn:(
            fun test_json ->
              match Data.Json.get_field "name" test_json, Data.Json.get_field "status" test_json with
              | (Some (Data.Json.String test_name), Some (Data.Json.String status)) when String.equal test_name name -> Some status
              | _ -> None
          ) |> List.head in
          if find_status "timeout_probe" = Some "timed_out" && find_status "after_timeout" = Some "passed" then
            Ok ()
          else Error "expected timeout_probe to time out and after_timeout to still run"
...
let test_run_tests_concurrency_flag_allows_overlap = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    if names = expected then
      Ok ()
    else Error ("unexpected concurrency probe names: " ^ String.concat ", " names)
...
let test_run_tests_concurrency_keeps_summary_order = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    if names = [ "ordered_slow_first"; "ordered_fast_second" ] then
      Ok ()
    else Error ("expected ordered summary results, got: " ^ String.concat ", " names)

let test_run_tests_linear_suite_forces_serial_execution = fun _ctx ->
  let output =
    run_linear_sample_capture
      [
        "run-tests";
        "linear_probe";
        "--json";
  ...
  if not (Int.equal output.status 0) then
    ...
    if names = [ "linear_probe_alpha"; "linear_probe_beta" ] then
      Ok ()
    else Error ("unexpected linear probe names: " ^ String.concat ", " names)
...
let linear_sample_main = fun ~args ->
  match args with
  | exe :: _sample :: rest -> Test.Cli.main ~execution_mode:Test.Cli.Linear ~name:"sample_linear" ~tests:linear_tests ~args:(exe :: rest) ()
