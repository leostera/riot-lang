...
let unique_var = fun prefix ->
  let suffix = Sync.Atomic.fetch_and_add unique_counter 1 in prefix ^ "_" ^ Int32.to_string (Process.id ()) ^ "_" ^ Int.to_string suffix
...
let test_get_float_invalid = fun _ctx ->
  with_var "STD_ENV_BAD_FLOAT" "wat"
    (
      fun () ->
...
let test_get_bool_true_false = fun _ctx ->
  with_var "STD_ENV_BOOL_TRUE" "true"
    (
      fun () ->
        match Env.get Env.Bool ~var:"STD_ENV_BOOL_TRUE" with
        | Some true ->
            with_var "STD_ENV_BOOL_FALSE" "false"
              (
                fun () ->
...
let test_get_bool_numeric = fun _ctx ->
  with_var "STD_ENV_BOOL_ONE" "1"
    (
      fun () ->
        match Env.get Env.Bool ~var:"STD_ENV_BOOL_ONE" with
        | Some true ->
            with_var "STD_ENV_BOOL_ZERO" "0"
              (
                fun () ->
...
let test_get_bool_is_case_insensitive = fun _ctx ->
  with_var "STD_ENV_BOOL_MIXED" "TrUe"
    (
      fun () ->
...
let test_get_char_rejects_multi_character_values = fun _ctx ->
  with_var "STD_ENV_CHAR_MULTI" "ab"
    (
      fun () ->
...
let test_vars_contains_inserted_pairs = fun _ctx ->
  ...
  with_var name_one "alpha"
    (
      fun () ->
        with_var name_two "beta"
          (
            fun () ->
              let vars = Env.vars () in
              let has_one = List.any vars ~fn:(
                fun (name, value) -> String.equal name name_one && String.equal value "alpha"
              ) in
              let has_two = List.any vars ~fn:(
                fun (name, value) -> String.equal name name_two && String.equal value "beta"
              ) in
              if has_one && has_two then
                Ok ()
              else Error "Env.vars should contain inserted key/value pairs"
          )
    )

let test_current_dir_matches_successful_set_current_dir = fun _ctx ->
  let original = Env.current_dir () in
  with_tempdir "std_env_current_dir"
    (
      fun dir ->
        ...
        | Ok () ->
            let result =
              ...
              | Ok current -> (
                ...
                | Ok _, Ok _ -> Error "Env.current_dir should reflect the directory set by Env.set_current_dir"
...
let test_set_current_dir_to_non_directory_returns_error = fun _ctx ->
  let original = Env.current_dir () in
  with_tempdir "std_env_non_directory"
    (
      fun dir ->
...
let tests = Test.[
  ...
  case "set returns the old value for previously set variables" test_set_on_previously_set_variable;
  case "vars contains inserted key value pairs" test_vars_contains_inserted_pairs;
  case "current_dir matches successful set_current_dir" test_current_dir_matches_successful_set_current_dir;
