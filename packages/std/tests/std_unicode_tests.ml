      ...
      if code = 0x48 && next_pos = 1 then
        ...
        let code_hex =
          ...
          let rec to_hex n acc len =
            if len = 0 then
              acc
            else to_hex (n / 16) (String.make ~len:1 ~char:(String.get_unchecked hex_chars ~at:(n mod 16)) ^ acc) (len - 1)
          in
          "U+" ^ to_hex code "" 4
        in
...
let test_wrap_lines_width_respected = fun _ctx ->
  ...
  let all_fit = List.all lines ~fn:(
    fun line -> String.width line <= 20
  ) in
  if all_fit then
    Ok ()
      ...
      if code = code2 then
        ...
        let to_hex n =
          let rec helper n acc =
            ...
            else helper (n / 16) (String.make ~len:1 ~char:(String.get_unchecked hex_chars ~at:(n mod 16)) ^ acc)
          in
          helper n ""
        in
...
let test_ascii_roundtrip_regression = fun _ctx ->
  ...
  let test_char c =
    ...
    | Some (r, _) ->
        let encoded = Unicode.Utf8.encode_rune r in encoded = String.make ~len:1 ~char:c
    | None -> false
  in
...
let test_integration_wrap_and_width = fun _ctx ->
  ...
  let all_valid = List.all lines ~fn:(
    fun line ->
      let w = String.width line in w <= 15
  ) in
  if all_valid then
    Ok ()
