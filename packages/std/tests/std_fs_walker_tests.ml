...
let should_skip_packages_subtree = fun ~root path -> (not (Path.equal root path)) && List.any packages_root_skipped_directories ~fn:(String.equal (Path.basename path))
...
let collect_file_paths = fun ~root ~sort ->
  let seen = ref [] in Fs.Walker.walk ~roots:[ root ] ~sort ~f:(
    fun entry ->
          ...
          with_tempdir "std_fs_walker"
            (
              fun root ->
                ...
                let seen = ref [] in
                Fs.Walker.walk ~roots:[ root ] ~sort:true ~f:(
                  fun entry ->
                    let path = Fs.Walker.FileItem.path entry in
                ...
                Test.assert_true (List.any seen ~fn:(String.equal (Path.to_string Path.(keep_dir / Path.v "a.ml"))));
                Test.assert_false (List.any seen ~fn:(String.equal (Path.to_string Path.(skip_dir / Path.v "b.ml"))));
                Ok ()
            )
      );
          ...
          with_tempdir "std_fs_walker_filter"
            (
              fun root ->
                ...
                Fs.write "let keep = ()\n" Path.(keep_dir / Path.v "a.ml") |> Result.expect ~msg:"write keep";
                Fs.write "let skip = ()\n" Path.(skip_dir / Path.v "b.ml") |> Result.expect ~msg:"write skip";
                let iter = Fs.Walker.create ~roots:[ root ] ~sort:true () |> Result.expect ~msg:"create walker" |> Fs.Walker.filter_entry ~f:(
                  fun entry ->
                    let path = Fs.Walker.FileItem.path entry in not (String.equal (Path.basename path) "skip")
                ) |> Fs.Walker.into_iter in
                let rec collect iter acc =
                  match Iter.Iterator.next iter with
                  | None, _ -> List.reverse acc
                  | Some (Ok (entry: Fs.Walker.FileItem.t)), iter' -> collect iter' (Fs.Walker.FileItem.path_string entry :: acc)
          ...
          with_tempdir "std_fs_walker_contents_first"
            (
              fun root ->
                ...
                let entries = Fs.Walker.create ~roots:[ root ] ~sort:true ~contents_first:true () |> Result.expect ~msg:"create walker" |> Fs.Walker.into_iter |> Iter.Iterator.to_list |> List.filter_map ~fn:(
                  function
                  | Ok (entry: Fs.Walker.FileItem.t) -> Some (Fs.Walker.FileItem.path_string entry)
                  | Error _ -> None
                ) in
                let file_index = find_index entries ~f:(String.equal (Path.to_string nested_file)) |> Option.expect ~msg:"expected nested file in walker output" in
                let dir_index = find_index entries ~f:(String.equal (Path.to_string nested_dir)) |> Option.expect ~msg:"expected nested dir in walker output" in
          ...
          with_tempdir "std_fs_walker_files_only"
            (
              fun root ->
                ...
                Fs.create_dir_all nested_dir |> Result.expect ~msg:"mkdir nested";
                Fs.write "let x = 1\n" nested_file |> Result.expect ~msg:"write nested";
                let entries = Fs.Walker.to_list ~roots:[ root ] ~sort:true ~include_directories:false () |> Result.expect ~msg:"to_list" |> List.map ~fn:Fs.Walker.FileItem.path_string in
          ...
          with_tempdir "std_fs_walker_multiple_roots"
            (
              fun root ->
                ...
                Fs.write "type handler = socket:Udp_socket.t -> bytes -> unit\n" Path.(net_dir / Path.v "udp_server.mli") |> Result.expect ~msg:"write udp_server.mli";
                Fs.write "type handler = socket:Udp_socket.t -> bytes -> unit\n" Path.(net_dir / Path.v "udp_server.ml") |> Result.expect ~msg:"write udp_server.ml";
                Fs.write "int shim(void) { return 1; }\n" Path.(native_dir / Path.v "shim.c") |> Result.expect ~msg:"write shim.c";
                let rec run iteration =
                  if iteration = 0 then
                    Ok ()
                  else
                    let actual = Fs.Walker.to_list ~roots:[ src_dir; native_dir ] ~sort:false ~include_directories:false () |> Result.expect ~msg:"to_list" |> List.map ~fn:Fs.Walker.FileItem.path_string |> sorted in
                    if actual = expected then
                      run (iteration - 1)
                    else Error ("expected unsorted multiple-root walker output [" ^ String.concat ", " expected ^ "] but got [" ^ String.concat ", " actual ^ "]")
                in
                run 25
            )
      );
    Test.case ~size:Large "fs walker packages file set is stable across sort modes"
      (
        fun _ctx ->
          ...
          let rec collect acc = function
            | [] -> Ok (List.reverse acc)
            | (label, sort) :: rest ->
                let* paths = collect_file_paths ~root:packages_root ~sort in collect ((label, paths) :: acc) rest
          in
