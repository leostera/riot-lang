...
let test_zero_min_and_max_constants = fun _ctx ->
  if not (Int32.equal Int32.zero 0l) then
    ...
    if not (Int32.equal Int32.min_int (-2_147_483_648l)) then
      Error "Int32.min_int should match the runtime value"
    else
      if not (Int32.equal Int32.max_int 2_147_483_647l) then
...
let test_neg_and_abs = fun _ctx ->
  if not (Int32.equal (Int32.neg 5l) (-5l)) then
    ...
    if not (Int32.equal (Int32.abs (-5l)) 5l) then
...
let test_basic_arithmetic = fun _ctx ->
  if not (Int32.equal (Int32.add 7l 3l) 10l) then
    ...
    if not (Int32.equal (Int32.sub 7l 3l) 4l) then
      Error "Int32.sub returned the wrong difference"
    else
      if not (Int32.equal (Int32.mul 7l 3l) 21l) then
        Error "Int32.mul returned the wrong product"
      else
        if not (Int32.equal (Int32.div 7l 3l) 2l) then
          Error "Int32.div should truncate toward zero"
        else
          if not (Int32.equal (Int32.rem 7l 3l) 1l) then
            Error "Int32.rem returned the wrong remainder"
          else Ok ()

let test_operator_syntax = fun _ctx -> let open Int32 in
if Int32.((7l + 3l) = 10l && (7l - 3l) = 4l && (7l * 3l) = 21l && (7l / 3l) = 2l && (7l mod 3l) = 1l && 7l > 3l) then
  Ok ()
else Error "Int32 operators should mirror the named helpers"
...
let test_bitwise_operations = fun _ctx ->
  if not (Int32.equal (Int32.logand 0b1100l 0b1010l) 0b1000l) then
    ...
    if not (Int32.equal (Int32.logor 0b1100l 0b0011l) 0b1111l) then
      Error "Int32.logor returned the wrong union"
    else
      if not (Int32.equal (Int32.logxor 0b1100l 0b1010l) 0b0110l) then
...
let test_bits_of_float_roundtrip = fun _ctx ->
  ...
  if Float.equal roundtrip value then
    Ok ()
  else Error "Int32.bits_of_float/float_of_bits should roundtrip the value"
...
let test_to_string_renders_signed_decimal = fun _ctx ->
  if String.equal (Int32.to_string (-123l)) "-123" then
    Ok ()
  else Error "Int32.to_string should render signed decimal text"
