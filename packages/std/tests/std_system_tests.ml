...
let test_target_triple_roundtrips_the_current_host = fun _ctx ->
  ...
  | Ok _ ->
      Error "System.TargetTriple.from_string should roundtrip System.host_triple"
  | Error message ->
      Error ("expected current host triple to parse, got: " ^ message)

let test_target_triple_rejects_incomplete_values = fun _ctx ->
  match System.TargetTriple.from_string "wasm32-wasi" with
  | Ok _ ->
      Error "System.TargetTriple.from_string should reject incomplete target triples"
  | Error _ ->
      Ok ()
...
let tests =
  Test.[
    ...
    case
      "System OS predicates agree with the current OS"
      test_os_predicates_match_current_os;
  ]
