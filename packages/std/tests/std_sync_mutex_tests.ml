...
let test_mutex_serializes_contended_access =
  Test.case
    "sync mutex serializes contended access"
    (fun _ctx ->
...
let test_mutex_try_lock_reports_held_and_free_states =
  Test.case
    "sync mutex try_lock reports held and free states"
    (fun _ctx ->
      let lock = Sync.Mutex.create () in
      let parent = self () in
      let holder =
...
let test_mutex_unlock_requires_ownership =
  Test.case
    "sync mutex unlock requires ownership"
    (fun _ctx ->
      let lock = Sync.Mutex.create () in
      let parent = self () in
      let holder =
...
let test_mutex_owner_exit_releases_lock =
  Test.case
    "sync mutex releases ownership when the owner exits"
    (fun _ctx ->
      let lock = Sync.Mutex.create () in
      let parent = self () in
      let owner =
