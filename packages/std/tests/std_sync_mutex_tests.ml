    ...
    (fun _ctx ->
      ...
      let rec collect remaining =
        if remaining = 0 then
          ...
          | Ok (`Overlap idx) -> Error ("mutex allowed overlapping access for worker "
          ^ Int.to_string idx)
      in
    ...
    (fun _ctx ->
      ...
      | Ok () ->
          if Sync.Mutex.try_lock lock then
            ...
            match
              await ~what:"mutex holder release"
                (
                  function
