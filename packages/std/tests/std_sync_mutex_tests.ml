...
let test_mutex_serializes_contended_access =
  Test.case "sync mutex serializes contended access"
    (
      fun _ctx ->
        ...
        let rec collect remaining =
          if remaining = 0 then
            ...
            match await ~what:"mutex workers"
              (
                function
            ...
            | Ok (`Overlap idx) -> Error ("mutex allowed overlapping access for worker " ^ Int.to_string idx)
        in
        match collect worker_count with
        | Error _ as err -> err
        | Ok () ->
            if Sync.Atomic.get completed = worker_count && Sync.Atomic.get active_holders = 0 then
              Ok ()
            else Error "mutex workers did not complete cleanly"
    )

let test_mutex_try_lock_reports_held_and_free_states =
  Test.case "sync mutex try_lock reports held and free states"
    (
      fun _ctx ->
        let lock = Sync.Mutex.create () in
        let parent = self () in
        ...
        in
        match await ~what:"mutex holder lock"
          (
            function
        ...
        | Ok () ->
            if Sync.Mutex.try_lock lock then
              ...
              (
                ...
                match await ~what:"mutex holder release"
                  (
                    function
...
let test_mutex_unlock_requires_ownership =
  Test.case "sync mutex unlock requires ownership"
    (
      fun _ctx ->
        let lock = Sync.Mutex.create () in
        let parent = self () in
        ...
        | Ok () ->
            ...
            let result =
              await ~what:"non-owner mutex unlock result"
                (
                  function
            ...
            ignore
              (
                await ~what:"mutex holder cleanup"
                  (
                    function
                    | Mutex_holder_released -> `select ()
                    | _ -> `skip
...
let test_mutex_owner_exit_releases_lock =
  Test.case "sync mutex releases ownership when the owner exits"
    (
      fun _ctx ->
        let lock = Sync.Mutex.create () in
        let parent = self () in
        ...
        match await ~what:"mutex owner ready"
          (
            function
        ...
        | Ok () ->
            ...
            (
              match await ~what:"mutex owner exit"
                (
                  function
