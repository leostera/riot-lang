...
let test_advance_moves_position_by_one = fun _ctx ->
  ...
  if
    Int.equal (MutCursor.position cursor) 1
    && String.equal (IoSlice.to_string (MutCursor.remaining cursor)) "ello"
  then
    Ok ()
  else
...
let test_advance_by_moves_within_bounds = fun _ctx ->
  ...
  if
    Int.equal (MutCursor.position cursor) 3
    && String.equal (IoSlice.to_string (MutCursor.remaining cursor)) "lo"
  then
    Ok ()
  else
...
let test_take_while_collects_prefix_and_advances = fun _ctx ->
  ...
  if
    ...
    && String.equal (IoSlice.to_string (MutCursor.remaining cursor)) "abc"
  then
    Ok ()
  else
...
let test_take_until_stops_before_the_matching_character = fun _ctx ->
  ...
  | Some taken when String.equal (IoSlice.to_string taken) "alpha"
  && String.equal (IoSlice.to_string (MutCursor.remaining cursor)) ":beta" -> Ok ()
...
let test_take_n_returns_requested_length = fun _ctx ->
  ...
  | Some taken when String.equal (IoSlice.to_string taken) "abc"
  && String.equal (IoSlice.to_string (MutCursor.remaining cursor)) "def" -> Ok ()
...
let test_remaining_is_empty_at_eof = fun _ctx ->
  ...
  MutCursor.advance_by cursor 3;
  if
    String.equal (IoSlice.to_string (MutCursor.remaining cursor)) "" && MutCursor.is_eof cursor
  then
    Ok ()
  else
...
let tests =
  Test.[
    case
      "create sets source position and remaining length"
      test_create_sets_source_position_and_length_remaining;
    ...
    case
      "take_until stops before the matching character"
      test_take_until_stops_before_the_matching_character;
    case
      "take_until_char stops before the matching character"
      test_take_until_char_stops_before_the_matching_character;
