...
let test_refcell_with_borrow_auto_releases =
  Test.case "sync refcell with_borrow acquires and releases automatically"
    (fun _ctx ->
      ...
      let observed =
        Sync.RefCell.with_borrow cell (fun value -> value)
      in
  ...
  Test.case "sync refcell with_borrow releases on exception"
    (fun _ctx ->
      ...
      let outcome =
        try
          let _ =
            Sync.RefCell.with_borrow cell (fun _ -> raise (Failure "boom"))
          in
  ...
  Test.case "sync refcell with_borrow_mut releases on exception"
    (fun _ctx ->
      ...
      let outcome =
        try
          let _ =
            Sync.RefCell.with_borrow_mut cell (fun _get _set -> raise (Failure "boom"))
          in
