  ...
  Test.case "sync refcell borrow_mut rejects shared borrows"
    (
      fun _ctx ->
        ...
        let outcome =
          try
            let _ = Sync.RefCell.borrow_mut cell in Error "expected borrow_mut to raise while shared borrowed"
          with
...
let test_refcell_borrow_rejects_mutable_borrow =
  Test.case "sync refcell borrow rejects a mutable borrow"
    (
      fun _ctx ->
        let cell = Sync.RefCell.create 1 in
        ...
        let outcome =
          try
            let _ = Sync.RefCell.borrow cell in Error "expected borrow to raise while mutably borrowed"
          with
...
let test_refcell_borrow_mut_allows_read_and_write =
  Test.case "sync refcell borrow_mut allows get_mut and set_mut"
    (
      fun _ctx ->
        ...
        if initial = 10 && updated = 99 && Sync.RefCell.get_unchecked cell = 99 then
          Ok ()
        else Error "expected borrow_mut to allow reading and updating the stored value"
    )

let test_refcell_releases_restore_availability =
  Test.case "sync refcell release helpers restore availability"
    (
      fun _ctx ->
        let cell = Sync.RefCell.create "value" in
...
let test_refcell_with_borrow_auto_releases =
  Test.case "sync refcell with_borrow acquires and releases automatically"
    (
      fun _ctx ->
        let cell = Sync.RefCell.create 21 in
        let observed =
          Sync.RefCell.with_borrow cell
            (
              fun value -> value
            )
        in
        if observed = 21 && not (Sync.RefCell.is_borrowed cell) && Sync.RefCell.borrow_count cell = 0 then
          Ok ()
        else Error "expected with_borrow to release its shared borrow automatically"
    )

let test_refcell_with_borrow_mut_auto_releases =
  Test.case "sync refcell with_borrow_mut acquires and releases automatically"
    (
      fun _ctx ->
        let cell = Sync.RefCell.create 5 in
        let observed =
          Sync.RefCell.with_borrow_mut cell
            (
              fun get set ->
                let before = get () in
                set 8;
        ...
        in
        if observed = 5 && Sync.RefCell.get_unchecked cell = 8 && not (Sync.RefCell.is_borrowed cell) && Sync.RefCell.borrow_count cell = 0 then
          Ok ()
        else Error "expected with_borrow_mut to release its mutable borrow automatically"
    )

let test_refcell_with_borrow_releases_on_exception =
  Test.case "sync refcell with_borrow releases on exception"
    (
      fun _ctx ->
        ...
        | Ok () ->
            if not (Sync.RefCell.is_borrowed cell) && Sync.RefCell.borrow_count cell = 0 then
              Ok ()
            else Error "expected with_borrow to release after an exception"
    )

let test_refcell_with_borrow_mut_releases_on_exception =
  Test.case "sync refcell with_borrow_mut releases on exception"
    (
      fun _ctx ->
        let cell = Sync.RefCell.create 7 in
        let outcome =
          try
            let _ =
              Sync.RefCell.with_borrow_mut cell
                (
                  fun _get _set -> raise (Failure "boom")
                )
            in
        ...
        | Ok () ->
            if not (Sync.RefCell.is_borrowed cell) && Sync.RefCell.borrow_count cell = 0 then
              Ok ()
            else Error "expected with_borrow_mut to release after an exception"
    )
