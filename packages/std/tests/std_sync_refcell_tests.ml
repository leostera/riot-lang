...
let test_refcell_shared_borrows_increment_count =
  Test.case
    "sync refcell allows shared borrows and counts them"
    (fun _ctx ->
...
let test_refcell_borrow_mut_rejects_shared_borrows =
  Test.case
    "sync refcell borrow_mut rejects shared borrows"
    (fun _ctx ->
...
let test_refcell_borrow_rejects_mutable_borrow =
  Test.case
    "sync refcell borrow rejects a mutable borrow"
    (fun _ctx ->
      let cell = Sync.RefCell.create 1 in
...
let test_refcell_borrow_mut_allows_read_and_write =
  Test.case
    "sync refcell borrow_mut allows get_mut and set_mut"
    (fun _ctx ->
...
let test_refcell_releases_restore_availability =
  Test.case
    "sync refcell release helpers restore availability"
    (fun _ctx ->
      let cell = Sync.RefCell.create "value" in
...
let test_refcell_with_borrow_mut_auto_releases =
  Test.case
    "sync refcell with_borrow_mut acquires and releases automatically"
    (fun _ctx ->
      let cell = Sync.RefCell.create 5 in
      let observed =
        Sync.RefCell.with_borrow_mut
          cell
          (fun get set ->
            let before = get () in
            set 8;
...
let test_refcell_with_borrow_releases_on_exception =
  Test.case
    "sync refcell with_borrow releases on exception"
    (fun _ctx ->
...
let test_refcell_with_borrow_mut_releases_on_exception =
  Test.case
    "sync refcell with_borrow_mut releases on exception"
    (fun _ctx ->
      let cell = Sync.RefCell.create 7 in
      let outcome =
        try
