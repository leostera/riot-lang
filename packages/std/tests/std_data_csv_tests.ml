...
let test_serialize_simple_row = fun () ->
  let row = [ "a"; "b"; "c" ] in
...
let test_serialize_with_comma = fun () ->
  let row = [ "a,b"; "c" ] in
  let str = Csv.to_string [ row ] in
...
let test_serialize_with_quote = fun () ->
  let row = [ "a\"b"; "c" ] in
  let str = Csv.to_string [ row ] in
  if String.contains str "\"" then
    Ok ()
...
let test_serialize_empty_field = fun () ->
  let row = [ "a"; ""; "c" ] in
  let str = Csv.to_string [ row ] in
  if String.contains str "," then
    Ok ()
...
let () =
  Miniriot.run ~main:(fun ~args -> Test.Cli.main ~name:"csv" ~tests ~args) ~args:Env.args ()
