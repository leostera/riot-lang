...
let bytes_set_string = fun dst ~offset ~width value ->
  let copy_len = Int.min width (String.length value) in IO.Bytes.blit_string value ~src_offset:0 ~dst ~dst_offset:offset ~len:copy_len
...
let octal_string = fun value ->
  let rec loop acc remaining =
    if Int64.equal remaining 0L then
      ...
      let ch = Char.from_int_unchecked (Char.to_int '0' + digit) in loop (String.make ~len:1 ~char:ch ^ acc) (Int64.div remaining 8L)
  in
...
let test_entries_lists_archive_members = fun _ctx ->
  ...
  | Ok entries ->
      let paths = List.map entries ~fn:(
        fun (entry: Tar.entry) -> Path.to_string entry.path
      ) in
      if paths = [ "src/"; "src/hello.txt" ] || paths = [ "src"; "src/hello.txt" ] then
        Ok ()
      else Error ("unexpected tar entries: " ^ String.concat ", " paths)
