...
let compare_with_baseline = fun input actual ->
  ...
  | Ok left, Ok right -> Error ("JsonStream parsed a different value. expected="
  ^ Json.to_string left
  ^ " actual="
  ^ Json.to_string right)
  | Error left, Error right -> Error ("JsonStream returned a different error. expected="
  ^ Json.error_to_string left
  ^ " actual="
  ^ Json.error_to_string right)
  | Ok left, Error right -> Error ("JsonStream failed when Json succeeded. expected="
  ^ Json.to_string left
  ^ " error="
  ^ Json.error_to_string right)
  | Error left, Ok right -> Error ("JsonStream succeeded when Json failed. error="
  ^ Json.error_to_string left
  ^ " actual="
  ^ Json.to_string right)
...
let test_extra_input_after_value_reports_position = fun _ctx ->
  ...
  | Error error -> Error ("expected Extra_input_after_value for trailing input, got "
...
let test_large_numeric_array_matches_json = fun _ctx ->
  let payload = "[" ^ (List.init ~count:5_000 ~fn:Int.to_string |> String.concat ",") ^ "]" in
