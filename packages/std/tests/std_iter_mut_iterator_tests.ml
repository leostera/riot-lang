...
let int_iter = fun items ->
  MutIterator.make (module ListMutIter) { ListMutIter.remaining = items }
...
let test_filter_map_drops_nones = fun _ctx ->
  let actual =
    MutIterator.filter_map (int_iter [ 1; 2; 3; 4 ])
      ~fn:(fun value ->
...
let test_flat_map_flattens_inner_iterators = fun _ctx ->
  let actual = MutIterator.flat_map
    (int_iter [ 1; 2; 3 ])
    ~fn:(fun value -> MutIterator.singleton (value * 10))
  |> MutIterator.to_list in
...
let test_fold_reduce_and_count_accumulate_items = fun _ctx ->
  let folded =
    MutIterator.fold (int_iter [ 1; 2; 3 ]) ~init:0 ~fn:(fun value acc -> acc + value)
  in
