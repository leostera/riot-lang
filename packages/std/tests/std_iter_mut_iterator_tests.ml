...
let test_take_and_drop_trim_the_sequence = fun _ctx ->
  let taken =
    MutIterator.take (int_iter [ 1; 2; 3; 4; ]) 2
    |> MutIterator.to_list
  in
  let dropped =
    MutIterator.drop (int_iter [ 1; 2; 3; 4; ]) 2
    |> MutIterator.to_list
  in
...
let test_enumerate_and_zip_produce_expected_pairs = fun _ctx ->
  let enumerated =
    MutIterator.enumerate (int_iter [ 10; 20 ])
    |> MutIterator.to_list
  in
  let zipped =
    MutIterator.zip (int_iter [ 1; 2; 3 ]) (int_iter [ 4; 5 ])
    |> MutIterator.to_list
  in
...
let test_chain_and_for_each_preserve_order = fun _ctx ->
  ...
  MutIterator.for_each
    chained
    ~fn:(fun value -> Sync.Atomic.set seen (value :: Sync.Atomic.get seen));
