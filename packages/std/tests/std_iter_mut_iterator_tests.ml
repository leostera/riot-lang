...
let test_clone_preserves_remaining_items = fun _ctx ->
  ...
  if MutIterator.to_list iter = [ 2; 3 ] && MutIterator.to_list clone = [ 2; 3 ] then
    ...
    Error "MutIterator.clone should preserve the remaining items"

let test_map_and_filter_transform_the_sequence = fun _ctx ->
  let actual =
    int_iter [ 1; 2; 3; 4 ]
    |> MutIterator.filter ~fn:is_even
    |> MutIterator.map ~fn:(fun value -> value * 10)
    |> MutIterator.to_list
  in
  if actual = [ 20; 40 ] then
    Ok ()
  else
    Error "MutIterator.map and filter should transform the sequence"

let test_filter_map_drops_nones = fun _ctx ->
  let actual =
    MutIterator.filter_map (int_iter [ 1; 2; 3; 4 ]) ~fn:(fun value ->
  ...
  if actual = [ 20; 40 ] then
    ...
    Error "MutIterator.filter_map should drop None values"

let test_flat_map_flattens_inner_iterators = fun _ctx ->
  let actual =
    MutIterator.flat_map (int_iter [ 1; 2; 3 ]) ~fn:(fun value ->
      MutIterator.singleton (value * 10))
    |> MutIterator.to_list
  in
  if actual = [ 10; 20; 30 ] then
    Ok ()
  else
    Error "MutIterator.flat_map should flatten inner iterators"

let test_fold_reduce_and_count_accumulate_items = fun _ctx ->
  let folded = MutIterator.fold (int_iter [ 1; 2; 3 ]) ~init:0 ~fn:(fun value acc -> acc + value) in
  let reduced = MutIterator.reduce (int_iter [ 1; 2; 3 ]) ~fn:( + ) in
  let counted = MutIterator.count (int_iter [ 1; 2; 3; 4 ]) in
  if Int.equal folded 6 && reduced = Some 6 && Int.equal counted 4 then
    Ok ()
  else
    Error "MutIterator.fold reduce and count should accumulate items"
...
let test_find_any_and_all_reflect_predicates = fun _ctx ->
  ...
  if found = Some 4 && any_even && all_even then
    ...
    Error "MutIterator.find/any/all should reflect predicate matches"

let test_take_and_drop_trim_the_sequence = fun _ctx ->
  let taken = MutIterator.take (int_iter [ 1; 2; 3; 4 ]) 2 |> MutIterator.to_list in
  let dropped = MutIterator.drop (int_iter [ 1; 2; 3; 4 ]) 2 |> MutIterator.to_list in
  if taken = [ 1; 2 ] && dropped = [ 3; 4 ] then
    Ok ()
  else
    Error "MutIterator.take and drop should trim the sequence"
...
let test_enumerate_and_zip_produce_expected_pairs = fun _ctx ->
  ...
  if enumerated = [ (0, 10); (1, 20) ] && zipped = [ (1, 4); (2, 5) ] then
    Ok ()
  else
    Error "MutIterator.enumerate and zip should produce expected pairs"
...
let test_chain_and_for_each_preserve_order = fun _ctx ->
  ...
  MutIterator.for_each chained ~fn:(fun value ->
    Sync.Atomic.set seen (value :: Sync.Atomic.get seen));
  if List.reverse (Sync.Atomic.get seen) = [ 1; 2; 3; 4 ] then
    Ok ()
  else
    Error "MutIterator.chain and for_each should preserve order"
...
let () =
  Runtime.run ~main:(fun ~args -> Test.Cli.main ~name:"iter_mut_iterator" ~tests ~args) ~args:Env.args ()
