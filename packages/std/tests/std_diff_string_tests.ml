...
let diff_strings left right =
  let max_len = max (String.length left) (String.length right) in
  let rec loop idx acc =
    if idx >= max_len then List.rev acc
    else
      let next =
        let left_char = if idx < String.length left then Some left.[idx] else None in
        let right_char =
          if idx < String.length right then Some right.[idx] else None
        in
...
let test_diff_identical_strings () =
  ...
  if diffs = [] then Ok () else Error "Identical strings should produce no changes"
...
let test_diff_one_empty () =
  ...
  if List.length (Diff.additions diffs) = 3 then Ok ()
  else Error "Expected all characters to be additions"

let test_diff_char_by_char () =
