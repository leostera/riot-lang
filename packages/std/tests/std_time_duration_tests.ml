...
let test_to_secs_string_respects_requested_precision = fun _ctx ->
  if
    String.equal (Duration.to_secs_string ~precision:3 (Duration.from_secs_float 1.125)) "1.125"
  then
    Ok ()
  else
...
let test_add_normalizes_nanoseconds = fun _ctx ->
  let result =
    Duration.add
      ...
      (Duration.make ~secs:2 ~nanos:200_000_000)
  in
  if
    Int.equal (Duration.to_secs result) 4 && Int.equal (Duration.subsec_nanos result) 100_000_000
  then
    Ok ()
  else
...
let test_sub_returns_positive_difference = fun _ctx ->
  ...
  in
  if
    Int.equal (Duration.to_secs result) 1 && Int.equal (Duration.subsec_nanos result) 700_000_000
  then
    Ok ()
  else
...
let test_mul_scales_both_seconds_and_nanoseconds = fun _ctx ->
  ...
  if
    Int.equal (Duration.to_secs result) 4 && Int.equal (Duration.subsec_nanos result) 500_000_000
  then
    Ok ()
  else
...
let tests =
  Test.[
    ...
    case
      "to_secs_string respects explicit precision"
      test_to_secs_string_respects_requested_precision;
    case
      "to_secs_string supports zero precision"
      test_to_secs_string_with_zero_precision_returns_whole_seconds;
    case
      "subsec_millis returns fractional milliseconds"
      test_subsec_millis_returns_fractional_milliseconds;
    case
      "subsec_micros returns fractional microseconds"
      test_subsec_micros_returns_fractional_microseconds;
    case
      "subsec_nanos returns fractional nanoseconds"
      test_subsec_nanos_returns_fractional_nanoseconds;
    ...
    case
      "checked_add succeeds on safe inputs"
      test_checked_add_returns_some_for_non_overflowing_inputs;
