...
let test_spec_dsl () =
  let spec = Config.Spec.for_app ~app:"testapp" [
...
let test_defaults () =
  let spec = Config.Spec.for_app ~app:"testapp" [
    Config.Spec.string "log_level" ~default:"info";
    Config.Spec.int "port" ~default:8080;
  ] in
...
let test_required_field () =
  let spec = Config.Spec.for_app ~app:"testapp" [
    Config.Spec.string "api_key" ~required:true;
  ] in
  
  let toml = Data.Toml.Table [] in
  
  match Config.Validator.validate spec toml with
  | Error _err -> Ok () (* Expected to fail *)
  | Ok _ -> Error "Should have failed on missing required field"

(* Test 9: Enum string - valid value *)
let test_enum_string_valid () =
  let spec = Config.Spec.for_app ~app:"testapp" [
    Config.Spec.enum 
      (Config.Spec.string "log_level" ~default:"info")
      [Config.Spec.String "debug"; Config.Spec.String "info"; Config.Spec.String "warn"; Config.Spec.String "error"];
  ] in
...
let test_enum_int_valid () =
  let spec = Config.Spec.for_app ~app:"testapp" [
    Config.Spec.enum 
      (Config.Spec.int "status_code" ~default:200)
      [Config.Spec.Int 200; Config.Spec.Int 201; Config.Spec.Int 400; Config.Spec.Int 404; Config.Spec.Int 500];
  ] in
...
let test_list_of_strings () =
  let spec = Config.Spec.for_app ~app:"testapp" [
    Config.Spec.list (Config.Spec.string "" ~default:"") "tags" ~default:[];
  ] in
  
  let toml = Data.Toml.Table [
    ("tags", Data.Toml.Array [
  ...
  | Ok (Config.Spec.Map validated) -> (
      ...
      | Some (Config.Spec.List items) ->
          ...
          if strings = ["tag1"; "tag2"; "tag3"] then Ok ()
          else Error "List items not correct"
      | _ -> Error "List not validated correctly")
  | Ok _ -> Error "Unexpected validation result"

(* Test 15: List with default *)
let test_list_default () =
  let spec = Config.Spec.for_app ~app:"testapp" [
    Config.Spec.list (Config.Spec.int "" ~default:0) "ports" ~default:[];
  ] in
...
let test_list_of_maps () =
  let spec = Config.Spec.for_app ~app:"testapp" [
    Config.Spec.list 
      (Config.Spec.map [
        Config.Spec.string "name" ~required:true;
        Config.Spec.int "age" ~default:0;
      ])
      "users"
      ~default:[];
  ] in
  ...
  | Ok (Config.Spec.Map validated) -> (
      ...
      | Some (Config.Spec.List items) ->
          if Collections.List.length items = 2 then Ok ()
          else Error ("Expected 2 users, got " ^ string_of_int (Collections.List.length items))
      | _ -> Error "List of maps not validated correctly")
  | Ok _ -> Error "Unexpected validation result"

(* Test 17: Discriminated union - console variant *)
let test_discriminated_union_console () =
  let spec = Config.Spec.for_app ~app:"testapp" [
    Config.Spec.key "handler"
      (Config.Spec.discriminated_union ~discriminant:"type" ~cases:[
        ("console", [
        ...
        ("file", [
          Config.Spec.string "id" ~required:true;
          Config.Spec.string "path" ~required:true;
        ]);
      ]);
  ] in
...
let test_discriminated_union_unknown () =
  let spec = Config.Spec.for_app ~app:"testapp" [
    Config.Spec.key "handler"
      (Config.Spec.discriminated_union ~discriminant:"type" ~cases:[
...
let test_list_of_discriminated_unions () =
  let spec = Config.Spec.for_app ~app:"testapp" [
    Config.Spec.list
      (Config.Spec.discriminated_union ~discriminant:"type" ~cases:[
        ("console", [
          Config.Spec.string "id" ~required:true;
          Config.Spec.list (Config.Spec.string "" ~default:"info") "levels" ~default:[];
        ]);
        ("file", [
          Config.Spec.string "id" ~required:true;
          Config.Spec.string "path" ~required:true;
...
let test_dotted_path_three_levels () =
  match Config.Loader.load_file "./packages/std/tests/fixtures/config/dotted.toml" with
  | Error err -> Error ("Failed to load file: " ^ err)
  | Ok toml -> (
...
let test_int_values_both_forms () =
  let spec = Config.Spec.for_app ~app:"testapp" [
    Config.Spec.int "port" ~default:8080;
    Config.Spec.int "timeout" ~default:30;
  ] in
  
  (* Test with Data.Toml.Int (native TOML integers) *)
  let toml_native = Data.Toml.Table [
    ("port", Data.Toml.Int 2112);
...
let () =
  Miniriot.run
    ~main:(fun ~args -> Test.Cli.main ~name:"config" ~tests ~args)
    ~args:Env.args ()
