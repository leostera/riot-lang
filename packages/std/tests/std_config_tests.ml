...
let assoc_value = fun entries key ->
  match Collections.List.find entries ~fn:(
...
let test_spec_dsl = fun _ctx ->
  let spec = Config.Spec.for_app ~app:"testapp" [ Config.Spec.key "server" (Config.Spec.map [ Config.Spec.string "host" ~default:"localhost"; Config.Spec.int "port" ~default:4_000 ]) ] in
  if Config.Spec.app_name spec = "testapp" then
    Ok ()
  else Error "Spec app name mismatch"
...
let test_env_detection = fun _ctx ->
  ...
  match env with
  | Config.Loader.Dev | Config.Loader.Test | Config.Loader.Prod -> Ok ()
...
let test_validation = fun _ctx ->
  ...
  let toml =
    Data.Toml.Table [
      "log_level", Data.Toml.String "debug";
    ]
  in
...
let test_defaults = fun _ctx ->
  let spec = Config.Spec.for_app ~app:"testapp" [ Config.Spec.string "log_level" ~default:"info"; Config.Spec.int "port" ~default:8_080 ] in
...
let test_nested_validation = fun _ctx ->
  ...
  let toml =
    Data.Toml.Table [
      "server", Data.Toml.Table [
        "host", Data.Toml.String "0.0.0.0";
        "port", Data.Toml.String "8080";
      ];
    ]
  in
  match Config.Validator.validate spec toml with
...
let test_enum_string_valid = fun _ctx ->
  let spec =
    Config.Spec.for_app ~app:"testapp"
      [
        Config.Spec.enum (Config.Spec.string "log_level" ~default:"info")
          [
...
let test_enum_string_invalid = fun _ctx ->
  ...
  let toml =
    Data.Toml.Table [
      "log_level", Data.Toml.String "invalid";
    ]
  in
  match Config.Validator.validate spec toml with
  | Error _err -> Ok ()
...
let test_enum_int_valid = fun _ctx ->
  let spec =
    Config.Spec.for_app ~app:"testapp"
      [
        Config.Spec.enum (Config.Spec.int "status_code" ~default:200)
          [
            Config.Spec.Int 200;
            Config.Spec.Int 201;
            Config.Spec.Int 400;
  ...
  let toml =
    Data.Toml.Table [
      "status_code", Data.Toml.String "404";
    ]
  in
  match Config.Validator.validate spec toml with
  | Error err -> Error ("Validation failed: " ^ err)
...
let test_enum_int_invalid = fun _ctx ->
  ...
  let toml =
    Data.Toml.Table [
      "status_code", Data.Toml.String "999";
    ]
  in
  match Config.Validator.validate spec toml with
  | Error _err -> Ok ()
...
let test_list_of_strings = fun _ctx ->
  let spec = Config.Spec.for_app ~app:"testapp" [ Config.Spec.list (Config.Spec.string "" ~default:"") "tags" ~default:[] ] in
  let toml =
    Data.Toml.Table [
      "tags", Data.Toml.Array [ Data.Toml.String "tag1"; Data.Toml.String "tag2"; Data.Toml.String "tag3" ];
    ]
  in
  match Config.Validator.validate spec toml with
  | Error err -> Error ("Validation failed: " ^ err)
  | Ok (Config.Spec.Map validated) -> (
    ...
    | Some (Config.Spec.List items) ->
        ...
        if strings = [ "tag1"; "tag2"; "tag3" ] then
          Ok ()
        else Error "List items not correct"
...
let test_list_default = fun _ctx ->
  let spec = Config.Spec.for_app ~app:"testapp" [ Config.Spec.list (Config.Spec.int "" ~default:0) "ports" ~default:[] ] in
  let toml = Data.Toml.Table [] in
  match Config.Validator.validate spec toml with
  | Error err -> Error ("Validation failed: " ^ err)
  | Ok (Config.Spec.Map validated) -> (
...
let test_list_of_maps = fun _ctx ->
  let spec = Config.Spec.for_app ~app:"testapp" [ Config.Spec.list (Config.Spec.map [ Config.Spec.string "name" ~required:true; Config.Spec.int "age" ~default:0 ]) "users" ~default:[] ] in
  let toml =
    Data.Toml.Table [
      "users", Data.Toml.Array [
        Data.Toml.Table [
          "name", Data.Toml.String "Alice";
          "age", Data.Toml.String "30";
        ];
        Data.Toml.Table [
          "name", Data.Toml.String "Bob";
          "age", Data.Toml.String "25";
        ];
      ];
    ]
  in
  ...
  | Ok (Config.Spec.Map validated) -> (
    ...
    | Some (Config.Spec.List items) ->
        if Collections.List.length items = 2 then
          Ok ()
        else Error ("Expected 2 users, got " ^ Int.to_string (Collections.List.length items))
...
let test_discriminated_union_console = fun _ctx ->
  let spec = Config.Spec.for_app ~app:"testapp" [ Config.Spec.key "handler" (Config.Spec.discriminated_union ~discriminant:"type" ~cases:[
    "console", [ Config.Spec.string "id" ~required:true; Config.Spec.string "format" ~default:"full" ];
    "file", [ Config.Spec.string "id" ~required:true; Config.Spec.string "path" ~required:true ];
  ]) ] in
  let toml =
    Data.Toml.Table [
      "handler", Data.Toml.Table [
  ...
  | Ok validated ->
      ...
      else Error ("Wrong variant: expected console, got " ^ variant)
...
let test_discriminated_union_unknown = fun _ctx ->
  ...
  let toml =
    Data.Toml.Table [
      "handler", Data.Toml.Table [
        "type", Data.Toml.String "syslog";
        "id", Data.Toml.String "remote";
      ];
    ]
  in
  match Config.Validator.validate spec toml with
...
let test_list_of_discriminated_unions = fun _ctx ->
  let spec = Config.Spec.for_app ~app:"testapp" [ Config.Spec.list (Config.Spec.discriminated_union ~discriminant:"type" ~cases:[
    "console", [ Config.Spec.string "id" ~required:true; Config.Spec.list (Config.Spec.string "" ~default:"info") "levels" ~default:[] ];
    "file", [ Config.Spec.string "id" ~required:true; Config.Spec.string "path" ~required:true ];
  ]) "handlers" ~default:[] ] in
  let toml =
    Data.Toml.Table [
      "handlers", Data.Toml.Array [
        Data.Toml.Table [
          "type", Data.Toml.String "console";
          "id", Data.Toml.String "main";
          ...
          (
            ...
            | Some handler ->
                ...
                if variant = "console" then
                  Ok ()
                else Error ("Expected console, got " ^ variant)
            | None -> Error "First handler not found"
          )
        else Error ("Expected 2 handlers, got " ^ Int.to_string (Collections.List.length items))
...
let test_int_values_both_forms = fun _ctx ->
  let spec = Config.Spec.for_app ~app:"testapp" [ Config.Spec.int "port" ~default:8_080; Config.Spec.int "timeout" ~default:30 ] in
