...
let test_spec_dsl = fun () ->
  ...
  [
    ...
    "server"
    (Config.Spec.map
    [ Config.Spec.string "host" ~default:"localhost"; Config.Spec.int "port" ~default:4_000;  ]);
...
let test_list_of_maps = fun () ->
  ...
  [
    Config.Spec.list
    (Config.Spec.map [ Config.Spec.string "name" ~required:true; Config.Spec.int "age" ~default:0;  ])
    "users"
    ~default:[];
...
let test_discriminated_union_console = fun () ->
  ...
  [
    ...
    ~cases:[
      (
        "console",
        [ Config.Spec.string "id" ~required:true; Config.Spec.string "format" ~default:"full";  ]
      );
      (
        "file",
...
let test_int_values_both_forms = fun () ->
  ...
  | Ok (Config.Spec.Map validated) -> (
      ...
      | Some (Config.Spec.Int 2_112) -> (
          ...
          | Ok _ ->
              Error "Unexpected validation result for string form"
        )
      | _ -> Error "Native form port not parsed correctly"
    )
  | Ok _ ->
      Error "Unexpected validation result for native form"
...
let () =
  Miniriot.run ~main:(fun ~args -> Test.Cli.main ~name:"config" ~tests ~args) ~args:Env.args ()
