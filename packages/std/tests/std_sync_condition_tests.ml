...
let test_condition_wait_releases_mutex_and_reacquires_after_signal =
  Test.case
    "sync condition wait releases the mutex and reacquires after signal"
    (fun _ctx ->
...
let test_condition_wait_requires_mutex_ownership =
  Test.case
    "sync condition wait requires owning the mutex"
    (fun _ctx ->
      let mutex = Sync.Mutex.create () in
      let condition = Sync.Condition.create () in
      let parent = self () in
...
let test_condition_broadcast_wakes_all_waiters =
  Test.case
    "sync condition broadcast wakes all waiting actors"
    (fun _ctx ->
      let mutex = Sync.Mutex.create () in
      let condition = Sync.Condition.create () in
      let parent = self () in
