  ...
  Test.case "sync condition wait releases the mutex and reacquires after signal"
    (
      fun _ctx ->
        ...
        match await ~what:"condition waiter readiness"
          (
            function
        ...
        | Ok () ->
            ...
            let rec collect remaining =
              if remaining = 0 then
                ...
                match await ~what:"condition signal flow"
                  (
                    function
  ...
  Test.case "sync condition wait requires owning the mutex"
    (
      fun _ctx ->
        ...
        match await ~what:"condition wait failure"
          (
            function
      ...
      fun _ctx ->
        ...
        let rec await_ready remaining =
          if remaining = 0 then
            ...
            match await ~what:"broadcast waiter readiness"
              (
                function
        ...
        | Ok () ->
            ...
            let rec collect remaining =
              if remaining = 0 then
                ...
                match await ~what:"broadcast wakeup"
                  (
                    function
                ...
                | Ok `Resumed ->
                    let _ = Sync.Atomic.fetch_and_add resumed 1 in collect (remaining - 1)
            in
            ...
            | Ok () ->
                if Sync.Atomic.get resumed = waiter_count then
                  Ok ()
                else Error "expected broadcast to wake every waiting actor"
    )

let name = "Sync.Condition"

let tests = [ test_condition_wait_releases_mutex_and_reacquires_after_signal; test_condition_wait_requires_mutex_ownership; test_condition_broadcast_wakes_all_waiters ]
