    ...
    (fun _ctx ->
      ...
      | Ok () ->
          ...
          | Ok () -> (
              ...
              | Some true ->
                  Error "condition waiter resumed before the signaler reported completion"
              | Some false ->
                  Error "condition waiter resumed without another actor acquiring the mutex"
              | None ->
                  Error "condition waiter never resumed"))

let test_condition_wait_requires_mutex_ownership =
  Test.case "sync condition wait requires owning the mutex" (fun _ctx ->
    let mutex = Sync.Mutex.create () in
    let condition = Sync.Condition.create () in
    let parent = self () in
    ignore
    ...
    | Ok (`Failed reason) ->
        Error ("unexpected condition wait failure: " ^ reason)
    | Ok `Returned ->
        Error "expected condition wait without mutex ownership to fail")

let test_condition_broadcast_wakes_all_waiters =
  Test.case "sync condition broadcast wakes all waiting actors" (fun _ctx ->
    let mutex = Sync.Mutex.create () in
    let condition = Sync.Condition.create () in
    let parent = self () in
