...
let diff_hashmaps = fun left right ->
  ...
  let keys =
    (
      HashMap.to_list left
      |> List.map ~fn:key_of_entry
    )
    @ (
      HashMap.to_list right
      |> List.map ~fn:key_of_entry
    )
...
let test_diff_nested_hashmaps = fun _ctx ->
  ...
  | [ { kind = Diff.Changed (left, right); _ } ] when HashMap.get left ~key:"y" = Some 2
  && HashMap.get right ~key:"y" = Some 3 -> Ok ()
