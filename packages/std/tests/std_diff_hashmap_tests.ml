...
let diff_hashmaps left right =
  let keys =
    (HashMap.to_list left |> List.map fst)
    @ (HashMap.to_list right |> List.map fst)
  ...
  List.filter_map
    (fun key ->
      ...
      | Some x, None ->
          Some { Diff.path = [ Diff.Key key ]; kind = Diff.Removed x }
      | Some x, Some y ->
          Some { Diff.path = [ Diff.Key key ]; kind = Diff.Changed (x, y) }
      | None, None -> None)
    keys

let make_map items = HashMap.of_list items

let test_diff_identical_hashmaps () =
  let diffs = diff_hashmaps (make_map [ ("a", 1) ]) (make_map [ ("a", 1) ]) in
  if not (Diff.has_changes diffs) then Ok ()
  else Error "Identical maps should produce no changes"

let test_diff_empty_hashmaps () =
  let diffs = diff_hashmaps (make_map []) (make_map []) in
  if diffs = [] then Ok () else Error "Empty maps should produce no changes"

let test_diff_added_keys () =
...
let test_diff_mixed_changes () =
  ...
  then Ok ()
  else Error "Expected one addition, one removal, and one change"

let test_diff_nested_hashmaps () =
  let before = make_map [ ("x", 1); ("y", 2) ] in
  let after = make_map [ ("x", 1); ("y", 3) ] in
  let nested_change =
    {
      Diff.path = [ Diff.Key "outer" ];
      kind = Diff.Changed (before, after);
    }
  in
  match Diff.changes [ nested_change ] with
  | [ { kind = Diff.Changed (left, right); _ } ]
    when HashMap.get left "y" = Some 2 && HashMap.get right "y" = Some 3 ->
      Ok ()
  | _ -> Error "Expected changed nested hashmap payload"

let test_diff_one_empty () =
  let diffs = diff_hashmaps (make_map []) (make_map [ ("a", 1); ("b", 2) ]) in
  if List.length (Diff.additions diffs) = 2 then Ok ()
  else Error "Expected additions for every key in the non-empty map"

let test_diff_different_sizes () =
  let diffs =
    diff_hashmaps
      (make_map [ ("a", 1) ])
      (make_map [ ("a", 1); ("b", 2); ("c", 3) ])
  in
  if List.length diffs = 2 then Ok ()
  else Error "Expected two additions from size mismatch"

let test_diff_all_different () =
  let diffs =
    diff_hashmaps
      (make_map [ ("a", 1); ("b", 2) ])
      (make_map [ ("c", 3); ("d", 4) ])
  in
  if List.length (Diff.additions diffs) = 2 && List.length (Diff.removals diffs) = 2
  then Ok ()
  else Error "Expected all keys to be removed or added"
