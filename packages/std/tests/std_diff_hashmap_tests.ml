...
let diff_hashmaps = fun left right ->
  ...
  let keys = (HashMap.to_list left |> List.map ~fn:key_of_entry) @ (HashMap.to_list right |> List.map ~fn:key_of_entry) |> List.unique ~compare:String.compare in List.filter_map keys ~fn:(
    fun key ->
      ...
      | Some x, Some y ->
          Some {
            Diff.path = [ Diff.Key key ];
            kind = Diff.Changed (x, y)
          }
      | None, None -> None
  )

let make_map = fun items -> HashMap.from_list items
...
let test_diff_empty_hashmaps = fun _ctx ->
  ...
  if diffs = [] then
    Ok ()
  else Error "Empty maps should produce no changes"
...
let test_diff_mixed_changes = fun _ctx ->
  ...
  in
  if List.length (Diff.additions diffs) = 1 && List.length (Diff.removals diffs) = 1 && List.length (Diff.changes diffs) = 1 then
    Ok ()
...
let test_diff_nested_hashmaps = fun _ctx ->
  ...
  | [ { kind = Diff.Changed (left, right); _ } ] when HashMap.get left ~key:"y" = Some 2 && HashMap.get right ~key:"y" = Some 3 -> Ok ()
  | _ -> Error "Expected changed nested hashmap payload"

let test_diff_one_empty = fun _ctx ->
  let diffs =
    diff_hashmaps (make_map [])
      (
        make_map
          [
            "a", 1;
