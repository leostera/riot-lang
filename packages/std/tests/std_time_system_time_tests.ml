...
let test_epoch_is_zero = fun _ctx ->
  if Int.equal (SystemTime.secs SystemTime.epoch) 0 && Int64.equal (SystemTime.nanos SystemTime.epoch) 0L && Int.equal (SystemTime.to_unix_timestamp SystemTime.epoch) 0 then
    Ok ()
  else Error "expected SystemTime.epoch to be zero in every representation"

let test_from_seconds_preserves_fractional_part = fun _ctx ->
  let time = SystemTime.from_seconds 1.5 in
  if Int.equal (SystemTime.secs time) 1 && Float.equal (SystemTime.secs_float time) 1.5 && Int64.equal (SystemTime.nanos time) 1_500_000_000L then
    Ok ()
  else Error "expected SystemTime.from_seconds 1.5 to represent exactly 1.5 seconds"

let test_from_nanos_preserves_exact_timestamp = fun _ctx ->
  let time = SystemTime.from_nanos 1_500_000_000L in
  if Int.equal (SystemTime.secs time) 1 && Float.equal (SystemTime.secs_float time) 1.5 && Int64.equal (SystemTime.nanos time) 1_500_000_000L then
    Ok ()
  else Error "expected SystemTime.from_nanos 1_500_000_000L to represent exactly 1.5 seconds"
...
let test_now_is_typically_nondecreasing = fun _ctx ->
  ...
  if SystemTime.compare first second != Order.GT then
    Ok ()
  else Error "expected SystemTime.now to be nondecreasing in normal operation"
...
let test_duration_since_same_time_is_zero = fun _ctx ->
  ...
  if Duration.is_zero (SystemTime.duration_since ~earlier:time time) then
    Ok ()
  else Error "expected SystemTime.duration_since on the same timestamp to return zero"
...
let test_elapsed_is_non_negative = fun _ctx ->
  ...
  if Duration.compare elapsed Duration.zero != Order.LT then
    Ok ()
  else Error "expected SystemTime.elapsed to be non-negative"
...
let test_add_then_duration_since_recovers_duration = fun _ctx ->
  ...
  if Duration.equal (SystemTime.duration_since ~earlier:start finish) delta then
    Ok ()
  else Error "expected SystemTime.add followed by duration_since to recover the original duration"
...
let test_sub_is_inverse_of_add_for_representable_durations = fun _ctx ->
  ...
  let shifted =
    SystemTime.sub start delta |> fun time -> SystemTime.add time delta
  in
  if SystemTime.equal shifted start then
    Ok ()
  else Error "expected SystemTime.sub followed by add to recover the original time"
...
let test_compare_equal_min_and_max_obey_ordering_laws = fun _ctx ->
  ...
  if SystemTime.equal first first && SystemTime.compare first second = Order.LT && SystemTime.equal (SystemTime.min first second) first && SystemTime.equal (SystemTime.max first second) second then
    Ok ()
  else Error "expected SystemTime.compare/equal/min/max to obey standard ordering laws"
...
let test_to_unix_timestamp_drops_fractional_seconds = fun _ctx ->
  if Int.equal (SystemTime.to_unix_timestamp (SystemTime.from_seconds 7.9)) 7 then
    Ok ()
  else Error "expected SystemTime.to_unix_timestamp to drop fractional seconds"
...
let test_duration_since_epoch_is_non_negative = fun _ctx ->
  if Duration.compare (SystemTime.duration_since_epoch ()) Duration.zero != Order.LT then
    Ok ()
  else Error "expected SystemTime.duration_since_epoch to be non-negative"
