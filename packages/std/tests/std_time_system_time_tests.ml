...
let test_sub_is_inverse_of_add_for_representable_durations = fun _ctx ->
  ...
  let shifted =
    ...
    |> fun time -> SystemTime.add time delta
  in
...
let tests =
  Test.[
    ...
    case
      "SystemTime.from_seconds preserves fractional seconds"
      test_from_seconds_preserves_fractional_part;
    case
      "SystemTime.from_nanos preserves the exact timestamp"
      test_from_nanos_preserves_exact_timestamp;
    case "SystemTime.now is typically nondecreasing" test_now_is_typically_nondecreasing;
    case
      "SystemTime.duration_since on the same timestamp returns zero"
      test_duration_since_same_time_is_zero;
    case "SystemTime.elapsed is non-negative" test_elapsed_is_non_negative;
    case
      "SystemTime.add then duration_since recovers the duration"
      test_add_then_duration_since_recovers_duration;
    case
      "SystemTime.sub is inverse of add for representable durations"
      test_sub_is_inverse_of_add_for_representable_durations;
    case
      "SystemTime.checked_add succeeds for representable values"
      test_checked_add_returns_some_for_representable_values;
    case "SystemTime.checked_sub returns None on underflow" test_checked_sub_underflow_returns_none;
    case
      "SystemTime.compare/equal/min/max obey ordering laws"
      test_compare_equal_min_and_max_obey_ordering_laws;
    case
      "SystemTime.to_unix_timestamp drops fractional seconds"
      test_to_unix_timestamp_drops_fractional_seconds;
