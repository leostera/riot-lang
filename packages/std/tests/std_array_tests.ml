...
let test_init = fun _ctx ->
  if Array.to_list (Array.init ~count:4 ~fn:(
    fun idx -> idx * idx
  )) = [
    0;
    1;
...
let test_length = fun _ctx ->
  if Int.equal
    ...
    3 then
    Ok ()
  else Error "expected array length = 3"
...
let test_set = fun _ctx ->
  ...
  if Array.to_list values = [ 1; 9; 3 ] then
    Ok ()
  else Error "expected Array.set to mutate one slot"
...
let test_clone = fun _ctx ->
  ...
  if Array.to_list clone = [ 1; 2; 3 ] then
    Ok ()
  else Error "expected Array.clone to detach copies"
  ...
  if String.equal
    (
      Array.fold_right
        ...
        ~init:""
        ~fn:(
          fun value acc -> Int.to_string value ^ acc
        )
    )
    "123" then
    Ok ()
  else Error "expected Array.fold_right to accumulate right-to-left"
