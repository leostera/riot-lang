...
let tests = [
  Test.case "fs walker rejects invalid depth ranges" (fun _ctx ->
    ...
    | Error (Fs.Walker.MinDepthCannotBeMoreThanMaxDepth { min_depth; max_depth }) ->
        ...
        Ok ()
    | Ok _ ->
        Error "expected invalid depth range");
  Test.case "fs walker can prune subtrees" (fun _ctx ->
    with_tempdir "std_fs_walker" (fun root ->
      ...
      Test.assert_true (List.exists (String.equal (Path.to_string Path.(keep_dir / Path.v "a.ml"))) seen);
      Test.assert_false (List.exists (String.equal (Path.to_string Path.(skip_dir / Path.v "b.ml"))) seen);
      Ok ()));
  Test.case "fs walker filter_entry skips directories lazily" (fun _ctx ->
    with_tempdir "std_fs_walker_filter" (fun root ->
      let keep_dir = Path.(root / Path.v "keep") in
      let skip_dir = Path.(root / Path.v "skip") in
      Fs.create_dir_all keep_dir |> Result.expect ~msg:"mkdir keep";
      Fs.create_dir_all skip_dir |> Result.expect ~msg:"mkdir skip";
      ...
      let iter =
        ...
        |> Fs.Walker.filter_entry ~f:(fun entry ->
          let path: Path.t = Fs.Walker.(entry.path) in
      ...
      let rec collect iter acc =
        ...
        | Some (Ok (entry: Fs.Walker.entry)), iter' ->
            collect iter' (Path.to_string Fs.Walker.(entry.path) :: acc)
        | Some (Error _err), iter' ->
            collect iter' acc
      in
      ...
      Test.assert_false (List.exists (String.equal (Path.to_string Path.(skip_dir / Path.v "b.ml"))) seen);
      Ok ()));
  Test.case "fs walker contents_first emits directories after descendants" (fun _ctx ->
    with_tempdir "std_fs_walker_contents_first" (fun root ->
      ...
      Ok ()));
  Test.case "fs walker to_list can omit directories" (fun _ctx ->
    with_tempdir "std_fs_walker_files_only" (fun root ->
      let nested_dir = Path.(root / Path.v "nested") in
      let nested_file = Path.(nested_dir / Path.v "a.ml") in
      Fs.create_dir_all nested_dir |> Result.expect ~msg:"mkdir nested";
      Fs.write "let x = 1\n" nested_file |> Result.expect ~msg:"write nested";
      let entries =
        ...
        |> List.map (fun (entry: Fs.Walker.entry) -> Path.to_string entry.path)
      in
...
let main = fun ~args ->
  Test.Cli.main ~name:"std_fs_walker_tests" ~tests ~args
