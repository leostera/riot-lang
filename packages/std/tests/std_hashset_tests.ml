...
let test_with_capacity = fun _ctx ->
  ...
  if HashSet.is_empty set then
    Ok ()
  else Error "expected HashSet.with_capacity to start empty"
...
let test_fold_left = fun _ctx ->
  let set = HashSet.from_list [ 1; 2; 3 ] in
  if Int.equal (HashSet.fold_left set ~init:0 ~fn:(
...
let test_mut_iter = fun _ctx ->
  if sort_ints (MutIterator.to_list (HashSet.mut_iter (HashSet.from_list [ 1; 2; 3 ]))) = [ 1; 2; 3 ] then
    Ok ()
