...
let test_create_with_custom_compare = fun _ctx ->
  let heap =
    Heap.create_with ~compare:(fun left right -> Int.compare (left mod 10) (right mod 10)) ()
  in
...
let test_from_list_with = fun _ctx ->
  if
    Heap.peek (Heap.from_list_with ~compare:(fun left right -> Int.compare right left) [ 3; 1; 2 ])
    = Some 3
  then
    Ok ()
  else
...
let test_pop_repeated = fun _ctx ->
  ...
  let popped =
    [ Heap.pop heap; Heap.pop heap; Heap.pop heap ]
...
let test_fold_left = fun _ctx ->
  ...
  if
    String.equal
      (Heap.fold_left heap ~init:"" ~fn:(fun acc value -> acc ^ Int.to_string value))
      "123"
  then
    Ok ()
  else
