...
let test_create = fun _ctx ->
  ...
  if Heap.is_empty heap then
    Ok ()
  else Error "expected Heap.create to start empty"

let test_create_max = fun _ctx ->
  let heap = Heap.create_max () in
  List.for_each [ 1; 3; 2 ] ~fn:(
    fun value -> Heap.push heap ~value
  );
  if Heap.peek heap = Some 3 then
    Ok ()
  else Error "expected max-heap peek to return maximum"

let test_create_with_custom_compare = fun _ctx ->
  let heap = Heap.create_with ~compare:(
    fun left right -> Int.compare (left mod 10) (right mod 10)
  ) () in
  List.for_each [ 32; 25; 18 ] ~fn:(
    fun value -> Heap.push heap ~value
  );
  if Heap.peek heap = Some 32 then
    Ok ()
  else Error "expected custom compare to determine heap root"
...
let test_from_list_with = fun _ctx ->
  if Heap.peek (Heap.from_list_with ~compare:(
    fun left right -> Int.compare right left
  ) [ 3; 1; 2 ]) = Some 3 then
    Ok ()
  else Error "expected from_list_with custom ordering to determine root"
...
let test_push_updates_peek = fun _ctx ->
  ...
  if Heap.peek heap = Some 1 then
    Ok ()
  else Error "expected push to update peek to smallest element"

let test_pop_empty = fun _ctx ->
  if Heap.pop (Heap.create ()) = None then
    Ok ()
  else Error "expected pop empty = None"

let test_pop_repeated = fun _ctx ->
  let heap = Heap.from_list [ 3; 1; 2 ] in
  let popped = [ Heap.pop heap; Heap.pop heap; Heap.pop heap ] |> List.filter_map ~fn:(
    fun value -> value
  ) in
...
let test_peek_empty = fun _ctx ->
  if Heap.peek (Heap.create ()) = None then
    Ok ()
  else Error "expected peek empty = None"
...
let test_length = fun _ctx ->
  ...
  if Int.equal (Heap.length heap) 3 then
    Ok ()
  else Error "expected Heap.length to track live items"
...
let test_is_empty_after_removing_all = fun _ctx ->
  ...
  if Heap.is_empty heap then
    Ok ()
  else Error "expected heap to be empty after removing all items"
...
let test_clear = fun _ctx ->
  ...
  if Heap.is_empty heap then
    Ok ()
  else Error "expected clear to empty heap"
...
let test_fold_left = fun _ctx ->
  let heap = Heap.from_list [ 3; 1; 2 ] in
  if String.equal (Heap.fold_left heap ~init:"" ~fn:(
