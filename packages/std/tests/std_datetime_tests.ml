...
let test_parse_utc_basic = fun _ctx ->
  ...
  | Ok dt ->
      if dt.year = 2_025 && dt.month = 8 && dt.day = 27 && dt.hour = 14 && dt.minute = 7 && dt.second = 31 && dt.time_zone = DateTime.Tz.Etc_UTC && dt.utc_offset = 0 then
        Ok ()
      else Error "Parsed datetime doesn't match expected values"
...
let test_parse_with_microseconds = fun _ctx ->
  ...
  | Ok dt ->
      ...
      if dt.year = 2_025 && dt.month = 8 && dt.day = 27 && dt.hour = 14 && dt.minute = 7 && dt.second = 31 && microseconds = 426_822 && precision = 6 && dt.time_zone = DateTime.Tz.Etc_UTC then
        Ok ()
      else Error "Parsed datetime with microseconds doesn't match expected values"
...
let test_parse_with_timezone_offset = fun _ctx ->
  ...
  | Ok dt ->
      if dt.year = 2_025 && dt.month = 8 && dt.day = 27 && dt.hour = 14 && dt.minute = 7 && dt.second = 31 && dt.time_zone = DateTime.Tz.Local && dt.utc_offset = 19_800 then
        Ok ()
      else Error "Parsed datetime with timezone offset doesn't match expected values"
...
let test_parse_negative_timezone_offset = fun _ctx ->
  ...
  | Ok dt ->
      if dt.year = 2_025 && dt.month = 8 && dt.day = 27 && dt.hour = 14 && dt.minute = 7 && dt.second = 31 && dt.time_zone = DateTime.Tz.Local && dt.utc_offset = (-25_200) then
        Ok ()
      else Error "Parsed datetime with negative timezone offset doesn't match expected values"
...
let test_parse_leap_year = fun _ctx ->
  ...
  | Ok dt ->
      if dt.year = 2_024 && dt.month = 2 && dt.day = 29 then
        Ok ()
      else Error "Failed to parse valid leap year date"
...
let test_roundtrip = fun _ctx ->
  ...
  | Ok parsed ->
      if parsed.year = original.year && parsed.month = original.month && parsed.day = original.day && parsed.hour = original.hour && parsed.minute = original.minute && parsed.second = original.second then
        Ok ()
      else Error "Roundtrip parsing failed"
...
let test_parse_with_space_separator = fun _ctx ->
  ...
  | Ok dt ->
      if dt.year = 2_025 && dt.month = 8 && dt.day = 27 && dt.hour = 14 && dt.minute = 7 && dt.second = 31 && dt.time_zone = DateTime.Tz.Etc_UTC then
        Ok ()
      else Error "Parsed datetime with space separator doesn't match expected values"
...
let test_parse_basic_format = fun _ctx ->
  ...
  | Ok dt ->
      if dt.year = 2_025 && dt.month = 8 && dt.day = 27 && dt.hour = 14 && dt.minute = 7 && dt.second = 31 && dt.time_zone = DateTime.Tz.Etc_UTC then
        Ok ()
      else Error "Parsed basic format datetime doesn't match expected values"
...
let test_parse_basic_format_with_offset = fun _ctx ->
  ...
  | Ok dt ->
      if dt.year = 2_025 && dt.month = 8 && dt.day = 27 && dt.hour = 14 && dt.minute = 7 && dt.second = 31 && dt.time_zone = DateTime.Tz.Local && dt.utc_offset = 19_800 then
        Ok ()
      else Error "Parsed basic format with offset doesn't match expected values"
...
let test_parse_with_comma_decimal = fun _ctx ->
  ...
  | Ok dt ->
      ...
      if dt.year = 2_025 && dt.month = 8 && dt.day = 27 && dt.hour = 14 && dt.minute = 7 && dt.second = 31 && microseconds = 426_822 && precision = 6 && dt.time_zone = DateTime.Tz.Etc_UTC then
        Ok ()
      else Error "Parsed datetime with comma decimal doesn't match expected values"
...
let test_parse_negative_year = fun _ctx ->
  ...
  | Ok dt ->
      if dt.year = (-2_015) && dt.month = 8 && dt.day = 27 && dt.hour = 14 && dt.minute = 7 && dt.second = 31 && dt.time_zone = DateTime.Tz.Etc_UTC then
        Ok ()
      else Error "Parsed negative year datetime doesn't match expected values"
...
let test_parse_positive_year_sign = fun _ctx ->
  ...
  | Ok dt ->
      if dt.year = 2_025 && dt.month = 8 && dt.day = 27 && dt.hour = 14 && dt.minute = 7 && dt.second = 31 && dt.time_zone = DateTime.Tz.Etc_UTC then
        Ok ()
      else Error "Parsed positive year datetime doesn't match expected values"
...
let test_parse_basic_with_microseconds = fun _ctx ->
  ...
  | Ok dt ->
      let (microseconds, precision) = dt.microseconds in
      if dt.year = 2_025 && dt.month = 8 && dt.day = 27 && dt.hour = 14 && dt.minute = 7 && dt.second = 31 && microseconds = 123_000 && precision = 6 then
        Ok ()
      else Error "Parsed basic format with microseconds doesn't match expected values"
...
let test_parse_space_and_comma = fun _ctx ->
  ...
  | Ok dt ->
      ...
      if dt.year = 2_025 && dt.month = 8 && dt.day = 27 && dt.hour = 14 && dt.minute = 7 && dt.second = 31 && microseconds = 123_000 && dt.utc_offset = 9_000 then
        Ok ()
      else Error "Parsed datetime with space and comma doesn't match expected values"
...
let test_parse_negative_year_leap = fun _ctx ->
  ...
  | Ok dt ->
      if dt.year = (-2_020) && dt.month = 2 && dt.day = 29 then
        Ok ()
      else Error "Failed to parse negative leap year date"
