...
let test_parse_utc_basic () =
  match DateTime.parse "2025-08-27T14:07:31Z" with
  | Ok dt ->
      if dt.year = 2025 && dt.month = 8 && dt.day = 27 &&
         dt.hour = 14 && dt.minute = 7 && dt.second = 31 &&
         dt.time_zone = DateTime.Tz.Etc_UTC && dt.utc_offset = 0 then
        Ok ()
      else
        Error "Parsed datetime doesn't match expected values"
  | Error _ -> Error "Failed to parse valid UTC datetime"

let test_parse_with_microseconds () =
  match DateTime.parse "2025-08-27T14:07:31.426822Z" with
  | Ok dt ->
      if dt.year = 2025 && dt.month = 8 && dt.day = 27 &&
         dt.hour = 14 && dt.minute = 7 && dt.second = 31 &&
         fst dt.microseconds = 426822 && snd dt.microseconds = 6 &&
         dt.time_zone = DateTime.Tz.Etc_UTC then
        Ok ()
      else
...
let test_parse_with_timezone_offset () =
  match DateTime.parse "2025-08-27T14:07:31+05:30" with
  | Ok dt ->
      if dt.year = 2025 && dt.month = 8 && dt.day = 27 &&
         dt.hour = 14 && dt.minute = 7 && dt.second = 31 &&
         dt.time_zone = DateTime.Tz.Local && dt.utc_offset = 19800 then
        Ok ()
      else
...
let test_parse_negative_timezone_offset () =
  match DateTime.parse "2025-08-27T14:07:31-07:00" with
  | Ok dt ->
      if dt.year = 2025 && dt.month = 8 && dt.day = 27 &&
         dt.hour = 14 && dt.minute = 7 && dt.second = 31 &&
         dt.time_zone = DateTime.Tz.Local && dt.utc_offset = -25200 then
        Ok ()
      else
...
let test_parse_invalid_timezone () =
...
let test_roundtrip () =
  ...
  | Ok parsed ->
      if parsed.year = original.year &&
         ...
         parsed.second = original.second then
        Ok ()
      else
...
let test_parse_with_space_separator () =
...
let test_parse_basic_format_with_offset () =
...
let test_parse_with_comma_decimal () =
  match DateTime.parse "2025-08-27T14:07:31,426822Z" with
  | Ok dt ->
      if dt.year = 2025 && dt.month = 8 && dt.day = 27 &&
         dt.hour = 14 && dt.minute = 7 && dt.second = 31 &&
...
let test_parse_negative_year () =
  match DateTime.parse "-2015-08-27T14:07:31Z" with
  | Ok dt ->
      if dt.year = -2015 && dt.month = 8 && dt.day = 27 &&
         dt.hour = 14 && dt.minute = 7 && dt.second = 31 &&
         dt.time_zone = DateTime.Tz.Etc_UTC then
        Ok ()
      else
...
let test_parse_positive_year_sign () =
  match DateTime.parse "+2025-08-27T14:07:31Z" with
  | Ok dt ->
      if dt.year = 2025 && dt.month = 8 && dt.day = 27 &&
         dt.hour = 14 && dt.minute = 7 && dt.second = 31 &&
...
let test_parse_basic_with_microseconds () =
  match DateTime.parse "20250827T140731.123Z" with
  | Ok dt ->
      if dt.year = 2025 && dt.month = 8 && dt.day = 27 &&
         dt.hour = 14 && dt.minute = 7 && dt.second = 31 &&
         fst dt.microseconds = 123000 && snd dt.microseconds = 6 then
        Ok ()
      else
...
let test_parse_space_and_comma () =
  match DateTime.parse "2025-08-27 14:07:31,123+02:30" with
  | Ok dt ->
      if dt.year = 2025 && dt.month = 8 && dt.day = 27 &&
         dt.hour = 14 && dt.minute = 7 && dt.second = 31 &&
         fst dt.microseconds = 123000 && dt.utc_offset = 9000 then
        Ok ()
      else
...
let test_parse_negative_year_leap () =
  match DateTime.parse "-2020-02-29T12:00:00Z" with
  | Ok dt ->
      if dt.year = -2020 && dt.month = 2 && dt.day = 29 then
        Ok ()
      else
...
let tests =
  Test.
    [
      ...
      (* New Elixir compatibility tests *)
...
let () =
  Miniriot.run
    ~main:(fun ~args -> Test.Cli.main ~name:"datetime" ~tests ~args)
    ~args:Env.args ()