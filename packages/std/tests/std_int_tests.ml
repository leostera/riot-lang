...
let test_add_sub_mul_div_rem = fun _ctx ->
  if Int.equal (Int.add 2 3) 5 && Int.equal (Int.sub 7 2) 5 && Int.equal (Int.mul 3 4) 12 && Int.equal (Int.div 12 3) 4 && Int.equal (Int.rem 13 5) 3 then
    Ok ()
  else Error "expected Int arithmetic wrappers to match runtime arithmetic"
...
let test_abs = fun _ctx ->
  if Int.equal (Int.abs (-7)) 7 then
    Ok ()
  else Error "expected Int.abs -7 = 7"
...
let test_from_float = fun _ctx ->
  if Int.equal (Int.from_float 4.8) 4 then
    Ok ()
  else Error "expected Int.from_float 4.8 = 4"

let test_parse_and_of_string = fun _ctx ->
  if Int.equal (Int.of_string "42") 42 && Int.parse "42" = Some 42 && Int.of_string_opt "42" = Some 42 then
    Ok ()
  else Error "expected Int string parsers to parse decimal strings"
...
let test_to_string = fun _ctx ->
  if String.equal (Int.to_string 12) "12" then
    Ok ()
  else Error "expected Int.to_string 12 = 12"
