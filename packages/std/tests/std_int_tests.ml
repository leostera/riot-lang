...
let test_zero_and_one = fun _ctx ->
  if Int.equal Int.zero 0 && Int.equal Int.one 1 then Ok ()
  else Error "expected Int.zero = 0 and Int.one = 1"
...
let test_equal_and_compare = fun _ctx ->
  if Int.equal 7 7 && Int.compare 3 5 < 0 && Int.compare 5 3 > 0 then Ok ()
  else Error "expected Int.equal and Int.compare to behave consistently"

let test_abs = fun _ctx ->
  if Int.equal (Int.abs (-7)) 7 then Ok () else Error "expected Int.abs -7 = 7"

let test_min_and_max = fun _ctx ->
  if Int.equal (Int.min 4 9) 4 && Int.equal (Int.max 4 9) 9 then Ok ()
  else Error "expected Int.min/max to choose smaller/larger values"

let test_succ_and_pred = fun _ctx ->
  if Int.equal (Int.succ 4) 5 && Int.equal (Int.pred 4) 3 then Ok ()
  else Error "expected Int.succ/pred to adjust values by one"

let test_from_float = fun _ctx ->
  if Int.equal (Int.from_float 4.8) 4 then Ok () else Error "expected Int.from_float 4.8 = 4"
...
let test_parse_invalid = fun _ctx ->
  if Int.parse "abc" = None && Int.of_string_opt "abc" = None then Ok ()
  else Error "expected invalid int strings to return None"

let test_hash = fun _ctx ->
  if Int.equal (Int.hash 12) (Int.hash 12) then Ok ()
  else Error "expected Int.hash to be stable across repeated calls"

let test_to_string = fun _ctx ->
  if String.equal (Int.to_string 12) "12" then Ok () else Error "expected Int.to_string 12 = 12"
