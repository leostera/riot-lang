...
let test_map_transforms_each_item = fun _ctx ->
  if
    Iterator.to_list (Iterator.map (int_iter [ 1; 2; 3 ]) ~fn:(fun value -> value * 2))
    = [ 2; 4; 6 ]
  then
    Ok ()
  else
...
let test_filter_keeps_only_matching_items = fun _ctx ->
  if Iterator.to_list (Iterator.filter (int_iter [ 1; 2; 3; 4; ]) ~fn:is_even) = [ 2; 4 ] then
    Ok ()
  else
    Error "Iterator.filter should keep only matching items"
...
let test_fold_and_reduce_accumulate_items = fun _ctx ->
  ...
  if Int.equal folded 6 && reduced = Some 6 then
    ...
    Error "Iterator.fold and Iterator.reduce should accumulate items"

let test_count_returns_number_of_items = fun _ctx ->
  if Int.equal (Iterator.count (int_iter [ 1; 2; 3; 4; ])) 4 then
    Ok ()
  else
    Error "Iterator.count should return the number of items"
...
let test_find_any_and_all_reflect_predicates = fun _ctx ->
  ...
  if found = Some 4 && any_even && all_even then
    ...
    Error "Iterator.find/any/all should reflect predicate matches"

let test_take_and_drop_trim_the_sequence = fun _ctx ->
  let taken =
    Iterator.take (int_iter [ 1; 2; 3; 4; ]) 2
    |> Iterator.to_list
  in
  let dropped =
    Iterator.drop (int_iter [ 1; 2; 3; 4; ]) 2
    |> Iterator.to_list
  in
  if taken = [ 1; 2 ] && dropped = [ 3; 4 ] then
    Ok ()
  else
    Error "Iterator.take and Iterator.drop should trim the sequence"

let test_enumerate_pairs_indices_with_values = fun _ctx ->
  let actual =
    Iterator.enumerate (int_iter [ 10; 20 ])
    |> Iterator.to_list
  in
  if actual = [ (0, 10); (1, 20); ] then
    Ok ()
  else
    Error "Iterator.enumerate should pair indices with values"

let test_zip_stops_at_the_shorter_input = fun _ctx ->
  let actual =
    Iterator.zip (int_iter [ 1; 2; 3 ]) (int_iter [ 4; 5 ])
    |> Iterator.to_list
  in
  if actual = [ (1, 4); (2, 5); ] then
    Ok ()
  else
    Error "Iterator.zip should stop when either input is exhausted"

let test_chain_appends_the_second_iterator = fun _ctx ->
  let actual =
    Iterator.chain (int_iter [ 1; 2 ]) (int_iter [ 3; 4 ])
    |> Iterator.to_list
  in
  if actual = [ 1; 2; 3; 4; ] then
    Ok ()
  else
    Error "Iterator.chain should append the second iterator after the first"
