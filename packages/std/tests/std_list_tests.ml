...
let test_length = fun _ctx ->
  if Int.equal (List.length []) 0 then
    Ok ()
  else Error "expected List.length [] = 0"
...
let test_fold_left = fun _ctx ->
  if Int.equal (List.fold_left [ 1; 2; 3 ] ~init:0 ~fn:(
...
let test_filter_map = fun _ctx ->
  if List.filter_map
    ...
    Ok ()
  else Error "expected List.filter_map to drop Nones and unwrap Somes"
