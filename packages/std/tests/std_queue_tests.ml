...
let wait_for_producers = fun ~producer_count ->
  let rec loop remaining =
    if remaining = 0 then
      ...
      match await ~what:"queue producer completion"
        (
          function
...
let collect_consumer_values = fun ~consumer_count ->
  let rec loop remaining acc =
    if remaining = 0 then
      ...
      match await ~what:"queue consumer completion"
        (
          function
...
let has_expected_set = fun ~expected ~actual ->
  ...
  let actual_set = HashSet.from_list actual in Int.equal (List.length actual) (List.length expected) && Int.equal (HashSet.length actual_set) (HashSet.length expected_set) && sort_ints actual = sort_ints expected
...
let test_create = fun _ctx ->
  ...
  if Queue.is_empty queue then
    Ok ()
  else Error "expected Queue.create to start empty"

let test_with_capacity = fun _ctx ->
  let queue = Queue.with_capacity ~size:4 in
  if Queue.is_empty queue then
    Ok ()
  else Error "expected Queue.with_capacity to start empty"
...
let test_push_then_front = fun _ctx ->
  ...
  if Queue.front queue = Some 1 then
    Ok ()
  else Error "expected front to return earliest pushed value"

let test_pop_empty = fun _ctx ->
  if Queue.pop (Queue.create ()) = None then
    Ok ()
  else Error "expected Queue.pop empty = None"
...
let test_length_after_push_pop = fun _ctx ->
  ...
  if Int.equal (Queue.length queue) 1 then
    Ok ()
  else Error "expected Queue.length to track live items"
...
let test_is_empty_after_removing_all = fun _ctx ->
  ...
  if Queue.is_empty queue then
    Ok ()
  else Error "expected queue to be empty after removing all items"
...
let test_clear = fun _ctx ->
  ...
  if Queue.is_empty queue then
    Ok ()
  else Error "expected Queue.clear to empty queue"

let test_for_each = fun _ctx ->
  let queue = Queue.from_list [ 1; 2; 3 ] in
  let seen = box [] in
  Queue.for_each queue ~fn:(
    fun value -> seen.value <- value :: seen.value
  );
...
let test_fold_left = fun _ctx ->
  let queue = Queue.from_list [ 1; 2; 3 ] in
  if String.equal (Queue.fold_left queue ~init:"" ~fn:(
    fun acc value -> acc ^ Int.to_string value
  )) "123" then
    Ok ()
...
let test_append = fun _ctx ->
  ...
  if Queue.to_list left = [
    1;
    2;
    3;
    4;
  ] && Queue.is_empty right then
    Ok ()
  ...
  let rec loop step model remaining =
    ...
    | [] ->
        if not (Queue.to_list queue = model) then
          Error ("final state: expected queue " ^ render_int_list model ^ ", got " ^ render_int_list (Queue.to_list queue))
        else
          if not (Int.equal (Queue.length queue) (List.length model)) then
            Error ("final state: expected length " ^ Int.to_string (List.length model) ^ ", got " ^ Int.to_string (Queue.length queue))
          else
            if not (Queue.is_empty queue = List.is_empty model) then
              Error ("final state: expected is_empty=" ^ Bool.to_string (List.is_empty model) ^ ", got " ^ Bool.to_string (Queue.is_empty queue))
            else Ok ()
    | (Push value) :: rest ->
        Queue.push queue ~value;
        let next_model = model @ [ value ] in
        if not (Queue.to_list queue = next_model) then
          Error ("step " ^ Int.to_string step ^ " " ^ render_operation (Push value) ^ ": expected queue " ^ render_int_list next_model ^ ", got " ^ render_int_list (Queue.to_list queue))
        else
          if not (Int.equal (Queue.length queue) (List.length next_model)) then
            Error ("step " ^ Int.to_string step ^ " " ^ render_operation (Push value) ^ ": expected length " ^ Int.to_string (List.length next_model) ^ ", got " ^ Int.to_string (Queue.length queue))
    ...
    | Pop :: rest ->
        ...
        if not (expected_pop = actual_pop) then
          Error ("step " ^ Int.to_string step ^ " Pop: expected " ^ render_int_option expected_pop ^ ", got " ^ render_int_option actual_pop)
        else
          if not (Queue.to_list queue = next_model) then
            Error ("step " ^ Int.to_string step ^ " Pop: expected queue " ^ render_int_list next_model ^ ", got " ^ render_int_list (Queue.to_list queue))
          else
            if not (Int.equal (Queue.length queue) (List.length next_model)) then
              Error ("step " ^ Int.to_string step ^ " Pop: expected length " ^ Int.to_string (List.length next_model) ^ ", got " ^ Int.to_string (Queue.length queue))
            else loop (step + 1) next_model rest
    | Clear :: rest ->
        Queue.clear queue;
        if not (Queue.is_empty queue) then
          Error ("step " ^ Int.to_string step ^ " Clear: expected queue to be empty, got " ^ render_int_list (Queue.to_list queue))
        else
          if not (Int.equal (Queue.length queue) 0) then
            Error ("step " ^ Int.to_string step ^ " Clear: expected length 0, got " ^ Int.to_string (Queue.length queue))
...
let test_multiple_producers_preserve_every_value = fun _ctx ->
  ...
  List.for_each producer_pids ~fn:(
    fun pid -> send pid Queue_test_go
  );
...
let tests = Test.[
  ...
  case ~size:Large "Queue preserves every value from concurrent producers" test_multiple_producers_preserve_every_value;
  case ~size:Large "Queue lets multiple consumers drain each prefetched value exactly once" test_multiple_consumers_drain_every_value_once;
