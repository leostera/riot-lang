...
let test_zero_min_and_max_constants = fun _ctx ->
  ...
  else if not (Int64.equal Int64.min_int (-9223372036854775808L)) then
    Error "Int64.min_int should match the runtime value"
  else if not (Int64.equal Int64.max_int 9223372036854775807L) then
...
let test_shift_operations = fun _ctx ->
  if not (Int64.equal (Int64.shift_left 1L 40) 1099511627776L) then
    Error "Int64.shift_left returned the wrong shifted value"
  else if not (Int64.equal (Int64.shift_right_logical (-1L) 1) 9223372036854775807L) then
