...
let tests = [
  Test.case
    "kernel regex compiles and matches"
    (fun _ctx ->
      let regex =
        Kernel.Regex.compile "foo[0-9]+"
        |> Result.expect ~msg:"compile regex"
      in
      ...
      Ok ());
  Test.case
    "kernel regex exposes first match spans"
    (fun _ctx ->
      let regex =
        Kernel.Regex.compile "foo[0-9]+"
        |> Result.expect ~msg:"compile regex"
      ...
      | Some { start; stop } ->
          ...
          Ok ());
  Test.case
    "kernel regex reports compile errors with offsets"
    (fun _ctx ->
