...
let test_map_err = fun _ctx ->
  match Result.map_err (Error 5) ~fn:(fun value -> value + 1) with
  | Error 6 -> Ok ()
...
let test_map_or_else = fun _ctx ->
  if
    Int.equal
      (Result.map_or_else (Error "boom") ~default:String.length ~fn:(fun value -> value * 2))
      4
  then
    Ok ()
  else
    Error "expected Result.map_or_else Error to use default"

let test_and_then = fun _ctx ->
  match Result.and_then (Ok 5) ~fn:(fun value -> Ok (value + 3)) with
  | Ok 8 -> Ok ()
  | _ -> Error "expected Result.and_then to chain Ok values"

let test_or_else = fun _ctx ->
  match Result.or_else (Error "boom") ~fn:(fun err -> Ok (String.length err)) with
  | Ok 4 -> Ok ()
