...
let cast_worker: type task other.
  (task, other) Type.eq ->
  other WorkerPool.DynamicWorkerPool.worker ->
  task WorkerPool.DynamicWorkerPool.worker = fun Type.Equal worker ->
  worker

let await_ready_worker: type task.
  task WorkerPool.DynamicWorkerPool.t ->
  ...
  await
    ...
    | WorkerPool.DynamicWorkerPool.WorkerReady worker -> (
        match Ref.type_equal pool.WorkerPool.DynamicWorkerPool.task_ref
...
let test_simple_worker_pool_empty_returns_immediately =
  Test.case "simple worker pool run on an empty task list returns []" (fun _ctx ->
...
let test_simple_worker_pool_propagates_worker_exceptions =
  Test.case "simple worker pool re-raises worker exceptions" (fun _ctx ->
    try
      let _ =
        WorkerPool.SimpleWorkerPool.run
          ~concurrency:2
...
let test_dynamic_worker_pool_sends_worker_ready_messages =
  Test.case "dynamic worker pool sends WorkerReady messages to the owner" (fun _ctx ->
    let pool =
