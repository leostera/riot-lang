...
type Message.t +=
  | Worker_pool_task_received of { payload: string; run_ref: string Ref.t }
...
let cast_worker: type task other. (task, other) Type.eq -> other WorkerPool.DynamicWorkerPool.worker -> task WorkerPool.DynamicWorkerPool.worker = fun Type.Equal worker -> worker

let await_ready_worker: type task. task WorkerPool.DynamicWorkerPool.t -> (task WorkerPool.DynamicWorkerPool.worker, string) result = fun pool ->
  await ~what:"worker readiness"
    (
      function
      | WorkerPool.DynamicWorkerPool.WorkerReady worker -> (
        match Ref.type_equal pool.WorkerPool.DynamicWorkerPool.task_ref (WorkerPool.DynamicWorkerPool.get_worker_task_ref worker) with
...
let test_simple_worker_pool_empty_returns_immediately =
  Test.case "simple worker pool run on an empty task list returns []"
    (
      fun _ctx ->
        match WorkerPool.SimpleWorkerPool.run ~tasks:[] ~fn:(
          fun x -> x
        ) () with
        | [] -> Ok ()
...
let test_simple_worker_pool_singleton_returns_indexed_result =
  Test.case "simple worker pool run on a singleton task returns one indexed result"
    (
      fun _ctx ->
...
let test_simple_worker_pool_preserves_input_order =
  Test.case "simple worker pool preserves input order instead of completion order"
    (
      fun _ctx ->
        let results = WorkerPool.SimpleWorkerPool.run ~concurrency:2 ~tasks:[ "slow"; "fast"; "medium" ] ~fn:(
          fun task ->
            if String.equal task "slow" then
              sleep (Time.Duration.from_millis 25)
            else
              if String.equal task "medium" then
...
let test_simple_worker_pool_concurrency_one_is_sequential =
  Test.case "simple worker pool concurrency 1 behaves like a sequential indexed map"
    (
      fun _ctx ->
        match WorkerPool.SimpleWorkerPool.run ~concurrency:1 ~tasks:[ 1; 2; 3 ] ~fn:(
          fun value -> value * 10
        ) () with
        | [ (0, 10); (1, 20); (2, 30) ] -> Ok ()
...
let test_simple_worker_pool_handles_concurrency_greater_than_task_count =
  Test.case "simple worker pool with excess concurrency returns every result once"
    (
      fun _ctx ->
        let results = WorkerPool.SimpleWorkerPool.run ~concurrency:8 ~tasks:[ 3; 1 ] ~fn:(
          fun value -> value + 1
        ) () in
        match results with
        | [ (0, 4); (1, 2) ] -> Ok ()
...
let test_simple_worker_pool_propagates_worker_exceptions =
  Test.case "simple worker pool re-raises worker exceptions"
    (
      fun _ctx ->
        try
          let _ = WorkerPool.SimpleWorkerPool.run ~concurrency:2 ~tasks:[ 1; 2; 3 ] ~fn:(
            fun value ->
              if value = 2 then
                raise (Failure "boom");
              value
          ) () in Error "expected SimpleWorkerPool.run to re-raise worker failures"
        with
...
let test_dynamic_worker_pool_sends_worker_ready_messages =
  Test.case "dynamic worker pool sends WorkerReady messages to the owner"
    (
      fun _ctx ->
        let pool = WorkerPool.DynamicWorkerPool.start ~concurrency:2 ~owner:(self ()) ~worker_fn:(
          fun ~owner:_ ~task:_ -> ()
        ) () in
...
let test_dynamic_worker_pool_send_task_executes_payload =
  Test.case "dynamic worker pool send_task delivers the supplied payload to the worker"
    (
      fun _ctx ->
        let run_ref = Ref.make () in
        let parent = self () in
        let pool = WorkerPool.DynamicWorkerPool.start ~concurrency:1 ~owner:parent ~worker_fn:(
          fun ~owner ~task -> send owner (Worker_pool_task_received { payload = task; run_ref })
        ) () in
        match await_ready_worker pool with
        | Error _ as err -> err
        | Ok worker ->
            ...
            let received =
              await ~what:"dynamic worker payload"
                (
                  function
