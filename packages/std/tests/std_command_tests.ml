...
let run_capture = fun () ->
  let cmd = Command.make (self_executable ()) ~args:[ "capture-both-streams" ] in Command.output cmd |> Result.expect ~msg:"failed to run capture helper"
...
let test_command_output_drains_stdout_and_stderr = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    if not (String.equal output.stdout stdout_payload) then
      Error ("unexpected stdout payload: " ^ output.stdout)
    else
      if not (Int.equal (String.length output.stderr) stderr_payload_size) then
...
let test_command_output_handles_delayed_shell_stdout = fun _ctx ->
  ...
  | Error (Command.SystemError message) -> Error ("expected delayed shell stdout to succeed, got: " ^ message)
  | Ok output ->
      if not (Int.equal output.status 0) then
        Error ("expected shell command to exit 0, got " ^ Int.to_string output.status)
      else
        if not (String.equal output.stdout "delayed-output") then
...
let test_command_output_streams_stdout_lines = fun _ctx ->
  ...
  match Command.output ~on_stdout_line:(
    fun line -> seen_lines := line :: !seen_lines
  ) cmd with
  | Error (Command.SystemError message) -> Error ("expected streamed stdout helper to succeed, got: " ^ message)
  | Ok output ->
      ...
      let same_lines =
        if not (Int.equal (List.length actual_lines) (List.length expected_lines)) then
          ...
          List.zip actual_lines expected_lines |> List.for_all
            (
              fun (actual, expected) -> String.equal actual expected
            )
      in
      if not (Int.equal output.status 0) then
        Error ("expected streamed helper to exit 0, got " ^ Int.to_string output.status)
      else
        if not same_lines then
          Error ("unexpected streamed lines: " ^ String.concat "" actual_lines)
        else
          if not (String.equal output.stdout streamed_stdout_payload) then
...
let test_command_output_emits_idle_callbacks = fun _ctx ->
  ...
  match Command.output ~on_idle:(
    fun elapsed -> seen := Time.Duration.to_micros elapsed :: !seen
  ) ~idle_interval:(Time.Duration.from_millis 10) cmd with
  | Error (Command.SystemError message) -> Error ("expected idle callback command to succeed, got: " ^ message)
  | Ok output ->
      if not (Int.equal output.status 0) then
        Error ("expected idle callback command to exit 0, got " ^ Int.to_string output.status)
      else
        if List.is_empty !seen then
          Error "expected at least one idle callback"
        else
          if not (String.equal output.stdout "idle-done") then
...
let test_command_output_returns_when_child_exits_with_inherited_writer = fun _ctx ->
  ...
  | Error (Command.SystemError message) -> Error ("expected inherited writer command to succeed, got: " ^ message)
  | Ok output ->
      ...
      if not (Int.equal output.status 0) then
        ...
        if not (String.equal output.stdout "parent-done") then
          Error ("unexpected inherited writer stdout payload: " ^ output.stdout)
        else
          if elapsed_us > 1_000_000 then
            Error ("expected command output to return before inherited writer closed, elapsed us: " ^ Int.to_string elapsed_us)
          else Ok ()
...
let test_command_output_handles_parallel_shell_commands = fun _ctx ->
  ...
  let _workers = List.init ~count ~fn:(
    fun index ->
      Runtime.spawn
        (
          fun () ->
            let result =
              match Command.output (Command.make "sh" ~args:[ "-c"; "sleep 0.05; printf parallel-output" ]) with
              | Error (Command.SystemError message) -> Error message
              | Ok output when not (Int.equal output.status 0) -> Error ("expected exit 0, got " ^ Int.to_string output.status)
              | Ok output when not (String.equal output.stdout expected_stdout) -> Error ("unexpected stdout: " ^ output.stdout)
              | Ok output when not (String.equal output.stderr "") -> Error ("unexpected stderr: " ^ output.stderr)
              | Ok _ -> Ok ()
            in
  ...
  let rec collect pending failures =
    if pending = 0 then
      if failures = [] then
        Ok ()
      else Error (String.concat "; " (List.reverse failures))
    else
      match Runtime.receive ~selector:(
        function
...
let test_command_output_handles_parallel_fast_exit_commands = fun _ctx ->
  ...
  let run_fast_exit () =
    ...
    | Error (Command.SystemError message) -> Error message
    | Ok output when not (Int.equal output.status 0) -> Error ("expected exit 0, got " ^ Int.to_string output.status)
    | Ok output when not (String.equal output.stdout "") -> Error ("expected empty stdout, got: " ^ output.stdout)
    | Ok output when not (String.equal output.stderr "") -> Error ("expected empty stderr, got: " ^ output.stderr)
    | Ok _ -> Ok ()
  in
  ...
  in
  let _workers = List.init ~count:worker_count ~fn:(
    fun index ->
      Runtime.spawn
        (
          fun () ->
