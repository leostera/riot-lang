...
let test_is_leap_year_2000 = fun _ctx ->
  if Calendar.is_leap_year ~year:2000 then Ok () else Error "expected 2000 to be a leap year"

let test_is_leap_year_1900 = fun _ctx ->
  if not (Calendar.is_leap_year ~year:1900) then Ok () else Error "expected 1900 not to be a leap year"

let test_is_leap_year_2024 = fun _ctx ->
  if Calendar.is_leap_year ~year:2024 then Ok () else Error "expected 2024 to be a leap year"

let test_is_leap_year_2023 = fun _ctx ->
  if not (Calendar.is_leap_year ~year:2023) then Ok () else Error "expected 2023 not to be a leap year"

let test_last_day_of_month_feb_leap = fun _ctx ->
  if Int.equal (Calendar.last_day_of_month ~year:2024 ~month:2) 29 then Ok ()
  else Error "expected February 2024 to have 29 days"

let test_last_day_of_month_feb_non_leap = fun _ctx ->
  if Int.equal (Calendar.last_day_of_month ~year:2023 ~month:2) 28 then Ok ()
  else Error "expected February 2023 to have 28 days"

let test_last_day_of_month_30_day = fun _ctx ->
  if Int.equal (Calendar.last_day_of_month ~year:2024 ~month:4) 30 then Ok ()
  else Error "expected April to have 30 days"

let test_last_day_of_month_invalid_0 = fun _ctx ->
  try
    let _ = Calendar.last_day_of_month ~year:2024 ~month:0 in
...
let test_last_day_of_month_invalid_13 = fun _ctx ->
  try
    let _ = Calendar.last_day_of_month ~year:2024 ~month:13 in
...
let test_is_valid_date_leap = fun _ctx ->
  if Calendar.is_valid_date (make_date ~year:2024 ~month:2 ~day:29) then Ok ()
  else Error "expected 2024-02-29 to be valid"

let test_is_valid_date_non_leap = fun _ctx ->
  if not (Calendar.is_valid_date (make_date ~year:2023 ~month:2 ~day:29)) then Ok ()
  else Error "expected 2023-02-29 to be invalid"

let test_is_valid_date_april_31 = fun _ctx ->
  if not (Calendar.is_valid_date (make_date ~year:2024 ~month:4 ~day:31)) then Ok ()
  else Error "expected 2024-04-31 to be invalid"

let test_date_to_gregorian_days_origin = fun _ctx ->
  if Int.equal (Calendar.date_to_gregorian_days (make_date ~year:0 ~month:1 ~day:1)) 0 then Ok ()
  else Error "expected origin date to map to zero gregorian days"

let test_date_to_gregorian_days_epoch = fun _ctx ->
  if
    Int.equal
      (Calendar.date_to_gregorian_days (make_date ~year:1970 ~month:1 ~day:1))
      Calendar.days_from_0_to_1970
  then
    Ok ()
  else
...
let test_gregorian_days_to_date_origin = fun _ctx ->
  ...
  if equal_date actual expected then Ok () else Error "expected gregorian day 0 to roundtrip to origin"

let test_gregorian_days_to_date_epoch = fun _ctx ->
  let actual = Calendar.gregorian_days_to_date Calendar.days_from_0_to_1970 in
  let expected = make_date ~year:1970 ~month:1 ~day:1 in
  if equal_date actual expected then Ok () else Error "expected unix epoch gregorian day to roundtrip"

let test_gregorian_days_roundtrip_leap = fun _ctx ->
  let date = make_date ~year:2024 ~month:2 ~day:29 in
  if equal_date (Calendar.gregorian_days_to_date (Calendar.date_to_gregorian_days date)) date then Ok ()
  else Error "expected leap date gregorian-day roundtrip"

let test_gregorian_days_roundtrip_non_leap = fun _ctx ->
  let date = make_date ~year:2023 ~month:11 ~day:21 in
  if equal_date (Calendar.gregorian_days_to_date (Calendar.date_to_gregorian_days date)) date then Ok ()
  else Error "expected non-leap date gregorian-day roundtrip"
...
let test_naive_to_gregorian_seconds_origin = fun _ctx ->
  ...
  if Int.equal (Calendar.naive_to_gregorian_seconds date time) 0 then Ok ()
  else Error "expected year 0 midnight to map to zero gregorian seconds"
...
let test_gregorian_seconds_roundtrip_midnight = fun _ctx ->
  let date = make_date ~year:2024 ~month:11 ~day:21 in
  let time = make_time ~hour:0 ~minute:0 ~second:0 in
  let (actual_date, actual_time) =
    Calendar.gregorian_seconds_to_naive (Calendar.naive_to_gregorian_seconds date time)
  in
  if equal_date actual_date date && equal_time actual_time time then Ok ()
  else Error "expected midnight gregorian-seconds roundtrip"
...
let test_gregorian_seconds_roundtrip_non_midnight = fun _ctx ->
  ...
  in
  if equal_date actual_date date && equal_time actual_time time then Ok ()
  else Error "expected non-midnight gregorian-seconds roundtrip"

let test_day_of_week_monday = fun _ctx ->
  if Calendar.day_of_week (make_date ~year:2024 ~month:11 ~day:25) = Calendar.Monday then Ok ()
  else Error "expected 2024-11-25 to be Monday"

let test_day_of_week_sunday = fun _ctx ->
  if Calendar.day_of_week (make_date ~year:2024 ~month:11 ~day:24) = Calendar.Sunday then Ok ()
  else Error "expected 2024-11-24 to be Sunday"

let test_iso_week_number_mid_year = fun _ctx ->
  let actual = Calendar.iso_week_number (make_date ~year:2024 ~month:11 ~day:25) in
  if Int.equal actual.year 2024 && Int.equal actual.week 48 then Ok ()
  else Error "expected 2024-11-25 to be ISO week 48 of 2024"

let test_iso_week_number_year_boundary = fun _ctx ->
  let actual = Calendar.iso_week_number (make_date ~year:2021 ~month:1 ~day:1) in
  if Int.equal actual.year 2020 && Int.equal actual.week 53 then Ok ()
  else Error "expected 2021-01-01 to belong to ISO week 53 of 2020"

let test_time_to_seconds = fun _ctx ->
  if Int.equal (Calendar.time_to_seconds (make_time ~hour:23 ~minute:59 ~second:59)) 86_399 then Ok ()
  else Error "expected 23:59:59 to equal 86399 seconds"

let test_seconds_to_time = fun _ctx ->
  let actual = Calendar.seconds_to_time 86_399 in
  if equal_time actual (make_time ~hour:23 ~minute:59 ~second:59) then Ok ()
  else Error "expected 86399 seconds to map to 23:59:59"
