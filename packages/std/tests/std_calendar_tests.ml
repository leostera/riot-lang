...
let test_date_to_gregorian_days_epoch = fun _ctx ->
  if
    Int.equal
      (Calendar.date_to_gregorian_days (make_date ~year:1_970 ~month:1 ~day:1))
...
let tests =
  Test.[
    ...
    case
      "Calendar gregorian-seconds roundtrip non-midnight"
      test_gregorian_seconds_roundtrip_non_midnight;
