...
let equal_date = fun (left: Calendar.date) (right: Calendar.date) -> Int.equal left.year right.year && Int.equal left.month right.month && Int.equal left.day right.day

let equal_time = fun (left: Calendar.time) (right: Calendar.time) -> Int.equal left.hour right.hour && Int.equal left.minute right.minute && Int.equal left.second right.second
...
let test_last_day_of_month_invalid_0 = fun _ctx ->
  try
    let _ = Calendar.last_day_of_month ~year:2_024 ~month:0 in Error "expected invalid month 0 to raise"
  with
  | Failure _ -> Ok ()
...
let test_last_day_of_month_invalid_13 = fun _ctx ->
  try
    let _ = Calendar.last_day_of_month ~year:2_024 ~month:13 in Error "expected invalid month 13 to raise"
  with
  | Failure _ -> Ok ()
  | Invalid_argument _ -> Ok ()
...
let test_date_to_gregorian_days_epoch = fun _ctx ->
  if Int.equal (Calendar.date_to_gregorian_days (make_date ~year:1_970 ~month:1 ~day:1)) Calendar.days_from_0_to_1970 then
    Ok ()
  else Error "expected 1970-01-01 gregorian days to match days_from_0_to_1970"
...
let test_gregorian_days_to_date_origin = fun _ctx ->
  ...
  if equal_date actual expected then
    Ok ()
  else Error "expected gregorian day 0 to roundtrip to origin"
...
let test_gregorian_days_to_date_epoch = fun _ctx ->
  ...
  if equal_date actual expected then
    Ok ()
  else Error "expected unix epoch gregorian day to roundtrip"
...
let test_gregorian_seconds_to_naive_origin = fun _ctx ->
  ...
  if equal_date date (make_date ~year:0 ~month:1 ~day:1) && equal_time time (make_time ~hour:0 ~minute:0 ~second:0) then
    Ok ()
  else Error "expected gregorian seconds origin to roundtrip to year 0 midnight"

let test_gregorian_seconds_roundtrip_midnight = fun _ctx ->
  let date = make_date ~year:2_024 ~month:11 ~day:21 in
  let time = make_time ~hour:0 ~minute:0 ~second:0 in
  let (actual_date, actual_time) = Calendar.gregorian_seconds_to_naive (Calendar.naive_to_gregorian_seconds date time) in
