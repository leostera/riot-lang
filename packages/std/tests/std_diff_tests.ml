...
let test_path_component_key =
  Test.case "create Key path component" @@ fun _ctx ->
...
let test_path_component_index =
  Test.case "create Index path component" @@ fun _ctx ->
...
let test_kind_added =
  Test.case "create Added kind" @@ fun _ctx ->
...
let test_kind_removed =
  Test.case "create Removed kind" @@ fun _ctx ->
...
let test_kind_changed =
  Test.case "create Changed kind" @@ fun _ctx ->
...
let test_change_empty_path =
  Test.case "change with empty path" @@ fun _ctx ->
...
let test_change_nested_path =
  Test.case "change with nested path" @@ fun _ctx ->
    let result = {
...
let test_change_mixed_path =
  Test.case "change with mixed Key and Index path" @@ fun _ctx ->
    let result = {
...
let test_has_changes_empty =
  Test.case "has_changes on empty list" @@ fun _ctx ->
    let result = Diff.has_changes [] in
    if not result then
      Ok ()
    else Error "Expected false for empty list"

let test_has_changes_with_changes =
  Test.case "has_changes with actual changes" @@ fun _ctx ->
    let changes = [ { Diff.path = []; kind = Diff.Added 1 } ] in
    let result = Diff.has_changes changes in
    if result then
      Ok ()
    else Error "Expected true for non-empty list"

let test_additions_filter =
  Test.case "additions filters only Added changes" @@ fun _ctx ->
    let changes =
      [
        { Diff.path = [ Diff.Key "a" ]; kind = Diff.Added 1 };
        { Diff.path = [ Diff.Key "b" ]; kind = Diff.Removed 2 };
        {
          Diff.path = [ Diff.Key "c" ];
          kind = Diff.Changed (3, 4)
        };
...
let test_removals_filter =
  Test.case "removals filters only Removed changes" @@ fun _ctx ->
    let changes = [ { Diff.path = [ Diff.Key "a" ]; kind = Diff.Added 1 }; { Diff.path = [ Diff.Key "b" ]; kind = Diff.Removed 2 }; { Diff.path = [ Diff.Key "c" ]; kind = Diff.Removed 3 } ] in
...
let test_changes_filter =
  Test.case "changes filters only Changed changes" @@ fun _ctx ->
    let changes =
      [
        { Diff.path = [ Diff.Key "a" ]; kind = Diff.Added 1 };
        {
          Diff.path = [ Diff.Key "b" ];
          kind = Diff.Changed (2, 3)
        };
        { Diff.path = [ Diff.Key "c" ]; kind = Diff.Removed 4 };
        {
          Diff.path = [ Diff.Key "d" ];
    ...
    match changed with
    | [ { path = [ Diff.Key "b" ]; kind = Diff.Changed (2, 3) }; { path = [ Diff.Key "d" ]; kind = Diff.Changed (5, 6) } ] -> Ok ()
    | _ -> Error ("Expected 2 specific changes, got " ^ Int.to_string (List.length changed))

let test_at_path_exact_match =
  Test.case "at_path with exact match" @@ fun _ctx ->
    let diffs = [ { Diff.path = [ Diff.Key "user"; Diff.Key "name" ]; kind = Diff.Added "Alice" }; { Diff.path = [ Diff.Key "user"; Diff.Key "age" ]; kind = Diff.Added "30" }; { Diff.path = [ Diff.Key "config"; Diff.Key "port" ]; kind = Diff.Added "8080" } ] in
...
let test_at_path_no_match =
  Test.case "at_path with no match" @@ fun _ctx ->
    ...
    if List.length at_config = 0 then
      Ok ()
    else Error "Expected no matches"
