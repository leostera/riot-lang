...
let test_create_starts_empty = fun _ctx ->
  ...
  if Int.equal (StringBuilder.length buffer) 0 && String.equal (StringBuilder.contents buffer) "" then
    Ok ()
  else Error "StringBuilder.create should start empty"
...
let test_add_string_and_subbytes_preserve_order = fun _ctx ->
  ...
  if String.equal (StringBuilder.contents buffer) "precde" then
    Ok ()
  else Error "StringBuilder should append strings and byte slices in order"
...
let test_add_utf_8_uchar_materializes_utf8 = fun _ctx ->
  ...
  if String.equal (StringBuilder.contents buffer) "α" then
    Ok ()
  else Error "StringBuilder.add_utf_8_uchar should append UTF-8 text"

let tests = Test.[ case "create starts empty" test_create_starts_empty; case "string and byte slices preserve order" test_add_string_and_subbytes_preserve_order; case "utf-8 runes materialize as text" test_add_utf_8_uchar_materializes_utf8 ]
