...
let test_udp_socket_send_to_and_recv_from = fun _ctx ->
  ...
  | Ok server, Ok client ->
      ...
      | Ok _ -> (
        ...
        | Ok { bytes_read; from } ->
            if not (String.equal (Bytes.sub server_buffer ~offset:0 ~len:bytes_read |> Result.expect ~msg:"server datagram slice should be valid" |> Bytes.to_string) "ping") then
              (
                Net.UdpSocket.close client;
                Net.UdpSocket.close server;
              ...
              if not (Int.equal (Net.Addr.port from) (Net.Addr.port client_addr)) then
                (
                  Net.UdpSocket.close client;
                  Net.UdpSocket.close server;
                ...
                | Ok _ -> (
                  ...
                  | Ok { bytes_read; from } ->
                      Net.UdpSocket.close client;
                      Net.UdpSocket.close server;
                      if not (String.equal (Bytes.sub client_buffer ~offset:0 ~len:bytes_read |> Result.expect ~msg:"client datagram slice should be valid" |> Bytes.to_string) "pong") then
                        Error "client received the wrong datagram payload"
                      else
                        if not (Int.equal (Net.Addr.port from) (Net.Addr.port server_addr)) then
  ...
  | Ok server, Ok client ->
      ...
      | Ok (), Ok () -> (
        ...
        | Ok _ -> (
          ...
          | Ok bytes_read ->
              if not (String.equal (Bytes.sub server_buffer ~offset:0 ~len:bytes_read |> Result.expect ~msg:"connected server datagram slice should be valid" |> Bytes.to_string) "hello") then
                (
                  Net.UdpSocket.close client;
                  Net.UdpSocket.close server;
                ...
                | Ok _ -> (
                  ...
                  | Ok reply_bytes ->
                      Net.UdpSocket.close client;
                      Net.UdpSocket.close server;
                      if String.equal (Bytes.sub client_buffer ~offset:0 ~len:reply_bytes |> Result.expect ~msg:"connected client datagram slice should be valid" |> Bytes.to_string) "world" then
                        Ok ()
                      else Error "connected reply recv returned the wrong payload"
                )
        )
      )

let tests = Test.[ case "UdpSocket send_to and recv_from preserve datagram boundaries" test_udp_socket_send_to_and_recv_from; case "UdpSocket connect enables send and recv without explicit peers" test_udp_socket_connect_supports_send_and_recv ]
