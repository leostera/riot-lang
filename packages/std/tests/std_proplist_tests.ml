...
let test_add_prepends_new_binding = fun _ctx ->
  if Proplist.add [ ("size", "l") ] ~key:"color" ~value:"red" = [ ("color", "red"); ("size", "l") ] then
    Ok ()
  else
...
let test_set_appends_missing_binding = fun _ctx ->
  if Proplist.set [ ("size", "l") ] ~key:"color" ~value:"red" = [ ("size", "l"); ("color", "red") ] then
    Ok ()
  else
...
let test_iter_yields_pairs_in_order = fun _ctx ->
  let actual = Proplist.iter [ ("color", "red"); ("size", "l"); ("color", "blue") ] |> Iterator.to_list in
