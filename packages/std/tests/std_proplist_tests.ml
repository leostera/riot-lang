...
let test_empty = fun _ctx ->
  if Proplist.is_empty Proplist.empty && Int.equal (Proplist.length Proplist.empty) 0 then
    Ok ()
  else Error "expected Proplist.empty to be empty"
...
let test_get_all_preserves_binding_order = fun _ctx ->
  if Proplist.get_all
    ...
    ~key:"color" = [ "red"; "blue" ] then
    Ok ()
  else Error "expected Proplist.get_all to return all matching values in order"
...
let test_add_prepends_new_binding = fun _ctx ->
  if Proplist.add
    ...
    Ok ()
  else Error "expected Proplist.add to prepend the new binding"
...
let test_set_replaces_existing_bindings_in_place = fun _ctx ->
  if Proplist.set
    ...
    Ok ()
  else Error "expected Proplist.set to replace all existing bindings while preserving order"
...
let test_set_appends_missing_binding = fun _ctx ->
  if Proplist.set
    ...
    Ok ()
  else Error "expected Proplist.set to append a missing binding"
...
let test_remove_drops_all_matching_bindings = fun _ctx ->
  if Proplist.remove
    ...
    Ok ()
  else Error "expected Proplist.remove to drop every matching binding"
...
let test_contains_key = fun _ctx ->
  ...
  in
  if Proplist.contains_key proplist ~key:"color" && not (Proplist.contains_key proplist ~key:"shape") then
    Ok ()
  else Error "expected Proplist.contains_key to reflect key membership"
...
let test_keys_and_values_preserve_pair_order = fun _ctx ->
  ...
  in
  if Proplist.keys proplist = [ "color"; "size"; "color" ] && Proplist.values proplist = [ "red"; "l"; "blue" ] then
    Ok ()
  else Error "expected Proplist.keys and Proplist.values to preserve pair order"
...
let test_fold_left_visits_pairs_in_order = fun _ctx ->
  ...
  if String.equal actual "color=red;size=l;" then
    Ok ()
  else Error "expected Proplist.fold_left to visit pairs in order"
...
let test_iter_yields_pairs_in_order = fun _ctx ->
  ...
  if actual = [
    ...
    Ok ()
  else Error "expected Proplist.iter to yield pairs in order"
...
let tests = Test.[
  ...
  case "Proplist.set replaces existing bindings in place" test_set_replaces_existing_bindings_in_place;
  ...
  case "Proplist.keys and Proplist.values preserve pair order" test_keys_and_values_preserve_pair_order;
