...
let seeded_rng = fun seed ->
  Random.Rng.standard ~seed ()
  |> Result.unwrap

let sample_list = fun ~rng distribution ~len ->
...
let test_bool_is_deterministic_for_the_same_seed = fun _ctx ->
  ...
  if left = right then
    ...
    Error "expected Random.bool to be deterministic for the same seed"
...
let test_int_with_bound_one_is_always_zero = fun _ctx ->
  ...
  if List.all samples ~fn:(Int.equal 0) then
    ...
    Error "expected Random.int 1 to always return 0"
...
let test_int_with_positive_bound_stays_in_range = fun _ctx ->
  ...
  if List.all samples ~fn:(fun value -> value >= 0 && value < bound) then
    Ok ()
  else
    Error "expected Random.int to stay inside [0, bound)"
...
let test_int_range_with_equal_bounds_returns_that_value = fun _ctx ->
  ...
  if List.all samples ~fn:(Int.equal 4) then
    ...
    Error "expected Random.int_range with equal bounds to return that exact value"
...
let test_int32_with_bound_one_is_zero = fun _ctx ->
  ...
  if List.all samples ~fn:(Int32.equal 0l) then
    ...
    Error "expected Random.int32 1l to always return 0l"
...
let test_int64_with_bound_one_is_zero = fun _ctx ->
  ...
  if List.all samples ~fn:(Int64.equal 0L) then
    ...
    Error "expected Random.int64 1L to always return 0L"
...
let test_float_with_zero_bound_returns_zero = fun _ctx ->
  ...
  if List.all samples ~fn:(Float.equal 0.0) then
    ...
    Error "expected Random.float 0.0 to always return 0.0"
...
let test_float_range_with_equal_bounds_returns_that_value = fun _ctx ->
  ...
  if List.all samples ~fn:(Float.equal 1.25) then
    ...
    Error "expected Random.float_range with equal bounds to return that exact bound"
...
let test_bernoulli_boundary_probabilities_are_constant = fun _ctx ->
  ...
  in
  if
    List.all always_false ~fn:(Bool.equal false) && List.all always_true ~fn:(Bool.equal true)
  then
    Ok ()
  else
    Error "expected bernoulli boundary probabilities to return constant values"
...
let test_one_of_singleton_returns_the_only_element = fun _ctx ->
  ...
  if List.all samples ~fn:(String.equal "only") then
    ...
    Error "expected Random.one_of on a singleton list to return its only element"
...
let test_choose_n_full_population_returns_each_element_once = fun _ctx ->
  ...
  | Ok sample ->
      if
        List.length sample = 4
        && List.sort sample ~compare:Int.compare = List.sort input ~compare:Int.compare
      then
        Ok ()
      else
        Error "expected Random.choose_n full-size sample to contain every input exactly once"
  | Error err ->
      Error ("expected Random.choose_n full-size sample to succeed, got "
...
let tests =
  Test.[
    case
      "Random.init with the same seed reproduces the default sequence"
      test_init_with_same_seed_repeats_default_sequence;
    case
      "Random.Rng.standard with the same seed is deterministic"
      test_seeded_standard_rngs_are_deterministic;
    case
      "Random.sample with an explicit RNG is independent from the default RNG"
      test_explicit_rng_is_independent_from_default_rng;
    case "Random.bits returns a non-negative int" test_bits_are_non_negative;
    case
      "Random.bool is deterministic for the same seed"
      test_bool_is_deterministic_for_the_same_seed;
    case "Random.int 1 always returns 0" test_int_with_bound_one_is_always_zero;
    case
      "Random.int with a positive bound stays in range"
      test_int_with_positive_bound_stays_in_range;
    case "Random.int rejects non-positive bounds" test_int_rejects_non_positive_bounds;
    case
      "Random.int_range with equal bounds returns that value"
      test_int_range_with_equal_bounds_returns_that_value;
    ...
    case
      "Random.float with a positive bound stays in range"
      test_float_with_positive_bound_stays_in_range;
    case
      "Random.float_range with equal bounds returns that value"
      test_float_range_with_equal_bounds_returns_that_value;
    case "Random.float_range rejects invalid bounds" test_float_range_rejects_invalid_bounds;
    case
      "Random.Distribution.bernoulli respects boundary probabilities"
      test_bernoulli_boundary_probabilities_are_constant;
    case
      "Random.Distribution.bernoulli rejects invalid probabilities"
      test_bernoulli_rejects_invalid_probabilities;
    case "Random.one_of [] returns EmptyPopulation" test_one_of_empty_population_errors;
    case
      "Random.one_of on a singleton list returns the only element"
      test_one_of_singleton_returns_the_only_element;
    case "Random.choose_n count 0 returns an empty list" test_choose_n_zero_returns_empty;
    case
      "Random.choose_n full-size sample returns every element once"
      test_choose_n_full_population_returns_each_element_once;
