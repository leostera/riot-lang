...
let test_encode_simple () =
  ...
  if encoded = "9jqo" then Ok ()
  else Error ("Expected '9jqo', got '" ^ encoded ^ "'")
...
let test_encode_zeros () =
  let encoded = Base85.encode "\x00\x00\x00\x00" in
  if encoded = "z" then Ok ()
  else Error ("Expected 'z', got '" ^ encoded ^ "'")
