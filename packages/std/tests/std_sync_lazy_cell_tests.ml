    ...
    (
      fun _ctx ->
        ...
        if not (Sync.LazyCell.is_initialized cell) && Option.is_none (Sync.LazyCell.take cell) then
          Ok ()
        else Error "expected a fresh LazyCell to start uninitialized"
    )
  ...
  Test.case "sync lazy_cell force computes once and caches the value"
    (
      fun _ctx ->
        ...
        if String.equal first "value" && String.equal second "value" && Sync.LazyCell.is_initialized cell && Sync.Atomic.get calls = 1 then
          Ok ()
        else Error "expected force to compute once and reuse the cached value"
    )
      ...
      fun _ctx ->
        ...
        let cell =
          Sync.LazyCell.create
            (
              fun () ->
                let _ = Sync.Atomic.fetch_and_add calls 1 in 9
            )
        in
        ...
        if taken = Some 9 && not (Sync.LazyCell.is_initialized cell) && Sync.Atomic.get calls = 1 then
          Ok ()
        else Error "expected take to return the cached value and clear storage"
    )
    ...
    (
      fun _ctx ->
        ...
        let cell =
          Sync.LazyCell.create
            (
              fun () -> Int.succ (Sync.Atomic.fetch_and_add calls 1)
            )
        in
        let first = Sync.LazyCell.force cell in
        ...
        if first = 1 && second = 2 && Sync.Atomic.get calls = 2 then
          Ok ()
        else Error "expected force to recompute after take clears the cache"
    )
