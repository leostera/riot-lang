...
let test_lazy_cell_force_computes_once =
  Test.case
    "sync lazy_cell force computes once and caches the value"
    (fun _ctx ->
...
let test_lazy_cell_get_is_force =
  Test.case
    "sync lazy_cell get is an alias for force"
    (fun _ctx ->
      let calls = Sync.Atomic.make 0 in
      let cell =
        Sync.LazyCell.create
...
let test_lazy_cell_take_returns_cached_value_and_clears_storage =
  Test.case
    "sync lazy_cell take returns the cached value and clears storage"
    (fun _ctx ->
      let calls = Sync.Atomic.make 0 in
      let cell =
        Sync.LazyCell.create
      ...
      if
        taken = Some 9 && not (Sync.LazyCell.is_initialized cell) && Sync.Atomic.get calls = 1
      then
        Ok ()
      else
