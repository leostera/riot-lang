...
let test_agent_start_and_get = fun _ctx ->
  ...
  Agent.stop agent;
  if Int.equal actual 41 then Ok () else Error "expected Agent.get to return initial state"
...
let test_agent_update_and_get = fun _ctx ->
  ...
  let actual = Agent.get agent ~fn:(fun value -> value) in
  Agent.stop agent;
  if Int.equal actual 1 then Ok () else Error "expected Agent.update to persist new state"
...
let test_agent_get_and_update = fun _ctx ->
  ...
  let actual = Agent.get agent ~fn:(fun value -> value) in
  Agent.stop agent;
  if Int.equal reply 10 && Int.equal actual 8 then Ok ()
...
let test_agent_cast_eventually_updates = fun _ctx ->
  ...
  in
  Agent.stop agent;
  if updated then Ok () else Error "expected Agent.cast to update state eventually"
...
let test_agent_start_link_supports_operations = fun _ctx ->
  ...
  let actual = Agent.get agent ~fn:(fun value -> value) in
  Agent.stop agent;
  if String.equal actual "hello world" then Ok ()
