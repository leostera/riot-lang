...
let test_agent_start_and_get = fun _ctx ->
  let agent =
    Agent.start ~fn:(fun () -> 41)
  in
  let actual =
    Agent.get agent ~fn:(fun value -> value)
  in
  Agent.stop agent;
...
let test_agent_get_and_update = fun _ctx ->
  let agent =
    Agent.start ~fn:(fun () -> 5)
  in
  let reply =
    Agent.get_and_update agent ~fn:(fun value -> (value * 2, value + 3))
  in
  let actual =
    Agent.get agent ~fn:(fun value -> value)
  in
...
let test_agent_cast_eventually_updates = fun _ctx ->
  let agent =
    Agent.start ~fn:(fun () -> 10)
  in
  Agent.cast agent ~fn:(fun value -> value + 5);
  let updated =
    wait_until ~attempts:50 ~delay:(Time.Duration.from_millis 10)
      ~fn:(fun () ->
        Int.equal (Agent.get agent ~fn:(fun value -> value)) 15)
  in
  Agent.stop agent;
  if updated then
    Ok ()
