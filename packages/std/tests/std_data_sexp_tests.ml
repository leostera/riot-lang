...
let test_parse_atom_with_numbers () =
...
let test_parse_list_with_atoms () =
...
let test_to_string_nested () =
  ...
  if Sexp.to_string sexp = "(outer (inner))" then Ok ()
...
let test_is_atom () =
  if Sexp.is_atom (Sexp.atom "test") && not (Sexp.is_atom (Sexp.list []))
  then Ok ()
  else Error "is_atom check failed"

let test_is_list () =
  if Sexp.is_list (Sexp.list []) && not (Sexp.is_list (Sexp.atom "test"))
  then Ok ()
...
let test_csexp_atom () =
  let sexp = Sexp.atom "hello" in
  if Sexp.Csexp.to_string sexp = "5:hello" then Ok ()
  else Error "Csexp atom serialization failed"

let test_csexp_list () =
  let sexp = Sexp.list [Sexp.atom "a"; Sexp.atom "b"] in
  if Sexp.Csexp.to_string sexp = "(1:a1:b)" then Ok ()
  else Error "Csexp list serialization failed"

let test_csexp_parse_atom () =
...
let () =
  Miniriot.run
    ~main:(fun ~args -> Test.Cli.main ~name:"sexp" ~tests ~args)
    ~args:Env.args
