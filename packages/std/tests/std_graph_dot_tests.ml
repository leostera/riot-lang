...
let test_add_node_renders_label_and_attributes = fun _ctx ->
  let graph = Graph.Dot.create ~name:"deps" ~style:Graph.Dot.Directed in
  let graph =
    Graph.Dot.add_node graph ~id:"A" ~label:"Module A" ~attrs:[ ("color", "blue"); ("shape", "box") ] ()
  in
  let rendered = Graph.Dot.to_string graph
  in
  if
    String.contains rendered "\"A\" [label=\"Module A\", color=\"blue\", shape=\"box\"];"
  then
    Ok ()
  else
