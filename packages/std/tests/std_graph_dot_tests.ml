...
let test_graph_level_attributes_are_rendered = fun _ctx ->
  let graph = {
    (Graph.Dot.create ~name:"deps" ~style:Graph.Dot.Directed)
    with graph_attrs = [ ("rankdir", "LR"); ("label", "Dependency Graph") ]
  } in
  let rendered = Graph.Dot.to_string graph in
  if
    String.contains rendered "rankdir=\"LR\";" && String.contains rendered "label=\"Dependency Graph\";"
  then
    Ok ()
  else
...
let test_special_characters_are_escaped = fun _ctx ->
  ...
  let rendered = Graph.Dot.to_string graph in
  if
    String.contains rendered "\"a\\\"b\\\\c\"" && String.contains rendered "label=\"quote\\\"slash\\\\line\\nbreak\""
  then
    Ok ()
  else
