...
let sample_benchmarks =
  ...
  [
    Bench.with_config ~config "alpha_long"
      (
        fun () -> ()
      );
    Bench.with_config ~config "beta"
      (
        fun () -> ()
      );
    Bench.with_config ~config "middle_long_case"
      (
        fun () -> ()
      );
  ]
...
let run_sample_capture = fun args ->
  let cmd = Command.make (self_executable ()) ~args:("sample" :: args) in Command.output cmd |> Result.expect ~msg:"failed to run sample bench cli"

let json_output_lines = fun stdout -> stdout |> String.split ~by:"\n" |> List.filter ~fn:(
  fun line -> not (String.equal (String.trim line) "")
)

let parse_json_output = fun stdout ->
  let line = json_output_lines stdout |> List.rev |> List.get ~at:0 |> Option.unwrap_or ~default:stdout in Data.Json.of_string line |> Result.expect ~msg:"failed to parse json output"
...
let bench_names_from_json = fun stdout ->
  ...
  | Some (Data.Json.Array benchmarks) -> benchmarks |> List.filter_map ~fn:(
    fun benchmark_json ->
...
let test_list_benchmarks_lists_all_cases = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    let lines = output.stdout |> String.split ~by:"\n" |> List.filter ~fn:(
    ...
    if lines = expected then
      Ok ()
    else Error ("unexpected listed benchmark names: " ^ String.concat ", " lines)
...
let test_list_benchmarks_json_includes_metadata = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    | first :: _ ->
        ...
        if has "index" (Data.Json.Int 1) && has "name" (Data.Json.String "alpha_long") && has "kind" (Data.Json.String "benchmark") && has "iterations" (Data.Json.Int 1) && has "warmup" (Data.Json.Int 0) && has "skip" (Data.Json.Bool false) then
          Ok ()
        else Error "expected list-benchmarks --json to include metadata fields"
...
let test_list_benchmarks_respects_pattern = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    if names = expected then
      Ok ()
    else Error ("unexpected filtered benchmark names: " ^ String.concat ", " names)
...
let test_run_benchmarks_pattern_matches_substring = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    if String.contains output.stdout "Running 2 benchmarks" && String.contains output.stdout "[1] alpha_long:" && String.contains output.stdout "[2] middle_long_case:" && not (String.contains output.stdout "beta:") then
      Ok ()
    else Error "unexpected filtered benchmark output for _long"

let test_run_benchmarks_succeeds_with_zero_matches = fun _ctx ->
  let output = run_sample_capture [ "run-benchmarks"; "missing_case" ] in
  if not (Int.equal output.status 0) then
    Error ("expected filtered benchmark run with no matches to succeed, got " ^ Int.to_string output.status)
  else
    if String.contains output.stdout "Running 0 benchmarks" && String.contains output.stdout "Summary: 0 total" then
      Ok ()
    else Error "expected no-match benchmark run to report zero benchmarks"
...
let test_run_benchmarks_json_flag_filters_results = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    if names = expected then
      Ok ()
    else Error ("unexpected filtered benchmark names for _long: " ^ String.concat ", " names)
...
let test_run_benchmarks_json_flag_reports_zero_matches = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    if bench_names_from_json output.stdout = [] then
      Ok ()
    else Error "expected zero-match benchmark json run to report an empty benchmark list"
...
let test_run_benchmarks_json_includes_timing_fields = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    let has_int_field name json =
      ...
      | _ -> false
    in
    if has_int_field "started_at_us" json && has_int_field "completed_at_us" json && has_int_field "duration_us" json then
      Ok ()
    else Error "expected benchmark json output to include timing fields"
    ...
    | Some (Data.Json.Array ((Data.Json.Object first) :: _)) -> (
      ...
      | Some (Data.Json.Object stats) -> (
        ...
        | Some (Data.Json.Object gc_fields) ->
            if (assoc_value "minor_collections" gc_fields |> Option.is_some) && (assoc_value "major_collections" gc_fields |> Option.is_some) && (assoc_value "compactions" gc_fields |> Option.is_some) then
              Ok ()
            else Error "expected benchmark json output to include gc fields"
...
let test_run_benchmarks_json_emits_case_started_progress = fun _ctx ->
  ...
  if not (Int.equal output.status 0) then
    ...
    let progress_lines = json_output_lines output.stdout |> List.filter_map ~fn:(
      fun line ->
    ...
    | first :: _ ->
        if Data.Json.get_field "index" first = Some (Data.Json.Int 1) && Data.Json.get_field "name" first = Some (Data.Json.String "alpha_long") && Data.Json.get_field "iterations" first = Some (Data.Json.Int 1) && Data.Json.get_field "warmup" first = Some (Data.Json.Int 0) then
          Ok ()
        else Error "expected BenchCaseStarted progress to include benchmark metadata"
...
let sample_main = fun ~args ->
  match args with
  | exe :: _sample :: rest -> Bench.Cli.main ~name:"sample" ~benchmarks:sample_benchmarks ~args:(exe :: rest)
