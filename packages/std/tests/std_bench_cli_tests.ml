...
let test_list_benchmarks_lists_all_cases () =
  ...
  if not (Int.equal output.status 0) then
    ...
    in
    if lines = expected then Ok ()
    else
      Error
        ("unexpected listed benchmark names: " ^ String.concat ", " lines)

let test_run_benchmarks_pattern_matches_substring () =
...
let test_run_benchmarks_succeeds_with_zero_matches () =
  let output = run_sample_capture [ "run-benchmarks"; "missing_case" ] in
  if not (Int.equal output.status 0) then
    Error
      ("expected filtered benchmark run with no matches to succeed, got "
...
let meta_tests =
  [
    Test.case
      "list-benchmarks lists all sample cases"
      test_list_benchmarks_lists_all_cases;
    Test.case
      "run-benchmarks pattern matches substring"
      test_run_benchmarks_pattern_matches_substring;
    Test.case
...
let sample_main ~args =
  match args with
  | exe :: _sample :: rest ->
      Bench.Cli.main ~name:"sample" ~benchmarks:sample_benchmarks
        ~args:(exe :: rest)
  | _ ->
      Error (Failure "expected sample subcommand arguments")
...
let meta_main ~args =
  let normalize_args = function
    ...
    | args -> args
  in
  Test.Cli.main ~name:"std_bench_cli_tests" ~tests:meta_tests
