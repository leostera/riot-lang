...
let test_floor_ceil_round = fun _ctx ->
  if Float.equal (Float.floor 3.9) 3.0 && Float.equal (Float.ceil 3.1) 4.0 && Float.equal (Float.round 3.5) 4.0 then
    Ok ()
  else Error "expected floor/ceil/round to behave predictably"

let test_pow = fun _ctx ->
  if Float.equal (Float.pow 2.0 5.0) 32.0 then
    Ok ()
  else Error "expected Float.pow 2 5 = 32"

let test_operator_syntax = fun _ctx -> let open Float in
if Float.((7.5 + 0.5) = 8.0 && (7.5 - 0.5) = 7.0 && (3.0 * 2.0) = 6.0 && (7.5 / 2.5) = 3.0 && 7.5 > 2.5) then
  Ok ()
else Error "expected Float operators to mirror the runtime float operations"
