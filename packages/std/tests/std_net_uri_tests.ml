...
let test_percent_encode_basic = fun () ->
...
let test_percent_encode_email = fun () ->
...
let test_percent_encode_unreserved = fun () ->
...
let test_percent_encode_special = fun () ->
...
let test_percent_encode_slash = fun () ->
...
let test_percent_encode_ampersand = fun () ->
...
let test_percent_decode_basic = fun () ->
...
let test_percent_decode_email = fun () ->
...
let test_percent_decode_percent = fun () ->
...
let test_percent_decode_uppercase = fun () ->
...
let test_percent_decode_lowercase = fun () ->
  let decoded = Uri.percent_decode "%2b%2f%3d" in
  if decoded = "+/=" then
    Ok ()
  else
...
let test_percent_decode_utf8 = fun () ->
...
let test_percent_decode_invalid = fun () ->
...
let test_percent_decode_incomplete = fun () ->
...
let test_percent_decode_mixed = fun () ->
...
let test_form_encode_space = fun () ->
...
let test_form_encode_special = fun () ->
...
let test_form_encode_unreserved = fun () ->
...
let test_form_encode_ampersand = fun () ->
...
let test_form_decode_percent = fun () ->
...
let test_form_decode_mixed = fun () ->
...
let test_form_decode_multiple_plus = fun () ->
...
let test_roundtrip_percent = fun () ->
...
let test_roundtrip_special_chars = fun () ->
...
let test_query_parse_simple = fun () ->
...
let test_query_parse_encoded = fun () ->
...
let test_query_parse_empty_value = fun () ->
...
let test_query_parse_no_value = fun () ->
...
let test_query_parse_special_chars = fun () ->
...
let test_query_parse_multiple_same_key = fun () ->
...
let test_query_parse_empty = fun () ->
...
let test_query_to_string_simple = fun () ->
...
let test_query_to_string_encoded = fun () ->
...
let test_query_to_string_special = fun () ->
...
let test_query_to_string_empty_value = fun () ->
...
let test_query_to_string_unreserved = fun () ->
...
let test_query_roundtrip_empty_value = fun () ->
...
let test_query_roundtrip_special_delimiters = fun () ->
...
let test_reserved_gen_delims = fun () ->
  (* gen-delims = ":" / "/" / "?" / "#" / "[" / "]" / "@" *)
  let tests = [
    ("%3A", ":");
    ("%2F", "/");
...
let test_reserved_sub_delims = fun () ->
  (* sub-delims = "!" / "$" / "&" / "'" / "(" / ")" / "*" / "+" / "," / ";" / "=" *)
  let tests = [
    ("%21", "!");
    ("%24", "$");
...
let test_percent_decode_filename_with_spaces = fun () ->
...
let test_percent_decode_path_with_spaces = fun () ->
...
let test_percent_decode_relative_path_with_spaces = fun () ->
...
let test_percent_decode_preserves_length = fun () ->
...
let test_percent_decode_multiple_spaces = fun () ->
...
let test_uri_parse_path_with_percent_encoding = fun () ->
...
let test_uri_parse_path_preserves_encoding = fun () ->
...
let test_uri_parse_and_decode_path = fun () ->
...
let test_uri_roundtrip_with_encoded_path = fun () ->
...
let test_uri_with_encoded_query = fun () ->
...
let () =
  Miniriot.run ~main:(fun ~args -> Test.Cli.main ~name:"net_uri" ~tests ~args) ~args:Env.args ()
