...
let test_roundtrip_percent = fun _ctx ->
  ...
  if decoded = original then
    Ok ()
  else Error ("Round-trip failed: '" ^ original ^ "' -> '" ^ encoded ^ "' -> '" ^ decoded ^ "'")
...
let test_query_parse_multiple_same_key = fun _ctx ->
  ...
  if List.length tags = 3 && List.contains tags ~value:"red" && List.contains tags ~value:"blue" && List.contains tags ~value:"green" then
    Ok ()
...
let test_query_parse_empty = fun _ctx ->
  ...
  if List.length params = 0 then
    Ok ()
  else Error "Empty query should parse to []"
...
let test_query_to_string_encoded = fun _ctx ->
  ...
  if query = "name=John+Doe&email=test%40example.com" then
    Ok ()
  else Error ("Expected 'name=John+Doe&email=test%40example.com', got '" ^ query ^ "'")
...
let test_reserved_gen_delims = fun _ctx ->
  ...
  in
  let results = List.map tests ~fn:(
    fun (enc, dec) -> Uri.percent_decode enc = dec
  ) in
  if List.all results ~fn:(
    fun value -> value
  ) then
    Ok ()
...
let test_uri_from_slice = fun _ctx ->
  ...
  | Ok uri ->
      if Uri.scheme uri != Some "https" then
        ...
        if Uri.host uri != Some "example.com" then
          Error "Expected example.com host"
        else
          if Uri.port uri != Some 8_443 then
            Error "Expected port 8443"
          else
            if Uri.path uri != "/a/b" then
              Error ("Expected /a/b path, got " ^ Uri.path uri)
            else
              if Uri.query uri != Some "q=1" then
                Error "Expected q=1 query"
              else
                if Uri.fragment uri != Some "frag" then
...
let test_uri_from_slice_origin_form = fun _ctx ->
  ...
  | Ok uri ->
      if Uri.scheme uri != None then
        ...
        if Uri.authority uri != None then
  ...
  | Ok uri ->
      if Uri.scheme uri != None then
        ...
        if Uri.authority uri != None then
          ...
          if Uri.path uri != "/_global-navigation/payloads.json" then
            Error ("Unexpected path: " ^ Uri.path uri)
          else
            if not (Option.is_some (Uri.query uri)) then
              Error "Expected query"
            else
              if Uri.fragment uri != None then
