...
let test_handler_replacement =
  Test.case "telemetry: handler replacement" @@ fun _ctx ->
    ...
    Telemetry.attach "my-handler"
      (
        fun _ -> first_called := true
      );
    Telemetry.attach "my-handler"
      (
        fun _ -> second_called := true
      );
    Telemetry.emit (TestEvent { value = 1 });
    Telemetry.stop ();
...
let test_pattern_matching =
  Test.case "telemetry: pattern matching" @@ fun _ctx ->
    ...
    if !test_count = 2 && !another_count = 1 then
      Ok ()
    else Error ("Expected test=2, another=1, got test=" ^ Int.to_string !test_count ^ ", another=" ^ Int.to_string !another_count)
...
let test_handler_exception_isolation =
  Test.case "telemetry: exception isolation" @@ fun _ctx ->
    ...
    Telemetry.attach "bad-handler"
      (
        fun _ -> panic "boom"
      );
    Telemetry.attach "good-handler"
      (
        fun _ -> good_handler_called := true
      );
    Telemetry.emit (TestEvent { value = 1 });
    Telemetry.stop ();
...
let test_restart_after_stop =
  Test.case "telemetry: restart after stop" @@ fun _ctx ->
    ...
    Telemetry.attach "first"
      (
        fun _ -> first_called := !first_called + 1
      );
    Telemetry.emit (TestEvent { value = 1 });
    Telemetry.stop ();
    let _pid = Telemetry.start () in
    ...
    Telemetry.attach "second"
      (
        fun _ -> second_called := !second_called + 1
      );
    Telemetry.emit (TestEvent { value = 2 });
    Telemetry.stop ();
    if !first_called = 1 && !second_called = 1 then
      Ok ()
    else Error ("Expected restart to isolate handler calls; first=" ^ Int.to_string !first_called ^ ", second=" ^ Int.to_string !second_called)

let test_stop_idempotent =
  Test.case "telemetry: stop idempotent and clears handlers view" @@ fun _ctx ->
    let _pid = Telemetry.start () in
    Telemetry.detach_all ();
    Telemetry.attach "tmp"
      (
        fun _ -> ()
      );
    Telemetry.stop ();
