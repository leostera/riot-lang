...
45. Keep off-heap syscall-facing byte storage owned by `kernel`, and re-export it from `Std.IO` under explicit names such as `IoBuffer`, `IoSlice`, and `Iovec`. Code above `std` should not reach into `Kernel.IO` directly for parsing or buffering primitives.
