...
46. Keep `Std.IO` explicit about copy boundaries. Off-heap buffers and borrowed views should flow through `Buffer`, `IoSlice`, `Iovec`, and `IoBuffer`; reader accumulation helpers should target caller-provided `Buffer` values by default (`read_into_buffer`, `read_all_into_buffer`), while heap text building belongs on `Std.StringBuilder`.
