...
46. Keep `Std.IO` explicit about copy boundaries. Off-heap buffers and borrowed views should flow through `Buffer`, `IoSlice`, `Iovec`, and `IoBuffer`; heap text building belongs on `Std.StringBuilder`, and heap conversions should stay on `from_*` / `to_*` names rather than hiding allocations behind generic helpers.
