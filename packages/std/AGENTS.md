...
45. Keep off-heap syscall-facing byte storage owned by `kernel`, and re-export it through `Std.IO`. `Std.IO.Buffer` is the default off-heap buffer surface, with `Std.IO.IoBuffer` retained as the exact kernel-shaped API; code above `std` should not reach into `Kernel.IO` directly for parsing or buffering primitives.
46. Keep `Std.IO` explicit about copy boundaries. Off-heap buffers and borrowed views should flow through `Buffer`, `IoSlice`, `Iovec`, and `IoBuffer`; reader and writer accumulation helpers should target caller-provided `Buffer` values by default (`read_into_buffer`, `read_all_into_buffer`, `write_buffer`, `write_all_buffer`), while heap text building belongs on `Std.StringBuilder`. `Std.IO.BufferedReader` may expose borrowed `IoSlice` views (`peek_slice`, `read_slice`, `read_line_slice`), but those views are only valid until the next buffered-reader operation that may refill or consume the internal buffer.
