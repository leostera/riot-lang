...
5. `std` owns the `std:no-stdlib` tusk-fix provider source under `src/tusk_fix_rules/`; keep its diagnostics aligned with the scheduler and ownership rationale.
