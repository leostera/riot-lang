...
6. `Std.Test.Cli` owns the portable test-binary contract (`list-tests`, `run-tests`, `--json`, query filtering, and shared selectors such as `--small`, `--long`, and `--flaky`). Keep individual test binaries on that runner instead of inventing local CLIs.
