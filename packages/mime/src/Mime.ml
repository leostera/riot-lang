...
type content_type = { media_type: string; subtype: string; parameters: (string * string) List.t }
...
type part = { headers: header List.t; content: string }
...
let find_header = fun name headers ->
  let name_lower = String.lowercase_ascii name in List.find headers ~fn:(
    fun h ->
      match h with
...
let percent_decode = fun s ->
  ...
  let rec decode i =
    if i >= len then
      ...
      if String.get_unchecked s ~at:i = '%' && i + 2 < len then
        (
...
let find_char_from = fun value ~start ~char ->
  ...
  let rec loop index =
    if index >= len then
      ...
      if String.get_unchecked value ~at:index = char then
        Some index
      else loop (index + 1)
  in
  if start < 0 then
...
let rec parse_content_type_string = fun value ->
        ...
        let value_end =
          if String.length rest > 0 && String.get_unchecked rest ~at:0 = '"' then
            ...
            | Some semi_idx ->
                ...
                let remaining = String.trim (String.sub rest ~offset:(semi_idx + 1) ~len:(String.length rest - semi_idx - 1)) in (value, remaining)
            | None -> String.trim rest, ""
        in
        let value, remaining = value_end in parse_params remaining ((key, value) :: acc)
  in
  ...
  | Some idx ->
      ...
      let params_str = String.trim (String.sub value ~offset:(idx + 1) ~len:(String.length value - idx - 1)) in
      let raw_params = parse_params params_str [] in
      let params = combine_rfc2231_params raw_params in (main_type, params)
and combine_rfc2231_params = fun raw_params ->
  ...
  let add_continuation ~name ~num ~decoded =
    let parts =
      ...
      | None ->
          ...
          let _ = Collections.HashMap.insert continuations ~key:name ~value:p in p
    in
  ...
  List.for_each ~fn:(
    fun ((key, value)) ->
      ...
      | `Encoded (name, value) ->
          let _charset, decoded = parse_rfc2231_value value in Cell.set regular ((name, decoded) :: Cell.get regular)
      | `Continuation (name, num, is_encoded, value) ->
          let decoded =
            if is_encoded then
              let _charset, decoded = parse_rfc2231_value value in decoded
            else value
          in
          add_continuation ~name ~num ~decoded
  ) raw_params;
  let combined = Cell.create (Cell.get regular) in
  Collections.HashMap.for_each continuations ~fn:(
    fun name parts_cell ->
      let parts = Cell.get parts_cell |> List.sort ~compare:(
        fun ((a, _)) ((b, _)) -> Int.compare a b
      ) in
      let value = String.concat "" (List.map ~fn:(
        fun (_, value) -> value
      ) parts) in Cell.set combined ((name, value) :: Cell.get combined)
  );
  Cell.get combined
...
let parse_content_type = fun value ->
  ...
  | Some slash ->
      ...
      let sub = String.sub main_type ~offset:(slash + 1) ~len:(String.length main_type - slash - 1) in { media_type = media; subtype = sub; parameters = params }
...
let parse_content_disposition = fun value ->
  ...
  let disp_type, params =
    ...
    | Some idx ->
        ...
        let (_, parameters) = parse_content_type_string ("x;" ^ rest) in (dtype, parameters)
  in
...
let rec parse = fun ~headers ~body ->
  ...
  | Some (ContentType ct) ->
      ...
      if String.starts_with ~prefix:"multipart/" full_type then
        ...
        | Some boundary ->
            let parts = parse_multipart boundary body in Ok (MultiPart { boundary; parts })
      else Ok (SinglePart { headers = typed_headers; content = body })
...
and parse_multipart = fun boundary body ->
  ...
  in
  let part_contents = find_parts [] None lines in List.filter_map ~fn:(
    fun part_lines ->
...
let quoted_printable_decode = fun s ->
  ...
  let rec decode i =
    if i >= len then
      ...
      if String.get_unchecked s ~at:i = '=' then
        if i + 1 >= len then
          Ok (Buffer.contents buf)
        else
          if String.get_unchecked s ~at:(i + 1) = '\n' then
            decode (i + 2)
          else
            if String.get_unchecked s ~at:(i + 1) = '\r' && i + 2 < len && String.get_unchecked s ~at:(i + 2) = '\n' then
              decode (i + 3)
            else
              if i + 2 < len then
                (
...
let get_decoded_content = fun part ->
  ...
  | None | Some SevenBit | Some EightBit | Some Binary -> Ok part.content
