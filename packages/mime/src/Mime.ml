...
let rec parse_content_type_string value =
  let rec parse_params str acc =
    ...
    | Some eq_idx ->
        let key =
          String.trim (String.sub str 0 eq_idx) |> String.lowercase_ascii
        in
        let rest =
          String.sub str (eq_idx + 1) (String.length str - eq_idx - 1)
        in
        let value_end =
          if String.length rest > 0 && rest.[0] = '"' then
            ...
            | Some semi_idx ->
                ...
                let remaining =
                  String.trim
                    (String.sub rest (semi_idx + 1)
                       (String.length rest - semi_idx - 1))
                in
                (value, remaining)
            | None -> (String.trim rest, "")
        in
  ...
  | Some idx ->
      ...
      let params_str =
        String.trim (String.sub value (idx + 1) (String.length value - idx - 1))
      in
...
and combine_rfc2231_params raw_params =
  ...
  Collections.HashMap.iter
    (fun name parts_cell ->
      let parts =
        Cell.get parts_cell |> List.sort (fun (a, _) (b, _) -> Int.compare a b)
      in
...
let parse_content_type value =
  ...
  | Some slash ->
      ...
      let sub =
        String.sub main_type (slash + 1) (String.length main_type - slash - 1)
      in
      { media_type = media; subtype = sub; parameters = params }

let parse_content_disposition value =
...
let parse_header (name, value) =
  let name_lower = String.lowercase_ascii name in
  match name_lower with
  | "content-type" -> ContentType (parse_content_type value)
  | "content-disposition" ->
      ContentDisposition (parse_content_disposition value)
  | "content-transfer-encoding" ->
      ContentTransferEncoding (parse_encoding_string value)
  | "content-id" -> ContentId value
  | "content-description" -> ContentDescription value
  | _ -> Other (name, value)

let parse_headers raw_headers = List.map parse_header raw_headers

let parse_part_headers_and_body content =
  ...
  let rec collect_headers acc lines =
    ...
    | line :: rest -> (
        ...
        if trimmed = "" then (List.rev acc, String.concat "\n" rest)
        else
          ...
          | Some idx ->
              ...
              let value =
                if value_start < String.length line then
                  String.trim
                    (String.sub line value_start
                       (String.length line - value_start))
                else ""
              in
...
and parse_multipart boundary body =
  ...
  let rec find_parts acc current_part lines =
    ...
    | [] ->
        let acc =
          match current_part with Some part -> part :: acc | None -> acc
        in
        List.rev acc
    | line :: rest -> (
        let trimmed = String.trim line in
  ...
  List.filter_map
    (fun part_lines ->
      if List.length part_lines = 0 then None
      else
        ...
        | Error _ ->
            Some
              (SinglePart
                 { headers = parse_headers raw_headers; content = body }))
    part_contents
...
let quoted_printable_decode s =
  ...
  let rec decode i =
    if i >= len then Ok (Buffer.contents buf)
      ...
      if i + 1 >= len then Ok (Buffer.contents buf)
      else if s.[i + 1] = '\n' then decode (i + 2)
