...
let rec parse_content_type_string = fun value ->
  let rec parse_params = fun str acc ->
...
and combine_rfc2231_params = fun raw_params ->
  ...
  let add_continuation = fun ~name ~num ~decoded ->
    let parts =
...
let parse_part_headers_and_body = fun content ->
  ...
  let rec collect_headers = fun acc lines ->
    match lines with
    | [] -> (List.rev acc, "")
    | line :: rest -> (
...
and parse_multipart = fun boundary body ->
  ...
  let lines = String.split_on_char '\n' body in
  let rec find_parts = fun acc current_part lines ->
    match lines with
    | [] ->
        let acc =
