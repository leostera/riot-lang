...
let test_rfc2231_continuation = fun _ctx ->
  let headers =
    [
      "Content-Disposition", "attachment; filename*0=\"Long\"; filename*1=\"Filename\"; \
         filename*2=\".pdf\"";
    ]
  in
  let body = "test content" in
    ...
    if List.length parts != 2 then
      ...
      | Some (MultiPart { parts = inner_parts; _ }) ->
          if List.length inner_parts = 2 then
            Ok ()
          else Error ("Expected 2 inner parts, got " ^ string_of_int (List.length inner_parts))
...
let test_content_type_parsing = fun _ctx ->
  ...
  | Ok (SinglePart part) -> (
    ...
    | Some ct ->
        ...
        else Error ("Expected text/html, got " ^ ct.media_type ^ "/" ^ ct.subtype)
