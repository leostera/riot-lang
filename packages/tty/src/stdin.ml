...
let read_utf8 = fun () -> Utf8_reader.read input_buffer ~read:(
  fun bytes ~offset ~len -> read_stdin_bytes (stdin ()) bytes ~offset ~len
)
...
let make_raw = fun () ->
  ...
  | Ok fd -> (
    ...
    | Error _ ->
        let _ = Platform.close fd in panic "failed to read terminal attributes"
    | Ok original_attrs ->
        let raw_attrs = Platform.make_raw_mode original_attrs in
        match Platform.set_attributes fd Platform.Now raw_attrs with
        | Error _ ->
            let _ = Platform.close fd in panic "failed to enable raw mode"
        | Ok () ->
            let size =
