...
let event_to_string =
  ...
  | `Mouse {
    ...
    modifiers
      ...
      ^ ")"
  | `Resize (w, h) ->
      "resize(" ^ Int.to_string w ^ "," ^ Int.to_string h ^ ")"
  | `Paste s ->
      let preview =
...
let parse_csi = fun seq ->
  ...
  if len < 3 then
    ...
    | "F" ->
        Some (make_key End)
    | "Z" ->
        Some (make_key ~mods:[ Shift ] BackTab)
    | "1~"
    | "7~" ->
        Some (make_key Home)
    | "4~"
          ...
          | [cb;cx;cy] ->
              ...
              let action =
                if is_release then
                  ...
                  Mouse_press
              in
              Some (`Mouse {button; action; x; y; modifiers = List.rev modifiers})
          | _ -> None
        with
        | _ -> None
      )
  ...
  | `Read s -> (
      ...
      if s = "\x1b[200~" then
        ...
        | ' ' ->
            make_key Space
        | '\x01' ->
            make_key ~mods:[ Ctrl ] (Char 'a')
        | '\x02' ->
            make_key ~mods:[ Ctrl ] (Char 'b')
        | '\x03' ->
            make_key ~mods:[ Ctrl ] (Char 'c')
        | '\x04' ->
            make_key ~mods:[ Ctrl ] (Char 'd')
        | '\x05' ->
            make_key ~mods:[ Ctrl ] (Char 'e')
        | '\x06' ->
            make_key ~mods:[ Ctrl ] (Char 'f')
        | '\x07' ->
            make_key ~mods:[ Ctrl ] (Char 'g')
        | '\x08' ->
            make_key Backspace
        | '\x0b' ->
            make_key ~mods:[ Ctrl ] (Char 'k')
        | '\x0c' ->
            make_key ~mods:[ Ctrl ] (Char 'l')
        | '\x0e' ->
            make_key ~mods:[ Ctrl ] (Char 'n')
        | '\x0f' ->
            make_key ~mods:[ Ctrl ] (Char 'o')
        | '\x10' ->
            make_key ~mods:[ Ctrl ] (Char 'p')
        | '\x11' ->
            make_key ~mods:[ Ctrl ] (Char 'q')
        | '\x12' ->
            make_key ~mods:[ Ctrl ] (Char 'r')
        | '\x13' ->
            make_key ~mods:[ Ctrl ] (Char 's')
        | '\x14' ->
            make_key ~mods:[ Ctrl ] (Char 't')
        | '\x15' ->
            make_key ~mods:[ Ctrl ] (Char 'u')
        | '\x16' ->
            make_key ~mods:[ Ctrl ] (Char 'v')
        | '\x17' ->
            make_key ~mods:[ Ctrl ] (Char 'w')
        | '\x18' ->
            make_key ~mods:[ Ctrl ] (Char 'x')
        | '\x19' ->
            make_key ~mods:[ Ctrl ] (Char 'y')
        | '\x1a' ->
            make_key ~mods:[ Ctrl ] (Char 'z')
        | c ->
            make_key (Char c)
