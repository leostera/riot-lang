...
type event = [`Key of key_event | `Text of string | `Mouse of mouse_event | `Resize of int * int | `Paste of string | `FocusGained | `FocusLost | `Unknown of string | `Retry | `End]
...
module Tokenizer = struct
  ...
  let is_csi_final = fun char ->
    let code = Char.to_int char in Int.(code >= 0x40 && code <= 0x7e)

  let rec find_next_escape = fun input ~from ->
    if from >= String.length input then
      from
    else
      if Char.equal (String.get_unchecked input ~at:from) '\x1b' then
        from
      else find_next_escape input ~from:(from + 1)

  let rec find_csi_end = fun input ~from ->
    if from >= String.length input then
      None
    else
      if is_csi_final (String.get_unchecked input ~at:from) then
        Some from
      else find_csi_end input ~from:(from + 1)
  ...
  let rec scan = fun input ~at acc ->
    if at >= String.length input then
      ...
      if not (Char.equal char '\x1b') then
        ...
        let text = String.sub input ~offset:at ~len:(next_escape - at) in scan input ~at:next_escape (Token.Text text :: acc)
      else
        if at + 1 >= String.length input then
          ...
          | '[' -> (
            ...
            | Some end_at ->
                ...
                let body = String.sub input ~offset:(at + 2) ~len:(end_at - at - 1) in scan input ~at:(end_at + 1) (Token.Control (Token.Csi { raw; body }) :: acc)
          )
          | 'O' ->
              if at + 2 >= String.length input then
                ...
                let body = String.sub input ~offset:(at + 2) ~len:1 in scan input ~at:(at + 3) (Token.Control (Token.Ss3 { raw; body }) :: acc)
          | ']' -> (
            match find_osc_end input ~from:(at + 2) with
            | None -> (reverse [] acc, String.sub input ~offset:at ~len:(String.length input - at))
            | Some (terminator_at, next_at) ->
                let raw = String.sub input ~offset:at ~len:(next_at - at) in
                let body = String.sub input ~offset:(at + 2) ~len:(terminator_at - at - 2) in scan input ~at:next_at (Token.Control (Token.Osc { raw; body }) :: acc)
          )
  ...
  let feed = fun state chunk ->
    let input =
      if String.is_empty state.pending then
        chunk
      else state.pending ^ chunk
    in
    let tokens, pending = scan input ~at:0 [] in ({ pending }, tokens)

  let flush = fun state ->
    if String.is_empty state.pending then
      (create (), [])
    else
      if String.equal state.pending "\x1b" then
        (create (), [ Token.Control Token.Escape ])
      else (create (), [ Token.Unknown state.pending ])
end

module Parser = struct
  type t = { tokenizer: Tokenizer.t; pending_escape: bool; in_paste: bool; paste_buffer: string }
  ...
  let append_paste = fun state fragment -> { state with paste_buffer = state.paste_buffer ^ fragment }

  let raw_of_control = function
    | Token.Escape -> "\x1b"
    | Token.Csi { raw; body = _ } | Token.Ss3 { raw; body = _ } | Token.Osc { raw; body = _ } -> raw
  ...
  let event_of_ascii_char = fun ~mods char ->
    ...
    if Char.equal char '\r' || Char.equal char '\n' then
      ...
      if Char.equal char '\t' then
        ...
        if Char.equal char '\x7f' || Char.equal char '\x08' then
          ...
          if Char.equal char ' ' then
            ...
            if code >= 1 && code <= 26 then
              let ctrl = Char.from_int_unchecked (code + 96) in make_key ~mods:(mods @ [ Ctrl ]) (Char ctrl)
            else
              if Char.equal char '\x1b' then
                ...
                else make_key ~mods (Char char)

  let split_first_rune = fun text ->
    match Unicode.Utf8.decode_rune text 0 with
    | Some (_rune, len) ->
        let head = String.sub text ~offset:0 ~len in
        let rest = String.sub text ~offset:len ~len:(String.length text - len) in (head, rest)
    | None ->
        let head = String.sub text ~offset:0 ~len:1 in
        let rest = String.sub text ~offset:1 ~len:(String.length text - 1) in (head, rest)
  ...
  let rec events_of_text = fun ?(mods = []) text acc ->
    if String.is_empty text then
      ...
      if String.length head = 1 then
        let char = String.get_unchecked head ~at:0 in events_of_text ~mods rest (event_of_ascii_char ~mods char :: acc)
      else events_of_text ~mods rest ((`Text head) :: acc)
  ...
  let handle_text = fun state text ->
    if state.in_paste then
      ...
      if state.pending_escape then
        ...
        let first_events =
          if String.length head = 1 then
            let char = String.get_unchecked head ~at:0 in [ event_of_ascii_char ~mods:[ Alt ] char ]
          else
            [
              `Unknown ("\x1b" ^ head);
            ]
        ...
        let rest_events = events_of_text rest [] in (state, first_events @ rest_events)
      else (state, events_of_text text [])
  ...
  let handle_control = fun state ->
    ...
    | Token.Escape ->
        if state.in_paste then
          ...
          else ({ state with pending_escape = true }, [])
    | control -> (
      ...
      | Token.Csi { raw = _; body } ->
          if state.in_paste && not (String.equal body "201~") then
            ...
            if String.equal body "200~" then
              ({ (clear_pending_escape state) with in_paste = true; paste_buffer = "" }, [])
            else
              if String.equal body "201~" then
                let event = `Paste state.paste_buffer in ({ (clear_pending_escape state) with in_paste = false; paste_buffer = "" }, [ event ])
              else
                let state = clear_pending_escape state in
                (
      ...
      | Token.Osc { raw = _; body = _ } ->
          if state.in_paste then
            (append_paste state raw, [])
          else (clear_pending_escape state, [
            `Unknown raw;
          ])
      | Token.Escape -> (state, [])
    )
  ...
  let rec handle_tokens = fun state tokens acc ->
    ...
    | token :: rest ->
        ...
        in
        handle_tokens state rest (Tokenizer.reverse acc events)
  ...
  let feed = fun state chunk ->
    ...
    let state = { state with tokenizer } in handle_tokens state tokens []
  ...
  let flush = fun state ->
    ...
    let state, flushed = flush_pending state in (state, events @ flushed)
end
