...
type size = Terminal.size = { rows: int; cols: int }
...
let to_string = fun t ->
  ...
  ^ "; mode=" ^ (
    ...
    | Immediate -> "immediate"
  ) ^ "; fd=" ^ Int.to_string (Platform.fd_to_int t.Terminal.fd) ^ " }"

let equal = fun left right -> Platform.fd_equal left.Terminal.fd right.Terminal.fd
