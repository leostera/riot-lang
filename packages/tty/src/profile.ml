...
let rec string_contains = fun ~sub_str ->
  function
  | "" -> sub_str = ""
  | s -> String.starts_with ~prefix:sub_str s || string_contains ~sub_str (String.sub s ~offset:1 ~len:(String.length s - 1))
...
let quantize_cube = fun component ->
  if component < 48 then
    ...
    if component < 115 then
      1
    else Int.min 5 ((component - 35) / 40)
...
let rgb_distance = fun (left_r, left_g, left_b) (right_r, right_g, right_b) ->
  ...
  let diff_b = left_b - right_b in (diff_r * diff_r) + (diff_g * diff_g) + (diff_b * diff_b)
...
let nearest_ansi_index = fun rgb ->
  let rec loop index best_index best_distance =
    if index >= Array.length ansi_palette then
      ...
      else loop (index + 1) best_index best_distance
  in
...
let rgb_of_ansi256 = fun index ->
  if index < 16 then
    ...
    if index < 232 then
      ...
      let blue = normalized mod 6 in (cube_level red, cube_level green, cube_level blue)
    else
      let shade = 8 + ((index - 232) * 10) in (shade, shade, shade)
...
let ansi256_of_rgb = fun (red, green, blue) ->
  if red = green && green = blue then
    ...
    let blue = quantize_cube blue in 16 + (36 * red) + (6 * green) + blue
...
let from_env = fun () ->
  ...
  let is_tmux =
    ...
    | _ -> false
  in
  let is_term sub_str = term |> Option.map ~fn:(string_contains ~sub_str) |> Option.unwrap_or ~default:false in
...
let convert = fun profile color ->
  ...
  | Color.RGB (red, green, blue), ANSI -> Color.ansi (nearest_ansi_index (red, green, blue))
  | Color.RGB (red, green, blue), ANSI256 -> Color.ansi256 (ansi256_of_rgb (red, green, blue))
