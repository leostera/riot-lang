...
let get () =
  ...
  | Ok (cols, rows) ->
      Log.debug ("[SIZE.GET] get_size(stdout) returned cols=" ^ Int.to_string cols ^ " rows=" ^ Int.to_string rows);
      Ok { rows; cols }
  | Error _ as e ->
      Log.debug "[SIZE.GET] Failed to get terminal size from stdout";
      e

let to_string { rows; cols } = "{ rows = " ^ Int.to_string rows ^ "; cols = " ^ Int.to_string cols ^ " }"
