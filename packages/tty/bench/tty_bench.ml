...
let mixed_trace_chunks =
  List.init
    ...
    ~fn:(fun index ->
      String.sub mixed_trace ~offset:index ~len:1)
...
let paste_trace_chunks =
  List.init
    ...
    ~fn:(fun index ->
      String.sub paste_trace ~offset:index ~len:1)

let plain_typing_chunks =
  List.init
    ~count:(String.length plain_typing_trace)
    ~fn:(fun index ->
      String.sub plain_typing_trace ~offset:index ~len:1)
...
let bench_style_short_fragments = fun () ->
  let _ = List.map styled_fragments ~fn:(fun fragment -> Tty.Style.styled style fragment) in
  ()
...
let benchmarks =
  Bench.[
    with_config
      ~config:{ iterations = 500; warmup = 50 }
      "tty cursor_position_seq"
      bench_cursor_position_seq;
    with_config
      ~config:{ iterations = 100; warmup = 10 }
      "tty strip ansi: 256 chunks"
      bench_strip_ansi;
    with_config
      ~config:{ iterations = 100; warmup = 10 }
      "tty width ansi: 256 chunks"
      bench_width_ansi;
    with_config
      ~config:{ iterations = 100; warmup = 10 }
      "tty width ascii: 256 chunks"
      bench_width_ascii;
    with_config
      ~config:{ iterations = 100; warmup = 10 }
      "tty strip osc+ansi: 256 chunks"
      bench_strip_osc_ansi;
    with_config
      ~config:{ iterations = 100; warmup = 10 }
      "tty width unicode: 256 chunks"
      bench_width_unicode;
    with_config ~config:{ iterations = 200; warmup = 20 } "tty style.styled" bench_style_styled;
    with_config
      ~config:{ iterations = 100; warmup = 10 }
      "tty style.short_fragments"
      bench_style_short_fragments;
    with_config
      ~config:{ iterations = 500; warmup = 50 }
      "tty input.parse_escape mouse"
      bench_input_parse_escape;
    with_config
      ~config:{ iterations = 500; warmup = 50 }
      "tty utf8_reader.ascii"
      bench_utf8_reader_ascii;
    with_config
      ~config:{ iterations = 500; warmup = 50 }
      "tty utf8_reader.emoji"
      bench_utf8_reader_emoji;
    with_config
      ~config:{ iterations = 200; warmup = 20 }
      "tty parser.chunked mixed trace"
      bench_parser_chunked_trace;
    with_config
      ~config:{ iterations = 500; warmup = 50 }
      "tty parser.whole mixed trace"
      bench_parser_whole_trace;
    with_config
      ~config:{ iterations = 200; warmup = 20 }
      "tty parser.chunked paste"
      bench_parser_chunked_paste;
    with_config
      ~config:{ iterations = 200; warmup = 20 }
      "tty parser.chunked plain typing"
      bench_parser_chunked_plain_typing;
  ]
