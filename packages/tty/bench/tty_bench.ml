...
let ansi_rich_text =
  let chunk = "\x1b[1;31merror\x1b[0m " in String.concat "" (List.init ~count:256 ~fn:(
    fun _ -> chunk
  ))

let ascii_text = String.concat "" (List.init ~count:256 ~fn:(
  fun _ -> "tty benchmark payload "
))

let osc_rich_text =
  let chunk = "\x1b]2;tty\x07\x1b[38;5;196mwarn\x1b[0m " in String.concat "" (List.init ~count:256 ~fn:(
    fun _ -> chunk
  ))

let styled_text = String.concat "" (List.init ~count:128 ~fn:(
  fun _ -> "tty benchmark payload "
))

let styled_fragments = List.init ~count:1_024 ~fn:(
  fun _ -> "tty"
)

let unicode_text = String.concat "" (List.init ~count:256 ~fn:(
  fun _ -> "Cafe\u{0301} 🙂 "
))

let mixed_trace = "\x1b[200~hello\x1b[201~\x1b[A\x1b[<0;10;20Mplain-text\x1b[I"

let mixed_trace_chunks = List.init ~count:(String.length mixed_trace) ~fn:(
  fun index -> String.sub mixed_trace ~offset:index ~len:1
)
...
let paste_trace_chunks = List.init ~count:(String.length paste_trace) ~fn:(
  fun index -> String.sub paste_trace ~offset:index ~len:1
)

let plain_typing_chunks = List.init ~count:(String.length plain_typing_trace) ~fn:(
  fun index -> String.sub plain_typing_trace ~offset:index ~len:1
)

let style = Tty.Style.default |> Tty.Style.bold |> Tty.Style.underline |> Tty.Style.fg (Tty.Color.make "#FF8800") |> Tty.Style.bg (Tty.Color.ansi 4)
...
let bench_style_short_fragments = fun () ->
  let _ = List.map styled_fragments ~fn:(
    fun fragment -> Tty.Style.styled style fragment
  ) in ()
  ...
  fun bytes ~offset ~len ->
    ...
    | chunk :: rest ->
        ...
        if Int.equal count (String.length chunk) then
          remaining := rest
        else remaining := String.sub chunk ~offset:count ~len:(String.length chunk - count) :: rest;
        `Ok count
...
let bench_parser_chunked_trace = fun () ->
  let rec loop parser = function
    ...
    | chunk :: rest ->
        let parser, _ = Tty.Input.Parser.feed parser chunk in loop parser rest
  in
