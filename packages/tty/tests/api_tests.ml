...
let test_set_raw_is_idempotent = fun _ctx ->
  ...
  | Ok tty ->
      ...
      if Tty.mode tty = Tty.Immediate then
        Ok ()
      else Error "Expected set_raw to be idempotent"
...
let test_set_line_buffered_is_idempotent = fun _ctx ->
  ...
  | Ok tty ->
      ...
      if Tty.mode tty = Tty.LineBuffered then
        Ok ()
      else Error "Expected set_line_buffered to be idempotent"
  ...
  | Ok tty ->
      ...
      if Tty.mode tty != Tty.LineBuffered then
        ...
        (
          ...
          if Tty.mode tty = Tty.Immediate then
            Ok ()
          else Error "Expected resume to restore immediate mode after suspend"
        )
...
let test_stdin_stdout_stderr_fds = fun _ctx ->
  ...
  let _stderr = Tty.stderr_fd () in Ok ()
