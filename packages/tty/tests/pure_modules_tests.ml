...
let test_color_make_invalid_length_rejected = fun _ctx ->
  try
    let _ = Tty.Color.make "#12345" in Error "Expected invalid hex length to be rejected"
  with
...
let test_color_make_invalid_hex_rejected = fun _ctx ->
  try
    let _ = Tty.Color.make "#GG0000" in Error "Expected invalid hex digit to be rejected"
  with
...
let test_color_ansi_rejects_negative = fun _ctx ->
  try
    let _ = Tty.Color.ansi (-1) in Error "Expected ansi -1 to be rejected"
  with
...
let test_color_ansi_range_validation = fun _ctx ->
  try
    let _ = Tty.Color.ansi 16 in Error "Expected ansi 16 to be rejected"
  with
...
let test_color_ansi256_range_validation = fun _ctx ->
  try
    let _ = Tty.Color.ansi256 256 in Error "Expected ansi256 256 to be rejected"
  with
...
let test_color_ansi256_rejects_negative = fun _ctx ->
  try
    let _ = Tty.Color.ansi256 (-1) in Error "Expected ansi256 -1 to be rejected"
  with
...
let test_color_to_escape_seq = fun _ctx ->
  ...
  if rgb = "38;2;1;2;3" && ansi = "44" && ansi256 = "38;5;196" then
    Ok ()
  else Error ("Unexpected color escape sequences: rgb=" ^ rgb ^ " ansi=" ^ ansi ^ " ansi256=" ^ ansi256)
...
let test_color_to_escape_seq_bright_ansi = fun _ctx ->
  ...
  if fg = "91" && bg = "104" then
    Ok ()
  else Error ("Unexpected bright ANSI escape sequences: fg=" ^ fg ^ " bg=" ^ bg)
...
let test_color_to_escape_seq_basic_ansi = fun _ctx ->
  ...
  if fg = "31" && bg = "44" then
    Ok ()
  else Error ("Unexpected basic ANSI escape sequences: fg=" ^ fg ^ " bg=" ^ bg)
...
let test_color_to_escape_seq_rgb_modes = fun _ctx ->
  ...
  if fg = "38;2;1;2;3" && bg = "48;2;1;2;3" then
    Ok ()
  else Error ("Unexpected RGB escape sequences: fg=" ^ fg ^ " bg=" ^ bg)
...
let test_style_styled_wraps_text = fun _ctx ->
  ...
  if seq = "1;4;38;2;255;0;0;44" && styled = "\x1b[1;4;38;2;255;0;0;44mhi\x1b[0m" then
    Ok ()
  else Error ("Unexpected styled rendering: seq=" ^ seq ^ " styled=" ^ styled)
...
let test_style_attribute_order_is_deterministic = fun _ctx ->
  ...
  if seq = "1;3;4" then
    Ok ()
  else Error ("Expected deterministic attribute order, got " ^ seq)
...
let test_style_fg_no_color_is_noop = fun _ctx ->
  ...
  if String.equal (Tty.Style.to_escape_seq style) "" && String.equal (Tty.Style.styled style "hello") "hello" then
    Ok ()
  else Error "Expected no_color foreground style to be a no-op"
...
let test_style_ansi_foreground_render = fun _ctx ->
  ...
  if styled = "\x1b[31mhi\x1b[0m" then
    Ok ()
  else Error ("Expected ANSI foreground rendering, got " ^ styled)
...
let test_style_ansi_background_render = fun _ctx ->
  ...
  if styled = "\x1b[44mhi\x1b[0m" then
    Ok ()
  else Error ("Expected ANSI background rendering, got " ^ styled)

let test_style_ansi256_render = fun _ctx ->
  let style = Tty.Style.default |> Tty.Style.fg (Tty.Color.ansi256 196) |> Tty.Style.bg (Tty.Color.ansi256 46) in
  let styled = Tty.Style.styled style "hi" in
  if styled = "\x1b[38;5;196;48;5;46mhi\x1b[0m" then
    Ok ()
  else Error ("Expected ANSI256 rendering, got " ^ styled)
...
let test_style_empty_string_policy = fun _ctx ->
  ...
  if styled = "\x1b[1m\x1b[0m" then
    Ok ()
  else Error ("Expected styled empty string to preserve wrapper policy, got " ^ styled)
...
let test_style_nested_policy = fun _ctx ->
  ...
  if outer = "\x1b[1mbefore \x1b[31minner\x1b[0m\x1b[0m" then
    Ok ()
  else Error ("Unexpected nested styling policy: " ^ outer)
...
let test_style_preserves_unicode_width = fun _ctx ->
  ...
  if Int.equal (Tty.Escape_seq.width styled) (String.width plain) then
    Ok ()
  else Error "Expected styling to preserve displayed width for unicode text"
...
let test_input_parse_focus_events = fun _ctx ->
  ...
  | Some focus_in, Some focus_out when Tty.Input.event_to_string focus_in = "focus-gained" && Tty.Input.event_to_string focus_out = "focus-lost" -> Ok ()
...
let test_input_parse_mouse_press = fun _ctx ->
  ...
  | Some (`Mouse { button = Tty.Input.Left; action = Tty.Input.Mouse_press; x = 10; y = 20; modifiers = [] }) -> Ok ()
...
let test_input_parse_mouse_release_with_modifiers = fun _ctx ->
  ...
  | Some (`Mouse { button = Tty.Input.Left; action = Tty.Input.Mouse_release; x = 7; y = 9; modifiers = [ Tty.Input.Shift; Tty.Input.Ctrl ] }) -> Ok ()
...
let test_input_parse_home_end_variants = fun _ctx ->
  match Tty.Input.parse_escape "\x1b[H", Tty.Input.parse_escape "\x1b[F", Tty.Input.parse_escape "\x1b[1~", Tty.Input.parse_escape "\x1b[4~" with
  | Some home, Some ending, Some legacy_home, Some legacy_end when Tty.Input.event_to_string home = "home" && Tty.Input.event_to_string ending = "end" && Tty.Input.event_to_string legacy_home = "home" && Tty.Input.event_to_string legacy_end = "end" -> Ok ()
  | _ -> Error "Expected home/end variants to parse"

let test_input_parse_insert_delete_paging = fun _ctx ->
  match Tty.Input.parse_escape "\x1b[2~", Tty.Input.parse_escape "\x1b[3~", Tty.Input.parse_escape "\x1b[5~", Tty.Input.parse_escape "\x1b[6~" with
  | Some insert, Some delete, Some page_up, Some page_down when Tty.Input.event_to_string insert = "insert" && Tty.Input.event_to_string delete = "delete" && Tty.Input.event_to_string page_up = "pageup" && Tty.Input.event_to_string page_down = "pagedown" -> Ok ()
...
let test_input_event_to_string_text = fun _ctx ->
  ...
  if rendered = "text(\"\\240\\159\\153\\130\")" then
    Ok ()
  else Error ("Expected text event rendering, got " ^ rendered)
...
let test_input_event_to_string_repeat = fun _ctx ->
  ...
  if rendered = "alt+x:repeat" then
    Ok ()
  else Error ("Expected alt+x:repeat, got " ^ rendered)
...
let test_tty_make_self_equal = fun _ctx ->
  ...
  | Ok tty ->
      if Tty.equal tty tty then
        Ok ()
      else Error "Expected a tty value to be equal to itself"
  | Error _ -> Ok ()
...
let test_tty_to_string_has_prefix = fun _ctx ->
  ...
  | Ok tty ->
      ...
      if String.starts_with ~prefix:"TTY { size=" rendered then
        Ok ()
      else Error ("Expected tty string representation, got " ^ rendered)
  | Error _ -> Ok ()
...
let test_size_to_string = fun _ctx ->
  ...
  if rendered = "{ rows = 20; cols = 80 }" then
    Ok ()
  else Error ("Unexpected size rendering: " ^ rendered)
