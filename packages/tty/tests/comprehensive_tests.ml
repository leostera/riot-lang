...
let test_mouse_sequences = fun _ctx ->
  ...
  in
  let errors = List.filter_map tests ~fn:(
...
let test_bracketed_paste = fun _ctx ->
  ...
  if enable = "\x1b[?2004h" && disable = "\x1b[?2004l" && start = "\x1b[200~" && end_paste = "\x1b[201~" then
    Ok ()
...
let test_color_sequences = fun _ctx ->
  ...
  if fg = "\x1b]10;255;0;0\x07" && bg = "\x1b]11;0;255;0\x07" && cursor = "\x1b]12;0;0;255\x07" && title = "\x1b]2;Test Title\x07" then
    Ok ()
...
let test_strip_osc = fun _ctx ->
  ...
  if output = "hello" then
    Ok ()
  else Error ("strip osc: expected 'hello', got '" ^ output ^ "'")
