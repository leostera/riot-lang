...
let test_tokenizer_parses_csi = fun _ctx ->
  ...
  match tokens with
  | [ Tty.Input.Token.Control (Tty.Input.Token.Csi { raw; body }) ] when raw = "\x1b[A"
  && body = "A" -> Ok ()
...
let tests =
  Test.[
    ...
    case
      "tokenizer_flushes_incomplete_osc_as_unknown"
      test_tokenizer_flushes_incomplete_osc_as_unknown;
