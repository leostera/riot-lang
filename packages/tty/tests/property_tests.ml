...
let assert_property = fun name property ->
  ...
  | Property.Error { exception_; backtrace } -> Error (name ^ " raised " ^ Exception.to_string exception_ ^ "\n" ^ backtrace)
  | Property.Assumption_violated -> Error (name ^ " exhausted assumptions")

let byte_chunks = fun value -> List.init ~count:(String.length value) ~fn:(
  fun index -> String.sub value ~offset:index ~len:1
)
...
let rec string_lists_equal = fun left right ->
  ...
  | [], [] -> true
  | left :: left_rest, right :: right_rest -> String.equal left right && string_lists_equal left_rest right_rest
  | _ -> false

let parse_whole = fun trace ->
  let parser, events = Tty.Input.Parser.feed (Tty.Input.Parser.create ()) trace in
  let _, flushed = Tty.Input.Parser.flush parser in event_strings (events @ flushed)
...
let parse_bytewise = fun trace ->
  ...
  let rec loop parser chunks =
    ...
    | [] ->
        ...
        event_strings flushed |> List.for_each ~fn:(
          fun event -> Vector.push events ~value:event
        );
        Vector.to_array events |> Array.to_list
    | chunk :: rest ->
        let parser, chunk_events = Tty.Input.Parser.feed parser chunk in
        event_strings chunk_events |> List.for_each ~fn:(
          fun event -> Vector.push events ~value:event
        );
        loop parser rest
  in
...
let short_printable = Arbitrary.make ~print:String.escaped Generator.(string_size (int_range 0 24) char_printable)
...
let ansiish_string =
  ...
  in
  Arbitrary.make ~print:String.escaped Generator.(map (String.concat "") (list_size (int_range 0 12) (one_of [ text_fragment; control_fragment ])))
...
let parser_trace =
  ...
  in
  Arbitrary.make ~print:String.escaped Generator.(map (String.concat "") (list_size (int_range 0 10) (one_of [ text_fragment; token_fragment ])))

let unknown_trace =
  let unknown_fragment = Generator.one_of [ Generator.return "\x1b[999~"; Generator.return "\x1b[?999h"; Generator.return "\x1b[>999u" ] in Arbitrary.make ~print:(
    fun fragments -> String.concat "|" (List.map fragments ~fn:String.escaped)
...
let strip_is_idempotent =
  Property.for_all ansiish_string
    (
      fun value ->
        let stripped = Tty.Escape_seq.strip value in String.equal (Tty.Escape_seq.strip stripped) stripped
    )
...
let strip_removes_escape_bytes =
  Property.for_all ansiish_string
    (
      fun value ->
        let stripped = Tty.Escape_seq.strip value in not (String.contains stripped "\x1b")
    )
...
let styled_width_matches_plain_width =
  let style = Tty.Style.default |> Tty.Style.bold |> Tty.Style.underline |> Tty.Style.fg (Tty.Color.make "#FF8800") in
  Property.for_all short_printable
    (
      fun value -> Int.equal (Tty.Escape_seq.width (Tty.Style.styled style value)) (String.width value)
    )
  ...
  Property.for_all Arbitrary.(triple int int int)
    (
      fun (red, green, blue) ->
        ...
        | Tty.Color.RGB (red, green, blue) -> Int.(red >= 0 && red <= 255 && green >= 0 && green <= 255 && blue >= 0 && blue <= 255)
        | _ -> false
    )
...
let styled_non_default_wraps_with_escape_and_reset =
  let style = Tty.Style.default |> Tty.Style.bold |> Tty.Style.fg (Tty.Color.ansi 1) in
  Property.for_all short_printable
    (
      fun value ->
        let styled = Tty.Style.styled style value in String.starts_with ~prefix:"\x1b[" styled && String.ends_with ~suffix:"\x1b[0m" styled
    )

let styled_does_not_inject_newline_or_nul =
  let style = Tty.Style.default |> Tty.Style.underline |> Tty.Style.fg (Tty.Color.make "#FF8800") in
  Property.for_all short_printable
    (
      fun value ->
        ...
        let styled_has_nul = String.contains styled "\000" in (not styled_has_newline || input_has_newline) && (not styled_has_nul || input_has_nul)
    )
...
let ansi_foreground_is_not_malformed =
  Property.for_all ansi_index
    (
      fun index ->
        let seq = Tty.Color.to_escape_seq ~mode:`fg (Tty.Color.ansi index) in not (String.starts_with ~prefix:"38;" seq)
    )
...
let ansi_background_is_not_malformed =
  Property.for_all ansi_index
    (
      fun index ->
        let seq = Tty.Color.to_escape_seq ~mode:`bg (Tty.Color.ansi index) in not (String.starts_with ~prefix:"48;" seq)
    )
