...
let ansiish_string =
  ...
  let control_fragment = Generator.one_of
    [
...
let parser_trace =
  ...
  let token_fragment = Generator.one_of
    [
      Generator.return "\x1b[A";
...
let unknown_trace =
  let unknown_fragment = Generator.one_of
    [ Generator.return "\x1b[999~"; Generator.return "\x1b[?999h"; Generator.return "\x1b[>999u"; ] in
  Arbitrary.make
    ~print:(fun fragments ->
      String.concat "|" (List.map fragments ~fn:String.escaped))
...
let style_default_is_identity =
  Property.for_all Arbitrary.string
    (fun value ->
      String.equal (Tty.Style.styled Tty.Style.default value) value)

let styled_width_matches_plain_width =
  let style = Tty.Style.default
  |> Tty.Style.bold
  |> Tty.Style.underline
  |> Tty.Style.fg (Tty.Color.make "#FF8800") in
...
let color_of_rgb_clamps_components =
  Property.for_all Arbitrary.(triple int int int)
    (fun (red, green, blue) ->
      ...
      | Tty.Color.RGB (red, green, blue) -> Int.(red >= 0
      && red <= 255
      && green >= 0
      && green <= 255
      && blue >= 0
