...
let apply_env_value = fun name value_opt ->
  ...
  | Some value ->
      let _ = Env.set ~var:name ~value in Ok ()
  | None ->
      let _ = Env.remove ~var:name in Ok ()
...
let with_env = fun bindings fn ->
  ...
  let rec apply = function
    | [] -> Ok ()
    | (name, value_opt) :: rest ->
        let* () = apply_env_value name value_opt in apply rest
  in
  ...
  match fn () with
  | Ok value ->
      let* () = restore () in Ok value
  | Error error ->
      let _ = restore () in Error error
    ...
    (
      fun () ->
        ...
        if before = still_default && dynamic = Tty.Color.no_color then
          Ok ()
        else Error "Expected default profile to stay stable while from_env follows the current environment"
    )
