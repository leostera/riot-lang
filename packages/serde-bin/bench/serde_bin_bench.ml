...
let primitive_decode = De.record_mut ~fields:primitive_fields ~create:(
  fun (): primitive_builder ->
    {
      ready = None;
      count = None;
      small = None;
      ...
      Ser.fields
        [
          Ser.field "ready" Ser.bool
            (
              fun (value: primitive_record) -> value.ready
            );
          Ser.field "count" Ser.int
            (
              fun (value: primitive_record) -> value.count
            );
          Ser.field "small" Ser.int32
            (
              fun (value: primitive_record) -> value.small
            );
          Ser.field "big" Ser.int64
            (
              fun (value: primitive_record) -> value.big
            );
          Ser.field "ratio" Ser.float
            (
              fun (value: primitive_record) -> value.ratio
            );
          Ser.field "label" Ser.string
            (
              fun (value: primitive_record) -> value.label
            );
          Ser.field "alias" (Ser.option Ser.string)
            (
              fun (value: primitive_record) -> value.alias
            );
          Ser.field "mode" mode_encode
            (
              fun (value: primitive_record) -> value.mode
            );
          Ser.field "unit_value" unit_encode
            (
              fun (value: primitive_record) -> value.unit_value
            );
        ]
    )

let batch_decode = De.record_mut ~fields:batch_fields ~create:(
  fun (): batch_builder ->
    {
      batch_id = None;
      name = None;
...
let batch_encode =
  Ser.record
    (
      Ser.fields
        [
          Ser.field "batch_id" Ser.int
            (
              fun (value: batch) -> value.batch_id
            );
          Ser.field "name" Ser.string
            (
              fun (value: batch) -> value.name
            );
          Ser.field "items" (Ser.list primitive_encode)
            (
              fun (value: batch) -> value.items
            );
          Ser.field "mirrors" (array_encode primitive_encode)
            (
              fun (value: batch) -> value.mirrors
            );
          Ser.field "featured" primitive_encode
            (
              fun (value: batch) -> value.featured
            );
          Ser.field "status" mode_encode
            (
              fun (value: batch) -> value.status
            );
        ]
    )
    ...
    (
      Ser.fields
        [
          ...
          Ser.field "batches" (Ser.list batch_encode)
            (
              fun (value: dataset) -> value.batches
            );
          Ser.field "mirrors" (array_encode batch_encode)
            (
              fun (value: dataset) -> value.mirrors
            );
          Ser.field "flags" (Ser.list mode_encode)
            (
              fun (value: dataset) -> value.flags
            );
          Ser.field "primary" mode_encode
            (
              fun (value: dataset) -> value.primary
            );
        ]
    )
...
let build_batch = fun batch_index ->
  ...
  for item_index = 0 to 11 do Vector.push items ~value:(build_primitive_record batch_index item_index) done;
  let mirrors = Array.init ~count:6 ~fn:(
    fun mirror_index -> build_primitive_record batch_index (mirror_index + 32)
  ) in ({
...
let build_dataset = fun spec ->
  ...
  for batch_index = 0 to spec.batch_count - 1 do Vector.push batches ~value:(build_batch batch_index) done;
  let mirrors = Array.init ~count:spec.mirror_count ~fn:(
    fun index -> build_batch (index + 512)
  ) in
  let flags = Vector.with_capacity ~size:64 in
  for index = 0 to 63 do Vector.push flags ~value:(build_mode (index + 2_000)) done;
  ({
    version = 2;
    source = spec.source;
    batches;
...
let bench_encode_serde_in_memory = fun fixture () -> ignore (Serde_bin.to_string dataset_encode fixture.dataset)
...
let bench_decode_serde_in_memory = fun fixture () -> ignore (Serde_bin.of_string dataset_decode fixture.serde_bytes)

let bench_decode_serde_reader = fun fixture () -> ignore (Serde_bin.of_reader dataset_decode (String.to_reader ~chunk_size:io_chunk_size fixture.serde_bytes))
...
let benchmark_suite = fun fixture ->
  ...
  let marshal_size = human_size (String.length fixture.marshal_bytes) in Bench.[
    with_config ~config:bench_config ("serde-bin encode in-memory " ^ fixture.label ^ " dataset (" ^ serde_size ^ ")") (bench_encode_serde_in_memory fixture);
    with_config ~config:bench_config ("serde-bin encode writer " ^ fixture.label ^ " dataset (" ^ serde_size ^ ")") (bench_encode_serde_writer fixture);
    with_config ~config:bench_config ("Stdlib.Marshal encode " ^ fixture.label ^ " dataset (" ^ marshal_size ^ ")") (bench_encode_marshal fixture);
    with_config ~config:bench_config ("serde-bin decode in-memory " ^ fixture.label ^ " dataset (" ^ serde_size ^ ")") (bench_decode_serde_in_memory fixture);
    with_config ~config:bench_config ("serde-bin decode reader " ^ fixture.label ^ " dataset (" ^ serde_size ^ ")") (bench_decode_serde_reader fixture);
    with_config ~config:bench_config ("Stdlib.Marshal decode " ^ fixture.label ^ " dataset (" ^ marshal_size ^ ")") (bench_decode_marshal fixture);
  ]

let main ~args =
  let small_fixture = build_fixture small_fixture_spec in
  let large_fixture = build_fixture large_fixture_spec in
  let benchmarks = benchmark_suite small_fixture @ benchmark_suite large_fixture in Bench.Cli.main ~name:"serde-bin benchmarks" ~benchmarks ~args
