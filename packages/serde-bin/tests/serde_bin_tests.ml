    ...
    (
      Ser.fields
        [
          ...
          Ser.field "score" Ser.int64
            (
              fun (value: person) -> value.score
            );
        ]
    )
...
let equal_person = fun (left: person) (right: person) -> String.equal left.name right.name && Int.equal left.age right.age && Bool.equal left.active right.active && vec_to_list left.tags = vec_to_list right.tags && left.nickname = right.nickname && left.pet = right.pet && Int64.equal left.score right.score
...
let test_int32_uses_raw_little_endian_bytes = fun _ctx ->
  ...
  | Ok encoded -> expect_equal ~expected:[
    4;
    3;
    2;
    1;
  ] ~actual:(byte_values encoded) ~message:"expected int32 to use raw little-endian bytes"
...
let test_int_uses_raw_little_endian_bytes = fun _ctx ->
  ...
  | Ok encoded -> expect_equal ~expected:[
    ...
    255;
    255;
  ] ~actual:(byte_values encoded) ~message:"expected int to use raw 8-byte little-endian bytes"
...
let test_int64_uses_raw_little_endian_bytes = fun _ctx ->
  ...
  | Ok encoded -> expect_equal ~expected:[
    ...
    2;
    1;
  ] ~actual:(byte_values encoded) ~message:"expected int64 to use raw little-endian bytes"
...
let test_float_uses_raw_ieee754_bytes = fun _ctx ->
  ...
  | Ok encoded -> expect_equal ~expected:[
    ...
    240;
    63;
  ] ~actual:(byte_values encoded) ~message:"expected float to use raw IEEE754 little-endian bytes"
...
let test_writes_to_writer = fun _ctx ->
  ...
  | Ok () -> expect_equal ~expected:[
    ...
    111;
    117;
  ] ~actual:(byte_values (IO.Buffer.contents buffer)) ~message:"expected serde-bin to emit compact variant bytes"
...
let test_encode_into_bytes = fun _ctx ->
  ...
  | Ok written ->
      let actual = IO.Bytes.sub_string dst ~offset:0 ~len:written |> byte_values in expect_equal ~expected:[
        1;
        8;
        0;
        0;
        ...
        111;
        117;
      ] ~actual ~message:"expected encode_into_bytes to write the compact variant payload"
