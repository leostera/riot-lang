...
let roundtrip_io = fun encode decode equal value ->
  ...
  | Ok () -> (
    match Serde_bin.of_reader decode (String.to_reader ~chunk_size:io_chunk_size (IO.Buffer.contents buffer)) with
    | Ok decoded -> equal decoded value
...
let bool_roundtrip_prop = run_property "serde-bin property bool roundtrips" Arbitrary.bool (roundtrip_in_memory Ser.bool De.bool Bool.equal)

let int_roundtrip_prop = run_property "serde-bin property int roundtrips" Arbitrary.int (roundtrip_in_memory Ser.int De.int Int.equal)

let int32_roundtrip_prop = run_property "serde-bin property int32 roundtrips" Arbitrary.int32 (roundtrip_in_memory Ser.int32 De.int32 Int32.equal)

let int64_roundtrip_prop = run_property "serde-bin property int64 roundtrips" Arbitrary.int64 (roundtrip_in_memory Ser.int64 De.int64 Int64.equal)

let float_roundtrip_prop = run_property "serde-bin property float roundtrips" finite_float_arb (roundtrip_in_memory Ser.float De.float Float.equal)

let string_roundtrip_prop = run_property "serde-bin property string roundtrips" Arbitrary.string (roundtrip_in_memory Ser.string De.string String.equal)

let option_string_roundtrip_prop = run_property "serde-bin property option string roundtrips" Arbitrary.(option string) (roundtrip_in_memory (Ser.option Ser.string) (De.option De.string) (=))

let string_list_roundtrip_prop = run_property ~examples:composite_examples "serde-bin property string list roundtrips" Arbitrary.(vector string) (roundtrip_in_memory (Ser.list Ser.string) (De.list De.string) equal_string_vec)

let int_array_roundtrip_prop = run_property ~examples:composite_examples "serde-bin property int array roundtrips" Arbitrary.(array int) (roundtrip_in_memory (Ser.array Ser.int) (De.array De.int) (=))

let mode_roundtrip_prop = run_property ~examples:composite_examples "serde-bin property mode roundtrips" mode_arb (roundtrip_in_memory mode_encode mode_decode equal_mode)

let pet_roundtrip_prop = run_property ~examples:composite_examples "serde-bin property pet roundtrips" pet_arb (roundtrip_in_memory pet_encode pet_decode equal_pet)

let sample_roundtrip_prop = run_property ~examples:composite_examples "serde-bin property sample roundtrips" sample_arb (roundtrip_in_memory sample_encode sample_decode equal_sample)

let sample_io_roundtrip_prop = run_property ~examples:composite_examples "serde-bin property sample roundtrips over io" sample_arb (roundtrip_io sample_encode sample_decode equal_sample)
