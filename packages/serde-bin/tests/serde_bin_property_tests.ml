...
let roundtrip_io = fun encode decode equal value ->
  ...
  | Ok () ->
      (
        match
          Serde_bin.of_reader decode (String.to_reader ~chunk_size:io_chunk_size (IO.Buffer.contents buffer))
        with
        | Ok decoded -> equal decoded value
...
let () =
  Actors.run
    ~main:(fun ~args -> Test.Cli.main ~name:"serde_bin_property_tests" ~tests ~args)
    ~args:Env.args
    ()
