...
type state = { target: encode_target; output: IO.Buffer.t; scratch: bytes }
...
let raise_length_out_of_range = fun kind value -> raise (Serde.Encode_error (`Msg ("serde-bin " ^ kind ^ " length is out of range: " ^ Int.to_string value)))
...
let rec list_backend: 'value. state -> 'value Serde.Ser.t -> 'value vec -> unit = fun state encode values ->
  ...
  Vector.for_each values ~fn:(
    fun value -> encode.run backend state value
  )
and array_backend: 'value. state -> 'value Serde.Ser.t -> 'value array -> unit = fun state encode values ->
  ...
  for index = 0 to len - 1 do encode.run backend state (Array.get_unchecked values ~at:index) done
    ...
    if Int.equal index (Array.length cases) then
      ...
      | Serde.Ser.Unit (_tag, matches) ->
          if matches value then
            ...
            else write_uint32_le state (encode_u32 "variant" index)
          else loop (index + 1)
...
and backend: state Serde.Ser.backend = {
  ...
  float = (
    fun state value -> write_int64_le state (Int64.bits_of_float value)
  );
  null = (
    fun _state -> ()
  );
...
let rec size_list_backend: 'value. size_state -> 'value Serde.Ser.t -> 'value vec -> unit = fun state encode values ->
  ...
  Vector.for_each values ~fn:(
    fun value -> encode.run size_backend state value
  )
and size_array_backend: 'value. size_state -> 'value Serde.Ser.t -> 'value array -> unit = fun state encode values ->
  state.bytes_written <- state.bytes_written + 4;
  for index = 0 to Array.length values - 1 do encode.run size_backend state (Array.get_unchecked values ~at:index) done
...
and size_backend: size_state Serde.Ser.backend = {
  bool = (
    fun state _value -> state.bytes_written <- state.bytes_written + 1
  );
  string = (
    fun state value -> state.bytes_written <- state.bytes_written + 4 + String.length value
  );
  int = (
    fun state _value -> state.bytes_written <- state.bytes_written + 8
  );
  int32 = (
    fun state _value -> state.bytes_written <- state.bytes_written + 4
  );
  int64 = (
    fun state _value -> state.bytes_written <- state.bytes_written + 8
  );
...
let encode_into_bytes = fun encode dst value ->
  let state = { target = Bytes_target { dst; pos = 0 }; output = IO.Buffer.create ~size:0; scratch = IO.Bytes.create ~size:8 } in
  match Serde.Ser.run encode backend state value with
  | Error err -> Error err
  | Ok () -> (
    match state.target with
    | Bytes_target target -> Ok target.pos
    | Buffer_target | Writer_target _ -> panic "serde-bin encode_into_bytes: expected bytes target"
  )
...
let to_writer = fun encode writer value ->
  let state = { target = Writer_target writer; output = IO.Buffer.create ~size:4_096; scratch = IO.Bytes.create ~size:8 } in
  match Serde.Ser.run encode backend state value with
  | Error err -> Error err
  | Ok () -> (
    try
