...
let parse_file = fun path ->
  let source = Fs.read path |> Result.expect ~msg:"fixture file should exist" in Krasny.parse_source ~filename:path source
...
let buffer_writer = fun buffer ->
  let module Write = struct
    ...
    let write = fun buffer ~from ->
      ...
      let _ = IO.Buffer.append_slice buffer (IO.Buffer.readable from) |> Result.expect ~msg:"failed to append writer buffer" in Ok len
...
let capture_write = fun result ->
  ...
  | Error (Krasny.Format_failed err) -> panic ("stream_format should render into the supplied writer: " ^ Krasny.format_error_to_string err)
  | Error (Krasny.Write_failed err) -> panic ("stream_format should write into the supplied writer: " ^ IO.error_message err)
...
let assert_idempotent = fun ~source ~msg ->
  ...
  let second = parse_ml first |> Krasny.format |> Result.expect ~msg:"formatted output should reformat" in Test.assert_equal ~expected:first ~actual:second

let assert_formatted_ml_snapshot = fun ~ctx ~msg source ->
  let actual = parse_ml source |> Krasny.format |> Result.expect ~msg in Test.Snapshot.assert_text ~ctx ~actual

let assert_formatted_mli_snapshot = fun ~ctx ~msg source ->
  let actual = parse_mli source |> Krasny.format |> Result.expect ~msg in Test.Snapshot.assert_text ~ctx ~actual
...
let assert_roundtrip_hash = fun path ->
  ...
  let reparsed_hash = Krasny.syntax_hash reparsed in Test.assert_equal ~expected:original_hash ~actual:reparsed_hash
...
let tests =
  [
    ...
    Test.case "syntax hash normalizes formatter-safe punctuation"
      (
        fun _ctx ->
          let expected = parse_ml {ocaml|type row={x:int;y:int}
let check=pos+pattern_len>String.length str
|ocaml} |> Krasny.syntax_hash in
          let actual = parse_ml {ocaml|type row={x:int;y:int;}
let check=(pos+pattern_len)>(String.length str)
|ocaml} |> Krasny.syntax_hash in
...
|ocaml} in
          ...
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|val id: 'a -> 'a
...
|ocaml} in
          let parsed = parse_mli source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|module ANSI : sig
  (**
     Convert an ANSI palette entry to RGB.
...
|ocaml} in
          let parsed = parse_mli source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|module Blink : sig
  (**
     # SSE
...
|ocaml} in
          ...
          Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let test = fun _ctx ->
  (* keep *)
  let value = 1 in value
|ocaml}
      );
...
|ocaml} in
          ...
          Test.Snapshot.assert_inline_text ~ctx ~actual:first ~expected:{ocaml|open Std
...
|ocaml} in
          let parsed = parse_mli source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|module S : sig
  val x: int
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|module Make (Order: Std_order.Ordered) = struct
  let empty = Empty
end
|ocaml}
      );
...
|ocaml} in
          let parsed = parse_mli source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|type 'a parse_result =
...
|ocaml} in
          let parsed = parse_mli source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|type error =
...
|ocaml} in
          let parsed = parse_mli source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|type cookie = {
  name: string;
  value: string;
...
|ocaml} in
          let parsed = parse_mli source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|type point = Base.point = private { x: int; y: string }
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let update state remaining = state.buffer <- remaining
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let run = fun ~args:_ -> ()
|ocaml}
      );
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let index_of = fun value ~char:needle ~fn:predicate ->
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let make: reader:IO.Reader.t -> writer:IO.Writer.t -> of_io_error:(IO.error -> Error.t) -> uri:Net.Uri.t -> t = fun ~reader ~writer ~of_io_error ~uri -> Ok ()
|ocaml}
      );
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let parse line =
  if line = "" then
    ()
  else
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let close = fun _conn -> ()
(* Reader/writer don't need explicit close *)
|ocaml}
      );
    Test.case "write keeps multiline ordinary comments idempotent"
      (
        fun ctx ->
...
|ocaml} in
          let first = parse_ml source |> Krasny.format |> Result.expect ~msg:"multiline ordinary comments should format" in
          let second = parse_ml first |> Krasny.format |> Result.expect ~msg:"formatted multiline ordinary comments should reformat" in
          Test.assert_equal ~expected:first ~actual:second;
          Test.Snapshot.assert_inline_text ~ctx ~actual:first ~expected:{ocaml|module M = struct
  (*
     first
       nested
  *)
...
|ocaml}
    ...
    Test.case "write normalizes multiline ordinary comment continuation indentation"
      (
        fun ctx ->
          let source = {ocaml|let link=fun ()->
let target_platform=target() in
(* NOTE: Dependency ld_flags must be collected here during linking, not in the profile.
                                             The profile contains only the current package's target-specific flags (applied in|ocaml} ^ " " ^ {ocaml|
...
transitive_deps
|ocaml} in
          let first = parse_ml source |> Krasny.format |> Result.expect ~msg:"multiline ordinary comment should format" in
          let second = parse_ml first |> Krasny.format |> Result.expect ~msg:"formatted multiline ordinary comment should reformat" in
          Test.assert_equal ~expected:first ~actual:second;
          Test.assert_false (has_trailing_horizontal_whitespace first);
          Test.Snapshot.assert_inline_text ~ctx ~actual:first ~expected:{ocaml|let link = fun () ->
  let target_platform = target () in
  (* NOTE: Dependency ld_flags must be collected here during linking, not in the profile.
...
|ocaml} in
          ...
          Test.assert_equal ~expected:first ~actual:second;
          Test.Snapshot.assert_inline_text ~ctx ~actual:first ~expected:{ocaml|let parse_payload = fun frame payload_data ->
  match frame.frame_type with
  | Frame.Headers ->
      Ok (
        {
...
|ocaml} in
          let parsed = parse_mli source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|val set: 'value t -> at:int -> value:'value -> (unit, error) Kernel.result
...
|ocaml} in
          let parsed = parse_mli source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|val ( = ): 'a -> 'a -> bool
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|include Kernel.Process
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|open Std
open Std.Collections
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let next = { state with count = state.count + 1; ready = true }
|ocaml}
      );
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let color = ((`rgb (1, 2, 3) :> color))
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let fin = fun Frame.{ fin; opcode } -> fin
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let ( + ) = add
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let item = Some (`Definition Cst.ClassDefinition.{ syntax_node = node; class_body = body })
|ocaml}
      );
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let one =
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let () =
  start ~apps:[] @@ fun () -> main ()
|ocaml}
      );
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let headers =
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let pair = fun a b -> (a, b)
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let make =
  list_init count
    (
      fun index ->
        let weight =
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let make = fun res fastest ->
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let length = String.length b |> Int.to_string
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let ok =
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let multiplier = 10.0 ** Float.from_int precision
|ocaml}
      );
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let seq = Color.to_escape_seq ~mode:`fg color
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let fields = timestamp_field () :: ("type", Data.Json.String "file") :: fields
|ocaml}
      );
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let values = fun slice last_newline line_start len ->
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let render = fun line_start column ->
  if line_start then
    (line_start, column + 1)
  else
    (
...
|ocaml} in
          ...
          let actual = capture_write parsed in
          Test.assert_false (has_trailing_horizontal_whitespace actual);
          Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|type context = {
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let fields =
  [
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let run = fun cond ->
  if cond then
    done_
  else
    (
...
|ocaml} in
          ...
          Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let f = fun cond ->
  match x with
  | Some data ->
      if cond then
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let sum = ref 0
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let run = fun () ->
  match x with
  | A ->
    foo ();
    bar ()
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let run = fun response body ->
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let main = fun result ->
  match result with
  | Error e ->
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let show = fun first_event ->
  Log.info
    (
      "First event: " ^ (
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let check = fun x ->
  assert (x = 1);
  assert (not (x = 2))
|ocaml}
      );
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let run = fun value ->
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let tagged =
  match result with
  | Ok () -> Ok () [@test]
...
|ocaml} in
          ...
          let reparsed = parse_ml actual in
          Test.assert_equal ~expected:(Krasny.syntax_hash parsed) ~actual:(Krasny.syntax_hash reparsed);
          Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|module Tests = struct
  let x = 1
end [@test]
...
|ocaml} in
          ...
          Test.assert_equal ~expected:(Krasny.syntax_hash parsed) ~actual:(Krasny.syntax_hash reparsed);
          Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|module Tests : sig
  val x: int
end [@test]
|ocaml}
      );
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let value =
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let sink = IO.Writer.from_sink (module Write) ()
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let unpack =
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|type t =
  | Conn : {
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|type 'dst sink = (module Write with type t = 'dst)
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|type Message.t +=
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|type event = ..
...
|ocaml} in
          let parsed = parse_mli source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|module Protocol : module type of Protocol
...
|ocaml} in
          let parsed = parse_mli source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|module Make (Order : Order.Ordered) : S with type key = Order.t
...
|ocaml} in
          ...
          let reparsed = parse_ml actual in
          Test.assert_equal ~expected:(Krasny.syntax_hash parsed) ~actual:(Krasny.syntax_hash reparsed);
          Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|module Protocol: Jsonrpc.ApplicationProtocol with type request = request and type response = response = struct end
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|module Name_map = Collections.Map.Make (String)
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|type decode_error = [`Invalid_octal | `Other of string]
...
|ocaml} in
          let parsed = parse_mli source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|val connect: mode:[ | `Client of string | `Server of string * string] -> unit
|ocaml}
      );
...
|ocaml} in
          let parsed = parse_mli source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|include module type of Global
|ocaml}
      );
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|external int64_bits_of_float: float -> int64 = "caml_int64_bits_of_float" "caml_int64_bits_of_float_unboxed" [@@ unboxed] [@@ noalloc]
...
|ocaml} in
          let parsed = parse_mli source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|type perform = {
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|type ('a, 'b) stack [@@ immediate]
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|type 'a t =
...
|ocaml} in
          ...
          | Error err ->
              let actual = Krasny.format_error_to_string err in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|unsupported type declaration body|ocaml}
      );
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let force = function
  | lazy value -> value
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let classify = function
  | `Alpha -> `Seen
  | `Beta value -> value
|ocaml}
      );
...
|} in assert_formatted_ml_snapshot ~ctx ~msg:"fun-body comment and docstring trivia should not need source reparsing" source
      );
...
|} in
          ...
          Test.assert_equal ~expected:{|let classify = fun flag ->
  if flag then
    value
    (* keep before else *)
  else
...
|} ~actual;
  ...
  (* keep before body *)
  use value
|} in assert_formatted_ml_snapshot ~ctx ~msg:"let rhs/body trivia should not need source reparsing" source
      );
...
let bind =
  ...
  (* keep before body *)
  finish value
|} in assert_formatted_ml_snapshot ~ctx ~msg:"sequence and binding-operator trivia should not need source reparsing" source
      );
  ...
  | A ->
      value
|} in assert_formatted_ml_snapshot ~ctx ~msg:"match case layout should not preserve source newlines after arrows" source
      );
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let get = fun value ->
  match value with
  | Some x -> (
    match x with
    | Some y -> y
...
|ocaml} in
          ...
          let reparsed = parse_ml actual in
          Test.assert_equal ~expected:(Krasny.syntax_hash parsed) ~actual:(Krasny.syntax_hash reparsed);
          Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let next = fun x ->
  match x with
  | Some y -> Some y, x
  | None -> None, x
|ocaml}
...
|ocaml} in
          ...
          let reparsed = parse_ml actual in
          Test.assert_equal ~expected:(Krasny.syntax_hash parsed) ~actual:(Krasny.syntax_hash reparsed);
          Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let update event model =
  match event with
  | `Clear ->
      (* Clear selection *)
...
|ocaml} in
          ...
          let reparsed = parse_ml actual in
          Test.assert_equal ~expected:(Krasny.syntax_hash parsed) ~actual:(Krasny.syntax_hash reparsed);
          Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let x =
  match a with
  | A -> (
    match b with
    | C -> (d, e)
...
|ocaml} in
          ...
          let reparsed = parse_ml actual in
          Test.assert_equal ~expected:(Krasny.syntax_hash parsed) ~actual:(Krasny.syntax_hash reparsed);
          Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let f = fun control ->
  (
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let value = (a, b), c
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let triple =
  match a, b, c with
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let value = fun x ->
  match x with
  | Some (a, b) -> a
  | None -> b
|ocaml}
...
let value = `Ok 1
|} in assert_formatted_ml_snapshot ~ctx ~msg:"polymorphic variant heads should format from tag tokens" source
      );
...
val id : 'a -> 'a
|} in assert_formatted_mli_snapshot ~ctx ~msg:"quoted core type variables should format from sigil and name tokens" source
      );
...
|} in
          ...
          Test.assert_equal ~expected:{|type 'value t = {
  mutable contents: 'value [@atomic];
}
|} ~actual;
...
|ocaml} in
          ...
          let reparsed = parse_ml actual in
          Test.assert_equal ~expected:(Krasny.syntax_hash parsed) ~actual:(Krasny.syntax_hash reparsed);
          Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let value = - !acc
...
|} in assert_formatted_ml_snapshot ~ctx ~msg:"typed named parameters should move to the synthesized outer annotation" source
      );
...
|} in
          ...
          Test.assert_equal ~expected:{|type color
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let symmetric ~h ~v : margin = {
  left = h;
  right = h;
  top = v;
  bottom = v
...
let y = a.(0)
let z = x.%(0)
|} in assert_formatted_ml_snapshot ~ctx ~msg:"index expressions should format from CST-carried delimiters, not token replay" source
      );
...
|ocaml} in
          ...
          let reparsed = parse_ml actual in
          Test.assert_equal ~expected:(Krasny.syntax_hash parsed) ~actual:(Krasny.syntax_hash reparsed);
          Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let a content pos = Some content.[!pos]
...
|} in assert_formatted_ml_snapshot ~ctx ~msg:"signed literal patterns should format from CST-carried sign tokens" source
      );
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let parse = function
  | Ok (Json.Int -123) -> Ok ()
...
|ocaml} in
          let parsed = parse_ml source in
          let actual = capture_write parsed in Test.Snapshot.assert_inline_text ~ctx ~actual ~expected:{ocaml|let show = function
...
(** doc for second *)
val second : int
|} in assert_formatted_mli_snapshot ~ctx ~msg:"signature docstring-led items should stay visually separated" source
      );
...
|} in assert_formatted_ml_snapshot ~ctx ~msg:"operator expressions and patterns should format from CST-carried operator tokens" source
      );
...
|} in
          ...
          Test.assert_equal ~expected:{|let negate value = ~-value
...
|} ~actual;
  ...
  | [value] -> hit
|} in assert_formatted_ml_snapshot ~ctx ~msg:"singleton list patterns should not preserve compact source spacing" compact_source
      );
...
|} in
          ...
          Test.assert_equal ~expected:{|let decide =
  if a && b || c then
    hit
  else
    miss
...
|} in assert_formatted_ml_snapshot ~ctx ~msg:"binding layout should not preserve multiline source for a simple wrapped value" source
      );
...
let bind =
  let* value = "ok" in
  finish value
|} in assert_formatted_ml_snapshot ~ctx ~msg:"simple string bindings should stay inline without a separate override" source
      );
...
|} ~actual;
    ...
    Test.case "format rewrites parameterized let bindings between formatted lets"
      (
        fun ctx ->
          let source = "(* intro *)\nlet x = 1 + 2\nlet f x = x + 1\nlet y = 3 + 4\n" in assert_formatted_ml_snapshot ~ctx ~msg:"parameterized let bindings should lower through explicit fun syntax" source
      );
...
|} in assert_formatted_ml_snapshot ~ctx ~msg:"list arguments should format" source
      );
  ...
  let open Option in
  value
|} in assert_formatted_ml_snapshot ~ctx ~msg:"let-open expressions should format structurally" source
      );
  ...
  ready, staged
|} in assert_formatted_ml_snapshot ~ctx ~msg:"local binding equals policy should stay stable while heuristics are isolated" source
      );
  ...
  total8, total9
|} in assert_formatted_ml_snapshot ~ctx ~msg:"local binding infix threshold should stay explicit and stable" source
      );
...
|} in assert_formatted_ml_snapshot ~ctx ~msg:"binding-operator equals policy should stay aligned with local bindings" source
      );
...
|} in
          ...
          Test.assert_equal ~expected:{|let outer value =
  let rec loop n =
    if n = 0 then
      value
    else
...
|} in assert_formatted_ml_snapshot ~ctx ~msg:"tuples should break from solver width" source
      );
...
|} listy |> Result.expect ~msg:"write listy";
                Fs.write {record_fixture|let explanation =
  Api.Explanation.
    {
...
|record_fixture} recordy |> Result.expect ~msg:"write recordy";
    ...
    Test.case "syntax hash normalizes commented pipeline parens"
      (
        fun _ctx ->
          let original = parse_ml {ocaml|let run events=if List.is_empty events then Error "empty" else (
(* Log each event *)
events|>List.enumerate|>List.for_each ~fn:(fun (_i,event)->ignore event);
Ok())
|ocaml} in
          let with_parens = parse_ml {ocaml|let run events=if List.is_empty events then Error "empty" else (
(* Log each event *)
(events|>List.enumerate)|>List.for_each ~fn:(fun (_i,event)->ignore event);
Ok())
|ocaml} in
...
|ocaml} in
          Test.assert_equal ~expected:(Krasny.syntax_hash parenthesized_triple) ~actual:(Krasny.syntax_hash bare_triple);
...
|ocaml} in
          Test.assert_equal ~expected:(Krasny.syntax_hash without_first_pipe) ~actual:(Krasny.syntax_hash with_first_pipe);
          Ok ()
      );
    Test.case "syntax hash normalizes record pattern trailing semis"
      (
        fun _ctx ->
          let with_trailing_semi = parse_ml {ocaml|let get=function|Some {left;right;}->left+right|None->0
|ocaml} in
          let without_trailing_semi = parse_ml {ocaml|let get=function|Some {left;right}->left+right|None->0
|ocaml} in
          Test.assert_equal ~expected:(Krasny.syntax_hash with_trailing_semi) ~actual:(Krasny.syntax_hash without_trailing_semi);
          Ok ()
      );
    Test.case "syntax hash normalizes list pattern trailing semis"
      (
        fun _ctx ->
          let with_trailing_semi = parse_ml {ocaml|let get=function|[left;right;]->left+right|[|left;right;|]->left+right|_->0
|ocaml} in
          let without_trailing_semi = parse_ml {ocaml|let get=function|[left;right]->left+right|[|left;right|]->left+right|_->0
|ocaml} in
          Test.assert_equal ~expected:(Krasny.syntax_hash with_trailing_semi) ~actual:(Krasny.syntax_hash without_trailing_semi);
          Ok ()
      );
...
|ocaml} in
    ...
    Test.case "syntax hash normalizes trailing sequence semis"
      (
        fun _ctx ->
          let with_trailing_semi = parse_ml {ocaml|let run=function|Ok value->println value;|Error error->println error
|ocaml} in
          let without_trailing_semi = parse_ml {ocaml|let run=function|Ok value->println value|Error error->println error
|ocaml} in
          Test.assert_equal ~expected:(Krasny.syntax_hash with_trailing_semi) ~actual:(Krasny.syntax_hash without_trailing_semi);
          Ok ()
      );
    Test.case "syntax hash normalizes trivia line indentation"
      (
        fun _ctx ->
          let shallow = parse_mli {ocaml|module S:sig
val run:unit->unit
(** doc
    details *)
end
|ocaml} in
          let indented = parse_mli {ocaml|module S:sig
val run:unit->unit
(** doc
      details *)
end
...
|} in
          ...
          Test.assert_equal ~expected:{|val ( = ): 'a -> 'a -> bool
...
|} in assert_format_ml_fails ~msg:"class declaration items are outside parser2 formatter scope" source
      );
...
|} in assert_format_mli_fails ~msg:"class type declaration items are outside parser2 formatter scope" source
      );
...
|} in assert_format_ml_fails ~msg:"shortcut class declaration modifiers are outside parser2 formatter scope" source
      );
...
|} in
    ...
    Test.case "format floating attributes from structural payload items"
      (
        fun ctx ->
          let source = "[@@@warning    \"-32\"]\n" in assert_formatted_ml_snapshot ~ctx ~msg:"floating attributes should render from structural payload items" source
      );
...
[%%bar let x = 1]
|} in assert_formatted_ml_snapshot ~ctx ~msg:"floating structure extensions should render structurally from the extension shell and payload" structure_source
      );
...
|} in
          ...
          Test.assert_equal ~expected:{|let trillion = 1.0e12
...
module M = [%foo]
|} in assert_formatted_ml_snapshot ~ctx ~msg:"module-expression and module-type extensions should render from the structural extension shell" source
      );
    Test.case "format structural core type token fallbacks"
      (
        fun _ctx ->
          let source = {|val use : #service -> M.(t list) -> < close : unit -> unit; next : int >
|} in
          let actual = parse_mli source |> Krasny.format |> Result.expect ~msg:"structural core type token fallback should format" in
          Test.assert_equal ~expected:{|val use: # service -> M.(t list) -> < close:unit -> unit; next:int >
|} ~actual;
          Ok ()
      );
...
|} in
          let actual = parse_ml source |> Krasny.format |> Result.expect ~msg:"first-class module types should format through token fallback" in
          Test.assert_equal ~expected:{|type packed = (module Transport with type t = int)
...
|} in assert_formatted_ml_snapshot ~ctx ~msg:"ordinary pattern-payload attributes should render structurally" source
      );
...
|} in assert_formatted_ml_snapshot ~ctx ~msg:"postfix expression attributes should preserve attributed apply and infix payloads" source
      );
...
let typed =
  (object
   ...
   : < m : int >)
|} in assert_format_ml_fails ~msg:"plain object expressions are not supported structurally yet" source
      );
...
|} in assert_format_ml_fails ~msg:"object bodies are outside parser2 formatter scope" source
      );
...
|} in assert_format_ml_fails ~msg:"object extension members are outside parser2 formatter scope" source
      );
    Test.case "format trailing variant comments with explicit separator policy"
      (
        fun ctx ->
          let source = "type t =\n  | A (* comment *)\n" in assert_formatted_ml_snapshot ~ctx ~msg:"trailing variant comments should format from explicit trivia separators" source
      );
    Test.case "format trailing variant docstrings with explicit separator policy"
      (
        fun ctx ->
          let source = "type t =\n  | A (** doc *)\n" in assert_formatted_ml_snapshot ~ctx ~msg:"trailing variant docstrings should format from explicit trivia separators" source
      );
...
|} in
    ...
    Test.case "format core-type extensions structurally"
      (
        fun ctx ->
          let source = "val use : [%foo: int]\n" in assert_formatted_mli_snapshot ~ctx ~msg:"core-type extensions should render structurally from the extension shell and payload" source
      );
...
|} in assert_formatted_ml_snapshot ~ctx ~msg:"pattern extensions should render structurally from the extension shell and payload" source
      );
...
|} in assert_format_ml_fails ~msg:"object override expressions are outside parser2 formatter scope" source
      );
...
|} in assert_formatted_ml_snapshot ~ctx ~msg:"expression extensions should render structurally from the extension shell and payload" source
      );
...
|} in assert_formatted_ml_snapshot ~ctx ~msg:"typed and polymorphic expressions should lower through structural core-type rendering" source
      );
...
|ocaml} in
          ...
          Test.assert_equal ~expected:first ~actual:second;
          Test.Snapshot.assert_inline_text ~ctx ~actual:first ~expected:{ocaml|let finish = fun lane lane_result ->
  {
...
;;
module M = struct end
|} in assert_formatted_ml_snapshot ~ctx ~msg:"top-level phrase separators should come from source-file tokens, not source gaps" source
      );
          ...
          with_tempdir "krasny_runner_scan"
            (
              fun tmpdir ->
                ...
                let expected = [ Path.to_string visible_ml; Path.to_string nested_mli ] |> List.sort ~compare:String.compare in
          ...
          with_tempdir "krasny_runner_ignore_tree"
            (
              fun tmpdir ->
                ...
                let files = Krasny.Runner.collect_ocaml_files ~roots:[ tmpdir ] ~should_ignore:(
                  fun path -> String.contains (Path.to_string path) "fixtures"
          ...
          with_tempdir "krasny_runner_check"
            (
              fun tmpdir ->
                ...
                let needs_result = result.files |> List.find ~fn:(
                  fun file_result -> String.equal (Path.to_string file_result.Krasny.Runner.file) (Path.to_string needs)
                ) |> Option.expect ~msg:"needs result missing" in
                let json = capture_json_event ~root:tmpdir (Krasny.Report.File needs_result) |> Data.Json.of_string |> Result.expect ~msg:"parse event json" in let open Data.Json in
                ...
                Test.assert_equal ~expected:(Some (String "needs_formatting")) ~actual:(get_field "status" json);
                Ok ()
            )
      );
          ...
          with_tempdir "krasny_runner_verify"
            (
              fun tmpdir ->
                ...
                let needs_result = result.files |> List.find ~fn:(
                  fun file_result -> String.equal (Path.to_string file_result.Krasny.Runner.file) (Path.to_string needs)
                ) |> Option.expect ~msg:"verify result missing" in
                let json = capture_json_event ~root:tmpdir (Krasny.Report.File needs_result) |> Data.Json.of_string |> Result.expect ~msg:"parse event json" in let open Data.Json in
          ...
          with_tempdir "krasny_runner_format"
            (
              fun tmpdir ->
                ...
                let file_result = result.files |> List.find ~fn:(
                  fun file_result -> String.equal (Path.to_string file_result.Krasny.Runner.file) (Path.to_string needs)
                ) |> Option.expect ~msg:"format result missing" in
                let json = capture_json_event ~root:tmpdir (Krasny.Report.File file_result) |> Data.Json.of_string |> Result.expect ~msg:"parse event json" in let open Data.Json in
          ...
          with_tempdir "krasny_runner_json_diagnostics"
            (
              fun tmpdir ->
                ...
                let file_result = result.files |> List.find ~fn:(
                  fun file_result -> String.equal (Path.to_string file_result.Krasny.Runner.file) (Path.to_string broken)
                ) |> Option.expect ~msg:"broken result missing" in
                ...
                | Some diagnostics ->
                    if List.is_empty diagnostics then
                      ...
                      let json = capture_json_event ~root:tmpdir (Krasny.Report.File file_result) |> Data.Json.of_string |> Result.expect ~msg:"parse event json" in
                      let expected = Some (Data.Json.Array (List.map diagnostics ~fn:Syn.Diagnostic.to_json)) in Test.assert_equal ~expected ~actual:(Data.Json.get_field "diagnostics" json);
                    Ok ()
          ...
          with_tempdir "krasny_runner_ignore"
            (
              fun tmpdir ->
                ...
                Fs.write "let ignored = 1\n" ignored |> Result.expect ~msg:"write ignored";
                let seen = cell [] in
                let result = Krasny.Runner.run_checks_streaming ~concurrency:1 ~roots:[ tmpdir ] ~should_ignore:(
                  fun path -> String.contains (Path.to_string path) "fixtures"
                ) ~on_result:(
                  fun file_result -> seen := Path.to_string file_result.file :: !seen
                ) () in
          ...
          with_tempdir "krasny_runner_stream"
            (
              fun tmpdir ->
                ...
                Fs.write "val x: int\n" needs |> Result.expect ~msg:"write needs";
                let seen = cell [] in
                let result = Krasny.Runner.run_checks_streaming ~concurrency:1 ~roots:[ tmpdir ] ~on_result:(
                  fun file_result -> seen := Path.to_string file_result.file :: !seen
                ) () in
                let actual = List.sort !seen ~compare:String.compare in
                let expected = [ Path.to_string formatted; Path.to_string needs ] |> List.sort ~compare:String.compare in
                Test.assert_equal ~expected ~actual;
                Test.assert_equal ~expected:2 ~actual:result.summary.total_files;
                ...
                Test.assert_equal ~expected:0 ~actual:result.summary.unsafe_to_format;
                Test.assert_equal ~expected:0 ~actual:result.summary.failed_files;
                let start_json = capture_json_event ~root:tmpdir (Krasny.Report.Start { mode = Krasny.Runner.Check; concurrency = 3 }) |> Data.Json.of_string |> Result.expect ~msg:"parse start json" in let open Data.Json in
