...
|ocaml});
  Test.case "write keeps adjacent module structures separated"
    (fun ctx ->
      let source = {ocaml|module Tcp:Intf=struct
let name="tcp"
end
...
|ocaml});
  Test.case "write breaks record type aliases when width is exceeded"
    (fun ctx ->
      let source = {ocaml|type export_entry=Manifest.export_entry={name:string;path:Std.Path.t;action_hash:string}
|ocaml}
      in
      let actual = Krasny.stream_format_to_string (parse_mli source) ~width:60
      |> Result.expect ~msg:"record type alias should format" in
      Test.Snapshot.assert_inline_text ~ctx ~actual
...
|ocaml});
  Test.case "write preserves shallow ordinary comment indentation"
    (fun ctx ->
...
|ocaml}
      in
      let first = parse_ml source |> Krasny.format |> Result.expect ~msg:"ordinary comment should format" in
      let second = parse_ml first |> Krasny.format |> Result.expect ~msg:"formatted ordinary comment should reformat" in
      Test.assert_equal ~expected:first ~actual:second;
      Test.Snapshot.assert_inline_text ~ctx ~actual:first
        ~expected:{ocaml|(* TODO:
  - keep this markdown indent
...
|ocaml}
      ...
      let actual = capture_write parsed in
      Test.Snapshot.assert_inline_text ~ctx ~actual
        ~expected:{ocaml|let one = let* item = read () in Ok item
...
|ocaml});
  Test.case "stream formatter renders polymorphic variant type bodies"
    (fun ctx ->
      let source = {ocaml|type color = [ ansi | rgb | xyz ]
|ocaml}
      in
      let actual = Krasny.stream_format_to_string (parse_ml source) ~width:100
      |> Result.expect ~msg:"polymorphic variant type body should format" in
      Test.Snapshot.assert_inline_text ~ctx ~actual
        ~expected:{ocaml|type color = [ansi | rgb | xyz]
|ocaml});
  Test.case "stream formatter keeps closed polymorphic variant aliases idempotent"
    (fun ctx ->
      let source = {ocaml|type cursor_visibility=[`hidden|`visible]
|ocaml}
      in
      let first = parse_ml source |> Krasny.format |> Result.expect ~msg:"closed polymorphic variant alias should format" in
      let second = parse_ml first |> Krasny.format |> Result.expect ~msg:"formatted closed polymorphic variant alias should reformat" in
      Test.assert_equal ~expected:first ~actual:second;
      Test.Snapshot.assert_inline_text ~ctx ~actual:first
...
let classify = function
  ...
  | _ -> 0
|ocaml});
  Test.case "write keeps or-pattern alternatives vertical"
    (fun ctx ->
      let source = {ocaml|let is_newline = function | '\n' | '\r' -> true | _ -> false
...
|ocaml});
  Test.case "format keeps explicit fun rhs bindings explicit"
    (fun _ctx ->
      ...
      Test.assert_equal ~expected:source ~actual;
      Ok ());
  Test.case "format inlines short explicit fun rhs with qualified apply bodies"
    (fun ctx ->
      let source = {ocaml|let execv=fun ~program ~args->Kernel.Process.execv program args
|ocaml}
      in
      let actual = parse_ml source |> Krasny.format |> Result.expect ~msg:"qualified multi-argument apply body should format" in
      Test.Snapshot.assert_inline_text ~ctx ~actual
...
|ocaml}
  ...
  | None -> 0
|ocaml});
  Test.case "write parenthesizes tuple match case bodies"
    (fun ctx ->
...
|ocaml});
  Test.case "write parenthesizes tuple scrutinees and tuple patterns"
    (fun ctx ->
...
|}
      ...
      Ok ());
  Test.case "write always parenthesizes tuple expressions and patterns"
    (fun ctx ->
      let source = {ocaml|let pair=a,b
let nested=a,(b,c)
let opened=M.(a,b)
...
let consume = function
  ...
  | M.(x, y) -> (x, y)
|ocaml});
  Test.case "write keeps small array expressions inline"
    (fun ctx ->
      let source = {ocaml|let array=[|1;2|]
|ocaml}
      in
      let actual = parse_ml source |> Krasny.format |> Result.expect ~msg:"small array should format" in
      Test.Snapshot.assert_inline_text ~ctx ~actual
        ~expected:{ocaml|let array = [|1; 2|]
|ocaml});
...
|}
  ...
  Test.case "write breaks let RHS before calls with multiline list arguments"
    (fun ctx ->
      let source = {ocaml|let ui=Element.row[Element.container~style:(Style.empty|>Style.width(Style.Fixed 20.0))[Element.text"Left"];Element.container~style:(Style.empty|>Style.width Style.Grow)[Element.text"Middle"];Element.container~style:(Style.empty|>Style.width(Style.Fixed 15.0))[Element.text"Right"]]
|ocaml}
      in
      let parsed = parse_ml source in
      let actual = capture_write parsed in
...
|}
      in
      assert_format_ml_fails ~msg:"object override expressions are outside parser2 formatter scope" source);
