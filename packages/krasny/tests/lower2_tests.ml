...
let assert_format2_ml = fun ~expected source ->
  ...
  if expected = actual then
    Ok ()
  else Error ("lower2 implementation output mismatch\nexpected:\n" ^ expected ^ "\nactual:\n" ^ actual)
...
let assert_format2_mli = fun ~expected source ->
  ...
  if expected = actual then
    Ok ()
  else Error ("lower2 interface output mismatch\nexpected:\n" ^ expected ^ "\nactual:\n" ^ actual)
  ...
  let rec loop paths = function
    ...
    | line :: rest ->
        if String.equal line "" || String.starts_with ~prefix:"#" line then
          ...
          let relpath = Path.from_string line |> Result.expect ~msg:"fixture manifest entry should be valid UTF-8" in loop (Path.(fixtures_dir / relpath) :: paths) rest
  in
  loop [] lines

let parser2_formatter_fixture_supported = fun path ->
  let path = Path.to_string path in not (String.contains path "class")
...
let assert_lower2_fixture_matches_approved = fun path ->
  ...
  | Error err -> Error (Path.to_string path ^ " failed lower2 formatting: " ^ Krasny.format_error_to_string err)
  | Ok formatted -> (
    ...
    if not (String.equal expected formatted) then
      Error (Path.to_string path ^ " lower2 output did not match approved formatter snapshot\nexpected:\n" ^ expected ^ "\nactual:\n" ^ formatted)
    else
      match format2_source ~filename:path formatted with
      | Error err -> Error (Path.to_string path ^ " formatted once but failed to format again: " ^ Krasny.format_error_to_string err)
      | Ok reformatted ->
          if formatted = reformatted then
            Ok ()
          else Error (Path.to_string path ^ " is not lower2-idempotent after one format\nfirst:\n" ^ formatted ^ "\nsecond:\n" ^ reformatted)
  )
...
|ocaml} {ocaml|let receive parsed = match parsed with | Http.Ws.Parser.Done frame -> (Buffer.clear buffer; Ok frame) | Http.Ws.Parser.Need_more -> (read_more (); receive parsed)
|ocaml}
      );
...
|ocaml} {ocaml|let receive parsed = match parsed with | Http.Ws.Parser.Need_more -> (let reader = to_reader stream in let chunk = read reader in match chunk with | Ok data -> data | Error error -> error) | Http.Ws.Parser.Done frame -> frame
|ocaml}
      );
    Test.case "lower2 indents commented parenthesized match bodies in cases"
      (
        fun _ctx -> assert_format2_ml ~expected:{ocaml|let next state =
...
|ocaml} {ocaml|let next state = match parse_event state.buffer with | Some event -> Some event | None -> ((* Need more data from connection *) match stream state.conn with | Error _error -> None | Ok _messages -> next state)
|ocaml}
      );
    Test.case "lower2 breaks parenthesized match after else"
      (
        fun _ctx -> assert_format2_ml ~expected:{ocaml|let pick header =
  if ready then
    Ok ()
  else
    (
...
|ocaml} {ocaml|let pick header = if ready then Ok () else (match header with | Some "chunked" -> Ok () | Some other -> Error other | None -> Error "missing")
|ocaml}
      );
  ...
  if state.done_ then
    ...
    | None ->
        ...
        next state
|ocaml} {ocaml|let next state = if state.done_ then None else (* Try to parse an event from current buffer *) match parse_event state.buffer with | Some event -> Some event | None -> (* Try parsing again with new data *) next state
|ocaml}
      );
    Test.case "lower2 wraps parenthesized infix arguments containing match"
      (
        fun _ctx -> assert_format2_ml ~expected:{ocaml|let log first_event = Log.info
  (
    "First event: " ^ (
...
|ocaml} {ocaml|let log first_event = Log.info ("First event: " ^ (match first_event with | Some _ -> "got one" | None -> "none"))
|ocaml}
      );
    Test.case "lower2 formats labels and optional labels"
      (
        fun _ctx -> assert_format2_ml ~expected:"let f ~x ?y = g ~x ?y\n" "let f ~x ?y = g ~x ?y\n"
      );
    Test.case "lower2 formats polymorphic variants"
      (
        fun _ctx -> assert_format2_ml ~expected:(top_level [ "let ok = `Ok 1"; "let classify = function\n  | `Ok value -> value\n  | `Error -> 0" ]) "let ok = `Ok 1\nlet classify = function | `Ok value -> value | `Error -> 0\n"
      );
    Test.case "lower2 preserves parens around polymorphic variant payload apply arguments"
      (
        fun _ctx -> assert_format2_ml ~expected:"let wrapped = Some (`Tag { value = 1 })\n" "let wrapped = Some (`Tag { value = 1 })\n"
      );
    Test.case "lower2 keeps simple polymorphic variant payload args bare"
      (
        fun _ctx -> assert_format2_ml ~expected:"let color_escape color = Color.to_escape_seq ~mode:`fg color\n" "let color_escape color = Color.to_escape_seq ~mode:`fg color\n"
      );
    Test.case "lower2 formats expression and pattern attributes"
      (
        fun _ctx -> assert_format2_ml ~expected:(top_level [ "let value = target [@inline always]"; "let (x [@foo]) = value" ]) "let value = target [@inline always]\nlet (x [@foo]) = value\n"
      );
    Test.case "lower2 formats expression pattern and item extensions"
      (
        fun _ctx -> assert_format2_ml ~expected:(
          top_level
            [
    ...
    Test.case "lower2 formats signature extension and attribute items"
      (
        fun _ctx -> assert_format2_mli ~expected:(top_level [ "[%%foo payload]"; "[@@@warning \"-32\"]"; "val id: int" ]) "[%%foo payload]\n[@@@warning \"-32\"]\nval id : int\n"
      );
    Test.case "lower2 formats selectors and index expressions"
      (
        fun _ctx -> assert_format2_ml ~expected:(top_level [ "let field = value.name"; "let item = values.(index)"; "let char = text.[index]" ]) "let field = value.name\nlet item = values.(index)\nlet char = text.[index]\n"
      );
    Test.case "lower2 keeps index expressions bare in apply arguments"
      (
        fun _ctx -> assert_format2_ml ~expected:(top_level [ "let char_ok = identifier_character source.[index - 1]"; "let item_ok = consume values.(index)" ]) "let char_ok = identifier_character source.[index - 1]\nlet item_ok = consume values.(index)\n"
      );
    Test.case "lower2 keeps deref prefix expressions bare in apply arguments"
      (
        fun _ctx -> assert_format2_ml ~expected:"let count_text = string_of_int !chunk_count\n" "let count_text = string_of_int !chunk_count\n"
      );
    Test.case "lower2 formats record expressions and patterns"
      (
        fun _ctx -> assert_format2_ml ~expected:(
          top_level
            [
    ...
    Test.case "lower2 formats binding operator expressions"
      (
        fun _ctx -> assert_format2_ml ~expected:(top_level [ "let value = let* x = fetch in let+ y = decode in pair x y"; "let both = let+ x = a and+ y = b in pair x y" ]) "let value = let* x = fetch in let+ y = decode in pair x y\nlet both = let+ x = a and+ y = b in pair x y\n"
      );
    Test.case "lower2 formats local open expressions and patterns"
      (
        fun _ctx -> assert_format2_ml ~expected:(
          top_level
            [
    ...
    Test.case "lower2 keeps delimited local opens bare in infix operands"
      (
        fun _ctx -> assert_format2_ml ~expected:"let preview = \"SSE Event: \" ^ Blink.SSE.(String.sub data ~offset:0 ~len:size)\n" "let preview = \"SSE Event: \" ^ Blink.SSE.(String.sub data ~offset:0 ~len:size)\n"
      );
    Test.case "lower2 formats first-class module expressions"
      (
        fun _ctx -> assert_format2_ml ~expected:(top_level [ "let packed = (module Foo.Bar)"; "let typed = (module Foo : S.T)"; "let advanced = (module Foo : S with type t = item and type state = state)" ]) "let packed = (module Foo.Bar)\nlet typed = (module Foo : S.T)\nlet advanced = (module Foo : S with type t = item and type state = state)\n"
      );
    Test.case "lower2 formats locally abstract and first-class module patterns"
      (
        fun _ctx -> assert_format2_ml ~expected:(top_level [ "let f (type a b) (module M : S.T) = value"; "let g (module _) = value"; "let h (module N : S with type t = item) = value" ]) "let f (type a b) (module M : S.T) = value\nlet g (module _) = value\nlet h (module N : S with type t = item) = value\n"
      );
    Test.case "lower2 formats package type value declarations"
      (
        fun _ctx -> assert_format2_mli ~expected:"val get: (module ConfigSpec with type t = 'a) -> ('a, error) result\n" "val get: (module ConfigSpec with type t = 'a) -> ('a, error) result\n"
      );
    ...
    Test.case "lower2 breaks let module before multiline bodies"
      (
        fun _ctx -> assert_format2_ml ~expected:{ocaml|let await = fun conn ->
  let module I = SSEIterator in
  Iter.MutIterator.make (module I) { I.conn; buffer = ""; done_ = false }
|ocaml} {ocaml|let await = fun conn -> let module I = SSEIterator in Iter.MutIterator.make (module I) { I.conn; buffer = ""; done_ = false }
|ocaml}
      );
    Test.case "lower2 formats let exception expressions"
      (
        fun _ctx -> assert_format2_ml ~expected:(top_level [ "let value = let exception Local of int * Foo.t in result"; "let bare = let exception Done in done_" ]) "let value = let exception Local of int * Foo.t in result\nlet bare = let exception Done in done_\n"
      );
    Test.case "lower2 formats unreachable expressions"
      (
        fun _ctx -> assert_format2_ml ~expected:"let value =\n  match maybe with\n  | Some value -> value\n  | None -> .\n" "let value = match maybe with | Some value -> value | None -> .\n"
      );
    Test.case "lower2 formats assertion and lazy expressions"
      (
        fun _ctx -> assert_format2_ml ~expected:(top_level [ "let _ = assert ready"; "let later = lazy compute" ]) "let _ = assert ready\nlet later = lazy compute\n"
      );
    Test.case "lower2 formats try expressions"
      (
        fun _ctx -> assert_format2_ml ~expected:"let value =\n  try read () with\n  | Failure -> 0\n" "let value = try read () with | Failure -> 0\n"
      );
    Test.case "lower2 formats while and for loops"
      (
        fun _ctx -> assert_format2_ml ~expected:(top_level [ "let poll = while ready do step () done"; "let up = for i = 0 to n do step i done"; "let down = for i = n downto 0 do step i done" ]) "let poll = while ready do step () done\nlet up = for i = 0 to n do step i done\nlet down = for i = n downto 0 do step i done\n"
      );
    Test.case "lower2 keeps while and for body sequences inside done boundaries"
      (
        fun _ctx -> assert_format2_ml ~expected:(top_level [ "let poll ready = while ready do\n  step ();\n  next ()\ndone"; "let count n = for i = 0 to n do\n  tick i;\n  total := !total + i\ndone" ]) "let poll ready = while ready do step (); next () done\nlet count n = for i = 0 to n do tick i; total := !total + i done\n"
      );
    Test.case "lower2 formats lazy exception and interval patterns"
      (
        fun _ctx -> assert_format2_ml ~expected:(top_level [ "let force = function\n  | lazy value -> value"; "let recovered =\n  match read () with\n  | exception Failure -> 0\n  | value -> value"; "let classify = function\n  | 'a' .. 'z' -> 1\n  | _ -> 0" ]) "let force = function | lazy value -> value\nlet recovered = match read () with | exception Failure -> 0 | value -> value\nlet classify = function | 'a' .. 'z' -> 1 | _ -> 0\n"
      );
    Test.case "lower2 adds a final newline"
      (
        fun _ctx -> assert_format2_ml ~expected:"let x = 1\n" "let x = 1"
      );
    Test.case "lower2 formats open declarations"
      (
        fun _ctx -> assert_format2_ml ~expected:"open Foo.Bar\n" "open Foo.Bar\n"
      );
    Test.case "lower2 formats simple include external and exception declarations"
      (
        fun _ctx -> assert_format2_ml ~expected:(top_level [ "include Foo.Bar"; "external id: 'a -> 'a = \"%identity\" \"caml_id\""; "exception Boom" ]) "include Foo.Bar\nexternal id : 'a -> 'a = \"%identity\" \"caml_id\"\nexception Boom\n"
      );
    Test.case "lower2 formats type extensions and structured exception rhs"
      (
        fun _ctx -> assert_format2_mli ~expected:(top_level [ "type 'a box +=\n  | More of 'a"; "exception Parse_error of string"; "exception Nested = Std.Result.Error" ]) "type 'a box += | More of 'a\nexception Parse_error of string\nexception Nested = Std.Result.Error\n"
      );
    Test.case "lower2 formats top-level exceptions after function bindings"
      (
        fun _ctx -> assert_format2_ml ~expected:(top_level [ "let error_to_string = function\n  | Error message -> message"; "exception Parse_exception of string" ]) "let error_to_string = function\n  | Error message -> message\n\nexception Parse_exception of string\n"
      );
    Test.case "lower2 formats simple module and module type declarations"
      (
        fun _ctx -> assert_format2_ml ~expected:(
          top_level
            [
    ...
    Test.case "lower2 formats constrained module type declarations"
      (
        fun _ctx -> assert_format2_ml ~expected:(top_level [ "module type S = Driver with type config = int and module Nested = Impl" ]) "module type S = Driver with type config = int and module Nested = Impl\n"
      );
    Test.case "lower2 formats simple signature module declarations"
      (
        fun _ctx -> assert_format2_mli ~expected:(
          top_level
            [
              "module Alias: Foo.S";
    ...
    Test.case "lower2 keeps include constraints in one signature item"
      (
        fun _ctx -> assert_format2_mli ~expected:"module type McpApplicationProtocol = sig\n\
    ...
    Test.case "lower2 keeps then-branch sequences inside if expressions with else"
      (
        fun _ctx -> assert_format2_ml ~expected:"let render ok =\n\
          \  if ok then\n\
          \    log ();\n\
          \    next ()\n\
          \  else\n\
    ...
    Test.case "lower2 keeps else-if chains after else-boundary comments"
      (
        fun _ctx -> assert_format2_ml ~expected:"let classify flag other =\n\
          \  if flag then\n\
          \    one\n\
          \    (* before next branch *)\n\
          \  else if other then\n\
    ...
    Test.case "lower2 keeps match-case sequences inside if then branches"
      (
        fun _ctx -> assert_format2_ml ~expected:"let classify ready input =\n\
          \  if ready then\n\
          \    match input with\n\
          \    | '!' | '^' ->\n\
          \        bump ();\n\
    ...
    Test.case "lower2 breaks multiline labeled applications after equals"
      (
        fun _ctx -> assert_format2_ml ~expected:"let () =\n\
          \  Runtime.run\n\
    ...
    Test.case "lower2 keeps medium pipelines inline in arrow bodies"
      (
        fun _ctx -> assert_format2_ml ~expected:"let about_handler = fun conn req -> conn |> Conn.respond ~status:Ok ~body:\"Suri - High-performance web framework\" |> Conn.send\n" "let about_handler = fun conn req -> conn |> Conn.respond ~status:Ok ~body:\"Suri - High-performance web framework\" |> Conn.send\n"
      );
    Test.case "lower2 keeps multiline parenthesized tuple args with callee"
      (
        fun _ctx -> assert_format2_ml ~expected:"let next = Some (\n\
    ...
    Test.case "lower2 keeps @@ fun applications bare"
      (
    ...
    Test.case "lower2 breaks long pipelines after arrow bodies"
      (
        fun _ctx -> assert_format2_ml ~expected:"let home_handler = fun conn req ->\n\
          \  conn\n\
          \  |> Conn.with_status Ok\n\
    ...
    Test.case "lower2 formats type aliases with parameters"
      (
    ...
    Test.case "lower2 breaks function arrows before multiline record bodies"
      (
        fun _ctx -> assert_format2_ml ~expected:"let create = fun method_ uri ->\n\
          \  {\n\
          \    method_;\n\
          \    uri;\n\
    ...
    Test.case "lower2 formats tuple type separators structurally"
      (
        fun _ctx -> assert_format2_mli ~expected:"type ('a, 'e) result_like = ('a, 'e) result\ntype pair = int * string\n" "type ('a, 'e) result_like = ('a, 'e) result\ntype pair = int * string\n"
      );
    Test.case "lower2 keeps qualified variant payload declarations multiline"
      (
        fun _ctx -> assert_format2_mli ~expected:"type source =\n\
          \  Version of string\n\
          \  | Path of Path.t\n\
          \  | Url of Net.Uri.t\n" "type source = Version of string | Path of Path.t | Url of Net.Uri.t\n"
      );
    Test.case "lower2 formats simple value declarations"
      (
        fun _ctx -> assert_format2_mli ~expected:"val id: 'a -> 'a\n" "val id : 'a -> 'a\n"
      );
    Test.case "lower2 breaks long value declarations after the colon"
      (
        fun _ctx -> assert_format2_mli ~expected:{ocaml|val resolve_module_path:
  lookup -> current_path:string list -> target_path:string list -> interface_source option
|ocaml} {ocaml|val resolve_module_path: lookup -> current_path:string list -> target_path:string list -> interface_source option
|ocaml}
      );
    Test.case "lower2 breaks medium value declarations after the colon"
      (
        fun _ctx -> assert_format2_mli ~expected:{ocaml|val materialize_package_exports:
  t -> exports:export_entry list -> target_dir:Std.Path.t -> (unit, error) result
|ocaml} {ocaml|val materialize_package_exports: t -> exports:export_entry list -> target_dir:Std.Path.t -> (unit, error) result
|ocaml}
      );
    Test.case "lower2 keeps fitting labeled value declarations inline"
      (
        fun _ctx -> assert_format2_mli ~expected:"val request: t -> Net.Http.Request.t -> ?body:string -> unit -> (unit, Error.t) result\n" "val request: t -> Net.Http.Request.t -> ?body:string -> unit -> (unit, Error.t) result\n"
      );
    Test.case "lower2 fully breaks wider labeled value declarations after the colon"
      (
        fun _ctx -> assert_format2_mli ~expected:"val parse:\n\
    ...
    Test.case "lower2 fully breaks named parameter declarations after the colon"
      (
        fun _ctx -> assert_format2_mli ~expected:"val make:\n\
          \  name:string ->\n\
          \  value:string ->\n\
          \  ?max_age:int ->\n\
          \  ?expires:string ->\n\
    ...
    Test.case "lower2 preserves consecutive docstring paragraphs"
      (
        fun _ctx -> assert_format2_mli ~expected:"val hash: t -> Crypto.hash\n\n\
    ...
    Test.case "lower2 keeps adjacent leading docstrings tight"
      (
        fun _ctx -> assert_format2_mli ~expected:{ocaml|(** Module overview. *)
(** Item doc. *)
type t
|ocaml} {ocaml|(** Module overview. *)
(** Item doc. *)
type t
|ocaml}
      );
    Test.case "lower2 keeps section headings tight to following signature docs"
      (
        fun _ctx -> assert_format2_mli ~expected:"type t = {\n  value: int;\n}\n(** {2 Parsing} *)\n(** Parse Cookie header into name-value pairs. *)\nval parse: string -> t\n" "type t = {\n  value: int;\n}\n(** {2 Parsing} *)\n(** Parse Cookie header into name-value pairs. *)\nval parse: string -> t\n"
      );
    Test.case "lower2 keeps adjacent signature values separated"
      (
        fun _ctx -> assert_format2_mli ~expected:"module type Intf = sig\n  val name: string\n\n  val connect: Net.Addr.stream_addr -> Net.Uri.t -> (Connection.t, Error.t) result\nend\n" "module type Intf = sig\nval name:string\nval connect: Net.Addr.stream_addr -> Net.Uri.t -> (Connection.t, Error.t) result\nend\n"
      );
    Test.case "lower2 formats module struct bodies from nested items"
      (
        fun _ctx -> assert_format2_ml ~expected:{ocaml|module Tcp: Intf = struct
  let name = "tcp"
...
|ocaml} {ocaml|module Tcp: Intf = struct
let name = "tcp"
...
|ocaml}
    ...
    Test.case "lower2 rejects unsupported shapes instead of replaying source"
      (
        fun _ctx -> assert_format2_ml_fails "let object_value = object end\n"
      );
    Test.case ~size:Large "lower2 formats the existing fixture corpus idempotently"
      (
