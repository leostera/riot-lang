...
let is_docstring_kind =
  function
  | Syn.SyntaxKind.DOCSTRING -> true
  | _ -> false

let is_comment_like_kind =
  function
  | Syn.SyntaxKind.COMMENT
  | Syn.SyntaxKind.DOCSTRING -> true
  | _ -> false

let is_redundant_paren_inner_kind =
  function
...
let syntax_hash = fun (result:Syn.Parser.parse_result) ->
  ...
  let rec should_skip_token = fun ~parent_kind token ->
    ...
    if is_trivia_kind token_kind || is_docstring_kind token_kind then
      ...
      | _ -> false
  and redundant_paren_child = fun node ->
...
let start_scanner = fun ~owner ~roots ~scanner_ref ~should_ignore ->
  ...
  let state = {owner; scanner_ref; should_ignore; seen; pending = List.sort compare_paths roots; } in
...
let run_streaming = fun ~mode ?(concurrency = System.available_parallelism) ?(should_ignore = fun _ -> false) ~roots ~on_result () ->
  ...
  in
  let rec collect = fun results_rev ->
    let selector : ([
