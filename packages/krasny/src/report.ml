...
let file_result_to_json = fun ~root (result: Runner.file_result) -> let open Data.Json in
let status =
...
let timestamp_field = fun () -> ("timestamp", Data.Json.String (DateTime.now_utc () |> DateTime.to_iso8601))
...
let event_to_json = fun ~root ->
  ...
  | File result -> (
    ...
    | Data.Json.Object fields -> Data.Json.Object (timestamp_field () :: ("type", Data.Json.String "file") :: fields)
    | _ -> panic "expected JSON object"
  )
  | Summary summary -> (
    match summary_to_json summary with
    | Data.Json.Object fields -> Data.Json.Object (timestamp_field () :: ("type", Data.Json.String "summary") :: fields)
    | _ -> panic "expected JSON object"
  )

let write_line = fun ~writer line ->
  let buffer = IO.Buffer.from_string (line ^ "\n") in IO.write_all writer ~from:buffer
...
let write_text_file_result = fun ~writer ~root (result: Runner.file_result) ->
  let status_char, suffix =
    ...
    | Runner.Unsafe_to_format, Some error -> "\027[1;31m✗\027[0m", " (unsafe to format: " ^ error ^ ")"
    | Runner.Unsafe_to_format, None -> "\027[1;31m✗\027[0m", " (unsafe to format)"
  in
  let path = relative_to_root ~root result.file in write_line ~writer (status_char ^ " " ^ path ^ suffix)
...
let write_text_summary = fun ~writer ~mode (summary: Runner.summary) ->
  ...
  in
  let line = status_char ^ " " ^ verb ^ " " ^ Int.to_string summary.total_files ^ " files in " ^ duration ^ "s (" ^ Int.to_string summary.already_formatted ^ " already formatted, " ^ Int.to_string summary.needs_formatting ^ " need formatting, " ^ Int.to_string summary.would_reformat ^ " would reformat safely, " ^ Int.to_string summary.unsafe_to_format ^ " unsafe to format, " ^ Int.to_string summary.formatted_files ^ " formatted, " ^ Int.to_string summary.failed_files ^ " failed)" in write_line ~writer line

let write_json_event = fun ~writer ~root event -> write_line ~writer (Data.Json.to_string (event_to_json ~root event))
