...
and sink = { writer: IO.Writer.t; flush_threshold: int; mutable error: IO.error option }
...
and t =
  | Empty
  | Node of node
and slice = { value: Slice.t; has_newline: bool }
and node = {
...
let write_pending_spaces = fun writer ->
  ...
  if count > 0 then
    (
      for _ = 1 to count do IO.Buffer.add_char writer.buffer ' ' done;
...
let write_string_segment = fun writer value ~off ~len ->
  if len > 0 then
    (
      ...
      else write_pending_spaces writer;
  ...
  let rec loop index =
    if Int.(index >= length) then
      ...
      if Char.equal char '\n' then
        ...
        if is_horizontal_whitespace char then
...
let flat_measure_of_text = fun value ->
  if String.contains value "\n" then
    None
  else Some { Doc.flat_width = String.length value; stops_at_line = false }

let flat_measure_of_slice = fun ~has_newline value ->
  if has_newline then
    None
  else Some { Doc.flat_width = Slice.length value; stops_at_line = false }

let add_flat_measure = fun left right -> { Doc.flat_width = Int.add left.Doc.flat_width right.Doc.flat_width; stops_at_line = right.Doc.stops_at_line }
...
let text = fun value ->
  if Int.(String.length value = 0) then
    Empty
  else node ~flat_measure:(flat_measure_of_text value) ~multiline:(String.contains value "\n") ~shape:(Text_shape value) ~emit:(
    fun writer -> emit_text writer value
  )

let raw_text = fun value ->
  if Int.(String.length value = 0) then
    Empty
  else node ~flat_measure:(flat_measure_of_text value) ~multiline:(String.contains value "\n") ~shape:(Raw_text_shape value) ~emit:(
    fun writer -> emit_raw_text writer value
  )

let slice = fun ~has_newline value ->
  if Int.(Slice.length value = 0) then
    Empty
  else node ~flat_measure:(flat_measure_of_slice ~has_newline value) ~multiline:has_newline ~shape:(Slice_shape { value; has_newline }) ~emit:(
    fun writer -> emit_slice writer ~has_newline value
  )

let space = node ~flat_measure:(Some { Doc.flat_width = 1; stops_at_line = false }) ~multiline:false ~shape:(Space_shape 1) ~emit:(
  fun writer ->
    if writer.line_start then
...
let spaces = fun count ->
  if Int.(count <= 0) then
    ...
    if Int.(count = 1) then
      space
    else node ~flat_measure:(Some { Doc.flat_width = count; stops_at_line = false }) ~multiline:false ~shape:(Space_shape count) ~emit:(
      fun writer ->
        if writer.line_start then
...
let line = node ~flat_measure:(Some { Doc.flat_width = 0; stops_at_line = true }) ~multiline:true ~shape:Line_shape ~emit:emit_line

let break = fun ?(flat = " ") () -> node ~flat_measure:(Some { Doc.flat_width = String.length flat; stops_at_line = false }) ~multiline:false ~shape:(Break_shape flat) ~emit:(
  fun writer ->
...
let doc_vector = fun docs ->
  ...
  | _ -> node ~flat_measure:(flat_measure_vector docs) ~multiline:(is_multiline_vector docs) ~shape:(Concat_shape docs) ~emit:(
    fun writer ->
      let length = Vector.length docs in
      let rec loop index =
        if Int.(index < length) then
  ...
  let add_spaces count =
    if Int.(count > 0) then
      ...
      if Int.equal current_length 0 then
        ...
        | Node { shape = Space_shape current; _ } -> Vector.set_unchecked output ~at:last_index ~value:(spaces (Int.add current count))
        | _ -> push (spaces count)
  in
...
let concat = fun docs -> concat_with ~iter:(
  fun append_doc -> List.for_each docs ~fn:append_doc
)

let concat_vector = fun docs -> concat_with ~iter:(
...
let rec fits = fun ~width remaining ->
  ...
  | (_, _, Node { shape = Slice_shape slice; _ }) :: rest ->
      ...
      else fits ~width (remaining - Slice.length slice.value) rest
  | (_, _, Node { shape = Space_shape count; _ }) :: rest -> fits ~width (remaining - count) rest
  | (_, _, Node { shape = Line_shape; _ }) :: _ -> true
  | (_, Flat, Node { shape = Break_shape flat; _ }) :: rest -> fits ~width (remaining - String.length flat) rest
  | (_, Break, Node { shape = Break_shape _; _ }) :: _ -> true
  | (indent, _, Node { shape = Group_shape doc; _ }) :: rest -> fits ~width remaining ((indent, Flat, doc) :: rest)
  | (indent, mode, Node { shape = Concat_shape docs; _ }) :: rest -> fits ~width remaining (push_many indent mode docs rest)
  | (indent, mode, Node { shape = Indent_shape (extra, doc); _ }) :: rest -> fits ~width remaining ((indent + extra, mode, doc) :: rest)
...
let group_mode = fun writer doc flat_measure ->
  ...
  | _ when fits ~width:writer.width (writer.width - writer.column)
    [
      writer.indent, Flat, doc;
    ] -> Flat
  | _ -> Break

let group = fun doc ->
  let flat_measure = flat_measure doc in node ~flat_measure ~multiline:(is_multiline doc) ~shape:(Group_shape doc) ~emit:(
    fun writer ->
...
let indent = fun spaces doc ->
  if Int.(spaces <= 0) then
    doc
  else node ~flat_measure:(flat_measure doc) ~multiline:(is_multiline doc) ~shape:(Indent_shape (spaces, doc)) ~emit:(
    fun writer ->
