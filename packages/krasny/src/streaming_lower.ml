...
and type_expr_starts_with_gt = fun type_expr ->
  ...
  | Opaque node ->
      ...
      if Int.equal (Vector.length tokens) 1 then
        ...
        false
  | Apply { argument=Some argument; _ } ->
      type_expr_starts_with_gt argument
  | _ ->
      false
...
and tuple_expr_needs_parens_in_list_item = fun expr ->
  if expr_is_tuple expr then
    ...
    let rec loop index =
      if Int.(index >= length) then
        false
      else if expr_can_capture_list_separator (Vector.get_unchecked items ~at:index) then
        true
      else
        loop (Int.add index 1)
    in
...
and render_struct_module_expr_node = fun state node ->
  ...
  if Int.equal (Vector.length items) 0 then
    ...
    emit_text state "struct";
    emit_line state;
    with_indent state 2 (fun () -> render_structure_items state items);
    emit_line state;
    emit_text state "end"
  )
...
and render_record_expr = fun state ~inline expr ->
  ...
  if Option.is_none base && Int.equal length 0 then
    ...
    render_inline ()
  else if record_expr_should_inline expr then
    render_inline ()
  else (
    emit_text state "{";
    emit_line state;
...
and render_module_declaration = fun state decl ->
  if module_declaration_has_ascribed_body decl || module_declaration_has_head_parameter decl then
    ...
    | Unsupported when not (Ast.ModuleDeclaration.has_typeof_body decl) -> render_module_declaration_members
      state
      decl
    | body ->
        emit_text state "module";
      ...
      (
        ...
        if Int.(index < Int.sub length 1) then
          if structure_item_is_attribute (Vector.get_unchecked items ~at:(Int.add index 1)) then
            ...
            structure_items_join_tightly item (Vector.get_unchecked items ~at:(Int.add index 1))
          then
            emit_line state
          else (
            emit_line state;
      ...
      (
        ...
        if Int.(index < Int.sub length 1) then
          if signature_item_is_attribute (Vector.get_unchecked items ~at:(Int.add index 1)) then
            ...
            signature_items_join_tightly item (Vector.get_unchecked items ~at:(Int.add index 1))
          then
            emit_line state
          else (
            emit_line state;
...
and render_structure_item = fun state item ->
  ...
  (
    ...
    | Include decl ->
        render_include_declaration state decl
    | External decl ->
        render_external_declaration state decl
    | Exception decl ->
        render_exception_declaration state decl
    | TypeExtension decl ->
        render_type_extension_declaration state decl
    | Attribute item ->
...
let render_implementation = fun state implementation ->
  render_structure_items state (collect_structure_items_from_node implementation)
