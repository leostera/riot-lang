...
let source_slice = fun source ->
  ...
  |> Result.expect ~msg:"failed to create format pipeline benchmark source slice"

let load_fixture = fun ~name path ->
  let source =
    Fs.read path
    |> Result.expect
      ~msg:("failed to read format pipeline benchmark fixture: " ^ Path.to_string path)
  in
  {
    name;
    path;
...
let bench_parse = fun () ->
  ...
  touch_int root.Syn.SyntaxTree.full_width
...
let bench_format_to_string = fun () ->
  let formatted =
    parse ()
    |> Krasny.format
    |> Result.expect ~msg:"format pipeline benchmark should format to string"
  in
  touch_string formatted

let bench_stream_format = fun () ->
  let (sink, writer) = counting_writer () in
  Krasny.stream_format (parse ()) ~writer ~width:100
  |> Result.expect ~msg:"format pipeline benchmark should stream unicode tables";
...
let benchmarks = [
  Bench.compare
    ("krasny pipeline: parse " ^ unicode_tables_fixture.name)
    [ make_case "parse" bench_parse; make_case "parse + Ast view" bench_ast_view ];
  Bench.compare
    ("krasny pipeline: format to output " ^ unicode_tables_fixture.name)
    [
      make_case "Krasny.format" bench_format_to_string;
      make_case "Krasny.stream_format" bench_stream_format;
    ];
]
