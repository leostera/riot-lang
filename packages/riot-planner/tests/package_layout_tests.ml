...
let make_package = fun ?library ?(binaries = []) name -> Package.make ~name:(package_name name) ~path:(Path.v ".") ~relative_path:(Path.v ".") ?library ~binaries:(List.map binaries ~fn:(
  fun (binary_name, path) -> Package.{ name = binary_name; path = Path.v path }
)) ()
...
let add_ml_node = fun graph ~namespace ~path -> G.add_node graph (Riot_planner.Module_node.make_ml (make_module ~namespace path) (Riot_planner.Module_node.Concrete (Path.v path)))

let add_generated_ml_node = fun graph ~namespace ~path ~contents -> G.add_node graph (Riot_planner.Module_node.make_ml (make_module ~namespace path) (Riot_planner.Module_node.Generated { path = Path.v path; contents }))

let add_binary_target = fun graph ~name ~source -> G.add_node graph (Riot_planner.Module_node.make_binary ~name ~source:(Path.v source) ~libraries:[] ~includes:[])
...
let analyzed_module = fun (node: Riot_planner.Module_node.t G.node) ~source ->
  ...
  let deps = Syn.Deps.of_parse_result parse_result in (node.id, Riot_planner.Module_graph.{
    display_path;
...
let validate_layout = fun ~package ~graph ~analyzed -> Riot_planner.Package_layout_validator.validate ~direct_dependency_modules:[] ~package ~module_graph:graph ~analyzed_modules:(List.map analyzed ~fn:(
  fun (node, source) -> analyzed_module node ~source
))

let assert_target_can_use_public_root_module = fun ~package_name ~target_name ~target_source_path ~target_source ->
  let package = make_package ~library:{ path = Path.v ("src/" ^ package_name ^ ".ml") } ~binaries:[
    target_name, target_source_path;
  ] package_name in
  let graph = G.make () in
  ...
  match validate_layout ~package ~graph ~analyzed:[
    target, target_source;
  ] with
  | Ok () -> Ok ()
  | Error err -> Error ("expected target to use public root, got: " ^ Riot_planner.Planning_error.to_string err)
...
let assert_target_cannot_use_internal_library_module_directly = fun ~package_name ~target_name ~target_source_path ~target_source ->
  ...
  | Error (Riot_planner.Planning_error.TargetDependsOnInternalLibraryModule { target_name = actual_target_name; source; requested_module; internal_module; public_module = actual_public_module }) ->
      ...
      Ok ()
  | Error err -> Error ("expected internal-library-module planner error, got: " ^ Riot_planner.Planning_error.to_string err)
...
let assert_target_cannot_use_namespaced_internal_library_module = fun ~package_name ~target_name ~target_source_path ~target_source ->
  ...
  | Error (Riot_planner.Planning_error.TargetDependsOnNamespacedInternalLibraryModule { target_name = actual_target_name; source; requested_module; internal_module; public_module = actual_public_module }) ->
      Test.assert_equal ~expected:target_name ~actual:actual_target_name;
      Test.assert_equal ~expected:(Path.v target_source_path) ~actual:source;
      ...
      Test.assert_equal ~expected:public_module ~actual:actual_public_module;
      Ok ()
  | Error err -> Error ("expected namespaced-internal planner error, got: " ^ Riot_planner.Planning_error.to_string err)
  | Ok () -> Error "expected namespaced internal access to fail"

let test_same_package_binary_can_use_public_root_module = fun _ctx ->
  let package = make_package ~library:{ path = Path.v "src/berrybot.ml" } ~binaries:[
  ...
  match validate_layout ~package ~graph ~analyzed:[
    main, "let () = ignore Berrybot.A.value\n";
  ] with
  | Ok () -> Ok ()
  | Error err -> Error ("expected public root access to be valid, got: " ^ Riot_planner.Planning_error.to_string err)
...
let test_same_package_binary_can_use_generated_public_root_module = fun _ctx ->
  ...
  let graph = G.make () in
  let root = add_generated_ml_node graph ~namespace:Namespace.empty ~path:"src/berrybot.ml" ~contents:"module A = A\n" in
  let a = add_ml_node graph ~namespace:(public_namespace package) ~path:"src/a.ml" in
  let main = add_ml_node graph ~namespace:(public_namespace package) ~path:"src/main.ml" in
  let _binary = add_binary_target graph ~name:"berrybot" ~source:"src/main.ml" in
  let () = add_dep root ~depends_on:a in
  ...
  | Ok () -> Ok ()
  | Error err -> Error ("expected generated public root access to be valid, got: " ^ Riot_planner.Planning_error.to_string err)
...
let test_binary_private_helper_can_use_public_root_module = fun _ctx ->
  ...
  match validate_layout ~package ~graph ~analyzed:[
    helper, "let value = Berrybot.A.value\n";
    main, "let () = ignore B.value\n";
  ] with
  | Ok () -> Ok ()
  | Error err -> Error ("expected binary-private helper to use public root, got: " ^ Riot_planner.Planning_error.to_string err)

let test_same_package_binary_can_use_public_root_via_open = fun _ctx ->
  let package = make_package ~library:{ path = Path.v "src/syn.ml" } ~binaries:[
    "syn", "src/main.ml";
  ] "syn" in
  let graph = G.make () in
  ...
  | Ok () -> Ok ()
  | Error err -> Error ("expected open public root access to be valid, got: " ^ Riot_planner.Planning_error.to_string err)
...
let test_same_package_binary_cannot_use_namespaced_internal_library_module = fun _ctx ->
  ...
  let () = add_dep main ~depends_on:a in
  match validate_layout ~package ~graph ~analyzed:[
    main, "let () = ignore Berrybot__A.value\n";
  ] with
...
let test_binary_private_helper_cannot_use_internal_library_module_directly = fun _ctx ->
  ...
  | Error (Riot_planner.Planning_error.TargetDependsOnInternalLibraryModule { target_name; source; requested_module; internal_module; public_module }) ->
      ...
      Test.assert_equal ~expected:"Berrybot" ~actual:public_module;
      Ok ()
  | Error err -> Error ("expected helper direct internal access to fail with planner error, got: " ^ Riot_planner.Planning_error.to_string err)
...
let test_binary_private_helper_cannot_use_namespaced_internal_library_module = fun _ctx ->
  ...
  let () = add_dep main ~depends_on:helper in
  match validate_layout ~package ~graph ~analyzed:[
    helper, "let value = Berrybot__A.value\n";
    main, "let () = ignore B.value\n";
  ] with
  | Error (Riot_planner.Planning_error.TargetDependsOnNamespacedInternalLibraryModule { target_name; source; requested_module; internal_module; public_module }) ->
      Test.assert_equal ~expected:"berrybot" ~actual:target_name;
      ...
      Test.assert_equal ~expected:"Berrybot" ~actual:public_module;
      Ok ()
  | Error err -> Error ("expected helper namespaced internal access to fail with planner error, got: " ^ Riot_planner.Planning_error.to_string err)
  | Ok () -> Error "expected helper namespaced internal access to fail"

let test_test_target_can_use_public_root_module = fun _ctx -> assert_target_can_use_public_root_module ~package_name:"berrybot" ~target_name:"berrybot-tests" ~target_source_path:"tests/berrybot_tests.ml" ~target_source:"let () = ignore Berrybot.A.value\n"

let test_example_target_can_use_public_root_module = fun _ctx -> assert_target_can_use_public_root_module ~package_name:"berrybot" ~target_name:"berrybot-example" ~target_source_path:"examples/demo.ml" ~target_source:"let () = ignore Berrybot.A.value\n"

let test_bench_target_can_use_public_root_module = fun _ctx -> assert_target_can_use_public_root_module ~package_name:"berrybot" ~target_name:"berrybot-bench" ~target_source_path:"bench/demo_bench.ml" ~target_source:"let () = ignore Berrybot.A.value\n"

let test_test_target_cannot_use_internal_library_module_directly = fun _ctx -> assert_target_cannot_use_internal_library_module_directly ~package_name:"berrybot" ~target_name:"berrybot-tests" ~target_source_path:"tests/berrybot_tests.ml" ~target_source:"let () = ignore A.value\n"

let test_example_target_cannot_use_internal_library_module_directly = fun _ctx -> assert_target_cannot_use_internal_library_module_directly ~package_name:"berrybot" ~target_name:"berrybot-example" ~target_source_path:"examples/demo.ml" ~target_source:"let () = ignore A.value\n"

let test_bench_target_cannot_use_namespaced_internal_library_module = fun _ctx -> assert_target_cannot_use_namespaced_internal_library_module ~package_name:"berrybot" ~target_name:"berrybot-bench" ~target_source_path:"bench/demo_bench.ml" ~target_source:"let () = ignore Berrybot__A.value\n"
...
let test_multiple_binaries_can_share_private_helper_module = fun _ctx ->
  ...
  | Ok () -> Ok ()
  | Error err -> Error ("expected shared helper module across binaries to be valid, got: " ^ Riot_planner.Planning_error.to_string err)

let test_no_library_package_can_use_private_helper_module = fun _ctx ->
  let package = make_package ~binaries:[
    "berrybot", "src/main.ml";
  ] "berrybot" in
  let graph = G.make () in
  ...
  | Ok () -> Ok ()
  | Error err -> Error ("expected helper module in no-library package to be valid, got: " ^ Riot_planner.Planning_error.to_string err)
...
let test_same_package_binary_cannot_use_namespaced_other_binary_root = fun _ctx ->
  ...
  let () = add_dep main ~depends_on:admin in
  match validate_layout ~package ~graph ~analyzed:[
    main, "let () = ignore Berrybot__Admin.run\n";
  ] with
  | Error (Riot_planner.Planning_error.TargetDependsOnOtherTargetRoot { target_name; source; requested_module; other_target_name; other_target_module; public_module }) ->
      Test.assert_equal ~expected:"berrybot" ~actual:target_name;
      Test.assert_equal ~expected:(Path.v "src/main.ml") ~actual:source;
...
let test_binary_private_helper_cannot_use_other_binary_root = fun _ctx ->
  ...
  | Error (Riot_planner.Planning_error.TargetDependsOnOtherTargetRoot { target_name; source; requested_module; other_target_name; other_target_module; public_module }) ->
      ...
      Test.assert_equal ~expected:"Berrybot" ~actual:public_module;
      Ok ()
  | Error err -> Error ("expected helper other-target-root planner error, got: " ^ Riot_planner.Planning_error.to_string err)
...
let tests = Test.[
  ...
  case "same-package binary can use generated public root module" test_same_package_binary_can_use_generated_public_root_module;
  case "binary-private helper can use public root module" test_binary_private_helper_can_use_public_root_module;
  case "same-package binary can use public root module via open" test_same_package_binary_can_use_public_root_via_open;
  case "same-package binary cannot use internal library module directly" test_same_package_binary_cannot_use_internal_library_module_directly;
  case "same-package binary cannot use namespaced internal library module" test_same_package_binary_cannot_use_namespaced_internal_library_module;
  case "binary-private helper cannot use internal library module directly" test_binary_private_helper_cannot_use_internal_library_module_directly;
  case "binary-private helper cannot use namespaced internal library module" test_binary_private_helper_cannot_use_namespaced_internal_library_module;
