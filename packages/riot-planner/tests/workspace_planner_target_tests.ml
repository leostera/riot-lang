...
let make_workspace = fun packages -> Workspace.make_realized ~root:(Path.v "/tmp/workspace_planner_target_tests") ~packages ()

let plan_workspace = fun workspace target scope -> Workspace_planner.plan_workspace ~workspace ~target ~scope ~load_errors:[] ~dev_artifacts:{ tests = true; examples = true; benches = true }

let package_names = fun plan -> Workspace_planner.packages_in_plan plan |> List.map ~fn:(
  fun (pkg: Package.t) -> pkg.name
)

let package_keys = fun plan -> Riot_planner.Package_graph.topological_sort plan.Workspace_planner.package_graph |> List.map ~fn:Riot_planner.Package_graph.get_key |> List.map ~fn:Riot_model.Package.key_to_string
...
let plan_all_runtime_returns_workspace_like_order = fun _ctx ->
  ...
  | Ok plan ->
      ...
      let position name =
        let name = package_name name in List.enumerate names |> List.find ~fn:(
          fun (_, current) -> Package_name.equal name current
        ) |> Option.map ~fn:(
          fun (index, _) -> index
...
let plan_unknown_package_reports_available_packages = fun _ctx ->
  ...
  | Error (PackageNotFound { name; available }) ->
      ...
      Test.assert_equal ~expected:(List.map [ "app"; "std" ] ~fn:package_name) ~actual:(List.sort available ~compare:Package_name.compare);
      Ok ()
  | Error _ -> Error "expected PackageNotFound"

let plan_multiple_unknown_packages_reports_all_missing_names = fun _ctx ->
  let workspace = make_workspace [ make_package "std"; make_package ~dependencies:[ "std" ] "app" ] in
  match plan_workspace workspace (Packages (List.map [ "missing-a"; "app"; "missing-b" ] ~fn:package_name)) Runtime with
...
let plan_reports_missing_dependencies_before_sorting = fun _ctx ->
  ...
  | Error (MissingDependencies { missing }) ->
      let entries = List.map missing ~fn:(
...
let plan_runtime_target_does_not_pull_build_dependency_runtime_cycle = fun _ctx ->
  ...
  in
  match plan_workspace workspace (Package (package_name "app")) Runtime with
  | Error (CycleDetected { cycle }) -> Error ("expected runtime target planning to avoid build-dependency cycle, got cycle: " ^ String.concat " -> " cycle)
  | Error _ -> Error "expected successful package-target plan"
  | Ok plan ->
      Test.assert_equal ~expected:[ "core:build"; "core:runtime"; "app:runtime" ] ~actual:(package_keys plan);
      Ok ()

let plan_targeted_runtime_ignores_unrelated_missing_dependencies = fun _ctx ->
  let workspace = make_workspace [ make_package "std"; make_package ~dependencies:[ "std" ] "app"; make_package ~dependencies:[ "missing-lib" ] "unrelated" ] in
  match plan_workspace workspace (Package (package_name "app")) Runtime with
  | Error err ->
      Error (
        "expected targeted runtime plan to ignore unrelated missing dependencies, got " ^ (
          ...
          | PackagesNotFound { names; _ } -> "PackagesNotFound(" ^ String.concat "," (List.map names ~fn:Package_name.to_string) ^ ")"
          | CycleDetected { cycle } -> "CycleDetected(" ^ String.concat "->" cycle ^ ")"
          | MissingDependencies { missing } -> "MissingDependencies(" ^ String.concat "," (List.map missing ~fn:(
            fun (item: Riot_planner.Package_graph.missing_dependency) -> item.package ^ "->" ^ item.dependency
          )) ^ ")"
