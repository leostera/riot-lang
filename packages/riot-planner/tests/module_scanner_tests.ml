...
let rec flatten_entries = fun entries -> List.flat_map entries ~fn:(
  function
...
let test_scan_tags_c_and_h_files = fun _ctx ->
  match Fs.with_tempdir ~prefix:"module_scanner_test"
    (
      fun tmpdir ->
        let src_dir = Path.(tmpdir / Path.v "src") in
        let _ = Fs.create_dir_all src_dir |> Result.expect ~msg:"create src failed" in
        let _ = Fs.write "int stub(void) { return 0; }" Path.(src_dir / Path.v "stubs.c") |> Result.expect ~msg:"write c source failed" in
        ...
        let has_c = List.any flat ~fn:(
          function
          | Riot_planner.Module_scanner.C (name, path) -> String.equal name "stubs.c" && Path.equal path Path.(Path.v "src/stubs.c")
          | _ -> false
        ) in
        let has_h = List.any flat ~fn:(
          function
          | Riot_planner.Module_scanner.H (name, path) -> String.equal name "stubs.h" && Path.equal path Path.(Path.v "src/stubs.h")
          | _ -> false
        ) in
        if has_c && has_h then
          Ok ()
...
let main ~args = Test.Cli.main ~name:"module_scanner_tests" ~tests ~args ()
