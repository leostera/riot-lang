...
let dependency = fun name -> Package.{
...
let make_package = fun ?(dependencies = []) ?(dev_dependencies = []) ?(build_dependencies = []) name -> Package.make ~name:(package_name name) ~path:(Path.v ("packages/" ^ name)) ~relative_path:(Path.v ("packages/" ^ name)) ~dependencies:(List.map dependencies ~fn:dependency) ~dev_dependencies:(List.map dev_dependencies ~fn:dependency) ~build_dependencies:(List.map build_dependencies ~fn:dependency) ~library:{ path = Path.v "src/lib.ml" } ()

let make_workspace = fun packages -> Workspace.make_realized ~root:(Path.v "/tmp/workspace_like_graph_tests") ~packages ()

let node_for = fun graph package_name scope -> Package_graph.package_key ~package_name scope |> Package_graph.get_node_by_key graph |> Option.expect ~msg:("missing node: " ^ package_name)

let dependency_keys_for_node = fun graph node -> Package_graph.get_dependencies_for_node graph node |> List.map ~fn:Package_graph.get_key |> List.sort ~compare:Package.key_compare

let package_keys_for_scope = fun scope names -> names |> List.map ~fn:(
  fun name -> Package_graph.package_key ~package_name:name scope
) |> List.sort ~compare:Package.key_compare

let assert_same_keys = fun ~expected ~actual -> Test.assert_equal ~expected:(List.sort expected ~compare:Package.key_compare) ~actual:(List.sort actual ~compare:Package.key_compare)
...
let dev_scope_does_not_inherit_build_only_dependencies = fun _ctx ->
  let packages =
    [
      ...
      make_package ~dependencies:[ "runtime-lib" ] ~dev_dependencies:[ "propane" ] ~build_dependencies:[ "codegen" ] "app";
    ]
  in
  let workspace = make_workspace packages in
  ...
  assert_same_keys ~expected:(package_keys_for_scope Runtime [ "propane"; "app" ]) ~actual:(dependency_keys_for_node graph app_dev);
  Ok ()

let missing_workspace_dependencies_are_reported = fun _ctx ->
  let packages = [ make_package ~dependencies:[ "missing_a" ] "left"; make_package ~dependencies:[ "missing_b" ] "right" ] in
  let workspace = make_workspace packages in
  ...
  | Error (Package_graph.MissingPackages { missing }) ->
      let missing_pairs = List.map missing ~fn:(
...
let filter_for_package_keeps_only_transitive_dependencies = fun _ctx ->
  ...
  let package_names = Package_graph.packages filtered |> List.map ~fn:(
...
let topological_sort_places_dependencies_before_dependents = fun _ctx ->
  ...
  let position_of key = List.enumerate sorted |> List.find ~fn:(
    fun (_, current) -> Package.key_equal key current
  ) |> Option.map ~fn:(
    fun (index, _) -> index
...
let runtime_nodes_with_build_dependencies_depend_on_their_build_nodes = fun _ctx ->
  let packages = [ make_package "std"; make_package "codegen"; make_package ~dependencies:[ "std" ] ~build_dependencies:[ "codegen" ] "app" ] in
  let workspace = make_workspace packages in
  let graph = Package_graph.create ~scope:Runtime workspace |> Result.expect ~msg:"expected runtime graph" in
...
let scope_node_counts_match_expected_projection = fun _ctx ->
  let packages = [ make_package "std"; make_package ~dependencies:[ "std" ] "kernel"; make_package ~dependencies:[ "kernel" ] "app" ] in
  let workspace = make_workspace packages in
...
let get_unplanned_dependencies_only_reports_unplanned_runtime_dependencies = fun _ctx ->
  ...
  Package_graph.mark_planned graph std_runtime_key ~module_graph:(Graph.SimpleGraph.make ()) ~action_graph:(Riot_planner.Action_graph.create ()) ~hash:(Crypto.hash_string "std-runtime");
  let unplanned = Package_graph.get_unplanned_dependencies graph app in
  Test.assert_equal ~expected:(List.map [ "kernel" ] ~fn:package_name) ~actual:(List.map unplanned ~fn:(
    fun (pkg: Package.t) -> pkg.name
  ));
  Ok ()
