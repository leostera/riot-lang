...
let write_package_fixture_files = fun package_root package_name ->
  ...
  write_file
    ...
    "type t = Token_stream of string list\n";
  write_file
    Path.(package_root / Path.v "src" / Path.v "utils.ml")
    "let normalize input = input\n";
  write_file
...
let make_package_fixture = fun root label ->
  ...
  let _ = Fs.create_dir_all Path.(package_root / Path.v "src")
  |> Result.expect ~msg:"create planner bench src dir should succeed" in
  ...
  { workspace; package; store; build_ctx; package_key }
...
let make_plan_package_cold_bench = fun root ->
  ...
  let fixtures =
    Array.init
      ~count:fixture_count
      ~fn:(fun index ->
        make_package_fixture root ("cold-package-" ^ Int.to_string index))
  in
  let cursor = ref 0 in
  fun () ->
...
let () =
  Runtime.run
    ~main:(fun ~args ->
      match
        Fs.with_tempdir ~prefix:"riot_planner_bench"
          (fun root ->
            Bench.Cli.main
