...
let template = fun (modules: Module.t list) ->
  ...
  let unique_modules = modules |> List.filter_map ~fn:(
    fun mod_ ->
      match Module.kind mod_ with
      | `interface -> None
      | `implementation ->
          let name = Module.module_name mod_ |> Module_name.to_string in
          let ns = Module.namespaced_name mod_ in Some (name, ns)
  ) |> List.sort ~compare:(
...
let make_node = fun (ns: Namespace.t) (modules: Module.t list) ->
  ...
  let kind = Module_node.ML mod_ in { Module_node.file; open_modules = []; kind }
