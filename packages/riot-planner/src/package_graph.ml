  ...
  List.for_each workspace.packages
    ~fn:(fun (pkg: Package_manifest.t) ->
      ...
      (
        match HashMap.get
          name_to_node
          ~key:(package_key ~package_name:(Package_name.to_string pkg.name) Runtime), HashMap.get
          name_to_node
          ~key:(package_key ~package_name:(Package_name.to_string pkg.name) Build) with
      ...
      (
        match HashMap.get
          name_to_node
          ~key:(package_key ~package_name:(Package_name.to_string pkg.name) Dev), HashMap.get
          name_to_node
          ~key:(package_key ~package_name:(Package_name.to_string pkg.name) Runtime) with
