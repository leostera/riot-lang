...
val plan_workspace: workspace:Workspace.t -> target:build_target -> scope:Package_graph.build_scope -> load_errors:Workspace_manager.load_error list -> dev_artifacts:Package_graph.dev_artifacts -> (workspace_plan_result, plan_error) result
...
*)
val plan_package_with_graph: workspace:Workspace.t -> toolchain:Riot_toolchain.t -> store:Riot_store.Store.t -> package_graph:Package_graph.t -> package_key:Package.key -> package:Package.t -> build_ctx:Build_ctx.t -> (package_plan_result, Planning_error.t) result
