...
let topo_sort = fun t ->
  ...
  | Ok sorted -> sorted
  | Error _cycle_ids -> (* Should never happen - cycles caught in module planning *)
...
let to_action_list = fun t ->
  let sorted = topo_sort t in sorted |> List.map ~fn:(
    fun (node: Action_node.t) -> node.value.actions
  ) |> List.concat

let hash_action_node = fun _t (node: Action_node.t) -> node.value.hash

let opens = fun mods -> List.filter_map mods ~fn:(
  fun (node: Module_node.t G.node) ->
    match node.value.kind with
    | ML mod_ | MLI mod_ -> Some (Riot_toolchain.Ocamlc.Open (Module.namespaced_name mod_))
    | _ -> None
)
...
let stdlib_flags = fun (package: Package.t) ->
  ...
  let has_stdlib_dep = List.any (Package.build_graph_dependencies package) ~fn:(
    fun (dep: Package.dependency) -> Package_name.equal dep.name (Package_name.from_string "stdlib" |> Result.expect ~msg:"expected valid package name")
  ) in
  ...
  else [ Riot_toolchain.Ocamlc.NoPervasives; Riot_toolchain.Ocamlc.NoStdlib ]
...
let profile_compile_flags = fun (profile: Profile.t) ->
  ...
  let flags =
    if List.is_empty profile.warnings then
      flags
    else flags @ [ Riot_toolchain.Ocamlc.Warning profile.warnings ]
  in
  let flags =
    if List.is_empty profile.errors then
      flags
    else flags @ [ Riot_toolchain.Ocamlc.WarnError profile.errors ]
  in
  let compare_compiler_flag left right =
    ...
    in
    compare_flag_parts (Riot_toolchain.Ocamlc.flags_to_string [ left ]) (Riot_toolchain.Ocamlc.flags_to_string [ right ])
  in
  List.unique ((flags @ List.map profile.open_modules ~fn:(
    fun mod_name -> Riot_toolchain.Ocamlc.Open mod_name
  )) @ List.map profile.ocamlc_flags ~fn:(
    fun flag -> Riot_toolchain.Ocamlc.Raw flag
  )) ~compare:compare_compiler_flag

let module_to_actions ~package ~profile ~ctx ~dep_includes ~get_dep_outputs ~get_dep_kind ~depset ~needs_unix ~needs_dynlink (module_node: Module_node.t) (deps: G.Node_id.t list): Action.t list * Path.t list * Path.t list =
  ...
  | { kind = ML mod_; file = Generated { path; contents }; open_modules; _ } ->
      ...
      let flags =
        if is_alias_file then
          base_compile_flags @ (Riot_toolchain.Ocamlc.Impl path :: Riot_toolchain.Ocamlc.NoAliasDeps :: opens open_modules)
        else base_compile_flags @ opens open_modules
      in
      let compile_action =
        Action.CompileImplementation {
          source = path;
  ...
  | { kind = MLI mod_; file = Generated { path; contents }; open_modules; _ } ->
      ...
      ([ write_action; compile_action ], outputs, sources)
  | { kind = Native { files }; _ } ->
      let c_files = List.filter files ~fn:(
      ...
      let ccflags =
        ...
        | Some sysroot ->
            let sysroot_flag = "--sysroot=" ^ Path.to_string sysroot in sysroot_flag :: base_ccflags
        | None -> base_ccflags
      in
      Log.debug ("[ACTION_GRAPH] C compilation cc_flags for " ^ Package_name.to_string package.name ^ ": " ^ String.concat " " ccflags);
      let actions = List.map c_files ~fn:(
        fun c_file ->
          let obj_file = Path.remove_extension c_file |> Path.add_extension ~ext:"o" in
          let output_name = Path.basename obj_file |> Path.v in Action.CompileC { source = c_file; outputs = [ output_name ]; ccflags }
      ) in
      let outputs = List.map c_files ~fn:(
        fun c_file ->
          let obj_file = Path.remove_extension c_file |> Path.add_extension ~ext:"o" in Path.basename obj_file |> Path.v
      ) in (actions, outputs, files)
  | { kind = C; file = Concrete _; _ } | { kind = C; file = Generated _; _ } | { kind = H; _ } | { kind = Root; _ } | { kind = PackageDependency _; _ } | { kind = Other _; _ } -> [], [], []
  | { kind = Library { name; includes }; _ } ->
      ...
      let sources = [] in
      let objects_with_duplicates = List.map deps ~fn:(
        fun dep_id ->
          let dep_outputs = get_dep_outputs dep_id in
          match get_dep_kind dep_id with
          | Some (Module_node.Native _) -> List.filter dep_outputs ~fn:(
            fun output -> Path.extension output = Some ".o"
          )
          | _ -> List.filter dep_outputs ~fn:(
      ...
      *)
      ...
      let objects = List.filter_map objects_with_duplicates ~fn:(
        fun object_path ->
      ...
      let create_lib = Action.CreateLibrary { outputs = [ library_name; archive_name ]; objects; includes } in
      let all_outputs = [ library_name; archive_name ] in ([ create_lib ], all_outputs, sources)
  | { kind = Binary { name; source; libraries; includes }; _ } ->
      ...
      let objects_with_duplicates = List.map deps ~fn:(
        fun dep_id ->
          let dep_outputs = get_dep_outputs dep_id in
          match get_dep_kind dep_id with
          | Some (Module_node.ML _) -> List.filter dep_outputs ~fn:(
            fun output -> Path.extension output = Some ".cmx"
          )
          | _ -> []
      ) |> List.concat in
      ...
      let cclibs = List.map package.foreign_dependencies ~fn:(
        fun (fdep: Package.foreign_dependency) -> (* Make foreign outputs absolute by joining with foreign dep path and normalizing *)
        List.map fdep.outputs ~fn:(
          fun out -> Path.normalize (Path.join fdep.path out)
        )
      ) |> List.concat in
      ...
      *)
      ...
      let dep_ldflags = List.map transitive_deps ~fn:(
        fun (dep: Dependency.t) ->
          match List.find dep.package.compiler.target_overrides ~fn:(
            fun (platform, _) -> String.equal platform target_platform
          ) with
      ...
      Log.debug ("[ACTION_GRAPH] Package " ^ Package_name.to_string package.name ^ ": base ld_flags = [" ^ String.concat ", " base_ldflags ^ "]");
      Log.debug ("[ACTION_GRAPH] Package " ^ Package_name.to_string package.name ^ ": dependency ld_flags = [" ^ String.concat ", " dep_ldflags ^ "]");
      Log.debug ("[ACTION_GRAPH] Package " ^ Package_name.to_string package.name ^ ": merged ld_flags = [" ^ String.concat ", " merged_ldflags ^ "]");
      (* Add sysroot if cross-compiling *)
      let ccflags =
        match Build_ctx.sysroot ctx with
        | Some sysroot ->
      ...
      let ldflags =
        match Build_ctx.sysroot ctx with
        | Some sysroot ->
            let sysroot_flag = "--sysroot=" ^ Path.to_string sysroot in sysroot_flag :: merged_ldflags
        | None -> merged_ldflags
      in
...
let from_module_graph ?analyzed_modules ~package ~profile ~ctx ~toolchain ~store ~depset ~needs_unix ~needs_dynlink (module_graph: Module_node.t G.t): t * Path.t list =
  let transitive_deps = Dependency.transitive_closure depset in
  (* Extract dependency cache include paths - no file copying needed! *)
  let dep_cache_includes = List.map transitive_deps ~fn:(
  ...
  let sorted_modules =
    ...
    | Ok sorted -> sorted
    | Error _cycle_ids -> (* Cycle should have been caught earlier in module planning *)
  ...
  let () =
    ...
    | Some analyzed_modules -> List.for_each analyzed_modules ~fn:(
      fun (node_id, analyzed_module) ->
  ...
  let library_root_candidates =
    if Option.is_none package.Package.library then
      []
    else List.filter sorted_modules ~fn:(
      fun (node: Module_node.t G.node) ->
        match node.value.kind with
        | Module_node.ML mod_ | Module_node.MLI mod_ -> String.equal (Module.module_name mod_ |> Module_name.to_string) package_namespace
        | _ -> false
    )
  in
  let concrete_library_root_modules = List.filter library_root_candidates ~fn:(
    fun (node: Module_node.t G.node) ->
  ...
  let library_root_modules =
    ...
    else concrete_library_root_modules
  in
  let same_module_interface_dependency_ids (node: Module_node.t G.node) =
    match node.value.kind with
    | Module_node.ML mod_ ->
        let qualified_name = Module.namespaced_name mod_ in List.filter node.deps ~fn:(
          fun dep_id ->
            match G.get_node module_graph dep_id with
            | Some dep_node -> (
              match dep_node.value.kind with
              | Module_node.MLI dep_mod -> String.equal (Module.namespaced_name dep_mod) qualified_name
              | _ -> false
            )
            | None -> false
        )
  ...
  let concrete_module_dependency_ids (node: Module_node.t G.node) =
    ...
    | Some analyzed_module -> analyzed_module.Module_graph.resolved_dep_ids |> List.filter ~fn:(
      fun dep_id ->
        match G.get_node module_graph dep_id with
        | Some dep_node -> (
  ...
  let concrete_reachability_dependency_ids (node: Module_node.t G.node) =
    ...
    | (Module_node.ML _ | Module_node.MLI _), Module_node.Concrete _ ->
        ...
        let () = List.for_each (concrete_module_dependency_ids node) ~fn:(
          fun dep_id ->
            let _ = HashSet.insert semantic_dep_ids ~value:dep_id in ()
        ) in
        let same_interface_dep_ids = HashSet.create () in
        let () = List.for_each (same_module_interface_dependency_ids node) ~fn:(
          fun dep_id ->
            let _ = HashSet.insert same_interface_dep_ids ~value:dep_id in ()
        ) in List.filter node.deps ~fn:(
          fun dep_id ->
            match G.get_node module_graph dep_id with
            | Some dep_node -> (
              match dep_node.value.kind, dep_node.value.file with
              | (Module_node.Root | Module_node.PackageDependency _), _ -> false
              | (Module_node.ML _ | Module_node.MLI _), Module_node.Concrete _ -> HashSet.contains semantic_dep_ids ~value:dep_id || HashSet.contains same_interface_dep_ids ~value:dep_id
              | (Module_node.ML _ | Module_node.MLI _), Module_node.Generated _ -> true
              | _ -> true
            )
            | None -> false
        )
    | (Module_node.ML _ | Module_node.MLI _), Module_node.Generated _ -> List.filter node.deps ~fn:(
      fun dep_id ->
        match G.get_node module_graph dep_id with
        | Some dep_node -> (
          match dep_node.value.kind, dep_node.value.file with
  ...
  let concrete_semantic_reachable_set start_nodes =
    ...
    let rec visit node_id =
      if HashSet.insert visited ~value:node_id then
        ...
        | None -> ()
    in
    let () = List.for_each start_nodes ~fn:(
      fun (node: Module_node.t G.node) -> visit node.id
    ) in visited
  in
  let concrete_implementation_counterpart_ids (node: Module_node.t G.node) =
    match node.value.kind, node.value.file with
    | Module_node.MLI mod_, Module_node.Concrete _ ->
        let qualified_name = Module.namespaced_name mod_ in List.filter_map sorted_modules ~fn:(
          fun (candidate: Module_node.t G.node) ->
            match candidate.value.kind, candidate.value.file with
            | Module_node.ML candidate_mod, Module_node.Concrete _ when String.equal (Module.namespaced_name candidate_mod) qualified_name -> Some candidate.id
            | _ -> None
        )
    | _ -> []
  in
  let generated_library_interface_public_seed_ids = List.filter library_root_candidates ~fn:(
    fun (node: Module_node.t G.node) ->
      match node.value.kind, node.value.file with
      | Module_node.MLI _, Module_node.Generated _ -> true
      | _ -> false
  ) |> List.map ~fn:(
    fun (node: Module_node.t G.node) -> List.map node.deps ~fn:(
      fun dep_id ->
        match G.get_node module_graph dep_id with
        | Some dep_node -> (
          match dep_node.value.kind, dep_node.value.file with
          | Module_node.MLI _, Module_node.Concrete _ -> dep_id :: concrete_implementation_counterpart_ids dep_node
          | _ -> []
        )
        | None -> []
    ) |> List.concat
  ) |> List.concat in
  let concrete_reachable_set_from_ids start_ids = List.filter_map start_ids ~fn:(
    fun node_id -> G.get_node module_graph node_id
  ) |> concrete_semantic_reachable_set in
  let library_concrete_reachable_set =
    let library_seed_ids = List.map concrete_library_root_modules ~fn:(
      fun (node: Module_node.t G.node) -> node.id
    ) @ generated_library_interface_public_seed_ids in
    if List.is_empty library_seed_ids then
      HashSet.create ()
    else concrete_reachable_set_from_ids library_seed_ids
  in
  let generated_deps_for_scope concrete_scope_set dep_ids = List.filter dep_ids ~fn:(
    fun dep_id ->
      match G.get_node module_graph dep_id with
      | Some dep_node -> (
        match dep_node.value.kind, dep_node.value.file with
  ...
  let scoped_module_deps concrete_scope_set (node: Module_node.t G.node) =
    match node.value.kind, node.value.file with
    | (Module_node.ML _ | Module_node.MLI _), Module_node.Concrete _ -> concrete_module_deps_for_scope node
    | (Module_node.ML _ | Module_node.MLI _), Module_node.Generated _ -> generated_deps_for_scope concrete_scope_set node.deps
    | _ -> node.deps
  in
  let scoped_traversal_deps concrete_scope_set (node: Module_node.t G.node) =
    if List.is_empty concrete_library_root_modules then
      node.deps
    else scoped_module_deps concrete_scope_set node
  in
  let traversal_deps (node: Module_node.t G.node) = scoped_traversal_deps library_concrete_reachable_set node in
    ...
    if List.is_empty library_root_modules then
      ...
      let rec visit node_id =
        if HashSet.insert visited ~value:node_id then
          ...
          | None -> ()
      in
      let () = List.for_each library_root_modules ~fn:(
        fun node -> visit node.id
      ) in HashSet.to_list visited
  in
  let library_reachable_set = HashSet.create () in
  let () = List.for_each library_reachable_ids ~fn:(
    fun node_id ->
      let _ = HashSet.insert library_reachable_set ~value:node_id in ()
  ) in
  let public_root_set = HashSet.create () in
  let () = List.for_each library_root_candidates ~fn:(
    fun (node: Module_node.t G.node) ->
  ...
  let node_matches_source_path source_path (node: Module_node.t G.node) =
    ...
    match node.value.kind, node.value.file with
    | (Module_node.ML _, Module_node.Concrete path) | (Module_node.MLI _, Module_node.Concrete path) -> Path.equal (relative_to_package_root path) source_path
    | _, _ -> false
  in
  let binary_source_nodes source_path = List.filter sorted_modules ~fn:(node_matches_source_path source_path) in
  let binary_concrete_reachable_set source_path = concrete_semantic_reachable_set (binary_source_nodes source_path) in
  let binary_scope_set source_path =
    let scope_set = HashSet.create () in
    let () = List.for_each (HashSet.to_list library_concrete_reachable_set) ~fn:(
      fun node_id ->
        let _ = HashSet.insert scope_set ~value:node_id in ()
    ) in
    let () = List.for_each (HashSet.to_list (binary_concrete_reachable_set source_path)) ~fn:(
      fun node_id ->
        let _ = HashSet.insert scope_set ~value:node_id in ()
    ) in scope_set
  in
  let binary_private_dependency_ids source_path =
    ...
    let rec visit node_id =
      if HashSet.insert visited ~value:node_id then
        ...
        | None -> ()
    in
    let () = List.for_each (binary_source_nodes source_path) ~fn:(
      fun (node: Module_node.t G.node) -> visit node.id
    ) in List.filter_map sorted_modules ~fn:(
      fun (node: Module_node.t G.node) ->
        if HashSet.contains visited ~value:node.id then
          ...
          | Module_node.Root | Module_node.PackageDependency _ -> None
          | _ ->
              if HashSet.contains library_reachable_set ~value:node.id then
                None
              else Some node.id
  ...
  let global_concrete_reachable_set =
    ...
    let () =
      if not (List.is_empty concrete_library_root_modules) then
        List.for_each (HashSet.to_list library_concrete_reachable_set) ~fn:(
          fun node_id ->
            let _ = HashSet.insert scope_set ~value:node_id in ()
        )
    in
    let () = List.for_each sorted_modules ~fn:(
      fun (node: Module_node.t G.node) ->
        match node.value.kind with
        | Module_node.Binary { source; _ } -> List.for_each (HashSet.to_list (binary_concrete_reachable_set source)) ~fn:(
          fun node_id ->
            let _ = HashSet.insert scope_set ~value:node_id in ()
        )
        | _ -> ()
  ...
  let effective_deps (node: Module_node.t G.node) =
    ...
    | Module_node.Library _ -> List.filter node.deps ~fn:(
      fun dep_id ->
        match G.get_node module_graph dep_id with
        | Some dep_node -> (
          match dep_node.value.kind with
          | Module_node.ML _ | Module_node.MLI _ -> HashSet.contains library_reachable_set ~value:dep_id
          | Module_node.Root | Module_node.PackageDependency _ -> false
          | _ -> true
        )
        | None -> false
    )
    | Module_node.Binary { source; _ } -> binary_private_dependency_ids source @ List.filter node.deps ~fn:(
      fun dep_id ->
        match G.get_node module_graph dep_id with
        | Some dep_node -> (
          match dep_node.value.kind with
    ...
    | _ when not (List.is_empty concrete_library_root_modules) -> scoped_module_deps global_concrete_reachable_set node
    | _ -> node.deps
  in
  let target_nodes = List.filter sorted_modules ~fn:(
    fun (node: Module_node.t G.node) ->
      match node.value.kind with
      | Module_node.Library _ | Module_node.Binary _ -> true
      | _ -> false
  ) in
  let included_node_ids =
    if List.is_empty target_nodes then
      List.map sorted_modules ~fn:(
        fun (node: Module_node.t G.node) -> node.id
      )
    else
      let visited = HashSet.create () in
      let rec visit node_id =
        if HashSet.insert visited ~value:node_id then
          ...
          | None -> ()
      in
      let () = List.for_each target_nodes ~fn:(
        fun node -> visit node.id
      ) in HashSet.to_list visited
  in
  let included_node_ids_set = HashSet.create () in
  let () = List.for_each included_node_ids ~fn:(
    fun node_id ->
  ...
  let get_dep_hash dep_id =
    ...
    | Some h -> h
    | None -> panic ("Dependency hash not found for node " ^ G.Node_id.to_string dep_id ^ ". Graph not in topological order!")
  in
  ...
  List.for_each sorted_modules ~fn:(
  ...
  List.for_each sorted_modules ~fn:(
    fun (module_node: Module_node.t G.node) ->
      if HashSet.contains included_node_ids_set ~value:module_node.id then
        ...
        let actions, outputs, sources = module_to_actions ~package ~profile ~ctx ~dep_includes ~get_dep_outputs ~get_dep_kind ~depset ~needs_unix ~needs_dynlink module_node.value deps in
        let _ = HashMap.insert node_outputs ~key:module_node.id ~value:outputs in
        if actions = [] then
          let placeholder_hash = Crypto.hash_string ("no-actions:" ^ G.Node_id.to_string module_node.id) in
          let _ = HashMap.insert action_spec_hashes ~key:module_node.id ~value:placeholder_hash in ()
        else
          let action_spec = Action_node.make ~actions ~outs:outputs ~srcs:sources ~package ~toolchain ~dependency_hashes:get_dep_hash ~deps in
          let action_node = add_node action_graph action_spec in List.for_each deps ~fn:(
            fun dep_id ->
      ...
      let _ = HashMap.insert action_spec_hashes ~key:module_node.id ~value:action_spec.hash in Cell.set all_outputs (outputs @ Cell.get all_outputs)
  );
  (action_graph, Cell.get all_outputs)

let to_json = fun t -> let open Data.Json in
let all_nodes = nodes t in
let sorted_nodes = List.sort all_nodes ~compare:(
  fun (a: Action_node.t) (b: Action_node.t) -> Int.compare (G.Node_id.to_int a.id) (G.Node_id.to_int b.id)
) in
obj
  [
    "nodes", array (List.map sorted_nodes ~fn:Action_node.to_json);
  ]
      ...
      if not (len mod 2 = 0) then
        ...
        let rec fill i =
          if i >= len then
            ...
            match hex |> String.get_unchecked ~at:i |> hex_value, hex |> String.get_unchecked ~at:(i + 1) |> hex_value with
            | Some hi, Some lo ->
                let byte = Char.from_int_unchecked ((hi lsl 4) lor lo) in
                let _ = Byte_buf.set out ~at:(i / 2) ~char:byte in fill (i + 2)
...
| Some (Array node_jsons) -> (
  ...
  let parse_actions actions_json =
    ...
    | Array action_jsons -> List.fold_left action_jsons ~init:(Ok []) ~fn:(
      fun acc action_json ->
        match acc with
        | Error _ -> acc
        | Ok actions -> (
  ...
  let parse_paths paths_json =
    ...
    | Array path_jsons -> Ok (List.filter_map path_jsons ~fn:(
      function
  ...
  let parse_dependencies deps_json =
    ...
    | Array dep_jsons -> List.fold_left dep_jsons ~init:(Ok []) ~fn:(
      fun acc dep_json ->
  ...
  in
  match List.fold_left node_jsons ~init:(Ok ()) ~fn:(
    fun acc node_json ->
      match acc with
      | Error _ -> acc
      | Ok () -> (
        match get_field "package" node_json, get_field "package_path" node_json, get_field "package_relative_path" node_json, get_field "actions" node_json, get_field "outputs" node_json, get_field "sources" node_json, get_field "hash" node_json, get_field "id" node_json, get_field "dependencies" node_json with
        | (Some (String pkg_name), pkg_path_json, pkg_rel_path_json, Some actions_json, Some outputs_json, Some sources_json, Some hash_json, Some (Int legacy_id), Some deps_json) -> (
          ...
          | Error err -> Error err
          | Ok actions -> (
            match parse_hash hash_json, parse_paths outputs_json, parse_paths sources_json, parse_dependencies deps_json with
            | Ok hash, Ok outputs, Ok sources, Ok dependency_ids ->
                ...
                (
                  ...
                  | Ok package_name ->
                      let package = Package.synthetic ~name:package_name ~path:package_path ~relative_path:package_relative_path in
  ...
  | Error err -> Error err
  | Ok () ->
      Vector.iter dependencies_to_wire |> Iterator.to_list |> List.for_each ~fn:(
        fun (node, dependency_ids) -> List.for_each (List.reverse dependency_ids) ~fn:(
          fun dep_id ->
...
let equal = fun g1 g2 ->
  ...
  let nodes2 = topo_sort g2 in List.compare_lengths ~left:nodes1 ~right:nodes2 = 0 && List.all (List.zip nodes1 nodes2) ~fn:(
    fun (left, right) -> Action_node.equal left right
  )
