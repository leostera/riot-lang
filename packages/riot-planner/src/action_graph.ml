      ...
      let cclibs =
        List.map
          ...
          ~fn:(fun (fdep: Package.foreign_dependency) ->
            ...
            List.map fdep.outputs ~fn:(fun out ->
...
let from_module_graph ?analyzed_modules ~package ~profile ~ctx ~toolchain ~store ~depset ~needs_unix ~needs_dynlink (
        ...
        List.filter
          ...
          ~fn:(fun dep_id ->
            ...
            | Some dep_node -> (
                match (dep_node.value.kind, dep_node.value.file) with
                | ((Module_node.Root
                | Module_node.PackageDependency _), _) -> false
                | ((Module_node.ML _
                | Module_node.MLI _), Module_node.Concrete _) ->
  ...
  let scoped_module_deps concrete_scope_set (node: Module_node.t G.node) =
    match (node.value.kind, node.value.file) with
    | ((Module_node.ML _
    | Module_node.MLI _), Module_node.Concrete _) -> concrete_module_deps_for_scope node
    | ((Module_node.ML _
    | Module_node.MLI _), Module_node.Generated _) ->
      ...
      match List.fold_left
        ...
        ~fn:(fun acc node_json ->
          ...
          | Ok () -> (
              ...
              | (Some (String pkg_name), pkg_path_json, pkg_rel_path_json, Some actions_json, Some outputs_json, Some sources_json, Some hash_json, Some (
                Int legacy_id
              ), Some deps_json) -> (
