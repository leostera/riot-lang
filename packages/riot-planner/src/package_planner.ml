...
let module_kind_of_json = fun json ->
  let open Std.Data.Json in
    ...
    | Object _ -> (
        ...
        | Some (String "binary") -> (
            match get_field "name" json, get_field "source" json, get_field "libraries" json, get_field
              "includes"
              json with
              ...
              List.fold_left node_jsons ~init:(Ok ())
                ~fn:(fun acc node_json ->
                  ...
                  | Ok () -> (
                      ...
                      | Object _ -> (
                          match get_field "id" node_json, get_field "file" node_json, get_field
                            "kind"
                            node_json, get_field "deps" node_json with
                          | (Some (Int legacy_id), Some file_json, Some kind_json, Some deps_json) -> (
                              match file_of_json file_json, module_kind_of_json kind_json, parse_int_array
                                deps_json with
...
let plan_bundle_of_json = fun ~(package:Package.t) json ->
  let open Std.Data.Json in
    match json with
    | Object _ -> (
        match get_field "version" json, get_field "package" json, get_field "module_graph" json, get_field
          "action_graph"
          json with
