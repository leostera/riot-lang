      ...
      if List.is_empty allowed_source_files then
        ...
        let root_mode =
          if Path.equal source_dir (Path.v "src") then
            ...
            | Some _ -> Module_graph.Library_root { library_name = Package_name.to_string package.name }
            | None -> Module_graph.Loose_sources
          else Module_graph.Loose_sources
        in
        Some Module_graph.{
          source_dir;
...
let file_to_json = fun (file: Module_node.file) -> let open Std.Data.Json in
match file with
...
let file_of_json = fun json -> let open Std.Data.Json in
match json with
| Object _ -> (
  ...
  | Some (String "generated"), Some (String path), Some (String contents) -> Ok (Module_node.Generated { path = Path.v path; contents })
  | _ -> Error "invalid module file payload"
)
| _ -> Error "module file must be an object"

let module_kind_to_json = fun (kind: Module_node.kind) -> let open Std.Data.Json in
match kind with
| Module_node.ML mod_ ->
    ...
    Object [
      ...
      "namespace", Array (List.map ns ~fn:(
        fun s -> String s
      ));
    ]
...
let parse_string_array = function
  | Std.Data.Json.Array xs -> List.fold_left xs ~init:(Ok []) ~fn:(
    fun acc item ->
      match acc, item with
      | Error e, _ -> Error e
...
let module_kind_of_json = fun json -> let open Std.Data.Json in
match json with
| Object _ -> (
  ...
  | Some (String "ml") -> (
    ...
    | Some (String filename), Some namespace_json -> (
      ...
      | Ok ns ->
          let mod_ = Module.make ~namespace:(Namespace.of_list ns) ~filename:(Path.v filename) in Ok (Module_node.ML mod_)
      | Error e -> Error e
    )
  ...
  | Some (String "mli") -> (
    ...
    | Some (String filename), Some namespace_json -> (
      match parse_string_array namespace_json with
      | Ok ns ->
          let mod_ = Module.make ~namespace:(Namespace.of_list ns) ~filename:(Path.v filename) in Ok (Module_node.MLI mod_)
      | Error e -> Error e
    )
  ...
  | Some (String "library") -> (
    ...
    | Some (String name), Some includes_json -> (
      ...
      | Ok includes -> Ok (Module_node.Library { name; includes = List.map includes ~fn:Path.v })
      | Error e -> Error e
    )
    | _ -> Error "invalid library kind payload"
  )
  | Some (String "binary") -> (
    match get_field "name" json, get_field "source" json, get_field "libraries" json, get_field "includes" json with
...
let module_graph_to_json = fun (module_graph: Module_node.t G.t) -> let open Std.Data.Json in
let nodes =
...
let node_to_json (node: Module_node.t G.node) =
  Object [
    ...
    "deps", Array (List.map node.deps ~fn:(
...
in
Object [
  "nodes", Array (List.map nodes ~fn:node_to_json);
]

let module_graph_of_json = fun json -> let open Std.Data.Json in
match json with
| Object _ -> (
  ...
  | Some (Array node_jsons) ->
      ...
      let parse_int_array = function
        | Array xs -> List.fold_left xs ~init:(Ok []) ~fn:(
          fun acc item ->
            match acc, item with
            | Error e, _ -> Error e
      ...
      in
      let result = List.fold_left node_jsons ~init:(Ok ()) ~fn:(
        fun acc node_json ->
          match acc with
          | Error _ -> acc
          | Ok () -> (
            ...
            | Object _ -> (
              match get_field "id" node_json, get_field "file" node_json, get_field "kind" node_json, get_field "deps" node_json with
              | (Some (Int legacy_id), Some file_json, Some kind_json, Some deps_json) -> (
                match file_of_json file_json, module_kind_of_json kind_json, parse_int_array deps_json with
      ...
      (
        ...
        | Error e -> Error e
        | Ok () ->
            Vector.iter pending_deps |> Iterator.to_list |> List.for_each ~fn:(
              fun (node, dep_ids) -> List.for_each (List.reverse dep_ids) ~fn:(
                fun dep_id ->
...
let plan_bundle_of_json = fun ~(package:Package.t) json -> let open Std.Data.Json in
match json with
| Object _ -> (
  match get_field "version" json, get_field "package" json, get_field "module_graph" json, get_field "action_graph" json with
  | Some (Int 1), Some (String pkg_name), Some module_graph_json, Some action_graph_json when String.equal pkg_name (Package_name.to_string package.name) -> (
...
let compute_input_hash = fun ?(planner_version = "planner-artifacts:v18") ~package ~depset ~workspace ~profile ~build_ctx ~toolchain () ->
  ...
  let sorted_deps = List.sort (Package.build_graph_dependencies package) ~compare:(
    fun (a: Package.dependency) (b: Package.dependency) -> Package_name.compare a.name b.name
  ) in
  List.for_each sorted_deps ~fn:(
    fun (dep: Package.dependency) ->
      ...
      | { Package.workspace = true; _ } -> (
        match List.find workspace.Workspace.packages ~fn:(
          fun (p: Package_manifest.t) -> Package_name.equal p.name dep.name
        ) with
        | Some dep_pkg -> (
  ...
  (* Dependency hashes *)
  let dep_hashes = depset |> List.map ~fn:(
    fun (dep: Dependency.t) -> dep.hash
  ) |> List.sort ~compare:Std.Crypto.Hash.compare in
  List.for_each dep_hashes ~fn:(
    fun hash -> H.write_hash state hash
  );
  H.finish state
...
let check_dependencies_built = fun ~store ~package_graph ~package_key ->
  let current_package_name, current_scope =
    ...
    | Some node -> Package_name.to_string (Package_graph.get_package node.value).Package.name, Package_graph.get_scope node.value
  ...
  let rec summarize_dependency: Package_graph.package_node -> Dependency.t option = fun node_value ->
    ...
    let is_ordering_only_self_dependency =
      String.equal (Package_name.to_string pkg.Package.name) current_package_name && match current_scope, Package_graph.get_scope node_value with
        ...
        if is_ordering_only_self_dependency then
          ...
          if Riot_store.Store.exists store hash then
            let dep_nodes =
              ...
              | None -> []
            in
            let dep_depset = List.filter_map dep_nodes ~fn:summarize_dependency in Some Dependency.{
              package;
...
let plan_package = fun ~workspace ~toolchain ~store ~package_graph ~package_key ~package ~build_ctx ->
  ...
  | Error (Failed failed) ->
      let dependency_check_duration = Time.Instant.duration_since ~earlier:dependency_check_started_at (Time.Instant.now ()) in Ok (FailedDependencies { package; failed; breakdown = { empty_breakdown with dependency_count = List.length failed; dependency_check_duration } })
  | Error (Missing missing) ->
      let dependency_check_duration = Time.Instant.duration_since ~earlier:dependency_check_started_at (Time.Instant.now ()) in Ok (MissingDependencies { package; missing; breakdown = { empty_breakdown with dependency_count = List.length missing; dependency_check_duration } })
  | Ok depset ->
      let dependency_check_duration = Time.Instant.duration_since ~earlier:dependency_check_started_at (Time.Instant.now ()) in
      ...
      let profile =
        ...
        Log.info ("Package " ^ Package_name.to_string package.name ^ ": looking for target." ^ target_platform ^ " overrides");
        Log.info ("Available targets: [" ^ (String.concat ", " (List.map package.compiler.target_overrides ~fn:(
          fun (target, _) -> target
        ))) ^ "]");
        match List.find package.compiler.target_overrides ~fn:(
          fun (target, _) -> String.equal target target_platform
        ) with
        ...
        | None ->
            Log.warn ("No target." ^ target_platform ^ " override found for package " ^ Package_name.to_string package.name);
            profile
      in
      ...
      let input_hash_duration = Time.Instant.duration_since ~earlier:input_hash_started_at (Time.Instant.now ()) in
      ...
      let cached_artifact =
        ...
        | _ -> None
      in
      let artifact_lookup_duration = Time.Instant.duration_since ~earlier:artifact_lookup_started_at (Time.Instant.now ()) in
      match cached_artifact with
      | Some (artifact, exports) ->
          Log.info ("Package " ^ Package_name.to_string package.name ^ ": cache hit via artifact + export metadata");
          Ok (
            Cached {
              package_key;
              package;
      ...
      | None -> (
        ...
        | Some json ->
            let plan_bundle_lookup_duration = Time.Instant.duration_since ~earlier:plan_bundle_lookup_started_at (Time.Instant.now ()) in
            let plan_bundle_decode_started_at = Time.Instant.now () in
            let parsed_bundle =
              try plan_bundle_of_json ~package json with
              | exn ->
                  Log.warn ("Package " ^ Package_name.to_string package.name ^ ": plan bundle decode raised exception, rebuilding plan graph (" ^ Exception.to_string exn ^ ")");
                  Error "plan bundle decode exception"
            in
            let plan_bundle_decode_duration = Time.Instant.duration_since ~earlier:plan_bundle_decode_started_at (Time.Instant.now ()) in
            (
              match parsed_bundle with
              | Ok (module_graph, action_graph) ->
                  Log.info ("Package " ^ Package_name.to_string package.name ^ ": plan bundle cache hit");
                  Ok (
                    Planned {
                      package_key;
                      package;
              ...
              | Error _ ->
                  Log.warn ("Package " ^ Package_name.to_string package.name ^ ": plan bundle parse failed, rebuilding plan graph");
                  let module_plan_started_at = Time.Instant.now () in
                  let plan_input = Module_planner.{
                    package;
                    profile;
                    ctx = build_ctx;
                    toolchain;
                  ...
                  | Ok { sources; module_graph; analyzed_modules = _; action_graph } ->
                      ...
                      let foreign_nodes = List.map package.foreign_dependencies ~fn:(
                        fun (fdep: Package.foreign_dependency) ->
                          ...
                          in
                          let foreign_node = Action_node.make ~actions:[ foreign_action ] ~outs:fdep.outputs ~srcs:[] ~package ~toolchain ~dependency_hashes:(
                            fun _ -> Crypto.hash_string ""
                          ) ~deps:[] in Action_graph.add_node action_graph foreign_node
                      ) in
                      (* Make all existing nodes depend on foreign dependency nodes *)
                      if List.length foreign_nodes > 0 then
                        (
                          let foreign_node_ids = List.map foreign_nodes ~fn:(
                            fun (node: Action_node.t) -> node.id
                          ) in
                      ...
                      let _ = Riot_store.Store.save_plan_bundle store ~hash:input_hash ~plan:(plan_bundle_to_json ~package ~module_graph ~action_graph) in
                      let module_plan_duration = Time.Instant.duration_since ~earlier:module_plan_started_at (Time.Instant.now ()) in
                      Ok (
                        Planned {
                          package_key;
                          package;
