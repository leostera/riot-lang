...
let insert = fun table key node_id ->
  ...
  | None ->
      let _ = HashMap.insert table key [ node_id ] in
      ()
  | Some ids ->
      let _ = HashMap.insert table key (node_id :: ids) in
      ()
...
let get_by_name = fun t name ->
  match HashMap.get t.by_name name with
  | None -> raise Not_found
  | Some ids -> ids

let get_by_qualified_name = fun t name ->
  match HashMap.get t.by_qualified_name name with
  | None -> raise Not_found
  | Some ids -> ids
