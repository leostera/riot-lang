...
let filter_workspace_for_target = fun ~(workspace:Workspace.t) ~target ~(scope:Package_graph.build_scope) ->
  ...
  | Packages _ ->
      ...
      let rec visit = function
        | [] ->
            ()
        | pkg_name :: rest when HashSet.contains seen ~value:pkg_name ->
            visit rest
        | pkg_name :: rest ->
...
let plan_workspace = fun ~(workspace:Workspace.t) ~target ~(scope:Package_graph.build_scope) ~load_errors ~dev_artifacts ->
  ...
  if List.length load_errors > 0 then
    ...
    if not (List.is_empty missing_targets) then
      ...
      let manifest_filter_duration = Time.Instant.duration_since
        ~earlier:manifest_filter_started_at
        (Time.Instant.now ()) in
      (
        ...
        | Ok (package_graph, package_graph_create_breakdown) ->
            let package_graph_duration = Time.Instant.duration_since
              ~earlier:package_graph_started_at
              (Time.Instant.now ()) in
            ...
            in
            let target_graph_filter_duration = Time.Instant.duration_since
            ...
            | Ok graph -> (
                ...
                in
                let topological_sort_duration = Time.Instant.duration_since
