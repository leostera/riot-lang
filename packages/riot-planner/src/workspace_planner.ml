...
let target_missing_package_names = fun ~(workspace: Workspace.t) target ->
  ...
  List.for_each workspace.packages ~fn:(fun (pkg: Package_manifest.t) ->
        ...
        (
          ...
          | Ok (package_graph, package_graph_create_breakdown) ->
              ...
              let target_graph_filter_duration =
                Time.Instant.duration_since ~earlier:target_graph_filter_started_at (Time.Instant.now ())
              in
              ...
              | Ok graph -> (
                  ...
                  let topological_sort_duration =
                    Time.Instant.duration_since ~earlier:topological_sort_started_at (Time.Instant.now ())
                  in
                  ...
                  | Ok nodes ->
                      ...
                      Ok { packages; nodes; package_graph = graph; workspace; breakdown }
                )
        )
