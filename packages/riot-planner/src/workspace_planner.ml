...
let manifest_dependency_names_for_scope = fun (scope: Package_graph.build_scope) (pkg: Package_manifest.t) ->
  ...
  | Package_graph.Dev -> List.concat [ List.map pkg.dependencies ~fn:dependency_name; List.map pkg.dev_dependencies ~fn:dependency_name ]
...
let target_missing_package_names = fun ~(workspace:Workspace.t) target ->
  ...
  target_package_names target |> List.filter ~fn:(
    fun pkg_name -> not (HashSet.contains available_set ~value:pkg_name)
  ) |> List.sort ~compare:Package_name.compare |> List.unique ~compare:Package_name.compare

let filter_workspace_for_target = fun ~(workspace:Workspace.t) ~target ~(scope:Package_graph.build_scope) ->
  match target with
  | All -> workspace
  | Package _ | Packages _ ->
      ...
      let () = visit initial_targets in { workspace with packages = List.filter workspace.packages ~fn:(
        fun (pkg: Package_manifest.t) -> HashSet.contains seen ~value:pkg.name
      ) }
...
let plan_workspace = fun ~(workspace:Workspace.t) ~target ~(scope:Package_graph.build_scope) ~load_errors ~dev_artifacts ->
  ...
  if List.length load_errors > 0 then
    ...
    let available = List.map workspace.packages ~fn:(
    ...
    if not (List.is_empty missing_targets) then
      ...
      let manifest_filter_duration = Time.Instant.duration_since ~earlier:manifest_filter_started_at (Time.Instant.now ()) in
      (
        ...
        | Ok (package_graph, package_graph_create_breakdown) ->
            let package_graph_duration = Time.Instant.duration_since ~earlier:package_graph_started_at (Time.Instant.now ()) in
            let target_graph_filter_started_at = Time.Instant.now () in
            let target_graph =
              let filter_packages pkg_names =
                let missing = List.filter pkg_names ~fn:(
                  fun pkg_name -> Option.is_none (Package_graph.find_package package_graph pkg_name)
                ) |> List.sort ~compare:Package_name.compare |> List.unique ~compare:Package_name.compare in
                match missing with
            ...
            in
            let target_graph_filter_duration = Time.Instant.duration_since ~earlier:target_graph_filter_started_at (Time.Instant.now ()) in
            ...
            | Ok graph -> (
              ...
              in
              let topological_sort_duration = Time.Instant.duration_since ~earlier:topological_sort_started_at (Time.Instant.now ()) in
