          ...
          | Some cursor -> (
              ...
              | Some (method_, line_cursor) ->
                  ...
                  | Some (path, line_cursor) ->
                      let version = StringCursor.skip_while line_cursor (fun c -> c = ' ')
                      |> StringCursor.remaining in
    ...
    | Some (line, cursor) -> (
        ...
        | Some cursor -> (
            ...
            | Some (name, line_cursor) -> (
                ...
                | Some line_cursor ->
                    let value = StringCursor.skip_while line_cursor (fun c -> c = ' ' || c = '\t')
                    |> StringCursor.remaining in
                    let name = StringCursor.skip_while
                      (StringCursor.create name)
                      (fun c -> c = ' ' || c = '\t')
                    |> StringCursor.remaining in
                    Done { value = (name, value); remaining = StringCursor.remaining cursor }
              )
          )
      )
  ...
  let parse = fun ?(max_request_line = 8_192) ?(max_headers = 100) ?(max_header_length = 8_192) input ->
    ...
    | Done { value=(method_, path, version); remaining } -> (
        ...
        | Done { value=(headers_list, body); _ } ->
            ...
            let uri = Std.Net.Uri.of_string path
            |> Result.unwrap_or ~default:(Std.Net.Uri.of_string "/" |> Result.unwrap) in
            let version = Std.Net.Http.Version.of_string version
...
module BorrowedParser = struct
  ...
  let trim_leading_ows = fun slice ->
    Cursor.skip_while (Cursor.from_slice slice) is_optional_whitespace |> Cursor.remaining
  ...
  let rec parse_headers = fun ?(max_count = 100) ?(max_length = 8_192) ?(acc = []) ?(count = 0) cursor ->
    if count >= max_count then
      ...
      | Some cursor -> Done { value = (List.rev acc, cursor); remaining = Cursor.remaining cursor }
      | None ->
          match parse_header_line cursor with
          | Need_more -> Need_more
          | Error error -> Error error
          | Done { value={ name; value; remaining=next_cursor }; _ } ->
              if Slice.length name + Slice.length value > max_length then
                ...
                parse_headers ~max_count ~max_length ~acc:((name, value) :: acc) ~count:(count + 1) next_cursor
...
let small_request = build_request
  ~method_:"GET"
  ~path:"/health"
...
let request_100k = build_request
  ~method_:"PUT"
  ~path:"/bulk"
  ~headers:[
    ("Host", "example.com");
...
let many_headers_request = build_request
  ~method_:"GET"
  ~path:"/headers"
...
let github_navigation_request =
  ...
  build_request ~method_:"GET" ~path
    ~headers:[
      ("Host", "github.com");
...
let github_navigation_request_slice = IO.IoVec.IoSlice.from_string github_navigation_request
|> Result.unwrap
