  ...
  | Done { value = parsed; _ } -> (
    ...
    if method_ != "GET" then
      ...
      if path != "/path" then
        ...
        if version != NetVersion.Http11 then
...
let test_request_with_1k_body = fun _ctx ->
  ...
  let req = build_request ~method_:"POST" ~path:"/upload" ~headers:[
    "Host", "example.com";
...
let test_request_with_100k_body = fun _ctx ->
  ...
  let req = build_request ~method_:"PUT" ~path:"/bulk" ~headers:[
    "Host", "example.com";
    "Content-Length", Int.to_string (String.length body);
  ] ~body in
  match expect_request_parse req with
...
let test_request_with_1m_body = fun _ctx ->
  ...
  let req = build_request ~method_:"PATCH" ~path:"/large" ~headers:[
    "Host", "example.com";
    "Content-Length", Int.to_string (String.length body);
  ] ~body in
  match expect_request_parse req with
...
let test_request_rejects_too_many_headers = fun _ctx ->
  let headers =
    let rec loop index acc =
      if index < 0 then
        acc
      else loop (index - 1) (("X-Test-" ^ Int.to_string index, "value") :: acc)
    in
    loop 100 []
  in
...
let test_chunk_single = fun _ctx ->
  ...
  | Done { value = chunk_result; _ } ->
      if chunk_result.data != "Hello" then
        ...
        if chunk_result.remaining != "" then
...
let tests = Test.[
  ...
  case "request_missing_lf_after_request_line_current_behavior" test_request_missing_lf_after_request_line_current_behavior;
