...
let header_arb = Arbitrary.make ~print:(
  fun h -> h.Http2.Hpack.name ^ ": " ^ h.Http2.Hpack.value
) header_gen

(* Property: HPACK encoding produces output *)
let hpack_encode_prop =
  property "HPACK encoding produces non-empty output for non-empty headers" Arbitrary.(make (Generator.non_empty_list header_gen))
    (
      fun headers ->
        let encoder = Http2.Hpack.create_encoder () in
        let encoded = Http2.Hpack.encode encoder ~sensitive_headers:[] () ~headers in IO.Bytes.length encoded > 0
    )
  ...
  property "HPACK encoding is deterministic" Arbitrary.(list header_arb)
    (
      fun headers ->
        ...
        let encoded2 = Http2.Hpack.encode encoder2 ~sensitive_headers:[] () ~headers in String.equal (IO.Bytes.to_string encoded1) (IO.Bytes.to_string encoded2)
    )
...
let frame_flags_gen = let open Generator in
map2
  (
  ...
  Generator.map
    (
      fun ((stream_id, payload_data, flags)) ->
        let length = String.length payload_data in Http2.Frame.{
          length;
          frame_type = Data;
          flags;
          stream_id;
...
let frame_length_prop =
  property "Frame serialization includes 9-byte header" Arbitrary.(make settings_frame_gen)
    (
      fun frame ->
        let serialized = Http2.Serializer.serialize_frame frame in String.length serialized >= 9
    )
...
let int_to_hex = fun n ->
  ...
  let rec loop acc value =
    ...
    else loop (String.make ~len:1 ~char:(digit (value mod 16)) ^ acc) (value / 16)
  in
  loop "" n

(* Property: Chunk encoding/decoding round-trip *)
let chunk_roundtrip_prop =
  property "HTTP/1 chunk encode/decode preserves data" Arbitrary.(make (Generator.string_size (Generator.int_range 1 50) Generator.char_printable))
    (
      fun data ->
  ...
  property "HTTP/1 request parsing succeeds for valid requests" Arbitrary.(make method_gen)
    (
      fun method_ ->
        ...
        | Done { value = parsed; _ } ->
            let parsed_method = Std.Net.Http.Request.method_ parsed |> Std.Net.Http.Method.to_string in parsed_method = method_
        | _ -> false
    )
