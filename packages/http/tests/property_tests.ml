...
let header_arb = Arbitrary.make
  ~print:(fun h -> h.Http2.Hpack.name ^ ": " ^ h.Http2.Hpack.value)
  header_gen

(* Property: HPACK encoding produces output *)
let hpack_encode_prop =
  property "HPACK encoding produces non-empty output for non-empty headers"
    Arbitrary.(list header_arb)
    (fun headers ->
      assume (List.length headers > 0);
      assume (List.for_all (fun h ->
        String.length h.Http2.Hpack.name > 0 &&
        String.length h.Http2.Hpack.value > 0
      ) headers);
...
let frame_length_prop =
  property "Frame serialization includes 9-byte header"
...
let chunk_roundtrip_prop =
  property "HTTP/1 chunk encode/decode preserves data"
    Arbitrary.(pair (make (Generator.int_range 1 50)) string)
    (fun (size, full_data) ->
      ...
      let hex_size = 
        ...
        if n < 10 then Int.to_string n
        else if n < 16 then String.make 1 (Char.chr (Char.code 'a' + n - 10))
        else Int.to_string n  (* fallback to decimal for larger *)
      in
...
let request_parse_prop =
  property "HTTP/1 request parsing succeeds for valid requests"
    ...
    (fun method_ ->
      ...
      | Done { value = parsed; _ } ->
          let parsed_method = Std.Net.Http.Request.method_ parsed 
            |> Std.Net.Http.Method.to_string in
