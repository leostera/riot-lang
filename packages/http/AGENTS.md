...
6. Keep parser-side view usage explicit about ownership. `Std.IO.StringView` and `IoBuffer` are the internal fast path; public request/response values should still materialize ordinary `string` fields at the boundary where HTTP leaves the protocol layer.
