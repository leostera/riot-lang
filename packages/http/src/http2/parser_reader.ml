...
type state = { config: config; phase: parse_phase Cell.t }
...
let create = fun ?(config = default_config) () -> { config; phase = Cell.create (ReadingFrameHeader { buffer = Buffer.create ~size:9; bytes_read = 0 }) }

let reset = fun state -> Cell.set state.phase (ReadingFrameHeader { buffer = Buffer.create ~size:9; bytes_read = 0 })
...
let parse_payload = fun frame payload_data ->
  ...
  | Frame.Settings ->
      ...
      let rec parse_settings offset acc =
        if offset >= String.length payload_data then
          ...
          if offset + 6 > String.length payload_data then
            Error Incomplete_settings_payload
          else
            let id = (byte_at payload_data offset lsl 8) lor byte_at payload_data (offset + 1) in
            let value = (byte_at payload_data (offset + 2) lsl 24) lor (byte_at payload_data (offset + 3) lsl 16) lor (byte_at payload_data (offset + 4) lsl 8) lor byte_at payload_data (offset + 5) in
            let setting_opt =
              match id with
  ...
  | Frame.WindowUpdate ->
      if String.length payload_data != 4 then
        ...
        let increment = (byte_at payload_data 0 lsl 24) lor (byte_at payload_data 1 lsl 16) lor (byte_at payload_data 2 lsl 8) lor byte_at payload_data 3 in
        let increment = increment land 0x7fff_ffff in Ok ({ frame with payload = Frame.WindowUpdatePayload increment })
  | Frame.RstStream ->
      if String.length payload_data != 4 then
        Error (Invalid_payload_length { frame_type = "RST_STREAM"; expected = 4; actual = String.length payload_data })
      else
        let code = (byte_at payload_data 0 lsl 24) lor (byte_at payload_data 1 lsl 16) lor (byte_at payload_data 2 lsl 8) lor byte_at payload_data 3 in
        let error_code =
          match code with
          | 0x0 -> Frame.NoError
  ...
  | Frame.Goaway ->
      if String.length payload_data < 8 then
        ...
        let last_stream_id = ((byte_at payload_data 0 lsl 24) lor (byte_at payload_data 1 lsl 16) lor (byte_at payload_data 2 lsl 8) lor byte_at payload_data 3) land 0x7fff_ffff in
        let error_code_int = (byte_at payload_data 4 lsl 24) lor (byte_at payload_data 5 lsl 16) lor (byte_at payload_data 6 lsl 8) lor byte_at payload_data 7 in
        let error_code =
  ...
  | Frame.Priority | Frame.PushPromise | Frame.Continuation -> (* Simplified: return placeholder *)
...
let rec parse = fun state reader ->
  ...
  | ReadingFrameHeader { buffer; bytes_read } ->
      ...
      if actual_read = 0 && bytes_read = 0 then
        ...
        if bytes_read + actual_read < 9 then
          (
  ...
  | ReadingFramePayload { header; buffer; bytes_read; total_length } ->
      ...
      if actual_read = 0 && bytes_read = 0 then
        ...
        if bytes_read + actual_read < total_length then
          (
            (* Still incomplete *)
            Cell.set state.phase
              (
