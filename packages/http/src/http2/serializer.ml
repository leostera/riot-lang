...
let flags_to_byte frame_type flags =
  let open Frame in
  let byte = 0 in
  let byte = if flags.end_stream then byte lor 0x01 else byte in
  let byte = if flags.end_headers then byte lor 0x04 else byte in
  let byte = if flags.padded then byte lor 0x08 else byte in
  let byte = if flags.priority then byte lor 0x20 else byte in
  let byte = if flags.ack then byte lor 0x01 else byte in
  byte
...
let serialize_data_payload payload =
  match payload with
...
let serialize_settings_payload payload =
  match payload with
