...
let create = fun ~role ?(config = default_config) () ->
  {role; state = Cell.create Idle; streams = HashMap.with_capacity 16; next_stream_id = Cell.create
      (
        match role with
        | Client -> 1
        | Server -> 2
      ); connection_window_size = Cell.create 65_535; local_settings = create_settings config; remote_settings = create_settings
    config; hpack_encoder = Hpack.create_encoder ~max_dynamic_table_size:4_096 (); hpack_decoder = Hpack.create_decoder
    ~max_dynamic_table_size:4_096
    (); last_stream_id = Cell.create 0; peer_last_stream_id = Cell.create 0; }
...
let process_data = fun conn data ->
  let rec process_all = fun events remaining ->
