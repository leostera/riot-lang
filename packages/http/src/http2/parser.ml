...
let read_uint24_be = fun data offset ->
  if offset + 3 > String.length data then
    ...
    let b2 = byte_at data (offset + 2) in Some ((b0 lsl 16) lor (b1 lsl 8) lor b2)
...
let read_uint32_be = fun data offset ->
  if offset + 4 > String.length data then
    ...
    let b3 = byte_at data (offset + 3) in Some ((b0 lsl 24) lor (b1 lsl 16) lor (b2 lsl 8) lor b3)
...
let read_uint16_be = fun data offset ->
  if offset + 2 > String.length data then
    ...
    let b0 = byte_at data offset in
    let b1 = byte_at data (offset + 1) in Some ((b0 lsl 8) lor b1)

let read_uint8 = fun data offset ->
  if offset >= String.length data then
    None
  else Some (byte_at data offset)
  ...
  if String.length data < 9 then
    ...
    | Some length -> (
      ...
      if length > config.max_frame_size then
        Error ("Frame size " ^ Int.to_string length ^ " exceeds maximum allowed " ^ Int.to_string config.max_frame_size)
      else
        (
    ...
    | Some pad_length ->
        if length < 1 + pad_length then
          ...
          if String.length data < length then
            ...
            let remaining = String.sub data ~offset:length ~len:(String.length data - length) in Done { value = Frame.DataPayload { data = payload_data; pad_length = Some pad_length }; remaining }
  else
    if String.length data < length then
      Need_more
    else
      let payload_data = String.sub data ~offset:0 ~len:length in
      let remaining = String.sub data ~offset:length ~len:(String.length data - length) in Done { value = Frame.DataPayload { data = payload_data; pad_length = None }; remaining }
  ...
  if length != 5 then
    ...
    if String.length data < 5 then
      ...
      | Some (stream_dependency, exclusive, weight) ->
          let remaining = String.sub data ~offset:5 ~len:(String.length data - 5) in Done { value = Frame.PriorityPayload { stream_dependency; exclusive; weight }; remaining }
  ...
  if length != 4 then
    ...
    if String.length data < 4 then
      ...
      | Some error_code_int ->
          ...
          in
          let remaining = String.sub data ~offset:4 ~len:(String.length data - 4) in Done { value = Frame.RstStreamPayload error_code; remaining }
  ...
  if flags.Frame.ack && length != 0 then
    ...
    if length mod 6 != 0 then
      ...
      if String.length data < length then
        ...
        | Ok settings ->
            let remaining = String.sub data ~offset:length ~len:(String.length data - length) in Done { value = Frame.SettingsPayload settings; remaining }
    ...
    | Some promised_stream_id_raw ->
        ...
        if header_fragment_length < 0 then
          ...
          if String.length data < length then
            ...
            let header_block_fragment = String.sub data ~offset ~len:header_fragment_length in
            let remaining = String.sub data ~offset:length ~len:(String.length data - length) in Done { value = Frame.PushPromisePayload { pad_length = pad_length_opt; promised_stream_id; header_block_fragment }; remaining }
...
let parse_ping_payload = fun length data ->
  if length != 8 then
    ...
    if String.length data < 8 then
      ...
      let remaining = String.sub data ~offset:8 ~len:(String.length data - 8) in Done { value = Frame.PingPayload opaque_data; remaining }
    ...
    if String.length data < length then
      ...
      | Some last_stream_id_raw -> (
        ...
        | Some error_code_int ->
            ...
            let remaining = String.sub data ~offset:length ~len:(String.length data - length) in Done { value = Frame.GoawayPayload { last_stream_id; error_code; debug_data }; remaining }
      )
    ...
    if String.length data < 4 then
      ...
      | Some increment_raw ->
          ...
          if increment = 0 then
            ...
            let remaining = String.sub data ~offset:4 ~len:(String.length data - 4) in Done { value = Frame.WindowUpdatePayload increment; remaining }
...
let parse_continuation_payload = fun length data ->
  if String.length data < length then
    ...
    let remaining = String.sub data ~offset:length ~len:(String.length data - length) in Done { value = Frame.ContinuationPayload header_block_fragment; remaining }
