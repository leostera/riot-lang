...
let read_uint8 data offset =
  if offset >= String.length data then None
  else Some (Char.code data.[offset])
            ...
            | Some frame_type ->
                ...
                | Some flags_byte ->
                    ...
                    | Some stream_id_raw ->
                        ...
                        Done { value = (length, frame_type, flags, stream_id); remaining = String.sub data 9 (String.length data - 9) }
  ...
  if has_padding then
    ...
    | Some pad_length ->
        if length < 1 + pad_length then
          ...
          Done { value = Frame.DataPayload { data = payload_data; pad_length = Some pad_length }; remaining }
  else
...
let parse_headers_payload length flags data =
  ...
  if header_fragment_length < 0 then
    ...
    let remaining = String.sub data length (String.length data - length) in
    let (stream_dependency, weight, exclusive) = match priority_info with
...
let parse_priority_payload length data =
  if length <> 5 then
    ...
    | Some (stream_dependency, exclusive, weight) ->
        ...
        Done { 
          value = Frame.PriorityPayload { stream_dependency; exclusive; weight }; 
          remaining 
        }
...
let parse_rst_stream_payload length data =
  if length <> 4 then
    ...
    | Some error_code_int ->
        let error_code = match Frame.int_to_error_code error_code_int with
          | Some ec -> ec
