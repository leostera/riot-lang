  ...
  if len < 2 then Need_more
    ...
    | Some opcode ->
        ...
        let header_size, payload_length =
          if payload_len_initial < 126 then (2, payload_len_initial)
          else if payload_len_initial = 126 then
            (* 16-bit length *)
            if len < 4 then (0, 0) (* Signal need more *)
            else
            ...
            (* 64-bit length - we only support up to 53-bit JS numbers *)
            len < 10
          then (0, 0) (* Signal need more *)
          else
        ...
        if header_size = 0 then Need_more
        else
          (* Add mask size if masked *)
          let mask_size = if masked then 4 else 0 in
          let total_header_size = header_size + mask_size in

          (* Check if we have full frame *)
          if len < total_header_size + payload_length then Need_more
          else
            ...
            (* Unmask if needed *)
            let payload =
              if masked then Frame.unmask mask raw_payload else raw_payload
            in
            ...
            let remaining =
              String.sub input
                ...
                (len - total_header_size - payload_length)
            in
