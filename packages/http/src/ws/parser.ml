          ...
          if payload_len_initial < 126 then
            ...
            if payload_len_initial = 126 then
              ...
              (* Signal need more *)
                ...
                let len_low = byte_at input 3 in (4, (len_high lsl 8) lor len_low)
            else
              if len < 10 then
                (0, 0)
              (* Signal need more *)
                ...
                in
                let payload_len = List.fold_left len_bytes ~init:0 ~fn:(
                  fun acc b -> (acc lsl 8) lor b
                ) in (10, payload_len)
        in
          ...
          if len < total_header_size + payload_length then
            ...
            let mask =
              if masked then
                ...
                let m3 = Int32.from_int (byte_at input (header_size + 3)) in Int32.(logor (shift_left m0 24) (logor (shift_left m1 16) (logor (shift_left m2 8) m3)))
              else Int32.zero
            in
            (* Extract payload *)
            ...
            let remaining = String.sub input ~offset:(total_header_size + payload_length) ~len:(len - total_header_size - payload_length) in Done { value = frame; remaining }
