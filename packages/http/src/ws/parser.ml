    ...
    | Some opcode ->
        ...
        let (header_size, payload_length) =
          ...
          (* Signal need more *)
            ...
            let len_bytes = [ byte_at input 6; byte_at input 7; byte_at input 8; byte_at input 9; ]
            in
            let payload_len = List.fold_left len_bytes ~init:0 ~fn:(fun acc b -> (acc lsl 8) lor b)
            in
            (10, payload_len)
        in
