...
let serialize = fun frame ->
  ...
  List.for_each extended_length ~fn:(
    fun byte -> Buffer.add_char header (Char.from_int_unchecked byte)
  );
  ...
  if masked then
    (
      ...
      let masked_payload = Frame.apply_mask mask payload in Buffer.add_string header masked_payload
    )
