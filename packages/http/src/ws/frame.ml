...
let unmask mask payload =
  ...
  for i = 0 to len - 1 do
    let mask_byte =
      Int32.(logand (shift_right mask (8 * (3 - (i mod 4)))) 0xffl |> to_int)
    in
...
let generate_mask () = Random.int32 Int32.max_int

(* Apply mask to payload *)
let apply_mask mask payload = unmask mask payload

(* Create frame helpers *)
let text ?(fin = true) payload =
  {
    fin;
    rsv1 = false;
    rsv2 = false;
...
let continuation ?(fin = false) payload =
  {
    fin;
    rsv1 = false;
    rsv2 = false;
