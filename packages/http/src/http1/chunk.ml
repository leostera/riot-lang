...
let parse_slice = fun input ->
  ...
  | Cursor_done { value = 0; remaining } -> Done { value = { data = ""; remaining = Slice.to_string (Cursor.remaining remaining) }; remaining = "" }
  | Cursor_done { value = size; remaining } -> (
    ...
    | Some (data, cursor) -> (
      match Cursor.advance_by cursor 2 with
      | None -> Need_more
      | Some cursor -> Done { value = { data = Slice.to_string data; remaining = Slice.to_string (Cursor.remaining cursor) }; remaining = "" }
    )
  )
