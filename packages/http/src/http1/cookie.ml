  ...
  | first :: attrs ->
      ...
      (match String.split_on_char '=' first with
       | name :: value_parts ->
           ...
           (* Parse attributes *)
           let cookie = List.fold_left (fun c attr ->
...
let to_set_cookie t =
  ...
  (* Add Max-Age *)
  let parts = match t.max_age with
    | Some age -> (String.concat "=" ["Max-Age"; string_of_int age]) :: parts
    | Option.None -> parts
  in
  
  (* Add Expires *)
  let parts = match t.expires with
  ...
  let parts = 
    if t.path = "/" then parts
    else (String.concat "=" ["Path"; t.path]) :: parts
  in
  
  (* Add Domain *)
  let parts = match t.domain with
    | Some d -> (String.concat "=" ["Domain"; d]) :: parts
    | Option.None -> parts
  in
  
  (* Add Secure flag *)
  let parts = if t.secure then "Secure" :: parts else parts in
  
  (* Add HttpOnly flag *)
  let parts = if t.http_only then "HttpOnly" :: parts else parts in
  
  (* Add SameSite *)
  let parts = match t.same_site with
...
let is_valid_value value =
  ...
  let rec check i =
    if i >= len then true
      ...
      if Char.code c < 32 || c = ';' || c = ',' then false
      else check (i + 1)
  in check 0

(** Create validated cookie *)
let make_validated ~name ~value ?max_age ?expires ?path ?domain ?secure ?http_only ?same_site () =
