...
module SliceCursor = struct
  type t = { source: Slice.t; pos: int; length: int }

  let create = fun source -> { source; pos = 0; length = Slice.length source }

  let of_cursor = fun cursor ->
    let source = Cursor.source cursor in { source; pos = Cursor.position cursor; length = Slice.length source }
  ...
  let take_until = fun cursor predicate ->
    ...
    let rec loop pos =
      if pos >= cursor.length then
        ...
        if predicate (Slice.get_unchecked cursor.source ~at:pos) then
          Some pos
        else loop (pos + 1)
    in
    match loop start with
    | None -> None
    | Some stop -> Some (Slice.sub_unchecked cursor.source ~off:start ~len:(stop - start), { cursor with pos = stop })

  let take_until_char = fun cursor needle ->
    take_until cursor
      (
        fun value -> value = needle
      )

  let take_n = fun cursor count ->
    if cursor.pos + count > cursor.length then
      None
    else Some (Slice.sub_unchecked cursor.source ~off:cursor.pos ~len:count, { cursor with pos = cursor.pos + count })
  ...
  let take_while = fun cursor predicate ->
    ...
    in
    let stop = loop start in (Slice.sub_unchecked cursor.source ~off:start ~len:(stop - start), { cursor with pos = stop })
  ...
  let remaining = fun cursor ->
    if is_eof cursor then
      Slice.empty
    else Slice.sub_unchecked cursor.source ~off:cursor.pos ~len:(cursor.length - cursor.pos)
end
...
let trim_leading_ows = fun slice -> SliceCursor.skip_while (SliceCursor.create slice) is_optional_whitespace |> SliceCursor.remaining
...
type header_line_owned = { header_name: string; header_value: string; next_cursor: SliceCursor.t }
          ...
          | Some (method_, line_cursor) ->
              ...
              | Some (path, line_cursor) ->
                  ...
                  if not (Slice.starts_with version ~prefix:"HTTP/") then
                    ...
                    let version = Std.Net.Http.Version.from_slice version |> Result.unwrap_or ~default:Std.Net.Http.Version.Http11 in
                    Slice_done {
  ...
  | Some (line, cursor) -> (
    ...
    | Slice_done cursor -> (
      ...
      | Some (name, line_cursor) -> (
        ...
        | Some line_cursor ->
            let value = SliceCursor.skip_while line_cursor is_optional_whitespace |> SliceCursor.remaining in
            let name = trim_leading_ows name in Slice_done { header_name = string_of_slice name; header_value = string_of_slice value; next_cursor = cursor }
      )
    )
  )
  ...
  if count >= max_count then
    ...
    | None ->
        ...
        | Slice_done { header_name; header_value; next_cursor } ->
            ...
            else parse_headers_owned ~max_count ~max_length ~acc:((header_name, header_value) :: acc) ~count:(count + 1) next_cursor
...
let parse_slice = fun ?(max_request_line = 8_192) ?(max_headers = 100) ?(max_header_length = 8_192) input ->
  ...
  | Slice_done { parsed_method; parsed_uri; parsed_version; next_cursor } -> (
    ...
    | Slice_done (headers_list, remaining) ->
        ...
        let request = request_of_parts parsed_method parsed_uri parsed_version headers_list body in Common.Done { value = request; remaining = "" }
  )

let parse = fun ?(max_request_line = 8_192) ?(max_headers = 100) ?(max_header_length = 8_192) input -> parse_slice ~max_request_line ~max_headers ~max_header_length (slice_of_string input)
