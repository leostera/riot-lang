      ...
      | Some cursor ->
          ...
          | Some (name, line_cursor) ->
              ...
              | Some line_cursor ->
                  ...
                  let name_cursor = Cursor.skip_while name_cursor (fun c -> c = ' ' || c = '\t') in
  ...
  | _ ->
      if List.length acc >= max_count then
        ...
        | Done { value = (name, value); remaining } ->
            ...
            if Cursor.length_remaining name_cursor + Cursor.length_remaining value_cursor > max_length then
              Error "Header too long"
            else
              let cursor = Cursor.create remaining in
              parse_headers ~max_count ~max_length ~acc:((name, value) :: acc) cursor

let parse ?(max_request_line=8192) ?(max_headers=100) ?(max_header_length=8192) input =
  match parse_request_line ~max_length:max_request_line input with
  | Need_more -> Need_more
  | Error e -> Error e
  | Done { value = (method_str, path_str, version_str); remaining } ->
      let cursor = Cursor.create remaining in
      match parse_headers ~max_count:max_headers ~max_length:max_header_length cursor with
      | Need_more -> Need_more
      | Error e -> Error e
      | Done { value = (headers_list, body_start); _ } ->
          (* Build Std.Net.Http.Request.t *)
          let method_ = Std.Net.Http.Method.of_string method_str in
          let uri = Std.Net.Uri.of_string path_str |> Result.unwrap_or ~default:(Std.Net.Uri.of_string "/" |> Result.unwrap) in
          ...
          let request = 
            Std.Net.Http.Request.create method_ uri
            |> (fun req -> Std.Net.Http.Request.with_version req version)
            |> (fun req -> Std.Net.Http.Request.with_headers req headers)
            |> (fun req -> if Cursor.length_remaining body_cursor > 0 then Std.Net.Http.Request.with_body req body_start else req)
          in
