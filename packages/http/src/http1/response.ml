        ...
        let response =
          (
            Std.Net.Http.Response.create status |> fun res ->
              Std.Net.Http.Response.with_version res version |> fun res -> Std.Net.Http.Response.with_headers res headers
          ) |> fun res ->
            if String.length body_start > 0 then
              Std.Net.Http.Response.with_body res body_start
            else res
        in
        let _ = reason in Done { value = response; remaining = body_start }
  )
