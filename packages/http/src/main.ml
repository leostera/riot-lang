              ...
              if actual_method <> exp_method then
                ...
                let header_errors =
                  List.filter_map
                    (fun (name, exp_value) ->
                      match
                        Std.Net.Http.Header.get actual_headers name
                      with
                      | Some actual_value ->
                      ...
                      | None ->
                          Some (format "Missing header: %s (expected '%s')" name exp_value))
                    exp_headers
                in
      ...
      | Some (exp_version, exp_status, exp_reason, exp_headers, exp_body) -> (
          ...
          | Http1.Common.Done { value = response; _ } ->
              ...
              if actual_version <> exp_version then
                ...
                Error
                  (format "Status code mismatch: expected %d, got %d"
  ...
  | Ok (Json.Object fields) ->
      ...
      let get_object key =
        Option.and_then (List.assoc_opt key fields) (function
          ...
          | _ -> None)
      in
      Some (get_int, get_string, get_object)
  | _ -> None
...
let test_uri (name, uri_file, expected_file) =
  Test.case (format "URI: %s" name) (fun () ->
      ...
      | Some (fields, get_opt_string, get_opt_int) -> (
          ...
          | Error e -> Error (format "Failed to parse URI: %s" (uri_error_to_string e))
          | Ok uri ->
              let errors = ref [] in
              ...
              let check_opt_string field_name getter =
                ...
                | Some expected ->
                    ...
                    if actual <> expected then
                      errors := (format "%s mismatch: expected %s, got %s" 
                        field_name 
                        (Option.map_or ~default:"null" Fun.id expected)
                        (Option.map_or ~default:"null" Fun.id actual)) :: !errors
                | None -> ()
              in
                ...
                | Some expected ->
                    ...
                    if actual <> expected then
                      errors := (format "%s mismatch: expected %s, got %s" 
                        ...
                        (Option.map_or ~default:"null" Int.to_string actual)) :: !errors
                | None -> ()
              in
              ...
              | Some (Json.String exp) ->
                  if actual_path <> exp then
                    errors := (format "path mismatch: expected '%s', got '%s'" exp actual_path) :: !errors
              | _ -> ());
...
let test_http2_frame (name, frame_file, expected_file) =
  Test.case (format "HTTP/2 Frame: %s" name) (fun () ->
      ...
      | None ->
          Error (format "Failed to parse expected JSON for %s" expected_file)
      | Some (get_int, get_string, _get_object) -> (
          ...
          | Http2.Parser.Done { value = frame; _ } ->
              ...
              | Some exp_len ->
                  if frame.Http2.Frame.length <> exp_len then
                    errors := (format "length mismatch: expected %d, got %d" exp_len frame.length) :: !errors
              | None -> ());
              ...
              | Some exp_type ->
                  ...
                  if actual_type <> exp_type then
                    errors := (format "frame_type mismatch: expected %s, got %s" exp_type actual_type) :: !errors
              | None -> ());
              
              (match get_int "stream_id" with
              | Some exp_stream_id ->
                  if frame.stream_id <> exp_stream_id then
                    errors := (format "stream_id mismatch: expected %d, got %d" exp_stream_id frame.stream_id) :: !errors
              | None -> ());
...
let string_to_hex s =
  ...
  String.iter (fun c ->
    Buffer.add_string buf (format "%02x" (Char.code c))
  ) s;
  Buffer.contents buf
          ...
          | Ws.Parser.Done { value = frame; _ } ->
              ...
              | Some exp_fin ->
                  if frame.Ws.Frame.fin <> exp_fin then
                    errors := (format "fin mismatch: expected %b, got %b" exp_fin frame.fin) :: !errors
              | None -> ());
              
              (match get_bool "masked" with
              | Some exp_masked ->
                  if frame.masked <> exp_masked then
                    errors := (format "masked mismatch: expected %b, got %b" exp_masked frame.masked) :: !errors
              | None -> ());
              ...
              | Some exp_opcode ->
                  ...
                  if actual_opcode <> exp_opcode then
                    errors := (format "opcode mismatch: expected %s, got %s" exp_opcode actual_opcode) :: !errors
              | None -> ());
              ...
              | Some exp_payload ->
                  ...
                  if actual_payload <> exp_payload then
                    errors := (format "payload mismatch: expected '%s', got '%s'" exp_payload actual_payload) :: !errors
              | None -> ());
...
let () =
  Miniriot.run
    ~main:(fun ~args ->
      ...
      let all_tests = request_tests @ response_tests @ uri_tests @ http2_tests @ ws_tests in
