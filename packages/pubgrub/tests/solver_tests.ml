...
let range_gen =
  Generator.frequency
    [
      6, simple_range_gen;
      4, Generator.map2 (Pubgrub.Ranges.union ~compare_v:Pubgrub.version_compare) simple_range_gen simple_range_gen;
    ]
  ...
  Generator.map3
    (
      fun positive pkg ranges ->
        ...
        else Pubgrub.Term.negative pkg ranges
    )
    Generator.bool
...
let assert_solution = fun expected_count result ->
  ...
  | Ok (Pubgrub.Solver.Success solution) ->
      if List.length solution = expected_count then
        Ok ()
      else Error ("Wrong number of packages: got " ^ (Int.to_string (List.length solution)) ^ ", expected " ^ (Int.to_string expected_count))
  ...
  let rec check_present = function
    ...
    | version :: rest ->
        if Pubgrub.Ranges.contains ~compare_v:Pubgrub.version_compare ranges version then
          check_present rest
        else Error ("Expected range to contain " ^ Pubgrub.version_to_string version)
  in
...
let assert_term = fun ~package ~positive ~ranges term ->
  if not (String.equal (Pubgrub.Term.package term) package) then
    ...
    if not (Bool.equal (Pubgrub.Term.is_positive term) positive) then
      Error ("Unexpected sign for term " ^ package)
    else assert_ranges_equal ~expected:ranges ~actual:(Pubgrub.Term.ranges term) ~message:("Unexpected ranges for term " ^ package)
...
let assert_version_equal = fun ~expected ~actual ~message ->
  if Pubgrub.version_compare expected actual = Order.EQ then
    Ok ()
  else Error (message ^ ": got " ^ Pubgrub.version_to_string actual ^ ", expected " ^ Pubgrub.version_to_string expected)
...
let assert_count_versions = fun ~expected result ->
  ...
  | Ok actual -> Error ("Unexpected version count: got " ^ Int.to_string actual ^ ", expected " ^ Int.to_string expected)
  | Error err -> Error ("Unexpected provider error: " ^ err)

let rec equal_dependencies = fun expected actual ->
  match expected, actual with
  | [], [] -> true
  | (expected_pkg, expected_ranges) :: expected_rest, (actual_pkg, actual_ranges) :: actual_rest -> String.equal expected_pkg actual_pkg && ranges_equal expected_ranges actual_ranges && equal_dependencies expected_rest actual_rest
  | _ -> false
...
let assert_dependencies = fun ~expected result ->
  ...
  | Ok (Pubgrub.Provider.Unavailable reason) -> Error ("Expected available dependencies but got unavailable: " ^ reason)
  | Error err -> Error ("Unexpected provider error: " ^ err)
...
let custom_incompat = fun terms -> Pubgrub.Incompatibility.create_external terms (Pubgrub.Incompatibility.Custom ("custom", Pubgrub.full, "test"))

let rec equal_string_lists = fun left right ->
  match left, right with
  | [], [] -> true
  | left :: left_rest, right :: right_rest -> String.equal left right && equal_string_lists left_rest right_rest
  | _ -> false

let assert_solution_packages = fun expected result ->
  match result with
  | Ok (Pubgrub.Solver.Success solution) ->
      let actual = List.map solution ~fn:(
        fun (pkg, _ver) -> pkg
      ) in
      if equal_string_lists expected actual then
        Ok ()
      else Error ("Wrong solution order: got [" ^ String.concat ", " actual ^ "], expected [" ^ String.concat ", " expected ^ "]")
  | Ok (Pubgrub.Solver.Failure incompat) -> Error ("Unexpected conflict: " ^ Pubgrub.explain_conflict incompat)
  | Error err -> Error ("Error: " ^ err)

let assert_raises = fun message fn ->
  try
    let _ = fn () in Error message
  with
  | _ -> Ok ()

let assert_inline_text = fun ~ctx ~expected ~actual -> Test.Snapshot.assert_inline_text ~ctx ~expected ~actual

let assert_int_equal = fun ~expected ~actual ~message ->
  if Int.equal expected actual then
    Ok ()
  else Error (message ^ ": got " ^ Int.to_string actual ^ ", expected " ^ Int.to_string expected)

let rec equal_solution_entries = fun expected actual ->
  match expected, actual with
  | [], [] -> true
  | (expected_pkg, expected_ver) :: expected_rest, (actual_pkg, actual_ver) :: actual_rest -> String.equal expected_pkg actual_pkg && Pubgrub.version_compare expected_ver actual_ver = Order.EQ && equal_solution_entries expected_rest actual_rest
  | _ -> false
...
let assert_solution_exact = fun expected result ->
  ...
  | Ok (Pubgrub.Solver.Success solution) ->
      let expected_entries = List.map expected ~fn:(
        fun (pkg, ver) -> pkg ^ "@" ^ Pubgrub.version_to_string ver
      ) in
      let actual_entries = List.map solution ~fn:(
        fun (pkg, ver) -> pkg ^ "@" ^ Pubgrub.version_to_string ver
      ) in Error ("Wrong exact solution: got [" ^ String.concat ", " actual_entries ^ "], expected [" ^ String.concat ", " expected_entries ^ "]")
  | Ok (Pubgrub.Solver.Failure incompat) -> Error ("Unexpected conflict: " ^ Pubgrub.explain_conflict incompat)
  | Error err -> Error ("Error: " ^ err)

type fuzz_dep = { package: string; allowed_versions: int list }

type fuzz_version_spec = { version: int; deps: fuzz_dep list }

type fuzz_package_spec = { name: string; versions: fuzz_version_spec list }

type fuzz_graph = { root_deps: fuzz_dep list; packages: fuzz_package_spec list }

type fuzz_version_slot = { present: bool; deps: int list option list }

type fuzz_graph_slot = { root_deps: int list option list; packages: fuzz_version_slot list list }
...
let fuzz_ranges_of_versions = fun versions ->
  let rec build = function
    | [] -> Pubgrub.empty
    | index :: rest -> Pubgrub.Ranges.union ~compare_v:Pubgrub.version_compare (Pubgrub.singleton (fuzz_pub_version index)) (build rest)
  in
  build versions
...
let rec zip_fuzz_deps = fun package_names deps ->
  ...
  | [], [] -> []
  | pkg :: pkg_rest, (Some allowed_versions) :: dep_rest -> { package = pkg; allowed_versions } :: zip_fuzz_deps pkg_rest dep_rest
...
let rec zip_fuzz_versions = fun version_indices slots ->
  ...
  | [], [] -> []
  | version :: version_rest, slot :: slot_rest when slot.present -> { version; deps = zip_fuzz_deps fuzz_package_names slot.deps } :: zip_fuzz_versions version_rest slot_rest
...
let rec zip_fuzz_packages = fun package_names slots ->
  ...
  | [], [] -> []
  | name :: name_rest, version_slots :: slot_rest -> { name; versions = zip_fuzz_versions fuzz_version_indices version_slots } :: zip_fuzz_packages name_rest slot_rest
  | _ -> []

let fuzz_graph_of_slot = fun (slot: fuzz_graph_slot): fuzz_graph -> { root_deps = zip_fuzz_deps fuzz_package_names slot.root_deps; packages = zip_fuzz_packages fuzz_package_names slot.packages }

let print_fuzz_dep = fun (dep: fuzz_dep) -> dep.package ^ "={" ^ int_list_to_string dep.allowed_versions ^ "}"
...
let print_fuzz_package_spec = fun (package: fuzz_package_spec) -> package.name ^ "[" ^ String.concat "|" (List.map package.versions ~fn:print_fuzz_version_spec) ^ "]"
          ...
          let rec try_versions = function
            ...
            | version_spec :: version_rest ->
                if int_member version_spec.version dep.allowed_versions then
                  match fuzz_oracle_search graph ((dep.package, version_spec.version) :: assignment) (version_spec.deps @ rest) with
                  | Some _ as solution -> solution
                  | None -> try_versions version_rest
                else try_versions version_rest
          in
...
let fuzz_solution_entries = fun assignment ->
  let versions = List.map assignment ~fn:(
    fun (pkg, version) -> (pkg, fuzz_pub_version version)
  ) in List.sort versions ~compare:(
...
let fuzz_version_index_of_pubgrub = fun version ->
  if Pubgrub.version_compare version (fuzz_pub_version 0) = Order.EQ then
    ...
    if Pubgrub.version_compare version (fuzz_pub_version 1) = Order.EQ then
      Some 1
    else
      if Pubgrub.version_compare version (fuzz_pub_version 2) = Order.EQ then
        Some 2
      else None
...
let canonical_result_string = fun result ->
  match result with
  | Ok (Pubgrub.Solver.Success solution) -> "success:" ^ String.concat "," (List.map solution ~fn:(
    fun (pkg, version) -> pkg ^ "@" ^ Pubgrub.version_to_string version
  ))
...
let validate_fuzz_solution = fun (graph: fuzz_graph) solution ->
  let rec ensure_known_versions = function
    ...
    | (pkg, version) :: rest when String.equal pkg "root" ->
        ...
        else Error "Root version changed in solver solution"
    | (pkg, version) :: rest -> (
      ...
      | Some version -> (
        ...
        | Some index ->
            if not (int_member index dep.allowed_versions) then
              ...
              if string_member dep.package reachable then
                walk reachable rest
              else
                match lookup_fuzz_package graph.packages dep.package with
                ...
                | Some package -> (
                  ...
                  | Some version_spec -> walk (dep.package :: reachable) (version_spec.deps @ rest)
                )
      )
    )
  in
  ...
  | Ok () -> (
    ...
    | Some version when Pubgrub.version_compare version (v 1 0 0) = Order.EQ -> (
      ...
      | Ok reachable ->
          ...
          let actual_packages = List.sort (List.map solution ~fn:(
            fun (pkg, _) -> pkg
          )) ~compare:String.compare in
          if equal_string_lists expected_packages actual_packages then
            Ok ()
          else Error ("Solver returned unexpected reachable set: got [" ^ String.concat ", " actual_packages ^ "], expected [" ^ String.concat ", " expected_packages ^ "]")
    )
...
let check_property = fun ~ctx ~name ~config property ->
  ...
  | Property.Success -> Ok ()
  | Property.Assumption_violated -> Error ("Property assumptions rejected every generated case for " ^ name)
  | Property.Failure { counter_example; shrink_steps } -> Error ("Property failed for " ^ name ^ " after " ^ Int.to_string shrink_steps ^ " shrink steps: " ^ counter_example)
  | Property.Error { exception_; backtrace } -> Error ("Property errored for " ^ name ^ ": " ^ Exception.to_string exception_ ^ "\n" ^ backtrace)

let prop_ranges_complement_matches_membership =
  Propane.property "Property: range complement matches membership negation" Arbitrary.(pair range_arb version_arb)
    (
      fun (ranges, version) ->
        let contains = Pubgrub.Ranges.contains ~compare_v:Pubgrub.version_compare ranges version in
        let complement_contains = Pubgrub.Ranges.contains ~compare_v:Pubgrub.version_compare (Pubgrub.Ranges.complement ~compare_v:Pubgrub.version_compare ranges) version in Bool.equal complement_contains (not contains)
    )

let prop_ranges_double_complement_is_identity =
  Propane.property "Property: range double complement is semantic identity" range_arb
    (
      fun ranges -> ranges_equal ranges (Pubgrub.Ranges.complement ~compare_v:Pubgrub.version_compare (Pubgrub.Ranges.complement ~compare_v:Pubgrub.version_compare ranges))
    )

let prop_ranges_union_matches_boolean_or =
  Propane.property "Property: range union membership matches boolean or" Arbitrary.(triple range_arb range_arb version_arb)
    (
      fun (left, right, version) ->
        let union_contains = Pubgrub.Ranges.contains ~compare_v:Pubgrub.version_compare (Pubgrub.Ranges.union ~compare_v:Pubgrub.version_compare left right) version in Bool.equal union_contains (Pubgrub.Ranges.contains ~compare_v:Pubgrub.version_compare left version || Pubgrub.Ranges.contains ~compare_v:Pubgrub.version_compare right version)
    )

let prop_ranges_intersection_matches_boolean_and =
  Propane.property "Property: range intersection membership matches boolean and" Arbitrary.(triple range_arb range_arb version_arb)
    (
      fun (left, right, version) ->
        let intersection_contains = Pubgrub.Ranges.contains ~compare_v:Pubgrub.version_compare (Pubgrub.Ranges.intersection ~compare_v:Pubgrub.version_compare left right) version in Bool.equal intersection_contains (Pubgrub.Ranges.contains ~compare_v:Pubgrub.version_compare left version && Pubgrub.Ranges.contains ~compare_v:Pubgrub.version_compare right version)
    )
...
let prop_term_negation_is_involutive =
  Propane.property "Property: term negation is involutive" term_arb
    (
      fun term ->
        let twice = Pubgrub.Term.negate (Pubgrub.Term.negate term) in String.equal (Pubgrub.Term.package term) (Pubgrub.Term.package twice) && Bool.equal (Pubgrub.Term.is_positive term) (Pubgrub.Term.is_positive twice) && ranges_equal (Pubgrub.Term.ranges term) (Pubgrub.Term.ranges twice)
    )
...
let test_solver_randomized_small_graphs_match_bruteforce =
  Test.property "Solver: randomized small graphs match brute-force satisfiability" ~examples:120
    (
      fun ctx ->
        let config = { Property.default_config with test_count = 120; max_size = 20; seed = Some 20_260_418 } in
            ...
            Property.for_all fuzz_graph_arb
              (
                fun (graph: fuzz_graph) ->
                      ...
                      (
                        Pubgrub.to_provider
                          (
                            ...
                            Pubgrub.add_package provider "root" (v 1 0 0) (List.map graph.root_deps ~fn:(
                              fun dep -> (dep.package, fuzz_ranges_of_versions dep.allowed_versions)
                            ));
                            let add_version (package: fuzz_package_spec) (version_spec: fuzz_version_spec) = Pubgrub.add_package provider package.name (fuzz_pub_version version_spec.version) (List.map version_spec.deps ~fn:(
                              fun dep -> (dep.package, fuzz_ranges_of_versions dep.allowed_versions)
                            )) in
                            let add_package (package: fuzz_package_spec) = List.for_each package.versions ~fn:(add_version package) in
                  ...
                  | Some witness, Ok (Pubgrub.Solver.Failure incompat) -> Property.fail ("Solver reported failure for satisfiable graph. Witness: " ^ String.concat ", " (List.map (fuzz_solution_entries witness) ~fn:(
                    fun (pkg, version) -> pkg ^ "@" ^ Pubgrub.version_to_string version
                  )) ^ ". Conflict: " ^ Pubgrub.explain_conflict incompat)
                  | None, Ok (Pubgrub.Solver.Success solution) -> Property.fail ("Solver reported success for unsatisfiable graph: " ^ String.concat ", " (List.map solution ~fn:(
                    fun (pkg, version) -> pkg ^ "@" ^ Pubgrub.version_to_string version
                  )))
...
let test_solver_randomized_insertion_order_is_deterministic =
  Test.property "Solver: randomized insertion order stays deterministic" ~examples:120
    (
      fun ctx ->
        let config = { Property.default_config with test_count = 120; max_size = 20; seed = Some 20_260_419 } in
        let build_provider ~reverse (graph: fuzz_graph) =
          ...
          Pubgrub.add_package provider "root" (v 1 0 0) (List.map root_deps ~fn:(
            fun dep -> (dep.package, fuzz_ranges_of_versions dep.allowed_versions)
          ));
          let add_version (package: fuzz_package_spec) (version_spec: fuzz_version_spec) =
            let deps = maybe_reverse_list version_spec.deps in Pubgrub.add_package provider package.name (fuzz_pub_version version_spec.version) (List.map deps ~fn:(
              fun dep -> (dep.package, fuzz_ranges_of_versions dep.allowed_versions)
            ))
          in
          let add_package (package: fuzz_package_spec) = List.for_each (maybe_reverse_list package.versions) ~fn:(add_version package) in
        ...
        check_property ~ctx ~name:"small graph determinism" ~config
          (
            Property.for_all fuzz_graph_arb
              (
                fun (graph: fuzz_graph) ->
                  let forward = Pubgrub.solve (Pubgrub.to_provider (build_provider ~reverse:false graph)) "root" (v 1 0 0) in
                  let reverse = Pubgrub.solve (Pubgrub.to_provider (build_provider ~reverse:true graph)) "root" (v 1 0 0) in
                  if String.equal (canonical_result_string forward) (canonical_result_string reverse) then
                    true
                  else Property.fail ("Insertion order changed result. Forward=" ^ canonical_result_string forward ^ " Reverse=" ^ canonical_result_string reverse)
              )
          )
    )
    ...
    (
      fun _ctx ->
        ...
        Pubgrub.add_package provider "root" (v 1 0 0)
          [
            "auth", Pubgrub.full;
            "bundler", Pubgrub.full;
            "framework", Pubgrub.full;
          ];
        Pubgrub.add_package provider "framework" (v 2 0 0)
          [
            "router", Pubgrub.singleton (v 5 0 0);
            "runtime", Pubgrub.singleton (v 2 0 0);
          ];
        Pubgrub.add_package provider "framework" (v 1 0 0)
          [
            "router", Pubgrub.singleton (v 4 0 0);
            "runtime", Pubgrub.singleton (v 1 0 0);
          ];
        Pubgrub.add_package provider "bundler" (v 3 0 0)
          [
            "runtime", Pubgrub.singleton (v 2 0 0);
          ];
        Pubgrub.add_package provider "bundler" (v 2 0 0)
          [
            "runtime", Pubgrub.singleton (v 1 0 0);
          ];
  ...
  Test.case "Corpus: SDK plugins backtrack to a common core API"
    (
      fun _ctx ->
        ...
        Pubgrub.add_package provider "root" (v 1 0 0)
          [
            "formatter", Pubgrub.full;
            "lsp", Pubgrub.full;
            "sdk", Pubgrub.full;
          ];
        Pubgrub.add_package provider "sdk" (v 3 0 0)
          [
            "core", Pubgrub.singleton (v 3 0 0);
          ];
        Pubgrub.add_package provider "sdk" (v 2 0 0)
          [
            "core", Pubgrub.singleton (v 2 0 0);
          ];
        Pubgrub.add_package provider "formatter" (v 2 0 0)
          [
            "core", Pubgrub.singleton (v 3 0 0);
          ];
        Pubgrub.add_package provider "formatter" (v 1 0 0)
          [
            "core", Pubgrub.singleton (v 2 0 0);
          ];
        Pubgrub.add_package provider "lsp" (v 1 0 0)
          [
            "core", Pubgrub.singleton (v 2 0 0);
          ];
  ...
  Test.case "Corpus: version holes pick the highest satisfiable island"
    (
      fun _ctx ->
        ...
        Pubgrub.add_package provider "root" (v 1 0 0)
          [
            "analyzer", Pubgrub.full;
            "compiler", Pubgrub.full;
          ];
        Pubgrub.add_package provider "compiler" (v 5 0 0)
          [
            "stdlib", Pubgrub.singleton (v 5 0 0);
          ];
        Pubgrub.add_package provider "compiler" (v 4 0 0)
          [
            "stdlib", Pubgrub.singleton (v 4 0 0);
          ];
        Pubgrub.add_package provider "compiler" (v 3 0 0)
          [
            "stdlib", Pubgrub.singleton (v 3 0 0);
          ];
        Pubgrub.add_package provider "analyzer" (v 1 0 0)
          [
            "stdlib", Pubgrub.singleton (v 4 0 0);
          ];
  ...
  Test.case "Solver: backtracks from an impossible self-dependency version"
    (
      fun _ctx ->
        ...
        Pubgrub.add_package provider "gamma" (v 2 0 0)
          [
      ...
      fun _ctx ->
        let build_provider ~reverse =
          ...
          let add_api_versions () =
            Pubgrub.add_package provider "api" (v 2 0 0)
              [
                "core", Pubgrub.singleton (v 2 0 0);
              ];
          ...
          let add_cli_versions () =
            Pubgrub.add_package provider "cli" (v 2 0 0)
              [
                "core", Pubgrub.singleton (v 2 0 0);
              ];
            Pubgrub.add_package provider "cli" (v 1 0 0)
              [
                "core", Pubgrub.singleton (v 1 0 0);
              ]
          in
          let add_plugin_versions () =
            Pubgrub.add_package provider "plugin" (v 1 0 0)
              [
                "core", Pubgrub.singleton (v 3 0 0);
              ]
          in
        ...
        | Ok (Pubgrub.Solver.Failure _), Ok (Pubgrub.Solver.Failure _) ->
            if String.equal (canonical_result_string forward) (canonical_result_string reverse) then
              Ok ()
            else Error ("Expected deterministic unsatisfiable result, got forward=" ^ canonical_result_string forward ^ " reverse=" ^ canonical_result_string reverse)
        | (Ok (Pubgrub.Solver.Success _), _) | (_, Ok (Pubgrub.Solver.Success _)) -> Error "Expected unsatisfiable plugin conflict to fail"
        | (Error err, _) | (_, Error err) -> Error ("Unexpected solver error: " ^ err)
    )
...
let test_ranges_full_contains_everything =
  Test.case "Ranges: full contains sampled versions"
    (
      fun _ctx -> assert_range_membership ~present:[
        v 0 0 0;
        v 1 0 0;
        v 2 0 0;
        v 9 0 0;
...
let test_ranges_singleton_exact =
  Test.case "Ranges: singleton contains only one version"
    (
      fun _ctx -> assert_range_membership ~present:[ v 1 0 0 ] ~absent:[ v 0 9 9; v 1 0 1; v 2 0 0 ] (Pubgrub.singleton (v 1 0 0))
    )

let test_ranges_higher_than_is_inclusive =
  Test.case "Ranges: higher_than is inclusive"
    (
      fun _ctx -> assert_range_membership ~present:[ v 2 0 0; v 3 0 0 ] ~absent:[ v 1 9 9 ] (Pubgrub.higher_than (v 2 0 0))
    )

let test_ranges_strictly_higher_than_is_exclusive =
  Test.case "Ranges: strictly_higher_than is exclusive"
    (
      fun _ctx -> assert_range_membership ~present:[ v 2 0 1; v 3 0 0 ] ~absent:[ v 2 0 0; v 1 9 9 ] (Pubgrub.strictly_higher_than (v 2 0 0))
    )

let test_ranges_lower_than_is_inclusive =
  Test.case "Ranges: lower_than is inclusive"
    (
      fun _ctx -> assert_range_membership ~present:[ v 0 9 9; v 2 0 0 ] ~absent:[ v 2 0 1; v 3 0 0 ] (Pubgrub.lower_than (v 2 0 0))
    )

let test_ranges_strictly_lower_than_is_exclusive =
  Test.case "Ranges: strictly_lower_than is exclusive"
    (
      fun _ctx -> assert_range_membership ~present:[ v 0 9 9; v 1 9 9 ] ~absent:[ v 2 0 0; v 2 0 1 ] (Pubgrub.strictly_lower_than (v 2 0 0))
    )

let test_ranges_between_is_half_open =
  Test.case "Ranges: between is half-open"
    (
      fun _ctx -> assert_range_membership ~present:[ v 1 0 0; v 1 9 9 ] ~absent:[ v 0 9 9; v 2 0 0; v 2 0 1 ] (Pubgrub.between (v 1 0 0) (v 2 0 0))
    )

let test_ranges_intersection_with_empty =
  Test.case "Ranges: intersection with empty is empty"
    (
      fun _ctx -> assert_ranges_equal ~expected:Pubgrub.empty ~actual:(Pubgrub.Ranges.intersection ~compare_v:Pubgrub.version_compare (Pubgrub.higher_than (v 1 0 0)) Pubgrub.empty) ~message:"Expected intersection with empty to be empty"
    )
...
let test_ranges_intersection_with_full =
  Test.case "Ranges: intersection with full preserves the range"
    (
      fun _ctx ->
        let ranges = Pubgrub.between (v 1 0 0) (v 3 0 0) in assert_ranges_equal ~expected:ranges ~actual:(Pubgrub.Ranges.intersection ~compare_v:Pubgrub.version_compare ranges Pubgrub.full) ~message:"Expected intersection with full to preserve the range"
    )
...
let test_ranges_union_with_empty =
  Test.case "Ranges: union with empty preserves the range"
    (
      fun _ctx ->
        let ranges = Pubgrub.between (v 1 0 0) (v 3 0 0) in assert_ranges_equal ~expected:ranges ~actual:(Pubgrub.Ranges.union ~compare_v:Pubgrub.version_compare ranges Pubgrub.empty) ~message:"Expected union with empty to preserve the range"
    )

let test_ranges_complement_of_singleton =
  Test.case "Ranges: complement of singleton excludes only that version"
    (
      fun _ctx -> assert_range_membership ~present:[ v 0 9 9; v 1 0 1; v 2 0 0 ] ~absent:[ v 1 0 0 ] (Pubgrub.Ranges.complement ~compare_v:Pubgrub.version_compare (Pubgrub.singleton (v 1 0 0)))
    )

let test_ranges_touching_exclusive_intersection_is_empty =
  Test.case "Ranges: touching exclusive bounds intersect to empty"
    (
      fun _ctx -> assert_ranges_equal ~expected:Pubgrub.empty ~actual:(Pubgrub.Ranges.intersection ~compare_v:Pubgrub.version_compare (Pubgrub.between (v 1 0 0) (v 2 0 0)) (Pubgrub.between (v 2 0 0) (v 3 0 0))) ~message:"Expected touching half-open ranges to be disjoint"
    )

let test_ranges_union_of_overlapping_intervals =
  Test.case "Ranges: union of overlapping intervals merges semantically"
    (
      fun _ctx -> assert_ranges_equal ~expected:(Pubgrub.between (v 1 0 0) (v 4 0 0)) ~actual:(Pubgrub.Ranges.union ~compare_v:Pubgrub.version_compare (Pubgrub.between (v 1 0 0) (v 3 0 0)) (Pubgrub.between (v 2 0 0) (v 4 0 0))) ~message:"Expected overlapping union to cover the merged interval"
    )
...
let test_ranges_complement_of_multiple_segments =
  Test.case "Ranges: complement of multiple segments yields the gaps"
    (
      fun _ctx ->
        let ranges = Pubgrub.Ranges.union ~compare_v:Pubgrub.version_compare (Pubgrub.between (v 1 0 0) (v 2 0 0)) (Pubgrub.between (v 3 0 0) (v 4 0 0)) in
        let expected = Pubgrub.Ranges.union ~compare_v:Pubgrub.version_compare (Pubgrub.strictly_lower_than (v 1 0 0)) (Pubgrub.Ranges.union ~compare_v:Pubgrub.version_compare (Pubgrub.between (v 2 0 0) (v 3 0 0)) (Pubgrub.higher_than (v 4 0 0))) in assert_ranges_equal ~expected ~actual:(Pubgrub.Ranges.complement ~compare_v:Pubgrub.version_compare ranges) ~message:"Expected complement to preserve the gaps between segments"
    )
  ...
  Test.case "Ranges: is_disjoint matches intersection emptiness"
    (
      fun _ctx ->
        ...
        if Bool.equal (Pubgrub.Ranges.is_disjoint ~compare_v:Pubgrub.version_compare left right) (ranges_equal intersection Pubgrub.empty) then
          Ok ()
        else Error "Expected is_disjoint to agree with intersection emptiness"
    )
  ...
  Test.case "Ranges: subset_of and double complement are semantic"
    (
      fun _ctx ->
        ...
        let roundtrip = Pubgrub.Ranges.complement ~compare_v:Pubgrub.version_compare (Pubgrub.Ranges.complement ~compare_v:Pubgrub.version_compare base) in
        if Pubgrub.Ranges.subset_of ~compare_v:Pubgrub.version_compare subset base && not (Pubgrub.Ranges.subset_of ~compare_v:Pubgrub.version_compare base subset) && ranges_equal base roundtrip then
          Ok ()
        else Error "Expected subset_of and double complement semantics to hold"
    )
...
let test_ranges_normalize_collapses_semantic_duplicates =
  Test.case "Ranges: normalize collapses overlapping semantic duplicates"
    (
      fun _ctx ->
        let ranges = Pubgrub.Ranges.union ~compare_v:Pubgrub.version_compare (Pubgrub.between (v 1 0 0) (v 3 0 0)) (Pubgrub.between (v 2 0 0) (v 4 0 0)) in
        let normalized = Pubgrub.Ranges.normalize ~compare_v:Pubgrub.version_compare ranges in assert_ranges_equal ~expected:(Pubgrub.between (v 1 0 0) (v 4 0 0)) ~actual:normalized ~message:"Expected normalize to collapse overlapping segments"
    )
...
let test_ranges_compare_is_semantic =
  Test.case "Ranges: compare is semantic after normalization"
    (
      fun _ctx ->
        let left = Pubgrub.Ranges.union ~compare_v:Pubgrub.version_compare (Pubgrub.between (v 1 0 0) (v 3 0 0)) (Pubgrub.between (v 2 0 0) (v 4 0 0)) in
        ...
        if Pubgrub.Ranges.compare ~compare_v:Pubgrub.version_compare left right = Order.EQ then
          Ok ()
        else Error "Expected compare to treat semantically equal ranges as equal"
    )
...
let test_ranges_to_string_is_stable =
  Test.case "Ranges: to_string prints canonical segments"
    (
      fun _ctx ->
        let ranges = Pubgrub.Ranges.union ~compare_v:Pubgrub.version_compare (Pubgrub.strictly_lower_than (v 1 0 0)) (Pubgrub.higher_than (v 2 0 0)) in
        ...
        if String.equal actual "(-inf, 1.0.0) | [2.0.0, +inf)" then
          Ok ()
        else Error ("Unexpected range string: " ^ actual)
    )
    ...
    (
      fun _ctx ->
        if Pubgrub.Term.is_any (Pubgrub.Term.positive "pkg" Pubgrub.full) then
          Ok ()
        else Error "Expected positive full term to be tautological"
    )
    ...
    (
      fun _ctx ->
        if Pubgrub.Term.is_any (Pubgrub.Term.negative "pkg" Pubgrub.empty) then
          Ok ()
        else Error "Expected negative empty term to be tautological"
    )
  ...
  Test.case "Term: equal package strings compare structurally"
    (
      fun _ctx ->
        ...
        try
          let term = Pubgrub.Term.intersection (Pubgrub.Term.positive dynamic_pkg Pubgrub.full) (Pubgrub.Term.positive "shared" (Pubgrub.singleton (v 1 0 0))) in
          if String.equal (Pubgrub.Term.package term) "shared" && Pubgrub.Term.is_positive term && Pubgrub.Ranges.contains ~compare_v:Pubgrub.version_compare (Pubgrub.Term.ranges term) (v 1 0 0) then
            Ok ()
          else Error "Expected structural package equality to preserve term intersection"
        with
...
let test_term_mixed_union_can_be_tautological =
  Test.case "Term: mixed union preserves tautologies"
    (
      fun _ctx ->
        let term = Pubgrub.Term.union (Pubgrub.Term.positive "pkg" Pubgrub.full) (Pubgrub.Term.negative "pkg" Pubgrub.full) in
        if Pubgrub.Term.is_any term then
          Ok ()
        else Error "Expected positive full union negative full to be tautological"
    )
...
let test_term_negation_is_involutive =
  Test.case "Term: negation is involutive"
    (
      fun _ctx ->
        let term = Pubgrub.Term.positive "pkg" (Pubgrub.between (v 1 0 0) (v 3 0 0)) in assert_term ~package:"pkg" ~positive:true ~ranges:(Pubgrub.between (v 1 0 0) (v 3 0 0)) (Pubgrub.Term.negate (Pubgrub.Term.negate term))
    )

let test_term_positive_positive_intersection =
  Test.case "Term: positive intersection uses range intersection"
    (
      fun _ctx -> assert_term ~package:"pkg" ~positive:true ~ranges:(Pubgrub.between (v 2 0 0) (v 3 0 0)) (Pubgrub.Term.intersection (Pubgrub.Term.positive "pkg" (Pubgrub.between (v 1 0 0) (v 3 0 0))) (Pubgrub.Term.positive "pkg" (Pubgrub.between (v 2 0 0) (v 4 0 0))))
    )

let test_term_positive_positive_union =
  Test.case "Term: positive union uses range union"
    (
      fun _ctx -> assert_term ~package:"pkg" ~positive:true ~ranges:(Pubgrub.between (v 1 0 0) (v 4 0 0)) (Pubgrub.Term.union (Pubgrub.Term.positive "pkg" (Pubgrub.between (v 1 0 0) (v 3 0 0))) (Pubgrub.Term.positive "pkg" (Pubgrub.between (v 2 0 0) (v 4 0 0))))
    )

let test_term_negative_negative_intersection =
  Test.case "Term: negative intersection uses range union semantics"
    (
      fun _ctx -> assert_term ~package:"pkg" ~positive:false ~ranges:(Pubgrub.between (v 1 0 0) (v 5 0 0)) (Pubgrub.Term.intersection (Pubgrub.Term.negative "pkg" (Pubgrub.between (v 1 0 0) (v 4 0 0))) (Pubgrub.Term.negative "pkg" (Pubgrub.between (v 2 0 0) (v 5 0 0))))
    )

let test_term_negative_negative_union =
  Test.case "Term: negative union uses range intersection semantics"
    (
      fun _ctx -> assert_term ~package:"pkg" ~positive:false ~ranges:(Pubgrub.between (v 2 0 0) (v 4 0 0)) (Pubgrub.Term.union (Pubgrub.Term.negative "pkg" (Pubgrub.between (v 1 0 0) (v 4 0 0))) (Pubgrub.Term.negative "pkg" (Pubgrub.between (v 2 0 0) (v 5 0 0))))
    )
...
let test_term_positive_negative_intersection =
  Test.case "Term: positive-negative intersection excludes the forbidden hole"
    (
      fun _ctx ->
        let term = Pubgrub.Term.intersection (Pubgrub.Term.positive "pkg" (Pubgrub.between (v 1 0 0) (v 4 0 0))) (Pubgrub.Term.negative "pkg" (Pubgrub.between (v 2 0 0) (v 3 0 0))) in
        if Pubgrub.Term.is_positive term && assert_range_membership ~present:[ v 1 0 0; v 3 0 0 ] ~absent:[ v 2 0 0; v 2 5 0; v 4 0 0 ] (Pubgrub.Term.ranges term) = Ok () then
          Ok ()
        else Error "Expected positive-negative intersection to preserve only the allowed parts"
    )

let test_term_positive_negative_union =
  Test.case "Term: positive-negative union keeps complement semantics"
    (
      fun _ctx -> assert_term ~package:"pkg" ~positive:false ~ranges:(Pubgrub.between (v 1 0 0) (v 2 0 0)) (Pubgrub.Term.union (Pubgrub.Term.positive "pkg" (Pubgrub.between (v 2 0 0) (v 4 0 0))) (Pubgrub.Term.negative "pkg" (Pubgrub.between (v 1 0 0) (v 3 0 0))))
    )
    ...
    (
      fun _ctx ->
        assert_raises "Expected union on different packages to raise"
          (
            fun () -> Pubgrub.Term.union (Pubgrub.Term.positive "left" Pubgrub.full) (Pubgrub.Term.positive "right" Pubgrub.full)
          )
    )
    ...
    (
      fun _ctx ->
        assert_raises "Expected intersection on different packages to raise"
          (
            fun () -> Pubgrub.Term.intersection (Pubgrub.Term.positive "left" Pubgrub.full) (Pubgrub.Term.positive "right" Pubgrub.full)
          )
    )
...
let test_provider_choose_version_missing_package =
  Test.case "Provider: missing package choose_version returns none"
    (
      fun _ctx ->
        let provider = Pubgrub.to_provider (Pubgrub.create_offline ()) in assert_choose_none (provider.choose_version "missing" Pubgrub.full)
    )
...
let test_provider_count_versions_missing_package =
  Test.case "Provider: missing package count_versions returns zero"
    (
      fun _ctx ->
        let provider = Pubgrub.to_provider (Pubgrub.create_offline ()) in assert_count_versions ~expected:0 (provider.count_versions "missing" Pubgrub.full)
    )
...
let test_provider_get_dependencies_missing_package =
  Test.case "Provider: missing package get_dependencies returns unavailable"
    (
      fun _ctx ->
        let provider = Pubgrub.to_provider (Pubgrub.create_offline ()) in assert_unavailable ~expected:"Package 'missing' not found" (provider.get_dependencies "missing" (v 1 0 0))
    )
  ...
  Test.case "Provider: choose_version picks the highest matching version"
    (
      fun _ctx ->
        ...
        let provider = Pubgrub.to_provider offline in assert_choose_version ~expected:(v 3 0 0) (provider.choose_version "foo" (Pubgrub.higher_than (v 2 0 0)))
    )
  ...
  Test.case "Provider: choose_version skips a nonmatching higher version"
    (
      fun _ctx ->
        ...
        Pubgrub.add_package offline "foo" (v 2 0 0) [];
        Pubgrub.add_package offline "foo" (v 3 0 0) [];
        let provider = Pubgrub.to_provider offline in assert_choose_version ~expected:(v 2 0 0) (provider.choose_version "foo" (Pubgrub.strictly_lower_than (v 3 0 0)))
    )
  ...
  Test.case "Provider: choose_version respects an exact singleton range"
    (
      fun _ctx ->
        ...
        Pubgrub.add_package offline "foo" (v 2 0 0) [];
        Pubgrub.add_package offline "foo" (v 3 0 0) [];
        let provider = Pubgrub.to_provider offline in assert_choose_version ~expected:(v 2 0 0) (provider.choose_version "foo" (Pubgrub.singleton (v 2 0 0)))
    )
  ...
  Test.case "Provider: count_versions handles disjoint semantic ranges"
    (
      fun _ctx ->
        ...
        let ranges = Pubgrub.Ranges.union ~compare_v:Pubgrub.version_compare (Pubgrub.singleton (v 1 0 0)) (Pubgrub.singleton (v 3 0 0)) in assert_count_versions ~expected:2 (provider.count_versions "foo" ranges)
    )
  ...
  Test.case "Provider: duplicate package versions use deterministic latest deps"
    (
      fun _ctx ->
        ...
        let provider = Pubgrub.to_provider offline in assert_dependencies ~expected:[
          "right", Pubgrub.singleton (v 2 0 0);
  ...
  Test.case "Provider: duplicate package versions count once"
    (
      fun _ctx ->
        ...
        let provider = Pubgrub.to_provider offline in assert_count_versions ~expected:1 (provider.count_versions "foo" Pubgrub.full)
    )
  ...
  Test.case "Provider: insertion order does not affect highest-match selection"
    (
      fun _ctx ->
        ...
        Pubgrub.add_package forward "foo" (v 3 0 0)
          [
        ...
        Pubgrub.add_package reverse "foo" (v 3 0 0)
          [
            "dep", Pubgrub.singleton (v 3 0 0);
          ];
        let forward_provider = Pubgrub.to_provider forward in
        let reverse_provider = Pubgrub.to_provider reverse in
        match forward_provider.choose_version "foo" Pubgrub.full, reverse_provider.choose_version "foo" Pubgrub.full with
        | Ok (Some left), Ok (Some right) -> assert_version_equal ~expected:left ~actual:right ~message:"Expected insertion order to preserve selected version"
    ...
    (
      fun _ctx ->
        ...
        if not (Int.equal (List.length terms) 2) then
          ...
          | Some ("root", "dep") -> (
            match assert_term_incompat ~package:"root" ~positive:true ~ranges:(Pubgrub.singleton (v 1 0 0)) incompat with
            | Error _ as err -> err
  ...
  Test.case "Incompatibility: from_dependency drops empty dependency terms"
    (
      fun _ctx ->
        ...
        else Error "Expected empty dependency range to produce only the parent term"
    )
  ...
  Test.case "Incompatibility: merge_dependents merges matching dependency causes"
    (
      fun _ctx ->
        ...
        | Some merged -> (
          match assert_term_incompat ~package:"root" ~positive:false ~ranges:(Pubgrub.Ranges.union ~compare_v:Pubgrub.version_compare (Pubgrub.singleton (v 1 0 0)) (Pubgrub.singleton (v 2 0 0))) merged with
          | Error _ as err -> err
...
let test_incompatibility_merge_dependents_different_ranges =
  Test.case "Incompatibility: merge_dependents keeps distinct dependency ranges separate"
    (
      fun _ctx ->
        let left = Pubgrub.Incompatibility.from_dependency "root" (v 1 0 0) ("dep", Pubgrub.between (v 1 0 0) (v 2 0 0)) in
        let right = Pubgrub.Incompatibility.from_dependency "root" (v 2 0 0) ("dep", Pubgrub.between (v 2 0 0) (v 3 0 0)) in
        match Pubgrub.Incompatibility.merge_dependents left right with
        | None -> Ok ()
...
let test_incompatibility_prior_cause_merges_and_canonicalizes =
  Test.case "Incompatibility: prior_cause merges target terms and canonicalizes duplicates"
    (
      fun _ctx ->
        let left = custom_incompat [ Pubgrub.Term.positive "shared" (Pubgrub.between (v 1 0 0) (v 3 0 0)); Pubgrub.Term.positive "left" (Pubgrub.singleton (v 1 0 0)); Pubgrub.Term.positive "side" (Pubgrub.between (v 1 0 0) (v 4 0 0)) ] in
        let right = custom_incompat [ Pubgrub.Term.positive "shared" (Pubgrub.between (v 2 0 0) (v 5 0 0)); Pubgrub.Term.positive "right" (Pubgrub.singleton (v 2 0 0)); Pubgrub.Term.positive "side" (Pubgrub.between (v 2 0 0) (v 3 0 0)) ] in
        let prior = Pubgrub.Incompatibility.prior_cause ~extra_term:(Pubgrub.Term.negative "extra" (Pubgrub.singleton (v 9 0 0))) left right "shared" in
        ...
        if not (Int.equal (List.length terms) 5) then
          ...
          match assert_term_incompat ~package:"shared" ~positive:true ~ranges:(Pubgrub.between (v 1 0 0) (v 5 0 0)) prior with
          | Error _ as err -> err
          | Ok () -> (
            match assert_term_incompat ~package:"side" ~positive:true ~ranges:(Pubgrub.between (v 2 0 0) (v 3 0 0)) prior with
            | Error _ as err -> err
            | Ok () -> (
              match assert_term_incompat ~package:"left" ~positive:true ~ranges:(Pubgrub.singleton (v 1 0 0)) prior with
              | Error _ as err -> err
              | Ok () -> (
                match assert_term_incompat ~package:"right" ~positive:true ~ranges:(Pubgrub.singleton (v 2 0 0)) prior with
                | Error _ as err -> err
                | Ok () -> assert_term_incompat ~package:"extra" ~positive:false ~ranges:(Pubgrub.singleton (v 9 0 0)) prior
              )
            )
          )
    )
    ...
    (
      fun _ctx ->
        ...
        if Pubgrub.Incompatibility.is_terminal incompat "root" (v 1 0 0) then
          Ok ()
        else Error "Expected not_root incompatibility to be terminal for the root decision"
    )
...
let test_relation_satisfied_when_all_terms_met =
  Test.case "Partial_solution: relation is satisfied when all terms are met"
    (
      fun _ctx ->
        let solution =
          Pubgrub.Partial_solution.empty () |> fun solution ->
            Pubgrub.Partial_solution.add_decision solution "foo" (v 1 0 0) |> fun solution -> Pubgrub.Partial_solution.add_decision solution "bar" (v 2 0 0)
        in
        let incompat = custom_incompat [ Pubgrub.Term.positive "foo" (Pubgrub.singleton (v 1 0 0)); Pubgrub.Term.negative "bar" (Pubgrub.singleton (v 1 0 0)) ] in assert_relation `Satisfied (Pubgrub.Partial_solution.relation solution incompat)
    )
...
let test_relation_almost_satisfied_with_one_undecided =
  Test.case "Partial_solution: relation is almost satisfied with one undecided package"
    (
      fun _ctx ->
        let solution =
          Pubgrub.Partial_solution.empty () |> fun solution -> Pubgrub.Partial_solution.add_decision solution "foo" (v 1 0 0)
        in
        let incompat = custom_incompat [ Pubgrub.Term.positive "foo" (Pubgrub.singleton (v 1 0 0)); Pubgrub.Term.positive "bar" (Pubgrub.singleton (v 2 0 0)) ] in assert_relation (`AlmostSatisfied "bar") (Pubgrub.Partial_solution.relation solution incompat)
    )
    ...
    (
      fun _ctx ->
        let solution =
          Pubgrub.Partial_solution.empty () |> fun solution -> Pubgrub.Partial_solution.add_decision solution "foo" (v 1 0 0)
        in
        let incompat = custom_incompat [ Pubgrub.Term.positive "foo" (Pubgrub.singleton (v 2 0 0)) ] in assert_relation (`Contradicted "foo") (Pubgrub.Partial_solution.relation solution incompat)
    )
...
let test_relation_constrained_positive_subset_is_satisfied =
  Test.case "Partial_solution: relation treats constrained positive subsets as satisfied"
    (
      fun _ctx ->
        let constraint_incompat = custom_incompat [ Pubgrub.Term.negative "foo" (Pubgrub.between (v 2 0 0) (v 3 0 0)) ] in
        let solution =
          Pubgrub.Partial_solution.empty () |> fun solution -> Pubgrub.Partial_solution.add_derivation solution "foo" constraint_incompat
        in
        let incompat = custom_incompat [ Pubgrub.Term.positive "foo" (Pubgrub.between (v 1 0 0) (v 4 0 0)) ] in assert_relation `Satisfied (Pubgrub.Partial_solution.relation solution incompat)
    )
...
let test_relation_constrained_positive_overlap_is_almost_satisfied =
  Test.case "Partial_solution: relation narrows overlapping constrained positives"
    (
      fun _ctx ->
        let constraint_incompat = custom_incompat [ Pubgrub.Term.negative "foo" (Pubgrub.between (v 2 0 0) (v 4 0 0)) ] in
        let solution =
          Pubgrub.Partial_solution.empty () |> fun solution -> Pubgrub.Partial_solution.add_derivation solution "foo" constraint_incompat
        in
        let incompat = custom_incompat [ Pubgrub.Term.positive "foo" (Pubgrub.between (v 3 0 0) (v 5 0 0)) ] in assert_relation (`AlmostSatisfied "foo") (Pubgrub.Partial_solution.relation solution incompat)
    )
...
let test_relation_constrained_negative_disjoint_is_satisfied =
  Test.case "Partial_solution: relation treats constrained negative disjointness as satisfied"
    (
      fun _ctx ->
        let constraint_incompat = custom_incompat [ Pubgrub.Term.negative "foo" (Pubgrub.between (v 1 0 0) (v 2 0 0)) ] in
        let solution =
          Pubgrub.Partial_solution.empty () |> fun solution -> Pubgrub.Partial_solution.add_derivation solution "foo" constraint_incompat
        in
        let incompat = custom_incompat [ Pubgrub.Term.negative "foo" (Pubgrub.between (v 2 0 0) (v 3 0 0)) ] in assert_relation `Satisfied (Pubgrub.Partial_solution.relation solution incompat)
    )
    ...
    (
      fun _ctx ->
        ...
        let solution =
          Pubgrub.Partial_solution.empty () |> fun solution -> Pubgrub.Partial_solution.add_derivation solution "foo" constraint_incompat
        in
        let incompat = custom_incompat [ Pubgrub.Term.negative "foo" (Pubgrub.between (v 2 0 0) (v 4 0 0)) ] in assert_relation (`Contradicted "foo") (Pubgrub.Partial_solution.relation solution incompat)
    )
...
let test_partial_solution_cached_constraints_intersect_derivations =
  Test.case "Partial_solution: cached constraints intersect derivations"
    (
      fun _ctx ->
        let left = custom_incompat [ Pubgrub.Term.negative "foo" (Pubgrub.between (v 1 0 0) (v 4 0 0)) ] in
        let right = custom_incompat [ Pubgrub.Term.negative "foo" (Pubgrub.between (v 2 0 0) (v 5 0 0)) ] in
        let solution =
          Pubgrub.Partial_solution.empty () |> fun solution ->
            Pubgrub.Partial_solution.add_derivation solution "foo" left |> fun solution -> Pubgrub.Partial_solution.add_derivation solution "foo" right
        in
        assert_constraint ~expected:(`Constrained (Pubgrub.between (v 2 0 0) (v 4 0 0))) (Pubgrub.Partial_solution.get_constraint solution "foo")
    )
  ...
  Test.case "Partial_solution: backtrack restores cached derivations"
    (
      fun _ctx ->
        ...
        let solution =
          Pubgrub.Partial_solution.empty () |> fun solution ->
            Pubgrub.Partial_solution.add_derivation solution "foo" constraint_incompat |> fun solution -> Pubgrub.Partial_solution.add_decision solution "foo" (v 2 1 0)
        in
        let backtracked = Pubgrub.Partial_solution.backtrack solution 0 in assert_constraint ~expected:(`Constrained (Pubgrub.between (v 2 0 0) (v 4 0 0))) (Pubgrub.Partial_solution.get_constraint backtracked "foo")
    )
  ...
  Test.case "Solve: returned solution order is deterministic"
    (
      fun _ctx ->
        ...
        assert_solution_packages [ "alpha"; "root"; "zeta" ] (Pubgrub.solve (Pubgrub.to_provider provider) "root" (v 1 0 0))
    )
...
let test_report_no_versions_includes_requested_range =
  Test.case "Report: no_versions explanation includes requested range"
    (
      fun ctx ->
        let incompat = Pubgrub.Incompatibility.no_versions "foo" (Pubgrub.between (v 1 0 0) (v 2 0 0)) in assert_inline_text ~ctx ~expected:"Conflict:\nno versions of foo match [1.0.0, 2.0.0).\n\nTherefore, version solving failed." ~actual:(Pubgrub.Report.explain_conflict incompat)
    )
...
let test_report_from_dependency_includes_dependency_range =
  Test.case "Report: dependency explanation includes dependency range"
    (
      fun ctx ->
        let incompat = Pubgrub.Incompatibility.from_dependency "root" (v 1 0 0) ("foo", Pubgrub.between (v 1 0 0) (v 2 0 0)) in assert_inline_text ~ctx ~expected:"Conflict:\nroot@1.0.0 depends on foo in [1.0.0, 2.0.0).\n\nTherefore, version solving failed." ~actual:(Pubgrub.Report.explain_conflict incompat)
    )
...
let test_report_derived_explanations_are_readable =
  Test.case "Report: derived explanations stay readable"
    (
      fun ctx ->
        let dep = Pubgrub.Incompatibility.from_dependency "root" (v 1 0 0) ("foo", Pubgrub.between (v 1 0 0) (v 2 0 0)) in
        let no_versions = Pubgrub.Incompatibility.no_versions "foo" (Pubgrub.between (v 1 0 0) (v 2 0 0)) in
        let incompat = Pubgrub.Incompatibility.create_derived [ Pubgrub.Term.positive "root" (Pubgrub.singleton (v 1 0 0)) ] dep no_versions None in assert_inline_text ~ctx ~expected:"Conflict:\nBecause:\n  root@1.0.0 depends on foo in [1.0.0, 2.0.0).\nAnd because:\n  no versions of foo match [1.0.0, 2.0.0).\nSo root in [1.0.0, 1.0.0].\n\nTherefore, version solving failed." ~actual:(Pubgrub.Report.explain_conflict incompat)
    )
  ...
  Test.case "Solver: solve_with_stats exposes structured counters"
    (
      fun _ctx ->
          ...
          | Ok () -> (
            ...
            | Ok () -> (
              ...
              | Ok () -> (
                ...
                | Error _ as err -> err
                | Ok () -> (
                  match assert_int_equal ~expected:0 ~actual:outcome.stats.conflicts ~message:"Unexpected conflict count" with
                  | Error _ as err -> err
                  | Ok () -> (
                    match assert_int_equal ~expected:1 ~actual:outcome.stats.provider_choose_version_calls ~message:"Unexpected choose_version call count" with
                    | Error _ as err -> err
                    | Ok () -> (
                      match assert_int_equal ~expected:1 ~actual:outcome.stats.provider_count_versions_calls ~message:"Unexpected count_versions call count" with
                      | Error _ as err -> err
                      | Ok () -> (
                        match assert_int_equal ~expected:2 ~actual:outcome.stats.provider_get_dependencies_calls ~message:"Unexpected get_dependencies call count" with
                        | Error _ as err -> err
                        | Ok () -> (
                          match assert_int_equal ~expected:4 ~actual:outcome.stats.provider_calls ~message:"Unexpected aggregate provider call count" with
                          | Error _ as err -> err
                          | Ok () -> assert_int_equal ~expected:2 ~actual:outcome.stats.max_decision_depth ~message:"Unexpected max decision depth"
                        )
                      )
                    )
                  )
        ...
        | Ok (Pubgrub.Solver.Failure incompat) -> Error ("Expected success but got failure: " ^ Pubgrub.Report.explain_conflict incompat)
  ...
  Test.case "Solver: max_iterations option is configurable"
    (
      fun _ctx ->
        ...
        | Ok (Pubgrub.Solver.Failure incompat) -> Error ("Expected iteration limit error but got conflict: " ^ Pubgrub.Report.explain_conflict incompat)
    )
  ...
  Test.case "Package with many versions (10)"
    (
      fun _ctx ->
        ...
        for i = 0 to 9 do Pubgrub.add_package provider "foo" (v 1 i 0) [] done;
        assert_solution 2 (Pubgrub.solve (Pubgrub.to_provider provider) "root" (v 1 0 0))
    )
  ...
  Test.case "Version selection with many options"
    (
      fun _ctx ->
        ...
        for major = 0 to 2 do
          for minor = 0 to 3 do
            for patch = 0 to 2 do Pubgrub.add_package provider "foo" (v major minor patch) [] done
          done
        done;
        assert_solution 2 (Pubgrub.solve (Pubgrub.to_provider provider) "root" (v 1 0 0))
    )
  ...
  Test.case "Package with 20 versions"
    (
      fun _ctx ->
        ...
        for i = 0 to 19 do Pubgrub.add_package provider "foo" (v 1 i 0) [] done;
        assert_solution 2 (Pubgrub.solve (Pubgrub.to_provider provider) "root" (v 1 0 0))
    )
  ...
  Test.case "Package with 50 versions"
    (
      fun _ctx ->
        ...
        for i = 0 to 49 do Pubgrub.add_package provider "foo" (v 1 i 0) [] done;
        assert_solution 2 (Pubgrub.solve (Pubgrub.to_provider provider) "root" (v 1 0 0))
    )
  ...
  Test.case "Narrow version range constraint"
    (
      fun _ctx ->
        ...
        for i = 0 to 10 do Pubgrub.add_package provider "foo" (v 1 i 0) [] done;
        assert_solution 2 (Pubgrub.solve (Pubgrub.to_provider provider) "root" (v 1 0 0))
    )
  ...
  Test.case "Patch version selection"
    (
      fun _ctx ->
        ...
        for i = 0 to 10 do Pubgrub.add_package provider "foo" (v 1 0 i) [] done;
        assert_solution 2 (Pubgrub.solve (Pubgrub.to_provider provider) "root" (v 1 0 0))
    )
      ...
      Test.case ("Web framework scenario " ^ (Int.to_string i))
        (
          fun _ctx ->
            ...
            Pubgrub.add_package provider "http" (v (1 + (i mod 3)) 0 0)
              [
                "sockets", Pubgrub.full;
              ];
            Pubgrub.add_package provider "router" (v 1 (i mod 5) 0)
              [
      ...
      Test.case ("Database scenario " ^ (Int.to_string i))
        (
          fun _ctx ->
            ...
            Pubgrub.add_package provider "db-driver" (v (1 + (i mod 4)) 0 0)
              [
                "connection-pool", Pubgrub.full;
              ];
            Pubgrub.add_package provider "migrations" (v 1 (i mod 6) 0)
              [
      ...
      Test.case ("Compiler toolchain scenario " ^ (Int.to_string i))
        (
          fun _ctx ->
            ...
            Pubgrub.add_package provider "lexer" (v (1 + (i mod 3)) 0 0)
              [
                "regex", Pubgrub.full;
              ];
            Pubgrub.add_package provider "parser" (v 1 (i mod 4) 0)
              [
                "ast", Pubgrub.full;
              ];
    ...
    (
      fun _ctx ->
        ...
        for i = 0 to 9 do
          ...
          for j = 0 to 9 do Pubgrub.add_package provider ("sub-" ^ (Int.to_string i) ^ "-" ^ (Int.to_string j)) (v 1 0 0) [] done
        done;
  ...
  Test.case "Massive versions: package with 100 versions"
    (
      fun _ctx ->
        ...
        for i = 0 to 99 do Pubgrub.add_package provider "lib" (v 1 i 0) [] done;
        assert_solution 2 (Pubgrub.solve (Pubgrub.to_provider provider) "root" (v 1 0 0))
    )
  ...
  Test.case "Deep graph with shared transitive dependencies"
    (
      fun _ctx ->
        ...
        Pubgrub.add_package provider "state" (v 1 0 0)
          [
            "store", Pubgrub.full;
          ];
        Pubgrub.add_package provider "api" (v 1 0 0)
    ...
    (
      fun _ctx ->
        ...
        | Ok (Pubgrub.Solver.Success sol) ->
            let packages = List.map sol ~fn:(
              fun (name, _) -> name
            ) in Error ("Expected failure but got success: " ^ (String.concat ", " packages))
        | Error err -> Error ("Unexpected error: " ^ err)
    )
      ...
      fun _ctx ->
        ...
        | Ok (Pubgrub.Solver.Success solution) ->
            if List.length solution = 3 then
              Ok ()
            else Error ("Expected 3 packages, got " ^ (Int.to_string (List.length solution)))
        | Ok (Pubgrub.Solver.Failure _) -> Error "Expected success but got failure"
        | Error err -> Error ("Unexpected error: " ^ err)
    )
  ...
  Test.case "Conflict with partial satisfier variant"
    (
      fun _ctx ->
        ...
        match Pubgrub.solve (Pubgrub.to_provider provider) "root" (v 1 0 0) with
        | Ok (Pubgrub.Solver.Success solution) ->
            let packages = List.map solution ~fn:(
              fun (name, ver) -> name ^ "@" ^ (Pubgrub.version_to_string ver)
            ) in
            if List.length solution = 3 then
              Ok ()
            else Error ("Expected 3 packages, got " ^ (Int.to_string (List.length solution)) ^ ": " ^ (String.concat ", " packages))
        | Ok (Pubgrub.Solver.Failure incompat) -> Error ("Expected success but got failure: " ^ (Pubgrub.Report.explain_conflict incompat))
        | Error err -> Error ("Unexpected error: " ^ err)
    )
  ...
  Test.case "TRACE: Conflict with partial satisfier"
    (
      fun ctx ->
        ...
        | Ok (Pubgrub.Solver.Success _) -> Test.Snapshot.assert_json ~ctx ~actual:(Pubgrub.Trace.to_json trace_ctx)
        | Ok (Pubgrub.Solver.Failure incompat) -> Error ("Expected success but got failure: " ^ (Pubgrub.Report.explain_conflict incompat))
        | Error err -> Error ("Unexpected error: " ^ err)
    )
      ...
      fun _ctx ->
        ...
        | Ok (Pubgrub.Solver.Success solution) ->
            ...
            if List.length solution = 4 then
              Ok ()
            else Error ("Expected 4 packages, got " ^ (Int.to_string (List.length solution)) ^ ": " ^ (String.concat ", " packages))
        | Ok (Pubgrub.Solver.Failure err) -> Error ("Expected success but got failure: " ^ (Pubgrub.Report.explain_conflict err))
        | Error err -> Error ("Unexpected error: " ^ err)
    )
  ...
  Test.case "REF: Confusing with lots of holes"
    (
      fun _ctx ->
        ...
        for i = 1 to 5 do
          Pubgrub.add_package provider "foo" (v i 0 0)
            [
              "bar", Pubgrub.full;
            ]
        done;
