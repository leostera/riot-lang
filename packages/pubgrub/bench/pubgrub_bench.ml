...
let build_provider = fun versions_per_package ->
  ...
  for patch = 0 to versions_per_package - 1 do add_package offline "pkg" (v 1 0 patch) [] done;
  to_provider offline

let build_partial_solution = fun derivation_count ->
  let incompat = Incompatibility.create_external [ Term.negative "pkg" (between (v 1 0 0) (v 1 0 derivation_count)) ] (Incompatibility.Custom ("pkg", full, "bench")) in
  let rec loop solution remaining =
    if remaining <= 0 then
      solution
    else loop (Partial_solution.add_derivation solution "pkg" incompat) (remaining - 1)
  in
...
let holey_range = Ranges.union ~compare_v:version_compare (between (v 1 0 0) (v 1 0 5)) (between (v 1 0 10) (v 1 0 15))
