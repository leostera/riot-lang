...
let build_partial_solution = fun derivation_count ->
  let incompat =
    Incompatibility.create_external
      [ Term.negative "pkg" (between (v 1 0 0) (v 1 0 derivation_count)) ]
      (Incompatibility.Custom ("pkg", full, "bench"))
  in
...
let benchmarks =
  Bench.[
    ...
    with_config
      ~config:medium
      "pubgrub provider.choose_version 100"
      bench_provider_choose_version_100;
    with_config
      ~config:heavy
      "pubgrub provider.choose_version 1000"
      bench_provider_choose_version_1000;
    with_config
      ~config:medium
