...
let equal_ranges = fun left right -> Ranges.subset_of ~compare_v:version_compare left right && Ranges.subset_of ~compare_v:version_compare right left
...
let equal_term = fun left right -> String.equal (Term.package left) (Term.package right) && Bool.equal (Term.is_positive left) (Term.is_positive right) && equal_ranges (Term.ranges left) (Term.ranges right)
...
let snapshot_stats = fun stats ->
  let provider_calls = stats.provider_choose_version_calls + stats.provider_count_versions_calls + stats.provider_get_dependencies_calls in
  {
...
let incompatibility_key = fun incompat ->
  let render_term term =
    ...
    in
    sign ^ Term.package term ^ ":" ^ Ranges.to_string ~to_string_v:Version.to_string (Term.ranges term)
  in
  Incompatibility.terms incompat |> List.map ~fn:render_term |> List.sort ~compare:String.compare |> String.concat "|"
...
let get_incompatibilities = fun store pkg -> List.sort (List.map (get_incompatibility_ids store pkg) ~fn:(get_incompatibility store)) ~compare:(
  fun left right -> String.compare (incompatibility_key left) (incompatibility_key right)
)
...
let add_incompatibility_to_store = fun store package incompat ->
  ...
  let id =
    ...
    | None ->
        ...
        let _ = HashMap.insert store.ids_by_key ~key ~value:id in id
  in
...
type state = { solution: Partial_solution.t; incompatibilities: incompatibility_store }

let add_incompatibility = fun state package incompat -> add_incompatibility_to_store state.incompatibilities package incompat

let normalize_packages = fun packages -> List.unique (List.sort packages ~compare:String.compare) ~compare:String.compare
...
let compare_dependency = fun (left_pkg, left_ranges) (right_pkg, right_ranges) ->
  ...
  if pkg_compare != Order.EQ then
    pkg_compare
  else String.compare (Ranges.to_string ~to_string_v:Version.to_string left_ranges) (Ranges.to_string ~to_string_v:Version.to_string right_ranges)
  ...
  let rec resolve current_incompat current_incompat_changed =
    if Incompatibility.is_terminal current_incompat root_package root_version then
      ...
      | `DifferentDecisionLevels previous_level ->
          ...
          emit (Trace.ConflictResolvedDifferent { package = pkg; previous_level; incompatibility = current_incompat });
          ...
          if current_incompat_changed then
            List.for_each (Incompatibility.terms current_incompat) ~fn:(
              fun term ->
                let term_pkg = Term.package term in add_incompatibility state term_pkg current_incompat
            );
          let terms = Incompatibility.terms current_incompat in
          Log.info ("📚 Learned incompatibility with " ^ Int.to_string (List.length terms) ^ " terms, backtracked to level " ^ Int.to_string previous_level);
          List.for_each terms ~fn:(
            fun t ->
              Log.info
                (
                  "    Term: " ^ (
      ...
      | `SameDecisionLevels { cause = satisfier_cause; extra_term } ->
          ...
          if Ptr.equal satisfier_cause current_incompat then
            (
              Log.error "Satisfier cause same as current incompatibility - treating as \
               terminal";
              Error current_incompat
            )
          else
...
let unit_propagation = fun ~stats ~emit root_package root_version state changed_packages ->
  ...
  let rec process_packages = fun state ->
    ...
    | pkg :: rest -> (
      ...
      | _ -> (
        Log.info ("  💡 Found " ^ (Int.to_string (List.length incompats)) ^ " incompatibilities for " ^ pkg);
        let rec check_incompats = fun state ->
          ...
          | incompat :: remaining -> (
            ...
            let terms_str = String.concat ", " (List.map incompat_terms ~fn:(
              fun t ->
            ...
            | `Satisfied -> (
              ...
              | Ok (resolved_pkg, root_cause, backtracked_solution) ->
                  ...
                  emit (Trace.LearnedIncompatibility { package = resolved_pkg; incompatibility = root_cause });
                  Log.info ("  ✅ Conflict resolved for " ^ resolved_pkg ^ ", adding derivation");
                  let before_constraint = Partial_solution.get_constraint backtracked_solution resolved_pkg in
                  (* Add derivation - it will negate the term from root_cause *)
                  let new_solution = Partial_solution.add_derivation backtracked_solution resolved_pkg root_cause in
                  ...
                  List.for_each (Incompatibility.terms root_cause) ~fn:(
                    fun term ->
                      let term_pkg = Term.package term in add_incompatibility state term_pkg root_cause
                  );
                  let new_state =
                    if equal_constraint before_constraint after_constraint then
                      state
                    else { state with solution = new_solution }
                  in
                  if equal_constraint before_constraint after_constraint then
                    process_packages new_state rest
                  else
                    (
                      Log.info ("🔄 Continuing unit propagation from learned package " ^ resolved_pkg);
            ...
            | `AlmostSatisfied satisfier_pkg -> (
              ...
              Log.info ("  📋 Remaining incompats: " ^ (Int.to_string (List.length remaining)) ^ ", remaining packages: " ^ (Int.to_string (List.length rest)));
              ...
              let new_solution = Partial_solution.add_derivation state.solution satisfier_pkg incompat in
              ...
              in
              emit (Trace.DerivedConstraint { package = satisfier_pkg; incompatibility = incompat; changed = not (equal_constraint before_constraint after_constraint) });
              (* Continue with remaining incompats, then process satisfier_pkg *)
              Log.info ("  ⏭️  Continuing: satisfier=" ^ satisfier_pkg ^ ", rest=" ^ (String.concat "," rest));
              match check_incompats new_state remaining with
              | Ok state' ->
                  let next_packages =
                    if equal_constraint before_constraint after_constraint then
                      rest
                    else satisfier_pkg :: rest
                  in
                  Log.info ("  ✅ check_incompats OK, processing [" ^ (String.concat "," next_packages) ^ "]");
                  process_packages state' next_packages
              | Error _ as err ->
                  Log.error "  ❌ check_incompats ERROR";
                  err
            )
            | `Contradicted _ -> check_incompats state remaining
...
let choose_version = fun stats provider state pkg ranges ->
  ...
  let term_satisfying_ranges term =
    if Term.is_positive term then
      Term.ranges term
    else Ranges.complement ~compare_v:version_compare (Term.ranges term)
  in
  List.for_each incompats ~fn:(
    fun incompat ->
          ...
          if not (String.equal term_pkg pkg) then
            (
              ...
              (
                ...
                | `Decided v -> Log.info ("      Term pkg=" ^ term_pkg ^ " is Decided@" ^ Version.to_string v)
              ...
              | `Decided ver ->
                  ...
                  let term_satisfied = (Term.is_positive term && in_range) || ((not (Term.is_positive term)) && not in_range) in
                  if not term_satisfied then
                    all_other_satisfied := false
              | `Constrained constrained_ranges ->
                  let term_satisfied =
                    if Term.is_positive term then
                      Ranges.subset_of ~compare_v:version_compare constrained_ranges (Term.ranges term)
                    else Ranges.is_disjoint ~compare_v:version_compare constrained_ranges (Term.ranges term)
                  in
                  if not term_satisfied then
                    all_other_satisfied := false
            )
      ...
      if !all_other_satisfied then
        (
          ...
          let pkg_terms = List.filter terms ~fn:(
            fun term -> String.equal (Term.package term) pkg
          ) in
          match pkg_terms with
          | [] -> ()
          | term :: rest ->
              let conflict_ranges = List.fold_left rest ~init:(term_satisfying_ranges term) ~fn:(
                fun acc term -> Ranges.intersection ~compare_v:version_compare acc (term_satisfying_ranges term)
              ) in
              let allowed_ranges = Ranges.complement ~compare_v:version_compare conflict_ranges in effective_ranges := Ranges.intersection ~compare_v:version_compare !effective_ranges allowed_ranges
        )
  );
  ...
  | Ok (Provider.Available deps) -> (
    ...
    List.for_each deps ~fn:(
      fun (dep_pkg, dep_ranges) ->
        ...
        if dep_pkg = root_package && not (Ranges.contains ~compare_v:version_compare dep_ranges root_version) then
          (
            Log.info ("Impossible root self-dependency: " ^ root_package ^ "@" ^ (Version.to_string root_version) ^ " depends on " ^ dep_pkg ^ " with incompatible range");
            impossible_root_deps := (dep_pkg, dep_ranges) :: !impossible_root_deps
          )
        else
          if Ranges.is_empty dep_ranges then
            (
              Log.info ("Impossible root dependency: " ^ root_package ^ "@" ^ (Version.to_string root_version) ^ " depends on " ^ dep_pkg ^ " with empty range");
              impossible_root_deps := (dep_pkg, dep_ranges) :: !impossible_root_deps
            )
    );
    ...
    | (dep_pkg, dep_ranges) :: _ ->
        let impossible_incompat = Incompatibility.from_dependency root_package root_version (dep_pkg, dep_ranges) in
    ...
    | [] ->
        ...
        List.for_each deps ~fn:(
          fun (dep_pkg, dep_ranges) ->
            let dep_incompat = Incompatibility.from_dependency root_package root_version (dep_pkg, dep_ranges) in
        ...
        | Ok state ->
            ...
            let rec solve_loop state iteration next_pkg =
              if iteration > max_iterations then
                (
                  Log.error ("Iteration limit reached after " ^ Int.to_string max_iterations ^ " iterations!");
                ...
                (
                  ...
                  Log.debug ("Iteration " ^ (Int.to_string iteration) ^ ": unit propagation on " ^ next_pkg);
                  ...
                  | Ok propagated_state -> (
                    ...
                    let prioritizer pkg ranges =
                      let num_incompats = List.length (get_incompatibility_ids propagated_state.incompatibilities pkg) in
                      ...
                      let constraint_score =
                        if Ranges.is_empty ranges || Ranges.equal ~compare_v:version_compare ranges Ranges.full then
                          0
                        else 1_000_000
                      in
                      let availability_score =
                        if matching_versions = Int.max_int then
                          0
                        else Int.max 0 (100_000 - matching_versions)
                      in
                    ...
                    | Some (pkg, ranges) -> (
                      ...
                      | Ok (None, effective_ranges) ->
                          let unavailable_ranges =
                            if Ranges.is_empty effective_ranges && not (Ranges.is_empty ranges) then
                              ranges
                            else effective_ranges
                          in
                          emit (Trace.NoVersionAvailable { package = pkg; ranges = unavailable_ranges });
                          ...
                          Log.info ("📭 No version available for " ^ pkg ^ ", adding no_versions incompatibility");
                      ...
                      | Ok (Some ver, _) -> (
                        ...
                        | Ok (Provider.Unavailable _reason) ->
                            ...
                            Log.debug ("Package " ^ pkg ^ "@" ^ Version.to_string ver ^ " unavailable");
                            solve_loop propagated_state (iteration + 1) pkg
                        | Ok (Provider.Available pkg_deps) -> (
                          Log.debug ("Processing " ^ Int.to_string (List.length pkg_deps) ^ " dependencies for " ^ pkg ^ "@" ^ Version.to_string ver);
                          (* Check for impossible dependencies first *)
                          let impossible_deps = ref [] in
                          List.for_each pkg_deps ~fn:(
                            fun (dep_pkg, dep_ranges) ->
                              (* Check for self-dependency with incompatible version *)
                              if dep_pkg = pkg && not (Ranges.contains ~compare_v:version_compare dep_ranges ver) then
                                (
                                  Log.info ("Impossible self-dependency: " ^ pkg ^ "@" ^ (Version.to_string ver) ^ " depends on " ^ dep_pkg ^ " with incompatible range");
                                  impossible_deps := (dep_pkg, dep_ranges) :: !impossible_deps
                                )
                              else
                                if Ranges.is_empty dep_ranges then
                                  (
                                    Log.info ("Impossible dependency: " ^ pkg ^ "@" ^ (Version.to_string ver) ^ " depends on " ^ dep_pkg ^ " with empty range");
                                    impossible_deps := (dep_pkg, dep_ranges) :: !impossible_deps
                                  )
                          );
                          ...
                          | (dep_pkg, dep_ranges) :: _ ->
                              let impossible_incompat = Incompatibility.from_dependency pkg ver (dep_pkg, dep_ranges) in
                              Log.info ("Impossible dependency for " ^ pkg ^ "@" ^ Version.to_string ver ^ ", learning incompatibility and retrying");
                              add_incompatibility propagated_state pkg impossible_incompat;
                              solve_loop propagated_state (iteration + 1) pkg
                          | [] -> (
                            (* PORT OF RUST: Just add decision and incompatibilities *)
                            Log.info ("✅ Adding decision: " ^ pkg ^ "@" ^ (Version.to_string ver));
                            let new_solution = Partial_solution.add_decision propagated_state.solution pkg ver in
                            ...
                            List.for_each pkg_deps ~fn:(
                              fun (dep_pkg, dep_ranges) ->
                                let dep_incompat = Incompatibility.from_dependency pkg ver (dep_pkg, dep_ranges) in
                                Log.info ("📦 Added dependency incompatibility for " ^ dep_pkg);
                            ...
                            Log.info ("🔄 Added " ^ (Int.to_string (List.length affected_packages)) ^ " dependency incompatibilities: " ^ (String.concat ", " affected_packages));
                            (* Run unit propagation on affected packages *)
                            match unit_propagation ~stats ~emit root_package root_version new_state affected_packages with
                            | Error terminal_incompat ->
                                Log.error "Terminal incompatibility after adding \
                                             dependencies";
                                finish (Ok (Failure terminal_incompat))
                            | Ok propagated_state ->
...
let solve = fun ?trace_ctx ?(options = default_options) provider root_package root_version ->
  let outcome = solve_with_stats ?trace_ctx ~options provider root_package root_version in outcome.result
