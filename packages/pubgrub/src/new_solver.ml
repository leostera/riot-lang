...
let compute_pending state : (package * version Ranges.t) list =
  ...
  let add_to_pending = fun pkg ranges ->
    let existing_ranges =
  ...
  let rec collect_from_assignments =
    function
    | [] -> ()
    | assignment :: rest ->
        (
  ...
  let result = ref [] in
  HashMap.iter (fun pkg ranges -> result := (pkg, ranges) :: !result) pending_map;
  ...
  let score_package = fun ((pkg, ranges)) ->
    let num_incompats =
...
let rec conflict_resolution = fun root_package root_version state incompat ->
  let rec resolve = fun current_incompat current_incompat_changed ->
...
let solve = fun provider root_package root_version ->
  ...
  | Ok (Provider.Available deps) -> (
      ...
      | None ->
          ...
          in
          (* Main solve loop *)
          let rec solve_loop = fun state iteration next_pkg ->
                  ...
                  | Some (pkg, ranges) -> (
                      ...
                      | Ok (Some ver) -> (
                          ...
                          | Ok (Provider.Available pkg_deps) -> (
                              Log.debug
                              ("Processing "
                              ^ string_of_int (List.length pkg_deps)
                              ^ " dependencies for "
                              ^ pkg
                              ^ "@"
