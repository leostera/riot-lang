  ...
  let rec resolve current_incompat current_incompat_changed =
    ...
    then Error current_incompat
      ...
      | `DifferentDecisionLevels previous_level ->
          ...
          HashMap.iter (fun incompat level ->
            if level > previous_level then to_remove := incompat :: !to_remove
          ) state.contradicted;
          List.iter (fun incompat -> ignore (HashMap.remove state.contradicted incompat)) !to_remove;
          ...
          Log.info "📚 Learned incompatibility, backtracked to level %d" previous_level;
      ...
      | `SameDecisionLevels satisfier_cause ->
          ...
          if satisfier_cause == current_incompat then (
            Log.error "Satisfier cause same as current incompatibility - treating as terminal";
...
let unit_propagation root_package root_version state changed_packages =
  Log.debug "Unit propagation called with %d changed packages"
    (List.length changed_packages);
        ...
        | Some incompats ->
            ...
            let rec check_incompats state = function
              ...
              | incompat :: remaining -> (
                  ...
                  | Some _ ->
                      Log.debug "    Skipping contradicted incompatibility";
                  ...
                  | `Satisfied -> (
                      ...
                      | Ok (resolved_pkg, root_cause, backtracked_solution) ->
                          Log.info "  ✅ Conflict resolved for %s, adding derivation" resolved_pkg;
                          (* Add derivation - it will negate the term from root_cause *)
                          let new_solution =
                            Partial_solution.add_derivation backtracked_solution
                              resolved_pkg root_cause
                          in
                  ...
                  | `AlmostSatisfied satisfier_pkg ->
                      Log.info "  🎯 Almost satisfied, deriving constraint for %s" satisfier_pkg;
                      ...
                      | Ok state' -> process_packages state' (satisfier_pkg :: rest)
                      | Error _ as err -> err)
                  | `Contradicted _ ->
                      (* Mark as contradicted so we don't check it again *)
                      let current_level = Partial_solution.current_decision_level state.solution in
                      ignore (HashMap.insert state.contradicted incompat current_level);
                      check_incompats state remaining
                  | `Unknown ->
                      check_incompats state remaining)
  ...
  | Ok (Provider.Available deps) ->
      ...
      List.iter (fun (dep_pkg, dep_ranges) ->
        ...
        if dep_pkg = root_package && not (Ranges.contains ~compare_v:version_compare dep_ranges root_version) then (
          Log.info "Impossible root self-dependency: %s@%s depends on %s with incompatible range" 
            root_package (Version.to_string root_version) dep_pkg;
          has_impossible_root_dep := Some (dep_pkg, dep_ranges)
        ) else if Ranges.is_empty dep_ranges then (
          Log.info "Impossible root dependency: %s@%s depends on %s with empty range"
            root_package (Version.to_string root_version) dep_pkg;
          has_impossible_root_dep := Some (dep_pkg, dep_ranges)
        )
      ) deps;
      ...
      let rec solve_loop state iteration =
        if iteration > 1000 then (
          ...
          Log.debug "Iteration %d: pending has %d packages" iteration
            (List.length pending);

          (* Unit propagation first - check ALL packages with incompatibilities *)
          let all_packages_with_incompats =
            let pkgs = ref [] in
            HashMap.iter
              (fun pkg _ -> pkgs := pkg :: !pkgs)
              state.incompatibilities;
            !pkgs
          in
          Log.debug "Running unit propagation on %d packages"
            (List.length all_packages_with_incompats);
          match
            unit_propagation root_package root_version state
              all_packages_with_incompats
          with
          | Error terminal_incompat ->
              Log.error "Terminal incompatibility, no solution";
          ...
          | Ok propagated_state -> (
              ...
              | (pkg, ranges) :: _rest -> (
                  ...
                  | Ok None -> (
                        ...
                        Log.info
                          "📭 No version available for %s, adding no_versions incompatibility" pkg;
                        (* Get constraint from partial solution, or use full if undecided *)
                        let constraint_ranges = match Partial_solution.get_constraint propagated_state.solution pkg with
                          | `Constrained r -> r
                    ...
                    | Ok (Some ver) -> (
                        ...
                        | Ok (Provider.Available pkg_deps) ->
                            Log.debug "Processing %d dependencies for %s@%s" (List.length pkg_deps) pkg (Version.to_string ver);
                            ...
                            List.iter
                              (fun (dep_pkg, dep_ranges) ->
                                ...
                                if dep_pkg = pkg && not (Ranges.contains ~compare_v:version_compare dep_ranges ver) then (
                                  Log.info "Impossible self-dependency: %s@%s depends on %s with incompatible range" 
                                    pkg (Version.to_string ver) dep_pkg;
                                  has_impossible_dep := Some (dep_pkg, dep_ranges)
                                ) else if Ranges.is_empty dep_ranges then (
                                  Log.info "Impossible dependency: %s@%s depends on %s with empty range"
                                    pkg (Version.to_string ver) dep_pkg;
                                  has_impossible_dep := Some (dep_pkg, dep_ranges)
                                ))
                              pkg_deps;
                            ...
                            | None ->
                                ...
                                Log.info "✅ Adding decision: %s@%s" pkg (Version.to_string ver);
                                ...
                                let new_dep_graph = DependencyGraph.add_dependencies new_state.dependency_graph pkg ver dep_list in
                                ...
                                (* Continue - unit_propagation at loop top will handle conflicts *)
