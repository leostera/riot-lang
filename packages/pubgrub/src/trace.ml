...
let bound_to_json = function
  ...
  | Ranges.Excluded version ->
      json_obj [ ("kind", json_string "excluded"); ("version", version_to_json version); ]
...
let ranges_to_json = fun ranges ->
  ...
  |> List.map
    ~fn:(fun (start, finish) ->
      json_obj
        [ ("start", bound_to_json start); ("end", bound_to_json finish); ])
  |> json_array
...
let event_to_json = function
  ...
  | Solved { solution } ->
      json_obj [ ("type", json_string "solved"); ("solution", solution_to_json solution); ]

let to_json = fun t ->
  json_obj
    [
      ("events", events t
      |> List.map ~fn:event_to_json
      |> json_array);
    ]
