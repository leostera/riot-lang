...
let bound_to_json = function
  | Ranges.Unbounded ->
      json_obj
        [
          "kind", json_string "unbounded";
        ]
  | Ranges.Included version ->
      json_obj
        [
          "kind", json_string "included";
          "version", version_to_json version;
        ]
  | Ranges.Excluded version ->
      json_obj
        [
          "kind", json_string "excluded";
          "version", version_to_json version;
        ]

let ranges_to_json = fun ranges -> Ranges.segments ranges |> List.map ~fn:(
  fun (start, finish) ->
    json_obj
      [
        "start", bound_to_json start;
        "end", bound_to_json finish;
      ]
) |> json_array
...
let solution_to_json = fun solution -> solution |> List.map ~fn:(
  fun (package, version) ->
    json_obj
      [
...
let event_to_json = function
  ...
  | NoVersionAvailable { package; ranges } ->
      json_obj
        [
  ...
  | DerivedConstraint { package; incompatibility; changed } ->
      json_obj
        [
  ...
  | ConflictResolvedDifferent { package; previous_level; incompatibility } ->
      json_obj
        [
  ...
  | ConflictResolvedSame { package; incompatibility; cause; prior } ->
      json_obj
        [
  ...
  | LearnedIncompatibility { package; incompatibility } ->
      json_obj
        [
...
let to_json = fun t ->
  json_obj
    [
      "events", events t |> List.map ~fn:event_to_json |> json_array;
    ]
