...
let indent_text = fun spaces text ->
  let prefix = String.make ~len:spaces ~char:' ' in text |> String.split ~by:"\n" |> List.map ~fn:(
    fun line ->
...
let format_term = fun term ->
  ...
  if Ranges.is_empty ranges && is_positive then
    ...
    if Ranges.equal ~compare_v:version_compare ranges Ranges.full && is_positive then
      pkg ^ " (any version)"
    else
      if is_positive then
        pkg ^ " in " ^ format_ranges ranges
      else pkg ^ " not in " ^ format_ranges ranges
...
let format_terms = fun terms ->
  ...
  | [ t ] -> format_term t
  | _ ->
      let formatted = List.map terms ~fn:format_term in String.concat ", " formatted
...
let format_external_cause = function
  ...
  | Incompatibility.FromDependency (pkg, ver, dep_pkg, dep_ranges) -> pkg ^ "@" ^ version_to_string ver ^ " depends on " ^ dep_pkg ^ " in " ^ format_ranges dep_ranges
  | Incompatibility.Custom (pkg, ranges, msg) ->
      if Ranges.equal ~compare_v:version_compare ranges Ranges.full then
        "package " ^ pkg ^ ": " ^ msg
      else "package " ^ pkg ^ " in " ^ format_ranges ranges ^ ": " ^ msg
...
let explain_conflict = fun incompat ->
  ...
  let explanation = format_derivation_tree tree |> ensure_trailing_period in "Conflict:\n" ^ explanation ^ "\n\nTherefore, version solving failed."
