...
type dependencies = Available of dependency_list | Unavailable of string
...
type offline_entry = { version : version; deps : dependency_list }

type offline = {
  packages : (package, offline_entry list) Collections.HashMap.t;
}

let create_offline () = { packages = Collections.HashMap.create () }

let add_package provider pkg ver deps =
  ...
  | Some entries ->
      ignore
        (Collections.HashMap.insert provider.packages pkg (entry :: entries))

let version_compare a b =
  match Version.compare a b with Lt -> -1 | Eq -> 0 | Gt -> 1

let to_provider (offline : offline) : string t =
  let choose_version pkg ranges =
    ...
    | Some entries ->
        let matching =
          List.filter
            (fun entry ->
              Ranges.contains ~compare_v:version_compare ranges entry.version)
            entries
        in
        let sorted =
          List.sort (fun a b -> version_compare b.version a.version) matching
        in
        Ok (match sorted with [] -> None | entry :: _ -> Some entry.version)
  in
  let get_dependencies pkg ver =
    ...
    | Some entries -> (
        match
          List.find_opt
            (fun entry -> version_compare entry.version ver = 0)
            entries
        with
        | None ->
            Ok
