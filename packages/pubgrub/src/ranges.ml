...
let higher_than = fun v ->
  [
    Included v, Unbounded;
  ]

let strictly_higher_than = fun v ->
  [
    Excluded v, Unbounded;
  ]

let lower_than = fun v ->
  [
    Unbounded, Included v;
  ]

let strictly_lower_than = fun v ->
  [
    Unbounded, Excluded v;
  ]

let between = fun v1 v2 ->
  [
...
let valid_segment = fun ~compare_v ((start, end_)) ->
  ...
  | (Included v1, Excluded v2) | (Excluded v1, Included v2) | (Excluded v1, Excluded v2) -> (
    match compare_v v1 v2 with
    | Order.LT -> true
...
let end_before_start = fun ~compare_v end_ start ->
  ...
  | (Included left, Included right) | (Included left, Excluded right) | (Excluded left, Included right) | (Excluded left, Excluded right) -> (
...
let normalize = fun ~compare_v ranges ->
  let sorted = List.sort (List.filter ranges ~fn:(valid_segment ~compare_v)) ~compare:(
    fun (left_start, left_end) (right_start, right_end) ->
    ...
    | segment :: rest -> (
      ...
      | (current_start, current_end) :: acc_rest ->
          ...
          if end_before_start ~compare_v current_end next_start then
            ...
            let merged_end = max_by (compare_bound_end ~compare_v) current_end next_end in merge ((current_start, merged_end) :: acc_rest) rest
    )
  in
  merge [] sorted
...
let contains = fun ~compare_v ranges version -> List.any ranges ~fn:(within_bounds ~compare_v version)
...
let subset_of = fun ~compare_v r1 r2 -> normalize ~compare_v (intersection ~compare_v r1 r2) = normalize ~compare_v r1
...
let bound_to_string = fun ~to_string_v bound ->
  match bound with
  | Unbounded -> ""
  | Included value | Excluded value -> to_string_v value
