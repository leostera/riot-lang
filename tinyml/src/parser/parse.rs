...
///
/// The lexer is responsible for appending `TokenKind::Eof`; this function
/// expects that sentinel and parses module items until it reaches it.
pub fn parse_module(src: &str, tokens: Vec<Token>) -> Result<Module, ParseError> {
    Parser {
        src,
        tokens,
        pos: 0,
...
///
/// `src` provides diagnostic source text. `tokens` is consumed in-place with
/// `bump()`, and `pos` points at the current token.
struct Parser<'a> {
    src: &'a str,
    tokens: Vec<Token>,
    pos: usize,
...
impl Parser<'_> {
            ...
            TokenKind::Ident(_) | TokenKind::Constructor(_) => {
                ...
                match self.peek() {
                    TokenKind::LBrace => {
                        self.bump();
                        let fields = self.parse_record_type_fields()?;
                        let span = name.span().join(self.prev_span());
    ...
    fn parse_ident(&mut self) -> Result<Ident, ParseError> {
        ...
        match self.peek() {
            TokenKind::Dot if !matches!(self.peek_n(1), TokenKind::Star | TokenKind::LBrace) => {
                self.bump();
    ...
    /// Return whether the current token has the same variant as `kind`.
    fn at(&self, kind: &TokenKind) -> bool {
        std::mem::discriminant(self.peek()) == std::mem::discriminant(kind)
    }
    /// Borrow the current token kind.
    fn peek(&self) -> &TokenKind {
        &self.tokens[self.pos].kind
    }
    /// Borrow the token kind `n` tokens ahead, clamped to EOF.
    fn peek_n(&self, n: usize) -> &TokenKind {
        &self.tokens[(self.pos + n).min(self.tokens.len() - 1)].kind
    }
    /// Consume and return the current token kind.
    ///
    /// Use this when all successful routing branches consume the current token.
    /// Use `peek()` and `bump()` when consumption depends on the selected branch.
    fn next(&mut self) -> TokenKind {
        self.bump().kind
    }

    /// Consume and return the full current token.
    ///
    /// Moves the token out of the vector and replaces its old slot with an EOF
    /// placeholder. The parser cursor advances to the next token.
    ...
    fn make_err(&self, message: &str) -> ParseError {
        ParseError {
            ...
            span: self.tokens[self.pos].span.as_source_span(),
