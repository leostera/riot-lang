...
fn emit_interface_records_type_and_actor_message_shapes() -> FixtureResult {
    ...
    if !emit_interface.status.success() {
        return fail(format!(
            "expected rich emit interface to succeed:\n{}",
            String::from_utf8_lossy(&emit_interface.stderr)
        ));
    }
    ...
    for expected in [
        "module ActorMessageSignatureShapes",
        "type box<'a> = { value: 'a }",
        "fn boxed_worker() -> actor_id<box<i64>>",
        "fn mixed_worker() -> actor_id<'a> : forall 'a. (unit -> actor_id<'a>)",
        "fn string_worker() -> actor_id<String>",
        "riot_mod_ActorMessageSignatureShapes_boxed_worker",
    ] {
        if !interface_text.contains(expected) {
            return fail(format!(
                "rich emit interface missed `{expected}`:\n{interface_text}"
            ));
        }
    }
    for forbidden in ["== cst ==", "== typed ==", "== ir ==", "== llvm =="] {
        if interface_text.contains(forbidden) {
            return fail(format!(
                "rich emit interface included pipeline marker `{forbidden}`:\n{interface_text}"
            ));
        }
    }
...
fn interface_diff_reports_dependency_only_workspace_changes() -> FixtureResult {
    ...
    for expected in [
        ...
        "fn unused",
        "changed App dependencies:",
        "dependency Worker",
        "App types unchanged",
        "App exports unchanged",
        "impacted modules:",
        "App imports Worker",
    ] {
...
fn interface_diff_reports_missing_workspace_dependency_artifacts() -> FixtureResult {
    ...
    if !compile_before.status.success() {
        return fail(format!(
            "expected missing-dependency before workspace compile-lib to succeed:\n{}",
            String::from_utf8_lossy(&compile_before.stderr)
        ));
    }
    ...
    if !workspace_diff.status.success() {
        return fail(format!(
            "expected missing-dependency workspace interface-diff to succeed:\n{}",
            String::from_utf8_lossy(&workspace_diff.stderr)
        ));
    }
    ...
    for expected in [
        ...
        "App imports Worker but no Worker.rsig is present",
    ] {
        if !diff_text.contains(expected) {
            return fail(format!(
                "missing-dependency workspace interface diff missed `{expected}`:\n{diff_text}"
            ));
        }
    }
...
fn interface_diff_summarizes_workspace_review_changes() -> FixtureResult {
    ...
    std::fs::write(&after_worker, "fn answer() -> String { \"forty-one\" }\n")?;
    std::fs::write(
        &after_app,
        "use Worker\nfn answer() -> String { Worker.answer() }\n",
    )?;
    ...
    for (dir, app, worker) in [
        ...
        if !compile_lib.status.success() {
            return fail(format!(
                "expected workspace compile-lib to succeed for {}:\n{}",
                dir.display(),
                String::from_utf8_lossy(&compile_lib.stderr)
            ));
        }
    ...
    if !workspace_diff.status.success() {
        return fail(format!(
            "expected workspace interface-diff to succeed:\n{}",
            String::from_utf8_lossy(&workspace_diff.stderr)
        ));
    }
    ...
    for expected in [
        ...
        if !diff_text.contains(expected) {
            return fail(format!("workspace interface diff missed `{expected}`:\n{diff_text}"));
        }
    }
