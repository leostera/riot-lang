...
- [ ] [P1] Extend each zort-linked `.ml` fixture with expected output, expected trace shape, and a tiny benchmark signal. Why: the harness should drive runtime features, not just binary success; What we need: fixture-local assertions for output, event/memprof/GC traces where relevant, and small benchmark reporting in `e2e-ml`; Cannot do yet: treat compiler/runtime integration as a repeatable acceptance harness instead of manual spot checks.
