...
- [ ] [P1] Register compiler-emitted startup metadata into `zort` compiler-compat state instead of only observing it. Why: `min_pure_startup` now proves the emitted tables are present, but they still do not participate in runtime services; What we need: code-fragment registration, frame-table ownership, and `<unit>.gc_roots` hookup to runtime root providers on the locked `aarch64-apple-darwin` path; Cannot do yet: trust backtraces, exception unwinding, or compiler-emitted allocation paths that depend on real metadata ingestion.
