...
- [ ] The next concrete slice is deleting the remaining member-level source-gap ownership helpers in `krasny/lower.ml`, especially the trailing-docstring scanners that still parse raw source to decide constructor/field/body ownership now that top-level, nested, and grouped-type paths are all CST-driven
