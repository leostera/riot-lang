...
- [ ] The next concrete slice is deleting the remaining source-gap ownership logic from grouped `type ... and ...` rendering in `krasny/lower.ml`, so declaration/member ownership no longer depends on `parse_trivia_between_offsets` there either
...
- [ ] The next concrete slice is removing the remaining declaration-local source-gap ownership in grouped type rendering, now that the top-level and nested item-stream joiners are layout-only
