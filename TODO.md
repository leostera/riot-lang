...
- stale rollout-era dead helpers continue to shrink during audits; unused locals such as `unwrap_parenthesized_expression`, `flatten_top_level_expression_item`, `render_structure_expression_run`, and the dead top-level `doc_with_expression_attributes` duplicate are gone from `lower.ml`.
