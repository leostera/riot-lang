...
- [ ] Current state: nested `CstBuilder.structure_items_of_module_expression` / `signature_items_of_module_type` streams now include terminal standalone comments/docstrings that live on the closing `end` token's `leading_trivia`, so nested item consumers no longer need raw-gap recovery just to see final body trivia
- [ ] The next concrete slice is deleting the remaining nested-body trivia repair heuristics in `krasny/lower.ml`, especially the `parse_between` paths that still recover whitespace/comment layout from raw nested `sig ... end` / `struct ... end` gaps instead of trusting the now-terminal-complete nested CST item streams
...
- [ ] The next concrete slice is deleting top-level trivia archaeology from `krasny/lower.ml`, now that top-level ownership and doc kind are both explicit in the CST
