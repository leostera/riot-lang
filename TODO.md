...
- `Syn.Cst.ModuleDeclaration` is still shared across structure and signature contexts; its broad optional `module_type` / `module_expression` fields are now explicitly documented as context-sensitive CST debt instead of pretending to be globally valid-by-construction.
