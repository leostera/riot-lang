...
- module-pack, assert, lazy, while, for, method-call, new, object, object-override, instance-variable-assign, typed, polymorphic, and coerce expressions now lower structurally; plain object expressions support self patterns plus method/value/inherit/initializer members and member attributes, while expression extensions and object extension members still fail explicitly instead of falling through raw fallback.
...
- [ ] Decide which missing structural facts belong in `syn` so `krasny` can stop guessing
  ...
  - explicit top-level and signature item boundary metadata, so `krasny` can stop recomputing item order and join spans via `span_of_syntax_node_*`, `type_declaration_owned_trivia_end`, and per-item span sorting
...
- [ ] Remove remaining red-tree token/span archaeology from `packages/krasny/src/lower.ml`
  - `render_structure_top_level_items` and `render_signature_top_level_items` still derive joins from syntax-node spans and boundary reconstruction even though they now trust ordered CST streams for item order
