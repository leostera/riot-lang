...
- [ ] The next concrete slice is removing explicit parser trivia consumption from the remaining branch-sensitive declaration/type ambiguity helpers that still speculatively advance and rewind, especially `parse_module_type_expr`'s `module type of` probe and any surviving declaration-local module-type lookahead, while keeping offsets and diagnostics stable
