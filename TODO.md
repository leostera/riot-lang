...
- [ ] The next concrete slice is deleting the remaining dead source-gap ownership scaffolding from `krasny/lower.ml`, especially the always-on `uses_source_file_item_stream` / `parse_between` branches in structure rendering, so `lower.ml` is left with layout-only decisions
