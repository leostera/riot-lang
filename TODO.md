...
- optional parameters now split into explicit plain-vs-defaulted CST forms, so typed binding patterns and default values survive structurally without `has_default` / `default_value` impossible states, and `krasny` renders parameters from CST shape instead of `Source.source_of_parameter`.
