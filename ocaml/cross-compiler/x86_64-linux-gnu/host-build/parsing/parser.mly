...
(* Riot instrumentation: inject reduction counter before function applications *)
let riot_instrument_apply ~loc:_ func args =
  (* Temporarily disabled to avoid circular dependency during stdlib build
     TODO: Enable conditionally based on compilation context *)
  Pexp_apply (func, args)

(* Riot instrumentation: inject reduction counter at each loop iteration *)
let riot_instrument_loop ~loc:_ body =
  (* Temporarily disabled to avoid circular dependency during stdlib build
     TODO: Enable conditionally based on compilation context *)
  body

let mkinfix arg1 op arg2 =
  let loc = (arg1.pexp_loc.Location.loc_start, arg1.pexp_loc.Location.loc_end) in
  riot_instrument_apply ~loc op [Nolabel, arg1; Nolabel, arg2]
...
let mkuminus ~sloc ~oploc name arg =
  ...
  | _ ->
      riot_instrument_apply ~loc:oploc (mkoperator ~loc:oploc ("~" ^ name)) [Nolabel, arg]
...
let mk_indexop_expr array_indexing_operator ~loc
  ...
  mkexp ~loc (riot_instrument_apply ~loc (ghexp ~loc (Pexp_ident fn)) args)
...
%inline fun_expr_attrs:
  ...
  | WHILE ext_attributes seq_expr do_done_expr
      { Pexp_while($3, riot_instrument_loop ~loc:$sloc $4), $2 }
  | FOR ext_attributes pattern EQUAL seq_expr direction_flag seq_expr
    do_done_expr
      { Pexp_for($3, $5, $7, $6, riot_instrument_loop ~loc:$sloc $8), $2 }
...
%inline expr_:
  | simple_expr nonempty_llist(labeled_simple_expr)
      { riot_instrument_apply ~loc:$sloc $1 $2 }
...
%inline simple_expr_:
  ...
  | op(PREFIXOP) simple_expr
      { riot_instrument_apply ~loc:$sloc $1 [Nolabel,$2] }
  | op(BANG {"!"}) simple_expr
      { riot_instrument_apply ~loc:$sloc $1 [Nolabel,$2] }
