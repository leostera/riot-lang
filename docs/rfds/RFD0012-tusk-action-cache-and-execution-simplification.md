...
cache unit of the `tusk` build system while explicitly keeping a package-level
planning cache. `tusk-planner` should memoize package plans by package input
hash, `tusk-executor` should schedule and reuse action nodes under a single
workspace-level concurrency budget, and `tusk-store` should atomically persist
...
- `tusk` should usually hit the package-level plan cache
...
```mermaid
flowchart TD
  A[tusk build] --> B[compute package input hash]
  B --> C{plan bundle hit?}
...
- `tusk-planner` decides graph shape and cacheable plan identity
...
If this RFD is not implemented, `tusk` will keep carrying package-level and
...
`tusk` already has enough pieces to move in that direction.
