...
This RFD proposes collapsing `tusk` back into a one-shot command-line build tool. `tusk` should boot, evaluate the requested command, perform the build or query work, and then exit. The current daemon-oriented architecture, including background server management and the `tusk-mcp` package, adds operational and conceptual overhead that is not justified by the current value it provides. The proposal keeps the useful internals, particularly actor-based build execution, while removing the long-lived server, daemon lifecycle, and MCP-facing layer from the core.

## Motivation
[motivation]: #motivation

The current `tusk` architecture has accumulated multiple layers between a contributor invoking `tusk` and the build work that actually happens:
...
The result is that simple workflows, such as `tusk build`, `tusk run`, `tusk bench`, and read-only workspace queries, now pay for transport and process management complexity that does not materially improve the common case.

This RFD is motivated by three concrete goals:

1. Make `tusk` easier to reason about.
...
- A contributor runs `tusk build` and expects the command to perform the build and exit, without background server reuse.
- A contributor runs `tusk run foo` and expects `tusk` to resolve, build, run, and exit in a single flow.
...
After this change, contributors should think about `tusk` as a normal build tool again.

`tusk` commands will work as one-shot operations:
...
There is no persistent daemon lifecycle in the user model. Contributors should not need `tusk server start`, `tusk server stop`, `tusk server kill`, or daemon status checks for normal operation.
...
The `tusk-mcp` package is removed from the core workspace. If agent integrations are desired, they should live in a separate integration layer that depends on `tusk`, not the other way around.
...
```mermaid
flowchart TD
  A[tusk command] --> B[scan workspace and init toolchain]
...
The current `Server_manager.ensure_running` should be replaced by a one-shot session constructor inside `tusk-cli`.
...
- move the local request helpers into `tusk-cli`
...
- delete `tusk-client`, `tusk rpc`, and the TCP/JSON-RPC bridge from the core path

This keeps CLI churn small while removing the daemon and network coupling.

`jsonrpc` and the TCP server may remain temporarily only if needed for migration compatibility, but they should no longer sit on the primary path for normal `tusk` commands.

### 3. Remove MCP from the core

`packages/tusk-mcp` should be deleted.

That removal includes:

- deleting the `tusk-mcp` package
- removing the `tusk mcp` CLI subcommand
- removing shell completion entries for `mcp`
- removing manifest dependencies that exist only for MCP support

The core `tusk` workspace should not need the `mcp` package or an MCP server implementation to build and run.
...
`tusk server foreground` may remain temporarily as a development/debug escape hatch if it materially helps migration, but it should not remain part of the normal architecture.
...
- Inline everything directly into `tusk-cli`.
...
- Keep `tusk-mcp` in-tree as an optional package.
  That still makes the core workspace carry integration complexity that is not required for the build tool itself.

If this RFD is not implemented, `tusk` remains harder to reason about than it needs to be, and future work will continue paying the cost of the current layering.
...
Within this repository, several subsystems already use actors as internal coordination mechanisms without exposing daemon lifecycle as part of the primary user model. That is the pattern this RFD wants for `tusk`: keep actors where they help execution, remove them as an operational burden.

The current `tusk` architecture itself is also prior art in what to avoid here. It demonstrates that a daemon-plus-client-plus-MCP stack can be built, but it also demonstrates the resulting maintenance cost and user-facing complexity.
...
The likely long-term direction is a `tusk` architecture with a very small CLI layer, a local execution/session layer, and focused planner/executor/toolchain packages underneath it.
