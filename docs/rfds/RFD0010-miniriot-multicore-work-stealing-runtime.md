...
- Feature Name: `miniriot_multicore_work_stealing_runtime`
...
After this change, contributors should think about `miniriot` as a runtime, not a single scheduler.
...
That domain-local state should be introduced at the `kernel` boundary rather than by adding raw `Stdlib.Domain` usage ad hoc inside `miniriot`.
...
The existing note in `packages/miniriot/docs/intrusive-mpsc-node-based-queue.html` is a good starting point for the `main_mailbox`.
...
Single-core `miniriot` can do this unsafely because there are no concurrent senders:
...
1. single-worker refactor parity: the refactored runtime still passes today's `miniriot` behavior tests
...
- Keep `miniriot` single-core and tell higher layers to use external thread pools.
...
- The 1024cores intrusive MPSC queue design is already referenced in `packages/miniriot/docs/intrusive-mpsc-node-based-queue.html` and fits the mailbox inject path well.
...
Riot should borrow the structure, not blindly copy any one runtime. `miniriot` needs a design that fits its own process, timer, and effect machinery.
