...
- Feature Name: `miniriot_pinned_and_blocking_spawn`
...
This RFD proposes two follow-up spawn modes on top of the multicore `miniriot` runtime:
...
Once `miniriot` has normal work-stealing schedulers, the default `spawn` should stay optimized for ordinary actor workloads:
...
After this change, `miniriot` has three actor placement classes:
...
```ocaml
let ui_cache_owner =
  Miniriot.spawn_pinned (fun () -> loop ())
```
...
```ocaml
let tui =
  Miniriot.spawn_blocked (fun () -> run_tui_loop ())
```
