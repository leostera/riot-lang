...
This RFD proposes turning `tusk-fix` into the first syntax-aware linting and
auto-fix tool in the Riot stack, exposed primarily as `tusk fix`. The system
...
1. A contributor runs `tusk fix --check` and gets actionable diagnostics about direct
   `Stdlib` or `Unix` usage in the wrong package.
2. A contributor runs `tusk fix` and the obvious safe rewrites are
...
Contributors should think about `tusk fix` as a parser-backed lint and rewrite
...
```mermaid
flowchart TD
  A[tusk fix] --> B[scan workspace files]
...
### Relationship to `tusk fmt`

`tusk fix` is not `tusk fmt`.

`tusk fix` should perform targeted, rule-driven rewrites.
`tusk fmt` would be a whole-program formatting tool with a much larger surface
...
The repository already contains a `packages/tusk-fix` package with the right
high-level pieces:
...
- Parse diagnostics are converted into `tusk-fix` diagnostics.
...
That makes `tusk-fix` the right place to evolve the feature rather than writing
`tusk fix` from scratch somewhere else.
...
- **CLI integration**: exposes the feature through `tusk`
...
The first release of `tusk fix` should only execute safe default-fix rules.
...
The first version of `tusk fix` should only apply fixes that are:
...
The primary entrypoint should live in `tusk-cli`.
...
If desired later, `tusk-fix` can keep a standalone binary for development, but
`tusk fix` should be the primary interface.
...
- **Build `tusk fmt` first**
  A formatter has much broader output-surface risk and a larger design space.
  `tusk fix` can deliver useful value sooner.

- **Keep `tusk-fix` as a standalone side tool**
  That makes adoption weaker and splits the user story away from `tusk`.
...
If Riot does not build `tusk fix`, then many of its conventions remain social
...
- Should `tusk-fix` keep its own binary in the long term, or become only a `tusk` subcommand?
- Should rule configuration live in `tusk.toml`, or should the first version stay convention-driven with almost no configuration?
...
Once `tusk fix` exists as a stable syntax-aware rewrite tool, several follow-on
features become much easier:

- `tusk fmt`, built on the same parser stack but with a different output model
- codemod-style migrations for Riot package APIs
- editor-integrated quick fixes powered by the same rule engine
- selective rule packs for package domains such as `std`, `suri`, or `tusk`
