...
- Feature Name: `tusk_foreign_dependency_model_snapshot`
...
This RFD documents the current foreign dependency model in `tusk`. A foreign
dependency is a package-local build hook declared in `tusk.toml` under
`[foreign-dependencies.<name>]`. `tusk-model` parses that declaration into a
structured `Package.foreign_dependency`, `tusk-planner` injects one
`BuildForeignDependency` action node per foreign dependency into the package
action graph, and `tusk-executor` runs the declared command directly in the
...
- `tusk-model` defines the manifest shape and hashes foreign inputs
- `tusk-planner` injects foreign build actions and threads foreign outputs into
  linker inputs
- `tusk-executor` runs the foreign command and validates the declared outputs

Some of that behavior is mentioned in broader `tusk` snapshots, but there is no
...
ask `tusk` to run an external build command before linking the package's OCaml
artifacts."
...
In other words, the current model is a practical bridge from `tusk` into an
...
If a package declares two foreign dependencies, `tusk` does not currently try to
...
- foreign inputs are not copied into the `tusk` sandbox
- foreign outputs are not modeled as a standalone immutable artifact family
- `clean_cmd` and `test_cmd` are not part of standard `tusk clean` or `tusk test`
...
`packages/tusk-model/src/package.ml` parses foreign dependencies from either:
...
manifest. Instead, `tusk-model` scans the foreign dependency directory and
...
`packages/tusk-planner/src/package_planner.ml` injects foreign dependency build
...
`packages/tusk-planner/src/action_graph.ml` threads foreign outputs into link
...
`packages/tusk-executor/src/action_executor.ml` treats
...
package outputs into `tusk-store`.
...
- `build_cmd` is used
- `clean_cmd` is not wired into `tusk clean`
- `test_cmd` is not wired into `tusk test`
...
- fine-grained action dependencies instead of "all package actions depend on all
  foreign nodes"
- dedicated foreign artifact caching in `tusk-store`
