...
- Feature Name: `miniriot_runtime_snapshot`
...
This RFD documents the current architecture of `miniriot`. It captures the single-core actor runtime as it exists today: process lifecycle, scheduler behavior, mailbox delivery, timers, cooperative effect handling, and the public runtime surface that `std` builds on.

## Motivation
[motivation]: #motivation

`miniriot` is one of the pillars of the Riot stack, but its current design lives mostly in code.
...
`miniriot` is a minimal single-core actor runtime.
...
The public surface exposed by `Miniriot` is intentionally compact:
...
Higher-level behavior lives above this layer. `miniriot` owns runtime mechanics, not application policy.

### Runtime structure

```mermaid
flowchart TD
  A[Miniriot.run] --> B[Scheduler.create]
...
`miniriot` depends only on `kernel`.
...
- `miniriot` turns those primitives into a runtime with processes, mailboxes, and scheduling semantics
- `std` then turns `miniriot` into a more ergonomic application-facing surface

## 2. Public module structure

The main public entrypoint is `packages/miniriot/src/miniriot.mli`.
...
The implementation in `packages/miniriot/src/miniriot.ml` is intentionally thin. It mostly forwards to `Scheduler`, `Effects`, and `Process`.

## 3. Scheduler state

The scheduler state in `packages/miniriot/src/scheduler.ml` currently contains:
...
`packages/miniriot/src/process.mli` exposes the process lifecycle model. A process currently owns:
...
`miniriot` uses effect-driven cooperative control flow.
...
`packages/miniriot/src/runtime.ml` exposes:
...
`miniriot` differs in a few important ways:
