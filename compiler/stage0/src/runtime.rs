...
impl RuntimeBuilder {
    ...
    pub(crate) fn build(&self) -> miette::Result<Utf8PathBuf> {
        let repo = Self::find_repo_root()?;
        self.build_for_repo(&repo)
    }

    fn find_repo_root() -> miette::Result<Utf8PathBuf> {
        let current_dir = std::env::current_dir()
            .into_diagnostic()
            .wrap_err("failed to read current directory")?;
        let mut dir = Utf8PathBuf::from_path_buf(current_dir).map_err(|path| {
            miette::miette!("current directory is not utf-8: {}", path.display())
        })?;
    ...
    fn build_for_repo(&self, repo: &Utf8Path) -> miette::Result<Utf8PathBuf> {
        let lib = repo.join("compiler/rt/target/release/libriot_rt.a");
        if self.runtime_library_is_current(repo, &lib)? {
            return Ok(lib);
        }
    ...
    fn runtime_library_is_current(&self, repo: &Utf8Path, lib: &Utf8Path) -> miette::Result<bool> {
        let lib_modified = match Self::modified_time(lib)? {
            Some(time) => time,
        ...
        Ok(!Self::directory_has_file_newer_than(
    ...
    fn directory_has_file_newer_than(dir: &Utf8Path, cutoff: SystemTime) -> miette::Result<bool> {
            ...
            .wrap_err_with(|| format!("failed to read {}", dir))?
        {
            ...
            if file_type.is_dir() {
                if Self::directory_has_file_newer_than(&path, cutoff)? {
                    return Ok(true);
                }
            } else if file_type.is_file()
                && let Some(time) = Self::modified_time(&path)?
                && time > cutoff
            {
                return Ok(true);
            }
