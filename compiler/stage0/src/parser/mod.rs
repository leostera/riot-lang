    ...
    fn parse_paren_or_tuple_or_unit(&mut self) -> Result<AstExpr, ParseError> {
        ...
        if self.match_kind(TokenKind::Comma).is_none() {
            if !self.at(TokenKind::RParen) {
                return Err(ParseError {
                    span: self.current().span,
                    message: "expected `,` between tuple items".to_owned(),
...
mod tests {
    ...
    #[test]
    fn source_parser_preserves_lexer_spans_after_comments() {
        let source = "// leading scanner trivia\nfn main() { /* parser trivia */ let value = add_one(41); dbg(value) }\n";
        let program = SourceParser::new()
            .parse(Utf8Path::new("pipeline.ml"), source)
            .unwrap();
        ...
        assert!(matches!(function.body.tail, Some(AstExpr::Call { .. })));
    }
