...
let compilation_of_frontend_and_backend = fun (frontend: Frontend_pipeline.t) backend_result ->
  ...
  |> List.map (fun diagnostic -> Raml_compilation.Parse diagnostic))
  @ (frontend.typing.lowering_diagnostics
  |> List.map (fun diagnostic -> Raml_compilation.Lowering diagnostic))
  @ (frontend.typing.typing_diagnostics
  |> List.map (fun diagnostic -> Raml_compilation.Typing diagnostic)) in
