...
let functor_id loc = Location.mkloc Longident.(Lapply (Lident "F", Lident "X")) loc

let complex_record loc = H.Pat.record ~loc [ (functor_id loc, H.Pat.any ~loc ()) ] Asttypes.Closed
...
let expr mapper e =
  ...
  | Pexp_extension ({ txt="no_args"; loc }, PStr [ { pstr_desc=Pstr_eval (e, _); _ } ]) -> empty_apply
    loc
    e
...
let pat mapper p =
  ...
  | Ppat_extension ({ txt="record_with_functor_fields"; loc }, _) -> complex_record loc
  | _ -> super.M.pat mapper p

let typ mapper ty =
  match ty.ptyp_desc with
  | Ptyp_extension ({ txt="empty_poly_binder"; loc }, _) -> empty_poly_binder loc
