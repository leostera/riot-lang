...
let parse_unit_id = fun json ->
  ...
  let* relpath =
    Result.map_err
      ...
      ~fn:(fun _ -> format Format.[ str scope; str ".relpath must be a valid path" ])
  in
...
let parse_constant = fun json ->
  ...
  | "bool" ->
      ...
      in
      Result.map value ~fn:(fun value -> Core_ir.Constant.Bool value)
  | "int" ->
      let* value = field scope "value" json in
      let value =
      ...
      in
      Result.map value ~fn:(fun value -> Core_ir.Constant.Int value)
  | "float" ->
      let* value = field scope "value" json in
      let value =
      ...
      in
      Result.map value ~fn:(fun value -> Core_ir.Constant.Float value)
  | "char" ->
...
let parse_binding_id = fun json ->
  ...
  | "local" ->
      ...
      let name = Core_ir.Surface_path.from_string name in
...
let parse_entity_id = fun json ->
  match Json.get_string json with
  | Some value -> Ok (Core_ir.Entity_id.from_surface_path (Core_ir.Surface_path.from_string value))
  | None ->
