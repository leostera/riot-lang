...
let diagnostics_to_list = fun (diagnostics: Typ.Diagnostics.t) ->
  vector_to_list diagnostics.items

let diagnostic_to_json = Typ.Diagnostics.Diagnostic.to_json

let source_slice = fun source ->
  IO.IoVec.IoSlice.from_string source
  |> Result.expect ~msg:"failed to create raml source slice"
...
let typing_state_of_ast_failure = fun parse_result lowering_diagnostics ->
  let parse_diagnostics =
    Syn.Parser.(parse_result.diagnostics)
    |> vector_to_list
  in
...
let typing_state_of_report = fun (report: Typ.Infer.infer_result) ~parse_diagnostics ->
  let typing_diagnostics = diagnostics_to_list report.diagnostics in
  let parse_issues = parse_diagnostics
  |> List.map ~fn:Syn.Diagnostic.to_json
  |> List.map ~fn:(wrap_issue "parse") in
  let lowering_diagnostics = [] in
  let lowering_issues = [] in
  let typing_issues = typing_diagnostics
  |> List.map ~fn:diagnostic_to_json
  |> List.map ~fn:(wrap_issue "typing") in
  let has_errors = not (List.is_empty parse_diagnostics) || not (List.is_empty typing_diagnostics) in
    ...
    json =
      Json.obj
        [ (
            ...
            "typing_diagnostics",
            Json.array (List.map typing_diagnostics ~fn:diagnostic_to_json)
          ); ("exports", Json.array []); ];
    semantic_tree =
      if is_complete then
        Some ()
...
let typing_state = fun ~config:_ ~filename ~source ->
  ...
  match Typ.Ast.from_parse_result ~source:source_value parse_result with
  | Ok ast ->
      let report = Typ.Infer.check ast in
      typing_state_of_report
        report
        ~parse_diagnostics
  | Error lowering_diagnostics -> typing_state_of_ast_failure parse_result lowering_diagnostics
