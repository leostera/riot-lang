...
module Modules = struct
  ...
  let compare_kind = fun left right ->
    ...
    | (Runtime, Runtime) -> Order.EQ
    | (Relative_unit, Runtime) -> Order.LT
    | (Runtime, Relative_unit) -> Order.GT

  let compare = fun left right ->
    let by_kind = compare_kind left.kind right.kind in
    match by_kind with
    | Order.EQ -> String.compare left.unit_name right.unit_name
    | order -> order

  let equal = fun left right -> compare left right = Order.EQ
