...
let imported_function_bindings = fun imports ->
  ...
  List.for_each
    (List.enumerate imports)
    ~fn:(fun (function_index, (import_: Types.Import.t)) ->
...
let rec compile_expr = fun ~layout ~imports ~env expr ->
  ...
  | Types.Expr.Let let_ -> begin
      ...
      | Core.Rec_flag.Nonrecursive ->
          ...
          List.for_each
            let_.bindings
            ~fn:(fun (binding: Types.Expr.binding) ->
...
let compile_start = fun ~layout ~imports (linked_program: Artifacts.Linked_program.t) ->
  ...
  |> List.fold_left
    ~init:0
    ~fn:(fun count (object_: Artifacts.Object.t) -> count + List.length object_.program.init) in
  ...
  List.for_each
    ...
    ~fn:(fun (object_: Artifacts.Object.t) ->
      List.for_each
