...
let collapse_adjacent_labels = fun instructions ->
  let rec loop current_label aliases acc instructions =
    ...
    | Instruction.Label name :: rest -> (
        ...
        | Some canonical ->
            loop (Some canonical) ((name, canonical) :: aliases) acc rest
        | None ->
            loop (Some name) aliases (Instruction.Label name :: acc) rest
      )
    | instruction :: rest ->
...
let rewrite_targets = fun aliases instructions ->
  let rewrite = fun instruction ->
    match instruction with
    | Instruction.Branch_if_zero branch -> Instruction.Branch_if_zero {
      branch with
      target = resolve_label aliases branch.target
    }
...
let remove_unreachable = fun instructions ->
  let rec loop reachable acc instructions =
    match instructions with
    | [] -> List.rev acc
    | Instruction.Label _ as instruction :: rest ->
        loop true (instruction :: acc) rest
    | instruction :: rest ->
...
let remove_fallthrough_jumps = fun instructions ->
  let rec loop acc instructions =
    match instructions with
    | Instruction.Jump target :: Instruction.Label next :: rest when String.equal target next ->
        loop (Instruction.Label next :: acc) rest
    | instruction :: rest ->
        loop (instruction :: acc) rest
    | [] ->
        List.rev acc
  in
  loop [] instructions
...
let program = fun (program: Program.t) ->
  {
    program with
    procedures = List.map schedule_procedure program.procedures;
  }
