...
and emit_function = fun ~level env function_ ->
  ...
  format
    Format.[
      str "(function(";
      str (String.concat ", " (List.map (fun (binder: Types.Binder.t) -> binder.name) function_.params));
      str ") {";
      str body;
      str "})";
    ]
...
let emit_import = fun env (import: Types.Import.t) ->
  let env =
    ...
    |> (fun env ->
         ...
         | None -> env
         | Some binder -> remember_binding env binder)
    |> (fun env -> List.fold_left (fun env (named: Types.Import.named) -> remember_binding env named.local) env import.names)
  in
  let line =
    match import.namespace with
    ...
    | None ->
        ...
        let bindings =
          ...
          | (Some default, Some named) -> Some (format Format.[ str default.name; str ", "; str named ])
        in
        match bindings with
        | None -> format
