...
let lower = fun primitive arguments ->
  ...
  | (Core.Primitive.Less_or_equal, [left;right]) -> Intrinsics.binary Jir.Operator.Less_or_equal left right
  | (Core.Primitive.Greater_than, [left;right]) -> Intrinsics.binary Jir.Operator.Greater_than left right
  | (Core.Primitive.Greater_or_equal, [left;right]) ->
      Intrinsics.binary Jir.Operator.Greater_or_equal left right
